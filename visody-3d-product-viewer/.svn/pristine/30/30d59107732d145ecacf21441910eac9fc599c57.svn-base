...
if (!function_exists('visody_get_viewer_html')) {
	 ...
	 * @return string
	 */
	function visody_get_viewer_html($product_id, $poster_id = '', $loop_item = false)
	{
		...
		$threed_viewer .= sprintf( 
			'<model-viewer src="%1$s" data-base_model_url="%1$s" interaction-prompt="none" auto-rotate-delay="0" camera-controls touch-action="pan-y" %2$s>',
			esc_url( $model_src ),
			$viewer_attributes
		);
...
if (!function_exists('visody_get_viewer_attributes')) {
	...
	function visody_get_viewer_attributes($template_id, $loop_item = false)
	{
		$viewer_attributes = '';
		if ( ! $template_id ) {
			$viewer_attributes .= ' ar camera-controls';
		...
		$env_image_url = '';
		if (get_post_meta($template_id, 'environment_image', true)) {
			$env_image_id = get_post_meta($template_id, 'environment_image', true);
			$env_image_url = wp_get_attachment_url($env_image_id, 'full');
		} else if (get_post_meta($template_id, 'environment_image_url', true)) {
			$env_image_url = sanitize_url(get_post_meta($template_id, 'environment_image_url', true));
		}
		...
		if ( '' !== $intensity) {
			$viewer_attributes .= ' shadow-intensity="' . esc_attr( $intensity ) . '"';
		}

		$softness = get_post_meta($template_id, 'shadow_softness', true);
		if ( '' !== $softness) {
			$viewer_attributes .= ' shadow-softness="' . esc_attr( $softness ) . '"';
		}
		...
		} else if (get_post_meta($template_id, 'set_camera_min_zoom', true)) {
			$minzoom = 'auto';
			if (get_post_meta( $template_id, 'set_camera_min_zoom', true ) ) {
				$minzoom = get_post_meta($template_id, 'camera_min_zoom', true);
			}
