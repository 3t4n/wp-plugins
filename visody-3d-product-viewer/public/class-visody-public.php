...
class Visody_Public
{
	...
	public function visody_load_shortcode_3d_model_viewer($attributes)
	{
		...
		if (isset($attributes['id'])) {
			...
			if ('yes' !== get_post_meta($viewer_id, 'visody_inline_shortcode_viewer', true)) {
				if (is_user_logged_in()) {
					return __('Viewer shortcode setting is not active. Please check option in your product to display viewer here.', 'visody');
				}
				return;
			}
		}
		...
		$model_html = visody_get_viewer_html($viewer_id, $poster_id);
		$panel_html = '';
		if (function_exists('visody_get_qrcode_panel_html')) {
