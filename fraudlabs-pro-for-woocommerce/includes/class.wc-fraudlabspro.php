...
class WC_FraudLabs_Pro {
	...
	public function validate_order($oldStatus = '', $newStatus = '') {
		...
		$queries = [
			...
			'first_name'					=> ( WC()->version < '2.7.0' ) ? $this->order->billing_first_name : $this->order->get_billing_first_name(),
			'last_name'						=> ( WC()->version < '2.7.0' ) ? $this->order->billing_last_name : $this->order->get_billing_last_name(),
			'bill_addr'						=> ( WC()->version < '2.7.0' ) ? trim( $this->order->billing_address_1 . ' ' . $this->order->billing_address_2 ) : trim( $this->order->get_billing_address_1() . ' ' . $this->order->get_billing_address_2() ),
			'bill_city'						=> ( WC()->version < '2.7.0' ) ? $this->order->billing_city : $this->order->get_billing_city(),
			'bill_state'					=> ( WC()->version < '2.7.0' ) ? $this->order->billing_state : $this->order->get_billing_state(),
			'bill_zip_code'					=> ( WC()->version < '2.7.0' ) ? $this->order->billing_postcode : $this->order->get_billing_postcode(),
			'bill_country'					=> $bill_country,
			'user_phone'					=> ( WC()->version < '2.7.0' ) ? $this->order->billing_phone : $this->order->get_billing_phone(),
			'ship_first_name'				=> ( WC()->version < '2.7.0' ) ? $this->order->shipping_first_name : $this->order->get_shipping_first_name(),
			'ship_last_name'				=> ( WC()->version < '2.7.0' ) ? $this->order->shipping_last_name : $this->order->get_shipping_last_name(),
			'ship_addr'						=> ( WC()->version < '2.7.0' ) ? trim( $this->order->shipping_address_1 . ' ' . $this->order->shipping_address_2 ) : trim( $this->order->get_shipping_address_1() . ' ' . $this->order->get_shipping_address_2() ),
			'ship_city'						=> ( WC()->version < '2.7.0' ) ? $this->order->shipping_city : $this->order->get_shipping_city(),
			'ship_state'					=> ( WC()->version < '2.7.0' ) ? $this->order->shipping_state : $this->order->get_shipping_state(),
			'ship_zip_code'					=> ( WC()->version < '2.7.0' ) ? $this->order->shipping_postcode : $this->order->get_shipping_postcode(),
			'ship_country'					=> $ship_country,
			'email'							=> ( WC()->version < '2.7.0' ) ? $this->order->billing_email : $this->order->get_billing_email(),
			'email_domain'					=> ( WC()->version < '2.7.0' ) ? substr( $this->order->billing_email, strpos( $this->order->billing_email, '@' ) + 1 ) : substr( $this->order->get_billing_email(), strpos( $this->order->get_billing_email(), '@' ) + 1 ),
			'email_hash'					=> ( WC()->version < '2.7.0' ) ? $this->hash_string( $this->order->billing_email ) : $this->hash_string( $this->order->get_billing_email() ),
			'user_order_id'					=> $this->order->get_order_number(),
			'amount'						=> $this->order->get_total(),
			'quantity'						=> $qty,
			'currency'						=> ( WC()->version < '2.7.0' ) ? $this->order->get_order_currency() : $this->order->get_currency(),
		...
		if ( ( $this->notification_approve == 'yes' && $response->fraudlabspro_status == 'APPROVE' ) || ( $this->notification_review == 'yes' && $response->fraudlabspro_status == 'REVIEW' ) || ( $this->notification_reject == 'yes' && $response->fraudlabspro_status == 'REJECT' ) ) {
			...
			$request = wp_remote_get( 'https://api.fraudlabspro.com/v1/zaptrigger?' . http_build_query( array(
				'key'		=> $this->api_key,
				'format'	=> 'json'
			) ) );
	...
	public function settings_page() {
		...
		$request = wp_remote_get( 'https://api.fraudlabspro.com/v1/plan?' . http_build_query( array(
			'key'		=> $this->api_key,
			'format'	=> 'json'
		) ) );
		...
		if ( ! is_wp_error( $request ) ) {
			...
			if ( is_object( $response ) ) {
				...
				$plan_upgrade = (($plan_name != 'FraudLabs Pro Enterprise') ? '<input type="button" name="button" id="button-upgrade" class="button button-primary" value="Upgrade" style="margin-left:20px;" />' : '');
		...
		$api_key = ( isset( $_POST['api_key'] ) ) ? sanitize_text_field($_POST['api_key']) : $this->get_setting( 'api_key' );
		...
		$wc_fraudlabspro_debug_log_path = ( isset( $_POST['wc_fraudlabspro_debug_log_path'] ) ) ? sanitize_text_field($_POST['wc_fraudlabspro_debug_log_path']) : $this->get_setting( 'debug_log_path' );
		...
		<div class="wrap">
			...
			<form id="form_settings" method="post" novalidate="novalidate">
				<table class="form-table">
					<tr>
						<td colspan=2>
							If you would like to learn more about the setup process, please visit <a href="https://www.fraudlabspro.com/supported-platforms/woocommerce" target="_blank">FraudLabs Pro for WooCommerce product page</a>.
						</td>
					</tr>
					<tr>
						<th scope="row">
					...
					<tr>
						<th scope="row" colspan="2">
							<p class="description">
								<input type="button" name="button" id="button-merchant" class="button button-primary" value="Login to Merchant Area" style="margin-top:5px; margin-bottom:5px;"/> ' . $plan_upgrade .'<br />
							</p>
						</th>
					</tr>
					...
					<tr>
						<td scope="row" colspan="2">
							<h2>Validation Rules</h2><hr />
						</td>
					</tr>

					<tr>
						<th scope="row">
							<label for="manage_rules">Manage Rules</label>
						</th>
						<td>
		...
		echo '
					...
					<tr>
						...
						<td>
							...
							<p class="description">
								You can trigger notification, such as email sending, using the Zapier service. Please configure the integration in Zapier.com before enabling this option. You can visit <a href="https://www.fraudlabspro.com/resources/tutorials/how-to-enable-notification-using-zapier-in-woocommerce/" target="_blank">https://www.fraudlabspro.com/resources/tutorials/how-to-enable-notification-using-zapier-in-woocommerce/</a> for details about how to enable the Zap.
							</p>
						</td>
					</tr>
	...
	public function javascript_agent() {
		...
		// echo '<script src="https://cdn.fraudlabspro.com/v3-agent.min.js"></script><script>var agent = new FraudLabsProAgent3("' . session_id() . '");window.onload = function() {agent.resolve();};</script>';
	}
						...
						$table .= '
							<tr>
								<td style="width:50%;" rowspan="2">
									...
									<img class="img-responsive" alt="" src="//cdn.fraudlabspro.com/assets/img/scores/meter-' . ( ( $row['fraudlabspro_score'] ) ? $row['fraudlabspro_score'] . '.png' : 'nofraudprotection.png' ) . '" style="width:160px;" />
								</td>
								<td style="width:50%;">
							...
							<tr>
								<td>
									...
									<p><a href="https://www.fraudlabspro.com/merchant/transaction-details/' . $row['fraudlabspro_id'] . '" target="_blank">' . $row['fraudlabspro_id'] . '</a></p>
								</td>
							</tr>
							<tr>
								<td>
							...
							<tr>
								<td>
									<b>IP to Billing Distance</b> <a href="javascript:;" class="flp-helper" title="Distance from IP address to Billing Location."><span class="dashicons dashicons-editor-help"></span></a>
							...
							<tr>
								...
								<td>
									...
									<p>' . ( ($row['ip_usage_type'] == 'NA' ) ? 'Not available [<a href="https://www.fraudlabspro.com/pricing" target="_blank">Upgrade</a>]' : $row['ip_usage_type'] ) . '</p>
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<b>Free Email Domain</b> <a href="javascript:;" class="flp-helper" title="Whether e-mail is from free e-mail provider."><span class="dashicons dashicons-editor-help"></span></a>
							...
							<tr>
								...
								<td>
									<b>Proxy IP Address</b> <a href="javascript:;" class="flp-helper" title="Whether IP address is from Anonymous Proxy Server."><span class="dashicons dashicons-editor-help"></span></a>
									<p>' . $this->parse_fraud_result( $row['is_proxy_ip_address'] ) . '</p>
								</td>
							</tr>
							<tr>
								<td>
									<b>IP in Blacklist</b> <a href="javascript:;" class="flp-helper" title="Whether the IP address is in our blacklist database."><span class="dashicons dashicons-editor-help"></span></a>
									<p>' . $this->parse_fraud_result( $row['is_ip_blacklist'] ) . '</p>
								</td>
								<td>
									<b>Email in Blacklist</b> <a href="javascript:;" class="flp-helper" title="Whether the email address is in our blacklist database."><span class="dashicons dashicons-editor-help"></span></a>
									<p>' . $this->parse_fraud_result( $row['is_email_blacklist'] ) . '</p>
								</td>
							</tr>
							<tr>
								<td>
									<b>Phone Verified</b> <a href="javascript:;" class="flp-helper" title="Whether the phone number is verified by the customer."><span class="dashicons dashicons-editor-help"></span></a>
									<p>'. ( isset( $row['is_phone_verified'] ) ? ( ( is_plugin_active( 'fraudlabs-pro-sms-verification/fraudlabspro-sms-verification.php' ) ) ? $row['is_phone_verified'] : '- [<a href="https://wordpress.org/plugins/fraudlabs-pro-sms-verification/" target="_blank">FraudLabs Pro SMS Verification Plugin Required</a>]' ) : 'NA [<a href="https://wordpress.org/plugins/fraudlabs-pro-sms-verification/" target="_blank">FraudLabs Pro SMS Verification Plugin Required</a>]' ) .'</p>
								</td>
								<td>
									...
									<p>' . ( strpos( $plan_name, 'Micro' ) ? '<span style="color:orange">Available for <a href="https://www.fraudlabspro.com/pricing" target="_blank">Mini plan</a> onward. Please <a href="https://www.fraudlabspro.com/merchant/login" target="_blank">upgrade</a>.</span>' : ( ( isset( $row['fraudlabspro_rules'] ) ) ? $row['fraudlabspro_rules'] : '-' ) )  . '</p>
								</td>
							</tr>
							<tr>
								<td>
									...
									<p>' . $row['fraudlabspro_credits'] . ' [<a href="https://www.fraudlabspro.com/pricing" target="_blank">Upgrade</a>]</p>
								</td>
								<td>
							...
							<tr>
								<td colspan="3">
									<p>Please login to <a href="https://www.fraudlabspro.com/merchant/transaction-details/' . $row['fraudlabspro_id'] . '" target="_blank">FraudLabs Pro Merchant Area</a> for more information about this order.</p>
								</td>
							</tr>
						</table>
					...
					if ( $row->fraudlabspro_id != '' ){
						...
						$table .= '
							...
							<tr>
								...
								<td>
									<b>Triggered Rules</b> <a href="javascript:;" class="flp-helper" title="FraudLabs Pro Rules triggered."><span class="dashicons dashicons-editor-help"></span></a>
									<p>' . ( strpos( $plan_name, 'Micro' ) ? 'Available for <a href="https://www.fraudlabspro.com/pricing" target="_blank">Mini plan</a> onward. Please <a href="https://www.fraudlabspro.com/merchant/login" target="_blank">upgrade</a>.' : ( ( isset( $row->fraudlabspro_rules ) ) ? $row->fraudlabspro_rules : '-' ) )  . '</p>
								</td>
							</tr>
							<tr>
								<td>
						...
						if( $row->fraudlabspro_status == 'REVIEW' ) {
							$table .= '
							<input type="submit" name="approve" id="approve-order" value="' . __( 'Approve', $this->namespace ) . '" style="padding:10px 5px; background:#45B6AF; color:#fff; border:1px solid #ccc; min-width:100px; cursor: pointer;" />
							<input type="submit" name="reject" id="reject-order" value="' . __( 'Reject', $this->namespace ) . '" style="padding:10px 5px; background:#F3565D; color:#fff; border:1px solid #ccc; min-width:100px; cursor: pointer;" />';
						}
		...
		try {
			...
			if ( ! is_wp_error( $request ) ) {
				...
				if ( is_object( $response ) ) {
					...
					die( 'SUCCESS_' . $response->plan_name );
				} else {
					die( 'Response Error.' );
				}
			} else {
	...
	private function create_flpwc_table() {
		...
		$tbExists = $wpdb->get_row("SHOW TABLES LIKE '" . $table_name . "'");
	...
	private function get_flpwc_data($table_name, $post_id, $key) {
		...
		$data = [];
		$metaValue = $wpdb->get_row("SELECT `meta_value` FROM $table_name WHERE post_id = '" . $post_id . "' AND meta_key = '" . $key . "' LIMIT 1");
