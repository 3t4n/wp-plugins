jQuery(document).ready(function ($) {
  ...
  let config = {'site_id':'', 'site_name':'', 'drive_id':'', 'drive_name':'', 'folder_id':'', 'folder_path':'', 'currentView':'', 'is_plugin':doc_sync_data.is_plugin, 'nextLink':{'link': '', 'redeemed': false}, 'searched_doc':false};
  let search_ajax = '';
  let nextLinkAjax = '';
  ...
  let load_paths = {
    ...
    'drive': {
        'init': function () {
            config['nextLink'] = {'link': '', 'redeemed': false};
            config['currentView'] = 'drive';
  ...
  function load_drive_docs(drive_id, drive_name) {
    ...
    docSyncHandleBackedndCalls('mo_sps_load_drive_docs', {drive_id:config['drive_id'], drive_name:config['drive_name'], breadcrumbs:breadcrumbs, is_plugin:config['is_plugin']}).then(async (res)=>{
      if(!res.success){
        mo_sps_show_error(res);
        return;
      }
      ...
      config['folder_id'] = '';
      config['folder_path'] = '';
      var content = await process_docs(res);
  ...
  function load_folder_docs(drive_id, folder_id) {
    ...
    $(`#mo_sps_all_errors`).hide();
    $(`#mo_sps_doc_refresh`).css('pointer-events','');
    docSyncHandleBackedndCalls('mo_sps_load_folder_docs', {drive_id:config['drive_id'], folder_id:config['folder_id'], breadcrumbs:breadcrumbs, is_plugin:config['is_plugin']}).then(async (res)=>{
      if(!res.success) {
        mo_sps_show_error(res);
        return;
      }
  ...
  function load_folder_docs_using_path(folder_path) {
    ...
    $(`#mo_sps_all_errors`).hide();
    $(`#mo_sps_doc_refresh`).css('pointer-events','');
    docSyncHandleBackedndCalls('mo_sps_get_folder_items_using_path', {folder_id:config['folder_id'], folder_path:folder_path, breadcrumbs:breadcrumbs, is_plugin:config['is_plugin']}).then(async (res)=>{
      if(!res.success) {
        mo_sps_show_error(res);
        return;
      }
  ...
  async function process_docs(res,searched = false) {
    let docs = res.data.value;
    config['nextLink']['link'] = res.data && res.data['@odata.nextLink'] ? decodeURI(res.data['@odata.nextLink']) : '';
    config['nextLink']['redeemed'] = false;
    ...
    if($.isEmptyObject(docs)) {
      ...
      docs.forEach(doc=>{
        if('folder' in doc) {
          content += `<tr title="${doc.name}" class="mo_sps_table_tr" style="cursor:pointer;" id="mo_sps_folder_doc_sync__${doc.id}" item_name="${doc.name}" web-url="${doc.webUrl}" item_path="${doc.parentReference && doc.parentReference.path ? doc.parentReference.path : ``}" item_id="${doc.id}">
          ...
          content += `<tr title="${doc.name}" class="mo_sps_table_tr" id="mo_sps_file_download" searched="${searched}" file-id="${doc.id}" key="${files_download_url.length - 1}">
            <td class="mo_sps_table_tbody_td" style="display:flex;"><img style="width:20px;height:20px;margin-right:10px;" src="${file_url}">
              ${download_url || config['searched_doc'] ? `<div id="mo_sps_file_doc_sync__${doc.id}" key="${files_download_url.length - 1}" searched="${searched}" file-id="${doc.id}" class="mo_sps_table_tr_file_name_div" style="max-width: 22rem;overflow-x: hidden;text-overflow: ellipsis;white-space:nowrap !important;">${doc.name}</div>` : `<div searched="${searched}" style="max-width: 16rem;overflow-x: hidden;text-overflow: ellipsis;white-space:nowrap !important;">${doc.name}</div>`}
  ...
  function handle_events() {
    const $element = $('#tableTOScroll');
    // handle infinite scroll to detect and call the backend.
    $element.on('scroll', function () {
        ...
        if (scrollTop + offsetHeight >= scrollHeight) {
          if('' !== config['nextLink']['link'] && ! config['nextLink']['redeemed']) {
            config['nextLink']['redeemed'] = true;
            docSyncHandleBackedndNextlinkCalls('mo_sps_get_documents_using_next_link', { nextLink:config['nextLink']['link'] });

            nextLinkAjax.then(async (res)=>{
              if(!res.success) {
                config['nextLink']['redeemed'] = false;
                return;
              }
              ...
              var content = await process_docs(res);
              $('#mo_sps_table_tbody').append(content);
            });
          }
        }
    });
    ...
    $(document).on('click', "img[id^='mo_sps_file_doc_download_'],div[id^='mo_sps_file_doc_sync__'], tr[id^='mo_sps_file_download'], div[id^='mo_sps_search_drpdn_file_']", function(e){
      e.preventDefault();
    ...
    $(document).on('click', "img[id^='mo_sps_file_doc_preview_']", function(e){
      ...
      if(config['searched_doc'] === true) {
        var ajax_url = `${doc_sync_data.admin_ajax_url}?action=mo_sps_get_file_web_url&nonce=${doc_sync_data.nonce}&drive_id=${config['drive_id']}&file_id=${file_id}`;
    ...
    $(document).on('click', "div[id^='mo_sps_search_drpdn_folder_'],tr[id^='mo_sps_folder_doc_sync__']", function(e){
      ...
      if((doc_sync_data.connector == 'personal') && doc_sync_data.app_type == 'auto') {
        ...
        if(doc_sync_data.connector == 'onedrive' && doc_sync_data.app_type == 'auto') {
          var drive_name = config['drive_name'] === 'OneDrive' ? 'Documents' : config['drive_name'];
          const [first, ...rest] = web_url.split(encodeURI(drive_name));
          var relative_path = rest.join(encodeURI(drive_name));
        }
        var path_ele = [];
        if(relative_path) {
      ...
      if (query_text && query_text.length > 2) {
        ...
        search_ajax.then(async (res)=>{
          ...
          if(evnt.keyCode == 13) {
            ...
            var content = await process_docs(res,true);
            $('#mo_sps_table_tbody').html(content);
          } else {
  ...
  function mo_sps_show_error(res) {
    ...
    var content = `<div id="mo_sps_doc_embed_error" class="mo_sps_table_loader_div">
        <div><img class="mo-sps-error-image" src="${doc_sync_data.error}"></div>
        &nbsp;&nbsp;
  ...
  function docSyncHandleBackedndCalls(task, payload) {
    return $.ajax({
      ...
      beforeSend: function() {
        if(nextLinkAjax !== '') {
          $(`#mo_sps_next_document_loader`).remove();
          nextLinkAjax.abort();
        }
  ...
  function docSyncHandleBackedndNextlinkCalls(task, payload) {
    nextLinkAjax = $.ajax({
      url:`${doc_sync_data.admin_ajax_url}?action=mo_doc_embed&nonce=${doc_sync_data.nonce}`,
      type: "POST",
      data: {
      ...
      cache: false,
      beforeSend: function() {
        var next_doc_loader = `<tr id="mo_sps_next_document_loader"><td colspan="3" class="mo_sps_table_tbody_td" style="text-align:center; padding: 0;"><img width="40px" height="40px" src="${doc_sync_data.loader_gif}" alt="Next Document Loader"/></td></tr>`
        $('#mo_sps_table_tbody').append(next_doc_loader);
      },
      success: function(data) {
        $(`#mo_sps_next_document_loader`).remove();
        return data;
