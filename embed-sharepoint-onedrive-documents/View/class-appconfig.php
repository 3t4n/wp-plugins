...
class AppConfig {
	...
	/**
	 * Holds the App_Config class instance.
	 *
	 * @var App_Config
	 */
	...
	/**
	 * Object instance(App_Config) getter method.
	 *
	 * @return App_Config
	 */
		...
		<div class="mo-ms-tab-content" style="width:77rem;">
			<div class="mo-ms-tab-app-config-connection-main-div">
				<div class="mo-ms-tab-app-config-connection-sub-div">
					...
					<span class="mo-sps-initial-configuration-doc">Manage your SharePoint/Onedrive Accounts <a href="<?php echo esc_url( PluginConstants::DEVELOPERS_DOC . 'initial-configuration' ); ?>" target="_blank">Learn More</a></span>
				</div>
	...
	/**
	 * Displays the connection details for SharePoint.
	 */
	...
	/**
	 * Displays the SharePoint connection form.
	 */
					...
					<span class="mo_sps_auto_connection" id="mo_sps_auto_connection_span" style="position:relative">
						...
						<div class="mo_sps_auto_connection_select_drpdn" id="mo_sps_auto_connection_select_drpdn">
							<ul class="mo_sps_auto_connection_select_ul">
								<li id="mo_sps_auto_connection_type_sharepoint" data-type="sharepoint" class="mo_sps_auto_connection_select_li <?php echo ( 'sharepoint' === $selected_connector ? 'mo_sps_auto_connection_selected_li' : '' ); ?>"><img width="14px" height="14px" src="<?php echo esc_url( MO_SPS_PLUGIN_URL . '/images/microsoft-sharepoint.svg' ); ?>">SharePoint</li>
								<li id="mo_sps_auto_connection_type_onedrive" data-type="onedrive" class="mo_sps_auto_connection_select_li <?php echo ( 'onedrive' === $selected_connector ? 'mo_sps_auto_connection_selected_li' : '' ); ?>"><img width="14px" height="14px" src="<?php echo esc_url( MO_SPS_PLUGIN_URL . '/images/onedrive.png' ); ?>">OneDrive Business</li>
								<li id="mo_sps_auto_connection_type_personal" data-type="personal" class="mo_sps_auto_connection_select_li <?php echo ( 'personal' === $selected_connector ? 'mo_sps_auto_connection_selected_li' : '' ); ?>"><img width="14px" height="14px" src="<?php echo esc_url( MO_SPS_PLUGIN_URL . '/images/onedrive.png' ); ?>">OneDrive Personal</li>
							</ul>
						</div>
					</span>
				</div>
	...
	/**
	 * Displays Azure configuration form.
	 */
	private function mo_sps_display__azure_config() {
		...
		$disabled = ( '' !== $client_secret && '' !== $client_id && '' !== $tenant_id ) ? '' : 'disabled';
		?>
						...
						<td>
							<div style="display: flex;justify-content:flex-start;align-items:center;">
								...
								<div style="margin:10px;">
									<input <?php echo esc_html( $disabled ); ?> style="height:30px;" id="mo_sps_auto_connection_type_personal" data-type="sharepoint" type="button" class="mo-ms-tab-content-button" value="Test Configuration">
								</div>
							</div>
						</td>
					</tr>
	...
	/**
	 * Converts bytes into a readable format (B, KB, MB, GB, TB).
	 *
	 * @param int $bytes The size in bytes.
	 * @return string The size in a readable format.
	 */
	private function bytes_to_gb( $bytes ) {
		if ( $bytes < 1024 ) {
	 ...
	 *
	 * @param array $response Response containing user details.
	 */
	private function mo_sps_display_user_info( $response ) {
		...
		$user_details    = array();
		$connector_image = ( MO_SPS_PLUGIN_URL . '/images/microsoft-sharepoint.svg' );
		...
		if ( 'auto' === $app_type ) {
			...
			switch ( $connector ) {
				case 'personal':
					$response                   = isset( $response[0] ) && ! empty( $response[0] ) ? $response[0] : $response;
					$owner                      = isset( $response['owner'] ) ? $response['owner'] : array();
					$user                       = isset( $owner['user'] ) ? $owner['user'] : array();
					$user_details['user_name']  = isset( $user['displayName'] ) ? $user['displayName'] : '';
					$user_details['user_email'] = isset( $config['upn'] ) ? $config['upn'] : '';
					$quota                      = isset( $response['quota'] ) ? $response['quota'] : array();
					$storage                    = $this->mo_sps_get_storage( $quota );
					$connector_image            = ( MO_SPS_PLUGIN_URL . '/images/onedrive.png' );
					break;
				case 'onedrive':
					$response                   = isset( $response[0] ) && ! empty( $response[0] ) ? $response[0] : $response;
					$owner                      = isset( $response['owner'] ) ? $response['owner'] : array();
			...
			$jwt_object   = '' !== $access_token ? json_decode( base64_decode( str_replace( '_', '/', str_replace( '-', '+', $access_token ) ) ), true ) : array(); //phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- base64 decoded the encoded access token value.
		...
		<form class="mo_sps_ajax_submit_form" action="" method="post">
			...
			<input type="hidden" name="mo_sps_tab" value="app_config">
			<?php wp_nonce_field( 'mo_sps_remove_configured_account' ); ?>
			<div class="mo-ms-tab-content-tile" style="width:138%; padding: 10px; border: 1px solid #ddd;">
				<div class="mo-ms-tab-content-tile-content">
					<div style="display:flex;padding: 0px 0px 15px 5px;margin-left: -45px;">
						...
						<img style="width:50px;height:50px;" src="<?php echo esc_url( $connector_image ); ?>">
							<div class="mo-ms-display-user-info-div1-innerdiv">
								...
								<?php echo esc_html( $user_details['connection_status'] ); ?>
							</div>
						</div>
						<div class="mo-ms-display-user-info-div2">
							<p><?php echo esc_html( $user_details['user_name'] ); ?></p>
							<span style="color:gray;margin-bottom: 5px;"><span class="dashicons dashicons-email"></span> <?php echo esc_html( $user_details['user_detail'] ); ?> <?php echo esc_html( $user_details['user_email'] ); ?></span>
							<div class="mo-ms-display-user-info-div2-innerdiv1">
								<div style="width:30%;"><span class="dashicons dashicons-database"></span> <?php echo esc_html( $storage ); ?> </div>
								<div><span class="dashicons dashicons-share"></span> <?php echo esc_html( $user_details['connection_type'] ); ?> </div>
							</div>
						</div>
	...
	/**
	 * Gets the test configuration URL for the connection.
	 *
	...
	/**
	 * Converts the storage quota data into a readable format.
	 *
	 * @param array $quota Storage quota data containing 'total' and 'used' storage in bytes.
	 * @return string readable storage usage in GB (e.g., "2 GB / 10 GB").
	 */
	private function mo_sps_get_storage( $quota ) {
		if ( ! empty( $quota ) ) {
			$total_storage = isset( $quota['total'] ) ? $this->bytes_to_gb( $quota['total'] ) : '-';
			$used_storage  = isset( $quota['used'] ) ? $this->bytes_to_gb( $quota['used'] ) : '-';
