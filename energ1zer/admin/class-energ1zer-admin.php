	...
	private function __construct() 
	{
	    ...
	    */
		...
		{	
			...
			$pageclass_esc = esc_attr($pageclass);
			...
			$display_title = get_post_meta($post->ID, '_energ1zer_meta_display_title', true);
			$display_title_hide = ""; $display_title_show = "";
			if ($display_title == "hide")
			{	$display_title_hide = " selected=\"selected\"";
			}
			else if ($display_title == "show")
			{	$display_title_show = " selected=\"selected\"";
			}
			
			$display_wpautop = get_post_meta($post->ID, '_energ1zer_meta_wpautop', true);
			$display_wpautop_hide = ""; $display_wpautop_show = "";
			if ($display_wpautop == "hide")
			{	$display_wpautop_hide = " selected=\"selected\"";
			}
			else if ($display_wpautop == "show")
			{	$display_wpautop_show = " selected=\"selected\"";
			}
			
			$display_comment_form = get_post_meta($post->ID, '_energ1zer_meta_display_comment_form', true);			
			$display_comment_form_hide = ""; $display_comment_form_show = "";
			if ($display_comment_form == "hide")
			{	$display_comment_form_hide = " selected=\"selected\"";
			}
			else if ($display_comment_form == "show")
			{	$display_comment_form_show = " selected=\"selected\"";
			}
			...
			echo <<<EOF
<br/>
<label for="energ1zer_widget_pageclass">
Page class
</label>
<input type="text" id="energ1zer_widget_pageclass" name="energ1zer_widget_pageclass" value="$pageclass_esc" size="15" />
<br/>
...
<br/>
<label>
Automatic wordpress &lt;p&gt;&lt;/p&gt;
</label>
<select name="energ1zer_wpautop">
	<option value="">Default</option>
	<option value="hide" $display_wpautop_hide>Hide</option>
	<option value="show" $display_wpautop_show>Show</option>
</select>
<br/>
<label>
Display Title
</label>
<select name="energ1zer_display_title">
	<option value="">Default</option>
	<option value="hide" $display_title_hide>Hide</option>
	<option value="show" $display_title_show>Show</option>
</select>
<br/>
<label>
Display Comment Form
</label>
<select name="energ1zer_display_comment_form">
	<option value="">Default</option>
	<option value="hide" $display_comment_form_hide>Hide</option>
	<option value="show" $display_comment_form_show>Show</option>
</select>
...
EOF;
		...
		{	/*
			...
			$pageclass = sanitize_text_field( $_POST['energ1zer_widget_pageclass'] );
			$showtext = sanitize_text_field( $_POST['energ1zer_widget_showtext'] );			
			$grayscale = "";
			if (isset($_POST['energ1zer_widget_grayscale']))
			{	$grayscale = "yes";
			}
			$display_title = sanitize_text_field( $_POST['energ1zer_display_title'] );
			$wpautop = sanitize_text_field( $_POST['energ1zer_wpautop'] );
			$display_comment_form = sanitize_text_field( $_POST['energ1zer_display_comment_form'] );
			...
			update_post_meta($post_id, '_energ1zer_meta_widget_pageclass', $pageclass );
			...
			update_post_meta($post_id, '_energ1zer_meta_display_title', $display_title );
			update_post_meta($post_id, '_energ1zer_meta_wpautop', $wpautop );
			update_post_meta($post_id, '_energ1zer_meta_display_comment_form', $display_comment_form );
		}
	...
	{
		wp_enqueue_style( $this->plugin_slug .'-admin-styles', plugins_url( 'assets/css/energ1zer.css', __FILE__ ), array(), Energ1zer::VERSION );
		/* if later we have CSS specific admin setting page 
		...
		{
			wp_enqueue_style( $this->plugin_slug .'-admin-settings-styles', plugins_url( 'assets/css/energ1zer-settings.css', __FILE__ ), array(), Energ1zer::VERSION );
		}
*/
	}
