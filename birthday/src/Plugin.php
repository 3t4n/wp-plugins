...
final class Plugin {
	...
	public function upgrade() {
		...
		// Only show in Birthday email section and WooCommerce coupons page.
		if ( ( ! empty( $_GET['section'] ) && 'wc_birthday_email' === $_GET['section'] ) || ( ! empty( $_GET['post_type'] ) && 'shop_coupon' === $_GET['post_type'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended

			if ( ! bir_fs()->is_premium() ) {
				$message = sprintf( /* translators: %1s - The Birthday For WooCommerce Plugin, %2s - Mini Plugins  */ __( 'Get %1s to set the birthday coupons so that customers can use the coupon on their birthday only.', 'birthday' ), '<a href="https://miniplugins.com/birthday-for-woocommerce-premium/">Birthday For WooCommerce PRO</a>' );
