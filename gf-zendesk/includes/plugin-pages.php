...
if( !class_exists( 'vxg_zendesk_pages' ) ) {
...
* @since       1.0.0
*/
class vxg_zendesk_pages   extends vxg_zendesk{
    public $ajax=false;
/**
...
require_once(self::$path . 'templates/crm-entry-box.php'); 
  ...
  */
  ...
  if(isset($_GET[$this->id."_logs"]) && current_user_can($this->id.'_read_settings')){
      $msg=__('Error While Clearing Zendesk Logs','gravity-forms-zendesk-crm');
      $level="error";
      if(!empty($_GET[$this->id."_logs"])){
      $msg=__('Zendesk Logs Cleared Successfully','gravity-forms-zendesk-crm');   
      $level="updated";
      }
  ...
  */
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . esc_html__('Settings', 'gravity-forms-zendesk-crm') . '</a>' );
        }
        return $links;
   }
  ...
  /**
    ...
    public function add_form_settings_menu( $tabs, $form_id ) {
        ...
        $tabs[] = array( 'name' => $this->id, 'label' => esc_html__("Zendesk", 'gravity-forms-zendesk-crm') , 'query' => array( 'fid' => null),'icon' => 'dashicons-cloud dashicons' );
...
$this->push($entry,$form,'add_note');
  ...
  */
  ...
  $menus[] = array("name" => $this->id, "label" => esc_html__('Zendesk','gravity-forms-zendesk-crm'), "callback" =>  array($this, "mapping_page"), "permission" => $this->id.'_read_feeds');
  ...
  */
  public  function setup(){
      ...
      RGForms::add_settings_page(array('name' => $this->id,'tab_label' => esc_html__('Zendesk','gravity-forms-zendesk-crm'),'icon' => 'dashicons-cloud dashicons',"handler"=>array($this, "settings_page")));
  ...
  $msg=__('You do not have permissions to export logs','gravity-forms-zendesk-crm');
  ...
  $fields=array(); $field_titles=array("#",__('Status','gravity-forms-zendesk-crm'),__('Zendesk ID','gravity-forms-zendesk-crm') ,__('Entry ID','gravity-forms-zendesk-crm'),__('Description','gravity-forms-zendesk-crm'),__('Time','gravity-forms-zendesk-crm'));
  ...
  $msg=__('You do not have permissions to clear logs','gravity-forms-zendesk-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=$this->get_data_object();
  ...
  */
  ...
  esc_html_e('You do not have permissions to access this page','gravity-forms-zendesk-crm');    
  return;
  }
  ...
  <p>
  <?php esc_html_e("Feed deleted.", 'gravity-forms-zendesk-crm') ?>
  </p>
  </div>
  <?php
  }
  ...
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'gravity-forms-zendesk-crm') ?>
  </p>
  </div>
  <?php
  }
...
$count=$this->data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','gravity-forms-zendesk-crm'),$count));  
    }
  ...
  if(is_array($entry)){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','gravity-forms-zendesk-crm'));  
  }
  ...
  case"time": $time_order=$order_icon; $time_class="";   break;    
    ...
    $bulk_actions=array(""=>__('Bulk Action','gravity-forms-zendesk-crm'),"delete"=>__('Delete','gravity-forms-zendesk-crm'),
  'send_to_crm_bulk'=>__('Send to Zendesk','gravity-forms-zendesk-crm'),'send_to_crm_bulk_force'=>__('Force Send to Zendesk - Ignore filters','gravity-forms-zendesk-crm'));
  $base_url=$this->get_base_url();

$objects=$this->get_objects();
      $statuses=array("1"=>__("Created",'gravity-forms-zendesk-crm'),"2"=>__("Updated",'gravity-forms-zendesk-crm'),"error"=>__("Failed",'gravity-forms-zendesk-crm'),"4"=>__("Filtered",'gravity-forms-zendesk-crm'),"5"=>__("Deleted",'gravity-forms-zendesk-crm')); 
...
public function get_menu_links($current_page=""){
      $settings_link=$this->link_to_settings();
       $id=isset($_GET['id']) ? $this->clean($_GET['id']) : '';
  ...
  $feeds_link=admin_url( "admin.php?page=gf_edit_forms&view=settings&subview={$this->id}&tab=feed&id={$id}" );
      ...
      $menu_links=array(
  'settings'=> array( 
  "title"=>__('Zendesk Settings','gravity-forms-zendesk-crm'),
  "link"=>$settings_link,
  "current"=>$current_page == 'settings' ? true : false
  ),
  'feed'=> array( 
  "title"=>__('Zendesk Feeds','gravity-forms-zendesk-crm'),
  "link"=>$feeds_link,
   "current"=>$current_page == 'feed' ? true : false
  ),
  'log'=> array( 
  "title"=>__('Zendesk Log','gravity-forms-zendesk-crm'),
  "link"=>$logs_link,
...
//
  ...
  $fields=$api->get_crm_fields($module); 
    ...
    foreach($fields as $k=>$v){
        if( empty($v['custom']) && !in_array($k,$phone)){ 
       $temp_fields[$k]=$v;     
        }
    }
   $fields= $temp_fields;
...
foreach($map as $field_k=>$field_v){
  ...
  $sel_fields=array(""=>__("Standard Field",'gravity-forms-zendesk-crm'),"value"=>__("Custom Value",'gravity-forms-zendesk-crm'));
...
include_once(self::$path . "templates/fields-mapping.php"); 
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = esc_html__('Zendesk','gravity-forms-zendesk-crm');
        return $plugins;
    }
  /**
  ...
  */
  ...
  $msg=__("Please Choose Object",'gravity-forms-zendesk-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'gravity-forms-zendesk-crm');
  }
  if($msg !=""){
  ?>
...
/*  $object=$this->post('object',$feed);
  ...
  $msg=__("Please Choose Account",'gravity-forms-zendesk-crm');
  }
  if($msg !=""){
  ?>
  <div class="error below-h2" style="background: #f3f3f3">
...
case"refresh_mailboxes":
   ...
   $data['crm_sel_status']=$this->gen_select($books,$owner_sel,__('Select Mailbox','gravity-forms-zendesk-crm'));
  ...
  case"refresh_users":
    ...
    $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','gravity-forms-zendesk-crm'));
  $meta['users']=$users;  
  $res['data']=$data;   
      break;
  case"refresh_owners":
    ...
    $data['crm_sel_owner']=$this->gen_select($users,$owner_sel,__('Customer','gravity-forms-zendesk-crm'));
  $meta['users']=$users;  
  $res['data']=$data;   
      break; 
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','gravity-forms-zendesk-crm');
    }
}
  die(json_encode($res));    
  }
...
$redir= urldecode($state)."&".http_build_query($part);
wp_redirect($redir);
die();
  }
...
if(isset($_REQUEST[$this->id.'_tab_action']) && $_REQUEST[$this->id.'_tab_action']=="get_token"){
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to add token','gravity-forms-zendesk-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $id=$this->post('id');
...
$res=$data->del_account($id);
 if($res){
       $msg=__('Account Deleted Successfully','gravity-forms-zendesk-crm');
  $msg_arr=array('msg'=>$msg,'class'=>'updated');   
 }else{
       $msg=__('Error While Removing Account','gravity-forms-zendesk-crm');
...
$log=$this->data->get_log_by_id($log_id); 
  ...
  $response=json_decode($log['response'],true);
    $triggers=array('manual'=>'Submitted Manually','paid'=>'Payment Completion','submit'=>'Form Submission','update'=>'Entry Update'
...
$field_options="<option>".__("Select Object",'gravity-forms-zendesk-crm')."</option>"; 
  if(is_array($objects)){
...
return $arr;
  ...
  */
  ...
  if(!current_user_can($this->id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','gravity-forms-zendesk-crm');   
  ...
  $this->uninstall();
  $uninstall_msg=sprintf(__("Gravity Forms Zendesk Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'gravity-forms-zendesk-crm'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to save settings','gravity-forms-zendesk-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','gravity-forms-zendesk-crm'));
  $valid_email=true;
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','gravity-forms-zendesk-crm'));
  }
 ...
 $new_account=$page_link."&id=".$new_account_id;
  ...
  check_admin_referer('vx_nonce');
  if(!current_user_can($this->id."_read_settings")){ 
  $msg=__('You do not have permissions to refresh lists','gravity-forms-zendesk-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  ...
  $msgs['refresh']=array("class"=>"updated","msg"=>__('Successfully Refreshed Picklists','gravity-forms-zendesk-crm')); 
...
if(!empty($info['data'])){
 ...
 if(isset($_POST['vx_test_connection'])){
  $msg=__('Connection to Zendesk is Working','gravity-forms-zendesk-crm');
  
  if($conn_class != "updated" ){
      $msg=__('Connection to Zendesk is NOT Working','gravity-forms-zendesk-crm');  
  }
  $title=__('Test Connection: ','gravity-forms-zendesk-crm');
...
$sel2_css=$base_url. 'css/select2.min.css';
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  esc_html_e('You do not have permissions to edit/save feed','gravity-forms-zendesk-crm'); 
  return;
  }
  //
...
$msg=sprintf(__("Feed Updated. %sback to list%s", 'gravity-forms-zendesk-crm'), '<a href="'.$feed_link.'">', "</a>");
  }
  else{
$msg=__("Feed could not be updated. Please enter all required information below.", 'gravity-forms-zendesk-crm');
$class='error';
  }
  if(!empty($msg)){
  ...
  $info=$this->get_info($account);
 $form_id=isset($_GET['id']) ? $this->clean($_GET['id']) : '';
...
include_once(self::$path . "templates/feed-account.php");
  ...
  */
  ...
  $desc.=" Web2".$row['object'];
   ...
   $desc=sprintf(__('%s Filtered','gravity-forms-zendesk-crm'),$row['object']);   
  }else   if($row['status'] == 5){
   $desc=sprintf(__('%s Deleted','gravity-forms-zendesk-crm'),$row['object']);  
  }else{
  $desc.=$row['object'];
  }
  }else{
  ...
  $title=__("Failed",'gravity-forms-zendesk-crm');   
  if( $row['status'] == 1){
  $title=__("Created",'gravity-forms-zendesk-crm');   
  }else if($row['status'] == 2){
  $title=__("Updated",'gravity-forms-zendesk-crm');   
  }else if($row['status'] == 4){
  $title=__("Filtered",'gravity-forms-zendesk-crm');   
  }else if($row['status'] == 5){
  $title=__("Deleted",'gravity-forms-zendesk-crm');   
  }
  $row['_crm_id']= $crm_id;
  $row['a_link']=$link;
  $row['desc']=$desc;
  ...
  */
  ...
  if(RGFormsModel::get_input_type($field) == "address") {
      $fields[] =  array($field["id"], GFCommon::get_label($field) . " (" . _x("Full" , 'Full field label', 'gravity-forms-zendesk-crm') . ")");
  }
  ...
  foreach($field["inputs"] as $input)
      ...
      if(empty($label) && isset($field->placeholder)){
  ...
  $fields[]=array('date_created',__('Entry Date','gravity-forms-zendesk-crm'));
  $fields[]=array('ip',__('User IP','gravity-forms-zendesk-crm'));
  $fields[]=array('source_url',__('Source Url','gravity-forms-zendesk-crm'));
  $fields[]=array('form_title',__('Form Title','gravity-forms-zendesk-crm'));
  $fields[]=array('status',__('Entry Status','gravity-forms-zendesk-crm'));
  $fields[]=array('payment_status',__('Payment Status','gravity-forms-zendesk-crm'));
  $fields[]=array('payment_date',__('Payment Date','gravity-forms-zendesk-crm'));
  $fields[]=array('payment_amount',__('Payment Amount','gravity-forms-zendesk-crm'));
  $fields[]=array('transaction_id',__('Transaction Id','gravity-forms-zendesk-crm'));
  $fields[]=array('currency',__('Currency','gravity-forms-zendesk-crm'));
  $this->fields=array('gf'=>array("title"=>__('Gravity Forms Fields','gravity-forms-zendesk-crm'),"fields"=>$fields)); 
...
if(is_array($fields_arr['fields'])){
  ...
  */
  ...
  $msg=__( 'Successfully Connected to Zendesk','gravity-forms-zendesk-crm' );
     if(!empty($info['_time'])){
