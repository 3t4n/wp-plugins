<?php
/**
* Plugin Name: WP Gravity Forms Zendesk
* Description: Integrates Gravity Forms with Zendesk allowing form submissions to be automatically sent to your Zendesk account 
* Version: 1.1.2
* Requires at least: 3.8
* Author URI: https://www.crmperks.com
* Plugin URI: https://www.crmperks.com/plugins/gravity-forms-plugins/gravity-forms-zendesk-plugin/
* Author: CRM Perks.
* Text Domain: gravity-forms-zendesk-crm
...
class vxg_zendesk {
 ...
 public  $url = 'https://www.crmperks.com';
  ...
  public  $id = 'vxg_zendesk';
  public  $domain = 'vxg-zendesk';
  public  $version = "1.1.2";
  public  $update_id = '30006';
  public  $min_gravityforms_version = '1.3.9';
  public $type = 'vxg_zendesk_pro';
  public  $fields = null;
  public  $data = null;
  ...
  public static $title='Gravity Forms Zendesk Plugin';  
  public static $path = ''; 
  public static $slug = '';
...
add_action('init', array($this,'init'));   
  ...
  load_plugin_textdomain('gravity-forms-zendesk-crm', FALSE,  $this->plugin_dir_name(). '/languages/' );
  ...
  $install=new vxg_install_zendesk();
...
self::$plugin->instance();
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo esc_html($id) ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Gravity Forms Zendesk Plugin','gravity-forms-zendesk-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','gravity-forms-zendesk-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice is-dismissible" data-id="<?php echo esc_html($id) ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Gravity Forms Zendesk Plugin','gravity-forms-zendesk-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
public function add_tags( $merge_tags, $form_id, $fields, $element_id ) {
  ...
  foreach($feeds as $v){
    $merge_tags[] = array('label' => substr($v['name'],0,20).' Zendesk Link', 'tag' => '{zendesklink_'.$v['id'].'}');
    $merge_tags[] = array('label' => substr($v['name'],0,20).' Zendesk ID', 'tag' => '{zendeskid_'.$v['id'].'}');
  }
    return $merge_tags;
}
public function replace_tags( $text, $form, $entry, $url_encode, $esc_html, $nl2br, $format ) {
    ...
    if(!empty($form['id']) && strpos( $text, '{zendesk' ) !== false ){    
      $data_db=$this->get_data_object(); 
...
$tags=array();
 ...
 $link= !empty(self::$feeds_res[$id]['link']) ? self::$feeds_res[$id]['link'] : '#'.self::$feeds_res[$id]['id'];         
   $tags['{zendesklink_'.$v['id'].'}']=$link;   
   $tags['{zendeskid_'.$v['id'].'}']=self::$feeds_res[$id]['id'];   
      }
  }
...
*/
  ...
  $message = sprintf(__("%sGravity Forms%s is required. %sPurchase it today!%s", 'gravity-forms-zendesk-crm'), "<a href='http://www.gravityforms.com/'>", "</a>", "<a href='http://www.gravityforms.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Gravity Forms is installed but not active. %sActivate Gravity Forms%s to use the Gravity Forms Zendesk Plugin','gravity-forms-zendesk-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=gravityforms/gravityforms.php'), 'activate-plugin_gravityforms/gravityforms.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sGravity Forms%s is required. %sPurchase it today!%s", 'gravity-forms-zendesk-crm'), "<a href='http://www.gravityforms.com/'>", "</a>", "<a href='http://www.gravityforms.com/'>", "</a>");
  }  
  ...
  */
  ...
  if(current_user_can('administrator') && !empty($_REQUEST['VXGZendeskError'])) { /// 
  $confirmation .= sprintf(__('%sThe entry was not added to Zendesk because: %s. %sYou are only being shown this because you are an administrator. Other users will not see this message.%s', 'gravity-forms-zendesk-crm'), '<div class="error" style="text-align:center; color:#790000; font-size:14px; line-height:1.5em; margin-bottom:16px;background-color:#FFDFDF; margin-bottom:6px!important; padding:6px 6px 4px 6px!important; border:1px dotted #C89797">','<strong>'.esc_html($_REQUEST['VXGZendeskError']). '</strong>', '<br /><em>', '</em></div>');
  }
  return $confirmation;
  }
...
/*  if(empty($field)){
   ...
   $value=date($formats[$field->dateFormat],strtotime($value));     
       ...
       foreach($value as $k=>$v){
           if(is_array($v)){
            ...
            if(empty($table)){
              $table='<tr><th>'.implode('</th><th> ',array_keys($v)).'</th></tr>';     
            }
         $v=implode('</td><td> ',array_values($v));     
           }
           $v='<tr><td>'.$v.'</td></tr>';    
           
        $table.=$v;   
       }
       if(!empty($table)){ $table='<table>'.$table.'</table>'; }
     $value=$table;  
   } }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.$this->id.'_update" id="'.$this->id.'_update" value="1" /><label for="'.$this->id.'_update" title="%s">%s</label><br /><br />', esc_html__('Create or update this entry in Zendesk. The fields will be mapped according to the form feed settings.', 'gravity-forms-zendesk-crm'), esc_html__('Send to Zendesk', 'gravity-forms-zendesk-crm'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  $button.= '<input type="submit" class="button button-large button-secondary alignright" name="'.$this->id.'_send" style="margin-left:5px; '.$margin.'" title="'.__('Create or update this entry in Zendesk. The fields will be mapped according to the form feed settings.','gravity-forms-zendesk-crm').'" value="'.__('Send to Zendesk', 'gravity-forms-zendesk-crm').'" onclick="jQuery(\'#action\').val(\'send_to_crm\')" />';
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Zendesk Logs','gravity-forms-zendesk-crm').'" href="'.$log_url.'">'.__('Zendesk Logs','gravity-forms-zendesk-crm').'</a>';
  ...
  */
  ...
  RGFormsModel::add_note($id, 0, esc_html__('Gravity Forms Zendesk Plugin','gravity-forms-zendesk-crm'), $note);
  }
...
do_action('plugin_status_'.$this->type,'activate');  
  ...
  */
  ...
  $crm_tooltips = array(
    'vx_feed_name' => '<h6>' . esc_html__('Feed Name', 'gravity-forms-zendesk-crm') . '</h6>' . esc_html__('Enter feed name of your choice.', 'gravity-forms-zendesk-crm'),
  'vx_sel_object' => '<h6>' .__('Zendesk Object', 'gravity-forms-zendesk-crm') . '</h6>' . esc_html__('Select the Object to Create when a Form is Submitted.', 'gravity-forms-zendesk-crm'),
   'vx_sel_account' => '<h6>' .__('Zendesk Account', 'gravity-forms-zendesk-crm') . '</h6>' . esc_html__('Select the Zendesk account you would like to export entries to.', 'gravity-forms-zendesk-crm'),
  'vx_sel_form' => '<h6>' . esc_html__('Gravity Form', 'gravity-forms-zendesk-crm') . '</h6>' . esc_html__('Select the Gravity Form you would like to integrate with Zendesk. Contacts generated by this form will be automatically added to your Zendesk account.', 'gravity-forms-zendesk-crm'),
  
  'vx_map_fields' => '<h6>' . esc_html__('Map Standard Fields', 'gravity-forms-zendesk-crm') . '</h6>' . esc_html__('Associate your Zendesk fields to the appropriate Gravity Form fields.', 'gravity-forms-zendesk-crm'),
  
  'vx_optin_condition' => '<h6>' . esc_html__('Opt-In Condition', 'gravity-forms-zendesk-crm') . '</h6>' . esc_html__('When the opt-in condition is enabled, form submissions will only be exported to Zendesk when the condition is met. When disabled all form submissions will be exported.', 'gravity-forms-zendesk-crm'),
  
  'vx_manual_export' => '<h6>' . esc_html__('Manual Export', 'gravity-forms-zendesk-crm') . '</h6>' . esc_html__('If you do not want all entries sent to Zendesk, but only specific, approved entries, check this box. To manually send an entry to Zendesk, go to Entries, choose the entry you would like to send to Zendesk, and then click the "Send to Zendesk" button.', 'gravity-forms-zendesk-crm'),
  
    'vx_entry_notes' => '<h6>' . esc_html__('Entry Notes', 'gravity-forms-zendesk-crm') . '</h6>' . esc_html__('Enable this option if you want to synchronize Gravity Forms entry notes to Zendesk Object notes. For example , when you add a note to a Gravity Forms entry, it will be added to the Zendesk Object selected in the feed.', 'gravity-forms-zendesk-crm'),
    
      'vx_primary_key' => '<h6>' . esc_html__('Primary Key', 'gravity-forms-zendesk-crm') . '</h6>' . esc_html__('Which field should be used to update existing objects?', 'gravity-forms-zendesk-crm'),
    

    'vx_source_check'=>'<h6>' . esc_html__('Enable Lead Source', 'gravity-forms-zendesk-crm') . '</h6>' .__('Enable this option if you want to assign a lead source to this object.','gravity-forms-zendesk-crm'),
        
'vx_sources'=>'<h6>' . esc_html__('Zendesk Lead Sources', 'gravity-forms-zendesk-crm') . '</h6>' .__('Get Lead Sources list from zendesk.','gravity-forms-zendesk-crm'),
'vx_source'=>'<h6>' . esc_html__('Select Lead Source', 'gravity-forms-zendesk-crm') . '</h6>' .__('Which Lead Source should be assigned to this object.','gravity-forms-zendesk-crm'),

    'vx_status_check'=>'<h6>' . esc_html__('Enable Lead Status', 'gravity-forms-zendesk-crm') . '</h6>' .__('Enable this option if you want to assign a lead status to this object.','gravity-forms-zendesk-crm'),    
'vx_status_list'=>'<h6>' . esc_html__('Zendesk Lead Status List', 'gravity-forms-zendesk-crm') . '</h6>' .__('Get Lead Status list from zendesk.','gravity-forms-zendesk-crm'),
'vx_status'=>'<h6>' . esc_html__('Select Ticket Status', 'gravity-forms-zendesk-crm') . '</h6>' .__('Which Status should be assigned to this object.','gravity-forms-zendesk-crm'),
'vx_priority'=>'<h6>' . esc_html__('Select Ticket Priority', 'gravity-forms-zendesk-crm') . '</h6>' .__('Which Priority should be assigned to this object.','gravity-forms-zendesk-crm'),
'vx_type'=>'<h6>' . esc_html__('Select Ticket Type', 'gravity-forms-zendesk-crm') . '</h6>' .__('What should be Ticket Type.','gravity-forms-zendesk-crm'),



   'vx_assign_person'=>'<h6>' . esc_html__('Zendesk Person', 'gravity-forms-zendesk-crm') . '</h6>' .__('Enable this option if you want to assign a person to this object.','gravity-forms-zendesk-crm'),
   
      'vx_assign_check'=>'<h6>' . esc_html__('Assign Person', 'gravity-forms-zendesk-crm') . '</h6>' .__('Enable this option if you want to assign a user to this conversation.','gravity-forms-zendesk-crm'),
   'vx_sel_person'=>'<h6>' . esc_html__('Select Person', 'gravity-forms-zendesk-crm') . '</h6>' .__('Person created by selected feed will be added to this feed Object.','gravity-forms-zendesk-crm'),
   
   
   'vx_owner_check'=>'<h6>' . esc_html__('Assign Owner', 'gravity-forms-zendesk-crm') . '</h6>' .__('Enable this option if you want to assign an owner to this object.','gravity-forms-zendesk-crm'),
   
   'vx_owners'=>'<h6>' . esc_html__('Zendesk Users', 'gravity-forms-zendesk-crm') . '</h6>' .__('Get Users list from Zendesk','gravity-forms-zendesk-crm'),
   
      'vx_sel_owner'=>'<h6>' . esc_html__('Select Owner', 'gravity-forms-zendesk-crm') . '</h6>' .__('Select a user as a owner of this object','gravity-forms-zendesk-crm'),
   
      'vx_box_check'=>'<h6>' . esc_html__('Add to mailbox', 'gravity-forms-zendesk-crm') . '</h6>' .__('Enable this option if you want to add conversation in a mailbox.','gravity-forms-zendesk-crm'),
   
   'vx_boxes'=>'<h6>' . esc_html__('Zendesk Mailboxes', 'gravity-forms-zendesk-crm') . '</h6>' .__('Get mailboxes from Zendesk','gravity-forms-zendesk-crm'),
   
      'vx_sel_box'=>'<h6>' . esc_html__('Select Mailbox', 'gravity-forms-zendesk-crm') . '</h6>' .__('Select a mailbox in which you want to add conversation.','gravity-forms-zendesk-crm'),
   
   
   
   'vx_order_notes'=>'<h6>' . esc_html__('Entry Notes', 'gravity-forms-zendesk-crm') . '</h6>' .__('Enable this option if you want to synchronize Entry notes to Zendesk Object notes. For example, when you add a note to a Entry, it will be added to the Zendesk Object selected in the feed.','gravity-forms-zendesk-crm'),
  

         'vx_entry_note'=>'<h6>' . esc_html__('Entry Note', 'gravity-forms-zendesk-crm') . '</h6>' .__('Check this option if you want to send more data as CRM entry note.', 'gravity-forms-zendesk-crm'),
   'vx_note_fields'=>'<h6>' . esc_html__('Note Fields', 'gravity-forms-zendesk-crm') . '</h6>' .__('Select fields which you want to send as a note', 'gravity-forms-zendesk-crm'),
   'vx_disable_note'=>'<h6>' . esc_html__('Note Fields', 'gravity-forms-zendesk-crm') . '</h6>' .__('Enable this option if you want to add note only for new CRM entry', 'gravity-forms-zendesk-crm')
...
$a=array();
  ...
  return $a;  
     ...
     $var=wp_unslash($var);   
  if(in_array($key,array('note_val1'))){
 $var=sanitize_textarea_field($var);      
  }else if(!in_array($key,array('value','note_val')) ){
...
return  $var;
  ...
  */
  ...
  $status=0;
  if(class_exists('vxg_zendesk_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/gravity-forms-zendesk-crm/gravity-forms-zendesk-crm.php')) {
  $status=2;
  } 
  return $status;
  }
...
public function do_actions(){
  ...
  */
  ...
  $api = false;
  if(!class_exists("vxg_zendesk_api"))
  require_once($this->get_base_path()."api/api.php");
     
  $this->api=$api= new vxg_zendesk_api($crm);
  ...
  */
  ...
  $this->data=new vxg_zendesk_data();
  return $this->data;
  }
...
if(!empty($data['owner']) && !empty($data['user'])){
  $feed['user']=apply_filters('vx_assigned_user_id',$data['user'],$this->id,$feed['id'],$entry,$form);  
} 
...
$temp=apply_filters('crmperks_gf_zendesk_post',$temp,$feed,$entry);
//var_dump($temp); die(); 
$no_filter=true;    
...
//  var_dump($feed_arr); echo '<hr>'; var_dump($data);  die();
  ...
  $res=apply_filters($this->id.'_response', $res ,$entry); 
  }
  ...
  if($this->post('id',$res)!=""){ 
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Zendesk (%s) with ID # %s .', 'gravity-forms-zendesk-crm'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Zendesk (%s) with ID # %s . View entry at %s', 'gravity-forms-zendesk-crm'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Zendesk', 'gravity-forms-zendesk-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Zendesk (%s) with ID # %s . View entry at %s', 'gravity-forms-zendesk-crm'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Zendesk (%s) with ID # %s .', 'gravity-forms-zendesk-crm'),$feed['object'],$res['id']);   
            }
     $screen_msg=__( 'Entry updated in Zendesk', 'gravity-forms-zendesk-crm');
      }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'gravity-forms-zendesk-crm');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Zendesk. Entry not sent! Check the Entry Notes below for more details.' , 'gravity-forms-zendesk-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Zendesk. Entry not sent' , 'gravity-forms-zendesk-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'gravity-forms-zendesk-crm'),$feed['object']);
...
//  } 
  ...
  $log_link=' <a href="'.$log_url.'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','gravity-forms-zendesk-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//  $this->add_note($entry["id"], $msg);
  ...
  */
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Zendesk','gravity-forms-zendesk-crm')." Error","info_title"=>"More Detail","info"=>$detail);
...
$vxg_zendesk=new vxg_zendesk();
$vxg_zendesk->instance();
$vx_gf['vxg_zendesk']='vxg_zendesk';
