...
.vxcf_options_row .howto{
  ...
  <div class="vx_wrap"> <img alt="<?php esc_html_e("Zendesk Feeds", 'gravity-forms-zendesk-crm') ?>" title="<?php esc_html_e("Zendesk Feeds", 'gravity-forms-zendesk-crm') ?>" src="<?php echo $this->get_base_url()?>images/<?php echo $this->crm_name ?>-crm-logo.png?ver=1" style="float:left; margin:0px 7px 10px 0;" height="46" />
  <h2 style="margin-bottom: 12px"><?php esc_html_e("Zendesk Feeds", 'gravity-forms-zendesk-crm'); ?>
  <?php
      if($fid != $config['id']){
  ?>
  <a class="add-new-h2" href="<?php echo esc_url($new_feed_link)?>"><?php esc_html_e("Add New", 'gravity-forms-zendesk-crm') ?></a>
  <?php
      }
      if(!empty($fid)){
  ?>
  <a class="add-new-h2" href="<?php echo esc_url($feeds_link)?>"><?php esc_html_e("Back to Feeds", 'gravity-forms-zendesk-crm') ?></a>
  <?php
      }
  ?>
  </h2>
...
if(empty($feed_name)){
  ...
  ?>
  <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop(sprintf(__("We are unable to login to Zendesk with the provided API token. Please make sure they are valid in the %sSettings Page%s", 'gravity-forms-zendesk-crm'), "<a href='".$this->link_to_settings()."'>", "</a>")); ?></div>
  <?php
  // return;
  } 
  ?>
  <div class="vx_wrapper">
  <div class="vx_heading"><?php echo sprintf(__('Edit Zendesk Feed # %d','gravity-forms-zendesk-crm') ,$fid) ?></div>
  ...
  <?php wp_nonce_field("vx_nonce") ?>
    <div class="vx_div">
      <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('1. Enter Feed name',  'gravity-forms-zendesk-crm' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_name" class="left_header"><?php esc_html_e("Feed Name", 'gravity-forms-zendesk-crm'); ?>
  ...
  <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('2. Select Zendesk Account.', 'gravity-forms-zendesk-crm'); ?></div>
<div class="crm_btn_div" title="<?php esc_html_e('Expand / Collapse','gravity-forms-zendesk-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_account" class="left_header"><?php esc_html_e("Zendesk Account", 'gravity-forms-zendesk-crm'); ?>
  ...
  <div class="clear"></div>
    ...
    <div id="crm_ajax_div_object" style="display:none; text-align: center; line-height: 100px;"><i> <?php esc_html_e('Loading, Please Wait...','gravity-forms-zendesk-crm'); ?></i></div>
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php esc_html_e('This is a required field','gravity-forms-zendesk-crm') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  e.preventDefault(); 
    if(!confirm('<?php esc_html_e('Are you sure to remove ?','gravity-forms-zendesk-crm') ?>')){
     return;   
    }
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php esc_html_e('Please Select Field Name','gravity-forms-zendesk-crm') ?>');
     return;   
    }  
...
var text= div.find('.vxc_filter_text');
 ...
 div.find('.cfx_op_sel option').removeAttr('disabled'); 
  ...
  function add_note_sel(){
    jQuery('#crm_note_fields').select2({ placeholder: '<?php esc_html_e('Select Field','gravity-forms-zendesk-crm') ?>'});
} 
...
vx_select();
  ...
  vx_select();
  function vx_select(){
      $('#vx_add_fields_select').select2({ placeholder: '<?php esc_html_e('Select Field','gravity-forms-zendesk-crm') ?>'});
