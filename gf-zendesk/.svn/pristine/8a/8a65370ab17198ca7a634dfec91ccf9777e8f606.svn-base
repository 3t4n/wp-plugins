...
if( !class_exists( 'vxg_zendesk_form' ) ) {
class vxg_zendesk_form extends vxg_zendesk{
...
$feed=get_option($this->id.'_lists',array()); 
  ...
  /**
  * Replace the Gravity Forms form choices with remotely-pulled Zendesk picklist options.
...
//var_dump($crm_fields); die();
 ...
 public function fields_html($id,$object,$refresh=false){
  ...
  if(!is_array($fields)){
      $msg= empty($fields) ? esc_html__('Fields Not Found','gravity-forms-zendesk-crm') : $fields;
  $str = '<div class="field_group_'.$this->id.'"><strong>'.__('Error:', 'gravity-forms-zendesk-crm').'</strong> '.wp_kses_post($msg).'</div>';
  ...
  */
  ...
  if(empty($value['value'])) { continue; }
        $default_html = !empty($value['default']) ?  '<strong class="default"> '.esc_html__('(Default)', 'gravity-forms-zendesk-crm').'</strong>' : '';
  ...
  */
  ...
  <tr>
  <th scope='col' class='crm_col_heading'>" . esc_html__("Pickist Field", 'gravity-forms-zendesk-crm') . "</th>
  <th scope='col' class='crm_col_heading'>" . esc_html__("Form Fields", 'gravity-forms-zendesk-crm') . "</th>
  </tr>
  </thead>
  <tbody>".$str."
  </tbody>
  ...
  $str='<div><b>'.__('No Options Found','gravity-forms-zendesk-crm').'</b></div>';    
  }
  /// $str = str_replace(array("\n", "\t", "\r"), '', str_replace("'", "\'", $str));
...
//  toggle_choices(check_box);  
  ...
  */
  ...
  $tooltips['vx_map_live_'.$this->id] = sprintf(__('%sUpdate from Zendesk %s If you update a picklist in Zendesk, the modifications will be added to your form without having to edit the field Choices in Gravity Forms. You will not be able to edit the Choices in Gravity Forms, you can only update them in Zendesk. The order of Choices as well as the default values are determined by the Zendesk picklist field settings.', 'gravity-forms-zendesk-crm'), '<h6>','</h6>');
  $tooltips['vx_map_once_'.$this->id] = sprintf(__('%sUpdate from Zendesk %s Field Choices will not be updated live from Zendesk and are editable in Gravity Forms. If you make an edit in Zendesk, it will not be updated in your form.', 'gravity-forms-zendesk-crm'), '<h6>','</h6>');
  return $tooltips;
  }
  /**
  ...
  */
  ...
  <input type="checkbox" autocomplete="off" class="<?php echo esc_attr($this->id) ?>_map_enabled crm_map_enabled" id="vx_check_<?php echo esc_attr($this->id) ?>" name="crm_map_enabled" value="<?php echo esc_attr($this->id) ?>" /> <?php esc_html_e("Enable Zendesk Field Mapping?", 'gravity-forms-zendesk-crm'); ?>
  </label>
  
  <div id="<?php echo esc_attr($this->id) ?>_map_ui" style="display: none;">
  
  <label for="<?php echo esc_attr($this->id) ?>_map_type_live">
  <input type="radio" class="<?php echo esc_attr($this->id) ?>_map_type crm_map_type" id="<?php echo esc_attr($this->id) ?>_map_type_live" name="<?php echo esc_attr($this->id) ?>_map_type" value="live" /> <?php esc_html_e("Live Remote Field Mapping ", 'gravity-forms-zendesk-crm'); gform_tooltip("vx_map_live_".$this->id); ?>
  <span class="howto" style="padding-left:1.25em;"><?php esc_html_e("Field Choices will be synced from Zendesk picklist values.", 'gravity-forms-zendesk-crm'); ?></span>
  </label>
  <label for="<?php echo esc_attr($this->id) ?>_map_type_once">
  <input type="radio" class="<?php echo esc_attr($this->id) ?>_map_type crm_map_type" id="<?php echo esc_attr($this->id) ?>_map_type_once" name="<?php echo esc_attr($this->id) ?>_map_type" value="once" /> <?php esc_html_e("Only Populate Choices ", 'gravity-forms-zendesk-crm'); gform_tooltip("vx_map_once_".$this->id) ?>
  <span class="howto" style="padding-left:1.25em;"><?php esc_html_e("Field Choices will not be updated live and are editable.", 'gravity-forms-zendesk-crm'); ?></span>
  </label>
    <label for="crm_accounts_list" style="width: 100px;" class="inline"><?php esc_html_e("Choose Account", 'gravity-forms-zendesk-crm'); ?></label>
  ...
  <option value=""><?php esc_html_e("Select a Zendesk Account", 'gravity-forms-zendesk-crm'); ?></option>
  <?php
 ...
 public function object_area($id,$object){
     ...
     $lists=array('ticket'=>'Ticket','user'=>'Users');   
     ?>
       <label for="crm_object_list" style="width: 100px;" class="inline"><?php esc_html_e("Choose Object", 'gravity-forms-zendesk-crm'); ?></label>
  <?php
  if(!is_array($lists)) {
  echo esc_html__("Could not load Zendesk objects.", 'gravity-forms-zendesk-crm');
  } else { ?>
  ...
  <option value=""><?php esc_html_e("Select a Zendesk Object", 'gravity-forms-zendesk-crm'); ?></option>
  <?php
