...
class InvoiceJapan {
	...
	public function order_details( $order ) {
		...
		if ( is_user_logged_in() ) {
			...
			$resending_status = 'no_resending_invoice';
			if ( 'pending' === $status ||
					'processing' === $status ||
					'on-hold' === $status ||
					'completed' === $status ) {
				...
				if ( $gateway_mail_timings[ $order->get_payment_method() ][ $status ] ) {
					$resending_status = 'resending_invoice';
				}
			}

			$asset_file = plugin_dir_path( __DIR__ ) . 'guten/build/resending/index.asset.php';
			...
			wp_enqueue_style(
				...
				plugin_dir_url( __DIR__ ) . 'guten/build/resending/index.css',
				array( 'wp-components' ),
				'1.0.0',
			);
			...
			wp_enqueue_script(
				'invoicejapan_resending',
				plugin_dir_url( __DIR__ ) . 'guten/build/resending/index.js',
				$asset['dependencies'],
				$asset['version'],
				array(
					'in_footer' => true,
	...
	public function plugin_options() {
		...
		if ( version_compare( $wp_version, $requires, '>=' ) ) {
			...
			$admin_screen = sprintf( 'この画面を表示するには、WordPress %s 以上が必要です。', $requires );
		}
		printf( '<div class="wrap" id="invoicejapanadmin">%s</div>', esc_html( $admin_screen ) );
		echo '<div class="wrap" id="invoicejapanaddonadmin"></div>';
	}
	...
	public function admin_scripts( $hook_suffix ) {
		...
		$asset_file = plugin_dir_path( __DIR__ ) . 'guten/build/admin/index.asset.php';
		...
		wp_enqueue_style(
			'invoicejapan',
			plugin_dir_url( __DIR__ ) . 'guten/build/admin/index.css',
			array( 'wp-components' ),
			'1.0.0',
		);
		...
		wp_enqueue_script(
			'invoicejapan',
			plugin_dir_url( __DIR__ ) . 'guten/build/admin/index.js',
			$asset['dependencies'],
			$asset['version'],
			array(
				'in_footer' => true,
