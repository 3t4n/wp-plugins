...
class Partnero_Api {
    ...
    /**
     * Holds the api key type to pass in calls
     *
     * @since    2.0.0
     * @access   protected
     * @var      string    $api_key_type
     */
    protected static $api_key_type;
    ...
    /**
     * Set key in api_key_type variable.
     * Can be use to set api key type from external class.
     *
     * @since  2.0.0
     * @param    string    $type
     */
    public static function set_api_key_type($type) {
        if (self::$api_key_type !== $type) {
            // Reset the API key if the type is changed
            self::$api_key = null;
        }
    ...
    public static function get_api_key() {
        ...
        if( empty( self::$api_key ) && !empty( self::$api_key_type ) && Partnero_Util::has_option( self::$api_key_type, 'api_key' ) ) {
            self::$api_key = Partnero_Util::get_option(self::$api_key_type, 'api_key');
        }
    ...
    /**
     * Sync WooCommerce customers to connected Refer-a-friend program
     *
     * @since 2.0.1
     *
     * @param $customers
     * @return mixed|null
     */
    public static function sync_customers_call($customers) {
        $url = self::$base_url . '/customers/woocommerce/sync';
        $data = ['data' => $customers];

        $result = self::common_request_call( $url, 'POST', $data);
