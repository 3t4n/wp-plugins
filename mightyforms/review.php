...
function mf_ask_to_leave_review_handler()
{
    ...
    if (!get_user_meta($user_id, 'mf_next_schedule_review_notice_time')) { ?>
        <div class="notice notice-success is-dismissible" style="padding-bottom: 1rem;">
            <p><?php esc_html_e('Thank you for using MightyForms. It would help us a great deal if you could give us your feedback on WP directory. We are hoping we earned your 5-stars! 😉', 'mightyforms'); ?></p>

            <div style="display: flex; flex-direction: column;">
                <a data-rate-action="do-rate" href="?mf_review=do" style="font-size: 16px;">
                    <img src="<?php echo esc_url(plugin_dir_url(__FILE__) . '/images/thumbs-up.svg'); // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>"
                         class="mf-feedback-icon"><?php esc_html_e('Sure!', 'mightyforms') ?>
                </a>
                <a data-rate-action="later" href="?mf_review=later">
                    <img src="<?php echo esc_url(plugin_dir_url(__FILE__) . '/images/later.svg'); // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>"
                         class="mf-feedback-icon"><?php esc_html_e('No, maybe later', 'mightyforms') ?>
                </a>
            </div>
        </div>
    <?php }
    ...
    if ($user_id && isset($_GET['mf_review'])) {
        ...
        if ($_GET['mf_review'] === 'later') {
            ...
            if (!empty($_SERVER['HTTP_REFERER'])) {
                $return_url = sanitize_url(stripslashes_deep($_SERVER['HTTP_REFERER']));
            } else {
                $return_url = admin_url('admin.php?page=mightyforms-forms');
            }
