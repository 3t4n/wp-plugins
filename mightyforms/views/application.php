...
function mightyforms_run_application()
{
    ...
    <div class="mf-main-block">
        ...
        <div class="application-box">
            ...
            <iframe id="mf" src="<?php echo esc_url($iframe_src); ?>" frameborder="0" style="width: 100%;"></iframe>
        </div>
    </div>
    <?php
}
...
function upsert_user_api_key()
{
    try {
        if (!current_user_can('manage_options') || empty($_POST['nonce'])
            || !wp_verify_nonce(sanitize_text_field(stripslashes_deep($_POST['nonce'])), 'user_api_key')
        ) {
            throw new Exception('Access denied');
        }

        if (isset($_POST['userApiKey'])) {
            $api_key = esc_sql(sanitize_text_field(stripslashes_deep($_POST['userApiKey'])));
