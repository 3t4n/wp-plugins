...
The XPay Build extension for Wordpress/WooCommerce enables you to integrate the XPay payment gateway into your website without additional implementations. XPay Build is a solution that allows, through the use of a Javascript SDK client, to host the payment form within your site, without redirecting the customer to the XPay checkout page, but at the same time avoiding to manage the card data.
...
1. Check the compatibility of the Nexi XPay Build plugin with the version of Woocommerce/Wordpress installed on your site.
...
2. Click on Nexi XPay Build to proceed with the configuration.
