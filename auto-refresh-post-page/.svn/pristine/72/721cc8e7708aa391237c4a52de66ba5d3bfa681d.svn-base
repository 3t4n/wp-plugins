...
define('ARPP_AUTO_REFRESH_VERSION', '1.0.1');
...
function ARPP_store_refresh_settings()
{
    ...
    foreach ($data as $key => $value) {
        ...
        // Check for time unit and duration
        if (strpos($key, 'time_unit-') === 0 || strpos($key, 'duration-') === 0) {
            $new_data[$key] = $value;
        } else {
            $new_data[$key] = $value === '1' ? 1 : 0;
        }
    }
...
function ARPP_setrefreshtime()
{
    ...
    foreach ($array_from_db as $key => $value) {
        if (strpos($key, 'duration-') === 0) {
            $post_type = str_replace('duration-', '', $key);
            $seconds = absint($value);

            $time_unit_key = "time_unit-" . $post_type;
            if (isset($array_from_db[$time_unit_key])) {
                switch ($array_from_db[$time_unit_key]) {
                    case 'minutes':
                        $seconds *= 60;
                        break;
                    case 'hours':
