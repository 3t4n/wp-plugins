<?php
/*
Plugin Name: Auto Poster to social networks
...
add_filter('cron_schedules', 'hsauto_bluesky_custom_cron_intervals_hs');
function hsauto_bluesky_custom_cron_intervals_hs($schedules) {
    $schedules['every_five_minutes'] = [
        'interval' => 300, // 5 minute
        'display' => esc_html__( 'Every 5 Minutes', 'auto-poster-to-networks' ),
    ];
    return $schedules;
}

function hsauto_tables_of_bluesky_network_hs() {
    global $wpdb;
...
register_activation_hook(__FILE__, 'hsauto_tables_of_bluesky_network_hs');
// Register a custom cron schedule globally
add_filter('cron_schedules', 'hsauto_bluesky_custom_cron_schedules_hs');
function hsauto_bluesky_custom_cron_schedules_hs($schedules) {
    // Retrieve the user-defined cron interval (default 5 minutes)
    $cron_interval = (int) get_option('bluesky_hsauto_cron_time', 5);
    ...
    esc_html__( 'Every %d minutes', 'auto-poster-to-networks' ),
    $cron_interval
), // Correct
    ];
...
register_activation_hook(__FILE__, 'hsauto_bluesky_schedule_cron_job_hs');
function hsauto_bluesky_schedule_cron_job_hs() {
    // Ensure custom cron schedule is available
    add_filter('cron_schedules', 'hsauto_bluesky_custom_cron_schedules_hs');

    // Clear any existing scheduled events
    hsauto_bluesky_clear_cron_job_hs();
...
register_deactivation_hook(__FILE__, 'hsauto_bluesky_clear_cron_job_hs');
function hsauto_bluesky_clear_cron_job_hs() {
...
add_action('update_option_bluesky_hsauto_cron_time', 'bluesky_on_cron_time_change', 10, 2);
function bluesky_on_cron_time_change($old_value, $new_value) {
    ...
    hsauto_bluesky_schedule_cron_job_hs();
}
// Register a custom cron schedule globally for refreshing tokens
add_filter('cron_schedules', 'hsauto_bluesky_custom_refresh_token_schedules_hs');
function hsauto_bluesky_custom_refresh_token_schedules_hs($schedules) {
...
register_activation_hook(__FILE__, 'hsauto_bluesky_schedule_refresh_token_cron_job_hs');
function hsauto_bluesky_schedule_refresh_token_cron_job_hs() {
    // Ensure the custom cron schedule is available
    add_filter('cron_schedules', 'hsauto_bluesky_custom_refresh_token_schedules_hs');

    // Clear any existing scheduled events
    hsauto_bluesky_clear_refresh_token_cron_job_hs();
...
register_deactivation_hook(__FILE__, 'hsauto_bluesky_clear_refresh_token_cron_job_hs');
function hsauto_bluesky_clear_refresh_token_cron_job_hs() {
...
add_action('update_option_bluesky_refresh_token_interval', 'hsauto_bluesky_on_refresh_token_interval_change_hs', 10, 2);
function hsauto_bluesky_on_refresh_token_interval_change_hs($old_value, $new_value) {
    ...
    hsauto_bluesky_schedule_refresh_token_cron_job_hs();
}



// Include necessary files
require_once plugin_dir_path(__FILE__) . 'includes/hsauto_admin.php';
require_once plugin_dir_path(__FILE__) . 'includes/hsauto_ajax.php';
require_once plugin_dir_path(__FILE__) . 'includes/hsauto_posting_to_bluesky.php';
require_once plugin_dir_path(__FILE__) . 'includes/hsauto_cron.php';
// Enqueue scripts and styles
function hsauto_view_post_enueue_hs() {
...
add_action('admin_enqueue_scripts', 'hsauto_view_post_enueue_hs');

function hsauto_bluesky_poster_enqueue_scripts_hs() {
    wp_enqueue_script(
...
add_action('admin_enqueue_scripts', 'hsauto_bluesky_poster_enqueue_scripts_hs');
function hsauto_bluesky_enqueue_styles_hs() {
...
add_action('admin_enqueue_scripts', 'hsauto_bluesky_enqueue_styles_hs');
add_action('wp_enqueue_scripts', 'hsauto_bluesky_enqueue_styles_hs');

function hsauto_media_uploader_enqueue_script_hs() {
...
add_action('admin_enqueue_scripts', 'hsauto_media_uploader_enqueue_script_hs');