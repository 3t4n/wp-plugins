...
add_action('admin_menu', 'hsauto_bluesky_poster_menu_hs');
function hsauto_bluesky_poster_menu_hs() {
    add_menu_page(
        'Bluesky Automated Poster',
        'Bluesky Poster',
        'manage_options',
        'bluesky-poster',
        'hsauto_bluesky_poster_dashboard_hs',
        'dashicons-megaphone',
        20
    );
    ...
    add_submenu_page(
        ...
        'hsauto_bluesky_manage_networks_hs'
    );
    ...
    add_submenu_page(
        ...
        'hsauto_bluesky_manage_posts_hs'
    );
    ...
    add_submenu_page(
        ...
        'manage_options',
        'bluesky-settings',
        'hsauto_bluesky_settings_page_hs'
    );
}

function hsauto_bluesky_poster_dashboard_hs(){
    global $wpdb;
        ...
        <table class="wp-list-table widefat fixed striped">
            ...
            <tbody>
                <tr>
                    ...
                    <td><?php echo esc_html( $total_posted_posts ) ? esc_html( $total_posted_posts ) : 'No data found'; ?></td>
                </tr>
                <tr>
                    <td>Total Scheduled Posts</td>
                    <td><?php echo esc_html($total_scheduled_posts) ? esc_html($total_scheduled_posts) : 'No data found'; ?></td>
                </tr>
                <tr>
                    <td>Total Networks</td>
                    <td><?php echo esc_html($total_networks) ? esc_html($total_networks) : 'No data found'; ?></td>
                </tr>
            </tbody>
        </table>
    </div>
...
function hsauto_bluesky_manage_networks_hs() {
    global $wpdb;
            ...
            <tbody>
                <?php if ($networks) : ?>
                    <?php foreach ($networks as $network) : ?>
                        <tr>
                            <td><?php echo esc_attr($network->id); ?></td>
                            <td><?php echo esc_attr($network->network_name); ?></td>
                            <td>
                                <!-- Add a toggle switch for the status -->
                                <label class="switch">
                                    <input type="checkbox" class="toggle-status" data-id="<?php echo esc_attr($network->id); ?>" <?php echo esc_attr($network->status) == 1 ? 'checked' : ''; ?>>
                                    <span class="slider round"></span>
                                </label>
                            </td>
                            <td>
                                <a href="#" class="edit-network" data-id="<?php echo esc_attr($network->id); ?>">Edit</a> |
                                <a href="#" class="delete-network" data-id="<?php echo esc_attr($network->id); ?>">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else : ?>
        ...
        if ($networks) {
            foreach ($networks as $network) {
                ...
                <div class="network-icon" data-id="<?php echo esc_attr($network->id); ?>" title="<?php echo esc_attr($network->network_name); ?>">
                    <img src="<?php echo esc_url( $network->avatar); ?>" alt="<?php echo esc_attr($network->network_name); ?>" id="avatar" />
                </div>
                <?php
            }
        } else {
...
function hsauto_bluesky_manage_posts_hs() {
...
add_action('admin_init', 'hsauto_bluesky_register_settings_hs');
function hsauto_bluesky_register_settings_hs() {
    // Register settings
    register_setting(
        'bluesky_hsauto_settings_group',
        'bluesky_hsauto_cron_time',
        array(
            'type'              => 'string', 
    ...
    add_settings_section(
        ...
        'hsauto_bluesky_settings_section_callback_hs',
        'bluesky-settings'
    );
    ...
    add_settings_field(
        ...
        'hsauto_bluesky_hsauto_cron_time_callback_hs',
        'bluesky-settings',
        'bluesky_settings_section'
    );
    ...
    add_settings_field(
        ...
        'hsauto_bluesky_delete_posts_callback_hs',
        'bluesky-settings',
        'bluesky_settings_section'
    );
    ...
    add_settings_field(
        ...
        'hsauto_bluesky_refresh_token_callback_hs',
        'bluesky-settings',
        'bluesky_settings_section'
    );
}

function hsauto_bluesky_settings_section_callback_hs() {
    echo '<p>Configure the settings for the Bluesky Automated Poster plugin.</p>';
}

// Callback for Cron Time
function hsauto_bluesky_hsauto_cron_time_callback_hs() {
    $value = get_option('bluesky_hsauto_cron_time', 5); // Default to 5 minutes
    echo '<input type="number" name="bluesky_hsauto_cron_time" value="' . esc_attr($value) . '" min="1" step="1">';
    echo '<p class="description">Enter the cron time interval in minutes. Default is 5 minutes.</p>';
}

// Callback for Delete Posts Option
function hsauto_bluesky_delete_posts_callback_hs() {
...
function hsauto_bluesky_refresh_token_callback_hs() {
...
function hsauto_bluesky_settings_page_hs() {
    ...
    settings_fields('bluesky_hsauto_settings_group');
    do_settings_sections('bluesky-settings');
    submit_button();
    echo '</form>';
    echo '</div>';
}
function hsauto_bluesky_view_posts_hs() {
    $image_url = plugins_url('images/loader.gif', __FILE__)
    ?>
    <div id="message-container"></div>
    <div id="loader-overlay" style="display: none;">
