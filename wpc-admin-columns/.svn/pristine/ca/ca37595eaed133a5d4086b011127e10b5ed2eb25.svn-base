...
class Wpcac_Backend {
	...
	function enqueue_scripts( $hook ) {
		if ( apply_filters( 'wpcac_ignore_backend_scripts', false, $hook ) ) {
			return null;
		}
				...
				switch ( $type ) {
					...
					default:
						if ( is_array( $value ) ) {
							...
							$response['status'] = 1;
							$response['value']  = (string) apply_filters( 'wpcac_custom_column_value', $value, $field, $type, $post_id );
						}
				}
			}
		}
				...
				case 'custom_field':
					if ( ! empty( $field ) ) {
						...
						switch ( $data_type ) {
							...
							default:
								if ( is_string( $value ) ) {
									$column_content = '<div class="wpcac-value" data-id="' . esc_attr( $postid ) . '" data-type="' . esc_attr( $saved_columns[ $column ]['data_type'] ) . '" data-field="' . esc_attr( $field ) . '">' . esc_html( apply_filters( 'wpcac_custom_column_value', $value, $field, $saved_columns[ $column ]['data_type'], $postid ) ) . '</div>';
								} else {
