 ...
 *
 */
class Oneto_Companion_Activator {

	public static function activate() {

        $fresh_site_activate = get_option( 'fresh_site_activate' );
		if ( (bool) $fresh_site_activate === false ) {
			        require oneto_companion_plugin_dir . 'inc/widgets/widget.php';
					$pages = array( esc_html__( 'Home', 'oneto-companion' ), esc_html__( 'Blog', 'oneto-companion' ) );
