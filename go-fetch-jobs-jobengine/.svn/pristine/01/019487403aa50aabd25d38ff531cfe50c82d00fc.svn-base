jQuery(document).ready(function($) {
	...
	$(document).on( 'click', '.refresh-templates', function(e) {
		...
		if ( ! $('.goft_je.processing.templates').length ) {

			template_actions.hide().after('<div class="goft_je processing templates">&nbsp;</div>');

			$('.import-feed').prop( 'disabled', true );
		}

		var data = {
			action: 'goft_je_update_templates_list',
			_ajax_nonce: goft_je_admin_l18n.ajax_nonce,
		};

		$.post( goft_je_admin_l18n.ajaxurl, data, function( response ) {

			if ( undefined !== response && undefined !== response.templates ) {

				$.each( response.templates, function( key, value ) {

					if ( $("#templates_list option[value='" + value + "']").length ) {
						return;
					}
			...
			$('.goft_je.processing.templates').remove();
	...
	$(document).on( 'click', '.import-feed', function(e) {
		...
		if ( ! url ) {
			alert( goft_je_admin_l18n.msg_specify_valid_url );
			return;
		}

		enable_import( false );

		$('.goft_je.no-jobs-found').remove();

		if ( ! $('.goft_je.processing.feed').length ) {
			$('input[name=import_feed]').hide();
			$('input[name=import_feed]').after('<span class="goft_je processing feed">&nbsp;</span>').after('<a class="button-secondary cancel-feed-load">' + goft_je_admin_l18n.cancel_feed_load + '</a>');
			$('input', template_actions).prop( 'disabled', true );
		}

		var data = {
			action: 'goft_je_import_feed',
			url: url,
		...
		xhr = $.post( goft_je_admin_l18n.ajaxurl, data, function( response ) {
			...
			if ( undefined !== response ) {
				...
				if ( undefined !== response.error ) {
					...
					$('input[name=load_images]').closest('p').after('<span class="goft_je no-jobs-found feed"> ' + goft_je_admin_l18n.msg_invalid_feed + ': ' + response.error );

				} else if ( response.total_items > 0 ) {

					$('.goft_je_jobs_found').remove();

					$('.goft_je_table').html( response.sample_item );

					$('.goft_je_table').after( '<h4 class="goft_je_jobs_found code">' + goft_je_admin_l18n.msg_jobs_found + ': ' + response.total_items + '</h4>');
					...
					$('input[name=load_images]').closest('p').after('<span class="goft_je no-jobs-found feed"> ' + goft_je_admin_l18n.msg_no_jobs_found + '</span>');
				}

			}

			$('.goft_je.processing').remove();
	...
	$(document).on( 'click', '.save-template', function(e) {
		...
		if ( ! template_name ) {
			alert( goft_je_admin_l18n.msg_template_missing );
			return;
		}

		if ( ! $('.goft_je.processing.save').length ) {
			$('.save-template').hide();
			$('.save-template').after('<span class="goft_je processing save">&nbsp;</span>');
		}

		var data = {
			action:         'goft_je_save_template',
			...
			_wpnonce:        goft_je_admin_l18n.ajax_save_nonce,
		};
		...
		$.post( goft_je_admin_l18n.ajaxurl, data, function( response ) {
			...
			if ( undefined !== response ) {
				$('.save-template').show();
				$('.save-template').after('<span class="template-saved-msg"> ' + goft_je_admin_l18n.msg_template_saved + '</span>');

				$('.template-saved-msg').delay(2000).fadeOut();

				if ( ! $("#templates_list option[value='" + template_name + "']").length ) {
			...
			$('.goft_je.processing.save').remove();
	...
	$(document).on( 'change', '#templates_list', function() {
		...
		var data = {
			action: 'goft_je_load_template_content',
			template: $(this).val(),
			_ajax_nonce: goft_je_admin_l18n.ajax_nonce,
		};
	...
	$(document).on( 'change', '#providers_list', function() {
		...
		$('.goft_je.processing.providers').remove();
		...
		if ( ! $('.goft_je.processing.templates').length ) {
			$('#providers_list').after('<div class="goft_je processing providers">&nbsp;</div>');
		}

		var data = {
			action: 'goft_je_load_provider_info',
			provider: $(this).val(),
			_ajax_nonce: goft_je_admin_l18n.ajax_nonce,
		};
	...
	$(document).on( 'click', 'a.provider-rss, a.provider-rss-custom', function(e) {
		...
		$('input[name=rss_feed_import]').val( $(this).attr('href') );

		$(this).after('<span class="rss-copied-msg"> ' + goft_je_admin_l18n.msg_rss_copied + '</span>').fadeIn();
		$('.rss-copied-msg').delay(1000).fadeOut();
	...
	$('.section-expand').on( 'click', function(e) {
		...
		$( context ).closest('tr').toggle( function() {
			...
			if ( $(this).is(':visible') ) {
				$( section_a ).text( goft_je_admin_l18n.simple );
			} else {
				$( section_a ).text( goft_je_admin_l18n.advanced );
			}
	...
	function auto_fill_custom_fields_defaults( provider ) {
		...
		if ( undefined !== goft_je_admin_l18n.default_query_args ) {
			all_query_args.push( goft_je_admin_l18n.default_query_args );
		}
		...
		$( context ).each( function() {
			...
			$( 'select, input[type!="button"], textarea', tr ).each( function() {
				...
				if ( $(this).is('select') ) {
					...
					value = $(this).is(':checked') ? goft_je_admin_l18n.label_yes : goft_je_admin_l18n.label_no;
				} else if ( $(this).val() ) {
					value = $(this).val();
				...
				if ( ! $( '.' + selector ).length ) {
					...
					var sel_values_placeholder =
						'<tr class="section-advanced temp-tr-hide">' +
							'<td><a href="#" class="section-expand" expand="' + section + '">' + goft_je_admin_l18n.msg_advanced + '</a></td>' +
							'<td class="tip">&nbsp;</td>' +
			...
			var sel_values = '<p class="goft-basic"><strong>' + label  + ': </strong>' + value + '</p> ' ;
	...
	// Single page schedules.

	$('#_goft_je_period_custom').closest('tr').hide();

	$(document).on( 'change', '#_goft_je_period', function() {

		if ( 'custom' == $(this).val() ) {
			$('#_goft_je_period_custom' ).closest('tr').fadeIn();
		} else {
			$('#_goft_je_period_custom' ).closest('tr').hide();
		}
	...
	$('#_goft_je_period').trigger('change');
