 ...
 */
class GoFetch_JobEngine_Importer
{
    ...
    {
        if ( empty($_POST['_ajax_nonce']) || !wp_verify_nonce( $_POST['_ajax_nonce'], 'goft_je_nonce' ) ) {
            return $feed;
        }
        if ( empty($_REQUEST['action']) || 'goft_je_import_feed' !== $_REQUEST['action'] ) {
            return $feed;
        }
    ...
    public static function import_feed( $url, $fetch_images = false, $cache = true )
    {
        ...
        if ( !empty($parsed_url['host']) ) {
            ...
            $provider_id = 'unknown';
            $host = __( 'Unknown', 'gofetch-je' );
        }
        
        $provider = GoFetch_JobEngine_RSS_Providers::get_providers( $provider_id );
        // Set provider data.
        $defaults = array(
            'id'          => $provider_id,
        ...
        $valid_item_tags = GoFetch_JobEngine_RSS_Providers::valid_item_tags();
        $valid_regexp_tags = GoFetch_JobEngine_RSS_Providers::valid_regexp_tags();
        ...
        foreach ( $items as $item ) {
            ...
            $new_items[] = apply_filters( 'goft_je_rss_item', $new_item );
        }
    ...
    public static function import( $items, $params )
    {
        ...
        if ( empty($items) ) {
            trigger_error( __( 'No items found!', 'gofetch-je' ) );
            return $imported;
        }
        
        $defaults = array(
            'post_type'     => GoFetch_JobEngine()->parent_post_type,
            'post_author'   => 1,
            'tax_input'     => array(),
            'meta'          => array(),
            'usermeta'      => array(),
        ...
        $items = self::_get_unique_post_items( $items, array(
    ...
    {
        do_action( 'goft_je_before_insert_job', $item, $params );
        ...
        do_action( 'goft_je_insert_job_failed', $item, $params );
        return false;
    }
     ...
     *
     * @uses apply_filters() Calls 'goft_je_item_meta_value'.
     */
...
new GoFetch_JobEngine_Importer();