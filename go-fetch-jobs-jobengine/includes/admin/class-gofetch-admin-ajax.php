 ...
 * Ajax class.
 */
class GoFetch_JobEngine_Ajax
{
    public function __construct()
    {
        add_action( 'wp_ajax_goft_je_load_template_content', array( $this, 'load_template_content' ) );
        add_action( 'wp_ajax_goft_je_update_templates_list', array( $this, 'update_templates_list' ) );
        add_action( 'wp_ajax_goft_je_load_provider_info', array( $this, 'load_provider_info' ) );
        add_action( 'wp_ajax_goft_je_import_feed', array( $this, 'import_feed' ) );
    }
    ...
    public function load_template_content()
    {
        ...
        if ( !wp_verify_nonce( $_POST['_ajax_nonce'], 'goft_je_nonce' ) ) {
            die( 0 );
        }
        ...
        if ( empty($_POST['template']) ) {
            ...
            $template_settings = $goft_je_options->templates[$template_name];
    ...
    public function load_provider_info()
    {
        ...
        if ( empty($_POST['provider']) ) {
            ...
            $data = array(
                'provider' => GoFetch_JobEngine_RSS_Providers::get_providers( $provider ),
                'setup'    => GoFetch_JobEngine_RSS_Providers::setup_instructions_for( $provider ),
            );
        }
    ...
    public function import_feed()
    {
        ...
        $items = GoFetch_JobEngine_Importer::import_feed( $url, $load_images, $cache = true );
        ...
        extract( $items );
        $args = array(
            'content_type' => GoFetch_JobEngine()->parent_post_type,
        );
        ...
        echo  json_encode( array(
            'provider'    => $provider,
            'sample_item' => ( $total_items ? GoFetch_JobEngine_Sample_Table::display( $args, $sample_item ) : '' ),
