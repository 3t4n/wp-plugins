 ...
 */
class GoFetch_JobEngine_Sample_Table extends GoFetch_JobEngine_HTML_Table
{
    /**
     * Constructor.
     */
    ...
    public static function display( $query_args = array(), $fields = array() )
    {
        ...
        // output the table
        $table = new GoFetch_JobEngine_Sample_Table( $fields, $query_args['content_type'] );
        ob_start();
        $table->show();
        return ob_get_clean();
    }
    ...
    {
        echo  $this->table( $this->fields, array(
            'class' => 'goft_je_table widefat fields',
        ) ) ;
    }
    ...
    public function header( $data )
    {
        ...
        $cells = $this->cells( array( __( 'Fields', 'gofetch-je' ), __( 'Sample Content', 'gofetch-je' ) ), $atts, 'td' );
    ...
    protected function row( $item = array(), $atts = array() )
    {
        ...
        if ( empty($item['data']) ) {
            ...
            $item['data'] = __( '[serialized data]', 'gofetch-je' );
        }
        ...
        $cells = $this->cells( array( html( 'span', array(
            'class' => 'field',
        ), ucfirst( $item['name'] ) ) . html( 'input', array(
            'type'  => 'hidden',
            'name'  => 'type[' . $item['name'] . ']',
            'value' => $item['type'],
        ) ), html( 'span', array(
    ...
    private function make_fields_struct( $fields, $type = 'custom', $content_type = 'post' )
    {
        ...
        foreach ( $fields as $field => $data ) {
            ...
            $default = array(
                'name'              => $field,
                'type'              => $type,
                'content_data_type' => GoFetch_JobEngine_Helper::get_field_type( $field, $content_type ),
                'data'              => '-',
                'label'             => '',
                'checked'           => false,
            );
