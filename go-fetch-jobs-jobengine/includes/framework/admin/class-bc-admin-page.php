 ...
 */
class bcAdminPage extends scbAdminPage
{
    /**
     ...
     * @return string
     */
    public static function row_wrap(
        $title,
        $content,
        $tip = '',
        $class = ''
    ...
    public function table_row( $field, $formdata = false )
    {
        ...
        if ( empty($field['tip']) ) {
            ...
            $tooltips_support = BC_ToolTips::supports_wp_pointer();
            $tip = html( 'span', array(
        ...
        if ( !empty($field['section_break']) ) {
            return html( 'tr class=\'' . implode( ' ', (array) $atts ) . ' hr-break\'', html( 'th colspan=3', $this->input( $field, $formdata ) ) );
        } else {
            
            if ( !empty($field['line_break']) ) {
                return html(
                    'tr class=\'' . implode( ' ', (array) $atts ) . ' hr-break\'',
                    html( 'th', '' ),
                    html( 'td', '' ),
