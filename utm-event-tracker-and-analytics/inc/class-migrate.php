...
final class Migrate {
	...
	public function __construct() {
		...
		add_action('init', array($this, 'modify_tables'), 1);
	...
	public function modify_tables() {
		global $wpdb;
		$create_ddl = "ALTER TABLE $wpdb->utm_event_tracker_sessions_table ADD `referrer` VARCHAR(255) NOT NULL DEFAULT '' AFTER `landing_page`;";

		require_once(ABSPATH . 'wp-admin/includes/upgrade.php');
		maybe_add_column($wpdb->utm_event_tracker_sessions_table, 'referrer', $create_ddl);

		$create_ddl = "ALTER TABLE $wpdb->utm_event_tracker_events_table ADD `title` VARCHAR(150) NULL AFTER `type`;";
		maybe_add_column($wpdb->utm_event_tracker_events_table, 'title', $create_ddl);
	}
	...
	public function migrate_event_data() {
		...
		$events = $wpdb->get_results("SELECT * FROM $wpdb->utm_event_tracker_events_table WHERE type = 'woocommerce_checkout' OR title is null");
