...
class Utils {
	...
	public static function license_activated() {
		if (!class_exists('\UTM_Event_Tracker_Pro\Upgrade')) {
			return false;
		}

		return \UTM_Event_Tracker_Pro\Upgrade::license_activated();
	}
	...
	public static function get_field_note($prepend = '', $append = '', $utm_source = 'settings', $utm_medium = 'get+pro') {
		...
		if (!self::is_pro_installed()) {
			printf(
				...
				esc_html__('Get the %1$s pro version%2$s to unlock this option.', 'utm-event-tracker'),
				'<a href="https://codiepress.com/plugins/utm-event-tracker-and-analytics-pro/?utm_campaign=utm+event+tracker&utm_source=' . $utm_source . '&utm_medium=' . $utm_medium . '" target="_blank">',
				'</a>',
			);
		}

		if (self::is_pro_installed() && !self::is_pro_activated()) {
			esc_html_e('Activate the "UTM Event Tracker and Analytics Pro" plugin to unlock this option.', 'utm-event-tracker');
		}

		if (self::is_pro_activated() && !self::license_activated()) {
			esc_html_e('Activate the license of "UTM Event Tracker and Analytics Pro" plugin to unlock this option.', 'utm-event-tracker');
		}
