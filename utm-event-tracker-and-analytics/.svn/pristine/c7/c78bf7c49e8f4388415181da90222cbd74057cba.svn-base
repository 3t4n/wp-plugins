...
final class Sessoin_Event {
		...
		<template id="utm-event-tracker-widget-event">
			<div class="utm-event-tracker-widget-item">
				<div class="header">
					...
					<div class="actions">
						<select v-model="type">
							<option value="events"><?php esc_html_e('By Events', 'utm-event-tracker'); ?></option>
				...
				<div class="content-body">
					<table :class="{'table-utm-event-tracker-report no-margin-top': true, loading: loading}">
						<thead>
							<tr v-if="type == 'events'">
								<th class="column-large"><?php esc_html_e('Event Title', 'utm-event-tracker'); ?></th>
								<th :class="['column-100', 'sortable-column', get_sort_column_class('events')]" @click="sort_report('events')"><?php esc_html_e('Total', 'utm-event-tracker'); ?></th>
							</tr>

							<tr v-if="type == 'date'">
								<th :class="['column-large', 'sortable-column', get_sort_column_class('date')]" @click="sort_report('date')"><?php esc_html_e('Date', 'utm-event-tracker'); ?></th>
	...
	public function admin_menu() {
		add_submenu_page(
			'utm-event-tracker',
			__('UTM Event Tracker - Events', 'utm-event-tracker'),
			__('Events', 'utm-event-tracker'),
			'manage_options',
	...
	public function screen() { ?>
		<div class="utm-event-tracker-header">
			<h3><?php esc_html_e('Events', 'utm-event-tracker'); ?></h3>
		</div>
			...
			<div class="utm-event-tracker-keyword-stats-container">
				<events-stats :dates="dates">
					<template #heading="{count}">
						<h4><?php esc_html_e('Top {{count}} Events', 'utm-event-tracker'); ?></h4>
					</template>
				</events-stats>
			</div>

			<session-list-events ref="event_list_table" :dates="dates" :keywords="keywords"></session-list-events>
		</div>
<?php
	}
	...
	public function dashboard_widget($widgets) {
		$widgets['session_event'] = array(
			...
			'title' => __('Events', 'utm-event-tacker'),
		);
	...
	public function widget() {
		...
		echo '<h3>' . esc_html__('Events', 'utm-event-tracker') . '</h3>';
		echo '</template>';
	...
	public function get_events_list() {
		...
		$items = $wpdb->get_results($wpdb->prepare(
			...
			) as views ON events.id = views.session_id
			...
			WHERE events.title LIKE %s AND events.created_on BETWEEN %s AND %s ORDER BY %i %5s LIMIT %d, %d",
