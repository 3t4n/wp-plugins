...
final class Session {
	...
	public static function is_available() {
		return Utils::is_utm_parameter_available() || !empty($_COOKIE['wordpress_utm_event_tracker_session']);
	}
	...
	public static function get_current_session() {
		$session_id = '';
		if (!empty($_COOKIE['wordpress_utm_event_tracker_session'])) {
			$session_id = sanitize_text_field($_COOKIE['wordpress_utm_event_tracker_session']);
		}
	...
	public function __construct($session_data = array()) {
		...
		if (empty($this->referrer) && !empty($_SERVER['HTTP_REFERER'])) {
			$this->referrer = esc_url_raw($_SERVER['HTTP_REFERER']);
		}
	}
	...
	public function save() {
		...
		if ($result) {
			...
			if ($this->is_new() || empty($_COOKIE['wordpress_utm_event_tracker_session'])) {
				$cookie_duration = sprintf('+%d days', Settings::get_instance()->get('cookie_duration', 30));
				setcookie('wordpress_utm_event_tracker_session', $this->get_session_id(), strtotime($cookie_duration), '/');
				$_COOKIE['wordpress_utm_event_tracker_session'] = $this->get_session_id();
			}
	...
	public function add_view() {
		if (!$this->is_available() || wp_doing_ajax() || $this->get_id() <= 0) {
			return;
		}
