...
class Manage_Compare {
    ...
    public function shareable_link(){
        ...
        $ids = $this->get_compared_products();
         ...
         if ( isset( $_GET['evcompare'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $query_perametter_ids = sanitize_text_field( wp_unslash($_GET['evcompare']) ); //phpcs:ignore WordPress.Security.NonceVerification.Recommended
        ...
        ?>
            <div class="ever-compare-shareable-link <?php echo esc_attr( $button_pos );?>">
                ...
                <p style="display: none;" class="evercompare-share-link"><?php echo esc_html($shareablelink); ?></p>
            </div>
        <?php
    ...
    public function get_compared_products_data() {
        ...
        // For shareable link
        $shareable_link = ever_compare_get_option('enable_shareable_link','ever_compare_table_settings_tabs','off');
        if ( ( $shareable_link === 'on' ) && isset( $_GET['evcompare'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $query_perametter_ids = sanitize_text_field( wp_unslash($_GET['evcompare']) ); //phpcs:ignore WordPress.Security.NonceVerification.Recommended
            if( !empty( $query_perametter_ids ) ){
                $ids = explode( ',', $query_perametter_ids );
            }
    ...
    public function compare_display_field( $field_id, $product ) {
        ...
        switch ( $type ) {
                ...
                ?>
                    <div class="htcompare-primary-content-area">
                        ...
                        <a href="<?php echo esc_url(get_permalink( $product['id'] )); ?>" class="htcompare-product-image">
                            <?php echo wp_kses_post($product['primary']['image']); ?>
                        </a>
                    </div>
                <?php
                break;
            ...
            case 'title':
                echo '<a href="'.esc_url(get_permalink( $product['id'] )).'" class="htcompare-product-title">'.esc_html($product[ $field_id ]).'</a>';
                break;
            ...
            case 'add_to_cart':
                echo wp_kses_post(apply_filters( 'htcompare_add_to_cart_btn', $product[ $field_id ] ));
                break;
            ...
            case 'weight':
                if ( $product[ $field_id ] ) {
                    ...
                    echo wp_kses_post(wc_format_localized_decimal( $product[ $field_id ] )) . ' ' . esc_attr( $unit );
                } 
                break;
    ...
    public function get_compared_products() {
        ...
        if(!empty( $_COOKIE[ $cookie_name ] )) {
            return json_decode( sanitize_text_field(wp_unslash( $_COOKIE[ $cookie_name ] )), true );
        }
        return [];
    }
