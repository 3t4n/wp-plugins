...
class Recommended_Plugins {
    ...
    function __construct( $args ) {
        ...
        $this->menu_label        =  !empty( $args['menu_label'] ) ? $args['menu_label'] : esc_html__( 'Recommendations', 'ever-compare' );
    ...
    public function enqueue_assets( $hook_suffix ) {
        ...
        */
        ...
        $localize_vars['buttontxt'] = array(
            'buynow'     => esc_html__( 'Buy Now', 'ever-compare' ),
            'preview'    => esc_html__( 'Preview', 'ever-compare' ),
            'installing' => esc_html__( 'Installing..', 'ever-compare' ),
            'activating' => esc_html__( 'Activating..', 'ever-compare' ),
            'active'     => esc_html__( 'Activated', 'ever-compare' ),
        );
                            ...
                            foreach( $plugins as $plugin ):
                                ...
                                if ( ! is_wp_error( $data ) ):
                                    ...
                                    if ( file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && is_plugin_inactive( $data['location'] ) ) {
                                        ...
                                        $button_text    = esc_html__( 'Activate', 'ever-compare' );

                                    // Not Installed.
                                    } elseif ( ! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) ) {

                                        $button_classes = 'button htrp-install-now';
                                        $button_text    = esc_html__( 'Install Now', 'ever-compare' );

                                    // Active.
                                    } else {
                                        $button_classes = 'button disabled';
                                        $button_text    = esc_html__( 'Activated', 'ever-compare' );
                                    }
                                    ...
                                    <div class="plugin-card htrp-plugin-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
                                        <div class="plugin-card-top">
                                            ...
                                            <div class="desc column-description" style="margin-right: 0;">
                                                ...
                                                <p class="authors">
                                                    <cite><?php echo esc_html__( 'By ', 'ever-compare' ); ?>
                                        ...
                                        <div class="plugin-card-bottom">
                                            <div class="column-updated">
                                                <?php
                                                    if (! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && $plugins_type == 'pro' ) {
                                                        echo '<a class="button button-primary" href="'.esc_url( $details_link ).'" target="'.esc_attr( $target ).'">'.esc_html__( 'Buy Now', 'ever-compare' ).'</a>';
                                                    }else{
                                                ?>
                                            ...
                                            <div class="column-downloaded">
                                                <a href="<?php echo esc_url( $details_link ) ?>" target="<?php echo esc_attr( $target ) ?>" class=" <?php echo esc_attr($modal_class); ?>" ><?php echo esc_html__('More Details', 'ever-compare') ?></a>
    ...
    public function plugin_activation() {
        ...
        if ( ! current_user_can( 'install_plugins' ) || empty( sanitize_text_field( wp_unslash($_POST['location']) ) ) ) {
            wp_send_json_error(
                array(
                    'success' => false,
                    'message' => esc_html__( 'Plugin Not Found', 'ever-compare' ),
                )
            );
        }

        $plugin_location = ! empty( sanitize_text_field(wp_unslash($_POST['location'])) ) ? sanitize_text_field(wp_unslash($_POST['location'])) : '';
        ...
        wp_send_json_success(
            array(
                'success' => true,
                'message' => esc_html__( 'Plugin Successfully Activated', 'ever-compare' ),
            )
        );
