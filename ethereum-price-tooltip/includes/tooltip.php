...
// Create tooltip for all elements in content except a, img and figure tags
function add_tooltip_to_eth($the_content)
{
    ...
    foreach ($eth_in_text as $keyword) {
        $pattern = '/(<a\b[^>]*>.*?<\/a>|<img\b[^>]*>|<figure\b[^>]*>.*?<\/figure>)|(?<!\w)' . preg_quote($keyword, '/') . '(?!\w)/iu';
        $the_content = preg_replace_callback($pattern, function($matches) use ($ethAnswerCurrency1, $ethAnswerCurrency2, $selected_currency_1, $selected_currency_2) {
            if (!empty($matches[1]) || !empty($matches[0]) && substr($matches[0], 0, 1) === '[' || preg_match('/<img\b[^>]*>/', $matches[0])) {
                ...
                return '<span class="tooltip" data-tooltip="' . $ethAnswerCurrency1 . ' ' . $selected_currency_1 . ' / ' . $ethAnswerCurrency2 . ' ' . $selected_currency_2 . '">' . $matches[0] . '</span>';
            }
        }, $the_content);
    }
