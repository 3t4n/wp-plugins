...
class TestimonialRest
{
	...
	public function register_routes()
	{
		...
		register_rest_route(
			...
			array(
				'methods' => 'POST',
				'callback' => array($this, 'testimonial_data'),
				'permission_callback' => function () {
					return current_user_can('manage_options');
				},
	...
	public function duplicate_post($post_data)
	{
		...
		if ($post->post_type != 'testimonial') {
			$response->error = true;
			$response->errorMessage = __("Post type is not testimonial");
			die(wp_json_encode($response));
		}
	...
	public function update_options($request)
	{
		...
		$value = isset($request['value']) ? testimonial_recursive_sanitize_arr($request['value']) : '';
		$message = "";
		if (!empty($value)) {
			$status = update_option($name, $value);
			$message = __("Options updated", "testimonial");
		} else {
			$status = false;
			$message = __("Value should not empty", "testimonial");
		}
	 ...
	 * @param WP_REST_Request $post_data Post data.
	 */
	public function testimonial_data($post_data)
	{
	...
	public function post_list($post_data)
	{
		...
		$prevText = !empty($post_data['prevText']) ? $post_data['prevText'] : __("Previous", "testimonial");
		$nextText = !empty($post_data['nextText']) ? $post_data['nextText'] : __("Next", "testimonial");
		$maxPageNum = !empty($post_data['maxPageNum']) ? $post_data['maxPageNum'] : 0;
...
$TestimonialRest = new TestimonialRest();
