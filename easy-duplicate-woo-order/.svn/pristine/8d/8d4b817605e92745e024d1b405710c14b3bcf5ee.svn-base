...
function wizbee_handle_duplicate_order_action($order) {
...
$original_meta_data = $originalOrderItem->get_meta_data();
    ...
    // Store the new order ID in a transient
    set_transient('wizbee_duplicate_order_redirect', $new_order->get_id(), 60 * 5); // Valid for 5 minutes
}

// Clear the transient via AJAX
add_action('wp_ajax_clear_wizbee_duplicate_order_redirect', 'wizbee_clear_duplicate_order_redirect');
function wizbee_clear_duplicate_order_redirect() {
    delete_transient('wizbee_duplicate_order_redirect');
}
