...
class CloudActions implements iCloudActions {
     ...
     * @var string
     */
    private $removeFileFromLocalFlag = false;
    ...
    public function __construct( iCloudManipulator $cloudManipulator ) {
        if ( $options = get_option( UPCASTED_S3_OFFLOAD_SETTINGS ) ) {
            ...
            if ( isset( $options[UPCASTED_KEEP_S3_FILES] ) ) {
                $this->removeFileFromCloudFlag = 'no' === $options[UPCASTED_KEEP_S3_FILES];
            }
            if ( isset( $options[UPCASTED_KEEP_LOCAL_FILES] ) ) {
                $this->removeFileFromLocalFlag = 'no' === $options[UPCASTED_KEEP_LOCAL_FILES];
            }
    ...
    public function upcasted_init( string $bucket ) {
        ...
        update_option( UPCASTED_S3_OFFLOAD_SETTINGS, [
            UPCASTED_KEEP_LOCAL_FILES              => ( !empty( $_POST['keep_local_files'] ) ? sanitize_post( $_POST['keep_local_files'] ) : false ),
            UPCASTED_KEEP_S3_FILES                 => ( !empty( $_POST['keep_s3_files'] ) ? sanitize_post( $_POST['keep_s3_files'] ) : false ),
            UPCASTED_S3_OFFLOAD_INCLUDED_FILETYPES => $includedFiletypes,
        ] );
        $options = get_option( UPCASTED_S3_OFFLOAD_SETTINGS );
        if ( !empty( $_POST['keep_local_files'] ) ) {
            $options[UPCASTED_KEEP_LOCAL_FILES] = sanitize_post( $_POST['keep_local_files'] );
        }
        if ( !empty( $_POST['keep_s3_files'] ) ) {
            $options[UPCASTED_KEEP_S3_FILES] = sanitize_post( $_POST['keep_s3_files'] );
        }
    ...
    public function upcasted_image_make_intermediate_size( string $attachment_path ) : string {
        if ( $this->upcasted_check_mime_type( $attachment_path ) ) {
    ...
    public function upcasted_get_attachment_url( $path, $attachment_id ) : string {
        ...
        if ( is_array( $metadata ) ) {
            if ( array_key_exists( 'bucket', $metadata ) && array_key_exists( 'file', $metadata ) ) {
                return "{$this->cloudManipulatorBasePath}/{$metadata['file']}";
            } else {
                if ( array_key_exists( 'bucket', $metadata ) ) {
     ...
     * @return bool
     */
    public function upcasted_check_mime_type( $attachment_id_or_path ) {
        return true;
    }
