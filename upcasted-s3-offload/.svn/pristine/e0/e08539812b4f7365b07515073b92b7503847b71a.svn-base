...
class AmazonCloudManipulator implements iCloudManipulator
{
    ...
    public function upload(string $bucket, string $destination, string $imagePath)
    {
        if (file_exists($imagePath)) {
            ...
            if (isset(get_option(UPCASTED_S3_OFFLOAD_SETTINGS)[UPCASTED_KEEP_LOCAL_FILES]) && 'no' === get_option(UPCASTED_S3_OFFLOAD_SETTINGS)[UPCASTED_KEEP_LOCAL_FILES]) {
                chmod($imagePath, '777');
    ...
    {
        if ($this->client->listBuckets()['Buckets'] !== null )
    ...
    public function create_bucket(string $bucketName)
    {
        try {
            ...
            echo "Failed to create/update bucket $bucketName with error: " . $exception->getMessage();
