		...
		private static function request( $endpoint, $path, $params = array() ) {
			...
			if ( !is_wp_error( $response ) ) {
				if ( 200 == $response['response']['code'] ) {
					...
					throw new GazChap_WC_GetAddress_Plugin_Exception( $response, $response['response']['message'], intval( $response['response']['code'] ) );
				}
			} else {
				throw new GazChap_WC_GetAddress_Plugin_Exception( $response, $response->get_error_message(), $response->get_error_code() ? intval( $response->get_error_code() ) : 0 );
			}
		}
