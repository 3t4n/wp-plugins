...
<?php
	...
	if($mode=="paypal_normal"){
		$first_name = sanitize_text_field( wp_unslash(isset($_REQUEST['first_name']) ? $_REQUEST['first_name'] : '') );
		$last_name = sanitize_text_field( wp_unslash(isset($_REQUEST['last_name']) ? $_REQUEST['last_name'] : '') );
		$email = sanitize_text_field( wp_unslash(isset($_REQUEST['email']) ? $_REQUEST['email'] : '') );
		...
		$payment_in_currency = sanitize_text_field( wp_unslash(isset($_REQUEST['payment_in_currency']) ? $_REQUEST['payment_in_currency'] : '') );
		$currency_sign = sanitize_text_field( wp_unslash(isset($_REQUEST['currency_sign']) ? $_REQUEST['currency_sign'] : '') );
...
<div class="skt_donation-plugin-container">
	...
	$payment_gatway_result = sanitize_text_field(wp_unslash(isset($_GET['payment_gatway_result']) ? $_GET['payment_gatway_result'] : ''));
	/*if ($payment_gatway_result !='') {
		$payment_gatway_result = sanitize_text_field($_GET['payment_gatway_result']);
	}*/
	if($payment_gatway_result=="result"){ 
			$payment_result_success="";
			$payment_result_success = sanitize_text_field(wp_unslash(isset($_GET['payment_result_success']) ? $_GET['payment_result_success'] : ''));
	
			$payment_gatway = sanitize_text_field(wp_unslash(isset($_GET['payment_gatway']) ? $_GET['payment_gatway'] : ''));
		
			$paymentMessage_failed = sanitize_text_field(wp_unslash(isset($_GET['payment_fail']) ? $_GET['payment_fail'] : ''));
		...
		$tabe_paypal_simple = sanitize_text_field(wp_unslash(isset($_GET['tabe_paypal_simple']) ? $_GET['tabe_paypal_simple'] : ''));
		if($tabe_paypal_simple !=''){
			/*$tabe_paypal_simple = sanitize_text_field($_GET['tabe_paypal_simple']);
			$paypal_payment_id = sanitize_text_field($_GET['paypal_payment_id']);
			$paypal_payer_id = sanitize_text_field($_GET['paypal_payer_id']);*/

			$tabe_paypal_simple = sanitize_text_field(wp_unslash(isset($_GET['tabe_paypal_simple']) ? $_GET['tabe_paypal_simple'] : ''));
			$paypal_payment_id = sanitize_text_field(wp_unslash(isset($_GET['paypal_payment_id']) ? $_GET['paypal_payment_id'] : ''));
			$paypal_payer_id = sanitize_text_field(wp_unslash(isset($_GET['paypal_payer_id']) ? $_GET['paypal_payer_id'] : ''));


			if($tabe_paypal_simple=="paypal_success"){
				esc_html("<h4>Your Transaction Success.Your Transaction ID for this transaction is ".$paypal_payment_id.".</h4>",'skt-donation');
			}
			if($tabe_paypal_simple=="paypal_faild"){
				esc_html("<h4>Your Transaction Faild.Your Transaction ID for this transaction is ".$paypal_payment_id.".</h4>",'skt-donation');
			}
		}
		$paypal_subscription = sanitize_text_field(wp_unslash(isset($_GET['paypal_subscription']) ? $_GET['paypal_subscription'] : ''));
		if($paypal_subscription !=''){
			...
			if($paypal_subscription=="subscription_success"){
				...
				$first_name = sanitize_text_field(wp_unslash(isset($_GET['first_name']) ? $_GET['first_name'] : ''));
				$last_name = sanitize_text_field(wp_unslash(isset($_GET['last_name']) ? $_GET['last_name'] : ''));
				$email = sanitize_text_field(wp_unslash(isset($_GET['email']) ? $_GET['email'] : ''));
				$phone = sanitize_text_field(wp_unslash(isset($_GET['phone']) ? $_GET['phone'] : ''));
				$donation_amount = sanitize_text_field(wp_unslash(isset($_GET['donation_amountdonation_amount']) ? $_GET['paypal_subscription'] : ''));
  ...
  <div> 
 	<?php if(isset($_POST['form_mode'])!=''){?>
	 	<?php if($paymentMessageSucceesed=="payment_Succeesed"){ ?>
	     	<div class="skt_paymentMessage_Successfull"> <?php echo esc_attr( $paymentMessage_Succeesed ); ?> </div>
	     <?php }
	...
	<?php 
	$payment_success = sanitize_text_field(wp_unslash(isset($_GET['payment_success']) ? $_GET['payment_success'] : ''));
	if ($payment_success !='') {
		$payment_success  = sanitize_text_field(wp_unslash(isset($_GET['payment_success']) ? $_GET['payment_success'] : ''));
		echo esc_attr ( $payment_success );
	}	
	?>
