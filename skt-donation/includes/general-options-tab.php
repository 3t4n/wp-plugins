<?php
  function sktdonation_general_optiontab(){
    if (!current_user_can('administrator')) {
      wp_die(esc_attr_e('You do not have sufficient permissions to access this page.','skt-donation'));
    }else{
?>
<div>
  ...
  <table id="skt_donation_myTable" cellspacing="0" cellpadding="0">
    ...
    <?php
      ...
      $get_donation_list = $wpdb->get_results ( "SELECT *, IF(subscription_normal='subscriptions', 'Subscriptions', 'Normal') AS subscriptions_status FROM $donation_amt_table" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
      ...
      <tr>
        <td>
            <div class="skt-donations-form-row">
              <div class="skt-donations-form-column first-name">
                <p><?php echo esc_attr($get_donation_member->customer_firstname);?></p>
              </div>
              <div class="skt-donations-form-column last-name">
                ...
                <p>
                  ...
                  <?php 
                    if($get_donation_member->duration_of_subscription !=''){
                      echo '('.esc_attr($get_donation_member->duration_of_subscription).')';
                    }
                  ?>
                </p>
              </div>
              <div class="skt-donations-form-column transaction-id">
                <p>
                  <?php if( $mode == "paypal"){ echo esc_attr($get_donation_member->paypal_payment_id);
                   }
                  ?>
                </p>
              </div>
              ...
              <div class="skt-donations-form-column delete">
                <p>
                  <?php
                    ...
                    ?>
                    <a onclick="return confirm('<?php esc_attr_e('Delete this record?','skt-donation');?>')" href="<?php echo esc_url( $action_url );?>"><?php echo esc_attr('Delete','skt-donation');?></a>
                </p>
              </div>
            </div>
        </td>
