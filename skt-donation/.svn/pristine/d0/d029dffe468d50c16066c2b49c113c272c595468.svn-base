...
function skt_donation_currency_page() {
	...
	$skt_choose_currency_paypal = $wpdb->prefix . "skt_choose_currency_paypal";
 	$select_choose_currency_paypal = $wpdb->get_row("SELECT * FROM $skt_choose_currency_paypal WHERE id='1'"); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	<form method="post" action="">
		<select name="paypal_sktcurrency_id" id="paypal_sktcurrency_id">
	 		<?php
 				...
 				$get_country_type_currency_paypal = $wpdb->get_results( "SELECT * FROM $skt_country_type_currency_paypal WHERE type_mode='paypal'" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
