...
function skt_donation_add_paypalsubscription_function(){
    if ( !isset( $_POST['add_paypal_nonce'] ) || !wp_verify_nonce($_POST['add_paypal_nonce'], 'paypal_subscriptionnormal' ) ){ // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.ValidatedSanitizedInput.MissingUnslash   
}else{
        ...
        $paypal_mode_subscription = sanitize_text_field(wp_unslash(isset($_POST['paypal_mode_subscription']) ? $_POST['paypal_mode_subscription'] : ''));
        ...
        if ($paypal_mode_subscription=='paypal_mode') {
            ...
            /*$product_currency =  sanitize_text_field(wp_unslash($_POST['payment_in_currency']);*/
            $product_currency =  sanitize_text_field(wp_unslash(isset($_POST['payment_in_currency']) ? $_POST['payment_in_currency'] : ''));
            $paypal_recurring =  sanitize_text_field(wp_unslash(isset($_POST['paypal_recurring']) ? $_POST['paypal_recurring'] : ''));
            
            if ($paypal_recurring == 'Daily') {
                $cycle = 'D';
                $duration_of_subscription ="daily";
            } else if ($paypal_recurring == 'Weekly') {
                $cycle = 'W';
                $duration_of_subscription ="week";
            } else if ($paypal_recurring == 'Month') {
                $cycle = 'M';
                $duration_of_subscription ="month";
            } else if ($paypal_recurring == 'Yearly') {
                $cycle = 'Y';
