...
class AdScout_Order_Processor
{
     ...
     *
     * @updated 2.3
     * Added compatibility with WooCommerce Sequential Order Numbers Pro
     *
     */
    public function sync_order($order_id)
    {
        ...
        if ( in_array( 'woocommerce-sequential-order-numbers-pro/woocommerce-sequential-order-numbers-pro.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            $order_sync_id = $order->get_meta('_order_number_formatted', true ) ?? $order->get_id();
        }
        ...
        $order_data = wp_json_encode([
            ...
            'orderNumber' => strval($order_sync_id),
            'orderType' => 'n',
