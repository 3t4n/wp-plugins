            ...
            <div class="ghl-tabs">
                <h2 class="nav-tab-wrapper-vertical">
                    <a href="?page=ib-ghlconnect"
                        class="nav-tab <?php if($tab===null):?>nav-tab-active<?php endif; ?>">Our Products</a>
                    <a href="?page=ib-ghlconnect&tab=pro"
                        class="nav-tab <?php if($tab==='pro'):?>nav-tab-active<?php endif; ?>">Upgrade to PRO</a>
                    <a href="?page=ib-ghlconnect&tab=cwghl"
                        class="nav-tab <?php if($tab==='cwghl'):?>nav-tab-active<?php endif; ?>">Connect with
                        GHL</a>
...
<?php	
		...
		public function ghlconnect_add_settings_link( $links ) {
	        $newlink = sprintf( "<a href='%s'>%s</a>" , admin_url( 'admin.php?page=ib-ghlconnect' ) , __( 'Settings' , 'ghl-connect' ) );
	        $links[] = $newlink;
