...
namespace SuperAdventCalendar\Vendor_Prefixed\DI\Invoker;
...
class FactoryParameterResolver implements ParameterResolver
{
    ...
    public function getParameters(
        ...
        foreach ($parameters as $index => $parameter) {
            ...
            if ($parameterClass === 'SuperAdventCalendar\Vendor_Prefixed\Psr\Container\ContainerInterface') {
                $resolvedParameters[$index] = $this->container;
            } elseif ($parameterClass === 'SuperAdventCalendar\Vendor_Prefixed\DI\Factory\RequestedEntry') {
