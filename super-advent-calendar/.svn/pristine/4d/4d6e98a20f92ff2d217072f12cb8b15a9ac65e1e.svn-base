...
use SuperAdventCalendar\Vendor_Prefixed\DI\Annotation\Inject;
use SuperAdventCalendar\Vendor_Prefixed\DI\Annotation\Injectable;
use SuperAdventCalendar\Vendor_Prefixed\DI\Definition\Exception\InvalidAnnotation;
use SuperAdventCalendar\Vendor_Prefixed\DI\Definition\ObjectDefinition;
use SuperAdventCalendar\Vendor_Prefixed\DI\Definition\ObjectDefinition\MethodInjection;
use SuperAdventCalendar\Vendor_Prefixed\DI\Definition\ObjectDefinition\PropertyInjection;
use SuperAdventCalendar\Vendor_Prefixed\DI\Definition\Reference;
use SuperAdventCalendar\Vendor_Prefixed\Doctrine\Common\Annotations\AnnotationRegistry;
use SuperAdventCalendar\Vendor_Prefixed\Doctrine\Common\Annotations\Reader;
use SuperAdventCalendar\Vendor_Prefixed\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use SuperAdventCalendar\Vendor_Prefixed\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'SuperAdventCalendar\Vendor_Prefixed\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'SuperAdventCalendar\Vendor_Prefixed\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('SuperAdventCalendar\Vendor_Prefixed\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()
                ->getClassAnnotation($class, 'SuperAdventCalendar\Vendor_Prefixed\DI\Annotation\Injectable');
