...
class ResourceService extends Service implements ResourceServiceInterface {
    ...
    public function get_modal_markup( $block_attributes ) : string {
        ...
        if ( !empty( $attrs['modalMaxWidth'] ) && in_array( $attrs['modalMaxWidth'], array(
            'lg',
            'xl',
            '2xl',
            '3xl',
            ...
            $modal_content_classes .= ' superac-dialog-content-max-width-' . $attrs['modalMaxWidth'];
        }
        if ( !empty( $attrs['modalFontSize'] ) && in_array( $attrs['modalFontSize'], array('small', 'large'), true ) ) {
            $modal_content_classes .= ' superac-dialog-content-font-' . $attrs['modalFontSize'];
        }
        // Add modal text color if attribute is set.
        if ( !empty( $attrs['modalTextColor'] ) ) {
            $modal_content_style .= 'color: ' . esc_attr( $attrs['modalTextColor'] ) . ';';
        }
        // Add modal background color if attribute is set.
        if ( !empty( $attrs['modalBackgroundColor'] ) ) {
            $modal_content_style .= 'background-color: ' . esc_attr( $attrs['modalBackgroundColor'] ) . ';';
        }
        return '<div class="superac-dialog-container" aria-hidden="true">
			<div class="superac-dialog-overlay" data-a11y-dialog-hide></div>
			<div class="superac-dialog-content' . esc_attr( $modal_content_classes ) . '" role="document"  style="' . esc_attr( $modal_content_style ) . '">
				<div class="superac-dialog-top-bar">
					<button class="superac-dialog-button" type="button" data-a11y-dialog-hide aria-label="Close dialog" style="' . esc_attr( $modal_content_style ) . '">
