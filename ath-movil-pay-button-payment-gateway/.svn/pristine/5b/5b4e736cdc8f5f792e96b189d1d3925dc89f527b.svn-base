...
class ATHMPBG_ATHMovil_Pay_Button_Hook_Helper
{
    ...
    function athmpbg_get_cart_data_items()
    {
        ...
        // Cart Data
        $data = ['total' => floatval($cart->total ?? 0) , 'subtotal' => floatval($subtotal_with_shipping) , 'tax' => floatval($total_taxes_with_shipping_taxes) , ];

        // Check cart items
        foreach ($cart->get_cart() as $cart_item_key => $cart_item)
        {
            $product = $cart_item['data'];
            $data['items'][] = ['name' => $product->get_name() ?? '', 'description' => $product->get_Description() ?? '' , 'quantity' => $cart_item['quantity'] ?? 1, 'price' => $cart_item['line_total'] ?? 0, 'tax' => $cart_item['line_tax'] ?? 0, 'metadata' => ''];
        }
