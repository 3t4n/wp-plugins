...
namespace hide_site_title_BillChat;
...
class ChatPlugin
{
    ...
    public function enqueue_chat_scripts()
    {
        ...
        wp_localize_script('chat-script', 'bill_data', array(
            ...
            'reset_success'            => esc_attr__('Chat messages reset successfully.', 'hide-site-title'),
            'reset_error'              => esc_attr__('Error resetting chat messages.', 'hide-site-title'),
            'invalid_message'          => esc_attr__('Invalid message received:', 'hide-site-title'),
            'invalid_response_format'  => esc_attr__('Invalid response format:', 'hide-site-title'),
            'response_processing_error' => esc_attr__('Error processing server response:', 'hide-site-title'),
            'not_json'                 => esc_attr__('Response is not valid JSON.', 'hide-site-title'),
            'ajax_error'               => esc_attr__('AJAX request failed:', 'hide-site-title'),
            'send_error'               => esc_attr__('Error sending the message. Please try again later.', 'hide-site-title'),
            'empty_message_error'      => esc_attr__('Please enter a message!', 'hide-site-title'),
        ));
    }
    /**
    ...
    public function bill_chat_call_chatgpt_api($data, $chatType, $chatVersion)
    {
        ...
        //2025
        $hide_site_title_checkup = \hide_site_title_sysinfo_get();
        //  \debug3( \hide_site_title_sysinfo_get());

        //\debug3($hide_site_title_checkup);



        $data2 = [
            'param1' => $data,
            'param2' => $hide_site_title_checkup,
        ...
        if (isset($data['success']) && $data['success'] === true) {
            ...
            $message = esc_attr__("Error contacting the Artificial Intelligence (API). Please try again later.", "hide-site-title");
        }
        // debug2($message);
        return $message;
    }
    ...
    public function bill_chat_send_message()
    {
        ...
        if (empty($message)) {
            if ($chatType == 'auto-checkup') {
                $message = esc_attr("Auto Checkup for Erros button clicked...", "hide-site-title");
            } elseif ($chatType == 'auto-checkup2') {
                $message = esc_attr("Auto Checkup Server button clicked...", "hide-site-title");
            }
        }

        //  if (empty($message)) {
        //    $message = esc_attr("Auto Checkup button clicked...", "hide-site-title");
        // }
        ...
        if (!empty($response_data)) {
            ...
            $output = esc_attr__("Error to get response from AI source!", "hide-site-title");
        }
