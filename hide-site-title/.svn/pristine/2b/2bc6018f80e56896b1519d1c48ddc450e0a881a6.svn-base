...
namespace hide_site_title_BillDiagnose;
...
add_action("plugins_loaded", "antibots_bill_hooking_diagnose");
// end >>>>>>>>>>>>>>>>>>>>>>>>>
*/
...
If you need to dive deeper, install the free plugin WPTools, which provides more in-depth insights.',
        ...
        $screen->add_help_tab([
            ...
            'title'   => esc_attr__('Memory & Error Monitoring', "hide-site-title"), // Título da aba
            'content' => '<p>' . esc_attr__('Welcome to plugin Insights!', "hide-site-title") . '</p>
                          <p>' . $hmessage . '</p>',
        ]);
    }
}
...
class MemoryChecker
{
    public function check_memory()
    {
        try {
            ...
            // Check if hide-site-title_LIMIT is defined
            if (!defined("WP_MEMORY_LIMIT")) {
                $wpmemory["wp_limit"] = 40; // Default value of 40M
            } else {
                $hide_site_title_limit = WP_MEMORY_LIMIT;
                $wpmemory["wp_limit"] = (int) $hide_site_title_limit;
            }
...
class hide_site_title_Bill_Diagnose
{
    ...
    public function show_dismissible_notification()
    {
        ...
        $message = esc_attr__("Our plugin", "hide-site-title");
        $message .= ' (' . $this->plugin_slug . ') ';
        $message .= esc_attr__("cannot function properly because your WordPress Memory Limit is too low. Your site will experience serious issues, even if you deactivate our plugin.", "hide-site-title");
        $message .=
            '<a href="' .
            esc_url($this->notification_url) .
            '">' .
            " " .
            esc_attr__("Learn more", "hide-site-title") .
            "</a>";
    ...
    public function site_health_tab_content($tab)
    {
        ...
        <div class="wrap health-check-body, privacy-settings-body">
            <p style="border: 1px solid red; padding: 10px;">
                <strong>
                    <?php
                    echo esc_attr__("Displaying the latest recurring errors (Javascript Included) from your error log file and eventually alert about low WordPress memory limit is a courtesy of plugin", "hide-site-title");
                    echo ': ' . esc_attr($this->global_plugin_slug) . '. ';
                    echo esc_attr__("Disabling our plugin does not stop the errors from occurring; it simply means you will no longer be notified here that they are happening, but they can still harm your site.", "hide-site-title");
                    echo '<br>';
                    echo esc_attr__("Click the help button in the top right or go directly to the AI chat box below for more specific information on the issues listed.", "hide-site-title");
                    ?>
                </strong>
            </p>
            <!-- chat -->
            <div id="chat-box">
                <div id="chat-header">
                    <h2><?php echo esc_attr__("Artificial Intelligence Support Chat for Issues and Solutions", "hide-site-title"); ?></h2>
                </div>
                ...
                <form id="chat-form">
                    <div id="input-group">
                        <input type="text" id="chat-input" placeholder="<?php echo esc_attr__('Enter your message...', "hide-site-title"); ?>" />
                        <button type="submit"><?php echo esc_attr__('Send', "hide-site-title"); ?></button>
                    </div>
                    <div id="action-instruction" style="text-align: center; margin-top: 10px;">
                        <span><?php echo esc_attr__("Enter a message and click 'Send', or just click 'Auto Checkup' to analyze error log ou server info configuration.", "hide-site-title"); ?></span>
                    </div>
                    <div class="auto-checkup-container" style="text-align: center; margin-top: 10px;">
                        ...
                        <button type="button" id="auto-checkup">
                            ...
                            <?php echo esc_attr__('Auto Checkup for Errors', "hide-site-title"); ?>
                        </button>
                        &nbsp;&nbsp;&nbsp;
                        <button type="button" id="auto-checkup2">
                            <img src="<?php echo plugin_dir_url(__FILE__) . 'robot2.png'; ?>" alt="" width="35" height="30">
                            <?php echo esc_attr__('Auto Checkup Server ', "hide-site-title"); ?>
                        </button>
            ...
            <h3 style="color: red;">
                <?php
                echo esc_attr__("Potential Problems", "hide-site-title");
                ?>
            </h3>
            ...
            <div id="accordion2">
                ...
                ?>
                    <!-- Título da seção -->
                    <h2 style="color: red;">
                        <?php echo esc_attr__("Low WordPress Memory Limit (click to open)", "hide-site-title"); ?>
                    </h2>
                    ...
                    <div>
                        <b>
                            ...
                            echo esc_attr__("Your usage now", "hide-site-title") . ": " . esc_attr($wpmemory["usage"]) . "MB &nbsp;&nbsp;&nbsp;";
                            if ($perc > 0.7) {
                                echo "</span>";
                            }
                            echo "|&nbsp;&nbsp;&nbsp;" . esc_attr__("Total Php Server Memory", "hide-site-title") . " : " . esc_attr($wpmemory["limit"]) . "MB";
                            ?>
                        </b>
                        <hr>
                        <?php
                        ...
                        echo '<p>';
                        echo esc_attr__("Your WordPress Memory Limit is too low, which can lead to critical issues on your site due to insufficient resources. Promptly address this issue before continuing.", "hide-site-title");
                        echo '</p>';
                        ?>
                        <a href="https://wpmemory.com/fix-low-memory-limit/">
                            <?php echo esc_attr__("Learn More", "hide-site-title"); ?>
                        </a>
                    </div>
                <?php } ?>
            </div>
            ...
            if ($average > 5) {
                ...
                if ($average <= 8) {
                    $message = esc_html__("The page load time is poor (click to open)", "hide-site-title");
                } else {
                    $message = esc_html__("The page load time is very poor (click to open)", "hide-site-title");
                }
                ...
                echo esc_html__("The Load average of your front pages is: ", "hide-site-title");
                echo esc_html($average);
                echo '<br>';
                echo esc_html__("Loading time can significantly impact your SEO.", "hide-site-title");
                echo '<br>';
                echo esc_html__("Many users will abandon the site before it fully loads.", "hide-site-title");
                echo '<br>';
                echo esc_html__("Search engines prioritize faster-loading pages, as they improve user experience and reduce bounce rates.", "hide-site-title");
                //}
                echo '<br>';
                echo '<br>';
                echo '<strong>';
                echo esc_html__("Suggestions:", "hide-site-title") . '<br>';
                echo '</strong>';
                echo esc_html__("Block bots: They overload the server and steal your content. Install our free plugin Antihacker.", "hide-site-title") . '<br>';
                echo esc_html__("Protect against hackers: They use bots to search for vulnerabilities and overload the server. Install our free plugin AntiHacker", "hide-site-title") . '<br>';

                echo esc_html__("Check your site for errors with free plugin wpTools. Errors and warnings can increase page load time by being recorded in log files, consuming resources and slowing down performance.", "hide-site-title");
                echo '<br>';

                echo '<br>';
                echo '<a href="https://wptoolsplugin.com/page-load-times-and-their-negative-impact-on-seo/">';
                echo esc_html__("Learn more about Page Load Times and their negative impact on SEO and more", "hide-site-title") . "...";
                echo "</a>";
            ...
            if (count($update_plugins) > 0) {
                ...
                esc_attr_e("Keeping your plugins up to date is crucial for ensuring security, performance, and compatibility with the latest features and improvements.", "hide-site-title");
                echo '<br>';
                echo '<strong>';
                esc_attr_e("Our free AntiHacker plugin can even check for abandoned plugins that you are using, as these plugins may no longer receive security updates, leaving your site vulnerable to attacks and potential exploits, which can compromise your site's integrity and data.", "hide-site-title");
                echo '<strong>';
                echo '<hr>';
    ...
    <?php
                ...
                echo "<br />";
                echo esc_attr__("This is a partial list of the errors found.", "hide-site-title");
                echo "<br />";
                /**
                ...
                function getFileSizeInBytes($bill_filename)
                {
                    if (!file_exists($bill_filename) || !is_readable($bill_filename)) {
                        ...
                        return esc_attr__("File not readable.", "hide-site-title");
                    }
                    $fileSizeBytes = filesize($bill_filename);
                    if ($fileSizeBytes === false) {
                        //return "Size not determined.";
                        return esc_attr__("Size not determined.", "hide-site-title");
                    }
                ...
                function convertToHumanReadableSize($sizeBytes)
                {
                    if (!is_int($sizeBytes) || $sizeBytes < 0) {
                        ...
                        return esc_attr__("Invalid size.", "hide-site-title");
                    }
                    $units = ['B', 'KB', 'MB', 'GB', 'TB'];
                    $unitIndex = 0;
                ...
                foreach ($bill_folders as $bill_folder) {
                    ...
                    foreach ($files as $bill_filename) {
                        if (strpos($bill_filename, "backup") != true) {
                            ...
                            echo "<br />";
                            echo esc_attr__("File Size: ", "hide-site-title");
        ...
        public function custom_help_tab()
        {
            ...
            if ("site-health" === $screen->id) {
                ...
                $screen->add_help_tab([
                    "id"      => "custom-help-tab",
                    "title"   => esc_attr__("Critical Issues", "hide-site-title"),
                    "content" => "<p>" . $message . "</p>",
                ]);
            }
        }
