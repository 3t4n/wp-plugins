...
$hide_site_title_plugin_data = get_file_data(__FILE__, array('Version' => 'Version'), false);
$hide_site_title_plugin_version = $hide_site_title_plugin_data['Version'];
define('HIDE_SITE_TITLEVERSION', $hide_site_title_plugin_version);
...
if ($hide_site_title_is_admin)
    require_once plugin_dir_path(__FILE__) . 'functions/function_sysinfo.php';
...
function hide_site_title_page_content()
{
    ...
    function hide_site_title_add_help_button()
    {
        ...
        if ($active_tab == "main") {
            //dashboard
            $help = esc_attr("The Hide Title Remover plugin allows you to easily hide titles from your WordPress posts and pages.
The title will not be hidden from administrators. Please log out and visit the site to see how it appears with the title hidden.", "hide-site-title");
            $help .= '<br>';
            $help .= esc_attr("Metabox is an added box in the page or post editor that enables users to mark and decide whether the title should be displayed or hidden for that specific page or post.", "hide-site-title");
        }
        if ($active_tab == "1") {
            $help = '
            <div><strong>' . esc_attr__('Metabox', 'hide-site-title') . '</strong></div>
            <div>' . esc_attr__('Metabox is an added box in the page or post editor that enables users to mark and decide whether the title should be displayed or hidden for that specific page or post.', 'hide-site-title') . '</div>
            <div>&nbsp;</div>
            <div><strong>' . esc_attr__('How to recover the ID or class from your title on your home page.', 'hide-site-title') . '</strong></div>
            <div>' . esc_attr__('Hover your mouse over the page title on your WordPress site.', 'hide-site-title') . '</div>
            <div>' . esc_attr__('Right-click on the title.', 'hide-site-title') . '</div>
            <div>' . esc_attr__('From the menu that appears, select "Inspect" or "Inspect Element."', 'hide-site-title') . '</div>
            <div>' . esc_attr__('A new panel or window will open, highlighting the HTML code of the page.', 'hide-site-title') . '</div>
            <div>' . esc_attr__('Look for the section that corresponds to the title, usually inside tags like &lt;h1&gt; or &lt;h2&gt;.', 'hide-site-title') . '</div>
            <div>' . esc_attr__('Check for any <code>class</code> or <code>id</code> attributes associated with the title.', 'hide-site-title') . '</div>';
        }
        if ($active_tab == "3") {
            $help = esc_attr__('Click the "Install" button for the desired plugin and wait for the installation to complete. Then, go to the Plugins page and activate it.', "hide-site-title");
        }
    ...
    function hide_site_title_load_chat()
    {
        global $hide_site_title_is_admin;
        if ($hide_site_title_is_admin and current_user_can("manage_options")) {
            // ob_start();
            ...
            if (! class_exists('hide_site_title_BillChat\ChatPlugin')) {
                require_once dirname(__FILE__) . "/includes/chat/class_bill_chat.php";
            }
        }
    }
    add_action('wp_loaded', 'hide_site_title_load_chat');
