...
class WRP_Variations_Admin {
				...
				if ( $commission ) {
					...
					if ($type == "rate") {
						...
						if ( $variation->is_taxable() && get_option('woocommerce_prices_include_tax') == 'yes' ) {
							$_tax       = new WC_Tax();
							$tax_rates  = $_tax->get_base_tax_rates( $variation->get_tax_class() );
							$taxes      = $_tax->calc_tax( $price, $tax_rates, true );
							$product_price      = $_tax->round( (float) $price - (float) array_sum( $taxes ) );
						}
	...
	public static function get_commission ( $product, $variation_id, $role = null ) {
		...
		if ( sizeof( $user_roles ) > 0 ) {
		    ...
		    if ( get_option( "wrp-guest", "-1" ) !== "-1" ) {
		        $discount = get_option( "wrp-guest" );
		    }
		}
		if ( $discount ) {
	 ...
	 */
	public static function get_user_role( $roles, $product, $variation_id ) {
		$result = null;
