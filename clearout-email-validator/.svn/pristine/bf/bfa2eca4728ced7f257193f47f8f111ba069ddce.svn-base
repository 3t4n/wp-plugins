...
function _co_email_validation( $email, $clearout_options, $clearout_form_source ) {
	...
	// Use the custom is_admin_request() function to check if in the admin context
	if ( is_admin_request() && CLEAROUT_TEST_PLUGIN_SOURCE != $clearout_form_source ) {
...
// checkoutFields reference: https://woocommerce.github.io/code-reference/classes/WC-Checkout.html#method_get_checkout_fields
/**
...
function clearout_woocom_checkout_validate_email( $fields, $errors ) {
	...
	foreach ( $fields as $key => $value ) {
		if ( strpos( $key, '_email' ) !== false ) {
			...
			// if the field's class has clearout skip validation identifier, dont validate
			$class_array = [];
			foreach ( $checkout_fields as $section => $section_fields ) {
				if ( isset( $section_fields[ $key ]['class'] ) ) {
					$class_array = $section_fields[ $key ]['class'];
					break;
				}
			}

			// if the clearout skip validation class is set for this field, skip it.
			if ( ! empty( preg_grep( CLEAROUT_IGNORE_VALIDATION_IDENTIFIER_REGEX, $class_array ) ) ) {
				continue;
			}
...
/**
 * WooCommerce Signup forms
 * 
 * @param mixed $username UserName
 * @param mixed $user_email User EMail
 * @param mixed $errors return Errors
 */
function clearout_woocommerce_registration_email_validator( $username, $user_email, $errors ) {
	$clearout_form_source = 'wooregister';

	// Get Clearout settings
	$clearout_options = get_option( 'clearout_email_validator' );
	$is_valid_email   = true;

	// Check if API key is set and email is provided
	if ( ! empty( $clearout_options['api_key'] ) && ! empty( $user_email ) ) {
		// Perform email validation
		$validation_result = _co_email_validation( $user_email, $clearout_options, $clearout_form_source );

		if ( is_array( $validation_result ) && array_key_exists( 'status', $validation_result ) ) {
			if ( false === $validation_result['status'] ) {
				$is_valid_email = false;

				// Remove all existing errors related to email validation
				foreach ( $errors->get_error_codes() as $code ) {
					$errors->remove( $code );
				}
