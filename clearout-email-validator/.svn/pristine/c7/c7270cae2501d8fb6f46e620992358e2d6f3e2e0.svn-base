	...
	<div style="float: left;padding: 0px 20px 0px 0px;">
		<div style="background-color: #fff;padding:5px 30px 5px 30px;">
			...
			<p style="font-size: 14px;">Clearout Email Validator plugin seamlessly integrated with all major forms to
				validate the user given email address in real-time. This plugin will perform <a
					href="https://clearout.io/email-verifier/#validation_checks" target="_blank">20+ refined validation</a>
				checks to
		...
		<div style="background-color: #ffffff61;padding:15px 30px 15px 30px;">
			...
			<ul style="list-style-type: disc;padding-left: 10px;font-size: 14px;">
				...
				<li>Option <b>Safe to Send</b> and <b>Business address</b> share the same priority and can be applied
					simulateneously </li>
...
function _co_form_select_setting_option() {
	...
	if ( ! isset( $options['options_initialized'] ) ) {
		...
		$wcr_on_off = isset( $options['wcr_on_off'] ) ? $options['wcr_on_off'] : 'on';
	} else {
		...
		$wcr_on_off = isset( $options['wcr_on_off'] ) ? $options['wcr_on_off'] : 'off';
	}
	...
	echo '<div><input type="checkbox" name="clearout_email_validator[chf_on_off]" id="chf_option" value="on"' . checked( $chf_on_off, 'on', false ) . ' /><label>WooCommerce Checkout Form</label></div>';
	...
	echo '<div><input type="checkbox" name="clearout_email_validator[wcr_on_off]" id="wcr_on_off" value="on"' . checked( $wcr_on_off, 'on', false ) . ' /><label>WooCommerce Registration Form </label></div>';
