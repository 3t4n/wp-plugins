...
class Booknow_Wpforms_Staffs_Field extends WPForms_Field {
	public function init() {
		$this->name  = esc_html__( 'BookNow Staffs', 'booknow' );
		$this->type  = 'booknow_staffs';
		$this->icon  = 'fa-file-o';
		$this->order = 30;
		$this->group = 'payment';
		$this->hooks();
	...
	function add_options($type,$field){
		...
		switch($type) {
			case "booknow":
				...
				$fld  = $this->field_element( 'label',    $field, array( 'slug' => 'staffs_show', 'value' => esc_html__( 'Show Any Staff', 'booknow' ) ), false );
				$value   = ! empty( $field['staffs_show'] ) || wp_doing_ajax();
				$fld .= $this->field_element( 'toggle', $field, array( 'slug' => 'staffs_show', 'value' => $value ), false );
				$output  .= $this->field_element( 'row',      $field, array( 'slug' => 'staffs_show', 'content' => $fld ), false );
				break;
		}
        printf("%s",$output);        
    }
	...
	public function field_preview( $field ) {
		esc_html_e("Booking Staffs","booknow");
	}
	...
	public function format( $field_id, $field_submit, $form_data ) {
		...
		if( is_numeric($field_submit)){
			$services_datas = get_post_meta( $field_submit   , '_booknow_staffs' , true );
			$field_submit = $services_datas["last_name"] . " " .$services_datas["first_name"];
		}
	...
	public function field_display( $field, $deprecated, $form_data ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		$name = $field["id"];
		$style   = isset( $field['service_type'] ) ? $field['service_type'] : 'list';
		$show_any   = isset( $field['staffs_show'] ) ? '1' : '0';
		echo do_shortcode( '[booknow_staffs style="'.$style.'" show_any="'.$show_any.'"]' );
		$class = $primary['class'];
		$class[] = "booknow-staff-name";
		$attr = $primary['attr'];
		if( $show_any == 1 ){
			$attr["value"] = "any";
		}
...
new Booknow_Wpforms_Staffs_Field;