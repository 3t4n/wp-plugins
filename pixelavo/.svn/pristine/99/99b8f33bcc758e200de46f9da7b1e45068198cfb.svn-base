 ...
 * Class.
 */
if ( ! class_exists( 'Pixelavo_Diagnostic_Data' ) ) {
    class Pixelavo_Diagnostic_Data {
        ...
        private function __construct() {
            $this->project_name = 'Pixelavo';
            $this->project_type = 'wordpress-plugin';
            $this->project_version = PIXELAVO_VERSION;
            ...
            $this->project_pro_slug = 'pixelavo-pro/pixelavo-pro.php';
            ...
            if( get_option('pixelavo_diagnostic_data_agreed') === 'yes' || get_option('pixelavo_diagnostic_data_notice') === 'no' ){
                return;
            }

            add_action( 'wp_ajax_pixelavo_diagnostic_data', function () {
                check_ajax_referer( 'pixelavo-diagnostic-data-ajax-request' );
            ...
            add_action('plugins_loaded', function(){
                $agreed  = ( isset( $_GET['pixelavo_diagnostic_data_agreed'] ) ? sanitize_key( $_GET['pixelavo_diagnostic_data_agreed'] ) : '' );
        ...
        public function notice_js() {
            ...
            echo '<script type="text/javascript">;(function($) {
                ...
                $(".pixelavo-diagnostic-data-list-toogle").on("click", function(e) {
                    e.preventDefault();
                    $(this).parents(".pixelavo-diagnostic-data-notice").find(".pixelavo-diagnostic-data-list").slideToggle("fast")
                });
                $(".pixelavo-diagnostic-data-button").on("click", function(e) {
                    e.preventDefault();
                    let thisButton = $(this),
                        noticeWrap = thisButton.closest(".pixelavo-admin-notice"),
                        agreed = thisButton.hasClass("pixelavo-diagnostic-data-agree") ? "yes" : "no";
                    $.ajax({
                        type: "POST",
                        url: "'.esc_url($ajax_url).'",
                        data: {
                            action: "pixelavo_diagnostic_data",
                            agreed: agreed,
                            _wpnonce: "'.esc_attr($ajax_nonce).'"
                        },
                        beforeSend: function() {
                            noticeWrap.addClass("pixelavo-diagnostic-data-loading")
                        },
                        success: function(response) {
                            ...
                            noticeWrap.removeClass("pixelavo-diagnostic-data-loading");
                            pixelavoDismissThanksNotice()
                        },
                        error: function() {
                            noticeWrap.removeClass("pixelavo-diagnostic-data-loading")
                        },
        ...
        private function is_show_core_notice() {
            $result = get_option( 'pixelavo_diagnostic_data_notice', 'yes' );
        ...
        private function process_data( $agreed, $ajax = false ) {
            ...
            update_option( 'pixelavo_diagnostic_data_agreed', $agreed );
            update_option( 'pixelavo_diagnostic_data_notice', $notice );
            set_transient( 'pixelavo-notice-id-pixelavo-diagnostic-notice', true );
        ...
        private function show_core_notice() {
            ...
            * translators: %4$s: a start tag with url
            * translators: %5$s: a end tag
            */
            ...
            /*
            * translators: %1$s: a start tag with url
            * translators: %2$s: a end tag
            */
            ...
            $button_text_1 = esc_html__( 'Count Me In', 'pixelavo' );
            $button_link_1 = add_query_arg( array( 'pixelavo_diagnostic_data_agreed' => '1' ) );

            $button_text_2 = esc_html__( 'No, Thanks', 'pixelavo' );
            $button_link_2 = add_query_arg( array( 'pixelavo_diagnostic_data_agreed' => '0' ) );
            ?>
            <div class="pixelavo-diagnostic-data-notice">
                <h4 class="pixelavo-diagnostic-data-title">
                    <?php
                        /* translators: %1$s: project name */
                        echo sprintf( esc_html__('🌟 Enhance Your %1$s Experience as a Valued Contributor!','pixelavo'), esc_html( $this->project_name ));
                    ?>
                </h4>
                <p class="pixelavo-diagnostic-data-message"><?php echo wp_kses_post( $message_l1 ); ?></p>
                <p class="pixelavo-diagnostic-data-list"><?php echo wp_kses_post( $message_l2 ); ?></p>
                <p class="pixelavo-diagnostic-data-buttons">
                    <a href="<?php echo esc_url( $button_link_1 ); ?>" class="pixelavo-diagnostic-data-button pixelavo-diagnostic-data-agree button button-primary"><?php echo esc_html( $button_text_1 ); ?></a>
                    <a href="<?php echo esc_url( $button_link_2 ); ?>" class="pixelavo-diagnostic-data-button pixelavo-diagnostic-data-disagree button button-secondary"><?php echo esc_html( $button_text_2 ); ?></a>
                </p>
            </div>
            <?php
        }
        ...
        private function get_thanks_notice() {
            ...
            * translators: %3$s: strong end tag
            */
            $message = sprintf( esc_html__( 'Thank you very much for supporting %2$s%1$s%3$s.', 'pixelavo' ), $this->project_name, '<strong>', '</strong>' );
            /*
            * translators: %1$s: Message content
            */
            $notice = sprintf( '<div class="pixelavo-diagnostic-data-thanks notice notice-success is-dismissible"><p>%1$s</p><button type="button" class="notice-dismiss"><span class="screen-reader-text"></span></button></div>', wp_kses_post( $message ) );
