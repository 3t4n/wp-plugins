...
function pixelavo_run_conversions_api($eventName, $data, $eventID = '') {
    ...
    if(!empty($pixels)) {
        foreach ($pixels as $pixel) {
            if(
                array_key_exists('conversion_api_status', $pixel) && $pixel['conversion_api_status'] == 'on' && 
                pixelavo_check_pixel_page($pixel) && array_key_exists('access_token', $pixel) && !empty($pixel['access_token'])
            ) {
                $url = "https://graph.facebook.com/v21.0/{$pixel['pixel_id']}/events?access_token={$pixel['access_token']}";
                wp_remote_post($url, [
                    'method' => 'POST',
                    'body' => [
                        "data" => [
...
function pixelavo_get_fluent_forms_list() {
    ...
    global $wpdb;
    $forms = $wpdb->get_results("SELECT id, title FROM {$wpdb->prefix}fluentform_forms");
    $options = [
        [
            'id' => 'all_forms',
            'text' => 'All Forms'
