...
if ( ! class_exists( 'Pixelavo_Notice' ) ){
    class Pixelavo_Notice{
        ...
        public function __construct(){
            ...
            add_action( 'pixelavo_admin_notices', [ $this, 'show_admin_notices' ] );
            add_action( 'pixelavo_admin_sidebar_notices', [ $this, 'show_admin_sidebar_notices' ] );
            add_action(	'admin_footer', [ $this, 'enqueue_scripts' ], 999 );
            add_action( 'wp_ajax_pixelavo_notices', [ $this, 'ajax_dismiss' ] );
        }
        ...
        public function ajax_dismiss() {
			...
			if ( ! apply_filters( 'pixelavo_notice_user_cap_check', current_user_can( $capability ) ) ) {
                $error_message = [
                    'message'  => __('You are not authorized.', 'pixelavo')
                ];
                wp_send_json_error( $error_message );
			}

            // Nonce verification check
            if( !wp_verify_nonce( $nonce, 'pixelavo_notices_nonce') ) {
                $error_message = [
                    'message'  => __('Are you cheating?', 'pixelavo')
                ];
                wp_send_json_error( $error_message );
            }

            if ( ! empty( $notice_id ) && (strpos( $notice_id, 'pixelavo-notice' ) !== false) ) {
        ...
        public function enqueue_scripts() {
            ...
            $styles = ".pixelavo-admin-notice {
                width: 100%;
                box-sizing: border-box;
            }
            .pixelavo-admin-notice.promo-banner {
                position: relative;
                padding-top: 20px;
                padding-right: 40px;
            }
                
            .pixelavo-admin-notice.sidebar-notice {
                margin: 0;
            }
                
            .pixelavo-admin-notice.notice img, .pixelavo-review-notice-wrap img{
                width: 100%;
            }
            .pixelavo-review-notice-wrap{
            ...
            .pixelavo-review-notice-content {
                margin-left: 15px;
            }
            .pixelavo-review-notice-action {
                display: flex;
                align-items: center;
                padding-top: 10px;
                gap: 15px;
            }
            .pixelavo-review-notice-action span.dashicons {
                font-size: 1.4em;
                padding-left: 0;
            }
            .pixelavo-review-notice-action a {
                padding-left: 0;
                text-decoration: none;
                display: flex;
                gap: 5px;
            }
            .pixelavo-review-notice-content h3 {
                margin: 0;
            }";

            $scripts = "jQuery(document).ready( function($) {
                $( '.pixelavo-admin-notice.is-dismissible' ).on( 'click', '.notice-dismiss,.pixelavo-notice-close', function(e) {
                    e.preventDefault();
                    let noticeWrap = $( this ).parents( '.pixelavo-admin-notice' ),
                        ...
                        noticeNonce = '".esc_html( wp_create_nonce( 'pixelavo_notices_nonce' ) )."';
        ...
        private function prepare_notice( $notice_data ){
            ...
            if ( isset( $notice['type'] ) ) {
                ...
                if( $notice['type'] !== 'custom'){
                    ...
                    $notice['dismissible_btn'] = '<button type="button" class="notice-dismiss"><span class="screen-reader-text">'.esc_html__('Dismiss this notice.','pixelavo').'</span></button>';
                }
            }
            ...
            $notice['id'] = 'pixelavo-notice-id-' . $notice['id'];
        ...
        public function show_admin_notices(){
            ...
            foreach ( $notices as $notice ) {
                ...
                $notice = $this->prepare_notice( $notice );

                if ( isset( $notice['is_show'] ) && current_user_can( $notice['capability'] ) ) {
                    if ( true === $notice['is_show'] && $notice['position'] !== 'sidebar' ) {
        ...
        /**
         * Show Admin Sidebar Notices
         *
         * @return void
         */
        public function show_admin_sidebar_notices(){
            $notices_displayed_count = 0;
            $notices = $this->get_notices();
            ...
            foreach ( $notices as $notice ) {
                ...
                if ( isset( $notice['is_show'] ) && current_user_can( $notice['capability'] ) ) {
                    if ( true === $notice['is_show'] && $notice['position'] === 'sidebar' ) {
                        $notice['classes'] = implode( ' ', [$notice['classes'], 'sidebar-notice'] );
                        if ( self::is_expired( $notice ) ) {
                            self::html( $notice );
                            ++$notices_displayed_count;
                        }
