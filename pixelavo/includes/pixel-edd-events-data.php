...
class PixelEddEventsData{
    ...
    function remove_from_cart_data() {
        ...
        if(pixelavo_pixel_status()) {
            $id = sanitize_text_field($_POST['download_id']);
            $quantity = sanitize_text_field($_POST['download_quantity']);
            $common_params = array_map('sanitize_text_field', (array)$_POST['common_params']);
            $download = edd_get_download($id);
