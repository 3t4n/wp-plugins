 ...
 *
 */
if ( ! class_exists( 'Chat_Whatsapp_Field_link_color' ) ) {
  class Chat_Whatsapp_Field_link_color extends Chat_Whatsapp_Fields {
    ...
    public function render() {
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="chat-whatsapp--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="chat-whatsapp-color"'. wp_kses_post($default_attr) . wp_kses_post($this->field_attributes()) .'/>';
          echo '</div>';
