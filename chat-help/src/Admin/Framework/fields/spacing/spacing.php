 ...
 *
 */
if ( ! class_exists( 'Chat_Whatsapp_Field_spacing' ) ) {
  class Chat_Whatsapp_Field_spacing extends Chat_Whatsapp_Fields {
    ...
    public function render() {
      ...
      if ( ! empty( $args['all'] ) ) {
        ...
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[all]' ) ) .'" value="'. esc_attr( $value['all'] ) .'"'. wp_kses_post($placeholder) .' class="chat-whatsapp-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( $unit ) ? '<span class="chat-whatsapp--label chat-whatsapp--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
        ...
        foreach ( $properties as $property ) {
          ...
          echo '<input type="number" name="'. esc_attr( $this->field_name( '['. $property .']' ) ) .'" value="'. esc_attr( $value[$property] ) .'"'. wp_kses_post($placeholder) .' class="chat-whatsapp-input-number'. esc_attr( $is_unit ) .'" step="any" />';
          echo ( $unit ) ? '<span class="chat-whatsapp--label chat-whatsapp--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
          echo '</div>';
