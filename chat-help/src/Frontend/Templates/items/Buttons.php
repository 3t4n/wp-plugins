...
namespace ThemeAtelier\ChatWhatsapp\Frontend\Templates\items;
...
class Buttons
{
    public static function buttons($options)
    {
        ...
        $phone_number = !empty($options['opt-number']) ? $options['opt-number'] : '+8801849687969';
        ...
        $circle_animation = $options['circle-animation'] ?? '1';
        ...
        if ($button_style === '1') {
            $bubble_type = '<button class="wHelp-bubble circle-bubble circle-animation-' . esc_attr($circle_animation) .' wHelp_' . $chat_type . ' ' . esc_attr($chat_type) . ' ' . esc_attr($tooltip_class) . '" style="--color-primary: ' . esc_attr($primary) . ';--color-secondary: ' . esc_attr($secondary) . ';">';
            ...
            if ($chat_type == 'button') {
                $bubble_type .= '<a href="https://wa.me/' . esc_attr($phone_number) . '" target="_blank" class="chat-link"></a>';
            }
        ...
        $bubble_type = '<button class="' . esc_attr($base_classes . $style_classes . ' wHelp_' . $chat_type . ' ' . $tooltip_class ) . '" style="--color-primary: ' . esc_attr($primary) . ';--color-secondary: ' . esc_attr($secondary) . ';">';
