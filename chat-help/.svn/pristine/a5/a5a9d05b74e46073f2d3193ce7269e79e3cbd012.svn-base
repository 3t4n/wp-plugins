...
if ( ! class_exists( 'Chat_Whatsapp_Field_link' ) ) {
  class Chat_Whatsapp_Field_link extends Chat_Whatsapp_Fields {
    ...
    public function render() {
      ...
      echo '<div class="'. esc_attr( $maybe_hidden ) .'"><div class="chat-whatsapp--result">'. sprintf( '{url:"%s", text:"%s", target:"%s"}', wp_kses_post($value['url']), esc_html($value['text']), wp_kses_post($value['target']) ) .'</div></div>';

      echo '<input type="hidden" name="'. esc_attr( $this->field_name( '[url]' ) ) .'" value="'. esc_attr( $value['url'] ) .'"'. wp_kses_post($this->field_attributes( array( 'class' => 'chat-whatsapp--url' ) )) .' />';
      ...
      echo '<a href="#" class="button button-primary chat-whatsapp--add'. esc_attr( $hidden ) .'">'. wp_kses_post($args['add_title']) .'</a> ';
      echo '<a href="#" class="button chat-whatsapp--edit'. esc_attr( $maybe_hidden ) .'">'. wp_kses_post($args['edit_title']) .'</a> ';
