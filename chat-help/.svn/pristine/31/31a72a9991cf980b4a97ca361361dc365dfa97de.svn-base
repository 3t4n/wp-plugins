...
namespace ThemeAtelier\ChatWhatsapp\Admin\Framework\Classes;

if (! class_exists('CHAT_WHATSAPP')) {
  class CHAT_WHATSAPP
  {

    // Default constants
    public static $premium  = true;
    public static $version  = CHAT_WHATSAPP_VERSION;
    public static $dir      = '';
    public static $url      = '';
    public static $css      = '';
    public static $file     = '';
    ...
    public function __construct()
    {
      ...
      // Init action
      do_action('CHAT_WHATSAPP_init');

      add_action('after_setup_theme', array('ThemeAtelier\ChatWhatsapp\Admin\Framework\Classes\CHAT_WHATSAPP', 'setup'));
      add_action('init', array('ThemeAtelier\ChatWhatsapp\Admin\Framework\Classes\CHAT_WHATSAPP', 'setup'));
      add_action('switch_theme', array('ThemeAtelier\ChatWhatsapp\Admin\Framework\Classes\CHAT_WHATSAPP', 'setup'));
      add_action('admin_enqueue_scripts', array('ThemeAtelier\ChatWhatsapp\Admin\Framework\Classes\CHAT_WHATSAPP', 'add_admin_enqueue_scripts'));
      add_action('wp_enqueue_scripts', array('ThemeAtelier\ChatWhatsapp\Admin\Framework\Classes\CHAT_WHATSAPP', 'add_typography_enqueue_styles'), 80);
      add_action('wp_head', array('ThemeAtelier\ChatWhatsapp\Admin\Framework\Classes\CHAT_WHATSAPP', 'add_custom_css'), 80);
      add_filter('admin_body_class', array('ThemeAtelier\ChatWhatsapp\Admin\Framework\Classes\CHAT_WHATSAPP', 'add_admin_body_class'));
    }
    ...
    public static function setup()
    {
      ...
      $params = array();
      if (class_exists('CHAT_WHATSAPP_Options') && ! empty(self::$args['admin_options'])) {
        foreach (self::$args['admin_options'] as $key => $value) {
          if (! empty(self::$args['sections'][$key]) && ! isset(self::$inited[$key])) {
            ...
            \CHAT_WHATSAPP_Options::instance($key, $params);
      ...
      $params = array();
      if (class_exists('CHAT_WHATSAPP_Metabox') && ! empty(self::$args['metabox_options'])) {
        foreach (self::$args['metabox_options'] as $key => $value) {
          if (! empty(self::$args['sections'][$key]) && ! isset(self::$inited[$key])) {
            ...
            \CHAT_WHATSAPP_Metabox::instance($key, $params);
          }
        }
      }
    ...
    public static function include_plugin_file($file, $load = true)
    {
      ...
      $path     = '';
      $file     = ltrim($file, '/');
      $override = apply_filters('CHAT_WHATSAPP_override', 'chat-whatsapp-override');
    ...
    public static function includes()
    {
      ...
      self::include_plugin_file('Classes/CHAT_WHATSAPP_Options.php');
      self::include_plugin_file('Classes/CHAT_WHATSAPP_Metabox.php');

      // Include all framework fields
      $fields = apply_filters('CHAT_WHATSAPP_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
      ...
      if (! empty($fields)) {
        foreach ($fields as $field) {
          ...
          if (! class_exists('CHAT_WHATSAPP_Field_' . $field) && class_exists('CHAT_WHATSAPP_Fields')) {
    ...
    public static function add_admin_enqueue_scripts()
    {
      ...
      if (! self::$enqueue) {
        ...
        if ($wpscreen->id === 'tools_page_chat-whatsapp-welcome') {
          self::$enqueue = true;
        }
      }

      if (! apply_filters('CHAT_WHATSAPP_enqueue_assets', self::$enqueue)) {
        return;
      }
      ...
      // Main RTL styles
      if (is_rtl()) {
        wp_enqueue_style('chat-whatsapp-rtl', self::include_plugin_url('assets/css/style-rtl' . $min . '.css'), array(), self::$version, 'all');
      }
      ...
      // Main scripts
      wp_enqueue_script('chat-whatsapp-plugins', self::include_plugin_url('assets/js/plugins' . $min . '.js'), array(), self::$version, true);
      wp_enqueue_script('chat-help', self::include_plugin_url('assets/js/main' . $min . '.js'), array('chat-whatsapp-plugins'), self::$version, true);

      // Main variables
      wp_localize_script('chat-help', 'CHAT_WHATSAPP_vars', array(
        'color_palette'     => apply_filters('CHAT_WHATSAPP_color_palette', array()),
        'i18n'              => array(
      ...
      if (! empty(self::$fields)) {
        foreach (self::$fields as $field) {
          if (! empty($field['type'])) {
            $classname = 'Chat_Whatsapp_Field_' . $field['type'];
      ...
      do_action('CHAT_WHATSAPP_enqueue');
    }
    ...
    public static function add_typography_enqueue_styles()
    {
      ...
      if (! empty(self::$webfonts)) {
        ...
        if (! empty(self::$webfonts['enqueue'])) {
          ...
          wp_enqueue_style('chat-whatsapp-google-web-fonts', esc_url(add_query_arg($query, '//fonts.googleapis.com/css')), array(), null);
        }
        ...
        if (! empty(self::$webfonts['async'])) {
          ...
          wp_enqueue_script('chat-whatsapp-google-web-fonts', esc_url('//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js'), array(), null);

          wp_localize_script('chat-whatsapp-google-web-fonts', 'WebFontConfig', array('google' => array('families' => $fonts)));
        }
      }
    }
    ...
    public static function add_admin_body_class($classes)
    {
      ...
      if (apply_filters('CHAT_WHATSAPP_fa4', false)) {
        $classes .= 'chat-whatsapp-fa5-shims';
      }
    ...
    public static function field($field = array(), $value = '', $unique = '', $where = '', $parent = '')
    {
      ...
      $is_pseudo  = (! empty($field['pseudo'])) ? ' chat-whatsapp-pseudo-field' : '';
      ...
      echo '<div class="chat-whatsapp-field chat-whatsapp-field-' . esc_attr($field_type . $is_pseudo . $class . $visible) . '"' . wp_kses_post($depend) . '>';

      if (! empty($field_type)) {

        if (! empty($field['title'])) {
          $subtitle = (! empty($field['subtitle'])) ? '<p class="chat-whatsapp-text-subtitle">' . $field['subtitle'] . '</p>' : '';
          $title_help = (! empty($field['title_help'])) ? '<span class="chat-whatsapp-help chat-whatsapp-title-help"><span class="chat-whatsapp-help-text">' . $field['title_help'] . '</span> <span class="tooltip-icon"><img src="' . self::include_plugin_url('assets/images/info.svg') . '"></span></span>' : '';
          echo wp_kses_post('<div class="chat-whatsapp-title"><h4>' . $field['title'] . $title_help . '</h4>' . $subtitle . '</div>');
        }

        echo (! empty($field['title'])) ? '<div class="chat-whatsapp-fieldset">' : '';

        $value = (! isset($value) && isset($field['default'])) ? $field['default'] : $value;
        $value = (isset($field['value'])) ? $field['value'] : $value;
        $classname = 'Chat_Whatsapp_Field_' . $field_type;
