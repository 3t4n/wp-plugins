 ...
 *
 */
if ( ! class_exists( 'Chat_Whatsapp_Field_select' ) ) {
  class Chat_Whatsapp_Field_select extends Chat_Whatsapp_Fields {
    ...
    public function render() {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if ( ! empty( $args['ajax'] ) ) {
          ...
          $args['settings']['data']['nonce'] = wp_create_nonce( 'CHAT_WHATSAPP_chosen_ajax_nonce' );
        ...
        $chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' chat-whatsapp-chosen-sortable' : '';
        $chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' chat-whatsapp-chosen-ajax' : '';
        $placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="'. esc_attr( $args['placeholder'] ) .'"' : '';
        $field_class      = ( $args['chosen'] ) ? ' class="chat-whatsapp-chosen'. esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) .'"' : '';
        ...
        if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
          ...
          if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
            ...
            echo '<select name="'. esc_attr($field_name) .'" class="chat-whatsapp-hide-select hidden"'. wp_kses_post($multiple_attr) . wp_kses_post($field_attr) .'>';
          ...
          echo '<select style="width: 300px" name="'. esc_attr( $field_name ) .'"'. wp_kses_post($field_class) . wp_kses_post($multiple_attr) . wp_kses_post($placeholder_attr) . wp_kses_post($field_attr) . wp_kses_post($chosen_data_attr) .'>';
