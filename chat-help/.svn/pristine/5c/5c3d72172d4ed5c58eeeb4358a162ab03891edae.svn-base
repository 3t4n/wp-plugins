...
class DBUpdates
{
    ...
    public function is_needs_update()
    {
        ...
        if (false === $first_version) {
            update_option('chat_help_first_version', CHAT_WHATSAPP_VERSION);
        }
        if (false === $activation_date) {
            update_option('chat_help_activation_date', current_time('timestamp'));
        }

        if (version_compare($installed_version, CHAT_WHATSAPP_VERSION, '<')) {
            return true;
        }
    ...
    public function perform_updates()
    {
        ...
        update_option('chat_help_version', CHAT_WHATSAPP_VERSION);
    }
}
