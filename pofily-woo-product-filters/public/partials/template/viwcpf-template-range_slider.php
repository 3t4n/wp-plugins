...
if ( $filter_for === 'filter_by_price' ) {
	$current_min_range = isset( $_GET['min_price'] ) ? floor( floatval( wp_unslash( wc_clean( $_GET['min_price'] ) ) ) / $arr_value['step_range_slide'] ) * $arr_value['step_range_slide'] : $arr_value['min_range_slide']; //phpcs:ignore WordPress.Security.NonceVerification.Recommended
	$current_max_range = isset( $_GET['max_price'] ) ? ceil( floatval( wp_unslash( wc_clean( $_GET['max_price'] ) ) ) / $arr_value['step_range_slide'] ) * $arr_value['step_range_slide'] : $arr_value['max_range_slide']; //phpcs:ignore WordPress.Security.NonceVerification.Recommended
} else if ( $filter_for === 'filter_by_metabox' ) {
	$get_min_range = '';
	$get_max_range = '';
	if (
		isset( $_GET[ 'viwcpf_metakey_' . $arr_value['name_metakey'] ] ) && isset( $_GET[ 'viwcpf_metakey_type_' . $arr_value['name_metakey'] ] )//phpcs:ignore WordPress.Security.NonceVerification.Recommended
	) {
		$viwcpf_metakey_value_filter = sanitize_text_field( $_GET[ 'viwcpf_metakey_' . $arr_value['name_metakey'] ] ); //phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	$current_min_range = ! empty( $get_min_range ) ? floor( floatval( wp_unslash( $get_min_range ) ) / $arr_value['step_range_slide'] ) * $arr_value['step_range_slide'] : $arr_value['min_range_slide']; //phpcs:ignore WordPress.Security.NonceVerification.Recommended
	$current_max_range = ! empty( $get_max_range ) ? ceil( floatval( wp_unslash( $get_max_range ) ) / $arr_value['step_range_slide'] ) * $arr_value['step_range_slide'] : $arr_value['max_range_slide']; //phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
<div class="viwcpf_wrap_filter-content"
    ...
    <h4 class="viwcpf_filter-title <?php echo esc_attr( $toggle_class_name ); ?>"><?php echo esc_html( $filter_name ); ?></h4>
