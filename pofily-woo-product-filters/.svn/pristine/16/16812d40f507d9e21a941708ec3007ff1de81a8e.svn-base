...
//		wp_enqueue_style( $this->viwcpf_woo_product_filters . '-select2', VIWCPF_FREE_DIR_URL . 'assets/css/select2.min.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		 ...
		 * class.
		 */
		wp_enqueue_script( $this->viwcpf_woo_product_filters . '-js-rangeSlide', VIWCPF_FREE_DIR_URL . 'assets/js/ion.rangeSlider.min.js', array( 'jquery' ), $this->version, true );
//		wp_enqueue_script( $this->viwcpf_woo_product_filters . '-select2', VIWCPF_FREE_DIR_URL . 'assets/js/select2.js', array( 'jquery' ) );
		wp_enqueue_script( $this->viwcpf_woo_product_filters . '-vi_dropdown', VIWCPF_FREE_DIR_URL . 'assets/js/vi_dropdown.js', array( 'jquery' ), $this->version, true );
		wp_enqueue_script( $this->viwcpf_woo_product_filters . '-off_canvas', VIWCPF_FREE_DIR_URL . 'public/js/off_canvas.js', array( 'jquery' ), $this->version, true );
			...
			if ( $this->check_conditional_menu( $id_menu ) ) {
				if ( $viwcpf_filter_menu ) {
                    ...
                    <form class="viwcpf_form_filter <?php echo esc_attr( $option_style . ' ' . $ajaxClassName . ' ' . $submitBtnClassName ); ?>"
                          method="GET" action="<?php echo esc_url_raw( home_url( $wp->request ) ); ?>">
						<?php
    ...
    */
	public function pre_get_post_filter( $query ) {
		if (
			...
			if ( isset( $_GET['instock_filter'] ) && sanitize_text_field( $_GET['instock_filter'] ) ) {//phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$query->set(
					'meta_query',
					array(
						array(
			...
			if ( isset( $_GET['onsale_filter'] ) && sanitize_text_field( $_GET['onsale_filter'] ) ) {//phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$query->set(
					'meta_query',
					array(
						array(
			...
			if ( isset( $_GET ) ) {//phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				foreach ( wc_clean( $_GET ) as $key_get => $value_get ) {//phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( sizeof( $viwcpf_arr_metakey ) > 0 ) {
					...
					foreach ( $viwcpf_arr_metakey as $item_metakey ) {
						...
						if ( in_array( $item_metakey, $viwcpf_arr_metakey_option ) ) {
							if ( isset( $_GET[ 'viwcpf_metakey_' . $viwcpf_metakey_filter ] ) && isset( $_GET[ 'viwcpf_metakey_type_' . $viwcpf_metakey_filter ] ) ) {//phpcs:ignore WordPress.Security.NonceVerification.Recommended
								$viwcpf_metakey_type_filter  = sanitize_text_field( $_GET[ 'viwcpf_metakey_type_' . $viwcpf_metakey_filter ] );//phpcs:ignore WordPress.Security.NonceVerification.Recommended
								$viwcpf_metakey_value_filter = sanitize_text_field( $_GET[ 'viwcpf_metakey_' . $viwcpf_metakey_filter ] );//phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
//		$assign_page = ' return ( is_archive("product") );';
    ...
    */
	public function viwcpf_render_value_block_filter( $filter_name, $filter_for, $filter_data, $filter_setting ) {
		...
		$input_hidden       = '';
		$link               = '';
		$base_link          = home_url( $wp->request );//phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$current_url_param  = wc_clean( $_GET );//phpcs:ignore WordPress.Security.NonceVerification.Recommended
					...
					foreach ( $sorted_terms as $sorted_term_item ) {
						...
						if ( $multiselect ) {
							...
							if (
								...
								$current_filter = isset( $_GET[ $tax_name ] ) ? explode( ' ', urldecode( wc_clean( wp_unslash( $_GET[ $tax_name ] ) ) ) ) : array();//phpcs:ignore WordPress.Security.NonceVerification.Recommended
							} else {
							...
							$current_filter = isset( $_GET[ $tax_name ] ) ? sanitize_text_field( $_GET[ $tax_name ] ) : '';//phpcs:ignore WordPress.Security.NonceVerification.Recommended
							/*Remove query args current*/
							$link = remove_query_arg( $tax_name, $base_link );
							/*Remove current tax value in $current_url_param*/
							unset( $current_url_param[ $tax_name ] );
...
//                        $data_tooltip = 'data-tooltip='.esc_attr($tooltip).'';
					...
					$current_min_price = isset( $_GET['min_price'] ) ? sanitize_text_field( $_GET['min_price'] ) : 0;//phpcs:ignore WordPress.Security.NonceVerification.Recommended
					$current_max_price = isset( $_GET['max_price'] ) ? sanitize_text_field( $_GET['max_price'] ) : PHP_INT_MAX;//phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				$input_hidden = array(
					...
					'name'  => 'min_price',
					'value' => isset( $_GET['min_price'] ) ? sanitize_text_field( $_GET['min_price'] ) : '',//phpcs:ignore WordPress.Security.NonceVerification.Recommended
				);
				array_push( $render_array['input_hidden'], $input_hidden );
				...
				$input_hidden = array(
					...
					'name'  => 'max_price',
					'value' => isset( $_GET['max_price'] ) ? sanitize_text_field( $_GET['max_price'] ) : '',//phpcs:ignore WordPress.Security.NonceVerification.Recommended
				);
				array_push( $render_array['input_hidden'], $input_hidden );
			...
			case 'filter_by_name_product':
				...
				$search_value               = isset( $_GET['s'] ) ? sanitize_text_field( $_GET['s'] ) : '';//phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$list_label_items           = array(
				...
				$input_hidden               = array(
					'class' => $filter_for . ' s',
					'name'  => 's',
					'value' => isset( $_GET['s'] ) ? sanitize_text_field( $_GET['s'] ) : '',//phpcs:ignore WordPress.Security.NonceVerification.Recommended
				);
				array_push( $render_array['input_hidden'], $input_hidden );
				break;
			default:
    ...
    */
		...
		if ( $_chosen_tax_query = WC_Query::get_main_tax_query() ) { // phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.Found, WordPress.CodeAnalysis.AssignmentInCondition.Found
			...
			foreach ( $_chosen_tax_query as $data_tax_query ) {
				...
				if (
					...
					if ( isset( $_GET[ $filter_name ] ) ) {//phpcs:ignore WordPress.Security.NonceVerification.Recommended
						$link = add_query_arg( $filter_name, wc_clean( wp_unslash( $_GET[ $filter_name ] ) ), $link );//phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function viwcpf_get_clear_url( $key_filters ) {
		...
		if ( sizeof( $arr_key_filters ) > 0 ) {
			foreach ( $arr_key_filters as $key_filter ) {
				if ( array_key_exists( $key_filter, $_GET ) ) {//phpcs:ignore WordPress.Security.NonceVerification.Recommended
					$has_filter = true;
				}
			}
			if ( $has_filter ) {
	...
	protected function get_filtered_term_product_counts( $term_ids, $taxonomy, $query_type ) {
		...
		$min_price = isset( $_GET['min_price'] ) ? sanitize_text_field( $_GET['min_price'] ) : '';//phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$max_price = isset( $_GET['max_price'] ) ? sanitize_text_field( $_GET['max_price'] ) : '';//phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		if ( ! isset( $cached_counts[ $query_hash ] ) ) {// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public function viwcpf_get_min_max_meta_values( $meta_key = '', $end = 'max' ) {
		...
		if ( $end == 'max' ) {
			$query_value = $wpdb->prepare( "SELECT max(cast( meta_value as unsigned)) FROM  {$wpdb->postmeta} WHERE meta_key = '%s'", $meta_key );//phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.QuotedSimplePlaceholder
		} else if ( $end == 'min' ) {
			$query_value = $wpdb->prepare( "SELECT min(cast( meta_value as unsigned)) FROM {$wpdb->postmeta} WHERE meta_key = '%s'", $meta_key );//phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.QuotedSimplePlaceholder
		}

		$result_value = $wpdb->get_var( $query_value );//phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,	WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
