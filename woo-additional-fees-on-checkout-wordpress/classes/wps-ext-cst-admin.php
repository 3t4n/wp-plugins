...
class WPS_EXT_CST_Admin
{
	public function __construct()
	{
		...
		add_action( 'admin_notices', array($this,'show_premium_buy_notice' ));
		add_action( 'admin_init', array($this,'show_premium_buy_notice_dismissed' ));
	}
	public function show_premium_buy_notice() {
	    $user_id = get_current_user_id();
	    if ( !get_user_meta( $user_id, 'wps_premium_buy_notice' ) ){
	        echo '<div class="notice notice-success is-dismissible" style="display:flex;">
	    			<img src="https://wpsuperiors-media.s3.amazonaws.com/wp-content/uploads/2025/01/30163651/limited.png" />
	    			<span style="padding-top:4%; font-size:20px;">WooCommerce Additional Fees On Checkout PREMIUM plugin at <b style="font-weight:700;">19 USD</b> only.
	    			<a class="button button-primary" href="?wps-dismissed" style="margin-top:-5px;">Click To Grab</a></span>
	    		</div>';
	    }
	}

	public function show_premium_buy_notice_dismissed() {
	    $user_id = get_current_user_id();
	    if ( isset( $_GET['wps-dismissed'] ) ){
	        add_user_meta( $user_id, 'wps_premium_buy_notice', 'true', true );
	    	wp_redirect( "https://www.wpsuperiors.com/shop/woo-additional-fees-on-checkout/", 301 );
	    	exit();
	  	}
	}
	...
	public static function wps_generate_new_fees(){
		$fees_number = (int)$_POST['number'];
		?>

		<div class="wps-ext-cst-fees" id="fees<?php echo $fees_number;?>">
    		<h3 style="border-bottom: 1px solid black;">
    			<span class="fees-title">Unlabelled Fees</span>
    			<span style="float:right; color:red; cursor: pointer;" class="dashicons dashicons-trash" onclick="remove_fees(<?php echo $fees_number;?>)"></span>
    		</h3>
    		<table class="form-table">
            	<tbody>
					<tr>
						...
						<td>
							<select name="ext_cst_extra[<?php echo $fees_number;?>][status]" id="ext_cst_status_extra">
					...
					<tr>
						...
						<td>
							<input type="text" name="ext_cst_extra[<?php echo $fees_number;?>][label]" class="regular-text code" id="ext_cst_label_extra" value="<?php echo 'Unlabelled Fees #'.$fees_number;?>"/>
						</td>
					</tr>
					<tr>
						<th scope="row"><label><?php _e( 'Label for Billing'); ?></labe></td>
						<td>
							<input type="text" name="ext_cst_extra[<?php echo $fees_number;?>][label_billing]" class="regular-text code" id="ext_cst_label_billing_extra" value="Unlabelled Fees #<?php echo $fees_number;?>"/>
						</td>
					</tr>
					<tr>
						<th scope="row"><label><?php _e( 'Type'); ?><label></th>
						<td>
							<select name="ext_cst_extra[<?php echo $fees_number;?>][amount_type]" id="ext_cst_amount_type_extra" onchange="showHideTaxShipping(this,<?php echo $fees_number;?>)">
					...
					<tr id="incTax-<?php echo $fees_number; ?>" class="incTax" style="display:none;">
					...
					<tr id="incShipCosts-<?php echo $fees_number; ?>" class="incShipCosts" style="display:none;">
					...
					<tr>
						...
						<td>
							<input type="number" step="any" name="ext_cst_extra[<?php echo $fees_number;?>][amount]" class="fees_amount regular-text code" id="ext_cst_amount_extra<?php echo $fees_number;?>" value="1"/>
						</td>
					</tr>
					<tr>
						<th scope="row"><label><?php _e( 'Apply Condition'); ?></label></th>
						<td>
							<select name="ext_cst_extra[<?php echo $fees_number; ?>][apply_type]" id="ext_cst_apply_type<?php echo $fees_number; ?>">
					...
					<tr>
						...
						<td>
							<select data-id="<?php echo $fees_number;?>" name="ext_cst_extra_apply_cndtn<?php echo $fees_number;?>" id="ext_cst_apply_cndtn_extra<?php echo $fees_number;?>" class="ext_cst_cndtn_dropdown"  onchange="show_hide_cndtn_extra(<?php echo $fees_number;?>)">
					...
					<tr id="cart_total_amount<?php echo $fees_number;?>" class="cndtn_mode_extra<?php echo $fees_number;?>" style="display: none;">
					...
					<tr id="cart_no_product<?php echo $fees_number;?>" class="cndtn_mode_extra<?php echo $fees_number;?>" style="display: none;">
					...
					<tr id="selected_product<?php echo $fees_number;?>" class="cndtn_mode_extra<?php echo $fees_number;?>" style="display: none;">
					...
					<tr id="selected_category<?php echo $fees_number;?>" class="cndtn_mode_extra<?php echo $fees_number;?>" style="display: none;">
					...
					<tr id="selected_pr_type<?php echo $fees_number;?>" class="cndtn_mode_extra<?php echo $fees_number;?>" style="display: none;">
