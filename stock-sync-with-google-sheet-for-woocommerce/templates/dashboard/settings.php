	...
	<div class="ssgs-admin">
		<div class="ssgs-dashboard__block">
			...
			<div class="form-group">
				<label :class="forUltimate">
					<div class="ssgs-check">
						<input :readonly="!isPro" type="checkbox" name="formula_active" class="check" x-model="option.formula_active" :checked="option.formula_active == 1" @change="save_change++">
						<span class="switch"></span>
					</div>
					<span class="label-text">
					<?php
					...
					<span class="ssgs-badge "  style="background-color: #A52A2A;">
					<?php
					esc_html_e(
						'Beta',
						'stock-sync-with-google-sheet-for-woocommerce'
				...
				<div class="description">
					...
					<?php
					esc_html_e(
						'Enable Formula feature in Google sheets',
						'stock-sync-with-google-sheet-for-woocommerce'
					);
					?>
					</p>
			...
			<div class="form-group" :class="forUltimate">
				...
				<span class="ssgs-badge green">New</span>
				<span x-show="!isPro" class="ssgs-badge purple ssgsw-promo">
					<?php esc_html_e( 'Ultimate', 'stock-sync-with-google-sheet-for-woocommerce' ); ?>
				</span>
				
				<div class="ssgsw_description" style="margin-top: 10px;" :class="forUltimate" x-init="select2Alpine_status">
					<?php
						$check_box_values = ssgsw_get_all_product_statuses();
						$checked_value  = ssgsw_get_option('show_product_status_data') ?? [];
					?>
					<select :disabled="!isPro" x-ref="show_product_status_info" class="show_product_status_data form-control" multiple="multiple" name="show_product_status_data[]">
						<?php
						foreach ( $check_box_values as $key => $value ) {
							if ( ! empty($checked_value) ) {
								if ( in_array($value, $checked_value) ) {
									printf('<option value="%s" selected>%s</option>', esc_html($value), esc_html($value));
								} else {
									printf('<option value="%s">%s</option>', esc_html($value), esc_html($value));
								}
							} else {
								printf('<option value="%s">%s</option>', esc_html($value), esc_html($value));
							}
				...
				<div class="description">
					<p>
						<?php
						esc_html_e(
							'Filter the product statuses to sync between WooCommerce and Google Sheets.',
							'stock-sync-with-google-sheet-for-woocommerce'
						);
						?>
					</p>
		...
		<div class="ssgs-dashboard__block">
			...
			<?php
			if ( ssgsw_get_ult_version() ) {
				?>
					<div class="form-group">
						<label :class="forUltimate">
							<div class="ssgs-check">
								<input :readonly="!isPro" type="checkbox" name="show_long_description" class="check" x-model="option.show_long_description" :checked="option.show_long_description == 1" @change="save_change++">
								<span class="switch"></span>
							</div>
							<span class="label-text">
						<?php
						esc_html_e(
							'Sync Long description',
							'stock-sync-with-google-sheet-for-woocommerce'
						);
						?>
							</span>
						...
						<?php
						esc_html_e(
							'Enable this button to show the long description of the product in a column on the Google Sheets',
							'stock-sync-with-google-sheet-for-woocommerce'
						);
						?>
							</p>
			...
			<div class="form-group">
				<label :class="forUltimate">
					<div class="ssgs-check">
						<input :readonly="!isPro" type="checkbox" name="show_product_url" class="check" x-model="option.show_product_url" :checked="option.show_product_url == 1" @change="save_change++">
						<span class="switch"></span>
					</div>
					<span class="label-text">
				   <?php
				...
				<div class="description">
					...
					<?php
					esc_html_e(
					   'Enable this feature to show the product URL in the Google Sheet',
					   'stock-sync-with-google-sheet-for-woocommerce'
					);
					?>
					</p>
			...
			<div class="form-group">
				<label :class="forUltimate">
					<div class="ssgs-check">
						<input type="checkbox" name="show_product_tags" class="check" x-model="option.show_product_tags" :checked="option.show_product_tags == 1" @change="save_change++">
						<span class="switch"></span>
					</div>
					<span class="label-text">
					<?php
				...
				<div class="description">
					...
					<?php
					esc_html_e(
						'Enable this button to show the tags of the product in a column on the Google Sheets',
						'stock-sync-with-google-sheet-for-woocommerce'
					);
					?>
					</p>
			...
			<div class="form-group">
				<label :class="forUltimate">
					<div class="ssgs-check">
						<input type="checkbox" name="show_product_status" class="check" x-model="option.show_product_status" :checked="option.show_product_status == 1 && isPro" @change="save_change++">
						<span class="switch"></span>
					</div>
					<span class="label-text show_product_status">
					<?php
					esc_html_e(
						'Sync Product Status',
						'stock-sync-with-google-sheet-for-woocommerce'
				...
				<div class="description">
					...
					<?php
					esc_html_e(
						'Enable this button to show the Status of the product in a column on the Google Sheets.',
						'stock-sync-with-google-sheet-for-woocommerce'
					);
					?>
					</p>
		...
		<div class="ssgsw_button_container" x-show="save_change" style="transition: opacity 300ms ease-in-out 0ms;">
			<button x-on:click="save_checkbox_settings('');isLoading = true" :disabled="isLoading" class="ssgsw_save_button">
				<span x-show="!isLoading"><?php esc_html_e('Save & Sync','stock-sync-with-google-sheet-for-woocommerce'); ?></span>
