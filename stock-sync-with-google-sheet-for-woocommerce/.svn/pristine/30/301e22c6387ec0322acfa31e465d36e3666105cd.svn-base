	...
	class Column extends Base {
		...
		public function get_all_columns() {
			$columns = [
				...
				'post_status' => [
					'label' => __('Status', 'stock-sync-with-google-sheet-for-woocommerce'),
					'type'  => 'field',
					'priority' => 9999,
				],
			];
		...
		public function custom_meta_fields() {
			$custom_array = [];
			return apply_filters('ssgsw_add_custom_meta_fileds', $custom_array);
		}
		...
		public function get_columns() {
			...
			$product_url            = true === wp_validate_boolean( ssgsw_get_option('show_product_url', false) );
			$long_description       = true === wp_validate_boolean( ssgsw_get_option('show_long_description', false) );
			$post_status            = true === wp_validate_boolean( get_option('ssgsw_show_product_status', false) );
			$product_tag       = true === wp_validate_boolean( ssgsw_get_option('show_product_tags', false) );
			...
			if ( ! $product_url ) {
				unset($columns['ssgsw_product_url']);
			}
			if ( ! $long_description ) {
				unset($columns['post_content']);
			}
			if ( ! $show_product_category ) {
				unset($columns['product_cat']);
			}
			if ( ! $show_product_images || ! ssgsw_is_license_valid() ) {
				unset($columns['product_image']);
			}
			if ( ! $post_status || ! ssgsw_is_license_valid() ) {
				unset($columns['post_status']);
			}
			if ( ! $show_total_sales ) {
