...
if ( ! class_exists('\StockSyncWithGoogleSheetForWooCommerce\API') ) {
	...
	class API extends Base {
		...
		public function action_sync( $request ) {
			...
			$ssgsw_items = isset( $params['ssgsw_items'] ) ? $params['ssgsw_items'] : false;
			$ssgsw_offset = isset( $params['ssgsw_offset'] ) ? $params['ssgsw_offset'] : 0;
			$ssgsw_batch_size = isset( $params['ssgsw_batch_size'] ) ? $params['ssgsw_batch_size'] : 0;
			$ssgsw_index = isset( $params['ssgsw_index'] ) ? $params['ssgsw_index'] : 1;
			try {
				if ( isset($params['ssgsw_items']) ) {
					if ( $ssgsw_items ) {
						update_option('ssgsw_license_sync', false);
						$response = ssgsw_count_all_product();
						if ( $response ) {
							ssgsw_end_sync();
							return $this->response(
								true,
						...
						$product = new Product();
						$update = $product->sync_batch_all($ssgsw_offset, $ssgsw_batch_size, $ssgsw_index);
						if ( is_array($update) && $update['success'] ) {
							ssgsw_end_sync();
							return $this->response(true, $update['count'] );
						} else {
							ssgsw_end_sync();
							return $this->response(false, __('Something went wrong', 'stock-sync-with-google-sheet-for-woocommerce'));
						}
					}
				} else {
					$product = new Product();
		...
		public function update_api_callback( $request ) {
			...
			$sync_all_active = isset( $body['sync_all'] ) && 'sync_all' === $body['sync_all'] ? true : false;
			$product_length = isset( $body['arrayLength'] ) ? $body['arrayLength'] : 1;
			$formula = isset( $body['formula'] ) ? $body['formula'] : false;
			...
			if ( $products && ( is_array($products) || is_object($products) ) && ! empty($products) ) {
				...
				try {
					$product  = new Product();
					if ( isset($body['newapp']) ) {
						if ( $formula ) {
							$response = $product->bulk_formula_update($products);
						} else {
							$response = $product->chunk_product_update($products, $first_index, $last_index);
						}
					} else {
						$response = $product->bulk_update($products, $sync_all_active, $product_length);
					}
					ssgsw_end_sync();
					if ( $response ) {
