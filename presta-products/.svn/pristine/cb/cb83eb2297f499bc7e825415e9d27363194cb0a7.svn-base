...
class PrestaProducts 
{
    ...
    public function __construct()
    {
        ...
        // GBO - 1.2.0
        add_option( 'ppgbo_use_categories', 0 );
        ...
        if ( is_admin() ) {
            ...
            // Securité si la clé n'est pas OK et 5 connexions échouées
                ...
                update_option( 'ppgbo_use_categories', 0);
            }
        ...
        function ppgbo_get_products_from_prestashop($atts) {
            ...
            if (isset($atts['bestsellers'])) {
                ...
                if ($http_code == '200') {
					...
					else if (isset($atts['auto_category'])) {
					    $id_category = $this::getCorrespondance(get_the_category()[0]->cat_ID);
					    
		                // Appel HTTP API
		                $link       = $prestashop_url . '/api/products/?filter[id_category_default]=[' . $id_category . ']&ws_key=' . $webservice_key . '&output_format=JSON';
		                $response   = wp_remote_get( $link );
		                $body       = wp_remote_retrieve_body( $response );
		                $http_code  = wp_remote_retrieve_response_code( $response );
            ...
            // Correspondance Catégories
			    ...
			    $id_category = getCorrespondance(get_the_category()[0]->cat_ID);

                // Appel HTTP API
                $link       = $prestashop_url . '/api/products/?filter[id_category_default]=[' . $id_category . ']&limit=' . $limit . '&ws_key=' . $webservice_key . '&output_format=JSON';
                $response   = wp_remote_get( $link );
                $body       = wp_remote_retrieve_body( $response );
                $http_code  = wp_remote_retrieve_response_code( $response );
        ...
        function ppgbo_delete_correspondance() {
            global $wpdb;
            
            $id = $_POST['id'];
            
            // Réinit la base
            $myTable    = $wpdb->prefix.'ppgbo_categories';
            $sql        = $wpdb->prepare("DELETE FROM %i WHERE id = %d", $myTable, $id); 
            $wpdb->query($sql) or wp_die(__( 'Problème de suppression de la correspondance.', 'presta-products' ));
            
            echo __( 'Correspondance supprimée.', 'presta-products' );
                    
            // On arrête l'ajax
            wp_die();
        }
        
        add_action( 'wp_ajax_deleteCorrespondance', 'ppgbo_delete_correspondance' );
        ...
        function shortcodes_init()
        {
            ...
            function display_product_function($atts, $content = null, $tags = '') 
            {
                ...
                if (!isset($atts['product']) && !isset($atts['products']) && !isset($atts['category']) && !isset($atts['categories']) && !isset($atts['bestsellers']) && !isset($atts['auto_category'])) 
                {
                    return "";    
                }
                ...
                $use_categories             = get_option( 'ppgbo_use_categories' );
        ...
        // Correspondance
        function getCorrespondance($id_cat_wp) {
            global $wpdb;
            
            $myTable    = $wpdb->prefix.'ppgbo_categories';
            $sql        = $wpdb->prepare("SELECT id_cat_prestashop FROM $myTable WHERE id_cat_wordpress = %d", $id_cat_wp); 
            
            return $wpdb->get_results($sql)[0]->id_cat_prestashop;
        }
    }
    ...
    public function ppgbo_plugin_menu()
    {
        ...
        if ($this->is_freemium) {
            ...
            add_submenu_page(
                'presta-products', 
                __( 'Correspondances', 'presta-products' ), 
                __( 'Correspondances', 'presta-products' ), 
                'manage_options', 
                'presta-products&tab=categories',
				'categories_page'
            );
        }
    }
    ...
    { 
        ...
        if ( is_admin() ) {
            ...
            else if (isset($_POST['type']) && $_POST['type'] == 'categories') {
                $use_categories             = (isset($_POST['use_categories'])) ? 1 : 0;
    
                // Modification des options
                update_option( 'ppgbo_use_categories', $use_categories );   
    
                // Gestion du message d'information
                update_option( 'ppgbo_message', __('Gestion de la correspondance des catégories enregistrée.', 'presta-products') );
            }
            // Formulaire des catégories (correspondances)
            else if (isset($_POST['type']) && $_POST['type'] == 'correspondances_cat') {
                $id_cat_wordpress   = (isset($_POST['id_cat_wordpress'])) ? sanitize_text_field($_POST['id_cat_wordpress']) : 0;
                $id_cat_prestashop  = (isset($_POST['id_cat_prestashop'])) ? sanitize_text_field($_POST['id_cat_prestashop']) : 0;
                $id                 = (isset($_POST['id']) && !empty($_POST['id'])) ? sanitize_text_field($_POST['id']) : 0;
                
                if ($id_cat_wordpress > 0 && $id_cat_prestashop > 0) {
                    $myTable    = $wpdb->prefix.'ppgbo_categories';
                    
                    if ($id > 0) {
                    	// Requête SQL de modification
                        $sql        = $wpdb->prepare("UPDATE $myTable SET id_cat_wordpress = %d, id_cat_prestashop = %d WHERE id = %d", intval($id_cat_wordpress), intval($id_cat_prestashop), intval($id));
                    }
                    else {
                    	// Requête SQL d'ajout
                        $sql        = $wpdb->prepare("INSERT IGNORE INTO $myTable (id_cat_wordpress, id_cat_prestashop) VALUES (%d, %d)", intval($id_cat_wordpress), intval($id_cat_prestashop));
                    }
                    ...
                    if ($result === false) {
                        // Gestion du message d'information
                        update_option( 'ppgbo_message', __('Problème lors de l\'enregistrement de la correspondance de la catégorie.', 'presta-products') );
                    }
                }
            }
            ...
            $page                       = (isset($_GET['tab']) && in_array($_GET['tab'], array('options', 'features', 'examples', 'used', 'cache', 'categories'))) ? sanitize_text_field($_GET['tab']) : ((isset($_GET['tab']) && !in_array($_GET['tab'], array('options', 'features', 'examples', 'used', 'cache', 'categories'))) ? '404' : 'options');
            $list                       = $this->ppgbo_getListing();
            $caches                     = $this->ppgbo_get_cache();
            $categories                 = get_categories();
            $correspondances            = $this->ppgbo_get_correspondances();
            $correspondance             = ($_GET['id']) ? $this->ppgbo_get_correspondances($_GET['id']) : null;
            ...
            if ($this->is_freemium) { 
                ...
                include 'includes/tabs/categories.php';
            }
    ...
    public function ppgbo_install_mysql() {
        ...
        // 1.2.0 : ajout de la correspondance des catégories
        // Création de la table
        $simple_sql = "CREATE TABLE {$wpdb->prefix}ppgbo_categories (
                 id bigint(20) unsigned NOT NULL auto_increment,
				 id_cat_wordpress bigint(20) NOT NULL default 0,
				 id_cat_prestashop bigint(20) NOT NULL default 0,
                 PRIMARY KEY  (id)
                ) {$charset_collate};";
        ...
        // Index unique
        $simple_sql = "CREATE UNIQUE INDEX id_cat_index ON {$wpdb->prefix}ppgbo_categories ( id_cat_wordpress );";
        $wpdb->query( $simple_sql );
    }
    ...
    public function ppgbo_get_correspondances($id = null) {
        global $wpdb;
        ...
        $result     = $wpdb->prepare($sql, $myTable);        
