...
class All_Images_Ai_Admin
{
    ...
    public function register_settings()
    {
        ...
        register_setting($page, 'all-images-ai-settings', array($this, 'general_settings_callback'));
    ...
    public function general_settings_callback($value)
    {
        $options = get_option('all-images-ai-settings');
        if (!empty($options['all_images_automatic'])) {
            $value['all_images_automatic'] = $options['all_images_automatic'];
        }
    ...
    public function handle_webhook()
    {
        ...
        if ($this->wpdb->rows_affected === 0) {
            wp_send_json_error(
                array('error' => sprintf(__('No line updated for query: %s', 'all-images-ai'), $this->wpdb->last_query))
            );
        } else {
...
SQL,
    ...
    public function select_image_for_library()
    {
        ...
        if (!is_wp_error($library_image_id)) {
            return wp_send_json_success(array(
                'attachment_id' => $library_image_id
            ));
        } else {
            return wp_send_json_error(array(
                'error' => sprintf('upload error: %s', $library_image_id->get_error_message())
            ), 500);
        }
    }
...
SQL,
    ...
    private function _get_image_by_url($id, $url, $post_id = null, $metadata = null)
    {
        ...
        $mime = wp_get_image_mime($result);

        if ($mime === false) {
            @unlink($result);
            return new WP_Error('mime_error', __('Type cannot be determined', 'all-images-ai'));
        }
        ...
        $move_new_file = @copy($result, $filename);
        @unlink($result);

        if ($move_new_file === false) {
            return new WP_Error('copy_error', __('File cannot be moved', 'all-images-ai'));
        }
