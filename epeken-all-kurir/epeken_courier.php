   ...
   )
{
    ...
    add_filter('woocommerce_billing_fields', 'epeken_customize_billing_fields', 10, 1);
    add_filter('woocommerce_shipping_fields', 'epeken_customize_shipping_fields', 10,1);
    function epeken_customize_billing_fields($billing_fields){
	    if(!is_checkout()) return $billing_fields;
	    $check_email = get_option('epeken_email_optional');
	    if($check_email === 'on')
		    $billing_fields['billing_email']['required'] = false;
	    $check_company = get_option('epeken_company_optional');
  	    if($check_company === 'on')
		    unset($billing_fields['billing_company']); 
	    return $billing_fields;
    }
    function epeken_customize_shipping_fields($shipping_fields) {
	    $check_company = get_option('epeken_company_optional');
  	    if($check_company === 'on')
...
function epeken_additional_cost_cod_kurir($shipping) {
   ...
   if($enable === 'on' && $pm === 'cod') {
	$valid_services = $shipping -> cod_kurir_services; 
        $new_array_of_tarif = array(); 
 	foreach ($shipping -> array_of_tarif as $tarif) {
 	 if(in_array($tarif['id'], $valid_services))	
		array_push($new_array_of_tarif, $tarif);
        }
	$shipping -> array_of_tarif = $new_array_of_tarif;
	if(count($shipping -> array_of_tarif) > 0)
	  add_action('woocommerce_cart_calculate_fees', 'epeken_add_fee_cod_kurir');
   } 
}

add_action('epeken_derive_gojek_tarif','epeken_disable_ojek_online', 99);
function epeken_disable_ojek_online($shipping) {
   $enable = get_option('epeken_enable_cod_kurir');
   $pm = trim(WC() -> session -> get('chosen_payment_method'));
   if($enable === 'on' && $pm === 'cod') {
	$shipping -> array_of_tarif =  array();
   }	
}

function epeken_add_fee_cod_kurir() {
   global $woocommerce;
   $perc = (int)get_option('epeken_cod_kurir_perc');
...
if(!has_filter('woocommerce_cart_item_name')){
...
/* Only Paypal for International Payments */
function set_payment_only_paypal ($gateways) {
            $gateway = new WC_Gateway_Paypal();
            $gateways = array();
            $gateways['paypal'] = $gateway;
            return $gateways;
  }

add_action('epeken_custom_international_tariff', 'epeken_only_paypal_payment_allowed');
function epeken_only_paypal_payment_allowed() {
 $flag = get_option('epeken_intl_payment_paypal');
 if($flag === 'on')
   add_filter('woocommerce_available_payment_gateways', 'set_payment_only_paypal', 1);  
}

add_action('woocommerce_after_checkout_validation', 'epeken_validate_cod_kurir_free_ongkir', 10,2);

function epeken_validate_cod_kurir_free_ongkir($fields, $errors) {
	$flag = get_option('epeken_enable_cod_kurir');
	$logger = new WC_Logger(); 
	$payment_method = $_POST['payment_method'];
	$shipping_method = $_POST['shipping_method'][0];
	if ($flag === 'on' && $payment_method === 'cod' && $shipping_method === 'epeken_courier') {
        	$errors -> add('validation','COD dengan kurir tidak berlaku untuk free ongkir'); 
	}
}
