...
add_filter('woocommerce_my_account_my_orders_actions','epeken_add_selesaikan_pesanan_button',1,2);
function epeken_add_selesaikan_pesanan_button($actions,$order) {
	$order_number = $order -> get_order_number();
	$order = new WC_Order($order_number);
	...
	if($order -> get_status() === 'completed')
    	...
    	$action['name'] =  'Selesaikan Pesanan';
	$action['url'] = add_query_arg( NULL, NULL ). '&complete-order='.$order_number;
	array_push($actions, $action);
        return $actions;		
}  

add_action('woocommerce_account_content', 'epeken_selesaikan_pesanan_oleh_pembeli');
function epeken_selesaikan_pesanan_oleh_pembeli() {
	$orders = isset($_GET['orders']) ? true : false;
	$complete_order = isset($_GET['complete-order']) ? $_GET['complete-order'] : null;

	if($orders === false)
		return;
	
	$current_user_id = get_current_user_id();
	$order = wc_get_order($complete_order);
	...
	if($current_user_id === $order_customer_id) {
         $order -> add_order_note('Pesanan dikonfirmasi selesai oleh pembeli');	
	 $order -> update_status('completed');
	}else{
	 echo 'Mohon maaf, Anda tidak berhak menyelesaikan pesanan yang tidak ditujukan untuk Anda.';
	}
	//TODO: Validasi user yang mengubah pesanan telah selesai.
}
...
function epeken_konfirmasi_pembayaran_shortcode_details($atts) {
	...
	//     echo "<p>Mohon maaf, formulir konfirmasi pembayaran tidak mendukung multi site.</p>";
        //     return;
	//}
