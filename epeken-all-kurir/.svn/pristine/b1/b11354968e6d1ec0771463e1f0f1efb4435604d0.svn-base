...
class WC_Shipping_Tikijne extends WC_Shipping_Method{	
	...
	public function __construct(){
		...
		$this -> cod_kurir_services = array('JNE_REG', 'jet.co.id_EZ', 'sicepat_REGULAR');
	...
	public function refresh_usd_rate() {
		...
		public function activate(){
			...
			//$this -> do_classic_checkout();
		}

		public function do_classic_checkout() {
	...
	public function generate_epeken_tools_html() {
                ...
                ?><tr valign="top">
                        <th>Tools</th><td>
							...
							<tr><td>Classic Checkout</td><td>
							<p><input class="button-primary woocommerce-save-button" type="submit" name="save" value="Do Classic Checkout" 
							onclick="if(confirm('Anda yakin ingin menggunakan halaman checkout versi klasik ?')==false){return false;}"
							></p>
							</td></tr>
							<tr><td>International Payment</td><td>
							<p><input <?php if(get_option('epeken_intl_payment_paypal') === 'on') {echo 'checked';}?> type="checkbox" name="epeken_intl_payment_paypal">Hanya pembayaran Paypal yang diperbolehkan untuk pesanan yang memiliki tujuan shipping luar Indonesia</input></p>
							</td></tr>
							<tr>
								<?php 
		 ...
		 <tr>
		<?php $email_optional = get_option('epeken_email_optional'); $company_optional = get_option('epeken_company_optional');?>
		 <td>Formulir Checkout</td>
		<td><p><input type="checkbox" name="email_optional" <?php if($email_optional) echo "checked";?>>Email <strong>tidak wajib</strong> saat checkout</input>
		<br><i>Mengisi email optional (tidak wajib) pada saat checkout</i></br></p>
		<p><input type="checkbox" name="company_optional" <?php if($company_optional) echo "checked";?>>Isian Company <strong>tidak ditampilkan</strong> saat checkout</input>
		<br><i>Isian Company dihilangkan dari formulir checkout</i></br></p>
		</td>
		 </tr>
		</table>
		</td>
...
?>
			...
			<div style="float: left;width: 100%; margin-bottom: 5px;"><hr></div><div style="clear: left;"/>
				...
				<?php
				 $markup_intl = get_option('epeken_markup_tarif_intl'); 
				 if (empty($markup_intl))
					$markup_intl = '0';
				?>
			 	  <strong>Mark Up Ongkir Internasional</strong> <input value="<?php echo $markup_intl;?>" style="width: 75px;" type="number" name="woocommerce_wc_shipping_tikijne_markup_intl"/> % (percent)
				</div>	
				</div>
				<div style="clear: left;">
				<div style="margin-top: 10px;margin-bottom: 10px;">
	...
	public function get_destination_city_and_kecamatan() {
			...
			$this -> shipping_city = str_replace('Kabupaten Morowali Utara', 'Kabupaten Morowali', urldecode($this -> shipping_city));
	}
	...
	public function set_shipping_cost_intl($package = array()) {
			...
			foreach ($json_tarrifs as $element) {
				...
				$usd_rate = $currency -> {'value'};
			      ...
			      #markup
				$markup_intl = get_option('epeken_markup_tarif_intl');
				if (!empty($markup_intl) && $markup_intl > 0)
				{
				  $rate = (1 + ($markup_intl/100)) * $rate;
				}
			      #endmarkup
	...
	public function set_shipping_cost(&$package = array()) {
			...
			if ($status != 200){
			 ...
			 add_action('woocommerce_cart_calculate_fees', array($this, 'add_subsidi_information'));
	...
	public function add_subsidi_information() {
		...
		$id_kurir = WC() -> session -> get('chosen_shipping_methods')[0];
		$price = 0;
   		foreach($this -> array_of_tarif as $t) {
         		if($t['id'] === $id_kurir)
           		{$price = $t['cost']; break;}
   		}
		if($cek && $price > 0 && $subsidi_ongkir_dengan_kupon > 0) {
			$subsidi = $subsidi_ongkir_dengan_kupon;

			if($subsidi > $price)
			   $subsidi = $price;

			$woocommerce -> cart -> add_fee('Diskon Ongkir Kupon '.$kode_kupon, $subsidi * (-1), false, '');
		}
	}
	...
	public function count_cart_weight_and_dimension($package = array()){
		...
		if($enable_dimensi_to_calc_ongkir === "yes" && $dimensi_barang > $cart_weight) {
		  $cart_weight = $dimensi_barang;
		}	
	...
	public function calculate_insurance() {
		 ...
		 if(empty($is_with_insurance)) {
			$is_with_insurance = isset($_POST['insurance_chkbox']) ? $_POST['insurance_chkbox'] : '';
		 }
                 if(empty($is_with_insurance)){
                        return;
                 }
