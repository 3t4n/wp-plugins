		...
		<form method="post" action="admin.php?page=wp_lcd.php">
			...
			<tr>
				<td style="padding-left:30px;padding-top:0;">
					...
					<br>
					<input type="radio" name="cookieMode" value="ckyes" id="ckMode-ckyes" <?php echo(get_option('cookieMode') == 'ckyes' ? 'checked': '') ?>>
					<label for="ckMode-ckyes"><?php _e('Lead Champion in cookieless mode with activation of cookies through the CookieYes&nbsp;&reg; manager','lead-champion-discover'); ?></label>
					<br>
...
function insert_lcd_script() {
	...
	if ($cookieMode == 'ckyes') {
		//error_log('insert_lcd_script: CKYES code');
		echo "<!-- BEGIN Lead Champion COOKIEYES compatibility tag from WP -->\n";
		echo "<script type=\"text/javascript\">\n";
		echo "   window.addEventListener('cookieyes_banner_load', function (e) {\n";
     		echo "      if (e.detail.categories.advertisement === true) {\n";
		echo "         /* when marketing cookies have been accepted */\n";
		echo "         window._lcCookie = 'on';\n";
		echo "      }\n";
		echo "   }, false);\n";
		echo "</script>\n";
		echo "<!-- END   Lead Champion COOKIEYES compatibility tag -->\n";
	}
}
