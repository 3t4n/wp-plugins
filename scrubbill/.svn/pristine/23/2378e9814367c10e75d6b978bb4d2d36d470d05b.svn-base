...
// WooCommerce sometimes tries to load this twice. Only allow it to be loaded once.
if ( ! class_exists( __NAMESPACE__ . '\Scrubbill_Shipping_Method' ) ) {
	/**
	...
	class Scrubbill_Shipping_Method extends \WC_Shipping_Method {
		...
		public function calculate_shipping( $package = array() ) {
			...
			// Get rates.
			$rates = $api->request( 'quote', 'POST', $data );
		...
		public function update_postnet_branches( $api, $api_last_updated ) {
			...
			if ( (string) $postnet_last_updated !== (string) $api_last_updated ) {
				$postnet_branches = $api->request( 'postnet/suburb' );

				if ( ! empty( $postnet_branches ) ) {
					$branches = array();

					foreach ( $postnet_branches as $branch ) {
						$branches[ $branch->id ] = array(
						'name'     => $branch->storeName, // @phpcs:ignore
						'suburb'   => $branch->suburb,
						'postcode' => $branch->postCode, // @phpcs:ignore
						);
					}
