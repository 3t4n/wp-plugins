...
.vxcf_options_row .howto{
  ...
  <div class="vx_wrap"> <img alt="<?php esc_html_e("Dynamics Feeds", 'gravity-forms-dynamics-crm') ?>" title="<?php esc_html_e("Dynamics Feeds", 'gravity-forms-dynamics-crm') ?>" src="<?php echo $this->get_base_url()?>images/<?php echo $this->crm_name ?>-crm-logo.png?ver=1" style="float:left; margin:0px 7px 10px 0;" height="46" />
  <h2 style="margin-bottom: 12px"><?php esc_html_e("Dynamics Feeds", 'gravity-forms-dynamics-crm'); ?>
  <?php
      if($fid != $config['id']){
  ?>
  <a class="add-new-h2" href="<?php echo esc_url($new_feed_link)?>"><?php esc_html_e("Add New", 'gravity-forms-dynamics-crm') ?></a>
  <?php
      }
      if(!empty($fid)){
  ?>
  <a class="add-new-h2" href="<?php echo esc_url($feeds_link)?>"><?php esc_html_e("Back to Feeds", 'gravity-forms-dynamics-crm') ?></a>
  <?php
      }
  ?>
  </h2>
...
if(empty($feed_name)){
  ...
  ?>
  <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop(sprintf(__("We are unable to login to Dynamics with the provided API token. Please make sure they are valid in the %sSettings Page%s", 'gravity-forms-dynamics-crm'), "<a href='".$this->link_to_settings()."'>", "</a>")); ?></div>
  <?php
  // return;
  } 
  ?>
  <div class="vx_wrapper">
  <div class="vx_heading"><?php echo sprintf(__('Edit Dynamics Feed # %d','gravity-forms-dynamics-crm') ,$fid) ?></div>
  ...
  <?php wp_nonce_field("vx_nonce") ?>
    <div class="vx_div">
      <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('1. Enter Feed name',  'gravity-forms-dynamics-crm' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_name" class="left_header"><?php esc_html_e("Feed Name", 'gravity-forms-dynamics-crm'); ?>
  ...
  <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php esc_html_e('2. Select Dynamics Account.', 'gravity-forms-dynamics-crm'); ?></div>
<div class="crm_btn_div" title="<?php esc_html_e('Expand / Collapse','gravity-forms-dynamics-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_account" class="left_header"><?php esc_html_e("Dynamics Account", 'gravity-forms-dynamics-crm'); ?>
  ...
  <div class="clear"></div>
    ...
    <div id="crm_ajax_div_object" style="display:none; text-align: center; line-height: 100px;"><i> <?php esc_html_e('Loading, Please Wait...','gravity-forms-dynamics-crm'); ?></i></div>
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php esc_html_e('This is a required field','gravity-forms-dynamics-crm') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  $.post(ajaxurl,{action:'get_objects_<?php echo vxg_dynamics::$id ?>',vx_crm_ajax:vx_crm_ajax,object:object,account:account},function(res){
  ...
  e.preventDefault(); 
    if(!confirm('<?php esc_html_e('Are you sure to remove ?','gravity-forms-dynamics-crm') ?>')){
     return;   
    }
  ...
  $.post(ajaxurl,{action:'refresh_data_<?php echo vxg_dynamics::$id ?>',vx_crm_ajax:vx_crm_ajax,vx_action:action,account:account},function(res){
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php esc_html_e('Please Select Field Name','gravity-forms-dynamics-crm') ?>');
     return;   
    }  
...
var text= div.find('.vxc_filter_text');
 ...
 div.find('.cfx_op_sel option').removeAttr('disabled'); 
  ...
  function add_note_sel(){
    jQuery('#crm_note_fields').select2({ placeholder: '<?php esc_html_e('Select Field','gravity-forms-dynamics-crm') ?>'});
} 
...
jQuery('#vx_add_fields_select').trigger('change');
  ...
  button_state_vx('ajax',button);
  vx_ajax=$.post(ajaxurl,{action:'get_field_map_<?php echo vxg_dynamics::$id ?>',form_id:vx_form_id,account:account,module:module,id:id,vx_crm_ajax:vx_crm_ajax,refresh:refresh},function(res){
  fields.html(res);   
...
var panel=$(this);
 verify_options(panel);   
  ...
  vx_ajax=$.post(ajaxurl,{action:'get_field_map_object_<?php echo vxg_dynamics::$id ?>',form_id:vx_form_id,account:account,id:id,vx_crm_ajax:vx_crm_ajax},function(res){
  fields.html(res);   
  //   primary.html(re.fields);
  fields.slideDown();   
start_tooltip();
...
vx_select();
  ...
  vx_select();
  function vx_select(){
      $('#vx_add_fields_select').select2({ placeholder: '<?php esc_html_e('Select Field','gravity-forms-dynamics-crm') ?>'});
  ...
  ajax.show();
  fields_div.hide();
  $.post(ajaxurl,{action:'get_objects_list_<?php echo vxg_dynamics::$id ?>',id:id},function(res){
  fields.html(res);    
  ajax.hide();
  })    
  });
