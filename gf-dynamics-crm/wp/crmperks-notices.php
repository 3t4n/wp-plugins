...
if( !class_exists( 'vxgf_dynamics_notice' )):
class vxgf_dynamics_notice{
public $plugin_url="https://www.crmperks.com";
public $review_link='https://wordpress.org/support/plugin/gf-dynamics-crm/reviews/?filter=5#new-post';
public $option='vxg-dynamics';
public $slug='gf-dynamics-crm/gf-dynamics-crm.php';

public function __construct(){

add_action( 'add_section_vxg_dynamics', array($this,'tab'),10);
add_action( 'add_section_mapping_vxg_dynamics', array($this,'upgrade_notice'),10);
...
if(isset($_GET['page']) && $_GET['page'] == 'gf_edit_forms' && isset($_GET['subview']) && $_GET['subview'] == 'vxg_dynamics' ){
add_filter( 'admin_footer_text', array( $this, 'admin_footer' ), 1, 2 );
add_filter( 'menu_links_vxg_dynamics', array( $this, 'menu_link' ), 60 );
add_filter( 'tab_contents_vxg_dynamics', array( $this, 'tab_pro' ), 60 );
}
...
public function menu_link($links){
    ...
    $logs_link=admin_url( "admin.php?page=gf_edit_forms&view=settings&subview=vxg_dynamics&tab=go_pro&id={sanitize_text_field('id')}" );
    $links['go_pro']=array( 
  "title"=>__('Go Premium','gravity-forms-dynamics-crm'),
  "link"=>$logs_link,
...
<p><i class="fa fa-check" style="color: #727f30; font-size: 18px; vertical-align: middle;"></i> Custom fields particularly Phone Number fields fields.</p>
<p><i class="fa fa-check" style="color: #727f30; font-size: 18px; vertical-align: middle;"></i> Dynamics CRM custom objects.</p>
<p><i class="fa fa-check" style="color: #727f30; font-size: 18px; vertical-align: middle;"></i> Assign object(Account,Contact etc) created by one feed to other feed object(Lead, Case) in Dynamics CRM.</p>
<p><i class="fa fa-check" style="color: #727f30; font-size: 18px; vertical-align: middle;"></i> Add a lead to Campaign in Dynamics CRM.</p>
...
$text=sprintf(__( 'if you enjoy using %sGravity Forms dynamics%s, please %s leave us a %s rating%s. A %shuge%s thank you in advance.','crm-perks-forms'),'<b>','</b>','<a href="'.$this->review_link.'" target="_blank" rel="noopener noreferrer">','&#9733;&#9733;&#9733;&#9733;&#9733;','</a>','<b>','</b>');
 return $text;
}
public function review_notice() { 
 ...
 if(!empty($install_time) && is_array($install_time) && !empty($install_time['time']) && empty($install_time['review_closed'])){
  ...
  <p><?php echo sprintf(__( 'You\'ve been using %sGravity Forms dynamics%s for some time now; we hope you love it!.%s If you do, please %s leave us a %s rating on WordPress.org%s to help us spread the word and boost our motivation.','contact-form-entries'),'<b>','</b>','<br/>','<a href="'.$this->review_link.'" target="_blank" class="vxcf_close_notice_a" rel="noopener noreferrer">','&#9733;&#9733;&#9733;&#9733;&#9733;','</a>'); ?></p>
