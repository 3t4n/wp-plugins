<?php
/**
* Plugin Name: WP Gravity Forms Dynamics CRM
* Description: Integrates Gravity Forms with Dynamics allowing form submissions to be automatically sent to your Dynamics account 
* Version: 1.1.4
* Requires at least: 3.8
* Author URI: https://www.crmperks.com
* Plugin URI: https://www.crmperks.com/plugins/gravity-forms-plugins/gravity-forms-dynamics-plugin/
* Author: CRM Perks.
* Text Domain: gf-dynamics-crm
...
if( !class_exists( 'vxg_dynamics' ) ):

class vxg_dynamics {
    
  public  $crm_name = 'dynamics';
  public static $id = 'vxg_dynamics';
  public  $domain = 'vxg-dynamics';
  public  $version = "1.1.4";
  public  $min_gravityforms_version = '1.3.9';
  public $type = 'vxg_dynamics_pro';
  public  $fields = null;
  ...
  public static $title='Gravity Forms Dynamics Plugin';  
  public static $path = ''; 
...
//add_action("gform_post_add_subscription_payment", array($this, 'gf_entry_paid_subscription'), 10, 2); //$entry,$pay_info
  ...
  load_plugin_textdomain('gravity-forms-dynamics-crm', FALSE,  $this->plugin_dir_name(). '/languages/' );
  ...
  $install=new vxg_install_dynamics();
...
self::$plugin=new vxcf_plugin_api(self::$id,$this->version,$this->type,$this->domain,$update_id,self::$title,$slug,self::$path,$settings_link,$is_plugin_page);
if($start_instance){
self::$plugin->instance();
}
} }
  /**
  * Install Gravity Forms Notice
  * 
  */
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo esc_html($id) ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Gravity Forms Dynamics Plugin','gravity-forms-dynamics-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','gravity-forms-dynamics-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice is-dismissible" data-id="<?php echo esc_html($id) ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Gravity Forms Dynamics Plugin','gravity-forms-dynamics-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sGravity Forms%s is required. %sPurchase it today!%s", 'gravity-forms-dynamics-crm'), "<a href='http://www.gravityforms.com/'>", "</a>", "<a href='http://www.gravityforms.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Gravity Forms is installed but not active. %sActivate Gravity Forms%s to use the Gravity Forms Dynamics Plugin','gravity-forms-dynamics-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=gravityforms/gravityforms.php'), 'activate-plugin_gravityforms/gravityforms.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sGravity Forms%s is required. %sPurchase it today!%s", 'gravity-forms-dynamics-crm'), "<a href='http://www.gravityforms.com/'>", "</a>", "<a href='http://www.gravityforms.com/'>", "</a>");
  }  
  ...
  */
  ...
  if(current_user_can('administrator') && !empty($_REQUEST['VXGDynamicsError'])) { /// 
  $confirmation .= sprintf(__('%sThe entry was not added to Dynamics because: %s. %sYou are only being shown this because you are an administrator. Other users will not see this message.%s', 'gravity-forms-dynamics-crm'), '<div class="error" style="text-align:center; color:#790000; font-size:14px; line-height:1.5em; margin-bottom:16px;background-color:#FFDFDF; margin-bottom:6px!important; padding:6px 6px 4px 6px!important; border:1px dotted #C89797">','<strong>'.esc_html($_REQUEST['VXGDynamicsError']). '</strong>', '<br /><em>', '</em></div>');
  }
  return $confirmation;
  }
  ...
  */
  ...
  if(empty($page)) {
  $page = vxg_dynamics::post("page");
  }
  if(isset($_GET['subview'])){
   $page = vxg_dynamics::post("subview");   
  }
  return $page == vxg_dynamics::$id;
  } 
  /**
  ...
  */
  ...
  if( $current_hook == 'admin_init' && empty( $_POST[vxg_dynamics::$id.'_send'] ) ) { return; } 
  if( $current_hook == 'gform_after_update_entry' && empty( $_POST[vxg_dynamics::$id.'_update'] ) ) { return; }
  ...
  if(!current_user_can(vxg_dynamics::$id."_send_to_crm")){ 
         return;  
       }
  // Export the entry
  ...
  // Don't send twice.
  unset($_POST[vxg_dynamics::$id.'_update']);
  unset($_POST[vxg_dynamics::$id.'_send']);
  }
  /**
  * get entry id 
  * 
...
public function get_entry_id($form_id){
  ...
  */
  ...
  $url = admin_url('admin.php?page=gf_settings&subview='.vxg_dynamics::$id);
...
if(empty($id)){
 ...
 $time = current_time( 'mysql' ,1);
  ...
  if( isset($data['data']) && is_array($data['data'])){
      $_data=vxg_dynamics::get_data_object();
...
if( (isset($field->storageType) && $field->storageType == 'json') || $field->type == 'fileupload' ){
  ...
  */
  ...
  $filters=vxg_dynamics::post('filters',$feed);
...
//  var_dump($final); die();
  ...
  */
  ...
  if(empty($_POST["screen_mode"]) || $_POST["screen_mode"] === 'view') { return; }
   ...
   if(!current_user_can(vxg_dynamics::$id."_send_to_crm")){return; }
  
  if( apply_filters( vxg_dynamics::$id.'_show_manual_export_button', true ) ) {
  printf('<input type="checkbox" name="'.vxg_dynamics::$id.'_update" id="'.vxg_dynamics::$id.'_update" value="1" /><label for="'.vxg_dynamics::$id.'_update" title="%s">%s</label><br /><br />', esc_html__('Create or update this entry in Dynamics. The fields will be mapped according to the form feed settings.', 'gravity-forms-dynamics-crm'), esc_html__('Send to Dynamics', 'gravity-forms-dynamics-crm'));
  } else {
  echo '<input type="hidden" name="'.vxg_dynamics::$id.'_update" id="'.vxg_dynamics::$id.'_update" value="1" />';
  }
  }
  /**
...
if(!current_user_can(vxg_dynamics::$id."_send_to_crm")){return; }
  ...
  $mode = empty($_POST["screen_mode"]) ? "view" : vxg_dynamics::post('screen_mode');
  if($mode === 'view') {
            $margin="";
      if(defined("vx_btn")){
      $margin="margin-top: 5px;";    
      }else{define('vx_btn','true');}
  $button.= '<input type="submit" class="button button-large button-secondary alignright" name="'.vxg_dynamics::$id.'_send" style="margin-left:5px; '.$margin.'" title="'.__('Create or update this entry in Dynamics. The fields will be mapped according to the form feed settings.','gravity-forms-dynamics-crm').'" value="'.__('Send to Dynamics', 'gravity-forms-dynamics-crm').'" onclick="jQuery(\'#action\').val(\'send_to_crm\')" />';
  //logs button

      $entry_id=vxg_dynamics::post('lid');
      $form_id = rgget('id');
      if(empty($entry_id)){
          $entry_id=$this->get_entry_id($form_id);
      }
      $log_url=admin_url( 'admin.php?page=gf_edit_forms&view=settings&subview='.vxg_dynamics::$id.'&tab=log&id='.$_GET['id'].'&entry_id='.$entry_id);  
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Dynamics Logs','gravity-forms-dynamics-crm').'" href="'.$log_url.'">'.__('Dynamics Logs','gravity-forms-dynamics-crm').'</a>';
  ...
  */
  ...
  RGFormsModel::add_note($id, 0, esc_html__('Gravity Forms Dynamics Plugin','gravity-forms-dynamics-crm'), $note);
  }
...
do_action('plugin_status_'.$this->type,'activate');  
  ...
  */
  ...
  $crm_tooltips = array(
    'vx_feed_name' => '<h6>' . esc_html__('Feed Name', 'gravity-forms-dynamics-crm') . '</h6>' . esc_html__('Enter feed name of your choice.', 'gravity-forms-dynamics-crm'),
  'vx_sel_object' => '<h6>' .__('Dynamics Object', 'gravity-forms-dynamics-crm') . '</h6>' . esc_html__('Select the Object to Create when a Form is Submitted.', 'gravity-forms-dynamics-crm'),
   'vx_sel_account' => '<h6>' .__('Dynamics Account', 'gravity-forms-dynamics-crm') . '</h6>' . esc_html__('Select the Dynamics account you would like to export entries to.', 'gravity-forms-dynamics-crm'),
  'vx_sel_form' => '<h6>' . esc_html__('Gravity Form', 'gravity-forms-dynamics-crm') . '</h6>' . esc_html__('Select the Gravity Form you would like to integrate with Dynamics. Contacts generated by this form will be automatically added to your Dynamics account.', 'gravity-forms-dynamics-crm'),
  
  'vx_map_fields' => '<h6>' . esc_html__('Map Standard Fields', 'gravity-forms-dynamics-crm') . '</h6>' . esc_html__('Associate your Dynamics fields to the appropriate Gravity Form fields.', 'gravity-forms-dynamics-crm'),
  
  'vx_optin_condition' => '<h6>' . esc_html__('Opt-In Condition', 'gravity-forms-dynamics-crm') . '</h6>' . esc_html__('When the opt-in condition is enabled, form submissions will only be exported to Dynamics when the condition is met. When disabled all form submissions will be exported.', 'gravity-forms-dynamics-crm'),
  
  'vx_manual_export' => '<h6>' . esc_html__('Manual Export', 'gravity-forms-dynamics-crm') . '</h6>' . esc_html__('If you do not want all entries sent to Dynamics, but only specific, approved entries, check this box. To manually send an entry to Dynamics, go to Entries, choose the entry you would like to send to Dynamics, and then click the "Send to Dynamics" button.', 'gravity-forms-dynamics-crm'),
  
    'vx_entry_notes' => '<h6>' . esc_html__('Entry Notes', 'gravity-forms-dynamics-crm') . '</h6>' . esc_html__('Enable this option if you want to synchronize Gravity Forms entry notes to Dynamics Object notes. For example , when you add a note to a Gravity Forms entry, it will be added to the Dynamics Object selected in the feed.', 'gravity-forms-dynamics-crm'),
    
      'vx_primary_key' => '<h6>' . esc_html__('Primary Key', 'gravity-forms-dynamics-crm') . '</h6>' . esc_html__('Which field should be used to update existing objects?', 'gravity-forms-dynamics-crm'),
      
    
      'vx_web' => '<h6>' . esc_html__('Web to Lead', 'gravity-forms-dynamics-crm') . '</h6>' . sprintf(__('Web-to-Lead is available for all Dynamics Editions. If you are not sure if your Dynamics Edition supports the API, you should use Web-to-Lead. Editions that do not support the Dynamics API: %s 1: Personal Edition %s 2: Group Edition %s 3: Professional Edition %s Note: You can purchase API access for a Professional Edition', 'gravity-forms-dynamics-crm'),'<br/>','<br/>','<br/>','<br/>'),
      
  'vx_api' => '<h6>' . esc_html__('Dynamics API', 'gravity-forms-dynamics-crm') . '</h6>' .sprintf(__('The API features are more powerful than Web-to-Lead. You can create different object types, as well as other advanced features. If you have any of the following Dynamics Editions, you can use the included API Add-on: %s 1: Enterprise Edition %s 2: Unlimited Edition %s 3: Developer Edition %s 4: Professional Edition - Requires API Upgrade', 'gravity-forms-dynamics-crm'),'<br/>','<br/>','<br/>','<br/>','<br/>'),
  

  'vx_custom_app'=>'<h6>' . esc_html__('Custom App', 'gravity-forms-dynamics-crm') . '</h6>' .__('This option is for advanced users who want to override default Dynamics App.','gravity-forms-dynamics-crm'),
  
  
  'vx_camps'=>'<h6>' . esc_html__('Dynamics Campaigns', 'gravity-forms-dynamics-crm') . '</h6>' .__('Get Campaigns and Status list from dynamics.','gravity-forms-dynamics-crm'),
  
  'vx_sel_price_book'=>__('Which Pricebook should be searched for product','gravity-forms-dynamics-crm'),
  
  'vx_sel_camp'=>'<h6>' . esc_html__('Select Campaign', 'gravity-forms-dynamics-crm') . '</h6>' .__('Which Campaign should be assigned to this object.','gravity-forms-dynamics-crm'),
  
  'vx_sel_status'=>'<h6>' . esc_html__('Campaign Status', 'gravity-forms-dynamics-crm') . '</h6>' .__('What should be Member Status.','gravity-forms-dynamics-crm'),
  
  'vx_pro_desc'=>'<h6>' . esc_html__('Price Book', 'gravity-forms-dynamics-crm') . '</h6>' .__('A new product will be created in selected Pricebook. You can add a description for new products created by this plugin.','gravity-forms-dynamics-crm'),
  
   'vx_assign_account'=>'<h6>' . esc_html__('Dynamics Account', 'gravity-forms-dynamics-crm') . '</h6>' .__('Enable this option if you want to assign an account this object.','gravity-forms-dynamics-crm'),
   
   'vx_sel_account'=>'<h6>' . esc_html__('Select Account', 'gravity-forms-dynamics-crm') . '</h6>' .__('Object created by this feed will be assigned to the selected Account.','gravity-forms-dynamics-crm'),
   
      'vx_assign_contract'=>'<h6>' . esc_html__('Assign Contract', 'gravity-forms-dynamics-crm') . '</h6>' .__('Enable this option , if you want to assign a Contract to this object','gravity-forms-dynamics-crm'),
      
   'vx_sel_contract'=>'<h6>' . esc_html__('Select Contact', 'gravity-forms-dynamics-crm') . '</h6>' .__('Select Contract feed. Contract created by this feed will be assigned to this object','gravity-forms-dynamics-crm'),
   
   'vx_camp_check'=>'<h6>' . esc_html__('Add to Campaign', 'gravity-forms-dynamics-crm') . '</h6>' .__('If enabled, Lead/Contact will be added to selected Campaign','gravity-forms-dynamics-crm'),
   
   'vx_camps'=>'<h6>' . esc_html__('Get Campaigns', 'gravity-forms-dynamics-crm') . '</h6>'.__('Get Campaigns from dynamics.','gravity-forms-dynamics-crm'),
'vx_camp'=>'<h6>' . esc_html__('Select Campaign', 'gravity-forms-dynamics-crm') . '</h6>'.__('Which Campaign should be assigned to this object.','gravity-forms-dynamics-crm'),

'vx_camp_check'=>'<h6>' . esc_html__('Add to Campaign', 'gravity-forms-dynamics-crm') . '</h6>'.__('If enabled, Lead will be added to this campaign','gravity-forms-dynamics-crm'),

'vx_lists'=>'<h6>' . esc_html__('Get Price Lists', 'gravity-forms-dynamics-crm') . '</h6>'.__('Get Price Lists from dynamics.','gravity-forms-dynamics-crm'),

'vx_list'=>'<h6>' . esc_html__('Select Price List', 'gravity-forms-dynamics-crm') . '</h6>'.__('Which Price List should be assigned to this object.','gravity-forms-dynamics-crm'),
'vx_list_check'=>'<h6>' . esc_html__('Assign Price List', 'gravity-forms-dynamics-crm') . '</h6>'.__('If enabled, Selected Price List will be assigned to the Order','gravity-forms-dynamics-crm'),


'vx_assign_contact'=>'<h6>' . esc_html__('Assign Contact', 'gravity-forms-dynamics-crm') . '</h6>'.__('Enable this option , if you want to assign a Contact to this object','gravity-forms-dynamics-crm'),
'vx_sel_contact'=>'<h6>' . esc_html__('Select Contact Feed', 'gravity-forms-dynamics-crm') . '</h6>'.__('Select Contact feed. Contact created by this feed will be assigned to this object','gravity-forms-dynamics-crm'),

'vx_assign_account'=>'<h6>' . esc_html__('Assign Account', 'gravity-forms-dynamics-crm') . '</h6>'.__('Enable this option , if you want to assign a Account to this object','gravity-forms-dynamics-crm'),

'vx_sel_account'=>'<h6>' . esc_html__('Select Account', 'gravity-forms-dynamics-crm') . '</h6>'.__('Select Account feed. Account created by this feed will be assigned to this object','gravity-forms-dynamics-crm'),

   'vx_owner_check'=>'<h6>' . esc_html__('Assign Owner', 'gravity-forms-dynamics-crm') . '</h6>' .__('Enable this option if you want to assign another object owner.','gravity-forms-dynamics-crm'),
   
   'vx_owners'=>'<h6>' . esc_html__('Dynamics Users', 'gravity-forms-dynamics-crm') . '</h6>' .__('Get Users list from Dynamics','gravity-forms-dynamics-crm'),
   
   'vx_order_notes'=>'<h6>' . esc_html__('Entry Notes', 'gravity-forms-dynamics-crm') . '</h6>' .__('Enable this option if you want to synchronize Entry notes to Dynamics Object notes. For example, when you add a note to a Entry, it will be added to the Dynamics Object selected in the feed.','gravity-forms-dynamics-crm'),
  
   'vx_sel_owner'=>'<h6>' . esc_html__('Select Owner', 'gravity-forms-dynamics-crm') . '</h6>' .__('Select a user as a owner of this object','gravity-forms-dynamics-crm'),
   
      'vx_entry_note'=>'<h6>' . esc_html__('Entry Note', 'gravity-forms-dynamics-crm') . '</h6>' .__('Check this option if you want to send more data as CRM entry note.', 'gravity-forms-dynamics-crm'),
   'vx_note_fields'=>'<h6>' . esc_html__('Note Fields', 'gravity-forms-dynamics-crm') . '</h6>' .__('Select fields which you want to send as a note', 'gravity-forms-dynamics-crm'),
   'vx_disable_note'=>'<h6>' . esc_html__('Note Fields', 'gravity-forms-dynamics-crm') . '</h6>' .__('Enable this option if you want to add note only for new CRM entry', 'gravity-forms-dynamics-crm')
...
//array('account'=>'Account','contact'=>'Contact','lead'=>'Lead','incident'=>'Case','salesorder'=>'Order');  
   $objects=array();   
   if(!empty($info)){   
   $meta=self::post('meta',$info);  
   }else{
   $meta=get_option(self::$id.'_meta',array());    
   }
 ...
 if($refresh && !empty($info)){
  ...
  if(is_array($objects)){
  $option=get_option(self::$id.'_meta',array());
  ...
  update_option(self::$id.'_meta',$option); //save objects for logs search option
...
return  $var;
  ...
  */
  ...
  $status=0;
  if(class_exists('vxg_dynamics_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/gravity-forms-dynamics-crm/gravity-forms-dynamics-crm.php')) {
  $status=2;
  } 
  return $status;
  }
...
$enc_str.=":".base64_encode($iv);
  ...
  */
  ...
  return isset($_REQUEST[$key][$key2]) && is_array($_REQUEST[$key]) ? self::clean($_REQUEST[$key][$key2]) : "";
  }
  /**
  * Get variable from array
  *  
...
public function do_actions(){
  ...
  */
  ...
  $this->api=$api= new vxcf_dynamics_api($crm);
  ...
  */
  ...
  if(!is_object(vxg_dynamics::$data))
  vxg_dynamics::$data=new vxg_dynamics_data();
  return vxg_dynamics::$data;
  }
  ...
  */
  public function gf_entry_created( $entry, $form, $event='submit'){
      ...
      if(is_array($entry) && isset($entry['status']) && $entry['status'] == 'active' && empty($entry['partial_entry_percent'])){
        ...
        $entry_id=vxg_dynamics::post('id',$entry);
        if($this->do_actions()){
  ...
  */
  public  function push($entry, $form,$event="",$is_admin=false,$log=""){ 
     ...
     $data_db=vxg_dynamics::get_data_object(); 
  ...
  $feed['meta']=$meta;
  $feed['data']=$data;
$object=vxg_dynamics::post('object',$feed); 
 if(!empty($data['contact_check']) || !empty($data['account_check'])){
...
$entry=apply_filters('vx_crm_post_fields',$entry,$entry_id,'gf',$form); 
  ...
  $data=$feed['data']; 
  $meta=$feed['meta'];  
  $account=vxg_dynamics::post('account',$feed);

$info=$this->get_info($account); 
if(!empty($info['meta']['objects'])){
 $feed['objects']=$info['meta']['objects'];   
}
$info_data=array();
if(isset($info['data'])){
    $info_data=$info['data'];
}
$api_type=vxg_dynamics::post('api',$info_data);
...
if(in_array($event,array('add_note','delete_note')) && !empty($log)){
   ...
   if($event == 'delete_note' && !empty(self::$note)){
    ...
    if(in_array($event,array('delete_note','add_note'))){
        //check feed
    $order_notes=vxg_dynamics::post('entry_notes',$data); //if notes sync not enabled in feed return
...
//
  ...
  $feed_event=vxg_dynamics::post('manual_export',$data);
 if(!$is_admin){
...
$skip_feed=false;
  ...
  if(vxg_dynamics::post('type',$v) == "value"){ //custom value
      $value=trim(vxg_dynamics::post('value',$v)); 
 $value=$this->process_tags($entry,$form,$value); 
}else{ //general field
  $field=vxg_dynamics::post('field',$v);
  if($field !=""){
  ...
  if(isset($data['owner']) && $data['owner'] == "1"){
   ...
   $feed['user']=apply_filters('vx_assigned_user_id',$data_user,self::$id,$feed['id'],$entry,$form);   
  }    
  //add note 
...
$no_filter=true;    
  ...
  if(!$is_admin && $event == 'submit' && vxg_dynamics::post('manual_export',$data) == "1"){ //if manual export is yes
 continue;   
  }    
  ...
  if(!$force_send && vxg_dynamics::post('optin_enabled',$data) == "1"){ //apply filters if not sending by force and optin is enabled
  $no_filter=$this->check_filter($data,$entry,$form); 
  $res=array("status"=>"4","extra"=>array("filter"=>$this->filter_condition),"data"=>$temp);  
  }

$temp=apply_filters('crm_perks_dynamics_crm_post_data', $temp ,$entry,$feed['id']);
  ...
  $res=apply_filters(self::$id.'_feed_response',$res,$feed,$entry);
  }
//var_dump($res); die();

  $feed_id=vxg_dynamics::post('id',$feed);
  self::$feeds_res[$feed_id]=$res;
  $status=$res['status'];  $error=""; 
  $id=vxg_dynamics::post('id',$res);
  $added=false;
  ...
  if( $added){ 
      $id=$res['id'];
      $action=vxg_dynamics::post('action',$res);
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Dynamics (%s) with ID # %s .', 'gravity-forms-dynamics-crm'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Dynamics (%s) with ID # %s . View entry at %s', 'gravity-forms-dynamics-crm'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Dynamics', 'gravity-forms-dynamics-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Dynamics (%s) with ID # %s . View entry at %s', 'gravity-forms-dynamics-crm'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Dynamics (%s) with ID # %s .', 'gravity-forms-dynamics-crm'),$feed['object'],$res['id']);   
            }
     $screen_msg=__( 'Entry updated in Dynamics', 'gravity-forms-dynamics-crm');
      }
   
  
  }else if(vxg_dynamics::post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'gravity-forms-dynamics-crm');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Dynamics. Entry not sent! Check the Entry Notes below for more details.' , 'gravity-forms-dynamics-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Dynamics. Entry not sent' , 'gravity-forms-dynamics-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'gravity-forms-dynamics-crm'),$feed['object']);
  if(vxg_dynamics::post('error',$res)!=""){
...
$this->send_error_email($info_data,$entry,$form);
  ...
  //insert log
  $arr=array("object"=>$feed["object"],"form_id"=>$form_id,"status"=>$status,"entry_id"=>$entry_id,"crm_id"=>$id,"meta"=>$error,"time"=>date('Y-m-d H:i:s'),"data"=>$res['data'],"response"=>vxg_dynamics::post('response',$res),"extra"=>vxg_dynamics::post('extra',$res),"feed_id"=>vxg_dynamics::post('id',$feed),"link"=>vxg_dynamics::post('link',$res),'parent_id'=>$parent_id,'event'=>$event);
   
 // $settings=get_option($this->type.'_settings',array());
 // if(vxg_dynamics::post('disable_log',$settings) !="yes"){ 
   $insert_id=$data_db->insert_log($arr,$log_id); 
//  } 
    if(!empty($insert_id)){ // 
          $log_url=admin_url( 'admin.php?page=gf_edit_forms&view=settings&subview='.vxg_dynamics::$id.'&tab=log&log_id='.$insert_id.'&id='.$form_id);   
  $log_link=' <a href="'.$log_url.'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','gravity-forms-dynamics-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//  $this->add_note($entry["id"], $msg);
  ...
  */
  public function send_error_email($info,$entry,$form){
        if( trim(vxg_dynamics::post('error_email',$info))!=""){
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Dynamics','gravity-forms-dynamics-crm')." Error","info_title"=>"More Detail","info"=>$detail);
...
$vxg_dynamics=new vxg_dynamics();
$vxg_dynamics->instance();
$vx_gf['vxg_dynamics']='vxg_dynamics';
