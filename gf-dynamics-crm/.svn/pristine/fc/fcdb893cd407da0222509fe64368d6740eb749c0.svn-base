...
<div class="postbox">
<h3 style="border-bottom: 1px solid #ddd;"><?php esc_html_e("Dynamics", 'gravity-forms-dynamics-crm') ?></h3>
<div class="inside">
...
echo '<p style="background: '.esc_attr($bg).'; padding: 10px; color: #fff " class="vx_msg_div"><i class="fa '.esc_attr($icon).'"></i> '.wp_kses_post($msg).'</p>';
  ...
  ?>
  <p>
  <button class="button" type="submit" name="<?php echo vxg_dynamics::$id ?>_send" value="yes" title="<?php esc_html_e("Send to Dynamics",'gravity-forms-dynamics-crm')?>"><?php esc_html_e("Send to Dynamics",'gravity-forms-dynamics-crm')?></button>
  <?php
      if($comments ){
  ?>
    <a class="button button-secondary" title="<?php esc_html_e('Go to Logs','gravity-forms-dynamics-crm') ?>" href="<?php echo esc_url($log_url) ?>"><?php esc_html_e('Go to Logs','gravity-forms-dynamics-crm') ?></a>
  <?php
      }
  ?>
  </p>
