...
$client=$this->client_info();
  ...
  if(!is_array($token)){ $token=array(); }
   ...
   $token['error_description']=vxg_dynamics::post('error_description');   
  }
  $info['msg']=$info['class']='';
...
if(!empty($this->id)){
 vxg_dynamics::update_info(array('data'=>$info),$this->id);
}
//var_dump($token); die();
return $info; 
}
...
//echo json_encode($v['RequiredLevel']['Value']);
  //  echo '<hr>';
    if(isset($v['IsValidForUpdate']) && $v['IsValidForUpdate'] == true && !empty($v['DisplayName']['UserLocalizedLabel']['Label'])){   // &&
       if(in_array($v['LogicalName'],$skip)){ continue; } //if( $v['LogicalName'] == 'ehtc_hearaboutus'){ var_dump($v); }
...
//star moved from here because parentcustomerid accepts parentcustomerid_account , it does not accept schema name
 ...
 if($v['AttributeType'] == 'Lookup'){ 
  $a['related_to']=$v['Targets'][0]; //some lookup fields have 100+ targets and need record1id_lead@data.bind=/leads(xxxx) format , we do not support those
  //star moved to  
...
//var_dump($res);  die('--------');
 ...
 //   $res=$this->post_crm('leads?$top=1');
   // $res=$this->post_crm('ehtc_webrequests?$top=100');
  //  var_dump($res); die();
//$res=$this->post_crm('salesorders(1c51cda1-999a-eb11-b1ac-0022487f195a)');
//$res=$this->post_crm('pagus_businesssegments(c69de21a-2cbe-eb11-8236-0022480a33ca)');
//var_dump($res); die();
//check primary key
...
if(!empty($meta['contact_check']) && !empty($meta['object_contact'])){ 
  ...
  $fields['ContactId']=array('value'=>$contact_res,'label'=>'Contact ID');     
   $post['customerid_contact@odata.bind']='/contacts('.$contact_res.')';  
    ...
    //'parentcustomerid@odata.bind' => string '/accounts(e4bd9ec2-5a33-ee11-bdf4-6045bdd0e669)' 
//var_dump($post); die(); 
//unset($post['new_SelectedMarket@odata.bind']);
//$post['ParentCustomerId@odata.bind']='/accounts(e4bd9ec2-5a33-ee11-bdf4-6045bdd0e669)';    
//$post['parentcustomerid_account@odata.bind']='/accounts(e4bd9ec2-5a33-ee11-bdf4-6045bdd0e669)';    //works for parent
...
// 
unset($post['ehtc_BusinessSegment@odata.bind']);
//unset($post['record1id@odata.bind']);
//unset($post['record2id@odata.bind']);
//  $post['record1id_lead@odata.bind'] = '/leads(30970bee-0b94-ee11-be37-6045bde6f991)';
//  $post['record2id_contact@odata.bind'] = '/contacts(69aa14b0-1093-ee11-be37-6045bde6f991)';
 // $post['ehtc_BusinessSegment_pagus_businesssegment@odata.bind'] = '/pagus_businesssegments(c69de21a-2cbe-eb11-8236-0022480a33ca)';
...
self::$feeds_res[$meta['id']]=$id;
 ...
 echo  $contents=trim(ob_get_clean());
  if($contents!=""){
  update_option(vxg_dynamics::$id."_debug",$contents);   
  }
  }
       //add entry note
 if(!empty($meta['__vx_entry_note']) && !empty($id)){
 $disable_note=vxg_dynamics::post('disable_entry_note',$meta);
