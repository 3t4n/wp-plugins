...
if( !class_exists( 'vxg_dynamics_pages' ) ) {

class vxg_dynamics_pages   extends vxg_dynamics{
    public $ajax=false;

  public function __construct() {
  
  vxg_dynamics::$data=vxg_dynamics::get_data_object();
global $pagenow;
  if(in_array($pagenow, array("admin-ajax.php"))){
  add_action('wp_ajax_update_feed_'.vxg_dynamics::$id, array($this, 'update_feed'));
  add_action('wp_ajax_update_feed_sort_'.vxg_dynamics::$id, array($this, 'update_feed_sort'));
  add_action('wp_ajax_get_field_map_'.vxg_dynamics::$id, array($this, 'get_field_map_ajax'));
  add_action('wp_ajax_get_field_map_object_'.vxg_dynamics::$id, array($this, 'get_field_map_object_ajax'));
  add_action('wp_ajax_get_objects_'.vxg_dynamics::$id, array($this, 'get_objects_ajax'));
  add_action('wp_ajax_log_detail_'.vxg_dynamics::$id, array($this, 'log_detail')); 
    add_action('wp_ajax_refresh_data_'.vxg_dynamics::$id, array($this, 'refresh_data'));
  }
  if($this->is_crm_page()){
 ...
 add_action( 'gform_form_settings_page_' . vxg_dynamics::$id, array( $this, 'form_settings_page' ) );
...
require_once(self::$path . 'templates/crm-entry-box.php'); 
  ...
  */
  ...
  $debug = !empty(self::$debug_html) && current_user_can(vxg_dynamics::$id.'_edit_settings');
  if($debug){ 
  echo "<div class='error'><p>".wp_kses_post(self::$debug_html)."</p></div>"; 
  self::$debug_html='';
  }
  if(isset($_GET[vxg_dynamics::$id."_logs"]) && current_user_can(vxg_dynamics::$id.'_read_settings')){
      $msg=__('Error While Clearing Dynamics Logs','gravity-forms-dynamics-crm');
      $level="error";
      if(!empty($_GET[vxg_dynamics::$id."_logs"])){
      $msg=__('Dynamics Logs Cleared Successfully','gravity-forms-dynamics-crm');   
      $level="updated";
      }
      $this->screen_msg($msg,$level);
  }
 // if(isset($_REQUEST[vxg_dynamics::$id.'_msg'])){ //send to crm in order page message
  $msg=get_option(vxg_dynamics::$id.'_msg');
  update_option(vxg_dynamics::$id.'_msg','');
  if(isset($msg['class'])){
  ...
  */
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . esc_html__('Settings', 'gravity-forms-dynamics-crm') . '</a>' );
        }
        return $links;
   }
  ...
  /**
    ...
    public function add_form_settings_menu( $tabs, $form_id ) {
        ...
        $tabs[] = array( 'name' => vxg_dynamics::$id, 'label' => esc_html__("Dynamics", 'gravity-forms-dynamics-crm') , 'query' => array( 'fid' => null),'icon' => 'dashicons-cloud dashicons' );
...
public function update_entry($form,$lead_id){
    ...
    $meta=get_option($this->type.'_settings',array());
      ...
      if(!empty($meta['update']) || isset($_POST[vxg_dynamics::$id.'_update'])){
...
$this->push($entry,$form,'add_note');
  ...
  */
  ...
  $menus[] = array("name" => vxg_dynamics::$id, "label" => esc_html__('Dynamics','gravity-forms-dynamics-crm'), "callback" =>  array($this, "mapping_page"), "permission" => vxg_dynamics::$id.'_read_feeds');
  ...
  */
  public  function setup(){
      ...
      RGForms::add_settings_page(array('name' => vxg_dynamics::$id,'tab_label' => esc_html__('Dynamics','gravity-forms-dynamics-crm'),'icon' => 'dashicons-cloud dashicons',"handler"=>array($this, "settings_page")));
 
           global $wpdb; 
  if(vxg_dynamics::post('vx_tab_action_'.vxg_dynamics::$id)=="export_log"){
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can(vxg_dynamics::$id."_export_logs")){ 
  $msg=__('You do not have permissions to export logs','gravity-forms-dynamics-crm');
  ...
  $sql_end=$data->get_log_query();
    $objects=vxg_dynamics::get_objects("");
  $forms=array();
  $sql="select * $sql_end limit 3000";
  $results = $wpdb->get_results($sql , ARRAY_A );
  $fields=array(); $field_titles=array("#",__('Status','gravity-forms-dynamics-crm'),__('Dynamics ID','gravity-forms-dynamics-crm') ,__('Entry ID','gravity-forms-dynamics-crm'),__('Description','gravity-forms-dynamics-crm'),__('Time','gravity-forms-dynamics-crm'));
  ...
  if(vxg_dynamics::post('vx_tab_action_'.vxg_dynamics::$id)=="clear_logs" ){
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can(vxg_dynamics::$id."_edit_settings")){ 
  $msg=__('You do not have permissions to clear logs','gravity-forms-dynamics-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=vxg_dynamics::get_data_object();
  ...
  wp_redirect(admin_url("admin.php?page=".vxg_dynamics::post('page')."&view=".vxg_dynamics::post('view')."&".vxg_dynamics::$id."_logs=".$clear));
  die();
  } 
  
  //send to crm
  if(isset($_POST[vxg_dynamics::$id.'_send'])){
  ...
  $push=$this->push($entry, $form,"",true); 
    ...
    if(!empty($push['msg'])){
        update_option(vxg_dynamics::$id.'_msg',array('msg'=>$push['msg'],'class'=>$push['class']));  
  }
  ...
  */
  ...
  if(!current_user_can(vxg_dynamics::$id.'_read_feeds')){
  esc_html_e('You do not have permissions to access this page','gravity-forms-dynamics-crm');    
  return;
  }
  $is_section=apply_filters('add_page_html_'.vxg_dynamics::$id,false);
  ...
  vxg_dynamics::$data->delete_feed($id);
  ?>
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feed deleted.", 'gravity-forms-dynamics-crm') ?>
  </p>
  </div>
  <?php
  }
  ...
  vxg_dynamics::$data->delete_feed($feed_id);
  }
  ?>
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'gravity-forms-dynamics-crm') ?>
  </p>
  </div>
  <?php
  }
  $form_id=vxg_dynamics::post('id');
  $feeds = vxg_dynamics::$data->get_feed_by_form($form_id); 
...
$page_link=$this->link_to_settings();
  ...
  //
  $config = vxg_dynamics::$data->get_feed('new_form');
...
include_once(self::$path . "templates/feeds.php");
  ...
  */
  ...
  if(!current_user_can(vxg_dynamics::$id.'_read_logs')){
  esc_html_e('You do not have permissions to access this page','gravity-forms-dynamics-crm');    
  return;
  }
  $is_section=apply_filters('add_page_html_'.vxg_dynamics::$id,false);
...
$offset=$this->time_offset(); 
  $log_ids=array();
   $bulk_action=vxg_dynamics::post('bulk_action');
  if($bulk_action!=""){
   $log_id=vxg_dynamics::post('log_id');  
...
$count=vxg_dynamics::$data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','gravity-forms-dynamics-crm'),$count));  
    }
    else if(in_array($bulk_action,array("send_to_crm_bulk","send_to_crm_bulk_force"))){
         self::$api_timeout='1000';
       foreach($log_ids  as $id){
  $log = vxg_dynamics::$data->get_log_by_id($id); 
  $form_id=vxg_dynamics::post('form_id',$log);
  $entry_id=vxg_dynamics::post('entry_id',$log);
  ...
  if(is_array($entry)){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','gravity-forms-dynamics-crm'));  
  }
  ...
  case"time": $time_order=$order_icon; $time_class="";   break;    
    ...
    $bulk_actions=array(""=>__('Bulk Action','gravity-forms-dynamics-crm'),"delete"=>__('Delete','gravity-forms-dynamics-crm'),
  'send_to_crm_bulk'=>__('Send to Dynamics','gravity-forms-dynamics-crm'),'send_to_crm_bulk_force'=>__('Force Send to Dynamics - Ignore filters','gravity-forms-dynamics-crm'));
  $base_url=$this->get_base_url();

$objects=vxg_dynamics::get_objects();

      $statuses=array("1"=>__("Created",'gravity-forms-dynamics-crm'),"2"=>__("Updated",'gravity-forms-dynamics-crm'),"error"=>__("Failed",'gravity-forms-dynamics-crm'),"4"=>__("Filtered",'gravity-forms-dynamics-crm'),"5"=>__("Deleted",'gravity-forms-dynamics-crm')); 
...
public function get_menu_links($current_page=""){
  ...
  $logs_link=admin_url( "admin.php?page=gf_edit_forms&view=settings&subview=".vxg_dynamics::$id."&tab=log&id={$id}" );
  $feeds_link=admin_url( "admin.php?page=gf_edit_forms&view=settings&subview=".vxg_dynamics::$id."&tab=feed&id={$id}" );
  
      $menu_links=array(
  'settings'=> array( 
  "title"=>__('Dynamics Settings','gravity-forms-dynamics-crm'),
  "link"=>$settings_link,
  "current"=>$current_page == 'settings' ? true : false
  ),
  'feed'=> array( 
  "title"=>__('Dynamics Feeds','gravity-forms-dynamics-crm'),
  "link"=>$feeds_link,
   "current"=>$current_page == 'feed' ? true : false
  ),
  'log'=> array( 
  "title"=>__('Dynamics Log','gravity-forms-dynamics-crm'),
  "link"=>$logs_link,
   "current"=>$current_page == 'log' ? true : false
  ));
$menu_links=apply_filters('menu_links_'.vxg_dynamics::$id,$menu_links); 
return $menu_links;
} 
/**
* feed link
...
public function get_feed_link($id="",$form_id=""){
    ...
    $str="admin.php?page=gf_edit_forms&view=settings&subview=".vxg_dynamics::$id."&id=$form_id" ;
    if(!empty($id)){
    $str.="&tab=feed&fid={$id}";    
    }else{
     $str.='&tab=feed';   
...
//
 ...
 // $obj=$api->get_crm_objects(); 
  ...
  $info_meta['feed_id']=vxg_dynamics::post('id');   
  vxg_dynamics::update_info( array('meta'=>$info_meta),$info['id']);        
  }   
  }else{
 $fields=vxg_dynamics::post('fields',$feed_meta); 
  }
  }
//var_dump($fields);
  if(!is_array($fields)){
...
//
  ...
  $sel_fields=array(""=>__("Standard Field",'gravity-forms-dynamics-crm'),"value"=>__("Custom Value",'gravity-forms-dynamics-crm'));
...
include_once(self::$path . "templates/fields-mapping.php"); 
  ...
  */
  ...
  if(!current_user_can(vxg_dynamics::$id."_edit_feeds")){ 
  return;   
  }
  $id = $this->post("feed_id");
  $feed = vxg_dynamics::$data->get_feed($id);
  vxg_dynamics::$data->update_feed(array("is_active"=>vxg_dynamics::post("is_active")),$id);
  } 
  ...
  */
  ...
  vxg_dynamics::$data->update_feed_order(vxg_dynamics::post('sort'));
  }
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = esc_html__('Dynamics','gravity-forms-dynamics-crm');
        return $plugins;
    }
  /**
  ...
  */
  ...
  $msg=__("Please Choose Object",'gravity-forms-dynamics-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'gravity-forms-dynamics-crm');
  }
  if($msg !=""){
  ?>
...
/*  $object=vxg_dynamics::post('object',$feed);
  ...
  $msg=__("Please Choose Account",'gravity-forms-dynamics-crm');
  }
  if($msg !=""){
  ?>
  <div class="error below-h2" style="background: #f3f3f3">
...
$this->field_map_object($account,$feed,$info);
  ...
  */
  public function refresh_data(){
      check_ajax_referer("vx_crm_ajax","vx_crm_ajax"); 
  if(!current_user_can(vxg_dynamics::$id."_read_settings")){ 
   die();  
 }   
  $res=array();
  $action=vxg_dynamics::post('vx_action');
  $camp_id_sel=vxg_dynamics::post('camp_id');

  $account=vxg_dynamics::post('account');
  $status_sel=vxg_dynamics::post('status');
  $owner_sel=vxg_dynamics::post('owner');
 ...
 if(!empty($info['meta']) ){
  ...
  switch($action){
    ...
    $data['crm_sel_camp']=$this->gen_select($camps,$status_sel,__('Select Campaign','gravity-forms-dynamics-crm'));
  $meta['camps']=$camps; 
  $res['data']=$data;   
      break;      
      case"refresh_lists":
    ...
    $data['crm_sel_list']=$this->gen_select($lists,$status_sel,__('Select Price List','gravity-forms-dynamics-crm'));
  $meta['lists']=$lists; 
  $res['data']=$data;   
      break;    
  case"refresh_users":
    ...
    $res['status']="ok";
     ...
     $res['error']=$users;   
        ...
        $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','gravity-forms-dynamics-crm'));
  $meta['users']=$users;  
  $res['data']=$data;   
      break;
  ...
  if(isset($info['id'])){
    vxg_dynamics::update_info( array("meta"=>$meta) , $info['id'] );
}
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','gravity-forms-dynamics-crm');
    }
}
  die(json_encode($res));    
  }
  ...
  */
  public function setup_plugin(){
    ...
    if(isset($_REQUEST[vxg_dynamics::$id.'_tab_action']) && $_REQUEST[vxg_dynamics::$id.'_tab_action']=="get_code" && current_user_can(vxg_dynamics::$id."_edit_settings")){
   $part=array('code'=>'');
if(isset($_REQUEST['code'])){
$part['code']=vxg_dynamics::post('code');   
}
if(isset($_REQUEST['error'])){
$part['error']=vxg_dynamics::post('error');   
$part['error_description']=vxg_dynamics::post('error_description');   
}
...
if(isset($_REQUEST[vxg_dynamics::$id.'_tab_action']) && $_REQUEST[vxg_dynamics::$id.'_tab_action']=="get_token"){
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can(vxg_dynamics::$id."_edit_settings")){ 
  $msg=__('You do not have permissions to add token','gravity-forms-dynamics-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $id=vxg_dynamics::post('id');
  $info=$this->get_info($id);
  $api=$this->get_api($info);
$info=$api->handle_code();
    //get objects after saving acces token
  $token=vxg_dynamics::post('access_token',$info);
  if(!empty($token)){
...
if(isset($_REQUEST[vxg_dynamics::$id.'_tab_action']) && $_REQUEST[vxg_dynamics::$id.'_tab_action']=="del_account"){ 
 check_admin_referer('vx_nonce','vx_nonce');
 if( current_user_can(vxg_dynamics::$id."_edit_settings")){ 
$id=vxg_dynamics::post('id');
$data=vxg_dynamics::get_data_object();
$res=$data->del_account($id);
 if($res){
       $msg=__('Account Deleted Successfully','gravity-forms-dynamics-crm');
  $msg_arr=array('msg'=>$msg,'class'=>'updated');   
 }else{
       $msg=__('Error While Removing Account','gravity-forms-dynamics-crm');
  $msg_arr=array('msg'=>$msg,'class'=>'error');      
 }
  update_option(vxg_dynamics::$id.'_msg',$msg_arr);
 }
  $redir=$this->link_to_settings();
wp_redirect($redir.'&'.vxg_dynamics::$id.'_msg=1');
die();
  }
...
$log_id=vxg_dynamics::post('id');
$log=vxg_dynamics::$data->get_log_by_id($log_id); 
...
$field_options="<option>".__("Select Object",'gravity-forms-dynamics-crm')."</option>"; 
  if(is_array($objects)){
...
$feeds=vxg_dynamics::$data->get_object_feeds($form_id,$account,$object,$skip);
$arr=array();
...
return $arr;
  ...
  */
  ...
  if(!current_user_can(vxg_dynamics::$id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','gravity-forms-dynamics-crm');   
  ...
  $offset=$this->time_offset();
   $id=vxg_dynamics::post('id');
  if(!empty($_POST[vxg_dynamics::$id."_uninstall"])){
  check_admin_referer("vx_nonce");
  if(!current_user_can(vxg_dynamics::$id."_uninstall")){
  return;
  }    
  $this->uninstall();
  $uninstall_msg=sprintf(__("Gravity Forms Dynamics Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'gravity-forms-dynamics-crm'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can(vxg_dynamics::$id."_edit_settings")){ 
  $msg=__('You do not have permissions to save settings','gravity-forms-dynamics-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','gravity-forms-dynamics-crm'));
  $valid_email=true;
  if(vxg_dynamics::post('error_email',$_POST['crm']) !=""){
   $emails=explode(",",vxg_dynamics::post('error_email',$_POST['crm']));
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','gravity-forms-dynamics-crm'));
  }
   $crm=$this->get_info($id);
   $data=isset($crm['data'])  && is_array($crm['data']) ? $crm['data'] : array();  
  /////////////
  $post=vxg_dynamics::post('crm');
  $data=array_merge($data,$post);


 // $data['disable_log']=vxg_dynamics::post('disable_log',$_POST['crm']);
  vxg_dynamics::update_info(array('data'=> $data),$id);
  $force_check=true;
  }                
 ...
 $new_account=$page_link."&id=".$new_account_id;
  ...
  //
    if(vxg_dynamics::post('vx_tab_action')== "refresh_lists_".vxg_dynamics::$id){ 
  check_admin_referer('vx_nonce');
  if(!current_user_can(vxg_dynamics::$id."_read_settings")){ 
  $msg=__('You do not have permissions to refresh lists','gravity-forms-dynamics-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $meta=vxg_dynamics::post('meta',$info);
  if(isset($meta['lists'])){
      unset($meta['lists']);
  }
  vxg_dynamics::update_info(array('meta'=>$meta),$id);
  $msgs['refresh']=array("class"=>"updated","msg"=>__('Successfully Refreshed Picklists','gravity-forms-dynamics-crm')); 
...
$info=$api->info; 
 ...
 // $tooltips=self::$tooltips ; 
  ...
  $conn_class=vxg_dynamics::post('class',$info);

 if(!empty($conn_class)){
      if(empty($info['msg']) && $conn_class == 'updated'){ 
          $info['msg']=__('Successfully Connected to Dynamics CRM','gravity-forms-dynamics-crm'); 
      }
  $msgs['connection']=array('class'=>$conn_class,'msg'=>$info['msg']);   
 }
 if(isset($_POST['vx_test_connection'])){
  $msg=__('Connection to Dynamics is Working','gravity-forms-dynamics-crm');
  
  if($conn_class != "updated" ){
      $msg=__('Connection to Dynamics is NOT Working','gravity-forms-dynamics-crm');  
  }
  $title=__('Test Connection: ','gravity-forms-dynamics-crm');
...
$sel2_css=$base_url. 'css/select2.min.css';
  ...
  $feed= vxg_dynamics::$data->get_feed($fid); 
//updating meta information
  if(isset($_POST[vxg_dynamics::$id."_submit"])){ 
  check_admin_referer("vx_nonce");
  if(!current_user_can(vxg_dynamics::$id.'_edit_feeds')){
  esc_html_e('You do not have permissions to edit/save feed','gravity-forms-dynamics-crm'); 
  return;
  }
  //
...
if(!empty($_POST['account'])){
 ...
 vxg_dynamics::update_info(array('meta'=>$info['meta']),$info['id']);
} }
if(is_array($feed_update) && is_array($feed)){
    $feed=array_merge($feed,$feed_update);
} 
  $is_valid=vxg_dynamics::$data->update_feed($feed_update,$fid);

  $msg=''; $class='updated';
  if($is_valid){
      $feed_link=$this->get_feed_link();
$msg=sprintf(__("Feed Updated. %sback to list%s", 'gravity-forms-dynamics-crm'), '<a href="'.$feed_link.'">', "</a>");
  }
  else{
$msg=__("Feed could not be updated. Please enter all required information below.", 'gravity-forms-dynamics-crm');
$class='error';
  }
  if(!empty($msg)){
  ...
  $accounts=$_data->get_accounts(true); 
     ...
     $this->account=$account=vxg_dynamics::post('account',$feed);
  $info=$this->get_info($account);
 $form_id=isset($_GET['id']) ? $this->post('id') : '';
  ...
  $config = vxg_dynamics::$data->get_feed('new_form');
   $new_feed_link=$this->get_feed_link($config['id']);
     ...
     $feeds_link=admin_url( 'admin.php?page=gf_edit_forms&view=settings&subview='.vxg_dynamics::$id."&tab=feed&id=$form_id" );
include_once(self::$path . "templates/feed-account.php");
  ...
  */
  ...
  $api_type=vxg_dynamics::post('api',$info);
  ...
  */
  ...
  $desc.=" Web2".$row['object'];
   ...
   $desc=sprintf(__('%s Filtered','gravity-forms-dynamics-crm'),$row['object']);   
  }else   if($row['status'] == 5){
   $desc=sprintf(__('%s Deleted','gravity-forms-dynamics-crm'),$row['object']);  
  }else{
  $desc.=$row['object'];
  }
  }else{
  ...
  $title=__("Failed",'gravity-forms-dynamics-crm');   
  if( $row['status'] == 1){
  $title=__("Created",'gravity-forms-dynamics-crm');   
  }else if($row['status'] == 2){
  $title=__("Updated",'gravity-forms-dynamics-crm');   
  }else if($row['status'] == 4){
  $title=__("Filtered",'gravity-forms-dynamics-crm');   
  }else if($row['status'] == 5){
  $title=__("Deleted",'gravity-forms-dynamics-crm');   
  }
  $row['_crm_id']= $crm_id;
  $row['a_link']=$link;
  $row['desc']=$desc;
  ...
  $fields[]=array('id',__('Entry ID','gravity-forms-dynamics-crm'));
  $fields[]=array('form_id',__('Form ID','gravity-forms-dynamics-crm'));
  $fields[]=array('entry_url',__('Entry URL','gravity-forms-dynamics-crm'));
  $fields[]=array('date_created',__('Entry Date','gravity-forms-dynamics-crm'));
  $fields[]=array('ip',__('User IP','gravity-forms-dynamics-crm'));
  $fields[]=array('source_url',__('Source Url','gravity-forms-dynamics-crm'));
  $fields[]=array('form_title',__('Form Title','gravity-forms-dynamics-crm'));
  $fields[]=array('status',__('Entry Status','gravity-forms-dynamics-crm'));
  $fields[]=array('payment_status',__('Payment Status','gravity-forms-dynamics-crm'));
  $fields[]=array('payment_date',__('Payment Date','gravity-forms-dynamics-crm'));
  $fields[]=array('payment_amount',__('Payment Amount','gravity-forms-dynamics-crm'));
  $fields[]=array('transaction_id',__('Transaction Id','gravity-forms-dynamics-crm'));
  $fields[]=array('currency',__('Currency','gravity-forms-dynamics-crm'));
  $this->fields=array('gf'=>array("title"=>__('Gravity Forms Fields','gravity-forms-dynamics-crm'),"fields"=>$fields)); 
...
if(is_array($fields_arr['fields'])){
  ...
  */
  ...
  $info=vxg_dynamics::post('data',$row);
$api_check=(int)vxg_dynamics::post('valid_api',$info);
 ...
 /* if($auto_check){
  ...
  if(vxg_dynamics::post('api',$info) == "web"){
  $res=$api->post_web(array(),$info,'Lead',true);
  $info["valid_api"]=current_time('timestamp')+86400; //api validity check
  //////////////////
  if($res === NULL){
  $info['msg']=__('Invalid Org ID. Please check your Org ID and try again.','gravity-forms-dynamics-crm'); 
  $info['class']="error";        
  }else{
  $info['msg']=__('Success: Your Org ID is properly configured','gravity-forms-dynamics-crm'); 
  $info['class']="updated";
  } 
   // save web2lead settings 
  vxg_dynamics::update_info(array( 'data' => $info),$info['id']);     
  ...
  $msg=__( 'Successfully Connected to Dynamics','gravity-forms-dynamics-crm' );
  ...
  $info['class']="";  
  if(isset($info['error']) && isset($info['dynamics_token_time']) && $info['error'] !=""){
  $info['msg']=$info['error']; 
  $info['class']="error"; 
  }       }
  }
