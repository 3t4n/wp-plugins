...
if( !class_exists( 'vxg_dynamics_form' ) ) {
class vxg_dynamics_form extends vxg_dynamics{
  ...
  add_action('wp_ajax_object_picklist_'.vxg_dynamics::$id, array(&$this, 'fields_html_ajax'));
  add_action('wp_ajax_object_area_'.vxg_dynamics::$id, array(&$this, 'object_html_ajax'));
  }
  }
$feed=get_option(vxg_dynamics::$id.'_lists',array()); 
  add_filter("gform_admin_pre_render", array(&$this, 'override_form'), 10);
  add_filter("gform_pre_render", array(&$this, 'override_form'), 10, 2);
  
  }
  /**
  * Replace the Gravity Forms form choices with remotely-pulled Dynamics picklist options.
  ...
  */
  ...
  if( vxg_dynamics::post('crm_map_enabled',$field) == vxg_dynamics::$id &&
  vxg_dynamics::post('crm_object',$field)!="" &&
  vxg_dynamics::post('crm_option',$field)!="" &&
  vxg_dynamics::post('crm_map_type',$field) == "live"
  ){
  $crm_field=vxg_dynamics::post('crm_option',$field); //crm field name
  $crm_object=vxg_dynamics::post('crm_object',$field); 
  $crm_account=vxg_dynamics::post('crm_account',$field); 
  if($crm_field!=""){
  ...
  */
  private function get_field_options($account,$object,$refresh=false){
    ...
    if(!is_array($feed)){
      ...
      $crm_fields=vxg_dynamics::post('fields',$fields); 
     $crm=array();
...
if(isset($info['data'])){
  ...
  $cach_time=(int)vxg_dynamics::post('cache_time',$data);
  if(empty($cach_time)){
      $cach_time=86400;
  }
  $field_time=(int)vxg_dynamics::post('time',$fields);
  ...
  $objects=vxg_dynamics::get_objects($info);

if(isset($objects[$object])){
  $lists[$object]=array("time"=>$time,"fields"=>$crm_fields);
 $feed['lists']=$lists;
  vxg_dynamics::update_info(array('meta'=>$feed),$account);
}else{
   $crm_fields=''; 
}
  }
  ...
  */
  ...
  foreach ($picklistValues as $key => $value) {
      if(!is_array($value)){ $value=array('value'=>$key,'label'=>$value); }
  $i++; 
  $choices[] = array(
  'text' => $value['label'],
  'value' => $value['value'],
  ...
  */
  ...
  $refresh=vxg_dynamics::post('refresh');
  $object=vxg_dynamics::post('object');
  $id=vxg_dynamics::post('account');
 ...
 public function fields_html($id,$object,$refresh=false){
  ...
  if(!is_array($fields)){
      $msg= empty($fields) ? esc_html__('Fields Not Found','gravity-forms-dynamics-crm') : $fields;
  $str = '<div class="field_group_'.vxg_dynamics::$id.'"><strong>'.__('Error:', 'gravity-forms-dynamics-crm').'</strong> '.wp_kses_post($msg).'</div>';
  ...
  */
  ...
  foreach($field as $key=>$value) {
   if(empty($value)) { continue; }     
        ...
        $default_html = !empty($value['default']) ?  '<strong class="default"> '.esc_html__('(Default)', 'gravity-forms-dynamics-crm').'</strong>' : '';
  ...
  */
  ...
  <label for='".vxg_dynamics::$id."_map_field_{$field['name']}'>
  <input value='{$field['name']}' type='radio' name='crm_map_field' class='".vxg_dynamics::$id."_map_field crm_map_field' id='".vxg_dynamics::$id."_map_field_{$field['name']}' />
  </label>
  </td>
  <td class='crm_field_cell'>
  <label for='".vxg_dynamics::$id."_map_field_{$field['name']}'><strong>" . stripslashes( $field['label'] )  . "</strong>
  ...
  <tr>
  <th scope='col' class='crm_col_heading'>" . esc_html__("Pickist Field", 'gravity-forms-dynamics-crm') . "</th>
  <th scope='col' class='crm_col_heading'>" . esc_html__("Form Fields", 'gravity-forms-dynamics-crm') . "</th>
  </tr>
  </thead>
  <tbody>".$str."
  </tbody>
  ...
  $str='<div><b>'.__('No Options Found','gravity-forms-dynamics-crm').'</b></div>';    
  }
  /// $str = str_replace(array("\n", "\t", "\r"), '', str_replace("'", "\'", $str));
...
public function editor_js() {
  ...
  // Reset the fields
  $('.<?php echo vxg_dynamics::$id ?>_map_type, .<?php echo vxg_dynamics::$id ?>_map_enabled').attr('checked', false);
  $('.<?php echo vxg_dynamics::$id ?>_field_list').html('');
   $(".<?php echo vxg_dynamics::$id ?>_object").val(""); 
  if(typeof field.crm_map_enabled !== "undefined" && field.crm_map_enabled=="<?php echo vxg_dynamics::$id ?>") { ///console.log(field.crm_map_enabled);
  ...
  div.find(".<?php echo vxg_dynamics::$id ?>_map_type[value='"+map_type+"']").prop('checked',true).trigger('change'); 
  setTimeout(function(){
  toggle_choices(obj);   
  },1000);     
  
 /// div.find(".<?php echo vxg_dynamics::$id ?>_object").val(field.crm_object).trigger('change');
 var account_sel=div.find(".<?php echo vxg_dynamics::$id ?>_account");
...
account_sel.val(crm_account);
 vx_load_account(account_sel,crm_object);
  ...
  $('.<?php echo vxg_dynamics::$id ?>_map_enabled').trigger('change'); 
  //select object
  
  });
  $('.<?php echo vxg_dynamics::$id ?>_map_enabled').unbind('change click').on('click change', function () {
  
  crm_checkbox($(this));
  });
  $('.<?php echo vxg_dynamics::$id ?>_map_type').unbind('change click').on('click change', function () {
  var val=$(this).val();
  ...
  $('.crm_setting').on('click','.<?php echo vxg_dynamics::$id ?>_map_field',function(e){
  ...
  //.unbind('change')
  $('.<?php echo vxg_dynamics::$id ?>_account').on('change',function(e){
vx_load_account($(this),field.crm_object);
  });

$(document).on('change','.<?php echo vxg_dynamics::$id ?>_object',function(e){ 
...
//  SetFieldProperty('crm_object',object); 
  ...
  div.find(".<?php echo vxg_dynamics::$id ?>_objects").html(res);
  div.find("#<?php echo vxg_dynamics::$id ?>_field_group").slideDown(); 
  sel_radio_option();
  ...
  var form_id=div.find(".crm_form_id").val();
  var action=elem.attr('id');
var account=div.find('.<?php echo vxg_dynamics::$id ?>_account').val();
  ...
  div.find(".<?php echo vxg_dynamics::$id ?>_field_list").html(res);
  div.find("#<?php echo vxg_dynamics::$id ?>_field_group").slideDown();
  ///div.find('._field_list').trigger('load');
sel_radio_option();
  //eval(res);
...
//  toggle_choices(check_box);  
 ...
 function sel_radio_option(){
  ...
  jQuery("#<?php echo vxg_dynamics::$id ?>_map_field_"+field.crm_option).attr('checked',true).trigger('change');
  /* var field = GetSelectedField();
  UpdateFieldChoices(field.type);
  LoadFieldChoices(field);*/   
  }
  ...
  */
  ...
  $tooltips['vx_map_live_'.vxg_dynamics::$id] = sprintf(__('%sUpdate from Dynamics %s If you update a picklist in Dynamics, the modifications will be added to your form without having to edit the field Choices in Gravity Forms. You will not be able to edit the Choices in Gravity Forms, you can only update them in Dynamics. The order of Choices as well as the default values are determined by the Dynamics picklist field settings.', 'gravity-forms-dynamics-crm'), '<h6>','</h6>');
  $tooltips['vx_map_once_'.vxg_dynamics::$id] = sprintf(__('%sUpdate from Dynamics %s Field Choices will not be updated live from Dynamics and are editable in Gravity Forms. If you make an edit in Dynamics, it will not be updated in your form.', 'gravity-forms-dynamics-crm'), '<h6>','</h6>');
  return $tooltips;
  }
  /**
  ...
  */
  ...
  <label for="vx_check_<?php echo vxg_dynamics::$id ?>">
  <input type="checkbox" autocomplete="off" class="<?php echo vxg_dynamics::$id ?>_map_enabled crm_map_enabled" id="vx_check_<?php echo vxg_dynamics::$id ?>" name="crm_map_enabled" value="<?php echo vxg_dynamics::$id ?>" /> <?php esc_html_e("Enable Dynamics Field Mapping?", 'gravity-forms-dynamics-crm'); ?>
  </label>
  
  <div id="<?php echo vxg_dynamics::$id ?>_map_ui" style="display: none;">
  
  <label for="<?php echo vxg_dynamics::$id ?>_map_type_live">
  <input type="radio" class="<?php echo vxg_dynamics::$id ?>_map_type crm_map_type" id="<?php echo vxg_dynamics::$id ?>_map_type_live" name="<?php echo vxg_dynamics::$id ?>_map_type" value="live" /> <?php esc_html_e("Live Remote Field Mapping ", 'gravity-forms-dynamics-crm'); gform_tooltip("vx_map_live_".vxg_dynamics::$id); ?>
  <span class="howto" style="padding-left:1.25em;"><?php esc_html_e("Field Choices will be synced from Dynamics picklist values.", 'gravity-forms-dynamics-crm'); ?></span>
  </label>
  <label for="<?php echo vxg_dynamics::$id ?>_map_type_once">
  <input type="radio" class="<?php echo vxg_dynamics::$id ?>_map_type crm_map_type" id="<?php echo vxg_dynamics::$id ?>_map_type_once" name="<?php echo vxg_dynamics::$id ?>_map_type" value="once" /> <?php esc_html_e("Only Populate Choices ", 'gravity-forms-dynamics-crm'); gform_tooltip("vx_map_once_".vxg_dynamics::$id) ?>
  <span class="howto" style="padding-left:1.25em;"><?php esc_html_e("Field Choices will not be updated live and are editable.", 'gravity-forms-dynamics-crm'); ?></span>
  </label>
    <label for="crm_accounts_list" style="width: 100px;" class="inline"><?php esc_html_e("Choose Account", 'gravity-forms-dynamics-crm'); ?></label>
      <select autocomplete="off" id="object_area_<?php echo vxg_dynamics::$id ?>" name="<?php echo vxg_dynamics::$id ?>_account" class="<?php echo vxg_dynamics::$id ?>_account fieldwidth-4 sel_account">
  <option value=""><?php esc_html_e("Select a Dynamics Account", 'gravity-forms-dynamics-crm'); ?></option>
  <?php
 ...
 <div class="<?php echo vxg_dynamics::$id ?>_objects"></div>
  </div>
 ...
 public function object_area($id,$object){
     ...
     $lists =vxg_dynamics::get_objects($info);
     ?>
       <label for="crm_object_list" style="width: 100px;" class="inline"><?php esc_html_e("Choose Object", 'gravity-forms-dynamics-crm'); ?></label>
  <?php
  if(!is_array($lists)) {
  echo esc_html__("Could not load Dynamics objects.", 'gravity-forms-dynamics-crm');
  } else { ?>

  <select autocomplete="off" id="object_picklist_<?php echo vxg_dynamics::$id ?>" name="crm_object_type" class="<?php echo vxg_dynamics::$id ?>_object fieldwidth-4 sel_object">
  <option value=""><?php esc_html_e("Select a Dynamics Object", 'gravity-forms-dynamics-crm'); ?></option>
  <?php
  ...
  <div class="<?php echo vxg_dynamics::$id ?>_field_list">
  <?php
