...
if( !class_exists( 'vxg_dynamics_data' ) ) {

/**
* since 1.0
*/
class vxg_dynamics_data extends vxg_dynamics{
/**
...
*/
  ...
  */
  ...
  global $wpdb;
  if(!empty($table))
  return $wpdb->prefix .  vxg_dynamics::$id."_".$table;
  else
  return $wpdb->prefix . vxg_dynamics::$id;
  }
  /**
  * get crm feeds
  * 
 ...
 */
  ...
  $items=isset($result[0]) ? $result[0]->total : 0;    
  $per_page = 20;
  $start = 0;
  ...
  $page_id=isset($_REQUEST['page_id'])&& $_REQUEST['page_id'] !="" ? vxg_dynamics::post('page_id') : "1";
  ...
  */
  ...
  // handle search
  $time_key=vxg_dynamics::post('time');
  ...
  $start_date=strtotime(vxg_dynamics::post('start_date').' 00:00:00');
  }
   if(!empty($_GET['end_date'])){
  $end_date=strtotime(vxg_dynamics::post('end_date').' 23:59:59');
   } 
  break;
  }
  ...
  if(vxg_dynamics::post('object')!=""){
  $search.=' and object ="'.esc_sql(vxg_dynamics::post('object')).'"';   
  }
  if(vxg_dynamics::post('status')!=""){
  $status=vxg_dynamics::post('status');
  if($status == "all"){$status="0";}
  $search.=' and status ='.esc_sql($status);   
  }
  if(vxg_dynamics::post('search')!=""){
  $search_s=esc_sql(vxg_dynamics::post('search'));
  ...
  if(vxg_dynamics::post('id')!=""){
  $form_id=esc_sql(vxg_dynamics::post('id'));
  $search.=' and form_id='.$form_id.'';   
  }
  if(vxg_dynamics::post('entry_id')!=""){
  $entry_id=esc_sql(vxg_dynamics::post('entry_id'));
  $search.=' and entry_id='.$entry_id.'';   
  }
  if($search!=""){
  $sql_end.=" where ".substr($search,4);
  }
  if(vxg_dynamics::post('orderby')!=""){
  $sql_end.=' order by '.esc_sql(vxg_dynamics::post('orderby'));   
  if(vxg_dynamics::post('order')!="" && in_array(vxg_dynamics::post('order'),array("asc","desc"))){
  $sql_end.=' '.vxg_dynamics::post('order'); 
  }
  }else{
