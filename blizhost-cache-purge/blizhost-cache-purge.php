...
class BlizCloudCachePurger {
	...
	public $p_version = '5.0.2';			// Plugin version
	...
	*/
	...
	public function bliz_cdn_imageurl_replace( $url ) {
		...
		// Use get_option('home') to get the base URL, including subdirectory
		...
		// Check if filename has size suffix like -670x443
		...
		if ( preg_match( $pattern, $filename, $matches ) ) {
			...
			$original_filename = $base_filename . '.' . $extension;
			$original_path = trailingslashit( $dirname ) . $original_filename;

			// Build the full file paths
			$upload_dir = wp_upload_dir();
			$upload_base_dir = $upload_dir['basedir']; // Absolute path to uploads directory
			$upload_base_url = $upload_dir['baseurl']; // Base URL to uploads directory

			// Remove the upload base URL from the original URL to get the relative path
			$relative_original_path = str_replace( $upload_base_url, '', $home_url_original . $original_path );
			$original_file_full_path = trailingslashit( $upload_base_dir ) . ltrim( str_replace( '/', DIRECTORY_SEPARATOR, $relative_original_path ), DIRECTORY_SEPARATOR );

			// Check if original file exists without using file_exists unnecessarily
			static $file_exists_cache = array();
			if ( isset( $file_exists_cache[ $original_file_full_path ] ) ) {
				$original_file_exists = $file_exists_cache[ $original_file_full_path ];
			} else {
				$original_file_exists = file_exists( $original_file_full_path );
				$file_exists_cache[ $original_file_full_path ] = $original_file_exists;
			}

			if ( $original_file_exists ) {
				// Original file exists, use it
				$cdn_image_path = $original_path;
			} else {
				// Original file does not exist, use the current path
				$cdn_image_path = $path;
				// Width and height are from filename
			}
		} else {
			// No size suffix found; use the original path
			$cdn_image_path = $path;

			// Attempt to get width and height from image metadata
			// Build the full file path
			$upload_dir = wp_upload_dir();
			$upload_base_dir = $upload_dir['basedir']; // Absolute path to uploads directory
			$upload_base_url = $upload_dir['baseurl']; // Base URL to uploads directory

			// Remove the upload base URL from the URL to get the relative path
			$relative_path = str_replace( $upload_base_url, '', $url );
			$image_file_full_path = trailingslashit( $upload_base_dir ) . ltrim( str_replace( '/', DIRECTORY_SEPARATOR, $relative_path ), DIRECTORY_SEPARATOR );

			// Check if file exists and get dimensions
			static $image_size_cache = array();
			if ( isset( $image_size_cache[ $image_file_full_path ] ) ) {
				list( $width, $height ) = $image_size_cache[ $image_file_full_path ];
			} else {
				if ( file_exists( $image_file_full_path ) ) {
					$image_info = getimagesize( $image_file_full_path );
					if ( $image_info ) {
						$width = $image_info[0];
						$height = $image_info[1];
						$image_size_cache[ $image_file_full_path ] = array( $width, $height );
					}
				}
			}
		}
		...
		// Build the CDN URL including the original host and the image path
		$cdn_url = 'https://i' . $this->bliz_cdn_get_server_num( $url ) . '.' . $cdn_domain . '/' . $host . $cdn_image_path;
