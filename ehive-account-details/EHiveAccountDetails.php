...
if (in_array('ehive-access/EHiveAccess.php', (array) get_option('active_plugins', array()))) {
    ...
    class EHiveAccountDetails {
    	...
    	const CURRENT_VERSION = 2; // Increment each time an upgrade is required. (Options added or deleted.)
        ...
        function ehive_account_details_admin_options_init() {
        	...
        	add_settings_section('google_map_section', 'Account Location - Google Maps', array(&$this, 'google_map_section_fn'), __FILE__);
        ...
        function google_map_section_fn() {
            add_settings_field('google_map_enabled', 'Show location', array(&$this, 'google_map_enabled_fn'), __FILE__, 'google_map_section');
            add_settings_field('google_map_api_key', 'Google Maps API key', array(&$this, 'google_map_api_key_fn'), __FILE__, 'google_map_section');
        }
        ...
        function google_map_enabled_fn() {
            $options = get_option('ehive_account_details_options');
            $checked = '';
            if(isset($options['google_map_enabled']) && $options['google_map_enabled']) {
                $checked = ' checked="checked" ';
            }
            echo "<input ".$checked." id='google_map_enabled' name='ehive_account_details_options[google_map_enabled]' type='checkbox' />";
        }
                
        function google_map_api_key_fn() {
            $options = get_option('ehive_account_details_options');
            echo "<input id='google_map_api_key' name='ehive_account_details_options[google_map_api_key]' class='regular-text' type='text' value='{$options['google_map_api_key']}' />";
            echo '<p>An API key is required for the <a href="https://developers.google.com/maps/documentation/javascript/tutorial" target="_blank">Google Maps JavaScript API</a>.</p>';
        }
        ...
        function plugin_css_enabled_fn() {
        	$options = get_option('ehive_account_details_options');
        	$checked = '';
        	if(isset($options['plugin_css_enabled']) && $options['plugin_css_enabled']) {
        		$checked = ' checked="checked" ';
        	}
        ...
        public function enqueue_scripts() {
        	...
        	if (is_page( $accountDetailsPageId )){        	        	
	        	...
	        	if (isset($options['google_map_enabled']) && $options['google_map_enabled'] == 'on') {	   
	        	    if ( (isset($options['google_map_api_key']) == false) || (isset($options['google_map_api_key']) && empty($options['google_map_api_key'])) ) {
	        	      wp_register_script($handle = 'googleMapAPI', $src='https://maps.google.com/maps/api/js?sensor=false', $deps = array('jquery'), '', false);
	        	    } else {
	        	        wp_register_script($handle = 'googleMapAPI', $src='https://maps.google.com/maps/api/js?key='.$options['google_map_api_key'].'&sensor=false', $deps = array('jquery'), '', false);
	        	    }
	        	  wp_enqueue_script( 'googleMapAPI' );
	        	}

	        	if (isset($options['google_map_enabled']) && $options['google_map_enabled'] == 'on') {
	        	  wp_register_script($handle = 'eHiveAccountDetailsMap', $src= plugins_url('eHiveAccountDetailsMap.js', '/ehive-account-details/js/eHiveAccountDetailsMap.js'), $deps = array('jquery','googleMapAPI'), $ver = '1.0.1', false);
	        	  wp_enqueue_script( 'eHiveAccountDetailsMap' );
	        	}
	        	
	        	wp_register_script($handle = 'eHiveAccountDetailsProfileImage', $src= plugins_url('eHiveAccountDetailsProfileImage.js', '/ehive-account-details/js/eHiveAccountDetailsProfileImage.js'), $deps = array('jquery','jcarousellite','prettyPhoto'), $ver = '1.0.1', false);          	
	        	wp_enqueue_script( 'eHiveAccountDetailsProfileImage' );	        	
        	}   
        }        	        
        ...
        public function ehive_account_details_shortcode($atts, $content) {
        	...
        	if ($account_id == 0) {
        	    $account_id = ehive_account_details_get_var('ehive_account_id');
        	}
        ...
        function add_rewrite_rules($rules) {
            ...
            if (isset($eHiveAccess)) {
                ...
                if ($pageId != 0) {
                    ...
                    $wp_rewrite->add_rewrite_tag($accountIdToken, '([0-9]+)', "pagename={$permalinkNoDomain}&ehive_account_id=");
                    $rules = $wp_rewrite->generate_rewrite_rules($wp_rewrite->root . "{$permalinkNoDomain}$accountIdToken") + $rules;
                }                
            }
            return $rules;
        }
		...
		function ehive_plugin_update() {     
			...
			if ( get_option(self::EHIVE_ACCOUNT_DETAILS_OPTIONS) === false ) {
				...
				if ( $updateVersion == self::CURRENT_VERSION ) {
					...
					if ( $updateVersion == 1 ) {
					    $options["google_map_enabled"] = "on";
					    $options["google_map_api_key"] = "";
    ...
    function ehive_account_details_get_var($name, $default = false) {
        global $wp_query;
    ...
    add_filter('init', array(&$eHiveAccountDetails, 'flushRules'));
