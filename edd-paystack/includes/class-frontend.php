...
class Frontend {
	...
	public function hooks() {
		...
		add_filter( 'edd_rwf_currency_filter_before', array( $this, 'format_rwf_currency_before' ), 10, 3 );
		add_filter( 'edd_rwf_currency_filter_after', array( $this, 'format_rwf_currency_after' ), 10, 3 );
	}
	...
	public function process_redirect() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( 'paystack' === strtolower( trim( sanitize_text_field( wp_unslash( $_GET['edd-listener'] ) ) ) ) ) {
			do_action( 'tbz_edd_paystack_redirect_verify' );
		}

		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( 'paystackipn' === strtolower( trim( sanitize_text_field( wp_unslash( $_GET['edd-listener'] ) ) ) ) ) {
	...
	public function process_ipn() {
		...
		if ( ( 'POST' !== strtoupper( sanitize_text_field( wp_unslash( $_SERVER['REQUEST_METHOD'] ) ) ) ) || ! array_key_exists( 'HTTP_X_PAYSTACK_SIGNATURE', $_SERVER ) ) {
			exit;
		}
	...
	public function payment_icons( $icons ) {
		...
		$icons[ TBZ_EDD_PAYSTACK_URL . 'assets/images/paystack-civ.png' ]   = __( 'Paystack (Côte d’Ivoire)', 'edd-paystack' );
	 ...
	 * @return string
	 */
	public function format_rwf_currency_before( $formatted, $currency, $price ) {
		$symbol = edd_currency_symbol( $currency );
	 ...
	 * @return string
	 */
	public function format_rwf_currency_after( $formatted, $currency, $price ) {
		$symbol = edd_currency_symbol( $currency );
	...
	private function get_supported_currencies() {
		$supported_currencies = array( 'GHS', 'NGN', 'USD', 'ZAR', 'KES', 'XOF', 'EGP', 'RWF' );
