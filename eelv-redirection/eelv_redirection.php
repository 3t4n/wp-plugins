...
<div class="wrap">
   ...
   <?php wp_nonce_field( 'eelv_redirection_settings' ); ?>

    <p>
            <label for="eelv_when_redirect"><?php _e('When:','eelv-redirection')?></label>
            <select name="eelv_when_redirect" id="eelv_when_redirect">
                <option value="-1" <?php selected($eelv_when_redirect, -1, true); ?>><?php echo esc_attr('Never (deactivated)','eelv-redirection')?></option>
                <option value="0" <?php selected($eelv_when_redirect, 0, true); ?>><?php echo esc_attr('Always','eelv-redirection')?></option>
                <option value="1" <?php selected($eelv_when_redirect, 1, true); ?>><?php echo esc_attr('Only non-logged-in users','eelv-redirection')?></option>
            </select>
        </p>
        <p>
            <label id="eelv_code_redirect"><?php _e('How:','eelv-redirection')?></label>
            <select name="eelv_code_redirect" id="eelv_code_redirect">
                        <option value="302" <?php selected($eelv_code_redirect, 302, true); ?>><?php echo esc_attr('302 : Moved Temporarily','eelv-redirection')?></option>
                <option value="301" <?php selected($eelv_code_redirect, 301, true); ?>><?php echo esc_attr('301 : Moved Permanently','eelv-redirection')?></option>
            </select>
        </p>
        <p>
            <label id='eelv_url_redirect'><?php _e('Where:','eelv-redirection')?></label>
            <input type="text" name='eelv_url_redirect' id='eelv_url_redirect' value="<?php echo esc_attr($eelv_url_redirect) ?>" size="50" placeholder="https://">
        ...
        <details>
            <summary><?php _e('Advanced settings','eelv-redirection')?></summary>
            <?php if(defined('REDIRECT_ALLOW_USER_AGENT')): ?>
                <p><?php echo esc_html(__('Allowed users agent pattern','eelv-redirection')); ?></p>
                <p><code><?php echo REDIRECT_ALLOW_USER_AGENT; ?></code></p>
            <?php else: ?>
                <p><?php echo esc_html(__('You can allow some user-agents to bypass redirect by adding this to your wp-config.php','eelv-redirection')); ?></p>
                <p><code>define('REDIRECT_ALLOW_USER_AGENT', 'HTTrack');</code></p>
                <p><?php echo esc_html(__('Replace HTTrack by a pattern to find in user-agents.','eelv-redirection')); ?></p>
            <?php endif; ?>
        </details>
