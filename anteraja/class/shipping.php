...
class Anteraja extends WC_Shipping_Method{
 ...
 public function init_form_fields(){
    ...
    'order_status_pickup_request' => array('type' => 'order_status_pickup_request')
  );
 ...
 public function generate_cod_setting_html() {
   $json_api_cod_eligible = epkn_antr_is_cod_eligible();
   $to_array = json_decode($json_api_cod_eligible, true); 
   $is_cod_eligible = $to_array['result']['status']['cod'] ? '' : 'disabled';
   $logger = new WC_Logger(); 
   $logger -> add('anteraja',$is_cod_eligible);
   ob_start();
   if(get_option('epeken_enable_cod_kurir') === 'on')
     update_option('epeken_anteraja_admin_cod_enabled','on');

   $admin_cod_enabled = get_option('epeken_anteraja_admin_cod_enabled');
   echo '<tr align="left">';
   echo '<th scope="row" class="titledesc">Cash On Delivery (COD)</th>';
   echo '<td>';
   echo '<p><input name="admin_cod_enabled" type="checkbox" '; 
   if($admin_cod_enabled === 'on') {
     echo 'checked';
   }
   echo ' '.$is_cod_eligible.'>Enable COD</input>';
   if($is_cod_eligible === 'disabled')
	   echo '<p><em>COD Kurir tidak tersedia. Hubungi Epeken Untuk mengaktifkan fungsi COD kurir.</em></p>';
   echo '</td></tr>'; 
   return ob_get_clean();
 }
 ...
 public function generate_order_status_pickup_request_html() {
	 //TODO
	ob_start(); 
	$option_status = sanitize_text_field(get_option('epeken_anteraja_order_status_pickup_request'));
	$statuses = wc_get_order_statuses();	
	echo '<tr align="left">';echo "\n";
	echo '<th scope="row" class="titledesc">Status Order <br>Permintaan Pickup</th><td>';echo "\n";
	$dropdown = '<select name="order_status_pickup_request">';
        foreach($statuses as $key => $value) {
                 if(empty($option_status) && $key === 'wc-processing') {
                        $dropdown .= '<option value="'.$key.'" selected>'.$value.'</option>';
                        update_option('epeken_anteraja_order_status_pickup_request','wc-processing');
                        continue;
                 }
                 $s = '';
                 
                 if ($option_status === $key)
                   $s = 'selected';

                 $dropdown .= '<option value="'.$key.'" '.$s.'>'.$value.'</option>';
        }
        $dropdown .= '</select>';
	echo $dropdown;
	echo '</td></tr>'; echo "\n";
 ...
 public function process_update_data_tarif() { 
  ...
  update_option('epeken_anteraja_admin_cod_enabled', sanitize_text_field($_POST['admin_cod_enabled']));
  update_option('epeken_enable_cod_kurir',sanitize_text_field($_POST['admin_cod_enabled']));
  update_option('epeken_anteraja_order_status_pickup_request', sanitize_text_field($_POST['order_status_pickup_request']));
 }
 ...
 public function calculate_shipping($package=array()){
   ...
   foreach($rates['result'] as $rate){
     ...
     $pid = '';
     $admin_cod_enabled = get_option('epeken_anteraja_admin_cod_enabled');
     if ($rate['is_cod'] == true && $admin_cod_enabled === 'on'){
	$pid = '_cod'; //If COD Eligible then add COD flag
	if($cod_payment == false) {
	  add_filter('woocommerce_available_payment_gateways',array($this, 'set_payment_cod'), 1);
	  $cod_payment = true;
	}
     }
     $normalized_rate = array('id' => $this -> id.'_'.$rate['product_code'].$pid,
		 'label' => $t.' ('.$rate['etd'].')',
		 'cost' => $rate['rates'],
		 'taxes' => false
	   );
...
public function is_chosen_shipping_cod ($chosen_shipping) {
     $check = strpos($chosen_shipping, '_cod');
     if($check !== FALSE)
       return true;
     else
       return false;
...
public function set_payment_cod ($gateways) {
