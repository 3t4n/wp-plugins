...
function epkn_antr_is_cod_eligible() {
  $license = sanitize_text_field(get_option('epeken_wcjne_license_key'));
  $url = EPEKEN_API_ANTERAJA_IS_COD_ELIGIBLE.$license;
  $response = wp_remote_get($url); 
  $msg = wp_remote_retrieve_body($response); 
  return $msg;
}

function epkn_antr_book($invoice_no, //invoice_no is order_id
			 $service_code, $parcel_total_weight,$receiver_name,
 ...
 if ($payment_method === 'cod'){
	$total_cod = (int)$order -> get_total();
	$post_data['cod'] = array('is_cod' => true, 'total_cod' => $total_cod);	
	$timestamp = time() + (60*60);
	$timestamp = date('Y-m-d H:i:s', $timestamp);
	$post_data['expect_time'] = $timestamp;
 }
 $response = wp_remote_post($url, array(
  ...
  if($content_json['status'] === 200) {
    ...
    $order -> update_meta_data('_anteraja_waybill_no', $content_json['content']['waybill_no']);
    $order -> save();
    ...
    $order_status_pickup_request = get_option('epeken_anteraja_order_status_pickup_request');
    if(empty($order_status_pickup_request))
	    $order_status_pickup_request = 'wc-processing';

    $order -> update_status($order_status_pickup_request); 
