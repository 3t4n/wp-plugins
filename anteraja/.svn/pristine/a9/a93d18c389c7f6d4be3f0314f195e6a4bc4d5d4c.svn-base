...
$epeken_antr_api_cod_eligible = EPEKEN_ANTERAJA_SERVER_URL.'/api/'.$epkn_antr_api_end_point.'/is_cod_eligible/';
...
define('EPEKEN_API_ANTERAJA_IS_COD_ELIGIBLE', $epeken_antr_api_cod_eligible);
...
function epkn_antr_save_shipping_meta($order_id) {
  ...
  if($order -> has_shipping_method('anteraja')) {
    ...
    if( version_compare( $woocommerce->version, '8.5' , ">=" )) {
	    $order_receiver_name = $order_data['billing']['first_name'].' '.$order_data['billing']['last_name'];
	    $order_receiver_postcode = $order_data['billing']['postcode'];
	    $receiver_address = $order_data['billing']['address_1'];
	    $receiver_kecamatan = $order_data['billing']['address_2'];
	    $receiver_city = $order_data['billing']['city'];
	    $receiver_kelurahan = $order_meta_data[2] -> __get('_billing_address_3');
	    $receiver_province = $order_data['billing']['state'];
	    $receiver_province = $country_states_array['ID'][$receiver_province];
	    $order_receiver_phone = $order_data['billing']['phone'];
	    $order_receiver_email = $order_data['billing']['email'];
	    if(!empty($order_data['shipping']))
	    {
	     $order_receiver_name = $order_data['shipping']['first_name'].' '.$order_data['shipping']['last_name'];
	    $order_receiver_postcode = $order_data['shipping']['postcode'];
	    $receiver_address = $order_data['shipping']['address_1'];
	    $receiver_kecamatan = $order_data['shipping']['address_2'];
	    $receiver_city = $order_data['shipping']['city'];
	    $receiver_kelurahan = $order_meta_data[2] -> __get('_billing_address_3');
	    $receiver_province = $order_data['shipping']['state'];
	    $receiver_province = $country_states_array['ID'][$receiver_province];
	    $order_receiver_phone = $order_data['shipping']['phone'];
	    }
    } else {
...
function epkn_antr_metabox() {
  ...
  $order_no = '';
  if(version_compare($woocommerce->version, '8.5', ">=")){
      $order_no = sanitize_text_field($_GET['id']);
  }else{
      $order_no = sanitize_text_field($_GET['post']);
  }
  if(empty($order_no))
      $order_no = sanitize_text_field($_GET['post']);
  //}
  ...
  $awb_no = $order -> get_meta('_anteraja_waybill_no');
  if(!empty($awb_no)) {
    echo '<p><strong>Nomor Resi Anteraja:</strong> <a>'.$awb_no.' </a></p>';
    ...
    echo '<input type="hidden" id="waybillno" value="'.$awb_no.'"/>';
    return;
  }
  ...
  $anteraja_ordermeta = get_post_meta($order_no,'_anteraja_postmeta', true);
  echo '<p>';
  ...
  echo '<input type="hidden" id="order_weight" value="'.(esc_html((int)sanitize_text_field($anteraja_ordermeta['order_weight'])) * 1000).'"/>';
...
function epkn_antr_get_order_parameters() {
   ...
   $use_insurance = false;
   $use_insurance = apply_filters('anteraja_use_insurance', $use_insurance, $declared_value); 
...
function epkn_antr_track_order($order) {
  ...
  if(empty($awb))
    $awb = $order -> get_meta('_anteraja_waybill_no');
...
add_action( 'epeken_custom_tariff', 'epkn_antr_hide_other_shipping_prices', 100 );
function epkn_antr_hide_other_shipping_prices( $epeken_shipping ) {
  $pm = trim(WC() -> session -> get('chosen_payment_method'));
  if($pm !== 'cod')
    return; //do nothing

  $new_rates = array();
  foreach ( $epeken_shipping -> array_of_tarif as $rate ) {
   if ( strpos($rate['id'], 'anteraja') === FALSE ) {
    //do nothing
   }else{
     array_push($new_rates,$rate);
   }
  }
  $epeken_shipping -> array_of_tarif = $new_rates;
}
?>
