...
class Feedaty_Woocommerce_Rating_Public {
	...
	/**
	 * Check if we can use javascript.
	 *
	 * @since    1.0.4
	 */
	public function js_enabled () {
		$enabled = true;
		return apply_filters( 'feedaty_enqueue_js', $enabled );
	}
	...
	public function enqueue_scripts() {
		if ( $this->js_enabled() && !empty( $this->options['script'] ) ) {
			wp_register_script( $this->plugin_name, $this->options['script'], [], false, true );
			wp_enqueue_script( $this->plugin_name );
		}

		//do_action( 'feedaty_cron_continuous' );
	...
	public function append_microdata ( $markup ) {
		if ( isset( $this->options['product_microdata'] ) && $this->options['product_microdata'] > 0 ) {
			...
			if ( is_object( $product ) ) {
				...
				if ( isset($ratings->Product->RatingsCount) && isset($ratings->Product->AvgRating) ) {
	...
	public function deploy_product_carousel () {
		...
		if ( is_object( $product ) ) {
			...
			if ( is_object( $badge ) && isset( $nadge->html ) ) {
				$html = $webservice->api_injector( $badge->html, $id );
				printf(
					'<div class="feedaty_fwr_product_badge">%s</div>',
					$html
	...
	public function get_product_identifier( $product ) {
		...
		if ( is_object( $product ) ) {
			...
			$id      = $product->get_id();
			$sku     = empty( $product->get_sku() ) ? $id : $product->get_sku();
    ...
    /**
     * Add hidden field to retrieve current locale and add it to the order metas
     *
    ...
    {
        $fields[ 'billing' ][ 'billing_locale' ] = [
            'type'     => 'hidden',
            'required' => false,
            'class'    => [ 'feedaty-hidden' ],
            'default'  => strtoupper( substr( get_locale(), 0, 2 ) )
        ];
