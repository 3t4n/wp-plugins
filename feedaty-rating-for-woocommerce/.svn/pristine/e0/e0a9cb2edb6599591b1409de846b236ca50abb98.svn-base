<?php
	...
	class class_feedaty_woocommerce_rating_widget_carousel extends WP_Widget 	{
		...
		function __construct ($plugin_name) {
            ...
            $this->version = isset( $this->options[ 'version' ] ) ? $this->options[ 'version' ] : '1.0.0';
            if ( is_array( $this->options ) && isset( $this->options['store_carousel_id'] ) && !empty( $this->options['store_carousel_enabled'] ) ) {
		...
		public function widget ( $args, $instance ) {
			...
			if ( is_array( $this->options ) && !empty( $this->options['store_carousel_enabled'] ) ) {
				if ( is_object( $this->badge ) && !empty( $this->badge->html ) ) {
					echo $this->webservice->api_injector( $this->badge->html );
				} else {
					if ( is_user_logged_in() && current_user_can( 'activate_plugins' ) ) {
						_e( 'We econtutered an error with the current widget. Please try with a different widget or contact Feedaty support.[Displays only to Admin]', $this->plugin_name );
					}
				}
			}
