	...
	class Feedaty_Woocommerce_Rating_Webservice	{
		...
		public function getWidgets( $collection = null, $caller = null ) {
			...
			if ( is_admin() || empty( $content ) ) {
				...
				//$url = add_query_arg( $args, 'https://widget.feedaty.com/' );echo $url;exit;
				$url = 'https://widget.feedaty.com/v3.0.0/widget_list/2021/'.$this->merchantCode; //echo $url;exit;
				$js = add_query_arg( $args_js, 'https://widget.feedaty.com/public/js/feedaty.min.js' );
				$this->options['script'] = $js;
		...
		public function hookCompleteFeedatyOrder ( $order_id ) {
			$token = $this->getRequestToken();
			
			$accessToken = json_decode( $this->getAccessToken( $token )['body'] );
			
			$this->sendOrdersDataHistory( $accessToken, $order_id );
		}
		
		function sendMerchantInfo ( $data ) {
			$url = 'https://zoo:rate@www.zoorate.com/ws/feedatyapi.svc/SetPluginKeyValue';
			$header = [
			...
			foreach ( $data as $key => $value ) {
				if ( 'merchant_code' == $key ) {
					$array['merchantCode'] = $value;
				}
				$array['keyValuePairs'][] = [
					'Key'   => $key,
					'Value' => $value
			...
			$remote_post = wp_remote_post( $url, [
		...
		private function getProducts( $items ){
			...
			foreach ( $items as $item_key => $item ) {
				...
				if ( is_object( $product ) ) {
					...
					//TODO: if SKU is enabled, pass product SKU instead of ID
					$sku = empty( $product->get_sku() ) ? $product_id : $product->get_sku();
					if( isset( $options['productIdentifier'] ) && $options['productIdentifier'] == 'sku' ) {
						$product_id = $sku;
					}
					...
					$fdProduct = [
						...
						'Name'         => $item->get_name(),
						'ThumbnailURL' => wp_get_attachment_image_url( $image_id, 'full' ),
		 ...
		 * @return $response
		 */
		function sendOrder( $accessToken, $order_id, $status = null ) {
			$order = null;
			
			// Check if $order_id is already a WC_Order object
			if (is_a($order_id, 'WC_Order')) {
				$order = $order_id;
				$order_id = $order->get_id();
				$url = 'https://api.feedaty.com/Orders/RecordOrdersDataHistory';
			} elseif (is_numeric($order_id)) { // Check if $order_id is a numeric ID and get the WC_Order object
				$url = 'https://api.feedaty.com/Orders/Insert';
		...
		/**
		 * retrieves an order and pass it to sendOrder
		 *
		 * @param $accessToken
		 * @param $order_id
		 *
		 ...
		 * @return callback
		 */
		public function sendOrdersDataHistory ( $accessToken, $order_id ) {
			$order = wc_get_order( $order_id );
			$status = $order->get_status();
			
			return $this->sendOrder( $accessToken, $order, $status );
		}
		...
		function getAccessToken( $token ) {
			...
			$url = "https://api.feedaty.com/OAuth/AccessToken";
		...
		function getRequestToken() {
			...
			$url = "https://api.feedaty.com/OAuth/RequestToken";
