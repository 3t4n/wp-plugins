...
class MagicLoginMail {
	...
	public function __construct() {
		...
		$this->upload_dir = untrailingslashit( $upload_dir ) . wp_normalize_path( '/' );
		...
		add_filter( 'magic_login_get_qr_code', array( $this, 'get_qr_code' ), 10, 1 );
		add_action( 'show_password_fields', array( $this, 'user_page_pass_fileds' ), 10, 2 );
	...
	public function front_end_login() {
		...
		if ( ! is_null( $account_email ) &&
			...
			$html .= '<input type="text" inputmode="url" pattern="[a-zA-Z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" name="magic_user_email" id="magic_user_email" class="' . esc_attr( $input_class_name ) . '" value="' . esc_attr( $account_email ) . '" size="' . esc_attr( $input_size ) . '" placeholder="' . esc_attr__( 'Email', 'magic-login-mail' ) . '" required="required" />';
	...
	public function send_link( $email_arr, $admin_batch ) {
		...
		foreach ( $email_arr as $email ) {
			...
			list( $valid_email, $uid ) = $this->valid_account( $email );
			...
			if ( ! $valid_email ) {
				...
				$exp_date_time = $this->get_exp_date_time( $uid );
				/* translators: %1$s blogname %2$s loginurl %3$s Expiration date and time */
				$message = apply_filters( 'magic_login_mail_message', sprintf( __( 'Hello ! <br><br>Login at %1$s by visiting this url or attached QR Code: <a href="%2$s">%2$s</a><br><br>The login expiration date is %3$s.', 'magic-login-mail' ), $blog_name, esc_url( $unique_url ), $exp_date_time ) );

				$filename = $this->generate_qr_code( $unique_url, 'file' );
				$sent_mail = wp_mail( $valid_email, $subject, $message, '', $filename );
	...
	/** ==================================================
	 * Get the unique login QR Code from email.
	 *
	 * @param array $email  email.
	 * @since 2.00
	 */
	public function get_qr_code( $email ) {
		...
		if ( $valid_email ) {
			$unique_url = $this->generate_url( $valid_email );
			$img = $this->generate_qr_code( $unique_url, 'base64' );
			$exp_date_time = $this->get_exp_date_time( $uid );
		}

		return array( $img, $exp_date_time );
	}

	/** ==================================================
	 * Generate QR Code with the unique login link.
	 *
	 * @param string $unique_url  unique login link.
	 * @param string $flag  'file': filename, 'base64': base64 Text.
	 * @since 2.00
	 */
	private function generate_qr_code( $unique_url, $flag ) {

		$generator = new QRCode( $unique_url );
		$image = $generator->render_image();

		switch ( $flag ) {
			case 'file':
				$img_or_file = $this->upload_dir . 'QR_Code.png';
			...
			case 'base64':
				...
				$img_or_file = base64_encode( $imagedata );
				break;
		}
		imagedestroy( $image );
	...
	/** ==================================================
	 * View QR Code for user's page.
	 *
	 * @param bool   $show  Whether to show the password fields. Default true.
	 * @param object $profile_user  User object for the current user to edit.
	 * @since 2.01
	 */
	public function user_page_pass_fileds( $show = true, $profile_user ) {

		$qrmail = $profile_user->data->user_email;
		list( $qrcode, $exp_date_time ) = $this->get_qr_code( $qrmail );
		$qrcode = 'data:image/png;base64,' . $qrcode;

		/* translators: %1$s Expiration date and time */
		$exp_message = sprintf( __( 'The login expiration date is %1$s.', 'magic-login-mail' ), $exp_date_time );

		?>
		<table class="form-table" role="presentation">
			<tbody>
			<tr class="user-description-wrap">
				<th><label for="description"><?php esc_html_e( 'Log in with QR Code', 'magic-login-mail' ); ?></label></th>
				<td>
					<div><img src="<?php echo esc_attr( $qrcode ); ?>" /></div>
					<p class="description"><?php echo esc_html( $exp_message ); ?></p>
				</td>
			</tr>
			</tbody>
		</table>
	 ...
	 * @return array( email, UserID ) / array( bool, bool )
	 * @since 1.00
	 */
	private function valid_account( $email ) {
		...
		if ( is_email( $valid_email ) && email_exists( $valid_email ) ) {
			$uid = email_exists( $valid_email );
			return array( $valid_email, $uid );
		}
	 ...
	 * @since 2.01
	 */
	private function get_exp_date_time( $uid ) {

		$hash_meta_expiration = get_user_meta( $uid, 'magic_login_mail_' . $uid . '_expiration', true );
		$exp_date_time = wp_date( 'Y-m-d H:i:s', $hash_meta_expiration );
