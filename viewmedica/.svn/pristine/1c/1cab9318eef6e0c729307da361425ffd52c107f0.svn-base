...
function swarm_viewmedica_display($atts = null, $content = null) {
    ...
    if( $atts != null ) {
        ...
        $sanitized_title = sanitize_title($a['openthis']); 
        $openthis = ucfirst($sanitized_title);
        ...
        if($openthis != '') {
            $viewmedica_div = "<div id='" . esc_attr($openthis) . "'></div>";
            $param_string .= 'openthis: "' . esc_js($openthis) . '", ';
        } else {
