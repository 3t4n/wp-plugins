...
if ($_POST['swarm_hidden'] == 'C') {
  ...
  $sql = $wpdb->prepare(
      "UPDATE " . $table_name . " SET vm_id = %s WHERE id = %d",
      $client, // Placeholder for vm_id
      1        
  );
  ...
  $secure = isset($_POST['vm_secure']) && $_POST['vm_secure'] == '1' ? 1 : 0;
  $brochures = isset($_POST['vm_brochures']) && $_POST['vm_brochures'] == '1' ? 1 : 0;
  $fullscreen = isset($_POST['vm_fullscreen']) && $_POST['vm_fullscreen'] == '1' ? 1 : 0;
  $disclaimer = isset($_POST['vm_disclaimer']) && $_POST['vm_disclaimer'] == '1' ? 1 : 0;
  $visible = isset($_POST['vm_visible']) && $_POST['vm_visible'] == '1' ? 1 : 0;
  ...
  function handleItem($item, $level, $format, $size, $float) {
    if (!$item->children) {
      return child(ucfirst($item->code), $item->labels->en, $item->file, $size, $float, $item->descriptions->en, $format, $level);
    }
    ...
    return $headingOpen . "<a href=\"#\" class=\"vm-link level-" . $level . "\" data-video=\"" . ucfirst($item->code) . "\" >" . $item->labels->en . "</a>" . $headingClose . "\r\n\r\n" . $children;
  }
