...
final class WooCommerce_Settings {
	...
	public static function register() {
        ...
        // Load scripts
        add_action('admin_enqueue_scripts', [$instance, 'enqueue_nofraud_settings_script']);
    ...
    public function admin_init() {
        ...
        wp_register_script( 'nofraud-protection-settings-script', plugins_url('../assets/js/settings.js', __FILE__) );
                ...
                foreach($ordersArray as $nfdataObj) {
                    if (!empty($nfdataObj['meta_value'])) {
                        ...
                        if (!empty($workaroundArray['to_status'])) {
                            ...
                            // only process if we actually retrieved an order here
	...
	private function get_settings() {
        ...
        $voidrefund_modes  = [
            'WOOCOMMERCE' => 'WooCommerce Refund',
            'GATEWAY' => 'Legacy Direct Gateway',
        ];
        ...
        $verbosity_types = [
            Debug::VERBOSITY_LOW => 'LOW',
            Debug::VERBOSITY_MEDIUM => 'MEDIUM',
            Debug::VERBOSITY_HIGH => 'HIGH',
        ];
		...
		$settings = [
            ...
            [
                'title' => __('Debug Verbosity', 'nofraud-protection'),
                'type' => 'select',
                'class' => 'wc-enhanced-select',
                'desc' => __('Set the verbosity level for debug logs', 'nofraud-protection'),
                'options' => $verbosity_types,
                'id' => 'woocommerce_nofraud_debug_verbosity',
                'default' => Debug::VERBOSITY_MEDIUM,
            ],
            [
            ...
            [
                'title' => __('Void/Refund Mode', 'nofraud-protection'),
                'type' => 'select',
                'class' => 'wc-enhanced-select',
                'desc' => __('WooCommerce Refund will create a refund via WooCommerce methods, Legacy Direct Gateway will only submit an API request to the payment gateway', 'nofraud-protection'),
                'options' => $voidrefund_modes,
                'id' => 'woocommerce_nofraud_automatic_voidrefund_mode',
                'default' => "WOOCOMMERCE",
            ],
            [
        ...
        if(!empty(Gateways::NOFRAUD_SUPPORTED_GATEWAYS['acceptblue']['enabled'])) {
            $settings[] =
                [
                    'name' => __('accept.blue ' . $paymentGatewaysArray['acceptblue']['detectedStr'], 'nofraud-protection-section-payments'),
                    'type' => 'checkbox',
        ...
        if(!empty(Gateways::NOFRAUD_SUPPORTED_GATEWAYS['braintree']['enabled'])) {
            $settings[] =
                [
                    ...
                    'type' => 'checkbox',
                    'desc' => 'Check to enable NoFraud screening for Braintree<br /><br />Supported Braintree Plugins:<br /><a target="_blank" rel="nofollow" href="https://wordpress.org/plugins/woo-payment-gateway/">' . __('Payment Plugins Braintree For WooCommerce', 'nofraud-protection-section-payments') . '</a><br /><a target="_blank" rel="nofollow" href="https://wordpress.org/plugins/woocommerce-gateway-paypal-powered-by-braintree/">' . __('Braintree for WooCommerce Payment Gateway', 'nofraud-protection-section-payments') . '</a>',
        ...
        if(!empty(Gateways::NOFRAUD_SUPPORTED_GATEWAYS['cardknox']['enabled'])) {
            $settings[] =
                [
                    ...
                    'id' => Gateways::NOFRAUD_SUPPORTED_GATEWAYS['cardknox']['key'],
                    'default' => 'yes',
                ];
        }
        ...
        $settings[] =
            [
                ...
                'type' => 'checkbox',
                'desc' => 'Check to enable NoFraud screening for Square transactions<br /><br />Supported Square Plugins:<br /><a target="_blank" rel="nofollow" href="https://wordpress.org/plugins/woocommerce-square/">' . __('WooCommerce Square', 'nofraud-protection-section-payments') . '</a><br /><a target="_blank" rel="nofollow" href="https://wordpress.org/plugins/squarewoosync/">' . __('Square Sync for WooCommerce', 'nofraud-protection-section-payments') . '</a>',
        ...
        if(!empty(Gateways::NOFRAUD_SUPPORTED_GATEWAYS['usaepay']['enabled'])) {
            $settings[] =
                [
                    'name' => __('USA ePay ' . $paymentGatewaysArray['usaepay']['detectedStr'], 'nofraud-protection-section-payments'),
                    'type' => 'checkbox',
                    'desc' => 'Check to enable NoFraud screening for USA ePay<br /><br />Supported Paypal Plugins:<br /><a target="_blank" rel="nofollow" href="https://woocommerce.com/products/usa-epay/">' . __(' USA ePay for WooCommerce ', 'nofraud-protection-section-payments') . '</a>',
                    'id' => Gateways::NOFRAUD_SUPPORTED_GATEWAYS['usaepay']['key'],
                    'default' => 'yes',
                ];
        }
        ...
        $settings[] =
            [
                'name' => __('Enable NoFraud REST API', 'nofraud-protection-section-payments'),
                'type' => 'checkbox',
                'desc' => __('If checked, NoFraud API endpoints will be enabled. This is disabled by default.', 'nofraud-protection-section-payments'),
                'id' => 'woocommerce_nofraud_enable_rest_api',
                'default' => 'no',
            ];
    ...
    public function enqueue_nofraud_settings_script($hook) {
        // Only load on WooCommerce settings page
        if ($hook === 'woocommerce_page_wc-settings') {
            wp_enqueue_script(
                'nofraud-protection-settings-script',
