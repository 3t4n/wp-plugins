	...
	*/
    ...
    public function thankyou_evaluate_transaction( $order_id )
    {
        ...
        if (empty($_nofraud_transaction_review)) {
            ...
            // if USAePay
            if ($payment_method == 'usa_epay_credit_card' && Database::get_nf_data($order_id, '_wc_usa_epay_credit_card_charge_captured') == 'no') {
                $isAuthorizeTransaction = true;
            }
	 ...
	 * @since 2.0.0
	 */
	public function evaluate_transaction( $order_id, $options = [] ) {
        Debug::add_debug_message([
		...
		$transaction_review = $this->get_transaction_review($order_id, $options);
                ...
                if (!isset($transaction_review->override)) {
                    ...
                    if (isset($transaction_screening_result->code) && '200' === $transaction_screening_result->code) {
                        ...
                        if ('true' === $transaction_screening_result->override) {
                            ...
                            if (!empty($transaction_review->override_message)) {
	 ...
	 */
	public function get_transaction_review( $order_id, $options = [] ) {
        Debug::add_debug_message([
        ...
        // sanity check don't create a new review if existing review
        if (!empty($transaction_review->decision)) {
            Debug::add_debug_message([
                'function' => 'NoFraud:Transaction_Manager:get_transaction_review():review_already_exists',
                'order_id' => $order_id,
                'existing_transaction_review' => $transaction_review,
            ]);
            return $transaction_review;
        }

        // sanity check don't do a decision if order is older than TRANSACTION_REVIEW_REFRESH_LIMIT
        $maxDatetimeRefreshable = time() - self::TRANSACTION_REVIEW_REFRESH_LIMIT;

        // exception: if TRANSACTION_REVIEW_REFRESH_LIMIT is defined in options, overwrite
        if (!empty($options['TRANSACTION_REVIEW_REFRESH_LIMIT'])) {
            Debug::add_debug_message([
                'function' => 'NoFraud:Transaction_Manager:get_transaction_review():custom_refresh_limit',
                'order_id' => $order_id,
                'TRANSACTION_REVIEW_REFRESH_LIMIT' => $options['TRANSACTION_REVIEW_REFRESH_LIMIT'],
            ]);
            $maxDatetimeRefreshable = time() - $options['TRANSACTION_REVIEW_REFRESH_LIMIT'];
        }

        if (strtotime($order->get_date_created()) < $maxDatetimeRefreshable) {
            Debug::add_debug_message([
		...
		return $this->create_transaction_review($order_id, $options);
	}
     ...
     * @return stdObject|false NoFraud transaction review.
	 */
	private function create_transaction_review( $order_id, $options = [] ) {
        Debug::add_debug_message([
        ...
        if (!empty($options['TRANSACTION_REVIEW_REFRESH_LIMIT'])) {
            Debug::add_debug_message([
                'function' => 'NoFraud:Transaction_Manager:create_transaction_review():custom_refresh_limit',
                'order_id' => $order_id,
                'TRANSACTION_REVIEW_REFRESH_LIMIT' => $options['TRANSACTION_REVIEW_REFRESH_LIMIT'],
            ]);
            $maxDatetimeRefreshable = time() - $options['TRANSACTION_REVIEW_REFRESH_LIMIT'];
        ...
        // flag MARK_REPROCESSED option set, update TRANSACTION_STATUS_KEY
        if (!empty($options['MARK_REPROCESSED'])) {
            Debug::add_debug_message([
                'function' => 'NoFraud:Transaction_Manager:create_transaction_review():MARK_REPROCESSED',
                'order_id' => $order_id,
            ]);
            Database::update_nf_data($order_id,Constants::TRANSACTION_STATUS_KEY, Constants::TRANSACTION_STATUS_REPROCESS_EVALUATED);
        }
