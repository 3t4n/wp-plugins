...
final class NoFraud_Authorize_Net_Cim_Credit_Card extends NoFraud_Payment_Method {
    ...
    public function voidrefund( $order, $_wc_authorize_net_cim_credit_card_trans_id, $nofraud_payment_method ) {
        ...
        // determine which mode to use to process this voidrefund
        $woocommerce_nofraud_automatic_voidrefund_mode = get_option('woocommerce_nofraud_automatic_voidrefund_mode', 'WOOCOMMERCE');

        if ($woocommerce_nofraud_automatic_voidrefund_mode == 'GATEWAY') {
            // Legacy Payment Gateway Mode
            //retrieve transaction details by transaction ID
            $request = new AnetAPI\GetTransactionDetailsRequest();
            $request->setMerchantAuthentication($this->merchantAuthentication);
            ...
            if (in_array($transactionStatus, ['capturedPendingSettlement','authorizedPendingCapture'])) {
                ...
                if ($tresponse != null && $tresponse->getMessages() != null) {
                    ...
                    $order_note = sprintf(__('NoFraud automatically voided the charge for this order (Voided Transaction ID: %s).', 'nofraud-protection'), $_wc_authorize_net_cim_credit_card_trans_id);
            ...
            if (!empty($order_items)) {
                foreach($order_items as $item_id => $item) {
                    ...
                    $refund_tax = 0;
                    if(!empty($tax_data[0]) && is_array($tax_data[0])) {
                        $refund_tax = array_map('wc_format_decimal', $tax_data[0]);
                    }
                    $calculated_refund_amount = wc_format_decimal($calculated_refund_amount) + wc_format_decimal($item_meta['_line_total'][0]);
                    $line_items[$item_id] = [
            ...
            $refund = wc_create_refund(array(
                ...
                'reason' => 'NoFraud automatically voided/refunded the charge for this order due to FAIL decision',
                'order_id' => $order_id,
                'refund_payment' => true,
                'restock_items' => true,
            ...
            if (is_wp_error($refund)) {
                $order_note = sprintf(__('NoFraud attempted to automatically void/refund the charge for this order but failed due to error: %s', 'nofraud-protection'), $refund->get_error_message());
                wc_create_order_note($order_id, $order_note);
            } else {
                $order->update_meta_data( 'nofraud_voidrefund_processed', 1 );

                $order_note = sprintf(__('NoFraud automatically voided/refunded the charge for this order (Transaction ID: %s)', 'nofraud-protection'), $transaction_id);
                wc_create_order_note($order_id, $order_note);
            }
        }
    }
