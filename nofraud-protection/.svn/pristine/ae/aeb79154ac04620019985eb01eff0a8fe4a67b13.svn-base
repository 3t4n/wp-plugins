...
final class Environment {
     ...
     * @since 4.5.0
     */
    public static function get_debug_verbosity() {
        static $verbosity = 'unknown';

        if ('unknown' != $verbosity) {
            return $verbosity;
        }

        $verbosity = get_option('woocommerce_nofraud_debug_verbosity');

        switch($verbosity) {
            case Debug::VERBOSITY_LOW:
            case Debug::VERBOSITY_MEDIUM:
            case Debug::VERBOSITY_HIGH:
                break;
            default:
                $verbosity = Debug::VERBOSITY_MEDIUM;
                break;
        }
