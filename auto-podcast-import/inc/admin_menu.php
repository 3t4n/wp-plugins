...
class AdminMenu{
    ...
    public function __construct(){
          ...
          add_action('admin_menu', function(){
            ...
            \add_action ( 'wp_ajax_aupi_update_feed',function(){
                ...
                $data['feed_before_title'] = !empty($_POST['feed_before_title']) ?  \wp_kses_post($_POST['feed_before_title']) :'';
            ...
            \add_action ( 'wp_ajax_aupi_contact_the_developer',function(){
                ...
                $report = isset($_POST['report']) && \wp_kses_data($_POST['report']) == 'yes' ? true : false;

                $to = ['systems@r2k.co.il'];
                $subject = 'New inquiry from Auto podcast import plugin';
                $content .= '<table style="text-align: left; vertical-align: top;">';
                    $content .= '<tbody>';
                        $content .= '<tr><th>Website</th><td>'.\esc_html(home_url()).'</td></tr>';
                        $content .= '<tr><th>Email</th><td>'.$email.'</td></tr>';
                        $content .= '<tr><th>Message</th><td>'.\nl2br($message).'</td></tr>';
                        ...
                        if ($report) {
                            ...
                            $data = \WP_Debug_Data::format( $info, 'debug'); 
                            $content .= '<tr><th colspan="2">Technical report</th></tr>';
                            $content .= '<tr><td colspan="2">'.nl2br($data).'</td></tr>';
                        }
                ...
                if ($sent) {
                    echo \wp_json_encode(['error'=>false,'message'=>esc_html__('Message sent','aupi')]);
                } else {
                    echo \wp_json_encode(['error'=>true,'message'=>esc_html__('Server error','aupi'),'html'=>'']);
                }
                die();
    ...
    public static function admin_option(){
        ...
        echo '<div class="aupi_container">';
            echo '<div class="aupi_tabs">';
                ...
                echo '<a class="'.($currentTab == 'contact' ? 'current' : '').'" href="'.esc_attr($baseUrl).'&tab=contact">'.esc_html__( 'Contact the developer', 'aupi' ).'</a>';
    ...
    public static function addTab($id = false){
        ...
        if($id){
            $vals['feed_before_title']=\get_post_meta($id,'feed_before_title',true);
        ...
        echo '<form class="aupi_add">';
                ...
                echo '<table class="form-table" role="presentation">';
                    echo '<tbody>';
                        ...
                        echo '<tr>';
                        echo '<th>';
                            echo '<label for="feed_before_title">'.esc_html__( 'Text to add before podcast title', 'aupi' ).':</label>';
                        echo '</th>';
                        echo '<td>';
                            echo '<input   type="text" id="feed_before_title" name="feed_before_title" value="'.esc_attr($vals['feed_before_title']).'">';
                        echo '</td>';
                    echo '</tr>';
                ...
                echo '<div data-error="'.esc_attr__( 'Error, please try later.', 'aupi' ).'" data-ajaxing="'.esc_attr__( 'Saving settings, please wait!', 'aupi' ).'" class="aupi_ajaxing"></div>';
    ...
    public static function settings(){
        ...
        public static function contact(){
            ...
            echo '<h1>'.esc_html__( 'Contact the developer', 'aupi' ).'</h1>';
    
            echo '<form class="aupi_contact">';

                    wp_nonce_field('aupi_nonce','aupi_nonce');
                
                    echo '<input type="hidden" name="action" value="aupi_contact_the_developer" />';
                    
                    echo '<table class="form-table" role="presentation">';
                        echo '<tbody>';
                            echo '<tr>';
                                echo '<th>';
                                    echo '<label for="email">'.esc_html__( 'Email', 'aupi' ).':</label>';
                                echo '</th>';
                                echo '<td>';
                                    echo '<input size="60" type="email" id="email" name="email" value="" required>';
                                echo '</td>';
                            echo '</tr>';
                            echo '<tr>';
                                echo '<th>';
                                    echo '<label for="message">'.esc_html__( 'Message', 'aupi' ).':</label>';
                                echo '</th>';
                                echo '<td>';
                                    echo '<textarea id="message" name="message" value="" cols="60" rows="8" required></textarea>';
                                echo '</td>';
                            echo '</tr>';

                            if (version_compare(get_bloginfo( 'version' ), '5.8.0') != -1) {
                                echo '<tr>';
                                    echo '<th>';
                                        echo '<label for="report">'.esc_html__( 'Attach technical data', 'aupi' ).':</label>';
                                    echo '</th>';
                                    echo '<td>';
                                        echo '<div class="checkbox">';
                                            echo '<input type="checkbox" id="report" name="report" value="yes">';
                                            echo '<p class="description">'.esc_html__( 'Attach debug report to your inquiry', 'aupi' ).'</p>';
                                        echo '</div>';
                                        echo '<p class="description">'.sprintf(esc_html__( 'See included data %sHere%s', 'aupi' ), '<a href="'.admin_url('site-health.php?tab=debug').'" target="_blank">', '</a>').'.</p>';
                                    echo '</td>';
                                echo '</tr>';
                            }
                    ...
                    echo '<div data-error="'.esc_attr__( 'Error, please try later.', 'aupi' ).'"  data-saved="'.esc_attr__( 'Saved.', 'aupi' ).'" data-ajaxing="'.esc_attr__( 'Sending message, please wait!', 'aupi' ).'" class="aupi_ajaxing"></div>';
    
                    echo '<button class="button button-primary button-large">'.esc_html__( 'Send', 'aupi' ).'</button>';
