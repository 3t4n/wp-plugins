				...
				if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
					...
					foreach ( $options as $option_key => $option ) {
						...
						if ( isset( $option['text'] ) ) {
							$pro_only_value = isset( $option['only_pro'] ) ? $option['only_pro'] : '';
					...
					if ( isset( $args['preview'] ) && $args['preview'] ) {
						echo '<img src="' . esc_url( SPF_SMART_BRANDS::include_plugin_url( 'assets/images/navigation-position/' . $selected_value . '.svg' ) ) . '" class="icon_preview">';
						echo '<div class="sp_brand-pro-notice"></div>';
					}
				} else {
