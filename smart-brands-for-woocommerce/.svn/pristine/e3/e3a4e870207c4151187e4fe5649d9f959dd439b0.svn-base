...
use ShapedPlugin\SmartBrands\Admin\DBUpdates;
...
class Admin {
	...
	public function __construct( $plugin_slug, $version ) {
		...
		new Review_Notice();
		/**
		* Database updater class. It will run when the version is higher than the current.
		*/
		new DBUpdates();
	}
	...
	public function spsb_review_text( $text ) {
		...
		if ( 'smart_brand_sc' === $screen->post_type ) {
			...
			$text = sprintf(
				...
				__( 'Enjoying %1$sSmart Brands for WooCommerce?%2$s Please rate us %3$sWordPress.org%4$s. Your positive feedback will help us grow more. Thank you! 😊', 'smart-brands-for-woocommerce' ),
				'<strong>',
				'</strong>',
				'<span class="spsb-footer-text-star">★★★★★</span> <a href="' . esc_url( $url ) . '" target="_blank">',
				'</a>'
			);
		}
		return $text;
	...
	public function redirect_after_activation( $file ) {
		...
		if ( plugin_basename( SMART_BRANDS_FILE ) === $file && ! ( defined( 'DOING_AJAX' ) && DOING_AJAX ) && ( ! ( defined( 'WP_CLI' ) && WP_CLI ) ) ) {
