...
; (function ($, window, document, undefined) {
    ...
    return this.each(function () {
      ...
      $links.each(function (index) {
        ...
        $(this).on('click', function (e) {
          ...
          $section.removeClass('hidden');
          $section.csf_reload_script();
          SPF_SMART_BRANDS.helper.set_cookie(
            "csf-last-metabox-tab-" + post_id + "-" + unique_id,
            section_id
          );
        ...
        var get_cookie = SPF_SMART_BRANDS.helper.get_cookie(
          "csf-last-metabox-tab-" + post_id + "-" + unique_id
        );
    ...
    return this.each(function () {
      ...
      base.init = function () {
        ...
        if ($preview_block.length) {
          ...
          $this.on('change', SPF_SMART_BRANDS.helper.debounce(function (event) {
  ...
  $.fn.csf_field_tabbed = function () {
    return this.each(function () {
      ...
      $links.on('click', function (e) {
        ...
        SPF_SMART_BRANDS.helper.set_cookie('activeTabIndex', index);
      ...
      var activeTabIndex = SPF_SMART_BRANDS.helper.get_cookie('activeTabIndex');
  ...
  $.fn.csf_siblings = function () {
    return this.each(function () {
      ...
      var $this = $(this),
        $siblings = $this.find('.csf--sibling:not(.csf-pro-only)'),
    ...
    return this.each(function () {
      ...
      $this.on({
        mouseenter: function () {
          ...
          $tooltip = $('<div class="sp_brand-tooltip ' + $class + '"></div>').html($this.find('.csf-help-text').html()).appendTo('body');
  ...
  $(document).ready(function () {
    ...
    $('.sp_brands_shortcode_header_support').csf_help();
  });
  ...
  function updateNavPositionType(selector, regex, type) {
    var str = "";
    $(selector + ' option:selected').each(function () {
      str = $(this).val();
    });
    var src = $(selector + ' .csf-fieldset img').attr('src');
    var result = src.match(regex);
    if (result && result[1]) {
      src = src.replace(result[1], str);
      $(selector + ' .csf-fieldset img').attr('src', src);
    }
    if (type.includes(str)) {
      $(selector + ' .sp_brand-pro-notice').hide();
    } else {
      var noticeText = "This is a <a href='https://shapedplugin.com/smart-brands/?ref=1' target='_blank'>Pro Feature!</a>";
      $(selector + ' .sp_brand-pro-notice').html(noticeText).show();
    }
  }

  // Change navigation position according to nav position selection.
  $('.sp_brand-carousel-nav-position').on('change', function () {
    updateNavPositionType(".sp_brand-carousel-nav-position", /navigation-position\/(.+)\.svg/, 'top_right');
  });

  // Save the top right position as default on page load.
  if ($('.sp_brand-carousel-nav-position').length > 0) {
    updateNavPositionType(".sp_brand-carousel-nav-position", /navigation-position\/(.+)\.svg/, 'top_right');
  }
