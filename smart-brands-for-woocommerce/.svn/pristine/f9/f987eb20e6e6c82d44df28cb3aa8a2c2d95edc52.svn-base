...
if ( ! class_exists( 'SPF_SMART_BRANDS_Taxonomy_Options' ) ) {
	/**
	 *
	 ...
	 * @version 1.0.0
	 */
	class SPF_SMART_BRANDS_Taxonomy_Options extends SPF_SMART_BRANDS_Abstract {
		 ...
		 * Get default value.
		 *
		 * @param array $field get fields array.
		 * @return string
		 */
		 ...
		 * Get meta value.
		 *
		 * @param array $field get meta fields.
		 * @param int   $term_id get term id.
		 * @return statement
		 */
		...
		public function render_taxonomy_form_fields( $term ) {
			...
			foreach ( $this->sections as $section ) {
				...
				if ( $taxonomy === $this->taxonomy ) {
					...
					echo ( $section_title || $section_icon ) ? '<div class="csf-section-title"><h3>' . $section_icon . esc_html( $section_title ) . '</h3></div>' : '';//phpcs:ignore
					...
					if ( ! empty( $section['fields'] ) ) {
						foreach ( $section['fields'] as $field ) {
							...
							SPF_SMART_BRANDS::field( $field, $this->get_meta_value( $field, $term_id ), $this->unique, 'taxonomy' );
