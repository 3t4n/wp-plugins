 ...
 * @version 2.0.0
 */
if ( ! class_exists( 'SPF_SMART_BRANDS_Field_radio' ) ) {
	/**
	 *
	 * Field: select
	 *
	 ...
	 * @version 2.0.0
	 */
	class SPF_SMART_BRANDS_Field_radio extends SPF_SMART_BRANDS_Fields {
		/**
		 * Field constructor.
		 *
		...
		public function render() {
			...
			$inline_class = ( $args['inline'] ) ? 'csf--inline-list' : '';
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( is_array( $options ) && ! empty( $options ) ) {
					...
					echo ( ! empty( $this->field['label'] ) ) ? '<span class="spf-smart-brands--text">' . esc_attr( $this->field['label'] ) . '</span>' : '';
					echo '</label>';
			}
