...
if ( ! class_exists( 'SPF_SMART_BRANDS_Field_media' ) ) {
	/**
	 *
	 * Field: media
	 *
	 ...
	 * @version 1.0.0
	 */
	class SPF_SMART_BRANDS_Field_media extends SPF_SMART_BRANDS_Fields {
		...
		public function render() {
			...
			$args = wp_parse_args(
				...
				array(
					...
					'preview_height' => '',
					'library'        => array(),
					'button_title'   => esc_html__( 'Upload', 'smart-brands-for-woocommerce' ),
					'remove_title'   => esc_html__( 'Remove', 'smart-brands-for-woocommerce' ),
			...
			$placeholder = ( empty( $this->field['placeholder'] ) ) ? ' placeholder="' . esc_html__( 'Not selected', 'smart-brands-for-woocommerce' ) . '"' : '';
			...
			echo '<input type="text" name="' . esc_attr( $this->field_name( '[url]' ) ) . '" value="' . esc_attr( $this->value['url'] ) . '" class="csf--url' . esc_attr( $hidden_url ) . '" readonly="readonly"' . wp_kses_post( $this->field_attributes() ) . $placeholder . ' />'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '<a href="#" class="button button-primary csf--button" data-library="' . esc_attr( $library ) . '" data-preview-size="' . esc_attr( $args['preview_size'] ) . '">' . $args['button_title'] . '</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
