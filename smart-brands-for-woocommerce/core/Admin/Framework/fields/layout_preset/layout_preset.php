			...
			if ( ! empty( $args['options'] ) ) {
				...
				foreach ( $args['options'] as $key => $option ) {
					...
					if ( isset( $option['option_demo_url'] ) ) {
						echo '<p class="csf-layout-type">' . esc_html( $option['text'] ) . '<a href="' . esc_url( $option['option_demo_url'] ) . '" tooltip="Demo" class="csf-live-demo-icon" target="_blank"><i class="sp_brand-icon-external_link"></i></a></p>';
					}
					echo '</figure>';
					if ( isset( $option['text'] ) && ! isset( $option['option_demo_url'] ) ) {
						echo '<p class="csf-layout-type">' . $option['text'] . '</p>';//phpcs:ignore
					}
					echo '</div>';
				}
				echo '</div>';
