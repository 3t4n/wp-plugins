...
class Frontend {
		...
		if ( is_array( $found_generator_id ) ) {
			foreach ( $found_generator_id as $views_id ) {
				if ( $views_id && is_numeric( $views_id ) && get_post_status( $views_id ) !== 'trash' ) {
					...
					include 'partials/responsive-breakpoints.php';
				}
			}
		} else {
	...
	public function register_all_scripts() {
		...
		wp_register_script( 'smart_brand_script', SMART_BRANDS_FRONT . '/assets/js/script.min.js', array(), SMART_BRANDS_VERSION, true );
	}
	...
	public function render_shortcode( $attributes ) {
		...
		$views_meta         = get_post_meta( $views_id, 'sp_smart_brand_metaboxes', true );
		$layout_meta        = get_post_meta( $views_id, 'sp_smart_brand_metabox_layouts', true );
		...
		Manager::views_html( $views_id, $views_meta, $main_section_title, $layout_meta );
		return ob_get_clean();
	}
}
