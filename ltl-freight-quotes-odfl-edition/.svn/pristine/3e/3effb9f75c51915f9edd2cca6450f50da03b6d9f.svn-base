jQuery(document).ready(function () {
	const form = jQuery('#odfl_add_shipping_rule');
	if (form) {
		form.on('submit', enOdflSaveShippingRule);
	}

	enOdflToggleRuleTypeFields();

	// #region: Add Shipping Rule
	jQuery('#en_odfl_add_shipping_rule_btn').on('click', function () {
		form[0]?.reset();
	...
	jQuery('#en_odfl_sr_rule_type').on('load change', function (e) {
		enOdflToggleRuleTypeFields();
	});
	...
	jQuery(document).on('click', '.en_odfl_sr_edit_link', function () {
		const id = jQuery(this).attr('data-id');
		enOdflEditShippingRule(id, this);
	});
	jQuery(document).on('click', '.en_odfl_sr_delete_link', function () {
		const id = jQuery(this).attr('data-id');
		enOdflDeleteShippingRule(id, this);
	});
	jQuery(document).on('click', '.en_odfl_sr_status_link', function (e) {
		enOdflUpdateShippingRuleStatus(this);
	});
	...
	function enOdflSRValidateInput(form_id = '') {
		let is_valid = true;
		...
		if (jQuery('#en_odfl_sr_rule_type').val() === 'Override Rates') {
	...
	function enOdflSaveShippingRule(e) {
		e.preventDefault();

		const is_valid = enOdflSRValidateInput('#odfl_add_shipping_rule');
		if (!is_valid) {
		...
		const postData = {
			action: 'en_odfl_save_shipping_rule',
			rule_id: jQuery('#edit_sr_form_id').val(),
			name: jQuery('#en_sr_rule_name').val(),
			type: jQuery('#en_odfl_sr_rule_type').val(),
		...
		jQuery.ajax({
			...
			dataType: 'json',
			beforeSend: function () {
				jQuery('.en_odfl_save_shipping_rule_form').addClass('spinner_disable').val('Loading...');
			},
			success: function (data) {
				jQuery('.en_odfl_save_shipping_rule_form').removeClass('spinner_disable').val('Save');
				const rowMarkup = enOdflGetRowMarkup({ ...data, ...postData });
				jQuery('.en_odfl_empty_sr_row').hide();

				if (data.insert_qry == 1) {
					const newRowMarkup = `<tr id='sr_row_${data.id}'>${rowMarkup}</tr>`;
					jQuery('#en_odfl_shipping_rules_list tbody').append(newRowMarkup);
					window.location.href = jQuery('.close').attr('href');
					jQuery('.odfl_sr_created').show('slow').delay(3000).hide('slow');
				} else if (data.update_qry == 1) {
					jQuery(`#sr_row_${data.id}`).html(rowMarkup);
					window.location.href = jQuery('.close').attr('href');
					jQuery('.odfl_sr_updated').show('slow').delay(3000).hide('slow');
				} else if (data.error) {
					jQuery('.odfl_sr_already_exist').show('slow');
					jQuery('.content').delay(200).animate({ scrollTop: 0 }, 300);
					setTimeout(function () {
						jQuery('.odfl_sr_already_exist').hide('slow');
					}, 3000);
				}
			},
	...
	if (typeof enOdflEditShippingRule != 'function') {
		function enOdflEditShippingRule(rule_id, elem) {
			form[0]?.reset();
			jQuery('.en_sr_err').html('');
			setDataAttribute(
				'en_sr_weight_from',
			...
			const postForm = {
				action: 'en_odfl_edit_shipping_rule',
				edit_id: rule_id,
			};
			...
			jQuery.ajax({
				...
				beforeSend: function () {
					...
					jQuery(elem).css(
						'background',
						`rgba(255, 255, 255, 1) url("${en_odfl_admin_script.plugins_url}/ltl-freight-quotes-odfl-edition/warehouse-dropship/wild/assets/images/processing.gif") no-repeat scroll 50% 50%`
					);
				},
				success: function (data) {
					...
					if (data) {
						...
						jQuery('#en_odfl_sr_rule_type').val(data?.rule_data?.type);
						...
						// Show popup modal
						window.location.href = jQuery('.en_odfl_add_sr_btn').attr('href');
						setTimeout(function () {
							if (jQuery('.en_odfl_add_sr_popup').is(':visible')) {
	...
	if (typeof enOdflDeleteShippingRule != 'function') {
		function enOdflDeleteShippingRule(id, elem) {
			const postForm = {
				action: 'en_odfl_delete_shipping_rule',
				delete_id: id,
			};
			...
			jQuery.ajax({
				...
				success: function (data) {
					...
					jQuery('.odfl_sr_deleted').show('slow').delay(3000).hide('slow');
					if (jQuery('.en_odfl_empty_sr_row')?.length) return false;

					if (jQuery('.en_odfl_empty_sr_row')?.length) {
						jQuery('.en_odfl_empty_sr_row').show();
					} else {
						const newRowMarkup = `<tr class="new_warehouse_add en_odfl_empty_sr_row" data-id=0><td class="en_wd_warehouse_list_data" colspan="5" style="text-align: center;">No data found!</td></tr>`;
						jQuery('#en_odfl_shipping_rules_list tbody').append(newRowMarkup);
					}
				},
				error: function (error) {},
	...
	function enOdflUpdateShippingRuleStatus(elem) {
		const rule_id = jQuery(elem).attr('data-id');
		const is_active = jQuery(elem).attr('data-status');

		// Submit the form to save settings
		const postData = {
			action: 'en_odfl_update_shipping_rule_status',
			rule_id,
	...
	function enOdflGetRowMarkup(rule) {
		const rowMarkup = `
				...
				<td class="en_wd_warehouse_list_data"><a href="#" class='en_odfl_sr_status_link' data-id='${
					rule?.id
				}' data-status='${rule?.is_active}'>${rule?.is_active ? 'Yes' : 'No'}</a></td>
				<td class="en_wd_warehouse_list_data">
					<!-- Edit rule link -->
					<a href="#" class="en_odfl_sr_edit_link" data-id="${rule?.id}">
						<img src="${
							en_odfl_admin_script.plugins_url
						}/ltl-freight-quotes-odfl-edition/warehouse-dropship/wild/assets/images/edit.png" title="Edit">
					</a>
					<!-- Delete rule link -->
					<a href="#" class="en_odfl_sr_delete_link" data-id="${rule?.id}">
						<img src="${
							en_odfl_admin_script.plugins_url
						}/ltl-freight-quotes-odfl-edition/warehouse-dropship/wild/assets/images/delete.png" title="Delete">
					</a>
				</td>
		`;
	...
	function enOdflToggleRuleTypeFields() {
		const type = jQuery('#en_odfl_sr_rule_type').val();
		jQuery('#en_odfl_sr_content').css('height', '80vh');
		...
		if (type === 'Hide Methods') {
			...
			jQuery('#odfl_add_shipping_rule').css('overflow', 'hidden');
			jQuery('#en_odfl_sr_content').css('height', '450px');
