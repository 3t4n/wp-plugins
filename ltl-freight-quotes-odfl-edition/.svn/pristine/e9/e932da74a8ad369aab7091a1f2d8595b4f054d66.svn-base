...
if (!class_exists("Odfl_Liftgate_As_Option")) {
    class Odfl_Liftgate_As_Option
    {
        ...
        {
            $this->odfl_as_option = get_option("odfl_quotes_liftgate_delivery_as_option");
            $this->label_sfx_arr = [];
        }
         ...
         * @return array string
         */
        public function odfl_update_carrier_service($post_data)
        {
            if (isset($this->odfl_as_option) && ($this->odfl_as_option == "yes")) {
         ...
         * @return array type
         */
        public function update_parse_odfl_output($surcharges)
        {
            $surcharge_amount = [];
         ...
         * @return array type
         */
        public function update_rate_whn_as_option_odfl($rate)
        {
            if (isset($rate) && (!empty($rate))) {
                ...
                $rate = apply_filters("en_woo_addons_web_quotes", $rate, en_woo_plugin_odfl_quotes);

                $label_sufex = (isset($rate['label_sufex'])) ? $rate['label_sufex'] : [];
                $label_sufex = $this->label_R_odfl($label_sufex);
                $rate['label_sufex'] = $label_sufex;

                if (isset($this->odfl_as_option, $rate['grandTotalWdoutLiftGate']) &&
                    ($this->odfl_as_option == "yes") && ($rate['grandTotalWdoutLiftGate'] > 0)) {
         ...
         * @return array type
         */
        public function filter_label_sufex_array_odfl($result)
        {
         ...
         * @return array type
         */
        public function label_R_odfl($label_sufex)
        {
