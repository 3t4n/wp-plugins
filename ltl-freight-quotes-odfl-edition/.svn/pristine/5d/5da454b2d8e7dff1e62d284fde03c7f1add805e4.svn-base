...
if (!class_exists('EnOdflLfqLogs')) {
    class EnOdflLfqLogs
    {
        ...
        {
            $obj_classs = new \ODFL_Curl_Request();
            $data = array(
                'serverName' => odfl_quotes_get_domain(),
                'licenseKey' => get_option('wc_settings_odfl_plugin_licence_key'),
                'lastLogs' => '25',
                'carrierName' => 'odfl4me',
            );
            ...
            $url = ODFL_FREIGHT_DOMAIN_HITTING_URL . '/request-log/index.php';
            $logs = $obj_classs->odfl_get_curl_response($url, $data);
            ...
            if (isset($logs['severity'], $logs['data']) && $logs['severity'] == 'SUCCESS') {
                ...
                foreach ($logs['data'] as $key => $shipment) {
                    ...
                    $class_name = 'odfl-log-' . $key . rand(1, 100);
