...
function odfl_hide_shipping($available_methods)
{
    ...
    foreach ($available_methods as $value) {
        ...
        if ($value->method_id == 'odfl' || strpos($value->id, 'odfl') !== false) {
    ...
    // Remove backup Rates
    $plugin_rates = get_option('odfl_backup_rates_display') == 'no_plugin_rates' && $plugin_rates_available;
    $other_rates = (empty(get_option('odfl_backup_rates_display')) || get_option('odfl_backup_rates_display') == 'no_other_rates') && count($available_methods) > 1;
    if (get_option('enable_backup_rates_odfl') == 'yes' && ($plugin_rates || $other_rates)) {
        $backup_rate_id = 'odfl:backup_rates';
 ...
 */
if (!function_exists("odfl_no_method_available")) {

    function odfl_no_method_available()
    {
