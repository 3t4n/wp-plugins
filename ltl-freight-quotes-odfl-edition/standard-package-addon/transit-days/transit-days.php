...
if (!class_exists("EnFedExSmallTransitDays")) {
    ...
    class EnFedExSmallTransitDays{
         ...
         */
        public function odfl_quotes_enable_disable_ups_ground($result)
        {
          
            $transit_day_type   =   get_option('restrict_radio_btn_transit_odfl_small');
            $response           =   (isset($result->q)) ? $result->q : [];
            $days_to_restrict   =   get_option('ground_transit_wwe_small_packages');
            
            $package = apply_filters('odfl_quotes_quotes_plans_suscription_and_features' , 'transit_days');
        ...
        public function odfl_quotes_disable_service_ground($response)
        {     
            $transit_day_type   =  get_option('restrict_radio_btn_transit_odfl_small'); //get value of check box to see which one is checked 
            $days_to_restrict   =   get_option('restrict_days_transit_package_odfl_small');
            $transit_days = apply_filters('odfl_quotes_quotes_plans_suscription_and_features' , 'transit_days');
