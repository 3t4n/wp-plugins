...
if (!class_exists("EnOdflShippingRulesAjaxReq")) {

    class EnOdflShippingRulesAjaxReq
    {
        /**
        ...
        {
            add_action('wp_ajax_nopriv_en_odfl_save_shipping_rule', array($this, 'en_odfl_save_shipping_rule_ajax'));
            add_action('wp_ajax_en_odfl_save_shipping_rule', array($this, 'en_odfl_save_shipping_rule_ajax'));

            add_action('wp_ajax_nopriv_en_odfl_edit_shipping_rule', array($this, 'en_odfl_edit_shipping_rule_ajax'));
            add_action('wp_ajax_en_odfl_edit_shipping_rule', array($this, 'en_odfl_edit_shipping_rule_ajax'));

            add_action('wp_ajax_nopriv_en_odfl_delete_shipping_rule', array($this, 'en_odfl_delete_shipping_rule_ajax'));
            add_action('wp_ajax_en_odfl_delete_shipping_rule', array($this, 'en_odfl_delete_shipping_rule_ajax'));

            add_action('wp_ajax_nopriv_en_odfl_update_shipping_rule_status', array($this, 'en_odfl_update_shipping_rule_status_ajax'));
            add_action('wp_ajax_en_odfl_update_shipping_rule_status', array($this, 'en_odfl_update_shipping_rule_status_ajax'));
        }
         ...
         * @global $wpdb
         */
        function en_odfl_save_shipping_rule_ajax()
        {
            global $wpdb;
            ...
            $qry = "SELECT * FROM " . $wpdb->prefix . "eniture_odfl_shipping_rules WHERE name = '" . $data['name'] . "'"; 
            ...
            if (!empty($get_shipping_rule_id)) {
                ...
                $update_qry = $wpdb->update(
                    $wpdb->prefix . 'eniture_odfl_shipping_rules', $data, array('id' => $get_shipping_rule_id)
                );
                ...
                if (!empty($get_shipping_rule)) {
                    ...
                    $data['settings'] = json_encode($data['settings']);
                    $insert_qry = $wpdb->insert($wpdb->prefix . 'eniture_odfl_shipping_rules', $data);
         ...
         * @global $wpdb
         */
        function en_odfl_edit_shipping_rule_ajax()
        {
            global $wpdb;
            $get_shipping_rule_id = (isset($_POST['edit_id']) && intval($_POST['edit_id'])) ? $_POST['edit_id'] : "";
            $shipping_rules_list = $wpdb->get_results(
                "SELECT * FROM " . $wpdb->prefix . "eniture_odfl_shipping_rules WHERE id=$get_shipping_rule_id"
            );
            $product_tags_markup = $this->en_odfl_get_product_tags_markup($shipping_rules_list);
         ...
         * @global $wpdb
         */
        function en_odfl_delete_shipping_rule_ajax()
        {
            global $wpdb;
            $get_shipping_rule_id = (isset($_POST['delete_id']) && intval($_POST['delete_id'])) ? $_POST['delete_id'] : "";
            $qry = $wpdb->delete($wpdb->prefix . 'eniture_odfl_shipping_rules', array('id' => $get_shipping_rule_id));
         ...
         * @global $wpdb
         */
        function en_odfl_update_shipping_rule_status_ajax()
        {
            global $wpdb;
         ...
         * @global $wpdb
         */
        function en_odfl_get_product_tags_markup($shipping_rules_list)
        {
            $tags_options = '';
            ...
            $selected_tags_detials = $this->en_odfl_get_selected_tags_details($tags_data['filter_by_product_tag_value']);
         ...
         * @global $wpdb
         */
        function en_odfl_get_selected_tags_details($products_tags_arr)
        {
         ...
         *
         * @param array $odfl_package request to get quotes
         ...
         */
        function apply_shipping_rules($odfl_package, $apply_on_rates = false, $rates = [], $loc_id = '')
        {
            if (empty($odfl_package)) return $apply_on_rates ? $rates : false;

            global $wpdb;
            $qry = "SELECT * FROM " . $wpdb->prefix . "eniture_odfl_shipping_rules"; 
            ...
            foreach ($rules as $rule) {
                ...
                if ($rule_type == 'Override Rates' && $apply_on_rates) {
                    $rates = $this->apply_override_rates_rule($odfl_package, $settings, $rates, $loc_id);
                } else if ($rule_type == 'Hide Methods') {
                    $is_rule_applied = $this->apply_hide_methods_rule($settings, $odfl_package);
         ...
         * @param array $odfl_package The package to apply the rule to.
         * @return bool Whether the rule was applied or not.
        */
        function apply_hide_methods_rule($settings, $odfl_package)
        {
            $is_rule_applied = false;

            if ($settings['apply_to'] == 'cart') {
                $formatted_values = $this->get_formatted_values($odfl_package);
                $is_rule_applied = $this->apply_rule_filters($settings, $formatted_values);
            } else {
                foreach ($odfl_package as $key => $pkg) {
                    $is_rule_applied = false;
                    $shipments = [];
                    $shipments[$key] = $pkg;
         ...
         *
         * @param array $odfl_package request array to get the quotes
         * @param array $settings rule settings
         * @param object $rates quotes object
         * @return $rates The updated rates.
         */
        function apply_override_rates_rule($odfl_package, $settings, $rates, $loc_id)
        {
        ...
        function get_updated_rates($rates, $settings)
        {
            ...
            } elseif ($service_type == 'hat_service') {
                $rates = $this->get_hat_service_rates($rates, $service_rate);
            }
        ...
        function apply_rule_filters($settings, $formatted_values)
        {
            ...
            $filters = ['country', 'weight', 'price', 'quantity'];
            ...
            foreach ($filters as $filter) {
                if (filter_var($settings['filter_by_' . $filter], FILTER_VALIDATE_BOOLEAN)) {
                    if ($filter == 'country') {
                        ...
                        $destination_address = $odfl_get_shipping_quotes->destinationAddressOdfl();
                        $destination_country = isset($destination_address['country']) ? $destination_address['country'] : '';
                        $is_filter_applied = $this->is_country_matched($settings['filter_by_country_value'], $destination_country);
                    } else {
        ...
        {
            $filters_checks = ['country', 'weight', 'price', 'quantity', 'product_tag'];
        ...
        {
            $services_rates = [];
            if (isset($rates->soapenvBody->ns2getLTLRateEstimateResponse->return) && $rates->soapenvBody->ns2getLTLRateEstimateResponse->return->success == true && empty($rates->soapenvBody->ns2getLTLRateEstimateResponse->return->errorMessages) && !empty($rates->soapenvBody->ns2getLTLRateEstimateResponse->return->rateEstimate->netFreightCharge)) {
                $services_rates = $rates->soapenvBody->ns2getLTLRateEstimateResponse->return;
            }

            if (!empty($services_rates)) {
                $surcharges = isset($services_rates->surcharges) ? $services_rates->surcharges : [];
                if (isset($services_rates->rateEstimate->netFreightCharge)) {
                    $services_rates->rateEstimate->netFreightCharge = floatval($service_rate) + $this->get_resi_and_lfg_fees($surcharges);
                    $services_rates->overrideRates = true;
                }

                isset($rates->soapenvBody->ns2getLTLRateEstimateResponse->return) ?
                $rates->soapenvBody->ns2getLTLRateEstimateResponse->return = $services_rates : null;
            }
            ...
            if (!empty($services_rates)) {
                ...
                if (!empty($surcharges)) {
                    ...
                    foreach ($surcharges_types as $s_type) {
                        if (isset($surcharges->$s_type) && $s_type == $type) {
                            $surcharges_fee = $surcharges->$s_type;
                            $services_rates->surcharges->$s_type = $service_rate;
                ...
                $rate_charge = isset($services_rates->rateEstimate->netFreightCharge) ? $services_rates->rateEstimate->netFreightCharge : 0;
                $rate_charge = floatval($rate_charge) - floatval($surcharges_fee);
                $is_surcharge_exist ? $rate_charge += floatval($service_rate) : null;

                if (isset($services_rates->rateEstimate->netFreightCharge)) {
                    $services_rates->rateEstimate->netFreightCharge = $rate_charge;
                    $services_rates->overrideRates = true;
                }

                isset($rates->soapenvBody->ns2getLTLRateEstimateResponse->return) ? $rates->soapenvBody->ns2getLTLRateEstimateResponse->return = $services_rates : null;
            }
        ...
        function get_resi_and_lfg_fees($surcharges)
        {
            ...
            foreach ($surcharges_types as $s_charge) {
                if (isset($surcharges->$s_charge) && $surcharges->$s_charge > 0) {
        ...
        function get_hat_service_rates($rates, $service_rate) 
        {
            if (isset($rates->holdAtTerminalResponse, $rates->holdAtTerminalResponse->totalNetCharge)) {
                $rates->holdAtTerminalResponse->totalNetCharge = floatval($service_rate);
                $rates->holdAtTerminalResponse->overrideRates = true;
            }
...
new EnOdflShippingRulesAjaxReq();
