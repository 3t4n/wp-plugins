class ChatBot {
  constructor() {
    ...
    this.agentStatusText = this.main.querySelector(
      "#agent-header-status .agent-header-status-text"
    );
    ...
    this.buttonShortcode = document.querySelectorAll(
      ".chat-agent-shortcode-button"
    );
    this.chatIcon = document.querySelector(".chat-agent-icon");

    // Get configurable variables from WordPress settings
    this.agentId = agent700_config.agentId;
    this.apiUrl = "https://app.agent700.ai/api/chat";
    this.isVisible = false;
    this.overlayOption = agent700_config.hideOverlay
    this.layout_type = agent700_config.layout_type
  ...
  set showChat(value) {
    ...
    if (value) {
      ...
      if (this.overlayOption === "" || (this.overlayOption === "1" && this.layout_type === "aside")) {
