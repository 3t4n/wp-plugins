(function( $ ) {
	...
	$( document ).ready( function() {
		...
		var countdown_timers = $( '.enba-countdown-timer' );
		if ( $( countdown_timers ).length > 0 ) {
			$( countdown_timers ).each( function( index, value ) {
				var unique_id = 'enba-uid-' + Math.floor( Math.random() * 99999 );
				//var unique_id = 'enba-countdown-timer ' + Math.floor( Math.random() * 99999 );
				var seconds = $( value ).find( 'span.enba-seconds-section' ).text();
				var format = $( value ).find( 'span.enba-countdown-format' ).html();
				var complete = $( 'h3.enba-countdown-complete' ).show();

				// Wrap the timer in a span with a unique id so we can refer to it
				// in the timer update code.
				$( value ).wrap( $( '<div>' ).attr( 'id', unique_id ).addClass( 'enba-timer-one' ));
			    //$( value ).wrap( $( '<div>' ).attr( 'id', unique_id ).addClass( 'enba-countdown-timer' ) );
