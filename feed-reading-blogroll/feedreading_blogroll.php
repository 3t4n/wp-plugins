...
define('FEEDREADING_VERSION', '1.5.9.2');
...
function feedreading_blogroll_setOptions() {
	...
	if (feedreading_blogroll_vars_are_set()) {
		...
		$feedreading_blogroll_settings['googleAPIKey']      = trim(strip_tags(stripslashes($_POST['googleAPIKey'])));
		...
		<form id="feedreading_options_form" action="" method="post"><?php
			...
			<table class="form-table" summary="<?php _e('Options','feedreading_blogroll'); ?>">
				<tr>
					...
					<td><input size="90" class="" id="googleAPIKey" name="googleAPIKey" type="text"
						value="<?php echo $feedreading_blogroll_settings['googleAPIKey']; ?>" title="<?php _e('This field is optional!','feedreading_blogroll')?>" />
					</td>
				</tr>
				<tr>
...
jQuery(function($) {
	if ($('.feedreading_blogroll_bookmarklist').length > 0){
		$.getScript("http://www.google.com/jsapi<?php if (!empty($googleAPIKey)){echo "?key=".$googleAPIKey;} ?>", function(){
			google.load("feeds", "1",{"callback" : initializeBR });
		});
	}

	//function run(){google.setOnLoadCallback(initializeBR);};
	//initializeBR();
				...
				?>;<?php
                    ...
                    foreach ((array)$bookmarks as $bookmark){
                        ...
                        if($("#feedreading_bookmark_<?php echo $bookmark->link_id;?>").length > 0){
                        	feed<?php echo $bookmark->link_id;?>.setNumEntries(1);
					...
					<?php
						foreach((array) $widget_keys as $key){
									...
									foreach ((array) $widgetbookmarks as $bookmark){
										...
										feed_single<?php echo $key.$bookmark->link_id;?>.setNumEntries(1);
...
<?php
										...
										<?php } else if ($feedDiscovery==0) { ?>
											...
											feed_single_<?php echo $key.$bookmark->link_id;?>.setNumEntries(1);
