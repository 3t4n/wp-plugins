...
Contributors: CCBill
Tags: CCBill, payment, gateway, for WooCommerce
...
*Note: Version 2.0.0 includes breaking changes.*  When upgrading an existing installation, please make note of your CCBill account values in the WooCommerce -> Settings -> Payments -> CCBill section of the Wordpress admin, as these values will need to be reentered after upgrading.  Version 2.0.0 adds support for Blocks checkout for the latest themes.
...
This plugin requires WooCommerce.  This plugin is not owned by, developed by, or otherwise affiliated with WooCommerce.

== External Services ==

This plugin connects to api.ccbill.com to collect payment information during checkout in order to process payment for the order.  When a customer places an order in your WooCommerce store and selects CCBill as the payment method, the customer is then directed to a form hosted by CCBill where payment information is securely entered and processed.  The customer is then directed back to your WooCommerce website where the result of the payment (success or failure) is reflected in the order.  All payment information is processed by CCBill and no payment information is stored by or transmitted through your WooCommerce website for a secure checkout experience.  This service is provided by "CCBill, LLC": [terms of use](https://ccbill.com/html_forms/terms_and_conditions-english.html), [Privacy Policy](https://ccbill.com/cs/acf/html/privacy_policy-english.html).
