...
$data = array(
	...
	'modals'=>array(
		...
		'core-anonymize_user_agent'=>array(
			...
			sprintf(
				__('Apocalypse Meow automatically anonymizes that and other leaky request headers to help bring WordPress into compliance with its %s damn %s. Haha.', 'apocalypse-meow'),
			...
			sprintf(
				 __('If for some reason you actually %s to share all that unnecessary data with random-ass third parties, simply disable this option.', 'apocalypse-meow'),
				sprintf('<em>%s</em>', __('want', 'apocalypse-meow')),
			),
		),
			...
			<div id="post-body" class="metabox-holder meow-columns one-two fixed fluid">
				...
				<div class="postbox-container two">
					...
					<div class="meow-fluid-tile">
						<div class="postbox">
							...
							<div class="inside">
								...
								<div class="meow-fieldset inline">
									<label for="login-lockdown_limit"><?php echo \__('Lockdown Limit', 'apocalypse-meow'); ?></label>

									<span class="dashicons dashicons-editor-help meow-info-toggle" v-bind:class="{'is-active' : modal === 'login-lockdown_limit'}" v-on:click.prevent="toggleModal('login-lockdown_limit')"></span>

									<input type="number" id="login-lockdown_limit" v-model.number="forms.settings.login.lockdown_limit" min="0" max="100" step="1" readonly required />
								</div>
					...
					<div class="meow-fluid-tile">
						<div class="postbox">
							...
							<div class="inside">
								...
								<div class="meow-fieldset inline">
									<label for="core-anonymize_user_agent">
										<input type="checkbox" id="core-anonymize_user_agent" v-model.number="forms.settings.core.anonymize_user_agent" v-bind:true-value="1" v-bind:false-value="0" v-bind:disabled="readonly.indexOf('core-anonymize_user_agent') !== -1" />
										<?php echo \__('Anonymize User Agent', 'apocalypse-meow'); ?>
									</label>

									<span class="dashicons dashicons-editor-help meow-info-toggle" v-bind:class="{'is-active' : modal === 'core-anonymize_user_agent'}" v-on:click.prevent="toggleModal('core-anonymize_user_agent')"></span>
								</div>
					...
					<div class="meow-fluid-tile">
						<div class="postbox">
							<h3 class="hndle">
								<?php echo \__('Response Headers', 'apocalypse-meow'); ?>
								<span class="dashicons dashicons-editor-help meow-info-toggle" v-bind:class="{'is-active' : modal === 'response'}" v-on:click.prevent="toggleModal('response')"></span>
							</h3>
							<div class="inside">
