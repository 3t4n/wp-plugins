...
class core {
	...
	public static function init() {
		...
		if ($settings['core']['anonymize_user_agent']) {
			\add_filter('http_request_args', array(static::class, 'core_anonymize_user_agent'), 500, 1);
		}
	...
	/**
	 * Anonymize User Agent Header.
	 *
	 * @param array $parsed_args Parsed arguments.
	 * @return array Parsed arguments.
	 */
	public static function core_anonymize_user_agent($parsed_args) {
		// Fix the user agent.
		if (isset($parsed_args['user-agent'])) {
			$parsed_args['user-agent'] = 'WordPress/0';
		}

		// Also remove these two non-standard headers WordPress adds as a
		// fallback in case they can't get their Big Data the usual way. Haha.
