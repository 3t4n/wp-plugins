...
class login {
	...
	/**
	 * Is Login Locked Down?
	 *
	 * Returns true if new login requests will be blocked.
	 *
	 * @return bool Locked.
	 */
	static function is_login_lockdown() {
		global $wpdb;

		// No limit, no lockdown!
		$limit = options::get('login-lockdown_limit');
		if (! $limit) {
			return false;
		}

		// Keep it open for whitelisted and returning customers.
		if (false !== ($ip = static::get_visitor_ip())) {
			// Check the whitelist first.
			if (static::is_listed($ip, options::get('login-whitelist'))) {
				return false;
			}

			// Look for valid logins.
			if ($ip === $wpdb->get_var("
				SELECT `ip`
		...
		// Otherwise it depends on how many different networks have failed
		// during the past minute.
		$window = \current_time('mysql');
		$count = $wpdb->get_var("
			SELECT COUNT(DISTINCT `ip`)
			FROM `{$wpdb->prefix}meow2_log`
			WHERE
				`type`='fail' AND
				`date_created` >= DATE_SUB('$window', INTERVAL 1 MINUTE)
		");
	...
	/**
	 * Login Lockdown Message.
	 *
	 * @param WP_Error $errors Errors or null.
	 * @return WP_Error Errors or null.
	 */
	public static function login_lockdown_message($errors) {
		if (
			...
			// And lastly, check we are actually in lockdown!
			static::is_login_lockdown()
		) {
			...
			$errors->add(
				'meow_login_lockdown',
				'<strong>' . \__('NOTE:', 'apocalypse-meow') . '</strong> ' . \__('New logins are temporarily disabled. Please try again in a few minutes.', 'apocalypse-meow'),
			);
		}
