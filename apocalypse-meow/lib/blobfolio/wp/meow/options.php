...
class options {
	...
	const OPTIONS = array(
		// Core settings.
		'core'=>array(
			'anonymize_user_agent'=>true, // Anonymize user agent.
		...
		'login'=>array(
			...
			'lockdown_limit'=>0,		// Global max fails.
	...
	public static function load($refresh=false) {
		// Define MEOW_LOGIN_LOCKDOWN_LIMIT if missing; it cannot be set
		// through WordPress.
		if (! \defined('MEOW_LOGIN_LOCKDOWN_LIMIT')) {
			\define('MEOW_LOGIN_LOCKDOWN_LIMIT', 0);
		}
	...
	protected static function sanitize(&$options) {
		...
		common\ref\sanitize::to_range($options['login']['lockdown_limit'], 0, 100);
