...
(int) **MEOW_LOGIN_LOCKDOWN_LIMIT**  
Lock down the login form for _everyone_ whenever this many distinct IPs have submitted bad credentials within the past minute. To disable, set to `0`.
...
(bool) **MEOW_CORE_ANONYMIZE_USER_AGENT**
Anonymize the `user-agent` header WordPress adds to all remote requests to prevent the core version and site URL from being shared with random third parties.
