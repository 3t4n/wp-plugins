 ...
 * @return string : url
 */
function cf7_preview_url($pathname = '')
{
	return plugins_url($pathname, cf7_preview_plugin_index());
}
...
{
	return dirname(cf7_preview_plugin_index()) . '/' . ltrim($path, '/');
}
...
{
	if ($path_file != '' && file_exists($file = cf7_preview_path('includes/' . $path_file))) {
		include($file);
