...
function cf7_preview_filter_wpcf7_form_hidden_fields($hidden_fields = array())
{
	if(is_array($hidden_fields)){
		...
		$post_id = isset($data['cf7-preview']) ? intval($data['cf7-preview']) : 0;
		if ($post_id > 0) {
			$hidden_fields['_wpcf7_preview_id'] = $post_id;
		}
	}
...
function cf7_preview_filter_contact_form_properties($properties = array())
{
	if (is_array($properties)) {
		...
		if ($post_id == 0) {
			...
			$post_id = isset($data_post['_wpcf7_preview_id']) ? intval($data_post['_wpcf7_preview_id']) : 0;
			if ($post_id > 0) {
				$_POST['_wpcf7_id'] = $data_post['_wpcf7_preview_id'];
			}
		}
...
function cf7_preview_form_site()
{
	...
	$content = isset($data['content']) ? format_to_edit($data['content'], true) : '';
