...
function cf7_preview_tab()
{
	...
	$post_id = isset($data['post']) ? intval($data['post']) : 0;
	...
	if ($post_id > 0) {
		...
		$form = str_replace($attr = 'name="', 'data-' . $attr, $form);
...
function cf7_preview_plugin_row_meta($plugin_meta = array(), $plugin_file = '')
{
	if ($plugin_file == plugin_basename(cf7_preview_plugin_index())) {
		$plugin_meta[] = sprintf('<a class="dashicons-before dashicons-awards" href="%s" target="_blank">%s</a>', cf7_preview_donate_url(), cf7_preview_text('Donate'));
	}
