...
class Genealogical_Tree_Admin {
    ...
    public function init_post_type_and_taxonomy() {
        $this->plugin->helper::post_type_and_taxonomy();
    }
    ...
    /**
     * Adds meta boxes to the 'gt-member' post type.
     *
     * This function dynamically adds various meta boxes to the 'gt-member' post type,
     * including basic, premium, and debug-specific meta boxes based on the plugin's
     * settings and environment. Premium features and debug tools are conditionally 
     * displayed if certain conditions are met.
     *
    ...
    public function add_meta_boxes_gt_member( $post_type, $post ) {
        ...
        // Add the basic Member Info meta box.
        add_meta_box(
            'genealogical-tree-meta-box-member-info',
            __( 'Member Info', 'genealogical-tree' ),
            [$this, 'render_meta_box_member_info'],
            'gt-member',
            'normal',
            'high'
        );
        // Add premium meta boxes if the premium plan or trial is active.
        if ( gt_fs()->is__premium_only() && (gt_fs()->is_plan( 'pro' ) || gt_fs()->is_trial()) ) {
            // Add Contact Information meta box.
            add_meta_box(
                'genealogical-tree-meta-box-member-contact',
                __( 'Contact Information', 'genealogical-tree' ),
                [$this, 'render_meta_box_member_contact__premium_only'],
                'gt-member',
                'normal',
                'high'
            );
            // Add Member Notes meta box.
            add_meta_box(
                'genealogical-tree-meta-box-member-notes',
                __( 'Member Notes', 'genealogical-tree' ),
                [$this, 'render_meta_box_member_notes__premium_only'],
                'gt-member',
                'normal',
                'high'
            );
            // Add Additional Member Fields meta box.
            add_meta_box(
                'genealogical-tree-meta-box-member-additional-field',
                __( 'Additional Member Fields', 'genealogical-tree' ),
                [$this, 'render_meta_box_member_additional_field__premium_only'],
                'gt-member',
                'normal',
                'high'
            );
            // Add Additional Member Info meta box.
            add_meta_box(
                'genealogical-tree-meta-box-member-additional-info',
                __( 'Additional Member Info', 'genealogical-tree' ),
                [$this, 'render_meta_box_member_additional_info__premium_only'],
                'gt-member',
                'normal',
                'high'
            );
            // Add Member Gallery meta box (side placement).
            add_meta_box(
                'genealogical-tree-meta-box-member-gallery',
                __( 'Member Gallery', 'genealogical-tree' ),
                [$this, 'render_meta_box_member_gallery__premium_only'],
                'gt-member',
                'side',
                'high'
            );
        }
        // Add a debug meta box if debugging is enabled.
        if ( defined( 'GENEALOGICAL_TREE_DEBUG' ) && true === GENEALOGICAL_TREE_DEBUG ) {
            add_meta_box(
                ...
                [$this, 'render_meta_box_member_debug'],
                'gt-member',
                'normal',
                'high'
            );
    ...
    /**
     * Adds a meta box to the 'gt-family' post type.
     *
     * This function conditionally adds a debug meta box to the 'gt-family' post type.
     * The meta box is only displayed if debugging is enabled in the plugin settings.
     *
     * @param string $post_type The slug of the current post type.
     * @param WP_Post $post The current post object.
     *
    ...
    /**
     * Adds meta boxes to the 'gt-tree' post type.
     *
     * This function adds the "Tree Settings" meta box to the 'gt-tree' post type
     * by default. Additionally, it conditionally adds a debug meta box when 
     * debugging mode is enabled in the plugin settings.
     *
     * @param string  $post_type The slug of the current post type.
     * @param WP_Post $post      The current post object.
     *
    ...
    public function add_meta_boxes_gt_tree( $post_type, $post ) {
        ...
        if ( defined( 'GENEALOGICAL_TREE_DEBUG' ) && true === GENEALOGICAL_TREE_DEBUG ) {
            add_meta_box(
                ...
                __( 'Family Info', 'genealogical-tree' ),
                [$this, 'render_meta_box_tree_debug'],
                'gt-tree',
                'normal',
                'high'
            );
    ...
    public function update_meta_boxes_gt_member( $post_id ) {
        ...
        */
        ...
        foreach ( $spouses as $key => $spouse ) {
            ...
            if ( isset( $spouse['id'] ) && $spouse['id'] || !empty( $chil ) ) {
    ...
    function search_members_ajax() {
    	...
    	foreach ($categories as $category_name => $members) {
    		if (empty($members)) {
    			...
    			$formatted_data[] = array(
    				'text' => $category_name,
    				'children' => array(array('id' => '', 'text' => 'No ' . strtolower($category_name) . ' found'))
    			);
    		} else {
    			...
    			$formatted_data[] = array(
    				...
    				'children' => array_map(function($id) {
    					return array('id' => $id, 'text' => get_the_title($id));
    				}, $members)
    			);
    		}
    	}
