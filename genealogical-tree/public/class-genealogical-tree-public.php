...
class Genealogical_Tree_Public {
    ...
    /**
     * Shortcode to display a list of family tree roots based on a family group.
     *
     * This shortcode generates links to different tree roots for a specified family group,
     * categorized into single, dual, and alone members. Each root links to the tree page
     * with a query parameter indicating the root ID.
     *
     * @param array       $atts    Shortcode attributes.
     * @param string|null $content Shortcode content (not used).
     * 
     * @return string The HTML output for the tree list or an error message.
     */
    public function gt_tree_list_shortcode( $atts, $content = null ) {
        // Parse and merge default attributes.
        $data = shortcode_atts( array(
            'family' => null,
        ), $atts );
        // Ensure the 'family' attribute is provided.
        if ( !$data['family'] ) {
            return esc_html__( 'No data', 'genealogical-tree' );
        }
        // Sanitize and validate the family group ID.
        $family_group_id = absint( $data['family'] );
        if ( !term_exists( $family_group_id, 'gt-family-group' ) ) {
            return esc_html__( 'Invalid family group', 'genealogical-tree' );
        }
        // Retrieve the associated tree page for the family group.
        $post_id = absint( get_term_meta( $family_group_id, 'tree_page', true ) );
        if ( !$post_id ) {
            return esc_html__( 'Tree page not found', 'genealogical-tree' );
        }
        // Get the tree root data based on the family group ID.
        $tree_root = $this->get_root_by_family_group( $family_group_id );
        // Check if there are any valid roots to display.
        if ( empty( $tree_root['single'] ) && empty( $tree_root['dual'] ) && empty( $tree_root['alone'] ) ) {
		...
		<div class="gt-tree-list-pub">
			<?php 
        // Render single-member tree roots.
        if ( !empty( $tree_root['single'] ) ) {
            $this->render_tree_links( $post_id, $tree_root['single'] );
        }
        // Render dual-member tree roots.
        if ( !empty( $tree_root['dual'] ) ) {
            $this->render_tree_links( $post_id, $tree_root['dual'], ' and ' );
        }
        // Render single (alone) members with a heading.
        if ( !empty( $tree_root['alone'] ) ) {
            ?>
				<h4><?php 
            echo esc_html__( 'Alone', 'genealogical-tree' );
            ?></h4>
				<?php 
            $this->render_tree_links( $post_id, $tree_root['alone'] );
        }
        ?>
		</div>
		<?php 
        ...
        $output = ob_get_clean();
        return apply_filters( 'genealogical_tree_list_shortcode_output', $output, $atts );
    }

    /**
     * Helper function to render tree links for given members.
     *
     * @param int    $post_id   The tree page ID to link to.
     * @param array  $members   Array of member IDs or pairs of IDs.
     * @param string $separator Separator used between titles for dual members.
     * 
     * @return void
     */
    private function render_tree_links( $post_id, $members, $separator = ' and ' ) {
        foreach ( $members as $member ) {
            // Ensure the member is treated as an array.
            $ids = (array) $member;
            // Retrieve titles for all IDs in the member array.
            $titles = array_map( 'get_the_title', $ids );
            // Output each member link.
            ?>
			<div>
				<?php 
            foreach ( $ids as $index => $id ) {
                ?>
					<a href="<?php 
                echo esc_url( get_the_permalink( $post_id ) . '?root=' . esc_attr( $id ) );
                ?>" loading="lazy">
						<span style="font-size: smaller; color: gray;"><?php 
                echo esc_html( $id );
                ?>:</span> <?php 
                echo esc_html( $titles[$index] );
                ?>
					</a>
					<?php 
