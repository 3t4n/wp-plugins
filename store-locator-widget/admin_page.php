...
function storelocatorwidget_settings_page() {
  ...
  wp_enqueue_style('storlocatorwidget_wp_admin_styles', $styles, '', 1.0);
  wp_enqueue_style('storlocatorwidget_wp_admin_semantic', $semantic, '', 1.0);

?>

<div class="wrap ui segment">

<img src="<?php echo esc_url(plugins_url( 'images/logo.png', __FILE__ ) ) // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>" width="300">	
	<hr>
...
<?php
	// 	if (isset($_REQUEST['admin_post_nonce_field']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['admin_post_nonce_field'])), 'admin_post_action')) echo '<div class="updated"><p>Settings updated successfully.</p></div>';
	// if (isset($_GET['settings-saved']) && !empty(sanitize_text_field(wp_unslash($_GET['settings-saved'])))) echo '<div class="updated"><p>Settings updated successfully.</p></div>';
	if ( isset( $_GET['nonce'] ) && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['nonce'])), 'settings-saved-nonce' )) echo '<div class="updated"><p>Settings updated successfully.</p></div>';
	// if ( isset( $_GET[ 'settings-saved' ] )) 
?>
<form method="post" action="admin-post.php">
	<?php wp_nonce_field('admin_post_action', 'admin_post_nonce_field'); ?>
    ...
    <table class="form-table">
        <tr valign="top">
          ...
          <td><input id="storelocatorwidget_api_box" type="text" name="storelocatorwidget_api_key" class="ui input" size="36" value="<?php echo esc_html($api) ?>" /></td>
        </tr>

        <tr valign="top">
          <th scope="row">Google Maps API key</th>
          <td><input type="text" name="google_api_key" class="ui input" size="45" value="<?php echo esc_html($gapi) ?>" /></td>
        </tr>
