...
Text Domain: store-locator-widget
*/
...
function storelocatorwidget_install () {
   ...
   // $installed_table_name = $wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $table_name));

  if( $installed_table_name != $table_name || $installed_ver != STORELOCATORWIDGET_DB_VERSION ) {
...
function storelocatorwidget_handler($incomingfrompost) {
  ...
  wp_enqueue_script( 'storelocatorwidget_google_maps_js', '//maps.googleapis.com/maps/api/js?key=' . $gapi . '&libraries=places', '', 1.0, true );
  wp_enqueue_script( 'storelocatorwidget_locator_js', '//cdn.storelocatorwidgets.com/widget/widget.js', '', 1.0, true );

  add_filter( 'script_loader_tag', 'storelocatorwidget_script_loader_tag', 10 ,2 );

  $incomingfrompost = shortcode_atts(array("headingstart" => $script_text), esc_html($incomingfrompost));
...
function storelocatorwidget_script_output($incomingfromhandler) {
  ...
  for ($demolp_count = 1; $demolp_count <= $categorylist; $demolp_count++) {
    ...
    $demolp_output .= " of ";
    $demolp_output .= esc_html($categorylist);
...
function storelocatorwidget_create_menu() {
  ...
  add_menu_page('StoreLocatorWidget Settings', 'Store Locator', 'administrator', 'storelocatorwidgets_slug', 'storelocatorwidget_settings_page',plugins_url('/images/icon.png', __FILE__));
}
