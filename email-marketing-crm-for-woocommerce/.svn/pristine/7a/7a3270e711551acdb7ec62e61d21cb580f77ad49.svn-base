...
class WC_AC_Hook_Sync {
	...
	public function __construct($options) {
		if (!is_numeric ($options['ac_list_id'])) {
			$this->log_message[] = sprintf( __( 'Error: List ID is not numeric = %s', 'woo-repupress-mail-crm' ), $options['ac_list_id']);
		}
		else {
			...
			if (!(int)$this->activecampaign_connection->credentials_test()) {
				$this->log_message[] = sprintf( __( 'Error: Invalid Active Campaign credentials, URL = %1$s, API Key = %2$s', 'woo-repupress-mail-crm' ), $options['ac_url'], $options['ac_api_key']);
			}
	...
	public function sync_contact($contact) {
		if ($this->api_valid) {
			...
			if ((int)$contact_sync->success) {
				...
				$this->log_message[] = sprintf( __( 'Contact synced successfully (ActiveCampaign ID = %s)', 'woo-repupress-mail-crm' ), $contact_id);
			}
			else $this->log_message[] = sprintf( __( 'Syncing contact failed. Error returned: %s)', 'woo-repupress-mail-crm' ), $contact_sync->error);
		}
	}
	...
	public function remove_tags($contact) {
		if ($this->api_valid) {
			...
			if (!(int)$tag_remove->success) 
				$this->log_message[] = sprintf( __( 'Remove tags on contact failed. Error returned: %s)', 'woo-repupress-mail-crm' ), $tag_remove->error);
		}		
	}
