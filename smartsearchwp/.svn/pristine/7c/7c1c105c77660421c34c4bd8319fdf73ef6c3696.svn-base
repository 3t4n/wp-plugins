...
function wdgpt_is_api_key_set() {
    ...
    if(WDGPT_DEBUG_MODE && empty($api_key)){
        WDGPT_Error_Logs::wdgpt_log_error( 'API key is empty', 200, 'wdgpt_is_api_key_set');
    }
...
function wdgpt_should_display() {
    ...
    if ( WDGPT_DEBUG_MODE ) {
        WDGPT_Error_Logs::wdgpt_log_error( 'Enable bot : '.$enable_bot, 200, 'wdgpt_should_display');
        WDGPT_Error_Logs::wdgpt_log_error( 'Active posts : '.count($active_posts) .' (Must be greater than 0)', 200, 'wdgpt_should_display');
        WDGPT_Error_Logs::wdgpt_log_error( 'Pending update : '.$has_pending_database_update, 200, 'wdgpt_should_display');
        WDGPT_Error_Logs::wdgpt_log_error( 'OPENAI API down : ' . ( wdgpt_is_api_down() ? 'YES' : 'NO' ), 200, 'wdgpt_should_display' );
        WDGPT_Error_Logs::wdgpt_log_error('allow_url_fopen enabled: ' . (ini_get('allow_url_fopen') ? 'YES' : 'NO'), 200, 'wdgpt_debug');
    }
