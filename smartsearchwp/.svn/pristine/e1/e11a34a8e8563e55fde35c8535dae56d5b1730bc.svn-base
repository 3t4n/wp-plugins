...
class WDGPT_Summaries_Table extends WP_List_Table {
	...
	public function column_title( $item ) {
        ...
        // Récupérer le statut du post
        $post = get_post($item['post_id']);
        $private_label = '';
        if ($post->post_status === 'private') {
            $private_label = ' <span style="color: #666;">— ' . __('Private', 'webdigit-chatbot') . '</span> ';
        }

		$permalink = get_permalink( $item['post_id'] );

		$item_title_with_permalink = sprintf(
            '%s%s <a href="%s" target="_blank"><i class="fa-solid fa-arrow-up-right-from-square"></i></a>',
			$item_title ?? $item['post_title'],
            $private_label,  // Ajout du label privé
			$permalink
		);
    ...
    public function column_cb($item) {
        return sprintf(
            '<input type="checkbox" name="summaries[]" value="%s" />',
            $item['post_id']
        );
    }
    ...
    public function get_bulk_actions() {
        return array(
            ...
            'generate_embeddings' => __('Generate Embeddings', 'webdigit-chatbot')
        );
    }
    ...
    public function process_bulk_action() {
        ...
        $post_ids = isset($_REQUEST['summaries']) ? array_map('intval', $_REQUEST['summaries']) : array();
        ...
        switch ($action) {
            case 'activate':
                foreach ($post_ids as $post_id) {
                    ...
                    $embeddings = get_post_meta($post_id, 'wdgpt_embeddings', true);
                    if ($embeddings) {
                        update_post_meta($post_id, 'wdgpt_is_active', 'true');
                        $processed++;
                    } else {
                        $skipped++;
                    }
                ...
                $message = sprintf(
                    __('%d items activated. %d items skipped (no embeddings).', 'webdigit-chatbot'),
                    $processed,
                    $skipped
                );
                break;
            ...
            case 'deactivate':
                foreach ($post_ids as $post_id) {
                    // Vérifier si le contexte est actuellement actif
                    $is_active = get_post_meta($post_id, 'wdgpt_is_active', true);
                    if ($is_active === 'true') {
                        update_post_meta($post_id, 'wdgpt_is_active', 'false');
                        $processed++;
                    } else {
                        $skipped++;
                    }
                ...
                $message = sprintf(
                    __('%d items deactivated. %d items skipped (not active).', 'webdigit-chatbot'),
                    $processed,
                    $skipped
                );
                break;
            ...
            case 'generate_embeddings':
                foreach ($post_ids as $post_id) {
                    // Vérifions si les embeddings existent déjà
                    $embeddings_exist = get_post_meta($post_id, 'wdgpt_embeddings', true);
                    if (!$embeddings_exist) {
                        try {
                            // Appel à l'API endpoint existant
                            $response = wp_remote_post(
                                get_rest_url(null, 'wdgpt/v1/save-embeddings/'),
                                array(
                            ...
                            if (is_wp_error($response)) {
                                if ( WDGPT_DEBUG_MODE ) {
                                    WDGPT_Error_Logs::wdgpt_log_error('Error generating embeddings for post ' . $post_id . ': ' . $response->get_error_message(),200, 'process_bulk_action');
                                }
                                $skipped++;
                                continue;
                            }
                            ...
                            if ( WDGPT_DEBUG_MODE ) {
                                WDGPT_Error_Logs::wdgpt_log_error('Exception when generating embeddings: ' . $e->getMessage(),200, 'process_bulk_action');
                            }
                            $skipped++;
                        }
                    } else {
                ...
                $message = sprintf(
                    __('Embeddings generation started for %d items. %d items skipped.', 'webdigit-chatbot'),
                    $processed,
                    $skipped
                );
                break;
