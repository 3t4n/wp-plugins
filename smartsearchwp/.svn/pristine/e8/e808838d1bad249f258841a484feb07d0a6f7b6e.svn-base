jQuery(document).ready(function($) {
    ...
    $verifyPremiumLicense.on('click', function(event) {
        if (1 === $verifyPremiumLicense.data('expired')) {
            window.open('https://www.smartsearchwp.com/product/license-premium/', '_blank');
        }else{
            event.preventDefault();
...
document.addEventListener("DOMContentLoaded", function() {
    const toggleBtn = document.getElementById("toggle_license_visibility");
    ...
    if (toggleBtn && licenseInput) {
        toggleBtn.addEventListener("click", function() {
            if (licenseInput.type === "password") {
                ...
                toggleBtn.innerHTML = '<i class="dashicons dashicons-hidden"></i>';
            } else {
                licenseInput.type = "password";
                toggleBtn.innerHTML = '<i class="dashicons dashicons-visibility"></i>';
            }
        });
    }
});