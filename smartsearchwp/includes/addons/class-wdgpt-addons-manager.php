...
class WDGPT_Addons_Manager {
	...
	public function retrieve_addons() {
		/*Fix performance issue while controlling the addons licence*/
		// Vérifier d'abord si on a un cache transient
		$cached_addons = get_transient( 'wdgpt_addons_cache' );
        ...
        if ($addons) {
            // Mettre le résultat dans le cache pendant 12 heures
            set_transient( 'wdgpt_addons_cache', $addons, 12 * HOUR_IN_SECONDS );
        }
	...
	public function install_addon( $addon, $action = 'install' ) {
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Start install addon', 200, 'install_addon');
            WDGPT_Error_Logs::wdgpt_log_error('Action received: ' . $action, 200, 'install_addon');
            WDGPT_Error_Logs::wdgpt_log_error('Addon received: ' . print_r($addon, true), 200, 'install_addon');
        }
		$action_query = '';
		switch ( $action ) {
			case 'install':
			...
			case 'update':
				...
				break;
            default:
                // Les seules actions possibles sont 'install' et 'update'
                // Si nous arrivons ici, c'est une erreur inattendue
                $action_query = '';
                break;
		}
		$id              = $addon['id'];
        ...
        // Vérification de l'URL de téléchargement
        $response = wp_remote_get($source);
        if (is_wp_error($response)) {
            $error_message = "Erreur lors du téléchargement de l'addon: " . $response->get_error_message();

            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::get_instance()->insert_error_log($error_message, 0, 'addon_install_error');
            }
        ...
        $body = wp_remote_retrieve_body($response);
        if (strpos($body, 'license key is expired') !== false) {
            $error_message = "Échec de l'installation : Clé de licence expirée.";

            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::get_instance()->insert_error_log($error_message, 0, 'addon_install_error');
            }

            //Supprime le cache pour éviter d'utiliser une licence expirée
        ...
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Installation result : ' . print_r($result, true), 200, 'install_addon');
        }

		if ( ! $result || is_wp_error( $result ) ) {
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Installation failure : ' . (is_wp_error($result) ? $result->get_error_message() : 'Raison inconnue'), 200, 'install_addon');
            }
			return false;
		}
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Installation success, trying to activate : ' . $activation_slug, 200, 'install_addon');
        }
		$result = activate_plugin( $activation_slug );
		if ( is_wp_error( $result ) ) {
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Activation failure : ' . $result->get_error_message(), 200, 'install_addon');
            }
			return false;
		}
