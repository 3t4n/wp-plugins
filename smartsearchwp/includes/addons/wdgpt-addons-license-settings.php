...
function wdgpt_license_section_callback() {
    ...
    // Récupérer la date d'expiration de la licence premium
			...
			<table class="form-table">
				<tr>
					<td colspan="2">
						...
						if ( 'active' === $premium_license_key['status'] ) {
							...
							$status_icon    = 'times';
                            $status_message = $expiry_date
                                ? sprintf( __( 'Your SmartSearchWP Premium license expired on %s. Please renew it to continue using the premium features.', 'webdigit-chatbot' ), $expiry_date )
                                : __( 'Your SmartSearchWP Premium license has expired. Please renew it to continue using the premium features.', 'webdigit-chatbot' );
						}
						?>
				...
				<tr valign="top">
					...
					<td>
                        ...
                        <div style="position: relative; display: inline-block;">
                            <input type="password" name="wd_smartsearch_license" id="license_key" style="width: 270px;" value="<?php echo esc_attr( get_option( 'wd_smartsearch_license' ) ); ?>" />
                            <span id="toggle_license_visibility" style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); cursor: pointer;">
				...
				<tr>
					...
					<td>
						...
						<?php
                            if ($license_data && isset($license_data->state) && 'expired' === $license_data->state) {
                                esc_html_e(
                                    'Renew your premium license',
                                    'webdigit-chatbot'
                                );
                            }else{
                                esc_html_e(
						...
						?>
						"
                        data-expired="<?php echo ($license_data && isset($license_data->state) && 'expired' === $license_data->state) ? '1' : '0'; ?>"
