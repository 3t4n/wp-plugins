 ...
 * Amended 2025-01-25 by Robert Grygier in order to avoid 0 being replaced by 1
 *
...
private function wdgpt_remove_accents($str) {
    ...
    // Call iconv for removal of accents.
    $output = @iconv('UTF-8', 'ASCII//TRANSLIT', $str);

    // Fallback in case iconv not available.
    if ($output === false) {
        $accents = 'ÀÁÂÃÄÅàáâãäåÒÓÔÕÖòóôõöÈÉÊËèéêëÇçÌÍÎÏìíîïÙÚÛÜùúûüÿÑñ';
        $accents_out = 'AAAAAAaaaaaaOOOOOOooooooEEEEeeeeCcIIIIiiiiUUUUuuuuyNn';
        $output = str_replace(mb_str_split($accents), mb_str_split($accents_out), $str);
    }
