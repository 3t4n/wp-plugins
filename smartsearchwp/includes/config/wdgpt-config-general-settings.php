...
function wdgpt_general_settings_save_options() {
    ...
    wdgpt_general_settings_save_option( 'wdgpt_debug_mode');
...
function wdgpt_settings_section_callback() {
	...
	?>
        ...
        ?>
        <div style="margin: 20px 0; color: #1d2327; font-size: 14px; font-weight: 600;">
            <?php echo esc_html('Version : ' . WDGPT_CHATBOT_VERSION); ?>
        </div>
		...
		<tr vertical-align="top">
			...
			<td>
                ...
                <p class="description">
				<?php
					//esc_html_e('If you want to use gpt-4o, you must subscribe to chatgpt API. The model gpt4-o is a better version of gpt-4, with faster speed and lower cost.','webdigit-chatbot');
				?>
				</p>
				-->
			</td>
        ...
        <tr>
            <th scope="row">
                <?php esc_html_e('Activate Debug mode', 'webdigit-chatbot'); ?>
            </th>
            <td>
                <label class="switch">
                    <input type="checkbox" name="wdgpt_debug_mode" id="wdgpt_debug_mode" <?php checked(get_option('wdgpt_debug_mode', false), 'on'); ?>>
                    <span class="slider round"></span>
                </label>
                <p class="description">
                    <?php
                    printf(
                        esc_html__('If enabled, all plugin debug logs will be recorded in the %s. This slows down the plugin => Leave disabled for production!', 'webdigit-chatbot'),
                        '<a href="' . esc_url(admin_url('admin.php?page=wdgpt_error_logs')) . '" target="_blank">' . esc_html__('error log', 'webdigit-chatbot') . '</a>'
                    );
                    ?>
                </p>
            </td>
