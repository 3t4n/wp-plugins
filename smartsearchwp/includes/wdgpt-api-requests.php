...
function wdgpt_save_embeddings( $request ) {
    ...
    if ( WDGPT_DEBUG_MODE ) {
        WDGPT_Error_Logs::wdgpt_log_error('Count tokens: ' . $count_tokens, 200, 'wdgpt_save_embeddings');
    }
    $splited_content = [];

    // Si le contexte est trop long, on le divise en contextes plus petits
    if($count_tokens > 4000){
        $split_context = ceil($count_tokens/4000);
        $split_size = intval(strlen($text) / $split_context + 10);
		$splited_content = str_split( $text, $split_size );
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs ::wdgpt_log_error('Text split into '.count($splited_content).' parts',200,'wdgpt_save_embeddings');
        }
    }

	if (!empty($splited_content)) {
		$embeding_splitted = [];
        // On traite chaque morceau séparément
		foreach ( $splited_content as $index => $text_part ) {
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Processing part ' . ($index + 1), 200, 'wdgpt_save_embeddings');
            }
			$generated_embedings = $answer_generator->wdgpt_retrieve_topic_embedding($text_part);

            // Si on a une erreur de quota, on attend un peu et on réessaie
            if ($generated_embedings === '__INSUFFICIENT_QUOTA__') {
                if ( WDGPT_DEBUG_MODE ) {
                    WDGPT_Error_Logs::wdgpt_log_error('Quota error, waiting 1 second before retry...', 200, 'wdgpt_save_embeddings');
                }
                sleep(1); // Attendre 1 seconde
                $generated_embedings = $answer_generator->wdgpt_retrieve_topic_embedding($text_part);
            }
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Generated embeddings type: ' . gettype($generated_embedings), 200, 'wdgpt_save_embeddings');
                WDGPT_Error_Logs::wdgpt_log_error('Generated embeddings content: ' . print_r($generated_embedings, true), 200, 'wdgpt_save_embeddings');
            }

            // Vérifions si $generated_embedings est un tableau
            if (is_array($generated_embedings) && !empty($generated_embedings)) {
                // Si ce sont des embeddings valides, on les stocke directement
                $embeding_splitted[] = $generated_embedings;
                if ( WDGPT_DEBUG_MODE ) {
                    WDGPT_Error_Logs::wdgpt_log_error('Added embeddings to array. Current count: ' . count($embeding_splitted), 200, 'wdgpt_save_embeddings');
                }
            } else {
                if ( WDGPT_DEBUG_MODE ) {
                    WDGPT_Error_Logs ::wdgpt_log_error('Failed to generate embeddings for part '.($index + 1),200,'wdgpt_save_embeddings');
                }
            }
		}
        // On fusionne tous les embeddings générés
        if (!empty($embeding_splitted)) {
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Merging ' . count($embeding_splitted) . ' embedding arrays', 200, 'wdgpt_save_embeddings');
            }
            if (count($embeding_splitted) === 1) {
                $embeddings_array = $embeding_splitted[0];
            } else {
                // Sinon, on crée un tableau final qui contiendra tous les embeddings
                $embeddings_array = [];
                foreach ($embeding_splitted as $embedding) {
                    // On s'assure que chaque embedding est bien un tableau
                    if (is_array($embedding)) {
                        $embeddings_array = array_merge($embeddings_array, $embedding);
                    }
                }
            }
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Final embeddings array count: ' . (is_array($embeddings_array) ? count($embeddings_array) : 'not an array'), 200, 'wdgpt_save_embeddings');
            }
        } else {
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('No valid embeddings generated', 200, 'wdgpt_save_embeddings');
            }
            $embeddings_array = [];
            return array(
                'success' => false,
                'date' => current_time('mysql'),
                'message' => 'Erreur lors de la génération des embeddings. Veuillez réessayer dans quelques instants.'
            );
        }
	} else {
        // Si le texte n'est pas trop long, on le traite directement
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Text is not too long, processing entire text at once', 200, 'wdgpt_save_embeddings');
        }
		$embeddings_array = $answer_generator->wdgpt_retrieve_topic_embedding($text);

        if ($embeddings_array === '__INSUFFICIENT_QUOTA__') {
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Quota error for single text, waiting 1 second before retry...', 200, 'wdgpt_save_embeddings');
            }
            sleep(1);
            $embeddings_array = $answer_generator->wdgpt_retrieve_topic_embedding($text);
        }

        if (!is_array($embeddings_array)) {
            return array(
                'success' => false,
                'date' => current_time('mysql'),
                'message' => 'Erreur lors de la génération des embeddings. Veuillez réessayer dans quelques instants.'
        ...
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs ::wdgpt_log_error('Direct embeddings type: '.gettype($embeddings_array),200,'wdgpt_save_embeddings');
            WDGPT_Error_Logs ::wdgpt_log_error('Direct embeddings content: '.print_r($embeddings_array,true),200,'wdgpt_save_embeddings');
        }
	}

    if ( WDGPT_DEBUG_MODE ) {
        WDGPT_Error_Logs::wdgpt_log_error('Final embeddings before save: ' . print_r($embeddings_array, true), 200, 'wdgpt_save_embeddings');
    }

    if (!empty($embeddings_array)) {
        $result = update_post_meta( $post_id, 'wdgpt_embeddings', $embeddings_array );
        $date = current_time( 'mysql' );
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Update post meta result: ' . var_export($result, true), 200, 'wdgpt_save_embeddings');
        }
        ...
        return array(
            ...
            'embeddings_count' => is_array($embeddings_array) ? count($embeddings_array) : 0,
            'total_tokens' => $count_tokens,
            'parts_count' => count($splited_content)
        );
    }else {
        return array(
            'success' => false,
...
function wdgpt_verify_license_ajax_handler() {
    ...
    // Suppression des transients AVANT la vérification de la licence
    delete_option('_transient_wdgpt_license_verification');
    delete_option('_transient_wdgpt_license_transient');
    delete_option('_transient_timeout_wdgpt_license_verification');
    delete_option('_transient_timeout_wdgpt_license_transient');

    if (WDGPT_DEBUG_MODE) {
        WDGPT_Error_Logs::wdgpt_log_error("License verification - Transients cleared via AJAX request.", 200, 'wdgpt_verify_license');
    }
...
function wdgpt_install_addon() {
    if ( WDGPT_DEBUG_MODE ) {
        WDGPT_Error_Logs::wdgpt_log_error('Start addon installation', 200, 'wdgpt_install_addon');
    }
    try{
        // Vérification nonce
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Checking the nonce', 200, 'wdgpt_install_addon');
        }
        check_ajax_referer('wdgpt_install_addon_nonce', 'security');
        if (!isset($_POST['id'])) {
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Error: ID missing in POST', 200, 'wdgpt_install_addon');
            }
            wp_send_json_error('No ID provided');
        }
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('ID received: ' . $_POST['id'], 200, 'wdgpt_install_addon');
        }
        // Récupération de l'addon
        $addons_manager = WDGPT_Addons_Manager::instance();
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Creating Addons Manager instance', 200, 'wdgpt_install_addon');
        }
        $id = sanitize_text_field(wp_unslash($_POST['id']));
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Cleaned ID: ' . $id, 200, 'wdgpt_install_addon');
        }
        $addon = $addons_manager->retrieve_addon($id);
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Addon retrieved: ' . print_r($addon, true), 200, 'wdgpt_install_addon');
        }
        if (!$addon) {
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Error: Addon not found', 200, 'wdgpt_install_addon');
            }
            wp_send_json_error('Addon not found');
        }
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Starting installation via Addons Manager', 200, 'wdgpt_install_addon');
        }
        $result = $addons_manager->install_addon($addon);
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Installation result: ' . print_r($result, true), 200, 'wdgpt_install_addon');
        }
        if ($result) {
        ...
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Exception during installation: ' . $e->getMessage(), 200, 'wdgpt_install_addon');
            WDGPT_Error_Logs::wdgpt_log_error('Trace: ' . $e->getTraceAsString(), 200, 'wdgpt_install_addon');
        }
...
function wdgpt_update_addon() {
    if ( WDGPT_DEBUG_MODE ) {
        WDGPT_Error_Logs::wdgpt_log_error('Start addon update', 200, 'wdgpt_update_addon');
    }
    try {
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Checking the nonce', 200, 'wdgpt_update_addon');
        }
        check_ajax_referer( 'wdgpt_update_addon_nonce', 'security' );
        if ( ! isset( $_POST['id'] ) ) {
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Error: ID missing', 200, 'wdgpt_update_addon');
            }
            wp_send_json_error( 'No ID provided' );
        }
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('ID received: ' . $_POST['id'], 200, 'wdgpt_update_addon');
        }
        $addons_manager = WDGPT_Addons_Manager::instance();
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Instance Addons Manager created', 200, 'wdgpt_update_addon');
        }
        $id = sanitize_text_field( wp_unslash( $_POST['id'] ) );
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Cleaned ID: ' . $id, 200, 'wdgpt_update_addon');
        }
        $addon = $addons_manager->retrieve_addon( $id );
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Addon retrieved: ' . print_r($addon, true), 200, 'wdgpt_update_addon');
        }
        if (!$addon) {
            if ( WDGPT_DEBUG_MODE ) {
                WDGPT_Error_Logs::wdgpt_log_error('Error: Addon not found', 200, 'wdgpt_update_addon');
            }
            wp_send_json_error('Addon not found');
            return;
        }
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Starting update via Addons Manager', 200, 'wdgpt_update_addon');
        }
        $result = $addons_manager->install_addon( $addon, 'update' );
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Update result: ' . print_r($result, true), 200, 'wdgpt_update_addon');
        }
        if ( $result ) {
            wp_send_json_success( $result );
        }else{
        ...
        if ( WDGPT_DEBUG_MODE ) {
            WDGPT_Error_Logs::wdgpt_log_error('Exception during update: ' . $e->getMessage(), 200, 'wdgpt_update_addon');
            WDGPT_Error_Logs::wdgpt_log_error('Trace: ' . $e->getTraceAsString(), 200, 'wdgpt_update_addon');
        }
        wp_send_json_error($e->getMessage());
    }
}
