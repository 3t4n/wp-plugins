...
add_action('wp_ajax_woocommerce_ajax_add_to_cart_iute', 'woocommerce_ajax_add_to_cart_iute');
add_action('wp_ajax_nopriv_woocommerce_ajax_add_to_cart_iute', 'woocommerce_ajax_add_to_cart_iute');

error_log("woocommerce_ajax_add_to_cart_iute 0");

function woocommerce_ajax_add_to_cart_iute()
{
...
function iute_init_gateway_class()
{
    ...
    class WC_Iutepay_Gateway extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
            ...
            $this->antifraudEnabled = 'yes' === $this->get_option('iutepay_antifraud_enabled', $this->getDefaultAntifraud());
            ...
            $this->fastCheckoutButtonOnProductPageHTML = $this->get_option('productPage_fast_checkout_button_html', $this->getDefaultFastCheckoutButtonHtml());
            $this->fastCheckoutButtonOnProductPageSelector = $this->get_option('productPage_fast_checkout_button_selector', $this->getDefaultFastcheckoutSelector());
        ...
        function getDefaultTitle() {
            $defaultTitle = __('Pay in parts', 'iutepay');
            if ($this->country == 'en' || $this->country == 'al') {
                $defaultTitle = __('Pay with iute', 'iutepay');
            }
            return $defaultTitle;
        }

        function getDefaultDescription() {
            $defaultDescription = __('Buy now, pay later with iute.', 'iutepay');
            if ($this->country == 'en' || $this->country == 'al') {
                if ($this->antifraudEnabled) {
                    $defaultDescription = __('Pay instantly from your Digital Wallet or Pay later in parts', 'iutepay');
                }
            }
            return $defaultDescription;
        }

        function getDefaultFastCheckoutButtonHtml() {
            $fastCheckoutButtonText = __('Start checkout with iute', 'iutepay');
            return <<<HTML
<p></p>
<div class="iute-add-to-cart-button iute-add-to-cart-button-woo button">
    <button class="iute-symbol-logo-primary">&nbsp;</button> {$fastCheckoutButtonText}
</div>
HTML;
        }

        function getDefaultAntifraud() {
            if ($this->country == 'en' || $this->country == 'al') {
                return 'yes';
            }
            return 'no';
        ...
        function getDefaultFastcheckoutSelector() {
            return "div.iute-add-to-cart-button";
        }

        function getResetToDefault($fieldName, $defaultValue) {
            $defaultValue = base64_encode($defaultValue);
            return <<<HTML
<br/><small><a class="resetToDefault" data-input-id='woocommerce_iutepay_{$fieldName}' data-default='{$defaultValue}'>Reset to default</a></small>
HTML;
        }
        ...
        public function init_form_fields()
        {
            ...
            $defaultAntifraud = $this->getDefaultAntifraud();
            $defaultFastCheckoutButton = $this->getDefaultFastCheckoutButtonHtml();
            $defaultFastCheckoutSelector = $this->getDefaultFastcheckoutSelector();
            ...
            $this->form_fields = array(
                ...
                'description' => array(
                    'title' => 'Description'.$this->getResetToDefault("description", $defaultDescription),
                    'type' => 'textarea',
                    'description' => __('Payment method description on checkout page', 'iutepay'),
                ...
                'productPage_fast_checkout_button_html' => array(
                    'title' => 'HTML layout'.$this->getResetToDefault("productPage_fast_checkout_button_html", $defaultFastCheckoutButton),
                    'type' => 'textarea',
                    'description' => __('HTML layout of the checkout button.', 'iutepay'),
                    'desc_tip' => true,
                    'default' => $defaultFastCheckoutButton,

                ),
                'productPage_fast_checkout_button_selector' => array(
                    'title' => 'click jQuery selector'.$this->getResetToDefault("productPage_fast_checkout_button_selector", $defaultFastCheckoutSelector),
                    'type' => 'text',
                    'description' => __('jQuery selector for onclick handler', 'iutepay'),
                    'desc_tip' => true,
                    'default' => $defaultFastCheckoutSelector
                ),
            );
...
<script>
jQuery(document).ready(function(){
    jQuery("a.resetToDefault").click(function () {
      const defaultValue = jQuery(this).attr('data-default');
      const inputId =  jQuery(this).attr('data-input-id');
      jQuery('#' + inputId).val(atob(defaultValue));
    })
});
</script>
OUT;
