...
function location_meta_box_callback($post) {
    ...
    <div class="col span_1_of_2">
      <input type="text" class="regular-text" name="txt_location_street" id="txt_location_street" placeholder="<?php esc_html_e('Street and No.', 'wp_event_booking');?>" value="<?php echo esc_html($location_street); ?>">
      <input type="text" class="regular-text" name="txt_location_zip" id="txt_location_zip" placeholder="<?php esc_html_e('Zip code', 'wp_event_booking');?>" value="<?php echo esc_html($location_zip); ?>">
      <input type="text" class="regular-text" name="txt_location_city" id="txt_location_city" placeholder="<?php esc_html_e('City', 'wp_event_booking');?>" value="<?php echo esc_html($location_city); ?>">
      <?php
/* @parm $id,
    ...
    <div class="col span_1_of_2"></div>
      ...
      <script type="text/javascript">
        jQuery(document).ready(function(){
          jQuery(".sel_location_country").val('<?php echo esc_html($location_country); ?>').trigger('change');
        });
      </script>
      <?php }?>
<h4><?php esc_html_e('Region', 'wp_event_booking');?></h4>
<hr />
<div class="section group">
  <div class="col span_1_of_2">
  ...
  <div class="col span_1_of_2">
    ...
    <input type="text" class="regular-text" name="txt_location_region_title" id="txt_location_region_title" placeholder="<?php esc_html_e('Title', 'wp_event_booking');?>">
    ...
    <input type="button" class="button button-primary" id="btn_cancel_location_region" name="btn_cancel_location_region" value="<?php esc_html_e('Cancel', 'wp_event_booking');?>">
  </div>
  <div class="col span_1_of_2"></div>
</div>
</div>
...
function save_location_meta_box($post_id) {
	...
	if (!isset($_POST['event_region_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['event_region_nonce'])), '_event_region_nonce')) {
		return;
	}
	...
	if (isset($_POST['txt_location_street'])) {
		update_post_meta($post_id, 'location_street', sanitize_text_field(wp_unslash($_POST['txt_location_street'])));
	}
	if (isset($_POST['txt_location_zip'])) {
		update_post_meta($post_id, 'location_zip', sanitize_text_field(wp_unslash($_POST['txt_location_zip'])));
	}
	if (isset($_POST['txt_location_city'])) {
		update_post_meta($post_id, 'location_city', sanitize_text_field(wp_unslash($_POST['txt_location_city'])));
	}
	if (isset($_POST['sel_location_country'])) {
		update_post_meta($post_id, 'location_country', sanitize_text_field(wp_unslash($_POST['sel_location_country'])));
	}
	if (isset($_POST['event_region'])) {
		update_post_meta($post_id, '_event_region', sanitize_text_field(wp_unslash($_POST['event_region'])));
	}
}
