...
function event_booking_settings_page() {
    ...
    if (!in_array('supplier', $current_user->roles)) {
		add_submenu_page('edit.php?post_type=cpt_events', esc_html__('Settings', 'wp_event_booking'), esc_html__('Settings', 'wp_event_booking'), 'edit_posts', 'wp_event_booking_settings', 'fnc_wp_event_booking_settings');
	}
	add_submenu_page('edit.php?post_type=cpt_events', esc_html__('Documentation', 'wp_event_booking'), esc_html__('Documentation', 'wp_event_booking'), 'edit_posts', 'wp_event_booking_doc', 'fnc_wp_event_booking_documentation');
	add_submenu_page('edit.php?post_type=cpt_events', esc_html__('Extensions', 'wp_event_booking'), esc_html__('Extensions', 'wp_event_booking'), 'edit_posts', 'http://awesometogi.com/awesome-event-booking/', '');
...
function register_wp_event_booking_settings() {
	...
	register_setting('wp-event-booking-settings', 'show_event_short_description');
	register_setting('wp-event-booking-settings', 'show_event_region');
	register_setting('wp-event-booking-settings', 'event_sorted_by');
	...
	register_setting('wp-event-booking-settings', 'wpeb_ms_notification_reciever');
	register_setting('wp-event-booking-settings', 'wpeb_multi_booking_failed_message');
}
...
<div class="wrap">
	<a href="http://awesometogi.com/awesome-event-booking/" target="_blank">
			<div style="text-align: center;">
				<img src="<?php echo esc_url(WPEB_URL) . 'src/img/banner-for-extensions.jpg' ?>">
			</div>
		</a>
...
<a href="?post_type=cpt_events&page=wp_event_booking_settings" class="nav-tab <?php echo esc_html($active_tab) == '' ? 'nav-tab-active' : ''; ?>"><?php esc_html_e('General Settings', 'wp_event_booking');?></a>
<?php }

add_action('wpeb_settings_new_menu_item', 'fnc_wpeb_settings_email_menu');
function fnc_wpeb_settings_email_menu($active_tab) {?>
<a href="?post_type=cpt_events&page=wp_event_booking_settings&tab=email" class="nav-tab <?php echo esc_html($active_tab) == 'email' ? 'nav-tab-active' : ''; ?>"><?php esc_html_e('Email Settings', 'wp_event_booking');?></a>
<?php }

add_action('wpeb_settings_new_menu_item', 'fnc_wpeb_settings_event');
function fnc_wpeb_settings_event($active_tab) {?>
<a href="?post_type=cpt_events&page=wp_event_booking_settings&tab=event" class="nav-tab <?php echo esc_html($active_tab) == 'event' ? 'nav-tab-active' : ''; ?>"><?php esc_html_e('Event Settings', 'wp_event_booking');?></a>
<?php }

add_action('wpeb_settings_new_menu_item', 'fnc_wpeb_settings_styling');
function fnc_wpeb_settings_styling($active_tab) {?>
<a href="?post_type=cpt_events&page=wp_event_booking_settings&tab=styling" class="nav-tab <?php echo esc_html($active_tab) == 'styling' ? 'nav-tab-active' : ''; ?>"><?php esc_html_e('Styling', 'wp_event_booking');?></a>
<?php }
/* Add Tab - End*/
...
function fnc_new_event_sign_up_admin_notification() {
	$_supplier_name = '';
	if(function_exists('wpeb_ms_is_active')) $_supplier_name = ', %supplier_name%';
	?>
	<tr valign="top">
		<th scope="row"><?php esc_html_e('Event sign up - Admin notification Subject', 'wp_event_booking');?></th>
		<td>
			<input type="text" name="signUpAdminNotificationSubject" value="<?php echo esc_attr(get_option('signUpAdminNotificationSubject')); ?>" />
			<p class="tooltip description"><?php esc_html_e('Possible shortcodes', 'wp_event_booking');?> :
			%event_title%, %event_date_time%, %event_price%, %event_location%<?php echo esc_html($_supplier_name);?>
	...
	<tr class="signUpAdminNotification-wrap">
		<th>
			<label for="signUpAdminNotification"><?php esc_html_e('Event sign up - Admin notification Body', 'wp_event_booking');?></label>
		</th>
		<td><?php wp_editor(get_option('signUpAdminNotification'), 'signUpAdminNotification', array('textarea_rows' => 10, 'teeny' => false, 'quicktags' => true));?>
			<p class="description"><?php esc_html_e('Possible shortcodes', 'wp_event_booking');?> : %event_title%, %event_title%, %event_date_time%, %event_price%,%event_location%, %user_details%<?php echo esc_html($_supplier_name);?></p>
		</td>
	</tr>
...
function fnc_new_event_sign_up_customer_notification() {
	...
	?>
	<tr valign="top">
		<th scope="row"><?php esc_html_e('Event sign up - Customer notification Subject', 'wp_event_booking');?></th>
		<td>
	...
	<tr class="signUpCustomerNotification-wrap">
		...
		<td><?php wp_editor(get_option('signUpCustomerNotification'), 'signUpCustomerNotification', array('textarea_rows' => 10, 'teeny' => false, 'quicktags' => false));?>
			<p class="description"><?php esc_html_e('Possible shortcodes', 'wp_event_booking');?> : %event_title%, %event_date_time%, %event_price%, %event_location%, %booking_id%, %attendant_first_name%, %attendant_last_name%, %attendant_phone_number%. %attendant_e_mail_address%, %attendant_address%, %attendant_zip_code%, %attendant_city%, %attendant_state%, %company_name%, %company_address%, %company_zip_code%, %company_city%, %company_state%, %company_vat_number%, %company_contact_person%, %company_contact_person_phone_number%, %company_invoice_e_mail_address%, %company_reference%, %personal_identification_number%, %credentials%<?php echo esc_html($_supplier_name);?></p>
		</td>
	</tr>
	<tr><td colspan="2"><hr /></td></tr>
	<?php
...
function FncAdminAddCustomerAdminNotifications() {
	...
	?>
	<tr valign="top">
		<th scope="row"><?php esc_html_e('Event sign up(Dashboard) - Admin notification Subject', 'wp_event_booking');?></th>
		<td>
	...
	<tr class="signUpAdminNotification-wrap">
		<th>
			<label for="signUpAdminNotification"><?php esc_html_e('Event sign up(Dashboard) - Admin notification Body', 'wp_event_booking');?></label>
		</th>
...
function FncAdminAddCustomerCustomerNotifications() {
	...
	?>
	<tr valign="top">
		<th scope="row"><?php esc_html_e('Event sign up(Dashboard) - Customer notification Subject', 'wp_event_booking');?></th>
		<td>
	...
	<tr class="signUpCustomerNotification-wrap">
		<th>
			<label for="description"><?php esc_html_e('Event sign up(Dashboard) - Customer notification Body', 'wp_event_booking');?></label>
		</th>
...
function fnc_event_cancel_admin_notification() {
	...
	?>
	<tr valign="top">
		<th scope="row"><?php esc_html_e('Admin Cancel Sign Up - Admin notification Subject', 'wp_event_booking');?></th>
		<td>
	...
	<tr class="cancelAdminNotification-wrap">
		<th>
			<label for="cancelAdminNotification"><?php esc_html_e('Admin Cancel Sign Up - Admin notification Body', 'wp_event_booking');?></label>
		</th>
...
function fnc_event_cancel_customer_notification() {
	...
	?>
	<tr valign="top">
		<th scope="row"><?php esc_html_e('Admin Cancel Sign Up - Customer notification Subject', 'wp_event_booking');?></th>
		<td>
	...
	<tr class="cancelCustomerNotification-wrap">
		<th>
			<label for="cancelCustomerNotification"><?php esc_html_e('Admin Cancel Sign Up - Customer notification Body', 'wp_event_booking');?></label>
		</th>
...
function fnc_event_cancel_2_admin_notification() {
	...
	?>
	<tr valign="top">
		<th scope="row"><?php esc_html_e('User Cancel Signup - Admin notification Subject', 'wp_event_booking');?></th>
		<td>
	...
	<tr class="usercancelAdminNotification-wrap">
		<th>
			<label for="usercancelAdminNotification"><?php esc_html_e('User Cancel Signup - Admin notification Body', 'wp_event_booking');?></label>
		</th>
...
function fnc_event_cancel_2_customer_notification() {
	...
	?>
	<tr valign="top">
		<th scope="row"><?php esc_html_e('User Cancel Signup - Customer notification Subject', 'wp_event_booking');?></th>
		<td>
	...
	<tr class="usercancelCustomerNotification-wrap">
		<th>
			<label for="usercancelCustomerNotification"><?php esc_html_e('User Cancel Signup - Customer notification Body', 'wp_event_booking');?></label>
		</th>
...
function fnc_sendReminderBefore_notification() {
	...
	?>
	<tr valign="top">
		<th scope="row"><?php esc_html_e('Event Reminder Subject', 'wp_event_booking');?></th>
		<td>
			<input type="text" name="sendReminderBeforeSubject" value="<?php echo esc_attr(get_option('sendReminderBeforeSubject')); ?>" />
			<p class="tooltip description"><?php esc_html_e('Possible shortcodes', 'wp_event_booking');?> : %event_title%, %event_date_time%, %event_price%, %event_location%, %booking_id%, %attendant_first_name%, %attendant_last_name%, %attendant_phone_number%. %attendant_e_mail_address%, %attendant_address%, %attendant_zip_code%, %attendant_city%, %attendant_state%, %company_name%, %company_address%, %company_zip_code%, %company_city%, %company_state%, %company_vat_number%, %company_contact_person%, %company_contact_person_phone_number%, %company_invoice_e_mail_address%, %company_reference%, %personal_identification_number%, %site_title%<?php echo esc_html($_supplier_name);?></p>
		</td>
	</tr>
	<tr class="sendReminderBeforeMessage-wrap">
		<th>
			<label for="sendReminderBeforeMessage"><?php esc_html_e('Event Reminder Message', 'wp_event_booking');?></label>
		</th>
