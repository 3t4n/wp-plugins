...
$options = [
    1 => esc_html__('Last 6 Months', 'wp_event_booking'),
    2 => esc_html__('This Year', 'wp_event_booking'),
    3 => esc_html__('Previous Year', 'wp_event_booking'),
    4 => esc_html__('Last 5 Years', 'wp_event_booking'),
    5 => esc_html__('Custom Range', 'wp_event_booking'),
];

$selected = isset($_GET['selected']) ? sanitize_text_field(wp_unslash($_GET['selected'])) : '1';
$start_date = isset($_GET['start-date']) ? sanitize_text_field(wp_unslash($_GET['start-date'])) : (new DateTime('first day of -6 months'))->format('Y-m-d');
$end_date = isset($_GET['end-date']) ? sanitize_text_field(wp_unslash($_GET['end-date'])) : (new DateTime('today'))->format('Y-m-d');

$active_text = isset($options[$selected]) ? $options[$selected] : $options[1];
?>
<div class="wrap">
    <h1><?php esc_html_e('Monthly Analytics', 'wp_event_booking');?></h1>
    ...
    <form id="filter-form" action="" method="get">
        ...
        <div id="date-range-filter">
            ...
            <div class="filter-input-grp" <?php echo $selected == 5 ? '' : 'style="display:none;"';?>>
    ...
    <div id="analytics-tables" class="analytics-table-container">
        <div class="analytics-table">
            ...
            <h2 class="table-h2"><?php esc_html_e('Sign Up Clicks Monthly', 'wp_event_booking');?></h2>
            <table id="sign_up_dated" class="widefat fixed">
                <thead>
                    <tr>
                    <th><?php esc_html_e('Month', 'wp_event_booking');?></th>
        ...
        <div class="analytics-table">
            <!-- Table for Detailed Overview Data -->
            <h2 class="table-h2"><?php esc_html_e('More Detail Clicks Monthly', 'wp_event_booking');?></h2>
    ...
    <!-- Chart Container -->
        ...
        <h2 class="table-h2"><?php esc_html_e('Sign Up Clicks Monthly Chart', 'wp_event_booking');?></h2>
        <div class="w-chart loading">
            <canvas id="signupClickAnalyticsMonthlyChart" width="400" height="300" style="height:300px; width:100%;"></canvas>
        </div>

        <h2 class="table-h2"><?php esc_html_e('More Detail Clicks Monthly Chart', 'wp_event_booking');?></h2>
        <div class="w-chart loading">
            <canvas id="moreDetailClickAnalyticsMonthlyChart" width="400" height="300" style="height:300px; width:100%;"></canvas>
        </div>
    </div>
    ...
    <div id="analytics-tables" class="analytics-table-container">
        <div class="analytics-table">
            <!-- Table for Detailed Overview Data -->
            <h2 class="table-h2"><?php esc_html_e('Sign Up Clicks by Event Monthly', 'wp_event_booking');?></h2>
        ...
        <div class="analytics-table">
            <!-- Table for Detailed Overview Data -->
            <h2 class="table-h2"><?php esc_html_e('More Detail Clicks by Event Monthly', 'wp_event_booking');?></h2>
    ...
    <div id="analytics-tables" class="analytics-table-container">
        <div class="analytics-table">
            <!-- Table for Detailed Overview Data -->
            <h2 class="table-h2"><?php esc_html_e('Published Events Monthly', 'wp_event_booking');?></h2>
        ...
        <div class="analytics-table">
            <!-- Table for Detailed Overview Data -->
            <h2 class="table-h2"><?php esc_html_e('Public Form Events Monthly', 'wp_event_booking');?></h2>
...
<script>
var signupMonthlyChart = null;
var moreDetailMonthlyChart = null;
...
jQuery(document).ready(function ($) {
    ...
    loadSignUpGraphMonthly(startDate, endDate);
    loadMoreDetailGraphMonthly(startDate, endDate);
...
function loadSignUpGraphMonthly(startDate, endDate) {
    ...
    jQuery.ajax({
        ...
        method: 'POST',
        data: {
            action: 'fetch_analytics_data_in_monthly_chart',
        ...
        success: function (response) {
            ...
            const isCenterLabel = months.length <= 3;

            if (signupMonthlyChart !== null) {
                signupMonthlyChart.destroy();
            }
            // Initialize the chart
            var ctx = document.getElementById('signupClickAnalyticsMonthlyChart').getContext('2d');
            signupMonthlyChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        x: {
                            ...
                            //     minRotation: 0
                            // },
                            // // Define maximum number of labels shown on the x-axis
                            // max: 60,
                            // min: 0,
                            // Enable scrolling
...
// load more detail graph function
function loadMoreDetailGraphMonthly(startDate, endDate) {
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            if (moreDetailMonthlyChart !== null) {
                moreDetailMonthlyChart.destroy();
            }

            // Initialize the chart
            var ctx = document.getElementById('moreDetailClickAnalyticsMonthlyChart').getContext('2d');
            moreDetailMonthlyChart = new Chart(ctx, {
                type: 'line', // You can change it to 'bar', 'line', 'pie', etc.
                data: {
                    labels: months, // Labels for the X-axis
                    datasets: [{
    ...
    jQuery('#sign_up_dated').DataTable({
        ...
        "ajax": {
            ...
            "data": function (d) {
                ...
                d['action'] = 'fetch_analytics_data_monthly';
...
function getSignUpEventTableListing(startDate, endDate) {
    ...
    jQuery('#sign_up_event_table').DataTable({
        ...
        "ajax": {
            ...
            "data": function (d) {
                d.page = (d.start / d.length) + 1;
                d['action'] = 'fetch_analytics_data_by_event_monthly';
                d['start_date'] = startDate;
                d['end_date'] = endDate;
                d['click_type'] = 'sign_up';
            }
