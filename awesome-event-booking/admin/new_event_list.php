...
function my_sort_events($query) {
	...
	if (isset($_GET['post_type']) && 'cpt_events' == sanitize_text_field(wp_unslash($_GET['post_type'])) && $query->is_main_query()) {
...
function my_cpt_events_columns($cpt_events_columns) {
	...
	$cpt_events_columns['amount_of_signups'] = esc_html__('Amount of Signups', 'wp_event_booking');
	$cpt_events_columns['amount_of_seats_left'] = esc_html__('Available Seats Left', 'wp_event_booking');
	$cpt_events_columns['event_start_date'] = esc_html__('Event Start Date', 'wp_event_booking');
	$cpt_events_columns['Region'] = esc_html__('Region', 'wp_event_booking');
	$cpt_events_columns['city'] = esc_html__('City', 'wp_event_booking');
...
function manage_events_columns($column_name, $id) {
	...
	case 'Region':
		...
		echo esc_html(get_the_title($event_region_id));
		break;
	case 'event_start_date':
		$event_start_date = get_post_meta($id, '_eventStartDate', true);
		echo esc_html(date_i18n($GLOBALS['date_format_for_display'], strtotime($event_start_date)));
		break;
	case 'amount_of_signups':
		...
		echo '<a class="wpeb_tooltip" alt="' . esc_html($id) . '" href="' . esc_url($permalink) . '">' . esc_html($bookings) . '<div class="wpeb_tooltipwrap"><div class="wpeb_tooltipcontent"></div></div></a>';
		break;
	case 'city':
		$event_loation_id = get_post_meta($id, '_event_location', true);
	...
	case 'wc_checkout':
		...
		if($wc_addon){
			echo '<span class="wc-checkout-value" data-value="true">'. esc_html__('Enabled', 'wpeb_wc_addon').'</span>';
		}else{
			echo '<span class="required wc-checkout-value" data-value="false">'. esc_html__('Disabled', 'wpeb_wc_addon').'</span>';
		}
		break;
	...
	if ('cpt_events' == $type) {
		?>
          ...
          <div class="date_time_picker new_filter_admin">
              ...
              <span style="font-size: 17px;"><?php esc_html_e('From : ', 'wp_event_booking');?> </span>
              <input type="text" name="txt_start_date_listing" id="txt_start_date_listing" class="date start" value="<?php echo (isset($_GET['txt_start_date_listing'])) ? esc_attr(sanitize_text_field(wp_unslash($_GET['txt_start_date_listing']))) : ''; ?>">

               <span style="font-size: 17px;"> <?php esc_html_e('To : ', 'wp_event_booking');?> </span>

              <input type="text" name="txt_end_date_listing" id="txt_end_date_listing" class="date end" value="<?php echo (isset($_GET['txt_end_date_listing'])) ? esc_attr(sanitize_text_field(wp_unslash($_GET['txt_end_date_listing']))) : ''; ?>">
            </p>
            </div>
           <!--  filter for event date -->
          <!--  filter for region -->
		...
		if (!empty($values)) {
			?>
          	<select name="filter_by_region">
                <option value=""><?php esc_html_e('Select Region', 'wp_event_booking');?></option>
                <?php
				$current_v = isset($_GET['filter_by_region']) ? sanitize_text_field(wp_unslash($_GET['filter_by_region'])) : '';
		...
		if (!empty($values_for_locations)) {
          ...
          <select name="filter_by_city">
                <option value=""><?php esc_html_e('Select City', 'wp_event_booking');?></option>
                <?php
				$current_v = isset($_GET['filter_by_city']) ? sanitize_text_field(wp_unslash($_GET['filter_by_city'])) : '';
...
function my_custom_filter($query) {
	...
	if ('cpt_events' == $type && is_admin() && $pagenow == 'edit.php' && isset($_GET['txt_start_date_listing']) && sanitize_text_field(wp_unslash($_GET['txt_start_date_listing'])) !== '' && $query->is_main_query()) {
		$strt_date = isset($_GET['txt_start_date_listing']) ? sanitize_text_field(wp_unslash($_GET['txt_start_date_listing'])) : '';
		$strt_time = isset($_GET['txt_start_time_listing']) ? sanitize_text_field(wp_unslash($_GET['txt_start_time_listing'])) : '00:00:00';
		$end_time = isset($_GET['txt_end_time_listing']) ? sanitize_text_field(wp_unslash($_GET['txt_end_time_listing'])) : '23:59:59';
		$end_date = isset($_GET['txt_end_date_listing']) ? sanitize_text_field(wp_unslash($_GET['txt_end_date_listing'])) : '';

		$start_date_query = gmdate('Y-m-d H:i:s', strtotime($strt_date . $strt_time));
		$end_date_query = gmdate('Y-m-d H:i:s', strtotime($end_date . $end_time));
	...
	if ('cpt_events' == $type && is_admin() && $pagenow == 'edit.php' && isset($_GET['filter_by_region']) && sanitize_text_field(wp_unslash($_GET['filter_by_region'])) !== '' && $query->is_main_query()) {
		$events_for_region = get_event_ids_from_region_ids(sanitize_text_field(wp_unslash($_GET['filter_by_region'])));
	...
	if ('cpt_events' == $type && is_admin() && $pagenow == 'edit.php' && isset($_GET['filter_by_city']) && sanitize_text_field(wp_unslash($_GET['filter_by_city'])) !== '' && $query->is_main_query()) {
		$events_for_location = get_event_ids_from_city(sanitize_text_field(wp_unslash($_GET['filter_by_city'])));
...
function remove_edit_post_views($views) {
	...
	if ((isset($_GET['post_type']) && sanitize_text_field(wp_unslash($_GET['post_type'])) == 'cpt_events') && (isset($_GET['tab']) && sanitize_text_field(wp_unslash($_GET['tab'])) == 'future_events')) {
		$add_class_future = "aria-current='page' class='current' ";
	} else {
		$add_class_future = "";
	}
	if (isset($_GET['tab']) && sanitize_text_field(wp_unslash($_GET['tab'])) == 'past_events') {
	...
	// $views['all'] = '<a href="' . admin_url() . 'edit.php?post_type=cpt_events" ' . $add_class_future . '>' . esc_html__('Future Events', 'wp_event_booking') . '</a>' . $future_count;

	$views['upcoming'] = '<a href="' . admin_url() . 'edit.php?post_type=cpt_events&tab=future_events" ' . $add_class_future . '>' . esc_html__('Future Events', 'wp_event_booking') . '</a>' . $future_count;

	$prev_count = '(' . get_previous_events_count() . ')';
	$views['pre'] = '<a href="' . admin_url() . 'edit.php?post_type=cpt_events&tab=past_events" ' . $add_class_past . '>' . esc_html__('Previous Events', 'wp_event_booking') . '</a>' . $prev_count;
...
function my_previous_events_list($q) {
	...
	$today = gmdate("Y-m-d H:i:s", strtotime('now'));
	if (is_admin() && isset($scr->base) &&  ($scr->base === 'edit') && $q->is_main_query()) {
		global $typenow, $pagenow;
		$meta_query =array();
		if ('cpt_events' == $typenow && is_admin() && $pagenow == 'edit.php' && isset($_GET['txt_start_date_listing']) && sanitize_text_field(wp_unslash($_GET['txt_start_date_listing'])) !== '' && $q->is_main_query()) {
			$strt_date = isset($_GET['txt_start_date_listing']) ? sanitize_text_field(wp_unslash($_GET['txt_start_date_listing'])) : '';
			$strt_time = isset($_GET['txt_start_time_listing']) ? sanitize_text_field(wp_unslash($_GET['txt_start_time_listing'])) : '00:00:00';
			$end_time = isset($_GET['txt_end_time_listing']) ? sanitize_text_field(wp_unslash($_GET['txt_end_time_listing'])) : '23:59:59';
			$end_date = isset($_GET['txt_end_date_listing']) ? sanitize_text_field(wp_unslash($_GET['txt_end_date_listing'])) : '';
		...
		else
		{
			...
			$pre = isset($_GET['tab']) ? sanitize_text_field(wp_unslash($_GET['tab'])):''; //filter_input(INPUT_GET, 'tab', FILTER_SANITIZE_STRING);
			...
			if ($pre == '' && (isset($_GET['post_type']) && sanitize_text_field(wp_unslash($_GET['post_type'])) == 'cpt_events')) {
				// $meta_query[] = array('key' => '_eventStartDate', 'value' => $today, 'compare' => '>=');
			}
		}
