...
require_once WPEB_DIR . 'library/dompdf/autoload.inc.php';
use Dompdf\Dompdf;
use Dompdf\Options;
...
function fnc_pull_attendees_callback()
{
    ...
    if (!empty($booking_ids)) {
        ...
        if (!empty($user_query->get_results())) {
            ...
            $error = esc_html__('Noting found...', 'wp_event_booking');
        }
    } else {
        $error = esc_html__('Noting found...', 'wp_event_booking');
    }
...
function create_event_location_callback()
{
    ...
    $txt_location_title = sanitize_text_field(wp_unslash($_POST['txt_location_title']));
    $txt_location_street = sanitize_text_field(wp_unslash($_POST['txt_location_street']));
    $txt_location_zip = sanitize_text_field(wp_unslash($_POST['txt_location_zip']));
    $txt_location_city = sanitize_text_field(wp_unslash($_POST['txt_location_city']));
    $sel_location_country = sanitize_text_field(wp_unslash($_POST['sel_location_country']));
    $sel_event_region = sanitize_text_field(wp_unslash($_POST['sel_event_region']));
    ...
    if (isset($_POST['sel_event_region'])) {
        update_post_meta($event_location_id, '_event_region', sanitize_text_field(wp_unslash($_POST['sel_event_region'])));
    }

    echo json_encode(array('location_id' => esc_html($event_location_id), 'location_title' => esc_html($txt_location_title)));
...
function create_event_manager_callback()
{
    ...
    $txt_event_manager_title = sanitize_text_field(wp_unslash($_POST['txt_event_manager_title']));
    $txt_event_manager_phone = sanitize_text_field(wp_unslash($_POST['txt_event_manager_phone']));
    $txt_event_manager_email = sanitize_text_field(wp_unslash($_POST['txt_event_manager_email']));
    $txt_event_manager_website = sanitize_text_field(wp_unslash($_POST['txt_event_manager_website']));
    ...
    echo json_encode(array('event_manager_id' => esc_html($event_manager_id), 'event_manager_title' => esc_html($txt_event_manager_title)));
...
function create_location_region_callback()
{
    ...
    $txt_location_region_title = sanitize_text_field(wp_unslash($_POST['txt_location_region_title']));
    $txt_location_region_description = sanitize_text_field(wp_unslash($_POST['txt_location_region_description']));
    ...
    echo json_encode(array('location_region_id' => esc_html($location_region_id), 'location_region_title' => esc_html($txt_location_region_title)));
...
function create_event_customer_callback()
{
    ...
    // Verify the nonce
    check_ajax_referer('create_event_customer_nonce', 'nonce');
    
    global $wpdb;

    $txt_customer_first_name = sanitize_text_field(wp_unslash($_POST['txt_attendant_first_name']));
    $txt_customer_last_name = sanitize_text_field(wp_unslash($_POST['txt_attendant_last_name']));
    //$txt_customer_phone = sanitize_text_field(wp_unslash($_POST['txt_customer_phone']));
    $user_name = $user_email = $txt_customer_email = sanitize_text_field(wp_unslash($_POST['txt_attendant_e_mail_address']));
    //$txt_customer_description = sanitize_text_field(wp_unslash($_POST['txt_customer_description']));
    ...
    if (!$user_id and email_exists($user_email) == false) {
        ...
        foreach ($sign_up_fields as $suf) {
            if (isset($_POST[$suf])) {
                update_user_meta($user_id, $suf, sanitize_text_field(wp_unslash($_POST[$suf])));
            }
        }
    }
    ...
    // Add role
    $u->add_role('customer');
    echo json_encode(array('customer_id' => esc_html($user_id), 'customer_name' => esc_html($user_info->display_name)));
...
{
    echo '<script type="text/javascript">var datepickerFormat = "' . esc_html(get_datepicker_format('js_format', get_option('datepickerFormat'))) . '";
	var timepickrformat ="' . esc_html(get_option('timeFormat')) . '";
	var addNewLocationPlaceholder = "' . esc_html__('Begin typing location or add new location', 'wp_event_booking') . '";
	var addNewManagerPlaceholder = "' . esc_html__("Begin typing to search or add new Event Manager", 'wp_event_booking') . '";
	var addNewCustomerPlaceholder = "' . esc_html__("Begin typing customer or add new customer", 'wp_event_booking') . '";
	var selectEventPlaceholder = "' . esc_html__("Type event name to select an event.", 'wp_event_booking') . '";
	var addNewRegionPlaceholder = "' . esc_html__("Begin typing to search or add new Region", 'wp_event_booking') . '";
	var suppliersPlaceholder = "' . esc_html__("Begin typing to search suppliers", 'wp_event_booking') . '";
	var country_trans = "' . esc_html__("Country", 'wp_event_booking') . '";
	var region_trans = "' . esc_html__("Region", 'wp_event_booking') . '";
    var location_trans = "' . esc_html__("Location", 'wp_event_booking') . '";
	</script>';
}

add_action('wp_head', 'fnc_wp_head');
function fnc_wp_head()
{
    echo '<script type="text/javascript">var datepickerFormat = "' . esc_html(get_datepicker_format('js_format', get_option('datepickerFormat'))) . '";
	var timepickrformat ="' . esc_html(get_option('timeFormat')) . '";
    var country_trans = "' . esc_html__("Country", 'wp_event_booking') . '";
...
function fnc_get_event_customers_callback()
{
    ...
    $search_string = sanitize_text_field(wp_unslash($_GET['q']));
...
{
    ?>
	<select name="<?php echo esc_html($id); ?>" class="regular-text <?php echo esc_html($id); ?>" id="<?php echo esc_html($id); ?>">
...
/* If displaying the 'duration' column. */
        case 'status':
            ...
            if (empty($status)) {
                ...
                if ($_event_id) {
                    ...
                    if (strtotime("now") < strtotime($_eventStartDate)) {
                        echo '<p class="success">' . esc_html__('Upcoming', 'wp_event_booking') . '</p>';
                    } else {
                        echo '<p class="warning">' . esc_html__('Past', 'wp_event_booking') . '</p>';
                    }
                }
            }
            ...
            /* If there is a duration, append 'minutes' to the text string. */
                    ...
                    echo '<p class="error">' . ucfirst(esc_html($status)) . '</p>';
                }
    ...
    {
        $bulk_actions['activate_booking'] = esc_html__('Activate Booking', 'wp_event_booking');
        $bulk_actions['cancel_booking'] = esc_html__('Cancel Booking', 'wp_event_booking');
        return $bulk_actions;
    }
    ...
    function my_bulk_action_handler($redirect_to, $doaction, $post_ids)
{
    ...
    function fnc_show_cancelled_link_unset_trash($views)
    {
        ...
        if ((is_admin()) && (sanitize_text_field(wp_unslash($_GET['post_type'])) == 'event_booking')) {
            if ( in_array('supplier', $current_user->roles) ) {
                ...
                // translators: %1$s is the URL for cancelled posts, and %2$d is the count of views.
                $translated_text = __('<a href="%1$s">Cancelled <span class="count">(%2$d)</span></a>', 'wp_event_booking');

                $views['publish_f'] = sprintf(
                $translated_text, 
                admin_url('edit.php?booking_status=cancelled&post_type=event_booking'), 
                $query->found_posts);
                return $views;
            }else{
    ...
    function remove_row_actions($actions, $post)
    {
        if (get_post_type() === 'event_booking') {
            ...
            if (get_post_meta($post->ID, 'booking_status', true) == 'cancelled') {
                ...
                if ($_GET && !empty($_GET['booking_status']) && sanitize_text_field(wp_unslash($_GET['booking_status'])) == 'cancelled') {
                ...
                $actions['activate'] = '<a class="submitactivate" title="' . esc_html__('Activate event booking', 'wp_event_booking') . '" href="' . $url . '">' . esc_html__('Activate', 'wp_event_booking') . '</a>';
            } else {
                ...
                $url = add_query_arg($qry_args, admin_url('edit.php'));
                $actions['cancel'] = '<a class="submitcancel" title="' . esc_html__('Cancel event booking', 'wp_event_booking') . '" href="' . $url . '">' . esc_html__('Cancel', 'wp_event_booking') . '</a>';
            }
        }
        return $actions;
    }
    ...
    function fnc_process_activate_cancel_click()
{
        ...
        if ($typenow == 'event_booking' && is_admin() && $pagenow == 'edit.php') {
            if ($_GET && !empty($_GET['process']) && !empty($_GET['booking_id']) && sanitize_text_field(wp_unslash($_GET['process'])) == 'activate_event_booking') {
            ...
            } elseif ($_GET && !empty($_GET['process']) && sanitize_text_field(wp_unslash($_GET['process'])) == 'cancel_event_booking' && !empty($_GET['booking_id'])) {
            $booking_id = sanitize_text_field(wp_unslash($_GET['booking_id']));
...
{
    if (is_admin() && $_GET && !empty($_GET['booking_status']) && sanitize_text_field(wp_unslash($_GET['booking_status'])) == 'cancelled') {
...
$sign_up_fields = array(
        array('field_name' => 'txt_attendant_phone_number',
            'field_title' => esc_html__('Attendant phone number', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_attendant_address',
            'field_title' => esc_html__('Attendant address', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_attendant_zip_code',
            'field_title' => esc_html__('Attendant zip code', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_attendant_city',
            'field_title' => esc_html__('Attendant city', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_attendant_state',
            'field_title' => esc_html__('Attendant state', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_company_name',
            'field_title' => esc_html__('Company name', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_company_address',
            'field_title' => esc_html__('Company address', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_company_zip_code',
            'field_title' => esc_html__('Company zip code', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_company_city',
            'field_title' => esc_html__('Company city', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_company_state',
            'field_title' => esc_html__('Company state', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_company_vat_number',
            'field_title' => esc_html__('Company vat number', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_company_contact_person',
            'field_title' => esc_html__('Company contact person', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_company_contact_person_phone_number',
            'field_title' => esc_html__('Company contact person phone number', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_company_invoice_e_mail_address',
            'field_title' => esc_html__('Company invoice e-mail address', 'wp_event_booking'),
            'field_type' => 'email',
        ),
        array('field_name' => 'txt_company_reference',
            'field_title' => esc_html__('Company reference', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_name' => 'txt_personal_identification_number',
            'field_title' => esc_html__('Personal identification number', 'wp_event_booking'),
            'field_type' => 'text',
        ),
    );
    foreach ($sign_up_fields as $SignupFN) {
        ?>
	<tr>
		<th><label for="<?php echo esc_html($SignupFN['field_name']); ?>"><?php echo esc_html(($SignupFN['field_title']));?></label></th>
		<td>
			<input type="<?php echo esc_html($SignupFN['field_type']); ?>" name="<?php echo esc_html($SignupFN['field_name']); ?>" id="<?php echo esc_html($SignupFN['field_name']); ?>" value="<?php echo esc_attr(get_the_author_meta($SignupFN['field_name'], $user->ID)); ?>" class="regular-text" />
		</td>
	</tr>
	<?php
}?>
...
{
    if (!wp_doing_ajax()) {
        if ($_POST && isset($_POST['event_id']) && isset($_POST['wpeb_signup_type']) && sanitize_text_field(wp_unslash($_POST['wpeb_signup_type'])) == 'quick_signup') {

            $wpeb_multi_email_booking = esc_attr(get_post_meta(sanitize_text_field(wp_unslash($_POST['event_id'])), 'wpeb_multi_email_booking', true));

            $aeb_settings =  get_option( 'wp_event_booking-settings' ); 
            $url = 'https://www.google.com/recaptcha/api/siteverify';
                $data=array(
                    'secret' => $aeb_settings['recaptcha']['secret_key'],
                    'response' => sanitize_text_field(wp_unslash($_POST['g-recaptcha-hd'])),
            ...
            if(($aeb_settings['recaptcha']['status']==1 && $response['success']!=true)) {
                 print esc_html__('Sorry, google recaptcha error shown', 'wp_event_booking');
                exit;
            } else if (!isset($_POST['verify_its_you']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['verify_its_you'])), '2h@tslogic')) {
                print esc_html__('Sorry, your nonce did not verify.', 'wp_event_booking');
                exit;
            } else {
                $event_id = sanitize_text_field(wp_unslash($_POST['event_id']));
                ...
                if (!empty($total_seats)) {
                    ...
                    if (sizeof($_POST['txt_attendant_first_name']) > $available_seats) {
                        print esc_html__('Sorry, not enough seats available.', 'wp_event_booking');
                        exit;
                    }
                }
                $j = 0;
                for ($i = 0; $i < sizeof($_POST['txt_attendant_first_name']); $i++) {
                    $txt_customer_first_name = sanitize_text_field(wp_unslash($_POST['txt_attendant_first_name'][$i]));
                    if (isset($_POST['txt_attendant_last_name'][$i]) && !empty($_POST['txt_attendant_last_name'][$i])) {
                        $txt_customer_last_name = sanitize_text_field(wp_unslash($_POST['txt_attendant_last_name'][$i]));
                    } else {
                        $txt_customer_last_name = '';
                    }
                    $user_name = $user_email = sanitize_text_field(wp_unslash($_POST['txt_attendant_e_mail_address'][$i]));
                    $user_id = '';
                    $user_id = username_exists($user_name);
                    ...
                    foreach ($sign_up_fields as $suf) {
                        if (isset($_POST[$suf][$i])) {
                            update_user_meta($user_id, $suf, sanitize_text_field(wp_unslash($_POST[$suf][$i])));
                        }
                    }
                    ...
                    if (empty($prev_bookings) || $wpeb_multi_email_booking == 1) {
                        ...
                        if ($booking_id) {
                            ...
                            update_post_meta($booking_id, '_author_id', $author_id);

                            // update suppliers into booking
                            if(get_post_meta($event_id, '_event_suppliers', true)){

                                update_post_meta($booking_id, '_suppliers_ids', get_post_meta($event_id, '_event_suppliers', true));
                            }
                            if(defined('WPEB_RE_ADDON_VERSION')) {

                                update_post_meta($booking_id, '_booked_event_start_date', get_post_meta($event_id, '_eventStartDate', true));
                                update_post_meta($booking_id, '_booked_event_end_date', get_post_meta($event_id, '_eventEndDate', true));
                                update_post_meta($booking_id, '_is_booked_event_all_day', get_post_meta($event_id, '_all_day_event', true));
                            }
                            
                            $user_info = get_userdata($user_id);
                            // Update the booking info into the database
                            wp_update_post(array('ID' => $booking_id, 'post_title' => esc_html__('Booking #', 'wp_event_booking') . $booking_id . esc_html__(' - Event: ', 'wp_event_booking') . get_the_title($event_id) . esc_html__(' - Customer: ', 'wp_event_booking') . $user_info->display_name));
...
add_action('wpeb_event_multi_email_booking', 'fnc_wpeb_event_multi_email_booking');
function fnc_wpeb_event_multi_email_booking($post)
{
    global $typenow, $pagenow;
    ?>
    <div>
        <label class="label-multi_email_booking"><?php $wpeb_multi_email_booking = esc_attr(get_post_meta($post->ID, 'wpeb_multi_email_booking', true));?>
        <input type="checkbox" name="wpeb_multi_email_booking" value="1" <?php echo ($wpeb_multi_email_booking == '1' ? 'checked="checked"' : ''); ?> /><?php esc_html_e('Allow multiple bookings on the same email', 'wp_event_booking');?></label>
        <p><span class="tooltip description"><?php esc_html_e('If disabled, only one booking per email will be allowed.', 'wp_event_booking');?></span></p>
    </div>
<?php
}
...
function fnc_wpeb_event_date_time($post)
{
    ...
    if (in_array($typenow, array('cpt_events')) && $pagenow == 'post-new.php') {
        if (empty($start_date)) {
            $start_date = gmdate($dateFormat);
        }
        if (empty($end_date)) {
            $end_date = gmdate($dateFormat);
        }
        if (empty($start_time)) {
            $start_time = '09:00am';
        }
        ...
        if ($_EventStartDate) {
            ...
            $start_time = gmdate('H:i:s', strtotime($_EventStartDate));
        }
        if ($_EventEndDate) {
            $end_date = gmdate($dateFormat, strtotime($_EventEndDate));
            $end_time = gmdate('H:i:s', strtotime($_EventEndDate));
        }
	...
	<p class="div_date_time">
      <input type="text" name="txt_start_date[]" id="txt_start_date" class="date start" value="<?php echo esc_html($start_date); ?>" />
      <input type="text" name="txt_start_time[]" id="txt_start_time" <?php echo esc_html($time_style); ?> class="time start" value="<?php echo esc_html($start_time); ?>" /> to
      <input type="text" name="txt_end_time[]" id="txt_end_time" <?php echo esc_html($time_style); ?> class="time end" value="<?php echo esc_html($end_time); ?>" />
      <input type="text" name="txt_end_date[]" id="txt_end_date" class="date end" value="<?php echo esc_html($end_date); ?>" />
      <span><input type="checkbox" name="chk_all_day_event[]" id="chk_all_day_event" value="yes" onchange="show_hide_time_textbox()" <?php if ($all_day_event == 'yes') {
        echo 'checked';
        }?>><?php esc_html_e(' All Day Event', 'wp_event_booking');?></span>
    </p>
<?php
}
...
function fnc_wpeb_sign_up_AttendantFields_callback($signup_AttendantFields)
{
    $AttendantFields = array(
        array('field_option' => 'show_attendant_first_name',
            ...
            'field_title' => esc_html__('Attendant first name', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        array('field_option' => 'show_attendant_last_name',
            ...
            'field_title' => esc_html__('Attendant last name', 'wp_event_booking'),
            'field_type' => 'text',
        ),
        ...
        array('field_option' => 'show_attendant_e_mail_address',
            ...
            'field_title' => esc_html__('Attendant e-mail address', 'wp_event_booking'),
            'field_type' => 'email',
        ),
...
{
    $event_title = $event_date = $event_time = $user_name = $user_phone = $user_address = $user_email = $event_price = $event_location = $credentials = $supplier_name = '';
    $args = array(
    ...
    if ($the_query->have_posts()) {
        while ($the_query->have_posts()) {
            ...
            $supplier_name = get_the_author();
            $event_date = gmdate($dateWithYearFormat, strtotime($_EventStartDate));
            $event_time = gmdate(get_option('timeFormat'), strtotime($_EventStartDate));
    ...
    if (!empty($_eventDates)) {
        ...
        foreach ($_eventDates as $_eventDate) {
            ...
            if ($_EventStartDate) {
                $start_date = gmdate($dateWithYearFormat, strtotime($_EventStartDate));
                $start_time = gmdate(get_option('timeFormat'), strtotime($_EventStartDate));
            }
            if ($_EventEndDate) {
                $end_date = gmdate($dateWithYearFormat, strtotime($_EventEndDate));
                $end_time = gmdate(get_option('timeFormat'), strtotime($_EventEndDate));
            }
            if (gmdate('Y', strtotime($_EventStartDate)) > gmdate('Y')) {
    ...
    foreach ($customerList as $CL) {
        ...
        if ($user_name) {
            $user_details .= '<p>' . esc_html__('User Name', 'wp_event_booking') . ' : ' . $user_name . '</p>';
        }
        if ($user_phone) {
            $user_details .= '<p>' . esc_html__('Phone', 'wp_event_booking') . ' : ' . $user_phone . '</p>';
        }
        if ($user_address) {
            $user_details .= '<p>' . esc_html__('Address', 'wp_event_booking') . ' : ' . $user_address . '</p>';
        }
        if ($user_email) {
            $user_details .= '<p>' . esc_html__('Email', 'wp_event_booking') . ' : ' . $user_email . '</p>';
        }
        if ($booking_id) {
            $user_details .= '<p>' . esc_html__('Booking Id', 'wp_event_booking') . ' : ' . $booking_id . '</p>';
        }
        $user_details .= '<hr>';
    }
    if ($mail_to == 'admin') {
    ...
    if (!empty($to) && is_email($to)) {
        $placeholders = array('%event_title%', '%event_date_time%', '%event_price%', '%event_location%', '%user_details%', '%supplier_name%');
        ...
        $values = array($event_title, $event_date_time, $event_price, $event_location, $user_details, $supplier_name);
        if (!empty($company_details)) {
...
{
    $post_id = sanitize_text_field(wp_unslash($_POST['event_id']));
...
{
    if ($_GET && !empty($_GET['page']) && sanitize_text_field(wp_unslash($_GET['page'])) == 'attendees') {
    ...
    if ($_GET && !empty($_GET['event_id'])) {
        ...
        if (!empty($booking_ids)) {
            ...
            if (!empty($user_query->get_results())) {
                ...
                $event_start_date = gmdate($date_format_for_display . $date_time_separator_for_display . $time_format_for_display, strtotime(get_post_meta($event_id, '_eventStartDate', true)));
				...
				<p class="h3"><a href="<?php echo esc_url(admin_url('post.php') . '?post=' . $event->ID . '&action=edit'); ?>"><?php echo esc_html($event_title); ?></a></p>
                    <p class="h4"><?php echo esc_html($event_start_date); ?></p>
                    <p class="h5"><?php esc_html_e('Total Booking :', 'wp_event_booking');?><?php echo esc_html($user_query->get_total()); ?></p>
					<p class="h5"><?php echo esc_html($location_city); ?></p>

                    <a class="button button-primary" href="<?php echo esc_url($permalink); ?>" role="button"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a>

                    <?php $current_url = "//" . sanitize_text_field(wp_unslash($_SERVER['HTTP_HOST'])) . sanitize_text_field(wp_unslash($_SERVER['REQUEST_URI']));?>

                    <a class="button button-primary" target="_blank" href="<?php echo esc_url(add_query_arg(array('wpeb_action' => 'download_list'), $current_url)); ?>" role="button"><i class="fa fa-download" aria-hidden="true"></i> <?php esc_html_e('Download', 'wp_event_booking');?></a>

                    <table class="tablesorter" id="user_details">
                        <thead>
                            <tr>
                                <th class="header"><?php echo esc_html__('Attendant name', 'wp_event_booking'); ?></th>
                                <th class="header"><?php echo esc_html__('Attendant Email', 'wp_event_booking'); ?></th>
                                <th class="header"><?php echo esc_html__('Phone Number', 'wp_event_booking'); ?></th>
                        ...
                        <tbody>
                            <?php foreach ($user_query->get_results() as $user) {
                                    ...
                                    ?>
                                    <tr>
                                    <td><a href="<?php echo esc_url(admin_url('user-edit.php') . '?user_id=' . $user->ID); ?>"><?php echo esc_html($user->display_name); ?></a></td>
        ...
        echo esc_html__('Something Wrong...', 'wp_event_booking');?>
        <a class="button button-primary" href="<?php echo esc_url($permalink); ?>" role="button"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back</a><?php
}?></div><?php
}
...
function wpeb_action_download_list_callback()
{
    ...
    if ($_GET && isset($_GET['wpeb_action']) && sanitize_text_field(wp_unslash($_GET['wpeb_action'])) == 'download_list') {
        if ($_GET && !empty($_GET['event_id'])) {
            global $wpdb; // this is how you get access to the database
            $event_id = sanitize_text_field(wp_unslash($_GET['event_id']));
            $output = $error = '';
            ...
            if (!empty($booking_ids)) {
                ...
                if (!empty($user_query->get_results())) {
                    ...
                    $output = '<div class="event_signup_users">
                        ...
                        <h5 class="h5">' . esc_html__('Total Booking :', 'wp_event_booking') . $user_query->get_total() . '</h5>
                        <p class="">' . $location_city . '</p>
                    <table class="tablesorter" id="user_details">
                    <thead>
                        <tr>
                            <th class="header">' . esc_html__('Attendant name', 'wp_event_booking') . '</th>
                            <th class="header">' . esc_html__('Attendant Email', 'wp_event_booking') . '</th>
                            <th class="header">' . esc_html__('Phone Number', 'wp_event_booking') . '</th>
        ...
        $options->set('isRemoteEnabled', true); // Enable fetching remote resources

        // Initialize DomPDF
        $dompdf = new Dompdf($options);

        // Combine stylesheet and HTML content
        $html = "<style>$stylesheet</style>$output";
        ...
        $dompdf->stream(gmdate('Y-m-d').'-attendees.pdf', ['Attachment' => true]); // `Attachment => false` opens in the browser
        exit();
    }
}
...
{
    if ((is_admin()) && (isset($_GET['post_type']) && sanitize_text_field(wp_unslash($_GET['post_type'])) == 'event_booking') && (isset($_GET['booking_status']) && sanitize_text_field(wp_unslash($_GET['booking_status'])) != 'cancelled')) {
        ...
        $allCalss = '';
        $publishCalss = '';
        if (isset($_GET['all_posts']) && sanitize_text_field(wp_unslash($_GET['all_posts'])) == '1') {
            $allCalss = 'class="current"';
        } 
        if (isset($_GET['post_status']) && sanitize_text_field(wp_unslash($_GET['post_status'])) == 'publish') {
 ...
 * ajax action
 */
function wpeb_location_region_preference() {
    $user_id = get_current_user_id();
    $meta_key = 'wpeb_location_region_screen_option';

    if (isset($_POST['preference'])) {
        update_user_meta($user_id, $meta_key, sanitize_text_field(wp_unslash($_POST['preference'])));
    }
    
    wp_die();
}
add_action('wp_ajax_wpeb_location_region_preference', 'wpeb_location_region_preference');

/**
 * load region menu first time
 */
function wpeb_enable_location_region_in_menus() {
    global $pagenow;
    ...
    if ($pagenow === 'nav-menus.php') {
        ...
        // Check if the user meta exists
        $wpeb_location_region_screen_option = get_user_meta($user_id, $meta_key, true);

        // If the user has not unchecked the option, enable it by default
        if ($wpeb_location_region_screen_option !== 'unchecked') {
            ?>
            <script type="text/javascript">
                jQuery(document).ready(function($) {
                    var locationRegionCheckbox = $('#add-post-type-location_region-hide');
                    if (!locationRegionCheckbox.is(':checked')) {
                        locationRegionCheckbox.trigger('click');
                    }

                    // Save user preference when checkbox is unchecked
                    locationRegionCheckbox.on('change', function() {
                        if (!$(this).is(':checked')) {
                            $.post(ajaxurl, {
                                action: 'wpeb_location_region_preference',
...
add_action('admin_footer', 'wpeb_enable_location_region_in_menus');
