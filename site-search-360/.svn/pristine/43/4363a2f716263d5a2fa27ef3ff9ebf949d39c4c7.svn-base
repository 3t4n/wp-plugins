...
class SiteSearch360Plugin
{
    ...
    private function sanitize_attribute($attr) {
    ...
    public function searchbox_shortcode($attrs) {
		...
		foreach($keys as $key) {
			if (isset($a[$key]) && $a[$key] !== NULL) {
				$dataStr = $dataStr . ' data-ss360-' . $key . '="[' . $this->sanitize_attribute($a[$key]) . ']"';
			}
		}

		$placeholder = '';
		if ($a['placeholder'] != null) {
			$placeholder = ' data-ss360-keep-placeholder="true" placeholder="' . $this->sanitize_attribute($a['placeholder']) . '"'; 
		}
		
        return '<input class="ss360-searchbox" type="search"'. $placeholder .' style="'.$this->sanitize_attribute($a['style']).'"'.($this->getType()!='full'?' name="s"':'').$dataStr.'>';
    }

    public function searchbutton_shortcode($attrs, $content = null) {
        $a = shortcode_atts(array('style' => ''), $attrs);
        $text = $content != null ? $content : '';
        return '<button class="ss360-searchbutton" style="'.$this->sanitize_attribute($a['style']).'">'.$text.'</button>';
    }
    ...
    public function form_shortcode($attrs, $content = null) {
        ...
        $result = $result . ' style="'.$this->sanitize_attribute($a['style']).'">';
        $result = $result . $inner;
        if($this->getType() != 'full') {
            $result = $result . '</form>';
        } else {
    ...
    public function embed_shortcode($attrs) {
        ...
        return '<section class="ss360-search-results-block" style="'.$this->sanitize_attribute($a['style']).'"></section>';
    }
        ...
        if (!function_exists('ss360Config'))   {
            ...
            function ss360Config()
            {
                ...
                if ($type != "filter"  && $should_inject_search) {
                    ...
                    ?>
						<!-- Site Search 360 WP v<?php $ss360_v = defined('SITESEARCH360_VERSION') ? SITESEARCH360_VERSION : '2.1.7'; echo $ss360_v; ?> -->
