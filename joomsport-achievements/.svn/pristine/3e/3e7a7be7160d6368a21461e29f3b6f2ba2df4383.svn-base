...
jQuery(document).ready(function(){  
    ...
    jQuery('body').on('click', '.jsAchvShrtPlayerAdd', function(){

        var shortcode = '[jsachPlayer';

        if(jQuery("#shrtc_pl_player_id").val() && jQuery("#shrtc_pl_player_id").val() != '0'){
            shortcode += ' playerid="'+jQuery('#shrtc_pl_player_id').val()+'"';
        }else{
            return '';
        }
        shortcode += ' seasonid="'+jQuery('select[name="shrtc_pl_season_id"]').val()+'"';
        shortcode += ' ordering='+jQuery("input[name='jsAchvStOrder']").val();
    ...
    jQuery('body').on('click', '.jsAchvShrtCalendarAdd', function(){

        var shortcode = '[jsachCalendar';

        if(jQuery("#shrtc_pl_season_id").val() && jQuery("#shrtc_pl_season_id").val() != '0'){
            shortcode += ' seasonid="'+jQuery('#shrtc_pl_season_id').val()+'"';
        }else{
            return '';
        }
        shortcode += ' navigation='+jQuery("input[name='jsAchvSDD']").val();
    ...
    jQuery('body').on('click', '.jsAchvShrtStageAdd', function(){

        var shortcode = '[jsachStage';

        if(jQuery("#shrtc_st_stage_id").val() && jQuery("#shrtc_st_stage_id").val() != '0'){
            shortcode += ' stageid="'+jQuery('#shrtc_st_stage_id').val()+'"';
        }else{
            return '';
        }
    ...
    jQuery('body').on('click', '.jsAchvShrtSeasonAdd', function(){

        var shortcode = '[jsachSeason';

        if(jQuery("#shrtc_seas_season_id").val() && jQuery("#shrtc_seas_season_id").val() != '0'){
            shortcode += ' seasonid="'+jQuery('#shrtc_seas_season_id').val()+'"';
        }else{
            return '';
        }
    ...
    jQuery('body').on("change","#shrtc_stages_season_id", function(e){
        var seasonID = jQuery(this).val();
        var data = {
            'action': 'joomsport_achv_season_stages',
            'seasonID': seasonID
        };

        jQuery.post(ajaxurl, data, function(response) {

            jQuery("#shrtc_st_stage_id").html(response);

        });
    });

    jQuery('a[href="admin.php?page=achievements-pro-link"]').attr("target","_blank");
    jQuery('a[href="admin.php?page=achievements-pro-link"]').text('Upgrade  ➤');
});
