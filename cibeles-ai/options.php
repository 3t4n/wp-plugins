...
add_filter('wp_kses_allowed_html', function($allowed_tags, $context) {
    // Verifica si la etiqueta 'span' ya está permitida
    if (isset($allowed_tags['span'])) {
        // Añade el atributo 'id' a la etiqueta 'span'
        $allowed_tags['span']['id'] = [];
    } else {
        // Si 'span' no está permitida, la añade con el atributo 'id' y otros atributos comunes
        $allowed_tags['span'] = [
            'id'    => [],
            'class' => [],
            'style' => [],
            // Añade otros atributos que consideres necesarios
        ];
    }
