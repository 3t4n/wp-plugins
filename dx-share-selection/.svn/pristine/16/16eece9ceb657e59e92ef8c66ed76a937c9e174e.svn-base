		...
		<form method="post">
			<div class="content">
				<div class="dxss-settings">
					...
					<section class="section color-settings">
						<div class="colors-wrapper">
							...
							<p>
								...
								<input name="dxss_bgColor" id="dxss_bgColor" class="color dxss-settings-input" type="text" value="<?php echo esc_attr( $dxss_settings['bgColor'] ); ?>"/>
							</p>
							<p>
								<label class="center-label" for="dxss_titleColor"><?php esc_html_e( 'Title Background color', 'dxss' ); ?></label>
								<input name="dxss_titleColor" id="dxss_titleColor" class="color dxss-settings-input" type="text" value="<?php echo esc_attr( $dxss_settings['titleColor'] ); ?>"/>
							</p>
							<p>
								<label class="center-label" for="dxss_titleTextColor"><?php esc_html_e( 'Title Text Color', 'dxss' ); ?></label>
								<input name="dxss_titleTextColor" id="dxss_titleTextColor" class="color dxss-settings-input" type="text" value="<?php echo esc_attr( $dxss_settings['titleTextColor'] ); ?>"/>
							</p>
							<p>
								<label class="center-label" for="dxss_hoverColor"><?php esc_html_e( 'Hover Color', 'dxss' ); ?></label>
								<input name="dxss_hoverColor" id="dxss_hoverColor" class="color dxss-settings-input" type="text" value="<?php echo esc_attr( $dxss_settings['hoverColor'] ); ?>"/>
							</p>
							<p>
								<label class="center-label" for="dxss_textColor"><?php esc_html_e( 'Text Color', 'dxss' ); ?></label>
								<input name="dxss_textColor" id="dxss_textColor" class="color dxss-settings-input" type="text" value="<?php echo esc_attr( $dxss_settings['textColor'] ); ?>"/>
							</p>
							<p class="p-with-small">
								<label class="smalltext-label" for="dxss_extraClass"><?php esc_html_e( 'Extra Class', 'dxss' ); ?></label>
								<span class="inner-span">
									<input name="dxss_extraClass" id="dxss_extraClass" class="dxss-settings-input" type="text" value="<?php echo esc_attr( $dxss_settings['extraClass'] ); ?>"/>
									<small class="smallText"><?php esc_html_e( 'This class will be added to the main Share Selection div', 'dxss' ); ?></small>
								</span>
							</p>
						</div>
						<div class="restore-button-parent"><input type="button" id="restore-customize" class="toolBt button dxss-restore-button" value="<?php esc_attr_e( 'Restore Customize Settings', 'dxss' ); ?>" /></div>
					</section>
					...
					<section class="section optional-settings">
						...
						<p class="p-with-small">
							...
							<span class="inner-span">
								...
								<small class="smallText"><?php echo wp_kses_data( __( 'Selected text will be truncated to this amount of characters when <code>%ts</code> is used in the URL', 'dxss' ) ); ?></small>
							</span>
						</p>
						<p class="p-with-small">
						...
						<p class="p-with-small">
							...
							<span class="inner-span">
								<input name="dxss_bitly_token" type="text" value="<?php echo ! empty( $dxss_settings['bitly_token'] ) ? esc_attr( DXSS_Encryption::$dumb_token_view ) : ''; ?>" autocomplete="off"/>
								<small class="smallText"><?php echo wp_kses_data( __( 'Bitly API Access Token. Used for <code>{surl}</code>', 'dxss' ) ); ?> <a href="https://bitly.is/accesstoken" target="_blank"><?php esc_html_e( 'Generate here', 'dxss' ); ?></a></small>
							</span>
						</p>
						<p class="p-checkbox">
							<label><?php esc_html_e( 'Deactivate on Desktop', 'dxss' ); ?></label>
							<input type="checkbox" name="dxss_deact_desktop" value="activate" <?php echo checked( 'activate', $dxss_settings['deactDesktop'], false ); ?>>
						</p>
						<p class="p-checkbox">
							<label><?php esc_html_e( 'Deactivate on Mobile', 'dxss' ); ?></label>
					...
					<?php wp_nonce_field( 'dxss_save_settings', 'dxss_settings_save_nonce' ); ?>
	...
	<div class="modal-overlay">
		<div class="modal custom-button-window">
			...
			<div class="wrap">
				...
				<div class="customURL">
					...
					<label for="dxss_custom_url"><?php esc_html_e( 'Use %s in the URL for the selected text.', 'dxss' ); // phpcs:ignore ?></label>
		...
		<div class="modal search-button-window">
			...
			<div class="wrap">
				...
				<div class="searchURL">
					...
					<label for="dxss_search_url"><?php esc_html_e( 'Use %s in the URL for the selected text.', 'dxss' ); // phpcs:ignore ?></label>
		...
		<div class="modal helpWindow">
			...
			<div class="wrap">
				<p><?php esc_html_e( 'The format for adding a custom button to the widget is', 'dxss' ); ?><br/>
					<?php echo wp_kses_data( __( '<code>Name of the button</code>, <code>Share / Search URL</code>, <code>Icon URL</code>', 'dxss' ) ); ?></p>

				<b><?php esc_html_e( 'Note:', 'dxss' ); ?></b><br/>
				<ol>
					<li><?php echo wp_kses_data( __( 'Use <code>%s</code> in the Share/Search URL to use the selected text.', 'dxss' ) ); // phpcs:ignore ?></li>
					<li><?php echo wp_kses_data( __( 'Use <code>%ts</code> in the URL to use the truncated selected text (115 characters by default. Used in Twitter URL).', 'dxss' ) ); ?></li>
					<li><?php echo wp_kses_data( __( 'Use the text <code>favicon</code> in the Icon URL to automatically get the button icon.', 'dxss' ) ); ?></li>
					<li><?php echo wp_kses_data( __( 'Use <code>{url}</code> in the URL to get the current page url.', 'dxss' ) ); ?></li>
					<li><?php echo wp_kses_data( __( 'Use <code>{surl}</code> in the URL to get the shortened current page URL.', 'dxss' ) ); ?></li>
					<li><?php echo wp_kses_data( __( 'Use <code>{title}</code> in the URL to get the current page title.', 'dxss' ) ); ?></li>
				</ol>
		...
		<div class="modal wpsrBox">
			<?php if ( class_exists( 'WPSR_Lists' ) ) : ?>
				...
				<small class="smallText"><?php esc_html_e( 'These buttons are taken from the WP Socializer plugin. You can now use these buttons for DX Share Selection', 'dxss' ); ?></small>
			...
			<?php else : ?>
				...
				<hr/>
				<p><?php echo wp_kses_data( __( 'Sorry, you need to install <b>WP Socializer plugin</b> to get the additional social buttons links and data.', 'dxss' ) ); ?></p>
				...
				<b><?php esc_html_e( 'Note:', 'dxss' ); ?></b><br/>
				<small class="smallText"><?php echo wp_kses_data( __( 'DX share Selection requires to install WP Socializer to link the additional 98 buttons. <a href="https://wordpress.org/plugins/wp-socializer/" target="_blank">See here</a> for more info', 'dxss' ) ); ?></small>
			<?php endif; ?>
		</div>
	</div>
	<div class="sidebars">
		<?php require_once 'sidebar-right-help.php'; ?>
		<?php require_once 'sidebar-right-plugins.php'; ?>
	</div>
</div>
