...
class DXSS_Option_Helper {
		...
		if ( empty( $dxss_settings ) ) {
			...
			$dxss_settings = array_merge(
				...
				array(
					...
					'lists' => "Search,$search_url,favicon\nTwitter,https://twitter.com/intent/tweet?text=%ts {url},favicon",
				)
			);
		}
	...
	public static function get_bitly_token( $bitly_token ) {
		...
		// If the user has set a token before encryption was implemented and it's not changed, encrypt the token before saving.
	...
	/**
	 * Return the array keys of settings, which are colors.
	 *
	 * @return string[]
	 */
	public static function get_color_options_keys() {
		return array(
			'borderColor',
			'bgColor',
			'titleColor',
