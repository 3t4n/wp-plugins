...
class S123_Invoice extends S123_BaseController
{
    ...
    */
    ...
    public function __construct($order, $api)
    {
        ...
        $this->currencyRate = $order->get_meta('_wcpay_multi_currency_order_exchange_rate') ?
            round($order->get_meta('_wcpay_multi_currency_order_exchange_rate'), 4) :
            1;
    }
    ...
    */
    private function s123_invoiceProducts($item): array
    {
        ...
        if ($item->get_subtotal() !== $item->get_total()) {
            $invoiceProduct['discount'] = $this->formatNumber($fixedDiscount);
            $invoiceProduct['total_discount'] = $this->formatNumber($fixedDiscount);
            $invoiceProduct['discount_type'] = 'fixed';
        }

        $invoiceProduct['total_vat'] = $this->formatNumber($item->get_total_tax());
        $invoiceProduct['total_vat_wo_discount'] = $this->formatNumber($item->get_total_tax());
        $invoiceProduct['total_wo_vat_and_discount'] = $this->formatNumber((float)$this->formatNumber($item->get_total()) + (float)$invoiceProduct['total_discount'], false);
        $invoiceProduct['total'] = $this->formatNumber((float)$invoiceProduct['total_wo_vat_and_discount'] + (float)$invoiceProduct['total_vat'] - $invoiceProduct['total_discount'], false);
    ...
    */
    private function s123_addShippingToPayment(): array
    {
        ...
        $invoiceShipping['total_vat'] = $this->formatNumber($this->order->get_shipping_tax());
        $invoiceShipping['total_vat_wo_discount'] = $this->formatNumber($this->order->get_shipping_tax());
        $invoiceShipping['total_wo_vat_and_discount'] = $this->formatNumber($this->order->get_shipping_total());
    ...
    private function s123_addOrderFee($fee): array
    {
        ...
        $orderFee['price'] = $this->formatNumber($fee->get_total());
        $orderFee['quantity'] = 1;
        $orderFee['total'] = $this->formatNumber((float)$fee->get_total() + (float)$fee->get_total_tax());
        $orderFee['unit_id'] = $this->settings['unit_id'];
        $orderFee['total_vat'] = $this->formatNumber($fee->get_total_tax());
        $orderFee['total_vat_wo_discount'] = $this->formatNumber($fee->get_total_tax());
        $orderFee['total_wo_vat_and_discount'] = $this->formatNumber($fee->get_total());
    ...
    */
    ...
    private function formatNumber($value, $convertCurrency = true): string
    {
        if (!$convertCurrency) {
            return number_format($value, $this->wooRoundingDecimals, '.', '');
        }

        return number_format($value / $this->currencyRate, $this->wooRoundingDecimals, '.', '');
    }
    ...
    {
        if (
            $this->formatNumber($this->order->get_total(), false) === $this->formatNumber(0, false) &&
