...
function chpter_payment_init()
{
    ...
    class WC_Gateway_Chpter extends WC_Payment_Gateway 
    {
       ...
       public function __construct() {
               ...
               $this->method_title = __('chpter 3.1.5', 'woocommerce');
        ...
        public function process_payment($order_id) {
            try {
                ...
                if ($payment_type == 'MPesa' && empty($tel)) {
                    throw new Exception(__("Enter your M-Pesa Number", 'woocommerce'));
                }
                ...
                $cart_products_objects = $this->get_cart_items($woocommerce->cart, $order);
                
                $payload = array(
                    "customer_details" => array(
                        "full_name" => sanitize_text_field($order->get_billing_first_name() . ' ' . $order->get_billing_last_name()),
                        "location" => sanitize_text_field($order->get_billing_city()),
                        "email" => sanitize_email($order->get_billing_email()),
                        "phone_number" => wc_sanitize_phone_number($updatedPhone),
                    ),
                    "products" => $cart_products_objects,
                    "amount" => array(
                        "currency" => sanitize_text_field(strtolower(get_woocommerce_currency())),
                        "delivery_fee" => (float)$order->get_shipping_total(),
                        "discount_fee" => (float)$order->get_discount_total(),
                        "total" => (float)$order->get_total(),
                    ),
                    "callback_details" => array(
                        "order_id" => sanitize_text_field($order_id),
                        "shop_url" => sanitize_url(get_site_url()),
                        "callback_url" => sanitize_url(get_site_url() . "/?wc-api=callback")
                    )
                );
    
                $apiUrl = $payment_type == "Card" ? $cardApiUrl : $mpesaApiUrl;
                $response = $this->api_post_request($apiUrl, $payload);
    
                if (is_wp_error($response)) {
                    throw new Exception($response->get_error_message());
                }
    
                $decodedResp = json_decode(wp_remote_retrieve_body($response), true);
    
                if (!isset($decodedResp["success"])) {
                    throw new Exception(__("Invalid response from payment gateway", 'woocommerce'));
                }
    
                if (!$decodedResp["success"]) {
                    throw new Exception(isset($decodedResp["message"]) ? $decodedResp["message"] : __("Payment processing failed", 'woocommerce'));
                }
    
                // Handle successful responses
                if ($payment_type == "Card" && isset($decodedResp["checkout_url"])) {
                    return array(
                        'result' => 'success',
                ...
                if ($payment_type == "MPesa") {
                    $status = $this->get_option('orderStatus') == "Completed" ? 'wc-completed' : 'wc-processing';
                ...
                throw new Exception(__("Invalid payment type or missing checkout URL", 'woocommerce'));
        ...
        private function get_cart_items($cart, $order) {
            if (empty($cart->get_cart())) {
                return array(array(
                    "product_name" => 'WooCommerce Product',
                    "quantity" => 1,
            ...
            foreach ($cart->get_cart() as $item => $values) {
                ...
                if ($_product->is_downloadable()) {
                    foreach ($_product->get_downloads() as $download) {
                ...
                $items[] = array(
                    ...
                    "unit_price" => (float)($_product->get_price() ?: $order->get_total()),
        ...
        protected function api_post_request($apiUrl, $arrayObj) {
            $token = sanitize_text_field($this->get_option('token'));
            
            return wp_remote_post(sanitize_url($apiUrl), array(
                'method' => 'POST',
                'timeout' => 180,
                'headers' => array(
