...
if(is_user_logged_in()){
    ...
    if($product_id) $product_id = base64_decode($product_id);
    $author_id = get_post_field( 'post_author', $product_id );
    if($current_user_product_access == 1 && ($author_id == $current_user->ID || $role == "administrator")){
...
?>
    <div class="bt-plugin-context" id="woocommerce-product-data">
        <div class="back-product-list">
            <a href="<?php echo esc_url( home_url('/my-account/product-list') ); ?>" class="bt-button-default bt-button-primary">Back To Product List</a>
        </div>
