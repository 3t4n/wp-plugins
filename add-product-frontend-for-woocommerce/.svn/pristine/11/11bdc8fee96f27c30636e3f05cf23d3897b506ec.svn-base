...
if(!class_exists('APFFW_Bytes_Add_Product_Frontend_For_Woocommerce_Public')){
	class APFFW_Bytes_Add_Product_Frontend_For_Woocommerce_Public {
		...
		public function apffw_product_account_product_list_body_class($classes){

			if ( get_query_var( 'edit-product-form', false) !== false  ) {
		           $classes[] = 'hide-account-menu';
		    }
		   return $classes;
		}

		public function apffw_product_account_remove_sidebar(){
			if ( get_query_var( 'edit-product-form', false) !== false  ) {
		...
		public function apffw_publish_or_update_product($post){
		    ...
		    $frontend_product_additional_fields = get_option('frontend_product_save_update_email');
		    if(get_post_type($post) == 'product' && (sanitize_email(get_bloginfo('admin_email')) !== $author_email) && $frontend_product_additional_fields == 1){
