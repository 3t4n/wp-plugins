...
if(!class_exists('APFFW_Bytes_Add_Product_Frontend_For_Woocommerce')){
	class APFFW_Bytes_Add_Product_Frontend_For_Woocommerce {
		...
		private function apffw_define_public_hooks() {
			...
			$this->loader->add_filter('wp',$plugin_public, 'apffw_product_account_remove_sidebar' ); // Remove sidebar from product edit page
			$this->loader->add_filter('body_class',$plugin_public, 'apffw_product_account_product_list_body_class', 10,1 ); // add custom class for product edit page
		}
