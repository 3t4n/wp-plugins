<?php
	...
	class AS_CM_Controllers_Options extends AS_CM_Classes_Controller {
		...
		public static function show_notice_if_empty_embed_code() {
			...
			if ( empty( $embed_code ) ) {
				AS_CM_Services_Notice::add_notice( 'empty-embed-code', __( "Please enter AdSimple ID to configure plugin and inserting popup to site.", 'adsimple-cookie-manager-for-wp' ),
					[ 'general', 'media', 'posts' ], [
						'href' => self::get_page_url(),
						'text' => __( 'Set AdSimple ID', 'adsimple-cookie-manager-for-wp' ),
					] );
			}
		}
		...
		public static function save_adsimple_id_after_install() {
			...
			if ( empty( $options[ $field['key'] ] ) ) {
				AS_CM_Services_Notice::add_notice( 'status-save-adsimple-id', __( "AdSimple ID can not be empty.", 'adsimple-cookie-manager-for-wp' ) );
			...
			try {
				AS_CM_Services_Notice::add_notice( 'status-save-adsimple-id', __( "We successfully saved your AdSimple ID.", 'adsimple-cookie-manager-for-wp' ) );
		...
		public static function save_cache_options_fields() {
			...
			AS_CM_Services_Notice::add_notice( 'status-save-cache-options', __( "We successfully saved cache options.", 'adsimple-cookie-manager-for-wp' ) );
		}
		...
		public static function save_fields() {
			...
			if ( $_GET['page'] != static::get_settings_page_key() ) {
				return;
			}

			if ( ! current_user_can( 'administrator' ) ) {
				AS_CM_Services_Notice::add_notice( 'status-save-adsimple-id', __( "You don't have access to make this.", 'adsimple-cookie-manager-for-wp' ) );
		...
		public static function init_options_page() {
			add_menu_page( __( 'Consent Manager', 'adsimple-cookie-manager-for-wp' ), __( 'Consent Manager', 'adsimple-cookie-manager-for-wp' ), 'manage_options', AS_CM_Controllers_Options::get_settings_page_key(),
		...
		/**
		 * get dashboard settings page key
		 *
		 * @param string $name
		 *
		 * @return string
		 ...
		 *
		 */
		public static function get_settings_page_key( $name = '' ) {
			return 'as_cm_settings' . ( $name !== '' ? ('-' . $name) : '');
		}
		...
		public static function get_page_url( $name = '', $args = [] ) {
			return AS_CM_Helpers_Transfer::get_link_with_params( array_merge( [
				'page' => static::get_settings_page_key( $name ),
		...
		public static function get_fields( $key = '' ) {
			$fields = [
				[
					'key'     => 'adsimple_id',
					'label'   => __( 'AdSimple ID', 'adsimple-cookie-manager-for-wp' ),
					'attr'    => [
						'type' => 'text',
					],
					'default' => '',
				...
				[
					'key'     => 'embed_code',
					'label'   => __( 'Embed code', 'adsimple-cookie-manager-for-wp' ),
					'attr'    => [
						'type' => 'textarea',
					],
					'default' => '',
		...
		public static function get_available_period_types() {
			return [
				'DAY_IN_SECONDS'    => __( 'days', 'adsimple-cookie-manager-for-wp' ),
				'HOUR_IN_SECONDS'   => __( 'hours', 'adsimple-cookie-manager-for-wp' ),
				'MINUTE_IN_SECONDS' => __( 'minutes', 'adsimple-cookie-manager-for-wp' ),
			];
		}
	}