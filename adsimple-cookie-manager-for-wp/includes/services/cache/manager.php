	...
	class AS_CM_Services_Cache_Manager extends AS_CM_Classes_Controller {
		...
		public static function handler_request_clear_cache() {
			...
			if ( ! static::is_cache_available() ) {
				AS_CM_Services_Notice::add_notice( 'status-clear-cache', __( "Cache disabled. You can not clear it.", 'adsimple-cookie-manager-for-wp' ) );

				return;
			}

			if ( static::delete_cache() ) {
				AS_CM_Services_Notice::add_notice( 'status-clear-cache', __( "Cache was cleared.", 'adsimple-cookie-manager-for-wp' ) );
			} else {
				AS_CM_Services_Notice::add_notice( 'status-clear-cache', __( "Error clear cache.", 'adsimple-cookie-manager-for-wp' ) );
			}
		}
