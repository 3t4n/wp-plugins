<?php
	/*
	*  Plugin Name:     Cookie Banner - Consent Manager by AdSimple (DSGVO/GDPR)
	*  Description:     With this Cookie Banner WordPress plugin, you can install a cookie banner on your WordPress website in just a few steps.
	*  Version:         2.0.17
	*  Author:          AdSimple
	*  Author URI:      https://www.adsimple.at
	*  Text Domain:     adsimple-cookie-manager-for-wp
	*  Domain Path:     /languages
	*/

	add_action( 'plugins_loaded', [ 'AS_CM_Manager', '_instance' ], 20 );

	register_activation_hook( __FILE__, [ 'AS_CM_Manager', 'activate_plugin' ] );

	register_deactivation_hook( __FILE__, [ 'AS_CM_Manager', 'deactivate_plugin' ] );
	class AS_CM_Manager {
		...
		private function __construct() {
			...
			add_action( 'init', [ $locale, 'load_textdomain' ], 30 );

			add_action( 'plugins_loaded', [ __CLASS__,	'load_plugin_parts' ], 60 );
		}
		...
		protected static function init_environment_error() {
			$message = sprintf( __( '<p>The <strong>%s</strong> plugin requires %s version %s or greater.</p>', 'adsimple-cookie-manager-for-wp' ), self::get_plugin_info( 'Name' ),
				self::get_environment_status(), self::get_environment( self::get_environment_status() ) );

			deactivate_plugins( plugin_basename( __FILE__ ) );

			wp_die( $message, __( 'Plugin Activation Error', 'adsimple-cookie-manager-for-wp' ), [ 'back_link' => true ] );
		}
