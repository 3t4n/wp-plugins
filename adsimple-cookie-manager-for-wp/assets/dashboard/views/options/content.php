	...
	<div class="as_cm-update-content as_cm-update-page__content">
		<form action="<?= AS_CM_Controllers_Options::get_page_url(); ?>" method="post">
			...
			<div class="as_cm-update-content__inner">
				<h2 class="as_cm-update-content__title"><?= sprintf( __( 'You use AdSimple Consent Manager %s!', 'adsimple-cookie-manager-for-wp' ), '<span>2.0</span>' ); ?></h2>
				<p class="as_cm-update-content__subtitle">
					<?= sprintf( __( 'Please enter your AdSimple ID, you can get it on %s.', 'adsimple-cookie-manager-for-wp' ),
				...
				<div class="as_cm-update-content__formpos">
					...
					<div class="as_cm-update-content__button">
						<button type="submit" class="as_cm-btn as_cm-btn--orange">
							<?= AS_CM_Controllers_Options::is_configured() ? __( 'Save', 'adsimple-cookie-manager-for-wp' ) : __( 'Set ID', 'adsimple-cookie-manager-for-wp' ); ?>
						</button>
					</div>
				</div>
				<div class="as_cm-update-content__notice">
					<?= AS_CM_Helpers_View::get_template_part( 'options/cache' ); ?>
					<p class="as_cm-update-content__notice-header"><strong><?= __( 'Cookie Overview', 'adsimple-cookie-manager-for-wp' ); ?></strong></p>
					<p><?= sprintf( __( 'Use the shortcode %s to show an ordered list of all cookies on a subpage of your website..', 'adsimple-cookie-manager-for-wp' ), '[' . AS_CM_Controllers_Shortcodes::SHORTCODE_COOKIE_LIST . ']' ); ?></p>
					<br/>
					<p>
						<?= sprintf( __( 'For include popup to site uses hook %s. You theme should have support of this hook.', 'adsimple-cookie-manager-for-wp' ), '<a href="https://developer.wordpress.org/reference/hooks/wp_head/" target="_blank"><strong>wp_head</strong></a>' ); ?>
					</p>
					<p>
						<?= sprintf( __( 'Current version of plugin %s.', 'adsimple-cookie-manager-for-wp' ), AS_CM_Manager::$version ); ?>
					</p>
				</div>
			</div>
			<!--/MAIN-->
