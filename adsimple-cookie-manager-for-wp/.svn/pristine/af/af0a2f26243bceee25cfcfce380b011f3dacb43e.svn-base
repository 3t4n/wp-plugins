		...
		<div class="as_cm-notice__button-block js-as_cm-notice-actions">
			<?php if( isset( $link['href'] ) && isset( $link['text'] ) ): ?>
				<a
					...
					data-confirm="<?= isset( $confirm ) && $confirm !== FALSE ? ( $confirm === TRUE ? __( 'Are you sure?', 'adsimple-cookie-manager-for-wp' ) : $confirm ) : ''; ?>"
				>
					<?= $link['text']; ?>
			...
			<?php endif; ?>
			<?= isset( $dismiss ) && $dismiss ? '<a href="#" class="as_cm-notice__button as_cm-notice__button_dismiss js-as_cm-notice-actions-dismiss" data-dismiss-url="'.$dismiss_url.'">'.__( 'DISMISS', 'adsimple-cookie-manager-for-wp' ).'</a>' : ''; ?>
		</div>
	<?php endif; ?>
</div>
<!--/NOTICE-->
