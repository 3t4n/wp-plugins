<?php
	...
	class AS_CM_Services_REST extends AS_CM_Classes_Controller {
		...
		public static function get_embed_code( $key, $domain, $types = array( 'notice', 'list' ) ) {
			...
			if ( $data === FALSE || ! is_array( $data ) ) {
				return new WP_Error( 'error_loading',
					sprintf( __( 'Error loading data from %s. Please repeat your request later.', 'adsimple-cookie-manager-for-wp' ),
			...
			switch ( $data['status'] ) {
				case 'without_pricing_plan':
					return new WP_Error( 'invalid_adsimple_id',
						sprintf( __( 'First you should select pricing plan on %s', 'adsimple-cookie-manager-for-wp' ),
						...
						array(
							'link' => array(
								...
								'text'   => __( 'Select pricing plan', 'adsimple-cookie-manager-for-wp' ),
								'target' => '_blank'
							)
						)
					);
				case 'invalid_adsimple_id':
					return new WP_Error( 'invalid_adsimple_id', __( 'Invalid AdSimple ID', 'adsimple-cookie-manager-for-wp' ) );
				...
				case 'success':
					if ( empty( $data['result'] ) ) {
						return new WP_Error( 'invalid_domain', sprintf( __( "Domain %s doesn't exist.", 'adsimple-cookie-manager-for-wp' ), $domain ) );
					}
					break;
			}
		...
		public static function get_full_url( $method, $key = null ) {
			return static::get_prepared_url_based_on_environment(
				sprintf(
					'%s/%s/%s/%s',
					static::SERVER_URL,
