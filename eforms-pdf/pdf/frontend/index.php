...
class Superaddons_Fdf_Create_frontend {
    ...
    function template_include($template) {
	    if( isset($_GET['pdf_preview']) ){
	        if( $_GET['pdf_preview'] == "preview") {
	             $template = SUPERADDONS_PDF_CREATOR_BUILDER_PATH."preview.php";   
	        }
	...
	public static function pdf_creator_get_header_footer($attrs,$template = "header"){
		if (!function_exists('str_get_html')) { 
			include SUPERADDONS_PDF_CREATOR_BUILDER_PATH."libs/simple_html_dom.php";
		}
	...
	public static function pdf_creator_preview($attrs){
		...
		$fontData  = Superaddons_Pdfbuilder_Pdf_Settings::get_list_fonts();
	    ...
	    include SUPERADDONS_PDF_CREATOR_BUILDER_PATH."pdf-templates/header.php";
	    ?>
	    <style type="text/css">
	    	<?php
	    ...
	    if( count($data_orders) > 1 ) {
	    	...
	    	if($data_attrs["html"] == ""){
    			...
    			echo do_shortcode($data_attrs["html"]);
    		}
	    }
	    include SUPERADDONS_PDF_CREATOR_BUILDER_PATH."pdf-templates/footer.php";
	...
	public static function get_html( $id_template, $datas="", $footer = false ){
		...
		foreach( $data_contents as $row){
			...
			<div class="container-row" style="width:100%;">
				<div class="row" style="<?php echo wp_kses_post($row_style); ?>">
					<?php 
					$i=0;
...
new Superaddons_Fdf_Create_frontend;