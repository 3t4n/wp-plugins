...
class ForumWP_Profile {
	...
	public function register_scripts() {
		if ( um_is_core_page( 'user' ) ) {
			wp_register_style( 'um-forumwp', um_forumwp_url . 'assets/css/um-forumwp' . UM()->frontend()->enqueue()::get_suffix() . '.css', array(), um_forumwp_version );
	...
	function check_profile_tab_privacy( $tabs ) {
		...
		if ( defined( 'UM_DEV_MODE' ) && UM_DEV_MODE && UM()->options()->get( 'enable_new_ui' ) ) {
			$tabs['forumwp']['subnav']['topics']  = array(
				'title'    => __( 'Topics', 'um-forumwp' ),
				'notifier' => FMWP()->user()->get_topics_count( $user_id ),
			);
			$tabs['forumwp']['subnav']['replies'] = array(
				'title'    => __( 'Replies', 'um-forumwp' ),
				'notifier' => FMWP()->user()->get_replies_count( $user_id ),
			);

			if ( is_user_logged_in() && get_current_user_id() === absint( $user_id ) ) {
				if ( FMWP()->modules()->is_active( 'likes' ) ) {
					$tabs['forumwp']['subnav']['likes'] = array(
						'title'    => __( 'Likes', 'um-forumwp' ),
						'notifier' => FMWP()->module( 'likes' )->get_user_likes_count( $user_id ),
					);
				}
				if ( FMWP()->modules()->is_active( 'bookmarks' ) ) {
					$tabs['forumwp']['subnav']['bookmarks'] = array(
						'title'    => __( 'Bookmarks', 'um-forumwp' ),
						'notifier' => FMWP()->module( 'bookmarks' )->get_user_bookmarks_count( $user_id ),
					);
				}
				if ( FMWP()->modules()->is_active( 'subscriptions' ) ) {
					$tabs['forumwp']['subnav']['subscriptions'] = array(
						'title'    => __( 'Subscriptions', 'um-forumwp' ),
						'notifier' => FMWP()->module( 'subscriptions' )->get_user_subscriptions_count( $user_id ),
					);
				}
			}
		} else {
			$tabs['forumwp']['subnav']['topics']  = __( 'Topics', 'um-forumwp' ) . '<span>' . FMWP()->user()->get_topics_count( $user_id ) . '</span>';
			$tabs['forumwp']['subnav']['replies'] = __( 'Replies', 'um-forumwp' ) . '<span>' . FMWP()->user()->get_replies_count( $user_id ) . '</span>';

			if ( is_user_logged_in() && get_current_user_id() === absint( $user_id ) ) {
				if ( FMWP()->modules()->is_active( 'likes' ) ) {
					$tabs['forumwp']['subnav']['likes'] = __( 'Likes', 'um-forumwp' ) . '<span>' . FMWP()->module( 'likes' )->get_user_likes_count( $user_id ) . '</span>';
				}
				if ( FMWP()->modules()->is_active( 'bookmarks' ) ) {
					$tabs['forumwp']['subnav']['bookmarks'] = __( 'Bookmarks', 'um-forumwp' ) . '<span>' . FMWP()->module( 'bookmarks' )->get_user_bookmarks_count( $user_id ) . '</span>';
				}
				if ( FMWP()->modules()->is_active( 'subscriptions' ) ) {
					$tabs['forumwp']['subnav']['subscriptions'] = __( 'Subscriptions', 'um-forumwp' ) . '<span>' . FMWP()->module( 'subscriptions' )->get_user_subscriptions_count( $user_id ) . '</span>';
				}
			}
		}
	...
	/**
	 * @todo deprecate since new UI is official and legacy is deprecated.
	 */
	...
	function profile_content_subscriptions() {
		...
		<div class="um-profile-subnav">
			<a href="<?php echo add_query_arg( 'forumwp_subscriptions', 'forums' ) ?>" class="<?php if ( ! isset( $_GET['forumwp_subscriptions'] ) || 'forums' == sanitize_key( $_GET['forumwp_subscriptions'] ) ) { ?>active<?php } ?>"><?php _e( 'Forums', 'um-forumwp' ) ?></a>
			<a href="<?php echo add_query_arg( 'forumwp_subscriptions', 'topics' ) ?>" class="<?php if ( isset( $_GET['forumwp_subscriptions'] ) && 'topics' == sanitize_key( $_GET['forumwp_subscriptions'] ) ) { ?>active<?php } ?>"><?php _e( 'Topics', 'um-forumwp' ) ?></a>
		</div>
	...
	function profile_content_likes() {
		...
		<div class="um-profile-subnav">
			<a href="<?php echo add_query_arg( 'forumwp_likes', 'topics' ) ?>" class="<?php if ( ! isset( $_GET['forumwp_likes'] ) || 'topics' == sanitize_key( $_GET['forumwp_likes'] ) ) { ?>active<?php } ?>"><?php _e( 'Topics', 'um-forumwp' ) ?></a>
			<a href="<?php echo add_query_arg( 'forumwp_likes', 'replies' ) ?>" class="<?php if ( isset( $_GET['forumwp_likes'] ) && 'replies' == sanitize_key( $_GET['forumwp_likes'] ) ) { ?>active<?php } ?>"><?php _e( 'Replies', 'um-forumwp' ) ?></a>
		</div>
	...
	function profile_content_bookmarks() {
		...
		<div class="um-profile-subnav">
			<a href="<?php echo add_query_arg( 'forumwp_bookmarks', 'topics' ) ?>" class="<?php if ( ! isset( $_GET['forumwp_bookmarks'] ) || 'topics' == sanitize_key( $_GET['forumwp_bookmarks'] ) ) { ?>active<?php } ?>"><?php _e( 'Topics', 'um-forumwp' ) ?></a>
			<a href="<?php echo add_query_arg( 'forumwp_bookmarks', 'replies' ) ?>" class="<?php if ( isset( $_GET['forumwp_bookmarks'] ) && 'replies' == sanitize_key( $_GET['forumwp_bookmarks'] ) ) { ?>active<?php } ?>"><?php _e( 'Replies', 'um-forumwp' ) ?></a>
		</div>
