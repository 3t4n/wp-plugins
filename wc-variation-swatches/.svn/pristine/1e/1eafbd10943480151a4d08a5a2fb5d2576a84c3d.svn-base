...
namespace WooCommerceVariationSwatches\ByteKit\Admin;

use WooCommerceVariationSwatches\ByteKit\Plugin;
		...
		foreach ( $this->notices as $notice ) {
			if ( $this->should_display( $notice ) ) {
				...
				printf(
					...
					$notice['dismissible'] ? '<button type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__( 'Dismiss this notice', 'wc-variation-swatches' ) . '</span></button>' : ''
				);
			}
		}
	}
