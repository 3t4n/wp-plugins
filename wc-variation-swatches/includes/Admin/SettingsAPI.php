...
class SettingsAPI {
		...
		foreach ( $this->settings_sections as $section ) {
			...
			if ( isset( $section['desc'] ) && ! empty( $section['desc'] ) ) {
				...
				$callback        = function () use ( $section ) {
					echo wp_kses( $section['desc'], array( 'div' => array( 'class' => array() ) ) );
				};
	 ...
	 *
	 * @param array $args Fields attribute array.
	 *
	 * @since 1.0.0
	 * @return string
	 */
	...
	public function callback_text( $args ) {
		...
		echo wp_kses( $html, wcvs_allowed_html_tags() );
	}
	...
	public function callback_checkbox( $args ) {
		...
		$html .= sprintf( '<input type="checkbox" class="checkbox" id="wpuf-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s %4$s />', $args['section'], $args['id'], checked( $value, 'on', false ), $attr );
	...
	public function callback_select( $args ) {
		...
		$html = sprintf( '<select class="%1$s" name="%2$s[%3$s]" id="%2$s[%3$s]" %4$s>', $size, $args['section'], $args['id'], $attr );
		foreach ( $args['options'] as $key => $label ) {
	...
	public function callback_file( $args ) {
		...
		$label = isset( $args['options']['button_label'] ) ? $args['options']['button_label'] : __( 'Choose File', 'wc-variation-swatches' );
	 ...
	 * @param string $default_text default text if it's not found.
	 *
	 * @since 1.0.0
	 * @return string
	 */
	public function get_option( $option, $section, $default_text = '' ) {
