...
class Products {
	...
	public function __construct() {
		...
		// phpcs:enable
		add_filter( 'product_attributes_type_selector', array( $this, 'add_attribute_type_selector' ) );
	}

	/**
	 * Add Attribute Type Selector.
	 *
	 ...
	 * @return array
	 */
	public function add_attribute_type_selector( $types ) {
		$swatches_types = wcvs_get_attribute_types();
	...
	public function initialize_settings() {
		$stylesheet = wcvs_get_settings( 'enable_stylesheet', 'on', 'general_settings' );
		if ( 'on' === $stylesheet ) {
			add_filter(
	...
	public function attribute_options_html( $html, $args ) {
		$types = wcvs_get_attribute_types();
		$attr  = wcvs_get_attr_tax_by_name( $args['attribute'] );
	...
	public function attribute_html( $html, $term, $attr, $args ) {
		...
		$attribute_behaviour = wcvs_get_settings( 'attribute_behaviour', 'with_cross', 'general_settings' );

		$shape_width  = wcvs_get_settings( 'width', '30', 'shape_settings' );
		$shape_height = wcvs_get_settings( 'height', '30', 'shape_settings' );
		...
		$shape_style       = wcvs_get_settings( 'shape_style', 'round', 'shape_settings' );
		...
		$enable_tooltip           = wcvs_get_settings( 'enable_tooltip', 'on', 'tooltip_settings' );
		$this->tooltip_bg_color   = wcvs_get_settings( 'tooltip_bg_color', '', 'tooltip_settings' );
		$this->font_size          = wcvs_get_settings( 'font_size', '15', 'tooltip_settings' );
		$this->tooltip_text_color = wcvs_get_settings( 'tooltip_text_color', '', 'tooltip_settings' );

		$border             = wcvs_get_settings( 'border', 'enable', 'border_settings' );
		$this->border_color = wcvs_get_settings( 'border_color', '#81d742', 'border_settings' );
