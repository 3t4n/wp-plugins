...
class Cart
{
    ...
    /**
     * Helper method to modify WC()->cart->get_totals(); and calculate taxes in total, subtotal, discount, etc.
     */
    private function getCartTotals() {

        $totals = WC()->cart->get_totals();

        if($totals['subtotal_tax']) {
            $totals['subtotal'] = $totals['subtotal'] + $totals['subtotal_tax'];
        }
        if($totals['discount_tax']) {
            $totals['discount_total'] = $totals['discount_total'] + $totals['discount_tax'];
        }
        if($totals['total_tax']) {
            $totals['total'] = $totals['total'] + $totals['total_tax'];
        }
        if($totals['cart_contents_tax']) {
            $totals['cart_contents_total'] = $totals['cart_contents_total'] + $totals['cart_contents_tax'];
        }
