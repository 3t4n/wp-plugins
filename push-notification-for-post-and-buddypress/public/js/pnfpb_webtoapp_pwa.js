...
$j(document).ready( async function() {
	...
	var pnfpbwebtoappid = parseInt(pnfpb_webtoapp_userid);
	
 	if (pnfpb_webview || is_webtoapp) {
		
		executeWhenAppReady( async function () {
		
    		try {
			
				pnfpbwebtoapptoken = (await getNotificationToken())["token"];
			
				await setNotificationTopicSubscriptionStatus(true, "pnfpbgeneral");	
		...
		$j(".group-button").on("click",".leave-group", function() {
			...
			if (pnfpbwebtoapptoken) {
			
				deviceid = pnfpbwebtoapptoken;
				...
				$j.post(pnfpb_ajax_object_webtoapp_push.ajax_url, data, function(response) {
		...
		$j(document).on("click",".leave-group", function() {
			...
			if (pnfpbwebtoapptoken) {
				...
				var deviceid = pnfpbwebtoapptoken;
		...
		if( typeof(pnfpb_subscribe_notification_group_element) != 'undefined' && pnfpb_subscribe_notification_group_element != null ) {
			...
			$j(document).on( "click",".subscribe-notification-group", function(e) {
				...
				if( typeof(pnfpb_group_users_subscribe_dialog_confirm) === 'undefined' || pnfpb_group_users_subscribe_dialog_confirm === null ) {
					...
					$j(this).append( '<div id="pnfpb_group_users_subscribe_dialog_confirm" class="pnfpb_group_users_subscribe_dialog_confirm" title="Subscribe" style="display:none;"><span class="ui-icon ui-icon-alert" style="float:left; margin:12px 12px 20px 0;"></span>'+pnfpb_ajax_object_webtoapp_push.group_subscribe_dialog_text+'</div><div id="pnfpb-group-unsubscribe-dialog" title="Confirmation"><div id="pnfpb-group-unsubscribe-alert-msg" class="pnfpb-group-unsubscribe-alert-msg"></div></div>' );
				
				}
			
 				var subscribe_button_text = pnfpb_ajax_object_webtoapp_push.subscribe_button_text;
				...
				$j( "#pnfpb_group_users_subscribe_dialog_confirm" ).dialog({  
					...
					closeText : '',
			   		...
			   		buttons: [{
						...
						open: function() {
							$j(this).attr('style','font-weight:bold;color:'+pnfpb_ajax_object_webtoapp_push.subscribe_button_text_color+';background-color:'+pnfpb_ajax_object_webtoapp_push.subscribe_button_color+';border:0px');
						},
      					click: function() {
							const d = new Date();
							...
							$j.post(pnfpb_ajax_object_webtoapp_push.ajax_url, data, async function(response) {
								
								//await setNotificationTopicSubscriptionStatus(true, "pnfpbgeneral");
							
								$j(".pnfpb-group-unsubscribe-alert-msg").html("<p>"+pnfpb_ajax_object_webtoapp_push.group_subscribe_dialog_text_confirm+"</p>");
		...
		if( typeof(pnfpb_unsubscribe_notification_group_element) != 'undefined' && pnfpb_unsubscribe_notification_group_element != null ) {
			...
			$j(document).on( "click",".unsubscribe-notification-group", function(e) {
				...
				if(typeof(pnfpb_group_users_unsubscribe_dialog_confirm) === 'undefined' || pnfpb_group_users_unsubscribe_dialog_confirm === null ) {
					...
					$j(this).append( '<div id="pnfpb_group_users_unsubscribe_dialog_confirm" class="pnfpb_group_users_unsubscribe_dialog_confirm" title="Unsubscribe" style="display:none;"><span class="ui-icon ui-icon-alert" style="float:left; margin:12px 12px 20px 0;"></span>'+pnfpb_ajax_object_webtoapp_push.group_unsubscribe_dialog_text+'</div><div id="pnfpb-group-unsubscribe-dialog" title="Confirmation"><div id="pnfpb-group-unsubscribe-alert-msg" class="pnfpb-group-unsubscribe-alert-msg"></div></div>' );
				
				}
			
				var unsubscribe_button_text = pnfpb_ajax_object_webtoapp_push.unsubscribe_button_text;
								...
								$j.post(pnfpb_ajax_object_webtoapp_push.ajax_url, data, function(response) {
			                		...
			                		if (response != 'fail')
			                    	{
    	                           		if (response != 'duplicate'){
											...
											$j(".pnfpb-group-unsubscribe-alert-msg").html("<p>"+pnfpb_ajax_object_webtoapp_push.group_unsubscribe_dialog_text_confirm+"</p>");
		...
		if (pnfpb_ajax_object_webtoapp_push.userid > 0 && typeof(pnfpb_push_notification_frontend_settings_element) != 'undefined' && pnfpb_push_notification_frontend_settings_element != null) {
			...
			$j.post(pnfpb_ajax_object_webtoapp_push.ajax_url, data, function(responseajax) {
					...
					$j(".pnfpb_push_notification_frontend_settings_submit").on( "click", function(event) {
						...
						$j.post(pnfpb_ajax_object_webtoapp_push.ajax_url, data, async function(responseajax) {
							...
							if (subscriptionoptionsarray.length >= 11)
							{
								...
								if (subscriptionoptionsarray[0] === '1')
								{
									...
									await setNotificationTopicSubscriptionStatus(false, "pnfpbgeneral");
								}
								...
								if (subscriptionoptionsarray[1] === '1')
								{
									...
									await setNotificationTopicSubscriptionStatus(false, "pnfpbgeneral");
									await setNotificationTopicSubscriptionStatus(true, "pnfpbpost");
								...
								else 
								{
									...
									await setNotificationTopicSubscriptionStatus(false, "pnfpbpost");									
								}
								...
								if (subscriptionoptionsarray[11] === '1')
								{
									...
									await setNotificationTopicSubscriptionStatus(false, "pnfpbgeneral");
									await setNotificationTopicSubscriptionStatus(true, "pnfpbactivity");									
								...
								else 
								{
									...
									await setNotificationTopicSubscriptionStatus(false, "pnfpbactivity");
								}									
								...
								if (subscriptionoptionsarray[2] === '1')
								{
									...
									await setNotificationTopicSubscriptionStatus(false, "pnfpbgeneral");
									await setNotificationTopicSubscriptionStatus(true, "pnfpbcomments");									
								}
								else 
								{
									$j('#pnfpb_ic_fcm_front_bcomment_enable').prop('checked', false);
									
									await setNotificationTopicSubscriptionStatus(false, "pnfpbcomments");
								}
