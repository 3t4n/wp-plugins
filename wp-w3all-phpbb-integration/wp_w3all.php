...
if(isset($w3reset_cookie_domain)){
  ...
  $w3cookie_domain = $w3reset_cookie_domain;
   ...
   $w3all_add_user_in_phpbb_onlogin_yn = isset($w3all_conf_pref['w3all_add_user_in_phpbb_onlogin_yn']) && ! empty($w3all_conf_pref['w3all_add_user_in_phpbb_onlogin_yn']) ? $w3all_conf_pref['w3all_add_user_in_phpbb_onlogin_yn'] : 0;
  ...
  // signon fail
   if( isset($w3all_exec_u_login) && is_wp_error($w3all_exec_u_login) OR isset($pass_match) && !$pass_match )
   {
    ...
    if(isset($w3all_exec_u_login->data->user_login)){
      wp_set_current_user( $w3all_exec_u_login->data->ID, '' );
...
if(isset($_COOKIE["w3all_set_cmsg"]) && !empty($_COOKIE["w3all_set_cmsg"])){
  ...
  function w3all_msgs( $message ) {
    ...
    if(trim($_COOKIE["w3all_set_cmsg"]) == 'phpbb_deactivated'){
     return __('Notice: the username is currently inactive in our forum. Please contact an administrator.', 'wp-w3all-phpbb-integration');
    }
    if(trim($_COOKIE["w3all_set_cmsg"]) == 'phpbb_uname_chars_error'){
     return __('Notice: the forum username contains characters forbidden by this system. Please contact an administrator.', 'wp-w3all-phpbb-integration');
    }
...
// phpBB allow \ char on password
   ...
   global $wpdb,$wp_hasher,$w3all_add_into_phpBB_after_confirm,$w3all_add_user_in_phpbb_onlogin_yn;
...
# the phpBB user come to login in WP and then is automatically added and should be logged in on fly
   ...
   # both the follow return the user obj instance of the created user, with hash of the phpBB pass hash (like when it is fired after 'function w3_check_phpbb_profile_wpnu($username){')
   # $wpu = get_user_by( 'ID', $user_id );
   # $wpu = new WP_User($user_id);
    ...
    if(empty($wpu)){
     $wpu = get_user_by( 'ID', $user_id ); // return the user obj instance of the created user, with hash of the phpBB pass hash, if it is a check after the user creation
    }
 ...
 if ( $hash && $check !== true && strlen($hash) > 32 && !isset($HArgon2i)){ // Wp check failed, check phpBB pass that's may not Argon2i
    ...
    # check that this user do not need to be added into phpBB, due to $w3all_add_into_phpBB_after_confirm
       ...
       if( $w3all_add_into_phpBB_after_confirm == 1 OR $w3all_add_user_in_phpbb_onlogin_yn > 0 )
       {
...
function wp_w3all_remove_bbcode_tags($post_str, $words){
 ...
 //$post_string = preg_replace("/\[(.*?)\]/is", '', $post_str);
...
########
#
# WP phpBB Roles/Groups switches START
#
########

# next two functions are about roles and phpBB groups switches
# TEST into an hooked function
#$phpbb_u_emails__wp_roles = 'jdddddddd@gmail.com[subscriber],pacos@peco.com[editor]';
#$phpbb_u_emails__wp_roles = 'jdddddddd@gmail.com[editor]';
#w3all_wp_USERS_roles_switch( $phpbb_u_emails__wp_roles );
#w3all_phpbb_USER_groupSwitch('jdddddddd@gmail.com', $phpbbGroupsAdd = '4,2', $phpbbGroupDefault = '4');
#exit;
...
function w3all_wp_USERS_roles_switch( $phpbb_u_emails__wp_roles )
{
    ...
    if( ! isset($_REQUEST["w3allastoken"]) && empty($phpbb_u_emails__wp_roles) )
     return;

  global $phpbb_config,$wpdb;
  if (empty($phpbb_config)) WP_w3all_phpbb::w3all_get_phpbb_config_res();
  if (empty($phpbb_config)) return;
  ...
  # $_REQUEST Security:
  # The token built into phpBB is the hash of the avatar salt, then passed as param into the request
  # There is also a check against a random 'W3GSWPEP_.....' var name
  # That of course could be set as more like here below so to fit another choosen token instead

# $_POST rand var name from phpBB
# random 'W3GSWPEP_.....' is built as follow into phpBB, so here it just repeat the same so to check the passed var name
$t1 = md5($phpbb_config["avatar_salt"]);
$t2 = substr($t1, 0, -15);
$t3 = strtoupper(substr($t1, -15));

       # Setup udata for EXTERNAL request
       # (??) Avoid if a function param passed togheter with
       # Check the random Requested var name: only follow if it match
     if( isset($_REQUEST["W3GSWPEP_".$t2.$t3]) && empty($phpbb_u_emails__wp_roles) )
     {
      $w3allastoken = trim(str_replace(chr(0), '', $_REQUEST["w3allastoken"])); # pass hash of the phpBB avatar salt
      $phpbb_u_emails__wp_roles = trim(str_replace(chr(0), '', $_REQUEST["phpbb_u_emails__wp_roles"]));

      # check that the presented token match if it is an 'endpoint request'

      if( !empty($phpbb_config["avatar_salt"]) && !empty($w3allastoken) )
      {
       if( ! password_verify($phpbb_config["avatar_salt"], $w3allastoken) )
       { return; }
      } else { return; }

    } # END Setup udata for EXTERNAL request


  if( preg_match('/[^-0-9A-Za-z_.@\:\,\|\[\]]/',$phpbb_u_emails__wp_roles) )
   return;

  $wpu_db_um = (is_multisite()) ? WPW3ALL_MAIN_DBPREFIX . 'usermeta' : $wpdb->prefix . 'usermeta';

  $phpbb_u_emails__wp_roles = explode(',', $phpbb_u_emails__wp_roles);

  foreach($phpbb_u_emails__wp_roles as $v){

   preg_match('/\[(.*?)\]/', $v, $matches);

   $uemail = explode('[', $v);

   if( !empty($uemail) && !empty($matches) ){
    $addTOroles[$uemail[0]] = explode('|', $matches[1]);
   }
  }

 if( !empty($addTOroles) )
 {
   $wproles = empty(wp_roles()->role_names) ? array() : wp_roles()->role_names;

   foreach( $addTOroles as $e => $r )
   {
     $wpu = new WP_User(get_user_by('email', $e));

     if( empty($wpu) OR $wpu->ID < 2 ) continue;

     if( in_array('PREV_ROLES_REMOVE',$r) ){ # remove the user from all previous roles
      # but $wpu->set_role remove himself the old user role, so this is not necessary into
      # a WP default
      #$wpdb->query("UPDATE $wpu_db_um SET meta_value = 'a:0:{}' WHERE user_id = '".$wpu->ID."' AND meta_key = 'wp_capabilities'");
     }

         #$roles = array(); #maybe for some plugin that allow multiple WP users Roles
     foreach( $r as $i )
     {
       if( $i != 'PREV_ROLES_REMOVE' && array_key_exists($i, $wproles) )
       {
         $wpu->set_role($i);
         #$roles[] = $i;
        }
      }
         #$wpdb->query("UPDATE $wpu_db_um SET meta_value = '".serialize($roles)."' WHERE user_id = '".$wpu->ID."' AND meta_key = 'wp_capabilities'");
   }
 }

} // END function w3all_wp_USERS_roles_switch()



if( isset($_REQUEST["W3GSWPEP"]) )
{
  add_action( 'init', 'w3all_wp_USERS_roles_switch' );
}
...
function w3all_phpbb_USER_groupSwitch( $wpu_email_id = '', $phpbbGroupsAdd = '', $phpbbGroupDefault = '', $phpbbGroupsRemove = '' ){
     ...
     # required - $phpbbGroupDefault - The default phpBB user group which the user will be set to
  ...
  if( empty($wpu_email_id) OR empty($phpbbGroupsAdd) OR intval($phpbbGroupDefault) < 2 ) return;

  if( preg_match('/[^0-9\,]/',$phpbbGroupsAdd )
   #OR preg_match('/[^0-9\,]/',$phpbbGroupsRemove)
   #OR preg_match('/[^0-9]/',$phpbbGroupDefault)
   ) return;
...
# TEST # TEST # TEST
     ...
     #$phpbbGroupDefault = '4';
     #$phpbbGroupsAdd = '2,4,3';
     ##$phpbbGroupsRemove = '6,7';

  global $w3all_config,$phpbb_config,$wpdb,$w3all_phpbb_connection;
  $wp_db_utab = (is_multisite()) ? WPW3ALL_MAIN_DBPREFIX . 'users' : $wpdb->prefix . 'users';

  if (empty($phpbb_config)) $phpbb_config = WP_w3all_phpbb::w3all_get_phpbb_config_res();
  if (empty($phpbb_config)) return;

     $getBY = is_numeric($wpu_email_id) ? 'ID' : 'email';

     if( $getBY == 'email' ) { $wpu_email_id = sanitize_email($wpu_email_id); }

     $wpu = get_user_by( $getBY, $wpu_email_id );

  if( empty($wpu->user_email) ) return;

     $phpbb_uid = $w3all_phpbb_connection->get_var("SELECT user_id FROM ".$w3all_config["table_prefix"]."users WHERE user_email = '".$wpu->user_email."'");

  if( empty($phpbb_uid) OR $phpbb_uid < 3 ) return;

   $addTo = explode( ',', $phpbbGroupsAdd );
   #$removeFrom = explode( ',', $phpbbGroupsRemove ); # not used, user removed from any phpBB group before the insertion

   $addTo = array_map('trim', $addTo);
   $addTo = array_map('intval', $addTo);
   #$removeFrom = array_map('trim', $removeFrom);

   # Skip if the passed default group do not exist into the group/s where the user will be added
   if ( !in_array($phpbbGroupDefault, $addTo) ) return;

   # If it is added into the Newly registered users group as default
   if( $phpbbGroupDefault == 7 ){ $userNew = true; }

# NOT USED: users records are ALL REMOVED from the 'user_group' tab before the insertion
# or (not clear why) it will cause DOUBLE record insertion into the 'user_group' tab, if it already exist a record for the user_id

# REMOVE from groups
# could be passed the value as is, anyway there is a check for passed values
   #if( !empty($removeFrom) && is_array($removeFrom) )
   #{
   /* $g = $gids = '';
      foreach( $removeFrom as $v ){
       if(intval($v) > 0){
        $g .= "'".intval($v)."',";
       }
      }
   ...
   #  if(!empty($gids)){
      # !! NOTE
      # cause a double record insertion after, into the same 'user_group' table if all the uid rows are not previously deleted
      #$w3all_phpbb_connection->query("DELETE FROM ".$w3all_config["table_prefix"]."user_group WHERE user_id = '".$phpbb_uid."' AND group_id IN(".$gids.")");
      $w3all_phpbb_connection->query("DELETE FROM ".$w3all_config["table_prefix"]."user_group WHERE user_id = '$phpbb_uid'");
  #   }
  #} # END REMOVE from groups
...
# ADD to group/s
   ...
   if( !empty($addTo) && is_array($addTo) )
   {
    $g = $gids = '';
      foreach( $addTo as $v ){
       if(intval($v) > 1){ # gid 1 = Guests in phpBB
        # here should also be checked if the gid exist in phpBB, but it require a query more
        # so it suppose that the passed data, always contain an existent and valid group id, without doing a further check
        $g .= intval($v).',';
       }
      }
     $gids = substr($g, 0, -1);
     ...
     if(!empty($gids)){
      $qat = '';
       foreach($addTo as $at){
          $qat .= "('".$at."','".$phpbb_uid."','0','0'),";
       }
      $queryat = substr($qat, 0, -1);
     }

     if( !empty($queryat) ){
       $w3all_phpbb_connection->query("INSERT INTO ".$w3all_config["table_prefix"]."user_group (group_id, user_id, group_leader, user_pending) VALUES $queryat");
...
# SET the Default group
    if( !isset($userNew) )
    {
      $w3all_phpbb_connection->query("UPDATE ".$w3all_config["table_prefix"]."users SET user_type = '0', user_permissions = '', group_id = '".$phpbbGroupDefault."' WHERE user_id = '".$phpbb_uid."'");
    } else {
       $w3all_phpbb_connection->query("UPDATE ".$w3all_config["table_prefix"]."users SET user_type = '0', user_permissions = '', group_id = '".$phpbbGroupDefault."' user_new = '1' WHERE user_id = '".$phpbb_uid."'");
      }

   # END SET DEFAULT group

} // END function w3all_phpbb_USER_groupSwitch()


########
#
# END WP phpBB Roles/Groups switches
#
########
...
# Using javascript instead, while this would work
