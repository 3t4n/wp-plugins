...
$w3wp_roles = empty(wp_roles()->role_names) ? array() : wp_roles()->role_names;
...
<?php
    if( !defined('W3PHPBBDBCONN') && empty($w3all_phpbb_dbconn) ){
...
<?php
echo __('Choosing to hash passwords in phpBB mode, when/if the integration will be disabled, WordPress users will have to reset their password to correctly login into WordPress (because maybe the password hash will not match). Choosing to hash passwords in the WordPress way, phpBB users will have to reset their password in order to login into phpBB, once the integration plugin has been disabled.<br /><br /><b>Important note: choosing the WordPress password hash, it is maybe mandatory to let your users to login AND update their password only in WordPress and not in phpBB.</b><br />By using phpBB hashes, users can login both in WordPress and phpBB, since WordPress with the integration plugin active will recognize any phpBB/wordpress hash, while phpBB will not recognize the WordPress hashes (if you do not add an extension into phpBB that could do this). <b>phpBB hash password</b> is the default setting', 'wp-w3all-phpbb-integration');
?>
<p><label""><input type="radio" name="w3all_phpbb_dbconn[w3all_pass_hash_way]" id="w3all_pass_hash_way_1" value="1" <?php checked('1', $w3all_pass_hash_way); ?> /> <?php echo __('<b>WordPress password hash</b> (user login and password update only into WordPress.)', 'wp-w3all-phpbb-integration'); ?></label></p>
<p><label""><input type="radio" name="w3all_phpbb_dbconn[w3all_pass_hash_way]" id="w3all_pass_hash_way_0" value="0" <?php checked('0', $w3all_pass_hash_way); ?> /> <?php echo __('<b>phpBB password hash (default)</b>', 'wp-w3all-phpbb-integration'); ?></label></p>
<hr />
<h3><?php echo __('Link WordPress and phpBB users', 'wp-w3all-phpbb-integration');?></h3>
<?php echo __('In <b><i>not linked users</i></b> mode it is possible to use transfers options and shortcodes or widgets<br />Hint: in <i>not linked users</i> mode it is also possible to retrieve posts and topics from different domain'); ?>
...
<p><label""><input type="radio" name="w3all_phpbb_dbconn[w3all_not_link_phpbb_wp]" id="w3all_not_link_phpbb_wp_0" value="0" <?php checked('0', $w3all_not_link_phpbb_wp); ?> /> <?php echo __('<b>Link phpBB and WordPress users (default)', 'wp-w3all-phpbb-integration'); ?></label></p>
<br />
...
<hr />
<strong><span style="color:#ff0000">NOTE: it is maybe mandatory</span></strong> to transfer existent WordPress users into phpBB when the integration start, if you allow your users to register also into phpBB and the phpBB extension (and related option "check existence of username/email in WP before to register users in phpBB") are not active/installed.<br /><br />Note that if a WP user do not exist into phpBB, will be added into phpBB when the same user will login into WordPress, if the option more below (<b>Add users in phpBB if they do not exist, when the user logs in to WordPress</b>) is active<br /><br />
...
<?php
echo __('it depend on how Cms are configured to run as integrated (where you allow users to register or update email) and/or how the integration plugin will be used<br />but normally all users must exists into both CMS<br />Use the <i>WP w3all check -> List phpBB users with duplicated usernames or emails</i> task<br /><strong style="color:#FF0000">NOTE: it is may required</strong> as explained on the <a target="_blank" href="https://www.axew3.com/w3/wordpress-phpbb-integration-install/">Help Install Page</a>, <strong style="color:#FF0000">to transfer all existent WordPress users into phpBB when the integration start</strong><br />
  While <strong>it is (may) not required</strong> to transfer phpBB users into WordPress when the integration start.<br />Or note as above mentioned that an old WP user that do not exist into phpBB because not transferred, will be added into phpBB when the same user will login into WordPress.<br />Run the check tasks before to start the integration or use it to check for user\'s problems time after time.<br />These options are also visible into WordPress admin under Tools Menu', 'wp-w3all-phpbb-integration');
...
<h3><?php echo __('Add users in phpBB if they do not exist, when the user logs in to WordPress', 'wp-w3all-phpbb-integration');?></h3>
<?php echo __('This option is useful if you don\'t want to transfer users from WordPress to phpBB when the integration starts. Users will be added to phpBB when they log in to WordPress', 'wp-w3all-phpbb-integration'); ?>
<p><input type="radio" name="w3all_conf_pref[w3all_add_user_in_phpbb_onlogin_yn]" id="w3all_add_user_in_phpbb_onlogin_yn_1" value="1" <?php checked('1', $w3all_add_user_in_phpbb_onlogin_yn); ?> /> <?php echo __('Yes', 'wp-w3all-phpbb-integration'); ?></p>
<p><input type="radio" name="w3all_conf_pref[w3all_add_user_in_phpbb_onlogin_yn]" id="w3all_add_user_in_phpbb_onlogin_yn_0" value="0" <?php checked('0', $w3all_add_user_in_phpbb_onlogin_yn); ?> /> <?php echo __('No', 'wp-w3all-phpbb-integration'); ?></p>
<hr />
...
<?php echo __('<b>NOTE:</b> activating this option, <b>it is mandatory</b> that you copy all the files inside the <i>views</i> folder, and all others custom used files, into the new custom created folder<br /><i>/wp-content/plugins/<b>wp-w3all-custom</b></i><br /><br />Files that display the shortcodes output and that reside into the folder <i>/wp-content/plugins/wp-w3all-phpbb-integration<b>/views</b></i> or some other custom file,<br />can be copied/pasted into the custom folder:<br /><i>/wp-content/plugins<b>/wp-w3all-custom</b></i><br /> so that custom modifications done into these files aren\'t overwritten when the plugin will be updated<br /><br />Note: old way (folder name) <i>wp-w3all-config</i> will still be working if you do not wish to change the folder name into the new <i>wp-w3all-custom</i>', 'wp-w3all-phpbb-integration'); ?>
...
<hr />
<?php echo __('<h3>Deactivate phpBB user account until WP confirmation</h3>If this option is set to Yes, users will be added in phpBB as <b><i>deactivated</i></b> when they register on WordPress. The phpBB user account will be <b><i>activated</i></b> only after his first login on WordPress. Normally it is not necessary and all will work as expected with users that you want to approve, before to be activated in WP/phpBB, but in case you can force this behavior by setting to yes this option<br /><br />Note: if the option above<br /> - <i>Add users in phpBB only after first successful login in WordPress</i><br />is set to Yes, then the user will be added into phpBB as deactivated and will not be able to login until not activated via admin in phpBB<br /><br />Note: maybe this option should not be used in conjunction with the <i>Add users in phpBB only after first successful login in WordPress</i> option, and only when the password is not chosen by the user during registration', 'wp-w3all-phpbb-integration');?>
