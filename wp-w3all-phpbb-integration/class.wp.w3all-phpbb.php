...
private static function w3all_wp_logout($redirect = ''){
 ...
 if( isset($_COOKIE[$k]) OR isset($_COOKIE[$sid]) ){
  ...
  # do not delete session for a legit user
...
private static function w3all_db_connect(){
 global $w3all_phpbb_connection,$w3all_config_db,$w3all_config,$w3deactivate_wp_w3all_plugin;

 if(isset($w3deactivate_wp_w3all_plugin)) return;
...
// HERE INSIDE IS SAFE //
 ...
 // so the user's data result into the array will contain records that are not as expected in these cases
 // that contains the user's data to display, into the serialized $nnn->notification_data record (see wp_w3all_phpbb_unotifications_short.php)

 // Exclude bots and deactivated phpBB users
  if( $w3all_phpbb_unotifications_yn > 0 && $phpbb_user_session[0]->user_type != 1 && $phpbb_user_session[0]->group_id != 6 )
  {
...
# switch groups/roles: also see -> public static function phpbb_update_profile($user_id, $old_user_data) {
 ...
 if( $w3all_link_roles_groups > 0 )
 {
  ...
  } # END if( $w3all_link_roles_groups > 0
  ...
  # -> Return Latin by default (so the username switch from another language will fail returning same 'not transliterated' username value, then the wp_insert_user will fail answering 'cannot create user with an empty username')

   $phpBBCharsUname = w3all_detectClean_language_chars($phpbb_user_session[0]->username, false, false, true);

    if( $phpBBCharsUname != 'Latin' && !empty($phpBBCharsUname) )
     {

        if( is_multisite() && !defined('WPW3ALL_USE_DEFAULT_WP_UCHARS') )
        {
          $phpBB_nonLatin_Uname = w3all_detectClean_language_chars($phpbb_user_session[0]->username, true);
        } else {

            $phpBB_nonLatin_Uname = w3all_detectClean_language_chars($phpbb_user_session[0]->username, $return_x_mums = false, $returnLatin = false, $returnDetectLang = false);
          }
      }
  ...
  # default wp allow [-0-9A-Za-z _.@]
    ...
    if( !empty($phpBBCharsUname) && $phpBBCharsUname != 'Latin' )
     { # set to latin wp default chars to create the user
       $phpbbLatUname = w3all_detectClean_language_chars($phpbb_user_session[0]->username, false, true);
         if ( is_multisite() && ! defined('WPW3ALL_USE_DEFAULT_WP_UCHARS') )
         { # mums allow only '[0-9A-Za-z]'
     ...
     } else
       {
         if ( is_multisite() && ! defined('WPW3ALL_USE_DEFAULT_WP_UCHARS') )
         { # mums allow only '[0-9A-Za-z]'
          $phpbbLatUname = w3all_detectClean_language_chars($phpbb_user_session[0]->username, true);
         } else {
            $phpbbLatUname = w3all_detectClean_language_chars($phpbb_user_session[0]->username, false, false, false);
           }
       }

  $phpbbLatUname = sanitize_user($phpbbLatUname); # it is redundant: the username has been already cleaned (but sanitize_user convert to ascii)

  $phpbbLatUnameEsc = esc_sql($phpbbLatUname);
  $phpbbUid = $phpbb_user_session[0]->user_id;
  $phpbbUemail = strtolower($phpbb_user_session[0]->user_email);

   $wpu = $wpdb->get_row("SELECT * FROM $wpu_db_utab WHERE LOWER(user_email) = '".$phpbbUemail."'");

     if( !empty($wpu) ){ $user_id = $wpu->ID; }
 ...
 if ( $user_id < 1 && $user_email < 1 && $phpbb_user_session[0]->user_type != 1 )
 {
   ...
   $userdata = array(
     ...
     #'user_email' => $phpbb_user[0]->user_email, # passing it, return error 'not enough data to create user'
   ...
   if ( ! is_wp_error( $user_id ) )
   {
     ...
     $uemail = $phpbb_user_session[0]->user_email;
     $upass  = $phpbb_user_session[0]->user_password;

    if( !defined( 'WPW3ALL_USE_UNAME_NATIVE_LANG_CHARS' ) ){
      $wpdb->query("UPDATE $wpu_db_utab SET user_pass = '".$upass."', user_email = '".$uemail."', display_name = '".$phpbbLatUname."' WHERE ID = ".$user_id."");
    } else {
             if( !empty($phpBB_nonLatin_Uname) && strlen($phpBB_nonLatin_Uname) < 50 ){
              ...
              $wpdb->query("UPDATE $wpu_db_utab SET user_login = '".$phpBB_nonLatin_Uname."', user_pass = '".$upass."', user_nicename = '".$nonLatinUnameLow."', user_email = '".$uemail."', display_name = '".$phpBB_nonLatin_Uname."' WHERE ID = ".$user_id."");
              $wpdb->query("UPDATE $wpu_db_umtab SET meta_value = '".$phpBB_nonLatin_Uname."' WHERE user_id = '$user_id' AND meta_key = 'nickname'");
 ...
 if( $user_id > 1 && $w3all_oninsert_wp_user > 0 )
 {
  // NOTE: duplicated users insertions "on iframe first time login" fixed more below (redirect when on page-forum)
  ...
  if( is_wp_error( $user_id ) ){
   ...
   if ( ! is_user_logged_in() && ! is_wp_error( $user_id ) && intval($user_id) > 1 )
   {

    $wpuID = $wpdb->get_var("SELECT ID FROM $wpu_db_utab WHERE LOWER(user_email) = '$phpbbUemail'");
    ...
    if ( ( empty( $redirect_to ) || $redirect_to == admin_url() ) )
    {
      ...
      if ( is_multisite() && !get_active_blog_for_user($wpuID) && !is_super_admin( $wpuID ) )
        $redirect_to = user_admin_url();
       elseif ( is_multisite() )
         $redirect_to = get_dashboard_url( $wpuID );
       elseif ( is_admin() )
 ...
 }  // END if ( $_COOKIE[$u] > 1 ){ // HERE INSIDE IS SAFE // END //
...
// End ban/deactivated
    ...
    $phpbb_user_id = $phpbb_u->user_id;
      ...
      if ( $phpbb_u->user_type == 1 && isset($wp_user_data->ID) && $wp_user_data->ID > 1 ){ // is this user deactivated/banned in phpBB? / logout/and deactivate in WP
         ...
         #update_user_meta($user_id, 'wp_capabilities', 'a:0:{}');
...
private static function create_phpBB_user($wpu, $action = ''){
 ...
 if ( empty($urank_id_a['rank_id']) ){
  ...
  if ( !defined( 'WP_ADMIN' ) OR $phpbb_u < 2 OR !empty($phpbb_u) && current_user_can( 'create_users' ) === true ) {
      ...
      # front end plugins should be managed each into function wp_check_password
      # so to determine if the user need to be then activated or not, based on their email
      # or other activation process
...
public static function phpBB_user_check( $sanitized_user_login, $user_email, $is_admin_action = 1 ){
     ...
     if( defined('W3ALL_PRE_UCK_EXEC') ): return; endif;
     global $w3all_config,$phpbb_config,$w3all_phpbb_connection;
...
public static function w3_check_phpbb_profile_wpnu($username){ // email/user_login
 ...
 if( defined('W3ALL_WPNU_CKU') OR empty($username) ) return;
  ...
  $wpu_db_utab = (is_multisite()) ? WPW3ALL_MAIN_DBPREFIX . 'users' : $wpdb->prefix . 'users';
  $wpu_db_umtab = (is_multisite()) ? WPW3ALL_MAIN_DBPREFIX . 'usermeta' : $wpdb->prefix . 'usermeta';
  $username = is_email($username) ? sanitize_email($username) : trim(str_replace(chr(0), '', $username));
  $username_esc = esc_sql($username);

  if( is_email($username) )
   { $wpu = get_user_by('email', $username);
    } else {
       $wpu = $wpdb->get_row("SELECT * FROM $wpu_db_utab WHERE user_login = '".$username_esc."'");
      }

// FIX by username here will not match if it is not Latin
// substitute with a query then get umeta for wpu capabilities
// $user_meta = get_userdata($user_id);

  $q = is_email($username) ? 'users.user_email) = \''.strtolower($username).'\'' : 'users.username) = \''.mb_strtolower($username).'\'';

  $phpbb_user = $w3all_phpbb_connection->get_results("SELECT *
    FROM ". $w3all_config["table_prefix"] ."groups
    JOIN ". $w3all_config["table_prefix"] ."users ON LOWER(". $w3all_config["table_prefix"] . $q ."
 ...
 if( $w3all_push_new_pass_into_phpbb == 1 ){
   ...
   if( isset($wpu->user_pass) && $wpu->user_pass != $phpbb_user[0]->user_password )
   {
     $new_pass_push = $phpbb_user[0]->user_password = $wpu->user_pass;
  ...
  // active in phpBB?
  if( !empty($wpu->ID) && $phpbb_user[0]->user_type != 1 ){ // re-activate this 'No role' WP user if it is the case

     $user = new WP_User($wpu->ID);

     if ( empty( $user->roles ) ) {
      $user_role_up = serialize(array($w3all_add_into_wp_u_capability => 1));
      $wpdb->query("UPDATE $wpu_db_umtab SET meta_value = '$user_role_up' WHERE user_id = '".$wpu->ID."' AND meta_key = 'wp_capabilities'");
     }
   }
...
// END banned or deactivated
 ...
 if ( !is_multisite() && !empty($phpbb_user) ) {
  if( !empty($wpu->ID) && $phpbb_user[0]->user_type == 1 && !empty($wpu->wp_capabilities) ){
   $user_email = strtolower($wpu->user_email);
  ...
  # w3all_detectClean_language_chars() see /wp-w3all-phpbb-integration/common/helpers.php

   $phpBBCharsUname = w3all_detectClean_language_chars($phpbb_user[0]->username, false, false, true);
    ...
    if( $phpBBCharsUname != 'Latin' && !empty($phpBBCharsUname) )
     {
        ...
        if( is_multisite() && !defined('WPW3ALL_USE_DEFAULT_WP_UCHARS') )
        {
          $phpBB_nonLatin_Uname = w3all_detectClean_language_chars($phpbb_user[0]->username, true);
        } else {

            $phpBB_nonLatin_Uname = w3all_detectClean_language_chars($phpbb_user[0]->username, false, false, false);
          }
      }
  ...
  # default wp allow [-0-9A-Za-z _.@]
    ...
    if( !empty($phpBBCharsUname) && $phpBBCharsUname != 'Latin' )
     { # set to latin wp default chars to create the user
       $phpbbLatUname = w3all_detectClean_language_chars($phpbb_user[0]->username, false, true);
         if ( is_multisite() && ! defined('WPW3ALL_USE_DEFAULT_WP_UCHARS') )
         { # mums allow only '[0-9A-Za-z]'
          $phpbbLatUname = w3all_detectClean_language_chars($phpbbLatUname, $return_x_mums = true);
         }
     } else
       {
         if ( is_multisite() && ! defined('WPW3ALL_USE_DEFAULT_WP_UCHARS') )
         { # mums allow only '[0-9A-Za-z]'
          $phpbbLatUname = w3all_detectClean_language_chars($phpbb_user[0]->username, true);
         } else {
            $phpbbLatUname = w3all_detectClean_language_chars($phpbb_user[0]->username, false, false, false);
           }
       }
  ...
  $phpbbLatUnameEsc = esc_sql($phpbbLatUname);
  $phpbbUid = $phpbb_user[0]->user_id;
  $phpbbUemail = $phpbb_user[0]->user_email;
 ...
 if ( $user_id < 1 && $user_email < 1 && $phpbb_user[0]->user_type != 1 && empty($wpu) )
 {
    ...
    $userdata = array(
     ...
     #'user_email' => $phpbb_user[0]->user_email, # return error 'not enough data to create user'
    ...
    if( !defined( 'WPW3ALL_USE_UNAME_NATIVE_LANG_CHARS' ) ){
      $wpdb->query("UPDATE $wpu_db_utab SET user_pass = '".$upass."', user_email = '".$phpbbUemail."', display_name = '".$phpbbLatUname."' WHERE ID = ".$user_id."");
             ...
             if( !empty($phpBB_nonLatin_Uname) && strlen($phpBB_nonLatin_Uname) < 50 ){
              $nonLatinUnameLow = esc_sql(mb_strtolower($phpBB_nonLatin_Uname));
              $phpBB_nonLatin_Uname = esc_sql($phpBB_nonLatin_Uname);
              $wpdb->query("UPDATE $wpu_db_utab SET user_login = '".$phpBB_nonLatin_Uname."', user_pass = '".$upass."', user_nicename = '".$nonLatinUnameLow."', user_email = '".$phpbbUemail."', display_name = '".$phpBB_nonLatin_Uname."' WHERE ID = ".$user_id."");
              $wpdb->query("UPDATE $wpu_db_umtab SET meta_value = '".$phpBB_nonLatin_Uname."' WHERE user_id = '$user_id' AND meta_key = 'nickname'");
   ...
   if( is_wp_error( $user_id ) ){
    ...
    if( $w3all_oninsert_wp_user == 1 && !defined("WPUSERCREATED") ) {
      define("WPUSERCREATED",true);
 ...
 define('W3ALL_WPNU_CKU', true);
  ...
  # login the user
   ...
   if(!empty($phpbb_user) && !empty($user_id) && isset($_POST['log']) && isset($_POST['pwd'])){
...
public static function wp_w3all_get_phpbb_user_info($username, $type=''){ // email/user_object/username
 ...
 if( is_email($username) ){
  ...
  $db_eu = is_email($is_email) ? 'users.user_email) = \''.$username.'\'' : 'users.username) = \''.$username.'\'';
...
public static function wp_w3all_get_phpbb_lastopics_short_wi( $atts ) {
  global $w3all_phpbb_connection,$phpbb_config,$w3all_config,$w3all_phpbb_usession,$w3all_url_to_cms,$w3all_lasttopic_avatar_num,$w3all_last_t_avatar_yn,$w3all_last_t_avatar_dim,$w3all_get_phpbb_avatar_yn,$w3all_phpbb_widget_mark_ru_yn,$w3all_custom_output_files,$w3all_phpbb_widget_FA_mark_yn,$w3all_get_topics_x_ugroup;
   ...
   $atts = array_map ('trim', $atts);
    ...
    $img_info_right_or_bottom = $ltm['info_right_or_bottom'] != 'bottom' ? 'right' : 'bottom'; // display link and image/file info on the right or below the image
...
//$gaf = $w3db_conn->get_results("SELECT DISTINCT forum_id FROM ".$w3all_config["table_prefix"]."acl_groups WHERE group_id = ".$ug." ORDER BY forum_id");
 ...
 $last_topics = $w3all_phpbb_connection->get_results("SELECT * FROM  ". $w3all_config["table_prefix"] ."posts
  ...
  ORDER BY ". $w3all_config["table_prefix"] ."posts.post_time DESC LIMIT 0,$topics_number");
   ...
   if( count($last_topics) < 2 ){
      ob_start();
        echo 'Almost two topics with attachments are required to display images from the choosen forums!';
      return ob_get_clean();
    }
