<?php
	...
	//Get paramenter
	if (isset($_GET['nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['nonce'])), 'l2h_nonce_action')) {
		// Proceed with the logic as the nonce is valid
		$ver  = isset($_GET['ver']) ? sanitize_text_field( wp_unslash ($_GET['ver'])) : '';
...
<span class = "latex_title">The User's Manual of LaTeX2HTML <?php echo esc_attr($ver); ?></span>
	<span class = "latex_author">Van Abel</span>
	<span class = "latex_date"><?php echo esc_attr($date) ?></span>
