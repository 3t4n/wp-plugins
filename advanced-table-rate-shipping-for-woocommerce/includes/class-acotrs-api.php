...
class ACOTRS_Api
{
    ...
    private function acotrs_zone_methods($method_id)
    {
        ...
        $zone_ids = $wpdb->get_results(
            $wpdb->prepare(
                'SELECT `zone_id` FROM ' . $wpdb->prefix . 'woocommerce_shipping_zone_methods WHERE `method_id`=%s GROUP BY `zone_id`',
                $method_id
            ),
            OBJECT
        );
    ...
    public function acotrs_zone_lists()
    {
        ...
        $zone_array = array(
            0 => esc_html__('Locations not covered by your other zones', 'aco-table-rate-shipping')
        );
    ...
    public function acotrs_list_of_zones($data)
    {
        ...
        $zone_id = $wpdb->get_row(
            $wpdb->prepare('SELECT `zone_id` FROM ' . $wpdb->prefix . 'woocommerce_shipping_zone_methods WHERE `instance_id`=%d', $instance_id),
            OBJECT
        );
        ...
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            if (is_wp_error($response)) {
                ...
                if (!empty($temp)) {
                    ...
                    $message = esc_html__('An error occurred, please try again.', 'aco-table-rate-shipping');
                }
            }
        } else {
                ...
                switch ($license_data->error) {
                    case 'expired':
                        $message = sprintf(
                            ...
                            esc_html__('Your license key expired on %s.', 'aco-table-rate-shipping'),
                            date_i18n(get_option('date_format'), strtotime($license_data->expires, current_time('timestamp')))
                        );
                        break;
                    case 'revoked':
                        $message = esc_html__('Your license key has been disabled.', 'aco-table-rate-shipping');
                        break;
                    case 'missing':
                        $message = esc_html__('Invalid license.', 'aco-table-rate-shipping');
                        break;
                    case 'invalid':
                    case 'site_inactive':
                        $message = esc_html__('Your license is not active for this URL.', 'aco-table-rate-shipping');
                        break;
                    case 'item_name_mismatch':
                        // Translators: appears to be an invalid license key for the plugin
                        $message = sprintf(esc_html__('This appears to be an invalid license key for %s.', 'aco-table-rate-shipping'), ACOTRS_PLUGIN_NAME);
                        break;
                    case 'no_activations_left':
                        $message = esc_html__('Your license key has reached its activation limit.', 'aco-table-rate-shipping');
                        break;
                    default:
                        $message = esc_html__('An error occurred, please try again.', 'aco-table-rate-shipping');
                        break;
    ...
    public function getConfig($data)
    {
        ...
        foreach ($posts as $product)
            array_push($products, array('id' => $product, 'title' => get_the_title($product)));
        ...
        $posts = get_posts($args);
        $coupons = array();
        foreach ($posts as $scoupon)
            array_push($coupons, array('id' => $scoupon, 'title' => get_the_title($scoupon)));
        ...
        foreach ($categories as $cat)
            array_push($catArray, array('term_id' => $cat->term_id, 'name' => $cat->name));

        // Get all shipping class
        $shipping_classes = get_terms(array('taxonomy' => 'product_shipping_class', 'hide_empty' => false));
        $shippingArray = array();
        foreach ($shipping_classes as $cat)
            array_push($shippingArray, array('sclass_id' => $cat->term_id, 'name' => $cat->name));
    ...
    {
        _doing_it_wrong(__FUNCTION__, esc_html__('Cheatin&#8217; huh?', 'aco-table-rate-shipping'), esc_html($this->_version));
    }
     ...
     * @since 1.0.0
     */
    public function esc_html__wakeup()
    {
        _doing_it_wrong(__FUNCTION__, esc_html__('Cheatin&#8217; huh?', 'aco-table-rate-shipping'), esc_html($this->_version));
    }
}
