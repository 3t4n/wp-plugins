    ...
    */
    public function acotrs_admin_menu_page_hook(){
        ...
        $this->hook_suffix[] = add_submenu_page( 
                'woocommerce', 
                esc_html__('Advanced Table Rate Shipping', 'aco-table-rate-shipping'), 
                esc_html__('Advanced Table Rate Shipping', 'aco-table-rate-shipping'), 
                'manage_woocommerce', 
                'acotrs', 
    ...
    public function acotrs_admin_page_callback(){
        echo (
            '<div id="' . esc_attr($this->token) . '_ui_root" class="bg-white border-round-5 pb-5">
                <div class="' . esc_attr($this->token) . '_loader"><p>' . esc_html__('Loading User Interface...', 'aco-table-rate-shipping') . '</p></div>
            </div>'
        );
    ...
    public function pluginActionLinks($links)
    {
        $action_links = array(
            'settings' => '<a href="' . admin_url('admin.php?page=' . $this->token) . '">'
                . esc_html__('Configure', 'aco-table-rate-shipping') . '</a>'
        );
