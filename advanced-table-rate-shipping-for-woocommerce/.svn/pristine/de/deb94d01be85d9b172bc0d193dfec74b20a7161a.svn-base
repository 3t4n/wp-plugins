...
if (class_exists('WooCommerce')) {
	...
	if (class_exists('ACOTRS_ShippingCalculation'))
		return;
	...
	class ACOTRS_ShippingCalculation
	{
			...
			if (is_array($package) && !empty($package) && isset($package['contents'])) {
				...
				if (!empty($cart_data)) {
					...
					foreach ($cart_data as $key => $data) {
						$rates[$key] = $this->acotrs_process_table_rates($data, $key);
				...
				if ($this->acotrs_method->base_rule_condition == "per_line_item") {
					$shipping_options = array();
					foreach ($rates as $key => $rate) {
						foreach ($rate as $op_key => $op) {
							if (!isset($shipping_options[$op_key])) {
								$shipping_options[$op_key] = $op;
								$shipping_options[$op_key]['cost'] = $op['cost'] * $cart_data[$key]['quantity'];
								...
								if ($shipping_options[$op_key]['label'] == '') {
									$shipping_options[$op_key]['label'] = $this->acotrs_method->method_title;
								}
								continue;
							}
					...
					foreach ($rates as $key => $rate) {
						foreach ($rate as $op_key => $op) {
							...
							$shipping_options[$op_key]['description'] = $op['description'] ?? ''; // Default to an empty string if not set
							$shipping_options[$op_key]['row_id'] = $op['row_id'] ?? null; // Handle missing row_id as well
		...
		public function acotrs_process_table_rates($cart_data)
		{
			...
			if (!empty($cart_data) && !empty($this->acotrs_method)) {
				...
				foreach ($this->acotrs_method->table_of_rates as $a_key => $option) {
					...
					if (isset($option['disable_option']) && (int) $option['disable_option'] === 1)
						continue;
					if (isset($option['rows']) && count($option['rows']) <= 0)
						continue;
					...
					if (isset($option['rows'])) {
						foreach ($option['rows'] as $r_key => $row) {
							...
							$delivery_date = 0;
							if ($this->acotrs_method->ad_set_delivery_date_automatically !== 0) {
					...
					if ($this->acotrs_method->ad_free_shipping_on_special_day && count($this->acotrs_method->ad_special_days)) {
						$matchDate = array_search(strtolower(gmdate('d-F')), array_column($this->acotrs_method->ad_special_days, 'date'));
						if ($matchDate !== false) {
							...
							if ((int) $this->acotrs_method->ad_special_day_persent > 0) {
								...
								$cost = 0;
								$option['title'] = $option['title'] . ' : ' . __('Free Shipping', 'aco-table-rate-shipping');
							}
						}
					}
			...
			if (count($rates) <= 0 && (int) $this->acotrs_method->flatrate > 0) {
				...
				$rates[$this->acotrs_method->id] = array(
					'id' => $option_id, // $this->acotrs_method->id,
		...
		public function acotrs_calculate_shipping_cost($costs = array(), $cart_data = array())
		{
			...
			$cost_ops = apply_filters('acotrs_shipping_cost_options', array(
				...
				'%' => '%',
				'x' => __('Multiplied by', 'aco-table-rate-shipping'),
				'every' => __('For every', 'aco-table-rate-shipping'),
			));
			$calcs = array();
			...
			foreach ($costs as $cost) {
				...
				switch ($cost['cost_unit']) {
					...
					case 'x':
						...
						$multipliedto = $cart_data[$cost['cost_multipliedby']];
						if ($cost['cost_multipliedby'] == 'products')
							$multipliedto = count($cart_data[$cost['cost_multipliedby']]);

						$calcs[] = $cost['cost'] * $multipliedto;
						break;
					case 'every':
			...
			if (is_array($cond) && isset($cond['condition'])) {
				...
				if (in_array($cond_type, array('subtotal', 'quantity', 'weight', 'height', 'width', 'length', 'area', 'volume'))) {
					...
					$cvalue = apply_filters('acotrs_conditional_tertiary_' . $cond_type, floatval($cond['cvalue']), $cond);
					 ...
					 if($this->acotrs_method->ad_include_coupons == 1 )  {
					...
					if (isset($cart_data[$cond_type])) {
						if ($cond_type === "subtotal") {
							$comparison = $cart_subtotal ;    //WC()->cart->get_subtotal();  // cart subtotal
						} else {
					...
					if (
						($cond['compair'] === 'excludes' && !in_array(strtolower(gmdate('l')), $daysArray)) ||
						($cond['compair'] === 'includes' && in_array(strtolower(gmdate('l')), $daysArray))
					) {
						return true;
					}
		...
		public function acotrs_calculate_totals_order($items)
		{
			...
			foreach ($items as $item_ar) {
				...
				if (isset($item_ar['data']) && $item_ar['data']->needs_shipping()) {
					...
					// $a1 = $this->acotrs_get_attribute_array_from_cart_item_data($item_ar['data']) ;
					$a1 = array_merge_recursive(
						$a1,
						$this->acotrs_get_attribute_array_from_cart_item_data($item_ar['data'])
					);
					$taxonomys = array_merge($taxonomys, $a1);
		...
		private function acotrs_get_line_item_weight($item)
		{
			...
			if (isset($this->acotrs_method->volume) && is_numeric($this->acotrs_method->volume) && $this->acotrs_method->volume > 0) {
				...
				if (!empty($this->acotrs_method->exclude_weight))
					return $volumetric;
		...
		public function acotrs_cartdata_basedon_class($items = array())
		{
			...
			foreach ($items as $item_ar) {
				...
				if (isset($item_ar['data']) && $item_ar['data']->needs_shipping()) {
					...
					//Add attributes
					// $data[ $shipping_class_id ] 					= array_merge(
					$data[$shipping_class_id] = array_merge_recursive(
						$data[$shipping_class_id],
						$this->acotrs_get_attribute_array_from_cart_item_data($item)
					);
		...
		/**
		 * Get Attributes Array From Cart Item Data
		 */
		private function acotrs_get_attribute_array_from_cart_item_data($item)
		{

			$data = array();

			// Get attributes from cart item
			$attributes = $item->get_attributes();

			// Get attribute names from attribute
			$attribute_names = array_keys($attributes);
			if (!empty($attribute_names)) {
				foreach ($attribute_names as $taxonomy) {
					if (taxonomy_exists($taxonomy)) {
						...
						if (!empty($terms)) {
							foreach ($terms as $term) {
								if ($term->slug == $attributes[$taxonomy]) {
		...
		public function acotrs_order_delivery_date($afterDay)
		{
			...
			if ($this->acotrs_method->ad_weekends && count($this->acotrs_method->ad_weekends) > 0) {
				...
				$dayname = array();
				for ($i = 0; $i <= $afterDay; $i++) {
					$dayname[] = [
						'name' => gmdate('l', strtotime(gmdate('Y-m-d') . ' + ' . $i . ' days')),
				...
				$targetDate = gmdate('Y-m-d', strtotime(gmdate('Y-m-d') . ' + ' . $afterDay . ' days'));


				// Initialize count for holidays
				$holiday_counts = 0;

				// Loop through each day in $dayname
				foreach ($dayname as $day) {

					$current_day = $day['name']; // Get the day name

					$is_holiday = in_array(strtolower($current_day), array_column($this->acotrs_method->ad_weekends, 'name'));

					// If it's a holiday, increment its count
					if ($is_holiday) {
						$holiday_counts++;
					}
				...
				$delivery_day_is_holiday = gmdate('l', strtotime(gmdate('Y-m-d') . ' + ' . $afterDay + $holiday_counts . ' days'));

				while (array_search(strtolower($delivery_day_is_holiday), array_column($this->acotrs_method->ad_weekends, 'name')) !== false) {
					$holiday_counts++;
					$delivery_day_is_holiday = gmdate('l', strtotime(gmdate('Y-m-d') . ' + ' . $afterDay + $holiday_counts . ' days'));
				}

				$delivery_date = gmdate('F\ d, Y', strtotime(gmdate('Y-m-d') . ' + ' . $afterDay + $holiday_counts . ' days'));


			} else {
				$delivery_date = gmdate('F\ d, Y', strtotime(gmdate('Y-m-d') . ' + ' . $afterDay . ' days'));
			}
