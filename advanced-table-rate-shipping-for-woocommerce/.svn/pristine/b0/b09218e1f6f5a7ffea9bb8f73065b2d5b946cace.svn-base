...
class ACOTRS_Shipping extends WC_Shipping_Method {
	 ...
	 * @param mixed $acotrs_instance_id
	 */
	public $acotrs_instance_id;

	/**
	 * Summary of method_title
	 * @var 
	 */
	public $method_title;
	...
	/**
	 * Summary of method_description
	 * @var 
	 */
	public $method_description;

	/**
	 * Summary of table_rate_options
	 * @var 
	 */
	public $table_rate_options;

	/**
	 * Summary of class_priorities_options
	 * @var 
	 */
	public $class_priorities_options;

	/**
	 * Summary of handling_rates_options
	 * @var 
	 */
	public $handling_rates_options;

	/**
	 * Summary of title_order_options
	 * @var 
	 */
	public $title_order_options;
	/**
	 * Summary of option_name
	 * @var 
	 */
	public $option_name;
	 ...
	 */
	public function __construct( $acotrs_instance_id = 0) {
		parent::__construct( $acotrs_instance_id );
		$this->id           			= self::METHOD_ID;
		$this->enabled      			= 'no';
		$this->acotrs_instance_id 		= absint( $acotrs_instance_id );
		$this->method_title 			= __('Advanced table rate shipping', 'aco-table-rate-shipping');
		$this->config 					= array();
		$this->zone_id 					= WC_Shipping_Zones::get_zone_by('instance_id', $this->acotrs_instance_id)->get_id();

		$this->method_description 		= __( 'Table rate shipping allows you to set numerous rates based on location and specified conditions. Click the headlines on left to expand or hide additional settings.', 'aco-table-rate-shipping' );
		...
		add_filter( 'woocommerce_shipping_chosen_method', array( $this, 'acotrs_select_default_rate' ), 10, 3 );
	...
	public function acotrs_custom_restriction_callback($result_array, $package, $method){
		...
		foreach( $method_condtions as $cond ) {			
			...
			if( $results[$cond['condition']] === false ) {
				$result_array[] = false;
			}
	...
	*/
	...
	/**
	 * Summary of tax_status
	 * @var 
	 */
	public $tax_status;

	/**
	 * Summary of base_rule_condition
	 * @var 
	 */
	public $base_rule_condition;

	/**
	 * Summary of handlingfree
	 * @var 
	 */
	public $handlingfree;
	...
	/**
	 * Summary of shipping_option_appear_for
	 * @var 
	 */
	public $shipping_option_appear_for;

	/**
	 * Summary of ship_to_role
	 * @var 
	 */
	public $ship_to_role;

	/**
	 * Summary of city_enable
	 * @var 
	 */
	public $city_enable;

	/**
	 * Summary of desable_other
	 * @var 
	 */
	public $desable_other;

	/**
	 * Summary of region_is
	 * @var 
	 */
	public $region_is;

	/**
	 * Summary of allowed_city
	 * @var 
	 */
	public $allowed_city;
	...
	/**
	 * Summary of exclude_weight
	 * @var 
	 */
	public $exclude_weight;

	/**
	 * Summary of method_condtion
	 * @var 
	 */
	public $method_condtion;

	/**
	 * Summary of ad_includingtax
	 * @var 
	 */
	public $ad_includingtax;

	/**
	 * Summary of ad_exclude_weight
	 * @var 
	 */
	public $ad_exclude_weight;

	/**
	 * Summary of ad_include_coupons
	 * @var 
	 */
	public $ad_include_coupons;

	/**
	 * Summary of ad_round_weight
	 * @var 
	 */
	public $ad_round_weight;

	/**
	 * Summary of ad_hide_this_method
	 * @var 
	 */
	public $ad_hide_this_method;

	/**
	 * Summary of ad_hide_other_method
	 * @var 
	 */
	public $ad_hide_other_method;

	/**
	 * Summary of ad_set_delivery_date_automatically
	 * @var 
	 */
	public $ad_set_delivery_date_automatically;

	/**
	 * Summary of ad_set_delivery_date
	 * @var 
	 */
	public $ad_set_delivery_date;

	/**
	 * Summary of ad_get_minimum_value_from_condition
	 * @var 
	 */
	public $ad_get_minimum_value_from_condition;

	/**
	 * Summary of ad_weekend_days
	 * @var 
	 */
	public $ad_weekend_days;

	/**
	 * Summary of ad_weekends
	 * @var 
	 */
	public $ad_weekends;

	/**
	 * Summary of ad_free_shipping_on_special_day
	 * @var 
	 */
	public $ad_free_shipping_on_special_day;

	/**
	 * Summary of ad_special_days
	 * @var 
	 */
	public $ad_special_days;

	/**
	 * Summary of ad_special_day_persent
	 * @var 
	 */
	public $ad_special_day_persent;

	/**
	 * Summary of ad_single_class
	 * @var 
	 */
	public $ad_single_class;

	/**
	 * Summary of table_of_rates
	 * @var 
	 */
	public $table_of_rates;
	...
	*/
	public function init() {
			...
			$this->ad_set_delivery_date 				= isset($this->config['additional_settings']) && isset($this->config['additional_settings']['ad_set_delivery_date']) ? $this->config['additional_settings']['ad_set_delivery_date'] : 1;
...
// send shipping rates to WooCommerce
    ...
    foreach( $rates as $key => $rate ) {
        ...
        if( isset($rate['default']) && (int)$rate['default'] == 1 ){
	...
	*/
	public function acotrs_select_default_rate($chosen_method, $_available_methods, $package) {

		   if (!empty($this->default) && isset($_available_methods[$this->default])) {
	...
	public function acotrs_hide_shipping_if_free_method_is_available( $rates ) {
			...
			if(!$this->desable_other && $this->ad_hide_this_method && count($rates) > 0){
				...
				foreach ( $rates as $rate_id => $rate ) {   
					// if ( 'free_shipping' === $rate->method_id ) { // 17-> BUG FIXED
					if ( 0 == $rate->cost ) {
			...
			if( $free_shipping ) { 
				foreach ( $rates as $rate_id => $rate ) {
					if (  $rate ->instance_id !==  $instance_id  &&  $this->id === $rate->method_id && strpos( $rate_id, $this->id . ':' . $this->acotrs_instance_id . '-') !== false ) {
						unset( $rates[ $rate_id ] );
					}
				}
			}
			...
			if($this->desable_other){
				...
				foreach ( $rates as $rate_id => $rate ) {
					if ($this->id !== $rate->method_id && strpos( $rate_id, $this->id . ':' . $this->acotrs_instance_id . '-') !== true && in_array($woocommerce->customer->get_shipping_city(), $citys) ) {
						unset( $rates[ $rate_id ] );
					}
				}
			}
