...
namespace recaptcha_for_all_BillDiagnose;
...
If you need to dive deeper, install the free plugin WPTools, which provides more in-depth insights.',
        ...
        $screen->add_help_tab([
            ...
            'title'   => esc_attr__('Memory & Error Monitoring', 'recaptcha-for-all'), // Título da aba
            'content' => '<p>' . esc_attr__('Welcome to plugin Insights!', 'recaptcha-for-all') . '</p>
                          <p>' . $hmessage . '</p>',
        ]);
    }
}
...
class recaptcha_for_all_Bill_Diagnose
{
    ...
    public function show_dismissible_notification()
    {
        ...
        return;
        // nao destá dismissing...



        // Verifica se a notificação já foi exibida hoje
        if ($this->is_notification_displayed_today()) {
            return;
        }

        // Define a variável $memory a partir da propriedade global
        $memory = $this->global_variable_memory;

        // Verifica se $memory não está definida ou não é um array
        if (!isset($memory) || !is_array($memory)) {
            return;
        }

        // Verifica se $memory é nulo
        if (is_null($memory)) {
            return;
        }

        if (!array_key_exists("free", $memory) || !array_key_exists("percent", $memory)) {
            return;
        }

        // Verifica condições para exibir ou não a notificação
        if ($memory["free"] > 30 && $memory["percent"] < 0.85) {
            return;
        }

        // 2 2025
        $message = esc_attr__("Our plugin", 'recaptcha-for-all');
        $message .= ' (' . $this->global_plugin_slug . ') ';
        //echo ': ' . esc_attr($this->global_plugin_slug) . '. ';

        //Warning: Undefined property: recaptcha_for_all_BillDiagnose\recaptcha_for_all_Bill_Diagnose::$plugin_slug in /home/minozzi/public_html/wp-content/plugins/recaptcha-for-all/includes/diagnose/class_bill_diagnose.php on line 652



        $message .= esc_attr__("cannot function properly because your WordPress Memory Limit is too low. Your site will experience serious issues, even if you deactivate our plugin.", 'recaptcha-for-all');
        $message .=
            '<a href="' .
            esc_url($this->notification_url) .
            '">' .
            " " .
            esc_attr__("Learn more", 'recaptcha-for-all') .
            "</a>";
    ...
    public function site_health_tab_content($tab)
    {
        ...
        <div class="wrap health-check-body, privacy-settings-body">
            <p style="border: 1px solid red; padding: 10px;">
                <strong>
                    <?php
                    echo esc_attr__("Displaying the latest recurring errors (Javascript Included) from your error log file and eventually alert about low WordPress memory limit is a courtesy of plugin", 'recaptcha-for-all');
                    echo ': ' . esc_attr($this->global_plugin_slug) . '. ';
                    echo esc_attr__("Disabling our plugin does not stop the errors from occurring; it simply means you will no longer be notified here that they are happening, but they can still harm your site.", 'recaptcha-for-all');
                    echo '<br>';
                    echo esc_attr__("Click the help button in the top right or go directly to the AI chat box below for more specific information on the issues listed.", 'recaptcha-for-all');
                    ?>
                </strong>
            </p>
            <!-- chat -->
            <div id="chat-box">
                <div id="chat-header">
                    <h2><?php echo esc_attr__("Artificial Intelligence Support Chat for Issues and Solutions", 'recaptcha-for-all'); ?></h2>
                </div>
                ...
                <form id="chat-form">
                    <div id="input-group">
                        <input type="text" id="chat-input" placeholder="<?php echo esc_attr__('Enter your message...', 'recaptcha-for-all'); ?>" />
                        <button type="submit"><?php echo esc_attr__('Send', 'recaptcha-for-all'); ?></button>
                    </div>
                    <div id="action-instruction" style="text-align: center; margin-top: 10px;">
                        <span><?php echo esc_attr__("Enter a message and click 'Send', or just click 'Auto Checkup' to analyze error log ou server info configuration.", 'recaptcha-for-all'); ?></span>
                    </div>
                    <div class="auto-checkup-container" style="text-align: center; margin-top: 10px;">
                        ...
                        <button type="button" id="auto-checkup">
                            ...
                            <?php echo esc_attr__('Auto Checkup for Errors', 'recaptcha-for-all'); ?>
                        </button>
                        &nbsp;&nbsp;&nbsp;
                        <button type="button" id="auto-checkup2">
                            <img src="<?php echo plugin_dir_url(__FILE__) . 'robot2.png'; ?>" alt="" width="35" height="30">
                            <?php echo esc_attr__('Auto Checkup Server ', 'recaptcha-for-all'); ?>
                        </button>
            ...
            <h3 style="color: red;">
                <?php
                echo esc_attr__("Potential Problems", 'recaptcha-for-all');
                ?>
            </h3>
            ...
            <div id="accordion2">
                ...
                // Verifica se $memory não está definida ou não é um array
                if (!isset($wpmemory) || !is_array($wpmemory)) {
                    return;
                }
                ...
                ?>
                    <!-- Título da seção -->
                    <h2 style="color: red;">
                        <?php echo esc_attr__("Low WordPress Memory Limit (click to open)", 'recaptcha-for-all'); ?>
                    </h2>
                    ...
                    <div>
                        <b>
                            ...
                            echo esc_attr__("Your usage now", 'recaptcha-for-all') . ": " . esc_attr($wpmemory["usage"]) . "MB &nbsp;&nbsp;&nbsp;";
                            if ($perc > 0.7) {
                                echo "</span>";
                            }
                            echo "|&nbsp;&nbsp;&nbsp;" . esc_attr__("Total Php Server Memory", 'recaptcha-for-all') . " : " . esc_attr($wpmemory["limit"]) . "MB";
                            ?>
                        </b>
                        <hr>
                        <?php
                        ...
                        echo '<p>';
                        echo esc_attr__("Your WordPress Memory Limit is too low, which can lead to critical issues on your site due to insufficient resources. Promptly address this issue before continuing.", 'recaptcha-for-all');
                        echo '</p>';
                        ?>
                        <a href="https://wpmemory.com/fix-low-memory-limit/">
                            <?php echo esc_attr__("Learn More", 'recaptcha-for-all'); ?>
                        </a>
                    </div>
                <?php } 
                ...
                */
                ...
                // Condição para exibir o aviso
                if ($wpmemory["free"] < 30 || $wpmemory["percent"] > 0.85) {
                    echo '<h2 style="color: red;">';
                    echo esc_attr__("Low WordPress Memory Limit (click to open)", 'recaptcha-for-all');
                    echo '</h2>';
                    echo '<div>';
                    echo '<b>';

                    $mb = "MB";
                    // Usa $wpmemory["percent"] diretamente em vez de recalcular
                    if ($wpmemory["percent"] > 0.7) {
                        echo '<span style="color:' . esc_attr($wpmemory["color"]) . ';">';
                    }
                    echo sprintf(
                        "WordPress Memory Limit: %s%s | %s: %s%s | %s: %s%s",
                        esc_attr($wpmemory["wp_limit"]),
                        esc_attr($mb),
                        esc_attr__("Your usage now", 'recaptcha-for-all'),
                        esc_attr($wpmemory["usage"]),
                        esc_attr($mb),
                        esc_attr__("Total Php Server Memory", 'recaptcha-for-all'),
                    ...
                    echo esc_attr__("Learn More", 'recaptcha-for-all');
                    echo '</a>';
                    echo '</div>';
                }
            ...
            if ($average > 5) {
                ...
                if ($average <= 8) {
                    $message = esc_html__("The page load time is poor (click to open)", 'recaptcha-for-all');
                } else {
                    $message = esc_html__("The page load time is very poor (click to open)", 'recaptcha-for-all');
                }
                ...
                echo esc_html__("The Load average of your front pages is: ", 'recaptcha-for-all');
                echo esc_html($average);
                echo '<br>';
                echo esc_html__("Loading time can significantly impact your SEO.", 'recaptcha-for-all');
                echo '<br>';
                echo esc_html__("Many users will abandon the site before it fully loads.", 'recaptcha-for-all');
                echo '<br>';
                echo esc_html__("Search engines prioritize faster-loading pages, as they improve user experience and reduce bounce rates.", 'recaptcha-for-all');
                //}
                echo '<br>';
                echo '<br>';
                echo '<strong>';
                echo esc_html__("Suggestions:", 'recaptcha-for-all') . '<br>';
                echo '</strong>';
                echo esc_html__("Block bots: They overload the server and steal your content. Install our free plugin Antihacker.", 'recaptcha-for-all') . '<br>';
                echo esc_html__("Protect against hackers: They use bots to search for vulnerabilities and overload the server. Install our free plugin AntiHacker", 'recaptcha-for-all') . '<br>';

                echo esc_html__("Check your site for errors with free plugin wpTools. Errors and warnings can increase page load time by being recorded in log files, consuming resources and slowing down performance.", 'recaptcha-for-all');
                echo '<br>';

                echo '<br>';
                echo '<a href="https://wptoolsplugin.com/page-load-times-and-their-negative-impact-on-seo/">';
                echo esc_html__("Learn more about Page Load Times and their negative impact on SEO and more", 'recaptcha-for-all') . "...";
                echo "</a>";
            ...
            if (count($update_plugins) > 0) {
                ...
                esc_attr_e("Keeping your plugins up to date is crucial for ensuring security, performance, and compatibility with the latest features and improvements.", 'recaptcha-for-all');
                echo '<br>';
                echo '<strong>';
                esc_attr_e("Our free AntiHacker plugin can even check for abandoned plugins that you are using, as these plugins may no longer receive security updates, leaving your site vulnerable to attacks and potential exploits, which can compromise your site's integrity and data.", 'recaptcha-for-all');
                echo '<strong>';
                echo '<hr>';
    ...
    <?php
                ...
                echo "<br />";
                echo esc_attr__("This is a partial list of the errors found.", 'recaptcha-for-all');
                echo "<br />";
                /**
                ...
                function getFileSizeInBytes($bill_filename)
                {
                    if (!file_exists($bill_filename) || !is_readable($bill_filename)) {
                        ...
                        return esc_attr__("File not readable.", 'recaptcha-for-all');
                    }
                    $fileSizeBytes = filesize($bill_filename);
                    if ($fileSizeBytes === false) {
                        //return "Size not determined.";
                        return esc_attr__("Size not determined.", 'recaptcha-for-all');
                    }
                ...
                function convertToHumanReadableSize($sizeBytes)
                {
                    if (!is_int($sizeBytes) || $sizeBytes < 0) {
                        ...
                        return esc_attr__("Invalid size.", 'recaptcha-for-all');
                    }
                    $units = ['B', 'KB', 'MB', 'GB', 'TB'];
                    $unitIndex = 0;
                ...
                foreach ($bill_folders as $bill_folder) {
                    ...
                    foreach ($files as $bill_filename) {
                        if (strpos($bill_filename, "backup") != true) {
                            ...
                            echo "<br />";
                            echo esc_attr__("File Size: ", 'recaptcha-for-all');
        ...
        public function custom_help_tab()
        {
            ...
            if ("site-health" === $screen->id) {
                ...
                $screen->add_help_tab([
                    "id"      => "custom-help-tab",
                    "title"   => esc_attr__("Critical Issues", 'recaptcha-for-all'),
                    "content" => "<p>" . $message . "</p>",
                ]);
            }
        }
