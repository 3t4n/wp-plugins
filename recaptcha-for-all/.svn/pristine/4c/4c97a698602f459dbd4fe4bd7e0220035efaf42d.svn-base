...
namespace recaptcha_for_all_BillChat;
...
class ChatPlugin
{
    ...
    public function enqueue_chat_scripts()
    {
        ...
        wp_localize_script('chat-script', 'bill_data', array(
            ...
            'reset_success'            => esc_attr__('Chat messages reset successfully.', 'recaptcha-for-all'),
            'reset_error'              => esc_attr__('Error resetting chat messages.', 'recaptcha-for-all'),
            'invalid_message'          => esc_attr__('Invalid message received:', 'recaptcha-for-all'),
            'invalid_response_format'  => esc_attr__('Invalid response format:', 'recaptcha-for-all'),
            'response_processing_error' => esc_attr__('Error processing server response:', 'recaptcha-for-all'),
            'not_json'                 => esc_attr__('Response is not valid JSON.', 'recaptcha-for-all'),
            'ajax_error'               => esc_attr__('AJAX request failed:', 'recaptcha-for-all'),
            'send_error'               => esc_attr__('Error sending the message. Please try again later.', 'recaptcha-for-all'),
            'empty_message_error'      => esc_attr__('Please enter a message!', 'recaptcha-for-all'),
        ));
    }
    /**
    ...
    public function bill_chat_call_chatgpt_api($data, $chatType, $chatVersion)
    {
        ...
        //2025
        $recaptcha_for_all_checkup = \recaptcha_for_all_sysinfo_get();
        //  \debug3( \wp-memory_sysinfo_get());

        //\debug3($wp-memory_checkup);



        $data2 = [
            'param1' => $data,
            'param2' => $recaptcha_for_all_checkup,
        ...
        if (isset($data['success']) && $data['success'] === true) {
            ...
            $message = esc_attr__("Error contacting the Artificial Intelligence (API). Please try again later.", 'recaptcha-for-all');
        }
        // debug2($message);
        return $message;
    }
    ...
    public function bill_chat_send_message()
    {
        ...
        if (empty($message)) {
            if ($chatType == 'auto-checkup') {
                $message = esc_attr("Auto Checkup for Erros button clicked...", 'recaptcha-for-all');
            } elseif ($chatType == 'auto-checkup2') {
                $message = esc_attr("Auto Checkup Server button clicked...", 'recaptcha-for-all');
            }
        }

        //  if (empty($message)) {
        //    $message = esc_attr("Auto Checkup button clicked...", 'recaptcha-for-all');
        // }
        ...
        if (!empty($response_data)) {
            ...
            $output = esc_attr__("Error to get response from AI source!", 'recaptcha-for-all');
        }
