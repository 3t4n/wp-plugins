...
$output .= '<li class="vskb-cat-list '.esc_attr($vskb_term->slug).'">';
	...
	$output .= '<div class="vskb-cat-name"><a href="'.esc_url(get_term_link( $vskb_term->term_id )).'" title="'.esc_attr($vskb_term->name).'" >'.esc_attr($vskb_term->name).'</a> '.$vskb_post_count.'</div>';
	...
	$output .= '<ul class="vskb-post-list">';
		...
		foreach( $vskb_posts as $vskb_post ) :
			...
			if (get_the_title( $vskb_post->ID ) == false) {
				...
				$title = get_the_title( $vskb_post->ID );
			}
			$slug = str_replace(' ', '-', strtolower($title));
			$output .= '<li class="vskb-post '.esc_attr($slug).'">';
			$output .= '<div class="vskb-post-name"><a href="'.get_permalink( $vskb_post->ID ).'" rel="bookmark" title="'.esc_attr($title).'">'.esc_attr($title).'</a></div>';
			// post meta
			if ($vskb_atts['post_meta'] == "true") {
				$output .= '<div class="vskb-post-meta">';
				$output .= '<span class="vskb-post-meta-date"><a href="'.esc_url( get_permalink( $vskb_post->ID ) ).'">'.esc_attr( get_the_date(get_option( 'date_format' ), $vskb_post->ID) ).'</a></span>';
				$output .= '<span class="vskb-post-meta-sep"> | </span>';
				$output .= '<span class="vskb-post-meta-author">'.sprintf( '<a href="%1$s">%2$s</a>', esc_url( get_author_posts_url( $vskb_post->post_author ) ), esc_attr( get_the_author_meta( 'display_name', $vskb_post->post_author ) ) ).'</span>';
				$output .= '</div>';
			}
			$output .= '</li>';
		endforeach;
	...
	if ($vskb_atts['view_all_link'] == "true") {
		...
		$output .= '<a href="'.get_term_link( $vskb_term->term_id ).'" title="'.esc_attr($vskb_term->name).'" >'.esc_attr($vskb_atts['view_all_link_label']).'</a>';
		$output .= '</div>';
	}
$output .=  '</li>';
