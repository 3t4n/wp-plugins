...
class Transfer {
    ...
    public function __construct()
    {
        if (current_user_can('activate_plugins')) {
            ...
            add_action('wp_ajax_check_premium_api_keys', [ $this, "checkPremiumApiKeys"]);
	...
	public function wpSiteCheck()
	{
        ...
        /**
         * Check if the user has exceeded the max size
         */
        $this->checkSiteWithinFreeTier($siteDetails);
		...
		/**
		 * Return the template for all users
		 */
        $response = [
    ...
    public function checkPremiumApiKeys()
    {
        check_ajax_referer('in_plugin_premium_upgrade', 'securityKey');

        try {
            /**
             * Check the user's API keys
             */
            $hasValidAPIKeys = $this->areAPIKeysValid(
                sanitize_text_field($_POST['data']['publicKey']),
                sanitize_text_field($_POST['data']['secretKey'])
            );

            /**
             * If the API Keys aren't valid
             * Throw an error
             */
            if (!$hasValidAPIKeys) {
             ...
             * Get Current Options
             */
            $options = get_option('transferito_settings_option');

            /**
             * Update the API Keys
             */
            $options['public_transferito_key'] = sanitize_text_field($_POST['data']['publicKey']);
            $options['secret_transferito_key'] = sanitize_text_field($_POST['data']['secretKey']);

            /**
             * Update the options
             */
            update_option('transferito_settings_option', $options);
            ...
            wp_send_json_error([ 'validated' => true ], 403);
        }
    }
	...
	public function cPanelCheck()
	{
        ...
        $localMigration = filter_var($_POST['localMigration'], FILTER_VALIDATE_BOOL);

        /**
         * If migrating to use the DesktopApp
         */
        if ($localMigration) {
             ...
             */
            $transferMethod = 'localSiteMigration';
            ...
            wp_send_json_success([
                'transferMethod'    => $transferMethod,
                'cpanelAllowed'     => false,
                'securityToken'     => wp_create_nonce("prepare_migration_files")
            ]);


//            $this->prepareLocalDownload();
...
//            ]);
	...
	public function databaseValidation()
	{
		try {
			 ...
			 */
			wp_send_json_success([
                'migrationDetail'   => array_map('stripslashes', $migrationDetail),
    ...
    public function prepareDownload()
    {
    	try {
             ...
             */
            $hasValidAPIKeys = $this->areAPIKeysValid(
                $settings['public_transferito_key'],
                $settings['secret_transferito_key']
            );

            /**
             * Get the Transfer Method
             */
            $transferMethod = $_POST['migrationDetails']['transferMethod'];

            /**
             * Fire the required upgrade flow
             */
            if (!$hasValidAPIKeys && $siteDetails['maxSizeExceeded']) {
            ...
            /**
             * If a local migration forward to the migration method
             */
            if ($transferMethod === 'localSiteMigration') {
                $this->prepareLocalDownload($hasValidAPIKeys);
                die();
            }
            ...
            /**
             * Get the current domain & add it to the migrationDetails
             */
            $migrationDetails['currentDomain'] = site_url();
		     ...
		     */
            $migrationPayload = array_merge($migrationDetails, $additionalData);
            $migrationPayload['isLocal'] = $siteAccessed;
            $migrationPayload['currentPath'] = ABSPATH;
    ...
    private function prepareLocalDownload($premiumUser)
    {
        try {
             ...
             * Create a migration and return a token
             */
            $createdMigration = $this->api->createMigration([
                'currentDomain'  => site_url(),
                'transferMethod' => $transferMethod,
                'domain'         => $destinationURL,
                'isLocal'        => true,
                'currentPath'    => ABSPATH
             ...
             */
            set_transient('transferito_transfer_detail', [
                'isLocalEnv'        => true, // Defaulted to true as will always upload to S3 in this instance
                'selectedFolders'   => false,
                'folders'           => null, // No folders have been selected set directly to null
                'directory'         => $backupDirectory,
                'token'             => $migrationToken,
                'timestamp'         => $createdMigration['message']->timestamp,
                'fromUrl'           => site_url(),
            ...
            $this->telemetry->pushEvent('migrationDetails', [
                'uploadBackup'          => true,
                'localMigration'        => true,
                'databaseExcluded'      => false,
                'selectedFolders'       => false,
                'migrationMethod'       => $transferMethod,
                'siteSize'              => isset($siteSize['totalSize']) ? $siteSize['totalSize'] : 0,
                'cPanelAPIToken'        => false
            ]);
            ...
            /**
             * @todo Implement a failure screen - If the migration creation fails
             */
            ...
            $errorMessage = 'prepareLocalDownload: ' . $exception->getMessage();
            $this->api->failedMigration($errorMessage);
            delete_transient('transferito_transfer_detail');
    ...
    public function startMigration()
    {
        ...
        $settings = get_option('transferito_settings_option');
        $chunkSize = isset($settings['transferito_chunk_size']) ? $settings['transferito_chunk_size'] : '10';
        $transferDetail = get_transient('transferito_transfer_detail');
        $installationInfo = get_transient('transferito_installation_size');
        $dbCharsetInfo = get_transient('transferito_database_charset_info');

        /**
         * Check if the migration is a local migration or not
         */
        $localMigration = get_transient('transferito_transfer_method') === 'localSiteMigration';
    ...
    public function cleanUp($url = null)
    {
        ...
        $completionHTMLTemplate = ($localMigration)
            ? 'parts/migration/local-migration-completed'
            : 'parts/migration/completed';
    	...
    	try {
		    ...
		    if ($validError) {
			    ...
			    $htmlTemplate = loadTemplate($completionHTMLTemplate, [
			        'url'       => $url,
                    'metadata'  => $metadata
                ]);
		    }
		    ...
		    if ($validError) {
			    ...
			    $htmlTemplate = loadTemplate($completionHTMLTemplate, [ 'url' => $url, 'error' => 'FAILED_BACKUP_REMOVAL' ]);
		    }
	    }
    ...
    private function areAPIKeysValid($publicKey, $secretKey) {
        $valid = null;

        try {
            $planInfo = $this->api->planInformation([
                'publicTransferito_APIKey'  => $publicKey,
                'secretTransferito_APIKey'  => $secretKey
            ]);
            $code = $planInfo['code'];
             ...
             */
            if ($code !== 200) {
                throw new \Exception('INVALID_API_KEYS');
            }
            ...
            $valid = ($code === 200 && $availableTransfers !== 0);
	...
	private function checkSiteWithinFreeTier($siteDetails)
	{
		try {

            /**
             * Default the user status to FREE
             */
            $userStatus = 'FREE';

            /**
             * Default if the user has valid API keys to false
             */
            ...
            if ($this->options && $this->options['public_transferito_key'] && $this->options['secret_transferito_key']) {
                 ...
                 */
                $hasValidAPIKeys = $this->areAPIKeysValid(
                    $this->options['public_transferito_key'],
                    $this->options['secret_transferito_key']
                );

                /**
                 * Only set the status if the API keys are valid
                 */
                if ($hasValidAPIKeys) {
                    $userStatus = 'PREMIUM';
                }
             ...
             */
            set_transient('transferito_user_status', $userStatus);

            /**
             * Only update the FREE User Options
             * If the user doesn't have Valid API Keys
             */
            if (!$hasValidAPIKeys) {
                /**
