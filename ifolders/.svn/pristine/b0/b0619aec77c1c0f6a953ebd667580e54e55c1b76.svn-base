...
class Folders {
    ...
    public function sidebarScripts() {
       if ( UserModel::hasAccess() ) {
            ...
            wp_enqueue_script( 'ifolders-cookie', IFOLDERS_PLUGIN_URL . 'assets/vendor/cookie/cookie.js', [], IFOLDERS_PLUGIN_VERSION, false );
            wp_enqueue_script( 'ifolders-url', IFOLDERS_PLUGIN_URL . 'assets/vendor/url/url.js', [], IFOLDERS_PLUGIN_VERSION, false );

            wp_enqueue_style( 'ifolders-overlayscrollbars', IFOLDERS_PLUGIN_URL . 'assets/vendor/overlayscrollbars/overlayscrollbars.css', [], IFOLDERS_PLUGIN_VERSION, false );
            wp_enqueue_script( 'ifolders-overlayscrollbars', IFOLDERS_PLUGIN_URL . 'assets/vendor/overlayscrollbars/overlayscrollbars.js', [], IFOLDERS_PLUGIN_VERSION, false );
            ...
            wp_enqueue_script( 'ifolders-tree', IFOLDERS_PLUGIN_URL . 'assets/js/tree.js', ['jquery'], IFOLDERS_PLUGIN_VERSION, false );

            wp_enqueue_style( 'ifolders-sidebar', IFOLDERS_PLUGIN_URL . 'assets/css/sidebar.css', [], IFOLDERS_PLUGIN_VERSION );
            wp_enqueue_script( 'ifolders-sidebar', IFOLDERS_PLUGIN_URL . 'assets/js/sidebar.js', ['jquery'], IFOLDERS_PLUGIN_VERSION, false );
            wp_localize_script( 'ifolders-sidebar', 'ifolders_sidebar_globals', $this->getGlobals() );
       }
    }
    ...
    public function postsСlauses( $clauses, $query ) {
        ...
        if ( $query->get( 'post_type' ) == $type && strpos( $clauses['where'], $type ) ) {
            ...
            $mode = sanitize_key( filter_input( INPUT_POST, 'ifolders_mode', FILTER_DEFAULT ) );
    ...
    public function prepareAttachment( $response, $attachment ) {
        ...
        if ( array_key_exists( 'media_hover_details', $config ) && $config['media_hover_details'] ) {
            ...
            if ( count( $list ) > 0 ) {
                $preview_details = '<div class="ifs-preview-details">';

                if ( in_array( 'title', $list ) ) {
                    $preview_details .= '<p>' . esc_html__( "Title", 'ifolders' ) . ': ' . $attachment->post_title . '</p>';
                }
                if ( in_array( 'alternative_text', $list ) ) {
                    $preview_details .= '<p>' . esc_html__( "Alternative text", 'ifolders' ) . ': ' . get_post_meta( $attachment_id , '_wp_attachment_image_alt', true ) . '</p>';
                }

                if ( in_array( 'file_url', $list ) ) {
                    $preview_details .= '<p>' . esc_html__( "File URL", 'ifolders' ) .': ' . $attachment->guid . '</p>';
                }
                ...
                if ( in_array( 'dimension', $list ) ) {
                    ...
                    if ( isset( $attachment_meta ) && !empty( $attachment_meta ) && array_key_exists( 'width', $attachment_meta ) ) {
                        $preview_details .= '<p>' . esc_html__( "Dimension", 'ifolders' ) . ': ' . $attachment_meta['width'] . ' x ' . $attachment_meta['height'] . '</p>';
                    }
                }

                if ( in_array( 'size', $list ) ) {
                    $preview_details .= '<p>' . esc_html__( "Size", 'ifolders' ) . ': ' . size_format( filesize( get_attached_file( $attachment->ID ) ), 0) . '</p>';
                }

                if ( in_array( 'filename', $list ) ) {
                    $preview_details .= '<p>' . esc_html__( "Filename", 'ifolders' ) . ': ' . $attachment->post_name . '</p>';
                }

                if ( in_array( 'type', $list ) ) {
                    $preview_details .= '<p>' . esc_html__( "Type", 'ifolders' ) . ': ' . $attachment->post_mime_type . '</p>';
                }

                if ( in_array( 'date', $list ) ) {
                    $preview_details .= '<p>' . esc_html__( "Date", 'ifolders' ) . ': ' . date_i18n( get_option( 'date_format' ), strtotime( $attachment->post_date ) ) . '</p>';
                }

                if ( in_array( 'uploaded_by', $list ) ) {
                    $preview_details .= '<p>' . esc_html__( "Uploaded by", 'ifolders' ) . ': ' . get_the_author_meta('display_name', $attachment->post_author) . '</p>';
                }
