...
class WC_Paybox
{
    ...
    public function buildSystemParams(WC_Order $order, $type, array $additionalParams = array())
    {
        ...
        $values['PBX_VERSION'] = WC_PAYBOX_PLUGIN . "-" . WC_PAYBOX_VERSION . "_WP" . get_bloginfo('version') . "_WC" . WC()->version;
    ...
    {
        $firstName = $this->formatTextValue($order->get_billing_first_name(), 'ANS', 22);
        $lastName = $this->formatTextValue($order->get_billing_last_name(), 'ANS', 22);
        ...
        $zipCode = $this->formatTextValue($order->get_billing_postcode(), 'ANS', 10);
        $city = $this->formatTextValue($order->get_billing_city(), 'ANS', 50);
        $countryCode = (int)WC_Paybox_Iso3166_Country::getNumericCode($order->get_billing_country());
        ...
        $mobilePhoneIntCode = WC_Paybox_Iso3166_Country::getPhoneCode($order->get_billing_country());
        $mobilePhoneNumber = $this->formatTextValue($order->get_billing_phone(), 'N', 10);
