...
abstract class WC_Paybox_Abstract_Gateway extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->icon = apply_filters(WC_PAYBOX_PLUGIN, plugin_dir_url(__DIR__) . 'images/') . apply_filters('icon', $this->_config->getIcon());
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['icon'] = array(
            'title' => __('Icon file', WC_PAYBOX_PLUGIN),
            'type' => 'select',
            'description' => __('Icon file to be displayed to customers. file are located in: ', WC_PAYBOX_PLUGIN) . apply_filters(WC_PAYBOX_PLUGIN, '' . plugin_dir_url(__DIR__) . 'images/'),
            'default' => __($defaults['icon'], WC_PAYBOX_PLUGIN),
            'options' => $fileList,
        );
        ...
        $this->form_fields['ips'] = array(
            'title' => __('IPN IPs', WC_PAYBOX_PLUGIN),
            'type' => 'text',
            'description' => __('A coma separated list of Paybox IPN IPs.', WC_PAYBOX_PLUGIN),
    ...
    public function receipt_page($orderId)
    {
        ...
        if (!is_multisite()) {
            ...
            $urls = array(
                'PBX_ANNULE' => add_query_arg(array('wc-api' => get_class($this), 'status' => 'cancel'), trailingslashit(site_url())),
                'PBX_EFFECTUE' => add_query_arg(array('wc-api' => get_class($this), 'status' => 'success'), trailingslashit(site_url())),
                'PBX_REFUSE' => add_query_arg(array('wc-api' => get_class($this), 'status' => 'failed'), trailingslashit(site_url())),
                'PBX_REPONDRE_A' => add_query_arg(array('wc-api' => get_class($this), 'status' => 'ipn'), trailingslashit(site_url())),
            );
        }
        ...
        try {
            ...
            echo "<form><center><button onClick='history.go(-1);return true;'>" . __('Back...', WC_PAYBOX_PLUGIN) . "</center></button></form>";
            exit;
        }
    ...
    public function on_payment_failed()
    {
        try {
            ...
            $this->logger->error($e, ['source' => WC_PAYBOX_PLUGIN]);
        }
    ...
    public function on_ipn()
    {
        ...
        try {
            ...
            if ($params['error'] == '00000') {
                ...
                $this->_paybox->addOrderPayment($order, 'failed_payment', $params);
                $this->_paybox->addOrderNote($order, $message);
            }
        } catch (Exception $e) {
            $this->logger->error($e, ['source' => WC_PAYBOX_PLUGIN]);
