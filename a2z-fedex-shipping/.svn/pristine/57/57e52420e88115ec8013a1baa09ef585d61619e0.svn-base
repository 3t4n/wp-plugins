...
function shipi_fedex_create_custom_table() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'shipi_fedex_meta';
    ...
    $migrated = get_option('shipi_migration_done');
    if (!$migrated) {
        // Fetch all options related to 'hitshippo_fedex_' prefix
        $options = $wpdb->get_results("SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name LIKE 'hitshippo_fedex_%'");

        foreach ($options as $option) {
            if (preg_match('/hitshippo_fedex_(rates|values|status|return_values|return_status)_(\d+)/', $option->option_name, $matches)) {
                $meta_key = $matches[1]; // 'rates', 'values', 'status', etc.
                ...
                $wpdb->insert(
                    ...
                    [
                        'order_id' => $order_id,
                        'meta_key' => $meta_key,
                        'meta_value' => maybe_serialize($option->option_value),
                    ],
                    ['%d', '%s', '%s']
                );
        ...
        update_option('shipi_migration_done', true);
    }
}

// Run on plugin activation
register_activation_hook(__FILE__, 'shipi_fedex_create_custom_table');
	...
	if( !class_exists('hitshippo_fedex_parent') ){
		Class hitshippo_fedex_parent
		{
			...
			public function __construct() {
				...
				add_action('plugin_loaded', array($this,'shipi_on_plugin_update'));
			...
			function shipi_on_plugin_update() {
				if (!get_option('shipi_migration_done')) {
					shipi_fedex_create_custom_table(); // Run migration only if not done before
					update_option('shipi_migration_done', true); // Mark migration as done
				}
			}
			function shipi_get_meta($order_id, $meta_key, $default = false) {
				global $wpdb;
				$table_name = $wpdb->prefix . 'shipi_fedex_meta';
				...
				return $meta_value !== null ? maybe_unserialize($meta_value) : $default;
			}
			function shipi_update_meta($order_id, $meta_key, $meta_value) {
				global $wpdb;
				$table_name = $wpdb->prefix . 'shipi_fedex_meta';
			...
			function shipi_delete_meta($order_id, $meta_key) {
				global $wpdb;
				$table_name = $wpdb->prefix . 'shipi_fedex_meta';
			...
			function add_tracking_info_to_order_view( $order_id ) {
				$data = $this->shipi_get_meta($order_id, 'values', false);
				if($data){
					$shipment_data = json_decode($data, true);
					if ($shipment_data){
				...
				if(isset($_GET['h1t_updat3_sh1pp1ng']) && isset($_GET['key']) && isset($_GET['user_id']) && isset($_GET['carrier']) && isset($_GET['track'])){
					...
					if(isset($general_settings['hitshippo_fedex_shippo_int_key']) && $general_settings['hitshippo_fedex_shippo_int_key'] == $key){
						...
						if(isset($_GET['label'])){
							...
							$result_arr = !empty($this->shipi_get_meta($order_id, 'values', [])) ? json_decode($this->shipi_get_meta($order_id, 'values', [])) : [];
						...
						$this->shipi_update_meta($order_id, 'values', json_encode($result_arr));
					}
					die();
				}
		    }
		...
		public function create_fedex_return_label_genetation($post){
			...
			$general_settings = get_option('hitshippo_fedex_main_settings',array());
			   ...
			   $json_data = $this->shipi_get_meta($order_id, 'return_values', []);
			   ...
			   if(empty($json_data)){
				...
				$notice = $this->shipi_get_meta($order_id, 'return_status', null);

				if($notice && $notice != 'success'){
					echo "<p style='color:red'>".$notice."</p>";
					$this->shipi_delete_meta($order_id, 'return_status');
				}
...
// echo '<pre>';print_r($custom_settings);die();
				$shipment_data = json_decode($this->shipi_get_meta($order_id, 'values', '[]'), true); 
				$notice = $this->shipi_get_meta($order_id, 'status', null); 
		       	...
		       	if ($notice && $notice == 'success') {
					...
					$this->shipi_delete_meta($order_id, 'status');
...
// echo '<pre>';print_r($shipment_data);die();
		       	foreach ($custom_settings as $ukey => $settingvalue) {
						...
						$select_box_content = "";
						$shipi_rates = $this->shipi_get_meta($order_id, 'rates', false);
						...
						if($shipi_rates){
							...
							$this->shipi_delete_meta($order_id, 'rates');
						}
		...
		public function hitshippo_fedex_wc_checkout_order_processed($order_id){
					...
					if (isset($general_settings['hitshippo_fedex_packing_type']) && $general_settings['hitshippo_fedex_packing_type'] == "box") {
						...
						foreach ($custom_settings as $key => $cvalue) {
							...
							$data['meta'] = array(
								...
								"rest_tracking_api_key" => isset($cvalue['hitshippo_fedex_rest_tracking_api_key']) ? $cvalue['hitshippo_fedex_rest_tracking_api_key'] : "",
								"rest_tracking_secret_key" => isset($cvalue['hitshippo_fedex_rest_tracking_secret_key']) ? $cvalue['hitshippo_fedex_rest_tracking_secret_key'] : "",
			...
			public function hitshippo_create_fedex_return_shipping($order_id){
		    	...
		    	if (  isset( $_POST[ 'hitshippo_fedex_return_reset' ] ) ) {
					$this->shipi_delete_meta($order_id, 'return_values');
		    	}
				...
				$mod_dim_unit = (!empty($config_weg_unit) && $config_weg_unit == 'LB_IN') ? 'in' : 'cm';
					...
					if (empty($pack_products)) {
						$this->shipi_update_meta($order_id, 'return_status', "Product data not found");
						return;
					}
					...
					if(!empty($general_settings) && isset($general_settings['hitshippo_fedex_shippo_int_key'])){
						...
						$data['meta'] = array(
							...
							"rest_tracking_api_key" => isset($custom_settings[$create_shipment_for]['hitshippo_fedex_rest_tracking_api_key']) ? $custom_settings[$create_shipment_for]['hitshippo_fedex_rest_tracking_api_key'] : "",
							"rest_tracking_secret_key" => isset($custom_settings[$create_shipment_for]['hitshippo_fedex_rest_tracking_secret_key']) ? $custom_settings[$create_shipment_for]['hitshippo_fedex_rest_tracking_secret_key'] : "",
						...
						if($output){
							if(isset($output['status'])){
								if(isset($output['status']) && $output['status'] != 'success'){
									$this->shipi_update_meta($order_id, 'return_status', $output['status']);

								}else if(isset($output['status']) && $output['status'] == 'success'){
									$output['user_id'] = $create_shipment_for;
									$val = $this->shipi_get_meta($order_id, 'return_values', []);
									$result_arr = array();
									if(!empty($val)){
										$result_arr = json_decode($val, true);
									}
									$result_arr[] = $output;
									$this->shipi_update_meta($order_id, 'return_values', json_encode($result_arr));

									$this->shipi_update_meta($order_id, 'return_status', $output['status']);
								}
							}else{
								$this->shipi_update_meta($order_id, 'return_status', 'Site not Connected with Shipi. Contact Shipi Team.');
							}
						}else{
							$this->shipi_update_meta($order_id, 'return_status', 'Site not Connected with Shipi. Contact Shipi Team.');
		...
		public function hitshippo_create_fedex_shipping($order_id){
			if ($this->hpos_enabled) {
		    	...
		    	if (  isset( $_POST[ 'hitshippo_fedex_reset' ] ) ) {
					$this->shipi_delete_meta($order_id, 'values');
		    	}
					...
					if($order){
						...
						$order_products = $this->get_products_on_order($general_settings, $order);
				       	if (empty($order_products)) {
							$this->shipi_update_meta($order_id, 'status', 'No product data found.');
							return;
				       	}
						...
						if (isset($_POST['shipi_fedex_add_cus_pack_'.$get_rates_for])) {
							if (!isset($_POST['shipi_fedex_pack_weight_'.$get_rates_for])) {
								$this->shipi_update_meta($order_id, 'status', 'Need packs for custom packing.');
			    				return;
							}
						...
						foreach ($custom_settings as $key => $cust_set) {
							...
							if (!empty($shipping_rates)) {
								...
								if($final_price){
									$this->shipi_update_meta($order_id, 'rates', json_encode($final_price));
								}else{
									$this->shipi_update_meta($order_id, 'status', 'No service data found.');
									return;
								}
							}else{
								$this->shipi_update_meta($order_id, 'status', 'No service data found.');
		    	...
		    	if (isset($_POST['hitshippo_fedex_create_label'])) {
			       ...
			       if($order){
					...
					if(empty($pack_products)){
						$this->shipi_update_meta($order_id, 'status', "Product data not found");
					...
					if(!empty($general_settings) && isset($general_settings['hitshippo_fedex_shippo_int_key'])){
						...
						if($output){
							if(isset($output['status'])){
								if(isset($output['status']) && $output['status'] != 'success'){
									$this->shipi_update_meta($order_id, 'status', $output['status']);

								}else if(isset($output['status']) && $output['status'] == 'success'){
									$output['user_id'] = $create_shipment_for;
									$result_arr = array();
									$data = $this->shipi_get_meta($order_id, 'values', []);
								...
								$this->shipi_update_meta($order_id, 'status', 'Site not Connected with Shipi. Contact Shipi Team.');
							}
						}else{
							$this->shipi_update_meta($order_id, 'status', 'Site not Connected with Shipi. Contact Shipi Team.');
						}
		...
		private function get_vendors_on_order($general_settings = [], $pack_products = [], $multi_ven = '', $order_data = [], $service_code = '')
		{
			...
			$custom_settings['default'] = array(
												...
												'hitshippo_fedex_rest_tracking_api_key' => isset($general_settings['hitshippo_fedex_rest_tracking_api_key']) ? $general_settings['hitshippo_fedex_rest_tracking_api_key'] : "",
												'hitshippo_fedex_rest_tracking_secret_key' => isset($general_settings['hitshippo_fedex_rest_tracking_secret_key']) ? $general_settings['hitshippo_fedex_rest_tracking_secret_key'] : "",
					...
					if($fedex_account != 'default'){
						...
						if(!empty($user_account)){
							if(!isset($vendor_settings[$fedex_account])){
								...
								if (isset($user_account['hitshippo_fedex_rest_tracking_api_key']) && isset($user_account['hitshippo_fedex_rest_tracking_secret_key']) && !empty($user_account['hitshippo_fedex_rest_tracking_api_key']) && !empty($user_account['hitshippo_fedex_rest_tracking_secret_key']) ) {
										$vendor_settings[$fedex_account]['hitshippo_fedex_rest_tracking_api_key'] = $user_account['hitshippo_fedex_rest_tracking_api_key'];
										$vendor_settings[$fedex_account]['hitshippo_fedex_rest_tracking_secret_key'] = $user_account['hitshippo_fedex_rest_tracking_secret_key'];
								}
