...
<form action="" method="post">
    <?php
        ...
        if(isset($_POST['insert_field'])){
            ...
            $field = $wpdb->get_results("SELECT * FROM $checkout_field ORDER BY position ASC");
            foreach($field as $field);
            $positon = $field->position + 1; 

            $sql = "INSERT INTO $checkout_field (`field_name`, `input_type`, `input_label`, `input_width`, `placeholder`, `input_required`, `input_remove`, `position`) VALUES ('$field_name', '$input_type', '$input_label', '$input_width', '$placeholder', '$required', '', '$positon')";
            $data = $wpdb->query($sql);
        ...
        if(isset($_POST['update_field'])){
           ...
           foreach($field_id as $key=>$id){
            $sql = "UPDATE $checkout_field SET input_type='$input_type[$id]', input_label='$input_label[$id]', input_width='$input_width[$id]', placeholder='$placeholder[$id]', input_required='$required[$id]', input_remove='$remove[$id]'  WHERE id='$id'";
        ...
        if(isset($_POST['delete_biling_field'])){
            ...
            $sql = "DELETE FROM $checkout_field WHERE id='$id'";
         ...
         */
        $show_field = $wpdb->get_results("SELECT * FROM $checkout_field ORDER BY position ASC");
    ...
    <div class="panel">
        <div class="panel-heading "><h4>Biiling Feild Settings</h4><span class="top-submit-btn"><button type="submit" class="billing-update-btn" name="update_field">Update</button></span></div>
            ...
            <table class="table table-reponsive">
                ...
                <tbody class="short">
                    <?php 
                        ...
                        $disable = $field->input_remove;
                        $disable == 'on' ? $disable = 'disabled' : $disable = '';
                    ?>
                    <tr data-index="<?php echo $field->id; ?>" data-position="<?php echo $field->position; ?>" class="ui-state-default sorted-field">
                        
                        <input type="hidden" id="field_id" name="field_id[]" value="<?php echo $id = esc_attr($field->id);?>">
                        <td><i class="fas fa-arrows-alt"></i> <label for=""><?php echo esc_attr(strtolower(str_replace(' ', '_', $field->field_name)));?></label></td>
                        <td <?php echo $disable; ?>> 
                            <select name="input_type[<?php echo $id; ?>]" style="width: 90px;">
                                <option value="<?php echo esc_attr($field->input_type);?>"><?php echo esc_attr($field->input_type);?></option>
                        ...
                        <td <?php echo $disable; ?>><input type="text" name="input_label[<?php echo $id; ?>]" value="<?php echo esc_attr($field->input_label);?>"></td>
                        <td <?php echo $disable; ?>>
                            <select name="input_width[<?php echo $id; ?>]" style="min-width:85%">
                                <option value="<?php echo esc_attr($field->input_width);?>"><?php echo esc_attr($field->input_width);?></option>
                        ...
                        <td <?php echo $disable; ?>><input type="text" name="placeholder[<?php echo $id; ?>]" value="<?php echo esc_attr($field->placeholder);?>"></td>
                        <td <?php echo $disable; ?>>
                            <?php $req = esc_attr($field->input_required);
                        ...
                        <td>
                            <?php $on = esc_attr($field->input_remove);
                        ...
                        <td>
                            ...
                            <?php else: ?>
                                <a href="#" data-id="<?php echo $field->id;?>" class="modal-open-delete-field billing">Remove</a> <span title="Permanently delete from database" class="question">?</span>
                            <?php endif; ?>
                        </td>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <form action="" method="post">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Add New Billing Field</h5>
...
<div class="modal fade bd-example-modal-sm" id="delete_biling_field_modal" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body delete-field-data-modal-billing">
            <!-- show data from ajax -->
        </div>
