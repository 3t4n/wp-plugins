...
<form action="" method="post">
    <?php
        ...
        if(isset($_POST['insert_field'])){
            ...
            $field_name         =   sanitize_text_field($_POST['iwm_field_name']);
            ...
            $field = $wpdb->get_results("SELECT * FROM $checkout_order_note_field ORDER BY o_position ASC");
            foreach($field as $field);
            $positon = $field->position + 1; 

            $sql = "INSERT INTO $checkout_order_note_field (`o_field_name`, `o_input_type`, `o_input_label`, `o_input_width`, `o_placeholder`, `o_input_required`, `o_input_remove`, `o_position`) VALUES ('$field_name', '$input_type', '$input_label', '$input_width', '$placeholder', '$required', '', '$positon')";
            $data = $wpdb->query($sql);
        ...
        if(isset($_POST['update_field'])){
           ...
           foreach($field_id as $key=>$id){
            $sql = "UPDATE $checkout_order_note_field SET o_input_type='$input_type[$id]', o_input_label='$input_label[$id]', o_input_width='$input_width[$id]', o_placeholder='$placeholder[$id]', o_input_required='$required[$id]', o_input_remove='$remove[$id]'  WHERE id='$id'";
        ...
        if(isset($_POST['delete_biling_field'])){
            ...
            $sql = "DELETE FROM $checkout_order_note_field WHERE id='$id'";
         ...
         */
        $show_field = $wpdb->get_results("SELECT * FROM $checkout_order_note_field ORDER BY o_position ASC");
    ...
    <div class="panel">
        <div class="panel-heading "><h4>Additional Feild Settings</h4><span class="top-submit-btn"><button type="submit" class="billing-update-btn" name="update_field">Update</button></span></div>
            ...
            <table class="table table-reponsive">
                ...
                <tbody class="short">
                    <?php 
                        ...
                        $disable = $field->o_input_remove;
                        $disable == 'on' ? $disable = 'disabled' : $disable = '';
                    ?>
                    <tr data-index="<?php echo $field->id; ?>" data-position="<?php echo $field->o_position; ?>" class="ui-state-default sorted-field">
                        
                        <input type="hidden" id="field_id" name="field_id[]" value="<?php echo $id = esc_attr($field->id);?>">
                        <td><i class="fas fa-arrows-alt"></i> <label for=""><?php echo esc_attr(strtolower(str_replace(' ', '_', $field->o_field_name)));?></label></td>
                        <td <?php echo $disable; ?>> 
                            <select name="input_type[<?php echo $id; ?>]" style="width: 90px;">
                                <option value="<?php echo esc_attr($field->o_input_type);?>"><?php echo esc_attr($field->o_input_type);?></option>
                        ...
                        <td <?php echo $disable; ?>><input type="text" name="input_label[<?php echo $id; ?>]" value="<?php echo esc_attr($field->o_input_label);?>"></td>
                        <td <?php echo $disable; ?>>
                            <select name="input_width[<?php echo $id; ?>]" style="min-width:85%">
                                <option value="<?php echo esc_attr($field->o_input_width);?>"><?php echo esc_attr($field->o_input_width);?></option>
                        ...
                        <td <?php echo $disable; ?>><input type="text" name="placeholder[<?php echo $id; ?>]" value="<?php echo esc_attr($field->o_placeholder);?>"></td>
                        <td <?php echo $disable; ?>>
                            <?php $req = esc_attr($field->o_input_required);
                        ...
                        <td>
                            <?php $on = esc_attr($field->o_input_remove);
                                $on =='on' ? ($check_remove = 'checked') : ($check_remove = '');
                            ?>
                            <input type="checkbox" <?php echo $check_remove; ?> name="remove[<?php echo $id; ?>]">
                        </td>
                        <td>
                            <a href="#" data-id="<?php echo $field->id;?>" class="modal-open-delete-field">Remove</a> <span title="Permanently delete from database " class="question">?</span>
                        </td>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <form action="" method="post">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Add New Extra Field</h5>
      ...
      <div class="modal-footer">
        <button type="submit" name="insert_field" class="add-row-btn">Add Field</button>
      </div>
      </form>
    </div>
  </div>
...
<div class="modal fade bd-example-modal-sm" id="delete_shipping_field_modal" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body delete-field-data-modal">
            <!-- show data from ajax -->
        </div>
