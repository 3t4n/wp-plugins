...
<script>
  ...
  var geoloc="<?php if(isset($geo_loc)) echo esc_js($geo_loc);?>";
  ...
  <?php 
  if(!empty($df_date)) {
    if(isset($_SERVER['HTTP_COOKIE'])) $sess=sanitize_text_field(substr($_SERVER['HTTP_COOKIE'],stripos($_SERVER['HTTP_COOKIE'],'wordpress_sec')+99,99)); else $sess=sanitize_text_field($_SERVER['REMOTE_ADDR']);
    $period_sync=get_transient("dyt_period_sync_{$wp_userid}_$sess");
    $period_set=get_transient("dyt_period_set");
    if($period_set>0 && (empty($period_sync) || $period_sync<$period_set)) {
      set_transient("dyt_period_sync_{$wp_userid}_$sess",$df_date,604800);
  ...
  var db_note=<?php echo '['.str_replace("\r\n","\\n",substr($note,0,-1)).']';?>;
