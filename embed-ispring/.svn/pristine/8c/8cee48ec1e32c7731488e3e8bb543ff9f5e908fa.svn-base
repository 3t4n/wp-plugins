...
?>
    ...
    .ispring_form div{padding-top:10px;}
    .notice-error,.notice-updated {margin-left:0px !important;}
    </style>
      <div>
		...
		?>
           <form class="ispring_form" method="post" enctype="multipart/form-data">
                <div>
                    <span><input type="text" id="file_title_name" required="" name="file_name" value="" placeholder="Title" /></span>
                </div>
                <div>
...
function tbc_ispring_display_uploaded_files_list() {
    ...
    if (empty($data)) {
		...
		?>
			<tr><td><?php echo $i; ?></td><td><?php echo esc_html( $file->file_name ); ?></td><td>[ppt_embedder id="<?php echo $file->id; ?>"]</td><td><a href=" <?php menu_page_url('ispring-embedder'); ?>&delete_ppt=<?php echo esc_html( $file->id ); ?>" Onclick="<?php echo esc_js( $onClickJs ); ?>" >Delete</a></td></tr>
		<?php 
			$i++;
			endforeach; 
        ?>
