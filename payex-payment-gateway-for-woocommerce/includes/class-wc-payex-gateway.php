...
class WC_Payex_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_description = 'Accept multiple payments and subscriptions using Payex Payment Gateway (https://www.payex.io/)'; // will be displayed on the options page.
		...
		$this->testmode          = $this->check_testmode();
		...
		if ( ! has_action( 'woocommerce_query_payex_payment_status' ) ) {
			add_action( 'woocommerce_query_payex_payment_status', array( $this, 'query_payex_payment_status' ), 10, 2 );
		}

		add_filter( 'woocommerce_cart_needs_payment', array( $this, 'filter_cart_needs_payment_callback' ), 100, 2 );
	}
	 ...
	 * @return bool
	 */
	public function filter_cart_needs_payment_callback( $needs_payment, $cart ) {
		return $cart->subtotal > 0 ? $needs_payment : false;
	}

	/**
	 * Filter to force payment for all orders, even when total is zero.
	 *
	 * @param bool   $needs_payment      Whether the order needs payment.
	 * @param object $order              The order object.
	 * @param array  $valid_order_statuses The valid order statuses.
	 */
	public function filter_woocommerce_order_needs_payment( $needs_payment, $order, $valid_order_statuses ) {
		$needs_payment = true;
	...
	public function process_payment( $order_id ) {
		...
		if ( $token ) {
			...
			wc_add_notice( 'Invalid payment settings, please inform website owner.', 'error' );
			return;
		}
		// get token.
	}
	...
	public function webhook() {
		...
		if ( $verified && isset( $_POST['reference_number'] ) &&
			...
			} elseif ( isset( $_POST['approval_status'] ) ) {
	...
	private function get_payex_payment_link( $url, $order, $callback_url, $token = null ) {
		...
		$items                 = array();
		$metadata              = array();
		$metadata['order_key'] = $order_data['order_key'];

		foreach ( $order_items as $item_id => $item ) {
			// order item data as an array
			$item_data = $item->get_data();
			array_push( $items, $item_data );
			$description .= $item_data['name'] . ' (' . $item_data['quantity'] . '), ';
		}

		$description = substr( rtrim( $description, ', ' ), 0, 200 );
	...
	private function get_payex_mandate_link( $url, $order, $callback_url, $token = null ) {
		...
		foreach ( $subscriptions as $subscription ) {
			...
			if ( date( 'Y-m-d', strtotime( $next ) ) < $cutoff ) {
				$autoDebit = true;
			}
		...
		if ( $initial_payment > 0 || $autoDebit ) {
			$debit_type = 'AD';
		}
	 ...
	 * @param mixed $renewal_order  A WC_Order object created to record the renewal payment.
	 */
	 ...
	 * @param mixed $renewal_order WC_Order A WC_Order object created to record the renewal payment.
	 */
	...
	/**
	 * Delete meta data for resubscribe order
	 *
	...
	/**
	 * Delete meta data for renewal order
	 *
	 * @param int $renewal_order    The order created for the customer to resubscribe to the old expired/cancelled subscription.
	 */
		...
		if ( ! $order->is_paid() ) {
			...
			if ( $token ) {
				$request = wp_remote_get(
					$url . self::API_QUERY . '?status=sales&reference_number=' . $order_id,
					array(
						'method'  => 'GET',
						'timeout' => 45,
						'headers' => array(
	 ...
	 *
	 * @param string $mandate_number Mandate number.
	 * @param string $order_id Customer order id.
	 * @return bool
	 */
	public function check_renewal_order_exist( $mandate_number, $order_id ) {
		...
		if ( $token ) {
			$request = wp_remote_get(
				$url . self::API_COLLECTIONS . '?reference_number=' . $mandate_number . '&collection_reference_number=' . $order_id,
				array(
					'method'  => 'GET',
					'timeout' => 45,
					'headers' => array(
	 ...
	 *
	 * @param  string $order           Customer order.
	 * @param  string $txn_id          Payex transaction id.
	 * @param  string $mandate_number  Payex mandate number.
	 * @param  string $txn_type        Payex transaction type.
	...
	private function complete_payment( $order, $txn_id, $mandate_number, $txn_type, $response_code ) {
		...
		if ( PAYEX_AUTH_CODE_SUCCESS == $response_code ) {
			...
			} elseif ( $txn_type == AUTO_DEBIT_AUTHORIZATION ) {
			...
			if ( $mandate_number ) {
				update_post_meta( $order->get_id(), 'payex_mandate_number', $mandate_number );
			}
			...
			} elseif ( $txn_type == DIRECT_DEBIT_APPROVAL ) {
				$order->add_order_note( 'Mandate (' . $mandate_number . ') approved by bank', false );
			} elseif ( $txn_type == DIRECT_DEBIT ) {
				$order->add_order_note( 'Direct Debit collection approved by bank', false );
			}
		} else {
			$order->add_order_note( 'Payment failed via Payex (Txn ID: ' . $txn_id . ') with Response Code: ' . $response_code, false );
		}
	}

	private function check_testmode() {
