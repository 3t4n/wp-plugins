...
class WB_MagicPost_Share extends WB_MagicPost_Base
{
	...
	public function dwqr_ajax_handler()
	{
		...
		if (has_excerpt($post_id)) {
			$excerpt = rtrim(trim(wp_strip_all_tags(get_the_excerpt($post_id))), '[原文链接]');
		} else {
	...
	function get_share_html()
	{
		...
		foreach ($share_items as $k => $v) {
			$wb_dwqr_share_html .= '<a class="share-logo-magicpost icon-' . $k . '" data-cmd="' . $k . '" title="分享到' . esc_attr($v['name']) . '" rel="nofollow"><svg class="wb-icon wbsico-magicpost-' . $k . '"><use xlink:href="#wbsico-magicpost-' . $k . '"></use></svg></a>';
		}
