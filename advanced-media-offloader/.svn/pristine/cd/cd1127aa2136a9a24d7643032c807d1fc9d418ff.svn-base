...
- Mirror deletion: automatically removes files from cloud storage when deleted from WordPress (when enabled).
...
- For [Wasabi](https://docs.wasabi.com/docs/creating-a-new-access-key), add the following to wp-config.php:
`
...
Currently, the plugin supports only Cloudflare R2, DigitalOcean Spaces, Amazon S3, Wasabi & MinIO, but we are working on adding support for other cloud storage platforms.
