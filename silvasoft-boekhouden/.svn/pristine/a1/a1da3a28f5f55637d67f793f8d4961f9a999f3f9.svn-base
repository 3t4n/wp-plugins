...
if (!class_exists("Silvasoft_Settings")) :
    ...
    class Silvasoft_Settings
    {
        ...
        function silvasoft_admin_settings_init()
        {
			...
			add_settings_section(
                'silva_pluginPage_section2a',
                __('Voorraad sync (2-way sync)', 'wordpress'),
                array($this,'silva_settings_callback'),
                'silva_pluginPage'
            );
		  ...
		  add_settings_field(
                ...
                __('Gebruik veld "address_2" als huisnummer?', 'wordpress'),
			...
			add_settings_field(
                'silvasoft_stocksync',
                __('Sync voorraad vanuit Silvasoft naar Woocommerce?', 'wordpress'),
                array($this,'silvasoft_render_select_yesno'),
                'silva_pluginPage',
                'silva_pluginPage_section2a',
                array(
                    'name' => 'silvasoft_stocksync',
					'hint' => 'Indien ingeschakelt zal de voorraad van Silvasoft doorgevoerd worden in uw WooCommerce omgeving. De plugin gebruikt hiervoor het CRON mechanisme en zal eens per X minuten voorraadwijzigingen opvragen aan Silvasoft. Deze wijzigingen worden doorgevoerd in uw Woocommerce producten.',
                )
            );
			
			add_settings_field(
                'silvasoft_stocksyncinterval',
                __('Hoe vaak syncen?', 'wordpress'),
                array($this,'silvasoft_render_select_syncinterval'),
                'silva_pluginPage',
                'silva_pluginPage_section2a',
                array(
                    'name' => 'silvasoft_stocksyncinterval',
					'hint' => 'Kies hoe vaak de voorraad bijgewerkt moet worden. Let op: gebruikt CRON. Gebruikt 1 API request per uitvoering.',
                )
            );
			
			add_settings_field(
                'silvasoft_stocktypeuse',
                __('Welke voorraad gebruiken?', 'wordpress'),
                array($this,'silvasoft_render_select_stock'),
                'silva_pluginPage',
                'silva_pluginPage_section2a',
                array(
                    'name' => 'silvasoft_stocktypeuse',
					'hint' => 'Kies welke voorraad vanuit Silvasoft aangehouden moet worden. De algemene productvoorraad of de verkoopbare voorraad.',
                )
            );
		   ...
		   if ( get_option('silva_settings') === false) {
				//set default values for options
				 update_option( 'silva_settings', array('silvasoft_status_credit' => 'wc-refunded',
														...
														'silvasoft_stocksync'=>'no',
														'silvasoft_stocktypeuse'=>'stockgeneral',
														'silvasoft_stocksyncinterval' => 5
														) );  
		   }
        }
		...
		{
			/* some styles */
			$htmlBefore =  '<style type="text/css"> .form-table input, .form-table select { min-width: 45%;} .form-table th { min-width: 275px;} #wpcontent form h2 {    border-bottom: 1px solid #ccc;    padding: 10px;    background: #fff;    margin-right: 15px;} div.notice { margin-left: 0px !important;} </style>';
			echo $htmlBefore;
		...
		/* Render the select for stock option */
		function silvasoft_render_select_stock($args)
        {
            $options = get_option('silva_settings');
			
		    ?>
            <select name='silva_settings[<?php echo $args['name']?>]'>                
               <option value='stockgeneral'
                <?php if(isset($options[$args['name']])) {
                     selected($options[$args['name']], 'stockgeneral');
                        }; ?>
                    >
                   Algemene product voorraad
                    </option>
                    
               <option value='stockavailable'
                <?php if(isset($options[$args['name']])) {
                     selected($options[$args['name']], 'stockavailable');
                        }; ?>
                    >
                    Verkoopbare product voorraad
                    </option>
                    
               <option value='stockavailableminuspurchase'
                <?php if(isset($options[$args['name']])) {
                     selected($options[$args['name']], 'stockavailableminuspurchase');
                        }; ?>
                    >
                    Verkoopbare product voorraad (excl. inkooporders)
                    </option>
		...
		/* Render the select for stock sync interval */
		function silvasoft_render_select_syncinterval($args)
        {
            $options = get_option('silva_settings');
        ...
        function silva_options_page()
        {
            ...
            <form action='options.php' method='post'>
                ...
                <h1>Silvasoft - WooCommerce koppeling</h1>
