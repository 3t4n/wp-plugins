...
class Silvasoft_Admin {
		...
		/* construct admin pages */  
	   	if (is_admin()) {
			...
			if (!class_exists("Silvasoft_Status"))
				require(SILVAPLUGINDIR . 'admin/class-silvasoft-status.php');
			$this->status = new Silvasoft_Status();
	...
	public function admin_menu() {
		...
		//Silvasoft status page
       	add_submenu_page('silvasoft', 'Status', 'Status', 'manage_options', __FILE__ . '/status', array($this->status,'page'));
	 ...
	 public	function silva_log_page() {
		  ...
		  if(isset($_REQUEST['grid_action']) && $_REQUEST['grid_action'] != '') {
			...
			if($_REQUEST['grid_action'] === 'syncStock') {
				  $this->apiconnector->silvasoft_woo_cron_stock();
			}
			  
		}
		
		echo '<div class="wrap"><h2>Silvasoft - Woocommerce koppeling - LOG</h2>'; 
		//get all the items for the table
		$logclass->prepare_items(); 
	
		//display the table, informatin text and search box
		echo '<div class="notice notice-info"><p>';
		echo 'Hieronder ziet u een log van alle orders die naar uw Silvasoft boekhouding verstuurd zijn. Ook ziet u in dit overzicht een log van eventueel opgetreden fouten. Onderaan deze pagina vind u nog enkele <a href="#tools">tools</a>';
		echo '</div>';
	?>	
		...
		$actions['resend'] = array(
		 ...
		 $actions['syncstock'] = array(
				'url'       => wp_nonce_url(add_query_arg( 
									array(
										'grid_action'=> 'syncStock'									
									)
								)),
				'name'      => __( 'Synchroniseer voorraad in WooCommerce met Silvasoft', 'woocommerce' ),
				'action'    => "link",
			);	
		 ...
		 echo '<h2 id="tools">Tools</h2>';
		foreach ( $actions as $key => $action ) {
			$txt = "";
			if($key == "resend") {
				$txt = "Wilt u alle orders resetten qua status voor of de order is verstuurd naar Silvasoft of niet? Dit kan bijvoorbeeld gebruikt worden indien u orders opnieuw wilt versturen naar Silvasoft, naar een andere administratie of omdat u ze heeft verwijderd in Silvasoft en opnieuw wilt sturen.";
			} else if($key == "syncstock") {
				$txt = "Synchroniseer uw voorraad vanuit Silvasoft met WooCommerce. Let op: haalt enkel artikelen op die in de afgelopen 5 minuten gewijzigd zijn qua voorraad. Via de instellingen kunt u inschakelen om deze sync automatisch in de achtergrond plaatst te laten vinden.";
			}
			
			$var = $txt . sprintf(
				'<a class="tips %1$s" href="%2$s" data-tip="%3$s">%4$s</a>',
			...
			echo '<div style="clear:both;display:block"></div><br/><div style="clear:both;border: 1px solid #ccc; background: #fff;padding:5px;margin-top: 5px;max-width: 95%;display:block;" class="notice-info"><p>'.$var.'</p></div>';
		}
		 echo '<br/><br/>';
