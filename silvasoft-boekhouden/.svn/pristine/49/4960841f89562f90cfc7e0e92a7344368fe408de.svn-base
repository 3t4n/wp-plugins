...
class Silvasoft_ApiConnector {
	...
	protected $silvasoft_stocktypeuse;
	protected $silvasoft_stocksyncinterval;
	private $status_sale;
	...
	public function __construct($plugin_name, $version, $silvalogger) {
		...
		$this->silvasoft_stocksync = $this->settingHasValue($silvasoftSettings,'silvasoft_stocksync') ? ($silvasoftSettings['silvasoft_stocksync'] == 'yes' ? true : false) : false;
		
		$this->silvasoft_stocktypeuse = $this->settingHasValue($silvasoftSettings,'silvasoft_stocktypeuse') ? $silvasoftSettings['silvasoft_stocktypeuse'] : null;
		
		$this->silvasoft_stocksyncinterval = $this->settingHasValue($silvasoftSettings,'silvasoft_stocksyncinterval') ? intval($silvasoftSettings['silvasoft_stocksyncinterval']) : 5;
	...
	* CRON stock sync
	*/
	public function silvasoft_woo_cron_stock($force=false) {
		if($this->silvasoft_debugmode) {
			$this->silvalogger->doLog('DEBUG','Start CRON Silvasoft <> WooCommerce stock sync');
		}
		
		if(!$this->silvasoft_stocksync && !$force) {
			if($this->silvasoft_debugmode) {
				$this->silvalogger->doLog('DEBUG','End CRON Silvasoft <> WooCommerce stock sync is disabled');
			}
			return true; 
		}
		
		//step 0: get last changedSince value from WP db
		$optionName = "silvachangedsince";
		$unixNow = time();
		$unixnowMinusFive = $unixNow-300; //now minus 5 minutes
		$unixUse = get_option($optionName,$unixnowMinusFive);
		
		//$unixUse = lastRun, defaulting to 5 mins ago
		$syncIntervalMinutes = $this->silvasoft_stocksyncinterval;
		
		if($syncIntervalMinutes>1) {
			//if interval = 1 minute always run the script because cron is 1 minute interval by default
			if(!($unixUse <= ($unixNow-($syncIntervalMinutes*55)))) {
				//use 55 seconds to make sure it runs in time.
				//the silva stock cron runs every 1 minute maximum
				//if someone has set an interval of 3 minutes, every 1 minute the interval is checked
				//by using 55 seconds we make sure after it has run, it can run again 3*55 seconds later, which is actuall 2,75minutes later
				//this way, by the time the third cron runs we are sure it is allowed to run, otherwise sometimes a 3 minute interval could result in actually running only every 4 minutes for example
				//This works up to 10 minutes interval, making the real interval 10*55 = 9,16minutes so it will run on the 10th cron. 
				//10 minutes is the highest interval availeble to a user. 
				if($this->silvasoft_debugmode) {
					$this->silvalogger->doLog('DEBUG','End CRON Silvasoft <> Skip due to sync interval');
				}
				return true;
			}
		}
		
		//echo '<br/>UNIX now ['.$unixNow.']';
		//echo '<br/>UNIX now minus 5 ['.$unixnowMinusFive.']';
		//echo '<br/>UNIX fetch from ['.$unixUse.']';
		
		//step 1: fetch changed products from Silvasoft
		$productRetrieveData['ChangedSince'] = $unixUse;
		$result = '';
		try {
		...
		if($validated['ok'] === true) {
			//api connection was succesful, update last sync time to keep only fetching new items
			update_option($optionName, $unixNow);	
			
			$prodsToUpdateJSON = $result["response"];
			$prodsToUpdateArr = json_decode($prodsToUpdateJSON,true);
			
			$articles = array();
			foreach($prodsToUpdateArr as $prodToUpdate) {
				$sku = $prodToUpdate["ArticleNumber"];
				$newStock = $prodToUpdate["StockQty"];
				if($this->silvasoft_stocktypeuse != null && $this->silvasoft_stocktypeuse == 'stockavailable') {
					$newStock = $prodToUpdate["QtySalable"];
				} 
				if($this->silvasoft_stocktypeuse != null && $this->silvasoft_stocktypeuse == 'stockavailableminuspurchase') {
					$correctionPurchaseStock = isset($prodToUpdate["QtyInPurchaseOrder"]) ? $prodToUpdate["QtyInPurchaseOrder"] : 0;
					 try {
						$newStock = floatval($prodToUpdate["QtySalable"]) - floatval($correctionPurchaseStock);
					 } catch (Exception $e) {
						$newStock = $prodToUpdate["QtySalable"];
					 }
				} 
				...
				$wcprodid = wc_get_product_id_by_sku( $sku );
				if($wcprodid == null) continue;
				
				$product = wc_get_product( $wcprodid ); // object of WC_Product class
				...
				array_push($articles,$sku);
			}
			
			$articlesString = implode(", ",$articles);
			$this->silvalogger->doLog('Voorraad sync','Voorraad bijgewerkt voor de artikelen: ' . $articlesString);
			...
			if($this->silvasoft_debugmode) {
				$this->silvalogger->doLog('DEBUG','End CRON Silvasoft <> WooCommerce stock sync - empty or invalid response');
			}
			return true;
		}
		
		//step 2: update those products stock in WooCommerce
		
		
		if($this->silvasoft_debugmode) {
			$this->silvalogger->doLog('DEBUG','End CRON Silvasoft <> WooCommerce stock sync');
		}
	...
	*/
	...
	public function transferOrderToSilvasoft($order_id,$creditnota = false,$partialrefund = false) {
		...
		if($customerPostResult['status'] === 'nok') {
			...
			$customerGuid = isset($customerPostResult['customerGuid']) ? $customerPostResult['customerGuid'] : 0;
		}
		...
		$data['RelationGuid'] = $customerGuid;
		
		$data['OrderLineRaiseLimit'] = 'y87Xk^)hUaQn5))i';
	...
	*/
	function getTaxInfoByLine($orderLineData,$zeroTotal,$orderLineType) {
	...
	function collectCustomer($order) {
		...
		if($validated['ok'] === true) {			
			...
			$customerGuid = $resultPHP[0]->RelationGUID;
			return array("status"=>"ok","customer"=>$customername,"email"=>$billingmail,"customerGuid"=>$customerGuid);
		} else{
			return array("status"=>"nok","msg"=>$msg);
		}
	}
