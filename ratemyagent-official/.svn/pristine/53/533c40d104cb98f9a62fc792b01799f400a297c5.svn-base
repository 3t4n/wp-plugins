...
class SetupWizardPage
{
	...
	private const NONCE_KEY = 'rma-setup-wizard';
	...
	private static function getActions(array $actions): array
	{
	...
	{
		$fields       		= (object) self::INPUTS;
		$available_actions 	= [$fields->set_region_action, $fields->set_client_action, $fields->set_profile_action];
		$actions 			=  self::getActions($available_actions);
		...
		if (in_array($fields->set_region_action, $actions)) {
			self::update_region(self::get_post_value('region'));
			self::$step = 2;
		}

		if (in_array($fields->set_client_action, $actions)) {
			try {
		...
		if (in_array($fields->set_profile_action, $actions)) {
