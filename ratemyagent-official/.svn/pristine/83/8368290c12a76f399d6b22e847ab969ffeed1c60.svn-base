...
class Settings
{
	...
	private static string  $NONCE_KEY = 'rma-base-settings';
	...
	private static function handle_actions(): ?string
	{
		$notification 		= null;
		$fields       		= (object) self::INPUTS;
		$available_actions 	= [$fields->update_action, $fields->test_action, $fields->reset_action];
		...
		if (in_array($fields->update_action, $actions)) {
		...
		if (in_array($fields->test_action, $actions)) {
			$notification = self::test_credentials();
		}

		if (in_array($fields->reset_action, $actions)) {
