=== RealHomes PayPal Payments ===
Contributors: inspirythemes, saqibsarwar, fahidjavid
Tags: PayPal, PayPal payments, Real estate payments, Property payments, Real estate PayPal
Requires at least: 6.0
Tested up to: 6.7.1
Stable tag: 2.0.4
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

This plugin allows RealHomes theme website admin to add PayPal payments functionality for individual properties submitted by website users.

== Description ==

This plugin works only with [RealHomes WordPress real estate theme.](https://realhomes.io/)

This plugin allows RealHomes theme website admin to add PayPal payments functionality for individual properties submitted by website users.

Plugin includes translation files for Spanish, Italian, French, German, Turkish and Portuguese languages.

== Installation ==

### Method 1: WordPress Admin Interface

1. Navigate to your WordPress dashboard and go to **Plugins > Add New**.
2. In the search bar, enter **RealHomes PayPal Payments** and press Enter.
3. Look for the plugin in the search results and click on the **Install Now** button.
4. Once installed, click on the **Activate** button to activate the plugin.

### Method 2: FTP Upload

1. Download the plugin ZIP file and extract it.
2. Connect to your server using an FTP client (e.g., FileZilla, Cyberduck).
3. Upload the extracted plugin folder to the `/wp-content/plugins/` directory on your server.
4. Activate the plugin through the WordPress dashboard in **Plugins > Installed Plugins**.

== Frequently Asked Questions ==

= Can this plugin work standalone? =

No, this plugin works only with [RealHomes WordPress real estate theme.](https://realhomes.io/)

== Screenshots ==

1. RealHomes PayPal Payments Settings

== Changelog ==

= 2.0.4 =
* Added PayPal Keys missing notice
* Improved plugin settings utilization
* Fixed text-domain notice
* Updated language file
* Tested plugin with WordPress 6.7.1

= 2.0.3 =
* Improved entry file information
* Improved readme.txt and readme.md file
* Updated language file
* Tested plugin with WordPress 6.6.0

= 2.0.2 =
* Added plugin action links
* Improved version management code
* Improved settings layout and design
* Improved settings validation
* Improved entry file information
* Improved readme.txt and readme.md file
* Updated language file
* Tested plugin with WordPress 6.5.2

= 2.0.1 =
* Improved plugin settings initialization
* Added plugin settings action messages display
* Tested plugin with WordPress 6.4.1

= 2.0.0 =
* Updated PayPal API to the latest JS SDK
* Revamped the whole plugin & payment system
* Updated language file
* Tested plugin with WordPress 6.4

= 1.0.4 =
* Updated language file
* Tested plugin with WordPress 6.2

= 1.0.3 =
* Tested plugin with WordPress 6.0.1

= 1.0.2 =
* Fixed undefined warnings issue for some setting fields with initial values.
* Tested plugin with WordPress 5.6.2

= 1.0.1 =
* Tested plugin with WordPress 5.5

= 1.0.0 =
* Initial Release