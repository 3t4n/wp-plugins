 ...
 * @license   GPL-3.0+
 * @link      https://wordpress.org/plugins/content-table/
 * @copyright 2020-23 KaizenCoders
 *
 * @wordpress-plugin
 *
 * Plugin Name:       Content Table
 * Plugin URI:        https://wordpress.org/plugins/shorten-url/
 * Description:       Insert a *table of content* in your posts.
 * Version:           1.5.3.2
 * Author:            KaizenCoders
...
if ( ! defined( 'KC_CT_MIN_PHP_VER' ) ) {
	define( 'KC_CT_MIN_PHP_VER', '5.6' );
}

if ( ! defined( 'KC_CT_MAX_PHP_VER' ) ) {
	define( 'KC_CT_MAX_PHP_VER', '7.4.33' );
}

if ( ! function_exists( 'kc_ct_fail_php_version_notice' ) ) {

	function kc_ct_fail_php_version_notice() {
		/* translators: %s: PHP version */
		$message      = sprintf( esc_html__( 'Table of Content plugin requires PHP version between %s & %s, plugin is currently NOT RUNNING.', 'shorten-url' ), KC_CT_MIN_PHP_VER,  KC_CT_MAX_PHP_VER);
		$html_message = sprintf( '<div class="error">%s</div>', wpautop( $message ) );
		echo wp_kses_post( $html_message );
	}
}

if ( ! version_compare( PHP_VERSION, KC_CT_MIN_PHP_VER, '>=' ) || ! version_compare( PHP_VERSION, KC_CT_MAX_PHP_VER, '<=' )) {
	add_action( 'admin_notices', 'kc_ct_fail_php_version_notice' );
