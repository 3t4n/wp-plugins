...
if (!class_exists("adminCell")) {
	class adminCell  {
		 ...
		 */
		function flush() {
			$action = wp_create_nonce( 'shorten_url_ajax_security' );
				...
				if (! empty($this->action)) {
					?>									<div class="row-actions">
						...
						foreach ($this->action as $l) {
							...
							if (strpos($l[1],"(")>0) {
								$l[1] = str_replace('"', '\'', $l[1]) ;
								?>										<span><a href="#" data-su-security="<?php echo $action; ?>" onclick="javascript: jQuery('#wait_<?php echo SLFramework_Utils::create_identifier($l[1]) ;?>_<?php echo $this->idLigne ;?>').show() ; jQuery('body').bind('DOMSubtreeModified',function() {jQuery('#wait_<?php echo SLFramework_Utils::create_identifier($l[1]) ;?>_<?php echo $this->idLigne ;?>').hide() ; })  ; <?php echo $l[1] ;?> ; return false ; " id="<?php echo SLFramework_Utils::create_identifier($l[1]) ;?>_<?php echo $this->idLigne ;?>"><?php echo $l[0] ;?></a><img id='wait_<?php echo SLFramework_Utils::create_identifier($l[1]) ;?>_<?php echo $this->idLigne ;?>' src='<?php echo plugin_dir_url("/").'/'.str_replace(basename(__FILE__),"",plugin_basename(__FILE__)); ?>img/ajax-loader.gif' style='display:none;'><?php if ($num!=count($this->action)) { echo " |" ; }?></span>
								<?php
							} else {

								?>										<span><a href="#" data-su-security="<?php echo $action; ?>" onclick="javascript: jQuery('#wait_<?php echo SLFramework_Utils::create_identifier($l[1]) ;?>_<?php echo $this->idLigne ;?>').show() ; jQuery('body').bind('DOMSubtreeModified',function() {jQuery('#wait_<?php echo SLFramework_Utils::create_identifier($l[1]) ;?>_<?php echo $this->idLigne ;?>').hide() ; }) ; <?php echo $l[1] ;?>(<?php echo $this->idLigne ; ?>) ; return false ; " id="<?php echo $l[1] ;?>_<?php echo $this->idLigne ;?>"><?php echo $l[0] ;?></a><img id='wait_<?php echo SLFramework_Utils::create_identifier($l[1]) ;?>_<?php echo $this->idLigne ;?>' src='<?php echo plugin_dir_url("/").'/'.str_replace(basename(__FILE__),"",plugin_basename(__FILE__)); ?>img/ajax-loader.gif' style='display:none;'><?php if ($num!=count($this->action)) { echo " |" ; }?></span>
							<?php				}
						}
						?>									</div>
					<?php
