<?php
/*
 Plugin Name: Eazy Under Construction
 ...
 Author URI: http://robjscott.com/wordpress/eazy-under-construction
 License: GPLv2 or later
 */

/*
 This file is part of Eazy Under Construction.
 Eazy Under Construction is free software: you can redistribute it and/or modify
 it under the terms of the GNU General Public License as published by
 the Free Software Foundation, either version 3 of the License, or
 (at your option) any later version.
 Eazy Under Construction is distributed in the hope that it will be useful,
 but WITHOUT ANY WARRANTY; without even the implied warranty of
 MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 GNU General Public License for more details.
 You should have received a copy of the GNU General Public License
 along with Eazy Under Construction.  If not, see <http://www.gnu.org/licenses/>.
 */
...
class underConstruction
{
	...
	{
		/* Register our script. */
		wp_register_script('underConstructionJS', WP_PLUGIN_URL.'/'.$this->installedFolder.'/underconstruction.min.js', array(), '1.0',true);
	}
		...
		if ($this->pluginIsActive())
		{
			...
			if(isset($_SERVER['REMOTE_ADDR'])) {	
				if(!in_array($_SERVER['REMOTE_ADDR'], $array)){
					...
					{
						echo esc_html($this->getCustomHTML(), ENT_QUOTES);
						die();
					}
...
function uc_get_ip_address(){
	if(isset($_SERVER['REMOTE_ADDR'])) {
		echo esc_url_raw(wp_unslash($_SERVER['REMOTE_ADDR']));
		die();
	}
}
...
function underConstructionPluginLinks($links, $file)
{
	...
	{
		//add settings page
		$manage_link = '<a href="'.admin_url('options-general.php?page='.$underConstructionPlugin->getMainOptionsPage()).'">'.__('Settings', 'eazy-under-construction').'</a>';
