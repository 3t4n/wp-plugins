...
define('IPBL_ANALYTICS_WWW_URL', 'https://wp-inspired.com/product/ip-based-login-analytics/');
...
global $wpdb;
	...
	if(!empty($_REQUEST['no_login'])){
		...
		setcookie('ipbl_'.str_replace(array('.', ' '), array('_', '_'), $current_user->user_login), 'no_login', time()+$expire_cookie, '/');
		wp_logout();
...
function ipbl_export(){
	...
	if(isset($_REQUEST['ipbl_export']) && $_REQUEST['ipbl_export'] == 'csv'){
		...
		if(empty($_REQUEST['ipbl_sample'])){
			$ipranges = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."ip_based_login;", 'ARRAY_A');
		}else{
			$ipranges[] = array('username' => 'admin', 'start' => '1.1.1.1', 'end' => '1.1.1.255', 'status' => 1, 'redirect_to' => 'hello-world', 'usage' => '', 'used' => '', 'src' => '', 'date' => time());
		}
		...
		if(ipbl_is_premium()){
			$allowed_fields += array('redirect_to' => 'Redirect to', 'usage' => 'Allowed Usage', 'used' => 'Used');
		}
...
function ip_based_login_admin_menu() {
	...
	if (version_compare($wp_version, '3.0', '>=')) {
		...
		if(current_user_can('activate_plugins')){
			...
			if(!defined('ipbl_analytics_version')){
				add_submenu_page('ip-based-login', __('IP Based Login Analytics', 'ip-based-login'), __('Analytics', 'ip-based-login'), 'activate_plugins', IPBL_ANALYTICS_WWW_URL);
			}
...
function ip_based_login_option_page(){
	...
	if(isset($_POST['delid'])){
		...
		// Delete selected IP ranges
			...
			$delid = (int) ipbl_sanitize_variables($_POST['delid']);
	...
	if(isset($_POST['statusid'])){
		...
		$statusid = (int) ipbl_sanitize_variables($_POST['statusid']);
		$setstatus = ipbl_sanitize_variables($_POST['setstatus']);
	...
	if(isset($_POST['ipbl_import'])){
		...
		if(empty($error)){
			...
			$ipbl_csv_delimiter = ipbl_detect_csv_delimiter($_FILES['ipbl_import_file']['tmp_name']);
			
			if(empty($ipbl_csv_delimiter)){
				$ipbl_csv_delimiter = ',';
			}
			
			if (($handle = fopen($_FILES['ipbl_import_file']['tmp_name'], "r")) !== false) {
				// Loop through each line
				while (($line = fgets($handle)) !== false) {
					// Parse the line using the detected delimiter
					$csv_as_array[] = str_getcsv($line, $ipbl_csv_delimiter);
				}
				fclose($handle);
			}
		...
		if(!empty($error)){
			...
			foreach($csv_as_array as $ik => $iv){
				...
				if(!empty($iv[6])){
					if(!ipbl_is_supported_feature('Usage')){
						$skipped_csv[] = '* '.__('Usage feature is not supported in the Free version ', 'ip-based-login').' - <b>'.$iv[6].'</b> - Row no. '.$row_no;
						continue;
					}
				}
				
				$options = array();
				$options['username'] = sanitize_text_field($iv[0]);
				$options['start'] = sanitize_text_field($iv[1]);
				$options['end'] = sanitize_text_field($iv[2]);
				$options['status'] = (!empty($iv[3]) ? 1 : 0);
				$options['redirect_to'] = sanitize_text_field(!empty($iv[4]) ? $iv[4] : '');
				$options['usage'] = (int) (!empty($iv[5]) ? $iv[5] : 0);
				$options['used'] = (int) (!empty($iv[6]) ? $iv[6] : 0);
				$options['date'] = date('Ymd');
	...
	$ipbl_options_nonce = wp_create_nonce('ip-based-login-options');
	...
	const ipbl_options_nonce = "<?php echo esc_js($ipbl_options_nonce); ?>"; 
	
	function delete_confirm(msg, delid){
		...
		form.method = 'POST';
		form.action = 'admin.php?page=ip-based-login';

		const data = {
			delid: delid,
			_wpnonce: ipbl_options_nonce,
		};
	...
	function change_status_confirm(msg, statusid, setstatus){
	  ...
	  <form action="admin.php?page=ip-based-login" method="post">
		...
		&nbsp;
		<a href="admin.php?page=ip-based-login&ipbl_export=csv&ipbl_sample=1" rel="nofollow" style="text-decoration:none;">
			<?php echo __('Sample CSV','ip-based-login'); ?>
		</a>
		</div>
		</div>
	...
	if(!empty($ipranges)){
		...
		<div style="float:right;padding-right:10px;margin-bottom:10px;">
			<a href="admin.php?page=ip-based-login" rel="nofollow" style="text-decoration:none;" onclick="delete_confirm('<?php echo __('Are you sure you want to delete all IP ranges ?','ip-based-login'); ?>', 'all'); return false;">
			...
			<?php
				...
				foreach($ipranges as $ik => $iv){
					...
					<tr>
						...
						<td>
							'.(!empty($iv['redirect_to']) ? '<a href="'.(preg_match('/^http/is', $iv['redirect_to']) ? esc_url($iv['redirect_to']) : esc_url(site_url().'/'.$iv['redirect_to'])).'" target="_blank" style="text-decoration:none;">'.esc_html($iv['redirect_to']).'</a>' : '<i>'.(!ipbl_is_premium() ? '<a href="'.IPBL_PRICING_URL.'" target="_blank" style="text-decoration:none;"><span style="color:red;">'.__('Upgrade to Pro!','ip-based-login').'</span></a>' : __('No Redirect','ip-based-login')).'</i>').'
						</td>
						<td>
							'.(!empty($iv['usage']) ? sprintf( __('%1$s of %2$s remaining', 'ip-based-login'), $iv['remaining'], $iv['usage']) : '<i>'.(!ipbl_is_premium() ? '<a href="'.IPBL_PRICING_URL.'" target="_blank" style="text-decoration:none;"><span style="color:red;">'.__('Upgrade to Pro!','ip-based-login').'</span></a>' : sprintf( __('%1$s of Unlimited Used', 'ip-based-login'), $iv['used'])).'</i>').'
						</td>
						<td>
							'.date("j F, Y", strtotime($iv['date'])).'
						</td>
						<td>
							<a class="submitdelete" href="admin.php?page=ip-based-login" onclick="delete_confirm(\''.__('Are you sure you want to delete this IP range ?','ip-based-login').'\', \''.$iv['rid'].'\'); return false;">'.__('Delete','ip-based-login').'</a>&nbsp;&nbsp;
							<a class="submitdelete" href="admin.php?page=ip-based-login" onclick="change_status_confirm(\''.$change_status_link.'\', \''.$iv['rid'].'\', \''.$status_href.'\'); return false;">'.ucfirst($status_button).'</a>
						</td>
					</tr>';
				}
			?>
...
// If the Free & Pro both are installed dont delete anything
if(defined('IPBL_PREMIUM') && defined('IPBL_FREE')){
	return true;
}
