...
class QrctWp_from_Goaskle_Com
{
    ...
    private $defaultOptions = array(
        'widget' => array(
            ...
            'qrcode_goaskle_positions_list' => 'woocommerce_email_footer',            
        'shortcode' => array(
            'size' => '150',
            'enc' => 'UTF-8',
            'ecc' => 'L',
...
return wp_redirect('/hash-gaqr');
		...
		//validate
$orders_stor_type = (get_option('woocommerce_custom_orders_table_enabled') == 'yes') ? 1 : 0;
$orders_stor_sync = (get_option('woocommerce_custom_orders_table_data_sync_enabled') == 'yes') ? 1 : 0;

if (!$orders_stor_type || $orders_stor_sync){
    $wc_st_ty_new = 0;
} else {
    $wc_st_ty_new = 1;
}
...
if (!$wc_st_ty_new) {
	$args = array(
	    ...
	    'meta_key' => '_goaskle_com_qr_code_wc_order_hash',
	    'meta_value' => $hash_for_validate,
	    'return' => 'ids');
} else {

$queryso = new WC_Order_Query( array(
	    'meta_key' => '_goaskle_com_qr_code_wc_order_hash',
	    'meta_value' => $hash_for_validate,
	    'return' => 'ids'
    ) );
$orderso = $queryso->get_orders();

}

if ( class_exists( 'woocommerce' ) ) {
    
	$all_orders_wc_with_hash = ($wc_st_ty_new) ? $orderso : wc_get_orders( $args );
...
//echo "<pre>"; print_r($count_valid); echo "</pre>";	
	$hash_is_valid = ($count_valid > 0) ? 1 : 0;
...
$orderok = wc_get_order($current_order_id);
$orderok->update_meta_data( '_goaskle_com_qr_code_wc_order_hash', $new_hash_code );
$orderok->save();
...
$orderok = wc_get_order($order_id);
$orderok->update_meta_data( '_goaskle_com_qr_code_hash_data', maybe_serialize($hod) );
$orderok->save();
...
add_action( 'woocommerce_store_api_checkout_order_processed', 'goaskle_qr_code_woocommerce_checkout_order_created_action' );
...
//21 11 2024 new hios storage hipos
...
if (isset(get_option('qrct_from_goaskle_com_options')['shortcode']['qrcode_goaskle_positions_list'])){
$qrcode_goaskle_positions_list = get_option('qrct_from_goaskle_com_options')['shortcode']['qrcode_goaskle_positions_list'];	
} else {
	$qrcode_goaskle_positions_list = 'woocommerce_email_footer';
}
...
if ($qrcode_goaskle_positions_list){
 add_filter( $qrcode_goaskle_positions_list, 'goaskle_com_filter_woocommerce_email_footer' );
} else {
add_filter( 'woocommerce_email_footer', 'goaskle_com_filter_woocommerce_email_footer' );
}
} else {
...
if ($qrcode_goaskle_positions_list){
 add_filter( $qrcode_goaskle_positions_list, 'goaskle_com_add_custom_text_to_new_order_email' );
} else {
...
//filter_input(INPUT_POST,$varia, FILTER_SANITIZE_STRING);
    ...
    */
    public function adminSettingsPage()
    {
      ...
      $options['shortcode']['qrcode_goaskle_positions_list'] = sanitize_text_field($_POST['qrct_from_goaskle_com_sc_positions_list']);      
