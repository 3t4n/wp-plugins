...
function enoty_create_meta_box( $post, $enotymeta_box )
{
	...
	foreach ( $enotymeta_box['fields'] as $field ){
		...
		switch( $field['type'] ){	
			case 'text':
				echo '<td><input type="text" name="enoty_meta['. esc_attr( $field['id'] ) .']" id="'. esc_attr( $field['id'] ) .'" value="'. ($meta ? esc_attr( $meta ) : $field['std']) .'" size="30" /></td>';
				break;	
				
			case 'shorttext':
				echo '<td><input style="width:43px !important;" type="text" name="enoty_meta['. $field['id'] .']" id="'. $field['id'] .'" value="'. ($meta ? esc_attr( $meta ) : esc_attr( $field['std'] )) .'"/>'.$field['aftr'].'
...
<span style="border-right:solid 1px #CCC;margin-left:9px; margin-right:10px !important; "></span>
    ...
    <?php }
			...
			case 'color':
				...
				<input style="margin-left:10px; width:75px !important;" name="enoty_meta['. $field['id'] .']" id="'. $field['id'] .'" type="text" value="'.($meta ? esc_attr( $meta ) : $field['std']).'" />';
                echo '</td>';
			    break;
			...
			case 'typo':
				...
				<input style="margin-'.( is_rtl() ? 'right' : 'left' ).':10px; width:75px !important;" name="enoty_meta['. $field['id'] .'_'.$field['color'].']" id="'. $field['id'] .'[color]" type="text" value="'.(get_post_meta($post->ID, $field['id'].'_'. $field['color'], true) ? esc_attr(get_post_meta($post->ID, $field['id'].'_'. $field['color'], true)) : $field['stcol']).'" />';
                echo '</td>';
			    break;							
    ...
    if ($meta) {
        foreach($meta as $row) {
            echo '<li><span class="thesort"></span><input style="width:395px !important;" type="text" name="enoty_meta['.$field['id'].']['.$i.']" id="'.$field['id'].'" value="'.($row ? esc_attr( $row ) : $field['std']).'" size="30" /><span class="repeatableremove"></span></li>';
            $i++;
		} 
    } else {
        foreach($field['std'] as $row) {
            echo '<li><span class="thesort"></span><input style="width:395px !important;" type="text" name="enoty_meta['.$field['id'].']['.$i.']" id="'.$field['id'].'" value="'.($row ? esc_attr( $row ) : '').'" size="30" /><span class="repeatableremove"></span></li>';
            $i++;
		} 
    }
    echo '</ul>';
...
/* Slider init */
    ...
    <div class="eno_metaslider"><div id="<?php echo $field['id']; ?>_slider" ></div><input style="margin-left:10px; margin-right:5px !important; width:40px !important;" name="enoty_meta[<?php echo $field['id']; ?>]" id="<?php echo $field['id']; ?>" type="text" value="<?php if ( $meta != "") { echo esc_attr( $meta ); } else { echo esc_attr( $field['std'] ); } ?>" /><?php echo $field['pixopr']; ?></div> 
			...
			case 'gradient':
				...
				<input style="margin-left:10px; width:75px !important;" name="enoty_meta['. $field['id'] .'_'.$field['from'].']" id="'. $field['id'].$field['from'].'" type="text" value="'.(get_post_meta($post->ID, $field['id'].'_'.$field['from'], true) ? esc_attr( get_post_meta($post->ID, $field['id'].'_'. $field['from'], true)) : $field['stdfrom']).'" /> to<div id="'.$field['id'].'_'.$field['to'].'_picker" class="colorSelector" style="float:none !important; top: 7px !important; margin-left: 20px !important;"><div></div></div>
				<input style="margin-left:10px; width:75px !important;" name="enoty_meta['.$field['id'] .'_'.$field['to'].']" id="'. $field['id'] .$field['to'].'" type="text" value="'.(get_post_meta($post->ID, $field['id'].'_'. $field['to'], true) ? esc_attr( get_post_meta($post->ID, $field['id'].'_'. $field['to'], true)) : $field['stdto']).'" /><input class="button button_color_calc" type="button" value="Generate Gradient" style="margin-top: 7px;"></input>';
                echo '</td>';
			    break;
...
function enoty_save_meta_box( $post_id ) {
	...
	if ( 'page' == $_POST['post_type'] ) {
		...
		if ( !current_user_can( 'edit_post', $post_id ) ) return;
			...
			if ( !is_array( $val ) ) {
				$_POST['enoty_meta'][$key] = sanitize_text_field( $val );
			}
			else {
				$_POST['enoty_meta'][$key] = array();
				foreach( $val as $arr_val ) {$_POST['enoty_meta'][$key][] = sanitize_text_field( $arr_val );}
			}
		}
		// save data
