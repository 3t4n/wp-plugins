(function ($) {
  ...
  $(document).ready(function () {
    ...
    $("body").on("click", ".nx-metatab-menu li, .utl-notifications-builder-tab-menu li, .nx-meta-next, .nx-quick-builder-btn", function (e) {
      e.preventDefault();
    ...
    $("body").on("change", ".nx-single-theme-wrapper > input:checked", function (e) {
      e.preventDefault();
    ...
    $("body").on("change", "#nx_meta_time_randomize", function () {
      ...
      if (!$("#nx_meta_enable_countdown").is(":checked") || !$("#nx_meta_evergreen_timer").is(":checked")) {
        $(".nx-time_randomize").hide();
        $(".nx-time_reset").hide();
        return;
      }
          ...
          success: function (response) {
            ...
            if (!self.hasClass("nx-bar_with_elementor_install")) {
              ...
              setTimeout(function () {
                ...
                $(".nx-bar_with_elementor").removeClass("hidden");
              }, 2000);
            }
          },
  ...
  $(window).load(function () {
    ...
    if (qVars != undefined) {
      if (qVars.indexOf("utl-notifications-settings") >= 0) {
        ...
        $('.utl-notifications-settings-menu li[data-tab="' + cSettingsTab[1] + '"]').trigger("click");
      }
    }
    ...
    $("body").on("change", ".nx_meta_reviews_source", function () {
      ...
      switch (source) {
        case "wp_reviews":
          $("#nx_meta_wp_reviews_template_adv").trigger("change");
          var thirdParam = $("#nx_meta_wp_reviews_template_new_third_param");
    ...
    $("body").on("change", ".utl-notifications-builder-content-wrapper .nx_meta_display_type", function (e) {
  ...
  $.notificationx.get_instructions_enabled = function (type, forSource) {
    ...
    if (!forSource && type != false) {
      Array.from(document.querySelectorAll("#nx-instructions .nxins-type")).forEach(function (item) {
      ...
      Array.from(document.querySelectorAll("#nx-instructions .nxins-type .nxins-type-source")).forEach(function (item) {
        item.style.display = "none";
  ...
  $.notificationx.create_nx_bar = function () {
    ...
    $(".nx-bar_with_elementor-import").on("click", function (e) {
      ...
      $(".nx-press-bar-modal-preload.active .nx-modal-loading-text").addClass("active");
      var self = $(this),
      ...
      $.ajax({
        ...
        success: function (res) {
          ...
          $(".nx-press-bar-modal-preload.active .nx-modal-loading-text.active").removeClass("active");
          $(".nx-press-bar-modal-preload.active .nx-modal-success-text").addClass("active");
        },
      }).fail(function (err) {
        console.log(err);
      });
  ...
  $.notificationx.bindEvents = function () {
    ...
    $("body").on("click", ".nx-adv-checkbox-wrap label, #nx_sound_checkbox, .nx-stats-tease, .nx-cmo-conf", function (e) {
    ...
    $(".nx-checkbox-area .nx-checkbox input:enabled, .utl-notifications-settings-field").on("click", function (e) {
      saveButton.addClass("nx-save-now").removeAttr("disabled").css("cursor", "pointer");
    });
  ...
  $.notificationx.initializeFields = function () {
    if ($(".nx-meta-field, .utl-notifications-settings-field").length > 0) {
      $(".nx-meta-field, .utl-notifications-settings-field").map(function (iterator, item) {
  ...
  $.notificationx.tabChanger = function (buttonName) {
    ...
    var contentMenu = $(".utl-notifications-builder-tab-menu").find('li[data-tab="content_tab"]'),
      cDisplay = "none";
    if (contentMenu.length > 0) {
      if (contentMenu[0] != undefined) {
        cDisplay = contentMenu[0].style.display;
        var lMenu = $('.utl-notifications-builder-tab-menu li[data-tabid="' + tabID + '"]');
    ...
    $('.utl-notifications-builder-tab-menu li[data-tabid="' + tabID + '"]').trigger("click");
  };

  $.notificationx.toggleFields = function () {
    $("body").delegate(".nx-meta-field, .utl-notifications-settings-field", "change", function (e) {
      if (this.type == "radio") {
        if (this.checked) {
    ...
    $("body").delegate(".nx-meta-field, .utl-notifications-settings-field", "click", function (e) {
