...
class InfoBox extends GenericWidget
{
	...
	{
		return esc_html__('Generic Info Box', 'generic-elements');
	}
	...
	public function infobox_content_controls()
	{
		...
		$this->add_responsive_control(
			'icon_position',
			[
				'label' => esc_html__('Icon/Image Position', 'elementor'),
				'type' => \Elementor\Controls_Manager::CHOOSE,
				'default' => 'top',
				'mobile_default' => 'top',
				'options' => [
	...
	protected function button_content_controls()
	{
		...
		$this->add_control(
			'button_ex_class',
			[
				'label' => esc_html__('Extra Class', 'generic-elements'),
				'type' => \Elementor\Controls_Manager::TEXT,
				'default' => '',
				'placeholder' => esc_html__('Type your class name here', 'generic-elements'),
				'label_block' => true,
				'dynamic' => [
					'active' => true,
				]
		...
		$this->add_control(
			...
			[
				...
				'selectors_dictionary' => [
					'start' => '-webkit-box-pack: start; -ms-flex-pack: start; justify-content: flex-start;',
					'center' => '-webkit-box-pack: center; -ms-flex-pack: center; justify-content: center;',
					'end' => '-webkit-box-pack: end; -ms-flex-pack: end; justify-content: flex-end;',
	...
	protected function icon_image_style_controls()
	{
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', 'em', '%'],
				'selectors' => [
					'{{WRAPPER}} .generic-info-box-icon' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', 'em', '%'],
				'selectors' => [
					'{{WRAPPER}} .generic-info-box-icon' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
	...
	protected function title_and_description_style_controls()
	{
		...
		$this->add_control(
			...
			[
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .generic-el-title:hover' => 'color: {{VALUE}};',
				],
			]
		);
	...
	protected function button_style_controls()
	{
		...
		$this->add_group_control(
			...
			[
				...
				'types' => ['classic', 'gradient'],
				'selector' => '{{WRAPPER}} .generic-el-btn:hover, {{WRAPPER}} .generic-el-btn:focus',
			]
		);
	...
	protected function render()
	{
		...
		extract($settings);
		$position_class = $settings['icon_position'];
		$button_ex_class =  $settings['button_ex_class'];
		...
		$title_html = sprintf('<%1$s %2$s>%3$s</%1$s>', tag_escape($settings['title_tag']), $this->get_render_attribute_string('title'), $title);

		if (!empty($settings['button_link'])) {
			$this->add_render_attribute('button', 'class', $button_ex_class . ' generic-border-btn generic-el-btn');
...
?>
		<div class="generice-el-wrapper <?php echo esc_attr($gen_extra_class); ?>">
			<div class="bdevs-generic-el generic-ifnobox generic-el-infobox-wrapper ">
				<div class="generic-infobox-content generic-el-content <?php echo esc_attr($position_class); ?>">
					<div class="generic-infobox-icon generic-infobox-icon d-inline-block">
					...
					<div class="generic-infobox-inner">
