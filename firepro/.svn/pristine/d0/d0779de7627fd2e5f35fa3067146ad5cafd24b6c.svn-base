<?php
/**
 * firepro-blocks.
 * User: Paul
 * Date: 2020-07-29
 */
namespace firepro;

if (!defined('WPINC')) {
    die;
}

class Settings extends Firepro
{

    public function __construct()
    {
      /**
      ** Listen For Admin Menu Hooks
      **/
      require 'pages/../../media/firepro_64.php';
      add_action('admin_menu', array($this, 'menu'));
      add_action('admin_enqueue_scripts', array($this, 'admin_page_scripts'));
    }

    public function menu()
    {
        /**
        ** Enqueue Menu Pages
        **/
        add_menu_page('Gettings Started', 'FirePro', 'manage_options', 'firepro-settings', array($this, 'firepro_getting_started_page'), 'data:image/svg+xml;base64,', 90);
        add_submenu_page('firepro-settings', 'Getting Started', 'Getting Started', 'manage_options', 'firepro-settings', array($this, 'firepro_getting_started_page'));
        add_submenu_page('firepro-settings', 'Pro Account', 'Pro Setup', 'manage_options', 'firepro-pro-animations', array($this, 'firepro_premium_page'));
    }

    public function admin_page_scripts()
    {
      $stylePath = substr(plugin_dir_url(__FILE__), 0, -9) . "/includes/admin_assets/admin_style.css";
      wp_register_style( 'admin_css', $stylePath, false, '1.0.0' );
      wp_enqueue_style( 'admin_css' );
      wp_enqueue_script( 'firepro', 'https://cdn.firepro.io/releases/firepro.js' );
    }

    public function firepro_api_key_render()
    {
        $options = get_option('firepro_api_settings');
        ?>
        <input type='text' name='firepro_api_settings[firepro_api_text_field_0]' value='<?php echo $options['firepro_api_text_field_0']; ?>' class="widefat">
        <?php
    }


    public function firepro_api_settings_section_callback()
    {

        esc_html_e('Hi there! You are currently using a trial version of FirePro. This version allows you to run 200 animations a month.', 'firepro');
        esc_html_e('You currently have X animations remaining for the month.', 'firepro');
        esc_html_e('If you would like to have more signup for a FirePro account and enter your API key:', 'firepro');
        // printf('<p>' . esc_html__('You have %d Calls left to the CDN this month', 'firepro') . '</p>', $this->calls_left());
    }

    public function firepro_getting_started_page(){
      echo "<style>.update-nag{display: none;}</style>";
      require 'pages/../../media/firepro_64.php';
      require 'pages/getting_started.php';
    }

    public function firepro_premium_page()
    {
      echo "<style>.update-nag{display: none;}</style>";
      require 'pages/../../media/firepro_64.php';
      require 'pages/pro_page.php';
    }

    public function firepro_demo_page()
    {
      require 'pages/template_manager.php';
    }
}
