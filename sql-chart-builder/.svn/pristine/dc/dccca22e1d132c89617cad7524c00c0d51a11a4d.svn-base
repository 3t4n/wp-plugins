...
function guaven_sqlcharts_recommended(){
    ...
    return '<table class="gf-alert gf-alert-info" style="margin-top:20px">
            ...
            <a href="'.admin_url().'/edit.php?post_type=gvn_schart&gvnsql_dismiss_recommendation=1'.'" style="position: absolute;right: 0;top: -15px;right: -10px;">{svg}
            </a>
...
function guaven_sqlcharts_my_admin_notice()
{
    ...
    if( 
        (!empty($_SERVER["REQUEST_URI"]) and strpos(sanitize_text_field(wp_unslash($_SERVER["REQUEST_URI"])),'post_type=gvn_schart')!==false)
        or 
        (!empty($post) and $post->post_type == 'gvn_schart')
     ){
        echo str_replace('{svg}','<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="11" height="14" viewBox="0 0 11 14">
            ...
            </svg>',wp_kses_post(guaven_sqlcharts_recommended(),[]));
     }
  ...
  <div class="updated gf-alert gf-alert-danger">Only administrators can manage this page</div>';
        ...
        if (empty($_GET["post"]) && strpos(sanitize_text_field(wp_unslash($_SERVER["REQUEST_URI"])), "post-new") === false) {
            $gf_message = __( 
                'Use <b>Add new</b> button above to create a new SQL report. And click on any existing rule names below to manage them.',
                'guaven_sqlcharts' 
            );
        } else {
            $gf_message = __(
                '1. Give any name to your report.<br>
                2. Choose chart type, type SQL query, enter field names, labels, and then press Publish/Update.<br>
        ...
        echo '<div style="float:left;max-width:calc(100% - 345px)">';
...
{   
    wp_enqueue_script('guaven_sqlcharts_chartjs', plugins_url('asset/bundle.min.js', __FILE__),array('jquery'),GVNSQLCHARTS_VERSION,false);
...
function guaven_sqlcharts_register_post()
{
    ...
    register_post_type('gvn_schart', array(
        'labels' => array(
            'name' => __('My SQL Charts','guaven_sqlcharts'),
            'singular_name' => __('My SQL chart','guaven_sqlcharts')
        ),
...
function gvn_chart_top_form($atts){
  ...
  foreach ($variables_raw as $vrow){
    ...
    if ($vrow_arr[3]=='date') {
      $dateexists=true;
      $topform.= $vrow_arr[2].' <input class="gws_datepicker" autocomplete="off" style="display:inline-block;max-width:210px;display:inline-block;color:red" type="text"
      value="'.$gvalue.'"
      data-toggle="datepicker" name="'.$vrow_arr[0].'" placeholder="'.$dvalue.'">
      ';}
    else {
      $topform.= $vrow_arr[2].' <input autocomplete="off" style="display:inline-block;max-width:210px;'.($vrow_arr[3]=='number'?'width:100px;':'').'"
      type="'.$vrow_arr[3].'"
  ...
  if (!empty($topform)) {
    ...
    $submit_button_value = get_post_meta($atts['id'], 'guaven_sqlcharts_formpartbutton', true) != '' 
        ? esc_attr(get_post_meta($atts['id'], 'guaven_sqlcharts_formpartbutton', true)) 
        : 'OK';
    
    $topform = '<form method="get" action="" class="guaven_sqlcharts_form">' . $topform . '
        <input type="submit" value="' . $submit_button_value . '"></form>';
    
    if ($dateexists) {
        $topform .= '<script>setTimeout(function(){jQuery(\'[data-toggle="datepicker"]\').datepicker({format: \'yyyy-mm-dd\'});},300);</script>';
    }
    
    echo wp_kses($topform, $allowed_html);
  }
}
...
function guaven_sqlcharts_local_shortcode($atts) {
    ...
    for($i=1;$i<20;$i++){ 
        if(strpos($sql,"{arg".$i."}")!==false){
            $replacearg=!empty($atts["arg".$i])?$atts["arg".$i]:0;
            $sql = str_replace("{arg".$i."}", "%s", $sql);
            $sql=$wpdb->prepare($sql,$replacearg);
        }
    ...
    gvn_chart_top_form($atts);
    ?>
    <canvas 
        id="ct-chart_<?php echo esc_attr($sqlcharts_inserted_script); ?>" 
        class="guaven_chart_canvas"
        style="width: <?php echo esc_attr(get_post_meta($atts['id']), 'guaven_sqlcharts_chartwidth', true); ?>px !important; height: <?php echo esc_attr(get_post_meta($atts['id'], 'guaven_sqlcharts_chartheight', true)); ?>px !important;"
    ></canvas>

     <script type="text/javascript" class="gvn_charts_script" async>
     var ctx = jQuery("#ct-chart_<?php
    echo esc_attr($sqlcharts_inserted_script);
    ?>");
    ...
    <?php
    if (!empty($atts["table"])) guaven_sqlcharts_tablepart($post_g->post_title, $labels, $values, $ylabel,$xlabel);
    $sqlcharts_inserted_script++;
    $ret=ob_get_clean();
    $ret=apply_filters( 'guaven_sqlcharts_final_output', $ret, $atts );
    return $ret;
}
...
add_shortcode("gvn_schart_2_cached",function($atts){
	...
	set_transient('cached_sql_charts_'.$atts["id"].$is_logged_in, $tobecached,$expire);//you can change 3600 yourself
	return $tobecached;
});
...
function guaven_sqlcharts_colors($index, $pid = null){
  ...
  return wp_rand(0, 255) . ',' . wp_rand(0, 255) . ',' . wp_rand(0, 255);
}
...
?>
    var data = {
    labels: [<?php guaven_sqlcharts_merge_labeldata($labels);?>],
    datasets: [
    <?php
...
?>
        {
            ...
            if(!empty($GLOBALS["guaven_sqlcharts_atts"]["params"])){
                ...
                echo wp_kses($GLOBALS["guaven_sqlcharts_atts"]["params"],[]);
            } 
            ?>
            label: "<?php
        echo wp_kses($ylabel[$key_ak],[]);
?>",
            backgroundColor: [
                <?php
        echo wp_kses(guaven_sqlcharts_colorgenerator(count($values_new[$key_ak]), 0, 0, guaven_sqlcharts_colors($i, $pid)),[]);
?>
            ],
            borderColor: [
                <?php
        echo  wp_kses(guaven_sqlcharts_colorgenerator(count($values_new[$key_ak]), 0, 0.2, guaven_sqlcharts_colors($i, $pid)),[]);
?>
            ],
            borderWidth: 1,
            data: [<?php
        echo  wp_kses(implode(",", $values_new[$key_ak]),[]);
?>],
        },
        <?php
    }
...
var options={
    ...
    <?php 
    guaven_sqlcharts_maybe_additional_parameters($pid);
    ?>
};
...
function guaven_sqlcharts_merge_labeldata($labels){
    if(count($labels)==1){echo wp_kses(implode(",",$labels[0]),[]);return;}
    $merged=[];
    foreach($labels as $label){
        $merged=array_merge($merged,$label);
    }
    echo  wp_kses(implode(",",array_unique($merged)),[]);
}
...
?>",
            fill: <?php echo $type=="false"? esc_attr($type):($i==0?'"+1"':'"origin"');
?>,
            lineTension: 0.1,
            backgroundColor:  <?php
        echo  wp_kses_post(guaven_sqlcharts_colorgenerator(1, 1, 0.2, guaven_sqlcharts_colors($i, $pid)));
?>
            borderColor:  <?php
        echo  wp_kses_post(guaven_sqlcharts_colorgenerator(1, 1, 0.2, guaven_sqlcharts_colors($i, $pid)));
?>
...
?>
            data: [<?php
        echo  wp_kses_post(implode(",", $values_new[$key_ak]));
?>],
            spanGaps: false,
        },
        <?php
...
function guaven_sqlcharts_maybe_additional_parameters($pid){
    if(function_exists('guaven_sqlcharts_maybe_additional_parameters_custom')){
        wp_kses(guaven_sqlcharts_maybe_additional_parameters_custom($pid),[]);
        return;
    }
    $guaven_sqlcharts_legend_position=get_post_meta($pid, 'guaven_sqlcharts_legend_position', true);
    if(in_array($guaven_sqlcharts_legend_position,['top','bottom','left','right'])){
    ...
    echo wp_kses( ",legend: {display: ".$display.",position:'".$position."'}",[]);
}
...
?>
        {
            ...
            ?>
            data: [<?php
        echo wp_kses(implode(",", $values[$i]),[]);
?>],
            backgroundColor: [
                <?php
                $ii=0;
                foreach($values[$i] as $vci=>$valuecolor){
                    echo wp_kses_post(guaven_sqlcharts_colorgenerator(1, 0, -0.1, guaven_sqlcharts_colors($ii, $pid)));
                    $ii++;
                }       
?>
            ],
            hoverBackgroundColor: [
               ...
               $ii=0;
               foreach($values[$i] as $vci=>$valuecolor){
                echo wp_kses_post(guaven_sqlcharts_colorgenerator(1, 0, 0.2, guaven_sqlcharts_colors($ii, $pid)));
                $ii++;
            } 
?>
            ]
