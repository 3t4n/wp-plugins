...
<div style="width:110px;float:left">
    <img src="<?php echo esc_url(plugin_dir_url(__FILE__) . 'asset/img/pie.png'); ?>" class="charttype" style="opacity:0.5;margin-top:20px;float:right;width:100px">
</div>
...
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_code', true));
?></textarea>
...
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_xarg_l', true));
?>">
<br>
...
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_yarg_l', true));
?>">
...
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_xarg_s', true));
?>"><br>
...
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_yarg_s', true));
?>"><br>
...
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_chartwidth', true));
?>">

<input type="number" name="guaven_sqlcharts_chartheight" id="guaven_sqlcharts_chartheight" value="<?php
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_chartheight', true));
?>">
</p>
...
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_colors', true));
?>" style="width:500px">
...
Legend block
<br>
<?php $guaven_sqlcharts_legend_position=esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_legend_position', true));?>
...
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_dbhost', true));
?>" placeholder="database host">

<input type="text" name="guaven_sqlcharts_dbname" id="guaven_sqlcharts_dbname" value="<?php
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_dbname', true));
?>" placeholder="database name">

<input type="text" name="guaven_sqlcharts_dblogin" id="guaven_sqlcharts_dblogin" value="<?php
echo esc_attr(get_post_meta($post->ID, 'guaven_sqlcharts_dblogin', true));
...
echo esc_textarea(get_post_meta($post->ID, 'guaven_sqlcharts_variables', true));
?>
</textarea>
...
<?php
    echo do_shortcode('[gvn_schart' . esc_attr($postfix) . ' id="' . esc_attr($post->ID) . '" ' . (get_post_meta($post->ID,'guaven_sqlcharts_tablepart', true) !== '' ? ' table="1"' : '') . ']');

}
?>
<script>
jQuery(".dynamicfieldbutton").click(function(){jQuery(".dynamicfielddiv").fadeToggle("slow")});
plugindirurl = '<?php echo esc_url(plugin_dir_url(__FILE__) . 'asset/img/'); ?>';
