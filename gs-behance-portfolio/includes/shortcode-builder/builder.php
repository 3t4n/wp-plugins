...
class Builder {
	...
	public function admin_scripts( $hook ) {
		...
		if ( 'toplevel_page_gs-behance' !== $hook ) {
			return;
		}
	...
	public function update_shortcode() {
		...
		if ( empty( $shortcode ) ) {
			wp_send_json_error( __( 'No shortcode found to update', 'gs-behance' ), 404 );
		}
		...
		$tableName = plugin()->db->get_table_name( 'shortcodes' );
		$wpdb      = $this->get_wp_db();
	...
	public function save_temp_shortcode_settings() {
		...
		if ( empty( $temp_key ) ) {
			wp_send_json_error( __( 'No temp key provided', 'gs-behance' ), 400 );
		}
		if ( empty( $shortcode_settings ) ) {
			wp_send_json_error( __( 'No temp settings provided', 'gs-behance' ), 400 );
		}

		delete_transient( $temp_key );

		$shortcode_settings = $this->validate_shortcode_settings( $shortcode_settings );
		$userid             = $shortcode_settings['userid'];
	...
	public function _get_shortcode( $shortcode_id, $is_ajax = false ) {
		...
		if ( $shortcode ) {
			...
			// reset orderby
			$order_by_values = wp_list_pluck( $this->get_order_by(), 'value' );
			if ( !empty( $order_by = $shortcode["shortcode_settings"]['order_by'] ) && ! in_array( $order_by, $order_by_values ) ) {
				$shortcode["shortcode_settings"]['order_by'] = 'published_on';
			}
		...
		if ( $is_ajax ) {
			wp_send_json_error( __( 'No shortcode found', 'gs-behance' ), 404 );
		}
	...
	public function validate_shortcode_settings( $shortcode_settings ) {
		...
		$shortcode_settings['number_of_paginate_items'] = intval( $shortcode_settings['number_of_paginate_items'] );
		...
		$shortcode_settings['textarea']                 = sanitize_text_field( $shortcode_settings['textarea'] );
		...
		$shortcode_settings['projects']                 = intval( $shortcode_settings['projects'] );
		$shortcode_settings['order']                    = sanitize_text_field( $shortcode_settings['order'] );
		$shortcode_settings['order_by']                 = sanitize_text_field( $shortcode_settings['order_by'] );
//        $shortcode_settings['categories']                       = sanitize_text_field( $shortcode_settings['categories'] );
	...
	public function get_order_by() {
		return array(
			[
				'label' => __( 'Published On', 'gs-behance' ),
				'value' => 'published_on'
			],
			[
				'label' => __( 'Views', 'gs-behance' ),
				'value' => 'views'
			],
			[
				'label' => __( 'Comments', 'gs-behance' ),
				'value' => 'comments'
			],
			[
				'label' => __( 'Likes', 'gs-behance' ),
				'value' => 'likes'
			],
		);
	}
	...
	public function get_strings() {
		return [
			...
			'projects'                      => __( 'Projects', 'gs-behance' ),
			'order'                         => __( 'Order', 'gs-behance' ),
			'order_by'                      => __( 'Order By', 'gs-behance' ),
			'categories'                    => __( 'Categories', 'gs-behance' ),
			'edit'                          => __( 'Edit', 'gs-behance' ),
			'pagination'                    => __( 'Pagination', 'gs-behance' ),
			'number_of_paginate_items'      => __( 'Paginate Items', 'gs-behance' ),
			...
			'general-settings'              => __( 'General', 'gs-behance' ),
			'style-settings'                => __( 'Style', 'gs-behance' ),
			...
			'textarea'                      => __( 'User Profile Desc', 'gs-behance' ),
		];
	}

	public function maybe_upgrade_data( $old_version ) {
		if ( version_compare( $old_version, '3.0.3', '<' ) ) {
			$this->upgrade_to_3_0_3();
		}
	}
	...
	public function upgrade_to_3_0_3() {
		...
		foreach ( $shortcodes as $shortcode ) {
			...
			if ( isset( $shortcode['userid'] ) && isset( $shortcode['count'] ) ) {
				...
				$shortcode['shortcode_settings']['textarea'] = $shortcode['textarea'];
		...
		$shortcodeTableName = plugin()->db->get_table_name( 'shortcodes' );
		$query = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = '%s' AND column_name = %s";

		if ( ! empty( $wpdb->get_results( $wpdb->prepare( $query, $shortcodeTableName, 'field' ) ) ) ) {
			$wpdb->query( "ALTER TABLE $shortcodeTableName DROP field" );
		}

		if ( ! empty( $wpdb->get_results( $wpdb->prepare( $query, $shortcodeTableName, 'count' ) ) ) ) {
			$wpdb->query( "ALTER TABLE $shortcodeTableName DROP count" );
		}

		if ( ! empty( $wpdb->get_results( $wpdb->prepare( $query, $shortcodeTableName, 'userid' ) ) ) ) {
			$wpdb->query( "ALTER TABLE $shortcodeTableName DROP userid" );
		}
