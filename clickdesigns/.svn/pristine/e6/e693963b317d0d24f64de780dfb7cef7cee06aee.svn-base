	...
	<div class="cd-wrapper-inner">
		<div class="main-screen">
			<a href="javascript:;" class="cd-logo">
				<img src="'.esc_url(CLICKDESIGNS_PLUGIN_URL).'/assets/images/logo.svg">
			</a>
			<h3>'.esc_html__('Welcome to ClickDesigns','clickdesigns').'</h3>';
			if(empty($api_key)){
		...
		if($wp_version != $cversion){
			echo '<p class="cd_note"><b>'.esc_html__('Note : ', 'clickdesigns').'</b>'.esc_html__('Please update your WordPress version to ','clickdesigns').esc_html($obj->offers[0]->current).esc_html__('.', 'clickdesigns').'</p>';
		}
	echo '</div>
</div>';