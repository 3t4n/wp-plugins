...
class ClickDesignsSetting{
	...
	public function clickdesigns_assets() {
		...
		// ajax
		wp_localize_script( 'clickdesigns-custom', 'frontendajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'nonce' => wp_create_nonce('ajax-nonce')));
	}	
	
	// Elementor editor script
	public function clickdesigns_editor_elementor_scripts() {
		wp_enqueue_media();
		wp_register_script( 'clickdesigns-elemenor', plugins_url( '../assets/js/clickdesigns-custom.js', __FILE__ ),array(), CLICKDESIGNS_VERSION, true );
		wp_localize_script( 'clickdesigns-elemenor', 'frontendajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'nonce' => wp_create_nonce('ajax-nonce') ) );
		wp_enqueue_script( 'clickdesigns-elemenor' );
	}
	
	// Elementor editor style
	public function clickdesigns_editor_elementor_styles() {
		wp_register_style( 'clickdesigns-elemenor', plugins_url( '../assets/css/clickdesigns-style.css', __FILE__ ), array(), CLICKDESIGNS_VERSION, 'all' );
