...
class ClickDesignsAjax{
	...
	*/
	public function clickdesigns_add_api(){
		...
		if(!empty($_POST)){
			...
			if ( wp_verify_nonce( $nonce, 'ajax-nonce' ) == 1 ) {
				if(!empty($_POST['key'])){
				...
				$data = array('status' => 'false', 'msg'=> 'Security Issue.' );
			}
	...
	*/
	public function clickdesigns_tab_media(){
		...
		if ( ! current_user_can( 'edit_posts') ) {
			$data = array('status' => 'false', 'msg'=> 'Security Issue.', 'data'=>$post_id );
		}else{
			$nonce = isset($_POST['nonce'])? sanitize_text_field( wp_unslash ( $_POST['nonce'] ) ):'';
			if ( wp_verify_nonce( $nonce, 'ajax-nonce' ) == 1 ) {
				...
				$data = array('status' => 'false', 'msg'=> 'Security Issue.', 'data'=>$_POST );
			}
		}
		echo wp_json_encode($data);
		wp_die();
	...
	*/
	public function clickdesigns_api_images(){
		if ( ! current_user_can( 'edit_posts' ) ) {
			echo esc_html__('No images found','clickdesigns');
		}else{
			$nonce = isset($_POST['nonce'])? sanitize_text_field( wp_unslash ( $_POST['nonce'] ) ):'';
			if ( wp_verify_nonce( $nonce, 'ajax-nonce' ) == 1 ) {
				$key = get_option('clickdesign_api');
				...
				echo '<div class="cd-media-section">
					<div class="cd-top-div">
						...
						<div class="cd-data-filter">
							<a href="javascript:void(0);" class="cds-btn cds_btn_refresh"><img src="'.esc_url(CLICKDESIGNS_PLUGIN_URL).'/assets/images/refresh.svg"> Refresh</a>
						...
						<div class="cd-media-images">
							...
							if($count <= 20){
								echo '<div class="load_btn">
									<a href="javascript:;" data-page=1 data-count='.esc_attr($count).' id="cd_load_more">'.esc_html__('Load More', 'clickdesigns').'</a>
								</div>';
							}
						echo '</div>
					</div>
	...
	*/
	public function clickdesigns_get_designs_images_tab_one(){
		$data = '';
		if ( ! current_user_can( 'edit_posts' ) ) {
			$data = array('status' => 'false', 'page' => 'false', 'html'=> 'Security Issue.' );
		}else{
			$nonce = isset($_POST['nonce'])? sanitize_text_field( wp_unslash ( $_POST['nonce'] ) ):'';
			if ( wp_verify_nonce( $nonce, 'ajax-nonce' ) == 1 ) {
				$key = get_option('clickdesign_api');
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			...
			if ( wp_verify_nonce( $nonce, 'ajax-nonce' ) == 1 ) {
				if(!empty($_POST['image'])){
					$image = sanitize_url( wp_unslash ( $_POST['image'] ) );
				}
				if(!empty($_POST['format'])){
					$format = sanitize_text_field( wp_unslash ( $_POST['format'] ) );  
				}
				if(!empty($_POST['title'])){
					$img_title = sanitize_text_field( wp_unslash ( $_POST['title'] ) );
				}
	...
	*/
	public function clickdesigns_user_media(){
		if ( ! current_user_can( 'edit_posts' ) ) {
			$data = array('status' => 'false', 'html'=> 'Security Issue.' );
		}else{
			$nonce = isset($_POST['nonce'])? sanitize_text_field( wp_unslash ( $_POST['nonce'] ) ):'';
			if ( wp_verify_nonce( $nonce, 'ajax-nonce' ) == 1 ) {
				$key = get_option('clickdesign_api');
