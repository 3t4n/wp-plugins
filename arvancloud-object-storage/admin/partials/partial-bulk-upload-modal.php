<?php $acs_settings	     = get_option( 'acs_settings' );?>
<form method="post"
    ...
    <div id="obs-form-acceptance" class="obs-modal-wrapper">
        <div class="obs-modal obs-modal-alert" id="form-acceptance">
            <div class="obs-modal-title">
                <p class="obs-modal-desc"><?php _e('Caution', 'arvancloud-object-storage'); ?></p></div>
            ...
            <div class="obs-modal-desc"><?php _e('Upload all files to bucket?', 'arvancloud-object-storage'); ?></div>
            <div class="obs-modal-desc-small"><?php _e('All files in your local will be uploaded to bucket', 'arvancloud-object-storage'); ?></div>

            <div class="obs-modal-desc-small"><?php _e("Don't forget to make a backup of your data first. This operation is irreversible!", 'arvancloud-object-storage'); ?></div>

            <details>
                <summary class="obs-modal-summary" style="cursor:pointer;"><?php _e('Filter upload files', 'arvancloud-object-storage'); ?></summary>
                <div style="display:flex !important;flex-direction: column;">
                <div style="display:inline-block;width:100%;padding:0 5px;margin-top:-25px;">
                        <p><?php _e('Filter file Extention', 'arvancloud-object-storage'); ?></p>
                        <textarea name='file_ext' class="tagify w-100" style="padding:0 2px;" placeholder='File Extension'>
                    ...
                    <div style="display:inline-block;width:100%;padding:0 5px;">
                        <p><?php _e('Filter file size', 'arvancloud-object-storage'); ?></p>
                        <input class="obs-input" type="number" name="file_f_size" id="keep-local-files" size="5" placeholder="<?php  _e( 'From size', 'arvancloud-object-storage' ) ?>" value="<?php echo isset($acs_settings['file_f_size']) ? $acs_settings['file_f_size']: ''; ?>" pattern="[0-9.]+"/> 
                        <input class="obs-input" type="number" name="file_t_size" id="keep-local-files" size="5" placeholder="<?php  _e( 'To size', 'arvancloud-object-storage' ) ?>" value="<?php echo isset($acs_settings['file_t_size']) ? $acs_settings['file_t_size']: '' ?>" pattern="[0-9.]+"/>
                        <div class="obs-custom-input"><?php  _e( 'Enter limit file size as MB.', 'arvancloud-object-storage' ) ?></div>
                    </div>
                    <div style="display:inline-block;width:100%;padding:0 5px;">
                        <p><?php _e('Keep local files after uploading them to storage.', 'arvancloud-object-storage'); ?></p>
            ...
            <div class="obs-modal-confirm d-flex items-center justify-center">
                ...
                <label for="form-acceptance-accept"><?php _e('I\'m sure of bulk uploading', 'arvancloud-object-storage'); ?></label>
            </div>
            ...
            <div class="obs-modal-actions">
                ...
                <button name="submit-form-acceptance" class="obs-btn-danger disabled" id="submit-form-acceptance"
                        data-action="do_bulk_upload"
                        data-statusaction="get_bulk_upload_task_status"
                        onclick="return false;"> <?php _e('Bulk upload', 'arvancloud-object-storage'); ?> </button>
            </div>
        </div>
        ...
        <div class="obs-content-wrapper" id="task-status-modal" style="display: none">
            <div class="obs-modal obs-modal-progress">
                ...
                <div class="obs-modal-desc"><?php _e('Bulk upload is in progress', 'arvancloud-object-storage'); ?></div>
                ...
                <div class="obs-modal-actions">
                    ...
                    <button class="obs-btn-primary" onclick="return false;"
                            data-action="stop_current_bulk_upload_task"
