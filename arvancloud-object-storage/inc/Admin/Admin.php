...
class Admin {
	...
	public function upload_media_to_storage( $post_id, $force_upload = false ) {
		...
		if( $force_upload || ( is_numeric( $post_id ) && !wp_attachment_is_image( $post_id ) ) ) {
			if(  
				...
				(is_admin() and isset($_GET['page']))|| wp_is_serving_rest_request()
			) {
	...
	public function delete_media_from_storage( $id ) {
		...
		if( /*( isset( $_POST['action'] ) && $_POST['action'] == 'delete-post' || $_POST['action'] == 'image-editor' ) &&*/ $this->is_attachment_served_by_storage( $id ) ) {
            ...
            $client  = S3Singletone::get_instance()->get_s3client();
			$credent = Helper::get_storage_settings();
			$bucket  = get_post_meta($id,'acs_storage_file_url',true);
			if(empty($bucket))
				return;
			
			$bucket  = substr($bucket,8,-(strlen($credent['endpoint-url'])-6));
			$fpath  = get_post_meta($id,'acs_storage_file_dir',true);
			if ( wp_attachment_is_image( $id ) ) {
				$args = wp_get_attachment_metadata( $id );

				$client->deleteObject ([
					'Bucket' => $bucket, 
					'Key' 	 => $fpath.basename( $args['file'] )
				]);
				...
				if ( $args && array_key_exists( "sizes", $args ) ) {
					foreach ( $args['sizes'] as $list_file ) {
						if ( $list_file['file'] != "" ) {
							$client->deleteObject ([
								'Bucket' => $bucket, 
								'Key' 	 => $fpath.basename( $list_file['file'] )
							]);
						}
					}
				}
	...
	public function calculate_image_srcset( $sources, $size_array, $image_src, $image_meta, $attachment_id ) {
		...
		foreach ( $sources as $key => $source ) {
			if ( wp_attachment_is_image( $attachment_id ) ) {
				...
				if ( !empty( $cdn ) ) {
					$source['url'] = str_replace( trailingslashit( $uploads ), trailingslashit( $cdn ), $source['url'] );
				}
			}
	...
	public function media_library_url_rewrite( $url, $attachment_id ) {
		...
		if( !empty( $storage_file_url )  and basename($filename) == basename($url)) {
    ...
    public function check_valid_attachment($attach_id){
		...
		if(isset($_GET['page']) and $_GET['page']=='wp-arvancloud-storage-download')
		return false;

		if($_POST['type'] == 'downloadable_product' and (empty($opt) or !empty($opt['wooc_prev_upload_product'])) )
		return false;

        return apply_filters('arva_prevent_upload', true);
    }
