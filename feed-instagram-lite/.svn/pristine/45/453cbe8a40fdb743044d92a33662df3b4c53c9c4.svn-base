<?php
/*
Plugin Name: GhozyLab Instagram Feed
Plugin URI: https://demo.ghozylab.com/plugins/instagram-feed-plugin/
Description: GhozyLab Instagram Feed - Display your Instagram photos anywhere you like. You can quickly customize your Instagram images to look exactly the way you want them to look.
...
define( 'IFLITE_CLIENT_ID', '1693290714581130' );
...
if ( ! defined( 'IFLITE_VERSION' ) ) {
    define( 'IFLITE_VERSION', '1.0.0.33' );
}

// PHP Version
if ( version_compare( PHP_VERSION, '7.1', '>' ) ) {
    define( 'IFLITE_PHP7', true );
} else {
    define( 'IFLITE_PHP7', false );
}
...
if ( ! defined( 'IFLITE_URL_FILE' ) ) {
    define( 'IFLITE_URL_FILE', plugin_dir_url( __FILE__ ) );
}

// All Filters & Actions
add_action( 'plugins_loaded', 'gifeed_run_first' );
...
function gifeed_run_first()
{

    include dirname( __FILE__ ) .'/inc/admin/gifeed-widget.php';

    if ( gifeed_is_gutenberg() ) {
        include_once plugin_dir_path( __FILE__ ).'inc/admin/gifeed-block/init.php';
    }

}

function gifeed_is_gutenberg()
{
...
function gifeed_settings_link( $link, $file )
{
    ...
    if ( $file == $this_plugin ) {
        $settings_link = '<a href="'.admin_url( 'edit.php?post_type=ginstagramfeed&page=ghozylab-instagram-settings' ).'"><span class="fil_settings_icon dashicons dashicons-admin-generic"></span>&nbsp;'.__( 'Settings', 'feed-instagram-lite' ).'</a>';
...
function gifeed_global_init()
{
    ...
    include dirname( __FILE__ ).'/inc/functions/endpoints/gifeed-ig-endpoints.php';
    include dirname( __FILE__ ).'/inc/functions/endpoints/gifeed-data-parser.php';
    ...
    if ( is_admin() ) {
        ...
        include dirname( __FILE__ ).'/inc/admin/tinymce_plugin/register_mce_button.php';
...
function fil_current_screen( $current_screen )
{

    if ( 'plugins' == $current_screen->id ) {
        add_action( 'admin_head', 'fil_custom_admin_head' );
    }

}

add_action( 'current_screen', 'fil_current_screen' );

function fil_custom_admin_head()
{

    echo '<style>.fil_settings_icon:before {font-size: 20px !important;background-color: rgba(0, 0, 0, 0) !important;padding: 0 !important;box-shadow: none !important;width: 20px !important;height: 20px !important;}.fil_settings_icon.dashicons {width: 20px !important;height: 20px !important;padding: 0 !important;position: relative;top: 1.3px;}</style>';
