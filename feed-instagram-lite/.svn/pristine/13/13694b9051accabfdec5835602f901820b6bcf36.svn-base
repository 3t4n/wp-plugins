jQuery(document).ready(function ($) {
	...
	jQuery("body").delegate("#ifeed_gut_shorcode-button, #ifpicons_gut_shorcode", "click", function () {
	...
	function grabFeed() {
		...
		FeedList.find('option').remove();
		$("<option/>").val('select').text('- Select Feed -').appendTo(FeedList);

		$.each(gifeed_tinymce_vars.feeds, function (i, option) {
			$("<option/>").val(option.id).text(option.name).appendTo(FeedList);
		});
