...
function gifeed_opt( $name )
{
    ...
    if ( is_array( $gifeed_values ) && array_key_exists( $name, $gifeed_values ) ) {
        return $gifeed_values[$name];
    }
...
function gifeed_def_opt()
{
    ...
    if ( ! is_array( $opt ) ) {
        ...
        $options = array(
            'access_token' => array(),
            'users' => array(),
            'gifeed_google_fonts_api_key' => '',
            'gifeed_google_fonts_short' => 'none',
            'gif_instagram_opt_autoupdate' => 'active',
            'gif_instagram_is_lite' => 'yes',
        ...
        if ( isset( $opt['gif_instagram_opt_autoupdate'] ) && $opt['gif_instagram_opt_autoupdate'] == '' ) {
    ...
    if ( ! is_network_admin() && empty( gifeed_opt( 'users' ) ) ) {
...
function gfeed_get_user_data( $uid )
{

    $user_data = gifeed_opt( 'users' );

    if (  ( isset( $user_data ) && empty( $user_data ) ) || ! is_array( $user_data ) || ! isset( $user_data[$uid] ) ) {
...
function gfeed_is_graph_ready( $uid, $giveInfo = false )
{

    $user_data = gifeed_opt( 'users' );

    if ( ! isset( $user_data ) || ! isset( $user_data[$uid] ) ) {

        if ( $giveInfo ) {
            return (object) array( 'status' => false, 'id' => '', 'access_token' => '' );
        }
	...
	// For Ready Graph API
    if ( isset( $user_data[$uid] ) && isset( $user_data[$uid]['dataGraphAPI'] ) ) {

        if ( $giveInfo ) {
            return (object) array( 'status' => true, 'id' => $user_data[$uid]['dataGraphAPI']['ig_user_id'], 'access_token' => $user_data[$uid]['dataGraphAPI']['access_token'] );
        }
	...
	// For Personal / Business without Graph API
    if ( isset( $user_data[$uid] ) && ! isset( $user_data[$uid]['dataGraphAPI'] ) ) {

        if ( $giveInfo ) {
            return (object) array( 'status' => false, 'id' => $user_data[$uid]['id'], 'access_token' => $user_data[$uid]['access_token'] );
        }
