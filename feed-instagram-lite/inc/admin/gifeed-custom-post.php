...
function ginstagramfeed_columns_edit_columns_list( $gifeed_columns, $post_id ){
	...
	switch ( $gifeed_columns ) {
		...
		case 'gifeed_type':
			...
			$feeds = get_post_meta( $post_id, 'gifeed_meta_ids_tags', true );
	
			if ( isset( $feeds ) && ! empty( $feeds ) ) {
	
				foreach ( $feeds as $itm => $indx ) {
	
					if ( $indx !== '' )
					// Changed after migrate to Basic Display API ( token to id )
					{
						echo '<p style="margin-top:5px;"><span class="dashicons '.( strpos( $indx, '#' ) !== false ? 'dashicons-tag' : '
			dashicons-businessman' ).'" style="margin-right:1px;"></span><strong>'.gifeed_id_to_username( $indx ).'</strong> '.( strpos( $indx, '#' ) !== false ? 'hashtag' : 'token' ).'</p>';
					} else {
						echo 'none';
					}
		...
		echo '<a class="gifeed_tooltips" alt="Edit Feed" href="'.get_edit_post_link( $post_id ).'"><span class="dashicons dashicons-edit gifeed_form_actions"></span></a>'.( current_user_can('edit_posts') ? '<a class="gifeed_tooltips" alt="Duplicate Feed" href="'.admin_url( 'admin-ajax.php?action=gifeed_duplicate_feed&amp;post=' . $post_id . '&amp;nonce='.wp_create_nonce( 'gifeed-duplicate-nonce' ).'"><span class="dashicons dashicons-admin-page gifeed_form_actions"></span></a>' ) : '').'<a class="gifeed_tooltips" alt="Preview" href="'.admin_url( 'admin-ajax.php?action=gifeed_generate_preview&security='.wp_create_nonce( 'gifeed_preview-nonce' ).'&feed_id='.$post_id.'&prev_type=review' ).'" target="_blank"><span class="dashicons dashicons-desktop gifeed_form_actions"></span></a><a class="gifeed_tooltips delfeed" alt="Delete Feed" href="'.( isset( $_GET['post_status'] ) && $_GET['post_status'] == 'trash' ? get_delete_post_link( $post_id, '', true ) : get_delete_post_link( $post_id ) ).'"><span class="dashicons dashicons-trash gifeed_form_actions"></span></a>';
