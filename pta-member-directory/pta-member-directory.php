<?php
/*
Plugin Name: Member Directory and Contact Form
...
Creates a default template for viewing directory on the public side.
...
if (!defined('PTA_MEMBER_DIRECTORY_VERSION_NUM'))
    define('PTA_MEMBER_DIRECTORY_VERSION_NUM', '1.8.0');
...
function pta_member_directory_init() {
	...
	$args = array(
		...
		'description'         => __( 'Board Members and Chairs', 'pta-member-directory' ),
		'labels'              => $labels,
	...
	$defaults = array(
				...
				'cell_padding' => '5',
				'enable_cfdb' => false,
				'form_title' => __('Member Directory Contact Form','pta-member-directory'),
 ...
 * @param object $post The Wordpress post Object
 *
 * @return  int     returns post_id if any errors, otherwise just saves the post and taxonomies
 */
 ...
 * alters the query if it includes our custom taxonomy
 */
 ...
 *
 * @return string The renamed (if needed) title text input
 */
 ...
 * @param array $options
 *
 * @return string HTML options form
 */
 ...
 * Calls and Processes the options page form
 */
function pta_directory_settings_page() {
	...
	<h2><?php _e('Member Directory - Options', 'pta-member-directory'); ?></h2>
...
function pta_directory_sort_page() {
	...
	<div class="wrap">
	<div id="icon-users" class="icon32"><br/></div>
	<h2><?php _e('Member Directory - Sort Position Display Order', 'pta-member-directory'); ?></h2>
		<?php 
		if (!$pta_categories) { ?>
...
/**
 * Creates the WordPress Help tabs within the Members pages
 */
...
function pta_save_member_categories() {
	$categories = get_option( 'pta_member_categories', array() );
	$args = array( 'hide_empty' => false, );
...
function pta_md_plugin_action_links( $links ) {
...
/**
 * Activation function - creates PTA Manager role
 */
