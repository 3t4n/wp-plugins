 ...
 * Description: myCred Learndash is a plugin for WordPress that enables you to build points based on Learndash plugin.
 * Version: 2.1.9
...
define('MYCRED_LEARNDASH_VERSION', '2.1.9');
...
register_activation_hook(__FILE__, 'mycred_learndash_on_activation');

// Add action for displaying admin notices
add_action('admin_notices', 'mycred_learndash_display_activation_notice');
 ...
 * @version 1.0
 */
function mycred_learndash_on_activation() {
    // Set a transient to display the activation notice
    set_transient('mycred_learndash_activation_notice', true, 5);
}
 ...
 * @version 1.0
 */
function mycred_learndash_display_activation_notice() {
    // Check if the transient is set
    if (get_transient('mycred_learndash_activation_notice')) {
        echo '<div class="notice notice-error is-dismissible">';
        echo wp_kses_post(
            __('<p><strong>myCred Learndash</strong> is depreciating from here and it will not be updated from now on. You can access <strong>myCred Learndash</strong> along with all the future updates on the <strong>myCred Toolkit</strong>.</p>', 'mycred_learndash')
        );
        echo '</div>';

        // Delete the transient to prevent repeated notices
        delete_transient('mycred_learndash_activation_notice');
