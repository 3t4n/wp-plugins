		...
		public function navigate_prev_post() {
			...
			if ( is_a( $previous_post, 'WP_Post' ) ) {
				if ( $is_classic_editor ) {
					echo wp_json_encode( admin_url( 'post.php?post=' . $previous_post->ID . '&action=edit&classic-editor&classic-editor__forget' ) );
				} else {
					echo wp_json_encode( admin_url( 'post.php?post=' . $previous_post->ID . '&action=edit&classic-editor__forget' ) );
				}
				exit;
			}

			echo wp_json_encode( $previous_post );
			exit;
		}
	}
