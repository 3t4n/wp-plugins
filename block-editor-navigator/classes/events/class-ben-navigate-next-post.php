		...
		public function navigate_next_post() {
			...
			if ( is_a( $next_post, 'WP_Post' ) ) {
				if ( $is_classic_editor ) {
					echo wp_json_encode( admin_url( 'post.php?post=' . $next_post->ID . '&action=edit&classic-editor&classic-editor__forget' ) );
				} else {
					echo wp_json_encode( admin_url( 'post.php?post=' . $next_post->ID . '&action=edit&classic-editor__forget' ) );
				}
				exit;
			}

			echo wp_json_encode( $next_post );
			exit;
		}
	}
