...
*/
add_action('admin_menu', 'wpdocc_admin_actions');
function wpdocc_admin(){
    require('wpdocc_admin.php');
    exit;
}
function wpdocc_admin_actions(){
    add_options_page('Dispaly OpenCart Category', 'Dispaly OpenCart Category', 1, 'DispalyOpenCartCategory', 'wpdocc_admin');
}
...
function oc_get_category_link($category_id) {
	global $ocdb, $ocdb_prefix, $oc_store_url; $oc_seo_enable;
	$route = 'category_id=' . $category_id;
	$url = 	$oc_store_url;
	if($oc_seo_enable=='Yes'){
