...
class Wizit_API {
	...
	private function parse_api_response( $response) {
		...
		if ( '200' == $responsecode ) {
			...
			$errormessage = "Error: API Response Code: ". print_r($responsecode, true) .". Some error occured while calling Wizit API!!";
		}
	...
	public function call_limit_api( $apikey = null, $api_url = null) {
		...
		if ( '' == $apiresult) {
			...
			$error = true;
			$errormessage = 'Call Transaction Limit Error: ' . print_r( $apiresult, true ); 
			$this->save_api_error($errormessage);
			$apiresult = $errormessage;
			$this->log->add( 'Wizit', sprintf( 'failure : %s', print_r( $apiresult, true ) ) );
		} else {
			$error = true;
			$errormessage = 'Error: ' . print_r( $apiresult, true ); 
			$this->save_api_error($errormessage);
			$apiresult = $errormessage;
			$this->log->add( 'Wizit', sprintf( 'failure : %s', print_r( $apiresult, true ) ) );
		}
	...
	public function get_order_payment_status_api( $apikey, $requestbody) {
		...
		if ( false !== $apiresult && is_array($apiresult) && '200' == $apiresult['responseCode'] ) {
			...
			$error = true;
			$errormessage = 'Order Status Error: ' . print_r( $apiresult, true ); 
			$this->save_api_error($errormessage);
			$apiresult = $errormessage;
		} elseif(is_array($apiresult)){
			$error = true;
	...
	public function handle_order_payment_status_api_error( $apiresult, $errormessage) {
		...
		if ('AUTH_APPROVED' != $apiresult['paymentStatus'] && 'CAPTURED' != $apiresult['paymentStatus']  && 'PARTIALLY_CAPTURED' != $apiresult['paymentStatus'] ) {
			...
			if ('AUTH_DECLINED' == $apiresult['paymentStatus'] ) { 
				$errormessage = 'Wizit Payment Failed. Wizit Transaction ' . $apiOrderId . ' has been Declined'. print_r( $apiresult, true );
			} elseif ('CAPTURE_DECLINED' == $apiresult['paymentStatus'] ) { 
				$errormessage = 'Wizit Transaction Wizit Transaction ' . $apiOrderId . ' Capture Attempt has been declined' . print_r( $apiresult, true );
			} elseif ('VOIDED' == $apiresult['paymentStatus'] ) { 
				$errormessage = 'Wizit Transaction ' . $apiOrderId . ' VOID'. print_r( $apiresult, true );
			} else {
				$errormessage = 'Wizit Transaction ' . $apiOrderId . ' Payment Failed.' . print_r( $apiresult, true );
			}
			$this->save_api_error($errormessage);
			$error = false;  
		}
	...
	public function immediate_payment_capture( $apikey, $requestbody) {
		...
		if ( false !== $apiresult && '200' == $apiresult['responseCode'] ) {
			...
			$error = true;
			$errormessage = 'Immediate Payment Capture Error: ' . print_r( $apiresult, true );
			$this->save_api_error($errormessage);
			$apiresult = $errormessage;
		} else {
			$error = true;
	...
	public function handle_immediate_payment_capture_error( $apiresult, $errormessage) {
		...
		if ('3005' == $apiresult['errorCode'] ) {
			...
			$errormessage = print_r( $apiresult, true ); //'Wizit Payment Failed. Wizit Transaction ' . $apiOrderId . ' Reason: ' . $apiresult['errorMessage'];
			$this->save_api_error($errormessage);
			$error = false;
		} 
		...
		if ('AUTH_APPROVED' != $apiresult['paymentStatus'] && 'CAPTURED' != $apiresult['paymentStatus'] && 'CAPTURE_DECLINED' != $apiresult['paymentStatus']) {
			...
			$orderMessage = '';
			if ('AUTH_DECLINED' == $apiresult['paymentStatus'] ) { 
				$errormessage = print_r( $apiresult, true );//'Wizit Payment Failed. Wizit Transaction ' . $apiOrderId . ' has been Declined';
			/*} elseif ('CAPTURE_DECLINED' == $apiresult['paymentStatus'] ) { 
				$errormessage = 'Wizit Transaction ' . $apiOrderId . ' Capture Attempt has been declined';
			*/
			} elseif ('VOIDED' == $apiresult['paymentStatus'] ) { 
				$errormessage =  print_r( $apiresult, true );//'Wizit Transaction ' . $apiOrderId . ' VOID';
			} else {
				$errormessage = print_r( $apiresult, true );//'Wizit Transaction ' . $apiOrderId . ' Payment Failed.';
			}
			$this->save_api_error($errormessage);
			$error = false;  
		}
	...
	public function order_partial_capture_api( $apikey, $requestbody, $apiOrderId) {
		...
		if ( false !== $apiresult && '200' == $apiresult['responseCode'] ) {
			...
			$error = true;
			$errormessage = 'Partial Payment Capture Error: ' . print_r( $apiresult, true );
			$this->save_api_error($errormessage);
			$apiresult = $errormessage;
		} else {
			$error = true;
