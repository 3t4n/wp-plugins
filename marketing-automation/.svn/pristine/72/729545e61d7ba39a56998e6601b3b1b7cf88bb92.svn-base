...
class Vbout {
	...
	public function safeCall($name, $args)
    {
		...
		if(defined('VBOUT_PLUGIN_API_URL')) {
			$request_url = VBOUT_PLUGIN_API_URL . trim($this->api_url . $name, '/') . '&' . http_build_query($params) . '&';
		}
		...
		if ( is_array($response) && isset($response['response']['header']['status']) ) {
			if ($response['response']['header']['status'] == 'error') {
				throw new VboutException($response['response']['data']);
			}
		}
		
		if ( $status != 200 ) {
			throw new VboutException(array(
				'errorMessage'=> $error,
			));
		}
