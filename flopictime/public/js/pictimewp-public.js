...
$("body").on("flo_pt_init_slider", function (event) {
  // used on the back end side in the Gutenberg block, when the block content is ready a little later
  flo_init_pt_masonry_grid();
  flo_pt_view_lazy_load();
});
...
function flo_init_pt_masonry_grid() {
  // grid view - masonry view
  jQuery('.packery-main-wrap').each(function (index, value) {
    var gutterNumeric = parseInt($(this).attr('data-gutter')),
        nrColumns = $(this).attr('data-nrcolumns'),
        itemPositions = $(this).attr('data-positions'),
        containerWidth = $(this).outerWidth();

    if (containerWidth >= 768) {
      var this_packery = new Packery($(this).data('gallery_id'), {
        itemSelector: ".packery-photo-wrap",
        shiftPercentResize: true,
        percentPosition: true,
        initLayout: false,
        gutter: gutterNumeric
      });
      setTimeout(function () {
        imagesLoaded($(this).data('gallery_id'), setSavedPositions(this_packery, $(this).data('gallery_id')));
      }, 100);
    } else {
      var this_packery_mob = new Packery($(this).data('gallery_id'), {
        itemSelector: ".packery-photo-wrap",
        // shiftPercentResize: true,
        // percentPosition: true,
        //initLayout: true,
      ...
      this_packery_mob.layout();
    }
  }); // grid view - PicTime view

  resetPictimeViewHeight();
  initPicTimeMobileView();
...
/**
 *
 * for the mobile view we will use the masonry 2 columns
 *
 */


function initPicTimeMobileView() {
  // PicTime view - masonry view for mobile
  jQuery('.grid-main-wrap.pictime-view').each(function (index, value) {
    if ($(window).width() < 768) {
      var this_pictime_mob = new Packery($(this).data('gallery_id'), {
        itemSelector: ".photo-wrap",
        gutter: 4
      });
      this_pictime_mob.layout();
    }
  });
}
/**
 * PicTime View:
 * We need to adjust the grid container height.
 * Initially we have the height and the width set in the dashboard which depends on the window width
 * used in the dashboard at the moment the gallery was saved.
 * Then we look at the current container width and adjust the height proportionally
 *
 */


function resetPictimeViewHeight() {
  var current_container_width = jQuery('.grid-main-wrap.pictime-view').innerWidth(),
      dashboar_container_width = jQuery('.grid-main-wrap.pictime-view').data('container_width') - 2 * jQuery('.grid-main-wrap.pictime-view').data('container_padding_x'),
      dashboar_container_height = jQuery('.grid-main-wrap.pictime-view').data('container_height'),
      adjusted_container_height = parseFloat(dashboar_container_height) * current_container_width / dashboar_container_width; // set the adjusted height to the grid container

  if (jQuery(window).width() < 768) {
    jQuery('.grid-main-wrap.pictime-view').css({
      '--height': adjusted_container_height + 'px'
    });
  } else {
    jQuery('.grid-main-wrap.pictime-view').css({
      'height': adjusted_container_height + 'px'
...
/**
 * Pictime View:
 * Lazy loading background images using IntersectionObserver
 *
 */
...
function flo_pt_view_lazy_load() {
  var lazyloadImages; // check if IntersectionObserver is supported

  if ("IntersectionObserver" in window) {
    console.log('Observer in window'); // we work with elements having class: selected-photo-as-bg & lazy

    lazyloadImages = document.querySelectorAll(".selected-photo-as-bg.lazy");
    var imageObserver = new IntersectionObserver(function (entries, observer) {
      entries.forEach(function (entry) {
        //console.log('entry.intersectionRatio: ', entry.intersectionRatio, entry.isIntersecting);
...
function setSavedPositions(this_packery, gallery_id) {
  var itemPositions = $(gallery_id).data('positions');

  if (itemPositions && itemPositions.length) {
    this_packery._resetLayout(); // if saved positions are available, set item order and horizontal position from saved positions


    this_packery.items = itemPositions.map(function (itemPosition) {
      //let itemElem = $($(this).data('gallery_id')).querySelector(`[data-id="${itemPosition.id}"]`)
      var itemElem = document.querySelector("[data-id=\"".concat(itemPosition.id, "\"]"));
      var item = this_packery.getItem(itemElem);

      if (typeof item !== 'undefined') {
        item.rect.x = parseFloat(itemPosition.x) * this_packery.packer.width;
        return item;
      }
    });
    this_packery.shiftLayout();
  } else {
...
!function (t, e) {
  ...
  "function" == typeof define && define.amd ? define("packery/js/rect", e) : "object" == (typeof module === "undefined" ? "undefined" : _typeof(module)) && module.exports ? module.exports = e() : (t.Packery = t.Packery || {}, t.Packery.Rect = e());
}(window, function () {
  "use strict";
  ...
  if ("function" == typeof define && define.amd) define("packery/js/packer", ["./rect"], e);else if ("object" == (typeof module === "undefined" ? "undefined" : _typeof(module)) && module.exports) module.exports = e(require("./rect"));else {
  ...
  "function" == typeof define && define.amd ? define("packery/js/item", ["outlayer/outlayer", "./rect"], e) : "object" == (typeof module === "undefined" ? "undefined" : _typeof(module)) && module.exports ? module.exports = e(require("outlayer"), require("./rect")) : t.Packery.Item = e(t.Outlayer, t.Packery.Rect);
}(window, function (t, e) {
  "use strict";
  ...
  "function" == typeof define && define.amd ? define(["get-size/get-size", "outlayer/outlayer", "packery/js/rect", "packery/js/packer", "packery/js/item"], e) : "object" == (typeof module === "undefined" ? "undefined" : _typeof(module)) && module.exports ? module.exports = e(require("get-size"), require("outlayer"), require("./rect"), require("./packer"), require("./item")) : t.Packery = e(t.getSize, t.Outlayer, t.Packery.Rect, t.Packery.Packer, t.Packery.Item);
}(window, function (t, e, i, n, o) {
  "use strict";
...
function flo_init_pt_slider() {
  jQuery('.pt-slideshow-view').each(function (index, value) {
    var loop_val = parseInt($(this).attr('data-loop')) == 1,
        nav_style = $(this).attr('data-nav_style'),
        slider_style = $(this).attr('data-slider_style'),
    ...
    if (slider_style == 'visible_nearby') {
      ...
      if (!loop_val) {
        // if infinite loop is disabled, we set the initial slide, the 2nd one
        settings.initialSlide = 1;
      }

      settings.prevArrow = jQuery('.pt-slider-nav.pt_nav_' + gallery_id + ' .pt-slick-prev');
      settings.nextArrow = jQuery('.pt-slider-nav.pt_nav_' + gallery_id + ' .pt-slick-next'); // add helper CSS var to be used in css

      jQuery('.pt-slideshow-view').attr('style', ' --container-width: ' + jQuery(".pt-slideshow-view").width() + 'px');
    }

    if (slider_style == 'single_image' && transition == 'fade') {
      settings.fade = true;
    }

    if (slider_style == 'single_image') {
      jQuery('.g_' + gallery_id).on('init', function (event, slick) {
        // On init we will preload the last image to be ready in case the user slides to the left
        if (slick.$slides !== null) {
          var last_img = slick.$slides[slick.slideCount - 1],
              last_img_src = $(last_img).find('img').data('lazysrc');
          jQuery(last_img).find('img').attr('src', last_img_src);
        }
      });
    }

    jQuery('.g_' + gallery_id).slick(settings); // custom lazy loading for 'single_image' slider

    if (slider_style == 'single_image') {
      jQuery('.g_' + gallery_id).on('beforeChange', function (event, slick, currentSlide, nextSlide) {
        var dir;
        var activeSlide = jQuery('.slick-slide.slick-active'); //detect the direction the user slides the slider :
        ...
        var next_img, next_next_img; // depending on the direction init properly the next 2 images objects

        if (dir == 'right') {
          next_img = activeSlide.next();
          next_next_img = activeSlide.next().next();
        } else {
          next_img = activeSlide.prev();
          next_next_img = activeSlide.prev().prev();
        }

        if (next_img.attr('lazyloaded') != 1) {
          var nex_img_src = next_img.find('img').data('lazysrc');
          next_img.find('img').attr('src', nex_img_src);
          next_img.attr('lazyloaded', 1);
        }

        if (next_next_img.attr('lazyloaded') != 1) {
          var next_next_img_src = next_next_img.find('img').data('lazysrc');
          next_next_img.find('img').attr('src', next_next_img_src);
          next_next_img.attr('lazyloaded', 1);
        }

        if (slick.slickCurrentSlide() == 0) {
          // if we are on the first slide, we will preload the image before last
          // because the code above does not work when gowing from first to last image
          if (slick.$slides !== null) {
            var last_img_2 = slick.$slides[slick.slideCount - 2],
                last_img_2_src = $(last_img_2).find('img').data('lazysrc');
            $(last_img_2).find('img').attr('src', last_img_2_src);
          }
        }
      });
    }
