...
function cts_callback_shortcode2($post){
	...
	//Daily Checkbox
	$is_daily = get_post_meta( $post->ID, 'td_is_daily', true);
	...
	?>
	<p style="color:red;font-weight:600;"><?php echo __('Al marcar la casilla se va a sobrescribir los tabs ya creados.','tdlang'); ?></p>
	<p><input type="checkbox" id="td_is_daily" name="td_is_daily" value="1" <?php if($is_daily == true){ echo "checked"; } ?> /> <?php echo __('Tab diario','tdlang'); ?></p>
	<hr>
...
function cts_callback_shortcode($post){
	echo "<script>";
	echo 'var lunes = "' . __('Lunes', 'tdlang') . '";';
	echo 'var martes = "' . __('Martes', 'tdlang') . '";';
	echo 'var miercoles = "' . __('Miercoles', 'tdlang') . '";';
	echo 'var jueves = "' . __('Jueves', 'tdlang') . '";';
	echo 'var viernes = "' . __('Viernes', 'tdlang') . '";';
	echo 'var sabado = "' . __('Sabado', 'tdlang') . '";';
	echo 'var domingo = "' . __('Domingo', 'tdlang') . '";';
	echo "</script>";
	...
	<table id="table-tabs" class="icon-manager">
		...
		<a class="button" href="#" id="button-more" style="<?php if($is_daily == true){ echo "display:none;"; } ?>" > + </a>
		<a class="button" href="#" id="button-less" style="<?php if($is_daily == true){ echo "display:none;"; } ?>" > - </a>
...
function cts_save_meta($post_id){
	...
	if(isset($_POST['tab_count'])){
		if( isset($_POST["td_is_daily"]) && $_POST["td_is_daily"] == "1"){
			$td_is_daily = true;
		}else{
			$td_is_daily = false;
		}
		...
		for($i = 0; $i < ($_POST['tab_count']); $i++){
			if( (isset($_POST['tab_' . $i . '_name']) && !empty($_POST['tab_' . $i . '_name']) ) || $td_is_daily ){

				if($td_is_daily){
					$_POST['tab_' . $i . '_name'] = $dias_semana[$i];
				}
		...
		update_post_meta( $post_id, 'td_is_daily', $td_is_daily );
