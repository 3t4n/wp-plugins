	...
	class tabManager {
		...
		function getData(){
			...
			if($this->custom || $this->isEdit){
				...
				$this->is_daily = get_post_meta( $this->id, 'td_is_daily', true);
		...
		function getMenu(){
			...
			if($this->custom || $this->isEdit){
				...
				/* DAILY CUSTOM TABS */
				if($this->is_daily == true){

					$this->print .= '<div id="' . $this->id . '" class="tab-menu-container" data-type="days" data-color="' . esc_attr($this->colors["td_active"]). '">';

					/* DROPDOWN MENU */
					$this->print .= '<select id="' . $this->id . '" style="display:none;" class="tab-menu-dropdown" >';
					$i=1;
					$z=0;
					$icons = array_values($this->icons);
					foreach($this->titles as $title){
						if($i == 7){ $i = 0;}
						$this->print .= '<option id="' . $i .'" value="' . $i .'" class="tab-menu-option"> <span id="' . $i .'" class="tab-menu-dropdown-selector dashicons-before ' . esc_attr($icons[$z]) . '" > ' . esc_attr($title) . ' </span></option>';
						$i++;
						$z++;
					}
					$this->print .= '</select>';
					...
					/* NORMAL MENU */
					$this->print .= '<ul class="tab-menu-ul" id="tab-menu-ul">';
					$i=1;
					$z=0;
					$icons = array_values($this->icons);
					foreach($this->titles as $title){
				...
				/* NORMAL CUSTOM TABS */
					...
					$this->print .= '<div id="' . $this->id . '" class="tab-menu-container" data-color="' . esc_attr($this->colors["td_active"]). '">';
					...
					foreach($this->data as $d){
						$this->print .= '<option id="' . $i .'" value="' . $i .'" class="tab-menu-option"> <span id="' . $i .'" class="tab-menu-dropdown-selector dashicons-before ' . esc_attr($this->data[$i]["ICON"]) . '" > ' . esc_attr($d["NAME"]) . ' </span></option>';
						$i++;
					}
					$this->print .= '</select>';
		...
		function getStyles(){
			...
			$this->print .= '<style id="css-dropdown">';

			$this->print .= '[id="' . $this->id . '"].tab-menu-dropdown option, [id="' . $this->id . '"].tab-menu-dropdown{ background-color: ' . esc_attr($this->colors["td_background"]). ' !important; }';
			$this->print .= '[id="' . $this->id . '"].tab-menu-dropdown option, [id="' . $this->id . '"].tab-menu-dropdown{ color: ' . esc_attr($this->colors["td_text"]). ' !important; }';
