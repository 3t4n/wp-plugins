...
class Dp_Intro_Tours_Public
{
				...
				if (is_array($related_url_list)) {
					...
					if ($step_idx >= 0 && !array_key_exists($tour_id, $tours_to_inc)) {
						...
						$tours_to_inc[$tour_id] = [
							...
							'url_vars' => ($step_idx < $start_url_cnt || $is_start_at_all_pages_tours)
	...
	protected function get_trigger_config($id, $triggerId)
	{
		...
		$first_user_visit = intval(Arr::get($trigger, 'first_n_user_visit', '1'));

		$res = [
			'first_user_visit_only' => Arr::get($trigger, 'first_user_visit_only', false),
			'allow_future_visits_checkbox' => Arr::get($trigger, 'allow_future_visits_checkbox', true),
			'first_n_user_visit' => max(1, $first_user_visit),
			'visit_count_mode' => Arr::get($trigger, 'visit_count_mode', 'server'),
		...
		if ($tours_to_inc && count($tours_to_inc)) {
			...
			$labeling_options = Dp_Intro_Tours_Helper::get_options_array(
				...
				[
					...
					'skipForFutureVisitsLabel' => Dp_Intro_Tours_Helper::get_generic_i18n('skip-for-future-visits'),
				]
			);
					...
					if (isset($step_data)) {
						...
						if ($shouldLoadScript) {
							...
							if (!$this->builder_mode && $current_user_id) {
								$logged_in_visit_cnt = Dp_Intro_Tours_Visit_Count::get_visit_count($current_user_id, $tour_id);
							}
							...
							$tourCfg = $this->filter_tour_script_config(
								[
									...
									'visitsLsKeyVersion' => Dp_Intro_Tours_Visit_Count::get_visits_ls_key_version($tour_id),
									'theme' => $theme,
	...
	protected function get_builder_config($tour_id = 0)
	{
		...
		return [
			...
			'i18n' => [
				...
				'skip_future_visits_in_builder' => __('Skipping this tour for future visits does not work in build mode.', 'dp-intro-tours'),
