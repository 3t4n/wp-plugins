...
class Dp_Intro_Tours
{
	...
	private function load_dependencies()
	{
		...
		include_once plugin_dir_path($pluginDir) . 'includes/enums/class-dpit-builder-origin.php';
		include_once plugin_dir_path($pluginDir) . 'includes/enums/class-dpit-builder-target-type.php';
		...
		include_once plugin_dir_path($pluginDir) . 'includes/class-dp-intro-tours-post-type.php';
		...
		if (DP_INTRO_TOURS_IS_PRO) {
			...
			include_once plugin_dir_path($pluginDir) . 'PRO/class-dp-intro-tours-wp-link.php';
		...
		include_once plugin_dir_path($pluginDir) . 'includes/dpit-acf-include.php';
		include_once plugin_dir_path($pluginDir) . '/admin/class-dp-intro-tours-acf-field-table.php';
		include_once plugin_dir_path($pluginDir) . 'includes/dpit-acf-config-functions.php';
	...
	{
		$plugin_post_type_registrar = new Dp_Intro_Tours_Post_Type();
		$plugin_upgrade             = new Dp_Intro_Tours_Upgrade();

		$this->loader->add_action('init', $plugin_post_type_registrar, 'register_intro_tour', 5);
		$this->loader->add_action('init', $this, 'init_acf');

		if (!wp_doing_ajax()) {
			$this->loader->add_action('init', $plugin_upgrade, 'init', 7);
		}
		...
		$this->loader->add_filter('mce_external_plugins', $this, 'custom_tinymce_plugins', 999999);
		$this->loader->add_action('admin_footer', $this, 'render_tiny_mce');
		$this->loader->add_action('wp_footer', $this, 'render_tiny_mce');
		$this->loader->add_filter('tiny_mce_before_init', $this, 'show_tiny_mce_toolbar_expanded');

		if (DP_INTRO_TOURS_IS_PRO) {
			$is_admin = is_admin();
			if ($is_admin || $builder_mode) {
				$dp_wp_link = new Dp_Intro_Tours_Wp_Link($builder_mode);
				$this->loader->add_action('wp_ajax_dp-wp-link-ajax', $dp_wp_link, 'dp_wp_link_ajax', 10, 0);

				$this->loader->add_action($is_admin ? 'admin_footer' : 'wp_footer', $dp_wp_link, 'wp_link_dialog');
			}
		}
	}
	...
	private function define_admin_hooks($plugin_public, $builder_mode)
	{
		...
		if (DP_INTRO_TOURS_IS_PRO) {
			...
			$this->loader->add_filter('display_post_states', $plugin_admin, 'append_custom_postfix_to_admin_title', 10, 90);
			$this->loader->add_filter('plugin_action_links_' . DP_INTRO_TOURS_PLUGIN_BASE_NAME, $adminator, 'action_links');
			$this->loader->add_action('in_plugin_update_message-dp-intro-tours-pro/dp-intro-tours.php', $plugin_admin, 'prefix_plugin_update_message');
		} else {
			...
			$this->loader->add_action('in_plugin_update_message-dp-intro-tours/dp-intro-tours.php', $plugin_admin, 'prefix_plugin_update_message');
		}
		...
		$this->loader->add_action('admin_init', $plugin_settings, 'initialize_mobile_breakpoints_options');
		...
		$this->loader->add_action('wp_print_footer_scripts', $plugin_admin, 'delete_dpit_builder_transient');
		...
		$this->loader->add_action('acf/input/admin_head', $plugin_admin, 'edit_form_acf_admin_head');

		$this->loader->add_filter('acf/update_value/name=intro_steps', $plugin_admin, 'on_update_intro_steps_field', 10, 2);
		$this->loader->add_filter('acf/load_value/name=intro_steps', $plugin_admin, 'on_load_intro_steps_field');
		...
		$this->loader->add_action('acf/render_field', $plugin_admin, 'add_content_after_specific_acf_field');
		...
		// One column layout
		$this->loader->add_filter('screen_layout_columns', $plugin_admin, 'adjust_screen_layout_selection_to_one_column_only');
		$this->loader->add_filter('get_user_option_screen_layout_dp_intro_tours', $plugin_admin, 'adjust_screen_layout_to_to_one_column');
	}
	...
	public function render_tiny_mce()
	{
		...
		$admin_tour_en = Settings::get_setting_array_bool('dp_it_general_options', 'admin_board_tour_en', '0') && DP_INTRO_TOURS_IS_PRO;
		if (!is_admin() && $builder_mode === Dpit_Builder_Target_Type::STEP) {
			Dp_Intro_Tours_Helper::render_tiny_mce();
		} elseif (is_admin() && Dp_Intro_Tours_Helper::is_singular_intro_tour()) {
			wp_enqueue_style('editor-buttons');
			Dp_Intro_Tours_Helper::render_tiny_mce();
		} elseif ($admin_tour_en && is_admin() && $builder_mode === Dpit_Builder_Target_Type::STEP) {
			wp_enqueue_style('editor-buttons');
			Dp_Intro_Tours_Helper::render_tiny_mce();
		}
	}
	...
	{
		if (is_admin() && Dp_Intro_Tours_Helper::is_dp_intro_cpt_admin() || !is_admin() && $this->builder_mode === Dpit_Builder_Target_Type::STEP) {
	...
	public function custom_tinymce_plugins($plugins)
	{
		// Add the "alignjustify" plugin to TinyMCE
		if (is_admin() && Dp_Intro_Tours_Helper::is_dp_intro_cpt_admin() || !is_admin() && $this->builder_mode === Dpit_Builder_Target_Type::STEP) {
			$plugins['alignjustify'] = plugin_dir_url(__FILE__) . 'assets/js/dpit-mce-align-justify.js';
		}
		return $plugins;
	}
	...
	public function show_tiny_mce_toolbar_expanded($defaults_config)
	{
		$defaults_config['wordpress_adv_hidden'] = false;
