...
class Dp_Intro_Tours_Admin_Promo {
	...
	public function get_pro_promo_texts() {
		...
		$themes          = Dp_Intro_Tours_Helper::get_themes_select_options( true, true );
		$themes_count    = $themes ? count( $themes ) : 6;
		return [
			...
			// translators: %1$s: plugin name, %2$d: themes count
			sprintf( __( 'In %1$s, you can choose from %2$d design themes, with customizable accent color, radiuses and font. Therefore, you can precisely match the vibe of your web.', 'dp-intro-tours' ), $plugin_PRO_name, $themes_count ),
			// translators: %s: plugin name
			...
			// translators: %s: plugin name
			sprintf( __( 'Would you like to offer your co-workers or those who have access to web administration interactive tutorials on how to use the WordPress admin board and all your custom integrated modules on it? In %s you can easily create a tour also on the back-end.', 'dp-intro-tours' ), $plugin_PRO_name ),
	...
	public function add_admin_notice() {
		...
		switch ( $screen->base ) {
			case 'dashboard':
				if ( ( DPIT_ASR_ENABLED && AdminPromo::is_right_time_for_ask_4_rating( 13, $a4r_notice_id, 'dp_it_basic_options' ) ) || DPIT_PROMO_DBG ) {
				...
				} elseif ( $this->is_right_time_for_pro_promo( 14, 25 ) ) {
					$this->render_random_promo_notice();
				}
				if ( DPIT_PROMO_DBG ) {
					$this->render_random_promo_notice();
				}
				Dp_Intro_Tours_Helper::try_render_create_your_first_tour_notice();
				break;
			case 'post':
			case 'edit':
				if ( $screen->post_type === 'dp_intro_tours' ) {
					if ( ( DPIT_ASR_ENABLED && AdminPromo::is_right_time_for_ask_4_rating( 8, $a4r_notice_id, 'dp_it_basic_options' ) ) || DPIT_PROMO_DBG ) {
						AdminNotice::render_ask_for_rating_notice(
							$activated_weeks,
							DP_INTRO_TOURS_NAME,
							DP_INTRO_TOURS_PRODUCT_ASK_FOR_RATING_LINK_FREE,
					...
					} elseif ( $this->is_right_time_for_pro_promo( 7, 64 ) ) {
						$this->render_random_promo_notice();
					}
					if ( DPIT_PROMO_DBG ) {
						$this->render_random_promo_notice();
	...
	public function render_random_promo_notice() {
		...
		if ( $promo_texts && count( $promo_texts ) ) {
			...
			AdminNotice::render_notice(
				...
				true,
				'', //__( '15% with coupon code: VBVHUUR9', 'dp-intro-tours' ),
				'dpit-notice',
				null,
				'dpit-notice--pro-promo'
			);
		}
	}

	public function is_right_time_for_pro_promo( $minDaysActivated, $randomChance, $cookie_name = '' ) {
		if ( DPIT_PROMO_ENABLED && AdminPromo::is_right_time_for_random( $minDaysActivated, $randomChance, 'dp_it_basic_options', $cookie_name ) ) {
			$args = [
