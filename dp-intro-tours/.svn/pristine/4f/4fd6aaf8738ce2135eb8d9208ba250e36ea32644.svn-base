			...
			if ( $parameters ) {
				...
				if ( isset( $step_data ) ) {
					...
					if ( $changes && count( $changes ) ) {
						$store_step_changes = true;
						foreach ( $changes as $stepIdx => $step_change ) {
							if ( $step_change && is_array( $step_change ) ) {
								foreach ( $allowedProps as $prop_name ) {
									if ( array_key_exists( $prop_name, $step_change ) ) {
										$propVal = $step_change[ $prop_name ];
										if ( $prop_name === 'url' ) {
											if ( $propVal !== $step_data[ $stepIdx ][ $prop_name ] ) {
												$needToRecalculateRelationships = true;
											}
										}
										$step_data[ $stepIdx ][ $prop_name ] = $propVal;
									} elseif ( ! array_key_exists( $prop_name, $step_data[ $stepIdx ] ) ) {
										$step_data[ $stepIdx ][ $prop_name ] = '';
									}
								}
							}
						}
					...
					if ( $store_step_changes ) {
						...
						foreach ( $step_data as $idx => $step ) {
							$new_url                  = esc_html( Arr::sget( $step, 'url', '' ) );
						...
						update_field( 'dpit_step_count', count( $step_data ), $tourId );
				...
				if ( $trigger_changes && count( $trigger_changes ) ) {
					foreach ( $trigger_changes as $triggerIdx => $triggerChange ) {
						if ( $triggerChange ) {
							Acf::update_group_field(
								$triggerIdx === 0 ? 'intro_trigger' : 'intro_trigger_2',
								'intro_trigger_selector',
								$triggerChange,
								$tourId
							);
						}
