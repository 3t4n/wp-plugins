...
! defined( 'WOOON_VERSION' ) && define( 'WOOON_VERSION', '1.5.3' );
...
! defined( 'WOOON_DIR' ) && define( 'WOOON_DIR', plugin_dir_path( __FILE__ ) );
...
if ( ! class_exists( 'WPCleverWooon' ) ) {
	class WPCleverWooon {
		...
		function init() {
			// load text-domain
			load_plugin_textdomain( 'woo-order-notes', false, basename( WOOON_DIR ) . '/languages/' );
		}
		...
		function admin_enqueue_scripts() {
			...
			wp_localize_script( 'wooon-backend', 'wooon_vars', [
					'nonce' => wp_create_nonce( 'wooon-security' )
				]
			);
		}
		...
		function ajax_quickview() {
			if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'wooon-security' ) || ! current_user_can( 'manage_woocommerce' ) ) {
				die( esc_html__( 'Permissions check failed!', 'woo-order-notes' ) );
			}

			$notes_html = '';
			$order      = absint( sanitize_text_field( $_POST['order'] ?? 0 ) );

			if ( $order ) {
				ob_start();
				echo '<div id="woocommerce-order-notes" data-id="' . esc_attr( $order ) . '">';
				$notes = wc_get_order_notes( [ 'order_id' => $order ] );
		...
		function ajax_update_order_note() {
			...
			wp_update_comment( [
				'comment_ID'      => sanitize_text_field( $_POST['note_id'] ?? 0 ),
				'comment_content' => sanitize_text_field( $_POST['note_content'] ?? '' )
			] );

			echo sanitize_text_field( $_POST['note_content'] ?? '' );
			wp_die();
		}
		...
		function ajax_add_order_note() {
			...
			$note_type        = wc_clean( wp_unslash( sanitize_text_field( $_POST['note_type'] ?? '' ) ) );
			...
			foreach ( $ids as $id ) {
				...
				if ( $order ) {
					$order->add_order_note( trim( sanitize_text_field( $_POST['note'] ?? '' ) ), $is_customer_note, true );
				}
			}
