...
class WC_FreePay_Order_Utils {
	...
	public static function get_transaction_link_params($order) {
		...
		if(WC_FreePay_Helper::option_is_enabled( WC_FP_MAIN()->s( 'freepay_ageverificationmode' ) ) == 1) {
			$minimumuserage = self::get_minimumuserage($order);
			$params['MinCardholderAge'] = $minimumuserage;
		}
	...
	public static function get_minimumuserage($order)
    {
        ...
        {
			$tmp_minimumuserage = 0;
            $ageverification = get_post_meta($cart_item['product_id'] , 'ageverification', true);

            if($ageverification > 0)
            {
                $tmp_minimumuserage = $ageverification;
            }

			//if product didn't define min age, then check category setting if available
			if($tmp_minimumuserage == 0) {
				$terms = get_the_terms( $cart_item['product_id'], 'product_cat' );
				if(!empty($terms) && is_array($terms)) {
					foreach( $terms as $cat ) {
						$catageverification = get_term_meta($cat->term_id , 'ageverification', true);
						if(!empty($catageverification)) {
							if($catageverification > $tmp_minimumuserage) {
								$tmp_minimumuserage = $catageverification;
							}
						}
					}
				}
			...
			if($tmp_minimumuserage > 0) {
				if($tmp_minimumuserage > $minimumuserage) {
					$minimumuserage = $tmp_minimumuserage;
				}
			}
        }
