...
final class WC_Gateway_FreePay_Blocks_Support extends AbstractPaymentMethodType {
	...
	public function get_payment_method_data() {
		$images_path       = '/../assets/images/cards/';
		$iconsArray = $this->gateway->s( 'freepay_icons' );
		...
		foreach($iconsArray as &$item) {
			$item = untrailingslashit( plugins_url( '/', __FILE__ ) ) . $images_path . $item . ".svg";
		}
		...
		return [
			...
			'icons'		  => $iconsArray,
			'iconMaxHeight' => $this->gateway->s( 'freepay_icons_maxheight' )
		];
	}
}
