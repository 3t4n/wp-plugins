...
function init_freepay_gateway() {
	...
	class WC_FreePay extends WC_Payment_Gateway {
		...
		public function woocommerce_order_status_changed( $post_id, $old_status, $new_status) {
			$order = wc_get_order( $post_id );

			// Only run logic on the correct instance to avoid multiple calls, or if all extra instances has not been loaded.
			if ( ( WC_FreePay_Statekeeper::$gateways_added && $this->id !== $order->get_payment_method() ) || ! WC_FreePay_Order_Utils::has_freepay_payment($order) || $new_status != 'cancelled' ) {
				return;
			}

			if ( apply_filters( 'woo_freepay_void_on_order_cancelled', WC_FreePay_Helper::option_is_enabled( $this->s( 'freepay_voidoncancel' ) ), $order ) ) {
				if ( ! WC_FreePay_Subscription_Utils::is_wcs_subscription( $order ) ) {
					$transaction_id = WC_FreePay_Order_Utils::get_transaction_id($order);
					$payment        = new WC_FreePay_API_Payment();
					...
					if ( $transaction_id ) {
						try {
							...
							if ( $payment->can_i_cancel() ) {
								$payment->cancel( $transaction_id, $order );
							}
						} catch ( FreePay_API_Exception $e ) {
							...
							$error = sprintf( 'Unable to cancel payment on order #%s. Problem: %s', $order->get_id(), $e->getMessage() );
		...
		public function callback_handler() {
			...
			$is_renew = array_key_exists('is_renew', $queries) && $queries['is_renew'] == '1';
	...
	*/
	...
	function woo_add_ageverification_select()
	{
		return woocommerce_wp_select(
			array(
				'id'      => 'ageverification',
				'label'   => __( 'Age verification', 'freepay-for-woocommerce' ),
				'options' => array(
					'0'  => __( 'None', 'freepay-for-woocommerce' ),
					'15' => __( '15 Years', 'freepay-for-woocommerce' ),
					'16' => __( '16 Years', 'freepay-for-woocommerce' ),
					'18' => __( '18 Years', 'freepay-for-woocommerce' ),
					'21' => __( '21 Years', 'freepay-for-woocommerce' )
				)
			)
		);
	}
	...
	add_action( 'woocommerce_product_bulk_edit_end', 'woo_add_custom_bulk_general_fields' );

	function woo_add_custom_bulk_general_fields() {
		echo '<label><span class="title">' . __( 'Age verification', 'freepay-for-woocommerce' ) . '</span>';
		echo '<span class="input-text-wrap">';
		echo '<select class="ageverification" name="_ageverification">';
		$options = array(
			''   => __( '— No change —', 'freepay-for-woocommerce' ),
			'0'  => __( 'None', 'freepay-for-woocommerce' ),
			'15' => __( '15 Years', 'freepay-for-woocommerce' ),
			'16' => __( '16 Years', 'freepay-for-woocommerce' ),
			'18' => __( '18 Years', 'freepay-for-woocommerce' ),
	...
	add_action( 'woocommerce_product_bulk_edit_save', 'woo_add_custom_bulk_general_fields_save' );

	function woo_add_custom_bulk_general_fields_save($product) {
		if ( isset($_REQUEST['_ageverification']) && $_REQUEST['_ageverification'] != '' ) {
            $product->update_meta_data('ageverification', intval($_REQUEST['_ageverification']));
            $product->save();
        }
	}

	if(!function_exists('mdz_add_prcat_cfs')) {
		function mdz_add_prcat_cfs($term)
		{
			$term_meta = 0;

			if(!empty($term) && $term != null && is_object($term)) {
				$t_id = $term->term_id;
				$term_meta = get_term_meta( $t_id, 'ageverification', true );
			}
			...
			echo '<tr class="form-field"><th valign="top" scope="row"><label for="ageverification">' . __( 'Age verification', 'freepay-for-woocommerce' ). '</label></th><td><select class="ageverification" name="ageverification">';
			foreach ( $options as $key => $value ) {
				echo '<option ' . ($key == $term_meta ? "selected" : "") . ' value="' . (int)$key . '">' . $value . '</option>';
			}
			echo '</select></td></tr>';
		}
	}
	add_action('product_cat_edit_form_fields', 'mdz_add_prcat_cfs');
	add_action('product_cat_add_form_fields', 'mdz_add_prcat_cfs');

	//Save custom-field to product-category (Woocommerce)
	if(!function_exists('mdz_save_prcat_cfs')){
		function mdz_save_prcat_cfs($term_id, $term)
		{
			if ( isset( $_POST['ageverification'] ) ) {
				update_term_meta( $term_id, 'ageverification', intval( $_POST['ageverification'] ) );
			}
		}
	}
	add_action( 'edited_product_cat', 'mdz_save_prcat_cfs', 10, 2);  
	add_action( 'create_product_cat', 'mdz_save_prcat_cfs', 10, 2);
}
