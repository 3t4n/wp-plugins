...
class WC_Gateway_Appture_Pay extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        if(empty($options["client_secret"]) || empty($options["client_secret"])) {
            add_action( 'admin_notices', function() {
                echo // Customize the message below as needed
                    '<div class="notice notice-warning is-dismissible">
                    <p>Appture Pay has not been set-up yet. Please go to the <a href="/wp-admin/admin.php?page=appturelab-appture-pay">Setup page</a> and add the appropriate API Client details.</p>
                    </div>'; 
            } );
        }
    ...
    function init_form_fields() {
        $this->form_fields = array(
            ...
            'description' => array(
                ...
                'default' => __("Pay via Appture Pay using a Visa or Mastercard bank card or Secure Instant EFT", 'woocommerce')
            )
        );
   ...
   public function is_valid_for_use() {
    ...
    public function generate_appture_pay_form( $order_id ) {
        ...
        if(!class_exists('ApptureLab\ApptureWebRequest')) {
        ...
        if($response && $response["success"]) {
            ...
            $order->set_transaction_id($transactionIdentifier);
            $order->save();
    ...
    public function handle_itn_request( $data ) {
		...
		if ( $description && ! empty( $description->renewal_order_id ) ) {
    ...
    public function amounts_equal($amount1, $amount2) {
        return !( abs(floatval($amount1) - floatval($amount2)) > PHP_FLOAT_EPSILON );
    }
