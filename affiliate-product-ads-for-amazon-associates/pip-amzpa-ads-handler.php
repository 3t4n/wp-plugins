...
class PipAmzpaAdsHandler
{
	...
	function callAmazonApi( $method, $searchString = '' ) {
		...
		if( $responseCodeAmazonApi == 200 ) {
			...
			if( is_object($amzJson) && property_exists( $amzJson, $responseResultKey ) ) {
				if( is_object($amzJson->$responseResultKey) && property_exists( $amzJson->$responseResultKey, 'Items' ) ) {
										...
										$content .= '<div class="pip-amzpa-price">';
											if(
												...
												if( isset( $item->Offers->Listings[0]->Price ) ) {
													if( isset( $item->Offers->Listings[0]->Price->Savings ) && !empty( $item->Offers->Listings[0]->Price->Savings->Percentage ) ) {
												...
												if( !empty( $item->Offers->Listings[0]->DeliveryInfo->IsPrimeEligible ) ) {
