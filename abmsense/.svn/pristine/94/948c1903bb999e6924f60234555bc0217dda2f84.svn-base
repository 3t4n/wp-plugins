...
function abmsense_check_consent_enabled($customer_name) {
    ...
    $cache_duration = 86400; // Cache for 1 day
    ...
    if (false === $json_data) {
        abmsense_log("Failed to encode data to JSON in consent check", 'error');
        return false;
    }

    // Use our new SSL-safe request system
    $args = abmsense_get_api_request_args($json_data);
    $response = abmsense_safe_api_request($api_url, $args);

    if (is_wp_error($response)) {
        abmsense_log('API request to check consent failed: ' . $response->get_error_message(), 'error');
        
        // If API call fails, use cached value if available, otherwise default to false
        $last_known_value = get_option('abmsense_last_known_consent_' . md5($customer_name), null);
        if ($last_known_value !== null) {
            abmsense_log('Using last known consent value due to API failure', 'warning');
            return $last_known_value === '1';
        }
        return false;
    }
    ...
    update_option('abmsense_last_known_consent_' . md5($customer_name), $is_consent_enabled ? '1' : '0');
...
function abmsense_transfer_data_to_main_db() {
    ...
    if (false === $local_data) {
        // If not in cache, query the database
        $sql = "SELECT * FROM " . $local_table_name;
    ...
    if ($total_records === 0) {
        abmsense_log("No records to transfer", 'info');
        return;
    }
    ...
    foreach ($local_data as $index => $record) {
        ...
        // Use our SSL-safe request system
        $args = abmsense_get_api_request_args($json_body);
        $response = abmsense_safe_api_request($api_upsert_url, $args);
        ...
        if ($response_code === 200 && isset($result['success']) && $result['success'] === true) {
            ...
            // Log successful transfer details for debugging
            abmsense_log(sprintf(
                "Successfully transferred record %d/%d for %s",
                $index + 1,
                $total_records,
                $record['visitor_company'] ?? 'unknown company'
            ), 'info');
        } else {
            $failed_transfers++;
            $error_message = isset($result['error']) ? $result['error'] : 'Unknown error';
            $response_status = $response_code ?? 'unknown status';
            
            abmsense_log(sprintf(
                "Failed to transfer record %d. Status: %s, Error: %s, Response: %s",
                $index + 1,
                $response_status,
                $error_message,
    ...
    // Handle cleanup based on transfer results
    if ($successful_transfers === $total_records) {
        // All transfers successful, truncate the table
        $table_name = $wpdb->prefix . 'abmsense_temp_data';
        
        if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $table_name))) {
            $wpdb->query("TRUNCATE TABLE {$wpdb->prefix}abmsense_temp_data");
            abmsense_log("All records transferred successfully. Table truncated.", 'info');
        }
    } else {
        abmsense_log(sprintf(
            "Transfer completed with %d successes and %d failures out of %d total records.",
            $successful_transfers,
            $failed_transfers,
            $total_records
        ), 'warning');
    ...
    // Clear the cache after transfer attempt
