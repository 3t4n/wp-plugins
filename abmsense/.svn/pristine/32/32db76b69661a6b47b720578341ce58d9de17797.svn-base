...
function abmsense_sanitize_frequency($input) {
    ...
    return in_array($input, $allowed_values) ? $input : 'weekly';
}
...
// Add this function to utility.php
function abmsense_get_api_request_args($body) {
    // First try to get the system CA bundle path
    $ca_bundle_paths = array(
        ABSPATH . WPINC . '/certificates/ca-bundle.crt', // WordPress bundle
    ...
    return array(
        ...
        'timeout'   => 30,
        'sslverify' => true,
        'sslcertificates' => $ca_bundle,
    );
}

// Update the save_data function in settings.php
function abmsense_save_data($customer_name, $report_email, $report_frequency, $is_consent, $date_registered, $version_installed) {
    ...
    if ($response_code !== 200 || !isset($result['success']) || $result['success'] !== true) {
        ...
        abmsense_log("save_data Error: API response indicates failure - " . $error_message, 'error');
        return false;
    }
    ...
    update_option(ABMSENSE_PREFIX . 'date_registered', $date_registered);
    update_option(ABMSENSE_PREFIX . 'version_installed', $version_installed);

    // Clear the consent cache
...
function abmsense_settings_page() {
    ...
    $report_frequency = get_option(ABMSENSE_PREFIX . 'report_frequency', 'weekly');
