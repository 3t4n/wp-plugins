...
class Admin {
	...
	private function validateGetAccess( $posted_data ): bool {
        ...
        if (strpos($posted_data['filename'], '..') !== false || strpos($posted_data['filename'], './') !== false) {
            return false;
        }
	...
	public function load_scripts( $hook_suffix ): void {
		$script = [];
		if ( file_exists( CODEMANAS_TYPESENSE_ROOT_DIR_PATH . '/build/admin/index.asset.php' ) ) {
			$script = include_once CODEMANAS_TYPESENSE_ROOT_DIR_PATH . '/build/admin/index.asset.php';
		}
		wp_register_script( 'cm-typesense-admin-script',
			CODEMANAS_TYPESENSE_ROOT_URI_PATH . 'build/admin/index.js',
			$script['dependencies'],
			$script['version'], true );
		wp_register_style( 'cm-typesense-admin-style',
			CODEMANAS_TYPESENSE_ROOT_URI_PATH . 'build/admin/style-index.css',
			[ 'wp-components' ], $script['version'] );
