...
jQuery( document ).ajaxComplete(function() {
	...
	setTimeout("hp_document_ready();", 600);

	setTimeout("calculate_margin();", 700);
});

function hp_document_ready() {
	...
	/* Set Custom Color */
	jQuery('.b-animate-go').each(function(index, element) {
		var color = jQuery(this).data('color');
		if(color != 'undefined') {
			jQuery(this).find('.b-top-line').css("background", color);
			jQuery(this).find('.b-bottom-line').css("background", color);
			jQuery(this).find('.b-line').css("background", color);
		}
	});
}
...
function calculate_margin() {
  // Vertical alignment
	jQuery('.b-animate-go .b-wrapper').each(function(i, v){
      ...
      jQuery(v).find('.b-animate').each(function(ii, vv){
      ...
      if(set_mar > 0) {
        jQuery(v).find('.h').css('margin-top', set_mar);
      } else {
