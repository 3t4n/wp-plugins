...
class WP_Announcement
{
    ...
    public function announcement_html()
    {
        ...
        if ( class_exists( 'WooCommerce' ) && is_product() ) {
            $query = $this->get_announcement_query_for_current_product();
        } else {
            $query = $this->get_announcement_query_for_current_page();
        }


        //if no notice found for this page then check if any global notice exists
        if (!$query->have_posts()) {
            $query = $this->get_announcement_query_for_all_pages();
        }
    ...
    private function get_announcement_query_for_current_page()
    {
    ...
    private function get_announcement_query_for_current_product()
    {
        if ( !(class_exists( 'WooCommerce' ) && is_product()) ) {
            return null;
        }
        
        $current_page_id = (string)get_queried_object_id();
    ...
    private function get_announcement_query_for_all_pages()
    {
        $args = [
            'post_type' => 'announcement',
            'meta_query' => [
