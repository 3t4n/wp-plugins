...
class Plugin {
    ...
    public function __construct() {
        ...
        if (!empty(self::$instance)) {
            ...
            if (\EAddonsForElementor\Core\Utils::is_plugin_active('e-addons-manager')) {                
                if (class_exists('\EAddonsManager\Modules\License\License')) {
                    \EAddonsManager\Modules\License\License::init_license($this);
                }
            }
        }
    }
    ...
    public function add_addon($plugin) {
        if (empty($this->addons_manager)) {
            $addons = $this->get_addons(); // init addons_manager
        }
