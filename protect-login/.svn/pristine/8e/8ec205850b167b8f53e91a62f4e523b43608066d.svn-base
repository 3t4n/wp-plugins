...
use ProtectLogin\Datatypes\IP_Address;
...
class LoginHandler {
	...
	/**
	 * Saves the hash of the anonymized IP-Address of the current user
	 *
	 * @param int $userid   Id of the user who has just logged in.
	 *
	 * @return void
	 */
	public function safe_hashed_address( int $userid ) {
		$ip_address = new IP_Address( self::get_remote_address() );

		$hashed_address   = array();
		$hashed_address[] = $ip_address->get_hashed_address( true );

		$previous_addresses = get_user_meta( $userid, 'protect_login_previous_addresses', true );
		if ( is_array( $previous_addresses ) ) {
			for ( $i = 0; $i < 4; $i++ ) {
				if ( isset( $previous_addresses[ $i ] ) ) {
					$hashed_address[] = $previous_addresses[ $i ];
				}
			}
		}
		update_user_meta( $userid, 'protect_login_previous_addresses', $hashed_address );
	}
	...
	public static function print__error_message( $error ): string {
		global $errors;
		if ( null === $errors || ! is_object( $error ) ) {
			return $error;
		}
	...
	/**
	 * Checks if login is allowed for xmlrpc.php .
	 *
	 * @param Object $user User Object.
	 *
	 * @return void
	 */
	public function protect_login_check_is_xml_allowed( $user ) {
		$loginhandler = self::get_instance();
		if ( ! $loginhandler->is_login_from_ip_allowed() ) {
			echo esc_html( $loginhandler->compose_error_message() );
			exit;
		}

		$this->safe_hashed_address( $user->ID );
	}

	/**
	 * Checks after a successful login if the access from ip address of the current user is allowed.
	 *
	 * @param Object $user User Object.
	 * @param string $username Username the user entered.
	 * @param string $password Password the user entered.
	 *
	 * @return mixed|void
	 */
	public function protect_login_on_successfull_login( $user, $username, $password ) {
		if ( ! session_id() ) {
			session_start();
		}

		if ( defined( 'XMLRPC_REQUEST' ) && XMLRPC_REQUEST ) {
			$this->protect_login_check_is_xml_allowed();
		} else {
			if ( ! is_wp_error( $user ) && $this->is_login_from_ip_allowed() && null !== $user ) {
				$this->safe_hashed_address( $user->ID );
				return $user;
			}
	 ...
	 * @return mixed|string
	 */
	public static function get_remote_address( string $type_name = '' ) {
	...
	public static function get_current_lockedout_address(): array {
		...
		$now             = current_time( 'timestamp', get_option( 'timezone', 'UTC' ) );
		foreach ( $addresses as $address => $lockedout_until ) {
			if ( $lockedout_until < $now ) {
				continue;
			}

			$active_lockouts[] = $address;
		}
