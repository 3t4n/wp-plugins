...
require_once PROTECT_LOGIN_PATH . '/datatypes/class-ip-address.php';
...
function protect_login_admin_init() {
	...
	protect_login_lla_section();
	protect_login_advanced_option_section();
	protect_login_password_section();
	protect_login_2fa_section();
	protect_login_remote_api_section();
	protect_login_clear_cookies_on_activation();
}
...
function protect_login_uninstall_plugin() {
	$settings = array(
		...
		'protect_login_active',
	);

	$user_meta_keys = array(
		'protect_login_previous_addresses',
	);
...
function protect_login_new_multisite( $blog_id, $user_id, $domain, $path, $site_id, $meta ) {
	$data = array(
		...
		'protect_login_active'                         => get_option( 'protect_login_active', false ),
...
/**
 * Function to reset login session and removing WordPress cookies.
 * This is required after activation of the plugin to avoid login issues / broken sessions on multisites.
 *
 * @return void
 */
function protect_login_clear_cookies_on_activation() {
	$protect_login_active = get_option( 'protect_login_active', false );
	if ( $protect_login_active ) {
		return;
	}

	protect_login_update_option_on_mulitsite( 'protect_login_active', true );
	...
	if ( isset( $_COOKIE ) ) {
		foreach ( $_COOKIE as $name => $value ) {
			...
			setcookie( $name, '', time() - 3600, str_replace( '//', '/', COOKIEPATH . '/wp-admin' ), COOKIE_DOMAIN );
			setcookie( $name, '', time() - 3600, str_replace( '//', '/', SITECOOKIEPATH . '/wp-admin' ), COOKIE_DOMAIN );

			setcookie( $name, '', time() - 3600, str_replace( '//', '/', COOKIEPATH . '/content/plugins' ), COOKIE_DOMAIN );
			setcookie( $name, '', time() - 3600, str_replace( '//', '/', SITECOOKIEPATH . '/content/plugins' ), COOKIE_DOMAIN );
		}
	}
}
