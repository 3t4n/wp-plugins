...
class Fast_Cart_Admin_Settings{
    ...
    public static function register_fcw_setting(){
    	register_setting( 'fast-cart-group', 'fast_cart_option' );
        register_setting( 'fast-cart-group_adavanced', 'fast_cart_option_advanced' );
        register_setting( 'fast-cart-group_license', 'fast_cart_license' );
    }
    ...
    public static function admin_assets() {
        ...
        if( wp_verify_nonce( $admin_settings_nonce, 'fc-admin-settings-nonce' ) ){
            ...
            if ( isset( $_GET['page'] ) && ! empty( $_GET['page'] ) && 'fast-cart' === $_GET['page'] ) {
                ...
                wp_enqueue_script('fcw-admin', plugins_url('admin/js/backend.js', FAST_CART_PLUGIN_FILE), array('jquery','wp-color-picker'), fast_cart()->version(), true);
    ...
    **/
    public function reset_setting(){
        ...
        if( isset( $_GET['action'] ) && 'reset' === $_GET['action'] ){
            ...
            if ( isset( $_REQUEST['_wpnonce'] ) && ! wp_verify_nonce( sanitize_key( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'fcw-settings' ) ) {
                die( esc_attr__( 'Security check', 'fast-cart' ) ); 
            } else {
                
                if( isset( $_GET['tab'] ) && 'advanced' === $_GET['tab'] ){
                    delete_option('fast_cart_option_advanced');
