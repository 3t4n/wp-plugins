...
function DSABAFW_load_textdomain_pro() {
    load_plugin_textdomain( 'different-shipping-and-billing-address-for-woocommerce-pro', false, dirname( DSABAFW_BASE_NAME ) . '/languages' ); 
}

// Use load textdomain mofile
function DSABAFW_load_my_own_textdomain_pro( $mofile, $domain ) {
    if ( 'different-shipping-and-billing-address-for-woocommerce-pro' === $domain && false !== strpos( $mofile, WP_LANG_DIR . '/plugins/' ) ) {
