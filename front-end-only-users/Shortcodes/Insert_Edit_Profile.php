...
function Insert_Edit_Profile($atts) {
	...
	$ReturnString .= "<input type='hidden' name='Omit_Fields' value='" . esc_attr( $omit_fields ) . "'>";
	...
	foreach ($Fields as $Field) {
		...
		if (!in_array($Field->Field_Name, $Omitted_Fields)) {
			...
			$ReturnString .= "<div id='ewd-feup-field-" . esc_attr( $Field->Field_ID ) . "' class='feup-pure-control-group'>";
			$ReturnString .= "<label for='" . esc_attr( $Field->Field_Name ) . "' id='ewd-feup-edit-" . esc_attr( $Field->Field_ID ) . "' class='ewd-feup-field-label'>" . esc_html( $Field->Field_Name ) . ": </label>";
			if ($Field->Field_Type == "text") {
			    $ReturnString .= "<input name='" . esc_attr( $Field->Field_Name ) . "' id='ewd-feup-register-input-" . esc_attr( $Field->Field_ID ) . "' class='ewd-feup-text-input' type='text' value='" . esc_attr($Value) . "' " . $Req_Text . "/>";
			}
			...
			elseif ($Field->Field_Type == "textarea") {
				$ReturnString .= "<textarea name='" . esc_attr( $Field->Field_Name ) . "' id='ewd-feup-register-input-" . esc_attr( $Field->Field_ID ) . "' class='ewd-feup-textarea' " . $Req_Text . ">" . esc_attr($Value) . "</textarea>";
			}
			elseif ($Field->Field_Type == "file") {
				$ReturnString .= $feup_Label_Current_File . " " . substr($Value, 10) . " | ";
				$ReturnString .= "<input name='" . esc_attr( $Field->Field_Name ) . "' id='ewd-feup-register-input-" . esc_attr( $Field->Field_ID ) . "' class='ewd-feup-file-input' type='file' value='' " . $Req_Text . "/>";
			} 
			elseif ($Field->Field_Type == "picture") {
				$ReturnString .= esc_html( $feup_Label_Current_Picture );
				$ReturnString .= "<img src='" . esc_attr( content_url("/uploads/ewd-feup-user-uploads/") . $Value ) . "' alt='" . esc_attr( $Field->Field_Name ) . "' class='ewd-feup-profile-picture' /><br />";
				$ReturnString .= "<div class='ewd-feup-update-picture'>" . esc_html( $feup_Label_Update_Picture ) . "<input name='" . esc_attr( $Field->Field_Name ) . "' id='ewd-feup-register-input-" . esc_attr( $Field->Field_ID ) . "' class='ewd-feup-file-input' type='file' value='' " . $Req_Text . "/></div>";
			} 
			...
			elseif ($Field->Field_Type == "checkbox") {
				...
				foreach ($Options as $Option) {
					if ($Counter != 0) {$ReturnString .= "</div><div class='feup-pure-control-group ewd-feup-negative-top'><label class='feup-pure-radio'></label>";}
					$ReturnString .= "<input type='checkbox' name='" . esc_attr( $Field->Field_Name ) . "[]' value='" . esc_attr($Option) . "' class='ewd-feup-checkbox' " . $Req_Text . " ";
	...
	$ReturnString .= "<div class='feup-pure-control-group'><label for='submit'></label><input type='submit' class='ewd-feup-submit feup-pure-button feup-pure-button-primary' name='Edit_Profile_Submit' value='" . esc_attr( $submit_text ) . "'></div>";
	$ReturnString .= "</form>";
	$ReturnString .= "</div>";
