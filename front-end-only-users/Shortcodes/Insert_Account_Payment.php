...
function EWD_FEUP_Account_Payment($atts) {
	...
	if ($username == "" or ($Payment_Types == "Levels" and $level == "")) {
		...
		if ($Payment_Types == "Levels") {
			$ReturnString .= "<h4>" . esc_html( $feup_Label_Upgrade_Account ) . "</h4>";
			$ReturnString .= "<p>" . esc_html( $feup_Label_Upgrade_Level ) . "</p>";
		}
		$ReturnString .= "<form action='#' method='post'>";
		if ($username == "") {
			$ReturnString .= "<div class='feup-pure-control-group'>";
			$ReturnString .= "<label for='Username' id='ewd-feup-paypal-username-div' class='ewd-feup-field-label'>" . esc_html( $feup_Label_Username )  . ": </label>";
			$ReturnString .= "<input type='text' class='ewd-feup-text-input ewd-feup-paypal-username-input' name='Username'>";
			$ReturnString .= "</div>";
		}
		else {
			$ReturnString .= "<div class='feup-pure-control-group'>";
			$ReturnString .= "<label for='Username' id='ewd-feup-paypal-username-label' class='ewd-feup-field-label'>" . esc_html( $feup_Label_Username )  . ": </label>";
			$ReturnString .= "<input type='hidden' name='Username' value='" . esc_attr( $username ) . "' />";
			$ReturnString .= "<span class='ewd-feup-username'>" . esc_html( $username ) . "</span>";
			$ReturnString .= "</div>";
		}
		if ($Payment_Types == "Levels") {
			if ($level == "") {
				$ReturnString .= "<div class='feup-pure-control-group'>";
				$ReturnString .= "<label for='level' id='ewd-feup-paypal-username-div' class='ewd-feup-field-label'>" . esc_html( $feup_Label_Level ) . ": </label>";
				...
				foreach ($Levels_Payment_Array as $Level_Payment_Item) {
					...
					$ReturnString .= "<option value='" . esc_attr( $Level_Payment_Item['Level'] ) . "'>" . esc_html( $Level->Level_Name ) . " (" . esc_html( $Level_Payment_Item['Amount'] ) . ")</option>";
				}
				$ReturnString .= "</select>";
				$ReturnString .= "</div>";
			}
		...
		$ReturnString .= "<div class='feup-pure-control-group'><label for='submit'></label><input type='submit' class='ewd-feup-submit feup-pure-button feup-pure-button-primary' name='PayPal_Username_Submit' value='" . esc_attr( $feup_Label_Next ) . "'></div>";
		$ReturnString .= "</form>";
		$ReturnString .= "</div>";
	}
