...
function Privilege_Level($atts, $content = null) {
	...
	if ($minimum_level != '' and $PrivilegeLevel->Level_Privilege < $minimum_level) {
		$ReturnString = "<div class='ewd-feup-error'>" . esc_html( $feup_Label_Low_Account_Level_Message ) . "</div>";
		if ($Payment_Types == "Levels") {$ReturnString .= do_shortcode("[account-payment]");}
	}
	if ($maximum_level != '' and $PrivilegeLevel->Level_Privilege > $maximum_level) {$ReturnString = "<div class='ewd-feup-error'>" .  esc_html( $feup_Label_High_Account_Level_Message ) . "</div>";}
	...
	if ($level != '' and $PrivilegeLevel->Level_Privilege != $level) {$ReturnString = "<div class='ewd-feup-error'>" .  esc_html( $feup_Label_Wrong_Account_Level_Message ) . "</div>";}
	if ($field_name != '') {
		...
		if ($Validate != "Yes") {$ReturnString = "<div class='ewd-feup-error'>" . esc_html(  __("Sorry, this content is only for those whose " . $field_name . " is " . $field_value . ".", 'front-end-only-users') ) . "</div>";}
	}
