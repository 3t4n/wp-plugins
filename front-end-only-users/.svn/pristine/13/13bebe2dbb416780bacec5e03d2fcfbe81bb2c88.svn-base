...
function User_Search($atts, $content = null) {
	...
	$ReturnString .= EWD_FEUP_Add_Modified_Styles();
		...
		foreach ($search_fields_array as $field) {	
				...
				$ReturnString .= "<label for='" . esc_attr( $field ) . "' id='ewd-feup-" . esc_attr( $field_clean ) . "-div' class='ewd-feup-field-label'>" . esc_html( $field ) . ": </label>";
				$ReturnString .= "<input type='text' class='ewd-feup-text-input' name='search_" . esc_attr( $field_clean ) . "' placeholder='" . esc_attr( $field ) . "...'>";
				$ReturnString .= "</div>";
		}
		$ReturnString .= "<div class='feup-pure-control-group'>";
