...
class Article extends JsonSchemaValue implements GetJsonData {
    ...
    protected function getKeysForSchemaManual() {
        return [
            ...
            'articleType' => '_rankology_fno_rich_snippets_article_type',
            ...
            'image' => '_rankology_fno_rich_snippets_article_img',
            'imageWidth' => '_rankology_fno_rich_snippets_article_img_width',
            'imageHeight' => '_rankology_fno_rich_snippets_article_img_height',
            'coverageStartDate' => '_rankology_fno_rich_snippets_article_coverage_start_date',
            'coverageStartTime' => '_rankology_fno_rich_snippets_article_coverage_start_time',
            'coverageEndDate' => '_rankology_fno_rich_snippets_article_coverage_end_date',
            'coverageEndTime' => '_rankology_fno_rich_snippets_article_coverage_end_time',
            'speakableCssSelector' => '_rankology_fno_rich_snippets_article_speakable_css_selector',
        ];
    }
    ...
    public function getJsonData($context = null) {
        ...
        $schema = rankology_get_service('JsonSchemaGenerator')->getJsonFromSchema(MainEntityOfPage::NAME, $context, ['remove_empty' => true]);
        ...
        $schema = rankology_get_service('JsonSchemaGenerator')->getJsonFromSchema(Organization::NAME, $context, ['remove_empty' => true]);
        if (count($schema) > 1 && isset($schema['@type']) && 'none' !== $schema['@type']) {
            $data['publisher'] = $schema;

            $logo = rankology_fno_get_service('OptionPro')->getArticlesPublisherLogo();
            if ( ! empty($logo)) {
                $variablesContext = [
                    'url' => $logo,
                    'width' => rankology_fno_get_service('OptionPro')->getArticlesPublisherLogoWidth(),
                    'height' => rankology_fno_get_service('OptionPro')->getArticlesPublisherLogoHeight(),
                ];
                $contextWithVariables = $context;
                $contextWithVariables['variables'] = $variablesContext;
        ...
        if (isset($variables['articleType']) && 'LiveBlogPosting' === $variables['articleType']) {
            ...
            $schema = rankology_get_service('JsonSchemaGenerator')->getJsonFromSchema(Thing::NAME, $context, ['remove_empty' => true]);
            if (count($schema) > 1) {
        ...
        if (isset($variables['speakableCssSelector']) && ! empty($variables['speakableCssSelector'])) {
            ...
            $schema = rankology_get_service('JsonSchemaGenerator')->getJsonFromSchema(Speakable::NAME, $contextWithVariables, ['remove_empty' => true]);
            if (count($schema) > 1) {
                $data['speakable'] = $schema;
            }
        }

        return apply_filters('rankology_fno_get_json_data_article', $data, $context);
    }
