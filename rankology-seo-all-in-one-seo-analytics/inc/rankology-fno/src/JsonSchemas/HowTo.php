...
class HowTo extends JsonSchemaValue implements GetJsonData {
    ...
    protected function getVariablesForManualSnippet($schemaManual) {
        $keys = [
            'step' => '_rankology_fno_rich_snippets_how_to',
            'name' => '_rankology_fno_rich_snippets_how_to_name',
            'description' => '_rankology_fno_rich_snippets_how_to_desc',
            'image' => '_rankology_fno_rich_snippets_how_to_img',
            'width' => '_rankology_fno_rich_snippets_how_to_img_width',
            'height' => '_rankology_fno_rich_snippets_how_to_img_height',
            'currency' => '_rankology_fno_rich_snippets_how_to_currency',
            'cost' => '_rankology_fno_rich_snippets_how_to_cost',
            'totalTime' => '_rankology_fno_rich_snippets_how_to_total_time',
        ];
        $variables = [];
    ...
    public function getJsonData($context = null) {
        ...
        if (isset($variables['step']) && ! empty($variables['step'])) {
            foreach ($variables['step'] as $key => $step) {
                ...
                $schema = rankology_get_service('JsonSchemaGenerator')->getJsonFromSchema(HowToStep::NAME, $contextWithVariables, ['remove_empty' => true]);
        ...
        return apply_filters('rankology_fno_get_json_data_how_to', $data, $context);
    }
}
