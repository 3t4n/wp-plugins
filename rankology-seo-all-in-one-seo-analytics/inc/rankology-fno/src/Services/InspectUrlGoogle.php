...
class InspectUrlGoogle {
    ...
    public function handle($postId) {
        ...
        //URL to inspect
        $url = apply_filters('rankology_inspect_url_permalink', get_permalink($postId));

        //Site URL
        $site_url = rankology_fno_get_service('OptionPro')->getGSCDomainProperty() ? true : site_url();

        if ($site_url === true) {
            $site_url = wp_parse_url(site_url());
            if (is_array($site_url) && isset($url['host'])) {
                $site_url = 'sc-domain:' . $site_url['host'];
            }
        }

        $site_url = apply_filters('rankology_inspect_url_home_url', site_url());

        $response = $this->request($site_url, $url);

        update_post_meta($postId, '_rankology_gsc_inspect_url_data', $response);
