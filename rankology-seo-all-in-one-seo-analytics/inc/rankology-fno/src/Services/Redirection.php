...
class Redirection {
    ...
    public function getPageByTitle($title, $output, $post_type) {
        ...
        $post_type = isset($post_type) ? $post_type : 'rankology_404';
        ...
        "
			...
			WHERE 1=1
			AND ( ( $wpdb->postmeta.meta_key = '_rankology_redirections_enabled'
			AND $wpdb->postmeta.meta_value = 'yes' ) )
			AND post_title = %s
			AND post_type = %s
			AND post_status = 'publish'
            AND ( ( mt1.meta_key = '_rankology_redirections_logged_status'
            AND mt1.meta_value = '$metaValueByLoggedIn' )
            OR ( mt1.meta_key = '_rankology_redirections_logged_status'
    ...
    public function update404CounterById($id) {
        $counter = get_post_meta($id, 'rankology_404_count', true);
        update_post_meta($id, 'rankology_404_count', ++$counter);

        //Update last time requested
        update_post_meta($id, '_rankology_404_redirect_date_request', time());
    }
    ...
    public function checkRegexRedirect() {
        ...
        $query_param = get_post_meta($redirectionMatch->ID, '_rankology_redirections_param', true);
    ...
    public function handleRedirectionWithId($id, $options = []) {
        $redirectionsEnabled = get_post_meta($id, '_rankology_redirections_enabled', true);
        ...
        //Query parameters
        $query_param = $query_param_value_safe = get_post_meta($id, '_rankology_redirections_param', true);
        ...
        $loggedStatus = get_post_meta($id, '_rankology_redirections_logged_status', true);
        ...
        $redirectionType = get_post_meta($id, '_rankology_redirections_type', true);
        $redirectionValue = get_post_meta($id, '_rankology_redirections_value', true);
        ...
        if ('410' == $redirectionType || '451' == $redirectionType) {
            //URL redirection
            $rankology_redirections_value = $initUrl;
            ...
            if (true == $if_exact_match) {
                header('Location:' . $rankology_redirections_value, true, $redirectionType);
                exit();
        ...
        elseif ($redirectionValue) {
            //URL redirection
            $rankology_redirections_value = html_entity_decode($redirectionValue);

            //Query parameters
            if ('with_ignored_param' === $query_param_value_safe && isset($_SERVER['QUERY_STRING'])) {
                $rankology_redirections_value = add_query_arg($_SERVER['QUERY_STRING'], '', $rankology_redirections_value);
            }
            ...
            //Do redirect
            if (true == $if_exact_match) {
                wp_redirect($rankology_redirections_value, $redirectionType);
                exit();
            } elseif (false == $if_exact_match && 'exact_match' != $query_param) {
                wp_redirect($rankology_redirections_value, $redirectionType);
                exit();
