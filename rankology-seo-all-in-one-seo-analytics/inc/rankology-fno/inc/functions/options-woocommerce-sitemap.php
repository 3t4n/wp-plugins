...
if (is_plugin_active( 'woocommerce/woocommerce.php' )) {
	if (!function_exists('rankology_woocommerce_xml_single_query')) {
		function rankology_woocommerce_xml_single_query($args, $cpt_key) {
			//Init
			$wc_pages_polylang 	= [];
			$wc_pages_wpml 		= [];
			$wc_pages 			= [];
			...
			if (function_exists('wc_get_page_id')) {
				if (rankology_fno_get_service('OptionPro')->getWCCustomerAccountPageNoindexEnable() ==='1') {
				...
				if (rankology_fno_get_service('OptionPro')->getWCCheckoutPageNoindexEnable() ==='1') {
					if (function_exists('icl_object_id') && is_plugin_active('sitepress-multilingual-cms/sitepress.php')) { //WPML
				...
				if (rankology_fno_get_service('OptionPro')->getWCCartPageNoindexEnable() ==='1') {
					if (function_exists('icl_object_id') && is_plugin_active('sitepress-multilingual-cms/sitepress.php')) { //WPML
		...
		add_filter('rankology_sitemaps_single_query', 'rankology_woocommerce_xml_single_query', 10, 2);
	}
}
