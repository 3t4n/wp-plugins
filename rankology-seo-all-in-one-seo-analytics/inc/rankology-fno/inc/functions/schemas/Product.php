...
//Products JSON-LD
function rankology_automatic_rich_snippets_products_option($schema_datas) {
    ...
    if (0 != count(array_filter($schema_datas))) {
        ...
        if (isset($product) && '' == $products_global_ids && method_exists($product, 'get_id') && '' != get_post_meta($product->get_id(), 'rkseo_wc_barcode_type_field', true) && 'none' != get_post_meta($product->get_id(), 'rkseo_wc_barcode_type_field', true)) {
            $products_global_ids = get_post_meta($product->get_id(), 'rkseo_wc_barcode_type_field', true);
        }

        $products_global_ids_value = $schema_datas['global_ids_value'];

        if (isset($product) && '' == $products_global_ids_value && method_exists($product, 'get_id') && '' != get_post_meta($product->get_id(), 'rkseo_wc_barcode_field', true) && 'none' != get_post_meta($product->get_id(), 'rkseo_wc_barcode_field', true)) {
            $products_global_ids_value = get_post_meta($product->get_id(), 'rkseo_wc_barcode_field', true);
        }
        ...
        if ('' == $products_condition) {
            $products_condition = rankology_check_ssl() . 'schema.org/NewCondition';
        }

        $products_availability = $schema_datas['availability'];

        if ('' == $products_availability) {
            $products_availability = rankology_check_ssl() . 'schema.org/InStock';
        }
        ...
        if (isset($product) && method_exists($product, 'is_type') && $product->is_type('variable')) {
            ...
            foreach ($variations as $key => $value) {
                ...
                if (isset($value['rankology_global_ids']) && ! empty($value['rankology_global_ids'])) {
                    $product_global_ids = $value['rankology_global_ids'];
                }
                if (isset($value['rankology_barcode']) && ! empty($value['rankology_barcode'])) {
                    $product_barcode = $value['rankology_barcode'];
                }
                ...
                $availability = sprintf('%s%s/InStock', rankology_check_ssl(), 'schema.org');
                if ( ! $value['is_in_stock']) {
                    $availability = sprintf('%s%s/OutOfStock', rankology_check_ssl(), 'schema.org');
                }
        ...
        $json = apply_filters('rankology_schemas_auto_product_json', $json);

        $json = '<script type="application/ld+json">' . json_encode($json) . '</script>' . "\n";

        $json = apply_filters('rankology_schemas_auto_product_html', $json);
