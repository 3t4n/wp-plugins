...
//Review JSON-LD
function rankology_automatic_rich_snippets_review_option($schema_datas) {
    //if no data
    if (0 != count(array_filter($schema_datas))) {
        ...
        $json = apply_filters('rankology_schemas_auto_review_json', $json);

        $json = '<script type="application/ld+json">' . json_encode($json) . '</script>' . "\n";

        $json = apply_filters('rankology_schemas_auto_review_html', $json);
