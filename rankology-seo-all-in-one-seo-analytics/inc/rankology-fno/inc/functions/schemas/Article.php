...
function rankology_automatic_rich_snippets_articles_option($schema_datas) {
    ...
    if (0 != count(array_filter($schema_datas))) {
        ...
        $article_canonical = '' ;
        if (get_post_meta(get_the_ID(), '_rankology_robots_canonical', true)) {
            $article_canonical = get_post_meta(get_the_ID(), '_rankology_robots_canonical', true);
        } else {
            global $wp;
            if (isset($wp->request)) {
                $article_canonical = home_url(add_query_arg([], $wp->request));
            }
        }
        ...
        if (!empty(rankology_get_service('SocialOption')->getSocialKnowledgeName())) {
            $json['publisher'] = [
                '@type' => 'Organization',
                'name' => rankology_get_service('SocialOption')->getSocialKnowledgeName(),
            ];
            if (!empty(rankology_fno_get_service('OptionPro')->getArticlesPublisherLogo())) {
                $json['publisher']['logo'] = [
                    '@type' => 'ImageObject',
                    'url' => rankology_fno_get_service('OptionPro')->getArticlesPublisherLogo(),
            ...
            $facebook = rankology_get_service('SocialOption')->getSocialAccountsFacebook();
            $twitter = rankology_get_service('SocialOption')->getSocialAccountsTwitter();
            $pinterest = rankology_get_service('SocialOption')->getSocialAccountsPinterest();
            $instagram = rankology_get_service('SocialOption')->getSocialAccountsInstagram();
            $youtube = rankology_get_service('SocialOption')->getSocialAccountsYoutube();
            $linkedin = rankology_get_service('SocialOption')->getSocialAccountsLinkedin();

            if (version_compare(RANKOLOGY_VERSION, '6.5', '>=')) {
                $extra = rankology_get_service('SocialOption')->getSocialAccountsExtra();
            }
        ...
        $json = apply_filters('rankology_schemas_auto_article_json', $json);

        $json = '<script type="application/ld+json">' . json_encode($json) . '</script>' . "\n";

        $json = apply_filters('rankology_schemas_auto_article_html', $json);
