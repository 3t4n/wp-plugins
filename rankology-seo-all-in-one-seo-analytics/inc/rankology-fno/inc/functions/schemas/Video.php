...
//Videos JSON-LD
function rankology_automatic_rich_snippets_videos_option($schema_datas) {
    //if no data
    if (0 != count(array_filter($schema_datas))) {
        ...
        if(!empty($videos_uploaded_date)){
            $json['uploadDate'] = $videos_uploaded_date;
        }
        ...
        $json = apply_filters('rankology_schemas_auto_video_json', $json);

        $json = '<script type="application/ld+json">' . json_encode($json) . '</script>' . "\n";

        $json = apply_filters('rankology_schemas_auto_video_html', $json);
