rkns_js.pages_meta_box = {
    ...
    view: function (args = []) {
        ...
        <tr>
            <th class="row-id"></th>
            <th colspan="6">${rkns_js._('title')}</th>
            <th width="10%">${rkns_js._('visits')}</th>
        </tr>`;

        const siteUrl = rkns_js.global.admin_url.replace('/wp-admin/', '');
        let i = 1;
        args.pages.forEach(function (value) {
            t += `<tr>
			<td class="row-id">${i}</td>
			<td colspan="6"><a href="${siteUrl}${value['str_url']}" title="${value['title']}" class='rkns-text-wrap' target="_blank">${value['title']} <span class="dashicons dashicons-geoipset" style="font-size: 15px; vertical-align: middle"></span></a></td>
