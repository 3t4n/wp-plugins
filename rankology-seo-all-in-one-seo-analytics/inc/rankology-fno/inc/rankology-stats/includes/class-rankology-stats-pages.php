...
class Pages
{
    ...
    {
        return (has_filter('rankology_stats_active_pages')) ? apply_filters('rankology_stats_active_pages', true) : Option::get('pages');
    }
    ...
    public static function get_page_type()
    {
        ...
        return apply_filters('rankology_stats_current_page', $current_page);
    }

    /**
     * Check Track All Page Rankology Stats
     *
     * @return bool
     */
    public static function is_track_all_page()
    {
        return apply_filters('rankology_stats_track_all_pages', Option::get('track_all_pages') || is_single() || is_page() || is_front_page());
    }
    ...
    public static function get_page_uri()
    {
        ...
        return apply_filters('rankology_stats_page_uri', $page_uri);
    }
    ...
    public static function record()
    {
        ...
        if (null !== $exist) {
            ...
            $pages = apply_filters('rankology_stats_pages_information', $pages);
    ...
    public static function save_page($page = array())
    {
        ...
        add_filter('query', array('\RANKOLOGY_STATS\DB', 'insert_ignore'), 10);
        ...
        remove_filter('query', array('\RANKOLOGY_STATS\DB', 'insert_ignore'), 10);

        # Do Action After Save New Visitor
        do_action('rankology_stats_save_page', $page_id, $page);
    ...
    public static function get_page_info($page_id, $type = 'post', $slug = false)
    {
        ...
        if (!empty($type)) {
            switch ($type) {
                ...
                case "home":
                    $arg = array(
                        'title' => __('Home Page', 'rankology-stats'),
                        'link'  => get_site_url()
                    );
                    break;
                case "author":
                    ...
                    break;
                case "feed":
                    $arg['title'] = __('Feed', 'rankology-stats');
                    break;
                case "loginpage":
                    $arg['title'] = __('Login Page', 'rankology-stats');
                    break;
                case "search":
                    $arg['title'] = __('Search Page', 'rankology-stats');
                    break;
                case "404":
                    $arg['title'] = __('404 not found', 'rankology-stats');
                    break;
                case "archive":
                    if ($slug) {
                        $post_type   = trim($slug, '/');
                        ...
                        if ($post_object instanceof \WP_Post_Type) {
                            $arg['title'] = sprintf(__('Post Archive: %s', 'rankology-stats'), $post_object->labels->name);
                        } else {
                            $arg['title'] = sprintf(__('Post Archive: %s', 'rankology-stats'), $slug);
                        }
                    } else {
                        $arg['title'] = __('Post Archive', 'rankology-stats');
                    }
