...
class log_page
{
     ...
     * @var string
     */
    public static $overview_ads_API = 'https://rankology-stats.com/wp-json/ads/overview';
    ...
    public function overview_page_ads()
    {
        ...
        if (apply_filters('rankology_stats_ads_overview_page_show', true) === false) {
            return;
        }

        // Get Overview Ads
        $get_overview_ads = get_option('rankology_stats_overview_page_ads', false);
        ...
        if ($get_overview_ads === false || (is_array($get_overview_ads) and (current_time('timestamp') >= ($get_overview_ads['timestamp'] + WEEK_IN_SECONDS)))) {
            ...
            // Set Option
            update_option('rankology_stats_overview_page_ads', $overview_ads, 'no');
        }
    }
