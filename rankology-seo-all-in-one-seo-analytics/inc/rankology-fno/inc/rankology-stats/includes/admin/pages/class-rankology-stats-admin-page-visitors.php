...
class visitors_page
{
    public function __construct()
    {
        if (Menus::in_page('visitors')) {
            ...
            add_filter('rankology_stats_days_ago_request', array('\RANKOLOGY_STATS\Helper', 'set_all_option_datepicker'));
    ...
    {
        // Page title
        $args['title'] = (count($_GET) > 1 ? __('Visitors', 'rankology-stats') : __('Recent Visitors', 'rankology-stats'));
        ...
        // Create Sub List
        $args['sub']['all'] = array('title' => __('All', 'rankology-stats'), 'count' => Visitor::Count($sql), 'active' => (isset($_GET['platform']) || isset($_GET['agent']) || isset($_GET['referrer']) || isset($_GET['referred']) || isset($_GET['ip']) || isset($_GET['location']) || isset($_GET['user_id']) ? false : true), 'link' => Menus::admin_url('visitors'));
        ...
        if ($args['filter']['number'] > 1) {
            $args['sub']['custom'] = array('title' => __('Custom filter', 'rankology-stats'), 'count' => Visitor::Count($sql), 'active' => true, 'link' => remove_query_arg('custom'));
        }
    ...
    public static function Filter()
    {
        ...
        // Code Button
        $filter['code'] = '<div class="rkns-pull-' . (is_rtl() ? 'left' : 'right') . '" id="visitors-filter"><span class="dashicons dashicons-filter"></span><span class="filter-text">' . __("Filters", "rankology-stats") . '</span> ' . ($filter['number'] > 0 ? '<span class="rkns-badge">' . number_format_i18n($filter['number']) . '</span>' : '') . '</div>';
