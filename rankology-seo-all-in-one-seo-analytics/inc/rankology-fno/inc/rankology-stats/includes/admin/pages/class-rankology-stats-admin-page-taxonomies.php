...
class taxonomies_page
{
    ...
    public function init()
    {
        ...
        if (Menus::in_page('taxonomies')) {
            ...
            self::$defaultTaxonomies = apply_filters('rankology_stats_default_taxonomies', ['category', 'post_tag']);
            ...
            if (isset($_GET['ID']) and (!is_numeric($_GET['ID']) || ($_GET['ID'] != 0 and term_exists((int)trim($_GET['ID']), self::$taxonomy) == null))) {
                wp_die(__("Request is not valid.", "rankology-stats"));
            }
        }
    }
    ...
    public static function view()
    {
        ...
        $args['title'] = __($taxonomyTitle . ' Statistics', 'rankology-stats');
        ...
        foreach (self::$taxonomies as $slug => $title) {
            ...
            $link  = Menus::admin_url('rkns_taxonomies_page', ['taxonomy' => $slug]);
            if (!in_array($slug, self::$defaultTaxonomies)) {
                $class .= ' rkns-locked';
                $link  = sprintf('%s/product/rankology-stats-detailed-data?utm_source=rankology_stats&utm_medium=display&utm_campaign=wordpress', RANKOLOGY_STATS_SITE_URL);
            }
            $args['tabs'][] = [
                'link'  => $link,
                'title' => $title,
        ...
        if (!isset($_GET['ID']) || (isset($_GET['ID']) and $_GET['ID'] == 0)) {
            ...
            $args['top_title']     = __('Top ' . $taxonomyTitle . ' Sorted by Hits', 'rankology-stats');

            // Push List Category
            foreach ($terms as $term) {
                $args['top_list'][$term->term_id] = array('ID' => $term->term_id, 'name' => $term->name, 'link' => add_query_arg('ID', $term->term_id), 'count_visit' => (int)rankology_stats_pages('total', null, $term->term_id, null, null, self::$taxonomy));
            }
            ...
            $args['top_title']     = __('Top posts Sorted by Hits in this taxonomy', 'rankology-stats');
            ...
            foreach ($post_lists as $post_id => $post_title) {
                $args['top_list'][$post_id] = array('ID' => $post_id, 'name' => $post_title, 'link' => Menus::admin_url('pages', array('ID' => $post_id)), 'count_visit' => (int)rankology_stats_pages('total', null, $post_id, null, null, 'post'));
            }
