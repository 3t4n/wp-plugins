...
class pages_page
{
    ...
    public function init()
    {
        ...
        if (Menus::in_page('pages')) {
            ...
            if (self::is_custom_page()) {
                ...
                if ($page_count < 1) {
                    wp_die(__('Your request is not valid.', 'rankology-stats'));
                }
            }

            self::$postTypes        = Helper::get_list_post_type();
            self::$defaultPostTypes = apply_filters('rankology_stats_default_post_types', ['post', 'page']);
    ...
    public static function view()
    {
        ...
        if (self::is_custom_page()) {
            ...
            // Page title
            $args['title'] = __('Top ' . $objectTitle, 'rankology-stats');

            // Top Trending Title
            $args['top_trending_title'] = __('Top 5 Trending ' . $objectTitle, 'rankology-stats');
            ...
            // Get List
            $args['lists'] = \RANKOLOGY_STATS\Pages::getTop(array(
            ...
            foreach (self::$postTypes as $slug) {
                ...
                $link         = Menus::admin_url('rkns_pages_page', ['type' => $postTypeSlug]);

                if (!in_array($postTypeSlug, self::$defaultPostTypes)) {
                    $class .= ' rkns-locked';
                    $link  = sprintf('%s/product/rankology-stats-detailed-data', RANKOLOGY_STATS_SITE_URL);
                }
    ...
    public static function custom_page_statistics()
    {
        ...
        // Page title
        $args['title'] = __('Page Statistics', 'rankology-stats');
        ...
        foreach (self::SINGLE_PAGE_COMPONENTS as $component) {
            $args[$component] = apply_filters('rankology_stats_pages_chart_' . $component,
