...
class words_page
{
    ...
    public static function view()
    {
        ...
        // Page title
        $args['title'] = __('Latest Search Words', 'rankology-stats');
        ...
        $args['search_engine']['all'] = array('title' => __('All', 'rankology-stats'), 'count' => rankology_stats_searchword('all', 'total'), 'active' => (isset($_GET['referred']) ? false : true), 'link' => Menus::admin_url('words'));
        foreach ($search_engines as $key => $se) {
            $args['search_engine'][$se['tag']] = array('title' => $se['translated'], 'count' => rankology_stats_searchword($key, 'total'), 'active' => ((isset($_GET['referred']) and $_GET['referred'] == $se['tag']) ? true : false), 'link' => add_query_arg('referred', $se['tag'], Menus::admin_url('words')));
        }
