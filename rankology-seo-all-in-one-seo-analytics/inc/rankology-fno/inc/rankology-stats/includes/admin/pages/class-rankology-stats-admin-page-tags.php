...
class tags_page
{
    ...
    public static function view()
    {
        ...
        // Page title
        $args['title'] = __('Tag Statistics', 'rankology-stats');
        ...
        $args['select_box'] = array(
            'name'  => 'ID',
            'title' => __('Select Tag', 'rankology-stats')
        );
        ...
        if (!isset($_GET['ID']) || (isset($_GET['ID']) and $_GET['ID'] == 0)) {
            ...
            $args['top_title']     = __('Top Tag Sorted by Hits', 'rankology-stats');

            // Push List Category
            foreach ($terms as $tag) {
                $args['top_list'][$tag->term_id] = array('ID' => $tag->term_id, 'name' => $tag->name, 'link' => add_query_arg('ID', $tag->term_id), 'count_visit' => (int)rankology_stats_pages('total', null, $tag->term_id, null, null, 'post_tag'));
            }
            ...
            $args['top_title']     = __('Top posts Sorted by Hits in this tag', 'rankology-stats');
