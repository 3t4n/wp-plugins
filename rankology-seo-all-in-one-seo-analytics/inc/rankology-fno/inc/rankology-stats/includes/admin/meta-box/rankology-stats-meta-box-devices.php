...
class devices extends MetaBoxAbstract
{
    ...
    public static function get($arg = array())
    {
        ...
        $sql = $wpdb->prepare("SELECT device, COUNT(*) as count FROM " . DB::table('visitor') . " WHERE device != '" . _x('Unknown', 'Device', 'rankology-stats') . "' AND `last_counter` BETWEEN %s AND %s GROUP BY device {$order_by}", reset($days_time_list), end($days_time_list));
        ...
        if (end($days_time_list) == TimeZone::getCurrentDate("Y-m-d")) {
            $title = sprintf(__('%s Statistics in the last %s days', 'rankology-stats'), __('Devices', 'rankology-stats'), self::$countDays);
        } else {
            $title = sprintf(__('%s Statistics from %s to %s', 'rankology-stats'), __('Devices', 'rankology-stats'), $args['from'], $args['to']);
        }
