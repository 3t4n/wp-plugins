...
class pages_chart
{
    ...
    public static function get($arg = array())
    {
        ...
        if (end($days_time_list) == TimeZone::getCurrentDate("Y-m-d")) {
            $title = sprintf(__('Traffic in the in the last %s days', 'rankology-stats'), $count_day);
        } else {
        ...
        foreach ($days_time_list as $d) {
            $getStatic      = (int)rankology_stats_pages($d, null, ($args['ID'] > 0 ? $args['ID'] : -1), null, null, $args['type']);
            $stats[]        = $getStatic;
            $total_in_dates = $total_in_dates + $getStatic;
        }

        // Get Total
        $total = (int)rankology_stats_pages('total', null, ($args['ID'] > 0 ? $args['ID'] : -1), null, null, $args['type']);
