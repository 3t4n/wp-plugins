...
class post
{
    ...
    public static function get($args = array())
    {
        ...
        $not_publish = array('content' => __('This post is not yet published.', 'rankology-stats'));
        ...
        // Prepare State
        $post_type = \RANKOLOGY_STATS\Pages::get_post_type($post->ID);

        // Get Number Search every Days
        foreach (array_keys($days_list) as $d) {
            $stats[] = rankology_stats_pages($d, '', $post->ID, null, null, $post_type);
        }

        // Push Basic Chart Data
        $response = array(
            'days'       => $days,
            'title'      => __('Traffic', 'rankology-stats'),
        ...
        $response['visitors'] = apply_filters('rankology_stats_meta_box_post_visitors',
