...
class top_pages_chart
{
    ...
    public static function get($arg = array())
    {
        ...
        if (end($days_time_list) == TimeZone::getCurrentDate("Y-m-d")) {
            $title = sprintf(__('Top 5 Trending Pages in the last %s days', 'rankology-stats'), $count_day);
        } else {
            $title = sprintf(__('Top 5 Trending Pages from %s to %s', 'rankology-stats'), $args['from'], $args['to']);
        }

        $post_type = !empty($args['type']) ? $args['type'] : 'post';

        // Get List Of Top Pages
        $top_pages = rankology_stats_get_top_pages(reset($days_time_list), end($days_time_list), 5, $post_type);
        ...
        foreach ($top_pages[1] as $item) {
            ...
            foreach ($days_time_list as $d) {
                $getStatic         = rankology_stats_pages($d, $item[0], -1, null, null, $post_type);
