...
class Ajax
{
    ...
    function __construct()
    {
        ...
        foreach ($list as $method) {
            add_action('wp_ajax_rankology_stats_' . $method, array($this, $method . '_action_callback'));
        }
    }
    ...
    public function close_notice_action_callback()
    {
        ...
        if (Helper::is_request('ajax') and User::Access('manage') and isset($_REQUEST['notice'])) {
            ...
            // Check Refer Ajax
            check_ajax_referer('wp_rest', 'rkns_nonce');
    ...
    public function close_overview_ads_action_callback()
    {
        ...
        if (Helper::is_request('ajax') and isset($_REQUEST['ads_id'])) {
            ...
            // Update Option
            $get_opt         = get_option('rankology_stats_overview_page_ads');
            $get_opt['view'] = sanitize_text_field($_REQUEST['ads_id']);
            update_option('rankology_stats_overview_page_ads', $get_opt, 'no');
        }
        exit;
    }
    ...
    public function delete_agents_action_callback()
    {
        ...
        if (Helper::is_request('ajax') and User::Access('manage')) {
            ...
            if (isset($_POST['agent-name'])) {
                ...
                // Show Result
                if ($result) {
                    echo sprintf(__('%s agent data deleted successfully.', 'rankology-stats'), '<code>' . esc_attr($agent) . '</code>');
                } else {
                    esc_html_e('No agent data found to remove!', 'rankology-stats');
                }
            } else {
                esc_html_e('Please select the desired items.', 'rankology-stats');
            }
        } else {
            esc_html_e('Access denied!', 'rankology-stats');
        }
    ...
    public function delete_platforms_action_callback()
    {
        ...
        if (Helper::is_request('ajax') and User::Access('manage')) {
            ...
            if (isset($_POST['platform-name'])) {
                ...
                // Return Result
                if ($result) {
                    echo sprintf(__('%s platform data deleted successfully.', 'rankology-stats'), '<code>' . esc_attr($platform) . '</code>');
                } else {
                    esc_html_e('No platform data found to remove!', 'rankology-stats');
                }
            } else {
                esc_html_e('Please select the desired items.', 'rankology-stats');
            }
    ...
    public function delete_ip_action_callback()
    {
        ...
        if (Helper::is_request('ajax') and User::Access('manage')) {
            ...
            if (isset($_POST['ip-address'])) {
                ...
                if ($result) {
                    echo sprintf(__('%s IP data deleted successfully.', 'rankology-stats'), '<code>' . esc_attr($ip_address) . '</code>');
                } else {
                    esc_html_e('No IP address data found to remove!', 'rankology-stats');
                }
            } else {
                esc_html_e('Please select the desired items.', 'rankology-stats');
            }
    ...
    public function purge_visitor_hits_action_callback()
    {
        ...
        if (Helper::is_request('ajax') and User::Access('manage')) {
            ...
            if ($purge_hits < 10) {
                esc_html_e('Number of hits must be greater than or equal to 10!', 'rankology-stats');
            } else {
    ...
    public function update_geoip_database_action_callback()
    {
        ...
        if (Helper::is_request('ajax') and User::Access('manage')) {
            ...
            if ($geoip_name == "country" && Option::get("geoip") !== 'on') {
                esc_html_e('Please first enable GeoIP Collection and save settings!', 'rankology-stats');
            } elseif ($geoip_name == "city" && Option::get("geoip_city") !== 'on') {
                esc_html_e('Please first enable GeoIP City and save settings!', 'rankology-stats');
            }
