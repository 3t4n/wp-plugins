...
class Export
{
    ...
    public function export_data()
    {
        if (isset($_POST['rkns_export'])) {
            ...
            if (!isset($_POST['table-to-export']) || !isset($_POST['export-file-type']) || !isset($_POST['rkns_export_file']) || !wp_verify_nonce($_POST['rkns_export_file'], 'rankology_stats_export_nonce')) {
                exit;
            }

            // Load Library
            if (!class_exists('\ExportData')) {
                include(RANKOLOGY_STATS_DIR . "includes/libraries/ExportData.php");
            }
            ...
            if (User::Access('manage')) {
                ...
                if ($table && $type) {
                    $file_name = 'rankology-stats' . '-' . TimeZone::getCurrentDate('Y-m-d-H-i');
