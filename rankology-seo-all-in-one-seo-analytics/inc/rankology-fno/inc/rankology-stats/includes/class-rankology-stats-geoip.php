...
class GeoIP
{
     ...
     * @var string
     */
    public static $city_cache_object_name = 'rankology_stats_users_city';
    ...
    {
        return wp_normalize_path(path_join(Helper::get_uploads_dir(RANKOLOGY_STATS_UPLOADS_DIR), self::$library[strtolower($pack)]['file'] . '.' . self::$file_extension));
    }
    ...
    public static function getCountry($ip = false, $return = 'isoCode')
    {
        ...
        $user_country = wp_cache_get('country-' . $ip, 'rankology-stats');
        ...
        if ($user_country != false) {
            wp_cache_set('country-' . $ip, $user_country, 'rankology-stats', DAY_IN_SECONDS);
            return $user_country;
        }
        ...
        if (isset($location) and !empty($location)) {
            wp_cache_set('country-' . $ip, $location, 'rankology-stats', DAY_IN_SECONDS);
            return $location;
        }
    ...
    public static function download($pack, $type = "enable")
    {
        ...
        if (false === function_exists('gzopen')) {
            ...
            return array_merge($result, array("notice" => __('Error the gzopen() function do not exist!', 'rankology-stats')));
        }
        ...
        $download_url = apply_filters('rankology_stats_geo_ip_download_url', $download_url, GeoIP::$library[$pack]['source'], $pack);
        ...
        if (is_wp_error($response)) {
            \RANKOLOGY_Stats::log(array('code' => 'download_geoip', 'type' => $pack, 'message' => $response->get_error_message()));
        ...
        if (wp_remote_retrieve_response_code($response) != '200') {
            return array_merge($result, array("notice" => sprintf(__('Error to get %s from %s', 'rankology-stats'), $pack, $download_url)));
        }
        ...
        if (!file_exists($upload_dir['basedir'] . '/' . RANKOLOGY_STATS_UPLOADS_DIR)) {
            if (!@mkdir($upload_dir['basedir'] . '/' . RANKOLOGY_STATS_UPLOADS_DIR, 0755)) {
                if ($type == "enable") {
                    Option::update(GeoIP::$library[$pack]['opt'], '');
                }

                return array_merge($result, array("notice" => sprintf(__('Error creating GeoIP database directory, make sure your web server has permissions to create directories in: %s', 'rankology-stats'), $upload_dir['basedir'])));
            }
        }

        if (!is_writable($upload_dir['basedir'] . '/' . RANKOLOGY_STATS_UPLOADS_DIR)) {
            if ($type == "enable") {
                Option::update(GeoIP::$library[$pack]['opt'], '');
            }

            return array_merge($result, array("notice" => sprintf(__('Error setting permissions of the GeoIP database directory, make sure your web server has permissions to write to directories in : %s', 'rankology-stats'), $upload_dir['basedir'])));
        }
        ...
        if (is_wp_error($TempFile)) {
            ...
            return array_merge($result, array("notice" => sprintf(__('Error downloading GeoIP database from: %s - %s', 'rankology-stats'), $download_url, $TempFile->get_error_message())));
        } else {
            ...
            if (!$ZipHandle) {
                ...
                unlink($TempFile);
                return array_merge($result, array("notice" => sprintf(__('Error could not open downloaded GeoIP database for reading: %s', 'rankology-stats'), $TempFile)));
            } else {
                ...
                if (!$DBfh) {
                    ...
                    unlink($TempFile);
                    return array_merge($result, array("notice" => sprintf(__('Error could not open destination GeoIP database for writing %s', 'rankology-stats'), $DBFile)));
                } else {
                    ...
                    $result["status"] = true;
                    $result["notice"] = __('GeoIP Database updated successfully!', 'rankology-stats');
        ...
        if (Option::get('geoip_report') == true) {
            ...
            Helper::send_mail(
                ...
                __('GeoIP update on', 'rankology-stats') . ' ' . get_bloginfo('name'),
                $result['notice'],
                true,
                array("email_title" => __('GeoIP update on', 'rankology-stats') . ' <a href="' . get_bloginfo('url') . '" target="_blank" style="text-decoration: underline; color: #999999; font-family: Nunito; font-size: 13px; font-weight: 400; line-height: 150%;">' . get_bloginfo('name') . '</a>')
            );
        }
    ...
    public static function Update_GeoIP_Visitor()
    {
        ...
        if ($reader === false) {
            return array('status' => false, 'data' => __('Unable to load the GeoIP database, make sure you have downloaded it in the settings page.', 'rankology-stats'));
        }
        ...
        return array('status' => true, 'data' => sprintf(__('Updated %s GeoIP records in the visitors database.', 'rankology-stats'), $count));
    }
    ...
    public static function getCity($ip = false, $return = 'name')
    {
        ...
        $user_city = wp_cache_get('city-' . $ip, 'rankology-stats');
        if ($user_city != false) {
            return $user_city;
        }
        ...
        // Default City Name
        $default_city = __('Unknown', 'rankology-stats');
    ...
    public static function setCacheCity($ip, $city, $opt = false)
    {
        ...
        wp_cache_set('city-' . $ip, $city, 'rankology-stats', DAY_IN_SECONDS);
