...
class Install
{
    ...
    public function __construct()
    {
        ...
        add_filter('plugin_action_links_' . plugin_basename(RANKOLOGY_STATS_MAIN_FILE), array($this, 'settings_links'), 10, 2);
    ...
    public function install($network_wide)
    {
        ...
        update_option('rankology_stats_plugin_version', RANKOLOGY_STATS_VERSION);
    }
    ...
    {
        if (is_plugin_active_for_network(plugin_basename(RANKOLOGY_STATS_MAIN_FILE))) {
            switch_to_blog($blog_id);
            self::table_sql();
            restore_current_blog();
        }
    ...
    public function settings_links($links, $file)
    {
        if (User::Access('manage')) {
            array_unshift($links, '<a href="' . Menus::admin_url('settings') . '">' . __('Settings', 'rankology-stats') . '</a>');
        }
        return $links;
    }
    ...
    {
        if ($file == plugin_basename(RANKOLOGY_STATS_MAIN_FILE)) {
            $plugin_url = '';

            $links[]  = '<a href="' . $plugin_url . '" target="_blank" title="' . __('Click here to visit the plugin on WordPress.org', 'rankology-stats') . '">' . __('Visit WordPress.org page', 'rankology-stats') . '</a>';
            $rate_url = '';
            $links[]  = '<a href="' . $rate_url . '" target="_blank" title="' . __('Click here to rate and review this plugin on WordPress.org', 'rankology-stats') . '">' . __('Rate this plugin', 'rankology-stats') . '</a>';
        }
    ...
    public function plugin_upgrades()
    {
        ...
        $installed_version = get_option('rankology_stats_plugin_version');
        if ($installed_version == RANKOLOGY_STATS_VERSION) {
            return;
        }
         ...
         * Added User_id column in rankology_stats_visitor Table
         *
        ...
        if (isset($installed_version) and version_compare($installed_version, '13.0', '<=')) {
            $wpdb->query("DELETE FROM `{$wpdb->usermeta}` WHERE `meta_key` = 'meta-box-order_toplevel_page_rkns_overview_page'");
        }
    ...
    /**
     * Update WordPress Page Type for older rankology-stats Version
     *
     * 
     *
     * -- List Methods ---
     ...
     * is_require_update_page        -> Check Rankology-stats require update page table
    ...
    public static function init_page_type_updater()
    {
        ...
        if (self::is_require_update_page() === true) {
            ...
            add_action('admin_notices', function () {
                echo '<div class="notice notice-info is-dismissible" id="rankology-stats-update-page-area" style="display: none;">';
                echo '<p style="margin-top: 17px; float:' . (is_rtl() ? 'right' : 'left') . '">';
                echo __('Rankology Stats database requires upgrade.', 'rankology-stats');
                echo '</p>';
                echo '<div style="float:' . (is_rtl() ? 'left' : 'right') . '">';
                echo '<button type="button" id="rkns-upgrade-db" class="button button-primary" style="padding: 20px;line-height: 0px;box-shadow: none !important;border: 0px !important;margin: 10px 0;"/>' . __('Upgrade Database', 'rankology-stats') . '</button>';
                echo '</div>';
                ...
                <script>
                    jQuery(document).ready(function () {
                        ...
                        jQuery(window).load(function () {
                            jQuery("#rankology-stats-update-page-area").fadeIn(2000);
                            jQuery("#rankology-stats-update-page-area button.notice-dismiss").hide();
                        });

                        // Update Page type function
                        function rankology_stats_update_page_type() {

                            //Complete Progress
                            let rkns_end_progress = `<div id="rkns_end_process" style="display:none;">`;
                            rkns_end_progress += `<p>`;
                            rkns_end_progress += `<?php esc_html_e('Database upgrade operation completed!', 'rankology-stats'); ?>`;
                            rkns_end_progress += `</p>`;
                            rkns_end_progress += `</div>`;
                            rkns_end_progress += `<button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button>`;
                            ...
                            jQuery.ajax({
                                ...
                                cache: false,
                                data: {
                                    'action': 'rankology_stats_update_post_type_db',
                                ...
                                success: function (data) {
                                    if (data.process_status === "complete") {
                                        ...
                                        // Get Process Area
                                        let rkns_notice_area = jQuery("#rankology-stats-update-page-area");
                                        //Add Html Content
                                        rkns_notice_area.html(rkns_end_progress);
                                        //Fade in content
                                        jQuery("#rkns_end_process").fadeIn(2000);
                                        //enable demiss button
                                        rkns_notice_area.removeClass('notice-info').addClass('notice-success');
                                    } else {

                                        //Get number Process
                                        jQuery("span#rkns_num_page_process").html(data.number_process);
                                        //Get process Percentage
                                        jQuery("progress#rkns_upgrade_html_progress").attr("value", data.percentage);
                                        jQuery("span#rkns_num_percentage").html(data.percentage);
                                        //again request
                                        rankology_stats_update_page_type();
                                    }
                                },
                                error: function () {
                                    jQuery("#rankology-stats-update-page-area").html('<p><?php esc_html_e('Error occurred during operation. Please refresh the page.', 'rankology-stats'); ?></p>');
                                }
                            });
                        }

                        //Click Start Progress
                        jQuery(document).on('click', 'button#rkns-upgrade-db', function (e) {
                            e.preventDefault();

                            // Added Progress Html
                            let rkns_progress = `<div id="rkns_process_upgrade" style="display:none;"><p>`;
                            rkns_progress += `<?php esc_html_e('Please don\'t close the browser window until the database operation was completed.', 'wp-statistic'); ?>`;
                            rkns_progress += `</p><p><b>`;
                            rkns_progress += `<?php echo __('Item processed', 'rankology-stats'); ?>`;
                            rkns_progress += ` : <span id="rkns_num_page_process">0</span> / <?php echo number_format(self::get_require_number_update()); ?> &nbsp;<span class="rkns-text-warning">(<span id="rkns_num_percentage">0</span>%)</span></b></p>`;
                            rkns_progress += '<p><progress id="rkns_upgrade_html_progress" value="0" max="100" style="height: 20px;width: 100%;"></progress></p></div>';

                            // set new Content
                            jQuery("#rankology-stats-update-page-area").html(rkns_progress);
                            jQuery("#rkns_process_upgrade").fadeIn(2000);
                        ...
                        //Remove Notice event
                        jQuery(document).on('click', '#rankology-stats-update-page-area button.notice-dismiss', function (e) {
                            e.preventDefault();
                            jQuery("#rankology-stats-update-page-area").fadeOut('normal');
                        });
                    });
                </script>
                <?php
        ...
        add_action('wp_ajax_rankology_stats_update_post_type_db', function () {
            global $wpdb;
                ...
                if (self::is_require_update_page() === true) {
                    ...
                    if ($number_process > 0) {
                        ...
                        if ($number_all > $number_per_query) {
                            ...
                            update_option('rankology_stats_update_page_type', 'yes');
                        }
                    }
                } else {
    ...
    public static function is_require_update_page()
    {
        ...
        $opt_name = 'rankology_stats_update_page_type';
