...
class Exclusion
{
    ...
    {
        return array(
            'ajax'            => __('Ajax', 'rankology-stats'),
            'cronjob'         => __('Cron job', 'rankology-stats'),
            'CrawlerDetect'   => __('Crawler Detect', 'rankology-stats'),
            'robot'           => __('Robot', 'rankology-stats'),
            'BrokenFile'      => __('Broken Link', 'rankology-stats'),
            'ip match'        => __('IP Match', 'rankology-stats'),
            'self referral'   => __('Self Referral', 'rankology-stats'),
            'login page'      => __('Login Page', 'rankology-stats'),
            'admin page'      => __('Admin Page', 'rankology-stats'),
            'referrer_spam'   => __('Referrer Spam', 'rankology-stats'),
            'feed'            => __('Feed', 'rankology-stats'),
            '404'             => __('404', 'rankology-stats'),
            'excluded url'    => __('Excluded URL', 'rankology-stats'),
            'user role'       => __('User Role', 'rankology-stats'),
            'hostname'        => __('Host name', 'rankology-stats'),
            'geoip'           => __('GeoIP', 'rankology-stats'),
            'honeypot'        => __('Honeypot', 'rankology-stats'),
            'robot_threshold' => __('Robot threshold', 'rankology-stats')
        );
    }
    ...
    public static function check()
    {
        ...
        // Get List Of Exclusion Rankology Stats
        $exclusion_list = apply_filters('rankology_stats_exclusion_list', array_keys(Exclusion::exclusion_list()));
        ...
        return apply_filters('rankology_stats_exclusion', $exclude);
    }

    /**
     * Record Exclusion in Rankology Stats DB.
     *
    ...
    public static function record($exclusion = array())
    {
        ...
        if (!$result) {
            ...
            do_action('rankology_stats_save_exclusion', $exclusion, $wpdb->insert_id);
        }
    }
    ...
    public static function exclusion_user_role()
    {
        ...
        if (Helper::is_rest_request()) {
            $user_id = $GLOBALS['rankology_stats_user_id'];
            if ($user_id) {
    ...
    public static function exclusion_hostname()
    {
        ...
        if (count($excluded_host) > 0) {
            $transient_name = 'rkns_excluded_hostname_to_ip_cache';
            ...
            if (in_array($RANKOLOGY_Stats->ip, $hostname_cache)) {
                return true;
            }
        }
