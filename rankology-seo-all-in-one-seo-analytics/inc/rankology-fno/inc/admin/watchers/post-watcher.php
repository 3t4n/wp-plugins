...
add_action( 'wp_trash_post', 'rankology_watcher_post_trash' );
 ...
 * 
 */
function rankology_watcher_post_trash($post_id)
{

	$can_autoredirect = rankology_can_post_autoredirect($post_id);
	...
	$url = rankology_get_permalink_for_updated_post($post_id);
	...
	$message =
	sprintf(
		__('<p>We have detected that you have deleted a post (<code>%s</code>).</p>', 'wp-rankology'),
		$url
	);
...
add_action( 'post_updated', 'rankology_watcher_slug_change', 12, 3 );
 ...
 * 
 */
function rankology_watcher_slug_change( $post_id, $post, $post_before ){
	...
	$url_post = rankology_get_permalink_for_updated_post($post);
	$url_post_before = rankology_get_permalink_for_updated_post( $post_before );
	$notices = rankology_get_option_post_need_redirects();
	...
	$authorize = apply_filters('rankology_watcher_slug_change_can_create_notification', $authorize, $post_id, $post, $post_before);
...
add_filter('trash_to_publish', 'rankology_remove_notice_if_needed');

function rankology_remove_notice_if_needed($post){
