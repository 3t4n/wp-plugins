 ...
 */
if ( ! class_exists('RANKOLOGY_Importer_Interface', false)) {
    include_once dirname(__FILE__) . '/class-csv-importer-interface.php';
}

/**
 * RANKOLOGY_Importer Class.
 */
abstract class RANKOLOGY_Importer implements RANKOLOGY_Importer_Interface {
    /**
     * CSV file.
     *
     * @var string
    ...
    public function get_parsed_data() {
        return apply_filters('rankology_csv_importer_parsed_data', $this->parsed_data, $this->get_raw_data());
    }
    ...
    protected function process_item($data) {
        try {
            do_action('rankology_csv_import_before_process_item', $data);
            ...
            return new WP_Error('rankology_csv_importer_error', $e->getMessage(), ['status' => $e->getCode()]);
        }
    }
    ...
    protected function memory_exceeded() {
        ...
        return apply_filters('rankology_metadata_importer_memory_exceeded', $return);
    }
    ...
    protected function time_exceeded() {
        $finish = $this->start_time + apply_filters('rankology_metadata_importer_default_time_limit', 20); // 20 seconds
        $return = false;
        if (time() >= $finish) {
            $return = true;
        }

        return apply_filters('rankology_metadata_importer_time_exceeded', $return);
    }
...
/**
 * RANKOLOGY_CSV_Importer Class.
 */
class RANKOLOGY_CSV_Importer extends RANKOLOGY_Importer {
    /**
    ...
    protected function read_file() {
        if ( ! RANKOLOGY_CSV_Setup_Wizard_Controller::is_file_valid_csv($this->file)) {
            wp_die(esc_html__('Invalid file type. The importer supports CSV and TXT file formats.', 'wp-rankology'));
        }
    ...
    public function parse_images_field($value) {
        ...
        $images    = '';
        $separator = apply_filters('rankology_metadata_import_image_separator', ',');
     ...
     *
     * By default is applied rankology_clean() to all not listed fields
    ...
    protected function get_formating_callback() {
        /**
         * Columns not mentioned here will get parsed with 'rankology_clean'.
        ...
        return apply_filters('rankology_metadata_importer_formatting_callbacks', $callbacks, $this);
    }
    ...
    protected function set_parsed_data() {
        ...
        foreach ($this->raw_data as $row_index => $row) {
            ...
            do_action('rankology_metadata_importer_before_set_parsed_data', $row, $mapped_keys);
            ...
            $this->parsed_data[] = apply_filters('rankology_csv_importer_parsed_data', $data, $this);
        }
    }
    ...
    protected function get_row_id($parsed_data) {
        ...
        if ($id) {
            ...
            $row_data[] = sprintf(__('ID %d', 'wp-rankology'), $id);
        }
    ...
    public function import() {
        ...
        foreach ($this->parsed_data as $parsed_data_key => $parsed_data) {
            do_action('rankology_metadata_import_before_import', $parsed_data);
