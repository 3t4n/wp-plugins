...
function rkseo_print_section_instant_indexing_general() {
?>
...
foreach ($indexing_plugins as $key => $value) {
    if (is_plugin_active($key)) { ?>
        <div class="rankology-notice is-warning">
            <h3><?php printf(__('We noticed that you use <strong>%s</strong> plugin.', 'wp-rankology'), $value); ?></h3>
            <p><?php printf(__('To prevent any conflicts with our Indexing feature, please disable it.', 'wp-rankology')); ?></p>
            <a class="btn btnPrimary" href="<?php echo admin_url('plugins.php'); ?>"><?php esc_html_e('Fix this!', 'wp-rankology'); ?></a>
        </div>
        <?php
    }
}
...
function rkseo_print_section_instant_indexing_settings() { ?>
<div class="rkseo-section-header">
    <h2>
