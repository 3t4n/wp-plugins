...
function rankology_premium_seo_pack_migration() {
    check_ajax_referer('rankology_premium_seo_pack_migrate_nonce', $_POST['_ajax_nonce'], true);
                ...
                foreach ($premium_query_terms as $taxonomies => $taxonomie) {
                    foreach ($taxonomie as $term_id => $term_value) {
                        if ('' != $term_value['psp_meta']['title']) { //Import title tag
                            update_term_meta($term_id, '_rankology_titles_title', $term_value['psp_meta']['title']);
                        }
                        if ('' != $term_value['psp_meta']['description']) { //Import meta desc
                            update_term_meta($term_id, '_rankology_titles_desc', $term_value['psp_meta']['description']);
                        }
                        if ('' != $term_value['psp_meta']['facebook_titlu']) { //Import Facebook Title
                            update_term_meta($term_id, '_rankology_social_fb_title', $term_value['psp_meta']['facebook_titlu']);
                        }
                        if ('' != $term_value['psp_meta']['facebook_desc']) { //Import Facebook Desc
                            update_term_meta($term_id, '_rankology_social_fb_desc', $term_value['psp_meta']['facebook_desc']);
                        }
                        if ('' != $term_value['psp_meta']['facebook_image']) { //Import Facebook Image
                            update_term_meta($term_id, '_rankology_social_fb_img', $term_value['psp_meta']['facebook_image']);
                        }
                        ...
                        if ('' != $term_value['psp_meta']['canonical']) { //Import Canonical URL
                            update_term_meta($term_id, '_rankology_robots_canonical', $term_value['psp_meta']['canonical']);
                        }
                    }
                }
            }
                ...
                foreach ($premium_query as $post) {
                    ...
                    if ( ! empty($psp_meta)) {
                        if ( ! empty($psp_meta['title'])) { //Import title tag
                            update_post_meta($post->ID, '_rankology_titles_title', $psp_meta['title']);
                        }
                        if ( ! empty($psp_meta['description'])) { //Import meta desc
                            update_post_meta($post->ID, '_rankology_titles_desc', $psp_meta['description']);
                        }
                        if ( ! empty($psp_meta['facebook_titlu'])) { //Import Facebook Title
                            update_post_meta($post->ID, '_rankology_social_fb_title', $psp_meta['facebook_titlu']);
                        }
                        if ( ! empty($psp_meta['facebook_desc'])) { //Import Facebook Desc
                            update_post_meta($post->ID, '_rankology_social_fb_desc', $psp_meta['facebook_desc']);
                        }
                        if ( ! empty($psp_meta['facebook_image'])) { //Import Facebook Image
                            update_post_meta($post->ID, '_rankology_social_fb_img', $psp_meta['facebook_image']);
                        }
                        ...
                        if ( ! empty($psp_meta['canonical'])) { //Import Canonical URL
                            update_post_meta($post->ID, '_rankology_robots_canonical', $psp_meta['canonical']);
                        }
                        if ( ! empty($psp_meta['mfocus_keyword'])) { //Import Focus Keywords
                            $target_kw = preg_split('/\r\n|\r|\n/', $psp_meta['mfocus_keyword']);

                            update_post_meta($post->ID, '_rankology_analysis_target_kw', implode(',', $target_kw));
                        }
                    }
                }
            }
