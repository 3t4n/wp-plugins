...
//Yoast migration
///////////////////////////////////////////////////////////////////////////////////////////////////
function rankology_yoast_migration() {
    check_ajax_referer('rankology_yoast_migrate_nonce', $_POST['_ajax_nonce'], true);
                ...
                foreach ($yoast_query_terms as $taxonomies => $taxonomie) {
                    foreach ($taxonomie as $term_id => $term_value) {
                        if ('' != $term_value['wpseo_title']) { //Import title tag
                            update_term_meta($term_id, '_rankology_titles_title', $term_value['wpseo_title']);
                        }
                        if ('' != $term_value['wpseo_desc']) { //Import meta desc
                            update_term_meta($term_id, '_rankology_titles_desc', $term_value['wpseo_desc']);
                        }
                        if ('' != $term_value['wpseo_opengraph-title']) { //Import Facebook Title
                            update_term_meta($term_id, '_rankology_social_fb_title', $term_value['wpseo_opengraph-title']);
                        }
                        if ('' != $term_value['wpseo_opengraph-description']) { //Import Facebook Desc
                            update_term_meta($term_id, '_rankology_social_fb_desc', $term_value['wpseo_opengraph-description']);
                        }
                        if ('' != $term_value['wpseo_opengraph-image']) { //Import Facebook Image
                            update_term_meta($term_id, '_rankology_social_fb_img', $term_value['wpseo_opengraph-image']);
                        }
                        if ('' != $term_value['wpseo_twitter-title']) { //Import Twitter Title
                            update_term_meta($term_id, '_rankology_social_twitter_title', $term_value['wpseo_twitter-title']);
                        }
                        if ('' != $term_value['wpseo_twitter-description']) { //Import Twitter Desc
                            update_term_meta($term_id, '_rankology_social_twitter_desc', $term_value['wpseo_twitter-description']);
                        }
                        if ('' != $term_value['wpseo_twitter-image']) { //Import Twitter Image
                            update_term_meta($term_id, '_rankology_social_twitter_img', $term_value['wpseo_twitter-image']);
                        }
                        ...
                        if ('' != $term_value['wpseo_canonical']) { //Import Canonical URL
                            update_term_meta($term_id, '_rankology_robots_canonical', $term_value['wpseo_canonical']);
                        }
                    }
                }
            }
            ...
            if ($yoast_query) {
                foreach ($yoast_query as $post) {
                    if ('' != get_post_meta($post->ID, '_yoast_wpseo_title', true)) { //Import title tag
                        update_post_meta($post->ID, '_rankology_titles_title', get_post_meta($post->ID, '_yoast_wpseo_title', true));
                    }
                    if ('' != get_post_meta($post->ID, '_yoast_wpseo_metadesc', true)) { //Import meta desc
                        update_post_meta($post->ID, '_rankology_titles_desc', get_post_meta($post->ID, '_yoast_wpseo_metadesc', true));
                    }
                    if ('' != get_post_meta($post->ID, '_yoast_wpseo_opengraph-title', true)) { //Import Facebook Title
                        update_post_meta($post->ID, '_rankology_social_fb_title', get_post_meta($post->ID, '_yoast_wpseo_opengraph-title', true));
                    }
                    if ('' != get_post_meta($post->ID, '_yoast_wpseo_opengraph-description', true)) { //Import Facebook Desc
                        update_post_meta($post->ID, '_rankology_social_fb_desc', get_post_meta($post->ID, '_yoast_wpseo_opengraph-description', true));
                    }
                    if ('' != get_post_meta($post->ID, '_yoast_wpseo_opengraph-image', true)) { //Import Facebook Image
                        update_post_meta($post->ID, '_rankology_social_fb_img', get_post_meta($post->ID, '_yoast_wpseo_opengraph-image', true));
                    }
                    if ('' != get_post_meta($post->ID, '_yoast_wpseo_twitter-title', true)) { //Import Twitter Title
                        update_post_meta($post->ID, '_rankology_social_twitter_title', get_post_meta($post->ID, '_yoast_wpseo_twitter-title', true));
                    }
                    if ('' != get_post_meta($post->ID, '_yoast_wpseo_twitter-description', true)) { //Import Twitter Desc
                        update_post_meta($post->ID, '_rankology_social_twitter_desc', get_post_meta($post->ID, '_yoast_wpseo_twitter-description', true));
                    }
                    if ('' != get_post_meta($post->ID, '_yoast_wpseo_twitter-image', true)) { //Import Twitter Image
                        update_post_meta($post->ID, '_rankology_social_twitter_img', get_post_meta($post->ID, '_yoast_wpseo_twitter-image', true));
                    }
                    ...
                    if ('' != get_post_meta($post->ID, '_yoast_wpseo_canonical', true)) { //Import Canonical URL
                        update_post_meta($post->ID, '_rankology_robots_canonical', get_post_meta($post->ID, '_yoast_wpseo_canonical', true));
                    }
                    if ('' != get_post_meta($post->ID, '_yoast_wpseo_focuskw', true) || '' != get_post_meta($post->ID, '_yoast_wpseo_focuskeywords', true)) { //Import Focus Keywords
                        ...
                        update_post_meta($post->ID, '_rankology_analysis_target_kw', implode(',', $y_fkws_clean));
                    }
                    ...
                    if (class_exists('WPSEO_Primary_Term')) {
                        ...
                        if ('' != $primary_term && is_int($primary_term)) {
                            update_post_meta($post->ID, '_rankology_robots_primary_cat', $primary_term);
                        }
                    }
                }
            }
...
add_action('wp_ajax_rankology_yoast_migration', 'rankology_yoast_migration');
