...
<form method="post" action="<?php echo admin_url('options.php'); ?>" class="rankology-option rankology-form-heading">
    <?php
    echo $this->rankology_feature_title('xml-sitemap');
    settings_fields('rankology_xml_sitemap_option_group'); ?>
    <div class="rankology-sub-tabs">
        <?php
        $current_tab = '';
        $plugin_settings_tabs = [
            'tab_rankology_xml_sitemap_general' => __('XML Enable', 'wp-rankology'),
            'tab_rankology_html_sitemap' => __('HTML Sitemap', 'wp-rankology'),
            'tab_rankology_xml_sitemap_post_types' => __('Post Types', 'wp-rankology'),
            'tab_rankology_xml_sitemap_taxonomies' => __('Taxonomies', 'wp-rankology'),
                ...
                <div class="nav-tab-wrapper">
                    ...
                    foreach ($plugin_settings_tabs as $tab_key => $tab_caption) {
                        ...
                        <div id="<?php echo $tab_key ?>" class="rankology-sub-content <?php if ($activeState == 1) {
                            ...
                            if ($activeState == 1) {
                                do_settings_sections('rankology-settings-admin-xml-sitemap-general');
                                rkseo_submit_button(__('Save changes', 'wp-rankology'));
                            }
                            if ($activeState == 2) {
                                do_settings_sections('rankology-settings-admin-xml-sitemap-post-types');
                                rkseo_submit_button(__('Save changes', 'wp-rankology'));
                            }
                            if ($activeState == 3) {
                                do_settings_sections('rankology-settings-admin-xml-sitemap-taxonomies');
                                rkseo_submit_button(__('Save changes', 'wp-rankology'));
                            }
                            if ($activeState == 4) {
                                do_settings_sections('rankology-settings-admin-html-sitemap');
                                rkseo_submit_button(__('Save changes', 'wp-rankology'));
                            }
