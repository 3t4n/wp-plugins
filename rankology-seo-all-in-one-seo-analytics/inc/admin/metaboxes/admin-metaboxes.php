...
function rankology_display_seo_metaboxe()
{
    ...
    function rankology_init_metabox()
    {
        ...
        if (!empty($rankology_get_post_types) && !rankology_get_service('EnqueueModuleMetabox')->canEnqueue()) {
            foreach ($rankology_get_post_types as $key => $value) {
                add_meta_box('rankology_cpt', __('Rankology On Page SEO', 'wp-rankology'), 'rankology_cpt', $key, 'normal', $metaboxe_position);
            }
        }
        add_meta_box('rankology_cpt', __('On-Page SEO', 'wp-rankology'), 'rankology_cpt', 'rankology_404', 'normal', $metaboxe_position);
    }
    ...
    function rankology_cpt($post)
    {
        ...
        if ('rankology_404' != $typenow) {
            ...
            wp_enqueue_script('rankology-cpt-counters-js', RANKOLOGY_ASSETS_DIR . '/js/rankology-counters.min.js', ['jquery', 'jquery-ui-tabs', 'jquery-ui-accordion'], RANKOLOGY_VERSION, true);
...
//        wp_localize_script('rankology-cpt-counters-js', 'rankologyAjaxInspectUrl', $rankology_inspect_url);
    ...
    {
        //Nonce
        if (!isset($_POST['rankology_cpt_nonce']) || !wp_verify_nonce($_POST['rankology_cpt_nonce'], plugin_basename(__FILE__))) {
            return $post_id;
        }
        ...
        if ('attachment' !== get_post_type($post_id)) {
            ...
            if (in_array('title-tab', $seo_tabs)) {
                if (!empty($_POST['rankology_titles_title'])) {
                    update_post_meta($post_id, '_rankology_titles_title', esc_html($_POST['rankology_titles_title']));
                } else {
                    delete_post_meta($post_id, '_rankology_titles_title');
                }
                if (!empty($_POST['rankology_titles_desc'])) {
                    update_post_meta($post_id, '_rankology_titles_desc', esc_html($_POST['rankology_titles_desc']));
                } else {
...
//                if ('attachment' !== get_post_type($post_id)) {
                    if (isset($_POST['rankology_analysis_target_kw'])) {
                        update_post_meta($post_id, '_rankology_analysis_target_kw', esc_html($_POST['rankology_analysis_target_kw']));
                    }

                if (isset($_POST['rankology_google_suggest_kw'])) {
                    update_post_meta($post_id, '_rankology_google_suggest_kw', esc_html($_POST['rankology_google_suggest_kw']));
                }
...
//                        if (!empty($elementor)) {
//                            if (isset($_POST['rankology_analysis_target_kw'])) {
//                                $elementor['_rankology_analysis_target_kw'] = esc_html($_POST['rankology_analysis_target_kw']);
//                            }
...
//                    }
            ...
            if (in_array('advanced-tab', $seo_tabs)) {
                ...
                if (!empty($_POST['rankology_robots_canonical'])) {
                    update_post_meta($post_id, '_rankology_robots_canonical', esc_html($_POST['rankology_robots_canonical']));
                } else {
                    delete_post_meta($post_id, '_rankology_robots_canonical');
                }
                if (!empty($_POST['rankology_robots_primary_cat'])) {
                    update_post_meta($post_id, '_rankology_robots_primary_cat', esc_html($_POST['rankology_robots_primary_cat']));
                } else {
            ...
            if (in_array('social-tab', $seo_tabs)) {
                ...
                if (!empty($_POST['rankology_social_fb_title'])) {
                    update_post_meta($post_id, '_rankology_social_fb_title', esc_html($_POST['rankology_social_fb_title']));
                } else {
                    delete_post_meta($post_id, '_rankology_social_fb_title');
                }
                if (!empty($_POST['rankology_social_fb_desc'])) {
                    update_post_meta($post_id, '_rankology_social_fb_desc', esc_html($_POST['rankology_social_fb_desc']));
                } else {
                    delete_post_meta($post_id, '_rankology_social_fb_desc');
                }
                if (!empty($_POST['rankology_social_fb_img'])) {
                    update_post_meta($post_id, '_rankology_social_fb_img', esc_html($_POST['rankology_social_fb_img']));
                } else {
                ...
                if (!empty($_POST['rankology_social_fb_img_attachment_id']) && !empty($_POST['rankology_social_fb_img'])) {
                    update_post_meta($post_id, '_rankology_social_fb_img_attachment_id', esc_html($_POST['rankology_social_fb_img_attachment_id']));
                } else {
                ...
                if (!empty($_POST['rankology_social_fb_img_width']) && !empty($_POST['rankology_social_fb_img'])) {
                    update_post_meta($post_id, '_rankology_social_fb_img_width', esc_html($_POST['rankology_social_fb_img_width']));
                } else {
                ...
                if (!empty($_POST['rankology_social_fb_img_height']) && !empty($_POST['rankology_social_fb_img'])) {
                    update_post_meta($post_id, '_rankology_social_fb_img_height', esc_html($_POST['rankology_social_fb_img_height']));
                } else {
                ...
                if (!empty($_POST['rankology_social_twitter_title'])) {
                    update_post_meta($post_id, '_rankology_social_twitter_title', esc_html($_POST['rankology_social_twitter_title']));
                } else {
                    delete_post_meta($post_id, '_rankology_social_twitter_title');
                }
                if (!empty($_POST['rankology_social_twitter_desc'])) {
                    update_post_meta($post_id, '_rankology_social_twitter_desc', esc_html($_POST['rankology_social_twitter_desc']));
                } else {
                    delete_post_meta($post_id, '_rankology_social_twitter_desc');
                }
                if (!empty($_POST['rankology_social_twitter_img'])) {
                    update_post_meta($post_id, '_rankology_social_twitter_img', esc_html($_POST['rankology_social_twitter_img']));
                } else {
                ...
                if (!empty($_POST['rankology_social_twitter_img_attachment_id']) && !empty($_POST['rankology_social_twitter_img'])) {
                    update_post_meta($post_id, '_rankology_social_twitter_img_attachment_id', esc_html($_POST['rankology_social_twitter_img_attachment_id']));
                } else {
                ...
                if (!empty($_POST['rankology_social_twitter_img_width']) && !empty($_POST['rankology_social_twitter_img'])) {
                    update_post_meta($post_id, '_rankology_social_twitter_img_width', esc_html($_POST['rankology_social_twitter_img_width']));
                } else {
                ...
                if (!empty($_POST['rankology_social_twitter_img_height']) && !empty($_POST['rankology_social_twitter_img'])) {
                    update_post_meta($post_id, '_rankology_social_twitter_img_height', esc_html($_POST['rankology_social_twitter_img_height']));
                } else {
            ...
            if (in_array('redirect-tab', $seo_tabs)) {
                if (isset($_POST['rankology_redirections_type'])) {
                    update_post_meta($post_id, '_rankology_redirections_type', $_POST['rankology_redirections_type']);
                }
                if (!empty($_POST['rankology_redirections_value'])) {
                    update_post_meta($post_id, '_rankology_redirections_value', esc_html($_POST['rankology_redirections_value']));
                } else {
                    delete_post_meta($post_id, '_rankology_redirections_value');
                }
                if (isset($_POST['rankology_redirections_param'])) {
                    update_post_meta($post_id, '_rankology_redirections_param', esc_html($_POST['rankology_redirections_param']));
                }
                ...
                if (isset($_POST['rankology_redirections_logged_status'])) {
                    update_post_meta($post_id, '_rankology_redirections_logged_status', $_POST['rankology_redirections_logged_status']);
                } else {
            ...
            if (did_action('elementor/loaded')) {
                ...
                if (!empty($elementor)) {
                    if (isset($_POST['rankology_titles_title'])) {
                        $elementor['_rankology_titles_title'] = esc_html($_POST['rankology_titles_title']);
                    }
                    if (isset($_POST['rankology_titles_desc'])) {
                        $elementor['_rankology_titles_desc'] = esc_html($_POST['rankology_titles_desc']);
                    }
                    if (isset($_POST['rankology_robots_index'])) {
                    ...
                    if (isset($_POST['rankology_robots_canonical'])) {
                        $elementor['_rankology_robots_canonical'] = esc_html($_POST['rankology_robots_canonical']);
                    }
                    if (isset($_POST['rankology_robots_primary_cat'])) {
                        $elementor['_rankology_robots_primary_cat'] = esc_html($_POST['rankology_robots_primary_cat']);
                    }
                    if (isset($_POST['rankology_social_fb_title'])) {
                        $elementor['_rankology_social_fb_title'] = esc_html($_POST['rankology_social_fb_title']);
                    }
                    if (isset($_POST['rankology_social_fb_desc'])) {
                        $elementor['_rankology_social_fb_desc'] = esc_html($_POST['rankology_social_fb_desc']);
                    }
                    if (isset($_POST['rankology_social_fb_img'])) {
                        $elementor['_rankology_social_fb_img'] = esc_html($_POST['rankology_social_fb_img']);
                    }
                    if (isset($_POST['rankology_social_twitter_title'])) {
                        $elementor['_rankology_social_twitter_title'] = esc_html($_POST['rankology_social_twitter_title']);
                    }
                    if (isset($_POST['rankology_social_twitter_desc'])) {
                        $elementor['_rankology_social_twitter_desc'] = esc_html($_POST['rankology_social_twitter_desc']);
                    }
                    if (isset($_POST['rankology_social_twitter_img'])) {
                        $elementor['_rankology_social_twitter_img'] = esc_html($_POST['rankology_social_twitter_img']);
                    }
                    if (isset($_POST['rankology_redirections_type'])) {
                        $elementor['_rankology_redirections_type'] = esc_html($_POST['rankology_redirections_type']);
                    }
                    if (isset($_POST['rankology_redirections_value'])) {
                        $elementor['_rankology_redirections_value'] = esc_html($_POST['rankology_redirections_value']);
                    }
                    if (isset($_POST['rankology_redirections_param'])) {
                        $elementor['_rankology_redirections_param'] = esc_html($_POST['rankology_redirections_param']);
                    }
                    if (isset($_POST['rankology_redirections_enabled'])) {
...
function rankology_display_ca_metaboxe()
{
    ...
    function rankology_init_ca_metabox()
    {
        ...
        //commented bottom tabs display from this code

//        if (!empty($rankology_get_post_types) && !rankology_get_service('EnqueueModuleMetabox')->canEnqueue()) {
//            foreach ($rankology_get_post_types as $key => $value) {
//                add_meta_box('rankology_content_analysis', __('Content overview', 'wp-rankology'), 'rankology_content_analysis', $key, 'normal', $metaboxe_position);
//            }
//        }
    }
    ...
    function rankology_content_analysis($post)
    {
        ...
        $rankology_analysis_target_kw = get_post_meta($post->ID, '_rankology_analysis_target_kw', true);
        $rankology_google_suggest_kw = get_post_meta($post->ID, '_rankology_google_suggest_kw', true);
        $rankology_analysis_data = get_post_meta($post->ID, '_rankology_analysis_data', true);
        $rankology_titles_title = get_post_meta($post->ID, '_rankology_titles_title', true);
        $rankology_titles_desc = get_post_meta($post->ID, '_rankology_titles_desc', true);
    ...
    {
        //Nonce
        if (!isset($_POST['rankology_content_analysis_nonce']) || !wp_verify_nonce($_POST['rankology_content_analysis_nonce'], plugin_basename(__FILE__))) {
            return $post_id;
        }
