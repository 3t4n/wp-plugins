...
function rankology_ai_openai_api_key_callback() {
    ...
    $options = get_option('rankology_instant_indexing_option_nameai');
    $check = isset($options['rankology_ai_openai_api_key']) ? 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx' : null;

    printf('<input type="password" name="rankology_instant_indexing_option_nameai[rankology_ai_openai_api_key]" autocomplete="off" spellcheck="false" autocorrect="off" autocapitalize="off" value="%s" aria-label="' . __('OpenAI API key', 'wp-rankology') . '"/>', esc_html($check));
    ?>
        <p class="description">
...
<select id="rankology_ai_openai_model" name="rankology_instant_indexing_option_nameai[rankology_ai_openai_model]">
    <?php
        $models = [
