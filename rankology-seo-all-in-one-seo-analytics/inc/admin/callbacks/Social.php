...
function rankology_social_knowledge_name_callback()
{
    ...
    printf(
        '<input type="text" name="rankology_social_option_name[rankology_social_knowledge_name]" placeholder="' . esc_html__('e.g. My Local Business', 'wp-rankology') . '" aria-label="' . __('Person name/organization', 'wp-rankology') . '" value="%s"/>',
        esc_html($check)
    );
}
...
function rankology_social_knowledge_phone_callback()
{
    ...
    printf(
        '<input type="text" name="rankology_social_option_name[rankology_social_knowledge_phone]" placeholder="' . esc_html__('e.g. +44123456789 (internationalized version required)', 'wp-rankology') . '" aria-label="' . __('Organization\'s phone number (only for Organizations)', 'wp-rankology') . '" value="%s"/>',
        esc_html($check)
    );
}
...
function rankology_social_accounts_facebook_callback()
{
    ...
    printf(
        '<input type="text" name="rankology_social_option_name[rankology_social_accounts_facebook]" placeholder="' . esc_html__('e.g. https://facebook.com/my-page-url', 'wp-rankology') . '" aria-label="' . __('Facebook Page URL', 'wp-rankology') . '" value="%s"/>',
        esc_html($check)
    );
}
...
function rankology_social_accounts_twitter_callback()
{
    ...
    printf(
        '<input type="text" name="rankology_social_option_name[rankology_social_accounts_twitter]" placeholder="' . esc_html__('e.g. @my_twitter_account', 'wp-rankology') . '" aria-label="' . __('Twitter Page URL', 'wp-rankology') . '" value="%s"/>',
        esc_html($check)
    );
}
...
function rankology_social_accounts_pinterest_callback()
{
    ...
    printf(
        '<input type="text" name="rankology_social_option_name[rankology_social_accounts_pinterest]" placeholder="' . esc_html__('e.g. https://pinterest.com/my-page-url/', 'wp-rankology') . '" aria-label="' . __('Pinterest URL', 'wp-rankology') . '" value="%s"/>',
        esc_html($check)
    );
}
...
function rankology_social_accounts_instagram_callback()
{
    ...
    printf(
        '<input type="text" name="rankology_social_option_name[rankology_social_accounts_instagram]" placeholder="' . esc_html__('e.g. https://www.instagram.com/my-page-url/', 'wp-rankology') . '" aria-label="' . __('Instagram URL', 'wp-rankology') . '" value="%s"/>',
        esc_html($check)
    );
}
...
function rankology_social_accounts_youtube_callback()
{
    ...
    printf(
        '<input type="text" name="rankology_social_option_name[rankology_social_accounts_youtube]" placeholder="' . esc_html__('e.g. https://www.youtube.com/my-channel-url', 'wp-rankology') . '" aria-label="' . __('YouTube URL', 'wp-rankology') . '" value="%s"/>',
        esc_html($check)
    );
}
...
function rankology_social_accounts_linkedin_callback()
{
    ...
    printf(
        '<input type="text" name="rankology_social_option_name[rankology_social_accounts_linkedin]" placeholder="' . esc_html__('e.g. http://linkedin.com/company/my-company-url/', 'wp-rankology') . '" aria-label="' . __('LinkedIn URL', 'wp-rankology') . '" value="%s"/>',
        esc_html($check)
    );
}
...
function rankology_social_accounts_extra_callback() {
    ...
    printf(
'<textarea id="rankology_social_accounts_extra" name="rankology_social_option_name[rankology_social_accounts_extra]" rows="8" placeholder="' . esc_html__('Enter 1 URL per line (e.g. https://example.com/my-profile)', 'wp-rankology') . '" aria-label="' . __('Enter 1 URL per line (e.g. https://example.com/my-profile)', 'wp-rankology') . '">%s</textarea>',
esc_html($check)); ?>
...
<div id="rankology_social_fb_img_placeholder_upload" class="rankology-img-placeholder" data_caption="<?php esc_html_e('Click to select an image', 'wp-rankology'); ?>">
    <img id="rankology_social_fb_img_placeholder_src" style="width: 524px;height: 274px;" src="<?php echo $options_set; ?>" />
</div>
...
<?php }
    ...
    $options_set = isset($options['rankology_social_facebook_img_cpt'][$rankology_cpt_key]['url']) ? esc_attr($options['rankology_social_facebook_img_cpt'][$rankology_cpt_key]['url']) : null;
?>

<p>
    <input
        id="rankology_social_facebook_img_cpt_meta_<?php echo $rankology_cpt_key; ?>"
        class="rankology_social_facebook_img_cpt_meta" type="text"
        value="<?php echo $options_set; ?>"
        name="rankology_social_option_name[rankology_social_facebook_img_cpt][<?php echo $rankology_cpt_key; ?>][url]"
        aria-label="<?php esc_html_e('Upload default image', 'wp-rankology'); ?>"
        placeholder="<?php esc_html_e('Select your default thumbnail', 'wp-rankology'); ?>" />
    ...
    <input
        ...
        data-input-value="#rankology_social_facebook_img_cpt_meta_<?php echo $rankology_cpt_key; ?>"
        type="button"
...
<?php if (isset($options['rankology_social_facebook_img_cpt'][$rankology_cpt_key]['url'])) {
                    esc_attr($options['rankology_social_facebook_img_cpt'][$rankology_cpt_key]['url']);
                }
            }
        } else { ?>
<p>
...
function rankology_social_facebook_link_ownership_id_callback()
{
    ...
    printf(
        '<input type="text" placeholder="' . __('1234567890','wp-rankology') . '" name="rankology_social_option_name[rankology_social_facebook_link_ownership_id]" value="%s"/>',
        esc_html($check)
    ); ?>
...
function rankology_social_facebook_admin_id_callback()
{
    ...
    printf(
        '<input type="text" placeholder="' . __('1234567890','wp-rankology') . '" name="rankology_social_option_name[rankology_social_facebook_admin_id]" value="%s"/>',
        esc_html($check)
    ); ?>
...
function rankology_social_facebook_app_id_callback()
{
    ...
    printf(
        '<input type="text" placeholder="' . __('1234567890','wp-rankology') . '" name="rankology_social_option_name[rankology_social_facebook_app_id]" value="%s"/>',
        esc_html($check)
    ); ?>

<p class="description">
    <?php esc_html_e('The Facebook app ID of the site\'s app. In order to use Facebook Insights you must add the app ID to your page. Insights lets you view analytics for traffic to your site from Facebook. Find the app ID in your App Dashboard. <a class="rankology-help" href="https://developers.facebook.com/apps/redirect/dashboard" target="_blank">More info here</a> <span class="rankology-help dashicons dashicons-redo"></span>', 'wp-rankology'); ?>
</p>
...
<div id="rankology_social_twitter_img_placeholder_upload" class="rankology-img-placeholder" data_caption="<?php esc_html_e('Click to select an image', 'wp-rankology'); ?>">
    <img id="rankology_social_twitter_img_placeholder_src" style="width: 600px;height: 314px;" src="<?php echo $options_set; ?>" />
</div>
