...
function rankology_news_enable_callback() {
    $options = get_option('rankology_instant_indexing_option_namegn');

    $check = isset($options['rankology_news_enable']); ?>

<label for="rankology_news_enable">
    <input id="rankology_news_enable" name="rankology_instant_indexing_option_namegn[rankology_news_enable]" type="checkbox" <?php if ('1' == $check) { ?>
    checked="yes"
    <?php } ?>
    value="1"/>
...
function rankology_news_name_callback() {
    ...
    printf(
    '<input type="text" name="rankology_instant_indexing_option_namegn[rankology_news_name]" aria-label="' . __('Publication Name', 'wp-rankology') . '" placeholder="' . esc_html__('Enter your Google News Publication Name', 'wp-rankology') . '" value="%s"></textarea>',
    esc_html($check)
    );
}
...
<div class="rankology_wrap_single_cpt">
    ...
    <label
        ...
        <input
            ...
            name="rankology_instant_indexing_option_namegn[rankology_news_name_post_types_list][<?php echo $rankology_cpt_key; ?>][include]"
