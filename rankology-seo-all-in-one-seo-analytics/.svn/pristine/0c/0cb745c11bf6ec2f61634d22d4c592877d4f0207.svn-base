...
class models extends MetaBoxAbstract
{
    ...
    public static function get($arg = array())
    {
        ...
        $sql = $wpdb->prepare("SELECT model, COUNT(*) as count FROM " . DB::table('visitor') . " WHERE model != '" . _x('Unknown', 'Model', 'rankology-stats') . "' AND `last_counter` BETWEEN %s AND %s GROUP BY model {$order_by}", reset($days_time_list), end($days_time_list));
        ...
        if (end($days_time_list) == TimeZone::getCurrentDate("Y-m-d")) {
            $title = sprintf(__('%s Statistics in the last %s days', 'rankology-stats'), __('Manufacturers', 'rankology-stats'), self::$countDays);
        } else {
            $title = sprintf(__('%s Statistics from %s to %s', 'rankology-stats'), __('Manufacturers', 'rankology-stats'), $args['from'], $args['to']);
        }
