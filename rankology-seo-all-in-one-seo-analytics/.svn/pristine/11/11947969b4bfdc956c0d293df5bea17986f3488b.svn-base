...
namespace RankologyElementorFAQScheme;
...
class FAQ_Schema {
	...
	private function __construct() {
		$rankology_get_toggle_rich_snippets_option = get_option( 'rankology_toggle' );
		$this->structured_data_enabled            = isset( $rankology_get_toggle_rich_snippets_option['toggle-rich-snippets'] ) ? $rankology_get_toggle_rich_snippets_option['toggle-rich-snippets'] : '0';
	...
	public function add_faq_json_ld_schema( $content, $widget ) {
		...
		if ( ! empty( $tabs ) ) {
			...
			return $content . apply_filters( 'rankology_schemas_faq_html', $schema );
		}
	...
	public function add_faq_enable_option( $element, $section_id, $args ) {
		...
		if (
			...
			$element->add_control(
				'show_faq_schema',
				array(
					'label' => __( 'Enable FAQ Schema', 'wp-rankology' ),
