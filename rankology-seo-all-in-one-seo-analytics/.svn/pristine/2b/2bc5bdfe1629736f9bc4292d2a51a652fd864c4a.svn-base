...
function rankology_slim_seo_migration() {
    check_ajax_referer('rankology_slim_seo_migrate_nonce', $_POST['_ajax_nonce'], true);
                ...
                foreach ($slim_seo_query_terms as $term_id) {
                    if ('' != get_term_meta($term_id, 'slim_seo', true)) {
                        ...
                        if ( ! empty($term_settings['title'])) { //Import title tag
                            update_term_meta($term_id, '_rankology_titles_title', $term_settings['title']);
                        }
                        ...
                        if ( ! empty($term_settings['facebook_image'])) { //Import FB image
                            update_term_meta($term_id, '_rankology_social_fb_img', $term_settings['facebook_image']);
                        }
                        if ( ! empty($term_settings['twitter_image'])) { //Import Tw image
                            update_term_meta($term_id, '_rankology_social_twitter_img', $term_settings['twitter_image']);
                        }
                    }
                }
            }
                ...
                foreach ($slim_seo_query as $post) {
                    if ('' != get_post_meta($post->ID, 'slim_seo', true)) {
                        ...
                        if ( ! empty($post_settings['title'])) { //Import title tag
                            update_post_meta($post->ID, '_rankology_titles_title', $post_settings['title']);
                        }
                        if ( ! empty($post_settings['description'])) { //Import meta desc
                            update_post_meta($post->ID, '_rankology_titles_desc', $post_settings['description']);
                        }
                        ...
                        if ( ! empty($post_settings['facebook_image'])) { //Import FB image
                            update_post_meta($post->ID, '_rankology_social_fb_img', $post_settings['facebook_image']);
                        }
                        if ( ! empty($post_settings['twitter_image'])) { //Import Tw image
                            update_post_meta($post->ID, '_rankology_social_twitter_img', $post_settings['twitter_image']);
                        }
                    }
                }
            }
