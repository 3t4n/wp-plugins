...
function rankology_automatic_rich_snippets_lb_option($schema_datas) {
    ...
    if (get_post_meta(get_the_ID(), '_rankology_robots_canonical', true)) {
        $lb_canonical = get_post_meta(get_the_ID(), '_rankology_robots_canonical', true);
    } else {
        global $wp;
        if (isset($wp->request)) {
    ...
    $json = apply_filters('rankology_schemas_auto_lb_json', $json);

    $json = '<script type="application/ld+json">' . json_encode($json) . '</script>' . "\n";

    $json = apply_filters('rankology_schemas_auto_lb_html', $json);
