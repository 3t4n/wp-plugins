...
class InspectUrl implements ExecuteHooks {
    ...
    public function register() {
        register_rest_route('rankology/v1', '/posts/(?P<id>\d+)/inspect', [
            'methods' => 'GET',
    ...
    public function processGet(\WP_REST_Request $request) {
        ...
        $data = get_post_meta($postId, '_rankology_gsc_inspect_url_data', true);
    ...
    public function processPost(\WP_REST_Request $request) {
        ...
        $data = rankology_fno_get_service('InspectUrlGoogle')->handle($postId);
