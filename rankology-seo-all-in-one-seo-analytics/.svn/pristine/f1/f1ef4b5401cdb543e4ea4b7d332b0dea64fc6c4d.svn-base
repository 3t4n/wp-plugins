...
function rankology_export_settings() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_export_nonce'], 'rankology_export_nonce')) {
        return;
    }
...
function rankology_import_settings() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_import_nonce'], 'rankology_import_nonce')) {
        return;
    }
    ...
    $settings = (array) json_decode(rankology_remove_utf8_bom(file_get_contents($import_file)), true);
...
function rankology_import_redirections_settings() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_import_redirections_nonce'], 'rankology_import_redirections_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', 'import_settings'))) {
        return;
    ...
    if ( ! $_POST['import_sep']) {
        wp_die(__('Please choose a separator', 'wp-rankology'));
    }
...
function rankology_import_yoast_redirections() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_import_yoast_redirections_nonce'], 'rankology_import_yoast_redirections_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', 'import_settings'))) {
        return;
    ...
    foreach (array_slice($csv, 1) as $_key => $_value) {
        ...
        if ( ! empty($csv_line[0])) {
            ...
            $id = wp_insert_post([
                ...
                'post_status'       => 'publish',
                'meta_input'        => [
                    '_rankology_redirections_value'          => urldecode($csv_line[1]),
                    '_rankology_redirections_type'           => $csv_type_redirects[2],
                    '_rankology_redirections_enabled'        => $csv_type_redirects[3],
...
function rankology_export_redirections_settings() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_export_redirections_nonce'], 'rankology_export_redirections_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', 'export_settings'))) {
        return;
...
function rankology_export_redirections_htaccess_settings() {
    if (empty($_POST['rankology_action']) || 'export_redirections_htaccess' != $_POST['rankology_action']) {
        return;
    }
    if ( ! wp_verify_nonce($_POST['rankology_export_redirections_htaccess_nonce'], 'rankology_export_redirections_htaccess_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', 'export_settings'))) {
        return;
...
function rankology_import_redirections_plugin_settings() {
    if (empty($_POST['rankology_action']) || 'import_redirections_plugin_settings' != $_POST['rankology_action']) {
        return;
    }
    if ( ! wp_verify_nonce($_POST['rankology_import_redirections_plugin_nonce'], 'rankology_import_redirections_plugin_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', 'import_settings'))) {
        return;
    ...
    foreach ($settings['redirects'] as $redirect_key => $redirect_value) {
        ...
        wp_insert_post([
            ...
            'post_status' => 'publish',
            'meta_input'  => [
                '_rankology_redirections_value'   => urldecode($redirect_value['action_data']['url']),
...
function rankology_import_rk_redirections() {
    if (empty($_POST['rankology_action']) || 'import_rk_redirections' != $_POST['rankology_action']) {
        return;
    }
    if ( ! wp_verify_nonce($_POST['rankology_import_rk_redirections_nonce'], 'rankology_import_rk_redirections_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', 'import_settings'))) {
        return;
        ...
        wp_insert_post(
            [
                ...
                'meta_input'  => [
                    ...
                    '_rankology_redirections_type'    => $type,
                    '_rankology_redirections_enabled' => $enabled,
                    '_rankology_redirections_enabled_regex' => $regex,
                    '_rankology_redirections_logged_status'  => 'both',
                    'rankology_404_count'             => $count,
                    '_rankology_redirections_param'   => $param,
                ],
...
function rankology_clean_404() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_clean_404_nonce'], 'rankology_clean_404_nonce')) {
        return;
    }
...
function rankology_clean_counters() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_clean_counters_nonce'], 'rankology_clean_counters_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', '404'))) {
        return;
...
function rankology_clean_all() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_clean_all_nonce'], 'rankology_clean_all_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', '404'))) {
        return;
...
function rankology_clean_content_scans() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_clean_content_scans_nonce'], 'rankology_clean_content_scans_nonce')) {
        return;
    }
...
function rankology_reset_notices_settings() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_reset_notices_nonce'], 'rankology_reset_notices_nonce')) {
        return;
    }
...
function rankology_reset_settings() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_reset_nonce'], 'rankology_reset_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', 'reset_settings'))) {
        return;
...
function rankology_bot_links_export_settings() {
    ...
    if ( ! wp_verify_nonce($_POST['rankology_export_csv_links_nonce'], 'rankology_export_csv_links_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', 'export_settings'))) {
        return;
...
function rankology_download_batch_export() {
    ...
    if ( ! wp_verify_nonce($_GET['nonce'], 'rankology_csv_batch_export_nonce')) {
        return;
    }
    if ( ! current_user_can(rankology_capability('manage_options', 'export_settings'))) {
        return;
