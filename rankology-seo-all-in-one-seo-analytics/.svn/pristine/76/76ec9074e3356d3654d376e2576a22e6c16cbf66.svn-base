...
class authors_page
{
    ...
    public static function view()
    {
        ...
        // Page title
        $args['title'] = __('Author Statistics', 'rankology-stats');
        ...
        if (!isset($_GET['ID']) || (isset($_GET['ID']) and $_GET['ID'] == 0)) {
            ...
            $args['top_title']     = __('Top Author Sorted by Hits', 'rankology-stats');

            // Push List Category
            foreach ($users as $user) {
                $args['top_list'][$user->ID] = array('ID' => $user->ID, 'name' => User::get_name($user->ID), 'link' => add_query_arg('ID', $user->ID), 'count_visit' => (int)rankology_stats_pages('total', null, $user->ID, null, null, 'author'));
            }
            ...
            $args['top_title']     = __('Top posts Sorted by Hits from the author', 'rankology-stats');
