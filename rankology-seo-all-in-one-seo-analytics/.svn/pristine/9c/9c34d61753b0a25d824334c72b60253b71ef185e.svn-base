jQuery(document).ready(function () {
    jQuery(document).on("click", "#rankology-seo-tab", function (e) {
        jQuery("#elementor-panel-footer-settings").trigger("click");
        jQuery(".elementor-control-rankology_title_settings").trigger("click");
    });
});
...
var textLetterCounterView = elementor.modules.controls.BaseData.extend({
    ...
    countLength: function (initial = false, currentEl = false) {
        ...
        if (initial) {
            ...
            $elementParent
                .find(".rankology_counters")
                .after(
                    `<div class="rankology_counters_val">/ ${maxLength}</div>`
                );
        }

        if (currentValue.length > 0) {
            $elementParent.find(".rankology_counters").text(currentValue.length);
            const pixels =
                this.fieldType === "text"
                    ? this.pixelTitle(currentValue)
                    : this.pixelDesc(currentValue);
            $elementParent.find(".rankology_pixel").text(pixels);
        }
        ...
        if (
            ...
            $elementParent.find(".rankology_counters").css("color", "red");
        } else {
            $elementParent.find(".rankology_counters").css("color", "#6d7882");
        }
        ...
        if (
            ...
            $elementParent.find(".rankology_pixel").css("color", "red");
        } else {
            $elementParent.find(".rankology_pixel").css("color", "#6d7882");
        }
        ...
        $elementParent
            .find(".rankology_counters_progress")
            .attr("aria-valuenow", progress);
        $elementParent.find(".rankology_counters_progress").text(progress + "%");
        $elementParent
            .find(".rankology_counters_progress")
            .css("width", progress + "%");
    },
    ...
    addTag: function (e) {
        ...
        const $mainParent = $currentBtn
            .parents(".rankology-text-letter-counter")
            .first();
...
elementor.addControlView("rankologytextlettercounter", textLetterCounterView);
