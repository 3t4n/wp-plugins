...
class SearchConsole {
    ...
    protected function getDateRange($options = []){
        ...
        $date_range = apply_filters('rankology_search_console_date_range', $date_range);

        $startDate = isset($options['startDate']) ? $options['startDate'] : (new \DateTime('today'))->modify($date_range)->format('Y-m-d');

        $startDate = apply_filters('rankology_search_console_start_date', $startDate);

        $endDate = isset($options['endDate']) ? $options['endDate'] : (new \DateTime('today'))->format('Y-m-d');

        $endDate = apply_filters('rankology_search_console_end_date', $endDate);
    ...
    protected function getUrl(){
        $url = rankology_fno_get_service('OptionPro')->getGSCDomainProperty() ? true : site_url();

        if ($url === true) {
            $url = wp_parse_url(site_url($post));
            if (is_array($url) && isset($url['host'])) {
                $url = 'sc-domain:' . $url['host'];
            }
        }

        $url = apply_filters('rankology_search_console_base_url', $url);

        if(RANKOLOGY_VERSION === "{VERSION}"){
    ...
    public function handle($options = []) {
        //Cleaning old data
        delete_post_meta_by_key('_rankology_search_console_analysis_clicks');
        delete_post_meta_by_key('_rankology_search_console_analysis_ctr');
        delete_post_meta_by_key('_rankology_search_console_analysis_impressions');
        delete_post_meta_by_key('_rankology_search_console_analysis_position');

        $client = rankology_fno_get_service('GoogleClient')->getClient();
    ...
    public function getKeywords($options = []){
        ...
        $rows = get_transient('rankology_search_console_keywords');
        if($rows){
            return [
                'data' => $rows,
                'status' => 'success'
        ...
        try {
            ...
            set_transient('rankology_search_console_keywords', $data, 60 * 60 * 24); // 1 day
    ...
    public function saveDataFromRowResult($row){
        ...
        update_post_meta($postId, '_rankology_search_console_analysis_clicks', $clicks);
        update_post_meta($postId, '_rankology_search_console_analysis_ctr', $ctr);
        update_post_meta($postId, '_rankology_search_console_analysis_impressions', $impressions);
        update_post_meta($postId, '_rankology_search_console_analysis_position', $position);
