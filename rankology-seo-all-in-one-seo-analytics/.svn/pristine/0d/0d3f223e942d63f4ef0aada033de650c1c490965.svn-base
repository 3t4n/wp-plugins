...
class Helper
{
    ...
    {
        if (empty($version)) {
            $version = RANKOLOGY_STATS_VERSION;
        }
    ...
    {
        return wp_normalize_path(path_join(RANKOLOGY_STATS_DIR, $path));
    }
    ...
    public static function is_active_cache_plugin()
    {
        ...
        return apply_filters('rankology_stats_cache_status', $use);
    }
     ...
     * @param string $path
     * @return mixed
     * @default For Rankology Stats Plugin is 'rankology-stats' dir
     */
    ...
    {
        global $RANKOLOGY_Stats;

        # Set Default
        $list = array();

        # Load From global
        if (isset($RANKOLOGY_Stats->robots_list)) {
            $list = $RANKOLOGY_Stats->robots_list;
        }

        # Load From file
        include RANKOLOGY_STATS_DIR . "includes/defines/robots-list.php";
        if (isset($rkns_robots_list_array)) {
            $list = $rkns_robots_list_array;
        }
    ...
    public static function get_date_install_plugin()
    {
        ...
        foreach ($list_tbl as $tbl => $val) {
            $first_day = $wpdb->get_var("SELECT `" . $val['column'] . "` FROM `" . RANKOLOGY_STATS\DB::table($tbl) . "` ORDER BY `" . $val['order_by'] . "` ASC LIMIT 1");
            if (!empty($first_day)) {
                break;
            }
        }
     ...
     *
     * @example add_filter( 'rankology_stats_days_ago_request', array( '', 'set_all_option_datepicker' ) );
     */
    ...
    public static function send_mail($to, $subject, $content, $email_template = true, $args = array())
    {
        // Email Template
        if ($email_template) {
            $email_template = wp_normalize_path(RANKOLOGY_STATS_DIR . 'includes/admin/templates/emails/layout.php');
        }
        ...
        $template_arg = array(
            ...
            'footer_text'  => '',
            'email_title'  => apply_filters('rankology_stats_email_title', __('Email from', 'rankology-stats') . ' ' . parse_url(get_site_url())['host']),
            'logo_image'   => apply_filters('rankology_stats_email_logo', RANKOLOGY_STATS_URL . 'assets/images/logo-statistics-header-blue.png'),
            'logo_url'     => apply_filters('rankology_stats_email_logo_url', get_bloginfo('url')),
            'copyright'    => apply_filters('rankology_stats_email_footer_copyright', Admin_Template::get_template('emails/copyright', array(), true)),
            'email_header' => apply_filters('rankology_stats_email_header', ""),
            'email_footer' => apply_filters('rankology_stats_email_footer', ""),
    ...
    {
        $taxonomies = array('category' => __("Category", "rankology-stats"), "post_tag" => __("Tags", "rankology-stats"));
    ...
    public static function mysql_time_conditions($field = 'date', $time = 'total', $range = array())
    {
        ...
        //Create Field Sql
        $field_sql = function ($time) use ($current_date, $field, $RANKOLOGY_Stats, $range) {
