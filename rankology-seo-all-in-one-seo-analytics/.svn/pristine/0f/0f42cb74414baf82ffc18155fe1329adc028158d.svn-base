rkns_js.referring_meta_box = {

    view: function (args = []) {
        let t = '<div class="o-table-wrapper">';
        t += `<table width="100%" class="o-table o-table--link o-table--space-between"><thead>
        <tr>
            <th>${rkns_js._('address')}</th>
            <th>${rkns_js.meta_box_lang('referring', 'references')}</th>
        </tr></thead><tbody>`;

        args.referring.forEach(function (value) {
            t += `<tr>
			<td>` + rkns_js.site_icon(value['domain']) + ` <a href='//${value['domain']}' title='${value['title']}' target="_blank">${value['domain']} <span class="dashicons dashicons-geoipset" style="font-size: 15px; vertical-align: middle"></span></a></td>
			<td class="rkns-middle-vertical"><a style="justify-content: flex-end;" href="${value['page_link']}">${value['number']} 
