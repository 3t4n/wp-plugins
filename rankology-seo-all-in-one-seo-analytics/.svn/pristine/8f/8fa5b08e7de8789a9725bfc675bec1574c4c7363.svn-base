...
?>
<div id="rankology-page-speed-results" class="metabox-holder">
    <?php
        if (get_transient('rankology_results_page_speed') == true && get_transient('rankology_results_page_speed_desktop') == true) {
            //Init
            $json = [];

            $json = json_decode(get_transient('rankology_results_page_speed'), true);
            $json_desktop = json_decode(get_transient('rankology_results_page_speed_desktop'), true);

            if ( ! empty($json) && array_key_first($json) !== 'error' && ! empty($json_desktop) && array_key_first($json_desktop) !== 'error') {
                $perf_score = rankology_fno_get_ps_score($json, true);
                $perf_score_desktop = rankology_fno_get_ps_score($json_desktop);
                $core_web_vitals_score = rankology_fno_get_cwv_score($json);
                ...
                $loading_experience_scores = [
                    __('Performance', 'wp-rankology') => [
                        ...
                        'unit' => '<p class="wrap-scale">' . __('<span><span class="score red"></span>0-49</span><span><span class="score yellow"></span>50-89</span><span><span class="score green"></span>90-100</span>', 'wp-rankology') . '</p>'
                    ],
                    __('Screenshot', 'wp-rankology') => [
                ...
                if (isset($json['loadingExperience'])) {
                    if (isset($json['loadingExperience']['metrics']['FIRST_CONTENTFUL_PAINT_MS'])) {
                        $loading_experience_scores['First Contentful Paint (FCP)'] = [
                            'score' => $json['loadingExperience']['metrics']['FIRST_CONTENTFUL_PAINT_MS']['percentile'] ? round((($json['loadingExperience']['metrics']['FIRST_CONTENTFUL_PAINT_MS']['percentile']) / 1000), 2) : __('N/A', 'wp-rankology'),
                            'unit' => 's',
                    ...
                    if (isset($json['loadingExperience']['metrics']['FIRST_INPUT_DELAY_MS'])) {
                        $loading_experience_scores['First Input Delay (FID)'] = [
                            'score' => $json['loadingExperience']['metrics']['FIRST_INPUT_DELAY_MS']['percentile'] ? $json['loadingExperience']['metrics']['FIRST_INPUT_DELAY_MS']['percentile'] : __('N/A', 'wp-rankology'),
                            'unit' => 'ms',
                    ...
                    if (isset($json['loadingExperience']['metrics']['LARGEST_CONTENTFUL_PAINT_MS'])) {
                        $loading_experience_scores['Largest Contentful Paint (LCP)'] = [
                            'score' => $json['loadingExperience']['metrics']['LARGEST_CONTENTFUL_PAINT_MS']['percentile'] ? round((($json['loadingExperience']['metrics']['LARGEST_CONTENTFUL_PAINT_MS']['percentile']) / 1000), 2) : __('N/A', 'wp-rankology'),
                            'unit' => 's',
                    ...
                    if (isset($json['loadingExperience']['metrics']['CUMULATIVE_LAYOUT_SHIFT_SCORE'])) {
                        $loading_experience_scores['Cumulative Layout Shift (CLS)'] = [
                            'score' => $json['loadingExperience']['metrics']['CUMULATIVE_LAYOUT_SHIFT_SCORE']['percentile'] ? $json['loadingExperience']['metrics']['CUMULATIVE_LAYOUT_SHIFT_SCORE']['percentile'] : __('N/A', 'wp-rankology'),
    ...
    <div class="wrap-rankology-score">
        <div class="rankology-cwv">
            <div>
                        ...
                        if ( ! is_plugin_active('wp-rocket/wp-rocket.php')) {
                            ...
                            <p>
                                <a class="rankology-help">
                                    <?php esc_html_e('We recommend WP Rocket caching plugin to quickly and easily optimize your Core Web Vitals.', 'wp-rankology'); ?>
                ...
                <p><?php printf(__('Computed from the %s Core Web Vitals metrics over the latest 28-day collection period.', 'wp-rankology'), $cwv_svg); ?>
                </p>
                <p><?php esc_html_e('The Core Web Vitals metrics are FID, LCP, and CLS. For aggregations with sufficient data in all three metrics, the aggregation passes the Core Web Vitals assessment if the 75th percentiles of all three metrics are Good. Otherwise, the aggregation does not pass the assessment. If the aggregation has insufficient data for FID, then it will pass the assessment if both the 75th percentiles of LCP and CLS are Good.', 'wp-rankology'); ?>
                </p>
                <p>
                    <a href="https://pagespeed.web.dev/report?url=<?php echo get_home_url(); ?>"
                        target="_blank">
                        <?php esc_html_e('Full report on Google Page Speed website', 'wp-rankology'); ?>
                    </a>
                    <span class="dashicons dashicons-redo"></span>
                </p>
            </div>
        ...
        <div class="rankology-summary-items">
            ...
            <div class="rankology-summary-item">
                ...
                <div class="rankology-summary-item-data">
            ...
            <?php
                ...
                if ( ! empty($screenshot_thumbnails)) {
                    ...
                    foreach ($screenshot_thumbnails as $value) {
                        ...
                        echo '<span>' . round($value['timing'] / 1000, 2) . __(' s', 'wp-rankology') . '</span>';
                        echo '</li>';
                    }
                    echo '</ul>';
                } ?>
        ...
        <div class="ps-audits">
            <?php foreach ($audits as $key => $cats) {
                    ...
                    switch ($key) {
                        case 'opportunities':
                            $title = __('Opportunities', 'wp-rankology');
                            break;
                        case 'diagnostics':
                            $title = __('Diagnostics', 'wp-rankology');
                            break;
                        case 'passed':
                            $title = __('Passed audits', 'wp-rankology');
                            break;
                    }
            ...
            if ( ! empty($matches[0])) {
                echo '<p class="learn-more"><a class="rankology-help" target="_blank" rel="noopener noreferrer nofollow" href="' . $matches[0] . '">' . __('Learn more', 'wp-rankology') . '</a><span class="rankology-help dashicons dashicons-redo"></span></p>';
            } ?>
                </div>
                <?php }
                } ?>
        ...
        <div class="rankology-notice">
            ...
            <p>
                <strong><?php esc_html_e('Captured at ', 'wp-rankology'); ?></strong>
