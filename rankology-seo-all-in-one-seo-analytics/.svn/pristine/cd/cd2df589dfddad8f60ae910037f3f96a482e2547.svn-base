...
//Jobs JSON-LD
function rankology_automatic_rich_snippets_jobs_option($schema_datas) {
    //if no data
    if (0 != count(array_filter($schema_datas))) {
        ...
        $json = apply_filters('rankology_schemas_auto_job_json', $json);

        $json = '<script type="application/ld+json">' . json_encode($json) . '</script>' . "\n";

        $json = apply_filters('rankology_schemas_auto_job_html', $json);
