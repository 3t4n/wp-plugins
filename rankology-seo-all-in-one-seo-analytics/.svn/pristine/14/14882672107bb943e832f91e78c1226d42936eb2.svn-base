...
class Product extends JsonSchemaValue implements GetJsonData {
    ...
    protected function getKeysForSchemaManual() {
        return [
            ...
            'name' => [
                'value' => '_rankology_fno_rich_snippets_product_name',
                'default' => '%%post_title%%',
            ],
            'description' => [
                'value' => '_rankology_fno_rich_snippets_product_description',
                'default' => '%%post_excerpt%%',
            ],
            'image' => [
                'value' => '_rankology_fno_rich_snippets_product_img',
                'default' => '%%post_thumbnail_url%%',
            ],
            'price' => [
                'value' => '_rankology_fno_rich_snippets_product_price',
                'default' => '%%wc_get_price%%',
            ],
            'priceValidDate' => [
                'value' => '_rankology_fno_rich_snippets_product_price_valid_date',
                'default' => '%%wc_price_valid_date%%',
            ],
            'sku' => [
                'value' => '_rankology_fno_rich_snippets_product_sku',
                'default' => '%%wc_sku%%',
            ],
            'brand' => '_rankology_fno_rich_snippets_product_brand',
            'globalIds' => '_rankology_fno_rich_snippets_product_global_ids',
            'globalIdsValue' => '_rankology_fno_rich_snippets_product_global_ids_value',
            'priceCurrency' => '_rankology_fno_rich_snippets_product_price_currency',
            'condition' => [
                'value' => '_rankology_fno_rich_snippets_product_condition',
                'default' => 'NewCondition',
            ],
            'availability' => '_rankology_fno_rich_snippets_product_availability',
            'positiveNotes' => '_rankology_fno_rich_snippets_product_positive_notes',
            'negativeNotes' => '_rankology_fno_rich_snippets_product_negative_notes',
            'energy_consumption' => '_rankology_fno_rich_snippets_product_energy_consumption',
        ];
    }
    ...
    public function getJsonData($context = null) {
        ...
        if (isset($variables['brand'])) {
            ...
            $schema = rankology_get_service('JsonSchemaGenerator')->getJsonFromSchema(Brand::NAME, $contextWithVariables, ['remove_empty' => true]);
            if (count($schema) > 1) {
                $data['brand'] = $schema;
            }
        }
        ...
        if (isset($context['product']) && null !== $context['product'] && isset($context['post']->ID) && comments_open($context['post']->ID)) {
            ...
            if (function_exists('wc_get_product')) {
                ...
                if (method_exists($product, 'get_review_count') && $product->get_review_count() >= 1) {
                    ...
                    $contextWithVariables['type'] = RichSnippetType::SUB_TYPE;
                    $schema = rankology_get_service('JsonSchemaGenerator')->getJsonFromSchema(AggregateRating::NAME, $contextWithVariables, ['remove_empty' => true]);
                    if (count($schema) > 1) {
        ...
        if (isset($variables['price'])) {
            ...
            $contextWithVariables['variables'] = [
                ...
                'itemCondition' => isset($variables['condition']) ? sprintf('%sschema.org/%s', rankology_check_ssl(), $variables['condition']) : sprintf('%sschema.org/%s', rankology_check_ssl(), $variables['NewCondition']),
                'availability' => isset($variables['availability']) ? sprintf('%sschema.org/%s', rankology_check_ssl(), $variables['availability']) : '',
            ];
        ...
        return apply_filters('rankology_fno_get_json_data_product', $data, $context);
    }
