...
class DomAnalysis
{
    ...
    public function getPostContentAnalyze($id)
    {
        ...
        $content = apply_filters('rankology_content_analysis_content', $content, $id);
    ...
    public function getDataAnalyze($data, $options)
    {
        ...
        $targetKeywords = isset($options['target_keywords']) && !empty($options['target_keywords']) ? $options['target_keywords'] : get_post_meta($options['id'], '_rankology_analysis_target_kw', true);

        $targetKeywords = array_filter(explode(',', strtolower($targetKeywords)));

        $targetKeywords = apply_filters( 'rankology_content_analysis_target_keywords', $targetKeywords, $options['id'] );
        ...
        $postContent = apply_filters('rankology_dom_analysis_get_post_content', $this->getPostContentAnalyze($options['id']));
        ...
        if (is_plugin_active('oxygen/functions.php') && function_exists('ct_template_output') && $oxygen_metabox_enabled === true) {
            $dataOxygen = get_post_meta($post->ID, '_rankology_analysis_data_oxygen', true);
