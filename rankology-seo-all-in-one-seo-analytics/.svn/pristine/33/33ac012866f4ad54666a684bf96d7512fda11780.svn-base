...
class Admin_Post
{
    ...
    {
        $columns['rankology-stats-post-hits'] = __('Traffic', 'rankology-stats');
        return $columns;
    }
    ...
    {
        if ($column_name == 'rankology-stats-post-hits') {
            ...
            $hit_number = rankology_stats_pages('total', "", $post_id, null, null, $hitPostType);

            if ($hit_number) {
                
                // Remove post_type_ from prefix of custom post type because of incompatibility with Rankology Stats MiniChart
                ...
                echo sprintf('<a href="%s" class="rkns-admin-column__link">%s</a>',
    ...
    {
        $columns['rankology-stats-post-hits'] = 'hits';
        return $columns;
    }

    /**
     * Sort Post By Traffic
     *
     * @param $clauses
     * @param $query
     */
    ...
    /**
     * Delete All Post Traffic When Post is Deleted
     *
     * @param $post_id
     */
    ...
    public function post_hit_misc()
    {
        ...
        if ($post->post_status == 'publish') {
            echo "<div class='misc-pub-section misc-pub-hits'>" . __('Traffic', 'rankology-stats') . ": <a href='" . Menus::admin_url('pages', array('ID' => $post->ID, 'type' => Pages::get_post_type($post->ID))) . "'>" . number_format(rankology_stats_pages('total', "", $post->ID)) . "</a></div>";
        }
    }
