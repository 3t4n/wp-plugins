...
class IP
{
    ...
    public static function getIP()
    {
         ...
         */
        $ip = apply_filters('rankology_stats_sanitize_user_ip', $ip);
        ...
        return apply_filters('rankology_stats_user_ip', sanitize_text_field($ip));
    }
    ...
    public static function getHashIP($ip = false)
    {
        ...
        if (Option::get('hash_ips') == true) {
            return apply_filters('rankology_stats_hash_ip', self::$hash_ip_prefix . sha1(($ip === false ? self::getIP() : $ip) . (UserAgent::getHttpUserAgent() == '' ? 'Unknown' : UserAgent::getHttpUserAgent())));
        }
    ...
    public static function getStoreIP()
    {
         ...
         * @example 888.888.888.888 -> e7b398f96b14993b571215e36b41850c65f39b1a
         */
        if (self::getHashIP()) {
