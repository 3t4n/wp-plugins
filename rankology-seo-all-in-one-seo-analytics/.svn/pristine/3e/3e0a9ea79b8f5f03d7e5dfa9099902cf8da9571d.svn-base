...
class Admin_Taxonomy
{
    ...
    public function init()
    {
        ...
        // Check Active
        if (!apply_filters('rankology_stats_show_taxonomy_hits', true)) {
            return;
        }
    ...
    public function add_column($columns)
    {
        ...
        if (isset($columns['handle'])) {
            ...
            foreach ($columns as $k => $v) {
                if ($k == "handle") {
                    $col['rankology-stats-tax-hits'] = __('Traffic', 'rankology-stats');
                }
                $col[$k] = $v;
            }
            return $col;
        ...
        $columns['rankology-stats-tax-hits'] = __('Traffic', 'rankology-stats');
        return $columns;
    }
    ...
    {
        if ($column_name == 'rankology-stats-tax-hits') {
            $term       = get_term($term_id);
            $hit_number = rankology_stats_pages('total', "", $term_id, null, null, $term->taxonomy);
    ...
    {
        $columns['rankology-stats-tax-hits'] = 'hits';
        return $columns;
    }
