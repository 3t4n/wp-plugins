...
function rankology_redirections_value() {
	...
	if (is_singular() && get_post_meta($post->ID,'_rankology_redirections_value',true)) {
		$rankology_redirections_value = html_entity_decode(esc_url(get_post_meta($post->ID,'_rankology_redirections_value',true)));
		...
		$rankology_redirections_value = html_entity_decode(esc_url(get_post_meta(get_option( 'page_for_posts' ),'_rankology_redirections_value',true)));
		return $rankology_redirections_value;
 	} elseif ((is_tax() || is_category() || is_tag()) && get_term_meta(get_queried_object_id(),'_rankology_redirections_value',true) !='') {
		$rankology_redirections_value = html_entity_decode(esc_url(get_term_meta(get_queried_object_id(),'_rankology_redirections_value',true)));
		return $rankology_redirections_value;
	} else {
		$rankology_redirections_value = basename(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH));
		...
		if ( $rankology_redirections_query->have_posts() ) {
			...
			if (in_array($rankology_redirections_value, $all_titles)) {
				//do_redirect
				return $rankology_redirections_value;
			}
			wp_reset_postdata();
