...
class TimeZone
{
    ...
    {
        return apply_filters('rankology_stats_current_timestamp', self::getCurrentDate('U'));
    }
    ...
    public static function getListDays($args = array())
    {
        ...
        foreach ($period as $key => $value) {
            $list[$value->format('Y-m-d')] = array(
                ...
                'format'    => $value->format(apply_filters('rankology_stats_request_days_format', $args['format']))
            );
        }
