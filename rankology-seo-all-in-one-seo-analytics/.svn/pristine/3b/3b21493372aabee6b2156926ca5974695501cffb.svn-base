    ...
    public static function purge_data($purge_days)
    {
        ...
        if ($purge_days > 30) {
            ...
            if ($result) {
                ...
                $result_string = sprintf(__('%s data older than %s days purged successfully.', 'rankology-stats'), '<code>' . $table_name . '</code>', '<code>' . $purge_days . '</code>');
            } else {
                $result_string = sprintf(__('No records found to purge from %s!', 'rankology-stats'), '<code>' . $table_name . '</code>');
            }
            ...
            if ($result) {
                ...
                $result_string .= '<br>' . sprintf(__('%s data older than %s days purged successfully.', 'rankology-stats'), '<code>' . $table_name . '</code>', '<code>' . $purge_days . '</code>');
            } else {
                $result_string .= '<br>' . sprintf(__('No records found to purge from %s!', 'rankology-stats'), '<code>' . $table_name . '</code>');
            }
            ...
            if (Option::get('prune_report') == true) {
                ...
                Helper::send_mail(Option::getEmailNotification(), __('Database pruned on', 'rankology-stats') . ' ' . get_bloginfo('name'), $result_string, true,
                    array("email_title" => __('Database pruned on', 'rankology-stats') . ' <a href="' . get_bloginfo('url') . '" target="_blank" style="text-decoration: underline; color: #999999; font-family: Nunito; font-size: 13px; font-weight: 400; line-height: 150%;">' . get_bloginfo('name') . '</a>'));
            }

            return $result_string;
        } else {
            return __('Please select a value over 30 days.', 'rankology-stats');
        }
    }
        ...
        if ($purge_hits > 9) {
            ...
            if (count($to_delete) > 0) {
                ...
                $result_string = sprintf(
                    __('%s records purged successfully.', 'rankology-stats'),
                    '<code>' . count($to_delete) . '</code>'
                );
            } else {
                $result_string = __('No visitors found to purge.', 'rankology-stats');
            }
        } else {
            $result_string = __('Number of hits must be greater than or equal to 10!', 'rankology-stats');
        }
