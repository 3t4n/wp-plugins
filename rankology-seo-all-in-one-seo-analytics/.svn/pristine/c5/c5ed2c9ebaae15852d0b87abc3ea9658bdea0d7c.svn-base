<form action="<?php echo esc_url(admin_url('admin.php')); ?>" method="get" class="rkns-inline" id="jquery-datepicker">
    <?php
    if (isset($select_box)) {
        ?>
        <br/>
        <?php echo esc_attr($select_box['title']); ?>:&nbsp;<select name="<?php echo esc_attr($select_box['name']); ?>" id="<?php echo esc_attr($select_box['name']); ?>">
            ...
            ?>
        </select><input type="submit" value="<?php esc_html_e('Select', 'rankology-stats'); ?>" class="button-primary btn-danger rkns-btn-inline"><br/>
        <?php
    }
    ?>
    ...
    <?php if (!empty($DateRang['from'])) { ?>
        <input type="hidden" name="<?php echo \RANKOLOGY_STATS\Admin_Template::$request_from_date; ?>" id="date-from" value="<?php echo esc_attr($DateRang['from']); ?>">
    <?php } ?>
    <?php if (!empty($DateRang['to'])) { ?>
        <input type="hidden" name="<?php echo \RANKOLOGY_STATS\Admin_Template::$request_to_date; ?>" id="date-to" value="<?php echo esc_attr($DateRang['to']); ?>">
    <?php } ?>
</form>
...
<div class="c-pages-date-range">
    <div class="c-footer__filter js-pages-date-range-picker">
        <div class="c-footer__filter__btn-group">
            ...
            <button class="c-footer__filter__btn js-date-range-picker-btn">
                ...
                <span><?php echo __('Last 30 Days', 'rankology-stats') ?></span>
            </button>
            <button onclick="jQuery('.ranges li').map((key, value) => { if(value.classList.contains('active')) { const prevDateRange = jQuery('.ranges li')[key + 1]; prevDateRange.click(); }})" class="c-footer__filter__btn c-footer__filter__btn--sm">
                <svg width="6" height="10" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <mask id="a" fill="#fff">
    ...
    <form action="<?php echo esc_url(admin_url('admin.php')); ?>" method="get" style="display: none" class="js-date-range-picker-form">
        ...
        ?>
        <?php if (!empty($DateRang['from'])) { ?>
            <input type="hidden" name="<?php echo \RANKOLOGY_STATS\Admin_Template::$request_from_date; ?>" class="js-date-range-picker-input-from" value="<?php echo esc_attr($DateRang['from']); ?>">
        <?php } ?>
        <?php if (!empty($DateRang['to'])) { ?>
            <input type="hidden" name="<?php echo \RANKOLOGY_STATS\Admin_Template::$request_to_date; ?>" class="js-date-range-picker-input-to" value="<?php echo esc_attr($DateRang['to']); ?>">
        <?php } ?>
    </form>
</div>
