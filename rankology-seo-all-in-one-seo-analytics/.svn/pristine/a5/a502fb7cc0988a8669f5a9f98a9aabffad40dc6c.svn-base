...
class optimization_page
{
    ...
    public function processForms()
    {
        ...
        if (isset($_POST['submit']) && (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'rkns_optimization_nonce'))) {
            return;
        }
        ...
        if (isset($_POST['submit'], $_POST['hash-ips-submit']) and intval($_POST['hash-ips-submit']) == 1) {
            ...
            // Show Notice
            Helper::addAdminNotice(__('IP Addresses replaced with hash values.', "rankology-stats"), "success");
        }
        ...
        if (isset($_POST['submit'], $_POST['install-submit']) and intval($_POST['install-submit']) == 1) {
            ...
            // Show Notice
            Helper::addAdminNotice(__('Install routine complete.', "rankology-stats"), "success");
        }
            ...
            if (!empty($tables)) {
                ...
                foreach ($tables as $table) {
                    ...
                    if ('OK' === $check->Msg_text) {
                        ...
                        $notice .= sprintf(__('The %s table is okay.', "rankology-stats"), "<code>$table</code>");
                        $notice .= '<br />';
                    } else {
                        $notice .= sprintf(__('The %1$s table is not okay. It is reporting the following error: %2$s. WordPress will attempt to repair this table&hellip;', "rankology-stats"), "<code>$table</code>", "<code>$check->Msg_text</code>");
                        $repair = $wpdb->get_row("REPAIR TABLE $table");

                        $notice .= '<br />';
                        if ('OK' === $repair->Msg_text) {
                            $notice .= sprintf(__('Successfully repaired the %s table.', "rankology-stats"), "<code>$table</code>");
                        } else {
                            $notice           .= sprintf(__('Failed to repair the %1$s table. Error: %2$s', "rankology-stats"), "<code>$table</code>", "<code>$check->Msg_text</code>") . '<br />';
                    ...
                    if ($okay) {
                        ...
                        if ('Table is already up to date' === $check->Msg_text) {
                            $notice .= sprintf(__('The %s table is already optimized.', "rankology-stats"), "<code>$table</code>");
                            $notice .= '<br />';
                        } else {
                            $check = $wpdb->get_row("OPTIMIZE TABLE $table");
                            if ('OK' === $check->Msg_text || 'Table is already up to date' === $check->Msg_text) {
                                $notice .= sprintf(__('Successfully optimized the %s table.', 'rankology-stats'), "<code>$table</code>");
                                $notice .= '<br />';
                            } else {
        ...
        if (isset($_POST['submit'], $_POST['historical-submit']) and intval($_POST['historical-submit']) == 1) {
            ...
            // Historical Visitors
            if (isset($_POST['rkns_historical_visitors'])) {

                // Update DB
                $result = $wpdb->update($historical_table, array('value' => sanitize_text_field($_POST['rkns_historical_visitors'])), array('category' => 'visitors'));
                if ($result == 0) {
                    $result = $wpdb->insert($historical_table, array('value' => sanitize_text_field($_POST['rkns_historical_visitors']), 'category' => 'visitors', 'page_id' => -1, 'uri' => '-1'));
                }
            }

            // Historical Visits
            if (isset($_POST['rkns_historical_visits'])) {
                // Update DB
                $result = $wpdb->update($historical_table, array('value' => sanitize_text_field($_POST['rkns_historical_visits'])), array('category' => 'visits'));

                if ($result == 0) {
                    $result = $wpdb->insert($historical_table, array('value' => sanitize_text_field($_POST['rkns_historical_visits']), 'category' => 'visits', 'page_id' => -2, 'uri' => '-2'));
                }
            }

            // Show Notice
            Helper::addAdminNotice(__('Updated Historical Values.', "rankology-stats"), "success");
        }
    }
}
