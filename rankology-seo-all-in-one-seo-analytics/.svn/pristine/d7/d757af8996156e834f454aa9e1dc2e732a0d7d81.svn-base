...
class category_page
{
    ...
    public static function view()
    {
        ...
        // Page title
        $args['title'] = __('Category Statistics', 'rankology-stats');
        ...
        $args['select_box'] = array(
            'name'  => 'ID',
            'title' => __('Select Category', 'rankology-stats')
        );
        ...
        // Set All Item
        $args['select_box']['list'][0] = __('All', 'rankology-stats');
        ...
        if (!isset($_GET['ID']) || (isset($_GET['ID']) and $_GET['ID'] == 0)) {
            ...
            $args['top_title']     = __('Top Category Sorted by Hits', 'rankology-stats');

            // Push List Category
            foreach ($terms as $category) {
                $args['top_list'][$category->term_id] = array('ID' => $category->term_id, 'name' => $category->name, 'link' => add_query_arg('ID', $category->term_id), 'count_visit' => (int)rankology_stats_pages('total', null, $category->term_id, null, null, 'category'));
            }
            ...
            $args['top_title']     = __('Top posts Sorted by Hits in this category', 'rankology-stats');
