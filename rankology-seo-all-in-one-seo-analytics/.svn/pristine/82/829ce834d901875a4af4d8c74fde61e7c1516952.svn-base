rkns_js.countries_meta_box = {

    view: function (args = []) {
        let t = '';
        t += `<div class="o-table-wrapper"><table width="100%" class="o-table o-table--layout-fixed o-table--link table-stats rkns-report-table"><tbody>
        <tr>
            <td></td>
            <td>${rkns_js._('country')}</td>
            <td>${rkns_js._('visitor_count')}</td>
        </tr>`;
        ...
        args.countries.forEach(function (value) {
			...
			<td><img src="${value['flag']}" title="${value['name']}" alt="${value['name']}" class="rkns-flag rkns-flag--first"/> ${value['name']}</td>
			<td><a href="${value['link']}" title="${value['name']}" target="_blank">${rkns_js.number_format(value['number'])} <svg style="margin-top: 3px;" width="10" height="10" fill="none" xmlns="http://www.w3.org/2000/svg">
