...
echo apply_filters('rankology_sitemaps_urlset', $urlset);
...
if (true == get_post_type_archive_link($path) && 0 == $offset) {
    ...
    if ('1' != rankology_get_service('TitleOption')->getTitlesCptNoIndexByPath($path)) {
        $sitemap_url = '';
        $archive_links = [];
        ...
        foreach($archive_links as $loc) {
            ...
            $sitemap_url = apply_filters('rankology_sitemaps_no_archive_link', $sitemap_url, $path);

            echo apply_filters('rankology_sitemaps_url', $sitemap_url, $rankology_url);
        }
    }
}
...
$args = apply_filters('rankology_sitemaps_single_query', $args, $path);

$postslist = get_posts($args);

//primary category
function rankology_sitemaps_primary_cat_hook($cats_0, $cats, $post) {
    $primary_cat	= null;
...
foreach ($postslist as $post) {
    ...
    $rankology_mod = $modified_date;

    if(!empty($modified_date)){
        if((new DateTime($post_date)) > (new DateTime($modified_date))){
            $rankology_mod = $post_date;
        }
    }

    // primary category
    if ( $path == 'post' ) {
        add_filter('post_link_category', 'rankology_sitemaps_primary_cat_hook', 10, 3);
    }
    if ( $path == 'product' ) {
        add_filter('wc_product_post_type_link_product_cat', 'rankology_sitemaps_primary_cat_hook', 10, 3);
    }
    ...
    $rankology_url = apply_filters( 'rankology_sitemaps_single_url', $rankology_url, $post );

    if (!empty($rankology_url['loc'])) {
        $sitemapData .= sprintf("\n<url>\n<loc>%s</loc>\n<lastmod>%s</lastmod>", $rankology_url['loc'], $rankology_url['mod']);
        ...
        if ('1' === rankology_get_service('SitemapOption')->imageIsEnable()) {
            ...
            if ('yes' != get_post_meta($post, '_rankology_robots_imageindex', true)) {
                ...
                if ((isset($images) && ! empty($images) && $images->length >= 1) || (isset($product) && ! empty($product_img)) || '' != $post_thumbnail) {
                                    ...
                                    if (false === strpos($url, 'data:image/')) {
                                        ...
                                        */
                                        ...
                                        if ( ! empty($parse_url['scheme']) && ! empty($parse_url['host']) && ! empty($parse_url['path'])) {
                                            $rankology_image_loc = sprintf('<![CDATA[%s://%s]]>', $parse_url['scheme'], $parse_url['host'] . $parse_url['path']);
                                        } else {
                                            $rankology_image_loc = '<![CDATA[' . $url . ']]>';
                                            $rankology_image_loc = sprintf('<![CDATA[%s]]>', $url);
                                        }

                                        $rankology_url['images'][] = [
                                            'src'   => $rankology_image_loc,
                                        ];

                                        /*
                                        * Build up the template.
                                        */
                                        $sitemapData .= sprintf("\n<image:image>\n<image:loc>%s</image:loc>", $rankology_image_loc);
                    ...
                    if ('' != $product && '' != $product_img) {
                        foreach ($product_img as $product_attachment_id) {
                            $rankology_image_loc = '<![CDATA[' . esc_attr(wp_filter_nohtml_kses(wp_get_attachment_url($product_attachment_id))) . ']]>';
                    ...
                    //Post thumbnail
                    if ('' != $post_thumbnail) {
                        $rankology_image_loc = '<![CDATA[' . $post_thumbnail . ']]>';
                ...
                $sitemapData = apply_filters('rankology_sitemaps_single_img', $sitemapData, $post);
            }
        }
        $sitemapData .= '</url>';
    }

    echo apply_filters('rankology_sitemaps_url', $sitemapData, $rankology_url);
}
wp_reset_postdata();
?>
</urlset>
