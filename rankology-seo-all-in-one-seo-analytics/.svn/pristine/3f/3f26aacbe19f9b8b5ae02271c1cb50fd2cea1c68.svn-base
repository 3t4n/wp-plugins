...
{
    $bulk_actions['rankology_noindex'] = __('Enable noindex', 'wp-rankology');
...
function rankology_bulk_actions_index($bulk_actions) {
    $bulk_actions['rankology_index'] = __('Enable index', 'wp-rankology');
...
function rankology_bulk_actions_nofollow($bulk_actions) {
    $bulk_actions['rankology_nofollow'] = __('Enable nofollow', 'wp-rankology');
...
function rankology_bulk_actions_follow($bulk_actions) {
    $bulk_actions['rankology_follow'] = __('Enable follow', 'wp-rankology');
...
function rankology_bulk_actions_add_instant_indexing($bulk_actions) {
    $bulk_actions['rankology_instant_indexing'] = __('Add to instant indexing queue', 'wp-rankology');
...
<fieldset class="inline-edit-col-left">
    <div class="inline-edit-col column-<?php echo $column_name; ?>">
        ...
        <label class="inline-edit-group">
            ...
            <span class="input-text-wrap"><textarea cols="18" rows="1" name="rankology_desc" autocomplete="off"
...
function rankology_bulk_quick_edit_save_post($post_id) {
    ...
    $_REQUEST += ['rankology_title_edit_nonce' => ''];

    if (! wp_verify_nonce($_REQUEST['rankology_title_edit_nonce'], plugin_basename(__FILE__))) {
        return;
    }
    if (isset($_REQUEST['rankology_title'])) {
        update_post_meta($post_id, '_rankology_titles_title', esc_html($_REQUEST['rankology_title']));
    }
    if (isset($_REQUEST['rankology_desc'])) {
        update_post_meta($post_id, '_rankology_titles_desc', esc_html($_REQUEST['rankology_desc']));
    }
    if (isset($_REQUEST['rankology_tkw'])) {
        update_post_meta($post_id, '_rankology_analysis_target_kw', esc_html($_REQUEST['rankology_tkw']));
    }
    if (isset($_REQUEST['rankology_canonical'])) {
        update_post_meta($post_id, '_rankology_robots_canonical', esc_html($_REQUEST['rankology_canonical']));
    }
    ...
    if (did_action('elementor/loaded')) {
        ...
        if (! empty($elementor)) {
            if (isset($_REQUEST['rankology_title'])) {
                $elementor['_rankology_titles_title'] = esc_html($_REQUEST['rankology_title']);
            }
            if (isset($_REQUEST['rankology_desc'])) {
                $elementor['_rankology_titles_desc'] = esc_html($_REQUEST['rankology_desc']);
            }
            if (isset($_REQUEST['rankology_noindex'])) {
            ...
            if (isset($_REQUEST['rankology_canonical'])) {
                $elementor['_rankology_robots_canonical'] = esc_html($_REQUEST['rankology_canonical']);
            }
            if (isset($_REQUEST['rankology_tkw'])) {
                $elementor['_rankology_analysis_target_kw'] = esc_html($_REQUEST['rankology_tkw']);
            }
