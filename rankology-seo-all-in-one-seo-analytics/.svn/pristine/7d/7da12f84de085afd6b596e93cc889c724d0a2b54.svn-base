...
class TitleDescriptionMeta implements ExecuteHooks {
    ...
    public function register() {
        register_rest_route('rankology/v1', '/posts/(?P<id>\d+)/title-description-metas', [
            'methods'             => 'GET',
    ...
    public function processGet(\WP_REST_Request $request) {
        ...
        $title       = get_post_meta($id, '_rankology_titles_title', true);
        $description = get_post_meta($id, '_rankology_titles_desc', true);
    ...
    public function processPut(\WP_REST_Request $request) {
        ...
        $dataKeysSave = [
            'title'       => '_rankology_titles_title',
            'description' => '_rankology_titles_desc',
        ];
