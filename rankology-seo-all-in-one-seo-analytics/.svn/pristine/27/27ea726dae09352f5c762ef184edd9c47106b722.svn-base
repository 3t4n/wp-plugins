...
//Service JSON-LD
function rankology_automatic_rich_snippets_services_option($schema_datas) {
    //if no data
    if (0 != count(array_filter($schema_datas))) {
        ...
        if (get_post_meta(get_the_ID(), '_rankology_robots_canonical', true)) {
            $service_canonical = get_post_meta(get_the_ID(), '_rankology_robots_canonical', true);
        } else {
            global $wp;
            if (isset($wp->request)) {
        ...
        $json = apply_filters('rankology_schemas_auto_service_json', $json);

        $json = '<script type="application/ld+json">' . json_encode($json) . '</script>' . "\n";

        $json = apply_filters('rankology_schemas_auto_service_html', $json);
