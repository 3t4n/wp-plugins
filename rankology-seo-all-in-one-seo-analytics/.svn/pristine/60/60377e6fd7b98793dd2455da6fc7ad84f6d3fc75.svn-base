...
function googlePreview() {
    jQuery.ajax({
        method: "GET",
        url: rankologyElementorBase.rankology_real_preview,
        data: {
            action: "rankology_do_real_preview",
            post_id: rankologyElementorBase.post_id,
            tax_name: rankologyElementorBase.post_tax,
            origin: rankologyElementorBase.origin,
            post_type: rankologyElementorBase.post_type,
            rankology_analysis_target_kw: rankologyElementorBase.keywords,
            is_elementor: rankologyElementorBase.is_elementor,
            _ajax_nonce: rankologyElementorBase.rankology_nonce
        },
        success: function (t) {
            ...
            const $metaTitle = jQuery("input[data-setting=_rankology_titles_title]");
            const $metaDesc = jQuery("textarea[data-setting=_rankology_titles_desc]");

            $metaTitle.attr('placeholder', t.data.title);
            $metaDesc.attr('placeholder', t.data.meta_desc);

            if ($metaTitle.val() == '') {
                elementor.modules.controls.Rankologytextlettercounter.prototype.countLength(false, $metaTitle);
            }

            if ($metaDesc.val() == '') {
                elementor.modules.controls.Rankologytextlettercounter.prototype.countLength(false, $metaDesc);
            }
        }
    })
}
...
function socialPreviewFillData(s) {
    ...
    if (data_arr.og_title) {
        $fbTitle = jQuery('input[data-setting=_rankology_social_fb_title]');

        $fbTitle.attr('placeholder', data_arr.og_title[0]);
    }

    if (data_arr.og_desc) {
        $fbDesc = jQuery('textarea[data-setting=_rankology_social_fb_desc]');
    ...
    if (data_arr.tw_title) {
        $twTitle = jQuery('input[data-setting=_rankology_social_twitter_title]');

        $twTitle.attr('placeholder', data_arr.tw_title[0]);
    }

    if (data_arr.tw_desc) {
        $twDesc = jQuery('textarea[data-setting=_rankology_social_twitter_desc]');
...
function contentAnalysis() {
    jQuery.ajax({
        ...
        beforeSend: function () {
            jQuery(".analysis-score p span").fadeIn().text(rankologyElementorBase.i18n.progress),
        ...
        success: function (s) {
            ...
            jQuery("#rkseo-advanced-alert").empty();

            var if_noindex = new RegExp('noindex');

            if (if_noindex.test(meta_robots)) {
                jQuery("#rkseo-advanced-alert").append('<span class="impact high" aria-hidden="true"></span>');
            }

            jQuery("#rankology-analysis-tabs").load("/wp-admin/post.php?post=" + rankologyElementorBase.post_id + "&action=edit #rankology-analysis-tabs-1");
...
function rkseo_social_img(social_slug) {
    ...
    if ($("#rankology_social_fb_title_meta").length) {
        ...
        var meta_img_val = $(
            "#rankology_social_" + social_slug + "_img_meta"
        ).val();
        ...
        // Check valid URL
        if (rkseo_is_valid_url(meta_img_val) === true) {
