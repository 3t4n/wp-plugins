...
class DashboardSettings implements ExecuteHooks {
    ...
    public function register() {
        register_rest_route('rankology/v1', '/options/dashboard-settings', [
            'methods'             => 'GET',
    ...
    public function processGet(\WP_REST_Request $request) {
        $options  = get_option('rankology_dashboard_option_name');
        $toggles  = get_option('rankology_toggle');
        $notices  = get_option('rankology_notices');
