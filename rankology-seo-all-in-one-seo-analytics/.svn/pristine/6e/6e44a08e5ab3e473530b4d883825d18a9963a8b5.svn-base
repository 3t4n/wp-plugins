        ...
        if (is_admin()) {
            ...
            if ('term.php' == $pagenow || 'edit-tags.php' == $pagenow) {
                if (!empty($_GET['taxonomy'])) {
                    $rankology_get_current_tax = sanitize_title(esc_attr($_GET['taxonomy']));

                    function rankology_tax_single_enable_metabox($rankology_get_taxonomies)
                    {
                        $rankology_get_current_tax = sanitize_title(esc_attr($_GET['taxonomy']));
                        if ('1' === rankology_get_service('TitleOption')->getTaxEnable($rankology_get_current_tax) && '' !== $rankology_get_current_tax) {
...
if ('1' == rankology_get_toggle_option('google-analytics')) {
    ...
    function rankology_after_update_cart()
    {
        ...
        $html = apply_filters('rankology_gtag_ec_add_to_cart_checkout_ev', $html);
    ...
    {
        //check_ajax_referer( 'rankology_cookies_user_consent_nonce', $_GET['_ajax_nonce'], true );
