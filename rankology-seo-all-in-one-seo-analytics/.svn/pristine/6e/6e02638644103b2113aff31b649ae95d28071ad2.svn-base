    ...
    if (is_plugin_active('woocommerce/woocommerce.php')) {
        ...
        if (rankology_get_service('GoogleAnalyticsOption')->getAddToCart()) {
            ...
            function rankology_loop_add_to_cart() {
                ...
                $js = apply_filters('rankology_gtag_ec_add_to_cart_archive_ev', $js);
            ...
            function rankology_single_add_to_cart() {
                ...
                $js = apply_filters('rankology_gtag_ec_add_to_cart_single_ev', $js);
        ...
        if (rankology_get_service('GoogleAnalyticsOption')->getAddToCart() && rankology_get_service('GoogleAnalyticsOption')->getRemoveFromCart()) {
            // Before update
            add_action('woocommerce_cart_actions', 'rankology_before_update_cart');
            function rankology_before_update_cart() {
                // Extract cart
                global $woocommerce;
                foreach ($woocommerce->cart->get_cart() as $key => $item) {
                    $product = wc_get_product($item['product_id']);
                ...
                $js = apply_filters('rankology_gtag_ec_remove_from_cart_checkout_ev', $js);
