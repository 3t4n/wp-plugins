...
function rankology_do_real_preview()
{
    ...
    check_ajax_referer('rankology_real_preview_nonce', $_GET['_ajax_nonce'], true);
    ...
    if (current_user_can('edit_posts') && is_admin()) {
        ...
        if (isset($_GET['post_id'])) {
            $rankology_get_the_id = $_GET['post_id'];
        }

        if ('yes' == get_post_meta($rankology_get_the_id, '_rankology_redirections_enabled', true)) {
            $data['title'] = __('A redirect is active for this URL. Turn it off to get the Google preview and content analysis.', 'wp-rankology');
        } else {
            //Get cookies
            if (isset($_COOKIE)) {
                $cookies = [];
            ...
            if (isset($_GET['post_type'])) {
                $rankology_get_post_type = $_GET['post_type'];
            } else {
            ...
            if (isset($_GET['origin'])) {
                $rankology_origin = $_GET['origin'];
            }

            //Tax name
            if (isset($_GET['tax_name'])) {
                $rankology_tax_name = $_GET['tax_name'];
            }
            ...
            if (is_wp_error($response) || '404' == wp_remote_retrieve_response_code($response)) {
                $data['title'] = __('To get your Google snippet preview, publish your post!', 'wp-rankology');
            } elseif (is_wp_error($response) || '401' == wp_remote_retrieve_response_code($response)) {
                $data['title'] = __('Your site is protected by an authentication.', 'wp-rankology');
            } else {
                ...
                if ($dom->loadHTML('<?xml encoding="utf-8" ?>' . $response)) {
                    ...
                    if (defined('THEMIFY_DIR') && method_exists('ThemifyBuilder_Data_Manager', '_get_all_builder_text_content')) {
                        ...
                        $builder_data = $ThemifyBuilder->get_builder_data($rankology_get_the_id);
                    ...
                    if (isset($_GET['rankology_analysis_target_kw']) && !empty($_GET['rankology_analysis_target_kw'])) {
                        $data['target_kws'] = esc_html(strtolower(stripslashes_deep($_GET['rankology_analysis_target_kw'])));
                        $rankology_analysis_target_kw = array_filter(explode(',', strtolower(get_post_meta($rankology_get_the_id, '_rankology_analysis_target_kw', true))));

                        $rankology_analysis_target_kw = apply_filters('rankology_content_analysis_target_keywords', $rankology_analysis_target_kw, $rankology_get_the_id);


                        $data['target_kws_count'] = rankology_get_service('CountTargetKeywordsUse')->getCountByKeywords($rankology_analysis_target_kw, $rankology_get_the_id);
                    }


                    $data['_rankology_titles_title'] = '';

                    $data['_rankology_titles_title'] = get_the_title($rankology_get_the_id);

                    $rankology_titles_desc = get_post_meta($rankology_get_the_id, '_rankology_titles_title', true);
                    if (!empty($rankology_titles_desc)) {
                        $data['_rankology_titles_title'] = convert_text_with_dynamic_variables($rankology_titles_desc, $rankology_get_the_id);

                    }

                    $data['_rankology_titles_title_ct'] = '';

                    $meta_key = '_rankology_titles_title_ct'; // Replace with your meta key

                    $meta_value = get_term_meta($rankology_get_the_id, $meta_key, true);
                    if (!empty($meta_value)) {
                        $data['_rankology_titles_title_ct'] = $meta_value;
                    }

                    $data['_rankology_titles_desc_ct'] = '';

                    $meta_key_ct = '_rankology_titles_desc_ct'; // Replace with your meta key

                    $meta_value_ct = get_term_meta($rankology_get_the_id, $meta_key_ct, true);
                    if (!empty($meta_value_ct)) {
                        $data['_rankology_titles_desc_ct'] = $meta_value_ct;
                    }

                    if (isset($_GET['rankology_google_suggest_kw']) && !empty($_GET['rankology_google_suggest_kw'])) {
                        // Process and sanitize input tags
                        $data['rankology_google_suggest_kw'] = esc_html(strtolower(stripslashes_deep($_GET['rankology_google_suggest_kw'])));
                        $input_tags = array_filter(explode(',', $data['rankology_google_suggest_kw']));

                        // Update the meta field with new tags
                        update_post_meta($rankology_get_the_id, '_rankology_google_suggest_kw', implode(',', $input_tags));

                        // Synchronize the post's tags
                        wp_set_post_terms($rankology_get_the_id, $input_tags, 'post_tag', false); // Replace existing tags
                    } else {
                        // No tags provided, clear meta field and tags
                        delete_post_meta($rankology_get_the_id, '_rankology_google_suggest_kw');
                        wp_set_post_terms($rankology_get_the_id, '', 'post_tag'); // Clear tags
                    }

                    // Optional: Count keyword usage (if needed in your logic)
                    $data['rankology_google_suggest_count'] = rankology_get_service('CountTargetKeywordsUse')->getCountByKeywords($input_tags ?? [], $rankology_get_the_id);
                    ...
                    $data['rankology_titles_desc'] = '';
                    $rankology_titles_desc = get_post_meta($rankology_get_the_id, '_rankology_titles_desc', true);
                    $converted_text = convert_text_with_dynamic_variables($rankology_titles_desc, $rankology_get_the_id);



                    $data['rankology_titles_desc'] = esc_attr(stripslashes_deep(wp_filter_nohtml_kses($converted_text)));


                    if (empty($data['rankology_titles_desc'])) {
                        // $rankology_titles_desc = get_post_meta($rankology_get_the_id, '_rankology_titles_desc', true);

                        $data['rankology_titles_desc'] = get_the_excerpt($rankology_get_the_id);
                    ...
                    if (isset($_GET['rankology_analysis_target_kw']) && !empty($_GET['rankology_analysis_target_kw'])) {
                        ...
                        if (is_plugin_active('permalink-manager-pro/permalink-manager.php')) {
                            ...
                            $kw_slug = urldecode($permalink_manager_uris[$rankology_get_the_id]);
                        }
...
{
    check_ajax_referer('rankology_toggle_features_nonce', $_POST['_ajax_nonce'], true);
    ...
    if (current_user_can(rankology_capability('manage_options', 'dashboard')) && is_admin()) {
        if (isset($_POST['feature']) && isset($_POST['feature_value'])) {
            ...
            $rankology_toggle_options[$_POST['feature']] = esc_attr($_POST['feature_value']);
...
{
    check_ajax_referer('rankology_display_nonce', $_POST['_ajax_nonce'], true);
    if (current_user_can(rankology_capability('manage_options', 'dashboard')) && is_admin()) {
        ...
        if (isset($_POST['notifications_center'])) {
            ...
            if ('1' == $_POST['notifications_center']) {
                $rankology_advanced_option_name['rankology_advanced_appearance_notifications'] = esc_attr($_POST['notifications_center']);
            } else {
        ...
        if (isset($_POST['news_center'])) {
            ...
            if ('1' == $_POST['news_center']) {
                $rankology_advanced_option_name['rankology_advanced_appearance_news'] = esc_attr($_POST['news_center']);
            } else {
        ...
        if (isset($_POST['tools_center'])) {
            ...
            if ('1' == $_POST['tools_center']) {
                $rankology_advanced_option_name['rankology_advanced_appearance_seo_tools'] = esc_attr($_POST['tools_center']);
            } else {
...
{
    check_ajax_referer('rankology_hide_notices_nonce', $_POST['_ajax_nonce'], true);

    if (current_user_can(rankology_capability('manage_options', 'dashboard')) && is_admin()) {
        if (isset($_POST['notice']) && isset($_POST['notice_value'])) {
            $rankology_notices_options = get_option('rankology_notices');
            $rankology_notices_options[$_POST['notice']] = $_POST['notice_value'];
...
{
    check_ajax_referer('rankology_video_regenerate_nonce', $_POST['_ajax_nonce'], true);
