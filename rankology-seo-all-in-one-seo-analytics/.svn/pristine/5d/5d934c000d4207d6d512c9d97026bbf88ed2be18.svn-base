...
class SignificantKeywords {
    public function getFullContentByPost($post) {
        ...
        $disabled = apply_filters('rankology_fno_significant_kw_disable_shortcode', '__return_true');
    ...
    public function retrieveSignificantKeywords($content) {
        ...
        if (strlen($content) < apply_filters('rankology_fno_minimum_length_content_linking', 200)) {
            return [];
        }

        $languagesSupported = apply_filters('rankology_fno_stop_words_languages_supported_keywords', ['en','fr']);
        $stopWords = rankology_fno_get_service('StopWords')->setLanguages($languagesSupported);
        $content = $stopWords->clean($content);

        $words = $this->getWordsByGroup($content);

        $words = array_filter($words, function ($item) {
            return strlen($item) >= apply_filters('rankology_fno_significant_keyword_min_length', 3);
    ...
    public function computeKeywords($words, $content, $postId) {
        ...
        $total = $this->getTotalWords($content);
        $totalDocuments = rankology_fno_get_service('SignificantKeywordsRepository')->countAllDocuments();

        $limit = apply_filters('rankology_fno_significant_keywords_limit', 5);
        $y = 0;
        foreach ($words as $word => $count) {
            if($y >= $limit){
                continue;
            }
            $allWordsCorrespondent = rankology_fno_get_service('SignificantKeywordsRepository')->getAllWordsCorrespondent($word, $postId);
