...
namespace Rankology\Services\Sitemap\Render;
...
class Single {
    ...
    protected function hooksWPMLCompatibility() {
        if (defined('ICL_SITEPRESS_VERSION')) {
            ...
            add_filter('rankology_sitemaps_single_query', function ($args) {
        ...
        add_filter('rankology_sitemaps_single_url', function($url, $post) {
            //Exclude custom canonical from sitemaps
            if (get_post_meta($post->ID, '_rankology_robots_canonical', true) && get_permalink( $post->ID) !== get_post_meta($post->ID, '_rankology_robots_canonical', true)) {
                return null;
            }
    ...
    public function render() {
        ...
        ob_start();
        include_once RANKOLOGY_TEMPLATE_SITEMAP_DIR . '/single.php';
        $xml = ob_get_contents();
        ob_end_clean();

        echo apply_filters('rankology_sitemaps_xml_single', $xml);
    }
}
