...
class Admin_Template
{
    ...
    public static function get_template($template, $args = array(), $return = false)
    {
        ...
        foreach ($template as $file) {
            ...
            $template_file = RANKOLOGY_STATS_DIR . "includes/admin/templates/{$file}.php";

            if (!file_exists($template_file)) {
                Helper::doing_it_wrong(__FUNCTION__, __('Template not found.', 'rankology-stats'));
                continue;
            }
    ...
    public static function isValidDateRequest()
    {
        ...
        $default_days_ago = apply_filters('rankology_stats_days_ago_request', self::$time_ago_days);
        ...
        if ((isset($_GET[self::$request_from_date]) and !isset($_GET[self::$request_to_date])) || (!isset($_GET[self::$request_from_date]) and isset($_GET[self::$request_to_date]))) {
            return array('status' => false, 'message' => __("Request is not valid.", "rankology-stats"));
        }
        ...
        if (TimeZone::isValidDate($_GET[self::$request_from_date]) === false || TimeZone::isValidDate($_GET[self::$request_to_date]) === false) {
            return array('status' => false, 'message' => __("Time request is not valid.", "rankology-stats"));
        }
    ...
    public static function paginate_links($args = array())
    {
        ...
        if ($total_page > 1) {
            ...
            if ($args['show_now_page']) {
                $export .= '<p class="rkns-page-number">' . sprintf(__('Page %1$s of %2$s', 'rankology-stats'), number_format_i18n($args['current']), number_format_i18n($total_page)) . '</p>';
            }
            $export .= '</div>';
    ...
    public static function DateRange($page_link = false)
    {
        ...
        $date_range = array(
            10  => __('10 Days', 'rankology-stats'),
            20  => __('20 Days', 'rankology-stats'),
            30  => __('30 Days', 'rankology-stats'),
            60  => __('2 Months', 'rankology-stats'),
            90  => __('3 Months', 'rankology-stats'),
            180 => __('6 Months', 'rankology-stats'),
            270 => __('9 Months', 'rankology-stats'),
            365 => __('1 Year', 'rankology-stats')
        );
        ...
        if ($first_day != false and !isset($date_range[(int)TimeZone::getNumberDayBetween($first_day)])) {
            $date_range[(int)TimeZone::getNumberDayBetween($first_day)] = __('All', 'rankology-stats');
        }

        // Apply_filter RageTime
        $date_range = apply_filters('rankology_stats_date_time_range', $date_range);
    ...
    {
        return '<span aria-hidden="true">—</span><span class="screen-reader-text">' . __("Unknown", 'rankology-stats') . '</span>';
    }
