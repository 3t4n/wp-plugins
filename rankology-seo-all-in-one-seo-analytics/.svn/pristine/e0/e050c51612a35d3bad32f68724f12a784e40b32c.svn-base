...
<label for="rankology_robots_enable">
    <input id="rankology_robots_enable" name="rankology_fno_option_name[rankology_robots_enable]" type="checkbox" <?php if ('1' == $check) { ?>
    checked="yes"
    <?php } ?>
    value="1"/>
...
<?php } else {
        if (is_network_admin() && is_multisite()) {
            ...
            printf(
            '<textarea id="rankology_robots_file" class="rankology_robots_file" name="rankology_fno_option_name[rankology_robots_file]" rows="15" aria-label="' . __('Virtual Robots.txt file', 'wp-rankology') . '" placeholder="' . esc_html__('This is your robots.txt file!', 'wp-rankology') . '">%s</textarea>',
            esc_html($check)
            );
        } ?>
<div class="wrap-tags">
