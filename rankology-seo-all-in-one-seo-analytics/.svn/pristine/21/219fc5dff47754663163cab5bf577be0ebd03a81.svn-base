...
function rankology_block_faq_render_frontend($attributes)
{
		...
		<?php
            foreach ($attributes['faqs'] as $faq) :
                ...
                if ($accordion) {
                    ...
                    if (!isset($css)) {
                        $css = '<style>.wprankology-hide {display: none;}.wprankology-accordion-button{width:100%}</style>';
                        $css = apply_filters( 'rankology_faq_block_inline_css', $css );
                        echo $css;
                    }
                ...
                <?php echo $listItemStyle; ?>
                    <?php if ($accordion) { ?>
                        <div id="wprankology-faq-title-<?php echo $i; ?>" class="wprankology-wrap-faq-question">
                            <button class="wprankology-accordion-button" type="button" aria-expanded="false" aria-controls="wprankology-faq-answer-<?php echo $i; ?>">
                    <?php } ?>
                    ...
                    <?php if ($accordion) { ?>
                        <div id="wprankology-faq-answer-<?php echo $i; ?>" class="wprankology-faq-answer wprankology-hide" aria-labelledby="wprankology-faq-title-<?php echo $i; ?>">
                    <?php } else { ?>
                    ...
                    <?php if (! empty($image_url)): ?>
                        ...
                        <?php if (! empty($faq['answer'])): ?>
                            <p class="wprankology-faq-answer-desc"><?php echo $faq['answer']; ?></p>
                        <?php endif; ?>
                    </div>
    ...
    if ( (bool) $attributes['isProActive'] && (int) $attributes['showFAQScheme'] ) {
        ...
        echo apply_filters('rankology_schemas_faq_html', $schema);
    }
