...
///////////////////////////////////////////////////////////////////////////////////////////////////
function rankology_seo_framework_migration() {
    check_ajax_referer('rankology_seo_framework_migrate_nonce', $_POST['_ajax_nonce'], true);
                ...
                foreach ($seo_framework_query_terms as $term_id) {
                    if ('' != get_term_meta($term_id, 'autodescription-term-settings', true)) {
                        ...
                        if ( ! empty($term_settings['doctitle'])) { //Import title tag
                            update_term_meta($term_id, '_rankology_titles_title', $term_settings['doctitle']);
                        }
                        if ( ! empty($term_settings['description'])) { //Import meta desc
                            update_term_meta($term_id, '_rankology_titles_desc', $term_settings['description']);
                        }
            ...
            if ($seo_framework_query) {
                foreach ($seo_framework_query as $post) {
                    if ('' != get_post_meta($post->ID, '_genesis_title', true)) { //Import title tag
                        update_post_meta($post->ID, '_rankology_titles_title', get_post_meta($post->ID, '_genesis_title', true));
                    }
                    if ('' != get_post_meta($post->ID, '_genesis_description', true)) { //Import meta desc
                        update_post_meta($post->ID, '_rankology_titles_desc', get_post_meta($post->ID, '_genesis_description', true));
                    }
                    if ('' != get_post_meta($post->ID, '_open_graph_title', true)) { //Import Facebook Title
                        update_post_meta($post->ID, '_rankology_social_fb_title', get_post_meta($post->ID, '_open_graph_title', true));
                    }
                    if ('' != get_post_meta($post->ID, '_open_graph_description', true)) { //Import Facebook Desc
                        update_post_meta($post->ID, '_rankology_social_fb_desc', get_post_meta($post->ID, '_open_graph_description', true));
                    }
                    if ('' != get_post_meta($post->ID, '_social_image_url', true)) { //Import Facebook Image
                        update_post_meta($post->ID, '_rankology_social_fb_img', get_post_meta($post->ID, '_social_image_url', true));
                    }
                    if ('' != get_post_meta($post->ID, '_twitter_title', true)) { //Import Twitter Title
                        update_post_meta($post->ID, '_rankology_social_twitter_title', get_post_meta($post->ID, '_twitter_title', true));
                    }
                    if ('' != get_post_meta($post->ID, '_twitter_description', true)) { //Import Twitter Desc
                        update_post_meta($post->ID, '_rankology_social_twitter_desc', get_post_meta($post->ID, '_twitter_description', true));
                    }
                    if ('' != get_post_meta($post->ID, '_social_image_url', true)) { //Import Twitter Image
                        update_post_meta($post->ID, '_rankology_social_twitter_img', get_post_meta($post->ID, '_social_image_url', true));
                    }
                    ...
                    if ('' != get_post_meta($post->ID, '_genesis_canonical_uri', true)) { //Import Canonical URL
                        update_post_meta($post->ID, '_rankology_robots_canonical', get_post_meta($post->ID, '_genesis_canonical_uri', true));
                    }
                    if ('' != get_post_meta($post->ID, 'redirect', true)) { //Import Redirect URL
                        ...
                        update_post_meta($post->ID, '_rankology_redirections_value', get_post_meta($post->ID, 'redirect', true));
                    }
...
add_action('wp_ajax_rankology_seo_framework_migration', 'rankology_seo_framework_migration');
