...
*/
///////////////////////////////////////////////////////////////////////////////////////////////////
function rankology_smart_crawl_migration() {
    check_ajax_referer('rankology_smart_crawl_migrate_nonce', $_POST['_ajax_nonce'], true);
                ...
                foreach ($smart_crawl_query_terms as $taxonomies => $taxonomie) {
                    foreach ($taxonomie as $term_id => $term_value) {
                        if ( ! empty($term_value['wds_title'])) { //Import title tag
                            update_term_meta($term_id, '_rankology_titles_title', $term_value['wds_title']);
                        }
                        if ( ! empty($term_value['wds_desc'])) { //Import meta desc
                            update_term_meta($term_id, '_rankology_titles_desc', $term_value['wds_desc']);
                        }
                        if ( ! empty($term_value['opengraph']['title'])) { //Import Facebook Title
                            update_term_meta($term_id, '_rankology_social_fb_title', $term_value['opengraph']['title']);
                        }
                        if ( ! empty($term_value['opengraph']['description'])) { //Import Facebook Desc
                            update_term_meta($term_id, '_rankology_social_fb_desc', $term_value['opengraph']['description']);
                        }
                        if ( ! empty($term_value['opengraph']['images'])) { //Import Facebook Image
                            ...
                            if (isset($img_url) && '' != $img_url) {
                                update_term_meta($term_id, '_rankology_social_fb_img', $img_url);
                            }
                        }
                        if ( ! empty($term_value['twitter']['title'])) { //Import Facebook Title
                            update_term_meta($term_id, '_rankology_social_twitter_title', $term_value['twitter']['title']);
                        }
                        if ( ! empty($term_value['twitter']['description'])) { //Import Facebook Desc
                            update_term_meta($term_id, '_rankology_social_twitter_desc', $term_value['twitter']['description']);
                        }
                        if ( ! empty($term_value['twitter']['images'])) { //Import Facebook Image
                            ...
                            if (isset($img_url) && '' != $img_url) {
                                update_term_meta($term_id, '_rankology_social_twitter_img', $img_url);
                            }
                        }
                        ...
                        if ('' != $term_value['wds_canonical']) { //Import Canonical URL
                            update_term_meta($term_id, '_rankology_robots_canonical', $term_value['wds_canonical']);
                        }
                    }
                }
            }
            ...
            if ($smart_crawl_query) {
                foreach ($smart_crawl_query as $post) {
                    if ('' != get_post_meta($post->ID, '_wds_title', true)) { //Import title tag
                        update_post_meta($post->ID, '_rankology_titles_title', get_post_meta($post->ID, '_wds_title', true));
                    }
                    if ('' != get_post_meta($post->ID, '_wds_metadesc', true)) { //Import meta desc
                        update_post_meta($post->ID, '_rankology_titles_desc', get_post_meta($post->ID, '_wds_metadesc', true));
                    }
                    if ('' != get_post_meta($post->ID, '_wds_opengraph', true)) {
                        ...
                        if ( ! empty($_wds_opengraph['title'])) {
                            update_post_meta($post->ID, '_rankology_social_fb_title', $_wds_opengraph['title']); //Import Facebook Title
                        }
                        if ( ! empty($_wds_opengraph['description'])) { //Import Facebook Desc
                            update_post_meta($post->ID, '_rankology_social_fb_desc', $_wds_opengraph['description']);
                        }
                        if ( ! empty($_wds_opengraph['images'])) { //Import Facebook Image
                            ...
                            if (isset($img_url) && '' != $img_url) {
                                update_post_meta($post->ID, '_rankology_social_fb_img', $img_url);
                            }
                        }
                    }
                    if ('' != get_post_meta($post->ID, '_wds_twitter', true)) {
                        ...
                        if ( ! empty($_wds_twitter['title'])) {
                            update_post_meta($post->ID, '_rankology_social_twitter_title', $_wds_twitter['title']); //Import Twitter Title
                        }
                        if ( ! empty($_wds_twitter['description'])) { //Import Twitter Desc
                            update_post_meta($post->ID, '_rankology_social_twitter_desc', $_wds_twitter['description']);
                        }
                        if ( ! empty($_wds_twitter['images'])) { //Import Twitter Image
                            ...
                            if (isset($img_url) && '' != $img_url) {
                                update_post_meta($post->ID, '_rankology_social_twitter_img', $img_url);
                            }
                        }
                    }
                    ...
                    if ('' != get_post_meta($post->ID, '_wds_canonical', true)) { //Import Canonical URL
                        update_post_meta($post->ID, '_rankology_robots_canonical', get_post_meta($post->ID, '_wds_canonical', true));
                    }
                    if ('' != get_post_meta($post->ID, '_wds_redirect', true)) { //Import Redirect URL
                        ...
                        update_post_meta($post->ID, '_rankology_redirections_type', '301');
                        update_post_meta($post->ID, '_rankology_redirections_value', get_post_meta($post->ID, '_wds_redirect', true));
                    }
                    if ('' != get_post_meta($post->ID, '_wds_focus-keywords', true)) { //Import Focus Keywords
                        update_post_meta($post->ID, '_rankology_analysis_target_kw', get_post_meta($post->ID, '_wds_focus-keywords', true));
                    }
                }
            }
            $offset += $increment;
...
add_action('wp_ajax_rankology_smart_crawl_migration', 'rankology_smart_crawl_migration');
