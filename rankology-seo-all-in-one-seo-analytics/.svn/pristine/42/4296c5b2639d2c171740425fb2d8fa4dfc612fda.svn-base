...
function rankology_squirrly_migration() {
    check_ajax_referer('rankology_squirrly_migrate_nonce', $_POST['_ajax_nonce'], true);
            ...
            foreach ($count_query as $value) {
                ...
                if (0 != $post_id && ! empty($value['seo'])) {
                    ...
                    if ('' != $seo['title']) { //Import title tag
                        update_post_meta($post_id, '_rankology_titles_title', $seo['title']);
                    }
                    if ('' != $seo['description']) { //Import description tag
                        update_post_meta($post_id, '_rankology_titles_desc', $seo['description']);
                    }
                    if ('' != $seo['og_title']) { //Import Facebook Title
                        update_post_meta($post_id, '_rankology_social_fb_title', $seo['og_title']);
                    }
                    if ('' != $seo['og_description']) { //Import Facebook Desc
                        update_post_meta($post_id, '_rankology_social_fb_desc', $seo['og_description']);
                    }
                    if ('' != $seo['og_media']) { //Import Facebook Image
                        update_post_meta($post_id, '_rankology_social_fb_img', $seo['og_media']);
                    }
                    if ('' != $seo['tw_title']) { //Import Twitter Title
                        update_post_meta($post_id, '_rankology_social_twitter_title', $seo['tw_title']);
                    }
                    if ('' != $seo['tw_description']) { //Import Twitter Desc
                        update_post_meta($post_id, '_rankology_social_twitter_desc', $seo['tw_description']);
                    }
                    if ('' != $seo['tw_media']) { //Import Twitter Image
                        update_post_meta($post_id, '_rankology_social_twitter_img', $seo['tw_media']);
                    }
                    ...
                    if ('' != $seo['canonical']) { //Import canonical
                        update_post_meta($post_id, '_rankology_robots_canonical', $seo['canonical']);
                    }
                }
            }
            $offset = 'done';
