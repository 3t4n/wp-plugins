 ...
 * @var string
 */
$api_key = apply_filters( 'rankology_video_youtube_api_key', 'AIzaSyAQ6Dy36nerusenbAt3xCClcOpOwAnpRu0');
 ...
 * @var string
 */
$content = apply_filters('rankology_fno_video_sitemap_content', $post->post_content);
...
if (preg_match_all('/(youtu\.be\/|youtube\.com\/|youtube-nocookie\.com\/)(v\/|watch\?v=|embed\/)?([a-zA-Z0-9\-_]*)/', $content, $matches, PREG_SET_ORDER)) {
    ...
    delete_post_meta($post->ID, '_rankology_video_xml_yt');
}
    ...
    foreach ($video_details as $video) {
        if (!is_wp_error($video) && !empty($video['body'])) {
            ...
            if (!empty($video_data->items)) {
                ...
                update_post_meta($post->ID, '_rankology_video_xml_yt', $video_xml);
            }
        }
    }
}
