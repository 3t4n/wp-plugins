...
class Admin_User
{
    ...
    {
        $column['visits'] = __("Visits", "rankology-stats");
        return $column;
    }
    ...
    public function modify_user_table_row($val, $column_name, $user_id)
    {
        switch ($column_name) {
            case 'visits' :
                ...
                return '<a href="' . Menus::admin_url('visitors', array('user_id' => $user_id)) . '" class="rkns-text-muted" target="_blank">' . number_format_i18n($count) . '</a>';
            default:
        }
        return $val;
    }
