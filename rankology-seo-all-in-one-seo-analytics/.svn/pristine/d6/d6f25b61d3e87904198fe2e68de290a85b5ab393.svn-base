...
//Events JSON-LD
function rankology_automatic_rich_snippets_events_option($schema_datas)
{
    //if no data
    if (0 != count(array_filter($schema_datas))) {
        ...
        if ('' != $events_status) {
            $events_status = rankology_check_ssl() . 'schema.org/' . $events_status;
        }
        ...
        if ($events_status == rankology_check_ssl() . 'schema.org/EventRescheduled' && '' != $events_previous_start_date) {
        ...
        $json = apply_filters('rankology_schemas_auto_event_json', $json);

        $json = '<script type="application/ld+json">' . json_encode($json) . '</script>' . "\n";

        $json = apply_filters('rankology_schemas_auto_event_html', $json);
