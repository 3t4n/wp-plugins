...
class Schedule
{
    ...
    public function __construct()
    {
        ...
        if (is_admin()) {
            ...
            if (!Helper::is_request('ajax')) {
                ...
                if (!wp_next_scheduled('rankology_stats_geoip_hook') && Option::get('schedule_geoip') && Option::get('geoip')) {
                    wp_schedule_event(time(), 'daily', 'rankology_stats_geoip_hook');
                }

                // Remove the GeoIP update schedule if it does exist and it should shouldn't.
                if (wp_next_scheduled('rankology_stats_geoip_hook') && (!Option::get('schedule_geoip') || !Option::get('geoip'))) {
                    wp_unschedule_event(wp_next_scheduled('rankology_stats_geoip_hook'), 'rankology_stats_geoip_hook');
                }

                //Construct Event
                add_action('rankology_stats_geoip_hook', array($this, 'geoip_event'));
            }

        } else {

            // Add the report schedule if it doesn't exist and is enabled.
            if (!wp_next_scheduled('rankology_stats_report_hook') && Option::get('stats_report')) {
                wp_schedule_event(time(), Option::get('time_report'), 'rankology_stats_report_hook');
            }
            ...
            if (!wp_next_scheduled('rankology_stats_referrerspam_hook') && Option::get('schedule_referrerspam')) {
                wp_schedule_event(time(), 'weekly', 'rankology_stats_referrerspam_hook');
            }

            // Remove the referrerspam update schedule if it does exist and it should shouldn't.
            if (wp_next_scheduled('rankology_stats_referrerspam_hook') && !Option::get('schedule_referrerspam')) {
                wp_unschedule_event(wp_next_scheduled('rankology_stats_referrerspam_hook'), 'rankology_stats_referrerspam_hook');
            }

            // Add the database maintenance schedule if it doesn't exist and it should be.
            if (!wp_next_scheduled('rankology_stats_dbmaint_hook') && Option::get('schedule_dbmaint')) {
                wp_schedule_event(time(), 'daily', 'rankology_stats_dbmaint_hook');
            }

            // Remove the database maintenance schedule if it does exist and it shouldn't.
            if (wp_next_scheduled('rankology_stats_dbmaint_hook') && (!Option::get('schedule_dbmaint'))) {
                wp_unschedule_event(wp_next_scheduled('rankology_stats_dbmaint_hook'), 'rankology_stats_dbmaint_hook');
            }

            // Add the visitor database maintenance schedule if it doesn't exist and it should be.
            if (!wp_next_scheduled('rankology_stats_dbmaint_visitor_hook') && Option::get('schedule_dbmaint_visitor')) {
                wp_schedule_event(time(), 'daily', 'rankology_stats_dbmaint_visitor_hook');
            }

            // Remove the visitor database maintenance schedule if it does exist and it shouldn't.
            if (wp_next_scheduled('rankology_stats_dbmaint_visitor_hook') && (!Option::get('schedule_dbmaint_visitor'))) {
                wp_unschedule_event(wp_next_scheduled('rankology_stats_dbmaint_visitor_hook'), 'rankology_stats_dbmaint_visitor_hook');
            }

            // Remove the add visit row schedule if it does exist and it shouldn't.
            if (wp_next_scheduled('rankology_stats_add_visit_hook') && (!Option::get('visits'))) {
                wp_unschedule_event(wp_next_scheduled('rankology_stats_add_visit_hook'), 'rankology_stats_add_visit_hook');
            }

            // Add the add visit table row schedule if it does exist and it should.
            if (!wp_next_scheduled('rankology_stats_add_visit_hook') && Option::get('visits')) {
                wp_schedule_event(time(), 'daily', 'rankology_stats_add_visit_hook');
            }

            //After construct
            add_action('rankology_stats_add_visit_hook', array($this, 'add_visit_event'));
            add_action('rankology_stats_dbmaint_hook', array($this, 'dbmaint_event'));
            add_action('rankology_stats_dbmaint_visitor_hook', array($this, 'dbmaint_visitor_event'));
            add_action('rankology_stats_report_hook', array($this, 'send_report'));
        }
    }
    ...
    static function define_schedules_time($schedules)
    {
        ...
        $RANKOLOGY_Stats_schedules = array(
            'weekly'   => array(
                'interval' => 604800,
                'display'  => __('Once Weekly'),
            ),
        );
        foreach ($RANKOLOGY_Stats_schedules as $key => $val) {
    ...
    public function add_visit_event()
    {
        ...
        if (!$insert) {
            if (!empty($wpdb->last_error)) {
                \RANKOLOGY_Stats::log($wpdb->last_error);
            }
        }
    }
    ...
    public function geoip_event()
    {
        ...
        $this_update = strtotime(__('First Tuesday of this month', 'rankology-stats')) + (86400 * 2);
    ...
    public function send_report()
    {
        ...
        if ($type == 'mail') {
             ...
             * @usage rankology-stats-advanced-reporting
             */
            $final_report_text = apply_filters('rankology_stats_final_text_report_email', $email_content);

            /**
             * Filter for enable/disable sending email by template.
             */
            $email_template = apply_filters('rankology_stats_report_email_template', true);

            /**
             * Email receivers
             */
            $email_receivers = apply_filters('rankology_stats_report_email_receivers', Option::getEmailNotification());             
            ...
            $result_email = Helper::send_mail(
                $email_receivers,
                __('Stats reporting', 'rankology-stats'),
                $final_report_text,
                $email_template
            );
             ...
             */
            do_action('rankology_stats_after_report_email', $result_email, $email_receivers, $email_content);
