    ...
    if (!empty($search_engines)) {
        foreach ($search_engines as $key => $value) {
            ...
            <div class="rankology_wrap_single_cpt">
                <label
                    for="rankology_instant_indexing_engines_<?php echo $key; ?>">
                    <input
                        id="rankology_instant_indexing_engines_<?php echo $key; ?>"
                        name="rankology_instant_indexing_option_name[engines][<?php echo $key; ?>]"
...
function rankology_instant_indexing_google_action_callback() {
    ...
    $actions = [
        'URL_UPDATED' => __('Update URLs', 'wp-rankology'),
...
<div class="rankology_wrap_single_cpt">
    ...
    <label
        for="rankology_instant_indexing_google_action_include_<?php echo $key; ?>">
        <input
            id="rankology_instant_indexing_google_action_include_<?php echo $key; ?>"
...
if (!empty($bing_response['response'])) {
    switch ($bing_response['response']['code']) {
        case 200:
            $msg = __('URLs submitted successfully', 'wp-rankology');
            break;
        case 202:
            $msg = __('URL received. IndexNow key validation pending.', 'wp-rankology');
            break;
        case 400:
            $msg = __('Bad request: Invalid format', 'wp-rankology');
            break;
        case 403:
            $msg = __('Forbidden: In case of key not valid (e.g. key not found, file found but key not in the file)', 'wp-rankology');
            break;
        case 422:
            $msg = __('Unprocessable Entity: In case of URLs don’t belong to the host or the key is not matching the schema in the protocol', 'wp-rankology');
            break;
        case 429:
            $msg = __('Too Many Requests: Too Many Requests (potential Spam)', 'wp-rankology');
            break;
        default:
            $msg = __('Something went wrong', 'wp-rankology');
    } ?>
...
function rankology_instant_indexing_google_api_key_callback() {
    ...
    printf(
    '<textarea id="rankology_instant_indexing_google_api_key" name="rankology_instant_indexing_option_name[rankology_instant_indexing_google_api_key]" rows="12" placeholder="' . esc_html__('Paste your Google JSON key file here', 'wp-rankology') . '" aria-label="' . __('Paste your Google JSON key file here', 'wp-rankology') . '">%s</textarea>',
    esc_html($check));
}
