...
function rankology_display_seo_term_metaboxe() {
    ...
    function rankology_tax($term) {
        ...
        $rankology_titles_title_ct             = get_term_meta($term->term_id, '_rankology_titles_title_ct', true);
        $rankology_titles_desc              = get_term_meta($term->term_id, '_rankology_titles_desc_ct', true);
    ...
    function rankology_tax_save_term($term_id) {
        ...
        if (in_array('title-tab', $seo_tabs)) {
            if (!empty($_POST['rankology_titles_title_ct'])) {
                
                  update_term_meta($term_id, '_rankology_titles_title_ct', esc_html($_POST['rankology_titles_title_ct']));
            } else {
                 delete_term_meta($term_id, '_rankology_titles_title_ct');
            }
            if (!empty($_POST['rankology_titles_desc_ct'])) {
               
                update_term_meta($term_id, '_rankology_titles_desc_ct', esc_html($_POST['rankology_titles_desc_ct']));
            } else {
                delete_term_meta($term_id, '_rankology_titles_desc_ct');
            }
            if (isset($_POST['rankology_analysis_target_kw']) && !empty($_POST['rankology_analysis_target_kw'])) {
                // Process the input keywords and convert them into an array
                $input_keywords = esc_html(strtolower(stripslashes_deep($_POST['rankology_analysis_target_kw'])));
                $keywords_array = array_filter(array_map('trim', explode(',', $input_keywords)));
            
                // Update the term meta as an array
                update_term_meta($term_id, '_rankology_analysis_target_kw', $keywords_array);
            
                // Apply any filters to the keywords array if necessary
                $filtered_keywords = apply_filters('rankology_content_analysis_target_keywords', $keywords_array, $term_id);
            
                // Calculate the count of target keywords usage
                $target_kws_count = rankology_get_service('CountTargetKeywordsUse')->getCountByKeywords($filtered_keywords, $term_id);
            
                // Store the count in a separate field if needed
                update_term_meta($term_id, '_rankology_analysis_target_kw_count', $target_kws_count);
            }
        ...
        if (in_array('advanced-tab', $seo_tabs)) {
            ...
            if (!empty($_POST['rankology_robots_canonical'])) {
                update_term_meta($term_id, '_rankology_robots_canonical', esc_html($_POST['rankology_robots_canonical']));
            } else {
        ...
        if (in_array('social-tab', $seo_tabs)) {
            ...
            if (!empty($_POST['rankology_social_fb_title'])) {
                update_term_meta($term_id, '_rankology_social_fb_title', esc_html($_POST['rankology_social_fb_title']));
            } else {
                delete_term_meta($term_id, '_rankology_social_fb_title');
            }
            if (!empty($_POST['rankology_social_fb_desc'])) {
                update_term_meta($term_id, '_rankology_social_fb_desc', esc_html($_POST['rankology_social_fb_desc']));
            } else {
                delete_term_meta($term_id, '_rankology_social_fb_desc');
            }
            if (!empty($_POST['rankology_social_fb_img'])) {
                update_term_meta($term_id, '_rankology_social_fb_img', esc_html($_POST['rankology_social_fb_img']));
            }
            if (!empty($_POST['rankology_social_fb_img_attachment_id']) && !empty($_POST['rankology_social_fb_img'])) {
                update_term_meta($term_id, '_rankology_social_fb_img_attachment_id', esc_html($_POST['rankology_social_fb_img_attachment_id']));
            } else {
            ...
            if (!empty($_POST['rankology_social_fb_img_width']) && !empty($_POST['rankology_social_fb_img'])) {
                update_term_meta($term_id, '_rankology_social_fb_img_width', esc_html($_POST['rankology_social_fb_img_width']));
            } else {
            ...
            if (!empty($_POST['rankology_social_fb_img_height']) && !empty($_POST['rankology_social_fb_img'])) {
                update_term_meta($term_id, '_rankology_social_fb_img_height', esc_html($_POST['rankology_social_fb_img_height']));
            } else {
            ...
            if (!empty($_POST['rankology_social_twitter_title'])) {
                update_term_meta($term_id, '_rankology_social_twitter_title', esc_html($_POST['rankology_social_twitter_title']));
            } else {
                delete_term_meta($term_id, '_rankology_social_twitter_title');
            }
            if (!empty($_POST['rankology_social_twitter_desc'])) {
                update_term_meta($term_id, '_rankology_social_twitter_desc', esc_html($_POST['rankology_social_twitter_desc']));
            } else {
                delete_term_meta($term_id, '_rankology_social_twitter_desc');
            }
            if (!empty($_POST['rankology_social_twitter_img'])) {
                update_term_meta($term_id, '_rankology_social_twitter_img', esc_html($_POST['rankology_social_twitter_img']));
            }
        }
        if (in_array('redirect-tab', $seo_tabs)) {
            if (isset($_POST['rankology_redirections_type'])) {
                update_term_meta($term_id, '_rankology_redirections_type', $_POST['rankology_redirections_type']);
            }
            if (isset($_POST['rankology_redirections_logged_status'])) {
                update_term_meta($term_id, '_rankology_redirections_logged_status', $_POST['rankology_redirections_logged_status']);
            }
            if (!empty($_POST['rankology_redirections_value'])) {
                update_term_meta($term_id, '_rankology_redirections_value', esc_html($_POST['rankology_redirections_value']));
            } else {
        ...
        do_action('rankology_seo_metabox_term_save', $term_id, $_POST);
    }
}
