...
class TargetKeywords implements ExecuteHooks
{
    ...
    {
        register_rest_route('rankology/v1', '/posts/(?P<id>\d+)/target-keywords', [
            'methods'             => 'GET',
    ...
    public function processGet(\WP_REST_Request $request)
    {
        ...
        $targetKeywords   =  array_filter(explode(',', strtolower(get_post_meta($id, '_rankology_analysis_target_kw', true))));
    ...
    public function processPut(\WP_REST_Request $request)
    {
        ...
        if (!isset($params['_rankology_analysis_target_kw'])) {
            return new \WP_REST_Response([
                'code'         => 'error',
        ...
        try {
            $targetKeywords = implode(',',array_map('trim', explode(',',$params['_rankology_analysis_target_kw'])));
            update_post_meta($id, '_rankology_analysis_target_kw', $targetKeywords);
