<?php
/*
 * Plugin Name:    Remove WooCommerce tabs & fields
...
namespace RemoveTabsAndFieldsFromWooCommerce
{
	...
	class PluginClass extends \Puvox\wp_plugin
	{
	  ...
	  public function declare_settings()
	  {
		...
		[
			...
			//
			'fill_billing_info_by_user' => false,
		]; 
	}
	...
	{ 
		...
		//
		$this->default_billing_values_init();
	}
	...
	public function HintDescriptionPart($image, $extra_comment = '')
	{
		return '<div class="'.$this->sName.'_eachblock">Typically, WooCommerce has such output: <br/><img src="'.$this->helpers->baseURL.'/assets/'.$image.'.png" /><br/> So, you can customize them</div>' . ($extra_comment !=='' ? '<p>'.$extra_comment.'</p>' : '');
	}
	...
	public function get_settings() 
	{
		...
		$settings[] = [
			...
			'type'      => 'title',
			'desc'      => $this->HintDescriptionPart('fields_on_checkout', '(Note, these settings are not applied to BLOCK-BASED checkeout page, you might need to setup separate page with `[woocommerce_checkout]` shortcode so these settings would have an effect)'),
			'id'        => $this->optName.'_5'
		];
	...
	// ##########################################################
	public function default_billing_values_init() {
		if ($this->opts['fill_billing_info_by_user']) {
			$arr = ['billing_first_name', 'billing_last_name', 'billing_email', 'billing_phone'];
			foreach ($arr as $key) {
				add_filter('default_checkout_' .  $key, 	[$this, 'default_billing_values'], 11, 2);  
			}
			// add_filter('woocommerce_checkout_get_value', 	[$this, 'default_billing_values'], 1115, 2);  
		}
	}
		
	
	public function default_billing_values( $value, $input_name ) {
		
		global $current_user;
		$usermeta = get_user_meta( $current_user->ID );
		switch ( $input_name ) :
			case 'billing_first_name':
			case 'shipping_first_name': return !empty($value) ? $value : $current_user->first_name;
			break;
			
			case 'billing_last_name':
			case 'shipping_last_name': return !empty($value) ? $value : $current_user->last_name;
			break;
	
			case 'billing_email': return !empty($value) ? $value : $current_user->data->user_email;
			break;
	
			case 'billing_phone': return !empty($value) ? $value : ( $current_user->phone ? $current_user->phone : '000000000000');
			break;
			// case 'billing_country': return '';
			// break;
	
			// case 'billing_city': return '';
	...
	{ 
		...
		{ 
			...
			$this->opts['fill_billing_info_by_user']			 = !empty($_POST[ $this->plugin_slug ]['fill_billing_info_by_user']);
			$this->update_opts(); 
		}
		?>
		...
		<div class="postbox">
		  ...
		  <tr class="def">
				<th scope="row">
					<?php _e('for logged-in users, billing details (first-name, last-name and email) can be filled automatically (only works if you do not hide them)'); ?>
				</th>
				<td>
					<p>
						<label>
							<input name="<?php echo $this->plugin_slug;?>[fill_billing_info_by_user]" type="radio" value="0" <?php checked(!$this->opts['fill_billing_info_by_user']); ?>><?php _e( 'No' );?>
						</label>
						<label>
							<input name="<?php echo $this->plugin_slug;?>[fill_billing_info_by_user]" type="radio" value="1" <?php checked( $this->opts['fill_billing_info_by_user']); ?>><?php _e( 'Yes' );?>
						</label>
					</p>
				</td>
			</tr>
