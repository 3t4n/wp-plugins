	...
	if (wp_verify_nonce($_POST['file_upload_nonce'], 'couponapi')) {
		...
		if (!$movefile or isset($movefile['error'])) {
			...
			if (!$result['error']) {
				...
				$sql = "SELECT * FROM " . $wp_prefix . "couponapi_config WHERE name IN ('API_KEY','pingback_setting')";
...
{
	$supported = array('clipmydeals', 'couponis', 'couponer', 'mts_coupon', 'clipper', 'couponxl', 'couponxxl', 'rehub', 'rehub-theme', 'wpcoupon', 'wp-coupon', 'wp-coupon-pro', 'CP', 'cp', 'CPq', 'couponhut', 'coupon-mart', 'coupon-press');
...
function pull_full_feed()
{
	...
	if ($api_key != $_GET['api_key']) {
		...
		if ($result['totalCounter'] == 0) {
			...
			wp_schedule_single_event(time(), 'couponapi_process_batch_event'); // process next batch
			if($config['pingback_setting'] == 'On') {
				couponapi_pingback_call($api_key, 'start');
			}
			return 'OK';
		} else {
			$wpdb->query('ROLLBACK');
