...
function couponapi_pull_incremental_feed() {
	...
	$config = $wpdb->get_row("SELECT
																	...
																	(SELECT value FROM " . $wp_prefix . "couponapi_config WHERE name = 'pingback_setting') pingback_setting
																FROM dual");
	...
	if ($result['totalCounter'] == 0) {
		...
		if($config->pingback_setting == 'On') {
			couponapi_pingback_call($config->API_KEY, 'start');
		}
...
function couponapi_process_batch() {
	...
	$result = $wpdb->get_results("SELECT * FROM " . $wp_prefix . "couponapi_config WHERE name IN ('API_KEY','import_images','cashback','batch_size','brandlogos_key', 'use_grey_image', 'use_logos', 'size', 'import_locations','pingback_setting','ctype_code','ctype_deal','store','category','code_text','expiry_text','generic_import_image','set_as_featured_image')");
	...
	} elseif ($theme == 'coupon-press') {
		couponapi_coupon_press_process_batch($config, $coupons);
	} else {
	...
	if ($remainingCoupons > 0) {
		...
		if($config['pingback_setting'] == 'On') {
			couponapi_pingback_call($config['API_KEY'], 'complete');
		}
	}
}
...
function couponapi_couponer_process_batch($config, $coupons) {
	...
	foreach ($coupons as $coupon) {
		...
		if ($coupon->status == 'new' or $coupon->status == '') {
			...
			update_post_meta($post_id, 'capi_id', $coupon->offer_id);
			update_post_meta($post_id, 'pending_shop_url', $coupon->affiliate_link);
			...
			update_post_meta($post_id, 'code_api', $coupon->affiliate_link);
			...
			foreach ($store_names as $str) {
				...
				if (!term_exists($str, 'coupon_store')) {
					...
					if (!is_wp_error($term)) { // Term did not exist. Got inserted now.
						...
						if (!empty($coupon->brand_logo) and $config['use_logos'] == 'on') {
							$attach_id = couponapi_import_image(couponapi_brandlogo_url($config, $coupon->brand_logo), $config['use_grey_image']);
							update_term_meta($term['term_id'], '_wpc_store_image_id', $attach_id);
							update_term_meta($term['term_id'], '_wpc_store_image', wp_get_attachment_image_url($attach_id, 'full'));
						}
						update_term_meta($term['term_id'], '_wpc_store_url', $coupon->merchant_home_page);
						update_term_meta($term['term_id'], '_wpc_store_name', $str);
					}
				} else {
					$term  = get_term_by('name', $str, 'coupon_store');
					$attach_id = get_term_meta($term->term_id, '_wpc_store_image_id');
				}
				wp_set_object_terms($post_id, $str, 'coupon_store', $append);
				$append = true;
			}
...
function couponapi_coupon_press_process_batch($config, $coupons) {
	global $wpdb;
	$wp_prefix = $wpdb->prefix;
	...
	foreach ($coupons as $coupon) {
		...
		if ($coupon->status == 'new' or $coupon->status == '') {
			...
			if(!empty($coupon->label)) {
				update_post_meta($post_id, '_wpc_coupon_save', $coupon->label);
			}
			$count_updated = $count_updated + 1;
		} elseif ($coupon->status == 'suspended') {
...
function couponapi_brandlogo_url($config, $url, $brand = '') {
	...
	$token = hash('sha256', $config['brandlogos_key'] . $brand);
...
function couponapi_pingback_call($api_key,$status) {
	$url = "https://couponapi.org/api/ping_setting.php?API_KEY=". $api_key ."&status=". $status ."&ping_url=". urlencode(site_url('wp-cron.php'));
	wp_remote_get($url,array('timeout' => 0.1));
}