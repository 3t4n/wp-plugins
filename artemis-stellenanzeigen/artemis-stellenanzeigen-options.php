...
function ASA_plugin_admin_init() {
    ...
    add_settings_field('plugin_default_perPage', 'Einträge pro Seite:', 'ASA_plugin_setting_perPage', 'artemis-wp-plugin', 'plugin_main');
...
function ASA_plugin_setting_perPage() {
    $options = get_option('plugin_options');
    $default_value = isset($options['default_perPage']) ? intval($options['default_perPage']) : 10;
    echo "<input id='plugin_default_perPage' name='plugin_options[default_perPage]' type='number' min='1' max='100' value='" . esc_attr($default_value) . "' />";
    echo "<p class='description'>Legen Sie fest, wie viele Stellenanzeigen pro Seite angezeigt werden sollen. Standard: 10</p>";
}
...
function ASA_plugin_options_validate($input) {
    ...
    // Validiere und setze die perPage Option
    $default_perPage = isset($input['default_perPage']) ? intval($input['default_perPage']) : 10;
    $options['default_perPage'] = max(1, min(100, $default_perPage)); // Begrenze auf Werte zwischen 1 und 100
