...
function ObtenerModalidadVentaExportacion()
{
    $modalidadventa = array(
        "1" => "A Firme / Sale agreed",
        "2" => "Bajo condición / Under condition",
        "3" => "En consignación libre / Free consignation",
        "4" => "En consignación con mínimo firme / Consignation with minimum sale",
        "9" => "Sin pago / No payment"
    );
    return $modalidadventa;
}
function ObtenerClausulaVentaExportacion()
{
    $clausulaventaexportacion = array(
        "1" => "CIF",
        "2" => "CFR",
        "3" => "EXW",
        "4" => "FAS",
    ...
    asort($clausulaventaexportacion);
    return $clausulaventaexportacion;
}
...
function facto_fe_custom_checkout_field($checkout)
{
    ...
    elseif ((get_option('facto_checkbox_fex') == "on") && (get_option('facto_docs_default') == "fex"))
    {
        $opciones['fex'] = 'Factura exportacion electronica';
    }
    ...
    if ((get_option('facto_checkbox_fex') == "on") && (get_option('facto_docs_default') != "fex"))
    {
        $opciones['fex'] = 'Factura exportacion electronica';
    }
    
    $modalidadVenta = ObtenerModalidadVentaExportacion();
    $clausulaVenta = ObtenerClausulaVentaExportacion();
    ...
    $array_modalidadVenta = array(
        'type'        => 'select',
        'options'     => $modalidadVenta,
        'class'       => array('form-row-wide'),
        'label'       => __('Modalidad Venta'),
        'required' => true,
        'placeholder' => __('Seleccione modalidad de venta'),
        'id' => 'modalidad_venta'
    );
    $array_clausulaVenta = array(
        'type'        => 'select',
        'options'     => $clausulaVenta,
        'class'       => array('form-row-wide'),
        'label'       => __('Clausula Venta'),
        'required' => true,
        'placeholder' => __('Seleccione clausula de venta'),
        'id' => 'clausula_venta'
    );
    ...
    $array_campo_rut = array(
        'type' => 'text',
        'class' => array('form-row-wide'),
        'label' => __('RUT / ID Extranjero / Tax ID'),
        'required' => true,
    ...
    <div id="div_datos_de_facto"><br>
        <h3 id="titulo_tipo_documento">Seleccione tipo de documento</h3>
        ...
        <div id='datosExportacion_div_facturaExportacion'> <?php
            woocommerce_form_field('modalidad_venta', $array_modalidadVenta, $checkout->get_value('modalidad_venta'));
            woocommerce_form_field('clausula_venta', $array_clausulaVenta, $checkout->get_value('clausula_venta'));
            ?>
        </div>
    </div>
    <?php
...
{
    $active_currency_plugin = 0;
    if (sanitize_text_field($_POST['kind_of_legal_invoice'])) {
        if (isset($_POST['billing_country']) && sanitize_text_field($_POST['billing_country']) !== ''){
        ...
        if (sanitize_text_field($_POST['kind_of_legal_invoice']) == "fe" || sanitize_text_field($_POST['kind_of_legal_invoice'] == "fee")) {
            ...
            }elseif (sanitize_text_field($_POST['kind_of_legal_invoice']) == "fex"){
                if (sanitize_text_field(!$_POST['campo_giro'])) wc_add_notice(__('Debe indicar un giro.'), 'error');
                wc_add_notice(__('El país no puede ser Chile para factura de exportación.'), 'error');
                if (sanitize_text_field(!$_POST['modalidad_venta'])) wc_add_notice(__('Debe seleccionar una modalidad de venta.'), 'error');
                if (sanitize_text_field(!$_POST['clausula_venta'])) wc_add_notice(__('Debe seleccionar una clausula de venta.'), 'error');
        }
        }else{
            //revisamos si tiene habilitada la factura de exportacion
            if((get_option('facto_checkbox_fex')) == "on"){
                //se obtiene array con los plugins activos
                $plugins = get_option( 'active_plugins', array() );
                //se recorre array y se verifica que exista el plugin de tipo de cambio
                foreach ($plugins as $plugin) {
                    if($plugin == "woocommerce-currency-switcher/index.php"){
                        $active_currency_plugin = 1;
                    }
                }
                if($active_currency_plugin == 0){
                    wc_add_notice(__('Para continuar debe seleccionar país Chile'), 'error');
                }else{
                    if (sanitize_text_field($_POST['kind_of_legal_invoice']) == "fex"){
                        if (sanitize_text_field(!$_POST['campo_giro'])) wc_add_notice(__('Debe indicar un giro.'), 'error');
                    }else{
                        wc_add_notice(__('Seleccione Factura de exportación.'), 'error');
                    }
                }
            }else{
                wc_add_notice(__('No se puede generar una Factura de exportación.'), 'error');
            }
        }
    } else wc_add_notice(__('No se puede generar la venta. Cambie tipo de moneda o país de emisión'), 'error');
...
function facto_fe_custom_checkout_field_update_order_meta($order_id)
{
    ...
    if (!empty($_POST['modalidad_venta'])) {
        update_post_meta($order_id, 'Modalidad venta', sanitize_text_field($_POST['modalidad_venta']));
    }
    if (!empty($_POST['clausula_venta'])) {
        update_post_meta($order_id, 'Clausula venta', sanitize_text_field($_POST['clausula_venta']));
    }
    ...
    if ($order->get_status() != "failed")
    {
        ...
        if (isset($respuesta['msg'])) {
            ...
            if (isset($respuesta['doc_link'])) {
                print "<input type='button' id='seeDocument' class ='button alt' style='margin-top: 10px;' onclick=\"window.open('" . esc_url($respuesta['doc_link']) . "','_blank');\"";
