...
{
    add_submenu_page('woocommerce', 'Configuraciones', 'FACTO', 'manage_options', 'facto_settings', 'facto_fe_submenu_settings_callback');
...
function facto_fe_submenu_settings_api()
{
 ...
 <form method="post" action="options.php" id="facto_formulario">
    ...
    <h2>Ingresar llave de acceso API</h2>
		...
		<p><b>CREA TU LLAVE:</b> Ingresa a tu cuenta en <a href="https://www.facto.cl" target="_blank">www.facto.cl</a>, y dirígete a Administración >> API >> Llave de acceso API y presiona la opción de crear llave</p>
		
		<p><b>NO CONFUNDAS TU LLAVE API:</b> Recuerda que tu llave API NO ES tu usuario y clave de FACTO.</p>
			...
			<tbody>
				...
				<tr valign="top">
					...
					<select
						...
						print ">PRODUCCIÓN - Integración a tu FACTO y generación real de documentos</option>";
...
<td colspan="2"><b>** Documentos adicionales ** </b></td>
            ...
            <tr valign="top">
                <th scope="row" class="titledesc"><label for="facto_checkbox_fex">Factura exportacion electronica</label></th>
                <td class="forminp forminp-checkbox"><input type="checkbox"
                                                            name="facto_checkbox_fex" id="facto_checkbox_fex" value="on"
                        <?php if (esc_attr(get_option('facto_checkbox_fex')) == "on") echo "checked"; ?> />
                </td>
                <td scope="row">
                    <p>*Si marcas esta opción necesitas instalar el plugin "FOX - Currency Switcher Professional for WooCommerce" que soporta CLP y USD.</p>
                </td>
            </tr>
			</tbody>
		</table>
		...
		<tr valign="top" id="facto_tr_user">
					...
					<td class="forminp forminp-text forminp-text-facto">
                    <select name="facto_docs_default" id="facto_docs_default">
						 <?php 
                        $docs_default = [
                            ...
                            'fee' => 'Factura exenta electrónica',
                            'be'  => 'Boleta electrónica',
                            'bee' => 'Boleta exenta electrónica',
						 
                            'fex' => 'Factura de exportación electrónica',
                        ];
						 
                        $docSelected = get_option('facto_docs_default');
                        foreach ($docs_default as $val => $name) {
                            $selected = ($docSelected === $val) ? 'selected' : '';
...
<h2>Avanzadas</h2>
		...
		<table class="form-table">
            ...
            <tr valign="top">
                ...
                <li>3. En la pestaña impuestos (Tax) de woocommerce, ingresar a "Tasas estándar" (Standard rates) y agregar una nueva fila con código de país "CL" y nombre de impuesto "IVA" (mantener mayúsculas) y tasa en 19%.</li>
...
function facto_fe_submenu_settings_logs()
{
    ...
    <h2>Log de comunicación con FACTO</h2>
...
function facto_fe_submenu_settings_help()
{
    ...
    <p>Este módulo conecta tu tienda WooCommerce con FACTO, permitiéndote recopilar automáticamente los datos de facturación de tus clientes y emitir de manera sencilla documentos tributarios electrónicos. Podrás generar:</p>
    <ul>
        <li>Facturas</li>
        <li>Facturas electrónicas.</li>
        <li>Facturas exentas electrónicas.</li>
        <li>Facturas de exportación electrónicas.</li>
        <li>Boletas</li>
        <li>Boletas electrónicas.</li>
        <li>Boletas exentas electrónicas.</li>
    </ul>
    <p>Además, el módulo permite realizar operaciones en moneda extranjera, como dólares estadounidenses (USD). Para la emisión de facturas de exportación, es obligatorio utilizar moneda USD y seleccionar un país distinto a Chile, cumpliendo con los requisitos tributarios correspondientes.</p>
    
    <h3>¿Cuál es el costo de habilitación?</h3>
    
    <p>Para mayor información visita nuestra página <a href="https://www.facto.cl/producto/integracion-facto-api/" target="_blank">AQUI</a></p>
...
<img src="<?php echo plugin_dir_url( __FILE__ ) ?>images/facto-logo.png" width="160"/>
</a>
</div>
<h1>Integración Factura Electrónica a tu Woocommerce</h1>
    ...
    <a href="?page=facto_settings&tab=api" class="nav-tab <?php if ((!isset($_REQUEST['tab'])) || ($_REQUEST['tab'] == "api"))
{
    print " nav-tab-active";
            ...
            <a href="?page=facto_settings&tab=docs" class="nav-tab <?php if (isset($_REQUEST['tab']) && $_REQUEST['tab'] == "docs")
{
    print " nav-tab-active";
}?>">Documentos</a>
            <a href="?page=facto_settings&tab=issuanceconditions" class="nav-tab <?php if (isset($_REQUEST['tab']) && $_REQUEST['tab'] == "issuanceconditions")
{
    print " nav-tab-active";
}?>">Condiciones para emitir</a>
            <a href="?page=facto_settings&tab=advanced" class="nav-tab <?php if (isset($_REQUEST['tab']) && $_REQUEST['tab'] == "advanced")
{
    print " nav-tab-active";
}?>">Avanzado</a>
            <a href="?page=facto_settings&tab=logs" class="nav-tab <?php if (isset($_REQUEST['tab']) && $_REQUEST['tab'] == "logs")
{
    print " nav-tab-active";
}?>">Logs</a>
            <a href="?page=facto_settings&tab=help" class="nav-tab <?php if (isset($_REQUEST['tab']) && $_REQUEST['tab'] == "help")
{
    print " nav-tab-active";
}?>">Ayuda</a>
</h2>
...
function facto_fe_register_facto_settings()
{
    ...
    register_setting('facto_settings_group_docs', 'facto_checkbox_fex');
    ...
    */
    ...
    if (isset($_REQUEST['facto_fe_facturar'])) {
        ...
        <input type="hidden" name="oml_order_id" id="oml_order_id" value="<?php echo esc_attr($order->get_id()); ?>" />
				...
				<h4><img src="<?php echo plugin_dir_url( __FILE__ ) ?>images/facto-isotipo.png" style="width:15px;">FACTO: Integración facturación electrónica</h4>
...
function facto_fe_custom_checkout_field_display_admin_order_meta($order)
{
    ...
    if (get_post_meta($order->get_id(), 'Tipo de documento', true) == "fex")
    {
        echo '<p><strong>' . __('Tipo de documento') . ':</strong> Factura exportacion electronica</p>';
    }
    if (get_post_meta($order->get_id(), 'Tipo de documento', true) == "fe" || get_post_meta($order->get_id(), 'Tipo de documento', true) == "fee" || get_post_meta($order->get_id(), 'Tipo de documento', true) == "fex")
    {
        ...
        if (get_post_meta($order->get_id(), 'Tipo de documento', true) == "fex"){
            echo '<p><strong>' . __('Modalidad venta') . ':</strong> ' . esc_html(get_post_meta($order->get_id(), 'Modalidad venta', true)) . '</p>';
            echo '<p><strong>' . __('Clausula venta') . ':</strong> ' . esc_html(get_post_meta($order->get_id(), 'Clausula venta', true)) . '</p>';
    }
