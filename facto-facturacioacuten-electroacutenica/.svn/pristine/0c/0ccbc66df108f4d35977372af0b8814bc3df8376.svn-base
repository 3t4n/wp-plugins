...
function facto_fe_data_orden($order_id)
{
    ...
    if (isset($WOOCS) && method_exists($WOOCS, 'set_currency')) {
    $WOOCS->set_currency($currency);
    $woocs_order_rate=$WOOCS->woocs_exchange_value(1000000);
    $woocs_order_rate = $woocs_order_rate /1000000;
    }else{
        $woocs_order_rate = 1;
    }
    $woocs_order_country = $order->get_billing_country();
    $data = array();
    $data['order_id'] = $order_id;
    
    $tipoDoc = get_post_meta($order_id, 'Tipo de documento', true);
    if (isset($tipoDoc)){
        if ($tipoDoc == "be")
    {
        $data['tipo_dte'] = 39;
    ...
    //acceder a datos personalizados
    $tax_id = get_post_meta($order_id, 'RUT', true);
    $razon_social = get_post_meta($order_id, 'Razon social', true);
    if (empty($razon_social))
        {
        $razon_social = get_post_meta($order_id, 'Raz&oacute;n social', true);
        }
    $giro = get_post_meta($order_id, 'Giro', true);
    $modalidadventa = get_post_meta($order_id, 'Modalidad venta', true);
    $clausulaventa = get_post_meta($order_id, 'Clausula venta', true);
        
    if ($data['tipo_dte'] == 33 || $data['tipo_dte'] == 34 || $data['tipo_dte'] == 110) {
        $data['receptor_razon'] = $razon_social;
        $data['receptor_giro'] = $giro;
        if ($data['tipo_dte'] == 110){
            $data['receptor_idextranjero'] = $tax_id;
            $data['modalidad_venta'] = $modalidadventa;
            $data['clausula_venta'] = $clausulaventa;
    } else {
            $data['receptor_rut'] = $tax_id;
        }
    }
    else {
        $data['receptor_rut'] = "";
        $data['receptor_idextranjero'] = "";
        $data['receptor_razon'] = "";
        $data['receptor_giro'] = "";
    ...
    if($data['tipo_dte'] != 110 || $woocs_order_country == "CL"){
        $data['pais_emision'] = "CL";
        $data['receptor_comuna'] = $billing_address["state"];
    }elseif ($woocs_order_country != "CL" && $data['tipo_dte'] == 110){
        if($data['receptor_idextranjero'] == ""){
            $data['receptor_idextranjero'] = "55.555.555-5";
        }
        if($data['receptor_razon'] == ""){
            $data['receptor_razon'] = $billing_address["first_name"] . " " . $billing_address["last_name"];
        }
        $data['pais_emision'] = $woocs_order_country;
        $data['valor_tipo_cambio'] = $woocs_order_rate;
        $data['tipo_cambio'] = $currency;
        $data['receptor_razon'] = $razon_social;
        $data['receptor_giro'] = $giro;
        $data['modalidad_venta'] = $clausulaventa;
        $data['clausula_venta'] = $clausulaventa;
    }
    $data['receptor_direccion'] = $billing_address["address_1"];
    $data['receptor_ciudad'] = $billing_address["city"];
    
    $data['receptor_telefono'] = $billing_address["phone"];
    $data['receptor_email'] = $billing_address["email"];
    ...
    foreach ($items as $item) {
        ...
        // Veamos si tenemos variaciones del producto
            ...
            $detalle['cantidad'] = (int)$item->get_quantity();
        $detalle['unidad'] = ""; //$item['type'];
            $detalle['glosa'] = $item->get_name();
        ...
        if(get_option('facto_checkbox_totalneto') && wc_tax_enabled())
        {
            ...
            $tax_label = "";            
                ...
                if ((isset($item['tax_class'])) && ($item['tax_class'] != "")) {
                    ...
                    if(($item['tax_class'] == "ila-cerveza") || ($item['tax_class'] == "ila-vino")){
                            if($data['pais_emision'] == "CL" && $currency == "USD"){
                                $detalle['taxes'] = ($item['total'] * 0.205);
                            }elseif($data['pais_emision'] != "CL" && $currency == "CLP"){
                                $detalle['taxes'] = ($item['total'] * 0.205)*$woocs_order_rate;
                            }
                            else{
                        $detalle['taxes'] = round ($item['total'] * 0.205);
                            ...
                            if($data['pais_emision'] == "CL" && $currency == "USD"){
                                $detalle['taxes'] = ($item['total'] * 0.315);
                            }else{
            ...
            //if ($tax_label == "IVA") {
                ...
                $detalle['monto_unitario'] = round($item['line_total'] / $detalle['cantidad'],6);
                        ...
                        if($data['pais_emision'] == "CL" && $currency == "USD"){
                            $total_afecto += round(($detalle['cantidad'] * $detalle['monto_unitario']) / $woocs_order_rate);
                            $total_afecto_bruto += round(($item['line_total'] * 1.19) / $woocs_order_rate);
                        }elseif($data['pais_emision'] != "CL" && $currency == "CLP"){
                            $total_afecto += round(($detalle['cantidad'] * $detalle['monto_unitario']) * $woocs_order_rate);
                            $total_afecto_bruto += round(($item['line_total'] * 1.19) * $woocs_order_rate);
                        }else{
                ...
                $detalle['monto_unitario'] = round($item['line_total'] / $detalle['cantidad'],6);
                        ...
                        if($data['pais_emision'] == "CL" && $currency == "USD"){
                            $total_exento += round(($detalle['cantidad'] * $detalle['monto_unitario']) / $woocs_order_rate);
                        }elseif($data['pais_emision'] != "CL" && $currency == "CLP"){
                            $total_exento += round(($detalle['cantidad'] * $detalle['monto_unitario']) * $woocs_order_rate);
                        }else{
                $total_exento += round($detalle['cantidad'] * $detalle['monto_unitario']);
        ...
        else
        {
                ...
                if ($data['tipo_dte'] == 34 || $data['tipo_dte'] == 41 || $data['tipo_dte'] == 110)
            {
    ...
    if ($gastos_de_envio > 0 && (esc_attr(get_option('facto_checkbox_add_shipping')) == 1 ||
            esc_attr(get_option('facto_checkbox_add_shipping')) == 3 || esc_attr(get_option('facto_checkbox_add_shipping')) == "")) {
        $detalle = array();
        $detalle['sku'] = "";
        $detalle['cantidad'] = 1;
        $detalle['unidad'] = "uni";
    ...
    if($data['pais_emision'] == "CL" && $currency == "USD"){
        ...
        {
            $detalles[$i]['total_tax'] = round($detalles[$i]['total_tax'] / $woocs_order_rate);
            $detalles[$i]['monto_unitario'] = round($detalles[$i]['monto_unitario'] / $woocs_order_rate);
            if(isset($detalles[$i]['taxes'])){
                $detalles[$i]['taxes'] = round($detalles[$i]['taxes'] / $woocs_order_rate);
                $total_iva += $detalles[$i]['taxes'];
            }
        }
    }
    if($data['pais_emision'] != "CL" && $currency == "CLP"){
        for($i = 0 ; $i < sizeof($detalles); $i++)
        {
            $detalles[$i]['total_tax'] = round($detalles[$i]['total_tax'] * $woocs_order_rate);
            $detalles[$i]['monto_unitario'] = round($detalles[$i]['monto_unitario'] * $woocs_order_rate);
    ...
    $total_iva = $total_iva + round($total_afecto*0.19);
...
function facto_fe_comunicacion_api($data)
{
    ...
    if ($data["tipo_dte"] == 110){
        $receptor_idextranjero = $data['receptor_idextranjero'];
        $modalidad_venta = $data["modalidad_venta"];
        $clausula_venta = $data["clausula_venta"];
    }else{
    ...
    try
    {
        ...
        if ($tipo_dte == 110){
            $cadena_xml .= "<receptor_idextranjero xsi:type='xsd:string'>" . facto_fe_encodingparaxml($receptor_idextranjero) . "</receptor_idextranjero>
                            <modalidad_venta xsi:type='xsd:string'><![CDATA[" . facto_fe_encodingparaxml($modalidad_venta) . "]]></modalidad_venta>
							<clausula_venta xsi:type='xsd:string'><![CDATA[" . facto_fe_encodingparaxml($clausula_venta) . "]]></clausula_venta>";
        }else{
            $cadena_xml .= "<receptor_rut xsi:type='xsd:string'>" . facto_fe_encodingparaxml($receptor_rut) . "</receptor_rut>";
        }
        $cadena_xml .= "	<receptor_razon xsi:type='xsd:string'><![CDATA[" . facto_fe_encodingparaxml($receptor_razon) . "]]></receptor_razon>
            ...
            if ($response["enlaces"]["dte_pdf"])
            {
                ...
                else
                {
                    ...
                    if ($tipo_dte == 33) {
                        ...
                        $mensajesalida = "El documento se ha generado correctamente / The document has been generated successfully.";
                        $doc_description = "Electronic invoice ";
                    }
