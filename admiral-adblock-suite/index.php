    ...
    try {
        ...
        if ($didInitialize && (!function_exists('is_admin') || !is_admin()) && !$isLogin) {
            add_action('wp_print_scripts', function() {
                ...
                if (!empty($embed) && !\wp\AdmiralAdBlockAnalytics::getHasEmbedded()) {
...
// only load the protect hooks if protect is on
if (\wp\AdmiralAdBlockAnalytics::isProtectEnabled()) {
    require_once('protectHooks.php');
}

if (\wp\AdmiralAdBlockAnalytics::isProxyAdmiralEnabled()) {
    require_once('proxyHooks.php');
}
