...
class Revenue_Functions {
	 ...
	 */
	public function get_available_campaigns( $product_id, $placement, $display_type = '', $position = '', $with_ids = false, $is_cart = false ) {
	...
	public function calculate_campaign_offered_price( $offer_type, $offer_value, $regular_price, $with_save_data = false, $offer_qty = 1 ) {
		...
		switch ( $offer_type ) {
			...
			case 'fixed_price':
				...
				$save_data['type']    = 'amount';
				$save_data['value']   = $offer_value;
				$save_data['message'] = sprintf( __( 'Save %s', 'revenue' ), wc_price( floatval( $regular_price ) - floatval( $offer_value ) ) );
				break;
	...
	public function campaign_style_generator( $type = 'inpage', $campaign = array(), $placement = '' ) {
		...
		if ( 'drawer' == $type ) {
			...
			if ( 'spending_goal' === $campaign['campaign_type'] || 'free_shipping_bar' === $campaign['campaign_type'] ) {
				$styles = revenue()->get_campaign_meta( $campaign['id'], 'builderdata', true )[ $type ];
			} else {
	...
	private static function load_cart_product_ids() {
		$cart_items             = WC() ? WC()->cart->get_cart() : array();
