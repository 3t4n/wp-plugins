...
class Revenue_Campaign {
	...
	public function run_hellobar_campaings() {
		...
		$campaigns = revenue()->get_available_campaigns( $post->ID, 'all_page', 'top' );
	...
	public function run_campaigns( $campaigns, $display_type = 'inpage', $placement = '', $position = '' ) {
		...
		foreach ( $campaigns as $campaign ) {
			...
			if ( isset( $campaign['campaign_type'] ) && 'spending_goal' === $campaign['campaign_type'] ) {

			} else {
				if ( 'publish' != $campaign['campaign_status'] ) {
					continue;
				}
				if ( 'double_order' != $campaign['campaign_type'] && 'spending_goal' != $campaign['campaign_type'] && 'free_shipping_bar' != $campaign['campaign_type'] ) {
			...
			if ( isset( $display_type_methods[ $display_type ] ) ) {
				...
				if ( $class ) {
					...
					if ( method_exists( $class, $method ) ) {
						...
						error_log( "Method $method does not exist in the class." );
						throw new Exception( "Method $method does not exist in the provided class." );
					}
	...
	public function render_campaign_view_shortcode( $atts ) {
		...
		if ( ! $campaign || ! isset( $campaign['campaign_type'] ) ) {
			return false;
		}
