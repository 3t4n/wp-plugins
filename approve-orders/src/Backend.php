...
class Backend {
	...
	public function replace_footer( $text ) {
		...
		if ( isset( $_GET['page'] ) && 'approve-orders' === sanitize_text_field( $_GET['page'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$text = __( 'Like Approve Orders for WooCommerce? Give it a', 'approve-orders' );

			$text .= ' <a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/support/plugin/approve-orders/reviews/?rate=5#new-post">';

			$text .= __( '★★★★★', 'aaprove-orders' ) . '</a>' . __( ' rating. A huge thanks in advance!', 'approve-orders' );
		}
	...
	public function replace_version( $text ) {
		...
		if ( isset( $_GET['page'] ) && 'approve-orders' === sanitize_text_field( $_GET['page'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$text = __( 'Version ', 'approve-orders' ) . AOFWC_VERSION;
		}
