...
defined( 'AOFWC_VERSION' ) or define( 'AOFWC_VERSION', '1.0.2' );
...
if ( !function_exists( 'ao_fs' ) ) {
    ...
    function ao_fs() {
        ...
        if ( !isset( $ao_fs ) ) {
            ...
            include_once AOFWC_DIR . '/vendor/freemius/wordpress-sdk/start.php';
            $ao_fs = fs_dynamic_init( array(
                'id'               => '16430',
                'slug'             => 'approve-orders',
                'type'             => 'plugin',
                'public_key'       => 'pk_99df04bd377ed9db2761480450c5b',
                'is_premium'       => false,
                'has_addons'       => false,
                'has_paid_plans'   => true,
                ...
                'menu'             => array(
                    'slug'       => 'approve-orders',
                    'first-path' => 'admin.php?page=approve-orders',
                    'support'    => false,
                ),
                'is_live'          => true,
                'anonymous_mode'   => true,
...
class ApproveOrders {
    ...
    public function action_links( $links ) {
        $links = array_merge( array('<a href="' . esc_url( admin_url( 'admin.php?page=approve-orders' ) ) . '">' . esc_html( 'Settings', 'approve-orders' ) . '</a>', '<a target="blank" href="' . esc_url( 'https://nitin247.com/support/' ) . '">' . __( 'Support Desk', 'approve-orders' ) . '</a>'), $links );
        return $links;
    }
    ...
    public function multisite_admin_notification() {
		...
		<div class="notice notice-error">
			<p><?php 
        esc_html_e( 'Multisite Not Supported, Please deactivate Approve Orders plugin', 'approve-orders' );
        ?> .</p>
		</div>
		<?php 
    }
