...
class PrePayGateway extends \WC_Payment_Gateway {
	...
	public function __construct() {
		...
		// Add Prepay Gateway to Block Checkout
		// Block Checkout Compatibility
		//add_filter( 'woocommerce_blocks_checkout_payment_gateway', array( $this, 'block_checkout_gateway' ), 10, 2 );
	}
	...
	/**
	 * Add the PrePay Gateway to the Block Checkout payment section
	 */
	// public function block_checkout_gateway( $gateway, $payment_method ) {
	// 	if ( 'preordergateway-approve-orders' === $payment_method->id ) {
	// 		$gateway['title']       = $this->title;
	// 		$gateway['description'] = $this->description;
	// 		$gateway['fields']      = array(
	// 			'reference_number' => array(
	// 				'label'    => __( 'Reference Number', 'approve-orders' ),
	// 				'type'     => 'text',
	// 				'required' => true,
	// 			),
	// 		);
