...
class CSS_JS_Files {
	...
	public static function insert_css() {
		self::insert_css_files(array(get_option('css_js_files_css_files')), get_option('css_js_files_timestamp'));
		if (self::is_post()) {
			global $post;
			//self::insert_css_files(get_post_meta($post->ID, 'css_js_files_css_files'), strtotime($post->post_modified));
			self::insert_css_files(get_post_meta($post->ID, 'css_js_files_css_files'), get_option('css_js_files_timestamp'));
		}
	}
	public static function insert_css_files($files, $version = 0) {
		if (is_array($files) && count($files) && $files[0] != '') {
			foreach (explode(',', $files[0]) as $file) {
				if ($file != '') {
					$uri = content_url().'/'.$file.'?v='.$version;;
	...
	public static function insert_js() {
		self::insert_js_files(array(get_option('css_js_files_js_files')), get_option('css_js_files_timestamp'));
		if (self::is_post()) {
			global $post;
			//self::insert_js_files(get_post_meta($post->ID, 'css_js_files_js_files'), strtotime($post->post_modified));
			self::insert_js_files(get_post_meta($post->ID, 'css_js_files_js_files'), get_option('css_js_files_timestamp'));
		}
	}
	public static function insert_js_files($files, $version = 0) {
		if (is_array($files) && count($files) && $files[0] != '') {
			foreach (explode(',', $files[0]) as $file) {
				if ($file != '') {
					$uri = content_url().'/'.$file.'?v='.$version;;
