...
class terms_popup_on_user_login {
	...
	private function load_dependencies() {
		...
		require_once plugin_dir_path(dirname(__FILE__)) . 'includes/license-key/TPUL_LicenseKeyHandler.php';
		require_once plugin_dir_path(dirname(__FILE__)) . 'utils/terms-popup-login-page-utility.php';
		require_once plugin_dir_path(dirname(__FILE__)) . 'service/terms-text-snapshot-hash-service.php';
		require_once plugin_dir_path(dirname(__FILE__)) . 'service/terms-content-service.php';
		require_once plugin_dir_path(dirname(__FILE__)) . 'service/class-terms-popup-userstate-serivce.php';
		require_once plugin_dir_path(dirname(__FILE__)) . 'service/terms-reset-service.php';
		require_once plugin_dir_path(dirname(__FILE__)) . 'service/terms-apisettings-service.php';
		require_once plugin_dir_path(dirname(__FILE__)) . 'service/terms-userstate-migrate-service.php';
		...
		require_once plugin_dir_path(dirname(__FILE__)) . 'includes/db/class-terms-popup-on-user-login-base-db-api.php';
		require_once plugin_dir_path(dirname(__FILE__)) . 'includes/db/class-terms-popup-on-user-login-db-service.php';
		require_once plugin_dir_path(dirname(__FILE__)) . 'includes/db/class-terms-user-state-model.php';
	...
	private function define_admin_hooks() {
		...
		/**
		 * Add Modal to the WP admin if modal was set
		 */

		$popup_type = new TPUL_Popup_Type();
		$should_show_on_wp_admin = $popup_type->should_show_on_wp_admin();
		if ($should_show_on_wp_admin) {

			$plugin_modal = new Terms_Popup_On_User_Login_Modal($this->get_plugin_name(), $this->get_version());

			// Add the actual Modal to footer
			$this->loader->add_action('admin_footer', $plugin_modal, 'add_modal_to_footer');

			// Add Body Class to trigger modal popup
			$this->loader->add_action('admin_body_class', $plugin_modal, 'add_admin_slug_body_class', 10, 1);
			// Add Body Class to track location if option is turned on
			$this->loader->add_action('admin_body_class', $plugin_modal, 'location_tracking_body_class', 10, 1);
		...
		/**
		 * Settings link for plugin listing interface
		 */
		$tpul_utils = new Terms_Popup_On_User_Utils();
		$this->loader->add_filter('plugin_action_links_' . plugin_basename(dirname(__DIR__) . '/terms-popup-on-user-login.php'), $tpul_utils, 'tpul_add_settings_link');
	}
	...
	private function define_public_hooks() {
		...
		/**
		 * Same but for login page
		 */
		if ($popup_type->is_loginpage_modal()) {

			$this->loader->add_action('login_footer', $plugin_modal, 'add_modal_to_footer');

			$this->loader->add_action('login_body_class', $plugin_modal, 'add_slug_body_class', 10, 1);
			$this->loader->add_action('login_body_class', $plugin_modal, 'location_tracking_body_class', 10, 1);
			/**
			 * Enqueue script for login page
			 */
			$plugin_public_assets = new Terms_Popup_On_User_Login_Public($this->get_plugin_name(), $this->get_version());
			$this->loader->add_action('login_enqueue_scripts', $plugin_public_assets, 'enqueue_scripts');
			$this->loader->add_action('login_enqueue_scripts', $plugin_public_assets, 'enqueue_styles');
		}
