...
class TPUL_Moddal_Visibility_Manager {
    ...
    private $debug_log_to_file = false;
    ...
    public function should_modal_render() {
        ...
        //     // Get it with logic
        //     $should_popup_render =  $this->should_modal_render_logic($this->debug);
        //     if ($this->debug_cache) error_log(" Cache MISS X-> should_popup_render:  " . $should_popup_render);
    ...
    public function should_modal_render_logic() {
        ...
        if ($this->debug) error_log("Popup Type:" . $this->popup_type->get_popup_type());

        /**
         * If login page modal and we are on login page
         */
        if ($this->popup_type->is_loginpage_modal()) {
            if (TPUL_LoginPage_Utility::is_login_page()) {
                if ($this->debug) error_log("Decision: true -- is_login_page");
                if ($this->debug_log_to_file) {
                    $this->debug_txt_logger->write_log($this->debug_current_user . "Decision: true" . " -- Reason: Login page modal and we are on login page.");
                }
                return true;
            }
        }
        ...
        if (!is_user_logged_in()) {
            if (
                ...
                !$this->popup_type->is_modal_customurl() &&
                !$this->popup_type->is_loginpage_modal() &&
                !$this->popup_type->is_modal_anypage_everyone()
            ) {
                if ($this->debug_log_to_file) {
                    $this->debug_txt_logger->write_log($this->debug_current_user . "Decision: false" . " -- Reason: Not Woo modal, not for everyoneon all page and user is not logged in.");
                }
                return false;
            }
        }
        ...
        // OR TEST MODE ON
        elseif (
            isset($this->generals_options['modal_to_show']) &&
            $this->generals_options['modal_to_show'] == 'terms_and_conditions_modal_test'
        ) {
            if ($this->debug) {
                error_log(print_r("terms_and_conditions_modal_test -- Reason: TEST mode is on.", true));
            }
            if ($this->debug_log_to_file) {
         ...
         * Custom URL Page
         */
        if ($this->popup_type->is_modal_customurl()) {
            if (TPUL_LoginPage_Utility::__isCustomURLPage()) {
                if (
                    is_user_logged_in() &&
                    $user_state_manager->did_accept_latest_terms()
                ) {
                    if ($this->debug) error_log("Decision: false -- is_custom_url_page but already accepted terms");
                    return false;
                }
                if ($this->debug) error_log("Decision: true -- is_custom_url_page");
                if ($this->debug_log_to_file) {
                    $this->debug_txt_logger->write_log($this->debug_current_user . "Decision: true" . " -- Reason: Login page modal and we are on login page.");
                }
                return true;
            } else {
                if ($this->debug) error_log("Decision: false -- is_custom_url_page but not on page");
                return false;
            }
        }

        /**
         * Modal every page for everyone
         */
        if ($this->popup_type->is_modal_anypage_everyone()) {
            /**
             * Show on every page for everyone
             */

            if ($this->debug) error_log("is_modal_anypage_everyone");

            // Logged in and already accepted latest terms
            if (
                is_user_logged_in() &&
                $user_state_manager->did_accept_latest_terms()
            ) {
                // User seen popup and status = 2 meaning they accepted latest
                if ($this->debug) error_log("Decision: false -- Reason: User has accepted latest terms and OPTION show on every page for everyone was selected by admin.");
                if ($this->debug_log_to_file) {
                    $this->debug_txt_logger->write_log($this->debug_current_user . "Decision: false" . " -- Reason: User has accepted latest terms and OPTION show on every page for everyone was selected by admin.");
                }
                return false;
            }

            if ($this->debug) error_log("Decision: true -- Reason: Show on every page for everyone was selected.");
            if ($this->debug_log_to_file) {
                $this->debug_txt_logger->write_log($this->debug_current_user . "Decision: true" . " -- Reason: Show on every page for everyone was selected.");
            }

            return true;
        } elseif (
            $this->popup_type->is_login_modal() && is_user_logged_in() ||
            $this->popup_type->should_show_on_wp_admin() && is_user_logged_in()
        ) {
            ...
            if ($this->terms_modal_show_only_once && $this->license_is_active) {
                 ...
                 */
                $user_seen_this_popup = TPUL_Terms_Userstate_Service::did_user_interact_with_terms($user_id);
                if ($user_seen_this_popup) {
                    if ($this->debug) error_log("Decision: false -- User has taken some action on this popup at some point and OPTION Do not throw popup if user already seen it was set by admin.");
                    if ($this->debug_log_to_file) {
            ...
            if ($this->debug) error_log("Decision true" . " -- User has not accepted latest terms No special options are on.");
            return true;
        }
        /********************************************************
        ...
        elseif ($this->popup_type->is_woo_modal() && $this->license_is_active) {
            ...
            if (
                ...
                $this->woo_options->get_display_on() == 'product_pages' &&
                $this->woo_handler->is_woo_product_page()
            ) {
                $are_we_on_right_woo_page = true;
                // is cart Page
            } elseif (
                $this->woo_options->get_display_on() == 'cart_page' &&
                $this->woo_handler->is_cart_page()
            ) {
                $are_we_on_right_woo_page = true;
            } elseif (
                $this->woo_options->get_display_on() == 'check_out_page' &&
                $this->woo_handler->is_checkout_page()
            ) {
                $are_we_on_right_woo_page = true;
            } elseif (
                $this->woo_options->get_display_on() == 'any_page' &&
                $this->woo_handler->is_any_page()
            ) {
                $are_we_on_right_woo_page = true;
            }

            /**
             * Check if we are on the right custom woo path page
             */
            if ($this->woo_options->is_current_path_custom_woo_path()) {
                $are_we_on_right_woo_page = true;
                if ($this->debug) error_log("We are on the right custom woo path page");
            }

            // if ($this->debug) error_log("WOO are_we_on_right_woo_page: " . $are_we_on_right_woo_page);
            // if ($this->debug) error_log("WOO PAge is set to: " . $this->woo_options->get_display_on());
            ...
            if (
                 ...
                 */
                if ($this->debug_log_to_file) {
                    if ($this->debug) error_log("Decision: false" . " -- Reason: WOO Option is on, User logged in but already accepted latest terms.");
            ...
            if ($this->debug) error_log("!!! not sure how we got here");
            if ($this->debug_log_to_file) {
