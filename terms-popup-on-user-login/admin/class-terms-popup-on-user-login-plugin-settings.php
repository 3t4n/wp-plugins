...
class Terms_Popup_On_User_Login_Admin_Settings {
    ...
    public function __construct($plugin_name, $version) {
        ...
        $this->license_key_handler = new TPUL_LicenseKeyHandler(new TPUL_LicsenseKeyDataProvider());
    ...
    public function initialize_terms_modal_options() {
        ...
        add_settings_field(
            'terms_modal_version',
            __('Terms Version', 'terms-popup-on-user-login'),
            array($this, 'terms_modal_version_render'),
            'tpul_settings_term_modal_options',
            'tpul_term_modal_section'
        );
        add_settings_field(
            'terms_modal_content',
            __('A) Content to show as terms in Popup', 'terms-popup-on-user-login'),
        ...
        add_settings_field(
            'terms_modal_pageid',
            __('B) Show your own page in terms in Popup', 'terms-popup-on-user-login'),
        ...
        add_settings_field(
            ...
            __('C) Load Content From URL', 'terms-popup-on-user-login'),
            array($this, 'terms_modal_content_from_url_render'),
            'tpul_settings_term_modal_options',
            'tpul_term_modal_section'
        );
        ...
        add_settings_field(
            'delimiter_login_url',
            __('', 'terms-popup-on-user-login'),
            array($this, 'delimiter_login_page_url_render'),
            'tpul_settings_term_modal_options',
            'tpul_term_modal_section'
        );
        ...
        add_settings_field(
            ...
            __('Custom Login Page Paths', 'terms-popup-on-user-login'),
            array($this, 'terms_modal_loginpage_paths_render'),
            'tpul_settings_term_modal_options',
            'tpul_term_modal_section'
        );
        add_settings_field(
            'terms_modal_customurls',
            __('Custom URLs', 'terms-popup-on-user-login'),
            array($this, 'terms_modal_customurls_render'),
            'tpul_settings_term_modal_options',
            'tpul_term_modal_section'
        );
    ...
    public function term_modal_options_callback() {
        ...
        echo '<div class="settings-legend">';
        echo '<p>' . __('Settings Legend', 'terms-popup-on-user-login') . '</p>';
        echo '<ol>';
        echo '<li><a href="#tpul_settings_1">' . __('Accept Button Settings', 'terms-popup-on-user-login') . '</a></li>';
        echo '<li><a href="#tpul_settings_2">' . __('Decline Button Settings', 'terms-popup-on-user-login') . '</a></li>';
        echo '<li><a href="#tpul_settings_3">' . __('Popup Content Settings', 'terms-popup-on-user-login') . '</a></li>';
        echo '<li><a href="#tpul_settings_4">' . __('Special Visibility Rules A), B) or C)', 'terms-popup-on-user-login') . '</a></li>';
        echo '<li><a href="#tpul_settings_5">' . __('Custom URLs, and login URLs', 'terms-popup-on-user-login') . '</a></li>';
        echo '<li><a href="#tpul_settings_6">' . __('Role Based Visibility Settings', 'terms-popup-on-user-login') . '</a></li>';
        echo '<li><a href="#tpul_settings_7">' . __('Trackings Settings', 'terms-popup-on-user-login') . '</a></li>';
        echo '<li><a href="#tpul_settings_8">' . __('Test and Debug Settings', 'terms-popup-on-user-login') . '</a></li>';
        echo '</ol>';
        echo '</div>';
    }
    ...
    public function validate_term_modal_options($input) {
        ...
        foreach ($input as $key => $value) {
            ...
            if (isset($input[$key])) {
                ...
                } elseif ($key == 'email_text_content') {
                    $output[$key] = wp_kses(stripslashes($input[$key]), $this->__get_body_allowed_html());
                } else {
                    // Strip all HTML and PHP tags and properly handle quoted strings

                    if ('terms_modal_content' == $key && $this->license_key_handler->is_active()) {
                        $output[$key] = wp_kses(stripslashes($input[$key]), $this->__get_body_allowed_html());
                        // error_log($key . " => " . $input[$key] . "  ------ " . $output[$key]);
                    } else {
                        $output[$key] = strip_tags(stripslashes($input[$key]));
                    }
                }
    ...
    public function initialize_terms_modal_woo_options() {
        ...
        add_settings_field(
            'terms_modal_woo_display_custom_page',
            __('Custom paths to display on', 'terms-popup-on-user-login'),
            array($this, 'terms_modal_woo_display_custom_page_render'),
            'tpul_settings_term_modal_woo_options',
            'tpul_term_modal_woo_section'
        );
    ...
    public function term_modal_woo_options_callback() {
        ...
        if ($woo_manager->woo_enabled()) {
            $woo_enabled = __('<b>Enabled</b>', 'terms-popup-on-user-login');
        } else {
            // WooCommerce plugin is NOT enabled!
            $woo_enabled = __('<b>Disabled</b>', 'terms-popup-on-user-login');
        }

        /**
         * Check if WooCommerce moded is on
         */
        $options = get_option('tpul_settings_general_options');
        if ($options['modal_to_show'] == 'terms_and_conditions_modal_woo') {
            $woo_mode = __('<b>WooCommerce Mode Enabled</b>', 'terms-popup-on-user-login');
        } else {
            $woo_mode = __('<b>NOT in WooCommerce Mode</b>', 'terms-popup-on-user-login');
        }

        echo '<p>' . __('For these options to take effect you must have ', 'terms-popup-on-user-login') . '</p>';
        echo '<ol>';
        echo '<li>' . __('WooCommerce plugin must be enabled: ', 'terms-popup-on-user-login') . $woo_enabled . '</p>';
        echo '<li>' . __('Show popup - WooCommerce Mode, must be selected under General tab: ', 'terms-popup-on-user-login') . $woo_mode . '</p>';
        echo '</ol>';
    }
    ...
    public function initialize_terms_modal_email_options() {
        ...
        add_settings_field(
            ...
            __('Send Email Notification COPY to Admins', 'terms-popup-on-user-login'),
        ...
        add_settings_field(
            'email_send_to_admins_separate',
            __('Separate Email Notification for Admins', 'terms-popup-on-user-login'),
            array($this, 'email_send_to_admins_separate_render'),
            $option_name,
            $section_id
        );
        ...
        add_settings_field(
            'email_allow_html',
            __('Allow HTML Email Content', 'terms-popup-on-user-login'),
            array($this, 'email_allow_html_render'),
            $option_name,
            $section_id
        );
...
?>
        ...
        <div class="wrap">
            ...
            <h2><?php esc_html_e('Terms Popup On User Login Options', 'terms-popup-on-user-login'); ?></h2>

            <?php settings_errors(); ?>

            <div class="tpul_admin_header">
                <div class="tpul_admin_banner">
                    <img class="tpul_banner_icon" src="<?php echo plugin_dir_url(__DIR__) . 'assets/shield.png'; ?>" alt="Shield Icon" />
                    <h2 class="tpul_banner_title">Legal Terms and Conditions Popup - TPUL</h2>
                </div>
                <?php if (!$this->license_key_handler->is_active()) { ?>
                    <div class="tpul_banner_premium_cta">
                        Upgrade to a <a href="https://lehelmatyus.com/terms-popup-on-user-login" target="_blank">Premium License</a> for exclusive features, exceptional support, and unbeatable performance!
                    </div>
                <?php } ?>
            </div>
                    ...
                    <div class="tg-outer">
                        ...
                        <table class="form-table" role="presentation">
                            <tbody>
                                ...
                                <tr>
                                    <th scope="row"><?php echo __('Your Settings Export'); ?></th>
                                    <td>
                                        <?php
                                        // JSON format export of all plugin settings
                                        $modal_option = new TPUL_Modal_Options();
                                        $general_options = new TPUL_General_Options();
                                        $display_option = new TPUL_Display_Options();
                                        $woo_option = new TPUL_Woo_Options();
                                        $email_option = new TPUL_Email_Options();
                                        $license_option = new TPUL_License_Options();

                                        $modal_options = $modal_option->export_all_settings();
                                        $general_options = $general_options->export_all_settings();
                                        $display_options = $display_option->export_all_settings();
                                        $woo_options = $woo_option->export_all_settings();
                                        $email_options = $email_option->export_all_settings();

                                        $all_options = json_encode(array(
                                            'php_version' => phpversion(),
                                            'wp_version' => get_bloginfo('version'),
                                            'curl_version' => function_exists('curl_version') ? curl_version()["version"] : 'cURL is not installed',
                                            'TPUL_General_Options' => $general_options,
                                            'TPUL_License_Options' => $license_option->get_options(),
                                            'TPUL_Modal_Options' => $modal_options,
                                            'TPUL_Display_Options' => $display_options,
                                            'TPUL_Woo_Options' => $woo_options,
                                            'TPUL_Email_Options' => $email_options
                                        ), JSON_PRETTY_PRINT);

                                        // preformat modal options
                                        echo __('Copy and paste the following JSON in your support request email to share your settings.', 'terms-popup-on-user-login');
                                        echo "<br><br>";
                                    ...
                                    <td>
                                        ...
                                        <?php if ($license_key_active) : ?>
                                            ...
                                            <?php else : ?>
                                                ...
                                                <p class="description">Consider updating your terms versioning before you hit reset. <a target="_blank" href="/wp-admin/options-general.php?page=terms_popup_on_user_login_options&tab=terms_modal_options#tpul_settings_3">Go to Edit version of your terms.</a></p>
    ...
    function dropdown_select_field_render() {
        ...
        $options = get_option('tpul_settings_general_options');
        // var_dump($options);
        if (!isset($options['modal_to_show'])) {
            $options['modal_to_show'] = 'none';
        }
    ?>
        <select name='tpul_settings_general_options[modal_to_show]'>
            ...
            <option value='terms_and_conditions_modal_anypageeveryone' <?php selected($options['modal_to_show'], 'terms_and_conditions_modal_anypageeveryone'); ?>><?php _e('Show popup - On every page for anyone', 'terms-popup-on-user-login'); ?></option>
            ...
            <?php if ($this->license_key_handler->is_active()) : ?>
                <option value='terms_and_conditions_modal_include_admin' <?php selected($options['modal_to_show'], 'terms_and_conditions_modal_include_admin'); ?>><?php _e('Show popup - On user login - including WP Dashboard Admin Interface', 'terms-popup-on-user-login'); ?></option>
            <?php else : ?>
                <option value='terms_and_conditions_modal_useon_admin' disabled <?php selected($options['modal_to_show'], 'terms_and_conditions_modal_useon_admin'); ?>><?php _e('Show popup - On user login - including WP Dashboard Admin Interface (needs License Key)', 'terms-popup-on-user-login'); ?></option>
            <?php endif; ?>

            <option value='terms_and_conditions_modal_onloginpage' <?php selected($options['modal_to_show'], 'terms_and_conditions_modal_onloginpage'); ?>><?php _e('Show popup - The login page only - prior to logging in', 'terms-popup-on-user-login'); ?></option>
            <option value='terms_and_conditions_modal_customurl' <?php selected($options['modal_to_show'], 'terms_and_conditions_modal_customurl'); ?>><?php _e('Show popup - Custom URL - Only when user is on a custom URL', 'terms-popup-on-user-login'); ?></option>
        ...
        <ol style="font-size:16px">
            <li style="padding-bottom: 8px;">
                <?php echo __('<b>Don\'t show popup</b> <br/> Popup will not show under any condition.', 'terms-popup-on-user-login'); ?>
            </li>
            <li style="padding-bottom: 8px;">
                <?php echo __('<b>Show popup - On every page for anyone</b> <br/> Popup will show for any visitor, logged in user and anonymous visitors as well until accepted. Can be used as a cookie policy popup.', 'terms-popup-on-user-login'); ?>
            </li>
            <li style="padding-bottom: 8px;">
                <?php echo __('<b>Show popup - On user login</b> <br/> Catches everyone who logs in. Popup will show immediately on user login. You cam force logout if they did not accept.', 'terms-popup-on-user-login'); ?>
            </li>
            <li style="padding-bottom: 8px;">
                <?php echo __('<b>Show popup - On user login - including WP Dashboard Admin Interface</b> <br/> Same as On User Login but including any page of the WordPress dashboard under /wp-admin <br> <span style="color: rgba(32, 153, 43, 1);">Active license key required for this feature.</span> ', 'terms-popup-on-user-login'); ?>
            </li>
            <li style="padding-bottom: 8px;">
                <?php echo __('<b>Show popup - The login page only - prior to logging in</b> <br/> Popup will show on your login page - login page URL\'s must be specified in "Labels, Content and Redirect Tab"', 'terms-popup-on-user-login'); ?>
            </li>
            <li style="padding-bottom: 8px;">
                <?php echo __('<b>Show popup - Custom URL</b> <br/> Popup will only show when user is on a custom page path, URL\'s must be specified in "Labels, Content and Redirect Tab"', 'terms-popup-on-user-login'); ?>
            </li>
            <li style="padding-bottom: 8px;">
                <?php echo __('<b>Show popup - WooCommerce mode</b> <br/> Popup will only show on WooCommerce Pages. Can be shown for Anonymous visitors as well. Extra configuration needed on "WooCommerce Integration" tab.', 'terms-popup-on-user-login'); ?>
            </li>
            <li style="padding-bottom: 8px;">
                <?php echo __('<b>TEST MODE</b> <br/> Popup will show for every user on every page load for easier testing. Remember to disable test mode after done testing!', 'terms-popup-on-user-login'); ?>
            </li>
        </ol>
        </p>
    ...
    ?>
        ...
        <?php if ($license_key_active) : ?>
            ...
            <?php else : ?>
                ...
                <div>
                    <?php if ($license_key_active) : ?>
                        <input id="tpul__deactivate-key-input" type='password' class="regular-text" name='tpul_settings_general_options[tplu_license_key]' value='<?php echo $options['tplu_license_key']; ?>' <?php echo $readonly; ?>>
        ...
        function delimiter_visibility_render() {
            echo "<h3 id='tpul_settings_4'>4. Special Visibility Rules A), B) or C)  -- <small>Ignore for WooCommerce operation</small></h3>";
        ...
        function delimiter_login_page_url_render() {
            echo "<h3 id='tpul_settings_5'>5. Custom URLs and Login URLs</h3>";
            echo  __("<p>If you selected Terms Popup Behaviour \"Show popup - On the login page only prior to logging in\" you may add all your custom login page URLs here if you have any.</p>", 'terms-popup-on-user-login');
            echo  __("<p>For any other Terms Popup Behavious ignore this section</p>", 'terms-popup-on-user-login');
        }

        function delimiter_accept_render() {
            echo "<h3 id='tpul_settings_1'>1. Accept Button Settings</h3>";
        }

        function delimiter_decline_render() {
            echo "<h3 id='tpul_settings_2'>2. Decline Button Settings</h3>";
        }

        function delimiter_content_render() {
            echo "<h3 id='tpul_settings_3'>3. Popup Content Settings</h3>";
            echo "<p>Select where you would like to pull the popup content from? </p>";
            echo "<ul>";
            echo "<li>A) Text from Textbox Below</li>";
            echo "<li>B) Page contetn From selected page</li>";
            echo "<li>C) content from URL, should contain only HTML markup, no scripts</li>";
            echo "</ul>";
            echo "<p>defaults to option A). if set option B) will override option A).  If set option C) wil override option A) and B)</p>";
        }

        function delimiter_role_visibility_render() {
            echo "<h3 id='tpul_settings_6'>6. Role Based Visibility Settings</h3>";
        }

        function delimiter_tracking_render() {
            echo "<h3 id='tpul_settings_7'>7. Trackings Settings</h3>";
        }

        function delimiter_testing_render() {
            echo "<h3 id='tpul_settings_8'>8. Test and Debug Settings</h3>";
        }
        ...
        function terms_modal_accept_redirect_render() {
            ...
            ?>
                ...
                <p class="description"> <b><?php echo esc_html__('When using it as LOGIN Page Modal OR with WooCommerce integration it is best to leave this field empty, or users will get redirected from the login page or Woo product!', 'terms-popup-on-user-login'); ?> </b></p>
            <?php
        }
        ...
        function terms_modal_designated_test_user_render() {
            ...
            ?>
                ...
                function terms_modal_version_render() {
                    $options = $this->tpul_settings_term_modal_options;
                    ...
                    ?>
                        <p class="description"> <?php echo __('Enter the version number of the current terms and conditions (e.g., v1.2 or 2024-12).', 'terms-popup-on-user-login'); ?> </p>
                        <p class="description"> <b><?php echo __('Warning! modifying this field will prompt everyone to re-accept your terms and conditions.', 'terms-popup-on-user-login'); ?> </b></p>
                    <?php
                }
                function terms_modal_content_render() {
                    ...
                    ?>
                        <p class="description"> <?php echo __('Paste in content you want to show in the popup. <br><b>HTML is only allowed if you have an active license key</b> <br> Alternatively you can use <b>"Show your own page"</b> option below to bring in an existing Terms Page as content.', 'terms-popup-on-user-login'); ?> </p>
                    <?php
                }
                    ...
                    ?>
                        ...
                        function dropdown_select_field_pages_render() {
                            ...
                            ?>
                                ...
                                function terms_modal_content_from_url_render() {
                                    $options = $this->tpul_settings_term_modal_options;
                                    $license_key_valid = $this->license_key_handler->is_active();
                                    AdminForm::text_input__active_key_required(
                                        ...
                                        'tpul_settings_term_modal_options',
                                        'terms_modal_content_from_url',
                                        __('Show your own page', 'terms-popup-on-user-login')
                                    );
                                    ?>
                                        <p class="description"> <?php echo __('<b>WARNING:</b> Do not load content from websites that you do not own.', 'terms-popup-on-user-login'); ?> </p>
                                    <?php
                                }
                                function terms_modal_loginpage_paths_render() {
                                    $options = $this->tpul_settings_term_modal_options;
                                    $license_key_valid = $this->license_key_handler->is_active();
                                    ...
                                    ?>
                                        <p class="description"> <?php echo __('Add paths to the pages you consider as login pages to your website. No need to add /wp-login.php', 'terms-popup-on-user-login'); ?> </p>
                                        <p class="description"> <?php echo __('Add any custom login page URLs to the pages on which you would like the popup to show', 'terms-popup-on-user-login'); ?> </p>
                                        <p class="description"> <?php echo __('Add each path on a seperate line starting with a slash character, example: ', 'terms-popup-on-user-login'); ?> </p>
                                        <p class="description"> <?php echo __('/my-custom-login-page ', 'terms-popup-on-user-login'); ?> </p>
                                        <p class="description"> <?php echo __('/dashboard/login ', 'terms-popup-on-user-login'); ?> </p>
                                    <?php
                                }
                                function terms_modal_customurls_render() {
                                    $options = $this->tpul_settings_term_modal_options;
                                    $license_key_valid = $this->license_key_handler->is_active();
                                    AdminForm::textarea__active_key_required(
                                        $license_key_valid,
                                    ...
                                    ?>
                                        <p class="description"> <?php echo __('Add any custom URLs where you would like the popup to show', 'terms-popup-on-user-login'); ?> </p>
                                        <p class="description"> <?php echo __('Add each path on a seperate line starting with a slash character, example: ', 'terms-popup-on-user-login'); ?> </p>
                                        <p class="description"> <?php echo __('/my-custom-url ', 'terms-popup-on-user-login'); ?> </p>
                                        <p class="description"> <?php echo __('/wp-admin/another-custom-url ', 'terms-popup-on-user-login'); ?> </p>
                                    <?php
                                }
                                    ...
                                    ?>
                                        ...
                                        <?php if ($license_key_valid) : ?>
                                            ...
                                            <?php else : ?>
                                                ...
                                                <p class="description"> <?php echo __('Unselected user roles will be exempt from seeing the terms popup unless "All logged in users" is selected.', 'terms-popup-on-user-login'); ?> </p>
                                                </div>
                                        ...
                                        public function terms_modal_woo_display_on_render() {
                                            ...
                                            $select_options_array = [
                                                ...
                                                'any_page' => [
                                                    'value' => 'any_page',
                                                    'label' => __('Any page of the website', 'terms-popup-on-user-login'),
                                                    'with_license_key_only' => true
                                                ],
                                            ];
                                            // Render Select box
                                        ...
                                        function terms_modal_woo_display_custom_page_render() {
                                            ...
                                            ?>
                                                <p class="description"> <?php echo __('Add paths to the pages are not standard WooComemrce pages but you would still like to show the popup on.', 'terms-popup-on-user-login'); ?> </p>
                                                <p class="description"> <?php echo __('Add each path on a seperate line starting with a slash character, example: ', 'terms-popup-on-user-login'); ?> </p>
                                                <p class="description"> <?php echo __('/my-custom-woo-page ', 'terms-popup-on-user-login'); ?> </p>
                                                <p class="description"> <?php echo __('/dashboard/custom-page ', 'terms-popup-on-user-login'); ?> </p>
                                            <?php
                                        }
                                        ...
                                        function email_send_to_admins_render() {
                                            ...
                                            AdminForm::checkbox_single__active_key_required(
                                                ...
                                                $this->email_option_getter->get_option_name(),
                                                'email_send_to_admins',
                                                __('Send a copy of the email sent to client to the admins as well', 'terms-popup-on-user-login')
                                            );
                                            ?>
                                                <p class="description"> <?php echo __('Best to have this turned on. Send a copy of the email sent to the user to the website admin as well.', 'terms-popup-on-user-login'); ?> </p>
                                                <p class="description"> <?php echo __('<b>Send Email Notification to Users</b> - must bee turned on, then this checkbox cehcked for a COPY to be sent', 'terms-popup-on-user-login'); ?> </p>
                                            <?php
                                        }
                                        function email_send_to_admins_separate_render() {
                                            $license_key_valid = $this->license_key_handler->is_active();
                                            ...
                                            AdminForm::checkbox_single__active_key_required(
                                                ...
                                                $this->email_option_getter->get_option_name(),
                                                'email_send_to_admins_separate',
                                                __('Send an Admin Secific Email Notification to admin Emails', 'terms-popup-on-user-login')
                                            );
                                            ?>
                                                <p class="description"> <?php echo __('Send an Admin specific email with all user details in the email.', 'terms-popup-on-user-login'); ?> </p>
                                            <?php
                                        }
                                        ...
                                        function email_allow_html_render() {
                                            AdminForm::checkbox_single(
                                                $this->email_options,
                                                $this->email_option_getter->get_option_name(),
                                                'email_allow_html',
                                                __('Allow HTML in email', 'terms-popup-on-user-login')
                                            );

                                            ?>
                                                <p class="description"> <?php echo __('By default, HTML is filtered out from emails. Select this checkbox if you would like to send formatted HTML emails. Scripts and other insecure tags will still be filtered out.', 'terms-popup-on-user-login'); ?></p>
                                            <?php
                                        }
                                        function email_text_content_render() {
                                            ...
                                            $content = $this->email_option_getter->get_option('email_text_content');
                                            $editor_id = 'email_text_content';
                                            $settings = [
                                                'textarea_name' => $this->email_option_getter->get_option_name() . '[email_text_content]',
                                                'textarea_rows' => 20,
                                                'media_buttons' => true,
                                                'quicktags' => false,
                                                'teeny' => true,
                                                'tinymce' => [
                                                    'toolbar1' => 'formatselect,bold,italic,underline,bullist,numlist,link,unlink,forecolor,backcolor,alignleft,aligncenter,alignright,alignjustify',
                                                    'toolbar2' => 'outdent,indent,blockquote,removeformat,undo,redo',
                                                    'toolbar3' => '',
                                                    'toolbar4' => '',
                                                ],
                                                'editor_css' => '<style>#' . $editor_id . '_ifr { width: 100%; }</style>',
                                            ];
                                            ...
                                            ?>
                                                ...
                                                <p class="description"> <?php echo "<b>[user-email]</b> - " . __('Email address of user', 'terms-popup-on-user-login'); ?></p>
                                                <p class="description"> <?php echo "<b>[user-first-name]</b> - " . __('First name of user', 'terms-popup-on-user-login'); ?></p>
                                                <p class="description"> <?php echo "<b>[user-last-name]</b> - " . __('Last name of user', 'terms-popup-on-user-login'); ?></p>
                                                <p class="description"> <?php echo "<b>[user-ip]</b> - " . __('Best Guess for User IP ("Track User IP" must be turned on under "Labels, Content and Redirects" Tab )', 'terms-popup-on-user-login'); ?></p>
                                                ...
                                                <p class="description"> <?php echo "<b>[accepted-date]</b> - " . __('Acceptance Date (Date format will be used from WordPress Settings > General )', 'terms-popup-on-user-login'); ?></p>
