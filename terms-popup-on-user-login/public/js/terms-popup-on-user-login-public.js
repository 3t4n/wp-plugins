(function ($) {
	 ...
	 * Handle Logout
	 */
	function declineTermsUser(clicktype = "Button Click") {
		let tpul_visitor_id = window.TPUL.__getVisitorId();
		$.ajax({
			url:
				tpulApiSettings.root +
				"terms-popup-on-user-login/v1/action/decline-terms",
			type: "POST",
			...
			.done(function (results) {
				// remove the visitor id synced flag
				window.TPUL.__removeVisitorIdUpdatedInDB();
				window.TPUL.__removeAcceptSessionCookie();
				window.TPUL.__removeAcceptUrl();
				window.TPUL.__removeLastAcceptedDateCookie();
	...
	function acceptTermsUser(clicktype = "Button Click") {
		 ...
		 */
		// If logged in send the request to the backend
		if (window.TPUL.__isUserLoggedIn()) {
			window.TPUL.__setAcceptUrl();
			...
			$.ajax({
				...
				.done(function (results) {
					if (results) {
						...
						let data_is_logged_in = window.TPUL.__isUserLoggedIn();
						if (results.data.redirect && data_is_logged_in) {
			...
			// If not logged in, save proof in cookie
			// cookie will be read out as soon as the user logs in
			// and close the modal
			MicroModal.close();
			__setAcceptSessionCookkieAndClose();
		}
	}
	...
	function loginPageAcceptAction() {
		...
		// save info in a cookie, process it when user logs in
		__setAcceptSessionCookkieAndClose();
	}

	function __setAcceptSessionCookkieAndClose() {
		/**
		 * Register Accept in a cookie
		 */
		window.TPUL.__setLastAcceptedDateCookie();
		window.TPUL.__setAcceptUrl();
		...
		/**
		 * Save Accept in a cookie
		 */
		if (window.TPUL.__isPopupSavesCookie()) {
			window.TPUL.__setAcceptSessionCookkie();
		}
		/**
		 * Set visitor ID
		 */
		let tpul_visitor_id = window.TPUL.__getVisitorId(); // saves it to cookies as well
	...
	$(function () {
		...
		if (
			...
			"terms_and_conditions_modal_customurl" == popup_type ||
			"terms_and_conditions_modal_woo" == popup_type ||
			window.TPUL.__isTestMode() ||
			(window.TPUL.__isLoginPage() &&
				window.TPUL.__resetHappenedSinceLastAccept())
		) {
			window.TPUL.__determinGeoLocation();

			let show_modal = true;

			// should check cookie for this session
			if (window.TPUL.__shouldPopupCheckCookie()) {
				...
				if (
					window.TPUL.__getAcceptSessionCookie() == true &&
					window.TPUL.__noResetSinceLastAccept()
				) {
					show_modal = false;
				}
			}
		...
		$(".modal_accept_anypageeveryone").click(function () {
			if (!$(this).hasClass("disabled")) {
				if (window.TPUL.__isTestMode()) {
					...
					let data_is_logged_in = window.TPUL.__isUserLoggedIn();
					let data_save_cookie = window.TPUL.__isPopupSavesCookie();
