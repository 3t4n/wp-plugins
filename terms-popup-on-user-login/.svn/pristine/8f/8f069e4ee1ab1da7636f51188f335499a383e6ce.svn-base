...
class Terms_Popup_On_User_Login_Admin {
	...
	public function __construct($plugin_name, $version) {
		...
		$this->apiSettingsService = new Terms_Popup_ApiSettings_Service();
	}
	...
	public function enqueue_scripts($hook) {
		...
		*/
		if ($hook == 'profile.php' || $hook == 'user-edit.php') {
			 ...
			 */
			$apiSettingsServiceArray = $this->apiSettingsService->get_tpulApiSettings();
			$apiSettingsServiceArray['tpul_nonce'] = wp_create_nonce('wp_rest');

			wp_localize_script($this->plugin_name . "-profile", 'tpulApiSettings', $apiSettingsServiceArray);
		}
		...
		if ($hook == 'settings_page_terms_popup_on_user_login_options') {
			...
			$apiSettingsServiceArray = $this->apiSettingsService->get_tpulApiSettings();
			$apiSettingsServiceArray['tpul_nonce'] = wp_create_nonce('wp_rest');
			wp_localize_script($this->plugin_name, 'tpulApiSettings', $apiSettingsServiceArray);
		}
		...
		if (is_user_logged_in()) {
			...
			$apiSettingsServiceArray = $this->apiSettingsService->get_tpulApiSettings();
			$apiSettingsServiceArray['tpul_nonce'] = wp_create_nonce('wp_rest');
			wp_localize_script($this->plugin_name . "-token", 'tpulApiSettings', $apiSettingsServiceArray);
		}
		...
		if (is_user_logged_in() && $should_show_on_wp_admin) {
			...
			if ($should_modal_show) {
				...
				$apiSettingsServiceArray = $this->apiSettingsService->get_tpulApiSettings();
				$apiSettingsServiceArray['tpul_nonce'] = wp_create_nonce('wp_rest');
				wp_localize_script($this->plugin_name . "-public", 'tpulApiSettings', $apiSettingsServiceArray);
			}
		}
	}
}
