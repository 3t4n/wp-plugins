...
class TPUL_Terms_Content_Service {
    public static function get_the_default_terms_content($do_shortcode = false) {
        ...
        if (!empty($content_page_id) && $license_is_active) {
            ...
            return wpautop(wp_kses_post($content, self::__get_body_allowed_html()));
        }
    ...
    public static function get_default_terms_content_hash() {
        $content = self::get_the_default_terms_content();
        return TPUL_Text_Hash_Utility::get_hash_from_text($content);
    }

    public static function get_default_terms_content_version() {
        $modal_options = new TPUL_Modal_Options();
        $terms_options_data = $modal_options->get_options();
        return $terms_options_data['terms_modal_version'];
    }
