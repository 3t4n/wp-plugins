...
class Terms_Popup_On_User_Login_Rest_API extends WP_REST_Posts_Controller {
    ...
    public function register_routes() {
        ...
        /* 
        Register visitor id to user
         */
        register_rest_route($this->namespace, '/' . $this->rest_base . '/save-anon-acceptance-token', array(
            array(
                'methods'             => WP_REST_Server::CREATABLE,
                'callback'            => array($this, 'addAcceptanceTokenToUser'),
                'permission_callback' => array($this, 'set_user_accepted_permission_check'),
                'show_in_index'       => false,
            ),
        ));
        ...
        /* Accept Terms
         * wp-json/terms-popup-on-user-login/v1/action/decline-terms
         */
        register_rest_route($this->namespace, '/' . $this->rest_base . '/decline-terms', array(
            array(
                'methods'             => WP_REST_Server::CREATABLE,
                'callback'            => array($this, 'set_user_declined'),
                'permission_callback' => array($this, 'set_user_declined_permission_check'),
                'show_in_index'       => false,
            ),
        ));

        /* Accept Terms
         * wp-json/terms-popup-on-user-login/v1/action/reset-users
         */
    ...
    public function addAcceptanceTokenToUser($request) {
        ...
        $user_state_manager->useraction_accepted($user_id, $request_body);

        /**
         * Send Email notification that they accepted it
         */
         ...
         * Send Email
         */
        if (!empty($options['email_send_to_user']) && $this->license_is_active) {
            $email_sender->notify_accept_user($user_id);
            /**
             * Send Copy email to Admins
             */
            if (!empty($options['email_send_to_admins']) && $this->license_is_active) {
                $email_sender->notify_admin($user_id);
            }
        }
        /**
         * Send Email to Admins
         */
        if (!empty($options['email_send_to_admins_separate']) && $this->license_is_active) {
            $email_sender->notify_admin_separate($user_id);
        }
    ...
    public function set_user_accepted($request) {
        ...
        if ($user_id != $this->terms_options['terms_modal_designated_test_user']) {
            ...
            $user_state_manager->useraction_accepted($user->ID, $request_body);
    ...
    private function __handle_anonymous_accepting_terms($request) {
        ...
        $acceptance_ip_and_location = $this->__get_ip_and_loc_from_request($request);
        $ip_for_email = implode(", ", $acceptance_ip_and_location['ip']);
        ...
        if ($this->license_is_active) {
            ...
            if (!empty($options['email_notify_about_anonymous'])) {
                $email_sender->notify_admins_of_anon_acceptance($tpul_visitor_id, $site_name, $currentURL, $ip_for_email);
            }
        }
     ...
     * Logout user
     */
    public function set_user_declined_permission_check($request) {
        return true;
    }

    public function set_user_declined($request) {
        ...
        /**
         * Set user to declined
         */
        $request_body = $request->get_json_params();
        $user_state_manager = new TPUL_User_State($user_id);
        $user_state_manager->useraction_declined($user_id, $request_body);
    ...
    public function reset_single_user($request) {
         ...
         * Reset User
         */
        TPUL_Terms_Userstate_Service::reset_user_record($the_user_id);
    ...
    public function activatekey($request) {
        ...
        $key_handler = new TPUL_LicenseKeyHandler(new TPUL_LicsenseKeyDataProvider());;
        ...
        if (!empty($_com_response->data->status) && $_com_response->data->status == 404) {
            $error->add("key_not_good", __($key_handler->get_message('key_not_good') . ' ' . implode(" ", $_com_response->get_error_messages())), array('status' => 404));
            $key_handler->flush_key_related_info();
            return $error;
        }
        ...
        if (
            ...
            $error->add("key_expired", __($key_handler->get_message('key_expired') . ' expired at: ' . $_com_response->data->expiresAt . ' ' . implode(" ", $_com_response->get_error_messages())), array('status' => 404));
            $key_handler->flush_key_related_info();
            return $error;
        }
        ...
        $error->add("unable_to_activate", __($key_handler->get_message('unable_to_activate') . " " . implode(" ", $_com_response->get_error_messages())), array('status' => 404));
        $key_handler->flush_key_related_info();
        return $error;
    }
    ...
    public function deactivatekey($request) {
         ...
         */
        $error->add("unable_to_deactivate", __($key_handler->get_message('unable_to_deactivate') . " " . implode(" ", $_com_response->get_error_messages())), array('status' => 404));
        $key_handler->flush_key_related_info();
        return $error;
    }
     ...
     * from the request
     */
    public function __get_ip_and_loc_from_request($request = null) {
        ...
        if (!empty($request)) {
            ...
            if ($modal_options->get_track_location()) {
                $accepted_options['locationCoordinates'] = json_decode($request_body['locationCoordinates'], true);
                $locationCoordinates = $accepted_options['locationCoordinates'];
            }
        }
