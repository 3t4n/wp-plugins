...
function sanitize_loadurl($urlparam)
    {
        // Allow only letters, numbers, periods, equals, colons, hyphens, question marks, hyphens, forward slashes, percent signs, spaces, and ampersands
        return preg_replace('/[^a-zA-Z0-9\.\=\:\?\/%\s&\-_]/', '', $urlparam);
    }
...
function pcr_frame($atts)
{
    ...
    $sid = intval($a['form']);
    $loadurl = $a['link'];
    $loadurl = htmlspecialchars_decode($loadurl, ENT_QUOTES);
    ...
    // Modify the URL when needed
    if (strpos($loadurl,'jobboard.') > -1 || strpos($loadurl,'temphrs.') > -1 || strpos($loadurl,'employer.') > -1 || strpos($loadurl,'/reg') > -1 || strpos($loadurl,'reg') > -1) {
		
	} else {
		if ($sid < 1 || strpos($loadurl, '.asp?') === false || strpos($loadurl, '.exe?') === false || strpos($loadurl, '.aspx?') === false) {
		 $loadurl = 'jobboard.aspx?uid=' . $loadurl;
		 $pcrframecss = '';
		}
	}

    // If a custom form is specified
    if ($sid && $loadurl !== "about:blank") {
        // Create the <script> tag with DOMDocument
        $doc = new DOMDocument('1.0', 'UTF-8');
        $script = $doc->createElement('script');
        $loadurl = urldecode($loadurl);
        $script->setAttribute('src', "https://host.pcrecruiter.net/pcrbin/{$loadurl}&action=opencustomform&sid={$sid}");
        $doc->appendChild($script);
        
        return "<!-- Start PCRecruiter Form -->"
            . $doc->saveHTML()
            . "<!-- End PCRecruiter Form -->";
    }

    // Prepend URL based on the link
    if (substr($loadurl, 0, 4) !== "http") {
        $prefix = (substr($loadurl, 0, 8) === "jobboard")
            ? 'https://host.pcrecruiter.net/pcrbin/'
            : 'https://www2.pcrecruiter.net/pcrbin/';
        $loadurl = $prefix . $loadurl;
    }

	$loadurl = str_replace('http:','https:',$loadurl);
	$loadurl = str_replace('.exe?','.aspx?',$loadurl);

    // Create iframe with DOMDocument
    $doc = new DOMDocument('1.0', 'UTF-8');
    ...
    $iframe->setAttribute('style', "height:{$a['initialheight']}px;width:100%;background-color:{$a['background']};border:0;margin:0;padding:0");
    $iframe->setAttribute('onload', 'pcrframeurl();');

    $doc->appendChild($iframe);

    return "<!-- Start PCRecruiter WP 1.4.33-->"
        . $pcrframecss
        . $doc->saveHTML()
        . "<!-- End PCRecruiter WP -->";
}
