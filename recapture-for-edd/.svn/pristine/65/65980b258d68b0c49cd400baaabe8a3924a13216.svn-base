...
if (!class_exists('Recapture')) {
    ...
    define('RECAPTURE_VERSION', '1.0.42');
    ...
    class Recapture
    {
        ...
        function allowed_redirect_hosts($hosts) {
            ...
            if ($custom) {
                $domain = parse_url($custom, PHP_URL_HOST);
                $full_hosts[] = $domain;
            }
        ...
        /**
         * Verify the nonce for the given name and action
         *
         * @param string $name
         * @param string $action
         */
        function verify_nonce($name, $action) {
            $nonce = isset($_POST[$name])
                ? sanitize_text_field(wp_unslash($_POST[$name]))
                : null;

            if ($nonce == null || !wp_verify_nonce($nonce, $action)) {
        ...
        public function ajax_submit_reviews() {
            // Security check
            $this->verify_nonce('recapture_submit_reviews', 'recapture_submit_reviews');
                ...
                <form method="post" action="<?php echo esc_html(admin_url('admin-post.php')); ?>" target="_blank">
                    ...
                    <?php } else { ?>
                        ...
                        <p>
                            ...
                            <?php wp_nonce_field('recapture_connect', 'recapture_connect'); ?>
        ...
        {
            $this->verify_nonce('recapture_connect', 'recapture_connect');
        ...
        {
            $this->verify_nonce('recapture_disconnect', 'recapture_disconnect');
        ...
        function confirm_disconnect_recapture()
        {
            ...
            <div class="wrap">
                ...
                <form method="post" action="<?php echo esc_html(admin_url('admin-post.php')); ?>">
                    <?php wp_nonce_field('recapture_disconnect', 'recapture_disconnect'); ?>
                    <p>
