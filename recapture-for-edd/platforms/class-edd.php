...
class RecaptureEDD extends RecaptureBasePlatform {
    ...
    public function add_actions() {
        ...
        // Used to handle conversions in the old PayPal Express gateway that does not support
        // the edd_complete_purchase action
        add_filter('edd_should_update_order_status', [$this, 'handle_paypal_express_conversions'], 99, 4);
    }
    ...
    public function remove_actions() {
        ...
        remove_filter('edd_should_update_order_status', [$this, 'handle_paypal_express_conversions']);
    }
    ...
    public function insert_payment($payment_id, $payment_data) {
        ...
        if ($cart_id == false) {
            ...
            // We are no longer setting a new ID because EDD creates a 'pending' order when the PayPal checkout
            // flow starts and if we create a new cart id and the customer visits another page or makes any
            // changes to their cart then we will see a duplicate cart in Recapture.
            //
            // Although not ideal, it isn't a problem to reuse the same cart id in these situations because we
            // set the cart's token to null in Recapture once a it has been converted.
            //
            // For more information see issue #2031
            //
            // RecaptureUtils::set_new_cart_id();
        }
    }

    function get_discount_code_for_order($order_id) {
        ...
        // Extract the discount codes
        $discount_codes = array();

        if (empty($discounts)) {
            return $discount_codes; // No discounts applied
        }
    ...
    public function order_completed($payment_id)
    {
        ...
        if (function_exists('edd_get_order_items')) {
            ...
            $products = array_map(function($item) {
                $image_url = wp_get_attachment_url(get_post_thumbnail_id($item->product_id));
                ...
                $variant_id = $item->price_id ? $item->price_id : null;
                $cart_item->variantId = $variant_id;
            ...
            $data->cart = (object) [
                ...
                'externalId' => $external_id != false ? $external_id : null,
                'discountCodes' => $this->get_discount_code_for_order($payment_id),
            ];
        }
    ...
    public function handle_paypal_express_conversions($should_update, $order_id, $new_status, $old_status) {
        try {
            // only apply to PayPalExpress gateways
            $payment = new EDD_Payment($order_id);

            // convert the payment
            if (
                edd_get_payment_gateway($order_id) == 'paypalexpress' &&
                $new_status == 'publish' &&
                $old_status != 'publish') {
