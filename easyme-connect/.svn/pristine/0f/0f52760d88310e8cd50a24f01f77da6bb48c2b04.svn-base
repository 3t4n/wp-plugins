...
class WP
{
    ...
    private static function getEasyMeClient()
    {
        ...
        try {
            ...
            if (array_key_exists('wp_settings', $res) && is_array($res['wp_settings'])) {
                foreach ($res['wp_settings'] as $k => $v) {
    ...
    public static function run()
    {
        ...
        if (is_admin()) {
            ...
            register_activation_hook(self::getPluginMainFile(), [__CLASS__, 'easymeActivate']);
            register_deactivation_hook(self::getPluginMainFile(), [__CLASS__, 'easymeDeactivate']);
            register_uninstall_hook(self::getPluginMainFile(), [__CLASS__, 'easymeUninstall']);

            add_action('admin_menu', [__CLASS__, 'addMenu']);

            add_action('admin_init', [__CLASS__, 'checkForOAuthCode']);

            if (!self::isConnected() && self::needsPostActivation()) {
                add_action('admin_init', [__CLASS__, 'easymePostActivation']);
            }
            ...
            add_action('wp_dashboard_setup', [__CLASS__, 'addwidget']);
            add_action('admin_enqueue_scripts', [__CLASS__, 'initUI']);

        } else {

            add_action('wp_enqueue_scripts', [__CLASS__, 'loadWidgetJS']);

            add_action('init', function () {

                /**
                 * Only works if we actually have an htaccess file, but power to those folks 
                 */
                add_action('parse_request', function ($wp) {
                    if ('.well-known/apple-developer-merchantid-domain-association' == $wp->request) {
                        $string = self::getSetting('applepay_domain_association');
                        if (!empty($string)) {
