...
class OAuthClient {
    ...
    private function getHeaders() {
        ...
        if(FALSE !== ($pData = WP::getPluginData())) {
            ...
            // get_plugin_data not available at run time
            $emFile = __DIR__ . '/../easyme.php';           
            if(is_readable($emFile)) {
                $versionLines = array_values(array_filter(file($emFile), function($l) {
                    return 0 === strpos($l, 'Version');
                }));
                if(1 == sizeof($versionLines)) {
                    $parts = explode(':', $versionLines[0]);
                    $hdrs['X-EasyMe-Wordpress-Plugin-Version'] = trim($parts[1]);
                }
            }             
        }
