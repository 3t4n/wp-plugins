...
function vod_video_upload(){
    if (!empty($_FILES['file']['name'])) {
        ...
        wp_send_json(['status'=>0,'msg'=>$fileid->get_error_message()]);

        $attch_url = wp_get_attachment_url($fileid);//'https://sample-videos.com/video321/mp4/720/big_buck_bunny_720p_1mb.mp4';
        if(empty($attch_url))
        ...
        $video = [
            ...
            'description' =>'',
            'video_url'   =>$attch_url,//'https://download.samplelib.com/mp4/sample-10s.mp4',
            'convert_mode'=>$_POST['convert'],
            'resType'     =>'',
            'file_id'     =>'',
            'profile_id'  =>'',
            'parallel_convert'=>false,
        ...
        if($result['status_code']==201){
            ...
            wp_send_json(['success' => true, 'name'=> $_FILES['file']['name'], 'edit' => admin_url("admin.php?page=arvancloud-vod-single-video&id={$result['id']}"),'url' => wp_get_attachment_url( $fileid )]);
            //"post.php?post=$fileid&action=edit"
        }else
        wp_send_json(['success' => false, 'msg' => __('Arvan error code: ','arvancloud-vod').$result['status_code']]);
    }
}
