...
/*
	...
	Description: 	Simple A to B taxi price calculator. Use shortcode [cabGrid] to display on your pages/posts. (<a href="https://cabgrid.com">Upgrade to Cab Grid Pro</a>)
	Version:     	1.6.25
	Author:     	M Williams
...
*/
	...
	define("_cabGridVersion","cgb-1.6.25");
	...
	function cabGrid_scripts(){
			...
			wp_register_script( 'cabGridJS', plugins_url( 'cab-grid.js', __FILE__ ), array( 'jquery') ,_cabGridVersion, true );
			
			$translation_array = array(
				'please_wait' => esc_attr(__( 'Please Wait', 'cab-grid' )),
				'get_price' => esc_attr(__( 'Get Price', 'cab-grid' )),
				'ua' => $_SERVER['HTTP_USER_AGENT'],
				'stylesheet' => plugins_url( 'cab-grid.css', __FILE__  ).'?ver='._cabGridVersion
			);
	...
	function cabGrid_CustomCSS() {
		$output='<link rel="preload" onload="this.rel=\'stylesheet\'" as="style" id="cabGridCSS" href="'.plugins_url( 'cab-grid.css', __FILE__  ).'?ver='._cabGridVersion.'" type="text/css" media="all" />';
	...
	function cabGrid_headerCSS(){
		$output='<!-- CabGrid Wordpress Taxi Plugin - https://cabgrid.com -->';
		$output.='<style id="cabGridHeaderCSS">';
		// $output.='.cabGrid {visibility: hidden;opacity:0}';
		$output.='.cabGrid {opacity:0;}';
		$output.='</style>';
		$output.='<link rel="preload" as="style" id="cabGridCSSpreLoad" href="'.plugins_url( 'cab-grid.css', __FILE__  ).'?ver='._cabGridVersion.'" type="text/css" media="screen" />';
		echo $output;
	}
	...
	function cabGrid_sanitize_options_no_tags( $options ) { // would be called from register_settings in admin when needed
		foreach ( $options as $key => &$value ) {
	...
	function cabGrid_sanitize_options_loose( $options ) { // called from register_settings in admin
		foreach ( $options as $key => &$value ) {
			//changed by Andy Moyle (https://wordpress.org/support/topic/security-and-message-html/)
			$value = wp_kses_post(stripslashes( $value )); // allow some tags
		}
		return $options;
	}
	  	...
	  	public function update($new_instance, $old_instance){
	   	...
	   	public function widget($args, $instance){
			...
			$cabGridForm=cabGrid_render();
	  		echo '<div class="cabGrid_widget" style="min-height:'.esc_attr($height).'px;">'.$cabGridForm.'</div>';
