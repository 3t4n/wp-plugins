/**
 * VrSquare
 * @type Object VrSquare
 */
var VrSquare = function () {
    ...
    var appendContainer = function () {
        var container = document.getElementById(settings.container.replace('#', ''));
        if (container === null) {
    ...
    var getContentUrl = function () {
        ...
        return window.location.protocol + url + '/square';
    };
    ...
    var onMessage = function (msg) {
        ...
        if (typeof msg === 'string') {
            if (msg.indexOf(msgId) != -1) {
                ...
                msg = {app: 'vr_square', cmd: 'resize', params: {height: data.height, width: data.width}};
            }
        }

        if (typeof msg.app == 'undefined' || msg.app != 'vr_square')
            return;
        switch (msg.cmd) {
            case 'resize':
                resize(msg.params);
