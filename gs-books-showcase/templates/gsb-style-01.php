...
if ( $query->have_posts() ) :
			...
			<div class="gsb-book-info-wrapper">
				...
				if ( ! empty( $authors ) && $gs_book_author ) :
					...
					<div class="gsb-author">
						<h4><?php esc_html_e( 'by', 'gsbookshowcase' ); ?> <?php echo Helpers::wp_kses( $authors ); ?></h4>
					</div>
					<?php
				endif;
				?>
				...
				<div class="gsb-book-info">
					<?php if ( ! empty( $publish ) && $gs_book_publish ) : ?>
						<p><span><?php esc_html_e( 'Publish:', 'gsbookshowcase' ); ?></span> <?php esc_html_e( $publish ); ?></p>
						<?php
					endif;

					if ( ! empty( $publisher ) && $gs_book_publisher ) :
						?>
						<p><span><?php echo esc_html__( 'Publisher: ', 'gsbookshowcase' ); ?></span> <?php echo Helpers::wp_kses( $publisher ); ?></p>
						<?php
					endif;

					if( !empty( $gsbks_st_isbn ) ) {
						?>
						<div class="gsb-isbn-ten" style="display: none;">
							<?php esc_html_e( $gsbks_st_isbn ); ?>
						</div>
						<?php
					}
					...
					if( !empty( $gsbks_st_isbn_13 ) ) {
						?>
						<div class="gsb-isbn-thirteen" style="display: none;">
							<?php esc_html_e( $gsbks_st_isbn_13 ); ?>
						</div>
						<?php
					}
					
					if( !empty( $gsbks_st_asin ) ) {
						?>
						<div class="gsb-book-asin" style="display: none;">
							<?php esc_html_e( $gsbks_st_asin ); ?>
						</div>
						<?php
					}
					...
					?>
					<p><?php echo esc_html( 'Store:', 'gsbookshowcase' ); ?> <?php echo Helpers::wp_kses( $store_links ); ?></p>
				<?php	endif; ?>
	...
	echo paginate_links(
		array(
			...
			'current'   => $paged,
			'format'    => '?gs_book=%#%',
			'prev_text' => __( '« Previous', 'gsbookshowcase' ),
			'next_text' => __( 'Next »', 'gsbookshowcase' ),
		)
	);
