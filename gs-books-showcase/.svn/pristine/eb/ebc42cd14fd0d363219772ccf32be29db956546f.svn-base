...
class Integration_Elementor {
    ...
    public function print_elementor_editor_scripts() {
        ...
        <script>
            ...
            window.onload = function() {
                ...
                elementor.hooks.addAction( 'panel/open_editor/widget/gs-bookshowcase', function( panel, model, view ) {

                    var $shortcode_field = jQuery('.elementor-control-gs_bookshowcase .elementor-control-input-wrapper select');
                    var $edit_link = jQuery('.elementor-control-gs_bookshowcase .gstm-edit-link');
    ...
    public function print_elementor_editor_styles() {
        ...
        $icon = GS_BOOKS_PLUGIN_URI . '/assets/img/icon.png';

        ?>

        <style>

            body #elementor-controls .elementor-control-gs_bookshowcase_shortcode .elementor-control-field-description {
                font-size: 12px;
                line-height: 1.8;
            }

            body #elementor-panel-elements-wrapper .icon .gs-bookshowcase {
    ...
    public function print_elementor_preview_scripts() {
        ...
        wp_enqueue_script( 'gs-bookshowcase-elementor-preview', GS_BOOKS_PLUGIN_URI . '/includes/integrations/assets/elementor/elementor-preview.min.js', ['jquery'], GS_BOOKS_VERSION, true );
    }
...
function load_elementor_widget_class() {
    ...
    class Elementor_Widget extends \Elementor\Widget_Base {
        ...
        public function get_name() {
            return 'gs-bookshowcase';
        }

        public function get_title() {
            return 'Book Showcase';
        }
        ...
        protected function register_controls() {
            ...
            $this->start_controls_section(
                'content_section',
                [
                    'label' => __( 'Content', 'gsbookshowcase' ),
                    'tab'   => \Elementor\Controls_Manager::TAB_CONTENT,
                ]
            );

            $this->add_control(
                'gs_bookshowcase_shortcode',
                [
                    'label'       => __( 'Testimonial Shortcode', 'gsbookshowcase' ),
        ...
        protected function get_field_description() {
            ...
            $edit_link = sprintf( '%s: <a class="gstm-edit-link" href="%s" target="_blank">%s</a>',
                __('Edit this shortcode', 'gsbookshowcase'),
                admin_url( "edit.php?post_type=gs_bookshowcase&page=gs-books-shortcode#/shortcode/" ),
                __('Edit', 'gsbookshowcase')
            );

            $create_link = sprintf( '%s: <a class="gs-tm-create-link" href="%s" target="_blank">%s</a>',
                __('Create new shortcode', 'gsbookshowcase'),
                admin_url( 'edit.php?post_type=gs_bookshowcase&page=gs-books-shortcode#/shortcode' ),
                __('Create', 'gsbookshowcase')
            );
        ...
        protected function render() {
            ...
            $shortcode_id = $this->get_settings_for_display( 'gs_bookshowcase_shortcode' );

            if ( empty($shortcode_id) ) {
                if ( is_user_logged_in() && current_user_can( 'manage_options' ) ) {
                    printf( '<div class="gsbook-elementor-no-shortcode" style="padding:20px;background:rgba(115, 113, 124, 0.3);color:#1a1828;font-size:18px;border-radius:4px;font-weight:500;">%s</div>', 'Book Showcase: ' . __('Please select a shortcode', 'gsbookshowcase') );
                }
            } else {
                echo do_shortcode( "[gs_bookshowcase id={$shortcode_id}]" );
            }
