...
class Integration_TagDiv {
    ...
    public function block_css() {
        ...
        $css = sprintf( ".tdc-element-ico.tdc-ico-td_gs_bookshowcase{background-image:url('%s');background-position:center center;background-size:cover}", esc_url_raw($icon) );
    ...
    public function enqueue_scripts() {
        ...
        if ( $load_in_composer_iframe ) {
            ...
            wp_add_inline_script( 'gs-books-showcase-public', "setInterval(function(){jQuery(document).trigger('gsbooks:scripts:reprocess');},500);" );
    ...
    public function register_block() {
        ...
        $file = plugin_dir_path( __FILE__ ) . 'includes/gs-bookshowcase-block.php';

        \td_api_block::add( 'td_gs_bookshowcase', array(

            'map_in_visual_composer' => false,
            'map_in_td_composer'     => true,
            "name"                   => 'Book Showcase',
            "base"                   => 'td_gs_bookshowcase',
            "class"                  => 'td_gs_bookshowcase',
            "controls"               => "full",
            "category"               => 'Content',
            'tdc_category'           => 'Blocks',
            'file'                   => $file,
            ...
            "params"                 => array(
                array(
					"param_name" => "gs_bookshowcase_shortcode",
					"type"       => "dropdown",
					"value"      => $this->get_shortcode_list(),
					"heading"    => __( 'Book Showcase Shortcode', 'gsbookshowcase' )
                ),
            )
        ));
    }
