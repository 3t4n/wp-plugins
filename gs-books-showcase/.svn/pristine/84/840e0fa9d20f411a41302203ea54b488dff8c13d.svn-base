...
use WP_Error;
use function GS_BOOKS_PRO\is_plugin_loaded;
...
class TemplateLoader {
    ...
    public function __construct() {
        ...
        self::$plugin_template_path = GS_BOOKS_PLUGIN_DIR . 'templates/';
        
        if ( is_pro_active() && is_plugin_loaded() ) {
            self::$pro_plugin_template_path = GS_BOOKS_PRO_PLUGIN_DIR . 'templates/';
        }
    ...
    public function set_theme_template_path() {
        ...
        $dir = apply_filters( 'gs_books_templates_folder', 'gs-books' );
    ...
    public static function locate_template( $template_file ) {
        ...
        if ( ! empty(self::$pro_plugin_template_path) && file_exists( self::$pro_plugin_template_path . $template_file ) ) {
            ...
            if ( ! file_exists( $path . $template_file ) ) {
                return new WP_Error( 'gs_books_template_not_found', __( 'Template file not found - GS Plugins', 'gsbookshowcase' ) );
            }
        }
