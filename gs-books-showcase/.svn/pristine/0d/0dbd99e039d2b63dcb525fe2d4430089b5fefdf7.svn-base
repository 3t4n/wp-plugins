    ...
    class GS_Plugins_Common_Pages {
		...
		public function wp_star_rating( $args = [] ) {
			...
			if ( $r['number'] ) {
				...
				$format = _n( '%1$s rating based on %2$s rating', '%1$s rating based on %2$s ratings', $r['number'], 'gsbookshowcase' );
				$title = sprintf( $format, number_format_i18n( $rating, 1 ), number_format_i18n( $r['number'] ) );
			} else {
				/* translators: 1: The rating */
				$title = sprintf( __( '%s rating', 'gsbookshowcase' ), number_format_i18n( $rating, 1 ) );
			}
			...
			if ( current_user_can( "install_plugins") || current_user_can("update_plugins") ) {
				...
				switch ($status["status"]) {
					case "install":
						if ( $status["url"] ) {
							/* translators: 1: Plugin name and version. */
							$action_links[] = '<a class="install-now button" href="' . esc_url($status['url']) . '" aria-label="' . esc_attr( sprintf("Install %s now", $name ) ) . '">' . __( 'Install Now', 'gsbookshowcase' ) . '</a>';
						}
					break;
					case "update_available":
						if ($status["url"]) {
							/* translators: 1: Plugin name and version */
							$action_links[] = '<a class="button" href="' . esc_url($status['url']) . '" aria-label="' . esc_attr( sprintf( "Update %s now", $name ) ) . '">' . __( 'Update Now', 'gsbookshowcase' ) . '</a>';
						}
					break;
					case "latest_installed":
					case "newer_installed":
						$action_links[] = '<span class="button button-disabled" title="' . esc_attr__( "This plugin is already installed and is up to date" ) . ' ">' . _x( 'Installed', 'plugin', 'gsbookshowcase' ) . '</span>';
					break;
				}
			}
