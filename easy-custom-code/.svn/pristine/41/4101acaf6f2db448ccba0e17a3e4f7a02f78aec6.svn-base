...
class lessc {
    ...
    protected function tryImport($importPath, $parentBlock, $out) {
        ...
        $response = wp_remote_get($realPath);

        if (is_wp_error($response)) {
            throw new Exception('Failed to fetch the remote file.');
        }

        $body = wp_remote_retrieve_body($response);

        if (empty($body)) {
            throw new Exception('The retrieved body is empty.');
        }
    ...
    protected function lib_data_uri($value) {
        ...
        if ($fullpath && ($fsize = filesize($fullpath)) !== false) {
            ...
            if ($fsize/1024 < 32) {
                ...
                $response = wp_remote_get($fullpath);
                ...
                $url = sprintf('data:%s;base64,%s', esc_attr($mime), base64_encode($body));
            }
        }
    ...
    protected function injectVariables($args) {
        ...
        foreach ($args as $name => $strValue) {
            ...
            if (!$parser->propertyValue($value)) {
                throw new Exception(esc_html("failed to parse passed in variable $name: $strValue"));
            }
    ...
    public function compileFile($fname, $outFname = null) {
        // Check if the file is readable (local file)
        if (!is_readable($fname)) {
            throw new Exception(esc_html('Load error: failed to find '.$fname));
        }
        ...
        // Check if $fname is a remote URL or a local file
        $body = false;
        if (filter_var($fname, FILTER_VALIDATE_URL)) {
            // Handle as remote URL
            $response = wp_remote_get($fname);
        ...
        $out = $this->compile($body, $fname);
        $this->importDir = $oldImport;

        // If output filename is provided, write the compiled content to the file
        if ($outFname !== null) {
            ...
            if (!$wp_filesystem->put_contents($outFname, $out, FS_CHMOD_FILE)) {
    ...
    public function parse($str = null, $initialVariables = null) {
        ...
        if ($str == null) {
            if (empty($this->_parseFile)) {
                throw new exception(esc_html("nothing to parse"));
            }
...
class lessc_parser {
    ...
    public function parse($buffer) {
        ...
        if ( !property_exists($this->env, 'parent') || !is_null($this->env->parent) )
            throw new exception(esc_html('parse error: unclosed block'));
    ...
    public function throwError($msg = "parse error", $count = null) {
        ...
        if ($this->peek("(.*?)(\n|$)", $m, $count)) {
            throw new exception(esc_html("$msg: failed at `$m[1]` $loc"));
        } else {
            throw new exception(esc_html("$msg: $loc"));
        }
    }
...
class lessc_formatter_classic {
    ...
    public function block($block) {
        ...
        if (!empty($block->selectors)) {
            ...
            if ($isSingle) {
                echo wp_kses_post($this->openSingle);
                $inner = "";
            } else {
                echo wp_kses_post($this->open . $this->break);
        ...
        if (!empty($block->selectors)) {
            if (!$isSingle && empty($block->children)) echo wp_kses_post($this->break);

            if ($isSingle) {
                echo wp_kses_post($this->closeSingle . $this->break);
            } else {
