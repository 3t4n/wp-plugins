...
function foloosiwebhookupdateorder(){ 
    if(!empty($_POST['transaction_no']) && !empty($_POST['order_id']) && !empty($_POST['secret_key'])){
        ...
        $response = wp_remote_get('https://api.foloosi.com/v1/api/transaction-detail/' . $foloosiPaymentId, $args);
...
function woocommerce_foloosi_init()
{
    ...
    class WC_Foloosi extends WC_Payment_Gateway
    {
        ...
        const foloosi_WOO_ORDER_ID           = 'foloosi_woo_order_id';
        ...
        */
        ...
        public function init_form_fields()
        {
            $defaultFormFields = array(
                ...
                'redirection' => array(
                    'title' => __('Redirection', $this->id),
                    'type' => 'checkbox',
                    'label' => __('Would you like a redirection method, which includes Apple Pay, instead of popup?', $this->id),
                    'default' => 'no'
                ),
                'savedCard' => array(
                    'title' => __('Saved Card', $this->id),
                    'type' => 'checkbox',
                    'label' => __('Would you like to use Saved card Feature?', $this->id),
                    'default' => 'no'
                ),
            );
        ...
        public function getReferenceToken($order_id){
            ...
            if($this->getSetting('redirection') == 'yes'){
                $argsArray['site_return_url'] = $this->getRedirectUrl();
            }
            if($this->getSetting('savedCard') == 'yes' && is_user_logged_in()){
                $current_user = wp_get_current_user();
                $argsArray['customer_unique_identifier'] = $current_user->user_email.$current_user->ID;
            }
            $args = array(
                'headers' => array(
                ...
                'body' => apply_filters(
					'woocommerce_foloosi_args', $argsArray
                )
            );
            $response = wp_remote_post('https://api.foloosi.com/aggregatorapi/web/initialize-setup', $args);
            return $response;
        }
        ...
        {
            global $woocommerce;
            //$refTokenForm = $this->getReferenceToken($order_id);
        ...
        {
            return get_site_url() . '/index.php/wc-api/' . $this->id;
        }
        /**
         * Generates the order form
         **/
        function generateOrderForm($refToken, $order_id)
        {
            $redirectUrl = $this->getRedirectUrl();
            // $this->enqueueCheckoutScripts($refToken);

            $redirect = $this->getSetting('redirection');
            $merchantKey = $this->getSetting('key_id');
            ...
            <input type="hidden" value="$order_id" name="foloosi_woo_order_id" id="foloosi_woo_order_id">
            ...
            <script src="https://www.foloosi.com/js/foloosipay.v2.js"></script>
            ...
            foloosiHandler(response, function (e) {
                // console.log(e.data);return false;
        ...
        public function check_foloosi_order_response($foloosiPaymentId){
            $foloosi_key_secret = $this->getSetting('key_secret');
            $args = array(
                'headers' => array(
                    'secret_key' => $foloosi_key_secret
                )
            );
            $response = wp_remote_get('https://api.foloosi.com/v1/api/transaction-detail/' . $foloosiPaymentId, $args);
            return $response;
        ...
        {            
            ...
            if($orderId === NULL){
                $orderId = (!empty($_POST['self::foloosi_WOO_ORDER_ID'])) ? $_POST['self::foloosi_WOO_ORDER_ID'] : $_POST['optional1'];
            }
            $order = new WC_Order($orderId);            
            //
            ...
            if ($orderId  && !empty($_POST[self::foloosi_PAYMENT_ID])) {
                $error = "";
                $success = false;
                try {
                    $success = false;
                    $foloosiPaymentId = sanitize_text_field($_POST[self::foloosi_PAYMENT_ID]);  
                    // $response = $this->check_foloosi_order_response($foloosiPaymentId);
                    $foloosi_key_secret = $this->getSetting('key_secret');

                    //get transaction details
                    $curl = curl_init();
                    curl_setopt_array($curl, array(
                        CURLOPT_URL => "https://api.foloosi.com/v1/api/transaction-detail/" . $foloosiPaymentId,
        ...
        private function enqueueCheckoutScripts($refToken)
        {
            ...
            $params = array(
                ...
                'merchant_key'         => $this->getSetting('key_id'),
                'redirect_url' => get_site_url(),
                'redirect' => ($this->getSetting('redirection') == 'yes') ? true : false
            );
