...
function awtoolkit_manage_suppliers() {
    if( !isset($_GET['action']) ){
        echo supplier_get_template('templete','supplier_list'); //phpcs:ignore
        ...
        echo supplier_get_template('templete','supplier_add'); //phpcs:ignore
    }
}
...
function awtoolkit_add_custom_supplier() {
    if(!get_option('aw_woo_msp')) return ;
    $args = array(
        'label' => __( 'Supplier', 'awcode-toolkit' ),
        'placeholder' => __( 'Enter Supplier here', 'awcode-toolkit' ),
        'id' => '_supplier',
        'desc_tip' => true,
        'description' => __( 'This Supplier is for internal use only.', 'awcode-toolkit' ),
    );
    ...
    */
    ...
    <div class="custom_field_supplier">
        ...
        <label class="alignleft">
            <span class="title"><?php esc_html_e('Supplier', 'awcode-toolkit' ); ?></span>
            <span class="input-text-wrap _supplier_field">
                <input type="text" id="_supplier" name="_supplier" class="text" autocomplete="off" placeholder="<?php esc_html_e( 'Enter Supplier here', 'awcode-toolkit' ); ?>" value="">
            </span>
        </label>
        <br class="clear">
    </div>
    ...
    */
    ...
    switch ( $column ) {
        case 'name' :
            ...
            ?>
            <div class="hidden custom_field_supplier_inline" id="custom_field_supplier_inline_<?php echo esc_attr($post_id); ?>">
                <div id="_custom_field_supplier"><?php echo esc_html(get_post_meta($post_id,'_supplier',true)); ?></div>
            </div>
            <?php
...
function _admin_products_supplier_column_content( $column, $product_id ){
    if ( $column == 'supplier' ) {
        ...
        echo ucfirst($text);//phpcs:ignore
    }
}
...
function _supplier_order_box_callback( $post )
{
    ...
    foreach ($supplier as $key => $value) {
        echo "<h4 style='margin-bottom: 5px;'>".esc_html($value)."</h4>";
        ...
        <div class="inline-edit-group">
            <label class="alignleft">
                ...
                <span class="input-text-wrap">
                    ...
                    <?php
                        $options = array(
                            ''  => __( '— No change —', 'awcode-toolkit' ),
                            '1' => __( 'Change to:', 'awcode-toolkit' ),
                        );
            ...
            <label class="change-input">
                <span class="input-text-wrap _supplier_field_bulk">
                    <input type="text" name="_supplier" id="_supplier_bulk" class="text _supplier" autocomplete="off" placeholder="<?php esc_html_e( 'Enter Supplier here', 'awcode-toolkit' ); ?>" value="" />
                </span>
            </label>
        </div>
    <?php
...
function location_filtering($post_type){
   ...
   //build a custom dropdown list of values to filter by
    ...
    echo '<option value="0">' . esc_html__( 'Filter by supplier', 'awcode-toolkit' ) . ' </option>';
    foreach($results as $supplier){
        if(!empty($supplier)){
          $select = ($supplier == $selected) ? ' selected':'';
          echo '<option value="'.esc_attr($supplier).'"'.esc_attr($select).'>' . esc_attr($supplier) . ' </option>';
        }
    }
    $select = ('unknown' == $selected) ? ' selected':'';
    echo '<option '.esc_attr($select).' value="unknown">' . esc_html_e( 'Unknown', 'awcode-toolkit' ) . ' </option>';
    echo '</select>';
}
