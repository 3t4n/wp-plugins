    ...
    if($column == 'order_number'){
        if(get_option('aw_woo_past_order_count')){
            ...
            if($user_id){
                ...
                echo(' <span title="'.esc_attr($customer_orders).' Total orders">('.esc_attr($customer_orders).')</span>');
            }
        }
        if(get_option('aw_woo_utm')){
            ...
            if(is_array($utm)){
                foreach($utm as $k=>$v){
                    if($v && $k !='landing'){
                        echo('<br><small title="'.esc_attr(str_replace(array('"',"'"),'',$v)).'" style="max-height:2em;display:inline-block;overflow:hidden;max-width: 100%;word-break: break-all;">UTM '.esc_attr($k).': '.esc_attr($v).'</small>');
                    }
                }
            }
        }
...
function awtoolkit_get_total_sales_per_product($product_id ='', $sumNotCount=false) { 
	...
	WHERE posts.post_type = 'shop_order'                   
	AND posts.post_status IN ( '".implode( "','",  $post_status  )."' )"//phpcs:ignore
	."AND order_items.order_item_type = 'line_item'
	AND order_item_meta.meta_key = '_qty'
	AND order_item_meta_2.meta_key = '_product_id'
...
function awtoolkit_utm_tracking() {
    ...
    if(isset($_COOKIE['wp_utm']) && isset($_GET['utm_source'])) {//phpcs:ignore
        //safety check if utm cookie set but blank. Debugging why not always saved
        $old_utm = awextract_utm_array();
        if(isset($old_utm['source']) && !$old_utm['source']){
            unset($_COOKIE['wp_utm']);
        }
    }
    if(!isset($_COOKIE['wp_utm'])) {//phpcs:ignore
        if(isset($_GET['utm_source']) || isset($_GET['utm_medium']) || isset($_GET['utm_campaign']) || isset($_GET['utm_term']) || isset($_GET['utm_content']) || isset($_SERVER['HTTP_REFERER'])){//phpcs:ignore
            $utm = [];
            $utm['source'] = isset($_GET['utm_source']) ? $_GET['utm_source'] : '';//phpcs:ignore
            $utm['medium'] = isset($_GET['utm_medium']) ? $_GET['utm_medium'] : '';//phpcs:ignore
            $utm['campaign'] = isset($_GET['utm_campaign']) ? $_GET['utm_campaign'] : '';//phpcs:ignore
            $utm['term'] = isset($_GET['utm_term']) ? $_GET['utm_term'] : '';//phpcs:ignore
            $utm['content'] = isset($_GET['utm_content']) ? $_GET['utm_content'] : '';//phpcs:ignore
            $utm['referer'] = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';//phpcs:ignore
            $utm['landing'] = $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];//phpcs:ignore
            $utm_str = base64_encode(json_encode($utm));//phpcs:ignore
...
//        if(!get_option('aw_woo_utm')) return ;
        ...
        $order->update_meta_data( '_utm',  base64_decode( str_replace('%3D', '=', $_COOKIE['wp_utm'])));//phpcs:ignore
        if(is_array($utm)){
            if(isset($utm['source']) && $utm['source']){ $order->update_meta_data( '_utm_source',  $utm['source']);}//phpcs:ignore
            if(isset($utm['medium']) && $utm['medium']){ $order->update_meta_data( '_utm_medium',  $utm['medium']);}//phpcs:ignore
            if(isset($utm['campaign']) && $utm['campaign']){ $order->update_meta_data( '_utm_campaign',  $utm['campaign']);}//phpcs:ignore
            if(isset($utm['term']) && $utm['term']){ $order->update_meta_data( '_utm_term',  $utm['term']);}//phpcs:ignore
            if(isset($utm['content']) && $utm['content']){ $order->update_meta_data( '_utm_content',  $utm['content']);}//phpcs:ignore
            if(isset($utm['referer']) && $utm['referer']){ $order->update_meta_data( '_utm_referer',  $utm['referer']);}//phpcs:ignore
            if(isset($utm['landing']) && $utm['landing']){ $order->update_meta_data( '_utm_landing',  $utm['landing']);}//phpcs:ignore
        }
    }else{
...
function awextract_utm_array(){
    $str = str_replace('%3D', '=', $_COOKIE['wp_utm']);//phpcs:ignore
    //had issue with url encoding making base64 fail
    return @json_decode(base64_decode($_COOKIE['wp_utm']), true);//phpcs:ignore
}
