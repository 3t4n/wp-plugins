...
<div class="wrap">
	...
	<form method="post" action="<?php echo esc_url(admin_url( 'options-general.php?page=awtoolbox'))?>">
        ...
        <div id="post-body" class="metabox-holder">
            <div class="awbox">
                    ...
                    <select name="replace_table" id="replace_table" style="width:100%;">
                        <?php
                            foreach($db_tables as $k=>$v){
                                ...
                                echo('<option name="'.esc_attr($table).'" '.(isset($_POST['replace_table']) && $_POST['replace_table'] == $table ? 'selected="selected"' : '').'>'.esc_attr($table).'</option>');
                            }
                        ?>
                    </select>
                </div>
                ...
                <?php if(isset($replace_msg) && $replace_msg){?><p style="font-weight:bold;"><php echo esc_attr($replace_msg) ?></div><?php } ?>
            </div>
            
            <div class="awbox">
                <h4>Woocommerce</h4>
                <p>
                    <input type="checkbox" name="aw_woo_prod_sales_count" id="aw_woo_prod_sales_count" <?php echo get_option('aw_woo_prod_sales_count') ?'checked="checked"':''?>>
                    <label for="aw_woo_prod_sales_count">Woocommerce product list will display total count of sales for each product</label>
                </p>
                <p>
                    <input type="checkbox" name="aw_woo_prod_dimensions" id="aw_woo_prod_dimensions" <?php echo get_option('aw_woo_prod_dimensions') ?'checked="checked"':''?>>
                    <label for="aw_woo_prod_dimensions">Woocommerce product list will display dimensions of each product</label>
                </p>
                <p>
                    <input type="checkbox" name="aw_woo_past_order_count" id="aw_woo_past_order_count" <?php echo get_option('aw_woo_past_order_count') ?'checked="checked"':''?>>
                    <label for="aw_woo_past_order_count">Woocommerce order list will display count of past orders for the customer</label>
                </p>
                <p>
                    <input type="checkbox" name="aw_woo_utm" id="aw_woo_utm" <?php echo get_option('aw_woo_utm') ?'checked="checked"':''?> >
                    <label for="aw_woo_utm">Woocommerce orders will track UTM parameters from inbound links</label>
                </p>
                <p>
                    <input type="checkbox" name="aw_woo_msp" id="aw_woo_msp" <?php echo get_option('aw_woo_msp') ?'checked="checked"':''?> >
            ...
            <div class="awbox">
                ...
                <p>
                    <input type="checkbox" name="aw_maint_mode" id="aw_maint_mode" <?php echo get_option('aw_maint_mode') ?'checked="checked"':''?>>
                ...
                <p>
                    ...
                    <input type="text" name="aw_maint_title" id="aw_maint_title" value="<?php echo get_option('aw_maint_title') ? esc_attr(get_option('aw_maint_title')) :'Website coming soon!'?>">
                </p>
                <label for="aw_maint_message">Content</label>
                <textarea name="aw_maint_message" style="width:100%;"><?php
                echo stripslashes(get_option('aw_maint_message'));//phpcs:ignore
                ?></textarea>
                <label for="aw_maint_exceptions">Exceptions (1 url per line)</label>
                <textarea name="aw_maint_exceptions" style="width:100%;"><?php
                echo stripslashes(get_option('aw_maint_exceptions'));//phpcs:ignore
                ?></textarea>
            </div>
            <div class="awbox">
