...
<div class="wrap supplier">
	...
	<div style="width: 50%;margin-top: 15px;">
		<form id="supplier_addnew" action="<?php echo esc_url(site_url()) ?>/wp-admin/admin-ajax.php" method="POST">
			<?php if( $post_id ){ ?>
			<input type="hidden" name="post_id" value="<?php echo esc_attr($post_id) ?>">
			<?php } ?>
			<input type="hidden" name="action" value="supplierAddNew">
			<input type="hidden" id="_wpnonce" name="_wpnonce" value="<?php echo esc_attr($wpnonce) ?>">
			<input type="hidden" name="_wp_http_referer" value="<?php echo esc_url(site_url()) ?>/wp-admin/edit.php?post_type=product&page=manage-suppliers">
			<div class="form-group">
				<label>Name</label>
				<input type="text" name="name" class="form-control" required value="<?php echo esc_attr($title) ?>">
			</div>
			<div class="form-group">
				<label>Contact info</label>
				<textarea name="info" class="form-control" rows="4" cols="5"><?php echo esc_attr($info) ?></textarea>
			</div>
