...
class XFAVI_Get_Unit_Offer_Variable extends XFAVI_Get_Unit_Offer {
	...
	use XFAVI_T_Variable_Get_Accessory_Type;
	use XFAVI_T_Variable_Get_Acea;
	use XFAVI_T_Variable_Get_Adtype;
	use XFAVI_T_Variable_Get_Aft;
	use XFAVI_T_Variable_Get_Another_Type;
	use XFAVI_T_Variable_Get_Api;
	use XFAVI_T_Variable_Get_Apparel;
	use XFAVI_T_Variable_Get_Appareltype;
	use XFAVI_T_Variable_Get_BodySparePartType;
	use XFAVI_T_Variable_Get_Brand;
	use XFAVI_T_Variable_Get_Astm;
	use XFAVI_T_Variable_Get_Availability;
	use XFAVI_T_Variable_Get_Avito_Id;
	use XFAVI_T_Variable_Get_Cabinet_Type;
	use XFAVI_T_Variable_Get_Capacity;
	use XFAVI_T_Variable_Get_Category;
	use XFAVI_T_Variable_Get_Color;
	use XFAVI_T_Variable_Get_Condition;
	use XFAVI_T_Variable_Get_Custom_Tags_From_Attr;
	use XFAVI_T_Variable_Get_Custom_Type;
	use XFAVI_T_Variable_Get_DCL;
	use XFAVI_T_Variable_Get_Generation;
	use XFAVI_T_Variable_Get_Forwhom;
	use XFAVI_T_Variable_Get_Gender;
	use XFAVI_T_Variable_Get_Dimensions;
	use XFAVI_T_Variable_Get_Description;
	use XFAVI_T_Variable_Get_Dot;
	use XFAVI_T_Variable_Get_EngineSparePartType;
	...
	use XFAVI_T_Variable_Get_OEMOil;
	use XFAVI_T_Variable_Get_Plumbing_Type;
	use XFAVI_T_Variable_Get_Polarity;
	use XFAVI_T_Variable_Get_Price;
	use XFAVI_T_Variable_Get_ProductSubType;
	use XFAVI_T_Variable_Get_Sae;
	use XFAVI_T_Variable_Get_Size;
	use XFAVI_T_Variable_Get_Spare_Part_Type;
	use XFAVI_T_Variable_Get_Stock;
	use XFAVI_T_Variable_Get_Technic_Spare_Part_Type;
	use XFAVI_T_Variable_Get_Transmission_Spare_Part_Type;
	use XFAVI_T_Variable_Get_TiresType;
	use XFAVI_T_Variable_Get_Title;
	use XFAVI_T_Variable_Get_Vendorcode;
	use XFAVI_T_Variable_Get_Voltage;
	use XFAVI_T_Variable_Get_Volume;
	use XFAVI_T_Variable_Get_Weight;
	use XFAVI_T_Variable_Get_XML_Stock_Item;
		...
		while ( $duplicate_number < $duplicate_count ) {
			...
			if ( get_term_meta( $this->get_feed_category_id(), 'xfavi_avito_standart', true ) !== '' ) {
				...
				new XFAVI_Error_Log( sprintf( 'FEED № %1$s; %2$s %3$s %4$s; Файл: %5$s; %6$s: %7$s',
					...
					'class-xfavi-get-unit-offer-variable.php',
	...
	private function get_common_xml_data( $result_xml = '' ) {
		...
		if ( class_exists( 'WOOCS' ) ) {
			$xfavi_wooc_currencies = common_option_get( 'xfavi_wooc_currencies', false, $this->get_feed_id(), 'xfavi' );
	 ...
	 * @return string
	 */
	public function get_variable_product_post_meta( $key, $prefix = '_xfavi_' ) {
	 ...
	 *    - `xfavi_f_variable_tag_value_{tag_value}`
	 *    - `xfavi_f_variable_tag_name_{tag_value}`
	 *    - `xfavi_f_variable_tag_{tag_value}`
	 * 
	...
	private function get_variable_tag( $tag_name, $tag_value, $tag_attributes_arr = [] ) {
		...
		$tag_value = apply_filters(
			'xfavi_f_variable_tag_value_' . $tag_name,
			$tag_value,
			[ 
				'product' => $this->get_product(),
				'offer' => $this->get_offer()
		...
		if ( empty( $tag_value ) ) {
			...
			$tag_name = apply_filters(
				'xfavi_f_variable_tag_name_' . $tag_name,
				$tag_name,
				[ 
					'product' => $this->get_product(),
					'offer' => $this->get_offer()
		...
		$result_xml = apply_filters(
			'xfavi_f_variable_tag_' . $tag_name,
			$result_xml,
			[ 
				'product' => $this->get_product(),
				'offer' => $this->get_offer()
