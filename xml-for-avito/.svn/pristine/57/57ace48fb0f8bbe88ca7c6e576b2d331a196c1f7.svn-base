...
/**
 * Traits PlumbingType for simple products
 *
...
trait XFAVI_T_Simple_Get_Plumbing_Type {

	/**
	 * Get `PlumbingType` tag.
	 * 
	 ...
	 * 
	 * @return string Example: `<PlumbingType>Бетон</PlumbingType>`
	 */
	public function get_plumbing_type( $tag_name = 'PlumbingType', $result_xml = '' ) {
		...
		$tag_value = apply_filters(
			'xfavi_f_simple_tag_value_plumbing_type',
			$tag_value,
			[ 
		...
		$result_xml = apply_filters(
			'xfavi_f_simple_tag_plumbing_type',
			$result_xml,
			[ 
				'product' => $this->get_product()
			],
