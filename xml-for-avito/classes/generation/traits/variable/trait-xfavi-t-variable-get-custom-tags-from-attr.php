 ...
 */
trait XFAVI_T_Variable_Get_Custom_Tags_From_Attr {
	...
	public function get_custom_tags_from_attr( $result_xml = '' ) {
		...
		for ( $i = 1; $i < 6; $i++ ) {
			...
			if ( $custom_tag_name === '' || $custom_tag_attr === 'disabled' ) {
				...
				$custom_tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $attr_id ) );
				if ( empty( $custom_tag_value ) ) {
					$custom_tag_value = $this->get_product()->get_attribute( wc_attribute_taxonomy_name_by_id( $attr_id ) );
				}
			}
			if ( ! empty( $custom_tag_value ) ) {
		...
		$result_xml = apply_filters(
			'xfavi_f_variable_custom_tags_from_attr',
			$result_xml,
			[ 
