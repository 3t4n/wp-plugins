...
function dvsfw_bulk_action_error_notices() {
	if ( isset( $_GET['dvsfw_bulk_done'] ) && ! empty( $_GET['dvsfw_bulk_done'] ) ) {
		...
		// sanitize the array
		$bulk_order_ids = array_map( 'sanitize_text_field', wp_unslash( $_GET['dvsfw_bulk_done'] ) );
...
function dvsfw_bulk_action_url_handler() {
	...
	if ( empty( $_GET['dvsfw_bulk_label_url'] ?? false ) ) {
		return;
	}

	// Sanitize and validate the URL
	$url = esc_url_raw( wp_unslash( $_GET['dvsfw_bulk_label_url'] ) );
	...
	if ( $encoded_url ) {
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const url = "<?php echo esc_url( $encoded_url ); ?>";
...
add_action( 'admin_footer', 'dvsfw_bulk_action_url_handler', 99 );
...
function dvsfw_bulk_label_error_notices() {
	if ( ! empty( $_GET['dvsfw_bulk_labels'] ) ) {
		...
		// sanitize the array
		$errors = array_map( 'sanitize_text_field', wp_unslash( $_GET['dvsfw_bulk_labels'] ) );
...
function dvsfw_bulk_action_notices() {
	...
	$li           = '';
	$bulk_done    = array_map( 'sanitize_text_field', wp_unslash( $_REQUEST['dvsfw_bulk_done'] ?? [] ) );
	$ship_numbers = $bulk_done['shipNumber'] ?? [];
	if ( ! empty( $ship_numbers ) ) {
