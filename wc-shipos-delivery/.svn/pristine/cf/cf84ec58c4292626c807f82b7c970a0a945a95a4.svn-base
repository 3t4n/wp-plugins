...
class Ajax {
	...
	public function matat_open_new_order( $automatic = null ) {
		...
		if ( ! $automatic ) {
			$order_id = absint( $_REQUEST['dvsfw_order_id'] ?? 0 );
		} else {
			$order_id = $automatic;
		...
		$order->add_meta_data( 'dvsfw_shipment_create_start', gmdate( 'Y-m-d H:i:s' ) );
		$order->save();
		...
		$ship_data['type']           = $automatic ? '1' : absint( $_REQUEST['dvsfw_delivey_type'] ?? 1 );
		$ship_data['packages']       = $automatic ? '1' : absint( $_REQUEST['dvsfw_packages'] ?? 1 );
		...
		$ship_data['exaction_date']  = $automatic ? current_time( 'Y-m-d' ) : sanitize_text_field( wp_unslash( $_REQUEST['dvsfw_exaction_date'] ?? current_time( 'Y-m-d' ) ) );
		$ship_data['collect']        = '';
		$ship_data['delivery_time']  = current_time( 'd-m-Y g-i-s' );
		$licenseKey                  = sanitize_text_field( wp_unslash( $_REQUEST['dvsfw_license'] ?? null ) );
		...
		foreach ( $order->get_items() as $item ) {
			...
			$order_item['meta_data'] = $meta_objects;
		...
		$order_data = array_map( function ( $item ) {
			if ( $item instanceof \WC_DateTime ) {
				return $item->date( 'Y-m-d H:i:s' );
			}

			return $item;
		}, $order_data );

		$response = $this->matat_web_service->create_ship( $ship_data, $order_data, $order_items, $shipping_lines, $licenseKey );
		if ( ! $response->success ) {
	...
	public function matat_get_order_details() {
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['matat_get_wpnonce'] ?? null ) ), 'dvsfw_submit_get_ship' ) ) {
			wp_die( esc_html__( 'Failed security check', 'wc-shipos-delivery' ) );

		}
		$order_id          = absint( $_REQUEST['dvsfw_order_id'] ?? 0 );
		$license_key       = sanitize_text_field( wp_unslash( $_REQUEST['license_key'] ?? null ) );
	...
	public function matat_change_order_status() {
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['matat_change_wpnonce'] ?? null ) ), 'dvsfw_submit_change_ship' ) ) {
			wp_die( esc_html__( 'Failed security check', 'wc-shipos-delivery' ) );
		}

		$ship_id     = sanitize_text_field( wp_unslash( $_REQUEST['dvsfw_ship_id'] ?? null ) );
		$license_key = sanitize_text_field( wp_unslash( $_REQUEST['dvsfw_license_key'] ?? null ) );
	...
	public function dvsfw_reopen_ship() {
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['dvsfw_reopen_wpnonce'] ?? null ) ), 'dvsfw_reopen_ship' ) ) {
			wp_die( esc_html__( 'Failed security check', 'wc-shipos-delivery' ) );
		}
		$order_id = sanitize_text_field( wp_unslash( $_REQUEST['dvsfw_woo_order_id'] ?? 0 ) );
	...
	public function dvsfw_get_pickup_locations() {
		...
		if ( ! WP_Filesystem() ) {
			dvsfw_log( 'Failed to initialise WC_Filesystem API while trying to sync Shipos Cities.' );
