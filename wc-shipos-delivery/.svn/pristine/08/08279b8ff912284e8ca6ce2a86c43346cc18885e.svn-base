...
class Delivery {
	...
	private function set_admin_hooks() {
		...
		if ( License::getInstance()->has_license() ) {
			...
			add_action( 'admin_init', array( $this, 'redirect_to_label' ), 20 );
			add_action( 'admin_init', array( $this, 'clear_cache' ), 20 );
	 ...
	 * @since 1.0.0
	 */
	public function redirect_to_label() {
		global $pagenow;

		if ( $pagenow == 'post.php' && isset( $_GET['matat_pdf'] ) && sanitize_text_field( wp_unslash( $_GET['matat_pdf'] ) ) == 'create' ) {
			if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['matat_label_wpnonce'] ?? null ) ), 'matat_create_label' ) ) {
			...
			$order     = wc_get_order( absint( sanitize_text_field( wp_unslash( $_GET['order_id'] ?? null ) ) ) );
			$ship_data = Utils::get_order_meta( $order, '_dvsfw_ship_data', false );
			$shipos    = new WebService();

			$shipping_code = ( (array) $ship_data[ $ship_id ] )['delivery_number'] ?? 0;
			$license_key   = $ship_data[ $ship_id ]['license_key'] ?? null;

			$url = $shipos->matat_label_url( $shipping_code, $license_key );
	...
	public function clear_cache() {
		if ( isset( $_GET['dvsfw_clear_cache'] ) ) {
