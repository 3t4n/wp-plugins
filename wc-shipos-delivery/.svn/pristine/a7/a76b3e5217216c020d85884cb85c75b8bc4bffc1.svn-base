...
define( 'DVSFW_PLUGIN_VERSION', '2.2.1' );
...
function dvsfw_activate_shipos_delivery() {
	...
	if ( empty( $dvsfw_install_date_db ) ) {
		add_option( 'dvsfw_install_date', gmdate( 'd-m-Y' ) );
	} else {
		update_option( 'dvsfw_install_date', gmdate( 'd-m-Y' ) );
	}
...
function dvsfw_activation_redirect( $plugin ) {
	...
	if ( $plugin == plugin_basename( __FILE__ ) ) {
		wp_redirect( esc_url_raw( admin_url( 'admin.php?page=deliver-via-shipos' ) ) );
		exit();
	}
}
...
/**
 *  Log error using the WC_Logger class or fallback to error_log
 *
 * @param string $message Log message
 * @param array $context Optional. Additional information for log handlers.
 * @param string $level Log severity level; options: 'emergency', 'alert', 'critical', 'error', 'warning', 'notice', 'info', 'debug'.
 * @param string $source Identifies where the log was generated; defaults to the plugin if not specified.
 * @param bool $backtrace If true, includes a stack trace in the context for error-related logs.
 *
 * @return void
 */
function dvsfw_log( string $message, array $context = [], string $level = 'error', string $source = 'deliver-via-shipos', bool $backtrace = true ) {
	try {
		$logger  = new WC_Logger();
		$context = array_merge( $context, [ 'source' => $source, 'backtrace' => $backtrace ] );

		$logger->log( $level, $message, $context );
	} catch ( Throwable $th ) {
		$log = wp_json_encode( [ "message" => $message, "context" => $context ], JSON_UNESCAPED_UNICODE );
		error_log( "$level: " . $log ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions
	}
}