...
class RestRoutes {
	...
	public function register_rest_routes() {
		...
		register_rest_route( 'wc/v3', '/dvsfw_update_custom_field', array(
			'methods'             => 'POST',
	...
	public function get_shipos_settings( WP_REST_Request $request ): WP_REST_Response {
		...
		$cache_key = 'dvsfw_options_cache';
		$options   = wp_cache_get( $cache_key );

		// Doing direct database query for performance reasons as we are fetching option based on a wildcard
		if ( ! $options ) {
			global $wpdb;
