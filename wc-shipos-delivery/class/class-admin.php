...
class Admin {
	...
	public function matat_meta_box_side( $_order ) {
		...
		if ( is_wp_error( $licenses ) ) {
			dvsfw_log( $licenses->get_error_message() );
...
//			$matat_delivery_number_array = Utils::get_order_meta( $order, '_dvsfw_ship_data', false );
		...
		if ( ! count( $licenses ) ) {
			esc_html_e( "You do not have any valid licenses", "wc-shipos-delivery" );
        ...
        <div class="matat-wrapper">
            <div id="matat_open_ship_new">
                ...
                <div style="display: flex; gap: 20px">
                    <div class="dvsfw_form-group">
                        <label for="dvsfw_exaction_date_new"><?php esc_html_e( 'Deliver on:', 'wc-shipos-delivery' ); ?></label>
                        <br>
                        <input id="dvsfw_exaction_date_new" type="date" name="date"
                               value="<?php echo esc_attr( gmdate( current_time( 'Y-m-d' ) ) ); ?>">
                    </div>

                    <div class="dvsfw_form-group">
                        <label for="dvsfw_packages_new"><?php esc_html_e( 'Packages:', 'wc-shipos-delivery' ); ?></label>
                        <br>
	...
	public function add_plugin_menu_and_options() {
		...
		$title = '<div style="display:flex; gap: .25rem"><img src="' . $logo . '"  alt="" style="width: 20px"/>Ship OS shipping ' . '</div>'; // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage
	...
	public function shipos_page() {
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'wc-shipos-delivery' ) );
		}

		if ( isset( $_GET['token'] ) ) {
			update_option( 'dvsfw_shipos_token', sanitize_text_field( wp_unslash( $_GET['token'] ) ) );
	...
	public function admin_bar_item( \WP_Admin_Bar $admin_bar ) {
		...
		$clear_cache_url = add_query_arg( 'dvsfw_clear_cache', '1', $current_url );
		$admin_bar->add_node( [
			'id'     => 'dvsfw_clear_cache',
			'title'  => 'Clear cache',
			'parent' => 'deliver-via-shipos',
