...
function dvsfw_pickup( $checkout ) {
	$shipos_delivery_location    = WC()->session->get( 'shipos_delivery_location' );
	$shipos_delivery_location_id = WC()->session->get( 'shipos_delivery_location_id' );
	// display Chita Delivery button on checkout page only and for local pickup only
	if ( $checkout->method_id == 'shipos_delivery' && ( is_cart() || is_checkout() ) ) { ?>
        ...
        <div class="shipos_opener_wrap">
            <button class="shipos_popup_open" type="button">
				...
				$button_text .= '</span>';
				echo wp_kses_post( apply_filters( 'dvsfw_shipos_delivery_button_title', $button_text ) );
				echo wp_kses_post( apply_filters( 'dvsfw_shipos_delivery_button_subtitle', '' ) );
				?>
            </button>
        </div>
		...
		woocommerce_form_field(
			...
			array(
				...
				'default'           => $shipos_delivery_location
			)
		);
		...
		woocommerce_form_field(
			...
			array(
				...
				'default'     => $shipos_delivery_location_id
			)
		);
...
function dvsfw_pickup_popup() {
	if ( ( is_cart() || is_checkout() ) ) {
		$reload_image   = plugins_url( 'admin/img/reload.gif', dirname( __FILE__ ) ); // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage -- Directly using the image is intentional and necessary here.
		$location_image = plugins_url( '/public/img/location.svg', dirname( __FILE__ ) ); // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage -- Directly using the image is intentional and necessary here.
		$time_image     = plugins_url( '/public/img/time.svg', dirname( __FILE__ ) ); // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage -- Directly using the image is intentional and necessary here.
		?>
                            ...
                            <div class="shipos_tabs">
                                ...
                                <div class="shipos_tab" v-show="activeTab === 'manual'">
                                    ...
                                    <div class="shipos_locations_loader" v-if="loading">
                                        <img src="<?php echo esc_url( $reload_image ); // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>"
                                             alt="Loading..."/>
                                    </div>
                                    <div class="pickup-result-holder result_active has_ship_actions"
                                            ...
                                            <div class="pickup-location-item" v-for="location in filteredLocations"
                                                ...
                                                <label>
                                                    ...
                                                    <div class="pickup-address">
                                                        <img src="<?php echo esc_url( $location_image ); // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>"
											   ...
											   target="_blank">לצפייה במפה</a>-->
                                                    ...
                                                    <div class="pickup-address">
                                                        <img src="<?php echo esc_url( $time_image ); // phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>"
                                                             class="time" alt="time">
                                                        <div class="text-wrap">
                                                            {{ location.remarks }}
                                                        </div>
...
function dvsfw_custom_shipos_checkout_field_notice() {
	...
	if ( false !== strpos( $chosen_shipping, 'shipos_delivery' ) ) {
		if ( ! sanitize_text_field( wp_unslash( $_POST['shipos_delivery'] ?? null ) ) ) {
...
function dvsfw_update_pickup_order_meta( $order_id ) {
	...
	WC()->session->__unset( 'shipos_delivery_location' );
	WC()->session->__unset( 'shipos_delivery_location_id' );
	...
	$is_shipos_delivery = false;
	$shipping_methods   = $order->get_shipping_methods();
	foreach ( $shipping_methods as $shipping_method ) {
		if ( $shipping_method->get_method_id() === 'shipos_delivery' ) {
			$is_shipos_delivery = true;
		}
	}

	if ( ! $is_shipos_delivery ) {
		return;
	}

	$shipos_delivery_address = sanitize_text_field( wp_unslash( $_POST['shipos_delivery'] ?? null ) );
	if ( $shipos_delivery_address ) {
		$order->update_meta_data( 'shipos_delivery_address', $shipos_delivery_address );
	}

	$shipos_delivery_address_id = sanitize_text_field( wp_unslash( $_POST['shipos_delivery_id'] ?? null ) );
	if ( $shipos_delivery_address_id ) {
		$order->update_meta_data( 'shipos_delivery_address_id', $shipos_delivery_address_id );
	}
	$order->save();
}
...
// Get delivery points for dropdown
function dvsfw_get_pickup_locations() {
	require_once ABSPATH . 'wp-admin/includes/file.php';
	if ( ! WP_Filesystem() ) {
		dvsfw_log( __( 'Failed to initialise WC_Filesystem API while trying to sync Shipos Cities.', 'wc-shipos-delivery' ) );
	...
	if ( file_exists( $filename ) ) {
		$delivery_points = json_decode( $wp_filesystem->get_contents( $filename ) );
	} else {
		$delivery_points = ( new WebService() )->matat_pickup_locations();
	}

	if ( ! isset( $delivery_points->spots->spot_detail ) ) {
		return [];
	}

	$delivery_points = $delivery_points->spots->spot_detail;

	$pickup_locations = [];
	foreach ( $delivery_points as $delivery_point ) {
		$pickup_locations[ $delivery_point->n_code ] = $delivery_point->name . ' ' . $delivery_point->street . ' ' . $delivery_point->house . ' ' . $delivery_point->city;
	}

	// Remove empty
	return array_filter( array_map( 'trim', $pickup_locations ) );
}

function dvsfw_get_pickup_location_by_id( $pickup_location_id ) {
	$delivery_points = dvsfw_get_pickup_locations();

	return $delivery_points[ $pickup_location_id ] ?? null;
}

function dvsfw_display_admin_order_meta( $order ) {
	$delivery_point_id = Utils::get_order_meta( $order, 'shipos_delivery_address_id' );
	$delivery_point    = dvsfw_get_pickup_location_by_id( $delivery_point_id );
		...
		echo wp_kses_post(
			sprintf(
				'<p class="form-field form-field-wide">
                    <strong>%s</strong>
                    <span class="shipos-delivery-display">%s</span>
                </p>',
				__( 'Requested Delivery point', 'wc-shipos-delivery' ),
				$delivery_point
			)
		);
	}
...
add_action( 'woocommerce_admin_order_data_after_shipping_address', 'dvsfw_display_admin_order_meta' );


// Add delivery point field to shipping address edit form
function dvsfw_add_delivery_point_to_edit_form( $fields, $order ) {
	$pickup_locations = dvsfw_get_pickup_locations();

	$selected_id = Utils::get_order_meta( $order, 'shipos_delivery_address_id' );

	if ( ! array_key_exists( $selected_id, $pickup_locations ) ) {
		$pickup_locations[0] = "Invalid Pickup Location";
	};

	if ( $selected_id ) {
		$fields['shipos_delivery_address_id'] = array(
			'label'         => __( 'Delivery Point', 'wc-shipos-delivery' ),
			'type'          => 'select',
...
add_filter( 'woocommerce_admin_shipping_fields', 'dvsfw_add_delivery_point_to_edit_form', 10, 2 );

// Save delivery point when shipping address is updated
function dvsfw_save_delivery_point( $order_id ) {
	$delivery_point_id = sanitize_text_field( wp_unslash( $_POST['_shipping_shipos_delivery_address_id'] ?? null ) );
	if ( $delivery_point_id ) {
		$delivery_point = dvsfw_get_pickup_location_by_id( $delivery_point_id );

		$order            = wc_get_order( $order_id );
		$current_point    = $order->get_meta( 'shipos_delivery_address' );
		$current_point_id = $order->get_meta( 'shipos_delivery_address_id' );

		// Save both ID and text
		$order->update_meta_data(
			'shipos_delivery_address_id',
			$delivery_point_id
		);
		$order->update_meta_data(
			'shipos_delivery_address',
			$delivery_point
		);
		$order->save();

		$order->add_order_note( "Delivery address updated from $current_point($current_point_id) to $delivery_point($delivery_point_id)" );

	}
}

add_action( 'woocommerce_process_shop_order_meta', 'dvsfw_save_delivery_point' );
...
add_action( 'wp_ajax_save_shipos_pickup_location', 'save_shipos_pickup_location_to_session' );
add_action( 'wp_ajax_nopriv_save_shipos_pickup_location', 'save_shipos_pickup_location_to_session' );

function save_shipos_pickup_location_to_session() {
	check_ajax_referer( 'dvsfw_pickup_checkout_js', 'nonce' );

	$location    = sanitize_text_field( wp_unslash( $_POST['location'] ?? null ) );
	$location_id = sanitize_text_field( wp_unslash( $_POST['location_id'] ?? null ) );
	...
	WC()->session->set( 'shipos_delivery_location', $location );
	WC()->session->set( 'shipos_delivery_location_id', $location_id );
