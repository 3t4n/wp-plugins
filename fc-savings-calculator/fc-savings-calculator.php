...
/**
 * AccurateCalculators.com savings calculator plugin
 *
...
/**
 * Plugin Name: AC Savings Calculator
 * Plugin URI: https://AccurateCalculators.com/calculator-plugins/savings-plus-plugin
 * Description: A responsive savings calculator with date-based savings schedule and charts. Rebrand with your name. Supports 90 currencies, 6 date formats, and 15 languages. Highly customizable via extensive configuration options. Version 2.0 is a major update—test before upgrading.
 * Version: 2.0
 * Author: AccurateCalculators.com
 * Author URI: https://AccurateCalculators.com
 * License: Commercial (but free)
 * Text Domain: ac-savings-calculator-plus
...
const FC_SAVINGS_VER = '2.0'; // for cache busting
const FC_SAVINGS_PLUGIN_NAME = 'AC Savings Calculator';
const FC_SAVINGS_CSS_SCOPED_RESET = 'dist/css/bootstrap-reboot-scoped.css';
const FC_SAVINGS_CSS_MAIN = 'dist/css/accurate-calculators.css';
const FC_SAVINGS_CSS_CUSTOM = 'dist/css/accurate-calculators-custom.css';
const FC_SAVINGS_JS_ENTRY = 'dist/js/interface.SAVINGS.gpl.js';
const FC_SAVINGS_DOWNLOAD_URL = 'https://accuratecalculators.com/plugins/ZIPs/fc-savings-calculator.2.0.zip';
const FC_SAVINGS_DERIVATIVE_OF = 'https://accuratecalculators.com/savings-calculator';

const FC_SAVINGS_TIME_OUT = 15; // seconds

// endpoint gets site's domain where plugin is installed
// dev testing
// const fc_savings_ACTIVATION_ENDPOINT = 'http://localhost:8090/wp-content/themes/accurate/plugin-endpoints/plugin-activation-handler.php'; // The server endpoint to send the data
// production
// const fc_savings_ACTIVATION_ENDPOINT = 'https://accuratecalculators.com/wp-content/themes/accurate/plugin-endpoints/plugin-activation-handler.php'; // The server endpoint to send the data

// Constants for the plugin update check
const FC_SAVINGS_PLUGIN_SLUG = 'ac-savings-calculator-plus/ac-savings-calculator-plus.php'; // Path to the main plugin file
// const fc_savings_UPDATE_ENDPOINT = 'https://accuratecalculators.com/wp-content/themes/accurate/plugin-endpoints/--plus-endpoint.php'; // URL to the update endpoint
const FC_SAVINGS_TIMEOUT_SECS = 15;

// Define constants for easy migration across plugins
const FC_SAVINGS_PLUGIN_AUTHOR = 'AccurateCalculators.com'; // Plugin author
const FC_SAVINGS_PLUGIN_AUTHOR_HOMEPAGE = 'https://accuratecalculators.com/contact';
const FC_SAVINGS_PLUGIN_HOMEPAGE = 'https://accuratecalculators.com/calculator-plugins/savings-plus-plugin';
// for fallback only
const FC_SAVINGS_PLUGIN_DESCRIPTION = '';
 ...
 */
// function fc_savings_information($result, $action, $args)
// {
...
// 	if (isset($args->slug) && $args->slug === FC_SAVINGS_PLUGIN_SLUG) {
// 		// Fetch the update information from the update endpoint
// 		$response = wp_remote_get(fc_savings_UPDATE_ENDPOINT, ['timeout' => FC_SAVINGS_TIMEOUT_SECS]);
...
// 			$result = new stdClass();
// 			$result->name = $plugin_data['name'] ?? FC_SAVINGS_PLUGIN_NAME;
// 			$result->slug = FC_SAVINGS_PLUGIN_SLUG;
// 			$result->version = $plugin_data['new_version'];
// 			$result->tested = get_bloginfo('version');
// 			$result->author = $plugin_data['author'] ?? FC_SAVINGS_PLUGIN_AUTHOR;
// 			$result->author_profile = $plugin_data['author_homepage'] ?? FC_SAVINGS_PLUGIN_AUTHOR_HOMEPAGE;
// 			$result->homepage = $plugin_data['homepage'] ?? FC_SAVINGS_PLUGIN_HOMEPAGE;
// 			$result->sections = [
// 				'description' => $plugin_data['description'] ?? FC_SAVINGS_PLUGIN_DESCRIPTION,
 ...
 */
// function fc_savings_get_update_info()
// {
// 	// Fetch the update info from the remote server.
// 	$response = wp_remote_get(fc_savings_UPDATE_ENDPOINT, array('timeout' => FC_SAVINGS_TIMEOUT_SECS));
 ...
 */
// function fc_savings_check_for_update($transient)
// {
...
// 	$update_info = fc_savings_get_update_info();
// 	if (empty($update_info)) {
// 		return $transient;
// 	}

// 	// Compare current version with new version.
// 	if (version_compare($transient->checked[FC_SAVINGS_PLUGIN_SLUG], $update_info['new_version'], '<')) {
// 		$plugin_info = new stdClass();
// 		$plugin_info->slug = FC_SAVINGS_PLUGIN_SLUG;
...
// 		$transient->response[FC_SAVINGS_PLUGIN_SLUG] = $plugin_info;
// 	}
// 	// else {
// 	// 	error_log('Current plugin version is up to date: ' . FC_SAVINGS_VER);
// 	// }

// 	return $transient;
// } // fc_savings_check_for_update
// add_filter('pre_set_site_transient_update_plugins', 'fc_savings_check_for_update', 20);


/**
 * Function: fc_savings_activate
 * activation hook
...
function fc_savings_activate()
{
	...
	// WAS: fcsavingscalc_plugin
	update_option('acsavings_key', array(
		'op_size' => null,
		'op_custom_style' => "No",
		'op_add_link' => "No",
		'op_brand_name' => "",
...
register_activation_hook(__FILE__, 'fc_savings_activate');


/**
 * Function: fc_savings_show_widget
 * WAS: show__fcsavings_widget
 * Shows the plugin in a WordPress widget area / sidebar
 *
 * args: $args (environment variables handled automatically by the hook)
 * returns: nothing
 */
function fc_savings_show_widget($args)
{
	extract($args);
	$options = get_option('acsavings_key');
	$title = null;
	...
	echo $after_widget;
} // fc_savings_show_widget


// Helper functions for ac_savingsplus
if (!function_exists('fc_savings_validate_yes_no')) {
	function fc_savings_validate_yes_no($value, $default = 'No', $option_name = null)
	{
...
if (!function_exists('fc_savings_validate_size')) {
	function fc_savings_validate_size($value, $default = 'large', $option_name = null)
	{
		$value = strtolower(sanitize_text_field($value));
...
if (!function_exists('fc_savings_validate_numeric')) {
	function fc_savings_validate_numeric($value, $default = '0', $option_name = null)
	{
		...
		if (is_numeric($value) && $value >= 0) {
			...
			if ($option_name) {
				echo __('Please enter a valid number for', 'fc-savings-calculator') . " \"$option_name.\"<br>";
			}
			return $default;
		}
	}
...
if (!function_exists('fc_savings_validate_text')) {
	function fc_savings_validate_text($value, $default = '', $option_name = null)
	{
		return preg_replace("/[^\w#&'\-,. ]/", '', $value);
	}
}

if (!function_exists('fc_savings_validate_color')) {
	function fc_savings_validate_color($value, $default = '#000000', $option_name = null)
	{
		$value = sanitize_text_field($value);
...
if (!function_exists('fc_savings_validate_dimension')) {
	function fc_savings_validate_dimension($value, $default = '0', $option_name = null)
	{
		$value = sanitize_text_field($value);
...
if (!function_exists('fc_savings_validate_font_stack')) {
	function fc_savings_validate_font_stack($value, $default = '', $option_name = null)
	{
		return sanitize_text_field($value);
	}
}

if (!function_exists('fc_savings_validate_shadow')) {
	function fc_savings_validate_shadow($value, $default = '', $option_name = null)
	{
		return sanitize_text_field($value);
	}
}

if (!function_exists('fc_savings_get_processed_option')) {
	function fc_savings_get_processed_option($option_name, $keys, $sources, $default, $validation_func = null)
	{
		$value = null;
		foreach ($keys as $key) {
...
/**
 * Displays the savings calculator GUI with options for shortcode attributes or direct function parameters.
 *
...
{
	wp_enqueue_script('fc-i18n-savings');
	wp_enqueue_script('fc-savings-interface');

	// Set translations for hack script only
	wp_set_script_translations('fc-i18n-savings', 'fc-savings-calculator', plugin_dir_path(__FILE__) . 'languages');
	...
	$shortcode = false;
	if (!empty($code) || (!empty($options) && array_key_exists(0, $options) && (strtolower($options[0]) == 'fcloanplugin' || strtolower($options[0]) == 'fcsavingsplugin'))) {
		$shortcode = true;
		...
		$temp_options = get_option('acsavings_key');
	...
	$option_mappings = array(
		...
		'save_amt' => array('sc_save_amt', 'op_save_amt'),
	...
	$validation_functions = array(
		'size' => function ($value, $default, $option_name) use ($allowed_sizes) {
			return fc_savings_validate_size($value, $default, $option_name);
		},
		'custom_style' => 'fc_savings_validate_yes_no',
		'add_link' => 'fc_savings_validate_yes_no',
		'brand_name' => 'fc_savings_validate_text',
		'hide_resize' => 'fc_savings_validate_yes_no',
		'hide_intl_conventions' => 'fc_savings_validate_yes_no',
		'save_amt' => function ($value, $default, $option_name) {
			return fc_savings_validate_numeric($value, '0', $option_name);
		},
		...
		'theme_base_font_size' => 'fc_savings_validate_dimension',
		'theme_primary_color' => 'fc_savings_validate_color',
		'theme_primary_color_hover' => 'fc_savings_validate_color',
		'theme_primary_color_light' => 'fc_savings_validate_color',
		'theme_primary_color_text' => 'fc_savings_validate_color',
		'theme_primary_color_text_inverse' => 'fc_savings_validate_color',
		'theme_background_muted' => 'fc_savings_validate_color',
		'theme_background_color_disabled' => 'fc_savings_validate_color',
		'theme_background_calculator_color' => 'fc_savings_validate_color',
		'theme_background_modal_color' => 'fc_savings_validate_color',
		'theme_border_color' => 'fc_savings_validate_color',
		'theme_text_color' => 'fc_savings_validate_color',
		'theme_tooltip_text_color' => 'fc_savings_validate_color',
		'theme_shadow_color' => 'fc_savings_validate_shadow',
		'theme_primary_font_family_stack' => 'fc_savings_validate_font_stack',
		'theme_mono_font_family_stack' => 'fc_savings_validate_font_stack',
		'calculator_min_width_tiny' => 'fc_savings_validate_dimension',
		'calculator_max_width_tiny' => 'fc_savings_validate_dimension',
		'calculator_min_width_small' => 'fc_savings_validate_dimension',
		'calculator_max_width_small' => 'fc_savings_validate_dimension',
		'calculator_min_width_medium' => 'fc_savings_validate_dimension',
		'calculator_max_width_medium' => 'fc_savings_validate_dimension',
		'calculator_min_width_large' => 'fc_savings_validate_dimension',
		'calculator_max_width_large' => 'fc_savings_validate_dimension',
	);
	...
	foreach ($option_mappings as $option_name => $keys) {
		...
		$processed_options[$option_name] = fc_savings_get_processed_option($option_name, $keys, $sources, $default, $validation_func);
	}
	...
	wp_register_style('accuratecalcs-reset-style', plugins_url(FC_SAVINGS_CSS_SCOPED_RESET, __FILE__), array(), FC_SAVINGS_VER, 'screen');

	// Register the main styles with 'accuratecalcs-reset-style' as a dependency
	wp_register_style('accuratecalcs-style', plugins_url(FC_SAVINGS_CSS_MAIN, __FILE__), array('accuratecalcs-reset-style'), FC_SAVINGS_VER, 'screen');
	...
	if (strtolower($custom_style) === 'yes') {
		wp_register_style('accuratecalcs-custom-style', plugins_url(FC_SAVINGS_CSS_CUSTOM, __FILE__), array('accuratecalcs-reset-style'), FC_SAVINGS_VER, 'screen');
 ...
 * See: show_fcsavings_plugin for enqueuing.
 * Enqueuing in show_fcsavings_plugin() prevents script from being loaded on pages where plugin is not utilized.
 */
function fc_savings_enqueue_scripts()
{
	// Register the hack script required for i18n support for ES6 modules.
	wp_register_script('fc-i18n-savings', plugin_dir_url(__FILE__) . 'dist/js/ac-i18n-fix.js', array('wp-i18n'), null, true);

	// Register the module script
	wp_register_script('fc-savings-interface', plugins_url(FC_SAVINGS_JS_ENTRY, __FILE__), array(), FC_SAVINGS_VER, true);

	// see: show_fcsavings_plugin
	// wp_set_script_translations('fc-i18n-savings', 'fc-savings-calculator', plugin_dir_path(__FILE__) . 'languages');

	// $locale = get_locale();
	// error_log('$locale -> ' . $locale);
}
add_action('wp_enqueue_scripts', 'fc_savings_enqueue_scripts');
 ...
 * 
 */
function fc_savings_init()
{
	// Load the plugin's .mo file
	load_plugin_textdomain('fc-savings-calculator', false, basename(dirname(__FILE__)) . '/languages/');
	// add_filter('plugins_api', 'fc_savings_information', 10, 3);
}
add_action('init', 'fc_savings_init');


/**
 * Function: fc_savings_show_admin_options
 *
...
function fc_savings_show_admin_options()
{
	...
	$options = $newoptions = get_option('acsavings_key');
	...
	if (!array_key_exists('op_size', $options) || !array_key_exists('op_custom_style', $options) || !array_key_exists('op_add_link', $options) || !array_key_exists('op_brand_name', $options) || !array_key_exists('op_hide_resize', $options) || !array_key_exists('op_save_amt', $options) || !array_key_exists('op_n_months', $options) || !array_key_exists('op_rate', $options) || !array_key_exists('op_currency', $options) || !array_key_exists('op_date_mask', $options)) {
		...
		$options = $newoptions = get_option('acsavings_key'); // keep everything in sync
	}


	// if widget's options have previously been set/saved in current session
	if (!empty($_POST['acsavings_opts'])) {
		$newoptions['op_size'] = strip_tags(stripslashes($_POST['acsavings-op_size']));
		...
		$newoptions['op_custom_style'] = strip_tags(stripslashes($_POST['acsavings-op_custom_style']));
		if (strtolower($newoptions['op_custom_style']) != 'yes') {
			$newoptions['op_custom_style'] = 'No';
		}
		$newoptions['op_add_link'] = strip_tags(stripslashes($_POST['acsavings-op_add_link']));
		...
		$newoptions['op_brand_name'] = preg_replace("/[^\w#&'\-,. ]/", '', $_POST['acsavings-op_brand_name']);
		if (strtolower($newoptions['op_add_link']) != 'yes') {
			$newoptions['op_brand_name'] = '';
		}
		$newoptions['op_hide_resize'] = strip_tags(stripslashes($_POST['acsavings-op_hide_resize']));
		...
		$newoptions['op_n_months'] = strip_tags(stripslashes($_POST['acsavings-op_n_months']));
		$newoptions['op_rate'] = strip_tags(stripslashes($_POST['acsavings-op_rate']));
		// [KT] 02/05/2020 - new options
		$newoptions['op_currency'] = strip_tags(stripslashes($_POST['acsavings-op_currency']));
		$newoptions['op_date_mask'] = strip_tags(stripslashes($_POST['acsavings-op_date_mask']));

		// [KT] 08/21/2024 - new options
		$newoptions['op_theme_base_font_size'] = strip_tags(stripslashes($_POST['acsavings-op_theme_base_font_size']));
		$newoptions['op_theme_primary_color'] = strip_tags(stripslashes($_POST['acsavings-op_theme_primary_color']));
		$newoptions['op_theme_primary_color_hover'] = strip_tags(stripslashes($_POST['acsavings-op_theme_primary_color_hover']));
		$newoptions['op_theme_primary_color_light'] = strip_tags(stripslashes($_POST['acsavings-op_theme_primary_color_light']));
		$newoptions['op_theme_primary_color_text'] = strip_tags(stripslashes($_POST['acsavings-op_theme_primary_color_text']));
		$newoptions['op_theme_primary_color_text_inverse'] = strip_tags(stripslashes($_POST['acsavings-op_theme_primary_color_text_inverse']));
		$newoptions['op_theme_background_muted'] = strip_tags(stripslashes($_POST['acsavings-op_theme_background_muted']));
		$newoptions['op_theme_background_color_disabled'] = strip_tags(stripslashes($_POST['acsavings-op_theme_background_color_disabled']));
		$newoptions['op_theme_background_calculator_color'] = strip_tags(stripslashes($_POST['acsavings-op_theme_background_calculator_color']));
		$newoptions['op_theme_background_modal_color'] = strip_tags(stripslashes($_POST['acsavings-op_theme_background_modal_color']));
		$newoptions['op_theme_border_color'] = strip_tags(stripslashes($_POST['acsavings-op_theme_border_color']));
		$newoptions['op_theme_text_color'] = strip_tags(stripslashes($_POST['acsavings-op_theme_text_color']));
		$newoptions['op_theme_tooltip_text_color'] = strip_tags(stripslashes($_POST['acsavings-op_theme_tooltip_text_color']));
		$newoptions['op_theme_shadow_color'] = strip_tags(stripslashes($_POST['acsavings-op_theme_shadow_color']));
		$newoptions['op_theme_primary_font_family_stack'] = strip_tags(stripslashes($_POST['acsavings-op_theme_primary_font_family_stack']));
		$newoptions['op_theme_mono_font_family_stack'] = strip_tags(stripslashes($_POST['acsavings-op_theme_mono_font_family_stack']));
		$newoptions['op_calculator_min_width_tiny'] = strip_tags(stripslashes($_POST['acsavings-op_calculator_min_width_tiny']));
		$newoptions['op_calculator_max_width_tiny'] = strip_tags(stripslashes($_POST['acsavings-op_calculator_max_width_tiny']));
		$newoptions['op_calculator_min_width_small'] = strip_tags(stripslashes($_POST['acsavings-op_calculator_min_width_small']));
		$newoptions['op_calculator_max_width_small'] = strip_tags(stripslashes($_POST['acsavings-op_calculator_max_width_small']));
		$newoptions['op_calculator_min_width_medium'] = strip_tags(stripslashes($_POST['acsavings-op_calculator_min_width_medium']));
		$newoptions['op_calculator_max_width_medium'] = strip_tags(stripslashes($_POST['acsavings-op_calculator_max_width_medium']));
		$newoptions['op_calculator_min_width_large'] = strip_tags(stripslashes($_POST['acsavings-op_calculator_min_width_large']));
		$newoptions['op_calculator_max_width_large'] = strip_tags(stripslashes($_POST['acsavings-op_calculator_max_width_large']));
		// [KT] 09/03/2024 - new options
		$newoptions['op_hide_intl_conventions'] = strip_tags(stripslashes($_POST['acsavings-op_hide_intl_conventions']));
		...
		if (!is_numeric($newoptions['op_save_amt'])) {
			...
			$newoptions['op_save_amt'] = $options['op_save_amt']; // leave it unchanged
			$error_messages[] = __('Please enter a valid number for "Default savings amount."', 'fc-savings-calculator') . "<br>";
		}
		if (!is_numeric($newoptions['op_n_months'])) {
			$newoptions['op_n_months'] = $options['op_n_months'];
			$error_messages[] = __('Please enter a valid number for "Default number of months."', 'fc-savings-calculator');
		}
		if (!is_numeric($newoptions['op_rate'])) {
			$newoptions['op_rate'] = $options['op_rate'];
			$error_messages[] = __('Please enter a valid number for "Default interest rate."', 'fc-savings-calculator');
		}
		// [KT] 02/05/2020 - new options
		if (!is_numeric($newoptions['op_currency'])) {
			$newoptions['op_currency'] = '999';
			$error_messages[] = __('Please enter a valid value for "Default Currency."', 'fc-savings-calculator');
		}
		if (!is_numeric($newoptions['op_date_mask'])) {
			$newoptions['op_date_mask'] = '999';
			$error_messages[] = __('Please enter a valid number for "Default Date Format."', 'fc-savings-calculator');
		}
		...
		foreach (
			...
			if (!preg_match('/^\d+(px|rem|em|%)$/', $newoptions[$option])) {
				...
				$error_messages[] = __(sprintf('Please enter a valid CSS dimension (e.g., "340px", "10rem") for "%s".', $option), 'fc-savings-calculator');
			}
		}
		...
		foreach (
			...
			if (!preg_match('/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$|^rgba?\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*(,\s*0?\.\d+)?\s*\)$/', $newoptions[$option])) {
				// Leave the value unchanged if validation fails
				$newoptions[$option] = $options[$option];
				$error_messages[] = __(sprintf('Please enter a valid color value (e.g., "#ffffff", "rgba(255, 255, 255, 0.5)") for "%s".', $option), 'fc-savings-calculator');
			}
		}
		...
		if (!empty($error_messages)) {
			error_log('AC Savings Calculator Plugin admin page error -> ' . implode(', ', $error_messages));
			// set_transient is not showing the error message - revisit
			// set_transient('acsavingsplus_error_messages', $error_messages, 30); // 30 seconds duration
		}
	}
	//debug
	//else {
	...
	if ($options != $newoptions && (wp_verify_nonce($_POST['acsavings_opts'], 'fc_options'))) {

		// 2nd check permissions
		if (is_user_logged_in() && current_user_can('update_plugins')) {
			$options = $newoptions;
			update_option('acsavings_key', $options);
		}
		//debug
		//else if (array_key_exists('acsavingsplus_opts', $_POST) && (!wp_verify_nonce($_POST['acsavings_opts'], 'fc_options'))) {
...
?>
	...
	<div class="ac-container">
		...
		<p class="small">
			<?php _e('Do not forget to save your changes. Scroll all the way down in this panel for the <span class="red">[Save]</span> button.', 'fc-savings-calculator'); ?>
		</p>

		<div class="ac-option">
			<label for="acsavingsplus-op_size">
				<?php _e('Widget\'s size (size limits adjustable below)?', 'fc-savings-calculator'); ?>:
				<select name="acsavingsplus-op_size" id="acsavingsplus-op_size" class="widefat">
					<option value="tiny" <?php selected($options['op_size'], 'tiny'); ?>><?php _e('Tiny (max width = 180px)', 'fc-savings-calculator'); ?></option>
					<option value="small" <?php selected($options['op_size'], 'small'); ?>><?php _e('Small (max width = 300px)', 'fc-savings-calculator'); ?></option>
		...
		<div class="ac-option">
			<label for="acsavingsplus-op_custom_style">
				<?php _e('Load custom style sheet?', 'fc-savings-calculator'); ?>:
				<select name="acsavingsplus-op_custom_style" id="acsavingsplus-op_custom_style" class="widefat">
			...
			<p class="ac-description">
				<?php _e('If "Yes" loads &lt;site&gt;\wp-content\plugins\ac-savings-calculator-plus\css\accurate-calculators-custom.css. Entries in <b>accurate-calculators-custom.css</b> override the plugin\'s look. Editing the provided custom stylesheet is another way to modify the look of the calculator. For best performance include only the rules in the custom stylesheet you are overriding. BACKUP YOUR CUSTOMIZATIONS! You will need to manually replace them when (if) you upgrade the plugin.', 'fc-savings-calculator'); ?>
			</p>
		</div>

		<div class="ac-option">
			<label for="acsavingsplus-op_currency">
				<?php _e('Set a default currency?', 'fc-savings-calculator'); ?>:
				<select name="acsavingsplus-op_currency" id="acsavingsplus-op_currency" class="widefat">
		...
		<div class="ac-option">
			<label for="acsavingsplus-op_date_mask">
				<?php _e('Set a date format convention?', 'fc-savings-calculator'); ?>:
				<select name="acsavingsplus-op_date_mask" id="acsavingsplus-op_date_mask" class="widefat">
			...
			<p class="ac-description">
				<?php _e('When a user first visits your site, the plugin will use the browser\'s international conventions to set the currency and date format. If you wish, you may override the plugin and set a default currency and date format. The user will still have the ability to override either setting if you do not hide the &quot;international conventions&quot; below.', 'fc-savings-calculator'); ?>
			</p>
		</div>

		<p class="small">
			<?php _e('<strong>Note:</strong> Your visitors may select a date format and currency by clicking on <span class="red">$ : MM/DD/YYYY</span> in the calculator\'s lower right corner if you do not set "Hide international conventions" to "yes".', 'fc-savings-calculator'); ?>
		</p>
		...
		<div class="ac-grid">
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_hide_resize">
					<?php _e('Hide the resize buttons?', 'fc-savings-calculator'); ?>:
					<select name="acsavingsplus-op_hide_resize" id="acsavingsplus-op_hide_resize" class="widefat">
			...
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_hide_intl_conventions">
					<?php _e('Hide international conventions?', 'fc-savings-calculator'); ?>:
					<select name="acsavingsplus-op_hide_intl_conventions" id="acsavingsplus-op_hide_intl_conventions" class="widefat">
						<option value="No" <?php selected($options['op_hide_intl_conventions'], 'No'); ?>><?php _e('No', 'fc-savings-calculator'); ?></option>
						<option value="Yes" <?php selected($options['op_hide_intl_conventions'], 'Yes'); ?>><?php _e('Yes', 'fc-savings-calculator'); ?></option>
					</select>
				</label>
			</div>
			<!-- </div>
		<div class="ac-grid"> -->
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_add_link">
					<?php _e('Link to', 'fc-savings-calculator'); ?>&nbsp;AccurateCalculators.com?:
					<select name="acsavingsplus-op_add_link" id="acsavingsplus-op_add_link" class="widefat">
		...
		<p class="ac-description">
			<?php _e('If "Link to AccurateCalculators.com" is set to "Yes", one very discreet follow link will be inserted in the calculator. If you allow the link, you may rebrand the calculator with your name. Resetting this option to "No" at any time will remove the links. See FAQ\'s in readme file for details.', 'fc-savings-calculator'); ?>
		</p>

		<div class="ac-option">
			<label for="acsavingsplus-op_brand_name">
				<?php _e('Add Your Brand Name', 'fc-savings-calculator'); ?>:
				<input id="acsavingsplus-op_brand_name" name="acsavingsplus-op_brand_name" type="text" value="<?php echo $brand_name; ?>" />
			</label>
			<p class="ac-description">
				<?php _e('You may brand this widget with your brand. <b>Example: "Abe\'s Best"</b> will be preappended to "Loan Calculator". For this to work, the above "Link to" option must be set to "Yes".', 'fc-savings-calculator'); ?>
			</p>
		</div>
		...
		<div class="ac-grid">
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_loan_amt">
					<?php _e('Default savings amount', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_loan_amt" name="acsavingsplus-op_loan_amt" type="text" value="<?php echo $save_amt; ?>" />
				</label>
			</div>
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_n_months">
					<?php _e('Default number of months', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_n_months" name="acsavingsplus-op_n_months" type="text" value="<?php echo $n_months; ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_rate">
					<?php _e('Default interest rate', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_rate" name="acsavingsplus-op_rate" type="text" value="<?php echo $rate; ?>" />
				</label>
			</div>
			<div class="ac-grid__item text__item">
				<span class="ac-note">
					<?php _e('Enter only digits.<br>No currency or formatting.', 'fc-savings-calculator'); ?>
				</span>
			</div>
		</div>
		...
		<p class="ac-note">
			<?php _e('Style options.', 'fc-savings-calculator'); ?>
			<br>
			<span class="small"><?php _e('(Defaults in parentheses. "hex", "rgb", or "rgba" values accepted for colors.)', 'fc-savings-calculator'); ?></span>
		</p>
		...
		<div class="ac-grid">
			...
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_theme_base_font_size">
					<?php _e('op_theme_base_font_size', 'fc-savings-calculator'); ?> (16px):
					<input id="acsavingsplus-op_theme_base_font_size" name="acsavingsplus-op_theme_base_font_size" type="text" value="<?php echo esc_attr($options['op_theme_base_font_size']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_theme_primary_color">
					<?php _e('op_theme_primary_color', 'fc-savings-calculator'); ?> (#28a745):
					<input id="acsavingsplus-op_theme_primary_color" name="acsavingsplus-op_theme_primary_color" type="text" value="<?php echo esc_attr($options['op_theme_primary_color']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_theme_primary_color_hover">
					<?php _e('op_theme_primary_color_hover', 'fc-savings-calculator'); ?>&nbsp(#218838):
					<input id="acsavingsplus-op_theme_primary_color_hover" name="acsavingsplus-op_theme_primary_color_hover" type="text" value="<?php echo esc_attr($options['op_theme_primary_color_hover']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_theme_primary_color_text">
					<?php _e('op_theme_primary_color_text', 'fc-savings-calculator'); ?> (#fff):
					<input id="acsavingsplus-op_theme_primary_color_text" name="acsavingsplus-op_theme_primary_color_text" type="text" value="<?php echo esc_attr($options['op_theme_primary_color_text']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="acsavingsplus-op_theme_primary_color_light">
					<?php _e('op_theme_primary_color_light', 'fc-savings-calculator'); ?> (#30c853):
					<input id="acsavingsplus-op_theme_primary_color_light" name="acsavingsplus-op_theme_primary_color_light" type="text" value="<?php echo esc_attr($options['op_theme_primary_color_light']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="acsavingsplus-op_theme_primary_color_text_inverse">
					<?php _e('op_theme_primary_color_text_inverse', 'fc-savings-calculator'); ?> (#fff):
					<input id="acsavingsplus-op_theme_primary_color_text_inverse" name="acsavingsplus-op_theme_primary_color_text_inverse" type="text" value="<?php echo esc_attr($options['op_theme_primary_color_text_inverse']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="acsavingsplus-op_theme_background_color_disabled">
					<?php _e('op_theme_background_color_disabled', 'fc-savings-calculator'); ?> (#efefef):
					<input id="acsavingsplus-op_theme_background_color_disabled" name="acsavingsplus-op_theme_background_color_disabled" type="text" value="<?php echo esc_attr($options['op_theme_background_color_disabled']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="acsavingsplus-op_theme_background_calculator_color">
					<?php _e('op_theme_background_calculator_color', 'fc-savings-calculator'); ?> (#fff):
					<input id="acsavingsplus-op_theme_background_calculator_color" name="acsavingsplus-op_theme_background_calculator_color" type="text" value="<?php echo esc_attr($options['op_theme_background_calculator_color']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="acsavingsplus-op_theme_background_modal_color">
					<?php _e('op_theme_background_modal_color', 'fc-savings-calculator'); ?> (#fff):
					<input id="acsavingsplus-op_theme_background_modal_color" name="acsavingsplus-op_theme_background_modal_color" type="text" value="<?php echo esc_attr($options['op_theme_background_modal_color']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_theme_background_muted">
					<?php _e('op_theme_background_muted', 'fc-savings-calculator'); ?> (#f7f7f7):
					<input id="acsavingsplus-op_theme_background_muted" name="acsavingsplus-op_theme_background_muted" type="text" value="<?php echo esc_attr($options['op_theme_background_muted']); ?>" />
				</label>
			</div>
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_theme_border_color">
					<?php _e('op_theme_border_color', 'fc-savings-calculator'); ?> (#dee2e6):
					<input id="acsavingsplus-op_theme_border_color" name="acsavingsplus-op_theme_border_color" type="text" value="<?php echo esc_attr($options['op_theme_border_color']); ?>" />
				</label>
			</div>
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_theme_text_color">
					<?php _e('op_theme_text_color', 'fc-savings-calculator'); ?> (#333333):
					<input id="acsavingsplus-op_theme_text_color" name="acsavingsplus-op_theme_text_color" type="text" value="<?php echo esc_attr($options['op_theme_text_color']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_theme_tooltip_text_color">
					<?php _e('op_theme_tooltip_text_color', 'fc-savings-calculator'); ?> (#fff):
					<input id="acsavingsplus-op_theme_tooltip_text_color" name="acsavingsplus-op_theme_tooltip_text_color" type="text" value="<?php echo esc_attr($options['op_theme_tooltip_text_color']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="acsavingsplus-op_theme_shadow_color">
					<?php _e('op_theme_shadow_color', 'fc-savings-calculator'); ?> (rgba(0, 0, 0, 0.1)):
					<input id="acsavingsplus-op_theme_shadow_color" name="acsavingsplus-op_theme_shadow_color" type="text" value="<?php echo esc_attr($options['op_theme_shadow_color']); ?>" />
				</label>
			</div>
		...
		<div class="ac-grid">
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_theme_primary_font_family_stack">
					<?php _e('op_theme_primary_font_family_stack', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_theme_primary_font_family_stack" name="acsavingsplus-op_theme_primary_font_family_stack" type="text" value="<?php echo esc_attr($options['op_theme_primary_font_family_stack']); ?>" />
				</label>
			</div>
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_theme_mono_font_family_stack">
					<?php _e('op_theme_mono_font_family_stack', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_theme_mono_font_family_stack" name="acsavingsplus-op_theme_mono_font_family_stack" type="text" value="<?php echo esc_attr($options['op_theme_mono_font_family_stack']); ?>" />
				</label>
			</div>
		</div>
		...
		<p class="ac-note">
			<?php _e('You may tweak the minimum and maximum widths.', 'fc-savings-calculator'); ?>
			<br>
			<span class="small"><?php _e('("px", "rem", and "em" units accepted.)', 'fc-savings-calculator'); ?></span>
		</p>
		...
		<div class="ac-grid">
			...
			<div class="ac-grid__item">
				<label for="acsavingsplus-op_calculator_min_width_tiny">
					<?php _e('op_calculator_min_width_tiny', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_calculator_min_width_tiny" name="acsavingsplus-op_calculator_min_width_tiny" type="text" value="<?php echo esc_attr($options['op_calculator_min_width_tiny']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_calculator_max_width_tiny">
					<?php _e('op_calculator_max_width_tiny', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_calculator_max_width_tiny" name="acsavingsplus-op_calculator_max_width_tiny" type="text" value="<?php echo esc_attr($options['op_calculator_max_width_tiny']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_calculator_min_width_small">
					<?php _e('op_calculator_min_width_small', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_calculator_min_width_small" name="acsavingsplus-op_calculator_min_width_small" type="text" value="<?php echo esc_attr($options['op_calculator_min_width_small']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_calculator_max_width_small">
					<?php _e('op_calculator_max_width_small', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_calculator_max_width_small" name="acsavingsplus-op_calculator_max_width_small" type="text" value="<?php echo esc_attr($options['op_calculator_max_width_small']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_calculator_min_width_medium">
					<?php _e('op_calculator_min_width_medium', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_calculator_min_width_medium" name="acsavingsplus-op_calculator_min_width_medium" type="text" value="<?php echo esc_attr($options['op_calculator_min_width_medium']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_calculator_max_width_medium">
					<?php _e('op_calculator_max_width_medium', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_calculator_max_width_medium" name="acsavingsplus-op_calculator_max_width_medium" type="text" value="<?php echo esc_attr($options['op_calculator_max_width_medium']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_calculator_min_width_large">
					<?php _e('op_calculator_min_width_large', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_calculator_min_width_large" name="acsavingsplus-op_calculator_min_width_large" type="text" value="<?php echo esc_attr($options['op_calculator_min_width_large']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="acsavingsplus-op_calculator_max_width_large">
					<?php _e('op_calculator_max_width_large', 'fc-savings-calculator'); ?>:
					<input id="acsavingsplus-op_calculator_max_width_large" name="acsavingsplus-op_calculator_max_width_large" type="text" value="<?php echo esc_attr($options['op_calculator_max_width_large']); ?>" />
				</label>
			</div>
	...
	<input type="hidden" id="acsavingsplus_opts" name="acsavingsplus_opts" value="<?php echo wp_create_nonce('acsavingsplus_ac_options'); ?>" />
<?php

} // fc_savings_show_admin_options()


/**
 * Function: fc_savings_register
 *
...
function fc_savings_register()
{
	$widget_ops = array('classname' => 'acsavings_key', 'description' => __('AC Savings Calculator by AccurateCalculators.com'));

	$name = __('AC Savings Calculator Plugin');
	...
	wp_register_sidebar_widget('fcsavingsplugin', $name, 'fc_savings_show_widget', $widget_ops);
	...
	wp_register_widget_control('fcsavingsplugin', $name, 'fc_savings_show_admin_options');
} // fc_savings_register


// Hooks a function on to a specific action.
add_action('widgets_init', 'fc_savings_register');
 ...
 */
function fc_savings_add_module_to_js($tag, $handle, $src)
{
	// Check if this is the handle for the script that needs the module type
	if ('fc-savings-interface' === $handle) {
		// Add the type="module" attribute
		$tag = '<script type="module" src="' . esc_url($src) . '"></script>';
	}
	return $tag;
}
add_filter('script_loader_tag', 'fc_savings_add_module_to_js', 10, 3);


/**
 * Enqueue backend stylesheet for Appearance -> Widget page
 */
function fc_savings_admin_styles($hook)
{
...
add_action('admin_enqueue_scripts', 'fc_savings_admin_styles');
...
// add_action('admin_notices', 'fc_savings_show_error_messages');

/*
function fc_savings_show_error_messages() {
    global $pagenow;
    ...
    if ($pagenow == 'widgets.php' || $pagenow == 'customize.php') {
        $error_messages = get_transient('acsavingsplus_error_messages');
        ...
        if (!empty($error_messages)) {
            ...
            delete_transient('acsavingsplus_error_messages');
        }
    }
}
*/
