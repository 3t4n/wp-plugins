...
export class SavingsCharts {
	...
	static initAnnualTotalChart () {
		...
		let annualBarChartData = {
			...
			datasets: [{
				...
				label: savingsStrings.strs.s228,
				...
				label: savingsStrings.strs.s2265,
				backgroundColor: PRIMARY_COLOR,
				data: this.annual_investment
			}, {
				type: 'bar',
				// label: 'Annual Investment Gain',
				label: savingsStrings.strs.s227,
					...
					y: {
						...
						ticks: {
							callback: function (label) {
								return Utils.formatNumericValueWithSym(label / THOUSANDS, Locales.moneyConventions, 0) + SavingsCharts.kStr;
							}
						}
					}
				}
	...
	static initAccumulatedTotalChart () {
		...
		let runningBarChartData = {
			...
			datasets: [{
				type: 'line',
				// label: 'Balance',
				label: savingsStrings.strs.s224,
				borderWidth: 1, // width in pixels
				...
				label: savingsStrings.strs.s2345,
				backgroundColor: PRIMARY_COLOR,
				data: this.running_investment
			}, {
				type: 'bar',
				// label: 'Running Investment Gain',
				label: savingsStrings.strs.s235,
				backgroundColor: SECONDARY_COLOR,
	...
	static initPIPieChart () {
		...
		let pieLabelArray = [savingsStrings.strs.s2065, savingsStrings.strs.s243];
						...
						callbacks: {
							label: function (context) {
								...
								if (context.parsed.y !== null) {
									...
									// if (SavingsCharts.payments > 0) {
									if (endingBalance > 0) {
										// Calculate exact percentage for this slice
										let exactPercentage = (context.parsed / endingBalance * 100);
										// let exactPercentage = (context.parsed / SavingsCharts.payments * 100);
	...
	static createSavingsCharts (schedule) {
		...
		if (L > 0) {
			...
			// Annual Savings and Interest Totals
			this.chart0Title = savingsStrings.strs.s2295;
			// Accumulated Savings and Interest
			this.chart1Title = savingsStrings.strs.s2305;
			// Amount Saved & Interest as Percentage of Total Value
			this.chart2Title = savingsStrings.strs.s2315;
