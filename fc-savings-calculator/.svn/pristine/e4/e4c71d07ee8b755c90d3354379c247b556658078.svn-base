...
export class SavingsCalculation {
	...
	static insertSubTotals (schedule) {
		...
		do {
			...
			if (((schedule[i - 1][YEAR] !== schedule[i][YEAR] && (schedule[i - 1][MONTH] <= DECM || schedule[i][MONTH] > DECM))) || ((schedule[i - 1][YEAR] === schedule[i][YEAR] && (schedule[i - 1][MONTH] <= DECM && DECM < schedule[i][MONTH])))) {
				...
				// ' YTD:'
				totals[PER_STR] = schedule[i - 1][YEAR] + ' ' + savingsStrings.strs.s101 + ':';
				...
				// 'Running Totals:'
				totals[PER_STR] = savingsStrings.strs.s102 + ':';
				totals[DATE_STR] = null;
				totals[CF] = runningCF;
		...
		if (schedule[schedule.length - 1][ROW_TYPE] !== Globals.ROW_TYPES.RUNNING_TOTALS) {
			...
			totals[ROW_TYPE] = Globals.ROW_TYPES.ANNUAL_TOTALS; // YTD total row marker
			// ' YTD:'
			totals[PER_STR] = schedule[schedule.length - 1][YEAR] + ' ' + savingsStrings.strs.s101 + ':';
			totals[DATE_STR] = null;
			totals[CF] = ytdCF;
			totals[CREDIT] = ytdCredit;
			totals[DEBIT] = ytdDebit;
	...
	static initSavingsScheduleData (obj, periodicRate) {
		...
		let L, balance, pmt, deposit, withdrawal, cf, netChange, scheduledDateStr, sortDateStr, periodYearString, periods, nYears = 1, interestAccrued = 0, principalPaid = 0, trans = [], priorDate = new Date(0), transDate = new Date(0), schedule = [];
		...
		transDate.setTime(obj.oDate.getTime());
		...
		do {
			...
			interestAccrued = Utils.roundNumber(this.calcInterest(periodicRate, balance, 1));
			netChange = Utils.roundNumber(obj.cf + interestAccrued);
			balance = Utils.roundNumber(balance + interestAccrued + obj.cf);
			scheduledDateStr = DateMath.dateToDateStr(transDate, Globals.dateConventions);
			...
			schedule.push(['', '', Globals.ROW_TYPES.DETAIL, periodYearString, scheduledDateStr, obj.cf, deposit, withdrawal, interestAccrued, null, netChange, balance, transDate.getMonth() + 1, transDate.getFullYear(), transDate, transDate.valueOf()]);
			L += 1;
		} while (L <= obj.n);
	...
	static calc (obj) {
		const PPY = 12;
		var periodicRate, s, fv;
		// most properties of obj are initialized to null when obj is allocated (dates initialized to 0 so objects are allocated)
		// n is either the number of cash flows or days
		// if n is null then periods and oddDays are calculated between obj.oDate and obj.lDate
		// if n is not null and obj.oDate & obj.fDate are not null - possible odd length inital period
		// most properties of obj are initialized to null when obj is allocated (dates initialized to 0 so objects are allocated)
		// n is either the number of cash flows or days
		// if n is null then periods and oddDays are calculated between obj.oDate and obj.lDate
		// if n is not null and obj.oDate & obj.fDate are not null - possible odd length inital period
		...
		// regular periods of the cash flow from fDate
		periodicRate = obj.nominalRate / PPY;
		s = Math.pow(1 + periodicRate, obj.n); // term = number of cash flows
		// fv = (fv * s) + obj.cf * (s - 1.0) * (1.0 + periodicRate * obj.pmtMthd) / periodicRate; // note negative result
		// fv = obj.cf * (s - 1.0) * (1.0 + periodicRate * obj.pmtMthd) / periodicRate; // note negative result
		fv = obj.cf * (s - 1.0) * (1.0 + periodicRate * 0) / periodicRate; // note negative result
		// if (obj.pmtMthd === Globals.PMT_METHOD.ARREARS) {
