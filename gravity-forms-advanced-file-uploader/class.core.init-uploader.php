	...
	*/
	private function gform_update_lead_details( $lead_id = NULL, $form_id = NULL,$field_id = NULL, $value = NULL ) {
		...
		if( isset($lead_id, $form_id, $field_id, $value) ) {
			...
			//Is this new version of Gforms
			if( function_exists('gform_update_meta') ) {
				return $this->gform_update_entry_meta( $lead_id, $value, $form_id );
			}
	...
	private function gform_update_entry_meta( $entry_id, $meta_value, $form_id ) {

		//vars
		$meta_key = 'prso_adv_uploads';
	...
	*/
	public function pluploader_entry_field_value( $value, $field, $entry, $form ) {
		...
		if( isset($field['type']) &&  $field['type'] === 'prso_gform_pluploader' ) {
			...
			//Detect legacy version of gforms
			if( function_exists('gform_get_meta') ) {
				$value = gform_get_meta( $entry['id'], 'prso_adv_uploads' );

				//Fallback, try and get legacy version of data
				if( false === $value ) {
					$value = self::get_gforms_legacy_entry_meta( $entry, $field['id'], $form );
				}
	...
	public static function get_gforms_legacy_entry_meta( $entry, $field_id, $form ) {

		if( false === class_exists('RGFormsModel') ) {
			return null;
		}

		if( false === method_exists('RGFormsModel', 'get_field_value_long') ) {
			return null;
		}

		return RGFormsModel::get_field_value_long( $entry, $field_id, $form );
