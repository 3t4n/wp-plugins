...
add_action('rest_api_init', function () {
    register_rest_route('atoa-pay/v1', '/webhook', array(
        'methods' => 'POST',
        'callback' => 'handle_atoa_webhook',
    ));
});


function handle_atoa_webhook($request) {
    $payload = $request->get_json_params();
    
    // Verify required fields
    if (!isset($payload['paymentRequestId']) || !isset($payload['status'])) {
    ...
    try {
        ...
        $isValid = $gateway->is_valid_signature($payload);
        if ($isValid != 1) return new WP_REST_Response(array(
            'success' => false,
            'message' => 'Invalid webhook payload: Missing required fields'
        ), 400);
        
        // Update order status
        $gateway->update_order_status($payload['paymentRequestId']);
	...
	class Atoa_Pay_Payment_Gateway extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			add_action(
				...
				20  // Priority 20 ensures this runs after process_admin_options (default priority 10)
			);
		}
		...
		public function handle_settings_save() { 
					$api = new API($this->get_option( 'access_secret' ));
        ...
        function is_valid_signature($payload) {
            $api = new API($this->access_secret, $this->testmode);
            return $api->verify_payment_signature($payload["orderId"], $payload["paymentRequestId"], $payload["signatureHash"]);
        }
	}
}
