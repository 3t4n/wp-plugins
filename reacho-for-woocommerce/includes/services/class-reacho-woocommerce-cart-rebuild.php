...
class Reacho_WooCommerce_Cart_Rebuild {
	/**
	 ...
	 */
	public function normalize_normal_product( $item ) {
		return array(
	 ...
	 */
	public function add_encoded_composite( $container_ids, $values ) {
	 ...
	 */
	public function reachowc_build_cart_data( $cart ) {
		$event_data         = array(
		...
		foreach ( $cart->get_cart() as $cart_item_key => $values ) {
			...
			$is_chained_product = $this->is_chained_product( $values );

			if ( class_exists( 'WC_Composite_Products' ) ) {
				$product_encoded    = wp_json_encode( $product );
				...
				if ( $product->get_type() == 'composite' ) {
					...
					foreach ( wc_cp_get_composited_cart_items( $values ) as $key => $val ) {
						$composite_product = $this->add_encoded_composite( $val['composite_data'], $values );
						break;
					}
					array_push( $composite_products, $composite_product );
				} elseif ( ! $is_composite_child && ! $is_chained_product ) {
					$normal_products[ $cart_item_key ] = $this->normalize_normal_product( $values );
				}
			 ...
			 *
			 * The `reachowc_checkout_item` filter allows you to add additional properties to the line items that make up the
			 ...
			 *
			 * add_filter('reachowc_checkout_item','reachowc_modify_checkout_item',1,2);
			 *
			 * function reachowc_modify_checkout_item($line_item, $product) {
			 ...
			 *
			 */
			$event_data['$extra']['Items'][] = apply_filters( 'reachowc_checkout_item', $item, $product );
		...
		$event_data['$extra']['CartRebuildKey'] = base64_encode( wp_json_encode( $wck_cart ) );
