jQuery(document).ready(function ($) {
  ...
  $("#generate_content")
    ...
    .on("click", function () {
      ...
      // var postType = $("input[name='post_type']:checked").val() || "post"; // Default to 'post' if not provided
      ...
      $.ajax({
        ...
        data: {
          ...
          // post_type: postType, // Send post type (post or page)
        },
        success: function (response) {
          if (response.success) {
            const postID = response.data.postId;
            const postType = response.data.postType; // Get post type from response
            ...
            if (postType === "page") {
              window.location.href = `/wp-admin/post.php?post=${postID}&action=edit&post_type=page`;
            } else {
...
document.addEventListener("DOMContentLoaded", function () {
  // Event listener for "View Content" link
  document
    .getElementById("viewContentID")
    .addEventListener("click", function (event) {
      event.preventDefault(); // Prevent default anchor behavior
      window.location.href = this.href; // Redirect to the URL in href
    });

  // Event listener for "Content Calendar" link
  document
    .getElementById("contentCalendarID")
    .addEventListener("click", function (event) {
      event.preventDefault(); // Prevent default anchor behavior
      window.location.href = this.href; // Redirect to the URL in href
    });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  updateButtons.forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      const google_access = this.dataset.googleaccesstoken;
      ...
      document.getElementById("update_keyword").value = keyword;
      document.getElementById("update_google_access_token").value =
        google_access;

      const googleSignInButton = document.getElementById(
        "update_google_signin_button"
      );

      // Update the Google Sign-In button state based on whether an access token exists
      if (google_access) {
        console.log("access ", google_access);
        ...
        fetch(
          ...
          .then((data) => {
            const siteSelect = document.querySelector("#update_site_select");
            console.log("data", data);
            if (data.length > 0) {
              siteSelect.innerHTML =
                '<option value="">Available sites..</option>'; // Reset previous options
              ...
              console.log("Loggeddd out..");
              siteSelect.innerHTML =
                "<option>No sites associated or Expired login, Please sign back in again</option>";
            }

            // Show the site container after fetching the sites
            document.getElementById(
              "update_google_sites_container"
            ).style.display = "block";
          })
          .catch((error) => {
        ...
        googleSignInButton.addEventListener("click", function signOutHandler() {
          // Clear the access token and update the button state
          document.getElementById("update_google_access_token").value = "";
        ...
        `;
          ...
          // Hide the site container when signed out
          document.getElementById(
            "update_google_sites_container"
          ).style.display = "none";

          // Remove this handler after sign-out to prevent duplicate handlers
          googleSignInButton.removeEventListener("click", signOutHandler);
        });
      } else {
        googleSignInButton.innerHTML = `
        <img src="https://developers.google.com/identity/images/g-logo.png" alt="Google Logo" class="google-logo">
      ...
      `;
        ...
        googleSignInButton.addEventListener("click", function signInHandler() {
          // Open Google OAuth window
          const authWindow = window.open(
            "https://ai.1upmedia.com:443/google/auth", // Replace with your actual Google OAuth URL
            "Google Auth",
            "width=600,height=400"
          );
          ...
          window.addEventListener("message", function handleMessage(event) {
            if (event.data.type === "googleAuthSuccess") {
              const accessToken = event.data.accessToken;
              // Set the access token to the hidden input field
              document.getElementById("update_google_access_token").value =
                accessToken;
            ...
            `;
              ...
              // Switch the button to sign-out mode
              googleSignInButton.removeEventListener("click", signInHandler);
              googleSignInButton.addEventListener(
                ...
                `;
                  ...
                  // Remove the sign-out handler after execution to avoid duplication
                  googleSignInButton.removeEventListener(
                    "click",
                    signOutHandler
                  );
                }
            ...
            // Remove this event listener after processing the message
...
jQuery(document).ready(function ($) {
  ...
  $("#generate_automated").on("click", function (e) {
    ...
    var data = {
      ...
      post_type_auto: $("#post_type_auto").val(),
  ...
  $("#generate_super_automated").on("click", function (e) {
    ...
    var data = {
      ...
      post_type_super: $("#post_type_super").val(),
  ...
  $("#generate_industry_automated").on("click", function (e) {
    ...
    var data = {
      ...
      post_type_industry: $("#post_type_industry").val(),
  ...
  $("#fetch-links").on("click", function () {
    // Define query and num for API request (replace with dynamic values if needed)
    // Retrieve dynamic values for query and num
    let query = $("#title").val().trim(); // Get value from #title input and trim whitespace
    let numInput = $("#number_of_pieces").val().trim(); // Get value from #number_of_pieces and trim whitespace

    // Check if either field is empty and alert the user if so
    if (!query || !numInput) {
      alert("Both Title and Number of Pieces fields cannot be empty.");
      return;
    }

    // Parse num as an integer and calculate the final value
    let num = parseInt(numInput) * 3;

    // Validate if num is a positive number
    if (isNaN(num) || num <= 0) {
      alert("Please enter a valid number in the Number of Pieces field.");
      return;
    }
    $("#fetch-links").prop("disabled", true).text("Loading...");

    $.ajax({
      url: "https://ai.1upmedia.com/google-custom-search",
      type: "POST",
      contentType: "application/json",
      dataType: "json",
      data: JSON.stringify({ query: query, num: num }),
      success: function (response) {
        if (response.result && Array.isArray(response.result)) {
          response.result.forEach(function (item) {
            var apiLink = `
                        <div class="link-item_full">
                            <input type="text" name="link_text_full[]" value="${item.title}" placeholder="Anchor Text">
                            <input type="url" name="link_url_full[]" value="${item.link}" placeholder="URL">
                            <button type="button" class="remove-link button">Remove</button>
                        </div>`;
            $("#links_full").append(apiLink);
          });
        } else {
    ...
    $("#fetch-links").prop("disabled", false).text("Fetch Links");
  });
...
jQuery(document).ready(function ($) {
  ...
  $("#find_business_details").on("click", function () {
    ...
    var keywords = $("#create_keyword").val();
...
jQuery(document).ready(function ($) {
  ...
  <div style="background-color: #fff; padding: 20px; border-radius: 5px; margin-bottom: 10px; line-height: 1.4; box-shadow: 0px 2px 8px rgba(0, 0, 0, 0.1);">
    ...
    <div style="margin-top: 15px;">
      ...
      <button id="generate-short-period-content" class="button button-primary" style="margin-bottom: 10px; background-color: #1a51bc;">Generate Short Period Contents </button>
  ...
  $(document).on("click", "#generate-short-period-content", function () {
    loadAuthors(showShortPeriodForm); // Fetch authors and show the form
  });

  function showShortPeriodForm(authors) {
    ...
    var formHtml = `
        <div id="shortPeriodContentForm" style="position: absolute; top: 100px; left: 50%; transform: translateX(-50%); background: white; padding: 20px; border: 1px solid #ccc; z-index: 100;">
            <h3>Generate Short Period Contents</h3>
            <label for="start-date-short">Start Date:</label>
            <input type="date" id="start-date-short" required><br/><br/>
            ...
            <div id="button-group-short">
                <button id="7days-btn-short" class="button button-primary">7 Days</button>
                <button id="14days-btn-short" class="button button-primary">14 Days</button>
                <button id="1month-btn-short" class="button button-primary">1 Month</button>
            </div>
            <button id="close-shortPeriodContentForm" class="button" style="margin-top: 10px;">Close</button>
            <div id="loading-message-short" style="display:none; margin-top:10px; color:blue;">Loading, please wait...</div>
        </div>
    `;

    // Remove any existing form and append the new form to the body
    $("#shortPeriodContentForm").remove();
    $("body").append(formHtml);

    // Event listeners for form buttons with new IDs
    $("#7days-btn-short").on("click", function () {
      submitGoalForm($("#start-date-short").val(), "7");
    });
    $("#14days-btn-short").on("click", function () {
      submitGoalForm($("#start-date-short").val(), "14");
    });
    $("#1month-btn-short").on("click", function () {
      submitGoalForm($("#start-date-short").val(), "30");
    });
    $("#close-shortPeriodContentForm").on("click", function () {
      $("#shortPeriodContentForm").remove();
    });
  }
  ...
  $("#confirm-to-create").on("click", function () {
    var confirmMessage = `
    Once you confirm, all the title-previewed articles will be created based on the generated calendar.
...
`;
  ...
  function showGenerateForm(authors) {
    ...
    //   // First, check if generation is allowed today
    //   $.ajax({
    //     url: myAjax.ajaxurl,
    //     method: "POST",
    //     data: {
    //       action: "check_last_generation_date", // Action to check last generation date
    //       nonce: myAjax.nonce, // Nonce for security (updated to match 'my_generic_action')
    //     },
    //     success: function (response) {
    //       if (response.success) {
    //         // Proceed with generation if allowed
    //         processBatch365(startDate, selectedAuthor, 30, 12); // Start the batch processing for 365 days

    //         // After starting the generation, update the last generation date in WordPress
    //         $.ajax({
    //           url: myAjax.ajaxurl,
    //           method: "POST",
    //           data: {
    //             action: "update_last_generation_date", // Action to update last generation date
    //             nonce: myAjax.nonce, // Nonce for security (updated to match 'my_generic_action')
    //           },
    //         });
    //       } else {
    //         // Error: generation already happened today
    //         alert(response.data.message);
    //       }
    //     },
    //     error: function () {
    //       alert("Error checking last generation date.");
    //     },
    //   });
    // });
...
jQuery(document).ready(function ($) {
  // Function to make the AJAX request to get keywords and target audience
  function fetchKeywordsAndTargetAudience() {
    const author = $("#author").val();
    const goal = $("#goal").val();
    const title = $("#title").val();

    // Validate that all required fields are filled
    if (!author || !goal || !title) {
      alert("Please select an author, goal, and enter a title.");
      return;
    }

    $("#target_audience_from_author_full")
      .prop("disabled", true)
      .text("Loading Target Audience...");
    $("#keywords_from_author_full")
      .prop("disabled", true)
    ...
    $.ajax({
      ...
      dataType: "json",
      data: {
        action: "get_keywords_and_target_audience",
        author: author,
        goal: goal,
        title: title,
        _ajax_nonce: myAjax.nonce,
      ...
      success: function (response) {
        if (response.success) {
          ...
          const targetAudienceTextarea =
            document.getElementById("target_audience");

          keywordsTextarea.value = response.data.keywords;
          targetAudienceTextarea.value = response.data.target_audience;

          // Adjust the height of the keywords textarea
          adjustTextareaHeight(keywordsTextarea);

          // Adjust the height of the target audience textarea
          adjustTextareaHeight(targetAudienceTextarea);
        } else {
          alert("Failed to retrieve data: " + response.data.message);
        }
      },
      ...
      complete: function () {
        ...
        $("#target_audience_from_author_full")
          .prop("disabled", false)
          .text("Get Target Audience from Author");
        $("#keywords_from_author_full")
          .prop("disabled", false)
          .text("Get Keywords from Author");
      },
    });
  }
  ...
  function fetchIdeaFromAuthor() {
    ...
    $("#title_from_author_full").prop("disabled", true).text("Loading Idea...");
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          titleTextarea.value = response.data.title;
      ...
      complete: function () {
        // Re-enable buttons and restore original text
        $("#title_from_author_full")
          .prop("disabled", false)
          .text("Get Idea from Author");
      },
    });
  }
  // Attach the same function to both buttons
  $("#target_audience_from_author_full, #keywords_from_author_full").on(
    "click",
    fetchKeywordsAndTargetAudience
  );

  $("#title_from_author_full").on("click", fetchIdeaFromAuthor);
});

jQuery(document).ready(function ($) {
  // Check if the Google access token is already filled and adjust the button state
  if ($("#create_google_access_token").val()) {
    $("#create_google_signin_button")
      .text("Google Added")
      .prop("disabled", true);
    fetchAndDisplaySites(
      $("#create_google_access_token").val(),
      "#create_site_select",
      "#create_google_sites_container"
    ); // Fetch sites if token exists
  }

  // Function to handle the Google Sign-In
  $("#create_google_signin_button").on("click", function () {
    // Open Google OAuth window
    const authWindow = window.open(
      "https://ai.1upmedia.com:443/google/auth", // Replace with your actual Google OAuth URL
      "Google Auth",
    ...
    window.addEventListener(
      ...
      function (event) {
        if (event.data.type === "googleAuthSuccess") {
          const accessToken = event.data.accessToken;
          // Set the access token to the hidden input field
          $("#create_google_access_token").val(accessToken);

          // Change the button text to "Google Added" and disable it
          $("#create_google_signin_button")
            .text("Google Added")
            .prop("disabled", true);

          // Fetch and display the sites and show the container
          fetchAndDisplaySites(
            accessToken,
            "#create_site_select",
            "#create_google_sites_container"
  ...
  // Fetch and display Google sites
  function fetchAndDisplaySites(accessToken, siteSelectId, containerId) {
    fetch(
      `https://ai.1upmedia.com:443/google/sites?accessToken=${encodeURIComponent(
        accessToken
      )}`
      ...
      .then((data) => {
        ...
        // Show the container that was initially hidden
        $(containerId).show();
      })
      .catch((error) => {
        console.error("Error fetching sites:", error);
...
jQuery(document).ready(function ($) {
  // Function to check the current state of the Google access token and adjust the button
  function updateGoogleButtonState() {
    const googleAccessToken = $("#update_google_access_token").val();
    ...
    if (googleAccessToken) {
      ...
      $("#update_google_signin_button")
        ...
        .prop("disabled", false);
      $("#update_google_sites_container").hide(); // Hide site select on sign-out
    }
  }

  // Call this function to make sure the button state is updated after the input is populated
  updateGoogleButtonState();

  // Handle the click event for the Google button
  $("#update_google_signin_button").on("click", function () {
    const googleAccessToken = $("#update_google_access_token").val();

    if (googleAccessToken) {
      // If there's a token, it means the user is signed in, so perform the sign-out action
      $("#update_google_access_token").val(""); // Clear the access token
      updateGoogleButtonState(); // Update the button state to show "Sign in with Google"
    } else {
      // If there's no token, proceed with the sign-in flow
      const authWindow = window.open(
        "https://ai.1upmedia.com:443/google/auth", // Replace with your actual Google OAuth URL
        "Google Auth",
        "width=600,height=400"
      ...
      window.addEventListener("message", function (event) {
        if (event.data.type === "googleAuthSuccess") {
          const accessToken = event.data.accessToken;
          // Set the access token to the hidden input field
          $("#update_google_access_token").val(accessToken);

          // Update the button state to "Sign out" and fetch sites
          updateGoogleButtonState();
        }
      });
    }
    ...
    fetch(
      ...
      .then((data) => {
        ...
        if (data) {
          ...
          siteSelect.html(
            "<h4>No sites associated or Expired login, Please sign back in again</h4>"
          );
        }
...
document.addEventListener("DOMContentLoaded", function () {
  const searchInputs = [
    {
      search: "update_industry_search",
      results: "update_industry_results",
      select: "update_industry",
    },
    {
      search: "create_industry_search",
      results: "create_industry_results",
      select: "create_industry",
    },
  ];

  searchInputs.forEach((inputConfig) => {
    const searchInput = document.getElementById(inputConfig.search);
    const resultsList = document.getElementById(inputConfig.results);
    const selectElement = document.getElementById(inputConfig.select);

    // Get all the options from the select box
    ...
    function filterOptions(query) {
      ...
      if (matches.length === 0) {
        resultsList.style.display = "none"; // Hide the results if no match
        return;
      }
    ...
    searchInput.addEventListener("input", function () {
      ...
      if (query) {
        filterOptions(query); // Filter options based on the input
      } else {
        resultsList.style.display = "none"; // Hide if input is cleared
      }
    });

    // Event listener for selecting an item from the results
    resultsList.addEventListener("click", function (event) {
      if (event.target.tagName === "LI") {
        ...
        // Select the correct value in the hidden select element
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function switchTab(tabElement) {
    navTabs.forEach(function (tab) {
      tab.classList.remove("nav-tab-active"); // Remove active class from all tabs
    });
    tabElement.classList.add("nav-tab-active"); // Add active class to the clicked tab
    ...
    document.querySelector(tabElement.getAttribute("href")).style.display =
      "block"; // Show selected tab content

    // Store the active tab in session storage
    sessionStorage.setItem("activeTab", tabElement.getAttribute("href"));
  }

  // Function to programmatically redirect to a tab
  window.redirectTo = function (tabSelector) {
    const targetTab = document.querySelector(`.nav-tab[href="${tabSelector}"]`);

    if (targetTab) {
      switchTab(targetTab);
      // Optionally, scroll to the top or adjust focus for better UX
      window.scrollTo({ top: 0, behavior: "smooth" });
    } else {
      console.error(`Tab with selector "${tabSelector}" not found.`);
    }
  };

  // On page load, check if a tab is stored in session storage
  const storedTab = sessionStorage.getItem("activeTab");
  if (storedTab) {
    redirectTo(storedTab); // Redirect to the stored tab
  } else {
    redirectTo("#welcome-screen"); // Default to the welcome screen
  }

  const updateUserForm = document.getElementById("update-user-form"); // Select the form by ID
  if (updateUserForm) {
    updateUserForm.addEventListener("submit", function (e) {
      // e.preventDefault(); // Prevent the default form submission

      // Store the desired tab in sessionStorage
      sessionStorage.setItem("activeTab", "#generate-content");

      // Now manually submit the form after storing the tab
    });
  }
});
