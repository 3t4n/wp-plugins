...
class AI_Content_Pipelines_Oneup_GenerateScheduledContent
{
  ...
  public function __construct()
  {
    ...
    add_action('wp_ajax_get_keywords_and_target_audience', [$this, 'get_keywords_and_target_audience_callback']);
    add_action('wp_ajax_get_idea_from_author', [$this, 'get_idea_from_author']);

  }


  function get_idea_from_author()
  {
    // Check for nonce security
    check_ajax_referer('my_generic_action', '_ajax_nonce');

    // Get data from the AJAX request
    $author_id = sanitize_text_field($_POST['author']);

    // Fetch author details (e.g., business details)
    $business_details = get_user_meta($author_id, '_assigned_business_detail', true);
    if (!$business_details) {
      wp_send_json_error(['message' => 'Business details not found for the selected author. Please add one under "Business Setup" tab']);
    }
    ...
    // Make an API request to get keywords and target audience
    $response = wp_remote_post('https://ai.1upmedia.com:443/get-idea-from-author', [
      'method' => 'POST',
      'headers' => [
    ...
    if (isset($data['result']['title'])) {
      wp_send_json_success([
        'title' => sanitize_text_field($data['result']['title'])
      ]);
    } else {
  ...
  function get_keywords_and_target_audience_callback()
  {
    // Check for nonce security
    check_ajax_referer('my_generic_action', '_ajax_nonce');
    ...
    // Make an API request to get keywords and target audience
    $response = wp_remote_post('https://ai.1upmedia.com:443/get-keywords-and-target-audience', [
      'method' => 'POST',
      'headers' => [
        'Content-Type' => 'application/json',
      ],
    ...
    // Check if the API returned a valid response
    if (isset($data['result']['keywords']) && isset($data['result']['target_audience'])) {
      wp_send_json_success([
        'keywords' => sanitize_text_field($data['result']['keywords']),
        'target_audience' => sanitize_text_field($data['result']['target_audience']),
      ]);
    } else {
      wp_send_json_error(['message' => 'Invalid response from API.']);
    }
    ...
    try {
      ...
      for ($i = 0; $i < $number_of_pieces; $i++) {
        ...
        try {
          ...
          $prompt = $this->build_full_workflow_prompt($title, $selected_goal, $target_audience, $keywords, $selected_search_intent, $links, $word_count, $tone, $selected_content_type, $content_strategy, $content_type_template, $language, $author_name);
          ...
          $this->updateProgressToPoll("Generating title $i of $number_of_pieces...");
          $tags = $this->get_tags_from_prompt($content);
          $post_id = $this->schedule_content($content, $scheduled_time, $generated_title, $category, $image_info, $post_status, $author_id, $selected_content_type, $tags, $admin_email, $post_type);
      ...
      while (!empty($failed_contents) && $retries > 0) {
        foreach ($failed_contents as $key => $failed_content) {
          try {
            ...
            $image_info = $this->generate_image_from_prompt($generated_title);
            $tags = $this->get_tags_from_prompt($content);
            $post_id = $this->schedule_content($content, $failed_content['scheduled_time'], $generated_title, $category, $image_info, $failed_content['post_status'], $failed_content['author'], $selected_content_type, $tags, $admin_email, $post_type);
  ...
  public function generate_automated_content()
  {
    ...
    try {
      ...
      $post_type = isset($_POST['post_type_auto']) ? sanitize_text_field(wp_unslash($_POST['post_type_auto'])) : 'post';
      ...
      for ($i = 0; $i < $number_of_pieces; $i++) {
        ...
        try {
          ...
          $prompt = $this->build_automated_prompt($topic, $selected_template_style, $links, $selected_goal, $selected_search_intent, $content_strategy, $content_type_template, $language, $author_id);
          $content = $this->generate_content_from_prompt($prompt);
          ...
          $image_info = $this->generate_image_from_prompt($generated_title);
          $tags = $this->get_tags_from_prompt($content);
          $post_id = $this->schedule_content($content, $scheduled_time, $generated_title, $category, $image_info, $post_status, $author_id, $selected_template_style, $tags, $admin_email, $post_type);
      ...
      for ($j = 0; $j < $retries; $j++) {
        foreach ($failed_contents as $key => $failed_content) {
          try {
            $links = $this->sanitize_links($link_texts, $link_urls);
            $prompt = $this->build_automated_prompt($failed_content['topic'], $failed_content['template_style'], $links, $failed_content['selected_goal'], $failed_content['selected_search_intent'], $content_strategy, $language, $author_name);
            $content = $this->generate_content_from_prompt($prompt);
            $category = $this->generate_category_from_prompt($content);
            $generated_title = $this->generate_title_from_prompt($content, $generated_titles, $content_strategy, $language);
            $generated_titles[] = $generated_title;
            $image_info = $this->generate_image_from_prompt($generated_title);
            $tags = $this->get_tags_from_prompt($content);
            $post_id = $this->schedule_content($content, $failed_content['scheduled_time'], $generated_title, $category, $image_info, $post_status, $author_id, $selected_template_style, $tags, $admin_email, $post_type);
  ...
  public function generate_super_automated_content()
  {
    ...
    try {
      ...
      $post_type = isset($_POST['post_type_super']) ? sanitize_text_field(wp_unslash($_POST['post_type_super'])) : '';
      ...
      for ($i = 0; $i < $number_of_pieces; $i++) {
        ...
        try {
          ...
          $this->updateProgressToPoll("Generating Content {$j}...");
          $links = $this->sanitize_links($link_texts, $link_urls);
          $prompt = $this->build_super_automated_prompt($topic, $current_journey_stage, $links, $generated_titles, $selected_goal, $selected_search_intent, $content_type_template, $language, $author_name);
          $content = $this->generate_content_from_prompt($prompt);
          $this->updateProgressToPoll("fetching categories for content {$j}...");
          $category = $this->generate_category_from_prompt($content);
          $this->updateProgressToPoll("fetching title for content {$j}...");
          ...
          $image_info = $this->generate_image_from_prompt($generated_title);
          $tags = $this->get_tags_from_prompt($content);
          $post_id = $this->schedule_content($content, $scheduled_time, $generated_title, $category, $image_info, $post_status, $author_id, $current_journey_stage, $tags, $admin_email, $post_type);
      ...
      for ($j = 0; $j < $retries; $j++) {
        foreach ($failed_contents as $key => $failed_content) {
          try {
            $links = $this->sanitize_links($link_texts, $link_urls);
            $prompt = $this->build_super_automated_prompt($failed_content['topic'], $failed_content['buyers_journey'], $links, $generated_titles, $failed_content['selected_goal'], $failed_content['selected_goal'], $failed_content['content_type_template'], $language, $author_name);
            $content = $this->generate_content_from_prompt($prompt);
            $category = $this->generate_category_from_prompt($content);
            ...
            $image_info = $this->generate_image_from_prompt($generated_title);
            $tags = $this->get_tags_from_prompt($content);
            $post_id = $this->schedule_content($content, $failed_content['scheduled_time'], $generated_title, $category, $image_info, $post_status, $author_id, $failed_content['buyers_journey'], $tags, $admin_email, $post_type);
  ...
  public function generate_industry_automated_content()
  {
    ...
    try {
      ...
      $post_type = isset($_POST['post_type_industry']) ? sanitize_text_field(wp_unslash($_POST['post_type_industry'])) : 'post';
      ...
      for ($i = 0; $i < $number_of_pieces; $i++) {
        ...
        try {
          ...
          $this->updateProgressToPoll("Generating Content {$j}...");
          $links = $this->sanitize_links($link_texts, $link_urls);
          $prompt = $this->build_super_automated_prompt($topic . "Industry: " . $user_industry . 'Location: ' . $user_location . " IMPORTANT! -> Please generate the content in" . $language . "Language.", $current_journey_stage, $links, $generated_titles, $selected_goal, $selected_search_intent, $content_type_template, $author_name);
      ...
      for ($j = 0; $j < $retries; $j++) {
        foreach ($failed_contents as $key => $failed_content) {
          try {
            $links = $this->sanitize_links($link_texts, $link_urls);
            $prompt = $this->build_super_automated_prompt($failed_content['topic'], $failed_content['buyers_journey'], $links, $generated_titles, $failed_content['selected_goal'], $failed_content['selected_goal'], $failed_content['content_type_template'], $author_name);
            $content = $this->generate_content_from_prompt($prompt);
            $category = $this->generate_category_from_prompt($content);
            $generated_title = $this->generate_title_from_prompt($content, $generated_titles, $failed_content['buyers_journey'], $language);
            $generated_titles[] = $generated_title;
  ...
  private function build_full_workflow_prompt($title, $goal, $target_audience, $keywords, $search_intent, $links, $word_count, $tone, $content_type, $content_strategy, $content_type_prompt_template = "", $language = "English", $author_name = "Edward Livingston")
  {
  ...
  private function build_super_automated_prompt($topic, $buyers_journey, $links, $generated_titles = [], $goal = '', $search_intent = '', $content_type_template = '', $language = "English", $author_name = "Hilbert Andrews")
  {
  ...
  private function schedule_content($content, $scheduled_time, $title, $categories, $image_info, $post_status, $author, $template_style, $tags, $admin_email = '', $post_type = 'post')
  {
  ...
  function generate_full_workflow_content_with_previewed_titles()
  {
    ...
    try {
      ...
      foreach ($titles_to_generate as $index => $generated_title) {
        ...
        try {
          $content_type_template = isset($template_styles[$selected_content_type]) ? $template_styles[$selected_content_type] : '';
          $prompt = $this->build_titled_workflow_prompt($generated_title, $selected_goal, $target_audience, $keywords, $selected_search_intent, $links, $word_count, $tone, $selected_content_type, $content_strategy, $content_type_template, $language, $author_name);
  ...
  private function build_titled_workflow_prompt($title, $goal, $target_audience, $keywords, $search_intent, $links, $word_count, $tone, $content_type, $content_strategy, $content_type_prompt_template = "", $language = "English", $author_name)
  {
    ...
    $prompt .= "Tone: " . $tone . "\n";
    $prompt .= "Content Strategy: " . $content_strategy . "\n";
    $prompt .= "Author/Interviewer Name: " . $author_name . "\n";
