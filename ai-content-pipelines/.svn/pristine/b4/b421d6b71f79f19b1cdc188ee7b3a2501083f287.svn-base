 ...
 * Description: Effortlessly manage your content lifecycle with our engine, which scans your site to automate workflows from ideation to distribution. Pipeline automation ensures each stage runs smoothly, producing consistent, high-quality content that enhances visibility and engagement across your channels.
 * Version: 1.6
 * Author: 1UP Media
...
class AI_Content_Pipelines_Oneup_AuthorPersonas
{
        ...
        <div class="container mt-5">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <!-- Google Sign-In or Sites -->
                    <div id="google_sign_in_or_sites">
                        <!-- This container will either show the Sign-In button or the Site Dropdown -->
                        <div id="google_signin_container" style="display:none;">
                            <!-- Google Sign-In Button -->
                            <button type="button" id="google_signin_button" class="google-signin-button">
                        ...
                        <div id="gsc_list_sites_container" style="display:none;" class="mb-4">
                    ...
                    <!-- Results Display -->
                    <div id="comparison_results" class="mt-5" style="display:none;">
                    ...
                    <input type="hidden" id="ac_google_access_token" name="google_access_token" value="">
                </div>
            </div>
        </div>
        <?php
    ...
    function render_google_search_console_page()
    {
        // Fetch all WordPress users
        $users = get_users();
        ?>
        <div class="wrap">
            ...
            <div id="user_selection_container">
                <h3>Select a User</h3>
                <select id="user_select" class="gsc-styled-select">
                    <option value="">Select a user</option>
                    <?php foreach ($users as $user): ?>
                        <option value="<?php echo esc_attr($user->ID); ?>">
                            <?php echo esc_html($user->display_name); ?>
            ...
            <!-- Google Sign-In or Available Sites -->
            <div id="gsc_sign_in_or_sites" style="display:none;">
                <div id="gsc_google_signin_container" style="display:none;">
                    <!-- Google Sign-In Button -->
                    <button type="button" id="gsc_google_signin_button" class="google-signin-button">
                        <img src="https://developers.google.com/identity/images/g-logo.png" alt="Google Logo"
                            class="google-logo">
                        Sign in with Google
                    </button>
    ...
    public function ai_content_pipelines_oneup_enqueue_admin_scripts()
    {
        ...
        // Enqueue Select2 JS and CSS from CDN
    ...
    public function generate_content()
    {
        ...
        $post_type = $post_id ? get_post_type($post_id) : (isset($_POST['post_type']) ? sanitize_text_field($_POST['post_type']) : 'post');
        // $post_type = isset($_POST['post_type']) ? sanitize_text_field(wp_unslash($_POST['post_type'])) : 'post'; // Get post type
        $this->updateProgressToPoll("Generating content for " . $post_type);
        ...
        $html_content = preg_replace('/<style\b[^>]*>[\s\S]*?<\/style>/', '', $html_content);
    ...
    public function user_profile_fields($user)
    {
        ...
        < class="form-table">
            ...
            <tr>
                <th><label for="author_business_detail"><?php esc_html_e('business_detail', 'ai-content-pipelines'); ?></label>
                </th>
                <td>
...
function AI_Content_Pipelines_Oneup_AuthorPersonas()
{
    ?>
        <div class="ai-content-pipelines-widget">
            ...
            <p>
                <a href="<?php echo esc_url(admin_url('admin.php?page=author-personas')); ?>" class="button button-primary">
                    Generate Pipelines
                </a>
            </p>
        </div>
...
function fetch_business_details()
{
    ...
    $response = wp_remote_post($api_url, array(
        'body' => json_encode(['url' => $url, 'location' => $location, 'keyword' => $keywords]),
        'headers' => array(
            'Content-Type' => 'application/json'
        ),
        'timeout' => 45
...
add_action('wp_ajax_check_google_access_token', 'check_google_access_token');
function check_google_access_token()
{
    ...
    $access_token = get_user_meta($user_id, '_assigned_google_access_token', true);
    if ($access_token) {
...
add_action('wp_ajax_save_google_access_token', 'save_google_access_token');
function save_google_access_token()
{
    check_ajax_referer('my_generic_action', '_ajax_nonce');
...
add_action('wp_ajax_remove_google_access_token', 'remove_google_access_token');
function remove_google_access_token()
{
    if (!isset($_POST['user_id']) || !isset($_POST['_ajax_nonce']) || !wp_verify_nonce($_POST['_ajax_nonce'], 'my_generic_action')) {
        wp_send_json_error(['message' => 'Invalid nonce or parameters.']);
    }

    $user_id = intval($_POST['user_id']);

    // Remove the access token from user meta
    if (delete_user_meta($user_id, '_assigned_google_access_token')) {
        wp_send_json_success(['message' => 'Access token removed successfully.']);
    } else {
        wp_send_json_error(['message' => 'Failed to remove access token.']);
    }
}


// Check last generation date
add_action('wp_ajax_check_last_generation_date', 'check_last_generation_date');
function check_last_generation_date()
{
    check_ajax_referer('my_generic_action', 'nonce'); // Verify nonce for security

    $last_generation_date = get_option('last_365_content_generation_date');
    $today = current_time('Y-m-d');

    if ($last_generation_date === $today) {
        wp_send_json_error(['message' => '365 content preview can only be generated once per day.']);
    } else {
        wp_send_json_success();
    }
}

// Update the last generation date after successful generation
add_action('wp_ajax_update_last_generation_date', 'update_last_generation_date');
function update_last_generation_date()
{
    check_ajax_referer('my_generic_action', 'nonce'); // Verify nonce for security

    $today = current_time('Y-m-d');
    update_option('last_365_content_generation_date', $today);
