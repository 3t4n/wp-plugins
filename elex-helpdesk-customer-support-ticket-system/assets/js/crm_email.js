jQuery(
	function () {
	jQuery( "#email_support_tab" ).on(
		"change",
		"#auto_send_creation_email",
		function (e) {
		e.preventDefault();
	...
	jQuery( "#imap_setup_tab" ).on(
		...
		var username     = jQuery( "#imap_username" ).val();
		var charset      = jQuery( "#charset" ).val();
		var delete_email = '';
				...
				jQuery.ajax(
					{
						...
						success: function (data) {
							...
							if (parse.status == 'success') {
								...
								jQuery('#activate_imap').prop('disabled', false)
								window.setTimeout(
									function () {
									jQuery( ".alert-danger" ).fadeTo( 500, 0 ).slideUp(
										500,
										function () {
										jQuery( this ).css( "display", "none" );
										}
		...
		jQuery.ajax(
			{
				...
				success: function (data) {
					...
					jQuery( "#oauth_setup_tab" ).html( data.data.page );
				},
				error: function (jqXHR, textStatus, errorThrown) {
					console.log( textStatus, errorThrown );
				}
		...
		jQuery.ajax(
			{
				...
				success: function (data) {
					...
					jQuery( "#imap_setup_tab" ).html( data.data.page );
				},
				error: function (jqXHR, textStatus, errorThrown) {
					console.log( textStatus, errorThrown );
				}
	...
	jQuery( "#filter_block_tab" ).on(
		"click",
		"#save_email_filter_block",
		function (e) {
		e.preventDefault();
		var new_block = {};
		...
		jQuery.ajax(
			{
				...
				success: function (data) {
					...
					jQuery( "#filter_block_tab" ).html( data.data.page );
				},
				error: function (jqXHR, textStatus, errorThrown) {
					console.log( textStatus, errorThrown );
				}
	...
	jQuery( "#filter_block_tab" ).on(
		"click",
		"#block_subject_cancel_add_button",
		function (e) {
		e.preventDefault();
		...
		function getValue_checkbox_values(name) {
			var chkArray = [];
			jQuery( "input[name='" + name + "']:checked" ).each(
				function () {
