...
jQuery(
	...
	jQuery( '#tickets_page_view' ).on(
		'click',
		'.tickets_panel span.filter',
		function (e) {
...
function trigger_load_single_ticket(ticket_id)
{
	...
	jQuery( '#reply_textarea_' + ticket_id + ' > .ql-editor' ).on(
		'blur',
		function() {
...
jQuery(
		...
		jQuery.ajax(
			{
				...
				success: function (data) {
					...
					jQuery( "#template_multiple_actions li:nth-child(2" ).after( data.data.page );
	...
	jQuery( document ).on(
		...
		".all_ticket_ids",
		function(){
		jQuery( ".merge_ticket_verify" ).prop( "disabled", false );
		jQuery( ".merge_ticket_confirm" ).prop( "disabled", true );
		}
	);
	jQuery( document ).on(
		"click",
		".merge_ticket_confirm",
		function(){
		var parent_id  = jQuery(this).data( 'ticket_id' );
	...
	jQuery( document ).on(
		...
		var ticket_ids = jQuery( "#all_ticket_ids_" + parent_id ).val();
		...
		jQuery.ajax(
				...
				success: function(data)
			{
					...
					jQuery( ".merge_ticket_confirm" ).prop( "disabled", false );
					jQuery( ".verify" ).html( data.data.page );
				},
				error: function (jqXHR, textStatus, errorThrown) {
					console.log( textStatus, errorThrown );
				}
	...
	jQuery( "#edit_tickets_modal" ).on(
		'click',
		'#bulk_edit_save',
		function(e){
		jQuery( "#bulk_edit_save" ).prop( "disabled", true );
		...
		fd.append( "tags",tags );
		fd.append( "action", "eh_crm_bulk_edit_save" );
		fd.append( "nonce", js_obj.nonce );
		fd.append( "params", JSON.stringify( getTicketsFilter() ) );
		jQuery.ajax(
			{
				type: 'POST',
				url: ajaxurl,
				...
				success: function (data) {
					...
					jQuery( '#bulk_edit_save' ).removeAttr( "disabled" );
					refresh_left_bar();
					refresh_right_bar();
					jQuery( ".alert-success" ).css( "display", "block" );
					jQuery( ".alert-success" ).css( "opacity", "1" );
		...
		jQuery.ajax(
			{
				...
				success: function (data) {
					jQuery( '#update_template_model_display' ).html( data.data.page );
					new Quill(
			...
			if (str !== "") {
				jQuery.ajax(
						...
						success: function (data)
					{
							...
							jQuery( "#template_multiple_actions_single_" + id + " li:nth-child(2" ).after( data.data.page );
				...
				jQuery.ajax(
					{
						...
						success: function (data) {
							...
							var editor = document.querySelector( '#reply_textarea_' + id ).__quill;
							editor.clipboard.dangerouslyPasteHTML( editor.getSelection( true ).index, data.data.page );
	...
	jQuery(document).on(
		"click",
		".ticket_template_confirm_action",
		function (e) {
		e.preventDefault();
		var based    = jQuery( this ).attr( "based" );
					...
					jQuery.ajax(
						{
							...
							data: {
								...
								params      : getTicketsFilter( {} ),
							},
							success: function (data) {
								var response = jQuery.parseJSON( data );
								jQuery( ".table_loader" ).css( "display", "none" );
								jQuery( '#preview_template_model' ).modal( 'hide' );
								jQuery( '#preview_template_model' ).on(
									'hidden.bs.modal',
									function (e) {
									jQuery.each(
										response,
	...
	jQuery( '#search_ticket_input' ).keypress(
		function(e){
			if (e.which == 13) {
				...
				jQuery.ajax(
					{
						...
						success: function (data) {
							...
							if (parse.data === "ticket") {
								...
								if (jQuery( ".elaborate > li#tab_" + parse.search_key ).length == 0) {
									var tab_head    = '<li role="presentation" class="visible_tab" id="tab_' + parse.search_key + '" style="min-width:200px;">' + parse.tab_head + '</li>';
									var tab_content = '<div class="tab-pane" id="tab_content_' + parse.search_key + '">' + parse.tab_content + '</div>';
									jQuery( '.elaborate > li' ).last().before( tab_head );
									jQuery( '.tab-content' ).append( tab_content );
									jQuery( '.visible_tab a#tab_content_a_' + parse.search_key ).click();
									collapse_tab();
								} else {
									jQuery( '.visible_tab a#tab_content_a_' + parse.search_key ).click();
								}
	...
	jQuery( ".nav-tabs" ).on(
		...
		var wsdesk_mode = jQuery( "#wsdesk_setup_mode" ).val();
			if (wsdesk_mode != 'enable') {
				refresh_left_bar();
				refresh_right_bar();
			}
		...
		function refresh_left_bar()
	{
			...
			jQuery( ".ticket_select_all" ).prop( "checked", false );
			...
			jQuery.ajax(
				{
					...
					success: function (data) {
						...
						jQuery( "#left_bar_all_tickets" ).html( data.data.page );
			...
			jQuery.ajax(
				{
					...
					success: function (data) {
						...
						jQuery( "#right_bar_all_tickets" ).html( data.data.page );
						jQuery( '[data-action="filter"]' ).filterTable();
						jQuery( ".direct_reply_textarea" ).each(
							function(){
							var id = jQuery( this ).prop( "id" );
							new Quill(
								'#' + id,
	...
	jQuery( ".tab-content" ).on(
		"change",
		"#dev-table-action-bar .ticket_select_all",
		function (e)
		{
			if (this.checked) {
				jQuery( ".ticket-delete-btn" ).show();
				jQuery( ".ticket-edit-btn" ).show();
				jQuery( '.ticket_select' ).each(
					function () {
						this.checked = true;
					}
				);
		...
		jQuery.ajax(
			{
				...
				success: function (data) {
					jQuery( ".table_loader" ).css( "display", "none" );
					if (jQuery( "#tab_" + ticket_id ).length != 0 && jQuery( "#tab_content_" + ticket_id ).length != 0) {
						jQuery( "#tab_content_" + ticket_id ).html( data.data.page );
						trigger_load_single_ticket( ticket_id );
					}
					refresh_left_bar();
					refresh_right_bar();
				...
				success: function (data) {
					...
					if (jQuery( "#tab_" + ticket_id ).length != 0 && jQuery( "#tab_content_" + ticket_id ).length != 0) {
						...
						trigger_load_single_ticket( ticket_id );
									...
									jQuery( ".alert-success" ).css( "display", "block" );
									jQuery( ".alert-success" ).css( "opacity", "1" );
									jQuery( "#success_alert_text" ).html( "<strong>" + js_obj.Tickets_Updated_Successfully + "</strong><br>Ticket #" + ticket_id );
					window.setTimeout(
						function () {
						jQuery( ".alert-success" ).fadeTo( 500, 0 ).slideUp(
							500,
		...
		jQuery( "#all_ticket_ids_" + ticket_id ).select2(
			{
				ajax: {
					...
					data: function (params) {
						params.action    = 'eh_crm_v2_tickets';
						params.page      = params.page || 1;
						params.length    = 25;
						params.ticket_id = params.term;
					...
					processResults: function (data, params) {
						...
						return {
							results: data.data.map(
								function (ticket) {
								return {id: ticket.ticket_id, text: ticket.ticket_id};
								}
							),
						pagination: {
							more: (params.page * (params.length || 10)) < data.recordsFiltered
							}
						};
					},
				},
	...
	jQuery( ".tab-content" ).on(
		...
		BootstrapDialog.show(
			{
				title: js_obj.WSDesk_Alert,
				message: js_obj.Do_You_want_to_Archive_Ticket,
				cssClass: 'wsdesk_wrapper',
				buttons: [{
					label: js_obj.Yes_archive,
					// no title as it is optional
					cssClass: 'btn-primary',
					action: function (dialogItself) {
						...
						jQuery.ajax(
							{
								...
								url: ajaxurl,
								data: {
									action: 'eh_crm_archive_single_ticket',
									nonce: js_obj.nonce,
									ticket_id: ticket_id
								},
								success: function (data) {
									...
									jQuery( ".alert-success" ).css( "display", "block" );
									jQuery( ".alert-success" ).css( "opacity", "1" );
									jQuery( "#success_alert_text" ).html( "<strong>" + js_obj.Tickets_Archived_Successfully + "</strong><br>Ticket #" + ticket_id );
									window.setTimeout(
										function () {
										jQuery( ".alert-success" ).fadeTo( 500, 0 ).slideUp(
											500,
									...
									jQuery.ajax(
										{
											...
											data: {
												...
												selectAll: dtFilter ? dtFilter.selectAll : false,
											},
											success: function (data) {
												jQuery( ".ticket-delete-btn" ).hide();
												jQuery( ".ticket-edit-btn" ).hide();
		...
		var title     = jQuery( '#ticket_title_' + ticket_id ).val();
			if (text !== "<br>" && text !== "" && email != "" && title != "") {
				...
				if (jQuery( ".ticket_input_text_" + ticket_id ).length != 0) {
					...
					jQuery( ".ticket_input_text_" + ticket_id ).each(
						function () {
				...
				if (jQuery( ".ticket_input_ip_" + ticket_id ).length != 0) {
					jQuery( ".ticket_input_ip_" + ticket_id ).each(
						function () {
						var key          = jQuery( this ).prop( "id" );
						var value        = jQuery( this ).val();
						input_field[key] = value;
				...
				if (jQuery( ".ticket_input_date_" + ticket_id ).length != 0) {
					var text_validate = true;
					jQuery( ".ticket_input_date_" + ticket_id ).each(
						function () {
						var key = jQuery( this ).prop( "id" );
							if (jQuery( this ).hasClass( 'text_required' ) && jQuery( this ).val() === "") {
								jQuery( this ).css( 'border','1px solid red' );
				...
				if (jQuery( ".ticket_input_email_" + ticket_id ).length != 0) {
					var text_validate = true;
					jQuery( ".ticket_input_email_" + ticket_id ).each(
						function () {
						var key = jQuery( this ).prop( "id" );
							if (jQuery( this ).hasClass( 'text_required' ) && jQuery( this ).val() === "") {
								jQuery( this ).css( 'border','1px solid red' );
				...
				if (jQuery( ".ticket_input_number_" + ticket_id ).length != 0) {
					var text_validate = true;
					jQuery( ".ticket_input_number_" + ticket_id ).each(
						function () {
						var key = jQuery( this ).prop( "id" );
							if (jQuery( this ).hasClass( 'text_required' ) && jQuery( this ).val() === "") {
								jQuery( this ).css( 'border','1px solid red' );
				...
				if (jQuery( ".ticket_input_pwd_" + ticket_id ).length != 0) {
					var text_validate = true;
					jQuery( ".ticket_input_pwd_" + ticket_id ).each(
						function () {
						var key = jQuery( this ).prop( "id" );
							if (jQuery( this ).hasClass( 'text_required' ) && jQuery( this ).val() === "") {
								jQuery( this ).css( 'border','1px solid red' );
				...
				if (jQuery( ".ticket_input_select_" + ticket_id ).length != 0) {
					var text_validate = true;
					jQuery( ".ticket_input_select_" + ticket_id ).each(
						function () {
						var key = jQuery( this ).prop( "id" );
							if (jQuery( this ).hasClass( 'text_required' ) && jQuery( this ).val() === "") {
								jQuery( this ).css( 'border','1px solid red' );
				...
				if (jQuery( ".ticket_input_radio_" + ticket_id ).length != 0) {
					...
					jQuery( ".ticket_input_radio_" + ticket_id ).each(
						function () {
						var key   = jQuery( this ).prop( "id" );
						var value = ((jQuery( "input[name='" + key + "']:checked" ).val() != undefined) ? jQuery( "input[name='" + key + "']:checked" ).val() : "");
							if (jQuery( this ).hasClass( 'radio_required' ) && value == "") {
								...
								input_field[key.replace('_'+ticket_id, '')] = value;
							}
						}
					);
					if ( ! radio_validate) {
				...
				if (jQuery( ".ticket_input_checkbox_" + ticket_id ).length != 0) {
					...
					jQuery( ".ticket_input_checkbox_" + ticket_id ).each(
						function () {
				...
				if (jQuery( ".ticket_input_textarea_" + ticket_id ).length != 0) {
					var text_validate = true;
					jQuery( ".ticket_input_textarea_" + ticket_id ).each(
						function () {
						var key = jQuery( this ).prop( "id" );
							if (jQuery( this ).hasClass( 'text_required' ) && jQuery( this ).val() === "") {
								jQuery( this ).css( 'border','1px solid red' );
				...
				window.setTimeout(
					function () {
					jQuery( ".alert-warning" ).fadeTo( 500, 0 ).slideUp(
						500,
						function () {
						jQuery( this ).css( "display", "none" );
						}
				...
				jQuery.ajax(
					{
						...
						success: function (data) {
							...
							if (parse['result'] == false) {
								jQuery( ".alert-warning" ).css( "display", "block" );
								jQuery( ".alert-warning" ).css( "opacity", "1" );
								jQuery( "#warning_alert_text" ).html( "<strong>" + parse['msg'] + "</strong>!" );
								window.setTimeout(
									function () {
									jQuery( ".alert-warning" ).fadeTo( 500, 0 ).slideUp(
										500,
								...
								jQuery.ajax(
									{
										...
										success: function (data) {
											...
											jQuery( ".ticket_reply_action_button_" + ticket_id ).prop( 'disabled', false );
										},
										error: function (jqXHR, textStatus, errorThrown) {
											console.log( textStatus, errorThrown );
										}
	...
	jQuery( ".tab-content" ).on(
		"click",
		".ticket_action_save_props",
		function (e) {
		e.preventDefault();
		const btn_rect = this.getBoundingClientRect();
		const is_manual_click = e.clientX > btn_rect.x && e.clientY > btn_rect.y;
		var ticket_id = jQuery( this ).prop( "id" );
		var assignee  = jQuery( "#assignee_ticket_" + ticket_id ).val();
		var tags      = jQuery( "#tags_ticket_" + ticket_id ).val();
		var cc        = [];
		...
		var input_field = {};
			...
			if (jQuery( ".ticket_input_checkbox_" + ticket_id ).length != 0) {
				...
				jQuery( ".ticket_input_checkbox_" + ticket_id ).each(
					...
					var key   = jQuery( this ).prop( "id" );
					var value = [];
					jQuery( ".ticket_input_checkbox_"+ ticket_id +":checked" ).each(
						function () {
							value.push( jQuery( this ).val() );
						}
					);
						if (jQuery( this ).hasClass( 'check_required' ) && value.length === 0 ) {
							jQuery( this ).css( 'border','1px solid red' );
							check_validate = false;
						} else {
							jQuery( this ).css( 'border','1px solid #ccc' );
			...
			if ( jQuery( ".pfs_order_product_" + ticket_id ).length != 0 ) {
				var ticket_order_id = jQuery( ".pfs_order_product_" + ticket_id ).val();
			}
		...
		jQuery.ajax(
			{
				...
				data: {
					...
					ticket_order_id: (ticket_order_id !== null) ? ticket_order_id : '',
					input: JSON.stringify( input_field ),
					load_tab: is_manual_click,
				},
				success: function (data) {
					jQuery( ".ticket_loader_" + ticket_id ).css( "display", "none" );
					...
					if (is_manual_click) {
						...
						jQuery( "#tab_" + ticket_id ).html( data.tab_head );
						jQuery( "#tab_content_" + ticket_id ).html( data.tab_content );
						trigger_load_single_ticket( ticket_id );

						jQuery( ".alert-success" ).css( "display", "block" );
						jQuery( ".alert-success" ).css( "opacity", "1" );
						jQuery( "#success_alert_text" ).html( "<strong>" + js_obj.Updated_and_Saved_Successfully + "</strong>" );
						window.setTimeout(
							function () {
							jQuery( ".alert-success" ).fadeTo( 500, 0 ).slideUp(
								500,
	...
	jQuery( ".tab-content" ).on(
		...
		function getValue_checkbox(id) {
			var chkArray = [];
			var table    = jQuery( '#all_tickets_table_v2' );
			if (table.length) {
				table.DataTable().rows( {selected: true} ).every(
					function (row) {
					chkArray.push( table.DataTable().row( row ).data().ticket_id );
					}
				);
			} else {
				jQuery( "#" + id + ":checked" ).each(
					function () {
		...
		function previewFiles(files, id) {
			...
			function readAndPreview(file) {
				...
				if (/\.(jpe?g|png|gif|3gp|avi|wmv|mpg|mov|flv)$/i.test( file.name )) {
					...
					if (/\.(doc?x|ppt?x|pdf|xml|csv|xls|xlsx|txt|zip|mp3|mp4|syx|rtf|cdr|bmp)$/i.test( file.name )) {
...
function collapse_tab()
{
	var TAB = {
		...
		reFlow: function () {
			...
			tab_wrapper.find( '>li.visible_tab:not(li.dropdown)' ).each(
				function () {
			...
			success: function (data) {
				...
				if (parse.data === "ticket") {
					...
					if (jQuery( ".elaborate > li#tab_" + ticket_id ).length == 0) {
						...
						jQuery( '.tab-content' ).append( tab_content );
						trigger_load_single_ticket( ticket_id );
						jQuery( "reply_textarea_" + title_ticket_edit+ ticket_id + ' > .ql-editor' ).html( jQuery( 'direct_reply_textarea_' + ticket_id + ' > .ql-editor' ).html() );
						jQuery( '.visible_tab a#tab_content_a_' + ticket_id ).click();
						collapse_tab();
					} else {
						jQuery( ".elaborate > li#tab_" + ticket_id ).children( 'a' ).click();
