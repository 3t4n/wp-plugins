...
class EH_CRM_Import_Tickets {
	protected $user_id;
	protected $plan;
	public function zendesk_get_ticket( $page, $attachment, $plan ) {
		$this->plan = $plan;
	...
	public function zendesk_get_comments( $client, $page, $ticket ) {
		$comments = $client->tickets( $ticket )->comments()->findAll( array( 'page' => $page ) );
		eh_crm_write_log( 'Comments Fetching' );
		return $comments;
	}

	public function zendesk_parse_url( $url ) {
		$parts = parse_url( $url );
		parse_str( $parts['query'], $query );
		return $query['page'];
	}

	public function zendesk_user_email( $client, $requester_id ) {
		$email = '';
	...
	public function zendesk_user_id( $email ) {
