...
class TicketRepository {
	...
	const TABLE_TICKETS      = 'wsdesk_tickets';
	const TABLE_TICKETS_META = 'wsdesk_ticketsmeta';
	...
	public function query() {
		return wpFluent()->table( self::TABLE_TICKETS )->where( 'ticket_parent', 0 )->where( 'ticket_trash', 0 );
	}
	...
	public function getLatestMessage( $tickets ) {
		...
		if ( count( $latest_tickets ) ) {
			...
			$users = Arr::pluck( $users, 'display_name', 'user_email' );
	...
	public function getCountByAgents( $subQuery ) {
		...
		$whereSubQuery = $this->getAdminUserQuery()->select( [ 'user_id' ] );
		if ( wsdesk_can_access_other_tickets() === false ) {
			$whereSubQuery->where( 'user_id', '=', get_current_user_id() );
		}
	...
	public function getAgents( $filter = array() ) {
		$users = $this->getAdminUserQuery( $filter )->get();
	...
	public function get_view_access( $view_slug ) {
		...
		return maybe_unserialize( $access->meta_value );
	}
	...
	public function get_available_views( $filters = [] ) {
		...
		$custom_views = Arr::where(
			...
			function ( $view ) use ( $roles, $views_only ) {
				$viewAccess = $this->get_view_access( $view );
	 ...
	 * @return array
	 */
	public function archiveTickets( $filter ) {
		global $wpdb;
		...
		$query  = 'insert into ' . $table_wsdesk_archived_tickets;
		$query .= ' select * from ' . $table_wsdesk_tickets . ' where (ticket_id in (' . $selectQuery . ')';
		$query .= ' or ticket_parent in  (' . $selectQuery . ')) and ticket_id not in (select ticket_id from ' . $table_wsdesk_archived_tickets . ') ';

		wpFluent()->statement( $query );

		// Copy tickets meta into archived tickets meta
		$query  = 'insert into ' . $table_wsdesk_archived_tickets_ticketsmeta;
		$query .= ' select * from ' . $table_wsdesk_ticketsmeta . ' where ticket_id in (' . $selectQuery . ') and meta_id not in (select meta_id from ' . $table_wsdesk_archived_tickets_ticketsmeta . ')';
		wpFluent()->statement( $query );

		// Get the count of tickets before delete
		$data['count'] = $baseQuery->count();
		// Delete tickets
		$query  = 'delete from ' . $table_wsdesk_tickets;
		...
		// Delete Tickets meta
		$query  = 'delete from ' . $table_wsdesk_ticketsmeta;
		$query .= ' where ticket_id not in ( select t1.ticket_id from ' . $table_wsdesk_tickets . ' as t1)';
