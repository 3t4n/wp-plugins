...
class EH_CRM_Mailbox {
	...
	protected function initImapStream() {
		$imapStream = @imap_open( $this->imapPath, $this->imapLogin, $this->imapPassword, $this->imapOptions, $this->imapRetriesNum, array( 'DISABLE_AUTHENTICATOR' => 'GSSAPI' ) );
	...
	public function getMail( $mailId, $markAsSeen = true ) {
		...
		$mail->headers     = $head;
		$mail->id          = $mailId;
		$mail->date        = gmdate( 'd-M-Y h:i:s A', isset( $head->date ) ? strtotime( preg_replace( '/\(.*?\)/', '', $head->date ) ) : time() );
	...
	protected function convertStringEncoding( $string, $fromEncoding, $toEncoding ) {
		...
		return $convertedString ? $string : $string;
	}
	...
	/** Function used to set image path
	 * 
	 * @param  $imapPath
	 * @return void
	 */
