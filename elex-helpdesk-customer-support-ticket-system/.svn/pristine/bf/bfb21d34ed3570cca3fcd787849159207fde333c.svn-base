...
jQuery( document ).on(
	'click',
	'#archive_tickets_table_v2 > tbody > tr > td:not(:nth-child(-n+2))',
	function (e) {
	...
	jQuery( ".tab-content" ).on(
		...
		var template  = jQuery( this ).prop( "id" );
			if (based === 'bulk') {
				var ticket = getValue_checkbox( "ticket_select_t" );
				if (ticket !== '') {
				...
				jQuery.ajax(
					{
						...
						success: function (data) {
							...
							jQuery( ".table_loader" ).css( "display", "none" );
							jQuery( "#reply_textarea_" + id + " > .ql-editor" ).html( data.data.page + jQuery( "#reply_textarea_" + id + " > .ql-editor" ).html() );
				...
				jQuery.ajax(
					{
						...
						url: ajaxurl,
						data: {
							action: 'eh_crm_ticket_search_archive',
							search: search
						},
						success: function (data) {
			...
			jQuery.ajax(
				{
					...
					url: ajaxurl,
					data: {
						action: 'eh_crm_ticket_refresh_left_bar_archive',
						active: active
					},
					success: function (data) {
		...
		function refresh_ticket_count()
	{
			jQuery.ajax(
				{
					...
					url: ajaxurl,
					data: {
						action: 'eh_crm_refresh_tickets_count_archive'
					},
					success: function (data) {
						var parse = jQuery.parseJSON( data );
		...
		function getUpdatedQueryParam(param, value)
	{
			var uri = new URLSearchParams( window.location.search );
			uri.set( param, value );
			return '//' + window.location.host + window.location.pathname + '?' + uri.toString();
		}

		function refresh_right_bar(ul, pagination, current)
	{
			var current_page = parseInt( jQuery( "#current_page_no" ).val() );
			ul               = ul || '';
			pagination       = pagination || '';

			if (pagination === 'next') {
				window.location.href = getUpdatedQueryParam( 'page_no', (current_page + 1) );
				return;
			}
			...
			if (pagination === 'current_page_n') {
				window.location.href = getUpdatedQueryParam( 'page_no', jQuery( '#current_page_n' ).val() );
				return;
			}
			...
			jQuery.ajax(
				{
					...
					url: ajaxurl,
					data: {
						action: 'eh_crm_ticket_refresh_right_bar_archive',
						active: active,
					...
					jQuery.ajax(
						{
							...
							url: ajaxurl,
							data: {
								action: 'eh_crm_ticket_single_view_archive',
								nonce: js_obj.nonce,
								ticket_id: ticket_id,
								pagination_id : jQuery( "#pagination_ids_traverse" ).val()
							},
	...
	jQuery( ".tab-content" ).on(
		"click",
		".multiple_ticket_unarchive_action",
		function (e) {
		e.preventDefault();
		var tickets = dtTable.rows( '.selected' ).data().toArray().map(
			function (row) {
			return row.ticket_id;
			}
		);
			if (tickets != '' || dtFilter.selectAll) {

				BootstrapDialog.show(
					{
						title: js_obj.WSDesk_Alert,
						message: js_obj.Do_You_want_to_Unarchive_Ticket,
						cssClass: 'wsdesk_wrapper',
						buttons: [{
							label: js_obj.Yes_Unarchive,
							// no title as it is optional
							cssClass: 'btn-primary',
							action: function (dialogItself) {
								jQuery( ".table_loader" ).css( "display", "inline" );
								jQuery.ajax(
									{
										...
										url: ajaxurl,
										data: {
											action: 'eh_crm_ticket_multiple_unarchive',
											nonce: js_obj.nonce,
											ticket_id: tickets,
											selectAll: dtFilter.selectAll ? 'true' : undefined,
										},
										success: function (data) {
											dtFilter.ticket_id = [];
											jQuery('#selectCurrentPageRows').prop('checked', false);
											...
											for (i = 0;i < tickets.length;i++) {
												if (jQuery( "#tab_" + tickets[i] ).length != 0 && jQuery( "#tab_content_" + tickets[i] ).length != 0) {
													jQuery( "#tab_" + tickets[i] + ">a>.close_tab" ).trigger( "click" );
												}
											}
											jQuery( ".table_loader" ).css( "display", "none" );
											jQuery( ".alert-success" ).css( "display", "block" );
											jQuery( ".alert-success" ).css( "opacity", "1" );
											jQuery( "#success_alert_text" ).html( "<strong>" + js_obj.WSDesk_Tickets_Notification + "</strong><br>" + js_obj.Tickets_Unarchived_Successfully + "!" );
											window.setTimeout(
												function () {
												jQuery( ".alert-success" ).fadeTo( 500, 0 ).slideUp(
													500,
		...
		function previewFiles(files, id) {
			...
			function readAndPreview(file) {
				...
				if (/\.(jpe?g|png|gif|3gp|avi|wmv|mpg|mov|flv)$/i.test( file.name )) {
					...
					if (/\.(doc?x|pdf|xml|csv|xls|xlsx|txt|zip|mp3|mp4|syx|rtf|cdr|bmp)$/i.test( file.name )) {
