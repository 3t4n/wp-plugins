...
! defined( 'WPCPC_VERSION' ) && define( 'WPCPC_VERSION', '1.1.3' );
...
! defined( 'WPCPC_DIR' ) && define( 'WPCPC_DIR', plugin_dir_path( __FILE__ ) );
	...
	function wpcpc_init() {
		...
		if ( ! class_exists( 'WPCleverWpcpc' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWpcpc {
				...
				function init() {
					// load text-domain
					load_plugin_textdomain( 'wpc-smart-product-collections', false, basename( WPCPC_DIR ) . '/languages/' );
