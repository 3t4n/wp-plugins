...
if ( !class_exists( 'CustomStoreLocatorPlugin' ) ) {
	class CustomStoreLocatorPlugin
	{
		...
		function csl_register_settings() {
			...
			add_option( 'csl_fullwidth_include_loc', '');
		...
		function csl_enqueue() {
			...
			if ($api_key) {
				...
				wp_enqueue_script( 
					'csl-gmapscript', 
					'https://maps.googleapis.com/maps/api/js?key=' . esc_attr($api_key) . '&libraries=geometry,places', 
					array('jquery'), 
					null, 
					false 
				);
				*/
				wp_register_script('csl-gmapscript', '//maps.googleapis.com/maps/api/js?&key=' . esc_attr($api_key) . '&libraries=geometry,places&callback=cslinitMap', array(), '', true);
				wp_script_add_data('csl-gmapscript', 'async defer', true);
				wp_script_add_data('csl-gmapscript', 'defer', true);
			}
