...
function csl_listing_parameters_shortcode( $atts ) {
	...
	<script>
		...
		<?php while ( $listing_query->have_posts() ) : $listing_query->the_post(); 
			...
			$category_marker = (!empty($locations_catlist[0])) ? get_term_meta($locations_catlist[0]->term_id, 'category_marker', true) : '';
    ...
    hours: "<?php echo wp_kses_data( '<p class=\"store-hours\"><strong>' . __( "Store Hours", "custom-store-locator" ) . ': </strong>' . nl2br( esc_html( $business_storehours ) ) . '</p>' ); ?>",
    <?php } else { ?>
    hours: '',
    <?php } ?>
    ...
    address: "<?php echo wp_kses_data( '<strong>' . __( "Address", "custom-store-locator" ) . ':</strong> ' . nl2br( esc_html( $business_address ) ) ); ?>",
    <?php } ?>
},
				<?php endwhile;
				wp_reset_postdata(); ?>
...
function csl_admin_scripts() {  
	...
	if( 'store_plugin' == $parent_file ) {
		...
		wp_register_script('my-upload', CSL_URL . '/assets/cls_custom_admin.js', array('jquery','media-upload','thickbox'));
		wp_enqueue_script('my-upload');	
	}
	if( 'edit.php?post_type=csl_locations' == $parent_file ) {
		wp_enqueue_style( 'csl-admin-page-css', CSL_URL . '/assets/cls_custom_admin.css' );
	}
	}
...
function csl_edit_category_marker_field($term) {
    ...
    jQuery(document).ready(function ($) {
        ...
        $('#upload_category_marker').on('click', function (e) {
            ...
            // Check if the media uploader is already open to prevent duplicates
            var mediaUploader = wp.media({
                title: '<?php echo esc_js(__('Select Marker', 'textdomain')); ?>',
                button: {
                    text: '<?php echo esc_js(__('Use This Marker', 'textdomain')); ?>'
                },
                multiple: false // Single selection only
            });
            // When a marker is selected
