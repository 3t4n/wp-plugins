...
function cslinitMap(locations = allLocations) {
  ...
  locations.forEach(function(location) {   
    ...
    infoWindowContent.push(['<div class="infoWindow"><h3>' + location.name + '</h3><p>' + 
      ...
      locfax +
      locwebsite + 
      '</p><a class="directionlink" href="' + directionlink + '" target="_blank">' + mapscript_object.get_direction + '<a></div>']);
  });	    
 ...
 var options_markerclusterer = {
  ...
  jQuery('select#locationslist').on('change', function (e) {
    ...
    if(selectedlocation !== "")
    {
     // console.log(jQuery(this).find('option:selected').attr('data-markerid'));
  ...
  jQuery('select#locationscat').on('change', function (e) {
   ...
   // console.log(selectedlocationcat);
...
jQuery(function($){
  ...
  function getLatLngViaHttpRequest(address) {
    ...
    $.get( request, function( data ) {
     // var searchResultsAlert = document.getElementById('csl-fullwidthmap-alert');
...
locations.forEach( function(location) {
  ...
  locemail +
  locfax + 
  locwebsite + '</p><a class="directionlink" href="' + directionlink + '" target="_blank">' + mapscript_object.get_direction + '<a></div>']);
  if(caticon !== "")
  {
    var myicon = {
      position:  new google.maps.LatLng(locations[i].lat, locations[i].lng),
