jQuery(document).ready(function($) {
    ...
    $('#toggle-fe-mode').on('click', function() {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    var widgets_mode = response.data.new_mode;
                    $('#debug-fe-status').text(widgets_mode === 'true' ? 'ON' : 'OFF');
                    $('#debug-fe-status').css('color', widgets_mode === 'true' ? 'red' : 'green');
    
                    // Optionally show or hide the floating widget based on the mode
                    if (widgets_mode === 'true') {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                // Optionally show or hide the floating widget based on the widgets_mode
                if (widgets_mode === 'true') {
                    $('#error-log-container').show();
                } else {
                    $('#error-log-container').hide();
