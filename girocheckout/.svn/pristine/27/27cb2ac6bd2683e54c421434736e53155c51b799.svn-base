...
class gc_paydirekt extends WC_Payment_Gateway
{
  ...
  public function __construct() {
    ...
    $this->icon = plugins_url('img/gc_paydirekt.png', dirname(__FILE__));
  ...
  public function process_payment($order_id) {
    ...
    try {
      ...
      $orderAmountExcShipping = $amountTotal - $iShippingTotal;
      ...
      $request->addParam('merchantId', $merchantId)
                ...
                ->addParam('amount', round($amountTotal * 100))
      ...
      error_log( "Exception in process_payment: (" . $e->getCode() . ") " . $e->getMessage() . ", Trace " . $e->getTraceAsString() );
      return;
    }
  ...
  public function capture_payment( $order_id )
  {
    ...
    if (!empty($strReference) && $order->payment_method == 'gc_paydirekt' && $transactiontype == 'authorize' &&
