<?php
class gc_maestro extends WC_Payment_Gateway
{
  ...
  public function __construct()
  {
    ...
    $this->id = 'gc_maestro';
    $this->method_title = __('Maestro', 'girocheckout');
    $this->title = __('Maestro', 'girocheckout');
    ...
    $this->description = __('Maestro is your ATM banking card that you can now use to pay online. It\'s a debit facility, so you can never spend more with your Maestro card than you have in your bank account.', 'girocheckout');
    $this->merchantid = $this->get_option('merchantid');
    $this->projectid = $this->get_option('projectid');
    $this->password = $this->get_option('password');
    $this->purpose = $this->get_option('purpose');
    $this->icon = plugins_url('img/gc_maestro.png', dirname(__FILE__));
  ...
  public function init_form_fields()
  {
    $this->form_fields = array(
      ...
      'enabled' => array(
        ...
        'type' => 'checkbox',
        'label' => __('Enable Maestro', 'girocheckout'),
        'default' => 'no',
      ),
      'title' => array(
        ...
        'default' => __('Maestro', 'girocheckout'),
        'desc_tip' => true,
      ),
      'merchantid' => array(
  ...
  {
    ?>
      <h3><?php _e('Maestro', 'girocheckout'); ?></h3>
      <p><?php _e('GiroCheckout Maestro payment', 'girocheckout'); ?></p>
  ...
  public function do_gc_redirect()
  {
    ...
    try {
      $notify = new GiroCheckout_SDK_Notify('maestroTransaction');
  ...
  public function process_refund($order_id, $amount = null, $reason = '')
  {
    ...
    try {
      ...
      $request = new GiroCheckout_SDK_Request('maestroRefund');
      $request->setSecret($password);
      $request->addParam('merchantId', $merchantId)
        ->addParam('projectId', $projectId)
