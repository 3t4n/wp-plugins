jQuery(document).ready(function () {
  ...
  let SearchMessage    = upsellajaxapi.beucw_search_msg;
  let SearchMessageOne = upsellajaxapi.beucw_msg_one;
  let SearchMessageTwo = upsellajaxapi.beucw_msg_two;

  let SearchMessageSave     = upsellajaxapi.beucw_msg_save;
  let SearchMessageSaveHtml = upsellajaxapi.beucw_msg_save_html;
  let SearchMessageUpdate   = upsellajaxapi.beucw_msg_update;
  let SearchMessageError    = upsellajaxapi.beucw_msg_error;

  let NoticeMessage = upsellajaxapi.beucw_dismiss_noti;
  let ProMessage    = upsellajaxapi.beucw_pro_notice;

  // Select2 library loading. 
  jQuery('.bucw-filter-box').select2({ width: '400px' });
  jQuery('.bucw-product-filter-box').select2({ width: '400px' });

  // Initailly hide all things from buc page.
  jQuery(
  ...
  jQuery("#beucw-filter-type").on("change", function () {
    ...
    switch (filterType) {
      ...
      case "bucw-sku":
        ...
        break;
      case "filter-by":
        jQuery('#bucw-filter-container').css('display', 'none');
        jQuery("#bucw-select-categories, #bucw-select-product, #bucw-select-tags").hide();
        break;
      default:
        break;
  ...
  jQuery("#bucw-search-product").on("click", function () {
    ...
    let filterType = jQuery("#beucw-filter-type").val();
  ...
  jQuery(".bucw-save").click(function (event) {
    let TableData = []; //initialize array;
    let bucw_data = ""; //empty var;
    ...
    jQuery("#beucw-products-table .beucw-product-row").each(function (index, row) {

      var currentRow = null;
      currentRow = jQuery(this);

      TableData[index] = {
        beucw_product_id: currentRow.find('.beucw-product-name .bucw-product-title a').attr("id"),
    ...
    jQuery.ajax({
      ...
      method: "POST",
      data: {
        action: "beucw_save_all_selected_products",
        nonce: upsellajaxapi.nonce,
      ...
      success: function (response) {
        ...
        if (0 == jQuery('#bucw-success-massage').length) {
          jQuery("#bucw-upsells-crosssell").before(
            '<div id="bucw-success-massage" class="updated notice is-dismissible"><p>' + SearchMessageUpdate + '</p><button id="bucw-dismiss-admin-message" class="notice-dismiss" type="button"><span class="screen-reader-text">' + NoticeMessage +'.</span></button></div>'
          );
        }

        // Remove success notice after saving value.
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        ...
        jQuery("#bucw-upsells-crosssell").before(
          ...
          errorThrown +
          '</p><button id="bucw-dismiss-admin-message" class="notice-dismiss" type="button"><span class="screen-reader-text">' + NoticeMessage + '</span></button></div>'
        );
