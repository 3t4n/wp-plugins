...
use YaySMTPSendinBlue\Helper\LogErrors;
...
class Functions {
	...
	private function doHooks() {
		...
		add_action( 'wp_ajax_' . YAY_SMTP_SENDINBLUE_PREFIX . '_save_settings', array( $this, 'saveSettings' ) );
		add_action( 'wp_ajax_' . YAY_SMTP_SENDINBLUE_PREFIX . '_send_mail', array( $this, 'sendTestMail' ) );
		add_action( 'wp_ajax_' . YAY_SMTP_SENDINBLUE_PREFIX . '_email_logs', array( $this, 'getListEmailLogs' ) );
		add_action( 'wp_ajax_' . YAY_SMTP_SENDINBLUE_PREFIX . '_set_email_logs_setting', array( $this, 'setYaySmtpEmailLogSetting' ) );
		add_action( 'wp_ajax_' . YAY_SMTP_SENDINBLUE_PREFIX . '_delete_email_logs', array( $this, 'deleteEmailLogs' ) );
		add_action( 'wp_ajax_' . YAY_SMTP_SENDINBLUE_PREFIX . '_delete_all_email_logs', array( $this, 'deleteAllEmailLogs' ) );
		add_action( 'wp_ajax_' . YAY_SMTP_SENDINBLUE_PREFIX . '_detail_email_logs', array( $this, 'getEmailLog' ) );
	}
	...
	public function saveSettings() {
		try {
			...
			if ( isset( $_POST['settings'] ) ) {
				...
				$yaysmtpSettingsDB = get_option( YAY_SMTP_SENDINBLUE_PREFIX . '_settings' );
				...
				update_option( YAY_SMTP_SENDINBLUE_PREFIX . '_settings', $yaysmtpSettings );

				wp_send_json_success( array( 'mess' => __( 'Settings saved.', 'smtp-sendinblue' ) ) );
			}
			wp_send_json_error( array( 'mess' => __( 'Settings Failed.', 'smtp-sendinblue' ) ) );
		} catch ( \Exception $ex ) {
		...
		try {
			...
			if ( isset( $_POST['emailAddress'] ) ) {
				...
				if ( ! is_email( $emailAddress ) ) {
					wp_send_json_error( array( 'mess' => __( 'Invalid email format!', 'smtp-sendinblue' ) ) );
				}

				$headers      = "Content-Type: text/html\r\n";
				$subjectEmail = __( 'YaySMTP - Test email was sent successfully!', 'smtp-sendinblue' );
				$html         = __( 'Yay! Your test email was sent successfully! Thanks for using <a href="https://yaycommerce.com/yaysmtp-wordpress-mail-smtp/">YaySMTP</a><br><br>Best regards,<br>YayCommerce', 'smtp-sendinblue' );

				if ( ! empty( $emailAddress ) ) {
					$sendMailSucc = wp_mail( $emailAddress, $subjectEmail, $html, $headers );
					if ( $sendMailSucc ) {
						Utils::setYaySmtpSetting( 'succ_sent_mail_last', true );
						wp_send_json_success( array( 'mess' => __( 'Email has been sent.', 'smtp-sendinblue' ) ) );
					} else {
				...
				wp_send_json_error( array( 'mess' => __( 'Email Address is not empty.', 'smtp-sendinblue' ) ) );
			}
			wp_send_json_error( array( 'mess' => __( 'Error send mail!', 'smtp-sendinblue' ) ) );
		} catch ( \Exception $ex ) {
			LogErrors::getMessageException( $ex, true );
		} catch ( \Error $ex ) {
			LogErrors::getMessageException( $ex, true );
		...
		try {
			...
			if ( isset( $_POST['params'] ) ) {
				...
				if ( ! empty( $valSearch ) ) {
					...
					$totalItems = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs WHERE $whereQuery" );

					$sqlRepare = $wpdb->prepare(
						"SELECT l.id, l.subject, l.email_from, l.email_to, l.mailer, l.date_time, l.status FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs AS l WHERE $whereQuery ORDER BY $sortField $sortVal LIMIT %d OFFSET %d",
						$limit,
						$offset
					);
				} else {
					$totalItems = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs WHERE $statusWhere" );

					$sqlRepare = $wpdb->prepare(
						"SELECT l.id, l.subject, l.email_from, l.email_to, l.mailer, l.date_time, l.status FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs AS l WHERE $statusWhere ORDER BY $sortField $sortVal LIMIT %d OFFSET %d",
						$limit,
						$offset
					);
				}
				...
				wp_send_json_success(
					array(
						...
						'mess'            => __( 'Successful', 'smtp-sendinblue' ),
					)
				);
			}
			wp_send_json_error( array( 'mess' => __( 'Failed.', 'smtp-sendinblue' ) ) );
		} catch ( \Exception $ex ) {
			LogErrors::getMessageException( $ex, true );
		} catch ( \Error $ex ) {
			LogErrors::getMessageException( $ex, true );
	...
	public function setYaySmtpEmailLogSetting() {
		try {
			...
			if ( isset( $_POST['params'] ) ) {
				...
				wp_send_json_success(
					array(
						'mess' => __( 'Save Settings Successful', 'smtp-sendinblue' ),
					)
				);
			}
			wp_send_json_error( array( 'mess' => __( 'Save Settings Failed.', 'smtp-sendinblue' ) ) );
		} catch ( \Exception $ex ) {
			LogErrors::getMessageException( $ex, true );
		} catch ( \Error $ex ) {
			LogErrors::getMessageException( $ex, true );
	...
	public function deleteEmailLogs() {
		try {
			...
			if ( isset( $_POST['params'] ) ) {
				...
				if ( empty( $ids ) ) {
					wp_send_json_error( array( 'mess' => __( 'No email log id found', 'smtp-sendinblue' ) ) );
				}

				$deleted = $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs WHERE ID IN( $ids )" ) );

				if ( '' !== $wpdb->last_error ) {
					wp_send_json_error( array( 'mess' => __( $wpdb->last_error, 'smtp-sendinblue' ) ) );
				}

				if ( ! $deleted ) {
					wp_send_json_error( array( 'mess' => __( 'Something wrong, Email logs not deleted', 'smtp-sendinblue' ) ) );
				}

				wp_send_json_success(
					array(
						'mess' => __( 'Delete successful.', 'smtp-sendinblue' ),
					)
				);
			}
			wp_send_json_error( array( 'mess' => __( 'No email log id found.', 'smtp-sendinblue' ) ) );
	...
	public function deleteAllEmailLogs() {
		try {
			Utils::checkNonce();
			global $wpdb;
			$wpdb->query( "DELETE FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs" );
	...
	public function getEmailLog() {
		try {
			...
			if ( isset( $_POST['params'] ) ) {
				...
				$resultQuery = $wpdb->get_row( $wpdb->prepare( "Select * FROM {$wpdb->prefix}yay_smtp_sendinblue_email_logs WHERE id = %d", $id ) );
				...
				if ( ! empty( $resultQuery ) ) {
					...
					wp_send_json_success(
						array(
							'mess' => __( 'Get email log #' . $id . ' successful.', 'smtp-sendinblue' ),
							'data' => $resultArr,
						)
					);
				} else {
					wp_send_json_error( array( 'mess' => __( 'No email log found.', 'smtp-sendinblue' ) ) );
				}
			}
			wp_send_json_error( array( 'mess' => __( 'No email log id found.', 'smtp-sendinblue' ) ) );
