<?php
namespace YaySMTPSendinBlue\Helper;
...
class Utils {
	...
	public static function getCurrentFromEmail() {
		...
		$yaysmtpSettings = get_option( YAY_SMTP_SENDINBLUE_PREFIX . '_settings' );
	...
	public static function setYaySmtpSetting( $key, $value = '', $mailer = '' ) {
		if ( empty( $mailer ) && ! empty( $key ) ) { // Update: fromEmail / fromName / currentMailer. Ex: ['fromEmail' => 'admin']
			...
			$setting[ $key ] = $value;
			update_option( YAY_SMTP_SENDINBLUE_PREFIX . '_settings', $setting );
		} elseif ( ! empty( $mailer ) && ! empty( $key ) ) { // Update settings of mailer. Ex: ['sendinblue' => ['api_key' => '123abc']]
			$setting                    = self::getYaySmtpSetting();
	...
	public static function getYaySmtpEmailLogSetting() {
		$rst             = array();
		$yaysmtpSettings = get_option( YAY_SMTP_SENDINBLUE_PREFIX . '_email_log_settings' );
		if ( ! empty( $yaysmtpSettings ) && is_array( $yaysmtpSettings ) ) {
			$rst = $yaysmtpSettings;
		}
		return $rst;
	...
	public static function setYaySmtpEmailLogSetting( $key, $value = '' ) {
		if ( ! empty( $key ) ) {
			...
			$setting[ $key ] = $value;
			update_option( YAY_SMTP_SENDINBLUE_PREFIX . '_email_log_settings', $setting );
		}
	}

	public static function getAdminPageUrl( $page = '' ) {
		if ( empty( $page ) ) {
			$page = YAY_SMTP_SENDINBLUE_PREFIX;
		}
	...
	public static function insertEmailLogs( $data ) {
		...
		if ( 'yes' == $saveSetting && ! empty( $data ) && is_array( $data['email_to'] ) ) {
			global $wpdb;
			$tableName = $wpdb->prefix . YAY_SMTP_SENDINBLUE_PREFIX . '_email_logs';
			$content   = array(
