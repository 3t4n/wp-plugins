...
class Settings {
	...
	public function enqueueAdminScripts( $screenId ) {
		...
		if ( isset( $this->hook_suffix[ YAY_SMTP_SENDINBLUE_PREFIX . '_main_page' ] ) && $screenId == $this->hook_suffix[ YAY_SMTP_SENDINBLUE_PREFIX . '_main_page' ] ) {
			...
			wp_enqueue_script( 'yaysmtp_sendgrid_purify', YAY_SMTP_SENDINBLUE_PLUGIN_URL . 'assets/js/purify.min.js', array(), YAY_SMTP_SENDINBLUE_VERSION, true );
