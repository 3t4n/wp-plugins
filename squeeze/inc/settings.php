...
class SqueezeSettings extends SqueezeInit {
    ...
    public function options_validate( $input ) {
        ...
        $input['compress_formats'] = $this->validate_image_formats( $input['compress_formats'] );
    ...
    protected function validate_image_formats( $input ) {
    ...
    public function add_custom_field_to_attachment( $form_fields, $post ) {
        ...
        if ( in_array( $post->post_mime_type, $allowed_mimes ) ) {
            ...
            $form_fields['squeeze_is_compressed'] = array(
                ...
                'input' => 'html',
                'html'  => ( $is_excluded ? '<label><span class="squeeze_status"><span style="padding-top: 0; line-height: 1; color: orange;" class="dashicons dashicons-warning"></span>&nbsp;' . __( 'Attachment is excluded from compression', 'squeeze' ) . ' (' . esc_html__( 'found substring: ', 'squeeze' ) . $is_excluded['exclude_reason'] . ')</span></label>' : (( $is_compressed ? '<label><span class="squeeze_status"><span style="padding-top: 0; line-height: 1; color: green;" class="dashicons dashicons-yes-alt"></span>&nbsp;' . __( 'Squeezed', 'squeeze' ) . '</span></label>' . (( $can_restore ? '
