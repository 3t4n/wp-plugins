(function() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // TBD: universal solution for all hidden options
        ...
        const webpLosslessCheckbox = document.querySelector('input[name="squeeze_options[webp_lossless]"]');
        webpLosslessCheckbox?.addEventListener('change', (event) => {
            const webpQualitySlider = document.querySelector('input[name="squeeze_options[webp_quality]"]');
            if (event.target.checked) {
                webpQualitySlider.classList.add('squeeze-hidden');
                webpQualitySlider.closest('tr').style.display = 'none';
            } else {
                webpQualitySlider.classList.remove('squeeze-hidden');
                webpQualitySlider.closest('tr').style.display = '';
            }
        });
    });
