...
! defined( 'WOOSL_VERSION' ) && define( 'WOOSL_VERSION', '3.3.2' );
...
! defined( 'WOOSL_DIR' ) && define( 'WOOSL_DIR', plugin_dir_path( __FILE__ ) );
	...
	function woosl_init() {
		...
		if ( ! class_exists( 'WPCleverWoosl' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoosl {
				...
				function init() {
					// load text-domain
					load_plugin_textdomain( 'wc-save-for-later', false, basename( WOOSL_DIR ) . '/languages/' );
				...
				function save_user_data() {
					...
					if ( ! empty( $_COOKIE[ $user_key ] ) && $user_id ) {
