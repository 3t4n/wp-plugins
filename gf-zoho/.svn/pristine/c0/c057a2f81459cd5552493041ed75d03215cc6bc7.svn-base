<?php
/**
* Plugin Name: WP Gravity Forms Zoho Add-on
* Description: Integrates Gravity Forms with Zoho allowing form submissions to be automatically sent to your Zoho account 
* Version: 1.2.5
* Requires at least: 3.8
* Author URI: https://www.crmperks.com
* Plugin URI: https://www.crmperks.com/plugins/gravity-forms-plugins/gravity-forms-zoho-plugin/
* Author: CRM Perks.
* Text Domain: gravity-forms-zoho-crm
...
class vxg_zoho {
 ...
 public  $url = 'https://www.crmperks.com';
  ...
  public  $id = 'vxg_zoho';
  public  $domain = 'vxg-zoho';
  public  $version = "1.2.5";
  public  $update_id = '30003';
  public  $min_gravityforms_version = '1.3.9';
  public $type = 'vxg_zoho_pro';
  public  $fields = null;
  public  $data = null;
  ...
  public static $title='Gravity Forms Zoho Plugin';  
  public static $path = ''; 
  public static $slug = '';
...
add_action('init', array($this,'init'));   
  ...
  load_plugin_textdomain('gravity-forms-zoho-crm', FALSE,  $this->plugin_dir_name(). '/languages/' );
...
public function add_tags( $merge_tags, $form_id, $fields, $element_id ) {
  ...
  foreach($feeds as $v){
    $merge_tags[] = array('label' => substr($v['name'],0,20).' Zoho Link', 'tag' => '{zoholink_'.$v['id'].'}');
    $merge_tags[] = array('label' => substr($v['name'],0,20).' Zoho ID', 'tag' => '{zohoid_'.$v['id'].'}');
  }
    return $merge_tags;
}
public function replace_tags( $text, $form, $entry, $url_encode, $esc_html, $nl2br, $format ) {
 
        $custom_merge_tag = '{zoho';
...
$tags=array();
 ...
 $link= !empty(self::$feeds_res[$id]['link']) ? self::$feeds_res[$id]['link'] : '#'.self::$feeds_res[$id]['id'];         
   $tags['{zoholink_'.$v['id'].'}']=$link;   
   $tags['{zohoid_'.$v['id'].'}']=self::$feeds_res[$id]['id'];;   
      }
  }
...
self::$plugin->instance();
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo esc_html($id) ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Gravity Forms Zoho Plugin','gravity-forms-zoho-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','gravity-forms-zoho-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice is-dismissible" data-id="<?php echo esc_html($id) ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Gravity Forms Zoho Plugin','gravity-forms-zoho-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sGravity Forms%s is required. %sPurchase it today!%s", 'gravity-forms-zoho-crm'), "<a href='http://www.gravityforms.com/'>", "</a>", "<a href='http://www.gravityforms.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Gravity Forms is installed but not active. %sActivate Gravity Forms%s to use the Gravity Forms Zoho Plugin','gravity-forms-zoho-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=gravityforms/gravityforms.php'), 'activate-plugin_gravityforms/gravityforms.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sGravity Forms%s is required. %sPurchase it today!%s", 'gravity-forms-zoho-crm'), "<a href='http://www.gravityforms.com/'>", "</a>", "<a href='http://www.gravityforms.com/'>", "</a>");
  }  
  ...
  */
  ...
  if(current_user_can('administrator') && !empty($_REQUEST['VXGZohoError'])) { /// 
  $confirmation .= sprintf(__('%sThe entry was not added to Zoho because: %s. %sYou are only being shown this because you are an administrator. Other users will not see this message.%s', 'gravity-forms-zoho-crm'), '<div class="error" style="text-align:center; color:#790000; font-size:14px; line-height:1.5em; margin-bottom:16px;background-color:#FFDFDF; margin-bottom:6px!important; padding:6px 6px 4px 6px!important; border:1px dotted #C89797">','<strong>'.esc_html($_REQUEST['VXGZohoError']). '</strong>', '<br /><em>', '</em></div>');
  }
  return $confirmation;
  }
...
/*  if(empty($field)){
  ...
  if( (isset($field->storageType) && $field->storageType == 'json') || $field->type == 'fileupload' ){
   ...
   if($field->type == 'date' ){
    $formats=array('mdy'=>'d/m/Y','dmy'=>'d/m/Y','dmy_dash'=>'d-m-Y','dmy_dot'=>'d.m.Y','ymd_slash'=>'Y/m/d','ymd_dash'=>'Y-m-d','ymd_dot'=>'Y.m.d');
 ...
 // $value=implode(", ",$value); //disable it , multiselect field of zoho needs array
  } 
  return $value;        
  }
  /**
...
public  function check_filter($feed,$entry,$form){
  ...
  */
  ...
  printf('<input type="checkbox" name="'.$this->id.'_update" id="'.$this->id.'_update" value="1" /><label for="'.$this->id.'_update" title="%s">%s</label><br /><br />', esc_html__('Create or update this entry in Zoho. The fields will be mapped according to the form feed settings.', 'gravity-forms-zoho-crm'), esc_html__('Send to Zoho', 'gravity-forms-zoho-crm'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Zoho Logs','gravity-forms-zoho-crm').'" href="'.$log_url.'">'.__('Zoho Logs','gravity-forms-zoho-crm').'</a>';
  ...
  */
  ...
  RGFormsModel::add_note($id, 0, esc_html__('Gravity Forms Zoho Plugin','gravity-forms-zoho-crm'), $note);
  }
...
do_action('plugin_status_'.$this->type,'activate');  
  ...
  */
  ...
  $crm_tooltips = array(
    'vx_feed_name' => '<h6>' . esc_html__('Feed Name', 'gravity-forms-zoho-crm') . '</h6>' . esc_html__('Enter feed name of your choice.', 'gravity-forms-zoho-crm'),
  'vx_sel_object' => '<h6>' .__('Zoho Object', 'gravity-forms-zoho-crm') . '</h6>' . esc_html__('Select the Object to Create when a Form is Submitted.', 'gravity-forms-zoho-crm'),
   'vx_sel_account' => '<h6>' .__('Zoho Account', 'gravity-forms-zoho-crm') . '</h6>' . esc_html__('Select the Zoho account you would like to export entries to.', 'gravity-forms-zoho-crm'),
  'vx_sel_form' => '<h6>' . esc_html__('Gravity Form', 'gravity-forms-zoho-crm') . '</h6>' . esc_html__('Select the Gravity Form you would like to integrate with Zoho. Contacts generated by this form will be automatically added to your Zoho account.', 'gravity-forms-zoho-crm'),
  
  'vx_map_fields' => '<h6>' . esc_html__('Map Standard Fields', 'gravity-forms-zoho-crm') . '</h6>' . esc_html__('Associate your Zoho fields to the appropriate Gravity Form fields.', 'gravity-forms-zoho-crm'),
  
  'vx_optin_condition' => '<h6>' . esc_html__('Opt-In Condition', 'gravity-forms-zoho-crm') . '</h6>' . esc_html__('When the opt-in condition is enabled, form submissions will only be exported to Zoho when the condition is met. When disabled all form submissions will be exported.', 'gravity-forms-zoho-crm'),
  
  'vx_manual_export' => '<h6>' . esc_html__('Manual Export', 'gravity-forms-zoho-crm') . '</h6>' . esc_html__('If you do not want all entries sent to Zoho, but only specific, approved entries, check this box. To manually send an entry to Zoho, go to Entries, choose the entry you would like to send to Zoho, and then click the "Send to Zoho" button.', 'gravity-forms-zoho-crm'),
  
    'vx_entry_notes' => '<h6>' . esc_html__('Entry Notes', 'gravity-forms-zoho-crm') . '</h6>' . esc_html__('Enable this option if you want to synchronize Gravity Forms entry notes to Zoho Object notes. For example , when you add a note to a Gravity Forms entry, it will be added to the Zoho Object selected in the feed.', 'gravity-forms-zoho-crm'),
    
      'vx_primary_key' => '<h6>' . esc_html__('Primary Key', 'gravity-forms-zoho-crm') . '</h6>' . esc_html__('Which field should be used to update existing objects?', 'gravity-forms-zoho-crm'),
     

  'vx_custom_app'=>'<h6>' . esc_html__('Custom App', 'gravity-forms-zoho-crm') . '</h6>' .__('This option is for advanced users who want to override default Zoho App.','gravity-forms-zoho-crm'),
  
  
  'vx_camps'=>'<h6>' . esc_html__('Zoho Campaigns', 'gravity-forms-zoho-crm') . '</h6>' .__('Get Campaigns list from zoho.','gravity-forms-zoho-crm'),
  
  'vx_sel_price_book'=>__('Which Pricebook should be searched for product','gravity-forms-zoho-crm'),
  
  'vx_sel_camp'=>'<h6>' . esc_html__('Select Campaign', 'gravity-forms-zoho-crm') . '</h6>' .__('Which Campaign should be assigned to this object.','gravity-forms-zoho-crm'),
  
  'vx_sel_status'=>'<h6>' . esc_html__('Campaign Status', 'gravity-forms-zoho-crm') . '</h6>' .__('What should be Member Status.','gravity-forms-zoho-crm'),
  
  'vx_pro_desc'=>'<h6>' . esc_html__('Price Book', 'gravity-forms-zoho-crm') . '</h6>' .__('A new product will be created in selected Pricebook. You can add a description for new products created by this plugin.','gravity-forms-zoho-crm'),
  
   'vx_assign_account'=>'<h6>' . esc_html__('Zoho Account', 'gravity-forms-zoho-crm') . '</h6>' .__('Enable this option if you want to assign an account this object.','gravity-forms-zoho-crm'),
   
   'vx_sel_account'=>'<h6>' . esc_html__('Select Account', 'gravity-forms-zoho-crm') . '</h6>' .__('Object created by this feed will be assigned to the selected Account.','gravity-forms-zoho-crm'),
   
      'vx_assign_vendor'=>'<h6>' . esc_html__('Assign Vendor', 'gravity-forms-zoho-crm') . '</h6>' .__('Enable this option , if you want to assign a Vendor to this object','gravity-forms-zoho-crm'),
      
   'vx_sel_vendor'=>'<h6>' . esc_html__('Select Vendor', 'gravity-forms-zoho-crm') . '</h6>' .__('Select Vendor feed. Vendor created by this feed will be assigned to this object','gravity-forms-zoho-crm'),
   
   'vx_camp_check'=>'<h6>' . esc_html__('Add to Campaign', 'gravity-forms-zoho-crm') . '</h6>' .__('If enabled, Lead/Contact will be added to selected Campaign','gravity-forms-zoho-crm'),
   'vx_owner_check'=>'<h6>' . esc_html__('Assign Owner', 'gravity-forms-zoho-crm') . '</h6>' .__('Enable this option if you want to assign another object owner.','gravity-forms-zoho-crm'),
   
   'vx_owners'=>'<h6>' . esc_html__('Zoho Users', 'gravity-forms-zoho-crm') . '</h6>' .__('Get Users list from Zoho','gravity-forms-zoho-crm'),
   
   'vx_order_notes'=>'<h6>' . esc_html__('Entry Notes', 'gravity-forms-zoho-crm') . '</h6>' .__('Enable this option if you want to synchronize Entry notes to Zoho Object notes. For example, when you add a note to a Entry, it will be added to the Zoho Object selected in the feed.','gravity-forms-zoho-crm'),
  
   'vx_sel_owner'=>'<h6>' . esc_html__('Select Owner', 'gravity-forms-zoho-crm') . '</h6>' .__('Select a user as a owner of this object','gravity-forms-zoho-crm'),
   
         'vx_entry_note'=>'<h6>' . esc_html__('Entry Note', 'gravity-forms-zoho-crm') . '</h6>' .__('Check this option if you want to send more data as CRM entry note.', 'gravity-forms-zoho-crm'),
   'vx_note_fields'=>'<h6>' . esc_html__('Note Fields', 'gravity-forms-zoho-crm') . '</h6>' .__('Select fields which you want to send as a note', 'gravity-forms-zoho-crm'),
   'vx_disable_note'=>'<h6>' . esc_html__('Note Fields', 'gravity-forms-zoho-crm') . '</h6>' .__('Enable this option if you want to add note only for new CRM entry', 'gravity-forms-zoho-crm')
...
/**
  ...
  */
  ...
  $status=0;
  if(class_exists('vxg_zoho_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/gravity-forms-zoho-crm/gravity-forms-zoho-crm.php')) {
  $status=2;
  } 
  return $status;
  }
...
public function do_actions(){
  ...
  */
  ...
  $api = false;
  if(!class_exists("vxg_zoho_api"))
  require_once($this->get_base_path()."api/api.php");
     
  $api= new vxg_zoho_api($crm);
...
$object=$this->post('object',$feed); 
 if(!empty($data['vendor_check']) || !empty($data['account_check'])){
...
// add note and save related extra info
 if( $event == 'add_note' && !empty(self::$note)){
         $temp=array('Title'=>array('label'=>'Note Title','value'=>self::$note['title']),'Body'=>array('label'=>'Note Content', 'value'=>self::$note['body']),'ParentId'=>array('label'=>'Parent_Id', 'value'=> $feed['crm_id']) ); 
...
if($skip_feed){ continue; }
  ...
  //add account or vendor
    if(!empty($data['vendor_check']) && !empty($data['object_vendor'])){
     $vendor_feed=$data['object_vendor']; 
       if( isset(self::$feeds_res[$vendor_feed]) ){

   $vendor_res=self::$feeds_res[$vendor_feed];
  /////
  if(!empty($vendor_res['id'])){
   $temp['VENDORID']=array('value'=> $vendor_res['id'],'label'=>'VENDORID');   
  ...
  if(!empty($account_res['id'])){
   $temp['ACCOUNTID']=array('value'=> $account_res['id'],'label'=>'ACCOUNTID');   
  }else{ //if empty continue
      continue;
  }    
   }  
...
if(isset($info['data'])){
  ...
  $res=$api->push_object($object,$temp,$feed_arr);
  }
  ...
  if($this->post('id',$res)!=""){ 
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Zoho (%s) with ID # %s .', 'gravity-forms-zoho-crm'),$object,$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Zoho (%s) with ID # %s . View entry at %s', 'gravity-forms-zoho-crm'),$object,$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Zoho', 'gravity-forms-zoho-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Zoho (%s) with ID # %s . View entry at %s', 'gravity-forms-zoho-crm'),$object,$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Zoho (%s) with ID # %s .', 'gravity-forms-zoho-crm'),$object,$res['id']);   
            }
     $screen_msg=__( 'Entry updated in Zoho', 'gravity-forms-zoho-crm');
      }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'gravity-forms-zoho-crm');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Zoho. Entry not sent! Check the Entry Notes below for more details.' , 'gravity-forms-zoho-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Zoho. Entry not sent' , 'gravity-forms-zoho-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'gravity-forms-zoho-crm'),$feed['object']);
...
$this->send_error_email($info_data,$entry,$form);
  ...
  //insert log
  $arr=array("object"=>$object,"form_id"=>$form_id,"status"=>$status,"entry_id"=>$entry_id,"crm_id"=>$id,"meta"=>$error,"time"=>date('Y-m-d H:i:s'),"data"=>$this->post('data',$res),"response"=>$this->post('response',$res),"extra"=>$this->post('extra',$res),"feed_id"=>$this->post('id',$feed),"link"=>$this->post('link',$res),'parent_id'=>$parent_id,'event'=>$event);
  ...
  $log_link=' <a href="'.$log_url.'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','gravity-forms-zoho-crm')."</a>";
 $screen_msg.=$log_link;
    }
  ...
  */
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Zoho','gravity-forms-zoho-crm')." Error","info_title"=>"More Detail","info"=>$detail);
...
$vxg_zoho=new vxg_zoho();
$vxg_zoho->instance();
$vx_gf['vxg_zoho']='vxg_zoho';
endif;
