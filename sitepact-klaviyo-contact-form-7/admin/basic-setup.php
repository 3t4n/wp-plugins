...
function KLCF_load_plugin_textdomain() {
    load_plugin_textdomain( 'sitepact-klaviyo-contact-form-7', false, '/sitepact-klaviyo-contact-form-7/languages/' );
}
...
function KLCF_plugin_row_meta( $links, $file ) {
	if ( strpos( $file, 'sitepact-klaviyo-contact-form-7.php' ) !== false ) {
		$new_links = array(
...
function klcf1_admin_notice() {
    ...
    <div class="notice notice-warning is-dismissible">
        <p><?php esc_html_e( 'Important! Contact Form 7 Extension For Klaviyo version 3 and above may need to be re-configured. Please go to form settings, reactivate and reconfigure to ensure form details are being sent to Klaviyo.', 'sitepact-klaviyo-contact-form-7' ); ?></p>
        <a href="<?php echo esc_url( get_admin_url( null, 'admin.php?page=wpcf7-klaviyo' ) ); ?>"><?php esc_html_e( 'Settings', 'sitepact-klaviyo-contact-form-7' ); ?></a>
        <p><a class="klcf-hide-notice button" href="<?php echo esc_url( $dismiss_url ); ?>"><?php esc_html_e( 'Already Done', 'sitepact-klaviyo-contact-form-7' ); ?></a></p>
    </div>
    <?php
}
...
function klcf_check_dismiss_notice() {
    ...
    if ( isset( $_GET['klcf_v3_update_notice_dismissed'] ) && '1' == sanitize_text_field( $_GET['klcf_v3_update_notice_dismissed'] ) && current_user_can( 'manage_options' ) ) {
        ...
        if ( ! wp_verify_nonce( $nonce, 'klcf_dismiss_notice' ) ) {
			wp_die( esc_html__( 'Security check failed.', 'sitepact-klaviyo-contact-form-7' ) );
        } else {
