...
class Unused_Media_List extends WP_List_Table {
    ...
    public function prepare_items() {
        ...
        $cache_key = 'unused_media_list_' . md5( serialize( array( $search, $this->author_id, $current_page, $per_page ) ) );
        ...
        if ( false === $cached_results ) {
            ...
            $query = "
                ...
                LEFT JOIN {$wpdb->postmeta} site_icon ON p.ID = site_icon.meta_value
                    AND site_icon.meta_key = 'site_icon'
                WHERE p.post_type = 'attachment'
                AND pm.meta_value IS NULL
                AND pp.ID IS NULL
                AND site_icon.meta_value IS NULL
            ";
            ...
            set_transient( $cache_key, array( 'items' => $this->items, 'total_items' => $total_items ), HOUR_IN_SECONDS );
        } else {
            // Use cached results.
