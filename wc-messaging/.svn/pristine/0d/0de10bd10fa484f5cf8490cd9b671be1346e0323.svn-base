...
if (!class_exists('Wcm_Send_Abandoned_Status')) {
    class Wcm_Send_Abandoned_Status
    {
        ...
        public function send_abandonment_data($event, $order_id, $whatsapp, $name, $order_date)
        {
            $url = get_option('woom_abandoned_cart_webhook', true);
            $message = [
                'event' => $event,
        ...
        public function abandonment_created_data($order_id = null)
        {
            if(empty($order_id)) {
                return;
            }
            ...
            if ($order) {
                ...
                $whatsapp =  $abandonment_checker->get_user_whatsapp_number($order_id);
                $name = $order->get_billing_first_name();
                // Get Order Dates
                $order_date = $order->get_date_created();
                $data = $this->send_abandonment_data('abandonment', $order_id, $whatsapp, $name, $order_date);
                return $data;
            }
            return;
        }
        public function abadonment_recovery_data($order_id = null)
        {
            ...
            if ($order) {
                ...
                $data = $this->send_abandonment_data('recovery', $order_id, $whatsapp, $name, $order_date);
                return $data;
            }
            return;
        }
