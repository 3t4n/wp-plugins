            ...
            <tr class="woom-field-row woom-field-row-<?php echo esc_attr($index + 1);?> <?php echo ($has_admin_row) ? 'has-admin-row' : ''; ?>">
                                ...
                                foreach ($field['options'] as $field_opt) {
                                    switch ($field_opt['type']) {
                                        ...
                                        case 'abandoned_coupon':
                                            ...
                                            printf('<button class="button button-small link" %1$s title="%3$s" onclick="woom_toggle_template_popup(event, this)">%2$s</button>', esc_attr($attr), esc_html($field_opt['name']), esc_attr($title));
                ...
                <?php
                    ...
                    if (isset($links['toggle_settings'])) {
                        printf('<tr class="woom-collapse-table"><td colspan="10"><table class="form-table woom-toggle-content-table">');
                        foreach ($links['toggle_settings'] as $settings_index => $toggle_settings) {
                            if (isset($links['toggle_prefixes'][$index])) {
                                ...
                                if (isset($links['toggle_prefixes'])) {
                                    ...
                                    printf('<td scope="row" class="woom-label">%1$s</td>', esc_html($toggle_settings['name']));
                                    printf('<td class="forminp woom-input-%2$s">%1$s</td>', wp_kses($this->woom_settings_field_template($toggle_settings, sprintf('%1$s_', esc_attr($links['toggle_prefixes'][$index]))), array(
                                        'input' => array('type' => array(), 'name' => array(), 'id' => array(), 'value' => array(), 'class' => array(), 'checked' => array()),
                                        'select' => array(
                                            'name' => array(),
                                            'id' => array(),
                                            'class' => array(),
