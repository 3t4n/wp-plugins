...
if (!class_exists('Wcm_Abandoned_Checker')) {
    class Wcm_Abandoned_Checker
    {
         ...
         **/
        public function get_abandoned_coupon_prefix()
        {
        ...
        public function trigger_abandoned_offer()
        {
            ...
            foreach ($orders as $order_id) {
                ...
                if (!$order->get_meta('woom_abandonment_order_created', true)) {
                    ...
                    $order->update_meta_data('woom_abandonment_order_created', current_time('timestamp'));
                    $order->save();
                    // Do something after initialise an order as abandoned 
                    do_action('woom_abandonment_order_created', $order_id);
                }
                if ($order->get_meta('woom_abandonment_order_created', true)) {
                    // Trigger abandoned message as per the rule of settings
                    $result = $this->send_abandoned_message($order_id);
                    // Do something after trigger message for abandoned order
                    do_action('woom_abandoned_message_after_trigger', $result, $order_id);
                } else {
                    // error logging if failed to initialise or get the order meta of woom_abandonment_order_created
                    // error_log(print_r(__("Failed to create meta data of woom_abandonment_order_created", 'wc-messaging'), true));
                }
            }
            return;
        }
        ...
        public function send_abandoned_message($order_id = 0)
        {
            ...
            foreach ($abandoned_triggers as $option_prefix => $option) :
                if (!empty($option['template'])) :
                    ...
                    $order_abandoned_created_time = floatval($order->get_meta('woom_abandonment_order_created', true));
                    ...
                    if (empty($order->get_meta($option_prefix . '_sent', true)) && $timestamp_diff > $option_duration_in_seconds) {
                        ...
                        if (isset($option['template'])) {
                            // get selected whatsapp template in settings table 
                            $template = $this->woom_whatsapp_class->get_message_template(null, $option['template']);
                        }
                        if (empty($template)) {
                            return $result = array('success' => false, 'message' => __('Please verify and update template. Otherwise something went wrong.', 'wc-messaging'));
                        }
                        $template_header_params = array();
                        $template_body_params = array();
                        $coupon_options = array();
                        // get header parameters if whatsapp template has header_params_count and table settings saved with header parameters
                        if (isset($template['header_params_count']) && $template['header_params_count'] > 0 && isset($option['header_params'])) {
                            // get header parameters with keys and values
                            $template_header_params = $this->woom_whatsapp_class->woom_get_mparams($type = "both", $method = "array", $order, $option['header_params']);
                            // get parameter keys if value is empty, and store to an array header
                            if (in_array('', array_values($template_header_params))) {
                                foreach ($template_header_params as $param_key => $param_val) {
                                    if (empty($param_val)) {
                                        $coupon_options['header'][] = $param_key;
                                    }
                                }
                            }
                        }

                        // get body parameters if whatsapp template has body_params_count and table settings saved with body parameters
                        if (isset($template['body_params_count']) && $template['body_params_count'] > 0 && isset($option['body_params'])) {
                            // get body parameters with keys and values
                            $template_body_params = $this->woom_whatsapp_class->woom_get_mparams($type = "both", $method = "array", $order,  $option['body_params']);
                            // get parameter keys if value is empty, and store to an array body
                            if (in_array('', array_values($template_body_params))) {
                                foreach ($template_body_params as $param_key => $param_val) {

                                    if (empty($param_val)) {
                                        $coupon_options['body'][] = $param_key;
                                    }
                                }
                            }
                        }
                        // list of options that get values after generate coupon
                        $coupon_avail_data = array('coupon_code', 'offer_amount', 'offer_type');
                        // boolean. any values of $coupon_avail_data exists in header parameters
                        $headers_has_coupon_data = (isset($coupon_options['header'])) ?  array_intersect($coupon_avail_data, $coupon_options['header']) : false;
                        // boolean. any values of $coupon_avail_data exists in body parameters
                        $body_has_coupon_data = (isset($coupon_options['body'])) ?  array_intersect($coupon_avail_data, $coupon_options['body']) : false;

                        /**
                         * Checking before generating couponcode
                         * condition 1
                         * header / body has empty parameters
                         * 
                         * condition 2
                         * generate coupon if:
                         *  headers empty parameters has coupon result values
                         *      OR 
                         *  body empty parameters has coupon result values
                         */
                        if (!empty($coupon_options) && (!empty($headers_has_coupon_data) || !empty($body_has_coupon_data))) {
                            $coupon_data = $this->get_message_params($order, $option_prefix);
                            // return if failed to generate coupon
                            if (empty($coupon_data)) {
                                return array('success' => false, 'message' => __('Failed to generate coupon', 'wc-messaging'));
                            }

                            // if header parameter has empty values and not empty and headers empty parameters has coupon result values
                            if (isset($coupon_options['header']) && !empty($coupon_options['header']) && !empty($headers_has_coupon_data)) {
                                foreach ($template_header_params as $param_key => $param_value) {
                                    if (empty($param_value) && isset($coupon_data[$param_key])) {
                                        $template_header_params[$param_key] = $coupon_data[$param_key];
                                    }
                                }
                            }
                            // if body parameter has empty values and not empty and body empty parameters has coupon result values
                            if (isset($coupon_options['body']) && !empty($coupon_options['body']) && !empty($body_has_coupon_data)) {
                                foreach ($template_body_params as $param_key => $param_value) {
                                    if (empty($param_value) && isset($coupon_data[$param_key])) {
                                        $template_body_params[$param_key] = $coupon_data[$param_key];
                                    }
                                }
                            }
                        }
                        ...
                        $result = $this->woom_whatsapp_class->send_message_template($phone, $template['name'], $template['language'], $template_body_params, $template_header_params);
                        // if trigger whatsapp template, create an order meta to mark as offer message triggered

                        if (isset($result['success']) && $result['success']) {
                            $order->update_meta_data($option_prefix . '_sent', 1);
                            $order->save();

                            $htmlspecifies = wp_specialchars_decode($this->woom_whatsapp_class->woom_get_whatsapp_template_by_name($template['name'], array('body' => $template_body_params, 'header' => $template_header_params)));
                            $msg = "--------------------------------------------\n" . $htmlspecifies;
                            $order->add_order_note(sprintf('%1$s - %2$s | %3$s %4$s', __('WC Messaging', 'wc-messaging'), ucfirst($order->get_status()), __('Customer abandoned cart notification', 'wc-messaging'), $msg), $is_customer_note = 0, $added_by_user = false);
                        } else {
                            do_action('woom_whatsapp_msg_sent_fail', array('result' => $result['message']));
                        }
                        return $result;
                    }
                endif;
         ...
         * @return void
         */
        function get_message_params($order_id = 0, $option_prefix = '')
        {
            if (empty($order_id) || empty($option_prefix)) {
                return array();
            }
            // get specific table row settings for coupon
            $coupon_settings = array(
                ...
                'expiry_in_seconds' => get_option($option_prefix . '_coupon_expiry_duration', 0),
            ...
            $abandoned_coupon = $this->get_abandoned_coupon($coupon_settings, $order_id);
         ...
         */
        function get_abandoned_coupon($coupon_data = array(), $order_id = null)
        {
            ...
            $coupon_code = uniqid($this->get_abandoned_coupon_prefix());
            $coupon = new WC_Coupon();
            $coupon->set_code($coupon_code);
            $coupon->set_description(__('WC Messaging Pro abandoned coupon', 'wc-messaging'));
            ...
            $abandoned_coupon_data = array(
                'coupon_code' => $coupon_code,
                'offer_amount' => $coupon_data['amount'],
                'offer_type' => $coupon_data['discount_type']
            );
            return $abandoned_coupon_data;
        }
    }
}
