...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (result.status_enable_disable !== "yes") {
    ...
    if (jQuery("#billing_autocomplete").length != 0) {
      // check if element exists.
      jQuery("#billing_autocomplete").val("");
    }

    if (
      jQuery("#billing_address_1").length != 0 &&
      result.address_autocomplete_field === "default"
    ) {
  ...
  if (jQuery(this).is(":checked") && result.status_enable_disable !== "yes") {
    if (jQuery("#ship-to-different-address-checkbox").is(":checked")) {
      ...
      jQuery("#shipping_state")
        ...
        .trigger("change")
        .attr("disabled", true);
      if (jQuery("#shipping_autocomplete").length) {
        // check if element exists.
        jQuery("#shipping_autocomplete").val("");
      }
      if (
        jQuery("#shipping_address_1").length != 0 &&
        result.address_autocomplete_field === "default"
      ) {
...
function initAutocomplete() {
  ...
  var billing_autocomplete_field =
    result.address_autocomplete_field === "default"
      ? "billing_autocomplete"
      : "billing_address_1";
  var shipping_autocomplete_field =
    result.address_autocomplete_field === "default"
      ? "shipping_autocomplete"
      : "shipping_address_1";
  billing_autocomplete = new google.maps.places.Autocomplete(
    /** @type {!HTMLInputElement} */ (
      document.getElementById(billing_autocomplete_field)
    ),
    { types: ["geocode"] }
  );
  if (document.getElementById(shipping_autocomplete_field)) {
    shipping_autocomplete = new google.maps.places.Autocomplete(
      /** @type {!HTMLInputElement} */ (
        document.getElementById(shipping_autocomplete_field)
      ),
      { types: ["geocode"] }
    );
...
function billing_fillInAddress() {
  ...
  for (var component in componentForm) {
    if (
      document.getElementById(billing_mappingForm[component]) &&
  ...
  if (
    ...
    for (var i = 0; i < place.address_components.length; i++) {
      ...
      if (componentForm[addressType]) {
        ...
        if (
          billing_mappingForm[addressType] === "billing_country" &&
    ...
    for (var i = 0; i < place.address_components.length; i++) {
      ...
      if (componentForm[addressType]) {
        ...
        if (addressType === "premise") {
          ...
          !isNZAddress &&
          billing_mappingForm[addressType] === "billing_state"
        ) {
          if (
            val === val.toUpperCase() &&
            jQuery(`#billing_state option[value=${val}]`).length > 0
          ) {
            // check if option exists in state dropdown.
          ...
          if (val != null && !isNZAddress) {
            document.getElementById(billing_mappingForm[addressType]).value =
              val;
          }
        }
      }
    ...
    if (result.street_address_behaviour !== "default") {
      ...
      document.getElementById("billing_address_1").value +=
        street_number + " " + route + " " + street_address_1;
    } else if (
      ["BR", "PL", "IT", "SE", "AT", "DE"].includes(
        jQuery("#billing_country").val()
      )
    ) {
      document.getElementById("billing_address_1").value =
        route + " " + street_number + " " + street_address_1;
    } else {
      document.getElementById("billing_address_1").value = null;
      if (
        premise === street_number.trim() + " " + route.trim() ||
        premise === ""
      ) {
        document.getElementById("billing_address_1").value =
          street_number + " " + route + " " + street_address_1;
        ...
        document.getElementById("billing_address_1").value =
          premise + " " + street_number + " " + route + " " + street_address_1;
      }
    }

    if (
      result.restrict_street_line == "yes" &&
    ...
    var street_address =
      doc.getElementsByClassName("street-address")?.[0]?.innerHTML;
    ...
    if (!isNZAddress) {
      for (var i = 0; i < addr_com_length; i++) {
        if (
          ...
          place.address_components[i].types[0] === "subpremise" ||
          place.address_components[i].types[0] === "street_address" ||
          place.address_components[i].types[0] === "route"
        ) {
          if (
            street_address?.includes(
...
function shipping_fillInAddress() {
  ...
  for (var component in componentForm) {
    if (
      document.getElementById(shipping_mappingForm[component]) &&
  ...
  for (var i = 0; i < place.address_components.length; i++) {
    ...
    if (componentForm[addressType]) {
      var val = place.address_components[i][componentForm[addressType]];
      if (
        shipping_mappingForm[addressType] === "shipping_country" &&
  ...
  for (var i = 0; i < place.address_components.length; i++) {
    ...
    if (componentForm[addressType]) {
      ...
      if (addressType === "premise") {
        ...
        !isNZAddress &&
        shipping_mappingForm[addressType] === "shipping_state"
      ) {
        if (
          val === val.toUpperCase() &&
          jQuery(`#shipping_state option[value=${val}]`).length > 0
        ) {
          // check if option exists in state dropdown.
        ...
        if (val != null && !isNZAddress) {
          document.getElementById(shipping_mappingForm[addressType]).value =
            val;
        }
      }
    }
  ...
  if (!isNZAddress) {
    for (var i = 0; i < addr_com_length; i++) {
      if (
        ...
        if (
          street_address.includes(place.address_components[i]["short_name"])
        ) {
          street_address = street_address.replace(
            place.address_components[i]["short_name"],
            place.address_components[i]["long_name"]
