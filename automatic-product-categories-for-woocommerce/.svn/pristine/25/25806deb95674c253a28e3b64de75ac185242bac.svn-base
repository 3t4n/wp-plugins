...
jQuery(document).ready(function($) {
	...
	function initRow($row) {
		...
		for (var i = 0; i < selectedConditions.length; ++i) {
			...
			$conditionSelect.siblings('select[name$="[match][' + i + ']"]:first').val(selectedConditions[i][1]).trigger('change');
			$conditionSelect.siblings(':input[name$="[value][' + i + '][]"]:first').val(selectedConditions[i][2][0]).trigger('change');
			if (selectedConditions[i][2].length > 1) {
				$conditionSelect.siblings(':input[name$="[value2][' + i + '][]"]:first').val(selectedConditions[i][2][1]).trigger('change');
			}
		}
