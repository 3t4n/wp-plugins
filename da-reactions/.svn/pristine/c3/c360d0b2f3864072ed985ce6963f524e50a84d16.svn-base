...
class MetaBox {
	 ...
	 * @since 1.0.0
	 */
	public function buildMetaBox( WP_Post $post ) {
		...
		?>
        <h3><?php esc_html_e( 'Global settings', 'da-reactions' ) ?></h3>
        <p><?php esc_html_e( 'Modify global settings for all contents of the same type', 'da-reactions' ); ?></p>
		<?php
		...
		?>
        <p>
            <input type="hidden" name="<?php echo esc_attr( $field_name ) ?>" value="off"/>
            <input id="id_<?php echo esc_attr( $field_name ) ?>" type="checkbox"
                   name="<?php echo esc_attr( $field_name ) ?>" <?php checked( $checked, 1 ) ?>
                   value="on"/>
            <label for="id_<?php echo esc_attr( $field_name ) ?>">
		        <?php esc_html_e( 'Enable reactions for all contents of this type globally?', 'da-reactions' ); ?>
            </label>
        </p>
		<?php
		if ( post_type_supports( $post->post_type, 'comments' ) ) {
            ...
            <p>
                ...
                <input id="id_<?php echo esc_attr( $field_name ) ?>" type="checkbox"
                       ...
                       value="on"/>
                <label for="id_<?php echo esc_attr( $field_name ) ?>">
		            <?php esc_html_e( 'Enable reactions for all comments under contents of this type globally?', 'da-reactions' ); ?>
                </label>
            </p>
			<?php
		}
		?>
        <hr/>
        <h3><?php esc_html_e( 'Statistics for this content', 'da-reactions' ) ?></h3>
        <canvas
                class="graph-canvas"
        ...
        <hr/>
        <h3><?php esc_html_e( 'Local settings', 'da-reactions' ) ?></h3>
		<?php
		...
		$checked        = isset( $saved_value ) && $saved_value === 'on';
		?>
        <p><?php echo esc_html( $fieldset_title ) ?></p>
        <p>
            <input type="hidden" name="<?php echo esc_attr( $field_name ) ?>" value="off"/>
            <input id="id_<?php echo esc_attr( $field_name ) ?>" type="checkbox"
                   name="<?php echo esc_attr( $field_name ) ?>" <?php checked( $checked, 1 ) ?>
                   value="on"/>
            <label for="id_<?php echo esc_attr( $field_name ) ?>">
		        <?php echo wp_kses( $field_icon, 'da-r-text' ) ?>
	...
	public function saveReactionsData( $post_id ) {
		...
		foreach ( $_POST[ $options_id ] as $key => $detail ) {
			$general_options->saveOption( filter_var( $key, FILTER_SANITIZE_FULL_SPECIAL_CHARS ), $detail );
		}
		return $post_id;
	}
}
