...
class Archive
{
    ...
    public function renderReactionsArchiveMenuItems()
    {
        ...
        <div id="da-reactions-plugin-div" class="posttypediv">
            <h4><?php esc_html_e( 'Archive by Reaction', 'da-reactions' ) ?></h4>
                        ...
                        <li>
                            <label class="menu-item-title">
                                <input type="checkbox" class="menu-item-checkbox"
                                       name="<?php echo esc_attr( $reaction->label ) ?>"
                                       value="<?php echo esc_url( get_site_url( null, self::ROUTE_PREFIX . '/' . $reaction->ID ) ) ?>">
                                <img src="<?php echo esc_attr( FileSystem::getImageUrl( $reaction->file_name ) ) ?>"
                                     alt="<?php echo esc_attr( $reaction->label ) ?>"
        ...
        <span class="add-to-menu">
            <input
                ...
                class="button-secondary submit-add-to-menu right"
                    value="<?php esc_attr_e('Add to Menu', 'da-reactions'); ?>"
    ...
    public function setReactionsArchiveTitle($title)
    {
        ...
        if ($reaction_id) {
            ...
            if ($reaction) {
                ...
                $title = sprintf(__('Archive for %s reaction', 'da-reactions'), $reaction->label);
            }
        }
        return $title;
    }
     ...
     * @return string
     */
    public function filterJoin($join) {
        $reaction_id = $this->getReactionId();
        if ($reaction_id) {
            global $wp_query;
            if ($wp_query->is_main_query()) {
                global $wpdb;
                $votes_table = Data::getVotesTable();
                $join .= " INNER JOIN $votes_table v ON $wpdb->posts.ID = v.resource_id AND $wpdb->posts.post_type = v.resource_type";
            }
        }
        return $join;
    }
     ...
     * @return string
     */
    public function filterOrderBy($args) {
        if ($this->shouldFilterQuery()) {
            $args = " total_reactions DESC";
        }
        return $args;
    }
}
