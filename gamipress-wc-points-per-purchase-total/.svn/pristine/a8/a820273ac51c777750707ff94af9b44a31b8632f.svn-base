...
function gamipress_wc_points_per_purchase_total_payment_complete( $order_id ) {
    ...
    foreach( gamipress_get_points_types() as $points_type => $data ) {
        ...
        // To use total or subtotal
        $option_order = gamipress_get_post_meta( $data['ID'], $prefix . 'checkout' );
        ...
        switch ( $option_order ) {
            case 'total':
                ...
                break;
            case 'subtotal':
                $points_to_award = absint( $order->get_subtotal() * $ratio );
                break;
            case 'shipping':
                $points_to_award = absint( ( $order->get_subtotal() + $order->get_shipping_total() ) * $ratio );
                break;
        }
            ...
            if( $register_on_user_earnings ) {
                ...
                gamipress_insert_user_earning( $user_id, array(
                    'title'	        => sprintf(
                        ...
                        strip_tags( wc_price( $order_price, array( 'currency' => get_woocommerce_currency() ) ) )
                    ),
                    'user_id'	    => $user_id,
                    'post_id'	    => $data['ID'],
