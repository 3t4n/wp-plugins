...
if( !$user_id ) {
    ...
    wp_add_inline_script( 'mine-cloudvod-user-script', 'jQuery(function(){debugger;
        if(window?.mcv_uc_general?.loginselector && jQuery(window.mcv_uc_general.loginselector).length>0){
            jQuery(window.mcv_uc_general.loginselector)[0].click();
            return;
        }
    });' );
    return;
        ...
        <div class="mcv-nav-area">
            <ul class="mcv-nav">
                ...
                <li class="mcv-nav-item"><a title="我的收藏" href="<?php 
                    ...
                    }else echo get_page_link(get_page_by_path('mcv-favorites')); ?>">我的收藏</a></li>
            </ul>
        </div>
    </aside>
    <?php endif; ?>
