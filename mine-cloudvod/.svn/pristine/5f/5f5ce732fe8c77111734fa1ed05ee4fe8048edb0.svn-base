...
?>
<div id="mcv_ketang_container" class="web index-placeholder-body<?php echo $is_iframe ? ' is-iframe' : ''; ?> style<?php echo MINECLOUDVOD_SETTINGS['mcv_lms_course']['lesson_style']??''; ?> ">
    <div class="study-header">
        <div class="mcv-breadcrumb">
            <a href="<?php echo get_post_type_archive_link(MINECLOUDVOD_LMS['course_post_type']); ?>"><?php echo __('All courses', 'mine-cloudvod'); ?></a>
            <?php if( $course_terms ): ?>
        ...
        <div class="operations">
            ...
            <?php
                ...
                if( $kefuqr ) echo '<div class="comment header-item qrtips" data-title="添加客服" data-qr="'.$kefuqr.'" data-tips="'.$kefutips.'"><span>客服</span></div>';
                $mpqr = MINECLOUDVOD_SETTINGS['mcv_lms_general']['mpqr']??'';
                $mptips = MINECLOUDVOD_SETTINGS['mcv_lms_general']['mptips']??'';
                if( $mpqr ) echo '<div class="comment header-item qrtips" data-title="关注公众号" data-qr="'.$mpqr.'" data-tips="'.$mptips.'"><span>公众号</span></div>';
                $qqgroup = get_post_meta( $course_id, 'qqgroup', true );
                $qqgrouptips = get_post_meta( $course_id, 'qqgrouptips', true );
                if( $qqgroup ) echo '<div class="comment header-item qqtips" data-title="QQ群" data-qq="'.$qqgroup.'" data-tips="'.$qqgrouptips.'"><span>QQ群</span></div>';
            ?>
            ...
            <div class="header-item profile">
                ...
                <ul class="profile-menu hidden">
                    ...
                    if( isset(MINECLOUDVOD_SETTINGS['mcv_lms_general']['user_courses']) && MINECLOUDVOD_SETTINGS['mcv_lms_general']['user_courses'] ){
                        echo get_page_link( MINECLOUDVOD_SETTINGS['mcv_lms_general']['user_courses'] );
                    }else echo get_page_link(get_page_by_path('mcv-my-courses')); 
                    ...
                    if( isset(MINECLOUDVOD_SETTINGS['mcv_lms_general']['fav_courses']) && MINECLOUDVOD_SETTINGS['mcv_lms_general']['fav_courses'] ){
                        echo get_page_link( MINECLOUDVOD_SETTINGS['mcv_lms_general']['fav_courses'] );
                    }else echo get_page_link(get_page_by_path('mcv-favorites')); ?>"
