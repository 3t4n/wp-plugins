    ...
    public function init()
    {
        add_action('admin_enqueue_scripts',    function ($hook) {
            ...
            if( $current_screen->post_type == 'mcv_video' ){
                ...
                wp_add_inline_script('clipboard', 'jQuery(function(){var clipboard = new ClipboardJS(".mcv_copy");clipboard.on("success", function(e) {var cur = jQuery(".mcv_copy[data-clipboard-text=\'"+e.text+"\']");cur.css("color","green");cur.after("<span>' . __('Copied', 'mine-cloudvod') . '</span>");setTimeout(function() {cur.next("span").remove();}, 2000);e.clearSelection();});});');
