...
class B_2
{
    ...
    public function get_user_id( $uid, $data ){
        global $wpdb;
        $res = $wpdb->get_var( 'SELECT user_id FROM '. $wpdb->usermeta . ' WHERE meta_key=\'zrz_weixin_uid\' AND meta_value=\''.$data['unionid'].'\'' );
        if( !$res ) $res = $wpdb->get_var( 'SELECT user_id FROM '. $wpdb->usermeta . ' WHERE meta_key=\'zrz_weixin_unionid\' AND meta_value=\''.$data['unionid'].'\'' );
        if( $res ) $uid = $res;
        return $uid;
    }
