...
namespace AC_SYNC\Includes\Classes\Product {
  ...
  class Action_Wear_Product
  {
    ...
    public static function getRechargedPrice($conf, $price)
    {
      ...
      $categories = (array) wc_get_product_category_list($conf->get_id());
    ...
    public static function createVariantBySimple($product, $simple, $conf, $dimensions, $cover_id)
    {
      ...
      if (isset($dimensions)) {
        if (isset($dimensions->l)) {
          ...
          $variation->set_width($dimensions->l);
        }
        if (isset($dimensions->h))
          $variation->set_height($dimensions->h);
      }
      if ($cover_id !== false)
      ...
      if ($res === TRUE) {
        ...
        if ($res === TRUE) {
          ...
          foreach ($listino->hierarchical as $sku => $product) {
            ...
            if (!empty($cover)) {
              $cover = str_replace("://cdn.action-wear.com", "://media.action-wear.com", $cover);
            }
    ...
    public static function getUrlListino()
    {
      ...
      $body = $all_products["raw"]["body"];
      $body = stripslashes(trim($body, '"'));

      // check if $body is valid url
