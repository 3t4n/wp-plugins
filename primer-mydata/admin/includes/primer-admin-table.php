...
class PrimerReceipts extends WP_List_Table {
		...
		if ($column_name !== 'receipt_date' && $column_name !== 'receipt_status' && $column_name !== 'receipt_id' && $column_name !== 'credit_receipt' && $column_name !== 'credit_log') {
			...
			if ($column_name == 'receipt_status') {
				if (!empty($item[$column_name])) {
                    ...
                    $escape = '<a href="' . $item[$column_name] . '" target="_blank" class="order-view"><strong>' . __('Log', 'primer') . '</strong></a>';
                    echo wp_kses($escape,$allowed_html);
				} else {
					echo '';
				}
			}
	...
	function extra_tablenav($which):void {
		if ($which !== 'bottom') {
                ...
                <div class="filter_blocks_wrapper">
                    <div class="left_wrap">
                        <div class="filter_block">
                                <label for="primer_order_year" style="float: left;"><?php esc_html_e('Year: ', 'primer'); ?></label>
                                <select name="primer_order_year" id="primer_order_year">
                                    <?php
                                    $year_from = !empty($orders_dates = $primer_orders->get_dates_from_orders()) ? date('Y', min($orders_dates)) : '';
                                    $year_to = date('Y');
                                    $primer_order_year = isset($_GET['primer_order_year']) ? sanitize_text_field($_GET['primer_order_year']) : $year_to;
                        ...
                        <div class="filter_block">
                            <label for="primer_order_client" style="float: left;"><?php esc_html_e('Client: ', 'primer'); ?></label>
                                <select name="primer_order_client" id="primer_order_client" data-placeholder="<?php esc_html_e('Select clients', 'primer'); ?>">
                                    <option value=""></option>
                                    <?php
                                    ...
                                    foreach ( $order_customers as $primer_orders_customer => $order_customer ) {
                                        if ($order_customer['order_client_id']) { ?>
                                            <option value="<?php echo esc_attr($order_customer['order_client_id']); ?>" <?php selected($get_customer, $order_customer['order_client_id']); ?>><?php echo esc_attr($order_customer['order_client']); ?></option>
                                        <?php } else { ?>
                                            <option value="<?php echo esc_attr($order_customer['order_client_id']); ?>" <?php selected($get_customer, $order_customer['order_client_id']); ?>><?php esc_html_e('Guest client', 'primer'); ?></option>
                                        <?php }
                                    } ?>
                                </select>
                        </div>
                    ...
                    <div class="right_wrap">
                        <div class="filter_block">
                                    <label for="primer_order_status" style="float: left;"><?php esc_html_e('Order Status: ', 'primer'); ?></label>
                                    <select name="primer_order_status" title="<?php esc_html_e('Select order status', 'primer'); ?>" id="primer_order_status">
                                    <option selected disabled><?php esc_html_e('Select order status', 'primer'); ?></option>
                                    <?php
                                    ...
                                    foreach ( $status_of_orders as $status_k => $status_value ) { ?>
                                        <option value="<?php echo esc_attr($status_k); ?>" <?php
                                             selected($status_k, $get_order_status);?>
                                             ><?php echo esc_attr($status_value); ?></option>
                                    <?php }
                                    ?>
                                    </select>
                        </div>
                        <div class="filter_block">
                                ...
                                $status_of_receipts = array(
                                        ...
                                        'not_issued' => __('Not Issued', 'primer'),
                                        'transmission_failure_check' => __('Transmission Failure', 'primer')
                                    );
                        ...
                        <div class="apply_btn btn_order"><input type="submit" class="button" id="filter_action" name="filter_action" value="<?php esc_html_e('Apply filter', 'primer'); ?>" /></div>
                    </div>
                </div>
            ...
            if (!(in_array(3, $primer_licenses['wpModules']))) {
                echo '<h4><i>'. esc_html(__('Orders for invoices are not shown because license key does not support them', 'primer')) .'</i></h4>';
            }
            if (!(in_array(4, $primer_licenses['wpModules']))) {
                echo '<h4><i>'. esc_html(__('Orders for invoices-receipts within E.U or out E.U. are not shown because license key does not support them', 'primer')).'</i></h4>';
            }
             ?>
            </div>
            ...
            <?php
             $formatted_min_order_date = '';
             $formatted_max_order_date = '';
            ...
            <script>
                jQuery(document).ready(function ($) {
                    ...
                    let select_year = $('select[name="primer_order_year"]').val();
                    let min_order_date = "<?php echo esc_attr($formatted_min_order_date); ?>";
                    let max_order_date = "<?php echo esc_attr($formatted_max_order_date); ?>";
                    let min_year_range = new Date(min_order_date).getFullYear();
                    let date_from = $('input[name="order_date_from"]'),
                    ...
                    $('input[name="order_date_from"], input[name="order_date_to"]').datepicker({
                        ...
                        changeYear: true,
                        dateFormat: "yy-mm-dd",
                        minDate: new Date(min_order_date),
                        maxDate: "now",
                        yearRange: min_year_range + ":now"
                    });

                    $('input[name="order_date_from"]').datepicker("option", "minDate", new Date(min_order_date));
                    $('input[name="order_date_to"]').datepicker("option", "maxDate", new Date(max_order_date));
	...
	function prepare_items():void {
		...
		$data                   = $this->items;
        $total_items            = $total_orders ?? array_sum((array)wp_count_posts('shop_order'));
        $this->items            = $data;
	...
	function no_items():void {
		esc_html_e('No orders found.', 'primer');
	}

	function process_bulk_action():void {
		$orders         = isset( $_REQUEST['wp_ajax_list_order'] ) ?? array();
	...
	function show_all_orders():string {
		ob_start();
	...
	function handle_main_primer_admin_menu():void {
		...
		<div class="wrap primer-admin-menu-wrap">
			<div class="plugin_caption_version"><?php echo esc_attr(PRIMER_NAME . ' v'. PRIMER_VERSION); ?></div>
		<?php

		 if ($_GET['page'] === 'wp_ajax_list_order') { ?>
		 	<h2><?php esc_html_e('Orders', 'primer'); ?>
			<?php
			do_action('primer_menu_nav_tabs', $action); ?>
			</h2>
			<?php
...
function convert_select_orders() {
	...
	if (!empty($orders)) {
		foreach ($orders as $order_id) {
            ...
            if (get_post_meta($id_of_order, 'receipt_status', true) == 'issued' && (get_post_meta($id_of_order, 'transmission_failure_check', true) == null || get_post_meta($id_of_order, 'transmission_failure_check', true) == 2)) {
            ...
            while ($i <= 1) {
                ...
                if ($response_code == 400 && json_decode($response_message, true)['response'][0]['errors']['error'][0]['code'] == 228 ) {
...
function fetch_primer_script():void {
...
function check_zone_country($country):bool {
