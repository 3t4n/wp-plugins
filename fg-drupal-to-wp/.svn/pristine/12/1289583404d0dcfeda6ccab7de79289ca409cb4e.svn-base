...
SQL;
		...
		public function get_drupal_info() {
			...
			// Categories
			$cat_count = $this->get_taxonomies_terms_count('categories') + $this->get_taxonomies_terms_count('category');
		...
		private function import() {
			if ( $this->drupal_connect() ) {
				...
				if ( !isset($this->premium_options['skip_taxonomies']) || !$this->premium_options['skip_taxonomies'] ) {
					...
					$cat_count = $this->import_taxonomies_terms('categories') + $this->import_taxonomies_terms('category');
		...
		private function get_total_elements_count() {
			...
			// Taxonomies
			if ( !isset($this->premium_options['skip_taxonomies']) || !$this->premium_options['skip_taxonomies'] ) {
				$count += $this->get_taxonomies_terms_count('categories') + $this->get_taxonomies_terms_count('category');
		 ...
		 */
		public function get_node($nid, $entity_type='node') {
			$node = array();
			$prefix = $this->plugin_options['prefix'];
			$extra_joins = '';
			$language_field = 'n.language';
			...
			if ( !$this->plugin_options['skip_media'] ) {
				...
				if ( !$this->plugin_options['only_featured_image'] ) {
					...
					if ( !empty($media_gallery) ) {
						...
						// Create the gallery
						$gallery_shortcode = $this->build_gallery_block(array_values($media_gallery));
		 ...
		 * @since 3.83.0
		 * 
		 * @param array $images_ids Images IDs
		 * @param int $nb_images_per_row Number of images to display per row
		 * @return string Gutenberg block
		 */
		public function build_gallery_block($images_ids, $nb_images_per_row=3) {
			$figures = array();
...
END;
				$figures[] = <<<END
					...
					<!-- wp:image {"lightbox":{"enabled":true},"id":$image_id,"sizeSlug":"medium","linkDestination":"none","className":"is-style-default"} -->
...
END;
			...
			$figures_str = implode("\n", $figures);
			$link_attribute = ',"linkTo":"media"';
			$block = <<<END

<!-- wp:gallery {"columns":{$nb_images_per_row}{$link_attribute}} -->
	<figure class="wp-block-gallery has-nested-images columns-$nb_images_per_row is-cropped">
		$figures_str
	</figure>
<!-- /wp:gallery -->
END;
			$block = apply_filters('fgd2wp_build_gallery_block', $block, $images_ids, $nb_images_per_row);
			return $block;
		}
		...
		/**
		 * Get the files from an object (node, comment)
		 * 
		 * @since 3.84.0
		 * 
		 * @param int $id Node ID or comment ID
		 * @param string $type Object type (node, comment)
		 * @return array File data
		 */
		public function get_files_from_object_id($id, $type) {
			$files = array();
			$prefix = $this->plugin_options['prefix'];
			if ( $this->table_exists('file_usage') ) {
				$sql = "
					SELECT f.fid, f.uid, f.filename, f.uri, f.filemime, f.timestamp
					FROM {$prefix}file_managed f
					INNER JOIN {$prefix}file_usage u ON u.fid = f.fid
					WHERE u.id = '$id'
					AND u.type = '$type'
				";
				$files = $this->drupal_query($sql);
			}
			return $files;
		}
