  ...
  class PluginClass extends \Puvox\wp_plugin
  {
		...
		if(!defined("ewdfad_included"))
		{
			...
			if( strpos($wp_config_content, $this->phrase_start) === false )
			{
				...
				"\r\n".'if(file_exists($a=__DIR__."/'. 
					str_replace( $this->helpers->slash_normalize_path(ABSPATH), '', $this->helpers->slash_normalize_path( $this->wp_congif_addon_php ))
				.'") && include_once($a)){  '.
	...
	public function my_head()
	{
		...
		function redirect_to_ewdfad(el){   
			let url = el.href;
			const p1 = prompt('<?php _e('1 [enables:  WP_DEBUG]\n2 [enables:  WP_DEBUG + WP_DEBUG_DISPLAY]\n3 [enables:  WP_DEBUG + WP_DEBUG_DISPLAY + WP_DEBUG_LOG]','enable-wp-debug-from-dashboard');?>', 2);
			if (!p1) return;
			const p2 = confirm('<?php _e('Click "OK" if it should be only enabled for your IP (otherwise, clicking the "cancel" will turn on wp-debug for everyone for 24 hours)\n','enable-wp-debug-from-dashboard');?>');
			url = url + '&ewdfad_debug_type='+ p1 + '&ewdfad_ip_type='+( p2 ? 1 : 0) ;  
			window.location = url;
		}
		</script>
		<?php 
