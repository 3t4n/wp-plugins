...
class Easygram_Widget extends WP_Widget {
	 ...
	 * @var      string
	 */
	protected $widget_slug = 'easygram';
	 ...
	 * @var      string
	 */
	protected $widget_text_domain = 'easygram-widget';
	...
	public function __construct() {
		...
		parent::__construct(
			$this->widget_slug,
			__( 'Easygram Widget', $this->widget_text_domain ),
			array(
		...
		$this->defaults = apply_filters( 'easygram_widget_modify_defaults', $this->defaults );
	...
	public function widget( $args, $instance ) {
		...
		$photos = $this->get_photos( $instance['username_hashtag'] );
		...
		ob_start();
		include $this->get_template( EASYGRAM_WIDGET_DIR .'views/widget_html' );
	...
	public function form( $instance ) {
		...
		include EASYGRAM_WIDGET_DIR . 'views/admin_html.php';
	...
	/**
	 * Get photos from Instagram base on username or hashtag with transient caching for one day
	 *
	 * @since 1.0.0
	 *
	 ...
	 */
	protected function get_photos( $usernames_or_hashtags ) {
	...
	function generate_transient_key( $usernames_or_hashtags ) {
		...
		$transient_key = md5( 'easygram_widget_' . $id );
	...
	public function register_widget_styles() {
		wp_enqueue_style( $this->widget_slug.'-widget-styles', EASYGRAM_WIDGET_URL . 'css/widget.css' );
	}
	...
	public function register_admin_script() {
		wp_enqueue_script( $this->widget_slug.'-admin-script', EASYGRAM_WIDGET_URL . 'js/admin.js', true, EASYGRAM_WIDGET_VER );
		wp_enqueue_style( $this->widget_slug.'-admin-styles', EASYGRAM_WIDGET_URL . 'css/admin.css' );
	}
