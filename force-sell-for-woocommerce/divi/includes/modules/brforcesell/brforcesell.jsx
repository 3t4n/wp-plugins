...
class BRFS_forcesell extends Component {

  static slug = 'et_pb_brforcesell';
  static parameters = ['product_id',
            'display_force_sell',
            'display_force_sell_linked',
            'display_as_link'];
  constructor(props) {
    super(props);
  ...
  componentDidUpdate(oldProps) {
      var update = false;
      BRFS_forcesell.parameters.forEach(key => {
  ...
  componentDidMount() {
    ...
    body.append('action', 'brfocesell_divi_module');
      ...
      .then(
        (result) => {
          if( typeof(result) === 'undefined' || ! result ) {
              this.htmlstate = (<div style={{padding:"2em 0", background: "#6c2eb9", color: "#fff", fontSize: "12px", fontWeight: "600", verticalAlign: "middle", textAlign: "center", borderRadius: "1em"}}><h3 style={{color: "#000", textShadow: "1px 0px white, -1px 0px white, 0px 1px white, 0px -1px white", fontWeight: "900"}}>BeRocket Force Sell</h3>Force Sell not displayed in Builder</div>);
              this.setState({
                isLoaded: true
              });
          } else {
