...
if ( !class_exists( 'Nedwp_Feedback_Modal_Fonts' ) ) {

	class Nedwp_Feedback_Modal_Fonts {
		
		public function get_list() {

			$data_fonts = get_transient( 'nedwp_fm_google_fonts' );

			if ( false === $data_fonts ) {

		        $api_uri = 'http://api.nedwp.com/google-fonts?source=nedwp_fm';
		        $api_result = wp_remote_get( $api_uri, array( 'sslverify' => false ) );

		        if ( is_array( $api_result ) && !empty( $api_result['body'] ) ) {
			        $data_fonts = json_decode( $api_result['body'], true );
			        set_transient( 'nedwp_fm_google_fonts', $data_fonts, 12 * HOUR_IN_SECONDS );
		        }

			}

			$data_fonts = ( !empty( $data_fonts ) ) ? $data_fonts : $this->fallback_libs();

			return (array) apply_filters( 'nedwp_fm_fonts', $data_fonts );
		}

		public function fallback_libs() {
			return array(
				'ABeeZee' => 'ABeeZee',
				'Abel' => 'Abel',
