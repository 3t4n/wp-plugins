...
class WebdWoocommerceBProducts {
	...
	public function vocabularySelect() {
		...
		print "<option value='' >" . esc_html__( 'Select..', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . '</option>';
	...
	public function editProductsDisplay() {
		?>
		<h2><?php esc_html_e( 'EDIT SIMPLE PRODUCTS', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></h2>	
		<?php
			$query = new WP_Query(
				array(
					'post_type'      => 'product',
			...
			?>
				<p>
					<span class='toggler button button-primary  '>
						<?php esc_html_e( 'Filter Products', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?> <i class='fa fa-filter '></i>
					</span>  
					<span class='togglerMassive button button-secondary warning btn btn-danger '>
						<?php esc_html_e( 'Massive Update', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?> <i class='fa fa-edit '></i>
					</span>
				</p>
				...
				<form id='selectTaxonomy' action= "<?php echo admin_url( 'admin.php?page=webd-woocommerce-product-excel-importer-bulk-edit&tab=search-edit' ); ?>" >
					<table class='wp-list-table widefat fixed table table-bordered'>	
						<tr>
							<td><?php esc_html_e( 'Choose Taxonomy', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></td>
				...
				<form name='editProductDisplay' id='editProductDisplay' method='post' action= "<?php echo admin_url( 'admin.php?page=webd-woocommerce-product-excel-importer-bulk-edit&tab=search-edit' ); ?>" >
					<table class='wp-list-table widefat fixed table table-bordered'>	
						...
						<?php if ( isset( $_REQUEST['vocabularySelect'] ) ) { ?>		
							<td><?php esc_html_e( 'Tax Terms', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></td>
							<td>
							<input type='hidden' name ='vocabularySelect' value='<?php print sanitize_text_field( $_REQUEST['vocabularySelect'] ); ?>' />
							<?php
							$terms = get_terms(
								sanitize_text_field(
									$_REQUEST['vocabularySelect'],
									array(
										'hide_empty' => true,
									)
								)
							...
							if ( count( $terms ) < 1 ) {
								...
								print "<option value='' >" . esc_html__( 'Choose Terms..', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . '</option>';
						...
						?>
							...
							<tr>
								<td><?php esc_html_e( 'Keywords', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></td>
								<td><input type='text' name='keyword' id='keyword' placeholder='Search term'/></td>
							</tr>
							<tr>
								<td><?php esc_html_e( 'SKU', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></td>
								<td><input type='text' name='sku' id='sku' placeholder='Search by SKU'/></td>
							</tr>
							<tr>
								<td><?php esc_html_e( 'Regular Price', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></td><td><input type='number' name='regular_price' id='regular_price' placeholder='Regular Price'/></td>
								<td><?php esc_html_e( 'Regular Price Selector', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></td>
							<td>
							...
							<tr>
								<td><?php esc_html_e( 'Sale Price', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></td>
								<td><input type='number' name='sale_price' id='sale_price' placeholder='Sale Price'/></td>								
								<td><?php esc_html_e( 'Sale Price Selector', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></td>
								<td>
							...
							<tr>
								<td><?php esc_html_e( 'Products Per Page', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?> </td><td><input type='number' min="1" max="100" style='width:100%;' required name='posts_per_page' id='posts_per_page' placeholder='100 max' value='100'/></td><td></td><td></td>
							</tr>
							
							<tr>
								<td><?php esc_html_e( 'Offset Products', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></td><td><input type='number' name='offset' min="1" max="100" style='width:100%;' id='offset' placeholder='Start from..'/></td>
								<td></td><td></td>
							</tr>
					...
					<table class='wp-list-table widefat fixed table table-bordered'>
					<legend>
						<h2><?php esc_html_e( 'TAXONOMIES TO SHOW', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></h2>
					</legend>
					<?php
					...
					?>
					<table class='wp-list-table widefat fixed table table-bordered'>
						<legend><h2><?php esc_html_e( 'FIELDS TO SHOW', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></h2></legend>
						<?php
						$cols = array( 'title', 'description', '_sku', '_regular_price', '_sale_price', '_weight', '_stock', '_stock_status', '_width', '_length', '_height' );
						print '<tr>';
						...
						submit_button( esc_html__( 'Search', 'webd-woocommerce-product-excel-importer-bulk-edit' ), 'primary', 'Search' );
						?>
										</form>
			<?php
			$this->updateMassiveForm();
			?>
			<div class='result'><?php $this->editProducts(); ?></div>
															<?php

		} else {
			print "<p class='warning'>" . esc_html__( 'No products found', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . '.</p>';
		}
	}
	...
	public function editProducts() {
		...
		if ( $_SERVER['REQUEST_METHOD'] == 'POST' && current_user_can( 'administrator' ) && $_REQUEST['searchProductList'] ) {
			...
			if ( $query->have_posts() ) {
						...
						?>
							...
							while ( $query->have_posts() ) {
								...
								<a href='<?php print esc_url( get_permalink( get_the_ID() ) ); ?>' target='_blank'>
									<i class='fa fa-eye' ></i> <?php esc_html_e( 'View', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?>								
								</a>  
								<a href='<?php print esc_url( get_edit_post_link( get_the_ID() ) ); ?>' target='_blank'>
									<i class='fa fa-pencil' ></i> <?php esc_html_e( 'Edit', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?>								
								</a>								
				...
				print "<p class=' error'>No Simple Product Found. For variable products please upgrade to <a  target='_blank'  href='https://webdeveloping.gr/product/woocommerce-product-excel-importer-bulk-editing-pro'>PRO VERSION</a>.</p>";// end if
			}
		}
		...
		<div>
			<form name='updateMassive' id='updateMassive' method='post' action= "<?php echo admin_url( 'admin.php?page=webd-woocommerce-product-excel-importer-bulk-edit&tab=search-edit' ); ?>">
				...
				<tr>
					...
					<td><label for='regular_price'><?php esc_html_e( 'Regular Price', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></label></td>
					<td><input step="0.01"  type='number' id='bulk_regular_price' name='regular_price' value='<?php print $this->regular_price; ?>' /></td>
					<td><label for='regular_operator'><?php esc_html_e( 'How to change Regular Price', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></label></td>
					<td>
						
						<select disabled name='regular_operator' id='regular_operator'>
							<option value='ea'> <?php esc_html_e( 'Exact Amount', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></option>
							<option value='mfa'><?php esc_html_e( 'Minus fixed amount', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></option>
							<option value='pfa'><?php esc_html_e( 'Plus fixed amount', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></option>
							<option value='mpa'><?php esc_html_e( 'Minus % amount', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></option>
							<option value='ppa'><?php esc_html_e( 'Plus % amount', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></option>
						</select>
					</td>					
				</tr>

				<tr>
					<td><label for='sale_price'><?php esc_html_e( 'Sale Price', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></label></td>
					<td><input min='0' step="0.01" type='number' id='bulk_sale_price' name='sale_price' value='<?php print $this->sale_price; ?>' /></td>
					<td><label for='sale_operator'><?php esc_html_e( 'How to change Sale Price', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></label></td>
					<td>						
				...
				<tr>					
					<td><label for='stock'><?php esc_html_e( 'Stock', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></label></td>
	...
	public function updateFromList() {
		if ( $_SERVER['REQUEST_METHOD'] == 'POST' && current_user_can( 'administrator' ) && isset( $_REQUEST['submitList'] ) ) {
			...
			if ( isset( $_REQUEST['_sku'] ) && ! empty( $_POST['_sku'] ) ) {
				update_post_meta( $this->id, '_sku', sanitize_text_field( $_POST['_sku'] ) );
			}
			if ( isset( $_REQUEST['_regular_price'] ) && ! empty( $_POST['_regular_price'] ) ) {
				update_post_meta( $this->id, '_regular_price', sanitize_text_field( $_POST['_regular_price'] ) );
			}
			if ( isset( $_REQUEST['_sale_price'] ) && ! empty( $_POST['_sale_price'] ) ) {
				...
				update_post_meta( $this->id, '_price', sanitize_text_field( $_POST['_regular_price'] ) );
			}
			if ( isset( $_REQUEST['_stock'] ) && ! empty( $_POST['_stock'] ) ) {
				update_post_meta( $this->id, '_stock', sanitize_text_field( $_POST['_stock'] ) );
			}
			if ( isset( $_REQUEST['_stock_status'] ) && ! empty( $_POST['_stock_status'] ) ) {
				update_post_meta( $this->id, '_stock_status', sanitize_text_field( $_POST['_stock_status'] ) );
			}
			if ( isset( $_REQUEST['_length'] ) && ! empty( $_POST['_length'] ) ) {
				update_post_meta( $this->id, '_length', sanitize_text_field( $_POST['_length'] ) );
			}
			if ( isset( $_REQUEST['_width'] ) && ! empty( $_POST['_width'] ) ) {
				update_post_meta( $this->id, '_width', sanitize_text_field( $_POST['_width'] ) );
			}
			if ( isset( $_REQUEST['_height'] ) && ! empty( $_POST['_height'] ) ) {
				update_post_meta( $this->id, '_height', sanitize_text_field( $_POST['_height'] ) );
			}
			if ( isset( $_REQUEST['_weight'] ) && ! empty( $_POST['_weight'] ) ) {
				update_post_meta( $this->id, '_weight', sanitize_text_field( $_POST['_weight'] ) );
			}
