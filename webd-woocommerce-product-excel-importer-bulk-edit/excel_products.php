...
class WebdWoocommerceEProducts {
	...
	public function importProductsDisplay() {
		?>
		<h2><?php esc_html_e( 'IMPORT / UPDATE PRODUCTS', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></h2>
		<div>    
			
			<p><?php _e( 'Download the sample excel file, save it and add your products. You can add your Custom Columns. Upload it using the form below.', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?> <a href='<?php echo plugins_url( '/example_excel/import_update_products.xlsx', __FILE__ ); ?>'>SAMPLE EXCEL FILE</a></p>
	...
	public function importProducts() {
		...
		if ( isset( $_SERVER['REQUEST_METHOD'] ) && $_SERVER['REQUEST_METHOD'] == 'POST' && current_user_can( 'administrator' ) && ! isset( $_POST['finalupload'] ) ) {
			...
			if ( isset( $_FILES['file']['size' ] ) && $_FILES['file']['size'] > 0 ) {
				...
				<p style='font-style:italic'><?php esc_html_e( ' DATA MAPPING: Drag and drop excel columns on the right to product properties on the left, OR ', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?><strong><i> <?php esc_html_e( 'Auto Match Columns', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?> <input type='checkbox' name='automatch_columns' id='automatch_columns' value='yes'  /> </i></strong></p>
				
								 
					<?php
					print "<div style='float:right;width:50%'>";
						print '<h3>' . esc_html__( 'EXCEL COLUMNS', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . '</h3><p>';
					...
					?>
									<p class='hideOnUpdateById'>
										<input type='checkbox' name='selectparent' id='selectparent' value='yes'  /> <b> <?php esc_html_e( 'Select Parent Categories as well ', 'webd-woocommerce-product-excel-importer-bulk-edit' ); ?></b>
									</p>				
					
					<?php
					print '<h3>' . esc_html__( 'PRODUCT FIELDS', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . '</h3>';

					echo '<p>' . esc_html__( 'POST AUTHOR', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input type='text' name='post_author' required readonly class='droppable' placeholder='Drop here column' /></p>";
					echo '<p>' . esc_html__( 'POST NAME', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input type='text' name='post_name' required readonly class='droppable' placeholder='Drop here column' /></p>";

					echo '<p>' . esc_html__( 'POST TITLE', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input type='text' name='post_title' required readonly class='droppable' placeholder='Drop here column' /></p>";
					echo '<p>' . esc_html__( 'POST STATUS', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input type='text' name='post_status' required readonly class='droppable' placeholder='Drop here column' /></p>";
					echo '<p>' . esc_html__( 'POST CONTENT', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input type='text' name='post_content' required readonly class='droppable' placeholder='Drop here column'  /></p>";
					echo '<p>' . esc_html__( 'POST EXCERPT', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input type='text' name='post_excerpt' required readonly class='droppable' placeholder='Drop here column'  /></p>";
					...
					echo '<p>' . esc_html__( 'IMAGE', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input style='border:1px solid red;background:#ccc;' type='text' style='min-width:200px' name='image' required readonly class='' placeholder='Premium Version Only'  /></p>";
					echo '<p>' . esc_html__( 'IMAGE GALLERY', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input style='border:1px solid red;background:#ccc;' type='text' style='min-width:200px' name='image' required readonly class='' placeholder='Premium Version Only'  /></p>";
					echo '<p>' . esc_html__( 'VIRTUAL', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . "<input type='text' style='min-width:200px' name='_virtual' required readonly class='droppable' placeholder='Downloadable Product'  /></p>";
					echo '<p>' . esc_html__( 'DOWNLOADABLE', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input style='border:1px solid red;background:#ccc;' type='text' style='min-width:200px' name='image' required readonly class='' placeholder='Premium Version Only'  /></p>";
					echo '<p>' . esc_html__( 'PURCHASE NOTE', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input style='border:1px solid red;background:#ccc;' type='text' style='min-width:200px' name='image' required readonly class='' placeholder='Premium Version Only'  /></p>";
					echo '<p>' . esc_html__( 'UPSELL IDS', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . "<input style='border:1px solid red;background:#ccc;' type='text' style='min-width:200px' name='image' required readonly class='' placeholder='Premium Version Only'  /></p>";
					echo '<p>' . esc_html__( 'CROSELL IDS', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input style='border:1px solid red;background:#ccc;' type='text' style='min-width:200px' name='image' required readonly class='' placeholder='Premium Version Only'  /></p>";
					echo '<p>' . esc_html__( 'TAXABLE', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input style='border:1px solid red;background:#ccc;' type='text' style='min-width:200px' name='image' required readonly class='' placeholder='Premium Version Only'  /></p>";
					echo '<p>' . esc_html__( 'TAX CLASS', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . " <input style='border:1px solid red;background:#ccc;' type='text' style='min-width:200px' name='image' required readonly class='' placeholder='Premium Version Only'  /></p>";
					print '<h3>' . esc_html__( 'CATEGORY AND TAGS', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . '</h3>';
					...
					'<h3>' . esc_html_e( 'Invalid File:Please Upload Excel File', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . '</h3>';
				}
			}
		}
			...
			if ( ! empty( $_POST['post_title'] ) ) {
				...
				for ( $i = 2;$i <= $data;$i++ ) {
					...
					if ( post_exists( $title ) === 0 ) {
						...
						print "<p class='success'><a href='" . esc_url( get_permalink( $id ) ) . "' target='_blank'>" . $title . '</a> ' . esc_html__( 'created', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . '.</p>';
					} else {
						// update
						...
						if ( $content != '' ) { // if column selected update, otherwise dont update
							...
							wp_update_post( $post );
							print "<p class='warning'><a href='" . esc_url( get_permalink( $id ) ) . "' target='_blank'>" . $title . '</a> ' . esc_html__( 'already exists. Updated', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . '.</p>';
					}
					...
					if ( ! empty( $allDataInSheet[ $i ][ $_POST['_virtual'] ] ) ) {
				...
				print "<h3 class='warning' >" . esc_html__( 'No title selected for your products', 'webd-woocommerce-product-excel-importer-bulk-edit' ) . '.</h3>';
			}
