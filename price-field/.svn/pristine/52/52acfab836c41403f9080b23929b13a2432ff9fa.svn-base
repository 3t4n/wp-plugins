...
function ic_block_render_price_filter( $atts = array() ) {
	...
	if ( ( empty( $block_content ) || ic_string_contains( $block_content, 'ic-empty-filter' ) ) && ic_is_rendering_block() ) {
		$block_content = '<div style="padding: 20px 10px">' . sprintf( __( '%s will only show up on the front-end if something is available in the context.', 'price-field' ), __( 'Catalog Price Filter', 'price-field' ) ) . '</div>';
	}
