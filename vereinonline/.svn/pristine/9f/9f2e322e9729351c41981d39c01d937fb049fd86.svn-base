...
if (!function_exists('vereinonline'))
{
   ...
   if (function_exists('get_option'))
   {
      function vereinonline_settings_api_init()
      {
         ...
         add_settings_field('vereinonline_setting_api', 'WordPress-Shortcut-API', 'vereinonline_setting_callback_api', 'general', 'vereinonline_setting_section');
         ...
         register_setting('general', 'vereinonline_setting_api');
         add_filter('pre_update_option_vereinonline_setting_pwd', 'vereinonline_encryptpwd', 10, 2);
      }

      function vereinonline_encryptpwd($new_value, $old_value)
      {
         if ($new_value=="" || substr($new_value,0,4)=="md5:" || substr($new_value,0,6)=="crypt:") return $new_value;
         return "md5:".md5($new_value);
      }

      function vereinonline_setting_section_callback_function() { echo '<p>VereinOnline-Zugang</p>'; }
      function vereinonline_setting_callback_url() { $setting = vereinonline_geturl(); echo "<input type='text' name='vereinonline_setting_url' value='".($setting!=""?$setting:"https://vereinonline.org/..../")."' size='60' /><br>Die Url Ihres VereinOnline-Systems"; }
      function vereinonline_setting_callback_usr() { $setting = esc_attr( get_option( 'vereinonline_setting_usr' ) ); echo "<input type='text' name='vereinonline_setting_usr' value='$setting' size='60' /><br>Der Login-Name des API-Nutzers"; }
      function vereinonline_setting_callback_pwd() { $setting = esc_attr( get_option( 'vereinonline_setting_pwd' ) ); echo "<input type='text' name='vereinonline_setting_pwd' value='$setting' size='60' /><br>Das Passwort des API-Nutzes"; }
      function vereinonline_setting_callback_web() { $setting = esc_attr( get_option( 'vereinonline_setting_web' ) ); echo "<input type='text' name='vereinonline_setting_web' value='$setting' size='60' /><br>".utf8_encode("Diese Einstellung wird nur benötigt, wenn Sie [vereinonline_kalender] verwenden und die Termine nicht per Link auf \"".(vereinonline_geturl()!=""?vereinonline_geturl():"https://vereinonline.org/..../")."?veranstaltung=....\" zeigen sollen, sondern stattdessen auf Ihre WordPress-Seite. Geben Sie hier die Url ein (z.B. \"https://ihrewebseite.de/termine/?id=\", das Plugin ergänzt diese pro Termin am Ende um die ID der Veranstaltung."); }
      ...
      function vereinonline_setting_callback_api() { $setting = esc_attr( get_option( 'vereinonline_setting_api' ) ); echo "<input type='checkbox' name='vereinonline_setting_api' value='1'".($setting!=''||vereinonline_geturl()==""?' checked':'')."/> nutzt das WordPress-Shortcut-API"; }

      if (get_option('vereinonline_setting_api', '')=="")
      {
      ...
      {
         add_shortcode('vereinonline_kalender', 'vereinonline_shortcuthandler_kalender');
         add_shortcode('vereinonline_termin', 'vereinonline_shortcuthandler_termin');
         add_shortcode('vereinonline_termine_filter', 'vereinonline_shortcuthandler_termine_filter');
         add_shortcode('vereinonline_termine', 'vereinonline_shortcuthandler_termine');
         add_shortcode('vereinonline_gruppentermine', 'vereinonline_shortcuthandler_gruppentermine');
         add_shortcode('vereinonline_bildergalerien', 'vereinonline_shortcuthandler_bildergalerien');
         add_shortcode('vereinonline_newsletter', 'vereinonline_shortcuthandler_newsletter');
         add_shortcode('vereinonline_news', 'vereinonline_shortcuthandler_news');
         add_shortcode('vereinonline_shop', 'vereinonline_shortcuthandler_shop');
         add_shortcode('vereinonline_mitglieder', 'vereinonline_shortcuthandler_mitglieder');
         add_shortcode('vereinonline_request', 'vereinonline_shortcuthandler_request');
         add_shortcode('vereinonline_subscribe', 'vereinonline_shortcuthandler_subscribe');
         add_shortcode('vereinonline_spenden', 'vereinonline_shortcuthandler_spenden');
         add_shortcode('vereinonline_gruppen', 'vereinonline_shortcuthandler_gruppen');
         add_shortcode('vereinonline_list', 'vereinonline_shortcuthandler_list');
         add_shortcode('vereinonline_for', 'vereinonline_shortcuthandler_for');
      }
   ...
   function vereinonline_shortcuthandler_kalender($atts, $content="")
   {
      $url=vereinonline_geturl();
      $web=get_option('vereinonline_setting_web', ''); if ($web=="") $web=$url."?veranstaltung=";

      $typ=$atts["typ"];
      $gruppenid=$atts["gruppenid"];
      if ($content!="")
      {
         ...
         $daten=VereinOnlineRequestUrl("GetEvents", $filter);
         ...
         {
             $template=VereinOnlineExecuteCondition($content, array(), array("keintag"=>true));
             $spalte++;
             ...
             $result.=VereinonlineVeranstaltung($rec, $template, $aktuell);
         }
         for($tag=1; $tag<=VereinOnlineTageMonat($mm,$yy); $tag++)
         {
             ...
             $template=VereinOnlineExecuteCondition($content, array(), array("keintermin"=>count($termine)==0, "eintermin"=>count($termine)==1));
             $spalte++;
             $template=VereinOnlineReplace("tr7", $spalte%7==0?"</tr><tr>":"", $template);
             if (count($termine)==0) { $data=new stdclass; $data->datum=sprintf("%04d-%02d-%02d",$yy,$mm,$tag); $result.=VereinonlineVeranstaltung($data, $template, $aktuell); } 
             elseif (count($termine)==1) $result.=VereinonlineVeranstaltung($termine[0], $template, $aktuell);
             else
             {
                ...
                $hh.=$template;
                $result.=VereinonlineVeranstaltung($termine[0], $hh, $aktuell); // Ersetzung mit 1. Termin falls ohne Schleife
             }
         }
      ...
      else
      {
         ...
         $pwd=get_option('vereinonline_setting_pwd', ''); if (substr($pwd,0,4)=="md5:") $pwd=substr($pwd,4); else if ($pwd!="" && substr($pwd,0,6)!="crypt:") $pwd=md5($pwd);
         $result="<div id=\"vereinonlinekalender\" class=\"vereinonlinekalender vereinonlineplugin\" url=\"$url\"></div>\r\n";
         $result.="<script> jQuery(document ).ready(function() { var d=new Date(); GoMonat('".$url."','".$web."',d.getMonth()+1, d.getFullYear(), '$typ', 'A/$usr/$pwd', '$gruppenid'); }); </script>";
      }
      return $result;
   }

   function vereinonline_shortcuthandler_termin($atts, $content="")
   {
      $id=$atts["id"]; if ($id=="#id#") $id=$_REQUEST["id"];
      $daten=VereinOnlineRequestUrl("GetEvent", array("id"=>$id));
      if (isset($daten->error) && $daten->error!="") return $daten->error;
      $aktuell="";
      return VereinonlineVeranstaltung($daten, $content, $aktuell);
   }

   function vereinonline_shortcuthandler_termine_filter($atts, $content="")
   {
      $url=vereinonline_geturl();
      $request=""; foreach($_REQUEST as $k=>$v) $request.=(is_array($v)?"&{$k}[]=".implode("&{$k}[]=",$v):"&$k=".$v);
      $par=$atts["par"];
      if ($par!="") $request.="&parameter=".urlencode($par);
      $result="<DIV id='vereinonlineterminefilter'></DIV>\r\n";
      $result.="<script>\r\nvar vereinonlineurl='".$url."', vereinonlinerequest='$request';\r\n</script>\r\n";
      return $result;
   }

   function vereinonline_shortcuthandler_termine($atts, $content="")
   {
      $typ=$atts["typ"];
      $filt=$atts["filter"];
      $jahr=$atts["jahr"];
      $maxanz=intval($atts["maxanzahl"]);
      $gruppenid=$atts["gruppenid"];

      if (intval($filt)!=0)
      {
         $kategorien=VereinOnlineRequestUrl("GetList", array("id"=>$filt));
      ...
      $daten=VereinOnlineRequestUrl("GetEvents", $filter);
      if (isset($daten->error) && $daten->error!="") return $daten->error;
      if (!is_object($daten) && !is_array($daten)) return "keine Daten";
      $result=""; $aktuell="";
      foreach($daten as $rec)
      {
         $result.=VereinonlineVeranstaltung($rec, $content, $aktuell, $kategorien, $filt);
         if ($maxanz>0) { $maxanz--; if ($maxanz==0) break; }
      }
      return $result;
   }

   function vereinonline_shortcuthandler_gruppentermine($atts, $content="")
   {
      $url=vereinonline_geturl();
      $web=get_option('vereinonline_setting_web', ''); if ($web=="") $web=$url."?veranstaltung=";
      $id=$atts["id"];
      $daten=VereinOnlineRequestUrl("GetEvents", array());
      if (isset($daten->error) && $daten->error!="") return $daten->error;
      $result="";
      foreach($daten as $rec)
      {
         if ($rec->akid==$id)
         {
            $result.="<a href=\"".str_replace("%year%", substr($rec->datum,0,4), $web).$rec->id."\">".VereinOnlineFormatDate($rec->datum, $rec->anzahltage).": ".str_replace("\"","",$rec->titel)."</a>\r\n";
         }
      }
      return $result;
   }

   function vereinonline_shortcuthandler_bildergalerien($atts, $content="")
   {
      $url=vereinonline_geturl();
      $va=$attr["va"];
      $maxanzahl=$attr["maximal"];

      $daten=VereinOnlineRequestUrl("GetPictures", array());
      if (isset($daten->error) && $daten->error!="") return $daten->error;
      $result="";
      foreach($daten as $nr=>$bildergalerie)
      {
         if ($maxanzahl!="") if ($nr>=$maxanzahl) break;
         if (($va=="*" && $bildergalerie->titel=="") || ($va=="!" && $bildergalerie->titel!="")) continue;
         $template=$content;
   ...
   function vereinonline_shortcuthandler_newsletter($atts, $content="")
   {
      $daten=VereinOnlineRequestUrl("GetNewsletter", array());
      if (isset($daten->error) && $daten->error!="") return $daten->error;
      $result="";
      foreach($daten as $rec)
      {
   ...
   function vereinonline_shortcuthandler_news($atts, $content="")
   {
      $anz=$atts["maximal"];
      ...
      $daten=VereinOnlineRequestUrl("GetNews", $filter);
      if (isset($daten->error) && $daten->error!="") return $daten->error;
      $result="";
      foreach($daten as $rec)
      {
   ...
   function vereinonline_shortcuthandler_shop($atts, $content="")
   {
      $daten=VereinOnlineRequestUrl("GetShop", array("parameter"=>$atts["maximal"].",".$atts["kategorie"].",".$atts["einheit"]));
      if (isset($daten->error) && $daten->error!="") return $daten->error;
      $result="";
      foreach($daten as $rec)
      {
   ...
   function vereinonline_shortcuthandler_mitglieder($atts, $content="")
   {
      $rolle=$atts["rolle"];
      $sort=$atts["sort"];
      $filterregel=$atts["condition"];
      ...
      $daten=VereinOnlineRequestUrl("GetMembers", $filter);
      if (isset($daten->error) && $daten->error!="") return $daten->error;
      $result="";
   ...
   function vereinonline_shortcuthandler_request($atts, $content="")
   {
      global $requestnr;
      $url=vereinonline_geturl();
      $par=$atts["par"];
      $requestid="vereinonlinesubmit".($requestnr==0?"":$requestnr); $requestnr++;
      $result="<DIV class=\"vereinonlineplugin\" id=\"$requestid\" url=\"$url\"></DIV>\n";
      $request=""; foreach($_REQUEST as $k=>$v) $request.=(is_array($v)?"&{$k}[]=".implode("&{$k}[]=",$v):"&$k=".urlencode($v));
      $result.="<script> jQuery(document ).ready(function() { GetRequest('$par$request&dialog=4', '$requestid'); }); </script>\n";
      return $result;
   }

   function vereinonline_shortcuthandler_subscribe($atts, $content="")
   {
      return VereinOnlineRequestForm("?subscribe");
   }

   function vereinonline_shortcuthandler_spenden($atts, $content="")
   {
      return VereinOnlineRequestForm("?spenden");
   }

   function vereinonline_shortcuthandler_gruppen($atts, $content="")
   {
      $daten=VereinOnlineRequestUrl("GetGroups", array());
      if (isset($daten->error) && $daten->error!="") return $daten->error;
      $result="";
      foreach($daten as $rec)
      {
   ...
   function vereinonline_shortcuthandler_list($atts, $content="")
   {
      $daten=VereinOnlineRequestUrl("GetList", array("id"=>$atts["id"], "filter"=>$atts["filter"]));
      if (isset($daten->error) && $daten->error!="") return $daten->error;
      $result="";
      foreach($daten as $rec)
      {
   ...
   function vereinonline_shortcuthandler_for($atts, $html="")
   {
      $par=explode("|",$atts["par"]);
      $result="";
   ...
   function vereinonline($content)
   {
      ...
      while (VereinOnlineContains($html,"[vereinonline_shop"))
      {
         ...
         $daten=VereinOnlineRequestUrl("GetShop", $filter);
         $aktuell="";
         if (isset($daten->error) && $daten->error!="")
            $h.=$daten->error;
         else
      ...
      while (VereinOnlineContains($html,"[vereinonline_subscribe]"))
      {
         ...
         $h.=VereinOnlineRequestForm("?subscribe");
      }
      $html=$h.$html;
      ...
      while (VereinOnlineContains($html,"[vereinonline_spenden]"))
      {
         ...
         $h.=VereinOnlineRequestForm("?spenden");
      }
      $html=$h.$html;
      ...
      while (VereinOnlineContains($html,"[vereinonline_list"))
      {
         ...
         $daten=VereinOnlineRequestUrl("GetList", array("id"=>$p1, "filter"=>$p2));
         $aktuell="";
   ...
   if (function_exists('get_option'))
   {
      function VereinOnlineRequestUrl($funktion, $daten)
      {
         $url=vereinonline_geturl();
         $usr=get_option('vereinonline_setting_usr', '');
         $pwd=get_option('vereinonline_setting_pwd', ''); if (substr($pwd,0,4)=="md5:") $pwd=substr($pwd,4); else if ($pwd!="" && substr($pwd,0,6)!="crypt:") $pwd=md5($pwd);
   ...
   function VereinOnlineRequestForm($parameter)
   {
      global $requestnr;
      $requestid="vereinonlinesubmit".($requestnr==0?"":$requestnr); $requestnr++;
      $url=vereinonline_geturl();
      ...
      return vereinonline_replace($result, $url, $requestid);
   }
   ...
   function VereinonlineVeranstaltung($rec, $template, &$aktuell, $kategorien=array(), $filt="")
   {
      global $useridentifier;
      if (!$rec) return $template;
   ...
   function VereinOnlineReplace($key,$val,$txt)
   {
      $txt=str_replace("[vereinonline:".$key."]", $val, $txt);
      $txt=str_replace("&#91;vereinonline:".$key."&#93;", $val, $txt);
      $txt=str_replace("[vereinonline\$".str_replace(":","\$",$key)."]", $val, $txt);
      return $txt;
   }
   ...
   function vereinonline_login($user, $username, $password)
   {
      ...
      $auth=VereinOnlineRequestUrl("CheckLogin", array("user"=>$username, "password"=>$password, "auth"=>$ath));
   ...
   function vereinonline_geturl()
   {
      $url=trim(esc_attr(get_option('vereinonline_setting_url', '')));
      if ($url!="") if (substr($url,strlen($url)-1)!="/") $url.="/";
      return $url;
   }
}
