...
function ShowHideEmpty(element,show)
{
   if (show) jQuery(element).show(); else jQuery(element).hide();
   if (!show)
   {
      jQuery(element).find('input[type=text],select').val('');
      jQuery(element).find('input[type=radio]').prop('checked',false);
...
function CheckBegleiter(check,min1)
{
   var cnt=0,fehler=0;
...
function AddBegleiter(maxbegleiter, maxfrei, anrededefault, setzefirma, firma, namehidden, adminpath, maxinfo)
{
   controlnr++;
...
function TerminplanungNeu(leer)
{
   jQuery.each( jQuery(".responsivetable tr"), function(key, value) { if (!leer || jQuery(this).find('select').val()=='') jQuery(this).show(); else jQuery(this).hide(); });
}
...
function ProcessBugNames(key, textfield, selectfield)
{
   if (key==='@') { atzeichen=true; attextfield=textfield; atselectfield=selectfield; atname=''; atstart=document.getElementById(attextfield).selectionStart; jQuery('#'+atselectfield+' .auswahl').html("..."); jQuery('#'+atselectfield).show(); } 
   else if (atzeichen && (key===' ' || key==='Enter')) { ProcessBugFinish(); }
   else if (atzeichen && key.length==1) { atname+=key; if (atname.length<3) jQuery('#'+atselectfield+' .auswahl').html(atname+'...'); else jQuery.ajax({ url: './?webservice=bugname&atname='+encodeURIComponent(atname), success: function(result) { ProcessBugResult(result); } }); }
   else if (atzeichen && key=='Backspace' && atname.length>0) { atname=atname.substring(0,atname.length-1); if (atname.length<3) jQuery('#'+atselectfield+' .auswahl').html(atname+'...'); else jQuery.ajax({ url: './?webservice=bugname&atname='+encodeURIComponent(atname), success: function(result) { ProcessBugResult(result); } }); }
   //else { jQuery('#'+atselectfield+' .auswahl').html(atname+'...['+key+']'); }
}
function ProcessBugResult(result)
{
   atresult=JSON.parse(result);
   if (atresult.error!=undefined) { jQuery('#'+atselectfield+' .auswahl').html('Fehler: '+atresult.error); return; }
   var txt=''; for(var z in atresult) txt+=atresult[z].caption+'<BR>';
   jQuery('#'+atselectfield+' .auswahl').html(txt);
}
...
function ProcessBugSetName(name)
{
   ...
   var pos=txt.length;
   txt+=html.substring(atstart+atname.length);
