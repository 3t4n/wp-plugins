...
exit(); */
 class msp_show extends msp{
  ...
  public function download( $params )
   {
    ...
    $ret["msg"] = __('Server is busy! Please try again later.',MYSTP_TEXT_DOMAIN);
    ...
    if (!isset($matches[0])) {
      $ret["msg"] = __('Incorrect image format', MYSTP_TEXT_DOMAIN);
      echo json_encode( $ret );
     exit();
    }
    ...
    if (is_wp_error($tmp)) {
      ...
      $file_array['tmp_name'] = '';
      $ret["msg"] = __('Failed to download image to local server', MYSTP_TEXT_DOMAIN);
      echo json_encode( $ret );
     exit();
    }
  ...
  public function resize( $params ){
     $data = array();
     $ret = array("code"=>0,"msg"=>"","data"=> NULL);
     $ret['msg'] = __('Service is busy! Please try again later.' ,MYSTP_TEXT_DOMAIN);
     ...
     if( $js_cnt && $js_cnt['success']){
        ...
        $ret['data'] = $js_cnt;
     	...
     	$ret['msg'] =   __('Failed to resize the image.' ,MYSTP_TEXT_DOMAIN);
     }
     echo  json_encode($ret) ;
     exit();
  }
  ...
  public function index($params=array()) {
       ...
       $cates =  is_readable( $bdir .MYSTP_CACHE_LIST.'.cache') ? file_get_contents( $bdir .MYSTP_CACHE_LIST.'.cache') : false;

       if( !$cates )
       {
          $dconf = include('cache.data.php');
         $cates =  unserialize( $dconf['cate']);
         $cates['t'] = time() - 600;
      ...
      if( !$cates ){
        $cnt=$this->request( $this->webUrl.'category');
          ...
          if( is_writable( $bdir.MYSTP_CACHE_LIST.'.cache' ))
