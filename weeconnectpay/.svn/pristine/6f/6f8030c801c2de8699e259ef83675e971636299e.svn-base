...
use WeeConnectPay\Integration\TenderCallbackLogger;
...
class WC_Gateway_Weeconnectpay extends WC_Payment_Gateway {
	...
	public function __construct() {
        ...
        add_action( 'woocommerce_admin_order_data_after_order_details', array( $this, 'display_weeconnectpay_charges_table') );
...
//				$totalShippingRefunded = $order->get_total_shipping_refunded();
     ...
     */
    function display_weeconnectpay_charges_table( WC_Order $order ) {
        // Retrieve the charges saved via your helper (stored under the meta key 'weeconnectpay_charges').
        ...
        <div class="order_data_column">
            <h4><?php esc_html_e( 'WeeConnectPay Charges', 'weeconnectpay' ); ?></h4>
            <table class="wp-list-table widefat fixed striped">
                <thead>
                <tr>
                    <!-- Set a minimum width and prevent wrapping for the Charge ID column -->
                    <th style="min-width: 150px; white-space: nowrap;"><?php esc_html_e( 'Charge ID', 'weeconnectpay' ); ?></th>
                    <th><?php esc_html_e( 'Amount', 'weeconnectpay' ); ?></th>
                    <th><?php esc_html_e( 'Card Type', 'weeconnectpay' ); ?></th>
                    <th><?php esc_html_e( 'Last 4', 'weeconnectpay' ); ?></th>
                    <th><?php esc_html_e( 'Exp Date', 'weeconnectpay' ); ?></th>
                    <th><?php esc_html_e( 'Postal Code', 'weeconnectpay' ); ?></th>
                    <th><?php esc_html_e( 'Status', 'weeconnectpay' ); ?></th>
                    <th><?php esc_html_e( 'Action', 'weeconnectpay' ); ?></th>
                </tr>
                </thead>
                <tbody>
                <?php foreach ( $charges as $charge ) :
                    // Prepare a display version (in dollars) and the raw refund amount (in cents)
                    $display_amount = number_format( $charge['amount'] / 100, 2, '.', '' );
                    $raw_refund_amount = $charge['amount']; // Raw cents value
                    ?>
                    <tr>
                        <td style="white-space: nowrap; min-width: 150px;"><?php echo esc_html( $charge['charge_id'] ); ?></td>
                        <td>
                            <?php
                            // Display the amount in dollars
                            echo esc_html( sprintf( '%s $%s', $charge['currency'], $display_amount ) );
                            ?>
                        </td>
                        <td><?php echo esc_html( $charge['card_type'] ); ?></td>
                        <td><?php echo esc_html( $charge['card_last4'] ); ?></td>
                        <td>
                            <?php
                        ...
                        <td><?php echo esc_html( $charge['card_postal_code'] ); ?></td>
                        <td>
                            <?php
                            $status = $charge['status'];
                            $color  = ( 'success' === $status ) ? 'green' : ( ( 'refunded' === $status ) ? 'orange' : '' );
                            ?>
                            <span style="color: <?php echo esc_attr( $color ); ?>;"><?php echo esc_html( $status ); ?></span>
                        </td>
                        <td>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                // Ensure ajaxurl is defined (it should be available in admin).
                ...
                $('.refund-charge-button').on('click', function(e) {
                    ...
                    var refundAmt = button.data('refund-amount'); // refund amount in dollars
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                // Update the UI: change status to refunded (orange) and remove the button.
                                button.closest('tr').find('td:nth-child(7) span')
                                    .css('color', 'orange')
                                    .text('refunded');
