...
//				}
//			}
			$available_shipping_methods = WC()->shipping()->calculate_shipping(WC()->cart->get_shipping_packages());
			$amwalwc_exclude_shipping_method_placement = amwalwc_get_option_or_set_default( AMWALWC_SETTING_EXCLUDE_SHIPPING_METHODS, array() );
			...
			foreach ($available_shipping_methods as $method) {
				foreach ($method['rates'] as $rate) {
					...
					amwalwc_log_info( 'Shipping Rates => '.print_r( $rate , true) );
					$decimals = wc_get_price_decimals();
						$price    = (float) $rate->cost;
						$taxes = 0;
						if ( count( $rate->taxes ) > 0 ) {
							$taxes += (float) array_sum( $rate->taxes );
						}
						$shipping_data[] = [
							"id"    => $rate->id,
							"label" => $rate->label,
							"price" => round( $price + $taxes, $decimals ),
							"tax" => round($taxes, $decimals)
						];
				}
			}
//			$available_shipping_methods = WC()->shipping()->calculate_shipping(WC()->cart->get_shipping_packages());
...
//			}
	...
	public function get_shipping_object( $chosen_shipping_method ) {
		$available_shipping_methods = WC()->shipping()->calculate_shipping( WC()->cart->get_shipping_packages() );
		foreach ( $available_shipping_methods as $method ) {
			foreach ( $method['rates'] as $rate ) {
				$rate_title          = explode( ":", $rate->get_id() )[0];
				$chosen_method_title = explode( ":", $chosen_shipping_method )[0];

				if ( $rate_title === $chosen_method_title ) {
					return $rate;
				}
			}
		}
...
//                $street = "$addressDetails[street1], $addressDetails[street2]";
	...
	function get_custom_meta_from_cart($cart, $meta_key) {
