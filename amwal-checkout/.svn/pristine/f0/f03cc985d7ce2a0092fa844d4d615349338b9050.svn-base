	...
	public function handle( $request ) {
		try {
			...
			if (empty($amwal_order_id)) {
				...
				$amwal_order_id = $checkout->create_order( [
					'payment_method' => AMWALWC_PAYMENT_METHOD_ID,
				] );
				if(is_wp_error($amwal_order_id)){
					$message = htmlspecialchars_decode( $amwal_order_id->get_error_message());
			...
			if (!empty($body['order_id'])){
				$amwal_transaction_id = $order_res->get_meta('amwal_transaction_id');
			}
			...
			if ( $order_status != $processing_status ) {
				...
				if(empty($body['order_id'] )){
					...
					$order_res->set_customer_id($customer->get_id());
					$order_res->set_billing( $customer->get_billing() );
					$order_res->set_shipping( $customer->get_shipping() );
					if ( isset( $transaction_details['shipping_details'] ) ) {
						$amwal_shipping_details  = $transaction_details['shipping_details'];
						$chosen_shipping_methods = [ $amwal_shipping_details['id'] ];
					} else {
						$chosen_shipping_methods = WC()->session->get( 'chosen_shipping_methods' );
					}
					if ( ! empty( $chosen_shipping_methods ) ) {
						$shipping_method = $this->get_shipping_object( $chosen_shipping_methods[0] );
						if ( $shipping_method ) {
							$shipping_item = new WC_Order_Item_Shipping();
							$shipping_item->set_shipping_rate($shipping_method );
							$order_res->add_item( $shipping_item );
						}
					}
				}
				...
				$order_res->save();
				if(empty($order_res->get_meta('amwal_transaction_id'))) {
				...
				if ( (float) $transaction_details['total_amount'] != $order_res->get_total() ) {
					return $this->order_creation_error_response( 'Order total amount does not match ' . $transaction_details['total_amount'] . ' != ' . $order_res->get_total() );
				}
...
//					$order_res->set_status( $invalid_statuses[3] );
	...
	protected function get_shipping_method_object($chosen_shipping_methods)
	{
		if (empty($chosen_shipping_methods) || !is_array($chosen_shipping_methods)) {
			return null; // Return null if no shipping methods are chosen or the input is invalid.
		}

		$method_id = $chosen_shipping_methods[0]; // Assuming single shipping method
		$method_parts = explode(':', $method_id);
		$method_slug = $method_parts[0];
		$instance_id = isset($method_parts[1]) ? $method_parts[1] : '';

		$available_shipping_methods = WC()->shipping->get_shipping_methods();

		if (isset($available_shipping_methods[$method_slug])) {
			$shipping_method = $available_shipping_methods[$method_slug];

			// Instance ID management could be done differently depending on how your methods are set up
			// But here, we'll skip the instance ID setting since it's not supported directly
