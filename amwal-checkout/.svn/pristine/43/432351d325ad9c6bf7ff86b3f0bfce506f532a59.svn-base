...
const AMWALWC_BTNPOS_CHECKOUT = "amwal-checkout";
const AMWALWC_BTNPOS_WOO_BLOCK = "amwal-woo-block";
...
function amwalwc_load_visual_hook_filters()
{
	...
	// woocommerce_checkout_before_order_review // working fine
	// woocommerce_checkout_before_customer_details // working fine
	// woocommerce_review_order_before_payment  // working fine 
	// woocommerce_checkout_after_order_review  // working fine 
	add_filter('woocommerce_review_order_before_payment', 'amwalwc_checkout_button', 10, 2);
	add_action('wp_ajax_amwalwc_woo_block', 'amwalwc_woo_block');
	add_action('wp_ajax_nopriv_amwalwc_woo_block', 'amwalwc_woo_block'); // Allow non-logged-in users
	add_filter( 'wp', 'amwalwc_checkout_block_button', 10, 2);
}
...
function amwalwc_checkout_button()
{
	// code...
    ...
    amwalwc_render_cart(AMWALWC_BTNPOS_CHECKOUT);
}

function amwalwc_checkout_block_button()
{
	// code...
	if (!amwalwc_should_render_button()) {
        return;
	...
	if ( is_cart() || is_checkout() ) {
	    ...
	    wp_enqueue_script('amwal-block-cart-button',plugins_url('../assets/src/amwal-block/amwalcheckout_cart_block.js', __FILE__) , array('jquery'), AMWALWC_VERSION,true);
	   	wp_localize_script(
	 		'amwal-block-cart-button', 'amwal_block', array(
	        'btn_ajax_url' => admin_url('admin-ajax.php'),
            'btn_ajax_nonce' => wp_create_nonce('btn_ajax_nonce') // Security token
	        // 'amwal_btn' => amwalwc_render_cart(AMWALWC_BTNPOS_WOO_BLOCK)
		));
...
function amwalwc_woo_block() {
    if (!isset($_POST['security']) || !wp_verify_nonce($_POST['security'], 'btn_ajax_nonce')) {
        wp_send_json_error(array("message" => "Invalid security token"), 400);
    }
	echo (amwalwc_render_cart(AMWALWC_BTNPOS_CHECKOUT));
	die;
}
...
function amwalwc_button($id,$taxes, $amount,$total_amount,$discount, $payload, $position,$coupons = null, $amwal_order_id = null, $payment_type = 'full')
{
	...
	$enable_dynamic_shipping_details = amwalwc_get_option_or_set_default(AMWALWC_SETTING_ENABLE_DYNAMIC_SHIPPING_DETAILS,1);
    $allowed_address_countries = amwal_get_shipping_country();
	$allowed_address_states = $enable_dynamic_shipping_details ? [] : amwal_get_shipping_states($allowed_address_countries);
	$allowed_address_cities = $enable_dynamic_shipping_details ? [] : amwalwc_get_shipping_cities($allowed_address_countries);
	...
	$btnStyle = '';
        if($position == AMWALWC_BTNPOS_CHECKOUT){
	       // $btnStyle = 'display: flex;justify-content: center; align-items: center;';
        	// $btnStyle = 'display: grid;place-items: center;height: 100%; padding-bottom:10px;';
        	$btnStyle = 'padding-bottom:15px;';
        	// $btnStyle = 'display: inline-table;justify-content: revert;align-items: center;width: 50%;padding-bottom:10px; ';
        }
        ...
        $html .= '<p  class="amwal-container ' . $position . '" style="'.$btnStyle.'">';
...
//        $html .= 'disabled="true"';
		...
		$redirect_installment_enabled = true;
...
//        $html .= 'enable-apple-checkout="true"';
		...
		$html .= "allowed-address-countries='".esc_html(json_encode($allowed_address_countries, JSON_UNESCAPED_UNICODE | JSON_PARTIAL_OUTPUT_ON_ERROR))."'";
        $html .= "allowed-address-states='".esc_html(json_encode($allowed_address_states, JSON_UNESCAPED_UNICODE | JSON_PARTIAL_OUTPUT_ON_ERROR))."'";
	    $html .= "allowed-address-cities='".esc_html(json_encode($allowed_address_cities, JSON_UNESCAPED_UNICODE ))."'";
        if(!empty($postcode_optional_countries) && in_array(gettype($postcode_optional_countries), ['array', 'object'])) {
	        $html .= "postcode-optional-countries='".esc_html(json_encode($postcode_optional_countries, JSON_UNESCAPED_UNICODE | JSON_PARTIAL_OUTPUT_ON_ERROR))."'";
		}
        ...
        if($address_required && !$amwal_order_id){
            ...
            $html .= 'enable-dynamic-shipping-details="'.((bool) $enable_dynamic_shipping_details ? "true" : "false").'"';
        ...
        if($user_info){
            ...
            $html .= "initial-address='".esc_html(json_encode($address, JSON_UNESCAPED_UNICODE | JSON_PARTIAL_OUTPUT_ON_ERROR))."'";
...
function amwalwc_button_style($position)
{
    switch ($position) {
        ...
        case AMWALWC_BTNPOS_BEFORE_CHECKOUT:
            ...
            break;
    	case AMWALWC_BTNPOS_CHECKOUT:
        case AMWALWC_BTNPOS_WOO_BLOCK:
	        $style = get_option(AMWALWC_SETTING_PDP_BUTTON_STYLES);
	        break;
        default:
            return false;
...
function amwalwc_render_cart( $position,$amwal_order_id = null, $payment_type = 'full' ) {
	try{
		$cart = WC()->cart;
		if ( $cart instanceof WC_Cart ) {
			...
			if($amount > 0) {
				echo amwalwc_button($target, $taxes, $amount, $total_amount, $discount_total, amwalwc_prepare_cart($cart), $position, $coupons, $amwal_order_id, $payment_type);
			}
		}
	} catch (Exception $e) {
		$sentryExceptionReport = new  SentryExceptionReport();
...
function amwal_get_shipping_states( $countries ) {
	$states       = [];
	$final_states = [];
	if ( in_array( 'cities-shipping-zones-for-woocommerce/cities-shipping-zones-for-woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
		$cities_shipping_zones_for_woocommerce = new WC_CSZ();
		$final_states                               = $cities_shipping_zones_for_woocommerce->wc_cities( $countries );
	} else {
