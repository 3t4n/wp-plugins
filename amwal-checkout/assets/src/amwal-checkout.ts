...
declare global {
  interface HTMLElementEventMap {
    ...
    amwalAddressCountryUpdate: AmwalCheckoutButtonCustomEvent<ICountry>
    amwalAddressStateUpdate: AmwalCheckoutButtonCustomEvent<IState>
...
function enableAmwalButtons (): void {
  // woo block based document.querySelectorAll might not be running in the correct scope.
  // so, using setTimeout for WooCommerce-related block events .
  setTimeout(() => {
    const buttons = document.querySelectorAll('amwal-checkout-button')
...
class AmwalCheckout {
  ...
  registerEventListeners (): void {
    ...
    const amwalAddressCountryUpdate = (ev: AmwalCheckoutButtonCustomEvent<ICountry>): void => {
      const urlForUpdate = AMWALWC_CONSTANTS.siteURL + '/wp-json/wc/amwal/v2/address/update'
      postData(urlForUpdate, {
        address_country: ev.detail.addressCountry
      })
        .then(async res => await res.json())
        .then(data => {
          this.checkoutButton.setAttribute('allowed-address-states', JSON.stringify(data))
          this.checkoutButton.allowedAddressStates = data
          this.checkoutButton.dispatchEvent(new Event('amwalAddressCountryAck'))
        })
        .catch(err => {
          console.log('Error in getting States', err)
        })
    }

    const amwalAddressStateUpdate = (ev: AmwalCheckoutButtonCustomEvent<IState>): void => {
      const urlForUpdate = AMWALWC_CONSTANTS.siteURL + '/wp-json/wc/amwal/v2/address/update'
      postData(urlForUpdate, {
        address_country: ev.detail.addressCountry,
        address_state: ev.detail.addressState
      })
        .then(async res => await res.json())
        .then(data => {
          this.checkoutButton.setAttribute('allowed-address-cities', JSON.stringify(data))
          this.checkoutButton.allowedAddressCities = data
          this.checkoutButton.dispatchEvent(new Event('amwalAddressStateAck'))
        })
        .catch(err => {
    ...
    this.checkoutButton.addEventListener('amwalAddressCountryUpdate', amwalAddressCountryUpdate, false)
    this.checkoutButton.addEventListener('amwalAddressStateUpdate', amwalAddressStateUpdate, false)
  ...
  async completeOrder (assertTransactionStatus: boolean = true): Promise<Response> {
    // const orderId = this.checkoutButton?.getAttribute('checkout-order-id') ?? ''
    const url = AMWALWC_CONSTANTS.siteURL + '/wp-json/wc/amwal/v2/order/complete'
    return await postData(url, {
      amwal_cart_id: this.buttonId,
      order_id: this.pre_existing_order_id,
