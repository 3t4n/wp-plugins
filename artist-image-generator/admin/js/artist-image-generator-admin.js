(function ($) {
    ...
    const TABS = ['generate', 'public', 'settings', 'variate', 'edit', 'about', 'services'];
    ...
    const TEMPLATES = {
        ...
        services: wp.template('artist-image-generator-services'),
    ...
    function initAdminPage() {
        const tabKeys = ['settings', 'variate', 'edit', 'about', 'services', 'public', 'generate'];
    ...
    function buildTab($tab, template, data) {
        ...
        if (
            ...
            tabClass !== TAB_CONTAINERS.services) {    
    ...
    if (window.location.search.includes('page=artist_image_generator') && 
        window.location.search.includes('action=services')) {

        $.ajax({
            url: aig_ajax_object.ajax_url,
            type: 'POST',
            data: {
                action: 'get_my_credits'
            },
            success: function(response) {
                if (response.success) {
                    $('#aig-credits-count').text(response.data.credits);
                } else {
                    $('#aig-credits-count').text('Error loading credits');
                }
            },
            error: function() {
                $('#aig-credits-count').text('Error loading credits');
