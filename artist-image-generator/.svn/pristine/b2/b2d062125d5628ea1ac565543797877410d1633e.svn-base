...
class Artist_Image_Generator_Setter {
    ...
    public static function sanitize(array $input): array {
        ...
        if (isset($input[Constants::ACCOUNT_JWT])) {
            $sanitizedValues[Constants::ACCOUNT_JWT] = self::sanitize_jwt($input[Constants::ACCOUNT_JWT]);
        }
     ...
     * Sanitizes the JWT.
     *
     * @param string $jwt The JWT to sanitize.
     * @return string The sanitized JWT.
     */
    private static function sanitize_jwt($jwt): string {
        $jwt = sanitize_text_field($jwt);

        if (empty($jwt)) {
            return '';
        }

        // Regular expression to validate JWT format
        $jwt_pattern = '/^[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+$/';

        if (!preg_match($jwt_pattern, $jwt)) {
            add_settings_error(
                Constants::OPTION_NAME,
                'invalid_jwt',
                esc_html__("Invalid JWT format", 'artist-image-generator'),
                'error'
            );
            return '';
        }
        ...
        if (count($parts) !== 3) {
            add_settings_error(
                Constants::OPTION_NAME,
                'invalid_jwt',
                esc_html__("Invalid JWT structure", 'artist-image-generator'),
                'error'
            );
            return '';
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE || !isset($payload['iat']) || !isset($payload['email']) || !isset($payload['id']) || !isset($payload['site']) || !isset($payload['username'])) {
            add_settings_error(
                Constants::OPTION_NAME,
                'invalid_jwt',
                esc_html__("Invalid JWT payload", 'artist-image-generator'),
                'error'
            );
            return '';
        }
    ...
    private static function render_input_field($key): void {
        ...
        if ($key === Constants::REFILL_PRODUCT_ID) {
            ...
            echo '<p class="description">' . sprintf(
                esc_html__('[OPTIONAL] Use more services with Stable diffusion. Paste your token here. For more information, visit %s.', 'artist-image-generator'),
