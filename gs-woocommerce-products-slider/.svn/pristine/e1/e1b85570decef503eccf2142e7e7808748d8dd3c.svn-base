...
final class Dummy_Data {
	...
	public function __construct() {
		...
		// Shortcodes
		add_action(
			'gswps_dummy_shortcodes_process_start',
			function () {
		...
		add_action(
			'gswps_dummy_shortcodes_process_finished',
			function () {
		...
		add_action( 'plugins_loaded', array( $this, 'maybe_import_shortcodes_initially' ), 100 );
	}

	public function import_shortcode_data() {

		// Validate nonce && check permission
		if ( ! check_admin_referer( '_gswps_import_demo_gs_' ) || ! current_user_can( 'manage_options' ) ) {
