...
class WPRU_Filters {
	...
	public function hooks() {
		...
		add_action( 'template_redirect', array( $this, 'restrict_archive_taxonomy' ), 10, 2 );
	...
	public function restrict_archive_taxonomy() {
		if ( !function_exists( 'is_woocommerce' ) ) {
			return;
		}
		...
		if ( is_product_category() ) {
			...
			$enable = get_term_meta( $term->term_id, 'wpru_enable', true );

			if( !$enable )
				return;

			$mode = get_term_meta( $term->term_id, 'wpru_mode', true );
			if( $mode != 'view' )
				return;
			...
			if ( ! is_user_logged_in() ) {
				...
				if ( ! in_array( get_current_user_id(), get_term_meta( $term->term_id, 'wpru_users', true ) ) ) {
					wp_safe_redirect( home_url() );
					die;
				}
			}
