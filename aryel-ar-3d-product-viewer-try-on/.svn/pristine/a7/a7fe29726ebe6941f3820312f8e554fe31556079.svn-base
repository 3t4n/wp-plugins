...
if ( ! class_exists( 'AryelAR3DProductViewer' ) ) {
	...
	class AryelAR3DProductViewer {
		...
		public static function enqueue_assets() {
			wp_register_style( 'aryel-front-style', plugins_url( '/dist/css/front.css', __FILE__ ), array(), '1.0.2' );
			wp_register_script( 'aryel-front-script', plugins_url( '/dist/js/front.js', __FILE__ ), array(), '1.0.2', array( 'in_footer' => false ) );
		...
		public static function enqueue_admin_assets() {
			...
			wp_register_style( 'aryel-front-style', plugins_url( '/dist/css/front.css', ARYEL_AR_3D_PRODUCT_VIEWER_PLUGIN_FILE ), array(), '1.0.2' );
			wp_register_style( 'aryel-admin-style', plugins_url( '/dist/css/admin.css', __FILE__ ), array( 'wp-color-picker' ), '1.0.2' );
			wp_register_script( 'aryel-admin-script', plugins_url( '/dist/js/admin.js', __FILE__ ), array( 'wp-color-picker' ), '1.0.2', array( 'in_footer' => false ) );

			if ( ! empty( $api_key ) ) {
				wp_register_script( 'aryel-lib-script', "https://assets.aryel.io/embed-viewer/aryel-loader.js?apiKey={$api_key}&v=2", array(), '1.0.2', array( 'in_footer' => false ) );
