...
if (!wp_verify_nonce($nonce, 'wvrbbp_download')) {
    ...
    wp_die(__('Sorry - download must be initiated from admin panel. You will need to reload the admin panel.', 'weaver-for-bbpress'));
}
...
$show_fn = sanitize_text_field($show_fn);
...
if (!$nonce || !$show_fn || !$ext) {
    @header('Content-Type: ' . get_option('html_type') . '; charset=' . get_option('blog_charset'));
    wp_die(__('Sorry - invalid download. Reload the admin panel.', 'weaver-for-bbpress'));
}
