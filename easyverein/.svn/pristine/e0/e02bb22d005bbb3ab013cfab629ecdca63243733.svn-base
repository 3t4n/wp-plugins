...
function easyVerein_printCalendar($atts = [])
{
    ...
    if ($easyVerein_calendar_atts['id'] == 0) {
        ...
        $calendarData = easyVereinAPIRequest('event', '{name,start,end,allDay,description,locationName}&ordering=start&start__gte=' . date("Y-m-d H:i:s.u") . '&calendar=' . $easyVerein_calendar_atts['id'] . $isPublicParam . '&deleted=false&limit=' . $count);
        // Build response
