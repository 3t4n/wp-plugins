...
?>
 <div class="wrap">
  <h1><?php esc_html_e('Maps for WP Settings', 'mfwp'); ?></h1> 
  ...
  <div id="dashboard-widgets-wrap"><div id="dashboard-widgets" class="metabox-holder">	
	<div id="postbox-container-1" class="postbox-container"><div class="meta-box-sortables" >
	 ...
	 <div class="postbox">
	   <div class="inside">
	    <h1><?php esc_html_e('Main parameters', 'mfwp'); ?></h1>
		<table class="form-table"><tbody>
		 <tr>
			<th scope="row"><label for="mfwp_map_from"><?php esc_html_e('Use maps from', 'mfwp'); ?></label></th>
			<td class="overalldesc">
				<select name="mfwp_map_from">					
					<option value="yandex" <?php selected($map_from, 'yandex'); ?>><?php esc_html_e('Yandex', 'mfwp'); ?></option>
					<option value="google" <?php selected($map_from, 'google'); ?>><?php esc_html_e('Google', 'mfwp'); ?></option>
				</select><br />
				<span class="description"><?php esc_html_e('Please note that in order to work correctly with Google Maps, you need to provide an API key', 'mfwp'); ?></span>
			</td>
		 </tr>
		 <tr>
			<th scope="row"><label for="mfwp_gapikey"><?php esc_html_e('Google Maps API Key', 'mfwp'); ?></label></th>
			<td class="overalldesc">
				<input type="text" name="mfwp_gapikey" value="<?php echo $gapikey; ?>" /><br />
				<span class="description"><?php esc_html_e('Required for Google Maps', 'mfwp'); ?>.<br /><?php esc_html_e('Google Maps require an API key to function', 'mfwp'); ?>. <a href="//maps-apis.googleblog.com/2016/06/building-for-scale-updates-to-google.html"><?php esc_html_e('Read more', 'mfwp'); ?></a>.<br />
				<a href="//developers.google.com/maps/documentation/javascript/get-api-key"><?php esc_html_e('You can create an API key here now (free)', 'mfwp'); ?></a>.
				</span>				
			</td>	
		 </tr>
		 <tr>
			<th scope="row"><label for="mfwp_default_point_img"><?php esc_html_e('Use standard map markers', 'mfwp'); ?></label></th>
			<td class="overalldesc">
				<input type="checkbox" name="mfwp_default_point_img" <?php checked($default_point_img, 'on' ); ?>/><br />
				<span class="description"><?php esc_html_e('Use standard map markers', 'mfwp'); ?></span>
			</td>
		 </tr>
		 <tr>
			<th scope="row"><label><?php esc_html_e('The image point on the map (36×36)', 'mfwp'); ?></label></th>
			<td>
				<div class="cacf">
		 ...
		 <tr>		
			<th scope="row"><label for="mfwp_type_map"><?php esc_html_e('Default Type of Maps', 'mfwp'); ?></label></th>
			<td class="overalldesc">
				<select name="mfwp_type_map">					
					<option value="roadmap" <?php selected($type_map, 'roadmap'); ?>><?php esc_html_e('Roadmap', 'mfwp'); ?></option>
					<option value="satellite" <?php selected($type_map, 'satellite'); ?>><?php esc_html_e('Satellite', 'mfwp'); ?></option>
					<option value="hybrid" <?php selected($type_map, 'hybrid'); ?>><?php esc_html_e('Hybrid', 'mfwp'); ?></option>
					<option value="terrain" <?php selected($type_map, 'terrain'); ?>><?php esc_html_e('Terrain (only Google Maps)', 'mfwp'); ?></option>					
				</select><br />
				<span class="description"><?php esc_html_e('Please note that in order to work correctly with Google Maps, you need to provide an API key', 'mfwp'); ?></span>
			</td>
		 </tr>		 
		 <tr>		
			<th scope="row"><label for="mfwp_style_map"><?php esc_html_e('Style of Maps', 'mfwp'); ?></label></th>
			<td class="overalldesc">
				<select name="mfwp_style_map">					
					<option value="default" <?php selected($style_map, 'default'); ?>><?php esc_html_e('Default', 'mfwp'); ?></option>
					<option value="blackwhite" <?php selected($style_map, 'blackwhite'); ?>><?php esc_html_e('Black-white', 'mfwp'); ?></option>
					<option value="blackout" <?php selected($style_map, 'blackout'); ?>><?php esc_html_e('Blackout', 'mfwp'); ?></option>
					<option value="сolorinversion" <?php selected($style_map, 'сolorinversion'); ?>><?php esc_html_e('Color inversion', 'mfwp'); ?></option>
				</select><br />
				<span class="description"><?php esc_html_e('Only for Yandex maps', 'mfwp'); ?></span>
			</td>
		 </tr>		 
		 <tr>
			<th scope="row"><label for="mfwp_h"><?php esc_html_e('Default maps height', 'mfwp'); ?></label></th>
			<td class="overalldesc">
				<input min="50" max="1300" type="number" name="mfwp_h" value="<?php echo $h; ?>" /><br />
				<span class="description"><?php esc_html_e('Default maps height in pixels', 'mfwp'); ?>.</span>				
			</td>
		 </tr>
		 <tr>
			<th scope="row"><label for="mfwp_zoom_OnePoint"><?php esc_html_e('Default Zoom map with one point', 'mfwp'); ?></label></th>
			<td class="overalldesc">
				<input min="0" max="18" type="number" name="mfwp_zoom_OnePoint" value="<?php echo $zoom; ?>" /><br />
				<span class="description"><?php esc_html_e('Zoom map with one point', 'mfwp'); ?>.</span>				
			</td>
		 </tr>
		 <tr>
			<th scope="row"><label for="mfwp_zoom_ManyPoints"><?php esc_html_e('Default Zoom map with many points', 'mfwp'); ?></label></th>
			<td class="overalldesc">
				<input min="0" max="18" type="number" name="mfwp_zoom_ManyPoints" value="<?php echo $zoomMany; ?>" /><br />
				<span class="description"><?php esc_html_e('Zoom map with many points', 'mfwp'); ?>.</span>				
			</td>
		 </tr>
		 <tr>
			<th scope="row"><label><?php esc_html_e('Latitude center map with many points', 'mfwp'); ?></label></th>
			<td><input required type="number" step="any" name="mfwp_center_lat_ManyPoints" value="<?php echo $center_lat_ManyPoints; ?>"></td>
		 </tr>
		 <tr>
			<th scope="row"><label><?php esc_html_e('Longitude center map with many points', 'mfwp'); ?></label></th>
	 ...
	 <div class="inside">
		<table class="form-table"><tbody>
		 <tr>
			...
			<td class="overalldesc"><?php wp_nonce_field('mfwp_nonce_action','mfwp_nonce_field'); ?><input class="button-primary" type="submit" name="mfwp_submit_action" value="<?php esc_html_e('Save', 'mfwp'); ?>" /><br />
			<span class="description"><?php esc_html_e('Click to save the settings', 'mfwp'); ?></span></td>
		 </tr>
		</tbody></table>
	  </div>
	 </div>
	...
	<div id="postbox-container-2" class="postbox-container"><div class="meta-box-sortables" >    
	 <div class="postbox">
	  <div class="inside">
	  <h1><?php esc_html_e('Please support the project!', 'mfwp'); ?></h1>
	  <p><?php esc_html_e('Thank you for using the plugin', 'mfwp'); ?> <strong>Maps for WP</strong></p>
	  <p><?php esc_html_e('Please help make the plugin better', 'mfwp'); ?> <a href="//docs.google.com/forms/d/1Tf9GQZfe8VHHcZZ4Lo4djiTBZLdHmD3hrSOrj1LNsPw" target="_blank" ><?php esc_html_e('answering 5 questions', 'mfwp'); ?>!</a></p>
	  <p><?php esc_html_e('If this plugin useful to you, please support the project one way', 'mfwp'); ?>:</p>
	  <ul>
		<li>- <a href="//wordpress.org/plugins/maps-for-wp/" target="_blank"><?php esc_html_e('Leave a comment on the plugin page', 'mfwp'); ?></a>.</li>
		<li>- <?php esc_html_e('Support the project financially. Even $1 is a help!', 'mfwp'); ?><a href="https://icopydoc.ru/donate/" target="_blank"> <?php esc_html_e('Donate now', 'mfwp'); ?></a>.</li>
		<li>- <?php esc_html_e('Noticed a bug or have an idea how to improve the quality of the plugin?', 'mfwp'); ?> <a href="mailto:pt070@yandex.ru"><?php esc_html_e('Let me know', 'mfwp'); ?></a>.</li>
	  </ul>
	  <p><?php esc_html_e('The author of the plugin Maxim Glazunov', 'mfwp'); ?>.</p>
	  <p><span style="color: red;"><?php esc_html_e('Accept orders for individual revision of the plugin', 'mfwp'); ?></span>:<br /><a href="mailto:pt070@yandex.ru"><?php esc_html_e('Leave a request', 'mfwp'); ?></a>.</p>  
	  </p>
	  </div>
	 </div>
	 ...
	 <div class="postbox">
	  <div class="inside">
		<h1><?php esc_html_e('Examples shotcodes', 'mfwp'); ?>:</h1>
		<p>[MapOnePoint id="" type="" lon="" lat="" zoom="" h="" img="" thover="" tclick=""]</p>
		<p><strong><?php esc_html_e('Where', 'mfwp'); ?>:</strong></p>
		<p><strong>id (required)</strong> - <?php esc_html_e('unique id', 'mfwp'); ?><br />
		<strong>type (required)</strong> - <?php esc_html_e('map layer', 'mfwp'); ?> (roadmap, satellite, hybrid, terrain)<br />
		<strong>lon (required)</strong> - <?php esc_html_e('longitude of the center of the map', 'mfwp'); ?><br />
		<strong>lat (required)</strong> - <?php esc_html_e('latitude of the center of the map', 'mfwp'); ?><br />
		<strong>mstyle (not required)</strong> - <?php esc_html_e('style of Maps', 'mfwp'); ?> (default, blackwhite, blackout, сolorinversion)<br />
		<strong>h (not required)</strong> - <?php esc_html_e('Map height in pixels', 'mfwp'); ?><br />
		<strong>img (not required)</strong> - <?php esc_html_e('URL image markers', 'mfwp'); ?><br />
		<strong>thover (not required)</strong> - <?php esc_html_e('Text when pointing to a point', 'mfwp'); ?><br />
		<strong>tclick (not required)</strong> - <?php esc_html_e('Text when clicking on a point', 'mfwp'); ?>
		</p>
