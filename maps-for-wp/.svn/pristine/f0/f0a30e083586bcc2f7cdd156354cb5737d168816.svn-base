 ...
 * Plugin URI:              https://wordpress.org/plugins/maps-for-wp/
	...
	add_action( 'admin_notices', function () {
		warning_notice( 'notice notice-error',
			sprintf(
				...
				'Maps for WP Lite',
				esc_html__( 'plugin requires a php version of at least', 'maps-for-wp' ),
				esc_html__( 'You have the version installed', 'maps-for-wp' ),
				phpversion()
			)
		);
	} );
...
define( 'MFWP_PLUGIN_VERSION', '1.2.5' ); // 0.1.0
...
final class MapsForWP {
	...
	public function add_admin_menu() {
		$page_suffix = add_menu_page( null, esc_html__( 'Maps for WP', 'mfwp' ), 'manage_options', 'mfwpset', 'mfwp_set_page', 'dashicons-location-alt', 29 );
		...
		//add_submenu_page( 'mfwpexport', esc_html__('Add Extensions', 'mfwp'), esc_html__('Extensions', 'mfwp'), 'manage_options', 'mfwpextensions', 'mfwp_extensions_page' );
	...
	function mfwp_visibility_map_manypoints( $atts ) {
		...
		if ( is_multisite() ) {
			if ( get_blog_option( get_current_blog_id(), 'mfwp_default_point_img' ) == 'on' ) {
				$src = '';
			}
		} else {
			if ( get_option( 'mfwp_default_point_img' ) == 'on' ) {
				$src = '';
			}
		}
	...
	if (typeof mfwp_setings_ManyPoints =='undefined'){
		...
		mfwp_setings_ManyPoints.push( " . esc_js( $js_obj2 ) . "); 
	} else {
		mfwp_setings_ManyPoints.push(" . esc_js( $js_obj2 ) . ");
	}
	</script>";
		...
		if ( $css_for_yandex !== '' ) {
			print '<style>#mfwpm' . sanitize_key( $id ) . ' [class*="ymaps-2"][class*="-ground-pane"]{' . esc_attr( $css_for_yandex ) . '}</style>';
		}

		return '<div style="width: 100%; height: ' . esc_attr( $h ) . 'px;" class="mfwpMany" id="mfwpm' . sanitize_key( $id ) . '"></div>';
	}
	...
	function mfwp_visibility_map_onepoint( $atts ) {
		 ...
		 */
		/* проверка данных */
		if ( isset( $atts['thover'] ) ) {
			$thover = $atts['thover'];
		} else {
			$thover = "";
		}
		if ( isset( $atts['tclick'] ) ) {
			$tclick = $atts['tclick'];
		} else {
			$tclick = "";
		}
		...
		$arr[] = array(
			'mfwp_id' => esc_attr( $id ),
			'mfwp_type' => esc_attr( $type ),
			'mfwp_lat' => esc_attr( $lat ),
			'mfwp_lon' => esc_attr( $lon ),
			'mfwp_zoom' => esc_attr( $zoom ),
			'iconImageHref' => esc_attr( $src ),
			'mfwp_thover' => esc_attr( $thover ),
			'mfwp_tclick' => esc_attr( $tclick )
		);
		$js_obj2 = json_encode( $arr );
	...
	if (typeof mfwp_setings_OnePoint =='undefined'){
		...
		mfwp_setings_OnePoint.push( " . esc_js( $js_obj2 ) . ");  
	} else {
		mfwp_setings_OnePoint.push( " . esc_js( $js_obj2 ) . "); 
	}
	</script>";
		...
		if ( $css_for_yandex !== '' ) {
			print '<style>#mfwp' . sanitize_key( $id ) . ' [class*="ymaps-2"][class*="-ground-pane"]{' . esc_attr( $css_for_yandex ) . '}</style>';
		}
		//return '<div id="mfwp_map_'.$id.'" class="mfwp_map"></div>';

		return '<div style="width: 100%; height: ' . esc_attr( $h ) . 'px;" class="mfwpOne" id="mfwp' . sanitize_key( $id ) . '"></div>';
	}
	...
	public function mfwp_admin_notices_function() {
		if ( is_multisite() ) {
			if ( ( get_blog_option( get_current_blog_id(), 'mfwp_gapikey' ) == '' ) && ( get_blog_option( get_current_blog_id(), 'mfwp_map_from' ) == 'google' ) ) {
				print '<div class="notice error is-dismissible"><p>' . esc_html__( 'Google Maps require an API key to function', 'mfwp' ) . '. <a href="//developers.google.com/maps/documentation/javascript/get-api-key">' . esc_html__( 'You can create an API key here now (free)', 'mfwp' ) . '</a>. ' . esc_html__( 'Fill in the "Google Maps API Key" field in the plugin&#39;s settings or switch to Yandex maps', 'mfwp' ) . '.</p></div>';
			}
		} else {
		...
		if ( isset( $_REQUEST['mfwp_submit_action'] ) ) {
			print '<div class="updated notice notice-success is-dismissible"><p>' . esc_html__( 'Updated', 'mfwp' ) . '.</p></div>';
		}
	}
} /* end class MapsForWP */