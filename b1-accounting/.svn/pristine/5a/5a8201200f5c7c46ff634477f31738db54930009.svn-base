...
class B1_Accounting_Manager extends B1_Accounting_Base
{
    ...
    public function generate_order_data($order_id, $debug = false)
    {
        try {
            $order_data = array();

            if($debug) {
                $order_data['b1_reference_id'] = B1_Accounting_Order::getOrderB1rReference($order_id);
            }
...
//        }
    ...
    public function generate_order_item_data($item, $order_id, $debug = false)
    {
        $itemData = [];
...
//            Product is deleted
        ...
        if($debug) {
            $itemData['debug'] = [
                ...
                'get_variation_id' => $item->get_variation_id(),
                'itemOrderId' => $itemOrderId,
                'wc_get_product' => gettype($product)
            ];
        }

        if (!$product) {
            if($debug) {
                $itemData['debug']['skip'] = 'true';
                $itemData['debug']['wc_get_product'] = 'false';
                return $itemData;
            }
            return "skip";
        }
...
//            wp_die(__('Internal error.', $this->plugin_name));
    ...
    private function check_orders($orders)
    {
        ...
        if(count($orders) == 0 && get_option('woocommerce_custom_orders_table_enabled') == 'yes' && get_option('woocommerce_custom_orders_table_data_sync_enabled') != 'yes') {
            $hasErrors = true;
        ...
        foreach ($orders as $order) {
            ...
            foreach ($wcOrder->get_items(['line_item']) as $item) {
                $orderItemData = $this->generate_order_item_data($item, $order->ID);
                if($orderItemData == 'skip') { // Bad item object
                    $errors[] = "Wrong order item object";
                    continue;
                }
