...
class B1_Accounting_Admin extends B1_Accounting_Base
{
    ...
    public function run_debug_handler()
    {
        try {
            $debugger = new B1_Accounting_Debugger($this->plugin_name, $this->version);
            echo $debugger->run();
        } catch (Exception $e) {
            echo $e->getMessage();
        }
    ...
    public function run_query_handler()
    {
        try {
            $debugger = new B1_Accounting_Debugger($this->plugin_name, $this->version);
            echo $debugger->runQuery(urldecode($_POST['query']));
        } catch (Exception $e) {
            echo $e->getMessage();
        }
        exit();
