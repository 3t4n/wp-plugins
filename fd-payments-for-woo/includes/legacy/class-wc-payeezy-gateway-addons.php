...
class WC_Payeezy_Gateway_Addons extends WC_Payeezy_Gateway {
	...
	protected function save_subscription_meta( $order_id, $card ) {
		$order = wc_get_order( $order_id );
		$order->update_meta_data( '_payeezy_token', $card['token'] );
		$order->update_meta_data( '_payeezy_expiry', $card['expiry'] );
		$order->update_meta_data( '_payeezy_cardtype', $card['cardtype'] );
		$order->save();
		...
		foreach ( wcs_get_subscriptions_for_order( $order_id ) as $subscription ) {
			$subscription->update_meta_data( '_payeezy_token', $card['token'] );
			$subscription->update_meta_data( '_payeezy_expiry', $card['expiry'] );
			$subscription->update_meta_data( '_payeezy_cardtype', $card['cardtype'] );
			$subscription->save();
		}
	}
	...
	protected function process_pre_order( $order_id ) {
		if ( WC_Pre_Orders_Order::order_requires_payment_tokenization( $order_id ) ) {
			...
			try {
				...
				$order->update_meta_data( '_payeezy_token', $card_meta['token'] );
				$order->update_meta_data( '_payeezy_expiry', $card_meta['expiry'] );
				$order->update_meta_data( '_payeezy_cardtype', $card_meta['cardtype'] );
				$order->save();
	...
	public function process_subscription_payment( $order, $amount = 0 ) {
		$card = array(
			'token'    => $order->get_meta( '_payeezy_token', true ),
			'expiry'   => $order->get_meta( '_payeezy_expiry', true ),
			'cardtype' => $order->get_meta( '_payeezy_cardtype', true ),
		);
