<?php
/**
 * Class WC_Payeezy_Credit_Cards file.
 *
...
class WC_Payeezy_Credit_Cards {
	...
	public function card_scripts() {
		wp_enqueue_script( 'payeezy', WC_PAYEEZY_PLUGIN_URL . '/assets/js/payeezy.js', array(), '1.0', true );
		...
		if ( false === $permission ) {
			...
			if ( isset( $response->transaction_status ) && 'approved' === $response->transaction_status ) {
				...
				if ( $new_card ) {
					...
					$card_id   = sanitize_text_field( wp_unslash( $_POST['payeezy-card-id'] ) );
					...
					/* translators: 1: token value, 2: expiration date */
					$card->post_title = sprintf( __( 'Token %1$s &ndash; %2$s', 'woocommerce-payeezy' ), $response->token->token_data->value, strftime( _x( '%1$b %2$d, %Y @ %I:%M %p', 'Token date parsed by strftime', 'woocommerce-payeezy' ) ) );
					wp_update_post( $card );
					$new_card_meta = array(
						'token'      => $response->token->token_data->value,
						'expiry'     => $response->card->exp_date,
				...
				$cards = $this->get_saved_cards();
				echo wp_kses_post( wc_get_template( 'credit-cards-table.php', array( 'cards' => $cards ), 'woocommerce-payeezy/', WC_PAYEEZY_TEMPLATE_PATH ) );
			} else {
				echo 'error';
			}
		}
