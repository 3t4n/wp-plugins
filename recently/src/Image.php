<?php
/**
 * This class builds/retrieves the thumbnail image of each recent posts.
 *
 *
 * @package    Recently
...
class Image {
    ...
    public function __construct(array $admin_options)
    {
        ...
        $this->uploads_dir['basedir'] = $wp_upload_dir['basedir'] . "/" . 'recently';
        $this->uploads_dir['baseurl'] = $wp_upload_dir['baseurl'] . "/" . 'recently';
    ...
    public function get($post_id, $size, $source, $crop = true, $build = 'manual')
    {
        ...
        if ( $cached ) {
            ...
            return $this->render(
                $cached,
                $size,
                is_array($classes) ? implode(' ', $classes) : 'recently-thumbnail recently_' . $source,
                is_string($alt) ? $alt : ''
            );
        }
        ...
        if (
            ( 'custom_field' == $source && ! $this->admin_options['tools']['markup']['thumbnail']['resize'] )
            || ( 'featured' == $source && 'predefined' == $build )
        ){
            // Get custom field image URL
            if ( 'custom_field' == $source && ! $this->admin_options['tools']['markup']['thumbnail']['resize'] ) {
                $thumb_url = get_post_meta(
                    $post_id,
                    $this->admin_options['tools']['markup']['thumbnail']['field'],
                    true
                );
            ...
            // Get Post Thumbnail
                ...
                if (
                    ...
                    if ( $this->admin_options['tools']['markup']['thumbnail']['lazyload'] && false == strpos($featured_image, 'loading="lazy"') ) {
    ...
    private function resize($path, $filename, $size, $crop = true)
    {
        ...
        if ( ! is_wp_error($image) ) {
             ...
             * @since   3.0.0
             */
            $retina_support = apply_filters('recently_retina_support', true);
    ...
    private function generate_thumbnail($path, $filename, $size, $crop = true)
    {
        ...
        if ( ! is_wp_error($image) ) {
            /**
             * Hook to change the image compression quality of Recently's thumbnails.
             * @since   3.0.0
             */
    ...
    public function render($src, $size, $class, $alt = '', $error = null)
    {
        ...
        return apply_filters('recently_render_image', $img_tag);
    }
    ...
    {
        if ( has_filter('recently_default_thumbnail_url') ) {
            $default_thumbnail_url = apply_filters('recently_default_thumbnail_url', $this->default_thumbnail, $post_ID);
