...
/**
 * CyberPress Settings API wrapper class
...
class CyberPress_Settings_API {
    ...
    /**
     * Displays a selectbox with select2 library for a settings field
     *
     * @param array $args settings field args
     */
    public function callback_select2( $args ) {
        ...
        $classes .= ' cyberpress-select2';
        if ( isset( $args['find_pages'] ) && ! empty( $args['find_pages'] ) && $args['find_pages'] ) {
            $classes .= ' cyberpress-select2-posts-ajax';
        }
     ...
     * @param array   $args settings field args
     */
    public function callback_repeatable( $args ) {

        if (!$this->repeatable_loaded) {
            wp_enqueue_script( 'jquery.repeatable',  cyberpress()->plugin_url . 'assets/vendor/repeatable/jquery.repeatable.js', array( 'jquery' ) );
        }
        ...
        if (count($ex_keys) > 0) {
            foreach ($ex_data[$ex_keys[0]] as $i=>$v) {
                ...
                foreach ($args['children'] as $child) {
                    $item_id = $child['name'].'_ex'.$i;

                    $html .= '<div>';
                    $html .= '<label for="'.$item_id.'">'.$child['label'].'</label>';
                    $html .= '<input type="text" name="'.$item_id.'" value="'.$ex_data[$child['name']][$i].'" id="'.$item_id.'" data-parent="'.$args['id'].'" data-child="'.$child['name'].'"/>';
                    $html .= '</div>';
                }
                $html .= '<input type="button" class="repeatable-container-delete button" value="' . esc_attr__( 'Remove', 'cyberpress' ) . '" />';
                $html .= '</div>';
            }
        }

        $html .= '</div><input type="button" value="' . esc_attr__( '+ Add', 'cyberpress' ) . '" class="repeatable-container-add button" />';
        ...
        $template .= '<input type="button" class="repeatable-container-delete button" value="' . esc_attr__( 'Remove', 'cyberpress' ) . '" />';
        $template .= '</div></script>';
        ...
        <script>
            jQuery(function($) {
                ...
                $(".repeatable-container").repeatable({
                    template: "#<?php echo esc_attr( $args['id'] ); ?>-template",
                    addTrigger: ".repeatable-container-add",
                    deleteTrigger: ".repeatable-container-delete",
                });
                ...
                $("form").on("submit", function () {
                    ...
                    <?php foreach ($args["children"] as $child):?>
                        ...
                        $("[data-child=<?php echo $child['name'];?>").each(function(idx,el) {
                            data['<?php echo $child["name"];?>'].push($(el).val());
                        });
                    <?php endforeach;?>

                    $("#<?php echo $args["section"];?>_<?php echo $args['id'];?>").val(JSON.stringify(data));
                });
            });
        </script>
        <?php
