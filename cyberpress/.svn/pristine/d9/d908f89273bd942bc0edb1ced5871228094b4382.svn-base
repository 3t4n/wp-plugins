...
/**
 * Class CyberPress_Tournaments_List_Block
 */
class CyberPress_Tournaments_List_Block {
    ...
    private $attributes = array(
        ...
        'displayMetaTotalPrizePool' => array(
            'type'    => 'boolean',
            'default' => true,
        ),
    );
    ...
    /**
     * CyberPress_Tournaments_List_Block constructor.
     */
    ...
    public function block_render( $attributes = array() ) {
        ...
        $attributes['className'] .= ' cyberpress-block-tournaments';
        ...
        if ( isset( $attributes['filterByTournaments'] ) && $attributes['filterByTournaments'] ) {
            $include_ids = explode( ',', $attributes['filterByTournaments'] );
        ...
        if ( isset( $attributes['filterByGames'] ) && $attributes['filterByGames'] ) {
            $args['meta_query'][] = $this->get_meta_query_for_multiple_selector( 'cyberpress_tournament_game', $attributes['filterByGames'] );
        }
        ...
        if ( $the_query->have_posts() ) {
            ...
            do_action( 'cyberpress_tournaments_wrapper_block_start', $attributes );

            do_action( 'cyberpress_tournaments_loop_block_start', $attributes );

            while ( $the_query->have_posts() ) {
                $the_query->the_post();

                do_action( 'cyberpress_tournaments_loop_block', $attributes );
            }

            do_action( 'cyberpress_tournaments_loop_block_end' );

            do_action( 'cyberpress_blocks_pagination', $the_query, isset( $attributes['displayPagination'] ) ? $attributes['displayPagination'] : false );

            do_action( 'cyberpress_tournaments_wrapper_block_end' );
...
new CyberPress_Tournaments_List_Block();
