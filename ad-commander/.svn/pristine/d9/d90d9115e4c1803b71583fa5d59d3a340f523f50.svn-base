...
class AdminGroupTermMeta extends Admin {
	...
	public function sort_group_terms( $term_query ) {
		...
		if ( ! is_admin() || $pagenow !== 'edit-tags.php' || ! isset( $_GET['taxonomy'] ) || sanitize_text_field( wp_unslash( $_GET['taxonomy'] ) ) !== AdCommander::tax_group() || ! isset( $_GET['orderby'] ) ) {
			return $term_query;
		}
		...
		if ( $orderby ) {
			...
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- nonce not needed in this case
			$term_query->query_vars['order'] = isset( $_GET['order'] ) ? sanitize_text_field( wp_unslash( $_GET['order'] ) ) : 'desc';
		}
