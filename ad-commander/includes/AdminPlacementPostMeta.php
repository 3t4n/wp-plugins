...
class AdminPlacementPostMeta extends Admin {
	...
	private function metaitem_placement_position() {
		...
		<div class="<?php echo esc_attr( Admin::metaitem_classes( array( 'head_close_tag' ) ) ); ?>  adcmdr-mode-restrict adcmdr-mode-restrict--head_close_tag">
			<?php
			$this->meta()->message( __( 'Intended for inserting scripts. All &lt;head&gt; placements are loaded with server-side rendering and HTML wrappers on ads and groups are disabled. Visitor targeting and consent management may not work as expected if using page caching.', 'ad-commander' ) )
			?>
		</div>
	...
	public function placement_position_filter( $post_type ) {
		...
		$selected = isset( $_GET['adcmdr_placement_position'] ) ? sanitize_text_field( wp_unslash( $_GET['adcmdr_placement_position'] ) ) : '';
	...
	public function placement_sort_pre_get_posts( $query ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- nonce not needed in this case
		if ( ! is_admin() || $pagenow !== 'edit.php' || ! isset( $_GET['post_type'] ) || AdCommander::posttype_placement() !== sanitize_text_field( wp_unslash( $_GET['post_type'] ) ) ) {
			return;
		}
		 ...
		 */
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- nonce not needed in this case
		$filtered_position = isset( $_GET['adcmdr_placement_position'] ) ? sanitize_text_field( wp_unslash( $_GET['adcmdr_placement_position'] ) ) : '';
	...
	public function filter_placements_by_position( $query ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- nonce not needed in this case
		$position = isset( $_GET['adcmdr_placement_position'] ) ? sanitize_text_field( wp_unslash( $_GET['adcmdr_placement_position'] ) ) : '';
