 ...
 * Description: This plugin extends the native customizer and provides settings for making the header sticky, with settings for scroll minification, shadow, background, spacing, text, menu and icons colors, etc. The plugin is compatible with Twenty Twenty -> Twenty Twenty-Four, Astra, and Hello Elementor themes.
 * Text Domain: sh2020
 * Domain Path: /langs
 * Version:     2.2.0
 * Author:      Iulia Cazan
 ...
 *
 * Copyright (C) 2019-2025 Iulia Cazan
 *
...
define( 'STICKY_HEADER_2020_VER', 2.20 );
...
if ( ! class_exists( 'Sticky_Header_2020' ) ) {
	...
	class Sticky_Header_2020 {
		...
		public static function register( WP_Customize_Manager $wp_customize ) {
			...
			$wp_customize->add_section(
				...
				[
					...
					'description' => __( 'Adjust the sticky header settings (these will apply if the sticky header is enabled).', 'sh2020' ),
					'class'       => 'no-pro-label',
					'attrs'       => [
						'class' => 'no-pro-label',
					],
			...
			if ( self::is_pro() ) {
				...
				self::append_simple_text(
					...
					'',
					sprintf(
						// Translators: %1$s - pro version label.
						__( 'You are using the free version, some settings are available only in the %1$s version.', 'sh2020' ),
						self::PRO_LABEL
					) . ' ' . sprintf(
						// Translators: %1$s - link, %2$s - end link.
						__( 'See more %1$sdetails%2$s.', 'sh2020' ),
			...
			self::append_simple_text(
				...
				'',
				sprintf(
					// Translators: %1$s - selector, %2$s - selector, %3$s - selector, %4$s - selector.
			...
			$wp_customize->add_control(
				...
				[
					...
					'section'  => STICKY_HEADER_2020_SECTION,
					'priority' => 2,
					'label'    => __( 'Align full header (useful for Gutenberg compatible themes, like Twenty Twenty-One for example)', 'sh2020' ),
				]
			);
			...
			$wp_customize->add_control(
				...
				[
					...
					'section'  => STICKY_HEADER_2020_SECTION,
					'priority' => 2,
					'label'    => __( 'Remove the sticky header settings when the plugin is deactivated', 'sh2020' ),
				]
			);
			...
			self::append_simple_text(
				...
				'',
				'<hr><br>' . __( 'Header Shadow Options', 'sh2020' ),
				5
			);
			if ( ! self::is_pro() ) {
				self::append_only_pro_text( $wp_customize, [
					esc_html__( 'Shadow Size', 'sh2020' ),
					esc_html__( 'Shadow Color', 'sh2020' ),
				], '5.1', 5 );
			}
			...
			self::append_simple_text(
				...
				'',
				'<hr>' . __( 'When scrolling the page, the header (and the logo) will reduce in height. The settings for the minified header can be adjusted below. To preview the changes you have to scroll up and down.', 'sh2020' ),
				7
			);
		 ...
		 *
		 * @param bool   $validity If the setting value is valid.
		 * @param string $value    The setting value.
		 */
		public static function validate_rgba( $validity, $value = '' ): bool { // phpcs:ignore
			$validity = true;
			return $validity;
		}
		 ...
		 * @param string $value The setting value.
		 */
		public static function sanitize_rgba( $value ): string { // phpcs:ignore
			// @TODO - maybe a regex.
			return $value;
		}
		...
		public static function output_custom_styles( array $css = [] ) {
			...
			foreach ( $css as $k => $v ) {
				$css[ $k ] = ! empty( $v ) ? wp_strip_all_tags( $v ) : '';
			}
		...
		/**
		 * Enqueue the computed custom CSS and the custom scripts (this is
		 * generated by the customizer), so we don't compute this for each page
		 * load.
		 */
				...
				<div class="tab-content-wrap">
					<form action="<?php echo esc_url( admin_url( 'options-general.php?page=sh2020-settings' ) ); ?>" method="POST">
						...
						<div class="rows with-sep">
							...
							<p><?php esc_html_e( 'Click to reset to default all the settings of this plugin (this will also turn off the sticky header and the styles will not override the theme defaults).', 'sh2020' ); ?></p>
							...
							<textarea rows="7"><?php echo $settings; // phpcs:ignore ?></textarea>
							<p><?php esc_html_e( 'The JSON string can be copied and imported into another instance.', 'sh2020' ); ?></p>

							<h3><?php esc_html_e( 'Import', 'sh2020' ); ?></h3>
							<textarea name="sh2020-import" rows="7"></textarea>
							<div>
		...
		public static function pro_teaser( $type = 'regular' ) { // phpcs:ignore
			...
			<div class="rows with-sep">
				<?php if ( 'regular' === $type ) : ?>
					<h3><?php esc_html_e( 'You are using the free version.', 'sh2020' ); ?></h3>
					<p>
						<?php
						echo wp_kses_post( sprintf(
				...
				<?php else : ?>
					<h3><?php esc_html_e( 'You are using the PRO version.', 'sh2020' ); ?></h3>
					<p>
				...
				<h3><?php esc_html_e( 'Sticky Header 2020', 'sh2020' ); ?></h3>
				<p>
					<?php esc_html_e( 'This plugin extends the native customizer and provides settings for making the header sticky, with settings for scroll minification, shadow, background, spacing, text, menu and icons colors, etc. The plugin is compatible with Twenty Twenty -> Twenty Twenty-Four, Astra, and Hello Elementor themes.', 'sh2020' ); ?>
				</p>
				<img src="<?php echo esc_url( SH2020_PLUGIN_URL . 'assets/images/banner-772x250.jpeg' ); ?>" loading="lazy">

				<h3><?php esc_html_e( 'The PRO version includes additional useful features', 'sh2020' ); ?></h3>
				<ol>
		...
		public static function donate_text(): string {
			...
			return trim(
				sprintf(
					...
					__( 'If you find the plugin useful and would like to support my work, please consider making a <a href="%s" target="_blank">donation</a>.', 'sh2020' ),
					$donate
				) . ' ' . sprintf(
					// Translators: %s - rating.
					__( 'It would make me very happy if you would leave a %s rating.', 'sh2020' ),
		...
		public static function admin_notices() {
			...
			if ( ! empty( $maybe_trans ) ) {
				...
				$activated = sprintf( __( '%1$s plugin was activated!', 'sh2020' ), '<b>' . $title . '</b>' );
				...
				<div id="item-<?php echo esc_attr( $slug ); ?>" class="notice is-dismissible">
					<div class="content">
						<a class="icon" href="<?php echo esc_url( admin_url( 'options-general.php?page=sh2020-settings' ) ); ?>"><img src="<?php echo esc_url( SH2020_PLUGIN_URL . 'assets/images/icon-128x128.gif' ); ?>"></a>
						<div class="details">
							<div>
		...
		public static function plugin_action_links( $links ): array { // phpcs:ignore
			...
			$all[] = '<a href="' . esc_url( admin_url( 'customize.php?autofocus[section]=' . $customizer ) ) . '" target="_blank">' . esc_html__( 'Customize', 'sh2020' ) . '</a>';
