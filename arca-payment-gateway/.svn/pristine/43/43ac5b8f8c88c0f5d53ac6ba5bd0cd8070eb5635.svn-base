		...
		if ($insert){
			...
			$args = array(
				...
				'body'		=> array(
					...
					'Amount'		=> $amount,
					'Currency'		=> $currency,
					'BackURL'		=> get_site_url() . "?arca_process=payment_completed&wc_orderId=$wc_orderId&gwp_donationId=$gwp_donationId&language=$language&currency=$currency&tatiosa_booking_id=$tatiosa_booking_id",
