...
Plugin URI: https://www.addonspress.com/
...
if ( ! class_exists( 'Coder_Limit_Login' ) ) {
	...
	class Coder_Limit_Login {
		 ...
		 * @since 1.0
		 */
		private $coder_limit_login_version = '1.2.5';
		 ...
		 * @since 1.0
		 */
		protected $coder_limit_login_minimum_wp_version = '5.0';
			...
			switch ( $column_name ) {
				case 'coder_is_logout':
					if ( $user->coder_is_logout == 'no' ) {
						$coder_message = sprintf( __( 'Currently Login. %1$s Last Active Time : %2$s', 'coder_limit_login' ), '<br />', date( 'Y-m-d, H:i:s', $user->coder_last_active_time ) );
						...
						$coder_message = sprintf( __( 'User Currently Inactive. %1$s Last Active Time : %2$s', 'coder_limit_login' ), '<br />', date( 'Y-m-d, H:i:s', $user->coder_last_active_time ) );
					}
		...
		public static function plugin_activation() {
			$coder_default_options = array(
				...
				'coder_already_login_message' => sprintf( __( '%1$sThe User with this username already login!%2$s', 'coder_limit_login' ), '<h1>', '</h1>' ),
				'coder_force_logout_message'  => sprintf( __( '%1$sThe User with this username already logout ! Please Login to Continue.%2$s', 'coder_limit_login' ), '<h1>', '</h1>' ),
			);
