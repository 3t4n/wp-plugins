...
Text Domain: smart-agenda-prise-de-rendez-vous-en-ligne
...
function smartagenda_admin_page_init() {
    ...
    add_settings_section(
        ...
        __( 'Configuration du plugin', 'smart-agenda-prise-de-rendez-vous-en-ligne' ),
...
function smartagenda_section_devloppers_cb( $args ) {
    ?>
    <p id="<?php echo esc_attr( $args['id'] ); ?>"> <?php esc_html_e("Gérez vos rendez-vous en ligne avec notre plugin WordPress. Intégrez la prise de rendez-vous directement sur votre site.",'smart-agenda-prise-de-rendez-vous-en-ligne' ); ?> </p>
...
function smartagenda_options_page_html() {
    ...
    if ( isset( $_GET['settings-updated'] ) ) {
        add_settings_error( 'smartagenda_messages', 'smartagenda_message', __( 'Modifications enregistrées', 'smart-agenda-prise-de-rendez-vous-en-ligne' ), 'updated' );
    }
			...
			<div id="post-body" class="metabox-holder columns-2">
				<div id="post-body-content">
					...
					<div class="smartagenda-wrap">
						<img class="illustration" src="<?php echo esc_attr(plugin_dir_url( __FILE__ )); ?>img/plugin-wordpress.png" alt="Plugin Wordpress SmartAgenda" style="max-width: 220px;" />
						<h1>
							<img style="float: left;" src="<?php echo esc_attr(plugin_dir_url( __FILE__ )); ?>img/logo-smartagenda.svg" width="240" alt="SmartAgenda" />
						...
						<p>
							<?php esc_html_e( "Le plugin fonctionne grâce à l'application de réservation en ligne SmartAgenda. Il nécessite d’avoir un compte actif pour utiliser les services.",'smart-agenda-prise-de-rendez-vous-en-ligne') ?><br>
							<strong><?php esc_html_e( "Vous n'avez pas encore de compte SmartAgenda ?",'smart-agenda-prise-de-rendez-vous-en-ligne' ); ?></strong> <a href="https://go.smartagenda.fr/activation2.html?source=wordpress&utm_source=plugin-wordpress&utm_campaign=plugin-Wordpress">Je créé mon agenda.</a>
						</p>
					</div>
					...
					<div class="smartagenda-wrap">
						<?php 
							...
							if (isset($_POST['update']) && !empty(sanitize_text_field(wp_unslash($_POST['update'])))) {
							
								if (isset($_POST['_wpnonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_wpnonce'])), 'nonce-ajouter-nameagenda')) {
								
									// Widget
									
									$name_agenda = !empty ($_POST['name_agenda']) ? sanitize_text_field(wp_unslash($_POST['name_agenda'])) : '';
									$name_agenda = esc_attr($name_agenda);
									update_option('name_agenda', $name_agenda);
									
									if (isset($_POST['contenu']) && !empty(sanitize_text_field(wp_unslash($_POST['contenu'])))) {
										$contenu = "true";
									...
									if (isset($_POST['entete']) && !empty(sanitize_text_field(wp_unslash($_POST['entete'])))) {
										$entete = "true";
									...
									if (isset($_POST['footer']) && !empty(sanitize_text_field(wp_unslash($_POST['footer'])))) {
										$footer = "true";
									...
									if (isset($_POST['bandeau']) && !empty(sanitize_text_field(wp_unslash($_POST['bandeau'])))) {
										$bandeau = "true";
									...
									if (isset($_POST['infosimportantes']) && !empty(sanitize_text_field(wp_unslash($_POST['infosimportantes'])))) {
									...
									if (isset($_POST['logo']) && !empty(sanitize_text_field(wp_unslash($_POST['logo'])))) {
										$logo = "true";
									...
									if (isset($_POST['photo']) && !empty(sanitize_text_field(wp_unslash($_POST['photo'])))) {
										$photo = "true";
									...
									if (isset($_POST['affrdv']) && !empty(sanitize_text_field(wp_unslash($_POST['affrdv'])))) {
										$affrdv = "true";
									...
									if (isset($_POST['activerBouton']) && !empty(sanitize_text_field(wp_unslash($_POST['activerBouton'])))) {
										$activerBouton = "true";
										$activerBouton_checked = "checked = 'checked'";
									...
									$url_bouton = !empty ($_POST['url_bouton']) ? sanitize_text_field(wp_unslash($_POST['url_bouton'])) : '';
									update_option('url_bouton', $url_bouton);
									
									$texte_bouton = !empty ($_POST['texte_bouton']) ? sanitize_text_field(wp_unslash($_POST['texte_bouton'])) : '';
									update_option('texte_bouton', $texte_bouton);
									
									if (isset($_POST['activerPopup']) && !empty(sanitize_text_field(wp_unslash($_POST['activerPopup'])))) {
										$activerPopup = "true";
										$activerPopup_checked = "checked = 'checked'";
									...
									$couleur_bouton = !empty ($_POST['couleur_bouton']) ? sanitize_text_field(wp_unslash($_POST['couleur_bouton'])) : '';
									update_option('couleur_bouton', $couleur_bouton);
									
									$couleur_texte_bouton = !empty ($_POST['couleur_texte_bouton']) ? sanitize_text_field(wp_unslash($_POST['couleur_texte_bouton'])) : '';
									...
									if (isset($_POST['position_bouton']) && !empty(sanitize_text_field(wp_unslash($_POST['position_bouton'])))) {
										$position_bouton = "bottomRight";
										$position_bouton_selected = "selected = 'selected'";
										update_option('position_bouton', $position_bouton);
										update_option('position_bouton_selected', $position_bouton_selected);
									}
									if (isset($_POST['position_bouton']) && !empty(sanitize_text_field(wp_unslash($_POST['position_bouton']))) && ($_POST['position_bouton']=="bottomLeft")) {
										$position_bouton = "bottomLeft";
										$position_bouton_selected = "selected = 'selected'";
										update_option('position_bouton', $position_bouton);
										update_option('position_bouton_selected', $position_bouton_selected);
									}
									if (isset($_POST['position_bouton']) && !empty(sanitize_text_field(wp_unslash($_POST['position_bouton']))) && ($_POST['position_bouton']=="topRight")) {
										$position_bouton = "topRight";
										$position_bouton_selected = "selected = 'selected'";
										update_option('position_bouton', $position_bouton);
										update_option('position_bouton_selected', $position_bouton_selected);
									}
									if (isset($_POST['position_bouton']) && !empty(sanitize_text_field(wp_unslash($_POST['position_bouton']))) && ($_POST['position_bouton']=="topLeft")) {
									...
									if (isset($_POST['ciblePage']) && !empty(sanitize_text_field(wp_unslash($_POST['ciblePage'])))) {
										$ciblePage = "nouvelleFenetre";
										$ciblePage_selected = "selected = 'selected'";
										update_option('ciblePage', $ciblePage);
										update_option('ciblePage_selected', $ciblePage_selected);
									}
									if (isset($_POST['ciblePage']) && !empty(sanitize_text_field(wp_unslash($_POST['ciblePage']))) && ($_POST['ciblePage']=="fenetreActive")) {
						...
						?>
							...
							<form action="<?php echo esc_url_raw(wp_unslash($_SERVER['REQUEST_URI'])); ?>" method="POST">
								...
								<?php wp_nonce_field('nonce-ajouter-nameagenda'); ?>
								<input type="hidden" value="1" name="update">
								
								<div class="parametres-agenda focus">
									<ul class="nomAgenda">
										<li><h2 class="nom-agenda">Nom de votre agenda :</h2> <input class="inputText" type="text" style="width: 370px;" value="<?php echo esc_attr($name_agenda); ?>" name="name_agenda"><br></li>
									</ul>
								...
								<?php if ($name_agenda!="") { ?>
										...
										<div class="panelSmartAgenda" style="display:block; padding: 30px;">
											...
											<div class="container-illustration-parametres">
												<img class="illustration-parametres" src="<?php echo esc_attr(plugin_dir_url( __FILE__ )); ?>img/parametres-bouton-smartagenda.png" alt="Paramètrage visuel du bouton flottant" />
											</div>
											...
											<div class="container-parametres">
												...
												<div class="container content-switch" style="margin: 15px 0;">
													<label class="switch" for="activerBouton">
														<input type="checkbox" id="activerBouton" name="activerBouton" value="<?php echo esc_attr($activerBouton); ?>" <?php if($activerBouton == 'true'){echo esc_attr($activerBouton_checked);} ?> />
												...
												<div class="container">
													...
													<input type="text" class="colorPicker" id="couleur_bouton" name="couleur_bouton" value="<?php if ($couleur_bouton=="") {?>#007eff <?php } else { echo esc_attr($couleur_bouton); }?>" />
												</div>
												
												<div class="container">
													<label class="labelSmart">Couleur du texte du bouton :</label>
													<input type="text" class="colorPicker" id="couleur_texte_bouton" name="couleur_texte_bouton" value="<?php if ($couleur_texte_bouton=="") {?>#ffffff <?php } else { echo esc_attr($couleur_texte_bouton); }?>" />
												</div>
												
												<div class="container">
														<label class="labelSmart">Texte à afficher :</label>
														<input class="inputText" type="text" style="width: 300px;" id="texte_bouton" name="texte_bouton" value="<?php if ($texte_bouton=="") {?>Prendre rendez-vous en ligne <?php } else { echo esc_attr($texte_bouton); }?>">
													</div>
												
												<div class="container content-switch" style="margin: 15px 0;">
													<label class="switch" for="activerPopup">
														<input type="checkbox" id="activerPopup" name="activerPopup" value="<?php echo esc_html($activerPopup); ?>" <?php if($activerPopup == 'true'){echo esc_attr($activerPopup_checked);} ?> />
												...
												<div class="panelSmartAgendaLink" style="display:none;">
													<div class="container">
														...
														<input class="inputText" type="text" style="width: 90%;" id="url_bouton" name="url_bouton" value="<?php if ($url_bouton=="") {?>https://www.smartagenda.fr/pro/<?php echo esc_attr($name_agenda); ?>/rendez-vous?affrdv <?php } else { echo esc_html($url_bouton); }?>">
													</div>
													
													<div class="container">
														<label class="labelSmart">Position du bouton :</label>
														<select id="position_bouton" name="position_bouton">
															<option value="bottomRight" <?php if($position_bouton == 'bottomRight'){echo esc_attr($position_bouton_selected);} ?>>En bas à droite (Recommandé)</option>
															<option value="bottomLeft" <?php if($position_bouton == 'bottomLeft'){echo esc_attr($position_bouton_selected);} ?>>En bas à gauche</option>
															<option value="topRight" <?php if($position_bouton == 'topRight'){echo esc_attr($position_bouton_selected);} ?>>En haut à droite</option>
															<option value="topLeft" <?php if($position_bouton == 'topLeft'){echo esc_attr($position_bouton_selected);} ?>>En haut à gauche</option>
														</select>
													</div>
													...
													<div class="container">
														...
														<select id="ciblePage" name="ciblePage">
															<option value="nouvelleFenetre" <?php if($ciblePage == 'nouvelleFenetre'){echo esc_attr($ciblePage_selected);} ?>>Nouvelle fenêtre</option>
															<option value="fenetreActive" <?php if($ciblePage == 'fenetreActive'){echo esc_attr($ciblePage_selected);} ?>>Fenêtre active</option>
														</select>
													</div>
												</div>
									...
									<br/>
										...
										<div class="panelSmartAgenda" style="display:none; padding: 30px;">
											...
											<br/>
											<div class="container-illustration-parametres">
												<img class="illustration-parametres" src="<?php echo esc_attr(plugin_dir_url( __FILE__ )); ?>img/parametres-widget-smartagenda.jpg" alt="Paramètrage visuel du widget" />
											</div>
															
											<div class="container-parametres">
												<div class="container content-switch">
													<label class="switch" for="bandeau">
														<input type="checkbox" id="bandeau" name="bandeau" value="<?php echo esc_html($bandeau); ?>" <?php echo esc_attr($bandeau_checked); ?> />
												...
												<div class="container content-switch">
													<label class="switch" for="logo">
														<input type="checkbox" id="logo" name="logo" value="<?php echo esc_html($logo); ?>" <?php echo esc_attr($logo_checked); ?> />
												...
												<div class="container content-switch">
													<label class="switch" for="entete">
														<input type="checkbox" id="entete" name="entete" value="<?php echo esc_html($entete); ?>" <?php echo esc_attr($entete_checked); ?> />
												...
												<div class="container content-switch">
													<label class="switch" for="infosimportantes">
														<input type="checkbox" id="infosimportantes" name="infosimportantes" value="<?php echo esc_html($infosimportantes); ?>" <?php echo esc_attr($infosimportantes_checked); ?> />
												...
												<div class="container content-switch">
													<label class="switch" for="photo">
														<input type="checkbox" id="photo" name="photo" value="<?php echo esc_html($photo); ?>" <?php echo esc_attr($photo_checked); ?> />
												...
												<div class="container content-switch">
													<label class="switch" for="contenu">
														<input type="checkbox" id="contenu" name="contenu" value="<?php echo esc_html($contenu); ?>" <?php echo esc_attr($contenu_checked); ?> />
												...
												<div class="container content-switch">
													<label class="switch" for="affrdv">
														<input type="checkbox" id="affrdv" name="affrdv" value="<?php echo esc_html($affrdv); ?>" <?php echo esc_attr($affrdv_checked); ?> />
												...
												<div class="container content-switch">
													<label class="switch" for="footer">
														<input type="checkbox" id="footer" name="footer" value="<?php echo esc_html($footer); ?>" <?php echo esc_attr($footer_checked); ?> />
												...
												<div class="codeBouton">
													<div id="codeShortcode">
														<pre><code>&#91;smartagenda agenda="<?php echo esc_attr($name_agenda);?>"&#93;</code></pre>
													</div>
						...
						<ul>
							...
							<li>3. Ajouter <code style="color:red;">[smartagenda agenda="<?php if($name_agenda!=""){ echo esc_attr($name_agenda); } else{ echo "agenda-demo"; } ?>"]</code> dans une page de votre choix</li>
						...
						<ul class="params-add">
							...
							<code>[smartagenda agenda="<?php if($name_agenda!=""){ echo esc_attr($name_agenda); } else{ echo "agenda-demo"; } ?>" utilisateur="1"]</code><br/>
							<code>[smartagenda agenda="<?php if($name_agenda!=""){ echo esc_attr($name_agenda); } else{ echo "agenda-demo"; } ?>" categ="2"]</code><br/>
							<code>[smartagenda agenda="<?php if($name_agenda!=""){ echo esc_attr($name_agenda); } else{ echo "agenda-demo"; } ?>" presta="8"]</code><br/>
							<code>[smartagenda agenda="<?php if($name_agenda!=""){ echo esc_attr($name_agenda); } else{ echo "agenda-demo"; } ?>" utilisateur="2" presta="4"]</code><br/>
							<code>[smartagenda agenda="<?php if($name_agenda!=""){ echo esc_attr($name_agenda); } else{ echo "agenda-demo"; } ?>" groupement="2"]</code>
							</li>
						</ul>
					</div>
					...
					<div id="preview-agenda" class="smartagenda-wrap" style="float:right; width: 58%;">
						...
						<script src='https://www.smartagenda.fr/pro/<?php echo esc_attr($name_agenda); ?>/smartwidget.js' type='text/javascript'></script>
						<script>
							window.onload = function(){
								var options = {
									contenu : <?php echo esc_html($contenu); ?>,
									entete : <?php echo esc_html($entete); ?>,		
									footer : <?php echo esc_html($footer); ?>,		
									bandeau : <?php echo esc_html($bandeau); ?>,		
									infosimportantes : <?php echo esc_html($infosimportantes); ?>,
									logo : <?php echo esc_html($logo); ?>,
									photo : <?php echo esc_html($photo); ?>,	
									affrdv : <?php echo esc_html($affrdv); ?>
								};
								var smartwidget = new SMARTAGENDAwidget(
									'smart-container',
									'https://www.smartagenda.fr/pro/<?php echo esc_attr($name_agenda); ?>',
									options);
								smartwidget.render();
							};
						</script>
					...
					<div style="clear:both;"></div>
							...
							<?php if (!empty($updated)): ?>
								<p style="color:#fff; background: #3ecf8e; padding: 15px; font-weight: bold;"><?php esc_html_e( "Modification enregistrée. Vous pouvez commencer à utiliser votre agenda.",'smart-agenda-prise-de-rendez-vous-en-ligne' ); ?></p>
							<?php endif; ?>
					</div>
				</div>
				...
				<div id="postbox-container-1" class="postbox-container" >
					...
					<div class="smartagenda-wrap" style="background: #fff; border: 1px solid #e5e5e5; margin-bottom: 20px; padding: 20px; margin-right: 20px;">
						<p><img style="width: 100%; height: auto;" src="<?php echo esc_attr(plugin_dir_url( __FILE__ )); ?>img/synchronisation-google-agenda.png" alt="Synchronisation Google Agenda" /></p>
...
function smartagenda_shortcode($atts, $content = null, $tag = ''){
	...
	{
		$agenda = sanitize_text_field($atts['agenda']);
		$o .= '<script src="https://www.smartagenda.fr/pro/'.esc_html($agenda).'/smartwidget.js" type="text/javascript"></script>
		<script>
			window.onload = function(){
				'.$options.'
				var smartwidget = new SMARTAGENDAwidget(
					"smart-container",		
					"https://www.smartagenda.fr/pro/'.esc_html($agenda).'",
				options);
				smartwidget.render();
			};
		</script>';
...
function smartagenda_boutonFlottant_init() {
	...
	$widget = '<div id="smart-container"></div>
				<script src="https://www.smartagenda.fr/pro/'.esc_attr($name_agenda).'/smartwidget.js" type="text/javascript"></script>
				<script>
					window.onload = function(){
					var options = {
					contenu : false,
					...
					var smartwidget = new SMARTAGENDAwidget(
					"smart-container",
					"https://www.smartagenda.fr/pro/'.esc_attr($name_agenda).'/",
					options);
					smartwidget.render();
					};
				</script>';
	...
	if ($activerPopup == "true"){
		$codeBoutonFlottant = '<style id="smartagenda-css" scoped>.btnSmartAgenda{display:block;text-align:center;background-color:'.esc_attr($couleur_bouton).';color:'.esc_attr($couleur_texte_bouton).';font-size:16px;text-transform: uppercase;text-decoration:none !important;letter-spacing: 1px;overflow:hidden;padding: 6px 18px;position:fixed;'.esc_attr($codePosition).';z-index:100000000000;line-height:30px;}.btnSmartAgenda:hover{color:'.esc_attr($couleur_texte_bouton).' !important; padding: 6px 18px 11px 18px;}@media (max-width: 767px) {.btnSmartAgenda{width: 94%;right:3%;font-size: 14px; top: auto; bottom:0; border-bottom-right-radius:0;border-bottom-left-radius:0;border-top-left-radius:4px;border-top-right-radius:4px;}}</style><a id="modal-external-button-smartagenda" class="btnSmartAgenda" href="#smartagenda-modal">'.esc_attr($texte_bouton).'</a>'; 
		...
		$codeBoutonFlottant = '<style id="smartagenda-css" scoped>.btnSmartAgenda{display:block;text-align:center;background-color:'.esc_attr($couleur_bouton).';color:'.esc_attr($couleur_texte_bouton).';font-size:16px;text-transform: uppercase;text-decoration:none !important;letter-spacing: 1px;overflow:hidden;padding: 6px 18px;position:fixed;'.esc_attr($codePosition).';z-index:100000000000;line-height:30px;}.btnSmartAgenda:hover{color:'.esc_attr($couleur_texte_bouton).' !important; padding: 6px 18px 11px 18px;}@media (max-width: 767px) {.btnSmartAgenda{width: 94%;right:3%;font-size: 14px; top: auto; bottom:0; border-bottom-right-radius:0;border-bottom-left-radius:0;border-top-left-radius:4px;border-top-right-radius:4px;}}</style><a '.esc_attr($codeCiblePage).' id="external-button-smartagenda" class="btnSmartAgenda" href="'.esc_attr($url_bouton).'">'.esc_attr($texte_bouton).'</a>'; 
	}
