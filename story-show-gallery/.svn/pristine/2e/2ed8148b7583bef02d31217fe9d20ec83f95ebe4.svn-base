...
class StoryShowGallery {
	...
	public function story_show_gallery_create_admin_page() {
		...
		<div class="wrap" id="ssg-wrap">
            ...
            <p class="text crucial">
                ...
                <br /> <br /> <strong class="important">Important!</strong>  
				Story Show Gallery requires that your gallery linking is set to <strong>Link images to media files</strong>. 
				<span class="expander-image">
					<a href="https://roman-flossler.github.io/StoryShowGallery/img/link-to-media-files.jpg" target="_blank">» How to set it?</a>
					<br><img src="https://roman-flossler.github.io/StoryShowGallery/img/link-to-media-files.jpg">
				</span>
                <br /> You can open a single image into SSG too. Use the same option "Link images to media files", but in the settings of that image.
            </p>
	...
	<?php }	?>	
		...
		<script>
			...
			// second section begins with <TR> number 10
			jQuery( trs(10,55) ).hide();
...
function ssg_scripts() {
	...
	if ( 'caption' == $story_show_gallery_options["captionsSource"] ) {
		$extractCaptions = "jQuery(document).ready(function(){!SSG.jQueryImgCollection&&SSG.beforeRun();var tit=[];SSG.jQueryImgCollection.each(function(index){tit[index]='none';jQuery(this).siblings('figcaption, p').each(function(sindex){if (sindex==0) { tit[index] = jQuery(this).html().replace(/(<br>)|(<br\/>)|(<br \/>)/gi, '<br> ') }});if(tit[index]=='none'){jQuery(this).parent().siblings().each(function(sindex){if(sindex==0){tit[index]=jQuery(this).html().replace(/(<br>)|(<br\/>)|(<br \/>)/gi,' - ').replace(/(<[^>]*>)+/g,' ').trim()}})}});SSG.jQueryImgCollection.each(function(index){if(tit[index]!='none'){jQuery(this).attr('data-caption',tit[index])}})});";
	} else {
		$extractCaptions ='';
	}
