...
class CartHelper
{
    ...
    /**
     * @param mixed $product_id1
     * @param mixed $product_id2
     * @param mixed $cart_object
     * 
     * @return bool
     */
    public static function is_bundle_in_cart($product_id1, $product_id2, $cart_object)
    {
        $product1_count = 0;
        $product2_count = 0;
        ...
        foreach ($cart_object->cart_contents as $cart_item) {
            if ($cart_item['product_id'] == $product_id1) {
                $product1_count += $cart_item['quantity'];
            }
            if ($cart_item['product_id'] == $product_id2) {
                $product2_count += $cart_item['quantity'];
            }
        }

        if ($product_id1 == $product_id2) {
            return $product1_count >= 2;
        } else {
            return $product1_count >= 1 && $product2_count >= 1;
        }
    }
     ...
     * @return string
     */
    public static function get_strikeout_price_html_bundle($cart_item)
    {

        // Get the regular & sale prices
        $regular_price = $cart_item['data']->get_regular_price();
        $sale_price = isset($cart_item['_wbdl_data']['sale_price']) ? $cart_item['_wbdl_data']['sale_price'] : $cart_item['data']->get_price();

        $price_html = "<div class='wbdl_cart_strikeout_line' style='display:inline-block;'>";

        if (isset($regular_price) && isset($sale_price) && ($regular_price > $sale_price)) {
            $price_html .= '<del><span class="woocommerce-Price-amount amount" val="' . $regular_price . '"><bdi>' . wc_price($regular_price) . '</bdi></span></del>&nbsp;';
        }

        if ($sale_price === 0) {
            $price_html .= '<ins><span class="woocommerce-Price-amount amount wbdl-free-gift"><bdi>FREE</bdi></span></ins>';
        } else {
            $price_html .= '<ins><span class="woocommerce-Price-amount amount"><bdi>' . wc_price($sale_price) . '</bdi></span></ins>';
        }
