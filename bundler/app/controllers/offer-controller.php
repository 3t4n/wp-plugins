...
class OfferController
{
     ...
     * @return array Bundler\Models|Offer
     */
    public function get_bundle_offers()
    {
        return Offer::get_bundle_offers();
    }
    ...
    public function get_offer($offer_id)
    {
        ...
        if ($offer) {
            if ($offer->type === 'volume_discount') {
                ...
                $bundle  = Bundle::get_bundle_by_offer_id($offer_id);
                $products = $this->get_offer_products($offer);
                ...
                $formattedBundle = [
                    ...
                    'discountMethod' => $bundle->discount_method,
                    'discountValue'  => $bundle->discount_value,
                    'freeGift'       => $bundle->free_gift_id,
                ];
        ...
        return (object) [
            'status' => false,
            'msg' => __('Offer not found', 'bundler'),
        ];
    }
    ...
    public function new_offer($offer_data)
    {
        ...
        if (!$type || $type === 'volume_discount') {
            ...
            if (!$offer_id) {
                return (object)[
                    'status'   => false,
                    'msg'      => __('Error while creating the offer.', 'bundler'),
                ];
            }

            $product_ids      = isset($offer_data['products']) ? array_column($offer_data['products'], 'id') : [];
            $collection_ids   = isset($offer_data['collections']) ? array_column($offer_data['collections'], 'id') : [];

            if (!empty($product_ids)) Offer::update_offer_products($offer_id, $product_ids);
            if (!empty($collection_ids)) Offer::update_offer_collections($offer_id, $collection_ids);
            ...
            $apply_on              = "specific_products";
            $wmc                   = 'off';
            $use_custom_variations = 'off';
            ...
            if (!empty($product_ids)) {
                Offer::update_offer_products($offer_id, $product_ids);
            }

            $bundle = $offer_data['bundle'];
            Bundle::add_bundle_to_offer(
                $offer_id,
                $bundle['discountType'],
                $bundle['discountMethod'],
    ...
    public function update_offer($offer_data)
    {
        ...
        if ($offer_id) {
            ...
            if (!$type || $type === 'volume_discount') {
                ...
                $bundle = $offer_data['bundle'];
                Bundle::remove_bundles($offer_id);
                Bundle::add_bundle_to_offer(
                    $offer_id,
                    $bundle['discountType'],
                    $bundle['discountMethod'],
     ...
     * @return Bundler\Models\Offer
     */
    public function get_the_bundle($product_id)
    {
        $offer = Offer::get_highest_priority_bundle_by_product_id($product_id);
        if ($offer) {
            $bundle = Bundle::get_bundle_by_offer_id($offer->id);
