	...
	final class Bundler
	{
		...
		public static function instance()
		{
			...
			if (
				...
				add_action('init', [self::$instance, 'init_tracker']);
			}
		...
		public function enqueue_script()
		{
			...
			wp_register_script('bdlr-bundle-script', BDLR_PLUGIN_URL . 'assets/js/bundle.js', array('jquery', 'wc-add-to-cart'), BDLR_VERSION, true);

			wp_enqueue_script('bdlr-script');
			wp_enqueue_script('bdlr-bundle-script');
			...
			$script_vars = array(
				...
				'cart_redirect'            => $this->settings && $this->settings->quantity_breaks ? $this->settings->quantity_breaks->cart_redirect : '',
				'checkout_redirect'        => $this->settings && $this->settings->quantity_breaks ? $this->settings->quantity_breaks->checkout_redirect : '',
				'bundle_cart_redirect'     => $this->settings && $this->settings->quantity_breaks ? $this->settings->quantity_breaks->cart_redirect : '',
				'bundle_checkout_redirect' => $this->settings && $this->settings->quantity_breaks ? $this->settings->quantity_breaks->checkout_redirect : '',
				'atc_price_text'           => $this->settings && $this->settings->quantity_breaks ? $this->settings->quantity_breaks->atc_price_text : '',
		...
		public function enqueue_style()
		{
			...
			wp_register_style('bdlr-bundle-style', BDLR_PLUGIN_URL . 'assets/css/bundle.css', false, BDLR_VERSION, 'all');

			wp_enqueue_style('bdlr-style');
			wp_enqueue_style('bdlr-bundle-style');
		}
		...
		public function includes()
		{
			...
			require_once BDLR_PLUGIN_DIR . 'app/models/bundle.php';
		...
		public function objects()
		{
			...
			$this->settings = $settings_controller->get_settings() ? $settings_controller->get_settings() : $settings_controller->get_default_settings();
