...
class Offer
{
     ...
     * @return array
     */
    public static function get_bundle_offers()
    {
        global $wpdb;
    ...
    public static function get_highest_priority_offer_by_product_id($product_id)
    {
        ...
        $query_specific = $wpdb->prepare(
         ...
         FROM $offer_table o 
         LEFT JOIN $offer_products_table op ON o.id = op.offer_id 
         WHERE op.product_id = %d AND o.status = 'on' AND o.type = 'volume_discount'
         GROUP BY o.id 
        ...
        $query_all = "SELECT o.*, GROUP_CONCAT(op.product_id) as product_ids
                  FROM $offer_table o 
                  LEFT JOIN $offer_products_table op ON o.id = op.offer_id 
                  WHERE o.apply_on = 'all_products' AND o.status = 'on' AND o.type = 'volume_discount'
                  GROUP BY o.id 
                  ORDER BY o.priority DESC 
                  LIMIT 1";
     ...
     * @param mixed $product_id
     * 
     * @return Bundler\Models\Offer|null
     */
    public static function get_highest_priority_bundle_by_product_id($product_id)
    {
        global $wpdb;
        ...
        $query = $wpdb->prepare(
         ...
         FROM $offer_table o 
         INNER JOIN $offer_products_table op ON o.id = op.offer_id 
         WHERE op.product_id = %d AND o.status = 'on' AND o.type = 'bundle'
         ORDER BY o.priority DESC 
         LIMIT 1",
            $product_id
        );

        $highest_priority_bundle = $wpdb->get_row($query);

        if (!$highest_priority_bundle) {
            return null;
        }

        $query = $wpdb->prepare(
            "SELECT o.*, GROUP_CONCAT(DISTINCT op.product_id ORDER BY op.id ASC) as product_ids
         FROM $offer_table o 
         LEFT JOIN $offer_products_table op ON o.id = op.offer_id 
         WHERE o.id = %d
         GROUP BY o.id",
            $highest_priority_bundle->id
        );

        $result = $wpdb->get_row($query);

        if ($result && !empty($result->product_ids)) {
            $result->product_ids = explode(',', $result->product_ids);
        }
    ...
    public static function update_offer_products($offer_id, $product_ids)
    {
        ...
        foreach ($product_ids as $product_id) {
            $wpdb->insert($offer_products_table, array(
                'offer_id' => $offer_id,
    ...
    public static function get_offer_products($offer_id)
    {
        ...
        $offer_products_table = $wpdb->prefix . DB::$offer_products_table_name;
        $query = $wpdb->prepare("
            SELECT op.* FROM $offer_products_table 
            WHERE op.offer_id = %s
        ", $offer_id);
