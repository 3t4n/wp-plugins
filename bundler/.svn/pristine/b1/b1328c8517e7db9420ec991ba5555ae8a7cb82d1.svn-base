...
class DB
{
    ...
    public static $bundle_settings_table_name          = "wbdl_bundle_settings";

    // current database version
    public static $current_db_version = '5.0';
    ...
    public static function create_tables()
    {
        ...
        if (count($existing_tables) > 0) { // existing tables => upgrade the db structure
            if ($installed_db_version && ($installed_db_version != self::$current_db_version)) { // only upgrade if the current version of the db is different than installed version
                ...
                self::create_bundle_table($wpdb);
                self::create_bundle_settings_table($wpdb);
    ...
    public static function create_volume_discount_table($wpdb)
    {
        ...
        $sql = "CREATE TABLE $volume_discount_table ( 
            ...
            global_variant varchar(5) DEFAULT 'off',
     ...
     * @return void
     */
    public static function create_bundle_table($wpdb)
    {
        $charset_collate = $wpdb->get_charset_collate();

        $table_name = $wpdb->prefix . self::$bundle_table_name;

        $sql = "CREATE TABLE $table_name ( 
            id mediumint(8) unsigned NOT NULL AUTO_INCREMENT,
            offer_id INT(11) DEFAULT NULL,
            discount_type varchar(255) DEFAULT '' NOT NULL,
            discount_method varchar(15) DEFAULT NULL,
            discount_value varchar(8) DEFAULT NULL,
     ...
     * @return void
     */
    public static function create_bundle_settings_table($wpdb)
    {

        $charset_collate = $wpdb->get_charset_collate();

        $table_name = $wpdb->prefix . self::$bundle_settings_table_name;
