 ...
 *
 */
if ( ! class_exists( 'ATCBW_Field_link_color' ) ) {
  class ATCBW_Field_link_color extends ATCBW_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'atcbw' ),
        'hover'    => esc_html__( 'Hover', 'atcbw' ),
        'active'   => esc_html__( 'Active', 'atcbw' ),
        'visited'  => esc_html__( 'Visited', 'atcbw' ),
        'focus'    => esc_html__( 'Focus', 'atcbw' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="atcbw--left atcbw-field-color">';
          echo '<div class="atcbw--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="atcbw-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
