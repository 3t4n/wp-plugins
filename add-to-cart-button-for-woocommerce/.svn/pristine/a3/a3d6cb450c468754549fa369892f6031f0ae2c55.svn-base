 ...
 *
 */
if ( ! class_exists( 'ATCBW_Field_sorter' ) ) {
  class ATCBW_Field_sorter extends ATCBW_Fields {
    ...
    public function render() {
      ...
      $args = wp_parse_args( $this->field, array(
        'disabled'       => true,
        'enabled_title'  => esc_html__( 'Enabled', 'atcbw' ),
        'disabled_title' => esc_html__( 'Disabled', 'atcbw' ),
      ) );
      ...
      echo '<div class="atcbw-sorter" data-depend-id="'. esc_attr( $this->field['id'] ) .'"></div>';

      echo ( $args['disabled'] ) ? '<div class="atcbw-modules">' : '';

      echo ( ! empty( $args['enabled_title'] ) ) ? '<div class="atcbw-sorter-title">'. esc_attr( $args['enabled_title'] ) .'</div>' : '';
      echo '<ul class="atcbw-enabled">';
      ...
      if ( $args['disabled'] ) {
        ...
        echo '<div class="atcbw-modules">';
        echo ( ! empty( $args['disabled_title'] ) ) ? '<div class="atcbw-sorter-title">'. esc_attr( $args['disabled_title'] ) .'</div>' : '';
        echo '<ul class="atcbw-disabled">';
