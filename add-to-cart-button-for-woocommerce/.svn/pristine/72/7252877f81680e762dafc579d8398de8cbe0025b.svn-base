...
( function( blocks, blockEditor, element, components ) {
  ...
  if ( !window.atcbw_gutenberg_blocks ) { return; }

  Object.values(window.atcbw_gutenberg_blocks).forEach( function( block ) {
    ...
    registerBlockType(block.name, {
      ...
      edit: function (props) {
        return (
          createElement('div', {className: 'atcbw-shortcode-block'},

            createElement(Button, {
              'data-modal-id': block.modal_id,
              'data-gutenberg-id': block.name,
              className: 'is-secondary atcbw-shortcode-button',
              onClick: function () {
                window.atcbw_gutenberg_props = props;
              },
            }, block.button_title ),
