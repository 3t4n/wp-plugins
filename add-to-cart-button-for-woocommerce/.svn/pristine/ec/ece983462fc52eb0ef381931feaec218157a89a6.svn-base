 ...
 *
 */
if ( ! class_exists( 'ATCBW_Abstract' ) ) {
#[AllowDynamicProperties]
abstract class ATCBW_Abstract {
  ...
  public function __construct() {
    ...
    if ( ! empty( $this->args['output_css'] ) || ! empty( $this->args['enqueue_webfont'] ) ) {
      ...
      ATCBW::$css = apply_filters( "atcbw_{$this->unique}_output_css", ATCBW::$css, $this );
    }
  ...
  public function recursive_output_css( $fields = array(), $combine_field = array() ) {
    ...
    if ( ! empty( $fields ) ) {
      ...
      foreach ( $fields as $field ) {
        ...
        $field_class  = 'ATCBW_Field_' . $field_type;
