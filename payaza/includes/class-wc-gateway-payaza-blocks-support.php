...
final class WC_Gateway_Payaza_Blocks_Support extends AbstractPaymentMethodType {
	 ...
	 * @var string
	 */
	protected $name = 'payaza';

	/**
	 * Initializes the payment method type.
	 */
	public function initialize() {
		$this->settings = get_option( 'woocommerce_payaza_settings', array() );
	...
	public function is_active() {
		...
		return $payment_gateways['payaza']->is_available();
	}
	...
	public function get_payment_method_script_handles() {
		$script_asset_path = plugins_url( '/assets/js/blocks/frontend/blocks.asset.php', WC_PAYAZA_MAIN_FILE );
		$script_asset      = file_exists( $script_asset_path )
			? require $script_asset_path
			: array(
				'dependencies' => array(),
				'version'      => WC_PAYAZA_VERSION,
			);

		$script_url = plugins_url( '/assets/js/blocks/frontend/blocks.js', WC_PAYAZA_MAIN_FILE );
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'wc-payaza-blocks', 'woo-payaza', );
		}

		return array( 'wc-payaza-blocks' );
	}
	...
	public function failed_payment_notice( PaymentContext $context, PaymentResult &$result ) {
		if ( 'payaza' === $context->payment_method ) {
			add_action(
				'wc_gateway_payaza_process_payment_error',
