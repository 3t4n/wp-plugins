...
/*
	...
	Tags: contact, form, contact form, email, mail
	Author: Jeff Starr
...
$contact_coldform_wp_vers = '4.6';
$contact_coldform_version = '20241102';
$contact_coldform_plugin  = 'Contact Coldform';
...
function coldform_i18n_init() {
	...
	load_plugin_textdomain('contact-coldform', false, dirname($contact_coldform_path) .'/languages/');
	
}
add_action('init', 'coldform_i18n_init');



function contact_coldform_require_wp_version() {
	
	global $contact_coldform_path, $contact_coldform_plugin, $contact_coldform_wp_vers;
...
function contact_coldform_footer_text($text) {
	
	$screen_id = contact_coldform_get_current_screen_id();
	
	$ids = array('settings_page_contact-coldform/contact-coldform');
	
	if ($screen_id && apply_filters('contact_coldform_admin_footer_text', in_array($screen_id, $ids))) {
		
		$text = __('Like this plugin? Give it a', 'contact-coldform');
		
		$text .= ' <a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/support/plugin/contact-coldform/reviews/?rate=5#new-post">';
		
		$text .= __('★★★★★ rating&nbsp;&raquo;', 'contact-coldform') .'</a>';
...
add_filter('admin_footer_text', 'contact_coldform_footer_text');



function contact_coldform_get_current_screen_id() {
...
function contact_coldform_name() {
	
	return isset($_POST['coldform_name']) ? esc_attr($_POST['coldform_name']) : '';
	
}

function contact_coldform_email() {
	
	return isset($_POST['coldform_email']) ? sanitize_email($_POST['coldform_email']) : '';
	
}

function contact_coldform_website() {
	
	return isset($_POST['coldform_website']) ? esc_url($_POST['coldform_website']) : '';
	
}

function contact_coldform_subject() {
	
	return isset($_POST['coldform_topic']) ? esc_attr($_POST['coldform_topic']) : '';
	
}

function contact_coldform_response() {
	
	return isset($_POST['coldform_response']) ? esc_attr($_POST['coldform_response']) : '';
	
}

function contact_coldform_message() {
	
	return isset($_POST['coldform_message']) ? esc_textarea($_POST['coldform_message']) : '';
	
}

function contact_coldform_strings() {
	
	$contact_coldform_name     = contact_coldform_name();
	$contact_coldform_email    = contact_coldform_email();
	$contact_coldform_website  = contact_coldform_website();
	$contact_coldform_subject  = contact_coldform_subject();
	$contact_coldform_response = contact_coldform_response();
	$contact_coldform_message  = contact_coldform_message();
...
function contact_coldform_input_filter() {
	...
	$dspcp = isset($options['display_captcha'])   ? $options['display_captcha']   : true;
	...
	$contact_coldform_strings = contact_coldform_strings();
	
	$key = isset($_POST['coldform_key']) ? sanitize_text_field($_POST['coldform_key']) : '';
	
	$nonce = (isset($_POST['coldform_nonce']) && !empty($_POST['coldform_nonce'])) ? sanitize_text_field($_POST['coldform_nonce']) : false;
	...
	if (!wp_verify_nonce($nonce, 'coldform_nonce')) return false;
	...
	if ($dspcp) {
		...
		if (($trust == false) || ($trust == true && !is_user_logged_in())) {
...
function contact_coldform_display_form() {
	...
	if ($options['display_captcha'] == true) {
		
		if (($options['coldform_trust'] == false) || ($options['coldform_trust'] == true && !is_user_logged_in())) {
		...
		<div id="coldform">
			<form action="" method="post">
				...
				<div class="coldform-submit">
					<input type="submit" id="coldform_submit" name="coldform_submit" value="'. esc_attr__('Send it!', 'contact-coldform') .'" />'. "\n\t\t\t\t\t" .
					wp_nonce_field('coldform_nonce', 'coldform_nonce', false, false) . "\n\t\t\t\t\t" .
					'<input type="hidden" id="coldform_key" name="coldform_key" value="process" />
				</div>
			</form>
		</div>
		'. $styles .'
...
function contact_coldform() {
	...
	$recipient    = isset($options['coldform_email'])   ? $options['coldform_email']   : null;
	$recipfrom    = isset($options['coldform_from'])    ? $options['coldform_from']    : null;
	$recipname    = isset($options['coldform_name'])    ? $options['coldform_name']    : null;
	$recipsite    = isset($options['coldform_website']) ? $options['coldform_website'] : null;
	$success      = isset($options['coldform_success']) ? $options['coldform_success'] : null;
	$thanks       = isset($options['coldform_thanks'])  ? $options['coldform_thanks']  : null;
	$offset       = isset($options['coldform_offset'])  ? $options['coldform_offset']  : null;
	$subject      = isset($options['coldform_subject']) ? $options['coldform_subject'] : null;
	$prefix       = isset($options['coldform_prefix'])  ? $options['coldform_prefix']  : null;
	$custom       = isset($options['coldform_custom'])  ? $options['coldform_custom']  : null;
	$disable_data = isset($options['disable_data'])     ? $options['disable_data']     : null;
	...
	$from    = !empty($recipfrom) ? $recipfrom : $recipient;
	
	$topic_display = $topic;
	
	$topic_send = $prefix . $topic;
	
	$message_display = htmlentities($message, ENT_QUOTES, get_option('blog_charset', 'UTF-8'));
...
Host:   $host
Agent:  $agent
Whois:  https://whois.arin.net/rest/net/NET-74-82-224-0-1/pft?s=$ip
...
";
	...
	foreach ($recipients as $recipient) wp_mail($recipient, $topic_send, $message_send, $headers);
...
function contact_coldform_plugin_action_links($links, $file) {
	...
	if ($file == $contact_coldform_path && current_user_can('manage_options')) {
...
function add_coldform_links($links, $file) {
	...
	if ($file == $contact_coldform_path) {
		...
		$home_href  = 'https://perishablepress.com/contact-coldform/';
		$home_title = esc_attr__('Plugin Homepage', 'contact-coldform');
		$home_text  = esc_html__('Homepage', 'contact-coldform');
		
		$links[] = '<a target="_blank" rel="noopener noreferrer" href="'. $home_href .'" title="'. $home_title .'">'. $home_text .'</a>';
		
		$rate_href  = 'https://wordpress.org/support/plugin/contact-coldform/reviews/?rate=5#new-post';
		$rate_title = esc_attr__('Give Contact Coldform a 5-star rating at WordPress.org', 'contact-coldform');
		$rate_text  = esc_html__('Rate this plugin', 'contact-coldform');
		
		$links[] = '<a target="_blank" rel="noopener noreferrer" href="'. $rate_href .'" title="'. $rate_title .'">'. $rate_text .'&nbsp;&raquo;</a>';
...
if (isset($contact_coldform_options['default_options']) && $contact_coldform_options['default_options'] == 1) {
	
	register_uninstall_hook (__FILE__, 'contact_coldform_delete_plugin_options');
	
}

function contact_coldform_get_domain() {
...
function contact_coldform_add_defaults() {
	...
	if ((isset($tmp['default_options']) && $tmp['default_options'] == '1') || (!is_array($tmp))) {
		...
		$arr = array(
			...
			'coldform_from'     => contact_coldform_get_domain(),
			...
			'coldform_success'  => '<p id="coldform_success" class="coldform_success">'. esc_html__('Success! Your message has been sent.', 'contact-coldform') .'</p>',
...
function contact_coldform_validate_options($input) {
	...
	$input['coldform_from']     = esc_attr($input['coldform_from']);
	...
	if (!isset($input['disable_data'])) $input['disable_data'] = null;
	$input['disable_data'] = ($input['disable_data'] == 1 ? 1 : 0);
...
function contact_coldform_render_form() {
	...
	<style type="text/css">
		...
		#mm-plugin-options .upgrade-x { margin-top: 0; padding: 10px; border: 1px solid #99cc99; background-color: #e9ffe9; }
		...
		#mm-plugin-options .mm-code { 
			display: inline-block; margin: 0 1px; padding: 5px; direction: ltr; unicode-bidi: embed;
		...
		@media (max-width: 880px) {
			#mm-panel-overview .mm-panel-overview { padding-left: 0; background-image: none; }
	...
	<div id="mm-plugin-options" class="wrap">
		<h1><?php echo $contact_coldform_plugin; ?> <small><?php echo $contact_coldform_version; ?></small></h1>
		...
		<form method="post" action="options.php">
				...
				<div class="meta-box-sortables ui-sortable">
					<div id="mm-panel-overview" class="postbox">
						...
						<div class="toggle<?php if (isset($_GET['settings-updated'])) { echo ' default-hidden'; } ?>">
							<div class="mm-panel-overview">
								<p class="upgrade-x">
									<strong><?php esc_html_e('Like Coldform?', 'contact-coldform'); ?></strong> 
									<?php esc_html_e('Check out', 'contact-coldform'); ?> 
									<a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/plugins/contact-form-x/"><?php esc_html_e('Contact Form X', 'contact-coldform'); ?></a>. 
									<?php esc_html_e('It&rsquo;s a better-built, easier-to-use, Ajax-powered contact form.', 'contact-coldform'); ?>
								</p>
								<p>
								...
								<ul>
									...
									<li>
										...
										<a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/plugins/contact-coldform/"><?php esc_html_e('Coldform Homepage', 'contact-coldform'); ?>&nbsp;&raquo;</a>
									</li>
									<li><?php esc_html_e('If you like this plugin, please', 'contact-coldform'); ?> 
										<a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/support/plugin/contact-coldform/reviews/?rate=5#new-post" title="<?php esc_attr_e('Thank you for your support!', 'contact-coldform'); ?>">
							...
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									<tr>
										...
										<div class="mm-item-caption"><?php esc_html_e('Where shall Coldform send your messages? (separate multiple emails with comma)', 'contact-coldform'); ?></div></td>
									</tr>
									<tr>
										<th scope="row"><label class="description" for="contact_coldform_options[coldform_from]"><?php esc_html_e('From Address', 'contact-coldform'); ?></label></th>
										<td><input type="text" size="50" maxlength="200" name="contact_coldform_options[coldform_from]" value="<?php echo $options['coldform_from']; ?>" />
										<div class="mm-item-caption"><?php esc_html_e('Here you may customize the address used for the &ldquo;From&rdquo; header (see plugin FAQs for info)', 'contact-coldform'); ?></div></td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td>
											...
											<div class="mm-item-caption">
												<?php esc_html_e('For example, +7 or -7. If no offset or unsure, enter 0 (zero).', 'contact-coldform'); ?> 
												<?php esc_html_e('Current time:', 'contact-coldform'); ?> <?php echo date("l, F jS, Y @ g:i a", time() + $offset * 60 * 60); ?>
											</div>
										</td>
									</tr>
									<tr>
										...
										<?php esc_html_e('Display the &ldquo;Website&rdquo; field in the contact form', 'contact-coldform'); ?></td>
									</tr>
									<tr>
										...
										<?php esc_html_e('Display checkbox to send carbon copy to the user', 'contact-coldform'); ?></td>
									</tr>
									<tr>
										<th scope="row"><label class="description" for="contact_coldform_options[disable_data]"><?php esc_html_e('Disable Data', 'contact-coldform'); ?></label></th>
										<td><input type="checkbox" name="contact_coldform_options[disable_data]" value="1" <?php if (isset($options['disable_data'])) { checked('1', $options['disable_data']); } ?> /> 
										<?php esc_html_e('Disable data collection (e.g., IP address, user agent, et al)', 'contact-coldform'); ?></td>
									</tr>
									<tr>
										<th scope="row"><?php esc_html_e('More Options', 'contact-coldform'); ?></th>
										<td>
											<?php esc_html_e('For more options and form fields, check out my new and improved contact-form plugin,', 'contact-coldform'); ?> 
											<a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/plugins/contact-form-x/" title="<?php esc_html_e('Download at WordPress Plugin Directory', 'contact-coldform'); ?>">Contact Form X &raquo;</a>
										</td>
									</tr>
								</table>
							</div>
							...
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									...
									<tr>
										<th scope="row"><?php esc_html_e('More Options', 'contact-coldform'); ?></th>
										<td>
											<?php esc_html_e('For more options like Google reCaptcha, check out my new and improved contact-form plugin,', 'contact-coldform'); ?> 
											<a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/plugins/contact-form-x/" title="<?php esc_html_e('Download at WordPress Plugin Directory', 'contact-coldform'); ?>">Contact Form X &raquo;</a>
										</td>
									</tr>
								</table>
							...
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									...
									<tr>
										<th scope="row"><?php esc_html_e('More Options', 'contact-coldform'); ?></th>
										<td>
											<?php esc_html_e('For more options and ways to customize, check out my new and improved contact-form plugin,', 'contact-coldform'); ?> 
											<a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/plugins/contact-form-x/" title="<?php esc_html_e('Download at WordPress Plugin Directory', 'contact-coldform'); ?>">Contact Form X &raquo;</a>
										</td>
									</tr>
								</table>
					...
					<div id="mm-panel-tertiary" class="postbox">
						...
						<div class="toggle<?php if (!isset($_GET['settings-updated'])) { echo ' default-hidden'; } ?>">
							...
							<p>
								...
								<a target="_blank" rel="noopener noreferrer" href="https://m0n.co/b">https://m0n.co/b</a>
							</p>
							<div class="mm-table-wrap">
								<table class="widefat mm-table">
									<tr>
					...
					<div id="mm-panel-secondary" class="postbox">
						...
						<div class="toggle default-hidden">
							...
							<p><span class="code mm-code">[coldform]</span></p>
							...
							<p><span class="code mm-code">&lt;?php if (function_exists('contact_coldform_public')) contact_coldform_public(); ?&gt;</span></p>
						</div>
					</div>
					<div id="mm-restore-settings" class="postbox">
						...
						<div class="toggle<?php if (!isset($_GET['settings-updated'])) { echo ' default-hidden'; } ?>">
							<p>
								...
								<?php esc_html_e('Restore default options upon plugin deactivation/reactivation.', 'contact-coldform'); ?>
							</p>
							<p>
					...
					<div id="mm-panel-current" class="postbox">
						<h2><?php esc_html_e('WordPress Resources', 'contact-coldform'); ?></h2>
						<div class="toggle">
			...
			<div class="mm-credit-info">
				<a target="_blank" rel="noopener noreferrer" href="<?php echo $contact_coldform_homeurl; ?>" title="<?php echo $contact_coldform_plugin; ?> Homepage"><?php echo $contact_coldform_plugin; ?></a> by 
				<a target="_blank" rel="noopener noreferrer" href="https://twitter.com/perishable" title="Jeff Starr on Twitter">Jeff Starr</a> @ 
				<a target="_blank" rel="noopener noreferrer" href="https://monzillamedia.com/" title="Obsessive Web Design &amp; Development">Monzilla Media</a>
			</div>
		</form>
	</div>
