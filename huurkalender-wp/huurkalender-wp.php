 ...
 * Plugin URI: 
 * Description: Huurkalender WP-plugin: Integreer kalender/boekingsmodule. Instellingen en gebruiksaanwijzing: Wordpress > Instellingen > Huurkalender plugin.
 * Version: 1.6.4
 * Author: Huurkalender
 * Author URI: https://www.huurkalender.nl
 * License: GPL-2.0+
 * License URI: http://www.gnu.org/licenses/gpl-2.0.txt
 * Text Domain: huurkalender-wp
...
class TSHCAL
{
    ...
    function __construct()
    {
        ...
        add_action('wp_enqueue_scripts', array($this, 'huurkalender_enqueue_assets'));
    }
    ...
    public function plugin_url()
    {
      ...
      public function huurkalender_enqueue_assets()
    {
        $url = 'https://www.huurkalender.nl/'; 
        wp_enqueue_script('huurkalender-script', esc_url($url . 'online/embed/huurkalender.js'), array(), $this->version, true);
        wp_enqueue_style('huurkalender-style', esc_url($url . 'online/embed/huurkalender.css'), array(), $this->version);
    }
    ...
    {
        $def = array("color_bg_calendar" => 'none', "color_bg" => 'none', 'type' => 0, 'lang' => 0, 'id' => 0, 'start' => 0, 'month' => 0);
        $atts = shortcode_atts($def, preg_replace("/<.+>/sU", "", $atts));

        // loop attributes, convert to int except lang and color
        foreach ($atts as $key => $value) {
            if ($key != 'lang' && $key != 'color_bg' && $key != 'color_bg_calendar' && $key != 'start' && $key != 'type') {
                // if isset and numeric
                if (isset($atts[$key])) {
                    $atts[$key] = (int) $value;
                } else {
                    $atts[$key] = $def[$key];
                ...
                // only allow numbers and letters and # and - 
                $atts[$key] = preg_replace('/[^a-zA-Z0-9#-]/', '', $value);
            }
        }
        ...
        if ($lang == 'auto') {
            ...
            if (isset($_GET['lang'])) {
                // sanitize get 
                $lang_get = sanitize_key($_GET['lang']);
                $lang_get = preg_replace('/[^a-zA-Z]/', '', $lang_get);
                if (!empty($lang_get)) {
                    $lang = $lang_get;
                }
            }
            ...
            if (function_exists('pll_current_language')) {
                ...
                if (!empty($polylang)) {
                    // only allow letters
                    $polylang = preg_replace('/[^a-zA-Z]/', '', $polylang);
                    $lang = $polylang;
                }
            } else {
                // check wmpml
        ...
        if (is_numeric($atts['id'])) {
            // Prepare the common parts of the URL
            $iframe_src = esc_url($url . $path . '/wordpress-wp-' . $atts['id'] . '.html?type=iframe&wordpress&lang=' . $lang . $month . $start . $color_bg);

            // Prepare the iframe ID
            $iframe_id = esc_attr($iframe . $atts['id']);

            // Construct the HTML
            if ($type == 'combi') {
                $html = sprintf(
                    '<div class="huurkalender-embed huurkalender-embed-container_overview"><iframe id="%s" src="%s" frameborder="0" width="100%%" allowfullscreen></iframe></div>',
                    $iframe_id,
                    $iframe_src
                );
            } else {
                $html = sprintf(
                    '<div class="huurkalender-embed huurkalender-embed-container"><iframe id="%s" src="%s" frameborder="0" width="100%%" allowfullscreen></iframe></div>',
                    $iframe_id,
                    $iframe_src
                );
            }
        } else {
            $html = '<style>.calendar_alert a{color:#0073aa}.calendar_alert{font-size:14px;line-height:20px;color:red;margin:0!important;height:250px;display:block;border:1px solid red;padding:20px;font-family:proxima-nova,"Helvetica Neue",Helvetica,Arial,sans-serif}</style><span class="calendar_alert">
           <b>ERROR onjuist ID</b><br>Voer een juist KALENDER-ID of ACCOUNT-ID in.<br>Uw ID kunt u vinden via: <a href="https://www.huurkalender.nl/account/widget/" target="_blank">https://www.huurkalender.nl/account/widget/</a><br><br>Kijk voor de handleiding in de kennisbank <a href="https://www.huurkalender.nl/kennisbank/handleidingen/" target="_blank">Huurkalender.nl/kennisbank</a>.<br>Vragen? Stuur een bericht via <a href="https://www.huurkalender.nl/#contact" target="_blank">Huurkalender.nl</a> wij helpen u graag.</span>';
        }
        return $html;
    }
    ...
    public function ts_register_submenu_page()
    {
        ...
        add_submenu_page(
            'options-general.php',
            __('Huurkalender plugin', 'huurkalender-wp'),
            __('Huurkalender plugin', 'huurkalender-wp'),
            'manage_options',
            'huurkalender-wp',
            array($this, 'ts_render_submenu_page')
        );
    }
...
?>
        ...
        <div class="wrap">
            <h2 style=""><?php echo esc_html__('Huurkalender WP plugin', 'huurkalender-wp'); ?></h2>
            <p>
            ...
            <ol>
                ...
                <li>Voeg uw gewenste attribute toe:<br><br>
                    ...
                    <span class="huurkal_set_code">[huurkalender id="KALENDER-ID" type="kalender" <b>start="<?php echo esc_attr(gmdate('Y')); ?>-01"</b>]</span><br>
...
function salcode_add_plugin_page_settings_link($links)
{
    $links[] = '<a href="' .
        ...
        '">' . __('Instellingen', 'huurkalender-wp') . '</a>';
    return $links;
}

function lang_sanitize($lang) {
    $lang = substr(strtolower(preg_replace('/[^a-z]/', '', $lang)), 0, 2);
    return $lang ;
}
