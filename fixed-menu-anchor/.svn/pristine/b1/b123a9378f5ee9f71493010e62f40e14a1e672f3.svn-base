...
> **Mobile First Feature and Premium Support** <br>
> The Plugins First team provides active one to one email support for everybody who bought [the PRO-version](https://plugins-first.io/en/product/fixed-menu-anchor-wordpress-plugin/). The PRO version comes with premium support and the mobile first feature.
...
Yes, with version 2 (only [PRO-version](https://plugins-first.io/en/shop/)), we introduced a feature, that lets you set a certain screen width and a value for the distance, which only applies until that width is reached. After the width is higher, the default value will be used.
...
1. With and without the plugin - On the right side you see how it looks *without* this plugin: your anchor-target is overlapped by the sticky menu. In comparison to that, on the left side, you see how it looks *with* this plugin: you will jump a little bit before the anchor-target so it will not be overlapped.
2. Administration user interface which contains two settings: set standard distance and a CSS class for DOM elements to ignore click events.
3. Additional feature: Mobile first. It allows to set a maximum width and according distance, which will be used instead of the standard distance. (only PRO-version)
