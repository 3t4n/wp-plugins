...
*/
	...
	function jalbumbridge_shortcode( $atts, $content = null ) {
	    ...
	    $ar = (array_key_exists('ar', $attributes) && is_numeric($attributes['ar']) && $attributes['ar'] > 10 && $attributes['ar'] < 400)?
	    	(int)$attributes['ar']
	    	:
	    	$ar = 75;
	...
	function jalbumbridge_dynamic_render_callback( $attributes ) {
	    ...
	    $align = (array_key_exists('align', $attributes) && in_array($attributes['align'], [ 'left', 'center', 'right' ]))?
	    	$attributes['align']
	    	: 
	    	'center';
