...
class WC_Gateway_Fygaro extends WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		if ( 'production' === $mode ) {
			...
			$payload_arr = [
				'amount' => $total,
				'currency' => $currency,
				'custom_reference' => strval($order_id)
			];

			if($total_tax > 0) {
				$payload_arr['tax'] = $total_tax;
			}

			if($exp_date > 0) {
				$payload_arr['exp'] = strtotime( date('c',$timeNow)." + ".$exp_date." minutes" );
			}
	...
	public function webhook() {
		...
		if ($is_token_expired || !$is_signature_valid) {
			...
			$order = wc_get_order( $order_id );
  			$order->payment_complete();
  			$order->add_order_note( "Payment Reference: ".$payload_decode->reference );
		}
