...
function aicc_settings_validate_openaikey( $input ) {
	...
	if ( ! empty( $input[ $setting ] ) ) {
		...
		if ( ! preg_match( '/^(sk\-)?[a-zA-Z0-9_\-]{32,256}$/', $input[ $setting ] ) &&
		! preg_match( '/^(sk\-project\-)?[a-zA-Z0-9_\-]{32,256}$/', $input[ $setting ] ) &&
		! preg_match( '/^(sk\-proj\-)?[a-zA-Z0-9_\-]{32,256}$/', $input[ $setting ] )
		) {
