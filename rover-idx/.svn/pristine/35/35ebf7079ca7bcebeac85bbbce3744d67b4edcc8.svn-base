...
#						}
		...
		#	Update `search_datetime_`

		if (is_array($social_settings = get_option(ROVER_OPTIONS_SOCIAL)))
			{
			date_default_timezone_set('UTC');

			$key								= self::get_last_search_date_key();
			$social_settings[$key]				= date('Y-m-d H:i:s', strtotime('now'));

			update_option(ROVER_OPTIONS_SOCIAL, $social_settings);
			}
		}
