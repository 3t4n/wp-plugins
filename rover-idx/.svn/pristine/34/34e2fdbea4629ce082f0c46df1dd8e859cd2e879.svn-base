...
class Rover_IDX_SITEMAP {
	...
	private static function sitemap_file_write($result_decoded)
		{
		$return_gz_url						= self::$upload_url.'/'.self::$sitemap_file.'.gz';

		if (
			isset($result_decoded['all_sitemaps']) &&
			isset($result_decoded['all_sitemaps']['version']) &&
			version_compare($result_decoded['all_sitemaps']['version'], '3.0.0.2907', 'gt')
			)
			{
			if (
				isset($result_decoded['all_sitemaps']['sitemaps']) &&
				is_array($result_decoded['all_sitemaps']['sitemaps']) &&
				isset($result_decoded['all_sitemaps']['sitemap_index']) &&
				is_array($result_decoded['all_sitemaps']['sitemap_index'])
				)
				{
				##################################################################################
				#	This will copy each of the indexed (1,000 entries per file) sitemaps (gz & xml)
				#	rover_sitemap_CCIAOR-0.xml
				##################################################################################

				foreach($result_decoded['all_sitemaps']['sitemaps'] as $one_sitemap)
					{
					if (isset($one_sitemap['xml']))
						self::copy_file_to_local($one_sitemap['xml'], basename($one_sitemap['xml']));
					if (isset($one_sitemap['gz']))
						self::copy_file_to_local($one_sitemap['gz'], basename($one_sitemap['gz']));
					}

				##################################################################################
				#	This will copy the single sitemap index that references each indexed sitemap
				##################################################################################

				foreach($result_decoded['all_sitemaps']['sitemap_index'] as $one_sitemap)	#	There is only one of these
					{
					if (isset($one_sitemap['xml']))
						self::copy_file_to_local($one_sitemap['xml'], basename($one_sitemap['xml']));
					if (isset($one_sitemap['gz']))
						...
						#	This is the sitemap index in gz format
						$return_gz_url		= rtrim(self::$upload_url,"/")."/".$one_sitemap['gz'];
						}
					}
				}
			}
		...
		return $return_gz_url;
		}

	static function copy_file_to_local($dest_url, $local_file)
		{
		$local_file							= rtrim(self::$upload_dir,"/")."/".$local_file;
