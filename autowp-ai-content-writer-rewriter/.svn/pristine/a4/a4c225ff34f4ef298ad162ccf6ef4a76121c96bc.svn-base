  ...
  <div class="form-group">
    <div class="card text-center">
      <div class="card-body">
        ...
        <p class="card-text"> <?php echo esc_html__('User Type:', 'autowp'); ?> <?php echo esc_html($isUserPremium); ?></p>
        <p class="card-text"> <?php echo esc_html__('Renewal Date:', 'autowp'); ?> <?php echo esc_html($user['renewal_date'] ?? esc_html__('Free User', 'autowp')); ?></p>
        <p class="card-text"> <?php echo esc_html__('Total Post Balance:', 'autowp'); ?> <?php echo esc_html($user['total_post_balance']) . ' ' . esc_html__('Posts', 'autowp'); ?></p>
  ...
  if (is_wp_error($response)) {
    ...
    if ( get_locale() === 'tr_TR' ) {
      echo '<div class="card text-center" style="width: 100%; margin-bottom: 20px;">
          <div class="card-body">
              <h4 class="card-title" style="font-weight: bold; color: #FF6347;">' . esc_html__('R10 Üyelerine Özel %400 İndirimli Fiyatlar için Tıklayın!', 'autowp') . '</h4>
...
private $table_data;
    ...
    function prepare_items()
    {
      ...
      function column_website_type($item){
        ...
        $website_page = autowp_get_page_slug_from_website_type($website_type);



        $actions = array(
            'edit' => sprintf('<a href="?page=' . $website_page . '&id=%s">%s</a>', $item['id'], __('Edit', 'autowp')),
            'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', sanitize_text_field($_REQUEST['page']), $item['id'], __('Delete', 'autowp')),
        );
...
<!-- Anthropic API Key -->
<div class="form-group">
    <label class="col-md-4 control-label" for="anthropic_api_key"><?php esc_html_e('Anthropic API Key', 'autowp'); ?></label>
    <div class="col-md-4">
        <input id="anthropic_api_key" name="anthropic_api_key" type="text" class="form-control" value="<?php echo esc_attr(unserialize(get_option("autowp_settings"))["anthropic_api_key"]); ?>" placeholder="<?php esc_html_e('Enter your Anthropic API key', 'autowp'); ?>">
