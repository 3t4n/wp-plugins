...
* Author: CRM Perks
* Text Domain: contact-form-infusionsoft
...
if( !class_exists( 'vxcf_infusionsoft' ) ):


class vxcf_infusionsoft {
 ...
 public  $url = "https://www.crmperks.com";
  ...
  public static $title='Contact Form Infusionsoft Plugin';  
  public static $path = ''; 
  public static $slug = "";
...
load_plugin_textdomain('contact-form-infusionsoft-crm', FALSE,  $this->plugin_dir_name(). '/languages/' );
  ...
  $install=new vxcf_infusionsoft_install();
...
$this->push($entry,$form,'',false);    
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo esc_attr($id) ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Contact Form Infusionsoft Plugin','contact-form-infusionsoft-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','contact-form-infusionsoft-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo esc_attr($id) ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Contact Form Infusionsoft Plugin','contact-form-infusionsoft-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sContact Form Entries%s is required. %s it today!%s", 'contact-form-infusionsoft-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Contact Form Entries is installed but not active. %sActivate Contact Form Entries%s to use the Contact Form Infusionsoft Plugin','contact-form-infusionsoft-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=contact-form-entries-pro/contact-form-entries-pro.php'), 'activate-plugin_contact-form-entries-pro/contact-form-entries-pro.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sContact Form Entries%s is required. %sPurchase it today!%s", 'contact-form-infusionsoft-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");
  }  
...
$value=trim(implode(' ',$value));
  ...
  * @param mixed $form
  */
  public  function check_filter($filters,$entry){
  ...
  */
  ...
  $link="N/A"; $desc=__("Added to ",'contact-form-infusionsoft-crm');
  ...
  $desc.=$feed['object'];
  if($feed['status'] == 5){
   $desc=sprintf(__("%s Deleted",'contact-form-infusionsoft-crm'),$feed['object']);   
  }
  }else if($feed['status'] == 4){
      $desc=sprintf(__("%s Filtered",'contact-form-infusionsoft-crm'),$feed['object']); 
  }else{
  $desc= !empty($feed['error']) ? $feed['error'] : esc_html__("Unknown Error",'contact-form-infusionsoft-crm');
  }
  $title=__("Failed",'contact-form-infusionsoft-crm');   
  if($feed['status'] == 1){
  $title=__("Created",'contact-form-infusionsoft-crm');   
  }else if($feed['status'] == 2){
  $title=__("Updated",'contact-form-infusionsoft-crm');   
  }else if($feed['status'] == 4){
  $title=__("Filtered",'contact-form-infusionsoft-crm');   
  }else if($feed['status'] == 5){
  $title=__("Deleted",'contact-form-infusionsoft-crm');   
  }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.esc_attr($this->id).'_update" id="'.esc_attr($this->id).'_update" value="1" /><label for="'.esc_attr($this->id).'_update" title="%s">%s</label><br /><br />', esc_html__('Create or update this entry in Infusionsoft. The fields will be mapped according to the form feed settings.', 'contact-form-infusionsoft-crm'), esc_html__('Send to Infusionsoft', 'contact-form-infusionsoft-crm'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  $button.= '<input type="submit" class="button button-large button-secondary alignright" name="'.esc_attr($this->id).'_send" style="margin-left:5px; '.$margin.'" title="'.__('Create or update this entry in Infusionsoft. The fields will be mapped according to the form feed settings.','contact-form-infusionsoft-crm').'" value="'.__('Send to Infusionsoft', 'contact-form-infusionsoft-crm').'" onclick="jQuery(\'#action\').val(\'send_to_crm\')" />';
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Infusionsoft Logs','contact-form-infusionsoft-crm').'" href="'.esc_url($log_url).'">'.__('Infusionsoft Logs','contact-form-infusionsoft-crm').'</a>';
...
return  $var;
  ...
  */
  ...
  $status=0;
  if(class_exists('vxcf_infusionsoft_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/contact-form-infusionsoft-crm/contact-form-infusionsoft-crm.php')) {
  $status=2;
  } 
  return $status;
  }
...
public function do_actions(){
  ...
  */
  ...
  $this->data=new vxcf_infusionsoft_data();
  return $this->data;
  }
...
if(empty($feed_log)){ $feed_log=array('status'=>''); }
...
$no_filter=true;    
  ...
  $no_filter=$this->check_filter($filters,$entry); 
...
$feed['data']=$data;
  ...
  if($this->post('id',$res)!=""){ 
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Infusionsoft (%s) with ID # %s .', 'contact-form-infusionsoft-crm'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Infusionsoft (%s) with ID # %s . View entry at %s', 'contact-form-infusionsoft-crm'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Infusionsoft', 'contact-form-infusionsoft-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Infusionsoft (%s) with ID # %s . View entry at %s', 'contact-form-infusionsoft-crm'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Infusionsoft (%s) with ID # %s .', 'contact-form-infusionsoft-crm'),$feed['object'],$res['id']);   
            }
          if($event == 'delete'){  
     $screen_msg=__( 'Entry deleted from Infusionsoft', 'contact-form-infusionsoft-crm');
          }else{
     $screen_msg=__( 'Entry updated in Infusionsoft', 'contact-form-infusionsoft-crm');
          }
          }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'contact-form-infusionsoft-crm');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Infusionsoft. Entry not sent! Check the Entry Notes below for more details.' , 'contact-form-infusionsoft-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Infusionsoft. Entry not sent' , 'contact-form-infusionsoft-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'contact-form-infusionsoft-crm'),$feed['object']);
...
//  } 
  ...
  $log_link=' <a href="'.esc_url($log_url).'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','contact-form-infusionsoft-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//$this->add_note($entry["id"], $msg);
  ...
  */
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Infusionsoft','contact-form-infusionsoft-crm')." Error","info_title"=>"More Detail","info"=>$detail);
...
$vxcf_infusionsoft=new vxcf_infusionsoft();
$vxcf_infusionsoft->instance();
$vx_cf['vxcf_infusionsoft']='vxcf_infusionsoft';
endif;
