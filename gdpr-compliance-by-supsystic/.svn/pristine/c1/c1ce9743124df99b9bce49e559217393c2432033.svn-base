...
class dbGdprsup {
     ...
     * #__  Worgdprsupess prefix
     * ^__  Store plugin tables prefix (@see GDPRSUP_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, GDPRSUP_DB_PREF, $wpdb->prefix. GDPRSUP_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampGdprsup($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, GDPRSUP_DATE_DL)) return dateToTimestampGdprsup($date);
        $arr = explode('-', $date);
        return dateToTimestampGdprsup($arr[2]. GDPRSUP_DATE_DL. $arr[1]. GDPRSUP_DATE_DL. $arr[0]);
    }
