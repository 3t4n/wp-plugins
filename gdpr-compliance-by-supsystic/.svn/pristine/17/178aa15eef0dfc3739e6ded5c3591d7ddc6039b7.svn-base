<div id="gdprsupSettingsTabs">
	<section class="supsystic-bar">
		<h3 class="nav-tab-wrapper gdprsupMainTabsNav" style="margin-bottom: 0px; margin-top: 12px;">
			<?php $i = 0;?>
			<?php foreach($this->tabs as $tKey => $tData) { ?>
				<?php
					$iconClass = 'gdprsup-edit-icon';
				...
				<a class="nav-tab <?php if($i == 0) { echo 'nav-tab-active'; }?>" href="#<?php echo $tKey?>">
					...
					<?php }?>
					<span class="gdprsupPopupTabTitle"><?php echo $tData['title']?></span>
				</a>
			<?php $i++; }?>
		</h3>
	</section>
	<section>
		<div class="supsystic-item supsystic-panel" style="padding-left: 10px;">
			<div id="containerWrapper">
				<form id="gdprsupSettingsForm">
					<?php foreach($this->tabs as $tKey => $tData) { ?>
						<div id="<?php echo $tKey?>" class="gdprsupTabContent">
							<?php echo $tData['content']?>
						</div>
					<?php }?>
					<?php echo htmlGdprsup::hidden('mod', array('value' => 'gdpr'))?>
					<?php echo htmlGdprsup::hidden('action', array('value' => 'save'))?>
					<?php echo htmlGdprsup::nonceForAction('save')?>
				</form>
