<?php
class optionsGdprsup extends moduleGdprsup {
	private $_tabs = array();
	private $_options = array();
	private $_optionsToCategoires = array();	// For faster search
	
	public function init() {
		//dispatcherGdprsup::addAction('afterModulesInit', array($this, 'initAllOptValues'));
	...
	public function getAdminPage() {
		//if(installerGdprsup::isUsed()) {
			return $this->getView()->getAdminPage();
		/*} else {
			installerGdprsup::setUsed();	// Show this welcome page - only one time
			frameGdprsup::_()->getModule('supsystic_promo')->getModel()->bigStatAdd('Welcome Show');
			frameGdprsup::_()->getModule('options')->getModel()->save('plug_welcome_show', time());	// Remember this
			return frameGdprsup::_()->getModule('supsystic_promo')->showWelcomePage();
		}*/
	}
	...
	public function getTabs() {
		if(empty($this->_tabs)) {
			$this->_tabs = dispatcherGdprsup::applyFilters('mainAdminTabs', array(
				//'main_page' => array('label' => __('Main Page', GDPRSUP_LANG_CODE), 'callback' => array($this, 'getTabContent'), 'wp_icon' => 'dashicons-admin-home', 'sort_order' => 0), 
			));
	...
	public function getActiveTab() {
		$reqTab = sanitize_text_field(reqGdprsup::getVar('tab'));
		return empty($reqTab) ? GDPRSUP_DEF_ADMIN_TAB : $reqTab;
	}
	public function getTabUrl($tab = '') {
		static $mainUrl;
		if(empty($mainUrl)) {
			$mainUrl = frameGdprsup::_()->getModule('adminmenu')->getMainLink();
		}
	...
	public function getAll() {
		if(empty($this->_options)) {
			$this->_options = dispatcherGdprsup::applyFilters('optionsDefine', array(
				'general' => array(
					'label' => __('General', GDPRSUP_LANG_CODE),
					'opts' => array(
