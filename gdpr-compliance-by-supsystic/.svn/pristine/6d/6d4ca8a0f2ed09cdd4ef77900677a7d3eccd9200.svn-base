<?php
class gdprModelGdprsup extends modelGdprsup {
	...
	public function save($d) {
		...
		frameGdprsup::_()->getModule('options')->getModel()->saveGroup($d);
		$this->setAgreements(isset($d['agreements']) ? $d['agreements'] : array());
	}
	public function getAgreements() {
		if(!$this->_agreementsLoaded) {
			$this->_agreementsLoaded = true;
			$this->_agreements = get_option('_gdprsup_agreements', array());
		}
		return $this->_agreements;
	}
	public function setAgreements($agreements) {
		update_option('_gdprsup_agreements', $agreements);
	}
