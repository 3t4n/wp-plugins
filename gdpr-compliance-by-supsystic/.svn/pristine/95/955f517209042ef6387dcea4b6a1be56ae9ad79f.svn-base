<?php
class mailGdprsup extends moduleGdprsup {
	...
	public function send($to, $subject, $message, $fromName = '', $fromEmail = '', $replyToName = '', $replyToEmail = '', $additionalHeaders = array(), $additionalParameters = array()) {
		//$type = frameGdprsup::_()->getModule('options')->get('mail_send_engine');
		$type = 'wp_mail';
		$res = false;
		switch($type) {
	...
	public function sendWpMail($to, $subject, $message, $fromName = '', $fromEmail = '', $replyToName = '', $replyToEmail = '', $additionalHeaders = array(), $additionalParameters = array()) {
		...
		if(!function_exists('wp_mail'))
			frameGdprsup::_()->loadPlugins();
		if(!frameGdprsup::_()->getModule('options')->get('disable_email_html_type')) {
		...
		switch($type) {
			case 'wp_mail': default:
				...
				if(empty($ts_mail_errors)) {
					$ts_mail_errors[] = __('Cannot send email - problem with send server', GDPRSUP_LANG_CODE);
				}
	...
	public function addOptions($opts) {
		$opts[ $this->getCode() ] = array(
			'label' => __('Mail', GDPRSUP_LANG_CODE),
			'opts' => array(
				'mail_function_work' => array('label' => __('Mail function tested and work', GDPRSUP_LANG_CODE), 'desc' => ''),
				'notify_email' => array('label' => __('Notify Email', GDPRSUP_LANG_CODE), 'desc' => __('Email address used for all email notifications from plugin', GDPRSUP_LANG_CODE), 'html' => 'text', 'def' => get_option('admin_email')),
			),
		);
		return $opts;
	}
