...
class fieldGdprsup {
	 ...
	 */
	public $adapt = array('htmlGdprsup' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterGdprsup::_($value, $this->adapt['dbFrom'], fieldAdapterGdprsup::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlGdprsup', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterGdprsup::_($this, $this->adapt['html'], fieldAdapterGdprsup::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameGdprsup::_()->getModule('optionsGdprsup')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', GDPRSUP_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsGdprsup'] = $options;
				}
			}
			$htmlContent = htmlGdprsup::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlGdprsup::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterGdprsup::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterGdprsup::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsGdprsup');
				$value = array();
				...
				if(empty($value))
					$value = __('N/A', GDPRSUP_LANG_CODE);
				else
				...
				if ($this->value == '') {
					...
					if(is_array($this->value)) {
						...
						if(!empty($options) && is_array($options)) {
							...
							$value = recImplodeGdprsup('<br />', $valArr);
						} else {
							$value = recImplodeGdprsup('<br />', $this->value);
						}
					} else
						$value = $this->value;
				}
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameGdprsup::_()->getModule('optionsGdprsup')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsGdprsup'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlGdprsup', $method)) {
			$output .= htmlGdprsup::$method($name, $params);
			$output .= htmlGdprsup::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', GDPRSUP_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', GDPRSUP_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', GDPRSUP_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterGdprsup::_($id,'getExtraFieldOptions',fieldAdapterGdprsup::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', GDPRSUP_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', GDPRSUP_LANG_CODE).':<br />';
					$output .= htmlGdprsup::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', GDPRSUP_LANG_CODE).':<br />';
					$output .= htmlGdprsup::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterGdprsup::_($this, $this->adapt['htmlParams'], fieldAdapterGdprsup::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', GDPRSUP_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', GDPRSUP_LANG_CODE).':<br />';
				$output .= fieldAdapterGdprsup::_($params,'getFieldAttributes',  fieldAdapterGdprsup::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsGdprsup::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperGdprsup');
		  $module = $this->checkVarFromParam($param,'moduleGdprsup');
		...
		// fill in the variables to configuration array
		  $config_params[$name] = array('type'=>$type,
										'label'=>$label,
										'helperGdprsup'=>$helper,
										'moduleGdprsup'=>$module,
										'values'=>$values,
										'default'=>$default,
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameGdprsup::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameGdprsup::_()->getModule($module)->isExternal())
		   $config_xml = frameGdprsup::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = GDPRSUP_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', GDPRSUP_LANG_CODE);
	   }
	   $output = '';
	   ...
	   elseif(is_object($this->value) || is_array($this->value))
			$params = objectToArrayGdprsup($this->value);
	   //if (!empty($params)) {
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameGdprsup::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsGdprsup'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlGdprsup::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
