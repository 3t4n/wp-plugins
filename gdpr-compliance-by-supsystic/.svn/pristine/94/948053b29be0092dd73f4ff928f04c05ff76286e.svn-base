...
class responseGdprsup {
	...
	public function getReqType() {
		return reqGdprsup::getVar('reqType');
	}
    ...
    public function ajaxExec($forceAjax = false) {
        $isAjax = $this->isAjax();
        $redirect = reqGdprsup::getVar('redirect');
        if(count($this->errors) > 0)
            $this->error = true;
        if($isAjax || $forceAjax)
            exit( json_encode_utf_normalGdprsup($this) );
        /*if($redirect)
            redirectGdprsup($redirect);*/
        return $this;
    }
	public function mainRedirect($redirectUrl = '') {
		$redirectUrl = empty($redirectUrl) ? GDPRSUP_SITE_URL : $redirectUrl;
		$redirectData = array();
		if(!empty($this->errors)) {
			$redirectData['gdprsupErrors'] = $this->errors;
		}
		if(!empty($this->messages)) {
			$redirectData['gdprsupMsgs'] = $this->messages;
		}
        return redirectGdprsup($redirectUrl. (strpos($redirectUrl, '?') ? '&' : '?'). http_build_query($redirectData));
	}
    ...
    /**
     * Alias for responseGdprsup::addError, @see addError method
     */
