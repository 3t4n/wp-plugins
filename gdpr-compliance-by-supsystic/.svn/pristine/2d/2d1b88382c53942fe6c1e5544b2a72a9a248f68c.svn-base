...
abstract class tableGdprsup {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUpGdprsup($table). strFirstUpGdprsup(GDPRSUP_CODE);
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjGdprsup('fieldGdprsup', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbGdprsup::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbGdprsup::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
		$this->_clearErrors();
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbGdprsup::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        if(empty($fields)) {
            $this->_addError(__('Nothing to update', GDPRSUP_LANG_CODE));
            return false;
        }
        ...
        if(dbGdprsup::query($query)) {
            if($method == 'INSERT')
                return dbGdprsup::lastID();
            else
                return true;
        } else
			$this->_addError(GDPRSUP_TEST_MODE ? dbGdprsup::getError() : __('Database error. Please contact your developer.', GDPRSUP_LANG_CODE));
        return false;
    }
            ...
            foreach($data as $k => $v) {
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if(isset($this->_fields[$k]) && $this->_fields[$k]->adapt['dbTo']) 
                        $val = fieldAdapterGdprsup::_($val, $this->_fields[$k]->adapt['dbTo'], fieldAdapterGdprsup::DB);
                    if($validate) {
                        if (isset($this->_fields[$k]) && is_object($this->_fields[$k])) {
                            $objForValidation = clone $this->_fields[$k];
                            $objForValidation->setValue($val);
                            if($errors = validatorGdprsup::_($objForValidation)) {
    ...
    /**
     * Add new fieldGdprsupGdprsup for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjGdprsup('fieldGdprsup', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
	/**
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbGdprsup::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
        ...
        foreach($this->_fields as $key => $f) {
            ...
            if($f->type == 'date') {
                if(empty($d[$key]) && !in_array($key, $ignore)) {
                    ...
                    $d[$key] = dbGdprsup::timeToDate($d[$key]);
                }
            }
        }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(GDPRSUP_DATE_FORMAT, dbGdprsup::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return dbGdprsup::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }
