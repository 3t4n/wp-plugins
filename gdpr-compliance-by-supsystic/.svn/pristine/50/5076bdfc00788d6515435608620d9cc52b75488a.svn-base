<?php
class optionsModelGdprsup extends modelGdprsup {
	...
	public function save($optKey, $val, $ignoreDbUpdate = false) {
		...
		if(!isset($this->_values[ $optKey ]) || $this->_values[ $optKey ]['value'] != $val) {
			...
			//frameGdprsup::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('option.'. $optKey);
		}
	}
		...
		foreach($options as $cKey => $cData) {
			foreach($cData['opts'] as $optKey => $optData) {
				...
				if(isset($optData['type'])) {
					switch($optData['type']) {
						case 'int':
							$this->_values[ $optKey ]['value'] = (int) $this->_values[ $optKey ]['value'];
							break;
						case 'bool':
							$this->_values[ $optKey ]['value'] = (bool) $this->_values[ $optKey ]['value'];
							break;
					}
				}
			}
    ...
    public function saveGroup($d = array()) {
		if(isset($d['opt_values']) && is_array($d['opt_values']) && !empty($d['opt_values'])) {
			dispatcherGdprsup::doAction('beforeSaveOpts', $d);
			...
			$this->pushError(__('Empty data to save option', GDPRSUP_LANG_CODE));
        return false;
    }
	private function _updateOptsInDb() {
		update_option(GDPRSUP_CODE. '_opts_data', $this->_values);
	}
	private function _loadOptValues() {
		if(!$this->_valuesLoaded) {
			$this->_values = get_option(GDPRSUP_CODE. '_opts_data');
