<?php
class optionsViewGdprsup extends viewGdprsup {
	private $_news = array();
	public function getNewFeatures() {
		$res = array();
		$readmePath = GDPRSUP_DIR. 'readme.txt';
		if(file_exists($readmePath)) {
			$readmeContent = @file_get_contents($readmePath);
			if(!empty($readmeContent)) {
				$matchedData = '';
				if(preg_match('/= '. GDPRSUP_VERSION. ' =(.+)=.+=/isU', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				} elseif(preg_match('/= '. GDPRSUP_VERSION. ' =(.+)/is', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				}
    ...
    public function getAdminPage() {
		...
		if(isset($tabs[ $activeTab ]) && isset($tabs[ $activeTab ]['callback'])) {
			//frameGdprsup::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('tab.'. $activeTab);
		...
		frameGdprsup::_()->addJSVar('adminOptionsGdprsup', 'ppsActiveTab', $activeTab);
		...
		$this->assign('breadcrumbs', frameGdprsup::_()->getModule('admin_nav')->getView()->getBreadcrumbs());
		//$this->assign('mainLink', frameGdprsup::_()->getModule('supsystic_promo')->getMainLink());
	...
	public function getTabContent() {
		frameGdprsup::_()->addScript('admin.mainoptions', $this->getModule()->getModPath(). 'js/admin.mainoptions.js');
	...
	public function getSettingsTabContent() {
		frameGdprsup::_()->addScript('admin.settings', $this->getModule()->getModPath(). 'js/admin.settings.js');
		frameGdprsup::_()->getModule('templates')->loadJqueryUi();
		
		$options = frameGdprsup::_()->getModule('options')->getAll();
		$this->assign('options', $options);
		$this->assign('exportAllSubscribersUrl', uriGdprsup::mod('subscribe', 'getWpCsvList'));
