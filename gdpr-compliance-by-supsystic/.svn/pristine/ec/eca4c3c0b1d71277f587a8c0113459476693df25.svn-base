 ...
 * @see field
 */
class fieldAdapterGdprsup {
    const DB = 'dbGdprsup';
    const HTML = 'htmlGdprsup';
    const STR = 'str';
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterGdprsup', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsGdprsup'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsGdprsup::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsGdprsup::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameGdprsup::_()->getModule('products')->getCategories();
        $brands = frameGdprsup::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', GDPRSUP_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', GDPRSUP_LANG_CODE). '</b><br />
                '. __('Destination', GDPRSUP_LANG_CODE). ':'. htmlGdprsup::checkboxlist('params[dest]', array('optionsGdprsup' => $listOfDest)). '<br />
                '. __('Country', GDPRSUP_LANG_CODE). ':'. htmlGdprsup::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', GDPRSUP_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', GDPRSUP_LANG_CODE) : htmlGdprsup::checkboxlist('params[categories][]', array('optionsGdprsup' => $cOptions))). '<br />
                    <b>'. __('Brands', GDPRSUP_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', GDPRSUP_LANG_CODE) : htmlGdprsup::checkboxlist('params[brands][]', array('optionsGdprsup' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', GDPRSUP_LANG_CODE).': '. htmlGdprsup::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', GDPRSUP_LANG_CODE).': '. htmlGdprsup::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', GDPRSUP_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameGdprsup::_()->getModule('optionsGdprsup')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlGdprsup::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', GDPRSUP_LANG_CODE).':<br />';
                $output .= htmlGdprsup::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', GDPRSUP_LANG_CODE).':<br />';
                $output .= htmlGdprsup::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsGdprsup']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsGdprsup'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', GDPRSUP_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsGdprsup'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameGdprsup::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameGdprsup::_()->getTable('states')
				->leftJoin( frameGdprsup::_()->getTable('countries'), 'country_id' )
	...
	static public function getFontsList() {
		return array("Abel", "Abril Fatface", "Aclonica", "Acme", "Actor", "Adamina", "Advent Pro",
			...
			"Butterfly Kids", "Cabin", "Cabin Condensed", "Cabin Sketch", "Caesar Dressing", "Cagliostro", "Calligragdprsuptti",
