<?php
abstract class moduleGdprsup extends baseObjectGdprsup {
	...
	public function __construct($d/*, $params = array()*/) {
		...
		if(isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsGdprsup::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsGdprsup::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if(empty($this->_externalDir)) {
			return GDPRSUP_MODULES_DIR. $this->getCode(). DS;
		} else {
	...
	public function getModPath() {
		if(empty($this->_externalPath)) {
			return GDPRSUP_MODULES_PATH. $this->getCode(). '/';
		} else {
	...
	protected function _createController() {
		...
		if(file_exists($this->getModDir(). 'controller.php')) {
			$className = '';
			if(importGdprsup($this->getModDir(). 'controller.php')) {
				$className = toeGetClassNameGdprsup($this->getCode(). 'Controller');
			}
			if(!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir().'helper.php')) {
			...
			importClassGdprsup($helper, $this->getModDir(). 'helper.php');
