...
jQuery.fn.serializeAnythingGdprsup = function(addData, returnArray) {
...
function gdprsup_str_replace(haystack, needle, replacement) { 
	var temp = haystack.split(needle); 
	return temp.join(replacement); 
}
/**
...
function toeListableGdprsup(params) {
	...
	this.draw = function(list, count) {
		this.table.find('tr').not('.gdprsupExample, .gdprsupTblHeader').remove();
		var exampleRow = this.table.find('.gdprsupExample');
		for(var i in list) {
			...
			newRow.removeClass('gdprsupExample').show();
...
function setCookieGdprsup(c_name, value, exdays) {
	var exdate = new Date();
 ...
 * @returns {jQuery}
 */
jQuery.fn.gdprsupZoom = function(level, origin) {
	origin = origin ? origin : 'center center';
	jQuery(this).data('zoom', level);
	return jQuery(this).css({
	/*	'zoom': level	// Didn't worked correctly for mobiles
...
/**
 * Retrive worgdprsupess attach ID from image, using img classes
 ...
 */
function gdprsupStrToMs(strDate) {
...
function randGdprsup( min, max, returnFloat ) {
