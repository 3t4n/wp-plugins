	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('gdprsupWpTabs')) {
					$this.addClass('gdprsupWpTabs');
				...
				if($this.find(selector).length) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.gdprsupTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId 
						? $this.find('.gdprsupTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.gdprsupTabContent');
					allTabsContent.hide();
