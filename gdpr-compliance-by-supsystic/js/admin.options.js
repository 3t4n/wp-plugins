var gdprsupAdminFormChanged = [];
window.onbeforeunload = function(){
	// If there are at lease one unsaved form - show message for confirnation for page leave
	if(gdprsupAdminFormChanged.length)
		return 'Some changes were not-saved. Are you sure you want to leave?';
};
jQuery(document).ready(function(){
	gdprsupInitMainPromoPopup();
	if(typeof(gdprsupActiveTab) != 'undefined' && gdprsupActiveTab != 'main_page' && jQuery('#toplevel_page_gdpr-compliance-by-supsystic').hasClass('wp-has-current-submenu')) {
		...
		subMenus.removeClass('current').each(function(){
			if(jQuery(this).find('a[href$="&tab='+ gdprsupActiveTab+ '"]').length) {
	...
	setTimeout(function() {
		...
		if(formsPreventLeave && formsPreventLeave.length) {
			...
			jQuery('#'+ formsPreventLeave.join(', #')).submit(function(){
				adminFormSavedGdprsup( jQuery(this).attr('id') );
			});
		}
	}, 1000);

	if(jQuery('.gdprsupInputsWithDescrForm').length) {
		jQuery('.gdprsupInputsWithDescrForm').find('input[type=checkbox][data-optkey]').change(function(){
			var optKey = jQuery(this).data('optkey')
			,	descShell = jQuery('#gdprsupFormOptDetails_'+ optKey);
			if(descShell.size()) {
	...
	gdprsupInitStickyItem();
	gdprsupInitCustomCheckRadio();
	//gdprsupInitCustomSelect();

	jQuery('.gdprsupFieldsetToggled').each(function(){
		var self = this;
		jQuery(self).find('.gdprsupFieldsetContent').hide();
		jQuery(self).find('.gdprsupFieldsetToggleBtn').click(function(){
			...
			jQuery(self).find('.gdprsupFieldsetContent').slideToggle( 300, function(){
				if(show) {
	...
	if(jQuery('#gdprsupPopupGoToTopBtn').length) {
		jQuery('#gdprsupPopupGoToTopBtn').click(function(){
			jQuery('html, body').animate({
				scrollTop: 0
			}, 1000);
			jQuery(this).parents('#gdprsupPopupGoToTop:first').hide();
			return false;
		});
	}
	...
	gdprsupInitTooltips();
	if(jQuery('.gdprsupCopyTextCode').length) {
		setTimeout(function(){	// Give it some time - wait until all other elements will be initialized
			...
			jQuery('.gdprsupCopyTextCode').attr('readonly', 'readonly').click(function(){
				this.setSelectionRange(0, this.value.length);
			}).focus(function(){
				this.setSelectionRange(0, this.value.length);
			});
			jQuery('input.gdprsupCopyTextCode').each(function(){
    ...
    gdprsupInitPlugNotices();
	jQuery(".supsystic-plugin .tooltipstered").removeAttr("title");
});
function gdprsupInitTooltips( selector ) {
	var tooltipsterSettings = {
		contentAsHTML: true
	,	interactive: true
	,	speed: 0
...
function changeAdminFormGdprsup(formId) {
	if(jQuery.inArray(formId, gdprsupAdminFormChanged) == -1)
		gdprsupAdminFormChanged.push(formId);
}
function adminFormSavedGdprsup(formId) {
	if(gdprsupAdminFormChanged.length) {
		for(var i in gdprsupAdminFormChanged) {
			if(gdprsupAdminFormChanged[i] == formId) {
				gdprsupAdminFormChanged.pop(i);
			}
		}
	}
}
function checkAdminFormSaved() {
	if(gdprsupAdminFormChanged.length) {
		if(!confirm(toeLangGdprsup('Some changes were not-saved. Are you sure you want to leave?'))) {
			return false;
		}
		gdprsupAdminFormChanged = [];	// Clear unsaved forms array - if user wanted to do this
	}
	return true;
}
...
function gdprsupInitStickyItem() {
	jQuery(window).scroll(function(){
		...
		,	footer = jQuery('.gdprsupAdminFooterShell')
		...
		if(wasSticking) {
			if(jQuery('#gdprsupPopupGoToTop').length)
				jQuery('#gdprsupPopupGoToTop').show();
		} else if(wasUnSticking) {
			if(jQuery('#gdprsupPopupGoToTop').length)
				jQuery('#gdprsupPopupGoToTop').hide();
		}
	});
}
function gdprsupInitCustomCheckRadio(selector) {
	return false;
	...
	,	radioClass: 'iradio_minimal'
		...
		// for checkboxHiddenVal type, see class htmlGdprsup
...
function gdprsupCheckDestroy(checkbox) {
	if(!jQuery.fn.iCheck) return;
	jQuery(checkbox).iCheck('destroy');
}
function gdprsupCheckDestroyArea(selector) {
	if(!jQuery.fn.iCheck) return;
	jQuery(selector).find('input[type=checkbox]').iCheck('destroy');
}
function gdprsupCheckUpdate(checkbox) {
	if(!jQuery.fn.iCheck) return;
	jQuery(checkbox).iCheck('update');
}
function gdprsupCheckUpdateArea(selector) {
	if(!jQuery.fn.iCheck) return;
	jQuery(selector).find('input[type=checkbox]').iCheck('update');
}
function gdprsupGetTxtEditorVal(id) {
...
function gdprsupSetTxtEditorVal(id, content) {
	if(typeof(tinyMCE) !== 'undefined'
		&& tinyMCE
		&& tinyMCE.get( id )
		&& !jQuery('#'+ id).is(':visible')
 ...
 */
function gdprsupGridSetListSearch(param, gridSelectorId) {
 ...
 * @param {string} gridSelectorId ID of grid table html element
 */
function gdprsupGridDoListSearch(param, gridSelectorId) {
	gdprsupGridSetListSearch(param, gridSelectorId);
 ...
 */
function gdprsupGetGridDataById(id, gridSelectorId) {
 ...
 */
function gdprsupGetGridColDataById(id, column, gridSelectorId) {
	var rowId = getGridRowId(id, gridSelectorId);
	if(rowId) {
...
function gdprsupInitPlugNotices() {
	...
	if($notices && $notices.size()) {
		$notices.each(function(){
			...
			var $enbStatsBtn = jQuery(this).find('.gdprsupEnbStatsAdBtn');
 ...
 */
function gdprsupGetMainPromoPopup() {
	if(jQuery('#gdprsupOptInProWnd').hasClass('ui-dialog-content')) {
		return jQuery('#gdprsupOptInProWnd');
	}
	return jQuery('#gdprsupOptInProWnd').dialog({
		modal:    true
	,	autoOpen: false
	,	width: 540
	,	height: 200
	,	open: function() {
			jQuery('#gdprsupOptWndTemplateTxt').hide();
			jQuery('#gdprsupOptWndOptionTxt').show();
		}
	});
}
function gdprsupInitMainPromoPopup() {
	if(!GDPRSUP_DATA.isPro) {
		var $proOptWnd = gdprsupGetMainPromoPopup();
		jQuery('.gdprsupProOpt').change(function(e){
			e.stopPropagation();
			var needShow = true
			...
			var parent = null;
			if(jQuery(this).parents('#gdprsupPopupMainOpts').length) {
				parent = jQuery(this).parents('label:first');
			} else if(jQuery(this).parents('.gdprsupPopupOptRow:first').length) {
				parent = jQuery(this).parents('.gdprsupPopupOptRow:first');
			} else {
				parent = jQuery(this).parents('tr:first');
			}
			if(!parent.size()) return;
			var promoLink = parent.find('.gdprsupProOptMiniLabel a').attr('href');
			if(promoLink && promoLink != '') {
				jQuery('#gdprsupOptInProWnd a').attr('href', promoLink);
			}
...
function gdprsupInitConnectedOpts(form) {
						...
						for(var connectVal in connected) {
							if(connected[ connectVal ] && connected[ connectVal ].length) {
								...
								for(var i = 0; i < connected[ connectVal ].length; i++) {
									show
									? jQuery(connected[ connectVal ][ i ]).show( g_gdprsupAnimationSpeed )
									: jQuery(connected[ connectVal ][ i ]).hide( g_gdprsupAnimationSpeed );
								}
							}
						}
					}
