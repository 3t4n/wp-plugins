<?php
class supsystic_promoControllerGdprsup extends controllerGdprsup {
    public function welcomePageSaveInfo() {
		$res = new responseGdprsup();
		installerGdprsup::setUsed();
		if($this->getModel()->welcomePageSaveInfo(reqGdprsup::get('get'))) {
			$res->addMessage(__('Information was saved. Thank you!', GDPRSUP_LANG_CODE));
		} else {
			$res->pushError($this->getModel()->getErrors());
		}
		$originalPage = reqGdprsup::getVar('original_page');
		$http = isset($_SERVER['HTTPS']) && !empty($_SERVER['HTTPS']) ? 'https://' : 'http://';
		if(strpos($originalPage, $http. $_SERVER['HTTP_HOST']) !== 0) {
			$originalPage = '';
		}
		redirectGdprsup($originalPage);
	}
	public function sendContact() {
		$res = new responseGdprsup();
		$time = time();
		$prevSendTime = (int) get_option(GDPRSUP_CODE. '_last__time_contact_send');
			...
			if($validate) {
				...
				foreach($validate as $v) {
					...
					switch($v) {
						case 'notEmpty':
							if(empty($data[ $fName ])) {
								$error = $fData['html'] == 'selectbox' ? __('Please select %s', GDPRSUP_LANG_CODE) : __('Please enter %s', GDPRSUP_LANG_CODE);
								$error = sprintf($error, $fData['label']);
							}
							break;
						case 'email':
							if(!is_email($data[ $fName ])) 
								$error = __('Please enter valid email address', GDPRSUP_LANG_CODE);
							break;
					}
					if(!empty($error)) {
		...
		if(!$res->error()) {
			...
			$msg .= 'Plugin: '. GDPRSUP_WP_PLUGIN_NAME. '<br />';
			foreach($fields as $fName => $fData) {
			...
			if(frameGdprsup::_()->getModule('mail')->send('support@supsystic.zendesk.com', $data['subject'], $msg, $data['name'], $data['email'])) {
				update_option(GDPRSUP_CODE. '_last__time_contact_send', $time);
			} else {
				$res->pushError( frameGdprsup::_()->getModule('mail')->getMailErrors() );
			}
	...
	public function addNoticeAction() {
		$res = new responseGdprsup();
		$code = reqGdprsup::getVar('code', 'post');
		$choice = reqGdprsup::getVar('choice', 'post');
		if(!empty($code) && !empty($choice)) {
			$optModel = frameGdprsup::_()->getModule('options')->getModel();
			switch($choice) {
				case 'hide':
	...
	public function getPermissions() {
		return array(
			GDPRSUP_USERLEVELS => array(
				GDPRSUP_ADMIN => array('welcomePageSaveInfo', 'sendContact', 'addNoticeAction'),
			),
		);
	}
}