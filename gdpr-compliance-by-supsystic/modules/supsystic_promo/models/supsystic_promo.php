<?php
class supsystic_promoModelGdprsup extends modelGdprsup {
	private $_apiUrl = '';
	...
	public function saveUsageStat($code, $unique = false) {
		...
		return dbGdprsup::query($query);
	}
	private function _checkUniqueStat($code) {
		$uniqueStats = get_option(GDPRSUP_CODE. '_unique_stats');
		...
		$uniqueStats[] = $code;
		update_option(GDPRSUP_CODE. '_unique_stats', $uniqueStats);
		return false;
	}
	...
	public function getUserStatsCount() {
		...
		return (int) dbGdprsup::get($query, 'one');
	}
