 ...
 */
function getRandNameGdprsup($lenFrom = 6, $lenTo = 9) {
	$res = '';
...
function setDefaultParamsGdprsup($params, $default) {
	foreach($default as $k => $v) {
		$params[$k] = isset($params[$k]) ? $params[$k] : $default[$k];
	}
	return $params;
}
function importClassGdprsup($class, $path = '') {
	if(!class_exists($class)) {
		if(!$path) {
			$classFile = $class;
			if(strpos(strtolower($classFile), GDPRSUP_CODE) !== false) {
				$classFile = preg_replace('/'. GDPRSUP_CODE. '/i', '', $classFile);
			}
			$path = GDPRSUP_CLASSES_DIR. $classFile. '.php';
		}
		return importGdprsup($path);
		...
		/*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
			...
			if(strpos($classFile, GDPRSUP_DIR) === false) {   //Class is not in our plugin directory
				...
				if(!empty($plugins)) {
					for($i = 0; $i < count($plugins); $i++) {
						if(strpos($plugins[$i], GDPRSUP_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
							unset($plugins[$i]);
						}
					}
 ...
 */
function toeGetClassNameGdprsup($class) {
	$className = '';
	if(class_exists($class. strFirstUpGdprsup(GDPRSUP_CODE))) 
		$className = $class. strFirstUpGdprsup(GDPRSUP_CODE);
	else if(class_exists(GDPRSUP_CLASS_PREFIX. $class)) 
		$className = GDPRSUP_CLASS_PREFIX. $class;
	else
		$className = $class;
	return $className;
}
 ...
 */
function toeCreateObjGdprsup($class, $params) {
	$className = toeGetClassNameGdprsup($class);
	$obj = NULL;
...
function in_array_arrayGdprsup($needle, $haystack) {
	if(is_array($needle)) {
		foreach($needle as $n) {
		...
		return in_array_arrayGdprsup($needle, $haystack);
}
function json_encode_utf_normalGdprsup($value) {
	if (is_int($value)) {
		return (string)$value;   
	...
	if ($with_keys) {
		foreach ($value as $key => $v) {
			$result[] = json_encode_utf_normalGdprsup((string)$key) . ':' . json_encode_utf_normalGdprsup($v);    
		}
		return '{' . implode(',', $result) . '}';                
	} else {
		foreach ($value as $key => $v) {
			$result[] = json_encode_utf_normalGdprsup($v);    
		}
		return '[' . implode(',', $result) . ']';
	}
}
 ...
 * @return array
 */
function prepareParamsGdprsup(&$d=array(), &$options = array()) {
	if (!empty($d['params'])) {
		...
		if (is_array($d['params'])) {
			$params = utilsGdprsup::jsonEncode($d['params']);
	...
	if(isset($d['code'])) {
		if ($d['code'] == '') {
			$d['code'] = prepareFieldCodeGdprsup($d['label']).'_'.rand(0, 9999999);
		}
	}
	return $d;
}
function prepareFieldCodeGdprsup($string) {   
 ...
 */
function recImplodeGdprsup($glue, $array) {
	$res = '';
	$i = 0;
	$count = count($array);
	foreach($array as $el) {
...
function objectToArrayGdprsup($data) {
	...
	foreach ($data as $key => $value) {
		...
		if (is_array($value)) 
			$result[$key] = objectToArrayGdprsup($value);
		else
			$result[$key] = $value;
	}
	return $result;
 ...
 */
function multArrayMapGdprsup($callback, $array) {
	if(is_array($array)) {
		foreach($array as $k => $v) {
			if(is_array($v)) {
				$array[ $k ] = multArrayMapGdprsup($callback, $v);
			} else {
