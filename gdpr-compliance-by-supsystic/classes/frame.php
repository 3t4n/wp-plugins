...
class frameGdprsup {
    ...
    public function __construct() {
        $this->_res = toeCreateObjGdprsup('response', array());

    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new frameGdprsup();
        }
        return $instance;
    }
    ...
    public function parseRoute() {
        // Check plugin
        $pl = reqGdprsup::getVar('pl');
        if($pl == GDPRSUP_CODE) {
            $mod = reqGdprsup::getMode();
            if($mod)
                $this->_mod = $mod;
            $action = reqGdprsup::getVar('action');
            if($action)
                $this->_action = $action;
        }
    }
	...
	private function _featchModulesList() {
		$modules = array();
		$modDirs = glob(GDPRSUP_MODULES_DIR. '*');
		$id = 1;
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsGdprsup::getExtModDir( $m['ex_plug_dir'] );
                }
                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassGdprsup($code. strFirstUpGdprsup(GDPRSUP_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameGdprsup($code);
    ...
    public function init() {
		...
		dispatcherGdprsup::doAction('afterModulesInit');

		modInstallerGdprsup::checkActivationMessages();
        ...
        register_activation_hook(  GDPRSUP_DIR. DS. GDPRSUP_MAIN_FILE, array('utilsGdprsup', 'activatePlugin')  ); //See classes/install.php file
        register_uninstall_hook(GDPRSUP_DIR. DS. GDPRSUP_MAIN_FILE, array('utilsGdprsup', 'deletePlugin'));
		register_deactivation_hook(GDPRSUP_DIR. DS. GDPRSUP_MAIN_FILE, array( 'utilsGdprsup', 'deactivatePlugin' ) );

		add_action('init', array($this, 'connectLang'));
        //$operationTime = microtime(true) - $startTime;
    }
	public function connectLang() {
		load_plugin_textdomain(GDPRSUP_LANG_CODE, false, GDPRSUP_PLUG_NAME. '/lang');
	}
    /**
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', GDPRSUP_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[GDPRSUP_METHODS])
                    && !empty($permissions[GDPRSUP_METHODS])
                ) {
                    foreach($permissions[GDPRSUP_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[GDPRSUP_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[GDPRSUP_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameGdprsup::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ GDPRSUP_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ GDPRSUP_METHODS ][ $action ]))
                            || (!is_array($permissions[ GDPRSUP_METHODS ][ $action ]) && $permissions[GDPRSUP_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[GDPRSUP_USERLEVELS])
                    && !empty($permissions[GDPRSUP_USERLEVELS])
                ) {
                    $currentUserPosition = frameGdprsup::_()->getModule('user')->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if(is_multisite() && is_admin() && is_super_admin()) {
						$currentUserPosition = GDPRSUP_ADMIN;
					}
                    foreach($permissions[GDPRSUP_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
	...
	protected function _doExec() {
		...
		if($mod && $this->checkPermissions($this->_mod, $this->_action)) {
			switch(reqGdprsup::getVar('reqType')) {
				case 'ajax':
    ...
    protected function _extractTables($tablesDir = GDPRSUP_TABLES_DIR) {
        if(is_dir($tablesDir)) {
    ...
    protected function _extractTable($tableName, $tablesDir = GDPRSUP_TABLES_DIR) {
        importClassGdprsup('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableGdprsup::_($tableName);
    }
    /**
     ...
     * @return object table
     * @example frameGdprsup::_()->getTable('products')->getAll()
     */
    ...
    public function addScript($handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array()) {
		$src = empty($src) ? $src : uriGdprsup::_($src);
		if(!$ver)
			$ver = GDPRSUP_VERSION;
		if($this->_setMinify) {
    ...
    /**
     * Add all scripts from _scripts array to worgdprsupess
     */
    ...
    public function isTplEditor() {
        $tplEditor = reqGdprsup::getVar('tplEditor');
	...
	public function isAdminPlugOptsPage() {
		$page = reqGdprsup::getVar('page');
		if(is_admin() && strpos((string)$page, frameGdprsup::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
	...
	public function savePluginActivationErrors() {
		update_option(GDPRSUP_CODE. '_plugin_activation_errors',  ob_get_contents());
	}
	public function getActivationErrors() {
		return get_option(GDPRSUP_CODE. '_plugin_activation_errors');
	}
