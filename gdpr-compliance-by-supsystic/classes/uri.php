...
class uriGdprsup {
    ...
    static public function fileToPageParam($file) {
        ...
        return substr($file, strpos($file, GDPRSUP_PLUG_NAME));
    }
    static public function _($params) {
		...
		if(is_string($params) 
			...
			|| strpos($params, GDPRSUP_PLUG_NAME) !== false)	// If relative links in WP is used (by other plugin for example)
		) {
			if(self::isHttps())
