...
class recapchaGdprsup {
	...
	public function __construct() {
		if(!function_exists('recaptcha_get_html')) {	// In case if this lib was already included by another plugin
			importGdprsup(GDPRSUP_HELPERS_DIR. 'recaptchalib.php');
		}
	}
	static public function getInstance() {
		static $instance = NULL;
		if(empty($instance)) {
			$instance = new recapchaGdprsup();
		}
		return $instance;
	}
	...
	public function getHtml() {
		if(reqGdprsup::getVar('reqType') == 'ajax') {
	...
	public function check() {
		$resp = recaptcha_check_answer($this->_privateKey, 
					$_SERVER['REMOTE_ADDR'], 
					reqGdprsup::getVar('recaptcha_challenge_field'),
					reqGdprsup::getVar('recaptcha_response_field'));
