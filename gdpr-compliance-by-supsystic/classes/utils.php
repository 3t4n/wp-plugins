...
class utilsGdprsup {
    static public function jsonEncode($arr) {
        return (is_array($arr) || is_object($arr)) ? json_encode_utf_normalGdprsup($arr) : json_encode_utf_normalGdprsup(array());
    }
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsGdprsup::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsGdprsup::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameGdprsup::_()->getModule($m)) 
                return false;
            if(!is_dir(frameGdprsup::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameGdprsup::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqGdprsup::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse worgdprsupess post/page/custom post type content for images and return it's IDs if there are images
	...
	static public function getReferalUrl() {
		// Simple for now
		return reqGdprsup::getVar('HTTP_REFERER', 'server');
	}
