...
class errorsGdprsup {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqGdprsup::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $gdprsupErrors = reqGdprsup::getVar('gdprsupErrors');
        if(!empty($gdprsupErrors)) {
            if(!is_array($gdprsupErrors)) {
                $gdprsupErrors = array( $gdprsupErrors );
            }
            $gdprsupErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $gdprsupErrors)));
            if(!empty($gdprsupErrors)) {
                self::$current = $gdprsupErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsGdprsup', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsGdprsup', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqGdprsup::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqGdprsup::clearVar('sesErrors', 'session');
    }
