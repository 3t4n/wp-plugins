<?php
abstract class controllerGdprsup {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameGdprsup::_()->getModule( $this->getCode() );
		$className = '';
		if(importGdprsup($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameGdprsup($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importGdprsup($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameGdprsup($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(GDPRSUP_ADMIN),
				'remove' => array(GDPRSUP_ADMIN),
				'restore' => GDPRSUP_ADMIN,
			),
			S_USERLEVELS => array(
				S_ADMIN => array('save', 'remove', 'restore')
			),
		);
	 * Can be used on of sub-array - GDPRSUP_METHODS or GDPRSUP_USERLEVELS
	 */
	...
	public function getModule() {
		return frameGdprsup::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) reqGdprsup::getVar('page');
		$rowsLimit = (int) reqGdprsup::getVar('rows');
		$orderBy = reqGdprsup::getVar('sidx');
		$sortOrder = reqGdprsup::getVar('sord');

		// Our custom search
		$search = reqGdprsup::getVar('search');
		...
		// jqGrid search
		$isSearch = reqGdprsup::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqGdprsup::getVar('searchField'));
			$searchString = trim(reqGdprsup::getVar('searchString'));
		...
		$res = dispatcherGdprsup::applyFilters($this->getCode(). '_getListForTblResults', $res);
		$res->ajaxExec();
	}
	public function removeGroup() {
		$res = new responseGdprsup();
		if($this->getModel()->removeGroup(reqGdprsup::getVar('listIds', 'post'))) {
