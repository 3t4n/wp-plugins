...
class langGdprsup {
	...
	static public function extract($d = array('dir' => '', 'langGdprsup' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langGdprsup']) && !empty($d['langGdprsup']))
			$langDirPath = GDPRSUP_LANG_DIR. $d['langGdprsup']. DS;
		else
			$langDirPath = GDPRSUP_LANG_DIR. GDPRSUP_WPLANG. DS;
