<?php
class validatorGdprsup {
    static public $errors = array();
    static public function validate($field, $label = '', $validate = array()) {
        self::$errors = array();
        if(is_object($field) && get_class($field) != 'fieldGdprsup') {
            $value = $field;
            $field = new fieldGdprsupGdprsup('noMatter');
            $field->label = $label;
        ...
        if(!empty($field->validate)) {
            foreach($field->validate as $v) {
                 if(method_exists('validatorGdprsup', $v)) 
                    self::$v($field);
            }
        }
        if(method_exists('validatorGdprsup', $field->type)) {
            $validate = $field->type;
            self::$validate($field);
        }
        if($field->maxlen) {
    ...
    static public function getProductValidationMethods() {
        $res = array();
        $all = get_class_methods('validatorGdprsup');
        foreach($all as $m) {
