<?php
abstract class modelGdprsup extends baseObjectGdprsup {
	...
	public function getModule() {
		return frameGdprsup::_()->getModule( $this->_code );
	}
	...
	public function getFromTbl($params = array()) {
		...
		$table = frameGdprsup::_()->getTable( $tbl );
	...
	protected function _buildQuery($table = null) {
		if(!$table)
			$table = frameGdprsup::_()->getTable( $this->_tbl );
	...
	public function removeGroup($ids) {
		...
		if(!empty($ids)) {
			if(frameGdprsup::_()->getTable( $this->_tbl )->delete(array('additionalCondition' => 'id IN ('. implode(',', $ids). ')'))) {
				return true;
			} else 
				$this->pushError (__('Database error detected', GDPRSUP_LANG_CODE));
		} else
			$this->pushError(__('Invalid ID', GDPRSUP_LANG_CODE));
		return false;
	}
	...
	public function delete($params = array()) {
		if(frameGdprsup::_()->getTable( $this->_tbl )->delete( $params )) {
			return true;
		} else 
			$this->pushError (__('Database error detected', GDPRSUP_LANG_CODE));
		return false;
	...
	public function insert($data) {
		...
		$id = frameGdprsup::_()->getTable( $this->_tbl )->insert( $data );
		if($id) {
			return $id;
		}
		$this->pushError(frameGdprsup::_()->getTable( $this->_tbl )->getErrors());
		return false;
	}
	public function updateById($data, $id = 0) {
		...
		if($id) {
			...
			$this->pushError(__('Empty or invalid ID', GDPRSUP_LANG_CODE));
		return false;
	}
	public function update($data, $where) {
		$data = $this->_dataSave($data, true);
		if(frameGdprsup::_()->getTable( $this->_tbl )->update( $data, $where )) {
			return true;
		}
		$this->pushError(frameGdprsup::_()->getTable( $this->_tbl )->getErrors());
		return false;
