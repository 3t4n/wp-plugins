...
class andreani_Shipping extends WC_Shipping_Method {
	...
	public function calculate_shipping( $package = array() ) {
		...
		$unidad_dimension = esc_attr( get_option('woocommerce_dimension_unit' ));
 		...
 		$peso_multi = 0;
		$dimension_multi = 0;
		$dimension_multiplicadores = [
			'm' => 1,
			'cm' => 100,
			'mm' => 1000,
		];
		
		$dimension_multiplicador = isset($dimension_multiplicadores[$unidad_dimension]) ? $dimension_multiplicadores[$unidad_dimension] : 0;
		
		$peso_multiplicadores = [
			'kg' => 1,
			'g' => 0.001,
		];
		
		$peso_multi = isset($peso_multiplicadores[$unidad_peso]) ? $peso_multiplicadores[$unidad_peso] : 0;
		...
		foreach ($paquetes_andreani as $pq) {
			...
			$monto_asegurado = $pq['InsuredValue']['Amount'];
				...
				$headers = array(
        		 ...
        		 $respuesta_api_andreani = wp_remote_post($wp_session['url_andreani_cotizador'], array(
  				...
  				if ( !is_wp_error( $respuesta_api_andreani ) ) {
					$respuesta_api_andreani = json_decode($respuesta_api_andreani['body']);		
	 				...
	 				if (!empty($respuesta_api_andreani->notice)) {
							if ( !isset($_COOKIE['andreani_notice'])) {
								@setcookie( 'andreani_notice', $respuesta_api_andreani->notice, time()+3600, "/");  /* expire in 1 hour */
							};
	  				} else {
					...
					if (!is_null($respuesta_api_andreani) && 
						!is_null($respuesta_api_andreani->tarifaConIva->total)) {
