...
class WC_Acceptblue_API{
	...
	public function request($request_path, $method, $request_data, $validation = true, $id = ''){
		...
		if($validation) {
			...
			if ( $this->is_response_error($status_code) ) {
                $message = trim($parsed_response['error_message'] ?? 'Unknown error from api');
                $details = $parsed_response['error_details'] ?? '';
                $details = is_array($details) ? $details['source'] : '';
                $details = is_array($details)? $details[0] : $details;
                if(!str_ends_with($message, '.')) $message .= '.';
                $new_message = $message . ' ' . $details;
				if(!empty($parsed_response['card_ref'])) {
					return new WP_Error( "acceptblue_error", "{$parsed_response['status']}: {$new_message}", 'payment-gateway-accept-blue-for-woocommerce' );
				}else {
					return new WP_Error( "acceptblue_error", "{$new_message}", 'payment-gateway-accept-blue-for-woocommerce' );
				}
			} else {
    ...
    private function try_to_parse_error_message($parsed_response){
        $re = '/(resulted in a (\'|"|`)(?<code>\d{3})?([^\']+|[^"]+|[^`]+)(\'|"|`))?(\s+)?response\:(\s+)?(?<message>\{.+\})/';
        $message = $parsed_response['error_message'];
        if(preg_match($re, $message, $matches)){
            try {
                $message_json_string = $matches['message'];
                $message_json = json_decode($message_json_string, true);
                $parsed_response['error_message'] = $message_json['error_message'];
                if (!str_ends_with($message_json['error_message'], '.')) {
                    $message_json['error_message'] .= '. ';
                }
                $parsed_response['error_message'] .= $message_json['error_details'];
            }catch(\Exception $e){
                return $parsed_response;
            }
        }else{
