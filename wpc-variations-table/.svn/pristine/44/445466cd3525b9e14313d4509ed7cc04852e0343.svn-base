...
! defined( 'WPCVT_VERSION' ) && define( 'WPCVT_VERSION', '3.7.5' );
		...
		if ( ! class_exists( 'WPCleverWpcvt' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWpcvt {
				...
				function __construct() {
					...
					// ajax add-to-cart
					add_action( 'wc_ajax_wpcvt_add_to_cart', [ $this, 'ajax_add_to_cart' ] );
				...
				function init() {
					// load text-domain
					load_plugin_textdomain( 'wpc-variations-table', false, basename( WPCVT_DIR ) . '/languages/' );
				...
				function shortcode( $attrs ) {
					...
					ob_start();
					self::variations_table( $attrs['id'] );

					return ob_get_clean();
				}

				function variations_table( $product_id = null ) {
					if ( $product_id ) {
						...
						if ( ! empty( $children ) ) {
							...
							if ( ! empty( $children_data ) ) {
								...
								if ( $layout === 'datatables' ) {
									...
									if ( $show_image === 'yes' ) {
										echo apply_filters( 'wpcvt_datatable_heading_image', '<th data-searchable="false">' . esc_html( self::localization( 'image', esc_html__( 'Image', 'wpc-variations-table' ) ) ) . '</th>' );
									}

									echo apply_filters( 'wpcvt_datatable_heading_name', '<th>' . esc_html( self::localization( 'name', esc_html__( 'Name', 'wpc-variations-table' ) ) ) . '</th>' );
									echo apply_filters( 'wpcvt_datatable_heading_action', '<th data-orderable="false" data-searchable="false">' . esc_html( self::localization( 'action', esc_html__( 'Action', 'wpc-variations-table' ) ) ) . '</th>' );
								...
								do_action( 'wpcvt_actions_above', $children_data, $global_product );
								...
								do_action( 'wpcvt_actions_under', $children_data, $global_product );
