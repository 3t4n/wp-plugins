 ...
 *
 * @package    Frase
 * @subpackage Frase/Frase_Api_Settings
 */
 ...
 * @see Frase_Api
 */
if ( ! class_exists( 'Frase_Api_Settings' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Frase_Api_Settings extends Frase_Api {
		 ...
		 */
		public function run() {
			$this->type      = 'frase_plugin_api_settings';
		...
		public function get_item( $request ) {
			...
			$saved_options = frase_plugin_get_options();
		...
		public function update_item( $request ) {
			...
			if ( is_wp_error( rest_validate_value_from_schema( $params, $schema ) ) ) {
				return new WP_Error(
					...
					sprintf( __( 'The %s property has an invalid stored value, and cannot be updated to null.', 'frase' ), FRASE_PLUGIN_OPTION_NAME ),
					array( 'status' => 500 )
				);
			}

			$sanitized_options = $this->prepare_value( $params, $schema );
			frase_plugin_update_options( $sanitized_options );
		...
		protected function get_registered_schema() {
			...
			$schema = frase_plugin_admin()->get_settings_schema();
...
/**
 * Return instance of  Frase_Api_Settings class
 *
 * @since 1.0.0
 *
 * @return Frase_Api_Settings
 */
function frase_plugin_api_settings() { //phpcs:ignore
	return Frase_Api_Settings::get_instance();
}
frase_plugin_api_settings()->run();
