...
namespace WcGetnet\WooCommerce\GateWays\Pix;
...
class WcGetnet_Pix extends Abstract_Payment {
	...
	public function handleHeader() {
		if ( 'no' === $this->enabled ) {
			\WcGetnet::render('partials/admin/modal', [
				'title'        => 'Aviso: Procedimentos para Ativação de Emissão de PIX',
				'content'      => ' <p>Cadastre uma chave Pix no Santander e negocie as taxas diretamente com o banco no convênio estabelecido.</p><p>Caso tenha dúvidas entre em contato com o e-mail: <strong>suporte.edigital@getnet.com.br</strong></p>',
				'button_label' => 'Entendi, fechar',
				'button_name'  => 'pix_information_displayed',
				'id'           => $this->id,
			]);
		}

		\WcGetnet::render('partials/settings/pix-page', [
			...
			'settingsFields'     => $this->generate_settings_html(Pix::getHeaderFields(), false)
		]);
	}
	...
	public function process_payment( $order_id ) {
		...
		$order->update_meta_data( '_getnet_payment_type', 'PIX' );
