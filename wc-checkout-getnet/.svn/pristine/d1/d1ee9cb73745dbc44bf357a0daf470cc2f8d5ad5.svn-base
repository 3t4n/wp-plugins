...
class WcGetnet_Settings {
	...
	public function handleContent() {
		?>
			<div class="gnt-form">
				<form method="post" action="options.php" class="<?php //echo ( get_option( '_policy_privacy_accept' ) == 1 ) ? '' : 'hide'; ?>">
				...
				<?php 
					//This line is disabled because this feature is yet to be implemented.
                    //The modal is validated and ready for use.
	...
	public function HandlePrivacyPolicy() {
		\WcGetnet::render('partials/admin/modal', [
			'content' => 'Aceito fornecer informações não sensíveis do meu cadastro para melhorar minha experiência.',
			'button_label' => 'Sim, eu aceito!',
			'button_name' => 'privacy_policy_information_displayed',
			'link_text' => 'Ler politicas de privacidade',
			'link' => '/politicas',
			'id' => 'privacy_policy',
		]);
	}
