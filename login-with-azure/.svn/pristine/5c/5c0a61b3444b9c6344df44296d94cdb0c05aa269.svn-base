...
require 'mail' . DIRECTORY_SEPARATOR . 'class-moazure-mail-config.php';
require 'mail' . DIRECTORY_SEPARATOR . 'class-moazure-manual-email-config.php';
require 'mail' . DIRECTORY_SEPARATOR . 'class-moazure-mail-handler.php';
...
class MOAzure_MS_Apps {
	...
	public function ms_apps_redirect( $tab ) {
		...
		switch ( $tab ) {
			...
			case 'mail_app':
				$ms_app_handler = MOAzure_Mail_Config::get_mail_config_obj();
				if ( ! empty( $ms_apps['mail_manual'] ) ) {
					$ms_app_handler->moazure_mail_app_page();
				} else {
					$ms_app_handler->moazure_mail_initial_page();
				}
				break;

			case 'mail_send':
				$ms_app_handler = MOAzure_Manual_Email_Config::get_manual_email_config_obj();
				$ms_app_handler->moazure_send_manual_email_config();
				break;
