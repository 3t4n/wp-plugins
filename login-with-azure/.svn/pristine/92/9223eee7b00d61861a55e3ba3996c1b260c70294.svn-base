...
class MOAzure_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct( 'moazure_widget', MO_AZURE_ADMIN_MENU, array( 'description' => __( 'All-in-One Microsoft', 'login-with-azure' ) ) );
	}
	...
	public function moazure_login_form() {
		...
		if ( ! is_user_logged_in() ) {
			...
			$link_with_username = __( 'Howdy, ', 'login-with-azure' ) . $current_user->display_name;
	...
	public function moazure_error_message() {
		...
		if ( ! empty( $_SESSION['msg_class'] ) && ! empty( $_SESSION['msg'] ) ) {
			echo '<div class="' .
			esc_attr( $_SESSION['msg_class'] ) //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitization not required as escaping is already done.
			. '">' .
			esc_attr( $_SESSION['msg'] ) //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitization not required as escaping is already done.
			. '</div>';
			unset( $_SESSION['msg'] );
...
function moazure_login_validate() {
	...
	if ( ( ! empty( $_REQUEST['option'] ) ) && ( ( 'moazure_oauth_redirect' === $_REQUEST['option'] ) || ( 'moazure_sps_test_config' === $_REQUEST['option'] ) ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Ignoring nonce verification because we are fetching data from URL and not on form submission.
		moazure_test_config_redirect( sanitize_text_field( wp_unslash( $_REQUEST['option'] ) ) ); //phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Ignoring nonce verification because we are fetching data from URL and not on form submission.
	}
			...
			try {
				...
				if ( ( ! empty( $_SESSION['appname'] ) && 'moazure_sps' === $_SESSION['appname'] ) || ! empty( $_COOKIE['moazure_sps'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Ignoring nonce verification because we are fetching data from URL and not on form submission.
					...
					if ( isset( $_REQUEST['id_token'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Ignoring nonce verification because we are fetching data from URL and not on form submission.
						...
						} elseif ( ! empty( $token_response['access_token'] ) ) {
					...
					if ( ! $id_token && ( ( $_COOKIE['moazure_test'] ) || ! empty( $_SESSION['moazure_test'] ) ) ) {
						echo '<div class="moazure-test-container">
							<div class="moazure-test-head moazure-head-error">
								Connection Failed
							</div>
							<svg xmlns="http://www.w3.org/2000/svg" clip-rule="evenodd" fill-rule="evenodd" height="30" image-rendering="optimizeQuality" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" viewBox="0 0 254000 254000" width="30">
								<g id="图层_x0020_1">
							...
							<br><br>
							<span class="" style="text-align: center; ">Invalid token received from the Azure.</span>
							<div class="moazure-test-close-cont moazure-flex">
								<button type="button" class="button button-large moazure_configure_btn moazure-rad" onclick="moazure_test_sps_close()">Close</button>
							</div>
						</div>';

						?>
						<style>
							.moazure-test-container {
								display: flex;
								align-items: center;
								flex-direction: column;
								border: 1px solid #eee;
						...
						wp_die( 'We could not sign you in. Please contact your Administrator', 'Invalid token received' );
					} else {
							...
							<div class="sso-test-conn-cont">
								<table class="sso-test-table">
									<tr class="sso-test-table-tr">
										<th class="sso-test-table-th">' . esc_attr__( 'Attribute Name', 'login-with-azure' ) . '</th>
										<th class="sso-test-table-th">' . esc_attr__( 'Attribute Value', 'login-with-azure' ) . '</th>
									</tr>';
