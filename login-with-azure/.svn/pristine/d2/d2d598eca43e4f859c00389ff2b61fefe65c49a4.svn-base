...
class MOAzure_PowerBI_Shortcode {
	...
	private function set_shortcode_config( $attrs ) {
		...
		$this->shortcode_config = array(
			'rid'    => sanitize_text_field( $attrs['report_id'] ),
			'wid'    => sanitize_text_field( $attrs['workspace_id'] ),
	...
	public static function moazure_pbi_shortcode_render( $attrs = '', $content = '' ) {
		...
		$res_id        = sanitize_text_field( $attrs['workspace_id'] . '=' . $attrs['workspace_id'] );
	...
	public function moazure_pbi_not_sso_user_content() {
		...
		$loginpage = esc_url_raw( home_url() . '/wp-admin' );
	...
	public function moazure_pbi_report_content() {
		...
		if ( isset( $report_res['error'] ) || ! $report_res ) {
			...
			$embedurl     = isset( $report_res['embedUrl'] ) ? esc_url_raw( $report_res['embedUrl'] ) : '';
			$access_token = $azure_api->get_access_token();
			$content      = '<div id="powerbi-embed' . esc_attr( $config['rid'] ) . '" style="width:' . esc_attr( $config['width'] ) . ';height:' . esc_attr( $config['height'] ) . '; max-width: -webkit-fill-available; max-height: -webkit-fill-available;">Loading Content...</div>
			...
			';//phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript PluginCheck.CodeAnalysis.Offloading.OffloadedContent -- These are the scripts provided directly from Power BI.
			return $content;
		}
	}
}
