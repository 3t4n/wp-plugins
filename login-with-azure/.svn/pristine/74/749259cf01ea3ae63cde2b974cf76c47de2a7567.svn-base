...
class MOAzure {
	...
	public function __construct() {
		...
		add_action( 'user_register', array( MOAzure_Mail_Handler::class, 'moazure_send_email_to_new_user' ), 10, 1 );
	}
	 ...
	 *
	 * @param string $post_option post option value.
	 * @param string $nonce_form_name nonce form name var.
	 * @param string $nonce_form_field nonce form field var.
	 * @return boolean
	 */
	public function moazure_verify_form( $post_option, $nonce_form_name, $nonce_form_field ) {
		return ( isset( $_POST['option'] ) && sanitize_text_field( wp_unslash( $_POST['option'] ) ) === $post_option && isset( $_REQUEST[ $nonce_form_field ] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST[ $nonce_form_field ] ) ), $nonce_form_name ) );
	}
	...
	public function moazure_azure_save_settings() {
		...
		if ( $this->moazure_verify_form( 'change_miniorange', 'moazure_goto_login_form', 'moazure_goto_login_form_field' ) ) {
		...
		if ( $this->moazure_verify_form( 'moazure_register_customer', 'moazure_register_form', 'moazure_register_form_field' ) ) {
			if ( current_user_can( 'administrator' ) ) {
				$email            = '';
				$phone            = '';
				$password         = '';
				...
				$lname            = '';
				$company          = '';
				//phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated WordPress.Security.ValidatedSanitizedInput.InputNotSanitized WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- As we are not storing these values in the database, so we can ignore sanitization and unslashing.
				if ( $this->moazure_check_empty_or_null( $_POST['email'] ) || $this->moazure_check_empty_or_null( $_POST['password'] ) || $this->moazure_check_empty_or_null( $_POST['confirmPassword'] ) ) {
					...
					return;
				} elseif ( strlen( $_POST['password'] ) < 8 || strlen( $_POST['confirmPassword'] ) < 8 ) { //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- As we are not storing password in the database, so we can ignore sanitization snd unslashing.
					$notice_arr['msg_type'] = 'error';
					...
					$password         = stripslashes( ( $_POST['password'] ) ); //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- As we are not storing password in the database, so we can ignore sanitization and unslashing. Preventing use of sanitization in password will lead to removal of special characters. 
					$confirm_password = stripslashes( ( $_POST['confirmPassword'] ) ); //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- As we are not storing password in the database, so we can ignore sanitization and unslashing. Preventing use of sanitization in password will lead to removal of special characters.
		...
		if ( $this->moazure_verify_form( 'moazure_goto_login', 'moazure_goto_login_form', 'moazure_goto_login_form_field' ) ) {
			delete_option( 'moazure_new_registration' );
			update_option( 'moazure_verify_customer', 'true' );
		}

		if ( $this->moazure_verify_form( 'moazure_verify_customer', 'moazure_verify_password_form', 'moazure_verify_password_form_field' ) ) {   // register the admin to miniOrange.
			if ( current_user_can( 'administrator' ) ) {
				if ( moazure_is_curl_installed() === 0 ) {
					return $this->moazure_show_curl_error();
				}
				...
				$email    = '';
				$password = '';
				if ( $this->moazure_check_empty_or_null( sanitize_text_field( wp_unslash( $_POST['email'] ) ) ) || $this->moazure_check_empty_or_null( $_POST['password'] ) ) { //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- As we are not storing password in the database, so we can ignore sanitization and unslashing.
					$notice_arr['msg_type'] = 'error';
					$notice_arr['msg_desc'] = 'All the fields are required. Please enter valid entries.';
					update_option( 'notice_settings', $notice_arr );
					return;
					...
					$email    = ! empty( $_POST['email'] ) ? sanitize_email( wp_unslash( $_POST['email'] ) ) : '';
					$password = ! empty( $_POST['password'] ) ? stripslashes( $_POST['password'] ) : ''; //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- As we are not storing password in the database, so we can ignore sanitization and unslashing.
				}
				update_option( 'moazure_admin_email', $email );
				$customer     = new MOAzure_Client_Customer();
		...
		} elseif ( $this->moazure_verify_form( 'moazure_add_app', 'moazure_add_app_form', 'moazure_add_app_form_field' ) ) {
			if ( current_user_can( 'administrator' ) ) {
				$scope        = '';
				...
				if ( $this->moazure_check_empty_or_null( $clientid ) || $this->moazure_check_empty_or_null( $clientsecret ) ) {
					...
					if ( 'azure-b2c' === $app_type ) {
						...
						$accesstoken_url = 'https://login.microsoftonline.com/' . $tenant_id . '/oauth2/v2.0/token';
		...
		} elseif ( $this->moazure_verify_form( 'moazure_attribute_mapping', 'moazure_attr_role_mapping_form', 'moazure_attr_role_mapping_form_field' ) ) {
		...
		} elseif ( $this->moazure_verify_form( 'moazure_save_role_mapping', 'moazure_role_mapping_form_nonce', 'moazure_role_mapping_form_field' ) ) {
			if ( current_user_can( 'administrator' ) ) {
		...
		} elseif ( $this->moazure_verify_form( 'moazure_pbi_sc_config', 'moazure_pbi_shortcode_form', 'moazure_pbi_shortcode_form_field' ) ) {
			if ( current_user_can( 'administrator' ) ) {
		...
		} elseif ( $this->moazure_verify_form( 'moazure_pbi_all_sc_delete', 'moazure_pbi_all_sc_del_form', 'moazure_pbi_all_sc_del_form_field' ) ) {
		...
		} elseif ( $this->moazure_verify_form( 'moazure_pbi_sc_delete', 'moazure_pbi_sc_del_form', 'moazure_pbi_sc_del_form_field' ) ) {
			if ( current_user_can( 'administrator' ) ) {
		...
		} elseif ( $this->moazure_verify_form( 'moazure_contact_us_query_option', 'moazure_support_form', 'moazure_support_form_field' ) ) {
			if ( current_user_can( 'administrator' ) ) {
				if ( moazure_is_curl_installed() === 0 ) {
					return $this->moazure_show_curl_error();
				}
		...
		} elseif ( $this->moazure_verify_form( 'moazure_video_demo_request_form', 'moazure_video_demo_request_form', 'moazure_video_demo_request_field' ) ) {
			if ( current_user_can( 'administrator' ) ) {
				if ( moazure_is_curl_installed() === 0 ) {
					return $this->moazure_show_curl_error();
				}
		...
		} elseif ( $this->moazure_verify_form( 'moazure_forgot_password_form_option', 'moazure_forgotpassword_form', 'moazure_forgotpassword_form_field' ) ) {
		...
		} elseif ( $this->moazure_verify_form( 'moazure_change_email', 'moazure_change_email_form', 'moazure_change_email_form_field' ) ) {
			// Adding back button.
		...
		} elseif ( $this->moazure_verify_form( 'moazure_app_logout_option', 'moazure_app_logout_form', 'moazure_app_logout_form_field' ) ) {
			if ( isset( $_POST['moazure_app_logout'] ) && 'Logout' === $_POST['moazure_app_logout'] ) {
				...
				switch ( $app_to_logout ) {
					...
					case 'mail':
						$ms_entra_apps['mail_manual'] = false;
						MOAzure_Admin_Utils::moazure_update_option( 'moazure_ms_entra_apps', $ms_entra_apps );
						MOAzure_Admin_Utils::moazure_delete_option( 'moazure_mail_config' );
						MOAzure_Admin_Utils::moazure_delete_option( 'moazure_manual_email_config' );
						break;
		...
		} elseif ( $this->moazure_verify_form( 'moazure_use_entra', 'moazure_use_entra_form', 'moazure_use_entra_form_field' ) ) {
			...
			switch ( $ms_manual_app ) {
				...
				case 'mail':
					$ms_entra_apps['mail_manual'] = true;
					MOAzure_Admin_Utils::moazure_update_option( 'moazure_ms_entra_apps', $ms_entra_apps );
					break;
		...
		} elseif ( $this->moazure_verify_form( 'moazure_remove_pbi_shortcode', 'moazure_remove_pbi_sc_form', 'moazure_remove_pbi_sc_form_field' ) ) {
			...
			update_option( 'notice_settings', $notice_arr );
		} elseif ( $this->moazure_verify_form( 'moazure_mail_send_config', 'moazure_mail_send_form', 'moazure_mail_send_form_field' ) ) {
			$mail_from = sanitize_email( wp_unslash( $_POST['moazure_mail_from'] ) );

			if ( empty( $mail_from ) ) {
				$notice_arr['msg_type'] = 'error';
				$notice_arr['msg_desc'] = 'Invalid or missing sender email.';
				MOAzure_Admin_Utils::moazure_update_option( 'notice_settings', $notice_arr );

				return;
			}

			$save_to_outlook_sentbox = isset( $_POST['moazure_save_to_outlook_sentbox'] ) ? (int) filter_var( sanitize_text_field( wp_unslash( $_POST['moazure_save_to_outlook_sentbox'] ) ) ) : 0;
			$send_to_new_user        = isset( $_POST['moazure_send_to_new_user'] ) ? (int) filter_var( sanitize_text_field( wp_unslash( $_POST['moazure_send_to_new_user'] ) ) ) : 0;

			$mail_config = array(
				'mail_from'               => $mail_from,
				'save_to_outlook_sentbox' => $save_to_outlook_sentbox,
				'send_to_new_user'        => $send_to_new_user,
			);

			MOAzure_Admin_Utils::moazure_update_option( 'moazure_mail_config', $mail_config );

			$notice_arr['msg_type'] = 'success';
			$notice_arr['msg_desc'] = 'Mail config saved successfully.';
			MOAzure_Admin_Utils::moazure_update_option( 'notice_settings', $notice_arr );
		} elseif ( $this->moazure_verify_form( 'moazure_manual_email_config', 'moazure_manual_email_config_form', 'moazure_manual_email_config_form_field' ) ) {
			$mail_to = ! empty( $_POST['moazure_manual_mail_to'] ) ? sanitize_email( wp_unslash( $_POST['moazure_manual_mail_to'] ) ) : '';

			if ( empty( $mail_to ) ) {
				$notice_arr['msg_type'] = 'error';
				$notice_arr['msg_desc'] = 'Invalid or missing receiver email found.';
				MOAzure_Admin_Utils::moazure_update_option( 'notice_settings', $notice_arr );

				return;
			}

			$mail_subject        = ! empty( $_POST['moazure_manual_mail_subject'] ) ? sanitize_text_field( wp_unslash( $_POST['moazure_manual_mail_subject'] ) ) : 'Microsoft Graph Mailer';
			$mail_content        = ! empty( $_POST['moazure_manual_mail_content'] ) ? wp_strip_all_tags( wp_unslash( $_POST['moazure_manual_mail_content'] ) ) : 'Hi, this is an email sent via Microsoft Graph API.';
			$manual_email_config = array(
				'mail_to' => $mail_to,
				'subject' => $mail_subject,
				'content' => $mail_content,
			);

			MOAzure_Admin_Utils::moazure_update_option( 'moazure_manual_email_config', $manual_email_config );

			$notice_arr['msg_type'] = 'success';
			$notice_arr['msg_desc'] = 'Receiver\'s mail config saved successfully.';
			MOAzure_Admin_Utils::moazure_update_option( 'notice_settings', $notice_arr );
		} elseif ( $this->moazure_verify_form( 'moazure_send_email', 'moazure_send_email_form', 'moazure_send_email_form_field' ) ) {

			$mail_handler = MOAzure_Mail_Handler::get_mail_handler_obj();
			$mail_handler->moazure_send_email_manually();
		}
	}
