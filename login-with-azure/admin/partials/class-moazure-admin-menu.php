...
class MOAzure_Admin_Menu {
			...
			<div class="container moazure-flex moazure_top_container">
				...
				<div>
					<a id="" class="mo_generic-btns-on-top moazure_header_link" href="https://plugins.miniorange.com/wordpress-azure-office365-integrations#pricing-plans" target="_blank" style="background-color: #0072c6!important; border-radius: 30px;color: white!important; padding: 10px;">
						...
						<span>
							<?php esc_html_e( 'Check Premium Plans', 'login-with-azure' ); ?>
						</span>
					</a>
				</div>
				...
				<div class="moazure_wrap moazure-flex">
					<div>
						<a id="" class="mo_generic-btns-on-top moazure_header_link" href="https://calendly.com/vishal-singh-inc/wordpress-azure-sso-setup" target="_blank" style="background-color: #0072c6!important; border-radius: 30px;color: white!important; padding: 10px;">
							<span>
								<?php esc_html_e( 'Schedule Free Setup', 'login-with-azure' ); ?>
							</span>
						</a>
					</div>
					<div>
						...
						">
							<span>
								<?php esc_html_e( 'Request for Demo/Trial', 'login-with-azure' ); ?>
							</span>
						</a>
					</div>
					<div>
						...
						">
							<span>
								<?php esc_html_e( 'Account Setup', 'login-with-azure' ); ?>
							</span>
						</a>
					</div>
				</div>
		...
		<div>
			<div class="moazure-features moazure-flex">
				<div class="feat-app moazure-rad <?php echo ( 'entra-id' === $currentapp ) ? 'sel-app' : ''; ?>" data-tab="moazure_config" data-app="entra-id">
					...
					<span class="feat-text"><?php esc_html_e( 'EntraID (AzureAD)', 'login-with-azure' ); ?></span>
				</div>
				<div class="feat-app moazure-rad <?php echo ( 'azure-b2c' === $currentapp ) ? 'sel-app' : ''; ?>" data-tab="moazure_config" data-app="azure-b2c">
					...
					<span class="feat-text"><?php esc_html_e( 'Azure AD B2C', 'login-with-azure' ); ?></span>
				</div>
				<div class="feat-app moazure-rad <?php echo ( ! empty( $currentapp ) && 'sharepoint' === $currentapp ) ? 'sel-app' : ''; ?>" data-tab="sps_preview" data-app="sharepoint">
					...
					<span class="feat-text"><?php esc_html_e( 'Sharepoint', 'login-with-azure' ); ?></span>
				</div>
				<div class="feat-app moazure-rad <?php echo ( ! empty( $currentapp ) && 'power-bi' === $currentapp ) ? 'sel-app' : ''; ?>" data-tab="pbi_app" data-app="power-bi">
					...
					<span class="feat-text"><?php esc_html_e( 'Power BI', 'login-with-azure' ); ?></span>
				</div>
				<div class="feat-app moazure-rad <?php echo ( ! empty( $currentapp ) && 'mail' === $currentapp ) ? 'sel-app' : ''; ?>" data-tab="mail_app" data-app="mail">
					<img class="feat-icon" src="<?php echo esc_url( dirname( plugin_dir_url( __FILE__ ) ) ); ?>/images/email.png" alt="mail logo"><br/>
					<span class="feat-text"><?php esc_html_e( 'Mail', 'login-with-azure' ); ?></span>
				</div>
				<div class="feat-app moazure-rad <?php echo ( ! empty( $currentapp ) && 'dynamics-crm' === $currentapp ) ? 'sel-app' : ''; ?>" data-tab="requestfordemo" data-app="dynamics-crm">
					...
					<span class="feat-text"><?php esc_html_e( 'Dynamics CRM', 'login-with-azure' ); ?></span>
				</div>
			</div>
		</div>
		<br/>
				...
				<h2 class="moazure_nav_tab_wrapper moazure-flex">
					...
					if ( 'entra-id' === $currentapp || 'azure-b2c' === $currentapp ) {
						...
						<?php
							echo ! empty( $_SERVER['REQUEST_URI'] ) ? esc_url_raw(
								add_query_arg(
									array(
										'tab' => 'moazure_config',
										'app' => ( 'azure-b2c' === $currentapp ) ? 'azure-b2c' : 'entra-id',
									),
									sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) )
								)
							) : '';
						...
						" class="moazure_nav-tab anglebg moazure-rad
							...
							?>
							">
									<?php esc_html_e( 'Configure Application', 'login-with-azure' ); ?>
						</a>
						<a id="tab-attrmapping" href="
						<?php
							echo ! empty( $_SERVER['REQUEST_URI'] ) ? esc_url_raw(
								add_query_arg(
									array(
										'tab' => 'attributemapping',
										'app' => ( 'azure-b2c' === $currentapp ) ? 'azure-b2c' : 'entra-id',
									),
									sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) )
								)
						...
						?>
						" class="moazure_nav-tab moazure-rad
							<?php
							if ( 'attributemapping' === $currenttab ) {
								echo 'moazure_nav-tab-active';}
							?>
							">
									<?php esc_html_e( 'Attribute/Role Mapping', 'login-with-azure' ); ?>
						</a>
						<a id="tab-signinsettings" href="
						<?php
							echo ! empty( $_SERVER['REQUEST_URI'] ) ? esc_url_raw(
								add_query_arg(
									array(
						...
						" class="moazure_nav-tab moazure-rad
							...
							?>
							">
									<?php esc_html_e( 'Login Button Customization', 'login-with-azure' ); ?>
									<span>
										<img class="moazure_premium-label" src="<?php echo esc_url( dirname( plugin_dir_url( __FILE__ ) ) . '/partials/apps/images/moazure_premium-label.png' ); ?>" alt="miniOrange Standard Plans Logo">
									</span>
						</a>
						...
						" class="moazure_nav-tab moazure-rad
							...
							?>
							">
									<?php esc_html_e( 'Troubleshoot', 'login-with-azure' ); ?>
						</a>
						<?php
					} elseif ( 'sharepoint' === $currentapp ) {
						?>
						<a id="tab-preview" href="
						<?php
							echo ! empty( $_SERVER['REQUEST_URI'] ) ? esc_url_raw(
								add_query_arg(
									array(
						...
						" class="moazure_nav-tab anglebg moazure-rad
							...
							?>
							">
									<?php esc_html_e( 'Configure Folders/ Files', 'login-with-azure' ); ?>
						</a>
						<a id="tab-sps-shortcode" href="
						<?php
							echo ! empty( $_SERVER['REQUEST_URI'] ) ? esc_url_raw(
								add_query_arg(
									array(
						...
						" class="moazure_nav-tab anglebg moazure-rad
							...
							?>
							">
									<?php esc_html_e( 'Embed Sharepoint Library', 'login-with-azure' ); ?>
						</a>
						<a id="tab-sps-shortcode" href="
						<?php
							echo ! empty( $_SERVER['REQUEST_URI'] ) ? esc_url_raw(
						...
						" class="moazure_nav-tab anglebg moazure-rad
							...
							?>
							">
									<?php esc_html_e( 'Embedded View', 'login-with-azure' ); ?>
									<span>
										<img class="moazure_premium-label" src="<?php echo esc_url( dirname( plugin_dir_url( __FILE__ ) ) . '/partials/apps/images/moazure_premium-label.png' ); ?>" alt="miniOrange Standard Plans Logo">
									</span>
						</a>
						...
						" class="moazure_nav-tab anglebg moazure-rad
							...
							?>
							">
								<?php esc_html_e( 'Advanced Settings', 'login-with-azure' ); ?>
								<span>
									<img class="moazure_premium-label" src="<?php echo esc_url( dirname( plugin_dir_url( __FILE__ ) ) . '/partials/apps/images/moazure_premium-label.png' ); ?>" alt="miniOrange Standard Plans Logo">
								</span>
						</a>
						...
						" class="moazure_nav-tab anglebg moazure-rad
							...
							?>
							">
								<?php esc_html_e( 'Configure Power BI', 'login-with-azure' ); ?>
						</a>
						<a id="tab-pbi-shortcode" href="
						<?php
							echo ! empty( $_SERVER['REQUEST_URI'] ) ? esc_url_raw(
								add_query_arg(
									array(
						...
						" class="moazure_nav-tab anglebg moazure-rad
							...
							?>
							">
								<?php esc_html_e( 'Settings', 'login-with-azure' ); ?>
								<span>
									<img class="moazure_premium-label" src="<?php echo esc_url( dirname( plugin_dir_url( __FILE__ ) ) . '/partials/apps/images/moazure_premium-label.png' ); ?>" alt="miniOrange Standard Plans Logo">
								</span>
						</a>
						<?php
					} elseif ( 'mail' === $currentapp ) {
						?>
						<a id="mail-app" href="
						<?php
							echo ! empty( $_SERVER['REQUEST_URI'] ) ? esc_url_raw(
						...
						" class="moazure_nav-tab anglebg moazure-rad
							<?php
							if ( 'mail_app' === $currenttab && 'mail' === $currentapp ) {
								echo 'moazure_nav-tab-active';
							}
							?>
							">
								<?php esc_html_e( 'Configure Mail', 'login-with-azure' ); ?>
						</a>
						<a id="tab-mail-send" href="
						<?php
							echo ! empty( $_SERVER['REQUEST_URI'] ) ? esc_url_raw(
								add_query_arg(
									array(
						...
						" class="moazure_nav-tab anglebg moazure-rad
							<?php
							if ( 'mail_send' === $currenttab && 'mail' === $currentapp ) {
								echo 'moazure_nav-tab-active';
							}
							?>
							">
								<?php esc_html_e( 'Send Mail', 'login-with-azure' ); ?>
						</a>
						<?php
			...
			<div class="moazure-flex" style="justify-content: space-between; margin-top: 0.5rem;">
				...
				<div>
					<form name="moazure_app_logout_form" method="post" action="">
						...
						<input type="hidden" name="ms_app" value="<?php echo esc_attr( $app ); ?>" />
						<input type="submit" class="button button-large moazure_delete_btn moazure-rad" name="moazure_app_logout" value="<?php esc_html_e( 'Logout', 'login-with-azure' ); ?>" />
					</form>
				</div>
			</div>
			<div>
				<p style="margin-bottom: 0;">
					<b>Note : </b>Logging out here will delete all the <?php echo esc_html( $app ); ?> app configurations from the plugin.
				</p>
			</div>
		</div>
		<?php
	...
	public static function show_tab( $currenttab, $currentapp ) {
		...
		} elseif ( 'entra-id' !== $currentapp && 'azure-b2c' !== $currentapp ) {
	...
	public static function show_support_sidebar( $currenttab, $currentapp ) {
		...
		if ( 'licensing' !== $currenttab ) {
			...
			if ( 'attributemapping' === $currenttab && $apptype === $currentapp ) {
				...
				MOAzure_Support::support();
			} elseif ( 'mail' === $currentapp && ! empty( $ms_entra_apps['mail_manual'] ) ) {
				self::ms_app_user_details( $currentapp );
				MOAzure_Support::support();
			} else {
				MOAzure_Support::support();
