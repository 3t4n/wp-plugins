			...
			<div class="mo_demo_layout moazure_contact_heading moazure_outer_div">
				<div class="moazure_request_demo_header">
					<div class="moazure_attribute_map_heading">
						<?php esc_html_e( 'Schedule a Demo with our Experts', 'login-with-azure' ); ?>
					</div>
				</div>
						...
						<?php wp_nonce_field( 'moazure_video_demo_request_form', 'moazure_video_demo_request_field' ); ?>
							...
							<table class="moazure_video_demo_table mo_demo_table_layout">
								...
								<tr id="integration-list">
									<td colspan="2">
										<p>
											<strong class="mo_strong"><?php esc_html_e( 'Select the Integrations you are interested in (Optional)', 'login-with-azure' ); ?> :</strong>
										</p>
										<blockquote class="moazure_blockquote">
											<i><strong class="mo_strong">(<?php esc_html_e( 'Note', 'login-with-azure' ); ?>: </strong> <?php esc_html_e( 'All-Inclusive plan entitles all the integrations in the license cost itself.', 'login-with-azure' ); ?> )</i>
										</blockquote>
										<table>
										<?php
										$count = 0;
										foreach ( MOAzure_Integrations::$all_integrations as $key => $value ) {
											if ( true === $value['in_allinclusive'] ) {
												...
												<tr>
													<td>
														<input type="checkbox" class="mo_input_checkbox moazure_demo_form_checkbox" <?php checked( $value['app'], $appparam ); ?> style="margin:7px 5px 7px 5px" name="<?php echo esc_attr( $value['tag'] ); ?>" value="true"> <?php echo esc_html( $value['title'] ); ?>
														<br/>
													</td>
													<?php
													++$count;
								...
								<tr>
									<td>
										<input type="submit" name="submit" value="<?php esc_html_e( 'Submit Demo Request', 'login-with-azure' ); ?>" class="button button-large moazure_demo_request_btn" />
									</td>
								</tr>
							</table>
						</div>
