 ...
 * Display Add-ons
 */
class MOAzure_Third_Party_Integrations {
	/**
...
.moazure_column_container > .moazure_column_inner {
box-sizing: border-box;
padding-left: 15px;
padding-right: 10px;
width: 100%;
...
.moazure_benefits-outer-block:hover{
...
.moazure_know_more_button:hover{
...
<input type="hidden" value="<?php echo esc_attr( moazure_is_customer_registered() ); ?>" id="mo_customer_registered_addon">
...
<form style="display:none;" id="loginform_addon" action="<?php echo esc_attr( get_option( 'host_name' ) ) . '/moas/login'; ?>" target="_blank" method="post">
			<?php wp_nonce_field( 'moazure_login_form_addon_nonce', 'moazur_loginform_addon_field' ); ?>
			<input type="email" name="username" value="<?php echo esc_attr( get_option( 'moazure_admin_email' ) ); ?>"/>
	...
	value="<?php echo 'https://plugins.miniorange.com/go/oauth-2fa-buy-now-payment'; ?>"/>
		...
		if ( count( $addons_recommended ) > 0 ) {
			?>
	<div class="moazure_table_layout">
	<b><p style="padding-left: 15px;font-size: 20px;"><?php esc_attr_e( 'Recommended Add-ons for you:', 'login-with-azure' ); ?></p></b>
	<div class="moazure_outermost-div">
	<div class="row-view">
			<?php
...
<div class="moazure_customization_header">
<div class="moazure_signing_heading">Check out our Add-ons :</div></div>
<div class="moazure_outermost-div">
...
<script type="text/javascript">
	function upgradeform(planType) {
				if(planType === "") {
					...
					location.href = "https://wordpress.org/plugins/login-with-azure/";
					return;
				} else {	
	...
	public static function get_single_addon_cardt( $tag ) {
		...
		if ( isset( $addon ) ) {
			?>
		<div class="grid_view moazure_column_container" style="border-radius: 5px; text-align:center;">
			<div class="moazure_column_inner" style="border-radius: 5px;">
			<div class="row moazure_benefits-outer-block">
					<div> 
					<img src="<?php echo esc_url( plugins_url( $addon['img'], __FILE__ ) ); ?>" class="mo_oauth_addon_img">
					</div>
				<div class="moazure_addon_headline"><strong class="mo_strong"><p><a  href= "<?php echo isset( $addon['link'] ) ? esc_attr( $addon['link'] ) : ''; ?>" target="_blank" rel="noopener"><?php echo esc_html( $addon['title'] ); ?></a></p></strong></div>
			<p class="moazuer_addon_description"><?php echo esc_html( $addon['desc'] ); ?></p>
			<a class="moazure_btn moazure_know_more_button" href= "<?php echo esc_url( $addon['link'] ); ?>" target="_blank" >Know More</a> 
			</div>
			</div>
		</div>
			<?php
