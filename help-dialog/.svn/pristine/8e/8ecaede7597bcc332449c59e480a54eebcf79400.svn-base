...
class EPHD_Help_Dialog_Front_Ctrl {
	...
	public function submit_contact_form() {
		...
		if ( empty( $_REQUEST['jsnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['jsnonce'] ) ), '_wpnonce_ephd_ajax_action' ) ) {
			wp_send_json_success( esc_html__( 'Thank you. We will get back to you soon.', 'help-dialog' ) );
		}
		...
		// get home page url
		if ( $page_id === 0 ) {
			$submitted_page_url = home_url();
		} else {
			$submitted_page_url = get_permalink( $page_id );
		}
		$submitted_page_url = empty( $submitted_page_url ) ? '' : $submitted_page_url;
		...
		if ( empty( $global_config['contact_submission_email'] ) ) {
			...
			$message_escaped = str_replace( [ "\r\n", '\r\n', "\n", '\n', "\r", '\r' ], '<br />', $message_escaped );
						...
						<table cellspacing="0" cellpadding="0" border="0" style="width: 100%;">
							<tbody>
			                    ...
			                    <tr style="background-color:#FFFFFF;">
									...
									<td style="font-family: sans-serif; font-size:12px;padding:3px;">' . esc_html( $reply_to_name ) . '</td>
			                    </tr>
			                    <tr style="background-color:#EAF2FA;">
			                    ...
			                    <tr style="background-color:#FFFFFF;">
									<td width="20" style="padding:3px;">&nbsp;</td>
									<td style="font-family: sans-serif; font-size:12px;padding:3px;"><a href="mailto:' . esc_html( $reply_to_email ) . '">' . esc_html( $reply_to_email ) . '</a></td>
			                    </tr>
			                    <tr style="background-color:#EAF2FA;">
			                    ...
			                    <tr style="background-color:#FFFFFF;">
									<td width="20" style="padding:3px;">&nbsp;</td>
									<td style="font-family: sans-serif; font-size:12px;padding:3px;">' . wp_kses_post( $message_escaped ) . '<br /></td>
			                    </tr> 
			                    <tr style="background-color:#EAF2FA;">
									<td colspan="2" style="font-family: sans-serif; font-size:12px;padding:3px;"><strong>' . esc_html__( 'Submitted Page URL', 'help-dialog' ) . '</strong></td>
			                    </tr> 
			                    <tr style="background-color:#FFFFFF;">
									<td width="20" style="padding:3px;">&nbsp;</td>
									<td style="font-family: sans-serif; font-size:12px;padding:3px;">' . ( empty( $submitted_page_url ) ? 'N/A' : '<a href="' . esc_url( $submitted_page_url ) . '" target="_blank">' . esc_html( $submitted_page_url ) . '</a>' ) . '</td>
			                    </tr>
							</tbody>
						</table>
					</body>
			...
			$send_result = EPHD_Utilities::send_email( $email_message, true, $global_config['contact_submission_email'], $reply_to_email, $reply_to_name, $subject );
		...
		//Add Event Submission to Analytics
		// is user role excluded from counting?
		$add_form_submission_to_analytics = true;
		...
		if ( ! empty( $current_user ) ) {
			...
			if ( ! empty( $current_user_excluded_roles ) ) {
				$add_form_submission_to_analytics = false;
			}
		}

		if ( $add_form_submission_to_analytics  ) {
			// Retrieve Object ID.
			$object_id = (int)EPHD_Utilities::post( 'contact_form_id' );
