...
class EPHD_Widgets_Page {
	...
	public function display_page() {
		...
		<div id="ephd-admin-page-wrap">
            ...
            <div class="ephd-widgets-page-container">				<?php
	             ...
	             *  Tab Navigation
	             */
	            echo $this->get_widget__preview_box_tab_navigation( $this->widgets_config );
	            ...
	            EPHD_HTML_Forms::dialog_confirm_action( array(
		            ...
		            'title'             => esc_html__( 'Deleting Widget', 'help-dialog' ),
		            'body'              => esc_html__( 'Are you sure you want to delete the Widget? You cannot undo this action.', 'help-dialog' ),
		            'accept_label'      => esc_html__( 'Delete', 'help-dialog' ),
		            'accept_type'       => 'warning',
		            'show_cancel_btn'   => 'yes',
		            'form_method'       => 'post',
	            ) );
	            ...
	            EPHD_HTML_Forms::dialog_confirm_action( array(
		            ...
		            'title'             => esc_html__( 'Deleting FAQs', 'help-dialog' ),
		            'body'              => esc_html__( 'Are you sure you want to delete the FAQs? You cannot undo this action.', 'help-dialog' ),
		            'accept_label'      => esc_html__( 'Delete', 'help-dialog' ),
		            'accept_type'       => 'warning',
		            'show_cancel_btn'   => 'yes',
		            'form_method'       => 'post',
		...
		foreach ( $this->message as $class => $message ) {
			echo  EPHD_HTML_Forms::notification_box_bottom( $message, '', $class );//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
	...
	public static function delete_question_confirm__dialog( $widgets_config ) {
		...
		ob_start(); ?>
        <p><?php echo esc_html__( 'Are you sure you want to delete the question? You cannot undo this action.', 'help-dialog' ); ?></p>
        <div class="ephd-admin__confirm-dialog-assigned-widgets">
            <p><?php echo esc_html__( 'Deleting this question will remove it from all assigned Widgets:', 'help-dialog' );  ?></p>
            <ul>    <?php
		...
		EPHD_HTML_Forms::dialog_confirm_action( array(
			...
			'title'             => esc_html__( 'Deleting Question', 'help-dialog' ),
    ...
    private function display_widget_preview_box() { ?>
        ...
        <div class="ephd-wp__widget-preview" data-preview-size="<?php echo esc_attr( $this->global_config['dialog_width'] ); ?>">
            <div class="ephd-wp__widget-preview-tooltip">
				...
				<div class="ephd-wp__widget-preview-tooltip--resource" style="display:none;"><?php
					esc_html_e( 'Live preview of the Resource tab.', 'help-dialog' ); ?></div>
		...
		if ( ( ! empty( $add_on_messages ) && is_array( $add_on_messages ) ) || did_action( 'hd_overview_add_on_errors' ) ) {
			...
			foreach ( $add_on_messages as $add_on_name => $add_on_message ) {
				...
				$error_boxes[] = array(
					...
					'title' => $add_on_name . ': ' . esc_html__( 'License issue', 'help-dialog' ),
					'description' => '',
					'html' => $add_on_message,
				);
			}
	...
	public function get_widget_form( $widget ) {
		...
		// Determine the status text based on the widget status
		switch ( $widget['widget_status'] ) {
			case 'draft':
				$status_text_body = esc_html__( 'Only admins can view this widget', 'help-dialog' );
				break;
			case 'published':
				$status_text_body = esc_html__( 'Visible to all site visitors', 'help-dialog' );
				break;
			default:
				$status_text_body = esc_html__( 'Unknown status', 'help-dialog' );
				break;
		}
		...
		EPHD_HTML_Admin::display_admin_form_header( array(
			...
			'title_desc'        => esc_html__( 'Widget Name', 'help-dialog' ) . ': ',
			'status_text'       => esc_html__( 'Status', 'help-dialog' ) . ': ',
			'status_text_body'  => $status_text_body,
		...
		$tabs_config = [
			'locations' => [
				'label' => esc_html__( 'Locations / Triggers', 'help-dialog' ),
				'tabs'  => [],
			],
			'structure' => [
				'label' => esc_html__( 'Structure', 'help-dialog' ),
				'tabs'  => [],
			],
			'tab_features' => [
				'label' => esc_html__( 'Main Features', 'help-dialog' ),
				'tabs'  => [],
			],
			'design' => [
				'label' => esc_html__( 'Design', 'help-dialog' ),
				'tabs'  => [],
			],
		];
		...
		$tabs_config['locations']['tabs'][] = array(
			'title'     => esc_html__( 'Pages', 'help-dialog' ),
			'icon'      => 'ephdfa ephdfa-file-text-o',
			'key'       => 'pages',
			'active'    => true,
			'contents'  => array(
				array(
					'title'         => esc_html__( 'Display Widget On These Pages', 'help-dialog' ),
					'desc'          => esc_html__( 'Choose pages to display the Help Dialog widget. Add more pages by searching for them.', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_widget_pages( $widget ),
					'read_more_url' => 'https://www.helpdialog.com/documentation/',
					'read_more_text'=> esc_html__( 'Read More', 'help-dialog' ),
				),
			),
		);
		...
		$tabs_config['locations']['tabs'][] = array(
			'title'     => esc_html__( 'Triggers', 'help-dialog' ),
			'icon'      => 'ephdfa ephdfa-eye',
			'key'       => 'triggers',
			'active'    => false,
			'contents'  => array(
				array(
					'title'         => esc_html__( 'First Time - Display Widget After Delay', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_trigger_delay( $widget ),
				),
				array(
					'title'         => esc_html__( 'First Time - Display Widget After Scroll', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_trigger_scroll( $widget ),
				),
				/*array(
					'title'         => esc_html__( 'Display Widget After Number of Days and Hours', 'help-dialog' ),
		...
		$tabs_config['structure']['tabs'][] = array(
			'title'         => esc_html__( 'Launcher', 'help-dialog' ),
			'icon'          => 'ephdfa ephdfa-font ephdfa-comments-o',
			'key'           => 'launcher',
			'active'        => false,
			'contents'  => array(
				array(
					'title'         => esc_html__( 'Settings', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_global_launcher_settings( $widget ),
				),
				array(
					'title'         => esc_html__( 'Initial Message', 'help-dialog' ) . ' (' . esc_html__( 'Optional', 'help-dialog' ) . ')',
					'tab_id'        => 'ephd-initial-message',
					'body_html'     =>  $this->pro_enabled ? $this->get_tab_content_initial_message( $widget ) :
						EPHD_HTML_Forms::pro_feature_ad_box( array(
							'id'				=> 'ephd-dialog-pro-feature-ad-initial-message',
							'title'				=> sprintf( __( "Additional %sInitial Message Feature%s", 'help-dialog' ), '<strong>', '</strong>' ),
							'list'				=> array(
								esc_html__( 'Show welcome message above the launcher', 'help-dialog' ),
								esc_html__( 'Show icon with text', 'help-dialog' ),
							),
							'btn_text'			=> esc_html__( 'Upgrade Now', 'help-dialog' ),
							'btn_url'			=> 'https://www.helpdialog.com/documentation/initial-message-settings/',
							'show_close_btn'	=> 'yes',
							'return_html' 		=> true,
						) ),
				),
		...
		$tabs_config['structure']['tabs'][] = array(
			'title'         => esc_html__( 'Dialog', 'help-dialog' ),
			'icon'          => 'ephdfa ephd-dialog-icon',
			'key'           => 'global-settings',
			'active'        => false,
			'contents'  => array(
				array(
					'title'         => esc_html__( 'Dialog Set Up', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_global_dialog_settings( $widget ),
				),
				array(
					'title'         => esc_html__( 'Article/Post Preview', 'help-dialog' ),
		...
		// Structure: Search
		$tabs_config['structure']['tabs'][] = array(
			'title'     => esc_html__( 'Search', 'help-dialog' ),
			...
			'contents'  => array(
				array(
					'title'         => esc_html__( 'Search', 'help-dialog' ),
					'desc'          => esc_html__( 'Choose Search to show in the Help Dialog widget.', 'help-dialog' ),
		...
		/* FUTURE TODO $tabs_config['structure']['tabs'][] = array(
			'title'     => esc_html__( 'Messages', 'help-dialog' ),
		...
		$general_tab_contents = array(
			array(
				'title'     => esc_html__( 'Widget Nickname', 'help-dialog' ),
		...
		if ( $widget['widget_id'] > EPHD_Config_Specs::DEFAULT_ID ) {
			$general_tab_contents[] = array(
				'title'     => esc_html__( 'Delete This Widget', 'help-dialog' ),
		...
		// Structure: General
		$tabs_config['structure']['tabs'][] = array(
			'title'     => esc_html__( 'General', 'help-dialog' ),
		...
		// Tab Features: Resource
		$tabs_config['tab_features']['tabs'][] = array(
			'title'         => esc_html__( 'Resource', 'help-dialog' ),
			'icon'          => 'ephdfa ephdfa-link',
			'key'           => 'resource',
			'active'        => false,
			'contents'  => array(
				array(
					'title'         => esc_html__( 'Resource', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_resource_settings( $widget ),
				),
			),
			'data'      => array( 'preview' => 1 ),
		);
		...
		// Tab Features: FAQs
		$tabs_config['tab_features']['tabs'][] = array(
			'title'     => esc_html__( 'FAQs', 'help-dialog' ),
			'icon'      => 'ephdfa ephd-faqs-icon',
			'key'       => 'faqs',
			'active'    => false,
			'contents'  => array(
				array(
					'title'         => esc_html__( 'Add Questions to the Help Dialog', 'help-dialog' ),
					'desc'          => esc_html__( 'All questions will be searchable, but only questions added to the Widget here will be shown when the Widget is displayed.', 'help-dialog' ),
		...
		$tabs_config['tab_features']['tabs'][] = array(
			'title'         => esc_html__( 'Contact Form', 'help-dialog' ),
		...
		$tabs_config['design']['tabs'][] = array(
			'title'         => esc_html__( 'Colors', 'help-dialog' ),
			'icon'          => 'ephdfa ephdfa-paint-brush',
			'key'           => 'colors',
			'active'        => false,
			'contents'  => array(
				array(
					'title'         => esc_html__( 'Predefined Colors', 'help-dialog' ),
				...
				array(
					'title'         => esc_html__( 'Help Dialog Window', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_colors_hd_window( $widget ),
				),
				array(
					'title'         => esc_html__( 'Search Results', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_colors_search( $widget ),
				),
				array(
					'title'         => esc_html__( 'FAQ Questions', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_colors_faqs( $widget ),
				),
				array(
					'title'         => esc_html__( 'Single Article', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_colors_article( $widget ),
				),
				array(
					'title'         => esc_html__( 'Back Button', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_colors_back_button( $widget ),
				),
				array(
					'title'         => esc_html__( 'Contact Us Submit Button', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_colors_contact( $widget ),
				),
				array(
					'title'         => esc_html__( 'Contact Us Acceptance Checkbox', 'help-dialog' ),
		...
		// Design: Labels
		$tabs_config['design']['tabs'][] = array(
			'title'         => esc_html__( 'Labels', 'help-dialog' ),
			...
			'contents'  => array(
				array(
					'title'         => esc_html__( 'Launcher', 'help-dialog' ),
					'body_html'     => $this->pro_enabled ? $this->get_tab_content_labels_launcher( $widget ) : '',
				),
				array(
					'title'         => esc_html__( 'FAQ', 'help-dialog' ),
				...
				array(
					'title'         => esc_html__( 'Resource', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_labels_resource( $widget ),
				),
				array(
					'title'         => esc_html__( 'Other', 'help-dialog' ),
					'body_html'     => $this->get_tab_content_labels_other( $widget ),
				),
				// TODO: add Chat labels (the 'chat' we have here is actually 'resource' and not the AI Chat feature)
			),
			'data'      => array( 'preview' => 1 ),
		);

		// filter out boxes with empty 'body_html'
		foreach ( $tabs_config['design']['tabs'] as $tab_index => $one_tab ) {
			foreach ( $one_tab['contents'] as $content_index => $one_content ) {
				if ( empty( $one_content['body_html'] ) ) {
					unset( $tabs_config['design']['tabs'][$tab_index]['contents'][$content_index] );
				}
			}
		}	?>
	...
	private function get_widget_locations_list( $locations_type, $widget_id ) {
        ...
        <div class="ephd-wp__selected-locations-popup">
            ...
            EPHD_HTML_Admin::widget_details_popup(
                esc_html__( 'Selected Locations', 'help-dialog' ) . ' (' . ucfirst( $locations_type ) . 's)',
	...
	private function display_widget_feature_option_toggle_field( $widget, $option_name, $tooltip ) {    ?>
		<!-- Widget Feature Option Field -->
		<div class="ephd-wp__feature-option-field ephd-wp__feature-option-field--text ephd-wp__feature-option-field--<?php echo esc_attr( $option_name ); ?>">
			<div class="ephd-wp__feature-option-field-title">
				<span><?php echo esc_html( $this->widget_specs[$option_name]['label'] ); ?></span>  <?php
				EPHD_HTML_Elements::display_tooltip( $this->widget_specs[$option_name]['label'], $tooltip );   ?>
			</div>
	...
	private function get_locations( $widget_locations_type, $order_by, $search_value, $excluded_locations, $included_locations, $include_all_if_empty=true, $language = 'all' ) {
		...
		$home_page_title = esc_html__( 'Home Page', 'help-dialog' );
		$page_on_front = get_option( 'page_on_front' );
		...
		$query_sql_escaped = "SELECT ID, post_title, post_type";

		// start assembling the SQL query
		$query_sql_escaped .= " FROM $wpdb->posts WHERE post_status IN ('publish', 'private', 'draft')";

		// excluded Location ids
		if ( ! empty( $excluded_locations ) ) {
			$sql_id_placeholders = array_fill( 0, count( $excluded_locations ), '%d' );
			$query_sql_escaped .= " " . $wpdb->prepare( "AND ID NOT IN(" . implode( ', ', $sql_id_placeholders ) . ")", $excluded_locations );
		}

		// included Location ids
		if ( ! empty( $included_locations ) ) {
			$sql_id_placeholders = array_fill( 0, count( $included_locations ), '%d' );
			$query_sql_escaped .= " " . $wpdb->prepare( "AND ID IN(" . implode( ', ', $sql_id_placeholders ) . ")", $included_locations );
		}

		// specify post types of Locations
		$query_sql_escaped .= " " . $wpdb->prepare( "AND post_type = %s AND post_mime_type = ''", $widget_locations_type );

		// optionally use search string
		if ( ! empty( $search_value ) ) {
			$search_value_escaped = '%' . $wpdb->esc_like( $search_value ) . '%';
			$query_sql_escaped .= " " . $wpdb->prepare( "AND post_title LIKE %s", $search_value_escaped );
		}

		$query_sql_escaped .= " " . $wpdb->prepare( "ORDER BY %s ASC", $order_by );

		// query Locations
		$locations = $wpdb->get_results( $query_sql_escaped );// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	private function display_locations_field( $locations_type, $widget_id, $tooltip , $cpt_tooltip_names, $kb_ad_button=false ) {
		...
		$locations_search_title = ''; //esc_html__( 'Search', 'help-dialog' );
		switch ( $locations_type ) {
			case 'page':
				$locations_search_title .= esc_html__( 'Add Pages', 'help-dialog' );
				$locations_search_placeholder = esc_html__( 'type to find page', 'help-dialog' );
				break;

			case 'post':
				$locations_search_title .= esc_html__( 'Add Posts', 'help-dialog' );
				$locations_search_placeholder = esc_html__( 'type to find post', 'help-dialog' );
				break;

			case 'cpt':
				$locations_search_title .= esc_html__( 'Add CPTs', 'help-dialog' );
				$locations_search_placeholder = esc_html__( 'type to find Custom Post Types', 'help-dialog' );
				break;
	...
	private function get_widget_locations( $widget ) {
		...
		foreach( $include_locations as $include_location ) {
			...
			if ( $include_location == EPHD_Config_Specs::HOME_PAGE ) {
				$home_page = new stdClass();
				$home_page->ID = EPHD_Config_Specs::HOME_PAGE;
				$home_page->post_title = esc_html__( 'Home Page', 'help-dialog' );
				$home_page->post_type = 'page';
	...
	private function get_tab_content_widget_pages( $widget ) {
		...
		if ( empty( $multilang_plugin ) ) { ?>
			...
			EPHD_HTML_Elements::dropdown( [
				...
				'options'           => array_merge_recursive( [ 'all' => esc_html__( 'All Languages', 'help-dialog' ) ], $language_options ),
			] );
		}
		...
		EPHD_HTML_Elements::radio_buttons_horizontal( [
			...
			'tooltip_body'      => esc_html__( 'Select how the fields below will be used.', 'help-dialog' ),
		] );

		// selected Locations and search inputs
		$this->display_locations_field( 'page', $widget['widget_id'], esc_html__( 'Select pages to display the Widget on', 'help-dialog' ), $cpt_tooltip_names );
		$this->display_locations_field( 'post', $widget['widget_id'], esc_html__( 'Select posts to display the Widget on', 'help-dialog'  ), $cpt_tooltip_names );

		// show CPTs of supported types if any
		$cpt_tooltip_desc = esc_html__( 'Select posts in Custom Post Types to display the Widget on. List of supported Custom Post Types:', 'help-dialog' ) . '<br/>';
		$cpt_tooltip_desc .= '	<ul>
									<li>LearnPress</li>
									<li>LearnDash</li>
									<li>bbPress</li>
		...
		$cpt_tooltip_desc .= esc_html__( 'If we are missing a Custom Post Type, please contact us.', 'help-dialog' );

		$this->display_locations_field( 'cpt', $widget['widget_id'], $cpt_tooltip_desc, $cpt_tooltip_names, true );

		EPHD_HTML_Elements::textarea( [
			'value'         => $widget['location_wildcard_url'],
			'specs'         => 'location_wildcard_url',
			'tooltip_body'  => esc_html__( 'Target pages by URL, including wildcards. Separate multiple URLs with commas. Example:', 'help-dialog' ) .
									' /knowledge-base/* ' . esc_html__( 'or', 'help-dialog' ) . ' /knowledge-base/service/*',
			'input_size'    => 'medium'
		] );
	...
	private function get_tab_content_widget_search( $widget ) {
		...
		// Search Input
		$this->display_widget_feature_option_toggle_field( $widget, 'search_option', esc_html__( 'Turn this option ON to enable search in the Widget', 'help-dialog' ) );

		// Search: Posts
		$this->display_widget_feature_option_toggle_field( $widget, 'search_posts', esc_html__( 'Turn this option ON to enable search Posts in the Widget', 'help-dialog' ) );
        ...
        if ( EPHD_KB_Core_Utilities::is_kb_or_amag_enabled() ) {
	        $this->display_widget_feature_option_select_field( $widget, $this->get_kb_cpt_options(), 'search_kb',
		        esc_html__( 'Do not search', 'help-dialog' ), esc_html__( 'Select Knowledge Base to display the Widget', 'help-dialog' ) );
        } else {    ?>
            <div class="ephd-wp__feature-option-field">
                <span><?php echo esc_html( $this->widget_specs['search_kb']['label'] ); ?></span>  <?php
	            EPHD_HTML_Elements::display_tooltip( $this->widget_specs['search_kb']['label'], esc_html__( 'Select Knowledge Base to display the Widget', 'help-dialog' ) ); ?>
    ...
    public static function get_tab_content_widget_faqs( $widget, $all_faqs, $return_html=false ) {
        ...
        <div class="ephd-all-questions-container">
            ...
            <div class="ephd-all-questions__body-container">
                ...
                <input type="hidden" name="faqs_sequence" class="ephd-fp__selected-questions" value="<?php echo esc_attr( implode( ',', $faqs_sequence ) ); ?>">
			...
			if ( empty( $all_faqs ) ) {
				EPHD_HTML_Forms::notification_box_middle( array(
					...
					'desc' => esc_html__( 'You have not created any questions yet.', 'help-dialog' ),
				) );
			} else {
				EPHD_HTML_Forms::notification_box_middle( array(
					'id'   => 'ephd-admin__assigned-question-message',
					'type' => 'success-no-icon',
					'desc' => esc_html__( 'All questions have been assigned.', 'help-dialog' ),
				) );
			}           ?>
				...
				foreach ( $color_sets as $id => $colors_set ) {
					...
					<div class="ephd-wp__options-container__option">
						<label class="ephd-wp__option__label" style="background-color:<?php echo esc_attr( $background_main_color ); ?>;color:<?php echo esc_attr( $font_main_color ); ?>;">
							<input type="radio" name="colors_set" value="<?php echo esc_attr( $id ); ?>"<?php checked( true, $currently_active ); ?> data-choose="<?php esc_attr_e( 'Choose', 'help-dialog' ); ?>">
						</label>
					</div>  <?php
				}   ?>
	...
	private function get_tab_content_delete_widget( $widget ) {
        ...
        <!-- Global Widget Settings Form -->
        <div class="ephd-wp__options-container ephd-wp__options-container--global-settings">    <?php
	        EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Delete', 'help-dialog' ), 'ephd_delete_widget', 'ephd-wp__delete-widget-wrap', '', false, '', 'ephd-error-btn' );    ?>
        </div>  <?php
	...
	private function get_tab_content_article_preview_settings() {
		...
		<div class="ephd-wp__options-container ephd-wp__options-container--preview-settings">   <?php
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				...
				'tooltip_body'      => esc_html__( 'Search results can list matching articles. Choose how the article preview is displayed within the Help Dialog widget.', 'help-dialog' ),
			] );
        ...
        <div class="ephd-wp__options-container ephd-wp__options-container--global-settings">   <?php
            if ( $this->pro_enabled ) {
	            EPHD_HTML_Elements::radio_buttons_horizontal( [
		            ...
		            'tooltip_body'      => esc_html__( 'The Help Dialog opens when a user clicks on the launch icon. Choose to show an icon or both an icon and text.', 'help-dialog' ) . '<a href="https://www.helpdialog.com/documentation/configure-widgets/#articleTOC_6" target="_blank">' . esc_html__( 'Learn More.', 'help-dialog' ) . '</a>',
	            ] );
            }
	        ...
	        EPHD_HTML_Elements::radio_buttons_horizontal( [
                    'value'             => $widget['dialog_initial_visibility'],
                    'specs'             => 'dialog_initial_visibility',
	...
	private function get_tab_content_initial_message( $widget ) {
		...
		<div class="ephd-wp__options-container ephd-wp__options-container--initial-message">   <?php
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				...
				'input_group_class' => 'ephd-radio-horizontal-button-group-container ephd-admin__input-update_preview',
				'tooltip_body'      => esc_html__( 'Initial Message is displayed above the Help Dialog Launcher.', 'help-dialog' ),
			] );
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				'value'             => $widget['initial_message_mode'],
				'specs'             => 'initial_message_mode',
				'tooltip_body'      => esc_html__( 'Choose if to display an icon near the initial message', 'help-dialog' ),
	...
	private function get_tab_content_global_dialog_settings( $widget ) {
		...
		$widget['tabs_position_1'] = ! $this->chat_available && $widget['tabs_position_1'] == 'chat' ? 'none' : $widget['tabs_position_1'];
		$widget['tabs_position_2'] = ! $this->chat_available && $widget['tabs_position_2'] == 'chat' ? 'none' : $widget['tabs_position_2'];
		$widget['tabs_position_3'] = ! $this->chat_available && $widget['tabs_position_3'] == 'chat' ? 'none' : $widget['tabs_position_3'];
		$widget['tabs_position_4'] = ! $this->chat_available && $widget['tabs_position_4'] == 'chat' ? 'none' : $widget['tabs_position_4'];
		...
		<div class="ephd-wp__options-container ephd-wp__options-container--global-settings">    <?php
			...
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				...
				'tooltip_body'      => esc_html__( 'Powered By promotion to help others discover Help Dialog.', 'help-dialog' ),
				'input_group_class' => 'ephd-admin__input-update_preview'
			] );
            EPHD_HTML_Elements::text( [
                'value'             => $this->global_config['mobile_break_point'],
	            'specs'             => 'mobile_break_point', 'input_size' => 'small',
	            'tooltip_body'      => esc_html__( 'This value is the screen size. When the screen becomes this small, the Dialog will become the full width of the page, making it more usable on small screens.', 'help-dialog' )
            ] );
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				'value'             => $widget['tabs_position_1'],
				'specs'             => 'tabs_position_1',
				'options_exclude'	=> $this->chat_available ? [] : ['chat'],
				'input_group_class' => 'ephd-admin__input-update_preview ephd-input-group--separator-before'
			] );
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				'value' => $widget['tabs_position_2'],
				'specs' => 'tabs_position_2',
				'options_exclude' => $this->chat_available ? [] : ['chat'],
				'input_group_class' => 'ephd-admin__input-update_preview ephd-input-group--separator-before'
			] );
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				'value'             => $widget['tabs_position_3'],
				'specs'             => 'tabs_position_3',
				'options_exclude'	=> $this->chat_available ? [] : ['chat'],
				'input_group_class' => 'ephd-admin__input-update_preview ephd-input-group--separator-before'
			] );
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				'value'             => $widget['tabs_position_4'],
				'specs'             => 'tabs_position_4',
				'tooltip_body'      => esc_html__( 'AI Chat tab for Help Dialog PRO is coming soon.', 'help-dialog' ),  // TODO update when AI Chat is available
				'options_exclude'	=> $this->chat_available ? [] : ['chat'],
				'input_group_class' => 'ephd-admin__input-update_preview ephd-input-group--separator-before'
			] );  ?>
		</div>  <?php
	...
	private function get_tab_content_labels_launcher( $widget ) {
		...
		<div class="ephd-wp__options-container ephd-wp__options-container--labels ephd-admin__input-update_preview">    <?php
			echo EPHD_HTML_Elements::text( ['value' => $widget['launcher_text'], 'specs' => 'launcher_text'] ); ?>
		</div>  <?php
	...
	private function get_tab_content_labels_faqs( $widget ) {
        ...
        <div class="ephd-wp__options-container ephd-wp__options-container--labels ephd-admin__input-update_preview">    <?php
	        ...
	        EPHD_HTML_Elements::text( ['value' => $widget['welcome_title'], 'specs' => 'welcome_title', 'tooltip_body' => esc_html__( 'Allowed HTML tags: a, strong, i, b.', 'help-dialog' ),] );
	        EPHD_HTML_Elements::textarea( ['value' => $widget['welcome_text'], 'specs' => 'welcome_text', 'tooltip_body' => esc_html__( 'Allowed HTML tags: a, strong, i, b.', 'help-dialog' ),] );
            ...
            <div class="ephd-input-group ephd-admin__text-field ">
			    ...
			    <div class="input_container ">
                    <a href="<?php echo esc_url( admin_url( 'admin.php?page=ephd-help-dialog-widgets&ephd_fe_option=no-results-found-content-html&widget_id=' ) . $widget['widget_id'] ) ?>" target="_blank">
                        <?php echo esc_html__( 'Edit', 'help-dialog' ); ?>
                    </a>
			    </div>
		    </div>
        </div>  <?php
    ...
    private function get_tab_content_labels_contact( $widget ) {
        ...
        <div class="ephd-wp__options-container ephd-wp__options-container--labels ephd-admin__input-update_preview">    <?php
            ...
            EPHD_HTML_Elements::text( ['value' => $widget['contact_welcome_title'], 'specs' => 'contact_welcome_title', 'tooltip_body' => esc_html__( 'Allowed HTML tags: a, strong, i, b.', 'help-dialog' ),] );
	        EPHD_HTML_Elements::textarea( ['value' => $widget['contact_welcome_text'], 'specs' => 'contact_welcome_text', 'tooltip_body' => esc_html__( 'Allowed HTML tags: a, strong, i, b.', 'help-dialog' ),] );
	        ...
	        EPHD_HTML_Elements::textarea( [
				...
				'tooltip_body' => esc_html__( 'Allowed HTML tags: a, strong, i, b.', 'help-dialog' ),
		        'input_group_class' => 'ephd-input-group--separator-after',
			] );
	 ...
	 * @return false|string
	 */
	private function get_tab_content_labels_resource( $widget ) {
		ob_start(); ?>

		<!-- Contact Form Labels Settings Form -->
		<div class="ephd-wp__options-container ephd-wp__options-container--labels ephd-admin__input-update_preview">    <?php
			EPHD_HTML_Elements::text( ['value' => $widget['resource_header_top_tab'], 'specs' => 'resource_header_top_tab'] );
			EPHD_HTML_Elements::text( ['value' => $widget['resource_header_title'], 'specs' => 'resource_header_title','tooltip_body' => esc_html__( 'Allowed HTML tags: a, strong, i, b.', 'help-dialog' ),] );
			EPHD_HTML_Elements::textarea( ['value' => $widget['resource_header_sub_title'], 'specs' => 'resource_header_sub_title','tooltip_body' => esc_html__( 'Allowed HTML tags: a, strong, i, b.', 'help-dialog' ),] );
			EPHD_HTML_Elements::textarea( ['value' => $widget['resource_welcome_text'], 'specs' => 'resource_welcome_text'] );
			EPHD_HTML_Elements::text( ['value' => $widget['resource_phone_label'], 'specs' => 'resource_phone_label'] );
			EPHD_HTML_Elements::text( ['value' => $widget['resource_custom_link_label'], 'specs' => 'resource_custom_link_label'] );  ?>
		</div>  <?php
	...
	private function get_tab_content_colors_chat( $widget ) {
		...
		<div class="ephd-wp__options-container ephd-wp__options-container--labels">    <?php
			EPHD_HTML_Elements::color( ['value' => $widget['resource_phone_color'], 'specs' => 'resource_phone_color'] );
			EPHD_HTML_Elements::color( ['value' => $widget['resource_phone_hover_color'], 'specs' => 'resource_phone_hover_color'] );
			EPHD_HTML_Elements::color( ['value' => $widget['resource_link_color'], 'specs' => 'resource_link_color'] );
			EPHD_HTML_Elements::color( ['value' => $widget['resource_link_hover_color'], 'specs' => 'resource_link_hover_color'] );
			EPHD_HTML_Elements::color( ['value' => $widget['resource_label_color'], 'specs' => 'resource_label_color'] );  ?>
		</div>  <?php
    ...
    /**
     * Display Tabs Navigation for Widget Settings Box
     * @param $widget_config
     *
     * @return false|string
     */
    public function get_widget__preview_box_tab_navigation( $widget_config, $widget_index = 0 ) {

        ob_start(); ?>

		<div class="ephd-wp__widgets-tab-container">

			<div class="ephd-wp__widgets-tabs">
				<div class="ephd-wp__widget-tabs__wrapper"><?php

					$i = $widget_index;
					foreach (  $widget_config as $widget ) {
						$widget_name = strpos( $widget['widget_name'], 'Widget #' ) === false ? $widget['widget_name'] : esc_html__( 'Widget Name', 'help-dialog' ); ?>
						<div class="ephd-wp__widget-tab__item ephd-wp__widget-tab--<?php echo esc_attr( $widget['widget_id'] . ( ( $i++ == $widget_index ) ? ' ephd-wp__widget-tab--active' : '' ) ); ?>">
							<input type="button" value="<?php echo esc_attr( $widget_name ); ?>" class="ephd_edit_widget" data-id="<?php echo esc_attr( $widget['widget_id'] ); ?>">
							<div class="ephd-wp__widget-tab__item_count"><?php echo esc_html__( 'Widget', 'help-dialog' ) . ' #' . $i; ?></div>
						</div>  <?php
                    } ?>

				</div>
				<input type="button" value="<?php esc_attr_e( 'Copy Active Widget', 'help-dialog' ); ?>" class="ephd_copy_widget ephd-primary-btn">
			</div>
		</div>  <?php
		...
		<div class="ephd-wp__tiny-mce-input__header">
			<div class="ephd-wp__tiny-mce-input__title-wrap">
				...
				<div class="ephd-wp__tiny-mce-input__actions">  <?php
					EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Save', 'help-dialog' ), 'ephd_tiny_mce_input_save', 'ephd-wp__tiny-mce-input__save-btn', '', false, '', 'ephd-success-btn' );  ?>
				</div>
			</div>
		</div>
	...
	private static function get_widget_form_actions_html( $widget ) {
        ...
        <div class="ephd-wp__widget-action__save-wrap">
            <div class="ephd-wp__widget-action__save-btns-wrap"> <?php
					EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Save', 'help-dialog' ), 'ephd_publish_widget', 'ephd-wp__widget-action__publish-btn', '', false, '', 'ephd-success-btn' );   ?>
            </div>
        </div>  <?php
	...
	private function get_tab_content_contact_form_settings( $widget ) {
		...
		<div class="ephd-wp__options-container ephd-wp__options-container--contact-form-settings">    <?php
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				...
				'tooltip_body'  => esc_html__( 'Add name input to Contact Form', 'help-dialog' ),
				'input_group_class' => 'ephd-admin__input-update_preview'
			] );
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				'specs'         => 'contact_subject_toggle',
				'value'         => $widget['contact_subject_toggle'],
				'tooltip_body'  => esc_html__( 'Add subject input to Contact Form', 'help-dialog' ),
				'input_group_class' => 'ephd-admin__input-update_preview'
			] );
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				'value'        => $widget['contact_acceptance_checkbox'],
				'specs'        => 'contact_acceptance_checkbox',
				'tooltip_body' => esc_html__( 'Add acceptance checkbox to Contact Form', 'help-dialog' ),
				'input_group_class' => 'ephd-admin__input-update_preview'
			] );
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				'value'        => $widget['contact_acceptance_title_toggle'],
				'specs'        => 'contact_acceptance_title_toggle',
				'tooltip_body' => esc_html__( 'Add acceptance title to Contact Form', 'help-dialog' ),
				'input_group_class' => 'ephd-admin__input-update_preview'
			] );    ?>
		</div>  <?php
	...
	/**
	 * Return HTML for Resource settings
	 *
	 * @param $widget
	 * @return false|string
	 */
	private function get_tab_content_resource_settings( $widget ) {

		ob_start(); ?>
		<!-- Chat Settings Form -->
		<div class="ephd-wp__options-container ephd-wp__options-container--chat-settings">    <?php
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				'value'             => $widget['resource_phone_toggle'],
				'specs'             => 'resource_phone_toggle',
				'input_group_class' => 'ephd-admin__input-update_preview'
			] );

			EPHD_HTML_Elements::text( [
				'value'         => $widget['resource_phone_country_code'],
				'specs'         => 'resource_phone_country_code',
				'tooltip_body'  => esc_html__( 'Look up your country code:', 'help-dialog' ) . ' <a href="https://countrycode.org" target="_blank">' . esc_html__( 'Learn More.', 'help-dialog' ) . '</a>',
				'input_size'    => 'small'
			] );

			EPHD_HTML_Elements::text( [
				'value'         => $widget['resource_phone_number'],
				'specs'         => 'resource_phone_number',
				'input_size'    => 'small',
			] );

            EPHD_HTML_Elements::text( [
                'value'             => $widget['resource_phone_number_image_url'],
                'specs'             => 'resource_phone_number_image_url',
                'input_group_class' => 'ephd-admin__input-update_preview ephd-input-group--separator-after'
            ] );

			EPHD_HTML_Elements::radio_buttons_horizontal( [
				'value'             => $widget['resource_custom_link_toggle'],
				'specs'             => 'resource_custom_link_toggle',
				'input_group_class' => 'ephd-admin__input-update_preview'
			] );

			EPHD_HTML_Elements::text( [
				'value'         => $widget['resource_custom_link_url'],
				'specs'         => 'resource_custom_link_url',
				'input_size'    => 'small'
			] );

            EPHD_HTML_Elements::text( [
                'value'             => $widget['resource_custom_link_image_url'],
                'specs'             => 'resource_custom_link_image_url',
                'input_group_class' => 'ephd-admin__input-update_preview'
            ] );
			?>
		</div>  <?php
	...
	private function get_tab_content_trigger_delay( $widget ) {
		...
		<div class="ephd-wp__options-container ephd-wp__options-container--trigger-delay-settings">    <?php
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				...
				'tooltip_body'  => esc_html__( 'Enable or disable the Delay Trigger. After the widget has appeared for the first time, it will always be visible on load.', 'help-dialog' )
			] );
			EPHD_HTML_Elements::text( [
	...
	private function get_tab_content_trigger_scroll( $widget ) {
		...
		<div class="ephd-wp__options-container ephd-wp__options-container--trigger-scroll-settings">    <?php
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				...
				'tooltip_body'  => esc_html__( 'Enable or disable the Scroll Trigger. After the user scrolls a specified percentage of the page length for the first time, the Help Dialog widget will appear. ' .
												'This is useful in helping the user if they are unable to find what they need while scrolling.', 'help-dialog' )
			] );
			EPHD_HTML_Elements::text( [
	...
	private function get_tab_content_trigger_days_and_hours( $widget ) {
		...
		<div class="ephd-wp__options-container ephd-wp__options-container--trigger-scroll-settings">    <?php
			EPHD_HTML_Elements::radio_buttons_horizontal( [
				...
				'tooltip_body'  => esc_html__( 'Enable or disable the Days and Hours Trigger. After the widget appeared for the first time, it will always be visible on-load - once the user is aware of the widget, the user expects it to always appear.', 'help-dialog' )
			] );
			EPHD_HTML_Elements::dropdown( [
