...
class EPHD_HTML_Forms {
	...
	public static function notification_box_middle( array $args = array(), $return_html=false ) {
		...
		<div <?php echo isset( $args['id'] ) ? 'id="' . esc_attr( $args['id'] ) . '"' : ''; ?> class="ephd-notification-box-middle <?php echo 'ephd-notification-box-middle--' . esc_attr( $box_type ); ?>">
			...
			<div class="ephd-notification-box-middle__body">                <?php
				...
				if ( ! empty( $args['html'] ) ) {
                    echo wp_kses( $args['html'], EPHD_Utilities::get_admin_ui_extended_html_tags() );
				}   ?>
			</div>
	...
	public static function dialog_pro_feature_ad( $args ) {
		...
		$class = empty( $args['img_list'] ) ? 'ephd-dialog-pro-feature-ad' : 'ephd-dialog-pro-feature-ad2';		?>
		...
		<div class="ephd-dialog-pro-feature-ad-black-background"></div>		<?php
	}
	...
	public static function advertisement_ad_box( $args ) {
		...
		<div<?php echo empty( $args['id'] ) ? '' : ' id="' . esc_attr( $args['id'] ) . '"'; ?> class="ephd-admin-ad-container <?php echo esc_attr( $args['class'] ); ?>">
			...
			<div class="ephd-admin-ad-ribbon">PRO</div>
	...
	public static function pro_feature_ad_box( $args ) {
		...
		<div<?php echo empty( $args['id'] ) ? '' : ' id="' . esc_attr( $args['id'] ) . '"'; ?> class="ephd-admin-pro-feature-ad-container <?php echo esc_attr( $args['class'] ); ?>">

			<div class="ephd-admin-ad-icon">
				<img src="<?php echo esc_url( Echo_Help_Dialog::$plugin_url . 'img/ad/' . 'hd-pro-icon.png' ); ?>">
			</div>

			<!----- Header ----->			<?php
			if ( !empty( $args['title'] ) ) { ?>
				<div class="ephd-aa__header-container">
					<div class="ephd-header__title"><?php echo wp_kses( $args['title'] , $allowed_tags ); ?></div>
				</div>			<?php
			} ?>

			<!----- Body ------->
			<div class="ephd-aa__body-container">				<?php

				if ( !empty( $args['desc'] ) ) { ?>
					<p class="ephd-body__desc"><?php echo esc_html( $args['desc'] ); ?></p>				<?php
				} ?>
			...
			<div class="ephd-aa__footer-container">				<?php
				if ( !empty( $args['footer_desc'] ) ) { ?>
					<p class="ephd-body__footer_desc"><?php echo esc_html( $args['footer_desc'] ); ?></p>				<?php
				}

				if ( $args['btn_text'] ) { ?>
					<a href="<?php echo esc_url( $args['btn_url'] ); ?>" class="ephd-btn" target="_blank" ><?php echo esc_html( $args['btn_text'] ); ?></a>				<?php
				} ?>

			</div>

			<img src="<?php echo esc_url( Echo_Help_Dialog::$plugin_url . 'img/ad/' . 'hd-pro-background.jpg' ); ?>" class="ephd-admin-pro-feature-ad-background-img" alt="">
		</div>	<?php
	...
	public static function pro_feature_ad_box_with_images( $args ) {
		...
		<div<?php echo empty( $args['id'] ) ? '' : ' id="' . esc_attr( $args['id'] ) . '"'; ?> class="ephd-admin-pro-feature-ad2-container <?php echo esc_attr( $args['class'] ); ?>">

			<!----- Header ----->
			<div class="ephd-aa__header-container">				<?php

				if ( ! empty( $args['title'] ) ) { ?>
					<div class="ephd-feature-header__title">
			...
			<div class="ephd-aa__body-container">
				...
				<div class="ephd-feature-list-container">					<?php

					if ( ! empty( $args['img_list'] ) ) {
						foreach ( $args['img_list'] as $index => $item ) { ?>

							<div class="ephd-feature-container <?php echo $index === 0 ? 'ephd-feature--active' : ''; ?>">

								<div class="ephd-feature-previous ephdfa ep_font_icon_arrow_carrot_left"></div>
								<div class="ephd-feature-next ep_font_icon_arrow_carrot_right"></div>

								<!-- Image -->
								<div class="ephd-feature-img">    <?php
									echo '<img class="ephd-featured-img-url" src="' . esc_html( $item[3] ) . '" >'; ?>
								</div>

								<div class="ephd-feature-footer">

									<!-- Description -->
									<div class="ephd-feature-footer__desc">
										<p><?php echo esc_html( $item[1] ); ?></p>
									</div>
									<!-- Button -->
									<div class="ephd-feature-footer__btn-container">										<?php
										echo '<a href="'.esc_url( $item[2] ).'" class="ephd-view-demo-btn" target="_blank">' . esc_html( $item[0] ) . '</a>';

										if ( $args['btn_text'] ) { ?>
											<a href="<?php echo esc_url( $args['btn_url'] ); ?>" class="ephd-buy-btn" target="_blank" ><?php echo esc_html( $args['btn_text'] ); ?></a>				<?php
										} ?>
									</div>
								</div>
	...
	public static function admin_settings_box( $box_options ) {
		...
		<!-- Admin Box -->
		<div class="ephd-admin__boxes-list__box <?php echo esc_attr( $box_options['class'] ); ?>">  <?php
			...
			<div class="ephd-admin__boxes-list__box__body">   <?php
				...
				$admin_ui_escaped = EPHD_Utilities::admin_ui_wp_kses( $box_options['html'], $box_options['extra_tags'] ); ?>
				<div class="ephd-admin__boxes-list__box__content"><?php echo $admin_ui_escaped;//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
	...
	public static function get_feature_box_html( $feature ) {
		...
		<div class="ephd-kbnh__feature-container__col ephd-kbnh__feature__content-col">
			<h3 class="ephd-kbnh__feature-name <?php echo esc_attr( $feature['title_class'] ); ?>"><?php echo esc_html( $feature['title'] ); ?> <?php
				if ( ! empty( $feature['experimental'] ) ) {   ?>
					<div class="ephd-kbnh__feature-experimental"><?php
					esc_html_e( 'Experimental', 'help-dialog' );
					EPHD_HTML_Elements::display_tooltip( '', $feature['experimental'], ['link_text' => '', ]); ?>
					</div><?php
				} ?>
			</h3> <?php
			...
			if ( ! empty( $feature['desc'] ) ) { ?>
				<div class="ephd-kbnh__feature-desc"><?php echo esc_html( $feature['desc'] ) . $feature['desc_escaped']; ?></div><?php
			}
			...
			<div class="ephd-kbnh__feature-links">  <?php
				...
				if ( ! empty( $feature['button_id'] ) && ! empty( $feature['button_title'] ) ) { ?>
					<button class="ephd-primary-btn" id="<?php echo esc_attr( $feature['button_id'] ); ?>" type="button"><?php echo esc_html( $feature['button_title'] ); ?></button> <?php
				}
		...
		<div class="ephd-kbnh__feature-container__col ephd-kbnh__feature__status-col">    <?php
			...
			// Plugin is not enabled
				...
				echo '<a class="ephd-kbnh__feature-status ephd-kbnh__feature--disabled ephd-success-btn" href="' . esc_url( EPHD_Core_Utilities::get_plugin_sales_page( $feature['plugin'] ) ) . '" target="_blank"><span>' . esc_html__( 'Install Now', 'help-dialog' ) . '</span></a>';
			} ?>

		</div>  <?php

		if ( ! empty( $feature['corner_label'] ) ) { ?>
			<div class="ephd-kbnh__feature-corner__label"><?php echo esc_html( $feature['corner_label'] ); ?></div><?php
		}
		...
		if ( $items_left > 0 ) {    ?>
			...
			<div class="ephd-admin__items-list__more-items-message">
				<form>
					...
					EPHD_HTML_Elements::submit_button_v2( esc_html__( 'See More', 'help-dialog' ), $load_more_action, 'ephd-admin__items-list__more-items-message__button', '', false );   ?>
				</form>
			</div>  <?php
		}
