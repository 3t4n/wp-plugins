...
class EPHD_Analytics_DB extends EPHD_DB {
	...
	const EVENTS_META = array(
		...
		'contact-form-submitted' => array(
			'type_id'     => 7,
			'description' => 'Contact Form Submitted Count',
			'aggregation' => 'monday this week', // 1 Week
			'expiration'  => 'first day of this month - 2 months', // 3 Months
			'object'      => 'page',
		),
	 ...
	 * @return int|null
	 */
	public function get_total_invocations_count( $from, $to ) {
		return $this->get_invocations_count( $from, $to );
	}
	 ...
	 * @return int|null
	 */
	public function get_total_impressions_count( $from, $to ) {
		return $this->get_impressions_count( $from, $to );
	}
	 ...
	 * @return int|null
	 */
	public function get_total_contact_opened_count( $from, $to ) {
		return $this->get_contact_opened_count( $from, $to );
	}

	/**
	 *  Get total contact form submitted count
	 *
	 * @return int|null
	 */
	public function get_total_form_submitted_count( $from, $to ) {
		return $this->get_contact_submitted_count( $from, $to );
	}
	...
	/**
	 *  Get contact form submitteds count By Date
	 *
	 * @param $date_from
	 * @param $date_to
	 * @param $where_data
	 ...
	 * @return int|null
	 */
	public function get_contact_submitted_count( $date_from, $date_to, $where_data=[] ) {
		$result = $this->get_event_sum_by_where_clause( 'contact-form-submitted', 'other_2', $date_from, $date_to, $where_data );
		if ( $result === null || is_wp_error( $result ) ) {
			return null;
		}
		return (int)$result;
	...
	public function get_contact_open_by_pages( $date_from, $date_to ) {
		...
		$result = $this->get_event_grouped_sum_by_where_clause( 'get_contact_open_by_pages', 'contact-tab-opened', $date_from, $date_to );
	...
	public function get_launcher_open_by_pages( $date_from, $date_to ) {
		$result = $this->get_event_grouped_sum_by_where_clause( 'get_launcher_open_by_pages', 'launcher-open', $date_from, $date_to );
		if ( empty( $result ) || is_wp_error( $result ) ) {
			return null;
		}
		return $result;
	...
	public function get_impressions_by_pages( $date_from, $date_to ) {
		$result = $this->get_event_grouped_sum_by_where_clause( 'get_impressions_by_pages', 'impressions', $date_from, $date_to );
		if ( empty( $result ) || is_wp_error( $result ) ) {
			return null;
		}
		return $result;
	...
	public function get_faq_click_by_pages( $date_from, $date_to ) {
		...
		$result = $this->get_event_grouped_sum_by_where_clause( 'get_faq_click_by_pages', 'faq-item-click', $date_from, $date_to );
		if ( empty( $result ) || is_wp_error( $result ) ) {
			return null;
		}
		return $result;
	...
	public function get_faq_group_click_by_pages( $date_from, $date_to ) {
		$result = $this->get_event_grouped_sum_by_where_clause( 'get_faq_group_click_by_pages', 'faq-group-click', $date_from, $date_to );
		if ( empty( $result ) || is_wp_error( $result ) ) {
			return null;
		}
		return $result;
	...
	public function get_article_click_by_pages( $date_from, $date_to ) {
		$result = $this->get_event_grouped_sum_by_where_clause( 'get_article_click_by_pages', 'article-item-click', $date_from, $date_to );
		if ( empty( $result ) || is_wp_error( $result ) ) {
			return null;
		}
		return $result;
	...
	public function get_click_by_faqs( $date_from, $date_to, $order, $location='' ) {
		...
		// click_1 - main FAQs list, click_2 - search results FAQs list
		$result = $this->get_event_grouped_sum_by_where_clause( 'get_click_by_faqs', 'faq-item-click', $date_from, $date_to, $where_data, $order );
		if ( empty( $result ) || is_wp_error( $result ) ) {
			return null;
		}
	...
	public function get_click_by_articles( $date_from, $date_to, $order, $location='' ) {
		...
		$result = $this->get_event_grouped_sum_by_where_clause( 'get_click_by_articles', 'article-item-click', $date_from, $date_to, $where_data, $order );
		if ( empty( $result ) || is_wp_error( $result ) ) {
			return null;
		}
	...
	public function get_custom_table_count( $table, $date_from, $date_to, $where_data=[] ) {
		...
		if ( ! empty( $where_data ) ) {
			$where_clause_escaped = $this->get_where_clause( $where_data );
			if ( empty( $where_clause_escaped ) ) {
				return null;
			}
			$where_clause_escaped .= ' AND ';
		}
		$where_clause_escaped .= " DATE(submit_date) BETWEEN %s AND %s ";

		$result = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}ephd_{$table} WHERE {$where_clause_escaped}", $date_from, $date_to ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	 ...
	 */
	public function get_event_sum_by_where_clause( $event_name, $column_name, $date_from, $date_to, $where_data=[] ) {
		global $wpdb;
		...
		$select_statement_escaped = " SUM(" . esc_sql( $column_name ) . ") AS times ";
		...
		$where_clause_escaped .= " event_type_id = {$column_format['event_type_id']} AND DATE(submit_date) BETWEEN %s AND %s ";

		$result = $wpdb->get_var( $wpdb->prepare( "SELECT {$select_statement_escaped} FROM {$this->table_name} WHERE {$where_clause_escaped}", $event_meta['type_id'], $date_from, $date_to ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		if ( $result === null && ! empty($wpdb->last_error) ) {
	 ...
	 */
	public function get_event_grouped_sum_by_where_clause( $query_name, $event_name, $date_from, $date_to, $where_data=[], $order='DESC', $limit=100 ) {
		global $wpdb;

		switch( $query_name ) {
			case 'get_click_by_articles':
				$select_escaped = ' object_id as obj_id, SUM(click_1) AS times ';
				$group_by_escaped = ' object_id ';
				break;
			case 'get_click_by_faqs':
				$select_escaped = " object_id as obj_id, SUM(click_1) + SUM(click_2) AS times ";
				$group_by_escaped = ' object_id ';
				break;
			case 'get_faq_click_by_pages':
				$select_escaped = " page_id as obj_id, SUM(click_1) + SUM(click_2) AS times ";
				$group_by_escaped = ' page_id ';
				break;
			case 'get_impressions_by_pages':
				$select_escaped = ' page_id as obj_id, SUM(view) AS times ';
				$group_by_escaped = ' page_id ';
				break;
			case 'get_contact_open_by_pages':
			case 'get_article_click_by_pages':
			case 'get_faq_group_click_by_pages':
			case 'get_launcher_open_by_pages':
			default:
				$select_escaped = " page_id as obj_id, SUM(click_1) AS times ";
				$group_by_escaped = ' page_id ';
				break;
		}
		...
		$format = empty( $column_formats['event_type_id'] ) ? '%s' : $column_formats['event_type_id'];
		...
		$where_clause_escaped .= " event_type_id = {$format_escaped} AND DATE(submit_date) BETWEEN %s AND %s ";

		$result = $wpdb->get_results( $wpdb->prepare( "SELECT {$select_escaped} FROM {$this->table_name} WHERE {$where_clause_escaped} GROUP BY {$group_by_escaped} ORDER BY times {$order} LIMIT {$limit}", $event_meta['type_id'], $date_from, $date_to ), OBJECT_K );// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public function get_custom_table_grouped_rows_by_where_clause( $table, $group_by, $group_by_name, $date_from, $date_to, $sum_column='', $where_data=[], $limit=100 ) {
		...
		$result = $wpdb->get_results( $wpdb->prepare( "SELECT {$group_by} AS {$group_by_name}, count(*) AS times {$select_sum_statement} FROM {$wpdb->prefix}ephd_{$table} " .
		                                              "WHERE {$where_clause_escaped} GROUP BY {$group_by_name} ORDER BY times DESC, {$group_by_name} ASC LIMIT {$limit}", $date_from, $date_to ), OBJECT_K );
		if ( ! empty( $wpdb->last_error ) ) {
			$wpdb_last_error = $wpdb->last_error;
			EPHD_Logging::add_log("DB failure: ", $wpdb_last_error);
			return new WP_Error('DB failure', $wpdb_last_error);
	...
	public static function get_analytics_column_fields() {
		return [
			'event_id'      => esc_html__( 'Event ID', 'help-dialog' ),
			'submit_date'   => esc_html__( 'Event Date', 'help-dialog' ),
			'widget_id'     => esc_html__( 'Widget ID', 'help-dialog' ),
			'page_id'       => esc_html__( 'Page ID', 'help-dialog' ),
			'object_id'     => esc_html__( 'Object ID', 'help-dialog' ),
			'event_type_id' => esc_html__( 'Event Type ID', 'help-dialog' ),
			'view'          => esc_html__( 'Event Value 1', 'help-dialog' ),
			'click_1'       => esc_html__( 'Event Value 2', 'help-dialog' ),
			'click_2'       => esc_html__( 'Event Value 3', 'help-dialog' ),
			'other_1'       => esc_html__( 'Event Value 4', 'help-dialog' ),
			'other_2'       => esc_html__( 'Event Value 5', 'help-dialog' )
		];
	}
