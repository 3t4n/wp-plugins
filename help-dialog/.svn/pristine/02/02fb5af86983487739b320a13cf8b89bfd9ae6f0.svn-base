...
class EPHD_HTML_Admin {
		...
		<div class="ephd-admin__header">
			<div class="ephd-admin__section-wrap ephd-admin__section-wrap__header">   <?php
				...
				switch ( $content_type ) {
					case 'header':
					default:
						echo self::admin_header_content( $args );//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						break;
					case 'logo':
	...
	private static function admin_settings_display_boxes_list( $page_view ) {
		...
		if ( ! empty( $page_view['list_top_actions_html'] ) && current_user_can( $page_view['top_actions_minimum_required_capability'] ) ) {
			$top_actions_escaped = $page_view['list_top_actions_html'];
			echo $top_actions_escaped;//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		...
		<div class="ephd-admin__header__logo-wrap">
			<div class="ephd__desc__logo">
				<span class="ephd__desc__logo__icon">   <?php
					echo self::get_hd_icon_html();//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped    ?>
				</span>
			</div>
	...
	public static function nonce( $return_html=false ) {
		...
		<input type="hidden" name="_wpnonce_ephd_ajax_action" value="<?php echo wp_create_nonce( '_wpnonce_ephd_ajax_action' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped  ?>">	<?php
				...
				foreach ( $group_config['tabs'] as $tab ) {  ?>
					<div class="ephd-admin__form-tab-wrap ephd-admin__form-tab-wrap--<?php echo esc_attr( $tab['key'] ); echo $tab['active'] ? ' ephd-admin__form-tab-wrap--active' : ''; ?>">  <?php
						...
						foreach ( $tab['contents'] as $content ) {	?>
							...
							<div class="ephd-admin__form-tab-content"<?php echo empty( $content['tab_id'] ) ? '' : ' id="' . esc_attr( $content['tab_id'] ) . '"'; ?>>
	 ...
	 */
	public static function display_config_error_page( $wp_error, $error_title= '', $error_html = '' ) {

		$wp_error = $wp_error->get_error_message();
		$title    = empty( $error_title ) ? esc_html__( 'Cannot load configuration.', 'help-dialog' ) : $error_title;
		$html     = empty( $error_html ) ? ( EPHD_Utilities::is_user_admin()
			...
			: EPHD_Utilities::contact_us_for_support() ) : $error_html;
        ?>
		<div id="ephd-admin-page-wrap" class="ephd-admin-page-wrap--config-error">  <?php
			...
			EPHD_HTML_Forms::notification_box_middle( [
				...
				'type'  => 'error',
				'title' => $title . ( empty( $wp_error ) ? '' : ' (' . $wp_error . ')' ),
				'html'  => $html
			] );    ?>
	...
	public static function display_admin_form_header( $header_config ) {
		$status_text = $header_config['status_text'] ?? esc_html__( 'Status: ', 'help-dialog' );
		$status_body_text = $header_config['status_text_body'] ?? esc_html__( 'Status: ', 'help-dialog' );    ?>

		<!-- Admin Form Header -->
		<div class="ephd-admin__form__header">
			<div class="ephd-admin__form-title">
				<div class="ephd-admin__form-text-container <?php echo esc_attr( ( $header_config['status'] == 'draft' ) ? ' ephd-admin__form-text-container--exclamation' : '' ); ?>">
					<?php EPHD_HTML_Elements::radio_buttons_horizontal( [
						'value'         => $header_config['status'],
						'specs'         => 'widget_status',
                        'label'         => $status_text,
                        'tooltip_body'  => $status_body_text,
					] ); ?>
				</div>
			</div><?php
			if ( !empty( $header_config['preview_url'] ) ) { ?>
				<div class="ephd-admin__frontend-link">
					<a href="<?php echo esc_url( $header_config['preview_url'] ); ?>" target="_blank">
						<?php   esc_html_e( ( $header_config['status'] == 'draft' ) ? 'Preview on frontend' : 'View on frontend', 'help-dialog' ); ?>
						<span class="ephdfa ephdfa-external-link"></span>
					</a>
				</div><?php
			} ?>
