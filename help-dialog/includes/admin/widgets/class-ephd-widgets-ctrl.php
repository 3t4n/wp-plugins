...
class EPHD_Widgets_Ctrl {
	...
	public function create_widget() {
		...
		$widget_preview_config = array(
            'class' => 'ephd-wp__widgets-tabs',
            'return_html' => true,
            'html'  => $widgets_page_handler->get_widget__preview_box_tab_navigation( array(
                array(
                    'widget_id'   => $updated_widget['widget_id'],
                    'widget_name' => $updated_widget['widget_name'],
                )
            ), count( $updated_widgets_config ) - 1 ),
        );;

		// Set notification message
		$notification_message = esc_html__( 'Configuration Saved', 'help-dialog');
		if ( $widget['widget_status'] == 'published' ) {
			$notification_message = esc_html__( 'Configuration is saved and the Widget is published', 'help-dialog');
		}
		if ( $widget['widget_status'] == 'draft' ) {
			$notification_message = esc_html__( 'Configuration is saved and the Widget is set to Draft', 'help-dialog');
		}
	...
	public function load_widget_form() {
		...
		// or copy current widget
			...
			if ( empty( $widget_name ) ) {
				EPHD_Utilities::ajax_show_error_die( esc_html__( 'Widget name cannot be empty.', 'help-dialog' ) );
			}
			$widget['widget_name'] = $widget_name;

		// or create new Widget configuration
		} else {
		...
		// adjust new Widget settings (copied or newly created)
		if ( $widget['widget_id'] == 0 ) {
			...
			// set status to draft as it currently has empty locations
			$widget['widget_status'] = 'draft';
		}
	...
	private static function get_sanitized_widget_from_input() {
		...
		$widget['location_wildcard_url'] = EPHD_Utilities::post( 'location_wildcard_url' );
		...
		// retrieve dialog initial visibility
		$widget['dialog_initial_visibility'] = EPHD_Utilities::post( 'dialog_initial_visibility' );

		// retrieve Launcher Location
		$widget['launcher_location'] = EPHD_Utilities::post( 'launcher_location' );

		// retrieve Launcher Text - enabled only on HD PRO
		if ( EPHD_Utilities::is_help_dialog_pro_enabled() ) {
		...
		$widget['resource_welcome_text'] = EPHD_Utilities::post( 'resource_welcome_text' );
		...
		$widget['resource_phone_label'] = EPHD_Utilities::post( 'resource_phone_label' );
		$widget['resource_custom_link_label'] = EPHD_Utilities::post( 'resource_custom_link_label' );

		// Labels
		$widget['resource_header_top_tab'] = EPHD_Utilities::post( 'resource_header_top_tab' );
		$widget['resource_header_title'] = EPHD_Utilities::post( 'resource_header_title', '', 'wp_editor' );
		$widget['resource_header_sub_title'] = EPHD_Utilities::post( 'resource_header_sub_title', '', 'wp_editor' );
		...
		if ( empty( $colors_set ) ) {
			...
			$widget['resource_phone_color'] = EPHD_Utilities::post( 'resource_phone_color' );
			$widget['resource_phone_hover_color'] = EPHD_Utilities::post( 'resource_phone_hover_color' );
			$widget['resource_link_color'] = EPHD_Utilities::post( 'resource_link_color' );
			$widget['resource_link_hover_color'] = EPHD_Utilities::post( 'resource_link_hover_color' );

			$widget['resource_label_color'] = EPHD_Utilities::post( 'resource_label_color' );
		} else {
		...
		$widget['resource_phone_toggle'] = EPHD_Utilities::post( 'resource_phone_toggle' );
		$widget['resource_phone_country_code'] = EPHD_Utilities::post( 'resource_phone_country_code' );
		$widget['resource_phone_number'] = EPHD_Utilities::post( 'resource_phone_number' );

		$widget['resource_phone_number_image_url'] = EPHD_Utilities::post( 'resource_phone_number_image_url' );

		$widget['resource_custom_link_toggle'] = EPHD_Utilities::post( 'resource_custom_link_toggle' );
		$widget['resource_custom_link_url'] = EPHD_Utilities::post( 'resource_custom_link_url' );

		$widget['resource_custom_link_image_url'] = EPHD_Utilities::post( 'resource_custom_link_image_url' );

		$widget['article_back_button_text'] = EPHD_Utilities::post( 'article_back_button_text' );

		// retrieve Tabs Position
		$widget['tabs_position_1'] = EPHD_Utilities::post( 'tabs_position_1' );
		$widget['tabs_position_2'] = EPHD_Utilities::post( 'tabs_position_2' );
		$widget['tabs_position_3'] = EPHD_Utilities::post( 'tabs_position_3' );
		$widget['tabs_position_4'] = EPHD_Utilities::post( 'tabs_position_4' );
