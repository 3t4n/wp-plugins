...
class EPHD_Analytics_Page {
	...
	private function set_filters_options() {
		...
		$this->filters_options['timeframe'] = array(
			'last_30_days' => esc_html__( 'Last 30 days', 'help-dialog' ),
			'this_week'    => esc_html__( 'This Week', 'help-dialog' ),
			'last_week'    => esc_html__( 'Last Week', 'help-dialog' ),
			'this_month'   => esc_html__( 'This Month', 'help-dialog' ),
			'last_month'   => esc_html__( 'Last Month', 'help-dialog' ),
			'this_year'    => esc_html__( 'This Year', 'help-dialog' )
        );
	}
	...
	public function display_page() {
		...
		foreach ( $this->message as $class => $message ) {
			echo EPHD_HTML_Forms::notification_box_bottom( $message, '', $class );//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function
		}
	}
	...
	private static function settings_tab_excluded_user_roles_box( $excluded_roles ) {
		...
		EPHD_HTML_Elements::checkboxes_multi_select( array(
				'label'             => esc_html__( 'Exclude specific user roles from analytics', 'help-dialog' ),
	...
	private static function settings_tab_actions_row() {
		...
		<div class="ephd-admin__list-actions-row"><?php
			EPHD_HTML_Elements::submit_button_v2( esc_html__( 'Save Settings', 'help-dialog' ), 'ephd_hd_save_settings_btn', 'ephd__hdl__action__save_order', '', true, '', 'ephd-success-btn' );    ?>
		</div>      <?php
                ...
                if ( in_array( 'timeframe', $filter_boxes ) ) {
	                EPHD_HTML_Elements::dropdown(
		                array(
			                ...
			                'label'         => esc_html__( 'Timeframe', 'help-dialog' ),
			    ...
			    if ( in_array( 'location', $filter_boxes ) && ! empty( $this->filters_options['location'] ) ) {
                    EPHD_HTML_Elements::dropdown(
	                    array(
		                    ...
		                    'input_class'   => 'ephd_admin_analytics_filters',
		                    'tooltip_title' => esc_html__( 'Page Selection', 'help-dialog' ),
		                    'tooltip_body'  => esc_html__( 'Pages with low traffic might not have any analytics data recorded for the time interval chosen. ' .
		                    ...
		                    'label'         => esc_html__( 'Location', 'help-dialog' ),
		                    'options'       => array( '' => esc_html__( 'Select Page', 'help-dialog' ) ) + $this->filters_options['location'],
	                    )
                    );
			    }   ?>
        </form> <?php
	...
	private function get_analytics_location_link_form() {
		...
		if ( $location == EPHD_Config_Specs::HOME_PAGE ) {
            $title = esc_html__( 'Home Page', 'help-dialog' );
            $href = home_url();
		} else {
    ...
    private function get_location_filter_options() {
        ...
        foreach ( $locations_obj as $location_id => $location_data ) {
	        $locations[$location_id] = array(
		        'label' => $location_data->title . ' ' . ( empty( $location_data->times ) ? esc_html__( '(no data)', 'help-dialog' ) : '' ),
		...
		foreach ( $boxes_list_config as $box ) {
			...
			switch ( $box['type'] ) {
				...
				case 'multiple-col-2' :
					...
					break;
				case 'multiple-col-2-core' :
                    $html = self::get_core_multiple_results_html( $box );
                    break;
				default :
					$html = '';
					break;
	...
	private function get_analytics_tab_boxes_list_config() {
		...
		$total_invocations    = $analytics_db->get_total_invocations_count( $this->filters['timeframe_from'], $this->filters['timeframe_to'] );
		$total_searches       = $analytics_db->get_total_searches_count( $this->filters['timeframe_from'], $this->filters['timeframe_to'] );
		$total_contact_opened = $analytics_db->get_total_contact_opened_count( $this->filters['timeframe_from'], $this->filters['timeframe_to'] );
        $total_form_submitted = $analytics_db->get_total_form_submitted_count( $this->filters['timeframe_from'], $this->filters['timeframe_to'] );

		if ( ! empty( $total_form_submitted ) && $total_form_submitted > $total_contact_opened ) {
			$total_contact_opened = $total_form_submitted;
		}
		...
		return [
			...
			// OVERVIEW
			'overview'   => [
				'tab_intro' => esc_html__( 'The number of events for the last 30 days is shown below.', 'help-dialog' ),
				[
					'type'          => 'single',
					'css_class'     => 'ephd-ap-results--single',
					'title'         => esc_html__( 'Dialog Opened', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Help Dialog Opened', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'Number of times the Help Dialog was opened.', 'help-dialog' ),
				...
				[
					'type'          => 'single',
					'css_class'     => 'ephd-ap-results--single',
					'title'         => esc_html__( 'Searches', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Searches', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'Number of times a user searched in the Help Dialog.', 'help-dialog' ),
				...
				[
					'type'          => 'single',
					'css_class'     => 'ephd-ap-results--single',
					'title'         => esc_html__( 'Contact Form Opened', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Contact Form Opened', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'Number of times the Contact Form was opened.', 'help-dialog' ),
					'results'       => $total_contact_opened,
					'is_pro'        => false
				],
				[
					'type'          => 'multiple-col-2-core',
					'css_class'     => 'ephd-ap-results--multiple-col-2',
					'title'         => esc_html__( 'Contact Tab Submissions Ratio', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Number of Clicks to Open the Help Dialog.', 'help-dialog' ),
					'tooltip_desc'  => sprintf( esc_html__( '%sCTR%s measures the usage of the Help Dialog in relation to page views. %sView:%s When a page is loaded for longer than 5 seconds, a view is recorded. ' .
															'%sClick:%s When the Help Dialog Launcher Icon is clicked, a click is recorded.', 'help-dialog' ),
												'<strong>', '</strong>', '<br><br><strong>', '</strong>', '<br><br><strong>', '</strong>' ) . '<hr/>' .
												esc_html__( 'Formula', 'help-dialog' ) . ': ' . esc_html__( 'Click-through Rate = Views / launcher clicks', 'help-dialog' ) .
												' <a href="https://www.helpdialog.com/documentation/reach-analytics/#articleTOC_1" target="_blank">' . esc_html__( 'Learn More', 'help-dialog' ) . '</a>.',
					'results'       => ( ! empty(  $total_form_submitted ) && ! empty( $total_contact_opened ) ) ? array(
						'ctr'           => round( ($total_form_submitted / $total_contact_opened ) * 100 ),
						'views'         => $total_contact_opened,
						'submissions'   => $total_form_submitted
					) : [],
					'columns'       => array(
						'views'         => esc_html__( 'Total Views', 'help-dialog' ),
						'submissions'   => esc_html__( 'Total Submissions', 'help-dialog' ),
						'ctr'           => esc_html__( 'Submissions Ration', 'help-dialog' ),
					)
				],
			],
			...
			// REACH
			'reach'      => [
				//'tab_intro' => esc_html__( 'Reach analytics shows initial user interaction with the Help Dialog. Did user open the Help Dialog and which pages did users interact with the most and least?
				//              By default, admins, editors, and authors have their interactions with Help Dialog excluded from analytics.', 'help-dialog' ),
				'tab_intro' => esc_html__( 'Reach analytics show user interaction with three areas of the Help Dialog: FAQs, search, and contact form.', 'help-dialog' ),
				[
					'type'          => 'multiple-col-2',
					'css_class'     => 'ephd-ap-results--multiple-col-2',
					'title'         => esc_html__( 'Help Dialog Total CTR', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Number of Clicks to Open the Help Dialog.', 'help-dialog' ),
					'tooltip_desc'  => sprintf( esc_html__( '%sCTR%s measures the usage of the Help Dialog in relation to page views. %sView:%s When a page is loaded for longer than 5 seconds, a view is recorded.' .
															'%sClick:%s When the Help Dialog Launcher Icon is clicked, a click is recorded.', 'help-dialog' ),
															'<strong>', '</strong>', '<br><br><strong>', '</strong>', '<br><br><strong>', '</strong>' ) . '<hr/>' .
					                                        esc_html__( 'Formula', 'help-dialog' ) . ': ' . esc_html__( 'Click-through Rate = Views / launcher clicks', 'help-dialog' ) .
															' <a href="https://www.helpdialog.com/documentation/reach-analytics/#articleTOC_1" target="_blank">' . esc_html__( 'Learn More', 'help-dialog' ) . '</a>.',
					// FUTURE TODO example 'tooltip_args'  => [ 'link_url' => '#', 'link_text' => esc_html__( 'Example Button', 'help-dialog' ) ], // TODO example button
					'results'       => isset( $results['click_through_rate'] ) ? $results['click_through_rate'] : [],
                    'columns'         => array(
	                    'views'  => esc_html__( 'Total Views', 'help-dialog' ),
	                    'clicks' => esc_html__( 'Total Clicks', 'help-dialog' ),
	                    'ctr'    => esc_html__( 'CTR', 'help-dialog' ),
                    )
				],
				[
					'type'          => 'multiple',
					'css_class'     => 'ephd-ap-results--multiple-row-1',
					'title'         => esc_html__( 'All Clicks', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Overall Clicks', 'help-dialog' ),
					...
					'columns'       => array(
						'launcher' => esc_html__( 'Launcher', 'help-dialog' ),
						'search'   => esc_html__( 'Search', 'help-dialog' ),
						'contact'  => esc_html__( 'Contact Form', 'help-dialog' ),
					),
                    'icons'          => array(
				...
				[
					...
					'title'         => esc_html__( 'Most Engaged Help Dialogs by CTR', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'On Which Pages Did Users Most Often Open the Help Dialog?', 'help-dialog' ),
					//'tooltip_desc'  => esc_html__( 'These pages are either too complex to understand, or they are of great interest to the user who is looking for more information.', 'help-dialog' ) . ' <a href="https://www.helpdialog.com/documentation/reach-analytics/#articleTOC_2" target="_blank">' . esc_html__( 'Learn More', 'help-dialog' ) . '</a>.',
					...
					'columns'       => array(
						'title'  => esc_html__( 'Top Pages', 'help-dialog' ),
						'views'  => esc_html__( 'Views', 'help-dialog' ),
						'clicks' => esc_html__( 'Clicks', 'help-dialog' ),
						'ctr'    => esc_html__( 'CTR', 'help-dialog' ),
					),
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--ctr',
					'title'         => esc_html__( 'Least Engaged Help Dialogs by CTR', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'On Which Pages Did Users Least Often Open the Help Dialog?', 'help-dialog' ),
				...
				[
					...
					'title'         => esc_html__( 'Searches Initiated', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'On Which Pages Did Users Search the Most?', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'Pages that have the most searches indicate that the shown FAQs in the Help Dialog are not sufficient or not relevant to help users with their questions.', 'help-dialog' ) . ' <a href="' . esc_url( Echo_Help_Dialog::$plugin_url . 'img/analytics/searches-initiated.png' ) . '" target="_blank">' . esc_html__( 'Learn More', 'help-dialog' ) . '</a>.',
					'results'       => isset( $results['searches_count_by_page'] ) ? $results['searches_count_by_page'] : [],
					'cells_wrap'    => $location_link_cells_wrap,
					'columns'       => array(
						'title' => esc_html__( 'Top Pages', 'help-dialog' ),
						'times' => esc_html__( 'Searches', 'help-dialog' ),
					),
				],
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Contact Form Views', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'On Which Pages Did Users Look at the Contact Form?', 'help-dialog' ),
					...
					'columns'       => array(
						'title' => esc_html__( 'Top Pages', 'help-dialog' ),
						'times' => esc_html__( 'Opened', 'help-dialog' ),
					),
				],
			],
			...
			// CONTENT
			'content'    => [
				'tab_intro' => esc_html__( 'Content analytics is showing popularity of specific questions, articles and searches.', 'help-dialog' ),
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--ctr',
					'title'         => esc_html__( 'Most Helpful Page FAQs', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Pages with Most Helpful FAQs', 'help-dialog' ),
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--ctr',
					'title'         => esc_html__( 'Least Helpful Page FAQs', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Pages with Least Helpful FAQs', 'help-dialog' ),
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Most Popular Questions', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Which Questions Are Users Looking for the Most?', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'These questions suggest that the pages are missing information or the topic is not well explained.', 'help-dialog' ),
					'results'       => isset( $results['most_popular_questions'] ) ? $results['most_popular_questions'] : [],
					'columns'       => array(
						'title' => esc_html__( 'Questions', 'help-dialog' ),
						'times' => esc_html__( 'Clicks', 'help-dialog' ),
					),
				],
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Least Popular Questions', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Which Questions Are Users Looking for the Least?', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'On these pages, the questions are either not relevant or are already convered by the page content.', 'help-dialog' ),
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Most Popular Posts', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Which Posts Are Users Looking for the Most?', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'These posts suggest that the FAQs and Articles are not sufficient and the pages are missing information or the topic is not well explained.', 'help-dialog' ) . '<hr/>' .
									   esc_html__( 'When a post is searched for and then clicked on, a click is recorded.', 'help-dialog' ),
					'empty_message' => '',
					'results'       => isset( $results['most_popular_posts'] ) ? $results['most_popular_posts'] : [],
					'columns'       => array(
						'title' => esc_html__( 'Posts', 'help-dialog' ),
						'times' => esc_html__( 'Clicks', 'help-dialog' ),
					),
				],
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Least Popular Posts', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Which Posts Are Users Looking for the Least?', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'On these pages, both FAQs and Articles are either not relevant or are already covered by the page content.', 'help-dialog' ),
					'empty_message' => '',
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Most Popular KB Articles', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Which Articles Are Users Looking for the Most?', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'These articles suggest that the FAQs and Articles are not sufficient and the pages are missing information or the topic is not well explained.', 'help-dialog' ) . '<hr/>' .
									   esc_html__( 'When an article is searched for and then clicked on, a click is recorded.', 'help-dialog' ),
					'empty_message' => $this->is_search_type_active( 'kb' ) === false ? esc_html__( 'KB search disabled', 'help-dialog' )  : '',
					'results'       => isset( $results['most_popular_articles'] ) ? $results['most_popular_articles'] : [],
					'columns'       => array(
						'title' => esc_html__( 'Articles', 'help-dialog' ),
						'times' => esc_html__( 'Clicks', 'help-dialog' ),
					),
				],
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Least Popular KB Articles', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Which Articles Are Users Looking for the Least?', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'On these pages, both FAQs and Articles are either not relevant or are already covered by the page content.', 'help-dialog' ),
					'empty_message' => $this->is_search_type_active( 'kb' ) === false ? esc_html__( 'KB search disabled', 'help-dialog' )  : '',
			...
			// SEARCH
			'search'    => [
				'tab_intro' => esc_html__( 'Search analytics list details about popular keywords and missing search results.', 'help-dialog' ),
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Searches Initiated', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'On Which Pages Did Users Search the Most?', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'Pages that have the most searches indicate that the shown FAQs in the Help Dialog are not sufficient or not relevant to help users with their questions.', 'help-dialog' ),
					'results'       => isset( $results['searches_count_by_page'] ) ? $results['searches_count_by_page'] : [],
					'cells_wrap'    => $location_link_cells_wrap,
					'columns'       => array(
						'title' => esc_html__( 'Top Pages', 'help-dialog' ),
				...
				[
					...
					'title'         => esc_html__( 'Top Searched Keywords', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Keywords Users Search for The Most', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'These keywords are popular and should have good matching content.', 'help-dialog' ),
					'results'       => isset( $results['all_frequent_searches'] ) ? $results['all_frequent_searches'] : [],
					'columns'       => array(
						'title' => esc_html__( 'Keywords', 'help-dialog' ),
						'times' => esc_html__( 'Count', 'help-dialog' ),
					),
				],
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--count',
					'title'         => esc_html__( 'Search Results With No FAQs Found', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Keywords Users Searched For That Did Not Have Matching FAQs.', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'These keywords have no matching content for user to get answers.', 'help-dialog' ),
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--count',
					'title'         => esc_html__( 'Search Results With No Articles Found', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Keywords Users Searched For That Did Not Have Matching Articles.', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'These keywords have no matching content for user to get answers.', 'help-dialog' ),
                    'empty_message' => $this->is_search_type_active( 'kb' ) === false ? esc_html__( 'KB search disabled', 'help-dialog' )  : '',
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--count',
					'title'         => esc_html__( 'Search Results With No Posts Found', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Keywords Users Searched For That Did Not Have Matching Posts.', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'These keywords have no matching content for user to get answers.', 'help-dialog' ),
					'empty_message' => $this->is_search_type_active( 'posts' ) === false ? esc_html__( 'Posts search disabled', 'help-dialog' )  : '',
            ...
            // Per Page Analytics
			'per_page'    => [
			    'tab_intro' => esc_html__( 'Shown are analytics about a specific page.', 'help-dialog' ),
				[
					'type'          => 'multiple-col-2',
					'css_class'     => 'ephd-ap-results--multiple-col-2',
					'title'         => esc_html__( 'Help Dialog CTR', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Number of Clicks to Open the Help Dialog.', 'help-dialog' ),
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Most Popular Articles', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Which Articles Are Users Looking for the Most?', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'These articles suggest that the FAQs and Articles are not sufficient and the pages are missing information or the topic is not well explained.', 'help-dialog' ),
					'empty_message' => $this->is_search_type_active( 'kb' ) === false ? esc_html__( 'KB search disabled', 'help-dialog' )  : '',
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--count',
					'box_intro'     => esc_html__( 'Searches', 'help-dialog' ),
					'title'         => esc_html__( 'Top Searched Keywords', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Keywords Users Search for The Most', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'These keywords are popular and should have good matching content.', 'help-dialog' ),
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--count',
					'title'         => esc_html__( 'Searched Keywords Without Search Results', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Keywords Users Searched For That Did Not Have Matching Content', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'These keywords have no matching content for a user to get answers.', 'help-dialog' ),
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'box_intro'     => esc_html__( 'Incomplete Searches', 'help-dialog' ),
					'title'         => esc_html__( 'Keywords With No Matches', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'TODO', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'TODO', 'help-dialog' ),
					'results'       => [], // TODO calculate results
					'columns'       => array(
						'title'       => esc_html__( 'Keywords', 'help-dialog' ),
						'times'       => esc_html__( 'Count', 'help-dialog' ),
						'hd_matching' => esc_html__( '# of matching Questions', 'help-dialog' ),
						'kb_matching' => esc_html__( '# of matching Articles', 'help-dialog' ),
					),
				],
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Searched Questions Not in Initial FAQs', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'TODO', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'TODO', 'help-dialog' ),
					'results'       => [], // TODO calculate results
					'columns'       => array(
				...
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Top Keywords With Unused Search Results', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'TODO', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'TODO', 'help-dialog' ),
					'results'       => [], // TODO calculate results
					'columns'       => array(
                ...
                */
				...
				[
					'type'          => 'multiple-col-2',
					'css_class'     => 'ephd-ap-results--multiple-col-2',
					'box_intro'     => esc_html__( 'Contact Form', 'help-dialog' ),
					'title'         => esc_html__( 'Contact Form CTR', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'Click Through Rate', 'help-dialog' ),
					...
					'columns'       => array(
						'views'  => esc_html__( 'Views', 'help-dialog' ),
						'clicks' => esc_html__( 'Submissions', 'help-dialog' ),
						'ctr'    => esc_html__( 'CTR', 'help-dialog' ),
					),
				],
                /* TODO FUTURE
				[
					'type'          => 'table',
					'css_class'     => 'ephd-ap-results--views',
					'title'         => esc_html__( 'Contact Form Submissions', 'help-dialog' ),
					'tooltip_title' => esc_html__( 'TODO', 'help-dialog' ),
					'tooltip_desc'  => esc_html__( 'TODO', 'help-dialog' ),
					'results'       => [], // TODO calculate results
					'columns'       => array(
						'date'    => esc_html__( 'Date', 'help-dialog' ),
						'subject' => esc_html__( 'subject', 'help-dialog' ),
						'link'    => esc_html__( 'Link', 'help-dialog' ),
					),
				],
                */
		    ],
				...
				if ( $args['is_pro'] && ! EPHD_Utilities::is_help_dialog_pro_enabled() ) {    ?>
                    ...
                    if ( empty( $args['results'] ) ) {    ?>
                        <div class="ephd-ap-widget-nodata-message">
                            ...
                            <div class="ephd-hd-no-results__title"><?php echo empty( $args['empty_message'] ) ? esc_html__( 'No data recorded yet for the selected time frame. Excluded users will not be counted.', 'help-dialog' ) : esc_html( $args['empty_message'] ); ?></div>
                        </div>  <?php
                    } else {    ?>
	...
	/**
	 * Display analytics multiple values results for core
	 *
	 * @param $args
	 * @return false|string
	 */
	private static function get_core_multiple_results_html( $args ) {
		ob_start(); ?>

        <div class="ephd-ap-widget">
            <div class="ephd-ap-widget-content">    <?php
	            if ( ! empty( $args['results'] ) && ! empty( $args['columns'] ) ) {
		            $main_cell_key = ( $args['type'] == 'multiple-col-2-core' ) ? array_key_first( $args['results'] ) : '';   ?>
                    <div class="ephd-ap-widget-multi-cells">
                        <div class="ephd-ap-widget-multi-cells__col">   <?php
				            foreach ( $args['results'] as $cell_key => $result ) {
					            if ( $cell_key === $main_cell_key ) {
                    ...
                    <div class="ephd-ap-widget-nodata-message">
                        <img class="ephd-hd-no-results__img" alt="<?php esc_attr_e( 'No Data', 'help-dialog' ); ?>" src="<?php echo esc_url( Echo_Help_Dialog::$plugin_url . 'img/no-faqs-defined.jpeg' ); ?>" />
                        <div class="ephd-hd-no-results__title"><?php echo empty( $args['empty_message'] ) ? esc_html__( 'No data recorded yet for the selected time frame. Excluded users will not be counted.', 'help-dialog' ) : esc_html( $args['empty_message'] ); ?></div>
                    </div>  <?php
	            }  ?>
            </div>
            ...
            <div class="ephd-ap-widget-content"> <?php
	            if ( $args['is_pro'] && ! EPHD_Utilities::is_help_dialog_pro_enabled() ) {    ?>
                    ...
                    if ( empty( $list_of_items ) ) {    ?>
						...
						$html_table_escaped = $handler->get_html_table( $short_list_of_items, $items_count, '', $args['columns'], [], [], 'ephd-admin__open-details-popup' );
	                    echo $html_table_escaped;//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    ...
    private static function error_log_notification() {
        ...
        <div id="ephd-admin-page-wrap" class="ephd-admin-page-wrap--config-error"> <?php
	        EPHD_HTML_Forms::notification_box_top(
		        array(
			        'type'  => 'warning',
			        'title' => esc_html__( 'Error Logs', 'help-dialog' ),
			        'desc'  => esc_html__( "There are {$error_logs_count} error logs in the last 7 days", 'help-dialog' )
		        )
	        );  ?>
        </div>  <?php
    }
	...
	private function get_regular_views_config() {
		...
		$core_views[] = [
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Reach', 'help-dialog' ),
		...
		$core_views[] = [
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Content', 'help-dialog' ),
		...
		$core_views[] = [
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Search', 'help-dialog' ),
        ...
        // Show notification if no location selected or if there are no pages in the location list
	        ...
	        $notification_desc = $this->filters['location'] == ''
                ? esc_html__( 'Please select a page', 'help-dialog' )
                : esc_html__( 'No analytics data were found for the chosen time interval', 'help-dialog' );
		...
		$core_views[] = [
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Per Page', 'help-dialog' ),
		...
		$core_views[] = array(
			...
			// Top Panel Item
			'label_text' => esc_html__( 'Settings', 'help-dialog' ),
			...
			'boxes_list' => array(
				...
				/* array(
					'title' => esc_html__( 'Analytics Configuration', 'help-dialog' ),
