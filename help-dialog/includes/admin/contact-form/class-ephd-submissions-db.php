...
class EPHD_Submissions_DB extends EPHD_DB {
	...
	public function get_submissions( $page_number=1 ) {
		...
		$submissions = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $this->table_name ORDER BY %s DESC LIMIT %d, %d ", self::PRIMARY_KEY, $offset, self::PER_PAGE ) );
	...
	public static function get_submission_column_fields() {
		return [
			'submit_date'           => esc_html__( 'Date', 'help-dialog' ),
			'name'                  => esc_html__( 'Name', 'help-dialog' ),
			'email'                 => esc_html__( 'Email', 'help-dialog' ),
			'page_name'             => esc_html__( 'Page Name', 'help-dialog' ),
			'notification_status'   => esc_html__( 'Notification Status', 'help-dialog' ),
		];
	}
	...
	public static function get_submission_row_fields() {
		return [
			...
			'comment' => esc_html__( 'Comment', 'help-dialog' ),
		];
	}
	...
	public static function get_submission_optional_row_fields() {
		return [
			'notification_details' => esc_html__( 'Notification Details', 'help-dialog' ),
		];
	}
	...
	public static function get_submission_notification_statuses() {
		return [
			'email_error'         => esc_html__( 'Error', 'help-dialog' ),
			'email_sent'          => esc_html__( 'Sent', 'help-dialog' ),
			'email_received'      => esc_html__( 'Submission Received', 'help-dialog' ),
			'no_submission_email' => esc_html__( 'No Destination Email', 'help-dialog' ),
		];
	}
	...
	private function filter_output( $submissions ) {
		...
		foreach ( $submissions as $index => $data ) {
			...
			$data->submit_date = date( 'M j, Y g:ia', strtotime( $data->submit_date ) );

			// Change notification status
			$notification_status = $data->notification_status;
			if ( isset( $notification_statuses[$notification_status] ) ) {
				$data->notification_status = $notification_statuses[$notification_status];
			} else {
			...
			if ( $show_settings_url && $notification_status == self::NOTIFICATION_STATUS_NO_EMAIL && empty( $global_config['contact_submission_email'] ) ) {
				$data->notification_status .= '<div><a href="#" data-target="contact-form-setup" class="ephd-admin__step-cta-box__link">' . esc_html__( 'Click here to update settings', 'help-dialog' ) . '</div>';
