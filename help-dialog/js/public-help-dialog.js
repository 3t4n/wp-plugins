...
jQuery(document).ready(function ($) {
    ...
    let parent_container = $('#ephd-help-dialog'),
        contactFormTextarea = $('#ephd-hd__contact-form-comment'),
        contactFormContainerInitialHeight = $('#ephd-hd-body-container').height(),
        initialTextAreaHeight = contactFormTextarea.outerHeight();

	// Open widget if initial visibility setting is on and widget current is closed
	function open_widget_if_required() {
		let help_dialog_toggle = $('.ephd-hd-toggle');
		let help_dialog_box = $('#ephd-help-dialog');
		if (help_dialog_box.hasClass('ephd-widget--initial-on') && ! help_dialog_toggle.hasClass('ephd-hd-toggle--on')) {
			help_dialog_toggle.trigger('click');
		}
	}
    ...
    function initialize_widget_trigger() {
        ...
        if (trigger_delay_seconds > 0) {
            ...
            setTimeout(function () {
                ...
                $('.ephd-hd-toggle').show(400, open_widget_if_required);
    ...
    if ($('.ephd-hd-toggle').length) {
        ...
        if (parent_container.data('ephd-triggers-visibility') === 'visible') {
			let help_dialog_toggle = $('.ephd-hd-toggle');
            let start_delay = help_dialog_toggle.data('ephd-start-wait');
            setTimeout(function () {
				help_dialog_toggle.show(400, open_widget_if_required);
            }, start_delay * 1000);
        }
    }
    ...
    function analytics_count_impression_view() {
        ...
        if (help_dialog_toggle.length && $('html.ephd-preview-iframe').length <= 0) {

            let analytics_delay = help_dialog_toggle.data('analytics-delay');
    ...
    function adjust_help_dialog_height() {
        ...
        let chatBodyHeight = 447;

        $('.ephd-help-dialog-container').each(function () {

            // extended chat class present. Increase height by 50% ( 228 px )
            if ($(this).hasClass('ephd-hd-extended-chat-height')) {
                minWindowHeight = 958
                chatBodyHeight = 685;
            }
            // Calculate the height difference and minus it from our fixed value of 477
            let diff = chatBodyHeight - (minWindowHeight - windowHeight);
            ...
            if (diff > chatBodyHeight) {
                $(this).find('#ephd-hd-body-container').css('height', (chatBodyHeight - logoSpace));
        ...
        contactFormTextarea.css('height', initialTextAreaHeight);
    }
    ...
    if ($('.ephd-help-dialog-container').length) {
        ...
        // create custom event to trigger increase chat body height
        $('#ephd-help-dialog').on('ephd_increase_chat_height', function () {
            adjust_help_dialog_height();
        });
            ...
            if ($search_value === $this_input.val()) {
                ...
                $.ajax({
                    ...
                    if (no_results.length > 0) {
                        ...
                        switch ($firstTab) {
                            case 'faq':
                                $('#ephd-hd__search-results-faqs-tab').addClass('ephd-hd-results__tab--active').trigger('click').trigger('focus');
                                $('#ephd-hd__search_results__faqs').addClass('ephd-hd-results__tab-content--active');
                                break;
                            case 'articles':
                                $('#ephd-hd__search-results-articles-tab').addClass('ephd-hd-results__tab--active').trigger('click').trigger('focus');
                                $('#ephd-hd__search_results__articles').addClass('ephd-hd-results__tab-content--active');
                                break;
                            case 'posts':
                                $('#ephd-hd__search-results-post-tab').addClass('ephd-hd-results__tab--active').trigger('click').trigger('focus');
    ...
    $(document).on('click', '.ephd-hd-toggle', function () {
        let help_dialog_box = $('#ephd-help-dialog');
        $('#ephd-hd-body-container .ephd-hd-faq__list__item-container').attr('tabindex', '0');
        $('#ephd-hd-body-container .ephd-hd-resource a').attr('tabindex', '0');
        $('#ephd-hd__contact-form-body .ephd-hd__contact-form-field input').attr('tabindex', '0');
        $('#ephd-hd-body__chat-container .ephd-hd-chat-search-container input').attr('tabindex', '0');
        ...
        help_dialog_box.slideToggle(400, function () {
            ...
            // Once the dialog is shown, listen for Esc key
            help_dialog_box.off('keydown').on('keydown', function(e) {
                if (e.key === 'Escape') {
                    // Trigger the toggle button click to close the dialog
                    $('.ephd-hd-toggle.ephd-hd-toggle--on').click();
                }
            });
        });
        ...
        help_dialog_box.attr('tabindex', function (i, attr) {
            return attr === '0' ? '-1' : '0';
        });
        ...
        if ($('.ephd-hd-tab--active').length) {
            $('.ephd-hd-tab--active').trigger('focus');
            // add focus to container if tabs are hidden
        } else {
            $('#ephd-hd-body-container div[role="tabpanel"]:first').trigger('focus');
        }
    });
    ...
    $(document).on('keydown', '.ephd-hd-tab', function (e) {
        if (e.which === 9) {
            ...
            // Resource Tab
            if ($(this).attr('id') === 'ephd-hd-resource-tab') {
                $('.ephd-hd-resource-container .ephd-hd-resource:first a').trigger('focus');
                console.log( 'Resource Tab')
            }
            // FAQs Tab
            if ($(this).attr('id') === 'ephd-hd-faq-tab') {
                $('#ephd-hd-body__content-container .ephd-hd-faq__list__item-container:first').trigger('focus');
            }
            // Contact Tab
            if ($(this).attr('id') === 'ephd-hd-contact-tab') {
                $('#ephd-hd-body__contact-container .ephd-hd__contact-form-field:first input').trigger('focus');
            }
            // Chat Tab
            if ($(this).attr('id') === 'ephd-hd-chat-tab') {
                $('.ephd-hd-chat-container .ephd_hd__chat-assistant-reply-container a').trigger('focus');
            }
    ...
    $(document).on('keydown', '#ephd-hd__search-terms, .ephd-hd-results__tab', function (e) {
        if (e.which === 9) {
            ...
            if ($active_tab.find('.ephd-hd_article-item:first').length) {
                $active_tab.find('.ephd-hd_article-item:first').trigger('focus');
                // FAQ Articles results
            } else if ($active_tab.find('.ephd-hd-faq__list__item-container:first').length) {
                $active_tab.find('.ephd-hd-faq__list__item-container:first').trigger('focus');
                // initial FAQs list
            } else {
                $('#ephd-hd-body__content-container .ephd-hd-faq__list__item-container:first').trigger('focus');
            }
    ...
    $(document.body).on('click', '.ephd-hd-tab', function () {
        ...
        switch (target_tab) {
            case 'chat':
                ...
                if ( parent_container.hasClass('ephd-hd-ai-chat--active') ) {
                    parent_container.addClass('ephd-hd-extended-chat-active');
                }
                break;
            case 'resource':
                // Set the top Container Class for the active Tab.
                ...
                parent_container.removeClass('ephd-hd-extended-chat-active');
                break;
            case 'faqs':
                // Set the top Container Class for the active Tab.
    ...
    $(document).on('keydown', '#ephd-help-dialog', function (e) {
        ...
        if (target_tab) {
            $(target_tab).trigger('click').trigger('focus');
        }
    });
    ...
    // Prevent actions for Resource links on admin preview
    $(document).on('click', '.ephd-hd-resource a', function (e) {
    ...
    //Contact form Textarea - Change container height
    const resizeObserver = new ResizeObserver(function (entries) {
        entries.forEach(function (entry) {
            let newTextareaHeight = $(entry.target).outerHeight(),
                heightDiff = newTextareaHeight - initialTextAreaHeight,
                newContainerHeight = contactFormContainerInitialHeight + heightDiff;

            if (newContainerHeight > contactFormContainerInitialHeight && newContainerHeight < 720) {
                $('#ephd-hd-body-container').css('height', newContainerHeight);
            }

            if (newTextareaHeight >= 335) {
                $(this).css('height', '335px');
                $('#ephd-hd-body-container').css('height', '720px');
            }
        });
    });

    if (contactFormTextarea && contactFormTextarea.length) {
        resizeObserver.observe(contactFormTextarea[0]);
    }
});