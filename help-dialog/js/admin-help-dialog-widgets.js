jQuery(document).ready(function ($) {
  ...
  $(".ephd-admin__reload-link").on("click", function () {
    $(
      '.ephd-admin__top-panel__item[data-target="' +
  ...
  function ephd_check_no_questions_message() {
    let all_questions = $(
      ".ephd-all-questions-list-container .ephd-faq-question-container"
    ).length;
    let active_questions = $(
      ".ephd-all-questions-list-container .ephd-faq-question--active"
    ).length;

    // Hide all questions
    $(
      "#ephd-admin__no-question-message, #ephd-admin__assigned-question-message"
    ).hide();
  ...
  $(document).on(
    "change",
    '.ephd-wp__widget-form .ephd-input-group input[name="launcher_powered_by"]',
    function () {
  ...
  function ephd_set_power_by_notification() {
    let notification = $(
      ".ephd-wp__widget-form #ephd-admin__notification-launcher-powered-by"
    );
    let input = $(
      '.ephd-wp__widget-form .ephd-input-group input[name="launcher_powered_by"]:checked'
    );
    if (input.val() == "hide") {
      notification.show();
    } else {
  ...
  function ephd_enable_sortable_for_faqs() {
    $(".ephd-hd-faq__faqs-container").sortable({
      ...
      update: function (event, ui) {
        ...
        $(this)
          .find(".ephd-hd-faq__list__item-container")
          .each(function (e) {
            ordered_faqs.push($(this).data("id"));
          });
        $(".ephd-wp__widget-form--active")
          .find('[name="faqs_sequence"]')
          .val(ordered_faqs.join(","));
      },
    });
  }
  ...
  $(document.body).on(
    "click",
    ".ephd-wp__selected-locations-popup .ephd-wp__popup-show-btn",
    function () {
      $(this)
        .closest(".ephd-wp__selected-locations-popup")
        .find(".ephd-admin__widget-details-popup")
        .addClass("ephd-admin__widget-details-popup--active");
      return false;
    }
  );
  ...
  $(document.body).on(
    "click",
    ".ephd-admin__widget-details-popup__overlay, .ephd-admin__widget-details-popup__accept-btn",
    function () {
      $(".ephd-admin__widget-details-popup").removeClass(
        "ephd-admin__widget-details-popup--active"
      );
    }
  );
  ...
  setTimeout(function () {
    ...
    if (typeof hash_parameters["widget"] !== "undefined") {
      $(
        ".ephd-wp__widget-tab--" +
          hash_parameters["widget"] +
          " input.ephd_edit_widget"
      ).trigger("click");
    }
  }, 50);
  ...
  function show_widget_search_loader(el) {
    ...
    $wrap
      .find(".ephd-wp__locations-list-input-wrap")
      .addClass("ephd-wp__locations-list-input-wrap--loader");
  }
  ...
  function hide_search_loader(el) {
    ...
    $wrap
      .find(".ephd-wp__locations-list-input-wrap")
      .removeClass("ephd-wp__locations-list-input-wrap--loader");
  }

  // Search input
  $(document.body).on(
    "input click",
    ".ephd-wp__widget-form .ephd-wp__locations-list-input",
    function (e) {
      ...
      setTimeout(function () {
        ...
        let postData = {
          ...
          location_page_filtering: $(
            '[name="location_page_filtering"]:checked'
          ).val(),
          widget_id: form.find('[name="widget_id"]').val(),
          location_language_filtering: $(
        ...
        ephd_send_ajax(
          ...
          function (response) {
            ...
            if (!response.error && typeof response.locations != "undefined") {
              form
                .find(
                  ".ephd-wp__locations-list-select--" +
                    locations_type +
                    " .ephd-wp__found-locations-list"
                )
                .html(response.locations)
                .show();
            }
          },
          false,
  ...
  $(document).on(
    "click",
    ".ephd-wp__widget-form .ephd-wp__found-locations-list li",
    function (e) {
      e.stopPropagation();
      // Do not add Location if it is already added to another widget
      if (
        $(this).data("assigned_widget_id") &&
        $(this).data("assigned_widget_id") !=
          $(this)
            .closest(".ephd-wp__widget-form")
            .find('[name="widget_id"]')
            .val()
      ) {
        return false;
      ...
      $(this).appendTo(
        $(this).parent().parent().find(".ephd-wp__selected-locations-list")
      );
  ...
  $(document).on(
    "click",
    ".ephd-wp__widget-form .ephd-wp__selected-locations-list li",
    function () {
      let location_id = $(this).data("id");

      // Remove element from widget page and from popup dialog
      $(
        '.ephd-wp__widget-form .ephd-wp__selected-locations-list li[data-id="' +
          location_id +
          '"]'
      ).remove();
  ...
  function ephd_update_selected_locations_visibility() {
    ...
    $(".ephd-wp__widget-form .ephd-wp__selected-locations-list").each(
      function () {
        let count = 0;
  ...
  $(document).on(
    "click",
    ".ephd-notification-box-basic__body__desc .ephd-hd__wizard-link",
    function () {
      $(
        '.ephd-admin__top-panel__item[data-target="' +
          $(this).data("target") +
  ...
  // Function to load form to edit Widget
  function loadEditWidgetForm(element) {
      if (element) {
          element.preventDefault();
      }

      let $activeWidgetTab = element ? $(element.target).closest('.ephd-wp__widget-tab__item') : $('.ephd-wp__widget-tab__item.ephd-wp__widget-tab--active');
      let widget_id = $activeWidgetTab.find( '.ephd_edit_widget' ).data("id");
      let widget_count = $activeWidgetTab.parent().find('.ephd-wp__widget-tab__item').length;
      widget_count++;
      ...
      // Toggle Active state
      $('.ephd-wp__widget-tab__item.ephd-wp__widget-tab--active').removeClass('ephd-wp__widget-tab--active');
      $activeWidgetTab.addClass('ephd-wp__widget-tab--active');

      ephd_send_ajax(postData, function (response) {
          if (!response.error && typeof response.message != "undefined") {
              // Hide elements before showing the form
              ...
              setTimeout(function () {
                  ...
                  if (typeof hash_parameters["tab"] !== "undefined") {
                      $(
                          '.ephd-admin__form-tab[data-target="' +
                          hash_parameters["tab"] +
                          '"]'
                      ).trigger("click");
                  }
    ...
    $(document).on("click", '.ephd-wp__widget-tab__item', function(e) {
        loadEditWidgetForm(e);
    });


    // Trigger the function on initial page load
    loadEditWidgetForm();
  ...
  $(document).on(
    ...
    function (e) {
      ...
      let widget_id = $( '.ephd-widgets-page-container .ephd-wp__widget-tab--active' ).find( '.ephd_edit_widget' ).data("id");

      let widget_count = $('.ephd-wp__widgets-tabs .ephd-wp__widget-tabs__wrapper').find(".ephd-wp__widget-tab__item").length;
      ...
      ephd_send_ajax(postData, function (response) {
        if (!response.error && typeof response.message != "undefined") {
          ...
          setTimeout(function () {
            $(
              ".ephd-wp__widget-form .ephd-wp__widget-action__publish-btn"
            ).trigger("click");
          }, 50);
        }
      });
  ...
  $(document).on(
    ...
    function (e) {
      ...
      // Do not allow to publish widget with empty locations - still allow to save as draft
      if ( $('#widget_status input:checked').val() !== 'draft' && postData.location_page_filtering === 'include' && ! postData.location_pages_list.length && ! postData.location_cpts_list.length && ! postData.location_posts_list.length ) {
        let output =
          '<div class="ephd-admin-dialog-box-loading ephd-admin-dialog-box-loading--save-warning">' +
          //<-- Header -->
          '<div class="ephd-admin-dbl__header">' +
          '<span class="ephp-admin-dbl__close ephdfa ephdfa-close"></span>' +
          '<div class="ephd-admin-text">' + ephd_help_dialog_vars.msg_save_widget + '</div>' +
          "</div>" +
          "</div>" +
      ...
      let active_tab = form
        .find(".ephd-admin__form-tab--active")
        .data("target");
      ...
      ephd_send_ajax(postData, function (response) {
        if (!response.error && typeof response.message != "undefined") {
          ...
          if ($(".ephd-wp__widget-tab--" + response.widget_id).length) {
            // Update preview box for Widget
            $(".ephd-wp__widget-tab--" + response.widget_id).replaceWith(
              $(response.widget_preview).find(".ephd-wp__widget-tab--" + response.widget_id)
            );
          } else {
            // Render preview box for new Widget
            $(".ephd-wp__widget-tabs__wrapper").append(
                $(response.widget_preview).find(".ephd-wp__widget-tab__item")
            );
          }

          // Set active tab
          $('.ephd-admin__form-tab[data-target="' + active_tab + '"]').trigger(
            "click"
          );
  ...
  //Close save popup warning
    $(document).on(
        "click",
        '.ephp-admin-dbl__close',
        function (e) {
            e.preventDefault();
            $('.ephd-admin-dialog-box-loading--save-warning').remove();
  ...
  $(document).on(
    "click",
    ".ephd-wp__delete-widget-wrap input.ephd_delete_widget",
    function (e) {
      $("#ephd-wp__delete-widget-confirmation").addClass(
  ...
  $(document).on(
    "submit",
    "#ephd-wp__delete-widget-confirmation",
    function (e) {
      e.preventDefault();
      ...
      ephd_send_ajax(postData, function (response) {
        if (!response.error && typeof response.message != "undefined") {
          ...
          $(".ephd-wp__widget-tab--" + widget_id).fadeOut(function () {
            $(this).remove();
          });

          //Open first tab
          $('.ephd-wp__widget-tab__item:first-child .ephd_edit_widget').trigger('click');
  ...
  $(document).on(
    "click",
    ".ephd-wp__widget-form .ephd-admin__form-tab",
    function () {
      let target_key = $(this).data("target"),
        parent_wrap = $(this).closest(".ephd-wp__widget-form");

      parent_wrap
        .find(".ephd-admin__form-tab")
        .removeClass("ephd-admin__form-tab--active");
      parent_wrap
        .find(".ephd-admin__form-tab-wrap")
        .removeClass("ephd-admin__form-tab-wrap--active");

      $(this).addClass("ephd-admin__form-tab--active");
      parent_wrap
        .find(".ephd-admin__form-tab-wrap--" + target_key)
        .addClass("ephd-admin__form-tab-wrap--active");
      ...
      if ($(this).data("preview")) {
        ...
        if ($(this).data("preview") == 1) {
          $wrap
            .find(".ephd-wp__widget-preview-page")
            .removeClass("ephd-wp__widget-preview-page--active");
        ...
        if ($(this).data("preview") == 2) {
          $wrap
            .find(".ephd-wp__widget-preview-page")
            .addClass("ephd-wp__widget-preview-page--active");
        ...
        let active_tab = $wrap
          .find(".ephd-hd-tab--active")
          .data("ephd-target-tab");
      ...
      check_message_mode();
      check_initial_dialog_visibility_mode();
    }
  );
  ...
  $(document).on(
    "click",
    ".ephd-wp__widget-form .ephd-admin__form-sub-tab",
    function () {
      let target_key = $(this).data("target"),
        parent_wrap = $(this).closest(".ephd-wp__widget-form");

      parent_wrap
        .find(".ephd-admin__form-sub-tab")
        .removeClass("ephd-admin__form-sub-tab--active");
      parent_wrap
        .find(".ephd-admin__form-sub-tab-content")
        .removeClass("ephd-admin__form-sub-tab-content--active");

      $(this).addClass("ephd-admin__form-sub-tab--active");
      parent_wrap
        .find(".ephd-admin__form-sub-tab-content--" + target_key)
        .addClass("ephd-admin__form-sub-tab-content--active");
    }
  );
  ...
  $(document).on(
    "click",
    '.ephd-wp__widget-form .ephd-admin__form-tab-wrap--search input[type="checkbox"]',
    function () {
  ...
  $(document).on(
    "change",
    '.ephd-wp__widget-form .ephd-admin__form-tab-wrap--colors .ekb-color-picker input[type="text"]',
    function () {
      // prevent multiple triggering when moving the cursor in the color palette
      clearTimeout(colorPickerUpdateTimeout);
      colorPickerUpdateTimeout = setTimeout(
        function (obj) {
          update_preview(obj);
        },
        300,
  ...
  function widget_form_search_fields_activity_toggle() {
    let search_option = $(
      '.ephd-wp__widget-form .ephd-admin__form-tab-wrap--search input[name="search_option"]'
    );

    let search_posts = $(
      '.ephd-wp__widget-form .ephd-admin__form-tab-wrap--search input[name="search_posts"]'
    );
    let search_kb = $(
      '.ephd-wp__widget-form .ephd-admin__form-tab-wrap--search select[name="search_kb"]'
    );
  ...
  $(document).on(
    "click",
    '.ephd-wp__widget-form .ephd-wp__options-container--premade-designs input[type="radio"]',
    function () {
      $(this)
        .closest(".ephd-wp__widget-form")
        .find(".ephd-btn-checked-icon")
        .removeClass("ephd-btn-checked-icon--active");
      $(this)
        .parent()
        .find(".ephd-btn-checked-icon")
        .addClass("ephd-btn-checked-icon--active");
    }
  );
  ...
  $(document).on(
    "change",
    ".ephd-wp__widget-form .ephd-wp__options-container--copy-design-from select",
    function () {
      let form = $(this).closest(".ephd-wp__widget-form");
  ...
  $(document).on(
    "change",
    ".ephd-wp__widget-form .ephd-wp__options-container--copy-design-to select",
    function () {
  ...
  let body_mutation_callback = function (mutationsList) {
    for (let mutation of mutationsList) {
      if (
        ...
        typeof mutation.removedNodes[0].id !== "undefined" &&
        mutation.removedNodes[0].id == "ephd-editor-popup"
      ) {
  ...
  body_observer.observe(document.getElementsByTagName("body")[0], {
    childList: true,
  });
  ...
  $(document).on(
    "change",
    ".ephd-admin__input-update_preview input, .ephd-admin__input-update_preview select, .ephd-admin__input-update_preview textarea",
    function () {
      update_preview(this);
    }
  );
  ...
  function update_preview(element) {
    ...
    let form = $(element).closest(".ephd-wp__widget-form--active"),
      ...
      active_tab = $(".ephd-wp__widget-preview")
        .find(".ephd-hd-tab--active")
        .data("ephd-target-tab");
    ...
    ephd_send_ajax(
      ...
      function (response) {
        if (!response.error && typeof response.preview != "undefined") {
          ...
          // Define preview size
          $(".ephd-wp__widget-preview").attr(
            "data-preview-size",
            postData.dialog_width
          );

          // Update Styles
          $("#ephd-public-styles-inline-css").html(response.demo_styles);

          ephd_preview_tab_click(
            $('[data-ephd-target-tab="' + active_tab + '"]')
          );
          ...
          if ($(".ephd-admin__form-tab--active").data("preview") == 1) {
            open_corresponding_widget_preview_tab(
              $(".ephd-admin__form-tab--active").data("target")
            );
          }
        }
      },
      ...
      false,
      undefined,
      $(".ephd-wp__widget-preview-content")
    );
  }
  ...
  function get_widget_form_data(form, action) {
    return {
      ...
      /** Internal Settings */
      widget_id: form.find('[name="widget_id"]').val(),
      widget_status: form.find('[name="widget_status"]:checked').val(),
      ...
      location_page_filtering: form
        .find('[name="location_page_filtering"]:checked')
        .val(),
      ...
      location_language_filtering: form
        .find('[name="location_language_filtering"]')
        .val(),
      location_wildcard_url: form.find('[name="location_wildcard_url"]').val(),

      /** Locations/Triggers: Triggers */
      trigger_delay_toggle: form
        .find('[name="trigger_delay_toggle"]:checked')
        .val(),
      trigger_delay_seconds: form.find('[name="trigger_delay_seconds"]').val(),
      trigger_scroll_toggle: form
        .find('[name="trigger_scroll_toggle"]:checked')
        .val(),
      trigger_scroll_percent: form
        .find('[name="trigger_scroll_percent"]')
        .val(),
      ...
      dialog_initial_visibility: form.find('[name="dialog_initial_visibility"]:checked').val(),
      launcher_location: form.find('[name="launcher_location"]:checked').val(),
      launcher_bottom_distance: form
        .find('[name="launcher_bottom_distance"]')
        .val(),
      launcher_start_wait: form.find('[name="launcher_start_wait"]').val(),
      initial_message_toggle: form
        .find('[name="initial_message_toggle"]:checked')
        .val(),
      initial_message_text: form.find('[name="initial_message_text"]').val(),
      initial_message_mode: form
        .find('[name="initial_message_mode"]:checked')
        .val(),
      initial_message_image_url: form
        .find('[name="initial_message_image_url"]')
        .val(),

      /** Structure: Dialog */

      /**  Structure: Search */
      search_option: form.find('[name="search_option"]:checked').length
        ? "show_search"
        : "hide_search",
      search_posts: form.find('[name="search_posts"]:checked').length
        ? "on"
        : "off",
      ...
      resource_phone_toggle: form
        .find('[name="resource_phone_toggle"]:checked')
        .val(),
      resource_phone_country_code: form
        .find('[name="resource_phone_country_code"]')
        .val(),
      resource_phone_number: form.find('[name="resource_phone_number"]').val(),
      resource_phone_number_image_url: form
        .find('[name="resource_phone_number_image_url"]')
        .val(),

      resource_custom_link_toggle: form
        .find('[name="resource_custom_link_toggle"]:checked')
        .val(),
      resource_custom_link_url: form
        .find('[name="resource_custom_link_url"]')
        .val(),
      resource_custom_link_image_url: form
        .find('[name="resource_custom_link_image_url"]')
        .val(),
      ...
      contact_name_toggle: form
        .find('[name="contact_name_toggle"]:checked')
        .val(),
      contact_subject_toggle: form
        .find('[name="contact_subject_toggle"]:checked')
        .val(),
      contact_acceptance_checkbox: form
        .find('[name="contact_acceptance_checkbox"]:checked')
        .val(),
      contact_acceptance_title_toggle: form
        .find('[name="contact_acceptance_title_toggle"]:checked')
        .val(),

      /** Design: Colors */
      back_text_color: form.find('[name="back_text_color"]').val(),
      back_text_color_hover_color: form
        .find('[name="back_text_color_hover_color"]')
        .val(),
      back_background_color: form.find('[name="back_background_color"]').val(),
      back_background_color_hover_color: form
        .find('[name="back_background_color_hover_color"]')
        .val(),
      launcher_background_color: form
        .find('[name="launcher_background_color"]')
        .val(),
      launcher_background_hover_color: form
        .find('[name="launcher_background_hover_color"]')
        .val(),
      launcher_icon_color: form.find('[name="launcher_icon_color"]').val(),
      launcher_icon_hover_color: form
        .find('[name="launcher_icon_hover_color"]')
        .val(),
      ...
      welcome_title_link_color: form
        .find('[name="welcome_title_link_color"]')
        .val(),

      breadcrumb_color: form.find('[name="breadcrumb_color"]').val(),
      breadcrumb_background_color: form
        .find('[name="breadcrumb_background_color"]')
        .val(),
      breadcrumb_arrow_color: form
        .find('[name="breadcrumb_arrow_color"]')
        .val(),
      faqs_qa_border_color: form.find('[name="faqs_qa_border_color"]').val(),
      faqs_question_text_color: form
        .find('[name="faqs_question_text_color"]')
        .val(),
      faqs_question_background_color: form
        .find('[name="faqs_question_background_color"]')
        .val(),
      faqs_question_active_text_color: form
        .find('[name="faqs_question_active_text_color"]')
        .val(),
      faqs_question_active_background_color: form
        .find('[name="faqs_question_active_background_color"]')
        .val(),
      faqs_answer_text_color: form
        .find('[name="faqs_answer_text_color"]')
        .val(),
      faqs_answer_background_color: form
        .find('[name="faqs_answer_background_color"]')
        .val(),
      found_faqs_article_active_tab_color: form
        .find('[name="found_faqs_article_active_tab_color"]')
        .val(),
      found_faqs_article_tab_color: form
        .find('[name="found_faqs_article_tab_color"]')
        .val(),
      article_post_list_title_color: form
        .find('[name="article_post_list_title_color"]')
        .val(),
      article_post_list_icon_color: form
        .find('[name="article_post_list_icon_color"]')
        .val(),
      single_article_read_more_text_color: form
        .find('[name="single_article_read_more_text_color"]')
        .val(),
      single_article_read_more_text_hover_color: form
        .find('[name="single_article_read_more_text_hover_color"]')
        .val(),

      contact_submit_button_color: form
        .find('[name="contact_submit_button_color"]')
        .val(),
      contact_submit_button_hover_color: form
        .find('[name="contact_submit_button_hover_color"]')
        .val(),
      contact_submit_button_text_color: form
        .find('[name="contact_submit_button_text_color"]')
        .val(),
      contact_submit_button_text_hover_color: form
        .find('[name="contact_submit_button_text_hover_color"]')
        .val(),
      contact_acceptance_background_color: form
        .find('[name="contact_acceptance_background_color"]')
        .val(),
      resource_phone_color: form.find('[name="resource_phone_color"]').val(),
      resource_phone_hover_color: form
        .find('[name="resource_phone_hover_color"]')
        .val(),
      resource_label_color: form.find('[name="resource_label_color"]').val(),
      resource_link_color: form.find('[name="resource_link_color"]').val(),
      resource_link_hover_color: form
        .find('[name="resource_link_hover_color"]')
        .val(),

      /** Design: Labels */
      welcome_title: form.find('[name="welcome_title"]').val(),
      contact_us_top_tab: form.find('[name="contact_us_top_tab"]').val(),
      resource_header_top_tab: form
        .find('[name="resource_header_top_tab"]')
        .val(),
      resource_header_title: form.find('[name="resource_header_title"]').val(),
      resource_header_sub_title: form
        .find('[name="resource_header_sub_title"]')
        .val(),
      resource_welcome_text: form.find('[name="resource_welcome_text"]').val(),
      ...
      search_input_placeholder: form
        .find('[name="search_input_placeholder"]')
        .val(),
      article_read_more_text: form
        .find('[name="article_read_more_text"]')
        .val(),
      search_results_title: form.find('[name="search_results_title"]').val(),
      breadcrumb_home_text: form.find('[name="breadcrumb_home_text"]').val(),
      breadcrumb_search_result_text: form
        .find('[name="breadcrumb_search_result_text"]')
        .val(),
      breadcrumb_article_text: form
        .find('[name="breadcrumb_article_text"]')
        .val(),
      found_faqs_tab_text: form.find('[name="found_faqs_tab_text"]').val(),
      found_articles_tab_text: form
        .find('[name="found_articles_tab_text"]')
        .val(),
      found_posts_tab_text: form.find('[name="found_posts_tab_text"]').val(),
      no_results_found_title_text: form
        .find('[name="no_results_found_title_text"]')
        .val(),
      protected_article_placeholder_text: form
        .find('[name="protected_article_placeholder_text"]')
        .val(),
      article_back_button_text: form
        .find('[name="article_back_button_text"]')
        .val(),
      search_instruction_text: form
        .find('[name="search_instruction_text"]')
        .val(),
      no_result_contact_us_text: form
        .find('[name="no_result_contact_us_text"]')
        .val(),
      contact_user_email_text: form
        .find('[name="contact_user_email_text"]')
        .val(),
      ...
      contact_acceptance_text: form
        .find('[name="contact_acceptance_text"]')
        .val(),
      contact_acceptance_title: form
        .find('[name="contact_acceptance_title"]')
        .val(),
      contact_button_title: form.find('[name="contact_button_title"]').val(),
      contact_success_message: form
        .find('[name="contact_success_message"]')
        .val(),

      resource_phone_label: form.find('[name="resource_phone_label"]').val(),
      resource_custom_link_label: form.find('[name="resource_custom_link_label"]').val(),

      /** Global Settings */
      logo_image_url: form.find('[name="logo_image_url"]').val(),
      mobile_break_point: form.find('[name="mobile_break_point"]').val(),
      tabs_position_1: form.find('[name="tabs_position_1"]:checked').val(),
      tabs_position_2: form.find('[name="tabs_position_2"]:checked').val(),
      tabs_position_3: form.find('[name="tabs_position_3"]:checked').val(),
      tabs_position_4: form.find('[name="tabs_position_4"]:checked').val(),
      preview_post_mode: form.find('[name="preview_post_mode"]:checked').val(),
      preview_kb_mode: form.find('[name="preview_kb_mode"]').length
        ? form.find('[name="preview_kb_mode"]:checked').val()
        : "",
      launcher_text: form.find('[name="launcher_text"]').val(),
      launcher_powered_by: form
        .find('[name="launcher_powered_by"]:checked')
        .val(),
  ...
  function get_widget_locations_data(form, location_type) {
    ...
    form
      .find(
        ".ephd-wp__locations-list-select--" +
          location_type +
          " .ephd-wp__locations-list-wrap > .ephd-wp__selected-locations-list li"
      )
      .each(function () {
  ...
  $(document).on(
    "click",
    ".ephd-wp__tiny-mce-input--no-results-found-content-html .ephd_tiny_mce_input_save",
    function () {
      ...
      let postData = {
        ...
        option_value: $("#wp-no_results_found_content_wpeditor-wrap").hasClass(
          "tmce-active"
        )
          ? tinymce.activeEditor.getContent()
          : $("#no_results_found_content_wpeditor").val(),
      };
  ...
  $(document).on(
    "click",
    ".ephd-wp__new-widget-form .ephd-wp__editor-link .ephd-wp__widget-form__cta-link",
    function (e) {
      e.preventDefault();
      e.stopPropagation();
      $("#ephd-wp__save-widget-confirmation").addClass(
        "ephd-dialog-box-form--active"
      );
      return false;
    }
  ...
  $(document).on("submit", "#ephd-wp__save-widget-confirmation", function (e) {
    ...
    $(this)
      .closest(".ephd-wp__new-widget-form")
      .find(".ephd-wp__widget-action__save-btn")
      .trigger("click");
    return false;
  });
  ...
  function check_pages_tab_labels() {
    let is_include =
      $("[name=location_page_filtering]:checked").val() == "include";
    let wildcard_url_tooltip = $(
      "label[for=location_wildcard_url] .ephd__option-tooltip "
    ).clone();

    $(
      ".ephd-wp__locations-list-select--page .ephd-wp__locations-list-search-title>span"
    ).text(is_include ? ephd_vars.include_pages : ephd_vars.exclude_pages);
    $(
      ".ephd-wp__locations-list-select--post .ephd-wp__locations-list-search-title>span"
    ).text(is_include ? ephd_vars.include_posts : ephd_vars.exclude_posts);
    $(
      ".ephd-wp__locations-list-select--cpt .ephd-wp__locations-list-search-title>span"
    ).text(is_include ? ephd_vars.include_cpts : ephd_vars.exclude_cpts);
    $("label[for=location_wildcard_url]")
      .text(
        is_include
          ? ephd_vars.include_wildcard_url
          : ephd_vars.exclude_wildcard_url
      )
      .append(wildcard_url_tooltip);
    ...
    // save selected pages
    $(this)
      .closest(".ephd-admin__form-tab-content-body")
      .find(".ephd-wp__locations-list-wrap")
      .each(function () {
        // create backup blocks for include
        if (
          $(this).find(".ephd-wp__selected-locations-list--backup-include")
            .length == 0
        ) {
          $(this).append(
            `<div class="ephd-wp__selected-locations-list--backup-include"></div>`
          );
        }

        // create backup blocks for exclude
        if (
          $(this).find(".ephd-wp__selected-locations-list--backup-exclude")
            .length == 0
        ) {
          $(this).append(
            `<div class="ephd-wp__selected-locations-list--backup-exclude"></div>`
          );
        }
        ...
        $(this)
          .find(
            ".ephd-wp__selected-locations-list--backup-" +
              (is_include ? "exclude" : "include")
          )
          .html($(this).find(".ephd-wp__selected-locations-list").html());

        // restore
        $(this)
          .find(".ephd-wp__selected-locations-list")
          .html(
            $(this)
              .find(
                ".ephd-wp__selected-locations-list--backup-" +
    ...
    // save and restore backup of wildcard url in exculde and include
    $(this)
      .closest(".ephd-admin__form-tab-content-body")
      .find("#location_wildcard_url_group")
      .each(function () {
        // create backup blocks for include
        if (
          $(this).find(".location_wildcard_url_group--backup-include").length ==
          0
        ) {
          $(this).append(
            `<div class="location_wildcard_url_group--backup-include"></div>`
          );
        }

        // create backup blocks for exclude
        if (
          $(this).find(".location_wildcard_url_group--backup-exclude").length ==
          0
        ) {
          $(this).append(
            `<div class="location_wildcard_url_group--backup-exclude"></div>`
          );
        }
        ...
        $(this)
          .find(
            ".location_wildcard_url_group--backup-" +
              (is_include ? "exclude" : "include")
          )
          .text($(this).find("#location_wildcard_url").val());

        // restore
        $(this)
          .find("#location_wildcard_url")
          .val(
            $(this)
              .find(
                ".location_wildcard_url_group--backup-" +
  ...
  function open_corresponding_widget_preview_tab(target_key) {
    ...
    if (target_key == "resource") {
      $("#ephd-hd-resource-tab").trigger("click");
    }
  ...
  function ephd_update_question_form() {
    ...
    if ($(".ephd-fp__wp-editor__language-" + current_language).length) {
      $(".ephd-fp__wp-editor__language").removeClass(
        "ephd-fp__wp-editor__language--active"
      );
      $(".ephd-fp__wp-editor__language-" + current_language).addClass(
        "ephd-fp__wp-editor__language--active"
      );
    }
    ...
    // fill the title
    $("#ephd-fp__wp-editor__question-title").val(
      question_form[current_language].title
    );
    ...
    $(".ephd-characters_left-counter").text(
      question_form[current_language].content.length + ""
    );
  }

  // get data about the question and fill the form
  function ephd_show_question_form(question_id) {
    ephd_editor_update_timer = setInterval(
  ...
  function ephd_calculate_characters_counter() {
    ...
    if (question_title.length) {
      ...
      if (question_length > 200) {
        $(".ephd-fp__wp-editor__question .ephd-characters_left-counter").text(
          200
        );
    ...
    if ($("#ephd-fp__wp-editor").length) {
      ...
      $(".ephd-fp__wp-editor__answer .ephd-characters_left-counter").text(
        answer.length
      );
    }
  }
  ...
  // hide editor
  $(".ephd__help_editor__action__cancel, .ephd-fp__wp-editor__overlay").on(
    "click",
    function () {
  ...
  $("#ephd-fp__article-form").on("submit", function (e) {
    ...
    if (!question_form[ephd_vars.default_language].title) {
      ...
      $(".ephd-fp__wp-editor__language-" + ephd_vars.default_language).trigger(
        "click"
      );
      return false;
    }
        ...
        response.data.forEach(function (question) {
          ...
          if ($(".ephd-faq-question--" + question.faq_id).length) {
            $(
              ".ephd-faq-question--" +
                question.faq_id +
                " .ephd-faq-question__text"
            ).html(question.title);

            // add article to the list
          } else {
            $(question.html)
              .appendTo(".ephd-all-questions-list-container")
  ...
  function save_question_form_to_object() {
    ...
    // save title
    question_form[current_language].title = $(
      "#ephd-fp__wp-editor__question-title"
    ).val();
  ...
  $(document).on("change keyup", "#ephd_all_articles_filter", function () {
    ...
    let current_questions = $(".ephd-wp__widget-form--active")
      .find('[name="faqs_sequence"]')
      .val()
      .split(",");

    $(".ephd-all-questions-list-container .ephd-faq-question-container").each(
      function () {
  ...
  function ephd_check_questions_filter() {
    ...
    if (
      $(
        ".ephd-all-questions-list-container .ephd-faq-question-container.ephd-faq-question--active"
      ).length
    ) {
      $(
        ".ephd__top-section__filter label, #ephd_all_articles_filter"
      ).removeClass("ephd__top-section__filter--disabled");
      ...
      $(".ephd__top-section__filter label, #ephd_all_articles_filter").addClass(
        "ephd__top-section__filter--disabled"
      );
    }
  }
  ...
  $(document).on("click", ".ephd-faq-question__delete", function () {
    ...
    let widget_id = $(
      '.ephd-wp__widget-form--active input[name="widget_id"]'
    ).val();

    $(".ephd-admin__confirm-dialog-assigned-widgets").hide();

    confirm_dialog
      .find(".ephd-admin__confirm-dialog-assigned-widget")
      .each(function () {
        let faq_sequence = $(this).data("faq-sequence").toString().split(",");
        if (
          faq_sequence.includes(id.toString()) &&
          $(this).data("widget-id").toString() !== widget_id.toString()
        ) {
  ...
  $("#ephd-fp_delete-question-confirmation form").on("submit", function () {
    ...
    ephd_send_ajax(postData, function (response) {
      if (!response.error && typeof response.message != "undefined") {
        ...
        // remove Question
        $(
          ".ephd-faq-question--" + id + ", .ephd-fp__faqs__question--" + id
        ).remove();
      }
      ...
      // Hide dialog
      $("#ephd-fp_delete-question-confirmation").removeClass(
        "ephd-dialog-box-form--active"
      );
    });
  ...
  $(document).on("click", ".ephd-faq-question__move_right", function () {
    ...
    let faqs_sequence_input = $(".ephd-wp__widget-form--active").find(
      '[name="faqs_sequence"]'
    );
    let faqs_sequence = faqs_sequence_input.val().split(",");

    $(".ephd-wp__widget-form--active .ephd-faq-question--" + id).addClass(
      "ephd-faq-question--active"
    );

    faqs_sequence = faqs_sequence.filter(function (e) {
      return e != id;
    });
  ...
  $(document).on("click", ".ephd-faq-question__add", function () {
    let id = $(this).closest(".ephd-faq-question-container").data("id");
    let faqs_sequence_input = $(this)
      .closest(".ephd-wp__widget-form--active")
      .find('[name="faqs_sequence"]');
    let faqs_sequence = faqs_sequence_input.val().split(",");

    $(this)
      .closest(".ephd-faq-question-container")
      .removeClass("ephd-faq-question--active");
  ...
  function add_buttons_to_hd_faqs_items() {
    $(
      ".ephd-wp__widget-preview #ephd-help-dialog .ephd-hd-faq__list__item-container"
    ).each(function () {
      let id = $(this).data("id");

      let buttons_block = $(
        ".ephd-wp__widget-form--active .ephd-faq-question__buttons_template"
      ).html();
  ...
  function ephd_preview_tab_click($tab) {
    ...
    switch (target_tab) {
      case "chat":
		...
		parent_container.removeClass("ephd-hd-resource-tab--active");
        break;
      case "resource":
        // Set the top Container Class for the active Tab.
        parent_container.addClass("ephd-hd-resource-tab--active");
    ...
    parent_container
      .closest(".ephd-wp__widget-preview")
      .find(".ephd-wp__widget-preview-tooltip>div")
      .hide();
    parent_container
      .closest(".ephd-wp__widget-preview")
      .find(".ephd-wp__widget-preview-tooltip--" + target_tab)
      .show();
  }
  ...
  $(document).on(
    "click",
    ".ephd__wp_editor__ai-help-sidebar-btn-open",
    function (e) {
      e.preventDefault();
  ...
  $(document).on("click", ".ephd-ai-help-sidebar-btn-close", function (e) {
    $(this)
      .closest(".ephd-fp__wp-editor")
      .removeClass("ephd-ai-help-sidebar--active");
  ...
  $(document).on(
    "click",
    ".ephd-ai__question-fix-spelling-and-grammar-btn",
    function (e) {
      e.preventDefault();

      // Do nothing if the prompt is empty or too short
      let question = ai_help_sidebar_sanitize_user_input(
        $("#ephd-fp__wp-editor__question-title").val()
      );
      ...
      ephd_send_ajax(postData, function (response) {
        if (
          ...
          typeof response.fixed_input_text != "undefined" &&
          response.fixed_input_text.length > 0
        ) {
          ephd_show_success_notification(response.message);
          ai_help_sidebar_open_alternatives_screen(
            question,
            response.tokens_used,
            action_title
          );
          $(".ephd-ai-help-sidebar__alternatives-list").html(
            '<div class="ephd-ai-help-sidebar__alternative-question">' +
              response.fixed_input_text +
              "</div>"
          );
        }
      });
  ...
  $(document).on(
    "click",
    ".ephd-ai__question-create-five-alternatives-btn",
    function (e) {
      e.preventDefault();
      ...
      ephd_send_ajax(postData, function (response) {
        if (
          !response.error &&
          typeof response.message != "undefined" &&
          typeof response.alternatives != "undefined" &&
  ...
  $(document).on(
    "click",
    ".ephd-ai__answer-fix-spelling-and-grammar-btn",
    function (e) {
      e.preventDefault();
      ...
      ephd_send_ajax(postData, function (response) {
        if (
          ...
          $(".ephd-ai-help-sidebar__alternatives-list").html(
            '<div class="ephd-ai-help-sidebar__alternative-answer">' +
              response.fixed_input_text +
              "</div>"
          );
        }
  ...
  $(document).on(
    "click",
    ".ephd-ai__answer-create-five-alternatives-btn",
    function (e) {
      e.preventDefault();
  ...
  $(document).on(
    "click",
    ".ephd-ai__create-answer-based-on-question-btn",
    function (e) {
      e.preventDefault();
      ...
      ephd_send_ajax(postData, function (response) {
        if (
          !response.error &&
          typeof response.message != "undefined" &&
          typeof response.generated_answer != "undefined" &&
          response.generated_answer.length > 0
        ) {
          ephd_show_success_notification(response.message);
          ai_help_sidebar_open_alternatives_screen(
            question,
  ...
  $(document).on(
    "click",
    ".ephd-ai-help-sidebar__alternatives-back-btn",
    function () {
  ...
  $(document).on(
    "click",
    ".ephd-ai-help-sidebar__alternative-question",
    function () {
        let HtmlContent = $( this ).html();
        HtmlContent = HtmlContent.replace(/^"|"$/g, '');

        $( '#ephd-fp__wp-editor__question-title' ).val( HtmlContent );
    }
  );
  ...
  $(document).on(
    "click",
    ".ephd-ai-help-sidebar__alternative-answer",
    function () {
      let editor = tinymce.get("ephd-fp__wp-editor");
      if (editor && $(".wp-editor-wrap").hasClass("tmce-active")) {
  ...
  function ai_help_sidebar_open_alternatives_screen(
    input_text,
    tokens_used,
    action_title
  ) {
    ...
    $(".ephd-ai-help-sidebar__alternatives-usage-tokens__value").html(
      tokens_used
    );
  ...
  function ephd_send_ajax(
    ...
    $.ajax({
      ...
      .done(function (response) {
        ...
        if (theResponse.error || typeof theResponse.message === "undefined") {
          ...
          errorMsg = theResponse.message
            ? theResponse.message
            : ephd_admin_notification("", ephd_vars.reload_try_again, "error");
        }
      })
      .fail(function (response, textStatus, error) {
        ...
        //noinspection JSUnresolvedVariable
        errorMsg = ephd_admin_notification(
          ephd_vars.error_occurred + ". " + ephd_vars.msg_try_again,
          errorMsg,
          "error"
        );
      })
  ...
  function ephd_loading_Dialog(displayType, message, $el) {
    if (displayType === "show") {
      let loadingClass =
        typeof $el == "undefined"
          ? ""
          : "ephd-admin-dialog-box-loading--relative";
      ...
      let output =
        ...
        "</div>" +
        "</div>" +
        '<div class="ephd-admin-dialog-box-overlay ' +
        loadingClass +
        '"></div>';
  ...
  $(document).on(
    "click",
    ".ephd-admin__input-disabled, .ephd__option-pro-tag",
    function () {
      let $tooltip = $(this)
        .closest(".ephd-input-group")
        .find(".ephd__option-pro-tooltip");
  ...
  $(document).on("click", function (e) {
    let target = $(e.target);
    if (
      !target.closest(".ephd__option-pro-tooltip").length &&
      !target.closest(".ephd-admin__input-disabled").length &&
      !target.closest(".ephd__option-pro-tag").length
    ) {
      $(".ephd__option-pro-tooltip").hide();
    }
  });
  ...
  function ephd_show_error_notification($message, $title = "") {
    $(".ephd-bottom-notice-message").remove();
    $("body #ephd-admin-page-wrap").append(
      ephd_admin_notification($title, $message, "error")
    );
  ...
  function ephd_show_success_notification($message, $title = "") {
    $(".ephd-bottom-notice-message").remove();
    $("body #ephd-admin-page-wrap").append(
      ephd_admin_notification($title, $message, "success")
    );
  ...
  function check_message_mode() {
    if (!$("#initial_message_mode").find("input:checked").length) {
      return;
    }
    if ($("#initial_message_mode").find("input:checked").val() === "icon_text") {
      $("#initial_message_image_url_group").show();
    } else {
      $("#initial_message_image_url_group").hide();
    }
  }
  function check_initial_dialog_visibility_mode() {
    // The HD PRO ad should not be affected by any setting
    if ($('#ephd-dialog-pro-feature-ad-initial-message').length) {
      return;
    }
    // Hide Initial Message settings if the widget is initially opened
    if ($('#dialog_initial_visibility').find("input:checked").val() === "off") {
        $("#ephd-initial-message").show();
      } else {
        $("#ephd-initial-message").hide();
      }
  }

  $("body").on("change", "#initial_message_mode input", check_message_mode);
  $("body").on("change", "#dialog_initial_visibility input", check_initial_dialog_visibility_mode);
});
