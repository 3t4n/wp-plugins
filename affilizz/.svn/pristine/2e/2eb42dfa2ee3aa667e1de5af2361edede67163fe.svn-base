...
class Ajax {
	...
	public function get_publication_contents( $context_id = false ) {
		...
		foreach ( $publication_api_object->containers as $container ) {
			$default_title = __( 'Deal', 'affilizz' );
			$truncate = true;
			if ( ! empty( trim( $container->tinyCode ) ) ) {
				$default_title = __( 'Deal', 'affilizz' ) . ' (<span class="affilizz-select-option--subtext">#' . $container->tinyCode . '</span>)';
				$truncate = false;
			}
			$name = trim( $container->name ) ?: $default_title;
			if ( $truncate ) {
				$name = substr( $name, 0, 30 ) . ( strlen( $name ) > 28 ? '…' : '' );
			}

			$publication_contents[$container->id] = array(
				'label' => esc_attr( $name ),
				'value' => $container->id,
				'name' => esc_attr( $name ),
				'selected' => in_array( $container->id, $selected_publication_contents ),
