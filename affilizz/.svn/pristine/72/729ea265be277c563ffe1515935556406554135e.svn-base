...
class Filters {
	...
	public function allow_affilizz_custom_tag_in_content( $tags, $context ) {
		...
		$attributes = [
			'affilizz-rendering-component' => [
				'loading', 'prevent-font-loading', 'subtag', 'design-id', 'render-api-url',
				'redirect-api-url', 'render-api-cache', 'page-session-id', 'custom-locale', 'custom-css',
				'publication-content-id', 'cacheable', 'client-cache'
			],
			'affilizz-deal-component' => [
				'loading', 'prevent-font-loading', 'subtag', 'design-id', 'render-api-url',
				'redirect-api-url', 'render-api-cache', 'page-session-id', 'custom-locale', 'custom-css',
				'content-container-id', 'content-container-tinyCode', 'deal-id', 'deal-tiny-code', 'client-cache'
			],
			'affilizz-magic-match' => [
				'loading', 'prevent-font-loading', 'subtag', 'design-id', 'render-api-url',
				'redirect-api-url', 'render-api-cache', 'page-session-id', 'custom-locale', 'custom-css',
			]
		];

		foreach ( $attributes as $tag => $tag_attributes ) {
			if ( ! isset( $tags[ $tag ] ) ) {
				$tags[ $tag ] = array();
			}

			foreach ( $tag_attributes as $attribute ) {
				if ( ! in_array( $attribute, $tags[ $tag ] ) ) {
	...
	public function allow_affilizz_custom_tag_in_editor( $init ) {
		...
		$added_tags_standalone = implode( ',', array_merge( $added_tags, array_map( function( $tag ) { return $tag . '[*]'; }, $added_tags ) ) );
		$added_tags_from_root = implode( ',', array_map( function( $tag ) { return '+body[' . $tag . ']'; }, $added_tags ) );

		// Add to extended_valid_elements if it already exists
		if ( ! isset( $init['extended_valid_elements'] ) ) {
			$init['extended_valid_elements'] = $added_tags_standalone;
		} else {
			$init['extended_valid_elements'] .= ',' . $added_tags_standalone;
		}

		if ( ! isset( $init['valid_children'] ) ) {
			$init['valid_children'] = $added_tags_from_root;
		} else {
			$init['valid_children'] .= ',' . $added_tags_from_root;
		}

		if ( ! isset( $init['custom_elements'] ) ) {
			$init['custom_elements'] = $added_tags_standalone;
		} else {
			$init['custom_elements'] .= ',' . $added_tags_standalone;
		}
