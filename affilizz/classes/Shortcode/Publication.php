		...
		if ( is_array( $publication_contents ) ) {
			foreach ( $publication_contents as $publication_content ) {
				$server_site_endpoint = add_query_arg(
					array( 'contextId' => wp_generate_uuid4() ), \Affilizz\Core::get_ssr_endpoint_root()
				);

				// Added in 1.14.6 : Support for the deals as they are located in a different child object
				if ( in_array( $publication_content->type, [ 'deal' ] ) ) {
					$server_site_endpoint = add_query_arg( array( 'contentContainerId' => $publication_content->id ), $server_site_endpoint );
				} else {
					$server_site_endpoint = add_query_arg( array( 'chrome' => 'false', 'publicationContentId' => $publication_content->id ), $server_site_endpoint );
				}

				if ( is_admin() || $current_rendering_mode == 'ssr' ) {
					// Added in 1.14.6 : Support for the deals as they are located in a different child object
					foreach( array( 'containers', 'contents' ) as $member ) {
						$children = ( isset( $publication_api_object->$member ) ? $publication_api_object->$member : array() );
						foreach ( $children as $child ) {
							if ( $publication_content->id != $child->id ) {
								continue;
							}
							switch ( strtolower( $child->type ) ) {
								case 'link':
									$href = trim( $child->redirectUrl ?? '#' );
									$title = trim( $child->items[0]->metadata->name ?? '' );
									$render = sprintf( '<a href="%1$s" title="%2$s">%3$s</a>', $href, esc_attr( $title ), $title );
									break;
								case 'box': case 'cta': case 'card':
								case 'carousel': case 'deal':
								default:
						...
						foreach ( $children as $child ) {
							...
							switch ( strtolower( $child->type ) ) {
								case 'link':
									...
									break;
								case 'deal':
									$render = sprintf( '<affilizz-deal-component deal-id="%1$s" loading="lazy" async></affilizz-deal-component>', $publication_content->id );
									break;
								case 'box':
								case 'cta':
								case 'card':
