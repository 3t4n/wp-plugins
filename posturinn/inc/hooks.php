...
function postis_hooks_rendering_extra_shipping_services() {
    ...
    if (is_array($chosen_methods)) {
        foreach ($chosen_methods as $method) {
            // Remove 'postis:' prefix if it exists
            $method = str_replace('postis:', '', $method);
            
            // Check if it's a postbox service (starts with DPO or DNO)
            if (strpos($method, 'DPO') === 0 || strpos($method, 'DNO') === 0) {
                echo '<tr>';
                ...
                echo '</td>';
                echo '</tr>';
                break; // Exit after finding first postbox service
            }
        }
    }
