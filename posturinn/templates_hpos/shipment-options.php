...
$is_dpo_selected = ($shipping_service === 'DPO' || $shipping_service === 'DNO');
        ...
        <div class="select-container" style="width:100%;">
            <div class="flex-field">
                ...
                <select id="postis-delivery-service-id" name="selected_delivery_service_id">
                    <option value="" disabled>Veldu sendingarmáta</option>
                    <?php 
                    if (isset($shipping_options['deliveryServicesAndPrices'])) {
                        foreach ($shipping_options['deliveryServicesAndPrices'] as $service) {
                            $is_selected = ($selected_delivery_service_id === $service['deliveryServiceId']) ? 'selected' : '';
                            printf(
                                '<option value="%s" %s>%s</option>',
                                esc_attr($service['deliveryServiceId']),
                                $is_selected,
                                esc_html($service['nameLong'])
                            );
                        }
                    }
                    ?>
            ...
            <div class="postis-phonenumber-section flex-field">
                ...
                <div>
                    <?php
                    $customer_phone = str_replace('+354', '', $wc_order->get_billing_phone()); // todo
                    $suggested_phone = '';
                    ...
                    $is_valid_mobile = preg_match('/^[45678]\d{6}$/', $suggested_phone);

                    
                    if ("DPO" === $shipping_service) {
                        $is_valid_mobile = preg_match('/^[678]\d{6}$/', $suggested_phone);
                    }
                    ?>
                    ...
                    <?php if (!$is_valid_mobile && !empty($suggested_phone)) : ?>
                        <span style="color: rgb(241, 5, 5);">
                            <?php
                            if ("DPO" === $shipping_service) {
                                _e('Parcelpoint, only Icelandic mobile phone numbers starting with 6, 7, or 8 are allowed', "postis");
                            } else {
                                _e('Only Icelandic phone numbers starting with 4, 5, 6, 7, or 8 are allowed', "postis");
                            }
                            ?>
                        </span>
                    <?php endif; ?>
