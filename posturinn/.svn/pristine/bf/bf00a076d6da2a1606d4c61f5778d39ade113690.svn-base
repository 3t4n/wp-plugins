...
if ( class_exists( 'WC_Shipping_Method' ) ) {
	...
	class POSTIS_Shipping extends WC_Shipping_Method {
		...
		function __construct(){
			...
			$this->domain = postis_get_homeurl();
			...
			add_filter( 'woocommerce_package_rates', array($this, 'hide_shipping_method'), 100, 2 );
		...
		function get_services_rates($services) {
		    ...
		    $has_tax = ($include_wc_vat === 'yes');
		    
		    $postis_services = isset($services['deliveryServicesAndPrices']) ? $services['deliveryServicesAndPrices'] : null;
		    if (!$postis_services) return false;
		    
		    $rates = array();
		    $show_shipping_description = postis_get_settings('show_shipping_description');

		    // Store DPO/DNO services for the dropdown
		    $postbox_services = array();
		    $has_postbox_service = false;
		    $postbox_base_price = null;

		    foreach ($postis_services as $service) {
		        $service_id = $service['deliveryServiceId'];
		        $base_service_id = substr($service_id, 0, 3);

		        // If this is a postbox service (DPO or DNO)
		        if ($base_service_id === 'DPO' || $base_service_id === 'DNO') {
		            // Store for dropdown generation
		            $postbox_services[] = $service;
		            
		            // Get the base price if not set
		            if ($postbox_base_price === null) {
		                $postbox_base_price = $service['priceRelated'][$price_method];
		            }

		            // Only add the generic postbox rate once
		            if (!$has_postbox_service) {
		                $rates[] = array(
		                    'id' => 'DPO', // Use generic DPO ID
		                    'cost' => round($postbox_base_price),
		                    'label' => 'Pósturinn - Pakki Póstbox',
		                    'taxes' => $has_tax,
		                    'meta_data' => $show_shipping_description !== 'no' ? array(
		                        'description' => $service['description']
		                    ) : array()
		                );
		                $has_postbox_service = true;
		            }
		        } else {
		            // Handle other services normally
		            $rates[] = array(
		                'id' => $service_id,
		                'cost' => round($service['priceRelated'][$price_method]),
		                'label' => 'Pósturinn - ' . $service['nameLong'],
		                'taxes' => $has_tax,
		                'meta_data' => $show_shipping_description !== 'no' ? array(
		                    'description' => $service['description']
		                ) : array()
		    ...
		    // Store postbox services in session for the dropdown
		    if (!empty($postbox_services)) {
		        WC()->session->set('postis_postbox_services', $postbox_services);
		    }
		...
		*/
		...
		function hide_shipping_method($rates, $package) {
		    ...
		    foreach ($this->postis_services as $code => $shipment_meta) {
		        if (isset($shipment_meta['free_shipping']) && 
		            $shipment_meta['free_shipping'] != '' && 
		            $shipment_meta['free_shipping'] > 0 && 
		            isset($shipment_meta['enabled']) && 
		            $shipment_meta['enabled'] == 'on') {

		            if ($total > $shipment_meta['free_shipping']) {
		                foreach ($rates as $rate_id => $rate) {
		                    // Strip numbers and handle both DPO and DNO
		                    $base_service_id = preg_replace('/[0-9]+/', '', $rate_id);
		                    if ($base_service_id === $code || 
		                        ($code === 'DPO' && $base_service_id === 'DNO')) {
