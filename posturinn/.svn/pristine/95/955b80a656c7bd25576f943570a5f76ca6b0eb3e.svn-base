...
function postis_get_endpoints( $type ) {
	...
	switch( $type ) {
		...
		case 'pdf':
			if( postis_get_plugin_mode() == 'live' ) {
				$postis_endpoint = 'https://api.mobiz.posturinn.is/api/wscm/v1/shipments/';
			} else {
		...
		case 'create_shipment':
			if( postis_get_plugin_mode() == 'live' ) {
				$postis_endpoint = 'https://api.mobiz.posturinn.is/api/wscm/v1/shipments/create';
			} else {
		...
		case 'calculate_shipping':
			if( postis_get_plugin_mode() == 'live' ) {
				$postis_endpoint = 'https://api.mobiz.posturinn.is/api/wscm/v1/deliveryservicesandprices';
			} else {
		...
		case 'postboxes':
			if( postis_get_plugin_mode() == 'live' ) {
				$postis_endpoint = 'https://api.mobiz.posturinn.is/api/wscm/v1/postboxes';
			} else {
		...
		case 'parcelpoints':
			if( postis_get_plugin_mode() == 'live' ) {
				$postis_endpoint = 'https://api.mobiz.posturinn.is/api/wscm/v1/parcelpoints';
			} else {
		...
		case 'print':
			if( postis_get_plugin_mode() == 'live' ) {
				$postis_endpoint = 'https://api.mobiz.posturinn.is/api/wscm/v1/print';
			} else {
...
function postis_get_dpo_postboxes() {
    $postbox_services = WC()->session->get('postis_postbox_services', array());
    
    if (!empty($postbox_services)) {
        $postboxes_array = array('' => __('--- Choose postbox ----', 'postis'));
        foreach ($postbox_services as $service) {
            $postboxes_array[$service['deliveryServiceId']] = $service['nameLong'];
        }
...
// New WC logger
function postis_log($message, $level = 'debug') {
    static $logger = null;

    if (!defined('POSTIS_REQUEST_ID')) {
        define('POSTIS_REQUEST_ID', substr(uniqid(), -4));
    }
    ...
    $ms = substr($micro[1], 0, 3);
    $timestamp = date('Y-m-d\TH:i:s') . '.' . $ms;
    
    $logger->$level(
        sprintf('[%s][Request-%s] %s', $timestamp, POSTIS_REQUEST_ID, $message),
        ['source' => 'postis-timer']
    );
}