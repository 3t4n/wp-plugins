...
class POSTIS_MAIN_CLASS {
    ...
    function __construct(){
        ...
        add_filter('woocommerce_package_rates', array($this, 'maybe_add_initial_rates'), 100, 2);
    }
    ...
    */
    ...
    public function maybe_add_initial_rates($rates, $package) {
        ...
        if (!$our_rates_exist && empty($package['destination']['postcode'])) {
            ...
            //$rates = array_merge($custom_rates, $rates);

        } else {
            //error_log('Our rates already exist or postcode is set');
        }

        // Get the value of the 'shipping_top' setting
        $shipping_top = postis_get_settings('shipping_top');

        // Merge and sort rates based on 'shipping_top' setting
        if ($shipping_top === 'yes') {
            // Merge custom rates at the top if shipping_top is 'yes'
            $rates = array_merge($custom_rates, $rates);
        } else {
            // Merge custom rates at the bottom if shipping_top is not 'yes'
            $rates = array_merge($rates, $custom_rates);
        }
