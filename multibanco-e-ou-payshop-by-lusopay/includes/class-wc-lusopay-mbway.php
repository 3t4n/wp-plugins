    ...
    function order_details_after_order_table( $order ) {
      ...
      dbDelta( "CREATE TABLE IF NOT EXISTS `" . $table_name . "` (
		...
		function process_payment( $order_id ) {
			...
			$mess=$response[3];
			$status=$response[2];
			//$texto = implode(", ", $response);
			//throw new Exception( __( $texto, 'woo' ) );
			if($status=='000') {
			// Mark as on-hold.
					...
					if ($val >= 0.15) {
						...
						if ($statuscode === '000') {
							...
							if ( 0 !== $id ) {
								...
								$order->update_status( $order->has_downloadable_item() ? 'completed' : 'processing', __( 'Payment received by MB Way.', 'lusopaygateway' ) );
