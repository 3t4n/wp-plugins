...
var e2wl_chrome_extension_loaded = false;
document.addEventListener('maAPILoaded', function () {
    jQuery("#chrome-notify").hide();
    e2wl_chrome_extension_loaded = true;
});
...
function e2wl_tmce_getContent(editor_id, textarea_id) {
...
function show_notification(text, danger) {
    ...
    if (jQuery(".e2wl-content .alert").length === 0) {
        jQuery('<div class="alert" role="alert">').prependTo(jQuery(".e2wl-content"));
    }

    var alert = jQuery(".e2wl-content .alert");
...
function update_product_info(data, on_update_calback) {
    data.action = 'e2wl_update_product_info';
...
function update_variation_info(data, on_update_calback) {
    var on_update_calback_call_cnt = 0;
    if (data.variations.length > 0) {
        var post_data = { action: 'e2wl_update_variation_info', id: data.id, variations: data.variations.splice(0, 100) }
        jQuery.post(ajaxurl, post_data).done(function (response) {
            ...
            if (on_update_calback && on_update_calback_call_cnt == 0) {
                on_update_calback_call_cnt++;
                on_update_calback();
            }
...
function chech_products_view() {
    ...
    if (products_to_load.length > 0) {
        var data = { 'action': 'e2wl_load_shipping_info', 'id': products_to_load };
        jQuery.post(ajaxurl, data).done(function (response) {
            var json = jQuery.parseJSON(response);
            if (json.state !== 'ok') {
                console.log(json);
...
(function ($, window, document, undefined) {
    $(function () {
        if (e2wl_chrome_extension_loaded === false) {
            $("#chrome-notify").show();
        }
        ...
        $("#e2wl-do-filter").click(function () {
            $(this).closest('form').submit();
            return false;
        });

        jQuery("#e2wl-search-form").submit(function () {
        ...
        $("#e2wl-sort-selector").change(function () {
            jQuery("#e2wl-search-form #e2wl_sort").val($(this).val());
            jQuery("#e2wl-search-form").submit();
            return false;
        });

        $("#e2wl-search-pagination li a").click(function () {
            jQuery("#e2wl-search-form #cur_page").val($(this).attr('rel'));
            jQuery("#e2wl-search-form").submit();
        ...
        $('#modal-country-select').on('change', function (evt) {
            ...
            if (tmp_data.country != $(this).val()) {
                ...
                $('.modal-shipping .shipping-method').html('<div class="e2wl-load-container"><div class="e2wl-load-speeding-wheel"></div></div>');

                var data = { 'action': 'e2wl_load_shipping_info', 'id': tmp_data.product_id, 'country': tmp_data.country };
                jQuery.post(ajaxurl, data).done(function (response) {
                    var json = jQuery.parseJSON(response);
                    if (json.state !== 'ok') {
                        console.log(json);
        ...
        function e2wl_js_add_to_import(products_to_import, state, befor_load_calback, on_load_calback) {
            if (products_to_import.length > 0) {
                ...
                jQuery.post(ajaxurl, data).done(function (response) {
                    ...
                    e2wl_js_add_to_import(products_to_import, state, befor_load_calback, on_load_calback);
                }).fail(function (xhr, status, error) {
                    console.log(error);
                    state.import_error_cnt++;
        ...
        $('.e2wl-content .import_all').click(function () {
            var this_btn = $(this);
            $('.modal-confirm').confirm_modal({
                ...
                yes: function () {
                    ...
                    $('.product-card:not(.product-card--added)').each(function () {
                        products_to_import.push({ 'action': 'e2wl_add_to_import', 'id': $(this).attr('data-id') });
                    });
                    if (products_to_import.length > 0) {
                        ...
                        e2wl_js_add_to_import(products_to_import, state, on_befor_import, on_import_load);
                        e2wl_js_add_to_import(products_to_import, state, on_befor_import, on_import_load);
                    }
                }
            });
        ...
        $(".product-card").find(".product-card__actions .btn").on("click", function () {
            ...
            if ($(_this).closest(".product-card").hasClass('product-card--added')) {
                ...
                var data = { 'action': 'e2wl_remove_from_import', 'id': $(_this).closest(".product-card").attr('data-id') };
                jQuery.post(ajaxurl, data).done(function (response) {
                    var json = jQuery.parseJSON(response);
                    if (json.state !== 'ok') {
                        console.log(json);
                ...
                products_to_import = [];
                products_to_import.push({ 'action': 'e2wl_add_to_import', 'id': $(_this).closest(".product-card").attr('data-id') });
        ...
        $("#import-by-id-url-btn").on("click", function () {
            ...
            if (id_value) {
                ...
                if (!product_id) {
                    var result = url_value.match(/.*\/([0-9]+)(\?|\s*$)/i);
                    if (result && result.length > 1) {
                        product_id = result[1];
                    }
                }
            ...
            if (product_id) {
                ...
                products_to_import = [];
                products_to_import.push({ 'action': 'e2wl_add_to_import', 'id': product_id });
                ...
                show_notification('Failed to get ID from URL ', true);
            }
        });
        ...
        function update_bulk_actions() {
            if ($('.e2wl-product-import-list .select :checkbox:checked').length > 0) {
                $("#e2wl-import-actions .action-with-check select option").first().text('Bulk Actions (' + $('.e2wl-product-import-list .select :checkbox:checked').length + ' selected)');
        ...
        $('#e2wl-import-actions .check-all').change(function () {
            var checkboxes = $('.e2wl-product-import-list .select :checkbox').not($(this));
            if ($(this).is(':checked')) {
        ...
        $('.e2wl-product-import-list [rel="images"] .image').click(function () {
        ...
        $('.e2wl-product-import-list [rel="images"] .image .icon-gallery-box').click(function () {
            $(this).toggleClass('selected');
            $('.e2wl-product-import-list [rel="images"] .image .icon-gallery-box').not(this).removeClass('selected');
        ...
        $('.e2wl-product-import-list .select :checkbox').change(function () {
            update_bulk_actions();
        });

        jQuery(".e2wl-product-import-list").on("click", ".post_import", function () {
            var this_btn = this;
            var product = $(this).parents('.product');
            var products_to_import = [];
            var data = { 'action': 'e2wl_push_product', 'id': $(product).attr('data-id') };
            products_to_import.push(data);

            $(this_btn).addClass('load');
            var on_load = function (id, response_state, response_message, state) {
                if (response_state !== 'error') {
                    $('.e2wl-product-import-list .select :checkbox[value="' + id + '"]').parents('.product').parents('.row').remove();
                    if ($('.e2wl-product-import-list').find('.product').length === 0) {
                        $('#e2wl-import-content').hide();
                        $('#e2wl-import-empty').show();
                    }
            ...
            var state = { num_to_import: products_to_import.length, import_cnt: 0, import_error_cnt: 0 };
            e2wl_js_post_to_woocomerce(products_to_import, state, on_load);

            return false;
        });


        $('#e2wl-import-actions .action-with-check select').change(function () {
            var ids = [];
            $('.e2wl-product-import-list .select :checkbox:checked:not(:disabled)').each(function () {
                ids.push($(this).val());
            });
            ...
            if ($(this).val() === 'remove') {
                ...
                $('.modal-confirm').confirm_modal({
                    title: 'Remove ' + $('.e2wl-product-import-list .select :checkbox:checked').length + ' products',
                    body: 'Are you sure you want to remove ' + $('.e2wl-product-import-list .select :checkbox:checked').length + ' products from your import list?',
                    yes: function () {
                        $('#e2wl-import-actions .action-with-check').addClass('load');
                        $("#e2wl-import-actions .action-with-check select").prop('disabled', true);

                        var data = { 'action': 'e2wl_delete_import_products', 'ids': ids };
                        jQuery.post(ajaxurl, data).done(function (response) {
                            var json = jQuery.parseJSON(response);
                            if (json.state !== 'ok') {
                                console.log(json);
                            }
                            $('#e2wl-import-actions .action-with-check').removeClass('load');
                            $("#e2wl-import-actions .action-with-check select").prop('disabled', false);
                            $('#e2wl-import-actions .action-with-check select').val(0);
                            $.each(ids, function (i, item) {
                                $('.e2wl-product-import-list .select :checkbox[value="' + item + '"]').closest('.product').parents('.row').remove();
                            });
                            update_bulk_actions();
                ...
                $('.modal-confirm').confirm_modal({
                    title: 'Push ' + $('.e2wl-product-import-list .select :checkbox:checked').length + ' products',
                    body: 'Are you sure you want to push ' + $('.e2wl-product-import-list .select :checkbox:checked').length + ' products to woocommerce?',
                    yes: function () {
                        $('#e2wl-import-actions .action-with-check').addClass('load');
                        $("#e2wl-import-actions .action-with-check select").prop('disabled', true);
                        ...
                        var products_to_import = [];
                        $.each(ids, function (i, item) {
                            products_to_import.push({ 'action': 'e2wl_push_product', 'id': item });
                        });

                        $('.e2wl-product-import-list .select :checkbox:checked:not(:disabled)').closest('.product').find('.post_import').addClass('load');
                        ...
                        var on_load = function (id, response_state, response_message, state) {
                            ...
                            if (response_state !== 'error') {
                                $('.e2wl-product-import-list .select :checkbox[value="' + id + '"]').closest('.product').parents('.row').remove();
                            }
        ...
        $('#e2wl-import-actions .link_category_all').click(function () {
            $(".set-category-dialog .categories").attr('data-link-type', 'all');
            $(".set-category-dialog").fadeIn(200);
            return false;
        });

        $('#e2wl-import-actions .delete_all').click(function () {
        ...
        $('#e2wl-import-actions .push_all').click(function () {
            var this_btn = $(this);
            $('.modal-confirm').confirm_modal({
                ...
                yes: function () {
                    $(this_btn).addClass('load');
                    $('.e2wl-product-import-list .select :checkbox:not(:disabled)').closest('.product').find('.post_import').addClass('load');

                    var data = { 'action': 'e2wl_get_all_products_to_import' };
                    jQuery.post(ajaxurl, data).done(function (response) {
                        var json = jQuery.parseJSON(response);
                        if (json.state !== 'ok') {
                            console.log(json);
                        ...
                        $.each(json.ids, function (i, id) {
                            products_to_import.push({ 'action': 'e2wl_push_product', 'id': id });
                        });
                        ...
                        var on_load = function (id, response_state, response_message, state) {
                            ...
                            if (response_state !== 'error') {
                                ...
                                $('.e2wl-product-import-list .select :checkbox[value="' + id + '"]').closest('.product').find('.post_import').removeClass('load');
                            }
        ...
        function e2wl_js_post_to_woocomerce(products_to_import, state, on_load_calback) {
            if (products_to_import.length > 0) {
                var data = products_to_import.shift();

                var product = $('.product[data-id="' + data.id + '"]');
                var need_update_product = $(product).attr('data-need-update-product');
                var need_update_variation = $(product).attr('data-need-update-variation');
                ...
                if (typeof need_update_product !== typeof undefined && need_update_product !== false) {
                    ...
                    if (typeof tinyMCE !== 'undefined') { update_data['description'] = encodeURIComponent(e2wl_tmce_getContent(data.id)); }
                    update_product_info(update_data, function () {
                        $(product).removeAttr('data-need-update-product');
                        jQuery.post(ajaxurl, data).done(function (response) {
                            var json = jQuery.parseJSON(response);
                            if (json.state !== 'ok') {
                                console.log(json);
                            ...
                            e2wl_js_post_to_woocomerce(products_to_import, state, on_load_calback);
                        }).fail(function (xhr, status, error) {
                            console.log(error);
                            state.import_error_cnt++;
                ...
                } else if (typeof need_update_variation !== typeof undefined && need_update_variation !== false) {
                    var update_data = { id: $(product).attr('data-id'), variations: [] };
                    $(product).find('.variants-table tbody tr').each(function () {
                        ...
                        update_data.variations.push(variation);
                    });
                    update_variation_info(update_data, function () {
                        $(product).removeAttr('data-need-update-variation');
        ...
        $('.e2wl-content .product input.title').change(function () {
            var product = $(this).parents('.product');
        ...
        $(".e2wl-content .product .select2, .e2wl-content .product .select2-tags").on("select2:select", function (e) {
            var product = $(this).parents('.product');
            $(product).attr('data-need-update-product', true);
            update_product_info({ id: $(product).attr('data-id'), title: $(product).find('.title').val(), tags: $(product).find('.tags').val(), type: $(product).find('.type').val(), status: $(product).find('.status').val(), categories: $(product).find('.categories').val() }, function () { $(product).removeAttr('data-need-update-product'); });
        });
        $(".e2wl-content .product .select2, .e2wl-content .product .select2-tags").on("select2:unselect", function (e) {
            var product = $(this).parents('.product');
            $(product).attr('data-need-update-product', true);
            update_product_info({ id: $(product).attr('data-id'), title: $(product).find('.title').val(), tags: $(product).find('.tags').val(), type: $(product).find('.type').val(), status: $(product).find('.status').val(), categories: $(product).find('.categories').val() }, function () { $(product).removeAttr('data-need-update-product'); });
        });

        $('.e2wl-content .variants-table .var_data input[type="text"]').change(function () {
            var product = $(this).closest('.product');
            $(product).attr('data-need-update-variation', true);
            waitForFinalEvent(function () {
                ...
                update_variation_info(data, function () { $(product).removeAttr('data-need-update-variation'); });
        ...
        $('.e2wl-content .price-edit-selector .dropdown-menu a').click(function () {
        ...
        $('.e2wl-content .price-edit-selector .price-box-top .apply').click(function () {
        ...
        $('.e2wl-content .price-edit-selector .price-box-top .close').click(function () {
            $(this).parents('.price-box-top').hide();
        });

        $('.e2wl-content .variants-actions .disable-var-price-change').change(function () {
            var $product = $(this).closest('.product');
        ...
        $('.e2wl-content .variants-actions .disable-var-quantity-change').change(function () {
            var $product = $(this).closest('.product');
        ...
        var e2wl_external_images_page_size = 1000;

        function e2wl_calc_external_images(on_load_calback) {
            var data = { 'action': 'e2wl_calc_external_images', 'page_size': e2wl_external_images_page_size };
            jQuery.post(ajaxurl, data).done(function (response) {
                var json = jQuery.parseJSON(response);

                var block_data = $('#e2wl_load_external_image_block').data();
        ...
        var e2wl_calc_external_images_calback = function (ids, block_data) {
            images_to_load = [];
            jQuery.each(ids, function (i, id) {
                images_to_load.push({ 'action': 'e2wl_load_external_image', 'id': id });
            });

            var on_load = function (state) {
                $("#e2wl_load_external_image_progress").html(sprintf(e2wl_common_data.lang.process_loading_d_of_d_erros_d, block_data.ok + state.import_cnt, block_data.total, block_data.error + state.import_error_cnt));

                if ((state.import_cnt + state.import_error_cnt) === state.num_to_import) {
                    block_data.ok += state.import_cnt;
                    block_data.error += state.import_error_cnt;

                    $('#e2wl_load_external_image_block').data(block_data);

                    if ((block_data.ok + block_data.error) < block_data.total) {
                        e2wl_calc_external_images(e2wl_calc_external_images_calback);
                    } else {
                        location.reload();
                    }
                }
            ...
            e2wl_load_external_image(images_to_load, state, on_load);
        };

        function e2wl_load_external_image(images_to_load, state, on_load) {
        ...
        $('#e2wl_load_external_image_block .load-images').click(function () {
            $('#e2wl_load_external_image_block .load-images').attr('disabled', 'disabled');
            var block_data = $('#e2wl_load_external_image_block').data();
            $("#e2wl_load_external_image_progress").html(sprintf(e2wl_common_data.lang.process_loading_d_of_d_erros_d, 0, block_data.total, 0));
            e2wl_calc_external_images(e2wl_calc_external_images_calback);
        });
        ...
        $('.set-category-dialog .modal-footer .btn').click(function () {
            if ($(this).hasClass('no-btn')) {
                ...
                if (new_categories) {
                    if (link_type === 'all') {
                        var ids = 'all';
                        $('.e2wl-product-import-list .product select.categories').each(function () {
                    ...
                    jQuery.post(ajaxurl, { 'action': 'e2wl_link_to_category', 'categories': new_categories, 'ids': ids }).fail(function (xhr, status, error) { console.log(error); });
                }
                $(this).parents(".modal-overlay").fadeOut(200);
            }
            return false;
        ...
        // init load button
        $('#e2wl_load_external_image_block').data({ total: 0, ok: 0, error: 0 });
        var data = { 'action': 'e2wl_calc_external_images_count' };
        jQuery.post(ajaxurl, data).done(function (response) {
            var json = jQuery.parseJSON(response);

            $('#e2wl_load_external_image_block').data({ total: json.total_images, ok: 0, error: 0 });
            if (json.total_images > 0) {
                $('#e2wl_load_external_image_block .load-images').removeAttr('disabled');
                $('#e2wl_load_external_image_block .load-images').val(sprintf(e2wl_common_data.lang.load_button_text, json.total_images));
            } else {
                $("#e2wl_load_external_image_progress").html(e2wl_common_data.lang.all_images_loaded_text);
            }
        ...
        $('#e2wl-import-filter form select[name="o"]').change(function () {
