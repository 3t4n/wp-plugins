...
/**
 * Description of E2WL_PriceFormula
 *
 * @author Andrey
 */
class E2WL_PriceFormula {
    ...
    public function load($id = false) {
        ...
        if ($load_id) {
            $formula_list = E2WL_PriceFormula::load_formulas_list(false);
    ...
    public function save() {
        ...
        e2wl_set_setting('formula_list', array_values($formula_list));
        return $this;
    }
    ...
    public static function deleteAll() {
        e2wl_del_setting('formula_list');
    }

    public static function normalize_product_price($product) {
        $product_price = false;
        if (isset($product['price']) && floatval($product['price'])) {
            $product_price = array('price'=>floatval($product['price']), 'regular_price'=>floatval($product['regular_price']));
        } else if (isset($product['price_min']) && floatval($product['price_min'])) {
            $product_price = array('price'=>floatval($product['price_min']), 'regular_price'=>floatval($product['price_min']));
        } else if (isset($product['price_max']) && floatval($product['price_max'])) {
            $product_price = array('price'=>floatval($product['price_max']), 'regular_price'=>floatval($product['price_max']));
        }
        return $product_price;
    }

    public static function apply_formula($product, $round = 2, $type = 'all') {
        $formula = E2WL_PriceFormula::get_formula_by_product($product);

        $product_price = E2WL_PriceFormula::normalize_product_price($product);
        
        if ($formula && $product_price) {
            $use_compared_price_markup = e2wl_get_setting('use_compared_price_markup');

            $price_cents = e2wl_get_setting('price_cents');
            $price_compared_cents = e2wl_get_setting('price_compared_cents');
            
            if ($type === 'all' || $type === 'price' || !isset($product['calc_price'])) {
                if ($formula->sign == "=") {
                    $product['calc_price'] = round(floatval($formula->value), $round);
                } else if ($formula->sign == "*") {
                    $product['calc_price'] = round(floatval($product_price['price']) * floatval($formula->value), $round);
                } else if ($formula->sign == "+") {
                    $product['calc_price'] = round(floatval($product_price['price']) + floatval($formula->value), $round);
                }
            ...
            if ($type === 'all' || $type === 'regular_price' || !isset($product['calc_regular_price'])) {
                ...
                if ($use_compared_price_markup) {
                    if ($formula->compared_sign == "=") {
                        ...
                        $product['calc_regular_price'] = round(floatval($product_price['regular_price']) * floatval($formula->compared_value), $round);
                    } else if ($formula->compared_sign == "+") {
                        $product['calc_regular_price'] = round(floatval($product_price['regular_price']) + floatval($formula->compared_value), $round);
                    }
                }
            ...
            if (isset($product['sku_products']['variations']) && $product['sku_products']['variations']) {
                foreach ($product['sku_products']['variations'] as &$var) {
                    $formula = E2WL_PriceFormula::get_formula_by_product($var);
                    if ($formula) {
                        ...
                        if ($type === 'all' || $type === 'regular_price' || !isset($var['calc_regular_price'])) {
                            ...
                            if ($use_compared_price_markup) {
                                if ($formula->compared_sign == "=") {
                                    ...
                                    $var['calc_regular_price'] = round(floatval($var['regular_price']) * floatval($formula->compared_value), $round);
                                } else if ($formula->compared_sign == "+") {
                                    $var['calc_regular_price'] = round(floatval($var['regular_price']) + floatval($formula->compared_value), $round);
                                }
                            }
    ...
    public static function get_default_formula() {
        $formula = e2wl_get_setting('default_formula');
        return new E2WL_PriceFormula($formula && is_array($formula) ? $formula : array('value' => 1, 'sign' => '*', 'compared_value' => 1, 'compared_sign' => '*'));
    }

    public static function set_default_formula($formula) {
        e2wl_set_setting('default_formula', get_object_vars($formula));
    }

    public static function get_default_formulas() {
        $f1 = new E2WL_PriceFormula();
        $f1->id = 1;
        $f1->min_price = 0;
        $f1->max_price = 10;
        $f1->sign = "*";
    ...
    public static function get_formula_by_product($product) {
        ...
        $use_extended_price_markup = e2wl_get_setting('use_extended_price_markup');
        if ($use_extended_price_markup) {
            
            $product_price = E2WL_PriceFormula::normalize_product_price($product);
            if($product_price){
                $formula_list = E2WL_PriceFormula::load_formulas_list();
                foreach ($formula_list as $formula) {
                    $check = true;

                    if (isset($formula->min_price) && $formula->min_price && floatval($formula->min_price) > $product_price['price']) {
                        $check = false;
                    }

                    if (isset($formula->max_price) && $formula->max_price && floatval($formula->max_price) < $product_price['price']) {
                        $check = false;
                    }
        ...
        return $res_formula ? $res_formula : E2WL_PriceFormula::get_default_formula();
    }
}
