...
/**
 * Description of E2WL_WooCommerceOrderItem
 *
 * @author MA_GROUP
 */
if (!class_exists('E2WL_WooCommerceOrderItem')):

	class E2WL_WooCommerceOrderItem {
		private $orderItem;
		...
		public function get_E2WL_ShippingTitle() {
			$shipping_title = '';
		...
		public function get_E2WL_ShippingCode() {
			$shipping_code = '';

			if (isset($this->orderItem['item_meta']['e2wl_shipping_code'])) {
				if (is_array($this->orderItem['item_meta']['e2wl_shipping_code'])) {
					$shipping_code = $this->orderItem['item_meta']['e2wl_shipping_code'][0];
				} else {
					$shipping_code = $this->orderItem['item_meta']['e2wl_shipping_code'];
				}
			}
