...
/**
 * Description of E2WL_Ebay
 *
 * @author Andrey
 */
if (!class_exists('E2WL_Ebay')) {
    ...
    class E2WL_Ebay
    {
        ...
        public function __construct()
        {
            ...
            $this->account = E2WL_Account::getInstance();
        }
        ...
        public function load_products($filter, $page = 1, $per_page = 20, $params = array())
        {
            ...
            $request_url = E2WL_RequestHelper::build_request('get_products', array_merge(array('page' => $page, 'per_page' => $per_page), $filter));
            // error_log($request_url);
            $request = e2wl_remote_get($request_url);
            // error_log(print_r($request,true));

            if (is_wp_error($request)) {
                $result = E2WL_ResultBuilder::buildError($request->get_error_message());
            } else if (intval($request['response']['code']) != 200) {
                $result = E2WL_ResultBuilder::buildError($request['response']['message']);
            } else {
                $result = json_decode($request['body'], true);
                // error_log(print_r($result,true));

                if ($result['state'] === 'error' && substr($result['message'], 0, 7) === "[11002]") {
                    $result = E2WL_ResultBuilder::buildError("Input correct Client ID and Client Secret on the Setting > Account settings page");
                } else if (isset($result['state']) && $result['state'] !== 'error') {
                    $default_type = e2wl_get_setting('default_product_type');
                    $default_status = e2wl_get_setting('default_product_status');
        ...
        {
            $token = $this->account->get_access_token();
            if ($token['state'] === 'error') {
                return E2WL_ResultBuilder::buildError($token['message']);
            }
            ...
            $request_url = E2WL_RequestHelper::build_request('get_product', array('product_id' => $product_id, 'site_id' => $site_id));
            $request_url = $request_url . "&access_token=" . urlencode($token['access_token']);
            // error_log($request_url);
            // $request = e2wl_remote_post($request_url,array('access_token'=>$token['access_token']));
            $request = e2wl_remote_get($request_url);
            ...
            if (is_wp_error($request)) {
                ...
                if ($result['state'] !== 'error') {
                    ...
                    $site = E2WL_EbaySite::get_site_by_id($site_id);
                    $result['product']['globalId'] = $site->sitecode;

                    if (e2wl_get_setting('use_random_stock')) {
                        ...
                        foreach ($result['product']['sku_products']['variations'] as &$variation) {
                            ...
                            $tmp_quantity = rand(intval(e2wl_get_setting('use_random_stock_min')), intval(e2wl_get_setting('use_random_stock_max')));
                    ...
                    $tmp_description = '';
                    if (defined('E2WL_SAVE_ATTRIBUTE_AS_DESCRIPTION') && E2WL_SAVE_ATTRIBUTE_AS_DESCRIPTION) {
                    ...
                    if (!e2wl_get_setting('not_import_description')) {
                        $tmp_description .= $this->clean_description($result['product']['description']);
                    }

                    $result['product']['description'] = E2WL_PhraseFilter::apply_filter_to_text($tmp_description);

                    $tmp_all_images = E2WL_Utils::get_all_images_from_product($result['product']);

                    $not_import_gallery_images = false;
                    $not_import_variant_images = false;
                    $not_import_description_images = e2wl_get_setting('not_import_description_images');

                    if (e2wl_get_setting('сonvert_images_to_large')) {
        ...
        {
            $request_url = E2WL_RequestHelper::build_request('get_reviews', array('lang' => E2WL_EbayLocalizator::getInstance()->language, 'product_id' => $product_id, 'page' => $page, 'page_size' => $page_size));
            ...
            if (is_wp_error($request)) {
                ...
                if ($result['state'] !== 'error') {
                    $result = E2WL_ResultBuilder::buildOk(array('reviews' => isset($result['reviews']['evaViewList']) ? $result['reviews']['evaViewList'] : array(), 'totalNum' => isset($result['reviews']['totalNum']) ? $result['reviews']['totalNum'] : 0));
                }
            }
        ...
        {
            $request_url = E2WL_RequestHelper::build_request('get_categories', array('site_id' => $site_id));

            $request = e2wl_remote_get($request_url, array('timeout' => 120));
            ...
            if (is_wp_error($request)) {
                ...
                $result = json_decode($request['body'], true);
                if ($result['state'] !== 'error') {
                    E2WL_Categories::save_categories($site_id, $result['categories']);
                }
        ...
        public function sync_products($product_ids, $params = array())
        {
            ...
            $request_url = E2WL_RequestHelper::build_request('sync_products', $request_params);
            $request_url = $request_url . "&access_token=" . urlencode($token['access_token']);
            // error_log($request_url);
            $request = e2wl_remote_get($request_url);
            if (is_wp_error($request)) {
                ...
                $use_random_stock = e2wl_get_setting('use_random_stock');
                if ($use_random_stock) {
                    $random_stock_min = intval(e2wl_get_setting('use_random_stock_min'));
                    $random_stock_max = intval(e2wl_get_setting('use_random_stock_max'));
        ...
        public function load_shipping_info($product_id, $quantity, $country_code, $country_code_form = '')
        {
            ...
            $request_url = E2WL_RequestHelper::build_request('get_shipping_info', array('product_id' => $product_id, 'quantity' => $quantity, 'country_code' => $country_code, 'country_code_from' => $country_code_form));
            ...
            if (is_wp_error($request)) {
                ...
                if (intval($request['response']['code']) == 200) {
                    ...
                    $result = E2WL_ResultBuilder::buildError($request['response']['code'] . ' - ' . $request['response']['message']);
                }
            }
        ...
        public static function clean_description($description)
        {
            ...
            $tags = apply_filters('e2wl_clean_description_tags', array('script', 'head', 'meta', 'style', 'map', 'noscript', 'object', 'iframe'));
            ...
            if (!in_array('img', $tags)) {
                ...
                for ($i = $elements->length; --$i >= 0;) {
                    $e = $elements->item($i);
                    $image_url = E2WL_Utils::clear_image_url($e->getAttribute('src'));
                    if ($image_url) {
            ...
            if (!defined('E2WL_KEEP_DESCRIPTION_HTML_TAGS') || (defined('E2WL_KEEP_DESCRIPTION_HTML_TAGS') && !E2WL_KEEP_DESCRIPTION_HTML_TAGS)) {
                ...
                $html = preg_replace('/<a[^>]*>\s*(.*)\s*<\/a>/iU', '', $html);
                ...
                for ($i = 0; $i < 5; $i++) {
                    $html = preg_replace("/<td[^>]*>(\s)*<\/td[^>]*>/iU", "", $html);
                    $html = preg_replace("/<th[^>]*>(\s)*<\/th[^>]*>/iU", "", $html);
                    $html = preg_replace("/<tr[^>]*>(\s)*<\/tr[^>]*>/iU", "", $html);
                    $html = preg_replace("/<tbody[^>]*>(\s)*<\/tbody[^>]*>/iU", "", $html);
                    $html = preg_replace("/<thead[^>]*>(\s)*<\/thead[^>]*>/iU", "", $html);
                    $html = preg_replace("/<table[^>]*>(\s)*<\/table[^>]*>/iU", "", $html);
                }
            }
