...
(function ($, window, document, undefined) {
    $(function () {
        ...
        $(document).on("click", ".e2wl-product-info", function () {
            var id = $(this).attr('id').split('-')[1];
            $.e2wl_show(id);
            return false;
        });

        $.e2wl_show = function (id) {
            $('<div id="e2wl-dialog' + id + '"></div>').dialog({
                dialogClass: 'wp-dialog',
                modal: true,
                title: "Ebay Info (ID: " + id + ")",
                open: function () {
                    $('#e2wl-dialog' + id).html(e2wl_wc_pl_script.lang.please_wait_data_loads);
                    var data = {'action': 'e2wl_product_info', 'id': id};
        ...
        function e2wl_js_update_product(products_to_update, state, on_load_calback) {
            if (products_to_update.length > 0) {
                ...
                jQuery.post(ajaxurl, data).done(function (response) {
                    ...
                    e2wl_js_update_product(products_to_update, state, on_load_calback);
        ...
        $("#doaction, #doaction2").click(function () {
            ...
            if ('e2wl_product_update_manual' === check_action) {
                ...
                var cnt = 0;
                var total_ids = 0;
                var data = {'action': 'e2wl_sync_products', 'ids': []}
                $('input:checkbox[name="post[]"]:checked').each(function () {
                    ...
                    if (cnt === update_per_request) {
                        ...
                        cnt = 0;
                        data = {'action': 'e2wl_sync_products', 'ids': []};
                    }
                });
                ...
                if (total_ids > 0) {
                    e2wl_show_block($('.wp-list-table.posts'), sprintf(e2wl_wc_pl_script.lang.process_update_d_of_d, 0, total_ids));
                    
                    sprintf(e2wl_wc_pl_script.lang.process_update_d_of_d, 0, total_ids)
                    var on_load = function (response_state, state) {
                        if ((state.update_cnt + state.update_error_cnt) === state.num_to_update) {
                            e2wl_update_block($(".wp-list-table.posts"), sprintf(e2wl_wc_pl_script.lang.complete_result_updated_d_erros_d, state.update_cnt, state.update_error_cnt));
                            //e2wl_hide_block($(".wp-list-table.posts"));
                            location.reload();
                        } else {
                            e2wl_update_block($(".wp-list-table.posts"), sprintf(e2wl_wc_pl_script.lang.process_update_d_of_d_erros_d, state.update_cnt, state.num_to_update, state.update_error_cnt));
                        }
                    };

                    var state = {num_to_update: total_ids, update_cnt: 0, update_error_cnt: 0};
                    e2wl_js_update_product(products_to_update, state, on_load);
                }
...
function e2wl_show_block(element, message){
...
function e2wl_update_block(element, message){
    jQuery(element).find('.blockUI.blockMsg').html(message);
}

function e2wl_hide_block(element){
