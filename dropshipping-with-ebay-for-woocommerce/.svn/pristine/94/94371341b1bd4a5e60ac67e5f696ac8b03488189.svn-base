<?php
/**
 * Description of E2WL_Settings
 *
 * @author andrey
 */

if (!class_exists('E2WL_Settings')) {
    ...
    class E2WL_Settings {
        ...
        public function load(){
			...
			if(e2wl_check_defined("E2WL_API_ENDPOINT")){
				$static_settings['api_endpoint'] = E2WL_API_ENDPOINT;
			}

			if(e2wl_check_defined("E2WL_DO_NOT_USE_HTTPS")){
				$static_settings['api_endpoint'] = str_replace("https", "http", $static_settings['api_endpoint']);
			}
            $this->settings = array_merge($this->default_settings, get_option('e2wl_settings', array()));
        }
        
        public function commit(){
            update_option('e2wl_settings', $this->settings);
        }
        ...
        public function set($setting, $value){
            ...
            do_action('e2wl_pre_set_setting_'.$setting, $old_value, $value, $setting);
            ...
            do_action('e2wl_set_setting_'.$setting, $old_value, $value, $setting);
        }
...
if (!function_exists('e2wl_settings')) {
    function e2wl_settings() {
        return E2WL_Settings::instance();
    }
}

if (!function_exists('e2wl_get_setting')) {
    function e2wl_get_setting($setting, $default='') {
        return e2wl_settings()->get($setting, $default);
    }
}

if (!function_exists('e2wl_set_setting')) {
    function e2wl_set_setting($setting, $value) {
        if (defined('E2WL_DEMO_MODE') && E2WL_DEMO_MODE && in_array($setting, array('use_external_image_urls'))) {
            return;
        }
        
        return e2wl_settings()->set($setting, $value);
    }
}

if (!function_exists('e2wl_del_setting')) {
    function e2wl_del_setting($setting) {
        return e2wl_settings()->del($setting);
    }
}
