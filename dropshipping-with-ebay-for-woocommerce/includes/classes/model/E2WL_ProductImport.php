...
/**
 * Description of E2WL_ProductImport
 *
 * @author andrey
 */
if (!class_exists('E2WL_ProductImport')) {

    class E2WL_ProductImport
    {
        ...
        public function del_product($product_id)
        {
            ...
            if ($to_del) {
                foreach ($to_del as $did) {
                    e2wl_delete_transient('e2wl_product#' . strval($did));
                }
            }
        }
        ...
        public function get_product_id_list($limit = null, $ofset = null)
        {
            ...
            if (defined('E2WL_SAVE_TRANSIENT_AS_OPTION') && E2WL_SAVE_TRANSIENT_AS_OPTION) {
                $results = $wpdb->get_results("select option_name from {$wpdb->options} where option_name like 'e2wl_product#%' order by option_id $limitStr", ARRAY_A);
            } else {
                $results = $wpdb->get_results("select option_name from {$wpdb->options} where option_name like '_transient_e2wl_product#%' order by option_id $limitStr", ARRAY_A);
            }
            $ids = array();
        ...
        public function get_products_count()
        {
            global $wpdb;
            if (defined('E2WL_SAVE_TRANSIENT_AS_OPTION') && E2WL_SAVE_TRANSIENT_AS_OPTION) {
                return $wpdb->get_var("select count(option_id) from {$wpdb->options} where option_name like 'e2wl_product#%'");
            } else {
                return $wpdb->get_var("select count(option_id) from {$wpdb->options} where option_name like '_transient_e2wl_product#%'");
            }
        }

        public function get_product($product_id)
        {
            $product = e2wl_get_transient('e2wl_product#' . strval($product_id));
            if ($product) {
                /*
                ...
                if (!isset($product['product_type'])) {
                    $product['product_type'] = e2wl_get_setting('default_product_type');
                }
                if (!isset($product['product_status'])) {
                    $product['product_status'] = e2wl_get_setting('default_product_status');
                }
        ...
        {
            e2wl_set_transient('e2wl_product#' . strval($product_id), $product);
        }
        ...
        {
            return array('id-asc' => __('Sort by External Id', 'dropshipping-with-ebay-for-woocommerce'), 'title-asc' => __('Sort by Product title', 'dropshipping-with-ebay-for-woocommerce'), 'date_add-asc' => __('Sort by Date add (old first)', 'dropshipping-with-ebay-for-woocommerce'), 'date_add-desc' => __('Sort by Date add (new first)', 'dropshipping-with-ebay-for-woocommerce'));
        }
