...
/**
 * Description of E2WL_PhraseFilter
 *
 * @author ma_group
 */
class E2WL_PhraseFilter {
    ...
    public function save() {
        E2WL_PhraseFilter::$_instance_objects = null;
        E2WL_PhraseFilter::$_instance_array = null;
        
        $formula_list = E2WL_PhraseFilter::load_phrase_list(false);
        ...
        e2wl_set_setting('phrase_list', array_values($formula_list));
        return $this;
    }
    ...
    public static function apply_filter_to_text($text, $phrase_list=array()) {
        ...
        if(!$phrase_list){
            $phrase_list = E2WL_PhraseFilter::load_phrase_list(false);
        }
        ...
        if (!empty($phrase_list)) {
            ...
            foreach ($phrase_list as $p) {
                $phrase[]='/\b'.preg_quote($p['phrase'],'/').'\b/u';
    ...
    public static function apply_filter_to_products() {
        ...
        $tmp_product_ids = $wpdb->get_results("SELECT p.ID, p.post_title, p.post_content from {$wpdb->posts} p LEFT JOIN {$wpdb->postmeta} as pm ON pm.post_id = p.ID WHERE (".implode(" OR ", $phrase_query_array).") AND pm.meta_key = '_e2w_external_id'", ARRAY_N);
        foreach ($tmp_product_ids as $row) {
            $new_title = E2WL_PhraseFilter::apply_filter_to_text($row[1], $phrase_list);
            $new_content = E2WL_PhraseFilter::apply_filter_to_text($row[2], $phrase_list);
    ...
    public static function apply_filter_to_reviews() {
        ...
        $tmp_comment_ids = $wpdb->get_results("SELECT c.comment_ID, c.comment_author, c.comment_content from {$wpdb->comments} c LEFT JOIN {$wpdb->commentmeta} as cm ON cm.comment_id = c.comment_ID WHERE (".implode(" OR ", $phrase_query_array).") AND cm.meta_key = 'e2wl_country'", ARRAY_N);
        foreach ($tmp_comment_ids as $row) {
            $new_author = E2WL_PhraseFilter::apply_filter_to_text($row[1], $phrase_list);
            $new_content = E2WL_PhraseFilter::apply_filter_to_text($row[2], $phrase_list);
    ...
    public static function deleteAll() {
        e2wl_del_setting('phrase_list');
    }

    public static function load_phrases() {
        return E2WL_PhraseFilter::load_phrase_list(true);
    }

    private static function load_phrase_list($asObject = true) {
        $result = array();

        if ($asObject && E2WL_PhraseFilter::$_instance_objects) {
            return E2WL_PhraseFilter::$_instance_objects;
        } else if (!$asObject && E2WL_PhraseFilter::$_instance_array) {
            return E2WL_PhraseFilter::$_instance_array;
        }
        
        $formula_list = e2wl_get_setting('phrase_list');
        $formula_list = $formula_list && is_array($formula_list) ? $formula_list : array();

        if ($asObject) {
            foreach ($formula_list as $formula) {
                $fo = new E2WL_PhraseFilter();
                ...
                $result[] = $fo;
                E2WL_PhraseFilter::$_instance_objects = $result;
            }
        } else {
            $result = $formula_list;
            E2WL_PhraseFilter::$_instance_array = $result;
        }
...
if (!function_exists('e2wl_phrase_apply_filter_to_text')) {

    function e2wl_phrase_apply_filter_to_text($text) {
        return E2WL_PhraseFilter::apply_filter_to_text($text);
    }
