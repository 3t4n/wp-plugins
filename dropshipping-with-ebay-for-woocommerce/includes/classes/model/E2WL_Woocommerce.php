...
/**
 * Description of E2WL_Woocommerce
 *
 * @author andrey
 */
if (!class_exists('E2WL_Woocommerce')) {

    class E2WL_Woocommerce
    {
        ...
        {
            $this->attachment_model = new E2WL_Attachment();
            $this->helper = new E2WL_Helper();
        }
        ...
        {
            if (!E2WL_Woocommerce::is_woocommerce_installed()) {
                return E2WL_ResultBuilder::buildError("Woocommerce is not installed");
            }

            global $wpdb;

            do_action('e2wl_woocommerce_before_add_product', $product, $params);

            $product_type = (isset($product['product_type']) && $product['product_type']) ? $product['product_type'] : e2wl_get_setting('default_product_type', 'simple');
            $product_status = (isset($product['product_status']) && $product['product_status']) ? $product['product_status'] : e2wl_get_setting('default_product_status', 'publish');
            ...
            $site = E2WL_EbaySite::get_site($product['site_id']);
            ...
            if (isset($product['attribute']) && $product['attribute'] && !e2wl_get_setting('not_import_attributes', false)) {
            ...
            if (isset($product['thumb_id'])) {
                foreach ($tmp_all_images as $img_id => $img) {
                    if ($img_id === $product['thumb_id']) {
                        $thumb_url = E2WL_Utils::clear_url($img['image']);
                        break;
                    }
                }
                /*
            ...
            do_action('e2wl_add_product', $product_id);

            return apply_filters('e2wl_woocommerce_after_add_product', E2WL_ResultBuilder::buildOk(array('product_id' => $product_id)), $product_id, $product, $params);
        }

        public function upd_product($product_id, $product, $params = array())
        {
            do_action('e2wl_woocommerce_before_upd_product', $product, $params);
            ...
            $not_available_product_status = e2wl_get_setting('not_available_product_status');
            ...
            do_action('e2wl_upd_product', $product_id);

            delete_transient('wc_attribute_taxonomies');

            return apply_filters('e2wl_woocommerce_after_upd_product', $result, $product_id, $product, $params);
        }
        ...
        public function build_description($product_id, $product)
        {
            ...
            for ($i = $elements->length; --$i >= 0;) {
                ...
                } else if (!e2wl_get_setting('use_external_image_urls')) {
                    $tmp_title = isset($product['title']) && $product['title'] ? $product['title'] : "Product " . $product['id'];
                    $attachment_id = $this->attachment_model->create_attachment($product_id, E2WL_Utils::clear_image_url($e->getAttribute('src')), $tmp_title, $tmp_title);
        ...
        public function set_images($product_id, $thumb_url, $images, $update, $title = '')
        {
            ...
            if ($images) {
                if (!get_post_meta($product_id, '_product_image_gallery', true) || $update) {
                    //$images_limit = intval(e2wl_get_setting('import_product_images_limit'));
        ...
        {
            if (defined('E2WL_IMPORT_EXTENDED_ATTRIBUTE')) {
                $extended_attribute = filter_var(E2WL_IMPORT_EXTENDED_ATTRIBUTE, FILTER_VALIDATE_BOOLEAN);
            } else {
                $extended_attribute = e2wl_get_setting('import_extended_attribute');
            }
        ...
        private function build_categories($product)
        {
            ...
            if (isset($product['categories']) && $product['categories']) {
                $result = is_array($product['categories']) ? array_map('e2wl_int_array_map', $product['categories']) : array(intval($product['categories']));
        ...
        private function add_variation($product_id, $product, $is_update = false)
        {
            ...
            if ($variations) {
                ...
                if ($variations && isset($variations['attributes']) && ($tmp_product->is_type('variable') || $tmp_product->is_type('simple'))) {
                    ...
                    $localCurrency = strtoupper(e2wl_get_setting('local_currency'));
                    ...
                    if ($localCurrency) {
                        ...
                        $currency_conversion_factor = floatval(e2wl_get_setting('currency_conversion_factor'));
                    }
                    ...
                    foreach ($variations['attributes'] as $key => $attr) {
                        $attribute_taxonomies = e2wl_get_setting('import_extended_variation_attribute');
                        ...
                        foreach ($attr['value'] as $val_key => $val) {
                            ...
                            if (isset($val['original_id'])) {
                                foreach ($attr['value'] as $val_oid_key => $val_oid) {
                                    if ($val_oid_key === $val['original_id']) {
                                        update_post_meta($product_id, '_e2w_original_name_' . $val_oid_key, $val_oid['name']);
                                        break;
                                    }
                                }
                            }
                            ...
                            $RELOAD_ATTR_IMAGES = defined('E2WL_RELOAD_ATTR_IMAGES') && E2WL_RELOAD_ATTR_IMAGES;
        ...
        public function get_product_by_post_id($post_id, $with_vars = true)
        {
            ...
            if ($external_id) {
                ...
                $e2wl_skip_meta = get_post_meta($post_id, "_e2w_skip_meta", true);

                $product['skip_vars'] = $e2wl_skip_meta && !empty($e2wl_skip_meta['skip_vars']) ? $e2wl_skip_meta['skip_vars'] : array();
                $product['skip_images'] = $e2wl_skip_meta && !empty($e2wl_skip_meta['skip_images']) ? $e2wl_skip_meta['skip_images'] : array();
