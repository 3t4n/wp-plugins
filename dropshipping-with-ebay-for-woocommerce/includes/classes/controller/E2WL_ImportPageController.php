...
/**
 * Description of E2WL_ImportPageController
 *
 * @author Andrey
 *
 * @autoload: e2wl_init
 */
if (!class_exists('E2WL_ImportPageController')) {

    class E2WL_ImportPageController extends E2WL_AbstractAdminPage
    {
        ...
        public function __construct()
        {
            ...
            parent::__construct("Import List", "Import List " . ($products_cnt ? ' <span class="update-plugins count-' . $products_cnt . '"><span class="plugin-count">' . $products_cnt . '</span></span>' : ''), 'manage_options', 'e2wl_import', 20);

            add_action('wp_ajax_e2wl_push_product', array($this, 'ajax_push_product'));
            add_action('wp_ajax_e2wl_delete_import_products', array($this, 'ajax_delete_import_products'));
            add_action('wp_ajax_e2wl_update_product_info', array($this, 'ajax_update_product_info'));
            add_action('wp_ajax_e2wl_update_variation_info', array($this, 'ajax_update_variation_info'));
            add_action('wp_ajax_e2wl_link_to_category', array($this, 'ajax_link_to_category'));
            add_action('wp_ajax_e2wl_get_all_products_to_import', array($this, 'ajax_get_all_products_to_import'));
        ...
        public function before_admin_render()
        {
            if (isset($_REQUEST['delete_id'])) {
                $this->product_import_model->del_product(sanitize_key($_REQUEST['delete_id']));

                wp_redirect(admin_url('admin.php?page=e2wl_import'));
                ...
                wp_redirect(admin_url('admin.php?page=e2wl_import'));
            } else if (((isset($_REQUEST['action']) && $_REQUEST['action'] == "delete") || (isset($_REQUEST['action2']) && $_REQUEST['action2'] == "delete")) && isset($_REQUEST['gi']) && is_array($_REQUEST['gi'])) {
                $this->product_import_model->del_product(array_map('sanitize_key', $_REQUEST['gi']));
        ...
        {
            $serach_query = !empty($_REQUEST['s']) ? sanitize_text($_REQUEST['s']) : '';
            ...
            $paginator = E2WL_Paginator::build($products_cnt);

            if (defined('E2WL_SKIP_IMPORT_SORTING') && E2WL_SKIP_IMPORT_SORTING) {
            ...
            foreach ($product_list as &$product) {
                $tmp_all_images = E2WL_Utils::get_all_images_from_product($product);
            ...
            //$paginator = E2WL_Paginator::build(count($product_list));
            ...
            $this->model_put("localizator", E2WL_EbayLocalizator::getInstance());
                ...
                if ($product_id) {
                    ...
                    if ($product) {
                        ...
                        if ($result['state'] !== 'error') {
                            ...
                            $result = E2WL_ResultBuilder::buildOk($result_objects);
                        } else {
                            $result = E2WL_ResultBuilder::buildError($result['message']);
                        }
                    } else {
                        $result = E2WL_ResultBuilder::buildError("Product " . $product_id . " not find.");
                    }
                } else {
                    $result = E2WL_ResultBuilder::buildError("import_product: waiting for ID...");
                }
        ...
        public function ajax_delete_import_products()
        {
            ...
            try {
                if (isset($_POST['ids']) && $_POST['ids']) {
                    $ids = is_array($_POST['ids']) ? array_map('sanitize_key', $_POST['ids']) : sanitize_key($_POST['ids']);
                    $this->product_import_model->del_product($ids);
                }
        ...
        public function ajax_update_product_info()
        {
            e2wl_init_error_handler();
            try {
                $post_id = isset($_POST['id']) ? sanitize_key($_POST['id']) : false;
                if ($post_id && ($product = $this->product_import_model->get_product($post_id))) {
                    ...
                    if (isset($_POST['description'])) {
                        $product['description'] = sanitize_textarea_field(trim(urldecode($_POST['description'])));
                    }
                    ...
                    $result = E2WL_ResultBuilder::buildError("update_product_info: waiting for ID...");
                }
        ...
        public function ajax_update_variation_info()
        {
            ...
            try {
                $product_id = isset($_POST['id']) ? sanitize_key($_POST['id']) : false;
                if ($product_id && isset($_POST['variations']) && is_array($_POST['variations']) && ($product = $this->product_import_model->get_product($product_id))) {
                    ...
                    $result = E2WL_ResultBuilder::buildOk($out_data);
                } else {
                    $result = E2WL_ResultBuilder::buildError("update_variation_info: waiting for ID...");
                }
        ...
        public function ajax_link_to_category()
        {
            if (!empty($_POST['categories']) && !empty($_POST['ids'])) {
                $new_categories = is_array($_POST['categories']) ? array_map('e2wl_int_array_map', $_POST['categories']) : array(intval($_POST['categories']));
                $ids = is_array($_POST['ids']) ? array_map('sanitize_key', $_POST['ids']) : sanitize_key($_POST['ids']);
                $ids = (is_string($ids) && $ids === 'all') ? $this->product_import_model->get_product_id_list() : (is_array($ids) ? $ids : array($ids));
                foreach ($ids as $id) {
                ...
                e2wl_set_setting('remember_categories', $new_categories);
            } else if (empty($_POST['categories'])) {
                e2wl_del_setting('remember_categories');
            }
        ...
        {
            echo json_encode(E2WL_ResultBuilder::buildOk(array('ids' => $this->product_import_model->get_product_id_list())));
            wp_die();
        }
