...
/**
 * Description of E2WL_ProductServiceController
 *
 * @author Andrey
 *
 * @autoload: e2wl_init
 */
if (!class_exists('E2WL_ProductServiceController')) {

    class E2WL_ProductServiceController
    {
        ...
        public function delete_post_action($post_id)
        {
            ...
            E2WL_Utils::delete_post_images($post_id);

            //second, mark variation as skip
            if (get_post_type($post_id) == 'product_variation') {
                $variation = new WC_Product_Variation($post_id);
                $parent_id = $variation->get_parent_id();
                $e2wl_skip_meta = get_post_meta($parent_id, "_e2w_skip_meta", true);
                if ($e2wl_skip_meta) {
                    $external_variation_id = get_post_meta($post_id, "external_variation_id", true);
                    if (!in_array($external_variation_id, $e2wl_skip_meta['skip_vars'])) {
                        $e2wl_skip_meta['skip_vars'][] = $external_variation_id;
                    }
                    update_post_meta($parent_id, "_e2w_skip_meta", $e2wl_skip_meta);
                }
            }
        }
        ...
        public function save_product_variation($variation_id, $i)
        {
            ...
            if ($e2wl_skip_meta) {
                ...
                if (in_array($variation->get_status(), array('publish', false))) {
                    $e2wl_skip_meta['skip_vars'] = array_diff($e2wl_skip_meta['skip_vars'], array($external_variation_id));
                } else {
                    if (!in_array($external_variation_id, $e2wl_skip_meta['skip_vars'])) {
                        $e2wl_skip_meta['skip_vars'][] = $external_variation_id;
                    }
        ...
        public function woocommerce_save_attributes()
        {
            ...
            if ($original_variations_attributes) {
                ...
                $product_type = !empty($_POST['product_type']) ? sanitize_key(wc_clean($_POST['product_type'])) : 'simple';
                    ...
                    if ($pa->get_variation() && !$pa->is_taxonomy()) {
                        foreach ($attributes as $a) {
                            if ($pa->get_position() === $a->get_position() && $pa->get_name() !== $a->get_name()) {
                                ...
                                global $wpdb;
                                $wpdb->query("update wp_postmeta pm, wp_posts p SET pm.meta_key='" . esc_sql("attribute_" . $new_name) . "' WHERE p.ID = pm.post_id AND p.post_parent='" . $product->get_id() . "' AND pm.meta_key='" . esc_sql("attribute_" . $old_name) . "'");
                ...
                update_post_meta($product_id, '_e2w_deleted_variations_attributes', $original_variations_attributes);
            }
        }
