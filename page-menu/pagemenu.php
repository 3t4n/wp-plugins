...
class pagemenu
{
    ...
    public function save_pgm_postdata($post_id)
    {
    ...
    public function pgm_menu_args($args)
    {
        if (is_archive()) {
            global $wp_query;
    ...
    public function pgm_nav_class($classes, $item, $args)
    {
    ...
    public function css_injector()
    {
        echo "<style>.pagemenu-hide{display:none !important;}</style>";
    }
    
    public function pgm_set_nav_menu()
    {
...
class Pgm_Walker extends Walker_Nav_Menu
{
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        } elseif (in_array($item->ID, $pgm_postOption['pgm_menulist'])) {
            $checked ='checked="checked"';
        }
