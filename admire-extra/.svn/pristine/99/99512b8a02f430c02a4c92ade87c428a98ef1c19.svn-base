    ...
    class AdmireXTRA_Demos {
        ...
        public static function ajax_demo_data() {
            ...
            <div id="fwp-demo-plugins">
                ...
                <h2 class="title"><?php echo sprintf(esc_attr('Import the %1$s', 'admire-extra'), esc_attr($demo)); ?></h2>
                ...
                <div class="fwp-popup-text">
                    ...
                    <p><?php
                        echo
                        sprintf(
                                esc_attr('Importing demo data allow you to quickly edit everything instead of creating content from scratch. It is recommended uploading sample data on a fresh WordPress install to prevent conflicts with your current content. You can use this plugin to reset your site if needed: %1$sWordpress Database Reset%2$s.', 'admire-extra'),
        ...
        public function ajax_required_plugins_activate() {
            ...
            if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['init'] ) || 
                empty( sanitize_text_field( wp_unslash( $_POST['init'] ) ) ) ) {
                wp_send_json_error(
                    array(
                        'success' => false,
        ...
        public function ajax_reset_mods() {
            if (!wp_verify_nonce($_POST['admire_import_demo_data_nonce'], 'admire_import_demo_data_nonce')) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
                die('This action was stopped for security purposes.');
            }
            $save_result = 1;
            // Get the selected demo
            $demo_type = '';
            if ( isset( $_POST['admire_reset_mods'] ) ) {
                $demo_type = sanitize_text_field( wp_unslash( $_POST['admire_reset_mods'] ) );
            }
            ...
            if ($save_result === 1) {
                ...
                if ( $wp_filesystem->is_writable( $directory ) ) {
                    $file = $directory . '/admire' . gmdate( 'Y-m-d-h-i-s' ) . '.json';

                    // Ensure proper path for FTP access
        ...
        public function ajax_import_xml() {
            ...
            // Get the selected demo
            $demo_type = '';
            if ( isset( $_POST['admire_import_demo'] ) ) {
                $demo_type = sanitize_text_field( wp_unslash( $_POST['admire_import_demo'] ) );
            }
        ...
        public static function ajax_after_import() {
            ...
            if ($_POST['admire_import_is_xml'] === 'true') { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated
                ...
                // Reset the postdata after the WP_Query
                wp_reset_postdata();
