...
function admire_display_testimonial_meta_box( $testimonial ) {
    ...
    <table width="100%">
        <tr>
            ...
            <td width="80%"><input size="80" type="text" name="<?php echo esc_attr('companyname','admire-extra');?>" value="<?php echo esc_attr( $companyname ); ?>" /></td>
        </tr> 
        <tr>
            <td width="20%"><?php echo esc_attr('Designation','admire-extra');?> </td>
            <td width="80%"><input size="80" type="text" name="<?php echo esc_attr('possition','admire-extra');?>" value="<?php echo esc_attr( $possition ); ?>" /></td>
        </tr>       
    </table>
    <?php    
}
...
function admire_add_testimonial_fields_function( $testimonial_id, $testimonial ) {
    ...
    if ( $testimonial->post_type == 'testimonials' ) {
		...
		if ( isset($_POST['companyname']) ) {
            update_post_meta( $testimonial_id, 'companyname', wp_filter_kses( sanitize_text_field( wp_unslash($_POST['companyname']) ) ) );
        } 
        if ( isset($_POST['possition']) ) {
            update_post_meta( $testimonial_id, 'possition', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['possition']) ) ) );
        }       
    }
}
...
function admire_display_team_meta_box( $team ) {
    ...
    <table width="100%">
        <tr>
            <td width="20%"><?php echo esc_attr('Designation','admire-extra');?> </td>
            <td width="80%"><input type="text" name="designation" value="<?php echo esc_attr( $designation ); ?>" /></td>
        </tr>
        <tr>
        	<td width="20%">&nbsp;</td>
        ...
        <tr>
            ...
            <td width="40%"><input type="text" name="<?php echo esc_attr('facebook','admire-extra');?>" value="<?php echo esc_attr( $facebook ); ?>" /></td>
            <td width="40%"><input style="width:500px;" type="text" name="<?php echo esc_attr('facebooklink','admire-extra');?>" value="<?php echo esc_attr( $facebooklink ); ?>" /></td>
        </tr>
        <tr>
            <td width="20%"><?php echo esc_attr('Social Link 2','admire-extra');?></td>
            <td width="40%"><input type="text" name="<?php echo esc_attr('twitter','admire-extra');?>" value="<?php echo esc_attr( $twitter ); ?>" /></td>
            <td width="40%"><input style="width:500px;" type="text" name="<?php echo esc_attr('twitterlink','admire-extra');?>" value="<?php echo esc_attr( $twitterlink ); ?>" /></td>
        </tr>
        <tr>
            <td width="20%"><?php echo esc_attr('Social Link 3','admire-extra');?></td>
            <td width="40%"><input type="text" name="<?php echo esc_attr('instagram','admire-extra');?>" value="<?php echo esc_attr( $instagram ); ?>" /></td>
            <td width="40%"><input style="width:500px;" type="text" name="<?php echo esc_attr('instagramlink','admire-extra');?>" value="<?php echo esc_attr( $instagramlink ); ?>" /></td>
        </tr>
        <tr>
            <td width="20%"><?php echo esc_attr('Social Link 4','admire-extra');?></td>
            <td width="40%"><input type="text" name="<?php echo esc_attr('linkedin','admire-extra');?>" value="<?php echo esc_attr( $linkedin ); ?>" /></td>
            <td width="40%"><input style="width:500px;" type="text" name="<?php echo esc_attr('linkedinlink','admire-extra');?>" value="<?php echo esc_attr( $linkedinlink ); ?>" /></td>
        </tr>        
        <tr>
            <td width="20%"><?php echo esc_attr('Social Link 5','admire-extra');?></td>
            <td width="40%"><input type="text" name="<?php echo esc_attr('pinterest','admire-extra');?>" value="<?php echo esc_attr( $pinterest ); ?>" /></td>
            <td width="40%"><input style="width:500px;" type="text" name="<?php echo esc_attr('pinterestlink','admire-extra');?>" value="<?php echo esc_attr( $pinterestlink ); ?>" /></td>
        </tr>
        <tr>
...
function admire_add_team_fields_function( $team_id, $team ) {
    ...
    if ( $team->post_type == 'team' ) {
        ...
        if ( isset($_POST['designation']) ) {
            update_post_meta( $team_id, 'designation', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['designation']) )) );
        }        
		if ( isset($_POST['facebook']) ) {
            update_post_meta( $team_id, 'facebook', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['facebook']) )) );
        }
		if ( isset($_POST['facebooklink']) ) {
            update_post_meta( $team_id, 'facebooklink', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['facebooklink']) )) );
        }
        if ( isset($_POST['twitter']) ) {
            update_post_meta( $team_id, 'twitter', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['twitter']) )) );
        }
		if ( isset($_POST['twitterlink']) ) {
            update_post_meta( $team_id, 'twitterlink', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['twitterlink']) )) );
        }
        if ( isset($_POST['instagram']) ) {
            update_post_meta( $team_id, 'instagram', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['instagram']) )) );
        }
		if ( isset($_POST['instagramlink']) ) {
            update_post_meta( $team_id, 'instagramlink', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['instagramlink']) )) );
        }		
        if ( isset($_POST['linkedin']) ) {
            update_post_meta( $team_id, 'linkedin', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['linkedin']) )) );
        }
		if ( isset($_POST['linkedinlink']) ) {
            update_post_meta( $team_id, 'linkedinlink', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['linkedinlink']) )) );
        }
		if ( isset($_POST['pinterest']) ) {
            update_post_meta( $team_id, 'pinterest', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['pinterest']) )) );
        }
		if ( isset($_POST['pinterestlink']) ) {
            update_post_meta( $team_id, 'pinterestlink', wp_filter_kses(sanitize_text_field( wp_unslash( $_POST['pinterestlink']) )) );
        }
    }
}
...
function admire_social($atts){
 extract(shortcode_atts(array(
  ...
  return '<a href="'. esc_url( $link ) .'" target="_blank" class="fa fa-'. esc_attr( $icon ).' fa-1x" title="'. esc_attr( $icon ) .'"></a>';
 }
...
function admire_foot_menu($atts, $content = null) {
	...
	return wp_nav_menu( array( 
		'menu'            => esc_url( $menu ), 
		'container'       => esc_url( $container ), 
		'container_class' => esc_attr( $container_class ), 
		'container_id'    => esc_url( $container_id ), 
		'menu_class'      => esc_attr( $menu_class ), 
		'menu_id'         => esc_attr( $menu_id ),
		'echo'            => false,
		'fallback_cb'     => esc_attr( $fallback_cb ),
		'before'          => esc_attr( $before ),
		'after'           => esc_attr( $after ),
		'link_before'     => esc_attr( $link_before ),
		'link_after'      => esc_attr( $link_after ),
		'depth'           => esc_attr( $depth ),
		'walker'          => esc_attr( $walker ),
...
function admire_space_shortcode_func($atts ) {
 ...
 $sptr = '<div class="spacecode" style="height:'. esc_attr( $height ) .'px;"></div>';
 $escaping_sptr = wp_kses_post( $sptr );

 return wp_kses_post( $escaping_sptr );
}
   ...
   extract( shortcode_atts( array(
	...
	if ( $query->have_posts() ) {
		while ( $query->have_posts() ) { 
			...
			$lbposts .= '<a href="'.get_permalink().'" title="'.esc_attr( get_the_title() ).'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '<img src="'.esc_url( get_template_directory_uri().'/images/default-post-img.jpg' ).'" class="post-default-img">' ).'</a>';
			if ( has_post_thumbnail() ){
			$lbposts .= '</div>';
			}
				$lbposts .= '<div class="post-title">
				<div class="post_block_style1_meta"><span>'.esc_attr( get_the_date('M d, Y') ).'</span></div>
				<h3><a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'">'.esc_attr( get_the_title() ).'</a></h3>
				<p>'.wp_trim_words( wp_kses_post( get_the_content() ), esc_attr( $excerptlength )).'</p>
				</div></div></div>';
		}
	}else{
   ...
   extract( shortcode_atts( array(
	...
	if ( $query->have_posts() ) {
		while ( $query->have_posts() ) {
			...
			}$lbposts .= '<a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '' ).'</a>'; if ( has_post_thumbnail() ){ $lbposts .= '</div>'; } $lbposts .= '
				<h3><a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'">'.get_the_title().'</a></h3>
				<div class="post_block_style2_meta">
                	<span><a href="'.esc_url( get_author_posts_url( $authordata->ID, $authordata->user_nicename )).'"><i class="fa fa-user fa-lg"></i> '.esc_attr( get_the_author() ).'</a></span><span><i class="fa fa-calendar"></i>
'.esc_attr( get_the_date('F j, Y') ).'</span>
                </div>
				<p>'.wp_trim_words( get_the_content(), esc_attr( $excerptlength ) ).'</p>
				</div>';
		}
	}else{
   ...
   extract( shortcode_atts( array(
	...
	if ( $query->have_posts() ) {
		while ( $query->have_posts() ) {
			...
			if(has_post_thumbnail() ){
			$lbposts .= '<div class="style3thumb"><a href="'.esc_url( get_permalink()).'" title="'.esc_attr( get_the_title() ).'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '' ).'</a></div>'; 
			}
					if(has_post_thumbnail() ){
					...
					$lbposts .= '<h3>'.esc_attr( get_the_title() ).'</h3>'; 
					$lbposts .= '<div class="shortdesc">'.wp_trim_words( wp_kses_post( get_the_content() ), esc_attr( $excerptlength ) ).'</div>';
					$lbposts .= '<div class="shortmore"><a href="'.esc_url( get_permalink()).'">'.'Read More'.'</a></div>';
					$lbposts .= '</div></div></div>';
		}
	}else{
		$lbposts .= '<p>Sorry! There are no post.</p>';
 ...
 $lbposts = '<div class="skt-posts-style5-row">';
	...
	if ( $query->have_posts() ) {
		while ( $query->have_posts() ) {
			...
			$lbposts .= '<a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '<img src="'.get_template_directory_uri().'/images/default-post-img.jpg" class="post-default-img">' ).'</a>';
			if ( has_post_thumbnail() ){
			$lbposts .= '</div>';
			}
				$lbposts .= '
		...
		<h3 class="skt-posts-style5-title"><a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'">'.esc_attr( get_the_title() ).'</a></h3>
		<div class="short-desc">'.wp_trim_words( wp_kses_post( get_the_content() ), esc_attr( $excerptlength ) ).'</div>	
		<div class="postread-more">
          <a class="skt-posts-style5-more" href="'.esc_url( get_permalink() ).'">'.esc_attr( $buttontext ).'</a>
          </div>
              <div class="clear"></div>
		</div></div></div>';
		}
  ...
  <div class="timeline-row">
	...
	if ( $query->have_posts() ) {
		...
		while ( $query->have_posts() ) {
			...
			<div class="timeleft"><a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '' ).'</a></div>'; 
			if ( has_post_thumbnail() ){$tmlposts .= '<div class="timeright">'; }else {$tmlposts .= '<div class="timerightfull">';}
			$tmlposts .= '<h3><a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'">'.esc_attr( get_the_title() ).'</a></h3><div class="metaline">
                	<span><a href="'.esc_url( get_author_posts_url( $authordata->ID, $authordata->user_nicename ) ).'"><i class="fa fa-user fa-lg"></i> '.esc_attr( get_the_author() ).'</a></span><span><i class="fa fa-calendar"></i>
'.esc_attr(get_the_date('F j, Y') ).'</span>
                </div><p>'.wp_trim_words( wp_kses_post( get_the_content() ), esc_attr( $excerptlength ) ).'</p></div>
			</div></li> ';
				$n++;
		}
	}else{
...
<div class="masonry">';
	...
	if ( $query->have_posts() ) {
		...
		while ( $query->have_posts() ) {
			$query->the_post();
			$gridposts .= '<div class="griditem"><a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '' ).'</a>
<h3><a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'">'.esc_attr( get_the_title() ).'</a></h3>
<div class="metaline">
                	<span><a href="'.esc_url( get_author_posts_url( $authordata->ID, $authordata->user_nicename ) ).'"><i class="fa fa-user fa-lg"></i> '.esc_attr( get_the_author()).'</a></span><span><i class="fa fa-calendar"></i>
'.esc_attr( get_the_date('F j, Y') ).'</span>
...
function admire_sidebar_menu($atts, $content = null) {
	...
	return wp_kses_post( wp_nav_menu( array( 
		'menu'            => $menu, 
		'container'       => $container, 
...
function admire_services_box_func( $atts ) {
   extract( shortcode_atts( array(
	  ...
	  extract( shortcode_atts( array( 'show' => $show,), $atts ) ); $ourtm = ''; wp_reset_postdata(); 
	...
	<div class="skt-services-image"><a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'"><img src="'.esc_attr( $servicesiconimg ).'" alt="'.esc_attr( get_the_title()).'"></a>';
			$ourtm .= '</div>';
			
                $ourtm .= '<div class="services-infobox">
							<h3 class="skt-services-title"><a href="'.esc_url( get_permalink() ).'">'.esc_attr( get_the_title() ).'</a></h3>
   ...
   extract( shortcode_atts( array(
	...
	if ( $query->have_posts() ) {
		while ( $query->have_posts() ) { 
			...
			<a href="'.esc_url( get_permalink() ).'" title="'.esc_attr( get_the_title() ).'">'.( (get_the_post_thumbnail( get_the_ID(), 'thumbnail') != '') ? get_the_post_thumbnail( get_the_ID(), 'full') : '<img src="'.get_template_directory_uri().'/images/team_thumb.jpg" alt="teamimage" />' ).'</a></div>';
                $ourtm .= '<div class="team-infobox">
				<div class="info">
                	<h3><a href="'.esc_url( get_permalink() ).'">'.esc_attr( get_the_title()).'</a></h3>';
					if( $designation != '' ){
                    $ourtm .= '<div class="team-desig">'.esc_attr( $designation ).'</div>';
					}
 						$ourtm .= '<div class="social-icons">';
						if( $facebook != '' ){
                    	$ourtm .= '<a href="'. esc_url( $facebooklink ).'" title="'. esc_attr( $facebook ).'" target="_blank"><i class="fa fa-'.esc_attr( $facebook ).' fa-lg"></i></a>';
						}
						if( $twitter != '' ){
                    	$ourtm .= '<a href="'. esc_url( $twitterlink ).'" title="'.esc_attr( $twitter ).'" target="_blank"><i class="fa fa-'.esc_attr( $twitter ).' fa-lg"></i></a>';
						}
						if( $instagram != '' ){
                    	$ourtm .= '<a href="'. esc_url( $instagramlink ).'" title="'.esc_attr( $instagram ).'" target="_blank"><i class="fa fa-'.$instagram.' fa-lg"></i></a>';
						}												
						if( $linkedin != '' ){
                    	$ourtm .= '<a href="'. esc_url( $linkedinlink ).'" title="'.esc_attr( $linkedin ).'" target="_blank"><i class="fa fa-'.esc_attr( $linkedin ).' fa-lg"></i></a>';
						}
						if( $pinterest != '' ){
                    	$ourtm .= '<a href="'. esc_url( $pinterestlink ).'" title="'.esc_attr( $pinterest ).'" target="_blank"><i class="fa fa-'.esc_attr( $pinterest ).' fa-lg"></i></a>';
						}
...
function admire_testimonials_box_func( $atts ) {
   extract( shortcode_atts( array(
	  ...
	  extract( shortcode_atts( array( 'show' => $show,), $atts ) ); $tstmnl = ''; wp_reset_postdata(); 
		...
		while ( $query->have_posts() ) { 
			...
			if( $col == 1 ){
                ...
                $tstmnl .= '<div class="testimonial-box"> 
					 <em>'.esc_attr( get_the_content() ).'</em>
                     </div>
                     <div class="testimonial-inforarea">
                     	<i class="fa fa-user"></i>
<h3>'.esc_attr( get_the_title() ).' </h3><div class="clear"></div>';
...
if(!empty($companyname)){
	$tstmnl .= esc_html( $companyname );
} 
$tstmnl .= '<br/>';
if(!empty($possition)){
	$tstmnl .= esc_html( $possition );
} 
  ...
  <div class="owl-carousel skt-testimonials">
	...
	<div class="item">
	  <div class="skt-testimonial-box"> 
        ...
        <div class="skt-testimonial-content"><?php esc_attr( the_content() ); ?></div> 
        <?php if(has_post_thumbnail() ) { ?>  
          <div class="skt-testimonial-image"><?php the_post_thumbnail(); ?></div>
        <?php } ?>  
        <div class="skt-testimonial-dtl">
            <h3 class="skt-testimonial-title"><?php esc_attr( the_title() ) ; ?></h3> 
            <div class="skt-testimonial-designation"><?php echo esc_attr( $companyname ); ?> <?php if (!empty($companyname)) { echo '<br/>'; } ?> <?php echo esc_html($position);?></div>
        </div>
	  </div>       	
	</div>
