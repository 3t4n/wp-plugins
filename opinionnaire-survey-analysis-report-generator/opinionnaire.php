...
*/
	...
	add_action('admin_menu','opin_setup_menu');
	add_action('wp_ajax_get_custom_demos','opin_demo_db');

	function opin_setup_menu() {
		$api_array = array('SurveyGizmo');
		add_menu_page('Opinionnaire Setup','<span class="opin-title" title="Opinionnaire&reg; Survey analysis and Fast Forum&reg; reports">Opinionnaire&reg;</span>','manage_options','opinionnaire','opin_main_menu');
		add_submenu_page('opinionnaire','SurveyGizmo','SurveyGizmo','manage_options','survey-gizmo','opin_survey_gizmo_form_init');
		add_submenu_page('opinionnaire','Manage Survey Source Credentials','Survey Credentials','manage_options','manage-survey-source-credentials','opin_manage_source_credentials_form_init');
		add_submenu_page('opinionnaire','Sample Data','Sample Data','manage_options','sample-data','opin_sample_data_init');
	}

	function opin_main_menu() {
	...
	function opin_demo_db($action = NULL, $survey_id = NULL, $collection = NULL) {
		global $wpdb;
		...
		if($_REQUEST && $action == 'insert') {
			...
			foreach($_REQUEST['custom_demo_select'] as $value) {
				...
				$data['serial_custom_demographic'] = serialize($sanitized_value);
			}
			if($wpdb->insert($table,$data)) {
				$newJson = unserialize($data['serial_custom_demographic']);
				$newJson = json_encode($newJson);
				echo wp_kses_post($newJson);
				echo "##|" . absint($wpdb->insert_id) . "##|";
				return true;
			}
	...
	//sample data views
	function opin_sample_data_init() {
		...
		$plugindir = plugins_url() . '/' . dirname(plugin_basename(__FILE__));
		wp_enqueue_script('loading', plugins_url( 'js/loading.js', __FILE__ ));
		if (!isset($_REQUEST['reset'])) {
			...
			if(isset($_REQUEST['value_statement_questions']) && isset($_REQUEST['demographic_questions']) && !empty($_REQUEST['value_statement_questions']) && !empty($_REQUEST['demographic_questions'])) {
				...
				foreach($_REQUEST as $kR => $vR) {
					...
					$kR = sanitize_key($kR);
					$vR = sanitize_text_field($vR);
					$check_p = opin_strstr_after($vR, 'valuestatements_', true);
					if($check_p) {
						$value_statement_question_array[] = $check_p;
					}
					$check_s = opin_strstr_after($vR, 'demographics_', true);
					if($check_s) {
						$demographic_question_array[] = $check_s;
					}
					$check_cd = opin_strstr_after($vR, 'custom_demo_select_', true);
					if($check_cd) {
		...
		if($submit == null || $submit == 'Start Over' || $submit == 'Back to Choose Survey') {
			...
			$submit = opin_choose_survey($api_selected,$survey_id,false);
		} else {
			switch($_REQUEST['submit']) {
				...
				case 'Back to Retrieve Survey':
					if(empty($survey_id)) {
						...
						$submit = opin_choose_survey_questions(
							$api_selected,
							$survey_id,
							$survey_title,
				...
				case 'Generate Report':
					...
					if(empty($value_statement_question_array) || (empty($demographic_question_array) && empty($custom_demographics_master))) {
						...
						echo '<input type="hidden" id="api_selected" name="api_selected" value="' . esc_attr($api_selected) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="survey_id" name="survey_id" value="' . esc_attr($survey_id) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="survey_title" name="survey_title" value="' . esc_attr($survey_title) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="significant_threshold" name="significant_threshold" value="' . esc_attr($significant_threshold) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="colorization_threshold" name="colorization_threshold" value="' . esc_attr($colorization_threshold) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="threshold_color_above" name="threshold_color_above" value="' . esc_attr($threshold_color_above) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="threshold_color_below" name="threshold_color_below" value="' . esc_attr($threshold_color_below) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="value_statement_questions" name="value_statement_questions" value="' . esc_attr($value_statement_questions) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="demographic_questions" name="demographic_questions" value="' . esc_attr($demographic_questions) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="participatory_resistance" name="participatory_resistance" value="' . esc_attr($participatory_resistance) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="report_0_cats" name="report_0_cats" value="' . esc_attr($report_0_cats) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="demographics_master" name="demographics_master" value="' . esc_attr($demographic_questions_master) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="valuestatements_master" value="' . esc_attr($value_statement_questions_master) . '" />' . PHP_EOL;
						echo '<input type="hidden" id="custom_demographics_master" name="custom_demographics_master" value="' . esc_attr($custom_demographics_master) . '" />' . PHP_EOL;
						echo '<h3>API: ' . wp_kses_post($api_selected) . '</h3>' . PHP_EOL;
						echo '<h3>Survey Title: ' . wp_kses_post($survey_title) . '</h3>' . PHP_EOL;
						echo '<h3>Category Significance Threshold: ' . wp_kses_post($significant_threshold) . '</h3>' . PHP_EOL;
						echo '<h3>Cell Colorization Threshold: ' . wp_kses_post($colorization_threshold) . '</h3>' . PHP_EOL;
						echo '<h3>Threshold Color Above: <span class="preview-color" style="background-color:' . esc_attr($threshold_color_above) . '">' . wp_kses_post($threshold_color_above) . '</span></h3>' . PHP_EOL;
						echo '<h3>Threshold Color Below: <span class="preview-color" style="background-color:' . esc_attr($threshold_color_below) . '">' . wp_kses_post($threshold_color_below) . '</span></h3>' . PHP_EOL;
						echo '<div>' . PHP_EOL;
						echo '<h3><div class="no-print"><label for="transparency">Threshold Color Transparency: <output id="t_output">' . wp_kses_post($transparency) . '</output></div></h3>' . PHP_EOL;
						echo '<div class="no-print">';
						echo '0.1';
						echo '<input type="button" id="transparency_decrease" name="transparency_decrease" class="transparency-modify" value="-" />';
						echo '<input id="transparency" name="transparency" type="range" value="' . esc_attr($transparency) . '" step="0.1" min="0.1" max="1.0" />';
						...
						echo '<h3>Participatory Resistance: ' . wp_kses_post($pr_show) . '</h3>' . PHP_EOL;
						$r0c = 'Do Not Suppress';
						if(!empty($report_0_cats)) $r0c = 'Suppress';
						echo '<h3>Supress Categories with 0 Responses: ' . wp_kses_post($r0c) . '</h3>' . PHP_EOL;
						echo '<h3>Preview:</h3>' . PHP_EOL;
						if(!isset($custom_demographics)) {
							$custom_demographics = null;
						}
						$created = opin_generate_report(
							$api_selected,
							$survey_id,
							$survey_title,
							$value_statement_question_array,
				...
				case 'Download':
					...
					echo '<input type="hidden" id="participatory_resistance" name="participatory_resistance" value="' . esc_attr($participatory_resistance) . '" />' . PHP_EOL;
					echo '<input type="hidden" id="report_0_cats" name="report_0_cats" value="' . esc_attr($report_0_cats) . '" />' . PHP_EOL;
					echo '<input type="hidden" id="transparency" name="transparency" value="' . esc_attr($transparency) . '" />' . PHP_EOL;
					echo '<input type="hidden" id="demographics_master" name="demographics_master" value="' . esc_attr($demographic_questions_master) . '" />' . PHP_EOL;
					echo '<input type="hidden" id="valuestatements_master" value="' . esc_attr($value_statement_questions_master) . '" />' . PHP_EOL;
					echo '<input type="hidden" id="custom_demographics_master" name="custom_demographics_master" value="' . esc_attr($custom_demographics_master) . '" />' . PHP_EOL;
					echo '<span style="display:none"><output id="t_output">' . wp_kses_post($transparency) . '</output></span>' . PHP_EOL;
					break;
				case 'Print':
					$recall = 'Back to Retrieve Survey';
					echo '<h3>Preview:</h3>' . PHP_EOL;
		...
		echo '<div class="no-print' . esc_attr($override_class) . '"' . wp_kses_post($override_css) . '>' . PHP_EOL;
		echo '<hr />' . PHP_EOL;
	...
	function opin_send_message($id, $message, $progress) {
		$d = array('message' => $message , 'progress' => $progress);
		$id = wp_kses($id);
		echo "id: ". wp_kses_post($id) . PHP_EOL;
	...
	function opin_manage_source_credentials_form_init() {
		$plugindir = plugins_url() . '/' . dirname(plugin_basename(__FILE__));
		...
		if(isset($_REQUEST['submit'])) {
			...
			if($_REQUEST['submit'] == 'Test') {
				...
				if(empty($test_message)) {
					...
					$sql = $wpdb->prepare("SELECT * FROM $users_table_name WHERE id= %d AND user_id= %d LIMIT 1;", $credential_id, $user_id);
					$ex_credentials_check = $wpdb->get_results( $sql );
					if(!empty($ex_credentials_check)) {
						if(opin_survey_test_api_call($ex_credentials_check)) {
			...
			if(!empty($credential_id)) {
				$sql = $wpdb->prepare("SELECT * FROM $users_table_name WHERE id= %d AND user_id= %d LIMIT 1;", $credential_id, $user_id);
				$retrieve_source_user = $wpdb->get_results( $sql );
		...
		$sql = $wpdb->prepare(
			"SELECT a.id, a.source_user, b.name 
			 FROM {$wpdb->prefix}opinionnaire_users a 
			 LEFT JOIN {$wpdb->prefix}opinionnaire_sources b 
			 ON b.id = a.source_id 
			 WHERE a.user_id = %d;",
			$user_id
		);
		$user_credentials = $wpdb->get_results( $sql);
		...
		if(isset($test_message[0])) {
			echo '<div class="error_message">' . PHP_EOL;
			echo wp_kses_post($test_message[0]);
			echo '</div>' . PHP_EOL;
		}
		...
		if(!empty($user_credentials)) {
			foreach($user_credentials as $user) {
				echo '<tr><td>' . PHP_EOL;
				echo '<form name="edit_credentials' . esc_attr($user->id) . '" method="post">' . PHP_EOL;
				echo '<input type="hidden" id="credential_id" name="credential_id" value="' . esc_attr($user->id) . '" />' . PHP_EOL;
				echo '<input type="submit" id="submit' . esc_attr($user->id) . '" name="submit" value="' . esc_attr($user->source_user) . '" />' . PHP_EOL;
				echo '</form>' . PHP_EOL;
				echo '</td><td>' . wp_kses_post($user->name) . '</td>' . PHP_EOL;
				echo '<td>' . PHP_EOL;
				echo '<form name="test_credentials' . esc_attr($user->id) . '" method="post">' . PHP_EOL;
				echo '<input type="hidden" id="credential_id" name="credential_id" value="' . esc_attr($user->id) . '" />' . PHP_EOL;
				echo '<input type="submit" id="submit' . esc_attr($user->id) . '" name="submit" value="Test" />' . PHP_EOL;
				echo '</td>' . PHP_EOL;
				echo '<td class="test-result-message">' . (isset($test_message[$user->id]) ? wp_kses_post($test_message[$user->id]) : '') . '</td>' . PHP_EOL;
				echo '</tr>' . PHP_EOL;
			}
		} else {
		...
		if(!empty($credential_id)) {
			...
			echo '<input type="hidden" id="ex_credential_id" name="ex_credential_id" value="' . esc_attr($credential_id) . '" />' . PHP_EOL;
		...
		if(!empty($errors)) {
			...
			if(isset($errors['general'])) echo '<br />' . wp_kses_post($errors['general']) . PHP_EOL;
			echo '</div>' . PHP_EOL;
		}
		echo '</div>' . PHP_EOL;
		...
		foreach($sources as $source) {
			echo '<option value="' . esc_attr($source->id) . '"';
			if($source->id == $source_id) echo ' selected="selected"';
			echo '>' . wp_kses_post($source->name) . '</option>' . PHP_EOL;
		}
		echo '</select>' . PHP_EOL;
		if(isset($errors['source_id'])) echo '<div class="error">' . wp_kses_post($errors['source_id']) . '</div>' . PHP_EOL;
		echo '</div>' . PHP_EOL;
		echo '<div class="input">' . PHP_EOL;
		echo '<label for="source_user">API Token: </label>' . PHP_EOL;
		echo '<input type="text" id="source_user" name="source_user" value="' . esc_attr($source_user) . '" />' . PHP_EOL;
		if(isset($errors['source_user'])) echo '<div class="error">' . wp_kses_post($errors['source_user']) . '</div>' . PHP_EOL;
		echo '</div>' . PHP_EOL;
		echo '<div class="input">' . PHP_EOL;
		echo '<label for="source_pass">API Token Secret: </label>' . PHP_EOL;
		echo '<input type="password" id="source_pass" name="source_pass" value="' . esc_attr($source_pass) . '" />' . PHP_EOL;
		if(isset($errors['source_pass'])) echo '<div class="error">' . wp_kses_post($errors['source_pass']) . '</div>' . PHP_EOL;
		echo '</div>' . PHP_EOL;
		echo '<input type="submit" id="submit" name="submit" value="' . esc_attr($button_text) . '" />' . PHP_EOL;
		echo '</div>' . PHP_EOL;
	}


	function opin_survey_gizmo_form_init() {
		global $wpdb;
		...
		$sql = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}opinionnaire_sources WHERE name= %s LIMIT 1;", $api_selected);
		$opin_source = $wpdb->get_results($sql);
		$check_survey_exists = opin_redirect_to_add_credentials($opin_source,$api_selected);
		$plugindir = plugin_dir_url(__FILE__);
		wp_enqueue_script(
			'plugin-loading-script', // Handle
			plugins_url('js/loading.js', __FILE__), // URL to the script
			array('jquery'), // Dependencies (optional)
			'1.0.0', // Version (optional)
			true // Load in footer (optional, set to true for footer)
		);
		
		if($check_survey_exists) {
			echo wp_kses_post($check_survey_exists);
		} else {
			//make sure the user has credentials for the source
			$survey_source_id = $opin_source[0]->id;
			$sql = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}opinionnaire_users WHERE user_id= %d AND source_id= %d LIMIT 1;", $user_id, $survey_source_id);
			$opin_credentials = $wpdb->get_results($sql);
			$check_credentials_exist = opin_check_credentials($opin_credentials,$api_selected);
			if($check_credentials_exist) {
				echo wp_kses_post($check_credentials_exist);
			} else {
				$significant_threshold = 0;
				$colorization_threshold = 0;
				$threshold_color_above = '#3485B5';
				...
				if(!isset($_REQUEST['reset'])) {
					...
					if(isset($_REQUEST['value_statement_questions']) && isset($_REQUEST['demographic_questions']) && !empty($_REQUEST['value_statement_questions']) && !empty($_REQUEST['demographic_questions'])) {
						...
						foreach ($_REQUEST as $kR => $vR) {
							...
							// Process the sanitized value
							// 'opinionnaire_strstr_after' presumably processes the string safely internally
							$check_p = opin_strstr_after($vR, 'valuestatements_', true);
							if ($check_p) {
								$value_statement_question_array[] = $check_p;
							}
				...
				if($submit == null || $submit == 'Start Over' || $submit == 'Back to Choose Survey') {
					$override_class = ' select_survey';
					$override_css = ' style="display:none"';
					$submit = opin_choose_survey($api_selected,$survey_id,$opin_credentials);
				} else {
					switch($_REQUEST['submit']) {
						case 'Retrieve Survey'://fall through
						case 'Back to Retrieve Survey':
							if(empty($survey_id)) {
								...
								$params = [
									...
									'value_statement_questions_array' => $value_statement_question_array,
									'demographic_question_array' => $demographic_question_array,
									'user_survey_title' => $user_survey_title,
									'transparency' => $transparency,
									'threshold_color_above' => $threshold_color_above,
									'threshold_color_below' => $threshold_color_below,
									'significant_threshold' => $significant_threshold,
									'colorization_threshold' => $colorization_threshold,
									'participatory_resistance' => $participatory_resistance,
									'report_0_cats' => $report_0_cats,
									'value_statement_questions_selected_array' => $value_statement_questions_selected_array,
									'demographic_questions_selected_array' => $demographic_questions_selected_array,
									'value_statment_questions_master' => $value_statement_questions_master,
									'demographic_questions_master' => $demographic_questions_master,
									'custom_demographics_array' => $custom_demographics_array,
									'custom_demographics_master' => $custom_demographics_master,
									'opin_credentials' => $opin_credentials
								];
								$submit = opin_choose_survey_questions(
									$api_selected,
									$survey_id,
						...
						case 'Generate Report':
							...
							if(empty($value_statement_question_array) || (empty($demographic_question_array) && empty($_REQUEST['custom_demo_select']))) {
								...
								echo '<h3>Preview:</h3>' . PHP_EOL;
								echo '<image src="' . esc_url($plugindir) . '/assets/loading.gif" id="loading_gif" name="loading_gif" alt="loading..." />' . PHP_EOL;
								$created = opin_generate_report(
									$api_selected,
									$survey_id,
									$survey_title,
				...
				echo '<div class="no-print' . esc_attr($override_class) . '"' . esc_attr($override_css) . '>' . PHP_EOL;
				echo '<hr />' . PHP_EOL;
				submit_button($submit,'primary','submit',false);
				if(!empty($recall)) {
					echo '&nbsp;&nbsp;' . PHP_EOL;
	...
	function opin_choose_api($api_array,$api_selected) {
		asort($api_array);
		...
		foreach($api_array as $aKey => $aValue) {
			$aKey = sanitize_key($aKey);
			$aValue = sanitize_text_field($aValue);
			$selected = '';
			if($api_selected == $aValue) $selected = ' selected';
			echo '<option value="' . esc_attr($aValue) . '"' . esc_attr($selected) . '>' . wp_kses_post($aValue) . '</option>' . PHP_EOL;
		}
		echo '</select>' . PHP_EOL;
		return 'Select API';
	}

	function opin_choose_survey($api_selected,$survey_id,$opin_credentials=false) {
		$plugindir = plugins_url() . '/' . dirname(plugin_basename(__FILE__));
		wp_enqueue_script(
			'plugin-questions-script', // Handle
			plugins_url('js/questions.js', __FILE__), // URL to the script
			array('jquery'), // Dependencies (optional)
			'1.0.0', // Version (optional)
			true // Load in footer (optional, set to true for footer)
		);
		$surveys = opin_form_populate($opin_credentials);
		...
		echo '<h3>API: ' . wp_kses_post($api_selected) . '</h3>' . PHP_EOL;
		echo '<div id="loading"><image src="' . esc_attr($plugindir) . '/assets/loading.gif" id="loading_gif" name="loading_gif" alt="loading..." /></div>' . PHP_EOL;
		...
		foreach($surveys as $sKey => $sValue) {
			...
			echo '<option value="' . esc_attr($sKey) . '"' . esc_attr($selected) . '>' . wp_kses_post($sValue) . '</option>' . PHP_EOL;
		}
		echo '</select>'. PHP_EOL;
		echo '</div>' . PHP_EOL;
	...
	function opin_choose_survey_questions(
		$api_selected,
		$survey_id,
		$survey_title,
		$value_statement_question_array,
		...
		$value_statement_question_array = explode(',', sanitize_text_field($_REQUEST['value_statement_questions']) );
		$value_statement_questions_selected_array = $value_statement_question_array;
		$questions = opin_retrieve_survey_questions($survey_id, $opin_credentials, array(), array());
		if ($questions !== '<h3>Report not found.</h3>' . PHP_EOL) {
			if(isset($_REQUEST['cd_list'])) {
				...
				echo "<input type='hidden' value='" . esc_attr($cd_list_sanitized) . "' id='cd_list'>" . PHP_EOL;
			}
			$plugindir = plugins_url() . '/' . dirname(plugin_basename(__FILE__));
			wp_enqueue_script(
				'plugin-questions-script', // Handle
			...
			echo '<input type="text" id="user_survey_title" name="user_survey_title" value="' . esc_attr($user_survey_title) . '" size=100 />' . PHP_EOL;
			echo '<h3>API: ' . wp_kses_post($api_selected) . '</h3>' . PHP_EOL;
			echo '<h3>Survey: ' . wp_kses_post($survey_title) . '</h3>' . PHP_EOL;
			echo '<div class="opin-input-numeric">' . PHP_EOL;
			echo '<label for="significant_threshold">Category Significance Threshold: </label> <output id="st_output">' . wp_kses_post($significant_threshold) . '</output>' . PHP_EOL;
			echo '<div>' . PHP_EOL;
			echo '0';
			echo '<input type="button" id="sig_threshold_decrease" name="sig_threshold_decrease" class="sig-threshold-modify" value="-" />';
			echo '<input id="significant_threshold" name="significant_threshold" type="range" value="' . esc_attr($significant_threshold) . '" step="1" min="0" max="50" />';
			echo '<input type="button" id="sig_threshold_increase" name="sig_threshold_increase" class="sig-threshold-modify" value="+" />';
			echo '50';
			echo '</div>' . PHP_EOL;
			echo '</div><br />' . PHP_EOL;
			echo '<div class="opin-input-numeric">' . PHP_EOL;
			echo '<label for="colorization_threshold">Cell Colorization Threshold: </label> <output id="ct_output">' . wp_kses_post($colorization_threshold) . '</output>' . PHP_EOL;
			echo '<div>' . PHP_EOL;
			echo '0';
			echo '<input type="button" id="col_threshold_decrease" name="col_threshold_decrease" class="col-threshold-modify" value="-" />';
			echo '<input id="colorization_threshold" name="colorization_threshold" type="range" value="' . esc_attr($colorization_threshold) . '" step="1" min="0" max="50" />';
			...
			echo '<span class="notIeSafari"><input id="threshold_color_above" name="threshold_color_above" type="color" value="' . esc_attr($threshold_color_above) . '" /></span>' . PHP_EOL;
			echo '<span class="ieSafari" style="display:none"><input class="color" id="threshold_color_above_sf" name="threshold_color_above_sf" value="' . esc_attr($threshold_color_above) . '" /></span>' . PHP_EOL;
			echo '</div><br />' . PHP_EOL;
			echo '<div class="opin-input-color">' . PHP_EOL;
			echo '<label for="threshold_color_below">Threshold Color Below: </label>' . PHP_EOL;
			echo '<span class="notIeSafari"><input id="threshold_color_below" name="threshold_color_below" type="color" value="' . esc_attr($threshold_color_below) . '" /></span>' . PHP_EOL;
			echo '<span class="ieSafari" style="display:none"><input class="color" id="threshold_color_below_sf" name="threshold_color_below_sf" value="' . esc_attr($threshold_color_below) . '" /></span>' . PHP_EOL;
			echo '</div><br />' . PHP_EOL;
			echo '<div class="opin-input-text">' . PHP_EOL;
			echo '<label for="transparency">Threshold Color Transparency: </label><output id="t_output">' . wp_kses_post($transparency) . '</output><br /><br />' . PHP_EOL;
			echo '0.1';
			echo '<input type="button" id="transparency_decrease" name="transparency_decrease" class="transparency-modify" value="-" />';
			echo '<input id="transparency" name="transparency" type="range" value="' . esc_attr($transparency) . '" step="0.1" min="0.1" max="1.0" />';
			echo '<input type="button" id="transparency_increase" name="transparency_increase" class="transparency-modify" value="+" />';
			...
			echo '<input id="participatory_resistance" name="participatory_resistance" type="checkbox" value="display"' . esc_attr($pr_checked) . ' />' . PHP_EOL;
			echo '</div><br />' . PHP_EOL;
			...
			echo '<input id="report_0_cats" name="report_0_cats" type="checkbox" value="Supress"' . wp_kses_post($r0c_checked) . ' />' . PHP_EOL;
			echo '</div>' . PHP_EOL;
			...
			echo '<input type="checkbox" name="demographics_master" id="demographics_master" value="demographics_master" class="primary-check-box" onchange="javascript:checkAllQuestions(\'demographics\',false);"' . esc_attr($checked_demo_master) . ' />' . PHP_EOL;
			...
			foreach($questions['demographic'] as $kQ => $vQ) {
				...
				echo '<input type="checkbox" name="question_' . esc_attr($kQ) . '" id="demographics_' . esc_attr($kQ) . '" value="demographics_' . esc_attr($kQ) . '" class="primary-check-box"' . esc_attr($checked_demo) . ' />' . PHP_EOL;
				echo '</td><td>' . PHP_EOL;
				echo '<label for="valuestatements_' . esc_attr($kQ) . '">' . wp_kses_post($vQ) . '</label>' . PHP_EOL;
				echo '</td></tr>' . PHP_EOL;
			}
			...
			$retrieved_custom_demos = opin_demo_db('list', $_REQUEST['survey_id']);
			if(!isset($checked_custom_demo_master)) {
				$checked_custom_demo_master = '';
			}
				echo '<hr>';
				echo '<table id="choose_questions_custom_demographics" style="' . esc_attr($display) . '"><thead>' . PHP_EOL;
			...
			echo '<input type="checkbox" name="custom_demographics_master" id="custom_demographics_master" value="custom_demographics_master" class="primary-check-box" onchange="javascript:checkAllQuestions(\'custom_demographics\',false);"' . esc_attr($checked_custom_demo_master) . ' />' . PHP_EOL;
			...
			if ($retrieved_custom_demos) {
				...
				foreach ($retrieved_custom_demos as $serial_obj) {
					...
					echo "<input type='checkbox' name='question_" . esc_attr($i) . "' id='custom_demographics_" . esc_attr($i) . "' value='" . esc_attr($i) . "' data-dbid='" . esc_attr($serial_obj->id) . "' data-json='" . esc_attr(json_encode($collections)) . "' class='cd-check-box primary-check-box'/>" . PHP_EOL;
					echo '</td><td>' . PHP_EOL;
					echo '<label for="custom_demographics_' . esc_attr($i) . '">' . wp_kses_post($collections['title']) . '</label>' . PHP_EOL;
					echo '<td class="edit-questions-radio edit-custom-demographics-radio">' . PHP_EOL;
					echo "<input type='checkbox' class='dashicons dashicon-edit' name='question_" . esc_attr($i) . "' id='edit_custom_demographics_" . esc_attr($i) . "' value='" . esc_attr($i) . "' data-dbid='" . esc_attr($serial_obj->id) . "' data-json='" . esc_attr(json_encode($collections)) . "' class='primary-check-box'/>" . PHP_EOL;
					echo '</td>';
					echo '<td class="remove-questions-radio custom-demographics-radio">' . PHP_EOL;
					echo "<a href='#' id='remove_custom_demographics_" . esc_attr($i) . "' data-dbid='" . esc_attr($serial_obj->id) . "'>x</a>" . PHP_EOL;
					echo '</td></tr>' . PHP_EOL;
					$i++;
				}
				echo '</tbody></table>' . PHP_EOL;
			...
			echo "<input type='hidden' id='cd_json' class='cd_json' value='" . esc_attr(str_replace("'", "&#39;", $custom_demo_json)) . "' />" . PHP_EOL;
			...
			echo '<input type="checkbox" id="valuestatements_master" value="valuestatements_master" class="primary-check-box" onchange="javascript:checkAllQuestions(\'valuestatements\',false);"' . esc_attr($checked_vs_master) . ' />' . PHP_EOL;
			echo '</td><td>Select All Value Statement Questions</td></tr>' . PHP_EOL;
			foreach($questions['value_statement'] as $kQ => $vQ) {
				$checked_vs = '';
				$kQ = sanitize_key($kQ);
				$vQ = sanitize_text_field($vQ);
				...
				echo '<input type="checkbox" name="question_' . esc_attr($kQ) . '" id="valuestatements_' . esc_attr($kQ) . '" value="valuestatements_' . esc_attr($kQ) . '" class="primary-check-box"' . esc_attr($checked_vs) . ' />' . PHP_EOL;
				echo '</td><td>' . PHP_EOL;
				echo '<label for="valuestatements_' . wp_kses_post($kQ) . '">' . wp_kses_post($vQ) . '</label>' . PHP_EOL;
				echo '</td></tr>' . PHP_EOL;
			}
			echo '</tbody></table>' . PHP_EOL;
			...
			if(!empty($questions['no_keycode'])) {
				...
				foreach($questions['no_keycode'] as $kNK => $vNK) {
					echo '<tr><td>' . wp_kses_post($kNK) . '</td><td>' . wp_kses_post($vNK) . '</td></tr>' . PHP_EOL;
				}
				echo '</tbody></table>' . PHP_EOL;
				echo '</div>' . PHP_EOL;
			}
			...
			if(!empty($questions['keycode_out_of_bounds'])) {
				...
				foreach($questions['keycode_out_of_bounds'] as $kKOOB => $vKOOB) {
					echo '<tr><td>' . wp_kses_post($kKOOB) . '</td><td>' . wp_kses_post($vKOOB) . '</td></tr>' . PHP_EOL;
				}
			...
			if(!empty($questions['wrong_subtype'])) {
				...
				foreach($questions['wrong_subtype'] as $kWS => $vWS) {
					echo '<tr><td>' . wp_kses_post($kWS) . '</td><td>' . wp_kses_post($vWS) . '</td></tr>' . PHP_EOL;
				}
				echo '</tbody></table>' . PHP_EOL;
				echo '</div>' . PHP_EOL;
			}
			...
			echo wp_kses_post($questions);
			return 'Start Over';
		}
	}

	//create cross tab report
	function opin_generate_report(
		$api_selected,
		$survey_id,
		$survey_title,
		$value_statement_question_array,
		...
		if(isset($survey_id)) {
			if($opin_credentials !== false) {//get live data
				...
				$questions = opin_retrieve_survey_questions($survey_id,$opin_credentials,$value_statement_question_array,$demographic_question_array);
				//raw survey responses
				$responses = opin_retrieve_survey($survey_id,$opin_credentials);
			} else {//get sample data
				//psuedo-raw selected questions
				$questions = opin_retrieve_survey_questions($survey_id,false,$value_statement_question_array,$demographic_question_array);
				//raw survey responses
				$responses = opin_retrieve_survey($survey_id,false);
			}
		}
		...
		echo "<input type='hidden' value='" . esc_attr($cd_list) . "' name='cd_list'>";
			...
			if(!empty($pat_res)) {
				...
				foreach($pat_res_final as $prfKey => $prfValue) {
					...
					if(isset($the_matrix[$prfKey][0])) {
						$datax[] = opin_graph_text_normalize($pat_res_key_map[$prfKey]) . "\r\n" . '(N=' . $the_matrix[$prfKey][0] . ')';
					} else {
				...
				$cross_tab .= '</div>' . PHP_EOL;
				$cross_tab .= '<div id="pr_graphic" style="text-align:left;" width="700px"><img src="' . plugins_url() . '/' .str_replace(basename( __FILE__),"",plugin_basename(__FILE__)) . 'out/' . $graphKey . '.png" alt="'. esc_attr__('Participatory Resistance', 'opinionnaire') . '" style="width:700px;" />' . PHP_EOL;
				$cross_tab .= '</div>';
				$cross_tab .= '<br />';
			} else {
	...
	//create cross tab report
	function opin_generate_report_print(
	...
	function opin_build_get_request($url_front,$url_back,$opin_credentials,$per_page) {
		global $wpdb;
		...
		$output = opin_execute_get_request($url . "&page=1&resultsperpage=" . $per_page);
		...
		if($output->total_count > 1) {
			...
			if($total_pages > 1) {
				...
				for($i=2;$i<$iterations_plus;$i++) {
					$additional_output = opin_execute_get_request($url . "&page=" . $i . "&resultsperpage=" . $per_page);
		...
		if($output->result_ok === false) {
			opin_display_survey_unable_to_connect($output->message);
		}
		return $output;
	}

	function opin_execute_get_request($url){
	...
	function opin_display_survey_unable_to_connect($message='') {
		echo '<div class="error">Unable to Connect to API: ' . wp_kses_post($message) . '</div>' . PHP_EOL;
	}

	//intermediary returns list array of surveys for a given api, possibly needless, we'll see when we incorporate other api's
	function opin_form_populate($opin_credentials=false) {
		$surveys = opin_survey_get_list($opin_credentials);
		return $surveys;
	}

	// gets a list of the surveys from the api and returns them as a list array('id'=>'name (id)')
	function opin_survey_get_list($opin_credentials=false) {
		if($opin_credentials !== false) {
			$url_front = "https://restapi.surveygizmo.com/v4/survey?";
			$url_back = '';
			$surveys = opin_build_get_request($url_front,$url_back,$opin_credentials,500);
			$survey_list = array();
			if(!empty($surveys)) {
	...
	function opin_survey_test_api_call($opin_credentials=false) {
		$result_ok = false;
		if($opin_credentials !== false) {
			$url_front = "https://restapi.surveygizmo.com/v4/survey?";
			$url_back = '';
	...
	function opin_retrieve_survey_questions($survey_id,$opin_credentials=false,$value_statement_question_array=array(),$demographic_question_array=array()) {
		...
		if(!empty($survey_id)) {
			if($opin_credentials !== false) {
				...
				$url_back = '';
				$questions = opin_build_get_request($url_front,$url_back,$opin_credentials,500);
			} else {//use sample data
				global $wpdb;
			...
			if(!empty($questions) && $questions->result_ok == 1 && $questions->total_count > 0) {
				if(empty($value_statement_question_array)) {
					$return_questions = opin_build_question_list($questions,$accepted_subtypes);
				} else {
					$return_questions = opin_build_chosen_questions($questions,$accepted_subtypes,$value_statement_question_array,$demographic_question_array);
				}
	...
	function opin_build_question_list($questions,$accepted_subtypes) {
		$return_questions = array();
		...
		foreach($questions->data as $qKey => $qValue) {
			$array_name = 'unknown';
			$disabled = $qValue->properties->disabled ?? false;
			if(!$disabled) {
	...
	function opin_build_chosen_questions($questions,$accepted_subtypes,$value_statement_question_array,$demographic_question_array) {
		$return_questions = array();
		foreach($questions->data as $qKey => $qValue) {
			if($qValue->_type == 'SurveyQuestion' && empty($qValue->properties->hidden) && in_array($qValue->_subtype,$accepted_subtypes)) {
	...
	function opin_retrieve_survey($survey_id,$opin_credentials=false) {
		if(isset($survey_id) && !empty($survey_id)) {
			...
			//echo $url;
			if($opin_credentials !== false) {
				$output = opin_build_get_request($url_front,$url_back,$opin_credentials,500);
			} else {//use sample data
				global $wpdb;
				$sample_data_table_name = $wpdb->prefix . 'opinionnaire_sample_data';
	...
	*/
	...
	function opin_strstr_after($haystack, $needle, $case_insensitive = false) {
	...
	function opin_form_handle_post() {
		$return = array();
		$return['data'] = '';
		if(isset($_REQUEST['survey_id'])) {
			$survey_id_sanitized = sanitize_text_field($_REQUEST['survey_id']);
			$return['questions'] = opin_retrieve_survey_questions($survey_id_sanitized);
		}
	...
	function opin_redirect_to_add_credentials($opin_source,$api_selected) {
		...
		opin_redirect_to_get_credentials($message);
	}
	 ...
	 * source_id: id of source for the given api
	 */
	function opin_redirect_to_get_credentials($message){
	 ...
	 * title: string
	 */
	function opin_graph_text_normalize($title='') {
