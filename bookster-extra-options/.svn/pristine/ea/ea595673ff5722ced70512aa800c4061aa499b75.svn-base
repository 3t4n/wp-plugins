...
class ExtraController extends BaseRestController {
        ...
        register_rest_route(
            ...
            [
                [
                    ...
                    'callback'            => [ $this, 'exec_delete_extras_bulk' ],
                    'permission_callback' => [ RestAuth::class, 'require_manage_shop_records_cap' ],
                ],
            ]
        );
    ...
    public function delete_extras_bulk( \WP_REST_Request $request ) {
        $args = $request->get_json_params();
        foreach ( $args['ids'] as $id ) {
            $this->extras_service->delete( $id );
        }
        return $this->get_all_extras();
    }
    ...
    public function exec_delete_extras_bulk( $request ) {
        return $this->exec_write( [ $this, 'delete_extras_bulk' ], $request );
    }
}
