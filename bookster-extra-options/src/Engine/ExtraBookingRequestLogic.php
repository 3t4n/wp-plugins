...
class ExtraBookingRequestLogic {
    ...
    protected function __construct() {
        ...
        add_filter( 'bookster_booking_duration', [ $this, 'add_duration_extra' ], 10, 3 );
    ...
    public function add_duration_extra( $booking_duration, $booking_input, $service ) {
        if ( empty( $booking_input['bookingMetaInput']['extraBookingItems'] ) ) {
            return $booking_duration;
        }

        $has_multiple_bookings = apply_filters( 'bookster_service_has_multiple_bookings', false, $service );
        if ( $has_multiple_bookings ) {
            // Duration need to be consistent between all bookings
            return $booking_duration;
        }
