...
class EWalletCartModule
{
    ...
    public function getCouponLabel($label, $coupon)
    {
        if (EWalletCoupon::isEWalletCoupon($coupon->get_code())) {
            /**
             * @since 3.7.1 Added filter for customizing translation.
             */
            return apply_filters('mlmsoft_integration_translate', MLMSoftPlugin::translate('Payment from an e-wallet'));
        }
        return $label;
    }
