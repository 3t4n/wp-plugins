...
use MLMSoft\integrations\woocommerce\paymentGateways\eWallet\MLMSoftEWalletGateway; // @since 3.7.1
use WP_Error;
...
class WCAccountModule
{
    ...
    public function __construct($mlmsoftPlugin, $options)
    {
         ...
         * @since 3.7.1
         */
        add_filter('woocommerce_registration_errors', array($this, 'checkPaymentMethod'), 9, 3);
    ...
    /**
     * Check the payment method before registering a new user on the checkout page 
     * to prevent the user and order from being created when the "Pay with bonus" option is selected.
     *
     * @since 3.7.1
     *
     ...
     * @return mixed
     */
    public function checkPaymentMethod($errors, $username, $email)
    {
        if ( is_checkout() ) {
            
            if ( ! empty($_POST['payment_method']) && $_POST['payment_method'] === MLMSoftEWalletGateway::GATEWAY_ID ) {
                
                /**
                 * @see mlm-soft-integration\integrations\woocommerce\paymentGateways\eWallet\MLMSoftEWalletGateway.php
                 */
                $message = apply_filters(
                    'mlmsoft_integration_checkout_ewallet_payments_message', 
                    MLMSoftPlugin::translate('This payment system is not meant for direct payments.')
                ); 
