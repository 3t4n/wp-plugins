...
class MLMSoftAuthModule
{
     ...
     *
     * @since 3.4.24
     * @since 3.7.0 Added `useOnlineOffice`, `onlineOfficeAuthSecretKey` options.
     */
    public function checkRemoteUserAuth()
    {
        ...
        $secret = $this->mlmsoftPlugin->options->onlineOfficeAuthSecretKey;
        if ( empty($secret) && defined('MLMSOFT_REMOTE_AUTH_AES_PHRASE') ) {
            /**
             * @see wp-config.php
             */
            $secret = MLMSOFT_REMOTE_AUTH_AES_PHRASE; 
        }
        ...
        /**
         * Added `is_user_logged_in` function.
         *
         * @since 3.8.3
         */
        if ( isset($_GET['incorrect_credentials']) && ! is_user_logged_in() ) {
            wc_add_notice( 
                __('Login or password is incorrect', 'mlmsoft'),					 
                'notice'
            );
        }
        ...
        if( isset($_GET['username']) && isset($_GET['token']) ) {
            ...
            if ( is_user_logged_in() ) {
                ...
                $urlPiece = str_replace($query, '', $_SERVER['REQUEST_URI']);
                wp_redirect( home_url( trim($urlPiece, '/') ) );
                exit;                
            }
            ...
            $password = MLMSoftAesEncryption::decryptRemoteAuthToken($_GET['token'], $secret);
