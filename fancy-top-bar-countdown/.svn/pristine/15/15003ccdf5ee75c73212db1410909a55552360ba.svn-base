(function( $ ) {
	...
	$( '.cmb-type-nn-select .select2' ).each(function() {
		$( this ).select2({
			allowClear: true
		});
	});

	$( '.cmb-type-nn-multiselect .select2' ).each(function() {
		var instance = $( this ),
		...
		$( instance ).select2({
			...
			initSelection: function( element, callback ) {
				...
				$( element.val().split( ',' ) ).each(function() {
					var text = nn_select2_find_text( this, instance_data );
	...
	function nn_select2_find_text( id, instance_data ) {
		var i, l;
