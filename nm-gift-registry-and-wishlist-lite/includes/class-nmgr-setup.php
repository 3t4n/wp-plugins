...
class NMGR_Setup {
	...
	public function run() {
		...
		NMGR\Blocks\Search::run();
		NMGR\Blocks\Cart::run();
		NMGR\Blocks\AddToWishlistButton::run();
		NMGR\Blocks\Archive::run();
		NMGR\Blocks\Wishlist::run();

		if ( nmgr()->is_licensed && class_exists( \NMGR\Sub\License::class ) ) {
			(new \NMGR\Sub\License() )->run();
		}

		new NMGR\Lib\WidgetCart();
		new NMGR\Lib\WidgetSearch();
