	...
	public static function save_settings() {
		...
		if ( !empty( $_POST[ 'nmgr_setup_submit' ] ) && wp_verify_nonce( $_POST[ '_wpnonce' ] ) ) {
			...
			if ( !empty( $_POST[ 'nmgr_setup_save' ] ) ) {
				...
				$wishlist_block = '<!-- wp:nm-gift-registry/wishlist {"wishlistType":"wishlist"} /-->';
				$gr_wishlist_block = '<!-- wp:nm-gift-registry/wishlist /-->';
				$archive_block = '<!-- wp:nm-gift-registry/archive /-->';

				if ( 'wishlist' === $posted_section ) {
					$block_code = $wishlist_block;
				} else {
					$block_code = !empty( $sanitized_posted_data[ 'enable_archives' ] ) ? $archive_block : $gr_wishlist_block;
				}

				if ( 'create' === ($sanitized_posted_data[ $page_id_key ] ?? '') ) {
					$page_args = [
						'post_title' => $sanitized_posted_data[ $field_key( 'page_title' ) ],
						'post_content' => $block_code,
						'post_status' => 'publish',
						'post_type' => 'page',
					...
					$page_args = [
						'post_content' => $block_code . $page->post_content,
						'comment_status' => 'closed',
						'ID' => $page->ID,
					];
