...
class NMGR_Ajax {
	...
	public function run() {
		add_action( 'wp_ajax_nmgr_post_action', array( $this, 'post_action' ) );
		add_action( 'wp_ajax_nopriv_nmgr_post_action', array( $this, 'post_action' ) );
	}

	protected function add_to_cart() {
		$data = $_POST[ 'nmgr_add_to_cart_item_data' ];
		nmgr()->order()->add_to_cart( $data );
		...
		$custom_data = array(
			...
			'redirect_url' => esc_url( apply_filters( 'nmgr_ajax_add_to_cart_redirect_url', $redirect_url, $success, $data ) ),
		);
	...
	protected function add_item() {
		...
		foreach ( $items_to_add as $item ) {
			...
			if ( $product ) {
				$qty = isset( $item[ 'product_qty' ] ) && $item[ 'product_qty' ] ? $item[ 'product_qty' ] : 1;
				$favourite = ( int ) ($item[ 'product_fav' ] ?? 0 );
	...
	protected function search_products() {
		...
		if ( !empty( $term ) ) {
			...
			foreach ( $product_objects as $product_object ) {
				...
				if ( !empty( $_POST[ 'display_stock' ] ) && $product_object->managing_stock() ) {
				...
				$products[ $product_object->get_id() ] = sanitize_text_field( $formatted_name );
			}
	...
	protected function load_wishlist_cart() {
		...
		wp_send_json( [
			'replace_templates' => [
				".nmgr-cart[data-type=$type]" => NMGR\Blocks\Cart::template( [ 'wishlistType' => $type ] )
			]
		] );
	}
	...
	protected function show_add_items_dialog() {
		...
		$columns = apply_filters( 'nmgr_add_items_table_columns', array(
			$prod_text => '<td data-title="' . $prod_text . '"><select class="nmgr-product-search" name="product_id" data-display_stock="true" data-text-input-too-short="' . $language . '" data-placeholder="' . $placeholder_text . '"></select></td>',
