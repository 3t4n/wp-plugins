...
function is_nmgr_wishlist() {
	_deprecated_function( __FUNCTION__, '5.7', 'See NMGR\Lib\Single::class for replacement' );
...
function is_nmgr_search( $query = '' ) {
	_deprecated_function( __FUNCTION__, '5.6' );
...
function is_nmgr_account_section( $section = '' ) {
	...
	return apply_filters_deprecated( 'is_nmgr_account_section', [ $bool ], '5.7' );
}
...
function nmgr_get_current_wishlist_id() {
	_deprecated_function( __FUNCTION__, '5.7', 'Use NMGR\Lib\Single->get_current_wishlist_id' );
	$single = new NMGR\Lib\Single();
	$single->type = $single->get_current_type();
	return $single->get_current_wishlist_id();
}
...
function nmgr_get_svg( $args ) {
	...
	$defaults = array(
		...
		/**
		 * Only show sprite if we are not in a rest request
		 * Sprite file is enqueued in the page on page load so it does not appear in the block editor
		 * which uses rest request. So we have to disable it when doing rest requests.
		 */
		'sprite' => !$is_rest_request,
		'role' => 'img',
...
function nmgr_user_can_manage_wishlist( $wishlist_id = 0 ) {
	return $wishlist_id ? (current_user_can( 'edit_posts' ) || nmgr_user_has_wishlist( $wishlist_id )) : false;
}
...
function nmgr_get_current_type() {
	_deprecated_function( __FUNCTION__, '5.7', 'NMGR\Lib\Single()->get_current_type' );
	return (new NMGR\Lib\Single)->get_current_type();
}
