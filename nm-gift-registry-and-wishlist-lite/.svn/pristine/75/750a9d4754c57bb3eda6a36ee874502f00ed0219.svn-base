...
class Single {
	...
	public $page;
	public $type;
	public $source = 'wishlist'; // assumes we are getting template from [nmgr_wishlist] shorcode or block.

	public function __construct( $id = 0 ) {
		if ( $id ) {
			$this->initialize_wishlist( $id );
		}
	}

	public function initialize_wishlist( $id ) {
		$this->wishlist = nmgr_get_wishlist( $id, true );
		$this->type = $this->wishlist ? $this->wishlist->get_type() : $this->type;
		$this->is_password_protected = $this->wishlist ? post_password_required( $this->wishlist->get_id() ) : false;
	}

	/**
	 * Set the current wishlist type, id and properties from the query.
	 * This is typically used when no wishlist id is supplied in the constructor.
	 */
	public function init() {
		$this->type = $this->type ?? $this->get_current_type();
		$this->initialize_wishlist( $this->get_current_wishlist_id() );
	}

	/**
	 * Get the current wishlist type based on the frontend query
	 *
	 * Returns 'gift-registry' as the default type in all instances except if the current query is for the wishlist
	 * type then it returns 'wishlist'.
	 */
	public function get_current_type() {
		global $wp_query;

		$current_page_id = !empty( $wp_query ) ? get_queried_object_id() : 0;
		return (( int ) $current_page_id === ( int ) nmgr_get_option( 'wishlist_page_id' )) ?
			'wishlist' :
			'gift-registry';
	}

	/**
	 * Get the current wishlist id based on the frontend query
	 */
	public function get_current_wishlist_id() {
		$query_var = get_query_var( 'nmgr_w' );

		if ( $query_var ) {
			$the_post = get_page_by_path( $query_var, '', [ 'nm_gift_registry' ] );
			$wishlist_id = !empty( $the_post->ID ) ? $the_post->ID : 0;
		} elseif ( !nmgr_get_type_option( $this->type, 'allow_multiple_wishlists' ) ) {
			$wishlist_id = nmgr_get_user_default_wishlist_id( '', $this->type );
		}

		return ( int ) ( $wishlist_id ?? 0);
	}
	...
	public function set_type( $type ) {
		_deprecated_function( __METHOD__, '5.7', '$this->type' );
		$this->type = $type;
	}

	public function set_page( $page ) {
		_deprecated_function( __METHOD__, '5.7', '$this->page' );
		$this->page = $page;
	}

	public function is_page( $query = '' ) {
		global $wp_query;
		...
		$has_wishlist_shortcode_or_block = ('wishlist' === $this->source);
		$has_archive_shortcode_or_block = ('archive' === $this->source);
		$nmgr_w = get_query_var( 'nmgr_w' );
		$nmgr_action = get_query_var( 'nmgr_action' );
		$allow_multi = nmgr()->is_pro && nmgr_get_type_option( $this->type, 'allow_multiple_wishlists' );
		$base = !$nmgr_w && !$nmgr_action && $has_wishlist_shortcode_or_block;
		$is_page = $base;

		switch ( $query ) {
			case 'new':
				$is_page = $query === $nmgr_w;
				break;
			case 'home':
				$is_page = ($query === $nmgr_w) || ($base && $allow_multi );
				break;
			case 'archive':
				$is_page = !$nmgr_w && !$nmgr_action && $has_archive_shortcode_or_block;
				break;
			case 'single':
			...
			case 'account_section':
				$is_page = $nmgr_w && $nmgr_action && !in_array( $nmgr_w, $base_actions );
				break;
		}

		return apply_filters_deprecated( 'is_nmgr_wishlist_page', [ $is_page, [ 'query' => $query ] ], '5.7' );
	}
	...
	public function process_notices() {
		$is_single = $this->is_page( 'single' ) || $this->is_page( 'account_section' );
		...
		if ( $is_gift_registry && $is_archived && $single_or_admin ) {
			$this->set_notice( 'wishlist_archived', $this->get_registered_notice( 'wishlist_archived' ) );
		}

		if ( $is_gift_registry && $single_or_admin && !$is_archived && $this->wishlist->needs_shipping_address() ) {
	...
	private function process_menu() {
		...
		foreach ( $account_sections as $dkey => $daction ) {
			...
			if ( $dkey === get_query_var( 'nmgr_action' ) ) {
	...
	private function get_menu_html() {
		...
		if ( !empty( $menu ) ) :
			...
			<div class="nmgr-dashboard">
				...
				<?php
				if ( nmgr_user_has_wishlist( $this->wishlist ) && $this->is_page( 'single' ) ) :
					?>
	...
	public static function get_template( $atts = [] ) {
		...
		if ( is_a( $id, \NMGR_Wishlist::class ) ) {
			_deprecated_argument( __METHOD__, '5.7', 'id should be an integer' );
			$id = $id->get_id();
		}
		...
		/**
		 * If we have the type of wishlist we want to get, let's set it here.
		 * This can be necessary to get the right template when initializing the wishlist if
		 * the wishlist id is 0 such as when showing in the block editor.
		 */
		$single->type = !empty( $atts[ 'type' ] ) ? sanitize_text_field( $atts[ 'type' ] ) : $single->type;

		/**
		 * Initialize the wishlist either with the supplied id, or by getting it automatically from the url
		 */
		$id ? $single->initialize_wishlist( $id ) : $single->init();

		/**
		 * When using the shortcode [nmgr_wishlist], in order to show a single wishlist the
		 * id key needs to be set like [nmgr_wishlist id=2]. When this is done, it means we want to
		 * show the wishlist with the id 2. In this case we set the single page as the default page
		 * if no page is given.
		 */
		if ( !empty( $atts[ 'id' ] ) ) {
			$single->page = ( $atts[ 'page' ] ?? 'single' );
		}
