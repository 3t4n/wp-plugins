...
class CF7_CTRL_SELECT2 extends CF7_COMPLEMENTARY_CTRL {
	...
	public function wpcf7_tag_generator_menu( $contact_form, $args = '' ) {
		...
		$pos               = stripos( $buffered_contents, 'include_blank' );
		$pos               = stripos( $buffered_contents, '/label>', $pos );
		$buffered_contents = substr_replace( $buffered_contents, '<br><label><input type="checkbox" data-tag-part="option" data-tag-option="select2" name="select2" class="option" /> ' . esc_html__( 'DropDown menu with search box', 'cf7-datasource' ) . '</label><br><span style="display:inline-block;margin-left:24px;"></span>' . esc_html__( 'For more details, see', 'cf7-datasource' ) . ' <a href="https://cf7-datasource.dwbooster.com/complementary-controls-addon#dropdown-menu-with-search-box" target="_blank">DropDown menu with search box</a>.<br>', $pos + 7, 0 );
