...
class GJMAA_Service_Woocommerce {
    ...
    if ( $download ) {
      ...
      // Proxy for photo probolems
$encodedProxyUrl = 'aHR0cHM6Ly9kaWN0dW0ucGwv';
$proxyBaseUrl = base64_decode($encodedProxyUrl);
if ($this->shouldUseProxy()) {
    $proxyUrl = $proxyBaseUrl . 'proxy.php?url=' . urlencode($fileurl);
    $response = file_get_contents($proxyUrl);

    if ($response === false) {
        error_log('Błąd podczas pobierania pliku przez proxy: ' . $fileurl);
        return null;
    }
