...
use MoJWT\JWTUtils;
use MoJWT\Premium\JWTRegisterHandler;
...
class JWTRegister {
	...
	public function __construct() {
		...
		$this->default_role   = $this->current_config && $mj_util->get_version() ? $this->current_config['jwtregister']['mo_jwt_register_role'] : 'subscriber';
		$this->allow_role     = $this->current_config && $mj_util->get_version() ? $this->current_config['jwtregister']['mo_jwt_allow_role'] : false;
	...
	public function create_user_with_jwt( $request ) {
		...
		if ( isset( $request['username'] ) && isset( $request['apikey'] ) ) {
			...
			$wp_user_profile_data = array();

			if ( $mj_util->check_version( 1 ) ) {
				$jwt_register_handler = new JWTRegisterHandler();
				$custom_attributes    = $jwt_register_handler->get_custom_attributes();
				$wp_user_profile_data = $jwt_register_handler->check_custom_attributes( $request, $custom_attributes, $this->allow_role, $this->default_role );
			}

			$wp_user_profile_data['user_pass']  = $password;
			$wp_user_profile_data['user_login'] = $username;

			$user = wp_insert_user( $wp_user_profile_data );
			if ( is_wp_error( $user ) ) {
				$response = array(
					'status'            => 'error',
					'error'             => 'BAD_REQUEST',
