...
class Account extends Module {
	...
	function register_form( $form_key = null ) {
		...
		if ( $_form_id = $this->_get_form_id() ) {
			if ( get_post_meta( $_form_id, 'amem_form_ajax_override', true ) ) {
				$form['data']['ajax'] = (bool) get_post_meta( $_form_id, 'amem_form_ajax', true );
			}
			if ( $this->current_tab == 'delete' || (!empty($_POST['_amem_account_tab']) && 'delete' == sanitize_key($_POST['_amem_account_tab'])) ) { // phpcs:disable WordPress.Security.NonceVerification
				$form['data']['success_message'] = get_post_meta( $_form_id, "amem_form_account_deleted_message", true );
				...
				// 	$form['data']['after_submit'] = get_post_meta($_form_id, "amem_form_after_account_delete", true);
				// 	$form['data']['redirect_url'] = get_post_meta($_form_id, "amem_form_account_delete_redirect_url", true);
				// } else {
