...
class Login extends Action {
	...
	function form_form_post( $form, $post ) {
		$login_data = [
			'rememberme'  => get_post_meta( $post->ID, 'amem_form_login_rememberme', true ),
			'passwordreset'  => get_post_meta( $post->ID, 'amem_form_login_password_reset', true ),
			'extra_button'  => get_post_meta( $post->ID, 'amem_form_login_extra_button', true ),
			'extra_text'  => get_post_meta( $post->ID, 'amem_form_login_extra_text', true ),
			'extra_url'  => get_post_meta( $post->ID, 'amem_form_login_extra_url', true ),
		];
	...
	function auto_login($form, $rememberme = 0) {
		...
		if ( !$loggedin || is_wp_error($loggedin) ) {
