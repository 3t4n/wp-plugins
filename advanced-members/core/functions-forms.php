...
function amem_form_from_post( $form_post ) {
	...
	if ( get_post_meta( $form_post->ID, 'amem_form_ajax_override', true ) ) {
		$form_ajax = get_post_meta( $form_post->ID, 'amem_form_ajax', true );
	}
	...
	$form = amem_get_valid_form( array(
		...
		'data' => array(
			'ajax' => $form_ajax,
			'description' => '',//get_post_meta( $form_post->ID, 'amem_form_description', true ),
			'submit_text' => get_post_meta( $form_post->ID, 'amem_form_submit_text', true ),
			'success_message' => get_post_meta( $form_post->ID, "amem_form_{$form_type}_show_message", true ),
			'after_submit' => '',
			'redirect_url' => '',
		),
	) );
...
function amem_form_success_message( $form, $args, $echo=false ) {
	...
	$output = sprintf( '<div class="acf-notice -success amem-notice amem-success-message" aria-live="assertive" role="alert">%s</div>', amem_format_message($success_message, true) );// message text escaped by amem_format_message()
...
function amem_form_updated_message( $form, $args, $echo=false ) {
	...
	$output = sprintf( '<div class="acf-notice -success amem-notice amem-updated-message" aria-live="assertive" role="alert">%s</div>', amem_format_message($message, true) );// message text escaped by amem_format_message()
...
function amem_form_error_message( $form, $args, $echo=false ) {
	...
	$message = amem()->errors->text($error_code);
	$message = apply_filters( 'amem/form/error_message', $message, $error_code, $form, $args );
	$message = apply_filters( 'amem/form/error_message/type=' . $form['type'], $message, $error_code, $form, $args );

	$message = amem_resolve_merge_tags( $message );

	$output = sprintf( '<div class="acf-notice -error amem-notice amem-error-message" aria-live="assertive" role="error">%s</div>', amem_format_message($message, true) );// message text escaped by amem_format_message()
...
function amem_enqueue() {
	...
	if ( apply_filters( 'amem/settings/enqueue_validation_hotfix', false ) ) {
		wp_enqueue_script( 'amem-multi-form-validation-hotfix', amem_get_url("assets/build/js/multi-form-validation-hotfix{$min}.js"), [ 'acf-input' ], AMEM_VERSION, true );
	}

	// Enqueue ACF scripts and styles
	acf_enqueue_scripts();

	wp_register_script( 'amem-password-strength', amem_get_url("assets/build/js/password-strength{$min}.js"), array( 'password-strength-meter' ), AMEM_VERSION, true );
	...
	wp_enqueue_script( 'amem-forms-script', amem_get_url("assets/build/js/forms{$min}.js"), ['jquery', 'acf-input', 'amem-input'], AMEM_VERSION, true );

	wp_enqueue_style( 'amem-form-style', amem_get_url("assets/build/css/form{$min}.css"), false, AMEM_VERSION );

	if ( amem()->options->get('load_theme', true) ) {
		wp_enqueue_style( 'amem-theme-default', amem_get_url("assets/build/css/themes/default{$min}.css"), false, AMEM_VERSION );
	}
