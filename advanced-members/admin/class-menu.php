...
class Menu {
	...
	public static function fields( $item_id, $item, $depth, $args ) {
		...
		<div class="amem-nav-edit">
			<div class="clear"></div>
			<h4 style="margin-bottom: 0.6em;"><?php esc_html_e( 'Advanced Members for ACF Settings', 'advanced-members' ) ?></h4>
		<?php
		if ( $menu_type ) : ?>
		...
		<?php elseif ( $none_amem ) : ?>
			...
			<p class="amem-nav-mode description description-wide">
				...
				<label for="amem-filter_users-<?php echo esc_attr( $item->ID ); ?>">
					...
					<select name="amem_nav_item[<?php echo esc_attr( $item->ID ); ?>][filter_users]" id="amem_nav_item-filter_users-<?php echo esc_attr( $item->ID ); ?>" class="widefat">
			...
			<p class="amem-nav-roles description description-wide" class="<?php echo esc_attr($roleCSS); ?>">
