...
class Account extends Action {
	...
	function error_message($message) {
		...
		if ( '' == $message && !is_user_logged_in() ) {
			...
			$message = sprintf( __( 'Please <a href="%s" title="Login">login</a> to manage account', 'advanced-members' ), amem_get_core_page('login') );
		}
	...
	function process($form, $fields, $args) {
		...
		$error = __('Failed to process account update with given data.', 'advanced-members' );
	...
	function register_local_fields($form, $args) {
		if ( $this->current_tab ) {
			...
			$this->local_fields_delete();
		}
	}

	function local_fields_general() {
		$fields = array(
			'user_login' => array(
				'key' => 'user_login',
	...
	function local_fields_password() {
		$fields = [];
	...
	function local_fields_delete($args='') {
		...
		$fields['user_delete_confirm'] = [
		 ...
		 '_amem_local' => true,
		 'ui' => 1,
			// 'wrapper' => ['class' => 'amem-notice acf-notice -warning'],
		];
