...
class amem_field_user_bio extends amem_field_textarea {
	...
	function initialize() {
		...
		$this->label      = __( 'User Bio (Members)', 'advanced-members' );
		$this->category = 'Members';
		$this->description = __( 'Biographical info of user. Synced to `description` user meta.', 'advanced-members' );
		$this->preview_image = amem_get_url('/assets/images/field-type-previews/field-preview-textarea.png');
		$this->defaults   = array(
			'default_value' => '',
			'new_lines'     => '',
			'maxlength'     => '',
