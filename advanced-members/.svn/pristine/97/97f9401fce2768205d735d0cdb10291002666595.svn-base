...
class amem_field_user_tos extends amem_field_true_false {
	...
	function initialize() {
		...
		$this->label      = __( 'Consent (Members)', 'advanced-members' );
		$this->category = 'Members';
		$this->description = __( 'Provides areement field like terms of service or pivacy policy.', 'advanced-members' );
		$this->preview_image = amem_get_url('/assets/images/field-type-previews/field-preview-true-false.png');
		$this->defaults      = array(
			'default_value' => 0,
			/* translators: site name string */
			'message'       => sprintf( __( 'By creating an account, you agree to %s\'s {TOS}.', 'advanced-members' ), get_bloginfo('name') ),
			'ui'            => 0,
			'ui_on_text'    => '',
			'ui_off_text'   => '',
			'tos_page_id' => 0,
	...
	function prepare_field( $field ) {
		...
		if ( $page_id = (int) $field['tos_page_id_2'] ) {
		...
		$field['message'] = str_replace( ['{TOS}', '{TOS2}'], [$link, $link_alt], $field['message'] );
	...
	function render_field_settings( $field ) {
		...
		acf_render_field_setting(
			...
			array(
				...
				'instructions' => __( 'Displays text alongside the checkbox. You can use TOS page link with {TOS} and {TOS2}', 'advanced-members' ),
				'type'         => 'text',
				'name'         => 'message',
			)
		);
		...
		acf_render_field_setting(
			$field,
			array(
				'label'         => __( 'Terms of Service Page', 'advanced-members' ),
				'name'          => 'tos_page_id',
				'type'          => 'select',
		...
		acf_render_field_setting(
			$field,
			array(
				'label'        => __( 'TOS page link text', 'advanced-members' ),
