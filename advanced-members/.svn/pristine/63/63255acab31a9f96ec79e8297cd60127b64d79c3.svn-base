...
class amem_field_display_name extends amem_field_select {
	...
	function initialize() {
		...
		$this->label    = __( 'Display Name (Members)', 'advanced-members' );
		$this->category = 'Members';
		$this->description = __( 'Select name to display name publicly as. Synced to `display_name` user data.', 'advanced-members' );
		$this->preview_image = amem_get_url('/assets/images/field-type-previews/field-preview-text.png');
		$this->defaults = array(
			'multiple'      => 0,
			'allow_null'    => 0,
			'choices'       => array(),
