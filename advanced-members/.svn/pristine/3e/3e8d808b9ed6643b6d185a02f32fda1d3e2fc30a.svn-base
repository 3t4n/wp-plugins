...
class Registration extends Action {
	...
	function from_post( $form, $post ) {
		$role = get_post_meta( $post->ID, 'amem_form_regist_role', true );
		if ( !$role )
			$role = get_option( 'default_role' );

		$form['data']['role'] = $role;

		$status = get_post_meta( $post->ID, 'amem_form_regist_status', true );
		if ( !$status )
	...
	function process( $form, $fields, $args ) {
		...
		$error = __('Failed to process registration with given data.', 'advanced-members' );
		...
		switch( $status ) {
			...
			add_filter( 'amem/form/submit/redirect/registration', function($url) {
				if ( '' == $url )
					return $url;
