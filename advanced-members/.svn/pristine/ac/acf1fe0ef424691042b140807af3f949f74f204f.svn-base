...
class amem_field_username extends amem_field_text {
	...
	function initialize() {
		// vars
		$this->name       = 'username';
		$this->label      = __( 'Username or Email (Members)', 'advanced-members' );
		$this->category = 'Members';
		$this->public = true;
		$this->description = __( 'User login name, You can use this field as "Username or Email" on Login/Registration form.', 'advanced-members' );
		$this->defaults   = array(
			'default_value' => '',
			'maxlength'     => '',
			'placeholder'   => '',
	...
	public function validate_value( $is_valid, $value, $field, $input ) {
		...
		if ( !in_array($this->mode(), ['login', 'passwordreset']) ) {
			...
			if ( !username_exists( $value ) && !email_exists( $value ) ) {
				/* translators: Username or email string */
				$username_not_exists = sprintf( __( 'The username or email %s is not existing in this site. Please try a different username', 'advanced-members' ), $value );
	...
	function render_field_settings( $field ) {
		...
		$msg1 = __( 'Username field works as "Username or Email" field on Login page.', 'advanced-members' );
		$msg2 = __( 'Users cannot change their username and this field displayed as disabled except registration form.', 'advanced-members' );
