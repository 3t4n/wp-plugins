...
class Redirects extends Module {
	...
	function logout_redirects($url) {
		...
		if ( amem()->options->get('roles/apply_roles_redirection') ) {
	...
	function form_redirects( $form ) {
		...
		// if ( get_post_meta( $form_post->ID, 'amem_form_redirect_override', true ) ) {
		// 	$after_submit = get_post_meta($form_post->ID, "amem_form_after_$form_type", true);
		// 	$redirect_url = get_post_meta($form_post->ID, "amem_form_{$form_type}_redirect_url", true);
		// } else {
