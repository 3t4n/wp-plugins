...
class ADMIN_OPTIONS extends Module {
	...
	function admin_menu() {
		...
		$page = add_submenu_page( $parent_slug, __( 'Settings', 'advanced-members' ), __( 'Settings', 'advanced-members' ), 'manage_options', 'amem_settings', array( $this, 'render' ) );
		add_action( 'load-' . $page, array( $this, 'admin_load' ) );
		add_submenu_page( $parent_slug, __( 'Forms', 'advanced-members' ), __( 'Forms', 'advanced-members' ), 'manage_options', 'edit.php?post_type=amem-form' );
	...
	public function update_dashboard() {
		...
		if( isset($_POST['amem_modules'])){ // phpcs:disable WordPress.Security.NonceVerification -- already verified by ACF
			update_option( 'amem_modules', amem_sanitize_vars($_POST['amem_modules']) ); // phpcs:disable WordPress.Security.NonceVerification -- already verified by ACF
			amem()->options->modules = amem_sanitize_vars($_POST['amem_modules']); // phpcs:disable WordPress.Security.NonceVerification -- already verified by ACF
	...
	public function update_option() {
		...
		if( isset($_POST['amem_options'])){
			$rules = array('accform', 'regform');
			foreach ($rules as $rule) {
				if( isset($_POST['amem_options'][$rule]['rules']) ){
					$i = 0;
					foreach ($_POST['amem_options'][$rule]['rules'] as $key => $value) {
						$_POST['amem_options'][$rule]['rules'][$i] = amem_sanitize_vars($value);
						unset($_POST['amem_options'][$rule]['rules'][$key]);
						$i = $i + 1;
					}
				}
			}
			if( isset($_POST['amem_options']['email'])){
				foreach ($_POST['amem_options']['email'] as $key => $mail_option) {
					if( isset($_POST['amem_options']['email'][$key]['body'])){
						$mail_option = amem_sanitize_vars( $mail_option );
						$_POST['amem_options']['email'][$key]['body'] = wpautop(stripslashes($mail_option['body']));
					}
				}
			}
			update_option('amem_options', amem_sanitize_vars($_POST['amem_options']));
	...
	public function admin_body_class( $classes ) {
		$classes .= ' amem-admin-page amem-option-page';
		return $classes;
	}
	...
	**/
	function postbox_dashboard_document( $post, $args ){
		...
		<div class="document_text">
			...
			you started.</p>
			<a target="_blank" href="https://advanced-members.com/docs/getting-started/">Browse Documentation</a>
		</div>
		<?php
	}
...
*/
		...
		$default_settings_tabs = apply_filters( 'amem/member_dashboard/dashboard_tabs', array(
			...
			'settings'	=> array( 'label' => __( 'Settings', 'advanced-members' ), 'link' => admin_url( 'edit.php?post_type=acf-field-group&page=amem_settings' ) ),
			'forms' 		=> array( 'label' => __( 'Forms', 'advanced-members' ), 'link' => admin_url( 'edit.php?post_type=amem-form' ) ),
		));

		// _acf_apply_get_local_field_groups()
		$settings_data = array(
			'page_title' 	=> __( 'Advanced Members for ACF Dashboard' , 'advanced-members' ),
	...
	function enqueue_admin_scripts() {
		...
		wp_enqueue_style( 'amem-admin', amem_get_url("assets/build/css/admin{$min}.css"), [], AMEM_VERSION );
		if ( !amem_is_screen( ['amem-form', 'edit-amem-form', 'acf-field-group', 'acf_page_amem_settings', 'acf_page_amem_dashboard']) )
			return;

		amem()->fields->enqueue_scripts();

		wp_register_script( 'amem-admin', amem_get_url("assets/build/js/amem-admin{$min}.js"), ['jquery', 'acf-input', 'amem-input'], AMEM_VERSION, true );
	...
	function register_core_page_option_button() {
		foreach( array_keys(amem()->config->core_pages) as $page_key ) {
