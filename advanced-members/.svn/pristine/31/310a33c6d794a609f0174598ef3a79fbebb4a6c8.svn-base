...
function amem_get_url( $filename = '', $base = null ) {
	return ( !$base ? AMEM_URL : $base ) . ltrim( $filename, '/' );
}
...
function amem_get_core_page( $slug, $updated = false, $redirect_to = '' ) {
	...
	if ( isset( amem()->options->permalinks[ $slug ] ) ) {
		$url = get_permalink( amem()->options->permalinks[ $slug ] );
		if ( $updated ) {
 ...
 * @return bool
 */
function amem_is_core_page( $page ) {
	$ID = get_the_ID();
	if ( empty( $ID ) ) {
		return false;
	}

	if ( isset( $ID ) && isset( amem()->options->permalinks[ $page ] ) && $ID == amem()->options->permalinks[ $page ] ) {
		return true;
	}
	return false;
}
 ...
 * @return array
 */
function amem_sanitize_vars($vars) {
	if ( !is_array($vars) ) {
		if ( is_string($vars) )
	...
	foreach( $vars as $k => $v ) {
		$k = sanitize_key($k);
		if ( is_array($v) ) {
			$new_vars[$k] = amem_sanitize_vars($v);
		} else if ( is_string($v) ) {
			$new_vars[$k] = sanitize_text_field($v);
		} else {
			$new_vars[$k] = $v;
		}
	}
 ...
 */
function amem_format_message( $message, $shortcode = false ) {
	if ( $shortcode )
		$message = do_shortcode( $message );
	return wp_kses_post( wpautop( wptexturize( $message ) ) );
}
