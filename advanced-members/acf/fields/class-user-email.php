...
class amem_field_user_email extends amem_field_email {
	...
	function initialize() {
		...
		$this->label      = __( 'User Email (Members)', 'advanced-members' );
		$this->category = 'Members';
		$this->description = __( 'Email of user. Synced to `user_email` user data.', 'advanced-members' );
		$this->preview_image = amem_get_url('/assets/images/field-type-previews/field-preview-email.png');
		$this->defaults   = array(
			'default_value' => '',
			'placeholder'   => '',
			'prepend'       => '',
	...
	public function validate_value( $is_valid, $value, $field, $input ) {
		...
		if ( $this->mode() == 'login' ) {
			if ( !email_exists( $value ) ) {
				return sprintf( __( 'The email is not existing in this site. Please try a different email', 'advanced-members' ) );
			}
