(function ($) {
	...
	function adqs_slick_init() {
		...
		$('.qsd-slick-wrapper').each(function () {
			...
			if ($(this).closest('.qsd-tax-ajax').data('carousel-settings')) {
				settings = $(this)
					.closest('.qsd-tax-ajax')
					.data('carousel-settings');
			}
		...
		$(document).on('click',function(e){
			$('.qsd-slick-wrapper').each(function(){
				if($(this).closest('[class*="tabs"]').length > 0){
	...
	$(window).on('elementor/frontend/init', function () {
		elementorFrontend.hooks.addAction(
			'frontend/element_ready/adqs_alllistings.default',
			adqs_slick_init
		);
		elementorFrontend.hooks.addAction(
			'frontend/element_ready/adqs_taxonomy.default',
			adqs_slick_init
		);
	});
})(jQuery);
