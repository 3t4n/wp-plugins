	...
	<div class="qsd-form-wrap qsd-form-field">
		...
		<?php if ($price_type == 'both') : ?>
			<div class="qsd-field-inline qsd-choose-fields qsd-radio-choose">
				<div class="qsd-form-check-control">
					<input type="radio" id="_price_type_price" name="_price_type" checked
						value="<?php echo esc_attr($name_price); ?>" <?php checked($value_price_type, $name_price); ?>>
					<label for="_price_type_price">
						<?php echo esc_html__('Price', 'adirectory'); ?>
					</label>
				</div>
				<div class="qsd-form-check-control">
					<input type="radio" id="_price_type_range" name="_price_type"
		...
		<?php if (($price_type == 'both') || ($price_type == 'unit')) : ?>
			<div class="qsd-priceFields_unit <?php echo ($price_type == 'both') ? esc_attr('display-if') : ''; ?>" <?php
																													if ($price_type == 'both') :
																													?> data-target_name="_price_type" data-target_type="radio"
				data-target_value="<?php echo esc_attr($name_price); ?>" <?php endif; ?>>
				<div class="qsd-price_units">
					<input type="text" id="_price" class="qsd-form-control" name="<?php echo esc_attr($name_price); ?>"
						value="<?php echo esc_attr($value_price); ?>"
						placeholder="<?php echo esc_html__('Price amount', 'adirectory'); ?>">
					<input type="text" id="_price_sub" class="qsd-form-control"
		...
		<?php if (($price_type == 'both') || ($price_type == 'range')) : ?>
			...
			?>
			<select id="_price_range"
				class="qsd-form-control <?php echo ($price_type == 'both') ? esc_attr('display-if') : ''; ?>"
				name="<?php echo esc_attr($name_price_range); ?>" <?php
																	if ($price_type == 'both') :
																	?> data-target_name="_price_type" data-target_type="radio"
				data-target_value="<?php echo esc_attr($name_price_range); ?>">
			<?php endif; ?>
			<option value="">
