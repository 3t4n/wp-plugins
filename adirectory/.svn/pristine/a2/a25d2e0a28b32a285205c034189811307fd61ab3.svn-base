...
class Categories extends \WP_Widget
{
	...
	public function widget($w_args, $instance)
	{
		...
		if ($widget_id === 'adqs_categories') {
			...
			if (!empty($display_number) && ($see_more !== '1')) {
				$args['number'] = $display_number;
			}
			if ($see_more === '1'):
?>
				<style>
					#qsd-widget-cat-tax-<?php echo esc_attr($tax_w_id) . ' '; ?>>li:nth-child(n+<?php echo esc_attr($display_number + 1); ?>),
					.qsd-tax-see-more:not(.active) {
						display: none;
					}
				</style>
		...
		<?php
			...
			echo '<div class="qsd-categories  qsd-widget-tax-wrap">';

			if (isset($instance['title']) && !empty($instance['title'])) {
				echo wp_kses_post($w_args['before_title'] . apply_filters('widget_title', $instance['title']) . $w_args['after_title']);
			}
			echo '<div class="qsd-widget-tax-lists">';
			echo '<ul class="qsd-categories-list qsd-widget-tax" id="qsd-widget-cat-tax-' . esc_attr($tax_w_id) . '">';
			...
			echo '</div>';
			if ($see_more === '1') {
				echo '<a href="#" class="qsd-tax-see-more" data-less-text="' . esc_html__('Show Less -', 'adirectory') . '">' . esc_html__('Show More +', 'adirectory') . '</a>';
			}
	...
	public function form($instance)
	{
		...
		$see_more   = $h->check_data($instance, 'see_more') ? $instance['see_more'] : '1';
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('related_directory')); ?>" style="display: block;margin-bottom: 2px;"><?php echo esc_html__('Related - directory listing type', 'adirectory'); ?>:</label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('see_more')); ?>" style="display: block;margin-bottom: 2px;"><?php echo esc_html__('See More', 'adirectory'); ?>:</label>
			<select id="<?php echo esc_attr($this->get_field_id('see_more')); ?>" name="<?php echo esc_attr($this->get_field_name('see_more')); ?>">
				<option value="1" <?php selected($see_more, '1'); ?>>
					<?php echo esc_html__('Yes', 'adirectory'); ?>
				</option>
				<option value="0" <?php selected($see_more, '0'); ?>>
					<?php echo esc_html__('No', 'adirectory'); ?>
				</option>
			</select>
		</p>
<?php
	...
	public function update($new_instance, $old_instance)
	{
		...
		$instance['see_more']      = $h->check_data($new_instance, 'see_more') ? sanitize_text_field($new_instance['see_more']) : '';
