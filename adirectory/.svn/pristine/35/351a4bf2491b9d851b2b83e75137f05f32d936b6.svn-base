...
class EmailSender
{
    ...
    public function adqs_mail($to, $subject, $message, $headers = '', $attachments = array())
    {
    ...
    public function send_mail($mail_trigger, $type, $data)
    {
        ...
        if ($type === "user") {
            switch ($mail_trigger) {
                case "new_user_reg":
                    ...
                    $search = array("{user_name}", "{reset_password}");
                    $replace = array($user_name ?? '', $reset_url ?? '');
                ...
                case "order_created":
                    ...
                    $search = array("{order_id}", "{customer_name}", "{order_url}");
                    $replace = array($order_id ?? '', $customer_name ?? '', $order_url ?? '');

                    $receipent[] = $user_email ?? '';

                    update_user_meta($customer_id, "adqs_sended_order_email_{$order_id}", 'yes');
                ...
                case "new_user_account":
                    ...
                    $receipent[] = $guest_email ?? '';
                    break;
                case "listing_expiry_notification":
                    if (!$this->get_email_trigger('user', 'listing_about_expire')) {
                        return;
                    }
                    $search = array("{user_name}", "{order_id}", "{expiry_data}");
                    $replace = array($user_name ?? '', $order_id ?? '', $expiry_date ?? '');
        ...
        if (!function_exists('wp_mail')) {
            ...
            $this->adqs_mail($receipent, $subject, $content, $headers);
        } else {
            wp_mail($receipent, $subject, $content, $headers);
        }
    }
}