...
class Directory extends Custom_Posts
{
	...
	public function register_custom_post_type()
	{
		...
		$settings   = array(
			'menu_icon'          => ADQS_DIRECTORY_ASSETS_URL . '/admin/img/menu-icon.svg',
	...
	public function custom_table_columns($columns)
	{
		$columns = array(
			...
			'expire_on'                => esc_html__('Expire On', 'adirectory'),
	...
	public function custom_table_custom_column($column, $post_id)
	{
		switch ($column) {
			...
			case 'expire_on':
				...
				if (empty($expiryNever)) {

					$value_converted = adqs_convert_expiry_date($post_id);
					if (!empty($value_converted)) {
						echo wp_date('Y/m/d \a\t h:i a', strtotime($value_converted));
					}
				} else {
					echo esc_html__('Never Expire', 'adirectory');
				}
