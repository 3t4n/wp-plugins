	...
	function adqs_metabox_sliders_images(selectror) {
		...
		if (add_sliders.exists()) {
			...
			selectror.on("click", ".qsd-remove-sliders", function () {
				let that = $(this),
					sliderImages = that.closest(".qsd-slider-images"),
					sliderInputName = sliderImages.data("name");

				sliderImages.find(".qsd-slider-image-preview").remove();
				sliderImages.append('<input type="hidden" name="' + sliderInputName + '[]">');
			});
			selectror
