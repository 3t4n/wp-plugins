...
class CookiesHandler
{
	...
	private $pending_cookies = []; // Store cookies to set later
	...
	public function __construct()
	{
		...
		// Hook into WordPress to set cookies
		add_action('init', [$this, 'applyCookies']);
	}
	...
	public function set($key, $value, $cookie_lifetime = null)
	{
		...
		// Store cookies to apply during `init`
	...
	{
		foreach ($this->pending_cookies as $cookie) {
	...
	public function remove($key)
	{
		...
		if (isset($data[$key])) {
			...
			$this->set($key, null, -3600); // Mark cookie for immediate expiry
		}
	...
	{
		$this->set($this->cookie_name, '', -3600); // Set expiry in the past
		return $this;
	}
}
