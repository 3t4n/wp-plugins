...
if (!function_exists('adqs_post_pagination')) {
	...
	function adqs_post_pagination($query = null, $base_url = null, $current_num = null)
	{
		global $wp_query;
		$big            = 999999999;
...
if (!function_exists('adqs_is_listing_archive')) {
	function adqs_is_listing_archive()
	{
		...
		if (is_tax() && isset($queried_object->taxonomy)) {
			...
			$allowed_taxonomies = ['adqs_category', 'adqs_location', 'adqs_tags'];
		...
		// Check if it's an author listing archive
		if (function_exists('adqs_is_author_listing_archive') && adqs_is_author_listing_archive()) {
			return true;
		}
