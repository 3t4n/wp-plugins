...
?>
				<div class="adqs-singleSection <?php echo esc_attr(AD()->Helper->text_to_slug($sectiontitle)); ?>" id="adqs-singleSection_<?php echo esc_attr($section_index); ?>">
...
if (!function_exists('adqs_mail_to_user')) {
	function adqs_mail_to_user($user_id, $reset_link = '')
	{
		...
		if ($user_id > 0) {
			...
			$user = new EmailSender();
			$user->send_mail('new_user_reg', 'user', array('user_email' => $user_email, 'user_name' => $user_name, 'reset_url' => $reset_link));
		}
	}
}
...
if (!function_exists('adqs_new_user_account')) {
	function adqs_new_user_account($reset_link, $message, $guest_email)
	{
		$user = new EmailSender();
		$user->send_mail('new_user_account', 'user', array('reset_link' => $reset_link, 'message' => $message, 'guest_email' => $guest_email));
	}
}
...
if (!function_exists('adqs_send_mail_user_expiry_package')) {
	function adqs_send_mail_user_expiry_package($getdata, $customerid, $order_id)
	{
		$user = get_userdata($customerid);
		$user_email = $user->user_email;
		$user_name = $user->user_login;
		$email_sender = new EmailSender();
		$email_sender->send_mail('listing_expiry_notification', 'user', array('user_email' => $user_email, 'user_name' => $user_name, 'expiry_date' => $getdata, 'order_id' => $order_id));
	}
}
...
if (!function_exists('adqs_order_created_mail_to_both')) {
	function adqs_order_created_mail_to_both($order_id, $customer_id, $order_url = "")
	{
		...
		$email_sender->send_mail('order_created', 'admin', $email_data);
		$email_sender->send_mail('order_created', 'user', $email_data);
	}
}

if (!function_exists('adqs_order_completed_mail_to_both')) {
	/**
	 * Sends order completion emails to both admin and user.
	 *
	 * @param int    $order_id     The ID of the completed order.
	 * @param int    $customer_id  The ID of the customer who completed the order.
	 * @param string $order_url    Optional. The URL of the order (default empty).
	 */
	function adqs_order_completed_mail_to_both($order_id, $customer_id, $order_url = "")
	{
		// Retrieve customer data using get_userdata
		$customer_data = get_userdata($customer_id);
		...
		// Send email to admin
		$email_sender->send_mail('order_completed', 'admin', $email_data);

		// Send email to user
		$email_sender->send_mail('order_completed', 'user', $email_data);
	}
}
