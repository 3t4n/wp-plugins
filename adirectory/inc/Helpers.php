...
class Helpers
{
	...
	public function get_author_ratings($author_id = 0)
	{
		...
		$cache_key = "adqs_authlrRatings_{$author_id}";
		$authlrRatings = wp_cache_get($cache_key, 'adqs_cache');

		if ($authlrRatings === false) {
			$query = $wpdb->prepare(
				"SELECT AVG(meta_value)
			FROM $wpdb->postmeta
			WHERE meta_key = 'adqs_avg_ratings'
			AND post_id IN (
				SELECT ID FROM $wpdb->posts
				WHERE post_type = 'adqs_directory'
				AND post_status = 'publish'
				AND post_author = %d
			)",
				$author_id
			);
			$authlrRatings = $wpdb->get_var($query);

			wp_cache_set($cache_key, $authlrRatings, 'adqs_cache');
		}

		$authlrRatings = $authlrRatings ? round($authlrRatings, 1) : '';

		$current_rating = get_user_meta($author_id, 'adqs_agent_rating', true);

		if ($authlrRatings !== $current_rating) {
			update_user_meta($author_id, 'adqs_agent_rating', $authlrRatings);
		}
	...
	public function get_author_review_count($author_id = 0)
	{
		...
		$authlrReviewCount = wp_cache_get("adqs_authlrReviewCount_{$author_id}", 'adqs_cache');
		if ($authlrReviewCount === false) {
			...
			wp_cache_set("adqs_authlrReviewCount_{$author_id}", $authlrReviewCount, 'adqs_cache');
		}
		$authlrReviewCount = !empty($authlrReviewCount) ? $authlrReviewCount : false;
		$current_review = get_user_meta($author_id, 'adqs_agent_review', true);

		if ($authlrReviewCount !== $current_review) {
			update_user_meta($author_id, 'adqs_agent_review', $authlrReviewCount);
		}
	...
	public function get_price($post_id)
	{
		$priceRangeTypes = apply_filters(
			...
			array(
				'skimming'       => esc_html__('High ($$$$)', 'adirectory'),
		...
		if ($value_price_type === '_price_range') {
			...
			return join(' ', array_filter([$price, $value_price_sub]));
		}
	}
