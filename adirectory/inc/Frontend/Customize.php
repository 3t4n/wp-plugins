...
class Customize {
	...
	public function __construct() {
		...
		add_action('adqs_tax_lists', array($this, 'tax_lists_html'), 10, 2);
		add_action('adqs_ad_search_tax', array($this, 'ad_search_tax'));
	...
	public function save_views_count($postID = null) {
		// Initialize $postID if not provided
		...
		// Retrieve view_ids from cookie, or initialize an empty array
		$view_ids = AD()->Cookie->has('view_counted') ? AD()->Cookie->get('view_counted') : [];


		// If the post ID is already in the cookie, do nothing
		if (in_array($postID, ($view_ids ?? []))) {
			return;
		}
		...
		// If the count is not set, initialize it
		if ($count == '') {
			$count = 0;
			delete_post_meta($postID, $count_key);
			add_post_meta($postID, $count_key, 0);
		} else {
			// Increment the count and update the post meta
			++$count;
			update_post_meta($postID, $count_key, $count);

			// Add the post ID to the view_ids and save it to the cookie
			$view_ids[] = $postID;
			AD()->Cookie->set('view_counted', $view_ids);
		}
	}
	...
	public function tax_lists_html($taxonomy = 'adqs_category', $saved_data = []) {

?>
		<div class="adqs-taxonomyDropdown-items">
			<ul>
				<?php
				...
				adqs_render_tax_lists(['taxonomy' => $taxonomy], $defaultOptions);
				?>
				<div class="adqs-dropdown-reset"><?php echo esc_html__('Reset', 'adirectory'); ?></div>
			</ul>
		</div>
		<div class="adqs-dropdown-close"><span class="dashicons dashicons-no-alt"></span></div>
	<?php
	}
	public function ad_search_tax() {

?>
		<input type="search" placeholder="<?php echo esc_attr('Type for search...'); ?>">
	<?php
	}
