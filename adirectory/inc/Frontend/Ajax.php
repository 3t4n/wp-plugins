...
use ADQS_Directory\Frontend\Traits\Ajax\Filters;
...
class Ajax
{
	...
	public function __construct()
	{
		...
		// ajax filters
		add_action('wp_ajax_adqs_tax_filters', array($this, 'ajax_tax_filters_listings'));
		add_action('wp_ajax_nopriv_adqs_tax_filters', array($this, 'ajax_tax_filters_listings'));

		// ajax filters
		add_action('wp_ajax_adqs_filters_listings', array($this, 'ajax_filters_listings'));
		add_action('wp_ajax_nopriv_adqs_filters_listings', array($this, 'ajax_filters_listings'));
	}
	...
	public function adqs_delete_listing()
	{
		...
		if (!current_user_can('read')) {
			wp_send_json_error(array('message' => 'Do not have any permission'));
		}
		...
		if ($post->post_type !== 'adqs_directory') {
			wp_send_json_error(array('message' => 'You can only delete listings of type adqs_directory'));
		}

		if ((int) $post->post_author !== get_current_user_id()) {
			wp_send_json_error(array('message' => 'You do not have permission to delete this listing'));
		}
		...
		if ($delete) {
			...
			wp_send_json_error(array('message' => 'Failed to delete listing'));
		}
	}
	...
	public function qsd_user_dash_get_listings()
	{
		...
		if ($listings->have_posts()) {
			while ($listings->have_posts()) {
				...
				$value = AD()->Helper->meta_val(get_the_ID(), '_expiry_date', '');
				if (!empty($value)) {
					$value = adqs_convert_expiry_date(get_the_ID());
					$default_expiry = wp_date('Y/m/d \a\t h:i a', strtotime($value));
				}
	...
	public function submit_ajax_review_comment()
	{
		...
		if (!empty($email) && ($hasComment > 0)) {
			...
			wp_cache_delete("adqs_authlrRatings_{$author_id}", 'adqs_cache');
			wp_cache_delete("adqs_authlrReviewCount_{$author_id}", 'adqs_cache');
...
<?php
	...
	/**
	 * Method ajax_filters_listings
	 *
	 * @return void
	 */
	public function ajax_tax_filters_listings()
	{
		check_ajax_referer('adqs_ajax_filter', 'security');

		$view_type = '';

		$form_data = isset($_REQUEST['form_data']) ? wp_unslash($_REQUEST['form_data']) : '';
		$base_url = isset($_REQUEST['base_url']) ? sanitize_url($_REQUEST['base_url']) : '';
		$paged = isset($_REQUEST['paged']) ? absint(sanitize_text_field($_REQUEST['paged'])) : 1;

		$per_page = isset($_REQUEST['per_page']) ? (int) sanitize_text_field($_REQUEST['per_page']) : 8;
		$pagination_type = isset($_REQUEST['pagination_type']) ? sanitize_text_field($_REQUEST['pagination_type']) : 'pagination';
		$tax_name = isset($_REQUEST['tax_name']) ? sanitize_text_field($_REQUEST['tax_name']) : 'adqs_category';
		...
		$terms = isset($_REQUEST['terms']) ? sanitize_text_field($_REQUEST['terms']) : '';
		$ajax_filter = isset($_REQUEST['ajax_filter']) ? sanitize_text_field($_REQUEST['ajax_filter']) : '';
		$uniqId = isset($_REQUEST['uniq_id']) ? sanitize_text_field($_REQUEST['uniq_id']) : '';
		parse_str($form_data, $datas);


		$active_type = $datas['directory_type'] ?? '';
	 ...
	 * @return void
	 */
	public function ajax_filters_listings()
	{
		check_ajax_referer('adqs_ajax_filter', 'security');
		...
		$form_data = isset($_REQUEST['form_data']) ? wp_unslash($_REQUEST['form_data']) : '';
		$base_url = isset($_REQUEST['base_url']) ? sanitize_url($_REQUEST['base_url']) : '';
		$filter_layout = isset($_REQUEST['filter_layout']) ? sanitize_text_field($_REQUEST['filter_layout']) : 'top';
		$paged = isset($_REQUEST['paged']) ? absint(sanitize_text_field($_REQUEST['paged'])) : 1;

		$setting_per_page = !empty(AD()->Helper->get_setting('listing_per_page')) ? absint(AD()->Helper->get_setting('listing_per_page')) : 6;
		$per_page = isset($_REQUEST['per_page']) ? (int) sanitize_text_field($_REQUEST['per_page']) : $setting_per_page;
		$pagination_type = isset($_REQUEST['pagination_type']) ? sanitize_text_field($_REQUEST['pagination_type']) : 'pagination';
		$directory_type = isset($_REQUEST['directory_type']) ? sanitize_text_field($_REQUEST['directory_type']) : '';
		$view_type = isset($_REQUEST['view_type']) ? sanitize_text_field($_REQUEST['view_type']) : 'grid';
		$short_by = isset($_REQUEST['short_by']) ? sanitize_text_field($_REQUEST['short_by']) : '';
		...
		$display_listings = isset($_REQUEST['display_listings']) ? sanitize_text_field($_REQUEST['display_listings']) : '';
		$uniqId = isset($_REQUEST['uniq_id']) ? sanitize_text_field($_REQUEST['uniq_id']) : '';
		...
		$notFiltersKeys = ['view_type', 'short_by'];
		...
		if (!empty($datas) && is_array($datas)) {
			foreach ($datas as $name => $value) {
				if (!in_array($name, $notFiltersKeys)) {
		...
		$query_args = adqs_listing_query_filter_args($queryArgs);
		...
		if (isset($datas['short_by'])) {
			$short_by = $datas['short_by'];
		}
		if (!empty(adqs_listing_query_sort_by($short_by))) {
			$query_args = array_merge($query_args, adqs_listing_query_sort_by($short_by));
		}

		$listings_query = new \WP_Query($query_args);

		if (isset($datas['view_type'])) {
		...
		adqs_get_template_part('grid/gridlist', 'view', compact('listings_query', 'view_type', 'base_url', 'paged', 'pagination_type', 'uniqId', 'filter_layout'));
