...
final class ADQS_Init
{
	...
	public function update_setting()
	{
		...
		if (!array_key_exists('adqs_admin_emails_triggers', $option)) {
			$option['adqs_admin_emails_triggers'] = [];
		}

		if (!array_key_exists('adqs_user_emails_triggers', $option)) {
			$option['adqs_user_emails_triggers'] = array();
		}
		...
		if (!array_key_exists('enable_guest_listing', $option)) {
			$option['enable_guest_listing'] = 0;
		}

		if (!array_key_exists('listing_expiry_date', $option)) {
			$option['listing_expiry_date'] = 365;
		}

		if (!array_key_exists('enable_guest_listing_post_approv', $option)) {
			$option['enable_guest_listing_post_approv'] = 1;
		}
		if (!array_key_exists('terms_condition_text', $option)) {
			$option['terms_condition_text'] = 'By clicking the "Sign Up" button you are agreeing to the <a href="#">Terms and Conditions</a>.';
		}
