...
class Directory_Type_Frontend extends Custom_Metabox
{
	...
	public function __construct()
	{
		...
		add_action('wp_ajax_nopriv_adqs_add_feature_image', array($this, 'adqs_add_feature_image'));
		add_action('wp_ajax_adqs_add_slider_images', array($this, 'adqs_add_slider_images'));
		add_action('wp_ajax_nopriv_adqs_add_slider_images', array($this, 'adqs_add_slider_images'));
	...
	public function front_add_listing($atts)
	{
		...
		$guest_listing = get_option('adqs_admin_settings', [])['enable_guest_listing'] ?? false;

		if (!$guest_listing && !is_user_logged_in()) {
			ob_start();
...
?>
		...
		if ($post_id) {
			...
			ob_start();
			adqs_get_template_part('listing-add/listing', 'add', compact('directory_type', 'post_id', 'log_status'));
			return ob_get_clean();
		}
	...
	public function save_metabox_data($post_id)
	{
		...
		do_action('adqs_frontend_after_save_metabox_data', $post_id);
	}
