...
class Directory_Type extends Custom_Metabox
{
	...
	public function add_misc_actions($post)
	{
		...
		$isFeatured = is_object($post) ? AD()->Helper->meta_val($post->ID, '_is_featured', '') : '';
...
?>
		<div class="misc-pub-section misc-pub-adpqs-featured-listing">
			<div id="adqs-fl-wrap">
				<label>
					<input value="yes" type="checkbox" name="_is_featured" id="_is_featured"
						<?php checked($isFeatured, 'yes'); ?>>
		...
		<?php
		// Get existing expiry date or set default
		$value = is_object($post) ? AD()->Helper->meta_val($post->ID, '_expiry_date', '') : '';
		...
		// Split the expiry date and time
		if (!empty($expireDays) && strpos($expireDates, ' ') !== false) {
			list($datePart, $timePart) = explode(' ', $expireDates);
			list($aa, $mm, $jj) = explode('-', $datePart);
			list($hh, $mn) = explode(':', $timePart);
		}
		...
		<div class="misc-pub-section misc-pub-adpqs-expiration-time">
			...
			<div class="adqs-never-expire" style="margin-top: 10px;">
				<?php
				$expiryNever = is_object($post) ? AD()->Helper->meta_val($post->ID, '_expiry_never', '') : '';
				?>
				<label>
					<input value="yes" type="checkbox" name="_expiry_never" id="_expiry_never"
						<?php checked($expiryNever, 'yes'); ?>>
				...
				<label>
					<select id="adqs_mm" name="_expiry_date[mm]">
						<?php foreach (range(1, 12) as $month) : ?>
							<option value="<?php printf('%02d', $month); ?>" <?php selected($mm ?? '', sprintf('%02d', $month)); ?>>
								<?php echo esc_html(date('M', mktime(0, 0, 0, $month, 1))); ?>
							</option>
						<?php endforeach; ?>
					</select>
				</label>
				<label>
					<input type="text" id="adqs_jj" placeholder="day" name="_expiry_date[jj]"
						value="<?php echo esc_attr($jj ?? ''); ?>" size="2" maxlength="2">
				</label>
				<label>
					<input type="text" id="adqs_aa" placeholder="year" name="_expiry_date[aa]"
						value="<?php echo esc_attr($aa ?? ''); ?>" size="4" maxlength="4">
				</label>
				at
				<label>
					<input type="text" id="adqs_hh" placeholder="hour" name="_expiry_date[hh]"
						value="<?php echo esc_attr($hh ?? ''); ?>" size="2" maxlength="2">
				</label> :
				<label>
					<input type="text" id="adqs_mn" placeholder="min" name="_expiry_date[mn]"
						value="<?php echo esc_attr($mn ?? ''); ?>" size="2" maxlength="2">
				</label>
			</div>
		</div>
	...
	public function slider_metabox_view($post)
	{
		...
		$name = '_images';
		$value = is_object($post) ? AD()->Helper->meta_val($post->ID, $name, []) : [];
	?>
			...
			<div class="qsd-form-wrap qsd-slider-images" data-name="<?php echo esc_attr($name); ?>">
				...
				?>
							<div class="qsd-slider-image-preview" style="background-image:url(<?php echo esc_url($attachment_image); ?>)">
								<input type="hidden" name="_images[]" value="<?php echo absint($image_id); ?>">
									...
									<div class="qsd-meta-fields-title qsd-mscHeading-wrap">
										...
										<div class="qsd-form-seaction">
											<div class="qsd-form-check-control">
												<input type="checkbox" id="adqs_mtsh_hide_<?php echo esc_attr($sectionId); ?>"
													name="adqs_mtsh_hide_<?php echo esc_attr($sectionId); ?>" value="yes"
													<?php checked(get_post_meta($post_id, "adqs_mtsh_hide_{$sectionId}", true), 'yes'); ?>>
												<label
													for="adqs_mtsh_hide_<?php echo esc_attr($sectionId); ?>"><?php echo esc_html__('Hide Heading on Single Listing Page', 'adirectory'); ?></label>
											</div>
										</div>
									</div>
									<?php
	...
	public function save_misc_actions($post_id = 0)
	{
		...
		do_action('adqs_after_save_metabox_data', $post_id);
	}
	...
	public function save_slider_metabox($post_id = 0)
	{
		// Security checks
		if (
			!isset($_POST['adqs_slider_metabox']) ||
			!wp_verify_nonce($_POST['adqs_slider_metabox'], basename(__FILE__)) ||
		...
		$getData = AD()->Helper->post_data($_POST, '_images', []);
		$getData = array_filter($getData); // Filter out empty values

		if (!empty($getData)) {
			$getData = map_deep($getData, 'absint'); // Ensure all values are integers
		}
