...
function order_users_by_post_count($query)
{
	global $wpdb;

	if (isset($query->query_vars['orderby']) && $query->query_vars['orderby'] === 'ad_listing_count') {
		$query->query_orderby = "ORDER BY post_count " . esc_sql($query->query_vars['order']);
		$query->query_fields .= ", (SELECT COUNT(*) FROM $wpdb->posts WHERE $wpdb->posts.post_author = $wpdb->users.ID AND $wpdb->posts.post_type = 'adqs_directory' AND $wpdb->posts.post_status = 'publish') AS post_count";
	}
}
add_action('pre_user_query', 'order_users_by_post_count');
