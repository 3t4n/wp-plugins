...
/**
 * Filter only valid locations for a set of locations.
 *
 * @param array $locations Set of locations.
 *
 * @return array Filtered valid locations.
 */
function geolocation_filter_valid_locations( $locations ) {
...
add_action(
	'save_post', function( $post_id, $post ) {
		...
		if ( ! $skip ) {
			...
			$valid_locations = geolocation_filter_valid_locations( $locations );

			wp_set_post_terms( $post_id, $valid_locations, 'location' );
		}
	}, 10, 2
);
...
add_action(
	'init', function() {
		...
		/**
		 * We register the taxonomy for each custom post type and we add the
		 * 'post' post type to that list.
		 *
		...
		foreach ( $post_types as $post_type ) {
			add_filter(
				'rest_prepare_' . $post_type, function( $response ) {
					$response->data['location'] = geolocation_filter_valid_locations( $response->data['location'] );
	...
	'pre_get_posts', function( $query ) {
		...
		if ( ! $skip ) {
			if ( is_tax( 'location' ) && $query->is_main_query() ) {
				 ...
				 * type, in the location archive main query we want post post
				 * types.
				 */
				 ...
				 * Locations not included have precedence over locations
				 * included.
				 */
				...
				/**
				 * We apply a filter to $location_ids, in case some script want
				 * to include (or exclude) other locations in the query.
				 */
				$location_ids = apply_filters( 'geolocated_content_pre_get_posts_locations', $location_ids, $query, $operator, $is_admin );

				if ( ! empty( $location_ids ) ) {
					/**
					 * In order to avoid overwriting current tax queries, we
					 * check if there are any before adding our new one.
					 */
