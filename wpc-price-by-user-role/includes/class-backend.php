...
if ( ! class_exists( 'Wpcpu_Backend' ) ) {
	class Wpcpu_Backend {
		...
		public function init() {
			// load text-domain
			load_plugin_textdomain( 'wpc-price-by-user-role', false, basename( WPCPU_DIR ) . '/languages/' );

			self::$capabilities = apply_filters( 'wpcpu_global_setting_capabilities', self::$capabilities );
		}
                ...
                <div class="options_group">
                    <p class="form-field">
                        ...
                        <select name="wpcpu_enable" id="wpcpu-select-enable">
                            ...
                            <option value="override" <?php selected( $enable, 'override' ); ?>
                                    disabled><?php esc_html_e( 'Override', 'wpc-price-by-user-role' ); ?></option>
                        </select>
                    </p>
                    <div style="color: #c9356e; padding-left: 12px; padding-right: 12px;">
                        ...
                        <a href="https://wpclever.net/downloads/wpc-price-by-user-role/?utm_source=pro&utm_medium=wpcpu&utm_campaign=wporg"
		...
		public function admin_menu() {
			...
			if ( ! empty( self::$capabilities ) && is_array( self::$capabilities ) ) {
				foreach ( self::$capabilities as $capability ) {
					if ( current_user_can( $capability ) ) {
						add_menu_page( esc_html__( 'WPC Price by User Role', 'wpc-price-by-user-role' ), esc_html__( 'WPC Price by User Role', 'wpc-price-by-user-role' ), $capability, 'wpclever-wpcpu-global', [
							$this,
		...
		public function admin_menu_content_global() {
			include WPCPU_DIR . 'includes/templates/global-settings.php';
		}
