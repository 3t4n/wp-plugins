...
Description:A discrete plugin to add counter/time/timestamp to mails received or frontend pages with shortcodes that can be added mail subject, mail content, and on the frontend of any page.
Version: 1.0.6
Author: StorePro
...
<?php
    ...
    public function mcf_special_mail_tags($output, $name)
    {
        ...
        $count = intval(get_option('mcf_cf7_count' . $id, 0)); // Allow count to be 0
...
// Dynamic shortcode handler for [cfc_serial_number_X] format
function cfc_dynamic_shortcode_handler($atts, $content = null, $tag = '') {
    preg_match('/cfc_serial_number_(\d+)/', $tag, $matches);

    if (!empty($matches[1])) {
        $id = intval($matches[1]);
        $counter_instance = new ContactFormCounter(); // create an instance
        return $counter_instance->mcf_view_serial_number(['id' => $id]);
    }

    return ''; // Return empty if the format doesn't match
}


// Register dynamic shortcodes for all forms
function cfc_register_dynamic_shortcodes() {
    global $wpdb;
    $form_ids = $wpdb->get_col("SELECT ID FROM {$wpdb->prefix}posts WHERE post_type = 'wpcf7_contact_form'");

    foreach ($form_ids as $form_id) {
        add_shortcode("cfc_serial_number_$form_id", 'cfc_dynamic_shortcode_handler');
    }
}
add_action('init', 'cfc_register_dynamic_shortcodes');
