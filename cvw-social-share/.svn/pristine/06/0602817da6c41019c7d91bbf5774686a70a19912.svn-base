...
function cvw_enqueue_script() {
    ...
    if( isset($post) && ( is_single() || has_shortcode( $post->post_content, 'cvw_social_links' ) ) ) {
        ...
        wp_enqueue_style( 'cvw_brands_styles', plugins_url( '../css/brands.min.css' , __FILE__ ) );
        wp_enqueue_style( 'cvw_regular_styles', plugins_url( '../css/regular.min.css' , __FILE__ ) );
        wp_enqueue_style( 'cvw_awesome_styles', plugins_url( '../css/fontawesome.css' , __FILE__ ) );
        ...
        switch ( $social_name ) {
            case 'facebook':
                if ( $cvw_social_options['option_facebook'] == 'on' ) {
                    $links .= '<a href="http://www.facebook.com/sharer.php?u='. $url .'" target="_blank" rel="noopener" class="social-link social-link-facebook"><i class="fa-brands fa-facebook-f"></i></a>';
                }
                break;            
            case 'twitter':
                if ( $cvw_social_options['option_twitter'] == 'on' ) {
                    $links .= '<a href="http://twitter.com/share?url='. $url .'&text='. $title .'" rel="noopener" class="social-link social-link-twitter"><i class="fa-brands fa-x-twitter"></i></a>';
                }
                break;        
            case 'linkedin':
                if ( $cvw_social_options['option_linkedin'] == 'on' ) {                  
                    $links .= '<a href="http://www.linkedin.com/shareArticle?mini=true&url='. $url .'" target="_blank" rel="noopener" class="social-link social-link-linkedin"><i class="fa-brands fa-linkedin-in"></i></a>';
                }
                break;
            case 'pinterest':
            ...
            case 'whatsapp':
                if ( $cvw_social_options['option_whatsapp'] == 'on' ) {
                    $links .= '<a href="https://wa.me/?text='. $url.'" target="_blank" rel="noopener" class="social-link-1 social-link-whatsapp"><i class="fa-brands fa-whatsapp"></i></a>';
                }
                break;
            case 'telegram':
                if ( $cvw_social_options['option_telegram'] == 'on' ) {
                    $links .= '<a href="https://t.me/share/url?url='. $url.'&text='. $title .'" target="_blank" rel="noopener" class="social-link-1 social-link-telegram"><i class="fa-brands fa-telegram"></i></a>';
                }
                break;
            case 'email':
                if ( $cvw_social_options['option_email'] == 'on' ) {
                    $links .= '<a href="mailto:?subject='. $title .'&amp;body='. $url.'" rel="noopener" class="social-link-1 social-link-email"><i class="fa-regular fa-envelope"></i></a>';
                }
                break;
        }
    }    
