...
add_filter('pre_set_site_transient_update_plugins', 'update_list_sub_pages');
function update_list_sub_pages($transient)
{
    ...
    if (isset($transient->checked[$plugin_folder])) {
        update_option('list_sp_activation_date', time());
    }
    return $transient;
}   
...
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'list_sp_add_action_links');
// Function for add action link
function list_sp_add_action_links($links_array)
{
	array_unshift($links_array, '<a href="' . admin_url('admin.php?page=sub-page') . '">Settings</a>');
	return $links_array;
}
...
{
	// echo LSP_PLUGIN_URL .'js/install_plugin_lsp.js';
	// exit;
	wp_enqueue_script('lsp-js', LSP_PLUGIN_URL	 .'js/install_plugin_lsp.js', array(), SUBPAGE_VERSION);
	add_thickbox();

}
add_action('admin_enqueue_scripts', 'lsp_enqueue_scripts');
...
function widget($args, $instance)
 {
   ...
   echo $before_widget;
    $title = empty($instance['title']) ? 'Pages' : apply_filters('widget_title', $instance['title']);
 	...
 	if (is_array($instance['sort_by']) && !empty($instance['sort_by'])) {
		...
		$sort_by_values = $instance['sort_by'];
	}
	if (is_array($instance['exclude_pages']) && !empty($instance['exclude_pages'])) {
		...
		$exclude_page_id = $instance['exclude_pages'];
	}
    	...
    	if($attachments)
    	{
    	 ...
    	 {	
    	   $args = array(
				...
				if (!empty($exclude_page_id)) {
					$args['exclude'] = $exclude_page_id;
				}
    			wp_list_pages($args);
    	 }?>
   </ul>
