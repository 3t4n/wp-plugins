...
class OxygenWooSettings {
	...
	public static function get_settings() {
		...
		$lang_pdf_manual = 'how-to-setup-oxygen-plugin-en.pdf';

        if(get_user_locale() === 'el'){
            $lang_pdf_manual = 'how-to-setup-oxygen-plugin-el.pdf';
        }

		// Add Download PDF Button
		$settings['download_pdf'] = array(
			'name' => '', // No name needed
			'type' => 'title', // Use 'html' type
			'desc' => '<a href="' . esc_url( plugin_dir_url( __FILE__ ) . '../assets/' . $lang_pdf_manual ) . '" target="_blank" download>' . esc_html__( 'Plugin Settings Instructions (PDF)', 'oxygen' ) . '</a>',
			'id'   => 'download_pdf'.get_user_locale(), // Unique ID for the field
		);
		...
		*/
		...
		$settings['oxygen_products_settings_header'] = array(
			'name' => '',
			'id'   => 'oxygen_products_settings_header',
			'type' => 'html',
			'html' => '<h4>' . __( 'Products settings receipt/invoice creating', 'oxygen' ) . '</h4>',
		);

		$settings['oxygen_products_variations'] = array(
			'name'     => __( 'Use parent product SKU code for variations products', 'oxygen' ),
			'desc_tip' => __( 'By enabling this option , variations products will use always the parent SKU code for receipt/invoice creation in pelatologio app.', 'oxygen' ),
		...
		$oxygen_payments = is_array($oxygen_payments) ? $oxygen_payments : array();

		$settings['oxygen_payment_default'] = array(
			'name' => __('Default payment method','oxygen'),
			'id' => 'oxygen_payment_default',
			'type' => 'select',
			'options' => array_merge(array('0' => ''), $oxygen_payments),
			'desc_tip' => __('In case of an empty payment method in the order details, the selected payment method will be executed.','oxygen')
		);
    ...
    public static function retrieve_oxygen_settings()
    {
        ...
        $settings = array(
            ...
            'oxygen_fetch_vat_fields'           => get_option('oxygen_fetch_vat_fields'),
            'oxygen_products_variations'        => get_option('oxygen_products_variations'),
            'oxygen_receipt_print_type'         => get_option('oxygen_receipt_print_type'),
