function getDemoTheme() {
    ...
    if (document.createStyleSheet != undefined) {
        ...
        var link = jQuery('<link rel="stylesheet" href="' + url + '" media="screen" />');
        link[0].onload = function () {
            if ($.jqx && $.jqx.ready) {
                $.jqx.ready();
            };
        }
        jQuery(document).find('head').append(link);
    }
    $.jqx = $.jqx || {};
    $.jqx.theme = theme;
    return theme;
        ...
        if (window.location.toString().indexOf('file://') >= 0) {
            ...
            if (addMessage) {
                jQuery(document).ready(function () {
                    setTimeout(function () {
                        jQuery(document.body).prepend(jQuery('<div style="font-size: 12px; font-family: Verdana;">Note: To run a sample that includes data binding, you must open it via "http://..." protocol since Ajax makes http requests.</div><br/>'));
                    }
                    , 50);
                });
            }
