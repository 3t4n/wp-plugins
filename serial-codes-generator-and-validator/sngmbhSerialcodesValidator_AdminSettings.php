...
class sngmbhSerialcodesValidator_AdminSettings {
			...
			while (($data = fgetcsv($handle, 4096, $delimiter)) !== FALSE) {
				...
				if ($total == 1) {
					...
					foreach($data as $idx => $column) {
						$column = preg_replace('/[^[:print:]]/', '', $column);
						$column = strtolower(trim($column));
						if (!empty($column) && array_key_exists($column, $columns_pos)) {
	...
	private function setWoocommerceTicketForCode($data) {
		...
		wc_delete_order_item_meta( $item_id, '_sngmbh_serial_code_is_ticket' );
		wc_add_order_item_meta( $item_id, '_sngmbh_serial_code_is_ticket', '1');
	...
	private function testing($data) {
		...
		/*
		$url = $this->getOptionValue("qrDirectURL");
		$codeObj = $this->getCore()->retrieveCodeByCode("96339296391483177556");
		echo $this->getCore()->replaceURLParameters($url, $codeObj);
		*
		/*
		$product_id = 185;
		$v_expiration_days = "2";
