function sngmbhSerialcodesValidator(_myAjaxVar, doNotInit){
	...
	function DateTime2Text(millisek) {
		return Date2Text(millisek, "Y-m-d H:i");
	}
			...
			if (serial != "" && typeof OPTIONS.infos.premium_expiration !== "undefined") {
				...
				if (expiration.last_run != 0 && expiration.timestamp >= 0) {
					...
					if (expirationDate <= today || toCheck >= expirationDate) {
						...
						let info_box = $('<div style="background-color:red;color:white;padding:10px;">').html("Your premium serial expires soon, at the "+expiration.expiration_date+ ' '+expiration.timezone+'<br>It will work, but no updates are possible for the premium plugin after the expiration date.<br>'+msg+'You can <a target="_blank" style="color:white;font-weight:bold;" href="https://vollstart.com/shop/support-update-plugin-serial-codes-generator-and-validator/">renew your license here</a>.');
	...
	function _displayOptionsArea(cbf) {
		...
		_makeGet('getOptions', {}, reply=>{
			...
			meta_tags_keys.forEach(v=>{
				let t = '<p><b>{META_'+v.key+'}</b>: '+v.label+'</p>';
				div_infos.append(t);
			});
	...
	class Layout {
		...
		renderAdminPageLayout(cbf) {
			function __showListImport() {
				...
				let form = $('<form>');
				form.append('<p>Choose delimiter of your columns:<br><select name="delimiter"><option value=",">, (Comma)</option><option value=";">; (Semicolon)</option></select></p>');
				...
				form.on("submit", event=>{
					...
					let delimiter = form.find('select[name="delimiter"]').val();
					var form_data = new FormData();
					form_data.append('action', myAjax._action);
					form_data.append('a_sngmbh', 'importLists');
					form_data.append('data[delimiter]', delimiter);
					...
					$.ajax({
						...
						success: function(result){
							...
							if (result.success) {
								...
								content.append('<p><b>ERROR:</b> '+result.data+'.</p>');
							}
			...
			function __showMaskList(editValues){
			    ...
			    let dlg = $('<div/>').html('<div style="color:gray;">ID: '+(editValues ? editValues.id : 'NEW')+'</div><form>Name<br><input name="inputName" type="text" style="width:100%;" required></form>');
				dlg.dialog(_options);
			...
			_makeGet('getOptions', {}, optionData=>{
				...
				drop_search.append('<option value="">Default search filter</option>');
				drop_search.append('<option value="LIST:">Filter for list id</option>');
				drop_search.append('<option value="ORDERID:">Filter for order id</option>');
				drop_search.append('<option value="CVV:">Filter for cvv value</option>');
				drop_search.append('<option value="STATUS:">Filter for status (1:active, 0:inactive, 2:stolen)</option>');
				drop_search.append('<option value="USERID:">Filter for registered user id</option>');
				drop_search.append('<option value="PRODUCTID:">Filter for product id</option>');
				...
				this.div_codes
					...
					.append(isPremium()?'':' Max. '+myAjax._max.codes_total+' codes. Unlimited with '+getLabelPremiumOnly()+' ').append(btn_codes_new));
	    			...
	    			function ___format(d) {
						...
						function __getData(_codeObj) {
							...
							_makeGet('getMetaOfCode',{'code':d.code}, dataMeta=>{
								...
								if (typeof metaObj['subs'] !== "undefined" && typeof metaObj['subs'].orders !== "undefined") {
									div.append("<h3>WooCommerce Subscription Orders</h3>");
									metaObj['subs'].orders.forEach(v=>{
										div.append($("<div>").text("<b>Order ID:</b> ").append($('<span>').html('<a target="_blank" href="post.php?post='+v.order_id+'&action=edit">#'+v.order_id+'</a> '+DateTime2Text(v.date*1000))));
									});
									// button to remove the sub entries
									// removeSubsFromCode
								}
	...
	function getLabelPremiumOnly() {
		return '[<a href="https://vollstart.com/serial-codes-validator-premium/">PREMIUM ONLY</a>]';
	}
