...
if (!defined('SNGMBH_SERIALCODES_VALIDATOR_PLUGIN_VERSION'))
	define('SNGMBH_SERIALCODES_VALIDATOR_PLUGIN_VERSION', '2.7.7');
...
class sngmbhSerialcodesValidator {
	...
	public function getQRHandler() {
		$className = "sngmbhSerialcodesValidator_QR";
		if (!class_exists($className)) {
			include_once __DIR__.'/'.$className.'.php';
		}
		return sngmbhSerialcodesValidator_QR::Instance();
	}
		...
		if ($this->isPremium()) {
			...
			if ($doCheck) {
				...
				if (!empty($serial)) {
					...
					$url = "https://vollstart.com/plugins/serial-code-validator-premium/"
	...
	*/
	...
	function register_options_page() {
		...
		if ($allowed) {
			...
			add_menu_page( 'Serial Codes Validator', 'Serial Codes Validator', 'read', 'sngmbh-serialcodes-validator', [$this,'options_page'], '', null );
		}
	...
	function options_page() {
		...
		<div>
			...
			<div style="margin-top:100px;">
				...
				<a href="https://vollstart.com/serial-codes-validator-premium/docs/" target="_blank">Click here for more help about the options</a>
				...
				<a href="https://vollstart.com/serial-codes-validator-premium/docs/#filters" target="_blank">Click here for more help about the functions</a>
				</p>
				<ul>
				...
				<center><a target="_blank" href="https://vollstart.com">VOLLSTART</a></center>
			</div>
	  	</div>
		<?php
	...
	public function replaceShortcode($attr=[], $content = null, $tag = '') {
		...
		$vars = array(
				...
				'list_id' => isset($attr['list_id']) ? intval($attr['list_id']) : 0
            );
		$vars['_messages'] = [
