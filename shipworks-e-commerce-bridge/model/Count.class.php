...
class Count_shipAdv {
						...
						if(WOOCOMMERCE_CUSTOM_TABLE_ADV){
							if (!is_plugin_active_custom( "shipworks-addon-eddieswelding/shipworks-addon-eddieswelding.php")) {
								$orders = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix."wc_orders WHERE date_updated_gmt > '" . $dateInLocal . "' AND date_updated_gmt < '".$now."'
													AND type = 'shop_order' AND status !='auto-draft' AND status !='draft' AND status !='wc-checkout-draft' AND status !='trash' order by date_updated_gmt ASC LIMIT 100", ARRAY_A );
							}
							else {
								$orders = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix."wc_orders WHERE date_updated_gmt > '" . $dateInLocal . "' AND date_updated_gmt < '".$now."'
							...
							if (!is_plugin_active_custom( "shipworks-addon-eddieswelding/shipworks-addon-eddieswelding.php")) {
								$orders = $wpdb->get_results("SELECT * FROM " . $wpdb->posts . " WHERE post_modified_gmt > '" . $dateInLocal . "' AND post_modified_gmt < '".$now."'
													AND post_type = 'shop_order' AND post_status !='draft' AND post_status !='wc-checkout-draft' AND post_status !='auto-draft' AND post_status !='trash' order by post_modified_gmt ASC LIMIT 100", ARRAY_A );
							} else {
								$orders = $wpdb->get_results("SELECT * FROM " . $wpdb->posts . " WHERE post_modified_gmt > '" . $dateInLocal . "' AND post_modified_gmt < '".$now."'
	...
	"SELECT * FROM " . $wpdb->prefix."wc_orders WHERE parent_order_id ='".$order['id']."' and type = 'order_shipment' AND status !='draft' AND status !='wc-checkout-draft' AND status !='auto-draft' AND status !='trash' order by date_updated_gmt ASC", ARRAY_A );
									} else {
										$sepOrders = $wpdb->get_results(
	"SELECT * FROM " . $wpdb->posts . " WHERE post_parent ='".$order['ID']."' and post_type = 'order_shipment' AND post_status !='draft' AND post_status !='wc-checkout-draft' AND post_status !='auto-draft' order by post_modified_gmt ASC", ARRAY_A );
									}
									if(!$sepOrders) :
										$newOrders[] = $order;
									else :
