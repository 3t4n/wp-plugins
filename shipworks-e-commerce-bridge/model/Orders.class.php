...
class Orders_shipAdv {
	...
	protected function setOrdersWoocommerce() {
		...
		if (is_plugin_active_custom( "woocommerce-shipping-multiple-addresses/woocommerce-shipping-multiple-address.php")
				...
				if(WOOCOMMERCE_CUSTOM_TABLE_ADV){
					...
					AND type = 'shop_order' AND status !='draft' AND status !='wc-checkout-draft' AND status !='auto-draft' AND status !='trash' order by date_updated_gmt ASC LIMIT 100" , ARRAY_A);
				} else {
					$rows = $wpdb->get_results( "SELECT * FROM " . $wpdb->posts . " WHERE post_modified_gmt > '" . $dateInLocal . "' AND post_modified_gmt < '".$now."'
					AND post_type = 'shop_order' AND post_status !='draft' AND post_status !='wc-checkout-draft' AND post_status !='auto-draft' order by post_modified_gmt ASC LIMIT 100" , ARRAY_A);
				}
			$newOrders = array();
	...
	"SELECT * FROM " . $wpdb->prefix."wc_orders WHERE parent_order_id ='".$order['id']."' and type = 'order_shipment' AND status !='draft' AND status !='wc-checkout-draft' AND status !='auto-draft' AND status !='trash' ORDER BY id ASC", ARRAY_A );
						$id = "id";
					} else {
						$sepOrders = $wpdb->get_results(
	"SELECT * FROM " . $wpdb->posts . " WHERE post_parent ='".$order['ID']."' and post_type = 'order_shipment' AND post_status !='draft' AND post_status !='wc-checkout-draft' AND post_status !='auto-draft' ORDER BY ID ASC", ARRAY_A );
						$id = "ID";
					}
					if(!$sepOrders) :
						$newOrders[] = $order;
