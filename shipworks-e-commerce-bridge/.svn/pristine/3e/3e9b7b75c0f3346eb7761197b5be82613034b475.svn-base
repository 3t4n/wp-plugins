...
class Order_shipAdv {
	...
	protected function setInfoWoocommerce() {
		...
		if(is_plugin_active_custom( "wt-woocommerce-sequential-order-numbers/wt-advanced-order-number.php")) {
			$orderId = Wt_Advanced_Order_Number_Common::get_order_meta($this->id_order, '_order_number');
			if($orderId != 0 and $orderId != false) $this->id_order = $orderId;
		}
		if(WOOCOMMERCE_CUSTOM_TABLE_ADV){
			$date_gmt = $this->row['date_created_gmt'];
			$this->modifiedDate = $date_modified_gmt = date("Y-m-d\TH:i:s\Z", strtotime($this->row['date_updated_gmt']));
		} else {
			$date_gmt = $this->row['post_date_gmt'];
			$this->modifiedDate = $date_modified_gmt = date("Y-m-d\TH:i:s\Z", strtotime($this->row['post_modified_gmt']));
		}
		...
		if($date_modified_gmt != "0000-00-00 00:00:00") {
			if (is_plugin_active_custom( "woocommerce-shipping-multiple-addresses/woocommerce-shipping-multiple-address.php")
				...
				if (!is_plugin_active_custom( "shipworks-addon-eddieswelding/shipworks-addon-eddieswelding.php")) {
					if(WOOCOMMERCE_CUSTOM_TABLE_ADV){
						$resultPostParent = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix."wc_orders WHERE id ='".$this->row['parent_order_id']."' and type = 'shop_order' AND status !='draft' AND status !='wc-checkout-draft' AND status !='auto-draft' AND status !='trash' order by date_updated_gmt ASC", ARRAY_A );
						$columnParent = "parent_order_id";
					}
					else {
						$resultPostParent = $wpdb->get_row("SELECT * FROM " . $wpdb->posts . " WHERE ID ='".$this->row['post_parent']."' and post_type = 'shop_order' AND post_status !='draft' AND post_status !='wc-checkout-draft' AND post_status !='auto-draft' order by post_modified_gmt ASC", ARRAY_A );
						$columnParent = "post_parent";
					}
				} else {
					if(WOOCOMMERCE_CUSTOM_TABLE_ADV){
				...
				/*
					...
					*/
					...
					$order                  = wc_get_order( $this->row[$columnParent] );
					$packages               = get_post_meta( $this->row[$columnParent], '_wcms_packages', true );
