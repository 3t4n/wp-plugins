...
class StatusManager_shipAdv {
	...
	protected function setInfoWoocommerce2v2() {
		...
		if (is_plugin_active_custom( "woocommerce-shipping-multiple-addresses/woocommerce-shipping-multiple-address.php")
					...
					//FIND PARENT ID
					if(WOOCOMMERCE_CUSTOM_TABLE_ADV){
						$resultPostParent = $wpdb->get_row("SELECT * FROM " . $wpdb->prefix."wc_orders WHERE id ='".$this->order."' and type = 'order_shipment' AND status !='draft' AND status !='wc-checkout-draft' AND status !='auto-draft' AND status !='trash' order by date_updated_gmt ASC", ARRAY_A );
						$columnParent = "parent_order_id";
					}
					else {
						$resultPostParent = $wpdb->get_row("SELECT * FROM " . $wpdb->posts . " WHERE ID ='".$this->order."' and post_type = 'order_shipment' AND post_status !='draft' AND post_status !='wc-checkout-draft' AND post_status !='auto-draft' order by post_modified_gmt ASC", ARRAY_A );
			...
			if (is_plugin_active_custom( "woocommerce-shipping-multiple-addresses/woocommerce-shipping-multiple-address.php")
				...
				if($woocommerce_api_shipAdv and class_exists( 'WC_Order' )) {
					...
					if(WOOCOMMERCE_CUSTOM_TABLE_ADV){
						...
						$select = $wpdb->get_row("SELECT * FROM ".$wpdb->posts." WHERE ID = ".$this->order, ARRAY_A);
					}
