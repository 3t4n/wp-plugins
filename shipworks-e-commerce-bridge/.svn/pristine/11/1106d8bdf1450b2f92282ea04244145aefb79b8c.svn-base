...
* Woocommerce ( Minimum version 2.0, tested up to 9.6.0 )

= Minimum Requires: =
* Wordpress 3.0 ( Minimum version 3.0, tested up to 6.7.1)
...
function modify_tracking_notes($string, $carrier, $tracking, $orderid){
  ...
  if($order){
    if($order->get_status() == "split-order"){
      return sprintf(__("Part of your order %s was shipped on %s via %s. Tracking number is %s."),$order->get_order_number(), $order->get_date_modified(), $carrier, $tracking);
    }
  }
  return sprintf(__("Your order %s was shipped on %s via %s. Tracking number is %s."),$order->get_order_number(), $order->get_date_modified(), $carrier, $tracking);
}
...
== Changelog ==
= 5.2.8 =
* Made a fix for WebToffee Sequential orders
= 5.2.7 =
* Add filter to modify Tracking number notes to the customer
= 5.2.6 =
* Reinforced security when credentials are created
= 5.2.5 =
* Fixed issue with the last modified date
= 5.2.4 =
* Fixed a bug related to Order numbers
= 5.2.3 =
* Fixed some issue with Sequential order numbers where some orders weren't downloaded in Shipworks
= 5.2.2 =
* Fixed issue with Gutenberg Woocommerce Checkout block that pulling Draft order into Shipworks
= 5.2.1 =
* Fixed issue on Attributes to have their first letter capitalized
= 5.2.0 =
* Woocommerce Sequential Order Numbers (By WebToffee) is now compatible
= 5.1.15 =
