...
class TrackingManager_shipAdv {
	...
	protected function setInfoWoocommerce() {
		...
		if ( !$row ) {
			...
			$dateModify = date("m-d-Y", strtotime($this->dateShipping));
			$note = apply_filters("adv_tracking_note", sprintf(__("Your order %s was shipped on %s via %s. Tracking number is %s."),$this->orderIdSent, $dateModify, $this->carrier, $this->tracking),$this->carrier, $this->tracking,$this->order);
