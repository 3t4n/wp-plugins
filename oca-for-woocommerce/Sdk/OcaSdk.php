...
class OcaSdk {
	...
	public function process_order(
		...
		$data_to_send = apply_filters( 'wc_oca_order_process_request_data', $data_to_send, $seller, $to, $items, $order, $shipping_code, $dispatch_day );
	 ...
	 */
	public function cancel_order( string $oca_number, WC_Order $order ): array { // phpcs:ignore
		$data_to_send = array(
		...
		$data_to_send = apply_filters( 'wc_oca_order_cancel_request_data', $data_to_send, $oca_number, $order );
	...
	public function send_reports( array $report_uris ): array {
		...
		try {
			$res = $api->post( '/oca/reports', array( 'reports' => $report_uris ) );
		} catch ( Exception $e ) {
			Helper::log_error( __FUNCTION__ . ': ' . $e->getMessage() );
			return array();
		}
