...
class HealthChecker {
	...
	public function __construct( OcaSdk $sdk ) {
		...
		add_action( 'wc_oca_test_orders_manager', array( $this, 'cancel_oca_order' ), 10, 2 );
	}
	...
	public function is_processor_api_valid(): bool {
		...
		$customer = new Customer(
			...
			new Address(
				...
				'Capital Federal',
				'C',
				'ORDEN DE PRUEBA - NO PROCESAR'
			)
		);
		$customer->get_address()->set_shipping_branch( 147 );

		$items = new Items( array( new Item( 0.1, 0.1, 0.1, 0.1, 100, 'Producto falso', 1, 100 ) ) );
		...
		$this->cancel_oca_order( (string) $response['oca_number'], $order );

		return true;
	}

	public function cancel_oca_order( string $oca_number, WC_Order $order ): void {
		$response = $this->sdk->cancel_order( $oca_number, $order );

		if ( empty( $response ) || isset( $response['error'] ) ) {
			wp_schedule_single_event( ( new \DateTime() )->modify( '+5 minutes' )->getTimestamp(), 'wc_oca_test_orders_manager', array( $oca_number, $order ) );
		}
	}
}
