...
class OrdersProcessor {
	...
	public function __construct( OcaSdk $sdk, AllLabelsProcessor $labels_processor, DaysManager $days_manager ) {
		...
		// Public actions (so anyone within the WordPress code can process orders)
		add_action( 'wc_oca_process_order', array( $this, 'process_order' ), 10, 2 );
	}
	...
	public function process_order( WC_Order $order, bool $force ): void {
		do_action( 'wc_oca_before_order_process_starts', $order );
		...
		if ( empty( $response ) ) {
			do_action( 'wc_oca_order_process_failed', $order, 'Empty response' );
			Helper::add_error( __( 'There was an error processing the order with OCA, please try again', 'wc-oca' ) );
			return;
		}

		if ( isset( $response['error'] ) ) {
			do_action( 'wc_oca_order_process_failed', $order, $response['error'] );
