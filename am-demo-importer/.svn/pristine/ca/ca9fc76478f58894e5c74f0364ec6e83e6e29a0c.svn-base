var Whizzie = (function($) {
  ...
  function PluginManager() {
        ...
        if (typeof response.url != 'undefined') {
          ...
          if (response.hash == current_item_hash) {
            ...
            jQuery.post(response.url, response, function(response2) {
              process_current();
              $current_node.find('.wizard-plugin-status').text(response.message + am_demo_importer_pro_whizzie_params.verify_text);
            }).fail(ajax_callback);
    ...
    function process_current() {
      if (current_item) {
        ...
        jQuery.post(am_demo_importer_pro_whizzie_params.ajaxurl, {
          action: 'setup_plugins',
          wpnonce: am_demo_importer_pro_whizzie_params.wpnonce,
          slug: current_item
  ...
  function WidgetManager() {
    ...
    function import_widgets() {
      jQuery.post(
        am_demo_importer_pro_whizzie_params.ajaxurl, {
          action: 'am_demo_importer_setup_elementor',
          wpnonce: am_demo_importer_pro_whizzie_params.wpnonce
...
jQuery(document).ready(function() {
  ...
  if (current_menu == 1) {
    ...
    jQuery('#toplevel_page_amdemoimporter-wizard').addClass('current');
  }
  ...
  jQuery('#am_demo_importer_pro_license_form button[id="change--key"]').on('click', function() {
    var $am_demo_importer_pro_license_form = jQuery('#am_demo_importer_pro_license_form');
    $am_demo_importer_pro_license_form.find('input[name="am_demo_importer_pre_license_key"]').val('');
    $am_demo_importer_pro_license_form.find('input[name="am_demo_importer_pre_license_key"]').attr('disabled', false);
    $am_demo_importer_pro_license_form.find('button[type="submit"]').val('');
    $am_demo_importer_pro_license_form.find('button[type="submit"]').attr('disabled', false);
    $am_demo_importer_pro_license_form.find('button[type="submit"]').text('Activate');
    jQuery('#start-now-next').hide();
    jQuery(this).remove();
  });
  jQuery('form#am_demo_importer_pro_license_form').on('submit', function(e) {
    jQuery('.wee_theme_activation_spinner').show();
    e.preventDefault();
    var key_to_send = jQuery('form#am_demo_importer_pro_license_form').serializeArray()[0].value;
    if (key_to_send == "") {
      ...
      jQuery.post(
        am_demo_importer_pro_whizzie_params.ajaxurl, {
          action: 'wz_activate_am_demo_importer_pro',
          wpnonce: am_demo_importer_pro_whizzie_params.wpnonce,
          am_demo_importer_pre_license_key: key_to_send
        },
        ...
        function(data, status) {
          if (status == 'success') {
            if (data.status) {
              ...
              jQuery('form#am_demo_importer_pro_license_form button[type="submit"]').css("background-color:#0a9d2c");
              jQuery('form#am_demo_importer_pro_license_form button[type="submit"]').text('Activated');
              jQuery('form#am_demo_importer_pro_license_form button[type="submit"]').attr('disabled', 'disabled');
