    ...
    switch ($current_tab) {
        ...
        case 'registered_users':
            ...
            foreach ($users as $user) {
                ...
                wp_nonce_field('awareid_verification_registered_users_nonce', '_wpnonce');
            ...
            for ($i = 1; $i <= $total_pages; $i++) {
                $pagination_url = add_query_arg([
                    ...
                    'page_num' => $i,
                    '_wpnonce' => wp_create_nonce('awareid_verification_registered_users_nonce'),
                    'search' => $search_term,
                ], admin_url('admin.php'));
            ...
            if (isset($_POST['update_user_meta'])) {
                ...
                if (!isset($_POST['_wpnonce']) || 
                    !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_wpnonce'])), 'awareid_verification_registered_users_nonce')) {
                ...
                if ($user_verified === '1') {
                    ...
                    delete_user_meta($user_id, 'last_verified');
                }
                ...
                if ($current_user_verified === '1' && $user_verified === '0') {
                    ...
                    if (!isset($options['awareid_field_awareid_domain'], $options['awareid_field_apikey'], 
                            $options['awareid_field_realm_name'], $options['awareid_field_client_secret'])) {
...
function awareid_settings_init()
{
    ...
    */
    ...
    add_settings_field(
        'awareid_field_registration_verification',
        __('Enable Verification at Registration', 'awareid-wc-integration'),
        'awareid_field_registration_verification_cb',
        'awareid',
        'awareid_section_developers',
        [
            'label_for' => 'awareid_field_registration_verification',
            'class' => 'awareid_row',
        ]
    );
...
function awareid_field_registration_verification_cb() {
    $options = get_option('awareid_options');
    $enabled = isset($options['awareid_field_registration_verification']) ? $options['awareid_field_registration_verification'] : '';
    ?>
    <input type='checkbox' id='awareid_field_registration_verification' 
           name='awareid_options[awareid_field_registration_verification]' 
           <?php checked($enabled, 1); ?> 
           value='1'>
    <p class="description">
        <?php _e('If enabled, users must complete identity verification before their account is activated.', 'awareid-wc-integration'); ?>
    </p>
    <?php
}
...
function awareid_disallowed_states_callback($args) {
    ...
    echo '<select multiple class="form-select" name="awareid_options[awareid_disallowed_states][]" id="awareid_field_disallowed_states">';
