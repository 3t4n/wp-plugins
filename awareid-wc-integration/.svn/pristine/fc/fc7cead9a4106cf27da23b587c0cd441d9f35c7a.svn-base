...
let checkWatchlistUrl = userData.check_watchlist_url;
...
function initializeModal() {
  ...
  if (cameraModal) {
    ...
    cameraModal.addEventListener("hidden.bs.modal", function () {
      if (welcomeTagline !== "") {
        let errorLocation = document.getElementById("mainTagline");
        if (errorLocation) errorLocation.innerText = welcomeTagline;
      }
      stopAllViaModal();
    });
  }
...
function extractFirstName(givenNames) {
...
async function checkWatchlist(userInfo) {
  const fieldMap = new Map();
  userInfo.forEach(item => fieldMap.set(item.name, item.fieldResult.visual));
  let payload = {
      body: {
          enrollmentToken: enrollmentToken,
          screeningRequest: {
              entityType: "INDIVIDUAL",
              firstName: extractFirstName(fieldMap.get('Given names')) || "",
              lastName: fieldMap.get('Surname') || "",
              gender: mapGender(fieldMap.get('Sex') || "U"),
              birthDate: formatDate(fieldMap.get('Date of birth') || "")
          }
      },
      nonce: userData.nonce,
      jwt: `${jwt}`,
      targetUrl: "onboarding/enrollment/compliance/checkWatchlist"
  };
  const watchlistResults = await postPayloadFetch(checkWatchlistUrl, payload);
...
async function validateDocument(strImageFront, strImageBack) {
  ...
  try {
    ...
    if (
      ...
      if (validationResult.enrollmentStatus === 1 && checks.includes("checkWatchlist"))  {
        videoSpinner.classList.remove("d-none");
        showFeedback("Performing watchlist checks...");
        let watchlistResults = await checkWatchlist(validationResult.ocrResults.fieldType);
        videoSpinner.classList.add("d-none");
        validationResult.enrollmentStatus = watchlistResults.enrollmentStatus;
        if (watchlistResults.enrollmentStatus === 0) {
          failAnimation.classList.remove("d-none");
          showResults("Watchlist Check failed.");
          setTimeout(() => {
      ...
      handleEnrollmentStatus(validationResult)
...
async function postPayloadFetch(url, body) {
  let wordpressResponse = await fetch(url, {
    ...
    headers: {
      ...
      ...(url.includes("my_namespace/v1/") ? { "X-WP-Nonce": userData.nonce } : {}),
    },
  ...
  if (wordpressResponse.ok) {
    if (!responseText) {
      throw new Error("Empty response received from WordPress server");
    }
    try {
    ...
    throw new Error(responseText || "Received invalid response from WordPress server");
  }
}
...
async function startEnrollment() {
  ...
  if (res) {
    ...
    let errorLocation = document.getElementById("mainTagline");
    welcomeTagline = errorLocation.innerText;
    if (checks.includes("addVoice") || checks.includes("addPin") || checks.includes("deviceSecurity")) {
      showFeedback("We currently do not support this workflow.");
      errorLocation.innerText = "We currently do not support this workflow"
      toggleSpinner(false);
      handleStopAutocapture()
    }
    if (checks.includes("addFace") && checks.includes("checkWatchlist") && !checks.includes("addDocument")) {
      showFeedback("We currently do not support this workflow.");
      errorLocation.innerText = "We currently do not support this workflow"
      toggleSpinner(false);
      handleStopAutocapture()
...
function handleEnrollmentStatus(result) {
  if (!result || typeof result !== 'object') {
    console.error("handleEnrollmentStatus: result is not a valid object. Exiting.");
    return;
  }
  if (typeof result.enrollmentStatus === 'undefined') {
    console.error("handleEnrollmentStatus: result.enrollmentStatus is undefined. Exiting.");
    return;
  }
  // 4) If enrollmentStatus is 2 (success)
  if (result.enrollmentStatus == 2) {
    ...
    if (currentPageUrl.includes(userData.cart_url)) {
      loggedIn = true;
      checkButtonStatus();
      return;
    }

    if (window.awareidRegistrationForm) {
      doRealSubmit(window.awareidRegistrationForm);
      window.awareidRegistrationForm = null;
      return;
    }
  } 
  else if (result.enrollmentStatus == 1 && typeof checks !== 'undefined' && checks.includes("addDocument")) {
...
function doRealSubmit($form) {
  ...
  // add to handled set so we don't intercept again
  $form.trigger('submit');
}
