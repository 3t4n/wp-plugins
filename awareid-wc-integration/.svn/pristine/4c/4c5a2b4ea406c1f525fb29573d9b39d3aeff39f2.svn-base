...
function awareid_start_session() {
    // Don't attempt to set cookie or start session if headers already sent
    if (headers_sent()) {
        return;
    }
    ...
    if (!isset($_COOKIE['cookie_user_id'])) {
        ...
        setcookie('cookie_user_id', $unique_id, [
...
function awareid_get_script_data() {
    ...
    return array(
        ...
        'handles' => array(
            ...
            'regula_js' => 'regula-js',
            'aware_capture' => 'aware-capture-main-script',
            'aware_css' => 'aware-capture-main-css',
            'aware_geolocation' => 'aware-geolocation',
            'aware_checkout' => 'aware-custom-checkout'
        ),
        'localize_data' => array(
            'pluginData' => array(
            ...
            'userData' => array(
                ...
                'check_watchlist_url' => rest_url('my_namespace/v1/check-watchlist'),
...
function awareid_enqueue_core_scripts($script_data) {
    $handles = $script_data['handles'];
    $version = $script_data['version'];

    // Core styles and scripts
    wp_enqueue_style($handles['bootstrap_css'], plugins_url('styles/bootstrap.min.css', __FILE__), array(), $version);
    wp_enqueue_script($handles['bootstrap_js'], plugins_url('js/bootstrap.bundle.min.js', __FILE__), array(), $version, true);
    wp_enqueue_style($handles['fontawesome_css'], plugins_url('styles/fontawesome.css', __FILE__), array(), $version, true);
    wp_enqueue_script($handles['aware_geolocation'], plugins_url('js/custom-verification.js', __FILE__), array(), $version, true);

    // Localize geolocation data
    wp_localize_script($handles['aware_geolocation'], 'verification_params', $script_data['localize_data']['verification_params']);
}

function awareid_enqueue_verification_scripts($script_data) {
    $handles = $script_data['handles'];
    $version = $script_data['version'];
    
    wp_enqueue_script($handles['regula_js'], plugins_url('js/regula.js', __FILE__), array(), $version, true);
    wp_enqueue_style($handles['aware_css'], plugins_url('styles/styles.css', __FILE__), array(), $version);
    wp_enqueue_script($handles['aware_capture'], plugins_url('js/main.js', __FILE__), array($handles['bootstrap_js']), $version, true);

    // Localize data for main.js
    foreach ($script_data['localize_data'] as $handle => $data) {
        wp_localize_script($handles['aware_capture'], $handle, $data);
    }
}

function awareid_enqueue_scripts() {
    $script_data = awareid_get_script_data();
    $handles = $script_data['handles'];
    $version = $script_data['version'];
    
    // Enqueue core scripts
    awareid_enqueue_core_scripts($script_data);

    // Checkout/cart specific functionality
    if (is_checkout() || is_page('verification-required') || is_cart()) {
        awareid_enqueue_verification_scripts($script_data);

        if (function_exists('wc_get_cart_url')) {
            $script_data['localize_data']['userData']['cart_url'] = wc_get_cart_url();
            $script_data['localize_data']['userData']['checkout_url'] = wc_get_checkout_url();
        }

        if (is_checkout()) {
            wp_enqueue_script($handles['aware_checkout'], plugins_url('js/disable-checkout.js', __FILE__), array('jquery'), $version, true);
            foreach ($script_data['localize_data'] as $handle => $data) {
                wp_localize_script($handles['aware_checkout'], $handle, $data);
            }
        }
    }

    // Registration verification
    if (isset($script_data['admin_options']['awareid_field_registration_verification']) && 
        $script_data['admin_options']['awareid_field_registration_verification'] == 1) {
        
        awareid_enqueue_verification_scripts($script_data);
        
        // Registration specific script
        wp_enqueue_script(
            'awareid-registration-verification',
            plugins_url('js/registration-verification.js', __FILE__),
            array('jquery', $handles['aware_capture']),
            $version,
            true
        );

        wp_localize_script('awareid-registration-verification', 'userData', $script_data['localize_data']['userData']);
        wp_localize_script('awareid-registration-verification', 'pluginData', $script_data['localize_data']['pluginData']);
    }
}
add_action('wp_enqueue_scripts', 'awareid_enqueue_scripts');

function awareid_login_enqueue_scripts() {
    // Add this debug line to verify the function is running
    error_log('AwareID: Attempting to enqueue login scripts');

    $script_data = awareid_get_script_data();
    
    // Only proceed if registration verification is enabled
    if (empty($script_data['admin_options']['awareid_field_registration_verification']) || 
        $script_data['admin_options']['awareid_field_registration_verification'] != 1) {
        error_log('AwareID: Registration verification not enabled');
        return;
    }
    ...
    // Core scripts with explicit jQuery dependency
    wp_enqueue_style($script_data['handles']['bootstrap_css'], plugins_url('styles/bootstrap.min.css', __FILE__));
    wp_enqueue_script($script_data['handles']['bootstrap_js'], plugins_url('js/bootstrap.bundle.min.js', __FILE__), array('jquery'));
    wp_enqueue_style($script_data['handles']['fontawesome_css'], plugins_url('styles/fontawesome.css', __FILE__));
    wp_enqueue_script($script_data['handles']['regula_js'], plugins_url('js/regula.js', __FILE__), array('jquery'));
    wp_enqueue_style($script_data['handles']['aware_css'], plugins_url('styles/styles.css', __FILE__));
    
    // Main script with dependencies
    wp_enqueue_script($script_data['handles']['aware_capture'], 
        plugins_url('js/main.js', __FILE__), 
        array('jquery', $script_data['handles']['bootstrap_js'], $script_data['handles']['regula_js'])
    );

    // Localize data for main.js
    wp_localize_script($script_data['handles']['aware_capture'], 'pluginData', $script_data['localize_data']['pluginData']);
    wp_localize_script($script_data['handles']['aware_capture'], 'userData', $script_data['localize_data']['userData']);

    // Registration verification script
    wp_enqueue_script('awareid-registration-verification',
        plugins_url('js/registration-verification.js', __FILE__),
        array('jquery', $script_data['handles']['aware_capture'])
    );

    error_log('AwareID: Successfully enqueued login scripts');
}

// Add the enqueuing hook
add_action('login_enqueue_scripts', 'awareid_login_enqueue_scripts', 10);
...
add_action('rest_api_init', function () {
    register_rest_route(
        ...
        array(
            'methods' => 'POST',
            'callback' => 'awareid_check_watchlist_callback',
            'permission_callback' => function (WP_REST_Request $request) {
                return true;
            },
        )
...
function awareid_check_watchlist_callback(WP_REST_Request $request)
{
    if (!awareid_is_nonce_valid($request)) {
        return new WP_Error('invalid_nonce', 'Invalid or expired nonce', ['status' => 403]);
    }
...
function awareid_handle_response($response, $request, $awareIDConfig, $userDetails)
{
    ...
    return wp_send_json($decoded_response);
}
...
function awareid_update_user_meta_based_on_enrollment($decoded_response)
{
    ...
    if (function_exists('is_checkout') && is_checkout()) {
        $unique_id   = isset($_COOKIE['cookie_user_id']) ? sanitize_key($_COOKIE['cookie_user_id']) : '';
        $userDetails = awareid_get_users_details();
        $email       = $userDetails['awareid_email'];
        ...
        $unique_id   = isset($_COOKIE['cookie_user_id']) ? sanitize_key($_COOKIE['cookie_user_id']) : '';
        set_transient('awareid_registration_code_' . $unique_id, $decoded_response->registrationCode, 10 * MINUTE_IN_SECONDS);
    }
}
...
function awareid_verification_form_shortcode()
{
    ...
    <?php endif; ?>
    <?php if (!is_checkout() && !is_woocommerce_blocks_based() && (is_cart() || is_page('verification-required'))): ?>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-outline-primary wp-element-button" data-bs-toggle="modal"
                        data-bs-dismiss="modal" id="captureBtn" data-bs-target="#identityVerificationModal" disabled>Agree and
                        Proceed</button>
                </div>
            </div>
        </div>
                ...
                <div class="modal-body" id="mainCard">
                    ...
                    <div class="main-box order-sm-1" id="loginBox">
                        ...
                        <div class="row d-flex justify-content-center main-title" id="main-title"
                            ...
                            <p class="fs-6 text-dark" id="mainTagline">
...
add_filter('wp_pre_insert_user_data', 'awareid_intercept_registration_minimal', 10, 2);
function awareid_intercept_registration_minimal($data, $update) {
    $options = get_option('awareid_options');

    if (empty($options['awareid_field_registration_verification']) ||
        $options['awareid_field_registration_verification'] != 1) {
        return $data; 
    }
    ...
    if (!empty($data['user_email']) && email_exists($data['user_email'])) {
        return $data; 
    }
    ...
    $registration_code = get_transient('awareid_registration_code_' . $unique_id);

    if (!$registration_code) {
        status_header(400);
        wp_send_json_error(array(
            'message' => __('Biometric verification is required before registration can proceed.', 'awareid')
        ));
    }

    return $data;
}
add_action('user_register', 'awareid_finalize_registration', 10, 1);
function awareid_finalize_registration($user_id) {
    $unique_id = isset($_COOKIE['cookie_user_id']) ? sanitize_key($_COOKIE['cookie_user_id']) : '';
    ...
    update_user_meta($user_id, 'user_verified', '1');
    update_user_meta($user_id, 'last_verified', current_time('mysql'));
    update_user_meta($user_id, 'user_registration_code', $registration_code);

    wp_set_auth_cookie($user_id);

    delete_transient('awareid_registration_code_' . $unique_id);
    delete_transient('awareid_email_' . $unique_id);
}


function awareid_force_modal_in_footer() {
    if (is_admin() || is_checkout() || is_cart() || is_page('verification-required')) {
        return;
    }
    echo do_shortcode('[awareid_verification_form]');
}
add_action('wp_footer', 'awareid_force_modal_in_footer', 999);