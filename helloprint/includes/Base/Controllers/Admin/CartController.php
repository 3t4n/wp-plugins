...
class CartController extends BaseController
{
    ...
    public function file_upload_validation($passed)
    {
        ...
        $files_count = isset($_FILES['wphp_product_file_upload']['name']) 
        ? count($_FILES['wphp_product_file_upload']['name']) 
        : 0;

        if ($files_count === 1) {
            // Access the file details
            $file_name = $_FILES['wphp_product_file_upload']['name'][0] ?? '';
            $file_tmp_name = $_FILES['wphp_product_file_upload']['tmp_name'][0] ?? '';

            // Use wp_check_filetype_and_ext to validate file type and extension
            ...
            if (empty($file_info['ext']) || empty($file_info['type'])) {
                return $passed; // File is not valid, return without processing further
            }
        }
        ...
        if ($files_count >= 1) {
            for ($i = 0; $i < $files_count; $i++) {
                $file_name = sanitize_text_field(wp_unslash($_FILES['wphp_product_file_upload']['name'][$i]));
                $tmp_name = $_FILES['wphp_product_file_upload']['tmp_name'][$i];

                // Use wp_check_filetype_and_ext to validate file type and extension
                $file_info = wp_check_filetype_and_ext($tmp_name, $file_name);
                $file_type = $file_info['type'];

                // Check if file type is in the list of valid MIME types
                if (!in_array($file_type, $validMimeTypes)) {
    ...
    public function helloprint_add_to_cart($cart_item_data, $product_id)
    {
        ...
        $actual_delivery_date =  sanitize_text_field(wp_unslash($_POST["wphp_actual_delivery_date"]));
        ...
        $cart_item_data['helloprint_product_setup'] = array(
            ...
            'custom_options' => isset($_POST['wphp_options']) ? wp_json_encode(array_map('sanitize_text_field',$_POST['wphp_options'])) : '',
            'appreal_size_options' => isset($_POST['appreal_size']) ? wp_json_encode(sanitize_text_field(wp_unslash($_POST['appreal_size']))) : '',
        ...
        $cart_item_data['unique_key'] = wp_kses(md5( microtime().wp_rand() ), false);
        return $cart_item_data;
    }
        ...
        foreach ($cart_item['helloprint_product_setup'] as $key => $value) {
            if (in_array($key, ['quantity', 'delivery_option', 'want_graphic_design', 'graphic_design_price', 'destination_country'])) {
                ...
                if ($key == "delivery_option") {
                    if (!empty($cart_item["helloprint_product_setup"]["actual_delivery_date"])) {
                        $key_value_for_cart .=  ' - ' . $cart_item["helloprint_product_setup"]["actual_delivery_date"];
                    }
                }
                $item_data[] = array(
