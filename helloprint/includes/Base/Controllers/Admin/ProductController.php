...
class ProductController extends BaseController
{
    ...
    public function add_helloprint_product_detail_panel()
    {
        ...
        try {
            ...
            $product_tiers = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT id, pricing_tier_id, profile FROM $productTierTableName WHERE product_id = %d",
                    $product_id
                )
            );
            ...
            $selected_tier_details = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT id FROM $tierTable WHERE id = %d",
                    $selected_pricing_tier
                )
            );
            ...
            $results_for_markups = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT id, name FROM $tierTableName WHERE tier_type = %s ORDER BY id ASC",
                    'markup'
                )
            );
        ...
        if ((isset($_POST['product-type']) && sanitize_text_field( wp_unslash( $_POST['product-type'] ) )  == 'helloprint_product') || (isset($_POST['product_type']) &&  sanitize_text_field( wp_unslash( $_POST['product_type'] ) ) == 'helloprint_product') && !empty($_POST['helloprint_external_product_id'])) {
            ...
            if ($product_mark_mar_option == 0) {
                ...
                if (!empty($product_tiers[0])) {
                    $wpdb->query(
                        $wpdb->prepare(
                            "UPDATE $productTierTableName SET pricing_tier_id = %d, profile = %s WHERE product_id = %d",
                            $pricing_tier_id,
                            $role,
                            $product_id
                        )
                    ...
                    $wpdb->query(
                        $wpdb->prepare(
                            "INSERT INTO $productTierTableName (product_id, pricing_tier_id, profile) VALUES (%d, %d, %s)",
                            $product_id,
                            $pricing_tier_id,
                            $role
                        )
                ...
                $wpdb->query(
                    $wpdb->prepare(
                        "DELETE FROM $productTierTableName WHERE product_id = %d",
                        $product_id
                    )
                );
            }
            ...
            if (\hp_is_pitchprint_plugin_active()) {
                if (!empty($response['quotes'])) {
                    ...
                    if (!empty($variantKey)) {
                        ...
                        $tableName = $wpdb->prefix . 'helloprint_pitch_prints'; // Dynamic table name
                        $query = $wpdb->prepare(
                            "SELECT id, name, pitchprint_design_id, hp_variant_key 
                            FROM {$tableName} 
                            WHERE hp_variant_key = %s", 
                            $variantKey
                        );
    ...
    {
        $allowed_domains = ['example.com', 'trustedsource.com']; // Replace with trusted sources
    ...
    private function save_img_as_preview($image_url = '', $post_id = null)
    {
        ...
        $context = array('source' => 'wphp-product-import');

        $logger->notice("Image Url :: " . $image_url . "  product id " . $post_id . " ProductController :: line no 588", $context);
        ...
        // Ensure the WP_Filesystem is initialized first
        ...
        $logger->notice("put image data of product id " . $post_id . " ProductController :: line no 615", $context);
        ...
        if (!in_array($wp_filetype['type'], $allowed_types)) {
            $wp_filesystem->delete($file); // Delete if the type is not allowed
            return;
        }
    ...
    public function helloprint_view_order_details($item_id, $item, $order)
    {
        ...
        // Fetch line item preset
        $lineItemPreset = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM $line_item_tableName WHERE line_item_id = %d",
                $item_id
            )
        );

        // Initialize SKU preset array
        $sku_Preset = [];
        if (!empty($product_sku)) {
            $sku_Preset = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM $order_preset_tableName WHERE helloprint_item_sku = %s",
                    $product_sku
                )
            );
        }

        // Check product type and preset existence
        ...
        $artworks = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM $artworkFileTable WHERE line_item_id = %d",
                $item_id
            )
        );
        if (empty($artworks)) {
            ...
            if (!empty($item['helloprint_product_setup'])) {
                    ...
                    if (!empty($fileUploads['file_path'])) {
                        ...
                        if (!empty($file_name) && !empty($file_url)) {
                            ...
                            if (empty($old_artworks)) {
                                // Secure the query with placeholders and $wpdb->prepare()
                                $insertQuery = $wpdb->prepare(
                                    "INSERT INTO {$artworkFileTable} (order_id, line_item_id, file_url) VALUES (%d, %d, %s)",
                                    $order_id,   // Integer placeholder for order_id
                                    $item_id,    // Integer placeholder for item_id
                                    $file_url    // String placeholder for file_url
                                );
                ...
                wc_update_order_item_meta($item_id, 'helloprint_product_setup', wp_json_encode($product_setup));
                $artworks = $wpdb->get_results(
                    $wpdb->prepare(
                        "SELECT * FROM $artworkFileTable WHERE line_item_id = %d",
                        $item_id
    ...
    public function helloprint_save_artwork_of_order()
    {
        ...
        foreach ($_FILES as $file) {
            $fileTmpName = is_array($file['tmp_name']) ? $file['tmp_name'][0] : $file['tmp_name'];
            $fileName = is_array($file['name']) ? $file['name'][0] : $file['name'];

            // Validate file type and extension
            $check = wp_check_filetype_and_ext($fileTmpName, $fileName);

            if (empty($check['ext']) || empty($check['type']) || !in_array($check['type'], $validFileTypes)) {
                wc_add_notice(
                    wp_kses(
                        _translate_helloprint(
                            'Please upload a valid file type for this product. Valid file types are: jpg, jpeg, png, tiff, tif',
                            'helloprint'
                        ),
                        []
                    ),
            ...
            // File passed validation, process further if needed
            $files = $file;
        }
        ...
        if (!empty($uploaded_files)) {
            ...
            foreach ($uploaded_files as $file) {
                ...
                // Prepare the query using placeholders and $wpdb->prepare() to avoid SQL injection
                $insertQuery = $wpdb->prepare(
    ...
    public function helloprint_product_attributes_shortcode()
    {
        ...
        if (!empty($product) && method_exists($product, "get_type") && $product->get_type() == 'helloprint_product') {
            ...
            $limit_sku_variant = get_post_meta($product->get_id(), 'helloprint_product_limit_variant_key', true);
            $product_external_id = get_post_meta($product->get_id(), 'helloprint_external_product_id', true);
            $args['product_external_id'] = $product_external_id;
            if ($limit_sku_variant) {
    ...
    public function helloprint_delete_artwork_of_order()
    {
        ...
        $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM $fileTableName WHERE id = %d", 
                $id
            )
        );
        ...
        foreach ($order->get_items() as $key => $item) {
            if ($item->get_id() == $item_id) {
                ...
                if (!empty($item['uploaded_files'])) {
                    $item['uploaded_files'] = [];
                    wc_update_order_item_meta($item_id, 'helloprint_product_setup', wp_json_encode($item));
                }
            }
        }
