...
class Obj {
  public function __call($name, $args) {
    ...
    trigger_error('Call to undefined method '.__CLASS__.'::' . esc_attr($name) . '()', E_USER_ERROR);
  }
}
 ...
 */
function random_0_1() {
  return (float)wp_rand() / (float)getrandmax();
}
...
class Marked {
  ...
  public function _marked($src, $opt = null, $callback = null) {
    ...
    if (gettype($src) !== 'string') {
      throw new Exception('marked(): input parameter is of type ' . esc_html(gettype($src)) . ', string expected');
    }
    if(!$src) {
      return $src;
    }
  ...
  public function __get($name) {
    ...
    trigger_error('Undefined property: ' . __CLASS__ . '::$' . esc_attr($name), E_USER_NOTICE);
  }
}
