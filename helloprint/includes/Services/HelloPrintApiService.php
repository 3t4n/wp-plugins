...
class HelloPrintApiService
{
    ...
    public function post($url, $params, $contentType = 'application/json')
    {
        ...
        $response =  wp_remote_post($this->base_url . $url, [
            'timeout' => 45,
            'headers' => $headers,
            'body' => ($contentType == 'application/json') ? \wp_json_encode($params) : $params,
            'method' => 'POST',
            'data_format' => 'body'
        ]);
        $this->updateLangFromResponse($response);
                                            ...
                                            foreach ($quoteDetails as $itemKey => $item) {
                                                if ($item['serviceLevel'] && !empty($item['serviceLevel']) && $item['serviceLevel'] != null) {
                                                    ...
                                                    if (isset($item['times']) && !empty($item['times'])) {
                                                        ...
                                                        $api_delivery_date = $item["times"]["minDeliveryDate"];
                                                        $delivery_date = __helloprint_add_working_days($delivery_buffer_days, $api_delivery_date);
                                                        $serviceLabelObj = [
                                                            'label' => wp_kses(_translate_helloprint(ucwords($item['serviceLevel']), "helloprint"), true) . ' - ' . wp_kses($delivery_date, true),
    ...
    public function createOrder($order)
    {
        ...
        foreach ($order->get_items() as $key => $item) {
            ...
            if (empty($lineItemPreset) && !empty($sku)) {
                $lineItemPreset = $wpdb->get_results(
                    $wpdb->prepare(
                        "SELECT $preset_tableName.* 
                        FROM $preset_tableName 
                        WHERE helloprint_item_sku = %s",
                        $sku  // %s for string (sku)
                    )
                );
                $lineItemPreset = ($lineItemPreset[0]) ?? [];
            ...
            if ( ! WP_Filesystem() ) {
                // Handle error if WP_Filesystem initialization fails
                return;
            }
            $homePath = ABSPATH;
            $baseFolder = $homePath . 'wp-content/uploads/print-tmp/';
            if ( ! $wp_filesystem->is_dir( $baseFolder ) ) {
                $wp_filesystem->mkdir( $baseFolder, 0755 ); // Creates the directory with permissions 0755
            ...
            if (empty($lineItemPreset) && !empty($wphp_product_id) && $is_preset_artwork) {
                $hpPresets = $wpdb->get_results(
                    $wpdb->prepare(
                        "SELECT * FROM $preset_tableName WHERE product_type = %s AND helloprint_product_id = %s", 
                        'hp',  // 'hp' is a static value, so you pass it directly
                        $wphp_product_id  // %s is for strings
                    )
                );
                if (!empty($hpPresets)) {
            ...
            $publicFiles = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM $line_item_public_file_tableName WHERE line_item_id = %d", 
                    $itemId  // %d for integers
                )
            );
        ...
        if (true === WP_DEBUG) {
            error_log("order Items details :: " . \wp_json_encode($orderItems));
            error_log("shipping details :: " . \wp_json_encode($shippingDetails));
        ...
        if (true === WP_DEBUG) {
            error_log("Final Order Request :: " . \wp_json_encode($order_array));
        }
