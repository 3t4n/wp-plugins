...
$show_product_variant_key = (isset($args["show_variant_key_in_product"]) && $args["show_variant_key_in_product"] == true) ? true : false;
?>
    ...
    <form class="wphp-product-cart" action="<?php echo esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())); ?>" method="post" enctype='multipart/form-data'>
        <div cellspacing="0">
            <div>
                <div style="display:none;" class="wphp-grid wphp-hidden-product-key">
                    <div>
                        <label for="wphp_product_product_key">
                            <?php echo wp_kses(_translate_helloprint('Product Key', 'helloprint'), true) ?>
                        </label>
                    </div>
                    <div align="right">
                        <span class="wphp_product_product_key"><?php echo wp_kses($args["product_external_id"], false);?></span>
                    </div>
                </div>
                <?php 
                ...
                <?php foreach ($args['product_attributes'] as $key => $attribute)  : ?>
                    ...
                    if ($switch_icon):
                        if ('size' === $attribute['id']) :
                            $do_have_size_icon = false;
                            if (!empty($attribute['options'])) {
                                foreach ($attribute["options"] as $sOption) {
                                    if (!empty($sOption["image"])) {
                                        $do_have_size_icon = true;
                                        break;
                                    }
                                }
                            }
                            $has_icon_switch = false;
                            if ($do_have_size_icon):
                    ...
                    if($show_color_icons) : ?>
                        <?php if('colours' == $attribute['id']) :
                            $do_have_color_icon = false;
                            if (!empty($attribute['options'])) {
                                foreach ($attribute["options"] as $sOption) {
                                    if (!empty($sOption["image"])) {
                                        $do_have_color_icon = true;
                                        break;
                                    }
                                }
                            }
                            $has_colour_icon_switch = false;
                            if($do_have_color_icon):
                    ...
                    if($show_print_position_icons) : ?>
                        <?php if('printposition' == $attribute['id']) :
                            ...
                            $do_have_print_icon = false;
                            if (!empty($attribute['options'])) {
                                foreach ($attribute["options"] as $sOption) {
                                    if (!empty($sOption["image"])) {
                                        $do_have_print_icon = true;
                                        break;
                                    }
                                }
                            }

                            if($do_have_print_icon):
                            include("inc/print-icons.php");
                            ?>
                            <?php $data_position++; 
                            else:
                                $show_print_position_icons = false;
                                endif;
                            ?>
                        <?php endif ?>
                    <?php endif ?>
                    ...
                    <?php if($enable_product_design) : ?>
                        <div class="wphp-add-design">
                            ...
                            <div>
                                ...
                                <label for="wphp_design"><?php echo wp_kses(_translate_helloprint('Yes please', 'helloprint'), true) ?> (<span class="wphp-design-price-label"><?php echo wp_kses(wc_price($graphic_design_price), true) ?></span><span class="wphp-design-incl-tax-price-label"></span>)</label>
                                </span>
                            </div>
                            <div>
                                <input checked class="wphp_design" type="radio" name="wphp_design" value="0">
                                <label for="wphp_design"><?php echo wp_kses(_translate_helloprint("No thank you, i'll supply print ready artwork.", 'helloprint'), true) ?>(<span class="wphp-no-design-price-label"><?php echo wp_kses(wc_price(0), true) ?></span>)</label>
                                </span>
                            </div>
                            <hr>
                        </div>
                ...
                <input type="hidden" id="wphp_select_one" value='<?php echo wp_kses(wp_strip_all_tags(wp_kses(_translate_helloprint('Select One', 'helloprint'), false)), false);?>' />
