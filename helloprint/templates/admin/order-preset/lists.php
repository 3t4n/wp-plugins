...
<div class="wrap">
    ...
    <form id="posts-filter" method="get">
        ...
        <p class="search-box">
            ...
            <input type="hidden" name="page" value="<?php echo !empty($_GET['page']) ? esc_attr(sanitize_text_field(wp_unslash($_GET['page']))) : 1;?>" />
                        ...
                        <tbody>
                            <?php foreach($order_presets as $key => $op): ?>
                                <tr class="<?php echo (0 === $key%2) ? 'even' : 'odd' ;?>">
                                    ...
                                    <td><?php echo ($op->product_type == "non_hp") ? wp_kses(_translate_helloprint('Non-Helloprint', "helloprint"), true) : wp_kses(_translate_helloprint('Helloprint', "helloprint"), true) . " (" . esc_attr($op->helloprint_product_id) . ")"; ?></td>
                                    ...
                                    <td>
                                        <?php if(!empty($op->file_url)):
                                            ...
                                            if (!empty($full_url)):
                                            ?>
                                            <a target="_blank" href="<?php echo  wp_kses($full_url, false);?>" class="dashicons-before dashicons-download btn btn-sm btn-outline-primary"><?php echo wp_kses(_translate_helloprint("Download", "helloprint"), true);?></a>
                                        <?php 
                                            endif;
                                            endif;?>
