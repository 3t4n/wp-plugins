...
class OrderController extends BaseController
{
    ...
    public function update_order_status()
    {
        try {
            ...
            if (true === WP_DEBUG) {
                error_log("Helloprint APi callback data :: " . \wp_json_encode($data));
            }
            ...
            if ($data['data']['status'] == 'ORDER_CREATED') {
                ...
                $order_status = $order->get_meta('helloprint_order_status', true);
                $logger->info("Order Created :: " . \wp_json_encode($order_status), $context);
                ...
                $order_status = $order->get_meta('helloprint_order_status', true);
                $logger->info("Shipped :: " . \wp_json_encode($order_status), $context);
                return wp_send_json_success($order_status);
            } else {
                ...
                if (isset($data['data']['status']) && isset($data['data']['orderId'])) {
                    ...
                    $order_status = $order->get_meta('helloprint_order_status', true);
                    $logger->info(\wp_json_encode($order_status), $context);
                    return wp_send_json_success($order_status);
                } else {
                    if (!empty($order_status)) {
                        update_post_meta($order_id, 'helloprint_order_status', [
                    ...
                    $order = wc_get_order($order_id);
                    $order_status = $order->get_meta('helloprint_order_status', true);
                    $logger->info("Failed :: " . \wp_json_encode($order_status), $context);
                    return wp_send_json_success($order_status);
                }
            }
    ...
    public function remove_helloprint_order_file()
    {
        ...
        $td = wc_update_order_item_meta($item_id, 'helloprint_product_setup', wp_json_encode($custom_field));
                        ...
                        $uploads_files[] = [
                            ...
                            if (!empty($file_url)) {
                                $insertQuery = $wpdb->prepare(
                                    "INSERT INTO $fileUploadTable (order_id, line_item_id, file_url) VALUES (%d, %d, %s)",
                                    $order_id,
                                    $item_id,
                                    $file_url
                                );
    ...
    */
    ...
    public function add_order_item()
    {
        ...
        if (
            ...
            if (!empty($product) && $product->get_type() == 'helloprint_product') {
                $customOptions = sanitize_text_field(wp_unslash(wp_json_encode($_POST['helloprint_product_custom_options'])));
               
                $apprealSizeOptions = sanitize_text_field(wp_unslash(wp_json_encode($_POST['helloprint_appreal_size_options'])));
                $order_notes = [];
                $added_items = [];
                $uploaded_file = [];
                ...
                $item->add_meta_data('helloprint_product_setup', wp_json_encode($helloprint_product_setup));
                ...
                $order->add_order_note(sprintf(_translate_helloprint('Added line items: %s', 'helloprint'), implode(', ', $order_notes)), false, true);
                $order->save();
        ...
        if (!empty($_POST['order_item_preset'])) {
            ...
            foreach ($order_presets as $id => $preset) {
                ...
                if (!empty($preset) && $preset != 0) {
                    $insertQuery = $wpdb->prepare(
                        "INSERT INTO {$tableName} (order_id, line_item_id, preset_id, service_level, quantity) VALUES (%d, %d, %s, %s, %d)",
                        $order_id,
                        $id,
                        $preset,
                        $slevel,
    ...
    private function load_preset_files($wpdb, $sku, $item_id, $order_id, $helloprint_order_id, $is_hp_product = false)
    {
        ...
        $product_type = "non_hp";
        $query = $wpdb->prepare(
            "SELECT id, order_preset_name, helloprint_item_sku, helloprint_variant_key, default_service_level, default_quantity, file_name, file_url
            FROM {$tableName} 
            WHERE (helloprint_item_sku = %s OR helloprint_item_sku = '') 
            AND product_type = %s 
            ORDER BY id DESC",
            $sku,
            $product_type
        );
        $allavailablePresets = $wpdb->get_results(
             // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            $query
        );
        ...
        $query = $wpdb->prepare(
            "SELECT * FROM {$presetPublicFileTable} WHERE line_item_id = %d",
            $item_id
        );
        $presetPublicFiles = $wpdb->get_results(
             // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            $query
        );
        ...
        if ($is_hp_product) {
            ...
            $query = $wpdb->prepare(
                ...
                AND helloprint_product_id = %s",
                'hp', // hardcoded 'hp' value for product_type
                $wphp_product_id // variable for helloprint_product_id
            );
            $all_hp_presets = $wpdb->get_results(
                 // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                $query
            );
