...
class BulkProductController extends BaseController
{
            ...
            if (count($_POST['products']) > $total_duplicates) {
                foreach (wp_unslash($_POST['products']) as $product) {
                    ...
                    if (!in_array($product_slug, $existing_hp_slugs)) {
                        $existingcount = $wpdb->get_var(
                            $wpdb->prepare(
                                "SELECT id FROM $bulkImportQueueTable WHERE category = %s AND product_id = %s AND date = %s AND imported = %d",
                                $category,
                                $product_slug,
                                $date,
                                0 // Ensure 'imported' is treated as a numeric value, which is '0'
                            )
                        );
                        ...
                        if (empty($existingcount)) {
                            $return_id = $wpdb->query(
                                $wpdb->prepare(
                                    "INSERT INTO $bulkImportQueueTable (category, product_id, date) VALUES (%s, %s, %s)",
                                    $category,
                                    $product_slug,
                                    $date
                                )
    ...
    public function async_import_helloprint_products($product, $category, $margin, $marginOption, $msg)
    {
        ...
        $attempts = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT attempts FROM $bulkImportQueueTable WHERE category = %s AND product_id = %s AND date = %s AND imported = '0'",
                $category,
                $product_slug,
                $date
            )
        ...
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE $bulkImportQueueTable SET `attempts` = (`attempts` + 1) WHERE category = %s AND product_id = %s AND date = %s AND imported = '0'",
                $category,
                $product_slug,
                $date
            )
    ...
    private function import_single_product($product, $category, $margin, $marginOption)
    {
        ...
        if (!empty($products[0])) {
            ...
            if ($image_id > 0) {
                $wpdb->query(
                    $wpdb->prepare(
                        "UPDATE $bulkImportQueueTable SET `imported` = 1 WHERE category = %s AND product_id = %s",
                        $category,
                        $product_slug
                    )
                );
        ...
        if (!empty($productDetails['preview_image'])) {
            ...
            $wpdb->query(
                $wpdb->prepare(
                    "UPDATE $bulkImportQueueTable SET `imported` = 1 WHERE category = %s AND product_id = %s AND date = %s",
                    $category,
                    $product_slug,
                    $date
                )
    ...
    private function save_image_for_import($image_url = '', $post_id = null)
    {
        ...
        if (strlen($image_data) > $max_size) {
            return; // Stop if image exceeds size limit
        }
    ...
    private function _get_existing_hp_slugs($product_slugs, $category)
    {
        ...
        $meta_key = 'helloprint_external_product_id';  // Define the meta_key explicitly
        $query = "
            SELECT meta_value
            FROM $meta_key_table
            WHERE meta_key = %s 
            AND post_id IN ($id_strings)
        ";

        // Use $wpdb->prepare() to sanitize the query
        $existing_hp_slugs = $wpdb->get_results(
            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            $wpdb->prepare($query, $meta_key), ARRAY_A
        );
    ...
    public function check_remaining_imports()
    {
        try {
            ...
            // Sanitize job_ids to ensure they are integers
            $job_ids_array = array_map('intval', explode(',', $job_ids));  // Split job_ids into an array and sanitize each ID

            // Prepare the placeholders for the IN clause
            $placeholders = implode(',', array_fill(0, count($job_ids_array), '%d'));

            // Query for completed jobs
            $completed_jobs_query = "
                SELECT *
                FROM $bulk_import_queue_table
                WHERE imported = %s
                AND id IN ($placeholders)
            ";
            $completed_jobs = $wpdb->get_results(
                // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                $wpdb->prepare($completed_jobs_query, '1', ...$job_ids_array),
                ARRAY_A
            );
            ...
            $failed_jobs = $wpdb->get_results(
                // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                $wpdb->prepare($failed_jobs_query, '0', 2, ...$job_ids_array),
                ARRAY_A
            );
