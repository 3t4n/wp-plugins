...
function p3dlite_activate() {
	...
	if (version_compare($current_version, '1.9.1.5', '<')) {
		p3dlite_clean_uploads();
	}

	if ( !file_exists( $upload_dir['basedir'].'/p3d/index.html' ) || version_compare($current_version, '1.9.1.5', '<')) {
...
<ifmodule mod_headers.c>
	...
	';
	if ( !file_exists( $upload_dir['basedir'].'/p3d/.htaccess' ) || version_compare($current_version, '1.9.1.5', '<') ) {
...
function p3dlite_check_install() {
	...
	$sql = "CREATE TABLE ".$wpdb->prefix."p3dlite_printers (
		  ...
		  default_infill varchar(8) DEFAULT '20' NULL,
...
//echo $wpdb->last_error;
	...
	$sql = "CREATE TABLE ".$wpdb->prefix."p3dlite_infills (
		  id mediumint(9) NOT NULL AUTO_INCREMENT,
		  status tinyint(1) DEFAULT '1' NOT NULL,
		  name varchar(64) DEFAULT '' NOT NULL,
		  description text DEFAULT '' NOT NULL,
		  photo varchar(2048) DEFAULT '' NOT NULL,
		  infill float DEFAULT '100' NOT NULL,
		  price varchar(256) DEFAULT '0' NOT NULL,
		  price_type varchar(32) DEFAULT 'pct' NOT NULL,
		  price1 varchar(256) DEFAULT '0' NOT NULL,
		  price_type1 varchar(32) DEFAULT 'pct' NOT NULL,
		  PRIMARY KEY id (id)
		) $charset_collate;";
	...
	$cols = $wpdb->get_col("SELECT * FROM ".$wpdb->prefix."p3dlite_infills LIMIT 1");

	if ( empty($cols) ){
		$default_infills[]=array(
			'status' => 1,
			'name' => '0%',
			'description' => '',
			'photo' => '',
		...
		foreach ($default_infills as $infill) {
			$wpdb->insert( $wpdb->prefix."p3dlite_infills", $infill );
		}
	...
	$sql = "CREATE TABLE ".$wpdb->prefix."p3dlite_price_requests (
		  ...
		  admin_attachment varchar(1024) DEFAULT '' NOT NULL,
		  admin_attachment_url text DEFAULT '' NOT NULL,
		  admin_delivery varchar(256) DEFAULT '' NOT NULL,
...
*/
	...
	$settings=array(
		...
		'price_debug_mode' => (isset($current_settings['price_debug_mode']) ? $current_settings['price_debug_mode'] : ''),
		...
		'tooltip_engine' => (isset($current_settings['tooltip_engine']) ? $current_settings['tooltip_engine'] : 'tippy'),
		'tooltip_theme' => (isset($current_settings['tooltip_theme']) ? $current_settings['tooltip_theme'] : 'light'),
...
'.esc_html__('E-mail','3dprint-lite').': [customer_email] <br>
'.esc_html__('Model','3dprint-lite').': [model_file]  <br>
'.esc_html__('Quantity','3dprint-lite').': [quantity]  <br>
'.esc_html__('Printer','3dprint-lite').': [printer_name] <br>
'.esc_html__('Material','3dprint-lite').': [material_name] <br>
'.esc_html__('Coating','3dprint-lite').': [coating_name] <br>
'.esc_html__('Infill','3dprint-lite').': [infill_name] <br>
'.esc_html__('Unit','3dprint-lite').': [unit] <br>
'.esc_html__('Resize Scale','3dprint-lite').': [resize_scale] <br>
'.esc_html__('Dimensions','3dprint-lite').': [dimensions] <br>
'.esc_html__('Estimated Unit Price','3dprint-lite').': [estimated_price] <br>
'.esc_html__('Comments','3dprint-lite').': [customer_comments] <br>
'.esc_html__('Manage Price Requests','3dprint-lite').': [price_requests_link] <br>
';
	}
...
'.esc_html__('Infill','3dprint-lite').': [infill_name] <br>
'.esc_html__('Dimensions','3dprint-lite').': [dimensions] <br>
'.esc_html__('Unit Price','3dprint-lite').': [price] <br>
'.esc_html__('Total Price','3dprint-lite').': [price_total] <br>
'.esc_html__('Comments','3dprint-lite').': [admin_comments] <br>
';
	}
...
function p3dlite_clean_uploads() {
	$uploads = wp_upload_dir();
	$files = glob($uploads['basedir']."/p3d/*");

	foreach ($files as $file) {
		$filename = basename($file);
		$extension = p3dlite_extension($file);
		if (is_file($file) && $filename != '.htaccess' && $filename != 'index.html') {
			if (!in_array($extension, array_merge(p3dlite_get_accepted_models(), p3dlite_get_support_extensions_inside_archive()))) {
				unlink($file);
			}
		}
	}
...
#	}
	...
	if ( $condition ) {
		...
		if ($settings['tooltip_engine']=='tooltipster') {
		...
		else if ($settings['tooltip_engine']=='tippy') {
			$tooltip_engine = 'tippy.js';
			wp_enqueue_script( 'popper.js',  plugin_dir_url( __FILE__ ).'ext/tippy/popper.js', array( 'jquery' ), $p3d_current_version );
			wp_enqueue_script( 'tippy.js',  plugin_dir_url( __FILE__ ).'ext/tippy/tippy.min.js', array( 'jquery', 'popper.js' ), $p3d_current_version );
			wp_enqueue_style( 'tippy-light.css', plugin_dir_url( __FILE__ ).'ext/tippy/tippy-light.css', array(), $p3d_current_version );

		}

		wp_enqueue_style( 'stellarnav.css', plugin_dir_url( __FILE__ ).'ext/stellarnav/css/stellarnav.css', array(), $p3d_current_version );
		...
		wp_enqueue_script( 'stellarnav.js',  plugin_dir_url( __FILE__ ).'ext/stellarnav/js/stellarnav.js', array( 'jquery' ), $p3d_current_version );
		...
		wp_localize_script( '3dprint-lite-frontend.js', 'p3dlite',
			array(
				...
				'error_box_fit' => wp_kses(__( '<span id=\'printer_fit_error\'><b>Error:</b> The model does not fit into the selected printer</span>', '3dprint-lite' ), 'post'),
				'warning_box_fit' => wp_kses(__( '<span id=\'printer_fit_warning\'><b>Warning:</b> The model might not fit into the selected printer</span>', '3dprint-lite' ), 'post'),
				'error_not_found' => __( '<span><b>Error:</b>Model not found</span>', '3dprint-lite' ),
				'warning_cant_triangulate' => wp_kses(__( '<b>Warning:</b> Can\'t triangulate', '3dprint-lite' ), 'post'),
				'text_coating' => esc_html__('Coating', '3dprint-lite'),
				'text_material' => esc_html__('Material', '3dprint-lite'),
				'text_printer' => esc_html__('Printer', '3dprint-lite'),
				'text_infill' => esc_html__('Infill', '3dprint-lite'),
				'text_fitting_printer' => __( "The model is too large, a fitting printer is selected.", '3dprint-lite' ),
#				'text_multiple_threejs' => __( "3DPrint Lite detected other 3D viewers loaded on this page! This may lead to conflicts!", '3dprint-lite' ),
				'pricing' => esc_html($settings['pricing']),
				'minimum_price_type' => esc_html($settings['minimum_price_type']),
				'price_debug_mode' => esc_html($settings['price_debug_mode']),
				'background1' => esc_html(str_replace( '#', '0x', $settings['background1'] )),
				'plane_color' => esc_html(str_replace( '#', '0x', $settings['plane_color'] )),
				'printer_color' => esc_html(str_replace( '#', '0x', $settings['printer_color'] )),
				'show_upload_button' => esc_html($settings['show_upload_button']),
				'show_grid' => esc_html($settings['show_grid']),
				'show_printer_box' => esc_html($settings['show_printer_box']),
				'file_max_size' => esc_html($settings['file_max_size']),
				'file_chunk_size' => esc_html($settings['file_chunk_size']),
				'file_extensions' => esc_html($settings['file_extensions']),
				'files_to_convert' => array('zip'),
				'currency_symbol' => esc_html($settings['currency']),
				'currency_position' => esc_html($settings['currency_position']),
				'selection_order' => esc_html($settings['selection_order']),
				'price_num_decimals' => esc_html($settings['num_decimals']),
				'thousand_sep' => esc_html($settings['thousand_sep']),
				'decimal_sep' => esc_html($settings['decimal_sep']),
				'min_price' =>  esc_html($settings['min_price']),
				'cookie_expire' => esc_html($settings['cookie_expire']),
				'auto_rotation' => esc_html($settings['auto_rotation']),
				'auto_rotation_speed' => $settings['auto_rotation_speed'],
				'auto_rotation_direction' => $settings['auto_rotation_direction'],
				'auto_scale' => esc_html($settings['auto_scale']),
				'resize_on_scale' => esc_html($settings['resize_on_scale']),
				'fit_on_resize' => esc_html($settings['fit_on_resize']),
				'shading' => esc_html($settings['shading']),
				'show_shadow' => esc_html($settings['show_shadow']),
				'ground_mirror' => esc_html($settings['ground_mirror']),
				'ground_color' => esc_html(str_replace( '#', '0x', $settings['ground_color'] )),
				'tooltip_engine' => esc_html($settings['tooltip_engine']),
				'tooltip_theme' => esc_html($settings['tooltip_theme']),
				'nonce' => wp_create_nonce('ajax-nonce')
			)
		);
		$custom_css = "
			.progress-button[data-perspective] .content { 
			 	background: ".esc_html($settings['button_color1'])."; 
			}

			.progress-button .progress { 
				background: ".esc_html($settings['button_color2'])."; 
			}

			.progress-button .progress-inner { 
				background: ".esc_html($settings['button_color3'])."; 
			}
			.progress-button {
				color: ".esc_html($settings['button_color4']).";
			}
			.progress-button .content::before,
			.progress-button .content::after  {
				color: ".esc_html($settings['button_color5']).";
			}
		";
...
function p3dlite_unassigned_warning() {
	...
	if ($unassigned_materials && count($unassigned_materials) > 0) {
		$message = sprintf(esc_html__( 'You have %s unassigned materials. They will not be displayed on the frontend.', '3dprint-lite' ), count($unassigned_materials));
		printf( '<div class="%1$s"><b>3DPrint</b><p>%2$s</p></div>', esc_html($class), esc_html($message) ); 
	}
}
...
function p3dlite_get_option ($option, $pagination=false) {
	...
	if(!is_null($wpdb->get_var("SHOW TABLES LIKE '".$wpdb->prefix."$option'")) && strtolower($wpdb->get_var("SHOW TABLES LIKE '".$wpdb->prefix."$option'")) == strtolower($wpdb->prefix.$option)) {
...
function p3dlite_save_thumbnail( $data, $filename ) {
	...
	if ( !empty($data) ) {
		...
		$link = esc_url($upload_dir['baseurl'].'/p3d/'.$new_filename);
	}
	return $link;
}
...
function p3dlite_format_price($price, $currency, $currency_position='left') {
	...
	if ($currency_position=='left') {
		$formatted_price=$currency.number_format_i18n($price, $settings['num_decimals']);
	}
	elseif ($currency_position=='left_space') {
		$formatted_price=$currency.' '.number_format_i18n($price, $settings['num_decimals']);
	}
	elseif ($currency_position=='right') {
		$formatted_price=number_format_i18n($price, $settings['num_decimals']).$currency;
	}
...
function p3dlite_fix_price ($price) {	
...
function p3dlite_get_enabled ($array) {
...
function p3dlite_sanitize_templates($templates) {
	$templates['admin_email_from']=sanitize_email($templates['admin_email_from']);
	$templates['admin_email_subject']=sanitize_text_field($templates['admin_email_subject']);
	$templates['admin_email_body']=wp_kses_post($templates['admin_email_body']);

	$templates['client_email_from']=sanitize_email($templates['client_email_from']);
	$templates['client_email_subject']=sanitize_text_field($templates['client_email_subject']);
	$templates['client_email_body']=wp_kses_post($templates['client_email_body']);
...
function p3dlite_handle_upload() {
	...
	if ( isset( $_REQUEST["name"] ) ) {
		...
		$fileName = sanitize_file_name(uniqid( "file_" ));
	}

#	$fileName = sanitize_file_name( $fileName );
	$filePath = $targetDir . DIRECTORY_SEPARATOR . $fileName;
	$uploadable_files = p3dlite_get_accepted_models();
	$uploadable_files[] = 'zip';

	if (!in_array(p3dlite_extension($fileName), $uploadable_files)) {
		die( '{"jsonrpc" : "2.0", "error" : {"code": 99, "message": "'.esc_html__( "Wrong file extension.", '3dprint-lite' ).'"}, "id" : "id"}' );
	}
	...
	if ( $cleanupTargetDir ) {
		if ( !is_dir( $targetDir ) || !$dir = opendir( $targetDir ) ) {
			die( '{"jsonrpc" : "2.0", "error" : {"code": 100, "message": "'.esc_html__( "Failed to open temp directory.", '3dprint-lite' ).'"}, "id" : "id"}' );
		}
	...
	if ( !$out = @fopen( "{$filePath}.part", $chunks ? "ab" : "wb" ) ) {
		die( '{"jsonrpc" : "2.0", "error" : {"code": 102, "message": "'.esc_html__( 'Failed to open output stream.', '3dprint-lite' ).'"}, "id" : "id"}' );
	}

	if ( !empty( $_FILES ) ) {
		if ( $_FILES["file"]["error"] || !is_uploaded_file( $_FILES["file"]["tmp_name"] ) ) {
			die( '{"jsonrpc" : "2.0", "error" : {"code": 103, "message": "'.esc_html__( 'Failed to move uploaded file. Error code: '.(int)$_FILES["file"]["error"], '3dprint-lite' ).'"}, "id" : "id"}' );
		}

		// Read binary input stream and append it to temp file
		if ( !$in = @fopen( $_FILES["file"]["tmp_name"], "rb" ) ) {
			die( '{"jsonrpc" : "2.0", "error" : {"code": 101, "message": "'.esc_html__( 'Failed to open input stream.', '3dprint-lite' ).'"}, "id" : "id"}' );
		}
	} else {
	...
	if ( !$chunks || $chunk == $chunks - 1 ) {
		...
		rename( "{$filePath}.part", $filePath );
                $original_filename = p3dlite_basename($filePath);

		$uploads = wp_upload_dir();
		$uploads['path'] = $uploads['basedir'].'/p3d/';
		$time = time();
		$wp_filename =  $time.'_'.rawurlencode( sanitize_file_name(p3dlite_basename( $filePath ) ) );
		$wp_filename = apply_filters( '3dprint-lite_filename', $wp_filename, $original_filename );
		...
		if ($extension=='zip') {
			if (class_exists('ZipArchive')) {
				...
				if ( $res === TRUE ) {
					...
					$zip->close();
					if ( !$file_found ) {
						die( '{"jsonrpc" : "2.0", "error" : {"code": 104, "message": "'.esc_html__( 'Model file not found.', '3dprint-lite' ).'"}, "id" : "id"}' );
					}
					rename($filePath, $uploads['path'].$wp_filename.'.zip');
				}
				else {
					die( '{"jsonrpc" : "2.0", "error" : {"code": 105, "message": "'.esc_html__( 'Could not extract the file.', '3dprint-lite' ).'"}, "id" : "id"}' );
				}
			}
			else {
				die( '{"jsonrpc" : "2.0", "error" : {"code": 106, "message": "'.esc_html__( 'The server does not support zip archives.', '3dprint-lite' ).'"}, "id" : "id"}' );
			}
		} 
		...
		if (filesize($uploads['path'].$wp_filename) > ((int)$settings['file_max_size'] * 1048576)) {
			...
			die( '{"jsonrpc" : "2.0", "error" : {"code": 113, "message": "'.esc_html__( 'Extracted file is too large.', '3dprint-lite' ).'"}, "id" : "id"}' );
		}
