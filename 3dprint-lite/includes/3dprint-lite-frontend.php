...
#		}
		...
		$db_infills=$infills_array=p3dlite_get_option( 'p3dlite_infills' );
...
#			$p3dlite_price_request['base64_filename'] = base64_encode( p3dlite_basename( $model_file ));
			...
			$replace_from = array('[customer_email]','[quantity]', '[printer_name]','[material_name]','[coating_name]','[infill]','[infill_name]','[model_file]','[unit]','[resize_scale]','[dimensions]','[estimated_price]','[estimated_price_total]', '[customer_comments]','[price_requests_link]');
			$replace_to = array($email_address, $quantity, $db_printers[$printer_id]['name'], $db_materials[$material_id]['name'], 
				   (isset($db_coatings[$coating_id]['name']) ? $db_coatings[$coating_id]['name'] : ''),  $infill, $infill_name, $link, $unit, $scale, $dimensions,
