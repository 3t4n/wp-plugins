    ...
    function mptsi_custom_header_script()
    {
        ...
        if (
            ...
            if (empty($api_data_value)) {
                ...
                // Secure API call
                $api_url = "https://pro.ip-api.com/json/" . urlencode($ip) . "?key=4UJdOf3YFJPAAue";
                ...
                if (is_wp_error($response)) {
                    ...
                    $ipInfo = json_decode($body);

                    if (!empty($ipInfo) && isset($ipInfo->status) && $ipInfo->status === "success") {
                        $timezone = sanitize_text_field($ipInfo->timezone);
                    } else {
                        $timezone = "America/Chicago"; // Fallback
                    }
                }
                ...
                $newdate = strtotime("+6 days", strtotime($today_date));
                $next_date = date("Y-m-d", $newdate);

                $masjid_id = get_option("masjid_id");

                // Secure API URL
                $url = esc_url_raw("https://masjidal.com/api/v2/masjids/{$masjid_id}?expand=times&date_start={$today_date}&date_end={$next_date}");
...
class MPSTI_wpb_widget extends WP_Widget
{
    ...
    {
      parent::__construct(
        "MPSTI_wpb_widget", // Widget ID

        // Widget name will appear in UI
        esc_html__("Current Day Masjid Timing", "masjidal"),
	...
	if (!empty($title)) {
        ...
        // Sanitize and retrieve user's IP address
$ip = sanitize_text_field($_SERVER["REMOTE_ADDR"]);
$api_url = "https://pro.ip-api.com/json/" . urlencode($ip) . "?key=4UJdOf3YFJPAAue";

// Secure API call to fetch timezone data
$response = wp_safe_remote_get($api_url);
$timezone = "America/Chicago"; // Default fallback timezone
...
// Securely set date and timezone
try {
    $date = new DateTime("now", new DateTimeZone($timezone));
} catch (Exception $e) {
    $date = new DateTime("now", new DateTimeZone("America/Chicago"));
...
// Get today's date and calculate next 6 days
$today_date = $date->format("Y-m-d");
$date->modify("+6 days");
...
$results = [];
if (!is_wp_error($file) && !empty($file["body"])) {
    $decoded_data = json_decode($file["body"]);
    if ($decoded_data) {
        $results = $decoded_data->times ?? [];
    }
}
//echo_log($results);
...
$newArray = isset($results->times) ? $results->times : [];
...
if (!empty($newArray)) {
    ...
    foreach ($newArray as $item) {
        $today_view_date = date("l, M d", strtotime($item->date));
        ...
        // Get options securely
        $timeformat_24 = get_option("timeformat_24");
        $color = esc_attr(get_option("highlighted_color"));
        $text_color = esc_attr(get_option("highlighted_text_color"));

        // Compute active class based on current time
        $iqamah_times = [
            "fajr" => strtotime($iqamah->fajr),
            "zuhr" => strtotime($iqamah->zuhr),
            "asr" => strtotime($iqamah->asr),
            "maghrib" => strtotime($iqamah->maghrib),
            "isha" => strtotime($iqamah->isha)
        ];

        $active_class = "";
        foreach ($iqamah_times as $key => $time) {
            if ($current_time < $time) {
                $active_class = "{$key}_class";
                break;
            }
        }

        echo "<style>
        .mySlides.count_1 li.active_new {
            background-color: {$color} !important;
            color: {$text_color} !important;
        }
        .mySlides.count_1 li.active_new h2 {
            color: {$text_color} !important;
        }
        .table_new.mySlides_new.count_1 tr.active_new td {
            background-color: {$color} !important;
            color: {$text_color} !important;
        }
        </style>";

        echo '<div class="mySlides fade count_' . esc_attr($counts) . '">
                <div class="main-center">
                    <div class="inner-center-back">
                        <div style="margin-bottom: 7px;">' . esc_html__("Prayer Timings", "masjidal") . '</div>
                        <div style="font-weight: normal; font-size: 14px;">' . esc_html($today_view_date) . '</div>
                        <div id="hijri-date_new">' . esc_html($item->hijri_month) . " " . esc_html($item->hijri_date) . '</div>
                    </div>
                </div>
              </div>';
        ...
        foreach (["fajr", "zuhr", "asr", "maghrib", "isha"] as $prayer) {
            $formatted_times[$prayer] = ($timeformat_24 == "yes")
                ? date("H:i", strtotime($iqamah->$prayer))
                : date("g:i A", strtotime($iqamah->$prayer));
        }

        echo "<ul>";
        echo "<li>
                <div class='time_namze_heading'>
                    <span class='text-right start_time'>" . esc_html__(get_option("starts_lable", "STARTS"), 'text-domain') . "</span>
                    <span class='text-center' style='font-weight: normal; border: none;'>" . esc_html__(get_option("iqamah_lable", "IQAMAH"), 'text-domain') . "</span>
                </div>
              </li>";

        foreach (["fajr", "zuhr", "asr", "maghrib", "isha"] as $prayer) {
            $prayer_label = esc_html__(get_option("{$prayer}_lable", strtoupper($prayer)), 'text-domain');
            $prayer_icon = esc_url(plugin_dir_url(dirname(dirname(__FILE__)))) . "public/assets/images/d_{$prayer}.png";

            echo "<li class='" . esc_attr($active_class == "{$prayer}_class" ? "active_new" : "") . "' style='padding: 15px 15px;'>
                    <div class='image_and_text_namze'>
                        <img src='{$prayer_icon}' alt=''>
                        <span class='namze_name'> {$prayer_label} </span>
                    </div>
                    <div class='time_namze'>
                        <span>" . esc_html($formatted_times[$prayer]) . "</span>
                        <span class='text-center'>" . esc_html($formatted_times[$prayer]) . "</span>
                    </div>
                  </li>";
        }
        ...
        if (!empty($iqamah->jummah1) && $iqamah->jummah1 !== "-") {
            $jummah1 = esc_html($iqamah->jummah1);
            echo "<div class='jamu-sec'>
                    <div class='col-6 text-center'><h1>{$jummah1}</h1><span>" . esc_html__(get_option("jumuah1_lable", "JUMU'AH"), "text-domain") . "</span></div>
                  </div>";
        }

        echo '<div class="am-pm">
                <div class="col-6 text-left">
                    <span>
                        <img src="' . esc_url(plugin_dir_url(dirname(dirname(__FILE__)))) . 'public/assets/images/am-icon.jpg" alt="">
                        ' . esc_html($formatted_times["fajr"]) . '
                    </span>
                </div>
                <div class="col-6 text-right">
                    <span>
                        ' . esc_html($formatted_times["maghrib"]) . '
                        <img src="' . esc_url(plugin_dir_url(dirname(dirname(__FILE__)))) . 'public/assets/images/sunset.png" alt="" style="margin: 0 0 0 10px">
                    </span>
                </div>
              </div>';

        echo '</div>'; // Closing mySlides div
        $counts++;
    }

    echo '</div>'; // Closing slideshow-container div
}

$masjid_calendar_type = get_option("masjid_calendar_type");
$masjid_id = get_option("masjid_id");

// Determine Monthly Calendar URL
switch ($masjid_calendar_type) {
    case "Custom_url":
    ...
    case "v2":
    case "v3":
        $theme = substr($masjid_calendar_type, -1);
        $monthly_view = "https://timing.athanplus.com/masjid/widgets/monthly?masjid_id={$masjid_id}&theme={$theme}";
        break;
    default:
        $monthly_view = "https://timing.athanplus.com/masjid/widgets/monthly?masjid_id={$masjid_id}&theme=3"; // Default theme
        break;
}

// Display Monthly Calendar Link
if (!empty($monthly_view)) { ?>
    <a class="montyly_view" style="float: right;" target="_blank" href="<?php echo esc_url($monthly_view); ?>">
        <?php esc_html_e("View Monthly Calendar", "masjidal"); ?>
    </a>
<?php } ?>

<div class="prowred_by">
    <span>
        <?php esc_html_e("Powered by", "masjidal"); ?>
        <a href="<?php echo esc_url("http://www.masjidal.com/"); ?>">
            <?php esc_html_e("www.masjidal.com", "masjidal"); ?>
        </a>
    </span>
</div>
...
<div style="text-align:center">
    <?php $count = 1; ?>
    <?php foreach ($newArray as $alldate) { ?>
        <span class="dot" onclick="currentSlide(<?php echo esc_attr($count); ?>)"></span>
        <?php $count++; ?>
    <?php } ?>
</div>
...
?> public function form($instance) { if (isset($instance["title"])) { $title = $instance["title"]; } else { $title = __("New title", "masjidal"); } // Widget admin form ?> <p>
    <label for="<?php echo esc_attr($this->get_field_id("title")); ?>"> <?php esc_html_e("Title:", "masjidal"); ?> </label>
...
if (!function_exists("mptsi_single_view_calendar")) {
    function mptsi_single_view_calendar()
    {
		...
		if (is_countable($newArray) && count($newArray) > 0) {
			//echo_log($newArray[0]);
			...
			if ($masjid_calendar_layout == "Layout1") {
				...
				if ($masjid_calendar_type == "Custom_url") {
    				$monthly_view = get_option("montly_pdf_url");
				} elseif ($masjid_calendar_type == "v1") {
					$monthly_view =
						"https://timing.athanplus.com/masjid/widgets/monthly?masjid_id=" .
						$masjid_id . 
						"&theme=1";
				} elseif ($masjid_calendar_type == "v2") {
					$monthly_view =
						"https://timing.athanplus.com/masjid/widgets/monthly?masjid_id=" .
						$masjid_id . 
						"&theme=2";
...
if (!function_exists("mptsi_masjidal_date")) {
    function mptsi_masjidal_date()
    {
		...
		if (!empty($results->timezone)) {
			...
			//echo_log("setting zone");
		} else {
			$ip = $_SERVER["REMOTE_ADDR"];
			$ipInfo = file_get_contents("https://pro.ip-api.com/json/" . $ip . "?key=4UJdOf3YFJPAAue");
...
{
    echo "<pre>" . esc_attr(print_r($what, true)) . "</pre>";
}