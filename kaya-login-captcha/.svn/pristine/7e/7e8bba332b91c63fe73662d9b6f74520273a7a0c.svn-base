	...
	class WPKLC_login_captcha
	{
		...
		private function update()
		{
			...
			if (wp_verify_nonce(sanitize_key($_POST['wpklc_crud_edit']), 'wpklc_crud_' . get_current_user_id()))
			{
				...
				$preparedSettingsData = $this->prepare($login_captcha);

				$this->saved = update_option(WPKLC_LOGIN_CAPTCHA_SETTINGS_DB, $preparedSettingsData, false);
				if (!$this->saved)
				{
					$currentSettingsData = get_option(WPKLC_LOGIN_CAPTCHA_SETTINGS_DB);
					if ($currentSettingsData == $preparedSettingsData)
					{
						// new settings value is same as current settings value
						$this->saved = true;
					}
				}
			}
