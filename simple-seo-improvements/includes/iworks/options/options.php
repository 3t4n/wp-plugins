...
class iworks_options {
	...
	public function build_options( $option_group = 'index', $echo = true, $term_id = false ) {
		...
		if ( ! is_array( $options['options'] ) ) {
			...
			esc_html_e( 'An error occurred while getting the configuration.', 'simple-seo-improvements' );
			echo '</strong></p></div>';
			return;
		}
		/**
			...
			switch ( $option['type'] ) {
				...
				case 'image':
					...
					$content .= sprintf(
						...
						esc_attr__( 'Select Image', 'simple-seo-improvements' ),
						esc_attr( $html_element_name )
					);
					$content .= sprintf(
						' <input type="button" class="button iworks_delete_button" value="%s" rel="#%s" %s/>',
						esc_attr__( 'Delete image', 'simple-seo-improvements' ),
						esc_attr( $html_element_name ),
		...
		if ( $options['show_submit_button'] ) {
			$content .= get_submit_button(
				esc_html__( 'Save Changes', 'simple-seo-improvements' ),
				'primary',
				'submit_button'
			);
		}
...
<?php } ?>
	...
	private function location( $name, $value = '', $args = array() ) {
		...
		$i18n     = array(
			'country' => esc_html__( 'Country', 'simple-seo-improvements' ),
			'city'    => esc_html__( 'City', 'simple-seo-improvements' ),
			'street'  => esc_html__( 'Street', 'simple-seo-improvements' ),
			'zip'     => esc_html__( 'ZIP code', 'simple-seo-improvements' ),
		);
		...
		$f = array(
			...
			array(
				...
				'wp_localize_script' => array(
					'buttons' => array(
						'select_media' => __( 'Select Image', 'simple-seo-improvements' ),
					),
				),
			),
			/**
	...
	public function get_switch_button_data() {
		$data = array(
			'labels' => array(
				'off_label' => esc_html__( 'OFF', 'simple-seo-improvements' ),
				'on_label'  => esc_html__( 'ON', 'simple-seo-improvements' ),
			),
		);
		return $data;
	}
	...
	private function get_nonce_value() {
		...
		return new WP_Error( 'security', esc_html__( 'Failed Security Check', 'simple-seo-improvements' ) );
	}
