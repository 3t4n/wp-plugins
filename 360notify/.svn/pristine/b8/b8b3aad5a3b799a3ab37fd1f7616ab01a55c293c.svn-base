...
function wk_plugin_settings_link( $links )
{
    ...
    $_link = '<a href="'.$url.'">' . __( 'Settings', '360notify' ) . '</a>';
    $links[] = $_link;
    return $links;
}
...
function custom_plugin_row_meta( $links, $file ) {
	...
	if ( strpos( $file, '360notify-woocommerce.php' ) !== false ) {
		$new_links = array(
			'<a href="https://360messenger.com/api" target="_blank">' . __( 'API docs', '360notify' ) . '</a>'
			);
...
function plugin_init() {
	...
	load_plugin_textdomain( '360notify', false, dirname(plugin_basename(__FILE__)).'/languages/' );
