...
class Notify_SendSMS_View implements Notify_Register_Interface {
	...
	public function register() {
        add_filter( 'notifysms_setting_section',     array($this, 'set_sendsms_setting_section' ) );
		add_filter( 'notifysms_setting_fields',      array($this, 'set_sendsms_setting_field' ) );
		add_action( 'notifysms_load_javascripts',    array($this, 'load_scripts' ) );
		...
		add_action( 'admin_post_notifysms_sms_form', array($this, 'mapi_send_sms' ) );
    ...
    {
        $from='360Messenger';$message_to='';$message='';$users='';$recipients='';$country='';$roles=[];
        $post_data = $_POST['notifysms_sendsms_setting'];
        //if(isset($post_data['notifysms_sendsms_from']))
            //$from = sanitize_text_field($post_data['notifysms_sendsms_from']);
            //$from = esc_attr($from);
        if(isset($post_data['notifysms_sendsms_message_to']))
            $message_to = sanitize_text_field($post_data['notifysms_sendsms_message_to']);
            //$message_to = esc_attr($message_to);
        if(isset($post_data['notifysms_sendsms_message']))
            $message = sanitize_text_field($post_data['notifysms_sendsms_message']);
            //$message = esc_textarea($message);
        if(isset($post_data['notifysms_sendsms_users'])){
            $users = array();
            foreach ($post_data['notifysms_sendsms_users'] as $value) {
                $users[] = sanitize_text_field($value);
            }
        }
            //$users = array_map( 'esc_attr', $users );
        if(isset($post_data['notifysms_sendsms_recipients']))
            $recipients = sanitize_text_field($post_data['notifysms_sendsms_recipients']);
            //$recipients = esc_textarea($recipients);
        // if(isset($post_data['notifysms_sendsms_country']))
        //     $country = sanitize_text_field($post_data['notifysms_sendsms_country']);

        if(isset($post_data['notifysms_sendsms_filters']))
            $filters = sanitize_text_field($post_data['notifysms_sendsms_filters']);
        if(isset($post_data['notifysms_sendsms_criteria'])) {
            $criteria = sanitize_text_field($post_data['notifysms_sendsms_criteria']);
        }

        $numbers = Notify_SendSMS_Sms::getPhoneNumber($message_to, $users, $recipients, $country, $filters, $criteria);
            ...
            if(is_array($numbers)){
                foreach($numbers as $number){
                    if($number instanceof WP_User) {
                        $user = $number;
                        $send_sms = Notify_SendSMS_Sms::send_sms($from, $user->phone, $message, $medium);
                    }
                    else {
                        $send_sms = Notify_SendSMS_Sms::send_sms($from, $number, $message, $medium);
                    }
                }
            }else{
                if($numbers instanceof WP_User) {
                    ...
                    $send_sms = Notify_SendSMS_Sms::send_sms($from, $numbers, $message, $medium);
                }
            }
        }
        wp_redirect(admin_url('options-general.php?page=360notify-woocoommerce-setting&sms_sent='.$send_sms)); exit;
    }

	public function set_sendsms_setting_section( $sections ) {
		$sections[] = array(
            'id'               => 'notifysms_sendsms_setting',
            'title'            => __( 'Send WhatsApp', '360notify' ),
            'submit_button'    => get_submit_button( __('Send Message', '360notify' ), 'primary large', 'sendMessage', true ,array('id' => 'sendMessage')),
	...
	public function set_sendsms_setting_field( $setting_fields ) {
		...
		$setting_fields['notifysms_sendsms_setting'] = array(
			array(
				'name'    => 'notifysms_sendsms_message_to',
				'label'   => __( 'To', '360notify' ),
				'desc'    => __('Select the recipients you wish to broadcast your message', '360notify' ),
				'type'    => 'select',
				'default' => 'customer_all',
				'options' => array(
					'customer_all'     => __('All users', '360notify' ),
					'customer'         => __('Specific users', '360notify' ),
					'phones'           => __('Specific phone number', '360notify' ),
					'spec_group_ppl'   => __('Specific Group of People', '360notify' ),
				)
			),
			array(
				'name'    => 'notifysms_sendsms_users',
				'label'   => __( 'Users', '360notify' ),
				'desc'    => __('Note: Please ensure <b>Mobile Number</b> field at <b>Additional profile information</b> is not empty for selected users.<br />', '360notify' ),
				'type'    => 'selectm',
				'default' => 'auto',
			...
			array(
				'name'    => 'notifysms_sendsms_recipients',
				'label'   => __( 'Recipients', '360notify' ),
				'desc'    => __('(Please insert country code along with mobile numbers,<br>e.g. 60124512978,60198745123,60161237841)', '360notify' ),
				'type'    => 'textarea',
				'rows'    => '8',
				'cols'    => '500',
				'css'     => 'min-width:350px',
            ...
            array(
				'name'    => 'notifysms_sendsms_filters',
				'label'   => __( 'Filter By', '360notify' ),
				'desc'    => __('Select the recipients you wish to filter by<br />', '360notify' ),
				'type'    => 'select',
				'default' => '-1',
				'options' => array(
                    '-1'          => "Select Filter",
            ...
            array(
				'name'    => 'notifysms_sendsms_criteria',
				'label'   => __( 'Criteria', '360notify' ),
				'desc'    => __('Select the criteria you wish to filter by<br />', '360notify' ),
				'type'    => 'select',
                // 'css'     => 'min-width:350px;',
				'default' => '-1',
				'options' => array(),
    ...
    public function getPhonePrefixByCode($country_code) {
        ...
        <label>Country<br/>
            <select name="country" class="specific_number_prefix">
                ...
                <option data-country-code="<?php echo esc_attr(strtolower($country['d_code'])); ?>"
                        value="<?php echo esc_attr(strtolower($country['code'])); ?>"
                        <?php
                            if(!empty(notifysms_get_options('notifysms_woocommerce_country_code', 'notifysms_setting', '' ))) {
                                echo (
                                    strtolower($country['code'])==notifysms_get_options('notifysms_woocommerce_country_code', 'notifysms_setting', '' )
                                    ) ? esc_attr('selected=selected') : '';
                            }
                            else {
                                echo (!empty($country['code']) && $country['code'] == 'US') ? esc_attr('selected=selected') : '';
...
<p><?php esc_html_e('WhatsApp Sent!', '360notify'); ?></p>
        </div>
        <?php
    }
        ...
        <script>
            ...
            <?php
				...
				if (function_exists('is_ultimatemember')) {
					...
					global $wpdb;
					$cache_key = 'distinct_account_status';
                ...
                if(function_exists('pmpro_hasMembershipLevel')) {
                    ...
                    $results = $wpdb->get_results ( 
						"SELECT id, name FROM wp_pmpro_membership_levels" );
                    foreach ($results as $result) {
            ...
            var filter_by_arr = <?php echo wp_json_encode($available_filters) ?>;

            var criteria_array = new Object;

            if(filter_by_arr.includes("roles")) {
                criteria_array['roles'] = <?php echo wp_json_encode($roles_arr); ?>;
            }
            if(filter_by_arr.includes("country")) {
                criteria_array['country'] = <?php echo wp_json_encode($country_arr); ?>;
            }
            if(filter_by_arr.includes("status")) {
                criteria_array['status'] = <?php echo wp_json_encode($um_arr); ?>;
            }
            if(filter_by_arr.includes("membership_level")) {
                criteria_array['membership_level'] = <?php echo wp_json_encode($pmpro_arr); ?>;
            }
            ...
            populateFilters("notifysms_sendsms_setting[notifysms_sendsms_filters]", "notifysms_sendsms_setting[notifysms_sendsms_criteria]");

            jQuery(function ($) {

                $('#notifysms_sendsms_setting\\[notifysms_sendsms_users\\]').closest("tr").hide();
                $('#notifysms_sendsms_setting\\[notifysms_sendsms_recipients\\]').closest("tr").hide();
                $('#notifysms_sendsms_setting\\[notifysms_sendsms_filters\\]').closest("tr").hide();
                $('#notifysms_sendsms_setting\\[notifysms_sendsms_criteria\\]').closest("tr").hide();

                $('#notifysms_sendsms_setting\\[notifysms_sendsms_message_to\\]').on('change', function() {
                    if($(this).val()=="customer_all") {
                        ...
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_users\\]').closest("tr").show();
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_recipients\\]').closest("tr").hide();
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_filters\\]').closest("tr").hide();
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_criteria\\]').closest("tr").hide();
                    }
                        ...
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_recipients\\]').closest("tr").show();
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_users\\]').closest("tr").hide();
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_filters\\]').closest("tr").hide();
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_criteria\\]').closest("tr").hide();
                    }
                    else if($(this).val()=="spec_group_ppl") {
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_filters\\]').closest("tr").show();
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_criteria\\]').closest("tr").show();
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_users\\]').closest("tr").hide();
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_recipients\\]').closest("tr").hide();
                    } else {
                        $('#notifysms_sendsms_setting\\[notifysms_sendsms_users\\]').closest("tr").hide();
                ...
                var validate = function () {
                    ...
                    if(!($('#notifysms_sendsms_setting\\[notifysms_sendsms_users\\]').val())
                        && $('#notifysms_sendsms_setting\\[notifysms_sendsms_message_to\\]').val() == 'customer') {
                        error = "users";
                        return false;
                    }

                    if(!($('#notifysms_sendsms_setting\\[notifysms_sendsms_recipients\\]').val())
                        && $('#notifysms_sendsms_setting\\[notifysms_sendsms_message_to\\]').val() == 'phones') {
                        error = "recipients";
                        return false;
                    }

                    if(!($('#notifysms_sendsms_setting\\[notifysms_sendsms_message\\]').val())) {
                        error = "message";
                        return false;
                    }
                    return true;
                ...
                $('#sendMessage').on('click', function(e) {
                    ...
                    e.stopPropagation();
                    if(validate()) {
                        $("#notifysms_sendsms_setting form").submit();
                    } else {
                        if(error == 'users'){
