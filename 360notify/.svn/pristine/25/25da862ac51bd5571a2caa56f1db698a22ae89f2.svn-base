...
class NotifySimpleMembership implements Notify_PluginInterface, Notify_Register_Interface {
    ...
    */
    ...
    private function get_sms_template_fields() {
        return array(
            array(
                ...
                'label'   => __( 'Recurring payment received', '360notify' ),
                ...
                'cols'    => '500',
                'css'     => 'min-width:350px;',
                'default' => __( 'Hi [first_name], your subscription of [total_amount] for [membership_level_name] via [payment_gateway] is [payment_status].', '360notify' )
            ),
            array(
                'name'    => 'notifysms_automation_sms_template_payment_cancelled',
                'label'   => __( 'Membership cancellation or end of term', '360notify' ),
                ...
                'cols'    => '500',
                'css'     => 'min-width:350px;',
                'default' => __( 'Hi [first_name], your membership for [membership_level_name] has expired or cancelled. Renew now to retain access.', '360notify' )
            ),
        );
    }
    ...
    public function send_sms_on($ipn_data)
    {
        ...
        $this->log->add("360MessengerWhatsApp", $ipn_data);
    ...
    */
    protected function replace_keywords_with_value($member, $ipn_data, $message)
    {
        ...
        if( isset($ipn_data['parent_txn_id']) ) {
            ...
            global $wpdb;
            $subscr_id = $ipn_data['subscr_id'];
			$cache_key = 'replace_keywords_with_value';
			$cache_group = 'custom_group';

			$query_db = wp_cache_get($cache_key, $cache_group);
			...
			wp_cache_set($cache_key, $query_db, $cache_group);
		}
            $stripe_mapper = array(
                '[first_name]'            => $member->first_name,
                '[last_name]'             => $member->last_name,
