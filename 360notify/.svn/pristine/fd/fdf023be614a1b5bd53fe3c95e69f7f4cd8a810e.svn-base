...
class NotifyLatePoint implements Notify_PluginInterface, Notify_Register_Interface {
    /*
    ...
    */
    ...
    public static function plugin_activated()
    {
        ...
        if (is_plugin_active(sprintf("%s/%s.php", self::$plugin_identifier, self::$plugin_identifier))) {
            try {
                ...
                $log->add("360MessengerWhatsApp", "Failed to import model files from LATEPOINT");
    ...
    public function get_setting_section_data()
    {
        return array(
            ...
            'title' => 'LatePoint Appointment Booking and Reservation',
        );
    }
    ...
    private function get_sms_template_fields() {
        return array(
            array(
                ...
                'cols'    => '500',
                'css'     => 'min-width:350px;',
                'default' => __( 'Greetings [cust_first_name], your appointment for [service_name] on [booking_start_date] [booking_start_time] is [booking_status]', '360notify' )
            ),
            array(
                'name'    => 'notifysms_automation_sms_template_pending',
                'label'   => __( 'Pending approval WhatsApp message', '360notify' ),
            ...
            array(
                'name'    => 'notifysms_automation_sms_template_payment_pending',
                'label'   => __( 'Payment pending WhatsApp message', '360notify' ),
                'desc'    => sprintf('Customize your WhatsApp with <button type="button" id="notifysms-open-keyword-%1$s-[dummy]" data-attr-type="pending" data-attr-target="%1$s[notifysms_automation_sms_template_payment_pending]" class="button button-secondary">Keywords</button>', $this->get_option_id() ),
                'type'    => 'textarea',
                'rows'    => '8',
                'cols'    => '500',
                'css'     => 'min-width:350px;',
    ...
    public function get_plugin_settings($with_identifier = false)
    {
        $settings = array(
            ...
            "notifysms_automation_sms_template_payment_pending"  => notifysms_get_options("notifysms_automation_sms_template_payment_pending", $this->get_option_id()),
    ...
    public function send_sms_on($booking, $old_status)
    {
        ...
        $this->log->add("360MessengerWhatsApp", "booking id: {$booking->id}");
        $this->log->add("360MessengerWhatsApp", "booking status: {$booking->status}");
    ...
    public function send_customer_notification($booking, $status)
    {
        ...
        if(empty($phone_no)) {
            ...
            $this->log->add("360MessengerWhatsApp", "customer phone number is EMPTY or NULL");
            $this->log->add("360MessengerWhatsApp", "Checking whether customer is associated with a wordpress user");
            if(empty($customer->wordpress_user_id)) {
                $this->log->add("360MessengerWhatsApp", "Customer not associated with any wordpress user");
                $this->log->add("360MessengerWhatsApp", "Nothing else to do here, abort");
                return;
            }
