...
class Notify_Automation_View implements Notify_Register_Interface {
	...
	public function register() {
        add_filter( 'notifysms_setting_section', array( $this, 'set_automation_setting_section' ) );
		add_filter( 'notifysms_setting_fields', array( $this, 'set_automation_setting_field' ) );
        ...
        $this->activated_plugins = NotifySupportedPlugin::get_activated_plugins();
	...
	public function set_automation_setting_section( $sections ) {
		...
		$sections[] = array(
			'id'             => 'notifysms_automation_setting',
			'title'          => __( 'Automation', '360notify' ),
            'submit_button'  => '',
            'children'       => $children,
		);
        ...
        <script>
            ...
            jQuery(function ($) {
                for (let [option_id, plugin_keywords] of Object.entries(plugins)) {
                    ...
                    $(`#notifysms-open-keyword-${option_id}-\\[dummy\\]`).click(function (e) {
                        ...
                        const buildTable = function (keywords) {
                            ...
                            chunkedKeywords.forEach(function (row, rowIndex) {
                                ...
                                row.forEach(function (col) {
                                    tableCode += `<td class="column"><button class="button-link" onclick="notifysms_bind_text_to_field_automation('${target}', '[${col}]')">[${col}]</button></td>`;
                                });
                                tableCode += '</tr>';
            ...
            function notifysms_bind_text_to_field_automation(target, keyword) {
