...
jQuery(function ($) {
    ...
    $("#notifysms_admin_setting\\[notifysms_woocommerce_admin_sms_recipients\\]").focusout(function () {
        var admin_mobile_no = $("#notifysms_admin_setting\\[notifysms_woocommerce_admin_sms_recipients\\]").val().trim();
    ...
    const setupPhoneHelper = function () {
        let selectedValue = $("#notifysms_multivendor_setting\\[notifysms_multivendor_selected_plugin\\]").val();
        let phoneFieldLocation = '[edit profile page > 360Notify WooCommerce > phone]';
        ...
        $("#notifysms_multivendor_setting\\[multivendor_helper_desc\\]").html(helperText);
    };

    $("#notifysms_multivendor_setting\\[notifysms_multivendor_selected_plugin\\]").change(setupPhoneHelper);
    setupPhoneHelper();

    $('.modal').css("height", "auto");
    $('.modal').css("overflow-x", "unset");
    $('.modal').css("overflow-y", "unset");
    $('#notify_sms\\[open-keywords\\]').click(function (e) {
        ...
        const orderKeywords = ['order_id', 'order_currency', 'order_amount', 'order_latest_cust_note', 'order_note', 'order_product_with_qty', 'order_product', 'order_status', 'order_total_discount','order_date_created', 'order_total_tax', 'order_subtotal'];
        let billingKeywords = ['billing_first_name', 'billing_last_name', 'billing_phone', 'billing_email', 'billing_company', 'billing_address', 'billing_country', 'billing_city', 'billing_state', 'billing_postcode', 'payment_method', 'shipping_method', 'transaction_id', 'shipment_tracking_number'];

        if ($('#notifysms_new_billing_field') && $('#notifysms_new_billing_field').val() !== '') {
            let newFields = $('#notifysms_new_billing_field').val().split(',');
        ...
        const buildTable = function (keywords) {
            ...
            chunkedKeywords.forEach(function (row, rowIndex) {
                ...
                row.forEach(function (col) {
                    tableCode += `<td class="column"><button class="button-link" onclick="notifysms_bind_text_to_field('${target}', '[${col}]')">[${col}]</button></td>`;
                });
                tableCode += '</tr>';
        ...
        $('#notify_sms\\[keyword-modal\\]').off();
        $('#notify_sms\\[keyword-modal\\]').on($.modal.AFTER_CLOSE, function () {
        ...
        $('#notify_sms\\[keyword-modal\\]').html(mainTable);
        $('#notify_sms\\[keyword-modal\\]').modal();
    });

    $('#notify_sms\\[open-keywords-low-product-stock\\]').click(function (e) {
        const type = $(e.target).attr('data-attr-type');
        const target = $(e.target).attr('data-attr-target');
...
function notifysms_bind_text_to_field(target, keyword) {
