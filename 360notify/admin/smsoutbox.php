<?php
/* This file is forked from the original author Micro Ocean Technologies's MoceanAPI Order SMS Notification plugin on 7/1/2024 */

class Notify_SMSOutbox_View implements Notify_Register_Interface {

	private $settings_api;

	function __construct() {
		$this->settings_api = new WeDevs_Settings_API;
	}

	public function register() {
        add_filter( 'notifysms_setting_section', array($this, 'set_smsoutbox_setting_section' ) );
		add_filter( 'notifysms_setting_fields',  array($this, 'set_smsoutbox_setting_field' ) );
        add_action( 'notifysms_setting_fields_custom_html', array($this, 'display_smsoutbox_page'), 10, 1);
	}

	public function set_smsoutbox_setting_section( $sections ) {
		$sections[] = array(
            'id'               => 'notifysms_smsoutbox_setting',
            'title'            => __( 'WhatsApp Outbox', '360notify' ),
            'submit_button'    => '',
            // 'action'           => 'notifysms_sms_form',
            // 'action_url'       => admin_url('admin-post.php'),
		);

		return $sections;
	}

	/**
	 * Returns all the settings fields
	 *
	 * @return array settings fields
	 */
	public function set_smsoutbox_setting_field( $setting_fields ) {
		return $setting_fields;
	}

    public function display_smsoutbox_page($form_id) {
        if($form_id != 'notifysms_smsoutbox_setting') { return; }
        global $wpdb;
		$db_table_name = NOTIFY_DB_TABLE_NAME;
		$count_result = wp_cache_get('count_' . $db_table_name, 'count_group');

		if ($count_result === false) {
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery
			$query = "SELECT COUNT(*) as count FROM {$db_table_name}";
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery , WordPress.DB.PreparedSQL.NotPrepared
			$count_result = $wpdb->get_results($query);

			wp_cache_set('count_' . $db_table_name, $count_result, 'count_group');
		}

		if (!empty($count_result)) {
			foreach ($count_result as $result) {
				$total = $result->count;
			}
		}

        $total_page = ceil($total / 10);
        $total_show_pages= 5;
        $middle_page_add_on_number = floor($total_show_pages/2);
        if(isset($_GET['pageno'])){
            $current_page = sanitize_text_field($_GET['pageno']);
        }else{
            $current_page = 1;
        }

        if($total_page < $total_show_pages)
        {
            $start_page = 1;
            $end_page = $total_page;
        }
        else
        {
            if(($current_page + $middle_page_add_on_number) > $total_page)
            {
                $start_page = $total_page - $total_show_pages + 1;
                $end_page = $total_page;
            }
        else if($current_page > $middle_page_add_on_number)
            {
                $start_page = $current_page - $middle_page_add_on_number;
                $end_page = $start_page + $total_show_pages - 1;
            }
            else
            {
                $start_page = 1;
                $end_page = $total_show_pages;
            }
        }

        $first_page = 1;
        $last_page = ($total_page > 0 ? $total_page : $first_page);
        $previous_page = ($current_page>1 ? $current_page -1 : 1);
        $next_page = ($current_page<$total_page ? $current_page +1 : $last_page);

        $admin_url = admin_url('options-general.php?page=360notify-woocoommerce-setting');
        $pageno = ($current_page - 1) * 10;
    ?>
        <br>
        <div class="bootstrap-wrapper">

            <nav aria-label="Page navigation example">
            <ul class="pagination">
<li class="page-item">
    <a class="page-link" href="<?php echo esc_url($admin_url . '&pageno=' . $first_page); ?>">
        <?php esc_html_e('First', '360notify'); ?>
    </a>
</li>
<li class="page-item">
    <a class="page-link" href="<?php echo esc_url($admin_url . '&pageno=' . $previous_page); ?>">
        <?php esc_html_e('Previous', '360notify'); ?>
    </a>
</li>
<?php for ($i = $start_page; $i <= $end_page; $i++) { ?>
    <li class="page-item">
        <a class="page-link" href="<?php echo esc_url($admin_url . '&pageno=' . $i); ?>">
            <?php echo esc_html($i); ?>
        </a>
    </li>
<?php } ?>
<li class="page-item">
    <a class="page-link" href="<?php echo esc_url($admin_url . '&pageno=' . $next_page); ?>">
        <?php esc_html_e('Next', '360notify'); ?>
    </a>
</li>
<li class="page-item">
    <a class="page-link" href="<?php echo esc_url($admin_url . '&pageno=' . $last_page); ?>">
        <?php esc_html_e('Last', '360notify'); ?>
    </a>
</li>
</ul>
</nav>

<span><?php esc_html_e('Page : ', '360notify'); ?><?php echo esc_html($current_page); ?></span>
<table class="table">
    <thead>
        <tr>
            <th scope="col" id='date'><?php esc_html_e('Date', '360notify'); ?></th>
            <th scope="col" id='sender'><?php esc_html_e('Sender', '360notify'); ?></th>
            <th scope="col" id='recipient'><?php esc_html_e('Recipient', '360notify'); ?></th>
            <th scope="col" id='message'><?php esc_html_e('Message', '360notify'); ?></th>
            <th scope="col" id='status'><?php esc_html_e('Status', '360notify'); ?></th>
        </tr>
    </thead>
            <tbody id="the-list" data-wp-lists='list:id'>
            <?php
                global $wpdb;
				
				$cache_key = 'query_results_' . $db_table_name;
				$cache_group = 'custom_group';

				$results = wp_cache_get($cache_key, $cache_group);

				if ($results === false) {
					// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					$query = $wpdb->prepare("SELECT * FROM {$db_table_name} ORDER BY id DESC LIMIT %d, 10",$pageno);
					// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery , WordPress.DB.PreparedSQL.NotPrepared
					$results = $wpdb->get_results($query);

					wp_cache_set($cache_key, $results, $cache_group);
				}

                foreach ( $results as $print ) {
                ?>
                <tr>
                <td><?php echo esc_attr($print->date);?></td>
                <td><?php echo esc_attr($print->sender);?></td>
                <td><?php echo esc_attr($print->recipient);?></td>
                <td><?php echo esc_attr($print->message);?></td>
                <td><?php echo esc_attr($print->status);?></td>
                </tr>
                    <?php }
            ?>
            </tbody>

            </table>
        </div>

    <?php
    }


}

?>
