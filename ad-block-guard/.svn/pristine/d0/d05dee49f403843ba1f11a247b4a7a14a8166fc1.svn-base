...
(function() {
    window.addEventListener('load', function() {
        ...
        js_var_const_style.innerHTML = `
			...
			.css-class-ad-block-overlay-content-inner div p {
			  margin:0;
			}

			.css-class-ad-block-overlay-content-inner div p + p {
			  margin-top:10px;
			}
        ...
        let js_var_wuadblockguard_method_custom_js = {{json_data_method_custom_load_js_enable}};
            ...
            return new Promise((resolve, reject) => {
                ...
                if (js_var_wuadblockguard_method_1) {
                    checks.push(new Promise((resolveMethod, rejectMethod) => {
                        ...
                        if ("{{json_data_method_custom_css_id}}".trim() !== "") {
                        	js_var_bait.id = "{{json_data_method_custom_css_id}}".trim();
                        }

                        js_var_bait.className = 'pub_300x250 pub_300x250m text-ad textAd text_ad adBanner ad-banner ad_box adbox';

                        if ("{{json_data_method_custom_css_class}}".trim() !== "") {
                        	js_var_bait.className = "{{json_data_method_custom_css_class}}".trim() + " " + js_var_bait.className;
                        }
                        ...
                        setTimeout(function() {
                            ...
                            if (js_var_bait.offsetHeight === 0) {
                                ...
                                js_func_wuadblockguard_detected('Method 1: Bait Elements Detection');
                                resolveMethod();
                            } else {
                                logMessage('no adblock detected: bait element height > 0');
                                js_func_wuadblockguard_not_detected('Method 1: Bait Elements Detection');
                                rejectMethod();
                            }
				...
				// Method Custom: Custom JavaScript Loading
                if (js_var_wuadblockguard_method_custom_js) {
                    checks.push(new Promise((resolveMethod, rejectMethod) => {
                        const xhr = new XMLHttpRequest();
                        xhr.open('GET', '{{json_data_method_custom_load_js_url}}', true);
                        xhr.onreadystatechange = function() {
                            logMessage('method custom: network requests - checking');
                            if (xhr.readyState === 4) {
                                if (xhr.status === 0 || xhr.responseText.toLowerCase().indexOf("ublock") > -1 || xhr.responseText.toLowerCase().indexOf("height:1px") > -1) {
                                    logMessage('ADBLOCK DETECTED: Method Custom');
                                    js_func_wuadblockguard_detected('Method Custom: Network Requests Detection');
                                    resolveMethod();
                                } else {
                                    logMessage('no adblock detected: xhr.status is ' + xhr.status);
                                    js_func_wuadblockguard_not_detected('Method Custom: Network Requests Detection');
                                    rejectMethod();
                                }
                                js_var_checks_completed++;
                                logMessage('checks completed: ' + js_var_checks_completed);
                        ...
                        xhr.send();
                        logMessage('Method Custom: network requests added');
                    }));
                }
                    ...
                    checks.push(new Promise((resolveMethod, rejectMethod) => {
                        if (!document.querySelector('script[src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"]')) {
                            ...
                            script.onerror = function() {
                                ...
                                js_func_wuadblockguard_detected('Method 2: Script Loading Detection');
                                resolveMethod();
                            };
                            script.onload = function() {
                                logMessage('method 2: script loading detection - loaded');
                                js_func_wuadblockguard_not_detected('Method 2: Script Loading Detection');
                                rejectMethod();
                            };
                        ...
                        xhr.onreadystatechange = function() {
                            ...
                            if (xhr.readyState === 4) {
                                if (xhr.status === 0 || xhr.responseText.toLowerCase().indexOf("ublock") > -1 || xhr.responseText.toLowerCase().indexOf("height:1px") > -1) {
                                    ...
                                    js_func_wuadblockguard_detected('Method 3: Network Requests Detection');
                                    resolveMethod();
                                } else {
                                    logMessage('no adblock detected: xhr.status is ' + xhr.status);
                                    js_func_wuadblockguard_not_detected('Method 3: Network Requests Detection');
                                    rejectMethod();
                                }
                                js_var_checks_completed++;
                                logMessage('checks completed: ' + js_var_checks_completed);
...
function logMessage(message) {
    console.log('css-id-ad-block-overlay: ' + message);
}