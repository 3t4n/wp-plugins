...
abstract class Base {
	...
	public function get_formatted_items( WC_Order $order ): array {
		...
		if ( sizeof( $order->get_refunds() ) > 0 ){
			...
			$order_total = (float) $order->get_data()['total'];
			$order_refund = (float) $order->get_total() * (-1);
			...
			if ( $partial_refund ) {
				...
				if (static::$provider == 'payper'){

					LogInvoice::info( sprintf( 'invoice lang (%s)', $this->get_option( 'payper_lang' ) ) );
					switch ( $this->get_option( 'payper_email_lang' ) ) {
					    case '0':
					        $refund_title = sprintf( __( 'קבלת זיכוי #%s', 'wc-pelecard-gateway' ), $order->get_order_number() );
					        break;
					    case '1':
				...
				if (static::$provider == 'payper'){
					...
					$items[] = [
						$name => $refund_title,
						$total => (float) $order->get_total() * 100 * (-1),
						$quantity => 1,
						'Include_vat' =>  $payper_include_vat,
					];
				} else {
					$items[] = [
						$name => $refund_title,
				...
				foreach ( $order->get_items() as $item ) {
					...
					$items[] = apply_filters( 'wppc/invoices/' . static::$provider . '_formatted_item', [
						$name => $item->get_name(),
