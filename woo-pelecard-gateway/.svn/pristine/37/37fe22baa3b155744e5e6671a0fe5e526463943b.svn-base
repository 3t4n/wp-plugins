...
class Transaction extends \WC_Data {
	...
	public function __construct( string $transaction_id = '' ) {
		...
		if ( ! empty( $this->get_id() ) && empty($this->get_meta_data()) ) {
	        $this->set_data( $this->get_transaction() );
	    }
	}
	...
	public function set_data( array $data ) {
		...
		if ( ! empty( $data['PayperData'] ) ) {
			foreach ( $data['PayperData'] as $key => $value ) {
				$this->add_meta_data( $key, $value, true );
			}
		}
	...
	public function is_bank_transfer_failure(): bool {
		return 655 === (int) $this->get_status_code();
	}
