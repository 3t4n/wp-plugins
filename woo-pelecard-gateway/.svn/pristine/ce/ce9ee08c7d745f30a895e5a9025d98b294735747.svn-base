...
class Api {
	...
	const API_BASE_URL = 'https://gateway21.pelecard.biz/';
	...
	public static function get_checkout_iframe_url( WC_Order $order, Gateway $gateway ) {
		$logo_url = $gateway->get_option( 'logo_url' );
		if ( empty($logo_url) ){
			$logo_url = 'https://gateway21.pelecard.biz/Content/images/Pelecard.png';
		}

		$args = apply_filters( 'wppc/checkout/iframe_args', [
	...
	public static function get_currency( $order = null ) {
		...
		$currencies = apply_filters( 'wppc/currencies', [
			'ILS' => 1,
			'USD' => 2,
			'GBP' => 826,
			'EUR' => 978,
		...
		return apply_filters( 'wppc/currency_code', $currency_code, $currency, $currencies, $order );
	}
	...
	private static function request( string $endpoint, array $data ) {
		...
		$request_body = apply_filters( 'wppc/api/request_body', $data, $endpoint );

		$args = apply_filters( 'wppc/api/request_args', [
		...
		if ( is_null( $result ) ) {
			return new WP_Error(
				'error_occurred',
				__( 'Sorry, an error occurred while processing your request.', 'wc-pelecard-gateway' )
			);
		}
	...
	public static function get_my_account_iframe_url( Gateway $gateway ) {
		$args = apply_filters( 'wppc/my_account/iframe_args', [
			'terminal' => $gateway->get_terminal(),
			'user' => $gateway->get_username(),
			'password' => $gateway->get_password(),
			...
			'CaptionSet' => [
				'cs_submit' => __( 'Add Card', 'wc-pelecard-gateway' ),
			],
		], $gateway );
	...
	public static function charge_by_token( WC_Order $order, Gateway $gateway, WC_Payment_Token_CC $token ) {
		$terminal = $gateway->get_terminal( true );

		$hook_forieng_card = $gateway->get_hook_forieng_card();

		Log::info( sprintf( 'START HOOK TERMINAL CHECKING hook_forieng_card' ) );
		Log::debug($hook_forieng_card);

		if ( 'yes' === $hook_forieng_card ){

			$common_terminal = $gateway->get_common_terminal();
			$hook_terminal_origin   = $gateway->get_hook_terminal();

			if ( !empty($hook_terminal_origin) ){
				$hook_terminal = self::check_hook_terminal( $order, $gateway, $token );
				Log::info( sprintf( 'START HOOK TERMINAL CHECKING ' ) );
				Log::debug($hook_terminal);
				if ( $hook_terminal ) {
					Log::info( sprintf( 'LOGIC IS OK ' ) );
					$terminal = $common_terminal;
				} else {
					Log::info( sprintf( 'ALARM ALARM' ) );
					$terminal = $hook_terminal_origin;
				}
			} else {
				Log::info( sprintf( 'START HOOK TERMINAL NO HOOK TERMINAL ' ) );
			}
			
		} else {
			Log::info( sprintf( 'START HOOK TERMINAL NOT CHECKING ' ) );
		}
		...
		$auth_number = $order->get_meta( '_wppc_auth_number' );
		...
		$args = apply_filters( 'wppc/checkout/rest_args', $args, $order, $gateway, $token, $service );
	...
	public static function get_transaction_by_id( string $transaction_id ) {
		...
		/*$new_transactio_id = '7a54ea10-5eae-4920-9679-805a10e4d8e3';*/
	    ...
	    $response = self::request( 'PaymentGW/GetTransaction', $args );

	    // Add a custom field to indicate this is a GetTransaction response
	    if (!is_wp_error($response)) {
	    	//Log::info( 'TRANSACTION FLAG ACTIVATED' );
	        $response['is_get_transaction_response'] = true;
	    } 
	...
	public static function charge_by_token_missed_order_tokens( WC_Order $order, Gateway $gateway, WC_Payment_Token_CC $token ) {
		$args = [
		...
		Log::info( sprintf( 'CHARGE WITH TOKEN (DELETED TOKEN) (%s)', $gateway->get_action_type() ) );
		if ( 0 < $order->get_id() ) {
			Log::info( sprintf( 'ORDER #%d: PAYMENT START (DELETED TOKEN)', $order->get_id() ) );
		}
	...
	public static function check_hook_terminal(WC_Order $order, Gateway $gateway, WC_Payment_Token_CC $token ) {
	    // Prepare the request parameters
	    $args = [
	        'terminalNumber' => $gateway->get_common_terminal(),
	        'user' => $gateway->get_username(true), // Include username
	        'password' => $gateway->get_password(true), // Include password
	        'token' => $token->get_token(), // Include token
	        'total' => $order->get_total() * 100, // Include total amount
	        'currency' => Api::get_currency($order), // Include currency
	        'paramX' => $order->get_id(), // Include order ID
	        'UserKey' => $order->get_order_key(), // Include order key
	    ];

	    $service = 'AuthorizeCreditCard';

	    // Placeholder for sending the request and receiving the response
		...
		return $response['ResultData']['CreditCardCompanyIssuer'] == 0;
	}
}
