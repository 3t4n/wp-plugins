...
class Gateway extends WC_Payment_Gateway_CC {
	...
	public function get_hook_terminal() {
		return $this->get_option( 'hook_terminal' );
	}

	public function get_common_terminal() {
		return $this->get_option( 'terminal' );
	}

	public function get_hook_forieng_card() {
		return $this->get_option( 'hook_forieng_card' );
	}
	...
	public function get_custom_payment( $total ) {
		...
		if (!is_array($custom_payments)) {
	       	//Prevent Uncaught TypeError: array_reduce() error
	       	// Log::info( sprintf( 'The $custtom_paymets is not an array' ) );
	        return null;
	    }
	    // Log::info( sprintf( 'The $custtom_paymets is an array' ) );
	    // Log::debug( $custom_payments );
	...
	public function do_payment( Transaction $transaction ): bool {
		...
		if ( $transaction->is_bank_transfer_failure() ) {
			$error_message = sprintf(
				__( 'Transaction failed (%2$s): %1$s', 'wc-pelecard-gateway' ),
				$transaction->get_error_message(),
				$transaction->get_status_code()
	...
	public function check_ipn_response() {
	    ...
	    $transaction->set_data($http_post_data);
	...
	public function maybe_process_redirect_order() {
		Log::info( 'maybe_process_redirect_order starts' );
		...
		// Check if transaction has been processed already
	    if ( $transaction->exists( $transaction->get_order() ) ) {
	        $order = $transaction->get_order();
	        if ( $order ) {
