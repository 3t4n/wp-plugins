...
Event Rocket is a set of hacks and utilities that add some extra capabilities to [The Events Calendar](http://wordpress.org/plugins/the-events-calendar/) and [Events Calendar PRO](http://tri.be/shop/wordpress-events-calendar-pro/). *It is no longer supported* and is unlikely to receive further fixes. Don't be fooled by the _Last Updated_ field that displays on the WordPress.org plugin page ... it does not reflect the true status of this plugin.

![Image test](https://dump.codingkills.me/test-imgs/aurora.jpg "Image test")
