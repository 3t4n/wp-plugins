<div class="wrap">
    ...
    if (isset($_POST['project_name'])) {
        ...
        if (isset($_POST['project_name']) && !empty($_POST['project_name'])) {
            ...
            if ($flag) {
                ...
                $res1 = $wpdb->delete($tableName, array('id_project_fk' => intval($id)));

                $tableName = $wpdb->prefix . "icp_project_links";
                $res2 = $wpdb->delete($tableName, array('id_project_fk' => intval($id)));

                $table_name = $wpdb->prefix . "icp_projects";

                $ignores = (!empty($_POST['ignores'])) ? sanitize_textarea_field($_POST['ignores']) : '';
                $ignores = array_map('trim', explode("\n", $ignores));
                ...
                $wpdb->update(
                    ...
                    array(
                        ...
                        'posts' => isset($_POST['hyperlink_posts']) ? 1 : 0,
                        'pages' => isset($_POST['hyperlink_pages']) ? 1 : 0,
                        'existing' => isset($_POST['hyperlink_existing']) ? 1 : 0,
                        'new' => isset($_POST['hyperlink_new']) ? 1 : 0,
                        'comments' => isset($_POST['comments']) ? 1 : 0,
                        'new_window' => isset($_POST['new_window']) ? 1 : 0,
                        'headings' => isset($_POST['headings']) ? 1 : 0,
                        'max_replace' => intval($_POST['max_replace'] ?? 3),
                        'max_keyword' => intval($_POST['max_keyword'] ?? 1),
                        'no_follow_weight' => intval(trim(sanitize_text_field($_POST['no_follow_weight'] ?? '0'), '%')),
                        'ignores' => $ignores,
                ...
                foreach ($keywords as $keyword) {
                    ...
                    if (!empty($keyword)) {
                        $wpdb->insert(
                            ...
                            array('id_project_fk' => $pId, 'keyword' => sanitize_text_field($keyword)),
                            array('%d', '%s')
                        );
                    }
                }
                ...
                foreach ($links as $link) {
                    $wpdb->insert(
                        ...
                        array('id_project_fk' => $pId, 'url' => esc_url_raw($link)),
                        array('%d', '%s')
                    );
                }
                ...
                $query = $wpdb->prepare(
                    "SELECT * FROM {$wpdb->prefix}posts WHERE post_type = %s OR post_type = %s",
                    'post',
                    'page'
                );
                $posts = $wpdb->get_results($query, OBJECT);

                $query = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}icp_projects WHERE id = %d", $pId);
                    ...
                    if (!in_array($post->ID, $exclude) && !in_array($post->post_name, $exclude)) {
                        if ($project->existing == 1) {
                            $comments = $wpdb->get_results(
                                $wpdb->prepare("SELECT * FROM {$wpdb->prefix}comments WHERE comment_post_ID = %d", $post->ID)
                            );
    ...
    if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete') {
        ...
        // Deleting the project from 'icp_projects'
        $tableName = $wpdb->prefix . "icp_projects";
        $res = $wpdb->delete($tableName, array('id' => $id));

        // Deleting the keywords related to the project
        $tableName = $wpdb->prefix . "icp_project_keywords";
        $res1 = $wpdb->delete($tableName, array('id_project_fk' => $id));

        // Deleting the project links
        $tableName = $wpdb->prefix . "icp_project_links";
        $res2 = $wpdb->delete($tableName, array('id_project_fk' => $id));

        // Deleting keyword stats for the project
        $tableName = $wpdb->prefix . "icp_keyword_stats";
        $res3 = $wpdb->delete($tableName, array('project_id' => $id));
        ...
        // Query for 'icp_projects' table
        $tableName = $wpdb->prefix . "icp_projects";
        $query = $wpdb->prepare("SELECT * FROM {$tableName} WHERE id = %d", $id);
        $result = $wpdb->get_row($query);

        // Query for 'icp_project_keywords' table
        $tableName = $wpdb->prefix . "icp_project_keywords";
        $query = $wpdb->prepare("SELECT * FROM {$tableName} WHERE id_project_fk = %d", $id);
        $result1 = $wpdb->get_results($query, OBJECT);

        // Query for 'icp_project_links' table
        $tableName = $wpdb->prefix . "icp_project_links";
        $query = $wpdb->prepare("SELECT * FROM {$tableName} WHERE id_project_fk = %d", $id);
