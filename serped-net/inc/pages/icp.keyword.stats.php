...
$globalStats = $wpdb->prepare(
    "SELECT k.keyword keyword, COUNT(k.keyword) count 
     FROM {$keyword_stats} ks, {$keywords} k 
...
$projectStats = $wpdb->prepare(
    "SELECT p.project_name name, COUNT(ks.id) count 
     FROM {$keyword_stats} ks, {$projects} p 
...
$postStats = $wpdb->prepare(
    "SELECT p.post_type type, p.post_title title, COUNT(ks.id) count 
     FROM {$keyword_stats} ks, {$posts} p 
     WHERE ks.post_id = p.ID 
     GROUP BY ks.post_id"
);
$postsStats = $wpdb->get_results($postStats);

// Keyword stats with links
$keywordStatsLinks = $wpdb->prepare(
    "SELECT k.keyword keyword, pl.url, COUNT(ks.link_id) count_links 
     FROM {$keyword_stats} ks, {$keywords} k, {$links} pl 
     WHERE ks.keyword_id = k.id 
     AND ks.link_id = pl.id 
     GROUP BY k.keyword, ks.link_id"
);
$keywordLinks = $wpdb->get_results($keywordStatsLinks);

// Keyword stats with posts
$keywordStatsPosts = $wpdb->prepare(
    "SELECT k.keyword keyword, p.post_type type, p.post_title, p.ID pid, COUNT(p.ID) count_posts 
     FROM {$keyword_stats} ks, {$keywords} k, {$posts} p 
     WHERE ks.keyword_id = k.id 
     AND ks.post_id = p.ID 
...
// Keyword stats with projects
$keywordStatsProject = $wpdb->prepare(
    "SELECT k.keyword keyword, p.project_name, COUNT(p.id) count_projects 
     FROM {$keyword_stats} ks, {$keywords} k, {$projects} p 
     WHERE ks.keyword_id = k.id 
     AND ks.project_id = p.id 
