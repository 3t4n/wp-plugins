...
<div class="wrap">
    ...
    if (isset($_POST['project_name']) && !empty($_POST['project_name'])) {
        ...
        if (count($links) > 0) {
            ...
            // Sanitize and process 'ignores'
            $ignores = (!empty($_POST['ignores'])) ? sanitize_textarea_field($_POST['ignores']) : '';
            $ignores_array = array_map('trim', explode("\n", $ignores));
            $ignores = implode(",", $ignores_array); // Safely rejoin
            ...
            $wpdb->insert(
                ...
                array(
                    ...
                    'posts' => isset($_POST['hyperlink_posts']) && $_POST['hyperlink_posts'] === 'on' ? 1 : 0,
                    'pages' => isset($_POST['hyperlink_pages']) && $_POST['hyperlink_pages'] === 'on' ? 1 : 0,
                    'existing' => isset($_POST['hyperlink_existing']) && $_POST['hyperlink_existing'] === 'on' ? 1 : 0,
                    'new' => isset($_POST['hyperlink_new']) && $_POST['hyperlink_new'] === 'on' ? 1 : 0,
                    'comments' => isset($_POST['comments']) && $_POST['comments'] === 'on' ? 1 : 0,
                    'new_window' => isset($_POST['new_window']) && $_POST['new_window'] === 'on' ? 1 : 0,
                    'headings' => isset($_POST['headings']) && $_POST['headings'] === 'on' ? 1 : 0,
                    'max_replace' => (!empty($_POST['max_replace'])) ? intval($_POST['max_replace']) : 3,
                    'max_keyword' => (!empty($_POST['max_keyword'])) ? intval($_POST['max_keyword']) : 1,
                    'no_follow_weight' => (!empty($_POST['no_follow_weight'])) ? intval(rtrim(sanitize_text_field($_POST['no_follow_weight']), "%")) : 0,
                    'ignores' => $ignores,
                    'created_at' => time()
                ),
            ...
            $query = $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}posts WHERE post_type IN (%s, %s)",
                'post',
                'page'
            );
            ...
            $project = $wpdb->get_row(
                $wpdb->prepare("SELECT * FROM {$wpdb->prefix}icp_projects WHERE id = %d", $pId)
            );

            $exclude = array_map('trim', explode(",", $project->ignores));
            ...
            echo '<script type="text/javascript">window.location = \'' . esc_url(admin_url('admin.php?page=icp_link_projects&type=updated&message=' . $message)) . '\';</script>';
            die();
        } else {
    ...
    <form method="post" action="" id="add_form">
        ...
        <table class="form-table">
            ...
            <tr>
                <td>
                    <label>Maximum hyperlink replacements per keyword on this site (sitewide)</label>
                </td>
                <td>
            ...
            <tr>
                <td>
                    <label>Weight of no-follow links on this site (sitewide)</label>
                </td>
                <td>
