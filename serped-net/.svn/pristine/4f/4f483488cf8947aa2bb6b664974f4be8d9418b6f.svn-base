...
class SRPD_Project_List_Table extends WP_List_Table {
    ...
    function column_keywords($item) {
        ...
        $query = $wpdb->prepare(
            "SELECT keyword FROM $table_name WHERE id_project_fk = %d",
            $item['id']
        );
        ...
        foreach ($results as $keyword) {
            $keywords .= '&bull; ' . esc_html($keyword->keyword) . "<br/> ";
        }
    ...
    function prepare_items() {
        ...
        // Prepare query for secure fetching
        $table_name = $wpdb->prefix . 'icp_projects';

        // Fetch and sanitize orderby and order parameters
        $orderby = (!empty($_REQUEST['orderby'])) ? sanitize_sql_orderby($_REQUEST['orderby']) : 'title'; // Default to 'title'
        $order = (!empty($_REQUEST['order']) && in_array(strtolower($_REQUEST['order']), ['asc', 'desc'])) ? sanitize_text_field($_REQUEST['order']) : 'asc';

        // Validate column exists in the table to prevent SQL injection
        $valid_columns = array_keys($this->get_columns());
        if (!in_array($orderby, $valid_columns)) {
            $orderby = 'title'; // Fallback to default column
        }

        // Use prepared statement for the main query
        $query = $wpdb->prepare(
            "SELECT * FROM $table_name ORDER BY $orderby %s",
            $order
        );
