...
class SRPD_Link_List_Table extends WP_List_Table {
    ...
    function column_url($item) {
        ...
        return sprintf(
            '<a href="%1$s">%2$s</a> <span style="color:silver"></span>%3$s',
            /* $1%s */
            $item['url'],
            $item['url'],
            /* $2%s */
    ...
    function prepare_items() {
        ...
        // Retrieve all items from the database (ensure to sanitize inputs)
        $query = 'SELECT * FROM ' . $wpdb->prefix . 'icp_links';

        // Handle sorting if there are orderby and order parameters
        $orderby = isset($_REQUEST['orderby']) ? sanitize_text_field($_REQUEST['orderby']) : 'title'; // Default to 'title' if not set
        $order = isset($_REQUEST['order']) ? sanitize_text_field($_REQUEST['order']) : 'asc'; // Default to 'asc' if not set

        // Ensure that the sorting column exists in the table (to avoid potential issues with invalid columns)
        $valid_columns = array_keys($columns);
        if (!in_array($orderby, $valid_columns)) {
            $orderby = 'title'; // Fallback to a valid column if the orderby is invalid
        }

        // Add sorting to the query
        $query .= $wpdb->prepare(" ORDER BY {$orderby} {$order}");

        // Get the data from the database
        $data = $wpdb->get_results($query, ARRAY_A);

        // Sort the data using PHP (optional if you use SQL order)
        function usort_reorder($a, $b) {
            global $orderby, $order;

            // Use the orderby and order parameters to sort
