...
if (file_exists ($src)) {
	...
	// add border
	$borderSettings = explode (',', $border);
	$borderSettings[0] = is_null($borderSettings[0]) ? 0 : intval($borderSettings[0]);
	for ($i=1; $i<=3; $i++) {
		$borderSettings[$i] = is_null($borderSettings[$i]) ? 255 : intval($borderSettings[$i]);
	}
	if ($borderSettings[0]>0) {
		$image = $canvas;
		$width = imagesx ($image);
		$height = imagesy ($image);
		...
		// Create a new border color for image
		$color = imagecolorallocatealpha ($canvas, $borderSettings[1], $borderSettings[2], $borderSettings[3], 0);
		
		// Completely fill the background of the new image with allocated color.
		imagefill ($canvas, 0, 0, $color);
		
		imagecopyresampled ($canvas, $image, $border, $border, 0, 0, $width, $height, $width, $height);
	}
