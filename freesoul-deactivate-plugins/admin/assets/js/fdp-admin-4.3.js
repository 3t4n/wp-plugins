jQuery(document).ready(function($){
	...
	$("td").on("click", function () {
		$(this).closest('tr').find('td').removeClass('eos-dp-auto-checked');
	});
	...
	$(".fdp-custom-rows-btn").on("click", function () {
		$('.eos-dp-opts-msg').addClass('eos-hidden');
		var data_url = {},urls = document.getElementsByClassName('eos-dp-url'),url = '',page_slug = $('.eos-dp-section ').attr('data-page_slug');
		eos_dp_show_all_plugins();
	...
	$(".eos-dp-save-eos_dp_admin").on("click", function () {
		...
		var data = {
			...
			"admin_menus" : JSON.stringify(eos_dp_admin_pages),
	...
	if('undefined' !== typeof(eos_dp_js.page) && ('eos_dp_url' === eos_dp_js.page || 'eos_dp_admin_url' === eos_dp_js.page || 'eos_dp_firing_order' === eos_dp_js.page || 'eos_dp_ajax' === eos_dp_js.page || 'eos_dp_logged' === eos_dp_js.page ) ){
	...
	$('.eos-dp-setts-menu-item').on('mouseenter',function(){
		var nextLink = document.getElementById(this.id + '-prefetch');
		if(null === nextLink || nextLink.length < 1){
		 	$('<link id="' + this.id + '-prefetch" rel="prefetch" href="' + $(this).find('a').attr('href') + '" as="document" />').appendTo("body");
	 	}
	});
	$('.eos-dp-setts-menu-item .eos-dp-submenu-item').on('mouseenter',function(){
		var nextLinkId = $(this).closest('.eos-dp-setts-menu-item').attr('id') + '-' + $(this).index() + '-prefetch',nextLink = $('#' + nextLinkId);
		if(null === nextLink || nextLink.length < 1){
		 	$('<link id="' + nextLinkId + '" rel="prefetch" href="' + $(this).find('a').attr('href') + '" as="document" />').appendTo("body");
	 	}
	});
...
// for(var n=1;n<7;++n){console.log(n + ': ' + jQuery('#eos-dp-search-' + n).is(':checked'));}
