jQuery(document).ready(function($){
	...
	var tr = table ? table.getElementsByTagName('tr') : false;
	firstRowTop = table && parseInt(eos_dp_js.plugins_n) > 1 && tr && tr.length > 0 ? tr[tr.length - 1].getBoundingClientRect().top : null;
	if(plugsN > 15){
	...
	$('#eos-dp-setts').on('click','.fdp-row-actions-ico',function(){
		...
		$('#eos-dp-setts tr').not(tr).removeClass('fdp-actions-on');
		tr.toggleClass('fdp-actions-on');
		$(this).closest('td').toggleClass('eos-dp-hover');
	});
	$('#fdp-show-page-filters').on('click',function(){
		$('#fdp-singles-filter').toggleClass('eos-hidden');
	...
	if('eos_dp_admin' === eos_dp_js.page){
		$("#eos-dp-by-admin-section .eos-dp-td-chk-wrp").on("click", function(e) {
			var td = $(this).closest('td'),is_active = td.hasClass('eos-dp-active'),input_class = e.target.className,els = $('#eos-dp-setts .eos-dp-view[href="' + $(this).closest('tr').find('.eos-dp-view').attr('href') + '"]'),path = td.attr('data-path');
			$(els).each(function(){
				var el = $(this).closest('tr').find('[data-path="' + path + '"]').find('.eos-dp-td-chk-wrp'),i = el.find('input')[0];
				if(i.className !== input_class){
					i.checked = is_active;
					if(is_active){
						el.closest('td').removeClass('eos-dp-active');
					}
					else{
						el.closest('td').addClass('eos-dp-active');
					}
				}
			});
		});
	...
	$("td").on("click", function () {
		$(this).closest('tr').find('td').removeClass('eos-dp-autochecked');
	});
	...
	$('#eos-dp-setts').on('click','.eos-dp-copy',function(){
		var row = $(this).closest('tr'),tooltip = $(this).find('.fdp-tooltip');
		...
		if(localStorage.getItem('eos_dp_last_copied_row') === JSON.stringify(window.eos_dp_last_copied_row)) {
			var msg = $(this).find('.fdp-msg-success');
		}
		else{
			var msg = $(this).find('.fdp-msg-error');
		}
			...
			success : function (response) {
				if (parseInt(response) == 1) {
					...
					if(encode_url){
						...
						a.href += 'dummy_html' !== theme ? '%26test_id%3D' + microtime : '';
					}
					else{
						a.href = a.href.split('&theme=')[0].split('%26').join('&').split('%3D').join('=');
						a.href += 'dummy_html' !== theme ? '&test_id=' + microtime : '';
					}
					if('_admin' === admin){
	...
	$('.eos-dp-close-actions').not('.eos-dp-themes-list').on('click',function(){
		$(this).closest('td').removeClass('eos-dp-hover').addClass('eos-dp-not-hover');
		$(this).closest('tr').removeClass('fdp-actions-on');
	...
	$('.eos-dp-title .fdp-title-text').each(function(){
		var name_wrp = $(this),txt = name_wrp.text();
		if(txt.length > 31 && txt.substring(0,28) !== txt){
			name_wrp.text(txt.substring(0,28) + ' ...');
		}
	});
	...
	$(".fdp-custom-rows-btn").on("click", function () {
		$('.eos-dp-opts-msg').addClass('eos-hidden');
		var data_url = {},urls = document.querySelectorAll('tr.eos-dp-url'),url = '',page_slug = $('.eos-dp-section ').attr('data-page_slug'),theme_activation = {};
		eos_dp_show_all_plugins();
		for(var n = 0;n < urls.length - 1;++n){
			var plugins = [],chks = urls[n].getElementsByClassName('eos-dp-td-url-chk-wrp');
			for(k = 0;k < chks.length;++k){
				if(!$(chks[k].getElementsByTagName('input')).closest('td').hasClass('eos-dp-active')){
					var plugin_name_td = document.getElementById('eos-dp-plugin-name-' + (k + 1));
					if(plugin_name_td){
						plugins[k] = plugin_name_td.getAttribute('data-path');
					}
				}
			}
			url = $(urls[n]).find('.eos-dp-url-input').val();
			if('undefined' !== typeof(chks[k -1])){
				theme_activation[url] = $(chks[k - 1].getElementsByTagName('input')).closest('td').hasClass('eos-dp-active');
			}
			data_url[n] = {};
			data_url[n]['url'] = url;
	...
	$(".eos-dp-save-eos_dp_admin_url").on("click", function () {
		$('.eos-dp-opts-msg').addClass('eos-hidden');
		eos_dp_show_all_plugins();
		var data_url = {},urls = document.getElementsByClassName('eos-dp-url'),url = '';
		for(var n = 0;n < urls.length - 1;++n){
			var plugins = [],chks = urls[n].getElementsByClassName('eos-dp-td-url-chk-wrp');
			for(k = 0;k < chks.length;++k){
				if(!$(chks[k].getElementsByTagName('input')).closest('td').hasClass('eos-dp-active')){
	...
	$(".fdp-one-col-sec+.eos-dp-btn-wrp input").on("click", function () {
		var slug = $('.fdp-one-col-sec').attr('data-section').replace('eos_dp_',''),chk,str = '';
		$('.eos-dp-opts-msg').addClass('eos-hidden');
		$('.fdp-one-col-i').each(function(){
			chk = $(this);
			str += !chk.is(':checked') ? ',' + $(this).attr('data-path') : ',';
		});
		eos_dp_send_ajax($(this),{
			"nonce" : $("#eos_dp_" + slug + "_setts").val(),
			"data" : str,
			"opt_name" : slug,
	...
	$('#eos-dp-setts').on('mouseenter','.eos-dp-post-row',function(){
		trHover = $(this);
	});
    ...
    if(e.keyCode == 67){
			...
			var slider = $('.fdp-plugins-slider');
			slider.val(slider.val() + 1).trigger('input');
    }
    else if(e.keyCode == 39){
			var slider = $('.fdp-plugins-slider');
			slider.val(Math.max(0,slider.val() - 1)).trigger('input');
    }
    else if(e.keyCode == 33){
			window.scrollTo(0,0);
    }
    else if(e.keyCode == 65){
			$('#fdp-plug-filter-all').trigger('click');
    }
    else if(e.keyCode > 48 && e.keyCode < $('.fdp-plug-filter').length + 48){
			$('.fdp-plug-filter').eq(e.keyCode - 48).trigger('click');
    }
	});
	...
	$(".fdp-plugins-activation-filter").on("click", function () {
		$(".fdp-plugins-activation-filter").removeClass('fdp-current-link');
		$(this).addClass('fdp-current-link');
		$('.fdp-inactive-plugin,.fdp-active-plugin').removeClass('eos-hidden');
		$('.' + $(this).attr('data-state')).addClass('eos-hidden');
	});
	...
	$(".eos-dp-pro-autosettings").on("click", function () {
		...
		if(backend){
			...
			else if('undefined' !== typeof(eos_dp_js.page) && 'eos_dp_by_term_archive' === eos_dp_js.page){
				data.term_type = button.closest('tr').attr('data-post-type');
				data.tax = button.closest('tr').attr('data-tax');
				data.href = button.closest('tr').attr('data-href');
			}
			else{
		...
		// fdp_suggest_plugins(button,data);
		return false;
	});
	...
	if(document.body.className.indexOf('fdp-sortable-page')>0){
		$('.fdp-sortable-page #eos-dp-setts tbody').sortable({
			axis : "y",
			containment : "parent",
	...
	$('#fdp-toggle-top-bar-wrp').on('click',function(){
		$('body').toggleClass('fdp-top-bar-open');
	});
	...
	$('.fdp-delete-transient-button').on('click',function(){
		var btn = $(this);
		$.post(ajaxurl,{
			transient: $(this).attr('data-transient'),
			action: 'eos_dp_pro_delete_transient'
		});
		$(this).closest('.eos-dp-notice').fadeOut(300);
	});
	$('.fdp-dismiss-pro-notice').on('click',function(){
		var btn = $(this);
		$.post(ajaxurl,{
			pointer: btn.attr('data-pointer-id'),
			action: 'dismiss-wp-pointer'
		});
		$(this).closest('.fdp-pro-notice').fadeOut(300);
	});
	$('#eos-dp-setts-nav-wrp').on('mouseenter',function(){
		var mainStyleLink = $('#fdp-main-style-prefetch');
		if(null === mainStyleLink || mainStyleLink.length < 1){
			$('<link id="fdp-main-style-prefetch" rel="prefetch" href="' + eos_dp_js.main_style + '" as="style" />').appendTo("body");
		}
	});
	if('function' === typeof($.fn.draggable)){
		jQuery('.fdp-draggable').css('cursor','move').draggable({cursorAt:{top:50,left:50 }});
	}
	...
	if(psiButtons && psiButtons.length > 0){
		setInterval(function(){
			...
			psiButtons.each(function(){
				...
				this.href = href + '%26eos_dp_preview%3D' + Date.now();
			});
		},5000);
	}
...
function eos_move_table_head(){
	if(null === table_head || parseInt(eos_dp_js.plugins_n) < 1) return;
	if(table && ('undefined' === typeof(is_single_post) || true !== is_single_post)){
		var prevDelta = 'undefined' !== typeof(window.fdp_delta) ? window.fdp_delta : -999999999,tr = table ? table.getElementsByTagName('tr') : false;
		window.fdp_delta = parseInt(eos_dp_js.plugins_n) > 1 && tr && tr.length > 0 ? tr[tr.length - 1].getBoundingClientRect().top - firstRowTop : 0;
		direction = window.fdp_delta > prevDelta ? 'up' : 'down';
		...
		if(window.scrollY > 150){
			...
			var ofs = document.getElementsByClassName('eos-dp-post-row')[0].getElementsByTagName('td')[0].clientWidth + 2.5,
		...
		success : function (response) {
			if(response) {
				...
				if('function' === typeof(jQuery.fn.draggable)){
					jQuery('#eos-dp-popup').css('cursor','move').draggable({cursorAt:{top:50,left:50 }});
				}
			}
				...
				if('error' !== response){
					...
					if(parseInt(window.eos_dp_autosuggest_counter) < Math.ceil(jQuery('.eos-dp-name-th').length/4)){
						...
						if('undefined' !== typeof(window.eos_dp_actual_row_in_progress) && null !== window.eos_dp_actual_row_in_progress){
							...
							var nextBtn = 'eos_dp_admin' !== eos_dp_js.page ? row.next().find('.eos-dp-pro-autosettings') : row.nextAll('.eos-dp-admin-row').not('.fdp-edit-single').first().find('.eos-dp-pro-autosettings');
							if(nextBtn.length > 0){
								nextBtn.trigger('click');
								window.eos_dp_autosuggest_counter = -1;
							}
							else{
...
function fdp_suggest_plugins(btn,data){
	var xhr = new XMLHttpRequest(),
		tr = jQuery(btn).closest('tr'),
		fd = new FormData(),nonce = 'undefined' !== typeof(eos_dp_js.page) && 'eos_dp_admin' === eos_dp_js.page ? document.getElementById('eos_dp_pro_auto_settings_admin').value : document.getElementById('eos_dp_pro_auto_settings').value;
	data['counter'] = window.eos_dp_autosuggest_counter;
	tr.find('td[data-path="' + data.plugins.split(',')[window.eos_dp_autosuggest_counter] + '"]').addClass('fdp-checking-this'),
	plugin_col = tr.find('td[data-path="' + data.plugins.split(',')[window.eos_dp_autosuggest_counter] + '"]');
	fd.append('data',JSON.stringify(data));
	fd.append('nonce',nonce);
	xhr.open("POST",eos_dp_js.ajaxurl + '?action=eos_dp_auto_settings',true);
	xhr.send(fd);
	xhr.onprogress = function(){
      ...
      if(parseInt(window.eos_dp_autosuggest_counter) < parseInt(eos_dp_js.plugins_n) + 1){
				if('' !== xhr.response){
					if('error' !== xhr.response){
						tr.find('td[data-path="' + JSON.parse(xhr.response)[0] + '"]').removeClass('eos-dp-active').find('input').attr('checked',1).trigger('change');
					}
					else{
						plugin_col.addClass('eos-dp-active').addClass('fdp-error').find('input').removeAttr('checked').trigger('change');
					}
				}
				else{
					plugin_col.addClass('eos-dp-active').find('innput').trigger('change');
				}
				plugin_col.removeClass('fdp-checking-this').addClass('eos-dp-autochecked');
				fdp_suggest_plugins(btn,data);
      }
      else{
				tr.removeClass('eos-test-in-progress').addClass('eos-post-locked');;
				if('undefined' !== typeof(window.eos_dp_actual_row_in_progress) && null !== window.eos_dp_actual_row_in_progress){
					++window.eos_dp_actual_row_in_progress;
					var nextBtn = 'eos_dp_admin' !== eos_dp_js.page ? tr.next().find('.eos-dp-pro-autosettings') : tr.nextAll('.eos-dp-admin-row').not('.fdp-edit-single').first().find('.eos-dp-pro-autosettings');
					if(nextBtn.length > 0){
						nextBtn.trigger('click');
						window.eos_dp_autosuggest_counter = -1;
					}
	...
	if(table_temp && '' !== table_temp){
		...
		jQuery('.eos-dp-post-row').each(function(n,r){
			jQuery(this).find('td').not('.eos-dp-post-name-wrp').each(function(idx,el){
				...
				if(1 ===  cols[idx]){
					jQuery(this).addClass('eos-dp-active').find('input').removeAttr('checked');
				}
				else{
					jQuery(this).removeClass('eos-dp-active').find('input').attr('checked',1);
				}
			});
		});
	}
...
function fdp_synchronize_dependencies(){
	if( 'undefined' !== typeof( window.fdp_bulk_selection ) && window.fdp_bulk_selection ) return;
	if('undefined' === typeof(eos_dp_js.dependencies) || ('undefined' !== typeof(window.fdp_inverting_selection) && true === window.fdp_inverting_selection)) return;
	var n=0,dependencies = JSON.parse(eos_dp_js.dependencies),single_column = 'undefined' !== typeof(eos_dp_js.page) && ('eos_dp_mobile' === eos_dp_js.page || 'eos_dp_desktop' === eos_dp_js.page || 'eos_dp_search' === eos_dp_js.page);
...
function fdp_update_parent(el,parent_plugin,string,single_column){
	if( 'undefined' !== typeof( window.fdp_bulk_selection ) && window.fdp_bulk_selection ) return;
	if('undefined' !== typeof(window.fdp_inverting_selection) && true === window.fdp_inverting_selection) return;
	if(parent_plugin !== el.dataset.path){
		if(-1 !== el.dataset.path.indexOf('-' + string) || -1 !== el.dataset.path.indexOf(string + '-')){
			var parent_plugin_el = single_column ? jQuery('[data-path="' + parent_plugin + '"]') : jQuery(el).closest('tr').find('[data-path="' + parent_plugin + '"]'),reverseCheck = 'undefined' !== typeof(eos_dp_js.page) && ('eos_dp_mobile' === eos_dp_js.page || 'eos_dp_desktop' === eos_dp_js.page || 'eos_dp_search' === eos_dp_js.page);
			if('undefined' !== typeof(el.type) && 'checkbox' === el.type && jQuery(el).closest('td').hasClass('eos-dp-active')){
				...
				if(reverseCheck){
					parent_plugin_el.prop('checked',true);
				}
				else{
				...
				if(reverseCheck){
					parent_plugin_el.find('input').prop('checked',true);
				}
				else{
...
function fdp_update_add_ons(el,parent_plugin,strings,single_column){
	if( 'undefined' !== typeof( window.fdp_bulk_selection ) && window.fdp_bulk_selection ) return;
	var reverseCheck = 'undefined' !== typeof(eos_dp_js.page) && ('eos_dp_mobile' === eos_dp_js.page || 'eos_dp_desktop' === eos_dp_js.page || 'eos_dp_search' === eos_dp_js.page);
	if('undefined' !== typeof(el.type) && 'checkbox' === el.type){
		if(!el.checked && !reverseCheck) return;
	}
	else{
...
function eos_cbi_copy_to_clipboard(text){
