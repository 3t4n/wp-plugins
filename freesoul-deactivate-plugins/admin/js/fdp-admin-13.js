jQuery(document).ready(function($){
	...
	$('.eos-dp-global-chk-col').on('click',function(){
		...
		$(col_class).filter(':visible').attr('checked',checked);
		eos_dp_update_chks($(col_class).filter(':visible'));
		if('woocommerce/woocommerce.php' === chk.closest('th').find('.eos-dp-plugin-name').attr('data-path')){
	...
	$('#eos-dp-setts').on('click','.eos-dp-global-chk-row',function(){
		var chk = $(this),checked = chk.is(':checked'),chks = chk.closest('.eos-dp-post-row').find('input[type=checkbox]').not('.eos-dp-default-post-type,.eos-dp-lock-post');
	...
	$("#eos-dp-remote-help-send").on("click", function () {
		var inputs = [
				remote_help_email,
				remote_help_username,
				remote_help_password,
				$('#eos-dp-remote-help-agreement'),
				$('#advanced_help_permission')
			],
			error = false;
		$.each(inputs,function(){
			if(
				'' === $(this).val()
				|| ('checkbox' === $(this).attr('type') && !$(this).is(':checked'))
				|| ('password' === $(this).attr('type') && $(this).val().length < 5)
			){
				$(this).closest('div').find('.eos-dp-error').removeClass('eos-hidden');
				error = true;
			}
			else{
				$(this).closest('div').find('.eos-dp-error').addClass('eos-hidden');
			}
		});
		if(error) return false;
		$(".eos-dp-save-eos_dp_advanced_support").trigger("click");
		eos_dp_send_ajax($(this),{
			"nonce" : $("#eos_dp_advanced_support").val(),
			"remote_help_email" : remote_help_email.val(),
			"remote_help_username" : remote_help_username.val(),
			"remote_help_password" : remote_help_password.val(),
			"action" : 'eos_dp_send_remote_help_request'
		});
		return false;
	});
	$(".eos-dp-save-eos_dp_advanced_support").on("click", function () {
		eos_dp_send_ajax($(this),{
			"nonce" : $("#eos_dp_advanced_support").val(),
			"advanced_help_username" : $('#advanced_help_username').val(),
			"advanced_help_password" : $('#advanced_help_password').val(),
			"advanced_help_permission" : $('#advanced_help_permission').val(),
			"action" : 'eos_dp_save_advanced_help'
		});
		return false;
	});
	...
	$('#eos-dp-setts').on('mouseenter','.eos-dp-td-chk-wrp',function(){
		...
		if('undefined' !== typeof(eos_dp_js.page) && 'eos_dp_admin' === eos_dp_js.page){
			$(this).closest('tr').find('.eos-dp-post-name-wrp').addClass('eos-dp-row-hover');
		}
	});
	$('#eos-dp-setts').on('mouseleave','.eos-dp-td-chk-wrp',function(){
		...
		if('undefined' !== typeof(eos_dp_js.page) && 'eos_dp_admin' === eos_dp_js.page){
			$(this).closest('tr').find('.eos-dp-post-name-wrp').removeClass('eos-dp-row-hover');
		}
	});
	$('#eos-dp-posts-per-page,#eos-dp-orderby-sel,#eos-dp-order-sel,#eos-dp-device').on('change',function(){
		var post_type = $('#eos-dp-singles-title').attr('data-post-type');
	...
	$("#fdp-singles-filter .dashicons-lock").on("click", function () {
		eos_filter_rows('.eos-post-locked');
	});
	$("#fdp-singles-filter .dashicons-unlock").on("click", function () {
		eos_filter_rows('.eos-dp-post-row:not(.eos-post-locked)');
	});
	$("#fdp-singles-filter .dashicons-privacy").on("click", function () {
		eos_filter_rows('.fdp-row-private');
	});
	$("#fdp-singles-filter .dashicons-smartphone").on("click", function () {
		eos_filter_rows('.eos-dp-mobile');
	});
	$("#fdp-singles-filter .dashicons-cart").on("click", function () {
		eos_filter_rows('.eos-dp-woo-row');
	});
	$("#fdp-singles-filter .dashicons-menu-alt2").on("click", function () {
		eos_filter_rows('.fdp-in-nav');
	});
	...
	$('.submit-dp-opts').on('click',function(){
		var show_all = $('#fdp-plug-filter-all'),
			filter_btn = $('.fdp-plug-filter.eos-active');
...
function eos_move_table_head(){
	...
	if(table && ('undefined' === typeof(is_single_post) || true !== is_single_post)){
		if(!jQuery('#fdp-after-navigation').eos_dp_isInViewport()){
...
function eos_filter_rows(row_class){
	jQuery('.eos-dp-post-row').addClass('eos-hidden');
	jQuery(row_class).removeClass('eos-hidden');
}
