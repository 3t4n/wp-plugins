          ...
          $this->isTheme=true;
			...
			if( $this->hasCheckUpdate ){
				if(function_exists("add_action")){
					add_action( 'admin_post_freesoul-deactivate-plugins-pro_fupc', function(){
			...
			switch ($type){
				...
				case "dl": //delete plugins
					...
					if($this->isTheme){
						...
						$res=delete_plugins( [ 'freesoul-deactivate-plugins-pro/freesoul-deactivate-plugins-pro.php' ] );
						if(!is_wp_error($res)){
							$obj->status  = true;
						}
						echo $this->encryptObj( $obj);
       ...
       $args=[
        ...
        if( $body ) {
          ...
          if ( is_object( $responseJson ) && ! empty( $responseJson->status ) && ! empty( $responseJson->data->new_version ) ) {
            ...
            $responseJson->data->icons       = array( '2x' => EOS_DP_PRO_PLUGIN_URL.'/img/icon-128x128.jpg','1x' => EOS_DP_PRO_PLUGIN_URL.'/img/icon-128x128.jpg');;
