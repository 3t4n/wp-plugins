jQuery(document).ready(function($){
	...
	$('.eos-dp-lock-post-wrp').on('click',function(event){
		...
		if($(this).parent().hasClass('eos-post-locked')){
			...
			$('#eos_dp_single_locked').val('locked');
			$('#eos-dp-post-types-override').addClass('eos-hidden');
		}
		else{
			...
			$.ajax({
				...
				data : {
						...
						"post_type" : eos_dp_js.post_type,
						"action" : 'eos_dp_check_single_padlock'
					},
				success : function (response) {
					if ('override' === response) {
						$('#eos-dp-post-types-override').removeClass('eos-hidden');
					}
