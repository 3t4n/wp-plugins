...
abstract class OPER_Emails_API extends OPER_Settings_API  {
...
//        $string = preg_replace($pattern,$newurl,$string);
    ...
    public function replace_shortcodes( $subject ) {
        ...
        $defaults = array(
		    ...
		    , 'sitelink'     => htmlspecialchars_decode( '<a href="' . home_url() . '">' . home_url() . '</a>' )
		    , 'remote_ip'    => oper_get_user_ip()                                                // The IP address from which the user is viewing the current page.
		    ...
		    , 'current_date' => date_i18n( get_oper_option( 'oper_date_format' ) )
		    , 'current_time' => date_i18n( get_oper_option( 'oper_time_format' ) )
        );
    ...
    public function get_headers( $additional_params = array() ) {
        ...
        $mail_headers = apply_filters( 'oper_email_api_get_headers_after', $mail_headers, $this->id, $this->fields_values , $this->replace, $additional_params );
    ...
    public function get_content() {
        ...
        $email_content = apply_filters( 'oper_email_api_get_content_after' , $email_content, $this->id, $this->get_email_content_type() );
        ...
        function get_content_plain() {
            ...
            $fields_values = apply_filters( 'oper_email_api_get_content_before' , $fields_values, $this->id , 'plain' );    //Ability to parse 'content', 'header_content', 'footer_content' for different languges, etc ....
            
            if (  function_exists( 'oper_email_template_' . $email_tpl_name . '_text' ) )
                $plain_email = call_user_func_array( 'oper_email_template_' . $email_tpl_name . '_text' , array( $fields_values ) );
            else 
                $plain_email = oper_email_template_text( $fields_values );
        ...
        function get_content_html() {
            ...
            $fields_values = apply_filters( 'oper_email_api_get_content_before' , $fields_values, $this->id , 'html' );    //Ability to parse 'content', 'header_content', 'footer_content' for different languges, etc ....

            if (  function_exists( 'oper_email_template_' . $email_tpl_name . '_html' ) )
                $html_email = call_user_func_array( 'oper_email_template_' . $email_tpl_name . '_html' , array( $fields_values ) );
            else 
                $html_email = oper_email_template_html( $fields_values );
    ...
    public function get_subject() {
        ...
        $subject = apply_filters( 'oper_email_api_get_subject_before' , $subject, $this->id );
        
        $subject = $this->esc_to_plain_text( $this->replace_shortcodes( $subject ) );
        
        $subject = apply_filters( 'oper_email_api_get_subject_after' , $subject, $this->id );
...
//debuge('In email', htmlentities($to), $subject, htmlentities($message), $headers, $attachments)  ;      
        ...
        $is_send_email = apply_filters( 'oper_email_api_is_allow_send', $is_send_email, $this->id, $this->fields_values );
        
		 if ( $is_send_email ) {
        	$to = apply_filters( 'oper_email_api_send_field_to', $to );                                                 //FixIn: 8.1.3.1
	        //FixIn: 8.5.2.22
	        if ( ! empty( $to ) ) {
        ...
        if (
            ...
            $subject = __('Email copy to', 'email-reminders') . ' ' . str_replace( array( '<', '>' ), '', $to) . ' - ' . $subject;
...
if ( ! empty( $this->fields_values['to'] ) ) {
	...
	if ( !is_array( $to_name ) ) $to_name = explode( ',', $to_name );
        ...
        $to = implode(',', $to);
            ...
            $is_send_email = apply_filters( 'oper_email_api_is_allow_send_copy', $is_send_email, $this->id, $this->fields_values );

            if ( $is_send_email ) {
            	$to = apply_filters( 'oper_email_api_send_copy_field_to', $to );                                        //FixIn: 8.1.3.1
	            //FixIn: 8.5.2.22
	            if ( ! empty( $to ) ) {
...
//debuge( $wp_error_object );
        if ( $this->sending ) {                                                 // Check  if this error generated for email relative to  this class.
            ...
            if ( ! empty( $error_description_array ) ) {
                ...
                do_action('oper_email_sending_error', $wp_error_object, $error_description );

            } else {
                do_action('oper_email_sending_error', $wp_error_object, '' );
            }
        }        
    }
