 ...
 * 
	<div>
		<input type="text" value="" wrap="off" placeholder="..." class="oper_file_urls" name="oper_file_urls" />
		<a href="javascript:void(0)" class="button oper_btn_upload"
		   data-modal_title="<?php echo esc_attr( __( 'Choose file', 'email-reminders' ) ); ?>"
			 data-btn_title="<?php echo esc_attr( __( 'Insert file URL', 'email-reminders' ) ); ?>"
		><?php _e( 'Upload File', 'email-reminders' ); ?></a>
	</div>
	<?php 

		$oper_upload = oper_upload();	// Get OPER_Upload obj instance

		$oper_upload->set_upload_button( '.oper_btn_upload' );

		$oper_upload->set_element_insert_url( '.oper_file_urls' );
	?> 
 * 
 */

// General Init Class    
final class OPER_Upload {

	public $settings = array(
							  'upload_button' => ''
							, 'element_insert_url' => ''
							, 'wp_media_uploader_params' => array( 'key' => 'oper_type', 'value' => 'oper_upload' )		// Required for setting OUR Dir for uploading and set it PROTECTED
				   );
	...
	public static function init() {
		...
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof OPER_Upload ) ) {
			
			self::$instance = new OPER_Upload;
			...
			//TODO: remove this
			// add_filter( 'posts_where', array( self::$instance, 'oper_filter_posts_where' ) );
			// add_action('pre_get_posts', array( self::$instance, 'oper_pre_get_posts' ) );
			
			// !!! If need to  use protected folder,  then  uncomment it !!!
	...
	/** Get Name of protected DIR name,  like oper_XXXXX
	 * 
	 * @return string
	 */
	public function get_protected_dir_name() {
		
		$get_protected_dir_name = get_oper_option( 'oper_protected_directory_name_level1' );
		
		if ( empty( $get_protected_dir_name ) ) {
			$get_protected_dir_name = 'oper_' . wp_generate_password( 20, false, false );
			update_oper_option( 'oper_protected_directory_name_level1', $get_protected_dir_name );
		}
...
//TODO: remove this	
/*	
	function oper_pre_get_posts( $query ) {
debuge_log( $_POST );
...
//TODO: remove this
	...
	*/
/*	
	function oper_filter_posts_where( $where = '' ) {
...
//debuge( maybe_unserialize( 'a:3:{s:6:"action";s:17:"query-attachments";s:7:"post_id";s:1:"0";s:5:"query";a:4:{s:7:"orderby";s:4:"date";s:5:"order";s:4:"DESC";s:14:"posts_per_page";s:2:"40";s:5:"paged";s:1:"1";}}' ));
		...
		if (       ( isset( $_POST['query'] ) )
			...
			global $wpdb;
			$where .= " AND guid LIKE '%".$wpdb->esc_like( untrailingslashit(  get_oper_option( 'oper_protected_directory_name_level1' ) ) )."%'";
		}
		return $where;
	}
*/
	 ...
	 * @param array $uploads Array of upload directory data:
															array (
																[path]		=> Z:\home\new\www/wp-content/uploads/oper_lSJacOT1yVLFnrkqt2xR/2017/04
																[url]		=> http://new/wp-content/uploads/oper_lSJacOT1yVLFnrkqt2xR/2017/04
															 
																[subdir]	=> /oper_lSJacOT1yVLFnrkqt2xR/2017/04
	 ...
	 * 
		*  May be we need to have 2 folders,  like /oper_xxxxx/XXXXXXXXXXXXX
		*  for prevent of dir listing at previous stage /oper_xxxxx with .htaccess file
		* 
	...
	<script type="text/javascript">
		...
		(function($){
			...
			jQuery( '<?php echo $this->get_settings('upload_button'); ?>' ).on( 'click', function( event ) {
				...
				var insert_field_separator = '<?php $oper_csv_separator = get_oper_option( 'oper_csv_separator' ); echo empty( $oper_csv_separator ) ? ',' : $oper_csv_separator; ?>';
				...
				// Create Media Frame
				///////////////////////////////////////////////////////////////////////
				oper_file_frame = wp.media.frames.oper_upload_file_frame = wp.media( {									// Check  here ../wp-includes/js/media-views.js
					...
					states: [
								new wp.media.controller.Library( {
										   ...
										   // We are checking in functon oper_filter_posts_where media files that  only  relative to  this medi Frame opening
				...
				///////////////////////////////////////////////////////////////////////
				// Set  custom parameters for uploader	->  $_POST['oper_type'] - checking in "upload_dir",  when filter_upload_dir
				///////////////////////////////////////////////////////////////////////
				oper_file_frame.on( 'ready', function () {
					oper_file_frame.uploader.options.uploader.params = {
						type: 'oper_download',
						<?php
						echo $media_uploader_params['key'] . ": '" . $media_uploader_params['value'] . "'";
						?>									
				...
				///////////////////////////////////////////////////////////////////////
				oper_file_frame.on( 'select', function () {

					if ( ! is_multi_selection ) { // Single file
						
						var attachment = oper_file_frame.state().get('selection').first().toJSON();

						// Put URL of file to text field
						j_btn.parent().find('<?php echo $this->get_settings('element_insert_url'); ?>').val( attachment.url );
						j_btn.parent().find('<?php echo $this->get_settings('element_insert_url'); ?>').trigger( "oper_upload_url_set" );

					} else { // Multiple files.
						
						var file_paths = '';
						var csv_data_line = '';
						oper_file_frame.state().get('selection').map( function ( attachment ) {
...
//console.log( attachment );
							...
							// 	csv_data_line = attachment.id + insert_field_separator + attachment.title  + insert_field_separator + attachment.oper_version_num  + insert_field_separator + attachment.description + insert_field_separator + attachment.url
						...
						j_btn.parent().parent().find('<?php echo $this->get_settings('element_insert_url'); ?>').val( file_paths );
						j_btn.parent().parent().find('<?php echo $this->get_settings('element_insert_url'); ?>').trigger( "oper_upload_url_set" );

						//jQuery( '#oper_products_csv_text' ).val( file_paths + "\n\n" + jQuery( '#oper_products_csv_text' ).val() );
					}
...
if (0) {							
				...
				* @returns {undefined}				 
				*/
				function oper_remove_media_element_from_container( my_model_obj , delay_time ) {
				
					/** Attributes:
						'id': 112
						'title': __71
						'filename': 71.zip 
						'url': http://server.com/wp-content/uploads/oper_lSJacOT1yVLFnrkqt2xR/2017/04/71.zip'
				...
				wp.media.model.Attachment.get("collection").collection.on( 'change', function( my_model_obj ) {
					oper_remove_media_element_from_container( my_model_obj , 1);
				});
				
				// Fires,  when  Content redraw
				oper_file_frame.on( 'content:activate:browse', function(){
					
						var oper_models = wp.media.model.Attachment.get("collection").collection.models;

						_.each( oper_models, function( my_model_obj, ind) {
...
/*
	...
	oper_file_frame.on( 'activate', function() { alert('activate'); } );

	// Fires after the frame markup has been built, but not appended to the DOM.
	// @see wp.media.view.Modal.attach()
	oper_file_frame.on( 'ready', function() { alert('ready'); } );

	// Fires when the frame's $el is appended to its DOM container.
	// @see media.view.Modal.attach()
	oper_file_frame.on( 'attach', function() { alert('attach'); } );

	// Fires when the modal opens (becomes visible).
	// @see media.view.Modal.open()
	oper_file_frame.on( 'open', function() { alert('open'); } );

	// Fires when the modal closes via the escape key.
	// @see media.view.Modal.close()
	oper_file_frame.on( 'escape', function() { alert('escape'); } );

	// Fires when the modal closes.
	// @see media.view.Modal.close()
	oper_file_frame.on( 'close', function() { alert('close'); } );

	// Fires when a user has selected attachment(s) and clicked the select button.
	// @see media.view.MediaFrame.Post.mainInsertToolbar()
	oper_file_frame.on( 'select', function() {
		var selectionCollection = oper_file_frame.state().get('select');
	} );

	// Fires when a mode is deactivated on a region { 'menu' | title | content | toolbar | router }
	oper_file_frame.on( 'content:deactivate', function() { alert('{region}:deactivate'); } );
	// and a more specific event including the mode.
	oper_file_frame.on( 'content:deactivate:{mode}', function() { alert('{region}:deactivate{mode}'); } );

	// Fires when a region is ready for its view to be created.
	oper_file_frame.on( 'content:create', function() { alert('{region}:create'); } );
	// and a more specific event including the mode.
	oper_file_frame.on( 'content:create:{mode}', function() { alert('{region}:create{mode}'); } );

	// Fires when a region is ready for its view to be rendered.
	oper_file_frame.on( 'content:render', function() { alert('{region}:render'); } );
	// and a more specific event including the mode.
	oper_file_frame.on( 'content:render:{mode}', function() { alert('{region}:render{mode}'); } );

	// Fires when a new mode is activated (after it has been rendered) on a region.
	oper_file_frame.on( 'content:activate', function() { alert('{region}:activate'); } );
	// and a more specific event including the mode.
	oper_file_frame.on( 'content:activate:{mode}', function() { alert('{region}:activate{mode}'); } );

	// Get an object representing the current state.
	//oper_file_frame.state();

	// Get an object representing the previous state.
	//oper_file_frame.lastState();
*/
				
if(0) {
// Debuge all events from  media Frame!
oper_file_frame.on("all", function(eventName) {
 ...
 *
 * Example: <?php $oper_upload = oper_upload(); ?>
 */
function oper_upload() {
    return OPER_Upload::init();
}
oper_upload();		// Start