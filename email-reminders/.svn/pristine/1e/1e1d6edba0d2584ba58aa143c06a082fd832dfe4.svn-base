...
abstract class OPER_Page_Structure {
    ...
    public function __construct() {
        ...
        add_action('oper_define_nav_tabs', array( $this, 'oper_define_nav_tabs' ) );            // This Hook fire after creation menu in class OPER_Admin_Menus

        add_action('oper_page_structure_show', array( $this, 'content_structure' ) );           // This Hook fire in the class OPER_Admin_Menus for showing page content of specific menu
    }
     ...
     * Example:
        ...
        $tabs = array();
        $tabs[ 'form' ] = array(
                              'title' => __('Form', 'email-reminders')                   // Title of TAB
                            , 'hint' => __('Customizaton of Form Fields', 'email-reminders')            // Hint
                            , 'page_title' =>ucwords( __('Form fields', 'email-reminders') )     // Title of Page
        ...
        $subtabs['fields'] = array( 
                            ...
                            , 'title' => __('Form', 'email-reminders')                   // Title of TAB
                            , 'page_title' => __('Form Settings', 'email-reminders')                            // Title of Page
                            , 'hint' => __('Customization of Form Settings', 'email-reminders')                // Hint
                            , 'link' => ''                                      // link
                            , 'position' => ''                                  // 'left'  ||  'right'  ||  ''
                            , 'css_classes' => ''                               // CSS class(es)
        ...
        $subtabs['form-goto'] = array( 
                            ...
                            , 'title' =>ucwords( __('Form fields', 'email-reminders') )   // Title of TAB
                            , 'hint' => ''                                      // Hint    
        ...
        $subtabs['form-save'] = array( 
                            ...
                            , 'title' => __('Save Changes', 'email-reminders')           // Title of TAB
                            , 'form' => 'oper_form'                             // Required for 'button'!  Name of Form  to submit
                        );
    ...
    */
     ...
     *   Exmaple:
        ...
        $this_submit_form  = 'oper_emails_toolbar';                             // Define form name
        
        if ( isset( $_POST['is_form_sbmitted_'. $this_submit_form ] ) ) {

            // Check   N o n c e
            check_admin_referer( 'oper_settings_page_' . $this_submit_form  );  // Its stop show anything on submiting, if its not refear to the original page

            // Make Update of settings 
            $edit_field_data = $this->update_oper_emails_toolbar( $menu_slug );
        } 
        ////////////////////////////////////////////////////////////////////////
     */
    ...
    public function content_structure( $page_tag ) {    
        ...
        $is_show_this_page = apply_filters( 'oper_before_showing_settings_page_is_show_page', true, $page_tag, $active_page_tab, $active_page_subtab  );      // Fires Before showing settings Content page
        
        if ( $is_show_this_page === false ) return  false;   
        
        do_action( 'oper_before_settings_content', $page_tag, $active_page_tab, $active_page_subtab  );                 // Fires Before showing settings Content page
        
        ?><div id="<?php echo $page_tag; ?>-admin-page" class="wrap oper_page"><?php
			do_action( 'oper_settings_before_header', $page_tag, $active_page_tab, $active_page_subtab  );				// FixIn: N
			?>            
            <h1 class="oper_header"><div class="oper_header_icon"></div><?php echo $this->get_page_header_h1(); ?> <!--sup style="margin: 0 0.5em;font-size: 0.7em;">&beta;eta</sup--></h1><?php
			do_action( 'oper_settings_after_header', $page_tag, $active_page_tab, $active_page_subtab  );				// FixIn: N
			?>
            <div class="oper_admin_message"></div>
            <div class="oper_admin_page">
                <div id="ajax_working"></div>
                <div class="clear oper_header_margin"></div>
                ...
                wp_nonce_field('oper_ajax_admin_nonce',  "oper_admin_panel_nonce" ,  true , true );
                ...
                do_action('oper_show_settings_content' , $page_tag, $active_page_tab, $active_page_subtab );
            ?></div>
        </div><?php    
        
        do_action( 'oper_after_settings_content', $page_tag, $active_page_tab, $active_page_subtab );                  // Fires After showing settings Content page
    }
...
//debuge($this_page, $_REQUEST);            
     ...
     */
    public function oper_define_nav_tabs() {                                    // Function executed after creation menu in class OPER_Admin_Menus
/*
  Array (
            [oper-resources] => Array ()
            [oper-settings] => Array
                (
                    [general] => Array
                        (
 ...
 */        
    ...
    public function show_tabs_structure( $menu_in_page_tag ) {
        ...
        oper_bs_toolbar_tabs_html_container_start();
        
        do_action( 'oper_toolbar_top_tabs_before' , $menu_in_page_tag );
        
        $this->show_tabs_line( $menu_in_page_tag );                     // T O P    T A B S                              
        
        do_action( 'oper_toolbar_top_tabs_after' , $menu_in_page_tag );
        
        oper_bs_toolbar_tabs_html_container_end();
            ...
            switch ( $tab['type'] ) {
                ...
                case 'button':                              // Button
                   ?><div class="clear-for-mobile"></div><input 
                            type="button" 
                            class="button button-primary oper_submit_button"
                            value="<?php echo $tab['title']; ?>" 
                            onclick="if (typeof document.forms['<?php echo $tab['form']; ?>'] !== 'undefined'){ document.forms['<?php echo $tab['form']; ?>'].submit(); } else { oper_admin_show_message( '<?php echo ' <strong>', __('Error', 'email-reminders') ,'!</strong> ', __('Form', 'email-reminders') , ' <strong>' , $tab['form'] , '</strong> ', __('does not exist', 'email-reminders'); ?>.', 'error', 10000 ); }"
                            /><?php
                break;
                ...
                case 'goto-link':
                    ?><a    class="nav-tab wpdevelop-submenu-tab go-to-link" 
                            ...
                            onclick="javascript:oper_scroll_to('#<?php echo esc_js( $tab['show_section'] ); ?>' );"
