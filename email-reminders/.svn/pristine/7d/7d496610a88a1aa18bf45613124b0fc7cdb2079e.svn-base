...
class OPER_Admin_Menus {
    ...
    protected $mune_icon_url;                                                   // Icon - if used 'none', then you can define it in the CSS backgound: .toplevel_page_oper .wp-menu-image { background-image: url("../img/icon-16x16.png"); background-position: 7px 7px; }   But its mean that  you are need to load this CSS in every admin page.
    ...
    function __construct( $slug, $param = array() ) {
        ...
        $this->in_menu = false;
        $this->menu_tag = $slug;                                                // For exmaple: 'oper-settings' - its 'page' parameter in URL
    ...
    public function load_js() {
        do_action( 'oper_load_js_on_admin_page' );
    }

    
    public function load_css() {
        do_action( 'oper_load_css_on_admin_page' );
    }
    ...
    public function new_admin_page(){
        ...
        if ( $this->in_menu == 'root' ) {    // Main Menu
            ...
            $page = $this->create_plugin_menu( 
                                    ...
                                    , $this->menu_tag                           // Slug     // I was used early -> plugin_basename(OPER_FILE).'oper'
                                    , $this->mune_icon_url                      // Icon     // - if used 'none', then you can define it in the CSS backgound: .toplevel_page_oper .wp-menu-image { background-image: url("../img/icon-16x16.png"); background-position: 7px 7px; }   But its mean that  you are need to load this CSS in every admin page.
        ...
        //do_action('oper_define_settings_pages', $this->menu_tag );              // Define sub classes, like: page-settings-general.php      $this->menu_tag - 'oper-settings' - its 'page' parameter in URL
        
        do_action('oper_menu_created',    $this->menu_tag );                    // Define sub classes, like: page-settings-general.php      $this->menu_tag - 'oper-settings' - its 'page' parameter in URL
        do_action('oper_define_nav_tabs', $this->menu_tag );                    // Define Nav tabs.
     ...
     * 
         Define page   S t r u c t u r e,   nav   T A B s ,  set   N O N C E:   oper_ajax_admin_nonce field
     
         in ..\any\class\class-admin-page-structure.php 
         
         then show page C O N T E N T in files, like page-structure.php
         
         $this->menu_tag - 'oper-settings' - its 'page' parameter in URL
     */
    public function content() {
        do_action('oper_page_structure_show', $this->menu_tag );
    }
    ...
    protected function create_plugin_menu($menu_page_title, $menu_title, $capability, $menu_slug, $mune_icon_url, $page_content ) {
	    ...
	    if ( '<svg' == substr( $mune_icon_url, 0, 4 ) ) {
			...
			*/
		    ...
		    $mune_icon_url = plugins_url( $mune_icon_url, OPER_FILE );
	    }
        ...
        $page = add_menu_page(  $menu_page_title,                               // The text to be displayed in the title tags of the page when the menu is selected
                                ...
                                $mune_icon_url//(($mune_icon_url=='none')?'none':((empty($mune_icon_url))?'':plugins_url($mune_icon_url, OPER_FILE)) )          // The url to the icon to be used for this menu. Using 'none' would leave div.wp-menu-image empty so an icon can be added as background with CSS.
