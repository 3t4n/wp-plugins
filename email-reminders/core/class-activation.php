...
abstract class OPER_Install {
    ...
    function __construct() {
        ...
        $default_init_option_names = array(
                            'option-version_num'                   => 'opl_pluginname_version_num'
                          , 'option-is_delete_if_deactive'         => 'opl_pluginname_is_delete_if_deactive'
                          , 'option-activation_process'            => 'opl_pluginname_activation_process'
                          , 'transient-oper_activation_redirect' => '_opl_pluginname_activation_redirect'
        ...
        register_activation_hook(   OPER_FILE, array( $this, 'oper_activate_initial' ) );                  // WordPress > Plugins > "Activate" link.

        register_deactivation_hook( OPER_FILE, array( $this, 'oper_deactivate' ) );                        // WordPress > Plugins > "Deactivate" link.

        add_filter('upgrader_post_install',    array( $this, 'oper_install_in_bulk_upgrade' ), 10, 2 );    // Upgrade during bulk upgrade of plugins
    ...
    */
    ...
    function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $context ) {
        ...
        $this_plugin = plugin_basename( OPER_FILE );

        if ($plugin_file == $this_plugin ) {

            $is_delete_if_deactive =  get_oper_option( $this->init_option['option-is_delete_if_deactive'] ); // check
            ...
            [$plugin_file] => opl_pluginname/OPER.php
            [$plugin_data] => Array
                (
                    [Name] => pluginname
                    [PluginURI] => http://oplugins.com/plugins/email-reminders/demo/
                    [Version] => 2.8.35
    ...
    private function check_if_need_to_update() {
        ...
        if( is_admin() ) {
            ...
            $oper_version_num = get_option( $this->init_option['option-version_num'] );

            if ($oper_version_num === false )
                $oper_version_num = '0';

            $is_make_activation = false;

            if ( version_compare( OPER_VERSION_NUM, $oper_version_num) > 0 ) {
            ...
            if ( $is_make_activation ) {
                add_action( 'plugins_loaded', array( $this, 'oper_activate_initial' ) , 1030 );
            }
        }
    }
     ...
     * @return type
     */
    public function oper_install_in_bulk_upgrade( $return, $hook_extra ){
     ...
     * @return type
     */
    public function oper_activate_initial(){

        // Activate the plugin
        $this->oper_activate();

        // Bail if this demo or activating from network, or bulk
        if ( is_network_admin() || isset( $_GET['activate-multi'] ) || oper_is_this_demo() )
                return;

        // Add the transient to redirect - Showing Welcome screen
        set_transient( $this->init_option['transient-oper_activation_redirect'], true, 30 );
    }
    ...
    public function oper_activate() {
        ...
        update_oper_option( $this->init_option['option-activation_process'], 'On' );

        make_oper_action( 'oper_activation' );                                    //  S T A R T

        update_oper_option( $this->init_option['option-version_num'], OPER_VERSION_NUM );

        update_oper_option( $this->init_option['option-activation_process'], 'Off');
    }


    /** Run Deactivate */
    public function oper_deactivate() {
        ...
        if ( $is_delete_if_deactive == 'On' ) {
            ...
            make_oper_action( 'oper_deactivation' );                              //  F I N I S H

            delete_oper_option( $this->init_option['option-version_num'] );

            delete_oper_option( $this->init_option['option-activation_process'] );
        }
    }
