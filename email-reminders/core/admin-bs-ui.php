 ...
 * @param array $item
                        array( 
                              ...
                              , 'hint' => ''                      // , 'hint' => array( 'title' => __('Select status' , 'email-reminders') , 'position' => 'bottom' )
...
function oper_bs_button( $item ) {
    ...
    ?><a  class="button <?php 
          ...
          <?php } ?>  
          <?php echo oper_get_custom_attr( $item_params ); ?>
        ><?php
              $btn_icon = '';
              if ( ! empty( $item_params['icon'] ) ) {                                

                  if ( substr( $item_params['icon'], 0, 4 ) != 'http') $img_path = OPER_PLUGIN_URL . '/assets/img/' . $item_params['icon'];
        ...
        foreach ( $item_params['options'] as $option_value => $option_data ) {
            ...
            if ( $option_data['optgroup'] ) {                                   // OPTGROUP
                ...
                ?><option 
                    ...
                    <?php echo oper_get_custom_attr( $option_data ); ?>
 ...
 */
function oper_bs_radio( $item ) {
    $item['type'] = 'radio';
    oper_bs_checkbox( $item );
}
 ...
 */
function oper_bs_checkbox( $item ) {
 ...
 */
function oper_bs_dropdown( $item ) {
        
    $default_item_params = array( 
                          'id' => ''                                            // HTML ID  of element
                        , 'default' => ''                                       // Some Value from optins array that selected by default
                        , 'hint' => ''                                          // Hint // array( 'title' => __('Delete' , 'email-reminders') , 'position' => 'bottom' )
    ...
    if ( empty( $params['id'] ) ) $params['id'] = 'oper_bs_dropdown' . $milliseconds;
    ...
    ?><a href="javascript:void(0)" 
         ...
         <?php echo oper_get_custom_attr( $params ); ?>
         ><?php 

            $btn_icon = '';
            if ( ! empty( $params['icon'] ) ) {                                

                if ( substr( $params['icon'], 0, 4 ) != 'http') $img_path = OPER_PLUGIN_URL . '/assets/img/' . $params['icon'];
        ...
        ?><ul   
            ...
            oper_bs_list_of_options_for_dropdown( $params , 'simple' , $is_this_simple_list);
            
        ?></ul><?php 
}


function oper_bs_list_of_options_for_dropdown( $params, $dropdown_type = 'simple', $is_this_simple_list = true ) {
                        ...
                        foreach ( $value as $item_params ) {
                            ...
                            switch ( $item_type ) {
                                ...
                                case 'button':
                                    ...
                                    $default_item_params = array( 
                                                        'action' => "oper_close_dropdown_selectbox( '" . $params['id'] . "' )" // Close list and uncheck  any checkboxes or radio boxes in dropdown list
                                                        ); 

                                    $item_params = wp_parse_args( $item_params, $default_item_params );

                                    oper_bs_button( $item_params );
                                    break;
                              ...
                              <?php 
                                if ( in_array( $value,  $params['disabled'] ) !== true ) { 
                                    ...
                                    if ( $dropdown_type == 'list' ) { ?>
                                                ...
                                                onclick="javascript:
                                                                    oper_show_selected_in_dropdown('<?php echo $params['id']; ?>', jQuery(this).html(), '<?php echo $value; ?>');
                                                            <?php if (! $is_this_simple_list ) { ?>
                                                                    oper_close_dropdown_selectbox( '<?php echo $params['id']; ?>' );
                                                            <?php } ?>"                                  
                                    <?php } else { ?>
 ...
 * 
    ...
    $params = array(  
                    ...
                    , 'label' => __('Actions:', 'email-reminders')                  // Label above the button group
                    , 'style' => ''                                         // CSS Style of entire div element
                    , 'items' => array(
                                        array(                                                 
                                              'type' => 'button' 
                                            , 'title' => __('Delete', 'email-reminders')    // Title of the button
                                            , 'hint' => array( 'title' => __('Delete' , 'email-reminders') , 'position' => 'bottom' ) // Hint
                                            , 'link' => 'javascript:void(0)'        // Direct link or skip  it
                                            , 'action' => ''                // Some JavaScript to execure, for example run  the function
                                        ...
                                        , array( 
                                            'type' => 'button'
                                            , 'title' => __('Delete', 'email-reminders')
    ...
    oper_bs_button_group( $params );
 
 */
function oper_bs_button_group( $args = array() ) {
 ...
 * 
    ...
    $params = array(  
                    ...
                    , 'label' => __('Min / Max cost:', 'email-reminders')           // Label above the input group
                    , 'style' => ''                                         // CSS Style of entire div element
                    , 'items' => array(
                                        array(      
                                            'type' => 'addon' 
                                            , 'element' => 'text'       // text | radio | checkbox
                                            , 'text' => __('Cost', 'email-reminders') . ':'
                                            , 'class' => ''                 // Any CSS class here
                            ...
                            , array(    
                                ...
                                , 'placeholder' => __('Reason of Cancelation', 'email-reminders')
                                ...
                                , 'attr' => array()             // Any  additional attributes, if this radio | checkbox element 
                                        ...
                                        , array(    
                                            ...
                                            , 'value' => ''                 // Value of Text field
                                            , 'placeholder' => __('Max Cost', 'email-reminders')
                                            , 'style' => ''                 // CSS of select element
                                            , 'class' => ''                 // CSS Class of select element
                                            , 'attr' => array()             // Any  additional attributes, if this radio | checkbox element 
                            ...
                            , array(    
                                ...
                                , 'options' => array( 'delete' => __('Delete', 'email-reminders'), 'any' => __('Any', 'email-reminders'), 'specific' => __('Specific', 'email-reminders')  )      // Associated array  of titles and values
                                ...
                                , array(                                            
                                    ...
                                    , 'options' => array(                       // Associated array  of titles and values   
                                                          'info' => array(  
                                                                        'title' => __('General Info', 'email-reminders')
                                                                        , 'id' => ''   
                                                                        , 'name' => ''  
                                                                        , 'style' => ''    
                                                                        , 'class' => ''     
                                                        ...
                                                        , 'optgroup_sf_s' => array( 
                                                                        'optgroup' => true
                                                                        , 'close'  => false
                                                                        , 'title'  => '&nbsp;' . __('Standard Fields' , 'email-reminders')
                                                                    )
                                                        , 'text' => array(  
                                                                        'title' => __('Text', 'email-reminders')
                                                                        , 'id' => ''   
                                                                        , 'name' => ''  
                                                                        , 'style' => ''
                                                                        , 'class' => ''     
                                                        ...
                                                        , 'select' => array(  
                                                                        'title' => __('Select', 'email-reminders')
                                                                        , 'id' => ''   
                                                                        , 'name' => ''  
                                                                        , 'style' => ''
                                                                        , 'class' => ''     
                                                        ...
                                                        , 'textarea' => array(  
                                                                        'title' => __('Textarea', 'email-reminders')
                                                                        , 'id' => ''   
                                                                        , 'name' => ''  
                                                                        , 'style' => ''
                                                                        , 'class' => ''     
                                                        ...
                                                        , 'checkbox' => array(  
                                                                        'title' => __('Checkbox', 'email-reminders')
                                                                        , 'id' => ''   
                                                                        , 'name' => ''  
                                                                        , 'style' => ''
                                                                        , 'class' => ''     
                                                        ...
                                                        , 'optgroup_af_s' => array( 
                                                                        'optgroup' => true
                                                                        , 'close'  => false
                                                                        , 'title'  => '&nbsp;' . __('Advanced Fields' , 'email-reminders')
                                                                    )
                                                        , 'info_advanced' => array(  
                                                                        'title' => __('Info', 'email-reminders')
                                                                        , 'id' => ''   
                                                                        , 'name' => ''  
                                                                        , 'style' => ''
                                                                        , 'class' => ''     
                                    ...
                                    , 'value' => ''                             // Some Value from optins array that selected by default                                                                              
                                    , 'onfocus' => ''
                                    , 'onchange' => "jQuery('.oper_field_generator').hide();jQuery('.oper_field_generator_' + this.options[this.selectedIndex].value ).show();"
                                )
                                        , array(                                                 
                                              'type' => 'button' 
                                            , 'title' => __('Delete', 'email-reminders')    // Title of the button
                                        ...
                                        , array( 
                                            ...
                                            , 'font_icon' => 'glyphicon glyphicon-trash'
                                            , 'icon_position' => 'left'
                                            , 'action' => "jQuery('#opl_pluginname_filters_formID').trigger( 'submit' );"
                                        )
                    )
              );     
    ?><div class="control-group oper-no-padding" style="max-width:730px;"><?php
            oper_bs_input_group( $params );
    ?></div><?php

 */
function oper_bs_input_group( $args = array() ) {
    ...
    ?><div class="oper-no-padding" style="width:auto;<?php echo $params['style']; ?>" ><?php
        if (! empty( $params['label'] ) ) { 
            ?><label for="<?php echo $params['label_for']; ?>" class="control-label"><?php echo $params['label']; ?></label><?php
        } 
 ...
 * @param array $args = array(
                                ...
                                , 'hint' => ''                                          // (optional) Hint: array( 'title' => __('Delete' , 'email-reminders') , 'position' => 'bottom' )
                                ...
                                , 'options' => array(                                   // Associated array  of titles and values 
                                                        __('Bla bla bla', 'email-reminders') => '1'
                                                        , 'divider0' => 'divider'
                                                        , __('Section Header', 'email-reminders') => 'header'
                                                            ...
                                                        ...
                                                        , '_buttons' => array( 
                                                                                    array(  'type' => 'button' 
                                                                                          ...
                                                                                          , 'hint' => ''                      // , 'hint' => array( 'title' => __('Select status' , 'email-reminders') , 'position' => 'bottom' )
                                                                                          , 'link' => 'javascript:void(0)'    // Direct link or skip  it
                                                                                          , 'action' => "oper_close_dropdown_selectbox( '" . $params['id'] . "' )" // Some JavaScript to execure, for example run  the function
                                                                                          , 'class' => 'button-secondary'     // button-secondary  | button-primary
                                                                                          , 'icon' => ''
                                                                                          , 'font_icon' => ''
                                                                                          , 'icon_position' => 'left'         // Position  of icon relative to Text: left | right
 ...
 * 
 Exmaple #1:
            $params = array(                                                    // Pending, Active, Suspended, Terminated, Cancelled, Fraud 
                            'id' => 'wh_approved'
                            , 'options' => array (
                                                    __('Pending', 'email-reminders') => '0',
                                                    __('Approved', 'email-reminders') => '1',
                                                    , 'divider0' => 'divider'
                                                    , __('Section Header', 'email-reminders') => 'header'
                                                    , __('Any', 'email-reminders') => ''
                                                 )
                            , 'disabled' => array( '' )                         // It will disable "Any" option    
                            , 'default' => ( isset( $_REQUEST[ 'wh_approved' ] ) ) ? sanitize_text_field( $_REQUEST[ 'wh_approved' ] ) : ''
                            , 'label' => ''//__('Status', 'email-reminders') . ':'
                            , 'title' => __('Items', 'email-reminders')
                        );            
                                                  
            oper_bs_dropdown_list( $params );

 Exmaple #2:
            $dates_interval = array(  
                                        1 => '1' . ' ' . __('day' , 'email-reminders')
                                      , 2 => '2' . ' ' . __('days' , 'email-reminders')
                                      , 3 => '3' . ' ' . __('days' , 'email-reminders')
                                      , 4 => '4' . ' ' . __('days' , 'email-reminders')
                                      , 5 => '5' . ' ' . __('days' , 'email-reminders')
                                      , 6 => '6' . ' ' . __('days' , 'email-reminders')
                                      , 7 => '1' . ' ' . __('week' , 'email-reminders')
                                      , 14 => '2' . ' ' . __('weeks' , 'email-reminders')
                                      , 30 => '1' . ' ' . __('month' , 'email-reminders')
                                      , 60 => '2' . ' ' . __('months' , 'email-reminders')
                                      , 90 => '3' . ' ' . __('months' , 'email-reminders')
                                      , 183 => '6' . ' ' . __('months' , 'email-reminders')
                                      , 365 => '1' . ' ' . __('Year' , 'email-reminders')
                                );
            $params = array(                                                    
                              'id'  => 'wh_opl_pluginname_date'
                            , 'id2' => 'wh_opl_pluginname_date2'
                            , 'default' =>  ( isset( $_REQUEST[ 'wh_opl_pluginname_date' ] ) ) ? sanitize_text_field( $_REQUEST[ 'wh_opl_pluginname_date' ] ) : ''
                            , 'default2' => ( isset( $_REQUEST[ 'wh_opl_pluginname_date2' ] ) ) ? sanitize_text_field( $_REQUEST[ 'wh_opl_pluginname_date2' ] ) : ''
                            , 'hint' => array( 'title' => __('Filter itenms by  dates' , 'email-reminders') , 'position' => 'top' )
                            , 'label' => ''//__('Booked Dates', 'email-reminders') . ':'
                            , 'title' => __('Dates', 'email-reminders')
                            , 'options' => array (
                                                      __('Current dates' , 'email-reminders')    => '0'
                                                    , __('Today' , 'email-reminders')            => '1'
                                                    , __('Previous dates' , 'email-reminders')   => '2'
                                                    , __('All dates' , 'email-reminders')        => '3'
                                                    , 'divider1' => 'divider'
                                                    , __('Today check in/out' , 'email-reminders')   => '9'
                                                    , __('Check In - Tomorrow' , 'email-reminders')  => '7'
                                                    , __('Check Out - Tomorrow' , 'email-reminders') => '8'
                                                    , 'divider2' => 'divider'                                
                                                    , 'next' => array(  
                                                                        array(
                                                                                'type' => 'radio'                                                        
                                                                              , 'label' => __('Next' , 'email-reminders')
                                                                              , 'id' => 'wh_opl_pluginname_datedays_interval1' 
                                                                              , 'name' => 'wh_opl_pluginname_datedays_interval_Radios'                                                                                            
                                                                              , 'style' => ''                     // CSS of select element
                                                                              , 'class' => ''                     // CSS Class of select element                                                                                
                                                                              , 'disabled' => false
                                                                              , 'attr' => array()                 // Any  additional attributes, if this radio | checkbox element 
                                                                              ...
                                                                              , 'selected' => ( isset($_REQUEST[ 'wh_opl_pluginname_datedays_interval_Radios'] ) 
                                                                                                && ( $_REQUEST[ 'wh_opl_pluginname_datedays_interval_Radios'] == '4' ) ) ? true : false
                                                                              )
                                                                        , array( 
                                                                                'type' => 'select'
                                                                              , 'attr' => array() 
                                                                              , 'name' => 'wh_opl_pluginname_datenext'
                                                                              , 'id' => 'wh_opl_pluginname_datenext'                                                                                                                                                                                                                                                                                
                                                                              , 'options' => $dates_interval
                                                                              , 'value' => isset( $_REQUEST[ 'wh_opl_pluginname_datenext'] ) ? sanitize_text_field( $_REQUEST[ 'wh_opl_pluginname_datenext'] ) : ''
                                                                              )                                                        
                                                                     ) 
                                                    , 'prior' => array( 
                                                                        array( 
                                                                                'type' => 'radio'
                                                                              , 'label' => __('Prior' , 'email-reminders')
                                                                              , 'id' => 'wh_opl_pluginname_datedays_interval2' 
                                                                              , 'name' => 'wh_opl_pluginname_datedays_interval_Radios'                                                                                            
                                                                              , 'style' => ''                     // CSS of select element
                                                                              , 'class' => ''                     // CSS Class of select element                                                                                
                                                                              , 'disabled' => false
                                                                              ...
                                                                              , 'selected' => ( isset($_REQUEST[ 'wh_opl_pluginname_datedays_interval_Radios'] ) 
                                                                                                && ( $_REQUEST[ 'wh_opl_pluginname_datedays_interval_Radios'] == '5' ) ) ? true : false
                                                                              )
                                                                        , array( 
                                                                                'type' => 'select'
                                                                              , 'attr' => array() 
                                                                              , 'name' => 'wh_opl_pluginname_dateprior'
                                                                              , 'id' => 'wh_opl_pluginname_dateprior'                                                                                                                                                                                                                                                                                
                                                                              , 'options' => $dates_interval
                                                                              , 'value' => isset( $_REQUEST[ 'wh_opl_pluginname_dateprior'] ) ? sanitize_text_field( $_REQUEST[ 'wh_opl_pluginname_dateprior'] ) : ''
                                                                              )                                                        
                                                                     )                                        
                                                    , 'fixed' => array( array(  'type' => 'group', 'class' => 'input-group text-group'), 
                                                                        array( 
                                                                                'type' => 'radio'
                                                                              , 'label' => __('Dates' , 'email-reminders')
                                                                              , 'id' => 'wh_opl_pluginname_datedays_interval3' 
                                                                              , 'name' => 'wh_opl_pluginname_datedays_interval_Radios'                                                                                            
                                                                              , 'style' => ''                     // CSS of select element
                                                                              , 'class' => ''                     // CSS Class of select element                                                                                
                                                                              , 'disabled' => false
                                                                              ...
                                                                              , 'selected' => ( isset($_REQUEST[ 'wh_opl_pluginname_datedays_interval_Radios'] ) 
                                                                                                && ( $_REQUEST[ 'wh_opl_pluginname_datedays_interval_Radios'] == '6' ) ) ? true : false
                                                                              )
                                                                        , array(                                                                                 
                                                                                'type'          => 'text' 
                                                                                , 'id'          => 'wh_opl_pluginname_datefixeddates'  
                                                                                , 'name'        => 'wh_opl_pluginname_datefixeddates'  
                                                                                , 'label'       => __('Check-in' , 'email-reminders') . ':'
                                                                                , 'disabled'    => false
                                                                                , 'class'       => 'wpdevbk-filters-section-calendar'           // This class add datepicker
                                                                                , 'style'       => ''
                                                                                , 'placeholder' => date( 'Y-m-d' )                                                                                                                                   
                                                                                , 'attr'        => array()    
                                                                                , 'value' => isset( $_REQUEST[ 'wh_opl_pluginname_datefixeddates'] ) ? sanitize_text_field( $_REQUEST[ 'wh_opl_pluginname_datefixeddates'] ) : ''
                                                                              )                                                        
                                                                        , array(                                                                                 
                                                                                'type'          => 'text' 
                                                                                , 'id'          => 'wh_opl_pluginname_date2fixeddates'  
                                                                                , 'name'        => 'wh_opl_pluginname_date2fixeddates'  
                                                                                , 'label'       => __('Check-out' , 'email-reminders') . ':'
                                                                                , 'disabled'    => false
                                                                                , 'class'       => 'wpdevbk-filters-section-calendar'                  // This class add datepicker
                                                                                , 'style'       => ''
                                                                                , 'placeholder' => date( 'Y-m-d' )                                                                                                                                   
                                                                                , 'attr'        => array()       
                                                                                , 'value' => isset( $_REQUEST[ 'wh_opl_pluginname_date2fixeddates'] ) ? sanitize_text_field( $_REQUEST[ 'wh_opl_pluginname_date2fixeddates'] ) : ''
                                                                              )                                                        
                                                                     )                                                     
                                                    , 'divider3' => 'divider'  
                                                    , 'custom' => array( array(  'type' => 'group', 'class' => 'input-group text-group') 
                                                                        , array( 
                                                                                'type' => 'radio'
                                                                              , 'label' => __('Tada' , 'email-reminders')
                                                                              , 'id' => 'wh_opl_pluginname_datedays_interval4' 
                                                                              , 'name' => 'wh_opl_pluginname_datedays_interval_Radios'                                                                                            
                                                                              , 'style' => ''                     // CSS of select element
                                                                              , 'class' => ''                     // CSS Class of select element                                                                                
                                                                              , 'disabled' => false
                                                                              ...
                                                                              , 'selected' => ( isset($_REQUEST[ 'wh_opl_pluginname_datedays_interval_Radios'] ) 
                                                                                                && ( $_REQUEST[ 'wh_opl_pluginname_datedays_interval_Radios'] == '10' ) ) ? true : false
                                                                              )
                                                        
                                                                        , array(                                                                                 
                                                                                'type'          => 'text' 
                                                                                , 'id'          => 'wh_opl_pluginname_datecustom'  
                                                                                , 'name'        => 'wh_opl_pluginname_datecustom'  
                                                                                , 'label'       => __('Custom' , 'email-reminders') . ':'
                                                                                , 'disabled'    => false
                                                                                , 'class'       => ''
                                                                                , 'style'       => ''
                                                                                , 'placeholder' => date( 'Y' )                                                                                                                                   
                                                                                , 'attr'        => array()    
                                                                                , 'value' => isset( $_REQUEST[ 'wh_opl_pluginname_datecustom'] ) ? sanitize_text_field( $_REQUEST[ 'wh_opl_pluginname_datecustom'] ) : ''
                                                                              )
                                                                        )
                                                    , 'divider4' => 'divider'  
                                                    , 'buttons' => array( array(  'type' => 'group', 'class' => 'btn-group' ), 
                                                                        array( 
                                                                                  'type' => 'button' 
                                                                                , 'title' => __('Apply' , 'email-reminders')                     // Title of the button
                                                                                , 'hint' => ''                      // , 'hint' => array( 'title' => __('Select status' , 'email-reminders') , 'position' => 'bottom' )
                                                                                , 'link' => 'javascript:void(0)'    // Direct link or skip  it
                                                                                , 'action' => "oper_show_selected_in_dropdown__radio_select_option("
                                                                                                                                                 . "  'wh_opl_pluginname_date'"
                                                                                                                                                 . ", 'wh_opl_pluginname_date2'"
                                                                                                                                                 . ", 'wh_opl_pluginname_datedays_interval_Radios' "
                                                                                                                                                 . ");"
                                                                        ...
                                                                        , array( 
                                                                                  'type' => 'button' 
                                                                                , 'title' => __('Close' , 'email-reminders')                     // Title of the button
                                                                                , 'hint' => ''                      // , 'hint' => array( 'title' => __('Select status' , 'email-reminders') , 'position' => 'bottom' )
                                                                                , 'link' => 'javascript:void(0)'    // Direct link or skip  it
                                                                                //, 'action' => ''                    // Some JavaScript to execure, for example run  the function
                                                                                , 'class' => 'button-secondary'     // button-secondary  | button-primary
...
Exmaple #3:
            $params = array(                                                    
                          'id'  => 'wh_pay_status'
                        // , 'id2' => 'wh_opl_pluginname_date2'
                        , 'default' =>  ( isset( $_REQUEST[ 'wh_pay_status' ] ) ) ? sanitize_text_field( $_REQUEST[ 'wh_pay_status' ] ) : 'all'
                        // , 'default2' => ( isset( $_REQUEST[ 'wh_opl_pluginname_date2' ] ) ) ? sanitize_text_field( $_REQUEST[ 'wh_opl_pluginname_date2' ] ) : ''
                        , 'hint' => array( 'title' => __('Payment status' , 'email-reminders') , 'position' => 'top' )
                        , 'label' => ''//__('Booked Dates', 'email-reminders') . ':'
                        , 'title' => __('Payment', 'email-reminders')
                        , 'options' => array (
                                                __('Any Status' , 'email-reminders')       =>'all',
                                                'divider0' => 'divider',
                                                __('Paid OK' , 'email-reminders') =>'group_ok',
                                                __('Unknown Status' , 'email-reminders')    =>'group_unknown',
                                                __('Not Completed' , 'email-reminders')     =>'group_pending',
                                                __('Failed' , 'email-reminders')            =>'group_failed'
                                                , 'divider2' => 'divider'                                
                                                , 'custom' => array( array(  'type' => 'group', 'class' => 'input-group text-group') 
                                                                    , array( 
                                                                            'type' => 'radio'
                                                                    ...
                                                                    , array(                                                                                 
                                                                            ...
                                                                            , 'label'       => __('Payment status' , 'email-reminders') . ':'
                                                                            , 'disabled'    => false
                                                                            , 'class'       => ''
                                                                            , 'style'       => ''
                                                                            , 'placeholder' => ''
                                                                            , 'attr'        => array()    
                                                                            , 'value' => isset( $_REQUEST[ 'wh_pay_statuscustom'] ) ? sanitize_text_field( $_REQUEST[ 'wh_pay_statuscustom'] ) : ''
                                                                          )
                                                                    )
                                                , 'divider4' => 'divider'  
                                                , 'buttons' => array( array(  'type' => 'group', 'class' => 'btn-group' ), 
 ...
 */
function oper_bs_dropdown_list( $args = array() ) {
    ...
    // Selected Value //////////////////////////////////////////////////////////
    $oper_value = $params[ 'default' ];
        
    $oper_selector_default = array_search( $oper_value, $params['options'] );
    if ( $oper_selector_default === false ) {
        $oper_selector_default = key( $params['options'] );
        $oper_selector_default_value = current( $params['options'] );
    } else
        $oper_selector_default_value = $oper_value;
    
    $oper_selector_default_value2 = $params[ 'default2' ];
            ...
            <div class="btn-group">                    
              <a href="javascript:void(0)" 
                 ...
                 ><label class="label_in_filters"
                      ><?php echo $params['title']; ?>: </label> <span class="oper_selected_in_dropdown"><?php echo $oper_selector_default; ?></span> &nbsp; <span class="caret"></span></a>
              <ul   
                  id="<?php echo $params['id']; ?>_container"
                  class="dropdown-menu dropdown-menu-<?php echo $params['align']; ?>" 
                   ><?php
                   
                    oper_bs_list_of_options_for_dropdown( $params , 'list', $is_this_simple_list );
                   
            ?></ul>
              <input type="hidden" autocomplete="off" value="<?php echo $oper_selector_default_value; ?>" id="<?php echo $params['id']; ?>"  name="<?php echo $params['id']; ?>" />
              <?php if ( ! empty($params['id2'] ) ) { ?>
                    <input type="hidden" autocomplete="off" value="<?php echo $oper_selector_default_value2; ?>" id="<?php echo $params['id2']; ?>"  name="<?php echo $params['id2']; ?>" />
              <?php } ?>
            </div>
        </div>
      </div>
...
oper_dropdown_menu( array(
                           'title' => __('Help', 'email-reminders')
                         , 'hint' => array( 'title' => __('Help info' , 'email-reminders') , 'position' => 'bottom' )
                         , 'font_icon' => 'glyphicon glyphicon-question-sign'
                         , 'position' => 'right'
                         , 'items' => array( 
                                             array( 'type' => 'link', 'title' => __('About Plugin', 'email-reminders'), 'url' => 'http://oplugins.com /' )
                                           , array( 'type' => 'divider' )
                                           , array( 'type' => 'text', 'title' => __('Text', 'email-reminders') )
                                           , array( 'type' => 'link', 'title' => __('Help', 'email-reminders'), 'url' => 'http://oplugins.com/plugins/email-reminders//help/' )
                                           , array( 'type' => 'link', 'title' => __('FAQ', 'email-reminders'), 'url' => 'http://oplugins.com/plugins/email-reminders/faq/' )
                                           , array( 'type' => 'link', 'title' => __('Technical Support', 'email-reminders'), 'url' => 'http://oplugins.com/plugins/email-reminders/support/' )
                                           , array( 'type' => 'divider' )
                                           , array( 'type' => 'link', 'title' => __('Upgrade Now', 'email-reminders'), 'url' => 'http://oplugins.com/', 'style' => 'font-weight: 600;' , 'attr' => array( 'target' => '_blank' ) )
                                      )
                  ) ); 
 * 
 * @param array $args
 */
function oper_bs_dropdown_menu( $args = array() ) {
        ...
        <ul class="dropdown-menu" role="menu" style="<?php echo ( ( $params['position'] == 'right' ) ? 'right:0px; left:auto;' : '' ); ?>">
            ...
            foreach ( $params['items'] as $items ) {
                switch ( $items['type'] ) {
                    ...
                    case 'text':
                            ?><li <?php echo oper_get_custom_attr( $items ); ?> ><?php echo $items['title']; ?></li><?php
                        break;

                    default:
                        ?><li><a <?php echo oper_get_custom_attr( $items ); ?> href="<?php echo $items['url']; ?>"><?php echo $items['title']; ?></a></li><?php
                        break;
                }
            }
            ?>
 ...
 *  Example:
    $params['btn_vm_calendar'] = array(
                                  'title' => ''
                                , 'hint' => array( 'title' => __('Calendar Overview' , 'email-reminders') , 'position' => 'bottom' )
 ...
 */
function oper_bs_vertical_buttons_group( $params ) {
               ...
               ><?php 
                    ...
                    if ( ! empty( $btn_params['icon'] ) ) {                                
                        ...
                        if ( substr( $btn_params['icon'], 0, 4 ) != 'http')
                            $img_path = OPER_PLUGIN_URL . '/assets/img/' . $btn_params['icon'];
                        else
 ...
 */
function oper_bs_display_tab( $args = array() ) {
    ...
    echo '>';                                                                   // Close  >  for A  or SPAN
        ...
        if ( ! empty( $tab['icon'] ) ) {                                
            ...
            if ( substr( $tab['icon'], 0, 4 ) != 'http')
                $img_path = OPER_PLUGIN_URL . '/assets/img/' . $tab['icon'];
            else
                $img_path = $tab['icon'];
...
function oper_bs_toolbar_tabs_html_container_start() {
...
function oper_bs_toolbar_tabs_html_container_end() {
...
function oper_bs_toolbar_sub_html_container_start() {
...
function oper_bs_toolbar_sub_html_container_end() {
...
function oper_bs_javascript_tooltips() {
    
    ?><span id="oper_tooltips_container"></span><?php
    ...
    ?><script type="text/javascript">
        jQuery(document).ready( function(){
          jQuery('.tooltip_right').tooltip( {
            ...
            , container: '#oper_tooltips_container'
...
function oper_bs_javascript_popover() {
        ...
        jQuery(document).ready( function(){
            ...
            if ( 'function' === typeof( jQuery(".popover_click.popover_bottom" ).popover )  ) {
				...
				jQuery('.popover_click.popover_bottom').popover( {
					...
					, container: '.oper_container_frame'
					, html: 'true'
				});
				...
				jQuery(document).on("click", ".popover_click" , function(){
					...
					jQuery('.oper_container_ajax_replace .popover.fade.bottom:not(.in)').css('left','-9000px');
 ...
 * @return type
 */
function oper_get_custom_attr( $field ) {
