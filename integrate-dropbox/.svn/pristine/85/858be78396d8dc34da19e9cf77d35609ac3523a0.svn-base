...
use CodeConfig\IntegrateDropbox\Ajax\UserAccess;
use CodeConfig\IntegrateDropbox\Ajax\LoginCheck;
use CodeConfig\IntegrateDropbox\Ajax\MediaLibrary as MediaLibraryAjax;
use CodeConfig\IntegrateDropbox\Ajax\Shared;
...
class Ajax
{
     ...
     * Ajax constructor.
     *
     * Initializes the Ajax class by setting up WordPress AJAX actions
     * for various operations related to Dropbox integration.
     * The actions are registered for both authenticated and unauthenticated
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_indbox_get_folders', [$this, 'indbox_get_folders']);
        add_action('wp_ajax_nopriv_indbox_get_folders', [$this, 'indbox_get_folders']);
        ...
        add_action('wp_ajax_indbox_get_all_role_list', [$this, 'get_all_role_list']);
        add_action('wp_ajax_indbox_get_current_user_details', [$this, 'get_current_user_details']);
        ...
        add_action('wp_ajax_nopriv_indbox_upload', [$this, 'indbox_upload']);
        ...
        add_action('wp_ajax_nopriv_indbox_file_search', [$this, 'indbox_file_search']);

        add_action('wp_ajax_indbox_download_zip', [$this, 'indbox_download_zip']);

        add_action('wp_ajax_indbox_attachment_upload', [$this, 'indbox_attachment_upload']);

        add_action('plugin_loaded', [$this, 'set_shortcode']);

        $this->client = Client::instance();

        LoginCheck::instance();
        UserAccess::instance();
        MediaLibraryAjax::instance();
        Shared::instance();
    }

    public function indbox_download_zip()
    {
        ...
        $folder_id = isset($_POST['id']) ? sanitize_text_field($_POST['id']) : null;

        if (empty($folder_id)) {
            wp_send_json_error(['message' => __('Folder id is required!', 'integrate-dropbox')]);
        }

        $zip_data = API::download_zip($folder_id);

        if (empty($zip_data)) {
            wp_send_json_error(['message' => __('Something went wrong!', 'integrate-dropbox')], 400);
        }
        wp_send_json_success(['data' => $zip_data]);
    }

    public function indbox_attachment_upload()
    {
        ...
        $attachment_id = isset($_POST['attachment_id']) ? sanitize_text_field($_POST['attachment_id']) : null;
        ...
        if (empty($attachment_id) || empty($folder_id) || empty($filename)) {
            wp_send_json_error(['message' => __('File and folder is required!', 'integrate-dropbox')]);
        }
        ...
        $uploaded_entry = API::upload_attachment($file_path, $path, $description);
        ...
        wp_send_json_success(['status' => 'ok', 'result' => $response_entry]);
    }
    ...
    {
        $fetched_roles = wp_roles();


        wp_send_json_success($fetched_roles);
    }
    ...
    public function get_thumbnail($id = false)
    {
        ...
        if (empty($thumbnail_url) || empty($thumbnail_size)) {
            ...
            if (empty($file)) {
                wp_send_json_error(['message' => __("Your provided ID is invalid!", 'integrate-dropbox')], 404);
            }

            $thumbnail = Client::instance()->get_thumbnail($file->entry, 200, 300);
            ...
            // if (Integration::instance()->is_active('media-library') && indbox_fs()->is_paying_or_trial()) {
            //     MediaLibrary::instance()->insert_attachment($file);
            // }
        }
    ...
    public function indbox_get_download_links()
    {
        ...
        $link = $this->generate_download_link($id);
    ...
    public function generate_download_link($id, $account_id = false)
    {
    ...
    public function indbox_get_file()
    {
        ...
        // if ((empty($id) || '/' === $id || 'root' === $id) && ! current_user_can('manage_indbox_files')) {
        //     wp_send_json_error(['message' => __('Unauthorized Request!', 'integrate-dropbox')], 401);
        // }

        if (empty($id) || '/' === $id || 'root' === $id) {
            $rootFolders = $this->checkRootFolder();
            if (!empty($rootFolders)) {
                $rootFolders = Database::instance()->get_entries($rootFolders);
    ...
    public function set_shortcode($args = [])
    {
    ...
    public function indbox_get_entries($id = '', $force = false)
    {
        ...
        if ($force) {
            delete_transient('integrate_dropbox_account_' . App::get_current_account()->get_id());
        }

        if (empty($id) || '/' === $id || 'root' === $id) {
            $rootFolders = $this->checkRootFolder();
            if (!empty($rootFolders)) {
                $_rootFolders = Database::instance()->get_entries($rootFolders);

                if (!empty($_rootFolders)) {
                    $entries = [
                        'self' => [
                            'id' => 'custom-root',
                            'name' => 'Dropbox',
                        ],
                        'children' => $_rootFolders,
                    ];
                    if ($isJsonSend) {
    ...
    public function indbox_get_folders()
    {
        $nonce = isset($_POST['nonce']) ? sanitize_text_field($_POST['nonce']) : null;
        ...
        $entries = $this->indbox_get_entries($id);

        if (Integration::instance()->is_active('media-library') && indbox_fs()->is_paying_or_trial() && !empty($entries['children'])) {
            MediaLibrary::instance()->insert_attachments($entries['children']);
        }
        ...
        if (!empty($entries)) {
            ...
            wp_send_json_success(['data' => $folders]);
        }

        wp_send_json_error(['message' => __('Data not found', 'integrate-dropbox')]);
    }

    private function checkRootFolder()
    {
        $accessFolders = User::instance()->get_user_access_folders();

        if (empty($accessFolders)) {
            return false;
        }
    ...
    public function indbox_get_setting()
    {
        ...
        if (!empty($settings)) {
            ...
            if (!empty($activeIntegration) && !$indbox_fs->is_paying_or_trial()) {
        ...
        if ($data === 'login') {
        ...
        } elseif ($data === 'current-account' && !empty($accounts)) {
            $current_account = App::get_current_account();
            ...
            if (isset($account_space_info['allocation']['allocated']) && $account_space_info['allocation']['allocated'] > 0) {
            ...
            $res = [
                ...
                'isSelectedUser' => User::instance()->is_setup_access_folder(),
                'accountSpaceInfo' => [
                    'used' => $used,
                    'allocated' => $allocated,
        ...
        $account_info = [
            'isPro' => $indbox_fs->is_paying_or_trial(),
            'adminUrl' => admin_url(),
            'appKey' => get_option('indbox-app-key', ''),
            'appSecret' => get_option('indbox-app-secret', ''),
            'redirectUrl' => Helpers::redirect_url(),
        ...
        if (!empty($entries) && !empty($settings)) {
            $response['entries'] = $this->indbox_get_entries($entries, (bool) $force);
        }
    ...
    public function indbox_set_setting()
    {
        ...
        if (!$res) {
            wp_send_json_error(['message' => __('Something went wrong!', 'integrate-dropbox')], 500);
        }
