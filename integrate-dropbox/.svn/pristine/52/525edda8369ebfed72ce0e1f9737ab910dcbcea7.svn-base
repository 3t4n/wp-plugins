                            ...
                            <div class="tutor-card-body">
                                <div class="tutor-row">
                                    <div class="tutor-col tutor-overflow-hidden">
                                        ...
                                        <div
                                            class="tutor-fs-7 tutor-color-muted"><?php esc_html_e('Size', 'integrate-dropbox'); ?>
    ...
    public function render_attachment_field($post)
    {
        ...
        if (! empty($attachments)) {
            foreach ($attachments as $attachment) {
                printf(
                    '<input type="hidden" name="indbox_tutor_attachments[%s][id]" value="%s" />',
                    esc_attr($attachment['id']),
                    esc_attr($attachment['id'])
                );
                printf(
                    '<input type="hidden" name="indbox_tutor_attachments[%s][accountId]" value="%s" />',
                    esc_attr($attachment['id']),
                    esc_attr($attachment['accountId'])
                );
                printf(
                    '<input type="hidden" name="indbox_tutor_attachments[%s][name]" value="%s" />',
                    esc_attr($attachment['id']),
                    esc_attr($attachment['name'])
                );
                printf(
                    '<input type="hidden" name="indbox_tutor_attachments[%s][size]" value="%s" />',
                    esc_attr($attachment['id']),
                    esc_attr($attachment['size'])
                );
            }
        }
    }
