...
class Database
{
    ...
    public function get_entry($id)
    {
        ...
        if (!is_wp_error($result) && !empty($result)) {
            ...
            $entry = maybe_unserialize($result);
            if (!empty($entry)) {
    ...
    public function get_entries($args)
    {
        ...
        foreach ($args as $pair) {
            $conditions[] = '(file_id = %s AND account_id = %s)';
            $values[] = $pair['file_id'];
            $values[] = $pair['account_id'];
        }
        $where_clause = implode(' OR ', $conditions);

        // Prepare the query with the values
        $prepared_sql = $this->wpdb->prepare("SELECT * FROM $this->table WHERE $where_clause", ...$values);
        ...
        $unserialize_results = $this->process_files($results);

        if (empty($unserialize_results)) {
            return false;
        }

        return $unserialize_results;
    }
    ...
    private function process_file($file)
    {
        ...
        $file->entry = maybe_unserialize($file->data);
        if ($file->entry->media instanceof \CodeConfig\IntegrateDropbox\SDK\Models\MediaInfo) {
            $metadata = $file->entry->media->getData();
