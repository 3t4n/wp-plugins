...
use CodeConfig\IntegrateDropbox\SDK\Models\FolderSharingInfo;

class Entry extends EntryAbstract
{
    /**
     * @param FileMetadata|FolderMetadata $api_entry
     */
    public function convert_api_entry($api_entry)
    {
        ...
        if ($api_entry instanceof FileMetadata) {
            $this->set_rev($api_entry->getRev());
            $this->set_media($api_entry->getMediaInfo());
        }

        if ($api_entry instanceof FolderMetadata) {
            $this->set_is_dir(true);
        }

        $pathinfo = Helpers::get_pathinfo($api_entry->path_lower);
        ...
        if ($this->is_file() && is_string($api_entry->client_modified)) {
            $dtime = \DateTime::createFromFormat('Y-m-d\\TH:i:s\\Z', $api_entry->client_modified, new \DateTimeZone('UTC'));
         ...
         */
        $previewsupport = ['pdf', 'txt', 'ai', 'eps', 'odp', 'odt', 'doc', 'docx', 'docm', 'ppt', 'pps', 'ppsx', 'ppsm', 'pptx', 'pptm', 'xls', 'xlsx', 'xlsm', 'rtf', 'jpg', 'jpeg', 'gif', 'png', 'webp', 'mp4', 'm4v', 'ogg', 'ogv', 'webmv', 'mp3', 'm4a', 'ogg', 'oga', 'wav', 'flac', 'paper', 'gdoc', 'gslides', 'gsheet', 'mov', 'mkv', 'webm'];
        ...
        // Thumbnail
        $can_always_create_thumbnail_extensions = ['csv', 'doc', 'docm', 'docx', 'ods', 'odt', 'pdf', 'rtf', 'xls', 'xlsm', 'xlsx', 'odp', 'pps', 'ppsm', 'ppsx', 'ppt', 'pptm', 'pptx', '3fr', 'ai', 'arw', 'bmp', 'cr2', 'crw', 'dcs', 'dcr', 'dng', 'eps', 'erf', 'gif', 'heic', 'jpg', 'jpeg', 'kdc', 'mef', 'mos', 'mrw', 'nef', 'nrw', 'orf', 'pef', 'png', 'psd', 'r3d', 'raf', 'rw2', 'rwl', 'sketch', 'sr2', 'svg', 'svgz', 'tif', 'tiff', 'x3f', '3gp', '3gpp', '3gpp2', 'asf', 'avi', 'dv', 'flv', 'm2t', 'm4v', 'mkv', 'mov', 'webm', 'mp4', 'mpeg', 'mpg', 'mts', 'oggtheora', 'ogv', 'rm', 'ts', 'vob', 'webm', 'wmv', 'paper', 'webp'];
