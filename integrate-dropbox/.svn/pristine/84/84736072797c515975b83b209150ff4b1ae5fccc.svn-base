...
class MediaPlayer extends Widget_Base
{

    public function get_name()
    {
        return 'indbox_media_player';
    }
    ...
    {
        return 'indbox-media-player indbox-pro';
    }
    ...
    public function register_controls()
    {
        ...
        $defaultData = '{"editData": {
            ...
            "openedPlaylist": true,
            "showThumbnailInPlaylist": true,
            "showNumberPrefixInPlaylist": true,
            "playlistPosition": "Bottom",
            "showNextPrevious": true,
            "mediaPlayerBackgroundColor": "#ffffff"
        }}';

        $this->start_controls_section(
            '_section_module_builder',
            [
                'label' => __('Media Player Module', 'integrate-dropbox'),
        ...
        if (!$indbox_fs->can_use_premium_code()) {
            $this->add_control('edit_module', [
                ...
                'description' => __('Use Media Player widget and dozens more pro features to extend your toolbox and build sites faster and better.', 'integrate-dropbox'),
            ]);
        } else {
    ...
    public function render()
    {
        ...
        if (!$indbox_fs->can_use_premium_code()) {
            if (is_user_logged_in()) {
                ...
                printf('<div class="indbox-pro-module-wrapper"><h2>Media Player Module - Premium Feature</h2><p>You are currently using the free license. To access this feature, you need to upgrade to a Pro license.</p> <a target="_blank" href="%s">Upgrade Now</a></div>', esc_url($indbox_fs->get_upgrade_url()));
            }
            return;
        }
        ...
        if ($is_init && Plugin::$instance->editor->is_edit_mode()) { ?>
            ...
            <div class="indbox-toplavel-wrapper integrate-dropbox-intro-preview-wrapper">
                <img src="<?php echo esc_attr(INDBOX_ASSETS . '/images/shortcode-builder/types/media.svg') ?>">
                <h3><?php esc_html_e('Media Player', 'integrate-dropbox'); ?></h3>
                <p><?php esc_html_e('Please, configure the module first to display the content', 'integrate-dropbox'); ?></p>
                <button type="button" class="indbox-btn btn-primary"
...
<?php } else {
            $shortcode = Shortcode::instance()->render_shortcode([], $settings_data['editData']);
            echo wp_kses($shortcode, 'post'); 
        }
    }
}
