...
class AutoSync
{
    ...
    public function schedule_indbox_cron_event()
    {
        ...
        if (! wp_next_scheduled('indbox_corn_fire')) {
            ...
            $get_schedules = wp_get_scheduled_event('indbox_corn_fire');
            if (!empty($get_schedules) && $get_schedules->interval != $this->get_interval()) {
                wp_clear_scheduled_hook('indbox_corn_fire');
                wp_schedule_event(time(), 'indbox_auto_sync', 'indbox_corn_fire');
            }
        }
    }
    ...
    /**
     * Calculates the interval for scheduling based on the autoSyncTimerUnit setting.
     *
     * The interval is determined by the value of 'autoSyncTimerUnit'. If the value is 'custom',
     * it uses the custom interval specified by 'autoSyncTimer'. Otherwise, it switches between
     * predefined intervals such as 5 minutes, 10 minutes, 1 hour, etc.
     *
     * @param int $interval The default interval in seconds, defaulting to one hour (HOUR_IN_SECONDS).
     * @return int The calculated interval in seconds.
     */
    private function get_interval($interval = HOUR_IN_SECONDS)
    {
