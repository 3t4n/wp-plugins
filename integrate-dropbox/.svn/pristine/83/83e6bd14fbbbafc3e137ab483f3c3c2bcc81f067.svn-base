...
class Enqueue
{
    ...
    public function frontend_scripts(): void
    {
        ...
        if(!empty($customCss)) {
            wp_add_inline_style('integrate-dropbox-admin-frontend', html_entity_decode($customCss));
        }
    }
    ...
    public function admin_scripts($hook): void
    {
        ...
        if (($hook === 'post.php' || $hook === 'post-new.php') && $isWC = Integration::instance()->is_active('woocommerce')) {
    ...
    private function common_enqueue_scripts($hook = false, $script = 'admin')
    {
        ...
        // Plyr CSS
        wp_register_style(
            'integrate-dropbox-plyr-style',
            INDBOX_ASSETS
                . '/css/plyr/plyr.min.css',
            [],
            '3.7.8'
        );

        wp_enqueue_style('integrate-dropbox-plyr-style');
    ...
    public function get_localize_data($hook = false, $script = 'admin', $additional_data = [])
    {
        ...
        $localize_data = [
            ...
            'version' => INDBOX_VERSION,
            'assetPath' => INDBOX_ASSETS
        ];
