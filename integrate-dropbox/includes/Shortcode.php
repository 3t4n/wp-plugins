                ...
                <div class="indbox-access-denied-message">
                    ...
                    <p>
                        ...
                        if (isset($this->data['accessDeniedMessage']) && !empty($this->data['accessDeniedMessage'])) {
                            echo esc_html($this->data['accessDeniedMessage']);
                        } else {
                            echo esc_html("We're sorry, but your account does not currently have access to this content. To gain access, please contact the site administrator who can assist in linking your account to the appropriate content. Thank you.");
                        }
                        ?>
                    </p>
                </div>
...
<?php
        ...
        if (($this->type === 'File Browser' || $this->type === 'Slider Carousel' || $this->type === 'Media Player') && !$indbox_fs->is_paying_or_trial() && is_user_logged_in()) {
            $html = '';
        ...
        ob_start();
        echo '<div class="indbox-toplavel-wrapper">
        <div class="integrate-dropbox-preview-wrapper" data-content="' . esc_attr(base64_encode(wp_json_encode($this->data))) . '">
        </div>
      </div>';
        $html = ob_get_clean();
    ...
    private function has_permission()
    {
        ...
        if ('Logged' === $permission && isset($data['displayForUsers'])) {
            return $this->has_access($data);
        }
    ...
    /**
     * Determine if the current user has access based on the display settings.
     *
     * @param array $data The data containing display settings.
     * @return bool True if the user has access, false otherwise.
     */
    private function has_access($data)
    {
        if (!is_user_logged_in()) {
            return false;
        }

        $allowedValues = array_column($data['displayForUsers'], 'value');
        $loggedInUserType = $data['loggedInUserType'] ?? '';
        $currentUser = wp_get_current_user();

        if ('users' === $loggedInUserType) {
            return $this->is_user_allowed($currentUser, $allowedValues);
        }

        if ('roles' === $loggedInUserType) {
            return $this->is_role_allowed($currentUser, $allowedValues);
        }
    ...
    /**
     * Check if the current user is explicitly allowed.
     *
     * @param WP_User $currentUser The current WordPress user.
     * @param array $allowedValues List of allowed usernames.
     * @return bool True if the user is allowed, false otherwise.
     */
    private function is_user_allowed($currentUser, $allowedValues)
    {
        $userName = $currentUser->data->user_login ?? '';
        return in_array($userName, $allowedValues, true);
    }

    /**
     * Check if the current user's roles are allowed.
     *
     * @param WP_User $currentUser The current WordPress user.
     * @param array $allowedValues List of allowed roles.
     * @return bool True if the user's roles are allowed, false otherwise.
     */
    private function is_role_allowed($currentUser, $allowedValues)
    {
        $currentUserRoles = (array) $currentUser->roles;
        return !empty(array_intersect($currentUserRoles, $allowedValues));
    }
