...
use CodeConfig\IntegrateDropbox\MediaLibrary\Importer;
...
class MediaLibrary
{
    ...
    public function filter_grid_attachments($query)
    {
        ...
        if (! empty($folder_id) && $folder_id !== 'trash') {
            $meta_query[] = [
            ...
            $meta_query[] = [
                'key'     => '_indbox_media_trash',
                'value'   => true,
                'compare' => 'NOT EXISTS',
            ];
        } elseif (! empty($folder_id) && $folder_id === 'trash') {
            $meta_query[] = [
                'key'     => '_indbox_media_trash',
                'value'   => true,
                'compare' => '=',
    ...
    {
        $file_id = get_post_meta($attachment->ID, '_indbox_media_file_id', true);
        ...
        if (!empty($file_id)) {
            ...
            $file = Database::instance()->get_file($file_id);

            if (empty($file)) {
                return $response;
            }

            $response['indbox_media'] = $file;

            $response['url'] = isset($meta['thumbnail']) ? $meta['thumbnail'] : null;
            $response['height'] = $meta['height'] ?? 1024;
            $response['width'] = $meta['width'] ?? 1024;
            ...
            $thumb_url = Ajax::instance()->get_thumbnail($id);
        ...
        if (is_array($image) && ! empty($file_id)) {
            ...
            if (in_array($size, $dropbox_full_size) && $extension !== 'svg') {
                $url_arg = [
                    ...
                    'account_id' => $account_id,
                    'action'     => 'indbox_preview_url',
                    'indbox'     => wp_create_nonce('indbox-preview-nonce')
                ];

                $url = add_query_arg($url_arg, admin_url('admin-ajax.php'));
            } else {
                $url = Ajax::instance()->get_thumbnail($file_id);
            }
    ...
    public function insert_attachment( $file, $folder_id = false ) {
        ...
        if ($file->thumbnail_size) {
            $dimensions = explode('x', $file->thumbnail_size);
            $meta['width'] = intval($dimensions[0]) ?? 150;
            $meta['height'] = intval($dimensions[1]) ?? 150;
        }
        ...
        if (isset($file->metadata)) {
            ...
            if (in_array($metadata['.tag'], ['photo', 'video'])) {
        ...
        if ((!empty($attached) && !file_exists($attached)) || empty($attached)) {
            $attached = Ajax::instance()->get_thumbnail($file->file_id);
        }

        $forceMp4 = apply_filters('indbox_force_mp4', ['video/quicktime', 'video/x-matroska']);

        $mime_type = in_array($file->type, $forceMp4) ? 'video/mp4' : $file->type;
    ...
    public function insert_attachments($files, $folder_id = false)
    {
        ...
        if (is_array($files)) {
            foreach ($files as $file) {
                ...
                $attachment_id = $this->insert_attachment($file);
    ...
    public function localize_data($data, $script)
    {
        ...
        $user = User::instance();
        $is_setup = $user->is_setup_access_folder();
        $accessFolders = $is_setup ? $user->get_user_access_folders() : null;

        global $pagenow;
        $data['pagenow'] = $pagenow;

        $data['rootFolders'] = isset($this->settings['mediaLibraryFolders']) && current_user_can('manage_options') && !$is_setup
            ? $this->settings['mediaLibraryFolders']
            : $accessFolders;

        $data['activeAccount'] = self::get_current_account_info() ?? [];
