...
class Helpers
{
    ...
    public static function apply_placeholders($value, $context = null, $extra = [])
    {
        ...
        if (! isset($extra['user_data'])) {
            if (is_user_logged_in()) {
                ...
                if (! isset($_COOKIE['indbox-ID'])) {
                    ...
                    $id = sanitize_text_field(wp_unslash($_COOKIE['indbox-ID']));
                }
        ...
        if (isset($extra['user_data'])) {
            ...
            $value = strtr($value, [
                ...
                '%user_registered%' => isset($user_data->user_registered) ? gmdate('Y-m-d', strtotime($user_data->user_registered)) : '',
            ]);
        }
    ...
    public static function is_user_can($cap = 'manage_indbox_files')
    {
    ...
    public static function clearAllIndboxAttachments()
    {
        $page = 1;
        do {
    ...
    public static function deleteIndboxAttachment($id, $permanent = false, $dropbox = false)
    {
        if ($permanent) {
            $file = get_post_meta($id, '_indbox_media_file_id', true);
            // $account_id = get_post_meta($id, '_indbox_media_account_id', true);

            if ($dropbox) {
                Client::instance()->delete_entries([
                    $file
                ]);
            }
            ...
            update_post_meta($id, '_indbox_media_trash', true);
        }

        return true;
    }

    public static function restoreIndboxAttachment($id)
    {
        if (delete_post_meta($id, '_indbox_media_trash')) {
            return true;
        }
    ...
    public static function updateAttachmentMetadataHeightWidth($fileID, $folderID, $dimensions)
    {
        $attachmentId = Database::instance()->is_attachment_exists($fileID, $folderID);
        ...
        $attachmentMetadata = get_post_meta($attachmentId, $metaKey, true);

        if (empty($attachmentMetadata)) {
            return add_post_meta($attachmentId, $metaKey, $newDimensions);
        }

        if (!empty($attachmentMetadata['sizes']['full'])) {
            $attachmentMetadata['sizes']['full']['height'] = $newDimensions['height'];
            $attachmentMetadata['sizes']['full']['width'] = $newDimensions['width'];
        }

        $updatedMetadata = wp_parse_args($newDimensions, $attachmentMetadata);

        return update_post_meta($attachmentId, $metaKey, $updatedMetadata);
    }
