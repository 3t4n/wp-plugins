...
class API
{
    ...
    public static function get_api_entry($id, $params = ['include_media_info' => true])
    {
        try {
    ...
    public static function get_entry($id, $params = ['include_media_info' => true])
    {
        ...
        $api_entry = self::get_api_entry($id, $params);
     ...
     *
     * @param string $id The identifier of the folder or file to download as a ZIP.
     *
     * @return mixed Returns the ZIP file data on success, or false on failure.
     */

    public static function download_zip($id)
    {
        try {
            $zip = App::instance()->get_sdk_client()->downloadZip($id);
            return $zip;
        } catch (\Throwable $th) {
            error_log(sprintf('[Integrate Dropbox message]: API Error on line %s: %s', __LINE__, $th->getMessage()));
            return false;
        }
    }
    ...
    public static function create_preview_url(Entry $entry, $params = [])
    {
        ...
        if (in_array($entry->get_extension(), ['mp4', 'm4v', 'ogg', 'ogv', 'webmv', 'mp3', 'm4a', 'ogg', 'oga', 'wav', 'flac', 'mkv', 'webm'])) {
     ...
     */
    public static function get_account_info($force = false)
    {
        $current_account = App::get_current_account();
        if (empty($current_account) || !method_exists($current_account, 'get_id')) {
            throw new Exception(esc_html__('Unable to get account information.', 'integrate-dropbox'));
        }

        $cache_key = 'integrate_dropbox_account_' . $current_account->get_id();
        if ($force) {
            delete_transient($cache_key);
        }
        $account_info = $force ? false : get_transient($cache_key);

        if ($force || empty($account_info)) {
            try {
                ...
                if ($account_info) {
                    ...
                    throw new \Exception(esc_html__('Failed to retrieve account information from the Dropbox client.', 'integrate-dropbox'));
                }
            } catch (\Throwable $e) {
     ...
     * @return array
     */
    public static function get_space_info($force = false)
    {
        $current_account = App::get_current_account();
        if (empty($current_account) || !method_exists($current_account, 'get_id')) {
            throw new Exception(esc_html__('Unable to get current account.', 'integrate-dropbox'));
        }

        $cache_key = 'integrate_dropbox_account_' . $current_account->get_id() . '_space';
        $space_info = $force ? false : get_transient($cache_key, false);

        if ($force || empty($space_info)) {
            try {
                ...
                if ($space_info) {
                    ...
                    throw new \Exception(esc_html__('Failed to retrieve space information from the Dropbox client.', 'integrate-dropbox'));
                }
            } catch (\Throwable $e) {
                error_log('Error fetching space info: ' . $e->getMessage());
                return false;
            }
        }
    ...
    public static function upload_file($file, $dropbox_path, $description = null, $params = [])
    {
        ...
        $entry = self::upload($file->tmp_name, $dropbox_path, $params);
    ...
    public static function upload_attachment($file_path, $dropbox_path, $description = null, $params = [])
    {

        $file_path = apply_filters('integrate_dropbox_api_upload_set_file', $file_path);
        $description = apply_filters('integrate_dropbox_api_upload_set_description', $description);
        ...
        // Do the actual upload
        $entry = self::upload($file_path, $dropbox_path, $params);
    ...
    private static function upload($file_path, $dropbox_path, $params = [])
    {
        ...
        do_action('integrate_dropbox_api_before_upload', $dropbox_path, $file_path, $params);

        try {
            $dropbox_path = Helpers::clean_folder_path($dropbox_path);
            $api_result = App::instance()->get_sdk_client()->upload($file_path, $dropbox_path, $params);
