...
use CodeConfig\IntegrateDropbox\App\Thumbnail;
...
class Client
{
    ...
    {
        try {
            return API::get_space_info();
        } catch (\Throwable $th) {
            if ($th->getMessage() === 'Access Token is required.') {
                $current_account = App::get_current_account();
                Accounts::instance()->remove_account($current_account->get_id());
                delete_transient('integrate_dropbox_account_' . $current_account->get_id());
            }
        }
    }
    ...
    public function preview_entry($id = null)
    {
        ...
        if (in_array($this->_entry->get_extension(), ['mp4', 'mkv', 'mov', 'webm', 'm4v', 'ogg', 'ogv', 'webmv', 'mp3', 'm4a', 'ogg', 'oga', 'wav', 'flac'])) {
    ...
    public function preview_entry_info($id)
    {
        ...
        if (empty($preview) || $force) {
            ...
            // $get_entry = API::get_entry($id);
            $this->_entry->set_preview_link($entry_info['url']);
            Database::instance()->update_file($id, 'data', maybe_serialize($this->_entry));
            $entry_info['entry'] = $this->_entry;

            if ($media = $this->_entry->get_media()) {
                if ($media instanceof \CodeConfig\IntegrateDropbox\SDK\Models\MediaInfo) {
                    $metadata = $media->getData();
                    if (isset($metadata['metadata']['dimensions'])) {
                        $dimensions = $metadata['metadata']['dimensions'];
                        $file = Database::instance()->get_file($id);

                        $folderId = $file->parent_id;

                        Helpers::updateAttachmentMetadataHeightWidth($id, $folderId, $dimensions);
                    }
                }
            }
    ...
    public function get_thumbnail(Entry $entry, $width = null, $height = null, $crop = false, $only_own_thumbnail = false)
    {
        ...
        if (false === $entry->has_own_thumbnail()) {
            ...
            $thumbnail_path = $thumbnail->get_location_thumbnail();
    ...
    public function build_thumbnail($src)
    {
        ...
        if (empty($entry->has_own_thumbnail())) {
            return $entry->get_icon_large();
        }
    ...
    public function get_shared_link(Entry $entry, $link_settings = ['audience' => 'public'], $create = false)
    {
        if (!$create) {
        ...
        return $this->create_shared_link($entry, $link_settings);
    }
    ...
    public function create_shared_link(Entry $entry, $link_settings)
    {
        ...
        if (empty($shared_links)) {
            exit(esc_html__('The sharing permissions on this file is preventing you from accessing this shared link. Please contact the administrator to change the sharing settings for this document in the cloud.', 'integrate-dropbox'));
        }
    ...
    public function add_folder($name_of_folder_to_create, $target_folder_path = null)
    {
        ...
        try {
            ...
            return new \WP_Error('broke', esc_html__('Failed to add folder', 'integrate-dropbox'));
        }
