...
class App
{
    ...
    public function refresh_token(Account $account)
    {
        ...
        try {
            $refreshToken = self::$_sdk_client->getAuthHelper()->refreshToken();

            $authorization->set_access_token($refreshToken);
            self::get_sdk_client()->setAccessToken($refreshToken);
        } catch (\Exception $ex) {
            $authorization->set_is_valid(false);
    ...
    {
        return $this->get_sdk_client()->getCurrentAccount();
    }
