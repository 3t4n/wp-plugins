...
class Quickcreator
{
	...
	public function __construct()
	{
		...
		add_filter("rest_request_after_callbacks", array($this, 'register_connection_api_filter'), 10, 3);
	}
	/**
	...
	public function register_connection_api_filter($response, $handler, $request)
	{
		// check quickcreatorblog/v1 request
		if (strpos($request->get_route(), 'quickcreatorblog/v1') !== false) {
			$log_data = [];
			...
			if (!empty($log_data)) {
				file_put_contents(
					Quickcreator()->get_basedir() . '/error.log',
	...
	public function quickcreator_connect_verify($request)
	{
		...
		if (false !== $token && $this->verify_connection_token($token)) {
			...
			$response = new WP_REST_Response(array(
				'error' => __('Token verification failed', 'quickcreator'),
				'token' => $token,
				'valid' => false !== $token && $this->verify_connection_token($token)
			), 403);
		}
	...
	private function verify_connection_token($token)
	{
		...
		if (false !== $wp_token) {
			quickcreator_error_log('Token mismatch: ' . $token . ' - ' . $wp_token);
		}
	...
	public function quickcreator_import_post($request)
	{
		try {
			...
			if (! is_wp_error($post_id)) {
				...
				$error_data = $post_id->get_all_error_data();
				$error_code = $post_id->get_error_code();
				$error_messages = $post_id->get_error_messages();
				return new WP_REST_Response(
					array(
						'error'            => __('There was an error on post adding', 'quickcreator'),
						'wp_error_messages' => $error_messages,
						'wp_error_code'    => $error_code,
						'wp_error_data'    => $error_data ? $error_data : __('No additional error data available', 'quickcreator'),
					),
					403
				);
			}
			...
			return new WP_REST_Response(
				array(
					'error'       => __('An exception occurred', 'your-text-domain'),
