...
trait REVIVESO_Fields {
	...
	protected function do_field( $data ) {
		...
		if ( $data['type'] == 'checkbox' ) {
			...
			echo '<div class="reviveso-toggle ' . esc_attr( $name ) . '-wrapper ' . esc_attr( implode( ' ', array_unique( $class ) ) ) . '" ' . wp_kses_post( implode( ' ', array_unique( $attr ) ) ) . '>';
		...
		if ( isset( $data['type'] ) ) {
			if ( in_array( $data['type'], array( 'text', 'email', 'password', 'date', 'number' ) ) ) {
				echo '<input type="' . esc_attr( $data['type'] ) . '" name="reviveso_plugin_settings[' . esc_attr( $name ) . ']" id="' . esc_attr( $data['name'] ) . '" class=" ' . ( class_exists( 'REVIVESO_Tailwind_UI' ) && method_exists( 'REVIVESO_Tailwind_UI', 'text_input' ) ? REVIVESO_Tailwind_UI::text_input() : '' ) . ' ' . esc_attr( implode( ' ', array_unique( $class ) ) ) . '" autocomplete="off" value="' . esc_attr( $value ) . '" ' . wp_kses_post( implode( ' ', array_unique( $attr ) ) ) . ' />';
			} elseif ( $data['type'] == 'textarea' ) {
				echo '<textarea class="' . esc_attr( implode( ' ', array_unique( $class ) ) ) . ' ' . ( class_exists( 'REVIVESO_Tailwind_UI' ) && method_exists( 'REVIVESO_Tailwind_UI', 'textarea_input' ) ? REVIVESO_Tailwind_UI::textarea_input() : '' ) . '" id="' . esc_attr( $data['name'] ) . '" name="reviveso_plugin_settings[' . esc_attr( $name ) . ']" ' . wp_kses_post( implode( ' ', array_unique( $attr ) ) ) . ' autocomplete="off">' . wp_kses_post( $value ) . '</textarea>';
			} elseif ( $data['type'] == 'select' ) {
				echo '<select id="' . esc_attr( $data['name'] ) . '" class="' . esc_attr( implode( ' ', array_unique( $class ) ) ) . ' ' . esc_attr( REVIVESO_Tailwind_UI::select() ) . '" name="reviveso_plugin_settings[' . esc_attr( $name ) . ']" ' . wp_kses_post( implode( ' ', array_unique( $attr ) ) ) . ' autocomplete="off">';
				...
				echo '<select id="' . esc_attr( $data['name'] ) . '" class="w-full ' . esc_attr( implode( ' ', array_unique( $class ) ) ) . '" name="reviveso_plugin_settings[' . esc_attr( $name ) . '][]" multiple="multiple" ' . wp_kses_post( implode( ' ', array_unique( $attr ) ) ) . '>';
				if ( ! empty( $data['options'] ) && is_array( $data['options'] ) ) {
					foreach ( $data['options'] as $key => $option ) {
				...
				?>
				<form action="<?php
				echo esc_attr( admin_url( 'admin-post.php' ) ); ?>" id="form_<?php
				echo esc_attr( $data['name'] ); ?>" method="POST">
					<?php
					wp_nonce_field( 'reviveso_' . $data['name'], 'reviveso_' . $data['name'] ); ?>
					<input type="hidden" name="action" value="reviveso_<?php
				...
				<form action="<?php
				echo esc_attr( admin_url( 'admin-post.php' ) ); ?>" method="post">
					<input type="hidden" name="action" value="reviveso_export_settings"/>
					<?php
					wp_nonce_field( 'reviveso_export_nonce', 'reviveso_export_nonce' ); ?>
					<?php
					submit_button( __( 'Export Settings', 'revive-so' ), 'button-large button-secondary default', 'reviveso-export', false ); ?>
				</form>
				<?php
			} elseif ( $data['type'] == 'tool-import' ) { ?>
				<form id="form_tool_import" action="<?php
				echo esc_attr( admin_url( 'admin-post.php' ) ); ?>" method="post" enctype="multipart/form-data">
					...
					<?php
					wp_nonce_field( 'reviveso_import_nonce', 'reviveso_import_nonce' ); ?>
					<?php
					submit_button( __( 'Import Settings', 'revive-so' ), 'button-large button-secondary default', 'reviveso-import', false ); ?>
				</form>
				<?php
			} elseif ( $data['type'] == 'tool-status' ) { ?>

				<span><?php
					esc_html_e( 'System Status', 'revive-so' ); ?></span>
				<p><?php
					esc_html_e( 'In order to use this plugin, please ensure your server meets the following PHP configurations. Your hosting provider will help you modify server configurations, if required.', 'revive-so' ); ?></p>
				<?php
				$this->systemStatus(); ?>
