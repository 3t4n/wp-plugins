...
class REVIVESO_Enqueue extends REVIVESO_BaseController
{
	...
	public function load_assets( $hook ) {
		...
		wp_register_style( 'reviveso-styles', REVIVESO_URL . 'assets/css/admin.css', array( 'reviveso-jquery-ui', 'reviveso-jquery-ui-timepicker', 'reviveso-confirm' ), REVIVESO_VERSION );
		...
		wp_register_style( 'reviveso-general', REVIVESO_URL . 'assets/css/tailwind-base.css', array(), REVIVESO_VERSION );
		// Tailwind files
		wp_register_style( 'reviveso-tailwind', REVIVESO_URL . 'assets/css/tailwind.css', array(), REVIVESO_VERSION );
		wp_register_style( 'reviveso-custom-tailwind', REVIVESO_URL . 'assets/css/tailwind-custom.css', array(), REVIVESO_VERSION );
		wp_register_script( 'reviveso-tailwind', REVIVESO_URL . 'assets/js/tailwind.js', array( 'jquery' ), REVIVESO_VERSION, true );
		...
		if ( 'revive-so_page_reviveso-extensions' === $hook ) {
			...
			wp_localize_script( 'reviveso-extensions', 'revivesoAddons', array(
				...
				'installing_text'   => esc_html__( 'Installing & Activating addon...', 'revive-so' ),
				'activating_text'   => esc_html__( 'Activating addon...', 'revive-so' ),
				'deactivating_text' => esc_html__( 'Deactivating addon...', 'revive-so' )
			) );
		}
		// Enqueue the wp_enqueue_style for all reviveso pages.
		if ( false !== strpos( $hook, 'reviveso' ) ) {
			wp_enqueue_style( 'reviveso-styles' );

			// Load upsells css/js in admin env.
			$manage_options_cap = apply_filters( 'reviveso_manage_options_capability', 'manage_options' );
			if( is_admin() && current_user_can( $manage_options_cap ) && apply_filters( 'reviveso_enqueue_upsells_scripts', true ) ){
