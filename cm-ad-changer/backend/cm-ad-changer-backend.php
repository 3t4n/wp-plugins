...
class CMAdChangerBackend {
	...
	public static function cmac_admin_menu() {
		...
		$campaignsg_subpage = add_submenu_page( CMAC_MENU_OPTION, 'Campaign Groups', 'Campaign Groups', 'manage_options', 'cmac_campaignsg', array( self::$calledClassName, 'cmac_load_page' ) );
		$campaignss_subpage = add_submenu_page( CMAC_MENU_OPTION, 'Statistics', 'Statistics', 'manage_options', 'cmac_statistics', array( self::$calledClassName, 'cmac_load_page' ) );
		$campaignst_subpage = add_submenu_page( CMAC_MENU_OPTION, 'Testing', 'Testing', 'manage_options', 'cmac_testing', array( self::$calledClassName, 'cmac_load_page' ) );
	...
	public static function cmac_campaigns_action() {
		...
		if ( !empty( $_POST ) && check_admin_referer( 'cmac-update-campaign-data' ) ) {
			...
			$data	 = CMAC_Data::cmac_handle_campaigns_post( $post );
			...
			if ( !empty( $data ) ) {
				...
				$fields_data[ 'category_ids' ]	 = isset( $post[ 'category_ids' ] ) ? $post[ 'category_ids' ] : null;
				$fields_data[ 'category_title' ] = isset( $post[ 'category_title' ] ) ? $post[ 'category_title' ] : null;
	...
	public static function cmac_settings_action() {		
		if ( !empty( $_POST ) && empty($_POST['cminds_nonce']) && check_admin_referer( 'cmac-update-settings-data' ) ) {
			$post	 = filter_input_array( INPUT_POST );
			$data	 = CMAC_Data::cmac_handle_settings_post( $post );
			if ( !empty( $data ) ) {
				$fields_data = $data[ 'fields_data' ];
