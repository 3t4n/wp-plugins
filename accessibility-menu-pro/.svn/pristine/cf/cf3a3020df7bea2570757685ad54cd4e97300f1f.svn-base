...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    foreach ($menus as $menu) {
        ...
        if (isset($_POST['bame1-settings-nonce-' . $menu_slug]) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['bame1-settings-nonce-' . $menu_slug])), 'bame1-settings-save')) {
            ...
            update_option('ampro_parent_arrow_element_height_' . $menu_name, intval($_POST['ampro_parent_arrow_element_height_' . $menu_name]));
            update_option('ampro_submenu_arrow_element_height_' . $menu_name, intval($_POST['ampro_submenu_arrow_element_height_' . $menu_name]));
...
<div id="appearance-style-settings" class="wrap">
    ...
    foreach ($menus as $menu) {
        ...
        $parent_arrow_element_height_option_name = "ampro_parent_arrow_element_height_" . $menu_name;
        $parent_arrow_element_height = get_option($parent_arrow_element_height_option_name, '48');
        echo '<tr>';
        echo '<th scope="row"><label for="' . esc_attr($parent_arrow_element_height_option_name) . '" title="Use this to vertically center arrow icon">Arrow element height (px)</label></th>';
        echo '<td>';
        echo '<input size="6" type="number" id="' . esc_attr($parent_arrow_element_height_option_name) . '" name="' . esc_attr($parent_arrow_element_height_option_name) . '" value="' . esc_attr($parent_arrow_element_height) . '" min="0" step="1"> px';
        echo '</td>';
        echo '</tr>';
        ...
        $submenu_arrow_element_height_option_name = "ampro_submenu_arrow_element_height_" . $menu_name;
        $submenu_arrow_element_height = get_option($submenu_arrow_element_height_option_name, '48');
        echo '<tr>';
        echo '<th scope="row"><label for="' . esc_attr($submenu_arrow_element_height_option_name) . '" title="Use this to vertically center arrow icon">Arrow element height (px)</label></th>';
        echo '<td>';
        echo '<input size="6" type="number" id="' . esc_attr($submenu_arrow_element_height_option_name) . '" name="' . esc_attr($submenu_arrow_element_height_option_name) . '" value="' . esc_attr($submenu_arrow_element_height) . '" min="0" step="1"> px';
        echo '</td>';
        echo '</tr>';
