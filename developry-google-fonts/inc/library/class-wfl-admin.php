...
if ( ! class_exists( 'WFL_Admin' ) ) {
	...
	class WFL_Admin {
		...
		public function __construct() {
			$this->admin_page = ( ! get_option( 'wfl_compact_mode', '' ) ) ? 'admin.php?page=' : 'themes.php?page=';
		}
		...
		public function check_nonce_token() {
			if ( ! check_ajax_referer( 'wfl_ajax_nonce', '_wpnonce', false ) ) {
				return false;
			}
		...
		public function get_invalid_nonce_token() {
			...
			$message    = esc_html__( '%1$s! Contact us @ %2$s.', 'developry-google-fonts' );
			$values_arr = array(
				'<strong>' . __( 'Invalid security token', 'developry-google-fonts' ) . '</strong>',
				'<a href="mailto:contact@' . WFL_PLUGIN_DOMAIN . '">contact@' . WFL_PLUGIN_DOMAIN . '</a>',
			);
		...
		public function get_invalid_user_cap() {
			...
			$message    = esc_html__( '%1$s! Current user does not have the capabilities to access this function.', 'developry-google-fonts' );
			$values_arr = array( '<strong>' . __( 'Access denied', 'developry-google-fonts' ) . '</strong>' );
