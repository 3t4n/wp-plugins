...
Affiliate_Power_Widget::prepare();
...
class Affiliate_Power_Widget {
	...
	static function getContent() {
		...
		if ($sum_today) $sum_today = number_format($sum_today, 2, ',', '.').' €';
		...
		if ($sum_yesterday) $sum_yesterday = number_format($sum_yesterday, 2, ',', '.').' €';
		...
		if ($sum_this_month) $sum_this_month = number_format($sum_this_month, 2, ',', '.').' €';
		...
		if ($sum_last_month) $sum_last_month = number_format($sum_last_month, 2, ',', '.').' €';
		...
		if ($sum_total) $sum_total = number_format($sum_total, 2, ',', '.').' €';
		...
		echo $refresh_button.'<br>
			<table style="clear:left;">
				<tr><td>'.__('Today', 'affiliate-power').':</td><td style="text-align:right">'.$sum_today.'</td></tr>
				<tr><td>'.__('Yesterday', 'affiliate-power').':</td><td style="text-align:right">'.$sum_yesterday.'</td></tr>
				<tr><td>'.__('This month', 'affiliate-power').':</td><td style="text-align:right">'.$sum_this_month.'</td></tr>
				<tr><td>'.__('Last month', 'affiliate-power').':</td><td style="text-align:right">'.$sum_last_month.'</td></tr>
				<tr><td>'.__('Total', 'affiliate-power').':</td><td style="text-align:right">'.$sum_total.'</td></tr>
			</table>
