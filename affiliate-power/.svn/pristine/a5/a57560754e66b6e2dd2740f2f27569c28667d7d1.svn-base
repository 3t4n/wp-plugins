...
class Affiliate_Power_Settings {
    ...
    static function prepare() {
        ...
        self::$network_data = array(
			...
			'webgains' => array(
                'slug' => 'webgains',
                'link_pattern' => 'wgcampaignid',
                'label' => 'Webgains',
                'check_login_params' => 2,
                'fields' => array(
					array('slug' => 'publisher_id', 'label' => 'Publisher-ID', 'info' => __('You can find your numeric Publisher-ID in your Publisher account settings (bottom left corner after logging in).'), 'validation' => 'ctype_digit'),
					array('slug' => 'token', 'label' => 'API Token', 'info' => __('To generate an API Token, login into your Webgains account, click on your username in the top right corner and on "Manage API Tokens". Click on "Generate new token", give it a name and copy the generated token (yes, it is crazy long).')),
                ),
            ),
        );
    }
	...
	static function optionsPage() {
		...
		?>
		<div class="wrap">
		<?php echo Affiliate_Power::$lite_optin_html; ?>
		<h1><?php _e('Affiliate Power Settings', 'affiliate-power'); ?></h1>
		<?php
		...
		<?php
		_e('<p>Please be patient while saving. The plugin performs a test login at the networks which takes a few seconds.</p>', 'affiliate-power');
	}
	
	
	static function optionsMainText() {
		_e('<p>Anything is not working or you have an idea for a cool feature? <a href="mailto:support@affiliatepowerplugin.com" target="_blank">Send me an email</a></p>', 'affiliate-power');
	}
	...
	static function optionsNetworksText() {
		_e('<p>Are you missing a network? <a href="https://forms.gle/TYu11eQiRdtPXrkF9" target="_blank">Make your voice heard</a></p>', 'affiliate-power');
	}
