...
function vform_wp_scripts(){
    ...
    wp_enqueue_style( 'vform_dev_style', VFORM_PLUGIN_URL . 'assets/css/style.css', array(), '3.1.3', 'all' );
    wp_enqueue_style('vform_dev_style2', VFORM_PLUGIN_URL."assets/css/fontawesome.css");
    wp_enqueue_script('vform_dev_script', VFORM_PLUGIN_URL."assets/js/custom.js", array(),'3.1.3',true);
...
function vform_admin_enqueue_scripts(){
    ...
    wp_enqueue_style( 'vform_dev_style', VFORM_PLUGIN_URL . 'assets/css/style.css', array(), '3.1.3', 'all' );
    wp_enqueue_style('vform_dev_style2', VFORM_PLUGIN_URL."assets/css/fontawesome.css");
    wp_enqueue_script('vform_dev_script', VFORM_PLUGIN_URL."assets/js/custom.js", array(),'3.1.3',false);
    // wp_enqueue_script('vform_dev_script2', VFORM_PLUGIN_URL."assets/js/jquery-ui.min.js", array(),'3.1.3',false);
    wp_localize_script('vform_dev_script','ajax_object',admin_url("admin-ajax.php"));
...
function vform_options_func(){
    ...
    wp_enqueue_style( 'vform_dev_style1', VFORM_PLUGIN_URL . 'assets/css/admin-dashboard.css', array(), '3.1.3', 'all' );
...
function entries_options_func(){
    ...
    wp_enqueue_style( 'vform_dev_style_entries', VFORM_PLUGIN_URL . 'assets/css/admin-entries.css', array(), '3.1.3', 'all' );
...
function myvformfrontsave(){
    ...
    if($_REQUEST['param']=='save_vform' && $valid){
        ...
        foreach ($_REQUEST as $key => $value) {
            if (!in_array($key, ['action', 'param', 'vfid', 'usertimetakes','browser','currentdate','currentdate_part2','timezone','vfm-nonce','_wp_http_referer','ip','formid','g-recaptcha-response','recapthca','ct_bot_detector_event_token'])) {
                $formData[$key] = $value;
            }
        }
        ...
        echo json_encode(array("status"=>1,"message"=>"Data inserted successful","confirmation"=>esc_html($data_conf1),"confirmation_value"=>esc_html($data_conf2),"inserted_id"=>$inserted_id,"bl"=>$formData));
    }
    wp_die();
