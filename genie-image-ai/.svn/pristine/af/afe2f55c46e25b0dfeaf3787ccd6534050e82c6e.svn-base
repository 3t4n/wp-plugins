(() => {
  ...
  var require_Integrations = __commonJS({
    "assets/src/admin/js/Integrations/index.js"() {
      var { doAction } = wp.hooks;
      (function(window2) {
        window2.addEventListener("load", function() {
          if (!jQuery("#genieimagecontainer").length) {
            jQuery("body").append('<div id="genieimagecontainer" class="getgenie genieimagemain-container"></div>');
          }
          ;
          doAction("genieimagerender-app", "getgenie_hooks");
          jQuery.fn.menu_current = function() {
            this.addClass("current").siblings().removeClass("current");
          };
          let $toplevel_page = jQuery(".toplevel_page_genieimage.wp-menu-open");
  ...
  // assets/src/admin/js/GenieImage/Image.js
  var { useState } = window.React;
  var { Libs } = window.genieImage.Components.Common;
  ...
  var { __ } = wp.i18n;
  var InsertButton = `${window.genieImage.config.assetsUrl}dist/admin/images/plus.svg`;
  var paintImg = `${window.genieImage.config.assetsUrl}dist/admin/images/spinner.gif`;
  var { Tooltip, Spin } = window.antd;
  var AntImage = window.antd.Image;
  var Image = ComposeComponents(
    ({ getInputs, sidebar, img }) => {
      ...
      const handleImageUpload = (imageUrl) => {
        ...
        HandleFetch(
          (res) => {
            setUploading(false);
            HandleResponse(res, () => {
              let wpUploader = document.querySelector(".supports-drag-drop:not([style*='display: none'])");
              if (wpUploader.id.includes("wp-uploader-id")) {
                wpUploader.querySelector("#menu-item-browse").click();
                let select = wpUploader.querySelector(".media-modal-content").querySelectorAll("select")[0];
          ...
          "genieImageSave",
          {
            imageUrl,
            hash: "",
            title: getInputs["input-keyword"]
          }
        );
      };
      return /* @__PURE__ */ React.createElement("div", {
        className: "genieimage-image-show-main"
      }, /* @__PURE__ */ React.createElement(Tooltip, {
        title: uploading ? "Saving image..." : "Save to media library",
        getPopupContainer: () => sidebar.rootContainer,
        zIndex: 999999
      }, /* @__PURE__ */ React.createElement("div", {
        className: "genieimage-image-show-select"
      }, uploading ? /* @__PURE__ */ React.createElement(Libs.Loading, null) : /* @__PURE__ */ React.createElement("img", {
        src: InsertButton,
        className: "genieimage-upload-icon",
        onClick: () => handleImageUpload(img.url)
      }))), /* @__PURE__ */ React.createElement(AntImage, {
        src: img.url,
        width: "100%",
        className: "genieimage-generated-img",
        preview: {
          getContainer: () => sidebar.rootContainer,
          maskClassName: "genieimage-image-preview-mask",
          rootClassName: "genieimage-image-preview"
        },
        placeholder: /* @__PURE__ */ React.createElement("div", {
          className: "genieimage-loading-spin"
  ...
  // assets/src/admin/js/GenieImage/GenieImage.js
  var { useEffect, useState: useState2, useRef } = window.React;
  var { Libs: Libs2 } = window.genieImage.Components.Common;
  var { Alert } = window.antd;
  var { HandleFetch: HandleFetch2, HandleResponse: HandleResponse2 } = window.genieImage.Components.Common.RequestManager;
  var { ComposeComponents: ComposeComponents2 } = window.genieImage.Components.Common.ReduxManager;
  var { Button, Row, Col } = window.antd;
  var GenieImage = ComposeComponents2(
    ({ getInputs, sidebar, setInput }) => {
      const [imageData, setImageData] = useState2([]);
      const [isLoading, setIsLoading] = useState2(false);
      const [isAlertVisible, setIsAlertVisible] = useState2(false);
      const ref = useRef();
      const subscription_type = wp.data.select("genieimage").limitUsage().subscription_type;
      ...
      const handleClick = () => {
        let data = {
          input: {
            instruction: getInputs["input-keyword"]
          },
          size: getInputs["selectedResolution"],
          templateSlug: "generate-image"
        };
        setIsLoading(true);
        HandleFetch2(
          (res) => {
            ...
            HandleResponse2(res, () => {
              ...
              setImageData((prevImageData) => [...prevImageData, { url: imgObj?.url, validUntil: new Date().getTime() + 36e5 }]);
      ...
      useEffect(() => {
        (async () => {
          const oldImages = JSON.parse(localStorage.getItem("genie-img")) || [];
          const images = [];
          try {
            for (const image of oldImages) {
              const isValid = image?.validUntil > new Date().getTime();
              if (isValid) {
                images.push(image);
              }
            }
        ...
        const oldValue = JSON.parse(localStorage.getItem("genie-img-input"));
        if (!oldValue) {
          return;
        }
        setInput("input-keyword", oldValue);
      }, []);
      useEffect(() => {
        if (!getInputs["input-keyword"]) {
          return;
        }
        window.localStorage.setItem("genie-img-input", JSON.stringify(getInputs["input-keyword"]));
      }, [getInputs["input-keyword"]]);
      useEffect(() => {
        window.localStorage.setItem("genie-img", JSON.stringify(imageData));
      }, [imageData]);
      const handleClear = () => {
        Libs2.ConfirmModal("Are you sure?", "Are you sure you want to clear all images?", () => {
          setImageData([]);
          setInput;
          localStorage.removeItem("genie-img");
        });
      };
      const handleOnSelectChange = (val) => {
        setInput["selectedImage", val];
      };
      const selectProps = {
        handleOnChange: (val) => handleOnSelectChange(val),
        name: "selectedResolution",
        defaultValue: ResolutionsOptions[0].value,
        options: ResolutionsOptions,
        style: { width: "25%" },
        label: subscription_type === "pro" ? "Image Resolutions (We recommend you to use the image size: 1024x1024 for better optimization of your site)" : "You can generate high-resolution images if you upgrade with pro",
        disabled: subscription_type === "pro" ? false : true
      };
      return /* @__PURE__ */ React.createElement("div", {
        className: "genieimage"
      }, /* @__PURE__ */ React.createElement("div", {
        className: "genieimage-container"
      }, /* @__PURE__ */ React.createElement("div", {
        className: "genieimage-input"
      }, /* @__PURE__ */ React.createElement("div", {
        className: "genieimage-textarea-container"
      }, /* @__PURE__ */ React.createElement(Libs2.Select, {
        ...selectProps
      }), /* @__PURE__ */ React.createElement(Libs2.Textarea, {
        name: "input-keyword",
        label: "Keyword:",
        placeholder: "Give Genie an idea on what topic/search term the images will be based on.",
        rows: "4"
      }), /* @__PURE__ */ React.createElement("div", {
        className: "genieimage-action-btn-container"
      }, /* @__PURE__ */ React.createElement(Button, {
        className: "genieimage-outline-btn",
        onClick: handleClick,
        disabled: !getInputs["input-keyword"],
        loading: isLoading
      }, "Generate"), imageData.length > 0 && /* @__PURE__ */ React.createElement(Button, {
        danger: true,
        onClick: handleClear,
        style: { borderRadius: "4px" }
      }, "Clear All")))), isAlertVisible && /* @__PURE__ */ React.createElement(Alert, {
        description: "Save images for later use before they disappear in an hour.",
        type: "info",
        closable: true
      }), /* @__PURE__ */ React.createElement(Row, {
        gutter: [16, 16],
        className: "genieimage-image-show"
      }, imageData?.map((item, index2) => /* @__PURE__ */ React.createElement(Col, {
        key: index2,
  ...
  // assets/src/admin/js/GenieImage/integration.js
  var { RenderElement } = window.genieImage.Components.Common;
  (() => {
    if (!wp.media) {
      return;
    }
    function genieImageMediaFrame(name, frame) {
      wp.media.view.MediaFrame[name] = frame.extend({
        initialize: function() {
          ...
          this.states.add([
            new State({
              id: "genieImage",
              search: false,
              title: "Genie Image AI"
            })
          ]);
          this.on(
            "content:render:genieImage",
            this.renderGenieImageContent,
            this
          );
        },
        ...
        renderGenieImageContent: function() {
          let GenieImageContainer = wp.Backbone.View.extend({
            render() {
              RenderElement(/* @__PURE__ */ React.createElement(GenieImage, null), this.el);
            }
          });
          let view = new GenieImageContainer();
          this.content.set(view);
        }
      });
    }
    if (window.genieImage.config.currentPage == "upload.php") {
      wp.media.view.Modal.prototype.on("ready", function(evt) {
        wp.media.view.Modal.prototype.off("open").on("open", function(e) {
          if (window.genieImage.config.currentPage == "upload.php") {
            wp.media.frame.content.mode("genieImage");
          }
        });
      });
      genieImageMediaFrame("Select", wp.media.view.MediaFrame.Select);
    } else {
      genieImageMediaFrame("Post", wp.media.view.MediaFrame.Post);
      genieImageMediaFrame("Select", wp.media.view.MediaFrame.Select);
    }
  })();
  ...
  var { Typography, Spin: Spin2 } = window.antd;
  var { __: __2 } = wp.i18n;
  var StatsTable = ({ data, loading, title, currentSubscriptions = [] }) => {
    ...
    }, /* @__PURE__ */ React.createElement("thead", null, /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("th", null, __2("Feature Type", "genie-image-ai")), /* @__PURE__ */ React.createElement("th", null, __2("Limit", "genie-image-ai")), /* @__PURE__ */ React.createElement("th", null, __2("Usage", "genie-image-ai")), /* @__PURE__ */ React.createElement("th", null, __2("Remaining Usage", "genie-image-ai")))), /* @__PURE__ */ React.createElement("tbody", null, loading ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("td", null, __2("Word Generate", "genie-image-ai"), ":"), /* @__PURE__ */ React.createElement("td", null, "---"), /* @__PURE__ */ React.createElement("td", null, "---"), /* @__PURE__ */ React.createElement("td", null, "---")), /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("td", null, __2("Serp Analysis", "genie-image-ai"), ":"), /* @__PURE__ */ React.createElement("td", null, "---"), /* @__PURE__ */ React.createElement("td", null, "---"), /* @__PURE__ */ React.createElement("td", null, "---")), /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("td", null, __2("Keyword Lookup", "genie-image-ai"), ":"), /* @__PURE__ */ React.createElement("td", null, "---"), /* @__PURE__ */ React.createElement("td", null, "---"), /* @__PURE__ */ React.createElement("td", null, "---")), /* @__PURE__ */ React.createElement("tr", {
      className: "Domain Limit"
    }, /* @__PURE__ */ React.createElement("td", null, __2("Domain Limit", "genie-image-ai"), ":"), /* @__PURE__ */ React.createElement("td", null, "---"), /* @__PURE__ */ React.createElement("td", null, "---"), /* @__PURE__ */ React.createElement("td", null, "---"))) : data?.map((item, index2) => /* @__PURE__ */ React.createElement("tr", {
      key: index2,
      className: item?.[0]
  ...
  var StatsTable_default = StatsTable;

  // assets/src/admin/js/Common/LimitUsage/index.js
  var { useState: useState3, useEffect: useEffect2 } = window.React;
  var { ComposeComponents: ComposeComponents3 } = window.genieImage.Components.Common.ReduxManager;
  var { Libs: Libs3 } = window.genieImage.Components.Common;
  var { HandleFetch: HandleFetch3, HandleResponse: HandleResponse3 } = window.genieImage.Components.Common.RequestManager;
  var LimitUses = ComposeComponents3(({ sidebar, setSidebar, limitUsage, setLimitUsage }) => {
    const [status, setStatus] = useState3(!!window.genieImage.config.siteToken);
    const [isModalVisible, setIsModalVisible] = useState3(sidebar.isUsageModalOpen);
    const [loadingTable, setLoadingTable] = useState3(false);
    const [siteData, setSiteData] = useState3(Object.keys(limitUsage.siteUsagesLimit));
    const [subscriptionData, setSubscriptionData] = useState3(Object.keys(limitUsage.subscriptionUsagesLimit));
    const [currentSite, setCurrentSite] = useState3("Current site");
    const [currentSubscriptions, setCurrentSubscriptions] = useState3([]);
    useEffect2(() => {
      setIsModalVisible(sidebar.isUsageModalOpen);
    }, [sidebar.isUsageModalOpen]);
    useEffect2(() => {
    ...
    const prepareUsageData = (result) => {
      ...
      if (Array.isArray(limit)) {
        ...
        data = Object.keys(limit).map((key) => {
          let arr = [];
    ...
    useEffect2(() => {
      if (Object.values(limitUsage?.subscriptionUsagesLimit).length === 0) {
        return;
      }
      const packageInfo = {
        limits: {},
      ...
      const packages = [];
      for (const obj of Object.values(limitUsage?.subscriptionUsagesLimit || {})) {
        packages.push({
      ...
      setCurrentSubscriptions(packages);
      let subscriptionResult = prepareUsageData(packageInfo);
      let domainResult = prepareUsageData(Object.values(limitUsage?.siteUsagesLimit || {})?.[0] || {});
      setCurrentSite(Object.keys(limitUsage?.siteUsagesLimit || {})?.[0] || "Current site");
      ...
      wp.data.dispatch("genieimage").setSidebar({ subscriptionStatistics: { usagePercentage: { ...usagePercentage } } });
    }, [limitUsage]);
    return /* @__PURE__ */ React.createElement(Libs3.Modal, {
      closeIcon: /* @__PURE__ */ React.createElement("span", {
        className: "genieimageicon-close1"
      }),
      isModalVisible,
      setIsModalVisible,
      onClose: closeModal,
      className: "genieimagelimit-usage-modal",
      centered: true,
      footer: null
    }, /* @__PURE__ */ React.createElement("div", {
      className: "genieimageinfo-wrapper-modal"
    }, /* @__PURE__ */ React.createElement("div", {
      className: "genieimagelicense-form",
      layout: "vertical"
  ...
  var LimitUsage_default = LimitUses;
    ...
    _proto.render = function render() {
      ...
      return /* @__PURE__ */ import_react2.default.createElement(context.Consumer, null, function(context2) {
        ...
        import_react2.default.Children.forEach(_this.props.children, function(child) {
          if (match == null && /* @__PURE__ */ import_react2.default.isValidElement(child)) {
            element = child;
        ...
        return match ? /* @__PURE__ */ import_react2.default.cloneElement(element, {
          location,
          computedMatch: match
        }) : null;
      });
  ...
  var { AdminPages } = window.genieImage.Components;
  var { ConfigProvider } = window.antd;
  var App = () => {
    const sidebar = wp.data.select("genieimage").sidebar();
    return /* @__PURE__ */ React.createElement(ConfigProvider, {
      getPopupContainer: (node) => sidebar.rootContainer || document.body,
      theme: {
        "components": {
          "Typography": {
    ...
    }, " ", /* @__PURE__ */ React.createElement(LimitUsage_default, null), AdminPages ? /* @__PURE__ */ React.createElement(HashRouter, {
      hashType: "noslash"
  ...
  var { RenderElement: RenderElement2 } = window.genieImage.Components.Common;
  addAction("genieimagerender-app", "getgenie_hooks", async () => {
    let appRoot = document.getElementById("genieimagecontainer");
    ...
    if (appRoot) {
      ...
      let cssURL = `${window.genieImage.config.assetsUrl + `dist/admin/styles/admin.css?ver=${window.genieImage.config?.version}`}`;
      await injectCSS(cssURL, shadow);
      const renderIn = document.createElement("div");
      renderIn.classList.add("genieimage");
      renderIn.classList.add("genieimageversion-" + window.genieImage.config?.version);
      renderIn.classList.add("genieimage-root");
      shadow.appendChild(renderIn);
      window.getGenie = window.getGenie || {};
      window.getGenie.shadow = window.getGenie.shadow || shadow;
      wp.data.dispatch("genieimage").setSidebar({
        rootContainer: appRoot.shadowRoot.querySelector(".genieimage-root")
      });
      RenderElement2(/* @__PURE__ */ React.createElement(App_default, null), renderIn);
    } else {
      console.warn("There is no toolbar.");
    }
  });
  setInterval(async () => {
    const response = await fetch(window.genieImage.config?.authTokenLeaserApi + "&_wpnonce=" + window.genieImage.config.restNonce).catch((err) => {
    });
    if (!response) {
      return;
    }
    if (response.ok) {
      try {
        ...
        window.genieImage.config.authToken = res;
      } catch (e) {
      }
    }
  }, 180 * 1e3);
