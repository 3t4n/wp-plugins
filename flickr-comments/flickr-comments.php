...
/*  Copyright 2012-2014  jdleung  (email : jdleungs@gmail.com)
...
function flickr_comments_config_page() {
	...
	if($fc_api['token']){
		...
		echo "<input name='Submit' type='submit' value='Do a manual-update' /></form><br><hr size=1>";		
...
function api_secret_input() {
	...
	echo "<font color='#777777'>Both API key and secret are needed. You can <a href='https://www.flickr.com/services/api/keys/'>Get Another Key</a> from Flickr. (Sign in first)<br><br />IMPORTANT: Edit the authentication flow to set its Callback URL to your site url: \"".$site_url."\".</font><br>";
	if($fc_api['token']){
...
function flcikr_comments_mupdate($manual_update_input){
	...
	$fc_api = get_option('flickr_comments_API');
	require_once('phpFlickr/phpFlickr.php');
	$f = new phpFlickr($fc_api['api_key'], $fc_api['api_secret'],true);
		...
		foreach ( $meta_infos as $meta_info ) {
			...
			if($comments['comments']['comment']) {
				foreach( $comments['comments']['comment'] as $comment ) {
					...
					$f_url = "https://www.flickr.com/photos/".$comment['author'];
				...
				if($post_id){	
					...
					foreach((array)$c['activity']['event'] as $key2=>$d){
						...
						if($type == "comment" && $c['type'] == "photo"){ // show only comments
							$f_url = "https://www.flickr.com/photos/".$d['user'];
...
function handle_comments($post_id,$f_commentdate,$f_url,$f_name,$f_comment,$f){
	...
	if(empty($has_dupes)){ 
		...
		if( preg_match("/(\[)(https.+photos\/.+)(\])/i",$f_comment,$matches2)){ 	//reply user icon from [flickr] to <img>
			...
			$f_comment = preg_replace("/(\[)(https.+photos\/.+)(\])/i", "<a href='".$user_link."'><img src='https://farm".$user_info['iconfarm'].".staticflickr.com/".$user_info['iconserver']."/buddyicons/".$user['id'].".jpg?3334118#".$user['id']."' alt='' width='24' height='24' border='0'></a>" ,$f_comment); 
		}		
		...
		if(empty($fc_config['allow_img'])){
			...
			if(preg_match('/(<img.*src.*staticflickr.*buddyicons.*#)(\d+@N\d+)(\'.*alt.*width.*>)/i', $f_comment, $matches3)){
				$user_link = "https://www.flickr.com/photos/".$matches3[2];
		...
		$f_comment = preg_replace('/<a href=\'\/photos\//', '<a href=\'https://www.flickr.com/photos/',$f_comment);
