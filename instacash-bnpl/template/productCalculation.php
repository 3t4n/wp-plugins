...
$extraClass = count($this->info['calculation']->payments) > 12 ? 'ic-scrollable' : '';
    ...
    <div id="InstaCash-calculation">
        <div class="border bg-ic mb-3 mt-3 rounded">
            <h4 class="border-bottom bg-white my-0 p-3 rounded-top">
                <span class="mx-2"><?php esc_html_e('Installment payment', 'instacash-bnpl'); ?></span>
            </h4>
            <div class="mx-2 p-3 rounded-bottom">
                <div class="mb-2" id="bnpl-one-line">
                    <?php echo wp_kses_post($this->info['one_line']); ?>
                </div>
                <div class="row mx-0 d-flex">
                    <?php foreach ($this->info['usp_list'] as $usp) : ?>
                        <?php if ($usp) : ?>
                            <div class="pt-3 pb-2">
                                <small class="usp border p-2 mr-2 rounded"><?php echo esc_html($usp); ?></small>
                            </div>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </div>
                <p class="pt-3 mb-0">
                    <a href="#" id="productCalculationModal" class="link-primary" data-base-price="<?php echo esc_attr($this->info['productPrice']); ?>" data-product-id="<?php the_ID(); ?>">
                        <strong><?php esc_html_e('Installment payment information', 'instacash-bnpl'); ?></strong>
                    </a>
                </p>
            </div>
        </div>
        ...
        <div class="modal-content bg-ic">
            <div style="margin-left:1.5 rem;">
                <div class="modal-header">
                    <h4 class="modal-title mb-0"><?php esc_html_e('Installment summary', 'instacash-bnpl'); ?></h4>
                    <span class="close">
                ...
                <div class="rounded shadow-md InstaCash">
                    <h5 class="modal-title my-0"><?php echo esc_html_e('Offer details', 'instacash-bnpl'); ?></h5>
                    <span id="until" hidden="hidden" style="display:none;"><?php esc_html_e(' until', 'instacash-bnpl'); ?></span>
                    ...
                    <div class="payments row p-3 <?php echo esc_attr($extraClass); ?>">
                        <?php
                        foreach ($this->info['calculation']->payments as $index => $occasion) :
                            $progress = 360 / count($this->info['calculation']->payments);
                            $dueDate  = gmdate('Y-m-d') == gmdate('Y-m-d', strtotime($occasion->dueDate)) ?
                            __('At the moment of purchase', 'instacash-bnpl') :
                            wp_date(get_option( 'date_format' ), strtotime($occasion->dueDate));
                            ?>
                        <div class="payment col-md-6 p-2 bg-white">
                            <div class="circle-wrapper">
                                <div class="payment-circle" style="--progress: <?php echo esc_attr($progress * $index); ?>deg;--progress-dark: 0deg <?php echo esc_attr($progress * ($index + 1)); ?>deg;"></div>
                            </div>
                            <div>
                                <h4 class="fw-bold pay-amount mb-0">
                                    <small class="pay"><?php echo wp_kses_post($occasion->sum); ?></small>
                                </h4>
                                <small class="date"><?php echo esc_attr($dueDate); ?></small>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
                    <div class="p-2 pt-2 offer-box">
                        <div class="offer-amounts">
                            <div class="row text-nowrap">
                                <div class="col">
                                    <small><?php esc_html_e('APR', 'instacash-bnpl'); ?></small>
                                </div>
                                <div class="col mw-25 text-right text-dark">
                                    <span class="offer-apr">
                                        <strong><?php echo esc_attr($this->info['calculation']->thm); ?></strong> %
                                    </span>
                                </div>
                            </div>
                            <div class="row text-nowrap">
                                <div class="col">
                                    <?php esc_html_e('Service fee', 'instacash-bnpl'); ?>
                                </div>
                                <div class="col mw-25 text-right text-dark">
                                    <span class="offer-fee">
                                        <strong><?php echo wp_kses_post($this->info['calculation']->fee); ?></strong>
                                    </span>
                                </div>
                            </div>
                            <div class="row text-nowrap">
                                ...
                                <div class="col mw-25 text-right text-dark">
                                    ...
                                    <strong><?php echo wp_kses_post($this->info['calculation']->total); ?></strong>
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="usp-blocks">
                        <div class="col px-2"><h4 class="mb-0"><?php echo esc_html($this->info['usp_title']); ?></h4></div>
                        ...
                        <div class="px-2 col d-flex py-3 usp rounded">
                            ...
                            <div class="d-grid px-2 ml-4">
                                <div><strong><?php echo wp_kses_post($this->info['usp_receipt_title']); ?></strong></div>
                                <div><?php echo wp_kses_post($this->info['usp_receipt_description']); ?></div>
                            </div>
                        </div>
                        ...
                        <div class="px-2 col d-flex py-3 usp rounded">
                            ...
                            <div class="d-grid px-2 ml-4">
                                <div><strong><?php echo wp_kses_post($this->info['usp_clock_title']); ?></strong></div>
                                <div><?php echo wp_kses_post($this->info['usp_clock_description']); ?></div>
                            </div>
                        </div>
                        ...
                        <div class="px-2 col d-flex py-3 usp rounded">
                            ...
                            <div class="d-grid px-2 ml-4">
                                <div><strong><?php echo wp_kses_post($this->info['usp_automat_title']); ?></strong></div>
                                <div><?php echo wp_kses_post($this->info['usp_automat_description']); ?></div>
                            </div>
                        </div>
                    ...
                    <div>
                        <a href="#" class="btn-primary" id="instacash-cart-trigger">
                            <?php esc_html_e('Add to cart', 'instacash-bnpl'); ?>
                        </a>
                    </div>
                </div>
            </div>
