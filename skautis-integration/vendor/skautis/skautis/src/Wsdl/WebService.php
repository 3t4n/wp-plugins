...
use Skautis_Integration\Vendor\Skautis\EventDispatcher\EventDispatcherTrait;
use Skautis_Integration\Vendor\Skautis\InvalidArgumentException;
use Skautis_Integration\Vendor\Skautis\SkautisQuery;
use SoapFault;
use stdClass;
use SoapClient;
/**
...
class WebService implements WebServiceInterface
{
    ...
    protected function prepareArgs($function_name, array $arguments)
    {
        ...
        if (!isset($arguments[1]) || $arguments[1] === null) {
            $function_name = \strtolower(\substr($function_name, 0, 1)) . \substr($function_name, 1);
            //nahrazuje lcfirst
            $args = [[$function_name . "Input" => $args]];
            return $args;
        }
        //pokud je zadan druhy parametr tak lze prejmenovat obal dat
        $matches = \preg_split('~/~', $arguments[1]);
        //rozdeli to na stringy podle /
