...
final class WpSpoki
{
	...
	public function render_buttons()
	{
        ...
        {
            $hide_non_working = isset($attrs['hide_non_working']) ? sanitize_text_field($attrs['hide_non_working']) : '';
            ...
            $phone = isset($attrs['phone']) ? sanitize_text_field($attrs['phone']) : Spoki()->shop['telephone'];
            $cta = isset($attrs['cta']) ? sanitize_text_field($attrs['cta']) : '';
            $title = isset($attrs['title']) ? sanitize_text_field($attrs['title']) : '';

            $message = isset($attrs['message']) ? sanitize_text_field($attrs['message']) : '';
            $non_working_message = isset($attrs['non_working_message']) ? sanitize_text_field($attrs['non_working_message']) : '';
            $is_non_working_days_times_text_enabled = isset($attrs['enable_non_working_message']) ? sanitize_text_field($attrs['enable_non_working_message']) : '';

            $final_message = $message;
            if ($is_non_working_day_time && $is_non_working_days_times_text_enabled == 1) {
                $final_message = $non_working_message;
            }

            $additional_class = isset($attrs['additional_class']) ? sanitize_html_class($attrs['additional_class']) : '';
            $type = 5;
            $color = isset($attrs['color']) ? sanitize_hex_color($attrs['color']) : '#23D366';
            $border_type = isset($attrs['border_type']) ? sanitize_text_field($attrs['border_type']) : null;
            $margin = [
                "top" => isset($attrs['margin_top']) ? intval($attrs['margin_top']) : 4,
                "bottom" => isset($attrs['margin_bottom']) ? intval($attrs['margin_bottom']) : 4,
                "left" => isset($attrs['margin_left']) ? intval($attrs['margin_left']) : 0,
                "right" => isset($attrs['margin_right']) ? intval($attrs['margin_right']) : 0,
            ];
            $padding = [
                "top" => isset($attrs['padding_top']) ? intval($attrs['padding_top']) : 8,
                "bottom" => isset($attrs['padding_bottom']) ? intval($attrs['padding_bottom']) : 8,
                "left" => isset($attrs['padding_left']) ? intval($attrs['padding_left']) : 14,
                "right" => isset($attrs['padding_right']) ? intval($attrs['padding_right']) : 14,
            ];
            $font_size = isset($attrs['font_size']) ? intval($attrs['font_size']) : 12;
            $id = isset($attrs['id']) ? sanitize_html_class($attrs['id']) : '';
            $class_names = isset($attrs['class_names']) ? sanitize_html_class($attrs['class_names']) : '';
            $custom_css = isset($attrs['custom_css']) ? wp_kses_post($attrs['custom_css']) : '';
    ...
    {
        $class = sprintf('button spoki-button %s', esc_attr(sanitize_html_class($additional_class)));
        if (isset($position)) {
        ...
        $furl = get_bloginfo('url');
        $href = SPOKI_BASE_API_BUTTONS . "?type={$type}&phone=" . rawurlencode($phone) . "&text=" . rawurlencode($final_message) . "&furl=" . rawurlencode($furl);
        $icon_size = intval($font_size) < 16 ? 16 : $font_size;

        $margin_style = "margin-top:" . esc_attr($margin['top']) . "px;margin-bottom:" . esc_attr($margin['bottom']) . "px;margin-left:" . esc_attr($margin['left']) . "px;margin-right:" . esc_attr($margin['right']) . "px;";
        $padding_style = "padding-top:" . esc_attr($padding['top']) . "px;padding-bottom:" . esc_attr($padding['bottom']) . "px;padding-left:" . esc_attr($padding['left']) . "px;padding-right:" . esc_attr($padding['right']) . "px;";

        echo "<div class='spoki-shadowed-button'><div class='spoki-button-relative " . esc_attr(sanitize_html_class($class_names)) . "' id='" . esc_attr(sanitize_html_class($id)) . "' style='{$margin_style}'><a href='". esc_url($href) ."' target='_blank' rel='nofollow' title='" . esc_attr($title) . "' class='{$class}' style='background-color:" . esc_attr(sanitize_hex_color($color)) . ";border-radius:" . esc_attr($border_radius) . ";font-size:" . esc_attr($font_size) . "px;{$padding_style}'>" . spoki_get_wa_logo($icon_size) . "<span>" . esc_html($cta) . "</span></a><style>" . wp_kses_post($custom_css) . "</style></div></div>";
    }
