...
class ContactsTest extends TestCase
{
    public function testShouldChangeEmail(): void
    {
        ...
        $changeEmailAddress = new ChangeEmailAddress('martin@smartemailing.cz', 'spock@smartemailing.cz');
        $api = $this->getApiMock();
        $api->expects($this->once())->method('post')->with('change-emailaddress', $changeEmailAddress->toArray())->will($this->returnValue(new Response(201, [], $expectedArray)));
        /** @var Contacts $api */
        $response = $api->changeEmailAddress($changeEmailAddress);
        $expectedObject = json_decode($expectedArray);
        $this->assertFalse(isset($expectedObject->data));
    ...
    public function testShouldGetAllContacts(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('contacts', (new \SmartemailingDeps\SmartEmailing\Api\Model\Search\Contacts())->toArray())->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var Contacts $api */
        $response = $api->getList(new \SmartemailingDeps\SmartEmailing\Api\Model\Search\Contacts());
        $expectedObject = json_decode($expectedArray);
    ...
    {
        $expectedArray = '';
        $uri = Helpers::replaceUrlParameters('contacts/forget/:id', ['id' => 1]);
        $api = $this->getApiMock();
    ...
    public function testShouldGetSingleContact(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('contacts/5')->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var Contacts $api */
