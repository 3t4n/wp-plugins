...
class WebhooksTest extends TestCase
{
    public function testShouldGetAllWebhooks(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('web-hooks', (new \SmartemailingDeps\SmartEmailing\Api\Model\Search\Webhooks())->toArray())->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var Webhooks $api */
        $response = $api->getList(new \SmartemailingDeps\SmartEmailing\Api\Model\Search\Webhooks());
    ...
    public function testShouldCreateWebhook(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('post')->with('web-hooks', $webhook->toArray())->will($this->returnValue(new Response(201, [], $expectedArray)));
        /** @var Webhooks $api */
    ...
    {
        $expectedArray = $this->getExpectedData();
        $uri = Helpers::replaceUrlParameters('web-hooks/:id', ['id' => 1]);
        $api = $this->getApiMock();
