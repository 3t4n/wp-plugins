...
use SmartemailingDeps\SmartEmailing\Api\Model\Bag\FeedItemBag;
use SmartemailingDeps\SmartEmailing\Api\Model\Bag\OrderItemBag;
...
class Order extends AbstractModel implements ModelInterface
{
    ...
    public function __construct(string $emailAddress, string $eshopName, string $eshopCode)
    {
    ...
    {
        AllowedTypeException::check($status, [self::STATUS_PLACED, self::STATUS_CANCELED, self::STATUS_PROCESSING, self::STATUS_SHIPPED, self::STATUS_UNKNOWN]);
        $this->status = $status;
        return $this;
    }
    ...
    #[ArrayShape(['emailaddress' => "string", 'eshop_name' => "string", 'eshop_code' => "string", 'status' => "string", 'paid_at' => "null|string", 'created_at' => "null|string", 'attributes' => "mixed", 'items' => "mixed", 'item_feeds' => "mixed"])]
    public function toArray(): array
    {
        return \array_filter(['emailaddress' => $this->getEmailAddress(), 'eshop_name' => $this->getEshopName(), 'eshop_code' => $this->getEshopCode(), 'status' => $this->getStatus(), 'paid_at' => $this->getPaidAt(), 'created_at' => $this->getCreatedAt(), 'attributes' => $this->getAttributeBag(), 'items' => $this->getOrderItemBag(), 'item_feeds' => $this->getFeedItemBag()], static fn($item) => !\is_object($item) && !empty($item) || \is_object($item) && \is_a($item, AbstractBag::class) && !$item->isEmpty());
    }
}
