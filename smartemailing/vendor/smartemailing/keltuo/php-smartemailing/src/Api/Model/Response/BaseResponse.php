...
use SmartemailingDeps\SmartEmailing\Api\Model\PropertyTrait;
...
class BaseResponse implements \JsonSerializable, \Stringable
{
    ...
    public function __construct(?ResponseInterface $response = null)
    {
        ...
        if (\is_object($json) && \property_exists($json, 'status')) {
            $this->json = $json;
            $this->setPropertyValue('status')->setPropertyValue('message');
        }
        ...
        if ($this->getStatus() === self::ERROR) {
            ...
            throw new RequestException($this, null, "Client error: {$errorMessage}", $this->getStatusCode());
        }
    }
    ...
    #[ArrayShape(['statusCode' => "int", 'status' => "string", 'meta' => "null|object", 'data' => "array", 'message' => "string"])]
    public function toArray(): array
    {
        return ['statusCode' => $this->getStatusCode(), 'status' => $this->getStatus(), 'meta' => $this->getMeta(), 'data' => $this->getData(), 'message' => $this->getMessage()];
    }
