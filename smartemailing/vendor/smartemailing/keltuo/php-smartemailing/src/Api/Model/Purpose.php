...
class Purpose extends AbstractModel
{
    ...
    public function __construct(string $lawfulBasis, string $name, Period $duration, ?string $notes = null)
    {
    ...
    {
        AllowedTypeException::check($lawfulBasis, [self::LAWFUL_CONSENT, self::LAWFUL_CONTRACT, self::LAWFUL_LEGAL_OBLIGATION, self::LAWFUL_LEGITIMATE_INTEREST, self::LAWFUL_VITAL_INTEREST, self::LAWFUL_PUBLIC_TASK]);
    ...
    #[ArrayShape(['lawful_basis' => "string", 'name' => "string", 'duration' => "SmartemailingDeps\\SmartEmailing\\Api\\Model\\Period", 'notes' => "null|string"])]
    public function toArray(): array
    {
        return ['lawful_basis' => $this->getLawfulBasis(), 'name' => $this->getName(), 'duration' => $this->getDuration(), 'notes' => $this->getNotes()];
    }
}
