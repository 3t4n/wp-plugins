...
namespace SmartemailingDeps\GuzzleHttp\Handler;

use SmartemailingDeps\GuzzleHttp\Exception\ConnectException;
use SmartemailingDeps\GuzzleHttp\Exception\RequestException;
use SmartemailingDeps\GuzzleHttp\Promise as P;
use SmartemailingDeps\GuzzleHttp\Promise\FulfilledPromise;
use SmartemailingDeps\GuzzleHttp\Promise\PromiseInterface;
use SmartemailingDeps\GuzzleHttp\Psr7\LazyOpenStream;
use SmartemailingDeps\GuzzleHttp\TransferStats;
use SmartemailingDeps\GuzzleHttp\Utils;
use SmartemailingDeps\Psr\Http\Message\RequestInterface;
use SmartemailingDeps\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \SmartemailingDeps\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \SmartemailingDeps\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \SmartemailingDeps\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \SmartemailingDeps\GuzzleHttp\Psr7\Utils::streamFor($sink);
