...
class ProcessingPurposesTest extends TestCase
{
    public function testShouldGetAll(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('purposes', (new \SmartemailingDeps\SmartEmailing\Api\Model\Search\Purposes())->toArray())->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var ProcessingPurposes $api */
        $response = $api->getList(new \SmartemailingDeps\SmartEmailing\Api\Model\Search\Purposes());
    ...
    public function testShouldGetAllConnections(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('purpose-connections', (new \SmartemailingDeps\SmartEmailing\Api\Model\Search\PurposeConnections())->toArray())->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var ProcessingPurposes $api */
        $response = $api->getListConnections(new \SmartemailingDeps\SmartEmailing\Api\Model\Search\PurposeConnections());
        $expectedObject = json_decode($expectedArray);
        $this->assertEquals($expectedObject->data, $response->getData());
        $this->assertEquals($expectedObject->meta, $response->getMeta());
        $this->assertEquals($expectedObject->status, $response->getStatus());
    ...
    public function testShouldCreatePurpose(): void
    {
        ...
        $purpose = new Purpose(Purpose::LAWFUL_CONTRACT, 'Pizza delivery', new Period(Period::DAYS, 10));
        $api = $this->getApiMock();
        $api->expects($this->once())->method('post')->with('purposes', $purpose->toArray())->will($this->returnValue(new Response(201, [], $expectedArray)));
        /** @var ProcessingPurposes $api */
        ...
        $this->assertEquals((!is_object($expectedObject->meta)) ? null : $expectedObject->meta, $response->getMeta());
        $this->assertEquals($expectedObject->status, $response->getStatus());
        $this->assertTrue($response->isSuccess());
    }
    public function testShouldRevokeConnections(): void
    {
        $uri = Helpers::replaceUrlParameters('purpose-connections/:id', ['id' => 1]);
        $api = $this->getApiMock();
        $api->expects($this->once())->method('delete')->with($uri)->will($this->returnValue(new Response(204)));
        /** @var ProcessingPurposes $api */
        $response = $api->revoke(1);
        $this->assertEquals(BaseResponse::HTTP_NOT_CONTENT_CODE, $response->getResponse()->getStatusCode());
        $this->assertEquals(BaseResponse::SUCCESS, $response->getStatus());
        $this->assertTrue($response->isSuccess());
    }
