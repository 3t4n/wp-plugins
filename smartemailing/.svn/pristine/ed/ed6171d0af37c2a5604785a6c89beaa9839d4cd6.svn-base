...
class TransactionalEmail extends AbstractModel
{
    ...
    public function __construct(SenderCredentials $senderCredentials, string $tag, ?TaskBag $taskBag = null)
    {
    ...
    {
        return \array_filter(['tag' => $this->getTag(), 'email_id' => $this->getEmailId(), 'message_contents' => $this->getMessageContent(), 'tasks' => $this->getTaskBag(), 'sender_credentials' => $this->getSenderCredentials()], static fn($item) => !\is_null($item));
    }
}
