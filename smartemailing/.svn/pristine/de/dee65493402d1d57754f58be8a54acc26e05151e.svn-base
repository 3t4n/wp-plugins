...
use SmartemailingDeps\SmartEmailing\Api\Model\Newsletter;
...
class NewsletterTest extends TestCase
{
    public function testShouldCreateNewsletter(): void
    {
        ...
        $newsletter = (new Newsletter(1, [1, 2]))->setPropertiesFromArr(json_decode('
                {
            ...
            ', \true));
        $api = $this->getApiMock();
        $api->expects($this->once())->method('post')->with('newsletter', $newsletter->toArray())->will($this->returnValue(new Response(201, [], $expectedArray)));
    ...
    {
        return \SmartemailingDeps\SmartEmailing\Api\Newsletter::class;
    }
}
