...
use SmartemailingDeps\SmartEmailing\Api\Model\Bag\OrderBag;
use SmartemailingDeps\SmartEmailing\Api\Model\FeedItem;
...
class EshopsTest extends TestCase
{
    public function testShouldImportOrders(): void
    {
        ...
        $orderItem1 = (new OrderItem('ABC123', 'My product', new Price(123.97, 150, 'CZK'), 1, 'https://www.example.com/my-product'))->setDescription('My product description')->setImageUrl('https://www.example.com/images/my-product.jpg');
        $orderItem1->getAttributeBag()->add(new Attribute('manufacturer', 'Factory ltd.'))->add(new Attribute('my other custom attribute', 'some value'));
        $orderItem2 = (new OrderItem('XYZ789', 'My another product', new Price(165.7, 200.5, 'CZK'), 2, 'https://www.example.com/my-another-product'))->setDescription('My another product description')->setImageUrl('https://www.example.com/images/my-another-product.jpg');
        $order1 = new Order('michal@smartemailing.cz', 'my-eshop', 'ORDER0001');
        $order1->setCreatedAt('2019-01-01 00:00:00');
        $order1->getAttributeBag()->add(new Attribute('discount', 'Black friday'));
        $order1->getOrderItemBag()->add($orderItem1)->add($orderItem2);
        $order1->getFeedItemBag()->add(new FeedItem('ZYX987', 'my-feed', 3));
        ...
        $importBag = (new OrderBag())->add($order1)->add($order2);
        $this->assertEquals(json_decode($this->getExpectedRequest(), \true), json_decode(json_encode($importBag), \true));
        $api = $this->getApiMock();
        $api->expects($this->once())->method('post')->with('orders-bulk', $importBag->toArray())->will($this->returnValue(new Response(201, [], $expectedArray)));
    ...
    public function testShouldCreateOrder()
    {
        ...
        $order1->getFeedItemBag()->add(new FeedItem('ZYX987', 'my-feed', 3));
        $api = $this->getApiMock();
        $api->expects($this->once())->method('post')->with('orders', $order1->toArray())->will($this->returnValue(new Response(201, [], $expectedArray)));
        /** @var Eshops $api */
    ...
    {
        return \SmartemailingDeps\SmartEmailing\Api\Eshops::class;
    }
}
