...
class Purpose extends AbstractModel implements ModelInterface
{
    ...
    #[ArrayShape(['id' => "int", 'valid_from' => "null|string", 'valid_to' => "null|string"])]
    public function toArray(): array
    {
        return \array_filter(['id' => $this->getId(), 'valid_from' => $this->getValidFrom(), 'valid_to' => $this->getValidTo()], static fn($item) => !\is_null($item));
    }
}
