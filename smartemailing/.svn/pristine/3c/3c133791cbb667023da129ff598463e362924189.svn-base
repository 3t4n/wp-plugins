...
use Stringable;
use function SmartemailingDeps\Symfony\Component\String\u;
abstract class AbstractSearch implements JsonSerializable, Stringable
{
    ...
    #[ArrayShape([self::TYPE_SELECT => "array", self::TYPE_EXPAND => "array", self::TYPE_SORT => "array", self::TYPE_ACTION_TYPE => "null|string", self::TYPE_LIMIT => "int", self::TYPE_OFFSET => "int"])]
    public function toArray(): array
    {
        return \array_filter([self::TYPE_SELECT => $this->select, self::TYPE_EXPAND => $this->expand, self::TYPE_SORT => $this->sort, self::TYPE_ACTION_TYPE => $this->actionType, self::TYPE_LIMIT => $this->limit, self::TYPE_OFFSET => $this->offset], static fn($item) => \is_array($item) && \count($item) > 0 || \is_string($item) && $item !== '' || !\is_string($item) && !\is_array($item) && !\is_null($item));
    }
    #[ArrayShape([self::TYPE_SELECT => 'array', self::TYPE_EXPAND => 'array', self::TYPE_SORT => 'array', self::TYPE_ACTION_TYPE => 'null|string', self::TYPE_LIMIT => 'int', self::TYPE_OFFSET => 'int'])]
