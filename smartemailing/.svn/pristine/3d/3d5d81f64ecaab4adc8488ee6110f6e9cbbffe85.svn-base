...
class EmailsTest extends TestCase
{
    public function testShouldCreateEmailFromTemplate(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('post')->with('emails/create-from-template', $template->toArray())->will($this->returnValue(new Response(200, [], $expectedArray)));
    ...
    public function testShouldCreateNewEmail(): void
    {
        ...
        $email = new \SmartemailingDeps\SmartEmailing\Api\Model\Email('Hello world, this is Subject!', 'Internal name of this e-mail, public will not see it', '&#x3C;span&#x3E;Hello!&#x3C;/span&#x3E;');
        $api = $this->getApiMock();
        $api->expects($this->once())->method('post')->with('emails', $email->toArray())->will($this->returnValue(new Response(201, [], $expectedArray)));
        /** @var \SmartEmailing\Api\Emails $api */
    ...
    public function testShouldGetList(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('emails', (new \SmartemailingDeps\SmartEmailing\Api\Model\Search\Emails())->toArray())->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var Emails $api */
        $response = $api->getList(new \SmartemailingDeps\SmartEmailing\Api\Model\Search\Emails());
        $expectedObject = json_decode($expectedArray);
    ...
    public function testShouldGetListConfirmation(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('confirmation-emails', (new \SmartemailingDeps\SmartEmailing\Api\Model\Search\Emails())->toArray())->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var Emails $api */
        $response = $api->getConfirmationList(new \SmartemailingDeps\SmartEmailing\Api\Model\Search\Emails());
        $expectedObject = json_decode($expectedArray);
        $this->assertEquals($expectedObject->data, $response->getData());
        $this->assertEquals($expectedObject->meta, $response->getMeta());
        $this->assertEquals($expectedObject->status, $response->getStatus());
    ...
    public function testShouldGetSingle(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with(Helpers::replaceUrlParameters('emails/:id', ['id' => 5]))->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var Emails $api */
