...
class Newsletter extends AbstractModel
{
     ...
     */
    protected bool $measureStats = \true;
    /**
     ...
     */
    public function __construct(int $emailId, array $contactListIds)
    {
    ...
    {
        return \array_filter(['email_id' => $this->getEmailId(), 'contactlists' => $this->getContactListIds(), 'excluded_contactlists' => $this->getExcludeContactListIds(), 'name' => $this->getName(), 'start' => $this->getStart(), 'measurestats' => $this->isMeasureStats(), 'sendOnPreferredTime' => $this->isSendOnPreferredTime(), 'senderemail' => $this->getSenderEmail(), 'sendername' => $this->getSenderName(), 'replyto' => $this->getReplyTo(), 'utm_source' => $this->getUtmSource(), 'utm_medium' => $this->getUtmMedium(), 'utm_campaign' => $this->getUtmCampaign(), 'utm_content' => $this->getUtmContent()], static fn($item) => !empty($item));
    }
}
