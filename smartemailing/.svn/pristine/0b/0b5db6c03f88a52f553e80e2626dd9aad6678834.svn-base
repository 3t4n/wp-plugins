...
use SmartemailingDeps\DI\Annotation\Inject;
use SmartemailingDeps\DI\Annotation\Injectable;
use SmartemailingDeps\DI\Definition\Exception\InvalidAnnotation;
use SmartemailingDeps\DI\Definition\ObjectDefinition;
use SmartemailingDeps\DI\Definition\ObjectDefinition\MethodInjection;
use SmartemailingDeps\DI\Definition\ObjectDefinition\PropertyInjection;
use SmartemailingDeps\DI\Definition\Reference;
use SmartemailingDeps\Doctrine\Common\Annotations\AnnotationRegistry;
use SmartemailingDeps\Doctrine\Common\Annotations\Reader;
use SmartemailingDeps\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use SmartemailingDeps\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'SmartemailingDeps\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'SmartemailingDeps\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('SmartemailingDeps\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'SmartemailingDeps\DI\Annotation\Injectable');
