...
use SmartemailingDeps\SmartEmailing\Api\Model\Search\CustomFieldOptions as SearchCustomFieldOptions;
use SmartemailingDeps\SmartEmailing\Api\Model\Search\SingleCustomFieldOptions as SearchSingleCustomFieldOptions;
/**
...
class CustomFieldOptions extends AbstractApi
{
    ...
    {
        return new Response($this->delete($this->replaceUrlParameters('customfield-options/:id', ['id' => $idCustomFieldOption])));
    }
    /**
    ...
    public function getSingle(int $idCustomFieldOption, ?SearchSingleCustomFieldOptions $search = null): Response
    {
        ...
        return new Response($this->get($this->replaceUrlParameters('customfield-options/:id', ['id' => $idCustomFieldOption]), $search->getAsQuery()));
    }
    /**
    ...
    {
        return new Response($this->patch($this->replaceUrlParameters('customfield-options/:id', ['id' => $idCustomFieldOption]), $customFieldOption->toArray()));
    }
}
