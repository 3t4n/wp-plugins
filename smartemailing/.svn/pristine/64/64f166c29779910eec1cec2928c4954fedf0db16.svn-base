...
class BulkUpload {
	...
	public function render_pending_customers_bulk_upload_notice(): void {
		if ( get_option( self::OPTION_CUSTOMERS_UPLOAD_PENDING_KEY ) ) {
			...
			<div class="notice notice-warning">
				<p><?php esc_html_e( 'The bulk upload of customers to SmartEmailing is pending.', 'smartemailing' ); ?></p>
			</div>
			<?php
		}
	}
	...
	public function render_pending_orders_bulk_upload_notice(): void {
		if ( get_option( self::OPTION_ORDERS_UPLOAD_PENDING_KEY ) ) {
			?>
			<div class="notice notice-warning">
				<p><?php esc_html_e( 'The bulk upload of orders to SmartEmailing is pending.', 'smartemailing' ); ?></p>
			</div>
			<?php
		}
	}
	...
	public function maybe_show_notice(): void {
		$process = filter_input( INPUT_GET, 'se-bulk-action', FILTER_SANITIZE_STRING );
		$run     = filter_input( INPUT_GET, 'run', FILTER_SANITIZE_STRING );
		...
		switch ( $run ) {
			case 'success':
				/* translators: Process type. */
				$string = sprintf( __( 'The action for %s was scheduled and will be running in the background soon.', 'smartemailing' ), $process );
				break;
			case 'already-run':
				/* translators: Process type. */
				$string      = sprintf( __( 'The action for %s is already scheduled and pending to run. It may take some time, please wait.', 'smartemailing' ), $process );
				$notice_type = 'warning';
				break;
		}
		...
		if ( $string ) {
			printf(
				'<div class="notice-%s notice"><p>%s</p></div>',
