...
use SmartemailingDeps\Wpify\Asset\AssetFactory;
...
class Templates {
	...
	public function render_web_form( $attributes = [], $content = '', $wfId = null ) {
		if ( isset( $attributes['webform_id'] ) ) {
			...
			// The script must be loaded this way.
			// phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript
			return '<div class="smartemailing-webform-list">' . $title . '<script src="' . self::WEBFORM_LINK . $hash . '" id="se-webformScriptLoader-' . $hash . '" async defer></script></div>';
		}
	...
	public function get_webform_html( $webForm, $hash, $title, $submit, $attributes ) {
		...
		$form .= '
				<div>
					<input type="submit" name="_submit" value="' . wp_strip_all_tags( $submit ) . '">
