...
class StatsTest extends TestCase
{
    public function testShouldGetAllSentCampaigns(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('campaign-stats-sent', (new \SmartemailingDeps\SmartEmailing\Api\Model\Search\CampaignStats())->toArray())->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var Stats $api */
        $response = $api->getCampaignSent(new \SmartemailingDeps\SmartEmailing\Api\Model\Search\CampaignStats());
    ...
    public function testShouldGetAllNewsletterSummary(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('newsletter-stats-summary', (new \SmartemailingDeps\SmartEmailing\Api\Model\Search\NewsletterStats())->toArray())->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var Stats $api */
        $response = $api->getNewsletterSummaries(new \SmartemailingDeps\SmartEmailing\Api\Model\Search\NewsletterStats());
        $expectedObject = json_decode($expectedArray);
        $this->assertEquals($expectedObject->data, $response->getData());
        $this->assertEquals($expectedObject->meta, $response->getMeta());
        $this->assertEquals($expectedObject->status, $response->getStatus());
