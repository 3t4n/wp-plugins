...
class WebFormsTest extends TestCase
{
    public function testShouldGetAllWebForms(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('web-forms')->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var WebForms $api */
    ...
    public function testShouldGetSingleWebForm(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('web-form-structure/5')->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var WebForms $api */
