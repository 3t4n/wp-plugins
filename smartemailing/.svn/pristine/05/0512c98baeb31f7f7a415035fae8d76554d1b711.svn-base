...
class FrontendTracking {
	...
	public function page_visit(): void {
		...
		if ( is_single( 'post' ) ) {
			$type       = 'article';
			$attributes = array( array( 'name' => 'post_id', 'value' => strval( get_the_ID() ) ) );
			...
			$type       = 'product';
			$attributes = array( array( 'name' => 'product_id', 'value' => strval( get_the_ID() ) ) );
		} elseif ( is_product_category() ) {
			$type       = 'category';
			$attributes = array( array( 'name' => 'category_id', 'value' => strval( get_the_ID() ) ) );
		} elseif ( is_search() ) {
			$type       = 'search';
			$attributes = array( array( 'name' => 'search_query', 'value' => get_search_query() ) );
		} else {
			$type       = 'other';
			$attributes = array( array( 'name' => 'page_id', 'value' => strval( get_the_ID() ) ) );
		}
		$data = array(
		...
		?>
		<script type="text/javascript">_se.push(['visit', <?php echo wp_json_encode( $data );?>]);</script>
	<?php }
