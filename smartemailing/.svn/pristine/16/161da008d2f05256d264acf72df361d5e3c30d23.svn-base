...
use SmartemailingDeps\SmartEmailing\Api\Model\LoginResponse;
...
class TestsApiTest extends TestCase
{
    public function testAliveness(): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method('get')->with('ping')->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var Tests $api */
    ...
    protected function loginTest($method): void
    {
        ...
        $api = $this->getApiMock();
        $api->expects($this->once())->method($method)->with('check-credentials')->will($this->returnValue(new Response(200, [], $expectedArray)));
        /** @var Tests $api */
        $method = $method . 'Login';
        /** @var LoginResponse $response */
        $response = $api->{$method}();
        $expectedObject = json_decode($expectedArray);
        $this->assertEquals($expectedObject->account_id, $response->getAccountId());
        $this->assertEquals(empty($expectedObject->meta) ? null : $expectedObject->meta, $response->getMeta());
        $this->assertEquals($expectedObject->status, $response->getStatus());
        $this->assertTrue($response->isSuccess());
    }
