...
class Webhook extends AbstractModel
{
    ...
    {
        AllowedTypeException::check($event, [self::EVENT_NEW_CONTACT, self::EVENT_DELIVERY_STATUS, self::EVENT_UPDATED_CONTACT, self::EVENT_UNSUBSCRIBE_CONTACT, self::EVENT_NEWSLETTER_SENDING_STARTED, self::EVENT_NEWSLETTER_SENDING_FINISHED, self::EVENT_SENT_EMAIL, self::EVENT_SCORING_RESULT_CHANGED]);
        $this->event = $event;
        return $this;
    }
    #[ArrayShape(['target_url' => "string", 'event' => "string"])]
    public function toArray(): array
    {
        return ['target_url' => $this->getTargetUrl(), 'event' => $this->getEvent()];
    }
}
