...
class Order {
	...
	public function __construct(
		...
		add_action( 'woocommerce_order_status_changed', array( $this, 'maybe_schedule_subscribe_to_smartemailing' ), 10, 3 );
		...
		<p class="form-row smartemailing-opt_in">
			<label class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox">
				<input type="checkbox" class="woocommerce-form__input woocommerce-form__input-checkbox input-checkbox"
				       name="<?php echo esc_attr( self::OPT_IN_KEY ); ?>" style="width: auto;"
					<?php
					// Missing input var ok, csrf ok.
					// phpcs:ignore WordPress.Security.NonceVerification.Missing
					checked( isset( $_POST[ self::OPT_IN_KEY ] ) || $this->settings_repository->get_general_setting( GeneralSettings::CHECKBOX_CHECKED ), true );
					?>
				/>
				<span class="smartemailing-opt_in-checkbox-text">
	                <?php
	                echo esc_html(
		                $this->settings_repository->get_general_setting( GeneralSettings::SUBSCRIPTION_TEXT ),
	                );
	                ?></span>&nbsp;
			</label>
		</p>
