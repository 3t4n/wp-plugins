...
class DSAMM_Admin_Interface {
    ...
    function dsamm_save_existing_menu( $menu_items ) {
        ...
        if ( !current_user_can( 'edit_theme_options' ) ) {
            return '<div id="message" class="error notice is-dismissible"><p>' . __( 'You do not have permission to edit menus.', 'advance-menu-manager' ) . '</p><button type="button" class="notice-dismiss"></button></div>';
        }
        ...
        $keys = range( 1, $end );
        $deleted_nodes = ( isset( $_POST['delete_menu_items'] ) ? sanitize_text_field( wp_unslash( $_POST['delete_menu_items'] ) ) : '' );
            ...
            foreach ( $nav_menus as $list_nav_menus ) {
                ...
                if ( !empty( $menu_locations ) && in_array( $list_nav_menus->term_id, $menu_locations, true ) ) {
                    ...
                    if ( !empty( $assigned_locations ) ) {
                        printf( ' (%1$s%2$s)', esc_html( implode( ', ', $assigned_locations ) ), ( count( $locations_assigned_to_this_menu ) > count( $assigned_locations ) ? ' &hellip;' : '' ) );
                    }
                }
                ?>
                                        </option>
