...
class DSAMM_Revision_Ajax_Action {
    ...
    public static function dsamm_menu_edit_action_method_own() {
        ...
        if ( !current_user_can( 'edit_posts' ) ) {
            $return_data['error'] = esc_html__( 'Unauthorized access.', 'advance-menu-manager' );
    ...
    public static function dsamm_add_new_menu_item_html_own() {
        $return_data = array();
        $action = filter_input( INPUT_POST, 'action', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
        if ( !empty( $action ) && 'my_action_for_add_new_menu_item_html' === $action ) {
            check_ajax_referer( 'dsamm_ajax_value_nonce', 'security' );
        }
    ...
    public static function dsamm_add_pagination_post_per_page_limit_method() {
        check_ajax_referer( 'dsamm_ajax_value_nonce', 'security' );
        $return_data = array();
        // Ensure the current user has the appropriate capability
        ...
        // Enforce a specific prefix for option keys
        $option_prefix = 'amm_';
        if ( 'my_action_for_add_pagination_limit' === $action && !empty( $amm_option_key ) ) {
            if ( strpos( $amm_option_key, $option_prefix ) === 0 ) {
                // Option key starts with the allowed prefix
                update_option( $amm_option_key, $page_per_post );
                $return_data['success'] = esc_html__( 'Settings updated successfully.', 'advance-menu-manager' );
            } else {
                // Invalid option key
                $return_data['error'] = esc_html__( 'Invalid option key.', 'advance-menu-manager' );
            }
        } else {
            $return_data['error'] = esc_html__( 'Invalid action or missing parameters.', 'advance-menu-manager' );
        }
        echo wp_json_encode( $return_data );
        exit;
    }
