...
namespace antibots_BillCatchErrors;
// created 06/23/23
...
function bill_install_mu_plugin()
{
    ...
    $antibots_mu_plugin_dir = WP_PLUGIN_DIR . '/antibots/includes/mu-plugins'; // Current path inside antibots
    ...
    */
    ...
    try {
        ...
        // Define the plugin file path in the antibots directory
        $source = $antibots_mu_plugin_dir . '/' . $plugin_file;
...
add_action("wp_ajax_bill_minozzi_js_error_catched", "antibots_BillCatchErrors\\bill_minozzi_js_error_catched");
add_action("wp_ajax_nopriv_bill_minozzi_js_error_catched", "antibots_BillCatchErrors\\bill_minozzi_js_error_catched");
    ...
    if (isset($_REQUEST)) {
        ...
        foreach ($errors as $error) {
            ...
            if ($log_error) {
                ...
                if ($antibots_plugin_slug  == 'antibots')
                    antibotsErrorHandler('Javascript', $errorMessage, $errorURL, $errorLine);
...
class antibots_bill_catch_errors
{
...
new antibots_bill_catch_errors();
//
