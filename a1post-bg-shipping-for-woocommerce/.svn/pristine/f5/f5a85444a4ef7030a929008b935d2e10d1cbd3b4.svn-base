<?php
/*
Plugin Name: A1POST.BG Shipping for WooCommerce
...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
  ...
  function a1post_window_print() {
    ...
    if (isset($_GET['a1post_batch'])) {
      ...
      if (!empty($d->error)) die(esc_html($d->error->message));
      if (!empty($d->lbl)) {
  ...
  function a1post_window() {
    ...
    if (isset($_GET['a1post'])) {
      $nonce = key_exists('_wpnonce', $_GET) ? sanitize_text_field(wp_unslash($_GET['_wpnonce'])) : '';
      if (!(wp_verify_nonce($nonce, 'A1P0st-bg')) || !(is_user_logged_in())) exit;
      $orders = !empty($_GET['o']) ? explode(',', preg_replace('/\D/', '', (int) $_GET['o'])) : '';
      $serv = !empty($_GET['serv']) ? sanitize_text_field(wp_unslash($_GET['serv'])) : '';
      $print = !empty($_GET['print']) ? sanitize_text_field(wp_unslash($_GET['print'])) : '';
      $total_wgh = 0;
      $notes = '';
      foreach ($orders as $order_id) {
        $order = new WC_Order($order_id);
        ...
        $data = ['name' => "$order->shipping_first_name $order->shipping_last_name", 'addr1' => $addr1, 'addr2' => $addr2, 'addr3' => $addr3, 'city' => $order->shipping_city, 'state' => $shipping_state, 'zip' => $order->shipping_postcode, 'iso' => $shipping_country, 'tel' => $tel, 'email' => $email, 'notes' => 'WOO- ' . $notes, 'weight' => $total_wgh, 'lbl' => $lbl_type, 'serv' => $serv];
      }

      if (!empty($print)) $c = wp_remote_get("https://api.a1post.bg/print.php?track=$print&lbl=$lbl_type", $args);
      else {
        $args['timeout'] = 15;
        $args['body'] = $data;
  ...
  function a1post_admin_settings_page() {
    ...
    if (isset($_POST['a1post_settings_submitted']) && $_POST['a1post_settings_submitted'] == 'submitted') {
      $nonce = key_exists('_wpnonce', $_POST) ? sanitize_text_field(wp_unslash($_POST['_wpnonce'])) : '';
      if (!(wp_verify_nonce($nonce, 'A1P0st-bg')) || !(is_user_logged_in())) exit;
      if (get_option('a1post_u') != $_POST['a1post_u']) update_option('a1post_u', $_POST['a1post_u']);		else $status = add_option('a1post_u', $_POST['a1post_u'], '', 'no');
      if (get_option('a1post_p') != $_POST['a1post_p']) update_option('a1post_p', $_POST['a1post_p']);		else $status = add_option('a1post_p', $_POST['a1post_p'], '', 'no');
      if (get_option('a1post_lbl') != $_POST['a1post_lbl']) update_option('a1post_lbl', $_POST['a1post_lbl']);	else $status = add_option('a1post_lbl', $_POST['a1post_lbl'], '', 'no');
      if (get_option('a1post_st') != $_POST['a1post_st']) update_option('a1post_st', $_POST['a1post_st']);	else $status = add_option('a1post_st', $_POST['a1post_st'], '', 'no');
...
<?php
  ...
  foreach ($statuses as $k => $v) {
    ...
    echo '<option value="'.esc_html($s).'" '.selected($s, $st_options).'>'.esc_html($v).'</option>';
  }
?>
</select></p>
</td></tr>
</table>
<input type="hidden" name="_wpnonce" value="<?php echo esc_html(wp_create_nonce('A1P0st-bg')); ?>">
<?php
submit_button(); 
?>
</form>
  ...
  function a1post_ship_order_box() {
    ...
    if (isset($_GET['a1post_del'])) {
      ...
      if ($_GET['a1post_del'] == $trk) {
        ...
        if ($http_code == "401") die('Грешно потребителско име/парола!');
        $d = json_decode(wp_remote_retrieve_body($c));
        if (isset($d->error)) die(esc_html($d->error->message));
...
<script>function a1open(service){jQuery("#woocommerce-shipment-a1post").html("<div align=\'center\'><div class=\'a1post-loader\'></div><h2 style=\'color:darkred\'>Зарежда, моля изчакайте.</h2></div>");jQuery.get("?", {a1post: 1, o: "'.$order->id.'", serv: service, _wpnonce: "'.esc_html(wp_create_nonce('A1P0st-bg')).'"}, function(d) {d = JSON.parse(d); if (d.lbl) {jQuery("#order_data").append("<div><object id=\"a1post-pdf\" type=\"application/pdf\" data=\"data:application/pdf;base64,"+d.lbl+"\"></object></div>");jQuery("#woocommerce-shipment-a1post").html("<div class=\'inside\'><p><a href=\'https://a1post.bg/track/"+d.track+"\' target=\'_blank\'>"+d.track+"</a></p><p><a class=\'button button-primary\' target=\'_blank\' href=\'?a1post_batch='.$order->id.'\'>Принтирай етикет</a></p><br><p>Анулирай текущата и генерирай нова товарителница:</p><p><a class=\'button button-primary\' href=\'post.php?post='.$post->ID.'&action=edit&a1post_del="+d.track+"\'>Нова товарителница</a></p></div>");}else jQuery("#woocommerce-shipment-a1post").html("<h2 style=\'color:white;background:#ea3a3a\'>Error: "+d.error.message+"</h2>");});return false;}jQuery(document).on("keydown",function(e){if(e.which==13)return false;});</script>';
  }
