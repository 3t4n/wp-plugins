...
if (!function_exists('izcrmef_activate_plugin')) {
    function izcrmef_activate_plugin()
    {
        ...
        if (version_compare($wp_version, '5.1', '<')) {
            wp_die(
                esc_html__('This plugin requires WordPress version 5.1 or higher.', 'izcrmef'),
                esc_html__('Error Activating', 'izcrmef')
            );
        }
        if (version_compare(PHP_VERSION, '5.6.0', '<')) {
            wp_die(
                esc_html__('Forms Integrations requires PHP version 5.6.', 'izcrmef'),
                esc_html__('Error Activating', 'izcrmef')
            );
        }
