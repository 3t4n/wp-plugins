...
if ( !class_exists( 'WPMM_Memory_Meter' ) ) {
	...
	define( 'WPMM_MEMORY_METER_VERSION', '2.3.0' );
	...
	class WPMM_Memory_Meter {
		...
		public function page() {
			...
			<div class="wrap">
				<h1 class="wp-heading-inline"><?php esc_html_e( 'Memory Meter', 'wpmm-memory-meter' ); ?></h1>
				<h2 class="wpmm-memory-meter-page-tabs nav-tab-wrapper">
					<a href="<?php echo esc_url( $this->dashboard_url ); ?>" class="nav-tab<?php echo ( '' == $tab ? ' nav-tab-active' : '' ) ; ?>"><?php esc_html_e( 'Logs', 'wpmm-memory-meter' ); ?></a>
					<a href="<?php echo esc_url( add_query_arg( 'tab', 'configuration' ) ); ?>" class="nav-tab<?php echo ( 'configuration' == $tab ? ' nav-tab-active' : '' ) ; ?>"><?php esc_html_e( 'Configuration', 'wpmm-memory-meter' ); ?></a>
					<?php if ( '' == $tab ) { ?>
						<div class="wpmm-memory-meter-page-tabs-buttons">
							<a href="<?php echo esc_url( $this->dashboard_url ); ?>" class="button button-primary button-small"><span class="dashicons dashicons-update"></span> <?php esc_html_e( 'Refresh logs', 'wpmm-memory-meter' ); ?></a>
							<form method="post">
								<?php wp_nonce_field( 'wpmm_memory_meter_clear_logs_submit', 'wpmm_memory_meter_clear_logs_submit_nonce' ); ?>
								<button name="wpmm_memory_meter_clear_logs" id="wpmm-memory-meter-clear-logs" class="button button-small"><span class="dashicons dashicons-dismiss"></span> <?php esc_html_e( 'Clear logs', 'wpmm-memory-meter' ); ?></button>
							</form>
						</div>
					<?php } ?>
				</h2>
				...
				if ( false == apply_filters( 'wpmm_memory_meter_logs', true ) ) {
					...
					<div class="wpmm-memory-meter-page-notice-logs-disabled wpmm-memory-meter-page-notice notice notice-error inline">
						<p><strong><?php esc_html_e( 'Logs are currently disabled in configuration so no further logging will occur.', 'wpmm-memory-meter' ); ?></strong></p>
					</div>
				...
				if ( '' == $tab ) {
					...
					<h2><?php esc_html_e( 'PHP and WordPress Configuration', 'wpmm-memory-meter' ); ?></h2>
					<p><?php esc_html_e( 'Current PHP and WordPress memory related configuration is displayed below:', 'wpmm-memory-meter' ); ?></p>
					<?php $this->php_wordpress_configuration(); ?>
					<hr>
					<h2><?php esc_html_e( 'Filter Hooks', 'wpmm-memory-meter' ); ?></h2>
					<p><?php esc_html_e( 'Configure memory meter using these filter hooks:', 'wpmm-memory-meter' ); ?></p>
					<?php
		...
		public function logs_table( $logs ) {
			...
			<table class="wpmm-memory-meter-page-table widefat striped fixed">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Date', 'wpmm-memory-meter' ); ?></th>
						<th><?php esc_html_e( 'Memory usage', 'wpmm-memory-meter' ); ?></th>
						<th><?php esc_html_e( 'Page request', 'wpmm-memory-meter' ); ?></th>
					</tr>
				</thead>
				<tbody>
					...
					if ( !empty( $logs ) && is_array( $logs ) ) {
						...
						foreach ( $logs as $log ) {
							...
							<tr>
								<td>
									<?php echo esc_html( get_date_from_gmt( gmdate( 'Y-m-d H:i:s' , $log['date'] ), get_option( 'date_format' ) ) ); ?><br>
									<small><?php echo esc_html( get_date_from_gmt( gmdate( 'Y-m-d H:i:s' , $log['date'] ), get_option( 'time_format' ) ) ); ?></small>
								</td>
								<?php // translators: status ?>
								<td title="<?php echo esc_attr( sprintf( esc_html__( 'Status: %s', 'wpmm-memory-meter' ), ucfirst( $log['status'] ) ) ); ?>">
									<?php if ( true == apply_filters( 'wpmm_memory_meter_memory_usage_colors', true ) ) { ?>
										<span class="wpmm-memory-meter-memory-usage-dot wpmm-memory-meter-memory-usage-color-<?php echo esc_html( $log['status'] ); ?>"></span>
									<?php } ?>
									<?php echo wp_kses_post( $log['usage'] ); ?>
								</td>
								<td>
									<?php
									if ( isset( $log['page'] ) ) {
										?>
										<small><a href="<?php echo esc_url( $this->site_url . $log['page'] ); ?>" target="_blank"><?php echo esc_url( $log['page'] ); ?></a></small>
										<?php
									} else {
										?>
										<small><strong><?php esc_html_e( 'Unavailable', 'wpmm-memory-meter' ); ?></strong></small>
										<?php
									}
									?>
								</td>
						...
						<tr>
							<td colspan="3"><?php esc_html_e( 'No logs yet.', 'wpmm-memory-meter' ); ?></td>
						</tr>
		...
		public function php_wordpress_configuration() {
			...
			<div class="wpmm-memory-meter-page-php-wordpress-configuration">
				<div class="wpmm-memory-meter-page-php-wordpress-configuration-box wpmm-memory-meter-page-php-wordpress-configuration-box-php">
					<?php $this->php_logo(); ?>
					<strong><?php echo esc_html( $this->php_memory_limit_display ); ?></strong>
					<a href="<?php echo esc_url( $this->php_memory_limit_url ); ?>" target="_blank" class="button"><?php esc_html_e( 'More info', 'wpmm-memory-meter' ); ?></a>
				</div>
				<div class="wpmm-memory-meter-page-php-wordpress-configuration-box wpmm-memory-meter-page-php-wordpress-configuration-box-php">
					<?php $this->php_logo(); ?>
					<strong><?php echo esc_html( $this->php_max_execution_time_display ); ?></strong>
					<a href="<?php echo esc_url( $this->php_max_execution_time_url ); ?>" target="_blank" class="button"><?php esc_html_e( 'More info', 'wpmm-memory-meter' ); ?></a>
				</div>
				<div class="wpmm-memory-meter-page-php-wordpress-configuration-box wpmm-memory-meter-page-php-wordpress-configuration-box-wordpress">
					...
					<strong><?php echo esc_html( $this->wp_memory_limit_display ); ?></strong>
					<a href="<?php echo esc_url( $this->wp_memory_limit_url ); ?>" target="_blank" class="button"><?php esc_html_e( 'More info', 'wpmm-memory-meter' ); ?></a>
				</div>
				<div class="wpmm-memory-meter-page-php-wordpress-configuration-box wpmm-memory-meter-page-php-wordpress-configuration-box-wordpress">
					<span class="dashicons dashicons-wordpress"></span>
					<strong><?php echo esc_html( $this->wp_max_memory_limit_display ); ?></strong>
					<a href="<?php echo esc_url( $this->wp_max_memory_limit_url ); ?>" target="_blank" class="button"><?php esc_html_e( 'More info', 'wpmm-memory-meter' ); ?></a>
				</div>
			</div>
		...
		public function filter_hooks_table() {
			...
			<table class="wpmm-memory-meter-page-table widefat fixed striped">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Hook name', 'wpmm-memory-meter' ); ?></th>
						<th><?php esc_html_e( 'Arguments', 'wpmm-memory-meter' ); ?></th>
						<th><?php esc_html_e( 'Description', 'wpmm-memory-meter' ); ?></th>
						<th><?php esc_html_e( 'Default value', 'wpmm-memory-meter' ); ?></th>
						<th><?php esc_html_e( 'Current value', 'wpmm-memory-meter' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						...
						<code>$capability</code><br>
							<small><?php esc_html_e( '(string)', 'wpmm-memory-meter' ); ?> <?php esc_html_e( 'WordPress capability', 'wpmm-memory-meter' ); ?></small>
						</td>
						<td>
							<?php
						...
						<td><code><?php echo esc_html( apply_filters( 'wpmm_memory_meter_capability', 'manage_options' ) ); ?></code></td>
					</tr>
					<tr>
						<td><code>wpmm_memory_meter_file_size_format</code></td>
						<td>
							<code>$file_size_format</code><br>
							<?php // translators: file size formats ?>
							<small><?php esc_html_e( '(string)', 'wpmm-memory-meter' ); ?> <?php echo sprintf( esc_html__( 'File size format as %1$s, %2$s or %3$s', 'wpmm-memory-meter' ), 'KB', 'MB', 'GB' ); ?></small>
						</td>
						<td><?php esc_html_e( 'Set the file size format for the memory meter.', 'wpmm-memory-meter' ); ?></td>
						<td><code>MB</code></td>
						<td><code><?php echo esc_html( apply_filters( 'wpmm_memory_meter_file_size_format', 'MB' ) ); ?></code></td>
					</tr>
					<tr>
						<td><code>wpmm_memory_meter_logs</code></td>
						<td>
							<code>$enabled</code><br>
							<small><?php esc_html_e( '(bool)', 'wpmm-memory-meter' ); ?> <?php esc_html_e( 'true or false', 'wpmm-memory-meter' ); ?></small>
						</td>
						<td><?php esc_html_e( 'Enable or disable memory usage logs.', 'wpmm-memory-meter' ); ?></td>
						<td><code>true</td>
						<td><code><?php echo esc_html( ( apply_filters( 'wpmm_memory_meter_logs', true ) ? 'true' : 'false' ) ); ?></code></td>
					</tr>
					<tr>
						...
						<td><?php esc_html_e( 'Enable or disable logging of Memory Meter dashboard based page requests.', 'wpmm-memory-meter' ); ?></td>
						<td><code>false</td>
						<td><code><?php echo esc_html( ( apply_filters( 'wpmm_memory_meter_logs_dashboard_page_requests', false ) ? 'true' : 'false' ) ); ?></code></td>
					</tr>
					<tr>
						<td><code>wpmm_memory_meter_logs_max</code></td>
						<td>
							<code>$max</code><br>
							<small><?php esc_html_e( '(int)', 'wpmm-memory-meter' ); ?> <?php esc_html_e( 'Max logs number', 'wpmm-memory-meter' ); ?></small>
						</td>
						<td><?php esc_html_e( 'Set the maximum number of logs.', 'wpmm-memory-meter' ); ?></td>
						<td><code>20</code></td>
						<td><code><?php echo esc_html( apply_filters( 'wpmm_memory_meter_logs_max', 20 ) ); ?></code></td>
					</tr>
					<tr>
						...
						<td><?php esc_html_e( 'Set whether memory usage colors should be applied.', 'wpmm-memory-meter' ); ?></td>
						<td><code>true</code></td>
						<td><code><?php echo esc_html( ( apply_filters( 'wpmm_memory_meter_memory_usage_colors', true ) ? 'true' : 'false' ) ); ?></code></td>
					</tr>
					<tr>
						...
						<?php // translators: PHP's memory_get_peak_usage() and $real_usage argument ?>
						<td><?php echo wp_kses_post( sprintf( __( 'Set if memory usage got from PHP\'s %1$s enables %2$s.', 'wpmm-memory-meter' ), '<a href="' . esc_url( 'https://www.php.net/manual/en/function.memory-get-peak-usage.php' ) . '" target="_blank">memory_get_peak_usage()</a>', '<code>$real_usage</code>' ) ); ?></td>
						<td><code>true</code></td>
						<td><code><?php echo ( apply_filters( 'wpmm_memory_meter_memory_usage_peak_real', true ) ? 'true' : 'false' ); ?></code></td>
					</tr>
					<tr>
						<td><code>wpmm_memory_meter_memory_usage_percentage_bad</code></td>
						<td>
							<code>$percentage</code><br>
							<small><?php esc_html_e( '(int)', 'wpmm-memory-meter' ); ?> <?php esc_html_e( 'Percentage number', 'wpmm-memory-meter' ); ?></small>
						</td>
						<td><?php esc_html_e( 'Set the bad memory usage percentage.', 'wpmm-memory-meter' ); ?></td>
						<td><code>90</code></td>
						<td><code><?php echo esc_html( apply_filters( 'wpmm_memory_meter_memory_usage_percentage_bad', 90 ) ); ?></code></td>
					</tr>
					<tr>
						...
						<td><?php esc_html_e( 'Set the warning memory usage percentage.', 'wpmm-memory-meter' ); ?></td>
						<td><code>75</code></td>
						<td><code><?php echo esc_html( apply_filters( 'wpmm_memory_meter_memory_usage_percentage_warning', 75 ) ); ?></code></td>
					</tr>
				</tbody>
			</table>
		...
		public function memory_usage() {
			...
			$memory_usage_string = sprintf( esc_html__( 'Memory: %1$s (%2$s of %3$s)', 'wpmm-memory-meter' ), $memory_usage, $memory_usage_percent . '%', esc_html( $this->php_memory_limit ) );
			...
			echo '<div id="wpmm-memory-meter-memory-usage" class="' . esc_attr( $memory_usage_class ) . '" title="' . esc_attr( sprintf( esc_html__( 'Status: %s', 'wpmm-memory-meter' ), $memory_usage_status_name ) ) . '" style="display: none !important;" data-memory-usage="' . esc_html( $memory_usage_string ) . '"></div>'; // Inline display: none !important added, this is done via CSS asset but also inline here as some plugins can call wp_footer but not the enqueuing functions (e.g. email customizer plugins), the memory usage is not included in the div and instead in the data-memory-usage attribute to stop search engines deeming it as page content

			// Output memory usage JS

			echo "<script>console.log( '" . esc_html__( 'PHP', 'wpmm-memory-meter' ) . " ' + document.getElementById( 'wpmm-memory-meter-memory-usage' ).getAttribute( 'data-memory-usage' ) );</script>"; // Memory usage is not echoed here and is got from the data-memory-usage attribute for same reasons as comment above
			...
			if ( true == apply_filters( 'wpmm_memory_meter_logs', true ) ) {
				...
				if ( count( $logs_latest ) >= (int) apply_filters( 'wpmm_memory_meter_logs_max', 20 ) ) {
					...
					$logs_latest = array_slice( $logs_latest, count( $logs_latest ) - (int) apply_filters( 'wpmm_memory_meter_logs_max', 20 ) + 1 ); // Ensures entries do not exceed the max set (this also means if wpmm_memory_meter_logs_max was a high number and then changed to a lower number the log entries are reduced accordingly)
				...
				if ( true == $memory_usage_flagged ) {
					...
					if ( count( $logs_flagged ) >= (int) apply_filters( 'wpmm_memory_meter_logs_max', 20 ) ) {
						...
						$logs_flagged = array_slice( $logs_flagged, count( $logs_flagged ) - (int) apply_filters( 'wpmm_memory_meter_logs_max', 20 ) + 1 ); // Ensures entries do not exceed the max set (this also means if wpmm_memory_meter_logs_max was a high number and then changed to a lower number the log entries are reduced accordingly)
				...
				$log_new = array(
					'date'		=> time(),
					// translators: memory usage, percent and limit
					'usage'		=> sprintf( esc_html__( '%1$s (%2$s of %3$s)', 'wpmm-memory-meter' ), $memory_usage, $memory_usage_percent . '%', esc_html( $this->php_memory_limit ) ),
