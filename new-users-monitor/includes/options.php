			...
			?>
				<div class="wrap">
					...
					<div class="wdpq_options_box">						
						<?php echo __('Warning! Data Incorrect. Update Disable.', 'new-users-monitor'); ?>
					</div>
				</div>
			<?php
	...
	if ($NUM_Setup_AdminOnly) {
		if (!current_user_can('edit_dashboard')) {
			...
			<div class="num_warning" style="margin: 40px;">				
				<?php echo __('Sorry, you are not allowed to view this page.', 'new-users-monitor'); ?>
			</div>
			<?php
		...
		<div class="num_addnew_box">
			<form name="form_NUM_Options" method="post" style="margin-top: 20px;">
				<div style="margin-left: 38px; margin-bottom: 10px; ">	
					<div style="margin-top: 10px;">
						<label for="num_option_adminonly" title="On/Off">
							<?php echo __('Enable this Page for Admin only', 'new-users-monitor'); ?>
						</label>
					...
					<div style="margin-top: 15px;">
						<label for="num_disable_login" title="On/Off">
							<?php echo __('Disable Login for Non-Confirmed User', 'new-users-monitor'); ?>
						</label>
						<input id="num_disable_login" name="num_disable_login" type="checkbox" <?php if($NUM_Disable_Login) {echo 'checked';} ?>>
					</div>						
				
					<div style="margin-top: 10px;">
						<label for="dashboard_newusers" title="How many New Users to display in the console widget"><?php echo __('Number of new users in the widget:', 'new-users-monitor'); ?> </label>
					...
					<div style="margin-top: 10px;">
						<label for="scan_newusers" title="New users can appear as a result of hacking or malicious intent. It would be good to know about this as quickly as possible!"><?php echo __('DB scanning period for finding new users (Hour):', 'new-users-monitor'); ?> </label>
						...
						<span style="color: grey;"><?php echo __('(notification will be sent to the Administrator automatically)', 'new-users-monitor'); ?></span>
					</div>				
				</div>
				...
				<div style="margin-top: 10px; margin-bottom: 5px; text-align: right;">
					...
					<input id="btn_options_save" type="submit" class="button button-primary" style="margin-right: 5px;" value="<?php echo __('Save', 'new-users-monitor'); ?>">
				</div>
	...
	class NUM_ListTable extends WP_List_Table {			
		...
		private function table_data() {
			...
			if ($Search_NUM) {
				...
				$targets = $wpdb -> get_results ($wpdb -> prepare ($Query, $like, $like, $like, $like), ARRAY_A); // phpcs:ignore		
			} else {
				$Query = "
					SELECT 
						users.ID, users.user_registered, users.user_nicename, users.display_name, users.user_email,
				...
				$targets = $wpdb -> get_results ($wpdb -> prepare ($Query, 1), ARRAY_A); // phpcs:ignore		
			}
