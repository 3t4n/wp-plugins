...
function dsnp_footer_text( $text ) {
    global $current_screen;
    if ( ! empty( $current_screen->id ) && strpos( $current_screen->id, 'dessky-snippets' ) !== false ) {
        return 'If you like <strong>Dessky Snippets</strong> please leave us a <a href="https://wordpress.org/support/plugin/dessky-snippets/reviews?rate=5#new-post" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a> rating to help us spread the word. A huge thanks in advance!';
    }
    
    return $text;
}
add_filter( 'admin_footer_text', 'dsnp_footer_text' );

/**
 * Plugin Text - meta links
 */

function dsnp_row_meta($input, $page) {
    ...
    return array_merge(
        $input,
        array(
            '<a href="https://dessky.com/services/" target="_blank">Premium support</a>',
        )
        );
}
add_filter( 'plugin_row_meta', 'dsnp_row_meta', 10, 2 );
