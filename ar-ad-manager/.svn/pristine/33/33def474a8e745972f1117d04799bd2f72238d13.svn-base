...
class Partials_Ar_Ad_Manager_Types
{
    ...
    public function ar_wp_ad_banners_columns($columns)
    {
        ...
        $dataColumns = [
            'wp_uid' => esc_html__('ID', 'ar-ad-manager'),
            'advertiser' => esc_html__('Advertiser', 'ar-ad-manager'),
            'weight' => esc_html__('Weight', 'ar-ad-manager'),
            'banner_is_active' => esc_html__('Status', 'ar-ad-manager'),
            'banner_linked_adzones' => esc_html__('Adzone', 'ar-ad-manager')
        ];

        $order = isset($_GET['order']) && $_GET['order'] === 'desc' ? 'asc' : 'desc';

        foreach ($dataColumns as $columnKey => $columnLabel) {
            $columns[$columnKey] = '<a href="' . admin_url( 'edit.php?post_type=ar-wp-ad-banners&orderby='. $columnKey .'&order=' . $order) .'">' .$columnLabel. '</a>';
        }
    ...
    public function ar_wp_ad_banners_show_columns($name)
    {
        ...
        {
            case 'advertiser':
                $advertiserId = esc_html(get_post_meta( $post->ID, Partials_Ar_Ad_Manager_Meta_Box_Abstract::AR_AD_MANAGER_FIELD_PREFIX . "_banner_advertiser", true));
                echo get_the_title($advertiserId);
    ...
    public function ar_wp_ad_adzones_columns($columns)
    {
        ...
        $dataColumns = [
            'wp_uid' => esc_html__('ID', 'ar-ad-manager'),
            'adzone_size' => esc_html__('Adzone Size', 'ar-ad-manager'),
            'hide_adzone_if_empty' => esc_html__('Hide adzone if empty', 'ar-ad-manager'),
            'code' => ''
        ];
        ...
        foreach ($dataColumns as $columnKey => $columnLabel) {
            $columns[$columnKey] = '<a href="' . admin_url( 'edit.php?post_type=ar-wp-ad-adzones&orderby='. $columnKey .'&order=' . $order) .'">' .$columnLabel. '</a>';
        }
                ...
                <div class="ar-ad-manager-shortcode-grid">
                    ...
                    <dialog class="mdl-dialog">
                        ...
                        <div class="mdl-dialog__content">
                            ...
                            <code>[ad_manager_display_adzone id="<?php echo esc_html($post->ID) ?>"]</code>
                            ...
                            <code>echo do_shortcode('[ad_manager_display_adzone id=<?php echo esc_html($post->ID) ?>]');</code>
                        </div>
    ...
    public function ar_wp_ad_advertisers_columns($columns)
    {
        ...
        $dataColumns = [
            'wp_uid' => esc_html__('ID', 'ar-ad-manager'),
            'is_advertiser_active' => esc_html__('Is Active', 'ar-ad-manager')
        ];

        $order = isset($_GET['order']) && $_GET['order'] === 'desc' ? 'asc' : 'desc';

        foreach ($dataColumns as $columnKey => $columnLabel) {
            $columns[$columnKey] = '<a href="' . admin_url( 'edit.php?post_type=ar-wp-ad-advertisers&orderby='. $columnKey .'&order=' . $order) .'">' .$columnLabel. '</a>';
        }
