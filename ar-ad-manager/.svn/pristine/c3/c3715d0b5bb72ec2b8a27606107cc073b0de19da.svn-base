...
abstract class Partials_Ar_Ad_Manager_Meta_Box_Abstract
{
    ...
    public function save($post_id)
    {
        ...
        if (isset($postValues['banner_is_active']) && !isset($postValues['banner_exclude_countries'])) {
            $postValues['banner_exclude_countries'] = [];
        }
        ...
        foreach ($postValues as $key => $value) {
            ...
            $includeHtmlKeys = ['desktop_banner_script', 'tablet_banner_script', 'mobile_banner_script'];

            if (!in_array($key, $includeHtmlKeys)) {
