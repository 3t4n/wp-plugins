(function ($) {
    ...
    $(window).load(function () {
                        ...
                        onChange: function (value) {
                            if (selector.classList.contains('banner-linked-adzones')) {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: ajaxurl,
                                    data: 'action=ar_ad_manager_grid_adzone_change&post_id=' + postId + '&value=' + value.join(',') + '&security=' + ar_ad_manager_variables.ajax_nonce
                                }).done(function (msg) {
            ...
            isActiveToggleGrid.forEach(function (toggleElBlock) {
                ...
                toggleInput.addEventListener('change', function (e) {
                    $.ajax({
                        type: 'POST',
                        url: ajaxurl,
                        data: 'action=ar_ad_manager_grid_status_toggle&post_id=' + postId + '&field=' + fieldId + '&is_active=' + e.target.checked + '&security=' + ar_ad_manager_variables.ajax_nonce
                    }).done(function (msg) {
        ...
        // Color input field functionality
        var colorInputs = document.querySelectorAll('.mdl-js-color-textfield');

        if (colorInputs && colorInputs.length) {
            colorInputs.forEach(pair => {
                const colorPicker = pair.querySelector('input[type=color]');
                const colorText = pair.querySelector('input[type=text]');
