...
if ( ! class_exists( 'DDS_WP_Term_Images' ) ) :
/**
 * Main WP Term Images class
 *
 * @since 0.1.0
 */
final class DDS_WP_Term_Images extends DDS_WP_Term_Meta_UI {
	...
	public function __construct( $file = '' ) {
		...
		// Setup the labels
		$this->labels = array(
			'singular'    => esc_html__( 'Image',  'degree-of-difficulty-for-sensei' ),
			'plural'      => esc_html__( 'Images', 'degree-of-difficulty-for-sensei' ),
			'description' => esc_html__( 'Assign terms a custom image to visually separate them from each-other.', 'degree-of-difficulty-for-sensei' )
		);
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'wp-term-images', 'i10n_WPTermImages', array(
			'insertMediaTitle' => esc_html__( 'Choose an Image', 'degree-of-difficulty-for-sensei' ),
			'insertIntoPost'   => esc_html__( 'Set as image',    'degree-of-difficulty-for-sensei' ),
	...
	public function help_tabs() {
		get_current_screen()->add_help_tab(array(
			...
			'title'   => __( 'Term Image', 'degree-of-difficulty-for-sensei' ),
			'content' => '<p>' . __( 'Terms can have unique images to help separate them from each other.', 'degree-of-difficulty-for-sensei' ) . '</p>',
		) );
	}
	...
	protected function form_field( $term = '' ) {
		...
		<a class="button-secondary wp-term-images-media">
			<?php esc_html_e( 'Choose Image', 'degree-of-difficulty-for-sensei' ); ?>
		</a>

		<a href="<?php echo esc_url( $remove_url ); ?>" class="button wp-term-images-remove"<?php echo $hidden; ?>>
			<?php esc_html_e( 'Remove', 'degree-of-difficulty-for-sensei' ); ?>
		</a>
