<?php
	...
	<div class="servicearea id-<?php echo esc_attr( $post_id ); ?>">
		...
		<div class="serviceboxs-col-lg-<?php echo esc_attr( $rsbbox_columns ); ?> serviceboxs-col-md-4 serviceboxs-col-sm-2 serviceboxs-col-xs-1">
			<div class="tpwp-style2-servicebox-items" <?php if( !empty( $rsbbox_back_color ) ) : ?> style="background-color:<?php echo $rsbbox_back_color;?> !important" <?php endif; ?>>
				<?php if( $rsbbox_hideicons == 1 ) : ?>
					<div class="tpwp-style2-service-icon" <?php if( !empty( $rsbbox_iconbg_color ) ) : ?> style="background-color:<?php echo $rsbbox_iconbg_color;?>" <?php endif; ?>>
						<i <?php if( !empty( $rsbbox_icon_color ) ) : ?> style="color:<?php echo $rsbbox_icon_color;?> !important" <?php endif; ?> class="fa <?php echo $rsbboxicons; ?>"></i>
					</div>
				<?php endif; ?>
				<div class="tpwp-style2-service-content">
					<?php if( $rsbbox_hidetitle == 1 ) : ?>
						<?php if( !empty( $shortdetails ) ) : ?>
							<h3 <?php if( !empty( $rsbbox_title_color ) ) : ?> style="color:<?php echo $rsbbox_title_color;?> !important" <?php endif; ?>>
								<?php if( $rsbbox_hidelinks == 1 ) : ?>
									<a <?php if( !empty( $rsbbox_title_color ) ) : ?> style="color:<?php echo $rsbbox_title_color;?> !important" <?php endif; ?> href="<?php echo esc_url( $rsbboxurl ); ?>"><?php the_title(); ?></a>
								<?php else : ?> 
									<?php the_title(); ?>
								<?php endif; ?>
							</h3>
					...
					<?php endif; ?>
					<?php if( !empty( $shortdetails ) ) : ?>
						<p <?php if( !empty( $rsbbox_content_color ) ) : ?> style="color:<?php echo $rsbbox_content_color;?> !important" <?php endif; ?>>
							<?php echo $shortdetails;?>
						</p>
					<?php endif; ?>
					<?php if( $rsbbox_hidereadmore == 1 ) : ?>
						<?php if( !empty( $rsbbox_button_text ) ) : ?>
							<a href="<?php echo esc_url($rsbboxurl);?>" class="tpwp-style2-readmore" <?php if( !empty( $rsbbox_moresize_color ) ) : ?> style="color:<?php echo $rsbbox_moresize_color;?> !important" <?php endif; ?>><?php _e( $rsbbox_button_text ); ?></a>
						<?php endif; ?>
					<?php endif; ?>
				</div>
			</div>
