...
/**
 * ShopSettingFilterBlock
 */
class ShopSettingFilterBlock {
	...
	public static function render() {
		...
		if ( 'custom' == $period ) {
			...
			echo '<div class="product-overwiev-periods-select">';
				...
				echo '<select id="cost-years" name="cost-years" class="cost-years 8" data-url="' . esc_url( admin_url() ) . 'admin.php?page=data-settings&subpage=shop-settings">';
	...
	public static function render_custom_periods( $periods ) {
		...
		if ( !empty( $array ) ) {
			ksort( $array );
			$html .= '<div class="product-overwiev-periods-data" id="data-item-date-' . esc_html( $item_dates['id'] ) . '">';
