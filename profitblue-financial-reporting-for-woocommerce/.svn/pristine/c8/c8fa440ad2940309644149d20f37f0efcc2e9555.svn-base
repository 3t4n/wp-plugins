...
/**
 * ProductsPeriodsFilterBlock
 */
class ProductsPeriodsFilterBlock {
	...
	public static function render_old() {
		...
		echo '<div class="product-overwiev-periods-inner">';
			...
			echo '<div class="product-overwiev-periods-select">';
				...
				echo '<select id="cost-years" name="cost-years" class="cost-years 5" data-url="' . esc_url( admin_url() ) . 'admin.php?page=data-settings&subpage=costs-of-goods-sold">';
			...
			foreach( $array as $key => $item ){
				...
				$html .= '<div class="data-item-dates">';
					foreach( $item as $item_dates ) {
						$html .= '<div class="data-item-date" id="data-item-date-' . esc_html( $item_dates['id'] ) . '">';
							$html .= '<a href="' . esc_url( admin_url() ) . 'admin.php?page=data-settings&subpage=costs-of-goods-sold&period=custom&date_start=' . esc_html( $item_dates['date-start'] ) . '&date_end=' . esc_html( $item_dates['date-end'] ) . '">' . esc_html( $item_dates['date-start'] ) . ' - ' . esc_html( $item_dates['date-end'] ) . '</a>';
