<?php
if ( ! defined( 'ABSPATH' ) ) {
    exit; // Exit if accessed directly.
}
$article_id      = get_post_meta($id, 'article_id', true);
$article         = addlly_get_article_by_id($id);
$article_data    = isset($article['data']) ? $article['data'] : array();
$articleContent  = isset($article_data->article_html) ? $article_data->article_html : '';
?>
<div class="blog-writer-content-block">
    <?php
    set_query_var('id', $id);
    set_query_var('active_tab', $active_tab);
    addlly_get_template_part('one-click-blog-writer/edit/top-navbar');
    addlly_get_template_part('one-click-blog-writer/edit/metadata');
    addlly_get_template_part('one-click-blog-writer/edit/version-history');
    ?>
    <div class="content-area-block d-flex reviewArticle">
        <div class="content-area position-relative">
            <?php
            $reviewArticleContent = get_post_meta($id, 'reviewArticleContent', true);
            $articleText = isset($reviewArticleContent['articleText']) ? $reviewArticleContent['articleText'] : '';
            //if( $articleText == '' ){
                $api_response = addlly_get_review_article($article_id, 'Short Article', '1-Click Blog', $articleContent);
                //update_post_meta($id, 'reviewArticleContent', $api_response);
                $articleText = isset($api_response['articleText']) ? $api_response['articleText'] : '';
            //}
            echo '<div class="custom-text-editor">
                <textarea id="textEditor">' . wp_kses_post($articleText) . '</textarea>
            </div>';
            echo '<div class="d-none">
                <textarea name="selectedText"></textarea>
            </div>';
            echo '<div class="custom-tooltip" style="left: 328.606px;" data-id="'. esc_attr($id) .'" data-type="reviewArticle">
                <div class="arrow-content"></div>
                <span class="d-flex align-items-center gap-2 cursor-pointer text-nowrap">
                    <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                        <path d="M123.6 391.3c12.9-9.4 29.6-11.8 44.6-6.4c26.5 9.6 56.2 15.1 87.8 15.1c124.7 0 208-80.5 208-160s-83.3-160-208-160S48 160.5 48 240c0 32 12.4 62.8 35.7 89.2c8.6 9.7 12.8 22.5 11.8 35.5c-1.4 18.1-5.7 34.7-11.3 49.4c17-7.9 31.1-16.7 39.4-22.7zM21.2 431.9c1.8-2.7 3.5-5.4 5.1-8.1c10-16.6 19.5-38.4 21.4-62.9C17.7 326.8 0 285.1 0 240C0 125.1 114.6 32 256 32s256 93.1 256 208s-114.6 208-256 208c-37.1 0-72.3-6.4-104.1-17.9c-11.9 8.7-31.3 20.6-54.3 30.6c-15.1 6.6-32.3 12.6-50.1 16.1c-.8 .2-1.6 .3-2.4 .5c-4.4 .8-8.7 1.5-13.2 1.9c-.2 0-.5 .1-.7 .1c-5.1 .5-10.2 .8-15.3 .8c-6.5 0-12.3-3.9-14.8-9.9c-2.5-6-1.1-12.8 3.4-17.4c4.1-4.2 7.8-8.7 11.3-13.5c1.7-2.3 3.3-4.6 4.8-6.9c.1-.2 .2-.3 .3-.5z"></path>
                    </svg>
                    Add Comment
                </span>
            </div>';
            ?>
        </div>
        <?php
        set_query_var('id', $id);
        addlly_get_template_part('one-click-blog-writer/edit/right-sidebar/reviewArticle'); 
        ?>
    </div>
</div>