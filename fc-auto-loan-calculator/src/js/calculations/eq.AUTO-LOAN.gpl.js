...
export class AutoLoanCalculation {
	...
	static insertSubTotals (schedule) {
		...
		do {
			...
			if (((schedule[i - 1][YEAR] !== schedule[i][YEAR] && (schedule[i - 1][MONTH] <= DECM || schedule[i][MONTH] > DECM))) || ((schedule[i - 1][YEAR] === schedule[i][YEAR] && (schedule[i - 1][MONTH] <= DECM && DECM < schedule[i][MONTH])))) {
				...
				// ' YTD:'
				totals[PER_STR] = schedule[i - 1][YEAR] + ' ' + autoStrings.strs.s101 + ':';
				...
				// 'Running Totals:'
				totals[PER_STR] = autoStrings.strs.s102 + ':';
				totals[DATE_STR] = null;
				totals[CF] = runningCF;
		...
		if (schedule[schedule.length - 1][ROW_TYPE] !== Globals.ROW_TYPES.RUNNING_TOTALS) {
			...
			totals[ROW_TYPE] = Globals.ROW_TYPES.ANNUAL_TOTALS; // YTD total row marker
			// ' YTD:'
			totals[PER_STR] = schedule[schedule.length - 1][YEAR] + ' ' + autoStrings.strs.s101 + ':';
			totals[DATE_STR] = null;
			totals[CF] = ytdCF;
			totals[CREDIT] = ytdCredit;
			totals[DEBIT] = ytdDebit;
	...
	static initLoanScheduleData (obj, periodicRate) {
		...
		do {
			...
			if (Math.abs(balance) > MAX_BALANCE) {
				...
				Utils.showMessageModal('<p>' + autoStrings.strs.s100 + '</p>');
				schedule = [];
				return schedule;
			}
			L += 1;
	...
	static calc (obj) {
		...
		this.sourceScheduleData = this.initLoanScheduleData(obj, periodicRate);
		return cf;
	}
}
...
// LoanCalculation.auto_loan_params = {
// 	nominalRate: null,
// 	n: null,
// 	cf: null,
// 	pv: null,
