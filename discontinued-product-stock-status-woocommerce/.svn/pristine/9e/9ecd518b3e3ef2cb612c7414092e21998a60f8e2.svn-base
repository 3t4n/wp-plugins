		...
		if ( 'yes' === $greyscale_effect_slider_enable ) {
			...
			if ( $product->is_type( 'simple' ) ) {
				...
				if ( 'yes' === $manage_stock ) {
					...
					$disc_parent    = get_post_meta( $product_id, '_stock_discontinued_product', true );
...
function dpssw_grouped_discontinued_products_message_( $value, $grouped_product_child ) {
	...
	$variable_discontinued_status = '';

	if ( $grouped_product_child->is_type( 'variable' ) ) {
		$variable_discontinued_status = get_post_meta( $post->ID, '_discontinued_product', true );
	}
	if ( 'discontinued' === $grouped_product_child->get_stock_status() || 'yes' === $variable_discontinued_status ) {
		...
		// this is present in settings of discontinued products.
		$custom_message_disable = get_option( 'discontinued_enable_custom_message' );

		$global_message = ! empty( get_option( 'discontinued_global_message' ) ) ? get_option( 'discontinued_global_message' ) : __( 'This product has been discontinued.', 'discontinued-products-stock-status' );
		...
		if ( 'yes' === $custom_message_disable ) {
			if ( 'product_specific_message' === $specific_messsage_dropdown ) {
		...
		} else { // This will run when custom_message_disable is 'yes'. Global message will overide the custom message.
...
function dpssw_to_show_discontinued_product_message() {
	...
	if ( $product->is_type( 'simple' ) ) {
		...
		if ( 'yes' === $manage_stock ) {
			...
			$disc_parent    = get_post_meta( $product->get_id(), '_stock_discontinued_product', true );
			if ( ( 'yes' === $disc_parent ) && ( '0' === $stock_quantity ) ) {
				$is_checked = 'yes';
			}
		}
	...
	if ( 'discontinued' === $stock_status ) {
		...
		// for custom message type discontinued product.
		if ( 'yes' === $custom_message_disable ) {
			if ( 'product_specific_message' === $specific_messsage_dropdown ) {
		...
		} else { // for overriding the custom message with global message when custom_message_disable is 'yes'.
		...
		if ( ! empty( $global_message_html ) ) {
			echo wp_kses_post( $global_message_html );
		}

		do_action( 'dpssw_pro_messages_div' ); // add a addditional messsage div in the simple product page.
	}
}
...
function dpssw_search_hide_discontinued_post( $query ) {
	...
	$hide_from_search = get_option( 'discontinued_show_in_catalog' );
...
function dpssw_plugin_notice() {
	...
	if ( ! get_user_meta( $user_id, 'dpssw_notices_time' ) ) {
		$after_10_day = strtotime( '+10 day', $today_date );
		update_user_meta( $user_id, 'dpssw_notices_time', $after_10_day );
	}

	// gets the option of user rating status and week status.
	$rate_status = get_user_meta( $user_id, 'dpssw_rate_notices', true );
	$next_w_date = get_user_meta( $user_id, 'dpssw_notices_time', true );
	...
	if ( 'rated' !== $rate_status && $today_date > $next_w_date ) {
		...
		<div class="notice notice-warning is-dismissible">
			...
			<span>
				<a href="https://wordpress.org/plugins/discontinued-product-stock-status-woocommerce/#reviews" target="_blank"><?php esc_html_e( "Yes, I'd like to rate it!", 'discontinued-products-stock-status' ); ?></a>
			</span>&nbsp; - &nbsp;
			<span>
				<a class="dpssw_hide_rate" href="#"><?php esc_html_e( 'I already did!', 'discontinued-products-stock-status' ); ?></a>
			</span>
			<br/><br/>
		</div>
		<?php
	...
	<script>
		let dpsswAjaxURL = "<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>";
		let dpsswNonce = "<?php echo esc_attr( wp_create_nonce( 'discontinued-products-stock-status' ) ); ?>";
...
add_action( 'woocommerce_subcategory_count_html', 'custom_subcategory_count_html', 10, 2 );
/**
 * Overriding the Count of Products on the Category list.
 *
 * @param string $html HTML of text rendering after the Category Title on the Products Category Tile.
 * @param string $category Product Category.
 */
function custom_subcategory_count_html( $html, $category ) {

	// Extract the current product count from the HTML.
	preg_match( '/\b\d+\b/', $html, $matches );

	if ( ! empty( $matches ) ) {
		// Get the current product count.
		$current_count = $matches[0];

		$show_in_catalog = get_option( 'discontinued_show_in_catalog' );

		if ( 'yes' === $show_in_catalog ) {
			$product_count = get_product_ids_in_category( $category->slug );
			$html          = str_replace( $current_count, $product_count, $html );
		}
	}
...
/**
 * Opens a div tag for initially removing the Product Category all Products Discontinued in it.
 */
function custom_before_subcategory_title( $category ) {
	...
	if ( 'yes' === $show_in_catalog ) {
		...
		if ( 0 == $product_count ) {
			...
			$excluded_categories[] = $category->slug;
			$excluded_categories   = array_unique( $excluded_categories );
			update_option( 'excluded_categories', $excluded_categories );

			// Add a class to the category tile for hiding via CSS.
			echo '<div class="hidden-category-tile">';
		} else {
			$excluded_categories = get_option( 'excluded_categories', array() );

			// Check if the element is present in the array.
			if ( in_array( $category->slug, $excluded_categories ) ) {
				// If present, find the key of the element.
				$key = array_search( $category->slug, $excluded_categories );

				// Remove the element from the array.
				unset( $excluded_categories[ $key ] );
			}
...
add_action( 'woocommerce_after_subcategory_title', 'custom_after_subcategory_title', 10, 1 );
/**
 * Closes the div tag for initially removing the Product Category with all Products Discontinued in it.
 */
function custom_after_subcategory_title( $category ) {
	$show_in_catalog = get_option( 'discontinued_show_in_catalog' );
...
if ( ! is_admin() ) {
	add_filter( 'get_terms', 'hide_prod_category', 10, 3 );
}
/**
 * Used to unset the Product Category to hide the Product Category with all Products Discontinued in it.
 */
function hide_prod_category( $terms, $taxonomies, $args ) {
	$show_in_catalog = get_option( 'discontinued_show_in_catalog' );

	if ( 'yes' === $show_in_catalog && in_array( 'product_cat', $taxonomies ) ) {
		$excluded_categories = get_option( 'excluded_categories', array() );

		foreach ( $terms as $key => $term ) {
			if ( is_object( $term ) && isset( $term->slug ) && in_array( $term->slug, $excluded_categories ) ) {
				unset( $terms[ $key ] );
			}
		}
	}
...
/**
 * Function to override the Count Value of Products of Category.
 *
 * @param string $category_slug Slug of Category.
 */
function get_product_ids_in_category( $category_slug ) {
	...
	foreach ( $products as $product ) {
		...
		if ( $product && ! is_bool( $product ) ) {
			...
			// for simple product hide discontinued price and do gray scale.
			if ( $product->is_type( 'simple' ) && 'discontinued' === $stock_status ) {
				$attr['class']            .= ' sft-discontinued';
				$cat_prod_without_dispro[] = $product_id;
	...
	return count( $products ) - count( $cat_prod_without_dispro );
}
