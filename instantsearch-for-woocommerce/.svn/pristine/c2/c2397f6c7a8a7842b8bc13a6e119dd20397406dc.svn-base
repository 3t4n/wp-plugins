<?php
/**
 * Fast Simon for WooCommerce.
 *
 * @package   WCISPlugin
 * @author    Fast Simon
 * @license   GPL-2.0+
 * @link      http://www.fastsimon.com
 * @copyright 2023 Fast Simon
 */
...
/**
 * @package WCISPlugin
 * @author  Fast Simon <support@fastsimon.com>
 */
class WCISPlugin
{
    const SERVER_URL = 'https://woo.instantsearchplus.com/';
    const NEW_SERVER_URL = 'https://sync.fastsimon.com/';
    const PING_SERVER_URL = 'https://ping.fastsimon.com/';
    const DASHBOARD_URL = 'https://dashboard.fastsimon.com/';
     ...
     * @var      string
     */
    public $plugin_slug = 'instantsearch-for-woocommerce';
...
//         add_action( 'publish_post', array( $this, 'on_product_update' ));
        ...
        */
        ...
        add_action('woocommerce_admin_css', array($this, 'enqueue_styles'));
        ...
        add_action('admin_post_wcis_save_email', array($this, 'save_email'));

        //open pop-ups to the admin user
        add_action('admin_footer', array($this, 'open_admin_popups'));
    }
    ...
    public function wcis_rewrite_cat_template($located)
    {
        ...
        if (strpos($located, 'archive-product.php') !== false &&
            ...
            //if the category template contains the category shortcode,
            // do not override it
            $file_content = file_get_contents($located);
            $shortcode_str = WCISCategory::$CATEGORY_SHORTCODE;
            if (!empty($file_content) && strpos($file_content, $shortcode_str) != false) {
                return $located;
            }
    ...
    {
        $theme = wp_get_theme();
        $themes_list = ['Best Shop', 'SmartShop', 'ProShop'];
        // ProShop is a WooCommerce theme, parent theme - Storefront

        $is_theme_in_list = (in_array($theme->name, $themes_list) ||
            in_array($theme->parent_theme, $themes_list));

        $is_pro_shop = $theme->name == 'ProShop' || $theme->parent_theme == 'ProShop';
        $category_template = $is_pro_shop ?
            'taxonomy-product-cat.php' : 'woocommerce.php';

        if ($is_theme_in_list &&
            strpos($template, $category_template) !== false &&
            WCISCategory::is_category_page()) {
    ...
    private function wcis_serp_activation()
    {
        ...
        $wcis_serp_page = array(
            ...
            'post_title' => __('Search Results', 'instantsearch-for-woocommerce'), //[ <string> ] // The title of your post.
...
//             'post_excerpt'   => __('Search Results','instantsearch-for-woocommerce'), //[ <string> ] // For all your post excerpt needs.
    ...
    public function schedule_ids_update()
    {
        ...
        if (array_key_exists('instantsearchplus_parameter', $_POST) &&
            ...
            if (array_key_exists('instantsearchplus_second_parameter', $_POST)) {
                $json_data = sanitize_text_field($_POST['instantsearchplus_second_parameter']);
                $decoded_data = json_decode($json_data, true);
                if (is_array($decoded_data)) {
                    $ids = array_unique(array_map('absint', $decoded_data));
                }
            }

            if (!is_array($decoded_data) || count($ids) == 0) {
                $response_js['success'] = false;
                $response_js['msg'] = 'No items to update';
                echo wp_json_encode($response_js);
                exit();
            }
    ...
    public static function wcis_add_action_links($links)
    {
        ...
        return array_merge(
            array(
                'Settings' => '<a href="' . $url . '" target="_blank">' .
                    __('Settings', 'instantsearch-for-woocommerce') . '</a>'
            ),
            $links
        );
    ...
    private function single_deactivate($network_wide)
    {
        ...
        $url = self::get_isp_server_url(true) . 'wc_update_site_state';
    ...
    private static function single_uninstall($network_wide)
    {
        ...
        $url = WCISPlugin::get_instance()->get_isp_server_url(true) . 'wc_update_site_state';
    ...
    private function single_activate($is_retry = false)
    {
        ...
        if (get_option('wcis_user_email') || get_option('authentication_key')) {

            if (function_exists('single_activation_approved')) {
                single_activation_approved($is_retry);
            }
        }
    ...
    public function single_activation_approved($is_retry = false)
    {
        ...
        $url = self::get_isp_server_url(true) . 'wc_install';
        $stores_array = array();
        ...
        $body = array(
            ...
            'store_id' => get_current_blog_id(),
            'email' => get_option('wcis_user_email'),
    ...
    public function save_email()
    {
        ...
        ?>
        <script>
            document.getElementById("fast-simon-admin-email").style.display='none';
        </script>
        <?php

        if (isset($_POST['is-cancel-user-email']) && $_POST['is-cancel-user-email']) {

            $plugin = 'instantsearch-for-woocommerce/instantsearch-for-woocommerce.php';
        ...
        if (isset($_POST['wcis_user_email']) ) {
            // Sanitize and save the email address to the options table
            $email = sanitize_email($_POST['wcis_user_email']);

            update_option('wcis_user_email', $email);

            $this->single_activation_approved(true);

            // Optionally, add a success notice after saving the email
            add_action('admin_notices', function () {
                ?>
                <div class="notice notice-success is-dismissible">
                    <p>Thank you! Your email has been saved successfully.</p>
                </div>
                <?php
            });
        ...
        // Redirect back to the admin dashboard to prevent form resubmission
    ...
    {
        $url = self::get_isp_server_url(true) . 'wc_additional_fetch';
    ...
    private function get_product_from_post($post_id)
    {
        ...
        } elseif ($product->is_type('grouped') && count($product->get_children()) > 0) {
            $child_prices = array();
...
//         $category = get_term_by( 'id', $category_id, 'product_cat', 'ARRAY_A' );
    ...
    public function on_variation_ajax_delete()
    {
        if (current_user_can('edit_products')) {
            $variation_ids = array();
            if (array_key_exists('variation_ids', $_POST)) {
                // Ensure it is an array and sanitize each ID
                $variation_ids = array_unique(array_map('absint', (array)$_POST['variation_ids']));
            }
    ...
    private function send_products_batch($products, $is_products_install_batch = true, $is_retry = false, $send_compressed = false)
    {
        ...
        $url = self::get_isp_server_url(true) . 'wc_install_products';
    ...
    function send_categories_as_batch($categorys_list = null)
    {
        ...
        if ($categorys_list != null) {
            ...
            $cats_number = wp_count_terms($args);
            if ($cats_number > 2000) {
        ...
        if ($category_array) {       // if there are categories to send
            $url = self::get_isp_server_url(true) . 'wc_update_categories';
    ...
    private function send_product_update($post_id, $action)
    {
        ...
        $url = self::get_isp_server_url(true) . 'wc_update_products';
    ...
    {
        $locale = apply_filters('plugin_locale', get_locale(), 'instantsearch-for-woocommerce');

        load_textdomain('instantsearch-for-woocommerce', plugin_dir_path(dirname(__FILE__)) . 'languages/' . 'instantsearch-for-woocommerce' . '-' . $locale . '.mo');
        load_plugin_textdomain('instantsearch-for-woocommerce', "", basename(plugin_dir_path(dirname(__FILE__))) . '/languages/');
    }
    ...
    {
        wp_enqueue_style(
            'wcis-plugin-styles',  // Handle
            plugins_url('assets/css/wcis_plugin_admin.css', __FILE__),  // Path to CSS
            array(),  // Dependencies
    ...
    function process_instantsearchplus_request($req)
    {
        if (array_key_exists('instantsearchplus', $req->query_vars)) {
            if ($req->query_vars['instantsearchplus'] == 'version') {
                ...
                $is_prettify = self::get_query_param($req, 1) == '1';
                ...
                if ($options && array_key_exists('serp_page_id', $options) && array_key_exists('is_serp_enabled', $options) && $options['is_serp_enabled']) {
                    ...
                    $serp_path = esc_url_raw(str_replace(home_url(), "", get_permalink($options['serp_page_id'])));
                }
                ...
                if ($is_prettify) {
                    $response_pretty =
                        '<style>.version-data-name{width: 238px;display:inline-block;}</style>';

                    $response_pretty .= '<p style="margin-top:40px;margin-left:50px;">';

                    foreach ($response as $key => $value) {
                        $value_typed = $value;
                        if (gettype($value) == 'boolean') {
                            $value_typed = $value ? 'true' : 'false';
                        }
                        $response_pretty .= '<span class="version-data-name">' . $key .
                            '</span>' . $value_typed . '</span><br>';
                    }

                    $response_pretty .= '</p>';

                    exit($response_pretty);
                } else {
                ...
                echo wp_json_encode(get_option('cron_update_product_list_by_date'));
                exit();
                ...
                if (array_key_exists('instantsearchplus_parameter', $req->query_vars) &&
                    ...
                    if (!$options) { //NO SERP vars in wp_options
                        $serp_page_title = __('Search Results', 'instantsearch-for-woocommerce');
                        $serp_page = null;

                        $query = new WP_Query([
                            'post_type' => 'page',
                            'title' => $serp_page_title,
                            'post_status' => 'publish',
                            'posts_per_page' => 1,
                        ]);

                        if ($query->have_posts()) {
                            $serp_page = $query->posts[0];
                        }
    ...
    public static function get_query_param($req, $i)
    {
        switch ($i) {
            case 0:
                return $req->query_vars['instantsearchplus'];
            case 1:
                return $req->query_vars['instantsearchplus_parameter'];
            case 2:
                return $req->query_vars['instantsearchplus_second_parameter'];
            case 3:
                return $req->query_vars['instantsearchplus_third_parameter'];
            default:
                return '';
        }
    }
    ...
    {
        //turn on manually for a specific site
        $is_send_error_report = false;
        if (!$is_send_error_report) {
            return;
        }

        $url = self::get_isp_server_url(true) . 'wc_error_log';
    ...
    function pre_get_posts_handler($wp_query)
    {
        ...
        if (isset($wp_query)) {
            ...
            if (!empty($query) && array_key_exists('post_type', $query)) {
                $post_type = $query['post_type'];
            }
        }

        /* get Fast Simon results inside WC template - cancelled */
        if (isset($wp_query) && !is_admin() && $wp_query->is_search() &&
            $wp_query->is_main_query() && $post_type == 'product') {
...
// 			$wp_query->query_vars['post_type'] = 'product';
    ...
    function show_admin_message()
    {
        if (is_admin()) {
            ...
            if (get_option('wcis_just_created_alert')) {
                ...
                printf(
                    wp_kses(
                        __('<b>Fast Simon for WooCommerce is installed :-)  </b><u><a href="%1$s" target="_blank"> Choose your settings </a></u>', 'instantsearch-for-woocommerce'),
                        array( // Allow only specific HTML tags and attributes
                            'b' => array(),
                            'u' => array(),
                            'a' => array(
                                'href' => array(),
                    ...
                    esc_url_raw(add_query_arg('instantsearchplus', 'remove_just_created_alert'))
                );
            ...
            if (get_option('wcis_site_alert')) {
                ...
                printf(
                    wp_kses(
                        __('<b>%1$s</b> | <b><a href="%2$s" target="_blank">Upgrade now</a></b> to enable back', 'instantsearch-for-woocommerce'),
                        array( // Allow only specific HTML tags and attributes
                            'b' => array(),
                            'a' => array(
                                'href' => array(),
                    ...
                    esc_html($msg), // Ensure the message is safe for output
                    esc_url_raw($dashboard_url)
                );
                ...
                printf(
                    wp_kses(
                        __('<b>Fast Simon for WooCommerce</b>: %1$s', 'instantsearch-for-woocommerce'),
                        array(
    ...
    {
        $url = self::get_isp_server_url(true) . 'ext_info';
        $args = array(
            'body' => array(
                'site_id' => get_option('wcis_site_id'),
                'version' => self::VERSION,
    ...
    public function add_plugin_admin_menu()
    {
        $this->plugin_screen_hook_suffix = add_menu_page(
            __('WooCommerce Fast Simon', 'instantsearch-for-woocommerce'),
            __('Fast Simon', 'instantsearch-for-woocommerce'),
            'manage_options',
            $this->plugin_slug,
            '',
    ...
    /* Set the link of our main menu item to open the dashboard in a new tab */
    public function add_plugin_admin_head()
    {
        $dashboard_url = esc_url_raw(self::get_dashboard_url());
        ?>
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                jQuery('ul li#toplevel_page_instantsearch-for-woocommerce a').attr('target', '_blank');
                jQuery('ul li#toplevel_page_instantsearch-for-woocommerce a').attr("href", "<?php echo($dashboard_url); ?>")
            });
        </script>
        <?php
    }
...
// 		}
    ...
    /**
     * Embed category results inside a template
     * @return string
     */
    public function wcis_category_shortcode()
    {
        if (WCISServing::is_svelte_category()) {
            return WCISServing::get_svelte_serving();
        } else {
            return "<p>Using the shortcode 'fast_simon_category' is possible 
                    only when the Smart Navigation Version is VNext,
                    to change the version please visit the Fast Simon 
                    Dashboard -> Collections tab -> Configuration.</p>";
        }
    }
    ...
    public function wcis_jquery_serp_results()
    {
        ...
        //$url = self::PING_SERVER_URL . 'wc_load_search_page';
        //also convert to get request for AKAMAY and get AKAMAY address
    ...
    public function wcis_cart_order_request($event, $order_id)
    {
        ...
        try {
            ...
            //$url = self::PING_SERVER_URL . 'wc_webhook';
            //check if turning to GET is needed
    ...
    private function get_isp_server_url($is_new = False)
    {
        $url = $is_new ? self::NEW_SERVER_URL : self::SERVER_URL;
    ...
    public static function get_products_count()
    {
        ...
        $sql_query = "SELECT COUNT(*) AS in_stock_products " .
            "FROM {$wpdb->posts} p " .
            "WHERE p.post_type = 'product' AND p.post_status = 'publish' " .
            "AND p.id NOT IN (SELECT object_id FROM {$wpdb->term_relationships} WHERE term_taxonomy_id = (" .
            "SELECT term_id FROM {$wpdb->terms} t WHERE t.slug = 'exclude-from-search'))";
    ...
    public function open_admin_popups()
    {


        // request admin email upon activation
        if (!get_option('wcis_user_email') && !get_option('authentication_key')) {

            ?>

            <div id="fast-simon-admin-email" >

                <div class="fast-simon-popup-back" >
                </div>


                <div class="fast-simon-popup-content" >

                    <p class="fast-simon-title" >
                        <strong>Complete Your Plugin Setup with Fast Simon Notifications.</strong>
                    </p>
                    <p class="fast-simon-describtion" >
                        To finish setting up Fast Simon for WooCommerce and receive essential updates, please enter your business email address.
                    </p>
                    <form class="fast-simon-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                        <input type="hidden" name="action" value="wcis_save_email">
                        <input type="hidden" name="is-cancel-user-email" id="is-cancel-user-email" value="0">
                        <input class="fast-simon-email-input" type="email" name="wcis_user_email" placeholder="Business Email Address">
                        <div id="fast-simon-admin-email-notification">Email is requiered!</div>
                        <p class="fast-simon-agreement" >

                            *By providing your email, you agree that the email will be used in order to finish the setup. [<a href="https://www.fastsimon.com/terms/">Learn more about our privacy policy.</a>]
                        </p>

                        <div class="fast-simon-popup-buttons">
                            <button class="email-cancel-button button button-Secondary"
                                    onclick="onEmailCancelButtonClicked()">
                                Cancel
                            </button>
                            <input type="submit" class="button button-primary fast-simon-submit-email"
                                   value="Opt-in and Continue" >
                        </div>
                    </form>
                </div>
            ...
            <script>
                function onEmailCancelButtonClicked() {
                    document.getElementById("is-cancel-user-email").value = '1';
                    document.getElementById("fast-simon-submit-email").click();
                }
            </script>
