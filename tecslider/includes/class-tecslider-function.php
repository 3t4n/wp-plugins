...
class Tecslider_Function {
    ...
    public function tecslider_generate_tec_events( $args ) {
        ...
        $colormode = ( true === $darkmode ? 'mwt-darkmode' : 'mwt-lightmode' );
        $slider_height = $this->tecslider_generate_height_with_defautls( $args, 'sh' );
        $image_height = $this->tecslider_generate_height_with_defautls( $args, 'ih' );
        $ticket_info = ( isset( $args['ticketInfo'] ) ? (bool) $args['ticketInfo'] : false );
        $display_time = ( isset( $args['displayTime'] ) ? (bool) $args['displayTime'] : false );
        ...
        if ( $events ) {
            ...
            foreach ( (array) $events as $array_id => $event ) {
                ...
                $time = tribe_get_start_date( $event, true, 'H:i' );
                ...
                if ( 'datetop' === $theme ) {
                    ...
                    if ( $display_time ) {
                        ?>
											<div class="tec-sse-date-timeholder">
												<?php 
                        echo esc_html( $time );
                        ?>
											</div>
                    ...
                    ?>
							...
							<?php 
                } elseif ( 'datefocus' === $theme ) {
                    ?>
								<div class="date-focus-box">
									<div class="tec-df-org-row">
										<?php 
                    $this->tecslider_generate_location( $event, $enable_location_icon );
                    ?>
										<?php 
                    ...
                    ?>
									...
									<div class="tec-df-date-row">
										<span class="tec-df-date">
											<?php 
                    echo esc_html( $month );
                    ?>
										</span>
										<span class="tec-df-timemonth">
											<span class="tec-df-month">
												<?php 
                    echo esc_html( $day );
                    ?>
											</span>
											<span class="tec-df-time">
												<?php 
                    echo esc_html( $time );
                    ?>
											</span>
									...
									<div class="tec-df-title-row  tec-sse-details">
										<div class="tec-slider-title">
											<?php 
                    echo esc_html( get_the_title( $event ) );
                    ?>
                    ...
                    ?>
							<?php 
                } elseif ( 'cardlayout' === $theme ) {
                    ?>
								<div class="cardlayout-design tec-sse-details">
									<div class="header-section">
										<div class="date-text"><?php 
                    echo esc_html( $month );
                    ?></div>
										<div class="month-text"><?php 
                    echo esc_html( $day );
                    ?></div>
									</div>
                    ...
                    ?>
							...
							<?php 
                } elseif ( 'splitlayout' === $theme ) {
                    ?>
								<div class="tec-top-section tec-sse-details">
									<div class="tec-date-text"><?php 
                    echo esc_html( $day );
                    ?></div>
									<div class="tec-month-text"><?php 
                    echo esc_html( $month );
                    ?></div>
									<div class="tec-title-text"><?php 
                    echo esc_html( get_the_title( $event ) );
                    ?></div>
								</div>

								<div class="tec-bottom-section image-middle-bottom">
									<div class="tec-detail-item">
										<?php 
                    echo esc_html( $time );
                    ?>
									</div>
            ...
            if ( 'datefocus' === $theme ) {
                ?>
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container .tec-slider-datefocus{
							background: <?php 
                echo esc_html( $color );
                ?>;
						}
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.datefocus.mwt-darkmode  .slick-prev::before,
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.datefocus.mwt-darkmode  .slick-next::before{
								color: <?php 
                echo esc_html( $color );
                ?>;
						}
            ...
            ?>
					<?php 
            if ( 'hoverdetail' === $theme || 'datefocus' === $theme ) {
                ?>
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container .slick-prev::before,
            ...
            ?>
					<?php 
            if ( 'splitlayout' === $theme ) {
                ?>
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.splitlayout .tec-top-section{
							background: <?php 
                echo esc_html( $color );
                ?>;
						}
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.splitlayout .slick-prev::before,
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.splitlayout .slick-next::before{
							color: #FFF;
						}
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.splitlayout.mwt-darkmode  .slick-prev::before,
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.splitlayout.mwt-darkmode  .slick-next::before{
								color: <?php 
                echo esc_html( $color );
                ?>;
						}
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.splitlayout.mwt-darkmode .tec-top-section{
            ...
            if ( 'datefocus' === $theme ) {
                ...
                ?>;
						...
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.datefocus.mwt-darkmode .tec-top-section{
							background: rgba( 0,0,0,0.8);
						}
					<?php 
            }
            ...
            if ( 'cardlayout' === $theme ) {
                ?>
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container .tec-slider-cardlayout{
							background: <?php 
                echo esc_html( $color );
                ?>;
							color: #FFF;
						...
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.cardlayout  .slick-prev::before,
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.cardlayout  .slick-next::before,
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.cardlayout .bx.tecslider-fe-icons{
								color: #FFF;
						}
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.cardlayout.mwt-darkmode  .slick-prev::before,
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.cardlayout.mwt-darkmode  .slick-next::before{
								color: <?php 
                echo esc_html( $color );
                ?>;
						}
						<?php 
                echo esc_attr( $slider_id_cid );
                ?>.tec-slider-container.cardlayout.mwt-darkmode  .tec-slider-cardlayout{
								color: #FFF;
								background: rgba( 0,0,0,0.8);
						}
					<?php 
    ...
    /**
     * Generate height along with defaults
     *
     * @param array  $args Arguments.
     * @param string $type Slide / Image Height.
     */
    public function tecslider_generate_height_with_defautls( $args, $type = 'sh' ) {
        $height = null;
        if ( 'sh' === $type ) {
            $height = ( isset( $args['slideheight'] ) ? $args['slideheight'] : null );
            if ( null === $height && 'singleevent' === $args['theme'] ) {
                $height = 260;
            }
        } else {
            $height = ( isset( $args['imageheight'] ) ? $args['imageheight'] : null );
        }
        return $height;
    }
