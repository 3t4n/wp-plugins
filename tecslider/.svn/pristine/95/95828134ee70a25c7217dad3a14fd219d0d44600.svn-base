...
class Tecslider_Shortcodes {
	...
	public function tecslider_block_shortcode( $attributes = array() ) {
		...
		$countdown_timer   = isset( $attributes['countdowntimer'] ) && 'true' === strtolower( $attributes['countdowntimer'] ) ? true : false;
		$dots              = isset( $attributes['dots'] ) && 'true' === strtolower( $attributes['dots'] ) ? true : false;
		$darkmode          = isset( $attributes['darkmode'] ) && 'true' === strtolower( $attributes['darkmode'] ) ? true : false;
		$category          = isset( $attributes['category'] ) ? $attributes['category'] : '';
		$slideheight       = isset( $attributes['slideHeight'] ) ? $attributes['slideHeight'] : null;
		$imageheight       = isset( $attributes['imageHeight'] ) ? $attributes['imageHeight'] : null;
		$ticket_info       = isset( $attributes['ticketInfo'] ) ? $attributes['ticketInfo'] : false;
		$display_time      = isset( $attributes['displayTime'] ) ? $attributes['displayTime'] : false;
		$defaults          = array(
			'passer'           => 'shortcode',
		...
		return $tecslider->function->tecslider_generate_tec_events( $defaults );
	}
}
