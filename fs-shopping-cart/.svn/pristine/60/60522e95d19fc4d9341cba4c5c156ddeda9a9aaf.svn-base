...
foreach ($products as $products) {
	...
	if ($ProductToCountry == TRUE) {
		...
		if ($fscartconfig['DisplayCategoryPageProductBuyButton'] == "1") {
			...
			if ($products->products_discontinued != 1) {
				...
				if ($ShowPrice == TRUE) { 
					...
					//}
					$page_content .= '<div class="sprice"'.$ProductPriceStyle.'>'.$products->products_price_label.' '.$_SESSION['currency_symbol'].fssc_currency_format($ProductPrice).'</div>';
					if ($InstantRebate != '0.00') {
						$page_content .= '<div id="fs-product-rebatel">'.$_SESSION['currency_symbol'].$InstantRebate.'<br />Instant Rebate</div><br />';
					}
				} else {
				...
				if ($ShowBuyButton == TRUE) { 
					if ($products->products_electronic_download != '' && $products->products_download_button == 2) {
						...
						$page_content .= '<div style="fssc-buy-button"><a href="'.$BuyButtonLink.'" class="fsscgradient fsscbutton fssclink" '.$BuyButtonStyle.'><span>'.$BuyButtonText.'</span></a></div>';
					}
				}
			} else {
				$page_content .= '&nbsp;';
		...
		$page_content .= '<h3'.$ProductNameStyle.'>'.$ProductLinkS.$ProductsName.$ProductLinkE.'</h3>';
		
		if ($products->short_description != '') {
			$page_content .= '<p'.$ProductDescStyle.'>'.$products->short_description.'</p>';
		} else {
