...
function gatewayapi__shortcode_handle_signup($atts)
{
  ...
  switch ($action) {
    ...
    case 'signup_confirm_sms':
      ...
      // save recipient groups, if any were selected
      $recipient_groups = array_map('intval', $_POST['gatewayapi']['_gatewayapi_recipient_groups'] ?? []);
      if ($recipient_groups) $data['recipient_groups'] = $recipient_groups;
    ...
    case 'signup_confirm_save':
      ...
      $recipient_groups = array_map('intval', get_transient('gwapi_subscriber_' . $msisdn)['recipient_groups'] ?? []);
