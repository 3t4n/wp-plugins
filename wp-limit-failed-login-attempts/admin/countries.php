...
?>
    ...
    <?php
        ...
        /* public function ip_info($ip = NULL, $purpose = "location", $deep_detect = TRUE)
        {
            ...
            //Solve IP Address Spoofing and safely get the data
            $output = NULL;
            if (filter_var($ip, FILTER_VALIDATE_IP) === FALSE) {
                $ip = $_SERVER["REMOTE_ADDR"];
                if ($deep_detect) {
                    // Check HTTP_X_FORWARDED_FOR and split by comma to take the first IP
                    if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
                        $forwarded_ips = explode(',', $_SERVER['HTTP_X_FORWARDED_FOR']);
                        $forwarded_ip = trim(string: $forwarded_ips[0]);
            ...
            if (filter_var($ip, FILTER_VALIDATE_IP) && in_array($purpose, $support)) {
                ...
                //Edited by Ibrahim Shatat 2/11/2024 
                //Safely get the data
                $response = wp_safe_remote_get("http://www.geoplugin.net/json.gp?ip=" . $ip);
    ...
    // Validate the IP or detect it
    if (filter_var($ip, FILTER_VALIDATE_IP) === FALSE) {
        $ip = $_SERVER["REMOTE_ADDR"];
        if ($deep_detect) {
            // Safely handle HTTP_X_FORWARDED_FOR header
            if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
                $forwarded_ips = explode(',', $_SERVER['HTTP_X_FORWARDED_FOR']);
                $forwarded_ip = trim($forwarded_ips[0]); // Corrected syntax
                if (filter_var($forwarded_ip, FILTER_VALIDATE_IP)) {
                    $ip = $forwarded_ip;
                }
            }
    ...
    // Standardize the purpose and define supported options
    $purpose = str_replace(array("name", "\n", "\t", " ", "-", "_"), '', strtolower(trim($purpose)));
    $support = array("country", "countrycode", "state", "region", "city", "location", "address");
    $continents = $this->get_country();
    ...
    if (filter_var($ip, FILTER_VALIDATE_IP) && in_array($purpose, $support)) {
        ...
        if (!is_wp_error($response) && wp_remote_retrieve_response_code($response) === 200) {
            ...
            if (isset($ipdat->geoplugin_countryCode) && strlen(trim($ipdat->geoplugin_countryCode)) === 2) {
                switch ($purpose) {
                    case "location":
                        $output = array(
                            ...
                            "continent"      => $continents[strtoupper($ipdat->geoplugin_continentCode)]['name'] ?? null,
