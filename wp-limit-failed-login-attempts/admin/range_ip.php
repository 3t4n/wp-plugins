...
if (!class_exists('WPLFLA_range_ip')) {
    class WPLFLA_range_ip
    {
        ...
        // Edited By Ibrahim Shatat 17/12/2024
        // Solve Sql Injection in my_ajax_get_range_ip_data function
        ...
        function my_ajax_get_range_ip_data()
        {
            ...
            $columnSortOrder = (strtolower($_POST['order'][0]['dir']) === 'asc') ? 'ASC' : 'DESC'; // Sort direction
            $searchValue    = sanitize_text_field($_POST['search']['value']);
            $type_intr      = absint($_GET['type']);

            $allowed_columns = ['start_ip', 'end_ip', 'date']; // Allowed columns for sorting
            $columnName = in_array($columnName, $allowed_columns) ? $columnName : 'start_ip'; // Fallback if invalid column name
            ...
            if (!empty($searchValue)) {
                $searchQuery .= " AND (start_ip LIKE %s OR end_ip LIKE %s OR date LIKE %s)";
                $likeSearch = '%' . $wpdb->esc_like($searchValue) . '%';
                array_push($query_params, $likeSearch, $likeSearch, $likeSearch);
            }

            $searchQuery .= " AND `type_intr` = %d";
            array_push($query_params, $type_intr);
            ...
            $totalRecordwithFilter = $wpdb->get_var(
                $wpdb->prepare(
                    "SELECT COUNT(*) FROM {$wpdb->prefix}WPLFLA_block_ip_range WHERE 1=1 {$searchQuery}",
                    ...$query_params
                )
            );

            // Fetch records with pagination and sorting
            $query_params[] = $row;         // Offset
            $query_params[] = $rowperpage;  // Limit

            $empRecords = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM {$wpdb->prefix}WPLFLA_block_ip_range 
            ...
            foreach ($empRecords as $row) {
                $data[] = array(
                    "start_ip" => esc_html($row->start_ip),
                    "end_ip"   => esc_html($row->end_ip),
                    "date"     => esc_html($row->date),
                    "action"   => '<a href="' . esc_url(admin_url('admin.php?page=WPLFLARANGEIP&step=delete&id=' . $row->id)) . '" class="delete_ip" onclick="if (!confirm(\'' . esc_js(__('Are you sure to delete this IP?', 'codepressFailed_pro')) . '\')) { return false; }">
