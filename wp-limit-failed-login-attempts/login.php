...
if (!class_exists('WPLFLA_page_login_pro')) {
    class WPLFLA_page_login_pro
    {
        ...
        // Edited by Ibrahim Shatat 2/11/2024
        // Solve Sql Injection in delete_block_ip_req function
        public function delete_block_ip_req($ip)
        {
            global $wpdb;
            $table  = $wpdb->prefix . 'WPLFLA_log_block_ip';
            $get_ip = $wpdb->get_row($wpdb->prepare("select `ip` FROM $table WHERE md5(id) = %s", $ip));
            if (isset($get_ip->ip)) {
                $table_failed  = $wpdb->prefix . 'WPLFLA_login_failed';
                $wpdb->query($wpdb->prepare("UPDATE $table_failed SET `status` = 1 WHERE `ip` = %s", $get_ip->ip));
            }
            $delete = $wpdb->query($wpdb->prepare("DELETE FROM $table WHERE md5(id) = %s", $ip));
            if ($delete) {
                return true;
            } else {
                return false;
...
?>
            <div id="login_error" style="margin-bottom: 10px;">
        ...
        public function add_login_field()
        {
            ...
            ?>
                <div id="login_error" style="margin-bottom: 10px;">
                    <strong><?php _e('Remaining attempts:'); ?></strong>
                    <?php echo esc_html($login_failed_option["WPLFLA_allowed"] + 1 - $log_block_count); ?>
                    <br>
                </div>
            <?php
            }
        ...
        // Edited by Ibrahim Shatat 2/11/2024
        // Solve Sql Injection in check_ip function
        public function check_ip($type)
        {
            $ip = $this->get_the_user_ip();
            global $wpdb;
            $empRecords = $wpdb->get_results($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "WPLFLA_block_ip_range  WHERE `type_intr` = %d", $type));
            foreach ($empRecords as $row) {
                if ($this->ip_in_range($row->start_ip, $row->end_ip, $ip)) {
                    return true;
                }
        ...
        // Edited by Ibrahim Shatat 2/11/2024
        // Solve Sql Injection in check_country function
        public function check_country($type)
        {
            $ip = $this->get_the_user_ip();
            $WPLFLA_countries = new WPLFLA_countries_PRO();
            global $wpdb;
            $totalRecords = $wpdb->get_var($wpdb->prepare("SELECT count(*) FROM " . $wpdb->prefix . "WPLFLA_block_countries where `type_intr` = %d and (`country_code` = %s or `country_code` = 'ALL')", $type, $WPLFLA_countries->ip_info("Visitor", "Country Code")));
            return $totalRecords > 0;
        }
        ...
        public function get_the_user_ip()
        {
            // Always rely on REMOTE_ADDR, which is the direct IP from the client
            $ip = $_SERVER['REMOTE_ADDR'];

            // Apply a filter for flexibility if needed
            return apply_filters('wpb_get_ip', $ip);
        }
