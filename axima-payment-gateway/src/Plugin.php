...
use Pays\PaymentGate\PaymentMethodType;
...
{   const URL_TYPE_OWN_PAGE = 2,
    ...
    public function renderSettings()
    {
        ...
        if (isset($_POST['_submit'])) {
            // use getPost second parameter instead of ?? '' just to maintain better compatibility
    ...
    public function register($file)
    {
        ...
        add_action('before_woocommerce_init', function() use ($file) {
        ...
        add_action('woocommerce_blocks_loaded', function() use($file) {
            if (class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType')) {
                add_action(
                    'woocommerce_blocks_payment_method_type_registration', function (PaymentMethodRegistry $payment_method_registry) {
                    $payment_method_registry->register(new PaymentMethodType());
                    });
            }
        });
        ...
        add_action('plugins_loaded', function () use ($that) {
            ...
            call_user_func(function () {
                ...
                if (class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType')) {
                    require_once __DIR__ . '/PaymentMethodType.php';
                }
            });
