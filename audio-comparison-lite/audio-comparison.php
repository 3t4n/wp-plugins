...
if( ! class_exists( 'audioComparisonLite' ) ) {
  class audioComparisonLite {
    ...
    const MY_VERSION_NUMBER = '3.6';
    const NONCE_ACTION = 'settings_apply';
    const NONCE_FIELD = self::OPTIONS_KEY;
    const IMPORTANT_HTML_ON_BUTTONS = 1730807058;
    const ACKNOWLEDGE_IMPORTANT = 'acknowledge_important';
    private $Styling;
    private $Promoter;
    ...
    public function shortcode($attr = [], $content = null, $tag = '') {
      ...
      $playButton = esc_html($attr['play_button']);
        ...
        $playbuttonbuff .= '<button class="' . self::MAIN_DIV . '-play-stop">' . $textPlay . '</button>' . $endl;
      ...
      $buff .= '<button class="' . self::MAIN_DIV . '-play-a">' . $buttonA . '</button>' . $endl;
      if(!empty($fileB)) {
        $buttonB = esc_html($attr['text_button_b']);
        $buff .= '<button class="' . self::MAIN_DIV . '-play-b">' . $buttonB . '</button>' . $endl;
      }
      $buff .= $playbuttonbuff;
      $buff .= $label;
      $buff .= "</div>";
    ...
    public function get_defaults() {
      return array(
        'important_notice_acknowledged' => '0',  
        'first_install' => '1731533297',         
        'text' => 'before',            
        'play_button' => 'left',       
    ...
    public function get_options() {
      ...
      $old_install = ! array_key_exists('first_install', $current_options);
        $current_options = array_merge($defaults, $current_options);
      if ( $old_install ) {
        $current_options['first_install'] = $current_options['important_notice_acknowledged'];
      }
    ...
    public function settings_page() {
      ...
      $settings_hint2 = '';
      $settings_hint3 = '';
      if ($current_options['important_notice_acknowledged'] < self::IMPORTANT_HTML_ON_BUTTONS  &&  $current_options['first_install'] < self::IMPORTANT_HTML_ON_BUTTONS) {
        $important_notice = '<strong>Important!</strong> Due to <a href="https://wpscan.com/blog/why-admin-xss-is-a-valid-security-issue/">WordPress security concerns</a>, the possibility to use HTML on text outputs and buttons had to be disabled. This applies to the general settings on this page as well as to shortcode attributes.' .
            '<ul style="list-style: revert;padding: revert;"><li>If you did NOT use HTML like <code>&lt;i class="fa-solid fa-play">&lt;/i></code> or <code>&lt;strong>Play&lt;/strong></code> in the settings or in your shortcodes, <strong>you are fine.</strong></li>' .
            '<li><strong>If you did, please change these usages to non-HTML, like CSS based solutions.</strong> HTML code will not render anymore, but instead be displayed on buttons and output text.</li></ul>' .
            'We apologize for the inconvenience, but unfortunately there is nothing we can do about it at the moment.'
            ;
      }
      if (!empty($_POST)) 
      {
        if ( ! isset( $_POST[self::NONCE_FIELD] ) || ! wp_verify_nonce( sanitize_key($_POST[self::NONCE_FIELD]), self::NONCE_ACTION ) ) {
          ...
          $ackroutine = !array_key_exists('text_ready', $_REQUEST);
          if ( isset($_POST[self::ACKNOWLEDGE_IMPORTANT]) )
          {
            $current_options['important_notice_acknowledged'] = time();
            $important_acknowledged = 1;
            $important_notice = '';
          } elseif (! $ackroutine) { 
    ...
    <div class="wrap <?php $this->eh(self::SHORTCODE) ?>_container">
      ...
      <?php if (!empty($important_notice)) : ?>
        ...
        <div class="error">
          <form method="post"><p>
            <?php echo wp_kses_post($important_notice); ?>
            <br><br><input type="checkbox" id="<?php $this->eh(self::ACKNOWLEDGE_IMPORTANT); ?>" name="<?php $this->eh(self::ACKNOWLEDGE_IMPORTANT); ?>"><label for="<?php $this->eh(self::ACKNOWLEDGE_IMPORTANT); ?>" style="font-weight: bold">Do not show this warning again</label>
            <?php wp_nonce_field( self::NONCE_ACTION, self::NONCE_FIELD ); ?>
            <br><button type="submit" id="acknowlede" class="<?php $this->ea(self::SHORTCODE)?>_settings button-primary" style="margin-top:3px;background-color: red;border-color: red;">Acknowledge</button>
          </p></form>
        </div>
      <?php endif; ?>
      <?php if (!empty($saved)) : ?>
      ...
      <?php endif; ?>
      <?php if (!empty($important_acknowledged)) : ?>
        <h2></h2>
        <div class="updated">
      ...
      <div class="AC_ROW">
        ...
        <div style="flex: 1;">
          <p><span style="font-size: x-large"><?php $this->eh(self::SETTINGS_TITLE)?></span> <span style="font-size: small"> Version 3.6</span></p>
          <p>
...
<div class="doc_hide">
      ...
      <h2>Notes</h2>
      <p><del>You can use HTML in the label and button texts, like '&lt;i>Play&lt;/i>'.</del> This feature has been removed due to WordPress security concerns.</p>
    ...
    public function admin_init() {
      $this->load_assets();
      if ($this->get_options()['important_notice_acknowledged'] < self::IMPORTANT_HTML_ON_BUTTONS) {
    ...
    function admin_notice() {
      ...
      $link = admin_url('options-general.php?page=' . self::OPTIONS_KEY);
      $clickhere = "<a href=\"{$link}\">check the settings</a>";
      $admin_pages = ['dashboard','plugins'];
      if ( in_array( $screen->id, $admin_pages ) ) {
        echo wp_kses_post(
          '<div class="notice notice-warning is-dismissible">' . 
            '<table style="margin-top:5px;margin-bottom:2px"><tr><td><img src="' . esc_url(plugins_url('/logo.png', __FILE__)) . '" style="width: 80px;vertical-aling:middle"></td>'.
              '<td>&nbsp;</td>' .
              '<td><strong>Important!</strong> ' . self::SETTINGS_TITLE . " removed the HTML-on-buttons feature due to WordPress security concerns.<br>To check if you need to take action, $clickhere.</td></tr>" .
            '</table>' .
          '</div>'); 
      }
    }
