 ...
 * License: GPL2
 * Text Domain: search-and-check-email-in-post-and-page
...
function seip_admin_settings( $actions, $plugin_file ) {
	...
	if ( $plugin === $plugin_file ) {
		$settings = array();
		$settings['settings']         = '<a href="' . esc_url( admin_url( 'admin.php?page=search-clickable-email' ) ) . '">' . esc_html__( 'Settings', 'search-and-check-email-in-post-and-page' ) . '</a>';
