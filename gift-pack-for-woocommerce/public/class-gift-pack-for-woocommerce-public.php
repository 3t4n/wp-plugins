...
public function gpfw_product_option_custom_field(){
	...
	public function gpfw_add_custom_product_data( $cart_item_data, $product_id, $variation_id ) {
		...
		if( isset( $_REQUEST['gpfw-gift-pack-to'] ) && !empty($_POST['gift_pack_option'])) {
			$cart_item_data[ 'gpfw_gift_pack_to' ] = sanitize_text_field($_REQUEST['gpfw-gift-pack-to']);
			$cart_item_data['unique_key'] = md5( microtime().rand() );
		}
		if( isset( $_REQUEST['gpfw-gift-pack-from'] ) && !empty($_POST['gift_pack_option'])) {
			$cart_item_data[ 'gpfw_gift_pack_from' ] = sanitize_text_field($_REQUEST['gpfw-gift-pack-from']);
			$cart_item_data['unique_key'] = md5( microtime().rand() );
		}
	...
	public function gpfw_display_custom_item_data($cart_item_data, $cart_item) {
		...
		if( isset( $cart_item['gpfw_gift_pack_to'] ) ) {
			...
			if($this->gpfw_options['gpfw_gift_pack_message_text']!=''){
				...
				$gpfw_gift_pack_message_text = __("Giftpack To","gift-pack-for-woocommerce");
			}	
			$cart_item_data[] = array( "name" => $gpfw_gift_pack_message_text, "value" => $cart_item['gpfw_gift_pack_to'] );
		}

		if( isset( $cart_item['gpfw_gift_pack_from'] ) ) {

			if($this->gpfw_options['gpfw_gift_pack_message_text']!=''){
				$gpfw_gift_pack_message_text = $this->gpfw_options['gpfw_gift_pack_message_text'];
			}
			else{
				$gpfw_gift_pack_message_text = __("Giftpack From","gift-pack-for-woocommerce");
			}	
			$cart_item_data[] = array( "name" => $gpfw_gift_pack_message_text, "value" => $cart_item['gpfw_gift_pack_from'] );
		}
