...
class Gift_Pack_For_Woocommerce_Admin
{
    ...
    public function gpfw_update_settings()
    {
        ...
        if (function_exists('icl_object_id') && $this->current_language) {
            ...
            $opt_gpfw_disable_gift_pack_to    = 'gpfw_disable_gift_pack_to' . $this->current_language;
            $opt_gpfw_disable_gift_pack_to_value = get_option($opt_gpfw_disable_gift_pack_to);
            if ( isset($opt_gpfw_disable_gift_pack_to_value) && $opt_gpfw_disable_gift_pack_to_value !== false) {
                $updated_disable_gift_pack_note = isset($_POST['gpfw_disable_gift_pack_to']) ? $_POST['gpfw_disable_gift_pack_to'] : '';
                update_option('gpfw_disable_gift_pack_to' . $this->current_language, sanitize_text_field($updated_disable_gift_pack_note) );
            } else {
                foreach (icl_get_languages('skip_missing=0') as $language) {
                    $lang_code   = $language['language_code'];
                    $option_name = 'gpfw_disable_gift_pack_to' . $lang_code;
                    if (isset($_POST['gpfw_disable_gift_pack_to'])) {
                        $updated_disable_gift_pack_note = sanitize_text_field(isset($_POST['gpfw_disable_gift_pack_to']) ? $_POST['gpfw_disable_gift_pack_to'] : '');
                        add_option($option_name, $updated_disable_gift_pack_note, null, 'no');
                    }
                }
            }

            $opt_gpfw_disable_gift_pack_from    = 'gpfw_disable_gift_pack_from' . $this->current_language;
            $opt_gpfw_disable_gift_pack_from_value = get_option($opt_gpfw_disable_gift_pack_from);
            if ( isset($opt_gpfw_disable_gift_pack_from_value) && $opt_gpfw_disable_gift_pack_from_value !== false) {
                $updated_disable_gift_pack_note = isset($_POST['gpfw_disable_gift_pack_from']) ? $_POST['gpfw_disable_gift_pack_from'] : '';
                update_option('gpfw_disable_gift_pack_from' . $this->current_language, sanitize_text_field($updated_disable_gift_pack_note) );
            } else {
                foreach (icl_get_languages('skip_missing=0') as $language) {
                    $lang_code   = $language['language_code'];
                    $option_name = 'gpfw_disable_gift_pack_from' . $lang_code;
                    if (isset($_POST['gpfw_disable_gift_pack_from'])) {
                        $updated_disable_gift_pack_note = sanitize_text_field(isset($_POST['gpfw_disable_gift_pack_from']) ? $_POST['gpfw_disable_gift_pack_from'] : '');
                        add_option($option_name, $updated_disable_gift_pack_note, null, 'no');
                    }
                }
            }
            ...
            $opt_gpfw_disable_gift_pack_note    = 'gpfw_gift_pack_note_require' . $this->current_language;
            $opt_gpfw_disable_gift_pack_note_value = get_option($opt_gpfw_disable_gift_pack_note);
            if ( isset($opt_gpfw_disable_gift_pack_note_value) && $opt_gpfw_disable_gift_pack_note_value !== false) {
                $updated_disable_gift_pack_note = isset($_POST['gpfw_gift_pack_note_require']) ? $_POST['gpfw_gift_pack_note_require'] : '';
                update_option('gpfw_gift_pack_note_require' . $this->current_language, sanitize_text_field($updated_disable_gift_pack_note) );
            } else {
                foreach (icl_get_languages('skip_missing=0') as $language) {
                    $lang_code   = $language['language_code'];
                    $option_name = 'gpfw_gift_pack_note_require' . $lang_code;
                    if (isset($_POST['gpfw_gift_pack_note_require'])) {
                        $updated_disable_gift_pack_note = sanitize_text_field(isset($_POST['gpfw_gift_pack_note_require']) ? $_POST['gpfw_gift_pack_note_require'] : '');
                        add_option($option_name, $updated_disable_gift_pack_note, null, 'no');
                    }
                }
            }

            $opt_gpfw_disable_gift_pack_to    = 'gpfw_gift_pack_to_require' . $this->current_language;
            $opt_gpfw_disable_gift_pack_to_value = get_option($opt_gpfw_disable_gift_pack_to);


            if ( isset($opt_gpfw_disable_gift_pack_to_value) && $opt_gpfw_disable_gift_pack_to_value !== false) {
                $updated_disable_gift_pack_note = isset($_POST['gpfw_gift_pack_to_require']) ? $_POST['gpfw_gift_pack_to_require'] : '';
                update_option('gpfw_gift_pack_to_require' . $this->current_language, sanitize_text_field($updated_disable_gift_pack_note) );
            } else {
                foreach (icl_get_languages('skip_missing=0') as $language) {
                    $lang_code   = $language['language_code'];
                    $option_name = 'gpfw_gift_pack_to_require' . $lang_code;
                    if (isset($_POST['gpfw_gift_pack_to_require'])) {
                        $updated_disable_gift_pack_note = sanitize_text_field(isset($_POST['gpfw_gift_pack_to_require']) ? $_POST['gpfw_gift_pack_to_require'] : '');
                        add_option($option_name, $updated_disable_gift_pack_note, null, 'no');
                    }
                }
            }

            $opt_gpfw_disable_gift_pack_from    = 'gpfw_gift_pack_from_require' . $this->current_language;
            $opt_gpfw_disable_gift_pack_from_value = get_option($opt_gpfw_disable_gift_pack_from);
            if ( isset($opt_gpfw_disable_gift_pack_from_value) && $opt_gpfw_disable_gift_pack_from_value !== false) {
                $updated_disable_gift_pack_note = isset($_POST['gpfw_gift_pack_from_require']) ? $_POST['gpfw_gift_pack_from_require'] : '';
                update_option('gpfw_gift_pack_from_require' . $this->current_language, sanitize_text_field($updated_disable_gift_pack_note) );
            } else {
                foreach (icl_get_languages('skip_missing=0') as $language) {
                    $lang_code   = $language['language_code'];
                    $option_name = 'gpfw_gift_pack_from_require' . $lang_code;
                    if (isset($_POST['gpfw_gift_pack_from_require'])) {
                        $updated_disable_gift_pack_note = sanitize_text_field(isset($_POST['gpfw_gift_pack_from_require']) ? $_POST['gpfw_gift_pack_from_require'] : '');
                        add_option($option_name, $updated_disable_gift_pack_note, null, 'no');
                    }
                }
            }
            ...
            if ($gpfw_options['gpfw_disable_gift_pack_to'] !== false) {
                update_option('gpfw_disable_gift_pack_to', sanitize_text_field($_POST['gpfw_disable_gift_pack_to']));
            } else {
                add_option('gpfw_disable_gift_pack_to', sanitize_text_field($_POST['gpfw_disable_gift_pack_to']), null, 'no');
            }

            if ($gpfw_options['gpfw_disable_gift_pack_from'] !== false) {
                update_option('gpfw_disable_gift_pack_from', sanitize_text_field($_POST['gpfw_disable_gift_pack_from']));
            } else {
                add_option('gpfw_disable_gift_pack_from', sanitize_text_field($_POST['gpfw_disable_gift_pack_from']), null, 'no');
            }

            if ($gpfw_options['gpfw_gift_pack_note_require'] !== false) {
                update_option('gpfw_gift_pack_note_require', sanitize_text_field($_POST['gpfw_gift_pack_note_require']));
            } else {

                add_option('gpfw_gift_pack_note_require', sanitize_text_field($_POST['gpfw_gift_pack_note_require']), null, 'no');
            }


            if ($gpfw_options['gpfw_gift_pack_to_require'] !== false) {
                update_option('gpfw_gift_pack_to_require', sanitize_text_field($_POST['gpfw_gift_pack_to_require']));
            } else {

                add_option('gpfw_gift_pack_to_require', sanitize_text_field($_POST['gpfw_gift_pack_to_require']), null, 'no');
            }


            if ($gpfw_options['gpfw_gift_pack_from_require'] !== false) {
                update_option('gpfw_gift_pack_from_require', sanitize_text_field($_POST['gpfw_gift_pack_from_require']));
            } else {
                add_option('gpfw_gift_pack_from_require', sanitize_text_field($_POST['gpfw_gift_pack_from_require']), null, 'no');
            }
        }
...
// Enable Gift Pack Note Character Limit Checkbox
if (function_exists('icl_object_id') && $this->current_language) {
    $opt_limit    = 'gpfw_enable_gift_pack_note_limit' . $this->current_language;
    $opt_note_limit = get_option($opt_limit);

    if (isset($opt_note_limit) && $opt_note_limit !== false) {
        $updated_gift_note_limit = isset($_POST['gpfw_enable_gift_pack_note_limit']) ? sanitize_text_field($_POST['gpfw_enable_gift_pack_note_limit']) : '';
        update_option($opt_limit, $updated_gift_note_limit);
    } else {
        foreach (icl_get_languages('skip_missing=0') as $language) {
            $lang_code   = $language['language_code'];
            $option_name = 'gpfw_enable_gift_pack_note_limit' . $lang_code;

            if (isset($_POST['gpfw_enable_gift_pack_note_limit'])) {
                $updated_gpfw_enable_gift_pack_note_limit = sanitize_text_field($_POST['gpfw_enable_gift_pack_note_limit']);
                add_option($option_name, $updated_gpfw_enable_gift_pack_note_limit, null, 'no');
            }
        }
    }
} else {
    ...
    if ($gpfw_options['gpfw_enable_gift_pack_note_limit'] !== false) {
        $updated_note_limit = isset($_POST['gpfw_enable_gift_pack_note_limit']) ? sanitize_text_field($_POST['gpfw_enable_gift_pack_note_limit']) : '';
        update_option('gpfw_enable_gift_pack_note_limit', $updated_note_limit);
    } else {
        $updated_note_limit = isset($_POST['gpfw_enable_gift_pack_note_limit']) ? sanitize_text_field($_POST['gpfw_enable_gift_pack_note_limit']) : '';
        add_option('gpfw_enable_gift_pack_note_limit', $updated_note_limit, null, 'no');
    }
}


// Set Gift pack note Character Limit
if (function_exists('icl_object_id') && $this->current_language) {
    $opt_limit    = 'gpfw_choose_gift_limit' . $this->current_language;
    $opt_note_limit = get_option($opt_limit);

    if (isset($opt_note_limit) && $opt_note_limit !== false) {
        $updated_gift_note_limit = isset($_POST['gpfw_choose_gift_limit']) ? sanitize_text_field($_POST['gpfw_choose_gift_limit']) : '';
        update_option($opt_limit, $updated_gift_note_limit);
    } else {
        foreach (icl_get_languages('skip_missing=0') as $language) {
            $lang_code   = $language['language_code'];
            $option_name = 'gpfw_choose_gift_limit' . $lang_code;

            if (isset($_POST['gpfw_choose_gift_limit'])) {
                $updated_gpfw_choose_gift_limit = sanitize_text_field($_POST['gpfw_choose_gift_limit']);
                add_option($option_name, $updated_gpfw_choose_gift_limit, null, 'no');
            }
        }
    }
} else {
    ...
    if ($gpfw_options['gpfw_choose_gift_limit'] !== false) {
        $updated_note_limit = isset($_POST['gpfw_choose_gift_limit']) ? sanitize_text_field($_POST['gpfw_choose_gift_limit']) : '';
        update_option('gpfw_choose_gift_limit', $updated_note_limit);
    } else {
        $updated_note_limit = isset($_POST['gpfw_choose_gift_limit']) ? sanitize_text_field($_POST['gpfw_choose_gift_limit']) : '';
        add_option('gpfw_choose_gift_limit', $updated_note_limit, null, 'no');
    }
}


// Gift pack image pop-up check box
if (function_exists('icl_object_id') && $this->current_language) {
