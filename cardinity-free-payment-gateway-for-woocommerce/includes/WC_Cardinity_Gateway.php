    ...
    function write_to_file( $file_path, $content ) {
        ...
        if ( $wp_filesystem->exists( $file_path ) ) {
            ...
            if ( $existing_content === false ) {
                return false; // Failed to read the existing file
            }
        ...
        if ( $wp_filesystem->put_contents( $file_path, $content, FS_CHMOD_FILE ) ) {
            return true; // Successfully wrote/appended the file
        } else {
...
class WC_Cardinity_Gateway extends WC_Payment_Gateway
{
    ...
    public function admin_options()
    {
        ...
        <div id="message" class="updated woocommerce-success">
            ...
            <a  href="<?php echo esc_url(admin_url( 'admin.php?page=wc-settings&tab=checkout&section=cardinity&review=dontshow' ))?>" >Dont show again</a>
            </p>
        </div>
        <?php
        }
        ...
        if(!$country){
            ...
            <p>You should enter a default customer location, its available under <a href="'.esc_url(admin_url('admin.php?page=wc-settings&amp;tab=general')).'">general settings page</a>. Leaving it empty might cause problem on checkout</p>
            </div>';
        }
        ...
        if( ! isset( $_GET['screen'] ) || $_GET['screen'] === '' ) {
            ...
            if($_GET['screen'] == 'transactions_log'){
                ...
                foreach($transactionLogFiles as $aFile){
                    ...
                    echo 'Download - <a  href="'.esc_url(admin_url( 'admin.php?page=wc-settings&tab=checkout&section=cardinity&screen=transactions_log_download&lognumber='.esc_attr($number) )).'" class="link">'.
                    esc_attr($aFile).
                    '</a><br/>';
                }
        ...
        if ($this->enabled == 'yes') {
            if (get_option('woocommerce_force_ssl_checkout') == 'no') {
                echo "<div class=\"error\"><p>" .
                    sprintf(
                        /* translators: 1: Cardinity checkout method 2: Checkout URL. */
                        esc_html(__('<strong>%1$s</strong> is enabled and WooCommerce is not forcing the SSL certificate on your checkout page. Please ensure that you have a valid SSL certificate and that you are <a href=\"%2$s\">forcing the checkout pages to be secured.</a>', 'cardinity-free-payment-gateway-for-woocommerce')),
    ...
    public function process_direct_payment($order_id)
    {
        ...
        self::log('Direct payment errors: \r\n' . wp_json_encode($result['errors']), "OrderID : $order_id");
    ...
    function receipt_page($order_id)
    {
        ...
        if($v2 == 'true'){
            ...
            echo wp_kses($this->generate_threed_v2_form(), array(
                'form' => array(
                    'action' => true,
                    'id' => true,
                    'method' => true,
            ...
            echo wp_kses($this->generate_threed_form(), array(
                'form' => array(
                    'action' => true,
                    'id' => true,
                    'method' => true,
    ...
    public function notify_response()
    {
        if (isset($_POST['order_id'])){
            ...
            echo esc_html("Notification received by WP OrderID :: $orderId");
    ...
    public function check_response_external($redirect = true)
    {
        ...
        if ($signature == $post_data['signature'] && $post_data['status'] == 'approved') {
            ...
            $order = false;
            if(isset($_GET['wc-order'])){
                $order_id = cardinity_wc_decode_base64url(sanitize_text_field(wp_unslash($_GET['wc-order'])));
    ...
    {
        $post_data = sanitize_post($_POST,'raw');
        self::log('Cardinity callback.'.wp_json_encode($_POST));
    ...
    private function sendCardinityRequest($method, $order_id)
    {
        ...
        try {
            ...
            if ($payment->getStatus() == 'approved') {
                ...
                /* translators: 1: Payment compelte datetime , 2: PaymentID. */
                $message    = sprintf(__('Cardinity payment completed at: %1$s. Payment ID: %2$s', 'woocommerce'), $payment->getCreated(), $payment_id);
            ...
            $payment_id = $payment->getId();
            /* translators: %1$s Payment Error, %2$s Payment ID */
            $message    = sprintf(__('Cardinity payment was declined: %1$s. Payment ID: %2$s', 'woocommerce'), $payment->getError(), $payment_id);
            ...
            foreach ($exception->getViolations() as $violation) {
                /* translators: Invalid Attribute Error */
            ...
            array_push($errors, sprintf(__('Validation error: %s.', 'woocommerce'), $exception->getErrorsAsString()));
    ...
    private function getPaymentParams($order)
    {
        if (WC()->version < '2.7.0') {
            ...
            $holder_name = '';
            if($this->get_option('show_holder') == 'yes' && isset($_POST[CARD_HOLDER])){
                $holder_name = sanitize_text_field(wp_unslash($_POST[CARD_HOLDER]));
            }
        ...
        if(!isset($_POST[CARD_CVC]) || !isset($_POST[CARD_PAN]) || !isset($_POST[CARD_EXPIRY])){
            return [];
        }
        $crd_order_id      = str_pad($order_id, 2, '0', STR_PAD_LEFT);
        $cvc               = sanitize_text_field(wp_unslash($_POST[CARD_CVC]));
        $card_number       = str_replace(' ', '', sanitize_text_field(wp_unslash($_POST[CARD_PAN])));
        $card_expire_array = explode('/', sanitize_text_field(wp_unslash($_POST[CARD_EXPIRY])));
        ...
        $cardinity_screen_width = (int) sanitize_text_field(wp_unslash($_POST['cardinity_screen_width']??"800"));
        $cardinity_screen_height = (int) sanitize_text_field(wp_unslash($_POST['cardinity_screen_height']??"600"));

        $cardinity_color_depth = (int) sanitize_text_field(wp_unslash($_POST['cardinity_color_depth']??24));
        $cardinity_time_zone = (int) sanitize_text_field(wp_unslash($_POST['cardinity_time_zone']??-60));

        $cardinity_browser_language = sanitize_text_field(wp_unslash($_POST['cardinity_browser_language']??"en-US"));
        ...
        return [
            ...
            'threeds2_data' =>  [
                ...
                "browser_info" => [
                    ...
                    "user_agent" => isset($_SERVER['HTTP_USER_AGENT']) ? sanitize_text_field(wp_unslash($_SERVER['HTTP_USER_AGENT'])) : "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:21.0) Gecko/20100101 Firefox/21.0",
    ...
    private function isCorrectExpireDate($toCheck)
    {
        ...
        $now       = time();
        $result    = false;
        $thisYear  = (int) gmdate('y', $now);
        $thisMonth = (int) gmdate('m', $now);
    ...
    public function generate_screen_button_html( $key, $value ) {
        ...
        <tr valign="top">
            ...
            <td class="forminp">
                <a  href="'.esc_url(admin_url( 'admin.php?page=wc-settings&tab=checkout&section=cardinity&screen=transactions_log' )).'" class="button">'.
                  __( 'View', 'wcommerce' ).
                '</a>
            </td>
        </tr>
    ...
    public function add_transaction_history($data){
        ...
        $currentFilename = "transactions-".gmdate("Y-m").'.log';
        ...
        try {
            write_to_file($transactionFile, $message."\n", FILE_APPEND);
