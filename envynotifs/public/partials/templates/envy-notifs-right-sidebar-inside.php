...
// Output for inside right sidebar
function notifs_show_inside_right() {
    $inside_right_notice_show = new WP_Query( array(
        'post_type' => 'envynotifs',
        'posts_per_page' => 1,
    ));     
    while( $inside_right_notice_show->have_posts() ) : $inside_right_notice_show->the_post() ?>
        <?php
        global $post; 
        $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'single-post-thumbnail' ); ?>
        <div class="envynotifs-right-sidebar notifino-inside-right-panel" style="background-image: url('<?php echo $image[0]; ?>'); background-repeat: no-repeat; background-size: cover;">
            <div class="envynotifs-right-sidebar-container">
                <ul class="envynotifs-right-sidebar-wrap">
                    <li class="envynotifs-right-sidebar-single">
                        <div class="envynotifs-right-sidebar-title">
                            <h2>
                    ...
                    if( ! $notifs_bar_end_date_new == '' ) : ?>
                        <li class="envynotifs-right-sidebar-single">
                            <div class="envynotifs-right-sidebar-time">
                                <ul class="envynotifs-right-sidebar-time-list">
                    ...
                    if( ! $notifs_bar_subscribe_new == '' ) : ?>
                        <li class="envynotifs-right-sidebar-single">
                            <div class="envynotifs-right-sidebar-sign-up">
                                ...
                                <span class="envynotifs-right-sidebar-subscribe-title"><?php echo esc_html__( $notifs_bar_subscribe_title_new ); ?></span>
                    ...
                    || ! $notifs_bar_social_skype_new == '' ) : ?>
                        <li class="envynotifs-right-sidebar-single">
                            <div class="envynotifs-right-sidebar-social-icon">
                                <ul class="envynotifs-right-sidebar-social-icon-list">
                                    <?php if( ! $notifs_bar_social_title_new == '' ) : ?>
                                    <li>
                                        <span class="envynotifs-right-sidebar-social">
                    ...
                    if( get_post_meta( get_the_ID(), 'custom_element_grid_class_meta_box', true ) ) :
                        ...
                        <li class="envynotifs-right-sidebar-single envynotifs-read-more">
                            <div class="envynotifs-right-sidebar-btn">
                                <a href="<?php echo get_post_meta( get_the_ID(), 'custom_element_grid_class_meta_box', true ); ?>" class="envynotifs-right-sidebar-button">
                                    <?php 
            ...
            <button id="envynotifs-close-btn5" class="envynotifs-right-sidebar-close-button">
...
if( $notifs_bar_mobile_hide_new && wp_is_mobile() ) : ?>
    <script>
        jQuery(document).ready(function(){
            jQuery('#notifino-open-close5').css('display', 'none');
            jQuery('.notifino-inside-right-panel').css('display', 'none');            
        });
    </script> 
<?php endif;
...
if( is_user_logged_in() ) : ?>
    <script>
        jQuery(document).ready(function(){
            jQuery(".envynotifs-right-sidebar").css('top', '55%');
            jQuery("#notifino-open-close5").css('top', '32px');
        });
    </script>
<?php endif;
...
add_action( 'wp_footer', 'notifs_show_inside_right' );
