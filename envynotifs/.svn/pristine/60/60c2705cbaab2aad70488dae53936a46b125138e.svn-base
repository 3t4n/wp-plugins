...
// Output for Top Bar
function notifs_show_top() {
    $top_notice_show = new WP_Query( array(
        'post_type' => 'envynotifs',
        'posts_per_page' => 1,
    ));     
    while( $top_notice_show->have_posts() ) : $top_notice_show->the_post() ?>
        <?php
        global $post; 
        $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'single-post-thumbnail' ); ?>
        <div class="envynotifs-bar notifino-top-panel" style="background-image: url('<?php echo $image[0]; ?>'); background-repeat: no-repeat; background-size: cover;">
            ...
            <div class="envynotifs-details" onclick="envynotifsshowToggle();">
                <span class="envynotifs-details-show fa fa-angle-double-down"></span>    
                <div class="envynotifsshow">
                    <p><?php the_content(); ?></p>
                    <span class="envynotifs-details-hide fa fa-angle-double-up"></span>
                </div>
            </div>
            <?php endif;
            ...
            <button id="envynotifs-close-btn1" class="envynotifs-close-button">
...
if( $notifs_bar_mobile_hide_new && wp_is_mobile() ) : ?>
    <script>
        jQuery(document).ready(function(){
            ...
            jQuery('.notifino-top-panel').css('display', 'none');            
        });
    </script>
<?php elseif( wp_is_mobile() ) : ?>
    <script>
        jQuery(document).ready(function(){
            jQuery('#notifino-open-close1').css('display', 'block');
            jQuery('.notifino-top-panel').css('display', 'none');            
        });
    </script>
<?php elseif( ! wp_is_mobile() ) : ?> 
...
if( is_user_logged_in() ) : ?>
    <script>
        jQuery(document).ready(function(){
            jQuery(".notifino-top-panel, #notifino-open-close1").css('top', '32px');
        });
    </script>
    <?php if( wp_is_mobile() ) : ?>
        <script>
            jQuery(document).ready(function(){
                jQuery(".notifino-top-panel, #notifino-open-close1").css('top', '46px');
            });
        </script> 
    <?php endif; ?>
<?php endif;
...
add_action( 'wp_footer', 'notifs_show_top' );
