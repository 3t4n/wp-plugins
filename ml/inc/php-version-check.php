...
if(version_compare(phpversion(), '5', '<')){
    ...
    $icl_sitepress_idx = array_search(ICL_PLUGIN_FOLDER . '/ml.php', $active_plugins);
    if(false !== $icl_sitepress_idx){
        unset($active_plugins[$icl_sitepress_idx]);
        update_option('active_plugins', $active_plugins);
        unset($_GET['activate']);
        $recently_activated = get_option('recently_activated');
        if(!isset($recently_activated[ICL_PLUGIN_FOLDER . '/ml.php'])){
            $recently_activated[ICL_PLUGIN_FOLDER . '/ml.php'] = time();
