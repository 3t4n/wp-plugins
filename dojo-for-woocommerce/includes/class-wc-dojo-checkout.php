...
if (!class_exists('WC_Dojo')) {
	...
	class WC_Dojo extends WC_Payment_Gateway
	{
		 ...
		 * DOJO URLs
		 */
		const URL_TEST_CARDS  = 'https://docs.dojo.tech/development-resources/testing';
		...
		protected function build_request_create_payment_intent($order)
		{
			...
			return [
				...
				'customer'        => [
					'emailAddress'=> sanitize_email(substr($order->get_billing_email(), 0, 254)),
					'phoneNumber' => sanitize_text_field(substr($order->get_billing_phone(), 0, 50)),
                ],
		...
		protected function get_order_items($order)
		{
			...
			foreach ($order->get_items() as $item) {
				$result[] = [
					...
					'name'        => substr(sanitize_text_field($item->get_product()->get_name()), 0, 100),
					'amountTotal' => [
			...
			if ($order->get_total_shipping() > 0) {
				$result[] = [
					...
					'name'	      => sanitize_text_field("Shipping"),
					'amountTotal' => [
