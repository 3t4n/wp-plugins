...
if (!class_exists('WC_Dojo_Webhooks_Processor')) {
	...
	class WC_Dojo_Webhooks_Processor
	{
			...
			try {
				...
				if ($order === null) {
					$this->logger->log(
						"Error",
						"process_webhook_call", 
						"Order not found by payment intent ID inside webhook handler. Id: " . $event->payment_intent_id,
						$api_secret
					);
				}
		...
		/**
		 * Validates dojo-signature (https://docs.dojo.tech/plugins/woocommerce/configure#step-3-add-a-webhook-endpoint)
		 *
