...
class eHiveObjectsTagCloud_Widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		try {
			switch($siteType) {
			case 'Account':
				$tagCloud = $eHiveApi->getTagCloudInAccount($accountId, $count);
				break;
			case 'Community':
				$tagCloud = $eHiveApi->getTagCloudInCommunity($communityId, $count);
				break;
			default:
				$tagCloud = $eHiveApi->getTagCloudInEHive($count);
				break;
			}
			...
			if ($cssClass == "") {
				...
				echo '<div class="ehive-tag-cloud-widget '.$cssClass.'">';
			}		
			...
			foreach ($tagCloud->tagCloudTags as $tagCloudTag) {    	
				...
				if (isset($eHiveSearch)) {
					...
					$searchParameters = '?' . $searchOptions['query_var'] . '=' . urlencode('tag:"' . $tagCloudTag->cleanTagName . '"');
					$link = $eHiveAccess->getSearchPageLink( $searchParameters );
				} else {
					$link = '#';
				}
