...
require_once 'classes/HelcimJSValidator.php';
require_once 'classes/HelcimJSRequestPayload.php';
...
require_once 'classes/HelcimDirectValidator.php';
require_once 'classes/HelcimDirectRequestPayload.php';
require_once 'classes/HelcimApiFactory.php';
require_once 'classes/HelcimUtilities.php';
...
class WCHelcimGateway extends WC_Payment_Gateway
{
    ...
    public const GOOGLE_RECAPTCHA_TEST_SITE_KEY = '6LcgxK0UAAAAAH9gzsILRr82KZSLrBcry9RynMn4';
    ...
    public function process_payment($orderId): array
    {
        ...
        if (!self::FORCE_HELCIM_JS_TO_RUN_VERIFY && $this->isJS()) {
            //
            // purchase via helcim js
            //
            if($this->isCheckoutBlocks()){
                $xmlString = base64_decode($_POST['xml']);
                $xmlHash = $_POST['hash'];
            }else{
                $xmlString = $_POST['xml'];
                $xmlHash = $_POST['xmlHash'];
            }
        ...
        if(empty($this->apiTokenV2)){
            ...
            if(!empty($_POST['customerCode']) && $this->isJS()){
                ...
                if($currentUserId && (string)$_POST['customerCode'] !== $currentUserId){
                    $customerCode = $currentUserId;
                    ...
                    if(!$customerIdCurrent){
                        ...
                        self::log("cannot update customer code from(".(string)$_POST['customerCode'].") to($currentUserId). new customer code is already used");
                    }
                }
            ...
            // process
            if(
                $this->isCheckoutBlocks()
                && (
                    (!$this->requestPayload instanceof HelcimJSRequestPayload && $this->isJS())
                    || (!$this->requestPayload instanceof HelcimDirectRequestPayload && $this->isDirect())
                )
            ){
                wc_add_notice('<b>Payment error:</b> Something went wrong please contact the Merchant', 'error');
                self::log('Missing Request Payload');
                return [];
            }

            if($this->transactionType === self::TRANSACTION_TYPE_PURCHASE){
                $responsePayload = $this->getHelcimDirectService()->processPurchase($order, $this, $customerCode, $this->requestPayload);
            }elseif($this->transactionType === self::TRANSACTION_TYPE_PREAUTH){
                $responsePayload = $this->getHelcimDirectService()->processPreauth($order, $this, $customerCode, $this->requestPayload);
            }else{
                wc_add_notice('<b>Payment error:</b> Something went wrong please contact the Merchant', 'error');
                self::log("Unknown transaction type: {$this->transactionType}");
                return [];
            }
            ...
            if(!isset($responsePayload['status']) || $responsePayload['status'] !== 'APPROVED'){
                if(!empty($this->getHelcimDirectService()->getError())){
                    if (str_contains($this->getHelcimDirectService()->getError(), 'must be a valid Phone Number')) {
                        $errorLog = 'Phone must be valid phone number (10-15 digits)';
                    }else{
                        $errorLog = $this->getHelcimDirectService()->getError();
                    }
                    wc_add_notice("<b>Payment error:</b> $errorLog", 'error');
                    self::log("ORDER {$order->get_id()} ERROR - $errorLog");
                }else{
                    if(!empty($responsePayload['errors']) && is_string($responsePayload['errors'])){
                        $errorLog = $responsePayload['errors'];
                    }elseif(!empty($responsePayload['status'] === 'DECLINED')){
                        $errorLog = 'Transaction Declined';
                    }else{
                        $errorLog = 'Something went wrong please contact the Merchant';
                    }
                    wc_add_notice("<b>Payment failed:</b> $errorLog", 'error');
                    self::log("ORDER {$order->get_id()} Declined - $errorLog");
                }
                $order->add_order_note("Helcim Payment Failed - $errorLog");
                return [];
            }
            $this->completeOrder(
                $order,
    ...
    public function payment_fields(): void
    {
        ...
        if ($order instanceof WC_Order || $order instanceof WC_Order_Refund) {
            ...
            $orderNumber = HelcimUtilities::generateOrderNumber((string)$order->get_order_number());
        } else {
    ...
    {
        if ($this->isJS()) {
            $this->validateJsFields();
            return;
        }

        if ($this->isDirect()) {
            $this->validateDirectFields();
            return;
        }
    ...
    public function isCheckoutBlocks(): bool
    {
        return !empty($_POST['ischeckoutblocks']);
    }

    private function validateJsFields(): void
    {
        if (!$this->getHelcimJSService() instanceof HelcimJSService) {
            $this->setHelcimJSService(new HelcimJSService());
        }

        $validator = new HelcimJSValidator();
        if ($this->isCheckoutBlocks()) {
            if(!$validator->isValidCheckoutBlocksFields($_POST, $this->getJsSecretKey())){
                wc_add_notice('<b>Payment error:</b> Something went wrong please contact the Merchant', 'error');
                self::log("Helcim JS Validation Failed - {$validator->error}");
                return;
            }
            $this->requestPayload = HelcimUtilities::buildObjectFromEncodedXMLString($_POST['xml'], HelcimJSRequestPayload::class);
            return;
        }

        if (!$validator->isValidFields($_POST, $this->getJsSecretKey())) {
            wc_add_notice('<b>Payment error:</b> Something went wrong please contact the Merchant', 'error');
            self::log("Helcim JS Validation Failed - {$validator->error}");
            return;
        }
    ...
    private function validateDirectFields(): void
    {
        $validator = new HelcimDirectValidator();
        if ($this->isCheckoutBlocks()) {
            if(!$validator->isValidCheckoutBlocksFields($_POST)){
                wc_add_notice($validator->error, 'error');
                self::log("validate fields failed. $validator->error");
                return;
            }
            $this->requestPayload = HelcimUtilities::buildObjectFromArray($_POST, HelcimDirectRequestPayload::class);
            return;
        }

        if(!$validator->isValidFields($_POST)){
            wc_add_notice($validator->error, 'error');
            self::log("validate fields failed. $validator->error");
        }
    }
