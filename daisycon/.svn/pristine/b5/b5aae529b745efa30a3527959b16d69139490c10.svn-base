 ...
 * File: prefill_energy_be.php
 *
 * View for the shorttags to be displayed on the website
 *
 */

class generalDaisyconPrefillEnergyBE{

	public static function adminDaisyconPrefillEnergyBE()
	{
		// Load files
		...
		if (false === empty($base['profile_id']))
		{
			...
			echo "<script type=\"text/javascript\">					
						processData(
							{
								url : 'https://daisycon.tools/api/energy/belgium/nl-NL/meter?language=nl',
							}
		...
		echo '<div class="dc_box">
				<img class="dc_box_header" src="https://static.daisycon.tools/energy-be/presentation/wp-plugin-daisycon-header.png" alt="Energievergelijker" />
				...
				echo '<p class="dc_box_description' . (true === empty($updateSettings['message']) ? ' dc_box_description--no_margin' : '') . '">Door de gegevens in te vullen genereert u op een eenvoudige manier onze prefill energie tool.</p>
							...
							<form action="" class="dc_settings" method="POST"/>
								...
								<div class="dc_shorttag" onclick="daisycon_select_all(this)">
									[daisycon_prefill_energy_be profile_id="' . $base['profile_id'] . '"]
								</div>
	...
	public static function frontDaisyconPrefillEnergyBE($array)
	{
		// Load settings
		...
		// Register files
		wp_register_script('daisycon_prefill_energy_be_js', '//daisycon.tools/prefill-energy-be/app.js');

		// Add files to head
		wp_enqueue_script('daisycon_prefill_energy_be_js');
