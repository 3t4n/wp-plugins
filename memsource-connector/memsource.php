...
define('MEMSOURCE_PLUGIN_VERSION', '4.7.3');
...
function memsource_check_php_version()
{
    ...
    if (!version_compare($phpVersion, MEMSOURCE_PLUGIN_REQUIERED_PHP_VERSION, '>=')) {
        wp_die(__(sprintf('Plugin requires PHP ' . MEMSOURCE_PLUGIN_REQUIERED_PHP_VERSION . ' or higher. Your version is %s. Please, update the version.', $phpVersion), 'memsource-connector'));
    }
}
...
function memsource_set_debug_mode()
{
    ...
    if (isset($_POST['logFilePathToggle']) && $_POST['logFilePathToggle'] === 'on' && !empty($_POST['logFilePath'])) {
        $logFilePath = $_POST['logFilePath'];
        $logFilePath = trim($logFilePath);
        if (strpos($logFilePath, '.') === 0) {
            memsource_redirect_with_error($logFilePath, LogUtils::ERROR_RELATIVE_PATH_NOT_ALLOWED);
        }
        if (!@is_dir($logFilePath)) {
            memsource_redirect_with_error($logFilePath, LogUtils::ERROR_LOG_PATH_NOT_DIRECTORY);
        }
        if (!is_writable($logFilePath)) {
            memsource_redirect_with_error($logFilePath, LogUtils::ERROR_LOG_PATH_NOT_WRITABLE);
        }
        $appRegistry->getOptionsService()->setLogFilePath($logFilePath);
    } else {
        $appRegistry->getOptionsService()->setLogFilePath(null);
    }
...
function memsource_redirect_with_error($logFilePath, $error)
{
    global $appRegistry;
    $appRegistry->getOptionsService()->setLogFilePath(null);
    $url = add_query_arg('error', $error, wp_get_referer());
    $url = add_query_arg('path', $logFilePath, $url);
