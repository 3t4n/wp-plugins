...
final class Cart_Products {
	...
	public function products_template() {
		...
		<template v-if="type == 'cart_products:products'">
			...
			<select class="select2-flex1" v-model="products" ref="select2_ajax" multiple v-else data-select2-map="<?php echo esc_attr(wp_json_encode($values_map)) ?>" data-placeholder="<?php esc_html_e('Products', 'advanced-rule-based-shipping'); ?>">
