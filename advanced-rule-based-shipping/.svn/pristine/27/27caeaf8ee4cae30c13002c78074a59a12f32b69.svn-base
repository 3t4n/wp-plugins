...
class Shipping_Method extends \WC_Shipping_Method {
	...
	public function get_matched_rule() {
		...
		array_walk($matched_rules, function (&$rule) {
			...
			if (in_array($rule['shipping_cost_type'], array('based_on_weight', 'based_on_quantity', 'based_on_subtotal'))) {
				$cart_items_data = array();
				foreach (WC()->cart->get_cart() as $cart_key => $item) {
					if ($item['data']->is_virtual() && isset($rule['shipping_cost_exclude_virtual_products']) && $rule['shipping_cost_exclude_virtual_products']) {
						continue;
					}
				...
				if ('based_on_quantity' === $rule['shipping_cost_type']) {
					$shipping_cost_type_value = array_sum(wp_list_pluck($cart_items_data, 'quantity'));
				}

				if ('based_on_subtotal' === $rule['shipping_cost_type']) {
					$subtotal = array_sum(wp_list_pluck($cart_items_data, 'line_subtotal'));

					$discount = 0;
					if (isset($rule['shipping_cost_exclude_discount']) && true == $rule['shipping_cost_exclude_discount']) {
						$discount = (float) WC()->cart->get_discount_total();
					}

					$shipping_cost_type_value = $subtotal - $discount;

					if (isset($rule['shipping_cost_including_tax']) && true == $rule['shipping_cost_including_tax']) {
						$shipping_cost_type_value += (float) array_sum(wp_list_pluck($cart_items_data, 'line_subtotal_tax'));

						if (isset($rule['shipping_cost_exclude_discount']) && true == $rule['shipping_cost_exclude_discount']) {
							$shipping_cost_type_value -= (float) WC()->cart->get_discount_tax();
						}
					}

					if ($shipping_cost_type_value <= 0) {
						$shipping_cost_type_value  = 0;
					}
				}
	...
	public function calculate_shipping($package = array()) {
		$matched_rule = $this->get_matched_rule();
		...
		$rate = array(
			'package' => $package,
			'id' => $this->get_rate_id(),
			'label' => $matched_rule['title'],
			'cost' => floatval($matched_rule['shipping_cost']),
		);
