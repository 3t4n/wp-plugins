...
final class User {
	...
	public function users() {
		...
		<template v-if="type == 'user:users'">
			...
			<select ref="select2_ajax" v-model="users" multiple v-else data-select2-map="<?php echo esc_attr(wp_json_encode($values_map)) ?>" data-placeholder="<?php esc_html_e('Select users', 'advanced-rule-based-shipping'); ?>">
