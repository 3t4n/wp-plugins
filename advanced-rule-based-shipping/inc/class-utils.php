...
class Utils {
	...
	public static function get_rule_values() {
		...
		return array_merge($values, array(
			...
			'shipping_cost_including_tax' => false,
			'shipping_cost_exclude_discount' => false,
			'shipping_cost_exclude_virtual_products' => false,
			'min_shipping_cost' => '',
			'max_shipping_cost' => '',
	...
	public static function get_all_conditions() {
		return apply_filters('advanced_rule_based_shipping/condition_types', array(
			...
			'cart:total_volume' => array(
				'group' => 'cart',
				'priority' => 25,
				'label' => __('Total volume', 'advanced-rule-based-shipping'),
			),
	...
	public static function get_shipping_cost_types() {
		...
		return array_merge($types, array(
			...
			'based_on_subtotal' => __('Based on subtotal', 'advanced-rule-based-shipping'),
		));
	}
	 ...
	 * @return float
	 */
	public static function get_volume_of_product($_product, $quantity = 1) {
	...
	public static function get_terms_total($taxonomy, $allow_terms = false) {
		...
		$exclude_virtual_products = get_query_var('advanced_rule_based_shipping_exclude_virtual_products', false);

		foreach (WC()->cart->get_cart() as $cart_key => $item) {
			if ($exclude_virtual_products && $item['data']->is_virtual()) {
				continue;
			}

			$cart_item_terms = wc_get_product_term_ids($item['product_id'], $taxonomy);
			foreach ($cart_item_terms as $term_id) {
				if (is_array($allow_terms) && !in_array($term_id, $allow_terms) || isset($term_totals[$cart_key])) {
					continue;
				}

				$term_totals[$cart_key] = self::get_cart_item_data($item);
			}
		}
	...
	public static function get_cart_item_data($cart_item) {
		...
		$cart_item_data = array(
			...
			'volume' => self::get_volume_of_product($product, $cart_item['quantity']),
		);

		$product_weight = $product->get_weight();
		if (is_numeric($product_weight)) {
			$cart_item_data['weight'] = $product_weight * $cart_item['quantity'];
		}
