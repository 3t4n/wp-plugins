...
class FrontendEnqueues {
    ...
    public function enqueue_styles() {
        ...
        wp_enqueue_style(
            $this->plugin_name,
            LPAC_DPS_PLUGIN_ASSETS_PATH_URL . 'public/css/lpac-dps-public.css',
            array(),
            $this->version,
            'all'
        );
        wp_enqueue_style(
            $this->plugin_name . '-flatpickr',
            LPAC_DPS_PLUGIN_ASSETS_PATH_URL . 'public/css/lib/flatpickr.min.css',
            array(),
            $this->version,
            'all'
        );
    ...
    public function enqueue_scripts() {
        ...
        wp_enqueue_script(
            $this->plugin_name,
            LPAC_DPS_PLUGIN_ASSETS_PATH_URL . 'public/js/' . $path . 'lpac-dps-public.js',
            array('jquery'),
            $this->version . $time,
            false
        );
        if ( is_checkout() && !is_wc_endpoint_url( 'order-received' ) ) {
            wp_enqueue_script(
                $this->plugin_name . '-flatpickr',
                LPAC_DPS_PLUGIN_ASSETS_PATH_URL . 'public/js/lib/flatpickr.min.js',
                array('jquery'),
                $this->version,
                false
            );
            wp_enqueue_script(
                $this->plugin_name . '-checkout-page-scripts',
                LPAC_DPS_PLUGIN_ASSETS_PATH_URL . 'public/js/' . $path . 'checkout/page.js',
                array('jquery', 'wp-util'),
                $this->version . $time,
                false
            );
            wp_enqueue_script(
                $this->plugin_name . '-checkout-page-flatpickr-scripts',
                LPAC_DPS_PLUGIN_ASSETS_PATH_URL . 'public/js/' . $path . 'checkout/flatpickr-related.js',
                array('jquery', 'wp-util'),
                $this->version . $time,
                false
            );
    ...
    public function getScriptsAsModules( string $tag, string $handle, string $src ) : string {
        if ( LPAC_DPS_DEBUG === false ) {
            // Live scripts are built in Parcel so no need to make them modules.
            return $tag;
        }
        $modules_handlers = array($this->plugin_name . '-locations');
