...
use Lpac_DPS\Models\PluginSettings\Misc;
...
class SetupFields extends BaseCheckoutPageController {
	...
	public function getJSConfig(): string {
		...
		$delivery_minimum_days_include_current_day = $delivery_settings->minimumDaysIncludeCurrentDay();

		$delivery_maximum_days_in_future           = $delivery_settings->get_maximum_days_in_future();
		$delivery_maximum_days_include_current_day = $delivery_settings->maximumDaysIncludeCurrentDay();
		...
		$pickup_minimum_days_include_current_day = $pickup_settings->minimumDaysIncludeCurrentDay();

		$pickup_maximum_days_in_future           = $pickup_settings->get_maximum_days_in_future();
		$pickup_maximum_days_include_current_day = $pickup_settings->maximumDaysIncludeCurrentDay();
		...
		$calendar_config = array(
			'delivery'       => array(
				...
				'minDaysIncludeCurrentDay' => $delivery_minimum_days_include_current_day,
				'maxDaysInFuture'          => $delivery_maximum_days_in_future,
				'maxDaysIncludeCurrentDay' => $delivery_maximum_days_include_current_day,
			...
			'pickup'         => array(
				...
				'minDaysIncludeCurrentDay' => $pickup_minimum_days_include_current_day,
				'maxDaysInFuture'          => $pickup_maximum_days_in_future,
				'maxDaysIncludeCurrentDay' => $pickup_maximum_days_include_current_day,
		...
		$general_config = array(
			'time' => array(
				...
				'currentTimestamp'   => FunctionsHelper::getCurrentTimestamp(),
				'currentMeridiem'    => FunctionsHelper::getCurrentTime( 'A' ),
