...
class SettingsSectionsContent {
    ...
    /**
     * Get the available tax classes created on the site.
     *
     * @return array
     * @since 1.3.0
     */
    private static function getAvailableTaxClasses() : array {
        $classes = \WC_Tax::get_tax_classes();
        $normalized = array(
            'Standard' => __( 'Standard', 'delivery-and-pickup-scheduling-for-woocommerce' ),
        );
    ...
    public static function general_settings__create_basic_fields() : array {
        return array(
            ...
            array(
                ...
                'type'        => 'select',
                'placeholder' => __( 'Select an option', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'options'     => FunctionsHelper::getDaysOfTheWeek(),
                'default'     => 'monday',
            )
        );
    }
    ...
    public static function scheduling__create_date_fields( string $order_type ) : array {
        return array(
            ...
            array(
                ...
                'options'    => FunctionsHelper::getDaysOfTheWeek(),
                'dependency' => array($order_type . '__enable_date_feature', '==', 'true'),
                'default'    => array_keys( FunctionsHelper::getDaysOfTheWeek() ),
            ),
            array(
            ...
            array(
                'id'         => $order_type . '__minimum_days_include_current_day',
                'type'       => 'switcher',
                'title'      => __( 'Inclusive of current day', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'subtitle'   => __( 'Whether to include the current day when calculating the minimum days.', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'text_on'    => __( 'Yes' ),
                'text_off'   => __( 'No' ),
                'default'    => true,
                'dependency' => array($order_type . '__enable_date_feature', '==', 'true'),
            ...
            array(
                ...
                'subtitle'   => __( 'Set the maximum number of days in the future (from the current day) that customers will be able to schedule an order. Enter 0 if you want customers to be able to place an order ONLY for the current day.', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'default'    => '120',
                'dependency' => array($order_type . '__enable_date_feature', '==', 'true'),
                'validate'   => 'csf_validate_numeric',
            ),
            array(
                'id'         => $order_type . '__maximum_days_include_current_day',
                'type'       => 'switcher',
                'title'      => __( 'Inclusive of current day', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'subtitle'   => __( 'Whether to include the current day when calculating the maximum days.', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'text_on'    => __( 'Yes' ),
                'text_off'   => __( 'No' ),
                'default'    => true,
                'dependency' => array($order_type . '__enable_date_feature', '==', 'true'),
    ...
    public static function scheduling__create_time_fields( string $order_type ) : array {
        ...
        $fields = array(
            ...
            array(
                'id'         => $order_type . '__time_slot_fees_taxable',
                'type'       => 'switcher',
                'title'      => __( 'Apply taxes to timeslot fees', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'subtitle'   => __( 'Turn on this option to apply taxes to the time slot fees.', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'text_on'    => __( 'Yes' ),
                'text_off'   => __( 'No' ),
                'dependency' => array(array($order_type . '__enable_time_feature', '==', 'true'), array($order_type . '__time_slot_fee', '==', 'true')),
            ),
            array(
                'id'         => $order_type . '__time_slot_fee_tax_class',
                'title'      => __( 'Tax class', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'subtitle'   => __( 'Select the tax class that should apply to the time slot fees.', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'type'       => 'select',
                'options'    => self::getAvailableTaxClasses(),
                'default'    => '',
                'dependency' => array(array($order_type . '__enable_time_feature', '==', 'true'), array($order_type . '__time_slot_fees_taxable', '==', 'true')),
            ),
            array(
                'id'         => $order_type . '__time_slot_fee_upsell',
                'type'       => 'submessage',
                'style'      => 'info',
                'content'    => sprintf( __( '%1$sAll fee types will be treated as "Standard flat fee" in the Free version of the plugin. Upgrade to unlock more powerful and dynamic checkout fee options.%2$s', 'delivery-and-pickup-scheduling-for-woocommerce' ), '<span style="font-size: 12px; line-height: 1.5">', '</span>' ) . '<br/><br/><a href="https://chwazidatetime.com/docs/delivery-pickup-scheduling/?utm_source=dps_settings&utm_medium=cta&utm_campaign=upsell#enable-time-slot-fees" rel="noreferrer" target="_blank" style="font-size: 12px">' . $learn_more . '&nbsp<i class="fas fa-external-link-alt"></i></a>',
                'dependency' => array(array($order_type . '__enable_time_feature', '==', 'true'), array($order_type . '__time_slot_fee', '==', 'true')),
            ),
            array(
                ...
                'fields'     => array(array(
                    ...
                    'options' => FunctionsHelper::getNormalizedDaysOfTheWeek( $order_type ),
                ), array(
                    'id'       => 'time_slots',
                    'type'     => 'repeater',
    ...
    public static function googleCalendarUpsellFields() : array {
        return array(array(
            'type'    => 'content',
            'content' => '<img src="' . esc_attr( LPAC_DPS_PLUGIN_ASSETS_PATH_URL . 'admin/img/google-calendar-addon.png' ) . '" width="100%"/>',
        ), array(
            'type'    => 'submessage',
            'style'   => 'info',
            'content' => sprintf(
    ...
    public static function localization__create_calendar_fields() : array {
        return array(array(
            ...
            'accordions' => array(
                 ...
                 *
                 * @see \Lpac_DPS\Models\PluginSettings::get_weekdays();
    ...
    public static function localization__create_checkout_fields() : array {
        $fields = array(
            ...
            array(
                'id'      => 'localization__checkout_no_available_time_slots_text',
                'type'    => 'text',
                'title'   => esc_html__( 'No available time slots text', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'default' => esc_html__( 'No available time slots', 'delivery-and-pickup-scheduling-for-woocommerce' ),
            ),
            array(
                'id'      => 'localization__checkout_please_choose_an_option_text',
                'type'    => 'text',
                'title'   => esc_html__( 'Please choose an option text', 'delivery-and-pickup-scheduling-for-woocommerce' ),
                'default' => esc_html__( 'Please choose an option', 'delivery-and-pickup-scheduling-for-woocommerce' ),
            )
        );
        return $fields;
    }
