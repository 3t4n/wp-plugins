...
use SoaringLeads\Chwazi\Carbon\Exceptions\InvalidFormatException;
use SoaringLeads\Chwazi\Carbon\Exceptions\InvalidTimeZoneException;
use DateTime;
...
class UtilitiesHelper {
	 ...
	 * @since 1.2.4
	 */
	public static function convertTo24HourFormat( string $time, string $return_format = 'H:i:s' ): string {
		return Carbon::parse( $time, FunctionsHelper::getTimezone() )->format( $return_format );
	}
	...
	public static function convertDateTimeToUTC( string $date_time ): ?string {
		return Carbon::createFromFormat( 'Y-m-d H:i:s', $date_time, FunctionsHelper::getTimezone() )->setTimezone( 'UTC' )->format( 'Y-m-d H:i:s' );
	}
	 ...
	 * @since 1.2.0
	 */
	public static function addMinutesToCurrentDateTime( int $minutes, string $return_format = 'Y-m-d H:i:s' ): string {
		$local_time = Carbon::now( FunctionsHelper::getTimezone() );
		return $local_time->addMinutes( $minutes )->format( $return_format );
	}
	...
	public static function getUTCFromTime( string $time, string $return_format = 'H:i:s' ): string {
		$utc_time = Carbon::createFromTimeString( $time, FunctionsHelper::getTimezone() )->setTimezone( 'UTC' );
	...
	public static function normalizePostTimeslot( string $time ): array {
		...
		// "+" signs are added automatically by WooCommerce/the Browser into the $posted_data selected time data in place of spaces.
		// Below we're removing the "+" signs so this can properly be normalized.
		...
		if ( FunctionsHelper::using24hrTime() ) {
			...
			$from_am_pm      = substr( $from, -2 );
			$from_minutes    = substr( $from, -4, 2 );
			$from_hours      = str_replace( $from_minutes . $from_am_pm, '', $from );
			$from_normalized = $from_hours . ':' . $from_minutes . ' ' . $from_am_pm;

			if ( ! empty( $to ) ) {
				$to_am_pm      = substr( $to, -2 );
				$to_minutes    = substr( $to, -4, 2 );
				$to_hours      = str_replace( $to_minutes . $to_am_pm, '', $to );
				$to_normalized = $to_hours . ':' . $to_minutes . ' ' . $to_am_pm;
			}
		}
