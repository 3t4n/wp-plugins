...
class EPWC_Gateway_Easypay_Checkout extends WC_Payment_Gateway

{
...

    {
        ...
        $settings_url = esc_url(get_admin_url().'admin.php?page=wc-settings&amp;tab=checkout&amp;section=wc_payment_gateway_easypay_checkout');
        ...
        $this->id = 'easypay_checkout';



        $this->checkout_easypay_logo = $this->get_option('checkout_easypay_logo');



        $this->pg_icons  = plugin_dir_url(__DIR__).'images/logos/pg-icons.png';



        if (! empty(get_option('easypay_logo'))) {

            $this->icon = get_option('easypay_logo');

        }else{

            $this->icon = plugin_dir_url(__DIR__).'images/logos/pg-icons.png';
        ...
        $this->method_title = esc_html__('easypay Checkout', 'easypay-checkout-for-woocommerce');

        $this->method_description = esc_html__('Pay with MB Way, Ref. Multibanco, Visa & Mastercard Cards, Apple Pay, Santander Consumer Finance', 'easypay-checkout-for-woocommerce');
...

    {
        ...
        $this->form_fields = [
            ...
            'enabled' => [
                ...
                'label' => esc_html__('Enable easypay Payment Gateway.', 'easypay-checkout-for-woocommerce'),
            ...
            'title' => [
                ...
                'default' => esc_html__('easypay Checkout', 'easypay-checkout-for-woocommerce'),
            ...
            'description' => [
                ...
                'default' => esc_html__('Pay with MB Way, Ref. Multibanco, Visa & Mastercard Cards, Apple Pay, Santander Consumer Finance', 'easypay-checkout-for-woocommerce'),
...

    {
        ...
        ?>
            ...
            <h3><?php echo esc_html('easypay standard Checkout', 'easypay-checkout-for-woocommerce'); ?></h3>

            <p><?php echo esc_html('easypay standard checkout works by addressing the user to easypay´s gateway to enter their payment information.', 'easypay-checkout-for-woocommerce'); ?></p>
...

    {
        ...
        return '<div id="easypay-checkout"></div>';
