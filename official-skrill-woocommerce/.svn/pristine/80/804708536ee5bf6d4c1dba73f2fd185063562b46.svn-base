...
class Skrill_Settings {
	...
	public static function validate_skrill_setting() {
		if ( Skrill_Payment::get_request_value( 'save' ) ) {
			$merchant_email = Skrill_Payment::get_request_value( 'skrill_merchant_account' );
			$api_pass = Skrill_Payment::get_request_value( 'skrill_api_passwd' );
			$secret_word = Skrill_Payment::get_request_value( 'skrill_secret_word' );

			if ( $api_pass ) {
				if ( $api_pass !== get_option( 'skrill_api_passwd' ) ) {
					$api_pass = md5( $api_pass );
					$_POST['skrill_api_passwd'] = $api_pass; // input var okay.
				}
			}
			
			if ( $secret_word ) {
				if ( $secret_word !== get_option( 'skrill_secret_word' ) ) {
					$secret_word = md5( $secret_word );
					$_POST['skrill_secret_word'] = $secret_word; // input var okay.
				}
			}
			...
			if ( ! $is_mandatory_field ) {
				...
				$redirect_error = add_query_arg( 'wc_error', rawurlencode( esc_attr( $error_message ) ), $redirect );
				wp_safe_redirect( $redirect_error );
				exit();
			}
			
			$is_merchant_email_valid = Skrill_Payment::is_merchant_email_valid( $merchant_email );

			if ( ! $is_merchant_email_valid ) {
				$is_valid = false;
				$error_message  = __( 'Failed to update the configuration. Merchant Email is not valid.', 'wc-skrill' );
				$redirect_error = add_query_arg( 'wc_error', rawurlencode( esc_attr( $error_message ) ), $redirect );
				wp_safe_redirect( $redirect_error );
				exit();
			}

			$is_api_password_valid = Skrill_Payment::is_api_password_valid( $merchant_email, $api_pass );

			if ( ! $is_api_password_valid ) {
				$is_valid = false;
				$error_message  = __( 'Failed to update the configuration. API Password is not valid.', 'wc-skrill' );
				$redirect_error = add_query_arg( 'wc_error', rawurlencode( esc_attr( $error_message ) ), $redirect );
				wp_safe_redirect( $redirect_error );
				exit();
			}

			$is_api_ipaddress_valid = Skrill_Payment::is_api_ipaddress_valid( $merchant_email, $api_pass );

			if ( ! $is_api_ipaddress_valid ) {
				$is_valid = false;
				$error_message  = __( 'Failed to update the configuration. Please make sure the app IP address is whitelisted in Automated Payment Interface (API).', 'wc-skrill' );
				$redirect_error = add_query_arg( 'wc_error', rawurlencode( esc_attr( $error_message ) ), $redirect );
				wp_safe_redirect( $redirect_error );
				exit();
			}

			$is_mqi_ipaddress_valid = Skrill_Payment::is_mqi_ipaddress_valid( $merchant_email, $api_pass );

			if ( ! $is_mqi_ipaddress_valid ) {
				$is_valid = false;
				$error_message  = __( 'Failed to update the configuration. Please make sure the app IP address is whitelisted in Merchant Query Interface (MQI).', 'wc-skrill' );
				$redirect_error = add_query_arg( 'wc_error', rawurlencode( esc_attr( $error_message ) ), $redirect );
				wp_safe_redirect( $redirect_error );
				exit();
			}
			...
			if ( $is_valid ) {
				...
				$redirect_success = add_query_arg( 'wc_message', rawurlencode( esc_attr( $message ) ), $redirect );
