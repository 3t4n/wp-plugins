...
class Skrill_Payment {
	...
	/**
	 * Skrill check merchant email
	 *
	 * @param  string $merchant_email - merchant email.
	 * @return boolean
	 */
    public static function is_merchant_email_valid( $merchant_email ) {
        $test_parameters["pay_to_email"] = $merchant_email;
        $test_parameters["amount"] = "10";
        $test_parameters["prepare_only"] = "1";

		$url         = self::$prepare_url;
		$post_result = self::post( $url, $test_parameters );

		if ( is_string( $post_result ) && 32 == strlen( $post_result ) ) {
            return true;
        }
	...
	/**
	 * Skrill check API passwword
	 *
	 * @param  string $merchant_email - merchant email.
	 * @param  string $api_password   - api password.
	 * @return boolean
	 */
    public static function is_api_password_valid( $merchant_email, $api_password )
    {
        $test_parameters["email"] = $merchant_email;
        $test_parameters['password'] = strtolower( $api_password );
        $test_parameters["action"] = "status_trn";
        $test_parameters["trn_id"] = "test";

		$query_url   = self::$query_url;
		$post_result = self::post( $query_url, $test_parameters );

        if ( false !== strpos( $post_result, "Cannot login: The email or password" ) ) {
            return false;
        }
	...
	/**
	 * Skrill check MQI IP Address whitelist
	 *
	 * @param  string $merchant_email - merchant email.
	 * @param  string $api_password   - api password.
	 * @return boolean
	 */
    public static function is_mqi_ipaddress_valid( $merchant_email, $api_password ) {
        $test_parameters["email"] = $merchant_email;
        $test_parameters['password'] = strtolower( $api_password );
        $test_parameters["action"] = "status_trn";
        $test_parameters["trn_id"] = "test";
        ...
        if ( false !== strpos( $post_result, "Cannot login: remote ip" ) ) {
            return false;
        }
	...
	/**
	 * Skrill check API IP Address whitelist
	 *
	 * @param  string $merchant_email - merchant email.
	 * @param  string $api_password   - api password.
	 * @return boolean
	 */
    public static function is_api_ipaddress_valid( $merchant_email, $api_password ) {
        $test_parameters["email"] = $merchant_email;
        $test_parameters['password'] = strtolower( $api_password );
		$test_parameters['action'] = "prepare";
        $test_parameters["amount"] = "1";
        $test_parameters["transaction_id"] = "test";

		$refund_url  = self::$refund_url;
		$post_result = self::post( $refund_url, $test_parameters );
		$xml_result  = simplexml_load_string( $post_result );

        if ( 'CANNOT_LOGIN' == (string) $xml_result->error->error_msg ) {
            return false;
        }
	...
	public static function get_request_value( $key, $default = false ) {
		if ( isset( $_POST['save'] ) && isset( $_POST['_wpnonce'] ) && isset( $_GET['page'] ) ) { // input var okay.
			// disable wp_verify_nonce on 03 May 2024 since this function still got bug 
