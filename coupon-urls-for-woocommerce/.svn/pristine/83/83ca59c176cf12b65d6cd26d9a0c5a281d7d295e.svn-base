...
class ActionsRegistratorForCurrentURL implements Subscriber, OptionalRegisterableSubscriber
{
    ...
    public static function canBeRegistered() : Validator
    {
        return new Validators([new ValidWhen(defined('REST_REQUEST') ? !REST_REQUEST : true), new ValidWhen(wc()->cart instanceof WC_Cart)]);
    }
