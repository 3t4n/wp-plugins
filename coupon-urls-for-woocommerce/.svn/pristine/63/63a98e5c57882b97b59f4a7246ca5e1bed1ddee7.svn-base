...
abstract class Types implements JsonSerializable
{
    ...
    protected $meta = ['_allowed' => [], '_default' => null];
    public abstract function hasDefaultValue();
    public abstract function getTypeAsString() : string;
    protected abstract function setType();
    protected abstract function concretePickValue($newValue);
    protected abstract function isCorrectType($value);
    ...
    {
        return ['meta' => array_merge($this->meta, ['_dataType' => $this->getTypeAsString(), '_allowed' => $this->allowedValues, '_default' => $this->defaultValue])];
    }
    protected function throwExceptionIfTypeIsInvalid($type)
    {
        switch ($type) {
            ...
            case static::BOOLEAN:
                ...
                break;
            default:
                throw new \Exception(\esc_html("Invalid type {$type}"));
                break;
        }
    }
    public static function castToExpectedType($value, $beforeResotringToNull = null)
    {
        throw new \Exception(\esc_html("cannot call abstract method castToExpectedType"));
    }
}