...
function editor_panels( $panels ) {
	...
	$editor_panel = function () use ( $prop, $service, $contact_form ) {
		...
		$script_data = array(
			...
			'app_id_selected' => esc_js( $app_id_selected ),			
		);
				...
				<tbody>
						...
						<tr>
							...
							<td>
								...
								<select name="wpc-wpcf7-airtable[app_id_selected]" id="wpc-wpcf7-airtable-app-id-selected" onchange="fetchAirtableTables(this.value)">
									<option value="-1" <?= $app_id_selected == '-1' || empty($app_id_selected) ? 'selected' : '' ; ?>><?php echo esc_html__( 'Choose a base', 'add-on-cf7-for-airtable' ); ?></option>
									<?php foreach ( $bases as $base_id => $base_name ) : ?>
										<option value="<?php echo esc_attr( $base_id ); ?>"
											<?php selected( $base_id, $app_id_selected ); ?>>
					...
					if ( strpos( $api_key, 'key' ) === 0 ) {
						...
						<tr>
							...
							<td>
								<?php $tables = Helpers\get_airtable_tables_token( $app_id_selected ); ?>
								<select name="wpc-wpcf7-airtable[table_selected]" id="wpc-wpcf7-airtable-table-id-selected">
									<option value="-1" <?= $table_selected == '-1' || empty($app_id_selected) ? 'selected' : '' ; ?>><?php echo esc_html__( 'Choose a table', 'add-on-cf7-for-airtable' ); ?></option>
									<?php foreach( $tables as $table_id => $table_name ): ?>
										<option value="<?php echo esc_attr( $table_id ); ?>"
											<?php selected( $table_id, $table_selected ); ?>>
											<?php echo esc_html( $table_name['name'] ); ?>
										</option>
									<?php endforeach; ?>
								</select>
							</td>
									...
									<td>
										...
										if ( $has_available_mapping ) {
											...
											foreach ( $options as $option_value => $option_text ) {
												printf( '<option value="%s" %s>%s</option>', esc_attr( $option_value ), (!empty($selected_column_type) ? ($selected_column_type === $option_value ? 'selected="selected"' : '') : (!$i ? 'selected="selected"' : '' )), esc_html( $option_text ) );
												++$i;
											}
											printf( '</select>' );
										}
