...
function wp_hotfix_init() {
	...
	switch ( $wp_version ) {
		case '6.7.1' :
			$hotfixes = array( '671_category_checkboxes' );
			break;
		case '5.1' :
...
function wp_hotfix_671_category_checkboxes() {
	if ( is_admin() ) {
		add_filter( 'script_loader_src', 'wp_hotfix_671_replace_post_js', 11, 2 );
	}
}

function wp_hotfix_671_replace_post_js( $src, $handle ) {
	if ( 'post' === $handle && is_string( $src ) && false === strpos( $src, 'ver=62504-20241121' ) ) {
		$suffix = wp_scripts_get_suffix();
		$src    = plugins_url( 'js/', __FILE__ ) . "post{$suffix}.js";
