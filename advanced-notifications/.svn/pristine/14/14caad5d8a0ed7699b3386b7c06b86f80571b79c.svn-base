...
function is_eis_option_hidden($current_interface_id, $current_input_id) {
	$current_input_id = str_replace('_eis_html', '', $current_input_id);
	global $post;
	$post_id = (isset($post->ID)) ? $post->ID : null;
	$return = false;
	$current_input_register_info = get_register_eis_interfaces_options($current_interface_id, $current_input_id);
	$current_input_parent_name = (isset($current_input_register_info['parent_name'])) ? $current_input_register_info['parent_name'] : false;
	if ($current_input_parent_name) {
		if (is_eis_option_hidden($current_interface_id, $current_input_parent_name)) {
			$return = true;
		} else {
			$current_input_show_on_value = (isset($current_input_register_info['show_on_value'])) ? array_map('trim',explode(",",$current_input_register_info['show_on_value'])) : false;
			$current_input_hide_on_value = (isset($current_input_register_info['hide_on_value'])) ? array_map('trim',explode(",",$current_input_register_info['hide_on_value'])) : false;
			if ($current_input_show_on_value) {
				...
				if ($current_input_parent_name_val && !in_array($current_input_parent_name_val, $current_input_show_on_value)) {
					$return = true;
			...
			} elseif ($current_input_hide_on_value) {
				$current_input_parent_name_info = get_register_eis_interfaces_options($current_interface_id, $current_input_parent_name);
				$current_input_parent_name_val = eis_get_option($current_interface_id, $current_input_parent_name, $post_id);
				$current_input_parent_name_val = str_replace(' ', '_', $current_input_parent_name_val);
...
function eis_meta_box_interface($post_info, $meta_box_info) {
	...
	<div class="eis-form" data-form="<?php echo esc_attr($post_type); ?>">
		...
		<div class="eis-interface-settings eis-meta-box<?php echo (!$interface['show_tabs'] ? ' no-tabs' : null) ?>" data-ajax-url="<?php echo admin_url('admin-ajax.php'); ?>" data-nonce="<?php echo wp_create_nonce('eis_interface_nonce'); ?>">
			<?php
			$file_path = eis_get_theme_file_path();
			if (file_exists($file_path)) {
				include_once $file_path;
...
function eis_save_current_tab() {
	$interface_id = sanitize_text_field($_POST['interface_id']);
	$interface = get_eis_interfaces($interface_id);
	$capability = ( isset($interface['capability']) && $interface['capability'] ) ? $interface['capability'] : 'manage_options';
	if (!current_user_can($capability) || !isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'eis_interface_nonce')) {
		exit;
	}
...
add_action( 'wp_ajax_eis_save_current_tab', 'eis_save_current_tab' );
