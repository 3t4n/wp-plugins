...
class DB_Manager {
	 ...
	 * @var int
	 */
	protected int $db_version = 280; // Increment with each change in db
	...
	public function migrate() {
		...
		if ( $current_db_version < 280 ) {
			$this->upgrade_280( $current_db_version );
		}
	...
	/**
	 * Executes changes made in 2.8.0.
	 *
	 * @since 2.8.0
	 *
	 * @param int $db_version The old (current) database version.
	 ...
	 * @return void
	 */
	private function upgrade_280( $db_version ) {

		if ( $db_version < 280 ) {

			// Delete old options in case they still exist
