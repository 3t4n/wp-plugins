...
class PublicFacing {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style(
			$handle,
			$this->plugin->plugin_url( "public/css/pwp{$suffix}.css" ),
			array(),
			$version
		);
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script(
			$handle,
			$this->plugin->plugin_url( 'public/js/pwp.js' ),
		...
		wp_localize_script(
			...
			array(
				...
				'delayInit' => (bool) $this->plugin->get_setting( 'performance_delay_initialisation' ),
			)
		);

		if ( ! empty( $this->plugin->get_setting( 'performance_legacy_blocks' ) ) ) {
			// Support legacy blocks with hardcoded dependancy on main script.
			$properties = require $this->plugin->plugin_path( 'public/js/wp-dxp.asset.php' );
			$js_version = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? $properties['version'] : $this->plugin->get_version();
			wp_enqueue_script(
				"{$handle}-legacy",
				$this->plugin->plugin_url( 'public/js/wp-dxp.js' ),
				array_merge( [ $handle ], $properties['dependencies'] ),
				$js_version,
				[ 'strategy' => 'defer' ]
			);
		}
	...
	public function register_endpoints() {
		...
		if ( ! empty( $this->plugin->get_setting( 'performance_legacy_blocks' ) ) ) {
			// Opt-in support legacy <wp-dxp> route with slightly different args.
			register_rest_route(
				"{$this->namespace}v1",
				'/blocks',
				[
	...
	protected function get_user_data_args() {
		...
		return [
			...
			'deviceType'         => [
				...
				'items'    => array(
					'type' => 'string',
					'enum' => array( 'mobile', 'tablet', 'desktop', 'windows', 'android', 'ios', 'macos', 'linux', '' ),
				),
	 ...
	 */
	protected function find_block( $ref, $blocks ) {
	 ...
	 * }
	 */
	protected function get_template( $ref ) {
		global $post;
