...
class Admin {
	...
	public function enqueue_styles() {
		...
		if ( $this->is_admin_page() ) {
			wp_enqueue_style(
				$this->personalizewp . '_jquery_ui',
				plugin_dir_url( __FILE__ ) . 'libs/css/jquery-ui.css',
				array(),
				'1.5'
			); // For datepicker etc
			...
			wp_enqueue_style(
				$this->personalizewp,
				plugin_dir_url( __FILE__ ) . "css/admin{$suffix}.css",
				array(),
				$version
			);
	...
	public function enqueue_editor_styles() {
		...
		wp_enqueue_style(
			$this->personalizewp . '-editor-styles',
			plugin_dir_url( __FILE__ ) . "css/editor{$suffix}.css",
			array(),
			$version
		);
	}
	...
	/**
	 * Enqueue block specific editor styles (contextual indicators, etc.)
	 *
	 * @since 2.8.0
	 */
	public function enqueue_block_editor_styles() {
		...
		wp_enqueue_style(
			$this->personalizewp . '-contextual-indicator-styles',
			plugin_dir_url( __FILE__ ) . "css/contextual{$suffix}.css",
			array(),
			$version
		);
	}
	...
	public function create_admin_menus() {
		...
		add_submenu_page(
			$parent_slug,
			esc_html__( 'Rules', 'personalizewp' ),
			esc_html__( 'Rules', 'personalizewp' ),
			'manage_options',
		 ...
		 *
		 * @since 1.0.0
		 * @deprecated 2.8.0 Use {@see 'personalizewp_menus_before_settings'} instead.
		 *
		 * @param string $parent_slug Slug of the main Personalize menu item
		 */
		do_action_deprecated( 'personalizewp_menus_after', [ $parent_slug ], '2.8.0', 'personalizewp_menus_before_settings' );

		/**
		 * Fires after most Personalize sub-menus are added to the menu bar, but before Settings and KB.
		 *
		 * @since 2.8.0
		 *
		 * @param string $parent_slug Slug of the main Personalize menu item
		 */
		do_action( 'personalizewp_menus_before_settings', $parent_slug );
		...
		/**
		 * Fires after Settings sub-menu has been added to the menu bar, but before KB.
		 *
		 * @since 2.8.0
		 *
		 * @param string $parent_slug Slug of the main Personalize menu item
		 */
		do_action( 'personalizewp_menus_after_settings', $parent_slug );
	 ...
	 *
	 * @since 2.7.0 Changed function visibility
	 *
	 * @return boolean
	 */
