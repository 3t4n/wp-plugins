...
class MoMo_RssFeed_Cron {
	...
	public function momo_acg_autoblog_content_writer( $tags, $category, $status, $no_of_posts, $time_basis, $writing_style, $add_image, $gen_title = 'off', $no_of_para = 5 ) {
		...
		$ptype               = $status;
		/* translators: %s: tags */
		$title = sprintf( esc_html__( 'a blog title with following tags ( %s ) ', 'momoacg' ), $tags, $writing_style );
	...
	public function momo_acg_single_content_writer( $title, $noofpara, $ptype, $other, $tags = '', $wstyle = '' ) {
		...
		/* translators: %1$s: tags, %2$s: writing style, %3$s: number of paragraph */
		$title = sprintf( esc_html__( 'Write a blog post with following tags ( %1$s ) in a %2$s writing style with %3$s number of paragraph(s).', 'momoacg' ), $tags, $wstyle, $noofpara );
		if ( 'on' === $other['gen_title'] ) {
			$title .= esc_html__( ' Please provide a Title (# Title: ) for that entire blog post at beginning of this post.', 'momoacg' );
		}
		$prompt    = $title;
		$new_msg   = array(
			'role'    => 'user',
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( isset( $response['status'] ) && 200 === $response['status'] ) {
				...
				foreach ( $choices as $choice ) {
					if ( 'gpt-3.5-turbo' === $model || 'gpt-4' === $model || 'gpt-3.5-turbo-0613' === $model ) {
		...
		if ( isset( $content_title['content'] ) && ! empty( $content_title['content'] ) ) {
			...
			$data = array(
				'post_title'   => ! empty( $content_title['title'] ) && 'on' === $other['gen_title'] ? $content_title['title'] : $tags,
