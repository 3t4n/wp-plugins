...
class MoMo_ACG_Admin_Init {
	...
	public function momoacg_print_admin_ss() {
		...
		wp_register_script( 'momoacg_admin_script', $momoacg->plugin_url . 'assets/js/momo_acg_admin.js', array( 'jquery', 'wp-tinymce' ), $momoacg->version, true );
		...
		$ajaxurl = array(
			...
			'saving_content'              => esc_html__( 'Saving Content..', 'momoacg' ),
			...
			'create_ft_popbox'            => esc_html__( 'Create new Fine Tune Model', 'momoacg' ),
			'empty_title'                 => esc_html__( 'Empty title field. Please enter title.', 'momoacg' ),
		);
		$ajaxurl = apply_filters( 'momo_acg_add_data_to_admin_locale', $ajaxurl );
