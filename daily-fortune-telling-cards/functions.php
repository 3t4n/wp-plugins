...
function power_daily_cards_plugin($atts) {
$frm_cont = $credits = '';
...
$abslt_url = "https://www.powerfortunes.com/cron/powerfortunes_fortunetellingcards_daily.json";
$ch = curl_init();
...
$json_dl=curl_exec($ch);
$status_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
curl_close($ch);
if($status_code == 200) {
$arr_dl = json_decode($json_dl, true);
} else {
$arr_dl = array();
}
?>

<!-- Start of Code for daily Zodiac Signs -->
<div class="grid-container">
<?php
if(empty($arr_dl)){
$frm_cont = '<div class="wrap-fit bord_cardhd"><div class="box_title card_hd_spc">Daily Fortune Telling Cards are presently unavailable.</div><!-- box_title --></div><!-- wrap-fit -->';
}//if
else {
$k = 0;
foreach($multi as $sn){
...
$frm_cont .= '<div class="wrap-fit bord_cardhd"><div class="box_title card_hd_spc">Daily Fortune Telling Cards for '.$sn.'</div><!-- box_title --></div><!-- wrap-fit -->';
for($i=0; $i<=3; $i++){
$frm_cont .='<img src="'.$arr_dl[strtoupper($sn)][$i].'"'.$lzy.' alt="'.$sn.', the fortune telling card for '.$sn.' for this day." class="borderRnd flx_card">';
}// for
$k++;
}// foreach

if($allowinfo==='yes' && !empty($arr_dl)){
...
<!-- End of Code for daily Zodiac Signs -->
...
// shortcode
add_shortcode( 'fortune_telling_cards', 'power_daily_cards_plugin' );

/* ------------- */
/* Plugin Deactivation */
/* ------------- */
function daily_cards_deactive() {
