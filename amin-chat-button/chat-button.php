        ...
        <form method="post" action="">
            ...
            <table class="cbp-form-table">
                ...
                <tr>
                    <th scope="row"><?php echo esc_html__('First message text and phone number. ', 'amin-chat-button'); ?><br><?php echo esc_html__('Other localization', 'amin-chat-button'); ?></th>
                    <td>
                        <input type="text" id="amin_chat_button_plugin_text_tr_loc" name="amin_chat_button_plugin_text_tr_loc" value="<?php echo esc_attr($text_tr_loc); ?>" oninput="this.value = this.value.toLowerCase()"><p class="cbp-description"><?php echo esc_html__('Localization code, for example "tr"', 'amin-chat-button'); ?></p><br>
...
function amin_chat_button_plugin_add_button() {
        ...
        // Проверка на существование $_SERVER['HTTP_HOST'] и $_SERVER['REQUEST_URI']
        if (isset($_SERVER['HTTP_HOST'], $_SERVER['REQUEST_URI'])) {
            $http_host = sanitize_text_field(wp_unslash($_SERVER['HTTP_HOST']));
            $request_uri = sanitize_text_field(wp_unslash($_SERVER['REQUEST_URI']));
            $current_url = 'https://' . $http_host . $request_uri;

            if (!empty($prohibited_urls)) {
                $prohibited_urls_array = array_map('trim', explode(',', $prohibited_urls));
                foreach ($prohibited_urls_array as $prohibited_url) {
                    if (stripos($current_url, $prohibited_url) !== false) {
                        return; // Не отображаем кнопку, если текущий URL запрещен
                    }
                }
            }
        } else {
            // Если данные отсутствуют, $current_url остается пустым
            $current_url = '';
        }

        // Переменная для хранения ссылки. Значение по умолчанию устанавливается сразу
        $link = 'https://api.whatsapp.com/send?phone=' . rawurlencode($phone) . '&text=' . rawurlencode($text);

        if ($msg_select == 'WhatsApp') {
            // Проверка на существование $_SERVER['HTTP_HOST'] и $_SERVER['REQUEST_URI']
            if (isset($_SERVER['HTTP_HOST'], $_SERVER['REQUEST_URI'])) {
                $http_host = sanitize_text_field(wp_unslash($_SERVER['HTTP_HOST']));
                $request_uri = sanitize_text_field(wp_unslash($_SERVER['REQUEST_URI']));
                ...
                // Расширенное регулярное выражение для поиска локализации
                preg_match('/\/([a-z]{2})(\/|$)/i', $url, $matches);

                // Если найден код локализации
                if (!empty($matches[1])) {
                    $locale = strtolower($matches[1]); // Приведение к нижнему регистру для универсальности

                    // Определение ссылки на основе локализации
                    switch ($locale) {
                        case "en":
                            $phone = (trim($phone_en)) ? $phone_en : $phone;
                            $text = (trim($text_en)) ? $text_en : $text;
                            break;
                        case "tr":
                            $phone = (trim($phone_tr)) ? $phone_tr : $phone;
                            $text = (trim($text_tr)) ? $text_tr : $text;
                            break;
                        default:
                            break;
                    }
                ...
                // Если данные $_SERVER отсутствуют
                $link = '';
            }
        }

        if ($msg_select == 'Telegram') {
        ...
        if ($msg_select == 'WhatsApp') {
        echo '" id="cbp-whatsapp-button" class="cbp-whatsapp-button ' . esc_attr($position) . '">';
        ...
        if ($msg_select == 'Telegram') {
        echo '" id="cbp-tg-button" class="cbp-tg-button ' . esc_attr($position) . '">';
        ...
        echo '</a>'; // Завершение HTML кода кнопки
}
