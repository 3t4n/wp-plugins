...
if ( !class_exists('FG_Magento_to_WooCommerce_Admin', false) ) {
	...
	class FG_Magento_to_WooCommerce_Admin extends WP_Importer {
		...
		public $entity_id_field = 'entity_id';	// entity_id | row_id (Magento EE)
		public $page_id_field = 'page_id';		// page_id | row_id (Magento EE)
		...
		public function enqueue_scripts() {
			...
			wp_localize_script( $this->plugin_name, 'objectL10n', array(
				...
				'update_completed' => __( 'UPDATE COMPLETED', 'fg-magento-to-woocommerce' ),
		...
		public function ajax_importer() {
			...
			if ( !empty($current_user) && $current_user->has_cap('import') ) {
				...
				if ( $action == 'update_wordpress_info') {
					...
					if ( (($action == 'empty') && check_admin_referer('empty', 'fgm2wc_nonce_empty'))
					  || (($action == 'import') && filter_input(INPUT_POST, 'automatic_empty', FILTER_VALIDATE_BOOLEAN) && check_admin_referer( 'parameters_form', 'fgm2wc_nonce')) ) {
						$this->empty_log_file();
					}
		...
		public function dispatch($action) {
			...
			$this->plugin_options = array(
				...
				'first_image_in_gallery'		=> 0,
				'thumbnail_in_gallery'			=> 0,
				'timeout'						=> 20,
				'price'							=> 'without_tax',
				'sale_price'					=> 'special',
			...
			elseif ( !empty($action) ) {
				switch($action) {
					...
					// Delete content
					case 'empty':
						if ( defined('WP_CLI') || check_admin_referer( 'empty', 'fgm2wc_nonce_empty' ) ) { // Security check
		...
		private function display_admin_page() {
			...
			$data['description'] = __('This plugin will import product categories, products, images and CMS from Magento to WooCommerce.<br />Compatible with Magento versions 1.3 to 2.4.', 'fg-magento-to-woocommerce');
		...
		public function set_store_id() {
			$this->store_id = $this->get_default_store_id();
		}
		
		/**
		 * Get the default Magento store ID
		 * 
		 * @since 3.39.0
		 * 
		 * @return int Store ID
		 */
		private function get_default_store_id() {
			$store_id = 0;
			$prefix = $this->plugin_options['prefix'];
...
SQL;
			...
			update_option('fgm2wc_last_category_id', 0);
		...
		private function save_wp_posts() {
			...
			update_option('fgm2wc_save_posts', $posts);
		}
		...
		private function save_wp_terms() {
			...
			update_option('fgm2wc_save_terms', $terms);
		}
		...
		private function save_wp_term_relationships() {
			...
			update_option('fgm2wc_save_term_relationships', $term_relationships);
		}
		...
		private function restore_wp_posts() {
			global $wpdb;
			$posts = get_option('fgm2wc_save_posts');
			foreach ( $posts as $post ) {
				$postmetas = $post['meta'];
				unset($post['meta']);
		...
		private function restore_wp_terms() {
			global $wpdb;
			$terms = get_option('fgm2wc_save_terms');
			foreach ( $terms as $term ) {
		...
		private function restore_wp_term_relationships() {
			global $wpdb;
			$term_relationships = get_option('fgm2wc_save_term_relationships');
...
SQL;
		...
		private function get_products_count() {
			$prefix = $this->plugin_options['prefix'];
			$website_criteria = '';
			if ( ($this->website_id != 'all') && $this->table_exists('catalog_product_website') ) {
			...
			$sql = "
				...
				INNER JOIN {$prefix}catalog_product_entity_int pei ON pei.{$this->entity_id_field} = p.{$this->entity_id_field}
		...
		private function get_cms_count() {
			...
			$sql = "
				...
				INNER JOIN {$prefix}cms_page_store s ON s.{$this->page_id_field} = a.page_id AND s.store_id IN (0, {$this->store_id})
			";
		...
		private function validate_form_info() {
			...
			return array(
				...
				'first_image_in_gallery'		=> filter_input(INPUT_POST, 'first_image_in_gallery', FILTER_VALIDATE_BOOLEAN),
				'thumbnail_in_gallery'			=> filter_input(INPUT_POST, 'thumbnail_in_gallery', FILTER_VALIDATE_BOOLEAN),
				'timeout'						=> filter_input(INPUT_POST, 'timeout', FILTER_SANITIZE_NUMBER_INT),
				'price'							=> filter_input(INPUT_POST, 'price', FILTER_SANITIZE_SPECIAL_CHARS),
				'sale_price'					=> filter_input(INPUT_POST, 'sale_price', FILTER_SANITIZE_SPECIAL_CHARS),
		...
		private function import() {
			if ( $this->magento_connect() ) {
				...
				$this->imported_media = $this->get_imported_magento_posts($meta_key = '_fgm2wc_old_file');
				
				// Hook for doing other actions before the pre import
				do_action('fgm2wc_pre_pre_import');
		...
		protected function create_woocommerce_product_types() {
			...
			$product_types = apply_filters('fgm2wc_product_types', $product_types);
		...
		private function get_default_tax_rate() {
			...
			try {
				$sql = "
					SELECT t.tax_rate
					FROM {$wpdb->prefix}woocommerce_tax_rates t
					WHERE t.tax_rate_priority = 1
					AND t.tax_rate_class = ''
					ORDER BY t.tax_rate_order ASC
					LIMIT 1
				";
		...
		private function set_media_path() {
			$this->media_path = '/media'; // by default
			...
			if ( !is_wp_error($response) && isset($response['body']) ) {
				$pattern_pub_media = preg_quote(trailingslashit($this->plugin_options['url']) . 'pub/media/');
				$pattern_media = preg_quote(trailingslashit($this->plugin_options['url']) . 'media/');
				$count_pub_media = preg_match_all('#' . $pattern_pub_media . '#', $response['body']);
				$count_media = preg_match_all('#' . $pattern_media . '#', $response['body']);
				if ( $count_pub_media > $count_media ) { // If more occurences of /pub/media are found on the home page
		...
		public function import_cms_article($post) {
			...
			$content = preg_replace('/{{media url="(.*?)"}}/', $this->media_path . "/$1", $content);
		...
		private function import_categories() {
			...
			do {
				...
				$category_ids = $this->get_category_ids($this->chunks_size);
				$categories_count = count($category_ids);
				foreach ( $category_ids as $category_id ) {
					// Increment the Magento last imported category ID
					update_option('fgm2wc_last_category_id', $category_id);
					
					$new_term = $this->import_category($category_id, $this->default_language);
					if ( !is_wp_error($new_term) ) {
						$imported_categories_count++;
						$terms[] = $new_term['term_id'];

						// Hook after inserting the category
						do_action('fgm2wc_post_import_product_category', $new_term['term_id'], $category_id);
					}
				}
				...
				// Hook after importing the categories
				do_action('fgm2wc_post_import_product_categories', $category_ids);
				
			} while ( ($category_ids != null) && ($categories_count > 0) );
			...
			$this->update_categories_hierarchy($this->default_language);
		 ...
		 */
		public function import_category($category_id, $language) {
			$taxonomy = 'product_cat';
			...
			if ( !empty($description) ) {
				if ( !$this->plugin_options['skip_media'] ) {
					$category_media = $this->import_media_from_content($description, $category['created_at']);
				} else {
					// Skip media
			...
			if ( !is_wp_error($new_term) ) {
				...
				add_term_meta($new_term['term_id'], '_fgm2wc_old_product_category_id' . '-lang' . $language, $category_id, true);
				...
				$this->imported_categories[$language][$category_id] = $new_term['term_id'];
		...
		/**
		 * Get the other category fields
		 * 
		 * @since 3.27.0
		 * 
		 * @param int $category_id Category ID
		 * @param int $category_entity_id Category Entity ID
		 * @return array Category data
		 */
		public function get_other_category_fields($category_id, $category_entity_id) {
			$fields = array(
				'name',
				'is_active',
				'description',
			...
			$fields = apply_filters('fgm2wc_get_other_fields', $fields, $category_entity_id);
			return $this->get_attribute_values($category_id, $category_entity_id, $fields);
		}
		 ...
		 */
		protected function guess_image_filename($data) {
			$filename = '';
			$potential_keys = array('image', 'sale_image', 'small_image', 'thumbnail');
			$potential_keys = apply_filters('fgm2wc_guess_image_filename_potential_keys', $potential_keys, $data);
			foreach ( $potential_keys as $key ) {
				if ( isset($data[$key]) && !empty($data[$key]) && ($data[$key] != 'no_selection') ) {
					$filename = $data[$key];
					break;
				}
			}
			$filename = apply_filters('fgm2wc_guess_image_filename', $filename, $data);
			return $filename;
		}
		...
		private function update_categories_hierarchy($language) {
			$this->display_admin_notice(__('Importing product categories hierarchy...', 'fg-magento-to-woocommerce'));
			$taxonomy = 'product_cat';
			foreach ( $this->imported_categories[$language] as $magento_category_id => $wp_category_id ) {
				$parent_id = $this->get_parent_category_id($magento_category_id);
		 ...
		 * @return int Magento parent category ID
		 */
		private function get_parent_category_id($category_id) {
			...
			$sql = "
				...
				ORDER BY c.{$this->entity_id_field} DESC -- to get only the category with the last row_id (Magento EE)
				LIMIT 1
			";
		...
		private function import_products() {
			...
			do {
				...
				$product_ids = $this->get_product_ids($this->chunks_size);
				$products_count = count($product_ids);
				foreach ( $product_ids as $product_id ) {
					// Increment the Magento last imported product ID
					update_option('fgm2wc_last_magento_product_id', $product_id);
					
					$new_post_id = $this->import_product($product_id, $this->default_language);
					if ( $new_post_id ) {
						$this->imported_products_count++;
						
						// Hook for doing other actions after importing the post
						do_action('fgm2wc_post_import_product', $new_post_id, $product_id);
					}
				}
			...
			} while ( ($product_ids != null) && ($products_count > 0) );
		 ...
		 */
		public function import_product($product_entity_id, $language) {
			
			$product = $this->get_product($product_entity_id);
			
			if ( ($language == $this->default_language) && isset($this->imported_products[$this->default_language]) && array_key_exists($product_entity_id, $this->imported_products[$this->default_language]) ) {
			...
			if ( $new_post_id ) {
				...
				$this->imported_products[$language][$product_entity_id] = $new_post_id;
				
				$prices = $this->import_product_details($new_post_id, $product, $product_medias, $post_media);
				
				// Add the Magento ID as a post meta
				if ( $language == $this->default_language ) {
					add_post_meta($new_post_id, '_fgm2wc_old_product_id', $product_entity_id, true);
				} else {
					add_post_meta($new_post_id, '_fgm2wc_old_product_id' . '-lang' . $language, $product_entity_id, true);
				}
		 ...
		 */
		protected function build_product_post(&$product, $product_entity_id, $language) {
			// Date
			$date = $product['created_at'];

			// Stock
			$stock = $this->get_stock($product_entity_id, $this->website_id);
			if ( empty($stock) ) {
				$stock = $this->get_stock($product_entity_id, 0); // Get the stock of the website 0
			}
			...
			$product_categories = $this->get_product_categories($product_entity_id);
			$menu_order = 0;
			...
			$status = (!isset($product['status']) || ($product['status'] == 1))? 'publish': 'draft';
			$post_name = isset($product['url_key'])? $product['url_key'] : sanitize_title($title);
			if ( $status == 'draft' ) {
				$post_name .= '-disabled'; // To avoid 404 errors (for published products with -2 appended to the slug)
			}
		...
		protected function import_product_details($new_post_id, $product, $product_medias, $post_media) {
			...
			if ( !$this->plugin_options['first_image_in_gallery'] ) {
			...
			// Prices
			if ( ($product['type_id'] == 'bundle') && (!isset($product['price_type']) || ($product['price_type'] == 0)) ) {
		 ...
		 */
		protected function replace_media_shortcodes($content) {
			$content = preg_replace('/{{media url="?(.*?)"?}}/', $this->media_path . "/$1", $content);
			return $content;
		}
		...
		public function get_stock($product_id, $website_id) {
			...
			if ( count($result) > 0 ) {
				$stock = $result[0];
				if ( $stock['qty'] < 0 ) {
					$stock['qty'] = 0; // Prevent negative value
				}
			}
			return $stock;
		}
		...
		public function get_other_product_fields($product_id, $product_entity_id) {
			...
			$fields = apply_filters('fgm2wc_get_other_fields', $fields, $product_entity_id);
		...
		public function import_product_medias($product) {
			...
			if ( !$this->plugin_options['skip_media'] ) {
				...
				// Featured image
				$image_filename = $this->guess_image_filename($product);
				if ( !empty($image_filename) ) {
				...
				if ( $this->plugin_options['thumbnail_in_gallery'] && isset($product['thumbnail']) && !empty($product['thumbnail']) && ($product['thumbnail'] != $product['image']) ) {
					$thumbnail = array(
						'value_id'	=> 0,
				...
				// Gallery images
				$product_images = $this->get_product_images($product['entity_id'], $this->store_id, $product);
				if ( empty($product_images) ) {
					$product_images = $this->get_product_images($product['entity_id'], 0, $product);
				}
				foreach ( $product_images as $image ) {
					if ( !isset($product['image']) || ($image['value'] != $product['image']) ) { // to avoid duplicate images
						$images[] = $image;
					}
				}
				...
				$post_media = $this->import_media_from_content($content, $product['created_at']);
			}
		 ...
		 */
		protected function build_unique_slug($slug, $used_slugs) {
			$matches = array();
		...
		/**
		 * Get the Magento product category IDs
		 * 
		 * @since 3.26.0
		 * 
		 ...
		 */
		private function get_category_ids($limit=1000) {
			$category_ids = array();
			$prefix = $this->plugin_options['prefix'];

			$last_magento_category_id = (int)get_option('fgm2wc_last_category_id'); // to restore the import where it left
			$sql = "
				SELECT DISTINCT c.entity_id
				FROM {$prefix}catalog_category_entity c
				WHERE c.parent_id != 0 -- don't import the root category
				AND c.entity_id > '$last_magento_category_id'
			...
			$category_ids = apply_filters('fgm2wc_get_category_ids', $category_ids);
		...
		protected function get_category($category_id) {
			...
			$sql = "
				SELECT c.{$this->entity_id_field} AS entity_id, c.parent_id, c.created_at, c.position
				FROM {$prefix}catalog_category_entity c
				WHERE c.entity_id = '$category_id'
				ORDER BY c.{$this->entity_id_field} DESC -- to get only the category with the last row_id (Magento EE)
				LIMIT 1
			...
			$category = apply_filters('fgm2wc_get_category', $category);
				...
				foreach ( $this->attribute_types[$entity_type_id] as $attribute_type => $attributes ) {
					...
					if ( !empty($attributes_ids) ) {
						...
						$sql = "
							...
							WHERE e.{$this->entity_id_field} = '$entity_id'
		 ...
		 */
		private function get_product_ids($limit=1000) {
			$product_ids = array();
			$prefix = $this->plugin_options['prefix'];
			...
			$product_ids = apply_filters('fgm2wc_get_product_ids', $product_ids);
		...
		public function get_product($product_id) {
			...
			$sql = "
				SELECT p.{$this->entity_id_field} as entity_id, p.type_id, p.sku, p.created_at
				FROM {$prefix}catalog_product_entity p
				WHERE p.entity_id = '$product_id'
				ORDER BY p.{$this->entity_id_field} DESC -- to get only the product with the last row_id (Magento EE)
				LIMIT 1
			";
			$results = $this->magento_query($sql);
			if ( count($results) > 0 ) {
			...
			$product = apply_filters('fgm2wc_get_product', $product);
		 ...
		 */
		public function get_product_images($product_id, $store_id=0, $product=array()) {
			$images = array();
			$prefix = $this->plugin_options['prefix'];
			...
			if ( version_compare($this->magento_version, '2', '<') ) {
				...
				$sql = "
					...
					INNER JOIN {$prefix}catalog_product_entity_media_gallery_value gv ON gv.value_id = g.value_id
					WHERE gve.{$this->entity_id_field} = '$product_id'
					AND gv.store_id = '$store_id'
					AND gv.disabled = 0
					ORDER BY gv.position
				";
			...
			$images = apply_filters('fgm2wc_get_product_images', $images, $product_id, $store_id, $product);
		...
		public function import_media($name, $filename, $date='0000-00-00 00:00:00', $options=array(), $image_caption='') {
			...
			$img_dir = date('Y/m', strtotime($date));
			...
			if ( $this->plugin_options['import_duplicates'] == 1 ) {
				...
				$new_filename = preg_replace('/\.(.*?)$/', '-' . $short_crc . ".$1", $new_filename); // Insert the CRC before the file extension
			}
...
//				print "Copy \"$old_filename\" => $new_full_filename<br />";
				...
				if ( $attachment_id ) {
					update_post_meta($attachment_id, '_fgm2wc_old_file', $old_filename);
		...
		public function get_wp_term_id_from_meta($meta_key, $meta_value) {
			...
			$sql = "SELECT term_id FROM {$wpdb->termmeta} WHERE meta_key = %s AND meta_value = %s LIMIT 1";
			$term_id = $wpdb->get_var($wpdb->prepare($sql, $meta_key, $meta_value));
			return $term_id;
		}

		/**
		 * Returns the imported user ID corresponding to a meta key and value
		 *
		 * @since 3.43.0
		 * 
		 * @param string $meta_key Meta key
		 ...
		 */
		public function get_wp_user_id_from_meta($meta_key, $meta_value) {
			global $wpdb;

			$sql = "SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key = %s AND meta_value = %s LIMIT 1";
			$user_id = $wpdb->get_var($wpdb->prepare($sql, $meta_key, $meta_value));
			return $user_id;
		}
