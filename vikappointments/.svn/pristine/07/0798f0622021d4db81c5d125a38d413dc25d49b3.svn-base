...
class VikAppointmentsControllerConfiguration extends VAPControllerAdmin
{
	...
	public function save()
	{
		...
		$args['replytoemail'] = $input->getString('replytoemail', '');

		// NOTIFICATIONS

		$args['mailcustwhen']         = $input->getString('mailcustwhen', []);
		$args['mailempwhen']          = $input->getString('mailempwhen', []);
		$args['mailadminwhen']        = $input->getString('mailadminwhen', []);
		$args['notifyonstatuschange'] = $input->getUint('notifyonstatuschange', 0);
		...
		$args['stockmailtmpl'] = $input->getString('stockmailtmpl', '');
		...
		$args['editablefields'] = $input->getString('editablefields', '');
		...
		// restrictions
		$args['restrtimeline'] = $input->getUint('restrtimeline', 0);
	...
	public function mailpreview()
	{
		...
		else if ($alias == 'stock')
		{
			...
			$options['limit'] = $app->input->getUint('limit');
		}
		else
		{
			if (!$id)
