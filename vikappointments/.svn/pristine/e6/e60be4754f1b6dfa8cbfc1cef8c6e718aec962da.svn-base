...
class VikAppointmentsControllerReservation extends VAPControllerAdmin
{
	...
	public function changestatusajax()
	{
		...
		/**
		 * Always include the flag to process the waiting list.
		 * The model will then make sure that the new status is actually cancelled.
		 * 
		 * @since 1.7.7
		 */
		$data['notifywl'] = true;
		...
		/**
		 * Automatically send a notification to the customer according to the mailing preferences.
		 * 
		 * @since 1.7.7
		 */
		if (VAPFactory::getConfig()->getBool('notifyonstatuschange'))
		{
			try
			{
				// send e-mail notification to the customer
				$model->sendEmailNotification($data['id'], [
