...
class VikAppointmentsControllerOption extends VAPControllerAdmin
{
	...
	public function save($copy = false)
	{
		...
		$args['stock']        = $input->getUint('stock', 0);
		$args['units']        = $input->getInt('units', 0);
		$args['notify_below'] = $input->getUint('notify_below', 0);
	...
	/**
	 * Checks whether there are enough units in stock for the specified option.
	 * 
	 * @return  void
	 * 
	 * @since   1.7.7
	...
	public function checkstock()
	{
		...
		$idOption = $app->input->getUint('id_option', 0);
		$idVariation = $app->input->getUint('id_variation', 0);
		$idIndex = $app->input->getUint('index', 0);
		$units = max(1, $app->input->getUint('units', 1));

		// calculate remaining number of units
		$remainingUnits = $this->getModel()->getStock($idOption, $idVariation, $idIndex);

		if ($remainingUnits === false)
		{
			// stock unused for this option
			$app->close();
		}

		if (($remainingUnits - $units) < 0)
		{
			// units not available, raise error
			UIErrorFactory::raiseError(400, JText::plural('VAP_OPTION_STOCK_N_ERR', $remainingUnits));
		}
