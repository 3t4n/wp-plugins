...
final class Amplitude {
    ...
    public function load_amplitude_sdk($api_key, $session_replay, $session_replay_sample_rate, $should_change_library, $is_eu_server_zone) {
        ...
        wp_enqueue_script($aampli_plg_scr_setup, 'https://cdn.amplitude.com/libs/analytics-browser-2.11.9-min.js.gz');

        if($session_replay) {
            wp_enqueue_script($aampli_plg_scr_session_replay, 'https://cdn.amplitude.com/libs/plugin-session-replay-browser-1.8.2-min.js.gz', array($aampli_plg_scr_setup));
            $configuration_dependency_list[] = $aampli_plg_scr_session_replay;
        }

        wp_enqueue_script($aampli_plg_scr_configure, plugins_url('scripts/amplitude-configure.js', __FILE__), $configuration_dependency_list);
