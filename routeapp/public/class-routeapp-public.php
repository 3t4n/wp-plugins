...
class Routeapp_Public {
    ...
    public function add_actions_and_filters() {
        ...
        add_action('woocommerce_after_order_object_save', array($this, 'routeapp_update_tracking_order_api_HPOS'), 20, 2);
    ...
    /**
     * Update tracking order data sending when HPOS is enable
     * @param  object $order
     * @param  object $data_store
     */
    public function routeapp_update_tracking_order_api_HPOS($order, $data_store) {
        ...
        // Ensure the $order object is valid and of type 'shop_order'
        if (!$order || $order->get_type() !== 'shop_order') {
            return false;
        }

        $this->routeapp_shipment_tracking->update($order->get_id(), $this);
    }
    ...
    public function routeapp_save_quote_to_order($order_id )
    {
            ...
            if (WC()->session) {
                ...
                if (isset($cached) && is_array($cached) && isset($cached['body'])) {
                    ...
                    if (!empty($fees)) {
                        foreach ($fees as $fee) {
                            if ($fee->get_name() === $this->routeapp_get_insurance_label()) {
                                ...
                                if (class_exists('Automattic\WooCommerce\Utilities\OrderUtil')
                                    && OrderUtil::custom_orders_table_usage_is_enabled()) {
                                    // HPOS usage is enabled.
                                    $order->update_meta_data('_routeapp_route_charge', $insurance_amount);
                                    $order->update_meta_data('_routeapp_route_protection', $insurance_selected);
                                    $order->save();
                                } else {
                                    // Traditional CPT-based orders are in use.
                                    update_post_meta($order->get_id(), '_routeapp_route_charge', $insurance_amount);
                                    update_post_meta($order->get_id(), '_routeapp_route_protection', $insurance_selected);
                                }
                            }
                        }
                        ...
                        try {
                            // Check if WC()->cart exists and is accessible
                            if (class_exists('WC_Cart') && isset(WC()->cart) && WC()->cart instanceof WC_Cart) {
                                // Fallback to WC()->cart->get_fees() if no fees are present in the order
                                $cart_fees = WC()->cart->get_fees();
                                if (!empty($cart_fees)) {
                                    foreach ($cart_fees as $cart_fee) {
                                        if ($cart_fee->name === $this->routeapp_get_insurance_label()) {
                                            $insurance_selected = true;
                                            $insurance_amount = $cart_fee->amount; // Correct method for cart fee amount
                            ...
                            // Log the exception and ensure the application flow continues
                            error_log("An error occurred while processing cart fees: " . $e->getMessage());
                        }
	    ...
	    //Update route included order statuses and route cancel order statuses when the order is created by the admin page without route fee
	    if(is_admin()){
			$this->updateRouteIncludedOrderStatusMetaFields( $order );
		}
    }
	 ...
	 * @return void
	 */
	public function updateRouteIncludedOrderStatusMetaFields( $order ): void {
		try {
			$acceptedOrderStatuses    = get_option( 'routeapp_included_order_statuses' ) ? get_option( 'routeapp_included_order_statuses' ) : [];
			$acceptedCanceledStatuses = get_option( 'routeapp_cancel_order_statuses' ) ? get_option( 'routeapp_cancel_order_statuses' ) : [];
			...
			// Log the exception and ensure the application flow continues
			error_log( "An error occurred while updating Route order status meta fields: " . $e->getMessage() );
		}
	}
