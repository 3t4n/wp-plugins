...
class SettingsActions extends BasePostAction {
	...
	/**
	 * Instance of `CertificateGenerationHandler` class
	 * to call its required methods.
	 *
	 * @var CertificateGenerationHandler $cert_generation_handler
	 */
	private $cert_generation_handler;
	...
	public function __construct() {
		...
		$this->cert_generation_handler = CertificateGenerationHandler::get_instance();
	...
	private $funcs = array(
		...
		'mo_idp_request_demo',
		'mo_idp_download_new_cert',
		'mo_idp_download_current_cert'
	);
	...
	public function route_post_data( $option ) {
		...
		switch ( $option ) {
			...
			case $this->funcs[10]:
				...
				break;
			case $this->funcs[11]:
				$this->cert_generation_handler->mo_idp_download_new_public_cert();
				break;
			case $this->funcs[12]:
				$this->cert_generation_handler->mo_idp_download_current_public_cert();
				break;
		}
	}
}
