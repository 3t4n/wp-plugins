...
final class MoDbQueries {
	...
	public function get_cert_by_sp_id( $sp_id ) {
		$sp_id = $this->get_valid_sp_id( $sp_id );
		if( -1 === $sp_id ) return null;
		global $wpdb;
	 ...
	 *
	 * @return int|false Returns false when there is no SP configured.
	 */
	...
	/**
	 * This function is used to make sure that we have a valid SP ID to work with.
	 * If the sp id passed is false we retrieve the id of 1st configured SP.
	 * If there is no SP configured returns -1 
	 *
	 * @param int|boolean $sp_id SP ID received in function call or false
	 * 
	 * @return int
	 */
	private function get_valid_sp_id( $sp_id ) {
		if ( false === $sp_id ) {
