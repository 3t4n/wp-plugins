<?php
/*
Plugin Name: WP Easy GEO Blocker and Redirect (by SiteGuarding.com)
...
if( !is_admin() ) 
{
	...
	function plgsggeo_footer_protectedby() 
	{
        ...
        {
            $avp_path = dirname( str_replace('easy-geo-blocker-and-redirect', 'wp-antivirus-site-protection', dirname(__FILE__)) );
    ...
    {
        plgsggeo_CopySiteGuardingTools(true);
    }
...
if( is_admin() ) {
            ...
            switch ($action)
            {
                ...
                case 'Save_backend_params':
                    if (check_admin_referer( 'name_3dfUejeked' ))
                    {
                        ...
                        if (isset($_POST['backend_ip_list_allow'])) $data['backend_ip_list_allow'] = sanitize_text_field($_POST['backend_ip_list_allow']);
    ...
    function plgsggeo_CopySiteGuardingTools($output = false)
    {
    ...
    function plgsggeo_API_Request($type = '')
    {
        $plugin_code = 2;
	...
	function plgsggeo_activation()
	{
        ...
        plgsggeo_API_Request(1);
        plgsggeo_CopySiteGuardingTools();
...
class SEO_SG_Protection_HTML
{
    ...
    if ($tab_id == 0)
    {
        ...
        <div <?php echo $block_class; ?>>
            ...
            <?php
                if (!$isPRO)  {
                    ?><div class="ui red mini message"><center>Your version has limits: maximum 15 countries and maximum 15 IP addresses to block.<br /><a href="https://www.siteguarding.com/en/buy-extention/wordpress-geo-website-protection" target="_blank">Get Full Version</a> and unlock all the features</center></div><?php
                }
            ?>
    ...
    if ($tab_id == 1)
    {
        ...
        <div <?php echo $block_class; ?>>
            ...
            <?php
                if (!$isPRO)  {
                    ?><div class="ui red mini message"><center>Your version has limits: maximum 15 countries and maximum 15 IP addresses to allow.<br /> <a href="https://www.siteguarding.com/en/buy-extention/wordpress-geo-website-protection" target="_blank">Get Full Version</a> and unlock all the features</center></div><?php
                }
            ?>
...
<br><br>
	...
	public static function blockPagePreview() {
        ...
        $blockpage_json['logo_url'] = '/wp-content/plugins/easy-geo-blocker-and-redirect/images/logo_siteguarding.svg';
...
class SEO_SG_Protection
{
	...
	public static function CheckWPConfig_file()
	{
		...
		if (!defined('ABSPATH') || strlen(ABSPATH) < 8) 
		{
			...
			$scan_path = str_replace(DIRSEP.'wp-content'.DIRSEP.'plugins'.DIRSEP.'easy-geo-blocker-and-redirect', DIRSEP, $scan_path);
    		//echo TEST;
		}
	...
	public static function PatchWPConfig_file($action = true)   // true - insert, false - remove
	{
        ...
        $integration_code = '<?php /* Siteguarding Block 140FEEA75CC5-START */ if (file_exists("'.$file.'"))include_once("'.$file.'");/* Siteguarding Block 140FEEA75CC5-END */?>';
    ...
    public static function CheckAntivirusInstallation()
    {
		...
		$avp_path = str_replace('easy-geo-blocker-and-redirect', 'wp-antivirus-site-protection', $avp_path);
        if ( file_exists($avp_path) ) return true;
        
        // Check for wp-antivirus-website-protection-and-website-firewall
        $avp_path = dirname(__FILE__);
		$avp_path = str_replace('easy-geo-blocker-and-redirect', 'wp-antivirus-website-protection-and-website-firewall', $avp_path);
        if ( file_exists($avp_path) ) return true;
        
        // Check for wp-website-antivirus-protection
        $avp_path = dirname(__FILE__);
		$avp_path = str_replace('easy-geo-blocker-and-redirect', 'wp-website-antivirus-protection', $avp_path);
        if ( file_exists($avp_path) ) return true;
