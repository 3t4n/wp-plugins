...
class Number
{
    ...
    private function displayCallback()
    {
        printf(
            ...
            esc_attr($this->minValue) ? 'min="' . esc_attr($this->minValue) . '"' : 'min="0"',
            esc_attr($this->maxValue) ? 'max="' . esc_attr($this->maxValue) . '"' : '',
            esc_attr($this->valueSteps()),
            esc_html($this->textRight),
            esc_html($this->description($this->description))
        );
    }
}
