...
class Textarea
{
    ...
    private function displayCallback()
    {
        printf(
            ...
            esc_attr( $this->settingsName),
            $this->placeholder ? 'placeholder="' . esc_attr($this->placeholder) . '"' : '',
            esc_attr(strtr($this->fieldStyle, '_', '-')),
            esc_html($this->optionValue),
            esc_html($this->description($this->description, $this->descriptionItalic))
        );
    }
}
