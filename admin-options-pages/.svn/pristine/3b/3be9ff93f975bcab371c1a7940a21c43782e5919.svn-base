...
class Data extends DB
{
    ...
    {
        if ($_GET['page'] === SubpageCreate::SLUG) {
            return Collection::make([
                'registered_options' => static::getAllOptionNamesExcludeCurrentPageFieldNames()
            ]);
        }

        if (!isset($_GET['action']) && !isset($_GET['optionpage']) || $_GET['page'] === SubpageMaster::SLUG) {
            return [];
        }

        $currentPage = static::listTableData()->filter(function ($value) {
            return $_GET['optionpage'] === $value['ID'];
        })->values();
    ...
    public static function requestGetSubpages()
    {
        ...
        return Collection::make($_POST['pages']);
    }
    ...
    public static function getAllRegisteredFieldNames()
    {
        ...
        return static::getFieldsfromId($_GET['optionpage'])->pluck('field_name');
    }
    ...
    public static function getFieldNamesCurrentPage()
    {
        ...
        return static::$getAllFromPageValueColumn->where('menu_id', $_POST['menu_id'])
            ->pluck('pages')->flatten(1)
            ->pluck('fields')->flatten(1)
            ->pluck('field_name');
    }
    ...
    public static function listTableData()
    {
        return Collection::make(static::getAllRowsFromPluginTable())->map(function ($row) {
            ...
            if (isset($pageValue['existing_menu'])) {
                ...
                // $menuName = '<span style="padding-right:10px;" class="dashicons ' . $dashicon . '"></span>' . $pageValue['existing_menu']['menu_name'];
                $menuIcon = $dashicon;
                ...
                $menuIcon = $pageValue['menu']['icon_url'];
    ...
    public static function deleteRowFromSpecificId()
    {
        ...
        if (!isset($action, $optionpage, $nonce)) {
            return;
        }
