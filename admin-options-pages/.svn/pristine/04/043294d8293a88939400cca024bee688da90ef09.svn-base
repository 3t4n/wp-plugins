...
class Input extends DB
{
    ...
    public function prepareDataForPages(Validation $validate)
    {
        ...
        if ($pagename !== SubpageCreate::SLUG && $pagename !== SubpageEdit::SLUG) {
            return;
        }

        if ($pagename === SubpageCreate::SLUG) {
            $nonceName   = SubpageCreate::$nonceName;
            $nonceAction = SubpageCreate::$nonceAction;
        }

        if ($pagename === SubpageEdit::SLUG) {
    ...
    public function processingFields($fields)
    {
        ...
        if ($_GET['page'] === SubpageEdit::SLUG) {
            ...
            $optionsNames = Data::getFieldsfromId($_GET['optionpage'])
            ...
            Notice::succes(__('Updated.', 'admin-options-pages'));
        }
    }
