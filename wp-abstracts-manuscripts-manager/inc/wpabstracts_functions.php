...
{
	global $wpdb;
	$sql = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}wpabstracts_topics WHERE `event_id` = %d ORDER BY `order` ASC", $event_id);
	return $wpdb->get_results($sql);
}
...
function wpabstracts_user_save_regform()
{
	...
	wp_send_json_success(['message' => __("Successfully updated user registration form.", 'wpabstracts')]);
	die();
}
	...
	<div class="wpabstracts pdf">
		...
		<div class="wpabstracts pdf_headerbar" style="text-align: right; font-style: italic;">
			...
			<?php if ($settings['topic']['enabled'] == true) { ?>
				<div class="pdf_header_row"><?php echo apply_filters('wpabstracts_title_filter', __('Topic', 'wpabstracts'), 'topic'); ?>: <?php echo stripslashes($topic->name); ?></div>
			<?php } ?>
...
// utility function to validate a nonce
