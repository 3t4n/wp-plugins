...
<?php
	$html = ob_get_contents();
	ob_end_clean();
	return apply_filters('wpabstracts_load_status', $html);
}
...
function wpabstracts_delete_abstract($abs_id, $showMsg)
{
	...
	global $wpdb;
	$wpdb->show_errors();
	$wpdb->delete("{$wpdb->prefix}wpabstracts_abstracts", ['abstract_id' => $abs_id], ['%d']);
	$wpdb->delete("{$wpdb->prefix}wpabstracts_attachments", ['abstracts_id' => $abs_id], ['%d']);
	if ($showMsg) {
...
function wpabstracts_delete_abstracts()
{
	...
	$wpdb->show_errors();
	if ($_POST) {
		if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], WPABSTRACTS_SECRET_KEY)) {
			wpabstracts_show_message(__('Unable to process your request. Please reload the page and try again.', 'wpabstracts'), 'alert-danger');
			return;
		}

		// Sanitize and validate inputs
		$abs_ids = isset($_POST['abs_ids']) && is_array($_POST['abs_ids'])
			? array_map('intval', $_POST['abs_ids'])
			: [];

		$isBulk = isset($_POST['isBulk']) ? filter_var($_POST['isBulk'], FILTER_VALIDATE_BOOLEAN) : false;

		// Ensure there are IDs to delete
		if (empty($abs_ids)) {
			wpabstracts_show_message(
				__('No valid Abstract IDs provided for deletion.', 'wpabstracts'),
				'alert-danger'
			);
			return;
		}

		// Delete abstracts and their attachments
		foreach ($abs_ids as $abs_id) {
			$wpdb->delete("{$wpdb->prefix}wpabstracts_abstracts", ['abstract_id' => $abs_id], ['%d']);
			$wpdb->delete("{$wpdb->prefix}wpabstracts_attachments", ['abstracts_id' => $abs_id], ['%d']);
		}
