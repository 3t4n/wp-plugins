				...
				<div class="wpabstracts row">
					...
					<div class="wpabstracts col-xs-12 col-sm-6">
						...
						<div class="wpabstracts panel panel-primary">
							...
							<div class="wpabstracts panel-body">
								<div class="wpabstracts form-group col-xs-12">
									<ul id="admin_columns_sortable">
										<?php foreach ($abstracts_columns as $key => $column) { ?>
...
<script>
	jQuery(document).ready(function() {
		var adminColumnsList = document.getElementById('admin_columns_sortable');
		new Sortable(adminColumnsList, {
			animation: 150,
			handle: '.drag-handle', // Restrict dragging to the drag handle
			onEnd: function() {
				const items = Array.from(adminColumnsList.querySelectorAll('li'));
				const order = items.map(item => item.dataset.key);
				// Send updated order to the server
				jQuery.post(wpabstracts.ajaxurl, {
					action: 'reorder_admin_columns',
					order: order,
