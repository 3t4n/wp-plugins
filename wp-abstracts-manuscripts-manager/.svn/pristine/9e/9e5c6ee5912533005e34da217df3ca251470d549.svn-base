...
function wpabstracts_add_event()
{
	...
	if ($_POST) {
		...
		wpabstracts_get_add_view('events', null);
	}
}
...
function wpabstracts_edit_event($id)
{
	...
	if ($_POST) {
		...
		$event = wpabstracts_get_edit_view('events', $id);
		if ($event) {
			echo $event;
		} else {
...
function wpabstracts_delete_event($id)
{
	...
	if ($event) {
		...
		if ($abs_ids) {
			$ids = array_map(function ($abs) {
				return $abs->abstract_id;
			}, $abs_ids);
			$ids = implode(',', $ids);
			$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}wpabstracts_abstracts WHERE `abstract_id` IN (%s)", $ids));
			$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}wpabstracts_attachments WHERE `abstracts_id` IN (%s)", $ids));
		}
		$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}wpabstracts_topics WHERE `event_id` = %d", $id));
		$wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}wpabstracts_events WHERE `event_id` = %d", $id));
