...
class WPAbstract_Attachments_Table extends WP_List_Table
{
    ...
    function column_default($item, $column_name)
    {
        switch ($column_name) {
            case 'abstracts_id':
                echo $item->title . " [" . $item->abstract_id . "]";
                break;
            case 'event':
            ...
            case 'filetype':
                ...
                break;
            case 'filesize':
                echo number_format(($item->filesize / 1048576), 2) . " MB";
                break;
        }
    }
    ...
    function process_bulk_action()
    {
        ...
        // bulk delete handed via jQuery('#doaction, #doaction2').on('click' in abstracts.attachments.php
        do_action('wpabstracts_bulk_action', $this->current_action(), 'attachments'); // action and tab
    }
