...
class WPAbstract_Events_Table extends WP_List_Table
{
    ...
    function column_name($item)
    {
        $actions = array(
            ...
            'delete' => '<a href="javascript:wpabstracts_delete_event(' . $item->event_id . ');">' . __('Delete', 'wpabstracts') . '</a>',
        );
...
class WPAbstracts_Event_Topics extends WP_List_Table
{
    ...
    {
        echo '<tr data-id="' . esc_attr($item->topic_id) . '">';
    ...
    function column_default($item, $column_name)
    {
        switch ($column_name) {
            case 'drag':
                return '<span class="drag-handle" title="' . esc_attr__('Drag to reorder', 'wpabstracts') . '">☰</span>';
            case 'event_name':
    ...
    function prepare_items()
    {
        ...
        $query .= "ORDER BY wpa_topics.order ASC, wpa_topics.topic_id DESC";
        $this->process_bulk_action();
        $this->items = $wpdb->get_results($query);
        $columns = $this->get_columns();
        $_wp_column_headers[$screen->id] = $columns;
