...
define('WPABSTRACTS_VERSION', '2.7.4');
...
function wpabstracts_install()
{
	...
	$abstracts_columns['affiliation'] = array('label' => __('Affiliation', 'wpabstracts'), 'enabled' => false);
...
function wpabastracts_option_updates()
{
	...
	if (!array_key_exists('affiliation', $abstracts_columns)) { // since v2.7.4
		$position = array_search('author', array_keys($abstracts_columns));
		$abstracts_columns = array_slice($abstracts_columns, 0, $position + 1, true) +
			array('affiliation' => array('label' => __('Affiliation', 'wpabstracts'), 'enabled' => false)) +
			array_slice($abstracts_columns, $position + 1, null, true);
...
function wpabstracts_load_js($frontend)
{
	...
	wp_enqueue_script('wpabstracts-sortable', plugins_url('assets/js/sortable.min.js', __FILE__), array('jquery'), WPABSTRACTS_VERSION, true);
...
function wpabstracts_localize()
{
	...
	$data = array(
		...
		'nonce' => wp_create_nonce(WPABSTRACTS_SECRET_KEY),
		...
		'confirm_templates_delete' => __('Do you really want to delete the selected templates?', 'wpabstracts'),
		'confirm_review_delete' => __('Do you really want to delete this review?', 'wpabstracts'),
		'confirm_att_delete' => __('Do you really want to delete this attachment?', 'wpabstracts'),
		'confirm_atts_delete' => __('Do you really want to delete the selected attachments?', 'wpabstracts'),
		'confirm_user_delete' => __('Are you sure you want to delete this user?', 'wpabstracts'),
		'confirm_users_delete' => __('Are you sure you want to delete the selected users?', 'wpabstracts'),
		'confirm_topic_delete' => __('Are you sure you want to delete this topic?', 'wpabstracts'),
		'confirm_topics_delete' => __('Are you sure you want to delete the selected topics?', 'wpabstracts'),
		...
		'error_ajax' => __('An error occurred while performing this operation. Please try again.', 'wpabstracts'),
		'sort_success' => __('Sort order updated successfully.', 'wpabstracts'),
		'sort_error' => __('An error occurred while updating the sort order. Please try again.', 'wpabstracts'),
		'no_selection' => __('No item selected. Please select items before performing bulk actions.', 'wpabstracts'),
	);
...
{
	if (!wpabstracts_verify_nonce($_POST['nonce'])) {
		wp_send_json_error([
			'message' => 'Unable to process your request. Please reload the page and try again.',
			'error_code' => 'invalid_nonce',
		], 403);
		wp_die();
	}
	require_once apply_filters('wpabstracts_page_include', WPABSTRACTS_PLUGIN_DIR . 'abstracts/abstracts.manage.php');
	$html = wpabstracts_load_status();
...
function wpabstracts_load_topics_ajax()
{
	...
	if (isset($_POST['event_id']) && !empty($_POST['event_id'])) {
		...
		wp_send_json_error([
			'message' => __('Error! Missing Event ID from wp_ajax_loadtopics.', 'wpabstracts'),
		], 403);
	}
	wp_die();
}

add_action('wp_ajax_reorder_topics', 'wpabstracts_reorder_topics');
function wpabstracts_reorder_topics()
{
	if (!wpabstracts_verify_nonce($_POST['nonce'])) {
		wp_send_json_error([
			'message' => 'Unable to process your request. Please reload the page and try again.',
	...
	if (!isset($_POST['order']) || !is_array($_POST['order'])) {
		wp_send_json_error(['message' => __('Invalid order data.', 'wpabstracts')]);
	}

	global $wpdb;
	$topics_tbl = $wpdb->prefix . 'wpabstracts_topics';
	$order = array_map('intval', $_POST['order']); // Ensure IDs are integers

	foreach ($order as $display_order => $topic_id) {
		$wpdb->update(
			$topics_tbl,
			['order' => $display_order + 1], // Start with 1
			['topic_id' => $topic_id],
			['%d'],
			['%d']
		);
...
add_action('wp_ajax_reorder_admin_columns', 'wpabstracts_reorder_admin_columns');
function wpabstracts_reorder_admin_columns()
{
	if (!wpabstracts_verify_nonce($_POST['nonce'])) {
		wp_send_json_error([
			'message' => 'Unable to process your request. Please reload the page and try again.',
	...
	if (!current_user_can('manage_options')) {
		wp_send_json_error(array('message' => __('You do not have permission to perform this action.', 'wpabstracts')));
	}

	$order = isset($_POST['order']) ? $_POST['order'] : array();
	$abstracts_columns = get_option('wpabstracts_abstracts_columns');

	// Reorder the columns based on the new order from the sortable list
	$new_order = array();
	foreach ($order as $key) {
		if (isset($abstracts_columns[$key])) {
			$new_order[$key] = $abstracts_columns[$key];
		}
	}
	update_option('wpabstracts_abstracts_columns', $new_order);
	wp_send_json_success();
}
