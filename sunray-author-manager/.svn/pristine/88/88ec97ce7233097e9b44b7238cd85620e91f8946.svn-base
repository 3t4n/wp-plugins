...
$sam_version = "1.0.19"; // REMEMBER TO CHANGE BELOW TOO!
...
function sam_meta_box($post)
{
    ...
    if ($my_query->have_posts()) {
        $html .= '<select name="sam_reprint_id" class="sam-select-2">';
        ...
        while ($my_query->have_posts()) {
            ...
            $html .= '<option value="' . $post->ID . '" ' . $selected . '>' . get_the_title() . " - $pub_name $pub" . '</option>';
        }
        $html .= '</select>';
        ...
        if ($opts['date_headings']) {
            if ($pub_date == PHP_INT_MAX) {
                if (!$date_headings['forthcoming']) {
                    $html .= '<h3 class="sam-date-heading">Forthcoming</h3>';
                    $date_headings['forthcoming']++;
                }
            } else {
                $year = date('Y', $pub_date);
                if (!$date_headings[$year]) {
                    $html .= '<h3 class="sam-date-heading">' . $year . '</h3>';
                    $date_headings[$year]++;
                }
            }
        }
...
function sam_do_biblio_single($post)
{
    ...
    $html .= '<li class="sam-biblio-single">';
