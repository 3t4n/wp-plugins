...
function bosta_create_edit_pickup_form()
{
	...
	$url = BOSTA_ENV_URL_V0 . '/pickup-locations ';
	...
	if ( !empty($_GET['pickupId']) ) {
		...
		$url = BOSTA_ENV_URL_V0 . '/pickups/' . esc_html($pickupId);
...
function bosta_create_pickup_action($action, $scheduledDate, $scheduledTimeSlot, $businessLocationId)
{
	...
	if (isset($_GET['pickupId'])) {
		$pickupId = sanitize_text_field($_GET['pickupId']);
		$result = wp_remote_request(BOSTA_ENV_URL_V0 . '/pickups/' . esc_html($pickupId), array(
			'timeout' => 30,
			'method' => 'PUT',
			'headers' => array(
				'Content-Type' => 'application/json',
		...
		$result = wp_remote_post(BOSTA_ENV_URL_V0 . '/pickups', array(
			'timeout' => 30,
			'method' => 'POST',
			'headers' => array(
				'Content-Type' => 'application/json',
...
function bosta_view_scheduled_pickups()
{
    ...
    ";
			...
			if (count($pickupInfo->deliveryTrackingNumbers) > 0) {
				$url = BOSTA_ENV_URL_V0 . '/deliveries/search?trackingNumbers=' . join(',', $pickupInfo->deliveryTrackingNumbers);
				$result = wp_remote_post($url, array(
					'timeout' => 30,
					'method' => 'GET',
					'headers' => array(
		...
		if ((isset($_GET['state']) && $_GET['state'] != 'history-pickups') || !isset($_GET['state'])) {
			$url = BOSTA_ENV_URL_V0 . '/pickups/search?state=Requested,Arrived+at+business,Route+Assigned,Picking+up,Receiving&pageId=-1';
		} else {
			$url = BOSTA_ENV_URL_V0 . '/pickups/search?state=Canceled,Picked up&pageId=-1';
		}
