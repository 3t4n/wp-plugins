...
use Bayarcash\WooCommerce\Blocks\CheckoutFeesBlocksIntegration;
...
final class Bayarcash
{
	...
	public function register_admin_hooks(): void {
		...
		add_filter('woocommerce_payment_gateways', function ($gateways) {
			...
			$number_of_gateways = get_option('bayarcash_additional_fpx', 1);

			for ($i = 2; $i <= intval($number_of_gateways); $i++) {
				$gateways[] = new DynamicFpxGateway($i);
			}
		...
		add_action(
			...
			function ($hook) {
				...
				// Check if we're on the WooCommerce settings page with checkout tab
		...
		add_action(
			...
			function () {
				...
				// Get all active payment methods from WooCommerce
				$payment_gateways = WC()->payment_gateways->payment_gateways();
				$fpx_methods = [];

				// Find all FPX payment methods
				foreach ($payment_gateways as $gateway) {
					if (strpos($gateway->id, 'bc-fpx-') === 0 && substr($gateway->id, -3) === '-wc') {
						$fpx_methods[] = $gateway->id;
					}
				}

				// Combine base and FPX payment methods
				$payment_methods = array_merge($base_payment_methods, $fpx_methods);

				// Build CSS for each payment method
				$dynamic_css = "";

				foreach ($payment_methods as $method) {
					// Get settings for each payment method
					$gateway_settings = get_option('woocommerce_' . $method . '_settings', []);

					// Get logo size settings with fallbacks
					$logo_size_unit = !empty($gateway_settings['logo_size_unit']) ? $gateway_settings['logo_size_unit'] : 'px';
					$logo_size_value = !empty($gateway_settings['logo_size_value']) ? $gateway_settings['logo_size_value'] : '180';

					// Add CSS for this payment method using label selector
					$dynamic_css .= "
                label[for='payment_method_{$method}'] img {
                    width: {$logo_size_value}{$logo_size_unit} !important;
                    height: auto !important;
                }
            ";
				}

				// Add inline CSS after the main stylesheet
				wp_add_inline_style($this->slug.'-checkout-css', $dynamic_css);
			}
		);
	...
	public function block_support(): void {
		// Declare compatibility for all gateways
		add_action('before_woocommerce_init', function() {
			if (class_exists('\Automattic\WooCommerce\Utilities\FeaturesUtil')) {
				...
				// Declare compatibility for each gateway
				foreach ($gateways as $gateway) {
					\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility('cart_checkout_blocks', BAYARCASH_WC['FILE'], true);
				}
			}
		});
		...
		add_action('woocommerce_blocks_loaded', function() {
			if (!class_exists('Automattic\WooCommerce\Blocks\Package')) {
				error_log('WooCommerce Blocks Package not found');
				return;
			}

			require_once BAYARCASH_WC['PATH'] . '/includes/src/blocks/class-checkout-fees-blocks-integration.php';
		...
		add_action('woocommerce_blocks_loaded', function() {
			...
			// Require all gateway block files
			require_once BAYARCASH_WC['PATH'] . '/includes/src/blocks/class-wc-gateway-fpx-blocks.php';
			require_once BAYARCASH_WC['PATH'] . '/includes/src/blocks/class-wc-gateway-duitnow-blocks.php';
			require_once BAYARCASH_WC['PATH'] . '/includes/src/blocks/class-wc-gateway-linecredit-blocks.php';
			require_once BAYARCASH_WC['PATH'] . '/includes/src/blocks/class-wc-gateway-duitnowqr-blocks.php';
			require_once BAYARCASH_WC['PATH'] . '/includes/src/blocks/class-wc-gateway-duitnowshopee-blocks.php';
			require_once BAYARCASH_WC['PATH'] . '/includes/src/blocks/class-wc-gateway-duitnowboost-blocks.php';
			require_once BAYARCASH_WC['PATH'] . '/includes/src/blocks/class-wc-gateway-duitnowqris-blocks.php';
			require_once BAYARCASH_WC['PATH'] . '/includes/src/blocks/class-wc-gateway-duitnowqriswallet-blocks.php';
			require_once BAYARCASH_WC['PATH'] . '/includes/src/blocks/class-wc-gateway-duitnownets-blocks.php';

			add_action(
				'woocommerce_blocks_payment_method_type_registration',
				function($payment_method_registry) {
					// Register all gateway blocks
					$payment_method_registry->register(new Blocks\WC_Gateway_Fpx_Blocks_Support());
					$payment_method_registry->register(new Blocks\WC_Gateway_DuitNow_Blocks_Support());
					$payment_method_registry->register(new Blocks\WC_Gateway_LineCredit_Blocks_Support());
					$payment_method_registry->register(new Blocks\WC_Gateway_DuitNowQR_Blocks_Support());
					$payment_method_registry->register(new Blocks\WC_Gateway_DuitNowShopee_Blocks_Support());
					$payment_method_registry->register(new Blocks\WC_Gateway_DuitNowBoost_Blocks_Support());
					$payment_method_registry->register(new Blocks\WC_Gateway_DuitNowQRIS_Blocks_Support());
					$payment_method_registry->register(new Blocks\WC_Gateway_DuitNowQRISWallet_Blocks_Support());
					$payment_method_registry->register(new Blocks\WC_Gateway_DuitNowNETS_Blocks_Support());
				}
			);
		});
	}
	...
	private function init_features(): void
	{
		...
		new OrderCancellationPrevention();
		 //Check if DirectDebit gateway is enabled
		$directdebit_settings = get_option('woocommerce_directdebit-wc_settings', []);
		if (!empty($directdebit_settings['enabled']) && $directdebit_settings['enabled'] === 'yes') {
