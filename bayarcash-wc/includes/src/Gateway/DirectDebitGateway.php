...
class DirectDebitGateway extends Gateway
{
	...
	public function add_identification_fields($checkout): void {
		// Check if subscriptions feature exists
		if (!$this->has_subscriptions()) {
			return;
		}

		// Get DirectDebit settings and check if enabled
		...
		add_action('wp_ajax_update_directdebit_fields', [$this, 'handle_directdebit_fields_update']);
		add_action('wp_ajax_nopriv_update_directdebit_fields', [$this, 'handle_directdebit_fields_update']);

		// Add JavaScript for payment method handling
		add_action('wp_footer', [$this, 'add_payment_method_script']);

		include_once BAYARCASH_WC['PATH'].'/includes/admin/checkout-fields.php';
	}

	public function handle_directdebit_fields_update(): void {
		check_ajax_referer('directdebit_fields_nonce', 'nonce');
	...
	public function add_payment_method_script(): void {
		if (!is_checkout()) return;
		?>
		<script type="text/javascript">
            jQuery(function($) {
                // Function to toggle identification fields
                function toggleIdentificationFields(show) {
                    const fields = $('#bayarcash_identification_type, #bayarcash_identification_id').closest('.form-row');
                    if (show) {
                        fields.show();
                ...
                toggleIdentificationFields(initialPaymentMethod === 'directdebit-wc');
                ...
                $('form.checkout').on('change', 'input[name="payment_method"]', function(e) {
                    ...
                    $.ajax({
                        ...
                        type: 'POST',
                        data: {
                            action: 'update_directdebit_fields',
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                toggleIdentificationFields(response.data.show_fields);
                            }
                        },
                ...
                $(document.body).on('updated_checkout', function() {
                    ...
                    toggleIdentificationFields(currentMethod === 'directdebit-wc');
                });
            });
		</script>
		<?php
	...
	public function process_payment($order_id): array {
		...
		$directdebit_settings = get_option('woocommerce_directdebit-wc_settings', []);
		if (empty($directdebit_settings['enabled']) || $directdebit_settings['enabled'] !== 'yes') {
			$errors[] = esc_html__('Direct Debit payment method is not enabled.', 'bayarcash-wc');
	...
	public function process_bayarcash(): void {
		...
		try {
			...
			// Validate subscription period and interval
			$this->validate_subscription_period($order);
			$this->validate_subscription_interval($order);
			...
			if (str_contains($e->getMessage(), 'Only weekly and monthly subscriptions')) {
				wc_add_notice(__('Only weekly and monthly subscriptions are supported.', 'bayarcash-wc'), 'error');
			} else if (str_contains($e->getMessage(), 'Current billing schedule:')) {
				// Pass through the full error message with the billing schedule details
				wc_add_notice($e->getMessage(), 'error');
			} else {
				wc_add_notice(__('Unable to process your order. Please try again later.', 'bayarcash-wc'), 'error');
			}
	 ...
	 * @throws Exception
	 */
	private function validate_subscription_period(\WC_Abstract_Order $order): void {
		foreach ($order->get_items() as $item) {
			$product = $item->get_product();
			if ($this->is_subscription_product($product)) {
				$period = \WC_Subscriptions_Product::get_period($product);
				if (!in_array($period, ['week', 'month'])) {
					throw new Exception(__('Only weekly and monthly subscriptions are supported.', 'bayarcash-wc'));
				}
			}
		}
	}
	 ...
	 * @throws Exception
	 */
	private function validate_subscription_interval(\WC_Abstract_Order $order): void {
		foreach ($order->get_items() as $item) {
			$product = $item->get_product();
			if ($this->is_subscription_product($product)) {
				$interval = \WC_Subscriptions_Product::get_interval($product);
				$period_text = \WC_Subscriptions_Product::get_period($product);

				$this->log(sprintf('Validating subscription interval: %d %s', $interval, $period_text));

				if ((int)$interval !== 1) {
					$period_text = \WC_Subscriptions_Product::get_period($product);
					throw new Exception(sprintf(
						__('Only subscriptions billed every week or every month are supported. Current billing schedule: every %d %s(s)', 'bayarcash-wc'),
						$interval,
						$period_text
					));
				}
	...
	private function prepare_enrollment_args( WC_Abstract_Order $order, array $settings): array
	{
		...
		// Get expiry date from subscription product
		$expiry_date = $this->get_subscription_expiry_date($order);
		...
		if ($expiry_date) {
			$args['expiry_date'] = $expiry_date->format('Y-m-d');
		}
	...
	/**
	 * Get the subscription expiry date based on product settings
	 *
	 * @param WC_Abstract_Order $order
	 *
	 * @return ?DateTime
	 */
	private function get_subscription_expiry_date( WC_Abstract_Order $order ): ?DateTime
	{
		foreach ($order->get_items() as $item) {
			$product = $item->get_product();
			...
			// Get subscription length and period
			$length = \WC_Subscriptions_Product::get_length($product);
			$period = \WC_Subscriptions_Product::get_period($product);

			// If no length specified or invalid period, subscription doesn't expire
			if (empty($length) || !in_array($period, ['week', 'month'])) {
				return null;
			}
			...
			// Add subscription length based on period
			switch ($period) {
				case 'week':
					$expiry_date->modify("+{$length} weeks");
					break;
				case 'month':
					$expiry_date->modify("+{$length} months");
					break;
			}
