(function($) {
    $(document).ready(function() {
        ...
        // Function to get FPX payment methods
        function getFpxPaymentMethods() {
            const fpxMethods = [];
            $('textarea[id^="woocommerce_bc-fpx-"][id$="-wc_bearer_token"]').each(function() {
                const id = $(this).attr('id');
                const method = id.match(/woocommerce_(bc-fpx-.*-wc)_bearer_token/);
                if (method && method[1]) {
                    fpxMethods.push(method[1]);
                }
            });
            return fpxMethods;
        }
        ...
        const paymentMethods = [...basePaymentMethods, ...getFpxPaymentMethods()];
