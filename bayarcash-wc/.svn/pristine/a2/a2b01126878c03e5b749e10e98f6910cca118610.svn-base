...
class BayarcashCheckoutFee {
	...
	private function initialize_dynamic_payment_methods(): void
	{
		...
		// Add dynamic FPX payment methods if additional gateways are configured
		if ($additional_fpx > 1) {
			for ($i = 2; $i <= $additional_fpx; $i++) {
				$this->payment_methods[] = "bc-fpx-{$i}-wc";
			}
		}
	}
	...
	public function add_checkout_fee($cart): void {
		...
		$fee_settings = [
			...
			'additional_charge_name' => $settings['additional_charge_name'] ?? '',
		];
		...
		// Calculate the actual total after discounts
		$total_after_discounts = $cart_total - $discount_total + $shipping_total;

		// Skip fee if total after discounts is 0 or negative
		if ($total_after_discounts <= 0) {
			return;
		}

		$fee = 0;
		$fee_name = !empty($fee_settings['additional_charge_name'])
			? $fee_settings['additional_charge_name']
			: __('Bayarcash Processing Fee', 'bayarcash-wc');

		switch ($charge_type) {
			case 'fixed':
				$fee = $charge_amount;
				$fee_label = sprintf('%s (RM %s)', $fee_name, number_format($charge_amount, 2));
				break;
			case 'percentage':
				// Calculate percentage based on total after discounts
				$fee = ($total_after_discounts * $charge_amount) / 100;
				$fee_label = sprintf('%s (%s%%)', $fee_name, $charge_amount);
				break;
			case 'both':
				$fixed_fee = $charge_amount;
				// Calculate percentage based on total after discounts
				$percentage_fee = ($total_after_discounts * $charge_percentage) / 100;
				$fee = $fixed_fee + $percentage_fee;
				$fee_label = sprintf('%s (RM %s + %s%%)', $fee_name, number_format($charge_amount, 2), $charge_percentage);
				break;
		}
