...
class DataStore
{
	...
	private function safe_empty_cart(): void {
		if (defined('DOING_CRON') && DOING_CRON) {
			bayarcash_debug_log('Skipping cart empty during cron job');
			return;
		}

		// Check if WC() is available and has cart
		if (function_exists('WC') && WC() !== null && WC()->cart !== null) {
			WC()->cart->empty_cart();
		} else {
			bayarcash_debug_log('WC cart not available, skipping cart empty');
		}
	}
