...
function popular_posts_options_page(){
	...
	_e('help and instructions', 'popular_posts_plugin'); 
	echo '</a></h2></div>';
	if (!PopularPosts::check_post_plugin_library(__('<h1>Please install the <a href="http://downloads.wordpress.org/plugin/post-plugin-library.zip">Post Plugin Library</a> plugin.</h1>', 'popular_posts_plugin'))) return;
...
function popular_posts_placement_options_subpage(){
	...
	?>
		<div class="wrap">
		<h2><?php _e('Placement Settings', 'popular_posts_plugin'); ?></h2>
		<form method="post" action="">
		<div class="submit"><input type="submit" name="update_options" value="<?php _e('Save Placement Settings', 'popular_posts_plugin') ?>" /></div>
		<table class="optiontable form-table">
			<?php 
...
function popular_posts_other_options_subpage(){
	...
	if (isset($_POST['update_options'])) {
		...
		if (defined('POC_CACHE_4')) poc_cache_flush();
		// Fill up the options with the values chosen...
		$options = ppl_options_from_post($options, array('stripcodes', 'exclude_users', 'handle_cache', 'count_single', 'count_page', 'count_home', 'count_archive', 'count_category', 'count_search', 'count_feed'));
		update_option('popular-posts', $options);
		// Show a message to say we've done something
		echo '<div class="updated fade"><p>' . __('Options saved', 'popular_posts_plugin') . '</p></div>';
	} 
	...
	?>
		...
		<table class="optiontable form-table">
			...
			?>
			<tr valign="top">
				<th scope="row"><?php _e('Exclude views by logged-in users?', 'popular_posts_plugin') ?></th>
				<td>
			...
			<tr valign="top">
				<th scope="row"><?php _e('WP-SuperCache (or similar) is active?', 'popular_posts_plugin') ?></th>
				<td>
				<select name="handle_cache" id="handle_cache">
				<option <?php if($options['handle_cache'] == 'false') { echo 'selected="selected"'; } ?> value="false">No</option>
				<option <?php if($options['handle_cache'] == 'true') { echo 'selected="selected"'; } ?> value="true">Yes</option>
				</select> 
				</td>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e('Count home page views?', 'popular_posts_plugin'); ?></th>
				<td><input type="checkbox" name="count_home" value="true" <?php $ischecked = ($options['count_home']) ? 'checked="checked"' : ''; echo $ischecked ?> /></td>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e('Count single page views?', 'popular_posts_plugin'); ?></th>
				<td><input type="checkbox" name="count_single" value="true" <?php $ischecked = ($options['count_single']) ? 'checked="checked"' : ''; echo $ischecked ?> /></td>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e('Count feed page views?', 'popular_posts_plugin'); ?></th>
				<td><input type="checkbox" name="count_feed" value="true" <?php $ischecked = ($options['count_feed']) ? 'checked="checked"' : ''; echo $ischecked ?> /></td>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e('Count archive page views?', 'popular_posts_plugin'); ?></th>
			...
			<tr valign="top">
				<th scope="row"><?php _e('Count category page views?', 'popular_posts_plugin'); ?></th>
			...
			<tr valign="top">
				<th scope="row"><?php _e('Count search page views?', 'popular_posts_plugin'); ?></th>
				<td><input type="checkbox" name="count_search" value="true" <?php $ischecked = ($options['count_search']) ? 'checked="checked"' : ''; echo $ischecked ?> /></td>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e('Count static page views?', 'popular_posts_plugin'); ?></th>
...
function popular_posts_install() {
	...
	if (!isset($options['handle_cache'])) $options['handle_cache'] = 'false';
	...
	if (!isset($options['feed_parameters'])) $options['feed_parameters'] = 'prefix=<strong>'.__('Popular Posts', 'popular_posts_plugin').':</strong><ul class="popular-posts">&suffix=</ul>';
	if (!isset($options['append_on'])) $options['append_on'] = 'false';
	if (!isset($options['append_priority'])) $options['append_priority'] = '10';
	if (!isset($options['append_parameters'])) $options['append_parameters'] = 'prefix=<h3>'.__('Popular Posts', 'popular_posts_plugin').':</h3><ul class="popular-posts">&suffix=</ul>';
	...
	// setup the tracker files
	$jst_file = file_get_contents(WP_PLUGIN_DIR.'/popular-posts-plugin/pvctracker.js.template');
	$jst_file = str_replace('WP_PLUGIN_URL', WP_PLUGIN_URL, $jst_file);
	$js_handle = fopen(WP_PLUGIN_DIR.'/popular-posts-plugin/pvctracker.js', 'w');
	fwrite($js_handle, $jst_file);
	fclose($js_handle);
	global $table_prefix;
	$phpt_file = file_get_contents(WP_PLUGIN_DIR.'/popular-posts-plugin/pvctracker.php.template');
	$phpt_file = str_replace('DB_HOST', DB_HOST, $phpt_file);
	$phpt_file = str_replace('DB_USER', DB_USER, $phpt_file);
	$phpt_file = str_replace('DB_PASSWORD', DB_PASSWORD, $phpt_file);
	$phpt_file = str_replace('DB_NAME', DB_NAME, $phpt_file);
	$phpt_file = str_replace('TABLE_PREFIX', $table_prefix, $phpt_file);
	$php_handle = fopen(WP_PLUGIN_DIR.'/popular-posts-plugin/pvctracker.php', 'w');
	fwrite($php_handle, $phpt_file);
	fclose($php_handle);
