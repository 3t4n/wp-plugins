...
class PopularPosts {
	...
	function execute($args='', $default_output_template='<li>{link}</li>'){
		if (!PopularPosts::check_post_plugin_library(__('Post-Plugin Library missing', 'popular_posts_plugin'))) return;
...
function pvc_tracker_filter($text) {
	global $post;
	$options = get_option('popular-posts');
	// if we have chosen to exclude anyone logged in we check for a login cookie
	if ('true' === $options['exclude_users'] && empty($_COOKIE[USER_COOKIE])) return $text;
	// this complex condition just checks if we want to count this page...
	if (	
			('true' === $options['count_home'] && is_home()) ||
			('true' === $options['count_single'] && is_single()) ||
		...
		// ... we made it through...
		return '<span class="pvc_count_me'.$post->ID.'" ></span>'.$text;
	}
	return $text;
}

function pvc_tracker_js(){
	echo "<script type='text/javascript' src='" . WP_PLUGIN_URL . "/popular-posts-plugin/pvctracker.js?ver=1.0'></script>";
}
...
function popular_posts_init () {
	...
	$options = get_option('popular-posts');
	if ($options['handle_cache'] === 'true') {
		add_action('wp_head', 'pvc_tracker_js',99);
		add_filter('the_content', 'pvc_tracker_filter');
	} else {
