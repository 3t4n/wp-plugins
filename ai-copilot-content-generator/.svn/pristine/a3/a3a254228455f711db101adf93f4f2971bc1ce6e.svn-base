...
"use strict";
	...
	PostsCreatePage.constructor.prototype.initPostsListTable = function () {
		...
		_this.postsTableObj = _this.postsListTable.DataTable({
			...
			fnDrawCallback : function() {
				/*$('#waicTopicKeywordsList_wrapper .dt-paging')[0].style.display = $('#waicTopicKeywordsList_wrapper .dt-paging  span .wbw-paginate').length > 1 ? 'block' : 'none';
				if (_this.topicKeywordsTable.height() > 250) {
					_this.topicKeywordsTable.closest('.dataTables_scroll').removeClass('waic-bordered-wrapper');
				} else {
					_this.topicKeywordsTable.closest('.dataTables_scroll').addClass('waic-bordered-wrapper');
				}*/
				setTimeout(function () {
	...
	PostsCreatePage.constructor.prototype.initPostsSelectTable = function () {
		...
		_this.postsSelectTableObj = _this.postsSelectTable.DataTable({
			...
			scrollX: false,
			language: {
				emptyTable: waicCheckSettings(_this.langSettings, 'emptyTableSearch'),
				paginate: {
					next: waicCheckSettings(_this.langSettings, 'pageNext'),
					previous: waicCheckSettings(_this.langSettings, 'pagePrev'),
					last: '<i class="fa fa-fw fa-angle-right">',
