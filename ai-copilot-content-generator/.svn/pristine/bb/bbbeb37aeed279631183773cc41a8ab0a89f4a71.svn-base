...
class WaicOpenaiModel extends WaicModel {
	private $apiKey;
	private $sleep = 20;
	private $lastTime = 0;
	...
	{
		if ($stream != null && array_key_exists('stream', $params)) {
			if (!$params['stream']) {
	...
	public function getImage( $params ) {
		...
		$params['size'] = $this->getImageSize($params['size'], $params['model']);
	...
	private function sendRequest($url, $method, $params = array(), $type = '') {
		...
		$results = array('error' => 1, 'tokens' => 0, 'length' => 0, 'data' => '');
		$data = json_decode( $data );
		if (isset($data->error)) {
			$results['msg'] = trim($data->error->message);
			if (empty($result['msg']) && isset($data->error->code) && $data->error->code == 'invalid_api_key') {
			...
			if (isset($data->choices) && is_array($data->choices)) {
				...
				if (empty($results['data'])){
					...
					$results['length'] = $this->getCountWords($results['data']);
				}
			} else {
				if ('image' == $type && isset($data->data) && is_array($data->data)) {
					$results['error'] = 0;
