...
$status = WaicUtils::getArrayValue($task, 'status', 9, 1);
...
//running_task
?>
<div class="waic-status-block waic-status-<?php echo esc_attr($status); ?>" data-status="<?php echo esc_attr($status); ?>"><?php echo esc_html($props['statuses'][$status]); ?></div>
