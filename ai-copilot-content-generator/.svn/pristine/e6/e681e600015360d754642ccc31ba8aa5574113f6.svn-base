...
class WaicFrame extends WaicBaseObject {
	...
	private function _writeLog($data, $type) {
		...
		$data = date('c') . ' ' . $type . ': ' . $data . PHP_EOL;
		file_put_contents(WAIC_LOG_DIR . 'waic' . date('Y-m-d') . '.log', $data, FILE_APPEND);
	}
	public function saveDebugLogging( $debug = '' ) {
