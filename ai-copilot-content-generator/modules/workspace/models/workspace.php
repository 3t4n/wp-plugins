...
class WaicWorkspaceModel extends WaicModel {
	...
	public function doGenerationTasks() {
		...
		WaicFrame::_()->saveDebugLogging('***********************************');
		$this->setRunningFlag();
		$openAI = $this->getModule()->getModel('openai')->init();
		if (!$openAI) {
			return false;
		}
		
		if (!$this->doGenerationTask($tackId, $openAI)) {
			$this->resetRunningFlag();
			return false;
		}
		$this->setStoppingTaskGeneration();
	...
	public function doGenerationTask( $id, $openAI ) {
		$tasksModel = $this->getModule()->getModel('tasks');
		...
		$openAI->setApiOptions(WaicUtils::getArrayValue($task['params'], 'api', array(), 2));
		$model = $module->getModel();
		$task = $this->controlSteps($task, $model);
		//if (!empty($task['steps']) && $task['steps'] > 3) { 
			set_time_limit(0);
		//}
		$results = $module->getModel()->doGeneration($task, $openAI);
		$task = $this->controlSteps($task, $model);
		if (is_array($results)) {
