...
class WaicPostscreateModel extends WaicModel {
	...
	public function setOpenAI( $openAI ) {
		$this->openAI = $openAI;
	}
	...
	public function getFields( $feature = 'postscreate' ) {
		...
		if (is_null($this->_fields)) {
			...
			$custom = WaicFrame::_()->getModule('workspace')->getTaxonomiesList('post');
			if (!empty($custom)) {
	...
	public function prepareGeneration( $task, $clear = true, $params = false ) {
		...
		foreach ($topics as $tk) {
			...
			//$fields = WaicUtils::getArrayValue($params, 'fields', array(), 2);
			$results = $this->prepareResults( $fields, $vars );
	...
	public function prepareResults( $params, &$vars ) {
					...
					case 'date':
						...
						switch ($mode) {
							...
							case 'random':
								...
								break;
							case 'period':
								$dt = WaicUtils::getArrayValue($field, 'period');
								break;
						}
						if (!empty($dt)) {
							$vars['pub_mode'] = 2;
	...
	public function doGeneration( $task, $openAI ) {
		$taskId = $task['id'];
	...
	public function generateResults( $data, $opts, $field ) {
		...
		$result = 'image' == $field ? $this->openAI->getImage($opts) : $this->openAI->getText($opts);
		if ($result['error']) {
			$data['s'] = 2;
			$data['m'] = $result['msg'];
		} else {
				...
				case 0:
					...
					if (strpos($var, '#')) {
						for ($i = 1; $i <= $maxSteps; $i++) {
							$rep = WaicUtils::getArrayValue($params, $var . $i);
							if (!empty($rep)) {
								$prompt = str_replace('{' . $var . $i . '}', $rep, $prompt);
							} else {
								break;
							}
						}
