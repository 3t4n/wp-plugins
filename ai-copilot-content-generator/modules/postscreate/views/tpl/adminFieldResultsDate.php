	...
	<div class="wbw-settings-fields col-10 waic-editable">
		...
		<?php 
			if (empty($pcData['post_id'])) {
				...
				$post = get_post($pcData['post_id']);
				if ($post) {
				?>
				<a href="<?php echo esc_url(get_post_permalink($pcData['post_id'])); ?>" target="_blank" class="waic-post-link"><?php echo esc_html($post->post_date); ?></a>
			<?php } ?>
		<?php } ?>
	</div>
</div>