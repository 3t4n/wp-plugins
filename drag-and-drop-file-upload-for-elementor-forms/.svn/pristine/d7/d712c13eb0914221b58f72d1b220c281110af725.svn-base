...
class Superaddons_EL_File_Uploads extends \ElementorPro\Modules\Forms\Fields\Field_Base {
	...
	public function update_controls( $widget ) {
		...
		$check_pro = get_option( '_redmuber_item_3353');
		if($check_pro == "ok") {
			$field_controls = [
				...
				'file_upload_dropbox' => [
					...
					'label' => esc_html__( 'Upload file to Dropbox', 'drag-and-drop-file-upload-for-elementor-forms' ),
					'type' => Controls_Manager::SWITCHER,
					'condition' => [
						'field_type' => $this->get_type(),
					],
				...
				'file_upload_dropbox_access_token' => [
					'name' => 'file_upload_dropbox_access_token',
					'label' => esc_html__( 'Dropbox access token', 'drag-and-drop-file-upload-for-elementor-forms' ),
					'type' => Controls_Manager::TEXTAREA,
					'condition' => [
						'file_upload_dropbox' => "yes",
						'field_type' => $this->get_type(),
					],
					'description' => esc_html__( 'See more:', 'drag-and-drop-file-upload-for-elementor-forms' ).'<a href="https://add-ons.org/creating-a-new-application-on-dropbox-for-upload-files/" target="_blank">https://add-ons.org/creating-a-new-application-on-dropbox-for-upload-files/</a>',
					'tab' => 'content',
					'inner_tab' => 'form_fields_content_tab',
					'tabs_wrapper' => 'form_fields_tabs',
				],
			...
			$field_controls = [
				...
				'file_upload_save_media_a' => [
					'name' => 'file_upload_save_media_a',
	...
	public function render( $item, $item_index, $form ) {
		...
		$nonce = "";
		if( isset($item['file_upload_dropbox']) && $item['file_upload_dropbox'] == "yes") {
			if( isset($item['file_upload_dropbox_access_token']) && $item['file_upload_dropbox_access_token'] !="" ) {
				$nonce = "dd9".$item['file_upload_dropbox_access_token']."iuz";
			}
		}
        ?>
        <div class="elementor-dragandrophandler-container">
            <div class="elementor-dragandrophandler" data-nonce="<?php echo esc_attr($nonce) ?>" data-type="<?php echo esc_attr( $allowed_size ) ?>" data-size="<?php echo esc_attr( $size ) ?>" data-max="<?php echo esc_attr( $max ) ?>">
	...
	function elementor_file_upload_remove(){
		if ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET[ 'nonce' ] ) ), 'elementor_file_upload' ) ) {
		...
		if ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST[ 'nonce' ] ) ), 'elementor_file_upload' ) ) {
			...
			if ( is_dir( $uploads_dir ) && is_writable( $uploads_dir ) ) {
				...
				if ( false !== $move_new_file ) {
					...
					//upload to dopbox
					if( isset($_REQUEST["type_nonce"]) && $_REQUEST["type_nonce"] != ""){
						require_once SUPERADDONS_FILE_UPLOAD_PLUGIN_PATH."fields/dropbox.php";
						$type_nonce = sanitize_text_field($_REQUEST["type_nonce"]);
						Yeeaddons_EL_Dropbox_API::uppload_files($this->get_upload_dir($filename),$filename,$type_nonce);
					}
    ...
    function add_lib(){	
        wp_enqueue_script("elementor_file_upload",SUPERADDONS_FILE_UPLOAD_PLUGIN_URL."assets/js/drap_drop_file_upload.js",array("jquery"));
        ...
        wp_enqueue_style("repeater_file_upload",SUPERADDONS_FILE_UPLOAD_PLUGIN_URL."assets/css/drap_drop_file_upload.css");
	}
}