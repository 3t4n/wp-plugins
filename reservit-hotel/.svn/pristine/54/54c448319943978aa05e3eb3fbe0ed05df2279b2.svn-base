...
*/
    function set_reservit_window_closed(){
        ...
        $reservitsafepost = isset($_POST['RsvitWidgetboxClosed']) && !empty($_POST['RsvitWidgetboxClosed']) ? stripslashes( wp_strip_all_tags($_POST['RsvitWidgetboxClosed'])) : '';
        if( $reservitsafepost =="yes"){
            $_SESSION['RsvitWidgetboxClosed']="$reservitsafepost";
        }
    }
    
    if (isset($_POST['RsvitWidgetboxClosed']) && !empty($_POST['RsvitWidgetboxClosed'])){
