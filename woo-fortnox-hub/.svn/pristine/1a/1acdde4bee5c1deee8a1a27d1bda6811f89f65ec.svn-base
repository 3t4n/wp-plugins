...
if (!class_exists('Fortnox_API', false)) {
    class Fortnox_API
    {
        ...
        const PROXY_SERVICE_URL = 'proxy.bjorntech.io';
        ...
        public function get_access_token($force = false)
        {
            ...
            if ($refresh_token && false === ($access_token = get_fortnox_hub_transient('fortnox_accesstoken'))) {
                ...
                WC_FH()->logger->add(sprintf('Getting access token from %s', $url));
                ...
                if (is_wp_error($response)) {
                    ...
                    WC_FH()->logger->add(sprintf('Got access token response: %s', json_encode($response_body, JSON_INVALID_UTF8_IGNORE)));
                    ...
                    if (isset($response_body->tenant_uuid)) {
                        update_option('fortnox_tenant_uuid', $response_body->tenant_uuid);
                        WC_FH()->logger->add(sprintf('Got tenant uuid "%s"', $response_body->tenant_uuid));
                    }
        ...
        {
            // Add the DNS servers hook before making the request
            add_action('http_api_curl', array($this, 'set_dns_servers'), 10, 3);
            
            $accelerate_connection = wc_string_to_bool(get_option('fortnox_hub_accelerate_connection')) && !$bypass_proxy;

            if ($bypass_proxy || wc_string_to_bool(get_option('fortnox_hub_bypass_proxy'))){
                if ($accelerate_connection){
                    $url = 'https://' . self::PROXY_SERVICE_URL . '/' . $entity;
                } else {
                    $url = 'https://' . self::DIRECT_SERVICE_URL . '/' . $entity;
                }
            } else {
            ...
            if ($accelerate_connection) {
                $args['headers']['x-proxy-key'] = get_option('fortnox_tenant_uuid') ? get_option('fortnox_tenant_uuid') : get_option('fortnox_authorization_code');
                $args['headers']['x-proxy-url'] = 'https://' . 'api.fortnox.se';
            }

            if (wc_string_to_bool(get_option('fortnox_hub_log_requests'))) {
                WC_FH()->logger->add(sprintf('Request to URL %s: %s', $url, json_encode($args, JSON_INVALID_UTF8_IGNORE)));
            }
            
            $response = wp_remote_request($url, $args);

            if (wc_string_to_bool(get_option('fortnox_hub_log_requests'))) {
                WC_FH()->logger->add(sprintf('Response from URL %s: %s', $url, json_encode($response, JSON_INVALID_UTF8_IGNORE)));
            }
        ...
        /**
         * Sets Cloudflare DNS servers for cURL requests
         * 
         * @param \CurlHandle|false $handle The cURL handle
         * @param array $parsed_args The request arguments
         * @param string $url The request URL
         */
        public function set_dns_servers($handle, $parsed_args, $url) {
            if ('yes' == get_option('fortnox_hub_dont_use_improved_curl')) {
                return;
            }

            // Check if cURL version is at least 7.24.0 which is required for CURLOPT_DNS_SERVERS
            $curl_version = curl_version();
            if (empty($curl_version) || version_compare($curl_version['version'], '7.24.0', '<')) {
                return;
            }

            //Check that constant CURLOPT_DNS_SERVERS exists
            if (!defined('CURLOPT_DNS_SERVERS')) {
                return;
            }

            //Check that constant CURLOPT_HTTP_VERSION exists
            if (!defined('CURLOPT_HTTP_VERSION')) {
                return;
            }

            // Only set the DNS servers for requests that are made by Fortnox Hub
            if (isset($parsed_args['is_bt_fortnox_request']) && $parsed_args['is_bt_fortnox_request']) {
                curl_setopt($handle, CURLOPT_DNS_SERVERS, '1.1.1.1,1.0.0.1,8.8.8.8,8.8.4.4');
        ...
        public function getAllCustomers($only_active = true, $last_modified = false, $filters = false)
        {
            ...
            $returnarray = array();
            do {
                $jsonarray = $this->apiCall('GET', '3/customers/?limit=' . self::DEFAULT_LIMIT . '&page=' . $i . '&sortby=customernumbernumber&sortorder=ascending' . $active_field . $modified_field);
                if ($filters !== false) {
        ...
        public function get_all_articles($only_active = true, $last_modified = false, $filters = false)
        {
            ...
            $returnarray = array();
            do {
                $jsonarray = $this->apiCall('GET', '3/articles/?limit=' . self::DEFAULT_LIMIT . '&page=' . $i . '&sortby=articlenumber&sortorder=ascending' . $active_field . $modified_field);
                if ($filters !== false) {
        ...
        public function getAllAccounts()
        {
            ...
            $returnarray = array();
            do {
                $jsonarray = $this->apiCall('GET', '3/accounts/?limit=' . self::DEFAULT_LIMIT . '&page=' . $i);
        ...
        public function get_all_invoices($last_modified = null)
        {
            ...
            $returnarray = array();
            do {
                $jsonarray = $this->apiCall('GET', '3/invoices/?limit=' . self::DEFAULT_LIMIT . '&sortby=documentnumber&sortorder=ascending&page=' . $i . $modified_field);
        ...
        {
            if (get_option('fortnox_warehouse_use_new_api') == 'yes') {
                return $this->warehouse_stockbalance($item_id, $stockplace);
            } else {
                return $this->apiCall('GET', 'api/warehouse/stockpoints-v1/itemsummary/' . $item_id . ($stockplace ? ('?q=' . $stockplace) : ''));
            }
        }

        public function warehouse_stockbalance($item_id, $stockplace = false)
        {
            $response = $this->apiCall('GET', 'api/warehouse/status-v1/stockbalance?itemIds=' . $item_id . ($stockplace ? ('&stockPointCodes=' . $stockplace) : ''));

            $stockpoints = apply_filters('fortnox_get_stockpoints', false);
            ...
            if (is_string($response) && $response == '[]') {
                ...
                if ($stockplace) {
                    ...
                    $stockpoint = current(array_filter($stockpoints, function($stockpoint) use ($stockplace) {
                        return $stockpoint['code'] === $stockplace;
                    }));

                    if ($stockpoint) {
                        $response[] = array(
                            'stockPointId' => $stockpoint['id'],
                            'stockPointCode' => $stockpoint['code'],
            ...
            for ($i = 0; $i < count($mapped_response); $i++) {
                $item = $mapped_response[$i];
                $stockpointCode = $item['stockPointCode'];
                $stockpoint = current(array_filter($stockpoints, function($stockpoint) use ($stockpointCode) {
                    return $stockpoint['code'] === $stockpointCode;
                }));

                if ($stockpoint) {
                    $item['stockPointName'] = $stockpoint['name'];
                    $item['stockPointCode'] = $stockpoint['code'];
                } else {
                    $item['stockPointName'] = 'N/A';
                    $item['stockPointCode'] = 'N/A';
                }

                $item['availableQuantity'] = $item['availableStock'];

                $mapped_response[$i] = $item;
