...
if (!class_exists('Woo_Fortnox_Hub_Invoice_Status_Handler', false)) {
    ...
    class Woo_Fortnox_Hub_Invoice_Status_Handler
    {
        ...
        public function __construct()
        {
            ...
            add_action('fortnox_process_changed_invoices', array($this, 'process_changed_invoices_queue'));
            add_action('fortnox_process_changed_invoices_single', array($this, 'process_changed_invoices'));
            ...
            add_action('wp_ajax_fortnox_check_invoices_email_turned_on', array($this, 'ajax_check_invoices_email_turned_on'));
        }

        public function process_changed_invoices_queue($invoice_number, $cron_job = false){
            if (!wc_string_to_bool(get_option('fortnox_hub_accelerate_connection')) && !$cron_job) {
                WC_FH()->logger->add(sprintf('process_changed_invoices_queue: Accelerate sync is enabled, skipping'));
                do_action('fortnox_process_changed_invoices_single', $invoice_number);
            }

            if (as_has_scheduled_action('fortnox_process_changed_invoices_single', array($invoice_number))) {
                WC_FH()->logger->add(sprintf('process_changed_invoices_queue: Action already scheduled for invoice %s', $invoice_number));
            } else {
                WC_FH()->logger->add(sprintf('process_changed_invoices_queue: Scheduling async action for invoice %s', $invoice_number));
                as_schedule_single_action(time(),'fortnox_process_changed_invoices_single', array($invoice_number));
            }
        }
        ...
        public function get_settings($settings, $current_section)
        {
            if ('invoice_status' == $current_section) {
                ...
                $settings[] = [
                    ...
                    'type' => 'title',
                    'id' => 'fortnox_order_completion_options',
                ];

                foreach (WCFH_Util::get_available_payment_gateways() as $key => $payment_gateway) {
                    $settings[] = [
                        'title' => sprintf(__('Automatic order completion for %s', 'woo-fortnox-hub'), $payment_gateway->get_title()),
                        'type' => 'select',
                        ...
                        'default' => (get_option('fortnox_fortnox_set_order_status') ? get_option('fortnox_fortnox_set_order_status') : ''),
                        'options' => array(
                        ...
                        'id' => 'fortnox_fortnox_set_order_status_' . $key,
                    ];
                }
                ...
                $accrual_and_cash_title = __('Automatically bookkeep invoices', 'woo-fortnox-hub');

                if ('CASH' === $accounting_method) {
                    $accrual_and_cash_title = __('Automatically set invoices as printed', 'woo-fortnox-hub');
                } 

                $settings[] = [
                    'title' => $accrual_and_cash_title,
                    'type' => 'title',
                    'id' => 'fortnox_bookkeeping_options',
                ];
                ...
                $settings[] = [
                    'title' => __('Automatically set invoices to paid', 'woo-fortnox-hub'),
                    'type' => 'title',
                    'id' => 'fortnox_paid_options',
                ];
        ...
        public function ajax_check_invoices_email_turned_on()
        {

            $response = $response = array(
                'result' => 'success',
                'is_email_turned_on' => false,
                'message' => __('No payment gateways have email turned on.', 'woo-fortnox-hub'),
            );

            if (!(empty($payment_gateways = WCFH_Util::get_available_payment_gateways()))) {
                foreach ($payment_gateways as $key => $payment_gateway) {
                    if (wc_string_to_bool(get_option('fortnox_send_customer_email_invoice_' . $key))) {
                        $response = array(
                            'result' => 'success',
                            'is_email_turned_on' => true,
                            'message' => sprintf(__('Automatic sending of invoices via email is turned on (under WooCommerce -> Settings -> Fortnox Hub -> Customers). Syncing old invoices could trigger emails to be sent out to customers. Are you sure you want to continue?', 'woo-fortnox-hub')),
                        );
        ...
        public function ajax_check_invoices()
        {
            ...
            WC_FH()->logger->add(sprintf( 'ajax_check_invoices: Checking invoices for %s days', $_POST['sync_days']));
        ...
        public function maybe_set_wc_order_to_completed($order, $order_id)
        {
            ...
            $payment_method = WCFH_Util::get_payment_method($order, 'maybe_set_wc_order_to_completed');

            WC_FH()->logger->add(sprintf('maybe_set_wc_order_to_completed (%s): Using payment method %s', $order_id, $payment_method));
            if (get_option('fortnox_fortnox_set_order_status_' . $payment_method)) {
                $set_order_status_setting = get_option('fortnox_fortnox_set_order_status_' . $payment_method);
                WC_FH()->logger->add(sprintf('maybe_set_wc_order_to_completed (%s): Using payment method specific setting %s for order status', $order_id, $set_order_status_setting));
            } else {
                $set_order_status_setting = get_option('fortnox_fortnox_set_order_status');
                WC_FH()->logger->add(sprintf('maybe_set_wc_order_to_completed (%s): Using global setting %s for order status', $order_id, $set_order_status_setting));
            }

            $set_order_status = apply_filters('fortnox_filter_set_wc_order_to_completed', $set_order_status_setting, $order, self::$fn_invoice);
        ...
        public function maybe_capture_klarna_order($order, $order_id) {
            ...
            try {
                ...
                WC_FH()->logger->add(sprintf('maybe_capture_klarna_order (%s): Klarna order %s capture failed', $order_id, $e->getMessage()));
                Fortnox_Notice::add(sprintf('Klarna order %s capture failed', $order_id),'error','klarna_capture_error',true);
            }
            ...
            try {
                    ...
                    if ('order' === $wc_order_creates) {
                        ...
                        if (($payment_remark = get_option('fortnox_invoice_payment_remark_' . $payment_method, 'yes')) && $order->get_date_paid()) {
                            ...
                            if (false !== strpos($payment_remark, 'custom_remark')) {
                                $custom_remark[] = get_option('fortnox_invoice_payment_custom_remark_' . $payment_method) ? get_option('fortnox_invoice_payment_custom_remark_' . $payment_method) : '';
            ...
            try {
                ...
                if (0 != ($total_to_sync = count($invoices))) {
                    ...
                    foreach ($invoices as $invoice) {
                        ...
                        do_action('fortnox_process_changed_invoices', $invoice['DocumentNumber'], true);
                    }
                }
