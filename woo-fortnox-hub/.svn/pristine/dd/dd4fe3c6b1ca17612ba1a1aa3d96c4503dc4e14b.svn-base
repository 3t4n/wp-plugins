    ...
    class Fortnox_Hub_Zettle_Handler
    {
        ...
        public function schedule_fortnox_sync_zettle()
        {
            ...
            if ('yes' === get_option('fortnox_create_izettle_payout_invoices')) {
                if (!as_has_scheduled_action('fortnox_izettle_sync')) {
        ...
        public function terms_of_payments_options($choice)
        {
            $izettle_array = array(
                ...
                'izettle_card_online' => new Fortnox_Hub_Mock_Payment_Gateway(__('Zettle card online', 'woo-fortnox-hub')),
            );
        ...
        private function process_izettle_payout($startdate, $enddate = null)
        {
            ...
            try {
                ...
                foreach ($payout_transactions as $payout_transaction) {
                    ...
                    $notice_id = 'fortnox_izettle_sync' . $payout_transaction->originatingTransactionUuid;
                    ...
                    if (!$fortnox_invoice) {
                        ...
                        if (!Fortnox_Notice::get($notice_id)) {
                            Fortnox_Notice::add(sprintf(__('Successfully created Fortnox Invoice %s from Zettle payouts on %s', 'woo-fortnox-hub'), $invoice_created['DocumentNumber'], $booking_date), 'success', $notice_id);
                        }
                        continue;
                    }
                    ...
                    if (!rest_sanitize_boolean($fortnox_invoice['Booked']) && !rest_sanitize_boolean($fortnox_invoice['Cancelled']) && !$fortnox_invoice['FinalPayDate']) {
                        ...
                        if (!Fortnox_Notice::get($notice_id)) {
                            Fortnox_Notice::add(sprintf(__('Successfully updated Fortnox Invoice %s from Zettle payouts on %s', 'woo-fortnox-hub'), $invoice_created['DocumentNumber'], $booking_date), 'success', $notice_id);
                        }
                        continue;
                    }
