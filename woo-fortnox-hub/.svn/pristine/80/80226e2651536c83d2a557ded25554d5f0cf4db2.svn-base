    ...
    class Fortnox_Notice
    {
        ...
        public static function add($message, $type = 'error', $id = false, $dismiss = true, $valid_to = false)
        {
            ...
            if ('yes' != get_option('fortnox_disable_notices')) {
                ...
                if ($type === 'error' && 'yes' === get_option('fortnox_send_error_email')   ) {
                    self::trigger_error_email($message);
                }
            }
        ...
        private static function trigger_error_email($message)
        {
            $mailer = WC()->mailer();
            $email = $mailer->emails['WC_Email_Fortnox_Sync_Error'];
            WC_FH()->logger->add(sprintf('trigger_error_email: Maybe sending email %s', $message));
            if ($email) {
                $email->trigger($message);
                WC_FH()->logger->add(sprintf('trigger_error_email: Email sent %s', $message));
            }
        }
    }
