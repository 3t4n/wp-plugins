...
if (!class_exists('Woo_Fortnox_Hub_Document', false)) {
    ...
    class Woo_Fortnox_Hub_Document
    {
        ...
        public function create_shipping_row($price, $shipping_item, $order, $tax_percent = '', $is_credit = false)
        {
            ...
            if (wc_string_to_bool(get_option('fortnox_skip_shipping_in_description'))) {
                $shipping_description = sprintf(__('%s', 'woo-fortnox-hub'), $shipping_item->get_method_title());
            } else {
                $shipping_description = sprintf(__('Shipping - %s', 'woo-fortnox-hub'), $shipping_item->get_method_title());
            }

            if ($shipping_has_no_price && !wc_string_to_bool(get_option('fortnox_always_populate_shipping'))) {
                $row = array(
                    "DeliveredQuantity" => 'API_BLANK',
                    "Description" => WCFH_Util::clean_fortnox_text($shipping_description, 50),
        ...
        public function get_details($order, $refund = true)
        {
            ...
            $payment_data = array_merge(array(
                "DeliveryDate" => (('yes' == get_option('fortnox_skip_delivery_date')) ? 'API_BLANK' : date('Y-m-d', $date_created->date('U') + ($delivery_correct * (24 * 60 * 60)))),
