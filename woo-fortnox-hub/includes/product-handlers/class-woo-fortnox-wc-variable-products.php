...
if (!class_exists('Fortnox_Hub_WC_Variable_Products', false)) {
    ...
    class Fortnox_Hub_WC_Variable_Products
    {
        ...
        public function maybe_enrich_wc_variable_product($row_in, $order_item, $order)
        {
            ...
            if (wc_string_to_bool(get_option('fortnox_wc_product_update_variable_parent_enrich_only_variant'))) {
                $row_in['Description'] = WCFH_Util::clean_fortnox_text($product->get_name(),50);
                WC_FH()->logger->add(sprintf('maybe_enrich_wc_variable_product (%s): Successfully added product name to row', $order->get_id()));
                WC_FH()->logger->add(sprintf('maybe_enrich_wc_variable_product (%s): Row after enrichment: %s', $order->get_id(), json_encode($row_in)));
                return $row_in;
            } else {
                $new_row = [];

                $new_row['Description'] = WCFH_Util::clean_fortnox_text($product->get_name(), 50);
                ...
                WC_FH()->logger->add(sprintf('maybe_enrich_wc_variable_product (%s): Row before enrichment: %s', $order->get_id(), json_encode($rows)));
