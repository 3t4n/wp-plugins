...
if (!class_exists('Woo_Fortnox_Hub_WC_Product_Handler', false)) {
    ...
    class Woo_Fortnox_Hub_WC_Product_Handler
    {
        ...
        public function price_settings()
        {
            ...
            $settings = apply_filters('fortnox_wc_prices_settings', $settings, $pricelists);
                ...
                if ((!$products_from_wc && $article_number) || 'always_create' == $products_from_wc) {
                    ...
                    try {
                        ...
                        if ($stocklevel_set = in_array($stock_option, array('manually', 'always'))) {
                            ...
                            if ('yes' == get_option('fortnox_warehouse_apply_primary_stockplace_to_all_products') && get_option('fortnox_warehouse_primary_stockplace')) {
                                $new_article['DefaultStockPoint'] = get_option('fortnox_warehouse_primary_stockplace');
                                WC_FH()->logger->add(sprintf('Updating Default Stock Point in Fortnox to "%s"', get_option('fortnox_warehouse_primary_stockplace')));
                            }
                        }
            ...
            if ($is_checked || WCFH_Util::is_syncable($product)) {
                ...
                if ($article = $this->get_or_create_article($product)) {
                    ...
                    if ($pricelist = get_option('fortnox_wc_product_pricelist')) {
                        $this->update_price($article, WCFH_Util::get_regular_price($product), $pricelist, $product, 'price');
                    }

                    if ($pricelist = get_option('fortnox_wc_product_sale_pricelist')) {
                        $this->update_price($article, WCFH_Util::get_sale_price($product), $pricelist, $product, 'sale_price');
                    }

                    do_action('fortnox_wc_prices_sync', $article, $product, $this);
        ...
        public function create_article_data($product, $article, $sync_all = false)
        {
            ...
            $is_variant = $product->get_type() == 'variation';

            if ($is_variant) {
                $current_name = WCFH_Util::generate_variant_identifier($current_name, $product);
            }
            ...
            if ('yes' == get_option('fortnox_warehouse_apply_primary_stockplace_to_all_products') && $article['DefaultStockPoint'] != get_option('fortnox_warehouse_primary_stockplace')) {
                $article_data['DefaultStockPoint'] = get_option('fortnox_warehouse_primary_stockplace');
                WC_FH()->logger->add(sprintf('Updating Default Stock Point in Fortnox to "%s"', get_option('fortnox_warehouse_primary_stockplace')));
                $changed = true;
            }
