...
class BVAccountCallback extends BVCallbackBase {
	...
	function process($request) {
		...
		case "addacc":
			SVAccount::addAccount($settings, $params['public'], $params['secret']);
			$resp = array("status" => SVAccount::exists($settings, $params['public']));
			break;
		case "rmacc":
			$resp = array("status" => SVAccount::remove($settings, $params['public']));
			break;
		case "updt":
			$account->updateInfo($params);
			$resp = array("status" => SVAccount::exists($settings, $params['pubkey']));
			break;
		case "updtapikey":
			SVAccount::updateApiPublicKey($settings, $params['pubkey']);
			$resp = array("status" => $settings->getOption(SVAccount::$api_public_key));
			break;
		case "rmbvscrt":
			$resp = array("status" => SVRecover::deleteDefaultSecret($settings));
			break;
		case "rmbvkeys":
		...
		case "rmoldbvacc":
			...
			break;
		case "fetch":
			$accounts = SVAccount::allAccounts($settings);
