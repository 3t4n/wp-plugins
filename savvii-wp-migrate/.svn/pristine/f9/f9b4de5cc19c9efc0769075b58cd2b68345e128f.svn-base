...
add_action('sv_clear_bv_services_config', array($wp_action, 'clear_bv_services_config'));
...
if ((array_key_exists('bvplugname', $_REQUEST)) && ($_REQUEST['bvplugname'] == "savvii")) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	$pubkey = isset($_REQUEST['pubkey']) ? SVAccount::sanitizeKey(wp_unslash($_REQUEST['pubkey'])) : '';
