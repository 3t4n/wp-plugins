...
require_once plugin_dir_path( __FILE__ ) . '/../includes/class-ovas-connectapi.php';
require_once plugin_dir_path( __FILE__ ) . '/includes/class-ovas-connect-settings.php';
require_once plugin_dir_path( __FILE__ ) . '/includes/class-ovas-connect-datafieldtype.php';
/**
...
class Ovas_Connect_Admin {
	...
	public function ovas_connect_add_settings_page() {
		add_menu_page(
			...
			'data:image/svg+xml;base64,' . base64_encode( file_get_contents( plugins_url( 'ovas-connector/admin/images/o64.svg' ) ) )
		);
	}
	...
	public function cb_display_api_information( array $params ) {
		if ( $this->options ) {
			...
			if ( $body && $body->request->status->success === true ) {
				$modules = '';
