...
function FGW_free_contain() {
  global $fgw_comman;
 	...
 	<div class="fgw_container">
      ...
      <?php } 
        if(isset($_REQUEST['message'])  && $_REQUEST['message'] == 'error'){ ?>
          <div class="notice notice-error is-dismissible">
            <p><?php _e('Error: Add valid products in "Gift Products" section.', 'free-gifts-for-woocommerce'); ?></p>
          </div>
      <?php } ?>
   		<form method="post">
              ...
              <div class="inside">
                <table>
                  ...
                  <tr>
                    ...
                    <td>
                      <input type="radio" name="fgw_comman[fgw_gift_prod_display]" value="after_cart_table" <?php if($fgw_comman['fgw_gift_prod_display'] == 'after_cart_table' ) { echo 'checked'; } ?>><?php _e( 'List View', 'free-gifts-for-woocommerce-pro' ); ?>
                      <input type="radio" name="fgw_comman[fgw_gift_prod_display]" value="popup" <?php if($fgw_comman['fgw_gift_prod_display'] == 'popup' ) { echo 'checked'; } ?>><?php _e( 'Popup', 'free-gifts-for-woocommerce-pro' ); ?>
                    </td>
                  </tr>
                  <tr>
                    <th>
                ...
                <table>
                  <tr>
                    ...
                    <td>
                        ...
                        <table class="fgw_tbl1">
                          ...
                          for($i=0; $i<count($table_fgw_combo_custom); $i++) {    
                            ...
                            <tr>
                              ...
                              <td>
                                <a class=" button-primary fgfw-button-primary ">Add</a>
                              </td>
                              <?php
                              }
                              ?>
                          ...
                          for($i=0; $i<count($table_minimum_price); $i++) {    
                            ...
                            <tr>
                              ...
                              <td>
                                ...
                                <input  type="number" step="0.01" name="minimum_price[]" value="<?php echo  esc_attr($table_minimum_price[$i])?>">
                              </td>
                              <td>
                                <label class="max_qnt">Maximum Price</label>
                                <input type="number" step="0.01" name="maximum_price[]" value="<?php echo  esc_attr($table_maximum_price[$i])?>">
                              </td>
                              <td>
                                <label>Number Of Allowed Gift</label>
                              ...
                              <td>
                                <a class="addrow button-primary  fgfw-button-primary">Add</a>
                              </td>
                              <?php
                              }
                              ?>
                      ...
                      <label id="pro-version-message" class="fgw_comman_link"><?php echo __('Multiple product availabel Only available in pro version So have to take pro version ',  'free-gifts-for-woocommerce' ); ?><a href="https://www.plugin999.com/plugin/free-gifts-product-for-woocommerce/" target="_blank"><?php echo __( 'link',  'free-gifts-for-woocommerce'); ?></a></label>
            ...
            <div class="postbox">
              ...
              <div class="inside">
                <table>
                  ...
                  <tr>
                    <th>
                      <?php echo __( 'Number of item show in List View',  'free-gifts-for-woocommerce' ); ?>
                    </th>
  ...
  <script  type="application/html" id="fgw_child_div_price_clone">
          <tr>
            ...
            <td>
              <label class="min_qnt">Minimum Price</label>
              <input  type="number" step="0.01" name="minimum_price[]" value="">
            </td>
            <td>
              <label class="max_qnt">Maximum Price</label>
              <input type="number" step="0.01" name="maximum_price[]" value="">
            </td>
            <td>
              <label>Number Of Allowed Gift</label>
...
function FGW_product_ajax() {
  if ( ! isset( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], 'fgw_ajax_nonce' ) ) {
...
function FGW_cats_ajax() {
  ...
  $cat_args = array(
    ...
    // 'hide_empty' => 0, // Change this to 1 if you want to hide empty categories
  );
...
function FGW_save_options(){
	if( current_user_can('administrator') ) {
        ...
        $fgw_gift_rule = $_REQUEST['fgw_comman']['fgw_gift_rule'];

        $is_error = false;
        if($fgw_gift_rule == 'custom'){
          // for product
          $fgw_combo_custom_multiple   = FGW_recursive_sanitize_text_field( $_REQUEST['fgw_combo_custom'] );
          $fgw_gift_custom_multiple   = FGW_recursive_sanitize_text_field( $_REQUEST['fgw_gift_multiple_custom'] ); 

          $fgw_combo_filtered_array = array_filter($fgw_combo_custom_multiple, function($value) {
            return empty($value) && trim($value) == '';
          });

          $fgw_gift_filtered_array = array_filter($fgw_gift_custom_multiple, function($value) {
            return empty($value) && trim($value) == '';
          });

          if (!empty($fgw_combo_filtered_array) || !empty($fgw_gift_filtered_array)) {
            $is_error = true;
          }
        } elseif ($fgw_gift_rule == 'category') {
          // for category
          $fgw_select_cats   = FGW_recursive_sanitize_text_field( $_REQUEST['fgw_select_cats_category'] );   
          $fgw_gift_multiple   = FGW_recursive_sanitize_text_field( $_REQUEST['fgw_gift_multiple_category'] ); 

          $fgw_cats_filtered_array = array_filter($fgw_select_cats, function($value) {
            return empty($value) && trim($value) == '';
          });

          $fgw_gift_filtered_array = array_filter($fgw_gift_multiple, function($value) {
            return empty($value) && trim($value) == '';
          });

          if (!empty($fgw_cats_filtered_array) || !empty($fgw_gift_filtered_array)) {
            $is_error = true;
          } 
          ...
          if (!empty($fgw_gift_filtered_array)) {
            $is_error = true;
          } 
        } else {
          $is_error = true;
        ...
        if ($is_error) {
          ...
          // Show error notice if the array is empty or contains only empty values
          wp_redirect( admin_url( '/admin.php?page=free_gift&message=error' ) );
          exit;
