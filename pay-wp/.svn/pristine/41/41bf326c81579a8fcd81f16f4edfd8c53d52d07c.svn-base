...
final class SerializationGraphNavigator extends GraphNavigator
{
    ...
    public function __construct(MetadataFactoryInterface $metadataFactory, HandlerRegistryInterface $handlerRegistry, AccessorStrategyInterface $accessor, ?EventDispatcherInterface $dispatcher = null, ?ExpressionEvaluatorInterface $expressionEvaluator = null)
    {
    ...
    public function accept($data, ?array $type = null)
    {
        ...
        if ($this->visitor instanceof NullAwareVisitorInterface && \true === $this->visitor->isNull($data)) {
            $type = ['name' => 'NULL', 'params' => []];
        }
        switch ($type['name']) {
            case 'NULL':
