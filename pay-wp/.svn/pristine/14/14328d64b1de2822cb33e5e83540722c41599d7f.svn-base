...
use WPPayVendor\JMS\Serializer\Annotation\UnionDiscriminator;
...
class AnnotationOrAttributeDriver implements DriverInterface
{
    ...
    public function loadMetadataForClass(\ReflectionClass $class): ?BaseClassMetadata
    {
        ...
        foreach ($this->getClassAnnotations($class) as $annot) {
            ...
            if ($annot instanceof ExclusionPolicy) {
                ...
                $virtualPropertyMetadata = new ExpressionPropertyMetadata($name, $annot->name, $this->parseExpression($annot->exp));
        ...
        if (!$excludeAll) {
            ...
            foreach ($propertiesMetadata as $propertyKey => $propertyMetadata) {
                $isExclude = \false;
                $isExpose = $propertyMetadata instanceof VirtualPropertyMetadata || $propertyMetadata instanceof ExpressionPropertyMetadata;
                    ...
                    if ($annot instanceof Since) {
                        ...
                        foreach ((array) $propertyMetadata->groups as $groupName) {
                            if (\false !== strpos($groupName, ',')) {
                                throw new InvalidMetadataException(sprintf('Invalid group name "%s" on "%s", did you mean to create multiple groups?', implode(', ', $propertyMetadata->groups), $propertyMetadata->class . '->' . $propertyMetadata->name));
                            }
                        }
                        ...
                        $propertyMetadata->setType(['name' => 'union', 'params' => [null, $annot->field, $annot->map]]);
                    }
                }
    ...
    protected function getClassAnnotations(\ReflectionClass $class): array
    {
        ...
        if (\PHP_VERSION_ID >= 80000) {
            $annotations = array_map(static fn(\ReflectionAttribute $attribute): object => $attribute->newInstance(), $class->getAttributes(SerializerAttribute::class, \ReflectionAttribute::IS_INSTANCEOF));
        }
        if (null !== $this->reader) {
    ...
    protected function getMethodAnnotations(\ReflectionMethod $method): array
    {
        $annotations = [];
        if (\PHP_VERSION_ID >= 80000) {
            $annotations = array_map(static fn(\ReflectionAttribute $attribute): object => $attribute->newInstance(), $method->getAttributes(SerializerAttribute::class, \ReflectionAttribute::IS_INSTANCEOF));
        }
        if (null !== $this->reader) {
    ...
    protected function getPropertyAnnotations(\ReflectionProperty $property): array
    {
        $annotations = [];
        if (\PHP_VERSION_ID >= 80000) {
            $annotations = array_map(static fn(\ReflectionAttribute $attribute): object => $attribute->newInstance(), $property->getAttributes(SerializerAttribute::class, \ReflectionAttribute::IS_INSTANCEOF));
        }
        if (null !== $this->reader) {
