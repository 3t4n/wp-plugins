...
use WPPayVendor\PHPStan\PhpDocParser\Ast\PhpDoc\TypeAliasImportTagValueNode;
use WPPayVendor\PHPStan\PhpDocParser\Ast\PhpDoc\TypeAliasTagValueNode;
...
final class DocBlockTypeResolver
{
    ...
    private function getPhpstanArrayType(\ReflectionClass $declaringClass, string $typeHint, $reflector): ?string
    {
        ...
        foreach ($phpDocNode->children as $node) {
            if ($node instanceof PhpDocTagNode && $node->value instanceof TypeAliasTagValueNode && $node->value->alias === $typeHint) {
                ...
                if (isset($foundPhpstanArray[0])) {
                    ...
                    return sprintf('array<%s>', implode(',', array_map(static fn(string $type) => $self->resolveType(trim($type), $reflector), $types)));
                }
                ...
                return $this->getPhpstanArrayType(new \ReflectionClass($importedFromFqn), $node->value->importedAlias, $reflector);
            }
        }
        return null;
    }
