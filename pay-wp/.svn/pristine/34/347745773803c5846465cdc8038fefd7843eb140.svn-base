...
final class EnumHandler implements SubscribingHandlerInterface
{
    ...
    public static function getSubscribingMethods()
    {
        ...
        foreach (['json', 'xml'] as $format) {
            $methods[] = ['type' => 'enum', 'direction' => GraphNavigatorInterface::DIRECTION_DESERIALIZATION, 'format' => $format, 'method' => 'deserializeEnum'];
            $methods[] = ['type' => 'enum', 'format' => $format, 'direction' => GraphNavigatorInterface::DIRECTION_SERIALIZATION, 'method' => 'serializeEnum'];
        }
        return $methods;
    }
    public function serializeEnum(SerializationVisitorInterface $visitor, \UnitEnum $enum, array $type, SerializationContext $context)
    {
