...
final class FormErrorHandler implements SubscribingHandlerInterface
{
    ...
    public static function getSubscribingMethods()
    {
        ...
        foreach (['xml', 'json'] as $format) {
            $methods[] = ['direction' => GraphNavigatorInterface::DIRECTION_SERIALIZATION, 'type' => Form::class, 'format' => $format];
            $methods[] = ['direction' => GraphNavigatorInterface::DIRECTION_SERIALIZATION, 'type' => FormInterface::class, 'format' => $format, 'method' => 'serializeFormTo' . ucfirst($format)];
            $methods[] = ['direction' => GraphNavigatorInterface::DIRECTION_SERIALIZATION, 'type' => FormError::class, 'format' => $format];
        }
        return $methods;
    }
    public function __construct(?object $translator = null, string $translationDomain = 'validators')
    {
        if (null !== $translator && (!$translator instanceof TranslatorInterface && !$translator instanceof TranslatorContract)) {
            throw new \InvalidArgumentException(sprintf('The first argument passed to %s must be instance of %s or %s, %s given', self::class, TranslatorInterface::class, TranslatorContract::class, get_class($translator)));
        }
