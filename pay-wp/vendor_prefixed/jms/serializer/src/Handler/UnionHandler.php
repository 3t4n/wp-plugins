...
final class UnionHandler implements SubscribingHandlerInterface
{
    ...
    public static function getSubscribingMethods()
    {
        ...
        foreach ($formats as $format) {
            $methods[] = ['type' => 'union', 'format' => $format, 'direction' => GraphNavigatorInterface::DIRECTION_DESERIALIZATION, 'method' => 'deserializeUnion'];
            $methods[] = ['type' => 'union', 'format' => $format, 'direction' => GraphNavigatorInterface::DIRECTION_SERIALIZATION, 'method' => 'serializeUnion'];
        }
        return $methods;
    }
    public function serializeUnion(SerializationVisitorInterface $visitor, mixed $data, array $type, SerializationContext $context): mixed
    {
    ...
    public function deserializeUnion(DeserializationVisitorInterface $visitor, mixed $data, array $type, DeserializationContext $context): mixed
    {
        ...
        if (3 === count($type['params'])) {
            ...
            $finalType = ['name' => $unionMap[$lookupValue], 'params' => []];
    ...
    {
        return in_array($type, ['int', 'integer', 'float', 'double', 'bool', 'boolean', 'string'], \true);
    }
