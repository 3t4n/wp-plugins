...
if( crossword_puzzle !== null ) {
    ...
    const puzzle_crossword_modal = createApp({
        ...
        methods : {
            ...
            fastLoad : function( type ){
                if( type == 'across' ){
                    if( Object.values( this.hor_clues ).length == 0 ){
                        alert( 'No Across Clues Found. Draw blanks first, and generate the clues.' );                        
                        return false;
                    }
                }else{
                    if( Object.values( this.ver_clues ).length == 0 ){
                        alert( 'No Down Clues Found. Draw blanks first, and generate the clues.' );
                        return false;
                    }
                } 

                this.fastLoadType = type;

                if( type == 'across' ){
                
                    this.$refs.fast_load_clues.value = '';
                    for( let item in this.hor_clues ){
                        this.$refs.fast_load_clues.value += this.hor_clues[ item ].q + '\n';
                    }
                }else{
                    this.$refs.fast_load_clues.value = '';
                    for( let item in this.ver_clues ){
                        this.$refs.fast_load_clues.value += this.ver_clues[ item ].q + '\n';
                    }
                }

                this.$refs.btn_modal_fast_load.click();
                
            },

            updateClues : function( $event ){
               let textarea = this.$refs.fast_load_clues.value;
               ...
               lines.forEach( ( line, index ) => {
                   ...
                   if( line.trim() !== '' ){
                       ...
                       if( this.fastLoadType == 'across' ){
                            try {
                                key =  Object.keys( this.hor_clues )[ index  ];
                                this.hor_clues[ key ].q = line.trim();                         
                            } catch (error) {
                                console.error( error)
                            }
                            ...
                            try {                               
                                key = Object.keys( this.ver_clues )[ index  ];
                                this.ver_clues[ key ].q = line.trim(); 
                            } catch (error) {
                                console.error( error);
                            }
               ...
               this.$refs.btn_close_modal_fast_load.click();
            },
