...
/**
 * AccurateCalculators.com loan calculator plugin
 *
...
/**
 * Plugin Name: AC Loan Calculator
 * Plugin URI: https://accuratecalculators.com/calculator-plugins/loan-plus-plugin
 * Description: A versatile loan calculator with an amortization schedule, payment dates, and interactive charts. Rebrand with your name. Supports 90 currencies, 6 date formats, and 15 languages. Highly customizable via extensive configuration options. Version 2.0 is a major update—test before upgrading.
 * Version: 2.0
 * Author: AccurateCalculators.com
 * Author URI: https://AccurateCalculators.com
 * License: Commercial (but free)
 * Text Domain: ac-loan-calculator-plus
...
/*
	...
	array('op_size' => "large",
				...
				[KT] 09/10/2024 - for possible future use
				'op_preparer' => null,
...
const FC_LNCALC_VER = '2.0'; // for cache busting
const FC_LNCALC_PLUGIN_NAME = 'AC Loan Calculator';
const FC_LNCALC_CSS_SCOPED_RESET = 'dist/css/bootstrap-reboot-scoped.css';
const FC_LNCALC_CSS_MAIN = 'dist/css/accurate-calculators.css';
const FC_LNCALC_CSS_CUSTOM = 'dist/css/accurate-calculators-custom.css';
const FC_LNCALC_JS_ENTRY = 'dist/js/interface.LOAN.gpl.js';
const FC_LNCALC_DOWNLOAD_URL = 'https://accuratecalculators.com/plugins/ZIPs/fc-loan-calculator.2.0.zip';
const FC_LNCALC_DERIVATIVE_OF = 'https://accuratecalculators.com/loan-calculator';

const FC_LNCALC_TIME_OUT = 15; // seconds

// Constants for the plugin update check
const FC_LNCALC_PLUGIN_SLUG = 'ac-loan-calculator-plus/ac-loan-calculator-plus.php'; // Path to the main plugin file
// const ac_lncalc_UPDATE_ENDPOINT = 'https://accuratecalculators.com/wp-content/themes/accurate/plugin-endpoints/loanplus-endpoint.php'; // URL to the update endpoint
const FC_LNCALC_TIMEOUT_SECS = 15;

// Define constants for easy migration across plugins
const FC_LNCALC_PLUGIN_AUTHOR = 'AccurateCalculators.com'; // Plugin author
const FC_LNCALC_PLUGIN_AUTHOR_HOMEPAGE = 'https://accuratecalculators.com/contact';
const FC_LNCALC_PLUGIN_HOMEPAGE = 'https://accuratecalculators.com/calculator-plugins/loan-plus-plugin';
// for fallback only
const FC_LNCALC_PLUGIN_DESCRIPTION = 'A versatile loan calculator with an amortization schedule, payment dates, and charts. Supports international conventions.';
 ...
 */
// function ac_lncalc_information($result, $action, $args)
// {
...
// 	if (isset($args->slug) && $args->slug === FC_LNCALC_PLUGIN_SLUG) {
// 		// Fetch the update information from the update endpoint
// 		$response = wp_remote_get(ac_lncalc_UPDATE_ENDPOINT, ['timeout' => FC_LNCALC_TIMEOUT_SECS]);
...
// 			$result = new stdClass();
// 			$result->name = $plugin_data['name'] ?? FC_LNCALC_PLUGIN_NAME;
// 			$result->slug = FC_LNCALC_PLUGIN_SLUG;
// 			$result->version = $plugin_data['new_version'];
// 			$result->tested = get_bloginfo('version');
// 			$result->author = $plugin_data['author'] ?? FC_LNCALC_PLUGIN_AUTHOR;
// 			$result->author_profile = $plugin_data['author_homepage'] ?? FC_LNCALC_PLUGIN_AUTHOR_HOMEPAGE;
// 			$result->homepage = $plugin_data['homepage'] ?? FC_LNCALC_PLUGIN_HOMEPAGE;
// 			$result->sections = [
// 				'description' => $plugin_data['description'] ?? FC_LNCALC_PLUGIN_DESCRIPTION,
 ...
 */
// function ac_lncalc_get_update_info()
// {

// 	// Fetch the update info from the remote server.
// 	$response = wp_remote_get(ac_lncalc_UPDATE_ENDPOINT, array('timeout' => FC_LNCALC_TIMEOUT_SECS));
 ...
 */
// function ac_lncalc_check_for_update($transient)
// {
...
// 	$update_info = ac_lncalc_get_update_info();
// 	if (empty($update_info)) {
// 		return $transient;
// 	}

// 	// Compare current version with new version.
// 	if (version_compare($transient->checked[FC_LNCALC_PLUGIN_SLUG], $update_info['new_version'], '<')) {
// 		$plugin_info = new stdClass();
// 		$plugin_info->slug = FC_LNCALC_PLUGIN_SLUG;
...
// 		$transient->response[FC_LNCALC_PLUGIN_SLUG] = $plugin_info;
// 	}
// 	// else {
// 	// 	error_log('Current plugin version is up to date: ' . FC_LNCALC_VER);
// 	// }

// 	return $transient;
// } // ac_lncalc_check_for_update
// add_filter('pre_set_site_transient_update_plugins', 'ac_lncalc_check_for_update', 20);


/**
 * Function: ac_lncalc_activate
 * activation hook
...
function ac_lncalc_activate()
{
	...
	update_option('aclncalc_key', array(
		'op_size' => null,
		'op_custom_style' => "No",
		'op_add_link' => "No",
		'op_brand_name' => "",
...
register_activation_hook(__FILE__, 'ac_lncalc_activate');


/**
 * Function: ac_lncalc_show_widget
 * Shows the plugin in a WordPress widget area / sidebar
 *
 * args: $args (environment variables handled automatically by the hook)
 * returns: nothing
 */
function ac_lncalc_show_widget($args)
{

	extract($args);

	$options = get_option('aclncalc_key');
	...
	show_fcloan_plugin($options);

	echo $after_widget;
} // ac_lncalc_show_widget

// Helper functions for ac_lncalcplus
if (!function_exists('ac_lncalc_validate_yes_no')) {
	function ac_lncalc_validate_yes_no($value, $default = 'No', $option_name = null)
	{
...
if (!function_exists('ac_lncalc_validate_size')) {
	function ac_lncalc_validate_size($value, $default = 'large', $option_name = null)
	{
		$value = strtolower(sanitize_text_field($value));
...
if (!function_exists('ac_lncalc_validate_numeric')) {
	function ac_lncalc_validate_numeric($value, $default = '0', $option_name = null)
	{
		...
		if (is_numeric($value) && $value >= 0) {
			...
			if ($option_name) {
				echo __('Please enter a valid number greater than 0 for', 'fc-loan-calculator') . " \"$option_name.\"<br>";
			}
			return $default;
		}
	}
...
if (!function_exists('ac_lncalc_validate_text')) {
	function ac_lncalc_validate_text($value, $default = '', $option_name = null)
	{
		return preg_replace("/[^\w#&'\-,. ]/", '', $value);
	}
}

if (!function_exists('ac_lncalc_validate_color')) {
	function ac_lncalc_validate_color($value, $default = '#000000', $option_name = null)
	{
		$value = sanitize_text_field($value);
...
if (!function_exists('ac_lncalc_validate_dimension')) {
	function ac_lncalc_validate_dimension($value, $default = '0', $option_name = null)
	{
		$value = sanitize_text_field($value);
...
if (!function_exists('ac_lncalc_validate_font_stack')) {
	function ac_lncalc_validate_font_stack($value, $default = '', $option_name = null)
	{
		return sanitize_text_field($value);
	}
}

if (!function_exists('ac_lncalc_validate_shadow')) {
	function ac_lncalc_validate_shadow($value, $default = '', $option_name = null)
	{
		return sanitize_text_field($value);
	}
}

if (!function_exists('ac_lncalc_get_processed_option')) {
	function ac_lncalc_get_processed_option($option_name, $keys, $sources, $default, $validation_func = null)
	{
		$value = null;
		foreach ($keys as $key) {
...
/**
 * Displays the loan calculator GUI with options for shortcode attributes or direct function parameters.
 *
 ...
 */
function show_fcloan_plugin($options = array(), $content = null, $code = "")
{
	wp_enqueue_script('fc-i18n-loan');
	wp_enqueue_script('fc-loan-interface');

	// Set translations for hack script only
	wp_set_script_translations('fc-i18n-loan', 'fc-loan-calculator', plugin_dir_path(__FILE__) . 'languages');
	...
	$shortcode = false;
	if (!empty($code) || (!empty($options) && array_key_exists(0, $options) && (strtolower($options[0]) == 'fcloanplugin' || strtolower($options[0]) == 'fcloanplugin'))) {
		$shortcode = true;
		...
		$temp_options = get_option('aclncalc_key');
	...
	$validation_functions = array(
		'size' => function ($value, $default, $option_name) use ($allowed_sizes) {
			return ac_lncalc_validate_size($value, $default, $option_name);
		},
		'custom_style' => 'ac_lncalc_validate_yes_no',
		'add_link' => 'ac_lncalc_validate_yes_no',
		'brand_name' => 'ac_lncalc_validate_text',
		'hide_resize' => 'ac_lncalc_validate_yes_no',
		'hide_intl_conventions' => 'ac_lncalc_validate_yes_no',
		'hide_payment_method' => 'ac_lncalc_validate_yes_no',
		'loan_amt' => function ($value, $default, $option_name) {
			return ac_lncalc_validate_numeric($value, '0', $option_name);
		},
		...
		'theme_base_font_size' => 'ac_lncalc_validate_dimension',
		'theme_primary_color' => 'ac_lncalc_validate_color',
		'theme_primary_color_hover' => 'ac_lncalc_validate_color',
		'theme_primary_color_light' => 'ac_lncalc_validate_color',
		'theme_primary_color_text' => 'ac_lncalc_validate_color',
		'theme_primary_color_text_inverse' => 'ac_lncalc_validate_color',
		'theme_background_muted' => 'ac_lncalc_validate_color',
		'theme_background_color_disabled' => 'ac_lncalc_validate_color',
		'theme_background_calculator_color' => 'ac_lncalc_validate_color',
		'theme_background_modal_color' => 'ac_lncalc_validate_color',
		'theme_border_color' => 'ac_lncalc_validate_color',
		'theme_text_color' => 'ac_lncalc_validate_color',
		'theme_tooltip_text_color' => 'ac_lncalc_validate_color',
		'theme_shadow_color' => 'ac_lncalc_validate_shadow',
		'theme_primary_font_family_stack' => 'ac_lncalc_validate_font_stack',
		'theme_mono_font_family_stack' => 'ac_lncalc_validate_font_stack',
		'calculator_min_width_tiny' => 'ac_lncalc_validate_dimension',
		'calculator_max_width_tiny' => 'ac_lncalc_validate_dimension',
		'calculator_min_width_small' => 'ac_lncalc_validate_dimension',
		'calculator_max_width_small' => 'ac_lncalc_validate_dimension',
		'calculator_min_width_medium' => 'ac_lncalc_validate_dimension',
		'calculator_max_width_medium' => 'ac_lncalc_validate_dimension',
		'calculator_min_width_large' => 'ac_lncalc_validate_dimension',
		'calculator_max_width_large' => 'ac_lncalc_validate_dimension',
	);
	...
	foreach ($option_mappings as $option_name => $keys) {
		...
		$processed_options[$option_name] = ac_lncalc_get_processed_option($option_name, $keys, $sources, $default, $validation_func);
	}
	...
	wp_register_style('accuratecalcs-reset-style', plugins_url(FC_LNCALC_CSS_SCOPED_RESET, __FILE__), array(), FC_LNCALC_VER, 'screen');

	// Register the main styles with 'accuratecalcs-reset-style' as a dependency
	wp_register_style('accuratecalcs-style', plugins_url(FC_LNCALC_CSS_MAIN, __FILE__), array('accuratecalcs-reset-style'), FC_LNCALC_VER, 'screen');
	...
	if (strtolower($custom_style) === 'yes') {
		wp_register_style('accuratecalcs-custom-style', plugins_url(FC_LNCALC_CSS_CUSTOM, __FILE__), array('accuratecalcs-reset-style'), FC_LNCALC_VER, 'screen');
 ...
 * See: show_fcloan_plugin for enqueuing.
 * Enqueuing in show_fcloan_plugin() prevents script from being loaded on pages where plugin is not utilized.
 */
function ac_lncalc_enqueue_scripts()
{
	// Register and enqueue the "hack" script for translation injection with wp-i18n dependency
	// Register the hack script required for i18n support for ES6 modules.
	wp_register_script('fc-i18n-loan', plugin_dir_url(__FILE__) . 'dist/js/ac-i18n-fix.js', array('wp-i18n'), null, true);

	// Register the module script
	wp_register_script('fc-loan-interface', plugins_url(FC_LNCALC_JS_ENTRY, __FILE__), array(), FC_LNCALC_VER, true);

	// see: show_fcloan_plugin
	// wp_set_script_translations('fc-i18n-loan', 'fc-loan-calculator', plugin_dir_path(__FILE__) . 'languages');

	// $locale = get_locale();
	// error_log('$locale -> ' . $locale);
}
add_action('wp_enqueue_scripts', 'ac_lncalc_enqueue_scripts');


/** 
 * Initialize the plugin: load text domain and setup plugin update hooks.
 */
function ac_lncalc_init()
{
	// Load the plugin's .mo file
	load_plugin_textdomain('fc-loan-calculator', false, basename(dirname(__FILE__)) . '/languages');

	// add_filter('plugins_api', 'ac_lncalc_information', 10, 3);
}
add_action('init', 'ac_lncalc_init');


/**
 * Function: ac_lncalc_show_admin_options
 *
...
function ac_lncalc_show_admin_options()
{
	...
	$options = $newoptions = get_option('aclncalc_key');
	...
	if (!array_key_exists('op_size', $options) || !array_key_exists('op_custom_style', $options) || !array_key_exists('op_add_link', $options) || !array_key_exists('op_brand_name', $options) || !array_key_exists('op_hide_resize', $options) || !array_key_exists('op_loan_amt', $options) || !array_key_exists('op_n_months', $options) || !array_key_exists('op_rate', $options) || !array_key_exists('op_currency', $options) || !array_key_exists('op_date_mask', $options)) {
		...
		$options = $newoptions = get_option('aclncalc_key'); // keep everything in sync
	}


	// if widget's options have previously been set/saved in current session
	if (!empty($_POST['aclncalc_opts'])) {
		$newoptions['op_size'] = strip_tags(stripslashes($_POST['aclncalc-op_size']));
		...
		$newoptions['op_custom_style'] = strip_tags(stripslashes($_POST['aclncalc-op_custom_style']));
		if (strtolower($newoptions['op_custom_style']) != 'yes') {
			$newoptions['op_custom_style'] = 'No';
		}
		$newoptions['op_add_link'] = strip_tags(stripslashes($_POST['aclncalc-op_add_link']));
		...
		$newoptions['op_brand_name'] = preg_replace("/[^\w#&'\-,. ]/", '', $_POST['aclncalc-op_brand_name']);
		if (strtolower($newoptions['op_add_link']) != 'yes') {
			$newoptions['op_brand_name'] = '';
		}
		$newoptions['op_hide_resize'] = strip_tags(stripslashes($_POST['aclncalc-op_hide_resize']));
		if (strtolower($newoptions['op_hide_resize']) != 'yes') {
			$newoptions['op_hide_resize'] = 'no';
		}
		$newoptions['op_loan_amt'] = strip_tags(stripslashes($_POST['aclncalc-op_loan_amt']));
		$newoptions['op_n_months'] = strip_tags(stripslashes($_POST['aclncalc-op_n_months']));
		$newoptions['op_rate'] = strip_tags(stripslashes($_POST['aclncalc-op_rate']));
		// [KT] 02/05/2020 - new options
		$newoptions['op_currency'] = strip_tags(stripslashes($_POST['aclncalc-op_currency']));
		$newoptions['op_date_mask'] = strip_tags(stripslashes($_POST['aclncalc-op_date_mask']));

		// [KT] 08/21/2024 - new options
		$newoptions['op_theme_base_font_size'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_base_font_size']));
		$newoptions['op_theme_primary_color'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_primary_color']));
		$newoptions['op_theme_primary_color_hover'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_primary_color_hover']));
		$newoptions['op_theme_primary_color_light'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_primary_color_light']));
		$newoptions['op_theme_primary_color_text'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_primary_color_text']));
		$newoptions['op_theme_primary_color_text_inverse'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_primary_color_text_inverse']));
		$newoptions['op_theme_background_muted'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_background_muted']));
		$newoptions['op_theme_background_color_disabled'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_background_color_disabled']));
		$newoptions['op_theme_background_calculator_color'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_background_calculator_color']));
		$newoptions['op_theme_background_modal_color'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_background_modal_color']));
		$newoptions['op_theme_border_color'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_border_color']));
		$newoptions['op_theme_text_color'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_text_color']));
		$newoptions['op_theme_tooltip_text_color'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_tooltip_text_color']));
		$newoptions['op_theme_shadow_color'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_shadow_color']));
		$newoptions['op_theme_primary_font_family_stack'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_primary_font_family_stack']));
		$newoptions['op_theme_mono_font_family_stack'] = strip_tags(stripslashes($_POST['aclncalc-op_theme_mono_font_family_stack']));
		$newoptions['op_calculator_min_width_tiny'] = strip_tags(stripslashes($_POST['aclncalc-op_calculator_min_width_tiny']));
		$newoptions['op_calculator_max_width_tiny'] = strip_tags(stripslashes($_POST['aclncalc-op_calculator_max_width_tiny']));
		$newoptions['op_calculator_min_width_small'] = strip_tags(stripslashes($_POST['aclncalc-op_calculator_min_width_small']));
		$newoptions['op_calculator_max_width_small'] = strip_tags(stripslashes($_POST['aclncalc-op_calculator_max_width_small']));
		$newoptions['op_calculator_min_width_medium'] = strip_tags(stripslashes($_POST['aclncalc-op_calculator_min_width_medium']));
		$newoptions['op_calculator_max_width_medium'] = strip_tags(stripslashes($_POST['aclncalc-op_calculator_max_width_medium']));
		$newoptions['op_calculator_min_width_large'] = strip_tags(stripslashes($_POST['aclncalc-op_calculator_min_width_large']));
		$newoptions['op_calculator_max_width_large'] = strip_tags(stripslashes($_POST['aclncalc-op_calculator_max_width_large']));
		// [KT] 09/03/2024 - new options
		$newoptions['op_hide_intl_conventions'] = strip_tags(stripslashes($_POST['aclncalc-op_hide_intl_conventions']));
		$newoptions['op_hide_payment_method'] = strip_tags(stripslashes($_POST['aclncalc-op_hide_payment_method']));
		...
		if (!is_numeric($newoptions['op_loan_amt'])) {
			...
			$error_messages[] = __('Please enter a valid number for "Default loan amount."', 'fc-loan-calculator');
		}
		if (!is_numeric($newoptions['op_n_months'])) {
			$newoptions['op_n_months'] = $options['op_n_months'];
			$error_messages[] = __('Please enter a valid number for "Default number of months."', 'fc-loan-calculator');
		}
		if (!is_numeric($newoptions['op_rate'])) {
			$newoptions['op_rate'] = $options['op_rate'];
			$error_messages[] = __('Please enter a valid number for "Default interest rate."', 'fc-loan-calculator');
		}
		// [KT] 02/05/2020 - new options
		if (!is_numeric($newoptions['op_currency'])) {
			$newoptions['op_currency'] = '999';
			$error_messages[] = __('Please enter a valid value for "Default Currency."', 'fc-loan-calculator');
		}
		if (!is_numeric($newoptions['op_date_mask'])) {
			$newoptions['op_date_mask'] = '999';
			$error_messages[] = __('Please enter a valid number for "Default Date Format."', 'fc-loan-calculator');
		}
		...
		foreach (
			...
			if (!preg_match('/^\d+(px|rem|em|%)$/', $newoptions[$option])) {
				...
				$error_messages[] = __(sprintf('Please enter a valid CSS dimension (e.g., "340px", "10rem") for "%s".', $option), 'fc-loan-calculator');
			}
		}
		...
		foreach (
			...
			if (!preg_match('/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$|^rgba?\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*(,\s*0?\.\d+)?\s*\)$/', $newoptions[$option])) {
				// Leave the value unchanged if validation fails
				$newoptions[$option] = $options[$option];
				$error_messages[] = __(sprintf('Please enter a valid color value (e.g., "#ffffff", "rgba(255, 255, 255, 0.5)") for "%s".', $option), 'fc-loan-calculator');
			}
		}
		...
		if (!empty($error_messages)) {
			error_log('AC Loan Calculator Plugin admin page error -> ' . implode(', ', $error_messages));
			// set_transient is not showing the error message - revisit
			// set_transient('aclncalcplus_error_messages', $error_messages, 30); // 30 seconds duration
		}
	}
	//debug
	//else {
	...
	if ($options != $newoptions && (wp_verify_nonce($_POST['aclncalc_opts'], 'fc_options'))) {

		// 2nd check permissions
		if (is_user_logged_in() && current_user_can('update_plugins')) {
			$options = $newoptions;
			update_option('aclncalc_key', $options);
		}
		//debug
		//else if (array_key_exists('aclncalcplus_opts', $_POST) && (!wp_verify_nonce($_POST['aclncalc_opts'], 'fc_options'))) {
...
?>
	...
	<div class="ac-container">
		...
		<p class="small">
			<?php _e('Do not forget to save your changes. Scroll all the way down in this panel for the <span class="red">[Save]</span> button.', 'fc-loan-calculator'); ?>
		</p>

		<div class="ac-option">
			<label for="aclncalcplus-op_size">
				<?php _e('Widget\'s size (size limits adjustable below)?', 'fc-loan-calculator'); ?>:
				<select name="aclncalcplus-op_size" id="aclncalcplus-op_size" class="widefat">
					<option value="tiny" <?php selected($options['op_size'], 'tiny'); ?>><?php _e('Tiny (max width = 180px)', 'fc-loan-calculator'); ?></option>
					<option value="small" <?php selected($options['op_size'], 'small'); ?>><?php _e('Small (max width = 300px)', 'fc-loan-calculator'); ?></option>
					<option value="medium" <?php selected($options['op_size'], 'medium'); ?>><?php _e('Medium (max width = 340px)', 'fc-loan-calculator'); ?></option>
					<option value="large" <?php selected($options['op_size'], 'large'); ?>><?php _e('Large (max width = 440px)', 'fc-loan-calculator'); ?></option>
				</select>
			</label>
		</div>

		<div class="ac-option">
			<label for="aclncalcplus-op_custom_style">
				<?php _e('Load custom style sheet?', 'fc-loan-calculator'); ?>:
				<select name="aclncalcplus-op_custom_style" id="aclncalcplus-op_custom_style" class="widefat">
					<option value="No" <?php selected($options['op_custom_style'], 'No'); ?>><?php _e('No', 'fc-loan-calculator'); ?></option>
					<option value="Yes" <?php selected($options['op_custom_style'], 'Yes'); ?>><?php _e('Yes', 'fc-loan-calculator'); ?></option>
				</select>
			</label>
			<p class="ac-description">
				<?php _e('If "Yes" loads &lt;site&gt;\wp-content\plugins\ac-loan-calculator\css\accurate-calculators-custom.css. Entries in <b>accurate-calculators-custom.css</b> override the plugin\'s look. Editing the provided custom stylesheet is another way to modify the look of the calculator. For best performance include only the rules in the custom stylesheet you are overriding. BACKUP YOUR CUSTOMIZATIONS! You will need to manually replace them when (if) you upgrade the plugin.', 'fc-loan-calculator'); ?>
			</p>
		</div>

		<div class="ac-option">
			<label for="aclncalcplus-op_currency">
				<?php _e('Set a default currency?', 'fc-loan-calculator'); ?>:
				<select name="aclncalcplus-op_currency" id="aclncalcplus-op_currency" class="widefat">
					<option value="999" <?php selected($options['op_currency'], '999'); ?>><?php _e('Use visitor\'s browser\'s settings to set currency - do not override.', 'fc-loan-calculator'); ?></option>
		...
		<div class="ac-option">
			<label for="aclncalcplus-op_date_mask">
				<?php _e('Set a date format convention?', 'fc-loan-calculator'); ?>:
				<select name="aclncalcplus-op_date_mask" id="aclncalcplus-op_date_mask" class="widefat">
					<option value="999" <?php selected($options['op_date_mask'], '999'); ?>><?php _e('Use visitor\'s browser\'s settings to set date format - do not override.', 'fc-loan-calculator'); ?></option>
			...
			<p class="ac-description">
				<?php _e('When a user first visits your site, the plugin will use the browser\'s international conventions to set the currency and date format. If you wish, you may override the plugin and set a default currency and date format. The user will still have the ability to override either setting if you do not hide the &quot;international conventions&quot; below.', 'fc-loan-calculator'); ?>
			</p>
		</div>

		<p class="small">
			<?php _e('<strong>Note:</strong> Your visitors may select a date format and currency by clicking on <span class="red">$ : MM/DD/YYYY</span> in the calculator\'s lower right corner if you do not set "Hide international conventions" to "yes".', 'fc-loan-calculator'); ?>
		</p>
		...
		<div class="ac-grid">
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_hide_resize">
					<?php _e('Hide the resize buttons?', 'fc-loan-calculator'); ?>:
					<select name="aclncalcplus-op_hide_resize" id="aclncalcplus-op_hide_resize" class="widefat">
						<option value="No" <?php selected($options['op_hide_resize'], 'No'); ?>><?php _e('No', 'fc-loan-calculator'); ?></option>
						<option value="Yes" <?php selected($options['op_hide_resize'], 'Yes'); ?>><?php _e('Yes', 'fc-loan-calculator'); ?></option>
					</select>
				</label>
			</div>
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_hide_intl_conventions">
					<?php _e('Hide international conventions?', 'fc-loan-calculator'); ?>:
					<select name="aclncalcplus-op_hide_intl_conventions" id="aclncalcplus-op_hide_intl_conventions" class="widefat">
						<option value="No" <?php selected($options['op_hide_intl_conventions'], 'No'); ?>><?php _e('No', 'fc-loan-calculator'); ?></option>
						<option value="Yes" <?php selected($options['op_hide_intl_conventions'], 'Yes'); ?>><?php _e('Yes', 'fc-loan-calculator'); ?></option>
					</select>
				</label>
			</div>
			<!-- </div>
		<div class="ac-grid"> -->
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_hide_payment_method">
					<?php _e('Hide the Payment Method?', 'fc-loan-calculator'); ?>:
					<select name="aclncalcplus-op_hide_payment_method" id="aclncalcplus-op_hide_payment_method" class="widefat">
						<option value="No" <?php selected($options['op_hide_payment_method'], 'No'); ?>><?php _e('No', 'fc-loan-calculator'); ?></option>
						<option value="Yes" <?php selected($options['op_hide_payment_method'], 'Yes'); ?>><?php _e('Yes', 'fc-loan-calculator'); ?></option>
					</select>
				</label>
			</div>
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_add_link">
					<?php _e('Link to', 'fc-loan-calculator'); ?>&nbsp;AccurateCalculators.com?::
					<select name="aclncalcplus-op_add_link" id="aclncalcplus-op_add_link" class="widefat">
						<option value="No" <?php selected($options['op_add_link'], 'No'); ?>><?php _e('No', 'fc-loan-calculator'); ?></option>
						<option value="Yes" <?php selected($options['op_add_link'], 'Yes'); ?>><?php _e('Yes', 'fc-loan-calculator'); ?></option>
					</select>
				</label>
			</div>
		</div>
		...
		<p class="ac-description">
			<?php _e('If "Link to AccurateCalculators.com" is set to "Yes", one very discreet follow link will be inserted in the calculator. If you allow the link, you may rebrand the calculator with your name. Resetting this option to "No" at any time will remove the links. See FAQ\'s in readme file for details.', 'fc-loan-calculator'); ?>
		</p>

		<div class="ac-option">
			<label for="aclncalcplus-op_brand_name">
				<?php _e('Add Your Brand Name', 'fc-loan-calculator'); ?>:
				<input id="aclncalcplus-op_brand_name" name="aclncalcplus-op_brand_name" type="text" value="<?php echo $brand_name; ?>" />
			</label>
			<p class="ac-description">
				<?php _e('You may brand this widget with your brand. <b>Example: "Abe\'s Best"</b> will be preappended to "Loan Calculator". For this to work, the above "Link to" option must be set to "Yes".', 'fc-loan-calculator'); ?>
			</p>
		</div>
		...
		<div class="ac-grid">
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_loan_amt">
					<?php _e('Default loan amount', 'fc-loan-calculator'); ?>:
					<input id="aclncalcplus-op_loan_amt" name="aclncalcplus-op_loan_amt" type="text" value="<?php echo $loan_amt; ?>" />
				</label>
			</div>
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_n_months">
					<?php _e('Default number of months', 'fc-loan-calculator'); ?>:
					<input id="aclncalcplus-op_n_months" name="aclncalcplus-op_n_months" type="text" value="<?php echo $n_months; ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_rate">
					<?php _e('Default interest rate', 'fc-loan-calculator'); ?>:
					<input id="aclncalcplus-op_rate" name="aclncalcplus-op_rate" type="text" value="<?php echo $rate; ?>" />
				</label>
			</div>
			<div class="ac-grid__item text__item">
				<span class="ac-note">
					<?php _e('Enter only digits.<br>No currency or formatting.', 'fc-loan-calculator'); ?>
				</span>
			</div>
		</div>
		...
		<p class="ac-note">
			<?php _e('Style options.', 'fc-loan-calculator'); ?>
			<br>
			<span class="small"><?php _e('(Defaults in parentheses. "hex", "rgb", or "rgba" values accepted for colors.)', 'fc-loan-calculator'); ?></span>
		</p>
		...
		<div class="ac-grid">
			...
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_theme_base_font_size">
					op_theme_base_font_size (16px):
					<input id="aclncalcplus-op_theme_base_font_size" name="aclncalcplus-op_theme_base_font_size" type="text" value="<?php echo esc_attr($options['op_theme_base_font_size']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_theme_primary_color">
					op_theme_primary_color (#28a745):
					<input id="aclncalcplus-op_theme_primary_color" name="aclncalcplus-op_theme_primary_color" type="text" value="<?php echo esc_attr($options['op_theme_primary_color']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_theme_primary_color_hover">
					op_theme_primary_color_hover&nbsp(#218838):
					<input id="aclncalcplus-op_theme_primary_color_hover" name="aclncalcplus-op_theme_primary_color_hover" type="text" value="<?php echo esc_attr($options['op_theme_primary_color_hover']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_theme_primary_color_text">
					op_theme_primary_color_text (#fff):
					<input id="aclncalcplus-op_theme_primary_color_text" name="aclncalcplus-op_theme_primary_color_text" type="text" value="<?php echo esc_attr($options['op_theme_primary_color_text']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="aclncalcplus-op_theme_primary_color_light">
					op_theme_primary_color_light (#30c853):
					<input id="aclncalcplus-op_theme_primary_color_light" name="aclncalcplus-op_theme_primary_color_light" type="text" value="<?php echo esc_attr($options['op_theme_primary_color_light']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="aclncalcplus-op_theme_primary_color_text_inverse">
					op_theme_primary_color_text_inverse (#fff):
					<input id="aclncalcplus-op_theme_primary_color_text_inverse" name="aclncalcplus-op_theme_primary_color_text_inverse" type="text" value="<?php echo esc_attr($options['op_theme_primary_color_text_inverse']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="aclncalcplus-op_theme_background_color_disabled">
					op_theme_background_color_disabled (#efefef):
					<input id="aclncalcplus-op_theme_background_color_disabled" name="aclncalcplus-op_theme_background_color_disabled" type="text" value="<?php echo esc_attr($options['op_theme_background_color_disabled']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="aclncalcplus-op_theme_background_calculator_color">
					op_theme_background_calculator_color (#fff):
					<input id="aclncalcplus-op_theme_background_calculator_color" name="aclncalcplus-op_theme_background_calculator_color" type="text" value="<?php echo esc_attr($options['op_theme_background_calculator_color']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="aclncalcplus-op_theme_background_modal_color">
					op_theme_background_modal_color (#fff):
					<input id="aclncalcplus-op_theme_background_modal_color" name="aclncalcplus-op_theme_background_modal_color" type="text" value="<?php echo esc_attr($options['op_theme_background_modal_color']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_theme_background_muted">
					op_theme_background_muted (#f7f7f7):
					<input id="aclncalcplus-op_theme_background_muted" name="aclncalcplus-op_theme_background_muted" type="text" value="<?php echo esc_attr($options['op_theme_background_muted']); ?>" />
				</label>
			</div>
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_theme_border_color">
					op_theme_border_color (#dee2e6):
					<input id="aclncalcplus-op_theme_border_color" name="aclncalcplus-op_theme_border_color" type="text" value="<?php echo esc_attr($options['op_theme_border_color']); ?>" />
				</label>
			</div>
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_theme_text_color">
					op_theme_text_color (#333333):
					<input id="aclncalcplus-op_theme_text_color" name="aclncalcplus-op_theme_text_color" type="text" value="<?php echo esc_attr($options['op_theme_text_color']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_theme_tooltip_text_color">
					op_theme_tooltip_text_color (#fff):
					<input id="aclncalcplus-op_theme_tooltip_text_color" name="aclncalcplus-op_theme_tooltip_text_color" type="text" value="<?php echo esc_attr($options['op_theme_tooltip_text_color']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item ac-grid__item--span-2">
				<label for="aclncalcplus-op_theme_shadow_color">
					op_theme_shadow_color (rgba(0, 0, 0, 0.1)):
					<input id="aclncalcplus-op_theme_shadow_color" name="aclncalcplus-op_theme_shadow_color" type="text" value="<?php echo esc_attr($options['op_theme_shadow_color']); ?>" />
				</label>
			</div>
		...
		<div class="ac-grid">
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_theme_primary_font_family_stack">
					op_theme_primary_font_family_stack:
					<input id="aclncalcplus-op_theme_primary_font_family_stack" name="aclncalcplus-op_theme_primary_font_family_stack" type="text" value="<?php echo esc_attr($options['op_theme_primary_font_family_stack']); ?>" />
				</label>
			</div>
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_theme_mono_font_family_stack">
					op_theme_mono_font_family_stack:
					<input id="aclncalcplus-op_theme_mono_font_family_stack" name="aclncalcplus-op_theme_mono_font_family_stack" type="text" value="<?php echo esc_attr($options['op_theme_mono_font_family_stack']); ?>" />
				</label>
			</div>
		</div>
		...
		<p class="ac-note">
			<?php _e('You may tweak the minimum and maximum widths.', 'fc-loan-calculator'); ?>
			<br>
			<span class="small"><?php _e('("px", "rem", and "em" units accepted.)', 'fc-loan-calculator'); ?></span>
		</p>
		...
		<div class="ac-grid">
			...
			<div class="ac-grid__item">
				<label for="aclncalcplus-op_calculator_min_width_tiny">
					op_calculator_min_width_tiny
					<input id="aclncalcplus-op_calculator_min_width_tiny" name="aclncalcplus-op_calculator_min_width_tiny" type="text" value="<?php echo esc_attr($options['op_calculator_min_width_tiny']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_calculator_max_width_tiny">
					op_calculator_max_width_tiny
					<input id="aclncalcplus-op_calculator_max_width_tiny" name="aclncalcplus-op_calculator_max_width_tiny" type="text" value="<?php echo esc_attr($options['op_calculator_max_width_tiny']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_calculator_min_width_small">
					op_calculator_min_width_small
					<input id="aclncalcplus-op_calculator_min_width_small" name="aclncalcplus-op_calculator_min_width_small" type="text" value="<?php echo esc_attr($options['op_calculator_min_width_small']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_calculator_max_width_small">
					op_calculator_max_width_small
					<input id="aclncalcplus-op_calculator_max_width_small" name="aclncalcplus-op_calculator_max_width_small" type="text" value="<?php echo esc_attr($options['op_calculator_max_width_small']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_calculator_min_width_medium">
					op_calculator_min_width_medium
					<input id="aclncalcplus-op_calculator_min_width_medium" name="aclncalcplus-op_calculator_min_width_medium" type="text" value="<?php echo esc_attr($options['op_calculator_min_width_medium']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_calculator_max_width_medium">
					op_calculator_max_width_medium
					<input id="aclncalcplus-op_calculator_max_width_medium" name="aclncalcplus-op_calculator_max_width_medium" type="text" value="<?php echo esc_attr($options['op_calculator_max_width_medium']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_calculator_min_width_large">
					op_calculator_min_width_large
					<input id="aclncalcplus-op_calculator_min_width_large" name="aclncalcplus-op_calculator_min_width_large" type="text" value="<?php echo esc_attr($options['op_calculator_min_width_large']); ?>" />
				</label>
			</div>

			<div class="ac-grid__item">
				<label for="aclncalcplus-op_calculator_max_width_large">
					op_calculator_max_width_large
					<input id="aclncalcplus-op_calculator_max_width_large" name="aclncalcplus-op_calculator_max_width_large" type="text" value="<?php echo esc_attr($options['op_calculator_max_width_large']); ?>" />
				</label>
			</div>
	...
	<input type="hidden" id="aclncalcplus_opts" name="aclncalcplus_opts" value="<?php echo wp_create_nonce('aclncalcplus_ac_options'); ?>" />
<?php

} // ac_lncalc_show_admin_options()


/**
 * Function: ac_lncalc_register
 *
...
function ac_lncalc_register()
{
	$widget_ops = array('classname' => 'aclncalc_key', 'description' => __('AC Loan Calculator by AccurateCalculators.com'));

	$name = __('AC Loan Calculator Plugin');
	...
	wp_register_sidebar_widget('fcloanplugin', $name, 'ac_lncalc_show_widget', $widget_ops);
	...
	wp_register_widget_control('fcloanplugin', $name, 'ac_lncalc_show_admin_options');
} // ac_lncalc_register


// Hooks a function on to a specific action.
add_action('widgets_init', 'ac_lncalc_register');
 ...
 */
function ac_lncalc_add_module_to_js($tag, $handle, $src)
{
	// Check if this is the handle for the script that needs the module type
	if ('fc-loan-interface' === $handle) {
		// Add the type="module" attribute
		$tag = '<script type="module" src="' . esc_url($src) . '"></script>';
	}
	return $tag;
}
add_filter('script_loader_tag', 'ac_lncalc_add_module_to_js', 10, 3);


/**
 * Enqueue backend stylesheet for Appearance -> Widget page
 */
function ac_lncalc_admin_styles($hook)
{
...
add_action('admin_enqueue_scripts', 'ac_lncalc_admin_styles');
...
// add_action('admin_notices', 'ac_lncalc_show_error_messages');

/*
function ac_lncalc_show_error_messages() {
    global $pagenow;
    ...
    if ($pagenow == 'widgets.php' || $pagenow == 'customize.php') {
        $error_messages = get_transient('aclncalcplus_error_messages');
        ...
        if (!empty($error_messages)) {
            ...
            delete_transient('aclncalcplus_error_messages');
        }
    }
}
*/

//  All shortcodes with defaults:


// 	[acloanplugin sc_size=null sc_custom_style="No" sc_add_link="No" sc_brand_name="" sc_hide_resize="No" sc_loan_amt="32500.0" sc_n_months="48" sc_rate="5.5" sc_currency="0" sc_date_mask="0" sc_theme_base_font_size="16px" sc_theme_primary_color="#28a745" sc_theme_primary_color_hover="#218838" sc_theme_primary_color_light="#30c853" sc_theme_primary_color_text="#fff" sc_theme_primary_color_text_inverse="#fff" sc_theme_background_muted="#f7f7f7" sc_theme_background_color_disabled="#efefef" sc_theme_background_calculator_color="#fff" sc_theme_background_modal_color="#fff" sc_theme_border_color="#dee2e6" sc_theme_text_color="#333333" sc_theme_tooltip_text_color="#fff" sc_theme_shadow_color="rgba(0, 0, 0, 0.1)" sc_theme_primary_font_family_stack="Roboto, 'Helvetica Neue', Helvetica, Arial, sans-serif" sc_theme_mono_font_family_stack="'Roboto Mono', 'Source Code Pro', ui-monospace, monospace" sc_calculator_min_width_tiny="150px" sc_calculator_max_width_tiny="250px" sc_calculator_min_width_small="200px" sc_calculator_max_width_small="300px" sc_calculator_min_width_medium="200px" sc_calculator_max_width_medium="400px" sc_calculator_min_width_large="200px" sc_calculator_max_width_large="440px" sc_hide_intl_conventions="no" sc_hide_payment_method="no"]

// a blue theme

// [acloanplugin sc_size="tiny" sc_custom_style="No" sc_add_link="No" sc_brand_name="" sc_hide_resize="No" sc_loan_amt="600500.0" sc_n_months="180" sc_rate="6.5" sc_currency="0" sc_date_mask="0" sc_theme_base_font_size="16px" sc_theme_primary_color="#007bff" sc_theme_primary_color_hover="#0056b3" sc_theme_primary_color_light="#66b2ff" sc_theme_primary_color_text="#fff" sc_theme_primary_color_text_inverse="#fff" sc_theme_background_muted="#f7f7f7" sc_theme_background_color_disabled="#efefef" sc_theme_background_calculator_color="#fff" sc_theme_background_modal_color="#fff" sc_theme_border_color="#dee2e6" sc_theme_text_color="#333333" sc_theme_tooltip_text_color="#fff" sc_theme_shadow_color="rgba(0, 0, 0, 0.1)" sc_theme_primary_font_family_stack="Roboto, 'Helvetica Neue', Helvetica, Arial, sans-serif" sc_theme_mono_font_family_stack="'Roboto Mono', 'Source Code Pro', ui-monospace, monospace" sc_calculator_min_width_tiny="150px" sc_calculator_max_width_tiny="250px" sc_calculator_min_width_small="200px" sc_calculator_max_width_small="300px" sc_calculator_min_width_medium="200px" sc_calculator_max_width_medium="400px" sc_calculator_min_width_large="200px" sc_calculator_max_width_large="440px" sc_hide_intl_conventions="no" sc_hide_payment_method="no"]


//   All options with defaults:

// 'op_size'=>"large", 'op_custom_style'=>"No", 'op_add_link'=>"No", 'op_brand_name'=>"", 'op_hide_resize'=>"No", 'op_loan_amt'=>"32500.0", 'op_n_months'=>"48", 'op_rate'=>"5.5", 'op_currency'=>"0", 'op_date_mask'=>"0", 'op_theme_base_font_size'=>"16px", 'op_theme_primary_color'=>"#28a745", 'op_theme_primary_color_hover'=>"#218838", 'op_theme_primary_color_light'=>"#30c853", 'op_theme_primary_color_text'=>"#fff", 'op_theme_primary_color_text_inverse'=>"#fff", 'op_theme_background_muted'=>"#f7f7f7", 'op_theme_background_color_disabled'=>"#efefef", 'op_theme_background_calculator_color'=>"#fff", 'op_theme_background_modal_color'=>"#fff", 'op_theme_border_color'=>"#dee2e6", 'op_theme_text_color'=>"#333333", 'op_theme_tooltip_text_color'=>"#fff", 'op_theme_shadow_color'=>"rgba(0, 0, 0, 0.1)", 'op_theme_primary_font_family_stack'=>"Roboto, 'Helvetica Neue', Helvetica, Arial, sans-serif", 'op_theme_mono_font_family_stack'=>"'Roboto Mono', 'Source Code Pro', ui-monospace, monospace", 'op_calculator_min_width_tiny'=>"150px", 'op_calculator_max_width_tiny'=>"180px", 'op_calculator_min_width_small'=>"200px", 'op_calculator_max_width_small'=>"300px", 'op_calculator_min_width_medium'=>"200px", 'op_calculator_max_width_medium'=>"400px", 'op_calculator_min_width_large'=>"200px", 'op_calculator_max_width_large'=>"440px", 'op_hide_intl_conventions'=>"No", 'op_hide_payment_method'=>"No"


// Notes:

// Primary Button Color: #007bff (a bold, primary blue)
// Hover Color: #0056b3 (a darker blue for hover effect)
// Light Version: #66b2ff (a light blue for subtle elements)
