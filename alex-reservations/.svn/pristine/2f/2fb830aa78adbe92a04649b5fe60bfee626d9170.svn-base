...
class MailManager {
	...
	public function send_email($to, $subject, $message, $pixel_code = null) {
		//ray('send_email');
		//ray($to);
		//ray($subject);
	...
	public function messageFormatted($content)
	{
		...
		return $this->addPixel($message);
	}

	protected function addPixel($message) {

		if (!$this->pixel_code) {
			return str_replace('{PIXEL}', '', $message);
		}
		return str_replace('{PIXEL}', $this->pixel_code, $message);
	}
