...
trait TablesCalculations {
	 ...
	 * @return array|false
	 */
	public function isAvailableBy_SearchTables($date_string, $time, $guests, $singleTablesRequireToBeOnline = false, $areaIdToFilter = null, $exclude_booking_id = null)
	{
		//ray('isAvailableBy_SearchTables ' . $date_string.' ' . ($time/3600) . ' ' . $guests . ' exclude_booking_id:' . $exclude_booking_id);

		if ($singleTablesRequireToBeOnline) {
			$result = $this->searchTableOrGroup_SingleTablesNeedToBeOnline($date_string, $time, $guests, $areaIdToFilter, $exclude_booking_id);
			//ray($result);
			return $result;
		} else {
			$result = $this->searchTableOrGroup_SingleTablesNoNeedToBeOnline($date_string, $time, $guests, $areaIdToFilter, $exclude_booking_id);
			//ray($result);
			return $result;
		}
	}

	public function searchTableOrGroup_SingleTablesNeedToBeOnline($date_string, $time, $guests, $areaIdToFilter = null, $exclude_booking_id = null)
	{
		// Busca mesas individuales libres online sin importar el numero de guests
		$check_number_of_guests = false;
		$listTablesFree = $this->getListTablesFree($date_string, $time, $guests, false, $check_number_of_guests, $exclude_booking_id);
		if ($areaIdToFilter)
		...
		if ($group_found) {
			...
			return $this->getGroupTables($group_found, $date_string);
		}

		return false;
	}

	public function searchTableOrGroup_SingleTablesNoNeedToBeOnline($date_string, $time, $guests, $areaIdToFilter = null, $exclude_booking_id = null)
	{
		...
		$listGroupsFree = $this->getListTablesGroupsFree($date_string, $time, $guests, $exclude_booking_id);
	 ...
	 * @return array
	 */
	public function getListTablesArray($use_static = true, $force_get_all_tables = false, $only_online_tables = true, $date = null)
	{
		...
		{
			$list_of_tables = $this->getListOfTablesForDate($date);
	 ...
	 * @return array
	 */
	public function getListTablesGroupsArray($use_static = true, $date = null)
	{
		static $result;
		...
		if ($this->availability_type == self::AVAILABILITY_SPECIFIC_TABLES) {
			$list_of_tables = $this->getListOfTablesForDate($date);
			$list = $this->filterGroupsWithOnlySpecificTables($list, $list_of_tables);
		}
	 ...
	 * @return array
	 */
	public function fillTablesWithBookings($listTables, $date_string, $time, $guests, $exclude_booking_id = null)
	{
		//ray('fillTablesWithBookings:'.$exclude_booking_id);
		...
		if ($use_cache && isset($listBookings[$key])){
			...
			$query = Booking::where('restaurant_id', $this->restaurant_id)
			...
			// Si estamos modificando una reserva, excluimos sus mesas actuales
	 ...
	 * @return array
	 */
	public function getListTablesFree($date_string, $time, $guests, $force_get_all_tables = false, $check_tables_allowed_guests = false, $exclude_booking_id = null)
	{
		//ray('>>> getListTablesFree service: '. $this->id.' '. $date_string.' '.$time.' '.$guests.' '.$force_get_all_tables);
		// Get all tables available online
		$listTables = $this->getListTablesArray(true, $force_get_all_tables, true, $date_string);
		//ray($listTables);

		// Tables with Bookings attached + if table is blocked assign a booking 99999999
		$listTables = $this->fillTablesWithBookings($listTables, $date_string, $time, $guests, $exclude_booking_id);
	 ...
	 * @return array
	 */
	public function getListTablesGroupsFree($date_string, $time, $guests, $exclude_booking_id = null)
	{
		...
		$tables_id_occupied = $this->getListTablesIdOccupied($date_string, $time, $duration, $exclude_booking_id);
	...
	protected function getGroupTables($group, $date = null)
	{
		$tables = $this->getListTablesArray(true, true, false, $date);
	...
	protected function getListTablesGroupsFree_WithAllTablesOnline($date_string, $time, $guests, $exclude_booking_id = null)
	{
		$groups = $this->getListTablesGroupsFree($date_string, $time, $guests, $exclude_booking_id);
	 ...
	 * @return int[]
	 */
	public function getListTablesIdOccupied($date_string, $slot, $duration, $exclude_booking_id = null)
	{
		...
		// Excluir la reserva que se está modificando
		if ($exclude_booking_id) {
			$query->where('id', '!=', $exclude_booking_id);
		}
