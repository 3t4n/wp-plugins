...
trait SendBookingEmails {
	...
	public function parseEmailTags($message, $force_lang = null)
	{
		...
		$tags = [
            ...
            '{area_table_assigned}' => $this->getAreaTableAssigned(),
	...
	protected function getAreaTableAssigned()
	{
        ...
        $names = Booking::tablesNamesWithAreaArray($list);
        return join(', ', $names);
    }
	...
	protected function sendEmailTemplate($template = 'booking_pending', $lang = null, $type = null, $type_id = null)
	{
        ...
        // PIXEL manager
        // Necesito general la notificacion primero para ponerla en el pixel
        $notification = null;
		if (!preg_match('#_admin#', $template)){
			$notification = $this->storeEmailSent($to, $subject, $content, 'pending', $type, $type_id);
		}

        $pixel_code = alexr_get_pixel_code($this->uuid, $notification ? $notification->id : 0);

		$result = $mailManager->send_email($to, $subject, $content, $pixel_code);
		...
		if (!preg_match('#_admin#', $template)){
            if (!$notification) {
                ...
                $this->updateEmailSent($notification->id, $result);
            }
		}
