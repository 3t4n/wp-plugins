...
class Booking extends Model
{
	...
	public $statusToStore; // Cuando envio manualmente un email status
	...
	public $appends = [
		...
		'emailPendingOpen', // pasa saber si se ha abierto o no
		'emailConfirmedOpen',
		'emailFinishedOpen'
	];
	...
	// CANCEL REASON
	public function setCancelReasonAttribute($reason) {
		$this->set_setting('cancelReason', $reason);
	}

	public function getCancelReasonAttribute() {
		//return 'NADA';
		return $this->get_setting('cancelReason');
	}

	// EMAILS OPENED
	// yes => se ha abierto
	// no => se ha enviado pero no se ha abierto
	// not_sent => no se ha enviado
	protected function loadNotifications(){
		if (!$this->listNotifications)
		{
			$this->listNotifications = BookingNotification::where('restaurant_id', $this->restaurant_id)
	...
	public function getEmailPendingOpenAttribute() {
		if (!defined('ALEXR_PRO_VERSION')) return '-';
		if (intval(ALEXR_PRO_RELEASE) < 102) return '-';

		$this->loadNotifications();

		// Buscar notificaciones con status 'pending'
		$pendingNotifications = array_filter($this->listNotifications, function($notification) {
			return isset($notification['payload']['status']) && $notification['payload']['status'] === BookingStatus::PENDING;
		});

		// Si no hay notificaciones pending, no se ha enviado
		if (empty($pendingNotifications)) {
			return 'not_send';
		}

		// Comprobar si alguna notificación se ha abierto
		foreach ($pendingNotifications as $notification) {
			if (isset($notification['payload']['opens']) && !empty($notification['payload']['opens'])) {
				return 'yes';
			}
		}

		// Si hay notificaciones pero ninguna se ha abierto
		return 'no';
	}

	public function getEmailConfirmedOpenAttribute() {
		if (!defined('ALEXR_PRO_VERSION')) return '-';
		if (intval(ALEXR_PRO_RELEASE) < 102) return '-';

		$this->loadNotifications();

		// Buscar notificaciones con status 'booked'
		$confirmedNotifications = array_filter($this->listNotifications, function($notification) {
			return isset($notification['payload']['status']) && $notification['payload']['status'] === BookingStatus::BOOKED;
		});

		// Si no hay notificaciones confirmed, no se ha enviado
		if (empty($confirmedNotifications)) {
			return 'not_send';
		}

		// Comprobar si alguna notificación se ha abierto
		foreach ($confirmedNotifications as $notification) {
			if (isset($notification['payload']['opens']) && !empty($notification['payload']['opens'])) {
				return 'yes';
			}
		}
	...
	public function getEmailFinishedOpenAttribute() {
		if (!defined('ALEXR_PRO_VERSION')) return '-';
		if (intval(ALEXR_PRO_RELEASE) < 102) return '-';

		$this->loadNotifications();

		// Buscar notificaciones con status 'finished'
		$finishedNotifications = array_filter($this->listNotifications, function($notification) {
			return isset($notification['payload']['status']) && $notification['payload']['status'] === BookingStatus::FINISHED;
		});

		// Si no hay notificaciones finished, no se ha enviado
		if (empty($finishedNotifications)) {
			return 'not_send';
		}

		// Comprobar si alguna notificación se ha abierto
		foreach ($finishedNotifications as $notification) {
			if (isset($notification['payload']['opens']) && !empty($notification['payload']['opens'])) {
				return 'yes';
			}
		}
