...
trait StoreEmailSent {
	...
	protected function storeEmailSent($to = '', $subject = '', $content = '', $result = null, $type = 'email', $type_id = null) {
		...
		$payload = [
			'status' => $this->statusToStore ? $this->statusToStore : $this->status,
			'to' => $to,
	...
	protected function updateEmailSent($notificationId, $result) {

		$notification = BookingNotification::find($notificationId);
		if ($notification) {
