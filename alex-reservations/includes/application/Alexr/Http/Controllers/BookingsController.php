...
class BookingsController extends Controller
{
	...
	{
		//ray('UPDATE', $request->query('tables'));
		...
		//ray('Original tables', $booking->tablesList);
		//ray('New tables', json_decode($request->query('tables')));
		$specific_changes = [
			'tables' => [
				'old' => $booking->tablesList,
				'new' => json_decode($request->query('tables'))
			],
			'tags' => [
				'old' => $booking->tagsList,
				'new' => json_decode($request->query('tags'))
			]
		];

		foreach (['tables', 'tags'] as $key) {
			$old = $specific_changes[$key]['old'];
			$new = $specific_changes[$key]['new'];

			// Creamos copias para no modificar los arrays originales
			$sorted_old = is_array($old) ? $old : [];
			$sorted_new = is_array($new) ? $new : [];

			sort($sorted_old);
			sort($sorted_new);

			if ($sorted_old == $sorted_new) {
				unset($specific_changes[$key]);
			}
		}
		...
		evavel_event(new EventBookingModified($booking, $user, $booking_original, $specific_changes));
	...
	public function updateTags(Request $request)
	{
		...
		if (!$user->canEdit('bookings', $request->tenant)) {
			return $this->response([
				'success' => false,
				'error' => __eva("You cannot edit this booking [updateTags].")
			]);
		}
		...
		if (!$booking) {
			return $this->response([
				'success' => false,
				'error' => __eva("Booking not found.")
			]);
		}

		try {
			// Obtener la lista de tags del request
			$tagsList = $request->params['tagsList'];

			// Guardar estado anterior para el evento
			$old_tags = $booking->tagsList;

			// Original
			$booking_original = ListenBookingEvents::getOriginal($booking);

			// Sincronizar los tags
			$booking->tags()->sync($tagsList);

			// Disparar evento de modificación
			evavel_event(new EventBookingModified(
				$booking,
				$user,
				$booking_original,
				[
			...
			return $this->response([
				'success' => false,
				'error' => __eva("Error updating booking tags: ") . $e->getMessage()
			]);
		}
	}
	...
	protected function haveSeatsChanged($old_tables_seats, $new_tables_seats) {
		...
		*/
		...
		//ray('list_old_seats', $list_old_seats);
		//ray('list_new_seats', $list_new_seats);
