...
class RestaurantsController extends Controller
{
	...
	protected function mapRestaurant($restaurant)
	{
		...
		$new_customers_last_month = Query::table('customers')
		 ...
		 // TESTRangos base para cada métrica
	    //$total_bookings = rand(250, 800);

	    // Bookings último mes: 15-25% del total
	    //$bookings_last_month = intval( round($total_bookings * (rand(15, 25) / 100)));

	    // Total customers: 4-5x total bookings
	    //$total_customers = intval( $total_bookings * (rand(40, 50) / 10) );

	    // Nuevos customers: 60-80% de bookings del último mes
	    //$new_customers_last_month = intval( round($bookings_last_month * (rand(60, 80) / 100)) );
