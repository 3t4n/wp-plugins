...
class TranslateController extends Controller {
	...
	public function save(Request $request)
	{
		...
		if (!file_exists($file_original)) {
			return $this->response([
				'success' => false,
				'error' => __eva('Original language file not found!')
			]);
		}

		$json_original = file_get_contents($file_original);

		// Normalize both JSON strings for comparison (remove whitespace and formatting)
		$original_decoded = json_decode($json_original, true);
		$new_decoded = json_decode($strings, true);

		if ($original_decoded === null || $new_decoded === null) {
			return $this->response([
				'success' => false,
				'error' => __eva('Invalid JSON format')
			]);
		}

		// Compare normalized versions
		if ($original_decoded == $new_decoded) {
			// If content is the same as original, delete the custom translation file
			$custom_file = ALEXR_CUSTOM_TRANSLATION_PATH . "{$lang}.json";
			if (file_exists($custom_file)) {
				if (unlink($custom_file)) {
					return $this->response([
						'success' => true,
						'message' => __eva('Translation file removed as it matches the original')
					]);
				} else {
					return $this->response([
						'success' => false,
						'error' => __eva('Error removing translation file')
					]);
				}
			}
		...
		// If content is different, proceed with saving
		...
		if (file_put_contents($file, $strings) === false) {
			return $this->response([
				'success' => false,
				'error' => __eva('Error saving translation file')
			]);
		}
	...
	public function syncForLanguage($lang)
	{
		...
		// Usar JSON_UNESCAPED_UNICODE para mantener los caracteres UTF-8
		$strings = json_encode($json_decode_original, JSON_UNESCAPED_UNICODE);
