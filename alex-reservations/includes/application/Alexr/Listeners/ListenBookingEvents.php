...
class ListenBookingEvents
{
	...
	public function handle($event)
	{
		...
		switch (get_class($event)) {
			...
			case EventBookingModified::class:
				...
				if ($event->user) {
					$this->actionUserModifiedBooking($event->booking, $event->user, $event->old_attributes, $event->specific_changes);
				} else {
			...
			case EventBookingTablesChanged::class:
				...
				if ($event->user) {
					...
					$this->actionCustomerChangedTables($event->booking, $event->old_tables, $event->new_tables);
				}
	...
	*/
	...
	protected function actionUserModifiedBooking(Booking $booking, User $user, $old_attributes, $specific_changes = null)
	{
		$params = [
			'name' => 'Modified',
			'event_type' => EventBookingModified::class,
		...
		if (is_array($specific_changes) && isset($specific_changes['tables']))
		{
			$old_tables = $specific_changes['tables']['old'];
			$new_tables = $specific_changes['tables']['new'];
			$old_tables_names = [];
			$new_tables_names = [];
			...
			$params['original']['tables_id'] = $old_tables;
			$params['original']['tables_names'] = $old_tables_names;
			$params['changes']['tables_id'] = $new_tables;
			$params['changes']['tables_names'] = $new_tables_names;
		}

		if (is_array($specific_changes) && isset($specific_changes['tags']))
		{
			$old_tags = $specific_changes['tags']['old'];
			$new_tags = $specific_changes['tags']['new'];
			$old_tags_names = [];
			$new_tags_names = [];

			if (is_array($old_tags)) {
				$old_tags_names = BTag::whereIn('id', $old_tags)->get()->pluck('name');
			}

			if (is_array($new_tags)) {
				$new_tags_names = BTag::whereIn('id', $new_tags)->get()->pluck('name');
			}

			$params['original']['tags_id'] = $old_tags;
			$params['original']['tags_names'] = $old_tags_names;
			$params['changes']['tags_id'] = $new_tags;
			$params['changes']['tags_names'] = $new_tags_names;
		}
	...
	protected function actionCustomerChangedTables(Booking $booking, $old_tables, $new_tables)
	{
		$old_tables_names = [];
		$new_tables_names = [];
