...
trait ShiftCalculations {
	...
	use AvailabilityWeekdaysHelpers;
	 ...
	 * @return int[]|string[]
	 */
	public function timeSlotsBookable($date = null)
	{
		$all_slots = $this->mapBlockSlots($date);
	 ...
	 * @return array
	 */
	protected function listSlots($interval, $first_seating, $last_seating) {
		$slots = [];
	 ...
	 * @return array
	 */
	public function mapBlockSlots($date = null)
	{
		$first_seating = $this->getFirstSeatingForDate($date);
		$last_seating = $this->getLastSeatingForDate($date);
		$block_slots = $this->getBlockSlotsForDate($date);

		$slots = $this->listSlots($this->interval, $first_seating, $last_seating);

		// Remove block slots
		foreach ($block_slots as $b_slot) {
	 ...
	 * @return bool
	 */
	public function isTimeSlotBookable($time, $date) {
		$first_seating = $this->getFirstSeatingForDate($date);
		$last_seating = $this->getLastSeatingForDate($date);

		if ($time < $first_seating) return false;
		if ($time > $last_seating) return false;

		// Check it is not blocked
		$slots = $this->mapBlockSlots($date);
	...
	public function isDateTimeBookable($date_string, $time)
	{
		...
		//ray('>>>>>>>>> isDateTimeBookable ' . $date_string. ' ' .$time);
		//ray($list);
	...
	public function openCloseWindowForSlots($booking_date = null, $timezone = null, $filter_only_available = true) {
		...
		$slots = $this->mapBlockSlots($booking_date);
	...
	public function mapTimeSlotCovers($date_string, $include_selected = true) {
		...
		$slots_covers = $this->prepareArraySlots($date_string);
	...
	protected function prepareArraySlots($date) {
		...
		$first_seating = $this->getFirstSeatingForDate($date);
		$last_seating = $this->getLastSeatingForDate($date);
		$slots = $this->listSlots($interval, $first_seating, $last_seating);
	 ...
	 * @return bool
	 */
	public function isAvailable($date_string, $time, $guests, $exclude_booking_id = null) {

		//ray('isAvailable exclude_booking_id:' . $exclude_booking_id);

		// Check date is bookable and rules open/close
		if (!$this->isDateTimeBookable($date_string, $time)) {
			//ray('NO DISPONIBLE ' . $date_string . ' ' . $time);
			return false;
		}
		...
		switch ($this->availability_type)
		{
			case self::AVAILABILITY_ALL_TABLES:
				if (method_exists($this, 'isAvailableBy_SearchTables')){
					$tables = $this->isAvailableBy_SearchTables($date_string, $time, $guests, $requireSingleTablesToBeOnlineForGroups, null, $exclude_booking_id);
					return $tables;
				}
	...
	/** 2025-02-10
	 * Get all available tables and combinations for a specific date, time and number of guests
	 * Returns an array with two keys: 'single_tables' and 'combinations'
	 *
	 * @param string $date_string Date in Y-m-d format
	 * @param int $time Time in seconds from start of day
	 * @param int $guests Number of guests
	 * @param bool $singleTablesRequireToBeOnline Whether single tables need to be online for combinations
	 * @param int|null $areaIdToFilter Optional area ID to filter results
	 *
	 * @return array|false Returns false if no tables available, otherwise returns array with available tables
	 */
	public function getAllAvailableTables( $date_string, $time, $guests, $singleTablesRequireToBeOnline = false, $areaIdToFilter = null ) {
		// Check if we should use table availability
		if ( ! in_array( $this->availability_type, [
			self::AVAILABILITY_ALL_TABLES,
			self::AVAILABILITY_SPECIFIC_TABLES
		] ) ) {
			return false;
		}
		...
		// Add all suitable single tables
		foreach ( $listTablesFree as $table ) {
			if ( $guests >= $table['min_seats'] && $guests <= $table['max_seats'] ) {
				$result['single_tables'][] = $table;
			}
		}

		// Get combinations based on single tables requirement
		...
		// Add all suitable combinations with their component tables
		foreach ( $listGroupsFree as $group ) {
			if ( $guests >= $group['min_seats'] && $guests <= $group['max_seats'] ) {
				$group['tables'] = $this->getGroupTables( $group, $date_string );
				$result['combinations'][] = $group;
			}
		}

		// Sort single tables by seats (ascending)
		usort( $result['single_tables'], function ( $a, $b ) {
			return $a['max_seats'] - $b['max_seats'];
		} );

		// Sort combinations by total seats (ascending)
		usort( $result['combinations'], function ( $a, $b ) {
			return $a['max_seats'] - $b['max_seats'];
		} );

		return ! empty( $result['single_tables'] ) || ! empty( $result['combinations'] ) ? $result : false;
	}
	 ...
	 * @return array
	 */
	public function getAllSlotsAvailable($guests, $date, $exclude_booking_id = null)
	{
		...
		// Full range of slots
		$slots = [];
		for ($seconds = $first_seating; $seconds <= $last_seating; $seconds += $this->interval)
		{
			//ray('Comprobando availability para '.$date.' '.$seconds.' '.$guests);
			if ($this->isAvailable($date, $seconds, $guests, $exclude_booking_id))
			{
				$slots[] = $seconds;
			}
		}
	...
	public function getBookingStatusForNewReservation($date, $time, $party, $email, $exclude_booking_id = null)
	{
		//ray('getBookingStatusForNewReservation: ' . $exclude_booking_id);
		...
		$rules = [
			...
			'rulePendingForXhoursBeforeTimeSlot'
		];

		// If some rule is pending then return pending
		foreach($rules as $rule) {
			$status = $this->{$rule}($date, $time, $party, $status, $email, $exclude_booking_id);
	...
	public function rulePendingAfterXConfirmed($date, $time, $party, $status, $email, $exclude_booking_id = null)
	{
		//ray('rulePendingAfterXConfirmed: ' . $exclude_booking_id);
		...
		$query = Booking::where(evavel_tenant_field(), $this->restaurant_id)
		                   ->where('shift_event_id', $this->id)
		                   ->where('date', $date)
	...
	public function rulePendingMoreThanXSeats($date, $time, $party, $status, $email, $exclude_booking_id = null)
	{
		// Check if rule is enabled
	...
	public function rulePendingForSpecificSlots($date, $time, $party, $status, $email, $exclude_booking_id = null)
	{
		// Check if rule is enabled
	...
	public function rulePendingForCustomers($date, $time, $party, $status, $email, $exclude_booking_id = null)
	{
	...
	public function rulePendingForSpecificDays($date, $time, $party, $status, $email, $exclude_booking_id = null)
	{
		// Check if rule is enabled
	...
	public function rulePendingForXhoursBeforeTimeSlot($date, $time, $party, $status, $email, $exclude_booking_id = null)
	{
		// Check if rule is enabled
		if ($this->rule_hours_before_enable !== true) {
			return $status;
		}

		$hours_before = intval($this->rule_hours_before);
		if ($hours_before <= 0) {
			return $status;
		}

		// Get current time in restaurant's timezone
		$now = evavel_now_timezone($this->restaurant->timezone);

		// Get booking datetime using the provided date and seconds since midnight
		$booking_datetime = evavel_carbon($date, $time, $this->restaurant->timezone);

		// Calculate difference in hours
		$diff_in_seconds = $booking_datetime->timestamp - $now->timestamp;
		$diff_in_hours = $diff_in_seconds / 3600;

		// If booking is within the hours_before window, mark as pending
		if ($diff_in_hours <= $hours_before) {
			return BookingStatus::PENDING;
		}
	...
	//====== FIRST SEATING , LAST SEATING , BLOCK SLOTS ==================================================================
	// Nuevos metodos para poder sobre escribir first_seating, last_seating, block_slots por cada dia de la semana

	public function getFirstSeatingForDate($date = null)
	{
		if ($date == null) return $this->first_seating;

		$dayOfWeek = strtolower(date('D', strtotime($date)));

		$day_active = "working_hours_$dayOfWeek";
		if ($this->{$day_active})
		{
			$key = "first_seating_$dayOfWeek";
			$value = $this->{$key};
			if ($value != null) {
				//ray('First seating custom ' . $value);
				return $value;
			}
		}

		//ray('First seating general ' . $this->first_seating);
		return $this->first_seating;
	}

	public function getLastSeatingForDate($date = null)
	{
		if ($date == null) return $this->last_seating;
		...
		if ($this->{$day_active})
		{
			$key = "last_seating_$dayOfWeek";
			$value = $this->{$key};
			if ($value != null) {
				//ray('Last seating custom ' . $value);
				return $value;
			}
		}

		//ray('Last seating general ' . $this->last_seating);
		return $this->last_seating;
	}

	// Devuelve un listado
	// [ ["time" => 50400, "block" => false], ["time" => 52200, "block" => false] ]
	public function getBlockSlotsForDate($date = null)
	{
		if ($date == null) return $this->block_slots;
		...
		if ($this->{$day_active})
		{
			$key = "block_slots_$dayOfWeek";
			$list_times = $this->{$key};

			$first_seating = intval($this->getFirstSeatingForDate($date));
			$last_seating = intval($this->getLastSeatingForDate($date));
			$interval = intval($this->interval);

			// Prepara un listado de slots con el formato apropiado
			$slots = [];
			for ($i = $first_seating; $i <= $last_seating; $i += $interval) {
				$slot = [
					'time' => $i,
					'block' => false
				];
				if (is_array($list_times) && in_array($i, $list_times)){
					$slot['block'] = true;
				}
				$slots[] = $slot;
			}
			//ray('SLOTS CUSTOM', $slots);
			return $slots;
		}

		//ray('SLOTS GENERAL', $this->block_slots);
		return $this->block_slots;
	}
	...
	public function isBlockingTables()
	{
		if ($this->skip_block_tables == 1) {
			return false;
		}
		return true;
	}
