...
trait SendBookingEmailReminders {
	...
	public function sendEmailReminder( $reminder_id, $lang = 'en' )
	{
		...
		$notification = $this->storeEmailSent($to, $subject, $content, 'pending', 'email_reminder', $reminder_id);

		// Pixel code
		$pixel_code = alexr_get_pixel_code($this->uuid, $notification ? $notification->id : 0);

		$result = $this->mailManager->send_email($to, $subject, $content, $pixel_code);
