...
else
{
	...
	$prim = $wpdb->get_results($wpdb->prepare('SHOW COLUMNS FROM %1s WHERE `Key` = "PRI"', sanitize_key($id)));
	$primKey = empty($prim)?'':$prim[0]->Field;

	if ($action==='delete' && $primKey!=='') 
	{ 
		$rid = atec_clean_request('rid');
		$wpdb->query($wpdb->prepare('DELETE FROM %1s WHERE `%1s`=%s', sanitize_text_field($id), sanitize_text_field($primKey), sanitize_text_field($rid)));
	}
	...
	$total_rows		= intval($wpdb->get_var($wpdb->prepare('SELECT COUNT(*) AS total FROM %1s', sanitize_text_field($id))));
	...
	$results	 		= $wpdb->get_results($wpdb->prepare('SELECT * FROM %1s LIMIT %d, %d', sanitize_text_field($id), sanitize_key($calc_page), sanitize_key($per_page)));
	...
	<table class="atec-table atec-table-tiny fixed">
		<thead>
			<tr>
				<th>#</th>'; 
				foreach ($colsName as $col) { echo '<th', $primKey===$col?' class="atec-violet"':'' ,'>', esc_attr($col), '</th>'; } 			
			if ($primKey!=='') echo '<th></th>';
			echo 
			'</tr>';
		echo 
		'</thead>
	...
	{
		$primValue = $primKey!==''?$row->{$primKey}:'';
		$c++;
		echo 
		'<tr>
			<td class="atec-TDBR">', esc_attr($c), '</td>'; 
			foreach ($row as $col) echo '<td class="atec-anywrap">', esc_attr(atec_short_string($col,256)), '</td>';
			if ($primKey) atec_create_button('delete&nav=Table','trash',true,$url,$id.'&rid='.$primValue,$nonce);
		echo 
		'</tr>';
	}
	echo '
	...
	@require('atec-pagination.php'); new ATEC_pagination($url, $nonce, $total_rows, $per_page, $tblPage);
}
