...
private function atec_group_star_list($mega)
{
	...
	'<div id="pro_package">
		<div class="atec-border-white atec-bg-w atec-fit" style="font-size: 16px !important; padding: 0 20px; text-align: left; margin:0 auto;">
			<ul class="atec-p-0">
				<li>🎁 <strong>', $mega?'Seven additional storage options':esc_attr__('Including 32 valuable plugins','atec-database'), '.</strong></li>
				<li style="line-height:5px;"><br></li>
				<li>⭐ ', esc_attr__('Priority support','atec-database'), '.</li>
				<li>⭐ ', esc_attr__('Upgrades & updates','atec-database'), '.</li>';
				...
				if ($mega) 
					...
					echo '
					<li>⭐ ', esc_attr__('„Lifetime-site-License“','atec-database'), '.</li>
					<li>⭐ ', esc_attr__('Access to all the „PRO“ features','atec-database'), '.</li>';		
			echo 
			'</ul>
		</div>';
}
		...
		'<div class="atec-g atec-border" style="padding: 20px 10px;">';
			...
			elseif ($nav=='Plugins_Overview')
			{
				...
				<div class="atec-g atec-fit" style="margin:0 auto;">';
					atec_table_header_tiny(['','Name (Link)','#wordpress','#admin-multisite',esc_attr__('Status','atec-database'),esc_attr__('Description','atec-database'),'#awards '.esc_attr__('PRO features','atec-database')],'','atec-table-med');
					...
					foreach ($atec_group_arr as $a)
					{
						...
						echo '<tr>';
							...
							else 
							{
								...
								echo 
								'<td>
									<span title="', $inReview?esc_attr__('In review','atec-database'):esc_attr__('In progress','atec-database'), '"><span class="',esc_attr(atec_dash_class($inReview?'visibility':'')) ,'"></span>
								</td>';
							}
				...
				<center>
					<p class="atec-fs-12" style="max-width:80%;">',
						esc_attr__('All our plugins are optimized for speed, size and CPU footprint with an average of only 1 ms CPU time','atec-database'), '.<br>',
						esc_attr__('Also, they share the same „atec-WP-plugin“ framework. Shared code will only load once across multiple plugins','atec-database'), '.	<br>',
						esc_attr__('Tested with','atec-database'), ': Linux (CloudLinux, Debian, Ubuntu), Windows & Mac-OS, Apache, NGINX & LiteSpeed.
					</p>
					<a class="atec-nodeco" class="atec-center" href="https://de.wordpress.org/plugins/search/atec/" target="_blank"><button class="button">', esc_attr__('Visit atec-plugins in the WordPress directory','atec-database'), '.</button></a>
				</center>';
			}
			elseif ($nav==='License')
			{
				...
				'<div class="atec-g atec-center" style="padding: 20px 10px;">
					...
					// @codingStandardsIgnoreEnd
					echo $mega?'Mega-Cache „PRO“ package':esc_attr__('atec-Plugins „PRO“ package','atec-database'), 
					'</h3>';
					...
					'<a class="atec-nodeco" style="width: fit-content !important; margin: 10px auto;" href="', esc_textarea($licenseUrl), '" target="_blank">
						<button class="button button-primary">', esc_attr__('GET YOUR „PRO“ PACKAGE NOW','atec-database'), '</button>
					</a>
					<div class="atec-small">Links to ', esc_textarea($licenseUrl), '</div>';
		
					echo 
					'<p styl="font-size: 18px !important;">',
						esc_attr__('Buy the „PRO“ package through one time payment','atec-database'), '.<br>',
						esc_attr__('The license is valid for the lifetime of your site (domain)','atec-database'), '.<br><b>',
						esc_attr__('No subscription. No registration required.','atec-database'), '</b>
					</p>';
