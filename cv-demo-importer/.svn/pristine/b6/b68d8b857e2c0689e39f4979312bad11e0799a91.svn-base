...
class CVDI_Parser {
	function parse( $file ) {
		...
		if ( extension_loaded( 'simplexml' ) ) {
			...
			$parser = new CVDI_Parser_SimpleXML;
		...
		if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
			...
			echo '<p><strong>' . esc_html__( 'There was an error when reading this WXR file', 'cv-demo-importer' ) . '</strong><br />';

			echo esc_html__( 'Details are shown above. The importer will now try again with a different parser...', 'cv-demo-importer' ) . '</p>';

		}

		// use regular expressions if nothing else available or this is bad XML
		$parser = new CVDI_Parser_Regex;
...
class CVDI_Parser_SimpleXML {
	...
	function parse( $file ) {
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new WP_Error( 'SimpleXML_parse_error', __( 'There was an error when reading this WXR file', 'cv-demo-importer' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version ) {
			return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'cv-demo-importer' ) );
		}
