		...
		public function save_option() {
			...
			if ( $this->csm_request('csm_save_snippet_action') !== null ) {
				...
				if (
					...
					!isset($_POST['csm_save_nonces']) ||
					!wp_verify_nonce(sanitize_key($_POST['csm_save_nonces']), 'csm_save_options' )
				) {
					exit;
				}
							...
							echo $csmForm->elementStart('div', $plugin_slug . 'tabs', FALSE );
								...
								echo $csmForm->elementStart('div', $plugin_slug . 'header', FALSE );
									...
									echo $csmForm->elementStart('div', FALSE, 'left plugin_description' );
										...
										echo $csmForm->link('demo.themeinprogress.eu/content-snippet-manager-pro/', FALSE, FALSE, '_blank', FALSE, esc_html__( ' - Documentation','content-snippet-manager') );
								...
								echo $csmForm->htmlList('ul', FALSE, $plugin_slug . 'navigation', $element['item'], sanitize_text_field(wp_unslash($_GET['tab'])));
							...
							if (isset($value['type'])) :
								...
								case 'start-form':
									...
									echo $csmForm->elementStart('div', str_replace(' ', '', $value['name']), FALSE );
										...
										echo $csmForm->input('csm_save_nonces', FALSE, FALSE, 'hidden', esc_attr(wp_create_nonce( 'csm_save_options' )));
									...
									echo $csmForm->tableElementStart('tbody', FALSE, 'table-body');
										...
										echo $csmForm->tableElementStart('tr', FALSE, 'upsell-row');
											...
											echo $csmForm->tableElementStart('td', FALSE, 'indicator');
												...
												echo $csmForm->link(esc_url(CSM_UPGRADE_LINK . '/?ref=2&campaign=csm-freepro'), FALSE, 'button button-primary', '_blank', FALSE, esc_html__( 'Upgrade now','content-snippet-manager') );
											...
											if ($csm_snippets) {
												...
												foreach ( $csm_snippets as $k => $v) {
													...
													echo $csmForm->elementStart('div', FALSE, $plugin_slug . 'container' );
														...
														echo $csmForm->elementStart('h5', FALSE, 'element linkable-element' );

															echo $csmForm->link('?page=csm_panel&tab=' . sanitize_text_field(wp_unslash($_GET['tab'])) .'&snippetID='.str_replace('slot','',$k), FALSE, FALSE, FALSE, FALSE, $v['name']);
														
														echo $csmForm->elementEnd('h5');
