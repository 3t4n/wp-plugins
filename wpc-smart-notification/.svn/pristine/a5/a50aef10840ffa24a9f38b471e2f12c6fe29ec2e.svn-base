...
class notification {
	...
	function cart() {
		...
		$item['content'] = apply_filters( 'WPCSN/cart/item_content', sprintf( $link, sprintf( Wpcsn_Helper()->localization( 'your_cart',/* translators: cart */ esc_html__( 'Your cart: %s', 'wpc-smart-notification' ) ), initialization::get_cart() ) ) );
		...
		if ( $the_query->have_posts() ) {
			while ( $the_query->have_posts() ) {
				...
				if ( $time_ago < HOUR_IN_SECONDS ) {
					$minute   = round( $time_ago / MINUTE_IN_SECONDS );
					$time_str = sprintf( 1 === (int) $minute ? Wpcsn_Helper()->localization( 'minute_ago',/* translators: minute */ esc_html__( 'about %d minute ago.', 'wpc-smart-notification' ) ) : Wpcsn_Helper()->localization( 'minutes_ago',/* translators: minute */ esc_html__( 'about %d minutes ago.', 'wpc-smart-notification' ) ), $minute );
				} elseif ( $time_ago < DAY_IN_SECONDS ) {
					$hour     = round( $time_ago / HOUR_IN_SECONDS );
					$time_str = sprintf( 1 === (int) $hour ? Wpcsn_Helper()->localization( 'hour_ago',/* translators: hour */ esc_html__( 'about %d hour ago.', 'wpc-smart-notification' ) ) : Wpcsn_Helper()->localization( 'hours_ago',/* translators: hour */ esc_html__( 'about %d hours ago.', 'wpc-smart-notification' ) ), $hour );
				} else {
					$day      = round( $time_ago / DAY_IN_SECONDS );
					$time_str = sprintf( 1 === (int) $day ? Wpcsn_Helper()->localization( 'day_ago',/* translators: day */ esc_html__( 'about %d day ago.', 'wpc-smart-notification' ) ) : Wpcsn_Helper()->localization( 'days_ago',/* translators: day */ esc_html__( 'about %d days ago.', 'wpc-smart-notification' ) ), $day );
				}

				$item = [];

				foreach ( $order->get_items() as $item_id => $_item ) {
					$product = apply_filters( 'WPCSN/new_orders/item_product', wc_get_product( $_item->get_product() ), $_item );
					$product = apply_filters( 'WPCSN/item_product', $product );

					if ( is_a( $product, 'WC_Product' ) && ! empty( $product->get_name() ) ) {
						...
						$item['content']   = sprintf( $link, esc_url( $product->get_permalink() ), esc_html( $product->get_name() ) );
						break;
					}
				}
				...
				$item['title']     = apply_filters( 'WPCSN/new_orders/item_title', sprintf( Wpcsn_Helper()->localization( 'name_address',/* translators: name & address */ esc_html__( '%1$s from %2$s purchased a', 'wpc-smart-notification' ) ), '<strong>' . $_name . '</strong>', '<strong>' . $_address . '</strong>' ), $_name, $_address, $order );
	...
	function virtual_orders() {
		...
		while ( $index < $number ) {
			...
			$product    = apply_filters( 'WPCSN/virtual_orders/item_product', wc_get_product( $product_id ), $product_id );
			$product    = apply_filters( 'WPCSN/item_product', $product );
			...
			$link = '<a href="%s" target="' . esc_attr( $this->options['options']['link'] === 'new' ? '_blank' : '_self' ) . '" ' . ( $this->options['options']['link'] === 'woosq' ? 'class="woosq-btn" data-id="' . esc_attr( $product->get_id() ) . '"' : '' ) . '>%s</a>';
			...
			$item['title']     = apply_filters( 'WPCSN/virtual_orders/item_title', sprintf( Wpcsn_Helper()->localization( 'name_address',/* translators: name & address */ esc_html__( '%1$s from %2$s purchased a', 'wpc-smart-notification' ) ), '<strong>' . $name . '</strong>', '<strong>' . $address . '</strong>' ), $name, $address, $data );
			$item['content']   = apply_filters( 'WPCSN/virtual_orders/item_content', sprintf( $link, esc_url( $product->get_permalink() ), esc_html( $product->get_name() ) ), $product, $data );
	...
	function on_sale_products() {
		...
		foreach ( $product_ids as $product_id ) {
			$product = apply_filters( 'WPCSN/on_sale_products/item_product', wc_get_product( $product_id ), $product_id );
			$product = apply_filters( 'WPCSN/item_product', $product );
			...
			$item['title']     = apply_filters( 'WPCSN/on_sale_products/item_title', sprintf( Wpcsn_Helper()->localization( 'on_sale',/* translators: product */ esc_html__( '%s is on-sale. Hurry up!', 'wpc-smart-notification' ) ), sprintf( $link, esc_url( $product->get_permalink() ), esc_html( $product->get_name() ) ) ), $product, $product_ids );
	...
	function low_stock_products() {
		...
		if ( $the_query->have_posts() ) {
			while ( $the_query->have_posts() ) {
				...
				$product    = apply_filters( 'WPCSN/low_stock_products/item_product', wc_get_product( $product_id ), $product_id );
				$product    = apply_filters( 'WPCSN/item_product', $product );
				...
				$item['title']     = apply_filters( 'WPCSN/low_stock_products/item_title', sprintf( Wpcsn_Helper()->localization( 'low_stock',/* translators: product & stock */ esc_html__( '%1$s has %2$d left only. Don\'t miss it!', 'wpc-smart-notification' ) ), sprintf( $link, esc_url( $product->get_permalink() ), esc_html( $product->get_name() ) ), $product->get_stock_quantity() ), $product );
	...
	function related_products() {
		...
		$product    = apply_filters( 'WPCSN/related_products/item_product', wc_get_product( $product_id ), $product_id );
		$product    = apply_filters( 'WPCSN/item_product', $product );
		...
		foreach ( $related_product_ids as $product_id ) {
			...
			$item['title']     = apply_filters( 'WPCSN/related_products/item_title', sprintf( Wpcsn_Helper()->localization( 'also_like',/* translators: product */ esc_html__( 'You may also like %s', 'wpc-smart-notification' ) ), sprintf( $link, esc_url( $product->get_permalink() ), esc_html( $product->get_name() ) ) ), $product );
	...
	function viewing() {
		...
		$product_id = ! empty( $this->args['ID'] ) ? absint( $this->args['ID'] ) : 0;
		$product    = apply_filters( 'WPCSN/viewing/item_product', wc_get_product( $product_id ), $product_id );
		$product    = apply_filters( 'WPCSN/item_product', $product );
		...
		if ( is_a( $product, 'WC_Product' ) ) {
			...
			$viewing = sprintf( Wpcsn_Helper()->localization( 'viewing',/* translators: counter */ esc_attr__( '%s people are viewing this product now.', 'wpc-smart-notification' ) ), rand( $min, $max ) );
