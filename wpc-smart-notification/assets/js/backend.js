    ...
    class wpcsnAdmin {
        ...
        AddMoreEvent($el) {
            ...
            $el.find('[href="#add-more"]').click(function (e) {
                ...
                html.find('[data-select2-id]').removeAttr('data-select2-id tatoggleex aria-hidden');
                html.find('.item-wrapper[data-field-name] .item-inner').first().siblings('.item-inner').remove();
        ...
        EventWidget($el) {
            ...
            $el.on('change', 'input, select, textarea', function (e) {
                ...
                if ($.type(_this.toggle.$bind[index]) == 'array') {
                    ...
                    if ($(this).attr('type') == 'checkbox') {
                        value = $(this).parents('.widget-' + _this.namePrefix + '-checkbox').first().find('input:checked').map(function () {
                            return $(this).val();
                        }).get();
                    } else {
                        value = [$(this).val()];
            ...
            $el.find('[data-js="select2"]:not([data-select2-id])').each(function (index, value) {
            ...
            if ($.fn.tipsy) {
                $el.find('input[title], select[title], textarea[title]').addClass('has-tip');
