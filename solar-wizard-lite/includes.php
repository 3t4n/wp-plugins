...
function solwzd_enqueue_script(){   
	...
	wp_enqueue_script( 'sw_analytics', 'https://io.solarwizardplugin.com/cdn/swizsdk.js', array('jquery'), SOLWZD_VERSION, false );
	wp_enqueue_script( 'svg_script', plugin_dir_url( __FILE__ ) . 'js/svg-inject.min.js', array('jquery'), SOLWZD_VERSION, false );
	wp_enqueue_script( 'sw_script', plugin_dir_url( __FILE__ ) . 'js/custom.js', array('jquery'), SOLWZD_VERSION, false );
	wp_localize_script( 'sw_script', 'ajax_object',array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );	
	wp_enqueue_script( 'validation_script', plugin_dir_url( __FILE__ ) . 'js/jquery.validate.js', array('jquery'), SOLWZD_VERSION, false );
	
	if(get_option('sw_google_autocomplete_address') != ''){
		wp_enqueue_script( 'map_script', 'https://maps.googleapis.com/maps/api/js?key='.get_option('sw_google_autocomplete_address').'&libraries=places', array('jquery'), SOLWZD_VERSION, false );
	}
    wp_enqueue_style( 'ui_style', plugin_dir_url( __FILE__ ) . 'css/jquery-ui.css', array(), SOLWZD_VERSION, false );
    wp_enqueue_style( 'sw_style', plugin_dir_url( __FILE__ ) . 'css/style.css', array(), SOLWZD_VERSION, false );
}
