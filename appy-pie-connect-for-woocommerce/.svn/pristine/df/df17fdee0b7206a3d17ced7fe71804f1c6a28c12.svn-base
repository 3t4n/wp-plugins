   ...
   //filter wc product by category by slug
    ...
    function create_product($request)
    {
            ...
            ->data->user_pass, $user->ID))
        {
            ...
            if (empty($post_title))
            {
                $error->add(400, __("Product name is required.", 'appy-pie-connect-for-woocommerce') , array(
                    'status' => 400
                ));
                return $error;
            }
    ...
    function delete_product($request){
            ...
            ->data->user_pass, $user->ID))
        {
        	...
        	if(empty($product)){
                ...
                return new WP_Error(400, sprintf(__('No %1$s is associated with #%2$d', 'appy-pie-connect-for-woocommerce'), 'product', $request['product_id']));
        	...
        	if($deleted) {
                ...
                return new WP_Error(404, sprintf(__('This %1$s cannot be deleted', 'appy-pie-connect-for-woocommerce'), 'product'));
            }
...
function appy_pie_connect_custom_field_to_variations( $loop, $variation_data, $variation ) {
  woocommerce_wp_text_input( array(
    ...
    'class' => 'short',
    'label' => __( 'Custom Field', 'appy-pie-connect-for-woocommerce' ),
