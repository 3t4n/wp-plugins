    ...
    class Appy_Pie_Connect_Woocommece_custom_api
    {
        ...
        function get_user_list(WP_REST_Request $request)
        {
            ...
            if (empty($username) || empty($password)) {
                return new WP_Error('error', __('Username and password are required.', 'appy-pie-connect-for-woocommerce'), array('status' => 400));
            }
        
            $user = get_user_by('login', $username);
            if (!$user || !wp_check_password($password, $user->data->user_pass, $user->ID)) {
                return new WP_Error('error', __('Incorrect credentials. Try again.', 'appy-pie-connect-for-woocommerce'), array('status' => 401));
            }
            ...
            if ($userRole !== 'administrator') {
                return new WP_Error('error', __('Sorry, you are not allowed to access this.', 'appy-pie-connect-for-woocommerce'), array('status' => 403));
            }
            ...
            if (empty($userData)) {
                return new WP_Error('error', __('No record found.', 'appy-pie-connect-for-woocommerce'), array('status' => 400));
            }
        ...
        function add_new_user(WP_REST_Request $request)
        {
            ...
            if (!is_email($email)) {
                return new WP_Error('invalid_email', __("Invalid email format.", 'appy-pie-connect-for-woocommerce'), array('status' => 400));
            }
            ...
            if (empty($username))
            {
                $error->add(400, __("Username field 'username' is required.", 'appy-pie-connect-for-woocommerce') , array(
                    'status' => 400
                ));
                return $error;
            }
            ...
            if (empty($email))
            {
                $error->add(401, __("Email field 'email' is required.", 'appy-pie-connect-for-woocommerce') , array(
                    'status' => 400
                ));
                return $error;
            }
            if (empty($password))
            {
                $error->add(404, __("Password field 'password' is required.", 'appy-pie-connect-for-woocommerce') , array(
                    'status' => 400
                ));
                return $error;
            }
            ...
            if (!$user_id && email_exists($email) == false)
            {
                ...
                if (!is_wp_error($user_id))
                {
                    ...
                    $translated_text = __("User '%s' Thanks for registration, check your inbox!", "appy-pie-connect-for-woocommerce");
            ...
            {
                $error->add(406, __("Email already exists, please try 'Reset Password'", 'appy-pie-connect-for-woocommerce') , array(
                    'status' => 400
                ));
                return $error;
            }
        ...
        function delete_wc_user(WP_REST_Request $request)
        {
            ...
            return array(
                'code' => 200,
                'message' => __("User was deleted successfully", "appy-pie-connect-for-woocommerce")
            );
        }
        ...
        function get_user_info_by_userid(WP_REST_Request $request)
        {
            ...
            if ($user_id <= 0 || !($user = get_user_by('ID', $user_id))) {
                return new WP_Error('invalid_user_id', __('Sorry, invalid User ID', 'appy-pie-connect-for-woocommerce'), array('status' => 400));
            }
        ...
        function user_login_route(WP_REST_Request $request)
        {
            ...
            if (empty($username)) {
                return new WP_Error('missing_username', __("Username field 'username' is required.", 'appy-pie-connect-for-woocommerce'), array('status' => 400));
            }

            if (empty($password)) {
                return new WP_Error('missing_password', __("Password field 'password' is required.", 'appy-pie-connect-for-woocommerce'), array('status' => 400));
            }
            ...
            return new WP_Error('invalid_credentials', __('Sorry, invalid username or password!', 'appy-pie-connect-for-woocommerce'), array('status' => 401));
        }
