...
function order_management_page_load()
{
	...
	if(filter(foxypress_FixPostVar("foxy_om_mode", "")) != "")
	{
		$Page_Mode =  filter(foxypress_FixPostVar("foxy_om_mode", ""));	
	}
	else if(filter(foxypress_FixGetVar('cid')) != null)
	{
		$Page_Mode = filter(foxypress_FixGetVar("mode", "search"));
	}
	else 
		$Page_Mode = filter(foxypress_FixGetVar("mode", "list"));
			...
			if($foxyXMLResponse->result == "SUCCESS")
			{
				...
				echo("<ul class=\"subsubsub\">
						...
						<form method=\"POST\" name=\"statusForm\" id=\"statusForm\">
							<table>
								...
								<tr>
									...
									<td><input type=\"submit\" class=\"button-primary\" id=\"foxy_om_transaction_submit\" name=\"foxy_om_transaction_submit\" value=\"Save\" /> <img src=\"" . plugins_url() . "/foxypress/img/ajax-loader.gif\" id=\"foxypress_transaction_submit_loading\" name=\"foxypress_transaction_submit_loading\" style=\"display:none;\" /><span id=\"transaction_submit_msg\"></span></td>
								</tr>
							</table>
						</form>
					</div>
					<div id=\"divViewCustomer\">
						...
						"<br> " . __('Customer ID', 'foxypress') . ": " . $foxyXMLResponse->transaction->customer_id . 
                    "</div>
				...
				echo("<form name=\"noteForm\" id=\"noteform\" method=\"POST\">
						...
						<input type=\"submit\" class=\"button\" name=\"foxy_om_note_submit\" id=\"foxy_om_note_submit\" value=\"" . __('Add Note', 'foxypress') . "\" />
					  </form>");
				...
				if(count($t_options)==0){
					...
					echo "	</select>" .
						 "	<input type=\"submit\" id=\"foxy_om_email_template_\" name=\"foxy_om_email_template_\" value=\"Ok\" class=\"button\" />";
				}
	...
	{
		if(foxypress_FixGetVar('cid') != '' && is_numeric(foxypress_FixGetVar('cid')))
		{
			$SearchValue = foxypress_FixGetVar('cid');
		}
		else
...
function ProcessSearch($SearchValue)
{
	...
	if($SearchValue == "")
	{
		_e("Invalid search term, please try again. You can search by First Name, Last Name, Customer ID, or Transaction ID.", "foxypress");
	}
	else
	{
		...
		$SearchSQL = "SELECT *
					  ...
					  SELECT *
					  	FROM " . $wpdb->prefix ."foxypress_transaction
						WHERE foxy_transaction_customer_id like '%" . $SearchValue. "%'
						AND foxy_blog_id = " . ((foxypress_IsMainBlog()) ? "foxy_blog_id" : "'" . $wpdb->blogid . "'") . "
					  UNION
					  SELECT *
					  	FROM " . $wpdb->prefix ."foxypress_transaction
		...
		if( !empty($Transactions) )
		{
			foreach($Transactions as $t)
			{
				echo("<tr>
							<td>
								<a href=\"?post_type=" . FOXYPRESS_CUSTOM_POST_TYPE . "&page=order-management&transaction=" . $t->foxy_transaction_id . "&mode=detail\">" . $t->foxy_transaction_id . "</a>
							</td>
							<td>" . $t->foxy_transaction_date . "</td>
							<td>" . $t->foxy_transaction_last_name . ", " . $t->foxy_transaction_first_name . "</td>
							<td>" . $t->foxy_transaction_email . "</td>
...
function Begin_Foxy_Order_Management()
{
	...
	<div class="wrap">
        ...
        <div>
            <div><i><?php _e('Search by First Name, Last Name, Customer ID, or Transaction ID', 'foxypress'); ?></i></div>
	...
	if($foxyXMLResponse->result == "SUCCESS")
	{
		foreach($foxyXMLResponse->transactions->transaction as $t)
		{
			...
			$sql = "INSERT IGNORE INTO " . $wpdb->prefix ."foxypress_transaction" .
				  ...
				  ", foxy_transaction_customer_id='" . mysql_escape_string($t->customer_id) . "'" .
