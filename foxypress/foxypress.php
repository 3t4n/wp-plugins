...
Copyright (C) 2008-2014 WebMovement, LLC
...
define('WP_FOXYPRESS_CURRENT_VERSION', "0.4.11");
...
if ( !empty ( $foxyStoreURL ) ){
    ...
    FoxyCart_Helper::$cart_url = "https://" . $foxyStoreURL . "/cart";
...
/**
 * Cleans up provided string of data
 * 
 * @param string $data String to filter
 * @return Filtered version of the provided string parameter
 */
...
/**
 * Includes the uploadify library for users that can upload files
 */
...
/**
 * Includes the document handler library for users that can upload files
 * 
 */
...
/**
 * Loads the langauges directory for multi-lingual capabilities
 * 
 */
...
/**
 * Sets up the FoxyPress sidebar menu in the admin panel
 * 
 */
function foxypress_menu()
{
	...
	if ( !empty ( $foxyStoreURL  ) )
	{
		...
		add_submenu_page('edit.php?post_type=' . FOXYPRESS_CUSTOM_POST_TYPE, __('Customer Management', 'foxypress'), __('Customer Management', 'foxypress'), 'manage_options', 'customer-management', 'customer_management_page_load');
		...
		add_submenu_page('edit.php?post_type=' . FOXYPRESS_CUSTOM_POST_TYPE, __('Custom Order', 'foxypress'), __('Custom Order', 'foxypress'), 'manage_options', 'custom-order', 'custom_order_page_load');
...
/**
 * Save meta data for a certain product post
 * 
 * @param int $post_id ID of a certain product post
 * @param string $fieldname Name of the field to be saved
 * @param string $input Value of the field to be saved
 */
...
/**
 * Set up fields in the DOM for an affiliate user's profile
 * 
...
/**
 * Sets up affiliate profile start
 */
...
<?php }
/**
 * Sets up client affiliate profile 
 */
...
/**
 * This method can be used to refresh WordPress' rewrite rules cache related to the FoxyPress User Portal
 * 
 */
 ...
 * This method can be used to refresh WordPress' rewrite rules cache related to the FoxyPress User Portal
 * 
 * @param string $rules Pass the existing rules as a string.
 *
 * @return string The new rewrite rules.
 */
...
/**
 * Sets the FoxyPress User Portal variables.
 * 
 * @param array Accepts an array of user variables.
 *
 * @return array Returns an array of FoxyPress user portal variables.
 */
...
/**
 * Flushes the rewrite rules/permalink structure.
 *
 */
...
/**
 * Link stylesheets in admin panel
 */
...
/**
 * Display transaction stats in the dashboard
 */
...
/**
 * Setup dashboard widget for statistics
 */
...
/**
 * Add buttons to wysiwyg editor for certain users
 */
function foxypress_addbuttons() {
   ...
   // Add only in Rich Editor mode
   	if ( get_user_option('rich_editing') == 'true') {
    	...
    	add_filter('mce_buttons', 'foxypress_register_plugin_button');
   	}
}

/**
 * Registers the buttons to wysiwyg editor for certain users
 * 
 */
...
/**
 * Send email with FoxyPress settings
 * @param string $mail_to Email recipient
 * @param string $mail_subject Email subject
 * @param string $mail_body Email body text
 * @param string $mail_from Email from address
 * @param bool $plaintext If plain text or HTML email
 */
...
/**
 * Get the symbold of currency used on thes site
...
/**
 * @param int $price Price to format based on FoxyPress currency setting
 * @return int Formatted price with currency symbol
 */
...
/**
 * Setup search module form and format results
 * @return string HTML of search form and search results to be inserted into the DOM
 */
...
/**
 * @param int $ID Product ID of product to retrive URL
...
/**
 * @return string HTML for tracking module
 */
...
/**
 * Get min and max quantities' hidden input fields for a product
 * @param int $DownloadableID Product ID of downloadable item
 ...
 */
function foxypress_handle_shortcode_list($items, $ItemsPerRow=2, $ShowDescription, $showMoreDetail=false, $ShowMainImage=true, $ShowThumbs=false, $ShowAddToCart=false, $ShowQuantityField=false, $order = 'ASC', $orderby = 'sort_order')
{
	$output = "";
	...
	if (!empty($items)) {
		...
		for ($i = 0; $i < count($items); $i++) {
			...
			// Print item
			$output .= foxypress_handle_shortcode_item($items[$i], $ShowDescription, $showMoreDetail=false, $ShowAddToCart, $ShowMainImage, $ShowThumbs, $ShowQuantityField, $CssSuffix, true, $order, $orderby);
...
/**
 * Shortcode function to display a list of items. 
 * 
 * @param int $CategoryID An integer of the category ID to output in list
 * @param int $Limit Number of items to display (default = 5)
 * @param int $ItemsPerRow Items per HTML row 
 * @param bool $showMoreDetail Show detail for product
 * @param bool $ShowMainImage Show or hide main product image (default = true)
 * @param bool $ShowThumbs Show or hide extra product thumbnail images (default = false)
 * @param bool $ShowAddToCart Show or hide Add to cart button (default = false)
 * @param bool $ShowQuantityField Show/enable quantity field (default = false)
 * @param string $order Order results in ascending or descending order (default = 'ASC')
 * @param string $orderby Order results by column (post_title, post_date, etc) (default = sort_order)
 * @param bool $ShowDescription Show or hide product description (default=true)
 * @return string HTML output of products
 */
function foxypress_handle_shortcode_listing($CategoryID, $Limit=5, $ItemsPerRow=2, $showMoreDetail=false, $ShowMainImage=true, $ShowThumbs=false, $ShowAddToCart=false, $ShowQuantityField=false, $order = 'ASC', $orderby = 'sort_order', $ShowDescription=true)
{
	global $wpdb;
	$Output = "";
	...
	$items = $wpdb->get_results("SELECT i.*
								...
								WHERE i.post_type = '" . FOXYPRESS_CUSTOM_POST_TYPE . "'
									...
									AND pm_active.meta_value = '1'
									AND (coalesce(pm_start_date.meta_value, now()) <= now() AND coalesce(pm_end_date.meta_value, now()) >= now())
								ORDER BY i" . ($orderby == 'sort_order' ? 'c.' : '.') . $orderby . ' ' . $order . 
								" LIMIT $start, $Limit");
	...
	if(!empty($items))
	{
		...
		foreach($items as $item)
		{
			...
			$Output .= foxypress_handle_shortcode_item($item->ID, $ShowDescription, $showMoreDetail, $ShowAddToCart, $ShowMainImage, $ShowThumbs, $ShowQuantityField, $CssSuffix, true);
			$counter++;
...
/**
 * Retrieve main image of inventory product
 * 
 * @param int $inventory_id Product ID
 * @return string relative URL of inventory product image
 */
...
/**
 * Retrieve the first image of an inventory product 
 * 
 * @param int $inventory_id Product ID
 * @return string relative URL of the first inventory product image
 */
...
/**
 * Deletes an image from an inventory product
 * 
 * @param int $inventory_image_id ID of product image
...
/**
 * Retrieves all inventory image sources for a given product ID
 *
 * @since 0.4.6
 *
 * @param int $post_id ID of post to retrieve images for
 *
 * @return Inventory images as array
 */
function foxypress_GetInventoryImageSources($post_id) {
	$allInventoryImages = array();
    
    // Get featured image if it exists
    $featuredImage = foxypress_GetFeaturedInventoryImage($post_id);
    if (!empty($featuredImage)) {
        array_push($allInventoryImages, $featuredImage);
    }
    ...
    foreach ($inventory_images as $inventoryImage) {
        $imageSrc = wp_get_attachment_image_src($inventoryImage->attached_image_id, "full");
        array_push($allInventoryImages, $imageSrc[0]);
    }
...
/**
 * Handles shortcode for displaying a single item
 * 
 * @param int $InventoryID Product ID
 * @param bool $showMoreDetail Show detail for product
 * @param bool $ShowAddToCart Show or hide Add to cart button (default = false)
 * @param bool $ShowMainImage Show or hide main product image (default = true)
 * @param bool $ShowQuantityField Show/enable quantity field (default = false)
 * @param string $CssSuffix Suffix for adding the proper div class
 * @param bool $IsPartOfListing (default = false)
 * @param bool $order Order results in ascending or descending order (alphabetically) (default = 'ASC')
 * @param bool $orderby Order results by column (post_title, post_date, etc) (default = sort_order)
 * @return HTML output to display product
 */
function foxypress_handle_shortcode_item($InventoryID, $ShowDescription = true, $showMoreDetail = false, $ShowAddToCart = true, $ShowMainImage = true, $ShowThumbs = false, $ShowQuantityField = false, $CssSuffix = '', $IsPartOfListing = false, $order = 'ASC', $orderby = 'post_title')
{
	global $wpdb; global $foxyStoreURL;
	...
	if($showMoreDetail)
	{
		$MoreDetail = "<div class=\"foxypress_item_readmore\"><a href=\"" . foxypress_get_product_url($item->ID) . "\">Read More</a></div>";
	}
	...
	if($ShowAddToCart)
	{
		...
		$Output = "<div class=\"foxy_item_wrapper" . $CssSuffix . "\">
				   		<div class=\"foxypress_item_content_wrapper" . $CssSuffix . "\">
							<form action=\"https://" . $foxyStoreURL . "/cart\" method=\"POST\" class=\"foxycart\" accept-charset=\"utf-8\" id=\"foxypress_form\">"
								.
									( (!$showQuantityField)
								...
								<input type=\"hidden\" name=\"h:affiliate_id\" value=\"" . $affiliate_id . "\" />"
								 ...
								 ($ShowDescription ? "<div class=\"foxypress_item_description" . $CssSuffix . "\">" . stripslashes($item->post_content) . "</div>" : '')
								 .
	...
	else
	{
		...
		$Output = "<div class=\"foxy_item_wrapper\">
						...
						<div class=\"foxypress_item_content_wrapper\">
							<div class=\"foxypress_item_name\">" . 
								( ($MoreDetail == "")
								? "<a href=\"" . foxypress_get_product_url($item->ID) . "\">" . stripslashes($item->post_title)  . "</a>"
								: stripslashes($item->post_title) 
							) 
							. "</div>
							...
							if($ShowDescription)
							{
								$Output .= "<div class=\"foxypress_item_description\">" . stripslashes($item->post_content) . "</div>";
							}

				$Output .=		foxypress_BuildAttributeList($InventoryID, "")
							.
								$MoreDetail;
				$Output .=  "</div>
					</div>";
...
/**
 * Handle shortcode to display detail of product
 * @param bool $ShowMainImage Show or hide main product image
 * @param bool $ShowQuantityField Show/enable quantity field
 * @param int $inventory_id ID of inventory product
 * @return string HTML output to display product detail
 */
function foxypress_handle_shortcode_detail($showMainImage, $showQuantityField, $inventory_id = '')
{
	...
	if(isset($_SESSION['affiliate_id'])){$affiliate_id=$_SESSION['affiliate_id'];}else{$affiliate_id="";}
	$CssSuffix="";
	$ItemDescri = /*(strpos(stripslashes(do_shortcode($item->post_content)), '<form') !== false) ? stripslashes(do_shortcode($item->post_content)) : */stripslashes($item->post_content);
				...
				<div class=\"foxy_item_wrapper_detail\">
					<form action=\"https://" . $foxyStoreURL . "/cart\" method=\"POST\" class=\"foxycart\" accept-charset=\"utf-8\" id=\"foxypress_form\">"
						...
						<input type=\"hidden\" name=\"h:affiliate_id\" value=\"" . $affiliate_id . "\" />"
						 ...
						 .
						 "</div>
						 <div class=\"foxypress_item_description_detail\">" .  $ItemDescri . "</div>"
						 .
...
/**
 * @param int @DownloadableID ID of inventory product
 * @return bool if product is downloadable product
 */
 ...
 * @param int $_weight1
 * @param int $_weight2
 * @return actual weight of product
 */
...
/**
 * Calculate and return price of product
 * @param number $price Price of product
 * @param number $saleprice Sale price of product
 * @param string $startdate String format of starting sale date
 * @param string $enddate String format of ending sale date
 * @return number Price of product currently
 */
...
/**
 * Determines if inventory product can be added to the cart
 * @param int $inventory_id ID of inventory product
 * @param int $quantity Quantity of product to be added to the cart
 * @return bool if inventory product can be added to the cart
 */
...
/**
 * Build list of all product attributes as hidden input fields
 * @param int $inventory_id ID of inventory product
 * @return string HTML output of attributes and their values as hidden input fields
 */
...
/**
 * Build select list of options for an inventory product
 * @param int $inventory_id ID of inventory product
 * @param int $formid ID of product form
 * @param int $defaultMaxQty Maximum allowed quantity of product
 * @return string HTML output to display product select options
 */
...
/**
 * Build list of attributes for the admin panel
 * @param int $inventory_id ID of inventory product
 * @param string $CssSuffix Suffix for adding the proper div class
 * @return string HTML output to display product detail
 */
...
*/
/**
 * Process the right shortcode based on attributes provided
 * @param array $atts Array of attributes in the shortcode
 * @param string $content
 * @return string HTML output created from shortcod
 */
function foxypress_shortcode( $atts, $content = null) {
	...
	$order = (trim($atts['order']) == '') ? 'ASC' : $atts['order'];
	$orderby = (trim($atts['orderby']) == '') ? 'sort_order' : $atts['orderby'];
	$ShowDescription = (trim($atts['showdescription']) == "1") ? true : false;
	$showMainImage = ((strtolower(trim($atts['show_main_image'])) == "1") || (strtolower(trim($atts['show_main_image'])) == ""))  ? true : false;
	$ShowThumbs = trim($atts['show_thumbs']) == "1"  ? true : false;
	...
	{
		$returnHTML =  foxypress_handle_shortcode_item(trim($atts['id']), $ShowDescription, $showMoreDetail, true, $showMainImage, $ShowThumbs, $showQuantity, "_single", false);
	}
	else if(trim($atts['categoryid']) != '' && $mode == 'list')
	{
		$returnHTML = foxypress_handle_shortcode_listing(trim($atts['categoryid']), $limit, trim($atts['cols']), $showMoreDetail, $showMainImage, $ShowThumbs, $showAddToCart, $showQuantity, $order, $orderby, $ShowDescription);
	}
	else if($mode == 'related')
	{
		$related_items = foxypress_GetRelatedItems(trim($atts['productid']));
		$returnHTML = foxypress_handle_shortcode_list($related_items, trim($atts['cols']), $ShowDescription, $showMoreDetail, $showMainImage, $ShowThumbs, $showAddToCart, $showQuantity, $order, $orderby);
	}
	else if($mode == 'detail')
	{
...
/**
 * Get number of products in category, providing a category name
 * @param string $category_name Name of category
 * @return int Number of product in a category
 */
...
/**
 * Get value of GET query parameter
 * @param string $variable Name of GET parameter
 * @param string $default Default value of parameter
 * @return string value of GET query parameter
 */
...
/**
 * Get value of POST query parameter
 * @param string $variable Name of POST parameter
 * @param string $default Default value of parameter
 * @return string value of POST query parameter
 */
...
/**
 * Delete file
 * @param string $fileloc Location of file to delete
 */
...
/**
 * Parses the file extension for a given filename.
 * 
...
/**
 * Generate new file name for a provided file
 * @param string $fileExtension 
 * @param int $inventory_id ID of inventory product
 * @param string $targetpath Path of new file location
 * @param string $prefix Prefix of new file name
...
/**
 * Encrypts an item for HMAC validation
 * 
 * @param string $item Item to encrypt.
 *
 * @return string Returns encrypted item.
 */
...
/**
 * Decrypts an item for HMAC validation
 * 
 * @param string $item Item to decrypt.
 *
 * @return string Returns decrypted item.
 */
...
/**
 * Make a curl request to FoxyCart server
 * @param string $url URL to make a curl post request to 
 * @param string $postData Data attached to request
 ...
 * @param bool $includeQS (default = true)
...
/**
 * Removes query string values from url
 * 
 * @param string $url Url to modify
 * @param string $remove Query string parameter to remove from url.
 *
 * @return string Returns a modified string based on input.
 */
...
/**
 * Adds query string values to url
 * 
 * @param string $url Url to modify
 * @param string $key Query string key to add to url.
 * @param string $value Query string value to add to url.
 *
 * @return string Returns a modified string based on input.
 */
 ...
 * @param int $total_pages Total number of page
 * @param int $limit Number of page anchors to display
 * @param string $targetpage Target page
 * @param string $qspagename (default = 'pagenum')
 ...
 * @param string $string String to remove new line characters from
...
/**
 * Format excerpt from provided string
 * @param string $str String to truncate
 * @param int $length Length of snippet to create
 * @return string Formatted string
 */
 ...
 * @param int $length Length of string to truncate (default = 100)
 * @param bool $exact whether to break words in truncating (default = false)
 * @param bool $considerHtml if text itself is shorter than length return it (default = true)
 * @return string Formatted string
 */
 ...
 * Upload image
 * @param string $key 
 * @param int $image_id ID of image to be uploaded
 * @return string Name of image or null if failed
 */
...
/**
 * Upload file
 * @param string $field Field name of the control
 * @param string $filename Name of file to upload
 * @param string $savetopath Path file will be saved to
 * @param bool $overwrite If to overwrite existing file
 * @param string $name Name of file
 * @return string If failed or success
 */
 ...
 * @param string $str Directory URL path
...
/**
 * Returns page start (0 or 1) based on FoxyCart version
 * @return int Page number to start pagination on
 */
 ...
 * @return string Links to external scrips and stylesheets
 */
function foxypress_GetFoxyPressIncludes()
{
	...
	if($version == "2.0")
	{
		$scripts = "<!-- BEGIN FOXYCART FILES -->"
							...
							.
							"<!-- FOXYCART -->
							<script src=\"//cdn.foxycart.com/" . get_option('foxycart_storeurl') . "/loader.js\" async defer></script>
							<!-- /FOXYCART -->

					<!-- END FOXYCART FILES -->";
        if(get_option('foxypress_image_mode') == FOXYPRESS_USE_COLORBOX)
    	{
    		$scripts .= "<script src=\"" . plugins_url() . "/foxypress/js/jquery.colorbox-min.js\" type=\"text/javascript\" charset=\"utf-8\"></script>
							<link rel=\"stylesheet\" href=\"" . plugins_url() . "/foxypress/css/colorbox.css\" type=\"text/css\" media=\"screen\" charset=\"utf-8\" />";
    	}
	}
	else if($version == "1.1")
	{
	...
	else
	{
		$scripts = "<!-- BEGIN FOXYCART FILES -->"
					...
					.
					"<script src=\"http://cdn.foxycart.com/" . $foxyStoreURL . "/foxycart.complete.2.js\" type=\"text/javascript\" charset=\"utf-8\"></script>
					<link rel=\"stylesheet\" href=\"http://static.foxycart.com/scripts/colorbox/1.3.16/style1_fc/colorbox.css\" type=\"text/css\" media=\"screen\" charset=\"utf-8\" />
					<!-- END FOXYCART FILES -->";
	}
	if($enablemuliship == "1")
 ...
 * @return string Links to external scripts
 */
	...
	if(get_option('foxycart_storeurl')!='')
	{
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function() {
				...
				jQuery('#foxypress_' + curProdID + ', #foxypress_form').submit(function(event) {
					var error = false;
 ...
 * Returns a GUID
 *
 * @return string Returns a unique guid
 */
function getGUID(){
...
/**
 * 
 * @return bool If Cart has validation
 */
...
/**
 * Register user on FoxyCart
 * @param int $UserID ID of WordPress user
 */
 ...
 * @return bool True or false if key is valid
 */
...
/**
 * Check if FoxyCart user exists
 * @param string $customer_email Email of customer
 * @return bool if customer is FoxyCart user or not
 */
 ...
 * Create FoxyCartUser
 * @param string $customer_email Email of customer
 * @param string $customer_pass Password of customer
 * @param string $customer_first_name First Name of customer
 * @param string $customer_last_name Last Name of customer
 * @return int FoxyCart Customer ID
 */
 ...
 * @param string $new_pass New desired password
 */
...
/**
 * Check if store is multisite
 * @return bool If store is multisite
 */
...
/**
 * Check if site is main blog
 * @return bool If site is main blog
 */
...
/**
 * Check if site has main blog
 * @return bool If site has main blog
 */
...
/**
 * Installs a new blog for Multisite installs 
 *
...
/**
 * Check if option exists for store
 * @param $option_name Name of option to check existance
 * @return bool If option exists
 */
...
/**
 * Handle database table alterations on install
 */
function foxypress_Installation_HandleTableAlterations()
{
	...
	$wpdb->query($sql);
	//add customer ID
	$sql = "ALTER TABLE " . $wpdb->prefix . "foxypress_transaction ADD foxy_transaction_customer_id INT(11) NULL AFTER foxy_transaction_last_name;";
	$wpdb->query($sql);
...
function foxypress_Installation_CreateTransactionTable()
{
	...
	$sql = "CREATE TABLE " . $wpdb->prefix . "foxypress_transaction (
			...
			foxy_transaction_customer_id INT(11) NULL,
...
/**
 * Create inventory options table
 */
...
/**
 * Create inventory option groups table
 */
...
/**
 * Create inventory attributes table
 */
...
/**
 * Create inventory to catgory table
 */
...
/**
 * Create download transaction table
 */
...
/**
 * Create downloadable download table
 */
...
/**
 * Create affiliate payments table
 */
 ...
 * @param string $api_key FoxyCart API key
 */
...
/**
 * Create downloadables directory
 */
 ...
 * @param string $file_to_move File name
 * @param int $post_id ID of post to insert file to
