...
class VI_WP_LUCKY_WHEEL_PLUGINS_9mail {
	...
	public function __construct() {
		...
		self::$is_active = true;
		add_filter( 'wplwl_update_settings_args', [ $this, 'wplwl_update_settings_args' ],10,1 );
		...
		add_filter( 'emtmpl_register_email_type', array( $this, 'register_email_type' ) );
    ...
    public function wplwl_update_settings_args($arg){
        if (!is_array($arg) || !isset($arg['wheel']) || !is_array($arg['wheel'])){
            return $arg;
        }
        $arg['wheel']['email_templates'] = isset( $_POST['email_templates'] ) ? array_map( 'sanitize_text_field', $_POST['email_templates'] ) : array();
        return $arg;
    }
	...
	public function wplwl_wheel_settings_slices_column_content( $index ) {
        ...
        <select class="vi-ui dropdown fluid" name="email_templates[]">
            <option value="0"><?php esc_html_e( 'None', 'wp-lucky-wheel' ) ?></option>
			<?php
	...
	public function wplwl_wheel_settings_slices_column() {
		...
		?>
        <th><?php esc_html_e( 'Email Template', 'wp-lucky-wheel' ) ?></th>
		<?php
	}
	...
	public function register_email_type( $emails ) {
		$emails['wplwl_coupon_email'] =[
			'name' => esc_html__( 'Wordpress Lucky Wheel - Coupon Email', 'wp-lucky-wheel' ),
			'hide_rules' =>['country','user_role'],
		];
	...
	public function register_email_sample_template( $samples ) {
		...
		$samples['wplwl_coupon_email'] = [
			'basic' => [
				'name' => esc_html__( 'Basic', 'wp-lucky-wheel' ),
				'data'=>$data
			],
		];
		return $samples;
