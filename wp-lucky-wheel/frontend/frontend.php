...
class VI_WP_LUCKY_WHEEL_Frontend_Frontend {
	 ...
	 * @throws Exception
	 */
	public function send_email( $user_email, $customer_name, $mobile = '', $value = '', $label = '', $email_template = '' ) {
		...
		$use_template = VI_WP_LUCKY_WHEEL_PLUGINS_9mail::send_mail( $email_template, $user_email, [
			'search'  => [
				'{wplwl_prize_label}',
				'{wplwl_customer_name}',
				'{wplwl_prize_value}',
			...
			'replace' => [ $label, $customer_name, strtoupper( $value ), $user_email, get_bloginfo( 'name' ) ],
		] );
		if ( ! $use_template ) {
            ...
            <div id="wrapper" dir="<?php echo is_rtl() ? 'rtl' : 'ltr' ?>"
                 style="<?php echo esc_attr($bg ? "background-color:{$bg};padding: 70px 0 70px 0;" : '')?>
                         margin: 0;
                    ...
                    <tr>
                        <td align="center" valign="top">
                            ...
                            <table border="0" cellpadding="0" cellspacing="0" id="template_container"
                                   style="width: 100%;box-shadow: 0 1px 4px rgba(0,0,0,0.1) !important;
                                    ...
                                    <tr>
                                        <td align="center" valign="top">
                                            ...
                                            <table border="0" cellpadding="0" cellspacing="0"
                                                   id="template_header"
                                                   style="width: 100%;background-color: <?php echo esc_attr( $base ); ?>;
                                ...
                                <tr>
                                    <td align="center" valign="top">
                                        <!-- Body -->
                                        <table border="0" cellpadding="0" cellspacing="0"
                                               id="template_body" style="width: 100%;">
                                            <tr>
                                ...
                                <?php
                                if (!empty($email_temp['footer_text'])){
                                    ?>
                                    <tr>
                                        <td align="center" valign="top">
                                            <!-- Footer -->
                                            <table border="0" cellpadding="10" cellspacing="0"
                                                   id="template_footer" style="width: 100%;">
                                                <tr>
                                                    <td valign="top">
                                                        <table border="0" cellpadding="10" cellspacing="0" width="100%">
                                                            <tr>
                                                                ...
                                                                padding: 0 48px 48px 48px;">
									                                <?php echo wp_kses_post( wpautop( wptexturize( $email_temp['footer_text'] ) ) ); ?>
                                                                </td>
                                                            </tr>
                                                        </table>
                                                    </td>
...
//            if (VI_WP_LUCKY_WHEEL_PLUGINS_9mail::$is_active || class_exists( 'EmTmplF\WP_Email_Templates_Designer' ) ){
	...
	public function frontend_enqueue() {
		...
		$this->settings::enqueue_script(
			...
			array( 'wp-lucky-wheel' ),
			array( 0 ),
		);
		$font = '';
		if ( $this->settings->get_params( 'wheel_wrap', 'font' ) ) {
			$font = $this->settings->get_params( 'wheel_wrap', 'font' );
			wp_enqueue_style( 'wp-lucky-wheel-google-font-' . strtolower( str_replace( '+', '-', $font ) ), '//fonts.googleapis.com/css?family=' . $font . ':300,400,700', '', VI_WP_LUCKY_WHEEL_VERSION );
			$font = str_replace( '+', ' ', $font );
		}

		$this->settings::enqueue_style(
			array( 'wordpress-lucky-wheel-frontend-style' ),
			array( 'wp-lucky-wheel' ),
			array()
		);
		...
		if ( ! in_array( $intent, [ 'popup_icon', 'show_wheel' ] ) ) {
			$intent = 'popup_icon';
		}
		...
		wp_localize_script( 'wordpress-lucky-wheel-frontend', '_wplwl_get_email_params', array(
			...
			'language'                        => '',
			'nonce'                           => wp_create_nonce( 'wplwl_nonce' ),
		) );
                ...
                <div class="wheel_content_right">
                    ...
                    <div class="wplwl_user_lucky">
                        ...
                        <span class="wplwl_chek_mail wplwl_spin_button button-primary"
                              id="wplwl_chek_mail"><?php echo esc_html( $spin_button ) ?></span>
						<?php
	...
	public function get_email() {
		...
		check_ajax_referer( 'wordpress_lucky_wheel_nonce_action', '_wordpress_lucky_wheel_nonce' );
		...
		if ( $the_query->have_posts() ) {
			while ( $the_query->have_posts() ) {
				...
				if ( $spin_meta['spin_num'] >= $this->settings->get_params( 'general', 'spin_num' ) ) {
					...
					if ( $wait_day ) {
						$wait_return = sprintf( esc_html( '%s days %s hours %s minutes %s seconds' ), $wait_day, $wait_hour, $wait_min, $wait_sec );
					} elseif ( $wait_hour ) {
						$wait_return = sprintf( esc_html( '%s hours %s minutes %s seconds' ), $wait_hour, $wait_min, $wait_sec );
					} elseif ( $wait_min ) {
						$wait_return = sprintf( esc_html( '%s minutes %s seconds' ), $wait_min, $wait_sec );
					}
					...
					if ( $wheel['prize_type'][ $stop ] != 'non' ) {
						...
						$this->send_email( $email, $name, '', $code, $wheel_label ,$email_templates[ $stop ] ??'');
						...
						$result_notification = str_replace( '{customer_name}', '<strong>' . ( isset( $_POST['user_name'] ) ? sanitize_text_field( $_POST['user_name'] ) : '' ) . '</strong>', $result_notification );
