...
<ul>
	...
	public function get_params( $name = '', $name_sub = '', $default = false) {
		if ( ! $name ) {
		...
		if ( $name_sub ) {
			$result = isset( $this->params[ $name ] ) ? ( $this->params[ $name ][ $name_sub ] ?? $default ) : $default;
		} else {
...
//			echo wp_kses($field['html'], self::filter_allowed_html());
	...
	public static function enqueue_style( $handles = array(), $srcs = array(), $is_suffix = array(), $des = array(), $type = 'enqueue' ) {
		...
		foreach ( $handles as $i => $handle ) {
			...
			$action( $handle, VI_WP_LUCKY_WHEEL_CSS . $srcs[ $i ] . $suffix_t . '.css', ! empty( $des[ $i ] ) ? $des[ $i ] : array(), VI_WP_LUCKY_WHEEL_VERSION );
		}
	}
	...
	public static function enqueue_script( $handles = array(), $srcs = array(), $is_suffix = array(), $des = array(), $type = 'enqueue', $in_footer = false ) {
		...
		foreach ( $handles as $i => $handle ) {
			...
			$suffix_t = ! empty( $is_suffix[ $i ] ) ? '.min' : $suffix;
			$action( $handle, VI_WP_LUCKY_WHEEL_JS . $srcs[ $i ] . $suffix_t . '.js', ! empty( $des[ $i ] ) ? $des[ $i ] : array( 'jquery' ),
				VI_WP_LUCKY_WHEEL_VERSION, $in_footer );
		}
	}
}