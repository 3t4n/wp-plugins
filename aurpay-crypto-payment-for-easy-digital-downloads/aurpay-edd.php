 ...
 * Plugin URI: https://wordpress.org/plugins/aurpay-crypto-payment-for-easy-digital-downloads
...
function aurpay_edd_process_payment($purchase_data)
{
    ...
    if ($payment) {
        ...
        $user = (!$userID) ? __('Guest', 'easy-digital-downloads') : "<a href='" . admin_url("user-edit.php?user_id=" . $userID) . "'>user" . $userID . "</a>";
        edd_insert_payment_note($payment, sprintf(__('Order Created by %s. <br/> Awaiting cryptocurrency payment ...', 'easy-digital-downloads'), $user) . '<br/>');
        ...
        edd_record_gateway_error(__('Payment Error', 'easy-digital-downloads'), sprintf(__('Payment creation failed while processing crypto purchase. Payment data: %s', 'easy-digital-downloads'), json_encode($payment_data)), $payment);
...
function aurpay_edd_cryptocoin_payment($payment)
{
    if (edd_get_payment_gateway($payment->ID) == AURPAY_GATEWAY_NAME && is_object($payment)) {
        ...
        if (!$payment || !$payment->ID) {
            ...
            echo '<h3>' . esc_html(__('ERROR', 'easy-digital-downloads')) . '</h3>' . esc_html(PHP_EOL);
            echo "<p class='edd-alert edd-alert-error'>" . esc_html(__('Unable to get payment object. You can contact the email(contact@aurpay.net) to get more help.', 'easy-digital-downloads')) . '</p>';
            return false;
        } else {
            if ($amount < 0) {
                aurpay_edd_log_error("[cryptocoin_payment] Order amount < 0, amount: " . $amount);
                echo '<h3>' . esc_html(__('ERROR', 'easy-digital-downloads')) . '</h3>' . esc_html(PHP_EOL);
                echo "<p class='edd-alert edd-alert-error'>" . esc_html(__("The order amount must be greater than or equal to 0. Please contact us(contact@aurpay.net) if you need assistance.", 'easy-digital-downloads') . esc_html(" ") . esc_html($currency)) . "</p>";
                return false;
                ...
                echo '<h3>' . esc_html(__('ERROR', 'easy-digital-downloads')) . '</h3>' . esc_html(PHP_EOL);
                echo "<p class='edd-alert edd-alert-error'>" . esc_html(__("The merchant did not set the plugin configuration. Please contact merchant or us(contact@aurpay.net) if you need assistance.", 'easy-digital-downloads')) . "</p>";
                return false;
            } else {
...
function aurpay_edd_action_links($links, $file)
{
    ...
    if ($file == $this_plugin) {
        ...
        $settings_link = '<a href="' . admin_url('edit.php?post_type=download&page=edd-settings&tab=gateways#aurpay') . '">' . __('Settings', 'easy-digital-downloads') . '</a>';
