...
add_filter( 'woocommerce_general_settings', 'mp_edit_general_fields');
function mp_edit_general_fields( $settings ) {
 $key = 0;
    ...
    foreach( $settings as $values ){
        ...
        if($values['id'] == 'woocommerce_store_city'){
            $new_settings[$key] = array(
                'title'    => __('Ciudad'),
                'desc'     => __('La ciudad donde está situado tu negocio.'),
                'id'       => 'woocommerce_store_city', // <= The field ID (important)
                'default'  => '',
                'type'     => 'select',
