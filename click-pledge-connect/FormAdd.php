...
if (isset($_POST["cnpbtnsave"])) {
    ...
    $addform = isset($_POST["addformval"]) ? sanitize_text_field(wp_unslash($_POST["addformval"])) : '';
...
$dsplycntnt = ($cnpsettingscount == 1) ? 'class="disabled"' : '';
$hidval = 1;
$act = isset($_REQUEST["cnpviewid"]) ? sanitize_text_field(wp_unslash($_REQUEST["cnpviewid"])) : '';
...
if (!empty($cnpviewid)) {
    ...
    if (!empty($cnpfrmdtresult) && is_array($cnpfrmdtresult)) {
        ...
        $cnpfrmEndDate = ($cnprtnval->cnpform_Form_EndDate === "0000-00-00 00:00:00") ? '' : $cnprtnval->cnpform_Form_EndDate;
