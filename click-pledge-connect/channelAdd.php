...
if (isset($_POST["cnpchnlbtnsave"])) {
    ...
    $addform = !empty($_POST["addchannelval"]) ? sanitize_text_field(wp_unslash($_POST["addchannelval"])) : '';
    global $wpdb, $cnp_channelgrptable_name;

    if ($addform == 1) {
        $cnprtnval = CNPCF_addNewChannel($cnp_channelgrptable_name, $_POST);
        $cnpredirectval = $cnprtnval ? 'saved' : 'failed';

        wp_redirect(admin_url("admin.php?page=cnp_pledgetvchannelsdetails&info=" . urlencode($cnpredirectval)));
        exit;
    } elseif ($addform == 2) {
     
        $cnprtnval = CNPCF_updateChannels($cnp_channelgrptable_name, $_POST);
        $cnpredirectval = $cnprtnval ? 'upd' : 'failed';
        wp_redirect(admin_url("admin.php?page=cnp_pledgetvchannelsdetails&info=" . urlencode($cnpredirectval)));
        exit;
    }
}
  ...
  if ($cnpsettingscount == 1) {
    $dsplycntnt = 'class="disabled"';
} else {
    $dsplycntnt = '';
}

$hidval = 1;
$act = !empty($_REQUEST["cnpviewid"]) ? sanitize_text_field(wp_unslash($_REQUEST["cnpviewid"])) : '';
...
if (!empty($act)) {
 ...
 $cnpviewid = isset($_GET['cnpviewid']) ? sanitize_text_field(wp_unslash($_GET['cnpviewid'])) : '';
...
if (!empty($cnpviewid)) {
    ...
    if (!empty($cnpfrmdtresult) && count($cnpfrmdtresult) > 0) {
    
        $cnprtnval = reset($cnpfrmdtresult);
        ...
        $cnpfrmEndDate         = ($cnprtnval->cnpchannelgrp_channel_EndDate === "0000-00-00 00:00:00") ? "" : $cnprtnval->cnpchannelgrp_channel_EndDate;
        ...
        $cnpchnlid = $cnpchnlgroupname = $cnpchnlAccountNumber = $cnpfrmshortcode = '';
        $cnpfrmStartDate = $cnpfrmEndDate = $cnpchnlerrmsg = $cnpfrmstatus = '';
        $btn = "Add form";
        $hidval = 1;
    }
}
				...
				<div class="form-field cnpaccountId">
					...
					<thead><tr><th><strong>Channel</strong>*</th><th><strong>Start Date</strong>*</th><th><strong>End Date</strong></th><th></th></tr>
						...
						//if($frminc != 1 || $frmscount !=1){
						 ...
						 <div style="text-align-last:center;position: relative;top: 10px;">
						 <div>
