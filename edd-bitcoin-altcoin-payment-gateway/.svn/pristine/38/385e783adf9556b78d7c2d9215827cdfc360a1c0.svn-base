...
class Scripts_Settings {
    ...
    public static function load_admin_settings_scripts( $page_id ){
        ...
        wp_enqueue_style  ( 'sweetalert', CS_EBAPG_PLUGIN_ASSET_URI . 'plugins/sweetalert/dist/sweetalert.css', false ); 
        wp_enqueue_script ( 'sweetalert', CS_EBAPG_PLUGIN_ASSET_URI . 'plugins/sweetalert/dist/sweetalert.min.js', false ); 

        if( $page_id == $altcoin_menu['add_new_coin'] ){
            wp_enqueue_style ( 'jquery-typehead', CS_EBAPG_PLUGIN_ASSET_URI . 'plugins/jquery-typeahead/jquery.typeahead.min.css', false ); 
            wp_enqueue_script ( 'jquery-typehead-js', CS_EBAPG_PLUGIN_ASSET_URI . 'plugins/jquery-typeahead/jquery.typeahead.min.js', false ); 
            wp_enqueue_style ( 'jquery-date-time-picker', CS_EBAPG_PLUGIN_ASSET_URI . 'plugins/jquery-date-time-picker/jquery.datetimepicker.min.css', false ); 
            wp_enqueue_script ( 'jquery-date-time-picker', CS_EBAPG_PLUGIN_ASSET_URI . 'plugins/jquery-date-time-picker/jquery.datetimepicker.full.min.js', false ); 
        }
        
        if( $page_id == $altcoin_menu['product_page_options_settings'] ){
            wp_enqueue_style ( 'select2', CS_EBAPG_PLUGIN_ASSET_URI . 'plugins/select2/dist/css/select2.min.css', false ); 
            wp_enqueue_script ( 'select2', CS_EBAPG_PLUGIN_ASSET_URI . 'plugins/select2/dist/js/select2.min.js', false ); 
        }
        
        wp_enqueue_media();
        wp_enqueue_script ( 'media-uploader', CS_EBAPG_PLUGIN_ASSET_URI . 'js/wp_media_uploader.js', false ); 
        wp_enqueue_style  ( 'Ebapg', CS_EBAPG_PLUGIN_ASSET_URI . 'css/style.css', false ); 
    }
    ...
    public static function load_admin_footer_script( $page_id ){
        ...
        Util::markup_tag( __( 'admin footer script start', 'edd-bitcoin-altcoin-payment-gateway' ) );
        ...
        Util::markup_tag( __( 'admin footer script end', 'edd-bitcoin-altcoin-payment-gateway' ) );
    }
            ...
            <script type="text/javascript">
                jQuery(document).ready(function( $ ){
                    $("form").submit(function(e){
                        ...
                        formData.append( "action", "_cs_ebapg_custom_call" );
                        formData.append( "method", $this.find('#cs_field_method').val() );
                        swal({ title: $this.find('#cs_field_swal_title').val(), text: 'Please wait a while...', timer: 200000, imageUrl: '<?php echo CS_EBAPG_PLUGIN_ASSET_URI .'img/loading-timer.gif'; ?>', showConfirmButton: false, html :true });
                        $.ajax({
                            url: ajaxurl,
                            type: 'POST',
                            data: formData,
