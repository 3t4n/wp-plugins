...
class EddFunctions
{
    ...
    public static function register_edd_ebapg_gateway($gateways)
    {
        ...
        if (!empty($gateway_config)) {
            $admin_label = isset($gateway_config['title']) ? Util::check_evil_script($gateway_config['title']) : '';
            $checkout_label = isset($gateway_config['description']) ? Util::check_evil_script($gateway_config['description']) : '';
    ...
    public static function register_edd_payment_icon($icons)
    {
        ...
        if (!empty($gateway_config['payment_icon_url'])) {
            $icon = Util::check_evil_script($gateway_config['payment_icon_url']);
        }
        $icons[$icon] = isset($gateway_config['title']) ? Util::check_evil_script($gateway_config['title']) : __('Cryptocurrency Payment', 'edd-bitcoin-altcoin-payment-gateway');
        return $icons;
    }
}
