...
class CsEbapgAutoOrderConfirm
{
    ...
    public function track_coin($raw_data)
    {
        ...
        $data = Util::check_evil_script($data);
        ...
        if (isset($data['trxid']) && empty($trxid = $data['trxid'])) {
            return wp_send_json(Util::notice_html(array(
                'error' => true,
                'response' => __('Please enter your coin transaction ID. Make sure you have fill up all the required fields marked as (*)', 'edd-bitcoin-altcoin-payment-gateway')
            )));
        }
        ...
        if (isset($data['secret_word']) && empty($secret_word = $data['secret_word'])) {
            return wp_send_json(Util::notice_html(array(
                'error' => true,
                'response' => __('Please enter a secret word. Make sure you have fill up all the required fields marked as (*).', 'edd-bitcoin-altcoin-payment-gateway')
            )));
        }
        ...
        if (cartFunctions::get_transaction_successful_log($premade_order_id) == 'success') {
            return wp_send_json(Util::notice_html(array(
                'success' => true,
                'response' => __('Thank you! Transaction completed successfully. Your order is processing right now!', 'edd-bitcoin-altcoin-payment-gateway')
            )));
        }
        ...
        if (false === $trxid_validator) {
            return wp_send_json(Util::notice_html(array(
                'error' => true,
                'response' => __('We are unable to match your payment information. Make sure you have entered the correct "secret word" used first time with this transaction id.', 'edd-bitcoin-altcoin-payment-gateway')
            )));
        }
        ...
        if (isset($config['api_key']) && empty($api_key = $config['api_key'])) {
            return wp_send_json(Util::notice_html(array(
                'error' => true,
                'response' => __('Api key not found! Please contact site administrator.', 'edd-bitcoin-altcoin-payment-gateway')
            )));
        }
        ...
        if (empty($cart_info)) {
            return wp_send_json(Util::notice_html(array(
                'error' => true,
                'response' => __('Something went wrong. Please refresh the page and try again.', 'edd-bitcoin-altcoin-payment-gateway')
            )));
        }

        $cart_info = array_map('trim', (array) $cart_info);
                ...
                if (isset($response->response)) {
                    ...
                    if ($confirmation >= $con_count) {
                        ...
                        $api_url = sprintf(
                            ...
                            trim($api_key),
                            'forceend',
                            trim($cart_info['coinAddress']),
                            Util::check_evil_script($trxid),
                            trim($cart_info['totalCoin']),
                            trim($cartTotal),
                            'completed'
                        );
