function parseWPconfig()
{
	...
	html=html.replace(/(WP_MEMORY_LIMIT|WP_ALLOW_REPAIR|WP_AUTO_UPDATE_CORE|SCRIPT_DEBUG|SAVEQUERIES|WP_CACHE)/gm, "<b class='atec-red'>$1</b>");
	html=html.replace(/(\/\/\s(.*)\n)/gm, "<font color=#aaa>$1</font>");
	html=html.replace(/(\/\*\*?\s(.*)\*\/\n)/gm, "<font color=#aaa>$1</font>");

	const regex = /(define\(\s?\'AUTH_KEY\'[\s\S;]*)\n\n\/\*\*#@\-\*\//gm;
	html=html.replace(regex, "<font color=#7F00C5>$1</font>");
	html=html.replace(/\n\n/gm, "\n");
	config.html(html);
}
