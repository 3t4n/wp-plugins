...
{ 
	echo '<span style="color:', ($enabled?($active?'black':'green'):'red'), '" title="', ($enabled?esc_attr__('Enabled','atec-debug'):esc_attr__('Disabled','atec-debug')), '" class="', esc_attr(atec_dash_class($enabled?'yes-alt':'warning')), '"></span>'; 
}
...
{ 
	...
	<div class="atec-header">
		...
		<div class="atec-center atec-vat atec-mt-2">',
			...
			if ($approved)
			{
				echo '<a class="atec-fs-12 atec-nodeco atec-btn-small atec-ml-10 atec-mt-0" style="border-color: ', esc_attr($color), ';" href="', esc_url($wordpress.$plugin.'/reviews/#new-post'), '" target="_blank">'; atec_dash_span('admin-comments'); echo '&nbsp;', esc_attr__('Post a review','atec-debug'), '</a>';
			}		
		echo '
		</div>
	</div>';
