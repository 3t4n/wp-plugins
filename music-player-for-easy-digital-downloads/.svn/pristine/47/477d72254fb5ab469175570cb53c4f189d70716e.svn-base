...
add_action( 'init', function(){
	add_filter( 'get_post_metadata', function( $v, $object_id, $meta_key, $single, $meta_type = '' ){
		if ( '_elementor_element_cache' == $meta_key ) {
			global $wpdb;
			if ( $wpdb->get_var( $wpdb->prepare('SELECT COUNT(*) FROM ' . $wpdb->postmeta . ' WHERE post_id=%d AND meta_key="_elementor_element_cache" AND meta_value LIKE "%eddmp%";', $object_id ) ) ) return false;
		}
		return $v;
	}, 10, 5 );
} );
...
// CONSTANTS
define( 'EDDMP_PLUGIN_VERSION', '1.2.9' );
...
if ( ! class_exists( 'EDDMusicPlayer' ) ) {
	class EDDMusicPlayer {
		...
		public function replace_playlist_shortcode( $atts ) {
			...
			if ( ! empty( $downloads ) ) {
				if ( ! empty( $title ) ) $output  .= '<div class="eddmp-widget-playlist-title">' . esc_html( $title ) . '</div>';
				...
				// Checkout URI initialization.
				$checkout_uri = edd_get_checkout_uri();
				$checkout_uri .= ( false === strpos( $checkout_uri, '?' ) ? '?' : '&' ) . 'edd_action=add_to_cart&download_id=';
				...
				foreach ( $downloads as $download ) {
					...
					$price = strip_tags( edd_price( $download->ID, false ) );
					...
					$output .= '<div class="eddmp-widget-download-price">' . $price;
					$output .= ' <a href="' . $checkout_uri . $download->ID . '"></a>';
					$output .= '</div><!-- download price -->';
					...
					foreach ( $audio_files as $index => $file ) {
						...
						$output    .= '
							<div class="eddmp-widget-download-file">
								' . $audio_tag . '<span class="eddmp-file-name">' . $file_title . '</span>' . ( 'all' != $controls && ( ! isset( $atts['duration'] ) || $atts['duration'] == 1 ) ? '<span class="eddmp-file-duration">' . esc_html( $duration ) . '</span>' : '' ) . '<div style="clear:both;"></div>
							</div><!--download file -->
						';
					}
		...
		public function include_all_players( $download ) {
			...
			if ( ! empty( $files ) ) {
				...
				if ( 1 == $counter ) {
					...
					print '<div class="eddmp-player-container download-' . esc_attr( $file['download'] ) . '" ' . ( $loop ? 'data-loop="1"' : '' ) . '>' . $audio_tag . '</div><div class="eddmp-player-title">' . esc_html( $title ) . ( 'all' != $player_controls ? '<span class="eddmp-file-duration">' . esc_html( $duration ) . '</span>' : '' ) . '</div><div style="clear:both;"></div>'; // phpcs:ignore WordPress.Security.EscapeOutput
				} elseif ( $counter > 1 ) {
					...
					foreach ( $files as $index => $file ) {
						...
						if ( 'all' != $player_controls ) {
							print '<tr class="' . esc_attr( $evenOdd ) . ' download-' . esc_attr( $file['download'] ) . '"><td class="eddmp-player-container eddmp-column-player-' . esc_attr( $player_style ) . '">' . $audio_tag . '</td><td class="eddmp-player-title eddmp-column-player-title">' . esc_html( $title ) . '</td><td class="eddmp-file-duration" style="text-align:right;font-size:16px;">' . esc_html( $duration ) . '</td></tr>'; // phpcs:ignore WordPress.Security.EscapeOutput
						} else {
		...
		private function _generate_audio_url( $download_id, $file_index, $file_data = array() ) {
			...
			$url  = EDDMP_WEBSITE_URL; // isset( $_SERVER['REQUEST_URI'] ) ? esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '';
