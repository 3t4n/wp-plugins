...
class Ct_Rbpaqp_Admin
{
	...
	public function ct_rbpaqp_admin_menu()
	{
		add_submenu_page(
			...
			'Product Restrictions', // Page title.
			esc_html__('Product Restrictions', 'product-restrictions '), // Title.
	...
	public function ct_rbpaqp_tab_callback()
	{
		...
		$nonce = isset($_GET['product-restrictions']) ? sanitize_text_field($_GET['product-restrictions']) : '';
		...
		if (is_array($tabs)) {
			...
			<div class="wrap woocommerce">
				<h2>
					<?php echo esc_html__('B2B  Setting', 'product-restrictions'); ?>
				</h2>
				...
				<?php foreach ($global_setting_section as $section_url) {
					...
					<li>
						<a href="<?php echo esc_url($url); ?>"
							class="  <?php echo esc_attr($active_section) === $section_url ? 'current' : ''; ?>">
							<?php
		...
		<form method="post" action="options.php">
			...
			if (str_contains('general_settings error_message', $active_tab)) {
				echo '<div class= "div2">';
				submit_button(esc_html__('Save Settings', 'product-restrictions'), 'primary');
				echo '</div>';
			}
			?>
		</form>
