	...
	<table class="wp-list-table widefat fixed striped table-view-list customers dataTable">
		<?php if (!wc_get_product($post_id)) { ?>
			<tr>
				...
				<td>
					<select name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_inlcuded_products[]" multiple
						style="width: 40%;" class=" ct_rbpaqp_mmq_inlcuded_products ct_rbpaqp_product_search">
			...
			<tr>
				...
				<td>
					<select name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_inlcuded_category[]" multiple
			...
			<tr>
				...
				<td>
					<select name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_inlcuded_tag[]" multiple style="width: 40%;"
		...
		<tr>
			<th>
				<?php echo esc_html__('Purchas Item Quantity Restrictions', 'product-restrictions'); ?>
			</th>
			<td>
				<input type="checkbox" name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_add_purchas_item_qty_or_not"
					class="ct_rbpaqp_mmq_add_purchas_item_qty_or_not" value="yes" <?php if (!empty(get_post_meta($post_id, 'ct_rbpaqp_mmq_add_purchas_item_qty_or_not', true))): ?> checked <?php endif ?>>
				<p>
					<i>
						<?php echo esc_html__('Durring Update cart and add to cart any product do you want to sum purchase item quantity for this checking max quantity limit.If yes enable checkbox', 'product-restrictions'); ?>
					</i>
				</p>
			</td>
		</tr>
		<tr>
			<th>
				<?php echo esc_html__('Order Statuses', 'product-restrictions'); ?>
			</th>
			<td>
				<select name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_order_status_purchas_item_qty[]" multiple
					class="ct-rbpaqp-live-search ct_rbpaqp_mmq_order_status_purchas_item_qty" style="width: 100%;">
				...
				<p>
					<i>
						<?php echo esc_html__('Select which order statuses for which you want to sum product quantity during cart update and add to cart product.', 'product-restrictions'); ?>
					</i>
				</p>
			</td>
		</tr>
			...
			<td>
				<table>
					<tr>
						...
						<td>
							<input type="date" name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_start_date"
								value="<?php echo esc_attr(get_post_meta($post_id, 'ct_rbpaqp_mmq_start_date', true)); ?>">
						</td>
						<td>
							<?php echo esc_html__('End Date', 'product-restrictions'); ?>
						</td>
						<td>
							<input type="date" name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_end_date"
								value="<?php echo esc_attr(get_post_meta($post_id, 'ct_rbpaqp_mmq_end_date', true)); ?>">
						</td>
					</tr>
				</table>
			</td>
		...
		<tr>
			...
			<td>
				<select name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_inlcuded_days[]" multiple style="width: 40%;"
					class="ct-rbpaqp-live-search">
...
function ct_rbpaqp_mmq_restriction($post_id)
{
	...
	<table class="wp-list-table widefat fixed striped table-view-list customers dataTable">
		...
		<thead>
			<th><?php echo esc_html__('Enable Setting For Role', 'product-restrictions'); ?></th>
			<th><?php echo esc_html__('Min Qty', 'product-restrictions'); ?></th>
			<th><?php echo esc_html__('Max Qty', 'product-restrictions'); ?></th>
			<th><?php echo esc_html__('Step', 'product-restrictions'); ?></th>
			<th><?php echo esc_html__('Select Countries', 'product-restrictions'); ?></th>
		</thead>
		<tbody>
				...
				<tr>
					...
					<th>
						<input type="checkbox" value="yes"
							name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_rules[<?php echo esc_attr($role_key); ?>][enable_setting]"
							<?php if (!empty($setting)): ?> checked<?php endif ?>>
						<?php echo esc_attr($role); ?>
					</th>
					<td>
						<input type="number" min="0.1" step="any"
							name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_rules[<?php echo esc_attr($role_key); ?>][min_qty]"
							value="<?php echo esc_attr($selected_min_qty); ?>">
					</td>
					<td>
						<input type="number" min="0.1" step="any"
							name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_rules[<?php echo esc_attr($role_key); ?>][max_qty]"
							value="<?php echo esc_attr($selected_max_qty); ?>">
					</td>
					<td>
						<input type="number" min="0.1" step="any"
							name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_rules[<?php echo esc_attr($role_key); ?>][step]"
							value="<?php echo esc_attr($selected_step); ?>">
					</td>


					<td>
						<select
							name="<?php echo esc_attr($post_id); ?>ct_rbpaqp_mmq_rules[<?php echo esc_attr($role_key); ?>][countries][]"
							multiple style="width: 100%;" class="ct-rbpaqp-live-search">

							<?php foreach ($country_obj->get_countries() as $country_key => $country): ?>

								<option value="<?php echo esc_attr($country_key); ?>" <?php if (
									 	in_array($country_key, (array) 
									 		$selected_country)
									 ): ?>selected<?php endif ?>>
