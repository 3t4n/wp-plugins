...
class Dwul_User_Register_Ajax_Callback {
	...
	public function dwul_action_callback() {
		...
		if ( ! wp_verify_nonce( $disable_nonce, 'disable_user_nonce' ) ) {
			$response_message = __( 'Invalid Nonce', 'wp-users-disable' );
		...
		$registered_users   = $wpdb->get_col( "SELECT `user_email` FROM {$wpdb->prefix}users" );
		$existing_users     = $wpdb->get_col( "SELECT `useremail` FROM {$wpdb->prefix}dwul_disable_user_email" );
		$user               = get_user_by( 'email', $disableemail );

		if ( 'administrator' == $user->roles[0] ) {
			$response_message = __( 'User with Administrator role can\'t be disabled.', 'wp-users-disable' );
			echo esc_html( $response_message );
			die();
		}

		if ( ! in_array( $disableemail, $registered_users ) ) {
			$response_message = __( 'User does not exist', 'wp-users-disable' );
			echo esc_html( $response_message );
			die();
		}

		if ( in_array( $disableemail, $existing_users ) ) {
			$response_message = __( 'User is already disabled.', 'wp-users-disable' );
			echo esc_html( $response_message );
			die();
		}
		...
		$response_message = ( $insertdata ) ?
			__( 'success', 'wp-users-disable' ) :
			__( 'Something went wrong.', 'wp-users-disable' );
			echo esc_html( $response_message );
			die();
	}
	...
	public function dwul_ajax_script() {
		...
		wp_localize_script(
			...
			array(
				...
				'error'         => __( 'Something went wrong with database.', 'wp-users-disable' ),
			)
		);
	...
	public function dwul_disable_user_login_message( $message ) {
		...
		if ( isset( $_GET['disabled'] ) && 1 == $_GET['disabled'] ) {
			$message = '<div id="login_error">' .
				apply_filters( 'ja_disable_users_notice', __( 'User Account Disabled.', 'wp-users-disable' ) ) .
			'</div>';
		}
	...
	public function dwul_enable_user_email() {
		...
		$delquery = $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}dwul_disable_user_email WHERE id = %d", $activateuserid ) );
