...
if (!class_exists("APPSDBQueryBuilder")) {
    class APPSDBQueryBuilder
    {
        ...
        public function group_by($groupby, $escape = true)
        {
            $groupby = ($escape ? $this->escape_key($groupby, " ,") : $groupby);
            $comma = ! empty($this->group_by) ? "," : "";

            $this->group_by .= "{$comma} {$groupby}";
        }
        ...
        public function getSelectQuery($table = '', $limit = NULL, $offset = NULL)
        {
            ...
            $query = "SELECT {$this->select} FROM {$this->from} {$this->join} {$this->where} {$this->group_by} {$this->order_by} {$this->limit}";
            $this->Reset();
