...
if (!class_exists("AppsBDModel")) {
    class AppsBDModel
    {
        ...
        function SetDBPropertyForInsertOrUpdate($isForUpdate = false)
        {
            ...
            $this->ResetSetForInsetUpdate(true);
            ...
            if (! empty($obj) && (is_object($obj) || is_array($obj))) {
                foreach ($obj as $key => $value) {
                    if (in_array($key, $this->htmlInputField)) {
                        $value = ((null !== $value) ? stripcslashes($value) : $value);
                    }
                    $this->$key = $value;
                }
            }
        ...
        function Save()
        {
            ...
            if ($this->db->insert($this->db->prefix . $this->tableName, $insertarray) !== false) {
        ...
        {
            $n = new static();
            if (is_string($property) && property_exists($n, $property)) {
                $n->$property($value);
                if (is_array($extraparam)) {
                    foreach ($extraparam as $key => $value) {
                        if (property_exists($n, $key)) {
        ...
        function ResetSetForInsetUpdate($isWitoutLog = false)
        {
            if ($isWitoutLog) {
