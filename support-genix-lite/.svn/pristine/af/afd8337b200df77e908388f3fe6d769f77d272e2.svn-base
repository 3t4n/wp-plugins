...
class Apbd_wps_role extends AppsBDBaseModuleLite
{
    ...
    public function add()
    {
        ...
        if (APPSBD_IsPostBack) {
            ...
            $capabilities = sanitize_text_field(APBD_PostValue("capabilities"));
    ...
    public function data_agent_access()
    {
        ...
        if (Apbd_wps_settings::isAgentLoggedIn()) {
            ...
            if (current_user_can('manage_options') || in_array('administrator', $role_slugs)) {
                $apiResponse->SetResponse(true, "", ['all']);
            } else {
                ...
                foreach ($role_slugs as $role_slug) {
                    if (isset($allRoles[$role_slug])) {
                        ...
                        if (isset($role->capabilities) && is_array($role->capabilities) && !empty($role->capabilities)) {
                            $apiResponse->SetResponse(true, "", array_keys($role->capabilities));
                        }
