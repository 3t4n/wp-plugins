...
class Mapbd_wps_role extends AppsBDModel
{
    ...
    public function SetFromPostData($isNew = false, $data = null)
    {
        ...
        $name = sanitize_text_field(APBD_PostValue('name', ''));
        $role_description = sanitize_text_field(APBD_PostValue('role_description', ''));
        $is_agent = sanitize_text_field(APBD_PostValue('is_agent', ''));
        ...
        foreach ($existingRoles as $key => $existingRole) {
            ...
            if (!$obj->Select()) {
                if ($key == "administrator" || !self::IsBuiltInRole($key)) {
                    ...
                    $roleAdded = Mapbd_wps_role::AddRole($key, $existingRole, $isEditable, $key == "administrator");
