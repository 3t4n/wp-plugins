...
class Mapbd_wps_edd extends AppsBDModel
{
    ...
    public function SetFromPostData($isNew = false, $data = null)
    {
        ...
        $api_endpoint = esc_url_raw(APBD_PostValue('api_endpoint', ''));
        $api_public_key = sanitize_text_field(APBD_PostValue('api_public_key', ''));
        $api_token = sanitize_text_field(APBD_PostValue('api_token', ''));
        $details_btn = sanitize_text_field(APBD_PostValue('details_btn', ''));
        $admin_url = esc_url_raw(APBD_PostValue('admin_url', ''));
