...
require_once dirname(__DIR__, 2) . '/traits/APBDWPSTicketAPITrait.php';

class APBDWPSTicketAPI extends Apbd_WPS_API_Base
{
    use APBDWPSTicketAPITrait;
    ...
    function routes()
    {
        ...
        $this->RegisterRestRoute('GET', 'download/(?P<ticketId>\d+)', [$this, "download_ticket"]);
        $this->RegisterRestRoute('post', 'suggested-docs', [$this, "suggested_docs"]);
    }
    ...
    function ticket_list()
    {
        ...
        if ($is_agent_logged_in && $check_assigned_on) {
            ...
            if (! $manage_other_agents_ticket && ! $manage_unassigned_ticket) {
                ...
                $mainobj->assigned_on("IN ($id,'','0')", true);
    ...
    {
        $allowedProps = ['status', 'cat_id', 'assigned_on', 'related_url', 'email_notification'];
    ...
    function create_ticket()
    {
        if (! empty($this->payload['title'] && $this->payload['ticket_body'])) {
            if (is_user_logged_in() && Apbd_wps_settings::isClientLoggedIn()) {
