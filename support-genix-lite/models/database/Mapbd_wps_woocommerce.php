...
class Mapbd_wps_woocommerce extends AppsBDModel
{
    ...
    public function SetFromPostData($isNew = false, $data = null)
    {
        ...
        $id = absint(APBD_GetValue('id'));
        $same_site = sanitize_text_field(APBD_PostValue('same_site', ''));
        $store_title = sanitize_text_field(APBD_PostValue('store_title', ''));
        $store_url = esc_url_raw(APBD_PostValue('store_url', ''));
        $api_consumer_key = sanitize_text_field(APBD_PostValue('api_consumer_key', ''));
        $api_consumer_secret = sanitize_text_field(APBD_PostValue('api_consumer_secret', ''));
        $disallow_opts = sanitize_text_field(APBD_PostValue('disallow_opts', ''));
        $verify_opts = sanitize_text_field(APBD_PostValue('verify_opts', ''));
        $wh_ticket = sanitize_text_field(APBD_PostValue('wh_ticket', ''));
        $wh_ticket_cat = absint(APBD_PostValue('wh_ticket_cat', ''));
        $wh_ticket_title = sanitize_text_field(APBD_PostValue('wh_ticket_title', ''));
        $wh_ticket_desc = sanitize_text_field(APBD_PostValue('wh_ticket_desc', ''));
        $status = sanitize_text_field(APBD_PostValue('status', ''));

        $same_site = 'Y' === $same_site ? 'Y' : 'N';
        $wh_ticket = 'Y' === $wh_ticket ? 'Y' : 'N';
        $wh_ticket_cat = strval($wh_ticket_cat);
        $status = 'A' === $status ? 'A' : 'I';

        if ($isNew && str_contains($api_consumer_key, '*')) {
            $api_consumer_key = '';
        }

        if ($isNew && str_contains($api_consumer_secret, '*')) {
            $api_consumer_secret = '';
        }

        if (
            (1 > strlen($store_title)) ||
            (
                ('Y' === $wh_ticket) &&
                (
                    (1 > strlen($wh_ticket_title)) ||
                    (1 > strlen($wh_ticket_desc))
                )
            )
        ) {
            return;
        ...
        if ('Y' === $wh_ticket) {
            $newData['wh_ticket_cat'] = $wh_ticket_cat;
            $newData['wh_ticket_title'] = $wh_ticket_title;
            $newData['wh_ticket_desc'] = $wh_ticket_desc;
        } elseif ($isNew) {
        ...
        if ('Y' === $same_site) {
            $exobj = Mapbd_wps_woocommerce::FindBy('same_site', $same_site);
            ...
            if (
                (1 > strlen($store_url)) ||
                (1 > strlen($api_consumer_key)) ||
                (1 > strlen($api_consumer_secret))
            ) {
                return;
            }

            $newData['store_url'] = $store_url;

            if (!str_contains($api_consumer_key, '*')) {
                $newData['api_consumer_key'] = $api_consumer_key;
            }

            if (!str_contains($api_consumer_secret, '*')) {
                $newData['api_consumer_secret'] = $api_consumer_secret;
            }
        }
