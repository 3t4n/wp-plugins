...
class aikct_suggest_content {
   ...
   <script>
    jQuery(document).ready(function($) {
        // Tạo biến lưu nonce
        const nonce = '<?php echo wp_create_nonce( "update_integration_option_nonce" ); ?>';
        ...
        $('.integration-toggle').on('change', function() {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    nonce: nonce, // Thêm nonce vào dữ liệu gửi
                    key: key,
                    value: value
                },
    ...
    public function update_integration_option() {
        ...
        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'update_integration_option_nonce' ) ) {
            wp_send_json_error( 'Invalid nonce.' );
            return;
        }
