...
if ( $shipping_method_action == 'new' || $shipping_method_action == 'edit' ) {
    ...
    $shipping_method_array['method_title']      = sanitize_key( $_POST['woocommerce_' . $this->id . '_method_title'] );
    $shipping_method_array['method_tax_status'] = sanitize_key( $_POST['woocommerce_' . $this->id . '_method_tax_status'] );
    $shipping_method_array['method_visibility'] = sanitize_key( $_POST['woocommerce_' . $this->id . '_method_visibility'] );
    ...
    if ( $shipping_method_action == 'new' ) {
        ...
        if ( 1 == 1 && headers_sent() ) {
            ?>
            <script>
                parent.location.replace('<?php echo esc_js( ! empty( $redirect ) ? $redirect : "" ); ?>');
            </script>
            <?php
        } else {
