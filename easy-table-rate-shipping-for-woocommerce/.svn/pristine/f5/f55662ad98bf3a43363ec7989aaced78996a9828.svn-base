...
<table class="form-table">
    ...
    if ( $shipping_method_action == 'new' || $shipping_method_action == 'edit' ) {
        ...
        ?>
        <input type="hidden" name="shipping_method_action" value="<?php echo ! empty( $shipping_method_action ) ? esc_attr( $shipping_method_action ) : ''; ?>" />
        <input type="hidden" name="shipping_method_id" value="<?php echo ! empty( $method_id ) ? esc_attr( $method_id ) : ''; ?>" />
        <input type="hidden" name="method_id_for_shipping" value="<?php echo ! empty( $method_id_for_shipping ) ? esc_attr( $method_id_for_shipping ) : ''; ?>" />
        <?php
        ...
        foreach ( $selected_shipping_methods_id as $removed_method_id ) {
            if ( isset( $get_shipping_methods_options_for_delete[ $removed_method_id ] ) ) {
                ...
                update_option( $wc_shipping_methods_option, etrs_sanitize_array( $get_shipping_methods_options_for_delete ) );
                // Update all shipping methods order after delete
                update_option( $wc_shipping_method_order_option, etrs_sanitize_array( $get_shipping_methods_order_for_delete ) );
            }
        }
