...
use Custom_WP_Framework\Admin\Models\Custom_Post_Types\CWF_CPT_DataModel;

/**
 * Viewmodel for editing custom post types.
 * 
 * @since   1.0.0
 */
class CWF_Edit_CPT_ViewModel {
    ...
    /**
     * Boolean variable indicating whether post migration is required.
     * 
     * @since   1.0.0
     * @var     bool    $migrate_posts
     */
    public $migrate_posts;

    /**
     * Current post type key of cpt in database.
     * 
     * @since   1.0.0
     * @var     string  $current_post_type_key
     */
    public $current_post_type_key;
    ...
    public function __construct( $id = null ) {
        ...
        /**
         * Assign custom post type id.
         */
        $this->cpt->post_type_id = $id;
        ...
        /**
         * Initialise migrate posts flag and set to null.
         */
        $this->migrate_posts = false;

        /**
         * Initialise current post type key and set to null.
         */
        $this->current_post_type_key = '';
    ...
    public function validate(){
         ...
         */
        if( ! isset( $_POST['cwf_edit_cpt_nonce_field'] )
            || ! wp_verify_nonce( $_POST['cwf_edit_cpt_nonce_field'], 'cwf_edit_cpt' ) ) {
        ...
        if( ! isset( $_POST['id'] ) ) {
             ...
             */
            $validation_error_msg = __( 'Post Type Id is a required field.', CUSTOM_WP_FRAMEWORK_TEXT_DOMAIN );
            
            /**
             * Add error message to validation errors.
             */
            $this->validation_errors['post-type-id'] = $validation_error_msg;
        ...
        if( ! isset( $_POST['post-type-key']) ) {
            ...
            /**
             * Create new instance of cpt model.
             */
            $model_cpt = new CWF_CPT_DataModel();

            /**
             * Get recorded post type key of cpt in database.
             * 
             * @since   1.0.0
             * @var     string  $cpt_post_type_key
             */
            $this->current_post_type_key = $model_cpt->get_cpt_post_type_key( sanitize_key( $_POST['id'] ) );

            /** 
             * Check to see whether user has updated post type key. 
             */
            if( $this->current_post_type_key !== $this->cpt->post_type_key ) {
                
                /**
                 * If post type key changed, set post migration flag to true.
                 */
                $this->migrate_posts = true;
            
                /**
                 * Verify new post type key is unique. 
                 */
        ...
        /**
         * Verify post type label (singular) is valid.
         */
        ...
        /**
         * Assign sanitized post type id.
         */
        $this->cpt->post_type_id = filter_var( $_POST['id'], FILTER_SANITIZE_NUMBER_INT );
        ...
        $this->cpt->post_type_query_var_slug = ! empty ( $_POST['post-type-query-var-slug'] ) && $this->cpt->post_type_query_var ? strtolower( sanitize_text_field( $_POST['post-type-query-var-slug'] ) ) : $this->cpt->post_type_key; 
         ...
         */
        $this->cpt->post_type_taxonomies = isset( $_POST['post-type-taxonomies'] ) ? $_POST['post-type-taxonomies'] : [];
