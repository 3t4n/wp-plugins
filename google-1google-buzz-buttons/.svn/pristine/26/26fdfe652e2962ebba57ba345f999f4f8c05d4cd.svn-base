...
add_action('init', 'zare_1_init');

add_filter('the_content', 'zare_1_content');

add_filter('the_excerpt', 'zare_1_excerpt');

add_filter('plugin_action_links', 'zare_1_add_settings_link', 10, 2 );

add_action('admin_menu', 'zare_1_menu');

add_shortcode( 'zare_1', 'zare_1_shortcode' );

// PUBLIC FUNCTIONS

function zare_1_init() {
	...
	$option = zare_1_get_options_stored();


	if ($option['active_buttons']['buzz']==true) {
		wp_enqueue_script('zare_1_buzz', 'http://www.google.com/buzz/api/button.js');
	}
	if ($option['active_buttons']['google1']==true) {
		wp_enqueue_script('zare_1_google1', 'http://apis.google.com/js/plusone.js');
	}

}    


function zare_1_menu() {
	add_options_page('Google +1,Google buzz button Options', 'Google +1,Google buzz button', 'manage_options', 'zare_1_options', 'zare_1_options');
}


function zare_1_add_settings_link($links, $file) {
	static $this_plugin;
	if (!$this_plugin) $this_plugin = plugin_basename(__FILE__);
 
	if ($file == $this_plugin){
		$settings_link = '<a href="admin.php?page=zare_1_options">'.__("Settings").'</a>';
...
function zare_1_content ($content) {
	return zare_1 ($content, 'the_content');
}


function zare_1_excerpt ($content) {
	return zare_1 ($content, 'the_excerpt');
}


function zare_1 ($content, $filter, $link='', $title='') {
	...
	if ($filter=='the_excerpt' and $last_execution=='the_content') {
		...
		remove_filter('the_content', 'zare_1_content');
	...
	$custom_field_disable = get_post_custom_values('zare_1_disable');
	...
	$out = '<div style="height:33px; padding-top:2px; padding-bottom:2px; clear:both;" class="zare_1">';
    if ($option['active_buttons']['facebook']==true) {
		$first_shown = true;

		// REMOVE HTTP:// FROM STRING
		$facebook_link = (substr($link,0,7)=='http://') ? substr($link,7) : $link;
		$out .= '<div style="float:left; width:100px;" class="zare_1_facebook">
	...
	if ($option['active_buttons']['facebook_like']==true) {
		...
		$out .= '<div style="float:left; width:'.$option['facebook_like_width'].'px; '.$padding.'" class="zare_1_facebook_like">
		...
		if ($option['facebook_like_send']) {
			...
			$out .= '<div style="float:left; width:50px; padding-left:10px;" class="zare_1_facebook_like_send">
	...
	if ($option['active_buttons']['buzz']==true) {
		...
		$out .= '<div style="float:left; '.$padding.'" class="zare_1_buzz">
	...
	if ($option['active_buttons']['google1']==true) {
		...
		$out .= '<div style="float:left; width:'.$option['google1_width'].'px; '.$padding.'" class="zare_1_google1"> 
...
function zare_1_options () {
	...
	if( isset($_POST['zare_1_position'])) {
		$option = array();

		foreach (array_keys($active_buttons) as $item) {
			$option['active_buttons'][$item] = (isset($_POST['zare_1_active_'.$item]) and $_POST['zare_1_active_'.$item]=='on') ? true : false;
		}
		foreach (array_keys($show_in) as $item) {
			$option['show_in'][$item] = (isset($_POST['zare_1_show_'.$item]) and $_POST['zare_1_show_'.$item]=='on') ? true : false;
		}
		$option['position'] = esc_html($_POST['zare_1_position']);

		$option['google1_count'] = (isset($_POST['zare_1_google1_count']) and $_POST['zare_1_google1_count']=='on') ? true : false;
		$option['google1_width'] = esc_html($_POST['zare_1_google1_width']);
	...
	$out .= '
	<style>
	#zare_1_form h3 { cursor: default; }
	#zare_1_form td { vertical-align:top; padding-bottom:15px; }
	</style>
	...
	<div style="float:left; width:74%; padding-right:1%;">
		...
		<form id="zare_1_form" name="form1" method="post" action="">
		...
		<div class="inside">
			...
			foreach ($active_buttons as $name => $text) {
				...
				$out .= '<div style="width:250px; float:left;">
						<input type="checkbox" name="zare_1_active_'.$name.'" '.$checked.' /> '
			...
			foreach ($show_in as $name => $text) {
				...
				$out .= '<div style="width:250px; float:left;">
						<input type="checkbox" name="zare_1_show_'.$name.'" '.$checked.' /> '
						. __($text, 'menu-test' ).' &nbsp;&nbsp;</div>';
			...
			<td><select name="zare_1_position">
		...
		<div class="inside">
			...
			<td>
				<input type="text" name="zare_1_google1_width" value="'.stripslashes($option['google1_width']).'" size="10"> px<br />
				<span class="description">'.__("default: 90", 'menu-test' ).'</span>
			</td></tr>
			<tr><td>'.__("Show counter", 'menu-test' ).':</td>
			<td>
				<input type="checkbox" name="zare_1_google1_count" '.$google1_count.' />
			</td></tr>
			</table>
		</div>
		</div>
...
function zare_1_shortcode ($atts) {
	return zare_1 ('', 'shortcode');
}


//FUNCTION AVAILABLE FOR EXTERNAL INCLUDING INSIDE THEMES AND OTHER PLUGINS
function zare_1_publish ($link='', $title='') {
	return zare_1 ('', 'shortcode', $link, $title);
}
...
function zare_1_get_options_stored () {
	//GET ARRAY OF STORED VALUES
	$option = get_option('zare_1');
	 
	if ($option===false) {
		//OPTION NOT IN DATABASE, SO WE INSERT DEFAULT VALUES
		$option = zare_1_get_options_default();
		add_option('zare_1', $option);
	} else if ($option=='above' or $option=='below') {
		// Versions below 1.2.0 compatibility
		$option = zare_1_get_options_default($option);
...
function zare_1_get_options_default ($position='above') {
	$option = array();
