...
jQuery(function() {
	...
	jQuery.validator.addMethod("breakWithinHours", function(value, element, params) {

		var breakTime = jQuery(params[0]).val();
		var dayTime = value;
		var operator = params[1];

		// Get unix with today's date at break end time
		var breakDate = new Date();
		var breakParts = breakTime.split(':');
		breakDate.setHours(breakParts[0], breakParts[1], 0, 0);
		var breakUnix = Math.floor(breakDate.getTime() / 1000);

		// Get unix with today's date at day end time
		var dayDate = new Date();
		var dayParts = dayTime.split(':');
		dayDate.setHours(dayParts[0], dayParts[1], 0, 0);
		var dayUnix = Math.floor(dayDate.getTime() / 1000);

		// Set comparison of business hours to break times
		function compareTimes(a, b, operator){
			if (operator === ">") return a > b;
			if (operator === "<") return a < b;
		}

		// Check if start date is less than end date
		return this.optional(element) || compareTimes(breakUnix, dayUnix, operator);
	}, __('Break must be during business hours.', 'timeslot'));
	...
	jQuery('#ts-business-hours-form').validate({
		...
		rules: (function(){
			...
			jQuery.each(weekdays, function(key, day){
				...
				hourRules[`timeslot-business-hours[${day}][break-start]`] = { breakRequired: [2,`.ts-${day}-break`], breakWithinHours: [`#timeslot-business-hours-${day}-start-hour`, '<'] };
				hourRules[`timeslot-business-hours[${day}][break-end]`] = { breakRequired: [2,`.ts-${day}-break`], breakWithinHours: [`#timeslot-business-hours-${day}-end-hour`, '>'] };
			})
			return hourRules;
		})(),
		messages:(function(){
