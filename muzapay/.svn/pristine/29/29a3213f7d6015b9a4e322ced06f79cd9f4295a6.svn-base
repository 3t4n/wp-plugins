...
namespace MuzaPayDeps\Wpify\Log;
...
class Tools
{
    ...
    {
        add_submenu_page('tools.php', __('WPify Logs', 'muzapay'), __('WPify Logs', 'muzapay'), 'read', 'wpify-logs', [$this, 'logs_screen']);
    }
    public function logs_screen()
    {
        ...
        if (!empty($_GET['log-file'])) {
            ...
            if (!\in_array($file, $files)) {
                ?>
                <p><?php 
                _e('File not found, cheating, huh?', 'muzapay');
                ?></p>
				<?php 
            } else {
