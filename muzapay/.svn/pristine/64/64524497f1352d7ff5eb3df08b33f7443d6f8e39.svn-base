...
use MuzaPayDeps\Wpify\Model\Attributes\SourceObject;
use MuzaPayDeps\Wpify\Model\Exceptions\CouldNotSaveModelException;
use MuzaPayDeps\Wpify\Model\Exceptions\IncorrectRepositoryException;
...
class PostRepository extends Repository
{
    ...
    public function get(mixed $source): ?ModelInterface
    {
        ...
        if (!$wp_post && is_string($source)) {
            $wp_posts = get_posts(array('name' => $source, 'post_type' => $this->post_types(), 'posts_per_page' => 1));
    ...
    public function save(ModelInterface $model): ModelInterface
    {
        ...
        if ($data['ID'] > 0) {
            $result = wp_update_post($data, \true);
            $action = 'update';
        } else {
            $result = wp_insert_post($data, \true);
            $action = 'insert';
        }
    ...
    {
        $defaults = array('post_type' => $this->post_types(), 'post_status' => 'any', 'posts_per_page' => -1);
    ...
    {
        $defaults = array('paged' => get_query_var('paged') ?: 1, 'post_type' => $this->post_types(), 'post_status' => 'any', 'posts_per_page' => -1);
        $args = wp_parse_args($args, $defaults);
        $this->query = new WP_Query($args);
        $pagination = $this->get_pagination();
        ...
        wp_reset_postdata();
        return ['pagination' => $pagination, 'paginate_link' => $paginate_links, 'items' => $collection];
    }
    /**
     * Finds all posts.
     *
    ...
    {
        $defaults = array('posts_per_page' => -1, 'post_status' => 'publish');
        $args = wp_parse_args($args, $defaults);
        return $this->find($args);
    }
    /**
    ...
    public function find_all_by_term(ModelInterface $model): array
    {
        ...
        if (method_exists($target_repository, 'taxonomy')) {
            return $this->find(array('tax_query' => array(array('taxonomy' => $target_repository->taxonomy(), 'field' => 'term_id', 'terms' => array($model->id)))));
        }
    ...
    {
        return $this->find(array('post_parent' => $model->id));
    }
    /**
    ...
    {
        return $this->find(array_merge($args, array('post__in' => $ids)));
    }
    /**
    ...
    public function assign_post_to_term(ModelInterface $model, array $terms, bool $append = \false): void
    {
        ...
        foreach ($to_assign as $taxonomy => $assigns) {
            wp_set_post_terms($model->id, array_values(array_map(fn($term) => $term->id, $assigns)), $taxonomy, $append);
        }
    }
}
