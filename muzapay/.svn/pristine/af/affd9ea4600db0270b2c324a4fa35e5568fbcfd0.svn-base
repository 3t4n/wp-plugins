...
use ArrayAccess;
use MuzaPayDeps\Wpify\Model\Attributes\AccessorObject;
use MuzaPayDeps\Wpify\Model\Attributes\Meta;
use MuzaPayDeps\Wpify\Model\Exceptions\PropertyNotDefinedException;
use MuzaPayDeps\Wpify\Model\Exceptions\RepositoryNotFoundException;
use MuzaPayDeps\Wpify\Model\Interfaces\AccessorAttributeInterface;
use MuzaPayDeps\Wpify\Model\Interfaces\ModelInterface;
use MuzaPayDeps\Wpify\Model\Attributes\ReadOnlyProperty;
use MuzaPayDeps\Wpify\Model\Exceptions\ReadOnlyPropertyException;
use MuzaPayDeps\Wpify\Model\Interfaces\RepositoryInterface;
...
abstract class Model implements ModelInterface, IteratorAggregate, ArrayAccess
{
    ...
    public function __construct(Manager $repository_manager, array $data = array())
    {
        ...
        foreach ($reflection->getProperties() as $property) {
            ...
            $this->_props[$property->getName()] = array('name' => $property->getName(), 'type' => $type ? $type->getName() : 'string', 'readonly' => \false, 'default' => $this->{$property->getName()} ?? null, 'allows_null' => $type ? $type->allowsNull() : \false);
            ...
            foreach ($property->getAttributes(ReadOnlyProperty::class, ReflectionAttribute::IS_INSTANCEOF) as $attribute) {
                $this->_props[$property->getName()]['readonly'] = \true;
            }
