...
namespace MuzaPayDeps\GuzzleHttp\Handler;

use MuzaPayDeps\GuzzleHttp\Exception\ConnectException;
use MuzaPayDeps\GuzzleHttp\Exception\RequestException;
use MuzaPayDeps\GuzzleHttp\Promise as P;
use MuzaPayDeps\GuzzleHttp\Promise\FulfilledPromise;
use MuzaPayDeps\GuzzleHttp\Promise\PromiseInterface;
use MuzaPayDeps\GuzzleHttp\Psr7\LazyOpenStream;
use MuzaPayDeps\GuzzleHttp\TransferStats;
use MuzaPayDeps\GuzzleHttp\Utils;
use MuzaPayDeps\Psr\Http\Message\RequestInterface;
use MuzaPayDeps\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \MuzaPayDeps\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \MuzaPayDeps\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \MuzaPayDeps\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \MuzaPayDeps\GuzzleHttp\Psr7\Utils::streamFor($sink);
