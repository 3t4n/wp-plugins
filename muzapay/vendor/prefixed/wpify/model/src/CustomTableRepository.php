...
use MuzaPayDeps\Wpify\Model\Attributes\Column;
use MuzaPayDeps\Wpify\Model\Exceptions\KeyNotFoundException;
use MuzaPayDeps\Wpify\Model\Exceptions\PrimaryKeyException;
use MuzaPayDeps\Wpify\Model\Exceptions\RepositoryNotInitialized;
use MuzaPayDeps\Wpify\Model\Exceptions\SqlException;
...
abstract class CustomTableRepository extends Repository
{
     ...
     */
    public function __construct(private bool $auto_migrate = \true, private bool $use_prefix = \true)
    {
    }
    /**
        ...
        if (empty($this->sql)) {
            ...
            if (count($foreign_keys) > 0) {
                ...
                foreach ($foreign_keys as $column_name => $settings) {
                    $columns[] = sprintf("CONSTRAINT `%s_ibfk_%d` FOREIGN KEY (`%s`) REFERENCES `%s` (`%s`) %s", $this->prefixed_table_name(), $foreign_keys_counter, $column_name, $this->db()->prefix . $settings[Column::FOREIGN_TABLE], $settings[Column::FOREIGN_COLUMN], $settings[Column::FOREIGN_SETTINGS] ?? '');
                    $foreign_keys_counter++;
                }
            }
            $columns[] = sprintf("PRIMARY KEY  (%s)", $this->primary_key());
            $sql = sprintf("CREATE TABLE %s (\n\t%s\n) %s;", $this->prefixed_table_name(), implode(",\n\t", $columns), $this->db()->get_charset_collate());
            $this->sql = $sql;
        }
        return $this->sql;
    }
        ...
        if (empty($this->columns)) {
            ...
            foreach ($this->reflection()->getProperties() as $property) {
                foreach ($property->getAttributes(Column::class, ReflectionAttribute::IS_INSTANCEOF) as $attribute) {
                    ...
                    $columns[$property->getName()] = array('property' => $property->getName(), 'name' => empty($column->name) ? $property->getName() : $column->name, 'type' => $property->getType()->getName(), 'attribute' => $column);
                }
            }
    ...
    public function query_single(mixed $source): ?object
    {
        ...
        $items = $this->find(array('where' => sprintf('`%s` = \'%s\'', $this->primary_key(), esc_sql($source)), 'limit' => 1));
    ...
    private function convert_value_for_sql($value)
    {
        if (is_bool($value)) {
            ...
            $value = '(' . join(',', array_map(function ($part) {
    ...
    {
        $primary_key = $this->primary_key();
        return $this->find(array('where' => array("{$primary_key} IN (" . join(',', $ids) . ')')));
    }
    /**
