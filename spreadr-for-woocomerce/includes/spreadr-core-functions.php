    		...
    		$category_name =  $meta['value'];
			  ...
			  // Check if the category exists by name
				...
				if ( ! $category ) {
					...
					// Handle potential errors during category creation
					if ( is_wp_error( $category_id ) ) {
						error_log( 'Error creating category: ' . $category_id->get_error_message() );
						return;
					}

					// Get the term_id of the newly created category
					$category_id = $category_id['term_id'];
				} else {
					// Use the existing category's term_id
