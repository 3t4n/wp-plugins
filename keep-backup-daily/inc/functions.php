		...
		function kbd_download_backup_zip(){
			...
			if(isset($_POST['kbd_export_selected'])){
				...
				if(!isset($_POST['kbd_nonce_field']) || !wp_verify_nonce($_POST['kbd_nonce_field'], 'kbd_nonce_action')){
					...
					$allowed_directory = str_replace('\\', '/', $allowed_directory);
					
					//pree($kbd_export_dir_selection);pree($real_path);pree($allowed_directory);exit;
					...
					$validated_paths = array_filter($path_to_zip, function($path) use ($allowed_directory) {
						$real_path = realpath($path);
						$real_path = ($real_path !== false ? str_replace('\\', '/', $real_path) : false);
						return ($real_path !== false && strpos($real_path, $allowed_directory) === 0);
					});
					

					
					if (empty($validated_paths)) {
						wp_die(__('Invalid download path detected.', 'wpkbd'));
					}

					
					kbd_download_download_zip_bulk($validated_paths);
