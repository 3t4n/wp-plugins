...
class Layout
{
    ...
    public function __construct()
    {
        ...
        $this->post_meta = PostMeta::get_instance();
        // get options
        $this->get_options();
    ...
    public function get_book_header_html($post_id)
    {
        $book_name = get_post_meta($post_id, "_kitab_book_name", true);
        $author = get_post_meta($post_id, "_kitab_book_author", true);
        $pdf_link = get_post_meta($post_id, "_kitab_pdf_link", true);
        $epub_link = get_post_meta($post_id, "_kitab_epub_link", true);
        $buy_link = get_post_meta($post_id, "_kitab_buy_link", true);

        // Early return if necessary fields are empty
        if (empty($book_name) || empty($pdf_link) && empty($epub_link) && empty($buy_link)) {
            return '';
        }

        // Check if post type is allowed and has thumbnail
        if (!$this->post_meta->is_post_type_allowed($post_id) || !has_post_thumbnail($post_id)) {
            return '';
        }
        // wp_die(var_dump($this->selected_show_fields));
        // Use output buffering to capture the HTML
        ob_start();
?>
<div class="kitab-book-header">
    <?php if ($this->selected_show_thumbnail && has_post_thumbnail($post_id)) : ?>
    <div class="kitab-book-cover">
        <?php the_post_thumbnail('kitab_thumbnail', array('class' => 'kitab-book-thumbnail')); ?>
    </div>
    <?php endif; ?>
    <div class="kitab-book-header-content">
        <h2 class="kitab-book-title"><?php echo $book_name; ?></h2>
        <p class="kitab-book-author"><?php echo $author; ?></p>
        <div class="kitab-book-actions">
            <?php if (!empty($pdf_link) && in_array('kitab_pdf_link', $this->selected_show_fields)) : ?>
            <a href="<?php echo $pdf_link; ?>" class="kitab-btn kitab-btn-primary" target="_blank"
                rel="noopener noreferrer">
                <span class="kitab-btn-icon">📥</span>
                <?php echo esc_html__('Download PDF', 'kitab'); ?>
            </a>
            <?php endif; ?>
            <?php if (!empty($epub_link) && in_array('kitab_epub_link', $this->selected_show_fields)) : ?>
            <a href="<?php echo $epub_link; ?>" class="kitab-btn kitab-btn-secondary" target="_blank"
                rel="noopener noreferrer">
                <span class="kitab-btn-icon">📦</span>
                <?php echo esc_html__('Download Epub', 'kitab'); ?>
            </a>
            <?php endif; ?>
            <?php if (!empty($buy_link) && in_array('kitab_buy_link', $this->selected_show_fields)) : ?>
            <a href="<?php echo $buy_link; ?>" class="kitab-btn kitab-btn-outline" target="_blank"
                rel="noopener noreferrer">
                <span class="kitab-btn-icon">🛒</span>
                <?php echo esc_html__('Buy Book', 'kitab'); ?>
            </a>
            <?php endif; ?>
        </div>
    </div>
...
<?php
    ...
    public function get_book_fields_table_html($post_id)
    {
        ...
        if ($this->post_meta->is_post_type_allowed($post_id) && !empty($book_name)) {
            ...
            $html .=  $this->get_book_header_html($post_id);
            ...
            foreach ($this->post_meta->get_book_fields() as $field_key => $field_details) {
                ...
                if (!empty($value)) {
                    ...
                    // if not has filter
                    if ($field_details['type'] === 'url' && isset($field_details['text'])) {
                        $html .= "<a href='{$value}' target='_blank' rel='noopener noreferrer'>{$field_details['text']}</a>";
                    }
