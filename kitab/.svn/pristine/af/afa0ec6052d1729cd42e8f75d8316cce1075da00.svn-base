...
class Kitab_Plugin
{
    ...
    // register thumbail sizes for  post thumbnail sizes for ebooks covers
    public function init()
    {
        // multiple book cover sizes

        add_image_size('kitab_thumbnail', 200, 302, true);
    }
...
<div id="kitab-admin-notice" class="notice notice-info is-dismissible">
    <p><?php esc_html_e('Thank you for using Kitab! If you enjoy this plugin, please consider leaving us a rating on the WordPress plugin repository.', 'kitab'); ?>
    </p>
    <p>
...
<?php
    ...
    public function kitab_loaded()
    {
        ...
        // Add custom taxonomies for authors and publishers
        $taxonomies = Kitab\Taxonomies::get_instance();

        // Add custom post type for books
        $custom_post_type = Kitab\CustomPostType::get_instance();
    }
...
// wp_die(var_dump(get_option('kitab_general_settings')));
