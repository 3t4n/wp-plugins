...
if ( ! class_exists( 'PLZ_Rest_API_Configuration' ) ) :
  class PLZ_Rest_API_Configuration {
    ...
    public function set_authentification( $request ) {
      ...
      if ( isset( $params['public'] ) && ! empty( $params['public'] ) && isset( $params['secret'] ) && ! empty( $params['secret'] ) ) :
        ...
        if ( $response_code && 200 === $response_code ) :
          ...
          return new WP_REST_Response( array( 'message' => __( 'Authentification access saved.', 'plezi' ), 'success' => true ), 200 );
        else :
          return new WP_REST_Response( array( 'message' => __( 'Authentification access not saved.', 'plezi' ), 'error' => __( 'Authentification issue.', 'plezi' ) ), 200 );
        endif;
      else :
        return new WP_REST_Response( array( 'message' => __( 'Authentification access not saved.', 'plezi' ), 'error' => __( 'Data issue.', 'plezi' ) ), 200 );
      endif;
    }
    ...
    public function remove_authentification() {
      ...
      return new WP_REST_Response( array( 'message' => __( 'Authentification access removed.', 'plezi' ), 'success' => true ), 200 );
    }
    ...
    public function set_tracking_manual_status( $request ) {
      ...
      if ( isset( $params['status'] ) ) :
        ...
        return new WP_REST_Response( array( 'message' => __( 'Tracking manual status updated.', 'plezi' ), 'date' => gmdate( 'H:i:s d/m/Y' ) ), 200 );
      else :
        return new WP_REST_Response( array( 'message' => __( 'Tracking manual status not updated.', 'plezi' ), 'error' => __( 'Data issue.', 'plezi' ) ), 200 );
      endif;
    }
    ...
    public function get_tracking_api() {
      ...
      if ( $signature && ! empty( $signature ) ) :
        ...
        if ( $result && isset( $result->data ) && isset( $result->data->script ) && ! empty( $result->data->script ) ) :
          ...
          return new WP_REST_Response( array( 'message' => __( 'Tracking code added.', 'plezi' ), 'script' => $script ), 200 );
        endif;

        return new WP_REST_Response( array( 'message' => __( 'Tracking code not added.', 'plezi' ), 'error' => __( 'Call API issue.', 'plezi' ) ), 200 );
      else :
        return new WP_REST_Response( array( 'message' => __( 'Tracking code not added.', 'plezi' ), 'error' => __( 'Signature issue.', 'plezi' ) ), 200 );
      endif;
    }
    ...
    public function set_tracking_api( $request ) {
      	...
      	return new WP_REST_Response( array( 'message' => __( 'Tracking code added.', 'plezi' ), 'script' => $params['tracking'] ), 200 );
			else :
				return new WP_REST_Response( array( 'message' => __( 'Tracking code not added.', 'plezi' ), 'error' => __( 'Tracking code not valide.', 'plezi' ) ), 200 );
			endif;
		}
    ...
    public function remove_tracking_api() {
      ...
      return new WP_REST_Response( array( 'message' => __( 'Tracking code removed.', 'plezi' ), 'success' => true ), 200 );
    }
    ...
    public function set_tracking_api_status( $request ) {
      ...
      if ( isset( $params['status'] ) ) :
        ...
        return new WP_REST_Response( array( 'message' => __( 'Tracking API status updated.', 'plezi' ), 'date' => gmdate( 'd/m/Y' ) ), 200 );
      else :
        return new WP_REST_Response( array( 'message' => __( 'Tracking API status not updated.', 'plezi' ), 'error' => __( 'Data issue.', 'plezi' ) ), 200 );
      endif;
    }
      ...
      if ( $signature && ! empty( $signature ) ) :
        ...
        if ( $result && isset( $result->data ) && is_array( $result->data ) && ! empty( $result->data ) ) :
			...
			foreach ( $forms as $form ) :
				...
				$form->attributes->custom_title = $form->attributes->title . ' - ' . __( 'Created on ', 'plezi' ) . date_i18n( get_option( 'date_format' ), $date->getTimestamp() ) . ' - ' . date_i18n( get_option( 'time_format' ), $date->getTimestamp() );
			endforeach;

			if ( isset( $params['default'] ) && ! empty( $params['default'] ) && $params['default'] ) :
				array_unshift( $forms, array( 'id' => '', 'type' => 'default', 'attributes' => array( 'id' => '', 'title' => __('Choose a Plezi form', 'plezi'), 'custom_title' => __('Choose a Plezi form', 'plezi') ) ) );
			endif;

          return new WP_REST_Response( array( 'message' => __( 'Forms list retrieved.', 'plezi' ), 'list' => $forms, 'metas' => $metas ), 200 );
        endif;

        return new WP_REST_Response( array( 'message' => __( 'Forms list not retrieved.', 'plezi' ), 'error' => __( 'Call API issue.', 'plezi' ) ), 200 );
      else :
        return new WP_REST_Response( array( 'message' => __( 'Forms list not retrieved.', 'plezi' ), 'error' => __( 'Signature issue.', 'plezi' ) ), 200 );
      endif;
    }
