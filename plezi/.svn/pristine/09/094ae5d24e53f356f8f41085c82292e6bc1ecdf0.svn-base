...
class Elementor_Plezi_Form_Widget extends \Elementor\Widget_Base {
	...
	protected function register_controls() {
		...
		$options = array( 'default' => __( 'Choose a Plezi form', 'plezi' ) );
	...
	protected function render() {
		...
		if ( $settings['plezi_form'] && ! empty( $settings['plezi_form'] ) && 'default' !== $settings['plezi_form'] ) :
			wp_enqueue_script( 'plezi-form-' . esc_attr( $settings['plezi_form'] ), 'https://brain.plezi.co/api/v1/web_forms/scripts?content_web_form_id=' . esc_attr( $settings['plezi_form'] ), [], PLZ_VERSION, [ 'in_footer' => true, 'strategy' => 'async' ] );

			echo '<form id="plz-form-' . esc_attr( $settings['plezi_form'] ) . '"></form>';
		else :
			echo '<p>' . esc_html( __( 'Choose a Plezi form inside the sidebar.', 'plezi' ) ) . '</p>';
		endif;
	}
}
