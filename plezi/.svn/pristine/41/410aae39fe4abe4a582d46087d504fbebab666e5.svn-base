...
if ( ! function_exists( 'plz_settings_link' ) ) :
  function plz_settings_link( $links ) {
    ...
    $signup = '<a href="' . $signup_url . '" title="' . __( 'Signup', 'plezi' ) . '" target="_blank">' . __( 'Signup', 'plezi' ) . '</a>';
  	$settings = '<a href="' . $settings_url . '" title="' . __( 'Settings', 'plezi' ) . '">' . __( 'Settings', 'plezi' ) . '</a>';
...
if ( ! function_exists( 'plz_add_pages' ) ) :
  function plz_add_pages() {
    if ( current_user_can( 'manage_options' ) ) :
      add_menu_page( __( 'Plezi', 'plezi' ), __( 'Plezi', 'plezi' ), 'manage_options', 'plezi-for-wordpress-configuration.php', '', plugin_dir_url( __DIR__ ) . 'images/ico_logo.svg', 26 );
      add_submenu_page( 'plezi-for-wordpress-configuration.php', __( 'Setup', 'plezi' ), __( 'Setup', 'plezi' ), 'manage_options', 'plezi-for-wordpress-configuration.php', 'plz_admin_configuration', 1 );
      add_submenu_page( 'plezi-for-wordpress-configuration.php', __( 'Forms', 'plezi' ), __( 'Forms', 'plezi' ), 'manage_options', 'plezi-for-wordpress-forms.php', 'plz_admin_forms', 2 );
      add_submenu_page( 'plezi-for-wordpress-configuration.php', __( 'FAQ', 'plezi' ), __( 'FAQ', 'plezi' ), 'manage_options', 'plezi-for-wordpress-faq.php', 'plz_admin_faq', 3 );
    endif;
  }
endif;
...
if ( ! function_exists( 'plz_admin_configuration' ) ) :
  function plz_admin_configuration() {
    if ( ! current_user_can( 'manage_options' ) ) :
      wp_die( esc_attr__( 'You do not have sufficient permissions to access this page.', 'plezi' ) );
    endif;
    ...
    echo '<h1><strong>' . esc_attr__( 'Setup', 'plezi' ) . '</strong></h1>';
    echo '<div class="plezi-wrap-page-row">';
    echo '<div class="plezi-left-column">';
    echo '<ul class="plezi-tabs-buttons">';
    echo '<li class="active"><span class="status ' . esc_attr( $tab_authentification_status ) . '"></span><a href="#" title="' . esc_attr__( 'API connexion', 'plezi' ) . '" data-tab="authentification">' . esc_attr__( 'API Connexion', 'plezi' ) . '</a></li>';
    echo '<li><span class="status ' . esc_attr( $tab_tracking_status ) . '"></span><a href="#" title="' . esc_attr__( 'Tracking', 'plezi' ) . '" data-tab="tracking">' . esc_attr__( 'Tracking', 'plezi' ) . '</a></li>';
    echo '</ul>';
    echo '</div>';
    ...
    submit_button( __( 'Connect', 'plezi' ), 'plz-submit-button ' . $plz_class_get_authentification, 'submit', false, array( 'id' => 'plezi-get-authentification' ) );
    submit_button( __( 'Logout', 'plezi' ), 'plz-submit-button plz-logout-button ' . $plz_class_remove_authentification, 'submit', false, array( 'id' => 'plezi-remove-authentification' ) );
...
if ( ! function_exists( 'plz_admin_forms' ) ) :
  function plz_admin_forms() {
    ...
    echo '<div class="wrap plezi-wrap-page">';
    echo '<h1><strong>' . esc_attr__( 'Forms', 'plezi' ) . '</strong></h1>';
    ...
    if ( ! isset( $options['plz_authentification_status'] ) || empty( $options['plz_authentification_status'] ) || '0' === $options['plz_authentification_status'] ) :
      ...
      echo '<p>' . esc_attr__( 'To add your Plezi forms onto your pages, you need to connect Plezi to your WordPress account.', 'plezi' ) . '</p>';
      echo '<a class="plezi-btn-purple" href="' . esc_url( $setup ) . '" title="' . esc_attr__( 'Connect with API', 'plezi' ) . '">';
      echo esc_attr__( 'Connect', 'plezi' );
      echo '</a>';
      echo '</div>';
    else :
      ...
      if ( $forms_list_table && isset( $forms_list_table->items ) ) :
        echo '<div class="plezi-wrap-page-content-introduction">';
        echo '<p>' . esc_attr__( 'If you use Elementor, Gutenberg or Divi, that\'s fantastic! You’ll find a Plezi Form widget, directly in your page editor. Otherwise, you can copy your form\'s shortcode here and paste it once you get to your page\'s edition.', 'plezi' ) . '</p>';
        echo '<div class="plz-forms-notice">';
        echo '<p>' . esc_attr__( 'To follow your website performance, go check out your dashboard in Plezi.', 'plezi' ) . '</p>';
        echo '<a href="https://enjoy.plezi.co/dashboard?utm_medium=referral&utm_source=wordpress&utm_campaign=plezi_one&utm_content=plugin&utm_term=forms-page" title="' . esc_attr__( 'Go to dashboard', 'plezi' ) . '" target="_blank" class="plezi-btn-purple">' . esc_attr__( 'Go to dashboard', 'plezi' ) . '</a>';
        echo '</div>';
        ...
        echo '<a class="plz-popup-close" href="#" title="' . esc_attr__( 'Close the popup', 'plezi' ) . '">×</a>';
        ...
        echo '<p>' . esc_attr__( 'If you use Elementor, Gutenberg or Divi, that\'s fantastic! You’ll find a Plezi Form widget, directly in your page editor. Otherwise, you can copy your form\'s shortcode here and paste it once you get to your page\'s edition.', 'plezi' ) . '</p>';
		echo '<p class="plezi-image-center"><img src="' . esc_url( plugin_dir_url( __DIR__ ) ) . 'images/ico_forms_empty.svg" alt="' . esc_attr__( 'It seems that you\'ve never created any form in Plezi -yet ;)', 'plezi' ) . '" /></p>';		
		echo '<p class="plezi-strong">' . esc_attr__( 'It seems that you\'ve never created any form in Plezi -yet ;)', 'plezi' ) . '</p>';
		echo '<p class="plezi-button-center"><a class="plezi-btn-purple" href="https://enjoy.plezi.co/resources/create?utm_medium=referral&utm_source=wordpress&utm_campaign=plezi_one&utm_content=plugin&utm_term=forms-page" target="_blank" title="' . esc_attr__( 'Create my first form', 'plezi' ) . '">';
        echo esc_attr__( 'Create my first form', 'plezi' );
        echo '</a></p>';
        echo '</div>';
      endif;
    endif;
...
if ( ! function_exists( 'plz_admin_faq' ) ) :
  function plz_admin_faq() {
    ...
    $setup = admin_url( 'admin.php?page=plezi-for-wordpress-configuration.php' );
    $faq = array(
            __( 'Are there any pre-requisites to use this plugin?', 'plezi' ) => __( 'To use the Plezi plugin, you need:', 'plezi' ) . '<br />' . esc_attr__( '- to have a Plezi One account.', 'plezi' ) . ' <a href="https://enjoy.plezi.co/signup?utm_medium=referral&utm_source=wordpress&utm_campaign=plezi_one&utm_content=plugin&utm_term=faq-page" target="_blank" title="' . esc_attr__( 'Signup here', 'plezi' ) . '">' . esc_attr__( 'Signup here', 'plezi' ) . '</a>.<br />' . esc_attr__( '- connect your Plezi account to your Wordpress account.', 'plezi' ) . ' <a href="' . $setup . '" title="' . esc_attr__( 'Connect Plezi here', 'plezi' ) . '">' . esc_attr__( 'Connect Plezi here', 'plezi' ) . '</a>.',
            __( 'What is this plugin for?', 'plezi' ) => __( 'This plugin allows you to:', 'plezi' ) . '<br />' . esc_attr__( '- have the Plezi tracking script through all your website pages', 'plezi' ) . '<br />' . esc_attr__( '- publish your Plezi forms online, thanks to a tailored Plezi widget (on Gutemberg, Elementor and Divi builders) or simplified shortcodes', 'plezi' ) . '<br />' . esc_attr__( '- take shortcuts to your Plezi dashboard to follow your website stats', 'plezi' ),
            __( 'What is Plezi\'s tracking script for?', 'plezi' ) => __( 'The tracking script allows Plezi to see visits to your website and identify prospects when they return.', 'plezi' ),
            __( 'How do I install Plezi tracking script on my website with Wordpress?', 'plezi' ) =>
            __( 'To install Plezi tracking script, follow the instructions', 'plezi' ) . ' <a href="' . $setup . '" title="' . esc_attr__( 'on the Setup tab of this plugin', 'plezi' ) . '">' . esc_attr__( 'on the Setup tab of this plugin', 'plezi' ) . '</a>.',
            __( 'How can I get all my Plezi forms in WordPress?', 'plezi' ) => __( 'To find your Plezi forms in WordPress, you’ll need to authenticate your Plezi API keys.', 'plezi' ),
            __( 'How can I publish a Plezi form in WordPress?', 'plezi' ) => __( 'If you use Elementor, Gutenberg or Divi, that\'s fantastic! You’ll find a Plezi Form widget, directly in your page editor. Otherwise, you can copy your form\'s shortcode here and paste it in your editor.', 'plezi' ),
            __( 'How can I use Plezi shortcodes in WordPress?', 'plezi' ) => __( 'To publish a form online, copy its shortcode and paste it in a classic text bloc.', 'plezi' ),
          );

    echo '<div class="wrap plezi-wrap-page plezi-wrap-faq">';
    echo '<h1>' . esc_attr__( 'Frequently Asked', 'plezi' ) . ' <strong>' . esc_attr__( 'Questions', 'plezi' ) . '</strong></h1>';
    echo '<ul>';
...
if ( ! function_exists( 'plz_page_configuration_section_tracking' ) ) :
  function plz_page_configuration_section_tracking() {
    echo '<p>' . esc_attr__( 'Install Plezi\'s tracking script to see visits on your website. Get your script by 1) clicking on the pink button below if you\'ve connected Plezi to Wordpress or 2) by pasting it ', 'plezi' ) . ' <a href="https://enjoy.plezi.co/settings/configuration?utm_medium=referral&utm_source=wordpress&utm_campaign=plezi_one&utm_content=plugin&utm_term=settings-page" title="' . esc_attr__( 'from here', 'plezi' ) . '" target="_blank">' . esc_attr__( 'from here', 'plezi' ) . '</a> ' . esc_attr__( ', activate it... et voilà!', 'plezi' ) . '</p>';
    echo '<p>' . esc_attr__( 'If you\'d rather choose another method to install the script (e.g. via Google Tag Manager), that\'s alright!', 'plezi' ) . ' <strong>' . esc_attr__( 'Don\'t do it twice', 'plezi' ) . '</strong> ' . esc_attr__( 'unless you want to see your visits counted twice ;)', 'plezi' );
  }
endif;
...
if ( ! function_exists( 'plz_configuration_tracking_choice_field' ) ) :
  function plz_configuration_tracking_choice_field() {
    ...
    echo '<label for="api" class="plezi-radio-label">' . esc_attr__( 'Install with this plugin (faster)', 'plezi' ) . '</label>';
    ...
    echo '<label for="manual" class="plezi-radio-label">' . esc_attr__( 'Install script another way', 'plezi' ) . '</label>';
  }
endif;
...
if ( ! function_exists( 'plz_configuration_tracking_code_field' ) ) :
  function plz_configuration_tracking_code_field() {
    ...
    echo '<h5>' . esc_attr__( 'Tracking script', 'plezi' ) . '</h5>';
    ...
    echo '<p class="plz-date-validation ' . esc_attr( $plz_class_tracking_api_date ) . '">' . esc_attr__( 'Script added on ', 'plezi' ) . '<span>' . esc_attr( $tracking_date ) . '</span></p>';
    echo '</div>';
    echo '<div class="plezi-tracking-right">';
    echo '<button type="button" name="plezi-remove-tracking" id="plezi-remove-tracking" class="plezi-btn-purple plezi-btn-purple-active ' . esc_attr( $plz_class_tracking_api_btn_remove ) . '">' . esc_attr__( 'Remove tracking', 'plezi' ) . '</button>';
    echo '<button type="button" name="plezi-get-tracking" id="plezi-get-tracking" class="plezi-btn-purple ' . esc_attr( $plz_class_tracking_api_btn_get ) . '">' . esc_attr__( 'Get my tracking script', 'plezi' ) . '</button>';
    ...
    echo '</label>';
    echo '<span id="plz-label-active" class="plz-switch-label-active ' . esc_attr( $plz_class_tracking_api_label_active) . '">' . esc_attr__( 'Active', 'plezi' ) . '</span>';
    echo '<span id="plz-label-inactive" class="plz-switch-label ' . esc_attr( $plz_class_tracking_api_label_inactive) . '">' . esc_attr__( 'Inactive', 'plezi' ) . '</span>';
    echo '</div>';
    echo '</div>';
    echo '<div class="plz-tracking-confirmation-sentence ' . esc_attr( $plz_class_tracking_api_validation ) . '">';
    echo '<img src="' . esc_url( plugin_dir_url( __DIR__ ) ) . 'images/ico_valid_tracking.svg" alt="' . esc_attr__( 'The tracking script is integrated on your website! Visits will be tracked from now on in our application.', 'plezi' ) . '" />';
    echo '<p>' . esc_attr__( 'The tracking script is integrated to your website! Visits will be tracked from now on.', 'plezi' ) . '</p>';
    echo '</div>';
    echo '</div>';
    echo '</div>';
    echo '<div class="plezi-tracking-manual-wrapper">';
    echo '<div class="plezi-tracking-manual-row">';
    echo '<h5>' . esc_attr__( 'Note to myself', 'plezi' ) . '</h5>';
    ...
    echo '<label for="plz_configuration_tracking_enable_manual">' . esc_attr__( 'I installed the tracking script with Google Tag Manager or manually', 'plezi' ) . '</label>';
    echo '</p>';
    echo '</div>';
    echo '<button type="button" name="plezi-set-tracking-manual" id="plezi-set-tracking-manual" class="plezi-btn-purple plezi-btn-purple-active ' . esc_attr( $plz_class_tracking_button_manual_status ) . '" ' . esc_attr( $plz_configuration_tracking_button_manual_status ) . '>' . esc_attr__( 'Save', 'plezi' ) . '</button>';
    echo '<div class="plz-manual-date-validation-wrapper ' . esc_attr( $plz_class_tracking_date_manual ) . '">';
    echo '<hr />';
    echo '<p class="plz-date-validation"><span>' . esc_attr( $plz_configuration_tracking_date_manual_option ) . '</span> : ' . esc_attr__( 'Tracking script Installed via another method', 'plezi' ) . '</p>';
    echo '</div>';
    echo '</div>';
    echo '</div>';
  }
...
if ( ! function_exists( 'plz_configuration_authentification_public_key_field' ) ) :
  function plz_configuration_authentification_public_key_field() {
    ...
    echo '<label for="plz_authentification_public_key">' . esc_attr__( 'API key', 'plezi' ) . '</label>';
    ...
    echo '<img class="plz-validation-authentification ' . esc_attr( $authentification_status_class ) . '" src="' . esc_url( plugin_dir_url( __DIR__ ) ) . 'images/ico_valid_option.svg" alt="' . esc_attr__( 'API key', 'plezi' ) . '" />';
  }
endif;
...
if ( ! function_exists( 'plz_configuration_authentification_secret_key_field' ) ) :
  function plz_configuration_authentification_secret_key_field() {
    ...
    echo '<label for="plz_authentification_secret_key">' . esc_attr__( 'Secret key', 'plezi' ) . '</label>';
    ...
    echo '<img class="plz-validation-authentification ' . esc_attr( $authentification_status_class ) . '" src="' . esc_url( plugin_dir_url( __DIR__ ) ) . 'images/ico_valid_option.svg" alt="' . esc_attr__( 'Secret key', 'plezi' ) . '" />';
    echo '<p class="plz-error-authentification">';
    echo esc_attr__( 'Oh no! These keys don\'t exist. Please try again.', 'plezi' );
    echo '</p>';
  }
endif;
...
if ( ! function_exists( 'plz_admin_include_scripts' ) ) :
  function plz_admin_include_scripts() {
    ...
    if ( is_admin() && current_user_can( 'manage_options' ) && plz_check_screen( ) ) :
      ...
      wp_localize_script( 'plz-admin-script', 'plzlabels', array( 'plztrackingremovescript'      => __( 'Are you sure you want to delete the script? You can disable it with the toggle below.', 'plezi' ) ) );
...
if ( ! function_exists( 'plz_footer' ) ) :
  function plz_footer() {
    if ( plz_check_screen( ) ) :
      return '<span class="plz-footer">' . esc_html__( 'Powered with 💗︎ by ', 'plezi' ) . '</span><img src="' . esc_url( plugin_dir_url( __DIR__ ) ) . 'images/footer_logo.svg" alt="' . esc_attr__( 'Plezi', 'plezi' ) . '" class="plz-footer-image" />';
    endif;
  }
endif;
...
if ( ! function_exists( 'plz_load_textdomain' ) ) :
  function plz_load_textdomain( $mofile, $domain ) {
    if ( 'plezi' === $domain && false !== strpos( $mofile, WP_LANG_DIR . '/plugins/' ) ) :
...
if ( ! function_exists( 'plz_load_plugin' ) ) :
	function plz_load_plugin() {
    	load_plugin_textdomain( 'plezi', false, plugin_basename( __DIR__ ) . '/languages' );
...
if ( ! function_exists( 'plz_show_permalink_error' ) ) :
	function plz_show_permalink_error() {
        ...
        echo '<p>';
		echo esc_attr__( 'Please, edit your ', 'plezi' );
		echo '<a href="' . esc_url( admin_url( 'options-permalink.php' ) ) . '" title="' . esc_attr__( 'permalinks structure', 'plezi' ) . '">';
		echo esc_attr__( 'permalinks structure', 'plezi' );
		echo '</a>';
		echo esc_attr__( ' to use Plezi plugin.', 'plezi' );
		echo '</p>';
    	echo '</div>';
	}
endif;
...
if ( ! function_exists( 'plz_add_dashboard_widget' ) ) :
  function plz_add_dashboard_widget() {
    ...
    wp_add_dashboard_widget($plz_widget_id, __( 'Plezi’s dashboard', 'plezi' ), 'plz_dashboard_widget_content' );
...
if ( ! function_exists( 'plz_dashboard_widget_content' ) ) :
  function plz_dashboard_widget_content( $post, $callback_args ) {
    ...
    echo '<h3>' . esc_attr__( 'Follow your website performance on Plezi', 'plezi' ) . '</h3>';
    echo '<ul class="introduction">';
    echo '<li><span class="plz-dashboard-widget-validation">✔</span> ' . esc_attr__( 'Visits, conversion rate, leads', 'plezi' );
    echo '<li><span class="plz-dashboard-widget-validation">✔</span> ' . esc_attr__( 'Top digital channels', 'plezi' );
    echo '<li><span class="plz-dashboard-widget-validation">✔</span> ' . esc_attr__( 'Top keywords and SEO positions', 'plezi' );
    echo '</ul>';
    echo '<p class="plezi-button-center">';
    echo '<a class="plezi-btn-purple" href="https://enjoy.plezi.co/dashboard?utm_medium=referral&utm_source=wordpress&utm_campaign=plezi_one&utm_content=plugin&utm_term=widget-dashboard" target="_blank" title="' . esc_attr__( 'Go to dashboard', 'plezi' ) . '">';
    echo esc_attr__( 'Go to dashboard', 'plezi' );
    echo '</a>';
    echo '</p>';
    echo '</div>';
    echo '</div>';
...
if ( ! function_exists( 'plz_form_shortcode' ) ) :
  function plz_form_shortcode( $atts ) {
	$atts = shortcode_atts( [ 'form' => null ], $atts, 'plezi' );
	$form = isset( $atts['form'] ) ? sanitize_text_field( $atts['form'] ) : null;

    if ( $form && ! empty( $form ) && preg_match( '/^[a-zA-Z0-9-]+$/', $form ) ) :
		$escaped_form = esc_attr( $form );

        wp_enqueue_script( 'plezi-form-' . $escaped_form, 'https://brain.plezi.co/api/v1/web_forms/scripts?content_web_form_id=' . $escaped_form, [], PLZ_VERSION, [ 'in_footer' => true, 'strategy' => 'async' ] );

		return '<form id="plz-form-' . $escaped_form . '"></form>';
    endif;
