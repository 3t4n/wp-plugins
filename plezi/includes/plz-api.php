		...
		if ( $signature && ! empty( $signature ) ) :
			...
			if ( $result_decode && isset( $result_decode->data ) && is_array( $result_decode->data ) && ! empty( $result_decode->data ) ) :
				...
				if ( isset( $params['default'] ) && ! empty( $params['default'] ) && $params['default'] ) :
					array_unshift( $forms, [ 'id' => '', 'type' => 'default', 'attributes' => [ 'id' => '', 'title' => __( 'Choose a Plezi form', 'plezi' ), 'custom_title' => __( 'Choose a Plezi form', 'plezi' ) ] ] );
				endif;

  				return [ 'message' => __( 'Forms list retrieved.', 'plezi' ), 'list' => $forms, 'metas' => $metas ];
			endif;

			return [ 'message' => __( 'Forms list not retrieved.', 'plezi' ), 'error' => __( 'Call API issue.', 'plezi' ) ];
		else :
			return [ 'message' => __( 'Forms list not retrieved.', 'plezi' ), 'error' => __( 'Signature issue.', 'plezi' ) ];
		endif;
	}
endif;
