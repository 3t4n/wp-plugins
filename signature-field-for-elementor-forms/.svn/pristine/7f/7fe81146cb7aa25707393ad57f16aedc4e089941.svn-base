...
class Superaddons_Elementor_Signature_Field extends \ElementorPro\Modules\Forms\Fields\Field_Base {
	...
	public function process_field( $field, $record, $ajax_handler) {
		...
		$elementor_uploads_root = trailingslashit( $uploads['basedir'] ) . 'elementor/forms/signature';
		...
		$file_url             = trailingslashit( $uploads['baseurl'] ) . 'elementor/forms/signature/' . trailingslashit( $form_directory ) . $file_name;
