...
use Felix_Arntz\AI_Services_Dependencies\Felix_Arntz\WP_OOP_Plugin_Lib\REST_Routes\Contracts\REST_Resource_Schema;
...
abstract class Abstract_REST_Resource_Schema implements REST_Resource_Schema
{
     ...
     */
    public final function prepare_resource(Entity $entity, WP_REST_Request $request) : WP_REST_Response
    {
        $fields = $this->get_fields_to_include($request);
        $response = rest_ensure_response($this->prepare_resource_fields($entity, $fields));
        if ($this->is_field_included('_links', $fields) || $this->is_field_included('_embedded', $fields)) {
            $response->add_links($this->prepare_resource_links($entity));
        }
        return $response;
    }
    /**
     ...
     */
    public final function prepare_resources_for_query(Entity_Query $query, WP_REST_Request $request) : WP_REST_Response
    {
        ...
        if ($page > $max_pages && $total_entities > 0) {
            throw REST_Exception::create(
                ...
                esc_html__('The page number requested is larger than the number of pages available.', 'ai-services'),
                // phpcs:ignore Generic.Files.LineLength.TooLong
                400
            );
        }
        ...
        $base_url = add_query_arg(urlencode_deep($request->get_query_params()), $this->get_collection_route_url());
        if ($page > 1) {
            $prev_page = $page - 1;
     ...
     */
    public final function get_public_schema() : array
    {
        $schema = $this->schema;
     ...
     */
    public final function get_schema_args(string $method = WP_REST_Server::CREATABLE) : array
    {
     ...
     */
    protected final function get_fields_to_include(WP_REST_Request $request) : array
    {
        ...
        if ($context) {
            foreach ($properties as $name => $options) {
                if (!empty($options['context']) && !\in_array($context, $options['context'], \true)) {
        ...
        return \array_reduce($requested_fields, static function ($response_fields, $field) use($fields) {
             ...
             */
            if (\in_array($nested_fields[0], $fields, \true)) {
                $response_fields[] = $field;
            }
            return $response_fields;
        }, array());
     ...
     */
    protected final function get_context_arg() : array
    {
        $param_details = array(
        ...
        if ($contexts) {
            ...
            if (\in_array('view', $param_details['enum'], \true)) {
     ...
     */
    protected final function is_field_included(string $field, array $fields) : bool
    {
     ...
     */
    protected final function get_route_url(string $rest_base) : string
    {
     ...
     */
    protected abstract function prepare_resource_fields($entity, array $fields) : array;
    /**
     ...
     */
    protected abstract function prepare_resource_links($entity) : array;
    /**
     ...
     */
    protected abstract function get_collection_route_url() : string;
    /**
