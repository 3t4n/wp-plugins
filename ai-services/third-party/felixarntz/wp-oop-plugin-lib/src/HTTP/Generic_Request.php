...
class Generic_Request implements Request
{
    ...
    public function __construct(string $url, array $data = array(), array $args = array())
    {
        if ($data && isset($args['body']) && $args['body']) {
            _doing_it_wrong(
                ...
                esc_html__('Both a request data array and a request body were provided, but only one of them is allowed.', 'ai-services'),
                ''
            );
            unset($args['body']);
        }
    ...
    {
        if ($this->body) {
            _doing_it_wrong(__METHOD__, esc_html__('Data cannot be added to a request that already has a body.', 'ai-services'), '');
            return;
        }
    ...
    private function sanitize_args(array $args, string $method) : array
    {
        if (isset($args['method']) && !$this->is_valid_method($args['method'])) {
            _doing_it_wrong(esc_html($method), esc_html(\sprintf(
                /* translators: %s: invalid method string */
                __('The value %s is not a valid HTTP request method.', 'ai-services'),
                (string) $args['method']
            )), '');
            unset($args['method']);
        }
        if (isset($args['body']) && !\is_string($args['body'])) {
            _doing_it_wrong(esc_html($method), esc_html__('The request body must be a string.', 'ai-services'), '');
            unset($args['body']);
        }
        if (isset($args['headers']) && (!\is_array($args['headers']) || $args['headers'] && wp_is_numeric_array($args['headers']))) {
            _doing_it_wrong(esc_html($method), esc_html__('The request headers must be an associative array.', 'ai-services'), '');
            unset($args['headers']);
        }
        return $args;
    }
    ...
    {
        return \in_array($method, array(Request::DELETE, Request::GET, Request::PATCH, Request::POST, Request::PUT), \true);
    }
}
