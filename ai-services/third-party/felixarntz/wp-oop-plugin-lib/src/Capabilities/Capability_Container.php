 ...
 */
namespace Felix_Arntz\AI_Services_Dependencies\Felix_Arntz\WP_OOP_Plugin_Lib\Capabilities;

use ArrayAccess;
use Felix_Arntz\AI_Services_Dependencies\Felix_Arntz\WP_OOP_Plugin_Lib\Capabilities\Contracts\Capability;
use Felix_Arntz\AI_Services_Dependencies\Felix_Arntz\WP_OOP_Plugin_Lib\General\Contracts\Container;
use Felix_Arntz\AI_Services_Dependencies\Felix_Arntz\WP_OOP_Plugin_Lib\General\Exception\Invalid_Type_Exception;
use Felix_Arntz\AI_Services_Dependencies\Felix_Arntz\WP_OOP_Plugin_Lib\General\Exception\Not_Found_Exception;
/**
...
class Capability_Container implements Container, ArrayAccess
{
    ...
    public function get(string $key)
    {
        if (!isset($this->capabilities[$key])) {
            throw new Not_Found_Exception(esc_html(\sprintf(
                /* translators: %s: capability key */
                __('Capability with key %s was not found in container', 'ai-services'),
                $key
            )));
        }
        if (!isset($this->instances[$key])) {
            ...
            if (!$instance instanceof Capability) {
                throw new Invalid_Type_Exception(esc_html(\sprintf(
                    /* translators: %s: capability key */
                    __('Capability with key %s is not of type Capability', 'ai-services'),
                    $key
                )));
            }
    ...
    {
        $this->set($key, function () use($key, $required_caps) {
