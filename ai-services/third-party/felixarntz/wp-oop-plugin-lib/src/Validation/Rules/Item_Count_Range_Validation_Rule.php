...
class Item_Count_Range_Validation_Rule implements Validation_Rule, With_Type_Support
{
    ...
    public function validate($value) : void
    {
        ...
        if (\count($value) < $this->min_count) {
            throw Validation_Exception::create('too_few_items', esc_html(\sprintf(
                /* translators: 1: value, 2: minimum count */
                _n('%1$s must contain at least %2$s item.', '%1$s must contain at least %2$s items.', $this->min_count, 'default'),
                'array',
                $this->min_count
            )));
        }
        if ($this->max_count > 0 && \count($value) > $this->max_count) {
            throw Validation_Exception::create('too_many_items', esc_html(\sprintf(
                /* translators: 1: value, 2: maximum count */
                _n('%1$s must contain at most %2$s item.', '%1$s must contain at most %2$s items.', $this->max_count, 'default'),
                'array',
                $this->max_count
            )));
        }
