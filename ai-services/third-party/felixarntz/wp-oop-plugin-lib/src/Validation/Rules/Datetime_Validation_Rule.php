...
class Datetime_Validation_Rule implements Validation_Rule, With_Type_Support
{
    ...
    public function validate($value) : void
    {
        ...
        if (!\preg_match('/^\\d{4}-\\d{2}-\\d{2}[Tt ]\\d{2}:\\d{2}:\\d{2}(?:\\.\\d+)?(?:Z|[+-]\\d{2}(?::\\d{2})?)?$/', (string) $value)) {
            throw Validation_Exception::create('invalid_datetime', \sprintf(
                /* translators: %s: value */
                esc_html__('%s is not a valid date and time string.', 'ai-services'),
    ...
    public function sanitize($value)
    {
        try {
            ...
            if (\is_string($value) && \preg_match('/\\d{4}-\\d{2}-\\d{2}[Tt ]\\d{2}:\\d{2}:\\d{2}(?:\\.\\d+)?(?:Z|[+-]\\d{2}(?::\\d{2})?)?/', $value, $matches)) {
                return $matches[0];
            }
            return '';
        }
