...
class Datetime_Range_Validation_Rule implements Validation_Rule, With_Type_Support
{
    ...
    public function validate($value) : void
    {
        ...
        if ($timestamp < \strtotime($this->min_datetime)) {
            throw Validation_Exception::create('out_of_bounds_min', \sprintf(
                /* translators: 1: value, 2: minimum date */
                esc_html__('%1$s must be at or before %2$s', 'ai-services'),
                esc_html((string) $value),
                esc_html($this->min_datetime)
            ));
        }
        if (null !== $this->max_datetime && $timestamp > \strtotime($this->max_datetime)) {
            throw Validation_Exception::create('out_of_bounds_max', \sprintf(
                /* translators: 1: value, 2: maximum date */
                esc_html__('%1$s must be before or at %2$s', 'ai-services'),
                esc_html((string) $value),
    ...
    public function sanitize($value)
    {
        try {
            ...
            if (\is_string($value) && !\is_numeric($value)) {
                if (!$value) {
                    // No way to detect the intended format if the given value is empty.
                    return '';
                }
