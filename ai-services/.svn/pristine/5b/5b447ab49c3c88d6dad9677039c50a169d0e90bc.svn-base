...
class Aggregate_REST_Route implements REST_Route
{
    ...
    {
        if (!$route_handlers) {
            throw new InvalidArgumentException(esc_html__('No route handlers provided for REST route.', 'ai-services'));
        }
        foreach ($route_handlers as $route_handler) {
            if (!$route_handler instanceof REST_Route) {
                throw new InvalidArgumentException(esc_html__('Invalid route handler provided for REST route.', 'ai-services'));
            }
            ...
            if ($this->base && $new_base !== $this->base) {
                throw new InvalidArgumentException(esc_html(\sprintf(
                    ...
                    __('Inconsistent REST bases %1$s and %2$s in route handlers provided for REST route.', 'ai-services'),
    ...
    {
        return \array_merge($this->global_args, \array_map(static function ($route) {
