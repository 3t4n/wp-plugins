 ...
 */
namespace Felix_Arntz\AI_Services_Dependencies\Felix_Arntz\WP_OOP_Plugin_Lib\General\Exception;
...
class WP_Error_Exception extends RuntimeException
{
    ...
    public function get_error_code() : string
    {
        if (!$this->error_code) {
            ...
            return \preg_replace('/[^a-z0-9_]+/', '', \str_replace(array(' ', '-'), '_', \strtolower($this->getMessage())));
        }
