...
class Term_Repository implements Entity_Repository, Cache_Aware_Entity_Repository
{
    ...
    public function update(int $id, array $data) : bool
    {
        ...
        if (!isset($data['taxonomy'])) {
            ...
            if (!$term || is_wp_error($term)) {
                if (WP_DEBUG) {
                    throw new Invalid_Entity_Data_Exception(esc_html__('Missing taxonomy.', 'ai-services'));
                }
                return \false;
            }
    ...
    public function add(array $data) : int
    {
        if (!isset($data['name'])) {
            if (WP_DEBUG) {
                throw new Invalid_Entity_Data_Exception(esc_html__('Missing name.', 'ai-services'));
            }
            return 0;
        }
        if (!isset($data['taxonomy'])) {
