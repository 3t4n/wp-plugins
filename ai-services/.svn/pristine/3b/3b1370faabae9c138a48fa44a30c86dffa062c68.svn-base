...
class Input
{
    ...
    public function __construct()
    {
        ...
        $this->fallback_map = array(\INPUT_ENV => $_ENV, \INPUT_SERVER => $_SERVER);
    }
    /**
    ...
    public function filter($type, $variable_name, $filter = \FILTER_DEFAULT, $options = 0)
    {
         ...
         */
        if (isset($this->fallback_map[$type]) && \in_array($value, array(null, \false), \true) && \array_key_exists($variable_name, $this->fallback_map[$type])) {
