...
use Felix_Arntz\AI_Services_Dependencies\Felix_Arntz\WP_OOP_Plugin_Lib\HTTP\Exception\Multiple_Requests_Exception;
use Felix_Arntz\AI_Services_Dependencies\Felix_Arntz\WP_OOP_Plugin_Lib\HTTP\Exception\Request_Exception;
use Felix_Arntz\AI_Services_Dependencies\Felix_Arntz\WP_OOP_Plugin_Lib\HTTP\Traits\Sanitize_Headers;
...
class HTTP
{
    ...
    public function request_multiple(array $requests) : array
    {
        ...
        foreach ($requests as $request) {
            if (!$request instanceof Request) {
                throw new InvalidArgumentException(esc_html__('Invalid request provided.', 'ai-services'));
            }
        }
        $requests_args = array();
        $responses = array();
        foreach ($requests as $key => $request) {
            ...
            $request_args['options'] = $this->prepare_options_for_requests($request_args['options'], $request_args['url'], $request_args['type']);
        ...
        // Any pre-filter responses are by definition success responses.
        $failed = array();
    ...
    protected function build_request_args(Request $request) : array
    {
        ...
        $request_args = array('url' => $request->get_url(), 'type' => $request->get_method(), 'options' => wp_parse_args($request->get_options(), $this->default_options));
        if ($headers) {
        ...
        $request_args['options'] = $this->merge_wp_default_options($request_args['options'], $request_args['url'], $request_args['type']);
        return $request_args;
    }
    /**
    ...
    private function run_wp_pre_http_request_filter(array $request_args)
    {
        ...
        if (\false !== $pre) {
            return $this->create_response($pre['response']['code'] ?? 200, $pre['body'] ?? '', $pre['headers'] ?? array());
        }
        return null;
    }
    /**
    ...
    {
        if (isset($headers['content-type']) && \in_array('application/json', \array_map('trim', \explode(';', $headers['content-type'])), \true)) {
    ...
    private function merge_wp_default_options(array $options, string $url, string $method) : array
    {
        ...
        $defaults = array('timeout' => apply_filters('http_request_timeout', 5, $url), 'redirection' => apply_filters('http_request_redirection_count', 5, $url), 'user-agent' => apply_filters('http_headers_useragent', $wp_user_agent, $url), 'sslverify' => \true, 'sslcertificates' => ABSPATH . WPINC . '/certificates/ca-bundle.crt', 'stream' => \false, 'filename' => null, 'limit_response_size' => null);
        if ('HEAD' === $method) {
    ...
    private function prepare_options_for_requests(array $options, string $url, string $method) : array
    {
        ...
        if ($proxy->is_enabled() && $proxy->send_through_proxy($url)) {
            ...
            if ($proxy->use_authentication()) {
                $options['proxy']->use_authentication = \true;
    ...
    private function migrate_wp_options_to_requests_options(array $options) : array
    {
        ...
        unset($options['limit_response_size'], $options['redirection'], $options['sslverify'], $options['sslcertificates'], $options['stream'], $options['user-agent']);
        return $options;
    }
}
