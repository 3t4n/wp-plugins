...
class Option_Container implements Container, ArrayAccess
{
    ...
    public function get(string $key)
    {
        if (!isset($this->options[$key])) {
            throw new Not_Found_Exception(esc_html(\sprintf(
                ...
                __('Option with key %s was not found in container', 'ai-services'),
                $key
            )));
        }
        if (!isset($this->instances[$key])) {
            ...
            if (!$instance instanceof Option) {
                throw new Invalid_Type_Exception(esc_html(\sprintf(
                    /* translators: %s: option key */
                    __('Option with key %s is not of type Option', 'ai-services'),
                    $key
                )));
            }
    ...
    public function set_by_args(string $key, Key_Value_Repository $repository, array $registration_args = array()) : void
    {
        ...
        $this->set($key, function () use($repository, $key, $registration_args) {
