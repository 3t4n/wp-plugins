...
class April_Payment_Action_Controller {
	 ...
	 */
	protected function get_april_gateway( $methodId ) {
		if ( empty( $methodId ) ) {
			$methodId = April::ID;
		}
	...
	protected function get_order_from_request() {
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( sanitize_key( $_GET['nonce'] ), 'wc_april_payment_action' ) ) {
			throw new Exception( esc_html( __( 'CSRF verification failed.', 'april' ) ) );
		}
		...
		if ( ! $order ) {
			throw new Exception( esc_html( __( 'Missing order ID for payment confirmation', 'april' ) ) );
		}
	...
	public function handle_payment_action_completion() {
		...
		$april_gateway = $this->get_april_gateway( $methodId );
	...
	protected function handle_error( $message ) {
		$error_message = sprintf( __( 'Error: %s', 'april' ), $message );
