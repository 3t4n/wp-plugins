...
class April extends WC_Payment_Gateway {
  ...
  use WC_April_Subscriptions_Trait;
	use WC_April_Order_Pay_Trait;
  ...
  function __construct() {
    ...
    $this->method_title = __( 'April', 'april' );
    $this->method_description = __( 'Let customer pay in full or with flexible instalments. No third-party branding.', 'april' );
    $this->description = ' ';
  ...
  protected function get_error_message($resp, $alt_message) {
    ...
    try {
      ...
      return __( 'Unknown error occured', 'april' );
    }
  ...
  public function payment_fields() {
    ...
    echo 'var wc_' . esc_js( $this->id ) . '_params_amount = ' . esc_js( strval(April_Helper::convert_to_cents($this->get_order_total())) ) . ';';
    echo 'var wc_' . esc_js( $this->id ) . '_params_currency = "' . esc_js( strval(get_woocommerce_currency()) ) . '";';
    echo '</script>';
  ...
  public function process_payment( $order_id ) {
    ...
    $order = new WC_Order( $order_id );
    $payment_token = array_key_exists( $this->id . '_payment_token', $_POST ) ? April_Helper::validate_payment_token( $_POST[$this->id . '_payment_token'] ) : null;
    $payment_data = array_key_exists( $this->id . '_payment_data', $_POST ) ? April_Helper::validate_payment_data_string( $_POST[$this->id . '_payment_data'] ) : null;
    $payment_source = array_key_exists( $this->id . '_payment_source', $_POST ) ? April_Helper::validate_payment_source( $_POST[$this->id . '_payment_source'] ) : null;

    if ( ! $this->secret_key ) {
      $msg = esc_html__( 'April secret key is not provided.', 'april' );
      April_Helper::write_log( $msg );
      throw new Exception( $msg );
    }

    if ( ! $payment_token && ! $payment_source ) {
      $msg = esc_html__( 'Transaction incomplete. No payment token or payment source found.', 'april' );
      April_Helper::write_log( $msg );
      throw new Exception( $msg );
    }
    ...
    if ( $has_subscriptions && !$payment_source ) {
      $msg = esc_html__( 'No payment source found.', 'april' );
      April_Helper::write_log( $msg );
      throw new Exception( $msg );
    }
    ...
    try {
      if ( $payment_token ) {
        $payment_token_data = $this->query_payment_token( $payment_token );
        $payment_data = $payment_token_data['method'];
      }
      ...
      if ( !empty( $payment_source ) ) {
        $payment_token_data = $this->generate_payment_token( $payment_source, $this->get_order_total(), get_woocommerce_currency() );
        $payment_token = $payment_token_data['paymentTokenId'];
        $payment_data = $payment_token_data['SavedPaymentSourceToken']['savedPaymentSource'];
      }
  ...
  protected function pay_for_april_order( $april_merchant_order_id, $payment_token, $payment_data = null, $payment_action = false, $request_3ds = false ) {
    ...
    } else if ( !is_null( $payment_data ) && ( array_key_exists('TokenSavedPaymentSourceCard', $payment_data) || array_key_exists('CardTokenDetails', $payment_data) )) {
      $pay_data = [
        'PayByCard' => [
          'paymentTokenId' => $payment_token,
    ...
    /**
	 * Fetch payment token data from April
	 *
   * @param string  $payment_token April payment token
	 * @throws Exception If failed to fetch payment token data
	 * @return array
	 */
  protected function query_payment_token( $payment_token ) {

    $payment_token_resp = April_API::request( [], 'tokens/' . $payment_token, 'GET' );
    $payment_token_data = json_decode( $payment_token_resp['body'], true );

    April_Helper::write_log( 'Fetching payment token data: April Token ' . $payment_token );

    if ($payment_token_resp['response']['code'] >= 200 && $payment_token_resp['response']['code'] <= 299) {
      return $payment_token_data;

    } elseif ( $payment_token_resp['response']['code'] >= 400 && $payment_token_resp['response']['code'] <= 499 ) {
      $error_msg = $this->get_error_message( $payment_token_resp, "An unknown error occurred while fetching the payment token data from April");
      throw new Exception( esc_html( $error_msg ) );

    } else {
      throw new Exception( 'Failed to fetch payment token data' );
    }
  }
  ...
  protected function complete_order( $order, $pay_data ) {
    ...
    $order->add_order_note( __( 'April payment completed.', 'april' ) );
  ...
  public function do_ssl_check() {
    if( $this->enabled == 'yes' ) {
      if (!wc_checkout_is_https()) {
        echo "<div class=\"error\"><p>". esc_html( sprintf( __( "<strong>%s</strong>: SSL certificate is not detected. Please ensure that you have a valid SSL certificate and that you are <a href=\"%s\">forcing the checkout pages to be secured.</a>" ), $this->method_title, admin_url( 'admin.php?page=wc-settings&tab=checkout' ) ) ) ."</p></div>";
      }
    }
  }
  ...
  public function generate_payment_token( $payment_source, $amount, $currency ) {
    ...
    try {
      ...
      return $resp_data['SavedPaymentSourceToken'];
    } catch( Exception $e ) {
      $error_msg = $e->getMessage();
      throw new Exception( esc_html( $error_msg ) );
    }
