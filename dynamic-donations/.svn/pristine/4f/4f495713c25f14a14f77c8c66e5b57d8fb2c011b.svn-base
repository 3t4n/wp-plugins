...
export default function ScreenThanks() {
	...
	useEffect(() => {
		if (global.settings.donations_url_type === 'page') {
			setYourDonationURL(`${location.origin}?p=${global.settings.donations_page}`);
		}
		...
		<>
			<Content>
				<h6 className="dydo_thanks__paragraph">
					{`${dydo_texts.screens.thanks.paragraph} `}
					<a href={yourDonationURL}>
						<strong>{dydo_texts.screens.thanks.link}</strong>
					</a>
				</h6>
			</Content>
			<MainButton title={dydo_texts.screens.thanks.done} onClick={handleSubmit} />
		</>
	);
};
