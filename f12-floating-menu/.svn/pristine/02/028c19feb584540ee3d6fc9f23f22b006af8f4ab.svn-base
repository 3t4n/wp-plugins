...
namespace forge12\floating_menu {
    ...
    class HTMLSelectOption
    {
        ...
        public function render()
        {
            if ($this->isSelected()) {
                ?>
                <option value="<?php esc_attr_e($this->getKey()); ?>"
                        selected="selected"><?php esc_html_e($this->getValue()); ?></option>
                <?php
            } else {
                ?>
                <option value="<?php esc_attr_e($this->getKey()); ?>"><?php esc_html_e($this->getValue()); ?></option>';
                <?php
            }
        }
