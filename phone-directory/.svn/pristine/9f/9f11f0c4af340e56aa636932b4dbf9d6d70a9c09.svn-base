...
echo do_shortcode('[qcpnd-directory mode="' . esc_attr($mode) .  '" list_id="' . esc_attr($list_id) . '" style="' . esc_attr($style) . '" column="' . esc_attr($column ). '" search="' . esc_attr($search) . '" category="' . esc_attr($category) . '" upvote="' . esc_attr($upvote) . '" item_count="on" orderby="date" order="' . esc_attr( $order) . '"][/qcpnd-directory]'); 
