...
if ( $list_query->have_posts() ) 
{
	...
	echo '<div class="qcld-main-style-simple"><div class="qcpnd-list-wrapper" '.$pd_enable_rtl.'>
			...
			<div class="qcpnd-single-list">
				...
				<ul>
					...
					<div class="qcld-custom-simple-style-outer">
						...
						?>
						<!-- List Anchor -->
						<a class="qcld-custom-simple-style-inn" <?php if( $mask_url == 'on') { echo 'onclick="document.location.href = \''.$item_url.'\'; return false;"'; } ?>
							...
							<div class="qcld-sbd-subcontent" style="<?php echo (!isset($list['show_phone_number'])|| $list['show_phone_number']==0)?'position: relative;top: -5px;':'' ?>">
							<!-- Link Text -->
							<h3><?php echo esc_html($list['qcpnd_item_title']); ?></h3>
							<?php 
								if(isset($list['qcpnd_item_description']) and $list['qcpnd_item_description']!=''){
									echo '<span style="display:block;font-size:14px">'.esc_html($list['qcpnd_item_description']).'</span>';
								}
							?>
							<?php if(isset($list['qcpnd_item_phone'])&&$list['qcpnd_item_phone']!='' && isset($list['show_phone_number']) && $list['show_phone_number']==1){
							
								echo '<span class="qcld-phone-address"> <span class="qcld-phone-ic"><i class="fa fa-phone"></i> '.esc_html($list['qcpnd_item_phone']).'</span>';
																									

								if(isset($list['qcpnd_item_subtitle']) and $list['qcpnd_item_subtitle']!=''){
									echo '<span class="qcld-phone-loc"><i class="fa fa-location-arrow"></i> '.esc_html($list['qcpnd_item_subtitle']).'</span>';
								}
								echo '</span>';
