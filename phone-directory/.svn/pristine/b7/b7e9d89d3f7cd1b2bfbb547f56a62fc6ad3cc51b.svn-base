<?php
global $wpdb;
wp_enqueue_style( 'qcpnd-style-2', OCPD_TPL_URL . "/$template_code/template.css");
wp_enqueue_style( 'qcpnd-style-2-fa', OCPD_TPL_URL . "/$template_code/css/font-awesome.min.css");
// The Loop
if ( $list_query->have_posts() ) 
{
	...
	echo '<div class="qcld-main-style-2"><div class="qcpnd-list-wrapper" '.$pd_enable_rtl.'><div id="opd-list-holder" class="qcpnd-list-holder">';
	...
	while ( $list_query->have_posts() ) 
	{
		...
		<div id="qcpnd-list-<?php echo esc_attr($listId) .'-'. esc_attr(get_the_ID()); ?>" class="<?php echo esc_attr($style); ?>">

			<div class="qcpnd-single-list-1">
				<h3>
					<?php echo esc_html(get_the_title()); ?>
				</h3>
				<ul class="ca-menu column<?php echo esc_attr($column); ?>">
					<?php $count = 1; ?>
					...
					?>
					<li id="item-<?php echo esc_attr(get_the_ID()) ."-". esc_attr($count); ?>" class="style-2-column<?php echo esc_attr($column); ?>" data-latlon="<?php echo esc_attr($latlon); ?>" data-title="<?php echo esc_html($list['qcpnd_item_title']); ?>" data-phone="<?php echo esc_html($list['qcpnd_item_phone']); ?>" data-address="<?php echo esc_html($list['qcpd_item_full_address']); ?>" data-url="<?php echo esc_url($list['qcpnd_item_link']); ?>">
						<?php 
						...
						<div class="pd-bottom-area">
							<?php if( $show_phone_icon == '1' ) : ?>
							   <p><a class="" href="tel:<?php echo preg_replace("/[^0-9]/", "",@esc_html($list['qcpnd_item_phone'])); ?>">
								 <i class="fa fa-phone"></i>
							   </a></p>
							<?php endif; ?>
							
							<?php if( $masked_url != '' && $show_link_icon==1) : ?>
							   <p><a class="" href="<?php echo $masked_url; ?>" <?php echo (isset($list['qcpnd_open_new_window']) && $list['qcpnd_open_new_window'] == 1) ? 'target="_blank"' : ''; ?>>
