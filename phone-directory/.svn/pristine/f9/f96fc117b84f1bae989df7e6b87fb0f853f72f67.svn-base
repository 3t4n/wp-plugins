...
if ( $list_query->have_posts() ) 
{
	...
	echo '<div class="qcld-main-style-3"><div class="qcpnd-list-wrapper" '.$pd_enable_rtl.'><div id="opd-list-holder" class="qc-grid qcpnd-list-holder">';
			...
			<div class="qcpnd-single-list-1">
				...
				<ul class="ca-menu column<?php echo esc_attr($column); ?>">
					...
					<li id="item-<?php echo get_the_ID() ."-". $count; ?>" data-latlon="<?php echo $latlon; ?>" data-title="<?php echo esc_html($list['qcpnd_item_title']); ?>" data-phone="<?php echo esc_html($list['qcpnd_item_phone']); ?>" data-address="<?php echo esc_html($list['qcpd_item_full_address']); ?>" data-url="<?php echo esc_url($list['qcpnd_item_link']); ?>">
						...
						?>
						<!-- List Anchor -->
						<a class="qcld-custom-column" <?php echo (isset($list['qcpnd_item_gotoweb']) && $list['qcpnd_item_gotoweb'] == 1) ? 'href="'.$masked_url.'" target="_blank"' : ''; ?> <?php echo (isset($list['qcpnd_item_gotoweb']) && $list['qcpnd_item_gotoweb'] == 0) ? 'href="tel:'.preg_replace("/[^0-9]/", "",$list['qcpnd_item_phone']).'"' : ''; ?> >
						...
						<?php 
								if(isset($list['qcpnd_item_description']) and $list['qcpnd_item_description']!=''){
									echo ' <span style="display:block; text-align: center;"> '.esc_html($list['qcpnd_item_description']).'</span>';
								} ?>
...
<div class="qcld-sld-content-dividar visible"></div>
						...
						<?php
								if(isset($list['qcpnd_item_phone'])&&$list['qcpnd_item_phone']!='' && isset($list['show_phone_number']) && $list['show_phone_number']==1){
									echo '<span><i class="fa fa-phone"></i> '.str_replace(array('(',')'),array('',''),esc_html($list['qcpnd_item_phone'])).'</span>';
								}
								if(isset($list['qcpnd_item_subtitle']) and $list['qcpnd_item_subtitle']!=''){
									echo ' <span> <i class="fa fa-location-arrow"></i> '.esc_html($list['qcpnd_item_subtitle']).'</span>';
								}
