...
jQuery(document).ready(function($){
	...
	$('#pnd_width_div').css('display', 'none');
	//sbd_list_elements

	var sbd_list_elements = $('#qcpnd_list_item01 .field-item[data-class="CMB_Group_Field"]').not(':hidden').length;
	console.log(sbd_list_elements);
	$('.qcld_sbd_alert_msg_modal').css({'display': 'none'});
	if ( sbd_list_elements === 1  ) {
        $('.qcld_sbd_alert_msg_modal').css({'display': 'block'});
    }

	$(document).on( 'click', '.sbd_alert_msg_close', function(){
        $('.qcld_sbd_alert_msg_modal').css({'display': 'none'});
    })

	$(document).on( 'click', '.sbd_add_more_item', function(){
        $('.qcld_sbd_alert_msg_modal').css({'display': 'none'});
        var scroll_top = $("body").get(0).scrollHeight;
        var scroll_tops = $(".sbd_list_elements").get(0).scrollHeight;
        $("html, body").animate({ scrollTop: scroll_top - scroll_tops }, 1500);
        $('.sbd_list_elements .button.repeat-field').trigger('click');
    })
    ...
    $(document).on( 'click', '.modal-content .close', function(){
	  ...
	  var shortcodedata = '[qcpnd-directory';
		  ...
		  if( pnd_width !== '' ){
			  shortcodedata +=' min_width="'+pnd_width+'px"';
		  }
	...
	}).on( 'change', '#pnd_column',function(){
	
		var pnd_column = $('#pnd_column').val();
		
		if( pnd_column == 3 || pnd_column == 4 ){
			$('#pnd_width_div').css('display', 'block');
		}
		else{
			$('#pnd_width_div').css('display', 'none');
		}
