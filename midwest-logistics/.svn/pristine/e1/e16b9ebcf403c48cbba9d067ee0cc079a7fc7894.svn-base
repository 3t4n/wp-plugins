<?php
use Automattic\WooCommerce\Internal\DataStores\Orders\CustomOrdersTableController; // at the beginning of the file
/*
 ...
 * @author mburton
 * 
 * When it comes to the status of the order there are three statuses possible.
 * 1 = do not send to Midwest
 * 2 = Send to Midwest.
 * 3 = Sent to Midwest
 */
class ML_Order {
    ...
    public function __construct() {
        ...
        //Used in Orders with errors and on the order screen to process orders.
        add_action( 'wp_ajax_midwest_logistics_process_shop_order',array($this, 'midwest_logistics_admin_ajax_process_order'),100 ); 
        //Used on the order screen to cancel orders.
        add_action( 'wp_ajax_midwest_logistics_cancel_shop_order',array($this, 'midwest_logistics_admin_ajax_cancel_order'),100 );
        ...
        add_action( 'add_meta_boxes',array($this, 'add_midwest_meta_box'),10,1 );               
        
    }  
    function midwest_logistics_process_order($orderId,$ignoreStatus = false) {
        ...
        if($default_shipping === "" || $default_shipping == "5") { //5 is old shipping
        ...
        if($ignoreStatus == true) {
            //when submitting the order manually via the interface we don't care that it is in the correct status. 
            $midwest_logistics_order_status_setting = "wc-" . $order->get_status();
        }
                        ...
                        if($curl_result === "200") {
                            ...
                            if(is_array($CRMorder)) {
                                ...
                                if($CRMorderId !== "0" && $CRMorderId != "") {
                                    ...
                                    $note = "Fulfillment order accepted by Midwest Logistics. Order# " . $CRMorderId;
    ...
    */
    ...
    function midwest_logistics_admin_ajax_process_order() {
        if(isset($_POST['order'])) {
            ...
            //mark it to be sent 
            $order->update_meta_data('_midwest_logistics_CRM_order_status',"2");
            $order->save_meta_data();
            $this->midwest_logistics_process_order($postedOrder,true);
    ...
    function midwest_logistics_admin_ajax_cancel_order() {
        if(isset($_POST['order'])) {
            $response = array("code"=>200,"message"=>"");

            $postedOrder = filter_input(INPUT_POST, "order");
            if($postedOrder == false) {
                $response = array(
                    "code"=>500,
            ...
            if($currentCRMId == "") {
                $response = array(
                   "code"=>500,
                   "message"=>"No Midwest Logistics order found."
                ); 
                echo json_encode($response);
                exit();
            }
            ...
            $settingOptions = get_option('Midwest_Logistics_settings','');
            $midwest_logistics_api_key = $settingOptions["Midwest_Logistics_API_Key_field_0"];
            if($midwest_logistics_api_key == "") {
                 $response = array(
                   "code"=>500,
                   "message"=>"Invalid Midwest Logistics API Key."
                ); 
                echo json_encode($response);
                exit();
            }
            ...
            if($httpcode == "200") {
                if(json_decode($response) !== null) {
                    ...
                    if($curl_result === "200") {
                        $CRMorder = $jsonResponse -> {"orders"};
                        if(is_array($CRMorder)) {
                            $apireturnedOrder = $CRMorder[0];
                            $CRMorderId = $apireturnedOrder -> CRMOrderId;
                            $responseStatus = $apireturnedOrder -> status;
                            
                            //if the order was not found then we are good.
                            if($apireturnedOrder->message == "") {
                                $responseStatus = "200";
                            }
                            if(strtolower($apireturnedOrder->message) == "order not found.") {
                                $responseStatus = "200";
                            }
                            
                            if($responseStatus == "200") {
                                $order->update_meta_data('_midwest_logistics_CRM_order_id', "" );
                                $order->update_meta_data('_midwest_logistics_CRM_order_status',"1");
                                $order->save_meta_data();
                                $code = 200;
                                
                                $note = "Midwest Logistics Fulfillment order canceled.";
                                $order->add_order_note($note); 
                            } else {
                                $message = "Cannot Cancel order. Api says: " . $apireturnedOrder->message;
                                
                                $note = "Midwest Logistics Fulfillment order cannot be canceled. Message: " . $apireturnedOrder->message;
                                $order->add_order_note($note); 
                            }
            ...
            //add the log
            midwest_logistics_add_communication_log($postString,$response,$order ->get_order_number(),"order",$currentCRMId,$responseText);
    ...
    function add_midwest_meta_box() {
        
        $screen = class_exists( '\Automattic\WooCommerce\Internal\DataStores\Orders\CustomOrdersTableController' ) && wc_get_container()->get( CustomOrdersTableController::class )->custom_orders_table_usage_is_enabled()
        ? wc_get_page_screen_id( 'shop-order' )
        : 'shop_order';

        //add_meta_box( 'midwest_order_meta_box', 'Midwest Logistics', array($this, 'output_meta_box'),"woocommerce_page_wc-orders","side");
        add_meta_box( 'midwest_order_meta_box', 'Midwest Logistics', array($this, 'output_meta_box'),$screen,"side");
    }
    function output_meta_box($post) {
        ...
        ?>
        <p class="form-field form-field-wide">            
            <strong><?php _e("Order Id")?>:</strong> <?php echo $currentCRMId ?>
        </p>
        <p class="form-field form-field-wide">   
            <strong><?php _e("Status")?>:</strong> <?php _e($status)?>
        </p>
        <?php 
        ...
        if($currentCRMId != "") {
            ?>
            <p class="form-field form-field-wide"> 
                <button type="button" data-order="<?php echo $order->get_id(); ?>" class="midwest-cancel-order-button button button-secondary"><?php _e("Cancel Order")?></button>
            </p>
            <?php 
        } else {
            ?>
            <p class="form-field form-field-wide"> 
                <button type="button" data-order="<?php echo $order->get_id(); ?>" class="midwest-send-order-button button button-secondary"><?php _e("Send Order")?></button>
            </p>
            <?php  
        }
    }
