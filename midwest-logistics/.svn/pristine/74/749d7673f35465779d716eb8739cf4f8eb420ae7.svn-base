...
function midwest_logisitcs_verify_sku_number($SKU) {
    ...
    $responseText = "";
    $responseArray =  array("result" => "500", "message" => "No response from API");               
    if($httpcode == "200") {
        if(json_decode($response) !== null) {
            $jsonResponse = json_decode($response);
            $curl_result = $jsonResponse -> {"result"};     
            $responseText = $jsonResponse -> {"message"};
            $responseArray =  array("result" => $curl_result, "message" => $jsonResponse -> {"message"});                   
        } else {
    ...
    //add the log
    midwest_logistics_add_communication_log($postString,$logResponse,get_the_ID(),"product",get_the_title(),$responseText);
