...
class ML_API {
    ...
    function send($data) {
        ...
        if (!empty( $settingOptions ) ) {
            if(isset($settingOptions['Midwest_Logistics_api_timeout_setting'])) {
                $apiTimeout = $settingOptions['Midwest_Logistics_api_timeout_setting'];
            }
        }
        if(!is_numeric($apiTimeout)) {
            $apiTimeout = "60";
        }
        ...
        if(is_wp_error($wp_remote_post_response)) {
            ...
            $response["response"] = $wp_remote_post_response->get_error_message($response["code"]);
        } else {
