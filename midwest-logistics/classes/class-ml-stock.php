...
class ML_Stock {
    ...
    public function __construct() {
        ...
        add_action( 'add_meta_boxes',array($this, 'add_product_meta_box'),10,1 );
        add_action( 'wp_ajax_midwest_logistics_process_product_update_stock',array($this, 'admin_ajax_process_product_update_stock'),100 ); 
    }      
    ...
    public function add_product_meta_box() {
        ...
        add_meta_box(
            'midwest_product_meta_box', // Unique ID
            'Midwest Logistics', // Box title
            array($this,'display_product_meta_box'), // Content callback
            'product', // Post type
            'side'
        );
    ...
    public function display_product_meta_box($post) {
        ...
        <p class="form-field form-field-wide"> 
            <p class="form-field form-field-wide">   
                <strong><?php _e("Pull current inventory")?>:</strong> 
            </p>
            <button type="button" data-product="<?php echo $product->get_id(); ?>" class="midwest-update-product-button button button-secondary"><?php _e("Pull Inventory")?></button>
        </p>
        <?php 
    ...
    public function admin_ajax_process_product_update_stock() {
        
        $errormsg = "";
        $code = 200;
        if(isset($_POST['pid'])) {
            
            $postedProduct = $_POST['pid'];
            if(!is_numeric($postedProduct)) {
                echo json_encode($response);
                exit();
            }
            //get the list of skus to be updated.
            $args = array(
                'status' => 'publish', 
                'limit' => 1,
                'include' => array((int) $postedProduct),
                'type' => array('simple','variable'),
            );
                    ...
                    if(json_decode($response,true) !== null) {
                        ...
                        if(is_array($products)) {
                            foreach($products as $product) {
                                ...
                                if($result == 500) {
                                    $code = 500;
                                    $errormsg = $errormsg . "SKU: " .$sku . " is Invalid\n";
                                }
                                if(is_numeric($stockAmount) ) {   
                                    if(isset($skusArray[$sku])) {
                                        $productId = $skusArray[$sku]["id"];
                                        ...
                                        $responseText = array(
                                            "result" => "200",
                                            "response" => true,                                        
                                            "message" => "Product Stock Updated Manually",
                                            "stock" => $stockAmount,
                                            "instock" => $inStock,
                                        );
            ...
            if($errormsg != "") {
                $errormsg = "Errors occured while updating inventory. Please review and fix the following error(s)\n" . $errormsg;
            }
            $response = array("code"=>$code,"message"=>$errormsg);
        }
