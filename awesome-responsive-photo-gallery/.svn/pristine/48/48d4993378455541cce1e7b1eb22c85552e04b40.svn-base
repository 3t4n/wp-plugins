...
( function() {
    "use strict";
var defaults = {
    autostart: false, // Boolean; If set as 'true' jGallery will be started automatically after loading the document(only for full-screen or standard mode).; [ true, false ]
    ...
    canChangeMode: false, // Boolean; If set as 'true' you can change display mode(only for full-screen or standard mode).; [ true, false ]
    ...
    mode: 'full-screen', // String; Display mode.; [ 'full-screen', 'standard', 'slider' ]
    ...
    morer: false, // Boolean; If set as 'true', view more will be shown from data-href attribute of img.; [ true, false ]
...
var Thumb = (function() {
    ...
    Thumb.prototype = {
        generateImgTag: function( img ) {
            ...
            if ( img.dtHref ) {
                $newImg.attr( 'data-href', img.dtHref );
            }
            if ( img.bgColor ) {
...
var Zoom = ( function( jLoader, overlay, historyPushState, jGalleryTransitions, jGalleryArrayTransitions, jGalleryBackwardTransitions, AdvancedAnimation, IconChangeAlbum ) {
    ...
    var Zoom = function( jGallery ) {
        ...
        this.$morer = this.$container.find( '.nav-bottom > .icons > .morer' );
    ...
    Zoom.prototype = {
        ...
        showPhotoSuccess: function( $a, $imgThumb, options ) {
            ...
            if ( this.jGallery.options.morer && $imgThumb.attr( 'data-href' ) ) {
				if($imgThumb.attr( 'data-tab' ) == 'true') {
	                this.$morer.html('<a class="fa fa-link" href="' + $imgThumb.attr( 'data-href' ) + '" target="_blank"></a>').removeClass( 'before' ).removeClass( 'after' );
				} else {
					this.$morer.html('<a class="fa fa-link" href="' + $imgThumb.attr( 'data-href' ) + '"></a>').removeClass( 'before' ).removeClass( 'after' );
				}
                this.jGallery.$element.addClass( 'has-morer' );
            }
            else {
                this.jGallery.$element.removeClass( 'has-morer' );
            }
            this.setJGalleryColoursForActiveThumb();
...
var JGallery = ( function( outerHtml, historyPushState, isInternetExplorer, isInternetExplorer8AndOlder, refreshHTMLClasses, defaults, defaultsFullScreenMode, defaultsSliderMode, requiredFullScreenMode, requiredSliderMode, IconChangeAlbum, Progress, Thumbnails, ThumbnailsGenerator, Zoom ) {
    ...
    JGallery.prototype = {
                        ...
                        <div class="zoom-container">\
                            ...
                            <div class="nav-bottom">\
                                <div class="icons">\
                                    ...
                                    <span class="morer jgallery-btn jgallery-btn-small"></span>\
                                </div>\
        ...
        show: function() {
            ...
            this.zoom.$morer.removeClass( 'hidden' );  
        ...
        hide: function( options ) {
            ...
            this.zoom.$morer.addClass( 'hidden' );
        ...
        setUserOptions: function() {
			...
			this.options.morer ? this.zoom.$morer.show(): this.zoom.$morer.hide();
        ...
        generateHtml: function( options ) {
            ...
            this.template.done( function( html ) {   
                ( function() {
                    ...
                    self.$jgallery.find( '.morer' ).attr( 'tooltip', options.tooltipMorer );
...
(function($) {
	'use strict';
	var $filters = $('.j_gallery_filter [data-filter]'),
	$boxes = $('.j_gallery [data-category]');
...
/*(function($) {
	'use strict';
	var container = document.querySelector('.j_gallery');
	var msnry;
