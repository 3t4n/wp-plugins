...
function CPIW_PincodeCheckInDataTable($pincode){
    ...
    // Initialize the where clause and params array
    $results = $wpdb->get_results(
        $wpdb->prepare(
            "SELECT * FROM {$tablename} WHERE pincode = %s",
            $pincode
        ),
        OBJECT
    );
...
function CPIW_CheckPincodeSingleProduct(){
        ...
        $avai_msg = '';  
         ...
         if($totalrec == 1) {
            ...
            if($caseondilvery == 1) {
                $caseondilvery_avail = 'Cash On Delivery Available';
            } else {
                $caseondilvery_avail = 'Cash On Delivery Not Available';
            }

            if($showdate == "enable") {
                $avai_msg .= '<div class="cpiw_avaitxt"><span class="cpiw_delicons">';
                $avai_msg .= '<svg aria-hidden="true" width="30" height="30" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" style="vertical-align: text-bottom;">
            ...
            if($cpiw_cash_dilivery_shw == "enable"){
                $avai_msg .= '<div class="cpiw_dlvrytxt" style="color:'.$cpiw_comman['codtextcolor'].';"><span class="cpiw_tficon">';
                $avai_msg .= '<svg xmlns="http://www.w3.org/2000/svg" width="38" height="38" id="Layer_1" data-name="Layer 1" viewBox="0 0 24 24">
                ...
                $avai_msg .= '</span><div class="cpiw_avacod"><p>'.$caseondilvery_avail.'</p></div></div>';
            }
...
function CPIW_DeliveryDate($date){
    ...
    // Use gmdate instead of date to avoid issues with timezone changes
    $CPIWdeliverydate = gmdate('jS M', strtotime($string)); // Always in GMT
    $dayofweek = gmdate('D', strtotime($string)); // Always in GMT
