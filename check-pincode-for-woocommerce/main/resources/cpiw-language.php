...
function CPIW_load_textdomain() {
    load_plugin_textdomain( 'check-pincode-for-woocommerce', false, dirname( plugin_basename( CPIW_PLUGIN_FILE ) ) . '/languages' ); 
}
function CPIW_load_my_own_textdomain( $mofile, $domain ) {
    if ( 'check-pincode-for-woocommerce' === $domain && false !== strpos( $mofile, WP_LANG_DIR . '/plugins/' ) ) {
