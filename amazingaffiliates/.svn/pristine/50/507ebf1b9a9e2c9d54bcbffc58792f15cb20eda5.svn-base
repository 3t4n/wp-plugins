...
foreach($response_updated_product_batch as $response_updated_product ) {
    ...
    if( isset( $itemResult->ItemsResult->Items[$b]->Images->Primary->Large->URL )  ) {
        ...
        $batch_img_large_url = $itemResult->ItemsResult->Items[$b]->Images->Primary->Large->URL;
        
        update_post_meta( $batch_prod_id, 'primary_img_url', $batch_img_large_url );
    ...
    if( isset( $itemResult->ItemsResult->Items[$b]->Offers->Listings[0]->Price->Amount )  )	{
        ...
        if($batch_current_price != '') {
            $batch_current_price_purified = intval($batch_current_price*100)/100;
            update_post_meta( $batch_prod_id, 'current_price', $batch_current_price_purified);
        }
    ...
    if( isset( $itemResult->ItemsResult->Items[$b]->Offers->Listings[0]->SavingBasis->Amount )  ) {
        ...
        $batch_list_price_purified = intval($batch_list_price*100)/100;
        
        if( empty( $batch_list_price_purified) OR $batch_list_price_purified < $batch_current_price_purified )	{
            $batch_list_price_purified = $batch_current_price_purified; 
        }
        
        if($batch_list_price_purified != '') {
            update_post_meta( $batch_prod_id, 'list_price', $batch_list_price_purified);
        }
    ...
    elseif( str_contains(strtolower($batch_availability), 'out') OR $batch_availability == '' ) {
        $stock = 0;
    }
    else {
        $stock = 0.5;
