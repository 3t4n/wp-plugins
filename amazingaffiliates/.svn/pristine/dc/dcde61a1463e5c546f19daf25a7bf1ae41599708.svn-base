...
class AmazingAffiliates {
	 ...
	 * @since    1.1.0
	 */
 	protected function define_core_hooks() {
		...
		$this->loader->add_action( 'add_meta_boxes', $plugin_admin, 'add_custom_post_meta_metaboxes', 1 );
		...
		$this->loader->add_action( 'init', $plugin_public, 'register_custom_block_category' );
	...
	protected function define_admin_hooks() {
        ...
        // setup notice
        $this->loader->add_action( 'amazingaffiliates_setupnotice', $plugin_admin, 'setupnotice' );
		
		// menu page
		$this->loader->add_action( 'admin_menu', $plugin_admin, 'dashboard_page' );
		$this->loader->add_action( 'amazingaffiliates_dashboard', $plugin_admin, 'dashboard_content' );			
		
		// api setup wall
		if( (float) get_option('amazingaffiliates_setup_status') >= 0.75 ) {
    	...
    	//check if apis are already setupped
    	$this->loader->add_action( 'init', $plugin_admin, 'setupcheck' );
		...
		// activation setup page
        $this->loader->add_action( 'admin_menu', $plugin_admin, 'setup_page' );
        $this->loader->add_action( 'wp_ajax_test_api', $plugin_admin, 'test_api' );
