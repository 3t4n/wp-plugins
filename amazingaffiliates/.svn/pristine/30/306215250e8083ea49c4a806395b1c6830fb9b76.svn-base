...
<?php if( current_user_can( 'edit_posts' ) AND !is_preview() AND !is_single() AND $shortcoded == 0 AND ! $product_chosen  ) : ?>
	...
	<div class="amazing_product outerbox">
        ...
        <div class="searchbox">
            <button onclick="parent.searchClear();" >Clear</button>
            <textarea
            class="searchbar"
            placeholder="Search a product by its title, ASIN or product ID"
            onchange="parent.searchInput(this);" ><?php echo esc_html( $search ); ?></textarea>
            <button onclick="parent.searchGo();" >Search</button>
        </div>
        ...
        <div class="entrylist">
            <?php if(  $product->have_posts() ) : ?>
                <?php for($i=0;$i<4;$i++) : ?>
                    ...
                    <div class="entrybox" style="border-color:<?php echo esc_attr( $availability_color ); ?>" 
						 
						 onclick="
								  let prodId = <?php echo (int) esc_attr( $queried_product_id ); ?>;
								  parent.document.getElementById('productid').value = prodId;
								  parent.document.getElementById('productid').click();
								  "
						 
						 >
						<p style="display:none;">
							document.getElementById('productid').value = <?php echo esc_attr( $queried_product_id ); ?>; jQuery('#productid').click();
						</p>
...
//	$thumbnail   				= wp_get_attachment_image(get_post_thumbnail_id($product_id),"medium","",array( "class" => "product_img" ));
    $amazon_api_img             = get_post_meta($product_id, 'primary_img_url', true );
    $thumbnail                  = '<img width="300" height="300" style="object-fit:contain;" alt="' . wp_trim_words($title,7,'') . '" src="' . $amazon_api_img . '" >';
	if(empty($amazon_api_img)) $thumbnail = wp_get_attachment_image(get_post_meta($product_id, 'primary_img', true),"medium","",array( "class" => "product_img" ));
    ...
    <div class="amazing_product wrapper" style="<?php echo esc_attr( $wrapper_bg ); echo esc_attr( $wrapper_padding ); ?>">
        ...
        <div class="product_block" style="background-color:white;"><?php /* block */ ?>
            ...
            <div class="product_info">    
                ...
                <?php if( empty($show_price) AND ! empty( $current_price )  ) : ?>
                    ...
                    <?php if( ! empty($list_price) AND $list_price > $current_price ): ?>
                        <?php 
                        $current_price_prefix = get_option('amazingaffiliates_settings_product_block_offerprice_prefix');
                        if( empty($current_price_prefix) ) $current_price_prefix = __('Offer', 'amazingaffiliates');
                        $list_price_prefix = get_option('amazingaffiliates_settings_product_block_listprice_prefix');
                        if( empty($list_price_prefix) ) $list_price_prefix = __('List Price', 'amazingaffiliates');
                        ?>
                        <p class="product_list_price" ><?php echo esc_html( $list_price_prefix ); ?> : <span class="product_barred_price"><?php echo esc_html( $list_price ); ?> <?php echo esc_html( $currency ); ?></span></p>
                    <?php else: ?>
                        <?php
                        $current_price_prefix = get_option('amazingaffiliates_settings_product_block_currentprice_prefix');
                        if( empty($current_price_prefix) ) $current_price_prefix = __('New starting from', 'amazingaffiliates');
                        ?>
                    <?php endif; ?>
                    ...
                    <p class="product_current_price">
                        <?php echo esc_html( $current_price_prefix ); ?>: <span class="product_highlighted_price" ><?php echo esc_html( $exploded_price[0] ); ?>.<small><?php echo esc_html( $exploded_price[1] ); ?></small> <?php echo esc_html( $currency ); ?></span>
                    </p>
            ...
            <?php if( get_option('amazingaffiliates_settings_product_block_automatic_offer_banner_display') == 1 AND get_option('amazingaffiliates_settings_product_block_automatic_offer_banner_threshold') <= $discount AND $offer == 1 AND $stock >= 0.5 ): ?>
                ...
                <?php
                    $text = get_option('amazingaffiliates_settings_product_block_offerflip_default_text');
                    if(empty($text)) $text = __('Today on sale with a special price!', 'amazingaffiliates');
                    $text_alt = get_option('amazingaffiliates_settings_product_block_offerflip_default_text_alt');
                    if(empty($text_alt)) $text_alt = __('Take advantage of this special offer now!', 'amazingaffiliates');
                    $background_color = get_option('amazingaffiliates_settings_product_block_offerflip_default_background_color');
                    if(empty($background_color)) $background_color = '#ffff00';
                    $text_color = get_option('amazingaffiliates_settings_product_block_offerflip_default_text_color');
                    if(empty($text_color)) $text_color = '#000000';
                ?>
                
                <div class="offer_flip" style="background-color:<?php echo esc_attr( $background_color ); ?>;color:<?php echo esc_attr( $text_color ); ?>;">
                    <div class="offer_front" style="color:<?php echo esc_attr( $text_color ); ?>;">
                        <?php echo esc_html( $text ); ?>
                    </div>
                    <div class="offer_rear" style="color:<?php echo esc_attr( $text_color ); ?>;">
                        <?php echo esc_html( $text_alt ); ?>
                    </div>
                </div>
                
            <?php endif; ?>
            
            <?php do_action( 'amazing_product_banner' ); ?>
