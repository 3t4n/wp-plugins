...
/*
    Copyright (C) 2018 Crunchify, LLC.
...
add_option('as_facebook_mem_data_tabs', '');
...
add_option('as_facebook_mem_widget_data_tabs', '');
...
{
    if (strpos($content, "<!--social-friends-->") !== FALSE) {
        $content = preg_replace('/<p>\s*<!--(.*)-->\s*<\/p>/i', "<!--$1-->", $content);
        $content = str_replace('<!--social-friends-->', as_facebook_mem_likebox(), $content);
    }

    if (!($fm_appid == "")) {
        $content .= "<!-- Social Friends Plugin by Crunchify: https://crunchify.com/facebook-members/ -->
...
function as_facebook_mem_likebox()
{
    ...
    $fm_datatabs = get_option('as_facebook_mem_data_tabs');
    ...
    if ($show_sponser1 == 1) {
        ...
        $sponserlink_profile = '<div align="left">- <a href="https://crunchify.com/facebook-members/" title="Social Friends WordPress Plugin by Crunchify" target="_blank" style="color: #777;font-size: smaller;">' . 'Social Friends WordPress Plugin' . '</a></div>';
    }

    $T1 = '<iframe src="//www.facebook.com/plugins/page.php?href=https%3A%2F%2Fwww.facebook.com%2F' . $fm_pagename . '&tabs='. $fm_datatabs . '&amp;width=' . $fm_width . '&amp;height=' . $fm_height . '&amp;small_header=' . $small_header_post. '&amp;adapt_container_width=' . $adapt_container_width_post .'&amp;hide_cover=' . $hide_cover_post . '&amp;show_facepile=' . $show_facepile_post . '&amp;show_border=false&amp;header=' . $fm_header . '" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:' . $fm_width . 'px; height:' . $fm_height . 'px; margin:-1px;" allowTransparency="true"></iframe>';
...
// Displays Wordpress Blog Social Friends Options menu
function as_facebook_mem_add_option_page()
{
    if (function_exists('add_options_page')) {
        add_options_page('Social Friends', 'Social Friends', 8, __FILE__, 'as_facebook_mem_options_page');
    }
}
...
function as_facebook_mem_options_page()
{
    ...
    if (isset($_POST['info_update'])) {
        ...
        update_option('as_facebook_mem_data_tabs', (string)$_POST["as_facebook_mem_data_tabs"]);
        ...
        update_option('as_facebook_mem_data_tabs', (string)$_POST['as_facebook_mem_data_tabs']);
        update_option('as_facebook_mem_widget_data_tabs', (string)$_POST['as_facebook_mem_widget_data_tabs']);
...
function show_as_facebook_mem_likebox_widget($args)
{
    ...
    $fm_widget_datatabs = get_option('as_facebook_mem_widget_data_tabs');
...
function as_facebook_mem_likebox_widget_control()
{
    ?>
<p>
    <? _e("Please go to <b>Settings -> Social Friends</b> for all required options. "); ?>
</p>
<?php
}
...
{
    $widget_options = array('classname' => 'widget_as_facebook_mem_likebox', 'description' => __("Display Social Friends"));
    wp_register_sidebar_widget('as_facebook_mem_likebox_widgets', __('Social Friends'), 'show_as_facebook_mem_likebox_widget', $widget_options);
    wp_register_widget_control('as_facebook_mem_likebox_widgets', __('Social Friends'), 'as_facebook_mem_likebox_widget_control');
}
