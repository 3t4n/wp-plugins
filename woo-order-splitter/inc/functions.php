...
?>
	...
	function wc_os_admin_scripts() {
		...
		if($is_edit_order_page){
			...
			$get_order_meta = wc_os_get_order_meta($order_id);
			
			if(array_key_exists('cloned_from', $get_order_meta)){
				$cloned_from = is_numeric($get_order_meta['cloned_from'])?$get_order_meta['cloned_from']:0;
				
				if($cloned_from){
					$get_order_meta_parent = wc_os_get_order_meta($cloned_from);
					foreach($get_order_meta_parent as $key=>$val){
						if(array_key_exists($key, $get_order_meta)){
							$possible_val = maybe_unserialize($get_order_meta[$key]);
							
							if(gettype($val)==gettype($possible_val)){
							}else{
								//pree($key);pree($possible_val);
								if(is_array($possible_val)){
									$current_val = current($possible_val);
									wc_os_update_order_meta($order_id, $key, $current_val);
								}
							}
						}
					}
