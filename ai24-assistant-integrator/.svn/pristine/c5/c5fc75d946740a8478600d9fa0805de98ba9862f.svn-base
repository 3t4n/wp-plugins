...
function AI24AI_enqueue_scripts() {
    ...
    wp_enqueue_script(
        'AI24AI-chat-main',
        plugins_url('assets/js/chat-main.js', __FILE__),
        array('wp-element'),
        '1.0',
        true
    );
    ...
    wp_script_add_data('AI24AI-chat-main', 'type', 'module');
...
function AI24AI_handle_chat_request() {
    ...
    if (empty($thread_id)) {
        // No thread - create a new thread and run
        $response = AI24AI_create_thread_and_run($user_input, $assistant_id);
        if (!$response) {
            wp_send_json_error(['error' => 'Thread and run creation failed']);
            return;
        }
        $thread_id = $response['thread_id'];
        $run_id = $response['run_id'];
    } else {
        // Existing thread - send a message and create a run
        $message_id = AI24AI_send_message_to_thread($thread_id, $user_input);
        $run_id = AI24AI_create_run($thread_id, $assistant_id);        

        if (!$run_id) {
            wp_send_json_error(['error' => 'Failed to start assistant run']);
            return;
        }
    }

    // Poll for Response
    $poll_result = AI24AI_poll_for_response_and_retrieve_details($thread_id, $run_id);
