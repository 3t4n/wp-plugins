...
class Helpers {
     ...
     * @since 2.0.8
     */
    public function getPinThumbnail( $src, $alt, $extraClasses = [] ) {
        $disable = wp_validate_boolean( gsPinBuilder()->preferences->get( 'disable_lazy_load' ) );
        $classes = array_merge( [], $extraClasses );

        if ( $disable ) {
            $classes[] = gsPinBuilder()->preferences->get( 'lazy_load_class' );
        }
     ...
     * @return string
     */
    public function getExcerpt( $content, $characters = 100 ) {
