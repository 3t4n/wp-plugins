...
if( ! defined( 'PSSG_DEV_VERSION' ) ){
    define( "PSSG_DEV_VERSION", '1.1.1.3' );
}
...
class PSSG_Init
{
    ...
    public function init()
    {
        ...
        if( is_admin() ) PSSG_Sync_Sheet\Framework\Plugin_Required::display_notice();
...
if( ! function_exists( 'pssg_load_textdomain' ) ){
    /**
     ...
     * @since 1.0.0
     */
    function pssg_load_textdomain() {
        load_plugin_textdomain( 'product-sync-master-sheet', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' ); 
    }
    
    add_action( 'init', 'pssg_load_textdomain' );
}
