...
class Page_Loader extends Admin_Base
{
    ...
    public function multiple_site_menu_page_render()
    {
        ...
        $multsite_con_html = $this->page_folder_dir . 'multsite_con_demo.php';
        if (! is_file($multsite_con_html)) return;
        include $multsite_con_html;
    }
    ...
    public function premium_version_update_notice()
    {
        ...
        if ( strpos($s_id, $this->plugin_prefix) === false && $probale) return;
        $min_version = '1.0.7';
        if (version_compare(PSSGP_DEV_VERSION, $min_version, '<')) { ?>
            <div class="notice notice-error">
                <p style="color: #d00;">Your <b><?php echo PSSGP_PLUGIN_NAME; ?></b> <s>(V<i><strong><?php echo PSSGP_DEV_VERSION; ?></strong></i>)</s> is old.</p>
                <p style="color: #999;">Strongly recmmended V<i><b><?php echo $min_version; ?></b></i> or gatter. Please update it.</p>
                <p>Otherwise, all new fetures will not work properly.</p>
            </div>
<?php
        }
    }
