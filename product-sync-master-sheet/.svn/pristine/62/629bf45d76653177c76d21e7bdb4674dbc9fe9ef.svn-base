...
class Products{
    ...
    public $orderby = [
        'title' => 'ASC',         // Order by name (title) in ascending order
        'ID'    => 'ASC'
    ];
    public $order = 'ASC';
     ...
     */
    public $base_chr = 64; //A = 64 //actually two column is auto added.
    ...
    public function __construct($setArgs = [])
    {
        ...
        $this->args = apply_filters( 'pssg_products_args', [], $this );
    }
    ...
    /**
     * Get sheet data for modify multiple product at a time
     * 
     * Actually to get product Array data for multiple product, 
     * we need to call this method
     * In this method we have used $this->get_sheet_row(['post__in' => $product_ids]);
     * This method will help u to get product data for multiple product
     * 
     *
    ...
    /**
     * Get columns label
     * To get only active column label, use 'active' as parameter
     * 
     ...
     * @param string $active By default, It will return all columns label, But If you want, active column label, then use 'active'
     * @return array Like: ['sku'=> 'SKU', 'title'=> 'Product Title']
     */
    public function get_columns_label( $active = 'all' )
    {
        $columns_label = [];
        if( $active == 'active' ){
