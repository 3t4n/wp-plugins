...
class Quick_Table extends Admin_Base 
{
    ...
    public function table_row_products( $prods )
    {
        ...
        $lables_keys = array_keys( $this->Products->get_columns_label( 'active' ) );
        $lables_values = array_values( $this->Products->get_columns_label(  'active' ) );
        ...
        foreach($prods as $key => $prod){
            ...
            foreach($prod as $td_key => $td){
                ...
                $each_td_data = [
                    ...
                    'editCellBool' => $editCellBool,
                    'attr_title' => $attr_title,
                    'td' => $td,
                    'td_key' => $td_key,
                    'td_class' => "each-cell col-{$td_key} cell-{$lables_keys[$td_key]}",
                ];
                if( $key > 0 ){
                    $each_td_data = apply_filters( 'pssg_quick_edit_table_each_td_data', $each_td_data, $product_id );
                }
                
                
                $keyword = $each_td_data['keyword'] ?? '';
                $keyword_label = $each_td_data['keyword_label'] ?? '';
                $editCellBool = $each_td_data['editCellBool'] ?? 'true';
                $attr_title = $each_td_data['attr_title'] ?? '';
                $td = $each_td_data['td'] ?? '';
                $td_key = $each_td_data['td_key'] ?? '';
                $td_class = $each_td_data['td_class'] ?? '';
            ?>
            <td 
            ...
            class='<?php echo esc_attr( $td_class ); ?>'><?php echo wp_kses_post( $td ); ?></td>
            <?php 
            }
            ?> 
            </tr>
    ...
    public function single_product_update($params = [], $updateSheet = true)
    {
        ...
        if( ! empty( $this->product_id ) && wc_get_product( $this->product_id ) ){
            ...
            //Actually to fix _price,_regular_price and _sale_price based on condition
            $this->price_fixing($cf_update);
            ...
            //If status is empty then unset
            $status_label = $this->columns_param_label['_post_status'] ?? 'Status';
            if( isset( $this->requested_data['param_value'][$status_label] ) && empty( $this->requested_data['param_value'][$status_label] ) ){
                unset( $this->requested_data['param_value'][$status_label] );
             ...
             */
            do_action('pssg_product_update_request', $this->requested_data, $this->columns_info, $response);
        }else{
