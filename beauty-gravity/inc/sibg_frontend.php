...
class sibg_frontend {
    ...
    public function GravityInit(){
        ...
        add_action( 'gform_enqueue_scripts', array( $this, 'terminate_gravityform_reset_theme' ), 100 );
    }

    public function terminate_gravityform_reset_theme(){
        wp_deregister_style( 'gravity_forms_theme_reset' );
        wp_dequeue_style( 'gravity_forms_theme_reset' );
        wp_enqueue_style( 
            'gravity_forms_theme_framework-default', 
            GFCommon::get_base_url() . '/assets/css/dist/gravity-forms-theme-framework.min.css', 
            array( 'gravity_forms_theme_foundation' ), 
            GFCommon::$version
        );
        
        wp_enqueue_style( 'BG_themes_default', SIBG_CSS . 'themes/BG_themes_default.css', array(), SIBG_VERSION );

        $file_path     = GFCommon::get_base_path() . '/assets/css/dist/gravity-forms-theme-framework.min.css';
        $saved_version = get_option( 'update_default_gf_theme', '' );

        if ( version_compare( GFCommon::$version, $saved_version, '>' ) ) {
            if (file_exists($file_path)) {
                $css_content = file_get_contents( $file_path );
                $pattern = '/.gform-theme--framework :where\(\n:not\(html\):not\(iframe\):not\(canvas\):not\(img\):not\(svg\):not\(video\)\n:not\(svg \*\):not\(symbol \*\)\n:not\(.gform-theme__no-reset--el\):not\(.gform-theme__no-reset--children \*\):not\(.gform-theme__disable\):not\(.gform-theme__disable \*\):not\(.gform-theme__disable-reset\):not\(.gform-theme__disable-reset \*\):not\(.gform_heading \*\):not\(.gfield--type-html \*\):not\(.gfield--type-section \*\):not\(.form_saved_message > \*\):not\(.form_saved_message_sent > \*\):not\(.gform_confirmation_message \*\):not\(.wp-editor-container\):not\(.mce-tinymce\):not\(.mce-tinymce \*\):not\(.wp-editor-area\):not\(.gfield_description > \*\):not\(.gform-field-label--type-inline > :not\(span\)\):not\(.ui-resizable-handle\):not\(.hidden\)\n\){all:unset;display:revert}/s';
                $updated_css = preg_replace( $pattern, '', $css_content );

                file_put_contents( $file_path, $updated_css );

                update_option( 'update_default_gf_theme', GFCommon::$version );
            }
        }
    }
...
//		}
    ...
    public function FormCustomization($content, $field, $value, $lead_id, $form_id){
		...
		if($this->addIcon){
			wp_enqueue_style( 'font-awesome-icon', "https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.2/css/all.min.css","",  1.0 );
		}
