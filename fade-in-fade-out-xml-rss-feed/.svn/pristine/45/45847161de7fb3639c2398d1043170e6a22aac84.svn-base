...
Description: Now everyone use fade in fade out text in some portion of the website to attract the user. This plugin directly retrieve title from RSS feed and create the fade in fade out effect in the word press website.
...
function FIFOXMLRSSFEED() 
{
	...
	if ( ! is_wp_error( $rss ) )
	{
		...
		?>
		<link rel="stylesheet" type="text/css" href="<?php echo plugins_url(); ?>/fade-in-fade-out-xml-rss-feed/fade-in-fade-out-xml-rss-feed.css" />
		<script type="text/javascript" src="<?php echo plugins_url(); ?>/fade-in-fade-out-xml-rss-feed/fade-in-fade-out-xml-rss-feed.js"></script>
...
function FIFOXMLRSSFEED_install() 
{
	...
	add_option('FIFOXMLRSSFEED_rss_1', "http://www.gopiplus.com/work/category/word-press-plug-in/feed/");
	...
	<div class="wrap">
	  <div class="form-wrap">
		...
		if (isset($_POST['FIFOXMLRSSFEED_submit'])) 
		{
			...
			$FIFOXMLRSSFEED_Title = stripslashes(sanitize_text_field($_POST['FIFOXMLRSSFEED_Title']));
			$FIFOXMLRSSFEED_rss_0 = esc_url_raw($_POST['FIFOXMLRSSFEED_rss_0']);
			$FIFOXMLRSSFEED_rss_1 = esc_url_raw($_POST['FIFOXMLRSSFEED_rss_1']);
			$FIFOXMLRSSFEED_rss_2 = esc_url_raw($_POST['FIFOXMLRSSFEED_rss_2']);
			$FIFOXMLRSSFEED_rss_3 = esc_url_raw($_POST['FIFOXMLRSSFEED_rss_3']);
			$FIFOXMLRSSFEED_FadeWait = intval($_POST['FIFOXMLRSSFEED_FadeWait']);
			
			if(!is_numeric($FIFOXMLRSSFEED_FadeWait) || $FIFOXMLRSSFEED_FadeWait == 0) { $FIFOXMLRSSFEED_FadeWait = 3000; }
...
function FIFOXMLRSSFEED_shortcode( $atts ) 
{
	...
	if ( ! is_wp_error( $rss ) )
	{
		...
		$JaFade = $JaFade . "<link rel='stylesheet' type='text/css' href='".plugins_url()."/fade-in-fade-out-xml-rss-feed/fade-in-fade-out-xml-rss-feed.css' />";
		$JaFade = $JaFade . "<script type='text/javascript' src='".plugins_url()."/fade-in-fade-out-xml-rss-feed/fade-in-fade-out-xml-rss-feed.js'></script>";
