...
if ( ! class_exists( 'ECOMFIT_Order' ) ) {
	...
	class ECOMFIT_Order {
		...
		public static function ecomfit_get_orders( $limit, $offset, $from, $to ) {
			$date_query = array(
				'inclusive' => true,
			);
			if ( $from ) {
			...
			$query    = array(
				...
				'offset'         => $offset,
				'post_status'    => ECOMFIT_ApiCommon::get_active_status_order(),
			...
			foreach ( $orders as $order ) {
				$item = self::ecomfit_get_order( $order->ID );
				if ( sizeof( $item ) ) {
		...
		public static function ecomfit_get_order( $order_id ) {
