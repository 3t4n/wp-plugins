 ...
 */
function dpap_load_textdomain() {
    load_plugin_textdomain('default-post-author', false, dirname(plugin_basename(__FILE__)) . '/languages');
}
add_action('plugins_loaded', 'dpap_load_textdomain');


// Add the field to the general settings page
function dpap_add_author_setting_field() {
    add_settings_field(
        'default_post_author', // ID of the settings field
        __('Default Post Author', 'default-post-author'), // Title of the field
        'dpap_default_post_author_field_html', // Function to display the field
...
add_action('admin_init', 'dpap_add_author_setting_field');

// HTML for the settings field
function dpap_default_post_author_field_html() {
...
function dpap_set_revision_author($post_has_changed, $last_revision, $post) {
    global $wpdb;
...
add_filter('wp_save_post_revision_check_for_changes', 'dpap_set_revision_author', 10, 3);
