jQuery( function( $ ) {
	...
	var owcpvFormbuilder = {
		...
		initPriceNumber: function(fld) {
			var inputField = $(fld).find('.option-priceValue:not(.inited-number), .fld-priceValue:not(.inited-number)');
			if(inputField.length) {
				var priceInit = { 
					'decimal': opalwoocu.currency_format_num_decimals, 
					// 'decimalAuto': opalwoocu.currency_format_num_decimals,
					'separator': opalwoocu.currency_format_decimal_sep,
					'separatorAuthorized': [opalwoocu.currency_format_decimal_sep],
				};
				inputField.inputNumberFormat(priceInit);
				inputField.addClass('inited-number');
			}
		},
		...
		initFormbuilder: function() {
			...
			var formBuilder = $('#owcpv_editor').formBuilder({
				...
				onAddFieldAfter: function(fieldId, fieldData) {
					...
					owcpvFormbuilder.initPriceNumber(fieldId);
