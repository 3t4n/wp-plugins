...
if (!function_exists('owcpv_calculate_price_by_percent')) {
    ...
    function owcpv_calculate_price_by_percent($product_id, $percent = 100, $variation_id = 0) {
        ...
        if ($price && !empty($percent)) {
            return round(floatval($price) / 100 * $percent, wc_get_price_decimals());
        }
        return 0;
    }
}

if (!function_exists('owcpv_convert_to_float')) {
    /**
     * @since  1.0.0
     */
    function owcpv_convert_to_float($number, $customDecimalSeparator = '.') {
        if (preg_match('/[^\d]/', $number)) {
            preg_match_all('/[^\d]/', $number, $separators);
    
            if (count($separators[0]) === 1) {
                $decimalSeparator = $separators[0][0];
                $number = str_replace($decimalSeparator, $customDecimalSeparator, $number);
            } elseif (count($separators[0]) > 1) {
                $thousandSeparator = $separators[0][0];
                $decimalSeparator = $separators[0][1];
