			...
			if (isset($data['values'])) {
				foreach ($data['values'] as $index => $item_value) {
					...
					if ($data['type'] == 'select') {
						...
						if ($show_field_price) {
							$data['values'][$index]['label'] .= ' ('.sprintf(get_woocommerce_price_format(), get_woocommerce_currency_symbol(), $price_value).')';
						}
					}
					elseif ($data['type'] == 'checkbox-group' && isset($data['checkboxsubtype']) && $data['checkboxsubtype'] == 'single-checkbox') {
						$data['label'] .= ' <span class="price_value"> ('.sprintf(get_woocommerce_price_format(), get_woocommerce_currency_symbol(), $price_value).')</span>';
					}
					else {
						if ($show_field_price) {
							$data['values'][$index]['label'] .= ' <span class="price_value"> ('.sprintf(get_woocommerce_price_format(), get_woocommerce_currency_symbol(), $price_value).')</span>';
						}
					}
				}
