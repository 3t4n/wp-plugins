<?php 
  ...
  $selected = (isset($_SESSION['florist-one-flower-delivery-recipient-state'])) ? (empty($_SESSION["florist-one-flower-delivery-recipient-state"]) && $autopop == 1) ? $config_options['locations'][0]->address_state : $_SESSION['florist-one-flower-delivery-recipient-state'] : "";
 
  echo '<optgroup label="United States">';
  foreach ($us_states as $key => $value) { 
	echo '<option value="' . esc_html($key) . '" ' . (esc_html($selected) == esc_html($key) ? 'selected="selected"' : '') . '>' .esc_html($value) . '</option>';
  }
  echo '</optgroup>';
