...
if ( ! defined( 'SFWC_CURRENT_VERSION' ) ) {
	define( 'SFWC_CURRENT_VERSION', '1.6.5' ); // MAJOR.MINOR.PATCH
}
...
/**
 * Declare plugin incompatible with WooCommerce Cart and Checkout Blocks feature.
 *
 * @since 1.5.6
 */
...
if ( ! function_exists( 'sfwc_is_plugin_active' ) ) {  // Check if function already exists from Pro plugin to avoid issues in case the Pro is activated first and then Free plugin.
	...
	function sfwc_is_plugin_active( $plugin_name ) {
		...
		// Get list of active plugins. In case of multisite, this will return a list of active plugins for the current sub-site.
		$active_plugins = (array) get_option( 'active_plugins', array() );
		
		// In case of multisite, get also network active plugins.
		$active_plugins_network = get_site_option('active_sitewide_plugins');

		$plugin_filenames = array();
		
		// Loop through site active plugins.
		...
		if ( ! empty( $active_plugins_network ) ) {
			...
			foreach( $active_plugins_network as $key => $value ) {
...
require_once( plugin_dir_path( __FILE__ ) . 'includes/third-party/hosting/pantheon.php' );