...
/**
 * Show notice: Cart/Checkout Blocks incompatibility.
 *
 * Since 1.5.6
 */
function sfwc_add_notice_cart_checkout_blocks_incompatibility() {
	
	$show_notice = get_transient( 'sfwc_show_cart_checkout_blocks_incompatibility_notice' );
	
	if ( ( WC_Blocks_Utils::has_block_in_page( wc_get_page_id( 'checkout' ), 'woocommerce/checkout' ) || WC_Blocks_Utils::has_block_in_page( wc_get_page_id( 'cart' ), 'woocommerce/cart' ) ) && $show_notice !== 'notice_dismissed' )  {
		
		set_transient( 'sfwc_show_cart_checkout_blocks_incompatibility_notice', 'show_notice' );
		
		$query_arg = add_query_arg( 
			array( 
				'sfwc_ignore_cart_checkout_blocks_incompatibility_notice' => '1',
				'sfwc_nonce' => wp_create_nonce( 'sfwc_ignore_cart_checkout_blocks_notice' )
			)
		);
		
		if ( ! wp_doing_ajax() && is_user_logged_in() && ( current_user_can( 'update_plugins' ) ) ) {
			?>
			<div class="notice notice-warning">
				<h3>Subaccounts for WooCommerce</h3>
				<p>
					<?php echo esc_html__( 'Subaccounts for WooCommerce does not yet support block-based Cart and Checkout pages. Switch to classic shortcode-based Cart and Checkout pages to ensure proper plugin functionality and a good shopping experience.', 'subaccounts-for-woocommerce' ); ?>
				</p>
				<a class="button button-secondary" style="display:inline-block; max-width:100px; margin:10px 0 10px 0; text-align:center;" href="<?php echo esc_url( $query_arg ); ?>">
					<?php echo esc_html__( 'Dismiss notice', 'subaccounts-for-woocommerce' ); ?>
				</a>
			</div>
			<?php
		}
...
add_action('admin_notices', 'sfwc_add_notice_cart_checkout_blocks_incompatibility');




/**
 * Dismiss notice: Cart/Checkout Blocks incompatibility.
 *
 * Since 1.5.6
 */
function sfwc_ignore_notice_cart_checkout_blocks_incompatibility() {
	 
	if ( isset( $_GET['sfwc_ignore_cart_checkout_blocks_incompatibility_notice'] ) ) {
		$sfwc_ignore_cart_checkout_blocks_incompatibility_notice = sanitize_text_field( $_GET['sfwc_ignore_cart_checkout_blocks_incompatibility_notice'] );
	}
	...
	if ( ! wp_doing_ajax() && is_user_logged_in() && ( current_user_can( 'update_plugins' ) ) ) {
		...
		if ( ( isset( $sfwc_ignore_cart_checkout_blocks_incompatibility_notice ) && $sfwc_ignore_cart_checkout_blocks_incompatibility_notice == '1' ) && $show_notice  !== 'notice_dismissed' ) {
			
			// Verify nonce.
			if ( isset( $sfwc_nonce_sanitized ) && wp_verify_nonce( $sfwc_nonce_sanitized, 'sfwc_ignore_cart_checkout_blocks_notice' ) ) {				
				set_transient( 'sfwc_show_cart_checkout_blocks_incompatibility_notice', 'notice_dismissed' );
			}
			else {
				...
				<div class="notice notice-error">
					<h3>Subaccounts for WooCommerce</h3>
					<p>
					<?php echo esc_html__( 'Notice could not be dismissed due to nonce verification failure.', 'subaccounts-for-woocommerce' ); ?>
					</p>
				</div>
				<?php
				} ); 
...
add_action('admin_init', 'sfwc_ignore_notice_cart_checkout_blocks_incompatibility', 10);
...
function sfwc_set_plugin_info_option() {
	...
	if ( ! wp_doing_ajax() ) {
		...
		$existing_values_version = ( isset( $existing_values['version'] ) ) ? $existing_values['version'] : '0.0.0'; // On clean installs $existing_values_version is not set,
																													 // so providing '0.0.0' as value will allow below code
																													 // within version_compare conditional to run.
		...
		if ( version_compare( SFWC_CURRENT_VERSION, $existing_values_version, '>' ) ) {
			...
			// Flush permalinks to avoid possible 'Error 404' on front-end 'subaccounts' endpoint.
			// Especially useful for first time installs.
			flush_rewrite_rules();
		}
 ...
 */
// Required for the add_meta_box to show the meta box in case HPOS is enabled.
...
function sfwc_add_meta_box() {
	...
	if ( ! wp_doing_ajax() ) {
		...
		// Detect if HPOS is enabled or not and assign value to the $screen variable accordingly.
		...
		if ( $sfwc_options_show_order_meta_box == '1' ) {
			add_meta_box('woocommerce-order-subaccounts', esc_html__( 'Subaccounts Info', 'subaccounts-for-woocommerce' ), 'sfwc_add_meta_box_content', $screen, 'side', 'core');
		}
	}
}
...
function sfwc_settings_init() {
    ...
    if ( ! function_exists( 'sfwc_pro_settings_init' ) ) {
		...
		add_settings_field(
				...
				. '<span class="tips woocommerce-help-tip" data-tip="' . esc_attr__( 'If the parent account has a different role than the subaccount, select both the parent account and subaccount roles.', 'subaccounts-for-woocommerce' ) . '"></span>',
		...
		add_settings_field(
				'sfwc_options_enable_user_switcher',
				esc_html__('Enable User Switcher', 'subaccounts-for-woocommerce')
				. '<span class="tips woocommerce-help-tip" data-tip="' . esc_attr__( 'Allow a parent account to log-in as a subaccount.', 'subaccounts-for-woocommerce' ) . '"></span>',
				'sfwc_options_enable_user_switcher_html',
				'subaccounts&tab=options',
				'sfwc_options_section'
		);
		...
		add_settings_field(
				...
				. '<span class="tips woocommerce-help-tip" data-tip="' . esc_attr__( 'Add the \'Subaccount Info\' box on WooCommerce Order pages in WordPress admin area.', 'subaccounts-for-woocommerce' ) . '"></span>',
...
function sfwc_switcher_pane_bg_color_html() {
	...
	<!-- <label for="my-input"><?php esc_html_e( 'My Input', 'subaccounts-for-woocommerce' ); ?></label> -->
...
function sfwc_option_selected_roles_html() {
	...
	if ( ! empty( $roles ) ) {
		...
		foreach ( $roles as $role => $value ) {
			...
			if ( in_array( $role, sfwc_get_valid_roles() ) ) {
...
function sfwc_option_subaccounts_number_limit_html() {
	...
	if ( ! is_plugin_active( 'subaccounts-for-woocommerce-pro/subaccounts-for-woocommerce-pro.php' ) ) {
		...
		$subaccounts_pro = '<a style="color: #148ff3; font-weight: 600;" href="' . 
							esc_url( admin_url( '/admin.php?checkout=true&page=subaccounts-pricing&plugin_id=10457&plan_id=17669&pricing_id=19941&billing_cycle=annual' ) ) . 
							'">Subaccounts Pro</a>';
		*/
...
function sfwc_options_enable_user_switcher_html() {
	...
	<input type="checkbox" id="sfwc_options_enable_user_switcher" name="sfwc_options[sfwc_options_enable_user_switcher]" value="1" <?php checked(1, $sfwc_options_enable_user_switcher, true) ?> />
	<?php
	esc_html_e( "Useful in case you want to allow a parent account to place orders on behalf of subaccounts.", 'subaccounts-for-woocommerce' );
}
...
function sfwc_options_validate( $input ) {
    ...
    if ( isset( $input['sfwc_option_selected_roles'] ) ) {
		...
		if ( ! empty( $sanitized_roles ) ) {
			...
			foreach ( $sanitized_roles as $sanitized_role ) {
				...
				if ( in_array( $sanitized_role, sfwc_get_valid_roles() ) ) {
    ...
    if ( isset( $input['sfwc_option_subaccount_mode'] ) ) {
        ...
        if ( in_array( $input['sfwc_option_subaccount_mode'], $valid_radio_subaccount_mode ) ) {
			...
			// When "Sub-User" mode is enabled, flush permalinks to prevent My Account > Subaccounts > Subaccount Orders from returning Error 404.
			if ( $input['sfwc_option_subaccount_mode'] == 'sub_user' ) {
				if ( get_transient( 'sfwc_flush_rewrite_rules_required' ) === false ) {
					set_transient('sfwc_flush_rewrite_rules_required', true);
				}
			}
	...
	$output['sfwc_options_enable_user_switcher'] = '0';

	// Check if the current option has a value.
	if ( isset( $input['sfwc_options_enable_user_switcher'] ) ) {

		$output['sfwc_options_enable_user_switcher'] = '1';
	}
...
/**
 * Flush rewrite rules after updating certain plugin settings.
 * 
 * Update permalink structure after updating below settings:
 *
 *	- Subaccount Mode
 *	- Choose who can create and add new subaccounts
 */
function sfwc_flush_rewrite_rules_after_settings_update() {

	if ( delete_transient( 'sfwc_flush_rewrite_rules_required' ) ) {
		flush_rewrite_rules();
	}
}
add_action('admin_init', 'sfwc_flush_rewrite_rules_after_settings_update');
...
function sfwc_admin_page_contents() {
	...
	<h1 style="margin-top:40px;">
		<?php
		echo '<img style="height:50px; width:auto; vertical-align:middle; padding-right:5px;" src="' . esc_url( WP_PLUGIN_URL ) . '/subaccounts-for-woocommerce/assets/images/sfwc_icon.png">';
		echo 'Subaccounts for WooCommerce'; // Not translatable for branding purpose.
		echo sfwc_is_plugin_active( 'subaccounts-for-woocommerce-pro.php' ) ? ' Pro' : '';
		?>
	</h1>
...
function sfwc_options_tab() {
	global $sfwc_active_tab;
	?>
	<a class="nav-tab <?php echo $sfwc_active_tab == 'options' || '' ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url( admin_url( 'admin.php?page=subaccounts&tab=options' ) ); ?>"><?php esc_html_e( 'Options', 'subaccounts-for-woocommerce' ); ?> </a>
	<?php
}
...
function sfwc_appearance_tab() {
	global $sfwc_active_tab;
	?>
	<a class="nav-tab <?php echo $sfwc_active_tab == 'appearance' ? 'nav-tab-active' : ''; ?>" href="<?php echo esc_url( admin_url('admin.php?page=subaccounts&tab=appearance') ); ?>"><?php esc_html_e('Appearance', 'subaccounts-for-woocommerce'); ?> </a>
	<?php
}
		...
		<tbody>
			<tr>
				<th scope="row" style="padding: 15px 10px 20px 0;">
					<?php echo esc_html__('Show subaccounts information on WooCommerce orders list page', 'subaccounts-for-woocommerce') . '<span class="tips woocommerce-help-tip" data-tip="' . esc_attr__("Add the 'Order placed by' column to the WooCommerce orders list page in WordPress admin area. In case Sub-User mode is active, 'Account Type' and 'Parent Accounts' columns will also show up.", 'subaccounts-for-woocommerce') . '"></span>'; ?>
					<br>
					<a style="color:#148ff3;" href="<?php echo esc_url( admin_url( '/admin.php?checkout=true&page=subaccounts-pricing&plugin_id=10457&plan_id=17669&pricing_id=19941&billing_cycle=annual' ) ); ?>">
