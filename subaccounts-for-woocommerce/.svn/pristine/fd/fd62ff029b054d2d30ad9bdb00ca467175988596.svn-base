...
// Required to detect if HPOS is enabled or not.
...
function sfwc_set_parent_account_cookie() {
	...
	if ( is_user_logged_in() && sfwc_is_current_user_role_valid() && sfwc_is_current_user_role_enabled() ) {
		...
		if ( ! isset( $_COOKIE[$finalUserCookieName] ) ) {
			...
			$cookie_parts = explode( '|', $finalUserCookieNameValue ); // 0 => user_login, 1 => expiration, 2 => token, 3 => hmac
		...
		if ( $account_level_type == 'manager' ) {
			...
			if ( ! isset( $_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_manager'] ) ) {
				
				// Set a trasient to check 'is_manager' cookie value against (they must be equal).
				set_transient( 'sfwc_is_or_was_manager_' . $user_session_token, $finalUserCookieNameValue, 3600 );

				// Set 'is_manager' cookie name and its value (same as 'wordpress_logged_in_...' value).
				setcookie( apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_manager', $finalUserCookieNameValue, [
					'expires' => '',
					'path' => COOKIEPATH,
				...
				$_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_manager'] = get_transient( 'sfwc_is_or_was_manager_' . $user_session_token );
			}
		}
		...
		if ( $account_level_type == 'supervisor'  ) {
			...
			if ( ! isset($_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_supervisor'] ) ) {

				// Set transient
				set_transient( 'sfwc_is_or_was_supervisor_' . $user_session_token, $finalUserCookieNameValue, 3600 );

				// Set 'is_supervisor' cookie name and its value (same as 'wordpress_logged_in_...' value)
				setcookie( apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_supervisor', $finalUserCookieNameValue, [
					'expires' => '',
					'path' => COOKIEPATH,
					'domain' => COOKIE_DOMAIN,
					'secure' => is_ssl(),
				...
				// Provide initial value to cookie otherwise frontend switcher will be shown only after page refresh
				$_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_supervisor'] = get_transient( 'sfwc_is_or_was_supervisor_' . $user_session_token );
			}
		}
	}
}
...
function sfwc_destroy_parent_account_cookie_on_logout() {
    ...
    if ( isset( $_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_manager'] ) ) {
		...
		$cookie_value_manager = sanitize_text_field( $_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_manager'] );
		...
		// Delete transient.
		delete_transient( 'sfwc_is_or_was_manager_' . $user_session_token );
		
		// Destroy is_manager cookie.
		setcookie( apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_manager', null, -1, COOKIEPATH);
    }
	
	if ( isset( $_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_supervisor'] ) ) {
		
		$cookie_value_supervisor = sanitize_text_field( $_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_supervisor'] );
		...
		// Delete transient.
		delete_transient( 'sfwc_is_or_was_supervisor_' . $user_session_token );
		
		// Destroy is_supervisor cookie
		setcookie( apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_supervisor', null, -1, COOKIEPATH);
	}
}
...
function sfwc_action_woocommerce_account_dashboard() {
	...
	#echo '<strong>is_or_was_manager:</strong> '; var_dump($is_or_was_manager); echo '<br>';
	#echo '<strong>is_or_was_supervisor:</strong> '; var_dump($is_or_was_supervisor);
    ...
    $sfwc_options = (array) get_option('sfwc_options');
		...
		$sfwc_options_enable_user_switcher = ( isset( $sfwc_options['sfwc_options_enable_user_switcher'] ) ) ? $sfwc_options['sfwc_options_enable_user_switcher'] : 1;
	...
	if( $sfwc_options_enable_user_switcher == 1 ) {
		...
		if ( is_user_logged_in() && sfwc_is_current_user_role_valid() && sfwc_is_current_user_role_enabled() ) {
			...
			if ( isset( $cookie_value_manager ) && ( $cookie_value_manager !== $is_or_was_manager ) && ! isset( $cookie_value_supervisor ) ) {
				...
				echo '<a id="sfwc-login-again" href="' . esc_url( wp_logout_url( wc_get_page_permalink( 'myaccount' ) ) ) . '">' . esc_html__('Login Again', 'subaccounts-for-woocommerce') . '</a></p>';
			}
			...
			if ( isset( $cookie_value_manager ) && ( $cookie_value_manager === $is_or_was_manager ) && ! isset( $cookie_value_supervisor ) ) {
				...
				***/
				...
				if ( ( $account_level_type == 'manager' && ! empty( $existing_children_ids ) ) || ( $account_level_type !== 'manager' && $account_level_type !== 'supervisor' ) ) {
					...
					echo '<div id="sfwc-user-switcher-pane" style="position:relative; background-color:' . esc_attr( $sfwc_switcher_pane_bg_color ) . ';">';
					...
					$class_selectize = empty( $user_query_manager->get_results() ) ? 'sfwc_frontend_children_selectize' : '';
					?>

					<form method="post">
						<?php wp_nonce_field( 'sfwc_switch_account_frontend_action', 'sfwc_switch_account_frontend' ); ?>
						
						<select id="sfwc_frontend_children" class="<?php echo esc_attr( $class_selectize ); ?>" name="sfwc_frontend_children" onchange="this.form.submit();" style="background-color:<?php echo esc_attr( $sfwc_switcher_pane_select_bg_color ); ?>; color:<?php echo esc_attr( $sfwc_switcher_pane_select_text_color ); ?>;">
					...
					<img id="sfwc-loading-icon" src="<?php echo esc_url( WP_PLUGIN_URL . '/subaccounts-for-woocommerce/assets/images/loader.gif' ); ?>" style="position:absolute; left:0; right:0; top:0; bottom:0; text-align:center; margin:auto; z-index:999999;">
					</div>
					
				
					<script>
					// Check if jQuery is loaded using Javascript.
					// This is to avoid getting "jQuery undefined" in case the loading of jquery.min.js is deferred.
						...
						if (window.jQuery) {
							...
							(function($) {
								...
								$(".sfwc_frontend_children_selectize").selectize({
									items: null, // Prevent Selectize from selecting the first item by default.
									placeholder: '<?php echo esc_html__( 'Select Account', 'subaccounts-for-woocommerce' ); ?>' + ' \u{21C4}',
								...
								$("#sfwc-user-switcher-pane #sfwc-loading-icon").hide();
								
								// ID of customer which is currently being edited on user-edit.php
								var value_input = <?php echo absint( $user_id ); ?>;

								var sfwc_ajax_url = "<?php echo esc_url( admin_url('admin-ajax.php') ); ?>";

								var sfwc_nonce = "<?php echo wp_create_nonce('sfwc_nonce'); ?>";
								...
								if( $account_level_type == 'manager' ){
									?>
									var currentAccountType = 'manager';
									ajaxAction = 'frontend_populate_user_switcher_dropdown_with_subaccounts';
									
									// Run function only on first click after page load.
									$("#sfwc-user-switcher-pane .selectize-input").one("click", function () {
										getUsersPopulateSelectField();
									});
									<?php
								}
								?>						
								...
								function wrapGetUsersPopulateSelectField() {
									
									$('#sfwc-user-switcher-pane .selectize-dropdown-content').on('scroll', function() {
									...
									/**
									 * Trigger getUsersPopulateSelectField() function some seconds after user has finished typing.
									 */
									 
									// Timer setup.
									var typingTimerUserSwitcher;                // Timer identifier.
									var doneTypingIntervalUserSwitcher = 2000;  // Time in ms.
									var $inputUserSwitcher = $('#sfwc-user-switcher-pane .selectize-input input');
									
									// On keyup, start the countdown.
									$inputUserSwitcher.on('keyup', function (e) {
										
										var TextboxValue = $('#sfwc_frontend_children').selectize()[0].selectize.getTextboxValue();
										
										/**
										 * Reference: https://developer.mozilla.org/en-US/docs/Web/API/UI_Events/Keyboard_event_key_values
										 *
										 * Note: do not use "KeyCode" since it's deprecated. "key" property is fine.
										 */
										if (   e.key == "CapsLock"
											|| e.key == "Shift"
											|| e.key == "Tab"
											|| e.key == "Escape"
											|| e.key == "Control"
											|| e.key == "Meta" /* E.g. Windows, Command Key */
											|| e.key == "Pause"
											|| e.key == "Alt"
											|| e.key == "Fn"
											|| e.key == "Home" 
											...
											|| e.key == "NumLock"
											|| e.key == "ScrollLock"
											|| (e.key = "Backspace" && TextboxValue.length == 0 && noMoreCharacters == false ) /* Backspace when there are no characters */
										) {									
											// Do nothing when above key are pressed.
											// Do not return, we need this function to continue.
										 } else {
											
											if (e.key = "Backspace" && TextboxValue.length == 0) {
												noMoreCharacters = false;
											} else {
												noMoreCharacters = true;
											}
											
											clearTimeout(typingTimerUserSwitcher);
											typingTimerUserSwitcher = setTimeout(doneTypingUserSwitcher, doneTypingIntervalUserSwitcher);
										}
									});
									
									// On keydown, clear the countdown.
									$inputUserSwitcher.on('keydown', function () {
									  clearTimeout(typingTimerUserSwitcher);
									});

									// User has finished typing.
									function doneTypingUserSwitcher() {
										search_val = $('#sfwc-user-switcher-pane .selectize-input input').val();
										page = 0;
										getUsersPopulateSelectField();
									}
								}
								wrapGetUsersPopulateSelectField();
							
							
								/**
								 * Retrieve users via Ajax and populate Selectize form.
								 */
								function getUsersPopulateSelectField() {
									
									$.ajax({
										type: 'POST',
										data: {
											action: ajaxAction,
											ajax_user_id: value_input,
											ajax_page: page + 1,
										...
										beforeSend: function () {
											...
											// Temporarily disable dropdown items selection.
											$('#sfwc-user-switcher-pane .selectize-dropdown-content').css({ 'opacity' : '0.5', 'pointer-events' : 'none' });
											
											// Show loading icon.
											$("#sfwc-user-switcher-pane #sfwc-loading-icon").show();									
										},
										success: function (value) {									
											...
											// Check if "value" contains any option tags, if not it means there are no more users to show.
											if ( value.indexOf('</option>') > -1 ) {
												...
												// Destroy previous Selectize instance.
												$('#sfwc_frontend_children').selectize()[0].selectize.destroy();
												
												//if ( ( search_val != null && page == 1 ) || changeAccountTypeRadio == true ) {
												if ( search_val != null && page == 1 ) {
													
													$('select#sfwc_frontend_children').html('');
												}
												
												// Append new options.
												$('select#sfwc_frontend_children').append(value);

												// Reinitialize Selectize with new values.
												$(".sfwc_frontend_children_selectize").selectize({
													items: null, // Prevent Selectize from selecting the first item by default.
													placeholder: '<?php echo esc_html__( 'Select Account', 'subaccounts-for-woocommerce' ); ?>' + ' \u{21C4}',
													copyClassesToDropdown: true,
												...
												// Set value of search text (if any).
												if ( TextboxValue.length > 0 ) {
													$( '#sfwc_frontend_children' ).selectize()[0].selectize.setTextboxValue( TextboxValue );
												}
												
												// Refresh Selectize options.
												$('#sfwc_frontend_children').selectize()[0].selectize.refreshOptions();
												
												// Keep scroll position.
												var $container = $('#sfwc-user-switcher-pane .selectize-dropdown-content');
												var $scrollTo = $('#sfwc-user-switcher-pane div.first-item-loaded:last').prevAll().eq(6); // 6 = number of visible items in selectize dropdown without scrolling (7) minus 1.
												jQuery(document).ready(function(){
													
													if ( $scrollTo.offset() && $scrollTo.offset() ) {
														$container.scrollTop(
												...
												// Re-enable dropdown items selection.
												$('#sfwc-user-switcher-pane .selectize-dropdown-content').css({ 'opacity' : '1', 'pointer-events' : '' });
...
function sfwc_set_current_user_cookies() {
	...
	if ( ! isset( $_POST['sfwc_switch_account_frontend'] ) ) {
		return;
	}
	elseif( isset( $_POST['sfwc_switch_account_frontend'] ) && ! wp_verify_nonce( $_POST['sfwc_switch_account_frontend'], 'sfwc_switch_account_frontend_action' ) ) {
		...
		if ( isset( $_POST['sfwc_frontend_children'] ) ) {
			...
			/**
			 * A form input value is always a string, so is_int is not an option here.
			 *
			...
			if ( is_numeric( $selected ) && $selected >= 1 && preg_match( '/^[1-9][0-9]*$/', $selected ) ) {
				...
				if ( ( is_user_logged_in() && $account_level_type == 'supervisor' ) && ( isset( $cookie_value_supervisor ) && ( $cookie_value_supervisor === $is_or_was_supervisor ) ) ) {
					...
					if ( in_array( $selected, $children_ids, true ) || in_array( $selected, $children_of_manager, true ) ) {
						...
						wc_setcookie( 'woocommerce_cart_hash', md5( wp_json_encode( WC()->cart->get_cart() ) ) );
		...
		if ( $sanitized_value === '' ) {
			...
			if ( $user_account_level_type == 'manager' ) {
				...
				echo '<img id="sfwc-loading-icon" src="' . esc_url( WP_PLUGIN_URL ) . '/subaccounts-for-woocommerce/assets/images/loader.gif">';
					...
					// Do not wrap below script in a document ready function.
					// As of jQuery v.3, $(window).on("load"... may not fire inside a document ready function.
					// Behavior confirmed on Firefox/-moz browsers.
					?>
						<script>
						// Check if jQuery is loaded using Javascript.
						// This is to avoid getting "jQuery undefined" in case the loading of jquery.min.js is deferred.
						window.addEventListener('load', function() {
							if (window.jQuery) {
								...
								(function($) {
									...
									$("#frontend_manage_subaccounts #sfwc-loading-icon").hide();
									...
									//$(window).on("load", function () {
										$.ajax({
											...
											beforeSend: function () {
												...
												// Show loading icon.
												$("#frontend_manage_subaccounts #sfwc-loading-icon").show();
											},
				...
				if ( ! empty( $existing_children_ids ) ) {	// Prevent showing the Subaccounts Order form in case the logged in customer has no subaccounts.
					...
					if ( ! isset( $sanitized_order_id ) ) {
						...
						$args = array(
							...
							'end_size' => 2,
							'prev_next' => true,
							'prev_text' => __( '&laquo; Previous', 'subaccounts-for-woocommerce' ),
							'next_text' => __( 'Next &raquo;', 'subaccounts-for-woocommerce' ),
							'type' => 'plain',
							'add_args' => false,
							'add_fragment' => ''
						);
 ...
 *
 * Remove default order number column, in: Subaccounts > Subaccount Orders.
 * We need to remove it and add our own order number column in order to change the URL of the order number links.
 */
function sfwc_remove_defualt_order_number_column( $columns ) {

	if ( is_wc_endpoint_url( 'subaccounts' ) ) {
		unset( $columns['order-number'] );
		$custom_order_number = array( 'sfwc-order-number' => esc_html( __( 'Order', 'woocommerce' ) ) );
		$columns = array_merge( $custom_order_number, $columns );
	}
    return $columns;
}
add_filter( 'woocommerce_account_orders_columns', 'sfwc_remove_defualt_order_number_column', 10, 1 );
 ...
 *
 * Add our own order number column, in: Subaccounts > Subaccount Orders.
 * We need to customize the URL of the order number links, otherwise if clicked will point to the default My Account > Orders page.
 */
function sfwc_add_custom_order_number_column( $order ) {
	
	if ( is_wc_endpoint_url( 'subaccounts' ) ) {
		$order_number_url = wc_get_endpoint_url( 'subaccounts/subaccount-orders' ) . '?order=' . $order->get_id();
		echo '<a href="' . esc_url( $order_number_url ) . '">' . esc_html( _x( '#', 'hash before order number', 'woocommerce' ) ) . absint( $order->get_order_number() ) . '</a>';
	}
}
add_action( 'woocommerce_my_account_my_orders_column_sfwc-order-number', 'sfwc_add_custom_order_number_column', 10, 1 );
...
function sfwc_add_subaccount_form_content () {
	...
	//Check 'Customer Display Name' in Subaccounts > Settings > Options and display it accordingly.
	if ( ( $sfwc_option_display_name == 'full_name' ) && ( $parent_user_data->user_firstname || $parent_user_data->user_lastname ) ) {

		// Echo 'Full Name + Email' (if either First Name or Last Name has been set).
		$user_parent_name = '<strong>' . esc_html( $parent_user_data->user_firstname ) . ' ' . esc_html( $parent_user_data->user_lastname ) . '</strong>';

	} elseif ( ( $sfwc_option_display_name == 'company_name' ) && ( $parent_user_data->billing_company ) ) {

		// Echo 'Company + Email' (if Company name has been set).
		$user_parent_name = '<strong>' . esc_html( $parent_user_data->billing_company ) . '</strong>';

	} else {

		// Otherwise echo 'Username + Email'.
		$user_parent_name = '<strong>' . esc_html( $parent_user_data->user_login ) . '</strong>';
	}
	 ...
	 */
	$already_children = get_user_meta( $parent_user_id, 'sfwc_children', true );
	
	$already_children_existing = array();
	...
	?>
		 ...
		 *
		 * Allow developers to add extra data on frontend subaccount creation form.
		 *
		 * 
		 * @since 1.5.0
		 */					  
		do_action( 'sfwc_frontend_add_subaccount_form_before_fields', $parent_user_id, $already_children_existing );
		?>
			
			<div class="user_login" style="margin-bottom:20px; width:48%; float:left;">
				<label for="user_login" style="display:block; margin-bottom:0; font-weight:600; <?php echo esc_attr( $username_required_css ); ?>"><?php esc_html_e( 'Username', 'subaccounts-for-woocommerce' ); ?> <span style="font-weight:bold;">*</span></label>
			...
			<div class="email" style="margin-bottom:20px; width:48%; float:right;">
				<label for="email" style="display:block; margin-bottom:0; font-weight:600; <?php echo esc_attr( $email_required_css ); ?>"><?php esc_html_e( 'Email', 'subaccounts-for-woocommerce' ); ?> <span style="font-weight:bold;">*</span></label>
			...
			<div class="first_name" style="margin-bottom:20px; width:48%; float:left;">
				<label for="first_name" style="display:block; margin-bottom:0; font-weight:600;"><?php esc_html_e( 'First Name', 'subaccounts-for-woocommerce' ); ?></label>
			...
			<div class="last_name" style="margin-bottom:20px; width:48%; float:right;">
				<label for="last_name" style="display:block; margin-bottom:0; font-weight:600;"><?php esc_html_e( 'Last Name', 'subaccounts-for-woocommerce' ); ?></label>
			...
			<div class="company" style="margin-bottom:20px; width:100%;">
				<label for="company" style="display:block; margin-bottom:0; font-weight:600;"><?php esc_html_e( 'Company', 'subaccounts-for-woocommerce' ); ?></label>
...
/**
 * Add "Subaccounts" menu item and content to My Account page (Pt.9).
 *
 * Handle form submitted data when adding a new subaccount.
 *
 * My Account -> Subaccounts -> Add Subaccount.
 *
 * See also: sfwc_validate_additional_fields_for_supervisor_in_frontend_add_subaccount_form function in Supervisor Add-On.
 */
function sfwc_add_subaccount_form_handler() {
	...
	// Get account type of currently logged-in user
	$parent_user_account_type = get_user_meta( $parent_user_id, 'sfwc_account_level_type', true );	
	...
	/**
	 * Validate Supervisor Add-On posted data.
	 * See function: sfwc_validate_additional_fields_for_supervisor_in_frontend_add_subaccount_form.
	 */
	if( isset( $_POST['account_type'] ) && $_POST['account_type'] == 'default' ) {
			
		/**
		 * Supervsior is adding a new Subaccount (Default account type).
		 */
		if( ! isset( $_POST['assigned_manager'] ) ) {
			
			wc_add_notice( esc_html__( 'Assigned Manager is required.', 'subaccounts-for-woocommerce' ), 'error');
			return;
		}
	}
	...
	if ( ( isset( $user_login ) && $user_login != "" && validate_username( $user_login ) ) && ( isset( $email ) && $email != "" && is_email( $email ) ) ) {
		...
		if ( ! isset( $_POST['sfwc_add_subaccount_frontend'] ) || isset( $_POST['sfwc_add_subaccount_frontend'] ) && ! wp_verify_nonce( $_POST['sfwc_add_subaccount_frontend'], 'sfwc_add_subaccount_frontend_action' ) ) {
			...
			// Check 'New User Default Role' in: Dashboard -> Settings -> General -> New User Default Role.
			$default_user_role = get_option('default_role');
			
			// Set user role.
			if ( in_array( $default_user_role, sfwc_get_valid_roles() ) ) {
			...
			if ( ! $user_id || is_wp_error( $user_id ) ) {
				...
				// If wp_insert_user has executed successfully and the id of the created user has been returned, use that id to insert Company name.
				update_user_meta( $user_id, 'billing_company', $company ); // Already sanitized.
				
				
				$switcher_notice = ( $sfwc_options_enable_user_switcher == 1 ) ? '<br>' . esc_html__( 'You can now switch to the newly added subaccount by selecting it from the drop-down menu.', 'subaccounts-for-woocommerce' ) : '';

				wc_add_notice( '<strong>' . esc_html__( 'Subaccount successfully added.', 'subaccounts-for-woocommerce' ) . '</strong>' . $switcher_notice, 'success');			
				...
				$list_of_children_of_all_managers = array();
					...
					foreach ( $customers_are_managers->get_results() as $user ) {
						...
						if ( ! empty( $list_of_children_for_single_user ) ) {
							...
							foreach ( $list_of_children_for_single_user as $single_id ) {
								...
								$list_of_children_of_all_managers[] = $single_id;
							}
						}
					}
				}

				// If loggedin user has a "default" account type and has not got a manager above him, turn it into a Manager.
				if ( ( $parent_user_account_type == "default" || $parent_user_account_type == "" ) && ! in_array( $parent_user_id, $list_of_children_of_all_managers ) ) {

					update_user_meta( $parent_user_id, 'sfwc_account_level_type', 'manager' );
				}
				...
				if( $parent_user_account_type !== 'supervisor' ) {
					
					$already_children = get_user_meta( $parent_user_id, 'sfwc_children', true ); // We need to get the value of the array and update it by adding the new ID,
					...
					update_user_meta( $parent_user_id, 'sfwc_children', $already_children );
				}
				...
				/**
				 * Hook: sfwc_frontend_after_add_subaccount_validation.
				 *
				 * Allow developers to save extra data on frontend subaccount creation.
				 *
				 * @param int         $user_id User ID of subaccount being created.
				 * @since 1.4.1
				 */
				do_action( 'sfwc_frontend_after_add_subaccount_validation', $parent_user_id, $user_id );
	...
	// $check_account_type = get_user_meta( $parent_user_id, 'sfwc_account_level_type', true );
...
/**
 * Add "Subaccounts" menu item and content to My Account page (Pt.10).
 *
...
function sfwc_frontend_edit_subaccount_form_handler() {
	...
	if ( is_wc_endpoint_url( 'subaccounts' ) ) {
		...
		/**
		 * Hook: sfwc_frontend_after_edit_subaccount_validation.
		 *
		 * Allow developers to save extra data when editing a subaccount on frontend.
		 *
		 * @param int         $user_id User ID of subaccount being edited.
		 * @since 1.5.5
		 */
		do_action( 'sfwc_frontend_after_edit_subaccount_validation', $user_id );		
 ...
 */
function sfwc_store_subaccounts_meta_data_on_order_creation( $order ) {
	...
	elseif ( $customer_account_level_type == 'default' || $customer_account_level_type == '' || empty( $customer_account_level_type ) ) {
		...
		} elseif ( ! isset($_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_supervisor'] )  && isset($_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_manager'] ) ) {
		 ...
		 *
		 * _sfwc_order_placed_by_user_id meta has been added for this purpose.
		 *
		 * From version 1.4.3 this value is always saved on order creation (even if Sub-User mode is enabled).
		 * This is to provide more compatibility in case of switch from Sub-User mode to Multi-User mode and vice versa.
		 *
		 ...
		 */
		$order->update_meta_data( '_sfwc_order_placed_by_user_id', absint( $customer_id ) );
		
		
		/**
		 * Stores the active subaccount mode at the time the order is placed.
		 *
		 * Useful to determine if one or more orders have been placed in a mode other than the one currently active
		 * and in this case perform some actions.
		 */
		$order->update_meta_data( '_sfwc_order_placed_during_subaccount_mode', sanitize_text_field( $sfwc_option_subaccount_mode ) );

		$order->save();
	}
}
add_action('woocommerce_checkout_create_order', 'sfwc_store_subaccounts_meta_data_on_order_creation', 20, 1);
add_action('woocommerce_store_api_checkout_update_order_meta', 'sfwc_store_subaccounts_meta_data_on_order_creation', 20, 1); // For block enabled themes using Checkout block instead of shortcode.
...
function sfwc_order_placed_by_update_order_meta_after_payment( $order_id ) {
	...
	if ( ! isset($_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_supervisor'] )  && isset($_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'is_manager'] ) ) {
...
add_action('woocommerce_thankyou', 'sfwc_order_placed_by_update_order_meta_after_payment', 5, 1); // Priority 5: run earlier then sfwc_save_approved_order from Order Approval Add-On.
...
function sfwc_my_account_orders_order_placed_by( $order ) {
	...
	if ( $order_placed_by ) {
		...
		if ( $order_placed_by == 'default_for_himself' ) {
			...
			// Multi-User mode.
			if ( $sfwc_option_subaccount_mode == 'multi_user' || $sfwc_option_subaccount_mode == 'sub_user' && sfwc_order_placed_during_subaccount_mode( $order_id ) == 'multi_user') {
				
				if ( ! empty( $userdata_customer_placing_order ) ) { // $userdata_customer_placing_order may not be available if the order was placed before enabling Multi-User mode with plugin version prior to 1.4.3.
			...
			if ( $sfwc_option_subaccount_mode == 'sub_user' || ( $sfwc_option_subaccount_mode == 'multi_user' ) ) {
			...
			if ( $sfwc_option_subaccount_mode == 'multi_user' || $sfwc_option_subaccount_mode == 'sub_user' && sfwc_order_placed_during_subaccount_mode( $order_id ) == 'multi_user') {
				...
				if ( ! empty( $userdata_customer_placing_order ) ) {
					...
					if ( ( $sfwc_option_display_name == 'full_name' ) && ( get_userdata( $userdata_customer_placing_order->ID )->user_firstname || get_userdata( $userdata_customer_placing_order->ID )->user_lastname ) ) {
						...
						if ( get_userdata($userdata_supervisor->ID)->billing_company === get_userdata( $userdata_customer_placing_order->ID )->billing_company ) {
							
							/**
							 * If both the supervisor and the subaccount have the same Company Name,
							 * for the subaccount we print the Full Name.
							 *
							 * This to avoid confusion, e.g.
							 * Order placed by: Supervisor (Company Name Inc.) on behalf of subaccount (Company Name Inc.).
							 */
							printf( '%1$s %2$s', esc_html( $userdata_customer_placing_order->user_firstname ), esc_html( $userdata_customer_placing_order->user_lastname ) );
						} else {
				...
				/**
				 * If $userdata_customer_placing_order is empty, it means the order was placed in one of the following situation:
				 *
				 *		- Order was placed before installing/activating the plugin (even if instantly put in Multi-User mode after activation);
				 *		- Order was placed before switching from Sub-User to Multi-User mode with plugin version lower than 1.4.3 (*)
				 *
				 *		(*) Since v.1.4.3, _sfwc_order_placed_by_user_id order meta value is always stored, regardless of whether Sub-User or Multi-User mode is active.
				 *
				 * In both cases the customer tied to the order can be normally retrieved with: $order->get_customer_id();
				 */
				else {
			...
			if ( $sfwc_option_subaccount_mode == 'multi_user' || $sfwc_option_subaccount_mode == 'sub_user' && sfwc_order_placed_during_subaccount_mode( $order_id ) == 'multi_user') {
				...
				if ( ! empty( $userdata_customer_placing_order ) ) {
					...
					if ( ( $sfwc_option_display_name == 'full_name' ) && ( get_userdata( $userdata_customer_placing_order->ID )->user_firstname || get_userdata( $userdata_customer_placing_order->ID )->user_lastname ) ) {
						...
						if ( get_userdata($userdata_manager->ID)->billing_company === get_userdata( $userdata_customer_placing_order->ID )->billing_company ) {
							
							/**
							 * If both the manager and the subaccount have the same Company Name,
							 * for the subaccount we print the Full Name.
							 *
							 * This to avoid confusion, e.g.
							 * Order placed by: Manager (Company Name Inc.) on behalf of subaccount (Company Name Inc.).
							 */
							printf( '%1$s %2$s', esc_html( $userdata_customer_placing_order->user_firstname ), esc_html( $userdata_customer_placing_order->user_lastname ) );
...
function sfwc_my_account_orders_add_select_subaccount_orders_form( $has_orders ) {
	...
	// Sanitize cookie value.
	if ( isset( $_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'subaccount_ids'] ) ) {

		// Decode cookie
		$subaccount_ids_cookie_decoded = json_decode( str_replace ('\"','', $_COOKIE[ apply_filters( 'sfwc_cookie_prefix', '' ) . 'subaccount_ids']), true );
	...
	*/
	...
	if ( $account_level_type == 'manager' && sfwc_is_current_user_role_valid() && sfwc_is_current_user_role_enabled() ) {
		...
		?>
			...
			<div id="sfwc-select-subaccount-orders-form-wrap" style="margin-bottom:50px; background-color:#f4f4f4; padding:30px; position:relative;">
				<form method="post">
					...
					<div style="width:70%; float:left;">
						<select id="sfwc-select-subaccount-orders" name="sfwc_subaccount_orders[]" multiple placeholder="<?php esc_attr_e( 'Select account...', 'subaccounts-for-woocommerce' ); ?>">							
						</select>

					</div>
					<input type="submit" value="<?php esc_attr_e( 'Submit', 'subaccounts-for-woocommerce' ); ?>" style="width:30%; float:left; font-weight:600;">
				</form>
				<img id="sfwc-loading-icon" src="<?php echo esc_url( WP_PLUGIN_URL . '/subaccounts-for-woocommerce/assets/images/loader.gif' ); ?>" style="position:absolute; left:0; right:0; top:0; bottom:0; text-align:center; margin:auto; z-index:999999;">
			</div>
		<?php
		...
		window.addEventListener('load', function() {
			if (window.jQuery) {
				...
				(function($) {
					...
					Selectize.define("deselect_all_options", function(options) {
						...
						self.setup = (function() {
							...
							return function() {
								...
								var clearBtn = $('<button type=\"button\" class=\"button\" style=\"margin-top:10px;\"><?php esc_html_e( 'Deselect All', 'subaccounts-for-woocommerce' ); ?></button>');

								clearBtn.on("click", function() {
									self.setValue([]);
								});

								this.$wrapper.append(clearBtn)
							};
						})();
					});
					...
					<?php
					// Validation of pre-selected items.
					if ( isset( $subaccount_ids_cookie_sanitized ) && ! empty ( $subaccount_ids_cookie_sanitized ) ) {
						
						foreach ( $subaccount_ids_cookie_sanitized as $single_id ) {
							...
							if ( $user_exists !== false ) {
								...
								if ( sfwc_is_user_role_valid( $single_id ) && sfwc_is_user_role_enabled( $single_id ) ) {
									...
									$existing_selected_ids[] = $single_id;
								}
							}
						}
					}
					...
					if( ! empty( $existing_selected_ids ) ) {

						foreach ( $existing_selected_ids as $id ) {
							
							if ( sfwc_is_user_role_valid( $id ) && sfwc_is_user_role_enabled( $id ) ) {
							
								$user = get_userdata( $id );
								
								//Check 'Customer Display Name' in Subaccounts > Settings > Options and display it accordingly
								if ( ( $sfwc_option_display_name == 'full_name' ) && ( get_userdata($user->ID)->user_firstname || get_userdata($user->ID)->user_lastname ) ) { ?>
									
									optionValue = <?php echo esc_attr( $user->ID ); ?>;
									optionText = "<?php echo 'ID: ' . esc_html( $user->ID ) . ' - ' . esc_html( $user->user_firstname ) . ' ' . esc_html( $user->user_lastname ) . ' (' .  esc_html( $user->user_email ) . ')'; ?>";
									
									$('#sfwc-select-subaccount-orders').selectize()[0].selectize.addOption( {value: optionValue, text: optionText} );
									$('#sfwc-select-subaccount-orders').selectize()[0].selectize.addItem( optionValue );
									
								<?php
								} elseif ( ( $sfwc_option_display_name == 'company_name' ) && ( get_userdata($user->ID)->billing_company ) ) { ?>
									
									optionValue = <?php echo esc_attr( $user->ID ); ?>;
									optionText = "<?php echo 'ID: ' . esc_html( $user->ID ) . ' - ' . esc_html( $user->billing_company ) . ' (' .  esc_html( $user->user_email ) . ')'; ?>";
								...
								<?php
									...
									optionValue = <?php echo esc_attr( $user->ID ); ?>;
									optionText = "<?php echo 'ID: ' . esc_html( $user->ID ) . ' - ' . esc_html( $user->user_login ) . ' (' .  esc_html( $user->user_email ) . ')'; ?>";
					...
					$("#sfwc-select-subaccount-orders-form-wrap #sfwc-loading-icon").hide();
					...
					var selected_users = $('#sfwc-select-subaccount-orders').selectize()[0].selectize.getValue();
					...
					if( $account_level_type == 'manager' ){
						?>
						var currentAccountType = 'manager';
						ajaxAction = 'frontend_populate_subaccount_orders_dropdown_with_subaccounts';
						
						// Run function only on first click after page load.
						$("#sfwc-select-subaccount-orders-form-wrap .selectize-input").one("click", function () {
							getUsersPopulateSelectFieldOrders();
						});
						<?php
					}
					?>
					...
					function wrapGetUsersPopulateSelectFieldOrders() {
						
						$('#sfwc-select-subaccount-orders-form-wrap .selectize-dropdown-content').on('scroll', function() {
							if( $(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight ) {
								//console.log('end reached');
								if (endReached != true ) {
									getUsersPopulateSelectFieldOrders();
						...
						// Detect changes in preselected values.	
						var selectizeControl = $("#sfwc-select-subaccount-orders").selectize()[0].selectize;		
						selectizeControl.on('change', function() {
							selected_users = $('#sfwc-select-subaccount-orders').selectize()[0].selectize.getValue();
							//console.log(selected_users);
						});
						
						
						/**
						 * Trigger getUsersPopulateSelectFieldOrders() function some seconds after user has finished typing.
						 */
						 
						// Timer setup.
						var typingTimerSubaccountOrders;          		// Timer identifier.
						var doneTypingIntervalSubaccountOrders = 2000;  // Time in ms.
						var $inputSubaccountOrders = $('#sfwc-select-subaccount-orders-form-wrap .selectize-input input');
						
						// On keyup, start the countdown.
						$inputSubaccountOrders.on('keyup', function (e) {
							
							var TextboxValue = $('#sfwc-select-subaccount-orders').selectize()[0].selectize.getTextboxValue();
							...
							if (   e.key == "CapsLock"
								...
								clearTimeout(typingTimerSubaccountOrders);
								typingTimerSubaccountOrders = setTimeout(doneTypingSubaccountOrders, doneTypingIntervalSubaccountOrders);
							}
						});
						
						// On keydown, clear the countdown.
						$inputSubaccountOrders.on('keydown', function () {
						  clearTimeout(typingTimerSubaccountOrders);
						});

						// User has finished typing.
						function doneTypingSubaccountOrders() {
							search_val = $('#sfwc-select-subaccount-orders-form-wrap .selectize-input input').val();
							page = 0;
							getUsersPopulateSelectFieldOrders();
						}
					}
					wrapGetUsersPopulateSelectFieldOrders();
				
				
					/**
					 * Retrieve users via Ajax and populate Selectize form.
					 */
					function getUsersPopulateSelectFieldOrders() {
						...
						$.ajax({
							...
							beforeSend: function () {
								...
								// Temporarily disable dropdown items selection.
								$('#sfwc-select-subaccount-orders-form-wrap .selectize-dropdown-content').css({ 'opacity' : '0.5', 'pointer-events' : 'none' });
								
								// Show loading icon.
								$("#sfwc-select-subaccount-orders-form-wrap #sfwc-loading-icon").show();									
							},
							success: function (value) {									
								...
								if ( value.indexOf('</option>') > -1 ) {
									...
									// Store pre-selected items in array.
									var preSelectedValues = new Array();
									if ( selected_users && selected_users.length > 0 ) {										
										var i;
										for ( i = 0; i < selected_users.length; ++i ) {
											
											var text =  $('#sfwc-select-subaccount-orders').selectize()[0].selectize.getItem(selected_users[i]).text().replace('×', ''); // Remove × added from "remove_button" plugin.
											//var text =  $('#sfwc-select-subaccount-orders').selectize()[0].selectize.getItem(selected_users[i]).text();
											preSelectedValues[i]= { 'optVal': selected_users[i], 'optText': text }
										}
									}									
									
									// Destroy previous Selectize instance.
									$('#sfwc-select-subaccount-orders').selectize()[0].selectize.destroy();
									
									if ( search_val != null && page == 1 ) {
										
										$('select#sfwc-select-subaccount-orders').html('');
									}
									
									// Append new options.
									$('select#sfwc-select-subaccount-orders').append(value);
									...
									// Set value of search text (if any).
									if ( TextboxValue.length > 0 ) {
										$( '#sfwc-select-subaccount-orders' ).selectize()[0].selectize.setTextboxValue( TextboxValue );
									}
										
									// Add pre-selected items from the array.
									if ( preSelectedValues && preSelectedValues.length > 0 ) {										
										
										var i;
										for ( i = 0; i < preSelectedValues.length; ++i ) {
											
											// For pre-selected values it is necessary to add options in order to select them.
											$('#sfwc-select-subaccount-orders').selectize()[0].selectize.addOption( {value: preSelectedValues[i]['optVal'], text: preSelectedValues[i]['optText']} );
											$('#sfwc-select-subaccount-orders').selectize()[0].selectize.addItem( preSelectedValues[i]['optVal'] );
										}
									}
									
									// Refresh Selectize options.
									$('#sfwc-select-subaccount-orders').selectize()[0].selectize.refreshOptions();
									
									// Keep scroll position.
									var $container = $('#sfwc-select-subaccount-orders-form-wrap .selectize-dropdown-content');
									var $scrollTo = $('#sfwc-select-subaccount-orders-form-wrap div.first-item-loaded:last').prevAll().eq(6); // 6 = number of visible items in selectize dropdown without scrolling (7) minus 1.
									jQuery(document).ready(function(){
									...
									// Re-enable dropdown items selection.
									$('#sfwc-select-subaccount-orders-form-wrap .selectize-dropdown-content').css({ 'opacity' : '1', 'pointer-events' : '' });
 ...
 *
 * Sanitize, validate submitted data and show orders in: My Account -> Subaccounts -> Subaccount Orders.
 */
function sfwc_my_account_orders_query_subaccounts_orders( $args ) {
	...
	if ( isset( $_POST['sfwc_subaccount_orders'] ) ) {
		$selected_subaccount_ids_sanitized = preg_replace('/[^0-9]/', '', $_POST['sfwc_subaccount_orders']);	// sanitize_text_field() only works for strings, so this is not the case.
	...
	if ( is_wc_endpoint_url( 'subaccounts' ) ) {
		...
		if ( $account_level_type == 'manager' ) {
			...
			if ( ! isset( $selected_subaccount_ids_sanitized ) ) {
			...
			if ( isset( $selected_subaccount_ids_sanitized ) ) {
				...
				if ( ! isset( $_POST['sfwc_filter_subaccount_orders'] ) || isset( $_POST['sfwc_filter_subaccount_orders'] ) && ! wp_verify_nonce( $_POST['sfwc_filter_subaccount_orders'], 'sfwc_filter_subaccount_orders_action' ) ) {
					...
					if ( ! empty( $selected_subaccount_ids_sanitized ) ) {

						foreach ( $selected_subaccount_ids_sanitized as $key => $value ) {
...
function sfwc_set_selected_subaccount_orders_cookie() {
	...
	if ( isset( $_POST['sfwc_subaccount_orders'] ) ) {
		...
		// Sanitizing
		$selected_subaccount_ids_sanitized = preg_replace('/[^0-9]/', '', $_POST['sfwc_subaccount_orders']);
		
		// Set cookie as an array (json encoded).
		setcookie( apply_filters( 'sfwc_cookie_prefix', '' ) . 'subaccount_ids', wp_json_encode( $selected_subaccount_ids_sanitized ), [
			'expires' => '',
			'path' => COOKIEPATH,
			'domain' => COOKIE_DOMAIN,
			'secure' => is_ssl(),
		...
		/**
		 * The same applies for My Account -> Orders (Multi-User mode enabled).
		 */		
		elseif ( is_wc_endpoint_url( 'orders' ) && $sfwc_option_subaccount_mode == 'multi_user' ) {
			wp_safe_redirect( esc_url( wc_get_account_endpoint_url( 'orders' ) ) );
			exit;			
		}
	}
}
add_action('template_redirect', 'sfwc_set_selected_subaccount_orders_cookie');
...
function sfwc_modify_available_action_buttons( $actions, $order ){
	...
	if ( is_wc_endpoint_url( 'subaccounts' ) ) {
		...
		foreach ( $actions as $key => $action ) {
			...
			if ( 
				$key !== 'view_subaccount_order' &&
				$key !== 'edit_subaccount_order' // In case Order Approval Add-On is active.
			) {
				unset( $actions[$key] );
			}
		}
...
function sfwc_add_subaccounts_related_information_in_order_details_page( $order_id ) {
	...
	*/
	...
	/**
	#FOR DEBUG
	echo '<strong>order placed by user id:</strong>';
	var_dump((int)$order->get_meta( '_sfwc_order_placed_by_user_id' ));
	echo '<br>';
	echo '<strong>$order->get_user_id:</strong>';
	...
	**/
	...
	if ( $customer_id && ( sfwc_is_user_role_valid( $customer_id ) && sfwc_is_user_role_enabled( $customer_id ) ) ) {
		...
		echo '<div id="sfwc-order-details">';
		
		if ( $sfwc_option_subaccount_mode == 'sub_user' && sfwc_order_placed_during_subaccount_mode( $order->get_id() ) == 'sub_user' ) {
		...
		if ( $order_placed_by ) {
			...
			if ( $order_placed_by == 'manager_for_himself' ) {
				...
				echo '</span> ' . esc_html__( 'on behalf of', 'subaccounts-for-woocommerce' ) . ' ' . esc_html__( 'subaccount', 'subaccounts-for-woocommerce' ) . ' <span style="background:#fdfdfd; padding:3px; border:1px solid #ebebeb; border-radius:6px; white-space:nowrap;">';
					...
					if ( $sfwc_option_subaccount_mode == 'sub_user' || ( $sfwc_option_subaccount_mode == 'multi_user' ) ) {
						...
						if ( ( $sfwc_option_display_name == 'full_name' ) && ( get_userdata( $userdata_customer->ID )->user_firstname || get_userdata( $userdata_customer->ID )->user_lastname ) ) {
							...
							if ( get_userdata($userdata_manager->ID)->billing_company === get_userdata( $userdata_customer->ID )->billing_company ) {
				...
				if ( $sfwc_option_subaccount_mode == 'multi_user' ) {
					echo esc_html__( 'Subaccount', 'subaccounts-for-woocommerce' );
				} 
				elseif ( $sfwc_option_subaccount_mode == 'sub_user' ) {
					echo esc_html__( 'Customer', 'subaccounts-for-woocommerce' );
				}
				
				echo ' <span style="background:#fdfdfd; padding:3px; border:1px solid #ebebeb; border-radius:6px; white-space:nowrap;">';
			...
			elseif ( $order_placed_by == 'supervisor_for_himself' ) {
				...
				/**
				 * Below check is to prevent showing "on behalf of + customer name" in case an order is placed by a Supervisor on behalf of a Manager (or a Subaccount) in Multi-User mode, 
				 * but for some reasons the admin switches to Sub-User mode afterwards.
				 *
				 * If not checked the following would show up:
				 *
				 *		Order placed by:
				 *		Supervisor (John Doe) on behalf of Manager (John Doe) 		// which of course doesn't make sense.
				 */
				if ( sfwc_order_placed_during_subaccount_mode( $order_id ) == 'multi_user' && $sfwc_option_subaccount_mode == 'multi_user' ) {

					echo esc_html__( 'on behalf of', 'subaccounts-for-woocommerce' ) . ' ' . esc_html__( 'Manager', 'subaccounts-for-woocommerce' ) . ' <span style="background:#fdfdfd; padding:3px; border:1px solid #ebebeb; border-radius:6px; white-space:nowrap;">';
				...
				/**
				 * Below check is to prevent showing "on behalf of + customer name" in case an order is placed by a Supervisor on behalf of a Subaccount in Multi-User mode, 
				 * but for some reasons the admin switches to Sub-User mode afterwards.
				 *
				 * If not checked the following would show up:
				 *
				 *		Order placed by:
				 *		Supervisor (John Doe) on behalf of Subaccount (John Doe) 		// which of course doesn't make sense.
				 */
				if ( sfwc_order_placed_during_subaccount_mode( $order_id ) == 'multi_user' && $sfwc_option_subaccount_mode == 'multi_user' ) {

					echo esc_html__( 'on behalf of', 'subaccounts-for-woocommerce' ) . ' ' . esc_html__( 'subaccount', 'subaccounts-for-woocommerce' ) . ' <span style="background:#fdfdfd; padding:3px; border:1px solid #ebebeb; border-radius:6px; white-space:nowrap;">';
 ...
 * 
 * Make "Billing Firstname", "Billing Lastname" and "Billing Email" checkout fields not editable.
 */
 function sfwc_multi_user_make_certain_fields_readonly( $checkout_fields ) {
	...
	if ( ! is_admin() && is_user_logged_in() && sfwc_is_current_user_role_valid() && sfwc_is_current_user_role_enabled() ) { 	// Do not check
																																// ! wp_doing_ajax() 
																																// here, otherwise Manager's address details
																																// will not be saved in place of subaccount's address details.
		...
		*/
		...
		if ( ! empty( $user_query_manager->get_results() ) ) {
			...
			return $checkout_fields;	// If the user being edited has not a Manager above him, there's no need to proceed and checkout fields can be normally edited.
		}
		...
		if ( $user_account_level_type != 'supervisor' && $user_account_level_type != 'manager' ) {
			...
			foreach ( $checkout_fields['billing'] as $key => $field ){
				...
				/**
				 * Email is mandatory when creating an account, so it is present for sure.
				 *
				 * Billing First Name and Billing Last Name instead, are not mandatory when adding new users (customers) from the backend.
				 * So those values could not be present at checkout. For this reason we need to check that those values exist before making the fields readonly.
				 */
				if ( $key == 'billing_email' || ( $key == 'billing_first_name' && $manager_billing_first_name !== '' )  || ( $key == 'billing_last_name' && $manager_billing_last_name !== '' )  ) {

					$checkout_fields['billing'][$key]['custom_attributes'] = array( 'readonly'=>'readonly' ); // Using 'disabled' will cause validation error during checkout.
				}
			}
		}
	}
...
add_action( 'woocommerce_checkout_fields','sfwc_multi_user_make_certain_fields_readonly' );
...
/**
 * Multi-User mode A.K.A. Company Accounts (Pt.4).
 * 
...
/**
 * Multi-User mode A.K.A. Company Accounts (Pt.5).		// Maybe integrate this function with: sfwc_my_account_orders_query_subaccounts_orders
 * 
 * Sanitize, validate submitted data and show orders in: My Account -> Orders for Managers.
 *
 * See also: sfwc_multi_user_my_account_orders_query_subaccounts_orders_for_supervisors function in Supervisor Add-On.
 */
function sfwc_multi_user_my_account_orders_query_subaccounts_orders( $args ) {
	...
	if ( is_wc_endpoint_url( 'orders' ) && $sfwc_option_subaccount_mode == 'multi_user' ) {	// <--- NOT is_wc_endpoint_url( 'subaccounts' ) here,
		...
		if ( $account_level_type == 'manager' ) {
			...
			/**
			 * If the Supervisor Add-On is active, include the user ID of the Supervisor.
			 *
			 * In case the Supervisor Add-On is active, when a Manager or a Subaccount places an order it will be assigned to the Supervisor afterwards.
			 * For this reason it is necessary to include the user ID of the Supervisor among the IDs of the users to show orders for.
			 */
			if ( sfwc_is_plugin_active( 'sfwc-supervisor-addon.php' ) ) {
				...
				if ( ! empty( $user_query_supervisor->get_results() ) ) {
					...
					array_push( $existing_children_ids, $supervisor_id );
				}
			}
			...
			if ( ! isset( $selected_subaccount_ids_sanitized ) ) {
				...
				if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
					...
					if( isset( $supervisor_id ) ) {
						...
						$excluded_order_ids = wc_get_orders( array(
							'return'		=> 'ids', // Return order IDs.
							'limit'			=> -1,
							'customer_id' 	=> $supervisor_id,
							'meta_query' 	=> array(
								'relation'	=> 'OR',
								array( // Select those orders placed by Supervisor for himself in Sub-User mode (_sfwc_order_placed_by_user_id is missing). 								hides #140
									'key' 		=> '_sfwc_order_placed_by_user_id',
									'compare'	=> 'NOT EXISTS',
								),
								array( // Select those orders placed by Supervisor for himself in Multi-User mode (_sfwc_order_placed_by_user_id equals to user ID of Supervisor). 			hides #146
									'key' 		=> '_sfwc_order_placed_by_user_id',
									'value' 	=> $supervisor_id,
									'compare'	=> 'LIKE',
								),
					...
					if( isset( $supervisor_id ) ) {
						...
						$excluded_order_ids = get_posts( array(
							'post_type'		=> 'shop_order',
							'fields'		=> 'ids', // Return order IDs.
							'post_status'	=> array_keys( wc_get_order_statuses() ),
							'numberposts'	=> -1,
							'meta_query' 	=> array(
								'relation'	=> 'AND',
								array( // Select all orders of Supervisor.
									'key'		=> '_customer_user',
									'value'		=> $supervisor_id,
									'compare'	=> 'LIKE',
								),
				...
				$args['customer'] = $existing_children_ids; // Initially show all results: both Manager and subaccount orders
															// and even those orders placed by a Manager or by a Subaccount 
															// and assigned to the Supervisor afterwards (Multi-User mode) if/when Supervisor Add-On is active.
					
				if( ! empty( $excluded_order_ids) ) {
					// We need to exclude all those orders placed by a Supervisor for himself during both Sub-User and Multi-User mode.
					// This because by default a Manager should not be able to see those orders.
			 ...
			 * Only orders that belong to the currently logged in parent account ID or orders that belong to the parent's subaccount IDs are valid.
			 */
			if ( isset( $selected_subaccount_ids_sanitized ) || isset( $subaccount_ids_cookie_sanitized ) ) {
					
				$validated_subaccount_orders = array();
				$not_valid_subaccount_orders = array();
				
				// Validate posted values.
				if ( isset( $selected_subaccount_ids_sanitized ) && ! empty( $selected_subaccount_ids_sanitized ) ) {
					...
					foreach ( $selected_subaccount_ids_sanitized as $key => $value ) {
						...
						// Check if submitted data is valid.
						if ( in_array( $value, $existing_children_ids, true ) || $value == $user_id ) {	// $existing_children_ids should already include $user_id,
																										// but for some reasons fails to validate.
																										// So keep there: $value == $user_id.
				...
				if ( ! empty( $validated_subaccount_orders ) ) {
					...
					if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
						...
						/**
						 * Retrieve orders placed by subaccounts that have been assigned to the Manager after successful order.
						 *
						 * Retrieve them by checking _sfwc_order_placed_by_user_id meta value.
						 */
						$order_ids = wc_get_orders( array(
							...
							'meta_query' 	=> array(
								array(
									'key' 		=> '_sfwc_order_placed_by_user_id',
									'value' 	=> $validated_subaccount_orders,
									'compare'	=> 'IN',
								)
							)
						) );
						...
						/**
						 * Retrieve orders placed by subaccounts before enabling Multi-User mode and/or orders directly tied to the selected user.
						 */
						$order_ids_previous = wc_get_orders( array(
							'return' => 'ids', // Return order IDs.
							'limit' => -1,
							'customer_id' => $validated_subaccount_orders,
						) );


						// Marge orders.
						$all_order_ids = array_merge( $order_ids, $order_ids_previous );
						
						

						if ( ! empty( $all_order_ids ) ) {
							$args['customer'] = $existing_children_ids; // Includes Manager ID and subaccount IDs.
							$args['post__in'] = $all_order_ids;
						}
						else {
							$args['customer'] = false;	// If the search did not produce any results, don't show any orders.
						}
		...
		elseif ( $account_level_type !== 'manager' && $account_level_type !== 'supervisor' && ! sfwc_is_plugin_active( 'sfwc-supervisor-addon.php' ) ) { // In case Supervisor Add-On is active, see:
			...
			*/
			...
			if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
				...
				/**
				 * Retrieve orders placed by the subaccount that have been assigned to the Manager after successful order.
				 *
				 * Retrieve them by checking _sfwc_order_placed_by_user_id meta value.
				 */
				$order_ids = wc_get_orders( array(
				...
				/**
				 * Retrieve orders placed by subaccount before enabling Multi-User mode.
				 *
				 * In this case orders are still tied to the subaccount itself (they were not assigned to the Manager afterwards).
				 */
				$order_ids_previous = wc_get_orders( array(
					'return' => 'ids', // Return order IDs.
					'limit' => -1,
				...
				// Marge orders.
				$all_order_ids = array_merge( $order_ids_previous, $order_ids );
				

				$args['customer'] = array( $manager_id, $user_id );
				$args['post__in'] = $all_order_ids;
				 ...
				 * Retrieve them by checking _sfwc_order_placed_by_user_id meta value.
				 *
				 * Also retrieve orders placed by subaccount before enabling Multi-User mode.
				 * In this case orders are still tied to the subaccount itself (they were not assigned to the Manager afterwards).
				 */
				$order_ids = get_posts( array(
					'post_type'		=> 'shop_order',
...
/**
 * Multi-User mode A.K.A. Company Accounts (Pt.6).
 * 
 * Set permissions to view orders on front end when Multi-User mode is enabled.
 *
 * See also function: sfwc_multi_user_allow_subaccount_to_view_manager_orders_supervisor
...
function sfwc_multi_user_allow_subaccount_to_view_manager_orders( $allcaps, $caps, $args, $user ) {
	...
	if ( $sfwc_option_subaccount_mode == 'multi_user' && is_wc_endpoint_url( 'view-order' ) ) {
		...
		if ( is_user_logged_in() && sfwc_is_current_user_role_valid() && sfwc_is_current_user_role_enabled() ) {
			 ...
			 */
			if ( $account_level_type !== 'manager' && $account_level_type !== 'supervisor' ) {
			...
			/**
			 * Logged user is a Manager.
			 * 
			 * Allow the Manager to view Subaccount orders in: My Account -> Orders -> View, in case those orders were placed before Multi-User mode was enabled.
			 *
			 * Orders placed by a subaccount before Multi-User mode was enabled are still tied to the subaccount (they were not assigned to the Manager afterwards).
			 *
			 * For this reason it is necessary to guarantee the Manager the appropriate permissions to be able to view the order.
			 */
			elseif ( $account_level_type == 'manager' ) {
				
				// Get children (array) of currently logged in user.
				$children_ids = get_user_meta( $user_id, 'sfwc_children', true );
					
				// Check if selected order is tied to a subaccount of currently logged Manager.
				if ( ! empty( $children_ids ) && in_array( strval($customer_id), $children_ids, true ) ) {
...
/**
 * Change the recipient in WC customer emails by replacing the Manager email with the subaccount email (Pt.1).
 * 
 * In Multi-User mode, whenever a subaccount places an order, the order is assigned to the Manager afterwards.
 * For this reason the order email notifications sent to the customer are sent to the Manager. However we need to prevent this behavior.
 *
 * What we do with this functions is: whenever a subaccount places an order (with Multi-User mode enabled), send order notifications to the subaccount itself,
 * indipendently from who the order is assigned to afterwards.
 */
function sfwc_multi_user_force_order_notifications_to_be_sent_to_subaccount( $recipient, $order = false ) {
	...
	// Get Account Level Type of currently logged in user (Superviore | Manager | Default).
	$account_level_type = get_user_meta( $order_placed_by_user_id, 'sfwc_account_level_type', true );
	...
	if ( $sfwc_option_subaccount_mode == 'multi_user' ) {
		...
		if ( $order_placed_by_user_id ) { 	// There is no user ID available for guest customers, 
											// so this control ensures that the order confirmation email is sent to the guest customer as normal.
		
			// Make sure the user is not a parent.
			if( $account_level_type !== 'supervisor' && $account_level_type !== 'manager' ) {
				
				$subaccount_object = new WC_Customer( $order_placed_by_user_id );
				
				// If no Billing Email is set, use Email of the user as a fallback.
				$subaccount_email = '' !== $subaccount_object->get_billing_email() ? $subaccount_object->get_billing_email() : $subaccount_object->get_email();
				
				$subaccount_billing_first_name = $subaccount_object->get_billing_first_name();

				// Set subaccount email as recipient.
				$recipient = $subaccount_email;
			}
			
			add_filter( 'wp_mail', function( $args ) use ( $order_id ) {
					return sfwc_swap_manager_first_name_with_subaccount_first_name( $args, $order_id );
				}, 10, 1
			);
		}
	}
...
// Composite hook: 'woocommerce_email_recipient_' . $email_id
add_filter( 'woocommerce_email_recipient_customer_on_hold_order', 'sfwc_multi_user_force_order_notifications_to_be_sent_to_subaccount', 10, 2 );
add_filter( 'woocommerce_email_recipient_customer_processing_order', 'sfwc_multi_user_force_order_notifications_to_be_sent_to_subaccount', 10, 2 );
add_filter( 'woocommerce_email_recipient_customer_completed_order', 'sfwc_multi_user_force_order_notifications_to_be_sent_to_subaccount', 10, 2 );
add_filter( 'woocommerce_email_recipient_customer_partially_refunded_order', 'sfwc_multi_user_force_order_notifications_to_be_sent_to_subaccount', 10, 2 );
add_filter( 'woocommerce_email_recipient_customer_invoice', 'sfwc_multi_user_force_order_notifications_to_be_sent_to_subaccount', 10, 2 );
add_filter( 'woocommerce_email_recipient_customer_note', 'sfwc_multi_user_force_order_notifications_to_be_sent_to_subaccount', 10, 2 );




/**
 * Change the recipient in WC customer emails by replacing the Manager email with the subaccount email (Pt.2).
 * 
 * In the "Hi Name," part of the email, swap the Manager first name with the subaccount first name.
 */
function sfwc_swap_manager_first_name_with_subaccount_first_name( $args, $order_id ) {
			
	// Get the order object.
	$order = wc_get_order( $order_id );
	
	// Retrieve the Manager's first name from the order object (in Multi-User mode the order is assigned to the Manager afterwards).
	$billing_first_name = $order->get_billing_first_name();
	
	// Retrieve the ID of the customer who did actually place the order (Multi-User mode).
	$order_placed_by_user_id = $order->get_meta( '_sfwc_order_placed_by_user_id' );
	
	// Get the subaccount user object.
	$subaccount_object = new WC_Customer( $order_placed_by_user_id );
	
	// Retrieve the subaccount's first name from the user object.
	$subaccount_billing_first_name = $subaccount_object->get_billing_first_name();
	
	/**
	 * Replace the Manager first name only in the "Hi Name," part of the email.
	 * Better not to just replace the first name:
	 * 
	 *		strpos( $args['message'], $billing_first_name )
	 *
	 * This to prevent replacing wrong strings in the email, e.g. cases where the customer's name is Violet
	 * but that's also the value of a color attribute.
	 */
	if ( strpos( $args['message'], 'Hi '. $billing_first_name . ',' ) !== false ) {
		$args['message'] = str_replace( 'Hi '. $billing_first_name . ',', 'Hi '. $subaccount_billing_first_name . ',', $args['message'] );
	}
