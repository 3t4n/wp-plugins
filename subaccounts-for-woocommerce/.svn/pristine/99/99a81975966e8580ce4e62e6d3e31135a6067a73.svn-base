			...
			if ( defined( 'SFWC_VALID_ROLE_LOOSE_MODE' ) && SFWC_VALID_ROLE_LOOSE_MODE == true ) {
				...
				if ( ! empty( $all_user_capabilities ) ) {
					...
					if ( array_key_exists( 'read', $all_user_capabilities ) && ! array_key_exists( 'edit_posts', $all_user_capabilities ) ) {
						...
						/**
						 * Filter hook: sfwc_add_valid_roles.
						 *
						 * Allow developers to enable additional roles for the subaccount system.
						 *
						 * @param	array	$current_user_roles		Array containing valid user roles.
						 * @since	1.6.0
						 */
						return apply_filters( 'sfwc_add_valid_roles', $current_user_roles );
		...
		if ( defined( 'SFWC_VALID_ROLE_LOOSE_MODE' ) && SFWC_VALID_ROLE_LOOSE_MODE == true ) {
			...
			if ( ! empty( $all_user_capabilities ) ) {
				...
				if ( array_key_exists( 'read', $all_user_capabilities ) && ! array_key_exists( 'edit_posts', $all_user_capabilities ) ) {
					 ...
					 * Allow developers to enable additional roles for the subaccount system.
					 *
					 * @param	array	$user_roles		Array containing valid user roles.
					 * @since	1.6.0
					 */
					return apply_filters( 'sfwc_add_valid_roles', $user_roles );
...
/**
 * Helper function to determine which roles are valid.
 *
 * If valid roles are found, this function returns an array containg all valid roles, otherwise returns false.
 *
 * Note: If the Loose Method is enabled in wp-config.php, the roles made valid via the "sfwc_add_valid_roles" filter are also returned.
 *
 * @since 1.6.0
 */
function sfwc_get_valid_roles() {
	...
	if ( ! empty( $roles ) ) {
		...
		if ( defined( 'SFWC_VALID_ROLE_LOOSE_MODE' ) && SFWC_VALID_ROLE_LOOSE_MODE == true ) {
			...
			if ( ! empty( $valid_roles ) ) {
				 ...
				 * @since	1.6.0
				 */
				return apply_filters( 'sfwc_add_valid_roles', $valid_roles );
...
/**
 * Helper function to determine if an order was placed while the plugin was in Sub-User or Multi-User mode (by passing $order_id).
 *
 * @since 1.4.4
 *
 * @return string 'sub_user' || 'multi_user'.
 *
 * ---------
 * Important
 * ---------
 ...
 * Always check with:
 *
 *	sfwc_order_placed_during_subaccount_mode( $order_id ) == 'sub_user'
 *	sfwc_order_placed_during_subaccount_mode( $order_id ) == 'multi_user'
 *
 * and not with:
 *
 *	sfwc_order_placed_during_subaccount_mode( $order_id ) !== 'sub_user'
 *	sfwc_order_placed_during_subaccount_mode( $order_id ) !== 'multi_user'
 *
 * If checked with the latter method, the statement will result TRUE also for NULL values, see below (*).
 */
function sfwc_order_placed_during_subaccount_mode( $order_id ) {
	
	$order = new WC_Order( $order_id );
		
	// Retrieve the currently active 'Subaccount Mode' from Options settings.
	...
	// Retrieve the active subaccount mode at the time the order is placed.
	$order_placed_during_subaccount_mode = $order->get_meta( '_sfwc_order_placed_during_subaccount_mode' );
	...
	if ( ! empty( $order_placed_during_subaccount_mode ) ) {
		
		if ( $order_placed_during_subaccount_mode == 'sub_user' ) {
			return 'sub_user';
		} else {
			return 'multi_user';
		}
		...
		/**
		 * _sfwc_order_placed_during_subaccount_mode is stored in order meta since v.1.4.4,
		 * so may be empty for previuosly placed orders.
		 *
		 * For previuosly placed orders, we can check if _sfwc_order_placed_by_user_id order meta is present.
		 *
		 * From v. 1.4.0 (Multi-User mode feature released as beta) to 1.4.2 _sfwc_order_placed_by_user_id was stored in the order meta
		 * only in case multi-user mode was enabled.
		 * So if this meta value is empty it means the order was placed while Sub-User mode was enabled.
		 */
		if ( empty( $order_placed_by_user_id ) ) {
			return 'sub_user';
		} else {
			
			/**
			 * Here we need to handle the situation where an order was placed with plugin v.1.4.3 installed,
			 * where _sfwc_order_placed_by_user_id is always stored (both in Sub-User and Multi-User modes) 
			 * and we have no _sfwc_order_placed_during_subaccount_mode meta to rely on.
			 */
			if ( (int)$order_placed_by_user_id !== $customer_id ) {
				/**
				 * In Sub-User mode $order_placed_by_user_id and $customer_id cannot be different.
				 * So if they are different the order was certainly placed while Multi-User mode was enabled.
				 */
				return 'multi_user';
			}
			
			/**
			 * (*) There are only 3 types of orders that we are unable to understand whether they were placed in Multi-User or Sub-User mode:
			 *
			 *		- Orders placed by a Manager for himself
			 *		- With plugin version 1.4.3
			 *		- And Supervisor Add-On not installed/active
			 *
			 *		- Orders placed by a Manager for himself
			 *		- With plugin version 1.4.3
			 *		- And Supervisor Add-On installed/active, but the manager does not have a Supervisor above him
			 *
			 *		- Orders placed by a Supervisor for himself
			 *		- With plugin version 1.4.3
			 *		- And Supervisor Add-On installed/active
			 *
			 * In all three situations above: $order_placed_by_user_id === $customer_id regardless of whether Sub-User or Multi-User
			 * mode was enabled at the time the order was placed.
			 *
			 * However, it is not necessary to distinguish the subaccount mode for the 3 types of orders above, since the relevant
			 * metadata is valid on both modes.
			 */
			else {
				return $sfwc_option_subaccount_mode;
			}
		}
	}
}
...
/**
 * Generic user query for subaccounts selection.
 *
 * Generic User Query to display users list for possible selection as subaccounts.
 *
 * From version 1.6.2, below function has been moved here from Subaccounts Pro,
 * so it's best to check that the function doesn't already exist.
 */
if ( ! function_exists( 'sfwc_query_users_for_possible_selection_as_subaccounts' ) ) {
	function sfwc_query_users_for_possible_selection_as_subaccounts( $meta_query_value = array( 'supervisor' ), $meta_query_operator = 'NOT IN', $meta_query_key_existence = 'EXISTS', $meta_query_relation = 'AND', $page = 1 ) {
		...
		// Retrieve (Ajax) user_id of customer which is currently being edited on user-edit.php
		$user_idx = isset( $_POST['ajax_user_id'] ) ? absint( $_POST['ajax_user_id'] ) : absint( $_GET['user_id'] );
		...
		$args_customers = array(
			...
			'role__in' => $sfwc_option_selected_roles,		// Only user roles enabled from plugin settings.
			'exclude' => $user_idx,							// Exclude Customer which admin is currently editing.
			//'exclude' => $list_of_children_for_all_users,	// Exclude users which already have parents set above them. Insted of excluding them, decided to make them 'disabled' (see below).
			'orderby' => 'ID',
			'order' => 'ASC',
			...
			'meta_query' => array(
				...
				'relation' => $meta_query_relation,			// This will be 'OR' only when Manager radio button is selected, to show also users where the 'sfwc_account_level_type' is missing in DB (*).
				array(
					'key' => 'sfwc_account_level_type',		// All customers with meta: Account Level Type = $meta_query_value
					'value' => $meta_query_value,
					'compare' => $meta_query_operator,
				),
				array(
					// (*) After a new user is created (but its profile has not been updated yet) the 'sfwc_account_level_type' is missing in DB,
					// but we need to include this user for possible selection as subaccount in case 'Manager' radio button is selected (while this shoud not be included if 'Supervisor' radio button is selected).
					'key' => 'sfwc_account_level_type',
					'compare' => $meta_query_key_existence,	// This will be 'NOT EXISTS' only when Manager radio button is selected.
				)
		...
		if( $ajax_search ) {
			...
			$args_customers['search'] = '*'.esc_attr( $ajax_search ).'*';
			
			$args_customers['search_columns'] = array(
					'user_login',
					'user_nicename',
					'user_email',
					'user_url',
		...
		return new WP_User_Query( $args_customers );
	}
}
...
function sfwc_add_meta_box_content( $order ) { // Leave $order, otherwise getting error on checkout page while placing an order.
	...
	if ( is_admin() ) { // Admin side only otherwise emails won't get sent.
	...
	if ( $customer_id && ( sfwc_is_user_role_valid( $customer_id ) && sfwc_is_user_role_enabled( $customer_id ) ) ) {
		...
		echo '<p style="background:' . esc_attr( $order_placed_by_css_background ) . '; border-radius: 3px; padding: 8px;"><strong>' . esc_html__( 'Order placed by', 'subaccounts-for-woocommerce' ) . ':</strong><br>';
