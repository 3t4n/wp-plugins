...
if (isset($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'], 'save')) {
    if (isset($_POST['options'])) {
        ...
        if (isset($_POST['DXOCapabilitiesEditDefault']) && $_POST['DXOCapabilitiesEditDefault'] == 1) {
            $optionsWP['DXOCapabilitiesEditDefault'] = 1;
            $options['DXOCapabilitiesEditDefault'] = 1;
            //echo "Modificado a editar SI </br>";
        } else {
            $optionsWP['DXOCapabilitiesEditDefault'] = 0;
            $options['DXOCapabilitiesEditDefault'] = 0;
            //echo "Modificado a editar NO </br>";
        }

        if (isset($_POST['DXOCapabilitiesDownloadDefault']) && $_POST['DXOCapabilitiesDownloadDefault'] == 1) {
            $optionsWP['DXOCapabilitiesDownloadDefault'] = 1;
            $options['DXOCapabilitiesDownloadDefault'] = 1;
            //echo "Modificado a descargar SI </br>";

        } else {
            $optionsWP['DXOCapabilitiesDownloadDefault'] = 0;
            $options['DXOCapabilitiesDownloadDefault'] = 0;
            //echo "Modificado a download NO </br>";
        }
...
<div class="wrap content DXOptionsConf">
    ...
    <div class="updated notice"
         id="conf_updated" <?php if (!isset($_POST['_wpnonce'])) echo 'style="display: none"'; ?>>
    ...
    <form action="" method="post">
        ...
        <h2 class="nav-tab-wrapper">
            <a href="#" class="nav-tab nav-tab-active" id="tab_1" onclick="return false"><i
                        class="fa fa-plug"> </i> <?php _e('Docxpresso Connection', 'document-data-automation'); ?></a>
            <a href="#" class="nav-tab" id="tab_2" onclick="return false"><i
                        class="fa fa-gear"> </i> <?php _e('General settings', 'document-data-automation'); ?></a>
            <a href="#" class="nav-tab" id="tab_3" onclick="return false"><i
                        class="fa fa-paint-brush"> </i> <?php _e('Styles', 'document-data-automation'); ?></a>
        </h2>

        <div id="DXOConnection" style="margin-left: 12px; margin-top: 20px">
            <p>
                <strong><i class="fa fa-globe"> </i> <?php _e('Associated Docxpresso Installation', 'document-data-automation'); ?>
                    :</strong>
                <input class="regular-text" type="text" id="dxoURL" name="options[DocxpressoUrl]"
                       value="<?php if (isset($options['DocxpressoUrl'])) {
                           echo $options['DocxpressoUrl'];
                       } ?>" placeholder="https://alias.docxpresso.net">
                <button class="button button-primary" id="testConnection" onclick="return false"><i
                            class="fa fa-plug"> </i> <?php _e('Test connection', 'document-data-automation'); ?>
                </button>
                <br/>
                <small><?php _e('Introduce the full URL of your Docxpresso SaaS installation.', 'document-data-automation'); ?></small>
                <br/>
                <small><?php _e('If you do not have an account yet you may request a free one in:', 'document-data-automation'); ?>
                    <a href="https://saas.docxpresso.net">https://saas.docxpresso.net</a></small>
            </p>
            <p><strong><i class="fa fa-lock"> </i> <?php echo _e('Private API Key', 'document-data-automation'); ?>
                    :</strong>
                <input class="regular-text" type="text" id="dxoKey" name="options[pKey]"
                       value="<?php if (isset($options['pKey'])) {
                           echo $options['pKey'];
                       } ?>" placeholder="private key" style="width: 36em">
                <br/>
                <small><?php _e('Introduce the API Key of your Docxpresso SaaS installation (you can find it under the Configuration > Account menu entry).', 'document-data-automation'); ?></small>
            </p>
            <p><strong><i class="fa fa-at"> </i> email:</strong>
                <input class="regular-text" type="text" id="dxoEmail" name="options[email]"
                       value="<?php if (isset($options['email'])) {
                           echo $options['email'];
                       } ?>" placeholder="Docxpresso registration email">
                <br/>
                <small><?php _e('Introduce your Docxpresso registration email or any other email with ADMIN permissions in your Docxpresso SaaS installation.', 'document-data-automation'); ?></small>
            </p>
        </div>

        <div id="DXOSettings" style="display: none; margin-left: 12px; margin-top: 20px">
            <p style="margin-bottom: 0.4em"><strong><i
                            class="fa fa-globe"> </i> <?php _e('Redirect URL', 'document-data-automation'); ?>:</strong>
                <select id="redirectType" name="options[dxoRedirectType]">
                    <option value="external" <?php if (isset($options['dxoRedirectType']) && $options['dxoRedirectType'] == 'external') {
                        echo 'selected';
                    } ?>><?php _e('External URL', 'document-data-automation'); ?></option>
                    <option value="internal" <?php if (!isset($options['dxoRedirectType']) || (isset($options['dxoRedirectType']) && $options['dxoRedirectType'] == 'internal')) {
                        echo 'selected';
                    } ?>><?php _e('WordPress page', 'document-data-automation'); ?></option>
                </select>
                <input hidden id="dxoRedirectURL" name="options[dxoRedirectURL]"
                       value="<?php if (isset($options['dxoRedirectURL'])) {
                           echo $options['dxoRedirectURL'];
                       } ?>"/>
            </p>
            <p id="externalURL"
               style="margin-left: 30px; margin-top: 0.3em; <?php if (!isset($options['dxoRedirectType']) || (isset($options['dxoRedirectType']) && $options['dxoRedirectType'] == 'internal')) {
                   echo 'display: none';
               } ?>"><i class="fa fa-link"> </i> <strong><?php _e('External URL', 'document-data-automation'); ?>
                    :</strong>
                <input class="regular-text" type="text" id="externalRedirectURL" name="externalRedirectURL"
                       value="<?php if (isset($options['dxoRedirectURL']) && (isset($options['dxoRedirectURL']) && $options['dxoRedirectType'] == 'external')) {
                           echo $options['dxoRedirectURL'];
                       } ?>" placeholder="http://redirect.mydomain.com">
                <br/>
                <small><?php _e('Choose an optional external default page were the users will be redirected after fulfilling a document.', 'document-data-automation'); ?>
                    <br></small>
            </p>
            <p id="internalURL"
               style="margin-left: 30px; margin-top: 0.3em; <?php if (isset($options['dxoRedirectType']) && $options['dxoRedirectType'] == 'external') {
                   echo 'display: none';
               } ?>"><i class="fa fa-wordpress"> </i>
                <strong> <?php _e('WordPress page', 'document-data-automation'); ?>:</strong>
                <select id="internalRedirectURL" name="internalRedirectURL">
                    <?php echo $select; ?>
                </select>
                <br/>
                <small><?php _e('Choose an optional default WordPress page were the users will be redirected after fulfilling a document.', 'document-data-automation'); ?>
                    <br></small>
            </p>
            <p><strong><i class="fa fa-bullhorn"> </i> Default message:</strong><br>
                <textarea id="dxoMessage" name="options[dxoMessage]" cols="80"
                          rows="2"><?php if (isset($options['dxoMessage'])) {
                        echo $options['dxoMessage'];
                    } ?></textarea>
                <br/>
                <small><?php _e('Choose a default message to be shown to your users after fulfilling the document.', 'document-data-automation'); ?></small>
            </p>
            <p><strong><i class="fa fa-user"> </i> <?php _e('User Capabilites','document-data-automation') ?>:</strong>
            <ul>
                <li><input type="checkbox" id="DXOCapabilitiesEditDefault"
                           name="DXOCapabilitiesEditDefault" <?php if (isset($options['DXOCapabilitiesEditDefault']) && $options['DXOCapabilitiesEditDefault'] == 1) {
                        echo "checked";
                    } ?> value="1">
                    <label> Edit</label>
                </li>
                <li><input type="checkbox" id="DXOCapabilitiesDownloadDefault"
                           name="DXOCapabilitiesDownloadDefault" <?php if (isset($options['DXOCapabilitiesDownloadDefault'])) {
                        if ($options['DXOCapabilitiesDownloadDefault'] == 1) {
                            echo "checked";
                        }
                    } else {
                        echo "checked";
            ...
            <p>
                <?php _e('Set default capabilites from non administrator users','document-data-automation') ?>
            </p>
        </div>

        <div id="DXOStyles" style="display: none; margin-left: 12px; margin-top: 20px">
            <p style="margin-bottom: 0.4em"><strong><i
                            class="fa fa-paint-brush"> </i> <?php _e('Document/Web form frame style', 'document-data-automation'); ?>
                    :</strong>
            <p><span><?php _e('Border', 'document-data-automation'); ?>: </span>
                ...
                <input hidden id="dxoFrameStyle" name="options[dxoFrameStyle]"
                       value="<?php if (isset($options['dxoFrameStyle'])) {
                           echo $options['dxoFrameStyle'];
                       } else {
                           echo $defaultStyleValues['frameStyle'];
                       } ?>"/>
            </p>

            <p style="margin-bottom: 0.4em"><strong><i
                            class="fa fa-paint-brush"> </i> <?php _e('Message box style', 'document-data-automation'); ?>
                    :</strong>
            <p><span><?php _e('Border', 'document-data-automation'); ?>: </span>
            ...
            <p><span><?php _e('Background color', 'document-data-automation'); ?>: </span>
                <input class="regular-text color-field DXOframeStyles" type="text" id="backgroundMessageColor"
                       value=""/>
            </p>
            ...
            <input hidden id="dxoMessageStyle" name="options[dxoMessageStyle]"
                   value="<?php if (isset($options['dxoMessageStyle'])) {
                       echo $options['dxoMessageStyle'];
                   } else {
                       echo $defaultStyleValues['messageBox'];
                   } ?>"/>
        </div>

        <p class="submit">
            <button class="button button-primary" type="submit" name="save"><i
                        class="fa fa-save"> </i> <?php _e('Save Configuration', 'document-data-automation'); ?></button>
        </p>
    </form>
</div>
<!-- /div-->
...
<script>
    ...
    jQuery(function ($) {
        ...
        function removeTrailingBars(){
            var dxoURL = $('#dxoURL').val();
            var newURL = dxoURL.replace(/\/$/, "");
            $('#dxoURL').val(newURL);
        }
