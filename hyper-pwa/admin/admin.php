...
class HyperPWAAdmin
{
	...
	private function get_tab()
	{
		...
		if ( 'manifest' === $tab || 'service-worker' === $tab || 'a2hs' === $tab || 'faq' === $tab || 'subscription' === $tab || 'custom-dev' === $tab || 'contact-us' === $tab )
		{
...
<div class="wrap">
	...
	public function field_callback( $args )
	{
			...
			case 'multiselect':
				if ( ! empty( $args['options'] ) && is_array( $args['options'] ) )
				{
					...
					foreach ( $args['options'] as $key => $label )
					{
						...
						{
							$options_markup .= sprintf( '<option value="%s">%s</option>', esc_attr( $key ), esc_html( $label ) );
						}
					}
					...
					printf( '<select name="%1$s[]" id="%1$s" %2$s>%3$s</select>', esc_attr( $args['uid'] ), esc_attr( $attributes ), $options_markup );
				}
				break;
			...
			case 'checkbox':
				if ( ! empty( $args['options'] ) && is_array( $args['options'] ) )
				{
					...
					foreach ( $args['options'] as $key => $label )
					{
						...
						if ( ! empty( $value ) )
						{
							$options_markup .= sprintf( '<input id="%1$s_%6$s" name="%1$s[]" type="%2$s" value="%3$s" %4$s /><label for="%1$s_%6$s"> %5$s</label><br>', esc_attr( $args['uid'] ), esc_attr( $args['type'] ), esc_attr( $key ), checked( $value[ array_search( $key, $value, TRUE ) ], $key, FALSE ), wp_kses_post( $label ), esc_attr( $iterator ) );
						}
						else
						{
							$options_markup .= sprintf( '<input id="%1$s_%5$s" name="%1$s[]" type="%2$s" value="%3$s" /><label for="%1$s_%5$s"> %4$s</label><br>', esc_attr( $args['uid'] ), esc_attr( $args['type'] ), esc_attr( $key ), wp_kses_post( $label ), esc_attr( $iterator ) );
						}
					}
		...
		{
			$helper = $args['helper'];
			printf( '<span class="helper">%s</span>', wp_kses_post( $helper ) );
		}

		if ( ! empty( $args['supplimental'] ) )
		{
			$supplimental = $args['supplimental'];
			printf( '<p class="description">%s</p>', wp_kses_post( $supplimental ) );
		}
	}
