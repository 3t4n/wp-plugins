...
class adminObserver {
	...
	public function mo_epbr_admin_observer() {
		if ( isset( $_REQUEST['option'] ) && 'testUser' === $_REQUEST['option'] ) {
		...
		if ( isset( $_REQUEST['option'] ) && 'mo_epbr_feedback' === $_REQUEST['option'] ) {
			check_admin_referer( 'mo_epbr_feedback' );
			$values_array = array();
		...
		if ( isset( $_REQUEST['option'] ) && 'mo_epbr_skip_feedback' === $_REQUEST['option'] ) {
			check_admin_referer( 'mo_epbr_skip_feedback' );
			$values_array = array();
			foreach ( $_POST as $key => $value ) {
				$values_array[ $key ] = sanitize_text_field( wp_unslash( $value ) );
			}
		...
		if ( isset( $_REQUEST['option'] ) && 'mo_epbr_contact_us_query_option' === $_REQUEST['option'] ) {
			check_admin_referer( 'mo_epbr_contact_us_query_option' );
	...
	private function mo_epbr_send_email_alert( $query, $is_skipped = false ) {
		...
		$deactivate_reason_message = array_key_exists( 'query_feedback', $query ) ? htmlspecialchars( sanitize_text_field( wp_unslash( $query['query_feedback'] ) ) ) : false;

		$reply_required = '';
		if ( isset( $query['get_reply'] ) ) {
			$reply_required = htmlspecialchars( sanitize_text_field( wp_unslash( $query['get_reply'] ) ) );
		}
		...
		if ( isset( $query['rate'] ) ) {
			$rate_value = htmlspecialchars( sanitize_text_field( wp_unslash( $query['rate'] ) ) );
		}
	...
	private function mo_epbr_send_support_query() {
		check_admin_referer( 'mo_epbr_contact_us_query_option' );
		$email = isset( $_POST['mo_epbr_contact_us_email'] ) ? sanitize_email( wp_unslash( $_POST['mo_epbr_contact_us_email'] ) ) : '';
		$phone = isset( $_POST['mo_epbr_contact_us_phone'] ) ? htmlspecialchars( sanitize_text_field( wp_unslash( $_POST['mo_epbr_contact_us_phone'] ) ) ) : '';
		$query = isset( $_POST['mo_epbr_contact_us_query'] ) ? htmlspecialchars( sanitize_text_field( wp_unslash( $_POST['mo_epbr_contact_us_query'] ) ) ) : '';

		$query = '[Embed Power BI Reports] ' . $query;

		if ( array_key_exists( 'mo_epbr_setup_call', $_POST ) === true ) {
			$time_zone = isset( $_POST['mo_epbr_setup_call_timezone'] ) ? sanitize_text_field( wp_unslash( $_POST['mo_epbr_setup_call_timezone'] ) ) : '';
			$call_date = isset( $_POST['mo_epbr_setup_call_date'] ) ? sanitize_text_field( wp_unslash( $_POST['mo_epbr_setup_call_date'] ) ) : '';
			$call_time = isset( $_POST['mo_epbr_setup_call_time'] ) ? sanitize_text_field( wp_unslash( $_POST['mo_epbr_setup_call_time'] ) ) : '';
