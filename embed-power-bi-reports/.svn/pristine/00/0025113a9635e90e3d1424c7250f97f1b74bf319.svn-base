...
class demorequest {
	...
	/**
	 * Holds the instance of DemoRequest class.
	 *
	 * @var DemoRequest
	 */
	...
	/**
	 * Object instance(DemoRequest) getter method.
	 *
	 * @return DemoRequest
	 */
	...
	/**
	 * Function to display the demo request tab view.
	 *
	 * @return void
	 */
	private function mo_epbr_display__demo_request_tab() {

		$wp_integrations    = pluginConstants::WP_INTEGRATIONS_TITLE;
			...
			<form class="mo_epbr_ajax_submit_form" method="post" action="">
				...
				<div style="content:'';display: block;height: 5px;background: #1f3668;margin-top: 9px;border-radius: 30px"></div>
					...
					<tr>
						...
						<td class="right-div"><input style="width:206px;" type="email" name="mo_epbr_demo_email" placeholder="We will use this email to setup the demo for you" required value="<?php echo esc_attr( ( get_option( 'mo_epbr_admin_email' ) ) === '' ) ? esc_attr( get_option( 'admin_email' ) ) : esc_attr( get_option( 'mo_epbr_admin_email' ) ); ?>"></td>
					</tr>
					<tr>
					...
					<div style="display:flex;gap:180px;margin:5px;font-size:16px;position:relative;margin-left:25px;">
						...
						foreach ( $azure_integrations as $key => $value ) {
							?>
							<div style="margin-top:2px;">
								<input type="checkbox" name="<?php echo esc_attr( $key ); ?>" value="true"> <span><?php echo esc_attr( $value ); ?></span>
							</div>
						<?php } ?>   
						</div>
					</div>
