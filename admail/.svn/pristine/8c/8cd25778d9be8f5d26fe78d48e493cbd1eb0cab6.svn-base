...
add_filter('woocommerce_single_product_summary', 'add_ales_hooks_in_summary');

function add_ales_hooks_in_summary() {
    
    global $product;

    if (has_action('ales_before_title_hook')) {
        
        $title = get_the_title();
        
        ob_start();
        
        do_action('ales_before_title_hook');
        
        $before_title_content = ob_get_clean();
        
        echo $before_title_content . $title;
        
    }

    if (has_action('ales_before_price_hook')) {
        
        ob_start();
        
        do_action('ales_before_price_hook');
        
        $before_price_content = ob_get_clean();
        
        $price = $product->get_price_html();
        
        echo $before_price_content . $price;
        
    }

    if (has_action('ales_after_price_hook')) {
        
        $price = $product->get_price_html();
        
        ob_start();
        
        do_action('ales_after_price_hook');
        
        $after_price_content = ob_get_clean();
        
        echo $price . $after_price_content;
    }

    if (has_action('ales_before_stock_status_hook')) {
        
        ob_start();
        
        do_action('ales_before_stock_status_hook');
        
        $before_stock_status_content = ob_get_clean();
        
        echo $before_stock_status_content . wc_get_stock_html($product);
    }
}
...
function ambisn_subscription_form_positioned(){
    ...
    $excluded_cats = array_filter(explode(',',ambisn_get_setting('exclude_categories')));
    
    $excluded_p_types = array_filter(explode(',',ambisn_get_setting('exclude_product_type')));
    
    $exclude_tag_ids = array_filter(explode(',',ambisn_get_setting('exclude_tag_names')));
    ...
    if($product){
        ...
        if ($product_type === 'grouped') {
            ...
            if ($product_type === 'simple') {
                ...
                if(ambisn_get_setting('enable_custom_form_position') == 1){
                    
                    $form_hook = ambisn_get_setting('s_products_position');
                    
                    $priority = ambisn_get_setting('s_hook_priority');
                ...
                if(!$product->is_in_stock() && get_option('woocommerce_hide_out_of_stock_items') === 'yes'){
                    ...
                    if(ambisn_get_setting('enable_custom_form_position') == 1){
                        ...
                        $form_hook = ambisn_get_setting('v_products_position');
                    
                        $priority = ambisn_get_setting('v_hook_priority');
...
add_action('template_redirect','ambisn_subscription_form_positioned',10);
