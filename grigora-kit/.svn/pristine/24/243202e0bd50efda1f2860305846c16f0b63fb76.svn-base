<?php
/**
 * Renderer function for post grid 3
 *
 * @package grigora-kit
 */

if ( ! function_exists( 'render_block_grigora_kit_post_grid_3' ) ) {
	/**
	 * Render Post Grid 3.
	 *
	 ...
	 */
	function render_block_grigora_kit_post_grid_3( $attributes, $content, $block ) {
		...
		if ( count( $data ) === 3 ) {
			...
			$second_block = sprintf(
				'<%1$s class="block23 block-style">%2$s%3$s%4$s%5$s%6$s%7$s%8$s%9$s</%10$s>',
				isset( $attributes['ContentTag'] ) ? $attributes['ContentTag'] : 'div',
				(
					( '<a class="a-container" target="' ) . ( isset( $attributes['newTab'] ) ? ( $attributes['newTab'] ? '_blank' : '_self' ) : '_self' ) .
