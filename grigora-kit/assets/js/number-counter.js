...
var CountUp = /** @class */ ( function () {
	function CountUp( target, endVal, options ) {
		...
		this.easeOutExpo = function ( t, b, c, d ) {
			return (
				( c * ( -Math.pow( 2, ( -10 * t ) / d ) + 1 ) * 1024 ) / 1023 +
				b
			);
		};
...
window.addEventListener( 'load', function () {
	...
	function assignNewCountUpInstance(
		id,
		countStart,
		countEnd,
		countTime,
	...
	for ( var i = 0; i < elements.length; i++ ) {
		...
		let countEnd = elements[ i ].dataset.end;
		let countTime = elements[ i ].dataset.time;
