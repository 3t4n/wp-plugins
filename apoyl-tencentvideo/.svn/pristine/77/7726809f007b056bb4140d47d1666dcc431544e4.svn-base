 ...
 * @subpackage APOYL_TENCENTVIDEO/admin
 * @author 凹凸曼 <3201361925@qq.com>
 *
 */
class Apoyl_Tencentvideo_Admin
{
    ...
    {
        $links[] = '<a href="' . esc_url(get_admin_url(null, 'options-general.php?page=apoyl-tencentvideo-settings')) . '">' . __('settingsname', 'apoyl-tencentvideo') . '</a>';
    ...
    {
        add_options_page(__('settings', 'apoyl-tencentvideo'), __('settings', 'apoyl-tencentvideo'), 'manage_options', 'apoyl-tencentvideo-settings', array(
            $this,
            'settings_page'
        ));
    }
    ...
    public function region_select($selected = '')
    {
        ...
        foreach ($arr as $v) {
            $a=sprintf(esc_html__( "%s", "apoyl-tencentvideo" ),esc_html($v));
            if ($selected === $v) {
    ...
    public function settings_page()
    {
        ...
        $options_name = 'apoyl-tencentvideo-settings';
        isset($_GET['do'])?$do=sanitize_text_field($_GET['do']):$do='';
        if($do=='syn'){
            require_once APOYL_TENCENTVIDEO_DIR . 'admin/partials/synsetting.php';
        }else{
            require_once APOYL_TENCENTVIDEO_DIR . 'admin/partials/setting.php';
        }
    }
    ...
    public function aliyunvideo_wp_generate_attachment_metadata($metadata,$attachment_id){
        ...
        $arr = get_option('apoyl-tencentvideo-settings');

        if($arr['open']&&$arr['accessid']&&$arr['secretkey']&&$arr['openauto']){
            $file=apoyl_tencentvideo_file('addvideo');
            if($file){
                include $file;
            }
        }
