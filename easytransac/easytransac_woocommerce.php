...
# 
class EasytransacLocalLogger extends EasyTransac\Core\Logger{
...
trait EasytransacCommon {
    ...
    public function easytransac_subscription_cancelled($subscription){
        ...
        EasytransacLocalLogger::getInstance()->setActive($this->get_option('debug_mode')=='yes');
        EasytransacLocalLogger::getInstance()->setFilePath($this->log_folder);
        EasytransacLocalLogger::getInstance()->setLogName($this->log_file);
        EasytransacLocalLogger::getInstance()->write(
        ...
        try {
            ...
            EasytransacLocalLogger::getInstance()->write(json_encode($response));
        }catch(Exception $exc) {
            EasytransacLocalLogger::getInstance()->write('Cancellation Exception: ' . $exc->getMessage());
        }
    ...
    function check_callback_response() {
        ...
        EasytransacLocalLogger::getInstance()->write('Received POST: ' . var_export($received_data, true));
        ...
        if($is_https || (!$is_https && !empty($received_data))) {
            ...
            try {
                ...
                // Log error
                EasytransacLocalLogger::getInstance()->write('Payment error: ' . $exc->getCode() . ' (' . $exc->getMessage().') ');
        ...
        if($invalidOrderIdFormat || ! ($order = new WC_Order($response->getOrderId())) || 0)
        {
            ...
            error_log($errMsg);
            EasytransacLocalLogger::getInstance()->write('Order ID missing: '. $errMsg);
        }
        ...
        if (function_exists('wcs_order_contains_subscription')
            ...
            if( ! $subscription->has_status( 'pending' )){
                ...
                if ( ! $subscription->has_status( 'active' ) ) {
                    ...
                    EasytransacLocalLogger::getInstance()->write($errMsg);
                    die($errMsg);
                }
                ...
                foreach ($related as $related_id) {
                    ...
                    if($related_order_tid == $response->getTid()
                        ...
                        return;
                        // error_log('EasyTransac: '.$errMsg);
                        // EasytransacLocalLogger::getInstance()->write('Subscription payment: '. $errMsg);
                        // die($errMsg);
            ...
            error_log($errMsg);
            EasytransacLocalLogger::getInstance()->write('Amounts mismatch: '. $errMsg);
        }
        ...
        if($order->get_status() != 'processing'){
            ...
            switch ($response->getStatus()) {
                case 'failed':
                    EasytransacLocalLogger::getInstance()->write('Payment error: ' . $response->getError() . ' - ' . $response->getMessage());
     ...
     * @param int $order_id
     */
    function process_payment_by_operation_type($order_id, $operation_type = 'payment') {
        ...
        if ($is_oneclick) {
            ...
            try {
                ...
                EasytransacLocalLogger::getInstance()->write('Payment Exception: ' . $exc->getMessage());
            }
            ...
            if ($response->isSuccess()) {
                ...
                if (in_array($doneTransaction->getStatus(), array('captured', 'pending'))) {
                    ...
                    // Log error
                    EasytransacLocalLogger::getInstance()->write('Payment failed: ' . $response->getErrorCode() . ' - ' . $response->getErrorMessage());
        ...
        if (!$response->isSuccess()) {
            // Log error
            EasytransacLocalLogger::getInstance()->write('Payment error: ' . $response->getErrorCode() . ' - ' . $response->getErrorMessage());
    ...
    public function cleanPhone($phone): string {
...
function init_form_fields_cc() {

    return array(
        'enabled' => array(
            'title' => __('Enable/Disable', 'easytransac_woocommerce') ,
            'type' => 'checkbox',
            'label' => __('Enable EasyTransac payment by credit card', 'easytransac_woocommerce') ,
            'default' => 'yes',
            'desc_tip' => true,
        ) ,
        'title' => array(
...
function init_form_fields_sdd() {

    return array(
        'enabled' => array(
            'title' => __('Enable/Disable', 'easytransac_woocommerce') ,
            'type' => 'checkbox',
            'label' => __('Enable EasyTransac payment by SEPA bank transfert', 'easytransac_woocommerce') ,
            'default' => 'yes',
            'desc_tip' => true,
        ) ,
        'title' => array(
            ...
            'type' => 'text',
            'description' => __('This controls the title which the client sees during checkout.', 'easytransac_woocommerce') ,
            'default' => __('SEPA bank transfer', 'easytransac_woocommerce') ,
            'desc_tip' => true,
        ) ,
        'api_key' => array(
            'title' => __('API Key', 'easytransac_woocommerce') ,
...
function init_easytransac_gateway() {
	...
	class EasytransacGateway extends WC_Payment_Gateway {
		...
		function __construct() {
			...
			$this->description = __('Use your credit card to pay with <a target="_blank" href="https://www.easytransac.com/en">Easytransac</a>.', 'easytransac_woocommerce') . $this->get_icon_html();
            $this->instructions = '';
        ...
        function init_form_fields() {
            $this->form_fields = init_form_fields_cc();
        }
		...
		*/
		function process_payment($order_id) {
			return $this->process_payment_by_operation_type($order_id, 'payment');
		}
        ...
        /**
         * Get gateway icon only.
         * @return string
         */
        public function get_icon_html() {
            $icon_url = plugin_dir_url(__FILE__) . '/includes/icon.png';
            $icon_html = '<p><img id="easytransac-icon" src="' . esc_attr($icon_url) . '" alt="' . esc_attr__('EasyTransac', 'easytransac_woocommerce') . '" /></p>';
    ...
    class EasytransacSddGateway extends WC_Payment_Gateway {
        use EasytransacCommon;
        function __construct() {

            $this->id = 'easytransac-sdd';
            $this->icon = '';
            $this->has_fields = false;
            // juste ET
            $this->method_title = __('Easytransac', 'easytransac_woocommerce');
            $this->method_description = __('Easytransac online payment service', 'easytransac_woocommerce');
            $this->description = __('Use your bank account to pay with <a target="_blank" href="https://www.easytransac.com/en">Easytransac</a>.', 'easytransac_woocommerce') . $this->get_icon_html();
            $this->instructions = '';
            $this->init_form_fields();
            $this->init_settings();
            // $this->settings['notifurl'] = get_site_url() . '/wc-api/easytransac';
        ...
        // Settings form
        function init_form_fields() {
            $this->form_fields = init_form_fields_sdd();
        }
         ...
         */
        function process_payment($order_id) {
            return $this->process_payment_by_operation_type($order_id, 'debit');
        }
         ...
         */
        public function get_icon() {
            $icon_html = "<script type=\"text/javascript\">function usingGateway(){\"easytransac-sdd\"==jQuery('form[name=\"checkout\"] input[name=\"payment_method\"]:checked').val()?document.getElementById(\"easytransac-sdd-icon\").style.visibility=\"visible\":document.getElementById(\"easytransac-sdd-icon\").style.visibility=\"hidden\"}jQuery(function(){jQuery(\"body\").on(\"updated_checkout\",function(){usingGateway(),jQuery('input[name=\"payment_method\"]').change(function(){usingGateway()})})});</script>";
            $icon_html .= $this->get_icon_html();
            ...
            if($oneclick == 'yes') {
                ...
                $icon_html .= __('Choose a card : ', 'easytransac_woocommerce');
                $icon_html .= '"; var payNow = "';
                $icon_html .= __('Pay using this bank account', 'easytransac_woocommerce') . '";</script>';
                $icon_html .= '<script type="text/javascript" src="' . plugin_dir_url(__FILE__) . '/includes/oneclick.js"></script>';
            }
            return apply_filters('woocommerce_gateway_icon', $icon_html, $this->id);
        }
         ...
         */
        public function get_icon_html() {
            $icon_url = plugin_dir_url(__FILE__) . '/includes/icon-sepa.png';
            $icon_html = '<p><img id="easytransac-sdd-icon" src="' . esc_attr($icon_url) . '" alt="' . esc_attr__('EasyTransac', 'easytransac_woocommerce') . '" /></p>';
...
function add_easytransac_gateway($methods) {
    ...
    $methods[] = 'EasyTransacSddGateway';
	return $methods;
}

// Register gateway in WooCommerce
add_filter('woocommerce_payment_gateways', 'add_easytransac_gateway', 10);

add_filter('woocommerce_available_payment_gateways', 'add_easytransac_gateway', 20);
...
/**
 * compatibility with the enabled WooCommerce feature 'High-Performance order storage'
 */
...
function oawoo_register_order_approval_payment_method_type() {
    ...
    require_once plugin_dir_path(__FILE__) . 'includes/class-easytransac-block-checkout.php';

    // Hook the registration function to the 'woocommerce_blocks_payment_method_type_registration' action
    add_action(
        'woocommerce_blocks_payment_method_type_registration',
        function( \Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry ) {
            // Register an instance of WC_Easytransac_Blocks
            $payment_method_registry->register( new WC_Easytransac_Blocks );
        }
    );

    // Include the custom Blocks Checkout class
    require_once plugin_dir_path(__FILE__) . 'includes/class-easytransac-sdd-block-checkout.php';
    ...
    add_action(
        ...
        function( \Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry ) {
            // Register an instance of WC_Easytransac_Blocks
            $payment_method_registry->register( new WC_Easytransac_Sdd_Blocks );
        }
    );
