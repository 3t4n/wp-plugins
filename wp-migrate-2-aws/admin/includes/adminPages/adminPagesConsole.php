...
class WPM2AWS_AdminPagesConsole extends WPM2AWS_AdminPagesGeneral
{
    ...
    protected static function makeActionButtons()
    {
        ...
        $unavailable = false;
        if (strtoupper($licenceType) === 'TRIAL') {
            $unavailable = true;
        }
        ...
        // Create Clone Block
        $html .= self::createCloneBlock($licenceType,'right', $unavailable);
    ...
    public static function loadAwsConsole()
    {
        ...
        if (empty(self::$consoleRemoteData)) {
            echo self::makeFeatureUnavailablePage('We are unable to Connect to your AWS Console at this time.');

            return;
        }

        if (isset(self::$consoleRemoteData['error']) === true) {
            echo self::makeFeatureUnavailablePage(self::$consoleRemoteData['error'], 'unauthorized');
    ...
    /**
     * Create the view for the page containing the various AWS actions, e.g clone, staging etc.
     *
     * @return void
     */
    public static function loadAwsActions()
    {
        self::makePageHeading('MAIN');
        ...
        $lightsailInstanceDetails = get_option('wpm2aws-lightsail-instance-details');
        if ($lightsailInstanceDetails === false || $lightsailInstanceDetails === '') {
            echo self::makeFeatureUnavailablePage('Available AWS actions will be available here on the migrated site.');
        ...
        echo self::makeActionsPage();
    }

    /**
     * Create the view for the "Create Clone" action page.
     *
     * @return void
     */
    public static function loadAwsCreateCloneView()
    {
        self::makePageHeading('CLONE');
        ...
        echo self::makeCreateClonePage();
    }

    /**
     * Create the view for the "Create Staging" action page.
     * @param bool $isAvailable
     * @return void
     */
    public static function loadAwsCreateStagingView($isAvailable)
    {
        self::makePageHeading('STAGING');

        if ($isAvailable === true) {
            echo self::makeCreateStagingPage();
        ...
        echo self::makeStagingUnavailablePage();
    }

    /**
     * Create the view for the "Upgrade Stack" action page.
     *
     * @return void
     */
    public static function loadAwsUpgradeStackView()
    {
        self::makePageHeading('UPGRADE');
        ...
        echo self::makeUpgradeStackPage();
    }

    /**
     * Create the view for the "Management Console" action page.
     *
     * @return void
     */
    private static function makeConsolePage()
    {
        ...
        // Right Side
        $html .= '<div class="wpm2aws-half-width-panel wpm2aws-half-width-panel-right wpm2aws-cloned-instance-summary">';
     ...
     * @return string
     */
    private static function makeCreateClonePage()
    {
        WPM2AWS_WelcomePanel::template();

        $overlayText = WPM2AWS_FEATURE_UNAVAILABLE;

        if (defined('WPM2AWS_MIGRATED_SITE') === true && WPM2AWS_MIGRATED_SITE === true) {
            $overlayText = WPM2AWS_FEATURE_INACCESSIBLE;
        }

        return self::makeFeatureUnavailablePage($overlayText);
    }

    /**
     * @return string
     */
    private static function makeStagingUnavailablePage()
    {
        WPM2AWS_WelcomePanel::template();
     ...
     * @return string
     */
    private static function makeCreateStagingPage()
    {
        WPM2AWS_WelcomePanel::template();
        ...
        $html .= self::createStagingOnAwsBlock($licenceType, $disabledSectionClass, $noSectionLink, $panelPosition);
     ...
     * @return string
     */
    private static function makeUpgradeStackPage()
    {
        WPM2AWS_WelcomePanel::template();
    ...
    private static function makeRegionSection()
    {
        ...
        $html .= '<div class="wpm2aws-console-region-flag"><span class="fi fi-'. self::getRegionFlag() .'"></span></div>';
    ...
    private static function makeRebootInstanceConfirmationSection()
    {
        ...
        $html .= '<input type="hidden" id="wpm2aws-console-reboot-instance-cross-check" name="wpm2aws-console-reboot-instance-cross-check" value="' . esc_attr('') . '"/>';
    ...
    private static function makeCreateInstanceSnapshotConfirmationSection()
    {
        ...
        $html .= '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '?action=' . $formAction . '">';
        $html .= '<input type="hidden" name="action" value="' . esc_attr($formAction) . '"/>';
        $html .= '<input type="hidden" id="wpm2aws-console-create-instance-snapshot-cross-check" name="wpm2aws-console-create-instance-snapshot-cross-check" value="' . esc_attr('') . '"/>';
        $html .= '<div class="wpm2aws-console-inputs-container">';
        $html .= '<input type="text" id="wpm2aws-console-create-instance-snapshot-name" name="wpm2aws-console-create-instance-snapshot-name" placeholder="Add Snapshot Name" value="' . esc_attr('') . '"/>';
        $html .= '</div>';
    ...
    private static function getChangeInstanceRegionCopyInstanceSection()
    {
        ...
        /** @var MigrationFormTemplateFieldDto $firstFormElement **/
        $firstFormElement = $formElements[0];

        $formElementFieldData = $firstFormElement->getFieldData();

        if ($formElementFieldData !== null && $formElementFieldData !== []) {
            ...
            $html .= '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '?action=' . $formAction . '">';
            $html .= '<input type="hidden" name="action" value="' . esc_attr($formAction) . '"/>';
            $html .= '<input type="hidden" id="wpm2aws-console-change-instance-region-cross-check" name="wpm2aws-console-change-instance-region-cross-check" value="' . esc_attr('') . '"/>';
            $html .= '<div class="wpm2aws-console-inputs-container">';
            ...
            foreach ($formElementFieldData as $dataKey => $dataVal) {
                $disabled = '';
    ...
    private static function getChangeInstanceRegionLaunchInstanceSection()
    {
        ...
        if ($formElementFieldData !== null && $formElementFieldData !== []) {
            ...
            $html .= '<input type="hidden" name="wpm2aws-console-change-instance-region-new-region-ref" value="' . esc_attr($newRegion) . '"/>';
            $html .= '<input type="hidden" name="wpm2aws-console-change-instance-region-use-snapshot-ref" value="' . esc_attr($newSnapshotName) . '"/>';
    ...
    private static function makeChangeInstancePlanConfirmationSection()
    {
        ...
        if (!empty(self::$consoleRemoteData['bundle-details'])) {
            ...
            $html .= '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '?action=' . $formAction . '">';
            $html .= '<input type="hidden" name="action" value="' . esc_attr($formAction) . '"/>';
            $html .= '<input type="hidden" id="wpm2aws-console-change-instance-plan-cross-check" name="wpm2aws-console-change-instance-plan-cross-check" value="' . esc_attr('') . '"/>';
            $html .= '<div class="wpm2aws-console-inputs-container">';
    ...
    private static function makeInstanceAlarmsSection()
    {
        ...
        $html .= '<h2 class="wpm2aws-console-header"><img  width="30" src="' . plugin_dir_url( dirname( dirname( __FILE__ ) ) ) . 'assets/images/alarm-icon.png"/> ' . esc_html(__('Alarms', 'migrate-2-aws')) . ' <span class="dashicons dashicons-editor-help"></span></h2>';
    ...
    private static function addNewAlarmSection()
    {
        ...
        $html .= '<br>';
        $html .= '<div id="wpm2aws-add-new-metric-alarm-button" style="color:#dd6b10;cursor:pointer;"><img src="' . plugin_dir_url( dirname( dirname( __FILE__ ) ) ) . 'assets/images/add-icon.png" width="30"> Add New Alarm</div>';
    ...
    private static function makeSystemDiskSection()
    {
        ...
        $html .= '<h2 class="wpm2aws-console-header"><img src="' . plugin_dir_url( dirname( dirname( __FILE__ ) ) ) . 'assets/images/server-icon.png" width="45">' . esc_html(__('System Disk', 'migrate-2-aws')) . ' <span class="dashicons dashicons-editor-help"></span></h2>';

        $html .= '<div class="wpm2aws-console-section wpm2aws-console-section-inverse wpm2aws-console-section-inner-panel wpm2aws-console-disk-panel" >';
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel">';
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel-icon">';
        $html .= '<img style="max-width:80%;" src="' . plugin_dir_url( dirname( dirname( __FILE__ ) ) ) . 'assets/images/disk-icon.png"/>';
        $html .= '</div>';
        $html .= '</div>';
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel">';
    ...
    private static function makeManualSnapshotSection()
    {
        ...
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel">';
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel-icon">';
        $html .= '<img style="max-width:80%;" src="' . plugin_dir_url( dirname( dirname( __FILE__ ) ) ) . 'assets/images/snapshot.png"/>';
        $html .= '</div>';
        $html .= '</div>';
    ...
    private static function makeLoadBalancerSection()
    {
        ...
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel">';
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel-icon">';
        $html .= '<img style="max-width:80%;" src="' . plugin_dir_url( dirname( dirname( __FILE__ ) ) ) . 'assets/images/loadBalancer.png"/>';
        $html .= '</div>';
        $html .= '</div>';
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel">';
    ...
    private static function makeDatabaseSection()
    {
        ...
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel">';
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel-icon">';
        $html .= '<img style="max-width:80%;" src="' . plugin_dir_url( dirname( dirname( __FILE__ ) ) ) . 'assets/images/mySqlDb.png"/>';
        $html .= '</div>';
        $html .= '</div>';
        $html .= '<div class="wpm2aws-console-section-inner-sub-panel">';
        if (empty(self::$consoleRemoteData['instance-details']['disks'])) {
    ...
    private static function makeInstanceHistorySection()
    {
        ...
        if (!empty(self::$consoleRemoteData['cloud-trail-details'])) {
            ...
            usort($cloudTrails, [WPM2AWS_AdminPagesConsole::class, 'sortByTime']);
        }
