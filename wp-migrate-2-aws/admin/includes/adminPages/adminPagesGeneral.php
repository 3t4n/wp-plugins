...
class WPM2AWS_AdminPagesGeneral extends WPM2AWS_adminPagesCommon
{
    ...
    /**
     * Re-usable spinner overlay. Triggered by JavaScript
     *
     * @return void
     */
    public static function makeLoadingOverlaySection()
    {
        $loadingOverlay = '';

        $loadingOverlay .= '<div class="wpm2aws_loading_overlay" style="display:none;">';
        $loadingOverlay .= '<div class="" style="text-align:center;">';
        $loadingOverlay .= '<h2 style="color:#0085ba;">We\'re working on it...</h2>';
        $loadingOverlay .= '<img src="' . plugins_url('assets/images/ajax-loader-circle.gif', dirname( dirname(__FILE__) )) . '"/ alt="Loading...">';
        $loadingOverlay .= '<h4><em style="color:#DD6B10;">please wait</em></h4>';
        $loadingOverlay .= '</div>';
        $loadingOverlay .= '</div>';
    ...
    public static function loadIAMform($pageTitle)
    {
        ...
        $isValidLicence = $validLicence !== false && $validLicence !== '' && $validLicence !== null;

        if ($isValidLicence === false ) {
            self::makeRegisterTrialSection();
            self::makeValidateLicenceSection();
            self::makeLoadingOverlaySection();

            echo '<div id="wpm2aws-settings-section-register-trial" style="width:50%;margin:auto;margin-top:50px;">';
            echo '<div class="wpm2aws-inputs-row">';
            echo '<div class="wpm2aws-inputs-row-body wpm2aws-inputs-row-body-fw">';
            echo self::makeServerAndVersionsReport();
            echo '</div>';
            echo '</div>';
            echo '</div>';
    ...
    public static function makeRegisterTrialSection()
    {
        echo '<div id="wpm2aws-settings-section-register-trial" style="width:50%;margin:auto;margin-top:50px;">';
        echo '<div class="wpm2aws-inputs-row">';
        echo '<div class="wpm2aws-inputs-row-header">';
        echo '<h2 class="wpm2aws-settings-heading">Register Trial</h2>';
        echo '<span id="wpm2aws-settings-button-pro-settings" class="button wpm2aws-control-button wpm2aws-settings-button">Pro settings</span>';
        echo '</div>';

        echo '<div class="wpm2aws-inputs-row-body wpm2aws-inputs-row-body-fw">';

        // Register trial
        $formElements = WPM2AWS_MigrationForm::getTemplate()->prop('register_trial_user_form');
        self::generateInputSection(
            ...
            'Register Trial',
            null,
            'wpm2aws-register-licence-field'
        );

        $unableToProceedError = get_option('wpm2aws-unable-to-proceed-error');
        $localInvSslInvCacertError = get_option('wpm2aws-local-inv-ssl-inv-cacert-error');

        if ($unableToProceedError !== false && $localInvSslInvCacertError !== false) {
            echo '<p class="wp2aws-cacert-error">This environment is not configured correctly to allow the process to proceed. (see detail below)</p>';
        }
    ...
    {
        echo '<div id="wpm2aws-settings-section-register-pro" style="width:50%;margin:auto;margin-top:50px;">';
        echo '<div class="wpm2aws-inputs-row">';
        echo '<div class="wpm2aws-inputs-row-header">';
        echo '<h2 class="wpm2aws-settings-heading">Pro User Settings</h2>';
        echo '<span id="wpm2aws-settings-button-trial-settings" class="button wpm2aws-control-button wpm2aws-settings-button">Trial settings</span>';
        echo '</div>';
     ...
     * @param $title
     * @param $formAction
     * @param array|MigrationFormTemplateFieldDto[] $formElements
     * @param $reference
     * @param $label
     ...
     * @return void
     */
    private static function generateInputSection($title, $formAction, $formElements, $reference, $label, $additionalInfo = null, $extraClass = null, $disabled = false)
    {
        $disabledInput = '';
        $extraClassText = '';
        ...
        <div class="wpm2aws-inputs-item<?php echo $extraClassText; ?>">
            ...
            if (!empty($formElements)) : ?>
                ...
                foreach ($formElements as $element) :
                    ...
                    $fieldValue = $element->getFieldValue();
                    $fieldPlaceholder = $element->getFieldPlaceholder();
                    $fieldData = $element->getFieldData();
                ?>
                    <?php if ($fieldType !== 'checkbox') : ?>
                        <?php if ($fieldLabel !== '' && $fieldLabel !== null) : ?>
                            <div style="font-size: 13px; margin-bottom:3px;">
                                <label for="<?php echo esc_attr($fieldId); ?>"><strong><?php echo esc_attr($fieldLabel); ?>: </strong></label>
                            </div>
                        <?php endif; ?>
                    <?php endif; ?>
                    ...
                    <?php if ($fieldType === 'select') : ?>
                        <div>
                        <select id="<?php echo esc_attr($fieldId); ?>" name="<?php echo esc_attr($fieldName); ?>" <?php echo $disabledInput;?>>
                        <option value=""><?php echo esc_attr($fieldPlaceholder); ?></option>
                        <?php foreach ($fieldData as $dataKey => $dataVal) : ?>
                            <?php
                            $selected = '';
                            if ($dataKey === $fieldValue || (string)$dataKey === (string)$fieldValue) {
                                $selected = 'selected';
                            } ?>
                    ...
                    <?php elseif ($fieldType === 'checkbox') : ?>
                        <label class="wpm2aws-checkbox-label" style="display: inline-block; font-size: 13px; margin-right: 10px;">
                            <input type="checkbox" name="<?php echo esc_attr($fieldName); ?>" id="<?php echo esc_attr($fieldId); ?>" value="<?php echo esc_attr($fieldValue); ?>" <?php echo $disabledInput; ?>/>
                            <?php if ($fieldLabel !== '' && $fieldLabel !== null) : ?>
                    ...
                    <?php else : ?>
                        <div>
                        <input type="<?php echo esc_attr($fieldType); ?>" name="<?php echo esc_attr($fieldName); ?>" id="<?php echo esc_attr($fieldId); ?>" placeholder="<?php echo esc_attr($fieldPlaceholder); ?>" value="<?php echo esc_attr($fieldValue); ?>" class="wpm2aws-input-field" <?php echo $disabledInput;?>/>
                        </div>
                    <?php endif; ?>
                <?php endforeach; ?>
                ...
                if ($additionalInfo !== null) {
                    echo '<p class="wpm2aws-inputs-additional-information">'.$additionalInfo.'</p>';
                }
                ?>
     ...
     * @return void
     */
    private static function makeAwsSetupSection($verified)
    {
        ...
        $upperCaseLicenceType = \strtoupper($licenceType);

        if ($upperCaseLicenceType === 'TRIAL') {
            echo self::makeTrialUserAwsSection();
        } else {
            $standardTitle = 'Validate Credentials';

            if ($verified === false) {
                $title = $standardTitle;
            } else {
                $title = sprintf('%s <span class="dashicons dashicons-yes-alt wpm2aws-success-text"></span>', $standardTitle);
            }
            ...
            self::generateInputSection(
                ...
                null,
                'wpm2aws-inputs-item-multiple-inputs wpm2aws-validate-credentials-field'
            );

            self::makeAwsRegionSection($verified);
        }
    ...
    private static function makeSectionFiveMain()
    {
        ...
        if (false !== get_option('wpm2aws_valid_licence_type') &&
            ...
            $launchTrialInstanceText = \sprintf(
                'This software launches trial instances in %s "%s" plan by default.',
                WPM2AWS_PLUGIN_AWS_LIGHTSAIL_DEFAULT_SIZE_GB,
                WPM2AWS_PLUGIN_AWS_LIGHTSAIL_DEFAULT_SIZE_NAME
            );

            $html .= $launchTrialInstanceText;
        } else {
            $launchInstanceText = \sprintf(
                'This software launches instances in %s "%s" plan by default unless otherwise specified above.',
                WPM2AWS_PLUGIN_AWS_LIGHTSAIL_DEFAULT_SIZE_GB,
                WPM2AWS_PLUGIN_AWS_LIGHTSAIL_DEFAULT_SIZE_NAME
            );

            $html .= $launchInstanceText;
            $html .= '</br>';
     ...
     * @return void
     */
    private static function makeAwsRegionSection($verified)
    {
        if ($verified === false) {
            $disabled = true;
            $additionalInformation = 'Validate AWS credentials first';
            $extraClass = 'wpm2aws-additional-information-danger';
        } else {
            $disabled = false;
        ...
        $standardTitle = 'AWS Region';
        $regionSelected = get_option('wpm2aws-aws-region');
     ...
     * @return string
     */
    private static function makeTrialUserAwsSection()
    {
        $noticeWrapperOpener = '<p style="color:red;">';
        $notice1 = 'We have detected that you are running a trial version of WP on AWS.';
        $lineSeparator = '</br>';
        $notice2 = 'AWS Setup has been completed automatically.';
        $noticeWrapperCloser = '</p>';

        $notice = \sprintf('%s%s%s%s%s', $noticeWrapperOpener, $notice1, $lineSeparator, $notice2, $noticeWrapperCloser);

        $awsRegionNameWrapperOpener = '<div class="wpm2aws-trial-user-aws-settings-region">';
        $awsRegionName = get_option('wpm2aws-aws-region');
        $awsRegionNameWrapperCloser = '</div>';
        $awsRegionNameBlock = \sprintf('%s%s%s', $awsRegionNameWrapperOpener, $awsRegionName, $awsRegionNameWrapperCloser);

        $awsRegionWrapperOpener = '<h3>';
        $awsRegionIntroductionText = 'AWS Trial Version Region:';
        $awsRegionWrapperCloser = '</h3>';

        $awsRegion =\sprintf('%s%s %s%s', $awsRegionWrapperOpener, $awsRegionIntroductionText, $awsRegionNameBlock, $awsRegionWrapperCloser);

        return \sprintf('%s%s', $notice, $awsRegion);
    }
    ...
    private static function makeS3SetupSection($verified, $managed)
    {
        ...
        if ($verified === true) {
            if ($managed === true) {
                ...
                if ($regionSelected === false) {
                    $disabled = true;
                    $additionalInformation = 'Select AWS Region first';
                    $extraClass = 'wpm2aws-additional-information-danger wpm2aws-validate-credentials-field';
                } else {
                    $disabled = false;
                    $additionalInformation = null;
                    $extraClass = 'wpm2aws-validate-credentials-field';
                }

                $standardTitle = 'Create Storage Target';

                $storageTargetSet = get_option('wpm2aws-aws-s3-bucket-name');

                if ($storageTargetSet === false) {
                    $title = $standardTitle;
                } else {
                    $title = sprintf('%s <span class="dashicons dashicons-yes-alt wpm2aws-success-text"></span>', $standardTitle);
                }
    ...
    private static function makeInputsSection()
    {
        ...
        if (
            ...
            $updatedCurrentActiveStepNumber = 2;
            wpm2awsAddUpdateOptions('wpm2aws_current_active_step', $updatedCurrentActiveStepNumber);
            $currentActiveStep = $updatedCurrentActiveStepNumber;
        }
        ?>
        ...
        <div id="wpm2aws-edit-inputs-section" class="wpm2aws-admin-section-container">
            ...
            self::makeAwsSetupSection($verifiedUser);
            if (
                false !== $userType &&
                'self' === $userType
            ) {
    ...
    /**
     * Retrieves the current database version from the WordPress database
     *
     * @global wpdb 
     * @return string
     */
    ...
    /**
     * Generates and returns an HTML report of the server and versions information
     *
     * @return string
     */
    private static function makeServerAndVersionsReport()
    {
        // Define text and colors for enabled/disabled status
        $enabledText = 'Enabled';
        $disabledText = 'Disabled';
        $isValidColour = 'green';
        $isNotValidColour = 'red';

        // Icons for valid/invalid statuses
        $isValidIcon = '<span style="color:' . $isValidColour . ';" class="dashicons dashicons-yes"></span>';
        $isNotValidIcon = '<span style="color:' . $isNotValidColour . ';" class="dashicons dashicons-no"></span>';
        $infoIcon = '<span class="dashicons dashicons-info-outline"></span>';

        // Use static methods from WPM2AWS_Settings

        $isSystemPhpVersionCompatibleWithSeahorseApiRequirements = WPM2AWS_Settings::isSystemPhpVersionCompatibleWithSeahorseApiRequirements();
        $systemPhpVersion = WPM2AWS_Settings::getSystemPhpVersion();
        // PHP version text and icon
        if ($isSystemPhpVersionCompatibleWithSeahorseApiRequirements === true) {
            $phpVersionText = $systemPhpVersion;
            $phpVersionColour = $isValidColour;
            $phpVersionIcon = $isValidIcon;
        } else {
            $phpVersionText = \sprintf('%s (outdated version incompatible with the Seahorse API)', $systemPhpVersion);
            $phpVersionColour = $isNotValidColour;
            $phpVersionIcon = $isNotValidIcon;
        }

        // WordPress version text and icon
        $isInstalledWordPressVersionCompatibleWithSeahorseApiRequirements = WPM2AWS_Settings::isInstalledWordPressVersionCompatibleWithSeahorseApiRequirements();
        $wordPressVersion = WPM2AWS_Settings::getWordPressVersion();
        if ($isInstalledWordPressVersionCompatibleWithSeahorseApiRequirements === true) {
            $wpVersionText = $wordPressVersion;
            $wpVersionColour = $isValidColour;
            $wpVersionIcon = $isValidIcon;
        } else {
            $wpVersionText = \sprintf('%s (outdated version incompatible with the Seahorse API)', $wordPressVersion);
            $wpVersionColour = $isNotValidColour;
            $wpVersionIcon = $isNotValidIcon;
        }

        // Simplified the color and text decision logic based on method results
        $allowUrlFopen = ini_get('allow_url_fopen');
        if ($allowUrlFopen === false) {
            $urlFopenText = $disabledText;
            $urlFopenColour = $isNotValidColour;
            $urlFopenIcon = $isNotValidIcon;
        } else {
            $urlFopenText = $enabledText;
            $urlFopenColour = $isValidColour;
            $urlFopenIcon = $isValidIcon;
        }

        $isCurlEnabled = function_exists('curl_version');
        if ($isCurlEnabled === true) {
            $curlText = $enabledText;
            $curlColour = $isValidColour;
            $curlIcon = $isValidIcon;
        } else {
            $curlText = $disabledText;
            $curlColour = $isNotValidColour;
            $curlIcon = $isNotValidIcon;
        }

        $isSslSupported = defined('OPENSSL_VERSION_TEXT');
        if ($isSslSupported === true) {
            $sslSupportedText = OPENSSL_VERSION_TEXT;
            $sslSupportedColour = $isValidColour;
            $sslSupportedIcon = $isValidIcon;
        } else {
            $sslSupportedText = 'Not Available';
            $sslSupportedColour = $isNotValidColour;
            $sslSupportedIcon = $isNotValidIcon;
        }

        // Handling the HTTPS and Localhost logic directly here, as it's specific to this method context
        $isValidHttps = WPM2AWS_Settings::isValidHTTPS();
        if ($isValidHttps === true) {
            $isHttpsText = 'true';
            $httpsColor = $isValidColour;
            $httpsIcon = $isValidIcon;
        } else {
            $isHttpsText = 'false';
            $httpsColor = $isNotValidColour;
            $httpsIcon = $isNotValidIcon;
        }

        $isLocalhost = WPM2AWS_Settings::isLocalhost();
        if ($isLocalhost === true) {
            $LocalhostColor = $isValidColour;
            $LocalhostIcon = $isValidIcon;
            $isLocalhostText = 'true';
        } else {
            $LocalhostColor = $isNotValidColour;
            $LocalhostIcon = $isNotValidIcon;
            $isLocalhostText = 'false';
        }

        // Cacert handling
        $isValidCaCertPresent = WPM2AWS_Settings::isValidCACertPresent();
        if ($isValidCaCertPresent === true) {
            $caCertColor = $isValidColour;
            $caCertIcon = $isValidIcon;
            $isCaCertEnabledText = 'Valid';
        } else {
            $caCertColor = $isNotValidColour;
            $caCertIcon = $isNotValidIcon;
            $isCaCertEnabledText = 'Invalid ** You must have a valid CA certificate (Localhost only) **';
        }

        $webServer = $_SERVER['SERVER_SOFTWARE'];
        $databaseVersion = self::getDatabaseVersion();

        // Constructing HTML for the report
        $html = '<h4>System Information</h4>';
        $html .= '<table class="wp2aws-table">';
        $html .= '<tr><th>Component</th><th>Version/Status</th></tr>';
        $html .= '<tr><td>Web Server</td><td>' . $infoIcon . ' ' . $webServer . '</td></tr>';
        $html .= '<tr><td>PHP Version</td><td style="color:' . $phpVersionColour . ';">'. $phpVersionIcon . '  ' . $phpVersionText .  '</td></tr>';
        $html .= '<tr><td>Database Version</td><td>' . $infoIcon . ' ' . $databaseVersion . '</td></tr>';
        $html .= '<tr><td>WordPress Version</td><td style="color:' . $wpVersionColour . ';">'. $wpVersionIcon . ' ' . $wpVersionText . '</td></tr>';
        $html .= '<tr><td>fopen()</td><td style="color:' . $urlFopenColour . ';">' . $urlFopenIcon . ' ' . $urlFopenText . '</td></tr>';
        $html .= '<tr><td>cURL</td><td style="color:' . $curlColour . ';">'. $curlIcon . ' ' . $curlText . '</td></tr>';
        $html .= '<tr><td>SSL Support</td><td style="color:' . $sslSupportedColour . ';">'. $sslSupportedIcon . ' ' . $sslSupportedText . '</td></tr>';
	    $html .= '<tr><td>HTTPS (Valid SSL)</td><td style="color:' . $httpsColor . ';">'. $httpsIcon . ' ' . $isHttpsText . '</td></tr>';
	    $html .= '<tr><td>Localhost</td><td style="color:' . $LocalhostColor . ';">'. $LocalhostIcon . ' ' . $isLocalhostText . '</td></tr>';

        if ($isLocalhost === true) {
            $html .= '<tr><td>Certification Authority (CA)</td><td style="color:' . $caCertColor . ';">'. $caCertIcon . ' ' . $isCaCertEnabledText . '</td></tr>';
        }

        $html .= '</table>';

        $isSystemReportLogged = get_option('wpm2aws_system_report_logged');

        if ($isSystemReportLogged === false) {
            wpm2awsLogRAction('wpm2aws-server-details', 'Server Info: ' . $webServer .' | PHP: '. $systemPhpVersion .' | DB: '. $databaseVersion .' | WP: '. $wordPressVersion . ' | fopen: '. $allowUrlFopen .' | cURL: '. $isCurlEnabled .' | SSL: '. $isSslSupported . ' | HTTPS: '. $isValidHttps .' | Localhost: '. $isLocalhost .' | cacert: ' . $isValidCaCertPresent);

            wpm2awsAddUpdateOptions('wpm2aws_system_report_logged', '1');
        }
    ...
    private static function makeSummarySection()
    {
        ...
        <div id="wpm2aws-summary-section" class="wpm2aws-admin-section-container">
            ...
            echo self::makeResetButton('Reset All');
    ...
    public static function makeCurrentSettingsSummarySection()
    {
        ...
        $html .= self::makeResetButton('Reset All');
    ...
    private static function makeCurrentSettingsSection()
    {
        ...
        // This logic is only for trial users
        if ($upperCaseLicenceType === 'TRIAL') {
            $noticeWrapperOpener = '<p style="color:red;">';
            $notice1 = 'We have detected that you are running a trial version of WP on AWS.';
            $notice2 = 'AWS Setup has been completed automatically.';
            ...
            $notice1 = \sprintf('%s%s%s', $noticeWrapperOpener, $notice1, $noticeWrapperCloser);
            $notice2 = \sprintf('%s%s%s', $noticeWrapperOpener, $notice2, $noticeWrapperCloser);

            $html .= $notice1;
            $html .= $notice2;

            $bucketName = 'Trial User S3 Bucket';
            $defaultBucketName = 'Trial User S3 Bucket';
            $lightsailInstanceName = 'Trial User Instance';
        } else {
            ...
            $defaultBucketName = get_option('wpm2aws-aws-s3-default-bucket-name');
            $lightsailInstanceName = get_option('wpm2aws-aws-lightsail-name');
        }
        ...
        $html .='<p><strong>Default: </strong><span>' . $defaultBucketName . '</span></p>';
        ...
        $html .='<p style="margin: 0.5em 0;"><strong>AWS Instance Name: </strong><span>' . $lightsailInstanceName . '</span></p>';
    ...
    private static function makeRunZippedFileSystemUploadButton()
    {
        ...
        if (
            ...
            $html .= '<input type="hidden" name="action" value="' . esc_attr($formAction) . '" />';
            $html .= wp_nonce_field($formAction);
            $html .= '<input type="submit" class="button button-primary" name="wpm2aws-process-zipped-fs-uploader-all-submit" value="' . esc_attr($btnLabel) . '" />';
            $html .= '</form>';
        } else {
    ...
    private static function makeRunZipFileSystemButton()
    {
        ...
        $html .= wp_nonce_field($formAction);
        $html .= '<input type="submit" class="button button-primary" name="wpm2aws-process-fszipper-all-submit" value="' . esc_attr($btnLabel) . '" />';
        $html .= '</form>';
    ...
    private static function makeRunDownloadedDataUploadButton()
    {
        ...
        if (self::fileSystemUploadSettingsSet() && false !== get_option('wpm2aws_download_db_complete') && get_option('wpm2aws_download_db_complete') === 'success') {
            ...
            $html .= '<input type="hidden" name="action" value="' . esc_attr($formAction) . '" />';
            $html .= wp_nonce_field($formAction);
            $html .= '<input type="submit" class="button button-primary" name="wpm2aws-process-admin-uploader-all-submit" value="' . esc_attr($btnLabel) . '" />';
            $html .= '</form>';
        } else {
            if (!self::fileSystemUploadSettingsSet() && false !== get_option('wpm2aws_download_db_complete') && get_option('wpm2aws_download_db_complete') === 'success') {
                $html .= '<p class="notice notice-error" style="margin: 0.5em 0;padding:10px;">';
    ...
    private static function makeRunFileSystemDownloadButton()
    {
        ...
        if (self::fileSystemUploadSettingsSet() && false !== get_option('wpm2aws_download_db_complete') && get_option('wpm2aws_download_db_complete') === 'success') {
            ...
            $html .= '<input type="hidden" name="action" value="' . esc_attr($formAction) . '" />';
            $html .= wp_nonce_field($formAction);
            $html .= '<input type="submit" class="button button-secondary" name="wpm2aws-process-download-all-submit" value="' . esc_attr($btnLabel) . '" />';
            $html .= '</form>';
        } else {
            if (!self::fileSystemUploadSettingsSet() && false !== get_option('wpm2aws_download_db_complete') && get_option('wpm2aws_download_db_complete') === 'success') {
                $html .= '<p class="notice notice-error" style="margin: 0.5em 0;padding:10px;">';
     ...
     * @return string
     */
    private static function makeResetButton($buttonText)
    {
        $formAction = 'wpm2aws-reset-all-settings';
        $btnLabel = esc_attr(__($buttonText, 'migrate-2-aws'));
        ...
        $html .= '<input type="hidden" name="action" value="' . esc_attr($formAction) . '" />';
        $html .= wp_nonce_field($formAction);
        $html .= '<input type="submit" class="button wpm2aws-btn-danger" name="wpm2aws-reset-all-settings" value="' . esc_attr($btnLabel) . '" />';
        $html .= '</form>';
    ...
    private static function makeRestartButton()
    {
        ...
        $html .= '<input type="hidden" name="action" value="' . esc_attr($formAction) . '" />';
        $html .=  wp_nonce_field($formAction);
        $html .=  '<input type="submit" class="button wpm2aws-btn-warning" name="wpm2aws-restart-process" value="' . esc_attr($btnLabel) . '" />';
        $html .=  '</form>';
    ...
    private static function makeRestartFileUploadButton($action, $option)
    {
        ...
        $html .= '<input type="hidden" name="action" value="' . esc_attr($formAction) . '" />';
        $html .= wp_nonce_field($formAction);
        $html .= '<input type="submit" class="button button-secondary wpm2aws-process-all-restart-button" style="display:none;" id="' . $inputRef . '" name="' . $inputRef . '" title="re-start file ' . $option . ' process" value="' . esc_attr('&#x21bb;&nbsp; ' . $btnLabel) . '" />';
        $html .= '</form>';
        $html .= '</div>';
    ...
    private static function makeRunDatabaseDownloadButton()
    {
        ...
        if (
            ...
            $html .= '<input type="submit" class="button button-primary wpm2aws-prepare-database-button" name="wpm2aws-run-download-db" value="' . esc_attr($btnLabel) . '" />';
            // $html .=  '</p>';
            $html .= '</form>';
        } else {
            $html .= '<span class="button button-primary" disabled>' . $btnLabel . '</span>';
    ...
    private static function makeRunMigrateButton($sidebar = false)
    {
        ...
        if (self::allSettingsSet()) {
            if (
                ...
                $html .= wp_nonce_field($formAction);
                $html .= '<p class="wpm2aws-run-migration-btn-container">';
                $html .= '<input type="submit" class="button button-primary wpm2aws-button-large wpm2aws-launch-aws-button" id="wpm2aws-demo-full-migration"  name="wpm2aws-demo-full-migration" value="' . esc_attr($btnLabel) . '" />';
                $html .= '</p>';
                $html .= '</form>';
            } else {
            ...
            $html .= wp_nonce_field($formAction);
            $html .= '<p class="wpm2aws-run-migration-btn-container">';
            $html .= '<input type="submit" class="button button-secondary wpm2aws-button-large wpm2aws-launch-aws-button" name="wpm2aws-full-migration-admin" value="' . esc_attr($btnLabel) . '" />';
            $html .= '</p>';
            $html .= '</form>';
        } else {
    ...
    private static function makeShhKeyDownloadButton()
    {
        ...
        $html .= '<input type="hidden" name="action" value="' . esc_attr('wpm2aws-download-key') . '" />';
        $html .= wp_nonce_field('wpm2aws-download-key');
        $html .= '<p>';
        $html .= '<input type="submit" class="button button-primary" name="wpm2aws-download-key" value="' . esc_attr('Download Key') . '" />';
        $html .= '</p>';
        $html .= '</form>';
    ...
    /**
     * Checks if a given IP address is active and responding.
     * 
     * @param string $ip The IP address to check.
     * @return bool True if the IP is active and responding, False otherwise.
     * 
     */
    private static function isIpActive($ip)
    {
        $curl = curl_init($ip);
    ...
    private static function makeConfirmedLightsailInfoSection()
    {
        ...
        $buildDurationSeconds = (8 * 60); // 8 minutes

        $remainingTime = self::remainingTimeInSecondsForProgressBar($buildDurationSeconds, $formattedLaunchDate);

        $lightsailPublicIp = $lightsailDetails['publicIp'];

        $isLightsailPublicIpActive = self::isIpActive($lightsailPublicIp);

        echo '<div class="wpm2aws-inputs-row wpm2aws-launch-instance-content" style="border-color:#DD6B10;width:97%;padding:15px 10px;display:flex;align-items: center;">';
        ...
        if ($remainingTime) {
            ...
            echo '<br>';
            echo 'Links to your cloned site will be displayed once the build has completed. (average build time: 10 minutes approximately)</p>';
        ...
        if (($isLightsailPublicIpActive === false) && ($remainingTime === false)) {
            echo self::makeResetButton('Reset All to Restart');
        } else {
            echo "<a class='wpm2aws-launch-build-launch-button' target='_blank' style='background: #1a7899;padding: 10px 20px;border-radius: 30px;color: #fff;text-decoration: none;font-weight: 600;line-height: 1;margin: 0;' href='" . esc_url($lightsailPublicIp) . "'>Visit Your Clone Site Here</a>";
        }
        ...
        if (
            ...
            echo '<p>This temporary site will remain active for review for ' . WPM2AWS_PLUGIN_AWS_LIGHTSAIL_TRIAL_INSTANCE_AVAILABLE_DURATION_HOURS . ' hours after launch at which point it will be shut-down.</p>';
