...
class WC_Gateway_Payssion_Request {
	...
	public function get_payssion_args( $order, $apply_filters = true) {
		...
		if (substr($pm_id, -2) === 'br' || in_array($pm_id, ['afterpay_us']) || substr($pm_id, 0, strlen('klarna')) === 'klarna') {
		    $data['billing_address'] = base64_encode($order->get_billing_address());
		    $data['order_items'] = base64_encode($order->get_order_lines());
		} else if (substr($pm_id, -2) === 'sa') {
		    $data['shipping_address'] = base64_encode($order->get_shipping_address());
		    $data['order_items'] = base64_encode($order->get_order_lines());
		}
