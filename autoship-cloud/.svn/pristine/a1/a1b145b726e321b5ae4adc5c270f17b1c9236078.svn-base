...
/**
 * Checks if cart or checkout page uses WooCommerce blocks
 * @return bool
 */
function autoship_woocommerce_uses_blocks() {
  $page = 'cart';
  if(is_checkout()) {
    $page = 'checkout';
  }
  return WC_Blocks_Utils::has_block_in_page( wc_get_page_id( $page ), 'woocommerce/' . $page );
}
...
/**
 * Display custom item data in the cart item data
 */
function autoship_get_cart_item_data( $item_data, $item ) {
  // Bail if not using blocks because autoship_display_cart_item_data() data will show
  if(!autoship_woocommerce_uses_blocks()) {
    return $item_data;
  }
  // If not an autoship item bail.
  $schedule_data = autoship_get_item_data_schedule_values( $item );
  ...
  $label = apply_filters( 'autoship_frequency_cart_order_item_schedule_display_label', __( 'Schedule', 'autoship' ), $item['data']->get_name(), $item );
...
/**
 * Outputs the Autoship Data assigned to Cart Items in cart item data
 * @see autoship_get_cart_item_data()
 */
add_filter( 'woocommerce_get_item_data', 'autoship_get_cart_item_data', 10, 2 );
