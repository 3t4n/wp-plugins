...
/**
 * Add the '_autoship_dissable_schedule_order_options' metadata field to handle custom query.
 * Uses != comparasion
 * @param array $query - Args for WP_Query.
 * @param array $query_vars - Query vars from WC_Product_Query.
 * @return array modified $query
 */
function autoship_handle_dissable_schedule_order_options_meta_query( $wp_query_args, $query_vars, $data_store_cpt ) {
  $meta_key = '_autoship_dissable_schedule_order_options'; // The custom meta_key
...
function autoship_get_available_schedulable_variations ( $args = array() ){
  ...
  $products_args = apply_filters('autoship_filter_schedulable_variation_products_query_args', array(
    ...
    // checks for != yes
    '_autoship_dissable_schedule_order_options' => 'yes',
...
function autoship_generate_product_upsert_data ( $product, $product_data = array() ){
  ...
  $product_data = array_merge( array(
    ...
    "backordersAllowed"     => $wc_product->backorders_allowed()
  ), $product_data );
  ...
  foreach ( $autoship_field_names as $name => $settings ) {
    ...
    if(in_array($name, [
      ...
      if ( $product && ( 'variable' == $product->get_type() ) && $product->has_child() ) {
        // Update the child variations
        foreach ( $product->get_children() as $variation_id ) {
          update_post_meta( $variation_id, $name, $val );
        }
      }
    }
...
/**
 * Updates product variants metadata for _autoship_sync_active_enabled key
 *
 * @param mixed $id. Post ID of the product.
 * @param string $val. Value to save.
 */
function autoship_update_product_variants_sync_active_flag( $id, $val ) {
  $product = wc_get_product( $id );
  if ( !$product )
  return;
  ...
  if ( 'variable' == $product->get_type() && $product->has_child() ) {
    ...
    foreach ( $product->get_children() as $variation )
      autoship_set_product_sync_active_enabled ( $variation , $val );
    }
}


/**
 *  Updates product variants metadata for _autoship_schedule_order_enabled key
 *
 * @param mixed $id. Post ID of the product.
 * @param string $val. Value to save.
 */
function autoship_update_product_add_to_scheduled_order_flag( $id, $val ) {
  $product = wc_get_product( $id );
  if ( !$product )
  return;
 ...
 */
function autoship_sync_product( $id, $product = false ){
  ...
  if( apply_filters( 'autoship_sync_product' , ( 'yes' == autoship_sync_active_enabled( $id ) ) && autoship_is_valid_sync_product( $id ), $id ) && !( did_action( 'wp_ajax_woocommerce_save_variations' ) && doing_action( 'woocommerce_update_product_variation' ) ) ){
    ...
    $result = autoship_push_product( $id, $overrides, $product );
 ...
 * @param object|bool $product the WC_Product object or false.
 * @return bool|WP_Error True on success else false|wp_error
 */
function autoship_push_product ( $id, $overrides = array(), $product = false ){
  ...
  // If $product param is passed use it instead of querying DB
  $id = $product !== false ? $product : $id;
 ...
 * @return bool
 */
function autoship_products_array_filter_is_in_stock( $product ) {
	return $product && is_a( $product, 'WC_Product' ) && $product->is_in_stock();
}
 ...
 * @see autoship_handle_dissable_schedule_order_options_meta_query()
 */
...
add_filter( 'woocommerce_product_data_store_cpt_get_products_query', 'autoship_handle_dissable_schedule_order_options_meta_query', 11, 3 );
...
add_action( 'woocommerce_update_product', 'autoship_sync_product', 20, 2 );
add_action( 'woocommerce_update_product_variation', 'autoship_sync_product', 20, 2 );
