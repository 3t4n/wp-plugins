...
function autoship_push_my_account_relative_urls() {
  ...
  try {
    ...
    $notice = autoship_expand_http_code( $e->getCode() );
    return new WP_Error( 'Updating Site My Account URLs failed', __( $notice['desc'], "autoship" ) );
...
function autoship_retain_and_grow_tabs(){
  ...
  $tabs = array(
    'coupons'             => array( 'label' => __( 'Coupons' ,'autoship'), 'callback' => 'autoship_admin_retain_and_grow_tabs_content', 'link_class' => '' ),
    'retention-workflows' => array( 'label' => __( 'Retention Workflows','autoship'), 'callback' => 'autoship_admin_retain_and_grow_tabs_content', 'link_class' => '' ),
    'dunning'             => array( 'label' => __( 'Dunning','autoship'), 'callback' => 'autoship_admin_retain_and_grow_tabs_content', 'link_class' => '' ),
  );
...
function autoship_reports_tabs(){
  ...
  $tabs = array(
    'schedule_orders_metrics'  => array( 'label' => __( 'Scheduled Orders Metrics' ,'autoship'), 'callback' => 'autoship_admin_reports_tabs_content', 'link_class' => '' ),
    'date_summary'            => array( 'label' => __( 'Products by Date Summary','autoship'), 'callback' => 'autoship_admin_reports_tabs_content', 'link_class' => '' ),
    'schedule_nxt_order'      => array( 'label' => __( 'Scheduled Orders by Product', 'autoship' ), 'callback' => 'autoship_admin_reports_tabs_content', 'link_class' => '' ),
    'churn_reports'           => array( 'label' => __( 'Churn Reports', 'autoship' ), 'callback' => 'autoship_admin_reports_tabs_content', 'link_class' => '' ),
    'survey_results'          => array( 'label' => __( 'Survey Results', 'autoship' ), 'callback' => 'autoship_admin_reports_tabs_content', 'link_class' => '' ),
    'customer_metrics'        => array( 'label' => __( 'Customer Metrics', 'autoship' ), 'callback' => 'autoship_admin_reports_tabs_content', 'link_class' => '' ),
    'email_delivery'          => array( 'label' => __( 'Email Delivery', 'autoship' ), 'callback' => 'autoship_admin_reports_tabs_content', 'link_class' => '' ),
  );
...
function autoship_admin_reports_tabs_content( $id ){
  ...
  'schedule_orders_metrics' => urlencode($site_id) . '/reports/scheduled-orders-metrics?tokenBearerAuth=' . urlencode($token_auth),
  'churn_reports'           => urlencode($site_id) . '/reports/churn-reports?tokenBearerAuth=' . urlencode($token_auth),
  'customer_metrics'        => urlencode($site_id) . '/reports/customer-metrics?tokenBearerAuth=' . urlencode($token_auth),
  'email_delivery'          => urlencode($site_id) . '/reports/email-delivery?tokenBearerAuth=' . urlencode($token_auth),
  'survey_results'          => urlencode($site_id) . '/reports/survey-results?tokenBearerAuth=' . urlencode($token_auth) );
...
function autoship_admin_retain_and_grow_tabs_content( $id ){
  ...
  $reports = array(
    'coupons'             => urlencode($site_id) . '/coupons?tokenBearerAuth=' . urlencode($token_auth),
    'dunning'             => urlencode($site_id) . '/settings/site-dunning?tokenBearerAuth=' . urlencode($token_auth),
    'retention-workflows' => urlencode($site_id) . '/retain-and-grow/retention?tokenBearerAuth=' . urlencode($token_auth),
  );
  ...
  <table class="form-table">
    ...
    <tr valign="top">
      ...
      <td>
        ...
        />
        <label for="autoship_scheduled_orders_v2_portal"><?php echo __( '(New) v2 Portal: Display Scheduled Orders in My Account using custom elements.', 'autoship' ); ?></label></br></br>
        ...
        <div class="autoship_native_ui_extra_options" 
          style="display:<?php echo 'template' === $autoship_settings['autoship_scheduled_orders_display_version'] ? 'block' : 'none' ?>;">
            <h2><?php echo __( 'Native UI: Additional Options', 'autoship' ); ?></h2>
            <h4><?php echo __( 'New! Display Product Upsell Carousel', 'autoship' ); ?></h4>
            ...
            />
            <label for="autoship_scheduled_order_upsell_carousel"><?php echo __( 'Enable to display the Product Upsell Carousel.', 'autoship' ); ?></label></br></br>
            ...
            style="display:<?php echo 'yes' === $autoship_settings['autoship_scheduled_order_upsell_carousel'] ? 'block' : 'none' ?>;">
              ...
              />
              <label for="autoship_scheduled_order_upsell_disable_carousel_js"><?php echo __( 'Select to disable loading of Carousel JS script.', 'autoship' ); ?></label></br></br>
            </div>

            <h4><?php echo __( 'Enable Selectable Shipping Rates', 'autoship' ); ?></h4>
        ...
        <h4><?php echo __('Legacy Display Options', 'autoship'); ?></h4>
        <p><?php echo __('The Hosted iFrame and Embedded App Options will not be receiving further updates. Please consider changing your display option to either the Native UI or v2 Portal as soon as possible. New Autoship Cloud merchants should not select a Legacy Display Option.', 'autoship'); ?></p>
        <br />
