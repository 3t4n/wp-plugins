...
jQuery(document).ready(function ($) {
  ...
  */
  ...
  /**
   * Show hide extra Native UI options
   */
  $(document).on('change', 'input[name="autoship_scheduled_orders_display_version"]', function(e) {
    if( 'template' == $(this).val()) {
      $('.autoship_native_ui_extra_options').show();
    } else {
      $('.autoship_native_ui_extra_options').hide();
    }
  });
  /**
   * Show/hide Disable Carousel JS option
   */
  $(document).on('change', 'input[name="autoship_scheduled_order_upsell_carousel"]', function() {
    if ($(this).is(':checked')) {
      $('.autoship_upsell_disable_carousel_js_wrap').show();
    } else {
      $('.autoship_upsell_disable_carousel_js_wrap').hide();
    }
  });
   ...
   * Wholesale Pricing
   */
  const dynamicResult = function( event, resultsValues ){
    ...
    resultsValues.form.find("#dynamic-pricing-bulk-notice a").attr("href", resultsValues.response.download_link );

  }

  $( document ).on( 'autoship_autoship_bulk_update_scheduled_orders_wholesale_pricing_BulkActionComplete', dynamicResult );
});
