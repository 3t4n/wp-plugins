...
- Fixed: Resolved an issue where a customer would not receive the WooCommerce product sale price when adding a one-time item to the cart

= 2.8.1 - 2025-02-17 =

- Fixed: Resolved a vulnerability related to Cross-Site Scripting (XSS).

- Fixed: Resolved an issue with the WooCommerce Wholesale Prices integration where, in My Account > Scheduled Orders, if a customer updated a product to a quantity below the minimum item count, the warning notification included the product name twice.

- Fixed: Resolved an issue with the WooCommerce Wholesale Prices integration where a critical error occurred in My Account > Scheduled Orders when both the Dynamic Pricing Extension and the legacy product sync option were enabled.

- Fixed: Resolved an issue with the Dynamic Pricing Extension where updated pricing was not displayed when switching between the one-time purchase and Autoship options in the cart, even though the correct price was applied at checkout.

= 2.8.0.1 - 2025-01-09 =

- Fixed: Resolved an issue that prevented product data from syncing with QPilot when the _WooCommerce Wholesale_ plugin was in use.

- Fixed: Resolved an issue with the _WooCommerce Wholesale Prices_ integration when requiring minimum item quantities for wholesale customers was not being validated and enforced by the Native UI Display of Scheduled Orders in My Account.  Now, when a wholesale customer is logged in and managing their Scheduled Order in My Account, and attempts to update their Scheduled Order to an item quantity that is less than the minimum required, they will be shown an error and the update will not succeed until the minimum item quantity is met.

- Functions updated:
  - **autoship_sync_product** - Passes a new parameter for **WC_Product**
  - **autoship_push_product** - Passes a new parameter for **WC_Product** or boolean and logic to check if the new param is not false
  - **autoship_product_dropdown_get_price_range** - Calls a new function that filters the prices in the Add product dropdown
  - **autoship_validate_wholesale_price** - Fixed logic that checks if the product meets the Minimum order quantity if set.
  - **autoship_update_schedule_items_wholesale_price** - Fixed bug with an undefined variable
  - **autoship_product_dropdown_get_price_range** - Calls a new function to filter the prices in the Add product dropdown
  - **autoship_validate_wholesale_price** - Fixed logic for checking if MOQ or MOS rules are set and whether to display a message if any of the rules are not met.
  - **autoship_update_schedule_items_wholesale_price** - Fixed an undefined variable

- Functions created:
  - **filter_woocommerce_quantity_input_args** - Adds min attribute to quantity input if the product has a minimum order quantity set
  - **autoship_filter_add_schedule_items_wholesale_price** - Filter function for Add product dropdown items

= 2.8.0 - 2024-12-09 =

- NEW! Autoship Cloud now has a direct integration with the [WooCommerce Wholesale Prices plugin](https://wordpress.org/plugins/woocommerce-wholesale-prices/) which enables B2B merchants to manage wholesale prices and customers with the features of Autoship: Subscription Products, Scheduled Orders, Recurring Payments, and more!

  - Merchants using the "Autoship Cloud Wholesale Pricing Extension" plugin to integrate with WooCommerce Wholesale Prices should deactivate the "Autoship Cloud Wholesale Pricing Extension" plugin after updating the Autoship Cloud plugin to version 2.8.0 or higher.

- Fixed: Resolved an issue in the WordPress Admin Menu ( _WP- Admin > Autoship Cloud >> Retain & Grow_ ) where navigation to "Retention Workflows" and "Dunning" pages was broken. "Retention Workflows" and "Dunning" features are now directly accessible in the plugin.
...
- Fixed: Resolved compatibility issues Scheduled Order metadata  and HPOS (High Performance Order Storage)
  - Functions updated:
    - **autoship_register_edit_order_metaboxes** - Added code that checks if HPOS is enabled and based on that register the metabox in the correct screen ID.
    - **autoship_add_scheduled_order_ids_metabox** - Added code that checks if $post param is an instance of WP_Post before getting WC order.
    - **autoship_maybe_save_new_scheduled_order_ids** - Added call to **remove_action** to prevent loop on metadata saving

- Fixed: Resolved compatibility issues with versions of PHP 8.3 by adding parameters required by the sprintf function.
  - Functions updated:
    - **autoship_log_qpilot_client_response_errors** - Added missing placeholder for sprintf function
    - **autoship_get_coupon_by_code** - Removed undeclared and unused variable
    - **autoship_upsert_scheduled_order** - Added missing parameters to sprintf function
    - **autoship_delete_scheduled_order_endpoint_wrapper** - Added missing parameters to sprintf function
    - **autoship_confirm_delete_schedule_notice_html** - Added missing parameters to sprintf function

= 2.6.8 -  2024-09-05 =

- Improved: Increased plugin compatibility when the Woo PayPal Payments plugin is active by refactoring the functionality to get tokens for advanced card processing when vaulting credit cards with PayPal.  This improvement removes all uses of WC Subscriptions classes from within the Autoship plugin which are listed below.
  - Functions updated:
    - **autoship_force_save_card** - Includes PayPal Payments Advanced card processing "Save card" into list of card fields to force saving
    - **autoship_adjust_sepa_order_payment_method_data** - Added check for whether the param is set to prevent warning
    - **autoship_get_ppcp_order_payment_data** - Added default values for variables
    - **autoship_get_ppcp_cc_order_payment_data** - Updated functionality to retrieve token for card payment
  - Functions deprecated:
    - **autoship_woocommerce_subscription_patch**
  - Functions removed:
    - **autosip_product_woo_subscription**
    - **autoship_woocommerce_subscriptions_is_renewal_order**
  - Classes removed:
...
- Fixed: Resolved critical errors caused by adding classes for WC Subscriptions by migrating code for the Woo PayPal Payments plugin to be enabled by a setting in the Options tab of the plugin ( _WP-Admin > Autoship Cloud > Settings >>  Options (tab)_ ).  This allows the additional classes required to support the Woo PayPal Payments to be included only when the option to support Woo PayPal Payments is enabled.

= 2.6.6 -  2024-08-27 =

- Improved: Credit Cards saved as payment methods by the [WooCommerce PayPal Payments plugin](https://woocommerce.com/products/woocommerce-paypal-payments/) now display a description of the Credit Card to customers in the format [Brand] ending in [Last Four Digts] (expires MM/YY).  For example: _Visa ending in 4321 (expires 12/29)_.
- Fixed: Critical error that would occur when the [WooPayments plugin](https://wordpress.org/plugins/woocommerce-payments/) was active.

= 2.6.5 -  2024-08-22 =

- **New!** Support for the plugin [WooCommerce PayPal Payments by Woo](https://woocommerce.com/products/woocommerce-paypal-payments/) is now available.  This integration features support for Vaulting PayPal Standard Accounts and Debit and Credit Card processing, integrated Order Item Details when Scheduled Orders are processed via PayPal, and support for customers adding new saved payment methods in My Account > Payment Methods.
  - Merchants using the PayPal Checkout plugin should refer to our documentation and follow the steps for [How to Transition to PayPal Payments from PayPal Checkout](https://support.autoship.cloud/article/1114-woocommerce-paypal-payments).  
- **New!** Retain & Grow features including surveys & offers that can be automatically triggered when subscribers select to "Cancel" or "Pause" their Scheduled Order are now available.
- Improved: The WordPress Admin menu for Autoship Cloud in _WP-Admin > Autoship Cloud_ has been organized to provide a better admin experience and includes access to new reports available in _WP-Admin > Autoship Cloud > Reports_
- Improved: Developers can now filter the URL for scripts used by the v2 Portal
  - Functions updated:
    - **autoship_enqueue_scripts** - Uses function to get URL instead of hardcoded value
  - Functions created:
    - **autoship_get_v2_portal_scripts_url** - Function that returns URL for V2 scripts

= 2.6.4 -  2024-07-03 =

* Improved: The URL for the WooCommerce Store's Payment Methods page in My Account ( _My Account > Payment Methods_ ) is now dynamically updated in its Connected QPilot Site's Properties.  This improves the ability for customer notifications and subscriber portals to send customers to the correct page when directing them to update their payment method.

* Functions updated:
  * **autoship_enqueue_scripts** - Gets the URL instead of a hardcoded value
  
= 2.6.3 -  2024-06-20 =

* **New! v2 Subscriber Portal:** The v2 Portal is a new option for displaying Scheduled Orders to customers in _My Account > Scheduled Orders_ and offers the most flexible subscriber experience of Autoship ever.  The v2 Portal is an application that is securely embedded in the _My Account > Scheduled Orders_ page without the use of an iFrame, and its display and style can be completely customized within your connected [QPilot Site's settings](https://docs.qpilot.cloud/docs/v2-subscriber-portal).  To display the v2 Portal in your WooCommerce store, navigate within your WordPress Admin to _WP-Admin > Autoship Cloud > Settings >> Options (tab)_ and then scroll to "Displaying Scheduled Orders in My Account".

= 2.6.2 -  2024-06-10 =

* **New! Product Upsell Carousel:** Sites using the Native UI to display Scheduled Orders to customers in _My Account > Scheduled Orders_ can now enable a new template that displays one or more Upsell Products within a Carousel that are linked to items already included in each Scheduled Order.  This new option called the "Product Upsell Carousel" can be enabled in the WordPress Admin ( _WP-Admin > Autoship Cloud >> Settings >> Options (tab)_ ) and will populate with any Products that are linked in WooCommerce ( _Products > Edit Product > Product Data (tab) >>> Linked Products >>> Upsell_ ) and enabled for Autoship.
* Improved: The options for **Displaying Scheduled Orders in My Account** are now better organized so that additional options related to each display option are easier to identify and enable/disable.
  * Functions updated:
    * **autoship_get_scheduled_order_item_add_url** - Added redirect param
    * **autoship_add_to_scheduled_order_endpoint_wrapper**- Added redirect param
    * **autoship_get_scheduled_order_link_params** - Added redirect param
    * **autoship_enqueue_scripts** - Includes new JS scripts for Carousel and v2 Portal display Option
    * **autoship_options_section** - Added HTML for new v2 Portal display option
  * Functions created:
    * **autoship_scheduled_order_upsell_products_template_display** - Displays the Upsell products widget
    * **autoship_scheduled_order_upsell_products_template_heading** - Filters Woo product upsells section heading
    * **autoship_scheduled_order_upsell_products_template_loop_add_to_cart**- Filters Woo After Shop loop item
    * **autoship_scheduled_orders_v2_portal_display** - Function for displaying v2 Portal template

= 2.6.1 -  2024-05-09 =

* Improved: The loading page speed is now about 25%-30% faster for the My Account > Scheduled Order page when using the Native UI Display option.  This decrease in load time was achieved by improving the code that fetches product variations shown in the Add Products dropdown.
* Fixed: a styling issue with the "Add product" dropdown in the Native UI Display option where the dropdown list was not displaying to the full width of the screen size when including a product or variation with a very long title. This was causing a display issue with the responsive layout on smaller screens.
  * Functions updated:
    * **autoship_activate_sync_bulk_action_handler - Added code that updates variants metadata for **_autoship_sync_active_enabled**
    * **autoship_deactivate_sync_bulk_action_handler** - Added code that updates variants metadata for **_autoship_sync_active_enabled**
    * **autoship_get_available_schedulable_variations** - Updates code for querying variants
    * **autoship_save_product_custom_fields** - Added code that updates variants metadata
  * Functions created:
    * **autoship_handle_dissable_schedule_order_options_meta_query** - Adds new metadata key for custom query
    * **autoship_update_product_variants_sync_active_flag** - Handles updating product variants metadata for **_autoship_sync_active_enabled** meta
    * **autoship_update_product_add_to_scheduled_order_flag** - Handles updating product variants metadata for **_autoship_schedule_order_enabled**

= 2.6.0 -  2024-04-04 =

* New!  Support for Cart and Checkout Blocks has been added.  Now sites can use Block elements to display the Cart or Checkout on their storefront work with Autoship Checkout.  There is a limitation for displaying Autoship Options for products in the Cart Block, and more options will be included in the future as additional properties are available for WooCommerce Block elements.
  * Functions updated:
    * **autoship_checkout_create_order_line_item** - Updated logic for checking where the order was created
    * **autoship_create_scheduled_orders_on_checkout** - Updated logic for checking where the order was created
    * **autoship_create_scheduled_orders_on_payment_status_changed** - Updated logic for checking where the was order created
    * **autoship_create_scheduled_orders_on_payment_captured_hook** - Updated logic for checking where the order was created
    * **autoship_get_scheduled_order_data** - Updated logic for checking where the order was created
    * **autoship_woocommerce_hpos_compatibility** - Added blocks compatibility
  * Functions created:
    * **autoship_woocommerce_uses_blocks** - Utility function that checks if cart and checkout pages are using blocks
    * **autoship_get_cart_item_data** - Adds information about the Autoship Item Schedule and Next Order into the Cart Item Data so that information displays in the Cart and Checkout blocks

* New: Added support for the **BackordersAllowed** property to the product data synced with QPilot.  This enables QPilot to use its improved support for avoiding unwanted Backorders when processing Autoship Scheduled Orders by checking if the Quantity In Stock for a Product or Variation is lower than the Quantity for an Item included in an upcoming Scheduled Order.
  * Functions updated:
    * **autoship_generate_product_upsert_data** - Added BackordersAllowed property
