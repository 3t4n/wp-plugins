<?php
  ...
  function autoship_shortcode_create_scheduled_order_action( $attributes = array() ) {
    ...
    $link = apply_filters(
      'autoship_shortcode_create_scheduled_order_action_link',
      sprintf(
          '%s<a href="%s" class="button autoship-action-btn %s" %s >%s</a>%s',
          wp_kses_post($attributes['wrap_start']), 
          esc_url(autoship_get_scheduled_order_create_url($params)), 
          esc_attr($attributes['classes']), 
          wp_kses_post($attributes['atts']),
          esc_html($attributes['label']), 
          wp_kses_post($attributes['wrap_end'])
      ),
      $attributes,
      $params
    );
