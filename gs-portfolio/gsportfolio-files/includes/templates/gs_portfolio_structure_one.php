...
$output .= '<div class="row clearfix gs_p_portfolio" id="gs_p_portfolio_'.esc_attr($portfolio_id).'">';
	if ( $gs_p_loop->have_posts() ) {
		...
		while ( $gs_p_loop->have_posts() ) {
			...
			$output .= '<div class="grid single-gsp col-md-'. esc_attr($port_cols_val) .' col-sm-6 col-xs-6 mix">';
								
				$output .= '<figure class="'.esc_attr($hover_effect).'">';
			
					$output .= '<img src="'.esc_url($gs_portfolio_thumb).'" alt="'. esc_attr($gs_portfolio_alt) .'"/>';
									
					$output .= '<figcaption>';	
						$output .= '<div>';
							$output .= '<h2>'. esc_html($gs_p_title) .'</h2>';
							$output .= '<div class="gs_p_icons">';
								if ( 'on' ==  $gsp_popup_link  ) :
									$output .= '<a class="gs_p_pop open-popup-link" href="#gs_p_popup_'.get_the_id().'" data-effect="'.esc_attr($popup_effect).'"><i class="fa fa-eye"></i></a>';
								endif;
								...
								if('on' ==  $gsp_extp_link){
									if ($gs_p_meta['client_url'][0]) :
										$output .= '<a class="gs_p_link" href="'. esc_url($gs_p_meta['client_url'][0]) .'" target="'.esc_attr($gs_p_link_tar).'"><i class="fa fa-paper-plane-o"></i></a>';
									endif;
								}
							$output .= '</div>';
							$output .= '<p>'. esc_html($gs_p_content) .'</p>';
						$output .= '</div>';
					$output .= '</figcaption>';
			...
			$output .= '<div class="container">';
				$output .= '<div class="row">';
					$output .= '<div class="gs_p_popup_img col-md-6 col-sm-12">';
						$output .= '<img src="'.esc_url($gs_portfolio_full_img).'" alt="'. esc_attr($gs_portfolio_alt) .'">';
					$output .= '</div>';
					...
					$output .= '<div class="gs_p_popup_content col-md-6 col-sm-12">';
						...
						if ($gs_p_meta['client_url'][0]) :
							$output .= '<a class="gsp_btn" href="'. esc_url($gs_p_meta['client_url'][0]) .'" target="'.esc_attr($gs_p_link_tar).'">'. esc_html($gs_p_button_txt) .'</a>';
						endif;
					$output .= '</div>';
