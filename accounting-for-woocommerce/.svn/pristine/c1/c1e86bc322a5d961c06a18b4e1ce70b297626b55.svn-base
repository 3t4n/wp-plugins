...
<div class="wrap">
	...
	<?php if($hpos === 'no' || $wccutdse === 'no'): ?>
		<div class="notice notice-warning ">
			<?php if($hpos === 'no'): ?>
			<p>
				<?php _e("High-performance order storage is recommanded.", 'accounting-for-woocommerce'); ?>
			</p>
			<?php endif; ?>
			<?php if($wccutdse === 'no'): ?>
			<p>
				<?php _e("Enabling compatibility mode is highly recommanded for accounting exports.", 'accounting-for-woocommerce'); ?>
			</p>
			<?php endif; ?>
			<p>
				<a href="<?php echo esc_url(add_query_arg( array('page'=>'wc-settings', 'tab'=>'advanced', 'section'=>'features'), admin_url('admin.php'))); ?>">
					<?php _e('Go to WooCommerce settings', 'accounting-for-woocommerce'); ?>
				</a>
			</p>
		</div>
	<?php endif; ?>
	...
	<?php foreach($export_formats as $name=>$label): ?>
		...
		<?php else: ?>
			<a href="<?php echo esc_url (add_query_arg('format', $name)); ?>" class="nav-tab ">
				<?php echo esc_html($label); ?>
			</a>
		<?php endif; ?>
	<?php endforeach; ?>
