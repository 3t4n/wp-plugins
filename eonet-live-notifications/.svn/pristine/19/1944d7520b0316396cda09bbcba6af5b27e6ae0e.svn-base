...
(function($) {
    ...
    $.eoSettingsManager = function(){
        ...
        inst.defaultPanelWatcher = function() {
            ...
            // Get the first element if noone is specified
            if( current_tab.length <= 0){
                current_tab = $nav_tabs.find('li a').first().attr('href');
            }

            var current_slug = current_tab.substring(8);
        ...
        inst.saveWatcher = function(){
            ...
            $parent.on('click', '.eo_admin_settings_saving button', function (e) {
                ...
                jQuery.post(ajaxurl, data, function(response) {
                    ...
                    if(object.length !== 0) {
                        // Alert :
                        var alertClass = (object.status === 'success') ? 'fa fa-check' : 'fa fa-times';
