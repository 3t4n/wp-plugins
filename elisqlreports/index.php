 ...
 *         /__/::\
 Copyright \__\/\:\__  © 2011-2025 Eli Scheetz (email: support@supersecurehosting.com)
 *            \  \:\/\
...
function ELISQLREPORTS_view_report($Report_Name = '', $MySQL = '') {
	...
	$report = '<div id="'.sanitize_title($Report_Name).'" class="ELISQLREPORTS-Report-DIV" style="'.preg_replace('/\s++/', " ", str_replace('"', "'", $GLOBALS["ELISQLREPORTS"]["settings_array"]["default_styles"])).'"><h2 class="ELISQLREPORTS-Report-Name">'.$Report_Name.'</h2>';
	if (is_admin()) {
	...
	if ($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["rows"] && $GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["result"] && is_array($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["result"]) && count($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["result"]) == $GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["rows"]) {
		...
		for ($row=0; $row<count($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["result"]); $row++) {
			...
			foreach ($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["result"][$row] as $field => $value)
				$report .= '<td>'.($value).'</td>';
			$report .= '</tr>';
		}
		$report .= '</tbody></table>';
	} elseif ($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["errors"]) {
		foreach ($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["errors"] as $errno => $error)
			$report .= '<div class="error"><ul><li>Error: '.(is_admin()?htmlspecialchars(print_r(array($errno => $error),1)):'Query failed!').'</li></ul></div>';
	} elseif ($GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["rows"])
		$report .= '<div class="updated"><ul><li>Query affected '.$GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["rows"].' rows in '.$GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["time"].' seconds!</li></ul></div>'.print_r(array("<pre>",$GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["result"],"</pre>"), 1);
	else
		$report .= '<li>No Results in '.$GLOBALS["ELISQLREPORTS"]["query_times"][$SQLkey]["time"].' seconds!</li>';
