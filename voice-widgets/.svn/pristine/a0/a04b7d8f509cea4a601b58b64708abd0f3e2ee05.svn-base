...
/**
 * Plugin Name: Voice Audio Widgets
 * Plugin URI: https://wordpress/org/voice-widgets/
 * Description: Voice Audio Widgets to speak to your users 
 * Author: Voice Audio Tools
 * Version: 5.9.0
 * Author URI: https://www.dna88.com/product/voice-widgets-pro/
...
if ( ! class_exists( 'QC_Voice_Widgets' ) ) {
    ...
    final class QC_Voice_Widgets {
        ...
        private function __construct() {
            ...
            add_action( 'admin_init', [$this, 'QC_text_to_speech_register_plugin_settings'] );

            add_action( 'admin_init', [$this, 'QC_register_plugin_settings_speech_to_text'] );
        ...
        public function admin_menu(){
            ...
            add_submenu_page(
                'edit.php?post_type=wp_voicemsg_record',
                __( 'Speech To Text', 'qcld-wp-voice' ),
                __( 'Speech To Text', 'qcld-wp-voice' ),
                'manage_options',
                'wpvoice_speech_to_text',
                array($this,'QC_wpvoicemessage_voice_speech_to_text_callback')
            );
            ...
            add_submenu_page(
                'edit.php?post_type=wp_voicemsg_record',
                __( 'Text to Speech', 'qcld-wp-voice' ),
                __( 'Text to Speech', 'qcld-wp-voice' ),
                'manage_options',
                'wpvoice_to_text_speech',
                array($this,'QC_wpvoicemessage_voice_to_text_speech_callback')
            );
        ...
        public function admin_scripts() {
            ...
            if( 'wp_voicemsg_record' === $typenow ) {
                ...
                $convert_recorded_audio                     =  get_option('convert_recorded_audio') ? get_option('convert_recorded_audio'): 'mp3';

                wp_enqueue_style( 'wp-color-picker' );
                wp_enqueue_style( 'qc-voice-widgets-audio',  QC_VOICEWIDGET_ASSETS_URL . 'css/audio_admin.css', array(), QC_VOICEWIDGET_VERSION );

                wp_enqueue_script( 'qc-voice-widgets-recorder-js', QC_VOICEWIDGET_ASSETS_URL .  'audio/WebAudioRecorder.min.js', ['jquery'], QC_VOICEWIDGET_VERSION, true );
        ...
        public function QC_text_to_speech_register_plugin_settings(){   


           
            if (isset($_POST['_wpnonce']) && $_POST['_wpnonce']) {

                wp_verify_nonce($_POST['_wpnonce'], 'voice-to-text-speech');

                if ( ( isset($_GET['page']) && $_GET['page'] == 'wpvoice_to_text_speech' ) && isset($_POST['submit']) ) {

                    if (isset($_POST['text_to_speech_enable']) == 1 ) {
                        update_option('text_to_speech_enable', sanitize_text_field( $_POST['text_to_speech_enable'] ) );
                    } else {
                        update_option('text_to_speech_enable', '');
                    }

                    if (isset($_POST['text_to_speech_voice'])) {
                        update_option('text_to_speech_voice', sanitize_text_field( $_POST['text_to_speech_voice'] ) );
                    }

                    if (isset($_POST['text_to_speech_pitch'])) {
                        update_option('text_to_speech_pitch', sanitize_text_field( $_POST['text_to_speech_pitch'] ) );
                    }

                    if (isset($_POST['text_to_speech_rate'])) {
                        update_option('text_to_speech_rate', sanitize_text_field( $_POST['text_to_speech_rate'] ) );
                    }

                    if (isset($_POST['text_to_speech_volume'])) {
                        update_option('text_to_speech_volume', sanitize_text_field( $_POST['text_to_speech_volume'] ) );
                    }

                    if (isset($_POST['text_to_speech_btn_text'])) {
                        update_option('text_to_speech_btn_text', sanitize_text_field( $_POST['text_to_speech_btn_text'] ) );
                    }

                    if (isset($_POST['text_to_speech_palyer_position'])) {
                        update_option('text_to_speech_palyer_position', sanitize_text_field( $_POST['text_to_speech_palyer_position'] ) );
                    }

                    if (isset($_POST['qcld_page_call_text_to_speech_show_pages'])) {
                        update_option('qcld_page_call_text_to_speech_show_pages', sanitize_text_field( $_POST['qcld_page_call_text_to_speech_show_pages'] ) );
                    }

                    if (isset($_POST['qcld_page_call_text_to_speech_show_pages_list'])) {
                        update_option('qcld_page_call_text_to_speech_show_pages_list', serialize( $_POST['qcld_page_call_text_to_speech_show_pages_list'] ));
                    }

                    if (isset($_POST['qcld_page_call_text_to_speech_show_blog_posts'])) {
                        update_option('qcld_page_call_text_to_speech_show_blog_posts', sanitize_text_field( $_POST['qcld_page_call_text_to_speech_show_blog_posts'] ) );
                    }

                    if (isset($_POST['text_to_speech_background_color'])) {
                        update_option('text_to_speech_background_color', sanitize_text_field( $_POST['text_to_speech_background_color'] ));
                    }

                    if (isset($_POST['text_to_speech_bg_hover_color'])) {
                        update_option('text_to_speech_bg_hover_color', sanitize_text_field( $_POST['text_to_speech_bg_hover_color'] ));
                    }

                    if (isset($_POST['text_to_speech_font_color'])) {
                        update_option('text_to_speech_font_color', sanitize_text_field( $_POST['text_to_speech_font_color'] ));
                    }
        ...
        public function QC_register_plugin_settings_speech_to_text(){

            if (isset($_POST['_wpnonce']) && $_POST['_wpnonce']) {

                wp_verify_nonce($_POST['_wpnonce'], 'voice-speech-to-text');

                if ( ( isset($_GET['page']) && $_GET['page'] == 'wpvoice_speech_to_text' ) && isset($_POST['submit']) ) {

                    if (isset($_POST['qc_voice_to_speech_stt_allow_control']) == 1 ) {
                        update_option('qc_voice_to_speech_stt_allow_control', sanitize_text_field( $_POST['qc_voice_to_speech_stt_allow_control'] ) );
                    } else {
                        update_option('qc_voice_to_speech_stt_allow_control', '');
                    }

                    if (isset($_POST['qc_voice_to_speech_stt_project_id'])) {
                        update_option('qc_voice_to_speech_stt_project_id', sanitize_text_field( $_POST['qc_voice_to_speech_stt_project_id'] ));
                    }

                    if (isset($_POST['qc_voice_to_speech_stt_private_key'])) {
                        update_option('qc_voice_to_speech_stt_private_key', stripslashes($_POST['qc_voice_to_speech_stt_private_key']) );
                    }

                    if (isset($_POST['qc_voice_to_speech_stt_lang_name'])) {
                        update_option('qc_voice_to_speech_stt_lang_name', sanitize_text_field( $_POST['qc_voice_to_speech_stt_lang_name'] ));
                    }

                    if (isset($_POST['qc_voice_to_speech_stt_lang_code'])) {
                        update_option('qc_voice_to_speech_stt_lang_code', sanitize_text_field( $_POST['qc_voice_to_speech_stt_lang_code'] ));
                    }
        ...
        public function QC_wpvoicemessage_voice_speech_to_text_callback(){
            //ob_start();
            wp_enqueue_style('qcld-wp-voice-help-page-css',  QC_VOICEWIDGET_ASSETS_URL . 'css/help-page.css');
            require_once( QC_VOICEWIDGET_PLUGIN_DIR . 'templates/voice_speech_to_text.php');

        }

        public function QC_wpvoicemessage_voice_to_text_speech_callback(){
            //ob_start();
            wp_enqueue_style('qcld-wp-voice-help-page-css',  QC_VOICEWIDGET_ASSETS_URL . '/css/help-page.css');
            require_once( QC_VOICEWIDGET_PLUGIN_DIR . 'templates/voice_to_text_speech.php');
        ...
        public function save_audio() {
            ...
            if ( isset( $_FILES['audio_data'] ) ) {
                $file_name = 'qc_audio_'.time().'.'.$convert_recorded_audio;
                //$file_name = 'qc_audio_'.time().'.wav';
               ...
               // var_dump( $_FILES['audio_data']['type'] );
               // wp_die();
        ...
        public function register_cpt() {
            register_post_type('wp_voicemsg_record', [
                ...
                'labels'              => [
                    ...
                    'all_items'             => esc_html__( 'Voice Widgets', 'voice-widgets' ),
        ...
        public function qc_voice_widgets_register_plugin_settings(){  
            ...
            register_setting( 'qc-voice-widgets-settings-group', 'convert_recorded_audio', $args );
            register_setting( 'qc-voice-widgets-settings-group', 'stt_sound_duration', $args );
            register_setting( 'qc-voice-widgets-settings-group', 'voice_cf7_enable_audio_webhooks', $args );
...
include_once('qcld-voice-to-text-speech.php');
include_once('qcld_speech_to_text.php');
...
function QC_VOICEWIDGET_pro_notice(){
    ...
    ?>
    <div id="message" class="notice notice-info is-dismissible" style="padding:4px 0px 0px 4px;background:#e80607;">
        <?php
            printf(
                __('%s  %s  %s', 'voice-widgets'),
                '<a href="'.esc_url('https://www.dna88.com/product/voice-widgets-pro/').'" target="_blank">',
                '<img src="'.esc_url(QC_VOICEWIDGET_ASSETS_URL).'/images/newyear24-voice-widgets.jpg" >',
                '</a>'
            );
        ?>
    </div>
...
add_action( 'admin_init' , 'QC_VOICEWIDGET_plugin_submenus' );
function QC_VOICEWIDGET_plugin_submenus( $menu_ord ){
    global $submenu;

    $arr = array();

    $arr[] = isset($submenu['edit.php?post_type=wp_voicemsg_record'][5]) ? $submenu['edit.php?post_type=wp_voicemsg_record'][5] : '';
    $arr[] = isset($submenu['edit.php?post_type=wp_voicemsg_record'][11]) ? $submenu['edit.php?post_type=wp_voicemsg_record'][11] : '';
    $arr[] = isset($submenu['edit.php?post_type=wp_voicemsg_record'][12]) ? $submenu['edit.php?post_type=wp_voicemsg_record'][12] : '';
    $arr[] = isset($submenu['edit.php?post_type=wp_voicemsg_record'][13]) ? $submenu['edit.php?post_type=wp_voicemsg_record'][13] : '';
    $arr[] = isset($submenu['edit.php?post_type=wp_voicemsg_record'][301]) ? $submenu['edit.php?post_type=wp_voicemsg_record'][301] : '';
    $arr[] = isset($submenu['edit.php?post_type=wp_voicemsg_record'][300]) ? $submenu['edit.php?post_type=wp_voicemsg_record'][300] : '';

    $submenu['edit.php?post_type=wp_voicemsg_record'] = $arr;
