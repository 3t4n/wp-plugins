...
<div class="wrap swpm-admin-menu-wrap">
    ...
    <div class="qcld-accordion-box">
        <form  action="<?php echo esc_attr(admin_url($action)); ?>" method="POST" enctype="multipart/form-data">
			...
			<div class="wppt-settings-section text_to_speech_wrap">
				<h4><a href="https://www.dna88.com/product/voice-widgets-pro/" target="_blank" style="font-weight: bold; color:red; text-decoration: none"> <?php echo esc_html__('Pro Feature', 'qcld-wp-voice' ); ?> ( <span style="font-weight: bold; color:#0000ff"><?php echo esc_html__('Upgrade to Pro', 'qcld-wp-voice' ); ?> </span> )</a></h4>
				<table class="form-table">
					<tr valign="top">
						<th scope="row"><?php echo esc_html__('Learning Mode Shortcode', 'qcld-wp-voice' ); ?></th>
						<td>
							<input type="text" name="qc_voice_to_shortcode_code" value="[qcld_stt_learning_mode]" />
                            <br>
							<i><?php echo esc_html__('Paste this shortcode anywhere on a page where you want the STT button to show', 'qcld-wp-voice' ); ?> </i>

							<p><?php echo esc_html__('The Speech to Text Learning mode is useful to learn a new language. It allows a learner to speak words or sentences and have the Google Cloud Speech try to recognize the words. The learner can immediately play back the spoken words to try and understand what went wrong if the words cannot be recognized by the Google Cloud Speech', 'qcld-wp-voice' ); ?></p>
						</td>
					</tr>
