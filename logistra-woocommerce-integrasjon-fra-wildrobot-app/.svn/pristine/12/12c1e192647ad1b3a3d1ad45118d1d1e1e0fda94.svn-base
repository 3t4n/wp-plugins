...
class Wildrobot_Logistra_Admin
{
	...
	public function order_actions($actions, $order)
	{
		...
		// Add send order action if not hidden
		if (get_option("wildrobot_logistra_hide_send_buttons", "no") !== "yes") {
			$actions['wildrobot_send'] = array(
				'url'       => add_query_arg(array(
					'action' => 'wildrobot_logistra_send_order',
					'order_id' => $order_id,
					'sent' => $sent,
					'show_label' => $show_label_download,
					'label_url' => urlencode($label_url)
				), admin_url('admin-ajax.php')),
				'name'      => __('Send ordre', 'wildrobot-logistra'),
				'action'    => "view wildrobot_send wildrobot-logistra-order-action-send wc-action-button-wildrobot_send"
			);
		}

		// Add override action if not hidden
		if (get_option("wildrobot_logistra_hide_override_buttons", "no") !== "yes") {
			$actions['wildrobot_override'] = array(
				'url'       => add_query_arg(array(
					'action' => 'wildrobot_logistra_override_order',
					'order_id' => $order_id,
					'sent' => $sent
				), admin_url('admin-ajax.php')),
				'name'      => __('Override', 'wildrobot-logistra'),
				'action'    => "view wildrobot_override wildrobot-logistra-order-action-override wc-action-button-wildrobot_override"
			);
		}

		// Add picklist action if active
		if (get_option("wildrobot_logistra_picklist_active") === "yes") {
			...
			$actions['wildrobot_picklist'] = array(
				'url'       => add_query_arg(array(
					'action' => 'wildrobot_logistra_picklist',
					'order_id' => $order_id,
					'picked' => $picked
				), admin_url('admin-ajax.php')),
				'name'      => __('Plukkliste', 'wildrobot-logistra'),
				'action'    => "view wildrobot_picklist wildrobot-logistra-picklist-order wc-action-button-wildrobot_picklist"
			);
		}
	...
	public function add_service_partner_picker_field_to_checkout($order)
	{
		...
		if ($order->get_meta('_wildrobot_logistra_service_partner_name', true)) {
			echo '<p><strong>' . __('Utleveringssted') . ':</strong> ' . $order->get_meta('_wildrobot_logistra_service_partner_name', true) . ', ' . $order->get_meta('_wildrobot_logistra_service_partner_city', true) . '</p>';
		} else {
