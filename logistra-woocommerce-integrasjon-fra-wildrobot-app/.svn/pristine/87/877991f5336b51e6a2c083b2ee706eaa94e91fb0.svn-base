...
class Wildrobot_Logistra_Order_Utils
{
	...
	public function bulk_actions_send_order_transport($actions)
	{
		...
		$actions['wildrobot_print_freight_labels'] = __('Print fraktetiketter', 'wildrobot-logistra');
		return $actions;
	}


	public function bulk_print_freight_labels($redirect_to, $action, $post_ids)
	{
		if ($action !== 'wildrobot_print_freight_labels') {
			return $redirect_to;
		}


		// Create new TCPDI object
		$pdf = new TCPDI();
		...
		try {
			...
			if (empty($pdf_urls)) {
				$logger->info('No PDF URLs found for the selected orders.', $context);
				wp_die('No freight labels found for the selected orders.');
			}
			...
			foreach ($pdf_urls as $url) {
				...
				if ($content === false) {
					...
					$logger->error('Failed to retrieve content from URL: ' . $url, $context);
					continue;
				}
				...
				try {
					...
					for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
						...
						$pdf->AddPage($size['orientation'], [102, 192]); // Endre til A4 her hvis man trenger
						$pdf->useTemplate($tplIdx);
					}
				} catch (Exception $e) {
					// Log the exception and continue with the next PDF
					$logger->error('Error processing PDF from URL: ' . $url . ' - ' . $e->getMessage(), $context);
				}
			}
			...
			if ($pdf->PageNo() == 0) {
				$logger->info('No pages added to the PDF.', $context);
				wp_die('No valid freight labels found to merge.');
			}

			// Clean the output buffer if any
			if (ob_get_length()) ob_end_clean();

			// Send headers to initiate file download
			header('Content-Type: application/pdf');
			header('Content-Disposition: attachment; filename="freight_labels_merged.pdf"');
			header('Cache-Control: private, max-age=0, must-revalidate');
			header('Pragma: public');

			// Output the merged PDF
			$pdf->Output('freight_labels_merged.pdf', 'I');

			// Terminate to prevent further output
			exit;
		} catch (Exception $e) {
			$logger->error('Error in bulk print freight labels: ' . $e->getMessage(), $context);
			wp_die('An error occurred while generating the merged freight labels.');
		}
	}

	public function display_freight_labels_notices()
	{
		$notice = get_transient('wildrobot_freight_labels_notice');
		if ($notice) {
	...
	public static function calculate_package_amount(&$packages, &$counter, &$counted_groups, $product_id, $quantity, $product_ids_to_quantity_map_for_container)
	{
		...
		for ($i = 0; $i < $package_amount; $i++) {
			// Only increment counter if there are more then one item in the container
			...
			for ($j = 0; $j < $packages_needed; $j++) {
				...
				$packages[$counter] = [
					...
					"amount"      => 1,
					"description" => strip_tags($package["name"]),
					"type"        => "package",
		...
		if ($product->get_meta('_wildrobot_separate_package_for_product', true) === "yes") {
			...
			if (!in_array($separate_package_for_product_name, $counted_groups, true)) {
				...
				// Only increment counter if there are more then one item in the container
				// if (count($product_ids_to_quantity_map_for_container) > 1) {
				// 	$counter++;
				// }
				// Do not add a package if there are no items in the current base package

				if ($counter === 0 && floatval($packages[$counter]["weight"]) === floatval(0)) {
					// don't add anything
				} else {
					// then we already have multiple packages so we need to add a new one
					$counter++;
				}
				...
				$packages[$counter] = [
					...
					"amount"      => 1,
					"description" => wp_strip_all_tags($separate_package_for_product_name),
					"type"        => "package",
