...
class Wildrobot_Logistra_Consignment_Order
{
	public static function get_consignment_args_from_order_id($order_id, $throw_on_empty_delivery_relation = true)
	{
		...
		foreach ($shipping_methods as $shipping_method) {
			...
			// For backward compatibility, just add the old meta data.
			$choosen_service_partner_number = $order->get_meta('_shipping_service_partner', true);
			$choosen_service_partner_customer_number = $order->get_meta('_wildrobot_logistra_service_partner_customer_number', true) ?? null;
			$service_partner_match = null;
			if (empty($choosen_service_partner_number)) {
				// if that is empty, we try to get the new meta data.
				$choosen_service_partner_number = $order->get_meta('_wildrobot_logistra_service_partner_number', true);
				if (!empty($choosen_service_partner_number)) {
					$service_partner_match = [
						"number" => $order->get_meta('_wildrobot_logistra_service_partner_number', true),
						"name" => $order->get_meta('_wildrobot_logistra_service_partner_name', true),
						"customer-number" => $order->get_meta('_wildrobot_logistra_service_partner_customer_number', true),
						"name" => $order->get_meta('_wildrobot_logistra_service_partner_name', true),
						"postcode" => $order->get_meta('_wildrobot_logistra_service_partner_postcode', true),
						"city" => $order->get_meta('_wildrobot_logistra_service_partner_city', true),
						"country" => $order->get_meta('_wildrobot_logistra_service_partner_country', true),
					];
				}
			}
			...
			$consignment_arg =  [
				...
				"choosen_service_partner_number" => $choosen_service_partner_number,
				"choosen_service_partner_customer_number" => $choosen_service_partner_customer_number,
				"service_partner_match" => $service_partner_match,
				"order_id" => $order_id,
				"order_number" => $order_number,
				"weight" => $weight,
				"country" => $country,
				...
				"service_partner_type" => $service_partner_type,
			];
			array_push($consignment_args, $consignment_arg);
			...
			foreach ($delivery_relation["services"] as $service) {
				if ($service == "wildrobot_automatic_print_return_label" || $service == "wildrobot_automatic_email_return_label") {
					...
					$service_partner_type_return = null;
					foreach ($print_return_delivery_relation["services"] as $service) {
						if ($service === "wildrobot_only_manned_service_partner") {
							$service_partner_type_return = "manned";
						}
						if ($service === "wildrobot_only_unmanned_service_partner") {
							$service_partner_type_return = "locker";
						}
					}
					...
					$print_return_order_args =  [
						...
						"total_with_tax" => $order_amounts["total_with_tax"],
						"total_without_tax" => $order_amounts["total_without_tax"],
						"service_partner_type" => $service_partner_type_return,
					];
	...
	private static function packages($order, $transport_description, $weight)
	{
		...
		if (get_option('wildrobot_logistra_calculate_dimensions') === "yes" || get_option('wildrobot_logistra_calculate_volume') === "yes") {
			$dimension_result = Wildrobot_Logistra_Order_Utils::get_dimensions_from_container($product_map);
	...
	private static function transport_description($order)
	{
		...
		if ($use_product_names) {
			$name_array = array_map(function ($product) {
				return $product->get_quantity() . 'x' . wp_strip_all_tags($product->get_name()) . ',';
			}, $items);
