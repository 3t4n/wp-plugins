...
function logistra_robots_shipping_method_init()
{
    if (!class_exists('WC_Logistra_Robots_Shipping_Method')) {
        class WC_Logistra_Robots_Shipping_Method extends WC_Shipping_Method
        {
            ...
            public $require_service_partner;
            public $require_user_groups;
            public $exclude_user_groups;
            ...
            function init()
            {
                ...
                $this->require_service_partner        = $this->get_option('require_service_partner');
                $this->require_user_groups = (array) $this->get_option('require_user_groups', array());
                $this->exclude_user_groups = (array) $this->get_option('exclude_user_groups', array());
            ...
            public function calculate_shipping($package = array())
            {
                ...
                if ($this->some_classes || $this->require_user_groups) {
                    $valid = false;
                    $some_class_valid = true;
                    $require_user_group_valid = true;
                    if ($this->some_classes) {
                        $some_class_valid = false;
                    ...
                    if ($this->require_user_groups) {
                        $require_user_group_valid = false;
                        $user = wp_get_current_user();
                        foreach ($this->require_user_groups as $required_user_group) {
                            if (in_array($required_user_group, (array) $user->roles)) {
                                $require_user_group_valid = true;
                                break;
                            }
                        }
                    }
                    if ($some_class_valid && $require_user_group_valid) {
                        $valid = true;
                    }
                }


                // THEN Restrictive checks

                if ($this->exclude_user_groups) {
                    $user = wp_get_current_user();
                    foreach ($this->exclude_user_groups as $exclude_user_group) {
                        if (in_array($exclude_user_group, (array) $user->roles)) {
                            $valid = false;
                        }
                    }
                }
                ...
                if ($valid) {
                    ...
                    $order_total = 0;
                    $order_total_with_discounts_subtracted = 0;
                    ...
                    $order_total_with_discounts_subtracted = $order_total - $order_total_without_free_freight;
                    $coupons = WC()->cart->get_coupon_discount_totals();
                    foreach ($coupons as $coupon_amount) {
                        if (is_numeric($coupon_amount)) {
                            $order_total_with_discounts_subtracted = $order_total_with_discounts_subtracted - $coupon_amount;
                        }
                    }
                    $fees = WC()->cart->get_fees();
                    foreach ($fees as $fee) {
                        $order_total_with_discounts_subtracted = $order_total_with_discounts_subtracted + $fee->amount;
                    }

                    $order_total_with_discounts_subtracted = apply_filters('logistra_robots_order_total_with_discounts_subtracted', $order_total_with_discounts_subtracted, $package);



                    // Check if service partner is required
                    if ($this->require_service_partner === "yes") {
                        try {
                            ...
                            if (empty($country) || empty($postcode)) {
                                throw new Exception("No country or postcode to check if service partner is required");
                            }
                            $delivery_relation = Wildrobot_Logistra_DB::get_delivery_relation_with_transport_agreement($rate["id"]);
                            if (empty($delivery_relation)) {
                                throw new Exception("No delivery relation to check if service partner is required, please map your shipping method to a transport agreement.");
                            }
                            $service_partner_data = Wildrobot_Logistra_Cargonizer::get_service_partners($country, $postcode, $delivery_relation["carrier"], $delivery_relation["wr_id"], empty($address_1) ? null : $address_1);
                            if (empty($service_partner_data["service_partners"])) {
                                throw new Exception("No service partners found when checking if service partner is required");
                            }
                        } catch (\Throwable $error) {
                            if ($this->debug === "yes") {
                                $logger->error("Error when checking if service partner is required: " . $error->getMessage(), $context);
                            }
                            $valid = false;
                        }
                    }
                    ...
                    if ($should_calculate_freight_cost) {
                        try {
                            ...
                            if ($this->debug === "yes") {
                                $logger->info("Freight cost RESPONSE: net " . wc_print_r($estimated_costs['net-amount'], true), $context);
                                $logger->info("Freight cost RESPONSE: gross " . wc_print_r($estimated_costs['gross-amount'], true), $context);
                                $logger->info("Freight cost RESPONSE: estimated " . wc_print_r($estimated_costs['estimated-cost'], true), $context);
                    ...
                    if ($this->free_treshhold === "yes") {
                        ...
                        if ($order_total_with_discounts_subtracted >= floatval($this->free_value)) {
                            if ($notice === "yes") {
                                wc_clear_notices();
                            ...
                            $rate['label'] = $rate['label'] . ' ' . apply_filters('logistra_robots_free_label_title_extended', __('(Gratis)', 'logistra-robots'));
                            $has_cost = true;
                        } else if ($notice && $almost_value > 0 &&  $order_total_with_discounts_subtracted >= floatval($this->free_value) - $almost_value) {
                            wc_clear_notices();
                            $message = apply_filters('logistra_robots_free_freight_message', sprintf(__('%s kr fra fraktfri levering. <a href="%s">Fortsett å handle</a>', 'logistra-robots'), round(floatval($this->free_value) - $order_total_with_discounts_subtracted, 0, PHP_ROUND_HALF_UP), wc_get_page_permalink("shop")));
            ...
            function settings()
            {
                ...
                return array(
                    ...
                    'require_user_groups' => array(
                        'title'                 => __('Krever brukergruppe', 'logistra-robots'),
                        'type'                  => 'multiselect',
                        'description'           => __('Hvis du velger brukergrupper her vil fraktmetoden kun bli synlig dersom kunden har en av disse brukergrupperne. For å fjerne markering, hold inne CTRL (Windows/Linux) / Command (MacOS) og klikk med musen.', 'logistra-robots'),
                        'class'                 => 'wc-enhanced-select',
                        'default'               => [],
                        'options'               => $user_roles,
                        'desc_tip'              => true
                    ),
                    'exclude_user_groups' => array(
                        'title'                 => __('Ekskluder brukergrupper', 'logistra-robots'),
                        'type'                  => 'multiselect',
                        'description'           => __('Hvis du velger brukergrupper her vil fraktmetoden kun være synlig dersom kunden ikke har en av disse brukergrupperne. For å fjerne markering, hold inne CTRL (Windows/Linux) / Command (MacOS) og klikk med musen.', 'logistra-robots'),
                        'class'                 => 'wc-enhanced-select',
                        'default'               => [],
                        'options'               => $user_roles,
                        'desc_tip'              => true
                    ),
                    'require_service_partner' => array(
                        'title'                 => __('Krever utleveringssted', 'logistra-robots'),
                        'type'                  => 'checkbox',
                        'class'                 => '',
                        'css'                   => 'width: 16px; paddding-top: 10px;',
                        'placeholder'           => '',
                        'label'                 => __('Hvis valgt, vil fraktmetoden kun være tilgjengelig dersom det er mulig å velge utleveringssted på på handlekurven. Dette vil også kreve at kunden har valgt et utleveringssted i utsjekkingsprosessen. Nb! Påkrevd innstilling for transportørene <b>Porterbuddy</b> og <b>Instabox</b>.', 'logistra-robots'),
                    ...
                    "dynamic_freight_prices" => array(
                        'title' => __('Dynamiske fraktpriser', 'logistra-robots'),
                        'type' => 'title',
                        'desc_tip' => true,
                    ),
                    'estimate_freight_cost' => array(
                        ...
                        'description'           => __('Krever at fraktmetoden er relatert til en leveranse i Wildrobot innstillinger. Gjør et API-kall til transport leverandør når kunden legger til varer for å prøve å få en estimert pris på leveransen som puttes inn i kostnaden til fraktmetoden.', 'logistra-robots'),
                        'default'               => 'no',
                        'desc_tip'              => true,
                        'custom_attributes'     => array(),
                        'disabled'              => false,
