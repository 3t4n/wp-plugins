...
class Wildrobot_Logistra_Consignment
{
	...
	// Change indicators
	private $choosen_service_partner_number;
	private $choosen_service_partner_customer_number;
	private $wr_id;
	...
	public function __construct($args)
	{
		...
		$args = wp_parse_args($args, [
			...
			"export_reason" => "",
			"export_type" =>  "",
			"choosen_service_partner_number" => "",
			"choosen_service_partner_customer_number" => "",
			"city" => "",
			"consignee" => [],
			"carrier_message" => "",
			"consignee_message" => "",
		...
		$args["service_partner_type"] = $args["service_partner_type"];
		...
		try {
			if ($this->transport_agreeement["service_partner_possible"]) {
				$service_partner_response = self::service_partner($args["postcode"], $args["country"], $args["carrier"], $args["wr_id"], $args["choosen_service_partner_number"], $args["choosen_service_partner_customer_number"], $order_id, $args["consignee"]["address1"], $args["service_partner_type"]);
				$this->service_partner_match = empty($args["service_partner_match"]) ? $service_partner_response["match"] : $args["service_partner_match"];
		...
		$this->choosen_service_partner_number = $args['choosen_service_partner_number'];
		$this->choosen_service_partner_customer_number = $args['choosen_service_partner_customer_number'];
	 ...
	 * @return void
	 */
	private function service_partner($postcode, $country, $carrier, $wr_id, $choosen_service_partner_number, $choosen_service_partner_customer_number, $order_id = null, $address = null)
	{
		...
		// } 
		// Only send with address with choosen_service_partner_number is not set and address is set
		$send_with_address = empty($choosen_service_partner_number) && !empty($address);
		...
		if (!empty($choosen_service_partner_number)) {
			$found_match = false;
			foreach ($service_partner_data["service_partners"] as  $servicePartner) {
				if (intval($choosen_service_partner_number) == intval($servicePartner['number'])) {
					if (!empty($choosen_service_partner_customer_number)) {
						if ($choosen_service_partner_customer_number == $servicePartner['customer-number']) {
			...
			// removed this as it is not relevant anymore.
			// if (!$found_match) {
			// 	$settings = implode(", ", [$postcode, $country, $carrier, $choosen_service_partner_number]);
	...
	public function send_backend($args = [])
	{
		...
		if (property_exists($shipment->cargonizerShipment, 'id') && property_exists($shipment->cargonizerShipment, 'number')) {
			...
			if ($order) {
				if (wc_string_to_bool($this->is_return_label)) {
					$order->update_meta_data('logistra-robots-freight-number-return', $shipment->cargonizerShipment->number);
				} else {
					$order->update_meta_data('logistra-robots-freight-number', $shipment->cargonizerShipment->number);
				}
			}
		}
		...
		if (property_exists($shipment->cargonizerShipment, 'trackingUrl') && !empty($shipment->cargonizerShipment->trackingUrl)) {
			...
			if ($order) {
				if (wc_string_to_bool($this->is_return_label)) {
					$order->update_meta_data('logistra-robots-tracking-url-return', $shipment->cargonizerShipment->trackingUrl);
				} else {
		...
		if (property_exists($shipment->cargonizerShipment, 'consignmentPdf') && !empty($shipment->cargonizerShipment->consignmentPdf)) {
			...
			if ($order) {
				if (wc_string_to_bool($this->is_return_label)) {
					$order->update_meta_data('logistra-robots-freight-label-url-return', $shipment->cargonizerShipment->consignmentPdf);
				} else {
	...
	public function to_json()
	{
		...
		$consignment = [
			...
			"messages" => [
				"carrier" => substr($this->carrier_message, 0, 70),
				"consignee" => substr($this->consignee_message, 0, 70),
			],
			"references" => [
		...
		if (!empty($this->service_partner_match)) {
			...
			$consignment["parts"]["service_partner"] = [
				...
				"customer-number" => $this->service_partner_match["customer-number"],
	...
	public function to_estimate()
	{
		...
		// remove, as this shoild not be in estmations request.
		// if (!empty($this->service_partner_match)) {
		...
		foreach ($this->packages as $package) {
			...
			foreach ($package["_attributes"] as $key => $value) {
				if (!empty($value)) {
					if ($key === "type" && $this->product === "tg_pall") {
						$item->addAttribute($key, apply_filters("wildrobot-logistra-estimate-package-type", "1_1_palette", $package, $this->order_id));
					} else {
