...
class Wildrobot_Logistra_Cargonizer
{
    ...
    public static function get_service_partners($country, $postcode, $carrier, $wr_id, $address = null, $city = null, $service_partner_type = null)
    {
        if (empty($address)) {
            $sanitized_type = $service_partner_type ?? "all";
            $transient_name = $country . '-' . $postcode . '-' . $wr_id . '-' . $sanitized_type . '-wildrobot';
        } else {
            $formatted_address =  preg_replace("/[^a-zA-Z0-9]+/", "", $address);
            $sanitized_type = $service_partner_type ?? "all";
            $transient_name = $country . '-' . $postcode . '-' . $wr_id . '-' . $formatted_address . '-' . $sanitized_type . '-wildrobot';
        }
        ...
        if (empty($transient) || (array_key_exists('service_partners', $transient) && empty($transient["service_partners"]))) {
            ...
            if (!empty($service_partner_type)) {
                $params = array_merge($params, array(
                    "type" => esc_html($service_partner_type),
                ));
            }
            ...
            if (!empty($wr_id)) {
                ...
                } else if (in_array($wr_data['product'], self::$helthjem_products)) {
                ...
                } else if (!empty($wr_data['product'])) {
                    $params = array_merge($params, array(
                        'product' => $wr_data['product'],
                    ));
                }
            }
            ...
            if (!empty($data["service_partners"])) {
                if (get_option("wildrobot_logistra_filter_out_pakkeautomat") === "yes" && $service_partner_type === null) {
