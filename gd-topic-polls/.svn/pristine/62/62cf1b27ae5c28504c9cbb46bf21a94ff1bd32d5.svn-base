...
class Poll {
    ...
    public function __construct( $data = array() ) {
        $this->respond = gdpol_settings()->get( 'poll_field_respond_default' );
        $this->show = gdpol_settings()->get( 'poll_field_show_default' );
        $this->removal = gdpol_settings()->get( 'poll_field_removal_default' );
        $this->notify = ( gdpol_settings()->get( 'active', 'notify' ) ? gdpol_settings()->get( 'poll_field_notify_default' ) : 'none' );
        $this->responses[0]['response'] = gdpol_settings()->get( 'poll_field_response_default_one' );
        $this->responses[1]['response'] = gdpol_settings()->get( 'poll_field_response_default_two' );
        if ( !empty( $data ) ) {
    ...
    public static function load( $poll_id ) {
        ...
        if ( isset( $_GET['gdpol-poll-results'] ) ) {
            ...
            if ( isset( $_GET['gdpol-vote-saved'] ) ) {
                $poll->mode = 'voted';
            } else {
                if ( isset( $_GET['gdpol-invalid-vote'] ) ) {
                    $poll->mode = 'invalid';
                } else {
                    if ( $poll->is_open() && $poll->has_voted( $poll->user_id() ) ) {
                        $poll->mode = 'voter';
                    }
                }
            }
    ...
    public function render_form_fields() {
        ...
        <script class="gdpol-poll-settings" type="application/json"><?php 
        echo json_encode( $data );
        ?></script>

        <input type="hidden" name="gdpol_poll_id" value="<?php 
        echo $this->id;
        ?>"/>
        <input type="hidden" name="gdpol_topic_id" value="<?php 
        echo $this->topic_id;
        ?>"/>
    ...
    public function render_results() {
        ...
        if ( gdpol_settings()->get( 'display_show_response_elements' ) != 'full' ) {
            $class[] = 'gdpol-results-compact';
        }
        ?>
    ...
    public function admin_render_settings() : string {
        ...
        $render .= '<li>' . __( 'Respond', 'gd-topic-polls' ) . ': <strong>' . $this->_form_value_to_label( 'respond' ) . '</strong></li>';
        ...
        $render .= '<li>' . __( 'Votes', 'gd-topic-polls' ) . ': <strong>' . $this->_form_value_to_label( 'removal' ) . '</strong></li>';
        $render .= '</ul>';
        return $render;
    }
    ...
    public function admin_render_results() : string {
        ...
        foreach ( $results['labels'] as $id => $label ) {
            ...
            $render .= '<li>[' . $id . '] ' . $label;
            $render .= '<span class="_votes"><a href="admin.php?page=gd-topic-polls-votes&poll=' . $this->id . '&answer=' . $id . '">' . $votes . ' ' . _n(
                'vote',
                'votes',
                $votes,
                'gd-topic-polls'
    ...
    public function render_message( $user_id = 0 ) {
        ...
        if ( $this->mode == 'voted' ) {
            ...
            if ( $this->mode == 'invalid' ) {
                ...
                if ( $this->is_open() && $this->has_voted( $user_id ) ) {
                    $vtd = true;
                    $msg = __( 'You have already voted.', 'gd-topic-polls' );
                } else {
                    if ( $this->is_open() && !is_user_logged_in() ) {
                        $msg = __( 'You must be logged in to participate.', 'gd-topic-polls' );
                        $cde = 'error';
                    } else {
                        if ( $this->is_open() && !$this->is_allowed_to_vote( $user_id ) && $this->respond == 'reply' ) {
                            $msg = __( 'You must reply to this topic first.', 'gd-topic-polls' );
                        }
                    }
                }
            }
    ...
    public function actions( $user_id = 0 ) : array {
        $actions = array();
        if ( $this->removal == 'allow' && $this->is_open() && $this->has_voted( $user_id ) ) {
            $actions['remove'] = '<a class="gdpol-action-removal" href="' . $this->url( 'gdpol-remove-vote', $this->id ) . '">' . __( 'Remove my vote', 'gd-topic-polls' ) . '</a>';
        }
    ...
    public function count_voters() : int {
        return gdpol_db()->count_poll_voters( $this->id );
    }

    public function count_votes() : int {
    ...
    public function list_close_terms() : array {
        $list = array(
    ...
    public function list_show_terms() : array {
        return array(
    ...
    public function list_removal_terms() : array {
        return array(
            'deny'  => __( 'Do not allow user to remove own votes', 'gd-topic-polls' ),
            'allow' => __( 'Allow users to remove own votes', 'gd-topic-polls' ),
        );
    }

    public function list_notify_terms() : array {
        $list = array(
            'none'    => __( 'No notifications', 'gd-topic-polls' ),
            'instant' => __( 'Instant vote notifications', 'gd-topic-polls' ),
        );
        if ( gdpol_settings()->get( 'digest_daily', 'notify' ) ) {
            $list['daily'] = __( 'Daily digest notifications', 'gd-topic-polls' );
        }
        return $list;
    }

    public function list_respond_terms() : array {
        return array(
            'all'   => __( 'All registered users', 'gd-topic-polls' ),
            'reply' => __( 'Only users who replied to this topic', 'gd-topic-polls' ),
        );
    }

    public function list_choice_terms() : array {
        return array(
    ...
    public function form_data( $name ) : array {
        switch ( $name ) {
            case 'respond':
                return $this->list_respond_terms();
            case 'notify':
                return $this->list_notify_terms();
            case 'choice':
                return $this->list_choice_terms();
            case 'close':
                return $this->list_close_terms();
            case 'show':
                return $this->list_show_terms();
            case 'removal':
                return $this->list_removal_terms();
        }
        return array();
    }
