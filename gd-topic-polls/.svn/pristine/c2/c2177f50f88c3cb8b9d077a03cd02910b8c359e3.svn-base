...
namespace Dev4Press\Plugin\TopicPolls\bbPress;

use Dev4Press\Plugin\GDBBX\Features\Icons;
use Dev4Press\Plugin\TopicPolls\Basic\Poll;
...
class Integrate {
	...
	public function file( $type, $name, $path, $package = 'default' ) : string {
		$get = TOPICPOLLS_URL . 'templates/' . $package . '/' . $path . '/' . $name;
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'gdpol-topic-polls', 'gdpol_polls_data', array(
			'flatpickr_locale' => Enqueue::i()->registered_locale( 'flatpickr' ),
	...
	public function add_plugin_stack( $stack ) {
		if ( $this->theme_package == 'quantum' ) {
			$stack[] = TOPICPOLLS_PATH . 'templates/quantum/bbpress';
		}

		$stack[] = TOPICPOLLS_PATH . 'templates/default/bbpress';
	...
	private function _poll_cleanup( $_poll, $topic_id = 0 ) : array {
		$poll = array(
			...
			'topic_id'       => $topic_id,
			'question'       => Sanitize::text( $_poll['question'] ?? '' ),
			'description'    => isset( $_poll['description'] ) ? Sanitize::html( $_poll['description'] ) : '',
			'responses'      => array(),
			'respond'        => Sanitize::key( $_poll['respond'] ?? 'all' ),
			'choice'         => Sanitize::key( $_poll['choice'] ?? 'single' ),
			'choice_limit'   => Sanitize::absint( $_poll['choice_limit'] ?? 2 ),
			'close'          => Sanitize::key( $_poll['close'] ?? 'topic' ),
			'close_users'    => Sanitize::absint( $_poll['close_users'] ?? 20 ),
			'close_datetime' => '',
			'show'           => Sanitize::key( $_poll['show'] ?? 'always' ),
			'removal'        => Sanitize::key( $_poll['removal'] ?? 'deny' ),
			'notify'         => Sanitize::key( $_poll['notify'] ?? 'none' ),
		);

		if ( $poll['close'] == 'date' ) {
			$_datetime = Sanitize::text( $_poll['close_datetime'] );
		...
		foreach ( $_responses as $r ) {
			if ( gdpol_settings()->get( 'poll_field_responses_allow_html' ) ) {
				...
				$response = Sanitize::text( $r['response'] );
			}
