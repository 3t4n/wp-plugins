...
function spam_master_private( WP_REST_Request $request ) {
	...
	if ( ! empty( $data['k'] ) && ! empty( $data['h'] ) ) {
		$my_k = sanitize_text_field( $data['k'] );
		$my_h = sanitize_text_field( $data['h'] );
		if ( ! empty( $data['v'] ) ) {
			$my_v = sanitize_text_field( $data['v'] );
		} else {
			$my_v = '0';
		}
		...
		if ( ! empty( $is_key ) && ! empty( $is_hash ) && '1' === $my_v ) {
			...
			$exempt_count['Statistics'][] = array(
				'Version' => $spam_master_version . '-' . $db_install_version . '-' . $spam_master_subtype,
			);
			// Process status.
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			...
			$exempt_count['Statistics'][] = array(
				'White Tran' => $spam_master_white_transient,
			);
