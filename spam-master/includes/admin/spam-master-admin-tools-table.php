...
if ( isset( $_POST['update_spam_master_tools_firewall'] ) ) {
	...
	if ( ! empty( $_POST['spam_master_subtype'] ) ) {
		$spam_master_subtype = sanitize_text_field( wp_unslash( $_POST['spam_master_subtype'] ) );
		$data_address        = array( 'spamvalue' => $spam_master_subtype );
		$where_address       = array(
			'spamkey'  => 'Option',
...
<table class="wp-list-table widefat fixed striped table-view-list" cellspacing="0">
	...
	<tbody>
		...
		<tr class="alternate">
			<td colspan="2"><span class="spam-master-admin-blue spam-master-top-admin-shadow-offline"><span class="dashicons dashicons-info-outline"></span></span> <strong><em><?php echo esc_attr( __( 'Normal', 'spam-master' ) ); ?></em></strong><?php echo esc_attr( __( ', active strict firewall stance for high levels of spam.', 'spam-master' ) ); ?> <strong><em><?php echo esc_attr( __( 'Relaxed', 'spam-master' ) ); ?></em></strong><?php echo esc_attr( __( ', active firewall stance for large corporate websites or local, state and federal government agencies.', 'spam-master' ) ); ?> <strong><em><?php echo esc_attr( __( 'Super Relaxed', 'spam-master' ) ); ?></em></strong><?php echo esc_attr( __( ', passive firewall stance with low footprint and for low spam levels.', 'spam-master' ) ); ?></td>
		</tr>
		<?php echo wp_kses( $is_link_fir_set, $selected_allowed ); ?>
		<tr class="alternate">
			<td class="spam-master-middle-20"><?php echo esc_attr( __( 'Select Website Type:', 'spam-master' ) ); ?></td>
			<td class="spam-master-middle">
				<select class="spam-master-100" id="spam_master_subtype" name="spam_master_subtype">
					<?php
					$select_attribute = '';
					if ( 'prod' === $spam_master_subtype ) {
						$select_prod = 'selected';
					} else {
						$select_prod = '';
					}
					if ( 'deve' === $spam_master_subtype ) {
						$select_deve = 'selected';
					} else {
						$select_deve = '';
					}
					?>
					<option value="prod" <?php echo esc_attr( $select_prod ); ?>><?php echo esc_attr( __( 'Live - Production' ) ); ?></option>
					<option value="deve" <?php echo esc_attr( $select_deve ); ?>><?php echo esc_attr( __( 'Test - Development' ) ); ?></option>
				</select>
			</td>
		</tr>
		<tr class="alternate">
			<td colspan="2"><span class="spam-master-admin-blue spam-master-top-admin-shadow-offline"><span class="dashicons dashicons-info-outline"></span></span> <?php echo esc_attr( __( 'Select Spam Master type of implementation. Live Production website or Test Development website.', 'spam-master' ) ); ?></td>
		</tr>
		<tr class="alternate">
