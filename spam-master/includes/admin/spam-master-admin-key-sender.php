...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
$spam_master_db_protection_hash_dt = substr( $wpdb->get_var( "SELECT spamvalue FROM {$spam_master_keys} WHERE spamkey = 'Option' AND spamtype = 'spam_master_db_protection_hash_dt'" ), 0, 64 );
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
...
if ( empty( $spam_master_db_protection_hash_dt ) ) {
	...
	$wpdb->insert(
		...
		array(
			'time'      => $current_time,
			'spamkey'   => 'Option',
			'spamtype'  => 'spam_master_db_protection_hash_dt',
			'spamy'     => 'localhost',
			'spamvalue' => $current_time,
		)
	);
	$spam_master_db_protection_hash_dt = $current_time;
}
// Process db protection hash date time.
$plus_hash_dt = gmdate( 'Y-m-d H:i:s', strtotime( '+7 days', strtotime( $spam_master_db_protection_hash_dt ) ) );
if ( $current_time >= $plus_hash_dt ) {
	...
	// Let's update new hash.
	$data_hash  = array( 'spamvalue' => $spam_master_db_protection_hash );
	$where_hash = array(
		'spamkey'  => 'Option',
		'spamtype' => 'spam_master_db_protection_hash',
	);
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	$wpdb->update( $spam_master_keys, $data_hash, $where_hash );
	// Let's update new hash_dt.
	$data_hash_dt  = array( 'spamvalue' => $current_time );
	$where_hash_dt = array(
		'spamkey'  => 'Option',
		'spamtype' => 'spam_master_db_protection_hash_dt',
	);
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	$wpdb->update( $spam_master_keys, $data_hash_dt, $where_hash_dt );
}
// Now get the hash.
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
...
$spam_count_pre_ar = array(
	...
	'sub' => $spam_master_subtype,
	'whf' => $spam_master_white_transient,
...
if ( ( 'VALID' === $spam_master_status || 'MALFUNCTION_1' === $spam_master_status || 'MALFUNCTION_2' === $spam_master_status || 'UNSTABLE' === $spam_master_status || 'HIGH_VOLUME' === $spam_master_status || 'CODE_TAMPER_1' === $spam_master_status ) && 'prod' === $spam_master_subtype ) {
	...
	if ( is_wp_error( $response ) ) {
		...
		if ( empty( $data ) ) {
			...
			$data_spam7  = array( 'spamvalue' => $current_time );
			$where_spam7 = array(
				'spamkey'  => 'Option',
