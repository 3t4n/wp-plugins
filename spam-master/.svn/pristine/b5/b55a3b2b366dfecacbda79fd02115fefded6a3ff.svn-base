...
if ( ! class_exists( 'SpamMaster' ) ) :
	...
	define( 'SPAM_MASTER_VERSION', '7.5.7' );
...
function spam_master_activate_upgrade() {
	...
	if ( is_multisite() ) {
		...
		foreach ( $blogs as $id ) {
			...
			$spam_master_upgrade_to_7_5_3  = get_blog_option( $id, 'spam_master_upgrade_to_7_5_3' );
			$spam_master_upgrade_to_7_5_5  = get_blog_option( $id, 'spam_master_upgrade_to_7_5_5' );
		...
		$spam_master_upgrade_to_7_5_3  = get_option( 'spam_master_upgrade_to_7_5_3' );
		$spam_master_upgrade_to_7_5_5  = get_option( 'spam_master_upgrade_to_7_5_5' );
	...
	if ( '1' !== $spam_master_upgrade_to_7_5_3 && '2.4' === $spam_master_keys_db_installed_ver ) {
		require_once WP_PLUGIN_DIR . '/spam-master/includes/admin/upgrade/spam-master-admin-upgrade-to-7-5-3.php';
	}
	if ( '1' !== $spam_master_upgrade_to_7_5_5 && '2.4' === $spam_master_keys_db_installed_ver ) {
		require_once WP_PLUGIN_DIR . '/spam-master/includes/admin/upgrade/spam-master-admin-upgrade-to-7-5-5.php';
	}
...
if ( $spam_master_keys_db_installed_ver === $spam_master_keys_db_version ) {
	...
	$spamtype33e = 'spam_master_code_tamper_1_date';
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	$spam_master_code_tamper_1_date = $wpdb->get_var(
		$wpdb->prepare(
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			"SELECT spamvalue FROM $spam_master_keys WHERE spamkey=%s AND spamtype=%s",
			$spamkey,
	...
	$spamtype33f = 'spam_master_code_tamper_1_notice';
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	$spam_master_code_tamper_1_notice = $wpdb->get_var(
		$wpdb->prepare(
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			"SELECT spamvalue FROM $spam_master_keys WHERE spamkey=%s AND spamtype=%s",
			$spamkey,
	...
	function spam_master_admin_notices() {
			...
			if ( 'settings_page_spam-master' !== $spam_master_screen->id ) {
				...
				if ( 'FREE' === $spam_master_type ) {
					if ( $spam_master_current_date >= $spam_master_invitation_notice_plus_7 && '1' !== $spam_master_invitation_free_wide_notice ) {
						?>
						<div class="notice notice-success spam-master-pad-notice">
		...
		if ( 'MALFUNCTION_1' === $spam_master_status ) {
			?>
			<div class="notice notice-warning is-dismissible spam-master-pad-notice">
		...
		if ( 'MALFUNCTION_3' === $spam_master_status ) {
			?>
			<div class="notice notice-error is-dismissible spam-master-pad-notice">
		...
		if ( 'HIGH_VOLUME' === $spam_master_status ) {
			...
			<p>
				...
				<?php echo esc_attr( __( 'of spam checks detected with free keys. ', 'spam-master' ) ); ?>
				<?php echo esc_attr( __( 'You are not Protected!!! Please wait 24 hours in order for spam checks count to decrease or consider a ', 'spam-master' ) ); ?>
		...
		if ( 'INACTIVE' === $spam_master_status ) {
			?>
			<div class="notice notice-error is-dismissible spam-master-pad-notice">
			<p><strong><?php echo esc_attr( __( 'Spam Master', 'spam-master' ) ); ?> <?php echo esc_attr( __( '- INACTIVE & OFFLINE!', 'spam-master' ) ); ?></strong>. <?php echo esc_attr( __( 'Go to www.spammaster.org to retrieve your connection key or get a new FREE connection key', 'spam-master' ) ); ?>.</p>
			</div>
			<?php
	...
	if ( 'CODE_TAMPER_1' === $spam_master_status ) {
		if ( $spam_master_emails_current_date >= $spam_master_code_tamper_1_date && '1' !== $spam_master_code_tamper_1_notice ) {
			 ...
			 * @return void
			 */
			function spam_master_code_tamper_1_notify_load() {
				$spammail                     = true;
				$spam_master_email_controller = new SpamMasterEmailController();
				$is_code_tamper_1             = $spam_master_email_controller->spammastercodetamper1notify( $spammail );
			}
			add_action( 'wp_loaded', 'spam_master_code_tamper_1_notify_load' );
