<?php
/**
 * Load spam master protection firewall.
 *
 * @package Spam Master
 */
...
if ( ( 'VALID' === $spam_master_status || 'MALFUNCTION_1' === $spam_master_status || 'MALFUNCTION_2' === $spam_master_status ) && 'prod' === $spam_master_subtype ) {
			...
			if ( ! empty( $is_white ) ) {
				...
				if ( ! empty( $is_buffer ) ) {
					...
					if ( ! empty( $_POST ) && ! is_user_logged_in() ) {
						...
						if ( ! empty( $spam_elusive['spammaster_extra_field_1'] ) || ! empty( $spam_elusive['spammaster_extra_field_2'] ) ) {
							if ( ! isset( $spam_elusive['spammaster_extra_field_1'] ) || empty( $spam_elusive['spammaster_extra_field_1'] ) ) {
								$spammaster_extra_field_1 = 'empty';
							} else {
								$spammaster_extra_field_1 = wp_unslash( $spam_elusive['spammaster_extra_field_1'] );
							}
							if ( ! isset( $spam_elusive['spammaster_extra_field_2'] ) || empty( $spam_elusive['spammaster_extra_field_2'] ) ) {
								$spammaster_extra_field_2 = 'empty';
							} else {
								// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
								$spammaster_extra_field_2 = wp_unslash( $spam_elusive['spammaster_extra_field_2'] );
							}
