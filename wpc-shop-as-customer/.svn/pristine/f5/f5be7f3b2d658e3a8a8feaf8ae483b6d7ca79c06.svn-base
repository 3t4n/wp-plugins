...
! defined( 'WPCSA_VERSION' ) && define( 'WPCSA_VERSION', '1.3.0' );
...
! defined( 'WPCSA_DIR' ) && define( 'WPCSA_DIR', plugin_dir_path( __FILE__ ) );
	...
	function wpcsa_init() {
		...
		if ( ! class_exists( 'WPCleverWpcsa' ) ) {
			class WPCleverWpcsa {
				...
				function init() {
					// load text-domain
					load_plugin_textdomain( 'wpc-shop-as-customer', false, basename( WPCSA_DIR ) . '/languages/' );
				}
                            ...
                            <div class="wpcsa-bar">
								...
								<?php if ( isset( $_COOKIE['wpcsa_user'], $_COOKIE['wpcsa_key'] ) ) {
									...
									if ( is_object( $back_user ) ) {
										?>
                                        <span>
                                        <a href="#" class="wpcsa-back" data-id="<?php echo esc_attr( $back_user->user_id ?? '' ); ?>" data-key="<?php echo esc_attr( $back_key ); ?>">
				...
				function ajax_login() {
					...
					if ( $switch_user_id ) {
						...
						if ( $switch_user ) {
							...
							$user_id = get_user_meta( $current_user->ID, 'wpcsa_id', true );

							if ( empty( $user_id ) ) {
								// add private id
								$user_id = wp_generate_password();
								update_user_meta( $current_user->ID, 'wpcsa_id', $user_id );
							}
				...
				function ajax_back() {
					...
					if ( ! empty( $user_id ) && ! empty( $user_key ) ) {
						...
						if ( is_array( $find_user ) && ! empty( $find_user ) ) {
							$switch_user = reset( $find_user );
						}

						if ( $switch_user && is_a( $switch_user, 'WP_User' ) ) {
							// found a user to switch
							// remove saved data
							self::wp_logout();

							wp_clear_auth_cookie();
							wp_set_current_user( $switch_user->ID );
							wp_set_auth_cookie( $switch_user->ID, true, is_ssl() );
							do_action( 'wp_login', $switch_user->user_login, $switch_user );
							add_filter( 'wc_session_use_secure_cookie', '__return_true' );
				...
				function wp_login( $user_login, $user ) {
					...
					if ( isset( $user->data->ID ) ) {
						$id = get_user_meta( $user->data->ID, 'wpcsa_id', true );

						if ( empty( $id ) ) {
							update_user_meta( $user->data->ID, 'wpcsa_id', wp_generate_password() );
						}
					}
				}

				function wp_logout( $user_id = null ) {
					// remove private id
					if ( $user_id ) {
						delete_user_meta( $user_id, 'wpcsa_id' );
					}
