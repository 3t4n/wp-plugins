...
class Meow_SCLEGN_Rest
{
	...
	function rest_api_init() {
		try {
			...
			register_rest_route( $this->namespace, '/delete_all_drafts', array(
				'methods' => 'POST',
				'permission_callback' => array( $this->core, 'can_access_settings' ),
				'callback' => array( $this, 'rest_delete_all_drafts' )
			) );
	...
	function rest_delete_all_drafts() {
		try {
			$this->core->delete_all_drafts_posts();
		} catch (Exception $e) {
			return new WP_REST_Response([
				'success' => false,
				'message' => $e->getMessage(),
