...
class Meow_SCLEGN_Core
{
	...
	protected function create_social_post_by_post( $post, $auto_schedule_posts, $defaults, $text_link = null )
	{
		...
		$draft_enabled = $this->get_option( 'draft_enabled', false );
				...
				while ( true ) {
					if ( isset( $setting_post_times[$schedule_post_day] ) ) {
						...
						if ( !empty( $id ) ) {
							...
							if ( $auto_schedule_posts == 1 ) {
								if ( !$draft_enabled ) {
									error_log( 'No draft, no defaults, immediate schedule, posting Post ID: ' . $id );
									$this->post( $id );
								}

								if ( $draft_enabled ) {
									error_log( 'Draft enabled, no defaults, immediate schedule, posting Post ID: ' . $id );
									wp_update_post( [
										'ID'          => $id,
										'post_status' => 'draft',
									] );
	...
	function delete_all_drafts_posts(  )
	{
		$args = [
			'post_type' => $this->get_social_post_type(  ),
			'post_status' => ['draft'],
