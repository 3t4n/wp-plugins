<?php
	...
	if (!function_exists('wuoc_sanitize_data')) {
		function wuoc_sanitize_data($input) {
			if (is_array($input)) {        
				return array_map('wuoc_sanitize_data', $input);
			} 
	...
	function wuoc_woocommerce_can_reduce_order_stock_inner($order_id = 0) {
		...
		if ($order instanceof WC_Order) {
			$meta_keys = ['_wuoc_merged_orders', '_wuoc_cloned_order', 'cloned_from', 'splitted_from', 'split_status'];
	...
	add_filter('woocommerce_prevent_adjust_line_item_product_stock', function ($prevent = false) {
		if (!empty($_POST['order_id'])) {
			...
			if ($order instanceof WC_Order) {
				$wuoc_processed_order = wuoc_woocommerce_can_reduce_order_stock_inner($order->get_id());
