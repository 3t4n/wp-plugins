...
*/
	...
	if ($wuoc_woocommerce) {
		...
		global $wuoc_orderslist_page_cron, $is_wuoc_edit_order_page, $wuoc_is_wc_orders_page, $is_wuoc_settings_page, $_wuoc_order_id;
		$wuoc_is_order = false;
		$wuoc_premium_copy = 'https://shop.androidbubbles.com/product/ultimate-order-combination';
		$wuoc_data = get_plugin_data(__FILE__, true, false);
		...
		$wuoc_custom_orders_table_enabled = ($hpos_enabled || (function_exists('wc_get_container') &&
		...
		$typenow = (!empty($_GET['post_type']) && in_array($_GET['post_type'], ['shop_order'])) 
					|| (!empty($_GET['page']) && in_array($_GET['page'], ['wc-orders'])) 
					? 'shop_order' 
					: '';
		...
		$wuoc_is_wc_orders_page = (isset($_GET['page']) && $_GET['page'] === 'wc-orders' || (isset($_GET['post_type']) && $_GET['post_type'] === 'shop_order'));

		$is_wuoc_edit_order_page = (
										(isset($_GET['post']) && is_numeric($_GET['post']) && get_post_type($_GET['post']) === 'shop_order') || 
										(isset($_GET['page'], $_GET['action'], $_GET['id']) && $_GET['page'] === 'wc-orders' && $_GET['action'] === 'edit' && is_numeric($_GET['id']))
									);
		...
		if ($is_wuoc_edit_order_page) {
			$_wuoc_order_id = isset($_GET['post']) ? intval($_GET['post']) : (isset($_GET['id']) ? intval($_GET['id']) : 0);
			
	
			$wuoc_is_order = (get_post_type($_wuoc_order_id)=='shop_order');
			// First, try the standard method
			if (!$wuoc_is_order) {
				// If HPOS is enabled, check in `wc_orders`
				$is_order = $wpdb->get_var($wpdb->prepare(
		...
		if (function_exists('wc_admin_report')) {
			add_filter('woocommerce_analytics_orders_query_args', 'wuoc_analytics_orders_query_args', 10, 1);
		} else {
