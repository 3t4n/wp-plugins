...
function ehive_object_tags_get_var($name, $default = false) {
    global $wp_query;
...
function ehive_object_tags_current_url() {
    return (isset($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] == "on" ? 'https' : 'http') . "://" . $_SERVER["SERVER_NAME"] . ":" . $_SERVER["SERVER_PORT"] . $_SERVER["REQUEST_URI"];
}
...
class EHiveObjectTags_Widget extends WP_Widget {
	...
	public function widget($args, $instance) {
	    ...
	    global $post, $eHiveAccess, $eHiveSearch;
		...
		if ( $eHiveAccess->getObjectDetailsPageId() == get_the_ID() ) {
			...
			try {		 
				...
				$object_record_id = ehive_object_tags_get_var('ehive_object_record_id', false);
				...
				if ($object_record_id) {
					...
					if ($objectRecordTagsCollection != false) {
						...
						foreach ($objectRecordTagsCollection->objectRecordTags as $objectTag) {
						    ...
						    $delete_link = ehive_object_tags_current_url();
						    $add_link = ehive_object_tags_current_url();
							
							if (isset($eHiveSearch)) {
								$searchOptions = $eHiveSearch->getSearchOptions();								
								$searchParameters = '?' . $searchOptions['query_var'] . '=' . urlencode('tag:"' . $objectTag->cleanTagName . '"');																		
								$search_link = $eHiveAccess->getSearchPageLink( $searchParameters );
							} else {
								$search_link = '#';
							}
					        ...
					        if ($instance['tag_search_link_enabled'] && isset($eHiveSearch)) {
	...
	function add_tags() {
	    ...
	    $object_record_id = '';
	    if (isset($_POST['ehive_object_record_id'])) {
		  $object_record_id = $_POST['ehive_object_record_id'];
	    }
		
	    $tags = '';
	    if (isset($_POST['ehive_add_tags'])) {
		...
		if ( $object_record_id && $tags ) {
			...
			if (strstr($tags, ',')) {
				...
				foreach ($tags as $key => $tagString) {
					$objectRecordTag = new EA\ObjectRecordTag();
	...
	function delete_tag() {
	    ...
	    $tagString = '';
	    if (isset($_POST['ehive_delete_tag'])) {
