...
class AdminPage {
	...
	public function onAdminMenu() {
		add_menu_page("Akari Worker Page", "Akari Worker", 'manage_options', 'akari-worker',
			...
			"/wp-content/plugins/akari-worker/assets/img/akari-icon.png?ver=" . AKARI_WORKER_VER);
	}
	...
	public function showAdminPage() {
		...
		<div class="akari-worker-wrapper">
			...
			<form method="post" action="<?= admin_url("admin-post.php?action=akari_worker_settings") ?>">
				...
				<p>Please remember to rescan using the CMP if changes are made to any scripts or tags.</p>

				<label>Google Analytics / Google Tag Manager ID</label>
				<input name="gtag-id" type="text" placeholder="Enter Analytics or Tag Manager ID" value="<?= get_option("akari_worker_google_analytics_id", "") ?>">
				...
				<input name="max-upload-size" type="number" placeholder="Enter max upload size" value="<?= get_option("akari_worker_max_upload_size", 20) ?>">
	...
	public static function settingsSave() {
		if (current_user_can("manage_options") && wp_verify_nonce($_POST["_wpnonce"], "akari_worker_settings")) {
			...
			$max_upload_size = isset($_POST["max-upload-size"]) ? absint($_POST["max-upload-size"]) : 20;
			...
			update_option("akari_worker_max_upload_size", $max_upload_size);
