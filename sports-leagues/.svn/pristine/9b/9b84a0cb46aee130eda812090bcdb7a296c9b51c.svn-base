...
( function( window, document, $, plugin ) {
	...
	plugin.bindEvents = function() {
		...
		$c.body.on( 'click', '[data-sl-recalculate-index-tables]', function( e ) {
			...
			jQuery.ajax( {
				...
				url: anwp.rest_root + 'sports-leagues/helper/recalculate-index-tables'
			} ).always( function() {
		...
		$c.body.on( 'click', '[data-sl-flush-plugin-cache]', function( e ) {
			...
			jQuery.ajax( {
				...
				url: anwp.rest_root + 'sports-leagues/helper/flush-plugin-cache'
			} ).always( function() {
				$this.text( $this.data( 'oldText' ) );
			} ).fail( function() {
				toastr.error( 'ERROR !!!' );
	...
	plugin.getChildStagesWithGroups = function( tournamentId, targetSelect ) {
		...
		$.ajax( {
			...
			url: anwp.rest_root + 'sports-leagues/get-tournament-groups/' + tournamentId
