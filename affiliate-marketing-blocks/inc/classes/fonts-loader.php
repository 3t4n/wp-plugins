...
if ( ! class_exists( 'Afmb_Fonts_Loader' ) ) {
	...
	class Afmb_Fonts_Loader {
		...
		public function __construct() {
			...
			add_action( 'afmb_render_block', array( $this, 'font_generator' ) );
		}
		...
		public function font_generator( $block ) {
			if ( isset( $block['attrs'] ) && is_array( $block['attrs'] ) ) {
				...
				foreach ( $attributes as $key => $value ) {
					if ( ! empty( $value ) && strpos( $key, 'afmb_' ) === 0 && strpos( $key, 'FontFamily' ) !== false ) {
				...
				if ( ! empty( $fonts ) ) {
					...
					if ( ! empty( $gfonts ) ) {
						...
						foreach ( $font_array as $font ) {
							...
							$font_handle = 'afmb-font-' . sanitize_title( $font );
...
new Afmb_Fonts_Loader();    // Initialize the class.
