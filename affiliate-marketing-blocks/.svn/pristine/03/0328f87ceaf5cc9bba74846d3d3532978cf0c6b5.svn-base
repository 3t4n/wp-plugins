<?php
/**
 * Afma Blocks - Generate and Manage Page-Specific Dynamic Styles with Minification
 *
 * @since 1.0.0
 * @package Afmb-blocks
 */
...
if ( ! class_exists( 'Afmb_Dynamic_Style' ) ) {

    /**
     * Afmb Blocks Dynamic Style Class
     *
     * @since 1.0.0
     * @package Afmb-blocks
     */
    class Afmb_Dynamic_Style {
        ...
        private function setup_directories() {
            ...
            $this->css_dir = $upload_dir['basedir'] . '/afmb-blocks/css';
            $this->css_url = $upload_dir['baseurl'] . '/afmb-blocks/css';
        ...
        private function init() {
            ...
            if( wp_is_block_theme(  ) ) {
                add_action( 'wp_enqueue_scripts', [ $this, 'generate_css_file' ] );
            } else {
                add_action( 'wp_footer', [ $this, 'generate_css_file' ] );
            }
        ...
        public function collect_dynamic_styles( $block_content, $block ) {
            if ( isset( $block['blockName'] ) && str_contains( $block['blockName'], 'afmb/' ) ) {
                do_action( 'afmb_render_block', $block );
                if ( isset( $block['attrs']['blockStyle'] ) ) {
                    $block_id = isset( $block['attrs']['blockId'] ) ? $block['attrs']['blockId'] : 'afmb-block-' . md5( serialize( $block['attrs'] ) );
        ...
        public function generate_css_file() {
            ...
            $css_filename = 'afmb-dynamic-styles-' . $page_id . '.min.css';
            ...
            $css_content = "/* afmb Blocks Dynamic Styles - Page ID: $page_id - Generated on " . current_time( 'mysql' ) . " */\n" . $this->minify_css($css_content);
        ...
        public function enqueue_dynamic_styles() {
            ...
            if ( file_exists( $css_file_path ) ) {
                wp_enqueue_style( 
                    'afmb-blocks-dynamic-styles-' . $page_id, 
                    $css_file_url, 
                    array(), 
        ...
        public static function cleanup_files() {
            $upload_dir = wp_upload_dir();
            $css_dir = $upload_dir['basedir'] . '/afmb-blocks/css';
...
new Afmb_Dynamic_Style(); // Initialize the Dynamic Style class.