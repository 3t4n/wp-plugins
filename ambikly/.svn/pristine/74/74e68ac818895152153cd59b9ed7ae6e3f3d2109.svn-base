...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var AmbiklyProduct = {
        ...
        bindSlugEvent: function () {
            ...
            var productSlugInput = document.querySelector('input[name="product_slug"]');
            if (productNameInput) {
                productNameInput.addEventListener('input', function () {
                    var productName = productNameInput.value;
                    var slug = (0,_utility_helper__WEBPACK_IMPORTED_MODULE_2__.generateProductSlug)(productName);
                    if (productSlugInput) {
                        productSlugInput.value = slug; // Set slug in product_slug input
                    }
                });
            }
        },
        initLib: function () {
            ...
            if (typeof jQuery.fn.select2 !== 'undefined') {
                var multiselectElements = document.querySelectorAll('.ambikly-multiselect');
                multiselectElements.forEach(function (element) {
                    // Ignoring type errors for Select2
                    // @ts-ignore
                    jQuery(element).select2({}); // Use jQuery here if necessary for select2
                });
            }
        }
    };
