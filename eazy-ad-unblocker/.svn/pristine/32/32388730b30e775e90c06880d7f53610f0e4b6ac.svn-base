...
class EazyAdUnblocker
{
	...
	public function __construct()
	{
		...
		//July 26 2024
		add_action( 'admin_enqueue_scripts', array($this, 'eazy_ad_unblocker_add_admin_scripts'));
	}
	
	public function eazy_ad_unblocker_add_admin_scripts()
	{
		wp_enqueue_script( "eazy_custom_admin", plugins_url("js/validator.js", __FILE__), array('jquery'), '1.1.12', true );
		
	}
	
	//Aug 18 2024
	public function eazy_ad_unblocker_is_category_matching($post_id)
	{
		$eazy_ad_unblock_option = get_option("eazy-ad-unblocker-settings");
		
		if(isset($eazy_ad_unblock_option['disabled_categories']))
		{
			...
			if(is_array($curr_cats))
			{
					
				foreach($curr_cats as $k=>$cat)
				{
					array_push($curr_cat_ids, $cat->term_id);
				}
			...
			if(is_array($eazy_ad_unblock_option['disabled_categories']))
			{
				$disable_cats = $eazy_ad_unblock_option['disabled_categories'];
				
				if(count(array_intersect($curr_cat_ids, $disable_cats)) > 0)
				{
					return true;
				}
				else{
	...
	//Aug 11 2024
	public function eazy_ad_unblocker_is_tag_matching($post_id)
	{
		$eazy_ad_unblock_option = get_option("eazy-ad-unblocker-settings");
		
		if(isset($eazy_ad_unblock_option['disabled_tags']))
		{
				
			$curr_tags = get_the_tags($post_id);
			
			//Aug 18 2024
			if(is_wp_error($curr_tags))
			{
				return false;
			}
			...
			if(is_array($curr_tags))
			{
					
				foreach($curr_tags as $k=>$tag)
				{
					array_push($curr_tag_ids, $tag->term_id);
				}
			...
			if(is_array($eazy_ad_unblock_option['disabled_tags']))
			{
				$disable_tags = $eazy_ad_unblock_option['disabled_tags'];
				
				if(count(array_intersect($curr_tag_ids, $disable_tags)) > 0)
				{
					return true;
				}
				else{
	...
	public function eazy_ad_unblocker_dialog_head_func()
	{
		...
		$eazy_ad_unblock_option = get_option("eazy-ad-unblocker-settings"); //July 26 2024
		...
		if(is_singular())
		{
			...
			//Aug 18 2024
			if($this->eazy_ad_unblocker_is_category_matching($post->ID))
			{
				return;
			}
			
			//Aug 11 2024
			if($this->eazy_ad_unblocker_is_tag_matching($post->ID))
			{
				return;
			}
		...
		wp_enqueue_style("eazy-custom-ui-css", plugins_url('css/style.css.php', __FILE__), array(), '1.2.4');
	}
	...
	public function eazy_ad_unblocker_add_jquery_ui()
	{
		...
		wp_enqueue_script( "eazy_custom", plugins_url("js/custom.js", __FILE__), array('jquery'), '1.2.4', true );
	}
	...
	public function eazy_ad_unblocker_admin_callback()
	{
		...
		add_submenu_page( "eazy-ad-unblocker", "Disable Selectively", "Disable Selectively", "administrator", "eazy-ad-unblocker-disable-selectively", array($this, 'eazy_ad_unblocker_disable_by_term_callback'));
	}
	...
	public function eazy_ad_unblocker_admin_redirects()
	{
		...
		if($pagenow == 'admin.php'){
			...
			//July 26 2024
			if(!empty($_POST["disable_eazy_ad_unblocker"]) && $_POST["disable_eazy_ad_unblocker"] == __("Disable", "eazy-ad-unblocker"))
			{
				
				wp_redirect(admin_url('/admin.php?page=eazy-ad-unblocker-disable-selectively')); //April 11 2021
				exit;
			}
	...
	//Aug 25 2024
	public function eazy_ad_unblocker_get_category_tree($cat, $level)
	{
		$eazy_unblocker_opts = get_option("eazy-ad-unblocker-settings");
		
		$args = array("hide_empty"=>false, "orderby"=>'name', "order"=>'ASC', "parent"=> $cat->term_id);
		...
		$next = get_categories($args);
		  ...
		  if( count($next) > 0 )
		  {	
			echo '<ul class="eazy_ad_unblocker_sub_category">'; //Aug 26 2024
			...
			foreach( $next as $cat )
			{
				...
				{
					$post_text = "(".($cat->count)." post)";
				}
				else{
					$post_text = "(".($cat->count)." posts)";
				}
				
				
				echo '<li class="eazy_ad_unblocker_sub_category_item">'.$padding.'<input type="checkbox" name="eazy_ad_unblocker_disable_cats[]" value="'.($cat->term_id).'" '.((isset($eazy_unblocker_opts['disabled_categories']) && in_array($cat->term_id, $eazy_unblocker_opts['disabled_categories']))?'checked="checked"':false).'" />&nbsp;'. ($cat->name)."&nbsp;&nbsp;".$post_text;
				$this->eazy_ad_unblocker_get_category_tree( $cat , $new_level);
				echo '</li>';
			}  
	...
	//July 26 2024
	public function eazy_ad_unblocker_disable_by_term_callback()
	{
		
		$eazy_ad_unblocker_cats = get_categories(array("hide_empty"=>false)); //Aug 7 2024 
		
		$eazy_ad_unblocker_tags = get_tags(); //Aug 7 2024 array("hide_empty"=>false)
		
		$eazy_unblocker_opts = get_option("eazy-ad-unblocker-settings");
		
		
		if(!empty($_POST['disable_eazy_ad_unblocker_by_category']))
		{ 
	
			if ( ! isset( $_POST['eazy_ad_unblocker_category_nonce'] ) || ! wp_verify_nonce( $_POST['eazy_ad_unblocker_category_nonce'], 'disable_category_eazy_ad_unblocker_settings' ) )
			{
				wp_die(__("Disabling categories not allowed", "eazy-ad-unblocker"));
			}
			
			$eazy_empty_cat= array();
			
			$eazy_unblocker_opts['disabled_categories'] = isset($_POST['eazy_ad_unblocker_disable_cats'])?$_POST['eazy_ad_unblocker_disable_cats']:$eazy_empty_cat;
			
			if(update_option("eazy-ad-unblocker-settings", $eazy_unblocker_opts))
			{
				$_SESSION["success"] = "<div class='updated notice is-dismissible'><p>".__("Categories saved!", "eazy-ad-unblocker")."</p></div>";
			}
			else{
				$_SESSION["failure"] = "<div class='notice notice-error is-dismissible error'><p>".__("Categories NOT saved!", "eazy-ad-unblocker")."</p></div>";
			}
		}
		
		//Aug 7 2024
		
		if(!empty($_POST['disable_eazy_ad_unblocker_by_tag']))
		{ 
	
			if ( ! isset( $_POST['eazy_ad_unblocker_tag_nonce'] ) || ! wp_verify_nonce( $_POST['eazy_ad_unblocker_tag_nonce'], 'disable_tags_eazy_ad_unblocker_settings' ) )
			{
				wp_die(__("Disabling tags not allowed", "eazy-ad-unblocker"));
			}
			
			$eazy_empty_tag= array();
			
			$eazy_unblocker_opts['disabled_tags'] = isset($_POST['eazy_ad_unblocker_disable_tags'])?$_POST['eazy_ad_unblocker_disable_tags']:$eazy_empty_tag;
			
			if(update_option("eazy-ad-unblocker-settings", $eazy_unblocker_opts))
			{
				$_SESSION["success"] = "<div class='updated notice is-dismissible'><p>".__("Tags saved!", "eazy-ad-unblocker")."</p></div>";
			}
			else{
				$_SESSION["failure"] = "<div class='notice notice-error is-dismissible error'><p>".__("Tags NOT saved!", "eazy-ad-unblocker")."</p></div>";
			}
		}
		...
		<div id="wpwrap">
		<div class="wrap">
		<h1>Disable Popup Selectively By Term</h1>
		<style>
		.eazy_ad_unblocker_category_disable, .eazy_ad_unblocker_tag_disable{ padding:10px; display: inline-block;  }
		
		.eazy-ad-unblocker-cat-tree .eazy_ad_unblocker_category_parent{ padding:5px; }
		.eazy_ad_unblocker_sub_category .eazy_ad_unblocker_sub_category_item { padding:2px; }
		</style>
		<?php if(isset($_SESSION["success"])){ echo $_SESSION["success"]; unset($_SESSION["success"]); } ?>
		<?php if(isset($_SESSION["failure"])){ echo $_SESSION["failure"]; unset($_SESSION["failure"]); } ?>
		<?php if(count($eazy_ad_unblocker_cats) > 0){ ?>
			
			<form action="<?php echo admin_url('/admin.php?page=eazy-ad-unblocker-disable-selectively');  //April 11 2021 ?>" method="POST" >
			<?php wp_nonce_field( 'disable_category_eazy_ad_unblocker_settings', 'eazy_ad_unblocker_category_nonce' ); ?>
			<label ><h3>Categories</h3></label>
			<a href='javascript:void(0);' id="eazy_ad_unblocker_disable_cats_none" />Select None</a>
			&nbsp;
			<a href='javascript:void(0);' id="eazy_ad_unblocker_disable_cats_all" />Select All</a>
			<br clear="all"/>
			<br />
			<ul class="eazy-ad-unblocker-cat-tree">
			<?php 
				foreach($eazy_ad_unblocker_cats as $k=>$category)
				{
					$level = 1;
					if($category->parent == 0)
					{
						...
						{
							$post_text = "(".($category->count)." post)";
						}
						else{
							$post_text = "(".($category->count)." posts)";
						}
						
					?>
						<li class="eazy_ad_unblocker_category_disable2 eazy_ad_unblocker_category_parent">
						<input type="checkbox" name="eazy_ad_unblocker_disable_cats[]" value="<?php echo $category->term_id; ?>" <?php if(isset($eazy_unblocker_opts['disabled_categories']) && in_array($category->term_id, $eazy_unblocker_opts['disabled_categories'])){?>checked="checked"<?php } ?> />
						&nbsp;<?php echo $category->name; ?>&nbsp;&nbsp;<?php echo $post_text; ?>
						<?php $this->eazy_ad_unblocker_get_category_tree($category, $level); ?>
						</li>
					<?php	
					}
				}
			...
			<br />
			<br clear="all"/>
			<input type="submit" name="disable_eazy_ad_unblocker_by_category" value="<?php echo __("Disable Category", "eazy-ad-unblocker"); ?>" class="btn-primary" />
			</form>
			<!--Aug 25 2024-->
			<?php } ?>
			<!--Aug 7 2024-->
			<br clear="all" />
			<br clear="all" />
			<?php if(count($eazy_ad_unblocker_tags) > 0){ ?>
			<form action="<?php echo admin_url('/admin.php?page=eazy-ad-unblocker-disable-selectively');  //April 11 2021 ?>" method="POST" >
			<?php wp_nonce_field( 'disable_tags_eazy_ad_unblocker_settings', 'eazy_ad_unblocker_tag_nonce' ); ?>
			<label ><h3>Tags</h3></label>
			<a href='javascript:void(0);' id="eazy_ad_unblocker_disable_tags_none" />Select None</a>
			&nbsp;
			<a href='javascript:void(0);' id="eazy_ad_unblocker_disable_tags_all" />Select All</a>
			<br clear="all"/>
			<br />
			<?php 
				foreach($eazy_ad_unblocker_tags as $k=>$tag)
				{
					
					?>
					<div class="eazy_ad_unblocker_tag_disable">
					<input type="checkbox" name="eazy_ad_unblocker_disable_tags[]" value="<?php echo $tag->term_id; ?>" <?php if(isset($eazy_unblocker_opts['disabled_tags']) && in_array($tag->term_id, $eazy_unblocker_opts['disabled_tags'])){?>checked="checked"<?php } ?> />
					&nbsp;<?php echo $tag->name; ?>
					</div>
					<?php
				}
			...
			<br clear="all"/>
			<br />
			<input type="submit" name="disable_eazy_ad_unblocker_by_tag" value="<?php echo __("Disable Tag", "eazy-ad-unblocker"); ?>" class="btn-primary" />
			</form>
			<?php } ?>
		</div>
		</div>
