...
<tr valign="top" id="service_options" class="rates_tab_field new_api_rates" >
	...
	<strong><?php esc_html_e( 'Services', 'wf-usps-woocommerce-shipping' ); ?></strong><br/>
		<table class="usps_services widefat" style="width:100%">
			<thead>
				<th class="sort">&nbsp;</th>
				<th><?php esc_attr_e( 'Name', 'wf-usps-woocommerce-shipping' ); ?></th>
			...
			<tbody>
				...
				if ( empty( $this->settings['auth_services'] ) ) {
					$this->settings['auth_services'] = array();
				}
				foreach ( $this->auth_services as $code => $values ) {
					if ( isset( $this->settings['auth_services'][ $code ]['order'] ) ) {
						$sort = $this->settings['auth_services'][ $code ]['order'];
					}
				...
				foreach ( $this->ordered_services as $value ) {
					$code   = $value[0];
					$values = $value[1];
					if ( ! isset( $this->settings['auth_services'][ $code ] ) ) {
						$this->settings['auth_services'][ $code ] = array();
					}
					?>
						<tr>
							<td class="sort">
								<input type="hidden" class="order" name="usps_auth_service[<?php esc_attr_e( $code ); ?>][order]" value="<?php esc_attr_e( isset( $this->settings['auth_services']['services'][ $code ]['order'] ) ? $this->settings['auth_services'][ $code ]['order'] : '' ); ?>" />
							</td>
							<td>
								<input type="text" name="usps_auth_service[<?php echo esc_attr_e( $code ); ?>][name]" placeholder="<?php echo esc_attr_e( $values['name'] ); ?> (<?php echo esc_attr_e( 'USPS' ); ?>)" value="<?php echo isset( $this->settings['auth_services'][ $code ]['name'] ) ? esc_attr_e( $this->settings['auth_services'][ $code ]['name'] ) : ''; ?>" size="35" />
							</td>
							<td>
								...
								<?php foreach ( $values['services'] as $key => $name ) : ?>
									<li style="line-height: 23px;">
										<label>
											<input type="checkbox" name="usps_auth_service[<?php esc_attr_e( $code ); ?>][<?php esc_attr_e( $key ); ?>][enabled]" <?php checked( ( ! isset( $this->settings['auth_services'][ $code ][ $key ]['enabled'] ) || ! empty( $this->settings['auth_services'][ $code ][ $key ]['enabled'] ) ), true ); ?> />
											<?php esc_attr_e( $name ); ?>
										</label>
									</li>
