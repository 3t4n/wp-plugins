...
<tr valign="top" id="service_options" class="rates_tab_field old_api_rates" >
		...
		<table class="usps_services widefat" style="width:100%">
			...
			<tbody>
				...
				foreach ( $this->services as $code => $values ) {
					...
					if ( isset( $this->settings['services'][ $code ]['order'] ) ) {
						$sort = $this->settings['services'][ $code ]['order'];
					}
				...
				foreach ( $this->ordered_services as $value ) {
					$code   = $value[0];
					$values = $value[1];
					if ( ! isset( $this->settings['services'][ $code ] ) ) {
						$this->settings['services'][ $code ] = array();
					}
					?>
						<tr>
							<td class="sort">
								<input type="hidden" class="order" name="usps_service[<?php esc_attr_e( $code ); ?>][order]" value="<?php esc_attr_e( isset( $this->settings['services']['services'][ $code ]['order'] ) ? $this->settings['services'][ $code ]['order'] : '' ); ?>" />
							</td>
							<td>
								<input type="text" name="usps_service[<?php echo esc_attr_e( $code ); ?>][name]" placeholder="<?php echo esc_attr_e( $values['name'] ); ?> (<?php echo esc_attr_e( 'USPS' ); ?>)" value="<?php echo isset( $this->settings['services'][ $code ]['name'] ) ? esc_attr_e( $this->settings['services'][ $code ]['name'] ) : ''; ?>" size="35" />
							</td>
							<td>
								...
								<?php foreach ( $values['services'] as $key => $name ) : ?>
									<li style="line-height: 23px;">
										<label>
											<input type="checkbox" name="usps_service[<?php esc_attr_e( $code ); ?>][<?php esc_attr_e( $key ); ?>][enabled]" <?php checked( ( ! isset( $this->settings['services'][ $code ][ $key ]['enabled'] ) || ! empty( $this->settings['services'][ $code ][ $key ]['enabled'] ) ), true ); ?> />
											<?php esc_attr_e( $name ); ?>
										</label>
									</li>
