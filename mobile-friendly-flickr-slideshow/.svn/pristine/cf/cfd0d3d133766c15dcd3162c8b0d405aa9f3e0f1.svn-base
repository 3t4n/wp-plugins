...
class FlickrSlideshow {
    ...
    public function fshow( $atts ) {
        if (strlen(get_option('fshow_flickr_api_key')) == 0) {
            return sprintf(__('You must obtain a <a href="%s" target="_blank">Flickr API Key</a> to use the new version of Flickr Slideshow correctly.','flickr_slideshow'),'https://www.flickr.com/services/apps/create/apply');
        }
        ...
        $return = sprintf('<div style="max-width: %spx; height: %spx" class="fshow-wrapper">',intval($atts['width']),intval($atts['height']))."\n";
