...
class WH_Helper
{
  ...
  function afterShipmentUpdate($meta_id, $object_id, $meta_key, $meta_value) {
    if ( $meta_key === self::METAKEY_SHIPMENT_TRACKING ) {
      ...
      if (empty($currentTracking)) {
        ...
        $deletedTracking = reset($this->_arrayDiffRecursive($this->_tracking, $currentTracking));
        ...
        $newTracking = reset($this->_arrayDiffRecursive($currentTracking, $this->_tracking));
        ...
        foreach (array_keys($this->_tracking) as $key) {
          if ($this->_arrayDiffRecursive($currentTracking[$key], $this->_tracking[$key])) {
  ...
  function insertChild($id, $variant = null) {
    if ($variant == null) {
      $variant = new WC_Product_Variation( $id );
    }

    do_action('a2c_wh_variant_created_action', ['store_id' => $this->_blogId, 'id' => $id, 'parent_id' => $variant->get_data()['parent_id']]);
  }

  function updateChild($id, $variant = null) {
    if ($variant == null) {
      $variant = new WC_Product_Variation( $id );
    }

    do_action('a2c_wh_variant_updated_action', ['store_id' => $this->_blogId, 'id' => $id, 'parent_id' => $variant->get_data()['parent_id']]);
  }
    ...
    foreach ($array1 as $key => $value) {
      if (array_key_exists($key, $array2)) {
        if (is_array($value) && is_array($array2[$key])) {
          $nestedDiff = $this->_arrayDiffRecursive($value, $array2[$key]);
          if (!empty($nestedDiff)) {
            $difference[$key] = $nestedDiff;
          }
        } elseif (is_object($value) && is_object($array2[$key])) {
          $nestedDiff = $this->_arrayDiffRecursive((array)$value, (array)$array2[$key]);
          if (!empty($nestedDiff)) {
            $diff[$key] = $nestedDiff;
          }
