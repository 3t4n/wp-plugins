...
load_plugin_textdomain('m1downloadlist', false, basename( dirname( __FILE__ ) ) . '/languages' );
$m1dll_index = 0;
...
function m1dll_shortcode( $atts ) {
	...
	if(!array_key_exists('m1dll_index_get', $_REQUEST)) $request['m1dll_index_get'] = ""; else $request['m1dll_index_get'] = sanitize_text_field(wp_unslash($_REQUEST['m1dll_index_get']));
    ...
    if ($request['d'] != ""  && $request['m1dll_index_get'] == $m1dll_index) {
        ...
        //path from URL
        $subdir = str_replace("..","",sanitize_text_field(wp_unslash(base64_decode(rawurldecode(wp_strip_all_tags($request['d']))))));
        $dirname.= $subdir;
    ...
    if (is_dir($dirname)) {
    	...
    	if ($atts['nobreadcrumb'] == '1') {
	        ...
	        if (strlen($atts['label']) > 0) {
	            $subdir_path = '<strong>'.__('path', 'm1downloadlist').':</strong> <a href="' . get_permalink(get_the_ID()) . '">'.$atts['label'].'</a>';
	        } else {
	            $subdir_path = '<strong>'.__('path', 'm1downloadlist').':</strong> <a href="' . get_permalink(get_the_ID()) . '">'.__('downloads', 'm1downloadlist').'</a>';
	        }
	        ...
	        $content.= '
	        <p class="m1dll_subdirpath">'.sanitize_text_field($subdir_path).'</p>
	        ';
				...
				if ($file != "." && $file != ".." && in_array($file, explode(',', $atts['hidefilename'])) === false)
				{
					...
					$fpath = $dirname ."/". $file;
						...
						if (is_dir($dirname ."/". $file) && $atts['hidedirs'] != "1")
						{
							...
							$str_filetime = strtotime(get_date_from_gmt(gmdate('Y-m-d H:i:s', $file_timestamp), 'Y-m-d H:i:s'));
						...
						else
						{
							...
							$href = wp_nonce_url($href, 'm1dll_index_get');
			...
			foreach ($ar_content as $f) {
				$content.= '
							<li>
											...
											<a href="'.$f['href'].'" class="test" style="background: url(\''. $f['bg-url'] .'\') left no-repeat; padding-left:20px;"'. esc_attr($f['target']) .'>
 ...
 */
function m1dll_css() {
	wp_enqueue_style( 'm1downloadlist', plugins_url('main.css', __FILE__));
} // m1dll_css()
