	...
	class MintHandler {
		...
		public function mint_nft() {
			...
			if ( isset( $_POST['mo_nft_verify_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mo_nft_verify_nonce'] ) ), 'mo_nft_wp_nonce' ) ) {
				...
				$media_cid = $this->monft_ipfs_media_upload( $attachment_url );
				$this->monft_ipfs_metadata_upload_marketplace( $data, $media_cid, $colleciton_name, $post_id );
			}
		}
		...
		public function monft_ipfs_media_upload( $media ) {
			...
			// Get the file body content.
			$file_content = wp_remote_retrieve_body( $file );

			// Prepare the file for multipart/form-data submission.
			...
			$response = wp_remote_post(
				\MoNft\Constants::IPFS_BASE_URL . 'uploadFileToIpfs',
				$args
			);
			...
			if ( ! isset( $media_body[0]['hash'] ) ) {
				wp_send_json_error( 'ipfs.hascoin.io API call malfunction (media)!' );
				exit();
			}

			$media_cid = $media_body[0]['hash'];
		 ...
		 * @param string          $media_cid Content of the nft to be uploaded to ipfs.
		 * @param colleciton_name $colleciton_name Collection name.
		 * @param post_id         $post_id Order id of current product.
		 */
		public function monft_ipfs_metadata_upload_marketplace( $data, $media_cid, $colleciton_name, $post_id ) {
			...
			$response = wp_remote_post( \MoNft\Constants::IPFS_BASE_URL . 'uploadJsonToIpfs', $args );
