	...
	class Marketplace {
		...
		public function view() {
			...
			if ( true === $mo_nft_util->is_developer_mode ) {
				...
				wp_enqueue_script( 'monft_marketplace_ui_filter', MONFT_URL . 'classes/free/Resources/js/web3/dev/filterNft.js', array(), $ver = \MoNft\Constants::MONFT_VER_CURR, $in_footer = true );
				...
				wp_enqueue_script( 'monft_marketplace_ui_filter', MONFT_URL . 'classes/free/Resources/js/web3/prod/filterNft.min.js', array(), $ver = \MoNft\Constants::MONFT_VER_CURR, $in_footer = true );
			...
			if ( ! empty( $mo_nft_collection ) ) {
				...
				$content .= '<svg class="monft-rotate-icon-90-deg" name="monft_filter_attributes" id="monft_filter_attributes" width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">';
				...
				foreach ( $mo_nft_collection as $key => $value ) {
					...
					} elseif ( 'Polygon' === $mo_nft_collection[ $key ]['blockchain'] || 'amoy' === $mo_nft_collection[ $key ]['blockchain'] ) {
						$chain = \MoNft\Constants::ALCHEMY_AMOY;
					} else {
					...
					foreach ( $metadata_body as $metadata_key => $value ) {
						if ( 'nfts' === $metadata_key ) {
							...
							foreach ( $value as $field_key => $fieldvalue ) {
								...
								$ipfs_url              = $mo_nft_util->resolve_ipfs_url( $cid );
								...
								$image_cid           = $mo_nft_util->resolve_ipfs_url( $metadata_body_array['image'] );
								...
								$content  .= '<img src="' . esc_attr( $image_cid ) . '" class="monft_image" name="monft_image" id="monft_image_' . esc_attr( $metadata_body['nfts'][ $field_key ]['tokenId'] ) . '"/>';
								$content  .= '</div>';
								$content  .= '<div class="monft-grid-info-panel" id="monft-grid-info-panel-' . esc_attr( $metadata_body['nfts'][ $field_key ]['tokenId'] ) . '">';
								$content  .= '<p  class="owner_of_nft" id="monft-owner-of-nft-' . esc_attr( $metadata_body['nfts'][ $field_key ]['tokenId'] ) . '" value=""></p>';
								$content  .= '<h3 class="monft-item-name">' . esc_attr( $metadata_body_array['name'] ) . ' #' . esc_attr( $metadata_body['nfts'][ $field_key ]['tokenId'] ) . '</h3>';
								$content  .= '<p name="monft-buy-text-' . esc_attr( $metadata_body['nfts'][ $field_key ]['tokenId'] ) . '" class="monft-buy-text"></p>';
								...
								if ( isset( $is_listed['0'] ) && 'yes' === $is_listed[0] ) {
									...
									$content .= '<div id="monft-buy-field-' . esc_attr( $metadata_body['nfts'][ $field_key ]['tokenId'] ) . '" class="monft-buy-field">';
									$content .= '<svg name="monft-cart-svg-' . esc_attr( $metadata_body['nfts'][ $field_key ]['tokenId'] ) . '"width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="padding-right:30px;">';
									...
									$content .= '</svg>';
									$content .= '<div class="monft_buy_now_filter">Buy now</div>';
									$content .= '</div>';
								...
								$content .= '<div id="monft_metadata_' . esc_attr( $metadata_body['nfts'][ $field_key ]['tokenId'] ) . '" class="monft_metadata" hidden>' . esc_attr( $metadatastringformat );
								if ( isset( $is_listed['0'] ) && 'yes' === $is_listed[0] ) {
									$content .= 'buynow';
								}
								$content .= '</div>';
