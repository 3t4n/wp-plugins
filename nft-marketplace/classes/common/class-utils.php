...
if ( ! class_exists( 'MoNft\Utils' ) ) {
	...
	class Utils {
		...
		public function get_total_plugin_activation_time() {
			...
			$activation_time = $mo_nft_util->get_option( 'mo_nft_plugin_activation_time' );
		 ...
		 * @return string
		 */
		public function resolve_ipfs_url( $url ) {
			$parsed_url = wp_parse_url( $url );

			// If the input is an HTTPS URL, return it as is.
			if ( isset( $parsed_url['scheme'] ) && 'https' === $parsed_url['scheme'] ) {
				return $url;
			}

			if ( isset( $parsed_url['scheme'] ) && 'ipfs' === $parsed_url['scheme'] ) {
				$cid = ltrim( $parsed_url['host'], '/ipfs/' );
			} else {
				$cid = $url;
			}

			// Check if the CID is v0 or v1.
			if ( preg_match( '/^Qm[1-9A-HJ-NP-Za-km-z]{44}$/', $cid ) ) {
				return \MoNft\Constants::IPFS_HASCOIN_ENDPOINT . $cid;
			} elseif ( preg_match( '/^[bB][a-zA-Z0-9]{58}$/', $cid ) ) {
				return \MoNft\Constants::IPFS_ENDPOINT . $cid;
			} else {
				return 'Invalid CID or URL';
			}
		}
		/**
		 * Deactivation hook.
		...
		public function deactivate_plugin() {
			...
			$this->delete_option( 'mo_nft_plugin_activation_time' );
