...
if (!function_exists('gourl_spelling_load') && !function_exists('gourl_spelling_load_textdomain')) // Exit if duplicate
{

	DEFINE('GOURLSPL', "gourl-spelling");
	
	// load spelling
	add_action( 'plugins_loaded',  'gourl_spelling_load_textdomain' );
	// load localisation
	add_action( 'plugins_loaded',  'gourl_spelling_load', 20 );
	
	
	function gourl_spelling_load_textdomain()
	{
		load_plugin_textdomain( GOURLSPL, false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	}
	
	
	function gourl_spelling_load()
	{
		...
		class GoUrl_Spelling
		{
			...
			*/
			public function __construct()
			{
				// localization
				$this->fields = array( 
						"form_line1" 		=> __( "Spelling or Grammar Error", GOURLSPL ),
						"form_line2"		=> __( "Webpage", GOURLSPL ),
						"form_line3"		=> __( "Describe the error and offer a solution", GOURLSPL ),
						"form_width"		=> 	"470px", 
				
						"form_button1"		=> __( "Send to Author", GOURLSPL ),
						"form_button2"		=> __( "Cancel", GOURLSPL ),
						"form_button3"		=> __( "Close Window", GOURLSPL ),
				
						"plugin_name"		=> __( "Webmaster Spelling Notifications", GOURLSPL ),
				
						"form_message1" 	=> __( "Please select no more than 400 characters!", GOURLSPL ),
						"form_message2"		=> __( "Please select the spelling error!", GOURLSPL ),
						"form_message3"		=> __( "Thank you! Your message has been successfully sent, we highly appreciate your support!", GOURLSPL ),
						"form_message4"		=> __( "Error! Message not sent, Please try again!", GOURLSPL ),
				
						"email_subject"		=> sprintf(__( "Spelling Error on %s", GOURLSPL ), ucfirst($_SERVER["SERVER_NAME"])),
						"email_line1"		=> __( "Webpage", GOURLSPL ),
						"email_line2"		=> __( "Error", GOURLSPL ),
						"email_line3"		=> __( "Comments", GOURLSPL ),
						"email_line4"		=> __( "User", GOURLSPL ),
						"email_line5"		=> __( "IP", GOURLSPL ),
						"email_line6"		=> __( "User Agent", GOURLSPL )
				);
			...
			*/
			public function settings_page()
			{
				...
				$tmp .= "<div style='float:right; margin-top:-20px'><a href='https://gourl.io/' target='_blank'><img title='".__('Free Bitcoin Payment Gateway for Your Website', GOURLSPL)."' src='".plugins_url('/images/gourl.png', __FILE__)."' border='0'></a></div>";
				$tmp .= "</h2>";
				
				if ($this->save_flag) $tmp .= "<br><div class='updated'><p>".(isset($_POST["restore_default"])?__('Settings has been restored <strong>successfully</strong>', GOURLSPL):__('Settings has been saved <strong>successfully</strong>', GOURLSPL))."</p></div><br>";
				...
				$tmp .= "<div style='margin:10px 20px 20px 20px'>";
				$tmp .= "<a href='".plugins_url('/images/img2.png', __FILE__)."'><img title='".__('Spelling Notifications', GOURLSPL)."' src='".plugins_url('/images/img1.png', __FILE__)."' border='0'></a>";
				$tmp .= "</div>";
				$tmp .= __('<b>Ready to use:</b> &#160; Select any text on webpage and press CTRL+ENTER on your keyboard :)', GOURLSPL) . "<br><br>";
				$tmp .= "<a target='_blank' href='https://gourl.io/php-spelling-notifications.html'>".__( 'Plugin Homepage', GOURLSPL )."</a> &#160;&amp;&#160; <a target='_blank' href='https://gourl.io/php-spelling-notifications.html#screenshot'>".__( 'screenshots', GOURLSPL )." &#187;</a><br>";
				$tmp .= "<a target='_blank' href='https://github.com/cryptoapi/Wordpress-Spelling-Notifications'>".__( 'Plugin on Github - 100% Free Open Source', GOURLSPL )." &#187;</a><br><br>";
				...
				$tmp .= __('Visitors should select text with a mouse, press Ctrl+Enter, enter comments and the webmaster will be notified about any such errors.', GOURLSPL).'<br><br>';
				$tmp .= sprintf(__( "If you want to use Webmaster Spelling Notifications plugin in a language other than English, see the page <a href='%s'>Languages and Translations</a>", GOURLSPL ), "https://gourl.io/languages.html");
				$tmp .= "<br><br><br><br>";
				$tmp .= "</th>";
				$tmp .= "</tr>";
				...
				$tmp .= "<p class='description'>".__('Please enter email address', GOURLSPL )."</p>";
				$tmp .= "</tr>";
				...
				$tmp .= "<tr valign='top'>";
				$tmp .= "<th colspan='2' style='padding-left:20px'><br><br><h3 class='title'>".__('Banner', GOURLSPL )."</h3>";
				$tmp .= "<table><tr>";
			...
			*/
			...
			{
				return sprintf( __( "If you like <b>Webmaster Spelling Notifications</b> please leave us a %s rating on %s. A huge thank you from GoUrl in advance!", GOURLSPL ), "<a href='https://wordpress.org/support/view/plugin-reviews/gourl-spelling-notifications?filter=5#postform' target='_blank'>&#9733;&#9733;&#9733;&#9733;&#9733;</a>", "<a href='https://wordpress.org/support/view/plugin-reviews/gourl-spelling-notifications?filter=5#postform' target='_blank'>WordPress.org</a>");
			}
