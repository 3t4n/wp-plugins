...
class Virtuaria_Correios_API {
	 ...
	 * @return mixed|false
	 */
	public function get_shipping_deadline( array $data ) {
		...
		$response = wp_remote_post(
			! isset( $data['easy_mode'] )
				? $this->endpoint . 'prazo/v1/nacional'
				: $this->basic_endpoint . 'deadline',
			array(
				'headers' => array(
	...
	public function get_shipping_cost( array $data ) {
		...
		$response = wp_remote_post(
			! isset( $data['easy_mode'] )
				? $this->endpoint . 'preco/v1/nacional/'
				: $this->basic_endpoint . 'cotation',
			array(
				'headers' => array(
					'Authorization' => 'Bearer ' . ( isset( $token )
						? $token
