	...
	class BloyalSnippetsController {
			...
			try {
				...
				if ( $post_url ) {
					...
					if ( is_wp_error( $result ) ) {
						...
						return $error;
					} elseif ( $api_type == 'websnippethtml' ) {
		...
		public function bloyal_fetch_snippet_html( $atts, $snipppet_code, $device_code, $snippet_type, $club_code ) {
				try {
					...
					$used_login             = (is_user_logged_in() == 1) ? 'true' : 'false';
					...
					if ( ! empty( $query_string_variables ) ) {
						...
						$customer_session_key = sanitize_text_field( wp_unslash( $_GET['bL_sk'] ) );
					} else {
					...
					$bloyal_snippet_args['OnSnippetComplete']     = 'WebSnippetCompleteFn'; // we added this code to add egift prodcut through egift web snippets
