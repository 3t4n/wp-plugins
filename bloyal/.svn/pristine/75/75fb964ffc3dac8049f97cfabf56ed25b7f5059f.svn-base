				...
				if ( isset( $bloyal_cart_object->data->Cart->Lines ) ) {
					...
					foreach ( $cart_object->cart_contents as $key => $value ) {
						...
						if ( isset( $line_items[ $itemSKu ] ) ) {
							...
							$price       = $line_items[ $itemSKu ]['price'];
							if ( $externalsalePrice != '' ) {
								$total = $sale_price - ( ( $product_dis + $order_dis ) / $qty );
							} else {
								// made condition to fix Uncaught DivisionByZeroError
								if ( $qty != 0 ) {
									$total = $price - ( ( $product_dis + $order_dis ) / $qty );
								}
							}
						}
		...
		public function bloyal_commit_cart( $order_id, $bloyal_obj ) {
			try {
				...
				if ( $cart_uid ) {
					...
					$commit_cart_request = wp_json_encode( $cart_request );
					...
					$commit_cart_response = wp_json_encode( $result );
					...
					} elseif ( isset( $bloyal_shipping_services ) && isset( $bloyal_shipping_services['isGuest'] ) && ( $bloyal_shipping_services['isGuest'] == 0 ) ) {
				...
				WC()->session->set( 'gift_item_sku', null );
				WC()->session->set( 'gift_item_price', null );
