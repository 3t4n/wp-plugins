...
if ( ! defined( 'BLOYAL_DIR' ) ) {
	define( 'BLOYAL_DIR', __DIR__ );
}
if ( ! defined( 'BLOYAL_SNIPPETS_DIR' ) ) {
	define( 'BLOYAL_SNIPPETS_DIR', __DIR__ );
}
...
function bloyal_after_checkout_validation( $data, $errors ) {
	try {
		...
		// Start
		// validate store pickup shipping method. if store is not available
		if ( ( is_checkout() ) ) {
			$chosen_shipping_methods = WC()->session->get( 'chosen_shipping_methods' );
			$shipping_method         = sanitize_key( ( isset( $chosen_shipping_methods[0] ) ? $chosen_shipping_methods[0] : '' ) );
			if ( strpos( $shipping_method, 'bloyal_pickup_store' ) !== false ) {
				$pickup_store_code         = WC()->session->get( 'session_store_code' );
				$store_pickup_options_data = bloyal_store_pickup_locations_data();
				if ( empty( $store_pickup_options_data ) ) {
					$bloyal_click_and_collect_error = get_option( 'click_collect_error' );
					$msg                            = $bloyal_click_and_collect_error . ' Please choose another shipping option';
					$errors->add( 'custom_error', __( $msg ) );
					return $errors;
				} elseif ( ( $pickup_store_code == 'no' ) || empty( $pickup_store_code ) ) {
					$msg = 'Select location for pickup from Pickup At store. ' . $pickup_store_code;
					$errors->add( 'custom_error', __( $msg ) );
					return $errors;
				}
			}
	...
	try {
		...
		if ( ! empty( $bloyal_approve_cart['alert'] ) ) {
			foreach ( $bloyal_approve_cart['alert'] as $key => $value ) {
				...
				} elseif ( ! empty( $bloyal_approve_cart['alert'] ) ) {
...
function bloyal_override_product_tax_class( $tax_class, $product ) {
	try {
		$tax_class = WC()->session->get( 'applied_line_taxes' );
		if ( ! empty( $tax_class ) ) {
			// $tax_class_data = $tax_class[ $product->get_id() ];
			$tax_class_data = isset( $tax_class[ $product->get_id() ] ) ? $tax_class[ $product->get_id() ] : '';
		} else {
			$tax_class_data = '';
		}
...
function bloyal_calculate_bloyal_promotions( $cart_object, $order_id = 0 ) {
	try {
		...
		if ( ! empty( $bLoyalCouponSession ) ) {
			foreach ( $bLoyalCouponSession as $bloyalCouponCode ) {
				$couponCodesData[] = $bloyalCouponCode['coupon_code'];
			}
		}
		$bLoyalSessionCartuid = WC()->session->get( 'bloyal_uid' );
		// $bloyalCart["CouponCodes"]          = [$bLoyalCouponSession[0]['coupon_code']];
		$bloyalCart['CouponCodes'] = ( ! empty( $bLoyalCouponSession ) ? $couponCodesData : null ); // chetu code 26-03-2024
		...
		if ( isset( $bloyal_cart_object->data ) ) {
			...
			if ( ! empty( $bloyal_cart_object->data->LoyaltySummary->AppliedCoupons ) || ! empty( $session_bloyal_coupons ) ) {
				foreach ( $bloyal_cart_object->data->LoyaltySummary->AppliedCoupons as $key => $value ) {
					...
					} elseif ( ! empty( $session_bloyal_coupons ) ) {
						foreach ( $session_bloyal_coupons as $session_bloyal_coupons_key => $session_bloyal_coupons_value ) {
							$session_bloyal_coupons[ $session_bloyal_coupons_key ]['flag'] = true;
							if ( ! $session_bloyal_coupons_value['flag'] ) {
			...
			bLoyal_add_to_cart_custom_line_item( $bloyal_cart_object->data->Cart->Lines, $cart_object );
		}
		if ( isset( $bloyal_cart_object->status ) && ( 'error' == $bloyal_cart_object->status ) ) {
			$msg = $bloyal_cart_object->message;
			wc_clear_notices();
...
function bloyal_include_scripts() {
	$bloyal_sk = WC()->session->get('bloyal_sk');
	$bloyal_session_key = null;
	if (!empty($bloyal_sk)) {
		$bloyal_session_key = isset( $bloyal_sk['bloyal_session_key'] ) ? sanitize_text_field( $bloyal_sk['bloyal_session_key'] ) : null;
	}
	...
	wp_enqueue_script( $plugin_slug . '-plugin-script', plugins_url( 'assets/js/shipping_details.js', __FILE__ ), array( 'jquery' ), '5.1.22', true );
...
function bloyal_cart_coupon_remove() {
	...
	if ( isset( $session_bloyal_coupons ) ) {
		if ( $session_bloyal_coupons ) {
			echo '<a href="javascript:void(0)"><span class="remove_coupons" style="cursor: pointer;">[Remove Coupons]</span></a>';
		}
	}
		echo '<br>';
	$time_post = microtime( true );
	...
	try {
		...
		if ( isset( $bloyal_session_coupons ) ) {
			if ( $bloyal_session_coupons ) {
				...
				<tr>
					<td colspan="2">
						<?php echo '<a href="javascript:void(0)"><span class="remove_coupons" style="cursor: pointer;">[Remove Coupons]</span></a>'; ?>
					</td>
				</tr>
				<?php
			}
...
function bloyal_refresh_cached_data() {
	try {
		...
		if ( 'success' == $result_tenders->status ) {
			update_option( 'bloyal_Tenders', wp_json_encode( $result_tenders ) );
		} else {
			$errorMessage = 'Could not refresh at the moment, please try again later';
			$isFailed     = true;
		}
		...
		if ( 'success' == $result_ShippingCarriers->status ) {
			update_option( 'bloyal_ShippingCarriers', wp_json_encode( $result_ShippingCarriers ) );
		} else {
			$errorMessage = 'Could not refresh at the moment, please try again later';
			$isFailed     = true;
		}
...
function bloyal_custom_shipping_checkout_fields( $fields ) {
	try {
		if ( is_checkout() || ( is_cart() ) ) {
			...
			$addrs_count                    = 0;
			$formatted_cust_bill_birth_date = ''; // define this variable to avoid the warnings (03-04-2024)
			...
			// resolved warnings for undefined varriable
			$customer_Fname        = '';
			$customer_Lname        = '';
			$customer_Address1     = '';
			$customer_Address2     = '';
			...
			if ( 'true' == $isDisplayAddressBook ) {
				...
				$saved_addresses   = wp_json_encode( $saved_addrs );
				$saved_addresses   = json_decode( $saved_addresses, true );
				$total_saved_addrs = 0; // define variable to avoid warnings
...
function bloyal_redirect_woocommerce_registered_user_to_snippet($user_id) {
	try {
		...
		if ( isset( $_REQUEST['ReturnUrl'] ) ) {
			...
			// code to add bL_sk session key after register via web snippet page 
			//bloyal_customer_session_api($user_id, $user_email);
			// $return_url = sanitize_url( wp_unslash( $_REQUEST['ReturnUrl'].'?bL_sk='.$_SESSION['bloyal_session_key'] ) ); 
			$return_url = sanitize_url( wp_unslash( $_REQUEST['ReturnUrl'] ) );
			wp_redirect( $return_url );
			exit;
		} else {
...
add_action('wp_logout', 'clear_session_storage_key');

// // Hook the function to wp_logout
function clear_session_storage_key() {
    // Since we can't use JavaScript here, we'll use PHP to delete the session storage key
    // However, PHP can't directly access the browser's session storage
    // We'll use a workaround by setting a cookie that will be read by JavaScript on the next page load
    setcookie('clear_session_storage_key', 'true', 0, '/');
}

add_filter( 'wp_login', 'bloyal_redirect_wordpress_loggedin_user_to_snippet' );
function bloyal_redirect_wordpress_loggedin_user_to_snippet($user_login) {
	try {
		...
		if ( 0 == $page_id ) {
			if ( isset( $_REQUEST['ReturnUrl'] ) ) {
				...
				// code to add bL_sk session key after login via web snippet page
				// bloyal_customer_session_api($user_id, $user_email);
				// $bloyal_sk = WC()->session->get( 'bloyal_sk' );
				// $bloyal_session_key = null;
				// if (!empty($bloyal_sk)) {
				// 	$bloyal_session_key = isset( $bloyal_sk['bloyal_session_key'] ) ? sanitize_text_field( $bloyal_sk['bloyal_session_key'] ) : null;
				// }
				// $return_url = sanitize_url( wp_unslash( $_REQUEST['ReturnUrl'].'?bL_sk='.$bloyal_session_key ) );
				$return_url = sanitize_url( wp_unslash( $_REQUEST['ReturnUrl'] ) );
...
function bloyal_redirect_wordpress_registered_user_to_snippet( $user_id ) {
	try {
		if ( empty( $_REQUEST['ReturnUrl'] ) ) {
			$current_user = wp_get_current_user();
			wp_set_current_user( $current_user->data->ID ?? null);
			wp_set_auth_cookie( $current_user->data->ID ?? null, false, is_ssl() );
		} else {
...
function bloyal_add_react_shortcodes() {
	try {
		...
		$react_shortcode  = array(); // made this code on 1-04-2024 to remove $react_shortcode warnings
		foreach ( $snippet_list as $snippet ) {
			...
			add_shortcode( $react_shortcode1, 'bloyal_react_club_shortcode_callback', false );
			add_shortcode( $react_shortcode2, 'bloyal_react_club_shortcode_callback', false );
			add_shortcode( $react_shortcode3, 'bloyal_react_club_shortcode_callback', false );
	...
	if ( $provisioningStatus == 'Pending' ) {
		...
		try {
			...
			$args                        = array(
				...
				'body'    => wp_wp_json_encode( $apiRequestData ),
				'method'  => 'POST',
				'timeout' => 45,
			);
...
function bloyal_update_bLoyal_provisioning_activated_data_wpdb( $results ) {
	try {
		...
		wp_send_json( wp_json_encode( $results ) );
...
function bloyal_remove_bloyal_access_key() {
	try {
		...
		update_option( 'bloyal_access_key', $adv_access_key );
		$result = array(
			'message' => 'Access Key succefully removed!',
			'status'  => 'success',
		);
		wp_send_json( wp_wp_json_encode( $result ) );
		wp_die();
...
function bloyal_carrier_company_custom_select_field( $method, $index ) {
	...
	try {
		...
		if ( ! empty( $chosen ) && $method->id === $chosen[ $index ] && in_array( $method->id, $targeted_methods ) ) {
			...
			// Start
			// modify code to display error message. if store is not available.
			if ( ! empty( $field_options ) ) {
				...
				$bloyal_click_and_collect_error = get_option( 'click_collect_error' );
				echo '<div class="custom-carrier" style="color:red;"> <strong>' . $bloyal_click_and_collect_error . '</strong></div>';
...
function bloyal_carrier_company_checkout_validation() {
	try {
		...
		if ( bloyal_has_carrier_field() && isset( $_POST[ $field_id ] ) && empty( $_POST[ $field_id ] ) ) {
			wc_add_notice(
				sprintf(
					__( $click_and_collect_label, 'woocommerce' ),
...
function bloyal_save_carrier_company_as_order_meta( $order ) {
	try {
		...
		if ( isset($_POST[ $field_id ]) && $_POST[ $field_id ] != null ) {
...
// bLoyal snippets log download function
add_action( 'wp_ajax_bloyal_snippets_log_download', 'bloyal_snippets_log_download' ); // Log download on bLoyal admin
add_action( 'wp_ajax_nopriv_bloyal_snippets_log_download', 'bloyal_snippets_log_download' );
function bloyal_snippets_log_download() {
	try {
		$filename = BLOYAL_UPLOAD_DIR_BASEPATH . '/bLoyal_snippets_log_file.txt';
		if ( file_exists( $filename ) ) {
			// Define header information
			header( 'Content-Description: File Transfer' );
			header( 'Content-Type: application/octet-stream' );
...
// End bLoyal snippets log download function
...
// bLoyal snippets log delete function
add_action( 'wp_ajax_bloyal_snippets_log_delete', 'bloyal_snippets_log_delete' );
add_action( 'wp_ajax_nopriv_bloyal_snippets_log_delete', 'bloyal_snippets_log_delete' );
function bloyal_snippets_log_delete() {
	try {
		$filename = BLOYAL_UPLOAD_DIR_BASEPATH . '/bLoyal_snippets_log_file.txt';
		if ( file_exists( $filename ) ) {
...
add_action( 'woocommerce_available_payment_gateways', 'rp_filter_gateways' );
	...
	// add custom line item in to the cart
function bLoyal_add_to_cart_custom_line_item( $line_data, $cart_object ) {
	try {
		foreach ( $line_data as $index => $line ) {
			
			if ( isset( $line->ProductCode ) && $line->ExternalId == null) {
				// code to update price of egiftcard web snippet as per the getCart API response start

				WC()->session->set( 'gift_item_price', $line->Price );
				WC()->session->set( 'gift_item_sku', $line->ProductCode );

				// code to update price of egiftcard web snippet as per the getCart API response end
				$bLoyalCustomProduct = get_product_by_sku( $line->ProductCode );
				if ( ! empty( $bLoyalCustomProduct ) ) {
					$product_id      = $bLoyalCustomProduct->get_id();
...
// set COD payment method by default
add_action( 'template_redirect', 'define_default_payment_gateway' );
function define_default_payment_gateway() {
	try {
		if ( is_checkout() && ! is_wc_endpoint_url() ) {
			// HERE define the default payment gateway ID
			$default_payment_id = 'payment_method_cod';
...
// we added this code to hide store pickup after ship_to_different_address selection on checkout page

function unset_bloyal_pickup_store_shipping_method( $rates ) {
	try {
		// Specify the shipping method ID you want to unset
		foreach ( $rates as $rate_id => $rate_val ) {
			if ( 'bloyal_pickup_store' === $rate_val->get_method_id() ) {
				unset( $rates[ $rate_id ] );
			}
		}
		return $rates;
...
// code to update cart page on shipping method change
// AJAX handler for updating shipping method in cart page
add_action( 'wp_ajax_update_shipping_method_cart', 'update_shipping_method_cart_callback' );
add_action( 'wp_ajax_nopriv_update_shipping_method_cart', 'update_shipping_method_cart_callback' );

function update_shipping_method_cart_callback() {
	try {
		if ( isset( $_POST['shipping_method'] ) ) {
			...
			// Calculate shipping cost and update totals
			...
			wp_send_json_success(
				array(
					'fragments' => apply_filters( 'woocommerce_update_cart_fragments', $fragments ),
				)
			);
		} else {
...
// Override mini-cart item price display
function custom_mini_cart_item_price( $product_price, $cart_item, $cart_item_key ) {
	// Check if current page is neither cart nor checkout
	if ( ! ( is_cart() || is_checkout() ) ) {
		$product_id         = $cart_item['product_id'];
		$product            = wc_get_product( $product_id );
		$product_sku        = strtolower( $product->get_sku() );
		$bloyal_cart_object = WC()->session->get( 'bloyal_cart_data' );
		if ( isset( $bloyal_cart_object->Cart->Lines ) ) {
			$custom_price = array();
			foreach ( $bloyal_cart_object->Cart->Lines as $line ) {
				$sku                  = strtolower( $line->ProductCode );
				$custom_price[ $sku ] = $line->NetPrice ?? $line->Price;
			}
			// $customProductPrice = $custom_price[ $product_sku ] ?? null;
			return wc_price( $custom_price[ $product_sku ] ) ?? $product_price;
		}
	} else {
		try {
...
add_filter( 'woocommerce_cart_item_price', 'custom_mini_cart_item_price', 20, 10 );

// web snippets for egiftcard complete developed by Chetu Developer
add_action( 'wp_ajax_nopriv_bloyal_web_snippets_complete', 'bloyal_web_snippets_complete' );
add_action( 'wp_ajax_bloyal_web_snippets_complete', 'bloyal_web_snippets_complete' );
function bloyal_web_snippets_complete() {
	try {
		
		if ( isset( $_POST['cart_uid'] ) && ! empty( $_POST['cart_uid'] ) ) {
			
			$access_key                        = get_option( 'bloyal_access_key' );
			$cartUid                           = $_POST['cart_uid'];
			$bloyal_snippet_obj                = new BloyalSnippetsController();
			$new_snippets_action_url           = 'carts?cartUid=' . $cartUid;

			$bloyal_web_snippet_getcart_object = $bloyal_snippet_obj->send_curl_request( '', $new_snippets_action_url, 'loyaltyengine', 0 );
			
			bLoyalLoggerService::write_custom_log( "\r\n ==========Response Cart API call============\r\n" . json_encode( $bloyal_web_snippet_getcart_object ) . "\r\n---------------\r\n" );
			
			// code to pass UID from getCart response to calculateCart request start
			$cart_uid = isset( $bloyal_web_snippet_getcart_object->data->Cart->Uid ) ? $bloyal_web_snippet_getcart_object->data->Cart->Uid : '';
			...
			if ( $cart_uid ) {
				...
				// code to pass UID from getCart response to calculateCart request end
				$blwebsnippetLineData = $bloyal_web_snippet_getcart_object->data->Cart->Lines;
				if(isset($blwebsnippetLineData) && !empty($blwebsnippetLineData)){
					bLoyal_add_to_cart_custom_line_item( $blwebsnippetLineData, $cart_object );
					$cart_url = wc_get_cart_url();
					$bloyal_cartUid =array('bL_cartUid' => $cart_uid, 'redirect' => $cart_url);
				}
				wp_send_json($bloyal_cartUid);
			}
		}
		wp_die();
	} catch ( Exception $ex ) {
...
add_action('wp_ajax_generate_bloyal_session_key', 'generate_bloyal_session_key');
add_action('wp_ajax_nopriv_generate_bloyal_session_key', 'generate_bloyal_session_key');
function generate_bloyal_session_key()
{
	if ( isset( $_POST['is_logged_in'] ) && ! empty( $_POST['is_logged_in'] ) ) {
		$user       = wp_get_current_user();
		$user_id    = $user->ID;
		$user_email = $user->user_email;
		$bloyal_snippets_obj = new BloyalSnippetsController();
		$user_details              = $bloyal_snippets_obj->get_new_customer_details_from_wbdb();
		$woo_session_id            = WC()->session->get_customer_id();
		$action                    = 'resolvedcustomersession?EmailAddress=' . $user_email . '&ExternalId=' . $user_id;
		$response_resolve_customer = $bloyal_snippets_obj->send_curl_request( '', $action, 'loyaltyengine', 0 );
		if ( 'success' === $response_resolve_customer->status && ( isset( $response_resolve_customer->data->SessionKey ) ) ) {
			$bloyal_sk = array( 
								'bloyal_session_key' => $response_resolve_customer->data->SessionKey, 
								'session_id'         => $woo_session_id,
							);
			...
			if ( empty( $response_resolve_customer->data->Customer->ExternalId ) ) {
				...
				$response_update_customer = $bloyal_snippets_obj->send_curl_request( $update_customer_request, $action, 'websnippet', 1 );
			}
			wp_send_json($bloyal_sk);
    		
		} elseif ( !isset( $response_resolve_customer->data->SessionKey )  ) {
				...
				wp_send_json($bloyal_sk);
				// swap code and place it inside this condition from outside on 01-04-2024
			if ( $response ) {
