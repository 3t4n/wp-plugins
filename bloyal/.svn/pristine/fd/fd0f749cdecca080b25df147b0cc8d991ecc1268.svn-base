...
//bLoayal snippets log downlaod for admin.
jQuery(function () {
	jQuery('#snipeets_download').click(function (e) {
		e.preventDefault();
		jQuery('#loading').show();
		var data = {
			action: 'bloyal_snippets_log_download',
		};
		...
		jQuery.post(ajaxurl, data, function (response) {
			...
			if (response == 0) {
				...
				download("bLoyal_snippets_log_file.txt", response);
			}
		});
	});
});
...
//bLoayal snippets log delete for admin.

jQuery(function () {
	jQuery('#snipeets_remove').click(function (e) {
		e.preventDefault();
		if (confirm('Are you sure you want to delete this log?')) {
			jQuery('#loading').show();
			var data = {
				action: 'bloyal_snippets_log_delete',
			};
