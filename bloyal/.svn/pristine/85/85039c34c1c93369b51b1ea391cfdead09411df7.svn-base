	...
	class ShippingController {
		...
		public function calculateShipping( $package ) {
			try {
				...
				$response         = wp_json_encode( $response_decode );
				bLoyalLoggerService::write_custom_log( "\n\r==========Calculate Shipping Request ============ \r\n" . wp_json_encode( $shipping_request ), 1 );
		...
		public function makeShippingRequest() {
			try {
				...
				if ( isset( $_REQUEST['wc-ajax'] ) && ( $_REQUEST['wc-ajax'] == 'update_order_review' ) ) {
					...
					bLoyalLoggerService::write_custom_log( "\n\r========== Shipping Request Data ============ \r\n" . wp_json_encode( $_REQUEST ), 1 );

					$shipping_request = array(
						'Shipments' => array(
							array(
								'Address' => array(
									'City'       => ( ( isset( $customerDetails['shipping_city'] ) ) && $customerDetails['shipping_city'] != '' ? $customerDetails['shipping_city'] : '' ),

									'State'      => ( ( ( isset( $_REQUEST['shipping_state'] ) && $_REQUEST['shipping_state'] ) != '' || ( isset( $_REQUEST['calc_shipping_state'] ) && $_REQUEST['calc_shipping_state'] != '' ) ) ? ( $_REQUEST['shipping_state'] ?? $_REQUEST['calc_shipping_state'] ) : ( ( isset( $customerDetails['shipping_state'] ) ) && $customerDetails['shipping_state'] != '' ? $customerDetails['shipping_state'] : '' ) ),

									'PostalCode' => ( ( ( isset( $_REQUEST['shipping_postcode'] ) && $_REQUEST['shipping_postcode'] ) != '' || ( isset( $_REQUEST['calc_shipping_postcode'] ) && $_REQUEST['calc_shipping_postcode'] != '' ) ) ? ( $_REQUEST['shipping_postcode'] ?? $_REQUEST['calc_shipping_postcode'] ) : ( ( isset( $customerDetails['shipping_postcode'] ) ) && $customerDetails['shipping_postcode'] != '' ? $customerDetails['shipping_postcode'] : '' ) ),

									'Country'    => ( ( ( isset( $_REQUEST['shipping_country'] ) && $_REQUEST['shipping_country'] ) != '' || ( isset( $_REQUEST['calc_shipping_country'] ) && $_REQUEST['calc_shipping_country'] != '' ) ) ? ( $_REQUEST['shipping_country'] ?? $_REQUEST['calc_shipping_country'] ) : ( ( isset( $customerDetails['shipping_country'] ) ) && $customerDetails['shipping_country'] != '' ? $customerDetails['shipping_country'] : '' ) ),
		...
		public function makeBloyalShipmentRequest( $bloyalCustomRate ) {
			try {
				...
				if ( is_integer( $bloyalCustomRate ) ) {
					...
					$arrShippingServiceData       = explode( ':', ( isset( WC()->session->get( 'chosen_shipping_methods' )[0] ) ? WC()->session->get( 'chosen_shipping_methods' )[0] : '' ) );
				...
				if ( isset( $_REQUEST['wc-ajax'] ) && ( $_REQUEST['wc-ajax'] == 'update_order_review' ) ) {
					...
					if ( ! empty( $request_post_data ) ) {
						...
						if ( isset( $params['billing_first_name'] ) ) {
							...
							if ( isset( $params['ship_to_different_address'] ) && $params['ship_to_different_address'] ) {
								...
								// add_filter( 'woocommerce_package_rates', 'unset_bloyal_pickup_store_shipping_method', 11, 5 );
					...
					if ( ! empty( $_REQUEST ) && isset( $_REQUEST['billing_first_name'] ) ) {
						...
						$billing_name       = isset( $guest_user_data['billing'] ) ? $guest_user_data['billing'] : array();
						$billing_first_name = ( ! empty( $billing_name ) ? ( $billing_name['first_name'] ? $billing_name['first_name'] : $customer['first_name'] ) : '' );
						$billing_last_name  = ( ! empty( $billing_name ) ? ( $billing_name['last_name'] ? $billing_name['last_name'] : $customer['last_name'] ) : '' );
						$billing_address1   = ( ! empty( $billing_name ) ? ( $billing_name['address_1'] ? $billing_name['address_1'] : $customer['address_1'] ) : '' );
						$billing_city       = ( ! empty( $billing_name ) ? ( $billing_name['city'] ? $billing_name['city'] : $customer['city'] ) : '' );
						$billing_state      = ( ! empty( $billing_name ) ? ( $billing_name['state'] ? $billing_name['state'] : $customer['state'] ) : '' );
						$billing_postcode   = ( ! empty( $billing_name ) ? ( $billing_name['postcode'] ? $billing_name['postcode'] : $customer['postcode'] ) : '' );
						$billing_country    = ( ! empty( $billing_name ) ? ( $billing_name['country'] ? $billing_name['country'] : $customer['country'] ) : '' );
						$shipping_details   = isset( $guest_user_data['shipping'] ) ? $guest_user_data['shipping'] : array();
						$ship_first_name    = isset( $shipping_details['first_name'] ) ? $shipping_details['first_name'] : '';
						$ship_last_name     = isset( $shipping_details['last_name'] ) ? $shipping_details['last_name'] : '';
						$ship_address_1     = isset( $shipping_details['address_1'] ) ? $shipping_details['address_1'] : '';
						$ship_address_2     = isset( $shipping_details['address_2'] ) ? $shipping_details['address_2'] : '';
						$ship_city          = isset( $shipping_details['city'] ) ? $shipping_details['city'] : '';
						$ship_state         = isset( $shipping_details['state'] ) ? $shipping_details['state'] : '';
						$ship_postcode      = isset( $shipping_details['postcode'] ) ? $shipping_details['postcode'] : '';
						$ship_country       = isset( $shipping_details['country'] ) ? $shipping_details['country'] : '';
						$ship_company       = isset( $shipping_details['company'] ) ? $shipping_details['company'] : '';
						if ( ! $ship_first_name ) {
							$ship_first_name = ( isset( $customer['shipping_first_name'] ) && $customer['shipping_first_name'] != '' ) ? $customer['shipping_first_name'] : ( isset( $billing_name['first_name'] ) ? $billing_name['first_name'] : '' );
						}
						if ( ! $ship_last_name ) {
							$ship_last_name = ( isset( $customer['shipping_last_name'] ) && $customer['shipping_last_name'] != '' ) ? $customer['shipping_last_name'] : ( isset( $billing_name['last_name'] ) ? $billing_name['last_name'] : '' );
						}
						if ( ! $ship_address_1 ) {
							$ship_address_1 = ( isset( $customer['shipping_address_1'] ) && $customer['shipping_address_1'] != '' ) ? $customer['shipping_address_1'] : ( isset( $billing_name['address_1'] ) ? $billing_name['address_1'] : '' );
						}
						if ( ! $ship_address_2 ) {
							$ship_address_2 = ( isset( $customer['shipping_address_2'] ) && $customer['shipping_address_2'] != '' ) ? $customer['shipping_address_2'] : ( isset( $billing_name['address_2'] ) ? $billing_name['address_2'] : '' );
						}
						if ( ! $ship_city ) {
							$ship_city = ( isset( $customer['shipping_city'] ) && $customer['shipping_city'] != '' ) ? $customer['shipping_city'] : ( isset( $billing_name['city'] ) ? $billing_name['city'] : '' );
						}
						if ( ! $ship_state ) {
							$ship_state = ( isset( $customer['shipping_state'] ) && $customer['shipping_state'] != '' ) ? $customer['shipping_state'] : ( isset( $billing_name['state'] ) ? $billing_name['state'] : '' );
						}
						if ( ! $ship_postcode ) {
							$ship_postcode = ( isset( $customer['shipping_postcode'] ) && $customer['shipping_postcode'] != '' ) ? $customer['shipping_postcode'] : ( isset( $billing_name['postcode'] ) ? $billing_name['postcode'] : '' );
						}
						if ( ! $ship_country ) {
							$ship_country = ( isset( $customer['shipping_country'] ) && $customer['shipping_country'] != '' ) ? $customer['shipping_country'] : ( isset( $billing_name['country'] ) ? $billing_name['country'] : '' );
						}
						if ( ! $ship_company ) {
							$ship_company = ( isset( $customer['shipping_company'] ) && $customer['shipping_company'] != '' ) ? $customer['shipping_company'] : ( isset( $billing_name['company'] ) ? $billing_name['company'] : '' );
						}
					}
				...
				$shipments[] = array(
					...
					'ServiceUid'                => $is_virtual_porduct_order ? '00000000-0000-0000-0000-000000000000' : ( isset( $wcShippingRateMetaData['meta_data']['bloyal_meta_data']['ServiceUid'] ) ? $wcShippingRateMetaData['meta_data']['bloyal_meta_data']['ServiceUid'] : null ),
					'ServiceCode'               => $is_virtual_porduct_order ? null : ( $wcShippingRateMetaData['meta_data']['bloyal_meta_data']['ServiceCode'] == 'bloyal_pickup_store' ? null : $wcShippingRateMetaData['meta_data']['bloyal_meta_data']['ServiceCode'] ),
