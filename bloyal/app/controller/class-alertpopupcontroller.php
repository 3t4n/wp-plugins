	...
	class BLOYAL_AlertPopUpController {
		...
		public function get_alert_html_content( $cart_uid, $alert_uid, $alert_category, $line_uid, $snippet_code ) {
			try {
				...
				$alert_snippet_div                            = "<div data-bloyal-snippet-code='" . $snippet_code . "' data-bloyal-login-domain='" . $loginDomain . "' data-bloyal-device-code='" . $strDeviceCode . "' data-bloyal-snippet-args='" . wp_wp_json_encode( $bloyal_snippet_args, true ) . "' id='root'></div><script> function EgiftSnippetComplete() { console.log('The Snippet is done') } </script>";
		...
		function display_alert_pop_up( $alertsData, $is_calculate_cart_call, $options_array ) {
			try {
				if ( is_page( 'cart' ) || is_page( 'checkout' ) ) {
					...
					wp_enqueue_script( 'bloyal_alerts_js', BLOYAL_URL . 'assets/js/alerts.js?version=2.9.4', array( 'jquery', 'jquery-ui-dialog' ), '2.9.1', true );
					wp_localize_script( 'bloyal_alerts_js', 'alertArray', wp_json_encode( $alertsData ) );
					wp_localize_script( 'bloyal_alerts_js', 'optionsArray', wp_json_encode( $options_array ) );
		...
		function ajax_alert_data_on_update() {
			try {
				...
				wp_send_json( wp_json_encode( $alertsData ) );
				wp_die();
			} catch ( Exception $ex ) {
				bLoyalLoggerService::write_custom_log( $ex->getMessage(), 3 );
			}
