		...
		public function make_bloyal_cart( $wc_cart_object, $cart_uid, $is_bloyal_applied_taxes, $order_id = 0 ) {
			try {
				...
				$bloyal_cart_data        = array(
					...
					'Uid'                       => $cart_uid == '' ? ( ! empty( $bLoyalSessionCartuid ) ? $bLoyalSessionCartuid['u_id'] : $cart_uid ) : $cart_uid,
					'ExternallyProcessedOrder'  => get_option( 'bloyal_use_order_engine' ) === 'true' ? false : true,
					'GuestCheckout'             => $is_guest,
					'Customer'                  => ( ! empty( $customer_data ) ? $customer_data : null ),
				...
				if ( ! empty( $_REQUEST ) ) {
					if ( isset( $_REQUEST['createaccount'] ) && '1' === $_REQUEST['createaccount'] ) {
						...
						if ( is_user_logged_in() ) {
							...
							$customer_data['EmailAddress'] = ( ( isset( $customer['email'] ) ) && $customer['email'] != '' ? $customer['email'] : '' );
						}

						// We have made some changes to store firstname, lastname, phone, and company name in connector.

						$customer_data['FirstName']              = ( ( isset( $customer['first_name'] ) ) && $customer['first_name'] != '' ) ? $customer['first_name'] : ( isset( $_REQUEST['billing_first_name'] ) ? $_REQUEST['billing_first_name'] : '' );
						$customer_data['LastName']               = ( ( isset( $customer['last_name'] ) ) && $customer['last_name'] != '' ) ? $customer['last_name'] : ( isset( $_REQUEST['billing_last_name'] ) ? $_REQUEST['billing_last_name'] : '' );
						$customer_data['CompanyName']            = ( ( isset( $customer['last_name'] ) ) && $customer['last_name'] != '' ) ? $customer['company'] : ( isset( $_REQUEST['billing_company'] ) ? $_REQUEST['billing_company'] : '' );
						$customer_data['Phone1']                 = ( ( isset( $customer['phone'] ) ) && $customer['phone'] != '' ) ? $customer['phone'] : ( isset( $_REQUEST['billing_phone'] ) ? $_REQUEST['billing_phone'] : '' );
						...
						$customer_data['Address']                = array();
						$customer_data['Address']['Address1']    = ( ( isset( $customer['address_1'] ) ) && $customer['address_1'] != '' ? $customer['address_1'] : '' );
						$customer_data['Address']['Address2']    = ( ( isset( $customer['address_2'] ) ) && $customer['address_2'] != '' ? $customer['address_2'] : '' );
						$customer_data['Address']['City']        = ( ( isset( $customer['city'] ) ) && $customer['city'] != '' ? $customer['city'] : '' );
						$customer_data['Address']['State']       = ( ( isset( $customer['state'] ) ) && $customer['state'] != '' ? $customer['state'] : '' );
						$customer_data['Address']['StateCode']   = ( ( isset( $customer['state'] ) ) && $customer['state'] != '' ? $customer['state'] : '' );
						$customer_data['Address']['PostalCode']  = ( ( isset( $customer['postcode'] ) ) && $customer['postcode'] != '' ? $customer['postcode'] : '' );
						$customer_data['Address']['CountryCode'] = ( ( isset( $customer['country'] ) ) && $customer['country'] != '' ? $customer['country'] : '' );
						$customer_data['Address']['Country']     = ( ( isset( $customer['country'] ) ) && $customer['country'] != '' ? $customer['country'] : '' );
						$request_post_data                       = isset( $_REQUEST['post_data'] ) ? $_REQUEST['post_data'] : array();
						if ( ! empty( $request_post_data ) ) {
							parse_str( $request_post_data, $params );
							$billingBirthDate = $params['billing_birth_date'];

							$billingFirstName = $params['billing_first_name']; // pass firstname in calculatecart request
							$billingLastName  = $params['billing_last_name']; // pass lastname in calculatecart request
							...
							if ( ! empty( $billingBirthDate ) ) {
								...
								$billing_birth_date         = isset(  $customer['billing_birth_date'] ) ? sanitize_text_field( wp_unslash(  $customer['billing_birth_date'] ) ) : '';
							...
							// code to add firstname and last name in calculatecart request (Start)
							if ( ! empty( $billingFirstName ) ) {
								$customer_data['FirstName'] = $billingFirstName;
							}

							if ( ! empty( $billingLastName ) ) {
								$customer_data['LastName'] = $billingLastName;
							}
							// code to add firstname and last name in calculatecart request (end)
		...
		public function create_lines_data( $lines, $is_bloyal_applied_taxes ) {
			try {
				...
				foreach ( $lines as $key => $line ) {
					...
					// code to update the egiftcard web snippet product price start
					if($product_data->get_sku() == WC()->session->get( 'gift_item_sku')){
						$regular_price = WC()->session->get( 'gift_item_price' );
