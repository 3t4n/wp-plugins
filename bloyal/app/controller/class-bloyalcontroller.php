...
if ( ! class_exists( 'BloyalController' ) ) {
	...
	class BloyalController {
			...
			try {
				...
				if ( is_array( $result ) && ! is_wp_error( $result ) ) {
					...
					if ( ! empty( $api_response->data ) ) {
						...
						$this->save_bLoyal_device_code(); // this function added to save device code.
		...
		// made this function to save device code in the database
		public function save_bLoyal_device_code() {
			try {
			...
			try {
				...
				foreach ( $order->get_items() as $item_id => $item_obj ) {
					...
					$line_items[] = array(
						...
						'SalePrice'     => floatval( $item_price ) - floatval( $sale_price_discount ), // made floatval varriable for for item_price and sale_price_discount
				...
				if ( $post_url ) {
					...
					if ( is_wp_error( $result ) ) {
						...
						if ( isset( $obj_response->data ) ) {
							...
							} elseif ( isset( $obj_response->data->LoyaltySummary->Alerts ) ) {
							...
							// Made condition to check null value of bloyalAlert varriable
							if ( ! empty( $bloyalAlerts ) ) {
								WC()->session->set( 'bloyal_alerts_data', $bloyalAlerts );
							}
						}
		...
		public function bloyal_update_products( WP_REST_Request $request ) {
			try {
				...
				} elseif ( $user_auth_query_result[0]->permissions != 'read_write' || $user_auth_query_result[0]->permissions != 'write' ) {
			...
			try {
					...
					foreach ( $bloyal_cart_data->Cart->Lines as $line ) {
						if ( ! empty( $arrLineDiscount ) ) {
							if ( ! isset( $arrDisplayedProducts[ $line->ProductName ] ) && isset( $arrLineDiscount[ $line->ProductName ] ) ) {
								...
								if ( isset( $arrLineDiscount[ $line->ProductName ]['sale_price_discount'] ) && $arrLineDiscount[ $line->ProductName ]['sale_price_discount'] > 0 ) {
				...
				if ( isset( $bloyal_cart_data->Cart->ExternallyAppliedDiscount ) && ! ( $bloyal_cart_data->Cart->ExternallyAppliedDiscount == true ) ) {
					if ( $bloyal_cart_data->Cart->Discount > 0 ) {
						...
						$temp_html           .= '<ul>';
						// we have updated this code to show order level discount summary
						if ( isset( $bloyal_cart_data->Cart->DiscountDetails ) && ! empty( $bloyal_cart_data->Cart->DiscountDetails ) ) {
							foreach ( $bloyal_cart_data->Cart->DiscountDetails as $DiscountData ) {
								$temp_html .= '<li><label style="font-size: 14px;">Product Name: ' . $DiscountData->Reason . '</label>';
								$temp_html .= '<table>';
								$temp_html .= '<tr><td width="60%" style="padding: 5px; font-size: 13px;">' . $DiscountData->ReasonCode . ' ( ' . $DiscountData->RuleCode . ' ) ' . '</td><td width="60%" style="padding: 5px; font-size: 13px;">-</td><td style="padding: 5px; font-size: 13px;">' . '-' . $currency_symbol . number_format( $DiscountData->Amount, 2, '.', '' ) . '</td></tr></table></li>';
							}
						}
						// $temp_html .= '</table>';
						$temp_html .= '</ul>';
