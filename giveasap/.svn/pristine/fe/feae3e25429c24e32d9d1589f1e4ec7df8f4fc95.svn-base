...
function giveasap_is_current_admin_page() {
    if ( ! is_admin() ) {
        return false;
    }

    if ( isset( $_GET['post_type'] ) && 'giveasap' === sanitize_text_field( $_GET['post_type'] ) ) {
        return true;
    }
...
function giveasap_about_notice() {
    if ( ! giveasap_is_current_admin_page() ) {
        return;
    }
				...
				if ( $integration_object ) {
					...
					if ( isset( $_GET['section'] ) && 'documentation' === $_GET['section'] && $integration_object->has_documentation ) {
						...
						<form id="<?php echo esc_attr( $integration_object->settings_id ); ?>" method="POST" action="">
                            <?php wp_nonce_field( 'save_integration', 'integration_nonce' ); ?>
...
function giveasap_about_page() {
	...
	<div class="wrap about-wrap full-width-layout sg-about-page">
	   ...
	   <?php
		...
		<div class="floating-header-section">
			...
			<div class="section-content has-2-columns feature-section two-col --fourth-col">
				<div class="section-item column col">
					<div class="inline-svg section-image " style="background: white;padding-top: 0;padding-bottom: 0;">
                        <img src="<?php echo untrailingslashit( GASAP_URI ) . '/assets/images/thresholds.png'; ?>" alt="">

                    </div>
					<h3>Prize Thresholds</h3>
					<p>If you pick option "Type" under Prize info in a giveaway and change it to "Prize are won when their entries are met", you'll get a "threshold" field when defining each prize.</p>
                ...
                <div class="section-item column col">
                    ...
                    <h3>Various fixes</h3>
                    <p>Update: Fixing Match Captcha integration where warnings were thrown regarding $_SESSION.</p>
                    <p>Fixes: various fixes around different warnings & notices.</p>
                    <p>Fix: Admin notice on new version shows only on pages related to Simple Giveaways</p>
                </div>
		...
		<div class="floating-header-section hidden">
			<div class="section-header">
				<h2><?php esc_html_e( 'Premium & Business Features', 'giveasap' ); ?>
					<?php
			...
			<div class="section-content has-2-columns feature-section two-col --three-col">
                ...
                <div class="section-item column col">
                    <div class="inline-svg white section-image " style="background:white;">
	                    <img src="<?php echo untrailingslashit( GASAP_URI ) . '/includes/integrations/assets/elementor.png'; ?>" alt="">
                    </div>
                    <h3>Elementor Improvements</h3>
                    <p>On the Elementor Giveaway widget, you can now choose to hide the rules</p>
                    <p>Elementor Widgets won't break the site if no giveaway is selected.</p>
                </div>
		...
		<div class="floating-header-section hidden">
			...
			<div class="section-content has-2-columns feature-section two-col --three-col ">
				<div class="section-item column col">
					...
					<p>You can now decide if the user will be redirected to the giveaway page after the purchase is done.</p>
					<p>You can now decide if the "Purchased Action" under a giveawa will be completed when an order is purchased. You can check that under integration settings for WooCommerce.</p>
