...
$metabox_scripts = new GiveASAP_Metabox( 'Scripts', 'giveasap_scripts', array( 'giveasap' ), 'side', 'high', '<i class="fal fa-code"></i>' );
...
$metabox_general->add_field(
	array(
		...
		'type'    => 'select',
		'options' => giveasap_get_giveaway_types(),
...
$metabox_general->add_field(
	array(
		'id'    => 'sg_login_url',
		'name'  => 'sg_login_url',
		'title' => __( 'Login URL', 'giveasap' ),
		'type'  => 'text',
		'class' => array( 'hide_if_any', 'show_if_logged_in' ),
		'desc'  => __( 'Enter where the login form is if a visitor wants to enter the giveaway.', 'giveasap' ),
	)
);

$metabox_general->add_field(
	array(
		'id'    => 'sg_registration_url',
		'name'  => 'sg_registration_url',
		'title' => __( 'Registration URL', 'giveasap' ),
		'type'  => 'text',
		'class' => array( 'hide_if_any', 'show_if_logged_in' ),
		'desc'  => __( 'Enter where the registration form is if a visitor wants to enter the giveaway.', 'giveasap' ),
	)
);
...
$metabox_general->add_field(
	array(
		'id'    => 'sg_giveaway_page',
		'name'  => 'sg_giveaway_page',
		'title' => __( 'Main Giveaway Page', 'giveasap' ),
		'type'  => 'text',
		'desc'  => __( 'Add the link where the giveaway is showing. If left empty, it will use the default giveaway page. This can be useful if you are using a shortcode/block to link to that page from emails.', 'giveasap' ),
	)
);
...
$metabox->add_field(
	array(
		...
		'type'  => 'checkbox',
		'desc'  => __(
			'If checked, Timezone will not be shown in giveaway dates.',
			'giveasap'
		),
	)
);
...
$metabox_prize->add_field(
	array(
		...
		'options' => array(
			...
			'threshold' => __( 'Prizes are won when their entry points are met', 'giveasap' ),
			//'spend'  => __( 'Prizes are won by spending entries for them', 'giveasap' ),
		),
...
function giveasap_free_metabox_text( $metabox_text ) {
	...
	$metabox_text->add_field(
		array(
			...
			'default' => __(
				"<p>NO PURCHASE NECESSARY TO ENTER OR WIN. MAKING A PURCHASE OR PAYMENT OF ANY KIND WILL NOT INCREASE YOUR CHANCES OF WINNING. VOID WHERE PROHIBITED OR RESTRICTED BY LAW.</p>
...
<p>SPONSOR: {{sponsor_name}}, {{sponsor_address}}, {{sponsor_email}}</p>",
				'giveasap'
			),
		)
	);
	...
	$metabox_text->add_field(
		array(
			...
			'type'    => 'wpeditor',
			'desc'    => 'This text will be sent to the winner(s). {{TITLE}} - placeholder for prize name<br/>For form fields use field key as FIELD_KEY in {{FORM_FIELD_KEY}}. Example: field key phone_number, to display it, use {{FORM_PHONE_NUMBER}}.<br/>For Prize instructions use {{PRIZE_INSTRUCTIONS}}',
...
$metabox_prize->add_field(
	array(
		...
		'type'    => 'custom',
		'desc'    => 'Enter the value of the prize (example: $1,000.00)',
		'render'  => 'giveasap_prizes_metabox_field',
        'validate' => 'giveasap_prizes_metabox_field_validate',
		'default' => array(),
	)
);
...
$metabox_prize->add_field(
	array(
		...
		'title'   => __( 'Show Prize Instructions', 'giveasap' ),
		'type'    => 'checkbox',
		'desc'    => 'If checked, the winner will see the prize instructions on the giveaway page.',
		'default' => 'no',
	)
);
...
$metabox_display->add_field(
	array(
		...
		'type'  => 'checkbox',
		'desc'  => __(
			'If checked, header and footer of your site will be displayed also.',
			'giveasap'
		),
	)
);
...
$metabox_display->add_field(
	array(
		...
		'type'  => 'image',
		'desc'  => __(
			'Logo of the sponsor, you or the prize',
			'giveasap'
		),
	)
);
...
$metabox_display->add_field(
	array(
		...
		'type'  => 'text',
		'desc'  => __(
			'Where will the logo link to? Leave empty for no link',
			'giveasap'
		),
	)
);
...
$metabox_display->add_field(
	array(
		'name'  => 'twitter_image',
		'title' => __( 'X Image', 'giveasap' ),
		'type'  => 'image',
		'desc'  => 'This image will be used when people share this giveaway on X (Twitter)',
	)
);
...
$metabox_display->add_field(
	array(
		...
		'title'   => __( 'X Account', 'giveasap' ),
		'type'    => 'text',
		'desc'    => 'X (Twitter) account (with @) to be applied in meta',
		'default' => '@wpgiveaways',
	)
);

$metabox_scripts->add_field(
    array(
        'name'    => 'scripts',
        'title'   => __( 'Scripts', 'giveasap' ),
        'type'    => 'code',
        'desc'    => 'Scripts that will load on the giveaway page',
    )
);

$metabox_scripts->add_field(
    array(
        'name'    => 'visitor_scripts',
        'title'   => __( 'Visitor Scripts', 'giveasap' ),
        'type'    => 'code',
        'desc'    => 'Scripts that will load on the giveaway page for people who are not signed up.',
    )
);

$metabox_scripts->add_field(
    array(
        'name'    => 'subscriber_scripts',
        'title'   => __( 'Subscribed Scripts', 'giveasap' ),
        'type'    => 'code',
        'desc'    => 'Scripts that will load on the giveaway page for people who are signed up.',
    )
);
 ...
 * @param $field
 */
function giveasap_prizes_metabox_field( $field ) {
		...
		<td>
			<div class="sg-prizes-field-container">
				...
				foreach ( $fields as $index => $form_field ) {
					$title     = isset( $form_field['title'] ) ? esc_html( $form_field['title'] ) : '';
					...
					$threshold = isset( $form_field['threshold'] ) ? $form_field['threshold'] : '';
							...
							<table class="form-table sg-form-table">
								...
								<tr class="sg-field-image">
									...
									<td>
										<ul id="sg-field-image-<?php echo esc_attr( $index ); ?>" class="sortable_wordpress_gallery">
											...
											if ( count( $gallery ) > 0 && $gallery[0] ) {
												foreach ( $gallery as $attachment_id ) {
													...
													?>
													<li tabindex="0" role="checkbox" aria-label="<?php echo esc_attr( get_the_title( $attachment_id ) ); ?>" aria-checked="true" data-id="<?php echo esc_attr( $attachment_id ); ?>" class="attachment save-ready selected details">
														<div class="attachment-preview js--select-attachment type-image subtype-jpeg portrait">
															<div class="thumbnail">
																<div class="centered">
																	<img src="<?php echo esc_url_raw( wp_get_attachment_thumb_url( $attachment_id ) ); ?>" draggable="false" alt="">
																</div>
															</div>
														</div>
														<button type="button" data-gallery="#sg-field-image-<?php echo esc_attr( $index ); ?>" class="button-link check remove-sortable-wordpress-gallery-image" tabindex="0">
															<span class="media-modal-icon"></span>
															<span class="screen-reader-text"><?php esc_html_e( 'Deselect', 'giveasap' ); ?></span>
														</button>
										...
										<input type="hidden" id="sg-field-image-<?php echo esc_attr( $index ); ?>_input" name="<?php echo esc_attr( $field['name'] ); ?>[<?php echo esc_attr( $index ); ?>][image]" value="<?php echo esc_attr( $image ); ?>" />
										<button type="button" class="button button-primary add-single-wordpress-image" data-gallery="#sg-field-image-<?php echo esc_attr( $index ); ?>"><?php esc_html_e( 'Add Image', 'giveasap' ); ?></button>
									</td>
								</tr>

                                <tr class="sg-prize-field-threshold show_if_any hide_if_instant hide_if_ticket">
                                    <th>
                                        <label for="sg-prizes-field-threshold-<?php echo esc_attr( $index ); ?>"><?php esc_html_e( 'Threshold', 'giveasap' ); ?></label>
                                    </th>
                                    <td>
                                        <input id="sg-prizes-field-threshold-<?php echo esc_attr( $index ); ?>" type="text" name="<?php echo esc_attr( $field['name'] ); ?>[<?php echo esc_attr( $index ); ?>][threshold]" value="<?php echo esc_attr( $threshold ); ?>" placeholder="Threshold"" />
                                        <p class="description"><?php esc_html_e( 'Enter the number of entries needed for winning this prize. Getting to this threshold automatically awards the prize.', 'giveasap' ); ?></p>
                                    </td>
                                </tr>
						...
						<table class="form-table sg-form-table">
							...
							<tr class="sg-field-image">
								...
								<td>
									...
									<button type="button" class="button button-primary add-single-wordpress-image" data-gallery="#sg-field-desc-{{ data.index }}"><?php esc_html_e( 'Add Image', 'giveasap' ); ?></button>
								</td>
							</tr>

                            <tr class="sg-prize-field-threshold show_if_any hide_if_instant hide_if_ticket">
                                <th>
                                    <label for="sg-prizes-field-threshold-{{ data.index }}"><?php esc_html_e( 'Threshold', 'giveasap' ); ?></label>
                                </th>
                                <td>
                                    <input id="sg-prizes-field-threshold-{{ data.index }}" type="text" name="<?php echo esc_attr( $field['name'] ); ?>[{{ data.index }}][threshold]" value="" placeholder="Threshold"" />
                                    <p class="description"><?php esc_html_e( 'Enter the number of entries needed for winning this prize. Getting to this threshold automatically awards the prize.', 'giveasap' ); ?></p>
                                </td>
                            </tr>
