...
if ( class_exists( 'OPI_Security_Boost' ) ) {
	return;
}

class OPI_Security_Boost {
	...
	public function __construct() {
		 ...
		 * WordPress Hooks
		 */
		add_action( 'init', array( $this, 'action_init_setup_plugin_options' ), PHP_INT_MAX );
		add_action( 'admin_init', array( $this, 'action_admin_init' ) );
	}
	public function action_admin_init() {
		opi_security_boost_options_init();
	}

	public function action_init_setup_plugin_options() {
		$this->options = opi_security_boost_get_options();
	}
		...
		foreach ( $fields as $name => $data ) {
			...
			foreach ( array( 'placeholder', 'style', 'class', 'id' ) as $extra_key ) {
				if ( isset( $data[ $extra_key ] ) ) {
					$extra .= sprintf( ' %s="%s" ', $extra_key, esc_attr( $data[ $extra_key ] ) );
				}
			}
			/**
			 * start row
