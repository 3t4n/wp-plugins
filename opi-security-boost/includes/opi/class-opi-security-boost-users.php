...
class OPI_Security_Boost_Users extends OPI_Security_Boost {
	...
	/**
	 * Last check to user meta, to avoid privilega remove just after upgrade
	 * this plugin.
	 *
	 * @since 1.0.8
	 */
	private $user_meta_last_check = '_opi_last_check';
	...
	public function __construct() {
		...
		add_action( 'shutdown', array( $this, 'action_shutdown_maybe_lower_users_privileges' ) );
		add_action( 'shutdown', array( $this, 'action_shutdown_maybe_set_last_check' ) );
		/**
	...
	public function filter_add_sortable_columns( $columns ) {
		$columns['user_registered']                            = array(
			...
			esc_html__( 'Table ordered by registration time.', 'opi-security-boost' ),
			'asc',
		);
		$columns[ $this->user_meta_last_login_timestamp_name ] = array(
			...
			esc_html__( 'Table ordered by last time.', 'opi-security-boost' ),
			'asc',
		);
		return $columns;
	}
	 ...
	 * @since 1.0.8
	 */
	private function get_exclude_roles_array() {
		return apply_filters(
			'opi/security-boost/wp_user_query/role__not_in',
			array(
				'administrator',
				'subscriber',
				/**
	...
	/**
	 * Check to lower user privileges.
	 *
	 * @since 1.0.8
	 *
	 */
	public function action_shutdown_maybe_lower_users_privileges() {
		if ( 1 < wp_rand( 1, 20 ) ) {
			// return;
		}
		/**
		 * Args for WP_User_Query
		 */
		$args = apply_filters(
			'opi/security-boost/privileges/args',
			array(
				// 'role__not_in' => $this->get_exclude_roles_array(),
				'fields'     => array(
					'ID',
					'user_login',
					'user_registered',
				...
				'meta_query' => array(
					'relation' => 'AND',
					array(
						'key'     => $this->user_meta_last_login_timestamp_name,
						'value'   => intval( time() - intval( apply_filters( 'opi/security-boost/privileges/time', YEAR_IN_SECONDS ) ) ),
						'compare' => '<',
						'type'    => 'NUMERIC',
					),
					array(
						'key'     => $this->user_meta_last_check,
						'value'   => time() - intval( apply_filters( 'opi/security-boost/check/time', YEAR_IN_SECONDS ) ),
						'compare' => '<',
						'type'    => 'NUMERIC',
					),
				),
		...
		// l($user_query);
		// l($user_query->get_results());
		if ( empty( $user_query->get_results() ) ) {
			return;
		}
		foreach ( $user_query->get_results() as $user ) {
			if ( strtotime( $user->user_registered ) < time() - YEAR_IN_SECONDS ) {
	...
	/**
	 * Adds check flag to avoid lowering privileges just after fresh plugin
	 * installation.
	 *
	 * @since 1.0.8
	 */
	public function action_shutdown_maybe_set_last_check() {
		/**
		 * Args for WP_User_Query
		 */
		$args = apply_filters(
			'opi/security-boost/check/args',
			array(
				'role__not_in' => $this->get_exclude_roles_array(),
				'fields'       => 'ID',
				'meta_query'   => array(
		...
		foreach ( $user_query->get_results() as $user_id ) {
			add_user_meta( $user_id, $this->user_meta_last_check, time(), true );
		}
	}
