...
class Ups
{
    ...
    {
        if($apiUrlOption = $this->getApiUrlOption($key)){
            return $apiUrlOption;
        }
        return isset($this->options[$key]) ? $this->options[$key] : $default;
    }

    protected function getApiUrlOption($key){
        if($key !== 'integration_api_url' && $key !== 'integration_picking_api_url'){
            return '';
        }
        if($this->options['integration_api_environment'] === 'test'){
            if($key === 'integration_api_url'){
                return 'https://testplugins.ship.co.il';
            }

            if($key === 'integration_picking_api_url'){
