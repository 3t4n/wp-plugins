	...
	class WC_Ups_PickUps {
        ...
        private function admin_page_change_pickup_point(){
            ...
            function change_pickup_point_page() {
                ...
                $orderId = isset($_GET['order_id']) ? sanitize_text_field($_GET['order_id']) : '';
                $orderPage = isset($_GET['order_page']) ? sanitize_text_field($_GET['order_page']) : '';

                // Create a sanitized URL for form action
                $changePickupPointUrl = admin_url('admin-ajax.php?action=change_pickup_point&order_page=' . rawurlencode($orderPage) . '&order_id=' . rawurlencode($orderId));
                ?>
                <h2>
                    <?php echo esc_html(__('Change Pickup Point For Order #', WC_Ups_PickUps::TEXT_DOMAIN)) . esc_html($orderId); ?>
                </h2>

                <form class="change-pickup-point-form" method="POST" action="<?= esc_url($changePickupPointUrl) ?>">
                    ...
                    <div class="ups-pickups-desc">
                        <?php echo esc_html(__("Click here to select your PickUP location", WC_Ups_PickUps::TEXT_DOMAIN)); ?>
                    </div>
        ...
        public function woocommerce_checkout_update_fields($order_id, $posted){
            ...
            $shippingFloorNumber = isset($posted['shipping_floor_num']) ? $posted['shipping_floor_num'] : '';
            if(empty($shippingFloorNumber)){
                $shippingFloorNumber = isset($posted['billing_floor_num']) ? $posted['billing_floor_num'] : '';
            }

            $shippingRoomNumber = isset($posted['shipping_room_num']) ? $posted['shipping_room_num'] : '';
            if(empty($shippingRoomNumber)){
                $shippingRoomNumber = isset($posted['billing_room_num']) ? $posted['billing_room_num'] : '';
            }
            if(isset($shippingFloorNumber)) {
                $order->update_meta_data('ups_order_shipping_floor_num', wc_clean($shippingFloorNumber));
            }
            if(isset($shippingRoomNumber)) {
                $order->update_meta_data('ups_order_shipping_room_num', wc_clean($shippingRoomNumber));
            }
            $order->save();
        }
