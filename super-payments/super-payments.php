 ...
 * Author URI: https://superpayments.com/
 * Version: 1.30.6
...
function wcsp_on_activate() {
	...
	// Schedule the product ingestion for 2 AM.
	$current_time = time();
	$today_2am    = strtotime( 'today 2:00', $current_time );
	$start_time   = $today_2am;

	if ( $today_2am < $current_time ) {
		$start_time = strtotime( 'tomorrow 2:00', $current_time );
	}

	as_schedule_single_action( $start_time, 'wcsp_process_ingestion_batch', [ [] ], 'product-ingestion' );
}
 ...
 */
function wcsp_handle_plugin_upgrade( $upgrader_object, $options ) {
	// Check if this is our plugin being upgraded.
		...
		if ( isset( $options['plugins'] ) ) {
			foreach ( $options['plugins'] as $plugin ) {
				if ( plugin_basename( __FILE__ ) === $plugin ) {
					...
					if ( 'completed' === $ingestion_status || is_numeric( $ingestion_status ) ) {
						return;
					}

					// Schedule product ingestion for 2 AM.
					$current_time = time();
					$today_2am    = strtotime( 'today 2:00', $current_time );
					$start_time   = $today_2am;
...
add_action( 'upgrader_process_complete', 'wcsp_handle_plugin_upgrade', 10, 2 );
