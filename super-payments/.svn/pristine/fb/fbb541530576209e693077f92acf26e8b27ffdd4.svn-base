...
class WC_Super_Payments_Base_Gateway extends WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		if ( ! is_wp_error( $response ) ) {
			...
			if ( isset( $body['redirectUrl'] ) ) {
				...
				if ( 402 === $response['response']['code'] ) {
					$error_message = isset( $body['message'] ) ? 'Unable to complete payment. ' . $body['message'] : __( 'Unable to complete payment. Please check your card details are correct, or try another card.', 'super-payments' );
				}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		$test_mode = $this->get_option( 'test_mode' );
		$response  = wp_remote_post( wcsp_get_super_api_url( $test_mode ) . '/2025-02-01/refunds', $post_discount_request );
		...
		if ( ! is_wp_error( $response ) ) {
			if ( 200 === $response['response']['code'] || 201 === $response['response']['code'] ) {
				...
				$refund_success_order_note = sprintf( __( 'Refund processed by Super Payments for payment with reference - %1$s', 'super-payments' ), $super_refund_reference );

				// Process the funding summary from the refund.
				if ( isset( $body['fundingSummary'] ) ) {
					$update_total_setting = $this->get_option( 'update_total' );
					...
					$refund_success_order_note .= $process_funding_summary_note;
				}
				...
				$order->add_order_note( $refund_success_order_note );
				return true;
			} else {
