...
function wcsp_admin_order_totals_after_shipping( $order_id ) {
	...
	if ( 'superpayments' === $order->get_payment_method() ) {
		...
		// If the last update total setting was not to update the order total, don't show the cash reward.
		if ( 'order_total' !== $update_total_setting ) {
			return;
		}

		$super_cash_reward_amount = $order->get_meta( 'wcsp_merchant_funded_amount_major_units' );

		// Check old meta key if new one is not present.
