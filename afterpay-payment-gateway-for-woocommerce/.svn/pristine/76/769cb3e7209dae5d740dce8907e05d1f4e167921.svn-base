...
function afterpay_custom_thankyou( $thankyou_text, $order ){
		$link = '<a href="https://my.riverty.com/" target="_blank"><strong>MyRiverty</strong></a>'; 
	?>	
		<style>
...
return '<p class="woocommerce-notice woocommerce-notice--success woocommerce-thankyou-order-received">' . $thankyou_text . '</p>
		<div class="checkout-custom-success">
			<div>
				<img style="height: 120px" class="afterpay-success-logo" src="https://cdn.riverty.design/logo/riverty-primary-badge.svg" alt="riverty_logo not found"/>
			</div>
			<div class="afterpay-text">
				...
				<p class="afterpay-thanks-message" style="margin-top: 10px">' . __( "Thank you for your purchase and for choosing Riverty as your payment method. An email with the order confirmation, details and tracking info is on the way, but feel free to discover what Riverty can do for you.", "afterpay-payment-gateway-for-woocommerce" ) . '</p>
				<p style="margin-top: 10px">' . sprintf(__( "Download our app on your smartphone or visit %s in your browser to find out the easiest way to keep track of outstanding payments, pause payments for returns or keep track and many more options.", "afterpay-payment-gateway-for-woocommerce" ), $link ) . '</p>
			</div>
		</div>';
}
...
function validate_bankaccount( WP_REST_Request $request ) {
  ...
  if ( isset( $afterpay_bankvalidation->order_result->return->isValid ) ) {
	...
	$is_valid = false;
	$message = __( 'An error has occured, please try again later', 'afterpay-payment-gateway-for-woocommerce' );
  }
