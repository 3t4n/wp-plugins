<?php
/**
 * AfterPay Campaign invoice payment method for Denmark
 *
 * @category   Class
 ...
 * AfterPay Campaign invoice payment method for Denmark
 *
 * @class         WC_Gateway_Afterpay_Dk_Campaign
 * @extends       WC_Gateway_Afterpay_Base_Rest
 * @package       Arvato_AfterPay
 * @author        AfterPay
 */
#[AllowDynamicProperties]
class WC_Gateway_Afterpay_Dk_Campaign extends WC_Gateway_Afterpay_Base_Rest {
	...
	public function __construct() {
		...
		$this->id           = 'afterpay_dk_campaign';
		$this->method_title = __( 'Riverty Denmark Campaign invoice', 'afterpay-payment-gateway-for-woocommerce' );
	...
	public function init_form_fields() {
		...
		$this->form_fields = apply_filters(
			'afterpay_dk_campaign_form_fields', array(
				'enabled'                               => array(
					'title'   => __( 'Enable/Disable', 'afterpay-payment-gateway-for-woocommerce' ),
					'type'    => 'checkbox',
					'label'   => __( 'Enable Riverty Denmark Campaign invoice', 'afterpay-payment-gateway-for-woocommerce' ),
					'default' => 'no',
				),
				'test_api_key'                               => array(
	...
	public function payment_fields() {
		...
		<fieldset class="riverty_fieldset">
			...
			<?php endif; ?>
			<p class="form-row">
				<label for="<?php echo esc_attr( $this->id ); ?>_ssn"><strong><?php esc_html_e( 'Tast inn dit CPR-nummer (10 siffer) for å gennemføre købet', 'afterpay-payment-gateway-for-woocommerce' ); ?>: <span class="required">*</span></strong></label>
				<input type="input" class="input-text" name="<?php echo esc_attr( $this->id ); ?>_ssn" placeholder="DDMMÅÅXXXX"/>
			</p>
			<div class="clear"></div>
