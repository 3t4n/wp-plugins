<?php
/**
 * AfterPay Installments payment method for Austria
 *
 * @category   Class
 ...
 * AfterPay Installments payment method for Austria
 *
 * @class    WC_Gateway_Afterpay_At_Installments
 * @extends  WC_Gateway_Afterpay_Base_Rest
 * @package  Arvato_AfterPay
 * @author   AfterPay
 */
#[AllowDynamicProperties]
class WC_Gateway_Afterpay_At_Installments extends WC_Gateway_Afterpay_Base_Rest {
	...
	public function __construct() {
		...
		$this->id           = 'afterpay_at_installments';
		$this->method_title = __( 'Riverty Austria Fixed instalments', 'afterpay-payment-gateway-for-woocommerce' );
		$this->has_fields   = true;

		// Setup field to load installmentPlans in.
		$this->available_installment_plans = array();
		...
		$this->title             = 'Ratenzahlung';
		$this->extra_information = 'Erst erleben, dann flexibel bezahlen';
		...
		$country_code 			    = ( $current_locale == 'de' ) ? 'de_at/' : 'en_at/';
		$afterpay_invoice_terms     = 'https://documents.myafterpay.com/consumer-terms-conditions/'. $country_code . $merchantid . 'fix_installments';
	...
	public function init_form_fields() {
		$this->form_fields = apply_filters(
			'afterpay_at_installments_form_fields', array(
				'enabled'                               => array(
					'title'   => __( 'Enable/Disable', 'afterpay-payment-gateway-for-woocommerce' ),
					'type'    => 'checkbox',
					'label'   => __( 'Enable Riverty Austria Fixed instalments', 'afterpay-payment-gateway-for-woocommerce' ),
					'default' => 'no',
				),
				'test_api_key'                               => array(
	...
	public function payment_fields() {
		...
		<style>
			...
			.installment_row input {
				margin: 0 0 0 10px;
			}
		</style>
		<script type="text/javascript">
			jQuery(document).on('click', 'input[type=radio][name="<?php echo esc_attr( $this->id ); ?>_installmentplan"]', function(e) {
		...
		<fieldset class="riverty_fieldset">
			<p class="form-row">
				<strong>Ratenplan wählen:</strong>
				<?php 
					$i = 0; 
					$reversed_installment_plans = array_reverse( $this->available_installment_plans );
				?>
				<?php foreach ( $reversed_installment_plans as $installment_plan ) : ?>
				<div class="installment_row">
					<input type="radio" id="<?php echo esc_attr( $this->id ); ?>_installmentplan_<?php echo esc_attr( $installment_plan->installmentProfileNumber ); ?>" value="<?php echo esc_attr( $installment_plan->installmentProfileNumber ); ?>" name="<?php echo esc_attr( $this->id ); ?>_installmentplan" class="afterpay_installmentplan" 
					<?php
						if ( 0 === $i ) {
							echo 'checked';}
						?>
					>
					<label for="<?php echo esc_attr( $this->id ); ?>_installmentplan_<?php echo esc_attr( $installment_plan->installmentProfileNumber ); ?>">
						<strong><?php echo esc_html( $installment_plan->installmentAmount ) . ' ' . get_woocommerce_currency_symbol(); ?> <?php esc_html_e( 'per month', 'afterpay-payment-gateway-for-woocommerce' ); ?></strong>
						<?php echo str_replace('%s', esc_html( $installment_plan->numberOfInstallments ), __( 'For %s months', 'afterpay-payment-gateway-for-woocommerce' ) ); ?>
					</label>
				</div>
				<?php $i++; ?>
				<?php endforeach; ?>
				...
				<div class="installment_plan 
					<?php
					if ( 0 === $i ) {
						echo 'installment_active';}
					?>
					" id="installment_plan_info_<?php echo esc_attr( $installment_plan->installmentProfileNumber ); ?>">
					<p class="afterpay_terms_and_conditions">
						Bei einem Warenkorb in Höhe von <?php echo esc_html( $installment_plan->basketAmount ) . ' ' . get_woocommerce_currency_symbol(); ?> und <?php echo esc_html( $installment_plan->numberOfInstallments ); ?> gleichbleibenden Raten beläuft sich der Gesamtbetrag auf <?php echo esc_html( $installment_plan->totalAmount ) . ' ' . get_woocommerce_currency_symbol(); ?>. Soll-Zinssatz ist <?php echo esc_html( $installment_plan->interestRate ); ?> % und Effektivzinssatz ist <?php echo esc_html( $installment_plan->effectiveInterestRate ); ?> %.
					</p>
				</div>
				<?php $i++; ?>
				<?php endforeach; ?>
		...
		<fieldset class="riverty_fieldset">
			...
			<?php endif; ?>
			<p class="form-row">
				<label for="<?php echo esc_attr( $this->id ); ?>_bankaccount"><strong><?php esc_html_e( 'Bankverbindung (IBAN):', 'afterpay-payment-gateway-for-woocommerce' ); ?> <span class="required">*</span></strong></label>
				...
				<script type="text/javascript">
					jQuery( document ).ready(function($) {
						...
						$( "#<?php echo esc_attr( $this->id ); ?>_bankaccount" ).mask(
							"<?php echo $this->get_bankaccount_placeholder( $this->afterpay_country  ) ?>", 
							{
								placeholder: "<?php echo $this->get_bankaccount_placeholder( $this->afterpay_country  ) ?>",
								translation: {
									'A': {
										pattern: /A/,
										fallback: 'A',
						...
						var minimumCharsErrorMessage = '<?php echo __( 'IBAN must be at least 15 characters minimum', 'afterpay-payment-gateway-for-woocommerce' ); ?>';
						var requestFailureErrorMessage = '<?php echo __( 'Something went wrong, please try again later', 'afterpay-payment-gateway-for-woocommerce' ); ?>';
						...
						input.addEventListener('keyup', function (e) {
							...
							if ( length >= 15 ) {
								...
								$("#<?php echo esc_attr( $this->id ); ?>_validation_response").text(minimumCharsErrorMessage);
						...
						function sendRequest()
						{
							...
							$.ajax({
								...
								function () {
									...
									$("#<?php echo esc_attr( $this->id ); ?>_validation_response").addClass("invalid_account");
									$("#<?php echo esc_attr( $this->id ); ?>_validation_response").css('display', 'inline-block');
									$("#<?php echo esc_attr( $this->id ); ?>_validation_response").text(requestFailureErrorMessage);
									$('#payment #place_order').removeAttr("disabled");
								}
							);
						}
			...
			<p class="form-row">
				...
				<?php
					if ( 0 === $i ) {
						echo 'installment_active';}
					?>
				" id="installment_plan_european_standard_<?php echo esc_attr( $installment_plan->installmentProfileNumber ); ?>">
					<p class="afterpay_terms_and_conditions">
						<?php echo str_replace("%s", esc_html( $installment_plan->readMore ), __( 'Here you will find the mandatory information according to <a class="afterpay_terms_link" href="https://documents.myafterpay.com/consumer-terms-conditions/de_DE/default/bgb507" target="_blank">§ 507 BGB</a> and the <a class="afterpay_terms_link" href="%s" target="_blank">European Standard Information for Consumer Credit</a>.', 'afterpay-payment-gateway-for-woocommerce' ) ); ?> 
					</p>
				</div>
				<?php $i++; ?>
				<?php endforeach; ?>
			...
			<p class="form-row" style="margin-top:15px">
				...
				<label style="display:inline">
					<?php echo $this->get_tracking_text('fix_instalments', 'at'); ?>
				</label>
			</p>
	...
	public function validate_fields() {
		...
		if ( ! isset( $_POST[ $this->id . '_installmentplan' ] ) || empty( $_POST[ $this->id . '_installmentplan' ] ) ) {
			array_push( $validation_messages, __( 'Please choose an installment plan.', 'afterpay-payment-gateway-for-woocommerce' ) );
		}
