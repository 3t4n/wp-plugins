...
function pickplugins_wl_add_to_wishlist($wishlist_id = 0, $item_id = 0, $obj_type = 'post')
{
    ...
    if ($wishlist_id == 0 || $item_id == 0 || $user_id == 0) return false;

    if (in_array($wishlist_id, pickplugins_wl_is_wishlisted($item_id, $user_id))) return false;
...
function pickplugins_wl_is_wishlisted($item_id, $user_id)
{

    if (empty($item_id) || $user_id == 0) return false;
    ...
    $results = $wpdb->get_results("
		...
		WHERE	post_id = $item_id AND user_id = $user_id
	");
