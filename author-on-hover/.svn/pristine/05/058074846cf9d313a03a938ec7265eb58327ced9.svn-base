		...
		if ( is_singular( 'post' ) ) {
			...
			if ( $aoh_has_multiple_author ) {
				...
				foreach ( $aoh_authors as $aoh_author_key => $aoh_author_id ) {
					...
					$aoh_profile_img_from = isset( $aoh_get_auth_meta['aoh_profile_img_from'] ) ? $aoh_get_auth_meta['aoh_profile_img_from'] : '';
					$aoh_auth_name_from   = isset( $aoh_get_auth_meta['aoh_auth_name_from'] ) ? $aoh_get_auth_meta['aoh_auth_name_from'] : '';
					$aoh_auth_name        = isset( $aoh_get_auth_meta['aoh_auth_name'] ) ? $aoh_get_auth_meta['aoh_auth_name'] : '';
					$aoh_auth_designation = isset( $aoh_get_auth_meta['aoh_auth_designation'] ) ? $aoh_get_auth_meta['aoh_auth_designation'] : '';

					echo '<figure class="aoh-multi-author-img">';
					if ( 'media_library' === $aoh_profile_img_from ) {
					...
					if ( 'default' === $aoh_auth_name_from ) {
						...
						echo esc_html( $aoh_auth_name );
					}

					echo '<span class="aoh-multi-author-designation">';
					echo $aoh_auth_designation ? wp_kses_post( $aoh_auth_designation ) : '';
					echo '</span>';
