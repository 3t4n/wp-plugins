...
class AOH_Author_On_Hover {
	...
	private function load_dependencies() {
		...
		/**
		 * The class responsible for Gutenberg block.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-aoh-block-output.php';
	...
	private function define_admin_hooks() {
		...
		function add_custom_notice_on_user_new_page() {

			$screen = get_current_screen();
			if ( $screen && $screen->id === 'user' ) {
				?>
				<div class="notice notice-info is-dismissible">
					<p><strong>Note:</strong> Please ensure that all new users have a secure password and a valid email address for proper account functionality.</p>
				</div>
				<?php
			} elseif ( $screen && $screen->id === 'user-edit' ) {
				?>
				<div class="notice notice-info is-dismissible">
					<p><strong>Note:</strong> Go to <a href="#aoh-targeted-to-scroll">Custom Profile Options</a> 👇</p>
				</div>
				<?php
			}
		}
		add_action( 'admin_notices', 'add_custom_notice_on_user_new_page' );
	...
	private function define_public_hooks() {
		 ...
		 * Block development.
		 */
		$plugin_block_output = new AOH_Block_Public_Output( $this->get_plugin_name(), $this->get_version() );
		$this->loader->add_action( 'init', $plugin_block_output, 'aoh_render_block_core' );
	}
