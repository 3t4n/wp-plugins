...
( function( blocks, blockEditor, element, components ) {
  ...
  if ( !window.aoh_gutenberg_blocks ) { return; }

  Object.values(window.aoh_gutenberg_blocks).forEach( function( block ) {
    ...
    registerBlockType(block.name, {
      ...
      edit: function (props) {
        return (
          createElement('div', {className: 'aoh-shortcode-block'},

            createElement(Button, {
              'data-modal-id': block.modal_id,
              'data-gutenberg-id': block.name,
              className: 'is-secondary aoh-shortcode-button',
              onClick: function () {
                window.aoh_gutenberg_props = props;
              },
            }, block.button_title ),
