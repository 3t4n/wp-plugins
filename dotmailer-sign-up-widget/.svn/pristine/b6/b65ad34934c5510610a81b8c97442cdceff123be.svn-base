...
class Dotdigital_WordPress_Public
{
    ...
    {
        $attributes = shortcode_atts(array('showtitle' => $atts['showtitle'] ?? 1, 'showdesc' => $atts['showdesc'] ?? 1, 'redirecturl' => $atts['redirection'] ?? null, 'is_ajax' => $atts['with_ajax'] ?? 0), $atts, $tag);
        \ob_start();
        the_widget(\Dotdigital_WordPress\Includes\Widget\Dotdigital_WordPress_Sign_Up_Widget::class, array(), array('showtitle' => $attributes['showtitle'], 'showdesc' => $attributes['showdesc'], 'redirection' => $attributes['redirecturl'], 'is_ajax' => $attributes['is_ajax']));
