...
if ( ! class_exists( 'WooCommerce_Putler_Connector' ) ) {
	...
	class WooCommerce_Putler_Connector {
			...
			if ( empty( $sub_details ) && empty( $sub_meta ) ) {// for non-initial data sync.
				...
				if ( ! empty( $results ) && count( $results ) > 0 ) {
					...
					foreach ( $results as $result ) {
						...
						if ( ! empty( $params['sub_details'] ) ) {
							$sub_details[ $result['mkey'] ] = $result['mvalue'];
						}

						if ( ! empty( $result['mkey'] ) && ! empty( $sub_meta_keys[ $result['mkey'] ] ) ) {
							$sub_meta[ $sub_meta_keys[ $result['mkey'] ] ] = $result['mvalue'];
						}
					}
				}
			} else {
		...
		public function get_orders( $params ) {
			...
			if ( ! empty( $results_order_details ) && count( $results_order_details ) > 0 ) {
				...
				if ( $results_order_item_details_count > 0 ) {
					...
					foreach ( $results_order_details as $order_detail ) {
						...
						foreach ( $cart_items as $item_id => $cart_item ) {
							...
							$order_item                = array();
							$order_item ['Type']       = ( false === $sub_trans && 'Refund' !== $main_response ['Type'] ) ? 'Shopping Cart Item' : $main_response ['Type'];
							$order_item ['Item_Title'] = html_entity_decode( ! empty( $cart_item['_product_id'] ) ? get_the_title( $cart_item['_product_id'] ) : $cart_item['product_name'] );
							...
							$order_item ['Gross']    = ( false === $sub_trans && 'Refund' !== $main_response ['Type'] && is_numeric( $cart_item['_line_total'] ) ) ? round( floatval( $cart_item['_line_total'] ), 2 ) : $main_response ['Gross'];
							...
							if ( 0.00 === floatval( $main_response ['Discount'] ) && 0.00 === floatval( $main_response ['Gross'] ) ) {
							...
							if ( empty( $params['trash'] ) ) {
								$response ['Raw_Data'] = ( false === $sub_trans || 'Refund' === $main_response ['Type'] ) ? '' : $main_response ['Raw_Data'];
							} else {
								$response ['Raw_Data'] = '';
							}
