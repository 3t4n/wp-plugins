...
class Admin_Notices {
	...
	public function admin_notices() {
		...
		if ( current_user_can( 'manage_options' ) ) {
            ...
            //add message if experirement cache is enabled
            if ( Plugin::$instance->experiments->is_feature_active( 'e_element_cache' ) ) {
                if ($this->notice_element_cache_enabled()){
                    return;
                }
            }
...
//            return false;
     ...
     * @since 1.3
     */
    private function notice_element_cache_enabled() {
        $notice_id = 'element_cache';

        if ( ! current_user_can( 'manage_options' ) ) {
            return false;
        }

        $elementor_settings_url = self_admin_url( 'admin.php?page=elementor-settings#tab-experiments' );

        $options = [
            'title' => esc_html__( 'Elementor: Experimental Cache', 'falang-for-elementor-lite' ),
            'description' => __( 'Elementor experimental caching is enabled, you have to disabled it to make Falang for Elementor working on Front-end. Elementor / Settings / Features', 'falang-for-elementor-lite' ),
            'id' => $notice_id,
            'classes' => [ 'alert', 'falang-notice' ],
             'button' => [
            'icon_classes' => 'dashicons dashicons-update',
            'text' => esc_html__( 'Change Settings', 'falang-for-elementor-lite' ),
            'url' => $elementor_settings_url,
        ],
        ];
