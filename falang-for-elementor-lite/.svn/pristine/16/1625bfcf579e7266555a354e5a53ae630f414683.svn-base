...
class FalangElementor
{
     ...
     * @update 1.24 remove code for essential addon's wrapper link and use classic way with priority
     */
    function init()
    {
        ...
        if (is_plugin_active('happy-elementor-addons/plugin.php') ||
            ...
            is_plugin_active('essential-addons-for-elementor-lite/essential_adons_elementor.php') ||
    ...
    * */
     ...
     * @update 1.20 fix typo error in the new extra param's way
     * @update 1.21 add Translate button and hidden field
     * @update 1.23 add condition on field translation (display only if necessary)
     * @update 1.26 remove hidden field/translate button use icon with parameter inside
     * */
        ...
        foreach ($sections as $falang_section) {
            ...
            if ($section_name === $section_id) {
                ...
                foreach ($this->language_list as $language) {
                    ...
                    $section_name_locale = $section_name . '_<span>' . $flag . '</span>';
                    ...
                    if (isset($falang_section['control']) && !empty($falang_section['control'])) {
                        ...
                        foreach ($falang_section['control'] as $key) {
                            ...
                            //add translate button to supported field
                            $icon_translate = "";
                            if (in_array($control['type'], ['text', 'textarea', 'wysiwyg'])) {
                                //e-translate-button class is used in the js to add event
                                if ($this->is_falang_for_elementor_pro_activated()) {
                                    $icon_translate = '<button  data-type="' . $control['type'] . '" 
                                                            data-name="' . $control['name'] . '" 
                                                            data-target="' . $language->locale . '" 
                                                            style="margin-left:10px;border:none"
                                                            title="Translate text"
                                                            class="e-translate-button"> 
                                                            <i class="eicon-globe"></i></button>';
                                } else {
                                    $icon_translate = '<button onclick="alert(\'This feature is only available with Falang for Elementor Pro.\')"
                                                                title="Translate text (Pro only)"
                                                               style="margin-left:10px;border:none" > 
                                                            <i class="eicon-globe"></i></button>';
                                }
                            }
                            ...
                            $newControl = [
                                'label' => trim($label . ' ' . $language->locale) . $icon_translate,
                                'type' => $control['type'],
                            ];
                            if (isset($control['placeholder'])) {
                                $newControl['placeholder'] = $control['placeholder'];
                            }
                            if (isset($control['options'])) {
                                $newControl['options'] = $control['options'];
                            }
                            if (isset($control['label_block'])) {
                                $newControl['label_block'] = $control['label_block'];
                            }
                            if (isset($control['condition'])) {
                                $newControl['condition'] = $control['condition'];
                            }
                            ...
                            //remove ai button disabled for now
                            if (isset($elts[$widget_name]["translate_service"]) && $elts[$widget_name]["translate_service"]) {
                                //$newControl['ai'] = ['active' => true];//remove ai button if translate
                            }
                    ...
                    if (isset($falang_section['repeatable']) && !empty($falang_section['repeatable'])) {
                        ...
                        foreach ($falang_section['repeatable'][0][$repeatable_name] as $key) {
                            ...
                            if (isset($control['fields'][$key]['placeholder'])) {
                                $newControl['placeholder'] = $control['fields'][$key]['placeholder'];
                            }
                            if (isset($control['fields'][$key]['label_block'])) {
                                $newControl['label_block'] = $control['fields'][$key]['label_block'];
                            }
                            if (isset($control['fields'][$key]['condition'])) {
                                $newControl['condition'] = $control['fields'][$key]['condition'];
                            }
    ...
    /**
     * NO MORE IN USE Prioriry is set to 100 in Wrapper_Link since 1.24
