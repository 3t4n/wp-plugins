...
* Author URI: https://listingsforappfolio.com/?utm_source=wp-plugin-author&utm_medium=wporg
* License: GPL+2
		...
		function apfl_config_callback(){
			...
			if($_POST){
				if(isset($_POST['apfl_config_submit'])){
					...
					if (!wp_verify_nonce($nonce, 'apfl_settings_option')) {
						wp_die( __('Error - Could not verify POST values') );
					}
					
					if(isset($_POST['apfl_config_url'])){
						$apfl_url = sanitize_url($_POST['apfl_config_url']);
			...
			<div class="wrap">
				<div id="apfl_settings">
					<form method='POST' action="">
						<?php
						wp_nonce_field('apfl_settings_option');
						?>
						<br>
						...
						<table class="form-table">
							<tr>
								...
								<td>
									<input type="text" name="apfl_config_url" id="apfl_config_url" style="min-width: 350px;" placeholder="For Example - https://example.appfolio.com" value="<?php echo esc_url( $apfl_listing_url ); ?>">
								</td>
							</tr>
							...
							<tr>
								...
								<td>
									<input type="text" name="apfl_config_gmap_api" id="apfl_config_gmap_api" style="min-width: 350px;" placeholder="Leave Blank to disable Google Map" value="<?php echo esc_attr( $apfl_gmap_api ); ?>">
								</td>
							</tr>
						</table>
