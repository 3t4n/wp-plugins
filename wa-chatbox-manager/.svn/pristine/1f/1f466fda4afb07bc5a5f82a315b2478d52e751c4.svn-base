...
if( !class_exists( 'chatbox_manager_panel' ) ) {
	...
	class chatbox_manager_panel {
		...
		public function add_script() {
			 ...
			 wp_enqueue_style ( 'chatbox_manager_notice', $file_dir.'css/notice.css' );
		...
		public function save_option() {
			...
			if ( $this->chatbox_manager_request('chatbox_manager_action') == 'Save' ) {
				...
				if (
					...
					!isset($_POST['chatbox_manager_save_nonces']) ||
					!wp_verify_nonce(esc_attr($_POST['chatbox_manager_save_nonces']), 'chatbox_manager_save_options' )
				) {
					exit;
				}
							...
							if (isset($value['type'])) :
								...
								case 'start-form':
									...
									echo $chatboxManagerForm->elementStart('div', str_replace(' ', '', $value['name']), FALSE );
										...
										echo $chatboxManagerForm->input('chatbox_manager_save_nonces', FALSE, FALSE, 'hidden', esc_attr(wp_create_nonce( 'chatbox_manager_save_options' )));
