...
<?php } else { ?>
	...
	if ($package) {
		...
		$p_index = 0;
		foreach ($item->packages->package as $package_choices) {
			$package_items[] = $package_choices;
			foreach ($package_choices as $choice) {
				if ((int)$choice->id != 0) {
					$included_uids[$p_index][] = (int)$choice->id;
				}
				...
				if (property_exists($day, 'items')) {
					$formatted_date = date('Y-m-d', isset($day->date) ? (int)$day->date : 0);
						...
						foreach ($day->items as $v) {
							if ($day->cond == 'a' && in_array((int)$v->uid, $included_uids[0])) {
									...
									choose_opt_<?php echo esc_html($i); ?>_<?php echo esc_html($j); ?> = function(index, uid){
										...
										$.ajax({
											...
											data: {
												...
												amount: amount, 
												currency_symbol: '<?php echo $company->currency_symbol; ?>',
												currency_decimals: '<?php echo $company->currency_decimals; ?>',
												currency_separator: '<?php echo $company->currency_separator; ?>',
												security: '<?php echo wp_create_nonce('rezgo-nonce'); ?>',
											},
											context: document.body,
											success: function(data) {
										...
										<?php } ?>
											...
											$.ajax({
												...
												data: {
													...
													included_uids: '<?php echo json_encode($included_uids[$i+1]); ?>',
										...
										let nextMonth = $(this)[0].currentYear + '-' + addLeadingZero($(this)[0].currentMonth + 1);
											...
											$.ajax({
												...
												data: {
													...
													included_uids: '<?php echo json_encode($included_uids[$i]); ?>',
												<?php if ($i >= 1) { ?>
								...
								$('#select_guests').addClass('locked');
									...
									$.ajax({
										...
										data: {
											...
											now: '<?php echo esc_html($now); ?>',
											uid: '<?php echo esc_html($choices[0]->id); ?>',
											packageCalendar: '0',
											included_uids: '<?php echo json_encode($included_uids[0]); ?>',
											package: '<?php echo esc_html($item->uid); ?>',
											package_index: 0,
											security: '<?php echo wp_create_nonce('rezgo-nonce'); ?>',
										},
										...
										success: function(data) {
											...
											<?php if (count(array_intersect($select_type_combination[0], $calendar_selects)) > 0) { ?>
												...
												setTimeout(() => {
													...
													$('.responsive-calendar-0 a[data-day="'+start_day+'"][data-month="'+start_month+'"][data-year="'+start_year+'"]').parent().addClass('select');
													this_date = start_year+'-'+ addLeadingZero(start_month) +'-'+ addLeadingZero(start_day);
													$('#date_input_0').val(this_date);
												}, 100);
							...
							success: function(data){
								...
								<?php if (!REZGO_LITE_CONTAINER){ ?>
									...
									if (response == null) {
										...
										show_package_err(response.message);
									}
