<?php 
	...
	if (REZGO_WORDPRESS) {
		...
		$payment_page = $_SERVER['SCRIPT_NAME'] == '/page_book.php' ? 1 : 0; 
	}
?>

<div id="rezgo-tips" class="<?php echo $payment_page ? 'payment-page' : ''; ?>" style="display:none;">
	<div class="flex-row">
	...
	jQuery(function($){	
		...
		function calculateTips(tips, overall_total) {
			...
			$('#rezgo-fixed-cart #total_value, #rezgo-fixed-cart #summary_total_value, #rezgo-fixed-cart-pr #total_value, .rezgo-total-payable #total_value').attr('rel', total_w_tips);
			$('#rezgo-fixed-cart #total_value, #rezgo-fixed-cart #summary_total_value, #rezgo-fixed-cart-pr #total_value, .rezgo-total-payable #total_value, #complete_booking_total').html(`${currency + total_w_tips.formatMoney()}`);
		...
		resetTips = function(overall_total) {
			...
			$('#rezgo-fixed-cart #total_value, #rezgo-fixed-cart #summary_total_value, #rezgo-fixed-cart-pr #total_value, .rezgo-total-payable #total_value').attr('rel', cart_total);
			$('#rezgo-fixed-cart #total_value, #rezgo-fixed-cart #summary_total_value, #rezgo-fixed-cart-pr #total_value, .rezgo-total-payable #total_value, #complete_booking_total').html(`${currency + cart_total.formatMoney()}`);
		...
		$('#cancel-custom-amt').click(function(){
			...
			// reset rezgo-custom-tip error state
			$('#rezgo-custom-tip').removeClass('error');
