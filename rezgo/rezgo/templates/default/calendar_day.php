<?php
	...
	$options = $site->getTours('t=com&q='.$com.$date_request.'&a=subtract_cart_availability&file=calendar_day');
...
<?php } ?>
	...
	<div class="panel-group" id="rezgo-select-option-<?php echo esc_attr($option_num); ?>">
		...
		foreach($options as $option) { ?>
			...
			<?php 
				$dynamic_start_time = ((string)$option->time_format == 'dynamic') ? 1 : 0;
				$hide_av = $site->exists($option->date->hide_availability) ? 1 : 0;
				...
				if ((($_REQUEST['type'] == 'calendar' || $_REQUEST['type'] == 'single') && (string) $option->date['value'] != 'open') 
					...
					<div class="panel panel-default<?php echo esc_attr($panel_unclass.$block_unclass); ?>">
						...
						check_<?php echo esc_html($option_num.'_'.$sub_option); ?> = function() {
							...
							var book_time_min_guests_<?php echo esc_html($option_num.'_'.$sub_option); ?> = $('#add_cart_<?php echo esc_html($option_num.'_'.$sub_option); ?>').data('min-guests');
							...
							if (book_time_min_guests_<?php echo esc_html($option_num.'_'.$sub_option); ?> != 0) {
								if(count_<?php echo esc_html($option_num.'_'.$sub_option); ?> < book_time_min_guests_<?php echo $option_num.'_'.$sub_option; ?>) {
									err = book_time_min_guests_<?php echo esc_html($option_num.'_'.$sub_option); ?> + ' minimum required to book.';
								}
							}
						...
						<div class="panel-body">
							...
							?>
								...
								<form class="rezgo-order-form" method="post" id="checkout_<?php echo esc_attr($option_num.'_'.$sub_option); ?>" <?php echo esc_attr($form_target); ?>>
									...
									<?php if (!$hide_av && !$dynamic_start_time) { ?>
									...
									<?php if ($dynamic_start_time) { ?>
											...
											<script>
												jQuery(function($){
													...
													$('.time_select_<?php echo $option->uid; ?>').click(function(){
														...
														let hide_av = <?php echo $hide_av ? 1 : 0; ?>;
														...
														// grab price tier changes
														$.ajax({
															url: '<?php echo admin_url('admin-ajax.php'); ?>' + '?action=rezgo',
															type: 'POST',
															data: { 
																method: 'book_time_ajax',
																rezgoAction: 'book_time',
																uid: '<?php echo $option->uid; ?>',
																date: '<?php echo $date; ?>',
																book_time: book_time,
															},
															success: function(response) {
																...
																if (result) {
																	...
																	$('#add_cart_<?php echo esc_html($option_num.'_'.$sub_option); ?>').data('min-guests', result.min_guests);

																	Object.entries(result).forEach(function([key, val]){
																		
																		if (key != 'min_guests') {
																			$('#'+ val.name +'_<?php echo esc_attr($option_num.'_'.$sub_option); ?>_price').html(val.formatted_price);
																			$('#'+ val.name +'_<?php echo esc_attr($option_num.'_'.$sub_option); ?>_price').data('price', val.price);
																		}
																	});
																}
															},
														...
														if( price_av_obj_<?php echo esc_html($option_num.'_'.$sub_option); ?>.hasOwnProperty(book_time) ) {
															...
															if (price_tier_av.price) {
																Object.keys(price_tier_av.price).forEach(key => {
																	let price_id = parseInt(price_tier_av.price[key].id) - 1;
																	let price_tier = price_tier_av.price[key];
																	let availability = parseInt(price_tier.av);
																	let existing_val = $('#' + price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>').val();
																	let price_tier_div = $('#price_tier_div_' + price_tier_array[price_id] + '_<?php echo $option_num.'_'.$sub_option; ?>');
																	let err_state = $('#max_pax_error_'+ price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>');
																	let plus_sign = $('#increase_'+ price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>');
																	let minus_sign = $('#decrease_'+ price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>');
																	let amt_left = $('#price_tier_amount_left_' + price_tier_array[price_id] + '_<?php echo $option_num.'_'.$sub_option; ?>');
																	let price_input = $('#' + price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>');
																	
																	price_tier_div.show();
																	err_state.hide();

																	// toggle change event listeners
																	$('#'+ price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>').off('change');
																	$('#'+ price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>').change(function(){
																		...
																		if ($(this).val() < 0){
																			...
																			document.getElementById(price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>').value = parseInt($(this).val());
																		}
																		...
																		if ($(this).val() > availability){
																			...
																			minus_sign.removeClass('not-allowed');
																	...
																	$('#increase_'+ price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>').click(function(){
																		let value = $('#'+ price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>').val();
																		let plus_sign = $(this);
																	...
																	$('#decrease_'+ price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>').click(function(){
																		let value = $('#'+ price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>').val();
																		let plus_sign = $('#increase_'+ price_tier_array[price_id] +'_<?php echo esc_html($option_num.'_'.$sub_option); ?>');
																		let minus_sign = $(this);
																	...
																	if (existing_val > availability) {
																		...
																		// reset input 
																		price_input.val('');
																		minus_sign.addClass('not-allowed');
																		plus_sign.removeClass('not-allowed');
																	...
																	if (!hide_av) {
																		if (availability < 10) {
																			amt_left.removeClass('d-none');
																			amt_left.html('<span class="edit-pax-max">Only ' + availability + ' Left</span>');

																			if (availability == 0) {
																				price_tier_div.hide();
																				price_input.val('');
																				amt_left.html('<span class="edit-pax-max">Not Available</span>');
																			}
																		} else {
																			amt_left.addClass('d-none');
																		}
																	} else {
																		if (availability == 0) {
																			price_tier_div.hide();
										...
										<input type="hidden"  class="hidden-inputs" name="add[0][uid]" value="<?php echo esc_html($option->uid); ?>">
										<input type="hidden"  class="hidden-inputs" name="add[0][date]" value="<?php echo esc_html($_REQUEST['date']); ?>">
										<?php if ($dynamic_start_time) { ?>
											<input id="book_time_<?php echo esc_attr($option_num.'_'.$sub_option); ?>" type="hidden"  class="hidden-inputs" name="add[0][book_time]" value="">
										<?php } ?>
									...
									<div class="row"> 
										<div class="col-12 rezgo-order-fields">
											...
											<?php foreach( $prices as $price ) { ?>
												...
												<?php // check if total for all price points equals to zero
													...
													$price_description = $price->description ?? 0;
												?>
												...
												<div class="edit-pax-wrp rezgo-option-<?php echo esc_attr($option->uid); ?>-price-tiers rezgo-option-<?php echo esc_attr($option->uid); ?>-price-tier-<?php echo esc_attr($price->id); ?>" style="--animation-order: <?php echo esc_attr($animation_order); ?>;">
												<div class="edit-pax-label-container">
													...
													<?php if ($price_description) { ?> 
														<span class="rezgo-price-description">
													...
													?>
													<div id="price_tier_amount_left_<?php echo $price->name; ?>_<?php echo $option_num.'_'.$sub_option; ?>">
														<?php 
															if (!$hide_av) {
																if(!$dynamic_start_time && isset($price->max) && $price->max < 10 && (int)$price->max != $option->date->availability) {
																...
																// show 'Not Available' even when hide availability setting is turned on
																if(!$dynamic_start_time && isset($price->max) && $price->max == 0) {
																	echo '<span class="edit-pax-max">
																			  <span>Not Available <div class="space-6"></div></span>
																		  </span>
																	';
																}
															}
												...
												<div id="price_tier_div_<?php echo $price->name; ?>_<?php echo $option_num.'_'.$sub_option; ?>" class="pax-price-container <?php echo (!$dynamic_start_time && isset($price->max) && $price->max == 0) ? 'd-none' : ''; ?>">				
														...
														<div class="edit-pax-wrp" style="--animation-order: <?php echo esc_attr($animation_order); ?>;">
															<div class="edit-pax-label-container">
																...
																<?php echo esc_html($bundle->name); ?> <br> <span class="rezgo-bundle-makeup">- includes <?php echo esc_html($bundle->makeup); ?></span>
																	...
																	<div id="price_tier_amount_left_<?php echo $price->id; ?>_<?php echo $option_num.'_'.$sub_option; ?>" class="<?php echo $dynamic_start_time ? 'd-none' : ''; ?>">
																	<?php 
										...
										<div class="col-12 float-end">
											...
											<button id="add_cart_<?php echo esc_html($option_num.'_'.$sub_option); ?>" type="submit" class="btn btn-block rezgo-btn-book rezgo-btn-add" value="addToCart" onclick="return check_<?php echo esc_js($option_num.'_'.$sub_option); ?>();"><span>Add To Order</span></button>
										</div>
