					...
					<?php } else { ?>
						...
						<div class="flex-container confirm-page-container">
							...
							<?php
								...
								$billing_address = (int)$company->billing_address;
								$billing_country_required = in_array($gateway_id, ['tmt','square']);
															...
															<?php 
																...
																<?php if($item->{$price->name.'_num'}) { ?>
																	...
																	<?php if(!$site->exists($item->deposit)) { 
                                                                    	$item_total[$cart_package_uid][$index] = (float)$item->overall_total;
																	} else {
                                                ...
                                                <?php foreach ($site->getPaymentMethods() as $pay) { ?>
                                                    ...
                                                    <?php if ($pay['name'] == 'Credit Cards') { ?>
                                                        ...
                                                        <div id="payment_cards" class="payment_method_container" style="display:none;">
															...
															<?php if (!$billing_address && $billing_country_required) { ?>
																<div class="form-group country-required-form-group">
												...
												<div class="form-group">
													<div class="rezgo-form-row">
														<div class="col-12 rezgo-form-input">
															...
															<select class="form-select" id="tour_stateprov" style="display:<?php echo (($companyCountry != 'ca' && $companyCountry != 'us' && $companyCountry != 'au') ? 'none' : ''); ?>;"></select>
...
jQuery(function($){
	...
	function show_response() {
        ...
        if(response.indexOf('STOP::') != -1) {  // debug handling
            ...
            if (response.status == 1) {
                 split[1] = '<div class="clearfix">&nbsp;</div>BOOKING COMPLETED WITHOUT ERRORS<div class="clearfix">&nbsp;</div><button type="button" class="btn btn-default" onclick="window.top.location.replace(\'<?php echo esc_js($site->base); ?>/complete/thank-you/' + response.txid + '\');">Continue to Receipt</button><div class="clearfix">&nbsp;</div>';
            ...
            if (response.status == '2') {
                ...
                if (response.txid) {
                    ...
                    <?php echo LOCATION_WINDOW; ?>.location.replace("<?php echo esc_html($site->base); ?>/complete/thank-you/" + response.txid);
							...
							if(!result) {
								...
								const tmtPaymentModal = new parent.tmtPaymentModalSdk({
									...
									data: {
										...
										// Lead Traveller
										firstname: '<?php echo $lead_passenger['first_name'] ?: 'First'; ?>',
										surname: '<?php echo $lead_passenger['last_name'] ?: 'Last'; ?>',
										email: '<?php echo $lead_passenger['email'] ?: 'email@address.com'; ?>',
										country: '<?php echo strtoupper($company->country); ?>',
										// Payment details
										payee_name:'<?php echo $lead_passenger['first_name'] ?: 'First'; ?>',
										payee_email: '<?php echo $lead_passenger['email'] ?: 'email@address.com'; ?>',
	...
	toggleCard = function() {
		...
		<?php if (!$billing_address && $billing_country_required) { ?>
			$('#tour_country').attr('disabled', true);
		<?php } ?>
		...
		if ($('input[name=payment_method]:checked').val() == 'Credit Cards') {
			...
			<?php if (!$billing_address && $billing_country_required) { ?>
				$('#tour_country').attr('disabled', false);
			<?php } ?>
