<?php
	...
	$bookings = $site->getBookings('q='.$trans_num);
?>
				...
				<div class="row rezgo-waiver-count div-box-shadow">
					<?php 
						...
						foreach ($site->getBookingPassengers() as $passenger) { 
							$waiver_pax[(int)$passenger->id] = (object)[
								'sign_total' => '',
								'signed' => '',
							];

							if ($passenger->id == $pax_id) $pax_data = $passenger;
							$booking_passengers_waivers[] = $passenger;

							$waiver_pax[(int)$passenger->id]->sign_total = (int)$passenger->sign_total;
							$waiver_pax[(int)$passenger->id]->signed = $passenger->signed_array->signed;
						...
						if ($pax_signed != $pax_count) { // hide if all waivers signed
							echo '<div class="text-center">';
								echo '<div style="white-space:nowrap;"><i class="far fa-exclamation-circle fa-lg"></i>&nbsp; <span><span id="pax-signed">' . $pax_signed . '</span> of ' . $pax_count . ' waivers signed</span></div>';
							echo '</div>';
							echo '<span class="btn-check"></span>';
							echo '<a href="'.$site->base.'/waiver/'.$site->waiver_encode($booking->trans_num).'" class="btn btn-lg rezgo-waiver-btn"><span><i class="far fa-pen"></i>&nbsp;&nbsp;Sign waivers</span></a>';
						} else {
							echo '<div class="text-center">';
								echo '<div style="white-space:nowrap;"><i class="far fa-check-circle fa-lg"></i>&nbsp; <span><span id="pax-signed">All waivers have been signed</span></span></div>';
							echo '</div>';
						}
					?>
				</div>
							...
							<div class="booking-receipt-forms-container <?php echo esc_attr($item->group); ?>_pax_info">
								...
								<div class="flex-table">
									<?php foreach ($site->getBookingPassengers() as $passenger) { ?>
										...
										<div class="flex-table-info d-flex flex-column">
											<?php if( $booking->waiver == '2' && !$no_pax_waivers) { 
													if ($waiver_pax[(int)$passenger->id]->sign_total > 1 ) { 

														foreach ($booking_passengers_waivers as $booking_pax) {

															if ($booking_pax->id == (int)$passenger->id) {

																for ($i=0; $i < $waiver_pax[(int)$passenger->id]->sign_total ; $i++) { 

																	$pax_uid = $booking_pax->id.'_'.($i+1);
																	$pax_signed = in_array($pax_uid, (array)$waiver_pax[(int)$passenger->id]->signed) ?? 0; ?>
																	
																	<?php if ( !$pax_signed ) { ?>

																		<?php 
																			if (REZGO_WORDPRESS) { 
																				$waiver_link = esc_js($domain).'.rezgo.com/waiver/'.esc_html($site->waiver_encode($booking->trans_num.'-'.$passenger->id.'-'.($i+1)));
																			} else {
																				$waiver_link = $site->base.'/waiver/'.$site->waiver_encode($booking->trans_num.'-'.$passenger->id.'-'.($i+1));
																			}
																		?>

																		<span class="btn-check"></span>
																		<button class="btn rezgo-btn-default btn-sm rezgo-waiver-sign" type="button" data-paxid="<?php echo $passenger->id; ?>" id="rezgo-sign-<?php echo $passenger->id; ?>"  onclick="<?php echo LOCATION_WINDOW; ?>.location.href='<?php echo $waiver_link; ?>'">
																			<span>
																				<span id="rezgo-sign-txt-<?php echo $passenger->id; ?>">
																					<i class="far far fa-pen"></i>&nbsp; Sign Waiver <?php echo $i+1; ?>
																				</span>
																			</span>
																		</button>
																	...
																	<?php } else { ?> 
																		...
																		<span id="rezgo-signed-<?php echo esc_attr($passenger->id); ?>" <?php echo (($pax_signed) ? '' : 'style="display:none;"'); ?>>
																		
																			<span class="rezgo-signed"> 
																				<i class="fa fa-check-circle"></i>
																				&nbsp; Waiver <?php echo $i+1; ?> Signed
																			</span>
																		</span>
													...
													} elseif ($waiver_pax[(int)$passenger->id]->sign_total == 1 ) { ?>
														...
														<span class="btn-check"></span>
														<button class="btn rezgo-btn-default btn-sm rezgo-waiver-sign" type="button" data-paxid="<?php echo $passenger->id; ?>" id="rezgo-sign-<?php echo $passenger->id; ?>" <?php echo (($passenger->signed) ? ' style="display:none;"' : ''); ?> onclick="<?php echo LOCATION_WINDOW; ?>.location.href='<?php echo $waiver_link; ?>'">
															<span>
																<span id="rezgo-sign-txt-<?php echo $passenger->id; ?>">
													...
													<?php } // if ($waiver_pax[(int)$passenger->id]->sign_total > 1 ) ?>
