<div class="row rezgo-form-group rezgo-additional-info guest-forms-container">
		<div class="select_guest_type_container">
			<select name="select_guest_type" id="add_new_guest_type" data-placeholder="Select a Guest Type" class="chosen-select chosen-guest-select form-control rezgo-custom-select required">
				...
				<?php 
					$p = 0;
					foreach($booking->price_data->price as $price) { 
						$show_price = 0;
						if ($price->vat){
							foreach ($vat_values[(int)$price->id]->line as $vat) {
								if ((float)$vat->amount > 0) {
									$vat_value[(int)$price->id] += (float)$vat->amount;
								}
							}
							$show_price = $vat_value[(int)$price->id] > 0  ? ($price->retail + $vat_value[(int)$price->id]) : $price->retail;
						} else {
							$show_price = $price->retail;
						}
						$booking_prices[] = $show_price; 
					} 
