<div class="wrap" id="rezgo_settings">
	<img src="<?php echo esc_url(rezgo_embed_settings_image('rezgo-logo.svg')); ?>" id="rezgo-logo" />
  ...
  <div id="rezgo_notices">
	...
	<form method="post" action="">
		<?php echo esc_html(settings_fields('rezgo_options')); ?>
		...
		<div class="field_frame">
			<fieldset>
				...
				<dl>
					...
					<dd>
						<input type="text" name="rezgo_cid" id="rezgo_cid" size="10" value="<?php echo esc_attr($rezgoCID) ?>" onkeyup="check_values()" />
					</dd>

					<dt>Rezgo API Key:</dt>

					<dd>
						<input type="text" name="rezgo_api_key" id="rezgo_api_key" size="20" value="<?php echo esc_attr($rezgoApiKey) ?>" onkeyup="check_values()" />
					</dd>
					...
					<div class="api_box" id="check_values">
						<?php if ($rezgoCID && $rezgoApiKey) { ?>
							<?php if (!empty($companyName)) { ?>
								...
								<span class="ajax_success_message"><?php echo esc_html($companyName) ?></span>
								<a href="<?php echo esc_url($companyDomain) ?>.rezgo.com" class="ajax_success_url" target="_blank">
									<span><?php echo esc_html($companyDomain) ?>.rezgo.com</span>
								</a>
							<?php } else { ?>
								<span class="ajax_error">XML API Error</span><br>
								<span class="ajax_error_message"><?php echo esc_html($companyError) ?></span>
							<?php } ?>
						<?php } else { ?>
		...
		<div class="field_frame">
			<fieldset>
				...
				<dl>
					...
					<dd>
						<input type="text" name="rezgo_google_key" size="50" value="<?php echo esc_attr(get_option('rezgo_google_key')) ?>" />
					</dd>
				</dl>
			</fieldset>
		</div>
		...
		<div class="field_frame">
			<fieldset>
				...
				<dl>
					...
					<dd>
						<input type="text" name="rezgo_captcha_pub_key" size="50" value="<?php echo esc_attr(get_option('rezgo_captcha_pub_key')) ?>" />
					</dd>
					<dt>reCAPTCHA v3 Secret Key:</dt>
					<dd>
						<input type="text" name="rezgo_captcha_priv_key" size="50" value="<?php echo esc_attr(get_option('rezgo_captcha_priv_key')) ?>"/>
					</dd>
				</dl>
			</fieldset>
		</div>
		...
		<div class="field_frame">
			<fieldset>
				...
				// OPTION overwrite_page_title
				// if overwrite page title is not yet set to anything, leave it as unchecked
				if (get_option('rezgo_optout_overwrite_page_title') === '' || get_option('rezgo_optout_overwrite_page_title') === false) {
					$overwrite_page_title = '';
				} else {
					$overwrite_page_title = (get_option('rezgo_optout_overwrite_page_title')) ? 'checked' : '';
				}
				// OPTION rezgo_legacy_template_use
				// if overwrite page title is not yet set to anything, leave it as unchecked
				if (get_option('rezgo_legacy_template_use') === '' || get_option('rezgo_legacy_template_use') === false) {
					$rezgo_legacy_template_use = '';
				} else {
					$rezgo_legacy_template_use = (get_option('rezgo_legacy_template_use')) ? 'checked' : '';
				}
				...
				<dl class="dl_general_settings">
					...
					<div class="rezgo-general-settings-wrp" id="rezgo-overwrite-title-wrp">
						<dt class=note>Opt out of overwriting WordPress page titles? Checking this option may solve broken page redirects.</dt>
						<dt>Opt out of overwriting page titles:</dt>
						<dd><input type="checkbox" class="rezgo-optout-override-title-checkbox" name="rezgo_optout_overwrite_page_title" value="1" <?php echo esc_attr($overwrite_page_title); ?> /></dd>
						<div class="clear"></div>
					</div>
					...
					<div class="rezgo-general-settings-wrp" id="rezgo-results-limit-wrp">
						...
						<dd>
							<input type="text" name="rezgo_result_num" size="5" value="<?php echo esc_attr($results_num) ?>" />
						</dd>
						<div class="clear"></div>
					</div>

					<!-- LEGACY TEMPLATE SWITCH -->
					<div class="rezgo-general-settings-wrp" id="rezgo-leg-use-wrp">
						<dt class=note>If you wish to use revert to the legacy template, check the option below. The legacy template might be missing certain features.</dt>
						<dt>Revert to legacy template:</dt>
						<dd><input type="checkbox" id="rzg-leg-use-checkbox" name="rezgo_legacy_template_use" value="1" <?php echo esc_attr($rezgo_legacy_template_use); ?> /></dd>
						<div class="clear"></div>
						<div id="rzg-leg-tmp-msg"></div>
					</div>
					...
					<div class="rezgo-general-settings-wrp" id="rezgo-cus-tmp-use-wrp">
						...
						<dd><input type="checkbox" id="rzg-cus-tmp-checkbox" name="rezgo_custom_template_use" class="rezgo-legacy-template-checkbox" value="1" <?php echo ($rezgo_custom_template_use) ? 'checked' : ''; ?> /></dd>
						<div class="clear"></div>
							...
							<dd>
								<select name="rezgo_template" id="template_select">
									<?php foreach(rezgo_get_tmp() as $v) { ?>
										<option value='<?php echo esc_attr($v); ?>' <?php echo (get_option('rezgo_template') == $v) ? 'selected' : ''; ?>><?php echo esc_html($v); ?></option>
									<?php } ?>
								</select>
							</dd>
						</div>
					...
					<div class="rezgo-general-settings-wrp" id="rezgo-forward-secure-wrp">
						...
						<dd><input type="checkbox" class="rezgo-forward-secure-checkbox" name="rezgo_forward_secure" value="1" <?php echo esc_attr($forward_secure_checked); ?> /></dd>
						<div class="clear"></div>
					</div>
...
jQuery(document).ready(function($){
	var cid_value = '<?php echo esc_html($rezgoCID); ?>';
	var key_value = '<?php echo esc_html($rezgoApiKey); ?>';
	var tmp = '<?php echo esc_html(get_option('rezgo_template')); ?>';
	...
	function check_values() {
		...
		if (cid_value != cid || key_value != key) {
			...
			if (cid && key) {
				$('#check_values').html('<img src="<?php echo esc_url(rezgo_embed_settings_image('load.gif')) ?>">');
	...
	$(document).on('click','#rzg-cus-tmp-checkbox',function(e){
		if ($(this).prop('checked')) {
			$('#rzg-leg-use-checkbox').prop('checked', false);
			use_tmp(1);
		} else {
			use_tmp(0);
		}
	...
	$(document).on('click','#rzg-leg-use-checkbox',function(e){
		if ($(this).prop('checked')) {
			$('#rzg-cus-tmp-checkbox').prop('checked', false);
			use_tmp(0);
		}
