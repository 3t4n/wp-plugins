...
$site = new RezgoSite();
$time_format = sanitize_text_field($_REQUEST['time_format'] . ' hours');
...
if (REZGO_WORDPRESS) $site->setTimeZone();
	...
	if ($rezgoAction == 'formatCurrency'){
		...
		$currency_decimals = sanitize_text_field($_REQUEST['currency_decimals'] ?? '');

		$num = sanitize_text_field($_REQUEST['amount']);
		if (strpos((string)$num, "-") !== false) {
			$num = str_replace("-", "", $num);
			$pre = '- ';
		}
		echo str_replace(" ", "&nbsp;", $pre.$currency_symbol.number_format((float)$num, (int)$currency_decimals, '.', (string)$currency_separator));
	}
