				...
				<script>
					jQuery(function($) {
						$('#rezgo-promo-form__mobile').submit( function(e){
							...
							let promo_value = document.querySelector('#rezgo-promo-code__mobile') ? document.querySelector('#rezgo-promo-code__mobile').value : '';
...
jQuery(document).ready(function($) {
	$('.rezgo-pax-edit-btn').each(function() {
		...
		$('.rezgo-btn-remove').click(function() {
			...
			Object.entries(pixel_package_details).forEach(entry => {
				const [key, item] = entry;
				if (key.includes(cart_package_uid)){
