...
class PayboxEncrypt {
    ...
    public function generateKey() {
        ...
        if (file_exists(PAYBOX_KEY_PATH_WP))
            unlink(PAYBOX_KEY_PATH_WP);
        $key = bin2hex($key);
        $td = mcrypt_module_open(MCRYPT_RIJNDAEL_128, '', MCRYPT_MODE_CBC, '');
        $size = mcrypt_enc_get_iv_size($td);
        $iv = mcrypt_create_iv($size, MCRYPT_RAND);
        $iv = bin2hex($iv);
        file_put_contents(PAYBOX_KEY_PATH_WP, "<?php" . $key . $iv . "?>");
        // Init vector
    }
    ...
    private function _getKey() {
        ...
        if (!file_exists(PAYBOX_KEY_PATH_WP)) {
            ...
            $key = file_get_contents(PAYBOX_KEY_PATH_WP);
            $key = substr($key, 5, 32);
            return $key;
        }
    }
    ...
    private function _getIv() {
        ...
        if (!file_exists(PAYBOX_KEY_PATH_WP)) {
            ...
            $iv = file_get_contents(PAYBOX_KEY_PATH_WP);
            $iv = substr($iv, 37, 16);
            return $iv;
        }
    }
