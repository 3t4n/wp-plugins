...
class Screen_Sharing_System {
    ...
    public function register_settings() {
		...
		register_setting('screen_sharing_settings', 'turn_server');
        register_setting('screen_sharing_settings', 'turn_username');
        register_setting('screen_sharing_settings', 'turn_password');
        ...
        add_settings_field(
            'turn_username',
            'TURN Username',
            array($this, 'turn_username_field'),
            'screen-sharing-settings',
            'screen_sharing_section'
        );
        ...
        add_settings_field(
            'turn_password',
            'TURN Password',
            array($this, 'turn_password_field'),
            'screen-sharing-settings',
            'screen_sharing_section'
        );
    }
	...
	public function turn_server_field() {
		$turn_server = get_option('turn_server', 'turn:turn.deter-mi.net');
		echo '<input type="text" id="turn_server" name="turn_server" value="' . esc_attr($turn_server) . '" />';
	}

	public function turn_username_field() {
		$turn_username = get_option('turn_username', 'brucewayne');
		echo '<input type="text" id="turn_username" name="turn_username" value="' . esc_attr($turn_username) . '" />';
	}

	public function turn_password_field() {
		$turn_password = get_option('turn_password', 'secret');
		echo '<input type="password" id="turn_password" name="turn_password" value="' . esc_attr($turn_password) . '" />';
	}
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        /*wp_localize_script('sss-admin-js', 'SSS', array(
            ...
            'ice_server' => get_option('ice_server', 'stun:stun.l.google.com:19302')
                ...
                'turnServerUrl' => get_option('turn_server', 'turn:turn.deter-mi.net'),
                'turnUsername' => get_option('turn_username', 'brucewayne'),
                'turnPassword' => get_option('turn_password', 'secret')
            ));
		}
    }
