              ...
              <?php
                ...
                if(count($egRecord) > 0)
                {
                    ...
                    foreach ($egRecord as $record)
                    {
						...
						$inc_ctransamount = str_replace("-", "", number_format(esc_html($record->inc_ctransamount), 2, '.', ''));
						
						if( !empty($record->inc_ctransaction) )
						{
							$inc_ctransaction = $record->inc_ctransaction;
						}
						else
						{
							$inc_ctransaction = 'Subscription Payment ($'.$inc_ctransamount.')';
						}
                ?>
                  <tr class="<?php if ($i&1) { echo'alternate'; } else { echo ''; }?>">
                        ...
                        <td><?php echo esc_html($inc_ctransaction); ?></td>
                        <td><?php echo esc_html($record->inc_ctranspaymentmethod); ?></td>
                        <td><?php echo $append; ?>$<?php echo $inc_ctransamount; ?> </td>
...
?>
				<tr>
                    <td style="border-top: 1px solid #e1e1e1;" colspan="9" align="right"><strong>Totals</strong></td>
