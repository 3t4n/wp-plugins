...
function apm_encrypt_email($email, $secret_key) 
{
    $encryption_key = hash('sha256', $secret_key);
...
//--> eShowcase CRM
function fetch_eShowcase_token_from_master($user_email_id) 
{
    $postdata = array(
		'mode' => 'get_eshowcase_crm_data',
    ...
    if (isset($response['code']) && $response['code'] === 'no_data') {
        return 'No data found for the given affiliate email.';
    }
	
	$eshowCRMResponse = $response['body'];

    //--> Return the fetched data
    return $eshowCRMResponse;
}


function check_eshowCRM_active_or_not($user_email_id) 
{
    $postdata = array(
		'mode' => 'is_eshowcase_crm_active',
		'user_email_id' => $user_email_id,
		'version' => $GLOBALS['APM_SUBSCRIPTION_VER'],
	);
...
function check_eshowCRM_invalid_grant($user_email_id) 
{
	$postdata = array(
		'mode' => 'is_eshowcase_crm_invalid_grant',
		'user_email_id' => $user_email_id,
		'version' => $GLOBALS['APM_SUBSCRIPTION_VER'],
	);
	...
	//--> Check if the data was returned successfully
    if (isset($response['body']) && $response['body'] === 'This refresh token is invalid') {
        return 'This refresh token is invalid';
    }
