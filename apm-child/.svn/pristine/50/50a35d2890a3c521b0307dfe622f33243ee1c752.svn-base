...
function apm_include_funnel_templates($template) 
{
	...
	if( in_array($apm_curr_page_id, $funnel_pageid_mapping_data) )
	{
		...
		if(isset($funnel_slug_mapping_data[$apm_funnel_id]))
		{
			...
			$apm_funnel_status = isset($funnel_status_mapping_data[$apm_funnel_id]) ? $funnel_status_mapping_data[$apm_funnel_id] : null;
...
//--> Warning Regarding the Expired Refresh Token
function apm_display_message_expired_refresh_token() 
{
	global $wpdb;

	$egSqlCrediantial = "SELECT * FROM `".APM_OPTIONS_TABLE."` WHERE 1 AND email_service_provider = 'eShowcaseCRM'";

	$record = array();

	$recordCrediantial = $wpdb->get_row($egSqlCrediantial, ARRAY_A);
	
	if(is_array($recordCrediantial) && count($recordCrediantial) > 0)
	{
		$error_response = check_eshowCRM_invalid_grant($recordCrediantial['user_email_id']);
		
		if($error_response != '')
		{
			$admin_url = admin_url('admin.php?page=apm_list_aweber');
			
			echo '<div class="notice notice-error apm-refresh-admin-message">
				<h3>APM Child - eShowcaseCRM</h3>
				<p>Your <strong>eShowcaseCRM Refresh Token</strong> has been invalid. Please reauthenticate the eShowcaseCRM from <a href="' . esc_url($admin_url) . '">here</a></p>
				<a href="' . esc_url($admin_url) . '" class="e-button e-button">Click here to reauthenticate it</a>
			  </div>';
		}
	}
}
add_action('admin_notices', 'apm_display_message_expired_refresh_token');


//--> Warning Regarding the Expired Refresh Token CSS
function apm_admin_message_styles_expired_refresh_token() {
    echo '<style>
        /* Style the custom error message */
        .apm-refresh-admin-message {
            margin-top: 20px;
            padding: 15px;
            background-color: #f8d7da; /* Red background for error */
            border-left: 4px solid #d9534f; /* Red border for error */
            font-size: 16px;
        }

        /* Style the link inside the message */
        .apm-refresh-admin-message a {
            color: #d9534f;
            font-weight: bold;
        }

        /* Optional: Add some padding to the admin headline area if needed */
        #wpbody-content .wrap {
            margin-top: 30px; /* Ensure space below the headline */
        }
		
		.apm-refresh-admin-message h3{
			font-size:16px;
			margin: 0;
		}
		
		.apm-refresh-admin-message .e-button{
...
add_action('admin_head', 'apm_admin_message_styles_expired_refresh_token');
