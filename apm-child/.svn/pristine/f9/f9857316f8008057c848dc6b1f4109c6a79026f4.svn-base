...
if($recordCrediantial['email_service_provider'] == "eShowcaseCRM")
{
	$mapping_type = "eShowcaseCRM";
}
...
?>
    <div class="updated fade">
        <p><strong>Your website has been successfully authorized by the eShowcase CRM.</strong></p>
    </div>
<?php
}
...
<?php			
		...
		elseif($recordCrediantial['email_service_provider'] == "eShowcaseCRM")
		{
			
			$base_url = site_url().'/wp-admin/admin.php';
			
			$user_email_id = $recordCrediantial['user_email_id'];
			
			//--> Query parameters
			$params = [
				'page' => 'apm_list_aweber',
				'uid' => base64_encode( $user_email_id ),
				'action' => 'apm_get_highlevel_token',
			];
			
			//--> Build the query string and encode the URL
			$query_string = http_build_query($params);
			$encoded_url = htmlspecialchars($base_url . '?' . $query_string, ENT_QUOTES, 'UTF-8');
			
			
			//--> Call REST API to get the eShowcase CRM Tokens
			
			if(check_eshowCRM_active_or_not($user_email_id) == "true")
			{
				echo fetch_eShowcase_token_from_master($user_email_id);
?>
                
				<a class="button button-primary" href="<?php echo APM_MASTER_PLUGIN_URL?>oauth/?actioncall=oauth_callback&redirect=<?php echo $encoded_url; ?>">Re-authorize with eShowcase CRM</a>
<?php
			}
			else
			{
?>
                <a class="button button-primary" href="<?php echo APM_MASTER_PLUGIN_URL?>oauth/?actioncall=oauth_callback&redirect=<?php echo $encoded_url; ?>">Authorize with eShowcase CRM</a>
                <p>You'll be taken to eShowcase CRM to authorize APM Child and generate access keys for this site.</p>
                <p>If you receive authorization errors, please log in to <a href="">eShowcase CRM</a> before attempting the connection.</p>
<?php
			}
?>
<?php			
