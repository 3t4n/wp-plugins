...
if (isset($_POST['hdnPageAction']) && $_POST['hdnPageAction'] == 'Edit')
{
	...
	if(is_array( $form['apm_funnel_term_id'] ) && count($form['apm_funnel_term_id']) > 0)
	{
		...
		else 
		{
			...
			//--> Get the current saved data from the options table.
			$existing_funnel_prospect_tags = get_option('apm_funnel_prospect_tags');
			
			//--> Decode the JSON data to an array.
			$existing_funnel_prospect_tags = $existing_funnel_prospect_tags ? json_decode($existing_funnel_prospect_tags, true) : [];
			...
			foreach ($form['apm_funnel_prospect_tags'] as $key => $funnel_prospect_tags) 
			{
				$existing_funnel_prospect_tags[$key] = $funnel_prospect_tags;
			}
			...
			$funnel_prospect_tags = json_encode($existing_funnel_prospect_tags);
