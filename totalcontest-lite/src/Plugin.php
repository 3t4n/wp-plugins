...
class Plugin extends \TotalContestVendors\TotalCore\Foundation\Plugin {
	...
	public function registerProviders() {
		...
		$this->container->share( 'validators.vote.unique', function () {
			return function ( Field $field, $args = [] ) {
				$value         = $field->getValue();
				$database      = TotalContest( 'database' );
				$databaseTable = TotalContest()->env( 'db.tables.log' );
				$search        = sprintf( '%%"%s":"%s"%%', $field->getName(), $field->getValue() );
				$sql           = "SELECT count(id) FROM {$databaseTable} WHERE contest_id = %d AND submission_id = %d and action = %s AND details LIKE %s";
				$query         = $database->prepare( $sql, $args['contestId'], $args['submissionId'], 'vote', $search );

				if ( ! empty( $value ) && $database->get_var( $query ) ):
					return esc_html__( '{{label}} has been used before.', 'totallcontest' );
				endif;
		...
		$this->container->add( 'upgrade-to-pro', function () {
			$url     = TotalContest()->env( 'links.upgrade-to-pro' );
			$tooltip = esc_html__( 'This feature is available in Pro version.', 'totalcontest' );
			?>
            <a href="<?php esc_attr_e( $url ); ?>" target="_blank" class="totalcontest-pro-badge"
               tooltip="<?php esc_attr_e( $tooltip ); ?>">Pro</a>
			<?php
		} );
		


		$this->container->share( 'allowed-tags', function () {
			...
			$genericAttributes = [
				...
				'totalcontest-mobile-scrollable'         => [],
				'totalcontest-submissions-toggle-layout' => [],
				'totalcontest-ajax-url'                  => [],
				'totalcontest-submissions-filter'        => [],
				'totalcontest-submissions'               => [],
				'totalcontest-submissions-item'          => [],
				'totalcontest'                           => [],
				'totalcontest-uid'                       => [],
				'totalcontest-screen'                    => [],
				'totalcontest-submission-id'             => [],
				'totalcontest-contest-id'                => [],
				'totalcontest-config'                    => [],
			];
			...
			$ngAttributes = array_merge( $genericAttributes, [
				'ng-app'            => [],
				'ng-bind'           => [],
				'ng-bind-html'      => [],
				'ng-bind-template'  => [],
			...
			$tags = [
				'div'              => $ngAttributes,
				'article'          => $ngAttributes,
				'section'          => $ngAttributes,
				'footer'           => $ngAttributes,
				'dl'               => $ngAttributes,
				'dd'               => $ngAttributes,
				'dt'               => $ngAttributes,
				...
				'circle'           => array_merge( $genericAttributes, $svgAttributes ),
				'rect'             => array_merge( $genericAttributes, $svgAttributes ),
				'line'             => array_merge( $genericAttributes, $svgAttributes ),
				'text'             => array_merge( $genericAttributes, $svgAttributes ),
				'defs'             => array_merge( $genericAttributes, $svgAttributes ),
				'linearGradient'   => array_merge( $genericAttributes, $svgAttributes ),
				'stop'             => array_merge( $genericAttributes, $svgAttributes ),
				'animateTransform' => array_merge( $genericAttributes, $svgAttributes ),
				...
				'iframe'           => $ngAttributes,
				'figure'           => $ngAttributes,
				'figcaption'       => $ngAttributes,
				'source'           => $ngAttributes,
				'img'              => $ngAttributes,
				'input'            => $ngAttributes,
				'button'           => $ngAttributes,
				'textarea'         => $ngAttributes,
				'select'           => $ngAttributes,
				'option'           => $ngAttributes,
				'template'         => $ngAttributes,
				'canvas'           => $ngAttributes,
				'script'           => $ngAttributes,
				'link'             => $ngAttributes,
			];
	...
	public function loadTextDomain() {
		...
		if ( ! $loaded ):
			load_textdomain( 'totalcontest', "{$path}languages/{$mofileFallback}" );
		endif;
	}
	...
	public function bootstrapExtensions() {
		...
		try {
			...
			if ( Misc::isDevelopmentMode() ):
				trigger_error( esc_html( $exception->getMessage() ), E_USER_WARNING );
			else:
