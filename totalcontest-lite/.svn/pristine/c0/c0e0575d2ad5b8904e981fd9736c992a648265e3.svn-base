<div id="totalcontest-submission-editor" ng-app="submission-editor" ng-controller="SubmissionCtrl as $ctrl" ng-cloak="">
    ...
    <textarea name="content" rows="60" class="widefat" readonly ng-bind-template="{{settings|json}}" hidden><?php if(isset( $GLOBALS['post'] )): ?><?php echo esc_textarea( $GLOBALS['post']->post_content ); ?><?php else: ?>{}<?php endif; ?></textarea>
</div>
