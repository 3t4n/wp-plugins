...
class CreateSubmission extends Command {
	...
	protected function handle() {
		...
		try {
			...
			foreach ( $fields as $field ):
				...
				elseif ( in_array( $field['type'], [ 'image', 'video', 'audio', 'file' ] ) ):
					...
					elseif ( isset( $_FILES['totalcontest'] ) && $_FILES['totalcontest']['error'][ $field['name'] ] === UPLOAD_ERR_OK ):
						...
						// Handle upload
						$submissionAttachmentFile = wp_handle_upload( $submissionUploadedFile,
						                                              [ 'test_form' => false ] );
						...
						$submissionAttachmentArgs = [
							...
							'post_title'     => preg_replace( '/\.[^.]+$/',
							                                  '',
							                                  basename( $submissionAttachmentFile['file'] ) ),
							'post_content'   => '',
							'post_status'    => 'inherit',
						];
						...
						wp_update_attachment_metadata(
							...
							wp_generate_attachment_metadata( $submissionAttachmentId,
							                                 $submissionAttachmentFile['file'] )
						);

						// Append content and thumbnails
						$submissionAttachmentFileType                       = explode( '/',
						                                                               $submissionAttachmentFile['type'] )[0];
						$submissionAttachmentFileType                       = in_array( $submissionAttachmentFileType,
						                                                                [
							                                                                'image',
							                                                                'video',
							                                                                'audio',
							                                                                'text',
						                                                                ] ) ? $submissionAttachmentFileType : 'file';
						$submissionAttributes['contents'][ $field['name'] ] = [
							// Type
							'type'       => 'attachment',
							// Attachment
							...
							// Preview
							'preview'    => sprintf( '[totalcontest-image id="%d" src="%s"]',
							                         $submissionAttachmentId,
							                         esc_attr( wp_get_attachment_thumb_url( $submissionAttachmentId ) ) ),
							// Embed shortcode
							'content'    => sprintf( '[totalcontest-%s id="%d" src="%s"]',
							                         $submissionAttachmentFileType,
							                         $submissionAttachmentId,
							                         esc_attr( $submissionAttachmentFile['url'] ) ),
						];
			...
			$submissionAttributes = apply_filters(
				'totalcontest/filters/contest/command/create-submission/attributes',
				$submissionAttributes,
				$this->contest,
				$this
			);
			...
			$submissionPostArgs = [
				...
				'post_content'  => wp_slash( json_encode( $submissionAttributes,
			...
			do_action(
				'totalcontest/actions/contest/command/create/post-inserted',
				$submissionPostId,
				$this->contest,
				$submissionPostArgs
			);
			...
			if ( $submissionPostId instanceof \WP_Error ):
				throw new \ErrorException( $submissionPostId->get_error_message(),
				                           $submissionPostId->get_error_code() );
			else:
				// Update title
				wp_update_post( [
					                'ID'         => $submissionPostId,
					                'post_title' => Strings::template(
						                ...
						                apply_filters(
							                'totalcontest/filters/contest/command/create/title-vars',
							                [
				...
				if ( $altSerialization && ! empty( $fields['category'] ) ):
					wp_set_post_terms( $submissionPostId,
					                   (array) $fields['category'],
					                   TC_SUBMISSION_CATEGORY_TAX_NAME );
				elseif ( ! $altSerialization && ! empty( $submissionAttributes['fields']['category'] ) ):
					wp_set_post_terms( $submissionPostId,
					                   (array) $submissionAttributes['fields']['category'],
					                   TC_SUBMISSION_CATEGORY_TAX_NAME );
				endif;
			endif;
			...
			$this->setCookie( $this->contest->getPrefix( "token_{$submissionPostId}" ),
			                  $submissionAttributes['token'] );
