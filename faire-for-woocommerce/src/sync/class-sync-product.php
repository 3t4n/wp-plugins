...
class Sync_Product {
	...
	public function show_faire_product_columns( string $column, int $id ) {
		...
		if ( self::FAIRE_PRODUCT_COLUMN === $column ) {
			...
			if ( $linking_error_message ) {
				if ( $faire_product_id ) {
					echo esc_html__( 'ID:', 'faire-for-woocommerce' ) . ' <span class="wc_faire_product_faire_id">' . esc_html( $faire_product_id ) . '</span><br>';
				}
				echo $linking_error_message;
				...
				echo esc_html__( 'ID:', 'faire-for-woocommerce' ) . ' <span class="wc_faire_product_faire_id">' . esc_html( $faire_product_id ) . '</span>';
				echo '<br>' . esc_html__( 'Last Sync:', 'faire-for-woocommerce' ) . ' <span class="wc_faire_product_last_sync_result">' . wp_kses_post( $last_sync_msg ) . '</span>';
				echo '<br><button data-product="' . esc_attr( $id ) . '" class="button-secondary wc_faire_manual_product_sync" type="button" name="wc_faire_manual_product_sync" style="margin: 3px 0">' . esc_html__( 'Sync Now', 'faire-for-woocommerce' ) . '</button>';
			...
			echo '<span>' . esc_html( $lifecycle_state_string ) . '</span>';
		}
	}
	...
	public function handle_bulk_actions( $redirect_to, $action, $post_ids ) {
		if ( ! in_array( $action, array( 'faire_unpublished', 'faire_published', 'faire_draft' ), true ) ) {
			return $redirect_to;
		}
	...
	public function log_summary_product_sync_result( $value, $type, $code = '', $timestamp = null ) {
		...
		if ( 'queued' === $type ) {
			...
			if ( (string) '500' === $code ) {
				$summary['500_failure'][] = $value;
			} elseif ( (string) '429' === $code ) {
				$summary['429_failure'][] = $value;
			} else {
	...
	public function save_faire_product(
		...
		if ( ! $this->settings->get_sync_enabled() ) {
			$faire_result    = (object) array(
				'error' => (object) array(
					'code'    => '0',
	...
	public function save_product_faire_ids( $faire_product, $product ) {
        ...
        $product_children_variation_ids = $product->get_children();
				...
				foreach ( $faire_product->variants as $faire_variant ) { 
          if ( ! isset( $faire_variant->idempotence_token ) ) continue;
	...
	public function set_product_args( $product, $ignore_excluded_args = false ) {
		...
		if ( 'simple' === $product->get_type() ) {
			...
			if ( $variations ) {
				...
				foreach ( $variations as $variation_id ) {
					...
					$variation_clones           = array();
					$variation_clone_options    = array();
					$variation_existing_options = array();
					foreach ( $variation_attributes as $attribute_name => $attribute_value ) {
						...
						if ( '' === $attribute_value ) {
							...
							if ( $_product_attribute && $_product_attribute['attribute_object']['is_taxonomy'] ) {
								...
								if ( $option_term && ! is_wp_error( $option_term ) ) {
					...
					// Create variant clones for unexisting options.
			...
			// Ignore variation, as these are managed from their parent product.
			return;
      ...
      'short_description'                => '',
			...
			'unit_multiplier'                  => apply_filters( 'faire_wc_product_unit_multiplier', 1, $product ), // REQUIRED FOR PUBLISHED.
			'minimum_order_quantity'           => apply_filters( 'faire_wc_product_minimum_order_quantity', 1, $product ),
			'per_style_minimum_order_quantity' => apply_filters( 'faire_wc_product_per_style_minimum_order_quantity', 0, $product ),
	...
	/**
	 * Build faire product inventory relatead args from a WooCommerce product, for use updating inventory avaialble quantities (stock levels)
	 *
	 * @return array
	 */
	...
	/**
	 * Get excluded sync faire product args, intepretting args to remove for setting values
	 *
	 * @return array
	 */
