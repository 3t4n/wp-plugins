...
class Order {
	...
	public function __construct( Order_Api $order_api ) {
		...
		add_action(
			...
			array( $this, 'fill_faire_order_id_column' ),
			20,
			2
		);
	 ...
	 */
	public function fill_faire_order_id_column( string $column, int $wc_order_id ) {
	...
	private function get_faire_order_id( int $wc_order_id ): string {
		return (string) get_post_meta( $wc_order_id, self::FAIRE_ORDER_ID_FIELD, true );
	}
	...
	public function ajax_backorder_products() {
		...
		if ( empty( $_POST['wc_order_id'] ) ) {
			wp_send_json_error( 'Missing order ID', 'faire-for-woocommerce', 400 );
		}
		if ( empty( $_POST['items_data'] ) ) {
			wp_send_json_error( 'Missing items data', 'faire-for-woocommerce', 400 );
		}
		if ( empty( $_POST['faire_order_id'] ) ) {
			wp_send_json_error( 'Missing Faire order ID', 'faire-for-woocommerce', 400 );
		}
		if ( empty( $_POST['availabilities'] ) ) {
			wp_send_json_error( 'Missing availabilities data', 'faire-for-woocommerce', 400 );
		}
