...
class Product extends WC_Product {
	 ...
	 *
	 * @param array $order_items The Faire order items.
	 *
	 * @return array List of products and order item data.
	 */
	public static function get_wc_products_in_faire_order(
		array $order_items
	): array {
		$products = array();
		foreach ( $order_items as $item ) {
			// Get the product ID using the provided SKU.
			$product = self::get_wc_product_by_faire_product_id(
				$item->product_id,
				$item->variant_id
			);
	 ...
	 * @return WC_Product|false The WC product or false if not found.
	 */
	public static function get_wc_variation_by_faire_variant_id(
		string $parent_id,
		string $faire_variant_id
	) {
		$settings   = new Settings();
	 ...
	 */
	public static function get_product_by_metadata(
		string $meta_field,
		string $meta_value
	) {
	 ...
	 *
	 * @param array  $order_products The list of products from the order.
	 * @param string $order_status  The order status.
	 *
	 * @return array List of products ID whose stock couldn't be updated.
	 */
