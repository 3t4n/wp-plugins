...
class Api_Client {
	...
	protected function get_response_body( Response $response, array $success_statuses ): object {
		...
		if ( in_array( $response->status, $success_statuses, true ) ) {
			$json_object = is_object( $response->body ) ? $response->body : json_decode( $response->body );
			if ( is_object( $json_object ) ) {
				return $json_object;
			}
			return (object) $response->body; // response body was empty.
		}
	...
	protected function throw_error( $response = null ) {
		...
		if ( is_object( $response_obj ) ) {
			// $error_message = (isset($response_obj->status_type)) ? $response_obj->status_type : $error_message;
			$error_message = ( isset( $response_obj->message ) ) ? $response_obj->message : $error_message;
			$status_code   = ( isset( $response_obj->status_code ) ) ? $response_obj->status_code : '';
		}
