...
class Sync_Product_Linking {
	...
	public function check_if_faire_products_exist(): array {
		...
		if ( isset( $response->error ) ) {
			...
			$result = $this->create_product_check_result_entry(
				false,
				sprintf(
					// translators: %s error.
	...
	public function import_products_linking(): array {
		...
		do {
			...
			$error  = $this->get_products_linking_page_errors( $products_linking );
			if ( $error ) {
				$results[] = $error;
				return $results;
			}
			...
			$error = $this->check_first_products_linking_page_empty( $products_linking_in_page );
			if ( $error ) {
				$results[] = $error;
				return $results;
			}
			...
			foreach ( $products_linking as $faire_product ) {
				...
				foreach ( $result as $r ) {
					if ( 'success' === $r['status'] ) {
						$synced_products_linking[] = $faire_product->id;
					}
					$results[] = $r;
				}
			}
		...
		$total_products_linking_synced = count( $synced_products_linking );
	 ...
	 *
	 * @param array $products Products retrieved or errors result.
	 *
		...
		if ( isset( $products_linking['error'] ) ) {
			$errors[] = Utils::create_import_error_entry(
				sprintf(
					...
					$this->faire_link_products_args['page'],
					$this->faire_link_products_args['updated_at_min']
				)
			);
			$errors[] = Utils::create_import_error_entry(
	...
	/**
	 * Checks if the first page of retrieved linking products is empty.
	 *
	 * @param int $products_linking_in_page Products retrieved.
	 *
	 * @return array Resulting error.
	 */
	private function check_first_products_linking_page_empty( int $products_linking_in_page ): array {
		$errors = array();
	...
	private function link_faire_to_wc_product ( $faire_product ) {
		...
		if ( is_array( $parent_ids_found ) && count($parent_ids_found) === 1 ) {
			...
			if ( ! $this->check_product_attributes_match_faire_options( $parent_product, $faire_product ) ) {
				...
				$results[] = Utils::create_import_error_entry(
					sprintf(
						__( 'Faire product %s option sets do not match product id %s.', 'faire-for-woocommerce' ),
						$faire_product->id,
						$parent_product_id
					)
				);
			...
			if ( 'simple' === $parent_product->get_type()  ) {
				...
				if ( ! $this->get_faire_product_id( $parent_product_id ) ) {
					...
					$results[] = Utils::create_import_error_entry(
						sprintf(
							__( 'Faire product %s already linked with product id %s.', 'faire-for-woocommerce' ),
							$faire_product->id,
							$parent_product_id
						)
					);
				...
				foreach ( $faire_product->variants as $faire_variant ) {
					...
					if ( ! in_array( $faire_variant->id, $linked_faire ) ) {
						// Next check by sku
						if ( $faire_variant->sku ) {
				...
				if ( !empty($faire_no_sku) || ( !empty($missing_faire) && !empty($missing_wc) ) ) {
					...
					$results[] = Utils::create_import_success_entry(
						sprintf(
							__( 'Manual linking required for faire variant %s on product id %s.', 'faire-for-woocommerce' ),
							$faire_variant->id,
							$parent_product_id
						)
					);
					...
					if ( !empty($missing_faire) && empty($missing_wc) ) {
						...
						$results[] = Utils::create_import_success_entry(
							sprintf(
								__( 'Faire product %s link queued for next sync with product id %s.', 'faire-for-woocommerce' ),
								$faire_product->id,
								$parent_product_id
							)
						);
					...
					// Handle scenario: WC variations are all linked, Faire has unlinked variants
					if ( empty($missing_faire) && !empty($missing_wc) ) {
						...
						foreach( $missing_wc as $faire_variant ) {
							...
							$results[] = Utils::create_import_success_entry(
								sprintf(
									__( 'Added missing faire variant %s to import CSV for product id %s.', 'faire-for-woocommerce' ),
									$faire_variant->id,
									$parent_product_id
								)
							);
			...
			$results[] = Utils::create_import_error_entry(
				sprintf(
					__( 'Multiple product matches. Faire product %s had variants skus on multiple different products: %s.', 'faire-for-woocommerce' ),
					$faire_product->id,
			...
			$results[] = Utils::create_import_error_entry(
				sprintf(
					__( 'Invalid product match. Faire product %s matched an invalid product type.', 'faire-for-woocommerce' ),
					$faire_product->id,
				)
			);
		...
		// If successfuly linked
		if ( $linked ) {
			$results[] = Utils::create_import_success_entry(
				sprintf(
					__( 'Faire product %s successfully linked with product id %s.', 'faire-for-woocommerce' ),
					$faire_product->id,
					$parent_product_id
				)
			);
	...
	private function check_product_attributes_match_faire_options( $product, $faire_product ) {
		...
		// Check if faire has options in WC
		foreach ( $faire_product->variant_option_sets as $option ) {
	 ...
	 * @param Object $faire_product      A Faire product.
	 *
	 * @return object/array/boolean  	 Product if successfly found, or array of all matches or boolean
	 */
	private function get_parent_product_for_faire_product( $faire_product ) {

		$found_products = [];

		//Find parent, ensure all variants match
		$parent_product_id = null;
		$log = false;
		$no_sku = false;
		foreach ( $faire_product->variants as $faire_variant ) {
			$sku = trim( $faire_variant->sku );
			if ( $sku === '' ) {
				$no_sku = true; 
				continue;
			}
			...
			if ( $product_id ) {
				...
				if ( $product && 'variation' === $product->get_type() ) {
					...
					if ( $parent_product_id ) {
						$found_products[] = $parent_product_id;
					}
					...
					if ( count($faire_product->variants) === 1 && empty($faire_product->variant_option_sets) ) {
						$found_products[] = $product_id;
					} else {
						return $this::PRODUCTS_LINKING_PRODUCT_MATCH_INVALID;
					}
				} elseif ( $product && $product->get_type() === 'variable' ) {
					return $this::PRODUCTS_LINKING_PRODUCT_MATCH_INVALID;
				}
			}
		}

		if ( empty($found_products) && $no_sku ) {
	...
	public function create_product_csv_rows( $faire_product ) {
		...
		$row['Type'] = $type;
		if ( $type === 'simple' ) {
			$row['SKU'] = ($faire_product->variants[0]->sku) ? $faire_product->variants[0]->sku : $faire_product->id;
		} else {
		...
		if ( $type === 'variable' ) {
			...
			$attribute_option_index = []; //Map from option name to attribute index
			$attr_i = 1;
			foreach ( $faire_product->variant_option_sets as $option ) {
			...
			if ( isset($faire_product->sale_state) ) {
				$row['In stock?'] = ( 'FOR_SALE' === $faire_product->sale_state ) ? 1 : 0; 
			} else {
				$row['In stock?'] = 1;
			}
			$row['Stock'] = ( isset($faire_variant->available_quantity) && $faire_product->variants[0]->available_quantity !== '' ) ? $faire_product->variants[0]->available_quantity : ''; // Leave empy string for no stock management
			...
			if ( !isset($faire_product->variants[0]->prices) && isset($faire_product->variants[0]->wholesale_price_cents) ) {
				$wholesale_price = $this->convert_cents_to_price( $faire_product->variants[0]->wholesale_price_cents );
				if ( isset( $faire_product->variants[0]->retail_price_cents ) ) {
					$row['Regular price'] = $this->convert_cents_to_price( $faire_product->variants[0]->retail_price_cents );
				}
			} else {
        if ( isset($faire_product->variants[0]->prices) && $faire_product->variants[0]->prices ) {
          $geo_constraint = $this->settings->get_faire_geo_constraint();
					foreach ($faire_product->variants[0]->prices as $price ) {
            if ( $price->wholesale_price->currency === get_woocommerce_currency() ) {
              if ( isset($price->geo_constraint->country) && $price->geo_constraint->country && isset($geo_constraint['country']) && $geo_constraint['country'] === $price->geo_constraint->country ) {
                $wholesale_price = $this->convert_cents_to_price( $price->wholesale_price->amount_minor );
							  $row['Regular price'] = $this->convert_cents_to_price( $price->retail_price->amount_minor );
              } elseif ( isset($price->geo_constraint->country_group) && $price->geo_constraint->country_group && isset($geo_constraint['country_group']) && $geo_constraint['country_group'] === $price->geo_constraint->country_group ) {
                $wholesale_price = $this->convert_cents_to_price( $price->wholesale_price->amount_minor );
							  $row['Regular price'] = $this->convert_cents_to_price( $price->retail_price->amount_minor );
              }
            }
      ...
      if ( '' === $wholesale_price && '' === $row['Regular price'] ) {
        error_log('product linking '.$faire_product->id.' failed to find any prices for geo constraint');
      }

			$row['Meta: woocommerce_faire_product_wholesale_price'] = $wholesale_price;
			$row['Meta: woocommerce_faire_product_tariff_code'] = $faire_product->variants[0]->tariff_code;
		}
	...
	public function create_variant_csv_rows( $faire_variant, $faire_product, $parent_col_match = '', $parent_product = null ) {
		...
		$row['Stock'] = ( isset($faire_variant->available_quantity) && $faire_variant->available_quantity !== '' ) ? $faire_variant->available_quantity : ''; // Leave empy string for no stock management
		...
		if ( !isset($faire_variant->prices) && isset($faire_variant->wholesale_price_cents) ) {
			...
			if ( isset($faire_variant->prices) && $faire_variant->prices ) {
        $geo_constraint = $this->settings->get_faire_geo_constraint();
    ...
    if ( '' === $variant_wholesale_price && '' === $row['Regular price'] ) {
      error_log('product linking '.$faire_variant->id.' failed to find any prices for geo constraint');
		...
		$attr_i = 1;
		foreach ( $faire_product->variant_option_sets as $option ) {
			// Get option attrbute index number 
			$option_attr = isset( $variant_attribute_option[ $attr_i ] ) ? $variant_attribute_option[ $attr_i ] : [];
			if ( $option_attr ) {
		...
		$row['Meta: woocommerce_faire_product_variation_wholesale_price'] = $variant_wholesale_price;
		$row['Meta: '.$this->meta_faire_variant_id] = $faire_variant->id; // faire variant id
		$row['Meta: woocommerce_faire_product_variation_tariff_code'] = $faire_variant->tariff_code;
	...
	public function get_wc_product_import_default_cols() {
		// Default CSV row 
		$_default_row = array('ID' => '', 'Type' => '', 'SKU' => '', 'Name' => '', 'Published' => '', 'Is featured?' => '', 
			...
			foreach ( $rows as $row ) {
				if ( 'simple' === $row['Type'] || 'variable' === $row['Type'] ) {
					...
					if ( $product ) {
						...
						$results[] = Utils::create_import_success_entry(
							sprintf(
								__( 'Created faire product %s as product id %s.', 'faire-for-woocommerce' ),
								'"'.$row['Name'].'"',
								$product->get_id()
							)
						);
						...
						$results[] = Utils::create_import_error_entry(
							sprintf(
								__( 'Failed to create faire product %s as product.', 'faire-for-woocommerce' ),
								'"'.$row['Name'].'"',
							)
						);
					}
					...
					if ( $variation ) {
						$results[] = Utils::create_import_success_entry(
							sprintf(
								__( 'Created faire variant %s on product id %s.', 'faire-for-woocommerce' ),
								'"'.$row['Name'].'"',
								$variation->get_id()
							)
						);
						...
						$results[] = Utils::create_import_error_entry(
							sprintf(
								__( 'Failed to create faire variant %s on product %s.', 'faire-for-woocommerce' ),
								'"'.$row['Name'].'"',
								$row['Parent']
							)
						);
	...
	public function create_product ( $row ) {
      ...
      if ( $image_gallery_ids ) {
        $product->set_gallery_image_ids( $image_gallery_ids );
      }
		}
	...
	public function create_variation ( $row, $parent_product = null ) {
		...
		if ( $parent_product && is_a( $parent_product, 'WC_Product' ) ) {
			...
			if ( 0 === stripos( $row['Parent'], 'id:' ) ) {
				...
				$lookup_id = wc_get_product_id_by_sku( $row['Parent'] );
			}
			if ( $lookup_id ) {
	...
	public function get_products_create_csv(): array {
		$products = get_option(self::OPTION_FAIRE_PRODUCT_LINKING_CREATE, []);
	...
	public function empty_product_create_csv() {
		update_option(self::OPTION_FAIRE_PRODUCT_LINKING_CREATE, [], false); //Set autoload = false
	}
	...
	public function get_variants_create_csv(): array {
		$variants = get_option(self::OPTION_FAIRE_VARIATIONS_LINKING_CREATE, []);
	...
	public function empty_variant_create_csv() {
		update_option(self::OPTION_FAIRE_VARIATIONS_LINKING_CREATE, [], false); //Set autoload = false
	}
