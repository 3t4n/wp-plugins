...
class WC_Order_Splitter_Script {
	...
	public function enqueue_scripts() {
		...
		// Check if "Pre-Orders WC" plugin is active
		$preOrdersInstalled = is_plugin_active('pre-orders-wc/pre-orders-wc.php');
		$show_notice = get_user_meta(get_current_user_id(), 'yoads_orders_splitter_to_pre_orders', true) != 1;
		$user = wp_get_current_user();
		$is_admin = in_array('administrator', (array) $user->roles, true);
	
		// Enqueue split order script for all eligible users
		wp_enqueue_script('split-order-js', plugin_dir_url(__FILE__) . '../../js/yoos-wc-order-splitter-split-table.js', array('jquery'), '1.4', true);
		...
		$translation_array = array(
			...
			'showNotice' => (get_user_meta(get_current_user_id(), 'yoads_orders_splitter_to_pre_orders', true) != 1 && $is_admin) ? 'yes' : 'no',
			'preOrdersInstalled' => is_plugin_active('pre-orders-wc/pre-orders-wc.php') ? 'yes' : 'no',
			'adminUrl' => admin_url(),
		);
		...
		wp_enqueue_style('split-order-css', plugin_dir_url(__FILE__) . '../../css/yoos-wc-order-splitter-style.css', '1.7', true);
	}	
