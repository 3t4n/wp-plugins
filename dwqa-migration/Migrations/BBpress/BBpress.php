<?php
namespace CDWQA\Migrations\BBpress;
class CDWQA_BBpress_Migration{
	...
	public function countTotal(){
		...
		if(!$this->is_local){
			$result[] = array(
				'action' => 'update_user',
				'count' =>  $this->countBBpressUsers()
			);
		}

		$result[] = array(
			'action' => 'update_category',
			'count' =>  $this->countBBpressForums()
		);

		$result[] = array(
			'action' => 'update_category_parent',
			'count' =>  $this->countBBpressCategoryParent()
		);
		...
		$result[] = array(
			...
			'count' =>  $this->countBBpressTopicReply()
		);
		$result[] = array(
	...
	public function runUpdateQuestionTag(){
		...
		if($posts){
			foreach($posts as $post){
				$dwqa_post_id = $this->getDWQAPostIdByBBpressPostID($post['bbpress_post_id']);
				$dwqa_tag_list = $this->getDWQATagsByBBpressTags($post['bbpress_tag_list']);
	...
	private function getDWQAPostIdByBBpressPostID($bbpress_post_id){
		if(!$bbpress_post_id){
			return 0;
		}

		$current_prefix = $this->current_db->db_prefix;
		$table_posts = $current_prefix . 'cdwqa_bbpress_posts';
		
		$sql = "
			SELECT dwqa_post_id FROM {$table_posts}
			WHERE bbpress_post_id = {$bbpress_post_id}
		";
	...
	private function getDWQATagsByBBpressTags($bbpress_tags = false){
		if(!$bbpress_tags){
			return array();
		}

		$current_prefix = $this->current_db->db_prefix;
		$table_tags = $current_prefix . 'cdwqa_bbpress_tags';

		$tags_in = "";

		if(is_string($bbpress_tags)){
			$tags_in = implode("','", explode(",", $bbpress_tags));
		}
		if(is_array($bbpress_tags)){
			$tags_in = implode("','", $bbpress_tags);
		}

		$sql = "
			SELECT dwqa_tag_id FROM {$table_tags}
			WHERE bbpress_tag_id IN ('{$tags_in}')
			";
	...
	public function getTagsQuestion(){
		...
		$sql = "
			SELECT tr.object_id AS bbpress_post_id, GROUP_CONCAT(DISTINCT tt.term_id SEPARATOR ',') AS bbpress_tag_list FROM {$remote_tab_term_taxonomy} tt
			INNER JOIN {$remote_tab_term_relationships} tr ON tt.term_taxonomy_id = tr.term_taxonomy_id
			WHERE tt.taxonomy = 'topic-tag'
	...
	public function runUpdateQuestionCategory(){
		...
		if($questions){
			foreach($questions as $question){
				$category = $this->getDWQAQuestionCategory($question['bbpress_post_parent_id']);
	...
	public function getDWQAQuestionCategory($bbpress_post_parent_id){
		$current_prefix = $this->current_db->db_prefix;
		$table_marked_categories = $current_prefix . 'cdwqa_bbpress_categories';

		$sql = "
			SELECT dwqa_category_id, term_taxonomy_id FROM {$table_marked_categories}
			WHERE bbpress_forum_id = {$bbpress_post_parent_id}
		";
	...
	public function countDWQAQuestions(){
		$current_prefix = $this->current_db->db_prefix;
		$table_marked_posts = $current_prefix . 'cdwqa_bbpress_posts';


		$sql = "
			SELECT COUNT(1) AS count  FROM {$table_marked_posts}
			WHERE dwqa_post_type = 'dwqa-question' AND bbpress_post_parent_id > 0
		";
	...
	public function getDWQAQuestions(){
		...
		$sql = "
			SELECT dwqa_post_id, bbpress_post_parent_id  FROM {$table_marked_posts}
			WHERE dwqa_post_type = 'dwqa-question' AND bbpress_post_parent_id > 0
		";
	...
	public function runUpdateQuestionAnswerTree(){
		...
		if($childs){
			foreach($childs as $child){
				$dwqa_post_parent_id = $this->getDWQAPostParent($child['bbpress_post_parent_id']);
	...
	public function getDWQAPostParent($bbpress_post_parent_id){
		$current_prefix = $this->current_db->db_prefix;
		$table_marked_posts = $current_prefix . 'cdwqa_bbpress_posts';
		...
		$sql = "
			...
			WHERE bbpress_post_id = {$bbpress_post_parent_id}
		";
		...
		if($result){
			...
			foreach($result as $item){
				$sql = "
					...
					WHERE post_parent = {$item['bbpress_post_id']} AND post_type = 'attachment'
				";
				// echo $sql;
				$this->remote_db->query($sql);
				...
				if($attachments){
					foreach($attachments as $attachment){
						$bbpress_post_id = $attachment['ID']; 
						$dwqa_post_id = $this->insertDWQAPost($attachment);

						$post_type = 'attachment';
						
						$this->markedPost($bbpress_post_id, $dwqa_post_id, $post_type, $attachment['post_parent']);
					}
				}
			}
		}
	...
	public function getDWQAParentAttachment(){
		...
		$sql = "
			SELECT dwqa_post_id, bbpress_post_id FROM {$table_marked_posts}
			WHERE dwqa_post_type = 'dwqa-question' OR dwqa_post_type = 'dwqa-answer'
			LIMIT {$limit}
			OFFSET {$offset}
		";
	...
	public function runUpdateQuestionAnswer(){
		if(!$this->remote_total || $this->remote_total < $this->remote_offset){
			//init
			$this->remote_total = $this->countBBpressTopicReply();
			$this->updateOption( 'cdwqa_remote_total', $this->remote_total );
		}
		if($this->remote_total < $this->remote_offset){
			//done
		...
		//
		$topic_reply = $this->getBBpressTopicReply();

		if($topic_reply){
			foreach($topic_reply as $item){
				$bbpress_post_id = $item['ID']; 
				...
				$this->markedPost($bbpress_post_id, $dwqa_post_id, $post_type, $item['post_parent']);
			}
		}
	...
	public function markedPost($bbpress_post_id, $dwqa_post_id, $dwqa_post_type, $bbpress_post_parent_id = 0){
		$current_prefix = $this->current_db->db_prefix;
		$table_marked_posts = $current_prefix . 'cdwqa_bbpress_posts';
		...
		if(!$check || empty($check)){
			$sql = "
				INSERT INTO {$table_marked_posts} (bbpress_post_id, dwqa_post_id, dwqa_post_type, bbpress_post_parent_id)
				VALUES({$bbpress_post_id}, {$dwqa_post_id}, '{$dwqa_post_type}', {$bbpress_post_parent_id})
			";
			$this->current_db->query($sql);
		}else{
			//only update if dwqa_post <> new dwqa_post
			$sql = "
				UPDATE {$table_marked_posts} SET dwqa_post_id = {$dwqa_post_id}, dwqa_post_type = '{$dwqa_post_type}', bbpress_post_parent_id = {$bbpress_post_parent_id} WHERE bbpress_post_id = {$bbpress_post_id} AND dwqa_post_id <> {$dwqa_post_id}
			";
			$this->current_db->query($sql);
		}
	}
	...
	public function getBBpressPostMeta($bbpress_post_id){
		$remote_prefix = $this->remote_db->db_prefix;
		$remote_blog_id = $this->remote_db->db_blog_id? $this->remote_db->db_blog_id . '_' : '';
	...
	public function insertDWQAPost($post = false){
		...
		//get post meta
		$bbpress_postmeta = $this->getBBpressPostMeta($post['ID']);
		...
		/*if($post_type == 'attachment'){
			...
			//copy file
			if(isset($bbpress_postmeta['_wp_attached_file'])){
				$remote_upload = rtrim($this->remote_db->upload_dir, '/');
				$current_upload = rtrim($this->current_db->upload_dir, '/');


				$remote_upload_file = $remote_upload. '/' .$bbpress_postmeta['_wp_attached_file'];
				if(file_exists($remote_upload_file)){
					$current_upload_file = $current_upload. '/' .$bbpress_postmeta['_wp_attached_file'];

					$current_file = $bbpress_postmeta['_wp_attached_file'];
					...
					//change meta
					$bbpress_postmeta['_wp_attached_file'] = $current_file;
		...
		foreach ($bbpress_postmeta as $k => $v) {
	...
	public function getDWQAPostAuthor($bbpress_author = false){
		if(!$bbpress_author){
			return 0;
		}
		...
		//prepare table temp
		$table_marked_users = $current_prefix . 'cdwqa_bbpress_users';

		$sql = "
			SELECT dwqa_user_id FROM {$table_marked_users} 
			WHERE bbpress_user_id = {$bbpress_author}
		";
		$this->current_db->query($sql);
	...
	public function checkDWQAPostExist($post) {
		...
		$sql = "
			...
			WHERE 
				ID IN ( SELECT dwqa_post_id FROM {$table_marked_posts} WHERE bbpress_post_id = {$post_id})
			LIMIT 1
		";
	...
	public function countBBpressTopicReply(){
		$remote_prefix = $this->remote_db->db_prefix;
		$remote_blog_id = $this->remote_db->db_blog_id? $this->remote_db->db_blog_id . '_' : '';
		...
		$sql = "
			...
			WHERE (post_type = 'topic' OR post_type = 'reply') AND post_status != 'draft' AND post_status != 'auto-draft'
		";
		$this->remote_db->query($sql);
		$result = $this->remote_db->getCell();
		return $result;
	...
	public function getBBpressTopicReply(){
		$remote_prefix = $this->remote_db->db_prefix;
		$remote_blog_id = $this->remote_db->db_blog_id? $this->remote_db->db_blog_id . '_' : '';
	...
	public function runUpdateTag(){
		...
		if($tags){
			foreach($tags as $tag){
				$bbpress_tag_id = $tag['term_id']; 
				$dwqa_tag = $this->insertDWQATag($tag);

				$this->markedTag($bbpress_tag_id, $dwqa_tag['term_id'], $dwqa_tag['term_taxonomy_id']);
			}
		}
	...
	public function markedTag($bbpress_tag_id, $dwqa_tag_id, $term_taxonomy_id){
		$current_prefix = $this->current_db->db_prefix;
		$table_marked_tags = $current_prefix . 'cdwqa_bbpress_tags';

		$sql = "
			SELECT id FROM {$table_marked_tags}
			WHERE bbpress_tag_id = {$bbpress_tag_id}
			LIMIT 1
		";
		...
		if(!$check || empty($check)){
			...
			$sql = "
				INSERT INTO {$table_marked_tags} (bbpress_tag_id, dwqa_tag_id, term_taxonomy_id)
				VALUES( {$bbpress_tag_id}, {$dwqa_tag_id}, {$term_taxonomy_id})
			";
			...
			$sql = "
				...
				WHERE bbpress_tag_id = {$bbpress_tag_id} AND dwqa_tag_id <> {$dwqa_tag_id}
			";
			$this->current_db->query($sql);
		}
	}
	...
	public function checkDWQATagExist($tag){
		...
		$sql = "
			...
			WHERE 
				tt.term_id IN ( SELECT dwqa_tag_id FROM {$table_marked_tags} WHERE bbpress_tag_id = {$bbpress_tag_id})
	...
	public function countBBpressTag(){
		...
		//prepare table
		$remote_tab_terms = $remote_prefix . $remote_blog_id .'terms';
		$remote_tab_term_taxonomy = $remote_prefix . $remote_blog_id .'term_taxonomy';

		$sql = "
			SELECT COUNT(1) AS count FROM {$remote_tab_terms} t
			INNER JOIN {$remote_tab_term_taxonomy} tt ON t.term_id = tt.term_id
			WHERE tt.taxonomy = 'topic-tag'
		";
		$this->remote_db->query($sql);
		$result = $this->remote_db->getResults();
	...
	public function getBBpressTag(){
		...
		$limit = $this->limit;
		$sql = "
			SELECT t.*, tt.term_taxonomy_id, tt.taxonomy, tt.description, tt.parent FROM {$remote_tab_terms} t
			INNER JOIN {$remote_tab_term_taxonomy} tt ON t.term_id = tt.term_id
			WHERE tt.taxonomy = 'topic-tag'
			LIMIT {$limit}
			OFFSET {$offset}
	...
	public function runUpdateCategoryParent(){
		...
		if(!$this->remote_total || $this->remote_total < $this->remote_offset){
			//init
			$this->remote_total = $this->countBBpressCategoryParent();
			$this->updateOption( 'cdwqa_remote_total', $this->remote_total );
		}
		if($this->remote_total < $this->remote_offset){
			//done
	...
	public function getMarkedCategories(){
		...
		//prepare table temp
		$table_categories = $current_prefix . 'cdwqa_bbpress_categories';
		...
		$sql = "
			...
			INNER JOIN {$table_categories} c2 ON c1.bbpress_forum_parent_id = c2.bbpress_forum_id
			WHERE 1=1
			LIMIT {$limit}
			OFFSET {$offset}
		";
	...
	public function countBBpressCategoryParent(){
		$current_prefix = $this->current_db->db_prefix;
		...
		$sql = "
			SELECT COUNT(1) AS count FROM {$table_categories} WHERE bbpress_forum_parent_id != 0
		";
		$this->current_db->query($sql);
		$result = $this->current_db->getResults();
		return $result && isset($result[0])?$result[0]['count']:0;
	...
	public function runUpdateCategory(){
		...
		if(!$this->remote_total || $this->remote_total < $this->remote_offset){
			//init
			$this->remote_total = $this->countBBpressForums();
			$this->updateOption( 'cdwqa_remote_total', $this->remote_total );
		}
		...
		//BBpress forum is DWQA category
		$forums = $this->getBBpressForums();
		
		if($forums){
			foreach($forums as $forum){

				$bbpress_forum_id = $forum['ID']; 
				$dwqa_category = $this->insertDWQACategory($forum);

				$this->markedCategory($bbpress_forum_id, $dwqa_category['term_id'], $dwqa_category['term_taxonomy_id'], $forum['post_parent']);
			}
		}
		//done 1 round => increase offset
		$this->increaseOffset();
	...
	public function checkDWQACategoryExist($forum){
		$current_prefix = $this->current_db->db_prefix;
		...
		$slug = strip_tags($forum['post_name']);
		$bbpress_forum_id = $forum['ID'];
		...
		$sql = "
			...
			INNER JOIN {$current_tab_term_taxonomy} as tt ON tt.term_id = t.term_id
			WHERE 
				tt.term_id IN ( SELECT dwqa_category_id FROM {$table_categories} WHERE bbpress_forum_id = {$bbpress_forum_id})
	...
	public function insertDWQACategory($forum = false){
		if(!$forum){
			return false;
		}

		//check existed
		$category = $this->checkDWQACategoryExist($forum);
		if($category){
		...
		$name = strip_tags($forum['post_title']);
		$slug = strip_tags($forum['post_name']);
	...
	public function markedCategory($bbpress_forum_id, $dwqa_category_id, $term_taxonomy_id, $bbpress_forum_parent_id = 0){
		$current_prefix = $this->current_db->db_prefix;
		$table_marked_categories = $current_prefix . 'cdwqa_bbpress_categories';

		$sql = "
			SELECT id FROM {$table_marked_categories}
			WHERE bbpress_forum_id = {$bbpress_forum_id}
			LIMIT 1
		";
		...
		if(!$check || empty($check)){
			...
			$sql = "
				INSERT INTO {$table_marked_categories} (bbpress_forum_id, dwqa_category_id, term_taxonomy_id, bbpress_forum_parent_id)
				VALUES( {$bbpress_forum_id}, {$dwqa_category_id}, {$term_taxonomy_id}, {$bbpress_forum_parent_id} )
			";

			$this->current_db->query($sql);
		}else{
			$sql = "
				UPDATE {$table_marked_categories} SET dwqa_category_id = {$dwqa_category_id}, term_taxonomy_id = {$term_taxonomy_id}, bbpress_forum_parent_id = {$bbpress_forum_parent_id} WHERE bbpress_forum_id = {$bbpress_forum_id} AND dwqa_category_id <> {$dwqa_category_id}
			";
			$this->current_db->query($sql);
		}
	}
	...
	public function countBBpressForums(){
		$remote_prefix = $this->remote_db->db_prefix;
		$remote_blog_id = $this->remote_db->db_blog_id? $this->remote_db->db_blog_id . '_' : '';
		...
		$sql = "
			SELECT COUNT(ID) AS count FROM {$remote_tab_posts}
	...
	public function getBBpressForums(){
		$remote_prefix = $this->remote_db->db_prefix;
		$remote_blog_id = $this->remote_db->db_blog_id? $this->remote_db->db_blog_id . '_' : '';
	...
	public function runUpdateUser(){
		...
		if(!$this->remote_total || $this->remote_total < $this->remote_offset){
			//init
			$this->remote_total = $this->countBBpressUsers();
			$this->updateOption( 'cdwqa_remote_total', $this->remote_total );
		}
		...
		//get users
		$users = $this->getBBpressUsers();

		if($users){
			foreach($users as $user){
				//check user via email
				$bbpress_user_id = $user['ID']; 
				$dwqa_user_id = $this->insertDWQAUser($user);
				$this->markedUser($bbpress_user_id, $dwqa_user_id);
			}
		}
	...
	public function checkDWQAUserExist($user){
		...
		$sql = "
			...
			WHERE 
				ID IN ( SELECT dwqa_user_id FROM {$table_marked_users} WHERE bbpress_user_id = {$user_id})
	...
	public function insertDWQAUser($user = false){
		...
		//get user meta
		$bbpress_usermeta = $this->getBBpressUserMeta($user['ID']);
		$meta_array = $this->getDWQAUserMetaKeys();

		foreach ($bbpress_usermeta as $k => $v) {
			if(isset($meta_array[$k])){
	...
	public function markedUser($bbpress_user_id, $dwqa_user_id){
		$current_prefix = $this->current_db->db_prefix;
		$table_marked_users = $current_prefix . 'cdwqa_bbpress_users';

		$sql = "
			SELECT id FROM {$table_marked_users}
			WHERE bbpress_user_id = {$bbpress_user_id}
			LIMIT 1
		";
		...
		if(!$check || empty($check)){
			...
			$sql = "
				INSERT INTO {$table_marked_users} (bbpress_user_id, dwqa_user_id)
				VALUES( $bbpress_user_id, $dwqa_user_id )
			";
			...
			$sql = "
				...
				WHERE bbpress_user_id = {$bbpress_user_id} AND dwqa_user_id <> {$dwqa_user_id}
			";
			$this->current_db->query($sql);
		}
	}

	public function countBBpressUsers(){
		$remote_prefix = $this->remote_db->db_prefix;
		$remote_blog_id = $this->remote_db->db_blog_id? $this->remote_db->db_blog_id . '_' : '';
	...
	public function getBBpressUserMeta($bbpress_user_id){
		$remote_prefix = $this->remote_db->db_prefix;
	...
	public function getBBpressUsers(){
	...
	public function prepareTable(){
		...
		//prepare table temp
		$table_users = $current_prefix . 'cdwqa_bbpress_users';
		$table_categories = $current_prefix . 'cdwqa_bbpress_categories';
		$table_tags = $current_prefix . 'cdwqa_bbpress_tags';
		$table_posts = $current_prefix . 'cdwqa_bbpress_posts';
		...
		$sql = "
			CREATE TABLE IF NOT EXISTS `$table_users`(
				...
				bbpress_user_id BIGINT(20) NOT NULL,
		...
		$sql = "
			CREATE TABLE IF NOT EXISTS `$table_categories` (
				id BIGINT(20) NOT NULL PRIMARY KEY AUTO_INCREMENT,
				bbpress_forum_id BIGINT(20) NOT NULL,
				dwqa_category_id BIGINT(20) NOT NULL,
				term_taxonomy_id BIGINT(20) NOT NULL,
				bbpress_forum_parent_id BIGINT(20) NOT NULL
			)$charset_collate;
		";
		$query = $this->current_db->query($sql);
		...
		$sql = "
			CREATE TABLE IF NOT EXISTS `$table_tags` (
				id BIGINT(20) NOT NULL PRIMARY KEY AUTO_INCREMENT,
				bbpress_tag_id BIGINT(20) NOT NULL,
		...
		$sql = "
			CREATE TABLE IF NOT EXISTS `$table_posts` (
				id BIGINT(20) NOT NULL PRIMARY KEY AUTO_INCREMENT,
				bbpress_post_id BIGINT(20) NOT NULL,
				dwqa_post_id BIGINT(20) NOT NULL,
				dwqa_post_type VARCHAR(255) NOT NULL,
				bbpress_post_parent_id BIGINT(20) NOT NULL
			)$charset_collate;
		";
		$query = $this->current_db->query($sql);
	}
	...
	//bbpress forum is dwqa category
	public function getBBpressForum(){
		global $remotedb;

		$forums = array();
		$forums_query = $remotedb->prepare( "SELECT * FROM {$remotedb->posts} WHERE post_type = %s", 'forum' );
		$forums_result = $remotedb->get_results( $forums_query );

		return $forums_result;
	}
}