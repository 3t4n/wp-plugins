...
<div id="at-cf7-form-cntr">
	<div class="clearfix flexbox-container">
		<div class="at-width-50">
			<h4 class="at-h4">External ID</h4>
			<p>Change the default setting if your ActiveTrail account uses External ID</p>
			<fieldset>
				<select id="external_id_tag" name="wpcf7-activetrail[external_id]" >
					<option <?php echo (isset($settings_values['external_id']) && $settings_values['external_id'] === 'default') ? 'selected' : ''; ?> value="default">Default</option>
					<option <?php echo (isset($settings_values['external_id']) && $settings_values['external_id'] === 'external') ? 'selected' : ''; ?> value="external">External ID</option>
				</select>
			</fieldset>
			<br />
			<hr />
			<h4 class="at-h4">Required Fields</h4>
			<p>Please note that <b>Email or Mobile</b> is required, you will need to fill at least one of them.</p>
			<fieldset>
				<?php foreach($this->at_cf7_metafields['form']['required'] as $key => $value): ?>
					<legend><?php if ($value === 'form_id') { echo "Form ID"; } else { echo esc_html(ucwords(str_replace('_', ' ', $key))) . ' field'; } ?></legend>
	...
	<fieldset>
		<?php foreach($this->at_cf7_metafields['activetrail'] as $key => $value): ?>
			<legend><?php if ($value === 'token_id') { echo "API token"; } else { echo esc_html(ucwords(str_replace('_', ' ', $value))); } ?> <?php if ($value == 'mailing_list_id') { echo "(Optional)"; } ?></legend>
			<input <?php if ($value === 'external_name' && (!isset($settings_values['external_id']) || $settings_values['external_id'] === 'default')) { echo "disabled"; } ?> style="width: 100%" type="text" placeholder="" id="<?php echo sanitize_text_field($value); ?>_tag" name="wpcf7-activetrail[<?php echo sanitize_text_field($value); ?>]" 
