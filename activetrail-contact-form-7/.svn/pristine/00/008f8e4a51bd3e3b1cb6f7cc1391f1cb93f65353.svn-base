...
class Activetrail_Cf7_Admin {
	...
	public function at_cf7_save_form($cf7) {
		...
		//Add new external_id setting in account_settings
		foreach ($this->at_cf7_metafields['form']['account_settings'] as $key => $field) {
			if (isset($_POST['wpcf7-activetrail'][$field])) {
				$post_fields[$field] = sanitize_text_field($_POST['wpcf7-activetrail'][$field]);
			}
		...
		foreach ($this->at_cf7_metafields['form']['required'] as $key => $field) {
			//Only one of the fields email or mobile is required.if one is missing but the other exits the post should be consider as valid
			if ($field === 'email' || $field === 'mobile') {
				if (
					(!isset($post_fields['email']) || !isset($meta_post_fields['email_metabox_nonce']))
					&& (!isset($post_fields['mobile']) || !isset($meta_post_fields['mobile_metabox_nonce']))
				) {
					wp_die('POST data violation');
				}
			} else {
	...
	public function at_cf7_post_form($cf7_form) {
		...
		$optional = array();
		$sms_metakey = $this->get_metakey($this->at_cf7_metafields['form']['required']['mobile']);
		$sms_key = null;
		$mobile_data = null;
		...
		if (array_key_exists($sms_key, $_POST) && isset($_POST[$sms_key])) {
			$sms = trim($_POST[$db_post_meta[$this->get_metakey($this->at_cf7_metafields['form']['required']['mobile'])][0]]);
			if ($sms && !empty($sms)) {
				$mobile_data = sanitize_text_field($sms);
				
				// Use regular expressions to extract numbers
                $mobile_data = preg_replace('/[^0-9]/', '', $mobile_data);
			}

			if (!is_numeric($mobile_data)) {
		...
		$external_id_metakey = $this->get_metakey($this->at_cf7_metafields['form']['account_settings']['external_id']);
		$external_id_key = null;
		$external_id = null;
		if (array_key_exists($external_id_metakey, $db_post_meta)) {
			$external_id_key = $db_post_meta[$external_id_metakey][0];
		}
				
		if (array_key_exists($external_id_key, $_POST) && isset($_POST[$external_id_key]) && !empty($_POST[$external_id_key])) {
			$external_id_key = trim($_POST[$external_id_key]);
			$external_id_key = sanitize_text_field($external_id_key);	
		}

		if ($external_id_key === 'external') {
			if ($lead_email && $mobile_data) {
				$external_id = $this->format_email_external_id($lead_email) . $this->format_mobile_external_id($mobile_data);
			} elseif ($lead_email) {
				$external_id = $this->format_email_external_id($lead_email);
			} elseif ($mobile_data) {
				$external_id = $this->format_mobile_external_id($mobile_data);
			} else {
				$this->at_api_error_stack[] = 'Email or Mobile is required, you will need to fill at least one of them';
			}
		} else {
			$external_id = $external_id_key;
		}

		$external_name_metakey = $this->get_metakey($this->at_cf7_metafields['activetrail']['external_name']);
		$external_name = null;
		
		if (array_key_exists($external_name_metakey, $db_post_meta)) {
			$external_name = $db_post_meta[$external_name_metakey][0];
		}
		...
		if ($external_id_key === 'external') {
			$contact =['contact' => $combined, 'externalId' =>  $external_id];

			if (isset($external_name) && $external_name) {
				$contact['externalName'] = $external_name;
			}
			$contact =  array( (object) $contact );
		} else {
			$contact = array($combined);
		}
		...
		if ($external_id_key === 'external') {
			$res_json = $activetrail_api->import_contacts_external($data);
		} else {
	...
	private function format_email_external_id($email) {
		return $email ? str_replace(['@', '.'], '', $email) : '';
	}

	private function format_mobile_external_id($mobile_data) {
		return $mobile_data ? str_replace('+', '', $mobile_data) : '';
	}
