...
class WC_Iugu_Credit_Card_Gateway extends WC_Iugu_Credit_Card_Woocommerce_Subscription_Gateway
{
	...
	public $value_save_card;
	public $value_use_card_default;
	...
	public function payment_fields()
	{
		...
		if (!is_add_payment_method_page()) {
			...
			if ($order_id > 0) {
				$order = wc_get_order($order_id);
				$iugu_card_installments = $order->get_meta('iugu_card_installments', true);
				if (!$registration_required) {
	...
	public function process_payment($order_id)
	{
		...
		if (isset($_POST['iugu_card_installments'])) {
			$order->update_meta_data('iugu_card_installments', sanitize_text_field($_POST['iugu_card_installments']));
			$order->save();
		}
		/**
