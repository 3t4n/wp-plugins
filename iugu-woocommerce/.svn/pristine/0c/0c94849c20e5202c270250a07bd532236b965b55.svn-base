...
class WC_Iugu_API
{
  ...
  public function add_error($message)
  {
    if (is_object($message) && is_wp_error($message)) {
      wc_add_notice($message->get_error_message());
   ...
   */
  public function create_customer_payment_method($order, $card_token, $customer_id = '', $default = false)
  {
  ...
  public function cancel_invoice_order_id($order_id)
  {
    ...
    if (isset($iugu_payment_booklets_installments) && $iugu_payment_booklets_installments >= 1) {
      for ($i = 1; $i <= $iugu_payment_booklets_installments; $i++) {
        $transaction_id = $order->get_meta('_iugu_wc_transaction_' . $i . '_id', true);
        $transaction_status = $order->get_meta('_iugu_wc_transaction_' . $i . '_status', true);
  ...
  public function process_payment_payment_booklets($order_id, $customer_id)
  {
    ...
    if ($payment_booklets) {
      if (isset($payment_booklets['errors'])) {
        ...
        $order->update_meta_data('_transaction_id', sanitize_text_field($payment_booklets['id']));
        $invoices = $payment_booklets['invoices'];
        $order->update_meta_data('iugu_payment_booklets_installments', sizeof($invoices));
        $i = 0;
        foreach ($invoices as $invoice) {
          $i = $i + 1;
          $order->update_meta_data('_iugu_wc_transaction_' . $i . '_id', $invoice['id']);
          $order->update_meta_data('_iugu_wc_transaction_' . $i . '_due_date', $invoice['due_date']);
          $order->update_meta_data('_iugu_wc_transaction_' . $i . '_total_cents', $invoice['total_cents']);
          $order->update_meta_data('_iugu_wc_transaction_' . $i . '_status', $invoice['status']);
          $order->update_meta_data('_iugu_wc_transaction_' . $i . '_secure_url', $invoice['secure_url']);
          $order->update_meta_data('_iugu_wc_transaction_' . $i . '_bank_slip_digitable_line', $invoice['bank_slip']['digitable_line']);
          $order->update_meta_data('_iugu_wc_transaction_' . $i . '_bank_slip_barcode_data', $invoice['bank_slip']['barcode_data']);
          $order->update_meta_data('_iugu_wc_transaction_' . $i . '_bank_slip_barcode', $invoice['bank_slip']['barcode']);
          if (isset($invoice['pix'])) {
            $order->update_meta_data('_iugu_wc_transaction_' . $i . '_pix_qrcode', $invoice['pix']['qrcode']);
            $order->update_meta_data('_iugu_wc_transaction_' . $i . '_pix_qrcode_text', $invoice['pix']['qrcode_text']);
          }
        }
        $order->save();
        $this->empty_cart();
  ...
  public function process_payment($order_id)
  {
    ...
    if ($order->get_total() == 0) {
      ...
      if ('credit-card' == $this->method) {
        ...
        $order->update_meta_data('iugu_card_installments', $iugu_card_installments);
        $order->update_meta_data('_iugu_customer_payment_method_id', $_iugu_customer_payment_method_id);
    ...
    if (isset($charge['invoice_id'])) {
      $order->update_meta_data('_transaction_id', sanitize_text_field($charge['invoice_id']));
      $order->save();
    } // end if;
     ...
     */
    if ('bank-slip' == $this->method) {
      $pdf = isset($charge['pdf']) ? $charge['pdf'] : '';
      $bankSlipUrl = isset($charge['bank_slip_url']) ? $charge['bank_slip_url'] : '';
      $bankSlipPdfUrl = isset($charge['bank_slip_pdf_url']) ? $charge['bank_slip_pdf_url'] : '';
      if (strlen($pdf) < 30) {
        $pdf = $bankSlipUrl;
      }
      if (strlen($pdf) < 30) {
      ...
      if (strlen($pdf) > 30) {
        ...
        $order->update_meta_data(__('iugu bank slip URL', IUGU), $payment_data['pdf']);
        $order->save();
      }
    ...
    if (function_exists('wcs_cart_contains_renewal')) {
      ...
      if (false !== $cart_item && isset($cart_item['subscription_renewal']['subscription_id'])) {
        ...
        $subscription->update_meta_data('_payment_method', $this->gateway->id);
        $subscription->update_meta_data('_payment_method_title', $this->gateway->title);
        $subscription->save();
      }
    }
    ...
    if (defined('WC_VERSION') && version_compare(WC_VERSION, '2.1.12', '<=')) {
      $order->update_meta_data(__('iugu transaction details', IUGU), 'https://iugu.com/a/invoices/' . sanitize_text_field($charge['id']));
      $order->save();
    } // end if;
    $this->empty_cart();
    if ('bank-slip' == $this->method) {
  ...
  protected function update_order_payment_booklet_status($order_id, $invoice_status, $meta_key, $invoice_id)
  {
    ...
    $order->update_meta_data(str_replace('_id', '_status', $meta_key), $invoice_status);
    $order->save();
    $order->add_order_note(sprintf(__('iugu: Invoice Status: %s.', IUGU), $invoice_status) . ' - ' . $invoice_id);
    if ($invoice_status == 'paid' || $invoice_status == 'externally_paid') {
      ...
      if (
        ...
        if (class_exists('WooCommerce') && class_exists('Automattic\WooCommerce\Utilities\OrderUtil')) {
          if (\Automattic\WooCommerce\Utilities\OrderUtil::custom_orders_table_usage_is_enabled()) {
            $id_field_meta_table = 'order_id';
            $orders_table = OrdersTableDataStore::get_orders_table_name();
            $orders_meta_table = OrdersTableDataStore::get_meta_table_name();
            $order_id = $wpdb->get_var($wpdb->prepare("SELECT id FROM $orders_table WHERE transaction_id = '%s'", $invoice_id));
          }
        }
        if ($orders_meta_table == $wpdb->postmeta) {
        ...
        if ($order_id) {
          ...
          $result = $wpdb->get_results($wpdb->prepare("SELECT $id_field_meta_table, meta_key FROM $orders_meta_table WHERE meta_key like '_iugu_wc_transaction_%_id' AND meta_value = '%s'", $invoice_id), ARRAY_A);
          if (sizeof($result) > 0) {
            $order_id = intval($result[0][$id_field_meta_table]);
