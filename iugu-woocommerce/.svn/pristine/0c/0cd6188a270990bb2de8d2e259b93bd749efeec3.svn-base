...
class WC_Iugu_Payment_Booklets_Gateway extends WC_Iugu_Gateway
{
	...
	public $origin_installment;
	public $iugu_number_installments_general;
	public $deadline;
	public $accepts_payment_pix;
	...
	public function get_interest_rate()
	{
		...
		$rates = apply_filters(
			'iugu_woocommerce_interest_rates',
			array(
				'1' => $p1,
				'2' => $p2,
				'3' => $p3,
	...
	public function process_payment($order_id)
	{
		...
		if (isset($_POST['iugu_payment_booklets_installments'])) {
			$order->update_meta_data('iugu_payment_booklets_installments', sanitize_text_field($_POST['iugu_payment_booklets_installments']));
			$order->save();
		}
		/**
	...
	public function email_instructions($order, $sent_to_admin, $plain_text = false)
	{
		...
		if (is_callable(array($order, 'get_meta'))) {
			...
			$data = $order->get_meta('_iugu_wc_transaction_data', true);
		}
	...
	function iugu_format_datetime($date, $format = '')
	{
	...
	function woocommerce_order_details_after_order_table($order)
	{
		if ('iugu-payment-booklets' == $order->get_payment_method()) {
			...
			for ($i = 1; $i <= $iugu_payment_booklets_installments; $i++) {
				...
				$tmp = $this->iugu_format_datetime(wc_string_to_datetime($order->get_meta('_iugu_wc_transaction_' . $i . '_due_date', true)));
				echo '			<td>' . $tmp . '</td>';
