...
class WC_Iugu_Credit_Card_Woocommerce_Subscription_Gateway extends WC_Iugu_Woocommerce_Subscription_Gateway
{
	...
	public function woocommerce_scheduled_subscription_payment($amount_total, $renewal_order)
	{
		if ($this->id == 'iugu-credit-card') {
			$iugu_card_installments = $renewal_order->get_meta('iugu_card_installments', true);
			if (!isset($iugu_card_installments) || ($iugu_card_installments == '')) {
				$iugu_card_installments = '1';
				$renewal_order->update_meta_data('iugu_card_installments', $iugu_card_installments);
				$renewal_order->save();
			}
			$_iugu_customer_payment_method_id = $renewal_order->get_meta('_iugu_customer_payment_method_id', true);
			if (!isset($_iugu_customer_payment_method_id) || ($_iugu_customer_payment_method_id == '')) {
				...
				if (isset($customer_id)) {
					...
					if (isset($iugu_customer)) {
						...
						if (isset($_iugu_customer_payment_method_id)) {
							$renewal_order->update_meta_data('_iugu_customer_payment_method_id', $_iugu_customer_payment_method_id);
							$renewal_order->save();
						}
					}
				}
	...
	{
		$subscription->update_meta_data('_iugu_customer_payment_method_id', $renewal_order->get_meta('_iugu_customer_payment_method_id', true));
		$subscription->update_meta_data('iugu_card_installments', $renewal_order->get_meta('iugu_card_installments', true));
		$subscription->save();
	}
		...
		$payment_meta[$this->id] = array(
			'post_meta' => array(
				'_iugu_customer_payment_method_id' => array(
					'value' => $subscription->get_meta('_iugu_customer_payment_method_id', true),
					'label' => __('IUGU Payment Method ID', IUGU),
				),
				'iugu_card_installments' => array(
					'value' => $subscription->get_meta('iugu_card_installments', true),
	...
	{
		if (!is_admin() && $subscription->get_meta('_payment_method', true) == $this->id) {
			$iugu_card_installments = $subscription->get_meta('iugu_card_installments', true);
			if (!isset($iugu_card_installments) || ($iugu_card_installments == '')) {
				$iugu_card_installments = '1';
			}
			$_iugu_customer_payment_method_id = $subscription->get_meta('_iugu_customer_payment_method_id', true);
