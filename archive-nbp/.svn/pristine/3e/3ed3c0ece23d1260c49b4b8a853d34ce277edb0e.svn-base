...
if ( ! class_exists( 'Archive_NBP' ) ) {
    class Archive_NBP {
        ...
        private function __construct() {
			...
			add_shortcode( 'anbp_currency_chart', [ $this, 'anbp_currency_chart_shortcode' ] );
			add_shortcode( 'anbp_last_rates', [ $this, 'anbp_last_rates_shortcode' ] );
			add_shortcode( 'anbp_currency_converter', [ $this, 'anbp_currency_converter_shortcode' ] );
        }
        ...
        public function anbp_get_nbp_rates($start_date, $end_date) {
            ...
            if( !is_wp_error($rates) && isset($rates['response']['code']) && $rates['response']['code'] === 200 ){
		...
		public function anbp_get_date_rate( \WP_REST_Request $request ) {
			...
			$dates = $wpdb->get_col( 
                $wpdb->prepare(
                    "SELECT exchange_date as date FROM %i WHERE exchange_date <= %s GROUP BY exchange_date ORDER BY exchange_date DESC LIMIT %d",
                    $this->rates_table_name,
                    $date_rate,
                    2
                )
		...
		public function anbp_currency_chart_shortcode($atts){
			$attributes = shortcode_atts(
				['date_begin' => '2002-01-01', 'date_end' => '2100-01-01', 'currency' => 'USD', 'attr' => 'class="wp-block-anbp-blocks-currency-chart"'],
				$atts,
				'anbp_currency_chart'
			);
			wp_enqueue_script( 'anbp-blocks-currency-chart-view-script', ANBP_DIR_PATH . '/blocks/currency-chart/view.js' );
			ob_start();
			include ANBP_DIR_PATH . '/blocks/currency-chart/render.php';
			return ob_get_clean();
		}
		
		public function anbp_last_rates_shortcode($atts){
			if(isset($atts['currencies'])){
				$atts['selectedCurrencies'] = explode(',', str_replace(' ', '', $atts['currencies']));
				unset($atts['currencies']);
			}
			if(isset($atts['date'])){
				$atts['rateDate'] = str_replace(' ', '', $atts['date']);
				unset($atts['date']);
			}
			$attributes = shortcode_atts(
				['rateDate' => '2100-01-01', 'selectedCurrencies' => ['USD','EUR','AUD','HKD','CAD','NZD','SGD','HUF','CHF','GBP','UAH','JPY','CZK','DKK','ISK','NOK','SEK','RON','BGN','TRY','ILS','CLP','PHP','MXN','ZAR','BRL','MYR','IDR','THB','INR','KRW','CNY','XDR'], 'attr' => 'class="wp-block-anbp-blocks-last-rates"'],
				$atts,
				'anbp_last_rates'
			);
			ob_start();
			include ANBP_DIR_PATH . '/blocks/last-rates/render.php';
			return ob_get_clean();
		}
		
		public function anbp_currency_converter_shortcode($atts){
			if(isset($atts['currencies'])){
				$atts['selectedCurrencies'] = explode(',', str_replace(' ', '', $atts['currencies']));
				unset($atts['currencies']);
			}
			$attributes = shortcode_atts(
				['selectedCurrencies' => ['USD','EUR','AUD','HKD','CAD','NZD','SGD','HUF','CHF','GBP','UAH','JPY','CZK','DKK','ISK','NOK','SEK','RON','BGN','TRY','ILS','CLP','PHP','MXN','ZAR','BRL','MYR','IDR','THB','INR','KRW','CNY','XDR'], 'attr' => 'class="wp-block-anbp-blocks-currency-converter"'],
				$atts,
				'anbp_currency_converter'
			);
			wp_enqueue_script( 'anbp-blocks-currency-converter-view-script', ANBP_DIR_PATH . '/blocks/currency-converter/view.js' );
			ob_start();
			include ANBP_DIR_PATH . '/blocks/currency-converter/render.php';
			return ob_get_clean();
		}
