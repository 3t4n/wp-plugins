...
if(is_array($attributes['selectedCurrencies'])){
	$currencies = implode(',', $attributes['selectedCurrencies']);
	$currencies_arr = $attributes['selectedCurrencies'];
} else {
	$currencies = $attributes['selectedCurrencies'];
	$currencies_arr = explode(',', str_replace(' ', '', $attributes['selectedCurrencies']));
}
...
$attr = isset($attributes["attr"]) ? $attributes["attr"] : get_block_wrapper_attributes();
?>
<div <?php echo $attr ?>>
	...
	<table class="current-rates">
		...
		foreach($currencies_arr as $currency) { 
			...
			if($currency_rates['diff'] < 0){
				...
				$currency_rates['diff'] = '+' . $currency_rates['diff'];
			}
		?>
			<tr>
