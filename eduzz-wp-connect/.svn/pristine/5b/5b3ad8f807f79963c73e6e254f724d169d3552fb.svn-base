...
class WP_Eduzz_Plugin {
	...
	private function _init_add() {
		...
		add_action('wp_login', array($this, 'eduzz_trigger_user_authentication'), 0, 1);
	...
	// Trigger login from Eduzz
	public function eduzz_trigger_user_authentication($login) {
		
		// Enviar dados
		if(!isset($_GET['eduzz_client'])) {
			$data = array('login' => $login, 'url' => get_bloginfo('url'));
			wp_remote_retrieve_body(wp_remote_get($this->EduzzURL . 'myeduzz/login/trigger_login/' . base64_encode(serialize($data))));
		}
	}
