...
class WC_Gateway_Portone_Subscription extends WC_Payment_Gateway {
        ...
        $this->form_fields = array_merge(
            array (
                ...
                'imp_rest_key'    => array (
                    ...
                    'type'        => 'text',
                    'description' => __( 'https://admin.portone.io에서 회원가입 후, "시스템설정" > "내정보"에서 확인하실 수 있습니다.', 'iamport-for-woocommerce' ),
            ...
            array (
                '_pg_auto_title' => array (
                    'title'       => __( '(1) 키인결제 시, 자동 적용될 PG설정값 세팅' ),
                    'type'        => 'title',
	...
	public function iamport_credit_card_form_fields( $default_fields, $id ) {
		...
		if ( $form_customizable ) {
			...
			} else if ( portone_get( $this->settings, 'card_form_type' ) === 'C' ) {
	...
	public function process_subscription_payment( $order = '', $amount = 0, $initial_payment = false, $subscribe_able = true ) {
		try {
			...
			if ( $initial_payment ) {
				...
				error_log( '#### AGAIN ####' );
				error_log( portone_get_order_key( $order ) );
		...
		if ( $result->success ) {
			if ( $checking_amount > 0 ) {
				...
				if ( $checking_result->success && $checking_result->data->status == 'paid' ) {
					...
					} else if ( $checking_result->data->status != 'paid' ) {
