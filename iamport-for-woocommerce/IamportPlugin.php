	...
	class Portone_For_WooCommerce {
		...
		function add_order_statuses( $order_statuses ) {
			...
			foreach ( $order_statuses as $key => $status ) {
				...
				if ( 'wc-cancelled' === $key ) {
					$label_refund          = Portone_Helper::display_label( Portone_Helper::STATUS_REFUND );
					$label_exchange        = Portone_Helper::display_label( Portone_Helper::STATUS_EXCHANGE );
					$label_vbank           = Portone_Helper::display_label( Portone_Helper::STATUS_AWAITING_VBANK );
					$label_address_changed = Portone_Helper::DEFAULT_STATUS_ADDRESS_CHANGED;;
