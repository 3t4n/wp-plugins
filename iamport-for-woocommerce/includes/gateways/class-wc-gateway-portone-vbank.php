...
//		$allCategories += Portone_Helper::get_all_categories();
    ...
    public function vbank_refund() {
        try {
            ...
            $params = array (
                'reason'         => $_REQUEST['refund_reason'],
                'imp_uid'        => $order->get_transaction_id(),
                'amount'         => $order->get_total(),
                'checksum'       => $order->get_total(),
                'refund_holder'  => $_REQUEST['refund_acc_name'],
                'refund_bank'    => $_REQUEST['refund_bank_code'],
                'refund_account' => $_REQUEST['refund_acc_num'],
            );
            ...
            if( $response->success ) {
                ...
                $order->update_meta_data( '_portone_vbank_refunded_code', $_REQUEST['refund_bank_code'] );
                $order->update_meta_data( '_portone_vbank_refunded_num', $_REQUEST['refund_acc_num'] );
                $order->update_meta_data( '_portone_vbank_refunded_name', $_REQUEST['refund_acc_name'] );
                $order->update_meta_data( '_portone_vbank_refunded_reason', $_REQUEST['refund_reason'] );
            ...
            throw new Exception( sprintf( __( '%s %s', 'iamport-for-woocommerce' ), $e->getCode(), $e->getMessage() ) );
        }
    }
}
