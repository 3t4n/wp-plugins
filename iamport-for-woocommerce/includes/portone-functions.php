...
function portone_get_order_row( $order_id ) {
	...
	if ( PORTONE_HPOS::enabled() ) {
		$synced_row = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wc_orders WHERE  id = {$order_id} FOR UPDATE", ARRAY_A );
	} else {
		$synced_row = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}posts WHERE ID = {$order_id} FOR UPDATE", ARRAY_A );
	}
...
function portone_send_usage_notification( $payment_gateway ) {
	...
	if ( ! empty( $merchant_id ) ) {
		...
		$params = array(
			"messageKey" => sprintf( "codemshop-%s%04d", date( "Ymd", strtotime( current_time( 'mysql' ) ) ), rand( 0, 9999 ) ),
			"pluginVer"  => $version,
...
if ( ! function_exists( 'iamport_order_endpoint_data' ) ) {
	function iamport_order_endpoint_data() {
		...
		if ( isset( $_GET['iamport-cancel-action'] ) && isset( $_GET['order-id'] ) && isset( $_GET['order-key'] ) ) {
			$iamport_cancel_action = wc_clean( $_GET['iamport-cancel-action'] );
			$order_id              = wc_clean( $_GET['order-id'] );
			$order_key             = wc_clean( $_GET['order-key'] );
			$page                  = isset( $_GET['order-page'] ) ? wc_clean( $_GET['order-page'] ) : '';
...
if ( ! function_exists( 'iamport_cancel_handle' ) ) {
	function iamport_cancel_handle() {
		...
		if ( isset( $_GET['iamport-cancel-action'] ) && isset( $_GET['order-key'] ) && isset( $_GET['order-id'] ) ) {
			$order_id = wc_clean( $_GET['order-id'] );
			$page     = isset( $_GET['order-page'] ) ? wc_clean( $_GET['order-page'] ) : '';
			$reason   = isset( $_GET['reason'] ) ? wc_clean( $_GET['reason'] ) : '구매자 요청';

			$order = new WC_Order( $order_id );
            if( is_a( $order, 'WC_Order' ) && portone_get_order_key( $order ) == wc_clean( $_GET['order-key'] ) ) {
...
if ( ! function_exists( 'ajax_iamport_vbank_refund' ) ) {
    function ajax_iamport_vbank_refund() {
        try {
            ...
            wp_send_json_error( sprintf( __( '[%s] %s', 'iamport-for-woocommerce' ), $e->getCode(), $e->getMessage() ) );
        }
    }
}
