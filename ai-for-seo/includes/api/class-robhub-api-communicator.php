...
class Ai4Seo_RobHubApiCommunicator {
    ...
    function init_free_account(): bool {
        ...
        $plugin_activation_time = (int) get_option(AI4SEO_PLUGIN_ACTIVATION_TIME_OPTION_NAME, 0);

        $parameters = array(
            "plugin_activation_time" => $plugin_activation_time,
    ...
    function use_this_credentials(string $api_username, string $api_password, bool $update_in_database = false): bool {
        ...
        if ($update_in_database) {
            return $this->update_environmental_variable(self::ENVIRONMENTAL_VARIABLE_AUTH_DATA, array($api_username, $api_password));
        }
    ...
    function read_all_environmental_variables(): array {
        ...
        foreach ($this->environmental_variables as $environmental_variable_name => $environmental_variable_value) {
    ...
    function update_environmental_variable(string $environmental_variable_name, $new_environmental_variable_value): bool {
        ...
        if (!isset($current_environmental_variables[$environmental_variable_name])) {
            error_log("ROBHUB: Environmental variable '" . $environmental_variable_name . "' does not exist. #4918251124");
            return false;
        }
