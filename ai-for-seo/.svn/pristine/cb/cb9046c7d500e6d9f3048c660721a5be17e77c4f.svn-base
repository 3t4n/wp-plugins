...
$ai4seo_current_automated_generation_order = ai4seo_get_automated_generation_order($ai4seo_post_type);
$ai4seo_current_automated_generation_new_or_existing_filter = ai4seo_get_automated_generation_new_or_existing_filter($ai4seo_post_type);
$ai4seo_automated_generation_new_or_existing_filter_reference_timestamp = ai4seo_get_automated_generation_new_or_existing_filter_reference_timestamp($ai4seo_post_type);
...
echo "<div class='ai4seo-automated-generation-settings-wrapper'>";

    echo "<h2 style='margin-top: 0;'>" . esc_html__("Bulk Generation", "ai-for-seo") . "</h2>";

        // output selection for automated generation new or existing filter
    echo "<label for='ai4seo-automated-generation-new-or-existing-filter-select'>";
        echo "<span>" . esc_html__("Generate media attributes for:", "ai-for-seo") . "</span> ";
        echo "<select id='ai4seo-automated-generation-new-or-existing-filter-select' onchange='ai4seo_update_automated_generation_new_or_existing_filter(\"" . esc_js($ai4seo_post_type) . "\", this.value);ai4seo_show_loading_icons(jQuery(this).parent().find(\".ai4seo-hidden-loading-icon-holder\"));' class='ai4seo-lockable'>";
            foreach (AI4SEO_AVAILABLE_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER_OPTIONS as $ai4seo_this_new_or_existing_filter_option => $ai4seo_this_new_or_existing_filter_option_label) {
        ...
        echo "</select>";
        echo ai4seo_get_icon_with_tooltip_tag(esc_html__("Choose whether to generate media attributes for existing media, newly uploaded media, or both. Changing this setting marks all current media files as 'existing', while any media files uploaded afterward are considered 'new'. This distinction is refreshed each time the setting is changed, except for direct swaps between 'New entries only' and 'Existing entries only'.", "ai-for-seo"));

        // Loading icon
        ai4seo_echo_loading_icon_output();
    echo "</label>";

    echo "<br><br>";

    // output selection for automated generation order
    echo "<label for='ai4seo-automated-generation-order-select'>";
        echo "<span>" . esc_html__("Order of bulk generation:", "ai-for-seo") . "</span> ";
        echo "<select id='ai4seo-automated-generation-order-select' onchange='ai4seo_update_automated_generation_order(\"" . esc_js($ai4seo_post_type) . "\", this.value);ai4seo_show_loading_icons(jQuery(this).parent().find(\".ai4seo-hidden-loading-icon-holder\"));' class='ai4seo-lockable'>";
            foreach (AI4SEO_AVAILABLE_AUTOMATED_GENERATION_ORDER_OPTIONS as $ai4seo_this_order_option => $ai4seo_this_order_option_label) {
...
echo "</label>";
    ...
    echo "<label for='ai4seo-toggle-automated-generation-checkbox'>";
        echo "<input type='checkbox' id='ai4seo-toggle-automated-generation-checkbox' onchange='ai4seo_toggle_automated_generation(\"" . esc_js($ai4seo_post_type) . "\", this.checked);ai4seo_show_loading_icons(jQuery(this).parent().find(\".ai4seo-hidden-loading-icon-holder\"));' class='ai4seo-lockable' " . esc_attr($ai4seo_is_bulk_generation_checked_phrase) . "/>";
        echo sprintf(esc_html__("Turn on bulk generation for media attributes.", "ai-for-seo"), esc_html($ai4seo_media_label_plural));
        echo " ";
        echo sprintf(esc_html__("Approximate processing speed: %.1f %s/min.", "ai-for-seo"), esc_html(AI4SEO_APPROXIMATE_ATTACHMENT_ATTRIBUTES_GENERATION_SPEED), esc_html($ai4seo_media_label_plural));

        if ($ai4seo_last_cron_job_call_timestamp && $ai4seo_is_bulk_generation_activated) {
            echo " ";
            echo sprintf(
                esc_html__("Last bulk generation run: %s.", "ai-for-seo"),
                ai4seo_format_unix_timestamp($ai4seo_last_cron_job_call_timestamp)
            );
        }
...
echo "<table class='widefat striped table-view-list attachments ai4seo-posts-table'>";
    ...
    foreach ($ai4seo_all_attachment_posts as $ai4seo_this_attachment) {
        ...
        if ($ai4seo_current_automated_generation_new_or_existing_filter == "new" && $ai4seo_this_attachment_date_timestamp <= $ai4seo_automated_generation_new_or_existing_filter_reference_timestamp) {
            $ai4seo_is_excluded_by_new_or_existing_filter = true;
        } else if ($ai4seo_current_automated_generation_new_or_existing_filter == "existing" && $ai4seo_this_attachment_date_timestamp > $ai4seo_automated_generation_new_or_existing_filter_reference_timestamp) {
            $ai4seo_is_excluded_by_new_or_existing_filter = true;
        }
