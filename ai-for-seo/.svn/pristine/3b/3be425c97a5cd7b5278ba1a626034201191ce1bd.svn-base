...
const AI4SEO_PLUGIN_VERSION_NUMBER = "1.2.14";
...
const AI4SEO_SETTING_AUTOMATED_GENERATION_ORDER = 'automated_generation_order';
const AI4SEO_SETTING_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER = 'automated_generation_new_or_existing_filter';
...
const AI4SEO_DEFAULT_SETTINGS = array(
    ...
    AI4SEO_SETTING_AUTOMATED_GENERATION_ORDER => array(),
    AI4SEO_SETTING_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER => array(),
...
const AI4SEO_DEFAULT_CONTEXT_SETTING_AUTOMATED_GENERATION_ORDER = "newest";
const AI4SEO_DEFAULT_CONTEXT_SETTING_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER = "both";
...
add_action("init", function() {
    define('AI4SEO_AVAILABLE_AUTOMATED_GENERATION_ORDER_OPTIONS', array(
    ...
    define('AI4SEO_AVAILABLE_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER_OPTIONS', array(
            "both" => __("New entries and existing", "ai-for-seo"),
...
const AI4SEO_ENVIRONMENTAL_VARIABLE_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER_REFERENCE_TIMES = "automated_generation_new_or_existing_filter_reference_times";
...
const AI4SEO_DEFAULT_ENVIRONMENTAL_VARIABLES = array(
    ...
    AI4SEO_ENVIRONMENTAL_VARIABLE_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER_REFERENCE_TIMES => array(),
);
...
const AI4SEO_ALLOWED_AJAX_FUNCTIONS = array(
    ...
    "ai4seo_update_automated_generation_order_via_ajax",
    "ai4seo_update_automated_generation_new_or_existing_filter_via_ajax",
    "ai4seo_submit_licence_key",
    "ai4seo_display_licence_information",
    ...
    "ai4seo_submit_troubleshooting_form",
);
...
function ai4seo_modify_html_header_adding_our_meta_tags(string $full_html_buffer): string {
    ...
    if ($add_this_metadata) {
        ...
        if ($add_this_meta_tags) {
            ...
            array_unshift($add_this_meta_tags, "\n\n\t<!-- [" . esc_html(AI4SEO_PLUGIN_NAME) . "] This site is optimized with the \"AI for SEO\" plugin v" . esc_html(AI4SEO_PLUGIN_VERSION_NUMBER) . " - " . esc_html(AI4SEO_OFFICIAL_WEBSITE) . " -->");
            $add_this_meta_tags[] = "<!-- [" . esc_html(AI4SEO_PLUGIN_NAME) . "] End -->";
...
function ai4seo_echo_inefficient_cron_jobs_notice() {
    ...
    if (ai4seo_is_wordpress_cron_disabled()) {
        $ai4seo_cron_job_too_slow_notification = esc_html__("Your server cron jobs do not appear to be functioning properly, limiting AI for SEO automation. Please ensure that server cron jobs run at least every 5 minutes (1 minute for best results) or (not recommended) enable WordPress' internal cron system.", "ai-for-seo");
    } else {
        $ai4seo_cron_job_too_slow_notification = esc_html__("The last bulk generation run occurred more than 10 minutes ago, which may indicate that WordPress' internal cron jobs are running too infrequently. We recommend switching to a server-based cron job during bulk generation and setting it to run at least every 5 minutes (every 1 minute for the fastest results). If you need assistance, please contact your hosting provider. If this is not an option, please check Help > Troubleshooting > Bulk Generation Duration for a workaround.", "ai-for-seo");
    }
...
function ai4seo_automated_metadata_generation($debug = false, $only_this_post_id = 0): bool {
    ...
    if ($generate_metadata_for_fully_covered_entries) {
        ...
        if (in_array($post_id, $generated_metadata_post_ids)) {
            if ($debug) {
    ...
    } else if (ai4seo_read_is_active_metadata_fully_covered($post_id)) {
...
function ai4seo_excavate_post_entries_with_missing_metadata(bool $debug = false): bool {
    ...
    foreach ($enabled_generation_post_types AS $this_post_type)  {
        // check order
        $automated_generation_order = ai4seo_get_automated_generation_order($this_post_type);

        switch ($automated_generation_order) {
            case "oldest":
        ...
        $automated_generation_new_or_existing_filter = ai4seo_get_automated_generation_new_or_existing_filter($this_post_type);
        $automated_generation_new_or_existing_filter_reference_timestamp = ai4seo_get_automated_generation_new_or_existing_filter_reference_timestamp($this_post_type);

        if ($automated_generation_new_or_existing_filter != "both" && $automated_generation_new_or_existing_filter_reference_timestamp && is_numeric($automated_generation_new_or_existing_filter_reference_timestamp)) {
            // determine $post_date_term_string, if $automated_generation_new_or_existing_filter is "new", the post has to be newer than $automated_generation_new_or_existing_filter_reference_timestamp
            // and if $automated_generation_new_or_existing_filter is "existing", the post has to be older than (or exactly) $automated_generation_new_or_existing_filter_reference_timestamp
            if ($automated_generation_new_or_existing_filter == "new") {
                $post_date_term_string = "post_date_gmt > '" . date("Y-m-d H:i:s", $automated_generation_new_or_existing_filter_reference_timestamp) . "'";
            } else {
                $post_date_term_string = "post_date_gmt <= '" . date("Y-m-d H:i:s", $automated_generation_new_or_existing_filter_reference_timestamp) . "'";
            }
        } else {
...
function ai4seo_excavate_attachments_with_missing_attributes(bool $debug = false): bool {
    ...
    // check order
    $automated_generation_order = ai4seo_get_automated_generation_order('attachment');
    ...
    $automated_generation_new_or_existing_filter = ai4seo_get_automated_generation_new_or_existing_filter('attachment');
    $automated_generation_new_or_existing_filter_reference_timestamp = ai4seo_get_automated_generation_new_or_existing_filter_reference_timestamp('attachment');
 ...
 * @param bool $debug if true, debug information will be printed
 * @param bool $refresh_all_posts_seo_coverage if true, all posts will be refreshed
 * @return bool true on success, false on failure
 */
function ai4seo_analyze_plugin_performance(bool $debug = false, bool $refresh_all_posts_seo_coverage = true): bool {
...
function ai4seo_create_empty_attachment_attributes_coverage_array(array $attachment_post_ids): array {
    ...
    foreach ($attachment_post_ids as $post_id) {
        ...
        foreach (AI4SEO_ATTACHMENT_ATTRIBUTES_DETAILS as $this_attachment_attribute_name => $this_attachment_attribute_details) {
...
function ai4seo_get_num_attachment_attributes(): int {
    return defined('AI4SEO_ATTACHMENT_ATTRIBUTES_DETAILS') ? count(AI4SEO_ATTACHMENT_ATTRIBUTES_DETAILS) : 0;
}
...
function ai4seo_reset_plugin_data() {
    ...
    if (isset($_POST["ai4seo_reset_environmental_variables"]) && $_POST["ai4seo_reset_environmental_variables"] === "true") {
        delete_option(AI4SEO_ROBHUB_ENVIRONMENTAL_VARIABLES_OPTION_NAME);
    ...
    if (isset($_POST["ai4seo_reset_metadata"]) && $_POST["ai4seo_reset_metadata"] === "true") {
        ...
        // do a new run and analyze all posts
        ai4seo_analyze_plugin_performance();
    }
...
/**
 * Called via AJAX - Updates the automated generation order
 * @return void
 */
function ai4seo_update_automated_generation_order_via_ajax() {
    // Make sure that this function is only called once
    if (!ai4seo_singleton(__FUNCTION__)) {
        return;
    }

    require_once(ai4seo_get_includes_ajax_process_path("update-automated-generation-order.php"));
    wp_die();
}
...
/**
 * Called via AJAX - Save troubleshooting form data
 * @return void
 */
function ai4seo_submit_troubleshooting_form() {
    ...
    // check for ai4seo_bulk_generation_duration, save to setting
    if (isset($_POST["ai4seo_bulk_generation_duration"])) {
        $ai4seo_bulk_generation_duration = (int) ($_POST["ai4seo_bulk_generation_duration"]);
        ai4seo_update_setting(AI4SEO_SETTING_BULK_GENERATION_DURATION, $ai4seo_bulk_generation_duration);
    }
...
/**
 * Called via AJAX - Updates the automated generation new or existing filter
 * @return void
 */
function ai4seo_update_automated_generation_new_or_existing_filter_via_ajax() {
    // Make sure that this function is only called once
    if (!ai4seo_singleton(__FUNCTION__)) {
        return;
    }

    require_once(ai4seo_get_includes_ajax_process_path("update-automated-generation-new-or-existing-filter.php"));
    wp_die();
}
...
function ai4seo_display_licence_information() {
    // Make sure that this function is only called once
    if (!ai4seo_singleton(__FUNCTION__)) {
        return;
    }

    ai4seo_update_environmental_variable(AI4SEO_ENVIRONMENTAL_VARIABLE_LICENCE_KEY_SHOWN, false);
    wp_send_json_success();
}
...
function ai4seo_show_terms_of_service() {
    ...
    // headline
    echo "<center>";
    echo "<img src='" . esc_url(ai4seo_get_ai_for_seo_logo_url("full")) . "' class='ai4seo-tos-plugin-logo'><br>";
...
function ai4seo_validate_setting_value(string $setting_name, $setting_value): bool {
    switch ($setting_name) {
        ...
        case AI4SEO_SETTING_BULK_GENERATION_DURATION:
            ...
            return is_int($setting_value) && $setting_value >= 10 && $setting_value <= 300;
        ...
        case AI4SEO_SETTING_AUTOMATED_GENERATION_ORDER:
            // Make sure that the new setting-value is an array
            if (!is_array($setting_value)) {
                error_log("AI4SEO: Setting value for setting '" . $setting_name . "' is not an array. #2711171224");
                return false;
            }

            // Make sure the keys consist of alphanumeric strings, with - and _ allowed and the values should be "1" or "0" only
            foreach ($setting_value as $context => $value) {
                if (!preg_match("/^[a-zA-Z0-9_-]+$/", $context)) {
                    error_log("AI4SEO: Invalid context in the automated generations order setting. #2811171224");
                    return false;
                }

                if (!defined('AI4SEO_AVAILABLE_AUTOMATED_GENERATION_ORDER_OPTIONS') || !isset(AI4SEO_AVAILABLE_AUTOMATED_GENERATION_ORDER_OPTIONS[$value])) {
                    error_log("AI4SEO: Invalid value in the automated generations order setting. #2911171224");
                    return false;
                }
            }
        ...
        case AI4SEO_SETTING_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER:
            // Make sure that the new setting-value is an array
            if (!is_array($setting_value)) {
                error_log("AI4SEO: Setting value for setting '" . $setting_name . "' is not an array. #3011171224");
                return false;
            }
            ...
            foreach ($setting_value as $context => $value) {
                if (!preg_match("/^[a-zA-Z0-9_-]+$/", $context)) {
                    error_log("AI4SEO: Invalid context in the automated generations new or existing filter setting. #3111171224");
                    return false;
                }

                if (!defined('AI4SEO_AVAILABLE_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER_OPTIONS') || !isset(AI4SEO_AVAILABLE_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER_OPTIONS[$value])) {
 ...
 */
function ai4seo_is_bulk_generation_enabled(string $context): bool {
...
function ai4seo_is_any_bulk_generation_enabled(): bool {
    $enabled_automated_generations = ai4seo_get_setting(AI4SEO_SETTING_ENABLED_AUTOMATED_GENERATIONS) ?: array();
    return in_array("1", $enabled_automated_generations);
}
...
/**
 * Function to get the order direction for a specific automated generation context
 * @param string $context The context of the automated generation
 * @return string The order direction
 */
function ai4seo_get_automated_generation_order(string $context): string {
    $automated_generation_order_direction_setting = ai4seo_get_setting(AI4SEO_SETTING_AUTOMATED_GENERATION_ORDER) ?: array();
    return $automated_generation_order_direction_setting[$context] ?? AI4SEO_DEFAULT_CONTEXT_SETTING_AUTOMATED_GENERATION_ORDER;
}

// =========================================================================================== \\

/**
 * Function to update the order direction for a specific automated generation context
 * @param string $context The context of the automated generation
 * @param string $order_direction The order direction
 * @return bool True if the order direction was updated, false if not
 */
function ai4seo_update_automated_generation_order_direction(string $context, string $order_direction): bool {
    $automated_generation_order_direction_setting = ai4seo_get_setting(AI4SEO_SETTING_AUTOMATED_GENERATION_ORDER) ?: array();
    $automated_generation_order_direction_setting[$context] = $order_direction;
    return ai4seo_update_setting(AI4SEO_SETTING_AUTOMATED_GENERATION_ORDER, $automated_generation_order_direction_setting);
}

// =========================================================================================== \\

/**
 * Function to get the new or existing filter for a specific automated generation context
 * @param string $context The context of the automated generation
 * @return string The new or existing filter
 */
function ai4seo_get_automated_generation_new_or_existing_filter(string $context): string {
    $automated_generation_new_or_existing_filter_setting = ai4seo_get_setting(AI4SEO_SETTING_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER) ?: array();
    return $automated_generation_new_or_existing_filter_setting[$context] ?? AI4SEO_DEFAULT_CONTEXT_SETTING_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER;
}

// =========================================================================================== \\

/**
 * Function to update the new or existing filter for a specific automated generation context
 * @param string $context The context of the automated generation
 * @param string $new_or_existing_filter The new or existing filter value
 * @return bool True if the new or existing filter was updated, false if not
 */
function ai4seo_update_automated_generation_new_or_existing_filter(string $context, string $new_or_existing_filter): bool {
    $automated_generation_new_or_existing_filter_setting = ai4seo_get_setting(AI4SEO_SETTING_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER) ?: array();
    $automated_generation_new_or_existing_filter_setting[$context] = $new_or_existing_filter;
    return ai4seo_update_setting(AI4SEO_SETTING_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER, $automated_generation_new_or_existing_filter_setting);
}

// =========================================================================================== \\

/**
 * Function to get the reference timestamp of a recently changed new or existing filter
 * @param string $context The context of the automated generation
 * @return int The reference timestamp of a recently changed new or existing filter
 */
function ai4seo_get_automated_generation_new_or_existing_filter_reference_timestamp(string $context): int {
    $automated_generation_new_or_existing_filter_reference_timestamps = ai4seo_read_environmental_variable(AI4SEO_ENVIRONMENTAL_VARIABLE_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER_REFERENCE_TIMES) ?: array();
    return $automated_generation_new_or_existing_filter_reference_timestamps[$context] ?? 0;
}

// =========================================================================================== \\

/**
 * Function to update the reference timestamp of a recently changed new or existing filter
 * @param string $context The context of the automated generation
 * @param int $reference_timestamp The reference timestamp of a recently changed new or existing filter
 * @return bool True if the reference timestamp was updated, false if not
 */
function ai4seo_update_automated_generation_new_or_existing_filter_reference_timestamp(string $context, int $reference_timestamp): bool {
    $automated_generation_new_or_existing_filter_reference_timestamps = ai4seo_read_environmental_variable(AI4SEO_ENVIRONMENTAL_VARIABLE_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER_REFERENCE_TIMES) ?: array();
    $automated_generation_new_or_existing_filter_reference_timestamps[$context] = $reference_timestamp;
    return ai4seo_update_environmental_variable(AI4SEO_ENVIRONMENTAL_VARIABLE_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER_REFERENCE_TIMES, $automated_generation_new_or_existing_filter_reference_timestamps);
}
...
function ai4seo_update_environmental_variable(string $environmental_variable_name, $new_environmental_variable_value): bool {
    ...
    if (!isset($current_environmental_variables[$environmental_variable_name])) {
        error_log("AI4SEO: Environmental variable name not found: " . $environmental_variable_name . ". #5018251124");
        return false;
    }
...
function ai4seo_validate_environmental_variable_value(string $environmental_variable_name, $environmental_variable_value): bool {
    switch ($environmental_variable_name) {
        ...
        case AI4SEO_ENVIRONMENTAL_VARIABLE_AUTOMATED_GENERATION_NEW_OR_EXISTING_FILTER_REFERENCE_TIMES:
            // Make sure that the new setting-value is an array
            if (!is_array($environmental_variable_value)) {
                error_log("AI4SEO: Environmental variable value for '" . $environmental_variable_name . "' is not an array. #5513171224");
                return false;
            }

            // Make sure the keys consist of alphanumeric strings, with - and _ allowed and the values should be a valid timestamp
            foreach ($environmental_variable_value as $context => $value) {
                if (!preg_match("/^[a-zA-Z0-9_-]+$/", $context)) {
                    error_log("AI4SEO: Invalid context in the automated generations new or existing filter reference times setting. #5613171224");
                    return false;
                }
...
function ai4seo_show_terms_of_service_modal() {
    ...
    $latest_tos_and_toc_and_pp_version = ai4seo_get_latest_tos_and_toc_and_pp_version();
    $content .= "<img src='" . esc_url(ai4seo_get_ai_for_seo_logo_url("full")) . "' class='ai4seo-tos-plugin-logo'>";
    $content .= "<h1>" . esc_html__("Terms of Service", "ai-for-seo") . "</h1>";

    if ($tos_toc_and_pp_accepted_time) {
        $content .= "<p>" . sprintf(esc_html__("Please review and accept the updated Terms of Service, effective from %s.", "ai-for-seo"), $datetime_of_change) . "</p>";
    } else {
        $content .= "<p>" . sprintf(esc_html__("Please accept the Terms of Service to start using the %s plugin.", "ai-for-seo"), "<span class='ai4seo-plugin-name'>" . AI4SEO_PLUGIN_NAME . "</span>") . "</p>";
    }

    $content .= "<div class='ai4seo-tos-box'>";

    $content .= "<div class='ai4seo-tos-version-number'>" . esc_html($latest_tos_and_toc_and_pp_version) . "</div>";
    ...
    if ($tos_content) {
        ...
        $content .= "<p>" . esc_html__("The Terms of Service could not be loaded. Please try again later.", "ai-for-seo") . "</p>";
    }
    ...
    $content .= "<div class='ai4seo-tos-checkboxes-wrapper'>";
        ...
        $content .= "<div class='ai4seo-tos-checkbox ai4seo-accept-tos-checkbox-wrapper'>";
            $content .= "<input type='checkbox' class='ai4seo-accept-tos-checkbox' id='ai4seo-accept-tos-checkbox' name='ai4seo-accept-tos-checkbox' value='1' onchange='ai4seo_refresh_tos_accept_button_state();'>";
            $content .= "<label for='ai4seo-accept-tos-checkbox'><strong>" . esc_html__("I have read and agree to the Terms and Service.", "ai-for-seo") . "</strong></label>";
        $content .= "</div>";
        ...
        if (!$tos_toc_and_pp_accepted_time || !$enhanced_reporting_accepted) {
            ...
            $extended_data_collection_tooltip_text = esc_html__("This data includes feature usage, performance metrics, and error logs. It will be stored for up to 30 days to assist with improving the plugin. You can opt out of data collection at any time through the plugin settings.", "ai-for-seo");
            $content .= "<div class='ai4seo-tos-checkbox'>";
                $content .= "<input type='checkbox' class='ai4seo-accept-enhanced-reporting-checkbox' id='ai4seo-accept-enhanced-reporting-checkbox' name='ai4seo-accept-enhanced-reporting-checkbox' value='1'>";
                $content .= "<label for='ai4seo-accept-enhanced-reporting-checkbox'><strong>" . esc_html__("I agree to share extended data to support the ongoing development of the plugin. I may opt out at any time.", "ai-for-seo") . " (" . esc_html__("optional", "ai-for-seo") . ")" . "</strong></label>";
                $content .= ai4seo_get_icon_with_tooltip_tag($extended_data_collection_tooltip_text);
            $content .= "</div>";
        }
    $content .= "</div>";
    ...
    $content .= "<button type='button' onclick='ai4seo_confirm_to_reject_tos();' class='button ai4seo-button ai4seo-abort-button'>" . esc_html__("Reject & Uninstall", "ai-for-seo") . "</button>";
    $content .= "</div>";
    ...
    if (!$is_user_inside_plugin_admin_pages && !$is_user_inside_installed_plugins_page) {
    ...
    // Right button
    $content .= "<div id='ai4seo-modal-right-button' class='ai4seo-modal-right-button' onclick='ai4seo_check_if_user_accepted_tos();'>";
    $content .= "<button type='button' onclick='ai4seo_accept_tos(" . esc_js($additional_accept_tos_parameter) . ");" . esc_js($additional_accept_tos_javascript) . "' class='button ai4seo-button ai4seo-disabled-button ai4seo-modal-submit-button ai4seo-accept-tos-button'>" . esc_html__("Accept & Continue", "ai-for-seo") . "</button>";
    $content .= "</div>";
