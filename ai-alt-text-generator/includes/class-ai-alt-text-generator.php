...
class AATG_Text_Generator {
	...
	public function generate_alt_text_ajax() {
		...
		error_log('Debug - Processing request with URL: ' . $image_url);
		...
		error_log('Debug - Attempting to generate alt text for URL: ' . $image_data['url']);
		...
		try {
			...
			error_log('Alt text generation error: ' . $e->getMessage());
	...
	private function generate_alt_text_from_image($image_url, $api_key) {
		...
		error_log('Debug - Calling AI service for URL: ' . $image_url);
