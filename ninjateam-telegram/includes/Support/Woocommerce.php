<?php
namespace NTA_Telegram\Support;
...
class Woocommerce {
	...
	public function init() {
		...
		add_filter( 'njt_telegram_is_page_or_shop_filter', array( $this, 'isPageOrShop' ), 10, 1 );
		add_filter( 'njt_telegram_get_post_id_filter', array( $this, 'getPostId' ), 10, 1 );
	...
	public function showAfterShortDescription( $post_excerpt ) {
		...
		foreach ( $this->activeAccounts as $row ) {
			$btnContent .= '<div class="nta-woo-products-button">' . do_shortcode( '[njtele_button id="' . $row->ID . '"]' ) . '</div>';
		}
	...
	public function insert_button() {
		...
		foreach ( $this->activeAccounts as $row ) {
			echo '<div class="nta-woo-products-button">' . do_shortcode( '[njtele_button id="' . esc_attr( $row->ID ) . '"]' ) . '</div>';
		}
	}
}
