...
if ( ! defined( 'ALUMNIONLINE_REUNION_WEBSITE_VERSION_NUM' ) ) {
	define( 'ALUMNIONLINE_REUNION_WEBSITE_VERSION_NUM', '1.0.9 BETA' ); // Plugin version constant.
}
...
function alumnionline_reunion_website_enqueue_css_js( $hook ) {
	...
	wp_localize_script(
		...
		array(
			...
			'fileerrortype' => sprintf( '%s Only', esc_attr( $file_formats ) ),
			'file_formats' => explode(',', esc_attr( $file_formats ) ),
		)
	);
