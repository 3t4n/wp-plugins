...
<main id="<?php echo esc_attr( $maincontentid ); ?>">
	<div class="alumnonline-reunion-plugin-message-post-wrapper">
		...
		<div class="alumnonline-reunion-plugin-featured-image">
				...
				if ( function_exists( 'alumnionline_reunion_website_create_attachment_list' ) ) {
					$attachments = alumnionline_reunion_website_create_attachment_list( $post->ID );
					$trustedtags = alumnionline_reunion_website_get_trusted_tags_array();
					echo wp_kses( $attachments, $trustedtags );
				}
				...
				if ( function_exists( 'alumnionline_reunion_website_create_video_playlist' ) ) {
					$videos = alumnionline_reunion_website_create_video_playlist( $post->ID );
					$trustedtags = alumnionline_reunion_website_get_trusted_tags_array();
					echo wp_kses( $videos, $trustedtags );
				}
			}
		}
