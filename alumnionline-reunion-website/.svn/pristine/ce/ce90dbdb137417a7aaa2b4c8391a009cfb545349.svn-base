...
function alumnionline_reunion_website_classmate_search() {
	...
	foreach ( $user_array as $user ) {
		...
		foreach ( $customfields as $key => $value ) {
			if ( strstr( $key, 'alumnionline_' )
			&& ! strstr( $key, '_alumnionline_' )
			&& ! strstr( $key, 'alumnionline_school_id' )
			&& ! strstr( $key, 'alumnionline_notifications' )
			&& ! strstr( $key, 'alumnionline_privacy' )
			&& ! strstr( $key, 'alumnionline_reunion_planner_access' )
			) {
...
function alumnionline_reunion_website_display_custom_user_profile_fields( $user ) {
	if ( ! function_exists( 'alumnionline_reunion_website_pro_display_custom_user_profile_fields' ) ) {
		...
		?>
<div id="alumnionline_school_information">
 ...
 **/
function alumnionline_reunion_website_cleanup_remove_admin_bar() {
...
add_action( 'after_setup_theme', 'alumnionline_reunion_website_cleanup_remove_admin_bar' );

/**
 * Remove unwanted content from the profile screen
 **/
function alumnionline_reunion_website_cleanup_profile_screen() {
	remove_action( 'admin_color_scheme_picker', 'admin_color_scheme_picker' );
}
add_filter( 'user_edit_form_tag', 'alumnionline_reunion_website_cleanup_profile_screen' );

/**
 * Change the logo link
 */
function alumnionline_reunion_website_login_url() {
	return get_bloginfo( 'url' );
}
add_filter( 'login_headerurl', 'alumnionline_reunion_website_login_url' );

/**
 * Change the alt text on the logo
 */
function alumnionline_reunion_website_login_title() {
	return get_option( 'blogname' );
}
add_filter( 'login_headertitle', 'alumnionline_reunion_website_login_title' );

/**
 * Change the logo
 */
function alumnionline_reunion_website_custom_login_logo() {
	...
	add_action( 'login_head', 'alumnionline_reunion_website_custom_login_logo' );

/**
 * Display website header on profile editor screen
 */
function alumnionline_reunion_website_frontheader() {
	global $pagenow;
	if ( 'profile.php' === $pagenow ) {
		get_header();
	}
}
add_action( 'admin_head', 'alumnionline_reunion_website_frontheader' );

	/**
	 * Removes the leftover 'Visual Editor', 'Keyboard Shortcuts' and 'Toolbar' options.
	 */
function alumnionline_reunion_website_cleanup_profile_screen_options( $html ) {

	$html = preg_replace( '#<h2>Personal Options</h2>.+?/table>#s', '', $html, 1 );

	$html = preg_replace( '#<h2>Account Management</h2>#s', '<h2 class="alumnionline_account_management">Account Management</h2>', $html, 1 );
	...
	// remove application password section.
	foreach ( $selector->query( '//div[contains(attribute::class, "application-passwords")]' ) as $e ) {
		$e->parentNode->removeChild( $e );
	}
	// remove footer section.
	foreach ( $selector->query( '//div[contains(attribute::id, "wpfooter")]' ) as $e ) {
		$e->parentNode->removeChild( $e );
	}

	// remove admin menu.
	foreach ( $selector->query( '//div[contains(attribute::id, "adminmenumain")]' ) as $e ) {
		$e->parentNode->removeChild( $e );
	}

	// remove admin bar.
	foreach ( $selector->query( '//div[contains(attribute::id, "wpadminbar")]' ) as $e ) {
		$e->parentNode->removeChild( $e );
	}

			// remove help link.
	foreach ( $selector->query( '//div[contains(attribute::id, "contextual-help-link")]' ) as $e ) {
		$e->parentNode->removeChild( $e );
	}
...
function alumnionline_reunion_website_cleanup_profile_screen_start() {
	ob_start( 'alumnionline_reunion_website_cleanup_profile_screen_options' );
}

function alumnionline_reunion_website_cleanup_profile_screen_end() {
	ob_end_flush();
}
if ( isset( $pagenow ) && 'profile.php' === $pagenow ) {
	add_action( 'admin_head-profile.php', 'alumnionline_reunion_website_cleanup_profile_screen_start' );
	add_action( 'admin_footer-profile.php', 'alumnionline_reunion_website_cleanup_profile_screen_end' );
}
?>
