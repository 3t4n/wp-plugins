...
function alumnionline_reunion_website_create_gallery_code( $post_id ) {
	...
	if ( '' != $ids ) {
		$gallery = '[gallery mode="custom" ids="' . rtrim( $ids, ',' ) . '" link="file"]';
		return $gallery;
	}
}
 ...
 **/
function alumnionline_reunion_website_create_attachment_list( $post_id ) {

	$attachmentlist = '';

	$attachments = get_attached_media( 'application', $post_id );

	foreach ( $attachments as $attachment ) {

		$title = get_the_title( $attachment->ID );
		if ( '' === $title ) {
			$title = __( 'Attachment', 'alumnionline-reunion-website' );
		}
		$attachmentlist .= '<br><a href="' . esc_url_raw( wp_get_attachment_url( $attachment->ID ) ) . '">' . esc_attr( $title ) . '</a>';
	}
 ...
 * Get video documents
 **/
function alumnionline_reunion_website_create_video_playlist( $post_id ) {
	$videolist = '';
	$videos    = get_attached_media( 'video', $post_id );

	foreach ( $videos as $video ) {
		$videolist .= '<p><video width="320" height="240" controls>
  <source src="' . esc_url_raw( wp_get_attachment_url( $video->ID ) ) . '" type="video/mp4">
...
function alumnionline_reunion_website_display_post_overlay() {
	...
	if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
		...
		foreach ( $terms as $term ) {
			...
			$formats    = explode( ',', $settings['file_formats'] );
	foreach ( $formats as $key => $value ) {
		$valueformat = explode( '/', $value );
		if ( strlen( $valueformat[1] ) === 3 ) {
			$formattext .= $valueformat[1] . ', ';
		}
	}
	$formattext = rtrim( $formattext, ', ' );
	...
	if ( ! isset( $settings['alumnionline_disable_photos'] ) || 'false' == $settings['alumnionline_disable_photos'] || '' == $settings['alumnionline_disable_photos'] ) {
		echo '<p><label for="alumnionline-reunion-website-upload">';
		echo esc_html__( 'Choose Files: (', 'alumnionline-reunion-website' );
		echo esc_attr( $formattext );
		echo esc_html__( ' Only)(max size: ', 'alumnionline-reunion-website' );
		echo esc_attr( $maxsize );
		echo esc_html__( 'MB)', 'alumnionline-reunion-website' );
		$formats = str_replace( ',', '|', $settings['file_formats'] );
		echo '<input name="alumnionline-reunion-website-upload[]" id="alumnionline-reunion-website-upload" type="file" accept="' . esc_attr( $formats ) . '" multiple /></label></p>';
	}
	echo '<p>';
	if ( current_user_can( 'manage_options' ) ) {
...
function alumnionline_reunion_website_forum_index() {
	...
	// echo '<h2 id="pageName">' . esc_html__( 'Message Board', 'alumnionline-reunion-website' ) . ' <span id="alumnionline-reunion-website-category-title"><span></h2>';
		...
		$settings = alumnionline_reunion_website_get_settings();
		if ( shortcode_exists('alumnionline_media_archive') && isset( $settings['alumnionline_media_page'] ) && '' !== $settings['alumnionline_media_page'] ) {
			echo '<a href="' . esc_url( get_the_permalink( $settings['alumnionline_media_page'] ) ) . '" class="alumnionline_medialink alumnionline_float_right alumnionline-reunion-website-action-button">';
			echo esc_html__( 'Media', 'alumnionline-reunion-website' );
			echo '</a>';
		}
	...
	<a class="alumnionline-reunion-website-modal-close" href="#" role="button"><i aria-hidden="true" class="fas fa-times"></i><span class="screen-reader-text">' . esc_html__( 'Close', 'alumnionline-reunion-website' ) . '</span></a>';
		...
		echo '">';
		echo '<button id="alumnionline-reunion-website-search-btn" style="margin-bottom: 10px; padding: 10px;">' . esc_html__( 'Search', 'alumnionline-reunion-website' ) . '</button>';
...
function alumnionline_reunion_website_topic_view( $parentid, $offset ) {
	...
	if ( '0' === $offset ) {
		...
		if ( isset( $post ) ) {
			...
			echo '<td style="text-align:center;">' . wp_kses_post( apply_filters( 'the_content', $postcontent ) );
			if ( function_exists( 'alumnionline_reunion_website_create_gallery_code' ) && 'true' === $settings['alumnionline_photo_thumbnails'] ) {
				echo do_shortcode( alumnionline_reunion_website_create_gallery_code( $post->ID ) );
			}
			if ( '' != alumnionline_reunion_website_create_gallery_code( $post->ID ) && 'true' !== $settings['alumnionline_photo_thumbnails'] ) {
			...
			echo ' <a class="alumnionline-reunion-website-action-button alumnionline-reunion-website-action-button alumnionline_a_button" href="' . esc_url( get_the_permalink( $post->ID ) ) . '"><i class="fas fa-eye" aria-hidden="true"></i> ' . esc_html__( 'VIEW', 'alumnionline-reunion-website' ) . '</a> ';
			if ( current_user_can( 'use_alumnionline_reunion_website_features' ) ) {
			...
			if ( function_exists( 'alumnionline_reunion_website_pro_get_meta_record' ) && has_term( 'reunions', 'message-category', $post->ID ) && is_user_logged_in() ) {
			    $enableform = 0;
				$enableform = alumnionline_reunion_website_pro_get_meta_record( $post->ID, 'alumnionline_enable_reunion_rsvp_form' );
				if ( is_array( $enableform ) && count( $enableform ) > 0 ) {
					echo '<a class="alumnionline-reunion-website-action-button alumnionline-reunion-website-action-button" href="' . esc_url( get_the_permalink( $post->ID ) ) . '"><i class="fas fa-ticket-alt" aria-hidden="true"></i> ' . esc_html__( 'RSVP', 'alumnionline-reunion-website' ) . '</a> ';
				}
			}
			echo '</td>';
	...
	foreach ( $posts as $post ) {
		...
		echo ' <a class="alumnionline-reunion-website-action-button alumnionline_a_button" href="' . esc_url( get_the_permalink( $post->ID ) ) . '"><i class="fas fa-eye" aria-hidden="true"></i> ' . esc_html__( 'VIEW', 'alumnionline-reunion-website' ) . '</a> ';
		echo '</td>';
		...
		if ( $args->theme_location === $settings['alumnionline_nav_menu'] ) {
			...
			&& '' !== $settings['alumnionline_forum_page'] ) {
				if ( alumnionline_reunion_website_secure_by_role() ) {
					...
					$links .= '<li class="menu-item menu-item-has-children menu-item-memberlinks"><a>';
					$links .= esc_html__( 'Members', 'alumnionline-reunion-website' );
					$links .= '</a><ul class="sub-menu">';

					$links .= '<li class="menu-item menu-item-memberlinks"><a href="' . esc_attr( get_edit_profile_url() ) . '">';
					$links .= __( 'Edit Profile', 'alumnionline-reunion-website' );
					$links .= '</a></li>';
					...
					if ( shortcode_exists('alumnionline_media_archive') &&  isset( $settings['alumnionline_media_page'] ) && '' !== $settings['alumnionline_media_page'] ) {
						$links .= '<li class="menu-item menu-item-' . esc_attr( $settings['alumnionline_media_page'] ) . '"><a href="' . esc_url( get_the_permalink( $settings['alumnionline_media_page'] ) ) . '">';
						$links .= esc_html__( 'Media', 'alumnionline-reunion-website' );
						$links .= '</a></li>';
					}
			...
			if ( $filenames[ $key ] ) {
				...
				foreach ( $_FILES as $file => $array ) {
					...
					if ( is_wp_error( $file_id ) ) {
						$error_string = $file_id->get_error_message();
						$returnvalue  = esc_html__( 'Error saving files.', 'alumnionline-reunion-website' ) . ' ' . esc_attr( $error_string );

						if ( function_exists( 'SimpleLogger' ) ) {
							SimpleLogger()->info( $error_string );
						}
					}
				}
			}
...
function alumnionline_reunion_website_gallery_photo_validate() {
	...
	for ( $i = 0; $i < $total_files; $i++ ) {
		...
		if ( isset( $_FILES['alumnionline-reunion-website-upload']['tmp_name'][ $i ] ) && file_exists( sanitize_text_field( $_FILES['alumnionline-reunion-website-upload']['tmp_name'][ $i ] ) ) ) {
			...
			$filetypearray = explode( '/', sanitize_text_field( $_FILES['alumnionline-reunion-website-upload']['type'][ $i ] ) );
			...
			if ( array_key_exists( $i, $_FILES['alumnionline-reunion-website-upload']['tmp_name'] ) ) {
				if ( file_exists( sanitize_text_field( $_FILES['alumnionline-reunion-website-upload']['tmp_name'][ $i ] ) ) ) {
					...
					} elseif ( ! stristr( $settings['file_formats'], $filetypearray[1] ) ) {
						$error  = __( 'Error: Only ', 'alumnionline-reunion-website' );
						$error .= esc_attr( $settings['file_formats'] );
						$error .= __( ' are allowed. <br> Found: ', 'alumnionline-reunion-website' );
						$error .= sanitize_text_field( $filetypearray[1] );
					}
				}
			} else {
...
function alumnionline_reunion_website_create_terms() {
	...
	if ( ! term_exists( 'Photos', 'message-category' ) ) {
		wp_insert_term( 'Photos', 'message-category' );
	}
	if ( ! term_exists( 'Year Books', 'message-category' ) ) {
		wp_insert_term( 'Year Books', 'message-category' );
	}
	if ( ! term_exists( 'Videos', 'message-category' ) ) {
		wp_insert_term( 'Videos', 'message-category' );
	}
