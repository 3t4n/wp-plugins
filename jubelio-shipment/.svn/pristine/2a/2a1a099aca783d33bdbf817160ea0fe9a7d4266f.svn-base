...
class JubelioShipment_Api {
	 ...
	 */
	private function request( $method, $url, $args = array(), $validate_data = true, $is_staging = '', $cache_key = '', $cache_expiration = 60 ) {
		if( !empty( $is_staging ) )
		{
	...
	public function get_all_locations_from_jubelio() {
		...
		$maxpage = 50;
		// $url     = 'https://api2-lb.jubelio.com/locations/list?page=1&pageSize=' . $maxpage . '&sortBy=location_name&sortDirection=ASC';
		$url  	 = 'https://api2-lb.jubelio.com/locations/v2/list?page=1&q=&sort_by=location_name&page_size=' . $maxpage . '&sort_direction=ASC';
	...
	public function getMultiOriginFromSku( $location_id, $itemCodes = array() ) {
    ...
    if ( false === $jubelio_auth ) {
		...
		$get_webstore_id = get_option( 'woocommerce_jubelioshipment_settings' );
		$webstore_id		 = $get_webstore_id['webstore_id'];

		if ( false === $webstore_id || empty( $location_id ) ) {
				// Determine which parameter is missing for the log and error message
				$missing_param = false === $webstore_id ? 'Webstore ID' : 'Location ID';

				error_log( "$missing_param is missing for request." );

				return new WP_Error(
						400, // Bad Request status code
						__( "Missing $missing_param", 'jubelio-shipment' ),
						array(
								'http_response' => 400,
								'errors'        => 'Bad Request',
						)
		...
		$url  = 'https://api2-lb.jubelio.com/inventory/locations/v2/' . $location_id . '?page=' . $page . '&pageSize=500&q=&storeIds[0]=' . $webstore_id . '&channelIds[0]=131072';

    if ( !empty( $itemCodes ) ) {
			foreach ( $itemCodes as $index => $code ) {
					$url .= '&itemCodes[' . $index . ']=' . urlencode( $code );
			}
		}
    ...
    $headers = array(
        'Content-Type'  => 'application/json',
        'Authorization' => 'Bearer '. $jubelio_auth, // Assuming the API requires an auth token
    );
    ...
    // Retrieve response body and response code
	...
	public function create_awb( $body ) {
	...
	public function print_labels( $shipment_id  ) {
		$url = '/shipments/generate-shipping-label';

		// Prepare the body parameter as JSON
		$body = array(
	...
	public function get_awb_details( $shipment_id ) {
	...
	public function cancel_awb( $reason, $awb_number ) {
