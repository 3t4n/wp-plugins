...
class JubelioShipment {
	...
	public function load_all_hooks() {
		...
		add_action( 'wp_ajax_check_product_stock', array( $this, 'check_product_stock' ), 1000 );
		add_action( 'wp_ajax_nopriv_check_product_stock', array( $this, 'check_product_stock' ), 1000 );

		add_action( 'wp_ajax_generate_awb', array( $this, 'generate_awb_callback' ) );
		add_action( 'wp_ajax_nopriv_generate_awb', array( $this, 'generate_awb_callback' ) );

		add_action( 'wp_ajax_print_label', array( $this, 'print_label_callback' ) );
		add_action( 'wp_ajax_nopriv_print_label', array( $this, 'print_label_callback' ) );

		add_action( 'wp_ajax_cancel_awb', array( $this, 'cancel_awb_callback' ) );
		add_action( 'wp_ajax_nopriv_cancel_awb', array( $this, 'cancel_awb_callback' ) );
		...
		add_action( 'woocommerce_order_details_after_order_table', array( $this, 'add_track_package_button' ) );
		add_filter( 'woocommerce_my_account_my_orders_columns', array( $this, 'add_track_package_column' ) );
		add_action( 'woocommerce_my_account_my_orders_column_track_package', array( $this, 'add_track_package_button') );
		add_filter( 'manage_edit-shop_order_columns', array( $this, 'add_generate_awb_column_header'), 20);
		add_action( 'manage_shop_order_posts_custom_column', array( $this, 'add_generate_awb_column_content'), 2);

		// add custom data in admin area
		add_action( 'add_meta_boxes', array( $this,'add_awb_meta_box' ) );
	}
	 ...
	 * @param Mixed  $default Default.
	 */
	public function get_option_jubelioshipment( $option_name, $default = null ) {
		$instance_id = get_shipping_method_instance_id_by_name( 'jubelioshipment' );
		$global_options = get_option( 'woocommerce_jubelioshipment_' .  $instance_id . '_settings', null );
		if ( null === $global_options ) {
			return $default;
		}
		$result = $global_options[ $option_name ] ?? $default;
	...
	public function enqueue_backend_assets( $hook ) {
		...
		$css_url 				= JUBELIO_SHIPMENT_URL . 'assets/css/jubelio-shipment.css';
		$css_admin_url 	= JUBELIO_SHIPMENT_URL . 'assets/css/admin-jubelio-shipment.css';
		$js_admin_url 	= JUBELIO_SHIPMENT_URL . 'assets/js/admin-jubelio-shipment.js';

		// Check if the current page is the order edit page in WooCommerce admin
		if ( is_admin() && get_post_type($_GET['post']) === 'shop_order' ) {
			...
			// Localize the admin-custom script if needed
			wp_localize_script(
				'admin_custom', // The handle of the script
				'admin_custom_vars', // JavaScript object name
				array(
						'ajaxurl' => admin_url( 'admin-ajax.php' ),
						'nonce'   => wp_create_nonce( 'admin_custom_nonce' ),
				)
			);
	...
	public function jubeship_admin_footer_script() {
		...
		// Modal HTML
		echo '
   		<div id="awb-cancel-modal" class="modal-overlay" style="display: none;">
      		<div class="modal-content">
				<div id="awb-cancel-modal">
						<div style="background:#fff; padding:20px; max-width:400px; position:relative;">
							<h3>Batalkan Pengirimian</h3>
							<p>Apakah anda yakin ingin membatalkan pengiriman ? Silahkan pilih salah satu alasan pembatalan:</p>
							<select id="cancel-reason" style="width: 100%;margin-bottom: 20px;">
								<option value="Permintaan Customer">Permintaan Customer</option>
								<option value="Alamat Pengiriman Salah">Alamat Pengiriman Salah</option>
								<option value="Produk Rusak">Produk Rusak</option>
								<option value="Produk Belum Siap">Produk Belum Siap</option>
								<option value="Ekspedisi Belum Siap">Ekspedisi Belum Siap</option>
						</select>
						<button type="button" id="confirm-cancel" class="button">Konfirmasi Pembatalan</button>
						<button type="button" id="close-popup" class="button">Tutup</button>
					</div>
				</div>
      		</div>
    	</div>';
    ...
    // API call with bulk SKUs
    $stock_response = $this->api->getMultiOriginFromSku( $location_id, $itemCodes );

		// print_r( $stock_response);

    if ( is_wp_error( $stock_response ) ) {
        error_log( 'Error fetching stock: ' . $stock_response->get_error_message() );
        wp_send_json_error( array( 'error' => $stock_response->get_error_message() ) );
    }
    ...
    foreach ( $products as $cart_item ) {
        ...
        $sku = $product->get_sku();
				 ...
				 // Decode JSON response if it's not already an object
				if ( is_string( $stock_response ) ) {
					$stock_response = json_decode( $stock_response );
				}

				if ( isset( $stock_response->data ) && is_array( $stock_response->data ) ) {
					foreach ( $stock_response->data as $item ) {

							if ( $sku === $item->item_code ) {
									$qty = floatval( $item->qty );

									if ( $qty <= 0 ) {
											$out_of_stock[] = $product->get_name(); // Add product name to out-of-stock list
									}
    ...
    if ( !empty( $out_of_stock ) ) {
        $messages[] = 'The following products are out of stock for this origin: ' . implode(', ', $out_of_stock);
    }
		...
		if ( !empty( $messages ) ) {
			...
			wp_send_json_success( array(
					'out_of_stock' => true,
					'out_of_stock_products' => $out_of_stock, // Send product names that are out of stock
					'not_available_products' => $not_available // Send product names not available
			) );
		} else {
	...
	public function jubeship_multiorigin() {
		...
		<?php
			...
			$origin_city = $this->get_option_jubelioshipment( 'origin_location_address' );
		?>
		<tr class="multi_origin">
			<td colspan="2" style="border-bottom:0;text-align:left">
				<h3 style="margin: 0 0 1px 0;text-transform: capitalize;font-size: 13px;"><?php esc_html_e( 'Choose Nearest Shipping Origin (For Better Shipping Cost)', 'jubelio-shipment' ); ?></h3>
			</td>
		</tr>
		<tr>
			<td colspan="2" style="padding: 0 0 15px 0; border: 0; text-align: left" id="multi_origin_field">
				<select style="margin: 1px 0 10px 0;" id="multi_origin" class="multi_origin update_meta_on_change">
					<option value="">
						<?php esc_html_e( 'Select Origin', 'jubelio-shipment' ); ?>
					</option>
					<option value="base_origin">
						HW <?php echo esc_html( $site_name ); ?> - <?php echo esc_html( $origin_city ); ?>
					</option>
				</select>
			</td>
		</tr>
		...
		data = [
			...
			{ id:<?php echo esc_html( $location['location_id'] ); ?>,
				text:"<?php echo esc_html( $location['location_name'] ); ?> - <?php echo esc_html( $location['province'] ); ?>, <?php echo esc_html( $location['city'] ); ?>",
				...
				jQuery( '#multi_origin' ).on(
					...
					function ( e ) {
						...
						request.done( function () {
							...
							// Skip stock checking if data.id is "base_origin"
							if (data.id === "base_origin") {

									jQuery('.woocommerce-error, .woocommerce-message').remove();

									console.log('Stock checking skipped for base origin.');
									return;
							}

							// After update_checkout, check product availability from external API
        			checkProductStock(data.id);
						});

						function checkProductStock(location_id) {

							// Manually block the checkout form
							...
							jQuery.ajax({
								...
								data: {
										...
										products: <?php echo json_encode( WC()->cart->get_cart() ); ?> // Send cart contents to the external API
								},
								beforeSend: function () {
										// Show WooCommerce checkout loader
    						...
    						success: function (response) {
								...
								if (response.success) {
										if ((response.data.out_of_stock || response.data.not_available) && (response.data.out_of_stock_products.length || response.data.not_available_products.length)) {

												// Merge and remove duplicates
												let combinedProducts = [...response.data.out_of_stock_products, ...response.data.not_available_products];
												let uniqueProducts = [...new Set(combinedProducts)]; // Remove duplicates
												let productList = uniqueProducts.join(', ');

												// Main message with view details link
												let mainMessage = `
												<div class="woocommerce-error" style="color: #fff;font-size: 12px;background: #b20000;margin-bottom: 20px;padding: 20px;border-radius: 5px;">
														Some products are out of stock or not available for this origin.
														<a href="#" class="view-details" style="color:#fff;font-weight:bold;text-decoration:underline;">View Details</a>
														<div class="product-details" style="display:none; margin-top: 10px;">
																<strong>Out of Stock:</strong> ${productList}
														</div>
												</div>`;

												// Insert message before checkout form
												jQuery('form.checkout').before(mainMessage);

												// Disable the Place Order button
        								jQuery('form.checkout button[type="submit"]').prop('disabled', true);
												jQuery('.woocommerce-shipping-totals.shipping').css('display', 'none');

												// Toggle product details when 'View Details' is clicked
												jQuery('.view-details').on('click', function (e) {
														e.preventDefault();
														jQuery(this).next('.product-details').slideToggle();
												});

										}else {
												// If products are available, ensure the Place Order button is enabled
									...
									// Inject WooCommerce error notice for API error
											jQuery('form.checkout').before('<div class="woocommerce-error" style="color:red">' + (response.data.error || 'Error occurred during stock check.') + '</div>');
											jQuery('form.checkout button[type="submit"]').prop('disabled', true);
											jQuery('.woocommerce-shipping-totals.shipping').css('display', 'none');
									}

									// Scroll to the top of the checkout form to show the notices
								...
								complete: function () {
										// Hide WooCommerce checkout loader
	...
	public function add_fee_to_cart( $cart ) {
		...
		if ( ! empty( $shipping_selected ) ) {
			...
			// Check if the courier supports COD
			$supports_cod = !empty($meta['_jubelio_shipment_supports_cod']) && $meta['_jubelio_shipment_supports_cod'] === 'yes';
			// Get the current cart hash

			// Retrieve COD rate from transient
			$cod_rate = (int) $meta['_jubelio_shipment_cod'];

			if ($supports_cod ){
				// Only add COD fee if the chosen payment method is COD
				if (WC()->session->get('chosen_payment_method') === 'cod') {
					$cart->add_fee(__('COD Fee', 'jubelio-shipment'), $cod_rate);
				}
	...
	// Add custom AWB meta box
	public function add_awb_meta_box() {
		// Retrieve the WooCommerce Jubelio Shipment settings.
		$jubelio_shipment_options = get_option( 'woocommerce_jubelioshipment_settings', false );

		// Check if options exist and retrieve client ID and client secret.
		if ( false === $jubelio_shipment_options ) {
				return; // No options, do not add metabox.
		}

		$client_id     = $jubelio_shipment_options['client_id'] ?? '';
		$client_secret = $jubelio_shipment_options['client_secret'] ?? '';

		// Only add metabox if both client ID and client secret are not empty.
		if ( ! empty( $client_id ) && ! empty( $client_secret ) ) {
				add_meta_box(
						'awb_meta_box',                // Unique ID
						'Jubelio Shipment',            // Box title
						array($this, 'display_awb_meta_box'), // Content callback
	...
	// Callback function to display the AWB meta box
	public function display_awb_meta_box($post) {
		$order 				 = wc_get_order( $post->ID );
		$awb_number 		 = get_post_meta($post->ID, '_jubelioshipment_awb_data', true);
		$shipping_label_data = get_post_meta($post->ID, '_jubelioshipment_label_data', true);
		$link_label = '';
		$target_link = '';

		$shipping_lines_data = $this->get_shipping_lines( $post->ID );
		$courier_id = '';

		foreach ( $shipping_lines_data as $shipping_line ) {

			// Accessing the metadata
			$metadata = $shipping_line['metadata'] ?? array();
			$shipping_data = $metadata['_jubelio_shipment_data'] ?? array();

			// Extract the courier_id and other details
			$courier_id = $shipping_data['courier_id'] ?? '';
		}

		if( !empty($shipping_label_data) ) {
			$link_label = $shipping_label_data['file_url'];
			$target_link = '_blank';
		}

		$disabled		= 'disabled';
		$danger_message = '';
		$origin_phone   = $this->get_option_jubelioshipment( 'origin_phone_number' );
		
		// Display the "Generate AWB" button if no AWB exists
		if ( $order->has_status('processing') || $order->has_status('completed') ) {
			 if (!empty($origin_phone)) {
				 $disabled = '';
			 } else {
				 $danger_message = '<p style="color: red;">Phone number is required to generate the AWB.</p>';
			 }
		}

		if (empty($awb_number)) {
			// Show the danger message if the phone number is empty
    		echo $danger_message;
			
			echo '<button '. $disabled .' type="button" class="button generate-awb" data-order-id="' . esc_attr($post->ID) . '">Generate AWB</button>';
		} else {
			// Check if the status is "waiting" to show the "Cancel AWB" button
			$awb_details = $this->api->get_awb_details($awb_number['shipment_id']);

			// Display the AWB number if it exists
			echo '<p>AWB Number: <strong>' . esc_html($awb_number['awb']) . '</strong></p>';
			echo '<p>Status Pengiriman: <strong>' . esc_html($awb_details['latest_status']) . '</strong></p>';

			if( $awb_details['latest_status'] !== 'CANCELED' ){
			echo '<a target="'. $target_link .'" href="'. $link_label .'" type="button" class="button print-awb-label" data-order-id="' . esc_attr($post->ID) . '" data-shipment-id="' . esc_attr($awb_number['shipment_id']) . '" data-awb-number="' . esc_attr($awb_number['awb']) . '">Cetak Label</a>';
			}

			if( $courier_id !== '11') {
				if( $awb_details['latest_status'] === 'WAITING' ){
					echo '<button style="margin-top: 10px" type="button" class="button cancel-label" data-awb-number="' . esc_attr($awb_number['awb']) . '">Cancel AWB</button>';
				}
			}
		}

		// Add nonce field for security
		wp_nonce_field('generate_awb_nonce_action', 'generate_awb_nonce');
	}

	public function cancel_awb_callback() {

		if (
			! isset( $_POST['admin_custom_nonce'] )
			&& ! wp_verify_nonce( sanitize_key( $_POST['admin_custom_nonce'] ), 'admin_custom_nonce' )
			) {
				wp_die( 'Nonce value cannot be verified.' );
		}

		$awb_number = sanitize_text_field($_POST['awb_number']);
    	$reason = sanitize_text_field($_POST['reason']);

		// Your logic to cancel the AWB with the specified reason
    	$response = $this->api->cancel_awb( $reason, $awb_number);

		// print_r( $response );
		//wp_send_json_success($response);
	}

	public function print_label_callback() {
		if (
			! isset( $_POST['admin_custom_nonce'] )
			&& ! wp_verify_nonce( sanitize_key( $_POST['admin_custom_nonce'] ), 'admin_custom_nonce' )
			) {
		...
		$shipemnt_id = isset( $_POST['shipemnt_id'] ) ? absint( $_POST['shipemnt_id'] ) : 0;
		$order_id = isset( $_POST['order_id'] ) ? absint( $_POST['order_id'] ) : 0;

		$print_label = $this->api->print_labels( $shipemnt_id );

		if ( is_wp_error( $print_label ) ) {
			return new WP_Error( 'Error get awb!' );
		}

		// Assuming $response contains the response from the AWB generation API
		if ( !empty($print_label) ) {

			// Save all relevant AWB data in one metadata field as an array
			$label_data = array(
					'status' => $print_label['message'],
					'file_url' => $print_label['file_url'],
			);

			// Save the array as a single metadata entry
			update_post_meta( $order_id, '_jubelioshipment_label_data', $label_data );
		}

		// Optionally send a JSON response back
		wp_send_json_success($label_data);
		wp_die();

	}

	public function generate_awb_callback() {
		...
		$shipping_lines = $this->get_shipping_lines( $order_id );
		$payment_method = $order->get_payment_method(); 

		$body_param_shipping = array();
		...
		foreach ( $shipping_lines as $shipping_line ) {
			...
			$courier_service_id = $shipping_data['courier_service_id'] ?? '';
			$courier_service_name = $shipping_data['courier_service_name'] ?? '';

			// Check if _jubelio_shipment_insurance exists and has a value
			$is_insurance = !empty($metadata['_jubelio_shipment_insurance']);
			
			$is_cod = false;

			if ($order && $order->get_payment_method() === 'cod') {
				// Check if _jubelio_shipment_cod exists and has a value
				$is_cod = true;
			}

			// Safely retrieve the shipping_promotion value if _jubelio_shipment_promotion is an array and contains 'value'
			$shipping_promotion = $metadata['_promotion_usage_id'] ?? 0;

			// Safely retrieve the shipping_insurance value if _jubelio_shipment_insurance is an array and contains 'value'
			$shipping_insurance = $metadata['_jubelio_shipment_insurance'] ?? 0;
			...
			$body_param_shipping[] = array(
				...
				'promotion_id' => $shipping_promotion,
				'is_cod' => $is_cod
			);
		}

		// Get origin  details
		$origin_address = $this->get_option_jubelioshipment( 'origin_location_address' );
		$origin_email	= get_option( 'admin_email' );
		$origin_area_id = $this->get_option_jubelioshipment( 'origin_location_subdistrict' );
		$origin_zipcode = $this->get_option_jubelioshipment( 'origin_location_zipcode' );
		$origin_phone   = $this->get_option_jubelioshipment( 'origin_phone_number' );
		$origin_coordinate = $this->get_option_jubelioshipment( 'origin_coordinate' ) ?? '';
		$store_name = get_bloginfo( 'name' );

		// Get destination details
		$has_different_shipping 	= $this->has_different_shipping_address( $order_id );
		$customer_address_details = $this->get_customer_address_details( $order_id );

		$shipping_full_address = $order->get_meta('_shipping_full_address') ?? '';
		$destination_address_1 = $customer_address_details['shipping']['address_1'] ?? '';

		$billing_full_address = $order->get_meta('_billing_full_address') ?? '';
		$billing_address_1 = $customer_address_details['billing']['address_1'] ?? '';

		// get order data customer
		if ( $has_different_shipping ) {
			$destination_first_name = $customer_address_details['shipping']['first_name'];
			$destination_last_name = $customer_address_details['shipping']['last_name'];
			$destination_email = $customer_address_details['shipping']['email'];
			$destination_phone = $customer_address_details['shipping']['phone'];
			$destination_address = trim($shipping_full_address . ' ' . $destination_address_1);
			$destination_area_id = $customer_address_details['shipping']['subdistrict'];
			$destination_zipcode = $customer_address_details['shipping']['postcode'];
			$destination_coordinate = $order->get_meta( '_shipping_coordinate' ) ?? '';
		} else {
			$destination_first_name = $customer_address_details['billing']['first_name'];
			$destination_last_name = $customer_address_details['billing']['last_name'];
			$destination_email = $customer_address_details['billing']['email'];
			$destination_phone = $customer_address_details['billing']['phone'];
			$destination_address = trim($billing_full_address . ' ' . $billing_address_1);
			$destination_area_id = $customer_address_details['billing']['subdistrict'];
			$destination_zipcode = $customer_address_details['billing']['postcode'];
			$destination_coordinate = $order->get_meta( '_billing_coordinate' ) ?? '';
		}

		// Get items array
		$items = $order->get_items();
		$body_params_items = array();
		...
		foreach ( $items as $item_id => $item ) {
			if ( is_a( $item, 'WC_Order_Item_Product' ) ) {
				...
				if ( $product ) {
					...
					// Get dimensions and weight, multiplied by quantity
		...
		// Send body request to API
		$body = array(
			'ref_no' => $ref_id,
			'courier_id' => $body_param_shipping[0]['courier_id'],
			'courier_service_id' => $body_param_shipping[0]['courier_service_id'],
			'shipping_insurance' => $body_param_shipping[0]['shipping_insurance'],
			'is_insurance' => $body_param_shipping[0]['is_insurance'],
			'is_cod' => $body_param_shipping[0]['is_cod'],
			'promotion_usage_id' => $body_param_shipping[0]['promotion_id'],
			'origin' => array(
				'name' => $store_name,
				'email' => $origin_email,
				'phone' => $origin_phone,
			...
			'destination' => array(
				'name' => $destination_first_name . ' ' . $destination_last_name,
			...
			'items' => $body_params_items,
		);
		
		$generate_awb = $this->api->create_awb( $body );
		
		if ( is_wp_error( $generate_awb ) ) {
			return new WP_Error( 'Error get awb!' );
		}

		// Assuming $response contains the response from the AWB generation API
		if ( !empty($generate_awb) ) {

			// Save all relevant AWB data in one metadata field as an array
			$awb_data = array(
					'awb' => $generate_awb['awb'],
					'shipment_id' => $generate_awb['shipment_id'],
					'tracking_url' => $generate_awb['tracking_url'],
					'price' => $generate_awb['price'],
			);

			// Save the array as a single metadata entry
			update_post_meta( $order_id, '_jubelioshipment_awb_data', $awb_data );

			// Add order note for successful AWB generation
			$order->add_order_note(
				sprintf(
					__( 'AWB successfully generated: %s. Tracking URL: %s.', 'jubelio-shipment' ),
					$awb_data['awb'],
					$awb_data['tracking_url']
				)
			);
		}

		// Optionally send a JSON response back
		wp_send_json_success($awb_data);
		wp_die();
	}

	/**
	 * Add "Track Package" button to My Account > Orders > View Order
	*/
	public function add_track_package_button( $order_id ) {
		$order 		= wc_get_order($order_id);
		$order_id = $order->get_id();
		$awb_number = get_post_meta( $order_id, '_jubelioshipment_awb_data', true);

		// Display the button if a tracking number is available
		if (!empty($awb_number)) {
				echo '<a target="_blank" data-track-id="' . esc_attr( $awb_number['awb'] ) . '" href="'.$awb_number['tracking_url'].'" class="button track-package">Track Package</a>';
		}
		else {
			echo 'N/A';
		}
	...
	/**
	 * Display track package button in my account page
	 */
	function add_track_package_column($columns) {
		$columns['track_package'] = __('Track Package', 'jubelio-shipment');
		return $columns;
	}

	/**
	 * Add custom column header for awb
	 */
	public function add_generate_awb_column_header( $columns ) {
		$columns['generate_awb'] = __('Jubelio Shipment', 'jubelio-shipment');
    	return $columns;
	}

	/**
	 * Add custom column content for awb
	 */
	public function add_generate_awb_column_content( $column ) {

		global $post;

		if ($column == 'generate_awb') {
			...
			if ( $order->get_status() === 'processing' && empty( $awb_number ) ) {
				// Output the button
				if (!empty($origin_phone)) {
					echo '<button type="button" class="button generate-awb" data-order-id="' . esc_attr($post->ID) . '">Generate AWB</button>';
				} else {
					echo $danger_message ;
				}
				//echo '<button type="button" class="button generate-awb" data-order-id="' . esc_attr($post->ID) . '">Generate AWB</button>';
			...
			else
			{
				if( !empty( $awb_number ) ) {
					...
					echo '<a target="'. $target_link .'" href="'. $link_label .'" type="button" class="button print-awb-label" data-order-id="' . esc_attr($post->ID) . '" data-shipment-id="' . esc_attr($awb_number['shipment_id']) . '" data-awb-number="' . esc_attr($awb_number['awb']) . '">Cetak Label</a>';
						...
						echo '<a target="" type="button" class="button print-awb-label">CANCELED</a>';
					}
	...
	/**
	 * Retrieve the shipping lines from a WooCommerce order.
	 *
	 * @param WC_Order $order The order object from which to retrieve shipping lines.
	 * @return array Array of shipping line items associated with the order.
	 */
	public function get_shipping_lines( $order ) {
		...
		$shipping_lines = $orders->get_items( 'shipping' );

		// Prepare an array to store shipping line details
    	$shipping_details = array();

		foreach ( $shipping_lines as $shipping_item ) {
			if ( is_a( $shipping_item, 'WC_Order_Item_Shipping' ) ) {

				  // Retrieve shipping line metadata
					$meta_data = $shipping_item->get_meta_data();

					// Prepare an array to store metadata key-value pairs
					$meta_details = array();

					foreach ( $meta_data as $meta ) {
							$meta_details[ $meta->key ] = $meta->value;
					}

					$shipping_details[] = array(
						'method_id'    => $shipping_item->get_method_id(),     // Shipping method ID
						'method_title' => $shipping_item->get_method_title(),  // Shipping method title
						'total'        => $shipping_item->get_total(),
						'metadata'		 => $meta_details,                        // Shipping total cost
					);
			}
		}
	...
	/**
	 * Check if an order has a different shipping address.
	 *
	 * @param int $order_id
	 * @return bool True if different shipping address, false otherwise.
	 */
	private function has_different_shipping_address( $order_id ) {
		// Get the order
		$order = wc_get_order( $order_id );
	...
	/**
	 * Get detailed customer address information from an order.
	 *
	 * @param int $order_id
	 * @return array|false Customer address details or false if order not found.
	 */
	private function get_customer_address_details( $order_id ) {
		// Get the order
		$order = wc_get_order( $order_id );
		...
		$customer_address_details = array(
				...
				'shipping' => array(
						...
						'subdistrict'  => $shipping_address['address_2'],
