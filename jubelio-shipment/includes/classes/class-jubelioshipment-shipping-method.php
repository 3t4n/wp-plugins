	...
	public function init_form_fields() {
		$settings = array(
			'origin_location_search'  => array(
				...
				'required'    => true, // Custom flag for required validation.
			),
			...
			'origin_phone_number'           => array(
				'title' => __( 'Phone Number*', 'jubelio-shipment' ),
				'type'  => 'text',
				'css'   => 'width: 100%',
				'required'    => true, // Custom flag for required validation.
			),
 ...
 */
	...
	public function calculate_shipping( $package = array() ) {
		...
		$cart_subtotal = WC()->cart->get_subtotal(); // Get subtotal
		$discount_total = WC()->cart->get_cart_discount_total(); // Get discounts
		$total_value = $cart_subtotal - $discount_total; // Calculate discounted total
		...
		foreach ( $rates as $rate ) {
			...
			$cod_rate = '';
			$is_cod_supported = !empty($rate['is_cod_supported']);
			// Get the current cart hash (unique identifier for the current cart session)
			$cart_hash = WC()->cart->get_cart_hash();
			
			if ($is_cod_supported) {
				// Determine COD fee based on insurance activation
			...
			$rate_data = array(
				...
				'meta_data' => array(
					...
					'_jubelio_shipment_supports_cod'  => $is_cod_supported ? 'yes' : 'no', // Add COD support flag
