...
if ( ! function_exists( 'find_modus' ) ) :
	/** Check is domain contains with jubelio.store
	 *
	 * @return String
	 */
	function find_modus(array $cells): array
	{
		$values = array_count_values($cells);
		$modes = array_keys($values, max($values));
		return $modes;
	}
endif;
...
if ( ! function_exists( 'get_shipping_method_instance_id_by_name' ) ) :
function get_shipping_method_instance_id_by_name( $shipping_method_name ) {
	global $wpdb;

	// Table name for WooCommerce shipping zone methods
	$table_name = $wpdb->prefix . 'woocommerce_shipping_zone_methods';

	// Prepare the SQL query to fetch instance_id based on method_id
	$query = $wpdb->prepare(
		"SELECT instance_id
			FROM $table_name
			WHERE method_id = %s",
