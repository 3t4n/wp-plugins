...
if ( !defined( 'FRAKTJAKT_API_VERSION' ) ) {
    define( 'FRAKTJAKT_API_VERSION', '4.5.0' );
}
if ( !defined( 'FRAKTJAKT_PLUGIN_VERSION' ) ) {
    define( 'FRAKTJAKT_PLUGIN_VERSION', '2.6.0' );
}
        ...
        if ( ! class_exists( 'WC_Fraktjakt_Shipping_Method' ) ) {
            class WC_Fraktjakt_Shipping_Method extends WC_Shipping_Method {                               
                ...
                function init() {
                    ...
                    $this->enable_skip_calculation  = isset( $this->settings['enable_skip_calculation'] ) ? $this->settings['enable_skip_calculation'] : 'no';
                    ...
                    $this->form_fields = array(
                        ...
                        array(
                            ...
                            'description' => __('Enter your Consignor ID and key from your Fraktjakt integration to connect this extension to your Fraktjakt account.', 'fraktjakt-shipping-for-woocommerce')
	                                            .getLoginLink(__( 'Direct link to Fraktjakt PROD API webshop settings', 'fraktjakt-shipping-for-woocommerce' )),
                        ),
                        ...
                        array(
                            ...
                            'estimated_delivery_info' => array(
		                        ...
		                        'enable_skip_calculation' => array(
		                             'title' => __('No calculation outside checkout', 'fraktjakt-shipping-for-woocommerce'),
		                             'type' => 'checkbox',
		                             'label' => __('Skip shipping calculation outside cart and checkout.', 'fraktjakt-shipping-for-woocommerce'),
		                             'default' => 'yes',
		                             'description' => __('Activate to optimize performance by skipping WooCommerce built-in shipping calculation when outside cart and checkout.', 'fraktjakt-shipping-for-woocommerce')
		                        ),
                ...
                public function calculate_shipping( $package = array() ) {
                    ...
                    $xml.='  </address_to>'."\r\n";
										...
										$commodities = commodities($cart_items);
                    ...
                    if($this->consignor_id!='' && $this->consignor_key!='' && $this->enable_frontend=='yes' && $package['destination']['postcode']!='' && $package['destination']['country']!='') {              
                        ...
                        $array = json_decode(json_encode($xml_data), true);
												...
												update_fraktjakt_meta($array, null);

												// Todo - implement agent selection in cart
                            ...
                            $fraktjakt_agent_selection_link = $array['shipment']['agent_selection_link'];
						  	      			// update_post_meta($order->get_id(), 'fraktjakt_agent_selection_link', $fraktjakt_agent_selection_link);
													  // $order->update_meta_data( 'fraktjakt_agent_selection_link', $fraktjakt_agent_selection_link);
												    // $order->save();
                        ...
                        if( !empty($array['shipment']) && is_array($array['shipment']) && array_key_exists('id', $array['shipment']) ) {
                            ...
                            if(empty($fraktjakt_shipment_id)){
                                ...
                                if($this->fallback_service_name!='' && $this->fallback_service_price!='') {
                                    ...
                                    wc_add_notice( __('Failed to retrieve shipping options', 'fraktjakt-shipping-for-woocommerce'), 'error' );
                                    $error_happened="true";
                                }                                 
                            }
...
function consignor_admin_error_notice() {
    ...
    echo "<div class='error'> <p><mark>$error_message</mark></p></div>"; 
}
 ...
 *  -------------------------------------------------------------------
 */
function getLoginLink($linkText) {
    $fraktjakt_shipping_method_settings = get_option( 'woocommerce_fraktjakt_shipping_method_settings' );
    $uri = 'https://api.fraktjakt.se/';
...
function fraktjakt_send_order_type_1($order, $shipping_product_id, $fraktjakt_shipment_id, $uri_query, $consignor_id, $consignor_key, $referrer_code){   
        ...
        $array = json_decode(json_encode($xml_data), true);
				...
				if ((array_key_exists('error_message', $array['result'])) && (!empty($array['result']['error_message']))) {
					wc_add_notice( __('Error:', 'fraktjakt-shipping-for-woocommerce').': '.$array['result']['error_message'], 'error' );
				}
}
...
function Fraktjakt_api_selecter($order_id){
    ...
    $fraktjakt_order_id = $order->get_meta('fraktjakt_order_id');
    $fraktjakt_shipment_id = $order->get_meta('fraktjakt_shipment_id'); 
    $fraktjakt_access_code = $order->get_meta('fraktjakt_access_code'); 
    $fraktjakt_access_link = $order->get_meta('fraktjakt_access_link'); 
    
    $fallback = false;
    
    if (empty($fraktjakt_shipment_id)) {
    	$fraktjakt_shipment_id = get_shipment_from_option($order);
    }
		...
		if($enable_frontend=='yes') { 
	    	...
	    	$item_meta_data = $item->get_meta_data();
		      ...
		      if (isset($metadata) && !empty($metadata)) {
					 if ($metadata == "fraktjakt_fallback") {
	           	$fallback = true;
	        	} else {
    ...
    else if (empty($fraktjakt_shipment_id) && empty($fraktjakt_access_code)) {
...
/** ---------------------------------------------------
 *   Get shipment from shipment option
 *  ---------------------------------------------------
 */

function get_shipment_from_option($order){
	error_log('get_shipment_id');
		//if($enable_frontend=='yes') { 
			// changed in 1.7.0 to get the fraktjakt method_id from meta_data instead of the method_id field
			foreach( $order->get_items('shipping') as $item ){
	    	// get order item data (in an unprotected array)
	           	...
	           	$fallback = true;
				      	...
				      	if (array_key_exists(count($method)-2, $method)) {
				        	if (!empty($method[count($method)-2])) {
				        		if ((empty($fraktjakt_shipment_id)) || (!is_numeric($fraktjakt_shipment_id))) {
				        			$fraktjakt_shipment_id=$method[count($method)-2];
									    $order->update_meta_data( 'fraktjakt_shipment_id', $fraktjakt_shipment_id);
									    $order->save();
											error_log('got it');
				        			}
				        		}
...
function Fraktjakt_order_meta_box_content()
{
    ...
    $fraktjakt_access_code = $order->get_meta('fraktjakt_access_code'); 
    $fraktjakt_access_link = $order->get_meta('fraktjakt_access_link'); 
    $fraktjakt_tracking_link = $order->get_meta('fraktjakt_tracking_link'); 
        ...
        echo "<input id=\"fraktjakt_shipment_button\" class=\"button-primary\" type=\"button\" value=\"".__( 'Manage shipment', 'fraktjakt-shipping-for-woocommerce' )."\" title=\"".__( 'Manage the shipment in Fraktjakt', 'fraktjakt-shipping-for-woocommerce' )."\">";
	        ...
	        echo "<script type=\"text/javascript\" >
	            jQuery('#fraktjakt_shipment_button').click(function($) {
	                ...
	                window.open('".$uri."shipments/display/".join('',$fraktjakt_shipment_id)."?access_code=".join('',$fraktjakt_access_code)."','_blank'); 
	            });
	        </script>"; 
        } else {
	        echo "<script type=\"text/javascript\" >
	            jQuery('#fraktjakt_shipment_button').click(function($) {
	                ...
	                window.open('".$uri."shipments/display/".join('',$fraktjakt_shipment_id)."','_blank'); 
	            });
	        </script>";    
				}        
    }
    ...
    if (empty($fraktjakt_access_link) && empty($fraktjakt_shipment_id)) {
      ...
      echo '&nbsp; <input id="fraktjakt_create_button" class="button" type="button" value="'.__( 'Create order connection', 'fraktjakt-shipping-for-woocommerce' ).'" title="'.__( 'Create order connection to Fraktjakt', 'fraktjakt-shipping-for-woocommerce' ).'">';    
      echo "<script type=\"text/javascript\" >
...
function Fraktjakt_order_actions($actions, $the_order) {
    ...
    $fraktjakt_order_id = $the_order->get_meta('fraktjakt_order_id');
    $fraktjakt_shipment_id = $the_order->get_meta('fraktjakt_shipment_id'); 
    $fraktjakt_access_code = $the_order->get_meta('fraktjakt_access_code');
    $fraktjakt_access_link = $the_order->get_meta('fraktjakt_access_link');
    $fraktjakt_tracking_link = $the_order->get_meta('fraktjakt_tracking_link');

		if (empty($fraktjakt_shipment_id)) {
    	$fraktjakt_shipment_id = get_shipment_from_option($the_order);
    	if (empty($fraktjakt_shipment_id)) {
    ...
    if ((!empty($fraktjakt_shipment_id)) || !empty($fraktjakt_access_link)) {
    		if (!empty($fraktjakt_access_link)) {
	        $url = $fraktjakt_access_link;
    		} elseif (!empty($fraktjakt_access_code)) {
	        $url = "https://www.fraktjakt.se/shipments/display/".$fraktjakt_shipment_id."?access_code=".$fraktjakt_access_code;
    		} else {
	        $url = "https://www.fraktjakt.se/shipments/display/".$fraktjakt_shipment_id;
    		}
        if (!empty($fraktjakt_access_link)) {
        	$manage=__( 'Manage shipment', 'fraktjakt-shipping-for-woocommerce' );
					$ikon="view fraktjakt-handle-shipment";
        } else {
					$manage=__( 'Preview shipment', 'fraktjakt-shipping-for-woocommerce' );
					$ikon="view fraktjakt-preview-shipment";
        }
...
function update_fraktjakt_meta($array, $order) {
        ...
        if (isset($array['result']['shipment_id'] )) {
	        ...
	        //update_post_meta($order->get_id(), 'fraktjakt_shipment_id', $fraktjakt_shipment_id);
				    ...
				    $order->update_meta_data( 'fraktjakt_shipment_id', $fraktjakt_shipment_id);
						...
						WC()->session->set('fraktjakt_shipment_id', $fraktjakt_shipment_id);		    
			    }
	      }
        if (isset($array['result']['access_code'] )) {
	        $fraktjakt_access_code = $array['result']['access_code'];
					if (isset($order)) {
				    $order->update_meta_data( 'fraktjakt_access_code', $fraktjakt_access_code);
			    } else {
						WC()->session->set('fraktjakt_access_code', $fraktjakt_access_code);		    
			    }
	      }
        if (isset($array['result']['access_link'] )) {
	 	      $fraktjakt_access_link = $array['result']['access_link'];
					if (isset($order)) {
				    $order->update_meta_data( 'fraktjakt_access_link', $fraktjakt_access_link);
			    } else {
						WC()->session->set('fraktjakt_access_link', $fraktjakt_access_link);		    
			    }
	      }
        if (isset($array['result']['tracking_link'] )) {
	 	      $fraktjakt_tracking_link = $array['result']['tracking_link'];
					if (isset($order)) {
				    $order->update_meta_data( 'fraktjakt_tracking_link', $fraktjakt_tracking_link);
			    } else {
						WC()->session->set('fraktjakt_tracking_link', $fraktjakt_tracking_link);		    
			    }
	      }
        if (isset($array['result']['tracking_code'] )) {
	 	      $fraktjakt_tracking_code = $array['result']['tracking_code'];
					if (isset($order)) {
				    $order->update_meta_data( 'fraktjakt_tracking_code', $fraktjakt_tracking_code);
			    } else {
						WC()->session->set('fraktjakt_tracking_code', $fraktjakt_tracking_code);		    
			    }
	      }
        if (isset($array['result']['agent_selection_link'] )) {
	 	      $fraktjakt_agent_selection_link = $array['result']['agent_selection_link']||"";
				    ...
				    $order->update_meta_data( 'fraktjakt_agent_selection_link', $fraktjakt_agent_selection_link);
						...
						WC()->session->set('fraktjakt_agent_selection_link', $fraktjakt_agent_selection_link);		    
			    }
	  			// Todo - implement agent selection in cart
  			  setcookie('fraktjakt_agent_selection_link', $fraktjakt_agent_selection_link, time() + (600), "/");
  	  	} else {
        ...
        if (isset($array['result']['agent_link'] )) {
	 	      $fraktjakt_agent_link = $array['result']['agent_link'];
					if (isset($order)) {
				    $order->update_meta_data( 'fraktjakt_agent_link', $fraktjakt_agent_link);
			    } else {
						WC()->session->set('fraktjakt_agent_link', $fraktjakt_agent_link);		    
			    }
        ...
        if (isset($array['result']['agent_info'] )) {
	 	      $agent_info = $array['result']['agent_info'];
					if (isset($order)) {
				    $order->update_meta_data( 'agent_info', $agent_info);
			    } else {
						WC()->session->set('agent_info', $agent_info);		    
			    }
...
function address_to($order) {
    ...
    if (!is_user_logged_in()) {
        ...
        $shipping_company = empty($order->get_shipping_company()) ? get_user_meta( $user_id, 'shipping_company',true ) : $order->get_shipping_company();
        $shipping_address_1 = empty($order->get_shipping_address_1()) ? get_user_meta( $user_id, 'shipping_address_1',true ) : $order->get_shipping_address_1();
        $shipping_address_2 = empty($order->get_shipping_address_2()) ? get_user_meta( $user_id, 'shipping_address_2',true ) : $order->get_shipping_address_2();
        $shipping_postcode = empty($order->get_shipping_postcode()) ? get_user_meta( $user_id, 'shipping_postcode',true ) : $order->get_shipping_postcode();
        $shipping_city = empty($order->get_shipping_city()) ? get_user_meta( $user_id, 'shipping_city',true ) : $order->get_shipping_city();
        $shipping_country = empty($order->get_shipping_country()) ? get_user_meta( $user_id, 'shipping_country',true ) : $order->get_shipping_country();
        $shipping_company = empty($order->get_shipping_company())&& (! is_admin()) ? get_user_meta( $user_id, 'shipping_company',true ) : $order->get_shipping_company();	      
    } 
...
function recipient($order) {
    ...
    if (!is_user_logged_in()) {
        ...
        $shipping_phone = empty($order->get_shipping_phone()) ? $order->get_billing_phone() : $order->get_shipping_phone();
        ...
        $shipping_company = $order->get_shipping_company();
    		...
    		$user_id = get_current_user_id();
        $billing_email = empty($order->get_billing_email()) ? get_user_meta( $user_id, 'billing_email',true ) : $order->get_billing_email();
        $shipping_phone = empty($order->get_shipping_phone()) ? $order->get_billing_phone() : $order->get_shipping_phone();
        ...
        $shipping_first_name = empty($order->get_shipping_first_name())&& (! is_admin()) ? get_user_meta( $user_id, 'shipping_first_name',true ) : $order->get_shipping_first_name();
        $shipping_last_name = empty($order->get_shipping_last_name())&& (! is_admin()) ? get_user_meta( $user_id, 'shipping_last_name',true ) : $order->get_shipping_last_name(); 
        $shipping_company = empty($order->get_shipping_company())&& (! is_admin()) ? get_user_meta( $user_id, 'shipping_company',true ) : $order->get_shipping_company();	      
    } 
...
function custom_woocommerce_email_order_meta( $order, $sent_to_admin, $plain_text ) {
    $tracking_link=$order->get_meta('fraktjakt_tracking_link');  
	  //$agent_link=get_post_meta( $order_id, 'agent_link', true);  
		$fraktjakt_agent_link=$order->get_meta('fraktjakt_agent_link');
		$agent_info=$order->get_meta('agent_info');

		$fraktjakt_shipping_method_settings=get_option('woocommerce_fraktjakt_shipping_method_settings');
    if (isset($fraktjakt_shipping_method_settings['enable_tracking_in_email']) && ($fraktjakt_shipping_method_settings['enable_tracking_in_email'] == 'yes') && (!empty($tracking_link))){
			if ( $plain_text === false ) {
  	...
  	if (isset($fraktjakt_shipping_method_settings['enable_agent_email_selection']) && ($fraktjakt_shipping_method_settings['enable_agent_email_selection'] == 'yes') && (!empty($fraktjakt_agent_link))){
...
function agent_selection_link() {
	...
	$fraktjakt_shipping_method_settings=get_option('woocommerce_fraktjakt_shipping_method_settings');
  // if (isset($fraktjakt_shipping_method_settings['enable_agent_checkout_selection']) && ($fraktjakt_shipping_method_settings['enable_agent_checkout_selection'] == 'yes') && ($fraktjakt_shipping_method_settings['enable_frontend'] == 'yes') && (!empty($fraktjakt_agent_selection_link))){
...
/** ---------------------------------------------------
 *   Direct link to plugin settings
 *  ---------------------------------------------------
 */
function add_settings_link($links)
{
    $settings_link = '<a href="admin.php?page=wc-settings&tab=shipping&section=fraktjakt_shipping_method">' . __('Settings') . '</a>';
 ...
 *  ---------------------------------------------------
 */
function test_get_checkout_get_value( $input_value, $input_key ) {
...
add_filter( 'woocommerce_checkout_get_value', 'test_get_checkout_get_value', 10, 2 );
