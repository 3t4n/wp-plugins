...
function aiosc_combined_shortcode_year($atts)
{
    ...
    if ($offset !== 0) {
        return gmdate($date_format, strtotime("{$offset} years"));
    }
...
{
    $current_day = gmdate('z') + 1; // z returns the day of the year (0 to 365), so adding 1 to start from 1
    return $current_day;
}
...
{
    $current_week = gmdate('W'); // W returns the week number of the year (1 to 52/53)
    return $current_week;
}
...
{
    $current_month = gmdate('n'); // n returns the numeric representation of the current month (1 to 12)
...
function aiosc_shortcode_days_year($atts)
{
    ...
    if (isset($atts['go'])) {
        ...
        $current_year = gmdate('Y') + $year_offset;
    } else {
...
function aio_shortcode_first_day_of_year($atts)
{
    ...
    if ($format === 'short') {
        $first_day_name = gmdate('D', mktime(0, 0, 0, 1, 1, $current_year));
    } else {
        $first_day_name = gmdate('l', mktime(0, 0, 0, 1, 1, $current_year));
    }
...
function aio_shortcode_last_day_of_year($atts)
{
    ...
    $format = strtolower($atts['format']);
    if ($format === 'short') {
        $last_day_name = gmdate('D', mktime(0, 0, 0, 12, 31, $current_year));
    } else {
        $last_day_name = gmdate('l', mktime(0, 0, 0, 12, 31, $current_year));
    }
