...
function aiosc_page_featured_image_shortcode($atts) {
    ...
    // Get the featured image ID for the page
    $thumbnail_id = get_post_thumbnail_id($page_id);

    if ($thumbnail_id) {
        // Get the image HTML using wp_get_attachment_image()
        $image_html = wp_get_attachment_image($thumbnail_id, $atts['size'], false, array('alt' => esc_attr(get_the_title($page_id))));

        // Check if link attribute is set to yes
        if ($atts['link'] === 'yes') {
            $target = $atts['new_window'] === 'yes' ? ' target="_blank"' : '';
            return '<a href="' . esc_url(get_permalink($page_id)) . '"' . $target . '>' . $image_html . '</a>';
        } else {
            return $image_html;
        }
    } else {
