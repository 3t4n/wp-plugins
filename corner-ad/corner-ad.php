...
add_action( 'init', function(){
	add_filter( 'get_post_metadata', function( $v, $object_id, $meta_key, $single, $meta_type = '' ){
		if ( '_elementor_element_cache' == $meta_key ) {
			global $wpdb;
			if ( $wpdb->get_var( $wpdb->prepare('SELECT COUNT(*) FROM ' . $wpdb->postmeta . ' WHERE post_id=%d AND meta_key="_elementor_element_cache" AND meta_value LIKE "%CornerAd%";', $object_id ) ) ) return false;
		}
		return $v;
	}, 10, 5 );
} );
...
// CONST
define( 'CORNER_AD_PLUGIN_VERSION', '1.2.0' );
	...
	function corner_ad_replace_shortcode( $attr ) {
		...
		if ( ! isset( $corner_ad_inserted ) ) {
			...
			if ( $ad ) {
				...
				if ( $row ) {
					$img = corner_ad_get_images(
						$row->imgPath,
						( ( ! empty( $row->thumbPath ) ) ? $row->thumbPath : '' ),
						( empty( $extra ) || ! isset( $extra['optimized_images'] ) ||  $extra['optimized_images'] ? true : false )
					);
				}
                            ...
                            printCornerAd(
                                {
									...
									fixed:" . esc_js( ! empty( $extra ) && ! empty( $extra['fixed'] ) ? 1 : 0 ) . ",
