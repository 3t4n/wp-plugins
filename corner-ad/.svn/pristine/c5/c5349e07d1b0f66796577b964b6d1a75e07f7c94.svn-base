(function(){
	...
	window[ 'printCornerAd' ] = function ( settings )
	{
		...
		if( !tools.isEmpty( settings[ 'imgPath' ] ) )
		{
			...
			cornerContainer
				.attr( 'style',  'position:' + ( ( 'fixed' in settings && settings['fixed'] ) ? 'fixed' : 'absolute')+ ';width:100px;height:100px;top:0;z-index:999999;overflow:hidden;'+( ( isLeft) ? 'left' : 'right' )+':0;' )
				.html( '<div style="position:absolute;'+( ( isLeft ) ? 'left:0' : 'right:-1px' )+';"></div>' );
			...
			$( pageLeafMirror.node ).closest('div').on('mouseenter',
				function()
				{
					leafClickedActions();
				}
			...
			$(backgroundFillBig.node).on('mouseleave',
				function()
				{
					shrinkLeaf();
				}
