...
class CMA_BuddyPress {
	...
	static function bootstrap() {
		if (function_exists('bp_is_active')) {
			...
			/*--------------------------------------------------------------------------*/
			add_action( 'bp_setup_nav', array(__CLASS__, 'bp_setup_nav' ));
			add_action( 'cma_question_post_after', array(__CLASS__, 'thread_posted_activity'), 99);
			add_action( 'cma_answer_post_after', array(__CLASS__, 'answer_posted_activity'), 99, 2);
	...
	static function bp_setup_nav() {
		if (function_exists('bp_core_new_nav_item')) bp_core_new_nav_item(array(
			...
			'screen_function' => array(__CLASS__, 'questions_list'),
	...
	static function questions_list() {
		add_action( 'bp_template_content', array(__CLASS__, 'profile_questions_loop' ));
