...
class Pi_cdrw_selection_rule_pickup_location{
    ...
    function addRule($rules){
        $rules[$this->condition] = array(
            'name'=>__('Pickup location', 'conditional-discount-rule-woocommerce'),
    ...
    function ajaxCall(){
        ...
        if(self::datePluginInstalled()){
            ...
            echo Pi_cdrw_selection_rule_main::createSelect($this->pickup_locations(), $count, $this->condition,  "multiple",null,'static');
        }else{
    ...
    function savedDropdown($html, $values, $count){
        if(self::datePluginInstalled()){
            $html = Pi_cdrw_selection_rule_main::createSelect($this->pickup_locations(), $count, $this->condition,  "multiple", $values,'static');
            return $html;
        }
    ...
    function getSelectedPickupLocation(){
        if(!isset($_POST['post_data']) && !isset($_POST['pickup_location'])){
            $get_from_session = WC()->session->get('pickup_location');
            $type = class_exists('pi_dtt_delivery_type') ? pi_dtt_delivery_type::getType() : '';
            if(!empty($get_from_session) && $type == 'pickup'){
                return $get_from_session;
            }
...
new Pi_cdrw_selection_rule_pickup_location(PI_CDRW_SELECTION_RULE_SLUG);