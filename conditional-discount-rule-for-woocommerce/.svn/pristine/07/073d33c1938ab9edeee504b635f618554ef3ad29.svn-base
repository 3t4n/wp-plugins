...
class Pi_cdrw_selection_rule_coupon{
    ...
    function addRule($rules){
        $rules[$this->condition] = array(
            'name'=>__('Coupon','conditional-discount-rule-woocommerce'),
            'group'=>'cart_related',
    ...
    function ajaxCall(){
        ...
        echo Pi_cdrw_selection_rule_main::createSelect($this->allCoupons(), $count, $this->condition,  "multiple",null,'static');
        die;
    }

    function savedDropdown($html, $values, $count){
        $html = Pi_cdrw_selection_rule_main::createSelect($this->allCoupons(), $count, $this->condition,  "multiple", $values,'static');
        return $html;
    }
...
new Pi_cdrw_selection_rule_coupon(PI_CDRW_SELECTION_RULE_SLUG);