...
class Pi_cdrw_selection_rule_selected_delivery_date{
    ...
    function ajaxCall(){
        ...
        if(self::datePluginInstalled()){
            ...
            echo Pi_cdrw_selection_rule_main::createTextField($count, $this->condition, null);
    ...
    function savedDropdown($html, $values, $count){
        if(self::datePluginInstalled()){
            $html = Pi_cdrw_selection_rule_main::createTextField($count, $this->condition,  $values);
...
new Pi_cdrw_selection_rule_selected_delivery_date(PI_CDRW_SELECTION_RULE_SLUG);
