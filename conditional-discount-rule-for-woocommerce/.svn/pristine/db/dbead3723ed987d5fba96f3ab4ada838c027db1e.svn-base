...
class Pi_cdrw_selection_rule_quantity{
    ...
    function addRule($rules){
        $rules[$this->condition] = array(
            'name'=>__('Quantity of product in cart','conditional-discount-rule-woocommerce'),
            'group'=>'cart_related',
    ...
    function ajaxCall(){
        ...
        echo Pi_cdrw_selection_rule_main::createNumberField($count, $this->condition, null,1);
        die;
    }

    function savedDropdown($html, $values, $count){
        $html = Pi_cdrw_selection_rule_main::createNumberField($count, $this->condition,  $values,1);
        return $html;
    }
...
new Pi_cdrw_selection_rule_quantity(PI_CDRW_SELECTION_RULE_SLUG);