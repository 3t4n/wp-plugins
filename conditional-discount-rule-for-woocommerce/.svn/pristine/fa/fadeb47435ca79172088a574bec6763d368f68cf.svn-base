...
class Pi_cdrw_selection_rule_zones{
    ...
    function addRule($rules){
        $rules[$this->condition] = array(
            'name'=>__('Zones','conditional-discount-rule-woocommerce'),
    ...
    function ajaxCall(){
        ...
        echo Pi_cdrw_selection_rule_main::createSelect($this->allZones(), $count, $this->condition,  "multiple",null,'static');
        die;
    }

    function savedDropdown($html, $values, $count){
        $html = Pi_cdrw_selection_rule_main::createSelect($this->allZones(), $count, $this->condition,  "multiple", $values,'static');
        return $html;
    }
...
new Pi_cdrw_selection_rule_zones(PI_CDRW_SELECTION_RULE_SLUG);