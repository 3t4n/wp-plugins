...
class pi_products_matching_rule{
    function __construct($discount_id, $cart){
    ...
    function getMatchedProductsCost(){
        ...
        foreach($products as $product){
            if($this->isMatch($product)){
                $total += pisol_cdrw_cart_clone::get_product_subtotal($product['data'], $product['quantity']);
            }
        }
        return $total;
    }
    ...
    function getRules(){
        $rules = get_post_meta($this->discount_id, 'pi_metabox', true);
        return $rules;
    }
