...
class Pi_cdrw_selection_rule_cart_subtotal_before_discount{
    ...
    function addRule($rules){
        $rules[$this->condition] = array(
            'name'=>__('Cart Subtotal (Before Discount)', 'conditional-discount-rule-woocommerce'),
            'group'=>'cart_related',
    ...
    function ajaxCall(){
        ...
        echo Pi_cdrw_selection_rule_main::createNumberField($count, $this->condition, null);
        die;
    }

    function savedDropdown($html, $values, $count){
        $html = Pi_cdrw_selection_rule_main::createNumberField($count, $this->condition,  $values);
        return $html;
    }
    ...
    function conditionCheck($result, $cart, $logic, $values){
                    ...
                    $or_result = false;
                    $cart_subtotal = function_exists('WC') && is_object(WC()->cart) ? (float)WC()->cart->get_displayed_subtotal() : 0;
...
new Pi_cdrw_selection_rule_cart_subtotal_before_discount(PI_CDRW_SELECTION_RULE_SLUG);