...
class Pi_cdrw_selection_rule_main{
    ...
    */
    ...
    function savedRows(){
        $html = '<div class="my-2 pisol-no-cond-msg alert alert-danger text-center" style="display:none">You have not added any condition, you need to add condition to make the discount apply. <br>Add condition by clicking "Add Condition" button</div>';
    ...
    function conditionValue($condition, $values, $count){
        $html = apply_filters('pi_cdrw_saved_values_'.$condition, "", $values, $count);
        return $html;
    }
    ...
    static function createSelect($array, $count, $condition ="",  $multiple = "",  $values = array(), $dynamic = ""){
        ...
        $html = '<select class="form-control pi_condition_value pi_values_'.$dynamic.'" data-condition="'.$condition.'" name="pi_selection['.$count.'][pi_'.PI_CDRW_SELECTION_RULE_SLUG.'_condition_value][]" '.$multiple.' placeholder="Select">';
    ...
    static function createNumberField($count, $condition ="",  $values = array(), $step = 'any'){
        ...
        $html = '<input type="number" step="'.$step.'" class="form-control" data-condition="'.$condition.'" name="pi_selection['.$count.'][pi_'.PI_CDRW_SELECTION_RULE_SLUG.'_condition_value][]" '.$value.' >';
        return $html;
    }
    ...
    static function createTextField($count, $condition ="",  $values = array()){
        ...
        $html = '<input required type="text" class="form-control" data-condition="'.$condition.'" name="pi_selection['.$count.'][pi_'.PI_CDRW_SELECTION_RULE_SLUG.'_condition_value][]" '.$value.' >';
        return $html;
    }
...
add_action( 'admin_enqueue_scripts', 'pisol_cdrw_dynamic_rule_main_script');

function pisol_cdrw_dynamic_rule_main_script(){
    if(isset($_GET['page']) && 'pisol-cdrw' == $_GET['page']){
        wp_enqueue_script( 'pisol_cdrw_dynamic_rule_main_script', plugin_dir_url( __FILE__ ) . 'js/dynamic_form.js',array( 'jquery' ), '1.7.19' );
