...
function scan_posts($options = array(), $exclude = array()) {
    ...
    $links = array();
    /* Turn Off some weird error for parsing...obviously content is not going to contain real data */
...
function scan_custom_fields($fields = array(), $options = array()) {
    ...
    $links = array();
    /* Turn Off some weird error for parsing...obviously content is not going to contain real pure html data */
    libxml_use_internal_errors(true);
    //ob_implicit_flush(true);
    //ob_end_flush();
