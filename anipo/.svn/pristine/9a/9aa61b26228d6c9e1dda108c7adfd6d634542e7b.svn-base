...
final class ANIPO_API
{
    ...
    public function get_barcode()
    {
        try {
            ...
            if (!isset($_POST['order_id']) || !isset($_POST['order_weight']) || !isset($_POST['order_box_size']) || !isset($_POST['order_box_type']) || !isset($_POST['order_send_type']) || !isset($_POST['order_payment_type'])) {
                wp_send_json_success(['status' => false, 'message' => 'Invalid input']);
            }
            $order_id = intval($_POST['order_id']);
            $order_weight = intval($_POST['order_weight']);
            $order_box_size = intval($_POST['order_box_size']);
            $order_box_type = intval($_POST['order_box_type']);
            $order_send_type = intval($_POST['order_send_type']);
            $order_payment_type = $_POST['order_payment_type'];
            $order = new WC_Order($order_id);
            if (!$order) {
                wp_send_json_success(['status' => false, 'message' => 'Invalid order ID']);
            }
            $api_response = $this->get_barcode_api($order, $order_weight, $order_box_size, $order_box_type, $order_send_type, $order_payment_type);
            if (!is_wp_error($api_response)) {
                ...
                if ($body->status) {
                    ...
                    $order->update_meta_data('anipo_barcode_send_type', $body->data->sendType);
                    $order->update_meta_data('anipo_barcode_pay_type', $body->data->payType);
                    ...
                    $order->update_meta_data('anipo_box_non_standard', $body->data->nonstandard);
    ...
    private function get_barcode_api(WC_Order $order, $order_weight, $order_box_size, $order_box_type, $order_send_type, $order_payment_type)
    {
        global $anipo;
        ...
        $orderData = [
            ...
            'weight' => $order_weight,
            'boxSizeId' => $order_box_size,
            'isnonstandard' => $order_box_type,
            'payType' => $order_payment_type,
            'sendType' => $order_send_type,
            'sum' => $order->get_total(),
        ];
        $body = [
    ...
    public function has_voluminous_product(WC_Order $order)
    {
        foreach ($order->get_items() as $item_id => $item) {
            ...
            $is_voluminous = get_post_meta($product_id, '_anipo_is_voluminous', true);
            if ($is_voluminous === 'yes') {
                return true;
            }
        }
        return false;
    ...
    public function issued_order(WP_REST_Request $request)
    {
        ...
        $send_type = $query_params['sendType'] ?? null;
        $pay_type = $query_params['payType'] ?? null;
        $post_cost = $query_params['post_cost'] ?? null;
        $tax = $query_params['tax'] ?? null;
        $boxSizeId = $query_params['box_size_id'] ?? null;
        $nonstandard = $query_params['nonstandard'] ?? null;
...
//        if (!$is_issued_barcode) {
        ...
        $wc_order->update_meta_data('anipo_barcode_send_type', $send_type);
        $wc_order->update_meta_data('anipo_barcode_pay_type', $pay_type);
        ...
        $wc_order->update_meta_data('anipo_box_non_standard', $nonstandard);
...
//        return new WP_REST_Response( [ 'status' => true, 'anipo_is_issued_barcode' => true, 'is_updated' => false ], 200 );
    ...
    public function barcode_print_data(WC_Order $order)
    {
        ...
        $payment_method = $order->get_payment_method(); // Retrieves the payment method (e.g., 'paypal', 'stripe')
        $payment_method_title = $order->get_payment_method_title(); // Gets the title of the payment method

        // Get shipping method
        $shipping_methods = $order->get_shipping_methods(); // Retrieves an array of shipping methods
        $shipping_method = !empty($shipping_methods) && isset($shipping_methods[0]) ? $shipping_methods[0]->get_method_id() : ''; // Get the first shipping method ID
        $shipping_method_title = !empty($shipping_methods) && isset($shipping_methods[0]) ? $shipping_methods[0]->get_name() : ''; // Get the name of the first shipping method
        ...
        if (intval($anipo->get_print_factor_setting()) === 1) {
            ...
            foreach ($order->get_items() as $item_id => $item) {
                ...
                // Initialize product properties array
                $product_properties = [];
                // Fetch product attributes (if exist)
                if ($product->get_attributes()) {
                    foreach ($product->get_attributes() as $attribute_name => $attribute) {
                        $product_properties[] = $product->get_attribute($attribute_name);
                    }
                }
        ...
        $is_pay_in_place = false;
        $print_type = $anipo->get_print_type_setting();
        $pay_type = $order->get_meta('anipo_barcode_pay_type');
        if (!in_array($pay_type, ['payOnline', 'payInPlace', 'fareAtDestination'])) {
            $pay_type = 'payOnline';
        }
        if ($pay_type === 'payInPlace') {
            $is_pay_in_place = true;
            if ($print_type === 3) {
                $print_type = 1;
            }
        }

        $send_type = array_key_exists(intval($order->get_meta('anipo_barcode_send_type')), $anipo->send_type_array) ? intval($order->get_meta('anipo_barcode_send_type')) : 1;
        ...
        return [
            ...
            'post_type' => $anipo->send_type_array[$send_type],
            'post_pocket' => 'بسته',
            'non_standard' => intval($order->get_meta('anipo_box_non_standard')) === 1 || intval($order->get_meta('anipo_box_size_id')) >= 4 ? 1 : 0,
            'domain' => $domain,
            ...
            'customer_state_number' => $anipo->get_state_number($customer_state),
            ...
            'is_pay_in_place' => $is_pay_in_place,
        ];
    }
}
