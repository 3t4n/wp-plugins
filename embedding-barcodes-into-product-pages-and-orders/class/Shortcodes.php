...
use UkrSolution\BarcodesDigital\Makers\WoocommercePostsA4BarcodesMaker;
...
class Shortcodes
{
    ...
    public function get($attributes)
    {
        ...
        if (isset($attributes['class']) && is_product()) {
            wp_enqueue_style('product-barcode', Variables::$A4B_PLUGIN_BASE_URL . 'assets/css/style-2.0.4-gf536e91.css', array());
            wp_enqueue_script('product-barcode-js', Variables::$A4B_PLUGIN_BASE_URL . 'assets/js/barcodes-core-2.0.4-gf536e91.js', array('jquery'), null, true);
    ...
    private function generateBarcode($shortcode, $attributes, $postType = '')
    {
        ...
        if ($postType === 'orders') {
            ...
            if (is_object($order) && method_exists($order, 'get_date_modified')) {
                $postUpdated = strtotime($order->get_date_modified());
            } else {
            ...
            if ($list["listItems"]) {
                ...
                foreach ($list['listItems'] as &$listItem) {
                    if (!empty($listItem['image'])) {
                        $listItem['image'] = false === strpos($listItem['image'], 'base64') ? $listItem['image'] .  '?r=' . md5(mt_rand()) : $listItem['image'];
                    }
                }
            }
    ...
    private function saveShortcode($data, $type)
    {
        ...
        if ($id) {
            if (isset($data["template"]) && $type === "custom") {
                ...
                $result = $wpdb->update(
                    $tableShortcodes,
                    array("matching" => json_encode($data), "shortcode" => $shortcode, 'datetime' => date('Y-m-d H:i:s')),
                    array("id" => $id,),
                    array('%s',),
                    array('%d')
                );
            } else {
                $wpdb->update($tableShortcodes,
                    array("matching" => json_encode($data), 'datetime' => date('Y-m-d H:i:s')),
                    array("id" => $id,),
                    array('%s',),
                    array('%d')
                );
