...
class Hidden extends Field {
    ...
    public function field_preview_html($fieldindex, $fieldid, $field_infos) {
        ob_start();
        $value = wplf_valueof($field_infos, 'value');
        $preview = "<input style='border: 1px dashed #dddddd;' type='text' disabled='disabled' name='submitform[]' id='{$fieldindex}' class='form-control' value='{$value}' />";
