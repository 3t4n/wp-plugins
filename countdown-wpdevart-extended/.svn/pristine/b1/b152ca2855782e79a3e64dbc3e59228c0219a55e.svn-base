			...
			foreach($param_heading_value['params'] as $key=>$value){
				if(isset($_POST[$key])){
					if(is_array($_POST[$key])){						
						...
						//allow html tags in tinmce editor
						if($value['function_name'] == 'tinmce'){
							$params_array[$key]=stripslashes(wp_kses_post($_POST[$key]));
						}else{
...
/*############  Update timer ID function  ################*/
	...
	private function remove_timer($id){
		...
		global $wpdb;
		$wpdb->query($wpdb->prepare('DELETE FROM ' . wpda_contdown_extend_databese::$table_names['timer'].' WHERE id="%d"', array($id)));
	}
...
jQuery(document).ready(function(e) {
	...
	private function generete_timer_parametrs($id=0){
		...
		$timer_params = NULL;
		if($id)
			$timer_params = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.wpda_contdown_extend_databese::$table_names['timer'].' WHERE id=%d',array($id)));	
            ...
            <div class="conteiner">
                ...
                <div class="option_panel">            
                    ...
                    <div class="all_options_panel">
                        <input type="text" class="timer_name" name="name" placeholder="Enter name here" value="<?php echo isset($name)?esc_attr($name):'' ?>" >
