...
class wpdevart_countdown_extended_woocommerce_settings extends WC_Settings_Page {
	...
	public function output() {
			...
			<tr valign="top">
				...
				<td class="forminp forminp-select">
					<select onmousedown="wpdevart_pro()" name="wpdevart_countdown_woocommerce_all_timer" id="wpdevart_countdown_woocommerce_all_timer" class=" wpdevart_pro">
						<option value="0" <?php selected($timer,'0') ?>>Select Timer</option>
						<?php foreach($timer_array as $key => $value){ ?>
						<option value="<?php echo esc_attr($key) ?>" <?php selected($key,$timer) ?>><?php echo esc_html($value); ?></option>
						<?php } ?>						
					</select>
				</td>
				</tr>
		...
		function wpdevart_pro(){
			alert('If you want to use this feature upgrade to Countdown extended Pro');
			return false;
		}
		</script>
		<style>
		.form-table th.titledesc {
			...
			.wpdevart_support:before {
				content: "";
				background: url(<?php echo esc_url(wpda_contdown_extend_plugin_url).'includes/admin/images/support-white.png' ?>) no-repeat;
				width: 25px;
				height: 25px;
