...
class wpdevart_countdown_extended extends WP_Widget {
	...
	function form($instance) {
        ...
        <p class="flb_field">
          ...
          <br>
          <input id="<?php echo esc_html($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($instance['title']); ?>" class="widefat">
        </p>
        
        <p class="flb_field">  

			<select style="width: 100%" id="<?php echo esc_attr($this->get_field_id('timer')); ?>" name="<?php echo esc_attr($this->get_field_name('timer')); ?>">
				<option value="0">Select Timer</option>
				<?php foreach($timers as $id => $name){
					echo "<option ".selected($id,$instance['timer'],false)." value=\"".esc_attr($id)."\">".esc_html($name)."</option>";
				} ?>
			</select>  
        ...
        <p class="flb_field">  
			<select style="width: 100%" id="<?php echo esc_attr($this->get_field_id('theme')); ?>" name="<?php echo esc_attr($this->get_field_name('theme')); ?>">
				<option value="0">Select Theme</option>
				<?php foreach($themes as $id => $name){
					echo "<option ".selected($id,$instance['theme'],false)." value=\"".esc_attr($id)."\">".esc_html($name)."</option>";
				} ?>
			</select>  
        </p>
