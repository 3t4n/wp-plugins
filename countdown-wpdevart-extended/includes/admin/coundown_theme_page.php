...
class wpda_contdown_extend_theme_page{
	...
	public function controller_page(){
		...
		$id = 0;
		if(isset($_GET["task"])){
			$task = $_GET["task"]; // this $task used only in switch so i didnt sanitize it.
		}
		if(isset($_GET["id"])){
			$id = intval($_GET["id"]);
		}		
	...
	private function display_table_list_theme(){
        ...
        <div class="wrap">
			<div class="wpdevart_plugins_header div-for-clear">
				...
				<a target="blank" href="<?php echo esc_url(wpdevart_countdown_extended_support_url); ?>" class="wpdevart_support">Have any Questions? Get quick support!</a>
			</div>
            ...
            <table class="wp-list-table widefat fixed pages" style="width:95%">
                ...
                <tbody>
                 <tr ng-repeat="rows in names | filter:filtering_table" class="description_row">
					 ...
					 <td><a href="<?php echo urldecode(htmlspecialchars_decode(wp_nonce_url('admin.php?page=wpda_contdown_extend_themes&task=set_default_theme&id={{rows.id}}','wpda_nonce','wpda_nonce'))); ?>"><img src="<?php echo esc_url(wpda_contdown_extend_plugin_url).'includes/admin/images/default' ?>{{rows.default}}.png"></a></td>
...
jQuery(document).ready(function(e) {
	...
	private function generete_theme_parametrs($id = 0){
		...
		$new_theme=1;
		if($id){
			$theme_params = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.wpda_contdown_extend_databese::$table_names['theme'].' WHERE id=%d', array($id)));	
			$new_theme=0;
		}else{
	...
	private function set_default_theme($id){
		...
		$save = $wpdb->update(wpda_contdown_extend_databese::$table_names['theme'], array('default' => 1), array('id' => $id), array('%d') );		
	}
