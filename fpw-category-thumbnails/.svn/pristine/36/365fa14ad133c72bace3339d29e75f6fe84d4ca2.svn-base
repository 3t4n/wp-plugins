		...
		if ( !( '' === $thumbID ) ) {
			...
			if ( 'ngg-' == substr( $thumbID, 0, 4 ) ) {
				if ( class_exists( 'nggdb' ) ) {
					$thumbnail_id = substr( $thumbID, 4 );
					$picture = nggdb::find_image($thumbnail_id);
					if ( !$picture ) {
						$fpwImage = '';
					} else {
						$fpwImage = $picture->thumbURL;
					}
				} else {
					$fpwImage = '';
				}
			} else {
				$fpwImage = wp_get_attachment_image_src( $thumbID )[0];
			}
			if (!('' === $fpwImage)) {
				$pic .= '<img src="' . $fpwImage . '" class="wp-post-image-' . $type . ' wp-post-image" alt="Featured Image" title="' . $catNames . '" width="' . $this->fptOptions[ $type ][ 'width' ] . '"></div>';
			} else {
				$pic = '';
			}
		}
