...
//	quit if not wp 3.4 or higher
global $wp_version;

if ( version_compare( $wp_version, '3.4', '<' ) ) 
	wp_die( '<p style="text-align:center">Cannot activate! FPW Category Thumbnails plugin ' . 
			'requires WordPress version 3.4 or higher!</p><p style="text-align:center">' . 
...
if ( is_admin() ) {
	...
	$fpw_CT = new fpwCategoryThumbnails( dirname( __FILE__ ), $version );

	$o = get_option( 'fpw_category_thumb_opt' );
	if ( is_array( $o ) && $o[ 'fpt' ] ) {
		require_once dirname( __FILE__ ) . '/classes/fpw-post-thumbnails-class.php';
		$fpw_PT = new fpwPostThumbnails( dirname( __FILE__ ), $version );
	}
} else {
	...
	$fpw_CT = new fpwCategoryThumbnails( dirname( __FILE__ ), $version, $hide );
