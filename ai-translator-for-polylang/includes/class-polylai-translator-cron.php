...
class polylai_Cron {
    ...
    public function __construct() {
        try {
            ...
            polylai_Utils::db_log( 'error', 'ignore_user_abort', $e->getMessage() );
        }
    ...
    public function execCron( $exit = true ) {
        polylai_Utils::db_log( type: 'debug', operation: 'cron', message: 'exec cron' );
        if ( $this->cli ) {
            set_transient( self::ACTIVITY_TRANSIENT_KEY, gmdate( 'Y-m-d H:i:s' ), 70 );
        }
        $this->fetch_posts();
        polylai_Utils::db_log( 'debug', 'cron', 'exec cron completed' );
        if ( $exit ) {
            die;
        }
    }
    ...
    public function check_for_posts() {
        //polylai_Utils::db_log('debug', 'cron', 'check_for_posts');
        global $wpdb;
    ...
    public function fetch_posts() {
        polylai_Utils::db_log( 'debug', 'cron', 'fetch_posts' );
        if ( !$this->check_for_posts() ) {
            delete_transient( self::LOCK_TRANSIENT_KEY );
            return;
        }
        $running = get_transient( self::LOCK_TRANSIENT_KEY );
        if ( $running ) {
            polylai_Utils::db_log( 'debug', 'cron', 'already running, exiting' );
    ...
    private function translate_meta( $id, $current_locale_name, $locale_name ) {
        $meta_to_translate = [];
        $meta = [];
        $meta_yoast = polylai_Yoast::translate_meta(
            $id,
            $current_locale_name,
            $locale_name,
            $this->ai_engine
        );
        $meta_elementor = polylai_Elementor::translate_meta(
            $id,
            $current_locale_name,
            $locale_name,
            $this->ai_engine
        );
        $meta = array_merge( $meta, $meta_yoast );
        $meta = array_merge( $meta, $meta_elementor );
    ...
    private function is_plain_text( $string ) {
    ...
    private function translate(
        ...
        polylai_Utils::db_log(
            'info',
            "translate",
            count( explode( " ", $content ) ) . " words {$locale}",
            $id
        );
        $translated_text = $this->ai_engine->translate(
            $content,
            $current_locale,
            $locale,
            $id,
            $this->is_plain_text( $content )
        );
    ...
    private function translate_blocks( $id, $locale, $locale_name ) {
        $post = get_post( $id );
        $current_locale = pll_get_post_language( $id, 'slug' );
        $current_locale_name = pll_get_post_language( $id, 'name' );
        ...
        try {
            ...
            if ( class_exists( '\\Elementor\\Plugin' ) ) {
                ...
                polylai_Utils::db_log( 'debug', 'translate', "clear elementor cache" );
            }
            delete_post_meta( $id, POLYLAI_KEY );
        } catch ( Exception $e ) {
            polylai_Utils::db_log( 'error', 'translate', $e->getMessage() );
