...
class polylai_Admin
{
	...
	public function check_usage()
	{
		...
		if (count($results) >= 3) {
			...
			<div class="notice notice-success">
				...
				<p>If you like the plugin, please <a
						href="https://wordpress.org/support/plugin/ai-translator-for-polylang/reviews/#new-post"
						target="_blank">rate it with ⭐⭐⭐⭐⭐</a> on WordPress.org. It will help us to improve the plugin and make it
	...
	public function get_post_tr_status()
	{
		...
		echo wp_json_encode([
			...
			'allowed' => $allowed,
			'has_cron' => !!$has_cron,
			'has_key' => !!polylai_Utils::has_api_key(),
	...
	private function run_http_cron()
	{
		...
		try {
			...
			polylai_Utils::db_log('error', 'run_http_cron', e->getMessage());
		}
	}
	...
	public function enqueue_translations()
	{
		if (wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['nonce'])), 'polylai_run_cron')) {
			...
			update_post_meta(
				$id,
				POLYLAI_NEED_TR_KEY,
				['locales' => $locales, 'locales_names' => $locales_names, 'status' => $status]
			);
	...
	public function progress()
	{
		...
		try {
			$has_cron = get_transient(polylai_Cron::ACTIVITY_TRANSIENT_KEY);
			if (!$has_cron && polylai_Cron::getInstance()->check_for_posts()) {
				$exec_url = home_url('/index.php?action=polylai_run_cron');
				wp_remote_post($exec_url, [
					'timeout' => 0.01,
					'blocking' => false,
					'headers' => [
						'Connection' => 'Close',
