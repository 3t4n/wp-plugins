...
function add_post_footer_googleplusone( $text ){
	...
	if (
		...
		// if html5 valid syntax option is checked, prepend the attributes (ea data-width=..)
		if (get_option( 'googleplusone_htmlfive' ) == 1){ $preData='data-'; } else { $preData=''; }
		...
		// disable showing of recommendations in the hover window if option is selected
		if ( get_option( 'googleplusone_recommendations' ) == 1 ){ $recommendations = ' '.$preData.'recommendations="false" '; } else { $recommendations = ''; }
		// set width, default not set
		if (get_option( 'googleplusone_width' ) != ''){ $width=$preData.'width="'.get_option( 'googleplusone_width' ).'" '; } else { $width=''; }
		// set size, default = none
		if (get_option( 'googleplusone_size' ) != ''){ $size=$preData.'size="'.get_option( 'googleplusone_size' ).'" '; } else { $size=''; }
		// set count, can be inline, none and bubble (default)
		switch(get_option( 'googleplusone_displaycount' )){
			case 'inline': $displaycount=$preData.'annotation="inline"';	break;
			case 'bubble': $displaycount=''; break;
			default: $displaycount=$preData.'annotation="none"'; // none
		} // switch
		// js callback function
		if (get_option( 'googleplusone_jscallback' ) != ''){ $jscallback=$preData.'callback="'.trim(stripslashes(get_option( 'googleplusone_jscallback' ))).'" '; } else { $jscallback=''; }
		// code for the button
		if (get_option( 'googleplusone_htmlfive' ) == 1){
			$iframe='<div class="g-plusone" data-href="'.trim(get_permalink( $post->ID )).'" '.$size.' '.$displaycount.' '.$jscallback.' '.$width.' '.$recommendations.'></div>';
		} else {
			$iframe='<g:plusone href="'.trim(get_permalink( $post->ID )).'" '.$size.' '.$displaycount.' '.$jscallback.' '.$width.' '.$recommendations.'></g:plusone>';	
		} // if
...
function googleplusone_options_page(){
	if ( isset( $_POST ) ){
		if ( isset( $_POST['Submit'] ) ){
			...
			update_option( 'googleplusone_recommendations', $_POST['recommendations'] );
			update_option( 'googleplusone_showonlyinsingle', $_POST['showonlyinsingle'] );
			update_option( 'googleplusone_size', $_POST['size'] );
			update_option( 'googleplusone_width', $_POST['gp_width'] );
                ...
                <table class="form-table">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <br />
							<label for="googleone_width"><?php _e( 'Width', 'googleplusone' );?>:</label>
							<input name="gp_width" id="googleone_width" value="<?php echo stripslashes(( get_option( 'googleplusone_width' ) ));?>"/> <?php _e( 'Width must be a whole number, and at least 120(pixels). Leave blank if unsure.', 'googleplusone' );?>
							<br />                            
                            ...
                            <br />
                            <input type="checkbox" id="googleone_recommendations" value="1" <?php if (get_option( 'googleplusone_recommendations' ) == '1' ) echo 'checked="checked"'; ?> name="recommendations" />
                            <label for="googleone_recommendations"><?php _e( 'Disable showing recommendations within the hover window', 'googleplusone' );?></label>
							<br />							
                        </td>
                    </tr>
                    <tr>
