	...
	jQuery(document).ready(function($){
		...
		jQuery('#flipping-card').sortable({
			...
			update: function( event, ui ) {
				...
				jQuery.post(ajaxurl, {action: 'fc_order_img', id: jQuery(ui.item).find('img.thumbnail').attr('rel'), order: (ui.item.index()+1), _ajax_nonce: '<?php wp_create_nonce( "order_image_fc" ); ?>' });
	    ...
	    jQuery('#flipping-card img.remove').click(function(){
	    	...
	    	jQuery.post(ajaxurl, {action: 'fc_remove_img', id: jQuery(_this).attr('rel'), _ajax_nonce: '<?php echo esc_attr(wp_create_nonce( "remove_image_fc" )); ?>'}, function(){
...
<?php
...

	{
...

		{
			...
			echo '<li><img class="thumbnail" rel="'.(int)$img->id.'" src="'.esc_url($img->image).'" /><img class="remove" rel="'.(int)$img->id.'" src="'.esc_url(plugins_url( 'flipping-cards/images/remove.png' )).'" />

			<form class="form_edit_fc_img">';

				wp_nonce_field( 'update_image_fc_' .$img->id, "_wpnonce", true);

				echo '<input type="hidden" name="id" value="'.(int)$img->id.'" />

				<input type="hidden" name="action" value="fc_save_img" />

				<label>Image : </label><input type="text" name="image" value="'.esc_url($img->image).'"><input type="button" name="upload-btn" class="upload-btn button-secondary" value="Change Image"><br />

				<label>Text : </label><textarea name="text">'.esc_textarea($img->text).'</textarea><br />

				<label>Link : </label><input type="text" name="link" value="'.esc_url($img->link).'" /><input type="checkbox" name="blank" value="1" '.($img->blank == 1 ? 'checked="checked"' : '').' /> Open in a new window ?<br />

				<input type="submit" value="Save" /><img src="'.esc_url(plugins_url( 'flipping-cards/images/loading.gif' )).'" class="loading" />
