...
class autoglot_dom {
    ...
    function loadHTML($source) {
        $this->dom_content->loadHTML($source, LIBXML_HTML_NODEFDTD | LIBXML_SCHEMA_CREATE);
    ...
    function HTML2Array($sourceHTML){
        ...
        if($this->autoglot->options->translate_urls) {//need to grab all URLs
            ...
            if ($permalinks->length) foreach($permalinks as $node)
            {
                ...
                if (strpos($href, $this->autoglot->homeURL) !== FALSE && $type != "languageswitcher" && autoglot_utils::check_not_admin_link($href) && autoglot_utils::check_not_downloadable_file($href) ) {
        ...
        $metadoms = $xpath->query("//input[@type=\"submit\"]/@value | //button[@value!='']/@value");
        if ($metadoms->length) foreach($metadoms as $node)
        {
            $mvalue = $node->value;
            $metaarray[autoglot_utils::gettexthash($mvalue)] = $mvalue;
        }
        $metadoms = $xpath->query("//input[@placeholder!='']/@placeholder | //textarea[@placeholder!='']/@placeholder");
        if ($metadoms->length) foreach($metadoms as $node)
        {
            $mvalue = $node->value;
            $metaarray[autoglot_utils::gettexthash($mvalue)] = $mvalue;
        }
        $metadoms = $xpath->query("//a[@title!='']/@title | //img[@title!='']/@title | //img[@alt!='']/@alt");
        if ($metadoms->length) foreach($metadoms as $node)
        {
            $mvalue = $node->value;
            $metaarray[autoglot_utils::gettexthash($mvalue)] = $mvalue;
        }
        if(count($this->autoglot->third_party->json_data_strings)) {
            
            $find = $this->autoglot->third_party->json_data_strings;
            $json_array = array();

            // Iterate over the $find array to search for JavaScript variables
            foreach ($find as $variable => $keysToCheck) {
                // Regex to find the JSON object assigned to the variable
                $pattern = "/var\s+" . preg_quote($variable, '/') . "\s*=\s*(\{.*?\});/s";
                if (preg_match($pattern, $sourceHTML, $matches)) {
                    // Decode the JSON content
                    $jsonContent = $matches[1];
                    $decodedArray = json_decode($jsonContent, true);

                    if (json_last_error() === JSON_ERROR_NONE) {
                        // Collect the values of the keys in the decoded array
                        $json_array = array_merge($json_array, autoglot_utils::get_json_values($decodedArray, $keysToCheck));
                    }
                }
            }
            $metaarray = array_merge($metaarray, $json_array);
        }
...
//file_put_contents(__DIR__."/debug.txt", $innerHTML."\r\n", FILE_APPEND);
    ...
    function Array2HTML($array_translated){
        ...
        $metadoms = $xpath->query("//input[@type=\"submit\"][@value!=''] | //button[@value!='']");
        if ($metadoms->length) foreach($metadoms as $node)
        {
            $mvalue = $node->getAttribute("value");
            if(array_key_exists(autoglot_utils::gettexthash($mvalue), $this->array_translated)){
                $node->setAttribute("value", $this->array_translated[autoglot_utils::gettexthash($mvalue)]);
            }
        }
        $metadoms = $xpath->query("//button[@id=\"place_order\"][@data-value!='']");//4 WooCommerce
        if ($metadoms->length) foreach($metadoms as $node)
        {
            $mvalue = $node->getAttribute("data-value");
            if(array_key_exists(autoglot_utils::gettexthash($mvalue), $this->array_translated)){
                $node->setAttribute("data-value", $this->array_translated[autoglot_utils::gettexthash($mvalue)]);
            }
        }
        $metadoms = $xpath->query("//input[@placeholder!=''] | //textarea[@placeholder!='']");
        if ($metadoms->length) foreach($metadoms as $node)
        {
            $mvalue = $node->getAttribute("placeholder");
            if(array_key_exists(autoglot_utils::gettexthash($mvalue), $this->array_translated)){
                $node->setAttribute("placeholder", $this->array_translated[autoglot_utils::gettexthash($mvalue)]);
            }
        }
        $metadoms = $xpath->query("//a[@title!=''] | //img[@title!='']");
        if ($metadoms->length) foreach($metadoms as $node)
        {
            $mvalue = $node->getAttribute("title");
            if(array_key_exists(autoglot_utils::gettexthash($mvalue), $this->array_translated)){
                $node->setAttribute("title", $this->array_translated[autoglot_utils::gettexthash($mvalue)]);
            }
        }
        $metadoms = $xpath->query("//img[@alt!='']");
        if ($metadoms->length) foreach($metadoms as $node)
        {
            $mvalue = $node->getAttribute("alt");
            if(array_key_exists(autoglot_utils::gettexthash($mvalue), $this->array_translated)){
                $node->setAttribute("alt", $this->array_translated[autoglot_utils::gettexthash($mvalue)]);
            }
        }
        ...
        $add_lngcode_links_to = $add_lngcode_links_from = array();
        if(is_array($this->autoglot->options->add_lngcode)){
            $add_lngcode_links_from = explode("\r\n", $this->autoglot->options->add_lngcode[0]);
            $add_lngcode_links_to = explode("\r\n", str_replace(array(AUTOGLOT_ADDLINKCODE_LNG, AUTOGLOT_ADDLINKCODE_LNG2, AUTOGLOT_ADDLINKCODE_DMN), array($this->autoglot->langURL, strtr($this->autoglot->langURL, AUTOGLOT_MORE_LANGCODES), autoglot_utils::get_language_flag($this->autoglot->langURL)),$this->autoglot->options->add_lngcode[1]));
            if(count($add_lngcode_links_from) != count($add_lngcode_links_to)) $add_lngcode_links_to = $add_lngcode_links_from = array();
        }
        ...
        if ($permalinks->length) foreach($permalinks as $node)
        {
            ...
            $href = autoglot_utils::make_link_absolute($href, $this->autoglot->homeURL);
            // Check if the link contains the old domain and does not contain admin links 
            if ($type != "languageswitcher" && autoglot_utils::check_not_admin_link($href) && autoglot_utils::check_not_downloadable_file($href) ) {
                if (strpos($href, $this->autoglot->homeURL) !== FALSE) {
                    $trylang = autoglot_utils::get_language_from_url($href, $this->autoglot->homeURL);
                    if($trylang && strlen($trylang)>1 && in_array($trylang, $this->autoglot->options->active_languages)) {
                        //language already here, nothing to do?
                    } else {
                ...
                } elseif(is_array($add_lngcode_links_from) && count ($add_lngcode_links_from)) {
                    $href_add_langcode = str_replace($add_lngcode_links_from, $add_lngcode_links_to, $href);
                    $node->setAttribute('href', $href_add_langcode);
                }
            }
        }
        ...
        if(count($this->autoglot->third_party->json_data_strings)) {
            ...
            foreach ($find as $variable => $keysToCheck) {
                // Regex to find the JSON object assigned to the variable
                $pattern = "/var\s+" . preg_quote($variable, '/') . "\s*=\s*(\{.*?\});/s";
                if (preg_match($pattern, $translatedHTML, $matches)) {
                    // Decode the JSON content
                    $jsonContent = $matches[1];
                    $decodedArray = json_decode($jsonContent, true);
                    ...
                    if (json_last_error() === JSON_ERROR_NONE) {
                        // Collect the values of the keys in the decoded array
                        $json_out = autoglot_utils::set_json_values($decodedArray, $keysToCheck, $this->array_translated);
                        $updatedJsonContent = json_encode($json_out);
                        $translatedHTML = preg_replace($pattern, "var $variable = $updatedJsonContent;", $translatedHTML);
                    }
                }
            }
        }
    ...
    function DOM2HTML(){
        ...
        $output = "";
        if($this->autoglot->json_custom_request || wp_is_json_request() || $this->autoglot->response_content_type == "json"){
            $body = $this->dom_translated->getElementsByTagName('body')->item(0);
            if ($body) foreach ($body->childNodes as $child) {
                $output .= $this->dom_translated->saveHTML($child);
            }
        } 
        else $output = $this->dom_translated->saveHTML($this->dom_translated);
        return $output;   
    }
