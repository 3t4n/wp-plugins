...
class autoglot_editor extends WP_List_Table
{
    ...
    function column_translated($item) {
        if($item["translated"] !== AUTOGLOT_TRANSLATION_INPROGRESS) {
            $actions = array(
                // 'edit' => sprintf('<a href="?page=%s&action=%s&book=%s">Edit</a>', $_REQUEST['page'], 'edit', 1/*$item['ID']*/),
            ...
            $actions = array(
                'utilities' => '<a href="' . admin_url(sprintf('admin.php?page=%s">', "autoglot_translation_utilities")) . __('Remove empty translations', 'autoglot') . "</a>",
            );
            return "<code>".__('Translation currently in progress', 'autoglot')." <span class=\"dashicons dashicons-update-alt\"></span></code>". $this->row_actions($actions);
        }
    }
