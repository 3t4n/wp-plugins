...
if ( !class_exists('Autoglot') ) {
    ...
    class Autoglot {
        ...
        /** is it custom ajax/json request like woo? */
        public $json_custom_request = false;
        
        /** content_type of generated response */
        public $response_content_type;
        
        /** we need permalinks for Autoglot to function */
        public $using_permalinks = false;

        function __construct() {

            if ( get_option('permalink_structure')) { //we need permalinks
            ...
            if (autoglot_utils::is_custom_json()) {//is custom ajax/json like woocommerce requests
                $this->langURL = autoglot_utils::get_language_from_url($_SERVER['HTTP_REFERER'], $this->homeURL);
                $this->json_custom_request = true;
            }elseif(wp_is_json_request()) {//not custom ajax/json but json, so check lang and try from referer 
                if(!strlen($this->langURL)) $this->langURL = autoglot_utils::get_language_from_url($_SERVER['HTTP_REFERER'], $this->homeURL);
            }
            ...
            if(!$this->using_permalinks) return;//makes no sense to continue without permalinks
            ...
            /*woo hooks for ajax with referrer*/
            add_filter('woocommerce_get_checkout_order_received_url', array($this, 'autoglot_woocommerce_filter_url'));
            ...
            if ( (!is_admin() || (is_admin() && wp_doing_ajax())) && count($this->options->active_languages) > 1){//no admin dashboard (except for admin-ajax.php) and active_languages > default language
				...
				if(strlen($this->langURL) && $this->language_active($this->langURL) && $this->autoglot_database->db_exists()) {
                    ...
                    add_filter( 'comments_template_query_args', array($this, 'autoglot_hide_unapproved_comments'));

                    /*woo hooks*/
                    add_filter('woocommerce_get_cart_url', array($this, 'autoglot_woocommerce_filter_url'));
                    add_filter('woocommerce_get_checkout_url', array($this, 'autoglot_woocommerce_filter_url'));
                }
            }      
        ...
        function add_autoglot_floatbox() {
            ...
            if(count($this->options->active_languages) && (!$this->options->translation_adminonly || $this->options->translation_adminonly && current_user_can('manage_options'))){
                ...
                switch($this->options->floatbox_position){
                    case "topleft":
                        $customcss = "top: 0; left: 80px;";
                    break;
                    case "topright":
                        $customcss = "top: 0; right: 80px;";
                    break;
                    case "bottomleft":
                        $customcss = "bottom: 0; left: 80px;";
                    break;
                    case "bottomright":
                    default:
                        $customcss = "bottom: 0; right: 80px;";
                }

                echo '<div id="ag_floatblox" class="'.AUTOGLOT_NOTRANSLATE_LANGUAGESWITCHER.'" style="'.$customcss.'"><a href="#" name="ag_modal" box="ag_languageswitcher">';
        ...
        /**
         * Try to add lang ID in case of wp_redirect
         */

        function on_wp_redirect($location, $status) {

            $trylang = autoglot_utils::get_language_from_url($location, $this->homeURL);
            if (strlen($trylang) && $this->language_active($trylang)) {
                return $location;
            } else {
                if($this->options->translate_urls) $location = $this->translate_url($location, $this->langURL); 
                $location = autoglot_utils::add_language_to_url($location, $this->homeURL, $this->langURL);
        ...
        function process_page($buffer) {
            ...
            $this->response_content_type = autoglot_utils::get_response_content_type();
            
            if(!strlen($buffer)) return $buffer;
            
            //check content-type, need to be html or json, we don't process other types now
            if($this->response_content_type != "json" && $this->response_content_type != "html"){
                return $buffer;
            }

            //someone flushed buffer? who let the dogs out?
            if ($this->langURL == '') {
                if (!$buffer) {
            ...
            if($this->response_content_type == "json" && !$this->json_custom_request){//it's json, let's check if it contains [html], return if not
                if ($buffer[0] == '{') {
                    $tryjson = json_decode($buffer);
                    if($tryjson) {
                        //file_put_contents(__DIR__."/debug".time().".txt", print_r($tryjson->fragments, true));
                        if (isset($tryjson->html) && strlen($tryjson->html)) {
                            $tryjson->html = $this->translate_json($tryjson->html);
                            //$tryjson->translated_by = "Autoglot";
                            return json_encode($tryjson); // return translated json 
                        }
                    }
                }
                return $buffer;
            ...
            if($this->json_custom_request) {
                if ($buffer[0] == '{') {
                    $tryjson = json_decode($buffer);
                    if($tryjson) {
                        //file_put_contents(__DIR__."/debug".time().".txt", print_r($tryjson->fragments, true), FILE_APPEND);
                        if ($tryjson->fragments->{'.woocommerce-checkout-review-order-table'}) {
                            $tryjson->fragments->{'.woocommerce-checkout-review-order-table'} = $this->translate_json($tryjson->fragments->{'.woocommerce-checkout-review-order-table'});
                        }
                        if ($tryjson->fragments->{'.woocommerce-checkout-payment'}) {
                            $tryjson->fragments->{'.woocommerce-checkout-payment'} = $this->translate_json($tryjson->fragments->{'.woocommerce-checkout-payment'});
                        }
                        //file_put_contents(__DIR__."/debug".time().".txt", print_r($tryjson->fragments, true), FILE_APPEND);
...
//debug echo htmlspecialchars($html);
        ...
        function translate_json($json_string) {

            if(!strlen($json_string)) return $json_string;

            $json_string = mb_convert_encoding($json_string, 'HTML-ENTITIES', 'UTF-8');

            $arraycontent = $this->dom->HTML2Array($json_string);

            $arraytranslated = $this->translate_content($arraycontent, $this->langURL, "content", get_the_ID());

            //file_put_contents(__DIR__."/debug".time().".txt", print_r($arraytranslated, true), FILE_APPEND);                        
            $this->page_links_translated = $this->translate_urls($this->page_links, $this->langURL);
            //file_put_contents(__DIR__."/debug_tr".time().".txt", $this->langURL."\r\n\r\n".print_r($this->page_links_translated, true), FILE_APPEND);                        

            $json_string_translated = $this->dom->Array2HTML($arraytranslated);

            //replace links to important sites
            //if(is_array($this->options->add_lngcode)) $json_string_translated = $this->languagelinks($json_string_translated);
            
            //replace other things
            if(isset($this->options->replace_text) && @count($this->options->replace_text)) $json_string_translated = strtr($json_string_translated, $this->options->replace_text);

            $json_string_translated = str_replace(chr(0xC2) . chr(0xA0), '&nbsp;', $json_string_translated);

            return $json_string_translated;
        }
        ...
        /**
         * Do not show unapproved comments to prevent excess translation costs
         */
        function autoglot_hide_unapproved_comments($comment_args) {
            if(array_key_exists("include_unapproved", $comment_args) && is_array($comment_args["include_unapproved"])) {
                add_action( 'comment_form_before', function(){//let's mimic standard WP behaviour
        ...
        /**
         * Redirect user to correct language in woocommerce
         */
        function autoglot_woocommerce_filter_url($url) {

            if(!strlen($url)) return;
            if($this->langURL == autoglot_utils::get_language_from_url($url, $this->homeURL)) return $url;

            if($this->langURL){
                if($this->options->translate_urls) $url = $this->translate_url($url, $this->langURL); 
                $url = autoglot_utils::add_language_to_url($url, $this->homeURL, $this->langURL);
            }elseif(strlen($_SERVER['HTTP_REFERER'])){//ajax?
                $trylang = autoglot_utils::get_language_from_url($_SERVER['HTTP_REFERER'], $this->homeURL);
                if (strlen($trylang) && $this->language_active($trylang)) {
                    if($this->options->translate_urls) $url = $this->translate_url($url, $trylang); 
                    $url = autoglot_utils::add_language_to_url($url, $this->homeURL, $trylang);
