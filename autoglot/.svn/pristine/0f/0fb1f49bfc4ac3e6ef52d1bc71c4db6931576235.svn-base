...
class autoglot_3rdparty {
    ...
    public $json_data_strings;

    function __construct(&$autoglot) {
        
        $this->autoglot = &$autoglot;
        
        $this->json_data_strings = array();
...
//                add_filter( 'rank_math/sitemap/enable_caching', '__return_false');
    		...
    		if(strlen($this->autoglot->langURL) && $this->autoglot->language_active($this->autoglot->langURL)) {
                add_action( 'wp_head', array(&$this, 'woocommerce_autoglot') );

                /*add_filter( 'plugin_locale', function ( $locale, $textdomain ) {what if....
    ...
    /**
     * Woocommerce-related stuff
     */

    function woocommerce_autoglot(){

        if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
            
            if(is_shop() || is_cart() || is_checkout()){
                $this->json_data_strings["wc_add_to_cart_params"] = array("i18n_view_cart");
            }
    
            if(is_cart() || is_checkout()) {
                $this->json_data_strings["wc_address_i18n_params"] = array("label", "placeholder", "i18n_required_text", "i18n_optional_text");
            }
            
            if(is_product()) {
                $this->json_data_strings["wc_single_product_params"] = array("i18n_required_rating_text");
            }
        }
    }
