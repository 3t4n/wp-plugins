...
class autoglot_admin {
	...
	public function __construct( &$autoglot, $plugin_name, $version ) {
        ...
        if($this->autoglot->autoglot_database->db_exists())
            foreach ($this->autoglot->options->active_languages as $lng) 
                if($lng != $this->autoglot->options->default_language)
                    $this->db_stats["w2translate"] += max(0,$this->db_stats['wpcount'] - $this->db_stats['countactivewords'][$lng]);   
        ...
        if(!$this->autoglot->using_permalinks)$this->notification_count++;
	...
	public function section_callback( $arguments ) {
		switch( $arguments['id'] ){
			...
			case 'section_dashboard':
                ...
                echo '</td><td></td></tr></thead><tbody>';
                printf('<tr><td>'.(!$this->autoglot->using_permalinks?'<span class="autoglot-bubble-red">!</span> ':'') . __('Permalinks enabled:', 'autoglot') . '</td>'.($this->autoglot->using_permalinks?'<td style="background-color:#99FF99"><strong><i class="dashicons dashicons-yes"></i></strong></td></tr>':'<td style="background-color:#FF9999"><strong><i class="dashicons dashicons-no"></i></strong> <a href="%s">'.__('Permalinks must be enabled to let Autoglot detect the language in URLs.', 'autoglot').'</a></td></tr>').'', admin_url( 'options-permalink.php'));
                ...
                printf('<tr><td>'.($this->autoglot->options->translation_adminonly?'<span class="autoglot-bubble-red">!</span> ':'') . __('Translated pages published:', 'autoglot') . '</td>'.(!$this->autoglot->options->translation_adminonly ?'<td style="background-color:#99FF99"><strong><i class="dashicons dashicons-yes"></i></strong></td></tr>':'<td style="background-color:#FF9999"><strong><i class="dashicons dashicons-no"></i></strong> '.__('Unpublished. Only site administrators can see and check translated pages.', 'autoglot').' <a href="%s">'.__('Click here to enable translation for everyone.', 'autoglot').'</a></td></tr>').'', admin_url( 'admin.php?page=autoglot_translation_settings'));
                ...
                if(count($this->autoglot->options->active_languages)>1){
                    if(is_plugin_active( 'woocommerce/woocommerce.php' )) {//recalculate and allow 500 words for WooCommerce pages
                        foreach ($this->autoglot->options->active_languages as $lng) 
                            if($lng != $this->autoglot->options->default_language)
                                $this->db_stats["w2translate"] += max(0, 500 + $this->db_stats['wpcount'] - $this->db_stats['countactivewords'][$lng]);   
                    $this->db_stats["w2translate"] = max(0, $this->db_stats["w2translate"]);
                    }
                ...
                if(is_plugin_active( 'woocommerce/woocommerce.php' )) {//recalculate and allow 500 words for WooCommerce pages
                    echo '<h2>'.__('WooCommerce Integration', 'autoglot').'</h2>';
    				echo '<p>' . __('There are a few important things to remember about WooCommerce integration.', 'autoglot') . '</p>';
                    echo '<table class="wp-list-table widefat fixed"><thead><tr><td>';
                    echo '<strong>' . __('WooCommerce detected on your website', 'autoglot') . '</strong>';
                    echo '</td><td><span class="autoglot-bubble-woo">&check;</span></td></tr></thead><tbody>';
                    echo '<tr><td>' . __('WooCommerce cart and checkout pages:', 'autoglot') . '</td><td>Please use classic cart and checkout pages. New block pages are JavaScript-based and therefore cannot be translated on the server.</td></tr>';
                    echo '<tr><td>' . __('WooCommerce cart and checkout translation:', 'autoglot') . '</td><td>In addition to the words calculated in the Word Counter above, please allocate up to 1,000 words per language for translation of cart and checkout pages.</td></tr>';
                    echo '<tr><td>' . __('Dynamic translation:', 'autoglot') . '</td><td>WooCommerce may show unique dynamic messages for different users. If they have not been shown before, Autoglot will translate them using the translation balance.</td></tr>';
                    echo '<tr><td>' . __('Email messages:', 'autoglot') . '</td><td>Currently, Autoglot does not translate outgoing emails in order to prevent excess charges.</td></tr>';
                    echo '</tbody></table>';
                }
                ...
                echo '</td><td></td></tr></thead><tbody>';
                printf('<tr><td>' . __('Number of records in Autoglot translation DB:', 'autoglot') . '</td><td><a href="%s">%s</a></td></tr>', admin_url( 'admin.php?page=autoglot_translation_editor'), number_format_i18n($this->db_stats['countall']));
...
/*				echo '<li>';
	...
	public function setup_fields() {
        ...
        if(isset($this->setup_wizard) && in_array($this->setup_wizard, array(1,2,3))){
            ...
            $language_switcher_positions = array(
				'bottomright' => __( 'Bottom Right', 'autoglot'),
				'bottomleft' => __( 'Bottom Left', 'autoglot'),
				'topright' => __( 'Top Right', 'autoglot'),
				'topleft' => __( 'Top Left', 'autoglot'),
            );
    		...
    		$fields = array(
    			...
    			array(
    				...
    				'label' => __('Translations Published', 'autoglot'),
    				'section' => 'section_main',
                    'page' => 'autoglot_translation_settings',
    				'helper' => '',
    				'supplemental' => __('This can be useful to let admins preview translations before publishing them for all visitors. Unpublishing translated pages will hide "alternate hreflang" block and will make widget visible only for admin.', 'autoglot'),
    				'type' => 'radio',
    				'options' => array(
    					1 => __('Unpublished. Only site administrators can preview translated pages.', 'autoglot'),
    					0 => __('Published. Every site visitor can see translated pages.', 'autoglot'),
    				),
    			...
    			array(
                    ...
                    'page' => 'autoglot_translation_settings',
    				'type' => 'radio',
    				'supplemental' => sprintf(__('For custom positioning, please add and modify %s element in Customize - Additional CSS or Site Editor - Styles.', 'autoglot'), "<code>#ag_floatblox</code>"),
    				'options' => array(
    			...
    			array(
    				'uid' => 'autoglot_translation_floatbox_position',
    				'label' => __('Position of Floating Box with Language Switcher', 'autoglot'),
    				'section' => 'section_switcher',
                    'page' => 'autoglot_translation_settings',
    				'supplemental' => sprintf(__('For custom positioning, please add and modify %s element in Customize - Additional CSS or Site Editor - Styles.', 'autoglot'), "<code>#ag_floatblox</code>"),
    				'type' => 'select',
    				'options' => $language_switcher_positions,
    				'default' => array(autoglot_consts::LANGUAGE_SWITCHER_POSITIONS[0]),
                    'sanitize_callback' => array($this, 'sanitize_language_switcher_positions'),
    			),
    			array(
    				'uid' => 'autoglot_translation_default_language',
    				'label' => __('Default Language', 'autoglot'),
    ...
    //                array_combine(autoglot_consts::REPEAT_BALANCE_NOTIFICATIONS,$translatedelays),
    			...
    			array(
    				...
    				'label' => __('Links For Replacement', 'autoglot'),
    				'section' => 'section_linksmod',
                    'page' => 'autoglot_translation_linksmod',
    				'helper' => '',
                    'toplabel' => array(__('Original Links, one per line.', 'autoglot'), __('Replaced Links, one per line', 'autoglot')),
    ...
    function check_options() {
        ...
        if ( $pagenow == 'admin.php' && strpos($_GET['page'], 'autoglot_translation')!==false && !$this->setup_wizard) {
            ...
            if(!strlen($this->autoglot->using_permalinks)){
                $this->admin_notice(sprintf(__('Permalinks must be enabled to let Autoglot detect the language in URLs.', 'autoglot').' <a href="%s">'.__('Please change permalinks structure here.', 'autoglot').'</a>', admin_url( 'options-permalink.php')),"error");
            }
        }
    }
...
/*			case 'image':
    ...
    public function sanitize_textarea2($input){
        if(is_array($input) && count($input) == 2){
            if(strlen(trim($input[0])) == 0) return;
            if(strlen(trim($input[1])) == 0) return;
            $from = explode("\r\n", $input[0]);
            $to = explode("\r\n", $input[1]);
            if(is_array($from) && is_array($to))
            {
                if(count($from) > count ($to)){
                    $from = array_slice($from, 0, count($to));
                    $input[0] = implode ("\r\n", $from);
    ...
    public function sanitize_language_switcher_positions ($input){
        if(is_array($input) && in_array($input[0], autoglot_consts::LANGUAGE_SWITCHER_POSITIONS, 1)) {return $input;}
        else return;
    }
...
//		add_rewrite_rule( $url_slug . '/([a-zA-Z0-9\-]{1,})/?$', 'index.php?autoglot_translation=detail&autoglot_translation_vehicle=$matches[1]', 'top' );
	...
	function glance_word_count() {
        ...
        printf('<style scoped>.ag_langs-count a:before { content:\'\\f533\' !important; }</style><li class=\'ag_langs-count\'><a href='.$url_langs.'><tr><td>' . __('%s Languages active', 'autoglot') . '</a></li>', count($this->autoglot->options->active_languages)-1);
