...
<div class="content-section">
	<div class="container-pj">
		...
		<input type="hidden" id="hiddenhost" value="<?php if (isset($project_details->data->domain->public_host)) {
														echo wp_kses_data($project_details->data->domain->public_host);
													} ?>">
			...
			<div id="config" class="tabcontent tbpad content-card" style="display: block">
				<?php if (isset($redirection_flag) && $redirection_flag === true) { ?>
					<div class="notice notice-success is-dismissible ">
						<p>Successfully saved redirection settings</p><span class="closeicon" aria-hidden="true">&times;</span>
					</div>
				<?php }
				if (isset($redirection_flag) && $redirection_flag === 408) { ?>
					<div class="notice notice-error is-dismissible ">
						<p>SORTD Server is not reachable</p><span class="closeicon" aria-hidden="true">&times;</span>
					</div>
				<?php }
				update_option('sortd_' . $project_id . '_redirection_status', 0);  ?>
					...
					<div id="sortd_services">
						...
						<div class="serviceChek">
							<div class="chkServ form-check form-check-inline">
								<input class="form-check-input sortd_services_dropdown" type="radio" name="inlineRadioOptions" id="inlineRadio1" <?php if ($sortd_services === 'pwa_and_amp_both') {
																																						$checked = "checked";
																																					} else {
																																						$checked = '';
																																					}
																																					echo esc_attr($checked); ?> value="pwa_and_amp_both">
								<label class="form-check-label" for="inlineRadio1">PWA & AMP </label>
								<span>(Enable mobile site and amp pages)(recommended)</span>
							</div>
							<div class="chkServ form-check form-check-inline">
								<input class="form-check-input sortd_services_dropdown" type="radio" name="inlineRadioOptions" id="inlineRadio2" <?php if ($sortd_services === 'pwa_only') {
																																						$checked = "checked";
																																					} else {
																																						$checked = '';
																																					}
																																					echo esc_attr($checked); ?> value="pwa_only">
								<label class="form-check-label" for="inlineRadio2">PWA</label>
								<span>(Enable mobile website)</span>
							</div>
							<div class="chkServ form-check form-check-inline">
								<input class="form-check-input sortd_services_dropdown" type="radio" name="inlineRadioOptions" id="inlineRadio3" <?php if ($sortd_services === 'amp_default') {
																																						$checked = "checked";
																																					} else {
																																						$checked = '';
																																					}
																																					echo esc_attr($checked); ?> value="amp_default">
								<label class="form-check-label" for="inlineRadio3">AMP</label>
								<span>(Enable AMP pages for mobile website)</span>
							</div>
							<div class="chkServ form-check form-check-inline">
								<input class="form-check-input sortd_services_dropdown" type="radio" name="inlineRadioOptions" id="inlineRadio3" <?php if ($sortd_services === 'amp_only') {
																																						$checked = "checked";
																																					} else {
																																						$checked = '';
																																					}
																																					echo esc_attr($checked); ?> value="amp_only">
				...
				<script type="text/javascript">
					...
					function sortd_addMoreExcludeUrl() {
						...
						textfield.type = "text";
						textfield.id = "exclude_url_add" + addCount;
						// textfield.style= "";
						...
						spanfield.innerHTML = "Space is not allowed";
						spanfield.id = "exclude_url_span_add" + addCount;
						...
						spanfieldvalidate.innerHTML = "Please enter value or remove the fields";
						spanfieldvalidate.id = "exclude_url_spanvalida_add" + addCount;
						//spanfield.style= "width:90%;margin-left:5%;color:red;display:none";
...
<script>
	<?php echo wp_kses_post($chatbot_dashboard_data->data); ?>
</script>
<?php
//page function end
