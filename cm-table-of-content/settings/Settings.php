...
abstract class Settings {
    ...
    public static function renderSetting($key, $config) {
		...
		if (isset($config['html']) && $config['html'] != '') {
			if (strpos($config['html'], 'Available in Pro version and above') !== false) {
				$class = $class.' onlyinpro';
				$style = 'display:initial;';
			}
		}
        ?>
        <tr valign="top" class="<?php echo $class; ?> <?php echo $key; ?> wrapper-<?php echo esc_attr($config['type']); ?>">
            <th scope="row">
                <div><?php echo esc_attr($config['label']); ?></div>
            </th>
            <td class="field-<?php echo esc_attr($config['type']); ?>" style="<?php echo $style; ?>">
                <?php
