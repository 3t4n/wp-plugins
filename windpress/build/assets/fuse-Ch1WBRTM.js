...
function ft(e) {
  return e === true || e === false || dt(e) && st(e) == "[object Boolean]";
}
function et(e) {
...
function nt(e) {
  ...
  if (M(e) || m(e)) n = e, t = U(e), s = j(e);
  else {
    if (!J.call(e, "name")) throw new Error(Et("name"));
    const u = e.name;
    if (n = u, J.call(e, "weight") && (r = e.weight, r <= 0)) throw new Error(Ct(u));
...
function Bt(e, t) {
  let s = [], n = false;
  const r = (i, u, c) => {
    if (E(i)) if (!u[c]) s.push(i);
    else {
      let o = u[c];
      const h = i[o];
      if (!E(h)) return;
      if (c === u.length - 1 && (M(h) || tt(h) || ft(h))) s.push(lt(h));
      else if (m(h)) {
        n = true;
        for (let a = 0, f = h.length; a < f; a += 1) r(h[a], u, c + 1);
      } else u.length && r(h, u, c + 1);
    }
  };
  return r(e, M(t) ? t.split(".") : t, 0), n ? s : s[0];
}
const Mt = { includeMatches: false, findAllMatches: false, minMatchCharLength: 1 }, Dt = { isCaseSensitive: false, ignoreDiacritics: false, includeScore: false, keys: [], shouldSort: true, sortFn: (e, t) => e.score === t.score ? e.idx < t.idx ? -1 : 1 : e.score < t.score ? -1 : 1 }, mt = { location: 0, threshold: 0.6, distance: 100 }, yt = { useExtendedSearch: false, getFn: Bt, ignoreLocation: false, ignoreFieldNorm: false, fieldNormWeight: 1 };
var l = { ...Dt, ...Mt, ...mt, ...yt };
const xt = /[^ ]+/g;
function _t(e = 1, t = 3) {
...
class Y {
  constructor({ getFn: t = l.getFn, fieldNormWeight: s = l.fieldNormWeight } = {}) {
    this.norm = _t(s, 3), this.getFn = t, this.isCreated = false, this.setIndexRecords();
  }
  setSources(t = []) {
    this.docs = t;
  }
  ...
  create() {
    this.isCreated || !this.docs.length || (this.isCreated = true, M(this.docs[0]) ? this.docs.forEach((t, s) => {
      this._addString(t, s);
  ...
  _addString(t, s) {
    if (!E(t) || P(t)) return;
    let n = { v: t, i: s, n: this.norm.get(t) };
    this.records.push(n);
  }
  _addObject(t, s) {
    let n = { i: s, $: {} };
...
function $(e, { errors: t = 0, currentLocation: s = 0, expectedLocation: n = 0, distance: r = l.distance, ignoreLocation: i = l.ignoreLocation } = {}) {
  const u = t / e.length;
  if (i) return u;
...
const w = 32;
function wt(e, t, s, { location: n = l.location, distance: r = l.distance, threshold: i = l.threshold, findAllMatches: u = l.findAllMatches, minMatchCharLength: c = l.minMatchCharLength, includeMatches: o = l.includeMatches, ignoreLocation: h = l.ignoreLocation } = {}) {
  if (t.length > w) throw new Error(pt(w));
  const a = t.length, f = e.length, d = Math.max(0, Math.min(n, f));
  let g = i, A = d;
  const p = c > 1 || o, F = p ? Array(f) : [];
  let y;
  for (; (y = e.indexOf(t, A)) > -1; ) {
    let C = $(t, { currentLocation: y, expectedLocation: d, distance: r, ignoreLocation: h });
  ...
  for (let C = 0; C < a; C += 1) {
    let x = 0, _ = S;
    for (; x < _; ) $(t, { errors: C, currentLocation: d + _, expectedLocation: d, distance: r, ignoreLocation: h }) <= g ? x = _ : S = _, _ = Math.floor((S - x) / 2 + x);
    S = _;
    let V = Math.max(1, d - _ + 1), T = u ? f : Math.min(d + _, f) + a, R = Array(T + 2);
    R[T + 1] = (1 << C) - 1;
    for (let B = T; B >= V; B -= 1) {
      let O = B - 1, Q = s[e.charAt(O)];
      if (p && (F[O] = +!!Q), R[B] = (R[B + 1] << 1 | 1) & Q, C && (R[B] |= (D[B + 1] | D[B]) << 1 | 1 | D[B + 1]), R[B] & ht && (L = $(t, { errors: C, currentLocation: O, expectedLocation: d, distance: r, ignoreLocation: h }), L <= g)) {
        if (g = L, A = O, A <= d) break;
        V = Math.max(1, 2 * d - A);
      }
    }
    if ($(t, { errors: C + 1, currentLocation: d, expectedLocation: d, distance: r, ignoreLocation: h }) > g) break;
    D = R;
  }
  const v = { isMatch: A >= 0, score: Math.max(1e-3, L) };
  if (p) {
    const C = St(F, c);
    C.length ? o && (v.indices = C) : v.isMatch = false;
  }
  return v;
}
function Lt(e) {
...
class it {
  constructor(t, { location: s = l.location, threshold: n = l.threshold, distance: r = l.distance, includeMatches: i = l.includeMatches, findAllMatches: u = l.findAllMatches, minMatchCharLength: c = l.minMatchCharLength, isCaseSensitive: o = l.isCaseSensitive, ignoreDiacritics: h = l.ignoreDiacritics, ignoreLocation: a = l.ignoreLocation } = {}) {
    if (this.options = { location: s, threshold: n, distance: r, includeMatches: i, findAllMatches: u, minMatchCharLength: c, isCaseSensitive: o, ignoreDiacritics: h, ignoreLocation: a }, t = o ? t : t.toLowerCase(), t = h ? k(t) : t, this.pattern = t, this.chunks = [], !this.pattern.length) return;
    const f = (g, A) => {
      this.chunks.push({ pattern: g, alphabet: Lt(g), startIndex: A });
    }, d = this.pattern.length;
    if (d > w) {
      let g = 0;
      const A = d % w, p = d - A;
      for (; g < p; ) f(this.pattern.substr(g, w), g), g += w;
      if (A) {
        const F = d - w;
  ...
  searchIn(t) {
    ...
    const { location: i, distance: u, threshold: c, findAllMatches: o, minMatchCharLength: h, ignoreLocation: a } = this.options;
    let f = [], d = 0, g = false;
    this.chunks.forEach(({ pattern: p, alphabet: F, startIndex: y }) => {
      const { isMatch: D, score: L, indices: S } = wt(t, p, F, { location: i + y, distance: u, threshold: c, findAllMatches: o, minMatchCharLength: h, includeMatches: r, ignoreLocation: a });
      D && (g = true), d += L, D && S && (f = [...f, ...S]);
    });
...
class ut extends I {
  constructor(t, { location: s = l.location, threshold: n = l.threshold, distance: r = l.distance, includeMatches: i = l.includeMatches, findAllMatches: u = l.findAllMatches, minMatchCharLength: c = l.minMatchCharLength, isCaseSensitive: o = l.isCaseSensitive, ignoreDiacritics: h = l.ignoreDiacritics, ignoreLocation: a = l.ignoreLocation } = {}) {
    super(t), this._bitapSearch = new it(t, { location: s, threshold: n, distance: r, includeMatches: i, findAllMatches: u, minMatchCharLength: c, isCaseSensitive: o, ignoreDiacritics: h, ignoreLocation: a });
  }
  static get type() {
    return "fuzzy";
  }
...
class ct extends I {
  ...
  search(t) {
    ...
    for (; (n = t.indexOf(this.pattern, s)) > -1; ) s = n + i, r.push([n, s - 1]);
    const u = !!r.length;
    return { isMatch: u, score: u ? 0 : 1, indices: r };
  }
}
const K = [Rt, ct, Ot, $t, Nt, kt, bt, ut], Z = K.length, vt = / +(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)/, Tt = "|";
function Pt(e, t = {}) {
...
class Kt {
  constructor(t, { isCaseSensitive: s = l.isCaseSensitive, ignoreDiacritics: n = l.ignoreDiacritics, includeMatches: r = l.includeMatches, minMatchCharLength: i = l.minMatchCharLength, ignoreLocation: u = l.ignoreLocation, findAllMatches: c = l.findAllMatches, location: o = l.location, threshold: h = l.threshold, distance: a = l.distance } = {}) {
    this.query = null, this.options = { isCaseSensitive: s, ignoreDiacritics: n, includeMatches: r, minMatchCharLength: i, findAllMatches: c, ignoreLocation: u, location: o, threshold: h, distance: a }, t = s ? t : t.toLowerCase(), t = n ? k(t) : t, this.pattern = t, this.query = Pt(this.pattern, this.options);
  }
...
const N = { AND: "$and", OR: "$or" }, G = { PATH: "$path", PATTERN: "$val" }, H = (e) => !!(e[N.AND] || e[N.OR]), zt = (e) => !!e[G.PATH], Gt = (e) => !m(e) && et(e) && !H(e), q = (e) => ({ [N.AND]: Object.keys(e).map((t) => ({ [t]: e[t] })) });
function ot(e, t, { auto: s = true } = {}) {
  const n = (r) => {
    let i = Object.keys(r);
    const u = zt(r);
    if (!u && i.length > 1 && !H(r)) return n(q(r));
    if (Gt(r)) {
      const o = u ? r[G.PATH] : i[0], h = u ? r[G.PATTERN] : r[o];
      if (!M(h)) throw new Error(At(o));
      const a = { keyId: j(o), pattern: h };
...
function Ht(e, { ignoreFieldNorm: t = l.ignoreFieldNorm }) {
  e.forEach((s) => {
    ...
    s.matches.forEach(({ key: r, norm: i, score: u }) => {
      ...
      n *= Math.pow(u === 0 && c ? Number.EPSILON : u, (c || 1) * (t ? 1 : i));
    }), s.score = n;
  });
}
function Yt(e, t) {
  ...
  t.matches = [], E(s) && s.forEach((n) => {
    if (!E(n.indices) || !n.indices.length) return;
    const { indices: r, value: i } = n;
    let u = { indices: r, value: i };
...
function Qt(e, t, { includeMatches: s = l.includeMatches, includeScore: n = l.includeScore } = {}) {
  const r = [];
  return s && r.push(Yt), n && r.push(Vt), e.map((i) => {
    const { idx: u } = i, c = { item: t[u], refIndex: u };
...
class b {
  ...
  setCollection(t, s) {
    if (this._docs = t, s && !(s instanceof Y)) throw new Error(gt);
    this._myIndex = s || rt(this.options.keys, this._docs, { getFn: this.options.getFn, fieldNormWeight: this.options.fieldNormWeight });
  }
  add(t) {
  ...
  search(t, { limit: s = -1 } = {}) {
    const { includeMatches: n, includeScore: r, shouldSort: i, sortFn: u, ignoreFieldNorm: c } = this.options;
    let o = M(t) ? M(this._docs[0]) ? this._searchStringList(t) : this._searchObjectList(t) : this._searchLogical(t);
    return Ht(o, { ignoreFieldNorm: c }), i && o.sort(u), tt(s) && s > -1 && (o = o.slice(0, s)), Qt(o, this._docs, { includeMatches: n, includeScore: r });
  }
  _searchStringList(t) {
    ...
    return n.forEach(({ v: i, i: u, n: c }) => {
      ...
      o && r.push({ item: i, idx: u, matches: [{ score: h, value: i, norm: c, indices: a }] });
    }), r;
  }
  _searchLogical(t) {
    const s = ot(t, this.options), n = (c, o, h) => {
      if (!c.children) {
        const { keyId: f, searcher: d } = c, g = this._findMatches({ key: this._keyStore.get(f), value: this._myIndex.getValueForItemAtKeyId(o, f), searcher: d });
      ...
      for (let f = 0, d = c.children.length; f < d; f += 1) {
        ...
        else if (c.operator === N.AND) return [];
      }
      return a;
  ...
  _findMatches({ key: t, value: s, searcher: n }) {
    if (!E(s)) return [];
    let r = [];
    if (m(s)) s.forEach(({ v: i, i: u, n: c }) => {
      if (!E(i)) return;
      const { isMatch: o, score: h, indices: a } = n.searchIn(i);
      o && r.push({ score: h, key: t, value: i, idx: u, norm: c, indices: a });
    });
    else {
