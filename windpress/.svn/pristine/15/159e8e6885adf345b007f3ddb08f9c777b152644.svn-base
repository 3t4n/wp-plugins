...
class Compile
{
    ...
    public function get_contents($metadata) : array
    {
        ...
        $sql = $wpdb->prepare("SELECT COUNT(*) FROM %i WHERE enabled = %d", $table_name, 1);
        ...
        foreach ($results as $result) {
            $contents[] = ['id' => $result['id'], 'title' => \sprintf('#%s: %s', $result['id'], $result['title']), 'content' => $result['original_code']];
        }
        $total_batches = \ceil($total / $per_page);
        return ['metadata' => ['total_batches' => $total_batches, 'next_batch' => $total_batches > $next_batch ? $next_batch + 1 : \false], 'contents' => $contents];
    }
}
