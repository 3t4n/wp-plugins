...
class Volume
{
    public static function get_entries() : array
    {
        ...
        $finder = new Finder();
        $finder->ignoreUnreadableDirs()->in($data_dir)->files()->followLinks()->name(['*.css', '*.js']);
        \do_action('a!windpress/core/volume:get_entries.finder', $finder);
        foreach ($finder as $file) {
            if (!\is_readable($file->getPathname())) {
                continue;
            }
            $entries[] = ['name' => $file->getFilename(), 'relative_path' => $file->getRelativePathname(), 'content' => $file->getContents(), 'handler' => \strpos($file->getPathname(), $data_dir) === \false ? 'read-only' : 'internal', 'signature' => \wp_create_nonce(\sprintf('%s:%s', WIND_PRESS::WP_OPTION, $file->getRelativePathname()))];
        }
        ...
        if ($main_css_key === \false) {
            $entries[] = ['name' => 'main.css', 'relative_path' => 'main.css', 'content' => $stubs_main_css, 'handler' => 'internal', 'signature' => \wp_create_nonce(\sprintf('%s:%s', WIND_PRESS::WP_OPTION, 'main.css'))];
        ...
        if ($tailwindcss_version === 3) {
            ...
            $tailwind_config_js_key = \array_search('tailwind.config.js', \array_column($entries, 'name'), \true);
            if ($tailwind_config_js_key === \false) {
                $entries[] = ['name' => 'tailwind.config.js', 'relative_path' => 'tailwind.config.js', 'content' => $stubs_tailwind_config_js, 'handler' => 'internal', 'signature' => \wp_create_nonce(\sprintf('%s:%s', WIND_PRESS::WP_OPTION, 'tailwind.config.js'))];
            ...
            $wizard_js_key = \array_search('wizard.js', \array_column($entries, 'name'), \true);
            if ($wizard_js_key === \false) {
                $entries[] = ['name' => 'wizard.js', 'relative_path' => 'wizard.js', 'content' => $stubs_wizard_js, 'handler' => 'internal', 'signature' => \wp_create_nonce(\sprintf('%s:%s', WIND_PRESS::WP_OPTION, 'wizard.js'))];
    ...
    public static function save_entries($entries)
    {
        ...
        foreach ($entries as $entry) {
            ...
            if (!isset($entry['signature'])) {
                ...
                if (!\in_array(\pathinfo($entry['name'], \PATHINFO_EXTENSION), ['css', 'js'], \true)) {
                    continue;
                }
