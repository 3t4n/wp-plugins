<?php
	...
	if (isset($_POST['aajodatestimonials_opt_hidden']) && $_POST['aajodatestimonials_opt_hidden'] == 'Y' )  {
		...
		update_option('aajodatestimonials_code',wp_kses($code, array('style' => array())));
		...
		$code =wp_kses( get_option('aajodatestimonials_code'), array('style' => array()));
		//$code = esc_html(get_option('aajodatestimonials_code'));
...
<form  name="aajoda_testimonials_form" method="post" action="<?php echo admin_url('options-general.php').'?page='.$_GET['page']; ?>">
	...
	<div>
		...
		<select name='aajoda_version'>
			<option value='2.2' <?php selected( $version, "2.2" ); ?>>2.2</option>
	...
	<div class="metabox-holder">
		<div class="postbox"> 
			...
			<div style="padding:0 10px 10px 10px;" >
				...
				<textarea name="aajodatestimonials_code" rows="8" cols="90" /><?php echo esc_textarea($code."hello"); ?></textarea>
			</div>
		</div>
	</div>
