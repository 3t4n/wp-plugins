<?php
    ...
    class Pargo_Public
    {
        ...
        /**
         * Add the suburb field to the checkout page
         */
        public function pargo_register_suburb_field()
        {
            if (function_exists('woocommerce_register_additional_checkout_field')) {
                woocommerce_register_additional_checkout_field(
                    array(
                        'id' => $this->plugin_name . '/suburb',
                        'label' => 'Suburb',
                        'location' => 'address',
                        'required' => true,
                        'attributes' => array(
        ...
        /**
         * Sanitize the suburb field
         */
        public function pargo_sanitize_suburb_field($field_value, $field_key)
        {
            if ($this->plugin_name.'/suburb' === $field_key) {
                // strip off any funny characters and trim whitespace
                $field_value = preg_replace('/[^a-zA-Z0-9\s]/', '', $field_value);
         ...
         */
        public function pargo_validate_suburb_field(WP_Error $errors, $field_value, $field_key)
        {
            if ($this->plugin_name.'/suburb' === $field_key) {
                if (empty($field_value)) {
                    $errors->add('suburb_empty', 'Please enter a suburb');
                }
            }
            return $errors;
        }
        ...
        /**
         * function to check if the block checkout is being used in the the checkout page
         */
        ...
        public function pargo_checkout_fields($fields)
        {
            ...
            // if using block checkout this should not be used
	    ...
	    public function pargo_account_suburb_address_field($fields, $customer_id, $type) {
            ...
            if ($type === "billing") {
                $billingSuburb = get_user_meta($customer_id, 'billing_suburb', true);
                $fields['suburb'] = $billingSuburb ?? "";
            }
            if ($type === "shipping") {
                $shippingSuburb = get_user_meta($customer_id, 'shipping_suburb', true);
                $fields['suburb'] = $shippingSuburb ?? "";
            }
		    return $fields;
        }
        ...
        public function pargo_order_suburb_billing_address_field($fields, $order) {
            ...
            $billingSuburb = get_post_meta($order->get_id(), '_billing_suburb', true);
            if ($billingSuburb) {
                $fields['suburb'] = $billingSuburb;
            }
            return $fields;
        ...
        public function pargo_order_suburb_shipping_address_field($fields, $order) {
            $shippingSuburb = get_post_meta($order->get_id(), '_shipping_suburb', true);
            if ($shippingSuburb) {
                $fields['suburb'] = $shippingSuburb;
            }
            return $fields;
        ...
        public function validate_pargo_fields()
        {
            ...
            if (empty($_POST['billing_suburb']) && !$is_block_checkout) {
                wc_add_notice('Please add a billing suburb.', 'error');
            }
            if (isset($_POST['ship_to_different_address']) && !$is_block_checkout) {
        ...
        public function pargo_hide_shipping_based_on_order_weight($rates, $package)
        {
	        ...
	        switch(strtolower($weight_unit)) {
		        ...
		        case 'g':
			        ...
			        break;
		        case 'lbs':
			        $weight_multiplier = 0.453592;
			        break;
		        case 'oz':
			        $weight_multiplier = 0.0283495;
			        break;
		        default:
			        $weight_multiplier = 1;
	        }
        ...
        /**
         * Checks to see if the checkout page is using block or classic checkout and sends analytics event
         */
        public function event_checkout_page_loaded(  )
        {
            if (is_checkout()) {
                if ( $this->is_block_checkout() ) {
                    Analytics::submit( 'customer', 'view', 'checkout_block' );
                } else {
                    Analytics::submit( 'customer', 'view', 'checkout_classic' );
                }
            }
        }
    }
