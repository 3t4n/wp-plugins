...
class Pargo_Admin {
	...
	public function enqueue_scripts() {
        ...
        wp_enqueue_script( $this->plugin_name.'-utils', PARGO_PLUGIN_PATH . 'src/PargoAdmin/js/pargo-admin.js', ['jquery'], $this->version, true );
        wp_localize_script(
...
//        Analytics::submit('client_admin', 'click', 'add_pargo_shipping_method');
	...
	/**
	 * Add an action to the order to create a Pargo order
	 * Only show the action if the order is processing and has a Pargo shipping method
	 * and there is no pargo_waybill
	 *
	 * @return array
	 */
	public function add_pargo_order_action($actions) {
		global $theorder;
		if (!get_post_meta($theorder->get_id(), 'pargo_waybill', true)
		    && $theorder->get_status() === 'processing'
		    && ($theorder->has_shipping_method('wp_pargo_home') || $theorder->has_shipping_method('wp_pargo'))) {
			$actions['pargo_submit_pargo_order'] = 'Submit Order to Pargo';
		}
		return $actions;
	}

	/**
	 * Process the Pargo order action
	 *
	 * @return void
	 */
	public function process_pargo_order_action($order) {
    ...
    /**
     * Admin notice to leave a rating on the latest major version of the plugin, must also not show again if dismissed until next major update
     *
     */
    public function leave_rating_notice()
    {
        // if the page is the dashboard
        if (is_admin() && get_current_screen()->base === 'dashboard') {
            // get the version number of the plugin
            $current_version = PARGO_VERSION;
            // get the version number that was last dismissed
            $dismissed_version = get_option('pargo_dismissed_rating_notice_version', "0.0.0");

            // get minor version number
            $current_minor_version = explode('.', $current_version)[1];
            $dismissed_minor_version = explode('.', $dismissed_version)[1];

            // if the version number is different from the last dismissed version number
            if ($current_minor_version !== $dismissed_minor_version) {
                // display the notice
                ?>
                <div class="notice notice-pargo" id="pargo_rating_notice">
                    <button type="button" class="notice-dismiss" id="pargo_rating_notice_dismiss">Dismiss</button>
                    <h3><?php _e("We’d Love Your Feedback!", 'pargo'); ?></h3>
                    <p><?php _e('Thank you for using Pargo Shipping plugin. If you’re enjoying the latest version, please take a moment to leave a rating.', 'pargo'); ?></p>
                    <p><?php _e('Your feedback will help us enhance the plugin and provide an even better experience.', 'pargo'); ?></p>
                    <p><a href="https://wordpress.org/support/plugin/pargo/reviews/#new-post" target="_blank" class="button-pargo"><?php _e('Leave a rating', 'pargo'); ?></a></p>
                </div>
                <?php
            }
        }
    ...
    /**
     * Log that the rating notice has been dismissed in the options table
     */
    public function dismiss_rating_notice()
    {
        ...
        // get the version number of the plugin
        $current_version = PARGO_VERSION;
        // save the version number as the last dismissed version number
        update_option('pargo_dismissed_rating_notice_version', $current_version);
