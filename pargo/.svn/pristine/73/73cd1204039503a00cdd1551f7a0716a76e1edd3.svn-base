...
class Pargo_Orders extends Pargo_Admin_API
{
	 ...
	 * @return float
	 */
	public function calculate_product_weight($product, $quantity, string $weight_unit = ""): float {
		...
		$weight = $product->get_weight() ? floatval($product->get_weight()) : 0.01;
		return floatval($weight * intval($quantity) / $weight_multiplier);
	}

	/**
	 * Function to get the weight of the order
	...
	public function get_order_weight(WC_Order $order) {
		...
		foreach ($items as $item) {
			...
			$weight += $this->calculate_product_weight($product, $item->get_quantity());
		}
    ...
    final public function placeOrder(WC_Order $order, string $processType)
    {
        ...
        if ($chosen_shipping === 'wp_pargo') {
            // if no pickup point is set then post the order without the pickup point
            $pup_code = $order->get_meta('pargo_pc');
            if (empty($pup_code)) {
				// set the pargo_pc to null
                $order->update_meta_data('pargo_pc', null);
                $note = __("Note: order has no Pargo Pickup Point set");
    ...
    public function get_suburb(WC_Order $order, $type="shipping") {
        $meta_keys_shipping = [
            '_shipping_suburb',
            '_wc_shipping/pargo/suburb'
        ];
        $meta_keys_billing = [
            '_billing_suburb',
            '_wc_billing/pargo/suburb'
        ];
        if ($type === 'shipping') {
            $meta_keys = $meta_keys_shipping;
        } else {
            $meta_keys = $meta_keys_billing;
        }
        foreach ($meta_keys as $meta_key) {
            $suburb = $order->get_meta($meta_key);
            if (!empty($suburb)) {
                return $suburb;
            }
        }
        return $order->get_data()[$type]['city'];
    }
    ...
    private function getW2pData(WC_Order $order): string
    {
		...
		$suburb = $this->get_suburb($order, 'billing');
        $data = [
            ...
            "data" => [
                ...
                "attributes" => [
	                ...
	                "cubicWeight" => $this->get_order_weight($order),
                ],
            ],
        ];
