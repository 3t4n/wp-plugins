...
class Pargo {
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_filter( 'woocommerce_order_actions', $plugin_admin, 'add_pargo_order_action' );
		// Actions
		...
		$this->loader->add_action( 'woocommerce_order_action_pargo_submit_pargo_order', $plugin_admin, 'process_pargo_order_action');
        $this->loader->add_action( 'admin_notices', $plugin_admin, 'leave_rating_notice' );
        $this->loader->add_action( 'wp_ajax_pargo_rating_notice_dismiss', $plugin_admin, 'dismiss_rating_notice' );
	}
	...
	private function define_public_hooks() {
        ...
        $this->loader->add_action( 'wp', $plugin_public, 'event_checkout_page_loaded' );

        $this->loader->add_action ('woocommerce_init', $plugin_public, 'pargo_register_suburb_field' );
        $this->loader->add_action ('woocommerce_sanitize_additional_field', $plugin_public, 'pargo_sanitize_suburb_field', 10, 2 );
        $this->loader->add_action ( 'woocommerce_validate_suburb_field', $plugin_public, 'pargo_validate_suburb_field', 10, 3 );
