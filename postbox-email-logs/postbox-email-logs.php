...
function pbeml_get_log_data() {
    ...
    if( ! ( current_user_can('pbeml_access') || current_user_can( 'manage_options' ) ) ){
        die;
    }
    ...
    if (!wp_verify_nonce($nonce, 'pbeml_get_log_data')) {
        echo wp_json_encode(['redirect' => true]);
        die;
    }
    ...
    echo apply_filters( 'pbeml_popup_html',  $popup_html); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	die();
}
...
function pbeml_plugin_activate() {
  ...
  update_option( 'pbeml_postbox_install_date', gmdate('Y-m-d G:i:s') );
...
function pbeml_admin_notice() {
	...
	$install_date = empty( $install_date ) ? gmdate('Y-m-d G:i:s') : '';
    ...
    if ( empty( $rated ) && isset($_GET['page']) && $_GET['page'] == 'postbox-email-logs.php' ) {
        ...
        // Translators: %1$s is the URL to dismiss the notice, %2$s is the URL to the WordPress plugin page.
        printf(__('Awesome, you\'ve been using <a href="admin.php?page=postbox-email-logs.php">PostBox Email Log</a> for more than 1 week. May we ask you to give it a 5-star rating on WordPress? | <a href="%2$s" target="_blank">Ok, you deserved it</a> | <a href="%1$s">I alredy did</a> | <a href="%1$s">No, not good enough</a>',// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            'postbox-email-logs'), 	
