    ...
    */
    public function prepare_items()
    {
        ...
        if( !empty($_GET['from-date'])){
            ...
            $to_date   = empty($_GET['to-date']) ? gmdate('d/m/Y') : sanitize_text_field( $_GET['to-date'] );
    ...
    public function get_bulk_actions() {
        ...
        return array(
            'read'   => __( 'Read', 'postbox-email-logs' ),
            'unread' => __( 'Unread', 'postbox-email-logs' ),
    ...
    private function table_data()
    {
        ...
        $query        = '';
        $nonce        = wp_create_nonce('pbeml_get_log_data');
        ...
        foreach ( $results as $result ) {
            ...
            foreach ( $this->column_titles as $col_title) {
                ...
                $row[ $col_title ] = sprintf('<a href="" data-nonce="'.$nonce.'" data-log="'.$row['id'].'" class="pbeml-row">%s</a>', $row[ $col_title ]);
    ...
    public function search_box( $text, $input_id ) {
        ...
        <p class="search-box">
            ...
            <input type="search" placeholder="<?php esc_attr_e('From Date', 'postbox-email-logs') ?>"  name="from-date" class="pbeml-date" value="<?php echo esc_attr( $from_date ) ?>" autocomplete="off" />
            <input type="search" placeholder="<?php esc_attr_e('To Date', 'postbox-email-logs') ?>"  name="to-date" class="pbeml-date" value="<?php echo esc_attr( $to_date ) ?>"  autocomplete="off"/>
            <input type="search" placeholder="<?php esc_attr_e('Search by Term', 'postbox-email-logs') ?>" id="<?php echo esc_attr( $input_id ); ?>" name="s" value="<?php _admin_search_query(); ?>" />
    ...
    public function process_bulk_action(){
        ...
        if( 'delete' === $action ) {
            ...
            endforeach;
            echo "<script type='text/javascript'>window.top.location='admin.php?page=postbox-email-logs.php&paged=$pagenum';</script>";// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped  
        }else if( 'delete_all' === $action ){
            $db->query("TRUNCATE TABLE $table_name");
            echo "<script type='text/javascript'>window.top.location='admin.php?page=postbox-email-logs.php';</script>";// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped  
            exit;
        }else if( 'read' === $action ){
            foreach ((array)$ids as $id):
                $id = (int) $id;
    ...
    protected function bulk_actions( $which = '' ) {
        ...
        echo '<label for="bulk-action-selector-' . esc_attr( $which ) . '" class="screen-reader-text">' . __( 'Select bulk action', 'postbox-email-logs' ) . '</label>';// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
