...
function gamipress_ld_points_per_quiz_score_quiz_completed( $quiz_data, $current_user ) {
    ...
    if( ! $quiz_data['quiz'] instanceof WP_Post ) {
        $quiz_data['quiz'] = get_post( $quiz_data['quiz'] );
    }
    ...
    foreach( gamipress_get_points_types() as $points_type => $data ) {
         ...
         * @return int
         */
        $points_to_award = (int) apply_filters( 'gamipress_ld_points_per_quiz_score_points_to_award', $points_to_award, $user_id, $points_type, $order_id, $percent, $quiz_id );
...
add_action( $quiz_submitted_hook, 'gamipress_ld_points_per_quiz_score_quiz_completed', 10, 2 );