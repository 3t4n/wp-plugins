...
class AttireBlocksSettings {
	...
	function verify_license_key() {
		if ( ! current_user_can( 'manage_options' ) || ! wp_verify_nonce( $_REQUEST['__nonce_atbs_verify_license'], NONCE_KEY ) ) {
			echo json_encode( array(
				'success' => false,
				'msg'     => __( 'Authorization failed!', 'attire-blocks' )
			), JSON_FORCE_OBJECT );
			wp_die();
		}
	...
	function disable_fe_assets() {
		if ( ! current_user_can( 'manage_options' ) || ! wp_verify_nonce( $_REQUEST['__nonce_atbs_disable_fe_assets'], NONCE_KEY ) ) {
			echo json_encode( array(
				'success' => false,
				'msg'     => __( 'Authorization failed!', 'attire-blocks' )
			), JSON_FORCE_OBJECT );
		...
		echo json_encode( array(
			'success' => true,
			'msg'     => __( 'Settings updated successfully.', 'attire-blocks' )
		), JSON_FORCE_OBJECT );
		wp_die(); // this is required to terminate immediately and return a proper response
	}
