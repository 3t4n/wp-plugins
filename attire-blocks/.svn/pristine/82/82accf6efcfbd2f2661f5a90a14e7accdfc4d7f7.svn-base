        ...
        jQuery(function ($) {
            ...
            $('body').on('click', '#reset-attire', function (e) {
                ...
                $.post(ajaxurl, {
                    ...
                    __reset_attire_nonce: '<?php echo wp_create_nonce(NONCE_KEY); ?>'
                }, function (res) {
...
add_action("wp_ajax_reset_attire_options", function () {
    if (wp_verify_nonce($_REQUEST['__reset_attire_nonce'], NONCE_KEY) && current_user_can('manage_options')) {
    ...
    class Attire_Color_Choice_panel extends \WP_Customize_Control
    {
        ...
        public function atbs_build_field_html($key, $setting)
        {
            ...
            echo '<td  style="width: 33.33%">
                        <input style="height: 30px;border: 0;padding: 0;margin-top: 5px;margin-right: 5px" type="color" value="' . esc_attr($value) . '" ' . $this->get_link($key) . ' />
                      </td>';
        }
        ...
        public function render_content()
        {
            ...
            <div class="customize-control">
                ...
                if (is_array($this->description)) {
                    echo '<p>' . implode('</p><p>', esc_html($this->description)) . '</p>';
                } else {
...
function atbs_customize_css()
{
	...
	$uicolors=\Attire\Blocks\__::sanitize_array($uicolors);
