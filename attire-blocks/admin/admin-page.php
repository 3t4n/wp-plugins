                    ...
                    <div class="form-group">
                        <div class="input-group mb-3">
                            ...
                            <input name="atbs_license_key" id="atbs_license_key" type="text" class="form-control"
                                   ...
                                   value="<?php esc_attr_e( $license ? $license : '' ) ?>"
...
<script>
    jQuery(function ($) {
        $("#atbs-admin-license").submit(function (event) {
            ...
            $.post(ajaxurl, {
                action: 'atbs_verify_license',
                __nonce_atbs_verify_license: '<?php echo wp_create_nonce( NONCE_KEY );?>',
                key: $('#atbs_license_key').val()
            }, function (res) {
                res = JSON.parse(res);
        ...
        $("#atbs-admin-fe-assets").submit(function (event) {
            ...
            $.post(ajaxurl, {
                action: 'atbs_disable_fe_assets',
                __nonce_atbs_disable_fe_assets: '<?php echo wp_create_nonce( NONCE_KEY );?>',
                data
            }, function (res) {
                res = JSON.parse(res);
                if (res.success !== true) {
