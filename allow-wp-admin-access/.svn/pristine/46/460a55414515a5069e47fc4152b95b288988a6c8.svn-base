 ...
 * Text Domain:       allow-wp-admin-access
 */
...
function awpaa_admin_settings( $actions, $plugin_file ) {
	...
	if ( $plugin === $plugin_file ) {
		$settings = array();
		$settings['settings']         = '<a href="' . esc_url( admin_url( 'admin.php?page=awa-wp-admin-option-page' ) ) . '">' . esc_html__( 'Settings', 'allow-wp-admin-access' ) . '</a>';
