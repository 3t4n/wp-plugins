...
class Admin_Menu {
	public static $handle = 'ols-dashboard-admin-settings';
	/**
	 * Initiate class.
	 *
	 * @return void
	...
	public static function init() {
		...
		add_action( 'admin_menu', array( __CLASS__, 'main_admin_menu' ) );
		add_action( 'admin_menu', array( __CLASS__, 'modify_admin_menu' ), 12 );
	}
	...
	public static function register_assets() {
		$asset_file = OLS_DASHBOARD_BASE_DIR . 'build/non-blocks/admin/settings/index.asset.php';
		...
		$deps[] = 'ols-dashboard-admin-ui';
		$ver               = isset( $asset_file_values['version'] ) ? $asset_file_values['version'] : false;

		$src = sprintf( '%sbuild/non-blocks/admin/settings/index.js', OLS_DASHBOARD_BASE_URL );
		wp_register_script( self::$handle, $src, $deps, $ver, true );

		$src = sprintf( '%sbuild/non-blocks/admin/settings/index.css', OLS_DASHBOARD_BASE_URL );
		wp_register_style( self::$handle, $src, array( 'ols-dashboard-admin-ui' ), $ver );
	}
	 ...
	 * @return void
	 */
	public static function main_admin_menu() {
		add_menu_page(
			// 'themes.php',
			__( 'Advanced Login Page Customizer', 'advanced-login-page-customizer' ), // Page title
			__( 'One Loop Studio', 'advanced-login-page-customizer' ), // Menu title
			'manage_options',
			'ols-settings',
			array( __CLASS__, 'login_ui_builder_page_cb' ), // Callback function
			'data:image/svg+xml;base64,' . base64_encode( '<svg height="800px" width="800px" version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
...
<g>
	...
	public static function modify_admin_menu() {
		...
		unset( $submenu['ols-settings'][0] ); // [TODO: Temporary remove as we dont have dashboard.]
	...
	public static function login_ui_builder_page_cb() {
		echo "<div id='ols-app'>OLS App Loading...</div>";
