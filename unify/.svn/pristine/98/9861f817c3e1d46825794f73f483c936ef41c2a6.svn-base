...
add_action('admin_menu', function() {
    // Check if the pro license is present
    $pro_license = \CodeClouds\Unify\Service\Helper::getProLicenseFromUnify();
    // If there is a pro license, add the 'remove_free_menu' action
    if (!empty($pro_license)) {
        // Add the 'remove_free_menu' method to the admin_menu hook
        add_action('admin_menu', ['CodeClouds\Unify\Actions\PlatformApi', 'remove_free_menu'], 999);
    }
});
...
add_action( 'init', function(){
	...
	if(isset($_GET['unify_plugin_downgrade'])) :
        ...
        \CodeClouds\Unify\Service\Helper::dropUnifyOptionsDataTable();
    endif;
