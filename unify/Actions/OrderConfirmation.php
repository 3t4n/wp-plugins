...
/* Show Decline Message If order Canceled from Paypal Window*/
    public static function wnd_checkout_code()
    {
        ...
        // Check if $wc_codeclouds_unify_settings is not false and is an array
        if ($wc_codeclouds_unify_settings && is_array($wc_codeclouds_unify_settings)) {
            $connection = isset($wc_codeclouds_unify_settings['connection']) ? get_post_meta($wc_codeclouds_unify_settings['connection']) : false;

            // Check if $connection is not false and is an array
            if ($connection && is_array($connection)) {
                $crm_conection_name = ConfigEncryption::metaDecryptSingle(
                    isset($connection['unify_connection_crm'][0]) ? $connection['unify_connection_crm'][0] : '',
                    isset($connection['unify_connection_crm_salt'][0]) ? $connection['unify_connection_crm_salt'][0] : ''
                );
