...
class Dashboard
{
    ...
    public static function dashboard_page()
    {
        ...
        if(!empty(wc()->api)){
            ...
            $report = [];
            self::$environment = [];
            self::$environment['version'] = WC()->version;
            $memory_limit = ini_get('memory_limit');

            // Convert the memory limit to bytes if it's in a human-readable format
            if ( false !== strpos( $memory_limit, 'M' ) ) {
                $memory_limit = intval( $memory_limit ) * 1024 * 1024;
            } elseif ( false !== strpos( $memory_limit, 'G' ) ) {
                $memory_limit = intval( $memory_limit ) * 1024 * 1024 * 1024;
            }
            
            $uploads_dir = wp_upload_dir();
            if(!empty($uploads_dir['basedir'])){
                $wc_logs_path = trailingslashit( $uploads_dir['basedir'] ) . 'wc-logs/';
            }
        }
        ...
        $current_configuration = [
            (class_exists('WooCommerce') && !empty(self::$environment['server_info']))  ? self::$environment['server_info'] : (!empty($_SERVER['SERVER_SOFTWARE']) ? $_SERVER['SERVER_SOFTWARE'] : apache_get_version()),
            (class_exists('WooCommerce') && !empty(self::$environment['php_version'])) ? self::$environment['php_version'] : phpversion(),
            (class_exists('WooCommerce') && !empty(self::$environment['php_max_execution_time'])) ? self::$environment['php_max_execution_time'] : ini_get('max_execution_time'),
            (class_exists('WooCommerce') && !empty(self::$environment['wp_version'])) ? self::$environment['wp_version'] : get_bloginfo('version'),
            (class_exists('WooCommerce') && !empty(self::$environment['version'])) ? self::$environment['version'] : (!empty(WC()->version) ? WC()->version : 0),
            (class_exists('WooCommerce') && !empty(self::$environment['curl_version'])) ? self::$environment['curl_version'] : curl_version()['version'],
            (class_exists('WooCommerce') && !empty(self::$environment['mysql_version'])) ? self::$environment['mysql_version'] : $mysqlVersion,
            (class_exists('WooCommerce') && !empty(self::$environment['max_upload_size'])) ? self::$environment['max_upload_size'] : wp_max_upload_size(),
            (class_exists('WooCommerce') && !empty(self::$environment['wp_memory_limit'])) ? self::$environment['wp_memory_limit'] : (!empty($memory_limit) ? $memory_limit : 'Not Available'),
            (class_exists('WooCommerce') && !empty(self::$environment['wp_cron'])) ? self::$environment['wp_cron'] : 'Not Available',
        (class_exists('WooCommerce') && !empty(self::$environment['log_directory'])) ? self::$environment['log_directory'] : (!empty($wc_logs_path) ? $wc_logs_path : 'Not Available'),
            (class_exists('WooCommerce') && !empty(self::$environment['log_directory'])) ? self::$environment['log_directory'] : (!empty($wc_logs_path) ? $wc_logs_path : 'Not Available'),
        ];
