...
use CodeClouds\Unify\Models\ProLicense;
/**
...
class Helper
{
    ...
    {
        $unify_plugin_pro = Helper::getProLicenseFromUnify();
        $showmsg = 0;
    ...
    public static function getProLicenseFromUnify($licenseData = [])
    {
        $proLicenseFromOptionTable = [];
        $option_key = 'codeclouds_unify_pro_license';
        $proLicense = new ProLicense;

        // Table does not exist, so we will create it
        $proLicense->createTable();
        // Table exists, fetch data
        $fetchLicenseData = $proLicense->fetchData($option_key);
        if (!empty($fetchLicenseData->option_value)) {
            // Option exists, handle the option value
            $proLicenseFromOptionTable =  $fetchLicenseData->option_value;
        } else{
            !empty($licenseData) && $proLicenseFromOptionTable = $licenseData;

            if(empty($proLicenseFromOptionTable)){
                // Fetch license from wp_options table
                $proLicenseFromOptionTable = \get_option('codeclouds_unify_pro_license');
            }

            if(!empty($proLicenseFromOptionTable)){
                // Option does not exist but row exist
                if(!empty($fetchLicenseData->id)){
                    $proLicense->update($fetchLicenseData->id,$option_key,$proLicenseFromOptionTable);
                }
                else {
                    // Insert unify pro license data into the table
                    $proLicense->saveData($option_key,$proLicenseFromOptionTable);
                }
            }
        }
        !empty($proLicenseFromOptionTable) && is_string($proLicenseFromOptionTable) && $proLicenseFromOptionTable = unserialize($proLicenseFromOptionTable);
        return $proLicenseFromOptionTable;
    }

    public static function dropUnifyOptionsDataTable(){
        $option_key = 'codeclouds_unify_pro_license';
        // Table exists, truncate it
        $proLicense = new ProLicense;
        $deleted = $proLicense->deleteAll($option_key);
        if ($deleted) {
            return;
        } else {
            return json_encode(['status' => 0, 'msg' => "Error deleting table"]);
        }  
    }
}
