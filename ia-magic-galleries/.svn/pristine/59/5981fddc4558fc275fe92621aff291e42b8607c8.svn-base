...
(function () {
    ...
    if (debug)  console.log('In iamg-block', iap_loader_settings);
    ...
    wp.blocks.registerBlockType('ia-mg/gallery', {
        ...
        attributes: {
            ...
            pres_id: {type: 'string'}, //stores the local id of the presentation. This is different from the post id of the iamg post of the presentation
            ...
            width_percent: {type: 'boolean', default: true},
            properties: {type: 'string', default: ""},
            background_color: {type: 'string', default: "#fff"},
            background_opacity: {type: 'string', default: "0"}
        },
        usesContext: ['postId'],
        edit: edit,
        save: save,
    ...
    function edit(props) {
        ...
        if (debug)  console.log('In Edit', props.clientId,
        ...
        //The ID of the page
        page_id = (props.context) ? props.context.postId : null;

        // page_id = props.context.postId;
        ...
        setTimeout(
            () => {
                if (window.IA_Presenter_loader) {
                    ...
                    if (debug)  console.log("Presenter Loader Library not loaded");
                }
        ...
        if (height && props.attributes.height_percent) {
            height = height + "vh";
        }
        ...
        let suffix = (props.attributes.width_percent) ? "vw" : "px";

        let style = {height: height};
        if (width) {
            style['width'] = width + suffix;
            style["margin-left"] = "calc(50% - " + (width / 2) + suffix + ")";
        }
        const html = el('div',
            {
        ...
        // if (debug)  console.log("In Edit", html);
    ...
    function save(props) {
        ...
        if (props.attributes.pres_id) {
            ...
            //     bh.width = props.attributes.width;
            //     bh.width_type = (props.attributes.width_percent) ? "percent" : "pixel";
            // }
            let behavior = process_behavior_object(bh);
            let text = '[ia_magic_gallery id="' + props.attributes.pres_id + '"' + behavior + ']';
            let element = el('div', {}, text);

            if (debug)  console.log('In Save Fun', props.clientId, props, behavior, text);
    ...
    function load_iamg_block_events(attributes) {
        ...
        eve_ia.on(['global', 'iamg', 'gallery', 'save'], function (locator, new_gal_post_id) {
            const gui = this;
            const block_id = gui.getBlockId();
            const props = glob_props[block_id];

            let gal_post_id = new_gal_post_id || props.attributes.pres_id;


            if (debug)  console.log('Saving', block_id, gal_post_id, locator, glob_props, props);

            const command = {
                command: 'save',
                block_id: block_id,
                pres_id: gal_post_id,
                page_id: page_id, //use to be post_id
                locator: locator,
            };

            if (gal_post_id !== props.attributes.pres_id) {
                command['old_pres_id'] = props.attributes.pres_id;
            }
            ...
            let process_responce = function (response) {
                ...
                if (!response.success) {
                    if (debug)  console.log('Error', response.status, response.error);

                    gui.eve("gui.alert", __("Gallery was not saved successfully") + ".", [220, 70], [__("OK")], [], margin)
                    gui.eve("gui.error", response.status + " " + response.error)
                ...
                if (debug)  console.log('Properties', server_settings)
                props.setAttributes({
                ...
                if (new_gal_post_id) {
                    props.setAttributes({pres_id: new_gal_post_id});
                }
                if (debug)  console.log("Saved the post on server", props);
                gui.eve("gui.alert", __("Gallery is saved successfully") + ".", [200, 70], [__("OK")], undefined, margin)
                gui.eve("iamg.gallery_saved", gal_post_id, title);
                wp.data.dispatch('core/editor').savePost()
            };
            gui.comManager.wpCommand(command,
                process_responce,
                function (response) {
                    if (debug)  console.log('Fail', response);
                },
            );
        ...
        eve_ia.on(['global', 'iamg', 'gallery', 'removed'], function (locator) {
            ...
            if (debug)  console.log('Removing', block_id, presId);
            ...
            const command = {
                ...
                block_id: block_id,
                pres_id: presId,
                page_id: page_id, //used to be post_id
                locator: locator,
            };
        ...
        eve_ia.on(['ia', 'gui', 'created'], function (id) {
            const gui = this;
            // if (debug)  console.log('Creating gui..............', id);

            if (Snap_ia.window().MutationObserver) {
                const excluded_classes = ['is-hovered']
                let observer = new MutationObserver(function (mutationList) {
                    if (mutationList[0].attributeName === 'class') {
                        //get the class difference by finding the exact class that is different
                        let old_class = mutationList[0].oldValue.split(' ');
                        let new_class = Array.from(mutationList[0].target.classList);

                        let removed_classes = old_class.filter(x => !new_class.includes(x) && excluded_classes.includes(x));
                        let added_classes = new_class.filter(x => !old_class.includes(x) && excluded_classes.includes(x));

                        if (removed_classes.length || added_classes.length) return;
                    }
                    if (debug)  console.log("Observed Div resize")
                ...
                block_container && observer.observe(block_container, {
                    attributes: true, attributeOldValue: true, attributeFilter: ['data-align', 'class'],
                });
            }

            if (Snap_ia.window().ResizeObserver) {
                let observer = new ResizeObserver(function (mutationList) {
                    if (debug)  console.log("Observed skeleton resize")
                    gui.eve('gui.panel.resize');
                },);
            ...
            // if (debug)  console.log("prev_properties", prev_properties);
    ...
    function match_images_iamg(image_info, original_requested_images) {
        const images = []
    ...
    window.match_images_iamg = window.match_images_iamg || match_images_iamg;
    ...
    function set_current_gallery_state(gui) {
        ...
        if (debug)  console.log("Presenter loaded");
        if (debug)  console.log("Setting previous properties: ", properties)
        ...
        const tile = attributes.title;
        const post_id = attributes.pres_id;
        ...
        images = match_images_iamg(image_info, original_requested_images);

        gui.eve(["iamg", "load_gallery_setup"], undefined, gallery_type, images, panel_state, resource, post_id, tile);
    }
    ...
    function set_behavior(is_full, props, use_saved) {
        if (is_full && is_full_allowed()) {
            ...
            last_fixed_settings["width"] = props.attributes.width;
            last_fixed_settings["width_percent"] = props.attributes.width_percent;
            props.setAttributes({
                behavior: "full",
                height: 100,
                height_percent: true,
        ...
        if (!is_full) {
            if (use_saved && props.attributes.behavior === "full" && last_fixed_settings.hasOwnProperty("height")) {
                props.setAttributes({
                    ...
                    width: last_fixed_settings.width,
                    width_percent: last_fixed_settings.width_percent,
                    behavior: "fixed"
                })
            } else {
        ...
        // if (debug)  console.log("Atributes", props.attributes)
    }
        ...
        let color_panel = el(
            wp.blockEditor.PanelColorSettings,
            {
                ...
                colorSettings: [
                    ...
                    //     value: textColor,
                    //     onChange: (colorValue) => {
                    //         if (debug)  console.log(colorValue)
    ...
    function setting_bar_controller(props) {
        ...
        // if (debug)  console.log(("In Setting Bar"), gui);

        const is_full = props.attributes.behavior !== "fixed"

        const is_percent_h = props.attributes.height_percent;
        let percent_or_px_button_h = (is_percent_h) ?
            el(wp.components.Button,
                {
                    ...
                    onClick: () => {
                        ...
                        let height = +props.attributes.height;
                        const to_pixel = height * (window.innerHeight - admin_bar_height) / 100;
                        props.setAttributes({height: String(to_pixel)});
            ...
            el(wp.components.Button,
                {
                    ...
                    onClick: () => {
                        ...
                        let height = +props.attributes.height;
                        const to_percent = 100 * height / (window.innerHeight - admin_bar_height);
                        props.setAttributes({height: String(to_percent)});
        ...
        let panel_height = (is_full) ? undefined
            : el(wp.components.PanelBody,
                ...
                el(wp.components.PanelRow, {},
                    ...
                    el("div", {class: "IAMG_Layout_FlexRow"},
                        el(wp.components.TextControl,
                            {
                                // label: 'Height',
                                type: 'number',
                                onChange: (value) => {
                                    props.setAttributes({height: String(value)})
                                },
                                value: props.attributes.height || "",
                                class: "IAMG_Layout_ButtHeight"
                            }
                        ),
                        // el("div", {},
                        percent_or_px_button_h,
                        //     el("text", {},
                        //         (is_percent) ?
                        //             " Percent of window" :
                        //             " Pixels of Height")
        ...
        const is_percent_w = props.attributes.width_percent;
        let percent_or_px_button_w = (is_percent_w) ?
            el(wp.components.Button,
                {
                    isDefault: true,
                    onClick: () => {
                        props.setAttributes({width_percent: false});

                        const to_pixel = +props.attributes.width * window.innerWidth / 100;
                        props.setAttributes({width: String(to_pixel)});
                    }
                },
                "%")
            :
            el(wp.components.Button,
                {
                    isDefault: true,
                    onClick: () => {
                        props.setAttributes({width_percent: true});

                        const to_percent = 100 * +props.attributes.width / window.innerWidth;
                        props.setAttributes({width: String(to_percent)});
                    },
                    marginTop: "auto"
                },
                "Px");

        let panel_width = el(wp.components.PanelBody,
            {
                title: __('Width'), initialOpen: false,
                className: 'IAMG_Width_Panel_' + block_id,
            },
            el(wp.components.PanelRow, {},
                el("text", {},
                    "It is better to set width with block alignment for a standalone block! "),
            ),
            el(wp.components.PanelRow, {},
                el("text", {},
                    "Width must be set in the context of Row or Columns!"),
            ),
            el(wp.components.PanelRow, {},
                el("div", {class: "IAMG_Layout_FlexRow"},
                    el(wp.components.TextControl,
