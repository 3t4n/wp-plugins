...
var IA_Presenter_loader = (function (root) {
    ...
    var post_process_gui = function (gui) {
        if (debug)  console.log("Loaded Presentation");
        // gui.fitWindow();
    ...
    function loadPreScripts(settings, relative_script_path, pres_gen) {
        ...
        if (debug)  console.log('Loading IA Presenter');
        ...
        post_scripts.forEach(function (url) {
            ...
            if (url) {
                ...
                if (!loaded_postscrpts[url]) {
                    if (debug)  console.log('Loading post ', url);
                    loaded_postscrpts[url] = true;
                    jQuery.getScript(url, function () {
                        if (debug)  console.log('Loaded post ', url);
                    });
                }
            }
        });
    ...
    async function load_secure_scripts(scripts, defer) {
        ...
        let processed_scripts = await Promise.all(scripts.map(async (script, i) => {
            ...
            if (isSignatureValid) {
                ...
                if (debug)  console.log('IA_Designer_Loader: Invalid Signature for script', script);
                let decoded = decode();
                if (decoded) {
                    return decoded;
                }
    ...
    function error_loading(scr, textStatus) {
        if (debug)  console.log('IA_Designer_Loader: Problem Loading script...');
    }
