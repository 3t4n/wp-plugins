...
//    public $name;
    ...
    private $routes = [
        ...
        "vid_info" => "vidInfo.php" //route for requesting video information
    ];
    /**
...
//        return $pre;
    ...
    public function record_saved($id, $page_id, $post_id)
    {

        if (!$id) {
            return;
        }
        $params = ["record_save" => $id];
        //get url to the page with given page_id
        if ($page_id) {
            $page_id = get_permalink($page_id);
            if ($page_id) {
                $params["page_link"] = $page_id;
            }
        }
        if ($post_id) {
            $post_id = get_permalink($post_id);
            if ($post_id) {
                $params["post_link"] = $post_id;
            }
        }

        $this->send_request($params, $this->routes['gallery'], false, true);
    }
...
//            return gzuncompress(base64_decode(get_option(IAMG_SLUG . self::ADMIN_EDITOR))); //old code
    ...
    public function update_app_script()
    {
        ...
        if (isset($results['lib'])) {
            ...
            if (isset($results['blocks'])) {
                update_option(IAMG_SLUG . self::APP_SCRIPT_BLOCKS, $results['blocks'], false);
            }
        ...
        if (isset($results['prelib'])) {
            ...
            if ($prescript) {
                update_option(IAMG_SLUG . self::APP_HAS_PRE_SCRIPT, true, false);
            } else {
                update_option(IAMG_SLUG . self::APP_HAS_PRE_SCRIPT, false, false);
            }
        ...
        if (isset($results['adminpres'])) {
            ...
            if ($adminpres) {
                if (isset($results['adminpres_version'])) {
                    update_option(IAMG_SLUG . self::ADMIN_EDITOR_VERSION, $results['adminpres_version'], false);
                }
    ...
    public function get_gallery($settings)
    {
        ...
        if (isset($settings["requested_images"])) {
            //strip out the requested images from the settings before sending the request to the server.
            $requested_images = $settings["requested_images"];
            unset($settings["requested_images"]);
        }

        $encrypt = !true;
        $results = $this->send_request(["settings" => $settings], $this->routes['gallery'], true, $encrypt);
        ...
        if (isset($results["svg"])) {
            if (isset($results['demo']) && !$results['demo']) {
                ...
                if ($requested_images) {
                    //put back the requested images to store them locallay storage.
                    $settings["requested_images"] = $requested_images;
                }

                $id = $results["id"] ?? "";

                $locator = $this->save_gallery_in_temp_storage($results["svg"], $settings, $dependence, $id);
                $results["locator"] = $locator;
            }

            if (defined('IAMG_DEBUG') && IAMG_DEBUG) {
                $res = $results;
                $res['svg'] = substr($res['svg'], 0, 100);
                IAMG_AdminNotice::add_debug_message($results);
            }
    ...
    public function get_video_info($video_id_or_search, $service = null, $page = null, $local = "")
    {
        $route = $this->routes['vid_info'];
        if ($service) {
            //video info mode
            $parms = ["search" => $video_id_or_search];
            ...
            $results = $this->send_request($parms,
                $route, true, false);
        } else {
            //search mode

            $results = $this->send_request(["video" => $video_id_or_search],
                $route, true, false);
        }


        if (!$results) {
            return ["error" => ($service) ? __("No results for the search") : __("No results for the video")];
        }

        return $results;
    }

    public function generate_video_thumbnail($url, $width = null, $height = null, $time = null)
    {
        if (is_array($url)) {
        ...
        $params = ["thumbnail" => $url, "width" => $width, "height" => $height];
        if ($time && is_numeric($time) && $time >= 0) {
            $params["time"] = $time;
        }
        $results = $this->send_request($params,
            $this->routes['vid_info'], true, false);

        if (!$results) {
            return ["error" => __("No results for the video")];
        }

        unset($results['contact_server']);

        return $results;
    }

    /**
     * Sets a gallery to a post.
     *
     * @param string $locator The unique identifier for the gallery in temporary storage.
     * @param int $local_id The local ID of the gallery.
     * ********Page-specific parameters********
     * @param string $block_id The block ID where the gallery is placed.
     * @param int $page_id The ID of the page where the gallery is placed.
     * @param string $title The title of the gallery.
     * @param int $post_id The ID of the post where the gallery is placed.
     * @param bool $is_gallery_post Whether the post is a gallery post.
     * @param int|null $old_post_id The old post ID, if any.
     * @return bool|array Returns the gallery settings if successful, "expired" if the gallery has expired, or false on failure.
     */
    public function set_gallery_to_post(
        $locator,
        $local_id,
        $block_id,
        $page_id,
    ...
    )
    {
        if ($locator) {
            ...
            if ($gallery) {
                /** @var \WP_Post $update_post */
                $update_post = \IAMG_posttype::update_post($local_id, $gallery["svg"], $gallery["settings"], $title,
                    $block_id, $page_id, $post_id, $is_gallery_post, $gallery["dependence"], $old_post_id);
                if ($update_post) {
                    $id = $gallery["id"] ?? "";
                    return [$gallery["settings"], $update_post->post_title, "", $id];
                }
                return [false, "", "update_post_failed", ""];
            } else {
...
//                update_option(IAMG_SLUG . "_called_reg_client", $results);
                ...
                update_option(IAMG_SLUG . "_called_reg_client_exception", $e->getMessage(), false);
...
//                        echo "in save\n";
                        update_option(IAMG_SLUG . self::REGISTERED_SERVER, $results['contact_server'], false);
                        $this->save_key($key);
...
//        return $server_success ." local: ". $local_success;
    ...
    {
        update_option(IAMG_SLUG . "_api_key", $key, false);
    }
    ...
    public function send_request($params, $route, $blocking = true, $encrypt = PHP_INT_MAX, $backup_server = false)
    {
        ...
        $time = microtime(true);
        $access_error = $this->check_server_access_error($url);
...
//            wp_send_json($data);
            ...
            if ($access_error === self::ERROR_NO_ACCESS_TO_INTERNET) {
...
//        wp_send_json($response['body']);
    ...
    private function check_server_access_error($url, $timeout = 5)
    {
        $headers = array(
        ...
        $options_response = wp_remote_request($url, [
            'method' => 'OPTIONS',
            'timeout' => $timeout,
            'headers' => $headers,
        ]);
        ...
        if (is_wp_error($options_response) || wp_remote_retrieve_response_code($options_response) !== 200) {
           return $this->analyze_curl_error($options_response);
        }
...
//        }
    ...
    private function save_gallery_in_temp_storage($svg, $settings, $dependence = 0, $id = "")
    {
        $locator = md5($svg);
        set_transient($this->get_slug() . $locator,
            ['svg' => $svg, 'settings' => $settings, 'id' => $id, 'dependence' => $dependence], 60 * 60);
