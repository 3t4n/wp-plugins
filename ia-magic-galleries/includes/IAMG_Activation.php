...
//        return ["success" => false, "error" => "Early end"];
        ...
        update_option($client->get_slug() . '_called_reg_client', $registered, false); //for debugging
...
//            return $this->end_activation($message);
        ...
        if ($registered["success"]) {
            $this->update_app();
            update_option($client->get_slug() . '_registered', gmdate("Y-m-d H:i:s"), false);
            return ["success" => true];
        }
    }


    function iamg_add_custom_action_link($links)
    {
        $link = admin_url('admin-ajax.php') . '?action=iamg_com&command=reload_resources';
        $custom_link = '<a href="' . $link . '">' . esc_html__('Reload Resources',
                'ia-magic-galleries') . '</a>';
        $links[] = $custom_link;
        return $links;
    }
...
//                IAMG_AdminNotice::display_notice("A notice here using display notice4");
            ...
            $action_name = 'plugin_action_links_' . plugin_basename(IAMG_MAIN_FILE);
//            die($action_name);
            add_filter($action_name, [$this, 'iamg_add_custom_action_link']);

            $notice = get_transient('iamg_admin_notice');
            if ($notice) {
                IAMG_AdminNotice::display_notice($notice, IAMG_AdminNotice::INFO);
