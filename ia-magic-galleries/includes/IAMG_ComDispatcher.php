        ...
        if ($this->is_allowed()) {
            ...
            if ($command && $command[0] !== '_' && $command !== __FUNCTION__ && method_exists($this, $command)) {
                ...
                if (defined('IAMG_DEBUG') && IAMG_DEBUG) {
                    $debug = IAMG_AdminNotice::dump_debug_messaged();
                    if ($debug && is_array($output)) {
                        $output['debug'] = $debug;
                    }
                }
...
//    }
    ...
    public function save()
    {
        ...
        $old_pres_id = $this->_get_param('old_pres_id');
        if ($old_pres_id && !$this->_validate_press_id($old_pres_id)) {
            wp_send_json(['error' => 'Invalid Old Presentation ID']);
        }
        ...
        if ($locator) { // we should always have a locator
            ...
            list($settings, $title, $error, $id) = $IAMG_Client->set_gallery_to_post(
                $locator, // locator in temporary storage where the just-created gallery is stored
                $pres_id, // local id for the gallery post (stored as post-meta)
                /// This is for a gallery on a page
                $block_id, // the block id of the block that created the gallery (in the Gutenberg editor)
                $page_id, // the page id of the page where the gallery is created (in the Gutenberg editor)
                /// This is for a gallery as a post
                $title,
                // the title of the post to be used as gallery post title. Only provided when creating a new gallery directly as a post (not as a block)
                $post_id, // the post id of the post where the gallery is created directly as a post (not as a block)
                $is_gallery_post, // true if the gallery is created directly as a post (not as a block)
                $old_pres_id, // the old presentation id of the gallery post if it is being updated
            );
            ...
            if ($settings === 'expired') {
                wp_send_json([
                    ...
                    'success' => false,
                    'status' => 'Expired',
                    'error' => 'Please, click preview button again.'
                ]);
            } elseif ($settings) {

                if ($id) {
                    if (defined('IAMG_DEBUG') && IAMG_DEBUG) {
                        IAMG_AdminNotice::add_debug_message([
                            'id' => $id,
                            'page_id' => $page_id,
                            'post_id' => $post_id,
                            'locator' => $locator,
                    ...
                    register_shutdown_function(function () use ($id, $IAMG_Client, $page_id, $post_id) {
                        $IAMG_Client->record_saved($id, $page_id, $post_id);
                    });
//            });
                }
...
//        wp_send_json($imageHandler->get_for_library($start, $num_images, true));
        $for_library = $imageHandler->get_for_library($start, $num_images, true);

        //for debugging purposes, we can add more videos if we have less than requested
        if (false) {
            if (count($for_library) === 0) {
                $for_library = $imageHandler->get_for_library(0, $num_images, true);
            }
            if (count($for_library) < $num_images) {
                //if we have less than requested videos, copy the last video to the end of the array
                $for_library = array_merge($for_library,
                    array_fill(0, $num_images - count($for_library), end($for_library)));
            }
        }
    ...
    {
        if (!$this->_verify_nonce(['iamg_direct', 'iamg_admin_direct'])) {
            return ['error' => 'Video Info Access Denied'];
        }

        $video = $this->_get_param('video');
        //validate video for being ascii string
        if (!preg_match('/^[\x20-\x7E]*$/', $video)) {
            return ['error' => 'Invalid Video ID/Search string'];
        }
        $service = $this->_get_param('service');
        if ($service && !in_array($service, ['vimeo', 'youtube'])) {
            return ['error' => 'Invalid Service'];
        }
        $page = null;
        if ($service) {
            $page = $this->_get_param('p');
            if ($service === 'youtube') {
                //validate page for being a correct pageTocken
                if (!preg_match('/^[a-zA-Z0-9_-]{1,19}$/', $page)) {
                    $page = null;
                }
            } else {
                //validate page for being a number
                if (!is_numeric($page)) {
                    $page = null;
                }
            }
        ...
        return wp_json_encode((new IAMG_Client())->get_video_info($video, $service, $page));
    }
    ...
    public function make_gallery()
    {
        ...
        $imageHandler = new IAMG_ImageHandler(
            $has_images ? ($has_video ? "all" : false) : true
        );

        $image_info = $imageHandler->get_for_gallery($images);

//        wp_send_json($image_info);////
...
//            return ['error' => 'Gallery Info Access Denied'];
//        }


        $locator = $this->_get_param('gal_id');
        if (!$locator) {
            return ['error' => 'Invalid Gallery ID'];
        }

        $info = IAMG_posttype::get_post_info($locator);

        return $info ?? ['error' => 'Gallery not found'];
    }
    ...
    public function reload_resources()
    {
        if (!$this->_verify_nonce(['iamg_direct', 'iamg_admin_direct']) ||
            !current_user_can('manage_options')) {
            return ['error' => 'Settings Access Denied'];
        ...
        $client = new IAMG_Client();
        $results = $client->update_app_script();

        if (!$results || isset($results['error']) && $results['error']) {
            $report = ['error' => 'Failed to update resources'];
        } else {
            $report = ['success' => 'Resources updated'];
        }

        //if post request send the response as json
        if (isset($_POST['command'])) {
            wp_send_json($report);
        } else {
            if (isset($report['error'])) {
                $error = __("Failed to update resources!", 'ia-magic-galleries') . ": "
                    . (isset($results['error']) ? json_encode($results['error']) : "");

                $error .= " " . json_encode($results["message"] ?? "");

                set_transient('iamg_admin_notice', $error, 5);
            } else {
                set_transient('iamg_admin_notice', __("Resources updated successfully!", 'ia-magic-galleries'), 5);
            }
    ...
    public function iamg_posts()
    {

        if (!$this->_verify_nonce(['iamg_direct', 'iamg_admin_direct'])
//            || !current_user_can('manage_options')
        ) {
            return ['error' => 'Posts Access Denied'];
        }
        ...
        $info = IAMG_posttype::get_all_iamg_posts();

        $info = array_slice($info, $start, $num_images);
