...
if ( ! class_exists( 'Wpcis_Backend' ) && class_exists( 'WC_Product' ) ) {
	class Wpcis_Backend {
		...
		function style_settings( $product_id = 0 ) {
			...
			$swap        = self::get_swap();
			$slider      = self::get_slider( 0, 'setting' );
			...
			if ( $product_id ) {
				...
				$slider      = self::get_slider( $product_id, 'setting' );
		...
		public static function get_slider( $product_id = 0, $context = 'display' ) {
			...
			return apply_filters( 'wpcis_get_slider', $slider, $product_id, $context );
		}
