...
class TotalClicks {
    ...
    setupEventListeners() {
        ...
        jQuery('#linkcentral-stats-all-links').on('click', (e) => {
            e.preventDefault();
    ...
    updateAllLinksButtonState(isSelected) {
        const $allLinksButton = jQuery('#linkcentral-stats-all-links');
        if (isSelected) {
    ...
    updateChart(data) {
        ...
        this.chart.updateOptions({
            ...
            dataLabels: {
                enabled: data.labels.length <= 30, // Only show data labels if we have 30 or fewer points
