...
class LinkCentral_Total_Clicks {
    ...
    public function ajax_get_stats() {
        ...
        // Get stats data
        $data = $this->get_stats_data($days, $start_date, $end_date, $since_start);
     ...
     *
     * @param int|string $days       The number of days to retrieve data for, or 'since_start'.
     ...
     * @param bool       $since_start Whether to get data since the first recorded click.
     * @return array An array containing labels and click data for the chart.
     */
    public function get_stats_data($days, $start_date, $end_date, $since_start = false) {
        global $wpdb;
        ...
        if ($since_start && linkcentral_fs()->can_use_premium_code__premium_only()) {
            // Get the date of the first click
            $first_click = $wpdb->get_var("SELECT MIN(click_date) FROM {$wpdb->prefix}linkcentral_stats");
            if ($first_click) {
                $start = new DateTime($first_click);
                $start->setTime(0, 0, 0); // Start of the day
                $end = new DateTime('tomorrow'); // Include today
            } else {
                // If no clicks yet, show last 7 days
    ...
    public function ajax_get_specific_link_stats() {
        ...
        // Get stats data
        $data = $this->get_specific_link_stats_data($link_id, $days, $start_date, $end_date, $since_start);
        wp_send_json_success($data);
    }
     ...
     *
     * @param int       $link_id    The ID of the link to get stats for.
     * @param int|string $days       The number of days to retrieve data for, or 'since_start'.
     * @param string    $start_date The start date for custom date range (format: Y-m-d).
     * @param string    $end_date   The end date for custom date range (format: Y-m-d).
     * @param bool      $since_start Whether to get data since the first recorded click.
     * @return array An array containing labels and click data for the chart.
     */
    private function get_specific_link_stats_data($link_id, $days, $start_date, $end_date, $since_start = false) {
        global $wpdb;
        ...
        // Determine the date range
        if ($since_start && linkcentral_fs()->can_use_premium_code__premium_only()) {
            // Get the date of the first click for this specific link
            $first_click = $wpdb->get_var($wpdb->prepare(
                "SELECT MIN(click_date) FROM {$wpdb->prefix}linkcentral_stats WHERE link_id = %d",
                $link_id
            ));
            if ($first_click) {
                $start = new DateTime($first_click);
