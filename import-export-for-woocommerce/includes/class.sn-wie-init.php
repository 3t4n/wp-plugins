...
if ( ! class_exists( 'SN_WIE_INIT' ) ) {
    ...
    class SN_WIE_INIT {
        ...
        public static function set_message( $message, $type ) {
            set_transient('sn_wie_message', ['type' => $type, 'message' => $message], 60);
        }
        ...
        public static function show_message() {
            $message = get_transient('sn_wie_message');
            if( $message ) {
                echo('<div id="message" class="sn-wie-message updated notice notice-success is-dismissible">');
                echo('<p>'. $message['message'] .'</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">'.__('Dismiss this notice', SN_WIE_SLUG).'</span></button>');
                echo('</div>');
            }
            delete_transient('sn_wie_message');
        }
