	...
	class template_editor_class {
        ...
        public static function render_block($block_content, $block) {
        	...
        	if (
                ...
                wp_enqueue_style('ofbt-has-expandable-modal-submenus');
        	...
        	if (
                ...
                isset($block['attrs']['hasFullBlockLink']) &&
                $block['attrs']['hasFullBlockLink']
            ) {

                $block_content = preg_replace(
                    '/' . preg_quote( 'class="', '/' ) . '/',
                    'class="' . esc_attr('has-full-block-link') . ' ',
                    $block_content,
                    1
	            );

                wp_enqueue_script('ofbt-has-full-block-link');
        	...
        	if (
                ...
                isset($block['attrs']['hasCloseOtherDetailsWhenOpened']) &&
                $block['attrs']['hasCloseOtherDetailsWhenOpened']
            ) {

                $block_content = preg_replace(
                    '/' . preg_quote( 'class="', '/' ) . '/',
                    'class="' . esc_attr('has-close-other-details-when-opened') . ' ',
                    $block_content,
                    1
	            );

                wp_enqueue_script('ofbt-has-close-other-details-when-opened');
...
<?php
        ...
        public function wp_enqueue_scripts() {
            ...
            if (!is_admin()) {
                ...
                wp_register_style(
                    'ofbt-has-expandable-modal-submenus',
                ...
                wp_register_script(
                    'ofbt-has-full-block-link',
                    plugin_dir_url( __FILE__ ) . 'js/has-full-block-link.js',
                    array(),
                    teCommon::plugin_version(),
                    true
                );

                wp_register_script(
                    'ofbt-has-close-other-details-when-opened',
                    plugin_dir_url( __FILE__ ) . 'js/has-close-other-details-when-opened.js',
                    array(),
                    teCommon::plugin_version(),
                    true
                );
