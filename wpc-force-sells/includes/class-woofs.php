...
if ( ! class_exists( 'WPCleverWoofs' ) && class_exists( 'WC_Product' ) ) {
	class WPCleverWoofs {
		...
		function __construct() {
			...
			add_action( 'wp_ajax_woofs_add_text', [ $this, 'ajax_add_text' ] );
			...
			if ( WPCleverWoofs_Helper()->get_setting( 'position', 'before' ) === 'before' ) {
			...
			if ( WPCleverWoofs_Helper()->get_setting( 'hide_linked_order', 'no' ) === 'yes_text' || WPCleverWoofs_Helper()->get_setting( 'hide_linked_order', 'no' ) === 'yes_list' ) {
			...
			// Get item data
			if ( WPCleverWoofs_Helper()->get_setting( 'hide_linked', 'no' ) === 'yes_text' || WPCleverWoofs_Helper()->get_setting( 'hide_linked', 'no' ) === 'yes_list' ) {
			...
			// Search filters
			if ( WPCleverWoofs_Helper()->get_setting( 'search_sku', 'no' ) === 'yes' ) {
				add_filter( 'pre_get_posts', [ $this, 'search_sku' ], 99 );
			}
			if ( WPCleverWoofs_Helper()->get_setting( 'search_exact', 'no' ) === 'yes' ) {
				add_action( 'pre_get_posts', [ $this, 'search_exact' ], 99 );
			}
			if ( WPCleverWoofs_Helper()->get_setting( 'search_sentence', 'no' ) === 'yes' ) {
		...
		function init() {
			...
			// load text-domain
			load_plugin_textdomain( 'wpc-force-sells', false, basename( WOOFS_DIR ) . '/languages/' );
		}
            ...
            <div class="wpclever_settings_page wrap">
                ...
                <div class="wpclever_settings_page_desc about-text">
                    <p>
                        ...
                        <br/>
                        <a href="<?php echo esc_url( WOOFS_REVIEWS ); ?>"
                           target="_blank"><?php esc_html_e( 'Reviews', 'wpc-force-sells' ); ?></a> |
                        <a href="<?php echo esc_url( WOOFS_CHANGELOG ); ?>"
                           target="_blank"><?php esc_html_e( 'Changelog', 'wpc-force-sells' ); ?></a> |
                        <a href="<?php echo esc_url( WOOFS_DISCUSSION ); ?>"
                           target="_blank"><?php esc_html_e( 'Discussion', 'wpc-force-sells' ); ?></a>
                    </p>
                </div>
                ...
                <div class="wpclever_settings_page_nav">
                    <h2 class="nav-tab-wrapper">
                        <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-woofs&tab=settings' ) ); ?>"
                           class="<?php echo esc_attr( $active_tab === 'settings' ? 'nav-tab nav-tab-active' : 'nav-tab' ); ?>">
							<?php esc_html_e( 'Settings', 'wpc-force-sells' ); ?>
                        </a>
                        <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-woofs&tab=localization' ) ); ?>"
                        ...
                        <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-woofs&tab=premium' ) ); ?>"
                ...
                <div class="wpclever_settings_page_content">
					<?php if ( $active_tab === 'settings' ) {
						$position              = WPCleverWoofs_Helper()->get_setting( 'position', 'before' );
						$layout                = WPCleverWoofs_Helper()->get_setting( 'layout', 'list' );
						$show_thumb            = WPCleverWoofs_Helper()->get_setting( 'show_thumb', 'yes' );
						$show_price            = WPCleverWoofs_Helper()->get_setting( 'show_price', 'yes' );
						$show_description      = WPCleverWoofs_Helper()->get_setting( 'show_description', 'no' );
						$link                  = WPCleverWoofs_Helper()->get_setting( 'link', 'yes' );
						$variations_selector   = WPCleverWoofs_Helper()->get_setting( 'variations_selector', 'default' );
						$change_image          = WPCleverWoofs_Helper()->get_setting( 'change_image', 'yes' );
						$change_price          = WPCleverWoofs_Helper()->get_setting( 'change_price', 'yes' );
						$hide_linked           = WPCleverWoofs_Helper()->get_setting( 'hide_linked', 'no' );
						$hide_linked_mini_cart = WPCleverWoofs_Helper()->get_setting( 'hide_linked_mini_cart', 'no' );
						$hide_linked_order     = WPCleverWoofs_Helper()->get_setting( 'hide_linked_order', 'no' );
						$exclude_unpurchasable = WPCleverWoofs_Helper()->get_setting( 'exclude_unpurchasable', 'no' );
						$cart_contents_count   = WPCleverWoofs_Helper()->get_setting( 'cart_contents_count', 'both' );
						?>
                        <form method="post" action="options.php">
                            <table class="form-table">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <label> <select name="woofs_settings[variations_selector]">
                                                ...
                                                <option value="woovr" <?php selected( $variations_selector, 'woovr' ); ?>><?php esc_html_e( 'Use WPC Variations Radio Buttons', 'wpc-force-sells' ); ?></option>
                                            </select> </label> <span class="description">If you choose "Use WPC Variations Radio Buttons", please install <a
                                                    ...
                                                    class="thickbox" title="WPC Variations Radio Buttons">WPC Variations Radio Buttons</a> to make it work.</span>
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <label>
                                            <select name="woofs_settings[change_price]"
                                                    class="woofs_change_price">
                                            ...
                                            <input type="text" class="woofs_change_price_custom"
                                                   name="woofs_settings[change_price_custom]"
                                                   value="<?php echo WPCleverWoofs_Helper()->get_setting( 'change_price_custom', '.summary > .price' ); ?>"
                            ...
                            <table class="form-table">
                                    ...
                                    <td>
                                        <label>
                                            <input type="text" class="regular-text"
                                                   name="woofs_localization[additional]"
                                                   value="<?php echo esc_attr( WPCleverWoofs_Helper()->localization( 'additional' ) ); ?>"
                                                   placeholder="<?php esc_attr_e( 'Additional price:', 'wpc-force-sells' ); ?>"/>
                                        </label>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <label>
                                            <input type="text" class="regular-text"
                                                   name="woofs_localization[total]"
                                                   value="<?php echo esc_attr( WPCleverWoofs_Helper()->localization( 'total' ) ); ?>"
                                                   placeholder="<?php esc_attr_e( 'Total:', 'wpc-force-sells' ); ?>"/>
                                        </label>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <label>
                                            <input type="text" class="regular-text"
                                                   name="woofs_localization[linked]"
                                                   value="<?php echo esc_attr( WPCleverWoofs_Helper()->localization( 'linked' ) ); ?>"
                                                   placeholder="<?php /* translators: product name */
											       esc_attr_e( '(linked to %s)', 'wpc-force-sells' ); ?>"/>
                                        </label>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <label>
                                            <input type="text" class="regular-text"
                                                   name="woofs_localization[choose]"
                                                   value="<?php echo esc_attr( WPCleverWoofs_Helper()->localization( 'choose' ) ); ?>"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <label>
                                            <input type="text" class="regular-text"
                                                   name="woofs_localization[clear]"
                                                   value="<?php echo esc_attr( WPCleverWoofs_Helper()->localization( 'clear' ) ); ?>"
                                                   placeholder="<?php esc_attr_e( 'Clear', 'wpc-force-sells' ); ?>"/>
                                        </label>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <label>
                                            <input type="text" class="large-text"
                                                   name="woofs_localization[above_text]"
                                                   value="<?php echo esc_attr( WPCleverWoofs_Helper()->localization( 'above_text' ) ); ?>"/>
                                        </label>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <label>
                                            <input type="text" class="large-text"
                                                   name="woofs_localization[under_text]"
                                                   value="<?php echo esc_attr( WPCleverWoofs_Helper()->localization( 'under_text' ) ); ?>"/>
                                        </label>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <label>
                                            <input type="text" class="regular-text"
                                                   name="woofs_localization[linked_products]"
                                                   value="<?php echo esc_attr( WPCleverWoofs_Helper()->localization( 'linked_products' ) ); ?>"
                                                   placeholder="<?php esc_attr_e( 'Linked products', 'wpc-force-sells' ); ?>"/>
                                        </label>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <label>
                                            <input type="text" class="regular-text"
                                                   name="woofs_localization[linked_products_s]"
                                                   value="<?php echo esc_attr( WPCleverWoofs_Helper()->localization( 'linked_products_s' ) ); ?>"
                                                   placeholder="<?php /* translators: product name */
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <label>
                                            <input type="text" class="large-text"
                                                   name="woofs_localization[alert_selection]"
                                                   value="<?php echo esc_attr( WPCleverWoofs_Helper()->localization( 'alert_selection' ) ); ?>"
                                                   placeholder="<?php esc_attr_e( 'Please select a purchasable variation for [name] before adding this product to the cart.', 'wpc-force-sells' ); ?>"/>
                                        </label>
                                    </td>
                                </tr>
                                <tr class="submit">
					...
					<?php } elseif ( $active_tab === 'premium' ) { ?>
                        <div class="wpclever_settings_page_content_text">
                            <p>
                                ...
                                <a href="https://wpclever.net/downloads/force-sells?utm_source=pro&utm_medium=woofs&utm_campaign=wporg"
                                   target="_blank">https://wpclever.net/downloads/force-sells</a>
                            </p>
		...
		function search_settings() {
			$search_sku      = WPCleverWoofs_Helper()->get_setting( 'search_sku', 'no' );
			$search_id       = WPCleverWoofs_Helper()->get_setting( 'search_id', 'no' );
			$search_exact    = WPCleverWoofs_Helper()->get_setting( 'search_exact', 'no' );
			$search_sentence = WPCleverWoofs_Helper()->get_setting( 'search_sentence', 'no' );
			$search_same     = WPCleverWoofs_Helper()->get_setting( 'search_same', 'no' );
			?>
            <tr>
                <th><?php esc_html_e( 'Search limit', 'wpc-force-sells' ); ?></th>
                <td>
                    <label>
                        <input class="woofs_search_limit" type="number" name="woofs_settings[search_limit]"
                               value="<?php echo esc_attr( WPCleverWoofs_Helper()->get_setting( 'search_limit', 10 ) ); ?>"/>
                    </label>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
					<?php
					$search_types  = WPCleverWoofs_Helper()->get_setting( 'search_types', [ 'all' ] );
		...
		function enqueue_scripts() {
			...
			wp_localize_script( 'woofs-frontend', 'woofs_vars', apply_filters( 'woofs_vars', [
					...
					'change_image'             => WPCleverWoofs_Helper()->get_setting( 'change_image', 'yes' ),
					'change_price'             => WPCleverWoofs_Helper()->get_setting( 'change_price', 'yes' ),
					'price_selector'           => WPCleverWoofs_Helper()->get_setting( 'change_price_custom', '' ),
					'variations_selector'      => WPCleverWoofs_Helper()->get_setting( 'variations_selector', 'default' ),
					'additional_text'          => WPCleverWoofs_Helper()->localization( 'additional', esc_html__( 'Additional price:', 'wpc-force-sells' ) ),
					'total_text'               => WPCleverWoofs_Helper()->localization( 'total', esc_html__( 'Total:', 'wpc-force-sells' ) ),
					'alert_selection'          => WPCleverWoofs_Helper()->localization( 'alert_selection', esc_html__( 'Please select a purchasable variation for [name] before adding this product to the cart.', 'wpc-force-sells' ) ),
				] )
			);
		}
		...
		function action_links( $links, $file ) {
			...
			if ( ! isset( $plugin ) ) {
				$plugin = plugin_basename( WOOFS_FILE );
			}
				...
				if ( ! empty( $items ) && is_array( $items ) ) {
					foreach ( $items as $key => $item ) {
						...
						if ( $item['id'] ) {
							...
							if ( ( WPCleverWoofs_Helper()->get_setting( 'exclude_unpurchasable', 'no' ) !== 'yes' ) && ( ! $item_product->is_purchasable() || ! $item_product->is_in_stock() || ! $item_product->has_enough_stock( $product_items[ $key ]['qty'] ) ) ) {
								wc_add_notice( sprintf( /* translators: product name */ esc_html__( '"%s" is un-purchasable.', 'wpc-force-sells' ), esc_html( $item_product->get_name() ) ), 'error' );
								wc_add_notice( esc_html__( 'You cannot add this product to the cart.', 'wpc-force-sells' ), 'error' );
					...
					if ( WPCleverWoofs_Helper()->get_setting( 'exclude_unpurchasable', 'no' ) !== 'yes' ) {
						...
						if ( ! $item_product->is_type( 'variable' ) && $item_product->is_in_stock() && $item_product->is_purchasable() && $item_product->has_enough_stock( $item_qty ) ) {
							...
							if ( $separately !== 'on' ) {
								$item_data = [
									...
									'woofs_price'         => WPCleverWoofs_Helper()->format_price( $item_price ),
									'woofs_qty'           => $item_qty
								];
		...
		function before_order_itemmeta( $order_item_id, $order_item ) {
			if ( $ids = $order_item->get_meta( '_woofs_ids' ) ) {
				...
				if ( is_array( $items ) && ! empty( $items ) ) {
					...
					echo apply_filters( 'woofs_before_order_itemmeta', '<div class="woofs-itemmeta-linked">' . sprintf( WPCleverWoofs_Helper()->localization( 'linked_products_s', /* translators: product name */ esc_html__( 'Linked products: %s', 'wpc-force-sells' ) ), $items_str ) . '</div>', $order_item_id, $order_item );
				}
			}
		...
		function mini_cart_item_visible( $visible, $cart_item ) {
			if ( ! empty( $cart_item['woofs_parent_id'] ) ) {
				...
				if ( WPCleverWoofs_Helper()->get_setting( 'hide_linked_mini_cart', 'no' ) === 'yes' ) {
					return false;
				}
			}
		...
		function cart_item_visible( $visible, $cart_item ) {
			if ( ! empty( $cart_item['woofs_parent_id'] ) ) {
				...
				if ( WPCleverWoofs_Helper()->get_setting( 'hide_linked', 'no' ) !== 'no' ) {
					return false;
				}
			}
		...
		function order_item_visible( $visible, $order_item ) {
			if ( $parent_id = $order_item->get_meta( '_woofs_parent_id' ) ) {
				...
				if ( WPCleverWoofs_Helper()->get_setting( 'hide_linked_order', 'no' ) !== 'no' ) {
					return false;
				}
			}
		...
		function order_item_meta_start( $order_item_id, $order_item ) {
			if ( $ids = $order_item->get_meta( '_woofs_ids' ) ) {
				...
				if ( is_array( $items ) && ! empty( $items ) ) {
					if ( WPCleverWoofs_Helper()->get_setting( 'hide_linked_order', 'no' ) === 'yes_list' ) {
						$items_arr = [];
					...
					echo apply_filters( 'woofs_order_item_meta_start', '<div class="woofs-itemmeta-linked">' . sprintf( WPCleverWoofs_Helper()->localization( 'linked_products_s', /* translators: product name */ esc_html__( 'Linked products: %s', 'wpc-force-sells' ) ), $items_str ) . '</div>', $order_item_id, $order_item );
				}
			}
		}
		...
		function get_item_data( $data, $cart_item ) {
			...
			if ( is_array( $items ) && ! empty( $items ) ) {
				if ( WPCleverWoofs_Helper()->get_setting( 'hide_linked', 'no' ) === 'yes_list' ) {
					$items_str = [];
					...
					$data['woofs_data'] = [
						'key'     => WPCleverWoofs_Helper()->localization( 'linked_products', esc_html__( 'Linked products', 'wpc-force-sells' ) ),
		...
		function ajax_get_search_results() {
			$types       = WPCleverWoofs_Helper()->get_setting( 'search_types', [ 'all' ] );
			$keyword     = sanitize_text_field( $_POST['woofs_keyword'] );
			$id          = absint( sanitize_text_field( $_POST['woofs_id'] ) );
			$exclude_ids = [ $id ];
			$added_ids   = explode( ',', WPCleverWoofs_Helper()->clean_ids( sanitize_text_field( $_POST['woofs_ids'] ) ) );

			if ( ( WPCleverWoofs_Helper()->get_setting( 'search_id', 'no' ) === 'yes' ) && is_numeric( $keyword ) ) {
				// search by id
				$query_args = [
					'p'         => absint( $keyword ),
					'post_type' => 'product'
				...
				$limit = WPCleverWoofs_Helper()->get_setting( 'search_limit', 10 );
				...
				if ( WPCleverWoofs_Helper()->get_setting( 'search_same', 'no' ) !== 'yes' ) {
		...
		function product_data_li( $product, $price = '100%', $qty = 1, $search = false, $key = null ) {
			if ( ! $key ) {
				$key = WPCleverWoofs_Helper()->generate_key();
			}
		...
		function product_data_panels() {
			...
			?>
            <div id='woofs_settings' class='panel woocommerce_options_panel woofs_table'>
                <div id="woofs_search_settings" style="display: none"
                     data-title="<?php esc_html_e( 'Search settings', 'wpc-force-sells' ); ?>">
                    <table>
						<?php self::search_settings(); ?>
                        <tr>
                            <th></th>
                ...
                <table>
                    <tr>
                        <th><?php esc_html_e( 'Search', 'wpc-force-sells' ); ?> (<a
                                    href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-woofs&tab=settings#search' ) ); ?>"
                                    id="woofs_search_settings_btn"><?php esc_html_e( 'settings', 'wpc-force-sells' ); ?></a>)
                        </th>
                        <td>
                            <div class="w100">
                                        <span class="loading" id="woofs_loading"
                                              style="display:none;"><?php esc_html_e( 'searching...', 'wpc-force-sells' ); ?></span>
                                <label for="woofs_keyword"></label><input type="search" id="woofs_keyword"
                                                                          placeholder="<?php esc_html_e( 'Type any keyword to search', 'wpc-force-sells' ); ?>"/>
                    ...
                    <tr class="woofs_tr_space">
                        ...
                        <td>
                            <a href="https://wpclever.net/downloads/force-sells?utm_source=pro&utm_medium=woofs&utm_campaign=wporg"
                               target="_blank" class="woofs_add_txt button"
                    ...
                    <tr class="woofs_tr_space">
                        ...
                        <td>
                            <label for="woofs_sync_quantity"></label><input id="woofs_sync_quantity"
                                                                            name="woofs_sync_quantity"
                                                                            type="checkbox" <?php echo esc_attr( get_post_meta( $product_id, 'woofs_sync_quantity', true ) !== 'off' ? 'checked' : '' ); ?>/>
                            <span class="woocommerce-help-tip"
                                  data-tip="<?php esc_attr_e( 'By default, the quantity of Force Sells items is synced with that of the main product. Uncheck to stick to the default quantities.', 'wpc-force-sells' ); ?>"></span>
                        </td>
                    </tr>
                    <tr class="woofs_tr_space">
                        <th><?php esc_html_e( 'Add separately', 'wpc-force-sells' ); ?></th>
                        <td>
                            <label for="woofs_separately"></label><input id="woofs_separately"
                                                                         name="woofs_separately"
                                                                         type="checkbox" <?php echo esc_attr( get_post_meta( $product_id, 'woofs_separately', true ) === 'on' ? 'checked' : '' ); ?>/>
                            <span class="woocommerce-help-tip"
                                  data-tip="<?php esc_attr_e( 'If enabled, the force sell products will be added as separate items and stay unaffected from the main product, their prices will change back to the original.', 'wpc-force-sells' ); ?>"></span>
                        </td>
                    </tr>
                    <tr class="woofs_tr_space">
                        <th><?php esc_html_e( 'Layout', 'wpc-force-sells' ); ?></th>
                    ...
                    <tr class="woofs_tr_space">
                        ...
                        <td>
                            <div class="w100">
                                <label>
                                            <textarea name="woofs_before_text" rows="1"
                                                      style="width: 100%"><?php echo esc_textarea( get_post_meta( $product_id, 'woofs_before_text', true ) ); ?></textarea>
                                </label>
                            </div>
                        </td>
                    </tr>
                    <tr class="woofs_tr_space">
                        ...
                        <td>
                            <div class="w100">
                                <label>
                                            <textarea name="woofs_after_text" rows="1"
                                                      style="width: 100%"><?php echo esc_textarea( get_post_meta( $product_id, 'woofs_after_text', true ) ); ?></textarea>
                                </label>
                            </div>
                        </td>
                    </tr>
		...
		function process_product_meta( $post_id ) {
			if ( isset( $_POST['woofs_ids'] ) ) {
				update_post_meta( $post_id, 'woofs_ids', WPCleverWoofs_Helper()->sanitize_array( $_POST['woofs_ids'] ) );
			} else {
		...
		function show_items() {
			...
			if ( ! empty( $items ) && is_array( $items ) ) {
				...
				if ( $before_text = apply_filters( 'woofs_before_text', get_post_meta( $product_id, 'woofs_before_text', true ) ?: WPCleverWoofs_Helper()->localization( 'above_text' ), $product_id ) ) {
				...
				$sync_quantity  = get_post_meta( $product_id, 'woofs_sync_quantity', true ) ?: 'on';
				$separately     = get_post_meta( $product_id, 'woofs_separately', true ) ?: 'off';
				$layout         = $layout !== 'unset' ? $layout : WPCleverWoofs_Helper()->get_setting( 'layout', 'list' );
                ...
                <div class="<?php echo esc_attr( $products_class ); ?>"
                     data-sync-quantity="<?php echo esc_attr( $sync_quantity ); ?>"
                     data-separately="<?php echo esc_attr( $separately ); ?>"
                     data-product-type="<?php echo esc_attr( $product->get_type() ); ?>"
                     data-variables="<?php echo esc_attr( self::has_variables( $items ) ? 'yes' : 'no' ); ?>"
                     data-product-price="<?php echo esc_attr( apply_filters( 'woofs_product_price', $product->get_type() === 'variable' ? '0' : wc_get_price_to_display( $product ), $product ) ); ?>"
                     data-product-regular-price="<?php echo esc_attr( apply_filters( 'woofs_product_regular_price', $product->get_type() === 'variable' ? '0' : wc_get_price_to_display( $product, [ 'price' => $product->get_regular_price() ] ), $product ) ); ?>"
					...
					foreach ( $items as $item_key => $item ) {
						if ( ! empty( $item['id'] ) ) {
							...
							if ( ( WPCleverWoofs_Helper()->get_setting( 'exclude_unpurchasable', 'no' ) !== 'yes' ) && ( ! $product->is_in_stock() || ! $product->has_enough_stock( $item_qty ) || ! $product->is_purchasable() ) ) {
								$item_data_id = 0;
							} else {
							...
							?>
                            <div class="<?php echo esc_attr( apply_filters( 'woobt_item_class', $item_class, $item ) ); ?>"
                                 data-key="<?php echo esc_attr( $item_key ); ?>"
                                 data-name="<?php echo esc_attr( $product->get_name() ); ?>"
                                 data-id="<?php echo esc_attr( $item_data_id ); ?>"
                                 data-price="<?php echo esc_attr( apply_filters( 'woofs_item_new_price', $item_data_price, $item, $product ) ); ?>"
                                 data-price-ori="<?php echo esc_attr( apply_filters( 'woofs_item_price', wc_get_price_to_display( $product ), $item, $product ) ); ?>"
                                 data-regular-price="<?php echo esc_attr( apply_filters( 'woofs_item_regular_price', wc_get_price_to_display( $product, [ 'price' => $product->get_regular_price() ] ), $item, $product ) ); ?>"
								...
								<?php if ( WPCleverWoofs_Helper()->get_setting( 'show_thumb', 'yes' ) !== 'no' ) { ?>
                                ...
                                <div class="woofs-title">
									...
									if ( WPCleverWoofs_Helper()->get_setting( 'link', 'yes' ) !== 'no' ) {
										echo '<a ' . ( WPCleverWoofs_Helper()->get_setting( 'link', 'yes' ) === 'yes_popup' ? 'class="woosq-link" data-id="' . $item_id . '" data-context="woofs"' : '' ) . ' href="' . get_permalink( $item_id ) . '" ' . ( WPCleverWoofs_Helper()->get_setting( 'link', 'yes' ) === 'yes_blank' ? 'target="_blank"' : '' ) . '>' . $product_name . '</a>';
									} else {
										echo $product_name;
									}
									...
									if ( WPCleverWoofs_Helper()->get_setting( 'show_description', 'no' ) === 'yes' ) {
								...
								<?php if ( WPCleverWoofs_Helper()->get_setting( 'show_price', 'yes' ) !== 'no' ) { ?>
                                    <div class="woofs-price">
                                        ...
                                        <div class="woofs-price-ori">
											<?php if ( ( $item_price !== '100%' ) && ( get_post_meta( $product_id, 'woofs_separately', true ) !== 'on' ) ) {
												echo '<del>' . wc_price( $product->get_price() ) . '</del> ' . wc_price( WPCleverWoofs_Helper()->new_price( $product->get_price(), $item_price ) );
											} else {
				...
				if ( $after_text = apply_filters( 'woofs_after_text', get_post_meta( $product_id, 'woofs_after_text', true ) ?: WPCleverWoofs_Helper()->localization( 'under_text' ), $product_id ) ) {
			...
			foreach ( $cart_contents as $cart_item_key => $cart_item ) {
				if ( ! empty( $cart_item['woofs_parent_key'] ) ) {
					...
					if ( isset( $cart_item['data'], $cart_item['woofs_price'] ) && ( $cart_item['woofs_price'] !== '' ) ) {
						$new_price = WPCleverWoofs_Helper()->new_price( apply_filters( 'woofs_product_original_price', $cart_item['data']->get_price() ), $cart_item['woofs_price'] );
			...
			if ( ! empty( $ids ) ) {
				...
				if ( is_array( $_items ) && count( $_items ) > 0 ) {
					foreach ( $_items as $_item ) {
						...
						$_item_key     = $_item_data[0] ?? WPCleverWoofs_Helper()->generate_key();
						...
						if ( ( $context === 'view' ) && ( ( WPCleverWoofs_Helper()->get_setting( 'exclude_unpurchasable', 'no' ) === 'yes' ) && ( ! $_item_product->is_purchasable() || ! $_item_product->is_in_stock() ) ) ) {
							continue;
						}

						$items[ $_item_key ] = [
							'id'    => $_item_id,
							'price' => WPCleverWoofs_Helper()->format_price( $_item_price ),
							'qty'   => (float) $_item_qty,
