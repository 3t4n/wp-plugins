			...
			if ( $more ) {
				...
				if ( $i && $more ) {
					if ( 1 == $i ) {
						...
						$output .= '<a href="' . $link . '">' . $link_before. $previouspagelink . $link_after . '</a>';
					}
				}
				$i = $page + 1;
				if ( $i <= $numpages && $more ) {
					if ( 1 == $i ) {
						...
						$output .= '<a href="' . $link . '">' . $link_before. $nextpagelink . $link_after . '</a>';
					}
				}
				$output .= $after;
			}
...
function the_summaries($lang = 'xml:lang,lang', $title = 'GENGO_DEFAULT') {
	...
	if ($title) {
		...
		?>
		<legend id="gengo_summaries_title" for="gengo_summaries_container"><?php echo $title ?></legend>
		<?php
	}
	?>
	...
	foreach ($viewable_summaries as $summary_id => $summary) {
		...
		?><p class="gengo_summary_inner" id="gengo_summary_<?php echo $summary_id ?>" style="display: <?php echo ($summary_id == $default_id) ? 'block' : 'none' ?>"<?php echo $lang_atts ?>><?php echo $summary['summary'] ?></p><?php
