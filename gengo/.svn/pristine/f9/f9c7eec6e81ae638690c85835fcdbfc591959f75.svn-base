=== Gengo ===
Contributors: majelbstoat, pixline, julienvdg
...
Gengo is a full featured plugin that provides multi-language blogging for WordPress 2.7+. 

== Description ==

Gengo is a full featured plugin that provides multi-language blogging for WordPress 2.7+. It allows for an unlimited number of translations and summaries for any post and provides template tags to display language information.  It allows you to edit translations side by side, detects and filters by language automatically when a visitor comes to your site and automatically generates semantic information for links and content blocks.  It is configurable via an options page.

Gengo is mainly written by [Jamie Talbot](http://jamietalbot.com/), with some help from [Paolo Tresso](http://pixline.net/en/) and [Julien Viard de Galbert] (http://silicone.homelinux.org)
...
* `gengo_link_pages()` - replacement for deprecated `link_pages()`.
* `gengo_wp_link_pages()` - replacement for `wp_link_pages()`.
