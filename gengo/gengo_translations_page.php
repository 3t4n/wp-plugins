...
<ul class="subsubsub">
<li><a href='admin.php?page=gengo/gengo_translations_page.php&amp;show=all'<?php echo $flag1;?>><?php _e('All groups', GENGO_DOMAIN) ?></a> |</li>
<li><a href='admin.php?page=gengo/gengo_translations_page.php&amp;show=new'<?php echo $flag2;?>><?php _e('Add new', GENGO_DOMAIN) ?></a></li>
</ul>
...
if ($groups = $wpdb->get_col("SELECT DISTINCT translation_group FROM $gengo->post2lang_table WHERE translation_group != 0 ORDER BY translation_group DESC LIMIT $lower_bound, ". GENGO_TRANSLATIONS_PAGED)) {
  $groups = implode(',', $groups);
	$results = $wpdb->get_results("SELECT ID AS post_id, post_title, translation_group, language FROM $wpdb->posts p INNER JOIN $gengo->post2lang_table p2l ON p.ID = p2l.post_id INNER JOIN $gengo->language_table l ON p2l.language_id = l.language_id WHERE p2l.translation_group IN ($groups) ORDER BY translation_group, post_id", ARRAY_A)	?>
		<div class="tablenav">
			<div class="alignleft">
	<p><?php _e('Use this page to manage existing translation groups and create new ones.  The rules for updating translation groups are as follows:', GENGO_DOMAIN); ?>
		<ul style="list-style-type:circle; margin:5px; padding:2px; font-size:80%;">
	...
	?>
			<div class="tablenav">
			<div class="alignleft">
				<p><?php _e('No translation groups defined yet.', GENGO_DOMAIN) ?></p>
			</div>
			<div class="tablenav-pages">
			</div>
		</div>
