...
final class Ajax {
	...
	public function dispatch() {
		...
		$action = isset( $_POST['action_actirise'] ) ? sanitize_text_field( wp_unslash( $_POST['action_actirise'] ) ) : null;
	...
	private function set_settings() {
		...
		$post_data = array(
			'uuid'  => isset( $_POST['uuid'] ) ? sanitize_text_field( wp_unslash( $_POST['uuid'] ) ) : null,
			'force' => isset( $_POST['force'] ) ? sanitize_text_field( wp_unslash( $_POST['force'] ) ) : false,
		);
		...
		if ( $settings === true && ! Options::get( 'init' ) ) {
			Options::update( 'init', true, true );
		}
		...
		wp_send_json(
			...
			?
				array(
					'success' => true,
					'data'    => array(
						'type'        => Options::get( 'settings-uuid-type' ),
						'presizedDiv' => Options::get( 'presizeddiv-selected', array() ),
						'adstxt'      => Options::get( 'adstxt-actirise', '' ),
						'customv'     => array(
	...
	private function set_custom_var() {
		...
		$custom_var = isset( $_POST['customVar'] ) ? sanitize_text_field( wp_unslash( $_POST['customVar'] ) ) : null;
		...
		foreach ( $json_custom_var as $custom ) {
			Options::update( $custom->name, $custom->value );
		}
	...
	private function toggle_feature() {
		...
		$post_data = array(
			'feature'       => isset( $_POST['feature'] ) ? sanitize_text_field( wp_unslash( $_POST['feature'] ) ) : null,
			'feature_value' => isset( $_POST['feature_value'] ) ? sanitize_text_field( wp_unslash( $_POST['feature_value'] ) ) : null,
		);
		...
		Options::update( $post_data['feature'] . '-active', $post_data['feature_value'] );
	...
	private function get_adstxt() {
		if ( ! Options::get( 'settings-uuid' ) || ! Options::get( 'settings-uuid-type', 'boot' ) ) {
		...
		if ( false !== $response ) {
			...
			Logger::add_log( 'get_adstxt false', 'admin/include/ajax', 'error' );
			$error = new WP_Error( '500', 'Error' );
	...
	private function get_adstxt_custom() {
		$adstxt_custom = Options::get( 'adstxt-custom', array() );
	...
	private function set_adstxt() {
		...
		$post_data = array(
			'adsTxtCustom' => isset( $_POST['adsTxtCustom'] ) ? sanitize_text_field( wp_unslash( $_POST['adsTxtCustom'] ) ) : null,
		);
		...
		Options::update( 'adstxt-custom', $post_data['adsTxtCustom'] );
	...
	private function set_adstxt_notification_read() {
		Options::update( 'adstxt-update', 'false' );
	...
	private function save_settings( $identifiant, $force ) {
		if ( $force ) {
			Options::update( 'settings-uuid', $identifiant, true );
			Options::update( 'settings-uuid-type', 'universal', true );
			...
			if ( $response_entity->get_error_code() === 404 ) {
				...
				if ( is_wp_error( $response_website ) ) {
					...
					if ( $response_website['datas'][0] === 'OK' ) {
						Options::update( 'settings-uuid', $identifiant, true );
						Options::update( 'settings-uuid-type', 'boot', true );
	...
	private function set_presized_div() {
		...
		$post_data = array(
			'presizedDiv' => isset( $_POST['presizedDiv'] ) ? sanitize_text_field( wp_unslash( $_POST['presizedDiv'] ) ) : null,
		);
		...
		Options::update( 'presizeddiv-selected', $presized_div );
	...
	private function get_articles_by_keywords() {
		...
		$post_data = array(
			'keywords' => isset( $_POST['keywords'] ) ? sanitize_text_field( wp_unslash( $_POST['keywords'] ) ) : null,
		);
	...
	private function add_no_pub() {
		...
		$post_data = array(
			'article' => isset( $_POST['article'] ) ? sanitize_text_field( wp_unslash( $_POST['article'] ) ) : null,
			'summary' => isset( $_POST['summary'] ) ? sanitize_text_field( wp_unslash( $_POST['summary'] ) ) : null,
		);

		if ( is_null( $post_data['article'] ) || is_null( $post_data['summary'] ) ) {
			if ( is_null( $post_data['article'] ) ) {
				Logger::add_log( 'add_no_pub post_data is null for article', 'admin/include/ajax', 'error' );
			}

			if ( is_null( $post_data['summary'] ) ) {
				Logger::add_log( 'add_no_pub post_data is null for summary', 'admin/include/ajax', 'error' );
			}
		...
		$no_pub = Options::get( 'nopub', array() );
	...
	private function check_url_no_pub() {
		...
		if ( is_null( $post_data['url'] ) ) {
			Logger::add_log( 'check_url_no_pub post_data is null for url', 'admin/include/ajax', 'error' );
		...
		if ( $post !== 0 ) {
			...
			if ( $post !== null ) {
				...
				Logger::add_log( 'check_url_no_pub post is null for:' . $url, 'admin/include/ajax', 'error' );
			}
		} else {
			Logger::add_log( 'check_url_no_pub url_to_postid is 0 for:' . $url, 'admin/include/ajax', 'error' );
		}
	...
	private function set_presized_div_notification_read() {
		Options::update( 'presizeddiv-notif', array() );
	...
	private function url_exists() {
		...
		if ( is_null( $post_data['id'] ) || is_null( $post_data['type'] ) ) {
			if ( is_null( $post_data['id'] ) ) {
				Logger::add_log( 'url_exists post_data is null for id', 'admin/include/ajax', 'error' );
			}

			if ( is_null( $post_data['type'] ) ) {
				Logger::add_log( 'url_exists post_data is null for type', 'admin/include/ajax', 'error' );
			}
	...
	private function enabled_debug() {
		...
		$post_data = array(
			'debug' => isset( $_POST['debug'] ) ? sanitize_text_field( wp_unslash( $_POST['debug'] ) ) : null,
		);
		...
		Options::update( 'debug-enabled', $debug );
	...
	private function cta_cache() {
		Options::update( 'cta-cache-200', true );
	...
	private function update_token_analytics() {
		...
		$post_data = array(
			'jwt'    => isset( $_POST['jwt'] ) ? sanitize_text_field( wp_unslash( $_POST['jwt'] ) ) : null,
			'userId' => isset( $_POST['userId'] ) ? sanitize_text_field( wp_unslash( $_POST['userId'] ) ) : null,
		);
		...
		Options::update( 'settings-analytics-token', $post_data['jwt'] );
		Options::update( 'settings-analytics-userid', $post_data['userId'] );
	...
	private function toggle_fast_cmp() {
		...
		$post_data = array(
			'enabled' => isset( $_POST['enabled'] ) ? sanitize_text_field( wp_unslash( $_POST['enabled'] ) ) : null,
		);
		...
		Options::update( 'fastcmp-enabled', $fast_cmp_enabled );
	...
	private function options_fast_cmp() {
		...
		foreach ( $options as $name => $value ) {
			Options::update( 'fastcmp-' . $name, $value );
		}
	...
	private function login_analytics() {
		...
		$response_check_api = $this->api->post(
			'api',
			'api_tokens',
			array(
				'name' => 'wordpress',
		...
		if ( is_wp_error( $response_check_api ) ) {
			Logger::add_log( 'login_analytics', 'admin/include/ajax', $response_check_api->get_error_message() );
			$error = new WP_Error( '500', 'Error while login wp_e_t' );

			wp_send_json_error( $error );
		}

		if ( ! is_array( $response_check_api ) ) {
			$error = new WP_Error( '500', 'Error while login not_a_t' );

			wp_send_json_error( $error );
		}

		if ( ! array_key_exists( 'token', $response_check_api ) ) {
			$error = new WP_Error( '500', 'Error while login arr_k_e_t' );

			wp_send_json_error( $error );
		}

		$reponse_uuid = $this->api->get(
			'api',
			'organizations',
			array(
		...
		if ( is_wp_error( $reponse_uuid ) ) {
			$error = new WP_Error( '500', 'website 1' );

			wp_send_json_error( $error );
		}

		if ( ! is_array( $reponse_uuid ) ) {
			$error = new WP_Error( '500', 'website 2' );

			wp_send_json_error( $error );
		}

		if ( count( $reponse_uuid ) === 0 ) {
			$error = new WP_Error( '500', 'website 3' );

			wp_send_json_error( $error );
		}

		if ( ! array_key_exists( 'uuid', $reponse_uuid[0] ) ) {
			$error = new WP_Error( '500', 'website 4' );

			wp_send_json_error( $error );
		}

		$uuid = $reponse_uuid[0]['uuid'];

		wp_send_json(
			array(
				'success' => true,
				'data'    => array(
					'token'    => $response_check_api['token'],
					'uuid'     => $uuid,
					'userId'   => $user_id,
					'currency' => $currency,
				),
	...
	private function register() {
		...
		$post_data = array(
			'firstname'       => isset( $_POST['firstname'] ) ? sanitize_text_field( wp_unslash( $_POST['firstname'] ) ) : null,
			'lastname'        => isset( $_POST['lastname'] ) ? sanitize_text_field( wp_unslash( $_POST['lastname'] ) ) : null,
			'email'           => isset( $_POST['email'] ) ? sanitize_text_field( wp_unslash( $_POST['email'] ) ) : null,
			'password'        => isset( $_POST['password'] ) ? sanitize_text_field( wp_unslash( $_POST['password'] ) ) : null,
			'type'            => isset( $_POST['type'] ) ? sanitize_text_field( wp_unslash( $_POST['type'] ) ) : null,
			'companyName'     => isset( $_POST['companyName'] ) ? sanitize_text_field( wp_unslash( $_POST['companyName'] ) ) : null,
			'companyWebsite'  => isset( $_POST['companyWebsite'] ) ? sanitize_text_field( wp_unslash( $_POST['companyWebsite'] ) ) : null,
			'companyLocation' => isset( $_POST['companyLocation'] ) ? sanitize_text_field( wp_unslash( $_POST['companyLocation'] ) ) : null,
		);
		...
		if ( is_wp_error( $response ) ) {
			Logger::add_log( 'Error while register wp_e', 'admin/include/ajax', 'error' );
		...
		if ( ! is_array( $response ) ) {
			Logger::add_log( 'Error while register not_a', 'admin/include/ajax', 'error' );
	...
	private function update_currency() {
		...
		/** @var string $user_id */
		$user_id = Options::get( 'settings-analytics-userid', '' );
		/** @var string $token */
		...
		if ( $user_id !== '' && $token !== '' ) {
			...
			if ( is_wp_error( $response ) ) {
				Logger::add_log( 'Error while updating currency ' . $response->get_error_code(), 'admin/include/ajax', 'error' );
		...
		Options::update( 'currency', $post_data['currency'] );
	...
	private function set_auto_update() {
		...
		$post_data = array(
			'autoUpdate' => isset( $_POST['autoUpdate'] ) ? sanitize_text_field( wp_unslash( $_POST['autoUpdate'] ) ) : null,
		);

		if ( is_null( $post_data['autoUpdate'] ) ) {
			$error = new WP_Error( '500', 'Post empty' );
