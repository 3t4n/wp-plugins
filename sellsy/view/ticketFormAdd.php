...
if (isset($_POST['add']) && $_POST['add']) {
    ...
    if (empty($_POST['ticket_form_name'])) {
        $errors[] = esc_html(__('Name', PLUGIN_NOM_LANG));
    }
    if (empty($_POST['ticket_form_subject_prefix'])) {
        $errors[] = esc_html(__('Subject prefix', PLUGIN_NOM_LANG));
    }
    ...
    // DISPLAY ERROR
        ...
        $mess = '<strong>';
        if (sizeof($errors) == 1) {
            $mess .= esc_html(__('Required field : ', PLUGIN_NOM_LANG));
        } else {
            $mess .= esc_html(__('Required fields : ', PLUGIN_NOM_LANG));
        }
...
<div class="wrap">
    ...
    <h2><?php esc_html_e('Support ticket', PLUGIN_NOM_LANG); ?></h2>
