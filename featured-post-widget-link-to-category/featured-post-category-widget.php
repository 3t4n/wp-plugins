<?php
/*
Plugin Name: Featured Post Widget - Link to Category
Plugin URI: http://wordpress.org/extend/plugins/genesis-featured-post-widget-link-to-category
Description: This is a modification to the Genesis Featured Post Plugin. It enables an override
			 that links to the category instead of to the article.
Author: Brian Myrick
Version: 1.0
...
/**
 * Adds the Featured Post Widget. Allows an override option to link to the category page
 ...
 *
 * This work is a derived work originally created by StudioPress (www.studiopress.com)
 * This modification is released on May 1, 2013
 */

/**
 * Genesis Featured Post Category widget class.
 *
 * @category Genesis
 * @package Widgets
 *
...
add_action( 'widgets_init', 'enable_featured_post_link_to_category_widget' );
function enable_featured_post_link_to_category_widget () {
	register_widget( 'Genesis_Featured_Post_Category' );
	unregister_widget ('Genesis_Featured_Post');
}

class Genesis_Featured_Post_Category extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			'classname'   => 'featuredpostcategory',
			'description' => __( 'Displays featured posts with thumbnails and allows linking to the category page instead of the article.', 'genesis' ),
		);

		$control_ops = array(
			'id_base' => 'featured-post-category',
			'width'   => 505,
			'height'  => 350,
		);

		$this->WP_Widget( 'featured-post-category', __( 'Genesis - Featured Posts | Category', 'genesis' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( $featured_posts->have_posts() ) : while ( $featured_posts->have_posts() ) : $featured_posts->the_post();
			...
			if ( ! empty( $instance['link_to_category'] ) ) {
				$category = get_the_category();
				$linkedto = get_category_link($category[0]->term_id);
			} else {
				$linkedto = get_permalink();
			}

			if ( ! empty( $instance['show_image'] ) && $image = genesis_get_image( array( 'format' => 'html', 'size' => $instance['image_size'] ) ) )
				printf( '<a href="%s" title="%s" class="%s">%s</a>', $linkedto, the_title_attribute( 'echo=0' ), esc_attr( $instance['image_alignment'] ), $image );
			...
			if ( ! empty( $instance['show_title'] ) )
				printf( '<h2><a href="%s" title="%s">%s</a></h2>', $linkedto, the_title_attribute( 'echo=0' ), get_the_title() );
	...
	function form( $instance ) {
		...
		<div class="genesis-widget-column genesis-widget-column-right">
			...
			<div class="genesis-widget-column-box">
				...
				<p><?php _e( 'Normally, the link is to the article. This option allows you to override that and link to the category instead.', 'genesis' ); ?>:</p>
				
				<p>
					<input id="<?php echo $this->get_field_id( 'link_to_category' ); ?>" type="checkbox" name="<?php echo $this->get_field_name( 'link_to_category' ); ?>" value="1" <?php checked( $instance['link_to_category']);?>
					<label for="<?php echo $this->get_field_id( 'link_to_category' ); ?>"><?php _e( ' Link To Category', 'genesis' ); ?></label>
				</p>
