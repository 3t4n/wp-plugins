...
<a href="<?php echo esc_url($helpers->get_tracking_url($order_data)); ?>" target="_blank">
	...
	<img src="<?php echo esc_url($order_data->screenshot->url); ?>" alt="<?php echo esc_attr__('Order Screenshot', 'alopeyk-shipping'); ?>" class="awcshm-order-screenshot">
</a>
<?php
	}
?>
<ul class="order_actions">
	<li class="wide awcshm-meta-box-content-container">
		<p>
			<span><?php echo esc_html__( 'Status', 'alopeyk-shipping' ); ?>: </span>
		...
		<p>
			<span><?php echo esc_html__( 'Invoice Number', 'alopeyk-shipping' ); ?>: </span>
			<strong><?php echo esc_html($order_data->invoice_number); ?></strong>
		</p>
		<?php } ?>
		<?php if ( isset( $order_data->id ) ) { ?>
		<p>
			<span><?php echo esc_html__( 'ID', 'alopeyk-shipping' ); ?>: </span>
			<strong><?php echo esc_html($order_data->id); ?></strong>
		</p>
		<?php } ?>
		<?php if ( isset( $order_data->price ) ) { ?>
		<p>
			<span><?php echo esc_html__( 'Cost', 'alopeyk-shipping' ); ?>: </span>
		...
		<?php if ( isset( $order_data->order_discount ) && $order_data->order_discount ) { ?>
				<p>
					<span><?php echo esc_html__( 'Discount Code Value', 'alopeyk-shipping' ); ?>: </span>
		...
		<p>
			<span><?php echo esc_html__( 'Transport Type', 'alopeyk-shipping' ); ?>: </span>
			<strong><?php echo esc_html($order_data->transport_type_name); ?></strong>
		</p>
		<?php } ?>
		<p>
			<span><?php echo esc_html__( 'Shop Order(s)', 'alopeyk-shipping' ); ?>: </span>
			<?php
		...
		<p>
			<span><?php echo esc_html__( 'Customer(s)', 'alopeyk-shipping' ); ?>: </span>
			<?php
	...
	<li class="wide awcshm-meta-box-actions-container">
		...
		?>
		<a class="button" target="_blank" href="<?php echo esc_url($helpers->get_invoice_url($order_data)); ?>"><?php echo esc_html__('Invoice', 'alopeyk-shipping'); ?></a>
		<?php
			}
			if ( $helpers->can_be_tracked( $order_data ) ) {
		?>
		<a class="button" target="_blank" href="<?php echo esc_url($helpers->get_tracking_url($order_data)); ?>"><?php echo esc_html__('Track', 'alopeyk-shipping'); ?></a>
		<?php
			}
		...
		?>
		<button type="button" class="button button-primary awcshm-cancel-modal-toggler" data-order-id="<?php echo esc_attr($post->ID); ?>"><?php echo esc_html__('Cancel Order', 'alopeyk-shipping'); ?></button>
		<?php
				} else {
		?>
	<!--This image is of the main WordPress files located in the wp-includes folder.-->
		...
		?>
		<button type="button" class="button button-primary awcshm-order-modal-toggler" data-order-ids="<?php echo esc_attr(implode(',', $order_ids)); ?>" data-order-types="<?php echo esc_attr($order_data->transport_type); ?>"><?php echo esc_html__('Ship Again', 'alopeyk-shipping'); ?></button>
		<?php
			}
		?>
	</li>
