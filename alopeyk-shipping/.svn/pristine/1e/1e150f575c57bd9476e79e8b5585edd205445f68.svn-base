...
<form class="awcshm-create-order-form">
	<table cellpadding="0" cellspacing="0" border="0" class="fixed striped awcshm-horizonal-table">
		<tbody>
			<tr>
				<th valign="middle"><?php echo esc_html__( 'Transport Type', 'alopeyk-shipping' ); ?></th>
				<td valign="middle">
					<select name="type">
						<?php
			...
			<tr>
				<th valign="top"><?php echo esc_html__( 'Shipping Time', 'alopeyk-shipping' ); ?></th>
				<td valign="top">
					<input id="ship_now_true" type="radio" name="ship_now" value="true" checked="checked"><label for="ship_now_true"><?php echo esc_html__( 'Now', 'alopeyk-shipping' ); ?></label>&nbsp;
					<input id="ship_now_false" type="radio" name="ship_now" value="false" class="awcshm-shipping-time-filter-trigger"><label for="ship_now_false"><?php echo esc_html__( 'Later', 'alopeyk-shipping' ); ?></label>
			...
			<tr>
				<th valign="top"><?php echo esc_html__( 'Origin Description', 'alopeyk-shipping' ); ?></th>
				<td valign="top">
					<textarea name="description" rows="5" style="width: 100%;"><?php echo esc_textarea(isset($data['description']) ? $data['description'] : ''); ?></textarea>
					<span class="awcshm-meta"><?php echo esc_html__( 'This will be shown on courier device and usually consists of order value, address details or any other sort of data needed for courier to know about the origin address or the whole order.', 'alopeyk-shipping' ); ?></span>
				</td>
			</tr>
			<?php
			...
			<tr>
				...
				<td valign="top">
					<?php
						if ( $count == 0 ) {
							echo esc_html__( 'No order selected for shipping.', 'alopeyk-shipping' );
						} else if ( $count == 1 ) {
							$order_id = $orders[0];
					?>
