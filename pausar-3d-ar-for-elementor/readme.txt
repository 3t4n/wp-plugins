...
* **Easy customization** - 3D and AR content can be quickly added and customized, similar to other Elementor widgets. More complex settings are also possible using the extensive widget menu.
...
= Known Issues =
* Problems when using the plugin **WP-Optimize** with JavaScript minification enabled:<br><br>The popular plugin **WP-Optimize** can compress JavaScript code to shorten loading times, but this makes PausAR's code unusable.<br><br>If you want to use WP-Optimize with PausAR, you must add the PausAR plugin file path as an exception within the WP-Optimize settings. To do this, add the following file paths:
...
* Bugfixes
    * Fixed copy function in pop-up window for devices with iOS 17 or later. "Copy to clipboard" no longer worked in some browsers (e.g. LinkedIn browser)
    * Fixed touchscreen detection required to display the "Start AR" popup
    * Fixed bug where autostart would be aborted if multiple PausAR widgets were scrolled past too quickly
* Improvements
    * Fullscreen mode overhaul for devices that do not support the general fullscreen API
    * Improved error detection when specifying file paths (custom URL). Mismatches of "http://" and "https://" are now corrected for AR mode as well.
