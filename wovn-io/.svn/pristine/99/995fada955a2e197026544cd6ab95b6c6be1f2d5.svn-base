...
class HtmlConverter
{
    ...
    private function insert_hreflang_tags($html)
    {
        ...
        if (isset($this->options['wovn_hreflang_x_default_lang']) && !preg_match($x_default_hreflang_regex, $html)) {
            $x_default_lang = $this->options['wovn_hreflang_x_default_lang'];
            $href = htmlentities(Url::add_lang_code($this->accessed_url, $x_default_lang, $this->options));
