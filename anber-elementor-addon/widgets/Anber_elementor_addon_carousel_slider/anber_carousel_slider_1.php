...
$anber_car_item_gap = esc_js($anber_car_item_gap);
$anber_car_button = esc_js($anber_car_button);
$anber_next_icon_html = wp_json_encode($anber_next_icon_html);
$anber_previous_icon_html = wp_json_encode($anber_previous_icon_html);
    ...
    jQuery(document).ready(function ($) {
        setTimeout(function () {
            $('#anber_ea_carousel').owlCarousel({
                loop: true,
                margin: {$anber_car_item_gap},
                dots: " . ($anber_car_button == 'dots' || $anber_car_button == 'both' ? 'true' : 'false') . ",
                nav: " . ($anber_car_button == 'nav' || $anber_car_button == 'both' ? 'true' : 'false') . ",
                navText: [{$anber_next_icon_html}, {$anber_previous_icon_html}],
                autoplay: true,
                autoplayHoverPause: true,
                responsive: {
                    0: {
                        items: {$anber_carousel_items['mobile']}
                    },
                    600: {
                        items: {$anber_carousel_items['tablet']}
                    },
                    1000: {
                        items: {$anber_carousel_items['desktop']}
                    }
                }
            });
...
if (\Elementor\Plugin::instance()->editor->is_edit_mode()) {
    echo wp_get_inline_script_tag($anber_inline_script);
} else {
    wp_add_inline_script('anber-owl-carousel-script', $anber_inline_script);
}
?>
