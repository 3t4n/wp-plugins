...
class Wpcf7 {
	...
	function make_form_newletters( $form_id, $field_name ) {
		...
		// prepare table
		$args  = [ 
			'table_name' => $this->newletters_table_name,
			'menu_title' => $this->newletters_table_label,
			'fields'     => [ 
			...
			$panels['thankyou_panel'] = array(
				...
				<p>
					<label for="enable-thankyou">
						<input type="checkbox" id="enable-thankyou" name="_enable_thankyou" value="1" <?php checked( $enable_thankyou, 1 ); ?> />
				...
				<p>
					...
					<select id="thankyou-style" name="_thankyou_style">
						<option value="alert_content" <?php selected( $thankyou_style, 'alert_content' ); ?>>Alert content</option>
						<option value="toggle_content" <?php selected( $thankyou_style, 'toggle_content' ); ?>>Toggle content</option>
					</select>
				</p>
				<p>
					<label for="thankyou-content">Thankyou Content:</label>
					<textarea id="thankyou-content" name="_thankyou_content" rows="4" style="width: 100%;"
						placeholder="Html or shortcode"><?php echo esc_textarea( $thankyou_content ); ?></textarea>
				</p>
				<?php
				},
			);
			...
			<script type="text/javascript">
				...
				document.addEventListener('wpcf7mailsent', function (event) {
					...
					if (data) {
						...
						} else if (data.style === 'toggle_content') {
