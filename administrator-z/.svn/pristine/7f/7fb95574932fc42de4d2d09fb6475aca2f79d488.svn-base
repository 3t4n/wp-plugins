...
final class Woocommerce {
	...
	function plugin_loaded() {
		...
		if ( ( $this->settings['order_notification_telegram'] ?? '' ) == 'on' ) {
			global $adminz;
			if (
				$telegram_botToken = $adminz['Api']->settings['telegram_botToken'] and
				$telegram_chatId = $adminz['Api']->settings['telegram_chatId']
			) {
				new \Adminz\Helper\WooTeleGram( $telegram_botToken, $telegram_chatId );
			}
		}
	...
	function register_settings() {
		...
		// // add section
		// add_settings_section(
		// 	'adminz_woocommerce_payment_methods',
		// 	'Payment methods',
		// 	function () {},
		// 	$this->id
		// );
		...
		add_settings_field(
			...
			function () {
				...
				echo adminz_field( [ 
					...
					'note'      => 'Go to tab <strong>' . adminz_tab_link( 'Api' ) . '</strong>'
				] );
			},
			$this->id,
			'adminz_woocommerce_order'
		...
		add_settings_field(
			...
			function () {
				...
				echo adminz_field( [ 
					...
					'note'      => 'Change current currency unit',
				] );
