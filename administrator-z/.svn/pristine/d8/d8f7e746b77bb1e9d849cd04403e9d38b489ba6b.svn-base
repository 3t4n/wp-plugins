...
class Crawl {
	...
	function __construct( $post ) {
		if ( defined( 'ADMINZ_CRAWL_SKIP_CHECK_EXISTS' ) and ADMINZ_CRAWL_SKIP_CHECK_EXISTS ) {
			$this->skip_check_exist = true;
		}

		// setup
		$this->config = wp_parse_args(
			$post['adminz_tools'] ?? [],
			get_option( 'adminz_tools' )
		);
	...
	function get_product_data( $html = false ) {
		...
		if ( !is_null( $product_category ) ) {
			$return['product_cat'] = [];
			foreach ( (array) $product_category as $element ) {
				$nodes = $element->childNodes ?? [];
	...
	function get_image_src( $image ) {
		// Kiểm tra xem phần tử có thuộc tính data-src không
		if ( $image->hasAttribute( 'data-src' ) ) {
			// Nếu có, trả về giá trị của data-src
			return $image->getAttribute( 'data-src' );
		}

		// Nếu không có, trả về giá trị của src
	...
	function fix_href( $href ) {
		...
		if ( strpos( $href, '//' ) === 0 ) {
			return $parsed_base_url['scheme'] . ':' . $href;
		}
		...
		return $parsed_base_url['scheme'] . '://' . $parsed_base_url['host'] . '/' . ltrim( $href, '/' );
	}
