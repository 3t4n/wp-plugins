...
function adminz_get_filtered_price() {
	...
	//
	$post_types = apply_filters( 'woocommerce_price_filter_post_type', array( 'product' ) );
	$post_types_escaped = array_map( 'esc_sql', $post_types );
	$post_type_in = implode( "','", $post_types_escaped );
	$post_status = 'publish';
	$join_sql  = $tax_query_sql['join'] . $meta_query_sql['join'];
	$where_sql = $tax_query_sql['where'] . $meta_query_sql['where'] . $search_query_sql;
	$sql = $wpdb->prepare(
		"
