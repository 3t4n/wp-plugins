...
class TaxonomySync {
	...
	function run() {
        ...
        /*
        Có thể dùng action 'save_post' khi muốn priority muộn hơn để check một số thông tin khác.
        get_post_meta
        get_post_status
        */
    ...
    function add_term_admin_column($columns){
		...
		foreach ( $columns as $key => $value ) {
			if ( $key == 'name' ) {
				// phpcs:ignore
				$new_columns[$this->admin_column_key] = sprintf(__("SyncZ %s"), $this->post_type_name);
			}
