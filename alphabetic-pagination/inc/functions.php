<?php if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly
	...
	if(!function_exists('alphabets_bar')){
                    function alphabets_bar(){                                			
											...
											$alphabetz_bar .= '<a href="' . esc_url( add_query_arg( array( sanitize_key($arg) => 'numeric' ), esc_url($url_x) ) ) . '" class="' . esc_attr( strtolower($ap) == 'numeric' ? 'selected' : '' ) . '">#</a>';
                                            ...
                                            foreach($alphabetz as $num=>$alphabet){
													...
													if($alpha_jump==0){					
														...
														$alphabetz_bar .= '<a href="' . esc_url( add_query_arg( array( sanitize_key( $arg ) => sanitize_text_field( $alphabet ) ), esc_url( $url_x ) ) ) . '" class="' . esc_attr( strtolower( $ap ) == strtolower( $alphabet ) ? 'selected' : '' ) . '">' . esc_html( $alphabet ) . '</a>';
														...
														if($alpha_jump_count<=$alpha_jump){
															...
															if($alpha_jump_count==$alpha_jump || $alpha_count==count($alphabetz)){
																...
																$alphabetz_bar .= '<a href="' . esc_url( add_query_arg( array( sanitize_key($arg) => sanitize_text_field($alphabet_arg) ), esc_url($url_x) ) ) . '" class="' . esc_attr( strtolower($ap) == strtolower($alphabet_arg) ? 'selected' : '' ) . '">' . esc_html( $alphabet_arg ) . '</a>';
