        ...
        <h4 class="accordion-header" id="heading-<?php echo esc_html($key_cats) ?>" data-category-name='<?php echo esc_html($hierarchy . $category->name . $category->term_id) ?>' data-category-slug='<?php echo esc_html($category->slug) ?>'>
          ...
          <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-<?php echo esc_html($key_cats) ?>" aria-expanded="true" aria-controls="collapse-<?php echo esc_html($key_cats) ?>">
            ...
            <span class="<?php echo esc_html($category->slug); ?>">
              ...
              <small class="cat-id"><?php echo __('ID:', 'ts-wchc') ?> <?php echo esc_html($category->term_id); ?></small>
              <small id="counter-<?php echo esc_html($key_cats) ?>" class="rules-counter">
                <?php if ($has_rules):
                  /* translators: %d is the count of hidden categories */
                  printf(
                    esc_html(_n('Hidden for %d Role', 'Hidden for %d Roles', $has_rules, 'ts-wchc')),
                    esc_html($has_rules)
                  );
                endif; ?>
              </small>
        ...
        <div id="collapse-<?php echo esc_html($key_cats) ?>" class="accordion-collapse collapse" aria-labelledby="heading-<?php echo esc_html($key_cats) ?>">
          ...
          <div class="accordion-body">
            ...
            <div class="actions-bar">
              ...
              <h6><?php echo __('Hide For', 'ts-wchc'); ?></h6>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                  ...
                  <ul class="navbar-nav mr-auto">
                    <li class="nav-item active">
                      <button type="button" class="btn tswchc-clear-all btn-clear btn-sm">
                        <?php echo __('Clear All', 'ts-wchc'); ?>
                      </button>
                    </li>
                    <li class="nav-item">
                      <button type="button" class="btn check-all btn-warning btn-sm">
                        <?php echo __('Check All', 'ts-wchc'); ?>
                      </button>
                    </li>
                  </ul>
            ...
            <div id="btn-group-<?php echo esc_html($category->slug); ?>" class="btn-group btn-group-xs" data-counter="counter-<?php echo esc_html($key_cats) ?>" role="group-<?php echo esc_html($key_cats) ?>" aria-label="<?php echo __('Roles Available on Site', 'ts-wchc'); ?>">
